# Issue 25189: Upgrade cysignals to version 1.7.1

archive/issues_024952.json:
```json
{
    "assignees": [],
    "body": "Unfortunately, cysignals-1.7.0 is broken on ppc64le GNU/Linux.\n\n**Tarball**: https://files.pythonhosted.org/packages/81/64/0959438193254c2c6ad127fb5f9aad4140bc4288d752d03b1976ef8366e0/cysignals-1.7.1.tar.gz\n\nBranch/Commit: **[4ed886b](https://github.com/sagemath/sagetrac-mirror/commit/4ed886b8dab273279e49e6ca818c96df8f4fc8c8)**\n\nReviewer: **Vincent Klein**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25189_\n\n",
    "closed_at": "2018-05-08T17:27:59Z",
    "created_at": "2018-04-17T10:36:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Upgrade cysignals to version 1.7.1",
    "type": "issue",
    "updated_at": "2018-05-08T17:27:59Z",
    "url": "https://github.com/sagemath/sage/issues/25189",
    "user": "https://github.com/jdemeyer"
}
```
Unfortunately, cysignals-1.7.0 is broken on ppc64le GNU/Linux.

**Tarball**: https://files.pythonhosted.org/packages/81/64/0959438193254c2c6ad127fb5f9aad4140bc4288d752d03b1976ef8366e0/cysignals-1.7.1.tar.gz

Branch/Commit: **[4ed886b](https://github.com/sagemath/sagetrac-mirror/commit/4ed886b8dab273279e49e6ca818c96df8f4fc8c8)**

Reviewer: **Vincent Klein**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/25189_





---

archive/issue_events_330598.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-17T10:36:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330598"
}
```



---

archive/issue_events_330599.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-17T10:36:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330599"
}
```



---

archive/issue_events_330600.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-17T10:36:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330600"
}
```



---

archive/issue_events_330601.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-17T10:36:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330601"
}
```



---

archive/issue_comments_387813.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Unfortunately, the latest cysignals is broken on ppc64le.\n+Unfortunately, the latest cysignals is broken on ppc64le GNU/Linux.\n``````\n",
    "created_at": "2018-04-17T10:37:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387813",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Unfortunately, the latest cysignals is broken on ppc64le.
+Unfortunately, the latest cysignals is broken on ppc64le GNU/Linux.
``````




---

archive/issue_comments_387814.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n Unfortunately, the latest cysignals is broken on ppc64le GNU/Linux.\n+\n+**Tarball**: https://files.pythonhosted.org/packages/81/64/0959438193254c2c6ad127fb5f9aad4140bc4288d752d03b1976ef8366e0/cysignals-1.7.1.tar.gz\n``````\n",
    "created_at": "2018-04-17T12:22:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387814",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 Unfortunately, the latest cysignals is broken on ppc64le GNU/Linux.
+
+**Tarball**: https://files.pythonhosted.org/packages/81/64/0959438193254c2c6ad127fb5f9aad4140bc4288d752d03b1976ef8366e0/cysignals-1.7.1.tar.gz
``````




---

archive/issue_comments_387815.json:
```json
{
    "body": "Branch: **[u/jdemeyer/upgrade_cysignals_to_version_1_7_1](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/upgrade_cysignals_to_version_1_7_1)**",
    "created_at": "2018-04-17T12:24:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387815",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/upgrade_cysignals_to_version_1_7_1](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/upgrade_cysignals_to_version_1_7_1)**



---

archive/issue_comments_387816.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4ed886b8dab273279e49e6ca818c96df8f4fc8c8\">4ed886b</a></td><td><code>Upgrade cysignals to version 1.7.1</code></td></tr></table>\n",
    "created_at": "2018-04-17T12:36:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387816",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4ed886b8dab273279e49e6ca818c96df8f4fc8c8">4ed886b</a></td><td><code>Upgrade cysignals to version 1.7.1</code></td></tr></table>




---

archive/issue_comments_387817.json:
```json
{
    "body": "Commit: **[4ed886b](https://github.com/sagemath/sagetrac-mirror/commit/4ed886b8dab273279e49e6ca818c96df8f4fc8c8)**",
    "created_at": "2018-04-17T12:36:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387817",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[4ed886b](https://github.com/sagemath/sagetrac-mirror/commit/4ed886b8dab273279e49e6ca818c96df8f4fc8c8)**



---

archive/issue_events_330602.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-17T12:36:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330602"
}
```



---

archive/issue_comments_387818.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n-Unfortunately, the latest cysignals is broken on ppc64le GNU/Linux.\n+Unfortunately, cysignals-1.7.0 is broken on ppc64le GNU/Linux.\n \n **Tarball**: https://files.pythonhosted.org/packages/81/64/0959438193254c2c6ad127fb5f9aad4140bc4288d752d03b1976ef8366e0/cysignals-1.7.1.tar.gz\n``````\n",
    "created_at": "2018-04-17T12:37:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387818",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
-Unfortunately, the latest cysignals is broken on ppc64le GNU/Linux.
+Unfortunately, cysignals-1.7.0 is broken on ppc64le GNU/Linux.
 
 **Tarball**: https://files.pythonhosted.org/packages/81/64/0959438193254c2c6ad127fb5f9aad4140bc4288d752d03b1976ef8366e0/cysignals-1.7.1.tar.gz
``````




---

archive/issue_events_330603.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-04-17T20:33:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330603"
}
```



---

archive/issue_events_330604.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-04-17T20:33:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330604"
}
```



---

archive/issue_comments_387819.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThats not a supported platform -> next beta cycle",
    "created_at": "2018-04-17T20:33:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387819",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thats not a supported platform -> next beta cycle



---

archive/issue_events_330605.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-17T20:47:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330605"
}
```



---

archive/issue_events_330606.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-17T20:47:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330606"
}
```



---

archive/issue_comments_387820.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@vbraun](#comment%3A6):\n> Thats not a supported platform\n\nWhat's your definition of \"supported platform\" and why does this system not satisfy?\n\nPrevious versions of Sage worked perfectly with 100% doctests passing and I actually use this machine for serious stuff (i.e. it's not a museum machine that I only use to annoy release managers with bug reports).\n\nPS: I know that I've been messing up with cysignals breakage in this release cycle, but there is always some strange OS quirk that comes up.",
    "created_at": "2018-04-17T20:47:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387820",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@vbraun](#comment%3A6):
> Thats not a supported platform

What's your definition of "supported platform" and why does this system not satisfy?

Previous versions of Sage worked perfectly with 100% doctests passing and I actually use this machine for serious stuff (i.e. it's not a museum machine that I only use to annoy release managers with bug reports).

PS: I know that I've been messing up with cysignals breakage in this release cycle, but there is always some strange OS quirk that comes up.



---

archive/issue_comments_387821.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nImho supported platform requires having a buildbot\n\nThe 2 users of Sage on that arch can just follow the develop branch.\n\nWe've seen how easy it is to break something by mucking with signal handlers in this release cycle, so I'm against last minute changes if they can be avoided",
    "created_at": "2018-04-17T22:45:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387821",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:8" align="right">Comment 8</div>

Imho supported platform requires having a buildbot

The 2 users of Sage on that arch can just follow the develop branch.

We've seen how easy it is to break something by mucking with signal handlers in this release cycle, so I'm against last minute changes if they can be avoided



---

archive/issue_comments_387822.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nReplying to [@vbraun](#comment%3A8):\n> Imho supported platform requires having a buildbot\n\nThat just means that we *should* put a buildbot on that machine. For a while now, I have a patchbot running there.\n\n> The 2 users of Sage on that arch can just follow the develop branch.\n\nCan't argue with that I guess.\n\n> We've seen how easy it is to break something by mucking with signal handlers in this release cycle, so I'm against last minute changes if they can be avoided \n\nIf it helps, there are no actual changes to signal handling, only to threading :-)",
    "created_at": "2018-04-18T07:28:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387822",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:9" align="right">Comment 9</div>

Replying to [@vbraun](#comment%3A8):
> Imho supported platform requires having a buildbot

That just means that we *should* put a buildbot on that machine. For a while now, I have a patchbot running there.

> The 2 users of Sage on that arch can just follow the develop branch.

Can't argue with that I guess.

> We've seen how easy it is to break something by mucking with signal handlers in this release cycle, so I'm against last minute changes if they can be avoided 

If it helps, there are no actual changes to signal handling, only to threading :-)



---

archive/issue_events_330607.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-04-18T23:29:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330607"
}
```



---

archive/issue_events_330608.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-04-18T23:29:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330608"
}
```



---

archive/issue_comments_387823.json:
```json
{
    "body": "Reviewer: **Vincent Klein**",
    "created_at": "2018-04-20T12:38:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387823",
    "user": "https://github.com/vinklein"
}
```

Reviewer: **Vincent Klein**



---

archive/issue_comments_387824.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nAll tests run for me. Let me know if you see more specific test to do.",
    "created_at": "2018-04-20T12:38:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387824",
    "user": "https://github.com/vinklein"
}
```

<div id="comment:11" align="right">Comment 11</div>

All tests run for me. Let me know if you see more specific test to do.



---

archive/issue_events_330609.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-23T06:59:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330609"
}
```



---

archive/issue_events_330610.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-23T06:59:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330610"
}
```



---

archive/issue_events_330611.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-30T07:23:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330611"
}
```



---

archive/issue_events_330612.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-30T07:23:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330612"
}
```



---

archive/issue_events_330613.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-30T07:23:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330613"
}
```



---

archive/issue_events_330614.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-30T07:23:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330614"
}
```



---

archive/issue_comments_387825.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\n@vbraun: can we at least make this a 8.3 blocker then, in the sense that it should be merged with priority in the 8.3 branch?",
    "created_at": "2018-04-30T07:23:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387825",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:13" align="right">Comment 13</div>

@vbraun: can we at least make this a 8.3 blocker then, in the sense that it should be merged with priority in the 8.3 branch?



---

archive/issue_comments_387826.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/upgrade_cysignals_to_version_1_7_1](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/upgrade_cysignals_to_version_1_7_1)** to **[4ed886b](https://github.com/sagemath/sagetrac-mirror/commit/4ed886b8dab273279e49e6ca818c96df8f4fc8c8)**",
    "created_at": "2018-05-08T17:27:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25189#issuecomment-387826",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/upgrade_cysignals_to_version_1_7_1](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/upgrade_cysignals_to_version_1_7_1)** to **[4ed886b](https://github.com/sagemath/sagetrac-mirror/commit/4ed886b8dab273279e49e6ca818c96df8f4fc8c8)**



---

archive/issue_events_330615.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-08T17:27:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330615"
}
```



---

archive/issue_events_330616.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5edef8b2fddd0ba3d8983e8e0b89257af9666dca",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-08T17:27:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25189",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25189#event-330616"
}
```
