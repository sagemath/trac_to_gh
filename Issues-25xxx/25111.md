...
```json
{
    "body": "I actually do this in sage-on-gentoo, but I do it at the packaging level rather than the building level.",
...
```json
{
    "body": "This saves me almost 400 MB, by the way. (\"This\" = `replace_with_master_directory(os.path.join(SAGE_LOCAL, 'share', 'doc', 'sage', 'html'))`.)",
...
```json
{
    "body": "See possibly related discussion at #25089.",
...
```json
{
    "body": "I'm confused by this ticket, because it already does that, per #25089...",
...
```json
{
    "body": "I see the difference--it does already do this within the `en/reference` docs, where each \"reference\" section is treated as a sub-document of the reference \"master document\", and in that case the `_static` directories get symlinked up to the master document.  My assumption was that all of the Sage docs (including \"reference\") were in turn treated as sub-documents of a higher-level master document but apparently that's not the case.\n\nIMO treating the entire tree of Sage docs as such a hierarchy with shared static resources would be the best approach.",
...
```json
{
    "body": "Some parts of the documentation tree have slightly different `_static` directories, which is where the approach in comment 2 comes from: compare each `_static` directory to the top-level one, replacing files (and directories) with symlinks when possible.",
...
```json
{
    "body": "Couldn't a different `_static/translations.js` be used per language?  That is, somehow namespace that file by the language in the first place.  That or at least have an alternate location for it. `html_static_path` can be a list.",
...
```json
{
    "body": "I don't think that `html_static_path` will help: it provides a list from which the output `_static` directories are produced \u2013 it does not provide a list of directories to use instead of the output `_static` directories. In fact, we already set `html_static_path` in `src/doc/common/conf.py`.",
...
```json
{
    "body": "Too bad that I missed this ticket earlier. This should have been an 8.3 blocker.",
...
```json
{
    "body": "I see now your sage-devel post where you reported the same.",
...
```json
{
    "body": "It seems to be more subtle: sometimes the symlinks are correctly generated and sometimes not.",
...
```json
{
    "body": "I created a new ticket #26152 specifically for the mathjax symlink issue.",
...
```json
{
    "body": "In a recently built copy of the Sage documentation, there seem to be 27 copies of MathJax installed in various `_static` directories, which translates into about 430 MB of disk space on my computer.",
