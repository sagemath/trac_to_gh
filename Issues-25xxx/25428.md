# Issue 25428: Faster without_bounds()

archive/issues_025191.json:
```json
{
    "assignees": [],
    "body": "This patch makes `without_bounds()` of a poset much faster.\n\n\nCC:  @tscrim @fchapoton\n\nBranch/Commit: **[bf81f8b](https://github.com/sagemath/sagetrac-mirror/commit/bf81f8b6d05da5f42478f977621a9a2c9de7b895)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25428_\n\n",
    "closed_at": "2018-05-27T17:01:33Z",
    "created_at": "2018-05-23T05:36:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Faster without_bounds()",
    "type": "issue",
    "updated_at": "2018-05-27T17:01:33Z",
    "url": "https://github.com/sagemath/sage/issues/25428",
    "user": "https://github.com/jm58660"
}
```
This patch makes `without_bounds()` of a poset much faster.


CC:  @tscrim @fchapoton

Branch/Commit: **[bf81f8b](https://github.com/sagemath/sagetrac-mirror/commit/bf81f8b6d05da5f42478f977621a9a2c9de7b895)**

Reviewer: **Travis Scrimshaw**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/25428_





---

archive/issue_events_333812.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-23T05:36:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25428#event-333812"
}
```



---

archive/issue_events_333813.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-23T05:36:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25428#event-333813"
}
```



---

archive/issue_events_333814.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-23T05:36:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25428#event-333814"
}
```



---

archive/issue_events_333815.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-23T05:36:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25428#event-333815"
}
```



---

archive/issue_comments_392020.json:
```json
{
    "body": "Branch: **[u/jmantysalo/faster-without_bounds](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/faster-without_bounds)**",
    "created_at": "2018-05-23T05:37:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25428#issuecomment-392020",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/faster-without_bounds](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/faster-without_bounds)**



---

archive/issue_events_333816.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-23T05:37:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25428#event-333816"
}
```



---

archive/issue_comments_392021.json:
```json
{
    "body": "Commit: **[bf81f8b](https://github.com/sagemath/sagetrac-mirror/commit/bf81f8b6d05da5f42478f977621a9a2c9de7b895)**",
    "created_at": "2018-05-23T05:37:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25428#issuecomment-392021",
    "user": "https://github.com/jm58660"
}
```

Commit: **[bf81f8b](https://github.com/sagemath/sagetrac-mirror/commit/bf81f8b6d05da5f42478f977621a9a2c9de7b895)**



---

archive/issue_comments_392022.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bf81f8b6d05da5f42478f977621a9a2c9de7b895\">bf81f8b</a></td><td><code>Faster code.</code></td></tr></table>\n",
    "created_at": "2018-05-23T05:37:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25428#issuecomment-392022",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bf81f8b6d05da5f42478f977621a9a2c9de7b895">bf81f8b</a></td><td><code>Faster code.</code></td></tr></table>




---

archive/issue_comments_392023.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-05-23T05:52:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25428#issuecomment-392023",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_392024.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI agree with the general idea to avoid `subposet` and instead just manipulate the Hasse diagram/digraph.",
    "created_at": "2018-05-23T05:52:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25428#issuecomment-392024",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

I agree with the general idea to avoid `subposet` and instead just manipulate the Hasse diagram/digraph.



---

archive/issue_events_333817.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-05-23T05:52:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25428#event-333817"
}
```



---

archive/issue_events_333818.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-05-23T05:52:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25428#event-333818"
}
```



---

archive/issue_comments_392025.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/faster-without_bounds](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/faster-without_bounds)** to **[bf81f8b](https://github.com/sagemath/sagetrac-mirror/commit/bf81f8b6d05da5f42478f977621a9a2c9de7b895)**",
    "created_at": "2018-05-27T17:01:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25428#issuecomment-392025",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/faster-without_bounds](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/faster-without_bounds)** to **[bf81f8b](https://github.com/sagemath/sagetrac-mirror/commit/bf81f8b6d05da5f42478f977621a9a2c9de7b895)**



---

archive/issue_events_333819.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-27T17:01:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25428#event-333819"
}
```



---

archive/issue_events_333820.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "aceee6e4b7668b2da45ebbee9f9cd504672e92d7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-27T17:01:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25428",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25428#event-333820"
}
```
