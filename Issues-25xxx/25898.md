# Issue 25898: Enable setting attributes on a lazy imported object

archive/issues_025661.json:
```json
{
    "assignees": [],
    "body": "Scenario:\n\n```\n   from sage.all import Tableau # just one of the lazy imported objects in Sage\n   Tableau.foo = 3\n\n    sage: Tableau.foo = 1\n    ...\n    AttributeError: 'sage.misc.lazy_import.LazyImport' object has no attribute 'foo'\n```\n\nDo you foresee any problem with just implementing `__setattr__` similarly to the current `__getattr__`?\n\nCC:  @jdemeyer @embray\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25898_\n\n",
    "created_at": "2018-07-22T15:07:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Enable setting attributes on a lazy imported object",
    "type": "issue",
    "updated_at": "2022-12-29T01:42:26Z",
    "url": "https://github.com/sagemath/sage/issues/25898",
    "user": "https://github.com/nthiery"
}
```
Scenario:

```
   from sage.all import Tableau # just one of the lazy imported objects in Sage
   Tableau.foo = 3

    sage: Tableau.foo = 1
    ...
    AttributeError: 'sage.misc.lazy_import.LazyImport' object has no attribute 'foo'
```

Do you foresee any problem with just implementing `__setattr__` similarly to the current `__getattr__`?

CC:  @jdemeyer @embray

_Issue created by migration from https://trac.sagemath.org/ticket/25898_





---

archive/issue_events_339850.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2018-07-22T15:07:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25898",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25898#event-339850"
}
```



---

archive/issue_events_339851.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2018-07-22T15:07:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25898",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25898#event-339851"
}
```



---

archive/issue_events_339852.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2018-07-22T15:07:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25898",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25898#event-339852"
}
```



---

archive/issue_comments_400929.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nIt don't see any obvious problems. Don't forget `__delattr__` while you're at it.",
    "created_at": "2018-07-22T15:36:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25898#issuecomment-400929",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">Comment 1</div>

It don't see any obvious problems. Don't forget `__delattr__` while you're at it.



---

archive/issue_events_339853.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:42:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25898",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25898#event-339853"
}
```
