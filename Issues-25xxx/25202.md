# Issue 25202: Enable optional rollback functionality when upgrading packages

archive/issues_024965.json:
```json
{
    "assignees": [],
    "body": "With #25139 it becomes possible to fully uninstall all files associated with a package, which is done before upgrading that package to a new version.\n\nThis would just as easily allow creating a *backup* of the package by moving its files to a temporary location rather than deleting them.  If the package upgrade fails, the old version could then be restored.\n\nOne could even imagine keeping backups that can be restored even after a seemingly successful upgrade (e.g. the upgrade succeeds building, but causes runtime regressions).  One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).\n\nBut to keep things focused I think this ticket should just consider rollback on build failures to start with.\n\nDepends on #25139\n\nComponent: **build**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25202_\n\n",
    "created_at": "2018-04-18T08:59:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/pending"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Enable optional rollback functionality when upgrading packages",
    "type": "issue",
    "updated_at": "2019-03-25T10:44:36Z",
    "url": "https://github.com/sagemath/sage/issues/25202",
    "user": "https://github.com/embray"
}
```
With #25139 it becomes possible to fully uninstall all files associated with a package, which is done before upgrading that package to a new version.

This would just as easily allow creating a *backup* of the package by moving its files to a temporary location rather than deleting them.  If the package upgrade fails, the old version could then be restored.

One could even imagine keeping backups that can be restored even after a seemingly successful upgrade (e.g. the upgrade succeeds building, but causes runtime regressions).  One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).

But to keep things focused I think this ticket should just consider rollback on build failures to start with.

Depends on #25139

Component: **build**

_Issue created by migration from https://trac.sagemath.org/ticket/25202_





---

archive/issue_events_348840.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-18T08:59:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348840"
}
```



---

archive/issue_events_348841.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-18T08:59:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "000080",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348841"
}
```



---

archive/issue_events_348842.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-18T08:59:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348842"
}
```



---

archive/issue_events_348843.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-18T08:59:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348843"
}
```



---

archive/issue_comments_385336.json:
```json
{
    "body": "Replying to [ticket:25202 embray]:\n> One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).\n\nJust FYI in case you didn't know: our release manager is one of the main contributors to hashdist. The project looks pretty dead though: the last commit on github was 2 years ago.",
    "created_at": "2018-07-13T14:19:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25202#issuecomment-385336",
    "user": "https://github.com/jdemeyer"
}
```

Replying to [ticket:25202 embray]:
> One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).

Just FYI in case you didn't know: our release manager is one of the main contributors to hashdist. The project looks pretty dead though: the last commit on github was 2 years ago.



---

archive/issue_comments_385337.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nReplying to [@jdemeyer](#comment:1):\n> Replying to [ticket:25202 embray]:\n> > One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).\n\n> \n> Just FYI in case you didn't know: our release manager is one of the main contributors to hashdist. The project looks pretty dead though: the last commit on github was 2 years ago.\n\nI know.  I tried using it a while ago to build Sage on Cygwin and had some problems due to a lack of Cygwin-specific patches for a number of dependencies that needed to be ported over.  Other than that, a few other details I had quibbles with, the way it works makes a lot of sense and I'm surprised Volker didn't push more to get Sage using it.  I would have...",
    "created_at": "2018-07-13T14:33:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25202#issuecomment-385337",
    "user": "https://github.com/embray"
}
```

<div id="comment:2" align="right">comment:2</div>

Replying to [@jdemeyer](#comment:1):
> Replying to [ticket:25202 embray]:
> > One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).

> 
> Just FYI in case you didn't know: our release manager is one of the main contributors to hashdist. The project looks pretty dead though: the last commit on github was 2 years ago.

I know.  I tried using it a while ago to build Sage on Cygwin and had some problems due to a lack of Cygwin-specific patches for a number of dependencies that needed to be ported over.  Other than that, a few other details I had quibbles with, the way it works makes a lot of sense and I'm surprised Volker didn't push more to get Sage using it.  I would have...



---

archive/issue_events_348844.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:56:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348844"
}
```



---

archive/issue_events_348845.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:56:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348845"
}
```



---

archive/issue_events_348846.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348846"
}
```



---

archive/issue_events_348847.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348847"
}
```



---

archive/issue_comments_385338.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nRetargeting some of my tickets (somewhat optimistically for now).",
    "created_at": "2018-12-28T14:10:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25202#issuecomment-385338",
    "user": "https://github.com/embray"
}
```

<div id="comment:5" align="right">comment:5</div>

Retargeting some of my tickets (somewhat optimistically for now).



---

archive/issue_events_348848.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348848"
}
```



---

archive/issue_events_348849.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348849"
}
```



---

archive/issue_comments_385339.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nRemoving most of the rest of my open tickets out of the 8.7 milestone, which should be closed.",
    "created_at": "2019-03-25T10:44:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25202#issuecomment-385339",
    "user": "https://github.com/embray"
}
```

<div id="comment:6" align="right">comment:6</div>

Removing most of the rest of my open tickets out of the 8.7 milestone, which should be closed.



---

archive/issue_events_348850.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:44:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348850"
}
```



---

archive/issue_events_348851.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:44:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25202",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25202#event-348851"
}
```
