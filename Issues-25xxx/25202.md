# Issue 25202: Enable optional rollback functionality when upgrading packages

archive/issues_024965.json:
```json
{
    "body": "With #25139 it becomes possible to fully uninstall all files associated with a package, which is done before upgrading that package to a new version.\n\nThis would just as easily allow creating a *backup* of the package by moving its files to a temporary location rather than deleting them.  If the package upgrade fails, the old version could then be restored.\n\nOne could even imagine keeping backups that can be restored even after a seemingly successful upgrade (e.g. the upgrade succeeds building, but causes runtime regressions).  One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).\n\nBut to keep things focused I think this ticket should just consider rollback on build failures to start with.\n\nStatus: new\n\nDependencies: #25139\n\nIssue created by migration from https://trac.sagemath.org/ticket/25202\n\n",
    "created_at": "2018-04-18T08:59:16Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-pending",
    "title": "Enable optional rollback functionality when upgrading packages",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25202",
    "user": "https://github.com/embray"
}
```
With #25139 it becomes possible to fully uninstall all files associated with a package, which is done before upgrading that package to a new version.

This would just as easily allow creating a *backup* of the package by moving its files to a temporary location rather than deleting them.  If the package upgrade fails, the old version could then be restored.

One could even imagine keeping backups that can be restored even after a seemingly successful upgrade (e.g. the upgrade succeeds building, but causes runtime regressions).  One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).

But to keep things focused I think this ticket should just consider rollback on build failures to start with.

Status: new

Dependencies: #25139

Issue created by migration from https://trac.sagemath.org/ticket/25202





---

archive/issue_comments_350985.json:
```json
{
    "body": "Replying to [ticket:25202 embray]:\n> One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).\n\n\nJust FYI in case you didn't know: our release manager is one of the main contributors to hashdist. The project looks pretty dead though: the last commit on github was 2 years ago.",
    "created_at": "2018-07-13T14:19:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25202#issuecomment-350985",
    "user": "https://github.com/jdemeyer"
}
```

Replying to [ticket:25202 embray]:
> One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).


Just FYI in case you didn't know: our release manager is one of the main contributors to hashdist. The project looks pretty dead though: the last commit on github was 2 years ago.



---

archive/issue_comments_350986.json:
```json
{
    "body": "<a id='comment:2'></a>Replying to [comment:1 jdemeyer]:\n> Replying to [ticket:25202 embray]:\n> > One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).\n\n> \n> Just FYI in case you didn't know: our release manager is one of the main contributors to hashdist. The project looks pretty dead though: the last commit on github was 2 years ago.\n\n\nI know.  I tried using it a while ago to build Sage on Cygwin and had some problems due to a lack of Cygwin-specific patches for a number of dependencies that needed to be ported over.  Other than that, a few other details I had quibbles with, the way it works makes a lot of sense and I'm surprised Volker didn't push more to get Sage using it.  I would have...",
    "created_at": "2018-07-13T14:33:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25202#issuecomment-350986",
    "user": "https://github.com/embray"
}
```

<a id='comment:2'></a>Replying to [comment:1 jdemeyer]:
> Replying to [ticket:25202 embray]:
> > One could easily get carried away with something like this though and end up re-implementing [hashdist](https://github.com/hashdist/hashdist) (which honestly wouldn't be such a bad thing to have built into Sage...).

> 
> Just FYI in case you didn't know: our release manager is one of the main contributors to hashdist. The project looks pretty dead though: the last commit on github was 2 years ago.


I know.  I tried using it a while ago to build Sage on Cygwin and had some problems due to a lack of Cygwin-specific patches for a number of dependencies that needed to be ported over.  Other than that, a few other details I had quibbles with, the way it works makes a lot of sense and I'm surprised Volker didn't push more to get Sage using it.  I would have...



---

archive/issue_events_063224.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:56:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25202#event-63224"
}
```



---

archive/issue_events_063225.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25202#event-63225"
}
```



---

archive/issue_events_063226.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "milestone": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25202#event-63226"
}
```



---

archive/issue_comments_350987.json:
```json
{
    "body": "<a id='comment:5'></a>Retargeting some of my tickets (somewhat optimistically for now).",
    "created_at": "2018-12-28T14:10:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25202#issuecomment-350987",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>Retargeting some of my tickets (somewhat optimistically for now).



---

archive/issue_events_063227.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "milestone": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25202#event-63227"
}
```



---

archive/issue_events_063228.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25202#event-63228"
}
```



---

archive/issue_comments_350988.json:
```json
{
    "body": "<a id='comment:6'></a>Removing most of the rest of my open tickets out of the 8.7 milestone, which should be closed.",
    "created_at": "2019-03-25T10:44:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25202#issuecomment-350988",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'></a>Removing most of the rest of my open tickets out of the 8.7 milestone, which should be closed.



---

archive/issue_events_063229.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:44:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25202#event-63229"
}
```



---

archive/issue_events_063230.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:44:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25202",
    "milestone": "sage-pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25202#event-63230"
}
```
