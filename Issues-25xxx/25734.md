...
<a id='comment:6'></a>Replying to [gh-MareoRaft](#comment%3A4):
> My two cents:
> 
> Having `s([2, 1])` return `s[2, 1]` and `s([1, 2])` return `(2*t+1)*s[]` is inconsistent behavior and can lead to a lot of confusion.
> 
...
<a id='comment:13'></a>Replying to [zabrocki](#comment%3A12):
> 
> Can you clarify
> > This check
> > [snip]
