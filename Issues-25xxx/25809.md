# Issue 25809: sage --info tries to write to SAGE_SPKG_SCRIPTS

archive/issues_025572.json:
```json
{
    "assignees": [],
    "body": "sage --info currently unnecessarily needs write access to SAGE_SPKG_SCRIPTS, which typically will be in a read-only or root owned location for distros. This fixes that by moving the mkdir commands to the appropriate section of the file, after `--info` has exited.\n\nCC:  @embray @kiwifb\n\nBranch/Commit: **[f02714d](https://github.com/sagemath/sagetrac-mirror/commit/f02714d4aea80e17cb8df62bab75d7c1a1b61a8e)**\n\nReviewer: **Erik Bray**\n\nAuthor: **Timo Kaufmann**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25809_\n\n",
    "closed_at": "2018-08-05T08:16:33Z",
    "created_at": "2018-07-09T18:48:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage --info tries to write to SAGE_SPKG_SCRIPTS",
    "type": "issue",
    "updated_at": "2018-08-05T08:16:33Z",
    "url": "https://github.com/sagemath/sage/issues/25809",
    "user": "https://github.com/timokau"
}
```
sage --info currently unnecessarily needs write access to SAGE_SPKG_SCRIPTS, which typically will be in a read-only or root owned location for distros. This fixes that by moving the mkdir commands to the appropriate section of the file, after `--info` has exited.

CC:  @embray @kiwifb

Branch/Commit: **[f02714d](https://github.com/sagemath/sagetrac-mirror/commit/f02714d4aea80e17cb8df62bab75d7c1a1b61a8e)**

Reviewer: **Erik Bray**

Author: **Timo Kaufmann**

_Issue created by migration from https://trac.sagemath.org/ticket/25809_





---

archive/issue_events_338664.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-07-09T18:48:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25809#event-338664"
}
```



---

archive/issue_events_338665.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-07-09T18:48:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25809#event-338665"
}
```



---

archive/issue_events_338666.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-07-09T18:51:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25809#event-338666"
}
```



---

archive/issue_events_338667.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-07-09T18:51:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25809#event-338667"
}
```



---

archive/issue_comments_399240.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nCC'ing fbissey since you'll probably have the same error on gentoo.",
    "created_at": "2018-07-09T18:51:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25809#issuecomment-399240",
    "user": "https://github.com/timokau"
}
```

<div id="comment:1" align="right">Comment 1</div>

CC'ing fbissey since you'll probably have the same error on gentoo.



---

archive/issue_comments_399241.json:
```json
{
    "body": "Changed branch from **[u/gh-timokau/ino-no-side-effect](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/ino-no-side-effect)** to **[u/gh-timokau/info-no-side-effect](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/info-no-side-effect)**",
    "created_at": "2018-07-09T18:52:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25809#issuecomment-399241",
    "user": "https://github.com/timokau"
}
```

Changed branch from **[u/gh-timokau/ino-no-side-effect](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/ino-no-side-effect)** to **[u/gh-timokau/info-no-side-effect](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/info-no-side-effect)**



---

archive/issue_comments_399242.json:
```json
{
    "body": "Commit: **[f02714d](https://github.com/sagemath/sagetrac-mirror/commit/f02714d4aea80e17cb8df62bab75d7c1a1b61a8e)**",
    "created_at": "2018-07-09T18:52:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25809#issuecomment-399242",
    "user": "https://github.com/timokau"
}
```

Commit: **[f02714d](https://github.com/sagemath/sagetrac-mirror/commit/f02714d4aea80e17cb8df62bab75d7c1a1b61a8e)**



---

archive/issue_comments_399243.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f02714d4aea80e17cb8df62bab75d7c1a1b61a8e\">f02714d</a></td><td><code>Don't attempt to create dirs when showing pkg info</code></td></tr></table>\n",
    "created_at": "2018-07-09T18:52:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25809#issuecomment-399243",
    "user": "https://github.com/timokau"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f02714d4aea80e17cb8df62bab75d7c1a1b61a8e">f02714d</a></td><td><code>Don't attempt to create dirs when showing pkg info</code></td></tr></table>




---

archive/issue_comments_399244.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nOn a related note, it might be nice to make the sage installation read-only when doctesting in CI.",
    "created_at": "2018-07-09T18:54:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25809#issuecomment-399244",
    "user": "https://github.com/timokau"
}
```

<div id="comment:3" align="right">Comment 3</div>

On a related note, it might be nice to make the sage installation read-only when doctesting in CI.



---

archive/issue_events_338668.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-07-09T18:55:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25809#event-338668"
}
```



---

archive/issue_events_338669.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-10T10:43:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25809#event-338669"
}
```



---

archive/issue_events_338670.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-10T10:43:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25809#event-338670"
}
```



---

archive/issue_comments_399245.json:
```json
{
    "body": "Reviewer: **Erik Bray**",
    "created_at": "2018-07-10T10:43:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25809#issuecomment-399245",
    "user": "https://github.com/embray"
}
```

Reviewer: **Erik Bray**



---

archive/issue_comments_399246.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nYikes, I didn't even know about `sage --info`.  Yes, this definitely makes more sense.",
    "created_at": "2018-07-10T10:43:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25809#issuecomment-399246",
    "user": "https://github.com/embray"
}
```

<div id="comment:5" align="right">Comment 5</div>

Yikes, I didn't even know about `sage --info`.  Yes, this definitely makes more sense.



---

archive/issue_events_338671.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-05T08:16:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25809#event-338671"
}
```



---

archive/issue_events_338672.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3ed549f927ed5888e2e78f2f4a4ca46a36f00c14",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-08-05T08:16:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25809#event-338672"
}
```



---

archive/issue_comments_399247.json:
```json
{
    "body": "Changed branch from **[u/gh-timokau/info-no-side-effect](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/info-no-side-effect)** to **[f02714d](https://github.com/sagemath/sagetrac-mirror/commit/f02714d4aea80e17cb8df62bab75d7c1a1b61a8e)**",
    "created_at": "2018-08-05T08:16:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25809#issuecomment-399247",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-timokau/info-no-side-effect](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/info-no-side-effect)** to **[f02714d](https://github.com/sagemath/sagetrac-mirror/commit/f02714d4aea80e17cb8df62bab75d7c1a1b61a8e)**
