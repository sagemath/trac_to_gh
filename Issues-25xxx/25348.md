# Issue 25348: sage.misc.lazy_import_cache.get_cache_file() creates a file name > 255 chars

archive/issues_025111.json:
```json
{
    "assignees": [],
    "body": "If the SAGE_SRC is in a really long path like in conda, the cache file name has the long path with / replaced by _ which is > 255 chars. Linux paths can be really long (4096 chars), but the filename has to be less than 255.\n\nPossible workaround\nhttps://github.com/conda-forge/sagelib-feedstock/blob/master/recipe/patches/fix_cache_file_length.patch\n\nCC:  @saraedum\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25348_\n\n",
    "closed_at": "2019-07-09T06:24:01Z",
    "created_at": "2018-05-11T19:11:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.misc.lazy_import_cache.get_cache_file() creates a file name > 255 chars",
    "type": "issue",
    "updated_at": "2019-07-09T16:56:24Z",
    "url": "https://github.com/sagemath/sage/issues/25348",
    "user": "https://github.com/isuruf"
}
```
If the SAGE_SRC is in a really long path like in conda, the cache file name has the long path with / replaced by _ which is > 255 chars. Linux paths can be really long (4096 chars), but the filename has to be less than 255.

Possible workaround
https://github.com/conda-forge/sagelib-feedstock/blob/master/recipe/patches/fix_cache_file_length.patch

CC:  @saraedum

_Issue created by migration from https://trac.sagemath.org/ticket/25348_





---

archive/issue_events_320679.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2018-05-11T19:11:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-320679"
}
```



---

archive/issue_events_320680.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2018-05-11T19:12:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-320680"
}
```



---

archive/issue_comments_390377.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nupdate milestone 8.3 -> 8.4",
    "created_at": "2018-08-03T19:20:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25348#issuecomment-390377",
    "user": "https://github.com/videlec"
}
```

<a id='comment:2'>Comment 2:</a>
update milestone 8.3 -> 8.4



---

archive/issue_events_320681.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-320681"
}
```



---

archive/issue_events_320682.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-320682"
}
```



---

archive/issue_comments_390378.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nFixed in #28014",
    "created_at": "2019-07-08T23:31:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25348#issuecomment-390378",
    "user": "https://github.com/isuruf"
}
```

<a id='comment:3'>Comment 3:</a>
Fixed in #28014



---

archive/issue_events_320683.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-09T06:24:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-320683"
}
```



---

archive/issue_events_320684.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-09T06:24:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-320684"
}
```



---

archive/issue_events_320685.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-09T06:50:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-320685"
}
```



---

archive/issue_comments_390379.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nwrong move, sorry. I did not see that this was already closed..",
    "created_at": "2019-07-09T06:54:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25348#issuecomment-390379",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'>Comment 6:</a>
wrong move, sorry. I did not see that this was already closed..



---

archive/issue_comments_390380.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThanks for closing",
    "created_at": "2019-07-09T16:56:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25348#issuecomment-390380",
    "user": "https://github.com/isuruf"
}
```

<a id='comment:7'>Comment 7:</a>
Thanks for closing
