# Issue 25348: sage.misc.lazy_import_cache.get_cache_file() creates a file name > 255 chars

archive/issues_025111.json:
```json
{
    "assignees": [],
    "body": "If the SAGE_SRC is in a really long path like in conda, the cache file name has the long path with / replaced by _ which is > 255 chars. Linux paths can be really long (4096 chars), but the filename has to be less than 255.\n\nPossible workaround\nhttps://github.com/conda-forge/sagelib-feedstock/blob/master/recipe/patches/fix_cache_file_length.patch\n\nCC:  @saraedum\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25348_\n\n",
    "closed_at": "2019-07-09T06:24:01Z",
    "created_at": "2018-05-11T19:11:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.misc.lazy_import_cache.get_cache_file() creates a file name > 255 chars",
    "type": "issue",
    "updated_at": "2019-07-09T16:56:24Z",
    "url": "https://github.com/sagemath/sage/issues/25348",
    "user": "https://github.com/isuruf"
}
```
If the SAGE_SRC is in a really long path like in conda, the cache file name has the long path with / replaced by _ which is > 255 chars. Linux paths can be really long (4096 chars), but the filename has to be less than 255.

Possible workaround
https://github.com/conda-forge/sagelib-feedstock/blob/master/recipe/patches/fix_cache_file_length.patch

CC:  @saraedum

_Issue created by migration from https://trac.sagemath.org/ticket/25348_





---

archive/issue_events_332635.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2018-05-11T19:11:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332635"
}
```



---

archive/issue_events_332636.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2018-05-11T19:11:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332636"
}
```



---

archive/issue_events_332637.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2018-05-11T19:12:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332637"
}
```



---

archive/issue_comments_390375.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nupdate milestone 8.3 -> 8.4",
    "created_at": "2018-08-03T19:20:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25348#issuecomment-390375",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">Comment 2</div>

update milestone 8.3 -> 8.4



---

archive/issue_events_332638.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332638"
}
```



---

archive/issue_events_332639.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332639"
}
```



---

archive/issue_comments_390376.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nFixed in #28014",
    "created_at": "2019-07-08T23:31:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25348#issuecomment-390376",
    "user": "https://github.com/isuruf"
}
```

<div id="comment:3" align="right">Comment 3</div>

Fixed in #28014



---

archive/issue_events_332640.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-09T06:24:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332640"
}
```



---

archive/issue_events_332641.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-09T06:24:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332641"
}
```



---

archive/issue_events_332642.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-09T06:24:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332642"
}
```



---

archive/issue_events_332643.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-09T06:24:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332643"
}
```



---

archive/issue_events_332644.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-09T06:50:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25348#event-332644"
}
```



---

archive/issue_comments_390377.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nwrong move, sorry. I did not see that this was already closed..",
    "created_at": "2019-07-09T06:54:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25348#issuecomment-390377",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

wrong move, sorry. I did not see that this was already closed..



---

archive/issue_comments_390378.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks for closing",
    "created_at": "2019-07-09T16:56:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25348",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25348#issuecomment-390378",
    "user": "https://github.com/isuruf"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks for closing
