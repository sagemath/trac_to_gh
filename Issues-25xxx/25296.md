...
```json
{
    "body": "<a id='comment:3'></a>`RingConverter` is no longer used in Sage itself since 1b82d7, and it is really unlikely that anyone else was relying on it failing in that case...",
...
<a id='comment:3'></a>`RingConverter` is no longer used in Sage itself since 1b82d7, and it is really unlikely that anyone else was relying on it failing in that case...
