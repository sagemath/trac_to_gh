...
```json
{
    "body": "Nasser, do you happen to have a simple integration result containing a `rootOf`? This would be really helpful for testing purposes!",
...
```json
{
    "body": "`@`gh-nasser1: dear Nasser, could you please check whether this works for you now? (it was actually a fair bit of work, so I'd rather get back to it now than later)\n\nPlease test especially integrals with parameters (that is, extra variables) showing up in the `rootOf` expressions.",
...
```json
{
    "body": "this fixes pyflake errors\n\n---\nNew commits:",
...
```json
{
    "body": "if this works for you, please set it to positive review",
...
```json
{
    "body": "Great! Yes, I rebuilt and all test still pass.  (My intention was to fix the pyflakes stuff in a separate ticket, but I am very happy if they are already fixed now.)\n\nNot sure whether \"you\" is me or Eric, so I am not taking action...",
...
```json
{
    "body": "I cannot reproduce this, but it looks like being a problem with maxima...",
...
```json
{
    "body": "Fricas and Maxima have  in Sage ECL, as a common dependency. It seems that here for some reason ECLs used for them are different, but this doesn't quite work.",
...
```json
{
    "body": "I'm cc'ng Arch maintainer - is there something fishy with fricas support withe current Arch package?",
