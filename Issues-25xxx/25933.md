# Issue 25933: Add ability to generate graphs based on correlations of sequences

archive/issues_025696.json:
```json
{
    "assignees": [],
    "body": "Given a list of labeled real-valued vectors and a significance level `alpha`, generate a graph with vertices corresponding to the list's labels where two vertices are connected by an edge if the corresponding vectors have a correlation coefficient that is positive or negative at the specified significance level. An object creating the graph should support the `plot` method, which will plot the generated graph using a circular layout.\n\n**Assignee:** @rkingan\n\n**Keywords:** sagedays@icerm\n\n**Branch:** [correlation-graph](https://github.com/sagemath/sagetrac-mirror/tree/correlation-graph)\n\n**Author:** Robert Kingan\n\nIssue created by migration from https://trac.sagemath.org/ticket/25933\n\n",
    "created_at": "2018-07-26T02:50:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add ability to generate graphs based on correlations of sequences",
    "type": "issue",
    "updated_at": "2018-07-26T10:44:59Z",
    "url": "https://github.com/sagemath/sage/issues/25933",
    "user": "https://github.com/rkingan"
}
```
Given a list of labeled real-valued vectors and a significance level `alpha`, generate a graph with vertices corresponding to the list's labels where two vertices are connected by an edge if the corresponding vectors have a correlation coefficient that is positive or negative at the specified significance level. An object creating the graph should support the `plot` method, which will plot the generated graph using a circular layout.

**Assignee:** @rkingan

**Keywords:** sagedays@icerm

**Branch:** [correlation-graph](https://github.com/sagemath/sagetrac-mirror/tree/correlation-graph)

**Author:** Robert Kingan

Issue created by migration from https://trac.sagemath.org/ticket/25933





---

archive/issue_events_312654.json:
```json
{
    "actor": "https://github.com/rkingan",
    "created_at": "2018-07-26T02:50:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25933",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25933#event-312654"
}
```



---

archive/issue_events_312655.json:
```json
{
    "actor": "https://github.com/rkingan",
    "created_at": "2018-07-26T02:50:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25933",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25933#event-312655"
}
```



---

archive/issue_comments_401825.json:
```json
{
    "body": "<a id='comment:1'></a>\nAdded branch name",
    "created_at": "2018-07-26T10:43:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25933#issuecomment-401825",
    "user": "https://github.com/rkingan"
}
```

<a id='comment:1'></a>
Added branch name



---

archive/issue_comments_401826.json:
```json
{
    "body": "**Branch:** [correlation-graph](https://github.com/sagemath/sagetrac-mirror/tree/correlation-graph)",
    "created_at": "2018-07-26T10:43:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25933#issuecomment-401826",
    "user": "https://github.com/rkingan"
}
```

**Branch:** [correlation-graph](https://github.com/sagemath/sagetrac-mirror/tree/correlation-graph)



---

archive/issue_comments_401827.json:
```json
{
    "body": "**Assignee:** @rkingan",
    "created_at": "2018-07-26T10:44:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25933#issuecomment-401827",
    "user": "https://github.com/rkingan"
}
```

**Assignee:** @rkingan



---

archive/issue_events_312656.json:
```json
{
    "actor": "https://github.com/rkingan",
    "created_at": "2018-07-26T10:44:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25933",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "08517b",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25933#event-312656"
}
```
