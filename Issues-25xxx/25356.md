# Issue 25356: Make python3 print syntax warning

archive/issues_025119.json:
```json
{
    "assignees": [],
    "body": "For some reason nix's version of python reports\n\n```\nSyntaxError: Missing parentheses in call to 'print'. Did you mean print(\"invalid print statement\")?\n```\n\nwhile sage expects the message without the suggestion:\n\n\n```\nSyntaxError: Missing parentheses in call to 'print'\n```\n\nThis patch makes sages doctests accept both versions.\n\nBranch/Commit: **[edaaeea](https://github.com/sagemath/sagetrac-mirror/commit/edaaeea9ba1cda0c779e32ee93902eda3071a494)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Timo Kaufmann**\n\nComponent: **distribution**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25356_\n\n",
    "closed_at": "2018-05-15T22:33:10Z",
    "created_at": "2018-05-13T18:13:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make python3 print syntax warning",
    "type": "issue",
    "updated_at": "2018-05-15T22:33:10Z",
    "url": "https://github.com/sagemath/sage/issues/25356",
    "user": "https://github.com/timokau"
}
```
For some reason nix's version of python reports

```
SyntaxError: Missing parentheses in call to 'print'. Did you mean print("invalid print statement")?
```

while sage expects the message without the suggestion:


```
SyntaxError: Missing parentheses in call to 'print'
```

This patch makes sages doctests accept both versions.

Branch/Commit: **[edaaeea](https://github.com/sagemath/sagetrac-mirror/commit/edaaeea9ba1cda0c779e32ee93902eda3071a494)**

Reviewer: **Frédéric Chapoton**

Author: **Timo Kaufmann**

Component: **distribution**

_Issue created by migration from https://trac.sagemath.org/ticket/25356_





---

archive/issue_events_350797.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-05-13T18:13:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350797"
}
```



---

archive/issue_events_350798.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-05-13T18:13:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "000080",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350798"
}
```



---

archive/issue_events_350799.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-05-13T18:13:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350799"
}
```



---

archive/issue_events_350800.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-05-13T18:13:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350800"
}
```



---

archive/issue_events_350801.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-05-13T18:13:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350801"
}
```



---

archive/issue_events_350802.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-05-13T18:20:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350802"
}
```



---

archive/issue_events_350803.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-05-13T18:20:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350803"
}
```



---

archive/issue_comments_387870.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nok",
    "created_at": "2018-05-13T18:20:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25356#issuecomment-387870",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">comment:2</div>

ok



---

archive/issue_comments_387871.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-05-13T18:20:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25356#issuecomment-387871",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_350804.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-05-13T18:21:28Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "title_is": "Make python3 print syntax warning",
    "title_was": "Make python3 print syntax warmi",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350804"
}
```



---

archive/issue_events_350805.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-15T22:33:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350805"
}
```



---

archive/issue_events_350806.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-15T22:33:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25356#event-350806"
}
```



---

archive/issue_comments_387872.json:
```json
{
    "body": "Changed branch from **[u/gh-timokau/python-syntax-warning-formatting](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/python-syntax-warning-formatting)** to **[edaaeea](https://github.com/sagemath/sagetrac-mirror/commit/edaaeea9ba1cda0c779e32ee93902eda3071a494)**",
    "created_at": "2018-05-15T22:33:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25356#issuecomment-387872",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-timokau/python-syntax-warning-formatting](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/python-syntax-warning-formatting)** to **[edaaeea](https://github.com/sagemath/sagetrac-mirror/commit/edaaeea9ba1cda0c779e32ee93902eda3071a494)**
