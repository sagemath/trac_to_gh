# Issue 25158: avoid cmake installation when possible

archive/issues_024921.json:
```json
{
    "assignees": [],
    "body": "If `cmake` is already installed (system wide) and is a dependencie of another sage package `cmake` sage package shouldn't be installed.\n\nThe desired behaviour is sort of similar to `gcc`, `yasm` or `curl`, except `cmake` is an optional package.\n\nThis ticket is following the ticket #25109 discussion.\n\n\n\n\nDepends on #24919\n\nCC:  @kiwifb @videlec @slel\n\nKeywords: **cmake**\n\nReviewer: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25158_\n\n",
    "closed_at": "2019-04-14T06:06:25Z",
    "created_at": "2018-04-13T07:58:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "avoid cmake installation when possible",
    "type": "issue",
    "updated_at": "2019-04-14T06:06:25Z",
    "url": "https://github.com/sagemath/sage/issues/25158",
    "user": "https://github.com/vinklein"
}
```
If `cmake` is already installed (system wide) and is a dependencie of another sage package `cmake` sage package shouldn't be installed.

The desired behaviour is sort of similar to `gcc`, `yasm` or `curl`, except `cmake` is an optional package.

This ticket is following the ticket #25109 discussion.




Depends on #24919

CC:  @kiwifb @videlec @slel

Keywords: **cmake**

Reviewer: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/25158_





---

archive/issue_events_330213.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-13T07:58:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330213"
}
```



---

archive/issue_events_330214.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-13T07:58:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330214"
}
```



---

archive/issue_events_330215.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-13T07:58:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330215"
}
```



---

archive/issue_events_330216.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-13T07:58:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330216"
}
```



---

archive/issue_comments_387247.json:
```json
{
    "body": "Changed author from **Vicnent Klein** to **Vincent Klein**",
    "created_at": "2018-04-13T08:05:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25158#issuecomment-387247",
    "user": "https://github.com/vinklein"
}
```

Changed author from **Vicnent Klein** to **Vincent Klein**



---

archive/issue_comments_387248.json:
```json
{
    "body": "Changed author from **Vincent Klein** to none",
    "created_at": "2018-04-13T08:25:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25158#issuecomment-387248",
    "user": "https://github.com/vinklein"
}
```

Changed author from **Vincent Klein** to none



---

archive/issue_comments_387249.json:
```json
{
    "body": "Dependencies: **#24919**",
    "created_at": "2018-05-01T12:59:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25158#issuecomment-387249",
    "user": "https://github.com/videlec"
}
```

Dependencies: **#24919**



---

archive/issue_comments_387250.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nSee also #27487.",
    "created_at": "2019-04-14T02:38:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25158#issuecomment-387250",
    "user": "https://github.com/slel"
}
```

<div id="comment:4" align="right">Comment 4</div>

See also #27487.



---

archive/issue_comments_387251.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nSet to positive review if you agree this is a duplicate of #27487.",
    "created_at": "2019-04-14T02:40:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25158#issuecomment-387251",
    "user": "https://github.com/slel"
}
```

<div id="comment:5" align="right">Comment 5</div>

Set to positive review if you agree this is a duplicate of #27487.



---

archive/issue_events_330217.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-04-14T02:40:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330217"
}
```



---

archive/issue_events_330218.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-04-14T02:40:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330218"
}
```



---

archive/issue_events_330219.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-04-14T02:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330219"
}
```



---

archive/issue_events_330220.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-04-14T02:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330220"
}
```



---

archive/issue_events_330221.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-04-14T06:04:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330221"
}
```



---

archive/issue_events_330222.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-04-14T06:04:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330222"
}
```



---

archive/issue_comments_387252.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2019-04-14T06:04:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25158#issuecomment-387252",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_events_330223.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-04-14T06:06:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330223"
}
```



---

archive/issue_events_330224.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-04-14T06:06:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25158",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25158#event-330224"
}
```
