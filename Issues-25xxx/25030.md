# Issue 25030: Use boost dominator_tree instead of immediate_dominators

archive/issues_024793.json:
```json
{
    "assignees": [],
    "body": "Ticket #23210 adds `immediate_dominators` method for `DiGraph` while the Boost method `dominator_tree` provides the same result and is significantly faster.\n\nWith this ticket, we:\n- enable the computation of the dominator tree in the reverse graph. This is done by adding parameter `reverse` to methods initializing boost graphs\n- use `dominator_tree` in method `strong_articulation_points`\n- deprecate method `immediate_dominators` that is slower and do less than the `dominator_tree` method.\n\nCC:  @tscrim\n\nBranch/Commit: **[71bad7c](https://github.com/sagemath/sagetrac-mirror/commit/71bad7c5e2c05f5ec924eb08b79fca150186526f)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25030_\n\n",
    "closed_at": "2018-05-15T22:33:54Z",
    "created_at": "2018-03-24T11:02:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Use boost dominator_tree instead of immediate_dominators",
    "type": "issue",
    "updated_at": "2018-05-15T22:33:54Z",
    "url": "https://github.com/sagemath/sage/issues/25030",
    "user": "https://github.com/dcoudert"
}
```
Ticket #23210 adds `immediate_dominators` method for `DiGraph` while the Boost method `dominator_tree` provides the same result and is significantly faster.

With this ticket, we:
- enable the computation of the dominator tree in the reverse graph. This is done by adding parameter `reverse` to methods initializing boost graphs
- use `dominator_tree` in method `strong_articulation_points`
- deprecate method `immediate_dominators` that is slower and do less than the `dominator_tree` method.

CC:  @tscrim

Branch/Commit: **[71bad7c](https://github.com/sagemath/sagetrac-mirror/commit/71bad7c5e2c05f5ec924eb08b79fca150186526f)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/25030_





---

archive/issue_events_328134.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-03-24T11:02:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328134"
}
```



---

archive/issue_events_328135.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-03-24T11:02:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328135"
}
```



---

archive/issue_events_328136.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-03-24T11:02:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328136"
}
```



---

archive/issue_events_328137.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-03-24T11:02:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328137"
}
```



---

archive/issue_events_328138.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-03-24T12:13:45Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "title_is": "Use boost dominator_tree instead of immediate_dominators",
    "title_was": "Use boost dominator_tree for immediate_dominators",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328138"
}
```



---

archive/issue_comments_384053.json:
```json
{
    "body": "Branch: **[u/dcoudert/25030_use_dominator_tree](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/25030_use_dominator_tree)**",
    "created_at": "2018-03-24T12:13:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25030#issuecomment-384053",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[u/dcoudert/25030_use_dominator_tree](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/25030_use_dominator_tree)**



---

archive/issue_comments_384054.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4c9d8eae8656395eb576284106fc651dc00956e1\">4c9d8ea</a></td><td><code>trac #25030: add parameter reverse to boost graph</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cc14d3d6788c9f13086af354f7fdfda3828b3a21\">cc14d3d</a></td><td><code>trac #25030: use boost dominator tree for strong articulation points</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/58d90e359fe11a61c42e965bddfb49e398e2d418\">58d90e3</a></td><td><code>trac #25030: deprecate method immediate_dominators</code></td></tr></table>\n",
    "created_at": "2018-03-24T12:13:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25030#issuecomment-384054",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4c9d8eae8656395eb576284106fc651dc00956e1">4c9d8ea</a></td><td><code>trac #25030: add parameter reverse to boost graph</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cc14d3d6788c9f13086af354f7fdfda3828b3a21">cc14d3d</a></td><td><code>trac #25030: use boost dominator tree for strong articulation points</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/58d90e359fe11a61c42e965bddfb49e398e2d418">58d90e3</a></td><td><code>trac #25030: deprecate method immediate_dominators</code></td></tr></table>




---

archive/issue_events_328139.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-03-24T12:13:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328139"
}
```



---

archive/issue_comments_384055.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,6 @@\n-Ticket #23210 adds `immediate_dominators` method for DiGraph while the Boost method `dominator_tree` provides same result, faster.\n+Ticket #23210 adds `immediate_dominators` method for `DiGraph` while the Boost method `dominator_tree` provides the same result and is significantly faster.\n+\n+With this ticket, we:\n+- enable the computation of the dominator tree in the reverse graph. This is done by adding parameter `reverse` to methods initializing boost graphs\n+- use `dominator_tree` in method `strong_articulation_points`\n+- deprecate method `immediate_dominators` that is slower and do less than the `dominator_tree` method.\n``````\n",
    "created_at": "2018-03-24T12:13:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25030#issuecomment-384055",
    "user": "https://github.com/dcoudert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,6 @@
-Ticket #23210 adds `immediate_dominators` method for DiGraph while the Boost method `dominator_tree` provides same result, faster.
+Ticket #23210 adds `immediate_dominators` method for `DiGraph` while the Boost method `dominator_tree` provides the same result and is significantly faster.
+
+With this ticket, we:
+- enable the computation of the dominator tree in the reverse graph. This is done by adding parameter `reverse` to methods initializing boost graphs
+- use `dominator_tree` in method `strong_articulation_points`
+- deprecate method `immediate_dominators` that is slower and do less than the `dominator_tree` method.
``````




---

archive/issue_comments_384056.json:
```json
{
    "body": "Commit: **[58d90e3](https://github.com/sagemath/sagetrac-mirror/commit/58d90e359fe11a61c42e965bddfb49e398e2d418)**",
    "created_at": "2018-03-24T12:13:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25030#issuecomment-384056",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[58d90e3](https://github.com/sagemath/sagetrac-mirror/commit/58d90e359fe11a61c42e965bddfb49e398e2d418)**



---

archive/issue_comments_384057.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/71bad7c5e2c05f5ec924eb08b79fca150186526f\">71bad7c</a></td><td><code>trac #25030: Merged with 8.3.beta0</code></td></tr></table>\n",
    "created_at": "2018-05-13T17:04:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25030#issuecomment-384057",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/71bad7c5e2c05f5ec924eb08b79fca150186526f">71bad7c</a></td><td><code>trac #25030: Merged with 8.3.beta0</code></td></tr></table>




---

archive/issue_comments_384058.json:
```json
{
    "body": "Changed commit from **[58d90e3](https://github.com/sagemath/sagetrac-mirror/commit/58d90e359fe11a61c42e965bddfb49e398e2d418)** to **[71bad7c](https://github.com/sagemath/sagetrac-mirror/commit/71bad7c5e2c05f5ec924eb08b79fca150186526f)**",
    "created_at": "2018-05-13T17:04:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25030#issuecomment-384058",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[58d90e3](https://github.com/sagemath/sagetrac-mirror/commit/58d90e359fe11a61c42e965bddfb49e398e2d418)** to **[71bad7c](https://github.com/sagemath/sagetrac-mirror/commit/71bad7c5e2c05f5ec924eb08b79fca150186526f)**



---

archive/issue_events_328140.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-05-13T17:05:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328140"
}
```



---

archive/issue_events_328141.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-05-13T17:05:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328141"
}
```



---

archive/issue_comments_384059.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-05-13T21:52:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25030#issuecomment-384059",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_328142.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-05-13T21:52:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328142"
}
```



---

archive/issue_events_328143.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-05-13T21:52:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328143"
}
```



---

archive/issue_comments_384060.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLGTM.",
    "created_at": "2018-05-13T21:52:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25030#issuecomment-384060",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
LGTM.



---

archive/issue_comments_384061.json:
```json
{
    "body": "Changed branch from **[u/dcoudert/25030_use_dominator_tree](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/25030_use_dominator_tree)** to **[71bad7c](https://github.com/sagemath/sagetrac-mirror/commit/71bad7c5e2c05f5ec924eb08b79fca150186526f)**",
    "created_at": "2018-05-15T22:33:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25030#issuecomment-384061",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dcoudert/25030_use_dominator_tree](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/25030_use_dominator_tree)** to **[71bad7c](https://github.com/sagemath/sagetrac-mirror/commit/71bad7c5e2c05f5ec924eb08b79fca150186526f)**



---

archive/issue_events_328144.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-15T22:33:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328144"
}
```



---

archive/issue_events_328145.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "286548fd314a214fcaf8ca9edef25cb144a0f94d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-15T22:33:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25030",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25030#event-328145"
}
```
