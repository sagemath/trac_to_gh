# Issue 25887: Add dependencies to p_group_cohomology

archive/issues_025650.json:
```json
{
    "assignees": [],
    "body": "When installing the singular spkg, the folder `$SAGE_SHARE/singular/` is completely overridden. However, `p_group_cohomology` installs two .lib files into that folder. So, singular must be listed as a dependency for p_group_cohomology, so that it will be rebuilt after each upgrade of singular.\n\nAlso it adds some files into `$SAGE_SHARE/sage/ext/gap/modular_cohomology`, but (as I just tested) re-installing GAP preserves that folder.\n\nSo, I suggest adding just singular to the dependency list of p_group_cohomology.\n\nKeywords: **p_group_cohomology singular dependency**\n\nBranch/Commit: **[dbbaf1e](https://github.com/sagemath/sagetrac-mirror/commit/dbbaf1e65171e64994178c9142ca808a1b4f1f4d)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Simon King**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25887_\n\n",
    "closed_at": "2018-07-22T22:59:49Z",
    "created_at": "2018-07-20T11:37:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add dependencies to p_group_cohomology",
    "type": "issue",
    "updated_at": "2018-07-22T22:59:49Z",
    "url": "https://github.com/sagemath/sage/issues/25887",
    "user": "https://github.com/simon-king-jena"
}
```
When installing the singular spkg, the folder `$SAGE_SHARE/singular/` is completely overridden. However, `p_group_cohomology` installs two .lib files into that folder. So, singular must be listed as a dependency for p_group_cohomology, so that it will be rebuilt after each upgrade of singular.

Also it adds some files into `$SAGE_SHARE/sage/ext/gap/modular_cohomology`, but (as I just tested) re-installing GAP preserves that folder.

So, I suggest adding just singular to the dependency list of p_group_cohomology.

Keywords: **p_group_cohomology singular dependency**

Branch/Commit: **[dbbaf1e](https://github.com/sagemath/sagetrac-mirror/commit/dbbaf1e65171e64994178c9142ca808a1b4f1f4d)**

Reviewer: **Jeroen Demeyer**

Author: **Simon King**

_Issue created by migration from https://trac.sagemath.org/ticket/25887_





---

archive/issue_events_339722.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2018-07-20T11:37:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339722"
}
```



---

archive/issue_events_339723.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2018-07-20T11:37:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339723"
}
```



---

archive/issue_events_339724.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2018-07-20T11:37:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339724"
}
```



---

archive/issue_events_339725.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2018-07-20T11:37:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339725"
}
```



---

archive/issue_comments_400730.json:
```json
{
    "body": "Branch: **[u/SimonKing/add_dependencies_to_p_group_cohomology](https://github.com/sagemath/sagetrac-mirror/tree/u/SimonKing/add_dependencies_to_p_group_cohomology)**",
    "created_at": "2018-07-20T11:49:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400730",
    "user": "https://github.com/simon-king-jena"
}
```

Branch: **[u/SimonKing/add_dependencies_to_p_group_cohomology](https://github.com/sagemath/sagetrac-mirror/tree/u/SimonKing/add_dependencies_to_p_group_cohomology)**



---

archive/issue_comments_400731.json:
```json
{
    "body": "Commit: **[dbbaf1e](https://github.com/sagemath/sagetrac-mirror/commit/dbbaf1e65171e64994178c9142ca808a1b4f1f4d)**",
    "created_at": "2018-07-20T11:49:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400731",
    "user": "https://github.com/simon-king-jena"
}
```

Commit: **[dbbaf1e](https://github.com/sagemath/sagetrac-mirror/commit/dbbaf1e65171e64994178c9142ca808a1b4f1f4d)**



---

archive/issue_comments_400732.json:
```json
{
    "body": "Author: **Simon King**",
    "created_at": "2018-07-20T11:49:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400732",
    "user": "https://github.com/simon-king-jena"
}
```

Author: **Simon King**



---

archive/issue_comments_400733.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nDone...\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dbbaf1e65171e64994178c9142ca808a1b4f1f4d\">dbbaf1e</a></td><td><code>Add singular as dependency for p_group_cohomology</code></td></tr></table>\n",
    "created_at": "2018-07-20T11:49:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400733",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:2" align="right">Comment 2</div>

Done...

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dbbaf1e65171e64994178c9142ca808a1b4f1f4d">dbbaf1e</a></td><td><code>Add singular as dependency for p_group_cohomology</code></td></tr></table>




---

archive/issue_events_339726.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2018-07-20T11:49:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339726"
}
```



---

archive/issue_events_339727.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-21T06:49:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339727"
}
```



---

archive/issue_events_339728.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-21T06:49:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339728"
}
```



---

archive/issue_events_339729.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-21T06:49:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339729"
}
```



---

archive/issue_events_339730.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-21T06:49:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339730"
}
```



---

archive/issue_events_339731.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-21T06:49:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339731"
}
```



---

archive/issue_events_339732.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-21T06:49:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339732"
}
```



---

archive/issue_comments_400734.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2018-07-21T06:49:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400734",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_400735.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNow optional packages are also release blockers...",
    "created_at": "2018-07-21T12:54:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400735",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Now optional packages are also release blockers...



---

archive/issue_comments_400736.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis is causing breakage on one of the patchbots. So it should be a blocker indeed.",
    "created_at": "2018-07-21T14:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400736",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">Comment 5</div>

This is causing breakage on one of the patchbots. So it should be a blocker indeed.



---

archive/issue_comments_400737.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nCausing breakage in a patchbot that tests this particular optional package, perhaps?",
    "created_at": "2018-07-21T15:05:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400737",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:6" align="right">Comment 6</div>

Causing breakage in a patchbot that tests this particular optional package, perhaps?



---

archive/issue_comments_400738.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@vbraun](#comment%3A6):\n> Causing breakage in a patchbot that tests this particular optional package, perhaps? \n\nYes, obviously.",
    "created_at": "2018-07-21T17:37:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400738",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@vbraun](#comment%3A6):
> Causing breakage in a patchbot that tests this particular optional package, perhaps? 

Yes, obviously.



---

archive/issue_comments_400739.json:
```json
{
    "body": "Changed branch from **[u/SimonKing/add_dependencies_to_p_group_cohomology](https://github.com/sagemath/sagetrac-mirror/tree/u/SimonKing/add_dependencies_to_p_group_cohomology)** to **[dbbaf1e](https://github.com/sagemath/sagetrac-mirror/commit/dbbaf1e65171e64994178c9142ca808a1b4f1f4d)**",
    "created_at": "2018-07-22T22:59:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25887#issuecomment-400739",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/SimonKing/add_dependencies_to_p_group_cohomology](https://github.com/sagemath/sagetrac-mirror/tree/u/SimonKing/add_dependencies_to_p_group_cohomology)** to **[dbbaf1e](https://github.com/sagemath/sagetrac-mirror/commit/dbbaf1e65171e64994178c9142ca808a1b4f1f4d)**



---

archive/issue_events_339733.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-07-22T22:59:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339733"
}
```



---

archive/issue_events_339734.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4ca0be713b742c1607c9ee3720b03252456888ef",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-07-22T22:59:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25887",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25887#event-339734"
}
```
