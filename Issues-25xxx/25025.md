# Issue 25025: Fix jmol package in Cygwin

archive/issues_024788.json:
```json
{
    "assignees": [],
    "body": "This does two things:\n1) Original motivation is to ensure a Windows native path is used when passing the path to `Jmol.jar` to the java interpreter.\n\n2) Cleans things up a bit by providing our own, slightly simpler `jmol` script as opposed to the script that comes with the jmol binary package, rather than patching that script.\n\nDepends on #25021\n\nBranch/Commit: **[880330f](https://github.com/sagemath/sagetrac-mirror/commit/880330fc5f4c80782e25e23a53eebc4826728758)**\n\nReviewer: **Julian R\u00fcth**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25025_\n\n",
    "closed_at": "2018-03-30T23:28:30Z",
    "created_at": "2018-03-22T14:59:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix jmol package in Cygwin",
    "type": "issue",
    "updated_at": "2018-03-30T23:28:30Z",
    "url": "https://github.com/sagemath/sage/issues/25025",
    "user": "https://github.com/embray"
}
```
This does two things:
1) Original motivation is to ensure a Windows native path is used when passing the path to `Jmol.jar` to the java interpreter.

2) Cleans things up a bit by providing our own, slightly simpler `jmol` script as opposed to the script that comes with the jmol binary package, rather than patching that script.

Depends on #25021

Branch/Commit: **[880330f](https://github.com/sagemath/sagetrac-mirror/commit/880330fc5f4c80782e25e23a53eebc4826728758)**

Reviewer: **Julian Rüth**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/25025_





---

archive/issue_events_328048.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-22T14:59:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328048"
}
```



---

archive/issue_events_328049.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-22T14:59:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328049"
}
```



---

archive/issue_events_328050.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-22T14:59:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328050"
}
```



---

archive/issue_events_328051.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-22T14:59:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328051"
}
```



---

archive/issue_comments_383931.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nI feel this is fairly critical actually--although in most cases threejs should probably be preferred at this point over jmol, I still get lots of users on ask.sagemath.org having problems with this.",
    "created_at": "2018-03-22T15:00:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25025#issuecomment-383931",
    "user": "https://github.com/embray"
}
```

<div id="comment:1" align="right">Comment 1</div>

I feel this is fairly critical actually--although in most cases threejs should probably be preferred at this point over jmol, I still get lots of users on ask.sagemath.org having problems with this.



---

archive/issue_events_328052.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-22T15:00:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328052"
}
```



---

archive/issue_events_328053.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-22T15:00:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328053"
}
```



---

archive/issue_events_328054.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-22T15:00:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328054"
}
```



---

archive/issue_comments_383932.json:
```json
{
    "body": "Dependencies: **#25021**",
    "created_at": "2018-03-22T15:01:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25025#issuecomment-383932",
    "user": "https://github.com/embray"
}
```

Dependencies: **#25021**



---

archive/issue_comments_383933.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2018-03-23T21:51:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25025#issuecomment-383933",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_events_328055.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-23T21:51:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328055"
}
```



---

archive/issue_events_328056.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-23T21:51:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328056"
}
```



---

archive/issue_events_328057.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-29T08:26:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328057"
}
```



---

archive/issue_events_328058.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-29T08:26:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328058"
}
```



---

archive/issue_comments_383934.json:
```json
{
    "body": "Changed branch from **[u/embray/cygwin/jmol-paths](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/jmol-paths)** to **[880330f](https://github.com/sagemath/sagetrac-mirror/commit/880330fc5f4c80782e25e23a53eebc4826728758)**",
    "created_at": "2018-03-30T23:28:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25025#issuecomment-383934",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/cygwin/jmol-paths](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/jmol-paths)** to **[880330f](https://github.com/sagemath/sagetrac-mirror/commit/880330fc5f4c80782e25e23a53eebc4826728758)**



---

archive/issue_events_328059.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-30T23:28:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328059"
}
```



---

archive/issue_events_328060.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "97a9c3fe1acc4b4209ec71e4201497cd30b90a6c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-03-30T23:28:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25025",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25025#event-328060"
}
```
