...
```json
{
    "body": "I fixed a couple of places where field and row size are not assigned, and I enabled the use of the optional `start_col` argument of `add_multiple_of_row_c` and `rescale_row_c`. Tests in sage/matrix/ pass.\n\n---\nNew commits:",
...
```json
{
    "body": "PS: I needed to import some additional stuff from SharedMeatAxe, which means that I needed to change sage/libs/meataxe.pxd as well.",
...
```json
{
    "body": "I should make it clear I do not have any concrete example where I could write C code that would be definitely faster than Cython's version. Although I have not currently tried such a comparison.",
...
```json
{
    "body": "Can someone verify my change? That is the only thing missing from a positive review here.",
