# Issue 25816: git broken in ./sage -sh on Cygwin

archive/issues_025579.json:
```json
{
    "body": "This is a problem that has been plaguing me for a few months but I haven't taken much time to work out the source of the problem yet.\n\nIt might also be dependent on the package version of Cygwin's git pacakge, but when using the system git from Cygwin in a `./sage -sh` shell, it's broken.  A workaround is to put `PATH=/usr/bin:$PATH` in front.  So it seems maybe some library installed as part of Sage is incompatible with the system git.\n\nThis is a problem, in particular, for `pip install`ing packages from a git repo, for example.\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/25816\n\n",
    "created_at": "2018-07-10T14:47:35Z",
    "labels": [
        "component: porting: cygwin",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-pending",
    "title": "git broken in ./sage -sh on Cygwin",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25816",
    "user": "https://github.com/embray"
}
```
This is a problem that has been plaguing me for a few months but I haven't taken much time to work out the source of the problem yet.

It might also be dependent on the package version of Cygwin's git pacakge, but when using the system git from Cygwin in a `./sage -sh` shell, it's broken.  A workaround is to put `PATH=/usr/bin:$PATH` in front.  So it seems maybe some library installed as part of Sage is incompatible with the system git.

This is a problem, in particular, for `pip install`ing packages from a git repo, for example.

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/25816





---

archive/issue_comments_494945.json:
```json
{
    "body": "<a id='comment:1'></a>In the process of `git clone` it forks and spawns `/usr/libexec/git-core/git-remote-https.exe`. This in turn loads:\n\n```\n--- Process 1812 loaded C:\\Windows\\System32\\ntdll.dll at 00007FFEA4BF0000\n--- Process 1812 loaded C:\\Windows\\System32\\kernel32.dll at 00007FFEA3D30000\n--- Process 1812 loaded C:\\Windows\\System32\\KernelBase.dll at 00007FFEA1B90000\n--- Process 1812 thread 13644 created\n--- Process 1812 thread 17792 created\n--- Process 1812 thread 29828 created\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygcurl-4.dll at 00000003F6D60000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygwin1.dll at 0000000180040000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygintl-8.dll at 00000003ECE50000\n--- Process 1812 loaded C:\\Users\\Erik M. Bray\\Home\\src\\sagemath\\sage\\local\\bin\\cygiconv-2.dll at 00000003CB550000\n--- Process 1812 loaded C:\\Users\\Erik M. Bray\\Home\\src\\sagemath\\sage\\local\\bin\\cygpcre-1.dll at 00000003C6FC0000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygz.dll at 00000003E8DC0000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygcrypto-1.0.0.dll at 00000003F5180000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cyggssapi_krb5-2.dll at 00000003F2EF0000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygldap-2-4-2.dll at 00000003F26A0000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygidn2-0.dll at 00000003F2BC0000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cyglber-2-4-2.dll at 00000003F2750000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygnghttp2-14.dll at 00000003ECF70000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygpsl-5.dll at 00000003EBEA0000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygssh2-1.dll at 00000003EB010000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygssl-1.0.0.dll at 00000003EAFA0000\n--- Process 1812, exception c0000139 at 00007FFEA4C89968\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygkrb5-3.dll at 00000003EC900000\n--- Process 1812 loaded C:\\cygwin64\\bin\\cygk5crypto-3.dll at 00000003EC9E0000\n--- Process 1812 thread 21968 exited with status 0xc0000139\n--- Process 1812 thread 17792 exited with status 0xc0000139\n--- Process 1812 thread 29828 exited with status 0xc0000139\n--- Process 1812 exited with status 0xc0000139\n```\n\nand then dies.  It's odd that it loads iconv and pcre from Sage.  Rather, it's expected in a way, but where it gets strange is that `git.exe` also loads these DLLs but it uses the versions from the system:\n\n```\n--- Process 9444 created\n--- Process 9444 loaded C:\\Windows\\System32\\ntdll.dll at 00007FFEA4BF0000\n--- Process 9444 loaded C:\\Windows\\System32\\kernel32.dll at 00007FFEA3D30000\n--- Process 9444 loaded C:\\Windows\\System32\\KernelBase.dll at 00007FFEA1B90000\n--- Process 9444 thread 9700 created\n--- Process 9444 thread 4776 created\n--- Process 9444 thread 1016 created\n--- Process 9444 loaded C:\\cygwin64\\bin\\cygwin1.dll at 0000000180040000\n--- Process 9444 loaded C:\\cygwin64\\bin\\cygiconv-2.dll at 00000003F2530000\n--- Process 9444 loaded C:\\cygwin64\\bin\\cygintl-8.dll at 00000003ECE50000\n--- Process 9444 loaded C:\\cygwin64\\bin\\cygpcre-1.dll at 00000003EC540000\n--- Process 9444 loaded C:\\cygwin64\\bin\\cygz.dll at 00000003E8DC0000\n```\n\nIt could be some subtlety about the DLL order?  That remains unclear at the moment.  I need to take a closer look at the DLL dependency trees.",
    "created_at": "2018-07-10T15:56:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494945",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'></a>In the process of `git clone` it forks and spawns `/usr/libexec/git-core/git-remote-https.exe`. This in turn loads:

```
--- Process 1812 loaded C:\Windows\System32\ntdll.dll at 00007FFEA4BF0000
--- Process 1812 loaded C:\Windows\System32\kernel32.dll at 00007FFEA3D30000
--- Process 1812 loaded C:\Windows\System32\KernelBase.dll at 00007FFEA1B90000
--- Process 1812 thread 13644 created
--- Process 1812 thread 17792 created
--- Process 1812 thread 29828 created
--- Process 1812 loaded C:\cygwin64\bin\cygcurl-4.dll at 00000003F6D60000
--- Process 1812 loaded C:\cygwin64\bin\cygwin1.dll at 0000000180040000
--- Process 1812 loaded C:\cygwin64\bin\cygintl-8.dll at 00000003ECE50000
--- Process 1812 loaded C:\Users\Erik M. Bray\Home\src\sagemath\sage\local\bin\cygiconv-2.dll at 00000003CB550000
--- Process 1812 loaded C:\Users\Erik M. Bray\Home\src\sagemath\sage\local\bin\cygpcre-1.dll at 00000003C6FC0000
--- Process 1812 loaded C:\cygwin64\bin\cygz.dll at 00000003E8DC0000
--- Process 1812 loaded C:\cygwin64\bin\cygcrypto-1.0.0.dll at 00000003F5180000
--- Process 1812 loaded C:\cygwin64\bin\cyggssapi_krb5-2.dll at 00000003F2EF0000
--- Process 1812 loaded C:\cygwin64\bin\cygldap-2-4-2.dll at 00000003F26A0000
--- Process 1812 loaded C:\cygwin64\bin\cygidn2-0.dll at 00000003F2BC0000
--- Process 1812 loaded C:\cygwin64\bin\cyglber-2-4-2.dll at 00000003F2750000
--- Process 1812 loaded C:\cygwin64\bin\cygnghttp2-14.dll at 00000003ECF70000
--- Process 1812 loaded C:\cygwin64\bin\cygpsl-5.dll at 00000003EBEA0000
--- Process 1812 loaded C:\cygwin64\bin\cygssh2-1.dll at 00000003EB010000
--- Process 1812 loaded C:\cygwin64\bin\cygssl-1.0.0.dll at 00000003EAFA0000
--- Process 1812, exception c0000139 at 00007FFEA4C89968
--- Process 1812 loaded C:\cygwin64\bin\cygkrb5-3.dll at 00000003EC900000
--- Process 1812 loaded C:\cygwin64\bin\cygk5crypto-3.dll at 00000003EC9E0000
--- Process 1812 thread 21968 exited with status 0xc0000139
--- Process 1812 thread 17792 exited with status 0xc0000139
--- Process 1812 thread 29828 exited with status 0xc0000139
--- Process 1812 exited with status 0xc0000139
```

and then dies.  It's odd that it loads iconv and pcre from Sage.  Rather, it's expected in a way, but where it gets strange is that `git.exe` also loads these DLLs but it uses the versions from the system:

```
--- Process 9444 created
--- Process 9444 loaded C:\Windows\System32\ntdll.dll at 00007FFEA4BF0000
--- Process 9444 loaded C:\Windows\System32\kernel32.dll at 00007FFEA3D30000
--- Process 9444 loaded C:\Windows\System32\KernelBase.dll at 00007FFEA1B90000
--- Process 9444 thread 9700 created
--- Process 9444 thread 4776 created
--- Process 9444 thread 1016 created
--- Process 9444 loaded C:\cygwin64\bin\cygwin1.dll at 0000000180040000
--- Process 9444 loaded C:\cygwin64\bin\cygiconv-2.dll at 00000003F2530000
--- Process 9444 loaded C:\cygwin64\bin\cygintl-8.dll at 00000003ECE50000
--- Process 9444 loaded C:\cygwin64\bin\cygpcre-1.dll at 00000003EC540000
--- Process 9444 loaded C:\cygwin64\bin\cygz.dll at 00000003E8DC0000
```

It could be some subtlety about the DLL order?  That remains unclear at the moment.  I need to take a closer look at the DLL dependency trees.



---

archive/issue_comments_494946.json:
```json
{
    "body": "<a id='comment:2'></a>I think I know the reason.  It's because DLL search order normally looks first in the same directory as the executable loading the DLL, then resorts to `$PATH`.  Since `git.exe` is in `/usr/bin/` it has most of its dependent DLLs right beside it.  But executables installed under `/usr/libexec/` do not, so they find these DLLs first in `$SAGE_LOCAL/bin`.\n\nThis is definitely a bit of an oddity to git, since so much of it is installed user `/usr/libexec/git-core`.  \n\nI'm not sure much can be done about this from the Sage end of things.  Perhaps for the Sage-for-Windows build I can ensure that some DLLs are copied into `/usr/libexec/git-core/` in order for git to function properly.",
    "created_at": "2018-07-10T16:24:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494946",
    "user": "https://github.com/embray"
}
```

<a id='comment:2'></a>I think I know the reason.  It's because DLL search order normally looks first in the same directory as the executable loading the DLL, then resorts to `$PATH`.  Since `git.exe` is in `/usr/bin/` it has most of its dependent DLLs right beside it.  But executables installed under `/usr/libexec/` do not, so they find these DLLs first in `$SAGE_LOCAL/bin`.

This is definitely a bit of an oddity to git, since so much of it is installed user `/usr/libexec/git-core`.  

I'm not sure much can be done about this from the Sage end of things.  Perhaps for the Sage-for-Windows build I can ensure that some DLLs are copied into `/usr/libexec/git-core/` in order for git to function properly.



---

archive/issue_comments_494947.json:
```json
{
    "body": "<a id='comment:3'></a>Turns out this only affects those two DLLs:\n\n```\n$ cygcheck.exe /usr/libexec/git-core/*.exe | grep 'sage\\\\local' | sed 's/^ *//' | sort | uniq\nC:\\Users\\Erik M. Bray\\Home\\src\\sagemath\\sage\\local\\bin\\cygiconv-2.dll\nC:\\Users\\Erik M. Bray\\Home\\src\\sagemath\\sage\\local\\bin\\cygpcre-1.dll\n```",
    "created_at": "2018-07-10T16:27:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494947",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'></a>Turns out this only affects those two DLLs:

```
$ cygcheck.exe /usr/libexec/git-core/*.exe | grep 'sage\\local' | sed 's/^ *//' | sort | uniq
C:\Users\Erik M. Bray\Home\src\sagemath\sage\local\bin\cygiconv-2.dll
C:\Users\Erik M. Bray\Home\src\sagemath\sage\local\bin\cygpcre-1.dll
```



---

archive/issue_comments_494948.json:
```json
{
    "body": "<a id='comment:4'></a>This works as a general solution.  I'll add it to the [Cygwin64Port](Cygwin64Port) wiki page:\n\n```\nfor dllname in $(cygcheck.exe /usr/libexec/git-core/*.exe | grep 'sage\\\\local' | sed 's|.*\\\\\\(.\\+\\.dll\\)|\\1|' | sort | uniq); do\n    cp /usr/bin/$dllname /usr/libexec/git-core/\ndone\n```\n\nThis might also be worth mentioning upstream in the Cygwin git package, since this is a bit fragile.",
    "created_at": "2018-07-10T16:35:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494948",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'></a>This works as a general solution.  I'll add it to the [Cygwin64Port](Cygwin64Port) wiki page:

```
for dllname in $(cygcheck.exe /usr/libexec/git-core/*.exe | grep 'sage\\local' | sed 's|.*\\\(.\+\.dll\)|\1|' | sort | uniq); do
    cp /usr/bin/$dllname /usr/libexec/git-core/
done
```

This might also be worth mentioning upstream in the Cygwin git package, since this is a bit fragile.



---

archive/issue_comments_494949.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [comment:2 embray]:\n> I'm not sure much can be done about this from the Sage end of things.  Perhaps for the Sage-for-Windows build I can ensure that some DLLs are copied into `/usr/libexec/git-core/` in order for git to function properly.\n\n\nWell, one thing we could do is prefer not to build those libraries in the first place.  That, of course, is being worked on...",
    "created_at": "2018-07-10T16:39:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494949",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>Replying to [comment:2 embray]:
> I'm not sure much can be done about this from the Sage end of things.  Perhaps for the Sage-for-Windows build I can ensure that some DLLs are copied into `/usr/libexec/git-core/` in order for git to function properly.


Well, one thing we could do is prefer not to build those libraries in the first place.  That, of course, is being worked on...



---

archive/issue_comments_494950.json:
```json
{
    "body": "Changing branch from \"\" to \"u/mstipulanti/adding_a_note_in_the_doc_of_the_method_delta___available_in_combinat_words_finite_word\"",
    "created_at": "2018-07-11T14:40:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494950",
    "user": "https://trac.sagemath.org/admin/accounts/users/mstipulanti"
}
```

Changing branch from "" to "u/mstipulanti/adding_a_note_in_the_doc_of_the_method_delta___available_in_combinat_words_finite_word"



---

archive/issue_comments_494951.json:
```json
{
    "body": "Changing commit from \"\" to \"6398d0e3721a1ef19450ed55fe250b9dffb151f3\"",
    "created_at": "2018-07-11T15:32:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494951",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "" to "6398d0e3721a1ef19450ed55fe250b9dffb151f3"



---

archive/issue_comments_494952.json:
```json
{
    "body": "<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2018-07-11T15:32:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494952",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_events_064549.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:56:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25816#event-64549"
}
```



---

archive/issue_events_064550.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25816#event-64550"
}
```



---

archive/issue_events_064551.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "milestone": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25816#event-64551"
}
```



---

archive/issue_events_064552.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "milestone": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25816#event-64552"
}
```



---

archive/issue_events_064553.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25816#event-64553"
}
```



---

archive/issue_comments_494953.json:
```json
{
    "body": "<a id='comment:10'></a>Retargeting some of my tickets (somewhat optimistically for now).",
    "created_at": "2018-12-28T14:10:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494953",
    "user": "https://github.com/embray"
}
```

<a id='comment:10'></a>Retargeting some of my tickets (somewhat optimistically for now).



---

archive/issue_events_064554.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:44:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25816#event-64554"
}
```



---

archive/issue_events_064555.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:44:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "milestone": "sage-pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25816#event-64555"
}
```



---

archive/issue_comments_494954.json:
```json
{
    "body": "<a id='comment:11'></a>Removing most of the rest of my open tickets out of the 8.7 milestone, which should be closed.",
    "created_at": "2019-03-25T10:44:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494954",
    "user": "https://github.com/embray"
}
```

<a id='comment:11'></a>Removing most of the rest of my open tickets out of the 8.7 milestone, which should be closed.



---

archive/issue_comments_494955.json:
```json
{
    "body": "Changing branch from \"u/mstipulanti/adding_a_note_in_the_doc_of_the_method_delta___available_in_combinat_words_finite_word\" to \"\"",
    "created_at": "2019-06-15T21:36:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494955",
    "user": "https://github.com/embray"
}
```

Changing branch from "u/mstipulanti/adding_a_note_in_the_doc_of_the_method_delta___available_in_combinat_words_finite_word" to ""



---

archive/issue_comments_494956.json:
```json
{
    "body": "Changing commit from \"6398d0e3721a1ef19450ed55fe250b9dffb151f3\" to \"\"",
    "created_at": "2019-06-15T21:36:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494956",
    "user": "https://github.com/embray"
}
```

Changing commit from "6398d0e3721a1ef19450ed55fe250b9dffb151f3" to ""



---

archive/issue_comments_494957.json:
```json
{
    "body": "<a id='comment:12'></a>This branch was probably for another ticket.",
    "created_at": "2019-06-15T21:36:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494957",
    "user": "https://github.com/embray"
}
```

<a id='comment:12'></a>This branch was probably for another ticket.



---

archive/issue_comments_494958.json:
```json
{
    "body": "<a id='comment:13'></a>Replying to [comment:12 embray]:\n> This branch was probably for another ticket.\n\n\nYes, it was for #25826.",
    "created_at": "2019-06-15T23:02:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25816#issuecomment-494958",
    "user": "https://github.com/slel"
}
```

<a id='comment:13'></a>Replying to [comment:12 embray]:
> This branch was probably for another ticket.


Yes, it was for #25826.
