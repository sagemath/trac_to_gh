# Issue 25967: Optimize is_constructible_by_doublings

archive/issues_025730.json:
```json
{
    "assignees": [],
    "body": "A lattice can be built from lower pseudo-intervals iff `|Ji(L)| = |Ji (Con L)|`. But we can not have `|Ji(L)| < |Ji (Con L)|`. Hence we can stop when we found two principal congruences to be the same.\n\nAs an extreme example if `L = posets.DiamondPoset(100)` then `L.is_constructible_by_doublings('lower')` took 460 ms before the patch and now it takes 20 ms. To check for all 10-element finite lattices the time drops from 19,9 to 7,2 seconds.\n\nCC:  @fchapoton @tscrim\n\nBranch/Commit: **[460135a](https://github.com/sagemath/sagetrac-mirror/commit/460135abd0a72846aebede426b722d1de9f9c7d8)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25967_\n\n",
    "closed_at": "2018-09-08T17:55:46Z",
    "created_at": "2018-07-29T08:52:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Optimize is_constructible_by_doublings",
    "type": "issue",
    "updated_at": "2018-09-08T17:55:46Z",
    "url": "https://github.com/sagemath/sage/issues/25967",
    "user": "https://github.com/jm58660"
}
```
A lattice can be built from lower pseudo-intervals iff `|Ji(L)| = |Ji (Con L)|`. But we can not have `|Ji(L)| < |Ji (Con L)|`. Hence we can stop when we found two principal congruences to be the same.

As an extreme example if `L = posets.DiamondPoset(100)` then `L.is_constructible_by_doublings('lower')` took 460 ms before the patch and now it takes 20 ms. To check for all 10-element finite lattices the time drops from 19,9 to 7,2 seconds.

CC:  @fchapoton @tscrim

Branch/Commit: **[460135a](https://github.com/sagemath/sagetrac-mirror/commit/460135abd0a72846aebede426b722d1de9f9c7d8)**

Reviewer: **Travis Scrimshaw**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/25967_





---

archive/issue_events_340652.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-07-29T08:52:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25967#event-340652"
}
```



---

archive/issue_events_340653.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-07-29T08:52:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25967#event-340653"
}
```



---

archive/issue_events_340654.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-07-29T08:52:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25967#event-340654"
}
```



---

archive/issue_events_340655.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-07-29T08:52:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25967#event-340655"
}
```



---

archive/issue_comments_402486.json:
```json
{
    "body": "Branch: **[u/jmantysalo/optimize_is_constructible_by_doublings](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/optimize_is_constructible_by_doublings)**",
    "created_at": "2018-09-06T08:47:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402486",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/optimize_is_constructible_by_doublings](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/optimize_is_constructible_by_doublings)**



---

archive/issue_comments_402487.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/42e9acf7d29fc92307fc558b373e6d299acdcf15\">42e9acf</a></td><td><code>Short-circuit algorithm.</code></td></tr></table>\n",
    "created_at": "2018-09-06T08:51:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402487",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/42e9acf7d29fc92307fc558b373e6d299acdcf15">42e9acf</a></td><td><code>Short-circuit algorithm.</code></td></tr></table>




---

archive/issue_comments_402488.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,3 @@\n-(TBD after currently waiting tickets are merged to `hasse_diagram.py`.)\n-\n A lattice can be built from lower pseudo-intervals iff `|Ji(L)| = |Ji (Con L)|`. But we can not have `|Ji(L)| < |Ji (Con L)|`. Hence we can stop when we found two principal congruences to be the same.\n \n+As an extreme example if `L = posets.DiamondPoset(100)` then `L.is_constructible_by_doublings('lower')` took 460 ms before the patch and now it takes 20 ms. To check for all 10-element finite lattices the time drops from 19,9 to 7,2 seconds.\n``````\n",
    "created_at": "2018-09-06T08:51:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402488",
    "user": "https://github.com/jm58660"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,3 @@
-(TBD after currently waiting tickets are merged to `hasse_diagram.py`.)
-
 A lattice can be built from lower pseudo-intervals iff `|Ji(L)| = |Ji (Con L)|`. But we can not have `|Ji(L)| < |Ji (Con L)|`. Hence we can stop when we found two principal congruences to be the same.
 
+As an extreme example if `L = posets.DiamondPoset(100)` then `L.is_constructible_by_doublings('lower')` took 460 ms before the patch and now it takes 20 ms. To check for all 10-element finite lattices the time drops from 19,9 to 7,2 seconds.
``````




---

archive/issue_comments_402489.json:
```json
{
    "body": "Commit: **[42e9acf](https://github.com/sagemath/sagetrac-mirror/commit/42e9acf7d29fc92307fc558b373e6d299acdcf15)**",
    "created_at": "2018-09-06T08:51:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402489",
    "user": "https://github.com/jm58660"
}
```

Commit: **[42e9acf](https://github.com/sagemath/sagetrac-mirror/commit/42e9acf7d29fc92307fc558b373e6d299acdcf15)**



---

archive/issue_events_340656.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-09-06T08:51:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25967#event-340656"
}
```



---

archive/issue_comments_402490.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nRather than using `SetPartition`, it would be better to use lower-level Python objects:\n\n```diff\n-S = SetPartition(H.congruence([[v, next(H.neighbor_out_iterator(v))]]))\n+S = frozenset(map(frozenset, H.congruence([[v, next(H.neighbor_out_iterator(v))]])))\n```\nto avoid the overhead, including constructing the parent, all of this is in Python, and the checks of `SetPartition`.\n\nOtherwise LGTM",
    "created_at": "2018-09-06T09:26:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402490",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

Rather than using `SetPartition`, it would be better to use lower-level Python objects:

```diff
-S = SetPartition(H.congruence([[v, next(H.neighbor_out_iterator(v))]]))
+S = frozenset(map(frozenset, H.congruence([[v, next(H.neighbor_out_iterator(v))]])))
```
to avoid the overhead, including constructing the parent, all of this is in Python, and the checks of `SetPartition`.

Otherwise LGTM



---

archive/issue_comments_402491.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-09-06T09:26:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402491",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_402492.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/460135abd0a72846aebede426b722d1de9f9c7d8\">460135a</a></td><td><code>Set of sets instead of SetPartition.</code></td></tr></table>\n",
    "created_at": "2018-09-06T10:15:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402492",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/460135abd0a72846aebede426b722d1de9f9c7d8">460135a</a></td><td><code>Set of sets instead of SetPartition.</code></td></tr></table>




---

archive/issue_comments_402493.json:
```json
{
    "body": "Changed commit from **[42e9acf](https://github.com/sagemath/sagetrac-mirror/commit/42e9acf7d29fc92307fc558b373e6d299acdcf15)** to **[460135a](https://github.com/sagemath/sagetrac-mirror/commit/460135abd0a72846aebede426b722d1de9f9c7d8)**",
    "created_at": "2018-09-06T10:15:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402493",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[42e9acf](https://github.com/sagemath/sagetrac-mirror/commit/42e9acf7d29fc92307fc558b373e6d299acdcf15)** to **[460135a](https://github.com/sagemath/sagetrac-mirror/commit/460135abd0a72846aebede426b722d1de9f9c7d8)**



---

archive/issue_comments_402494.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@tscrim](#comment%3A3):\n> Rather than using `SetPartition`, it would be better to use lower-level Python objects:\n\nTrue, this gave another 8 % more speed.",
    "created_at": "2018-09-06T10:16:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402494",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@tscrim](#comment%3A3):
> Rather than using `SetPartition`, it would be better to use lower-level Python objects:

True, this gave another 8 % more speed.



---

archive/issue_events_340657.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-09-06T13:43:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25967#event-340657"
}
```



---

archive/issue_events_340658.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-09-06T13:43:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25967#event-340658"
}
```



---

archive/issue_events_340659.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-09-08T17:55:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25967#event-340659"
}
```



---

archive/issue_events_340660.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d965612cb0d6de27417505c1db83883320db2c5b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-09-08T17:55:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25967#event-340660"
}
```



---

archive/issue_comments_402495.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/optimize_is_constructible_by_doublings](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/optimize_is_constructible_by_doublings)** to **[460135a](https://github.com/sagemath/sagetrac-mirror/commit/460135abd0a72846aebede426b722d1de9f9c7d8)**",
    "created_at": "2018-09-08T17:55:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25967#issuecomment-402495",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/optimize_is_constructible_by_doublings](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/optimize_is_constructible_by_doublings)** to **[460135a](https://github.com/sagemath/sagetrac-mirror/commit/460135abd0a72846aebede426b722d1de9f9c7d8)**
