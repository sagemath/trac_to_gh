...
```json
{
    "body": "<a id='comment:5'></a>This should take care of comment:2 Eric. However, the doctest for `set_calc_order` in `tensorfield.py` and `tensorfield_paral.py` are the same. Florentin, Eric, could one of you get a modified doctest that calls that method specifically?",
...
<a id='comment:5'></a>This should take care of comment:2 Eric. However, the doctest for `set_calc_order` in `tensorfield.py` and `tensorfield_paral.py` are the same. Florentin, Eric, could one of you get a modified doctest that calls that method specifically?
...
<a id='comment:9'></a>Replying to [comment:7 egourgoulhon]:
>There are still some documentation issues, that I shall address in a next commit...
