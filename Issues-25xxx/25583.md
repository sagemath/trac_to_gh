# Issue 25583: py3: get rid of one unicode in sage_setup

archive/issues_025346.json:
```json
{
    "assignees": [],
    "body": "src/sage_setup/docbuild/ext/multidocs.py:143: undefined name 'unicode'\n\n\nBranch/Commit: **[d9f73d9](https://github.com/sagemath/sagetrac-mirror/commit/d9f73d938ab20fe4d9fc635f0219b0e9f076315c)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25583_\n\n",
    "closed_at": "2018-06-17T22:15:11Z",
    "created_at": "2018-06-15T10:00:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: get rid of one unicode in sage_setup",
    "type": "issue",
    "updated_at": "2018-06-17T22:15:11Z",
    "url": "https://github.com/sagemath/sage/issues/25583",
    "user": "https://github.com/fchapoton"
}
```
src/sage_setup/docbuild/ext/multidocs.py:143: undefined name 'unicode'


Branch/Commit: **[d9f73d9](https://github.com/sagemath/sagetrac-mirror/commit/d9f73d938ab20fe4d9fc635f0219b0e9f076315c)**

Reviewer: **Jeroen Demeyer**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/25583_





---

archive/issue_events_335834.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-15T10:00:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25583#event-335834"
}
```



---

archive/issue_events_335835.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-15T10:00:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25583#event-335835"
}
```



---

archive/issue_events_335836.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-15T10:00:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25583#event-335836"
}
```



---

archive/issue_events_335837.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-15T10:00:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25583#event-335837"
}
```



---

archive/issue_comments_395316.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d9f73d938ab20fe4d9fc635f0219b0e9f076315c\">d9f73d9</a></td><td><code>get rid of unicode()</code></td></tr></table>\n",
    "created_at": "2018-06-15T11:43:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25583#issuecomment-395316",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d9f73d938ab20fe4d9fc635f0219b0e9f076315c">d9f73d9</a></td><td><code>get rid of unicode()</code></td></tr></table>




---

archive/issue_comments_395317.json:
```json
{
    "body": "Commit: **[d9f73d9](https://github.com/sagemath/sagetrac-mirror/commit/d9f73d938ab20fe4d9fc635f0219b0e9f076315c)**",
    "created_at": "2018-06-15T11:43:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25583#issuecomment-395317",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[d9f73d9](https://github.com/sagemath/sagetrac-mirror/commit/d9f73d938ab20fe4d9fc635f0219b0e9f076315c)**



---

archive/issue_comments_395318.json:
```json
{
    "body": "Branch: **[u/chapoton/25583](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/25583)**",
    "created_at": "2018-06-15T11:43:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25583#issuecomment-395318",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/25583](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/25583)**



---

archive/issue_events_335838.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-15T11:43:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25583#event-335838"
}
```



---

archive/issue_comments_395319.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2018-06-15T11:52:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25583#issuecomment-395319",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_335839.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-15T11:52:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25583#event-335839"
}
```



---

archive/issue_events_335840.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-15T11:52:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25583#event-335840"
}
```



---

archive/issue_comments_395320.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-06-15T11:52:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25583#issuecomment-395320",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_395321.json:
```json
{
    "body": "Changed branch from **[u/chapoton/25583](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/25583)** to **[d9f73d9](https://github.com/sagemath/sagetrac-mirror/commit/d9f73d938ab20fe4d9fc635f0219b0e9f076315c)**",
    "created_at": "2018-06-17T22:15:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25583#issuecomment-395321",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/25583](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/25583)** to **[d9f73d9](https://github.com/sagemath/sagetrac-mirror/commit/d9f73d938ab20fe4d9fc635f0219b0e9f076315c)**



---

archive/issue_events_335841.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-06-17T22:15:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25583#event-335841"
}
```



---

archive/issue_events_335842.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d2b411d20ee9d0d198234d165103115624c9c775",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-06-17T22:15:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25583",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25583#event-335842"
}
```
