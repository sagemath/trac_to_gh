# Issue 25031: Invalid dependencies when building with gcc

archive/issues_024794.json:
```json
{
    "assignees": [],
    "body": "\n```\nmake[1]: *** No rule to make target `/home/buildbot/slave/sage_git/build/local/bin/gccmpir', needed by `/home/buildbot/slave/sage_git/build/local/var/lib/sage/installed/arb-2.12.0.p0'.  Stop.\n```\nFrom build/make/Makefile:\n\n```\ndeps_arb =  $(SAGE_LOCAL)/bin/gcc$(MP_LIBRARY) mpfr flint\n```\n\nCC:  @jdemeyer\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25031_\n\n",
    "closed_at": "2018-03-29T05:34:17Z",
    "created_at": "2018-03-25T10:20:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Invalid dependencies when building with gcc",
    "type": "issue",
    "updated_at": "2018-03-29T11:46:45Z",
    "url": "https://github.com/sagemath/sage/issues/25031",
    "user": "https://github.com/vbraun"
}
```

```
make[1]: *** No rule to make target `/home/buildbot/slave/sage_git/build/local/bin/gccmpir', needed by `/home/buildbot/slave/sage_git/build/local/var/lib/sage/installed/arb-2.12.0.p0'.  Stop.
```
From build/make/Makefile:

```
deps_arb =  $(SAGE_LOCAL)/bin/gcc$(MP_LIBRARY) mpfr flint
```

CC:  @jdemeyer

_Issue created by migration from https://trac.sagemath.org/ticket/25031_





---

archive/issue_events_328150.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-25T10:20:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328150"
}
```



---

archive/issue_events_328151.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-25T10:20:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328151"
}
```



---

archive/issue_events_328152.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-25T10:20:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328152"
}
```



---

archive/issue_events_328153.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-25T10:20:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328153"
}
```



---

archive/issue_comments_384060.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis line reads \"`deps_arb = $(MP_LIBRARY) mpfr flint`\" in my clang installation.",
    "created_at": "2018-03-25T12:36:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25031#issuecomment-384060",
    "user": "https://github.com/rwst"
}
```

<div id="comment:1" align="right">Comment 1</div>

This line reads "`deps_arb = $(MP_LIBRARY) mpfr flint`" in my clang installation.



---

archive/issue_comments_384061.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBecause you have a working gcc so the optional built-in gcc is not compiled...",
    "created_at": "2018-03-25T18:20:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25031#issuecomment-384061",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">Comment 2</div>

Because you have a working gcc so the optional built-in gcc is not compiled...



---

archive/issue_events_328154.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-26T11:15:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328154"
}
```



---

archive/issue_events_328155.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-26T11:15:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328155"
}
```



---

archive/issue_events_328156.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-26T11:15:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328156"
}
```



---

archive/issue_comments_384062.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThis was already fixed by #24961",
    "created_at": "2018-03-26T11:15:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25031#issuecomment-384062",
    "user": "https://github.com/embray"
}
```

<div id="comment:3" align="right">Comment 3</div>

This was already fixed by #24961



---

archive/issue_events_328157.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-26T11:15:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328157"
}
```



---

archive/issue_events_328158.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-29T05:34:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328158"
}
```



---

archive/issue_events_328159.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-29T05:34:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328159"
}
```



---

archive/issue_events_328160.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-29T05:34:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328160"
}
```



---

archive/issue_events_328161.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-29T11:46:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25031",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25031#event-328161"
}
```
