# Issue 25367: Permutations.unrank sometimes uses iteration

archive/issues_025130.json:
```json
{
    "body": "The following is almost instantaneous\n\n```\nsage: Permutations(20).unrank(2**60)\n[10, 11, 2, 8, 14, 20, 5, 17, 16, 6, 9, 19, 1, 15, 13, 3, 12, 18, 4, 7]\n```\nbut the following (which should be the same) takes forever\n\n```\nsage: Permutations(range(20)).unrank(2**60)\n```\n\nCC:  tmonteil\n\nKeywords: MathExp2018\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/25367\n\n",
    "created_at": "2018-05-15T14:01:09Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.4",
    "title": "Permutations.unrank sometimes uses iteration",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25367",
    "user": "https://github.com/videlec"
}
```
The following is almost instantaneous

```
sage: Permutations(20).unrank(2**60)
[10, 11, 2, 8, 14, 20, 5, 17, 16, 6, 9, 19, 1, 15, 13, 3, 12, 18, 4, 7]
```
but the following (which should be the same) takes forever

```
sage: Permutations(range(20)).unrank(2**60)
```

CC:  tmonteil

Keywords: MathExp2018

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/25367





---

archive/issue_comments_484620.json:
```json
{
    "body": "<a id='comment:1'></a>So this is because `Permutations(range(20))` is a completely different object/implementation and does not use the `StandardPermtutations`. IMO, what should happen is the generic permutations code should just be an adapter to `StandardPermutations` with labels.",
    "created_at": "2018-05-15T22:57:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25367",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25367#issuecomment-484620",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'></a>So this is because `Permutations(range(20))` is a completely different object/implementation and does not use the `StandardPermtutations`. IMO, what should happen is the generic permutations code should just be an adapter to `StandardPermutations` with labels.



---

archive/issue_comments_484621.json:
```json
{
    "body": "<a id='comment:2'></a>update milestone 8.3 -> 8.4",
    "created_at": "2018-08-03T19:20:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25367",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25367#issuecomment-484621",
    "user": "https://github.com/videlec"
}
```

<a id='comment:2'></a>update milestone 8.3 -> 8.4



---

archive/issue_events_063625.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25367",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25367#event-63625"
}
```
