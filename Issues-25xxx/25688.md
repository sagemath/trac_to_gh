# Issue 25688: Inaccuracy in symbolic series result on 32bit Linux

archive/issues_025451.json:
```json
{
    "assignees": [],
    "body": "From [#24838 comment:64](https://github.com/sagemath/sage/issues/24838#comment:64)\n\nOn 32-bit:\n\n```\nFile \"src/sage/functions/hypergeometric.py\", line 148, in sage.functions.hypergeometric\nFailed example:\n    hypergeometric_U(2, 2, x).series(x == 3, 100).subs(x=1).n()\nExpected:\n    0.403652637676806\nGot:\n    0.403651580752398\n```\n\nPynac is not involved in the numerics (`hypergeometric_U` is not a `GinacFunction`), so the hypothesis is that the expression before `.n()` is different with 32bit Linux pynac-0.7.22.\n\nCC:  @slel tmonteil\n\nKeywords: **pynac, 32-bit, overflow**\n\nBranch/Commit: **[689e08f](https://github.com/sagemath/sagetrac-mirror/commit/689e08f9393ac651c420546b0e63efbb1220156b)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Dave Morris**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25688_\n\n",
    "closed_at": "2022-03-09T23:37:45Z",
    "created_at": "2018-06-28T14:34:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Inaccuracy in symbolic series result on 32bit Linux",
    "type": "issue",
    "updated_at": "2022-03-09T23:37:45Z",
    "url": "https://github.com/sagemath/sage/issues/25688",
    "user": "https://github.com/rwst"
}
```
From [#24838 comment:64](https://github.com/sagemath/sage/issues/24838#comment:64)

On 32-bit:

```
File "src/sage/functions/hypergeometric.py", line 148, in sage.functions.hypergeometric
Failed example:
    hypergeometric_U(2, 2, x).series(x == 3, 100).subs(x=1).n()
Expected:
    0.403652637676806
Got:
    0.403651580752398
```

Pynac is not involved in the numerics (`hypergeometric_U` is not a `GinacFunction`), so the hypothesis is that the expression before `.n()` is different with 32bit Linux pynac-0.7.22.

CC:  @slel tmonteil

Keywords: **pynac, 32-bit, overflow**

Branch/Commit: **[689e08f](https://github.com/sagemath/sagetrac-mirror/commit/689e08f9393ac651c420546b0e63efbb1220156b)**

Reviewer: **Matthias Koeppe**

Author: **Dave Morris**

_Issue created by migration from https://trac.sagemath.org/ticket/25688_





---

archive/issue_events_337174.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-06-28T14:34:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337174"
}
```



---

archive/issue_events_337175.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-06-28T14:34:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337175"
}
```



---

archive/issue_events_337176.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-06-28T14:34:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337176"
}
```



---

archive/issue_events_337177.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-06-28T14:34:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337177"
}
```



---

archive/issue_comments_397521.json:
```json
{
    "body": "Branch: **[public/25688](https://github.com/sagemath/sagetrac-mirror/tree/public/25688)**",
    "created_at": "2021-06-19T07:50:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397521",
    "user": "https://github.com/DaveWitteMorris"
}
```

Branch: **[public/25688](https://github.com/sagemath/sagetrac-mirror/tree/public/25688)**



---

archive/issue_comments_397522.json:
```json
{
    "body": "Commit: **[e8ee3fc](https://github.com/sagemath/sagetrac-mirror/commit/e8ee3fc291a8d547459e89ad40f3cf0e7a953d93)**",
    "created_at": "2021-06-19T07:58:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397522",
    "user": "https://github.com/DaveWitteMorris"
}
```

Commit: **[e8ee3fc](https://github.com/sagemath/sagetrac-mirror/commit/e8ee3fc291a8d547459e89ad40f3cf0e7a953d93)**



---

archive/issue_comments_397523.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis bug is fixed by the pynac patch in #31585. (So the inaccuracy in the sum is due to an arithmetic overflow.) \n\nThere is only new commit in the branch here. It just deletes the \"known bug\" tag from the doctest in src/sage/functions/hypergeometric.py that is in the description of this ticket.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f1a99fa926a777a54bc78571f36efd4de9396bfd\">f1a99fa</a></td><td><code>trac 31585 pynac overflow patch</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3fcf0726cb17a701d279be7cbe724d73ec98370f\">3fcf072</a></td><td><code>add doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fdde3d9b4a16e51f22181744036920124811a1ef\">fdde3d9</a></td><td><code>update pynac patch level</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e8ee3fc291a8d547459e89ad40f3cf0e7a953d93\">e8ee3fc</a></td><td><code>trac 25688 remove \"known bug\" tag</code></td></tr></table>\n",
    "created_at": "2021-06-19T07:58:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397523",
    "user": "https://github.com/DaveWitteMorris"
}
```

<a id='comment:2'>Comment 2:</a>
This bug is fixed by the pynac patch in #31585. (So the inaccuracy in the sum is due to an arithmetic overflow.) 

There is only new commit in the branch here. It just deletes the "known bug" tag from the doctest in src/sage/functions/hypergeometric.py that is in the description of this ticket.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f1a99fa926a777a54bc78571f36efd4de9396bfd">f1a99fa</a></td><td><code>trac 31585 pynac overflow patch</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3fcf0726cb17a701d279be7cbe724d73ec98370f">3fcf072</a></td><td><code>add doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fdde3d9b4a16e51f22181744036920124811a1ef">fdde3d9</a></td><td><code>update pynac patch level</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e8ee3fc291a8d547459e89ad40f3cf0e7a953d93">e8ee3fc</a></td><td><code>trac 25688 remove "known bug" tag</code></td></tr></table>




---

archive/issue_events_337178.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2021-06-19T07:58:24Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337178"
}
```



---

archive/issue_events_337179.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2021-06-19T07:58:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337179"
}
```



---

archive/issue_comments_397524.json:
```json
{
    "body": "Changed keywords from **** to **pynac, 32-bit, overflow**",
    "created_at": "2021-06-19T07:58:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397524",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changed keywords from **** to **pynac, 32-bit, overflow**



---

archive/issue_comments_397525.json:
```json
{
    "body": "Dependencies: **#31585**",
    "created_at": "2021-06-19T07:58:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397525",
    "user": "https://github.com/DaveWitteMorris"
}
```

Dependencies: **#31585**



---

archive/issue_events_337180.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2021-06-19T07:58:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337180"
}
```



---

archive/issue_comments_397526.json:
```json
{
    "body": "Author: **Dave Morris**",
    "created_at": "2021-06-19T07:58:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397526",
    "user": "https://github.com/DaveWitteMorris"
}
```

Author: **Dave Morris**



---

archive/issue_comments_397527.json:
```json
{
    "body": "Changed branch from **[public/25688](https://github.com/sagemath/sagetrac-mirror/tree/public/25688)** to **[public/25688r1](https://github.com/sagemath/sagetrac-mirror/tree/public/25688r1)**",
    "created_at": "2021-06-28T05:04:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397527",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changed branch from **[public/25688](https://github.com/sagemath/sagetrac-mirror/tree/public/25688)** to **[public/25688r1](https://github.com/sagemath/sagetrac-mirror/tree/public/25688r1)**



---

archive/issue_comments_397528.json:
```json
{
    "body": "Changed commit from **[e8ee3fc](https://github.com/sagemath/sagetrac-mirror/commit/e8ee3fc291a8d547459e89ad40f3cf0e7a953d93)** to **[73e975c](https://github.com/sagemath/sagetrac-mirror/commit/73e975c28997f6662454b3b53dfe2cdebd22f410)**",
    "created_at": "2021-06-28T05:06:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397528",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changed commit from **[e8ee3fc](https://github.com/sagemath/sagetrac-mirror/commit/e8ee3fc291a8d547459e89ad40f3cf0e7a953d93)** to **[73e975c](https://github.com/sagemath/sagetrac-mirror/commit/73e975c28997f6662454b3b53dfe2cdebd22f410)**



---

archive/issue_comments_397529.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nRebased on the new #31585 that eliminates a merge conflict.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ecb99c85602cda86f775cff70003cfa548fd9ad4\">ecb99c8</a></td><td><code>trac 31585 pynac overflow patch</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2d04fa4a45f277a2deabeda50cf0f3d9c4a58590\">2d04fa4</a></td><td><code>add doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5a340ce206534e0a48d48edeb48197906fb9812a\">5a340ce</a></td><td><code>update pynac patch level</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/73e975c28997f6662454b3b53dfe2cdebd22f410\">73e975c</a></td><td><code>trac 25688 remove \"known bug\" tag</code></td></tr></table>\n",
    "created_at": "2021-06-28T05:06:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397529",
    "user": "https://github.com/DaveWitteMorris"
}
```

<a id='comment:4'>Comment 4:</a>
Rebased on the new #31585 that eliminates a merge conflict.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ecb99c85602cda86f775cff70003cfa548fd9ad4">ecb99c8</a></td><td><code>trac 31585 pynac overflow patch</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2d04fa4a45f277a2deabeda50cf0f3d9c4a58590">2d04fa4</a></td><td><code>add doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5a340ce206534e0a48d48edeb48197906fb9812a">5a340ce</a></td><td><code>update pynac patch level</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/73e975c28997f6662454b3b53dfe2cdebd22f410">73e975c</a></td><td><code>trac 25688 remove "known bug" tag</code></td></tr></table>




---

archive/issue_events_337181.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-07-24T14:55:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337181"
}
```



---

archive/issue_events_337182.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-07-24T14:55:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337182"
}
```



---

archive/issue_comments_397530.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nPlease rebase.",
    "created_at": "2021-07-24T14:55:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397530",
    "user": "https://github.com/slel"
}
```

<a id='comment:5'>Comment 5:</a>
Please rebase.



---

archive/issue_comments_397531.json:
```json
{
    "body": "Changed branch from **[public/25688r1](https://github.com/sagemath/sagetrac-mirror/tree/public/25688r1)** to **[public/25688r2](https://github.com/sagemath/sagetrac-mirror/tree/public/25688r2)**",
    "created_at": "2021-07-25T02:04:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397531",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changed branch from **[public/25688r1](https://github.com/sagemath/sagetrac-mirror/tree/public/25688r1)** to **[public/25688r2](https://github.com/sagemath/sagetrac-mirror/tree/public/25688r2)**



---

archive/issue_comments_397532.json:
```json
{
    "body": "Changed dependencies from **#31585** to none",
    "created_at": "2021-07-25T02:09:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397532",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changed dependencies from **#31585** to none



---

archive/issue_comments_397533.json:
```json
{
    "body": "Changed commit from **[73e975c](https://github.com/sagemath/sagetrac-mirror/commit/73e975c28997f6662454b3b53dfe2cdebd22f410)** to **[689e08f](https://github.com/sagemath/sagetrac-mirror/commit/689e08f9393ac651c420546b0e63efbb1220156b)**",
    "created_at": "2021-07-25T02:09:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397533",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changed commit from **[73e975c](https://github.com/sagemath/sagetrac-mirror/commit/73e975c28997f6662454b3b53dfe2cdebd22f410)** to **[689e08f](https://github.com/sagemath/sagetrac-mirror/commit/689e08f9393ac651c420546b0e63efbb1220156b)**



---

archive/issue_comments_397534.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThanks for pointing out the merge conflict -- I hadn't noticed. I rebased on 9.4b6 (and deleted the dependency on #31585, which has already been merged).\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/689e08f9393ac651c420546b0e63efbb1220156b\">689e08f</a></td><td><code>trac 25688 remove \"known bug\" tag</code></td></tr></table>\n",
    "created_at": "2021-07-25T02:09:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397534",
    "user": "https://github.com/DaveWitteMorris"
}
```

<a id='comment:7'>Comment 7:</a>
Thanks for pointing out the merge conflict -- I hadn't noticed. I rebased on 9.4b6 (and deleted the dependency on #31585, which has already been merged).

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/689e08f9393ac651c420546b0e63efbb1220156b">689e08f</a></td><td><code>trac 25688 remove "known bug" tag</code></td></tr></table>




---

archive/issue_events_337183.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2021-07-25T02:09:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337183"
}
```



---

archive/issue_events_337184.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2021-07-25T02:09:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337184"
}
```



---

archive/issue_events_337185.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337185"
}
```



---

archive/issue_events_337186.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337186"
}
```



---

archive/issue_events_337187.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337187"
}
```



---

archive/issue_events_337188.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337188"
}
```



---

archive/issue_comments_397535.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nStalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.",
    "created_at": "2021-12-18T19:53:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397535",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'>Comment 10:</a>
Stalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.



---

archive/issue_events_337189.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-08T06:34:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337189"
}
```



---

archive/issue_events_337190.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-08T06:34:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337190"
}
```



---

archive/issue_comments_397536.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2022-03-08T06:34:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397536",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_397537.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nThanks!",
    "created_at": "2022-03-08T06:54:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397537",
    "user": "https://github.com/DaveWitteMorris"
}
```

<a id='comment:12'>Comment 12:</a>
Thanks!



---

archive/issue_comments_397538.json:
```json
{
    "body": "Changed branch from **[public/25688r2](https://github.com/sagemath/sagetrac-mirror/tree/public/25688r2)** to **[689e08f](https://github.com/sagemath/sagetrac-mirror/commit/689e08f9393ac651c420546b0e63efbb1220156b)**",
    "created_at": "2022-03-09T23:37:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25688#issuecomment-397538",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/25688r2](https://github.com/sagemath/sagetrac-mirror/tree/public/25688r2)** to **[689e08f](https://github.com/sagemath/sagetrac-mirror/commit/689e08f9393ac651c420546b0e63efbb1220156b)**



---

archive/issue_events_337191.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-03-09T23:37:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337191"
}
```



---

archive/issue_events_337192.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4b99c49c2492657311636da402919024bd63ba9a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-03-09T23:37:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25688",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25688#event-337192"
}
```
