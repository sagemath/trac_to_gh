# Issue 25285: Upgrade to Python 3.7.0

archive/issues_025048.json:
```json
{
    "assignees": [],
    "body": "The goal of this ticket is to upgrade to Python 3.7.0 when it is released.\n\n- [Release announcement for Python 3.7.0b4, the final beta release for Python 3](https://mail.python.org/pipermail/python-announce-list/2018-May/011918.html)\n- [Python 3.7.0b4 release](https://www.python.org/downloads/release/python-370b4/)\n\n**Tarball**\n\n- Gzipped source tarball for Python 3.7.0b4: https://www.python.org/ftp/python/3.7.0/Python-3.7.0b4.tgz\n\nCC:  @fchapoton @embray @kiwifb @jdemeyer @mkoeppe @slel\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25285_\n\n",
    "closed_at": "2018-06-28T15:49:27Z",
    "created_at": "2018-05-03T12:36:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Upgrade to Python 3.7.0",
    "type": "issue",
    "updated_at": "2018-06-28T15:49:27Z",
    "url": "https://github.com/sagemath/sage/issues/25285",
    "user": "https://github.com/slel"
}
```
The goal of this ticket is to upgrade to Python 3.7.0 when it is released.

- [Release announcement for Python 3.7.0b4, the final beta release for Python 3](https://mail.python.org/pipermail/python-announce-list/2018-May/011918.html)
- [Python 3.7.0b4 release](https://www.python.org/downloads/release/python-370b4/)

**Tarball**

- Gzipped source tarball for Python 3.7.0b4: https://www.python.org/ftp/python/3.7.0/Python-3.7.0b4.tgz

CC:  @fchapoton @embray @kiwifb @jdemeyer @mkoeppe @slel

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/25285_





---

archive/issue_events_349893.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-05-03T12:36:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25285",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25285#event-349893"
}
```



---

archive/issue_events_349894.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-05-03T12:36:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25285",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25285#event-349894"
}
```



---

archive/issue_events_349895.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-05-03T12:36:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25285",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25285#event-349895"
}
```



---

archive/issue_events_349896.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-05-03T12:36:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25285",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25285#event-349896"
}
```



---

archive/issue_events_349897.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-28T15:49:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25285",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25285#event-349897"
}
```



---

archive/issue_events_349898.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-28T15:49:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25285",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25285#event-349898"
}
```



---

archive/issue_comments_386680.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nDuplicate of #25680",
    "created_at": "2018-06-28T15:49:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25285#issuecomment-386680",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">comment:1</div>

Duplicate of #25680



---

archive/issue_events_349899.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-28T15:49:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25285",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25285#event-349899"
}
```



---

archive/issue_events_349900.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-28T15:49:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25285",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25285#event-349900"
}
```
