# Issue 25005: integrals with real variables

archive/issues_024768.json:
```json
{
    "assignees": [],
    "body": "It seems to me that\n\n```\nassume(x, 'real')\nintegral(1/x, x)\n```\nshould return\n\n```\nlog(abs(x))\n```\nbut it returns\n\n```\nlog(x)\n```\nBut maybe I am missing something.\n\nCC:  @slel\n\nKeywords: **integral real variable**\n\nComponent: **calculus**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25005_\n\n",
    "created_at": "2018-03-18T16:55:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20calculus",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/needs%20review"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "integrals with real variables",
    "type": "issue",
    "updated_at": "2022-01-05T20:35:04Z",
    "url": "https://github.com/sagemath/sage/issues/25005",
    "user": "https://github.com/matswb"
}
```
It seems to me that

```
assume(x, 'real')
integral(1/x, x)
```
should return

```
log(abs(x))
```
but it returns

```
log(x)
```
But maybe I am missing something.

CC:  @slel

Keywords: **integral real variable**

Component: **calculus**

_Issue created by migration from https://trac.sagemath.org/ticket/25005_





---

archive/issue_events_345831.json:
```json
{
    "actor": "https://github.com/matswb",
    "created_at": "2018-03-18T16:55:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25005#event-345831"
}
```



---

archive/issue_events_345832.json:
```json
{
    "actor": "https://github.com/matswb",
    "created_at": "2018-03-18T16:55:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25005#event-345832"
}
```



---

archive/issue_events_345833.json:
```json
{
    "actor": "https://github.com/matswb",
    "created_at": "2018-03-18T16:55:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25005#event-345833"
}
```



---

archive/issue_events_345834.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-01-05T19:51:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25005#event-345834"
}
```



---

archive/issue_events_345835.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-01-05T19:51:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25005#event-345835"
}
```



---

archive/issue_events_345836.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-01-05T19:51:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25005#event-345836"
}
```



---

archive/issue_events_345837.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-01-05T19:51:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25005#event-345837"
}
```



---

archive/issue_comments_380864.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\non the negative real axis, log(abs(x)) and log(x) differ by a multiple of `I pi`, so they are equally valid primitives.",
    "created_at": "2022-01-05T19:51:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25005#issuecomment-380864",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">comment:1</div>

on the negative real axis, log(abs(x)) and log(x) differ by a multiple of `I pi`, so they are equally valid primitives.



---

archive/issue_comments_380865.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nDo we need a new `real_valued_integral` function?",
    "created_at": "2022-01-05T20:34:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25005#issuecomment-380865",
    "user": "https://github.com/slel"
}
```

<div id="comment:2" align="right">comment:2</div>

Do we need a new `real_valued_integral` function?



---

archive/issue_events_345838.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2022-01-05T20:35:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25005",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25005#event-345838"
}
```
