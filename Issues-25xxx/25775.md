# Issue 25775: fix uninstall script (cur_dir -> dirname)

archive/issues_025538.json:
```json
{
    "assignees": [],
    "body": "There was a mistake in the uninstall script (mispelled variable).\n\nCC:  @embray @jdemeyer\n\nBranch/Commit: **[u/vdelecroix/25775](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/25775) @ [50172dd](https://github.com/sagemath/sagetrac-mirror/commit/50172dd801cc4671a27a304bc1135c5993bf4630)**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25775_\n\n",
    "closed_at": "2018-07-05T08:16:30Z",
    "created_at": "2018-07-05T06:35:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix uninstall script (cur_dir -> dirname)",
    "type": "issue",
    "updated_at": "2018-07-05T08:16:30Z",
    "url": "https://github.com/sagemath/sage/issues/25775",
    "user": "https://github.com/videlec"
}
```
There was a mistake in the uninstall script (mispelled variable).

CC:  @embray @jdemeyer

Branch/Commit: **[u/vdelecroix/25775](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/25775) @ [50172dd](https://github.com/sagemath/sagetrac-mirror/commit/50172dd801cc4671a27a304bc1135c5993bf4630)**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/25775_





---

archive/issue_events_338233.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-07-05T06:35:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338233"
}
```



---

archive/issue_events_338234.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-07-05T06:35:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338234"
}
```



---

archive/issue_events_338235.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-07-05T06:35:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338235"
}
```



---

archive/issue_events_338236.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-07-05T06:35:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338236"
}
```



---

archive/issue_comments_398665.json:
```json
{
    "body": "Commit: **[50172dd](https://github.com/sagemath/sagetrac-mirror/commit/50172dd801cc4671a27a304bc1135c5993bf4630)**",
    "created_at": "2018-07-05T06:35:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25775#issuecomment-398665",
    "user": "https://github.com/videlec"
}
```

Commit: **[50172dd](https://github.com/sagemath/sagetrac-mirror/commit/50172dd801cc4671a27a304bc1135c5993bf4630)**



---

archive/issue_comments_398666.json:
```json
{
    "body": "Branch: **[u/vdelecroix/25775](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/25775)**",
    "created_at": "2018-07-05T06:35:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25775#issuecomment-398666",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/25775](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/25775)**



---

archive/issue_events_338237.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-07-05T06:35:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338237"
}
```



---

archive/issue_comments_398667.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/50172dd801cc4671a27a304bc1135c5993bf4630\">50172dd</a></td><td><code>fix uninstall script</code></td></tr></table>\n",
    "created_at": "2018-07-05T06:35:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25775#issuecomment-398667",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/50172dd801cc4671a27a304bc1135c5993bf4630">50172dd</a></td><td><code>fix uninstall script</code></td></tr></table>




---

archive/issue_comments_398668.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nDuplicate of #25731",
    "created_at": "2018-07-05T08:16:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25775#issuecomment-398668",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

Duplicate of #25731



---

archive/issue_events_338238.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-05T08:16:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338238"
}
```



---

archive/issue_events_338239.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-05T08:16:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338239"
}
```



---

archive/issue_events_338240.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-05T08:16:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338240"
}
```



---

archive/issue_events_338241.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-05T08:16:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338241"
}
```



---

archive/issue_events_338242.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-05T08:16:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25775",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25775#event-338242"
}
```
