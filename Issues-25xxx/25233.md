...
```json
{
    "body": "<a id='comment:1'></a>I guess the natural fix would be to add support for repeated variables (renaming some automatically if necessary) in `FlatteningMorphism`, and use that in `Polynomial.gcd()` and `MPolynomial.gcd())`.",
...
<a id='comment:1'></a>I guess the natural fix would be to add support for repeated variables (renaming some automatically if necessary) in `FlatteningMorphism`, and use that in `Polynomial.gcd()` and `MPolynomial.gcd())`.
...
```json
{
    "body": "<a id='comment:2'></a>not finished yet\n\n---\nNew commits:",
    "created_at": "2018-04-23T19:25:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25233",
    "type": "issue_comment",
...
<a id='comment:2'></a>not finished yet
...
```json
{
    "body": "<a id='comment:9'></a>Moving the last two commits (plus further improvements) to a separate ticket #25346.",
...
<a id='comment:9'></a>Moving the last two commits (plus further improvements) to a separate ticket #25346.
...
```json
{
    "body": "<a id='comment:10'></a>A small performance notice: `hasattr` should be avoided as much as possible (it costs a dictionary lookup). It is better to replace\n\n```\nif hasattr(a, 'f'):\n    return a.f()\nelse:\n    raise NotImplementedError\n```\nby\n\n```\ntry:\n    return a.f()\nexcept AttributeError:\n    raise NotImplementedError\n```",
...
<a id='comment:10'></a>A small performance notice: `hasattr` should be avoided as much as possible (it costs a dictionary lookup). It is better to replace
...
```json
{
    "body": "<a id='comment:11'></a>(rest looks good)",
    "created_at": "2018-05-12T19:25:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25233",
    "type": "issue_comment",
...
<a id='comment:11'></a>(rest looks good)
...
<a id='comment:13'></a>adding my comment on #25346 here as this is where that error was added.
...
<a id='comment:15'></a>Replying to [bhutz](#comment%3A13):
> However, I'd like to know what prompted the addition of ArithmeticError? to the gcd check in dehomomgenize for affine morphisms.
...
<a id='comment:16'></a>Looking at that example failure, here is what is boils down to: the following gets the ArithmeticError
...
<a id='comment:20'></a>Replying to [bhutz](#comment%3A16):
> Looking at little further into the example, it is interesting that if you make either R or S a univariate polynomial ring, then the gcd computes
> 
> ```
> R.<d,c>=QQbar[]
...
<a id='comment:22'></a>Replying to [bhutz](#comment%3A16):
> it is interesting that if you make either R or S a univariate polynomial ring, then the gcd computes
...
<a id='comment:23'></a>Replying to [vdelecroix](#comment%3A11):
> (rest looks good)
