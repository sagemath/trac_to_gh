...
```json
{
    "body": "<a id='comment:10'></a>\nAdditional notes : when converting `b` in `component_function` from a list/tuple of bits to integer, you may use `self.from_bits()` inside the SBox module since it takes care of the \"endianness\". By default [SageMath](../wikis/SageMath) use little-endian ordering (i.e., the leftmost bit in a list/tuple/vector is the least-significant bit).",
