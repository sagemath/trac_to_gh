# Issue 25495: Get rid of SAGE_ENV_VERSION

archive/issues_025258.json:
```json
{
    "body": "`src/bin/sage-env` contains versioning logic, which is meant to ensure that environment variables are reinitialized in the middle of an upgrade. This appears to be obsolete because `sage --upgrade` makes sure that `sage-env` is not run.\n\n\n\nCC:  @jdemeyer @embray\n\nReviewer: Dima Pasechnik\n\nDependencies: #32745\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/25495\n\n",
    "closed_at": "2021-12-03T18:41:01Z",
    "created_at": "2018-06-02T04:10:29Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Get rid of SAGE_ENV_VERSION",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25495",
    "user": "https://github.com/mkoeppe"
}
```
`src/bin/sage-env` contains versioning logic, which is meant to ensure that environment variables are reinitialized in the middle of an upgrade. This appears to be obsolete because `sage --upgrade` makes sure that `sage-env` is not run.



CC:  @jdemeyer @embray

Reviewer: Dima Pasechnik

Dependencies: #32745

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/25495





---

archive/issue_comments_488053.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#32745\"",
    "created_at": "2021-12-02T01:39:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25495#issuecomment-488053",
    "user": "https://github.com/mkoeppe"
}
```

Changing dependencies from "" to "#32745"



---

archive/issue_comments_488054.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2021-12-02T01:39:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25495#issuecomment-488054",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_events_063958.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-02T01:39:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25495",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25495#event-63958"
}
```



---

archive/issue_comments_488055.json:
```json
{
    "body": "<a id='comment:1'></a>Outdated after #32745, which extends `SAGE_ENV_VERSION` for a new purpose",
    "created_at": "2021-12-02T01:39:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25495#issuecomment-488055",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'></a>Outdated after #32745, which extends `SAGE_ENV_VERSION` for a new purpose



---

archive/issue_comments_488056.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2021-12-02T23:31:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25495#issuecomment-488056",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_488057.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Dima Pasechnik\"",
    "created_at": "2021-12-02T23:31:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25495#issuecomment-488057",
    "user": "https://github.com/dimpase"
}
```

Changing reviewer from "" to "Dima Pasechnik"



---

archive/issue_events_063959.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-03T18:41:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/25495",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25495#event-63959"
}
```



---

archive/issue_comments_488058.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2021-12-03T18:41:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25495#issuecomment-488058",
    "user": "https://github.com/mkoeppe"
}
```

Resolution: invalid
