# Issue 25397: Smaller coefficients in element_with_valuation for function field valuations

archive/issues_025160.json:
```json
{
    "assignees": [],
    "body": "Currently,\n\n```\nsage: K.<a> = NumberField(x^3+6)\nsage: v = QQ.valuation(2).extension(K)\nsage: R.<x> = K[]\nsage: w = GaussValuation(R, v).augmentation(x, 1/123)\nsage: K.<x> = FunctionField(K)\nsage: w = w.extension(K)\nsage: w.element_with_valuation(122/123)\nx^122\nsage: w.element_with_valuation(1)\nx^123\n```\n\nOf course `2/x` and `2` would be much better choices here.\n\nCC:  swewers\n\nBranch/Commit: **[e4209bb](https://github.com/sagemath/sagetrac-mirror/commit/e4209bb671f0be9621b26b844cc9e19263f35383)**\n\nReviewer: **Stefan Wewers**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25397_\n\n",
    "closed_at": "2018-08-05T08:18:17Z",
    "created_at": "2018-05-19T02:47:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Smaller coefficients in element_with_valuation for function field valuations",
    "type": "issue",
    "updated_at": "2018-08-05T08:18:17Z",
    "url": "https://github.com/sagemath/sage/issues/25397",
    "user": "https://github.com/saraedum"
}
```
Currently,

```
sage: K.<a> = NumberField(x^3+6)
sage: v = QQ.valuation(2).extension(K)
sage: R.<x> = K[]
sage: w = GaussValuation(R, v).augmentation(x, 1/123)
sage: K.<x> = FunctionField(K)
sage: w = w.extension(K)
sage: w.element_with_valuation(122/123)
x^122
sage: w.element_with_valuation(1)
x^123
```

Of course `2/x` and `2` would be much better choices here.

CC:  swewers

Branch/Commit: **[e4209bb](https://github.com/sagemath/sagetrac-mirror/commit/e4209bb671f0be9621b26b844cc9e19263f35383)**

Reviewer: **Stefan Wewers**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/25397_





---

archive/issue_events_333458.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-19T02:47:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333458"
}
```



---

archive/issue_events_333459.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-19T02:47:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333459"
}
```



---

archive/issue_events_333460.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-19T02:47:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333460"
}
```



---

archive/issue_events_333461.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-19T02:47:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333461"
}
```



---

archive/issue_comments_391611.json:
```json
{
    "body": "Branch: **[u/saraedum/25397](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/25397)**",
    "created_at": "2018-05-19T02:49:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391611",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/25397](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/25397)**



---

archive/issue_comments_391612.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/59221ba0947f130cbec9d41956e229f9611b07e9\">59221ba</a></td><td><code>Implement restrictions/element_with_valuation in more places</code></td></tr></table>\n",
    "created_at": "2018-05-19T14:11:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391612",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/59221ba0947f130cbec9d41956e229f9611b07e9">59221ba</a></td><td><code>Implement restrictions/element_with_valuation in more places</code></td></tr></table>




---

archive/issue_comments_391613.json:
```json
{
    "body": "Commit: **[59221ba](https://github.com/sagemath/sagetrac-mirror/commit/59221ba0947f130cbec9d41956e229f9611b07e9)**",
    "created_at": "2018-05-19T14:11:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391613",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[59221ba](https://github.com/sagemath/sagetrac-mirror/commit/59221ba0947f130cbec9d41956e229f9611b07e9)**



---

archive/issue_comments_391614.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f87b7941d7a8a339cde98a58f45f1807d6d195f4\">f87b794</a></td><td><code>Merge remote-tracking branch 'trac/develop' into 25397</code></td></tr></table>\n",
    "created_at": "2018-05-28T12:04:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391614",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f87b7941d7a8a339cde98a58f45f1807d6d195f4">f87b794</a></td><td><code>Merge remote-tracking branch 'trac/develop' into 25397</code></td></tr></table>




---

archive/issue_comments_391615.json:
```json
{
    "body": "Changed commit from **[59221ba](https://github.com/sagemath/sagetrac-mirror/commit/59221ba0947f130cbec9d41956e229f9611b07e9)** to **[f87b794](https://github.com/sagemath/sagetrac-mirror/commit/f87b7941d7a8a339cde98a58f45f1807d6d195f4)**",
    "created_at": "2018-05-28T12:04:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391615",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[59221ba](https://github.com/sagemath/sagetrac-mirror/commit/59221ba0947f130cbec9d41956e229f9611b07e9)** to **[f87b794](https://github.com/sagemath/sagetrac-mirror/commit/f87b7941d7a8a339cde98a58f45f1807d6d195f4)**



---

archive/issue_comments_391616.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThis needs review but I have not run doctests yet. Let's see what the patchbot says.",
    "created_at": "2018-05-28T12:05:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391616",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:4'>Comment 4:</a>
This needs review but I have not run doctests yet. Let's see what the patchbot says.



---

archive/issue_events_333462.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-28T12:05:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333462"
}
```



---

archive/issue_events_333463.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-06-11T07:48:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333463"
}
```



---

archive/issue_events_333464.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-06-11T07:48:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333464"
}
```



---

archive/issue_comments_391617.json:
```json
{
    "body": "Work Issues: **merge develop to make build pass**",
    "created_at": "2018-06-11T07:48:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391617",
    "user": "https://github.com/saraedum"
}
```

Work Issues: **merge develop to make build pass**



---

archive/issue_comments_391618.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/076b3697194da9fe9c325cd526a23ac5d671f7db\">076b369</a></td><td><code>Merge remote-tracking branch 'trac/develop' into 25397</code></td></tr></table>\n",
    "created_at": "2018-06-11T16:49:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391618",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/076b3697194da9fe9c325cd526a23ac5d671f7db">076b369</a></td><td><code>Merge remote-tracking branch 'trac/develop' into 25397</code></td></tr></table>




---

archive/issue_comments_391619.json:
```json
{
    "body": "Changed commit from **[f87b794](https://github.com/sagemath/sagetrac-mirror/commit/f87b7941d7a8a339cde98a58f45f1807d6d195f4)** to **[076b369](https://github.com/sagemath/sagetrac-mirror/commit/076b3697194da9fe9c325cd526a23ac5d671f7db)**",
    "created_at": "2018-06-11T16:49:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391619",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f87b794](https://github.com/sagemath/sagetrac-mirror/commit/f87b7941d7a8a339cde98a58f45f1807d6d195f4)** to **[076b369](https://github.com/sagemath/sagetrac-mirror/commit/076b3697194da9fe9c325cd526a23ac5d671f7db)**



---

archive/issue_events_333465.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-06-11T16:50:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333465"
}
```



---

archive/issue_events_333466.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-06-11T16:50:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333466"
}
```



---

archive/issue_comments_391620.json:
```json
{
    "body": "Changed work issues from **merge develop to make build pass** to **is the patchbot happy?**",
    "created_at": "2018-06-11T16:50:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391620",
    "user": "https://github.com/saraedum"
}
```

Changed work issues from **merge develop to make build pass** to **is the patchbot happy?**



---

archive/issue_comments_391621.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/076b3697194da9fe9c325cd526a23ac5d671f7db\">076b369</a></td><td><code>Merge remote-tracking branch 'trac/develop' into 25397</code></td></tr></table>\n",
    "created_at": "2018-06-11T16:50:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391621",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:7'>Comment 7:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/076b3697194da9fe9c325cd526a23ac5d671f7db">076b369</a></td><td><code>Merge remote-tracking branch 'trac/develop' into 25397</code></td></tr></table>




---

archive/issue_comments_391622.json:
```json
{
    "body": "Changed commit from **[076b369](https://github.com/sagemath/sagetrac-mirror/commit/076b3697194da9fe9c325cd526a23ac5d671f7db)** to **[4cf5eea](https://github.com/sagemath/sagetrac-mirror/commit/4cf5eea905adcf81edf87449fb9f752680b2c0c2)**",
    "created_at": "2018-07-04T16:38:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391622",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[076b369](https://github.com/sagemath/sagetrac-mirror/commit/076b3697194da9fe9c325cd526a23ac5d671f7db)** to **[4cf5eea](https://github.com/sagemath/sagetrac-mirror/commit/4cf5eea905adcf81edf87449fb9f752680b2c0c2)**



---

archive/issue_comments_391623.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4cf5eea905adcf81edf87449fb9f752680b2c0c2\">4cf5eea</a></td><td><code>Merge develop and 25397</code></td></tr></table>\n",
    "created_at": "2018-07-04T16:38:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391623",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:8'>Comment 8:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4cf5eea905adcf81edf87449fb9f752680b2c0c2">4cf5eea</a></td><td><code>Merge develop and 25397</code></td></tr></table>




---

archive/issue_comments_391624.json:
```json
{
    "body": "Dependencies: **#25607**",
    "created_at": "2018-07-04T16:46:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391624",
    "user": "https://github.com/saraedum"
}
```

Dependencies: **#25607**



---

archive/issue_comments_391625.json:
```json
{
    "body": "Changed dependencies from **#25607** to none",
    "created_at": "2018-07-04T16:46:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391625",
    "user": "https://github.com/saraedum"
}
```

Changed dependencies from **#25607** to none



---

archive/issue_comments_391626.json:
```json
{
    "body": "Changed commit from **[4cf5eea](https://github.com/sagemath/sagetrac-mirror/commit/4cf5eea905adcf81edf87449fb9f752680b2c0c2)** to **[32ef392](https://github.com/sagemath/sagetrac-mirror/commit/32ef392ca225362f00784c0bd6dd9339f2cbba58)**",
    "created_at": "2018-07-04T16:50:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391626",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4cf5eea](https://github.com/sagemath/sagetrac-mirror/commit/4cf5eea905adcf81edf87449fb9f752680b2c0c2)** to **[32ef392](https://github.com/sagemath/sagetrac-mirror/commit/32ef392ca225362f00784c0bd6dd9339f2cbba58)**



---

archive/issue_comments_391627.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/af7335031a3d048d383720b3cf70e20c4d739885\">af73350</a></td><td><code>Fix bug in scaling of trivial valuations</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/32ef392ca225362f00784c0bd6dd9339f2cbba58\">32ef392</a></td><td><code>fix doctests outputs with smaller coefficients</code></td></tr></table>\n",
    "created_at": "2018-07-04T16:50:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391627",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/af7335031a3d048d383720b3cf70e20c4d739885">af73350</a></td><td><code>Fix bug in scaling of trivial valuations</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/32ef392ca225362f00784c0bd6dd9339f2cbba58">32ef392</a></td><td><code>fix doctests outputs with smaller coefficients</code></td></tr></table>




---

archive/issue_comments_391628.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nOne patchbot has its pyflakes plugin red. Could you take care of that ?",
    "created_at": "2018-07-24T15:34:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391628",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:12'>Comment 12:</a>
One patchbot has its pyflakes plugin red. Could you take care of that ?



---

archive/issue_comments_391629.json:
```json
{
    "body": "Changed work issues from **is the patchbot happy?** to none",
    "created_at": "2018-07-26T16:54:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391629",
    "user": "https://github.com/saraedum"
}
```

Changed work issues from **is the patchbot happy?** to none



---

archive/issue_comments_391630.json:
```json
{
    "body": "Changed commit from **[32ef392](https://github.com/sagemath/sagetrac-mirror/commit/32ef392ca225362f00784c0bd6dd9339f2cbba58)** to **[e4209bb](https://github.com/sagemath/sagetrac-mirror/commit/e4209bb671f0be9621b26b844cc9e19263f35383)**",
    "created_at": "2018-07-26T17:00:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391630",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[32ef392](https://github.com/sagemath/sagetrac-mirror/commit/32ef392ca225362f00784c0bd6dd9339f2cbba58)** to **[e4209bb](https://github.com/sagemath/sagetrac-mirror/commit/e4209bb671f0be9621b26b844cc9e19263f35383)**



---

archive/issue_comments_391631.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e4209bb671f0be9621b26b844cc9e19263f35383\">e4209bb</a></td><td><code>Silence pyflakes</code></td></tr></table>\n",
    "created_at": "2018-07-26T17:00:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391631",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:14'>Comment 14:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e4209bb671f0be9621b26b844cc9e19263f35383">e4209bb</a></td><td><code>Silence pyflakes</code></td></tr></table>




---

archive/issue_comments_391632.json:
```json
{
    "body": "Work Issues: **is the patchbot happy \u21d2 positive review**",
    "created_at": "2018-07-26T17:00:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391632",
    "user": "https://github.com/saraedum"
}
```

Work Issues: **is the patchbot happy ⇒ positive review**



---

archive/issue_comments_391633.json:
```json
{
    "body": "Changed work issues from **is the patchbot happy \u21d2 positive review** to none",
    "created_at": "2018-07-26T19:08:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391633",
    "user": "https://github.com/sagetrac-swewers"
}
```

Changed work issues from **is the patchbot happy ⇒ positive review** to none



---

archive/issue_comments_391634.json:
```json
{
    "body": "Reviewer: **Stefan Wewers**",
    "created_at": "2018-07-26T19:08:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391634",
    "user": "https://github.com/sagetrac-swewers"
}
```

Reviewer: **Stefan Wewers**



---

archive/issue_events_333467.json:
```json
{
    "actor": "https://github.com/sagetrac-swewers",
    "created_at": "2018-07-26T19:08:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333467"
}
```



---

archive/issue_events_333468.json:
```json
{
    "actor": "https://github.com/sagetrac-swewers",
    "created_at": "2018-07-26T19:08:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333468"
}
```



---

archive/issue_events_333469.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-05T08:18:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333469"
}
```



---

archive/issue_events_333470.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c7499c1fc2d7e85c99272342facf30f8b7df86b6",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-08-05T08:18:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25397#event-333470"
}
```



---

archive/issue_comments_391635.json:
```json
{
    "body": "Changed branch from **[u/saraedum/25397](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/25397)** to **[e4209bb](https://github.com/sagemath/sagetrac-mirror/commit/e4209bb671f0be9621b26b844cc9e19263f35383)**",
    "created_at": "2018-08-05T08:18:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25397#issuecomment-391635",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/25397](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/25397)** to **[e4209bb](https://github.com/sagemath/sagetrac-mirror/commit/e4209bb671f0be9621b26b844cc9e19263f35383)**
