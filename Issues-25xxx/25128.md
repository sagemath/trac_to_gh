# Issue 25128: Have py_scalar_to_element convert gmpy2 numbers

archive/issues_024891.json:
```json
{
    "assignees": [],
    "body": "Currently `py_scalar_to_element` function (`coerce.pyx`) doesn't convert `gmpy2 numbers`. `py_scalar_to_element` should work with `gmpy2` the same way it does with `numpy`.\n\nAssignee: **@vinklein**\n\nCC:  @videlec\n\nKeywords: **gmpy2**\n\nBranch/Commit: **[ff11b13](https://github.com/sagemath/sagetrac-mirror/commit/ff11b132b88f15ee73cc728e480395030728f17f)**\n\nReviewer: **Vincent Delecroix, Jeroen Demeyer**\n\nAuthor: **Vincent Klein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25128_\n\n",
    "closed_at": "2018-05-12T11:46:06Z",
    "created_at": "2018-04-09T14:44:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Have py_scalar_to_element convert gmpy2 numbers",
    "type": "issue",
    "updated_at": "2018-05-12T11:46:06Z",
    "url": "https://github.com/sagemath/sage/issues/25128",
    "user": "https://github.com/vinklein"
}
```
Currently `py_scalar_to_element` function (`coerce.pyx`) doesn't convert `gmpy2 numbers`. `py_scalar_to_element` should work with `gmpy2` the same way it does with `numpy`.

Assignee: **@vinklein**

CC:  @videlec

Keywords: **gmpy2**

Branch/Commit: **[ff11b13](https://github.com/sagemath/sagetrac-mirror/commit/ff11b132b88f15ee73cc728e480395030728f17f)**

Reviewer: **Vincent Delecroix, Jeroen Demeyer**

Author: **Vincent Klein**

_Issue created by migration from https://trac.sagemath.org/ticket/25128_





---

archive/issue_events_329609.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-09T14:44:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329609"
}
```



---

archive/issue_events_329610.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-09T14:44:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329610"
}
```



---

archive/issue_events_329611.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-09T14:44:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329611"
}
```



---

archive/issue_events_329612.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-09T14:44:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329612"
}
```



---

archive/issue_comments_386244.json:
```json
{
    "body": "Assignee: **@vinklein**",
    "created_at": "2018-04-09T14:44:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386244",
    "user": "https://github.com/vinklein"
}
```

Assignee: **@vinklein**



---

archive/issue_comments_386245.json:
```json
{
    "body": "Branch: **[u/vklein/have_py_scalar_to_element_convert_gmpy2_numbers](https://github.com/sagemath/sagetrac-mirror/tree/u/vklein/have_py_scalar_to_element_convert_gmpy2_numbers)**",
    "created_at": "2018-04-10T09:33:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386245",
    "user": "https://github.com/vinklein"
}
```

Branch: **[u/vklein/have_py_scalar_to_element_convert_gmpy2_numbers](https://github.com/sagemath/sagetrac-mirror/tree/u/vklein/have_py_scalar_to_element_convert_gmpy2_numbers)**



---

archive/issue_comments_386246.json:
```json
{
    "body": "Author: **Vincent Klein**",
    "created_at": "2018-04-10T09:40:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386246",
    "user": "https://github.com/vinklein"
}
```

Author: **Vincent Klein**



---

archive/issue_events_329613.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-10T09:40:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329613"
}
```



---

archive/issue_comments_386247.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8555214b0b83bffe03ea7001e68b685ce8a1f458\">8555214</a></td><td><code>Trac #25128: ``gmpy2``'s types and ``py_scalar_to_element``</code></td></tr></table>\n",
    "created_at": "2018-04-10T09:40:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386247",
    "user": "https://github.com/vinklein"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8555214b0b83bffe03ea7001e68b685ce8a1f458">8555214</a></td><td><code>Trac #25128: ``gmpy2``'s types and ``py_scalar_to_element``</code></td></tr></table>




---

archive/issue_comments_386248.json:
```json
{
    "body": "Commit: **[8555214](https://github.com/sagemath/sagetrac-mirror/commit/8555214b0b83bffe03ea7001e68b685ce8a1f458)**",
    "created_at": "2018-04-10T09:40:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386248",
    "user": "https://github.com/vinklein"
}
```

Commit: **[8555214](https://github.com/sagemath/sagetrac-mirror/commit/8555214b0b83bffe03ea7001e68b685ce8a1f458)**



---

archive/issue_events_329614.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-04-10T12:06:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329614"
}
```



---

archive/issue_events_329615.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-04-10T12:06:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329615"
}
```



---

archive/issue_comments_386249.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nYou have C functions for type checking: `MPZ_Check`, `MPQ_Check`, `MPFR_Check` and `MPC_Check`.",
    "created_at": "2018-04-10T12:06:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386249",
    "user": "https://github.com/videlec"
}
```

<div id="comment:4" align="right">Comment 4</div>

You have C functions for type checking: `MPZ_Check`, `MPQ_Check`, `MPFR_Check` and `MPC_Check`.



---

archive/issue_comments_386250.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2018-04-10T12:06:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386250",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_comments_386251.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\n(this is the kind of things that are of course not optimized by Cython)",
    "created_at": "2018-04-10T12:07:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386251",
    "user": "https://github.com/videlec"
}
```

<div id="comment:5" align="right">Comment 5</div>

(this is the kind of things that are of course not optimized by Cython)



---

archive/issue_comments_386252.json:
```json
{
    "body": "Changed commit from **[8555214](https://github.com/sagemath/sagetrac-mirror/commit/8555214b0b83bffe03ea7001e68b685ce8a1f458)** to **[c3a55ec](https://github.com/sagemath/sagetrac-mirror/commit/c3a55ec6e3ed5f1c2820d7438ab02bc88f04ebb4)**",
    "created_at": "2018-04-10T12:36:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386252",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8555214](https://github.com/sagemath/sagetrac-mirror/commit/8555214b0b83bffe03ea7001e68b685ce8a1f458)** to **[c3a55ec](https://github.com/sagemath/sagetrac-mirror/commit/c3a55ec6e3ed5f1c2820d7438ab02bc88f04ebb4)**



---

archive/issue_comments_386253.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c3a55ec6e3ed5f1c2820d7438ab02bc88f04ebb4\">c3a55ec</a></td><td><code>Trac #25128: Change ``gmpy2``'s type checking with ...</code></td></tr></table>\n",
    "created_at": "2018-04-10T12:36:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386253",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c3a55ec6e3ed5f1c2820d7438ab02bc88f04ebb4">c3a55ec</a></td><td><code>Trac #25128: Change ``gmpy2``'s type checking with ...</code></td></tr></table>




---

archive/issue_events_329616.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-10T12:38:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329616"
}
```



---

archive/issue_events_329617.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-10T12:38:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329617"
}
```



---

archive/issue_events_329618.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-10T12:39:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329618"
}
```



---

archive/issue_events_329619.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-10T12:39:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329619"
}
```



---

archive/issue_comments_386254.json:
```json
{
    "body": "Changed commit from **[c3a55ec](https://github.com/sagemath/sagetrac-mirror/commit/c3a55ec6e3ed5f1c2820d7438ab02bc88f04ebb4)** to **[95da017](https://github.com/sagemath/sagetrac-mirror/commit/95da017b881c8c44ae135a0105c067d0529e3657)**",
    "created_at": "2018-04-10T12:42:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386254",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c3a55ec](https://github.com/sagemath/sagetrac-mirror/commit/c3a55ec6e3ed5f1c2820d7438ab02bc88f04ebb4)** to **[95da017](https://github.com/sagemath/sagetrac-mirror/commit/95da017b881c8c44ae135a0105c067d0529e3657)**



---

archive/issue_comments_386255.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/95da017b881c8c44ae135a0105c067d0529e3657\">95da017</a></td><td><code>Trac #25128: Change ``gmpy2``'s type checking with ...</code></td></tr></table>\n",
    "created_at": "2018-04-10T12:42:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386255",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/95da017b881c8c44ae135a0105c067d0529e3657">95da017</a></td><td><code>Trac #25128: Change ``gmpy2``'s type checking with ...</code></td></tr></table>




---

archive/issue_events_329620.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-10T12:43:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329620"
}
```



---

archive/issue_events_329621.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-10T12:43:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329621"
}
```



---

archive/issue_events_329622.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-04-13T08:02:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329622"
}
```



---

archive/issue_events_329623.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-04-13T08:02:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329623"
}
```



---

archive/issue_comments_386256.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThere needs to be a linebreak after `::`\n\n```\n+    Test gmpy2's types::\n+        sage: import gmpy2                               # optional - gmpy2 \n```",
    "created_at": "2018-04-13T08:02:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386256",
    "user": "https://github.com/videlec"
}
```

<div id="comment:11" align="right">Comment 11</div>

There needs to be a linebreak after `::`

```
+    Test gmpy2's types::
+        sage: import gmpy2                               # optional - gmpy2 
```



---

archive/issue_comments_386257.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/783c01de5c2ae05fd5245a8d0ffef4363981adf4\">783c01d</a></td><td><code>Trac #25128: Fix missing linebreak after ``::``</code></td></tr></table>\n",
    "created_at": "2018-04-13T08:47:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386257",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/783c01de5c2ae05fd5245a8d0ffef4363981adf4">783c01d</a></td><td><code>Trac #25128: Fix missing linebreak after ``::``</code></td></tr></table>




---

archive/issue_comments_386258.json:
```json
{
    "body": "Changed commit from **[95da017](https://github.com/sagemath/sagetrac-mirror/commit/95da017b881c8c44ae135a0105c067d0529e3657)** to **[783c01d](https://github.com/sagemath/sagetrac-mirror/commit/783c01de5c2ae05fd5245a8d0ffef4363981adf4)**",
    "created_at": "2018-04-13T08:47:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386258",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[95da017](https://github.com/sagemath/sagetrac-mirror/commit/95da017b881c8c44ae135a0105c067d0529e3657)** to **[783c01d](https://github.com/sagemath/sagetrac-mirror/commit/783c01de5c2ae05fd5245a8d0ffef4363981adf4)**



---

archive/issue_comments_386259.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nFix missing linebreak.",
    "created_at": "2018-04-13T08:47:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386259",
    "user": "https://github.com/vinklein"
}
```

<div id="comment:13" align="right">Comment 13</div>

Fix missing linebreak.



---

archive/issue_events_329624.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-13T08:47:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329624"
}
```



---

archive/issue_events_329625.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-13T08:47:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329625"
}
```



---

archive/issue_events_329626.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-04-13T08:49:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329626"
}
```



---

archive/issue_events_329627.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-04-13T08:49:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329627"
}
```



---

archive/issue_comments_386260.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1 @@\n-Currently `py_scalar_to_element` function (`coerce.pyx`) didn't convert `gmpy2 numbers`.\n-`py_scalar_to_element` should work with `gmpy2` the same way it work with `numpy`.\n+Currently `py_scalar_to_element` function (`coerce.pyx`) doesn't convert `gmpy2 numbers`. `py_scalar_to_element` should work with `gmpy2` the same way it does with `numpy`.\n``````\n",
    "created_at": "2018-04-13T08:49:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386260",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1 @@
-Currently `py_scalar_to_element` function (`coerce.pyx`) didn't convert `gmpy2 numbers`.
-`py_scalar_to_element` should work with `gmpy2` the same way it work with `numpy`.
+Currently `py_scalar_to_element` function (`coerce.pyx`) doesn't convert `gmpy2 numbers`. `py_scalar_to_element` should work with `gmpy2` the same way it does with `numpy`.
``````




---

archive/issue_comments_386261.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nWhy `MPZ_Check(x)` instead of `type(x) is gmpy2.mpz`? There is nothing really wrong with `MPZ_Check` but all other places in Sage use the `type` check. For example in `src/sage/rings/complex_double.pyx`:\n\n```\nif HAVE_GMPY2 and type(x) is gmpy2.mpc:\n```\nIf you use only type checks, you can also remove the `import_gmpy2`.",
    "created_at": "2018-04-18T08:58:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386261",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:15" align="right">Comment 15</div>

Why `MPZ_Check(x)` instead of `type(x) is gmpy2.mpz`? There is nothing really wrong with `MPZ_Check` but all other places in Sage use the `type` check. For example in `src/sage/rings/complex_double.pyx`:

```
if HAVE_GMPY2 and type(x) is gmpy2.mpc:
```
If you use only type checks, you can also remove the `import_gmpy2`.



---

archive/issue_comments_386262.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nI just benchmarked it and `MPZ_Check()` is slightly slower than the `type()` check. This is probably because the `import_gmpy2` machinery gives a slight overhead compared to the direct import that Cython does.",
    "created_at": "2018-04-18T09:08:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386262",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:16" align="right">Comment 16</div>

I just benchmarked it and `MPZ_Check()` is slightly slower than the `type()` check. This is probably because the `import_gmpy2` machinery gives a slight overhead compared to the direct import that Cython does.



---

archive/issue_events_329628.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-18T14:04:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329628"
}
```



---

archive/issue_events_329629.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-18T14:04:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329629"
}
```



---

archive/issue_comments_386263.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nOk i will reverse to `type()` check then. I see no reason to keep `MPZ_Check()` if it's not faster.",
    "created_at": "2018-04-18T14:04:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386263",
    "user": "https://github.com/vinklein"
}
```

<div id="comment:17" align="right">Comment 17</div>

Ok i will reverse to `type()` check then. I see no reason to keep `MPZ_Check()` if it's not faster.



---

archive/issue_comments_386264.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ff11b132b88f15ee73cc728e480395030728f17f\">ff11b13</a></td><td><code>Trac #25128: ``gmpy2``'s types and ``py_scalar_to_element``</code></td></tr></table>\n",
    "created_at": "2018-04-18T14:53:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386264",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:18" align="right">Comment 18</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ff11b132b88f15ee73cc728e480395030728f17f">ff11b13</a></td><td><code>Trac #25128: ``gmpy2``'s types and ``py_scalar_to_element``</code></td></tr></table>




---

archive/issue_comments_386265.json:
```json
{
    "body": "Changed commit from **[783c01d](https://github.com/sagemath/sagetrac-mirror/commit/783c01de5c2ae05fd5245a8d0ffef4363981adf4)** to **[ff11b13](https://github.com/sagemath/sagetrac-mirror/commit/ff11b132b88f15ee73cc728e480395030728f17f)**",
    "created_at": "2018-04-18T14:53:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386265",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[783c01d](https://github.com/sagemath/sagetrac-mirror/commit/783c01de5c2ae05fd5245a8d0ffef4363981adf4)** to **[ff11b13](https://github.com/sagemath/sagetrac-mirror/commit/ff11b132b88f15ee73cc728e480395030728f17f)**



---

archive/issue_events_329630.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-18T14:55:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329630"
}
```



---

archive/issue_events_329631.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-18T14:55:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329631"
}
```



---

archive/issue_comments_386266.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nprevious commits squashed.",
    "created_at": "2018-04-18T14:55:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386266",
    "user": "https://github.com/vinklein"
}
```

<div id="comment:19" align="right">Comment 19</div>

previous commits squashed.



---

archive/issue_events_329632.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-18T14:55:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329632"
}
```



---

archive/issue_events_329633.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2018-04-18T14:55:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329633"
}
```



---

archive/issue_comments_386267.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nFine for me but I leave the final review to Vincent Delecroix.",
    "created_at": "2018-04-18T16:00:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386267",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:21" align="right">Comment 21</div>

Fine for me but I leave the final review to Vincent Delecroix.



---

archive/issue_comments_386268.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nSure it's fine.",
    "created_at": "2018-04-18T16:30:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386268",
    "user": "https://github.com/videlec"
}
```

<div id="comment:22" align="right">Comment 22</div>

Sure it's fine.



---

archive/issue_events_329634.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-04-18T16:30:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329634"
}
```



---

archive/issue_events_329635.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-04-18T16:30:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329635"
}
```



---

archive/issue_comments_386269.json:
```json
{
    "body": "Changed reviewer from **Vincent Delecroix** to **Vincent Delecroix, Jeroen Demeyer**",
    "created_at": "2018-04-18T16:30:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386269",
    "user": "https://github.com/videlec"
}
```

Changed reviewer from **Vincent Delecroix** to **Vincent Delecroix, Jeroen Demeyer**



---

archive/issue_events_329636.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-12T11:46:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329636"
}
```



---

archive/issue_events_329637.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ad72cec5b831650f82918c8a53fdc47143888cad",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-12T11:46:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25128#event-329637"
}
```



---

archive/issue_comments_386270.json:
```json
{
    "body": "Changed branch from **[u/vklein/have_py_scalar_to_element_convert_gmpy2_numbers](https://github.com/sagemath/sagetrac-mirror/tree/u/vklein/have_py_scalar_to_element_convert_gmpy2_numbers)** to **[ff11b13](https://github.com/sagemath/sagetrac-mirror/commit/ff11b132b88f15ee73cc728e480395030728f17f)**",
    "created_at": "2018-05-12T11:46:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25128#issuecomment-386270",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vklein/have_py_scalar_to_element_convert_gmpy2_numbers](https://github.com/sagemath/sagetrac-mirror/tree/u/vklein/have_py_scalar_to_element_convert_gmpy2_numbers)** to **[ff11b13](https://github.com/sagemath/sagetrac-mirror/commit/ff11b132b88f15ee73cc728e480395030728f17f)**
