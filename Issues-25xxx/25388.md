# Issue 25388: InheritComparisonUniqueRepresentation

archive/issues_025151.json:
```json
{
    "body": "This is a bit ugly, but it seems to be necessary if you want to use `UniqueRepresentation` with subclasses of `Element` (among others) which has the `InheritComparison` metaclass.  This otherwise creates a conflict with `ClasscallMetaclass`.\n\nThis class resolves that by making a `UniqueRepresentation` with the `InheritComparisonClasscallMetaclass`.  This can then be used as a mix-in class with `Element` subclasses.\n\nThere's at least some prior precedence for this, and I've updated those examples to use `InheritComparisonUniqueRepresentation` to demonstrate that it works.  I'm open to better ideas.\n\nCC:  @jdemeyer\n\nAuthor: Erik Bray\n\nBranch: u/embray/misc/inherit-comp-unique-repr\n\nStatus: needs_work\n\nCommit: 8d665aa799cecdcbf762148d11c625e0bb30bf82\n\nIssue created by migration from https://trac.sagemath.org/ticket/25388\n\n",
    "created_at": "2018-05-17T15:11:38Z",
    "labels": [
        "component: refactoring"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "InheritComparisonUniqueRepresentation",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25388",
    "user": "https://github.com/embray"
}
```
This is a bit ugly, but it seems to be necessary if you want to use `UniqueRepresentation` with subclasses of `Element` (among others) which has the `InheritComparison` metaclass.  This otherwise creates a conflict with `ClasscallMetaclass`.

This class resolves that by making a `UniqueRepresentation` with the `InheritComparisonClasscallMetaclass`.  This can then be used as a mix-in class with `Element` subclasses.

There's at least some prior precedence for this, and I've updated those examples to use `InheritComparisonUniqueRepresentation` to demonstrate that it works.  I'm open to better ideas.

CC:  @jdemeyer

Author: Erik Bray

Branch: u/embray/misc/inherit-comp-unique-repr

Status: needs_work

Commit: 8d665aa799cecdcbf762148d11c625e0bb30bf82

Issue created by migration from https://trac.sagemath.org/ticket/25388





---

archive/issue_comments_485225.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-05-17T15:24:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485225",
    "user": "https://github.com/embray"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_485226.json:
```json
{
    "body": "<a id='comment:2'></a>My first reaction is that it doesn't really make sense to use `UniqueRepresentation` for elements. I wonder what the reason for that is.",
    "created_at": "2018-05-17T15:42:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485226",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>My first reaction is that it doesn't really make sense to use `UniqueRepresentation` for elements. I wonder what the reason for that is.



---

archive/issue_comments_485227.json:
```json
{
    "body": "<a id='comment:3'></a>Perhaps the entire concept is wrong somehow?  That said as you can see there are cases of this (albeit with classes that only inherit `Element` indirectly).\n\nWhy doesn't it make sense?  Just as `1 is 1` it does make sense to have this for other objects.  Granted `1 is 1` is a consequence of caching small integers, but I don't see the harm of having it for other elements.\n\nAlternatively, it would be nice to have something that behaves somewhat like `UniqueRepresentation` just for `__eq__`.  That is, something like `CachedRepresentation` that compares instances according to their cache keys.  The idea is to avoid implementing a bunch of `__eq__` that compare two objects according to some set of attributes that are the same attributes used to initialize a \"unique\" instance of that class.  Does that make sense?",
    "created_at": "2018-05-17T16:26:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485227",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'></a>Perhaps the entire concept is wrong somehow?  That said as you can see there are cases of this (albeit with classes that only inherit `Element` indirectly).

Why doesn't it make sense?  Just as `1 is 1` it does make sense to have this for other objects.  Granted `1 is 1` is a consequence of caching small integers, but I don't see the harm of having it for other elements.

Alternatively, it would be nice to have something that behaves somewhat like `UniqueRepresentation` just for `__eq__`.  That is, something like `CachedRepresentation` that compares instances according to their cache keys.  The idea is to avoid implementing a bunch of `__eq__` that compare two objects according to some set of attributes that are the same attributes used to initialize a "unique" instance of that class.  Does that make sense?



---

archive/issue_comments_485228.json:
```json
{
    "body": "<a id='comment:4'></a>In any case, it does make sense, as long as we have a complicated metaclass hierarchy, to have a metaclass that combines `InheritComparison` with `UniqueRepresentation`.",
    "created_at": "2018-05-17T16:28:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485228",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'></a>In any case, it does make sense, as long as we have a complicated metaclass hierarchy, to have a metaclass that combines `InheritComparison` with `UniqueRepresentation`.



---

archive/issue_comments_485229.json:
```json
{
    "body": "<a id='comment:5'></a>I can think of certain morphisms that I could see benefiting from being `UniqueRepresentation`, but I wouldn't do non-equality comparisons on them. The other would be for small sets of key algebraic objects. We do this in `combinat/crystals/letters.pyx`, but we do it by ``@`cached_method` on `_element_constructor_` to help improve the construction time and to tie it directly with the parent.",
    "created_at": "2018-05-18T05:15:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485229",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'></a>I can think of certain morphisms that I could see benefiting from being `UniqueRepresentation`, but I wouldn't do non-equality comparisons on them. The other would be for small sets of key algebraic objects. We do this in `combinat/crystals/letters.pyx`, but we do it by ``@`cached_method` on `_element_constructor_` to help improve the construction time and to tie it directly with the parent.



---

archive/issue_comments_485230.json:
```json
{
    "body": "<a id='comment:6'></a>There are a couple of reasons why `UniqueRepresentation` should be used sparingly:\n- It makes instantiation of an object *expensive*: The construction parameters need to be processed and made into a dictionary key for lookup\n- It is very prone to creating memory leaks if the construction parameters consist of anything more complicated than integers and strings. That's because the construction parameters, being put in the key of a global `WeakValueDict`, have their life lower bounded by the object. So if those parameters end up carrying (indirectly) a reference to the object, it's now immortal.\n- it means that one has to be very careful caching `UniqueRepresentation` objects: they're already being globally cached (under expensive keys) and the places where one would like to cache them are generally places where that would create the kind of reference loops that cause memory leaks.\n\nThe reason why we have `UniqueRepresentation` is because it allows the coercion framework to do lightning-fast comparisons (because we can make do with an \"is\" test rather than a \"==\"). It's a compromise that makes memory management with *parents* much more complicated, for the benefit of making some operations with *elements* much faster.\n\nThat's why people think it's strange to make elements `UniqueRepresentation`.",
    "created_at": "2018-05-18T06:22:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485230",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:6'></a>There are a couple of reasons why `UniqueRepresentation` should be used sparingly:
- It makes instantiation of an object *expensive*: The construction parameters need to be processed and made into a dictionary key for lookup
- It is very prone to creating memory leaks if the construction parameters consist of anything more complicated than integers and strings. That's because the construction parameters, being put in the key of a global `WeakValueDict`, have their life lower bounded by the object. So if those parameters end up carrying (indirectly) a reference to the object, it's now immortal.
- it means that one has to be very careful caching `UniqueRepresentation` objects: they're already being globally cached (under expensive keys) and the places where one would like to cache them are generally places where that would create the kind of reference loops that cause memory leaks.

The reason why we have `UniqueRepresentation` is because it allows the coercion framework to do lightning-fast comparisons (because we can make do with an "is" test rather than a "=="). It's a compromise that makes memory management with *parents* much more complicated, for the benefit of making some operations with *elements* much faster.

That's why people think it's strange to make elements `UniqueRepresentation`.



---

archive/issue_comments_485231.json:
```json
{
    "body": "<a id='comment:7'></a>That's weird about the memory leaks since I thought the point of using weak reference caching was that you could construct an object once--it would be cached--but then if it isn't used again the cached keys would (potentially) go away.\n\nAs for this specific ticket, perhaps the use case of using `UniqueRepresentation` with Elements is not good, though as the diff shows there were still at least a few places where it was useful to define this (unfortunately verbose) `InheritComparisonUniqueRepresentation` class for reuse.\n\nHowever, my primary motivation to using more `UniqueRepresentation` was actually as a possible solution, in many cases, to #24551.  The reason it appealed to me is that it gives free hashing and equality comparison implementation for classes that have a property that identity of instances of that class is uniquely determined by some attributes of that instance that are provided at its time of construction (particularly via `__init__`).\n\nIf `UniqueRepresentation` is not right for this in all cases (something I can believe) then perhaps it would be good to add a new base class that has a similar property, but without caching (and thus without the guaranteed equality-by-id property, which for my use case is not actually so important).  In other words, it would basically generate the same cache key that would be generated by `CachedRepresentation`, but only use that to implement `__eq__` and `__hash__`, but not do any caching.",
    "created_at": "2018-05-18T08:53:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485231",
    "user": "https://github.com/embray"
}
```

<a id='comment:7'></a>That's weird about the memory leaks since I thought the point of using weak reference caching was that you could construct an object once--it would be cached--but then if it isn't used again the cached keys would (potentially) go away.

As for this specific ticket, perhaps the use case of using `UniqueRepresentation` with Elements is not good, though as the diff shows there were still at least a few places where it was useful to define this (unfortunately verbose) `InheritComparisonUniqueRepresentation` class for reuse.

However, my primary motivation to using more `UniqueRepresentation` was actually as a possible solution, in many cases, to #24551.  The reason it appealed to me is that it gives free hashing and equality comparison implementation for classes that have a property that identity of instances of that class is uniquely determined by some attributes of that instance that are provided at its time of construction (particularly via `__init__`).

If `UniqueRepresentation` is not right for this in all cases (something I can believe) then perhaps it would be good to add a new base class that has a similar property, but without caching (and thus without the guaranteed equality-by-id property, which for my use case is not actually so important).  In other words, it would basically generate the same cache key that would be generated by `CachedRepresentation`, but only use that to implement `__eq__` and `__hash__`, but not do any caching.



---

archive/issue_comments_485232.json:
```json
{
    "body": "<a id='comment:8'></a>Replying to [tscrim](#comment%3A5):\n> I can think of certain morphisms that I could see benefiting from being `UniqueRepresentation`, but I wouldn't do non-equality comparisons on them. The other would be for small sets of key algebraic objects. We do this in `combinat/crystals/letters.pyx`, but we do it by ``@`cached_method` on `_element_constructor_` to help improve the construction time and to tie it directly with the parent.\n\n\nOn that note, could you have a look at #25389 and tell me if the use of `UniqueRepresentation` *doesn't* make sense in any of those cases.  I felt that it did make sense but clearly there's a lot of hard-earned experience I don't have as to when it does or doesn't make sense to use. (Note: The branch in that ticket has this one as a dependency, so it's really only interesting to look at the changes in quaternion algebras: https://git.sagemath.org/sage.git/diff/src/sage/algebras/quatalg/quaternion_algebra.py?id=9ea55d9933a3cd2bedd391068276f804ff868e1f&id2=6fc1e20c666283a301b4ff3f855013de8d206b35)",
    "created_at": "2018-05-18T08:55:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485232",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>Replying to [tscrim](#comment%3A5):
> I can think of certain morphisms that I could see benefiting from being `UniqueRepresentation`, but I wouldn't do non-equality comparisons on them. The other would be for small sets of key algebraic objects. We do this in `combinat/crystals/letters.pyx`, but we do it by ``@`cached_method` on `_element_constructor_` to help improve the construction time and to tie it directly with the parent.


On that note, could you have a look at #25389 and tell me if the use of `UniqueRepresentation` *doesn't* make sense in any of those cases.  I felt that it did make sense but clearly there's a lot of hard-earned experience I don't have as to when it does or doesn't make sense to use. (Note: The branch in that ticket has this one as a dependency, so it's really only interesting to look at the changes in quaternion algebras: https://git.sagemath.org/sage.git/diff/src/sage/algebras/quatalg/quaternion_algebra.py?id=9ea55d9933a3cd2bedd391068276f804ff868e1f&id2=6fc1e20c666283a301b4ff3f855013de8d206b35)



---

archive/issue_comments_485233.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [embray](#comment%3A7):\n> That's weird about the memory leaks since I thought the point of using weak reference caching was that you could construct an object once--it would be cached--but then if it isn't used again the cached keys would (potentially) go away.\n\n\nThat's true, but the caching is not the primary design feature. It's a cost more than a benefit (but necessary for what it does). The global cache brings you very close to creating a global anchor for reference cycles: The *global* weakvaluedict that does the caching has *strong* references to the keys used (the construction parameters). As has been shown time and again, these keys have a tendency of carrying references to the cached object. Now your memory cycle is globally anchored and will be immune to cyclic garbage collection: a memory leak.\n\nThis kind of leak keeps occurring. See #23807#comment:14 for a recent example.\n\nThe `EqualityAndHashingBasedOnConstructionParameters` that you suggest makes a lot more sense: here you get to store the key on the object itself, so memory leaks should be much less of a problem. I suspect it will not be a very memory-efficient class, because the key objects you're storing are likely preserved elsewhere on the object as well.",
    "created_at": "2018-05-18T14:09:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485233",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:9'></a>Replying to [embray](#comment%3A7):
> That's weird about the memory leaks since I thought the point of using weak reference caching was that you could construct an object once--it would be cached--but then if it isn't used again the cached keys would (potentially) go away.


That's true, but the caching is not the primary design feature. It's a cost more than a benefit (but necessary for what it does). The global cache brings you very close to creating a global anchor for reference cycles: The *global* weakvaluedict that does the caching has *strong* references to the keys used (the construction parameters). As has been shown time and again, these keys have a tendency of carrying references to the cached object. Now your memory cycle is globally anchored and will be immune to cyclic garbage collection: a memory leak.

This kind of leak keeps occurring. See #23807#comment:14 for a recent example.

The `EqualityAndHashingBasedOnConstructionParameters` that you suggest makes a lot more sense: here you get to store the key on the object itself, so memory leaks should be much less of a problem. I suspect it will not be a very memory-efficient class, because the key objects you're storing are likely preserved elsewhere on the object as well.



---

archive/issue_comments_485234.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [embray](#comment%3A8):\n> On that note, could you have a look at #25389 and tell me if the use of `UniqueRepresentation` *doesn't* make sense in any of those cases.\n\n\nThe construction parameters of quaternionalgebras are sufficiently light that I would not expect memory leaks (as long as people don't take to caching quaternion algebras on the base ring!, which now that I thought of it, people will undoubtedly start doing).\n\nIn principle, `QuaternionAlgebra` is the kind of parent that could participate in coercion discovery, where `UniqueRepresentation` might be required (in particular, if people consider polynomial ring *over* a ring and expect coercion discovery, and in particular common covering parent construction, to work with it, then `UniqueRepresentation` might be required.\n\nI would find it surprising if people are actually doing that with quaternion algebras, so I don't think we're getting the designed benefit from quaternion algebras.\n\nBy turning quaternion algebras into globally unique objects, you are requiring the algebras to be immutable in a very strong sense: different parts could end up having references to the same algebra, so ANY change in state (even little representation/caching details) must be assumed to affect completely unrelated code. It's not possible to make your own little local algebra and scribble in it anymore.\n\nMore precisely. The kind of scenario in the coercion framework where I know that `UniqueRepresentation` is really required is to make this assertion hold:\n\n```\nsage: Zxy.<x,y>=ZZ[]\nsage: Qx.<x>=QQ[]\nsage: Qy.<y>=QQ[]\nsage: Qxy=QQ['x','y']\nsage: x=Qx.0\nsage: y=Zxy.1\nsage: (x+y).parent() is Qxy\nTrue\nsage: Qy.<y>=QQ[]\nsage: (Qy.0+Zxy.0).parent() is Qxy\nTrue\n```\n\nNote that the coercion framework needs to construct a common covering parent for `Zxy` and `Qx` and one for `Zxy` and `Qy` itself. It would be very surprising if the results were not identical. \n(the fact that the resulting parent is equal to `Qxy` constructed by the user is convenient but less important). I don't think people would be doing that with `QuaternionAlgebra` so I think you could safely leave off the `UniqueRep` for now.",
    "created_at": "2018-05-18T14:22:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485234",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:10'></a>Replying to [embray](#comment%3A8):
> On that note, could you have a look at #25389 and tell me if the use of `UniqueRepresentation` *doesn't* make sense in any of those cases.


The construction parameters of quaternionalgebras are sufficiently light that I would not expect memory leaks (as long as people don't take to caching quaternion algebras on the base ring!, which now that I thought of it, people will undoubtedly start doing).

In principle, `QuaternionAlgebra` is the kind of parent that could participate in coercion discovery, where `UniqueRepresentation` might be required (in particular, if people consider polynomial ring *over* a ring and expect coercion discovery, and in particular common covering parent construction, to work with it, then `UniqueRepresentation` might be required.

I would find it surprising if people are actually doing that with quaternion algebras, so I don't think we're getting the designed benefit from quaternion algebras.

By turning quaternion algebras into globally unique objects, you are requiring the algebras to be immutable in a very strong sense: different parts could end up having references to the same algebra, so ANY change in state (even little representation/caching details) must be assumed to affect completely unrelated code. It's not possible to make your own little local algebra and scribble in it anymore.

More precisely. The kind of scenario in the coercion framework where I know that `UniqueRepresentation` is really required is to make this assertion hold:

```
sage: Zxy.<x,y>=ZZ[]
sage: Qx.<x>=QQ[]
sage: Qy.<y>=QQ[]
sage: Qxy=QQ['x','y']
sage: x=Qx.0
sage: y=Zxy.1
sage: (x+y).parent() is Qxy
True
sage: Qy.<y>=QQ[]
sage: (Qy.0+Zxy.0).parent() is Qxy
True
```

Note that the coercion framework needs to construct a common covering parent for `Zxy` and `Qx` and one for `Zxy` and `Qy` itself. It would be very surprising if the results were not identical. 
(the fact that the resulting parent is equal to `Qxy` constructed by the user is convenient but less important). I don't think people would be doing that with `QuaternionAlgebra` so I think you could safely leave off the `UniqueRep` for now.



---

archive/issue_comments_485235.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [nbruin](#comment%3A9):\n> The `EqualityAndHashingBasedOnConstructionParameters` that you suggest makes a lot more sense: here you get to store the key on the object itself, so memory leaks should be much less of a problem. I suspect it will not be a very memory-efficient class, because the key objects you're storing are likely preserved elsewhere on the object as well.\n\n\nWhile I like the feature of `CachedRepresentation` that bases the cache key on the construction parameters, I don't consider it a critical feature either.  This could just as easily (more easily in fact) be implemented simply by defining a tuple of attribute names (or possibly methods that should be called) that should be used for testing equality and hashing.  It's just such a common pattern it really could save a lot of code and effort if it could be coded once for a lot of classes.\n\nHaving an additional, usually small, tuple of references is not generally a major memory concern but of course as with anything it depends on how many of these objects you expect to construct.",
    "created_at": "2018-05-21T10:58:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485235",
    "user": "https://github.com/embray"
}
```

<a id='comment:11'></a>Replying to [nbruin](#comment%3A9):
> The `EqualityAndHashingBasedOnConstructionParameters` that you suggest makes a lot more sense: here you get to store the key on the object itself, so memory leaks should be much less of a problem. I suspect it will not be a very memory-efficient class, because the key objects you're storing are likely preserved elsewhere on the object as well.


While I like the feature of `CachedRepresentation` that bases the cache key on the construction parameters, I don't consider it a critical feature either.  This could just as easily (more easily in fact) be implemented simply by defining a tuple of attribute names (or possibly methods that should be called) that should be used for testing equality and hashing.  It's just such a common pattern it really could save a lot of code and effort if it could be coded once for a lot of classes.

Having an additional, usually small, tuple of references is not generally a major memory concern but of course as with anything it depends on how many of these objects you expect to construct.



---

archive/issue_comments_485236.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [nbruin](#comment%3A10):\n> Note that the coercion framework needs to construct a common covering parent for `Zxy` and `Qx` and one for `Zxy` and `Qy` itself. It would be very surprising if the results were not identical. \n> (the fact that the resulting parent is equal to `Qxy` constructed by the user is convenient but less important). I don't think people would be doing that with `QuaternionAlgebra` so I think you could safely leave off the `UniqueRep` for now.\n\n\nThanks for looking at it and for the explanation.  I agree.  I guess what I really just want there is the aforementioned `EqualityAndHashingBasedOnConstructionParameters`, and I was using `UniqueRepresentation` more for that feature than anything (although it does seem useful to me for preventing construction of multiple copies of otherwise identical objects, but unless that's actually a problem for, say, `QuaternionOrder`s then it's a premature optimization).",
    "created_at": "2018-05-21T11:05:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485236",
    "user": "https://github.com/embray"
}
```

<a id='comment:12'></a>Replying to [nbruin](#comment%3A10):
> Note that the coercion framework needs to construct a common covering parent for `Zxy` and `Qx` and one for `Zxy` and `Qy` itself. It would be very surprising if the results were not identical. 
> (the fact that the resulting parent is equal to `Qxy` constructed by the user is convenient but less important). I don't think people would be doing that with `QuaternionAlgebra` so I think you could safely leave off the `UniqueRep` for now.


Thanks for looking at it and for the explanation.  I agree.  I guess what I really just want there is the aforementioned `EqualityAndHashingBasedOnConstructionParameters`, and I was using `UniqueRepresentation` more for that feature than anything (although it does seem useful to me for preventing construction of multiple copies of otherwise identical objects, but unless that's actually a problem for, say, `QuaternionOrder`s then it's a premature optimization).



---

archive/issue_events_063704.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:47:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63704"
}
```



---

archive/issue_comments_485237.json:
```json
{
    "body": "<a id='comment:13'></a>I believe this issue can reasonably be addressed for Sage 8.4.",
    "created_at": "2018-07-18T11:47:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485237",
    "user": "https://github.com/embray"
}
```

<a id='comment:13'></a>I believe this issue can reasonably be addressed for Sage 8.4.



---

archive/issue_comments_485238.json:
```json
{
    "body": "<a id='comment:14'></a>I think this utility class is still useful as it clearly makes a few minor simplifications.",
    "created_at": "2018-09-07T12:52:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485238",
    "user": "https://github.com/embray"
}
```

<a id='comment:14'></a>I think this utility class is still useful as it clearly makes a few minor simplifications.



---

archive/issue_comments_485239.json:
```json
{
    "body": "<a id='comment:15'></a>The problem is that I'm still not convinced that combining `UniqueRepresentation` and `InheritComparisonMetaclass` makes sense at all.",
    "created_at": "2018-09-08T09:27:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485239",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:15'></a>The problem is that I'm still not convinced that combining `UniqueRepresentation` and `InheritComparisonMetaclass` makes sense at all.



---

archive/issue_comments_485240.json:
```json
{
    "body": "<a id='comment:16'></a>Oh what basis?  On its face there's no reason not to and there are clearly classes that do use that combination.  Now, if you disagreed that those classes should be using that combination in the first place that's another story: If those were fixed then there would be no use for adding this class.  But first you'd have to justify why those classes shouldn't use `UniqueRepresentation` in particular and then fix that.",
    "created_at": "2018-09-10T15:33:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485240",
    "user": "https://github.com/embray"
}
```

<a id='comment:16'></a>Oh what basis?  On its face there's no reason not to and there are clearly classes that do use that combination.  Now, if you disagreed that those classes should be using that combination in the first place that's another story: If those were fixed then there would be no use for adding this class.  But first you'd have to justify why those classes shouldn't use `UniqueRepresentation` in particular and then fix that.



---

archive/issue_comments_485241.json:
```json
{
    "body": "<a id='comment:17'></a>Looking at the code referenced in your commits: Indeed, it looks to me these objects have no business being `UniqueRepresentation`. They are differentials: elements; morphisms. There's no need for those to have their equality determined by their ID, which is what `UniqueRepresentation` is for. Indeed, needing comparison inheritance shows that they are not `UniqueRepresentation`. The right way to \"fix\" this (if any fix is necessary) is to change the inheritance of these objects, not to introduce an oxymoronic new class.\n\nIt could be that those pieces of code are from before `UniqueRepresentation` and `CachedRepresentation` were split off, or perhaps they were written by someone who didn't understand the fine difference between the two. In this case, I don't think you would even want to do it as a transition, since you're causing a net increase in code, so it's not making it shorter either.",
    "created_at": "2018-09-10T16:47:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485241",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:17'></a>Looking at the code referenced in your commits: Indeed, it looks to me these objects have no business being `UniqueRepresentation`. They are differentials: elements; morphisms. There's no need for those to have their equality determined by their ID, which is what `UniqueRepresentation` is for. Indeed, needing comparison inheritance shows that they are not `UniqueRepresentation`. The right way to "fix" this (if any fix is necessary) is to change the inheritance of these objects, not to introduce an oxymoronic new class.

It could be that those pieces of code are from before `UniqueRepresentation` and `CachedRepresentation` were split off, or perhaps they were written by someone who didn't understand the fine difference between the two. In this case, I don't think you would even want to do it as a transition, since you're causing a net increase in code, so it's not making it shorter either.



---

archive/issue_comments_485242.json:
```json
{
    "body": "<a id='comment:18'></a>So I can answer for `sage.algebras.commutative_dga.Differential` why it is useful to have it inherit from `UniqueRepresentation`: These morphisms are used to define a differential graded commutative algebra (DGCA), so they need to be hashable and easily comparable. They also contain computationally difficult results that we want to cache and not duplicate, so they should be a cached representation. Hence, it makes sense to have them be `UniqueRepresentation`. From a quick thought, I believe it removes code duplication between the **Z** and multigraded cases.\n\nThis does not quite hold as much water for `sage.algebras.clifford_algebra.ExteriorAlgebraDifferential` as those are less likely to be compared or used to construct other objects. However, it does make the comparisons easier to work with in case someone does have a use-case for that.",
    "created_at": "2018-09-10T23:06:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485242",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:18'></a>So I can answer for `sage.algebras.commutative_dga.Differential` why it is useful to have it inherit from `UniqueRepresentation`: These morphisms are used to define a differential graded commutative algebra (DGCA), so they need to be hashable and easily comparable. They also contain computationally difficult results that we want to cache and not duplicate, so they should be a cached representation. Hence, it makes sense to have them be `UniqueRepresentation`. From a quick thought, I believe it removes code duplication between the **Z** and multigraded cases.

This does not quite hold as much water for `sage.algebras.clifford_algebra.ExteriorAlgebraDifferential` as those are less likely to be compared or used to construct other objects. However, it does make the comparisons easier to work with in case someone does have a use-case for that.



---

archive/issue_comments_485243.json:
```json
{
    "body": "<a id='comment:19'></a>Replying to [tscrim](#comment%3A18):\n> So I can answer for `sage.algebras.commutative_dga.Differential` why it is useful to have it inherit from `UniqueRepresentation`\n\n\nI see you make an argument for why `CachedRepresentation` is good for them via the cached bit.\nI also see you make an argument for `UniqueRepresentation` because you want them easily hashed and compared. Indeed, `UniqueRepresentation` makes those operations very fast because it's a composition of `CachedRepresentation` and `EqualityById`.\n\nHowever, it seems that the comparison and hashing you want is NOT the one from `UniqueRepresentation`, because otherwise you wouldn't need the `InheritComparison` thing. The whole difference between `UniqueRepresentation` and `CachedRepresentation` is the mixing in of `EqualityById`. If you don't want that, then obviously you need to be inheriting from `CachedRepresentation` instead. So `InheritComparisonCachedRepresentation` might make sense. `InheritComparisonUniqueRepresention` never does.\n\nBe careful with thinking that `CachedRepresentation` is actually more efficient: Creating objects the first time with them is actually MORE expensive, because the construction parameters need to be processed and hashed. You'll only see a benefit if you're actually recreating the same object from construction parameters multiple times. Often that's a silly thing to do; just keep the object!\nThe other penalty to pay, that you're now sharing global objects so that you REALLY have to take immutability seriously, has been mentioned many times already.",
    "created_at": "2018-09-11T02:11:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485243",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:19'></a>Replying to [tscrim](#comment%3A18):
> So I can answer for `sage.algebras.commutative_dga.Differential` why it is useful to have it inherit from `UniqueRepresentation`


I see you make an argument for why `CachedRepresentation` is good for them via the cached bit.
I also see you make an argument for `UniqueRepresentation` because you want them easily hashed and compared. Indeed, `UniqueRepresentation` makes those operations very fast because it's a composition of `CachedRepresentation` and `EqualityById`.

However, it seems that the comparison and hashing you want is NOT the one from `UniqueRepresentation`, because otherwise you wouldn't need the `InheritComparison` thing. The whole difference between `UniqueRepresentation` and `CachedRepresentation` is the mixing in of `EqualityById`. If you don't want that, then obviously you need to be inheriting from `CachedRepresentation` instead. So `InheritComparisonCachedRepresentation` might make sense. `InheritComparisonUniqueRepresention` never does.

Be careful with thinking that `CachedRepresentation` is actually more efficient: Creating objects the first time with them is actually MORE expensive, because the construction parameters need to be processed and hashed. You'll only see a benefit if you're actually recreating the same object from construction parameters multiple times. Often that's a silly thing to do; just keep the object!
The other penalty to pay, that you're now sharing global objects so that you REALLY have to take immutability seriously, has been mentioned many times already.



---

archive/issue_comments_485244.json:
```json
{
    "body": "<a id='comment:20'></a>So should `sage.algebras.commutative_dga.Differential` be using `WithEqualityById`?  Or `InheritComparison`?  Because I agree it doesn't make sense to use both (in which case I see now why `InheritComparisonUniqueRepresentation` doesn't make sense, but `InheritComparisonCachedRepresentation` might.\n\nOn another note, I'm not even convinced `InheritComparison` needs to be a metaclass.  ISTM it would work fine as a decorator.  Maybe the only argument for it being a metaclass is that then subclasses can inherit its behavior, but I don't know if that's really needed or not...",
    "created_at": "2018-09-11T14:07:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485244",
    "user": "https://github.com/embray"
}
```

<a id='comment:20'></a>So should `sage.algebras.commutative_dga.Differential` be using `WithEqualityById`?  Or `InheritComparison`?  Because I agree it doesn't make sense to use both (in which case I see now why `InheritComparisonUniqueRepresentation` doesn't make sense, but `InheritComparisonCachedRepresentation` might.

On another note, I'm not even convinced `InheritComparison` needs to be a metaclass.  ISTM it would work fine as a decorator.  Maybe the only argument for it being a metaclass is that then subclasses can inherit its behavior, but I don't know if that's really needed or not...



---

archive/issue_comments_485245.json:
```json
{
    "body": "<a id='comment:21'></a>Replying to [embray](#comment%3A20):\n\n> On another note, I'm not even convinced `InheritComparison` needs to be a metaclass.  ISTM it would work fine as a decorator.  Maybe the only argument for it being a metaclass is that then subclasses can inherit its behavior, but I don't know if that's really needed or not...\n\n\nI think that is because it actually pokes around in the class data structure, to \"hard\" inherit the slot value for the comparison functions, the way it would happen normally. The difference is that normally, comparison is not inherited if the hash is changed. The `InheritComparisonMetaClass` makes this happen even if hash is not inherited. I don't see at which point a decorator would have the opportunity to scribble in the relevant slots.",
    "created_at": "2018-09-11T19:56:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485245",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:21'></a>Replying to [embray](#comment%3A20):

> On another note, I'm not even convinced `InheritComparison` needs to be a metaclass.  ISTM it would work fine as a decorator.  Maybe the only argument for it being a metaclass is that then subclasses can inherit its behavior, but I don't know if that's really needed or not...


I think that is because it actually pokes around in the class data structure, to "hard" inherit the slot value for the comparison functions, the way it would happen normally. The difference is that normally, comparison is not inherited if the hash is changed. The `InheritComparisonMetaClass` makes this happen even if hash is not inherited. I don't see at which point a decorator would have the opportunity to scribble in the relevant slots.



---

archive/issue_comments_485246.json:
```json
{
    "body": "<a id='comment:22'></a>Ah, I just tried to implement it as a decorator and the problem is actually that Cython will not allow you to use \"arbitrary\" class decorators on cdef classes.  Which is a shame because I don't see any reason that couldn't work.  Yes, it is dangerous, but consenting adults and all...\n\nWhat's worse is, it still works just fine if you call it not as a decorator, but just at module-level after the class definition (note: I defined a Python function called `inherit_comparison` that implements the functionality as a class decorator):\n\n```\nsage: cython('''\n....: from sage.misc.inherit_comparison import inherit_comparison\n....:\n....: cdef class Base(object):\n....:     def __richcmp__(left, right, int op):\n....:         print(\"Base.__richcmp__\")\n....:         return left is right\n....:\n....: cdef class Derived(Base):\n....:     def __hash__(self): return 1\n....:\n....: inherit_comparison(Derived)\n....: ''')\n```\n\nSo I don't think it *needs* to be a decorator.  At the same time, as I wrote previously, I suppose the advantage of using a metaclass is that the behavior will automatically be inherited by subclasses, whereas with a decorator (whether used as above, or with the actual decorator syntax) one has to remember to use it when subclassing.\n\nSo in light of that, having the metaclass makes sense, unfortunately.\n\nOne bit of good news is that Python 3 (specifically 3.6+) has added a number of new dunder methods for customizing class creation that ease many of the most common use cases for metaclasses without explicitly using a metaclass.  For example, I think [__init_subclass__](https://docs.python.org/3/reference/datamodel.html#object.__init_subclass__) might be able to serve as a replacement for `InheritComparisonMetaclass`.  I don't know if Cython supports `__init_subclass__` yet, but if/when it does we  might be able to drop the metaclass even on Python 2.",
    "created_at": "2018-09-12T10:29:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485246",
    "user": "https://github.com/embray"
}
```

<a id='comment:22'></a>Ah, I just tried to implement it as a decorator and the problem is actually that Cython will not allow you to use "arbitrary" class decorators on cdef classes.  Which is a shame because I don't see any reason that couldn't work.  Yes, it is dangerous, but consenting adults and all...

What's worse is, it still works just fine if you call it not as a decorator, but just at module-level after the class definition (note: I defined a Python function called `inherit_comparison` that implements the functionality as a class decorator):

```
sage: cython('''
....: from sage.misc.inherit_comparison import inherit_comparison
....:
....: cdef class Base(object):
....:     def __richcmp__(left, right, int op):
....:         print("Base.__richcmp__")
....:         return left is right
....:
....: cdef class Derived(Base):
....:     def __hash__(self): return 1
....:
....: inherit_comparison(Derived)
....: ''')
```

So I don't think it *needs* to be a decorator.  At the same time, as I wrote previously, I suppose the advantage of using a metaclass is that the behavior will automatically be inherited by subclasses, whereas with a decorator (whether used as above, or with the actual decorator syntax) one has to remember to use it when subclassing.

So in light of that, having the metaclass makes sense, unfortunately.

One bit of good news is that Python 3 (specifically 3.6+) has added a number of new dunder methods for customizing class creation that ease many of the most common use cases for metaclasses without explicitly using a metaclass.  For example, I think [__init_subclass__](https://docs.python.org/3/reference/datamodel.html#object.__init_subclass__) might be able to serve as a replacement for `InheritComparisonMetaclass`.  I don't know if Cython supports `__init_subclass__` yet, but if/when it does we  might be able to drop the metaclass even on Python 2.



---

archive/issue_events_063705.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63705"
}
```



---

archive/issue_events_063706.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63706"
}
```



---

archive/issue_comments_485247.json:
```json
{
    "body": "<a id='comment:24'></a>Retargeting some of my tickets (somewhat optimistically for now).",
    "created_at": "2018-12-28T14:10:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485247",
    "user": "https://github.com/embray"
}
```

<a id='comment:24'></a>Retargeting some of my tickets (somewhat optimistically for now).



---

archive/issue_events_063707.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63707"
}
```



---

archive/issue_events_063708.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63708"
}
```



---

archive/issue_comments_485248.json:
```json
{
    "body": "<a id='comment:25'></a>Moving all my in-progress tickets to 8.8 milestone.",
    "created_at": "2019-03-25T10:43:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485248",
    "user": "https://github.com/embray"
}
```

<a id='comment:25'></a>Moving all my in-progress tickets to 8.8 milestone.



---

archive/issue_events_063709.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:43:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63709"
}
```



---

archive/issue_events_063710.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:43:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63710"
}
```



---

archive/issue_events_063711.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-03T11:37:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63711"
}
```



---

archive/issue_events_063712.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-03T11:37:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63712"
}
```



---

archive/issue_comments_485249.json:
```json
{
    "body": "<a id='comment:26'></a>Moving tickets from the Sage 8.8 milestone that have been actively worked on in the last six months to the next release milestone (optimistically).",
    "created_at": "2019-07-03T11:37:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485249",
    "user": "https://github.com/embray"
}
```

<a id='comment:26'></a>Moving tickets from the Sage 8.8 milestone that have been actively worked on in the last six months to the next release milestone (optimistically).



---

archive/issue_comments_485250.json:
```json
{
    "body": "<a id='comment:27'></a>Ticket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485250",
    "user": "https://github.com/embray"
}
```

<a id='comment:27'></a>Ticket retargeted after milestone closed



---

archive/issue_events_063713.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63713"
}
```



---

archive/issue_events_063714.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63714"
}
```



---

archive/issue_events_063715.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63715"
}
```



---

archive/issue_events_063716.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63716"
}
```



---

archive/issue_comments_485251.json:
```json
{
    "body": "<a id='comment:28'></a>Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485251",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:28'></a>Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_063717.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63717"
}
```



---

archive/issue_events_063718.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63718"
}
```



---

archive/issue_comments_485252.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2021-01-31T08:32:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485252",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_485253.json:
```json
{
    "body": "<a id='comment:30'></a>red branch => needs work",
    "created_at": "2021-01-31T08:32:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485253",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:30'></a>red branch => needs work



---

archive/issue_comments_485254.json:
```json
{
    "body": "<a id='comment:31'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-15T22:07:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485254",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:31'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_063719.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63719"
}
```



---

archive/issue_events_063720.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63720"
}
```



---

archive/issue_comments_485255.json:
```json
{
    "body": "<a id='comment:32'></a>Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25388#issuecomment-485255",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:32'></a>Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_063721.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63721"
}
```



---

archive/issue_events_063722.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63722"
}
```



---

archive/issue_events_063723.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63723"
}
```



---

archive/issue_events_063724.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63724"
}
```



---

archive/issue_events_063725.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63725"
}
```



---

archive/issue_events_063726.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63726"
}
```



---

archive/issue_events_063727.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63727"
}
```



---

archive/issue_events_063728.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25388",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25388#event-63728"
}
```
