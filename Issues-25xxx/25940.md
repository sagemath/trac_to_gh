# Issue 25940: Python 3: Chain complex fixes

archive/issues_025703.json:
```json
{
    "body": "This implements a hash function for chain complexes, and it also fixes a sorting issue in the `_latex_` method. Combined with #25935, this makes tests pass for `src/sage/homology/chain_*` with Python 3.\n\n\nCC:  @fchapoton\n\nKeywords: sagedays@icerm\n\nBranch/Commit: 0721159086bd74945039c402504ec992002384ed\n\nReviewer: Travis Scrimshaw\n\nAuthor: John Palmieri\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/25940\n\n",
    "closed_at": "2018-08-07T20:40:18Z",
    "created_at": "2018-07-26T18:49:40Z",
    "labels": [
        "component: python3",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.4",
    "title": "Python 3: Chain complex fixes",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25940",
    "user": "https://github.com/jhpalmieri"
}
```
This implements a hash function for chain complexes, and it also fixes a sorting issue in the `_latex_` method. Combined with #25935, this makes tests pass for `src/sage/homology/chain_*` with Python 3.


CC:  @fchapoton

Keywords: sagedays@icerm

Branch/Commit: 0721159086bd74945039c402504ec992002384ed

Reviewer: Travis Scrimshaw

Author: John Palmieri

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/25940





---

archive/issue_comments_498285.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jhpalmieri/chain-complex-py3\"",
    "created_at": "2018-07-26T18:50:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498285",
    "user": "https://github.com/jhpalmieri"
}
```

Changing branch from "" to "u/jhpalmieri/chain-complex-py3"



---

archive/issue_comments_498286.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-07-26T18:54:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498286",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_498287.json:
```json
{
    "body": "Changing commit from \"\" to \"06fa117b8ea3d74093b7216a0535bfd4879a58ab\"",
    "created_at": "2018-07-26T18:54:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498287",
    "user": "https://github.com/jhpalmieri"
}
```

Changing commit from "" to "06fa117b8ea3d74093b7216a0535bfd4879a58ab"



---

archive/issue_comments_498288.json:
```json
{
    "body": "<a id='comment:2'></a>\nRegarding `_latex_`: there used to be an `if...else` block, depending on whether the chain complex was graded over **Z** or over some other group. First, there was a bug in the code, so it was printing the wrong thing, and also printing incomplete information, in the non **Z** case. Second, the special case is not needed: the **Z** code works just as well for other cases, I think. There may be problems if the grading group is not free abelian, I'm not sure. In any case, this is an improvement over the previous situation.\n\n---\nNew commits:\n|                                                                                                                                          |                                                 |\n|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------|\n|[06fa117](https://git.sagemath.org/sage.git/commit?id=06fa117b8ea3d74093b7216a0535bfd4879a58ab)|`trac 25940: Python 3 fixes for chain_complex.py`|",
    "created_at": "2018-07-26T18:54:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498288",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
Regarding `_latex_`: there used to be an `if...else` block, depending on whether the chain complex was graded over **Z** or over some other group. First, there was a bug in the code, so it was printing the wrong thing, and also printing incomplete information, in the non **Z** case. Second, the special case is not needed: the **Z** code works just as well for other cases, I think. There may be problems if the grading group is not free abelian, I'm not sure. In any case, this is an improvement over the previous situation.

---
New commits:
|                                                                                                                                          |                                                 |
|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------|
|[06fa117](https://git.sagemath.org/sage.git/commit?id=06fa117b8ea3d74093b7216a0535bfd4879a58ab)|`trac 25940: Python 3 fixes for chain_complex.py`|



---

archive/issue_comments_498289.json:
```json
{
    "body": "<a id='comment:3'></a>\nLGTM modulo one probably extremely unlikely corner case if `deg` is in `SR`:\n\n```\nsage: SR(-1) < 0\n-1 < 0\nsage: sorted([1,2,3,4], reverse=(SR(-1) < 0))\n---------------------------------------------------------------------------\nValueError                                Traceback (most recent call last)\n<ipython-input-2-600718e2dfaa> in <module>()\n----> 1 sorted([Integer(1),Integer(2),Integer(3),Integer(4)], reverse=(SR(-Integer(1)) < Integer(0)))\n\n/home/travis/sage-build/local/lib/python2.7/site-packages/sage/symbolic/expression.pyx in sage.symbolic.expression.Expression.__int__ (build/cythonized/sage/symbolic/expression.cpp:9460)()\n   1112             rif_self = sage.rings.all.RIF(self)\n   1113         except TypeError:\n-> 1114             raise ValueError(\"cannot convert %s to int\" % self)\n   1115         if rif_self > 0 or (rif_self.contains_zero() and self > 0):\n   1116             result = floor(self)\n\nValueError: cannot convert -1 < 0 to int\n```\nThe fix would be `bool(deg < 0)`:\n\n```\nsage: sorted([1,2,3,4], reverse=bool(SR(-1) < 0))\n[4, 3, 2, 1]\n```",
    "created_at": "2018-07-26T23:15:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498289",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>
LGTM modulo one probably extremely unlikely corner case if `deg` is in `SR`:

```
sage: SR(-1) < 0
-1 < 0
sage: sorted([1,2,3,4], reverse=(SR(-1) < 0))
---------------------------------------------------------------------------
ValueError                                Traceback (most recent call last)
<ipython-input-2-600718e2dfaa> in <module>()
----> 1 sorted([Integer(1),Integer(2),Integer(3),Integer(4)], reverse=(SR(-Integer(1)) < Integer(0)))

/home/travis/sage-build/local/lib/python2.7/site-packages/sage/symbolic/expression.pyx in sage.symbolic.expression.Expression.__int__ (build/cythonized/sage/symbolic/expression.cpp:9460)()
   1112             rif_self = sage.rings.all.RIF(self)
   1113         except TypeError:
-> 1114             raise ValueError("cannot convert %s to int" % self)
   1115         if rif_self > 0 or (rif_self.contains_zero() and self > 0):
   1116             result = floor(self)

ValueError: cannot convert -1 < 0 to int
```
The fix would be `bool(deg < 0)`:

```
sage: sorted([1,2,3,4], reverse=bool(SR(-1) < 0))
[4, 3, 2, 1]
```



---

archive/issue_comments_498290.json:
```json
{
    "body": "Changing keywords from \"\" to \"sagedays@icerm\".",
    "created_at": "2018-07-26T23:15:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498290",
    "user": "https://github.com/tscrim"
}
```

Changing keywords from "" to "sagedays@icerm".



---

archive/issue_comments_498291.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2018-07-26T23:15:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498291",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_498292.json:
```json
{
    "body": "<a id='comment:4'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                   |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------|\n|[0721159](https://git.sagemath.org/sage.git/commit/?id=0721159086bd74945039c402504ec992002384ed)|`trac 25940: use bool(deg < 0) instead of (deg < 0), just in case.`|",
    "created_at": "2018-07-26T23:28:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498292",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                   |
|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------|
|[0721159](https://git.sagemath.org/sage.git/commit/?id=0721159086bd74945039c402504ec992002384ed)|`trac 25940: use bool(deg < 0) instead of (deg < 0), just in case.`|



---

archive/issue_comments_498293.json:
```json
{
    "body": "Changing commit from \"06fa117b8ea3d74093b7216a0535bfd4879a58ab\" to \"0721159086bd74945039c402504ec992002384ed\"",
    "created_at": "2018-07-26T23:28:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498293",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "06fa117b8ea3d74093b7216a0535bfd4879a58ab" to "0721159086bd74945039c402504ec992002384ed"



---

archive/issue_comments_498294.json:
```json
{
    "body": "<a id='comment:5'></a>\nGood idea.",
    "created_at": "2018-07-26T23:28:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498294",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>
Good idea.



---

archive/issue_comments_498295.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-07-26T23:37:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498295",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_498296.json:
```json
{
    "body": "<a id='comment:6'></a>\nLGTM. Thanks.",
    "created_at": "2018-07-26T23:37:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498296",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'></a>
LGTM. Thanks.



---

archive/issue_events_064843.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-07T20:40:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25940#event-64843"
}
```



---

archive/issue_comments_498297.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-08-07T20:40:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498297",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_498298.json:
```json
{
    "body": "Changing branch from \"u/jhpalmieri/chain-complex-py3\" to \"0721159086bd74945039c402504ec992002384ed\"",
    "created_at": "2018-08-07T20:40:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25940#issuecomment-498298",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/jhpalmieri/chain-complex-py3" to "0721159086bd74945039c402504ec992002384ed"
