# Issue 25664: encoding issue in databases/findstat.py

archive/issues_025427.json:
```json
{
    "assignees": [],
    "body": "#25472 introduces a non-ascii character, that breaks the docbuild on my 32bit VM:\n\n```\n[dochtml]   File \"/opt/sagemath/sage-8.2/local/lib/python2.7/site-packages/sage/databases/findstat.py\", line 77\n[dochtml] SyntaxError: Non-ASCII character '\\xc3' in file /opt/sagemath/sage-8.2/local/lib/python2.7/site-packages/sage/databases/findstat.py on line 78, but no encoding declared; see http://python.org/dev/peps/pep-0263/ for details\n```\n\n\nCC:  @seblabbe @mantepse @fchapoton\n\nKeywords: **sdl**\n\nBranch: **[d21a299](https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Thierry Monteil**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25664_\n\n",
    "closed_at": "2018-06-29T22:33:44Z",
    "created_at": "2018-06-26T07:17:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "encoding issue in databases/findstat.py",
    "type": "issue",
    "updated_at": "2019-08-27T19:45:19Z",
    "url": "https://github.com/sagemath/sage/issues/25664",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
#25472 introduces a non-ascii character, that breaks the docbuild on my 32bit VM:

```
[dochtml]   File "/opt/sagemath/sage-8.2/local/lib/python2.7/site-packages/sage/databases/findstat.py", line 77
[dochtml] SyntaxError: Non-ASCII character '\xc3' in file /opt/sagemath/sage-8.2/local/lib/python2.7/site-packages/sage/databases/findstat.py on line 78, but no encoding declared; see http://python.org/dev/peps/pep-0263/ for details
```


CC:  @seblabbe @mantepse @fchapoton

Keywords: **sdl**

Branch: **[d21a299](https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec)**

Reviewer: **Frédéric Chapoton**

Author: **Thierry Monteil**

_Issue created by migration from https://trac.sagemath.org/ticket/25664_





---

archive/issue_events_336817.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2018-06-26T07:17:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25664#event-336817"
}
```



---

archive/issue_events_336818.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2018-06-26T07:17:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25664#event-336818"
}
```



---

archive/issue_events_336819.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2018-06-26T07:17:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25664#event-336819"
}
```



---

archive/issue_events_336820.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2018-06-26T07:17:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25664#event-336820"
}
```



---

archive/issue_comments_396791.json:
```json
{
    "body": "Branch: **[u/tmonteil/encoding_issue_in_databases_findstat_py](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/encoding_issue_in_databases_findstat_py)**",
    "created_at": "2018-06-26T07:21:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396791",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Branch: **[u/tmonteil/encoding_issue_in_databases_findstat_py](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/encoding_issue_in_databases_findstat_py)**



---

archive/issue_events_336821.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2018-06-26T07:23:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25664#event-336821"
}
```



---

archive/issue_comments_396792.json:
```json
{
    "body": "Commit: **[d21a299](https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec)**",
    "created_at": "2018-06-26T07:23:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396792",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Commit: **[d21a299](https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec)**



---

archive/issue_comments_396793.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec\">d21a299</a></td><td><code>#25664 : fix findstat encoding issue</code></td></tr></table>\n",
    "created_at": "2018-06-26T07:23:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396793",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec">d21a299</a></td><td><code>#25664 : fix findstat encoding issue</code></td></tr></table>




---

archive/issue_events_336822.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-26T07:24:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25664#event-336822"
}
```



---

archive/issue_events_336823.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-26T07:24:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25664#event-336823"
}
```



---

archive/issue_comments_396794.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok",
    "created_at": "2018-06-26T07:24:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396794",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok



---

archive/issue_comments_396795.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-06-26T07:24:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396795",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_396796.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks! It was a tough week and I didn't find time to fill a ticket myself.",
    "created_at": "2018-06-28T07:25:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396796",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks! It was a tough week and I didn't find time to fill a ticket myself.



---

archive/issue_comments_396797.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-#25472 introduces a non-ascii character, that beacks the docbuild on my 32bit VM:\n+#25472 introduces a non-ascii character, that breaks the docbuild on my 32bit VM:\n \n ```\n [dochtml]   File \"/opt/sagemath/sage-8.2/local/lib/python2.7/site-packages/sage/databases/findstat.py\", line 77\n``````\n",
    "created_at": "2018-06-28T09:28:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396797",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-#25472 introduces a non-ascii character, that beacks the docbuild on my 32bit VM:
+#25472 introduces a non-ascii character, that breaks the docbuild on my 32bit VM:
 
 ```
 [dochtml]   File "/opt/sagemath/sage-8.2/local/lib/python2.7/site-packages/sage/databases/findstat.py", line 77
``````




---

archive/issue_comments_396798.json:
```json
{
    "body": "Changed branch from **[u/tmonteil/encoding_issue_in_databases_findstat_py](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/encoding_issue_in_databases_findstat_py)** to **[d21a299](https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec)**",
    "created_at": "2018-06-29T22:33:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396798",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/tmonteil/encoding_issue_in_databases_findstat_py](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/encoding_issue_in_databases_findstat_py)** to **[d21a299](https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec)**



---

archive/issue_events_336824.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-06-29T22:33:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25664#event-336824"
}
```



---

archive/issue_events_336825.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1dbd12183653c275bed1c1c068e8f6e88f50f629",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-06-29T22:33:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25664#event-336825"
}
```



---

archive/issue_comments_396799.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks. I wasn't able to build 8.3.beta7 on my patchbot because of this.",
    "created_at": "2018-07-03T10:59:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396799",
    "user": "https://github.com/embray"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks. I wasn't able to build 8.3.beta7 on my patchbot because of this.



---

archive/issue_comments_396800.json:
```json
{
    "body": "Changed commit from **[d21a299](https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec)** to none",
    "created_at": "2018-07-03T10:59:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396800",
    "user": "https://github.com/embray"
}
```

Changed commit from **[d21a299](https://github.com/sagemath/sagetrac-mirror/commit/d21a2991b8beb9fbfc355e94781d882ca008edec)** to none



---

archive/issue_comments_396801.json:
```json
{
    "body": "Changed keywords from none to **sdl**",
    "created_at": "2019-08-27T19:45:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25664#issuecomment-396801",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Changed keywords from none to **sdl**
