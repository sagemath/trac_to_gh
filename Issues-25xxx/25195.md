# Issue 25195: py3: fix segfault in element wrapper on Python 3

archive/issues_024958.json:
```json
{
    "assignees": [],
    "body": "This generated code is later used in such a way that assumes the `mapped_args` variable is a `list`--using `map()` on Python 3 sets it to a `map` object instead, resulting in a segfault when trying to use it as a `list`.\n\nBranch/Commit: **[8748726](https://github.com/sagemath/sagetrac-mirror/commit/8748726273096559dae4d02c1e0058c857f599ff)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25195_\n\n",
    "closed_at": "2018-05-14T17:35:45Z",
    "created_at": "2018-04-17T14:51:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fix segfault in element wrapper on Python 3",
    "type": "issue",
    "updated_at": "2018-05-14T17:35:45Z",
    "url": "https://github.com/sagemath/sage/issues/25195",
    "user": "https://github.com/embray"
}
```
This generated code is later used in such a way that assumes the `mapped_args` variable is a `list`--using `map()` on Python 3 sets it to a `map` object instead, resulting in a segfault when trying to use it as a `list`.

Branch/Commit: **[8748726](https://github.com/sagemath/sagetrac-mirror/commit/8748726273096559dae4d02c1e0058c857f599ff)**

Reviewer: **Jeroen Demeyer**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/25195_





---

archive/issue_events_330695.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-17T14:51:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330695"
}
```



---

archive/issue_events_330696.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-17T14:51:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330696"
}
```



---

archive/issue_events_330697.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-17T14:51:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330697"
}
```



---

archive/issue_events_330698.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-17T14:51:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330698"
}
```



---

archive/issue_events_330699.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-17T14:51:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330699"
}
```



---

archive/issue_comments_387945.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2018-04-20T10:41:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25195#issuecomment-387945",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_330700.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-20T10:41:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330700"
}
```



---

archive/issue_events_330701.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-20T10:41:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330701"
}
```



---

archive/issue_events_330702.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330702"
}
```



---

archive/issue_events_330703.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330703"
}
```



---

archive/issue_comments_387946.json:
```json
{
    "body": "Changed branch from **[u/embray/python3/autogen/map](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/autogen/map)** to **[8748726](https://github.com/sagemath/sagetrac-mirror/commit/8748726273096559dae4d02c1e0058c857f599ff)**",
    "created_at": "2018-05-14T17:35:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25195#issuecomment-387946",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/python3/autogen/map](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/autogen/map)** to **[8748726](https://github.com/sagemath/sagetrac-mirror/commit/8748726273096559dae4d02c1e0058c857f599ff)**



---

archive/issue_events_330704.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-14T17:35:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330704"
}
```



---

archive/issue_events_330705.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "eba77d3f14e47865985828e8a33d2bb45b6bba23",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-14T17:35:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25195",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25195#event-330705"
}
```
