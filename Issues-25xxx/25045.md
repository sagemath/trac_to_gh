# Issue 25045: Add DESTDIR support for mpfr

archive/issues_024808.json:
```json
{
    "body": "Implements #24024 for mpfr.\n\nThis one is slightly non-trivial especially in how it uses multiple `configure` calls.  However, my local experiments show that it's just fine to use `sdh_configure` there all the same (though it isn't strictly necessary either).\n\nKeywords: destdir mpfr\n\nBranch/Commit: faec6430e6e9eb433728282b821bab6f910557f2\n\nReviewer: Julian R\u00fcth\n\nAuthor: Erik Bray\n\nDependencies: #23733\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/25045\n\n",
    "closed_at": "2018-08-05T08:42:56Z",
    "created_at": "2018-03-27T15:18:18Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.4",
    "title": "Add DESTDIR support for mpfr",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25045",
    "user": "https://github.com/embray"
}
```
Implements #24024 for mpfr.

This one is slightly non-trivial especially in how it uses multiple `configure` calls.  However, my local experiments show that it's just fine to use `sdh_configure` there all the same (though it isn't strictly necessary either).

Keywords: destdir mpfr

Branch/Commit: faec6430e6e9eb433728282b821bab6f910557f2

Reviewer: Julian Rüth

Author: Erik Bray

Dependencies: #23733

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/25045





---

archive/issue_comments_476786.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-03-27T15:18:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476786",
    "user": "https://github.com/embray"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_476787.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-03-29T07:53:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476787",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_476788.json:
```json
{
    "body": "<a id='comment:2'></a>\nHere, I see no reason to use `sdh_configure`. This is not the real `./configure` run, it is only needed to check some defaults:\n\n```\n        ./configure --with-gmp=\"$SAGE_LOCAL\" $SAGE_CONF_OPTS $MPFR_CONFIGURE) &>/dev/null;\n```\n\nYou don't need\n\n```\nError configuring MPFR.\nSee above for the options passed to it, and the file\n  $(pwd)/config.log\nfor details.\n```\nbecause `sdh_configure` can never fail (it either succeeds or exists the script).\n|| sdh_die << _EOF",
    "created_at": "2018-03-29T07:53:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476788",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
Here, I see no reason to use `sdh_configure`. This is not the real `./configure` run, it is only needed to check some defaults:

```
        ./configure --with-gmp="$SAGE_LOCAL" $SAGE_CONF_OPTS $MPFR_CONFIGURE) &>/dev/null;
```

You don't need

```
Error configuring MPFR.
See above for the options passed to it, and the file
  $(pwd)/config.log
for details.
```
because `sdh_configure` can never fail (it either succeeds or exists the script).
|| sdh_die << _EOF



---

archive/issue_comments_476789.json:
```json
{
    "body": "<a id='comment:3'></a>\nDo we care about keeping that error message at all then?\n(I'm not sure we do--the config.log file won't even exist unless built with `SAGE_KEEP_BUILT_SPKGS=yes`).",
    "created_at": "2018-03-29T08:58:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476789",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'></a>
Do we care about keeping that error message at all then?
(I'm not sure we do--the config.log file won't even exist unless built with `SAGE_KEEP_BUILT_SPKGS=yes`).



---

archive/issue_comments_476790.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [embray](#comment%3A3):\n> Do we care about keeping that error message at all then?\n\n\nIdeally yes because the message saying to look at `config.log` is useful. But it should be put in `sdh_configure`, because it is generally useful for all packages using GNU configure. Of course, there should be a check that `config.log` exists.\n\n> the config.log file won't even exist unless built with `SAGE_KEEP_BUILT_SPKGS=yes`.\n\n\nDirectories of a *failed* build are kept anyway.",
    "created_at": "2018-03-29T09:27:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476790",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>
Replying to [embray](#comment%3A3):
> Do we care about keeping that error message at all then?


Ideally yes because the message saying to look at `config.log` is useful. But it should be put in `sdh_configure`, because it is generally useful for all packages using GNU configure. Of course, there should be a check that `config.log` exists.

> the config.log file won't even exist unless built with `SAGE_KEEP_BUILT_SPKGS=yes`.


Directories of a *failed* build are kept anyway.



---

archive/issue_comments_476791.json:
```json
{
    "body": "<a id='comment:5'></a>\nOh, I didn't know that.  Or if I did I'd forgotten.  I could move that message to `sdh_configure`.  I agree it's useful in general, at least so long as we're talking standard autoconf.  \n\nThe one wrinkle might be that in principle `config.log` can be somewhere other than `$(pwd)/config.log`.  Sometimes it will be in some place like `$(pwd)/config/config.log`.  I could either try to search for a `config.log`, or just assume `$(pwd)/config.log` and only print the message if it exists.  For most cases that should probably suffice...",
    "created_at": "2018-03-29T09:39:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476791",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>
Oh, I didn't know that.  Or if I did I'd forgotten.  I could move that message to `sdh_configure`.  I agree it's useful in general, at least so long as we're talking standard autoconf.  

The one wrinkle might be that in principle `config.log` can be somewhere other than `$(pwd)/config.log`.  Sometimes it will be in some place like `$(pwd)/config/config.log`.  I could either try to search for a `config.log`, or just assume `$(pwd)/config.log` and only print the message if it exists.  For most cases that should probably suffice...



---

archive/issue_comments_476792.json:
```json
{
    "body": "<a id='comment:6'></a>\nReplying to [embray](#comment%3A5):\n> Sometimes it will be in some place like `$(pwd)/config/config.log`.\n\n\nAre you sure? As far as I know, this cannot be customized with GNU autoconf.",
    "created_at": "2018-03-29T10:17:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476792",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>
Replying to [embray](#comment%3A5):
> Sometimes it will be in some place like `$(pwd)/config/config.log`.


Are you sure? As far as I know, this cannot be customized with GNU autoconf.



---

archive/issue_comments_476793.json:
```json
{
    "body": "<a id='comment:7'></a>\nYou know, I seem to recall seeing that before, but I wonder if that was some kind of `AC_SUBDIRS` thing going on with a trivial top-level directory or something.  You're right that in general it can't be customized.  So that makes things simpler.",
    "created_at": "2018-03-29T14:16:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476793",
    "user": "https://github.com/embray"
}
```

<a id='comment:7'></a>
You know, I seem to recall seeing that before, but I wonder if that was some kind of `AC_SUBDIRS` thing going on with a trivial top-level directory or something.  You're right that in general it can't be customized.  So that makes things simpler.



---

archive/issue_comments_476794.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#23733\"",
    "created_at": "2018-04-04T08:14:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476794",
    "user": "https://github.com/embray"
}
```

Changing dependencies from "" to "#23733"



---

archive/issue_comments_476795.json:
```json
{
    "body": "<a id='comment:9'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                          |\n|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|\n|[b4ecda9](https://git.sagemath.org/sage.git/commit/?id=b4ecda96068fe21d37277e36d631af3b4ee20ea0)|`trac 23733: deprecate SAGE64 and CFLAG64`|\n|[905e4d4](https://git.sagemath.org/sage.git/commit/?id=905e4d44a3da6d0265c9d2ae24d19691f40c0458)|`Stop supporting SAGE64 except in Numpy`|\n|[0db62eb](https://git.sagemath.org/sage.git/commit/?id=0db62ebee68d537fda728afacd9169f349d38f94)|`Merge branch 'u/jdemeyer/no-sage64' into u/embray/build/destdir-mpfr`|\n|[70d5021](https://git.sagemath.org/sage.git/commit/?id=70d50210f03cc61cc602823f1e8ed0593bada8c4)|`For sdh_configure, add a generic message with the path to config.log (if it exists) so that it can be inspected for errors`|\n|[faec643](https://git.sagemath.org/sage.git/commit/?id=faec6430e6e9eb433728282b821bab6f910557f2)|`A little additional cleanup`|",
    "created_at": "2018-04-04T08:41:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476795",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:9'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                          |
|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|
|[b4ecda9](https://git.sagemath.org/sage.git/commit/?id=b4ecda96068fe21d37277e36d631af3b4ee20ea0)|`trac 23733: deprecate SAGE64 and CFLAG64`|
|[905e4d4](https://git.sagemath.org/sage.git/commit/?id=905e4d44a3da6d0265c9d2ae24d19691f40c0458)|`Stop supporting SAGE64 except in Numpy`|
|[0db62eb](https://git.sagemath.org/sage.git/commit/?id=0db62ebee68d537fda728afacd9169f349d38f94)|`Merge branch 'u/jdemeyer/no-sage64' into u/embray/build/destdir-mpfr`|
|[70d5021](https://git.sagemath.org/sage.git/commit/?id=70d50210f03cc61cc602823f1e8ed0593bada8c4)|`For sdh_configure, add a generic message with the path to config.log (if it exists) so that it can be inspected for errors`|
|[faec643](https://git.sagemath.org/sage.git/commit/?id=faec6430e6e9eb433728282b821bab6f910557f2)|`A little additional cleanup`|



---

archive/issue_comments_476796.json:
```json
{
    "body": "Changing commit from \"c4e8f6f56081fb32b0d031c9058b6db6be8b7a9e\" to \"faec6430e6e9eb433728282b821bab6f910557f2\"",
    "created_at": "2018-04-04T08:41:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476796",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "c4e8f6f56081fb32b0d031c9058b6db6be8b7a9e" to "faec6430e6e9eb433728282b821bab6f910557f2"



---

archive/issue_comments_476797.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-04-04T08:41:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476797",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_events_062828.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25045#event-62828"
}
```



---

archive/issue_comments_476798.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Julian R\u00fcth\"",
    "created_at": "2018-07-17T15:40:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476798",
    "user": "https://github.com/saraedum"
}
```

Changing reviewer from "" to "Julian Rüth"



---

archive/issue_comments_476799.json:
```json
{
    "body": "<a id='comment:12'></a>\nLooks good to me (though it does a bit more than what it says in the ticket description.) Feel free to set this to positive review if you think that this still works.",
    "created_at": "2018-07-17T15:40:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476799",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:12'></a>
Looks good to me (though it does a bit more than what it says in the ticket description.) Feel free to set this to positive review if you think that this still works.



---

archive/issue_comments_476800.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-07-17T17:35:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476800",
    "user": "https://github.com/embray"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_062829.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:47:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25045#event-62829"
}
```



---

archive/issue_events_062830.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:47:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25045#event-62830"
}
```



---

archive/issue_comments_476801.json:
```json
{
    "body": "<a id='comment:14'></a>\nI believe this issue can reasonably be addressed for Sage 8.4.",
    "created_at": "2018-07-18T11:47:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476801",
    "user": "https://github.com/embray"
}
```

<a id='comment:14'></a>
I believe this issue can reasonably be addressed for Sage 8.4.



---

archive/issue_comments_476802.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-08-05T08:42:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476802",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_476803.json:
```json
{
    "body": "Changing branch from \"u/embray/build/destdir-mpfr\" to \"faec6430e6e9eb433728282b821bab6f910557f2\"",
    "created_at": "2018-08-05T08:42:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25045#issuecomment-476803",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/embray/build/destdir-mpfr" to "faec6430e6e9eb433728282b821bab6f910557f2"



---

archive/issue_events_062831.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-05T08:42:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/25045",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25045#event-62831"
}
```
