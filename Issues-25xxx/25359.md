# Issue 25359: Replace python 5755 patch by distutils patch

archive/issues_025122.json:
```json
{
    "body": "Compile Python itself with `-Wstrict-prototypes` but remove this flag when compiling extensions.\n\n**Upstream**: https://github.com/python/cpython/pull/6791\n\nCC:  @timokau\n\nUpstream: None of the above - read trac for reasoning.\n\nAuthor: Jeroen Demeyer\n\nBranch: u/jdemeyer/replace_python_5755_patch_by_distutils_patch\n\nDependencies: #25204\n\nCommit: 6bb4cac88218c7e8a97dfb2a74ad33c7c8027713\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/25359\n\n",
    "closed_at": "2018-06-04T12:51:16Z",
    "created_at": "2018-05-14T10:11:00Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Replace python 5755 patch by distutils patch",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25359",
    "user": "https://github.com/jdemeyer"
}
```
Compile Python itself with `-Wstrict-prototypes` but remove this flag when compiling extensions.

**Upstream**: https://github.com/python/cpython/pull/6791

CC:  @timokau

Upstream: None of the above - read trac for reasoning.

Author: Jeroen Demeyer

Branch: u/jdemeyer/replace_python_5755_patch_by_distutils_patch

Dependencies: #25204

Commit: 6bb4cac88218c7e8a97dfb2a74ad33c7c8027713

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/25359





---

archive/issue_comments_484369.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+Compile Python itself with `-Wstrict-prototypes` but remove this flag when compiling extensions.\n \n+**Upstream**: https://github.com/python/cpython/pull/6791\n``````\n",
    "created_at": "2018-05-14T10:30:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484369",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+Compile Python itself with `-Wstrict-prototypes` but remove this flag when compiling extensions.
 
+**Upstream**: https://github.com/python/cpython/pull/6791
``````




---

archive/issue_comments_484370.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"Reported upstream. No feedback yet.\"",
    "created_at": "2018-05-14T10:30:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484370",
    "user": "https://github.com/jdemeyer"
}
```

Changing upstream from "N/A" to "Reported upstream. No feedback yet."



---

archive/issue_comments_484371.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jdemeyer/replace_python_5755_patch_by_distutils_patch\"",
    "created_at": "2018-05-14T13:16:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484371",
    "user": "https://github.com/jdemeyer"
}
```

Changing branch from "" to "u/jdemeyer/replace_python_5755_patch_by_distutils_patch"



---

archive/issue_comments_484372.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-05-14T13:20:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484372",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_484373.json:
```json
{
    "body": "<a id='comment:3'></a>New commits:",
    "created_at": "2018-05-14T13:20:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484373",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>New commits:



---

archive/issue_comments_484374.json:
```json
{
    "body": "Changing commit from \"\" to \"6bb4cac88218c7e8a97dfb2a74ad33c7c8027713\"",
    "created_at": "2018-05-14T13:20:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484374",
    "user": "https://github.com/jdemeyer"
}
```

Changing commit from "" to "6bb4cac88218c7e8a97dfb2a74ad33c7c8027713"



---

archive/issue_comments_484375.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-05-15T05:43:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484375",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_484376.json:
```json
{
    "body": "Changing upstream from \"Reported upstream. No feedback yet.\" to \"Reported upstream. Developers deny it's a bug.\"",
    "created_at": "2018-05-15T12:16:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484376",
    "user": "https://github.com/jdemeyer"
}
```

Changing upstream from "Reported upstream. No feedback yet." to "Reported upstream. Developers deny it's a bug."



---

archive/issue_comments_484377.json:
```json
{
    "body": "<a id='comment:6'></a>Where did the developers deny its a bug?",
    "created_at": "2018-05-15T12:20:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484377",
    "user": "https://github.com/timokau"
}
```

<a id='comment:6'></a>Where did the developers deny its a bug?



---

archive/issue_comments_484378.json:
```json
{
    "body": "Changing upstream from \"Reported upstream. Developers deny it's a bug.\" to \"None of the above - read trac for reasoning.\"",
    "created_at": "2018-05-15T12:23:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484378",
    "user": "https://github.com/jdemeyer"
}
```

Changing upstream from "Reported upstream. Developers deny it's a bug." to "None of the above - read trac for reasoning."



---

archive/issue_events_063575.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-04T12:51:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25359#event-63575"
}
```



---

archive/issue_comments_484379.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2018-06-04T12:51:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484379",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: invalid



---

archive/issue_comments_484380.json:
```json
{
    "body": "<a id='comment:8'></a>Upstream CPython has accepted the patch which was originally in Sage.",
    "created_at": "2018-06-04T12:51:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25359#issuecomment-484380",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>Upstream CPython has accepted the patch which was originally in Sage.



---

archive/issue_events_063576.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-04T12:51:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25359",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25359#event-63576"
}
```
