# Issue 25961: Implement Column Bump and Reverse Column Bump for Semistandard Tableaux

archive/issues_025724.json:
```json
{
    "assignees": [],
    "body": "Currently, semistandard tableau (or any tableau with weakly increasing rows and strictly increasing columns) only have methods for row bumping and reverse row bumping. They should also be able to do column bumping and reverse column bumping.\n\nSpecifically interested in using this to implement cyclage posets (future ticket).\n\nPotentially related to #25070 , except that would run through the entire column bumping/reverse bumping procesure, and we want to only do a single step.\n\nKeywords: **sagedays@icerm**\n\nComponent: **combinatorics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25961_\n\n",
    "created_at": "2018-07-27T19:33:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement Column Bump and Reverse Column Bump for Semistandard Tableaux",
    "type": "issue",
    "updated_at": "2022-12-29T01:42:26Z",
    "url": "https://github.com/sagemath/sage/issues/25961",
    "user": "https://github.com/kevindilks"
}
```
Currently, semistandard tableau (or any tableau with weakly increasing rows and strictly increasing columns) only have methods for row bumping and reverse row bumping. They should also be able to do column bumping and reverse column bumping.

Specifically interested in using this to implement cyclage posets (future ticket).

Potentially related to #25070 , except that would run through the entire column bumping/reverse bumping procesure, and we want to only do a single step.

Keywords: **sagedays@icerm**

Component: **combinatorics**

_Issue created by migration from https://trac.sagemath.org/ticket/25961_





---

archive/issue_events_358677.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2018-07-27T19:33:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25961",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25961#event-358677"
}
```



---

archive/issue_events_358678.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2018-07-27T19:33:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25961",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25961#event-358678"
}
```



---

archive/issue_events_358679.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2018-07-27T19:33:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25961",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25961#event-358679"
}
```



---

archive/issue_events_358680.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2018-07-27T19:33:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25961",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25961#event-358680"
}
```



---

archive/issue_comments_399704.json:
```json
{
    "body": "Changed keywords from none to **sagedays@icerm**",
    "created_at": "2018-07-27T19:33:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25961#issuecomment-399704",
    "user": "https://github.com/kevindilks"
}
```

Changed keywords from none to **sagedays@icerm**



---

archive/issue_comments_399705.json:
```json
{
    "body": "I realized this is for cocyclage, not cyclage, so this might not actually be relevant.",
    "created_at": "2018-07-27T20:31:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25961#issuecomment-399705",
    "user": "https://github.com/jdmeza"
}
```

I realized this is for cocyclage, not cyclage, so this might not actually be relevant.



---

archive/issue_events_358681.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:42:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25961",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25961#event-358681"
}
```



---

archive/issue_comments_399706.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[cyclage.sagews.gz](https://github.com/sagemath/sage/files/ticket25961/cyclage.sagews.gz)**",
    "created_at": "2022-12-29T01:42:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25961#issuecomment-399706",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[cyclage.sagews.gz](https://github.com/sagemath/sage/files/ticket25961/cyclage.sagews.gz)**
