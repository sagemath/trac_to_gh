# Issue 25912: Infinite loop in pkg-config

archive/issues_025675.json:
```json
{
    "body": "Under some circumstances, `pkg-config` ends up executing itself.\n\nThis is the file `/home/jdemeyer/sage-test/local/bin/pkg-config`:\n\n```/usr/bin/env bash\n\nif [ -z \"$SAGE_LOCAL\" ]; then\n    echo >&2 \"This script requires that SAGE_LOCAL is set.\"\n    exit 1\nfi\n\nif [ -z \"$PKG_CONFIG_PATH\" ]; then\n    export PKG_CONFIG_PATH=\"$SAGE_LOCAL/lib/pkgconfig:$SAGE_LOCAL/share/pkgconfig\"\nelse\n    export PKG_CONFIG_PATH=\"$SAGE_LOCAL/lib/pkgconfig:$SAGE_LOCAL/share/pkgconfig:$PKG_CONFIG_PATH\"\nfi\n\n\n\nexec /home/jdemeyer/sage-test/local/bin/pkg-config \"$@\"\n```\n\nCC:  @embray\n\nBranch/Commit: 9605ff8587241409f045ecc36a26725606c9d5a9\n\nReviewer: Jeroen Demeyer\n\nAuthor: Erik Bray\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/25912\n\n",
    "closed_at": "2018-07-27T23:06:02Z",
    "created_at": "2018-07-24T09:42:28Z",
    "labels": [
        "component: packages: standard",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.3",
    "title": "Infinite loop in pkg-config",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25912",
    "user": "https://github.com/jdemeyer"
}
```
Under some circumstances, `pkg-config` ends up executing itself.

This is the file `/home/jdemeyer/sage-test/local/bin/pkg-config`:

```/usr/bin/env bash

if [ -z "$SAGE_LOCAL" ]; then
    echo >&2 "This script requires that SAGE_LOCAL is set."
    exit 1
fi

if [ -z "$PKG_CONFIG_PATH" ]; then
    export PKG_CONFIG_PATH="$SAGE_LOCAL/lib/pkgconfig:$SAGE_LOCAL/share/pkgconfig"
else
    export PKG_CONFIG_PATH="$SAGE_LOCAL/lib/pkgconfig:$SAGE_LOCAL/share/pkgconfig:$PKG_CONFIG_PATH"
fi



exec /home/jdemeyer/sage-test/local/bin/pkg-config "$@"
```

CC:  @embray

Branch/Commit: 9605ff8587241409f045ecc36a26725606c9d5a9

Reviewer: Jeroen Demeyer

Author: Erik Bray

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/25912





---

archive/issue_comments_497423.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe problem is that the m4 template that generates `$SAGE_LOCAL/bin/pkg-config` uses `command -v pkg-config` to get the path to the \"actual\" `pkg-config`, which ends up being a self-reference if `$SAGE_LOCAL/bin/pkg-config` is already installed (which it shouldn't be, but it could be if something went wrong cleaning/upgrading the pkgconf spkg).\n\nAn easy workaround is to just make absolutely sure to remove `$SAGE_LOCAL/bin/pkg-config`.",
    "created_at": "2018-07-24T10:13:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497423",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'></a>
The problem is that the m4 template that generates `$SAGE_LOCAL/bin/pkg-config` uses `command -v pkg-config` to get the path to the "actual" `pkg-config`, which ends up being a self-reference if `$SAGE_LOCAL/bin/pkg-config` is already installed (which it shouldn't be, but it could be if something went wrong cleaning/upgrading the pkgconf spkg).

An easy workaround is to just make absolutely sure to remove `$SAGE_LOCAL/bin/pkg-config`.



---

archive/issue_comments_497424.json:
```json
{
    "body": "Changing commit from \"\" to \"b10bbb7f64471daf317fc542c44fab3fb5f7d327\"",
    "created_at": "2018-07-24T10:18:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497424",
    "user": "https://github.com/embray"
}
```

Changing commit from "" to "b10bbb7f64471daf317fc542c44fab3fb5f7d327"



---

archive/issue_comments_497425.json:
```json
{
    "body": "<a id='comment:2'></a>\nNew commits:\n|                                                                                                                                          |                                                                       |\n|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------|\n|[b10bbb7](https://git.sagemath.org/sage.git/commit?id=b10bbb7f64471daf317fc542c44fab3fb5f7d327)|`Make absolutely sure that whatever else happened before installing or`|",
    "created_at": "2018-07-24T10:18:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497425",
    "user": "https://github.com/embray"
}
```

<a id='comment:2'></a>
New commits:
|                                                                                                                                          |                                                                       |
|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------|
|[b10bbb7](https://git.sagemath.org/sage.git/commit?id=b10bbb7f64471daf317fc542c44fab3fb5f7d327)|`Make absolutely sure that whatever else happened before installing or`|



---

archive/issue_comments_497426.json:
```json
{
    "body": "Changing branch from \"\" to \"u/embray/ticket-25912\"",
    "created_at": "2018-07-24T10:18:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497426",
    "user": "https://github.com/embray"
}
```

Changing branch from "" to "u/embray/ticket-25912"



---

archive/issue_comments_497427.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-07-24T10:18:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497427",
    "user": "https://github.com/embray"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_497428.json:
```json
{
    "body": "Changing author from \"\" to \"Erik Bray\"",
    "created_at": "2018-07-24T10:18:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497428",
    "user": "https://github.com/embray"
}
```

Changing author from "" to "Erik Bray"



---

archive/issue_comments_497429.json:
```json
{
    "body": "<a id='comment:3'></a>\nYou should bump the version again...",
    "created_at": "2018-07-24T10:43:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497429",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>
You should bump the version again...



---

archive/issue_comments_497430.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-07-24T10:43:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497430",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_497431.json:
```json
{
    "body": "<a id='comment:4'></a>\nMinor nitpick: why call the temporary file `pkg-config.out`? I would prefer the final name `pkg-config` there.",
    "created_at": "2018-07-24T10:45:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497431",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>
Minor nitpick: why call the temporary file `pkg-config.out`? I would prefer the final name `pkg-config` there.



---

archive/issue_comments_497432.json:
```json
{
    "body": "<a id='comment:5'></a>\nIt's the way it's been for a long time.  I don't care either way.",
    "created_at": "2018-07-24T12:34:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497432",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>
It's the way it's been for a long time.  I don't care either way.



---

archive/issue_comments_497433.json:
```json
{
    "body": "<a id='comment:6'></a>\nApparently this was done for a reason, in #16274.  I don't know if that reason still applies (I suspect not) but I'm not in a hurry to find out.",
    "created_at": "2018-07-24T12:40:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497433",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'></a>
Apparently this was done for a reason, in #16274.  I don't know if that reason still applies (I suspect not) but I'm not in a hurry to find out.



---

archive/issue_comments_497434.json:
```json
{
    "body": "<a id='comment:7'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                                                                                             |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|\n|[9605ff8](https://git.sagemath.org/sage.git/commit/?id=9605ff8587241409f045ecc36a26725606c9d5a9)|`although not strictly necessary, it would be good to bump the patch level if nothing else so the buildbots can see that the package changed`|",
    "created_at": "2018-07-24T12:42:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497434",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:7'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                                                                                             |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|
|[9605ff8](https://git.sagemath.org/sage.git/commit/?id=9605ff8587241409f045ecc36a26725606c9d5a9)|`although not strictly necessary, it would be good to bump the patch level if nothing else so the buildbots can see that the package changed`|



---

archive/issue_comments_497435.json:
```json
{
    "body": "Changing commit from \"b10bbb7f64471daf317fc542c44fab3fb5f7d327\" to \"9605ff8587241409f045ecc36a26725606c9d5a9\"",
    "created_at": "2018-07-24T12:42:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497435",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "b10bbb7f64471daf317fc542c44fab3fb5f7d327" to "9605ff8587241409f045ecc36a26725606c9d5a9"



---

archive/issue_comments_497436.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-07-24T12:42:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497436",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_497437.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-07-24T20:51:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497437",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_064783.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-07-24T20:53:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25912#event-64783"
}
```



---

archive/issue_comments_497438.json:
```json
{
    "body": "<a id='comment:10'></a>\nI doubt that the milestone actually matters, but let's be safe.",
    "created_at": "2018-07-24T20:53:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497438",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>
I doubt that the milestone actually matters, but let's be safe.



---

archive/issue_comments_497439.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2018-07-24T22:56:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497439",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_comments_497440.json:
```json
{
    "body": "<a id='comment:12'></a>\nI agree this should probably be fixed for 8.3.  I *think* an incremental upgrade direct from 8.2 to 8.3.rc2 should go smoothly, although I have not tested this.  But if someone is upgrading from a random earlier 8.3 beta or something it might not.  Fixing this now would head off a lot of likely support requests / failure reports, I suspect.\n\nBut I don't consider it a total blocker either, so I leave that up to Volker.",
    "created_at": "2018-07-25T13:27:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497440",
    "user": "https://github.com/embray"
}
```

<a id='comment:12'></a>
I agree this should probably be fixed for 8.3.  I *think* an incremental upgrade direct from 8.2 to 8.3.rc2 should go smoothly, although I have not tested this.  But if someone is upgrading from a random earlier 8.3 beta or something it might not.  Fixing this now would head off a lot of likely support requests / failure reports, I suspect.

But I don't consider it a total blocker either, so I leave that up to Volker.



---

archive/issue_comments_497441.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-07-27T23:06:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497441",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_497442.json:
```json
{
    "body": "Changing branch from \"u/embray/ticket-25912\" to \"9605ff8587241409f045ecc36a26725606c9d5a9\"",
    "created_at": "2018-07-27T23:06:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25912#issuecomment-497442",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/embray/ticket-25912" to "9605ff8587241409f045ecc36a26725606c9d5a9"



---

archive/issue_events_064784.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-07-27T23:06:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/25912",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25912#event-64784"
}
```
