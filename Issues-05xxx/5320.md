# Issue 5320: [with spkg, positive review] update Sphinx to 0.5.1, setuptools to 0.6c9

archive/issues_005320.json:
```json
{
    "body": "The spkg can be found at http://sage.math.washington.edu/home/mhansen/sphinx-0.5.1.spkg\n\nThis requires an update to setuptools 0.6c9 as well.\n\nAssignee: @mwhansen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5320\n\n",
    "closed_at": "2009-02-20T12:07:55Z",
    "created_at": "2009-02-20T11:20:32Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with spkg, positive review] update Sphinx to 0.5.1, setuptools to 0.6c9",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5320",
    "user": "https://github.com/mwhansen"
}
```
The spkg can be found at http://sage.math.washington.edu/home/mhansen/sphinx-0.5.1.spkg

This requires an update to setuptools 0.6c9 as well.

Assignee: @mwhansen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5320





---

archive/issue_comments_043590.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2009-02-20T11:27:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5320#issuecomment-43590",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_events_020905.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5320#event-20905"
}
```



---

archive/issue_events_020906.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "rename": {
        "from": "update Sphinx to 0.5.1",
        "to": "[with spkg, needs review] update Sphinx to 0.5.1"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5320#event-20906"
}
```



---

archive/issue_comments_043591.json:
```json
{
    "body": "Changing assignee from mabshoff to @mwhansen.",
    "created_at": "2009-02-20T11:27:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5320#issuecomment-43591",
    "user": "https://github.com/mwhansen"
}
```

Changing assignee from mabshoff to @mwhansen.



---

archive/issue_comments_043592.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe setup tools spkg can be found at http://sage.math.washington.edu/home/mhansen/setuptools-0.6c9.spkg",
    "created_at": "2009-02-20T11:27:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5320#issuecomment-43592",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
The setup tools spkg can be found at http://sage.math.washington.edu/home/mhansen/setuptools-0.6c9.spkg



---

archive/issue_events_020907.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "rename": {
        "from": "[with spkg, needs review] update Sphinx to 0.5.1",
        "to": "[with spkg, positive review] update Sphinx to 0.5.1, setuptools to 0.6c9"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5320#event-20907"
}
```



---

archive/issue_comments_043593.json:
```json
{
    "body": "<a id='comment:2'></a>\nBoth spkgs are clean, all changes checked in and documented. Positive review. I made spkg-install in setuptools-0.6c9.spkg executable without bumping the patch level, but if that would get lost it wouldn't matter too much.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T12:07:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5320#issuecomment-43593",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Both spkgs are clean, all changes checked in and documented. Positive review. I made spkg-install in setuptools-0.6c9.spkg executable without bumping the patch level, but if that would get lost it wouldn't matter too much.

Cheers,

Michael



---

archive/issue_events_020908.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-20T12:07:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5320#event-20908"
}
```



---

archive/issue_comments_043594.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.3.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T12:07:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5320#issuecomment-43594",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.3.rc3.

Cheers,

Michael



---

archive/issue_comments_043595.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-02-20T12:07:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5320#issuecomment-43595",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
