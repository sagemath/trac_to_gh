# Issue 5320: update Sphinx to 0.5.1, setuptools to 0.6c9

archive/issues_005320.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "The spkg can be found at http://sage.math.washington.edu/home/mhansen/sphinx-0.5.1.spkg\n\nThis requires an update to setuptools 0.6c9 as well.\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5320_\n\n",
    "closed_at": "2009-02-20T12:07:55Z",
    "created_at": "2009-02-20T11:20:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "update Sphinx to 0.5.1, setuptools to 0.6c9",
    "type": "issue",
    "updated_at": "2009-02-20T12:07:55Z",
    "url": "https://github.com/sagemath/sage/issues/5320",
    "user": "https://github.com/mwhansen"
}
```
The spkg can be found at http://sage.math.washington.edu/home/mhansen/sphinx-0.5.1.spkg

This requires an update to setuptools 0.6c9 as well.

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/5320_





---

archive/issue_events_062468.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62468"
}
```



---

archive/issue_events_062469.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62469"
}
```



---

archive/issue_events_062470.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62470"
}
```



---

archive/issue_events_062471.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62471"
}
```



---

archive/issue_events_062472.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62472"
}
```



---

archive/issue_events_062473.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62473"
}
```



---

archive/issue_events_062474.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62474"
}
```



---

archive/issue_events_062475.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62475"
}
```



---

archive/issue_events_062476.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62476"
}
```



---

archive/issue_events_062477.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62477"
}
```



---

archive/issue_comments_032823.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThe setup tools spkg can be found at http://sage.math.washington.edu/home/mhansen/setuptools-0.6c9.spkg",
    "created_at": "2009-02-20T11:27:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5320#issuecomment-32823",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

The setup tools spkg can be found at http://sage.math.washington.edu/home/mhansen/setuptools-0.6c9.spkg



---

archive/issue_events_062478.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:31Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "title_is": "update Sphinx to 0.5.1, setuptools to 0.6c9",
    "title_was": "update Sphinx to 0.5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62478"
}
```



---

archive/issue_events_062479.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62479"
}
```



---

archive/issue_events_062480.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62480"
}
```



---

archive/issue_comments_032824.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nBoth spkgs are clean, all changes checked in and documented. Positive review. I made spkg-install in setuptools-0.6c9.spkg executable without bumping the patch level, but if that would get lost it wouldn't matter too much.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T12:07:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5320#issuecomment-32824",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Both spkgs are clean, all changes checked in and documented. Positive review. I made spkg-install in setuptools-0.6c9.spkg executable without bumping the patch level, but if that would get lost it wouldn't matter too much.

Cheers,

Michael



---

archive/issue_events_062481.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62481"
}
```



---

archive/issue_events_062482.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-62482"
}
```



---

archive/issue_comments_032825.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.3.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T12:07:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5320#issuecomment-32825",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.3.rc3.

Cheers,

Michael
