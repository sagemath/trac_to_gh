# Issue 5672: Update MPIR to 1.0.rc5 (latest upstream)

archive/issues_005672.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "MPIR 1.0 brings significant speed improvements. An spkg can be found at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.4.1/rc0/gmp-mpir-1.0.rc5.spkg\n\nI have integrated two fixes for OSX and Solaris/x86 so that the test suite passes on both systems which will be in the final MPIR 1.0.\n\nCheers,\n\nMichael\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5672_\n\n",
    "closed_at": "2009-04-04T22:34:03Z",
    "created_at": "2009-04-03T01:37:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update MPIR to 1.0.rc5 (latest upstream)",
    "type": "issue",
    "updated_at": "2009-04-04T22:34:03Z",
    "url": "https://github.com/sagemath/sage/issues/5672",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
MPIR 1.0 brings significant speed improvements. An spkg can be found at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.4.1/rc0/gmp-mpir-1.0.rc5.spkg

I have integrated two fixes for OSX and Solaris/x86 so that the test suite passes on both systems which will be in the final MPIR 1.0.

Cheers,

Michael

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/5672_





---

archive/issue_events_067313.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-03T01:37:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5672#event-67313"
}
```



---

archive/issue_events_067314.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-03T01:37:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5672#event-67314"
}
```



---

archive/issue_events_067315.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-03T01:37:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5672#event-67315"
}
```



---

archive/issue_events_067316.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-03T01:37:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5672#event-67316"
}
```



---

archive/issue_events_067317.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-03T01:37:18Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5672#event-67317"
}
```



---

archive/issue_events_067318.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-03T01:37:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5672#event-67318"
}
```



---

archive/issue_comments_035959.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n MPIR 1.0 brings significant speed improvements. An spkg can be found at\n \n-/home/mabshoff/build-3.3/sage-3.3-fulvia/spkg/build/gmp-mpir-1.0.rc5\n+http://sage.math.washington.edu/home/mabshoff/release-cycles-3.4.1/rc0/gmp-mpir-1.0.rc5.spkg\n \n I have integrated two fixes for OSX and Solaris/x86 so that the test suite passes on both systems which will be in the final MPIR 1.0.\n \n``````\n",
    "created_at": "2009-04-04T05:46:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5672#issuecomment-35959",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 MPIR 1.0 brings significant speed improvements. An spkg can be found at
 
-/home/mabshoff/build-3.3/sage-3.3-fulvia/spkg/build/gmp-mpir-1.0.rc5
+http://sage.math.washington.edu/home/mabshoff/release-cycles-3.4.1/rc0/gmp-mpir-1.0.rc5.spkg
 
 I have integrated two fixes for OSX and Solaris/x86 so that the test suite passes on both systems which will be in the final MPIR 1.0.
 
``````




---

archive/issue_events_067319.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-04T06:16:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5672#event-67319"
}
```



---

archive/issue_comments_035960.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nVery good.  And it's nice that there is no patches/ directory and all issues are upstreamed :-).",
    "created_at": "2009-04-04T06:16:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5672#issuecomment-35960",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>

Very good.  And it's nice that there is no patches/ directory and all issues are upstreamed :-).



---

archive/issue_comments_035961.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.4.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-04T22:34:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5672#issuecomment-35961",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.4.rc0.

Cheers,

Michael



---

archive/issue_events_067320.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-04T22:34:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5672#event-67320"
}
```



---

archive/issue_events_067321.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-04T22:34:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5672",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5672#event-67321"
}
```
