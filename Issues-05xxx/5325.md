# Issue 5325: Make an experimental spkg for basemap

archive/issues_005325.json:
```json
{
    "assignees": [],
    "body": "This would be really nice to have for GIS things.\n\nDocs: http://matplotlib.sourceforge.net/basemap/doc/html/\n\nDownload: http://sourceforge.net/project/showfiles.php?group_id=80706&package_id=142792&release_id=653688\n\n\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/5325\n\n",
    "created_at": "2009-02-20T21:46:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wishlist"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make an experimental spkg for basemap",
    "type": "issue",
    "updated_at": "2010-05-11T20:27:24Z",
    "url": "https://github.com/sagemath/sage/issues/5325",
    "user": "https://github.com/jasongrout"
}
```
This would be really nice to have for GIS things.

Docs: http://matplotlib.sourceforge.net/basemap/doc/html/

Download: http://sourceforge.net/project/showfiles.php?group_id=80706&package_id=142792&release_id=653688



**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/5325





---

archive/issue_events_051411.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-20T21:46:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5325",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "08517b",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5325#event-51411"
}
```



---

archive/issue_events_051412.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-20T21:46:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5325",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5325#event-51412"
}
```



---

archive/issue_events_051413.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-20T21:46:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5325",
    "label": "https://github.com/sagemath/sage/labels/wishlist",
    "label_color": "008080",
    "label_name": "wishlist",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5325#event-51413"
}
```



---

archive/issue_events_051414.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T22:06:04Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5325",
    "rename": {
        "from": "Make an optional spkg for basemap",
        "to": "Make an experimental spkg for basemap"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5325#event-51414"
}
```



---

archive/issue_comments_034077.json:
```json
{
    "body": "<a id='comment:1'></a>\nHi Jason,\n\nis this all data or also code? \n\nOptional implies that we actually test this and do some sort of periodic regression testing and unless the demand is there things end up in experimental per default.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T22:06:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5325",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5325#issuecomment-34077",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'></a>
Hi Jason,

is this all data or also code? 

Optional implies that we actually test this and do some sort of periodic regression testing and unless the demand is there things end up in experimental per default.

Cheers,

Michael



---

archive/issue_comments_034078.json:
```json
{
    "body": "<a id='comment:2'></a>\nIt's both data and code.\n\nIt's okay for this to be in experimental.  There was the request for it, and it would be a cool thing to have in Sage.  I'd like to hand off the maintaining of the spkg, but I'm willing to (eventually) create an initial spkg for it.",
    "created_at": "2009-02-20T22:10:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5325",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5325#issuecomment-34078",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
It's both data and code.

It's okay for this to be in experimental.  There was the request for it, and it would be a cool thing to have in Sage.  I'd like to hand off the maintaining of the spkg, but I'm willing to (eventually) create an initial spkg for it.



---

archive/issue_events_051415.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-05-11T20:27:24Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5325",
    "milestone_number": null,
    "milestone_title": "sage-4.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5325#event-51415"
}
```
