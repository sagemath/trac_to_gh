# Issue 5588: developer's guide: more information about docstrings

archive/issues_005588.json:
```json
{
    "assignees": [
        "https://github.com/jhpalmieri"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe attachment does the following things:\n\n1. It says that the Sage style for typesetting the standard rings (ZZ, CC, etc.) is to use mathbf, not mathbb.\n\n2. It discusses trying to achieve a balance between the appearance of the docstring in interactive help vs. in the reference manual.\n\n3. In the paragraph starting \"Finally\", it mentions some nonstandard macros available for use in docstrings.  This part depends on #5555.\n\n4. It mentions that methods which start with an underscore don't appear in the reference manual, and this should be taken into consideration when writing documentation. The example in this part depends on #5529.\n\n\nComponent: **documentation**\n\nKeywords: **developer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5588_\n\n",
    "closed_at": "2009-04-30T00:59:35Z",
    "created_at": "2009-03-23T04:35:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "developer's guide: more information about docstrings",
    "type": "issue",
    "updated_at": "2009-04-30T00:59:35Z",
    "url": "https://github.com/sagemath/sage/issues/5588",
    "user": "https://github.com/jhpalmieri"
}
```
<div id="comment:0"></div>

The attachment does the following things:

1. It says that the Sage style for typesetting the standard rings (ZZ, CC, etc.) is to use mathbf, not mathbb.

2. It discusses trying to achieve a balance between the appearance of the docstring in interactive help vs. in the reference manual.

3. In the paragraph starting "Finally", it mentions some nonstandard macros available for use in docstrings.  This part depends on #5555.

4. It mentions that methods which start with an underscore don't appear in the reference manual, and this should be taken into consideration when writing documentation. The example in this part depends on #5529.


Component: **documentation**

Keywords: **developer**

_Issue created by migration from https://trac.sagemath.org/ticket/5588_





---

archive/issue_events_065655.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-03-23T04:35:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65655"
}
```



---

archive/issue_events_065656.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-03-23T04:35:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65656"
}
```



---

archive/issue_events_065657.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-03-23T04:35:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65657"
}
```



---

archive/issue_events_065658.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-03-23T04:35:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65658"
}
```



---

archive/issue_events_065659.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-03-23T04:35:58Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "subject": "https://github.com/jhpalmieri",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65659"
}
```



---

archive/issue_events_065660.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-03-23T04:35:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65660"
}
```



---

archive/issue_comments_035148.json:
```json
{
    "body": "Replying to [@jhpalmieri](https://github.com/sagemath/sage/issues/5588#comment:0):\n> 1. It says that the Sage style for typesetting the standard rings (ZZ, CC, etc.) is to use mathbf, not mathbb.\n\nI wonder if that was a conscious decision or whether it is just as it is (I personally prefer mathbb over mathbf).",
    "created_at": "2009-03-23T10:02:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5588#issuecomment-35148",
    "user": "https://github.com/malb"
}
```

Replying to [@jhpalmieri](https://github.com/sagemath/sage/issues/5588#comment:0):
> 1. It says that the Sage style for typesetting the standard rings (ZZ, CC, etc.) is to use mathbf, not mathbb.

I wonder if that was a conscious decision or whether it is just as it is (I personally prefer mathbb over mathbf).



---

archive/issue_comments_035149.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI happen to prefer mathbf, but I was basing this on the latex methods for ZZ, RR, etc., and also on the contents of the file SAGE_ROOT/sage/doc/common/macros.tex, which (in the old version of the documentation) was available for use in the docs.  The consistency among ZZ, RR, etc., led me to believe that it was a conscious decision, but I don't know this for a fact.  Of course, both mathbb and mathbf are used in the code many times...\n\nLet's see what people on sage-devel think.",
    "created_at": "2009-03-24T16:50:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5588#issuecomment-35149",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">comment:2</div>

I happen to prefer mathbf, but I was basing this on the latex methods for ZZ, RR, etc., and also on the contents of the file SAGE_ROOT/sage/doc/common/macros.tex, which (in the old version of the documentation) was available for use in the docs.  The consistency among ZZ, RR, etc., led me to believe that it was a conscious decision, but I don't know this for a fact.  Of course, both mathbb and mathbf are used in the code many times...

Let's see what people on sage-devel think.



---

archive/issue_events_065661.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-03-25T23:36:34Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "title_is": "[not ready for review] developer's guide: more information about docstrings",
    "title_was": "developer's guide: more information about docstrings",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65661"
}
```



---

archive/issue_comments_035150.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLet's delay this one for a while.  If #5610 gets in, then I'll need to revise the stuff about mathbf anyway...",
    "created_at": "2009-03-25T23:36:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5588#issuecomment-35150",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">comment:3</div>

Let's delay this one for a while.  If #5610 gets in, then I'll need to revise the stuff about mathbf anyway...



---

archive/issue_comments_035151.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[developer.patch.gz](https://github.com/sagemath/sage/files/ticket5588/developer.patch.gz)**\n\nNow that #5610 is in (as of 3.4.2.alpha0), here's a version of the patch which is ready for review.",
    "created_at": "2009-04-24T17:11:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5588#issuecomment-35151",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[developer.patch.gz](https://github.com/sagemath/sage/files/ticket5588/developer.patch.gz)**

Now that #5610 is in (as of 3.4.2.alpha0), here's a version of the patch which is ready for review.



---

archive/issue_events_065662.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-04-24T17:11:40Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "title_is": "developer's guide: more information about docstrings",
    "title_was": "[not ready for review] developer's guide: more information about docstrings",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65662"
}
```



---

archive/issue_events_065663.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-04-24T17:11:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65663"
}
```



---

archive/issue_comments_035152.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThe patch applies fine to 3.4.2.alpha0 and the docs build and read well.",
    "created_at": "2009-04-29T11:57:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5588#issuecomment-35152",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:5" align="right">comment:5</div>

The patch applies fine to 3.4.2.alpha0 and the docs build and read well.



---

archive/issue_events_065664.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-04-29T11:57:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65664"
}
```



---

archive/issue_events_065665.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-04-29T11:57:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65665"
}
```



---

archive/issue_comments_035153.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.4.2.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-30T00:59:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5588#issuecomment-35153",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.4.2.rc0.

Cheers,

Michael



---

archive/issue_events_065666.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-30T00:59:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65666"
}
```



---

archive/issue_events_065667.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-30T00:59:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65667"
}
```



---

archive/issue_events_065668.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-30T00:59:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65668"
}
```



---

archive/issue_events_065669.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-30T00:59:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5588",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5588#event-65669"
}
```
