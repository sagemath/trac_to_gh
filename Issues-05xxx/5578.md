# Issue 5578: [with patch, with positive  review] elliptic curve local data cache ignores proof flag

archive/issues_005578.json:
```json
{
    "body": "The fix is trivial.\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5578\n\n",
    "closed_at": "2009-03-26T00:36:49Z",
    "created_at": "2009-03-21T11:29:24Z",
    "labels": [
        "component: number theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.1",
    "title": "[with patch, with positive  review] elliptic curve local data cache ignores proof flag",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5578",
    "user": "https://github.com/robertwb"
}
```
The fix is trivial.

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5578





---

archive/issue_comments_046500.json:
```json
{
    "body": "Attachment [5578-local-data.patch](tarball://root/attachments/some-uuid/ticket5578/5578-local-data.patch) by @robertwb created at 2009-03-21 11:30:04",
    "created_at": "2009-03-21T11:30:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5578",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5578#issuecomment-46500",
    "user": "https://github.com/robertwb"
}
```

Attachment [5578-local-data.patch](tarball://root/attachments/some-uuid/ticket5578/5578-local-data.patch) by @robertwb created at 2009-03-21 11:30:04



---

archive/issue_events_022146.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5578",
    "rename": {
        "from": "[with patch, needs review] elliptic curve local data cache ignores proof flag",
        "to": "[with patch, with positive  review] elliptic curve local data cache ignores proof flag"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5578#event-22146"
}
```



---

archive/issue_comments_046501.json:
```json
{
    "body": "<a id='comment:1'></a>\nPositive review:  applies ok to 3.4 and all tests in elliptic_curves pass.\n\nApologies (for being responsible for the bug)!",
    "created_at": "2009-03-21T19:23:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5578",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5578#issuecomment-46501",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:1'></a>
Positive review:  applies ok to 3.4 and all tests in elliptic_curves pass.

Apologies (for being responsible for the bug)!



---

archive/issue_comments_046502.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.4.1.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-26T00:36:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5578",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5578#issuecomment-46502",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.4.1.alpha0.

Cheers,

Michael



---

archive/issue_comments_046503.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-03-26T00:36:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5578",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5578#issuecomment-46503",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_022147.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-03-26T00:36:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5578",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5578#event-22147"
}
```



---

archive/issue_events_022148.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-03-26T00:36:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5578",
    "milestone": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5578#event-22148"
}
```
