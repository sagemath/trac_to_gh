# Issue 5118: [with patch, positive review] Improve elliptic curve printing

archive/issues_005118.json:
```json
{
    "body": "Before:\n\n```\nElliptic Curve defined by y^2  = x^3 + x +1 ...\n```\n\nAfter:\n\n```\nElliptic Curve defined by y^2 = x^3 + x + 1 ...\n```\n\nAssignee: @williamstein\n\nCC:  @JohnCremona\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5118\n\n",
    "closed_at": "2009-02-02T05:53:43Z",
    "created_at": "2009-01-28T19:05:22Z",
    "labels": [
        "component: number theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch, positive review] Improve elliptic curve printing",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5118",
    "user": "https://github.com/rlmill"
}
```
Before:

```
Elliptic Curve defined by y^2  = x^3 + x +1 ...
```

After:

```
Elliptic Curve defined by y^2 = x^3 + x + 1 ...
```

Assignee: @williamstein

CC:  @JohnCremona

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5118





---

archive/issue_events_019918.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5118",
    "rename": {
        "from": "[with patch, needs review] Improve elliptic curve printing",
        "to": "[with patch, positive review] Improve elliptic curve printing"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5118#event-19918"
}
```



---

archive/issue_comments_041539.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac-5118.patch](tarball://root/attachments/some-uuid/ticket5118/trac-5118.patch) by mabshoff created at 2009-02-02 05:52:23\n\nPasses doctests and makes the printing consistent regarding spaces. Positive review. \n\nI am CCing John since he might be affected by this patch.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-02T05:52:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5118#issuecomment-41539",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Attachment [trac-5118.patch](tarball://root/attachments/some-uuid/ticket5118/trac-5118.patch) by mabshoff created at 2009-02-02 05:52:23

Passes doctests and makes the printing consistent regarding spaces. Positive review. 

I am CCing John since he might be affected by this patch.

Cheers,

Michael



---

archive/issue_comments_041540.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-02-02T05:53:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5118#issuecomment-41540",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_041541.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.3.alpha4.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-02T05:53:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5118#issuecomment-41541",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.3.alpha4.

Cheers,

Michael



---

archive/issue_events_019919.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-02T05:53:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5118",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5118#event-19919"
}
```
