# Issue 5071: [with patch, positive review] unit of least precision for RR and RDF

archive/issues_005071.json:
```json
{
    "body": "I wrote this while attacking #4746. Didn't turn out to be relevant to that, but it's useful to have on its own anyways. \n\nAssignee: somebody\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5071\n\n",
    "closed_at": "2009-02-07T01:38:34Z",
    "created_at": "2009-01-23T12:23:50Z",
    "labels": [
        "component: basic arithmetic",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch, positive review] unit of least precision for RR and RDF",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5071",
    "user": "https://github.com/robertwb"
}
```
I wrote this while attacking #4746. Didn't turn out to be relevant to that, but it's useful to have on its own anyways. 

Assignee: somebody

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5071





---

archive/issue_comments_041972.json:
```json
{
    "body": "<a id='comment:1'></a>Looks good.  The only change I would suggest is that the docstring should include the behavior on inf and NaN.",
    "created_at": "2009-01-24T09:49:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5071#issuecomment-41972",
    "user": "https://github.com/roed314"
}
```

<a id='comment:1'></a>Looks good.  The only change I would suggest is that the docstring should include the behavior on inf and NaN.



---

archive/issue_comments_041973.json:
```json
{
    "body": "Attachment [5071-ulp.patch](tarball://root/attachments/some-uuid/ticket5071/5071-ulp.patch) by @robertwb created at 2009-01-28 00:06:46",
    "created_at": "2009-01-28T00:06:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5071#issuecomment-41973",
    "user": "https://github.com/robertwb"
}
```

Attachment [5071-ulp.patch](tarball://root/attachments/some-uuid/ticket5071/5071-ulp.patch) by @robertwb created at 2009-01-28 00:06:46



---

archive/issue_comments_041974.json:
```json
{
    "body": "<a id='comment:2'></a>OK, doctests updated.",
    "created_at": "2009-01-28T00:07:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5071#issuecomment-41974",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:2'></a>OK, doctests updated.



---

archive/issue_comments_041975.json:
```json
{
    "body": "<a id='comment:3'></a>Code looks good, doctests pass.\n\nExcellent idea!\n\nPositive review.",
    "created_at": "2009-02-05T06:55:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5071#issuecomment-41975",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:3'></a>Code looks good, doctests pass.

Excellent idea!

Positive review.



---

archive/issue_events_011696.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-06T23:02:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5071#event-11696"
}
```



---

archive/issue_events_011697.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-07T01:38:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5071#event-11697"
}
```



---

archive/issue_comments_041976.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-02-07T01:38:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5071#issuecomment-41976",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_011698.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-07T01:38:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5071#event-11698"
}
```



---

archive/issue_events_011699.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-07T01:38:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5071#event-11699"
}
```



---

archive/issue_comments_041977.json:
```json
{
    "body": "<a id='comment:5'></a>Merged in Sage 3.3.alpha6.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-07T01:38:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5071#issuecomment-41977",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>Merged in Sage 3.3.alpha6.

Cheers,

Michael
