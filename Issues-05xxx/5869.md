# Issue 5869: [with patch, positive review] Fix libgpg-error shared library name on FreeBSD

archive/issues_005869.json:
```json
{
    "body": "Ensure that a symlink is created from libgpg-error.so to the actual .so name on FreeBSD.  This fixes the gnutls build on FreeBSD.\n\n\nAssignee: @mwhansen\n\nResolution: fixed\n\nAuthor: Peter Jeremy\n\nReviewer: Mike Hansen\n\nMerged: sage-4.1.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/5869\n\n",
    "closed_at": "2009-07-02T22:37:59Z",
    "created_at": "2009-04-23T07:04:38Z",
    "labels": [
        "component: porting: bsd",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1",
    "title": "[with patch, positive review] Fix libgpg-error shared library name on FreeBSD",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5869",
    "user": "https://github.com/peterjeremy"
}
```
Ensure that a symlink is created from libgpg-error.so to the actual .so name on FreeBSD.  This fixes the gnutls build on FreeBSD.


Assignee: @mwhansen

Resolution: fixed

Author: Peter Jeremy

Reviewer: Mike Hansen

Merged: sage-4.1.rc0

Issue created by migration from https://trac.sagemath.org/ticket/5869





---

archive/issue_comments_050940.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [libgpg_error-1.6.p0.patch](tarball://root/attachments/some-uuid/ticket5869/libgpg_error-1.6.p0.patch) by mabshoff created at 2009-04-23 07:31:38\n\nI will work on integrating this tomorrow.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-23T07:31:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50940",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>Attachment [libgpg_error-1.6.p0.patch](tarball://root/attachments/some-uuid/ticket5869/libgpg_error-1.6.p0.patch) by mabshoff created at 2009-04-23 07:31:38

I will work on integrating this tomorrow.

Cheers,

Michael



---

archive/issue_events_013800.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-23T08:09:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "milestone": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5869#event-13800"
}
```



---

archive/issue_comments_050941.json:
```json
{
    "body": "Changing assignee from tbd to mabshoff.",
    "created_at": "2009-04-23T08:51:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50941",
    "user": "https://github.com/peterjeremy"
}
```

Changing assignee from tbd to mabshoff.



---

archive/issue_comments_050942.json:
```json
{
    "body": "Changing component from algebra to freebsd.",
    "created_at": "2009-04-23T08:51:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50942",
    "user": "https://github.com/peterjeremy"
}
```

Changing component from algebra to freebsd.



---

archive/issue_comments_050943.json:
```json
{
    "body": "Changing assignee from mabshoff to @mwhansen.",
    "created_at": "2009-06-20T07:02:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50943",
    "user": "https://github.com/mwhansen"
}
```

Changing assignee from mabshoff to @mwhansen.



---

archive/issue_comments_050944.json:
```json
{
    "body": "Changing author from \"\" to \"Peter Jeremy\"",
    "created_at": "2009-06-20T07:02:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50944",
    "user": "https://github.com/mwhansen"
}
```

Changing author from "" to "Peter Jeremy"



---

archive/issue_comments_050945.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2009-06-20T07:02:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50945",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_comments_050946.json:
```json
{
    "body": "<a id='comment:4'></a>Looks good to me.\n\nThe spkg with this patch incorporated can be found at http://sage.math.washington.edu/home/mhansen/libgpg_error-1.6.p1.spkg .",
    "created_at": "2009-06-20T07:02:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50946",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:4'></a>Looks good to me.

The spkg with this patch incorporated can be found at http://sage.math.washington.edu/home/mhansen/libgpg_error-1.6.p1.spkg .



---

archive/issue_comments_050947.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Mike Hansen\"",
    "created_at": "2009-06-20T07:02:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50947",
    "user": "https://github.com/mwhansen"
}
```

Changing reviewer from "" to "Mike Hansen"



---

archive/issue_comments_050948.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-07-02T22:37:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50948",
    "user": "https://github.com/rlmill"
}
```

Resolution: fixed



---

archive/issue_comments_050949.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.1.rc0\"",
    "created_at": "2009-07-02T22:37:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5869#issuecomment-50949",
    "user": "https://github.com/rlmill"
}
```

Changing merged from "" to "sage-4.1.rc0"



---

archive/issue_events_013801.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-02T22:37:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5869",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5869#event-13801"
}
```
