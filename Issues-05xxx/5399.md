# Issue 5399: FinanceDate class

archive/issues_005399.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-psinis"
    ],
    "body": "FinanceDate class extends datetime.date in the Python Standard Lib. The goal is to introduce business-day calculations in Sage, and to give the user an easy API for coding futures & options expiries, generating swap & bond payment dates, and calculating holidays in all the major markets.\n\nTo be attached soon:\nFinanceDate.py\neaster.py\nrelativedelta.py\n\nKeywords: **finance, date**\n\nReviewer: **Jeroen Demeyer**\n\nComponent: **finance**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5399_\n\n",
    "closed_at": "2016-06-12T12:02:30Z",
    "created_at": "2009-02-28T03:34:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20finance",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "FinanceDate class",
    "type": "issue",
    "updated_at": "2016-06-12T12:02:30Z",
    "url": "https://github.com/sagemath/sage/issues/5399",
    "user": "https://github.com/sagetrac-psinis"
}
```
FinanceDate class extends datetime.date in the Python Standard Lib. The goal is to introduce business-day calculations in Sage, and to give the user an easy API for coding futures & options expiries, generating swap & bond payment dates, and calculating holidays in all the major markets.

To be attached soon:
FinanceDate.py
easter.py
relativedelta.py

Keywords: **finance, date**

Reviewer: **Jeroen Demeyer**

Component: **finance**

_Issue created by migration from https://trac.sagemath.org/ticket/5399_





---

archive/issue_events_063496.json:
```json
{
    "actor": "https://github.com/sagetrac-psinis",
    "created_at": "2009-02-28T03:34:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20finance",
    "label_color": "0000ff",
    "label_name": "c: finance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63496"
}
```



---

archive/issue_events_063497.json:
```json
{
    "actor": "https://github.com/sagetrac-psinis",
    "created_at": "2009-02-28T03:34:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63497"
}
```



---

archive/issue_events_063498.json:
```json
{
    "actor": "https://github.com/sagetrac-psinis",
    "created_at": "2009-02-28T03:34:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63498"
}
```



---

archive/issue_events_063499.json:
```json
{
    "actor": "https://github.com/sagetrac-psinis",
    "created_at": "2009-02-28T03:34:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "subject": "https://github.com/sagetrac-psinis",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63499"
}
```



---

archive/issue_events_063500.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-26T09:52:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63500"
}
```



---

archive/issue_comments_033425.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2016-04-26T09:52:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5399#issuecomment-33425",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_033426.json:
```json
{
    "body": "Replying to [ticket:5399 psinis]:\n> To be attached soon:\n> FinanceDate.py\n> easter.py\n> relativedelta.py\n\nI think it can be assumed that \"soon\" will never happen, given that 7 years have passed.",
    "created_at": "2016-04-26T09:52:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5399#issuecomment-33426",
    "user": "https://github.com/jdemeyer"
}
```

Replying to [ticket:5399 psinis]:
> To be attached soon:
> FinanceDate.py
> easter.py
> relativedelta.py

I think it can be assumed that "soon" will never happen, given that 7 years have passed.



---

archive/issue_events_063501.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-26T09:52:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63501"
}
```



---

archive/issue_events_063502.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-26T09:52:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63502"
}
```



---

archive/issue_events_063503.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-26T09:52:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63503"
}
```



---

archive/issue_events_063504.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-26T09:52:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63504"
}
```



---

archive/issue_events_063505.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-26T09:52:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63505"
}
```



---

archive/issue_events_063506.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63506"
}
```



---

archive/issue_events_063507.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5399",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5399#event-63507"
}
```
