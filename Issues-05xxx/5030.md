# Issue 5030: [with patch; positive review] get matrix/constructor.py up to 100% coverage

archive/issues_005030.json:
```json
{
    "body": "\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5030\n\n",
    "closed_at": "2009-01-23T02:54:51Z",
    "created_at": "2009-01-20T04:39:44Z",
    "labels": [
        "component: linear algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch; positive review] get matrix/constructor.py up to 100% coverage",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5030",
    "user": "https://github.com/williamstein"
}
```


Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5030





---

archive/issue_comments_040661.json:
```json
{
    "body": "this is against sage-3.3.alpha0",
    "created_at": "2009-01-20T04:41:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5030#issuecomment-40661",
    "user": "https://github.com/williamstein"
}
```

this is against sage-3.3.alpha0



---

archive/issue_comments_040662.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_5030.patch](tarball://root/attachments/some-uuid/ticket5030/trac_5030.patch) by @mwhansen created at 2009-01-20 21:36:45\n\nLooks good to me.",
    "created_at": "2009-01-20T21:36:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5030#issuecomment-40662",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [trac_5030.patch](tarball://root/attachments/some-uuid/ticket5030/trac_5030.patch) by @mwhansen created at 2009-01-20 21:36:45

Looks good to me.



---

archive/issue_events_019481.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5030",
    "rename": {
        "from": "[with patch; needs review] get matrix/constructor.py up to 100% coverage",
        "to": "[with patch; positive review] get matrix/constructor.py up to 100% coverage"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5030#event-19481"
}
```



---

archive/issue_comments_040663.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.3.alpha1",
    "created_at": "2009-01-23T02:54:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5030#issuecomment-40663",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.3.alpha1



---

archive/issue_comments_040664.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-01-23T02:54:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5030",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5030#issuecomment-40664",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_019482.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-23T02:54:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5030",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5030#event-19482"
}
```
