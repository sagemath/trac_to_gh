# Issue 5212: bug with numbers in names in sage.structure.parent_gens.normalize_names

archive/issues_005212.json:
```json
{
    "assignees": [],
    "body": "Try `PolynomialRing(QQ, 2, 'alpha0')` for hilarity.\n\nKeywords: **numbers in names normalize_names**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5212_\n\n",
    "closed_at": "2009-02-11T06:05:03Z",
    "created_at": "2009-02-09T04:20:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bug with numbers in names in sage.structure.parent_gens.normalize_names",
    "type": "issue",
    "updated_at": "2009-02-11T06:05:03Z",
    "url": "https://github.com/sagemath/sage/issues/5212",
    "user": "https://github.com/ncalexan"
}
```
Try `PolynomialRing(QQ, 2, 'alpha0')` for hilarity.

Keywords: **numbers in names normalize_names**

_Issue created by migration from https://trac.sagemath.org/ticket/5212_





---

archive/issue_events_054579.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-02-09T04:20:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-54579"
}
```



---

archive/issue_events_054580.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-02-09T04:20:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-54580"
}
```



---

archive/issue_events_054581.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-02-09T04:20:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-54581"
}
```



---

archive/issue_events_054582.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-02-09T04:20:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-54582"
}
```



---

archive/issue_events_054583.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-02-09T04:20:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-54583"
}
```



---

archive/issue_comments_033153.json:
```json
{
    "body": "Attachment: **[trac_5212-normalize-names.patch.gz](https://github.com/sagemath/sage/files/ticket5212/trac_5212-normalize-names.patch.gz)**",
    "created_at": "2009-02-09T04:23:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5212#issuecomment-33153",
    "user": "https://github.com/ncalexan"
}
```

Attachment: **[trac_5212-normalize-names.patch.gz](https://github.com/sagemath/sage/files/ticket5212/trac_5212-normalize-names.patch.gz)**



---

archive/issue_comments_033154.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nWith this patch applied to my current 3.3.rc0 merge tree all doctests pass.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-09T12:26:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5212#issuecomment-33154",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">Comment 1</div>

With this patch applied to my current 3.3.rc0 merge tree all doctests pass.

Cheers,

Michael



---

archive/issue_events_054584.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2009-02-11T05:36:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-54584"
}
```



---

archive/issue_comments_033155.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMerged in Sage 3.3.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-11T06:05:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5212#issuecomment-33155",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">Comment 3</div>

Merged in Sage 3.3.rc0.

Cheers,

Michael



---

archive/issue_events_054585.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-11T06:05:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-54585"
}
```



---

archive/issue_events_054586.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-11T06:05:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-54586"
}
```
