# Issue 5860: [with patch; with positive review] delete sage/catalogue

archive/issues_005860.json:
```json
{
    "body": "This turned out to be a bad idea. \n\nAssignee: cwitty\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5860\n\n",
    "closed_at": "2009-04-30T05:03:33Z",
    "created_at": "2009-04-22T23:34:15Z",
    "labels": [
        "component: misc"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.2",
    "title": "[with patch; with positive review] delete sage/catalogue",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5860",
    "user": "https://github.com/williamstein"
}
```
This turned out to be a bad idea. 

Assignee: cwitty

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5860





---

archive/issue_events_023429.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5860",
    "rename": {
        "from": "delete sage/catalogue",
        "to": "[with patch; needs review] delete sage/catalogue"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5860#event-23429"
}
```



---

archive/issue_comments_049754.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_5860.patch](tarball://root/attachments/some-uuid/ticket5860/trac_5860.patch) by @williamstein created at 2009-04-23 05:09:38",
    "created_at": "2009-04-23T05:09:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5860#issuecomment-49754",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [trac_5860.patch](tarball://root/attachments/some-uuid/ticket5860/trac_5860.patch) by @williamstein created at 2009-04-23 05:09:38



---

archive/issue_comments_049755.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac_5860-part2.patch](tarball://root/attachments/some-uuid/ticket5860/trac_5860-part2.patch) by dmharvey created at 2009-04-25 17:42:31\n\nI'm getting intermittent doctest failures for devel/sage/sage/server/misc.py for 3.4.1 on sage.math. But it seems to happen without this patch too. Hmmm I can't see how it could be related.\n\nThumbs up.",
    "created_at": "2009-04-25T17:42:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5860#issuecomment-49755",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```

<a id='comment:2'></a>
Attachment [trac_5860-part2.patch](tarball://root/attachments/some-uuid/ticket5860/trac_5860-part2.patch) by dmharvey created at 2009-04-25 17:42:31

I'm getting intermittent doctest failures for devel/sage/sage/server/misc.py for 3.4.1 on sage.math. But it seems to happen without this patch too. Hmmm I can't see how it could be related.

Thumbs up.



---

archive/issue_events_023430.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5860",
    "rename": {
        "from": "[with patch; needs review] delete sage/catalogue",
        "to": "[with patch; with positive review] delete sage/catalogue"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5860#event-23430"
}
```



---

archive/issue_comments_049756.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-04-30T05:03:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5860#issuecomment-49756",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_049757.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.4.2.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-30T05:03:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5860#issuecomment-49757",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.4.2.rc0.

Cheers,

Michael



---

archive/issue_events_023431.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-30T05:03:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5860",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5860#event-23431"
}
```
