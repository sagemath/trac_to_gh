...
```json
{
    "body": "<a id='comment:1'></a>Attachment [sage-5482.patch](tarball://root/attachments/some-uuid/ticket5482/sage-5482.patch) by justin created at 2009-03-11 07:42:22\n\nThe fix is to require the generator name at creation time, not when the ring is used.",
...
<a id='comment:1'></a>Attachment [sage-5482.patch](tarball://root/attachments/some-uuid/ticket5482/sage-5482.patch) by justin created at 2009-03-11 07:42:22
...
```json
{
    "body": "<a id='comment:2'></a>Why do you change the parameter name from \"names\" to \"name\"?  Is this function only used for univariate polynomial rings?  If so, fine.",
...
<a id='comment:2'></a>Why do you change the parameter name from "names" to "name"?  Is this function only used for univariate polynomial rings?  If so, fine.
...
```json
{
    "body": "<a id='comment:7'></a>I attached a new patch that assigns names automatically if they are not specified by the user, e.g. a quotient of `R.<x>` will have variable name `xbar`.  This is standard behaviour in other places in Sage.\n\nApply trac_5482.patch only.",
...
<a id='comment:7'></a>I attached a new patch that assigns names automatically if they are not specified by the user, e.g. a quotient of `R.<x>` will have variable name `xbar`.  This is standard behaviour in other places in Sage.
