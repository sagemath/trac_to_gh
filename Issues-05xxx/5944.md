# Issue 5944: fix a nasty pynac bug in exp

archive/issues_005944.json:
```json
{
    "body": "\n```\nsage: t = var('t', ns=1)\nsage: a = (-2*t).exp(); a\ne^(-2*t)\nsage: b = (-t).exp(); b\ne^(-t)\nsage: a - b\n0\nsage: a\ne^(-t)\nsage: b\ne^(-t)\n```\n\n**Assignee:** @burcin\n\n**CC:**  @mwhansen\n\nIssue created by migration from https://trac.sagemath.org/ticket/5944\n\n",
    "closed_at": "2009-05-20T23:47:28Z",
    "created_at": "2009-04-29T22:59:30Z",
    "labels": [
        "component: calculus",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0",
    "title": "fix a nasty pynac bug in exp",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5944",
    "user": "https://github.com/williamstein"
}
```

```
sage: t = var('t', ns=1)
sage: a = (-2*t).exp(); a
e^(-2*t)
sage: b = (-t).exp(); b
e^(-t)
sage: a - b
0
sage: a
e^(-t)
sage: b
e^(-t)
```

**Assignee:** @burcin

**CC:**  @mwhansen

Issue created by migration from https://trac.sagemath.org/ticket/5944





---

archive/issue_comments_039805.json:
```json
{
    "body": "add doctests for the fix",
    "created_at": "2009-05-05T23:20:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5944#issuecomment-39805",
    "user": "https://github.com/burcin"
}
```

add doctests for the fix



---

archive/attachments_006881.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5944-doctest.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket5944/trac_5944-doctest.patch",
    "created_at": "2009-05-05T23:22:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/burcin"
}
```



---

archive/issue_comments_039806.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe package for pynac 0.1.6 at #5777 has a fix for this. Attached patch adds doctests.",
    "created_at": "2009-05-05T23:22:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5944#issuecomment-39806",
    "user": "https://github.com/burcin"
}
```

<a id='comment:1'></a>
The package for pynac 0.1.6 at #5777 has a fix for this. Attached patch adds doctests.



---

archive/issue_events_038206.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-05-05T23:22:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5944#event-38206"
}
```



---

archive/issue_events_038207.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-05-05T23:22:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5944#event-38207"
}
```



---

archive/issue_comments_039807.json:
```json
{
    "body": "<a id='comment:2'></a>\nPositive review due to symbolics getting a positive review.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-20T23:42:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5944#issuecomment-39807",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Positive review due to symbolics getting a positive review.

Cheers,

Michael



---

archive/issue_events_038208.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-20T23:42:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5944#event-38208"
}
```



---

archive/issue_events_038209.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-20T23:47:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5944#event-38209"
}
```



---

archive/issue_events_038210.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-20T23:47:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5944#event-38210"
}
```



---

archive/issue_comments_039808.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 4.0.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-20T23:47:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5944#issuecomment-39808",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 4.0.rc0.

Cheers,

Michael



---

archive/issue_events_038211.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-20T23:47:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "milestone": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5944#event-38211"
}
```



---

archive/issue_events_038212.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-20T23:47:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5944",
    "milestone": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5944#event-38212"
}
```
