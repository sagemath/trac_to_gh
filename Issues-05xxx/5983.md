# Issue 5983: cmp related doctest failure in sage/schemes/jacobians/abstract_jacobian.py

archive/issues_005983.json:
```json
{
    "body": "This says it all and fails on occasion due to memory layout, etc. Writing a doctest this way is **wrong**:\n\n```\nsage -t -long \"devel/sage/sage/schemes/jacobians/abstract_jacobian.py\"\n**********************************************************************\nFile \"/home/mabshoff/build-3.4.2/sage-3.4.2-eno-gcc-4.3.3/devel/sage/sage/schemes/jacobians/abstract_jacobian.py\", line 118:\n    sage: J1 < P2\nExpected:\n    False\nGot:\n    True\n**********************************************************************\nFile \"/home/mabshoff/build-3.4.2/sage-3.4.2-eno-gcc-4.3.3/devel/sage/sage/schemes/jacobians/abstract_jacobian.py\", line 120:\n    sage: J1 > P2\nExpected:\n    True\nGot:\n    False\n**********************************************************************\n```\nI am CCing Alex since I believe he wrote this doctest :)\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\n**CC:**  @aghitza\n\nIssue created by migration from https://trac.sagemath.org/ticket/5983\n\n",
    "closed_at": "2009-05-05T04:21:02Z",
    "created_at": "2009-05-05T03:39:23Z",
    "labels": [
        "component: doctest coverage",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.2",
    "title": "cmp related doctest failure in sage/schemes/jacobians/abstract_jacobian.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5983",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
This says it all and fails on occasion due to memory layout, etc. Writing a doctest this way is **wrong**:

```
sage -t -long "devel/sage/sage/schemes/jacobians/abstract_jacobian.py"
**********************************************************************
File "/home/mabshoff/build-3.4.2/sage-3.4.2-eno-gcc-4.3.3/devel/sage/sage/schemes/jacobians/abstract_jacobian.py", line 118:
    sage: J1 < P2
Expected:
    False
Got:
    True
**********************************************************************
File "/home/mabshoff/build-3.4.2/sage-3.4.2-eno-gcc-4.3.3/devel/sage/sage/schemes/jacobians/abstract_jacobian.py", line 120:
    sage: J1 > P2
Expected:
    True
Got:
    False
**********************************************************************
```
I am CCing Alex since I believe he wrote this doctest :)

Cheers,

Michael

**Assignee:** mabshoff

**CC:**  @aghitza

Issue created by migration from https://trac.sagemath.org/ticket/5983





---

archive/issue_events_038617.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-05T03:39:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "milestone": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5983#event-38617"
}
```



---

archive/issue_events_038618.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-05T03:39:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "milestone": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5983#event-38618"
}
```



---

archive/issue_events_038619.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-05T03:41:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5983#event-38619"
}
```



---

archive/issue_events_038620.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-05T03:59:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5983#event-38620"
}
```



---

archive/issue_events_038621.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-05T03:59:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5983#event-38621"
}
```



---

archive/attachments_006964.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5983.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket5983/trac_5983.patch",
    "created_at": "2009-05-05T03:59:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_040339.json:
```json
{
    "body": "<a id='comment:3'></a>\n",
    "created_at": "2009-05-05T03:59:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5983#issuecomment-40339",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>




---

archive/issue_events_038622.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-05T04:21:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5983#event-38622"
}
```



---

archive/issue_events_038623.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-05T04:21:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5983#event-38623"
}
```



---

archive/issue_comments_040340.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.4.2.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-05T04:21:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5983#issuecomment-40340",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.4.2.

Cheers,

Michael



---

archive/issue_comments_040341.json:
```json
{
    "body": "Replying to [ticket:5983 mabshoff]:\n> I am CCing Alex since I believe he wrote this doctest :)\n\n\nYes, sorry about that.  It won't happen again (the bad cmp test, not writing doctest in general :)",
    "created_at": "2009-05-05T06:30:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5983#issuecomment-40341",
    "user": "https://github.com/aghitza"
}
```

Replying to [ticket:5983 mabshoff]:
> I am CCing Alex since I believe he wrote this doctest :)


Yes, sorry about that.  It won't happen again (the bad cmp test, not writing doctest in general :)



---

archive/issue_comments_040342.json:
```json
{
    "body": "<a id='comment:6'></a>\nReplying to [AlexGhitza](#comment%3A5):\n\n> Yes, sorry about that.  It won't happen again (the bad cmp test, not writing doctest in general :)\n\n\nHehe, I didn't catch this issue while running doctests dozens if not hundred of times on sage.math and many other systems, but it just triggered once on a SkyNet box. I also found an analog problem in other places, so you aren't the only one.\n\nKeep the doctests coming ;)\n\nCheers,\n\nMichael",
    "created_at": "2009-05-05T06:33:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5983#issuecomment-40342",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
Replying to [AlexGhitza](#comment%3A5):

> Yes, sorry about that.  It won't happen again (the bad cmp test, not writing doctest in general :)


Hehe, I didn't catch this issue while running doctests dozens if not hundred of times on sage.math and many other systems, but it just triggered once on a SkyNet box. I also found an analog problem in other places, so you aren't the only one.

Keep the doctests coming ;)

Cheers,

Michael
