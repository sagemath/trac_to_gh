# Issue 5760: [with patch, positive review] bring doctest coverage of plot3d/shapes.pyx to 100%

archive/issues_005760.json:
```json
{
    "body": "\n```\nplot/plot3d/shapes.pyx: 16% (5 of 31)\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5760\n\n",
    "closed_at": "2009-04-13T19:56:21Z",
    "created_at": "2009-04-11T19:04:00Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.1",
    "title": "[with patch, positive review] bring doctest coverage of plot3d/shapes.pyx to 100%",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5760",
    "user": "https://github.com/robertwb"
}
```

```
plot/plot3d/shapes.pyx: 16% (5 of 31)
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5760





---

archive/issue_comments_048276.json:
```json
{
    "body": "Attachment [5760-shapes-doctests.patch](tarball://root/attachments/some-uuid/ticket5760/5760-shapes-doctests.patch) by @robertwb created at 2009-04-11 23:56:14",
    "created_at": "2009-04-11T23:56:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5760",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5760#issuecomment-48276",
    "user": "https://github.com/robertwb"
}
```

Attachment [5760-shapes-doctests.patch](tarball://root/attachments/some-uuid/ticket5760/5760-shapes-doctests.patch) by @robertwb created at 2009-04-11 23:56:14



---

archive/issue_events_022895.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5760",
    "rename": {
        "from": "bring doctest coverage of plot3d/shapes.pyx to 100%",
        "to": "[with patch, needs review] bring doctest coverage of plot3d/shapes.pyx to 100%"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5760#event-22895"
}
```



---

archive/issue_comments_048277.json:
```json
{
    "body": "<a id='comment:2'></a>\nWow, not only does this greatly improve the coverage, it appears that several subtle bugs were fixed in 3d plotting along the way!\n\nThis example looks awesome, by the way.  It's a spotted icosahedron:\n\nshow(sphere(opacity=.8,size=1.31) + icosahedron(size=2,opacity=.5,color='red'), frame=False)",
    "created_at": "2009-04-13T14:30:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5760",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5760#issuecomment-48277",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
Wow, not only does this greatly improve the coverage, it appears that several subtle bugs were fixed in 3d plotting along the way!

This example looks awesome, by the way.  It's a spotted icosahedron:

show(sphere(opacity=.8,size=1.31) + icosahedron(size=2,opacity=.5,color='red'), frame=False)



---

archive/issue_events_022896.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5760",
    "rename": {
        "from": "[with patch, needs review] bring doctest coverage of plot3d/shapes.pyx to 100%",
        "to": "[with patch, positive review] bring doctest coverage of plot3d/shapes.pyx to 100%"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5760#event-22896"
}
```



---

archive/issue_comments_048278.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-04-13T19:56:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5760",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5760#issuecomment-48278",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_048279.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.4.1.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-13T19:56:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5760",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5760#issuecomment-48279",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.4.1.rc3.

Cheers,

Michael



---

archive/issue_events_022897.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-13T19:56:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5760",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5760#event-22897"
}
```
