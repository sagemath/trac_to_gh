# Issue 5863: remove some files from sage/algebras

archive/issues_005863.json:
```json
{
    "assignees": [
        "https://github.com/jhpalmieri"
    ],
    "body": "Coverage of some files in algebras:\n\n```\nalgebra_ideal.py: 0% (0 of 19)\nalgebra_ideal_element.py: 0% (0 of 1)\nalgebra_order.py: 0% (0 of 16)\nalgebra_order_element.py: 0% (0 of 13)\nalgebra_order_frac_ideal.py: 0% (0 of 17)\nalgebra_order_ideal.py: 0% (0 of 21)\nalgebra_order_ideal_element.py: 0% (0 of 1)\n```\nFurthermore, these don't seem to be used by any of the rest of the Sage code, so let's delete them.\n\nComponent: **algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5863_\n\n",
    "closed_at": "2009-04-23T07:04:47Z",
    "created_at": "2009-04-23T06:07:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "remove some files from sage/algebras",
    "type": "issue",
    "updated_at": "2009-04-23T07:04:47Z",
    "url": "https://github.com/sagemath/sage/issues/5863",
    "user": "https://github.com/jhpalmieri"
}
```
Coverage of some files in algebras:

```
algebra_ideal.py: 0% (0 of 19)
algebra_ideal_element.py: 0% (0 of 1)
algebra_order.py: 0% (0 of 16)
algebra_order_element.py: 0% (0 of 13)
algebra_order_frac_ideal.py: 0% (0 of 17)
algebra_order_ideal.py: 0% (0 of 21)
algebra_order_ideal_element.py: 0% (0 of 1)
```
Furthermore, these don't seem to be used by any of the rest of the Sage code, so let's delete them.

Component: **algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/5863_





---

archive/issue_events_070000.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-04-23T06:07:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70000"
}
```



---

archive/issue_events_070001.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-04-23T06:07:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70001"
}
```



---

archive/issue_events_070002.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-04-23T06:07:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70002"
}
```



---

archive/issue_events_070003.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-04-23T06:07:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70003"
}
```



---

archive/issue_events_070004.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-04-23T06:07:02Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "subject": "https://github.com/jhpalmieri",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70004"
}
```



---

archive/issue_events_070005.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-04-23T06:07:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70005"
}
```



---

archive/issue_comments_037778.json:
```json
{
    "body": "Attachment: **[nuke_algebras.patch.gz](https://github.com/sagemath/sage/files/ticket5863/nuke_algebras.patch.gz)**",
    "created_at": "2009-04-23T06:07:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5863#issuecomment-37778",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[nuke_algebras.patch.gz](https://github.com/sagemath/sage/files/ticket5863/nuke_algebras.patch.gz)**



---

archive/issue_events_070006.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-23T06:32:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70006"
}
```



---

archive/issue_comments_037779.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nIf anybody wants these to be in Sage, they should revert this patch, then get the coverage to 100% and get everything up to how things work in Sage now (e.g., coercion model, etc.).  Right now they are used nowhere and have no tests.",
    "created_at": "2009-04-23T06:32:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5863#issuecomment-37779",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

If anybody wants these to be in Sage, they should revert this patch, then get the coverage to 100% and get everything up to how things work in Sage now (e.g., coercion model, etc.).  Right now they are used nowhere and have no tests.



---

archive/issue_comments_037780.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.4.2.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-23T07:04:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5863#issuecomment-37780",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.4.2.alpha0.

Cheers,

Michael



---

archive/issue_events_070007.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-23T07:04:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70007"
}
```



---

archive/issue_events_070008.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-23T07:04:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70008"
}
```



---

archive/issue_events_070009.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-23T07:04:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70009"
}
```



---

archive/issue_events_070010.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-23T07:04:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5863",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5863#event-70010"
}
```
