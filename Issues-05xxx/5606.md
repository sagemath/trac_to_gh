...
```json
{
    "body": "<a id='comment:1'></a>It's still a bit slow, #5093 should fix this.",
...
<a id='comment:1'></a>It's still a bit slow, #5093 should fix this.
...
<a id='comment:3'></a>I added a doctest to get coverage up to 100%. 
...
```json
{
    "body": "<a id='comment:6'></a>The second and third hunk in 5606-complex-plot.patch are already in Sage via another patch, so I am attaching the version I merged (assuming doctests pass for me).\n\nCheers,\n\nMichael",
...
<a id='comment:6'></a>The second and third hunk in 5606-complex-plot.patch are already in Sage via another patch, so I am attaching the version I merged (assuming doctests pass for me).
...
<a id='comment:7'></a>Hmm, there are also the following two doctest failures:
...
```json
{
    "body": "<a id='comment:8'></a>I've updated the patch, should pass doctests now.",
...
<a id='comment:8'></a>I've updated the patch, should pass doctests now.
...
<a id='comment:10'></a>> I've updated the patch, should pass doctests now. 
...
```json
{
    "body": "<a id='comment:11'></a>Just FYI: As mentioned above two hunk that delete and import of \"sage.plot.all\" in log.py and mathml.py need to be deleted since they are already gone in 3.4.1.rc1. Then the patch applies and passes doctests for me.\n\nCheers,\n\nMichael",
...
<a id='comment:11'></a>Just FYI: As mentioned above two hunk that delete and import of "sage.plot.all" in log.py and mathml.py need to be deleted since they are already gone in 3.4.1.rc1. Then the patch applies and passes doctests for me.
...
```json
{
    "body": "<a id='comment:12'></a>Sorry, I will post a separate patch next time. Ironically, I've had people complain to me about that behavior too, but usually it's when the list of patches gets cumbersome.",
...
<a id='comment:12'></a>Sorry, I will post a separate patch next time. Ironically, I've had people complain to me about that behavior too, but usually it's when the list of patches gets cumbersome.
...
```json
{
    "body": "<a id='comment:13'></a>trac_5606-complex-plot.patch does apply to my merge tree and pass long doctests.\n\nCheers,\n\nMichael",
...
<a id='comment:13'></a>trac_5606-complex-plot.patch does apply to my merge tree and pass long doctests.
...
```json
{
    "body": "<a id='comment:15'></a>Merged rac_5606-complex-plot.patch in Sage 3.4.1.rc2.\n\nCheers,\n\nMichael",
...
<a id='comment:15'></a>Merged rac_5606-complex-plot.patch in Sage 3.4.1.rc2.
