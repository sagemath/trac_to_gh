# Issue 5233: improve timings for adjacency_matrix, weighted_adjacency_matrix, and kirchoff_matrix

archive/issues_005233.json:
```json
{
    "assignees": [],
    "body": "Before:\n\n```\nsage: %time m = graphs.GridGraph([50,50]).laplacian_matrix()\nCPU times: user 38.42 s, sys: 0.24 s, total: 38.66 s\nWall time: 39.02 s\n```\n\nAfter:\n\n```\nsage: %time m = graphs.GridGraph([50,50]).laplacian_matrix()\nCPU times: user 0.63 s, sys: 0.06 s, total: 0.69 s\nWall time: 0.89 s\n```\n\nAssignee: **@mwhansen**\n\nCC:  @rlmill\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5233_\n\n",
    "closed_at": "2009-02-11T06:53:28Z",
    "created_at": "2009-02-11T06:14:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve timings for adjacency_matrix, weighted_adjacency_matrix, and kirchoff_matrix",
    "type": "issue",
    "updated_at": "2009-02-11T06:53:28Z",
    "url": "https://github.com/sagemath/sage/issues/5233",
    "user": "https://github.com/mwhansen"
}
```
Before:

```
sage: %time m = graphs.GridGraph([50,50]).laplacian_matrix()
CPU times: user 38.42 s, sys: 0.24 s, total: 38.66 s
Wall time: 39.02 s
```

After:

```
sage: %time m = graphs.GridGraph([50,50]).laplacian_matrix()
CPU times: user 0.63 s, sys: 0.06 s, total: 0.69 s
Wall time: 0.89 s
```

Assignee: **@mwhansen**

CC:  @rlmill

_Issue created by migration from https://trac.sagemath.org/ticket/5233_





---

archive/issue_events_054850.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-11T06:14:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5233#event-54850"
}
```



---

archive/issue_events_054851.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-11T06:14:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5233#event-54851"
}
```



---

archive/issue_events_054852.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-11T06:14:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5233#event-54852"
}
```



---

archive/issue_events_054853.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-11T06:14:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5233#event-54853"
}
```



---

archive/issue_events_054854.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-11T06:14:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5233#event-54854"
}
```



---

archive/issue_comments_033295.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,15 @@\n+Before:\n \n+```\n+sage: %time m = graphs.GridGraph([50,50]).laplacian_matrix()\n+CPU times: user 38.42 s, sys: 0.24 s, total: 38.66 s\n+Wall time: 39.02 s\n+```\n+\n+After:\n+\n+```\n+sage: %time m = graphs.GridGraph([50,50]).laplacian_matrix()\n+CPU times: user 0.63 s, sys: 0.06 s, total: 0.69 s\n+Wall time: 0.89 s\n+```\n``````\n",
    "created_at": "2009-02-11T06:16:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5233#issuecomment-33295",
    "user": "https://github.com/mwhansen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,15 @@
+Before:
 
+```
+sage: %time m = graphs.GridGraph([50,50]).laplacian_matrix()
+CPU times: user 38.42 s, sys: 0.24 s, total: 38.66 s
+Wall time: 39.02 s
+```
+
+After:
+
+```
+sage: %time m = graphs.GridGraph([50,50]).laplacian_matrix()
+CPU times: user 0.63 s, sys: 0.06 s, total: 0.69 s
+Wall time: 0.89 s
+```
``````




---

archive/issue_comments_033296.json:
```json
{
    "body": "Changed assignee from **@rlmill** to **@mwhansen**",
    "created_at": "2009-02-11T06:16:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5233#issuecomment-33296",
    "user": "https://github.com/mwhansen"
}
```

Changed assignee from **@rlmill** to **@mwhansen**



---

archive/issue_comments_033297.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[trac_5233.patch.gz](https://github.com/sagemath/sage/files/ticket5233/trac_5233.patch.gz)**\n\nThe code looks reasonable, so positive review pending the doctests passing.",
    "created_at": "2009-02-11T06:46:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5233#issuecomment-33297",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[trac_5233.patch.gz](https://github.com/sagemath/sage/files/ticket5233/trac_5233.patch.gz)**

The code looks reasonable, so positive review pending the doctests passing.



---

archive/issue_comments_033298.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAll doctests pass with the rebased patch applied, so a positive review in Jason's name.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-11T06:53:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5233#issuecomment-33298",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
All doctests pass with the rebased patch applied, so a positive review in Jason's name.

Cheers,

Michael



---

archive/issue_events_054855.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-11T06:53:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5233#event-54855"
}
```



---

archive/issue_comments_033299.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMerged in Sage 3.3.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-11T06:53:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5233#issuecomment-33299",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:4'>Comment 4:</a>
Merged in Sage 3.3.rc0.

Cheers,

Michael



---

archive/issue_events_054856.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-11T06:53:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5233#event-54856"
}
```



---

archive/issue_events_054857.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-11T06:53:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5233",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5233#event-54857"
}
```
