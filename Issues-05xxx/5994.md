...
<a id='comment:22'></a>Replying to [SimonKing](#comment%3A15):
...
```json
{
    "body": "<a id='comment:23'></a>Gosh, is that still not fixed? Sorry that I spoiled it by asking questions in[comment:13](#comment%3A13) that nobody bothers to answer...",
...
<a id='comment:23'></a>Gosh, is that still not fixed? Sorry that I spoiled it by asking questions in[comment:13](#comment%3A13) that nobody bothers to answer...
...
<a id='comment:26'></a>Replying to [jmantysalo](#comment%3A25):
> Sounds good. Somehow the output of `pari.version()` seems best for me.
...
```json
{
    "body": "<a id='comment:34'></a>I am not so sure if it requires an uptream report. The error is about a missing file, and according to[comment:9](#comment%3A9), adding an empty `$SAGE_ROOT/local/share/singular/help.cnf` suppresses the error.",
...
<a id='comment:34'></a>I am not so sure if it requires an uptream report. The error is about a missing file, and according to[comment:9](#comment%3A9), adding an empty `$SAGE_ROOT/local/share/singular/help.cnf` suppresses the error.
...
<a id='comment:41'></a>Replying to [SimonKing](#comment%3A37):
> But for now, we need a work-around in Sage.
