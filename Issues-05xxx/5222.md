# Issue 5222: wrap FLINTs pseudo-division algorithm for univariate polynoials over ZZ

archive/issues_005222.json:
```json
{
    "assignees": [],
    "body": "Flint has a super-fast pseudo-division algorithm over ZZ, but Sage doesn't yet wrap it.  The point of this ticket is to wrap it.  \n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5222_\n\n",
    "closed_at": "2009-02-10T07:17:31Z",
    "created_at": "2009-02-09T19:05:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "wrap FLINTs pseudo-division algorithm for univariate polynoials over ZZ",
    "type": "issue",
    "updated_at": "2009-02-10T07:17:31Z",
    "url": "https://github.com/sagemath/sage/issues/5222",
    "user": "https://github.com/williamstein"
}
```
Flint has a super-fast pseudo-division algorithm over ZZ, but Sage doesn't yet wrap it.  The point of this ticket is to wrap it.  

Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/5222_





---

archive/issue_events_061044.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-09T19:05:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-61044"
}
```



---

archive/issue_events_061045.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-09T19:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-61045"
}
```



---

archive/issue_events_061046.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-09T19:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-61046"
}
```



---

archive/issue_events_061047.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-09T19:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-61047"
}
```



---

archive/issue_comments_032024.json:
```json
{
    "body": "Attachment: **[trac_5222.patch.gz](https://github.com/sagemath/sage/files/ticket5222/trac_5222.patch.gz)**",
    "created_at": "2009-02-09T20:15:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5222#issuecomment-32024",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac_5222.patch.gz](https://github.com/sagemath/sage/files/ticket5222/trac_5222.patch.gz)**



---

archive/issue_events_061048.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-09T20:29:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-61048"
}
```



---

archive/issue_events_061049.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-09T20:29:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-61049"
}
```



---

archive/issue_events_061050.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-09T20:29:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-61050"
}
```



---

archive/issue_comments_032025.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nPatch looks good, tests pass.",
    "created_at": "2009-02-09T20:29:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5222#issuecomment-32025",
    "user": "https://github.com/burcin"
}
```

<div id="comment:1" align="right">comment:1</div>

Patch looks good, tests pass.



---

archive/issue_comments_032026.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.3.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-10T07:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5222#issuecomment-32026",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.3.rc0.

Cheers,

Michael



---

archive/issue_events_061051.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-10T07:17:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-61051"
}
```



---

archive/issue_events_061052.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-10T07:17:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-61052"
}
```
