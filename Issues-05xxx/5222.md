# Issue 5222: wrap FLINTs pseudo-division algorithm for univariate polynoials over ZZ

archive/issues_005222.json:
```json
{
    "assignees": [],
    "body": "Flint has a super-fast pseudo-division algorithm over ZZ, but Sage doesn't yet wrap it.  The point of this ticket is to wrap it.  \n\nIssue created by migration from https://trac.sagemath.org/ticket/5222\n\n",
    "closed_at": "2009-02-10T07:17:31Z",
    "created_at": "2009-02-09T19:05:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "wrap FLINTs pseudo-division algorithm for univariate polynoials over ZZ",
    "type": "issue",
    "updated_at": "2009-02-10T07:17:31Z",
    "url": "https://github.com/sagemath/sage/issues/5222",
    "user": "https://github.com/williamstein"
}
```
Flint has a super-fast pseudo-division algorithm over ZZ, but Sage doesn't yet wrap it.  The point of this ticket is to wrap it.  

Issue created by migration from https://trac.sagemath.org/ticket/5222





---

archive/issue_events_050165.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-09T19:05:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-50165"
}
```



---

archive/issue_events_050166.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-09T19:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "08517b",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-50166"
}
```



---

archive/issue_events_050167.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-09T19:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-50167"
}
```



---

archive/issue_comments_033226.json:
```json
{
    "body": "**Attachment:** [trac_5222.patch.gz](https://github.com/sagemath/sage/files/ticket5222/trac_5222.patch.gz)",
    "created_at": "2009-02-09T20:15:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5222#issuecomment-33226",
    "user": "https://github.com/williamstein"
}
```

**Attachment:** [trac_5222.patch.gz](https://github.com/sagemath/sage/files/ticket5222/trac_5222.patch.gz)



---

archive/issue_events_050168.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-09T20:29:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-50168"
}
```



---

archive/issue_events_050169.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-09T20:29:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-50169"
}
```



---

archive/issue_events_050170.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-09T20:29:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-50170"
}
```



---

archive/issue_comments_033227.json:
```json
{
    "body": "<a id='comment:1'></a>\nPatch looks good, tests pass.",
    "created_at": "2009-02-09T20:29:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5222#issuecomment-33227",
    "user": "https://github.com/burcin"
}
```

<a id='comment:1'></a>
Patch looks good, tests pass.



---

archive/issue_comments_033228.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.3.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-10T07:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5222#issuecomment-33228",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.3.rc0.

Cheers,

Michael



---

archive/issue_events_050171.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-10T07:17:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-50171"
}
```



---

archive/issue_events_050172.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-10T07:17:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5222",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5222#event-50172"
}
```
