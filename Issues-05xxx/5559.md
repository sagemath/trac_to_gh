# Issue 5559: roots issue on fedora core 32-bit and gcc 4.3.3

archive/issues_005559.json:
```json
{
    "assignees": [],
    "body": "\n```\n>> x86-Linux-fc (cicero)\n>>\n>> The following tests failed:\n>>\n>>        sage -t  \"devel/sage/sage/rings/polynomial/complex_roots.py\"\n>\n> Could you send the output of this test failing?\n\nsage -t  \"devel/sage/sage/rings/polynomial/complex_roots.py\"\n**********************************************************************\nFile \"/home/mariah/sage/sage-3.4-x86-Linux-fc/devel/sage/sage/rings/polynomial/c\nomplex_roots.py\", line 271:\n   sage: complex_roots(x^2 + 27*x + 181)\nExpected:\n   [(-14.61803398874990?..., 1), (-12.3819660112501...? + 0.?e-27*I, 1)]\nGot:\n   [(-12.3819660112501?, 1), (-14.61803398874990? + 0.?e-27*I, 1)]\n**********************************************************************\n1 items had failures:\n```\n\nAssignee: **@sagetrac-mabshoff**\n\nCC:  @sagetrac-cwitty\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5559_\n\n",
    "closed_at": "2009-04-18T01:06:51Z",
    "created_at": "2009-03-18T14:26:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "roots issue on fedora core 32-bit and gcc 4.3.3",
    "type": "issue",
    "updated_at": "2009-04-18T01:06:51Z",
    "url": "https://github.com/sagemath/sage/issues/5559",
    "user": "https://github.com/williamstein"
}
```

```
>> x86-Linux-fc (cicero)
>>
>> The following tests failed:
>>
>>        sage -t  "devel/sage/sage/rings/polynomial/complex_roots.py"
>
> Could you send the output of this test failing?

sage -t  "devel/sage/sage/rings/polynomial/complex_roots.py"
**********************************************************************
File "/home/mariah/sage/sage-3.4-x86-Linux-fc/devel/sage/sage/rings/polynomial/c
omplex_roots.py", line 271:
   sage: complex_roots(x^2 + 27*x + 181)
Expected:
   [(-14.61803398874990?..., 1), (-12.3819660112501...? + 0.?e-27*I, 1)]
Got:
   [(-12.3819660112501?, 1), (-14.61803398874990? + 0.?e-27*I, 1)]
**********************************************************************
1 items had failures:
```

Assignee: **@sagetrac-mabshoff**

CC:  @sagetrac-cwitty

_Issue created by migration from https://trac.sagemath.org/ticket/5559_





---

archive/issue_events_059034.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-18T14:26:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59034"
}
```



---

archive/issue_events_059035.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-18T14:26:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59035"
}
```



---

archive/issue_events_059036.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-18T14:26:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59036"
}
```



---

archive/issue_events_059037.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-18T14:26:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59037"
}
```



---

archive/issue_events_059038.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-01T02:00:05Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "title_is": "roots issue on fedora core 32-bit and gcc 4.3.3",
    "title_was": "roots issue on fedora core 32-bit",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59038"
}
```



---

archive/issue_comments_036183.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis is \"only\" a problem with gcc 4.3.3.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-01T02:00:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5559#issuecomment-36183",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">Comment 1</div>

This is "only" a problem with gcc 4.3.3.

Cheers,

Michael



---

archive/issue_comments_036184.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis is a 3.4.1 blocker if there ever was one :)\n\nCheers,\n\nMichael",
    "created_at": "2009-04-06T18:32:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5559#issuecomment-36184",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">Comment 2</div>

This is a 3.4.1 blocker if there ever was one :)

Cheers,

Michael



---

archive/issue_events_059039.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-06T18:32:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59039"
}
```



---

archive/issue_events_059040.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-06T18:32:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59040"
}
```



---

archive/issue_comments_036185.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAfter chatting to cwitty about this problem a couple weeks ago: we can just use another polynomial since the specific choice is irrelevant. \n\nCheers,\n\nMichael",
    "created_at": "2009-04-09T21:26:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5559#issuecomment-36185",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">Comment 3</div>

After chatting to cwitty about this problem a couple weeks ago: we can just use another polynomial since the specific choice is irrelevant. 

Cheers,

Michael



---

archive/issue_events_059041.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-18T01:06:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59041"
}
```



---

archive/issue_events_059042.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-18T01:06:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59042"
}
```



---

archive/issue_events_059043.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-18T01:06:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59043"
}
```



---

archive/issue_events_059044.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-18T01:06:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5559#event-59044"
}
```



---

archive/issue_comments_036186.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThis is a dupe of #5378.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-18T01:06:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5559#issuecomment-36186",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">Comment 4</div>

This is a dupe of #5378.

Cheers,

Michael
