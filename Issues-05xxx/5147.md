# Issue 5147: [with patch, positive review] make plot output file in DOCTEST_MODE changeable for sage-mode.el

archive/issues_005147.json:
```json
{
    "body": "Whilst incorporating sage-view.el into sage-mode.el, I needed to be able to change the output filename while in DOCTEST_MODE.  This tiny patches adds a module scope variable name with the output file name.\n\nAssignee: mabshoff\n\nKeywords: doctest plot output filename\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5147\n\n",
    "closed_at": "2009-02-02T02:57:51Z",
    "created_at": "2009-02-01T02:42:56Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch, positive review] make plot output file in DOCTEST_MODE changeable for sage-mode.el",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5147",
    "user": "https://github.com/ncalexan"
}
```
Whilst incorporating sage-view.el into sage-mode.el, I needed to be able to change the output filename while in DOCTEST_MODE.  This tiny patches adds a module scope variable name with the output file name.

Assignee: mabshoff

Keywords: doctest plot output filename

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5147





---

archive/issue_comments_041819.json:
```json
{
    "body": "Attachment [trac_5147-plot-output-filename.patch](tarball://root/attachments/some-uuid/ticket5147/trac_5147-plot-output-filename.patch) by @ncalexan created at 2009-02-01 02:44:48",
    "created_at": "2009-02-01T02:44:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5147",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5147#issuecomment-41819",
    "user": "https://github.com/ncalexan"
}
```

Attachment [trac_5147-plot-output-filename.patch](tarball://root/attachments/some-uuid/ticket5147/trac_5147-plot-output-filename.patch) by @ncalexan created at 2009-02-01 02:44:48



---

archive/issue_comments_041820.json:
```json
{
    "body": "<a id='comment:1'></a>\nI see this as harmless, and it is very useful to me and users of sage-mode.el...",
    "created_at": "2009-02-01T02:48:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5147",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5147#issuecomment-41820",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:1'></a>
I see this as harmless, and it is very useful to me and users of sage-mode.el...



---

archive/issue_events_020080.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5147",
    "rename": {
        "from": "make plot output file in DOCTEST_MODE changeable for sage-mode.el",
        "to": "[with patch, needs review] make plot output file in DOCTEST_MODE changeable for sage-mode.el"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5147#event-20080"
}
```



---

archive/issue_comments_041821.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2009-02-01T20:37:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5147",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5147#issuecomment-41821",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_events_020081.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5147",
    "rename": {
        "from": "[with patch, needs review] make plot output file in DOCTEST_MODE changeable for sage-mode.el",
        "to": "[with patch, positive review] make plot output file in DOCTEST_MODE changeable for sage-mode.el"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5147#event-20081"
}
```



---

archive/issue_events_020082.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-02T02:57:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5147",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5147#event-20082"
}
```



---

archive/issue_comments_041822.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.3.alpha4.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-02T02:57:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5147",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5147#issuecomment-41822",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.3.alpha4.

Cheers,

Michael



---

archive/issue_comments_041823.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-02-02T02:57:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5147",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5147#issuecomment-41823",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
