# Issue 5408: Matrix kernel with PARI algorithm passes wrong type

archive/issues_005408.json:
```json
{
    "assignees": [
        "https://github.com/rbeezer"
    ],
    "body": "Using the PARI kernel algorithm returns a PARI object which is unpacked as a list with the rows given as tuples.  When this is passed to hermite_form() the object is not of the right type.\n\n```\nsage: a = matrix(ZZ, [[1,2],[2,4]])\nsage: a.kernel(algorithm='pari')\n---------------------------------------------------------------------------\nAttributeError                            Traceback (most recent call last)\n\n/home/rob/.sage/temp/wave/8545/_home_rob__sage_init_sage_0.py in <module>()\n\n/opt/sage-3.3/local/lib/python2.5/site-packages/sage/matrix/matrix_integer_dense.so in sage.matrix.matrix_integer_dense.Matrix_integer_dense.kernel (sage/matrix/matrix_integer_dense.c:16256)()\n\nAttributeError: 'list' object has no attribute 'hermite_form'\n```\n\nKeywords: **kernel, PARI**\n\nComponent: **linear algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5408_\n\n",
    "closed_at": "2009-03-08T06:16:41Z",
    "created_at": "2009-03-01T06:26:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Matrix kernel with PARI algorithm passes wrong type",
    "type": "issue",
    "updated_at": "2009-03-08T06:16:41Z",
    "url": "https://github.com/sagemath/sage/issues/5408",
    "user": "https://github.com/rbeezer"
}
```
Using the PARI kernel algorithm returns a PARI object which is unpacked as a list with the rows given as tuples.  When this is passed to hermite_form() the object is not of the right type.

```
sage: a = matrix(ZZ, [[1,2],[2,4]])
sage: a.kernel(algorithm='pari')
---------------------------------------------------------------------------
AttributeError                            Traceback (most recent call last)

/home/rob/.sage/temp/wave/8545/_home_rob__sage_init_sage_0.py in <module>()

/opt/sage-3.3/local/lib/python2.5/site-packages/sage/matrix/matrix_integer_dense.so in sage.matrix.matrix_integer_dense.Matrix_integer_dense.kernel (sage/matrix/matrix_integer_dense.c:16256)()

AttributeError: 'list' object has no attribute 'hermite_form'
```

Keywords: **kernel, PARI**

Component: **linear algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/5408_





---

archive/issue_events_063620.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-03-01T06:26:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63620"
}
```



---

archive/issue_events_063621.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-03-01T06:26:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63621"
}
```



---

archive/issue_events_063622.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-03-01T06:26:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63622"
}
```



---

archive/issue_events_063623.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-03-01T06:26:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63623"
}
```



---

archive/issue_events_063624.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-03-01T06:26:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63624"
}
```



---

archive/issue_comments_033516.json:
```json
{
    "body": "Attachment: **[trac_5408_pari_kernel_matrix.patch.gz](https://github.com/sagemath/sage/files/ticket5408/trac_5408_pari_kernel_matrix.patch.gz)**",
    "created_at": "2009-03-01T23:50:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5408#issuecomment-33516",
    "user": "https://github.com/rbeezer"
}
```

Attachment: **[trac_5408_pari_kernel_matrix.patch.gz](https://github.com/sagemath/sage/files/ticket5408/trac_5408_pari_kernel_matrix.patch.gz)**



---

archive/issue_events_063625.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-03-01T23:57:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63625"
}
```



---

archive/issue_comments_033517.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nReplying to [@rbeezer](#comment:1):\n\nmatrix_dense_integer._kernel_gens_using_pari() now returns a matrix, when it previously returned a list of tuples, which always caused the calling routine (kernel_matrix) to choke.  Two comments:\n\n1.  I may change the name of this routine as part of https://github.com/sagemath/sage/issues/5381\n\n2.  I've tried to be careful about sending back an empty matrix with the right dimensions when the matrix has full rank.  I suspect there are other places that could use changes like this.\n\nApply this patch before https://github.com/sagemath/sage/issues/5381",
    "created_at": "2009-03-01T23:59:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5408#issuecomment-33517",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:2" align="right">comment:2</div>

Replying to [@rbeezer](#comment:1):

matrix_dense_integer._kernel_gens_using_pari() now returns a matrix, when it previously returned a list of tuples, which always caused the calling routine (kernel_matrix) to choke.  Two comments:

1.  I may change the name of this routine as part of https://github.com/sagemath/sage/issues/5381

2.  I've tried to be careful about sending back an empty matrix with the right dimensions when the matrix has full rank.  I suspect there are other places that could use changes like this.

Apply this patch before https://github.com/sagemath/sage/issues/5381



---

archive/issue_events_063626.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-03-07T17:05:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63626"
}
```



---

archive/issue_events_063627.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-03-07T17:05:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63627"
}
```



---

archive/issue_comments_033518.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nReview: Looks good, applies ok to 3.4.alpha0 and tests in sage/matrix (plus a whole lot of other random kernel tests I did) pass fine.",
    "created_at": "2009-03-07T17:05:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5408#issuecomment-33518",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:3" align="right">comment:3</div>

Review: Looks good, applies ok to 3.4.alpha0 and tests in sage/matrix (plus a whole lot of other random kernel tests I did) pass fine.



---

archive/issue_events_063628.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-08T06:16:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63628"
}
```



---

archive/issue_events_063629.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-08T06:16:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63629"
}
```



---

archive/issue_comments_033519.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.4.rc1.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-08T06:16:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5408#issuecomment-33519",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.4.rc1.

Cheers,

Michael



---

archive/issue_events_063630.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-08T06:16:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63630"
}
```



---

archive/issue_events_063631.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-08T06:16:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5408",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5408#event-63631"
}
```
