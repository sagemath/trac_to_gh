# Issue 5959: Better doctest for __cmp__ in primes.py

archive/issues_005959.json:
```json
{
    "assignees": [],
    "body": "From sage-devel, regarding whether \n\n```\nPrimes()>x^2+x\n```\nor not\n\n\n\n> You should change the doctest to\n> \n> sage: P != x^2 + x\n> True\n> \n> The comparison is completely arbitrary and will be machine specific.\n> However equality or not is not arbitrary.\n\n> \n> > sage: cmp(SR(3), x) in [-1,1]\n> > True\n\n> \n\n\n\nAssignee: **mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5959_\n\n",
    "closed_at": "2009-05-03T01:47:22Z",
    "created_at": "2009-05-01T15:14:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Better doctest for __cmp__ in primes.py",
    "type": "issue",
    "updated_at": "2009-05-03T01:47:22Z",
    "url": "https://github.com/sagemath/sage/issues/5959",
    "user": "https://github.com/kcrisman"
}
```
From sage-devel, regarding whether 

```
Primes()>x^2+x
```
or not



> You should change the doctest to
> 
> sage: P != x^2 + x
> True
> 
> The comparison is completely arbitrary and will be machine specific.
> However equality or not is not arbitrary.

> 
> > sage: cmp(SR(3), x) in [-1,1]
> > True

> 



Assignee: **mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/5959_





---

archive/issue_events_062337.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-01T15:14:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5959",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5959#event-62337"
}
```



---

archive/issue_events_062338.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-01T15:14:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5959",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5959#event-62338"
}
```



---

archive/issue_events_062339.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-01T15:14:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5959",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5959#event-62339"
}
```



---

archive/issue_comments_040023.json:
```json
{
    "body": "Based on 3.4.2.rc0",
    "created_at": "2009-05-01T15:15:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5959#issuecomment-40023",
    "user": "https://github.com/kcrisman"
}
```

Based on 3.4.2.rc0



---

archive/issue_comments_040024.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_5959.patch.gz](https://github.com/sagemath/sage/files/ticket5959/trac_5959.patch.gz)**\n\nThis is superseded by #5966.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-03T01:47:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5959#issuecomment-40024",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_5959.patch.gz](https://github.com/sagemath/sage/files/ticket5959/trac_5959.patch.gz)**

This is superseded by #5966.

Cheers,

Michael



---

archive/issue_events_062340.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-03T01:47:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5959",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5959#event-62340"
}
```



---

archive/issue_events_062341.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-03T01:47:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5959",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5959#event-62341"
}
```



---

archive/issue_events_062342.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-03T01:47:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5959",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5959#event-62342"
}
```
