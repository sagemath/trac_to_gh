# Issue 5161: Remove outdated SHAREDFLAGS and Solaris specific injected flags from sage-env

archive/issues_005161.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "sage-env sets a bunch of environment variables like SHAREFLAGS that cause trouble on Cygwin and Solaris and are also pretty outdated. Setting global flags should be handled in a cleaner matter in case they are required, so for now remove the code from sage-env. It does break previously working setups and has cost me considerable time to work around before I discovered that the problem was introduced by sage-env.\n\nCheers,\n\nMichael\n\nComponent: **distribution**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5161_\n\n",
    "closed_at": "2009-02-03T01:03:42Z",
    "created_at": "2009-02-02T19:59:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove outdated SHAREDFLAGS and Solaris specific injected flags from sage-env",
    "type": "issue",
    "updated_at": "2009-02-03T01:03:42Z",
    "url": "https://github.com/sagemath/sage/issues/5161",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
sage-env sets a bunch of environment variables like SHAREFLAGS that cause trouble on Cygwin and Solaris and are also pretty outdated. Setting global flags should be handled in a cleaner matter in case they are required, so for now remove the code from sage-env. It does break previously working setups and has cost me considerable time to work around before I discovered that the problem was introduced by sage-env.

Cheers,

Michael

Component: **distribution**

_Issue created by migration from https://trac.sagemath.org/ticket/5161_





---

archive/issue_events_060260.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T19:59:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5161#event-60260"
}
```



---

archive/issue_events_060261.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T19:59:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "000080",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5161#event-60261"
}
```



---

archive/issue_events_060262.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T19:59:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5161#event-60262"
}
```



---

archive/issue_events_060263.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T19:59:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5161#event-60263"
}
```



---

archive/issue_events_060264.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T19:59:15Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5161#event-60264"
}
```



---

archive/issue_events_060265.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T19:59:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5161#event-60265"
}
```



---

archive/issue_events_060266.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-03T01:00:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5161#event-60266"
}
```



---

archive/issue_comments_031655.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_5161.patch.gz](https://github.com/sagemath/sage/files/ticket5161/trac_5161.patch.gz)**\n\nLooks good to me.",
    "created_at": "2009-02-03T01:00:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5161#issuecomment-31655",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_5161.patch.gz](https://github.com/sagemath/sage/files/ticket5161/trac_5161.patch.gz)**

Looks good to me.



---

archive/issue_events_060267.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-03T01:03:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5161#event-60267"
}
```



---

archive/issue_events_060268.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-03T01:03:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5161#event-60268"
}
```



---

archive/issue_comments_031656.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.3.alpha4.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-03T01:03:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5161",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5161#issuecomment-31656",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.3.alpha4.

Cheers,

Michael
