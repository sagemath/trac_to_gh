# Issue 5116: [with patch, positive review] update M4RI to newest upstream release

archive/issues_005116.json:
```json
{
    "body": "A new version of M4RI is available. \n\nRelease notes: http://bitbucket.org/malb/m4ri/wiki/M4RI-20090105\n\nAssignee: mabshoff\n\nKeywords: m4ri\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5116\n\n",
    "closed_at": "2009-02-04T00:08:29Z",
    "created_at": "2009-01-28T11:43:10Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch, positive review] update M4RI to newest upstream release",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5116",
    "user": "https://github.com/malb"
}
```
A new version of M4RI is available. 

Release notes: http://bitbucket.org/malb/m4ri/wiki/M4RI-20090105

Assignee: mabshoff

Keywords: m4ri

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5116





---

archive/issue_comments_041514.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [m4ri_200901.patch](tarball://root/attachments/some-uuid/ticket5116/m4ri_200901.patch) by @malb created at 2009-01-28 13:15:21\n\n* builds/checks/doctests clean on sage.math (Linux 64-bit)\n  * builds/checks/doctests clean on bsd (OSX 32-bit)\n  * builds/checks/doctests clean on iras (Linux IA-64)\n\nNote that the SPKG has `make check` enabled.",
    "created_at": "2009-01-28T13:15:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5116",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5116#issuecomment-41514",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'></a>
Attachment [m4ri_200901.patch](tarball://root/attachments/some-uuid/ticket5116/m4ri_200901.patch) by @malb created at 2009-01-28 13:15:21

* builds/checks/doctests clean on sage.math (Linux 64-bit)
  * builds/checks/doctests clean on bsd (OSX 32-bit)
  * builds/checks/doctests clean on iras (Linux IA-64)

Note that the SPKG has `make check` enabled.



---

archive/issue_events_019911.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5116",
    "rename": {
        "from": "update M4RI to newest upstream release",
        "to": "[with patch, needs review] update M4RI to newest upstream release"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5116#event-19911"
}
```



---

archive/issue_comments_041515.json:
```json
{
    "body": "<a id='comment:2'></a>\n* builds/checks/doctests clean on vbox-linux-32-bit (Linux 32-bit)",
    "created_at": "2009-01-28T23:40:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5116",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5116#issuecomment-41515",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'></a>
* builds/checks/doctests clean on vbox-linux-32-bit (Linux 32-bit)



---

archive/issue_comments_041516.json:
```json
{
    "body": "<a id='comment:3'></a>\nThe spkg can be found at\n\nhttp://sage.math.washington.edu/home/malb/spkgs/libm4ri-20090128.spkg\n\nCheers,\n\nMichael",
    "created_at": "2009-02-03T23:18:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5116",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5116#issuecomment-41516",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
The spkg can be found at

http://sage.math.washington.edu/home/malb/spkgs/libm4ri-20090128.spkg

Cheers,

Michael



---

archive/issue_comments_041517.json:
```json
{
    "body": "<a id='comment:4'></a>\nSpkg builds fine and passes make check also on OSX 10.4/PPC. The patch is also good to go and passes doctests.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-04T00:07:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5116",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5116#issuecomment-41517",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Spkg builds fine and passes make check also on OSX 10.4/PPC. The patch is also good to go and passes doctests.

Cheers,

Michael



---

archive/issue_events_019912.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5116",
    "rename": {
        "from": "[with patch, needs review] update M4RI to newest upstream release",
        "to": "[with patch, positive review] update M4RI to newest upstream release"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5116#event-19912"
}
```



---

archive/issue_comments_041518.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-02-04T00:08:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5116",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5116#issuecomment-41518",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_019913.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-04T00:08:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5116",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5116#event-19913"
}
```



---

archive/issue_comments_041519.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged patch and spkg into Sage 3.3.alpha4.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-04T00:08:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5116",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5116#issuecomment-41519",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged patch and spkg into Sage 3.3.alpha4.

Cheers,

Michael
