# Issue 5497: notebook: make sure a worksheet handles being run multiple times gracefully

archive/issues_005497.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "Currently the user can edit/start the same worksheet multiple times. But there is no synchronization, this is bad.\n\nCheers,\n\nMichael\n\nCC:  @williamstein @mwhansen\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5497_\n\n",
    "closed_at": "2009-11-15T13:48:51Z",
    "created_at": "2009-03-12T04:18:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook: make sure a worksheet handles being run multiple times gracefully",
    "type": "issue",
    "updated_at": "2009-11-15T21:35:56Z",
    "url": "https://github.com/sagemath/sage/issues/5497",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
Currently the user can edit/start the same worksheet multiple times. But there is no synchronization, this is bad.

Cheers,

Michael

CC:  @williamstein @mwhansen

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/5497_





---

archive/issue_events_064921.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-12T04:18:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64921"
}
```



---

archive/issue_events_064922.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-12T04:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64922"
}
```



---

archive/issue_events_064923.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-12T04:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64923"
}
```



---

archive/issue_events_064924.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-12T04:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64924"
}
```



---

archive/issue_events_064925.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-03-12T04:18:37Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64925"
}
```



---

archive/issue_events_064926.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-12T04:34:11Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "title_is": "notebook: make sure a worksheet handles being run multiple times gracefully",
    "title_was": "notebook: make sure each worksheet can only run once",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64926"
}
```



---

archive/issue_comments_034358.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Currently the user can edit/start the same worksheet multiple times. This is bad.\n+Currently the user can edit/start the same worksheet multiple times. But there is no synchronization, this is bad.\n \n Cheers,\n \n``````\n",
    "created_at": "2009-03-12T04:34:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5497#issuecomment-34358",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Currently the user can edit/start the same worksheet multiple times. This is bad.
+Currently the user can edit/start the same worksheet multiple times. But there is no synchronization, this is bad.
 
 Cheers,
 
``````




---

archive/issue_comments_034359.json:
```json
{
    "body": "Work Issues: **close a fixed or duplicate?**",
    "created_at": "2009-11-01T01:11:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5497#issuecomment-34359",
    "user": "https://github.com/qed777"
}
```

Work Issues: **close a fixed or duplicate?**



---

archive/issue_comments_034360.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI think #7254 (and #7362) address the problem.  Should we close this ticket?",
    "created_at": "2009-11-01T01:11:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5497#issuecomment-34360",
    "user": "https://github.com/qed777"
}
```

<div id="comment:2" align="right">comment:2</div>

I think #7254 (and #7362) address the problem.  Should we close this ticket?



---

archive/issue_comments_034361.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAs stated by mpatel, #7254 and #7362 already address this problem. Please close this.",
    "created_at": "2009-11-15T12:08:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5497#issuecomment-34361",
    "user": "https://github.com/TimDumol"
}
```

<div id="comment:3" align="right">comment:3</div>

As stated by mpatel, #7254 and #7362 already address this problem. Please close this.



---

archive/issue_events_064927.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-11-15T12:08:10Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "title_is": "[Please close this] notebook: make sure a worksheet handles being run multiple times gracefully",
    "title_was": "notebook: make sure a worksheet handles being run multiple times gracefully",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64927"
}
```



---

archive/issue_events_064928.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-15T13:48:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64928"
}
```



---

archive/issue_events_064929.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-15T13:48:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64929"
}
```



---

archive/issue_events_064930.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-15T13:48:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64930"
}
```



---

archive/issue_events_064931.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-15T13:48:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64931"
}
```



---

archive/issue_comments_034362.json:
```json
{
    "body": "Changed work issues from **close a fixed or duplicate?** to none",
    "created_at": "2009-11-15T21:35:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5497#issuecomment-34362",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed work issues from **close a fixed or duplicate?** to none



---

archive/issue_events_064932.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-15T21:35:56Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5497",
    "title_is": "notebook: make sure a worksheet handles being run multiple times gracefully",
    "title_was": "[Please close this] notebook: make sure a worksheet handles being run multiple times gracefully",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5497#event-64932"
}
```
