# Issue 5168: matrix0.pyx: fix doctest for commutator

archive/issues_005168.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "Here is the extent of the docstring for the commutator method in matrix0.pyx:\n\n```\n        Return the commutator self*other - other*self.\n\n        EXAMPLES:\n            sage: A = Matrix(QQ[['t']], 2, 2, range(4))\n```\nFix the doctest so that it actually computes a commutator.\n\n\nKeywords: **matrix0, commutator**\n\nComponent: **linear algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5168_\n\n",
    "closed_at": "2009-02-05T11:10:01Z",
    "created_at": "2009-02-03T21:17:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "matrix0.pyx: fix doctest for commutator",
    "type": "issue",
    "updated_at": "2009-02-05T11:10:01Z",
    "url": "https://github.com/sagemath/sage/issues/5168",
    "user": "https://github.com/jhpalmieri"
}
```
Here is the extent of the docstring for the commutator method in matrix0.pyx:

```
        Return the commutator self*other - other*self.

        EXAMPLES:
            sage: A = Matrix(QQ[['t']], 2, 2, range(4))
```
Fix the doctest so that it actually computes a commutator.


Keywords: **matrix0, commutator**

Component: **linear algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/5168_





---

archive/issue_events_060326.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-03T21:17:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5168#event-60326"
}
```



---

archive/issue_events_060327.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-03T21:17:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5168#event-60327"
}
```



---

archive/issue_events_060328.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-03T21:17:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5168#event-60328"
}
```



---

archive/issue_events_060329.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-03T21:17:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5168#event-60329"
}
```



---

archive/issue_events_060330.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-03T21:17:52Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "subject": "https://github.com/jhpalmieri",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5168#event-60330"
}
```



---

archive/issue_events_060331.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-03T21:17:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5168#event-60331"
}
```



---

archive/issue_comments_031680.json:
```json
{
    "body": "Attachment: **[5168.patch.gz](https://github.com/sagemath/sage/files/ticket5168/5168.patch.gz)**",
    "created_at": "2009-02-03T21:18:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5168#issuecomment-31680",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[5168.patch.gz](https://github.com/sagemath/sage/files/ticket5168/5168.patch.gz)**



---

archive/issue_events_060332.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-02-05T06:57:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5168#event-60332"
}
```



---

archive/issue_comments_031681.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nNew doctests look good, and they pass.\n\n(Good catch on noticing the original bug, too.)\n\nPositive review.",
    "created_at": "2009-02-05T06:57:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5168#issuecomment-31681",
    "user": "https://github.com/sagetrac-cwitty"
}
```

<div id="comment:1" align="right">comment:1</div>

New doctests look good, and they pass.

(Good catch on noticing the original bug, too.)

Positive review.



---

archive/issue_events_060333.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-05T11:10:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5168#event-60333"
}
```



---

archive/issue_events_060334.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-05T11:10:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5168#event-60334"
}
```



---

archive/issue_comments_031682.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.3.alpha6.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-05T11:10:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5168",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5168#issuecomment-31682",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.3.alpha6.

Cheers,

Michael
