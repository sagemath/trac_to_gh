# Issue 5897: add some javascript to make jsmath happier (very easy ticket to close, probably)

archive/issues_005897.json:
```json
{
    "assignees": [],
    "body": "This was on sage-support on April 25.  \n\n```\nThanks for posting the new example.  I have found that the problem is\ndue to the fact that Sage puts the output inside a <PRE> block, and\nIE7 gets confused about some of its measurements in that case.  It can\nbe fixed by including\n\n span.typeset {\n    white-space: normal;\n }\n\nin the css/main.css file for sage.  I will add this to jsMath in the\nnext release.\n\nDavide\n```\n\nWe should add this to sage, since who knows when it'll go into jsmath and get into sage.\n\nAssignee: **boothby**\n\nWork Issues: **Close as fixed**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5897_\n\n",
    "closed_at": "2009-11-15T14:17:11Z",
    "created_at": "2009-04-26T02:05:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add some javascript to make jsmath happier (very easy ticket to close, probably)",
    "type": "issue",
    "updated_at": "2009-11-15T14:17:11Z",
    "url": "https://github.com/sagemath/sage/issues/5897",
    "user": "https://github.com/williamstein"
}
```
This was on sage-support on April 25.  

```
Thanks for posting the new example.  I have found that the problem is
due to the fact that Sage puts the output inside a <PRE> block, and
IE7 gets confused about some of its measurements in that case.  It can
be fixed by including

 span.typeset {
    white-space: normal;
 }

in the css/main.css file for sage.  I will add this to jsMath in the
next release.

Davide
```

We should add this to sage, since who knows when it'll go into jsmath and get into sage.

Assignee: **boothby**

Work Issues: **Close as fixed**

_Issue created by migration from https://trac.sagemath.org/ticket/5897_





---

archive/issue_events_061629.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-26T02:05:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5897",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5897#event-61629"
}
```



---

archive/issue_events_061630.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-26T02:05:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5897",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5897#event-61630"
}
```



---

archive/issue_events_061631.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-26T02:05:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5897",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5897#event-61631"
}
```



---

archive/issue_comments_039436.json:
```json
{
    "body": "Work Issues: **Close as fixed**",
    "created_at": "2009-11-14T05:51:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5897#issuecomment-39436",
    "user": "https://github.com/qed777"
}
```

Work Issues: **Close as fixed**



---

archive/issue_comments_039437.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis is in the version of jsMath included with SageNB.  Should we close this ticket?",
    "created_at": "2009-11-14T05:51:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5897#issuecomment-39437",
    "user": "https://github.com/qed777"
}
```

<a id='comment:1'>Comment 1:</a>
This is in the version of jsMath included with SageNB.  Should we close this ticket?



---

archive/issue_events_061632.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-15T14:17:11Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5897",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5897#event-61632"
}
```



---

archive/issue_events_061633.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-15T14:17:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5897",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5897#event-61633"
}
```
