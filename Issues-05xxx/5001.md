# Issue 5001: [with patch positive review] kernels of integer matrices

archive/issues_005001.json:
```json
{
    "body": "\n```\nsage: id = matrix(ZZ, 2, 2, [[1, 0], [0, 1]]) \nsage: id.left_kernel()\nTraceback\n...\nTypeError: Argument K (= Integer Ring) must be a field.\n```\nOn the other hand, `id.right_kernel()` and `id.kernel()` both work, and `id.kernel()` actually computes the left kernel.  Note also that the documentation for both left_kernel and right_kernel says that the answer will be a vector space, not a module over the integers; this should be fixed, too.\n\n\n\nAssignee: @williamstein\n\nKeywords: matrix, kernel\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5001\n\n",
    "closed_at": "2009-01-24T18:05:31Z",
    "created_at": "2009-01-17T16:36:51Z",
    "labels": [
        "component: linear algebra",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch positive review] kernels of integer matrices",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5001",
    "user": "https://github.com/jhpalmieri"
}
```

```
sage: id = matrix(ZZ, 2, 2, [[1, 0], [0, 1]]) 
sage: id.left_kernel()
Traceback
...
TypeError: Argument K (= Integer Ring) must be a field.
```
On the other hand, `id.right_kernel()` and `id.kernel()` both work, and `id.kernel()` actually computes the left kernel.  Note also that the documentation for both left_kernel and right_kernel says that the answer will be a vector space, not a module over the integers; this should be fixed, too.



Assignee: @williamstein

Keywords: matrix, kernel

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5001





---

archive/issue_comments_040480.json:
```json
{
    "body": "Attachment [trac_5001.patch](tarball://root/attachments/some-uuid/ticket5001/trac_5001.patch) by @williamstein created at 2009-01-24 16:24:54",
    "created_at": "2009-01-24T16:24:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5001#issuecomment-40480",
    "user": "https://github.com/williamstein"
}
```

Attachment [trac_5001.patch](tarball://root/attachments/some-uuid/ticket5001/trac_5001.patch) by @williamstein created at 2009-01-24 16:24:54



---

archive/issue_events_019376.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5001",
    "rename": {
        "from": "kernels of integer matrices",
        "to": "[with patch; needs review] kernels of integer matrices"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5001#event-19376"
}
```



---

archive/issue_comments_040481.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good, all tests passed. A few comments: the documentation still says \"vector space\" when computing the kernel of an integer matrix, but I can live with that.  Perhaps more seriously, if you don't apply patch #5089, then computing the kernel (or left_kernel or right_kernel) of a sparse integer matrix leads to a segmentation fault.  Does this need to be investigated further?",
    "created_at": "2009-01-24T17:11:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5001#issuecomment-40481",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
Looks good, all tests passed. A few comments: the documentation still says "vector space" when computing the kernel of an integer matrix, but I can live with that.  Perhaps more seriously, if you don't apply patch #5089, then computing the kernel (or left_kernel or right_kernel) of a sparse integer matrix leads to a segmentation fault.  Does this need to be investigated further?



---

archive/issue_events_019377.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5001",
    "rename": {
        "from": "[with patch; needs review] kernels of integer matrices",
        "to": "[with patch; mostly positive review] kernels of integer matrices"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5001#event-19377"
}
```



---

archive/issue_events_019378.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5001",
    "rename": {
        "from": "[with patch; mostly positive review] kernels of integer matrices",
        "to": "[with patch positive review] kernels of integer matrices"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5001#event-19378"
}
```



---

archive/issue_comments_040482.json:
```json
{
    "body": "<a id='comment:3'></a>\nSince #5089 is being merged I am changing this to a positive review. The documentation issue about vector spaces vs. modules should be addressed via a followup ticket. \n\nCheers,\n\nMichael",
    "created_at": "2009-01-24T17:58:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5001#issuecomment-40482",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Since #5089 is being merged I am changing this to a positive review. The documentation issue about vector spaces vs. modules should be addressed via a followup ticket. 

Cheers,

Michael



---

archive/issue_comments_040483.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.3.alpha2\n\nCheers,\n\nMichael",
    "created_at": "2009-01-24T18:05:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5001#issuecomment-40483",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.3.alpha2

Cheers,

Michael



---

archive/issue_comments_040484.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-01-24T18:05:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5001#issuecomment-40484",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_019379.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-24T18:05:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5001",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5001#event-19379"
}
```
