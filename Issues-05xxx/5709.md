# Issue 5709: [with patch; positive review] singular interface -- use a smaller file size cutoff on solaris

archive/issues_005709.json:
```json
{
    "body": "\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5709\n\n",
    "closed_at": "2009-04-09T06:10:58Z",
    "created_at": "2009-04-08T01:04:28Z",
    "labels": [
        "component: interfaces",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.1",
    "title": "[with patch; positive review] singular interface -- use a smaller file size cutoff on solaris",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5709",
    "user": "https://github.com/williamstein"
}
```


Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5709





---

archive/issue_comments_047780.json:
```json
{
    "body": "Attachment [trac_5709.patch](tarball://root/attachments/some-uuid/ticket5709/trac_5709.patch) by @williamstein created at 2009-04-08 01:11:22",
    "created_at": "2009-04-08T01:11:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5709#issuecomment-47780",
    "user": "https://github.com/williamstein"
}
```

Attachment [trac_5709.patch](tarball://root/attachments/some-uuid/ticket5709/trac_5709.patch) by @williamstein created at 2009-04-08 01:11:22



---

archive/issue_events_022674.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5709",
    "rename": {
        "from": "singular interface -- use a smaller file size cutoff on solaris",
        "to": "[with patch; needs review] singular interface -- use a smaller file size cutoff on solaris"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5709#event-22674"
}
```



---

archive/issue_events_022675.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5709",
    "rename": {
        "from": "[with patch; needs review] singular interface -- use a smaller file size cutoff on solaris",
        "to": "[with patch; positive review] singular interface -- use a smaller file size cutoff on solaris"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5709#event-22675"
}
```



---

archive/issue_comments_047781.json:
```json
{
    "body": "<a id='comment:2'></a>\nPositive review - I have used a dirty version of this patch for about half a year :)\n\nCheers,\n\nMichael",
    "created_at": "2009-04-08T01:19:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5709#issuecomment-47781",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Positive review - I have used a dirty version of this patch for about half a year :)

Cheers,

Michael



---

archive/issue_comments_047782.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.4.1.rc2.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-09T06:10:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5709#issuecomment-47782",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.4.1.rc2.

Cheers,

Michael



---

archive/issue_comments_047783.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-04-09T06:10:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5709#issuecomment-47783",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_022676.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-09T06:10:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5709",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5709#event-22676"
}
```
