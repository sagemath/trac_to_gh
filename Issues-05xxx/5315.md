# Issue 5315: [with spkg, positive review] Fix MPIR.spkg build on more OSX MacIntel boxen

archive/issues_005315.json:
```json
{
    "body": "There is a known problem with PIC enabled MPIR code on 32 bit OSX when the CPU is capable of 64 bits. To work around that we delete some files, but there are some left that are used on older Macs:\n\n```\np6/mode1o.asm\np6/dive_1.asm\npentium/hamdist.asm\npentium/mod_1.asm\npentium/popcount.asm\npentium/mode1o.asm\npentium/dive_1.asm\n```\nDeleting them on demand will fix the build. See also the thread at\n\n http://groups.google.com/group/sage-devel/browse_thread/thread/88c084b8cd828ac6\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5315\n\n",
    "closed_at": "2009-02-20T16:30:48Z",
    "created_at": "2009-02-20T05:33:37Z",
    "labels": [
        "component: build",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with spkg, positive review] Fix MPIR.spkg build on more OSX MacIntel boxen",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5315",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
There is a known problem with PIC enabled MPIR code on 32 bit OSX when the CPU is capable of 64 bits. To work around that we delete some files, but there are some left that are used on older Macs:

```
p6/mode1o.asm
p6/dive_1.asm
pentium/hamdist.asm
pentium/mod_1.asm
pentium/popcount.asm
pentium/mode1o.asm
pentium/dive_1.asm
```
Deleting them on demand will fix the build. See also the thread at

 http://groups.google.com/group/sage-devel/browse_thread/thread/88c084b8cd828ac6

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5315





---

archive/issue_events_020881.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-20T06:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5315",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5315#event-20881"
}
```



---

archive/issue_comments_043560.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe spkg at \n\n http://sage.math.washington.edu/home/mabshoff/SPKG/gmp-mpir-0.9.spkg\n\nought to fix the problem. I have asked the reported of the original issue to test and report back.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T13:35:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5315#issuecomment-43560",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
The spkg at 

 http://sage.math.washington.edu/home/mabshoff/SPKG/gmp-mpir-0.9.spkg

ought to fix the problem. I have asked the reported of the original issue to test and report back.

Cheers,

Michael



---

archive/issue_events_020882.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5315",
    "rename": {
        "from": "Fix MPIR.spkg build on more OSX MacIntel boxen",
        "to": "[with spkg, needs review] Fix MPIR.spkg build on more OSX MacIntel boxen"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5315#event-20882"
}
```



---

archive/issue_events_020883.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5315",
    "rename": {
        "from": "[with spkg, needs review] Fix MPIR.spkg build on more OSX MacIntel boxen",
        "to": "[with spkg, positive review] Fix MPIR.spkg build on more OSX MacIntel boxen"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5315#event-20883"
}
```



---

archive/issue_comments_043561.json:
```json
{
    "body": "<a id='comment:4'></a>\nPositive review by proxy from Mark:\n\n```\n> I had a class this morning and have only just started the build.\n> It is running now and I can tell you that it has definitely made it\n> past this specific problem.\n\nOk, this is a positive review from you in my eyes for this problem and\nwe can merge the spkg. I will ask someone else to take another look,\nbut I can assure you I did a very clean checkin :)\n\n>  It should still take a couple of hours\n> to complete.  I'll report back when I've got the finished product.\n\nCool, let me know if anything else blows up for you. I would assume\n3.3.rc3 is out before your build finishes.\n```\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T16:28:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5315#issuecomment-43561",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Positive review by proxy from Mark:

```
> I had a class this morning and have only just started the build.
> It is running now and I can tell you that it has definitely made it
> past this specific problem.

Ok, this is a positive review from you in my eyes for this problem and
we can merge the spkg. I will ask someone else to take another look,
but I can assure you I did a very clean checkin :)

>  It should still take a couple of hours
> to complete.  I'll report back when I've got the finished product.

Cool, let me know if anything else blows up for you. I would assume
3.3.rc3 is out before your build finishes.
```

Cheers,

Michael



---

archive/issue_events_020884.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-20T16:30:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5315",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5315#event-20884"
}
```



---

archive/issue_comments_043562.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 3.3.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T16:30:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5315#issuecomment-43562",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 3.3.rc3.

Cheers,

Michael



---

archive/issue_comments_043563.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-02-20T16:30:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5315#issuecomment-43563",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
