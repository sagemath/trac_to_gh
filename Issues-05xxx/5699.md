# Issue 5699: [with patch; positive review] notebook -- %r mode is completely broken (trivial to fix)

archive/issues_005699.json:
```json
{
    "body": "In the notebook\n\n```\n%r\n2+5\n///\nTraceback (most recent call last):\n...\nTypeError: eval() got multiple values for keyword argument 'synchronize'\n```\n\nAssignee: boothby\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5699\n\n",
    "closed_at": "2009-04-11T00:00:10Z",
    "created_at": "2009-04-06T19:28:16Z",
    "labels": [
        "component: notebook",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.1",
    "title": "[with patch; positive review] notebook -- %r mode is completely broken (trivial to fix)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5699",
    "user": "https://github.com/williamstein"
}
```
In the notebook

```
%r
2+5
///
Traceback (most recent call last):
...
TypeError: eval() got multiple values for keyword argument 'synchronize'
```

Assignee: boothby

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5699





---

archive/issue_comments_047719.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_5699.patch](tarball://root/attachments/some-uuid/ticket5699/trac_5699.patch) by @williamstein created at 2009-04-07 18:59:12",
    "created_at": "2009-04-07T18:59:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5699#issuecomment-47719",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [trac_5699.patch](tarball://root/attachments/some-uuid/ticket5699/trac_5699.patch) by @williamstein created at 2009-04-07 18:59:12



---

archive/issue_events_022644.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5699",
    "rename": {
        "from": "notebook -- %r mode is completely broken (trivial to fix)",
        "to": "[with patch; needs review] notebook -- %r mode is completely broken (trivial to fix)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5699#event-22644"
}
```



---

archive/issue_comments_047720.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2009-04-10T22:14:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5699#issuecomment-47720",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_events_022645.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5699",
    "rename": {
        "from": "[with patch; needs review] notebook -- %r mode is completely broken (trivial to fix)",
        "to": "[with patch; positive review] notebook -- %r mode is completely broken (trivial to fix)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5699#event-22645"
}
```



---

archive/issue_comments_047721.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-04-11T00:00:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5699#issuecomment-47721",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_022646.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-11T00:00:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5699",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5699#event-22646"
}
```



---

archive/issue_comments_047722.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.4.1.rc2.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-11T00:00:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5699#issuecomment-47722",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.4.1.rc2.

Cheers,

Michael
