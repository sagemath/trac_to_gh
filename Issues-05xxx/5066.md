# Issue 5066: [with patch, positive review] break out relative number fields into separate file

archive/issues_005066.json:
```json
{
    "body": "This has been some time coming, but let's separate relative number fields from generic/absolute number fields now, while I'm looking at them.\n\nAssignee: @williamstein\n\nKeywords: relative number fields files\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5066\n\n",
    "closed_at": "2009-01-29T05:38:41Z",
    "created_at": "2009-01-23T08:59:32Z",
    "labels": [
        "component: number theory",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch, positive review] break out relative number fields into separate file",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5066",
    "user": "https://github.com/ncalexan"
}
```
This has been some time coming, but let's separate relative number fields from generic/absolute number fields now, while I'm looking at them.

Assignee: @williamstein

Keywords: relative number fields files

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5066





---

archive/issue_comments_040963.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_5066-relnf-file.patch](tarball://root/attachments/some-uuid/ticket5066/trac_5066-relnf-file.patch) by @ncalexan created at 2009-01-23 09:21:23\n\nFails a doctest, to be addressed by patches to #1357 (which will depend on this)",
    "created_at": "2009-01-23T09:21:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5066#issuecomment-40963",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:1'></a>
Attachment [trac_5066-relnf-file.patch](tarball://root/attachments/some-uuid/ticket5066/trac_5066-relnf-file.patch) by @ncalexan created at 2009-01-23 09:21:23

Fails a doctest, to be addressed by patches to #1357 (which will depend on this)



---

archive/issue_comments_040964.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good, assuming that the other patch you mentioned (which isn't #1357) gets in at the same time.  The only change that needs to be made is in sage.rings.polynomials.polynomial_quotient_ring_element, changing number_field to number_field_rel a couple places.",
    "created_at": "2009-01-24T09:47:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5066#issuecomment-40964",
    "user": "https://github.com/roed314"
}
```

<a id='comment:2'></a>
Looks good, assuming that the other patch you mentioned (which isn't #1357) gets in at the same time.  The only change that needs to be made is in sage.rings.polynomials.polynomial_quotient_ring_element, changing number_field to number_field_rel a couple places.



---

archive/issue_events_019634.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "rename": {
        "from": "[with patch, needs review] break out relative number fields into separate file",
        "to": "[with patch, needs work] break out relative number fields into separate file"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5066#event-19634"
}
```



---

archive/issue_comments_040965.json:
```json
{
    "body": "<a id='comment:3'></a>\nPatch should be at #1367, sorry.  With #5066 and #1367, all doctests pass on sage.math.",
    "created_at": "2009-01-24T10:00:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5066#issuecomment-40965",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:3'></a>
Patch should be at #1367, sorry.  With #5066 and #1367, all doctests pass on sage.math.



---

archive/issue_events_019635.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "rename": {
        "from": "[with patch, needs work] break out relative number fields into separate file",
        "to": "[with patch, needs review] break out relative number fields into separate file"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5066#event-19635"
}
```



---

archive/issue_events_019636.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "rename": {
        "from": "[with patch, needs review] break out relative number fields into separate file",
        "to": "[with patch, positive review] break out relative number fields into separate file"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5066#event-19636"
}
```



---

archive/issue_events_019637.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-29T01:00:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5066#event-19637"
}
```



---

archive/issue_events_019638.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-29T05:38:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5066#event-19638"
}
```



---

archive/issue_comments_040966.json:
```json
{
    "body": "<a id='comment:6'></a>\nMerged in Sage 3.3.alpha3.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-29T05:38:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5066#issuecomment-40966",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
Merged in Sage 3.3.alpha3.

Cheers,

Michael



---

archive/issue_comments_040967.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-01-29T05:38:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5066#issuecomment-40967",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
