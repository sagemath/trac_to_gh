# Issue 5884: OpenSuse 11/x86-64: doctest failure in groups/perm_gps/permgroup.py

archive/issues_005884.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "This might be a side effect of me screwing up at #5697:\n\n```\nsage -t -long \"devel/sage/sage/groups/perm_gps/permgroup.py\"\n********************************************************************\nFile \"/space/wstein/farm/sage-3.4.1/devel/sage/sage/groups/perm_gps/permgroup.py\", line 914:\n   sage: G.random_element()\nExpected:\n   (1,2)(4,5)\nGot:\n   (2,3)(4,5)\n********************************************************************\n```\n\nCC:  @wdjoyner\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5884_\n\n",
    "closed_at": "2009-05-15T14:32:09Z",
    "created_at": "2009-04-24T00:43:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "OpenSuse 11/x86-64: doctest failure in groups/perm_gps/permgroup.py",
    "type": "issue",
    "updated_at": "2009-10-14T03:04:58Z",
    "url": "https://github.com/sagemath/sage/issues/5884",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
This might be a side effect of me screwing up at #5697:

```
sage -t -long "devel/sage/sage/groups/perm_gps/permgroup.py"
********************************************************************
File "/space/wstein/farm/sage-3.4.1/devel/sage/sage/groups/perm_gps/permgroup.py", line 914:
   sage: G.random_element()
Expected:
   (1,2)(4,5)
Got:
   (2,3)(4,5)
********************************************************************
```

CC:  @wdjoyner

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/5884_





---

archive/issue_events_070278.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T00:43:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-70278"
}
```



---

archive/issue_events_070279.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T00:43:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-70279"
}
```



---

archive/issue_events_070280.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T00:43:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-70280"
}
```



---

archive/issue_events_070281.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T00:43:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-70281"
}
```



---

archive/issue_events_070282.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T00:43:57Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-70282"
}
```



---

archive/issue_events_070283.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-15T14:32:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-70283"
}
```



---

archive/issue_events_070284.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-15T14:32:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-70284"
}
```



---

archive/issue_events_070285.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-15T14:32:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-70285"
}
```



---

archive/issue_events_070286.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-15T14:32:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-70286"
}
```



---

archive/issue_comments_037977.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nHmm, this seems to be gone, so \"invalid\" until we find a reproducible test case.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-15T14:32:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5884#issuecomment-37977",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Hmm, this seems to be gone, so "invalid" until we find a reproducible test case.

Cheers,

Michael



---

archive/issue_comments_037978.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis has cropped up again in 4.1.2.rc2 -- see #7206.",
    "created_at": "2009-10-14T03:04:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5884#issuecomment-37978",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:2" align="right">comment:2</div>

This has cropped up again in 4.1.2.rc2 -- see #7206.
