# Issue 5884: OpenSuse 11/x86-64: doctest failure in groups/perm_gps/permgroup.py

archive/issues_005884.json:
```json
{
    "assignees": [],
    "body": "This might be a side effect of me screwing up at #5697:\n\n```\nsage -t -long \"devel/sage/sage/groups/perm_gps/permgroup.py\"\n********************************************************************\nFile \"/space/wstein/farm/sage-3.4.1/devel/sage/sage/groups/perm_gps/permgroup.py\", line 914:\n   sage: G.random_element()\nExpected:\n   (1,2)(4,5)\nGot:\n   (2,3)(4,5)\n********************************************************************\n```\n\nAssignee: **mabshoff**\n\nCC:  @wdjoyner\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5884_\n\n",
    "closed_at": "2009-05-15T14:32:09Z",
    "created_at": "2009-04-24T00:43:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "OpenSuse 11/x86-64: doctest failure in groups/perm_gps/permgroup.py",
    "type": "issue",
    "updated_at": "2009-10-14T03:04:58Z",
    "url": "https://github.com/sagemath/sage/issues/5884",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
This might be a side effect of me screwing up at #5697:

```
sage -t -long "devel/sage/sage/groups/perm_gps/permgroup.py"
********************************************************************
File "/space/wstein/farm/sage-3.4.1/devel/sage/sage/groups/perm_gps/permgroup.py", line 914:
   sage: G.random_element()
Expected:
   (1,2)(4,5)
Got:
   (2,3)(4,5)
********************************************************************
```

Assignee: **mabshoff**

CC:  @wdjoyner

_Issue created by migration from https://trac.sagemath.org/ticket/5884_





---

archive/issue_events_061497.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T00:43:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-61497"
}
```



---

archive/issue_events_061498.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T00:43:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-61498"
}
```



---

archive/issue_events_061499.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T00:43:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-61499"
}
```



---

archive/issue_events_061500.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-15T14:32:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-61500"
}
```



---

archive/issue_events_061501.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-15T14:32:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5884#event-61501"
}
```



---

archive/issue_comments_039329.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nHmm, this seems to be gone, so \"invalid\" until we find a reproducible test case.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-15T14:32:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5884#issuecomment-39329",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Hmm, this seems to be gone, so "invalid" until we find a reproducible test case.

Cheers,

Michael



---

archive/issue_comments_039330.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis has cropped up again in 4.1.2.rc2 -- see #7206.",
    "created_at": "2009-10-14T03:04:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5884#issuecomment-39330",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:2'>Comment 2:</a>
This has cropped up again in 4.1.2.rc2 -- see #7206.
