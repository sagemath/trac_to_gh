# Issue 5354: [with patch, positive review] stop paying attention to <stdlib.h> RAND_MAX (should fix problems on Solaris)

archive/issues_005354.json:
```json
{
    "body": "The randstate framework always provides 31-bit random numbers regardless of platform, so RAND_MAX should be ignored.\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5354\n\n",
    "closed_at": "2009-02-24T19:50:21Z",
    "created_at": "2009-02-24T02:21:00Z",
    "labels": [
        "component: porting: solaris",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4",
    "title": "[with patch, positive review] stop paying attention to <stdlib.h> RAND_MAX (should fix problems on Solaris)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5354",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
The randstate framework always provides 31-bit random numbers regardless of platform, so RAND_MAX should be ignored.

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5354





---

archive/issue_comments_043919.json:
```json
{
    "body": "Attachment [ignore-rand-max.patch](tarball://root/attachments/some-uuid/ticket5354/ignore-rand-max.patch) by cwitty created at 2009-02-24 02:21:20",
    "created_at": "2009-02-24T02:21:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5354#issuecomment-43919",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Attachment [ignore-rand-max.patch](tarball://root/attachments/some-uuid/ticket5354/ignore-rand-max.patch) by cwitty created at 2009-02-24 02:21:20



---

archive/issue_comments_043920.json:
```json
{
    "body": "<a id='comment:1'></a>\nNice. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-24T07:15:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5354#issuecomment-43920",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Nice. Positive review.

Cheers,

Michael



---

archive/issue_events_021045.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5354",
    "rename": {
        "from": "[with patch, needs review] stop paying attention to <stdlib.h> RAND_MAX (should fix problems on Solaris)",
        "to": "[with patch, positive review] stop paying attention to <stdlib.h> RAND_MAX (should fix problems on Solaris)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5354#event-21045"
}
```



---

archive/issue_events_021046.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-24T19:50:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5354",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5354#event-21046"
}
```



---

archive/issue_comments_043921.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-02-24T19:50:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5354#issuecomment-43921",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_043922.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.4.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-24T19:50:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5354#issuecomment-43922",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.4.alpha0.

Cheers,

Michael
