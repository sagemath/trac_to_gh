# Issue 5609: some functions for BooleanPolynomialIdeal

archive/issues_005609.json:
```json
{
    "assignees": [],
    "body": "Added `interreduced_basis()` and `__cmp__()` functions to `BooleanPolynomialIdeal`. \n\nAssignee: **@malb**\n\nCC:  PolyBoRi @burcin rpw\n\nKeywords: **polybori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5609_\n\n",
    "closed_at": "2009-05-12T21:50:10Z",
    "created_at": "2009-03-25T16:00:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "some functions for BooleanPolynomialIdeal",
    "type": "issue",
    "updated_at": "2009-05-12T21:50:10Z",
    "url": "https://github.com/sagemath/sage/issues/5609",
    "user": "https://github.com/malb"
}
```
Added `interreduced_basis()` and `__cmp__()` functions to `BooleanPolynomialIdeal`. 

Assignee: **@malb**

CC:  PolyBoRi @burcin rpw

Keywords: **polybori**

_Issue created by migration from https://trac.sagemath.org/ticket/5609_





---

archive/issue_events_059669.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-25T16:00:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59669"
}
```



---

archive/issue_events_059670.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-25T16:00:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59670"
}
```



---

archive/issue_events_059671.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-25T16:00:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59671"
}
```



---

archive/issue_events_059672.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-25T16:00:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59672"
}
```



---

archive/issue_events_059673.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-25T16:00:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59673"
}
```



---

archive/issue_comments_036706.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nthe attached patch depends on #5586 and #5576",
    "created_at": "2009-03-25T16:01:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5609#issuecomment-36706",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'>Comment 1:</a>
the attached patch depends on #5586 and #5576



---

archive/issue_comments_036707.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nJust a question after reading the patch: \n\nWhy do you implement both `__cmp__()` and `__richcmp__()` in `BooleanPolynomialIdeal`? The doctests for these two functions also seem to be the same.",
    "created_at": "2009-03-25T18:10:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5609#issuecomment-36707",
    "user": "https://github.com/burcin"
}
```

<a id='comment:2'>Comment 2:</a>
Just a question after reading the patch: 

Why do you implement both `__cmp__()` and `__richcmp__()` in `BooleanPolynomialIdeal`? The doctests for these two functions also seem to be the same.



---

archive/issue_comments_036708.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nIt is just because I am confused which one I have to implement (it seem Cython changed in that regard?)",
    "created_at": "2009-03-25T18:41:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5609#issuecomment-36708",
    "user": "https://github.com/malb"
}
```

<a id='comment:3'>Comment 3:</a>
It is just because I am confused which one I have to implement (it seem Cython changed in that regard?)



---

archive/issue_comments_036709.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI removed the `__richcmp__`, Burcin can you review this new patch?",
    "created_at": "2009-04-27T13:08:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5609#issuecomment-36709",
    "user": "https://github.com/malb"
}
```

<a id='comment:4'>Comment 4:</a>
I removed the `__richcmp__`, Burcin can you review this new patch?



---

archive/issue_comments_036710.json:
```json
{
    "body": "rebased to 3.4.2",
    "created_at": "2009-05-12T01:16:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5609#issuecomment-36710",
    "user": "https://github.com/malb"
}
```

rebased to 3.4.2



---

archive/issue_comments_036711.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nAttachment: **[pbori_interred.patch.gz](https://github.com/sagemath/sage/files/ticket5609/pbori_interred.patch.gz)**\n\nrebased the attached patch to 3.4.2",
    "created_at": "2009-05-12T01:16:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5609#issuecomment-36711",
    "user": "https://github.com/malb"
}
```

<a id='comment:5'>Comment 5:</a>
Attachment: **[pbori_interred.patch.gz](https://github.com/sagemath/sage/files/ticket5609/pbori_interred.patch.gz)**

rebased the attached patch to 3.4.2



---

archive/issue_comments_036712.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nLooks good to me.",
    "created_at": "2009-05-12T15:20:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5609#issuecomment-36712",
    "user": "https://github.com/burcin"
}
```

<a id='comment:6'>Comment 6:</a>
Looks good to me.



---

archive/issue_events_059674.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-05-12T15:20:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59674"
}
```



---

archive/issue_comments_036713.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThis patch completely moves the docstring of two `__init__` methods to the class. Is that the desired effect, i.e. why not add back minimal doctests? I understand that underscore and double underscore methods aren't in the manual, but this makes a difference for -coverage and ought to be fixed.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-12T21:47:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5609#issuecomment-36713",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:7'>Comment 7:</a>
This patch completely moves the docstring of two `__init__` methods to the class. Is that the desired effect, i.e. why not add back minimal doctests? I understand that underscore and double underscore methods aren't in the manual, but this makes a difference for -coverage and ought to be fixed.

Cheers,

Michael



---

archive/issue_events_059675.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T21:47:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59675"
}
```



---

archive/issue_events_059676.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T21:47:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59676"
}
```



---

archive/issue_comments_036714.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nMerged in Sage 4.0.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-12T21:50:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5609#issuecomment-36714",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:9'>Comment 9:</a>
Merged in Sage 4.0.alpha0.

Cheers,

Michael



---

archive/issue_events_059677.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T21:50:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59677"
}
```



---

archive/issue_events_059678.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T21:50:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5609",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5609#event-59678"
}
```
