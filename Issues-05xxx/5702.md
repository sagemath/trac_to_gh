# Issue 5702: make "make check" & friends doctest the documentation too

archive/issues_005702.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "Right now the various make check targets do not test the documentation. Fix that.\n\nPatch coming up.\n\nCheers,\n\nMichael\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5702_\n\n",
    "closed_at": "2009-04-16T10:20:11Z",
    "created_at": "2009-04-06T23:18:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make \"make check\" & friends doctest the documentation too",
    "type": "issue",
    "updated_at": "2009-04-16T10:20:11Z",
    "url": "https://github.com/sagemath/sage/issues/5702",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
Right now the various make check targets do not test the documentation. Fix that.

Patch coming up.

Cheers,

Michael

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/5702_





---

archive/issue_events_067655.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-06T23:18:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67655"
}
```



---

archive/issue_events_067656.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-06T23:18:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67656"
}
```



---

archive/issue_events_067657.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-06T23:18:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67657"
}
```



---

archive/issue_events_067658.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-06T23:18:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67658"
}
```



---

archive/issue_events_067659.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-06T23:18:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67659"
}
```



---

archive/issue_comments_036127.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nNote that this patch is a traditional patch since the file in question is not under version control.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-16T09:15:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5702#issuecomment-36127",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Note that this patch is a traditional patch since the file in question is not under version control.

Cheers,

Michael



---

archive/issue_events_067660.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-16T09:15:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67660"
}
```



---

archive/issue_comments_036128.json:
```json
{
    "body": "Attachment: **[trac_5702.patch.gz](https://github.com/sagemath/sage/files/ticket5702/trac_5702.patch.gz)**",
    "created_at": "2009-04-16T10:01:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5702#issuecomment-36128",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Attachment: **[trac_5702.patch.gz](https://github.com/sagemath/sage/files/ticket5702/trac_5702.patch.gz)**



---

archive/issue_events_067661.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-04-16T10:05:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67661"
}
```



---

archive/issue_events_067662.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-04-16T10:05:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67662"
}
```



---

archive/issue_comments_036129.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks good -- these are the same paths occurring in `sage/local/bin/sage-maketest`, which gives me a lot of confidence.",
    "created_at": "2009-04-16T10:05:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5702#issuecomment-36129",
    "user": "https://github.com/craigcitro"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks good -- these are the same paths occurring in `sage/local/bin/sage-maketest`, which gives me a lot of confidence.



---

archive/issue_events_067663.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-16T10:20:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67663"
}
```



---

archive/issue_events_067664.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-16T10:20:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5702#event-67664"
}
```



---

archive/issue_comments_036130.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.4.1.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-16T10:20:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5702#issuecomment-36130",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.4.1.rc3.

Cheers,

Michael
