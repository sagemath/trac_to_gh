# Issue 5702: [with patch, with positive review] make "make check" & friends doctest the documentation too

archive/issues_005702.json:
```json
{
    "body": "Right now the various make check targets do not test the documentation. Fix that.\n\nPatch coming up.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5702\n\n",
    "closed_at": "2009-04-16T10:20:11Z",
    "created_at": "2009-04-06T23:18:03Z",
    "labels": [
        "component: doctest coverage",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.1",
    "title": "[with patch, with positive review] make \"make check\" & friends doctest the documentation too",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5702",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Right now the various make check targets do not test the documentation. Fix that.

Patch coming up.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5702





---

archive/issue_comments_047743.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2009-04-06T23:18:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5702#issuecomment-47743",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_047744.json:
```json
{
    "body": "<a id='comment:2'></a>\nNote that this patch is a traditional patch since the file in question is not under version control.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-16T09:15:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5702#issuecomment-47744",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Note that this patch is a traditional patch since the file in question is not under version control.

Cheers,

Michael



---

archive/issue_events_022654.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5702",
    "rename": {
        "from": "make \"make check\" & friends doctest the documentation too",
        "to": "[with patch, needs review] make \"make check\" & friends doctest the documentation too"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5702#event-22654"
}
```



---

archive/issue_comments_047745.json:
```json
{
    "body": "Attachment [trac_5702.patch](tarball://root/attachments/some-uuid/ticket5702/trac_5702.patch) by mabshoff created at 2009-04-16 10:01:49",
    "created_at": "2009-04-16T10:01:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5702#issuecomment-47745",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [trac_5702.patch](tarball://root/attachments/some-uuid/ticket5702/trac_5702.patch) by mabshoff created at 2009-04-16 10:01:49



---

archive/issue_events_022655.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5702",
    "rename": {
        "from": "[with patch, needs review] make \"make check\" & friends doctest the documentation too",
        "to": "[with patch, with positive review] make \"make check\" & friends doctest the documentation too"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5702#event-22655"
}
```



---

archive/issue_comments_047746.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good -- these are the same paths occurring in `sage/local/bin/sage-maketest`, which gives me a lot of confidence.",
    "created_at": "2009-04-16T10:05:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5702#issuecomment-47746",
    "user": "https://github.com/craigcitro"
}
```

<a id='comment:3'></a>
Looks good -- these are the same paths occurring in `sage/local/bin/sage-maketest`, which gives me a lot of confidence.



---

archive/issue_comments_047747.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-04-16T10:20:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5702#issuecomment-47747",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_022656.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-16T10:20:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5702",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5702#event-22656"
}
```



---

archive/issue_comments_047748.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.4.1.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-16T10:20:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5702#issuecomment-47748",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.4.1.rc3.

Cheers,

Michael
