# Issue 5581: Face lattices and f-vectors for polytopes; bug fixes for polyhedra.py

archive/issues_005581.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mhampton"
    ],
    "body": "This patch adds the important functionality of computing face lattices and f-vectors of polytopes.  In the course of adding these, I found a number of bugs that occur for polyhedra of lower dimensions that are embedded in higher dimensions, which I believe I have fixed.\n\nKeywords: **polyhedra, face lattice, geometry**\n\nComponent: **geometry**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5581_\n\n",
    "closed_at": "2009-05-21T02:07:30Z",
    "created_at": "2009-03-21T17:12:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Face lattices and f-vectors for polytopes; bug fixes for polyhedra.py",
    "type": "issue",
    "updated_at": "2009-05-21T02:07:30Z",
    "url": "https://github.com/sagemath/sage/issues/5581",
    "user": "https://github.com/sagetrac-mhampton"
}
```
This patch adds the important functionality of computing face lattices and f-vectors of polytopes.  In the course of adding these, I found a number of bugs that occur for polyhedra of lower dimensions that are embedded in higher dimensions, which I believe I have fixed.

Keywords: **polyhedra, face lattice, geometry**

Component: **geometry**

_Issue created by migration from https://trac.sagemath.org/ticket/5581_





---

archive/issue_events_066093.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-03-21T17:12:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66093"
}
```



---

archive/issue_events_066094.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-03-21T17:12:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "c: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66094"
}
```



---

archive/issue_events_066095.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-03-21T17:12:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66095"
}
```



---

archive/issue_events_066096.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-03-21T17:12:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66096"
}
```



---

archive/issue_events_066097.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-03-21T17:12:27Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "subject": "https://github.com/sagetrac-mhampton",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66097"
}
```



---

archive/issue_comments_035141.json:
```json
{
    "body": "Attachment: **[trac_5581_face_lattice.patch.gz](https://github.com/sagemath/sage/files/ticket5581/trac_5581_face_lattice.patch.gz)**\n\nadds face lattices and f-vectors to polytopes",
    "created_at": "2009-03-21T17:15:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5581#issuecomment-35141",
    "user": "https://github.com/sagetrac-mhampton"
}
```

Attachment: **[trac_5581_face_lattice.patch.gz](https://github.com/sagemath/sage/files/ticket5581/trac_5581_face_lattice.patch.gz)**

adds face lattices and f-vectors to polytopes



---

archive/issue_events_066098.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-03-21T17:15:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66098"
}
```



---

archive/issue_comments_035142.json:
```json
{
    "body": "rebased against 3.4.2 and improved doctests.",
    "created_at": "2009-05-20T18:48:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5581#issuecomment-35142",
    "user": "https://github.com/sagetrac-mhampton"
}
```

rebased against 3.4.2 and improved doctests.



---

archive/issue_comments_035143.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[trac_5581_rebase.patch.gz](https://github.com/sagemath/sage/files/ticket5581/trac_5581_rebase.patch.gz)**\n\nI applied trac_5581_rebase.patch to Sage Version 4.0.alpha0, Release Date: 2009-05-15.  All doctests in sage/geometry passed, and I tried several other examples.  \n\nThis patch works with trac_4875_1.patch: the two work together regardless of the order in which they are applied.",
    "created_at": "2009-05-20T19:22:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5581#issuecomment-35143",
    "user": "https://github.com/sagetrac-dperkinson"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[trac_5581_rebase.patch.gz](https://github.com/sagemath/sage/files/ticket5581/trac_5581_rebase.patch.gz)**

I applied trac_5581_rebase.patch to Sage Version 4.0.alpha0, Release Date: 2009-05-15.  All doctests in sage/geometry passed, and I tried several other examples.  

This patch works with trac_4875_1.patch: the two work together regardless of the order in which they are applied.



---

archive/issue_events_066099.json:
```json
{
    "actor": "https://github.com/sagetrac-dperkinson",
    "created_at": "2009-05-20T19:22:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66099"
}
```



---

archive/issue_events_066100.json:
```json
{
    "actor": "https://github.com/sagetrac-dperkinson",
    "created_at": "2009-05-20T19:22:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66100"
}
```



---

archive/issue_events_066101.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-21T02:07:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66101"
}
```



---

archive/issue_events_066102.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-21T02:07:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66102"
}
```



---

archive/issue_events_066103.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-21T02:07:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66103"
}
```



---

archive/issue_events_066104.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-21T02:07:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5581#event-66104"
}
```



---

archive/issue_comments_035144.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged trac_5581_rebase.patch only in Sage 4.0.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-21T02:07:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5581",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5581#issuecomment-35144",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged trac_5581_rebase.patch only in Sage 4.0.rc0.

Cheers,

Michael
