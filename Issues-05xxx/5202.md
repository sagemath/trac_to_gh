# Issue 5202: Update MPFR to 2.4.0 (latest upstream)

archive/issues_005202.json:
```json
{
    "assignees": [],
    "body": "The spkg at\n\n http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha6/mpfr-2.4.0.spkg\n\nupdates the mpfr.spkg to latest upstream. For now the test suite is invoked automatically. It passes on\n\nSkyNet:\n\n* eno (x86_64, FC9)\n* mark (Sparc, Solaris)\n* fulvia (x86, Solaris)\n* cicero (x86, FC9)\n* menas (x86_64, OpenSUSE 10.3)\n* iras (Itanium, SLES 10)\n* cleo (Itanium, RHEL 5.2)\n* varro (PPC, OSX 10.4)\n\nMisc machines:\n\n* bsd (x86, OSX 10.5)\n* sage.math (x86_64, Ubuntu LTS 8.04)\n* sprocketer (x86-64, OSX 10.5)\n\nwith MPIR-0.9.rc3, which will be updated via a separate ticket.\n\nCheers,\n\nMichael\n\nAssignee: **mabshoff**\n\nCC:  @zimmermann6\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5202_\n\n",
    "closed_at": "2009-02-08T01:58:48Z",
    "created_at": "2009-02-08T01:08:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update MPFR to 2.4.0 (latest upstream)",
    "type": "issue",
    "updated_at": "2009-02-08T09:47:17Z",
    "url": "https://github.com/sagemath/sage/issues/5202",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
The spkg at

 http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha6/mpfr-2.4.0.spkg

updates the mpfr.spkg to latest upstream. For now the test suite is invoked automatically. It passes on

SkyNet:

* eno (x86_64, FC9)
* mark (Sparc, Solaris)
* fulvia (x86, Solaris)
* cicero (x86, FC9)
* menas (x86_64, OpenSUSE 10.3)
* iras (Itanium, SLES 10)
* cleo (Itanium, RHEL 5.2)
* varro (PPC, OSX 10.4)

Misc machines:

* bsd (x86, OSX 10.5)
* sage.math (x86_64, Ubuntu LTS 8.04)
* sprocketer (x86-64, OSX 10.5)

with MPIR-0.9.rc3, which will be updated via a separate ticket.

Cheers,

Michael

Assignee: **mabshoff**

CC:  @zimmermann6

_Issue created by migration from https://trac.sagemath.org/ticket/5202_





---

archive/issue_events_053073.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-53073"
}
```



---

archive/issue_events_053074.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-53074"
}
```



---

archive/issue_events_053075.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-53075"
}
```



---

archive/issue_events_053076.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-53076"
}
```



---

archive/issue_events_053077.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-53077"
}
```



---

archive/issue_comments_033096.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPaul,\n\nI can supply you with the config.guess output of all the machines above in case you want it for the MPFR website. I can also test on MIPS64/Linux.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-08T01:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5202#issuecomment-33096",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Paul,

I can supply you with the config.guess output of all the machines above in case you want it for the MPFR website. I can also test on MIPS64/Linux.

Cheers,

Michael



---

archive/issue_events_053078.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-08T01:48:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-53078"
}
```



---

archive/issue_comments_033097.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks good.  Passes all tests for me.",
    "created_at": "2009-02-08T01:48:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5202#issuecomment-33097",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'>Comment 3:</a>
Looks good.  Passes all tests for me.



---

archive/issue_events_053079.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:58:48Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "title_is": "Update MPFR to 2.4.0 (latest upstream)",
    "title_was": "update MPFR to 2.4.0 (latest upstream)",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-53079"
}
```



---

archive/issue_comments_033098.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMerged in Sage 3.3.alpha6.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-08T01:58:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5202#issuecomment-33098",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:4'>Comment 4:</a>
Merged in Sage 3.3.alpha6.

Cheers,

Michael



---

archive/issue_events_053080.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:58:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-53080"
}
```



---

archive/issue_events_053081.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:58:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-53081"
}
```



---

archive/issue_comments_033099.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\n> I can supply you with the config.guess output of all the machines above in case you want it for the MPFR website. I can also test on MIPS64/Linux.\n\nyes please supply it for configurations not already listed for mpfr-2.4.0.\n\nPaul",
    "created_at": "2009-02-08T09:47:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5202#issuecomment-33099",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:5'>Comment 5:</a>
> I can supply you with the config.guess output of all the machines above in case you want it for the MPFR website. I can also test on MIPS64/Linux.

yes please supply it for configurations not already listed for mpfr-2.4.0.

Paul
