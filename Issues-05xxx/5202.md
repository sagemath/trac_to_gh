# Issue 5202: Update MPFR to 2.4.0 (latest upstream)

archive/issues_005202.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "The spkg at\n\n http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha6/mpfr-2.4.0.spkg\n\nupdates the mpfr.spkg to latest upstream. For now the test suite is invoked automatically. It passes on\n\nSkyNet:\n\n* eno (x86_64, FC9)\n* mark (Sparc, Solaris)\n* fulvia (x86, Solaris)\n* cicero (x86, FC9)\n* menas (x86_64, OpenSUSE 10.3)\n* iras (Itanium, SLES 10)\n* cleo (Itanium, RHEL 5.2)\n* varro (PPC, OSX 10.4)\n\nMisc machines:\n\n* bsd (x86, OSX 10.5)\n* sage.math (x86_64, Ubuntu LTS 8.04)\n* sprocketer (x86-64, OSX 10.5)\n\nwith MPIR-0.9.rc3, which will be updated via a separate ticket.\n\nCheers,\n\nMichael\n\nCC:  @zimmermann6\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5202_\n\n",
    "closed_at": "2009-02-08T01:58:48Z",
    "created_at": "2009-02-08T01:08:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update MPFR to 2.4.0 (latest upstream)",
    "type": "issue",
    "updated_at": "2009-02-08T09:47:17Z",
    "url": "https://github.com/sagemath/sage/issues/5202",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
The spkg at

 http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha6/mpfr-2.4.0.spkg

updates the mpfr.spkg to latest upstream. For now the test suite is invoked automatically. It passes on

SkyNet:

* eno (x86_64, FC9)
* mark (Sparc, Solaris)
* fulvia (x86, Solaris)
* cicero (x86, FC9)
* menas (x86_64, OpenSUSE 10.3)
* iras (Itanium, SLES 10)
* cleo (Itanium, RHEL 5.2)
* varro (PPC, OSX 10.4)

Misc machines:

* bsd (x86, OSX 10.5)
* sage.math (x86_64, Ubuntu LTS 8.04)
* sprocketer (x86-64, OSX 10.5)

with MPIR-0.9.rc3, which will be updated via a separate ticket.

Cheers,

Michael

CC:  @zimmermann6

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/5202_





---

archive/issue_events_060970.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60970"
}
```



---

archive/issue_events_060971.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60971"
}
```



---

archive/issue_events_060972.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60972"
}
```



---

archive/issue_events_060973.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60973"
}
```



---

archive/issue_events_060974.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60974"
}
```



---

archive/issue_events_060975.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:08:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60975"
}
```



---

archive/issue_comments_031862.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPaul,\n\nI can supply you with the config.guess output of all the machines above in case you want it for the MPFR website. I can also test on MIPS64/Linux.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-08T01:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5202#issuecomment-31862",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Paul,

I can supply you with the config.guess output of all the machines above in case you want it for the MPFR website. I can also test on MIPS64/Linux.

Cheers,

Michael



---

archive/issue_events_060976.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-08T01:48:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60976"
}
```



---

archive/issue_comments_031863.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks good.  Passes all tests for me.",
    "created_at": "2009-02-08T01:48:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5202#issuecomment-31863",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks good.  Passes all tests for me.



---

archive/issue_events_060977.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:58:48Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "title_is": "Update MPFR to 2.4.0 (latest upstream)",
    "title_was": "update MPFR to 2.4.0 (latest upstream)",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60977"
}
```



---

archive/issue_comments_031864.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.3.alpha6.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-08T01:58:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5202#issuecomment-31864",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.3.alpha6.

Cheers,

Michael



---

archive/issue_events_060978.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:58:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60978"
}
```



---

archive/issue_events_060979.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:58:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5202#event-60979"
}
```



---

archive/issue_comments_031865.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\n> I can supply you with the config.guess output of all the machines above in case you want it for the MPFR website. I can also test on MIPS64/Linux.\n\nyes please supply it for configurations not already listed for mpfr-2.4.0.\n\nPaul",
    "created_at": "2009-02-08T09:47:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5202",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5202#issuecomment-31865",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:5" align="right">comment:5</div>

> I can supply you with the config.guess output of all the machines above in case you want it for the MPFR website. I can also test on MIPS64/Linux.

yes please supply it for configurations not already listed for mpfr-2.4.0.

Paul
