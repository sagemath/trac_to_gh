# Issue 5583: coercion bug in perm groups

archive/issues_005583.json:
```json
{
    "body": "```\nsage: PermutationGroup([(), (1,3)]).has_coerce_map_from( PermutationGroup([(), (1,2)]) )\nTrue\n```\n\nAssignee: joyner\n\nCC:  @tscrim\n\nBranch/Commit: 696b23b6240793a7ea068ddb1f1e8d284a40e2ad\n\nStopgaps: todo\n\nReviewer: Travis Scrimshaw\n\nAuthor: Fr\u00e9d\u00e9ric Chapoton\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5583\n\n",
    "closed_at": "2019-06-05T18:31:09Z",
    "created_at": "2009-03-22T11:36:05Z",
    "labels": [
        "component: group theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.8",
    "title": "coercion bug in perm groups",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5583",
    "user": "https://github.com/robertwb"
}
```
```
sage: PermutationGroup([(), (1,3)]).has_coerce_map_from( PermutationGroup([(), (1,2)]) )
True
```

Assignee: joyner

CC:  @tscrim

Branch/Commit: 696b23b6240793a7ea068ddb1f1e8d284a40e2ad

Stopgaps: todo

Reviewer: Travis Scrimshaw

Author: Frédéric Chapoton

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5583





---

archive/issue_events_013143.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13143"
}
```



---

archive/issue_events_013144.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13144"
}
```



---

archive/issue_events_013145.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13145"
}
```



---

archive/issue_events_013146.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13146"
}
```



---

archive/issue_events_013147.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13147"
}
```



---

archive/issue_events_013148.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13148"
}
```



---

archive/issue_events_013149.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13149"
}
```



---

archive/issue_comments_047595.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2015-08-19T23:12:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47595",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_047596.json:
```json
{
    "body": "Changing stopgaps from \"\" to \"todo\"",
    "created_at": "2015-08-19T23:12:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47596",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Changing stopgaps from "" to "todo"



---

archive/issue_comments_047597.json:
```json
{
    "body": "<a id='comment:6'></a>works in 8.8.b5:\n\n```\nsage: PermutationGroup([(), (1,3)]).has_coerce_map_from( PermutationGroup([(), (\n....: 1,2)]) )\n\nFalse\n```",
    "created_at": "2019-05-17T17:40:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47597",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'></a>works in 8.8.b5:

```
sage: PermutationGroup([(), (1,3)]).has_coerce_map_from( PermutationGroup([(), (
....: 1,2)]) )

False
```



---

archive/issue_events_013150.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-05-17T17:40:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13150"
}
```



---

archive/issue_events_013151.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-05-17T17:40:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13151"
}
```



---

archive/issue_comments_047598.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-05-17T17:40:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47598",
    "user": "https://github.com/fchapoton"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_047599.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-05-17T17:58:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47599",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_047600.json:
```json
{
    "body": "<a id='comment:7'></a>I agree. It works going back to Sage 8.3 (that's the oldest version I have installed), in both Python 2 and Python 3.",
    "created_at": "2019-05-17T17:58:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47600",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:7'></a>I agree. It works going back to Sage 8.3 (that's the oldest version I have installed), in both Python 2 and Python 3.



---

archive/issue_comments_047601.json:
```json
{
    "body": "Changing commit from \"\" to \"696b23b6240793a7ea068ddb1f1e8d284a40e2ad\"",
    "created_at": "2019-06-01T07:20:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47601",
    "user": "https://github.com/fchapoton"
}
```

Changing commit from "" to "696b23b6240793a7ea068ddb1f1e8d284a40e2ad"



---

archive/issue_comments_047602.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2019-06-01T07:20:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47602",
    "user": "https://github.com/fchapoton"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_047603.json:
```json
{
    "body": "Changing author from \"\" to \"Fr\u00e9d\u00e9ric Chapoton\"",
    "created_at": "2019-06-01T07:20:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47603",
    "user": "https://github.com/fchapoton"
}
```

Changing author from "" to "Frédéric Chapoton"



---

archive/issue_comments_047604.json:
```json
{
    "body": "<a id='comment:8'></a>I have decided to add a doctest, just to be safe.\n\n---\nNew commits:",
    "created_at": "2019-06-01T07:20:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47604",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'></a>I have decided to add a doctest, just to be safe.

---
New commits:



---

archive/issue_comments_047605.json:
```json
{
    "body": "Changing branch from \"\" to \"u/chapoton/5583\"",
    "created_at": "2019-06-01T07:20:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47605",
    "user": "https://github.com/fchapoton"
}
```

Changing branch from "" to "u/chapoton/5583"



---

archive/issue_events_013152.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-01T07:20:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13152"
}
```



---

archive/issue_events_013153.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-01T07:20:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13153"
}
```



---

archive/issue_comments_047606.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2019-06-01T07:20:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47606",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_047607.json:
```json
{
    "body": "<a id='comment:10'></a>green bot, please review this easy ticket",
    "created_at": "2019-06-01T10:25:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47607",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:10'></a>green bot, please review this easy ticket



---

archive/issue_comments_047608.json:
```json
{
    "body": "<a id='comment:11'></a>LGTM.",
    "created_at": "2019-06-01T23:37:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47608",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:11'></a>LGTM.



---

archive/issue_comments_047609.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-06-01T23:37:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47609",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_047610.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2019-06-01T23:37:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47610",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_047611.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-06-05T18:31:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47611",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_047612.json:
```json
{
    "body": "Changing branch from \"u/chapoton/5583\" to \"696b23b6240793a7ea068ddb1f1e8d284a40e2ad\"",
    "created_at": "2019-06-05T18:31:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5583#issuecomment-47612",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/chapoton/5583" to "696b23b6240793a7ea068ddb1f1e8d284a40e2ad"



---

archive/issue_events_013154.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-06-05T18:31:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5583",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5583#event-13154"
}
```
