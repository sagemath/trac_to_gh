# Issue 5064: Steenrod algebras are non-unique

archive/issues_005064.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: A = SteenrodAlgebra(17)\nsage: A(0).parent() is A\nFalse\n```\n\nCC:  @jhpalmieri\n\nComponent: **algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5064_\n\n",
    "closed_at": "2009-01-24T16:28:55Z",
    "created_at": "2009-01-23T02:16:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Steenrod algebras are non-unique",
    "type": "issue",
    "updated_at": "2009-01-24T16:28:55Z",
    "url": "https://github.com/sagemath/sage/issues/5064",
    "user": "https://github.com/boothby"
}
```

```
sage: A = SteenrodAlgebra(17)
sage: A(0).parent() is A
False
```

CC:  @jhpalmieri

Component: **algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/5064_





---

archive/issue_events_058996.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:16:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-58996"
}
```



---

archive/issue_events_058997.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:16:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-58997"
}
```



---

archive/issue_events_058998.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:16:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-58998"
}
```



---

archive/issue_events_058999.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:16:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-58999"
}
```



---

archive/issue_events_059000.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:16:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59000"
}
```



---

archive/issue_events_059001.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:16:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59001"
}
```



---

archive/issue_events_059002.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:16:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59002"
}
```



---

archive/issue_events_059003.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:16:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "c: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59003"
}
```



---

archive/issue_events_059004.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-01-23T02:16:45Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "subject": "https://github.com/boothby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59004"
}
```



---

archive/issue_events_059005.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:17:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "c: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59005"
}
```



---

archive/issue_events_059006.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-23T02:17:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59006"
}
```



---

archive/issue_events_059007.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-01-23T02:17:32Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "subject": "https://github.com/boothby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59007"
}
```



---

archive/issue_comments_030754.json:
```json
{
    "body": "Attachment: **[5064-unique-steenrod.patch.gz](https://github.com/sagemath/sage/files/ticket5064/5064-unique-steenrod.patch.gz)**",
    "created_at": "2009-01-24T10:09:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5064#issuecomment-30754",
    "user": "https://github.com/boothby"
}
```

Attachment: **[5064-unique-steenrod.patch.gz](https://github.com/sagemath/sage/files/ticket5064/5064-unique-steenrod.patch.gz)**



---

archive/issue_events_059008.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-24T10:12:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59008"
}
```



---

archive/issue_comments_030755.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\ntried the patch, it doesn't work for me.",
    "created_at": "2009-01-24T11:42:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5064#issuecomment-30755",
    "user": "https://github.com/shumow"
}
```

<div id="comment:4" align="right">comment:4</div>

tried the patch, it doesn't work for me.



---

archive/issue_events_059009.json:
```json
{
    "actor": "https://github.com/shumow",
    "created_at": "2009-01-24T11:42:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59009"
}
```



---

archive/issue_events_059010.json:
```json
{
    "actor": "https://github.com/shumow",
    "created_at": "2009-01-24T11:42:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59010"
}
```



---

archive/issue_comments_030756.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMy bad.  I screwed up the test.  It works for me now.\nI ran the tests and looked the code over, but I don't know enough about Steenrod Algebras to feel comfortable refereeing this.",
    "created_at": "2009-01-24T11:49:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5064#issuecomment-30756",
    "user": "https://github.com/shumow"
}
```

<div id="comment:5" align="right">comment:5</div>

My bad.  I screwed up the test.  It works for me now.
I ran the tests and looked the code over, but I don't know enough about Steenrod Algebras to feel comfortable refereeing this.



---

archive/issue_events_059011.json:
```json
{
    "actor": "https://github.com/shumow",
    "created_at": "2009-01-24T11:49:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59011"
}
```



---

archive/issue_events_059012.json:
```json
{
    "actor": "https://github.com/shumow",
    "created_at": "2009-01-24T11:49:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59012"
}
```



---

archive/issue_comments_030757.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nOn closer explanation from boothby, I feel I can give this a positive review.",
    "created_at": "2009-01-24T12:02:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5064#issuecomment-30757",
    "user": "https://github.com/shumow"
}
```

<div id="comment:6" align="right">comment:6</div>

On closer explanation from boothby, I feel I can give this a positive review.



---

archive/issue_events_059013.json:
```json
{
    "actor": "https://github.com/shumow",
    "created_at": "2009-01-24T12:02:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59013"
}
```



---

archive/issue_events_059014.json:
```json
{
    "actor": "https://github.com/shumow",
    "created_at": "2009-01-24T12:02:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59014"
}
```



---

archive/issue_events_059015.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T16:28:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59015"
}
```



---

archive/issue_events_059016.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T16:28:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59016"
}
```



---

archive/issue_comments_030758.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nMerged in Sage 3.3.alpha2",
    "created_at": "2009-01-24T16:28:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5064#issuecomment-30758",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:7" align="right">comment:7</div>

Merged in Sage 3.3.alpha2



---

archive/issue_events_059017.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T16:28:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59017"
}
```



---

archive/issue_events_059018.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T16:28:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5064",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5064#event-59018"
}
```
