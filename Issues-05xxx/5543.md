# Issue 5543: RealIntervalField parents are not unique

archive/issues_005543.json:
```json
{
    "assignees": [],
    "body": "Note that the attached patch has an apparently-spurious chunk that adds a single space to rings/polynomial/real_roots.pyx.  This is to force a recompilation for that file (to work around a bug in the dependency tracker); otherwise you end up with a broken Sage because the real_roots module won't load.  (I'll report the dependency tracker bug as a separate ticket in a minute.)\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5543_\n\n",
    "closed_at": "2009-04-13T02:39:34Z",
    "created_at": "2009-03-17T06:14:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "RealIntervalField parents are not unique",
    "type": "issue",
    "updated_at": "2009-04-13T02:39:34Z",
    "url": "https://github.com/sagemath/sage/issues/5543",
    "user": "https://github.com/sagetrac-cwitty"
}
```
Note that the attached patch has an apparently-spurious chunk that adds a single space to rings/polynomial/real_roots.pyx.  This is to force a recompilation for that file (to work around a bug in the dependency tracker); otherwise you end up with a broken Sage because the real_roots module won't load.  (I'll report the dependency tracker bug as a separate ticket in a minute.)

Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/5543_





---

archive/issue_events_065554.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-03-17T06:14:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65554"
}
```



---

archive/issue_events_065555.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-03-17T06:14:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65555"
}
```



---

archive/issue_events_065556.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-03-17T06:14:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65556"
}
```



---

archive/issue_events_065557.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-03-17T06:14:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65557"
}
```



---

archive/issue_events_065558.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-03-17T06:14:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65558"
}
```



---

archive/issue_comments_034773.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[rif-unique-parents.patch.gz](https://github.com/sagemath/sage/files/ticket5543/rif-unique-parents.patch.gz)**",
    "created_at": "2009-04-12T04:20:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5543#issuecomment-34773",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[rif-unique-parents.patch.gz](https://github.com/sagemath/sage/files/ticket5543/rif-unique-parents.patch.gz)**



---

archive/issue_events_065559.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-12T04:20:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65559"
}
```



---

archive/issue_events_065560.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T02:39:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65560"
}
```



---

archive/issue_events_065561.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T02:39:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65561"
}
```



---

archive/issue_comments_034774.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.4.1.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-13T02:39:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5543#issuecomment-34774",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.4.1.rc3.

Cheers,

Michael



---

archive/issue_events_065562.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T02:39:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65562"
}
```



---

archive/issue_events_065563.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T02:39:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5543",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5543#event-65563"
}
```
