# Issue 5350: sage-clone should use hard links for the build directory

archive/issues_005350.json:
```json
{
    "assignees": [],
    "body": "We copy the build directory when we're cloning the tree. This wastes disk space, and makes switching between branches slow, since new files need to be loaded from disk while the previous ones might already be in the cache.\n\nAttached patch to the scripts repository changes the sage-clone script to hard link the build directory. On my laptop this saves >450 MB per clone.\n\n```\nburcin@karr ~/sage/sage-3.3/devel $ du -sh sage-*\n593M    sage-hl\n125M    sage-hl1\n557M    sage-main\n```\n\nAlso the time to clone on my laptop goes from:\n\n```\nreal    0m14.709s\nuser    0m4.640s\nsys     0m1.924s\n```\nto\n\n```\nreal    0m6.100s\nuser    0m4.712s\nsys     0m0.928s\n```\nabout 2.8 seconds of which is spent in the sage -b step.\n\nUnfortunately, hard linking the .c, .cpp, and .h files doesn't work. This might be a problem with how cython handles its output when the output file is already present. This would save another ~100MB if it works.\n\nAssignee: **@burcin**\n\nCC:  cwitty @robertwb @hemmecke\n\nReviewer: **Robert Miller**\n\nAuthor: **Burcin Erocal**\n\nMerged: **sage-4.1.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5350_\n\n",
    "closed_at": "2009-06-26T17:45:24Z",
    "created_at": "2009-02-23T18:53:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage-clone should use hard links for the build directory",
    "type": "issue",
    "updated_at": "2023-01-14T14:13:31Z",
    "url": "https://github.com/sagemath/sage/issues/5350",
    "user": "https://github.com/burcin"
}
```
We copy the build directory when we're cloning the tree. This wastes disk space, and makes switching between branches slow, since new files need to be loaded from disk while the previous ones might already be in the cache.

Attached patch to the scripts repository changes the sage-clone script to hard link the build directory. On my laptop this saves >450 MB per clone.

```
burcin@karr ~/sage/sage-3.3/devel $ du -sh sage-*
593M    sage-hl
125M    sage-hl1
557M    sage-main
```

Also the time to clone on my laptop goes from:

```
real    0m14.709s
user    0m4.640s
sys     0m1.924s
```
to

```
real    0m6.100s
user    0m4.712s
sys     0m0.928s
```
about 2.8 seconds of which is spent in the sage -b step.

Unfortunately, hard linking the .c, .cpp, and .h files doesn't work. This might be a problem with how cython handles its output when the output file is already present. This would save another ~100MB if it works.

Assignee: **@burcin**

CC:  cwitty @robertwb @hemmecke

Reviewer: **Robert Miller**

Author: **Burcin Erocal**

Merged: **sage-4.1.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/5350_





---

archive/issue_events_056348.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-23T18:53:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56348"
}
```



---

archive/issue_events_056349.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-23T18:53:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56349"
}
```



---

archive/issue_events_056350.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-23T18:53:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56350"
}
```



---

archive/issue_events_056351.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-23T18:53:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56351"
}
```



---

archive/issue_comments_034267.json:
```json
{
    "body": "Attachment: **[trac_5350-clone_hardlink.patch.gz](https://github.com/sagemath/sage/files/ticket5350/trac_5350-clone_hardlink.patch.gz)**\n\nmake sage-clone use hard links",
    "created_at": "2009-02-23T18:54:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34267",
    "user": "https://github.com/burcin"
}
```

Attachment: **[trac_5350-clone_hardlink.patch.gz](https://github.com/sagemath/sage/files/ticket5350/trac_5350-clone_hardlink.patch.gz)**

make sage-clone use hard links



---

archive/issue_comments_034268.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI don't think we want this in a critical stable release :)\n\nCheers,\n\nMichael",
    "created_at": "2009-02-23T19:00:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34268",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
I don't think we want this in a critical stable release :)

Cheers,

Michael



---

archive/issue_events_056352.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-23T19:00:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56352"
}
```



---

archive/issue_events_056353.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-23T19:00:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56353"
}
```



---

archive/issue_comments_034269.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nCython 0.11 will make hard links on .c and .cpp files work too. \n\nhttp://trac.cython.org/cython_trac/ticket/220 and #4987",
    "created_at": "2009-03-18T04:54:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34269",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:2'>Comment 2:</a>
Cython 0.11 will make hard links on .c and .cpp files work too. 

http://trac.cython.org/cython_trac/ticket/220 and #4987



---

archive/issue_comments_034270.json:
```json
{
    "body": "rebased for 4.0.1",
    "created_at": "2009-06-07T12:19:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34270",
    "user": "https://github.com/burcin"
}
```

rebased for 4.0.1



---

archive/issue_comments_034271.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAttachment: **[trac_5350-clone_hardlink-take2.patch.gz](https://github.com/sagemath/sage/files/ticket5350/trac_5350-clone_hardlink-take2.patch.gz)**\n\n[attachment: trac_5350-clone_hardlink-take2.patch.gz](https://github.com/sagemath/sage/files/ticket5350/trac_5350-clone_hardlink-take2.patch.gz) is a new version of the patch. It is rebased to 4.0.1. We now use hard links for autogenerated cython files and documentation output too.",
    "created_at": "2009-06-07T12:24:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34271",
    "user": "https://github.com/burcin"
}
```

<a id='comment:3'>Comment 3:</a>
Attachment: **[trac_5350-clone_hardlink-take2.patch.gz](https://github.com/sagemath/sage/files/ticket5350/trac_5350-clone_hardlink-take2.patch.gz)**

[attachment: trac_5350-clone_hardlink-take2.patch.gz](https://github.com/sagemath/sage/files/ticket5350/trac_5350-clone_hardlink-take2.patch.gz) is a new version of the patch. It is rebased to 4.0.1. We now use hard links for autogenerated cython files and documentation output too.



---

archive/issue_events_056354.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-21T22:35:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56354"
}
```



---

archive/issue_comments_034272.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nEverything works fine, and the speedup is impressive, but I agree that a two-digit release might not be appropriate for something so invasive.",
    "created_at": "2009-06-21T22:35:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34272",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:4'>Comment 4:</a>
Everything works fine, and the speedup is impressive, but I agree that a two-digit release might not be appropriate for something so invasive.



---

archive/issue_events_056355.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-21T22:35:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56355"
}
```



---

archive/issue_events_056356.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-21T22:35:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56356"
}
```



---

archive/issue_comments_034273.json:
```json
{
    "body": "Merged: **sage-4.1.alpha2**",
    "created_at": "2009-06-26T17:45:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34273",
    "user": "https://github.com/sagetrac-boothby"
}
```

Merged: **sage-4.1.alpha2**



---

archive/issue_events_056357.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-06-26T17:45:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56357"
}
```



---

archive/issue_events_056358.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-06-26T17:45:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56358"
}
```



---

archive/issue_events_056359.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-26T20:06:37Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56359"
}
```



---

archive/issue_events_056360.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-26T20:06:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5350#event-56360"
}
```



---

archive/issue_comments_034274.json:
```json
{
    "body": "Author: **Burcin Erocal**",
    "created_at": "2009-06-26T20:06:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34274",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Burcin Erocal**



---

archive/issue_comments_034275.json:
```json
{
    "body": "Reviewer: **Robert Miller**",
    "created_at": "2009-06-26T20:06:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34275",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Robert Miller**



---

archive/issue_comments_034276.json:
```json
{
    "body": "Changed merged from **sage-4.1.alpha2** to **sage-4.1.alpha3**",
    "created_at": "2009-06-29T21:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5350#issuecomment-34276",
    "user": "https://github.com/rlmill"
}
```

Changed merged from **sage-4.1.alpha2** to **sage-4.1.alpha3**
