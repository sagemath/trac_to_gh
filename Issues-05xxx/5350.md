# Issue 5350: [with patch, positive review] sage-clone should use hard links for the build directory

archive/issues_005350.json:
```json
{
    "body": "We copy the build directory when we're cloning the tree. This wastes disk space, and makes switching between branches slow, since new files need to be loaded from disk while the previous ones might already be in the cache.\n\nAttached patch to the scripts repository changes the sage-clone script to hard link the build directory. On my laptop this saves >450 MB per clone.\n\n```\nburcin@karr ~/sage/sage-3.3/devel $ du -sh sage-*\n593M    sage-hl\n125M    sage-hl1\n557M    sage-main\n```\n\nAlso the time to clone on my laptop goes from:\n\n```\nreal    0m14.709s\nuser    0m4.640s\nsys     0m1.924s\n```\nto\n\n```\nreal    0m6.100s\nuser    0m4.712s\nsys     0m0.928s\n```\nabout 2.8 seconds of which is spent in the sage -b step.\n\nUnfortunately, hard linking the .c, .cpp, and .h files doesn't work. This might be a problem with how cython handles its output when the output file is already present. This would save another ~100MB if it works.\n\nAssignee: @burcin\n\nCC:  cwitty @robertwb ralf@hemmecke.de\n\nResolution: fixed\n\nAuthor: Burcin Erocal\n\nReviewer: Robert Miller\n\nMerged: sage-4.1.alpha3\n\nIssue created by migration from https://trac.sagemath.org/ticket/5350\n\n",
    "closed_at": "2009-06-26T17:45:24Z",
    "created_at": "2009-02-23T18:53:19Z",
    "labels": [
        "component: misc"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1",
    "title": "[with patch, positive review] sage-clone should use hard links for the build directory",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5350",
    "user": "https://github.com/burcin"
}
```
We copy the build directory when we're cloning the tree. This wastes disk space, and makes switching between branches slow, since new files need to be loaded from disk while the previous ones might already be in the cache.

Attached patch to the scripts repository changes the sage-clone script to hard link the build directory. On my laptop this saves >450 MB per clone.

```
burcin@karr ~/sage/sage-3.3/devel $ du -sh sage-*
593M    sage-hl
125M    sage-hl1
557M    sage-main
```

Also the time to clone on my laptop goes from:

```
real    0m14.709s
user    0m4.640s
sys     0m1.924s
```
to

```
real    0m6.100s
user    0m4.712s
sys     0m0.928s
```
about 2.8 seconds of which is spent in the sage -b step.

Unfortunately, hard linking the .c, .cpp, and .h files doesn't work. This might be a problem with how cython handles its output when the output file is already present. This would save another ~100MB if it works.

Assignee: @burcin

CC:  cwitty @robertwb ralf@hemmecke.de

Resolution: fixed

Author: Burcin Erocal

Reviewer: Robert Miller

Merged: sage-4.1.alpha3

Issue created by migration from https://trac.sagemath.org/ticket/5350





---

archive/issue_comments_043872.json:
```json
{
    "body": "make sage-clone use hard links",
    "created_at": "2009-02-23T18:54:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43872",
    "user": "https://github.com/burcin"
}
```

make sage-clone use hard links



---

archive/issue_events_021029.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-23T19:00:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "milestone": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5350#event-21029"
}
```



---

archive/issue_comments_043873.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_5350-clone_hardlink.patch](tarball://root/attachments/some-uuid/ticket5350/trac_5350-clone_hardlink.patch) by mabshoff created at 2009-02-23 19:00:38\n\nI don't think we want this in a critical stable release :)\n\nCheers,\n\nMichael",
    "created_at": "2009-02-23T19:00:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43873",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Attachment [trac_5350-clone_hardlink.patch](tarball://root/attachments/some-uuid/ticket5350/trac_5350-clone_hardlink.patch) by mabshoff created at 2009-02-23 19:00:38

I don't think we want this in a critical stable release :)

Cheers,

Michael



---

archive/issue_comments_043874.json:
```json
{
    "body": "<a id='comment:2'></a>\nCython 0.11 will make hard links on .c and .cpp files work too. \n\nhttp://trac.cython.org/cython_trac/ticket/220 and #4987",
    "created_at": "2009-03-18T04:54:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43874",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:2'></a>
Cython 0.11 will make hard links on .c and .cpp files work too. 

http://trac.cython.org/cython_trac/ticket/220 and #4987



---

archive/issue_comments_043875.json:
```json
{
    "body": "rebased for 4.0.1",
    "created_at": "2009-06-07T12:19:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43875",
    "user": "https://github.com/burcin"
}
```

rebased for 4.0.1



---

archive/issue_comments_043876.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2009-06-07T12:24:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43876",
    "user": "https://github.com/burcin"
}
```

Changing status from new to assigned.



---

archive/issue_comments_043877.json:
```json
{
    "body": "<a id='comment:3'></a>\nAttachment [trac_5350-clone_hardlink-take2.patch](tarball://root/attachments/some-uuid/ticket5350/trac_5350-clone_hardlink-take2.patch) by @burcin created at 2009-06-07 12:24:02\n\nattachment:trac_5350-clone_hardlink-take2.patch is a new version of the patch. It is rebased to 4.0.1. We now use hard links for autogenerated cython files and documentation output too.",
    "created_at": "2009-06-07T12:24:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43877",
    "user": "https://github.com/burcin"
}
```

<a id='comment:3'></a>
Attachment [trac_5350-clone_hardlink-take2.patch](tarball://root/attachments/some-uuid/ticket5350/trac_5350-clone_hardlink-take2.patch) by @burcin created at 2009-06-07 12:24:02

attachment:trac_5350-clone_hardlink-take2.patch is a new version of the patch. It is rebased to 4.0.1. We now use hard links for autogenerated cython files and documentation output too.



---

archive/issue_events_021030.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "rename": {
        "from": "[with patch, needs review] sage-clone should use hard links for the build directory",
        "to": "[with patch, positive review] sage-clone should use hard links for the build directory"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5350#event-21030"
}
```



---

archive/issue_comments_043878.json:
```json
{
    "body": "<a id='comment:4'></a>\nEverything works fine, and the speedup is impressive, but I agree that a two-digit release might not be appropriate for something so invasive.",
    "created_at": "2009-06-21T22:35:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43878",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:4'></a>
Everything works fine, and the speedup is impressive, but I agree that a two-digit release might not be appropriate for something so invasive.



---

archive/issue_events_021031.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-21T22:35:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "milestone": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5350#event-21031"
}
```



---

archive/issue_events_021032.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-21T22:35:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "milestone": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5350#event-21032"
}
```



---

archive/issue_comments_043879.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.1.alpha2\"",
    "created_at": "2009-06-26T17:45:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43879",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

Changing merged from "" to "sage-4.1.alpha2"



---

archive/issue_comments_043880.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-26T17:45:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43880",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

Resolution: fixed



---

archive/issue_events_021033.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2009-06-26T17:45:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5350#event-21033"
}
```



---

archive/issue_events_021034.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-06-26T20:06:37Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "milestone": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5350#event-21034"
}
```



---

archive/issue_events_021035.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-06-26T20:06:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "milestone": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5350#event-21035"
}
```



---

archive/issue_comments_043881.json:
```json
{
    "body": "Changing author from \"\" to \"Burcin Erocal\"",
    "created_at": "2009-06-26T20:06:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43881",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing author from "" to "Burcin Erocal"



---

archive/issue_comments_043882.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Robert Miller\"",
    "created_at": "2009-06-26T20:06:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43882",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing reviewer from "" to "Robert Miller"



---

archive/issue_comments_043883.json:
```json
{
    "body": "Changing merged from \"sage-4.1.alpha2\" to \"sage-4.1.alpha3\"",
    "created_at": "2009-06-29T21:00:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5350#issuecomment-43883",
    "user": "https://github.com/rlmill"
}
```

Changing merged from "sage-4.1.alpha2" to "sage-4.1.alpha3"
