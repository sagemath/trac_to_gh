# Issue 5935: graph theory docs -- sage: graphs_query.[tab]             # not tested

archive/issues_005935.json:
```json
{
    "assignees": [],
    "body": "Right at the top overview of graph theory the ref manual has the line\n\n```\nsage: graphs_query.[tab]             # not tested\n```\n\nhttp://www.sagemath.org/doc/reference/sage/graphs/graph.html\n\nIf it isn't tested it's broken, and that's the case here, since there is no graphs_query object available on the Sage command line.\n\nAssignee: **ekirkman**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5935_\n\n",
    "closed_at": "2009-07-08T20:30:52Z",
    "created_at": "2009-04-29T15:49:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "graph theory docs -- sage: graphs_query.[tab]             # not tested",
    "type": "issue",
    "updated_at": "2009-07-08T20:30:52Z",
    "url": "https://github.com/sagemath/sage/issues/5935",
    "user": "https://github.com/williamstein"
}
```
Right at the top overview of graph theory the ref manual has the line

```
sage: graphs_query.[tab]             # not tested
```

http://www.sagemath.org/doc/reference/sage/graphs/graph.html

If it isn't tested it's broken, and that's the case here, since there is no graphs_query object available on the Sage command line.

Assignee: **ekirkman**

_Issue created by migration from https://trac.sagemath.org/ticket/5935_





---

archive/issue_events_062036.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-29T15:49:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-62036"
}
```



---

archive/issue_events_062037.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-29T15:49:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-62037"
}
```



---

archive/issue_events_062038.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-29T15:49:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-62038"
}
```



---

archive/issue_comments_039759.json:
```json
{
    "body": "Changed assignee from **@rlmill** to **ekirkman**",
    "created_at": "2009-05-21T21:55:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5935#issuecomment-39759",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

Changed assignee from **@rlmill** to **ekirkman**



---

archive/issue_comments_039760.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nMarked as duplicate because this documentation change is covered in the patch for ticket #5936.",
    "created_at": "2009-05-21T21:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5935#issuecomment-39760",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

<a id='comment:2'>Comment 2:</a>
Marked as duplicate because this documentation change is covered in the patch for ticket #5936.



---

archive/issue_events_062039.json:
```json
{
    "actor": "https://github.com/sagetrac-ekirkman",
    "created_at": "2009-05-21T21:57:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-62039"
}
```



---

archive/issue_events_062040.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-08T20:30:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-62040"
}
```



---

archive/issue_events_062041.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-08T20:30:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-62041"
}
```
