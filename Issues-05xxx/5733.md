# Issue 5733: bug in 3d plotting of graphs

archive/issues_005733.json:
```json
{
    "assignees": [
        "https://github.com/rlmill"
    ],
    "body": "\n```\nsage: G=Graph({'a':['a','b','b','b','e'],'b':['c','d','e'],'c':\nsage: ['c','d','d','d'],'d':['e']})\nsage: G.show3d()\nTraceback (most recent call last):\n...\nZeroDivisionError: float division\n```\n\nReported by alec@mihailovs\n\nComponent: **graph theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5733_\n\n",
    "closed_at": "2009-04-13T06:23:19Z",
    "created_at": "2009-04-10T14:19:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bug in 3d plotting of graphs",
    "type": "issue",
    "updated_at": "2009-04-13T06:23:19Z",
    "url": "https://github.com/sagemath/sage/issues/5733",
    "user": "https://github.com/williamstein"
}
```

```
sage: G=Graph({'a':['a','b','b','b','e'],'b':['c','d','e'],'c':
sage: ['c','d','d','d'],'d':['e']})
sage: G.show3d()
Traceback (most recent call last):
...
ZeroDivisionError: float division
```

Reported by alec@mihailovs

Component: **graph theory**

_Issue created by migration from https://trac.sagemath.org/ticket/5733_





---

archive/issue_events_068030.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-10T14:19:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68030"
}
```



---

archive/issue_events_068031.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-10T14:19:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68031"
}
```



---

archive/issue_events_068032.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-10T14:19:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68032"
}
```



---

archive/issue_events_068033.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-10T14:19:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68033"
}
```



---

archive/issue_events_068034.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-04-10T14:19:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68034"
}
```



---

archive/issue_comments_036346.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nApparently show3d() chokes on loops (that's the error: I think it's trying to make a cylinder (edge) with length 0).  Also, show3d doesn't show multiple edges.",
    "created_at": "2009-04-10T14:38:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5733#issuecomment-36346",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:1" align="right">comment:1</div>

Apparently show3d() chokes on loops (that's the error: I think it's trying to make a cylinder (edge) with length 0).  Also, show3d doesn't show multiple edges.



---

archive/issue_comments_036347.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis needs to be implemented, but until then it needs to fail more gracefully. Thus the proposed patch. If implementing this isn't a ticket yet, it probably should be.",
    "created_at": "2009-04-10T15:32:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5733#issuecomment-36347",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:2" align="right">comment:2</div>

This needs to be implemented, but until then it needs to fail more gracefully. Thus the proposed patch. If implementing this isn't a ticket yet, it probably should be.



---

archive/issue_events_068035.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-04-10T15:32:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68035"
}
```



---

archive/issue_comments_036348.json:
```json
{
    "body": "Attachment: **[trac_5733.patch.gz](https://github.com/sagemath/sage/files/ticket5733/trac_5733.patch.gz)**",
    "created_at": "2009-04-10T15:32:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5733#issuecomment-36348",
    "user": "https://github.com/rlmill"
}
```

Attachment: **[trac_5733.patch.gz](https://github.com/sagemath/sage/files/ticket5733/trac_5733.patch.gz)**



---

archive/issue_events_068036.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-12T06:30:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68036"
}
```



---

archive/issue_events_068037.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-12T06:30:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68037"
}
```



---

archive/issue_events_068038.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T06:23:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68038"
}
```



---

archive/issue_events_068039.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T06:23:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68039"
}
```



---

archive/issue_comments_036349.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.4.1.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-13T06:23:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5733#issuecomment-36349",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.4.1.rc3.

Cheers,

Michael



---

archive/issue_events_068040.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T06:23:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68040"
}
```



---

archive/issue_events_068041.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T06:23:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5733",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5733#event-68041"
}
```
