# Issue 5229: Jmol axes in the wrong place

archive/issues_005229.json:
```json
{
    "body": "As is, it appears that 3d plots are wrong, according to the jmol axes, because the jmol axes apparently don't default to being centered at the origin.  This patch makes the jmol axes centered at the origin.  To show the jmol axes, you still need to right-click on the jmol plot, select Style, then Axes.\n\nAssignee: @williamstein\n\nStatus: needs_work\n\nIssue created by migration from https://trac.sagemath.org/ticket/5229\n\n",
    "created_at": "2009-02-10T21:18:01Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Jmol axes in the wrong place",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5229",
    "user": "https://github.com/jasongrout"
}
```
As is, it appears that 3d plots are wrong, according to the jmol axes, because the jmol axes apparently don't default to being centered at the origin.  This patch makes the jmol axes centered at the origin.  To show the jmol axes, you still need to right-click on the jmol plot, select Style, then Axes.

Assignee: @williamstein

Status: needs_work

Issue created by migration from https://trac.sagemath.org/ticket/5229





---

archive/issue_comments_042607.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_5229-jmol-axes.patch](tarball://root/attachments/some-uuid/ticket5229/trac_5229-jmol-axes.patch) by @ncalexan created at 2009-02-10 21:49:16\n\n\n```\n[1:28pm] ncalexan: jason|log: either it's not working or I don't get it.\n[1:28pm] ncalexan: plot3d(lambda x, y: x^2 + y^2, (-2,2), (-2,2))\n[1:28pm] ncalexan: gives me a smooth cup-like surface.\n[1:28pm] ncalexan: But the value at x=0, y=0 (which is z=0) does not coincide with the jmol axes.\n```",
    "created_at": "2009-02-10T21:49:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5229#issuecomment-42607",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:1'></a>
Attachment [trac_5229-jmol-axes.patch](tarball://root/attachments/some-uuid/ticket5229/trac_5229-jmol-axes.patch) by @ncalexan created at 2009-02-10 21:49:16


```
[1:28pm] ncalexan: jason|log: either it's not working or I don't get it.
[1:28pm] ncalexan: plot3d(lambda x, y: x^2 + y^2, (-2,2), (-2,2))
[1:28pm] ncalexan: gives me a smooth cup-like surface.
[1:28pm] ncalexan: But the value at x=0, y=0 (which is z=0) does not coincide with the jmol axes.
```



---

archive/issue_comments_042608.json:
```json
{
    "body": "<a id='comment:2'></a>\nAfter further IRC discussion, the proposed fix does not work in all situations.",
    "created_at": "2009-02-10T22:05:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5229#issuecomment-42608",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:2'></a>
After further IRC discussion, the proposed fix does not work in all situations.



---

archive/issue_events_020442.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "rename": {
        "from": "[with patch, needs review] Jmol axes in the wrong place",
        "to": "[with patch, negative review] Jmol axes in the wrong place"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20442"
}
```



---

archive/issue_events_020443.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "rename": {
        "from": "[with patch, negative review] Jmol axes in the wrong place",
        "to": "[with patch, needs work] Jmol axes in the wrong place"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20443"
}
```



---

archive/issue_comments_042609.json:
```json
{
    "body": "<a id='comment:3'></a>\nWe don't give negative reviews any more, so change it to \"needs work\".\n\nCheers,\n\nMichael",
    "created_at": "2009-02-11T02:21:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5229#issuecomment-42609",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
We don't give negative reviews any more, so change it to "needs work".

Cheers,

Michael



---

archive/issue_comments_042610.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2010-01-17T08:37:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5229#issuecomment-42610",
    "user": "https://github.com/jasongrout"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_042611.json:
```json
{
    "body": "<a id='comment:4'></a>\nSee #3862",
    "created_at": "2010-01-17T08:37:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5229#issuecomment-42611",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'></a>
See #3862



---

archive/issue_comments_042612.json:
```json
{
    "body": "<a id='comment:5'></a>\nAlso, and perhaps that merits a new ticket, the orientation of the axis is not what one would expect.\n\nNormally 3d plots are shown with the positive x axis to the viewer's left, the positive y axis to the right, and the positive z upwards. Currently it is: negative y axis to the left, positive x to the right, and positive z upwards.",
    "created_at": "2010-05-29T21:40:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5229#issuecomment-42612",
    "user": "https://trac.sagemath.org/admin/accounts/users/olazo"
}
```

<a id='comment:5'></a>
Also, and perhaps that merits a new ticket, the orientation of the axis is not what one would expect.

Normally 3d plots are shown with the positive x axis to the viewer's left, the positive y axis to the right, and the positive z upwards. Currently it is: negative y axis to the left, positive x to the right, and positive z upwards.



---

archive/issue_events_020444.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "rename": {
        "from": "[with patch, needs work] Jmol axes in the wrong place",
        "to": "Jmol axes in the wrong place"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20444"
}
```



---

archive/issue_events_020445.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20445"
}
```



---

archive/issue_events_020446.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20446"
}
```



---

archive/issue_events_020447.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20447"
}
```



---

archive/issue_events_020448.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20448"
}
```



---

archive/issue_events_020449.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20449"
}
```



---

archive/issue_events_020450.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20450"
}
```



---

archive/issue_events_020451.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5229",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5229#event-20451"
}
```
