# Issue 5444: [with patch, positive review] ellipses + float = boom

archive/issues_005444.json:
```json
{
    "body": "```\n   sage: [(1.0)..(2.0)]\n   [1.00000000000000, 2.00000000000000]\n   sage: [1.0..2.0]\n    line 4\n    (ellipsis_range(_sage_const_1p0 ,Ellipsis,_sage_const_2 RealNumber('.0')))\n                                                                     ^\nSyntaxError: invalid syntax\n```\n\nAssignee: @robertwb\n\nResolution: fixed\n\nAuthor: Mike Hansen\n\nReviewer: Robert Bradshaw\n\nMerged: 4.0.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/5444\n\n",
    "closed_at": "2009-06-13T21:50:49Z",
    "created_at": "2009-03-05T19:58:26Z",
    "labels": [
        "component: user interface",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0.2",
    "title": "[with patch, positive review] ellipses + float = boom",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5444",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```
```
   sage: [(1.0)..(2.0)]
   [1.00000000000000, 2.00000000000000]
   sage: [1.0..2.0]
    line 4
    (ellipsis_range(_sage_const_1p0 ,Ellipsis,_sage_const_2 RealNumber('.0')))
                                                                     ^
SyntaxError: invalid syntax
```

Assignee: @robertwb

Resolution: fixed

Author: Mike Hansen

Reviewer: Robert Bradshaw

Merged: 4.0.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/5444





---

archive/issue_comments_045903.json:
```json
{
    "body": "<a id='comment:2'></a>This has been resolved, probably while cleaning up the preparser code. \n\n```\n----------------------------------------------------------------------\n----------------------------------------------------------------------\n| Sage Version 3.4.2, Release Date: 2009-05-05                       |\n| Type notebook() for the GUI, and license() for information.        |\nsage: [1.0..2.0]\n [1.00000000000000, 2.00000000000000]\n\n```",
    "created_at": "2009-05-18T21:54:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45903",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:2'></a>This has been resolved, probably while cleaning up the preparser code. 

```
----------------------------------------------------------------------
----------------------------------------------------------------------
| Sage Version 3.4.2, Release Date: 2009-05-05                       |
| Type notebook() for the GUI, and license() for information.        |
sage: [1.0..2.0]
 [1.00000000000000, 2.00000000000000]

```



---

archive/issue_comments_045904.json:
```json
{
    "body": "Resolution: worksforme",
    "created_at": "2009-05-18T21:54:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45904",
    "user": "https://github.com/robertwb"
}
```

Resolution: worksforme



---

archive/issue_events_012719.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-05-18T21:54:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5444#event-12719"
}
```



---

archive/issue_comments_045905.json:
```json
{
    "body": "<a id='comment:3'></a>Has a doctest been added? Otherwise this ticket should be reopened.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-18T22:02:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45905",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>Has a doctest been added? Otherwise this ticket should be reopened.

Cheers,

Michael



---

archive/issue_comments_045906.json:
```json
{
    "body": "Resolution changed from worksforme to ",
    "created_at": "2009-05-19T04:57:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45906",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution changed from worksforme to 



---

archive/issue_comments_045907.json:
```json
{
    "body": "Changing status from closed to reopened.",
    "created_at": "2009-05-19T04:57:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45907",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from closed to reopened.



---

archive/issue_events_012720.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-19T04:57:15Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5444#event-12720"
}
```



---

archive/issue_comments_045908.json:
```json
{
    "body": "<a id='comment:4'></a>Reopening until someone either points to a doctests or post a doctest patch.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-19T04:57:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45908",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>Reopening until someone either points to a doctests or post a doctest patch.

Cheers,

Michael



---

archive/issue_comments_045909.json:
```json
{
    "body": "Attachment [trac_5444.patch](tarball://root/attachments/some-uuid/ticket5444/trac_5444.patch) by @mwhansen created at 2009-06-05 01:28:54",
    "created_at": "2009-06-05T01:28:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45909",
    "user": "https://github.com/mwhansen"
}
```

Attachment [trac_5444.patch](tarball://root/attachments/some-uuid/ticket5444/trac_5444.patch) by @mwhansen created at 2009-06-05 01:28:54



---

archive/issue_comments_045910.json:
```json
{
    "body": "<a id='comment:6'></a>Doctest looks fine to me.",
    "created_at": "2009-06-05T03:42:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45910",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:6'></a>Doctest looks fine to me.



---

archive/issue_events_012721.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-13T21:50:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5444#event-12721"
}
```



---

archive/issue_comments_045911.json:
```json
{
    "body": "Changing author from \"\" to \"Mike Hansen\"",
    "created_at": "2009-06-13T21:50:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45911",
    "user": "https://github.com/ncalexan"
}
```

Changing author from "" to "Mike Hansen"



---

archive/issue_comments_045912.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Robert Bradshaw\"",
    "created_at": "2009-06-13T21:50:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45912",
    "user": "https://github.com/ncalexan"
}
```

Changing reviewer from "" to "Robert Bradshaw"



---

archive/issue_comments_045913.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-13T21:50:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45913",
    "user": "https://github.com/ncalexan"
}
```

Resolution: fixed



---

archive/issue_comments_045914.json:
```json
{
    "body": "Changing merged from \"\" to \"4.0.2.alpha0\"",
    "created_at": "2009-06-13T21:50:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5444",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5444#issuecomment-45914",
    "user": "https://github.com/ncalexan"
}
```

Changing merged from "" to "4.0.2.alpha0"
