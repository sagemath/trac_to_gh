# Issue 5028: point2d? says point.options instead of point2d.options

archive/issues_005028.json:
```json
{
    "assignees": [
        "https://github.com/seblabbe"
    ],
    "body": "\n```\nsage: point2d?\n...\nDocstring:\n    \n        A point of size `pointsize' defined by point = (x,y).\n        Point takes either a single tuple of coordinates or a list of tuples.\n    \n        Type point.options to see all options.\n    \n        EXAMPLES:\n            ...\n\nsage: point.options\nTraceback (most recent call last):\n...\nAttributeError: 'function' object has no attribute 'options'\nsage: point2d.options\n{'alpha': 1, 'faceted': False, 'pointsize': 10, 'rgbcolor': (0, 0, 1)}\n```\n\nKeywords: **point2d**\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5028_\n\n",
    "closed_at": "2009-01-23T09:40:16Z",
    "created_at": "2009-01-19T16:53:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "point2d? says point.options instead of point2d.options",
    "type": "issue",
    "updated_at": "2009-01-23T09:40:16Z",
    "url": "https://github.com/sagemath/sage/issues/5028",
    "user": "https://github.com/seblabbe"
}
```

```
sage: point2d?
...
Docstring:
    
        A point of size `pointsize' defined by point = (x,y).
        Point takes either a single tuple of coordinates or a list of tuples.
    
        Type point.options to see all options.
    
        EXAMPLES:
            ...

sage: point.options
Traceback (most recent call last):
...
AttributeError: 'function' object has no attribute 'options'
sage: point2d.options
{'alpha': 1, 'faceted': False, 'pointsize': 10, 'rgbcolor': (0, 0, 1)}
```

Keywords: **point2d**

Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/5028_





---

archive/issue_events_058357.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2009-01-19T16:53:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58357"
}
```



---

archive/issue_events_058358.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2009-01-19T16:53:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58358"
}
```



---

archive/issue_events_058359.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2009-01-19T16:53:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58359"
}
```



---

archive/issue_events_058360.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2009-01-19T16:53:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58360"
}
```



---

archive/issue_events_058361.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2009-01-19T16:53:26Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "subject": "https://github.com/seblabbe",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58361"
}
```



---

archive/issue_comments_030565.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[point2d_doc_5028.patch.gz](https://github.com/sagemath/sage/files/ticket5028/point2d_doc_5028.patch.gz)**",
    "created_at": "2009-01-19T16:57:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5028#issuecomment-30565",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[point2d_doc_5028.patch.gz](https://github.com/sagemath/sage/files/ticket5028/point2d_doc_5028.patch.gz)**



---

archive/issue_events_058362.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2009-01-19T16:57:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58362"
}
```



---

archive/issue_comments_030566.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good to me.",
    "created_at": "2009-01-20T21:38:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5028#issuecomment-30566",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good to me.



---

archive/issue_events_058363.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-20T21:38:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58363"
}
```



---

archive/issue_events_058364.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-20T21:38:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58364"
}
```



---

archive/issue_comments_030567.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.3.alpha1\n\nCheers,\n\nMichael",
    "created_at": "2009-01-23T09:40:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5028#issuecomment-30567",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.3.alpha1

Cheers,

Michael



---

archive/issue_events_058365.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-23T09:40:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58365"
}
```



---

archive/issue_events_058366.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-23T09:40:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5028",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5028#event-58366"
}
```
