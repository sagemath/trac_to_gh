# Issue 5435: [with patch, positive review] sage-ptest "Using cached timings." message could be better

archive/issues_005435.json:
```json
{
    "body": "The attached patch changes the message from\n\n```\nUsing cached timings.\n```\nto\n\n```\nUsing cached timings to run slower doctests first.\n```\n\nHopefully this is enough to explain the otherwise surprising long pause at the beginning of parallel testing runs.\n\nAssignee: cwitty\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5435\n\n",
    "closed_at": "2009-03-10T23:13:45Z",
    "created_at": "2009-03-04T04:07:39Z",
    "labels": [
        "component: doctest coverage",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4",
    "title": "[with patch, positive review] sage-ptest \"Using cached timings.\" message could be better",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5435",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
The attached patch changes the message from

```
Using cached timings.
```
to

```
Using cached timings to run slower doctests first.
```

Hopefully this is enough to explain the otherwise surprising long pause at the beginning of parallel testing runs.

Assignee: cwitty

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5435





---

archive/issue_comments_044800.json:
```json
{
    "body": "Attachment [verbose-cached-timings.patch](tarball://root/attachments/some-uuid/ticket5435/verbose-cached-timings.patch) by cwitty created at 2009-03-04 04:07:49",
    "created_at": "2009-03-04T04:07:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5435#issuecomment-44800",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Attachment [verbose-cached-timings.patch](tarball://root/attachments/some-uuid/ticket5435/verbose-cached-timings.patch) by cwitty created at 2009-03-04 04:07:49



---

archive/issue_comments_044801.json:
```json
{
    "body": "<a id='comment:1'></a>\nLooks good to me.  It took me a second to figure out what this meant when I first encountered it.",
    "created_at": "2009-03-04T04:39:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5435#issuecomment-44801",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Looks good to me.  It took me a second to figure out what this meant when I first encountered it.



---

archive/issue_events_021389.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5435",
    "rename": {
        "from": "[with patch, needs review] sage-ptest \"Using cached timings.\" message could be better",
        "to": "[with patch, positive review] sage-ptest \"Using cached timings.\" message could be better"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5435#event-21389"
}
```



---

archive/issue_comments_044802.json:
```json
{
    "body": "<a id='comment:2'></a>\nMhh, I think this needs fixing, but I think `slower` is the wrong term here. Maybe `longer` is better? Either way I want this patch to go in, but if there is a consensus I would be happy to change the patch.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-04T05:44:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5435#issuecomment-44802",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Mhh, I think this needs fixing, but I think `slower` is the wrong term here. Maybe `longer` is better? Either way I want this patch to go in, but if there is a consensus I would be happy to change the patch.

Cheers,

Michael



---

archive/issue_comments_044803.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.4.final.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-10T23:13:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5435#issuecomment-44803",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.4.final.

Cheers,

Michael



---

archive/issue_comments_044804.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-03-10T23:13:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5435#issuecomment-44804",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_021390.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-03-10T23:13:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5435",
    "milestone": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5435#event-21390"
}
```



---

archive/issue_events_021391.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-03-10T23:13:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5435",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5435#event-21391"
}
```
