# Issue 5914: [with patch, positive review] default edge color is *invisible*

archive/issues_005914.json:
```json
{
    "body": "If you do\n\n```\nsage: G = graphs.CompleteGraph(5)\nsage: G.show(edge_colors={'red':(0,1)})\n```\n\nwhat you get is one red edge...\n\nThe default color should be black.\n\nAssignee: @rlmill\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5914\n\n",
    "closed_at": "2009-04-30T02:40:15Z",
    "created_at": "2009-04-27T19:06:21Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.2",
    "title": "[with patch, positive review] default edge color is *invisible*",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5914",
    "user": "https://github.com/rlmill"
}
```
If you do

```
sage: G = graphs.CompleteGraph(5)
sage: G.show(edge_colors={'red':(0,1)})
```

what you get is one red edge...

The default color should be black.

Assignee: @rlmill

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5914





---

archive/issue_comments_050323.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,9 @@\n If you do\n \n+```\n sage: G = graphs.CompleteGraph(5)\n sage: G.show(edge_colors={'red':(0,1)})\n+```\n \n what you get is one red edge...\n \n``````\n",
    "created_at": "2009-04-27T19:06:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5914#issuecomment-50323",
    "user": "https://github.com/rlmill"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,9 @@
 If you do
 
+```
 sage: G = graphs.CompleteGraph(5)
 sage: G.show(edge_colors={'red':(0,1)})
+```
 
 what you get is one red edge...
 
``````




---

archive/issue_comments_050324.json:
```json
{
    "body": "<a id='comment:2'></a>\nvertex_colors might also suffer from the same problem...",
    "created_at": "2009-04-27T19:08:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5914#issuecomment-50324",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
vertex_colors might also suffer from the same problem...



---

archive/issue_events_023621.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5914",
    "rename": {
        "from": "default edge color is *invisible*",
        "to": "[with patch, needs review] default edge color is *invisible*"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5914#event-23621"
}
```



---

archive/issue_comments_050325.json:
```json
{
    "body": "<a id='comment:4'></a>\nAttachment [trac_5914.patch](tarball://root/attachments/some-uuid/ticket5914/trac_5914.patch) by @williamstein created at 2009-04-28 14:00:57\n\nQuick remark: If you do \n\n```\nsage: G = graphs.CompleteGraph(5)\nsage: G.show(edge_colors={'red':(0,1)})\n```\nas you put in the ticket description, you get an error.  The proper input is:\n\n```\nsage: G = graphs.CompleteGraph(5)\nsage: G.show(edge_colors={'red':[(0,1)]})\n```\n\nThis patch works fine.",
    "created_at": "2009-04-28T14:00:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5914#issuecomment-50325",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:4'></a>
Attachment [trac_5914.patch](tarball://root/attachments/some-uuid/ticket5914/trac_5914.patch) by @williamstein created at 2009-04-28 14:00:57

Quick remark: If you do 

```
sage: G = graphs.CompleteGraph(5)
sage: G.show(edge_colors={'red':(0,1)})
```
as you put in the ticket description, you get an error.  The proper input is:

```
sage: G = graphs.CompleteGraph(5)
sage: G.show(edge_colors={'red':[(0,1)]})
```

This patch works fine.



---

archive/issue_events_023622.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5914",
    "rename": {
        "from": "[with patch, needs review] default edge color is *invisible*",
        "to": "[with patch, positive review] default edge color is *invisible*"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5914#event-23622"
}
```



---

archive/issue_events_023623.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-30T02:40:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5914",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5914#event-23623"
}
```



---

archive/issue_comments_050326.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-04-30T02:40:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5914#issuecomment-50326",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_050327.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 3.4.2.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-30T02:40:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5914#issuecomment-50327",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 3.4.2.rc0.

Cheers,

Michael
