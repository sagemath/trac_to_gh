# Issue 5874: Fix readline build on FreeBSD

archive/issues_005874.json:
```json
{
    "assignees": [],
    "body": "Chase shared library name difference on FreeBSD. Without this patch, the build claims that expected files don't exist.\n\nAssignee: **@mwhansen**\n\nCC:  chet.ramey@case.edu\n\nAuthor: **Peter Jeremy**\n\nReviewer: **Mike Hansen**\n\nMerged: **sage-4.1.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5874_\n\n",
    "closed_at": "2009-07-02T23:07:32Z",
    "created_at": "2009-04-23T08:47:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20bsd",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix readline build on FreeBSD",
    "type": "issue",
    "updated_at": "2009-07-02T23:07:32Z",
    "url": "https://github.com/sagemath/sage/issues/5874",
    "user": "https://github.com/peterjeremy"
}
```
Chase shared library name difference on FreeBSD. Without this patch, the build claims that expected files don't exist.

Assignee: **@mwhansen**

CC:  chet.ramey@case.edu

Author: **Peter Jeremy**

Reviewer: **Mike Hansen**

Merged: **sage-4.1.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/5874_





---

archive/issue_events_061379.json:
```json
{
    "actor": "https://github.com/peterjeremy",
    "created_at": "2009-04-23T08:47:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5874#event-61379"
}
```



---

archive/issue_events_061380.json:
```json
{
    "actor": "https://github.com/peterjeremy",
    "created_at": "2009-04-23T08:47:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20bsd",
    "label_color": "000080",
    "label_name": "component: porting: bsd",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5874#event-61380"
}
```



---

archive/issue_events_061381.json:
```json
{
    "actor": "https://github.com/peterjeremy",
    "created_at": "2009-04-23T08:47:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5874#event-61381"
}
```



---

archive/issue_events_061382.json:
```json
{
    "actor": "https://github.com/peterjeremy",
    "created_at": "2009-04-23T08:47:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5874#event-61382"
}
```



---

archive/issue_events_061383.json:
```json
{
    "actor": "https://github.com/peterjeremy",
    "created_at": "2009-04-23T08:47:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5874#event-61383"
}
```



---

archive/issue_comments_039203.json:
```json
{
    "body": "Attachment: **[readline-5.2.p6.patch.gz](https://github.com/sagemath/sage/files/ticket5874/readline-5.2.p6.patch.gz)**",
    "created_at": "2009-04-23T08:48:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5874#issuecomment-39203",
    "user": "https://github.com/peterjeremy"
}
```

Attachment: **[readline-5.2.p6.patch.gz](https://github.com/sagemath/sage/files/ticket5874/readline-5.2.p6.patch.gz)**



---

archive/issue_comments_039204.json:
```json
{
    "body": "Author: **Peter Jeremy**",
    "created_at": "2009-06-20T02:26:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5874#issuecomment-39204",
    "user": "https://github.com/mwhansen"
}
```

Author: **Peter Jeremy**



---

archive/issue_comments_039205.json:
```json
{
    "body": "Changed assignee from **mabshoff** to **@mwhansen**",
    "created_at": "2009-06-20T02:26:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5874#issuecomment-39205",
    "user": "https://github.com/mwhansen"
}
```

Changed assignee from **mabshoff** to **@mwhansen**



---

archive/issue_events_061384.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-20T02:26:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5874#event-61384"
}
```



---

archive/issue_comments_039206.json:
```json
{
    "body": "Reviewer: **Mike Hansen**",
    "created_at": "2009-06-20T02:26:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5874#issuecomment-39206",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Mike Hansen**



---

archive/issue_comments_039207.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nLooks good to me.\n\nThe spkg with the patch incorporated is at http://sage.math.washington.edu/home/mhansen/readline-5.2.p7.spkg",
    "created_at": "2009-06-20T02:26:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5874#issuecomment-39207",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'>Comment 1:</a>
Looks good to me.

The spkg with the patch incorporated is at http://sage.math.washington.edu/home/mhansen/readline-5.2.p7.spkg



---

archive/issue_comments_039208.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nCc maintainer.  Apologies for neglecting this before",
    "created_at": "2009-06-27T08:18:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5874#issuecomment-39208",
    "user": "https://github.com/peterjeremy"
}
```

<a id='comment:2'>Comment 2:</a>
Cc maintainer.  Apologies for neglecting this before



---

archive/issue_events_061385.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-02T23:07:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5874#event-61385"
}
```



---

archive/issue_events_061386.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-02T23:07:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5874#event-61386"
}
```



---

archive/issue_comments_039209.json:
```json
{
    "body": "Merged: **sage-4.1.rc0**",
    "created_at": "2009-07-02T23:07:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5874#issuecomment-39209",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.1.rc0**
