# Issue 5226: convert sage.structure.* docstrings to Sphinx

archive/issues_005226.json:
```json
{
    "assignees": [],
    "body": "I was going to review this one, and noticed it wasn't there. \n\n_Issue created by migration from https://trac.sagemath.org/ticket/5226_\n\n",
    "closed_at": "2009-02-24T19:18:13Z",
    "created_at": "2009-02-10T02:53:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "convert sage.structure.* docstrings to Sphinx",
    "type": "issue",
    "updated_at": "2009-02-24T19:18:13Z",
    "url": "https://github.com/sagemath/sage/issues/5226",
    "user": "https://github.com/robertwb"
}
```
I was going to review this one, and noticed it wasn't there. 

_Issue created by migration from https://trac.sagemath.org/ticket/5226_





---

archive/issue_events_053371.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-02-10T02:53:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5226",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5226#event-53371"
}
```



---

archive/issue_events_053372.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-02-10T02:53:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5226",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5226#event-53372"
}
```



---

archive/issue_events_053373.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-02-10T02:53:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5226",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5226#event-53373"
}
```



---

archive/issue_events_053374.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-02-10T02:53:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5226",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5226#event-53374"
}
```



---

archive/issue_events_053375.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-02-10T02:53:55Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5226",
    "title_is": "convert sage.structure.* docstrings to Sphinx",
    "title_was": "convert sage.structure.* to ReST",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5226#event-53375"
}
```



---

archive/issue_events_053376.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-24T19:18:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5226",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5226#event-53376"
}
```



---

archive/issue_comments_033246.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nFixed via #5330 in Sage 3.4.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-24T19:18:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5226#issuecomment-33246",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Fixed via #5330 in Sage 3.4.alpha0.

Cheers,

Michael
