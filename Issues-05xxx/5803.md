# Issue 5803: Upgrade Cython to 0.11.1

archive/issues_005803.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "Spkg up at http://sage.math.washington.edu/home/robertwb/cython/cython-0.11.1.spkg\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5803_\n\n",
    "closed_at": "2009-04-24T08:13:59Z",
    "created_at": "2009-04-16T08:38:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Upgrade Cython to 0.11.1",
    "type": "issue",
    "updated_at": "2009-04-24T08:13:59Z",
    "url": "https://github.com/sagemath/sage/issues/5803",
    "user": "https://github.com/robertwb"
}
```
Spkg up at http://sage.math.washington.edu/home/robertwb/cython/cython-0.11.1.spkg

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/5803_





---

archive/issue_events_069088.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-04-16T08:38:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69088"
}
```



---

archive/issue_events_069089.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-04-16T08:38:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69089"
}
```



---

archive/issue_events_069090.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-04-16T08:38:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69090"
}
```



---

archive/issue_events_069091.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-04-16T08:38:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69091"
}
```



---

archive/issue_events_069092.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-16T08:38:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "subject": "https://github.com/robertwb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69092"
}
```



---

archive/issue_events_069093.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-04-16T08:38:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69093"
}
```



---

archive/issue_comments_037049.json:
```json
{
    "body": "Attachment: **[5803-cython-0.11.1.patch.gz](https://github.com/sagemath/sage/files/ticket5803/5803-cython-0.11.1.patch.gz)**",
    "created_at": "2009-04-16T08:44:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5803#issuecomment-37049",
    "user": "https://github.com/robertwb"
}
```

Attachment: **[5803-cython-0.11.1.patch.gz](https://github.com/sagemath/sage/files/ticket5803/5803-cython-0.11.1.patch.gz)**



---

archive/issue_events_069094.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-04-16T08:45:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69094"
}
```



---

archive/issue_events_069095.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T07:41:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69095"
}
```



---

archive/issue_events_069096.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T07:41:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69096"
}
```



---

archive/issue_comments_037050.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nOk, positive review on the spkg. I did a number of cleanups and added a proper SPKG.txt changelog entry that Robert has been skipping forever. In the ultimate move of irony I also deleted the .hg repo inside the src repo (given the current discussion on the cython mailing list). That way the size of the spkg decreased by 3.5 MB :)\n\n  http://sage.math.washington.edu/home/mabshoff/release-cycles-3.4.2/alpha0/cython-0.11.1.p0.spkg\n\nRobert: Please base future spkgs on this one since I have cleaned up this spkg before. You have then based it on the a non-clean spkg and last time I let it slide, but it won't happen again and I will give it a non-positive review :)\n\nDoctesting the patch right now, but changing the review in anticipation.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-24T07:41:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5803#issuecomment-37050",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Ok, positive review on the spkg. I did a number of cleanups and added a proper SPKG.txt changelog entry that Robert has been skipping forever. In the ultimate move of irony I also deleted the .hg repo inside the src repo (given the current discussion on the cython mailing list). That way the size of the spkg decreased by 3.5 MB :)

  http://sage.math.washington.edu/home/mabshoff/release-cycles-3.4.2/alpha0/cython-0.11.1.p0.spkg

Robert: Please base future spkgs on this one since I have cleaned up this spkg before. You have then based it on the a non-clean spkg and last time I let it slide, but it won't happen again and I will give it a non-positive review :)

Doctesting the patch right now, but changing the review in anticipation.

Cheers,

Michael



---

archive/issue_events_069097.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T07:41:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69097"
}
```



---

archive/issue_events_069098.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T07:41:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69098"
}
```



---

archive/issue_events_069099.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T08:13:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69099"
}
```



---

archive/issue_events_069100.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-24T08:13:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5803#event-69100"
}
```



---

archive/issue_comments_037051.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged patch and spkg into Sage 3.4.2.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-24T08:13:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5803#issuecomment-37051",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged patch and spkg into Sage 3.4.2.alpha0.

Cheers,

Michael
