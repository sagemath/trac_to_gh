# Issue 5740: typo in docstring for taylor series (really easy!)

archive/issues_005740.json:
```json
{
    "assignees": [],
    "body": "\n```\n        -  ``taylor`` - (default: False); if True, use Taylor\n           series, which allows more integrals to be computed (but may also\n           crash in some obscure cases due to bugs in Maxima).\n\nintegrals --> \"limits\"\n```\n\nAssignee: **@burcin**\n\nAuthor: **Minh Van Nguyen**\n\nReviewer: **Burcin Erocal**\n\nMerged: **sage-4.1.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5740_\n\n",
    "closed_at": "2009-06-26T23:12:08Z",
    "created_at": "2009-04-11T00:59:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "typo in docstring for taylor series (really easy!)",
    "type": "issue",
    "updated_at": "2009-06-26T23:12:08Z",
    "url": "https://github.com/sagemath/sage/issues/5740",
    "user": "https://github.com/williamstein"
}
```

```
        -  ``taylor`` - (default: False); if True, use Taylor
           series, which allows more integrals to be computed (but may also
           crash in some obscure cases due to bugs in Maxima).

integrals --> "limits"
```

Assignee: **@burcin**

Author: **Minh Van Nguyen**

Reviewer: **Burcin Erocal**

Merged: **sage-4.1.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/5740_





---

archive/issue_events_059619.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-11T00:59:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5740#event-59619"
}
```



---

archive/issue_events_059620.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-11T00:59:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5740#event-59620"
}
```



---

archive/issue_events_059621.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-11T00:59:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5740#event-59621"
}
```



---

archive/issue_events_059622.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-11T00:59:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5740#event-59622"
}
```



---

archive/issue_comments_037739.json:
```json
{
    "body": "based on Sage 4.1.alpha1",
    "created_at": "2009-06-26T04:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5740#issuecomment-37739",
    "user": "https://github.com/sagetrac-mvngu"
}
```

based on Sage 4.1.alpha1



---

archive/issue_comments_037740.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_5740.patch.gz](https://github.com/sagemath/sage/files/ticket5740/trac_5740.patch.gz)**",
    "created_at": "2009-06-26T04:13:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5740#issuecomment-37740",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_5740.patch.gz](https://github.com/sagemath/sage/files/ticket5740/trac_5740.patch.gz)**



---

archive/issue_comments_037741.json:
```json
{
    "body": "Author: **Minh Van Nguyen**",
    "created_at": "2009-06-26T04:13:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5740#issuecomment-37741",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Minh Van Nguyen**



---

archive/issue_events_059623.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-26T04:13:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5740#event-59623"
}
```



---

archive/issue_comments_037742.json:
```json
{
    "body": "Reviewer: **Burcin Erocal**",
    "created_at": "2009-06-26T08:36:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5740#issuecomment-37742",
    "user": "https://github.com/burcin"
}
```

Reviewer: **Burcin Erocal**



---

archive/issue_events_059624.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-06-26T08:36:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5740#event-59624"
}
```



---

archive/issue_events_059625.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-06-26T08:36:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5740#event-59625"
}
```



---

archive/issue_comments_037743.json:
```json
{
    "body": "Merged: **sage-4.1.alpha2**",
    "created_at": "2009-06-26T23:12:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5740#issuecomment-37743",
    "user": "https://github.com/sagetrac-boothby"
}
```

Merged: **sage-4.1.alpha2**



---

archive/issue_events_059626.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-06-26T23:12:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5740#event-59626"
}
```



---

archive/issue_events_059627.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-06-26T23:12:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5740",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5740#event-59627"
}
```
