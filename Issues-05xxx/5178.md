...
<a id='comment:1'></a>Gonzalo Tornaria sent this review in email to me:
...
```json
{
    "body": "<a id='comment:2'></a>I reported the bug upstream.  Is Gonzalo's +1 good enough?  I'm not really comfortable with LLL enough to sign off on this.",
...
<a id='comment:2'></a>I reported the bug upstream.  Is Gonzalo's +1 good enough?  I'm not really comfortable with LLL enough to sign off on this.
...
<a id='comment:3'></a>Email from Bill Allombert in response to the bug:
...
<a id='comment:4'></a>So, it this a positive review then? 
...
```json
{
    "body": "<a id='comment:5'></a>Can we get some movement on this?  I don't think an updated pari is happening anytime soon but it would be nice to close this.",
...
<a id='comment:5'></a>Can we get some movement on this?  I don't think an updated pari is happening anytime soon but it would be nice to close this.
...
<a id='comment:7'></a>Just got an email from Bill Allombert regarding this bug:
...
```json
{
    "body": "<a id='comment:9'></a>oops, didn't realize changing the \"upstream\" field would close the ticket",
...
<a id='comment:9'></a>oops, didn't realize changing the "upstream" field would close the ticket
...
```json
{
    "body": "<a id='comment:16'></a>Turned the 2009 patch into a branch based on Sage 9.2.beta12.\n\nPlease review.\n\n---\nNew commits:",
    "created_at": "2020-09-18T09:58:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5178",
    "type": "issue_comment",
...
<a id='comment:16'></a>Turned the 2009 patch into a branch based on Sage 9.2.beta12.
...
<a id='comment:19'></a>Replying to [comment:6 craigcitro]:
> This looks good. The only objection I have is that the documentation
> isn't correctly formatted for sphinx -- in particular, the various
> sections of the docstring (like `EXAMPLES`) should have a double colon,
> and there should be more blank lines to get things to format correctly.
...
<a id='comment:20'></a>Possible follow-up improvements:
...
```json
{
    "body": "<a id='comment:22'></a>Follow-up at #30678.",
    "created_at": "2020-09-28T10:07:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5178",
    "type": "issue_comment",
...
<a id='comment:22'></a>Follow-up at #30678.
