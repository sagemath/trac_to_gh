# Issue 5191: coercion issue of tanh(2) into QQbar

archive/issues_005191.json:
```json
{
    "body": "This is a followup to #5023: Paul Zimmermann reports:\n\n```\nsage: a=tanh(2)\nsage: a._algebraic_(QQbar)\n...\nTypeError: Unable to coerce e (<class 'sage.functions.constants.E'>) to Rational\n```\n\nCarl: If this is invalid just close the ticker as invalid.\n\nCheers,\n\nMichael\n\nAssignee: cwitty\n\nCC:  @zimmermann6\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/5191\n\n",
    "closed_at": "2009-02-06T03:26:20Z",
    "created_at": "2009-02-06T00:20:10Z",
    "labels": [
        "component: coercion",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "coercion issue of tanh(2) into QQbar",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5191",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
This is a followup to #5023: Paul Zimmermann reports:

```
sage: a=tanh(2)
sage: a._algebraic_(QQbar)
...
TypeError: Unable to coerce e (<class 'sage.functions.constants.E'>) to Rational
```

Carl: If this is invalid just close the ticker as invalid.

Cheers,

Michael

Assignee: cwitty

CC:  @zimmermann6

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/5191





---

archive/issue_events_012015.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/cwitty",
    "created_at": "2009-02-06T03:26:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5191",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5191#event-12015"
}
```



---

archive/issue_comments_040766.json:
```json
{
    "body": "<a id='comment:1'></a>QQbar(sinh(2)) fails with the above error message; I believe that sinh(2) is not algebraic, so the conversion must fail, although I suppose the error message could be nicer.\n\nHowever, QQbar(sinh(log(2))) correctly returns 3/4, so the code does work.  Therefore, I'm marking this invalid.",
    "created_at": "2009-02-06T03:26:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5191#issuecomment-40766",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:1'></a>QQbar(sinh(2)) fails with the above error message; I believe that sinh(2) is not algebraic, so the conversion must fail, although I suppose the error message could be nicer.

However, QQbar(sinh(log(2))) correctly returns 3/4, so the code does work.  Therefore, I'm marking this invalid.



---

archive/issue_comments_040767.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2009-02-06T03:26:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5191#issuecomment-40767",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Resolution: invalid



---

archive/issue_events_012016.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/cwitty",
    "created_at": "2009-02-06T03:26:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5191",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5191#event-12016"
}
```
