# Issue 5078: [with patch, positive review] bug in factoring out constant literal

archive/issues_005078.json:
```json
{
    "body": "\n```\nsage: R1 = PolynomialRing(QQ, 'x,y,z')\nsage: R1.0\nTraceback (most recent call last):\n...\nNameError: name 'R1_sage_const_p0' is not defined\nsage: R1.gen(0)\nx\n```\n\nAssignee: cwitty\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5078\n\n",
    "closed_at": "2009-01-24T16:28:32Z",
    "created_at": "2009-01-23T22:23:39Z",
    "labels": [
        "component: misc",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch, positive review] bug in factoring out constant literal",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5078",
    "user": "https://github.com/williamstein"
}
```

```
sage: R1 = PolynomialRing(QQ, 'x,y,z')
sage: R1.0
Traceback (most recent call last):
...
NameError: name 'R1_sage_const_p0' is not defined
sage: R1.gen(0)
x
```

Assignee: cwitty

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5078





---

archive/issue_comments_041045.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [5078-preparse-const.patch](tarball://root/attachments/some-uuid/ticket5078/5078-preparse-const.patch) by @robertwb created at 2009-01-24 02:05:15",
    "created_at": "2009-01-24T02:05:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5078#issuecomment-41045",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:1'></a>
Attachment [5078-preparse-const.patch](tarball://root/attachments/some-uuid/ticket5078/5078-preparse-const.patch) by @robertwb created at 2009-01-24 02:05:15



---

archive/issue_events_019692.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5078",
    "rename": {
        "from": "bug in factoring out constant literal",
        "to": "[with patch, needs review] bug in factoring out constant literal"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5078#event-19692"
}
```



---

archive/issue_events_019693.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5078",
    "rename": {
        "from": "[with patch, needs review] bug in factoring out constant literal",
        "to": "[with patch, positive review] bug in factoring out constant literal"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5078#event-19693"
}
```



---

archive/issue_comments_041046.json:
```json
{
    "body": "<a id='comment:2'></a>\nWorks for me.",
    "created_at": "2009-01-24T02:36:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5078#issuecomment-41046",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

<a id='comment:2'></a>
Works for me.



---

archive/issue_events_019694.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-24T16:28:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5078",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5078#event-19694"
}
```



---

archive/issue_comments_041047.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.3.alpha2",
    "created_at": "2009-01-24T16:28:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5078#issuecomment-41047",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.3.alpha2



---

archive/issue_comments_041048.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-01-24T16:28:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5078#issuecomment-41048",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
