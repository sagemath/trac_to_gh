...
```json
{
    "body": "<a id='comment:1'></a>Here's a very simple patch.",
...
<a id='comment:1'></a>Here's a very simple patch.
...
```json
{
    "body": "<a id='comment:2'></a>Shouldn't this patch add a doctest while adding the deprecation warning? Otherwise it isn't obvious that the function is depreacted from looking at \"order?\"\n\nCheers,\n\nMichael",
...
<a id='comment:2'></a>Shouldn't this patch add a doctest while adding the deprecation warning? Otherwise it isn't obvious that the function is depreacted from looking at "order?"
...
```json
{
    "body": "<a id='comment:3'></a>> Shouldn't this patch add a doctest while adding the deprecation warning?\n\n\nNo, I don't think so... Just kidding.  Here's a new patch with a doctest.",
...
<a id='comment:3'></a>> Shouldn't this patch add a doctest while adding the deprecation warning?
...
```json
{
    "body": "<a id='comment:5'></a>Rubber stamp. We don't need no stinkin' review for trivial review patches fixing an obvious typo ;)\n\nCheers,\n\nMichael",
...
<a id='comment:5'></a>Rubber stamp. We don't need no stinkin' review for trivial review patches fixing an obvious typo ;)
