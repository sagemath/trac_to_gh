# Issue 5375: minor problems with ReST in geometry/lattice_polytope.py

archive/issues_005375.json:
```json
{
    "assignees": [
        "https://github.com/jhpalmieri"
    ],
    "body": "This fixes the few open problems from ticket #4912.\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5375_\n\n",
    "closed_at": "2009-02-28T16:24:29Z",
    "created_at": "2009-02-26T00:22:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "minor problems with ReST in geometry/lattice_polytope.py",
    "type": "issue",
    "updated_at": "2009-02-28T16:24:29Z",
    "url": "https://github.com/sagemath/sage/issues/5375",
    "user": "https://github.com/jhpalmieri"
}
```
This fixes the few open problems from ticket #4912.

Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/5375_





---

archive/issue_events_063156.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-26T00:22:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63156"
}
```



---

archive/issue_events_063157.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-26T00:22:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "c: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63157"
}
```



---

archive/issue_events_063158.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-26T00:22:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63158"
}
```



---

archive/issue_events_063159.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-26T00:22:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63159"
}
```



---

archive/issue_events_063160.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-26T00:22:43Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "subject": "https://github.com/jhpalmieri",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63160"
}
```



---

archive/issue_events_063161.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-26T00:22:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63161"
}
```



---

archive/issue_comments_033176.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[geom-rst.patch.gz](https://github.com/sagemath/sage/files/ticket5375/geom-rst.patch.gz)**",
    "created_at": "2009-02-26T00:37:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5375#issuecomment-33176",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[geom-rst.patch.gz](https://github.com/sagemath/sage/files/ticket5375/geom-rst.patch.gz)**



---

archive/issue_events_063162.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-26T00:37:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "c: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63162"
}
```



---

archive/issue_events_063163.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-26T00:37:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63163"
}
```



---

archive/issue_comments_033177.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nREFEREE REPORT\n\n\n\nThe patch **geom-rst.patch** applied cleanly against 3.4.alpha0 and all doctests passed with the options `-t -long -optional`. The reference manual built fine with\n\n```\nsage -docbuild reference html\n```\nLooking at the relevant HTML file\n\n```\n/path/to/reference/sage/geometry/lattice_polytope.html\n```\nthe suggested patch fixed the problem it's intends to. So positive review on my part. Note that there are still a large number of typos in the file that **geom-rst.patch** touches, namely\n\n```\nsage/geometry/lattice_polytope.py\n```\nBut please open another ticket for that.",
    "created_at": "2009-02-27T07:34:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5375#issuecomment-33177",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

REFEREE REPORT



The patch **geom-rst.patch** applied cleanly against 3.4.alpha0 and all doctests passed with the options `-t -long -optional`. The reference manual built fine with

```
sage -docbuild reference html
```
Looking at the relevant HTML file

```
/path/to/reference/sage/geometry/lattice_polytope.html
```
the suggested patch fixed the problem it's intends to. So positive review on my part. Note that there are still a large number of typos in the file that **geom-rst.patch** touches, namely

```
sage/geometry/lattice_polytope.py
```
But please open another ticket for that.



---

archive/issue_events_063164.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-02-27T07:34:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63164"
}
```



---

archive/issue_comments_033178.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.4.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-28T16:24:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5375#issuecomment-33178",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.4.rc0.

Cheers,

Michael



---

archive/issue_events_063165.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-28T16:24:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63165"
}
```



---

archive/issue_events_063166.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-28T16:24:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5375",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5375#event-63166"
}
```
