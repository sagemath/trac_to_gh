# Issue 5493: [with patch, positive review] improve Sphinx documentation of sr.py

archive/issues_005493.json:
```json
{
    "body": "The attached patch makes the Sphinx output for `mq.SR` look much nicer.\n\nAssignee: @malb\n\nKeywords: sphinx, crypto\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/5493\n\n",
    "closed_at": "2009-03-20T20:50:41Z",
    "created_at": "2009-03-11T23:03:38Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.1",
    "title": "[with patch, positive review] improve Sphinx documentation of sr.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5493",
    "user": "https://github.com/malb"
}
```
The attached patch makes the Sphinx output for `mq.SR` look much nicer.

Assignee: @malb

Keywords: sphinx, crypto

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/5493





---

archive/issue_comments_045536.json:
```json
{
    "body": "Attachment [sr_sphinx.patch](tarball://root/attachments/some-uuid/ticket5493/sr_sphinx.patch) by @malb created at 2009-03-11 23:03:54\n\nyay, my first sphinx patch",
    "created_at": "2009-03-11T23:03:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5493#issuecomment-45536",
    "user": "https://github.com/malb"
}
```

Attachment [sr_sphinx.patch](tarball://root/attachments/some-uuid/ticket5493/sr_sphinx.patch) by @malb created at 2009-03-11 23:03:54

yay, my first sphinx patch



---

archive/issue_events_021667.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5493",
    "rename": {
        "from": "[with patch, needs review]",
        "to": "[with patch, needs review] improve Sphinx documentation of sr.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5493#event-21667"
}
```



---

archive/issue_comments_045537.json:
```json
{
    "body": "<a id='comment:2'></a>\nREFEREE REPORT\n\n\n\nThe patch **sr_sphinx.patch** applied fine against Sage version 3.4. All tests passed, even with the `-long` option. The reference manual (which the patch touches) builds OK and looks rather prettier, which is what Martin wants :-)  Positive review.\n\n\n\nNote that while reviewing this ticket, I also noticed some further enhancements that can be done to `sr.py`. But these are addressed in ticket #5527.",
    "created_at": "2009-03-16T07:50:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5493#issuecomment-45537",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:2'></a>
REFEREE REPORT



The patch **sr_sphinx.patch** applied fine against Sage version 3.4. All tests passed, even with the `-long` option. The reference manual (which the patch touches) builds OK and looks rather prettier, which is what Martin wants :-)  Positive review.



Note that while reviewing this ticket, I also noticed some further enhancements that can be done to `sr.py`. But these are addressed in ticket #5527.



---

archive/issue_events_021668.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/5493",
    "rename": {
        "from": "[with patch, needs review] improve Sphinx documentation of sr.py",
        "to": "[with patch, positive review] improve Sphinx documentation of sr.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5493#event-21668"
}
```



---

archive/issue_comments_045538.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.4.1.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-20T20:50:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5493#issuecomment-45538",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.4.1.alpha0.

Cheers,

Michael



---

archive/issue_events_021669.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-03-20T20:50:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5493",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5493#event-21669"
}
```



---

archive/issue_comments_045539.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-03-20T20:50:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5493#issuecomment-45539",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
