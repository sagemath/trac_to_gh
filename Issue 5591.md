# Issue 5591: alt-enter makes new input cell *after* the following text cell

archive/issues_005591.json:
```json
{
    "body": "Assignee: boothby\n\n```\nIf I want to insert a \n> calculation cell before an html cell, I can't just press Alt-Enter in \n> the foregoing calculation cell, because this creates a new calculation \n> cell AFTER the html cell. Is there another way?. \n```\n\nalt-enter should put a new input cell directly after the current input cell (i.e., before any text cell following the current input cell).\n\nIssue created by migration from https://trac.sagemath.org/ticket/5591\n\n",
    "created_at": "2009-03-23T14:43:45Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "alt-enter makes new input cell *after* the following text cell",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5591",
    "user": "https://github.com/jasongrout"
}
```
Assignee: boothby

```
If I want to insert a 
> calculation cell before an html cell, I can't just press Alt-Enter in 
> the foregoing calculation cell, because this creates a new calculation 
> cell AFTER the html cell. Is there another way?. 
```

alt-enter should put a new input cell directly after the current input cell (i.e., before any text cell following the current input cell).

Issue created by migration from https://trac.sagemath.org/ticket/5591





---

archive/issue_events_013170.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5591#event-13170"
}
```



---

archive/issue_events_013171.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5591#event-13171"
}
```



---

archive/issue_events_013172.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5591#event-13172"
}
```



---

archive/issue_events_013173.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5591#event-13173"
}
```



---

archive/issue_events_013174.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5591#event-13174"
}
```



---

archive/issue_events_013175.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5591#event-13175"
}
```



---

archive/issue_events_013176.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5591#event-13176"
}
```



---

archive/issue_comments_043476.json:
```json
{
    "body": "Reported \"upstream\" at https://github.com/sagemath/sagenb/issues/222",
    "created_at": "2014-09-17T02:20:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5591#issuecomment-43476",
    "user": "https://github.com/kcrisman"
}
```

Reported "upstream" at https://github.com/sagemath/sagenb/issues/222



---

archive/issue_events_013177.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2020-03-29T02:09:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5591#event-13177"
}
```



---

archive/issue_comments_043477.json:
```json
{
    "body": "Closing deprecated notebook tickets",
    "created_at": "2020-03-29T02:09:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5591#issuecomment-43477",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

Closing deprecated notebook tickets



---

archive/issue_comments_043478.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2020-03-29T02:09:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5591#issuecomment-43478",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

Resolution: invalid
