# Issue 10867: Binary Quadratic forms graphics routines for plotting level sets on a lattice

archive/issues_010799.json:
```json
{
    "assignees": [],
    "body": "This patch introduces some nice graphics routines to the BinaryQF class for displaying (filled) level sets Q(x,y) = m, against the integer lattice, and for highlighting the lattice points on the level set.  There are also additional options for customizing the resulting graphic (size, color, thickness, etc.).\n\nNote: This code presently only applies to positive definite binary quadratic forms.  \n\nAssignee: **justin**\n\nCC:  @jonhanke @williamstein @tornaria @rlmill\n\nKeywords: **quadratic, binary, ellipse, graphics**\n\nBranch/Commit: **[u/chapoton/10867](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/10867) @ [c322697](https://github.com/sagemath/sagetrac-mirror/commit/c322697cf64733617fe97449fe6a82d12a01e2cd)**\n\nAuthor: **Jonathan Hanke**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10867_\n\n",
    "created_at": "2011-03-03T00:57:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Binary Quadratic forms graphics routines for plotting level sets on a lattice",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/10867",
    "user": "https://github.com/jonhanke"
}
```
This patch introduces some nice graphics routines to the BinaryQF class for displaying (filled) level sets Q(x,y) = m, against the integer lattice, and for highlighting the lattice points on the level set.  There are also additional options for customizing the resulting graphic (size, color, thickness, etc.).

Note: This code presently only applies to positive definite binary quadratic forms.  

Assignee: **justin**

CC:  @jonhanke @williamstein @tornaria @rlmill

Keywords: **quadratic, binary, ellipse, graphics**

Branch/Commit: **[u/chapoton/10867](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/10867) @ [c322697](https://github.com/sagemath/sagetrac-mirror/commit/c322697cf64733617fe97449fe6a82d12a01e2cd)**

Author: **Jonathan Hanke**

_Issue created by migration from https://trac.sagemath.org/ticket/10867_





---

archive/issue_events_131439.json:
```json
{
    "actor": "https://github.com/jonhanke",
    "created_at": "2011-03-03T00:57:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131439"
}
```



---

archive/issue_events_131440.json:
```json
{
    "actor": "https://github.com/jonhanke",
    "created_at": "2011-03-03T00:57:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
    "label_color": "0000ff",
    "label_name": "component: quadratic forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131440"
}
```



---

archive/issue_events_131441.json:
```json
{
    "actor": "https://github.com/jonhanke",
    "created_at": "2011-03-03T00:57:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131441"
}
```



---

archive/issue_events_131442.json:
```json
{
    "actor": "https://github.com/jonhanke",
    "created_at": "2011-03-03T00:57:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131442"
}
```



---

archive/issue_events_131443.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131443"
}
```



---

archive/issue_events_131444.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131444"
}
```



---

archive/issue_comments_107350.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[binary_qf_graphics__15057.patch.gz](https://github.com/sagemath/sage/files/ticket10867/binary_qf_graphics__15057.patch.gz)**",
    "created_at": "2013-08-13T15:35:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107350",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[binary_qf_graphics__15057.patch.gz](https://github.com/sagemath/sage/files/ticket10867/binary_qf_graphics__15057.patch.gz)**



---

archive/issue_events_131445.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131445"
}
```



---

archive/issue_events_131446.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131446"
}
```



---

archive/issue_events_131447.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131447"
}
```



---

archive/issue_events_131448.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131448"
}
```



---

archive/issue_events_131449.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131449"
}
```



---

archive/issue_events_131450.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131450"
}
```



---

archive/issue_comments_107351.json:
```json
{
    "body": "Commit: **[c26ae9f](https://github.com/sagemath/sagetrac-mirror/commit/c26ae9fbfb337e71972e30ae5145355755b9a0a6)**",
    "created_at": "2014-11-06T19:47:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107351",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[c26ae9f](https://github.com/sagemath/sagetrac-mirror/commit/c26ae9fbfb337e71972e30ae5145355755b9a0a6)**



---

archive/issue_comments_107352.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b84cb9eed160df0719d91451d2b30dd85c23dc94\">b84cb9e</a></td><td><code>Added some ellipse info, plotting and spectral decomposition routines for BinaryQFs.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c26ae9fbfb337e71972e30ae5145355755b9a0a6\">c26ae9f</a></td><td><code>trac #10867 first draft</code></td></tr></table>\n",
    "created_at": "2014-11-06T19:47:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107352",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b84cb9eed160df0719d91451d2b30dd85c23dc94">b84cb9e</a></td><td><code>Added some ellipse info, plotting and spectral decomposition routines for BinaryQFs.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c26ae9fbfb337e71972e30ae5145355755b9a0a6">c26ae9f</a></td><td><code>trac #10867 first draft</code></td></tr></table>




---

archive/issue_comments_107353.json:
```json
{
    "body": "Branch: **[u/chapoton/10867](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/10867)**",
    "created_at": "2014-11-06T19:47:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107353",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/10867](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/10867)**



---

archive/issue_comments_107354.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ca8888576706a01b52e08a93011b5243d82bdbb3\">ca88885</a></td><td><code>Merge branch 'u/chapoton/10867' of ssh://trac.sagemath.org:22/sage into 6.5.b4</code></td></tr></table>\n",
    "created_at": "2014-12-27T12:30:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107354",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ca8888576706a01b52e08a93011b5243d82bdbb3">ca88885</a></td><td><code>Merge branch 'u/chapoton/10867' of ssh://trac.sagemath.org:22/sage into 6.5.b4</code></td></tr></table>




---

archive/issue_comments_107355.json:
```json
{
    "body": "Changed commit from **[c26ae9f](https://github.com/sagemath/sagetrac-mirror/commit/c26ae9fbfb337e71972e30ae5145355755b9a0a6)** to **[ca88885](https://github.com/sagemath/sagetrac-mirror/commit/ca8888576706a01b52e08a93011b5243d82bdbb3)**",
    "created_at": "2014-12-27T12:30:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107355",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c26ae9f](https://github.com/sagemath/sagetrac-mirror/commit/c26ae9fbfb337e71972e30ae5145355755b9a0a6)** to **[ca88885](https://github.com/sagemath/sagetrac-mirror/commit/ca8888576706a01b52e08a93011b5243d82bdbb3)**



---

archive/issue_comments_107356.json:
```json
{
    "body": "Changed commit from **[ca88885](https://github.com/sagemath/sagetrac-mirror/commit/ca8888576706a01b52e08a93011b5243d82bdbb3)** to **[4846b23](https://github.com/sagemath/sagetrac-mirror/commit/4846b2310f06f7086044a88c69ac3e935c2dbbaf)**",
    "created_at": "2016-03-01T11:28:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107356",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ca88885](https://github.com/sagemath/sagetrac-mirror/commit/ca8888576706a01b52e08a93011b5243d82bdbb3)** to **[4846b23](https://github.com/sagemath/sagetrac-mirror/commit/4846b2310f06f7086044a88c69ac3e935c2dbbaf)**



---

archive/issue_comments_107357.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4846b2310f06f7086044a88c69ac3e935c2dbbaf\">4846b23</a></td><td><code>Merge branch 'u/chapoton/10867' into 7.1.b5</code></td></tr></table>\n",
    "created_at": "2016-03-01T11:28:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107357",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4846b2310f06f7086044a88c69ac3e935c2dbbaf">4846b23</a></td><td><code>Merge branch 'u/chapoton/10867' into 7.1.b5</code></td></tr></table>




---

archive/issue_comments_107358.json:
```json
{
    "body": "Changed commit from **[4846b23](https://github.com/sagemath/sagetrac-mirror/commit/4846b2310f06f7086044a88c69ac3e935c2dbbaf)** to **[c322697](https://github.com/sagemath/sagetrac-mirror/commit/c322697cf64733617fe97449fe6a82d12a01e2cd)**",
    "created_at": "2017-06-08T19:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107358",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4846b23](https://github.com/sagemath/sagetrac-mirror/commit/4846b2310f06f7086044a88c69ac3e935c2dbbaf)** to **[c322697](https://github.com/sagemath/sagetrac-mirror/commit/c322697cf64733617fe97449fe6a82d12a01e2cd)**



---

archive/issue_comments_107359.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c322697cf64733617fe97449fe6a82d12a01e2cd\">c322697</a></td><td><code>Merge branch 'u/chapoton/10867' in 8.0.b9</code></td></tr></table>\n",
    "created_at": "2017-06-08T19:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10867#issuecomment-107359",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:9'>Comment 9:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c322697cf64733617fe97449fe6a82d12a01e2cd">c322697</a></td><td><code>Merge branch 'u/chapoton/10867' in 8.0.b9</code></td></tr></table>




---

archive/issue_events_131451.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10867",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10867#event-131451"
}
```
