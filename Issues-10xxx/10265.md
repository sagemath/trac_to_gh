# Issue 10265: Add palindrome defect word to the word constructor

archive/issues_010264.json:
```json
{
    "assignees": [],
    "body": "Add the finite word `w = a b^k a b^{k-1} a a b^{k-1} a b^{k} a` to the words constructor.\n\nAs described by Brlek, Hamel, Nivat and Reuteunaer in `[1]`, this\nfinite word `w` is such that the infinite periodic word `w^\\omega`\nhave palindromic defect ``k``.\n\n`[1]` S. Brlek, S. Hamel, M. Nivat and C. Reutenauer, On the palindromic complexity of in\ufb01nite words, Int. J. Found. Comput. Sci. 15: 2 (2004) 293\u2013306.\n\n\nAssignee: **@seblabbe**\n\nCC:  @sagetrac-abmasse\n\nAuthor: **S\u00e9bastien Labb\u00e9**\n\nReviewer: **Alexandre Blondin Mass\u00e9**\n\nMerged: **sage-4.6.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10265_\n\n",
    "closed_at": "2011-01-12T06:33:59Z",
    "created_at": "2010-11-14T18:05:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add palindrome defect word to the word constructor",
    "type": "issue",
    "updated_at": "2011-01-12T06:33:59Z",
    "url": "https://github.com/sagemath/sage/issues/10265",
    "user": "https://github.com/seblabbe"
}
```
Add the finite word `w = a b^k a b^{k-1} a a b^{k-1} a b^{k} a` to the words constructor.

As described by Brlek, Hamel, Nivat and Reuteunaer in `[1]`, this
finite word `w` is such that the infinite periodic word `w^\omega`
have palindromic defect ``k``.

`[1]` S. Brlek, S. Hamel, M. Nivat and C. Reutenauer, On the palindromic complexity of inﬁnite words, Int. J. Found. Comput. Sci. 15: 2 (2004) 293–306.


Assignee: **@seblabbe**

CC:  @sagetrac-abmasse

Author: **Sébastien Labbé**

Reviewer: **Alexandre Blondin Massé**

Merged: **sage-4.6.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/10265_





---

archive/issue_events_123005.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:05:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123005"
}
```



---

archive/issue_events_123006.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:05:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123006"
}
```



---

archive/issue_events_123007.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:05:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123007"
}
```



---

archive/issue_events_123008.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:05:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123008"
}
```



---

archive/issue_comments_097727.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNeeds review!",
    "created_at": "2010-11-14T18:09:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97727",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:1" align="right">Comment 1</div>

Needs review!



---

archive/issue_comments_097728.json:
```json
{
    "body": "Author: **S\u00e9bastien Labb\u00e9**",
    "created_at": "2010-11-14T18:09:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97728",
    "user": "https://github.com/seblabbe"
}
```

Author: **Sébastien Labbé**



---

archive/issue_events_123009.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:09:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123009"
}
```



---

archive/issue_comments_097729.json:
```json
{
    "body": "Applies over sage-4.6",
    "created_at": "2010-11-14T18:44:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97729",
    "user": "https://github.com/seblabbe"
}
```

Applies over sage-4.6



---

archive/issue_comments_097730.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAttachment: **[trac_10265-defect-word-sl.patch.gz](https://github.com/sagemath/sage/files/ticket10265/trac_10265-defect-word-sl.patch.gz)**\n\nI just re-uploaded the patch (fixing one doctest comment).",
    "created_at": "2010-11-14T18:45:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97730",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:2" align="right">Comment 2</div>

Attachment: **[trac_10265-defect-word-sl.patch.gz](https://github.com/sagemath/sage/files/ticket10265/trac_10265-defect-word-sl.patch.gz)**

I just re-uploaded the patch (fixing one doctest comment).



---

archive/issue_events_123010.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123010"
}
```



---

archive/issue_events_123011.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123011"
}
```



---

archive/issue_events_123012.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123012"
}
```



---

archive/issue_events_123013.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123013"
}
```



---

archive/issue_events_123014.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123014"
}
```



---

archive/issue_events_123015.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123015"
}
```



---

archive/issue_comments_097731.json:
```json
{
    "body": "Reviewer: **Alexandre Blondin Mass\u00e9**",
    "created_at": "2010-11-26T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97731",
    "user": "https://github.com/sagetrac-abmasse"
}
```

Reviewer: **Alexandre Blondin Massé**



---

archive/issue_comments_097732.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI just finished testing the patch. Everything seems good, all tests passed and the documentation builds find. Positive review.",
    "created_at": "2010-11-26T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97732",
    "user": "https://github.com/sagetrac-abmasse"
}
```

<div id="comment:3" align="right">Comment 3</div>

I just finished testing the patch. Everything seems good, all tests passed and the documentation builds find. Positive review.



---

archive/issue_comments_097733.json:
```json
{
    "body": "Merged: **sage-4.6.2.alpha0**",
    "created_at": "2011-01-12T06:33:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97733",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.2.alpha0**



---

archive/issue_events_123016.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-12T06:33:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123016"
}
```



---

archive/issue_events_123017.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-12T06:33:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-123017"
}
```
