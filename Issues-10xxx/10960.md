# Issue 10960: Better product error messages when can't multiply

archive/issues_010881.json:
```json
{
    "body": "See #10683, where the following less-than-helpful message was discovered.\n\n```\nsage: k.<b> = FiniteField(9); k\nFinite Field in b of size 3^2\nsage: vector([1,i]).dot_product(vector(k,[1,b]))\n---------------------------------------------------------\nArithmeticError: \n```\nOf course you can't multiply elements of the symbolic ring and this finite field.  But if this happened buried deep in some other code, it could take a long time to track down.  \n\nThis actually happens when you just multiply as well, unsurprisingly, so that's where it would have to be caught.\n\n```\nsage: k.<b> = FiniteField(9)\nsage: i*b\n<boom>\n``` \n\nAssignee: @aghitza\n\nCC:  @rbeezer @orlitzky\n\nKeywords: multiply, arithmetic, coercion, incompatible\n\nReviewer: Michael Orlitzky\n\nResolution: duplicate\n\nIssue created by migration from https://trac.sagemath.org/ticket/10960\n\n",
    "closed_at": "2013-01-10T09:24:41Z",
    "created_at": "2011-03-18T14:45:37Z",
    "labels": [
        "component: basic arithmetic",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Better product error messages when can't multiply",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10960",
    "user": "https://github.com/kcrisman"
}
```
See #10683, where the following less-than-helpful message was discovered.

```
sage: k.<b> = FiniteField(9); k
Finite Field in b of size 3^2
sage: vector([1,i]).dot_product(vector(k,[1,b]))
---------------------------------------------------------
ArithmeticError: 
```
Of course you can't multiply elements of the symbolic ring and this finite field.  But if this happened buried deep in some other code, it could take a long time to track down.  

This actually happens when you just multiply as well, unsurprisingly, so that's where it would have to be caught.

```
sage: k.<b> = FiniteField(9)
sage: i*b
<boom>
``` 

Assignee: @aghitza

CC:  @rbeezer @orlitzky

Keywords: multiply, arithmetic, coercion, incompatible

Reviewer: Michael Orlitzky

Resolution: duplicate

Issue created by migration from https://trac.sagemath.org/ticket/10960





---

archive/issue_comments_140149.json:
```json
{
    "body": "<a id='comment:1'></a>This is fixed by Burcin's patch in #13739. I just accidentally rebased it without noticing that it had a dependency, but I think the dependency is just for the line or two of context in the patch.\n\nAnyway, I added the test case from this ticket, so when #13739 gets applied we can close this, too.",
    "created_at": "2012-12-02T01:50:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10960#issuecomment-140149",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:1'></a>This is fixed by Burcin's patch in #13739. I just accidentally rebased it without noticing that it had a dependency, but I think the dependency is just for the line or two of context in the patch.

Anyway, I added the test case from this ticket, so when #13739 gets applied we can close this, too.



---

archive/issue_comments_140150.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-01-08T00:32:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10960#issuecomment-140150",
    "user": "https://github.com/orlitzky"
}
```

Changing status from new to needs_review.



---

archive/issue_events_028407.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2013-01-08T00:32:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10960",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10960#event-28407"
}
```



---

archive/issue_comments_140151.json:
```json
{
    "body": "<a id='comment:2'></a>The fix and doctest are both in, so this can be closed.",
    "created_at": "2013-01-08T00:32:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10960#issuecomment-140151",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:2'></a>The fix and doctest are both in, so this can be closed.



---

archive/issue_comments_140152.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Michael Orlitzky\"",
    "created_at": "2013-01-08T00:32:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10960#issuecomment-140152",
    "user": "https://github.com/orlitzky"
}
```

Changing reviewer from "" to "Michael Orlitzky"



---

archive/issue_comments_140153.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-01-08T00:32:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10960#issuecomment-140153",
    "user": "https://github.com/orlitzky"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_028408.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-10T09:24:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10960",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10960#event-28408"
}
```



---

archive/issue_comments_140154.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2013-01-10T09:24:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10960#issuecomment-140154",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: duplicate
