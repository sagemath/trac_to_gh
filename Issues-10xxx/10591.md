# Issue 10591: Implement univariate polynomial rings over absolute number fields

archive/issues_010538.json:
```json
{
    "assignees": [],
    "body": "After some discussion with Sebastian Spancratz it can be interesting to add a specific implementation for univariate polynomials over number fields. This can improve a lot performance, at least for multiplication, addition and gcd.\n\nOne approach is to implement Nuberfield(f)[x] more likely QQ[x][y].quotient(f(y))\n\nNote, with patch #10255\n\n```\nsage: K=QQ[x]['y']\nsage: y=K.gen()\nsage: L=K.quotient(y^16+y^5+y^4+y^3+y^2+y+1)\nsage: f=L(K.random_element(16,1500))\nsage: g=L(K.random_element(16,1500))\nsage: P=NumberField(x^16+x^5+x^4+x^3+x^2+x+1,'a')[x]\nsage: f1 = P.random_element(1500)\nsage: g1 = P.random_element(1500)\nsage: def nfpol_to_pari(f):\n    return pari([c._pari_('a') for c in f.list()]).Polrev()\n....: \nsage: fpari = nfpol_to_pari(f1)\nsage: gpari = nfpol_to_pari(g1)\nsage: %time _ = f*g\nCPU times: user 1.92 s, sys: 0.00 s, total: 1.92 s\nWall time: 1.94 s\nsage: %time _ = f1*g1\nCPU times: user 20.29 s, sys: 0.04 s, total: 20.32 s\nWall time: 20.34 s\nsage: %time _ = fpari*gpari\nCPU times: user 66.50 s, sys: 0.02 s, total: 66.52 s\nWall time: 66.58 s\nsage: %time _=f+g\nCPU times: user 0.00 s, sys: 0.00 s, total: 0.00 s\nWall time: 0.01 s\nsage: %time _=f1+g1\nCPU times: user 0.02 s, sys: 0.00 s, total: 0.02 s\nWall time: 0.02 s\nsage: %time _=fpari+gpari\nCPU times: user 0.01 s, sys: 0.00 s, total: 0.01 s\nWall time: 0.01 s\n```\n\nRelated tickets: #8558, #10255\n\nThings to do (to be completed):\n\n- Faster multiplication\n- Fast modular gcd #8558\n- Avoid PARI nfinit to compute factorization\n- Try Newton-based quo_rem\n\n\nAssignee: **@aghitza**\n\nCC:  @sagetrac-wbhart @miguelmarco\n\nKeywords: **number fields, polynomials, performance**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10591_\n\n",
    "created_at": "2011-01-11T15:50:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement univariate polynomial rings over absolute number fields",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/10591",
    "user": "https://github.com/lftabera"
}
```
After some discussion with Sebastian Spancratz it can be interesting to add a specific implementation for univariate polynomials over number fields. This can improve a lot performance, at least for multiplication, addition and gcd.

One approach is to implement Nuberfield(f)[x] more likely QQ[x][y].quotient(f(y))

Note, with patch #10255

```
sage: K=QQ[x]['y']
sage: y=K.gen()
sage: L=K.quotient(y^16+y^5+y^4+y^3+y^2+y+1)
sage: f=L(K.random_element(16,1500))
sage: g=L(K.random_element(16,1500))
sage: P=NumberField(x^16+x^5+x^4+x^3+x^2+x+1,'a')[x]
sage: f1 = P.random_element(1500)
sage: g1 = P.random_element(1500)
sage: def nfpol_to_pari(f):
    return pari([c._pari_('a') for c in f.list()]).Polrev()
....: 
sage: fpari = nfpol_to_pari(f1)
sage: gpari = nfpol_to_pari(g1)
sage: %time _ = f*g
CPU times: user 1.92 s, sys: 0.00 s, total: 1.92 s
Wall time: 1.94 s
sage: %time _ = f1*g1
CPU times: user 20.29 s, sys: 0.04 s, total: 20.32 s
Wall time: 20.34 s
sage: %time _ = fpari*gpari
CPU times: user 66.50 s, sys: 0.02 s, total: 66.52 s
Wall time: 66.58 s
sage: %time _=f+g
CPU times: user 0.00 s, sys: 0.00 s, total: 0.00 s
Wall time: 0.01 s
sage: %time _=f1+g1
CPU times: user 0.02 s, sys: 0.00 s, total: 0.02 s
Wall time: 0.02 s
sage: %time _=fpari+gpari
CPU times: user 0.01 s, sys: 0.00 s, total: 0.01 s
Wall time: 0.01 s
```

Related tickets: #8558, #10255

Things to do (to be completed):

- Faster multiplication
- Fast modular gcd #8558
- Avoid PARI nfinit to compute factorization
- Try Newton-based quo_rem


Assignee: **@aghitza**

CC:  @sagetrac-wbhart @miguelmarco

Keywords: **number fields, polynomials, performance**

_Issue created by migration from https://trac.sagemath.org/ticket/10591_





---

archive/issue_events_127288.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2011-01-11T15:50:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127288"
}
```



---

archive/issue_events_127289.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2011-01-11T15:51:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127289"
}
```



---

archive/issue_events_127290.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2011-01-11T15:51:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127290"
}
```



---

archive/issue_comments_103157.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-A specific implementation can improve a lot performance. At least for multiplication, addition and gcd.\n+After some discussion with Sebastian Spancratz it can be interesting to add a specific implementation for univariate polynomials over number fields. This can improve a lot performance, at least for multiplication, addition and gcd.\n \n One approach is to implement Nuberfield(f)[x] more likely QQ[x][y].quotient(f(y))\n \n``````\n",
    "created_at": "2011-01-11T15:53:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10591#issuecomment-103157",
    "user": "https://github.com/lftabera"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-A specific implementation can improve a lot performance. At least for multiplication, addition and gcd.
+After some discussion with Sebastian Spancratz it can be interesting to add a specific implementation for univariate polynomials over number fields. This can improve a lot performance, at least for multiplication, addition and gcd.
 
 One approach is to implement Nuberfield(f)[x] more likely QQ[x][y].quotient(f(y))
 
``````




---

archive/issue_comments_103158.json:
```json
{
    "body": "Assignee: **@aghitza**",
    "created_at": "2011-01-11T23:21:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10591#issuecomment-103158",
    "user": "https://github.com/lftabera"
}
```

Assignee: **@aghitza**



---

archive/issue_events_127291.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2011-01-11T23:21:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127291"
}
```



---

archive/issue_comments_103159.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -39,3 +39,11 @@\n ```\n \n Related tickets: #8558, #10255\n+\n+Things to do (to be completed):\n+\n+- Faster multiplication\n+- Fast modular gcd #8558\n+- Avoid PARI nfinit to compute factorization\n+- Try Newton-based quo_rem\n+\n``````\n",
    "created_at": "2011-01-11T23:21:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10591#issuecomment-103159",
    "user": "https://github.com/lftabera"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -39,3 +39,11 @@
 ```
 
 Related tickets: #8558, #10255
+
+Things to do (to be completed):
+
+- Faster multiplication
+- Fast modular gcd #8558
+- Avoid PARI nfinit to compute factorization
+- Try Newton-based quo_rem
+
``````




---

archive/issue_events_127292.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127292"
}
```



---

archive/issue_events_127293.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127293"
}
```



---

archive/issue_events_127294.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127294"
}
```



---

archive/issue_events_127295.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127295"
}
```



---

archive/issue_events_127296.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127296"
}
```



---

archive/issue_events_127297.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127297"
}
```



---

archive/issue_events_127298.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127298"
}
```



---

archive/issue_events_127299.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127299"
}
```



---

archive/issue_comments_103160.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nFlint should have suppport for this through Antic, right?",
    "created_at": "2014-08-25T14:01:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10591#issuecomment-103160",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:8" align="right">Comment 8</div>

Flint should have suppport for this through Antic, right?



---

archive/issue_events_127300.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10591",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10591#event-127300"
}
```
