...
```json
{
    "body": "I think it would be great to have more differential geometry in Sage! It may be nice also to have some unification of differential and algebraic geometry, so that it is easy to switch from working with a smooth variety to working with a manifold and back.",
...
Attachment [trac_10132_differential_geometry_sage.patch](tarball://root/attachments/some-uuid/ticket10132/trac_10132_differential_geometry_sage.patch) by mikarm created at 2010-10-20 00:27:25
...
Attachment [Parametrized Curve_ Computing Geodesics.sws](tarball://root/attachments/some-uuid/ticket10132/Parametrized Curve_ Computing Geodesics.sws) by @jvkersch created at 2010-10-26 05:45:40
...
Attachment [ParametrizedSurface3D_ Examples.sws](tarball://root/attachments/some-uuid/ticket10132/ParametrizedSurface3D_ Examples.sws) by mikarm created at 2010-11-01 02:28:49
...
Attachment [parametrized_surface3d.py](tarball://root/attachments/some-uuid/ticket10132/parametrized_surface3d.py) by @jvkersch created at 2010-11-05 05:46:21
...
Attachment [trac_10132_differential_geometry_sage_v3.patch](tarball://root/attachments/some-uuid/ticket10132/trac_10132_differential_geometry_sage_v3.patch) by mikarm created at 2010-11-06 14:10:36
...
Attachment [trac_10132_differential_geometry_sage_v3.2.patch](tarball://root/attachments/some-uuid/ticket10132/trac_10132_differential_geometry_sage_v3.2.patch) by @jasongrout created at 2010-11-08 16:14:43
...
Attachment [Minimal_surfaces_and_surface_of_revolution.sws](tarball://root/attachments/some-uuid/ticket10132/Minimal_surfaces_and_surface_of_revolution.sws) by @jvkersch created at 2010-11-24 19:35:11
...
```json
{
    "body": "I've updated the file with some more working doctests, and I've added it to the documentation index, so that HTML docs can be created.",
...
```json
{
    "body": "You can use queues to automatically import the patch directly from the trac website, which preserves the author and message.\n\n```\nhg qimport http://trac.sagemath.org/sage_trac/raw-attachment/ticket/10132/trac_10132_diff_geom_sage_doctests.patch\nhg qpush\n```",
...
```json
{
    "body": "Joris, I submit the final version of the patch. I think it is ok, it passes through the tests well, the only thing I have not done yet is the \"long tests\". Please, look through, may be you have some improvements, or comments.",
...
```json
{
    "body": "Made a patchbot-friendly description.",
    "created_at": "2011-03-16T13:52:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10132",
    "type": "issue_comment",
...
```json
{
    "body": "Someone should delete my previous message, since I'm just rehashing what Vincent already pointed out.  Anyway, I agree with the need for code dealing with simplifying symbolic matrices and vectors, and this is now #10552 and #11335, respectively.",
...
```json
{
    "body": "I removed the simplification code for vectors and matrices, moved the files to a subfolder of `sage.geometry` and smoothed out some parts of the code.  Doctests seem to pass and the documentation builds cleanly.",
...
```json
{
    "body": "I forgot to add that I based the patch on sage-4.7.1.alpha3.  If you use plain 4.7, also apply #11335 and #11381.",
...
Attachment [trac_10132_final_allfiles_done_no_matrix_simplify.patch](tarball://root/attachments/some-uuid/ticket10132/trac_10132_final_allfiles_done_no_matrix_simplify.patch) by @jvkersch created at 2011-12-20 22:52:40
...
```json
{
    "body": "Apply trac_10132_final_allfiles_done_no_matrix_simplify.patch \n\n(for patchbot)",
...
```json
{
    "body": "I've updated the patch for sage-5.0.beta10.",
...
```json
{
    "body": "I removed all trailing whitespace from the patch.  I'm a bit worried this might not be sufficient since this is such a simple algorithmic task.  Surely the patchbot can do this by itself?  Anyway, let me know if any other modifications are needed -- I'm quite keen on finally finishing off this patch.  \n\nVincent, are you still able to review the patch?  Thanks!",
...
```json
{
    "body": "I'm curious: what's the status of this?  Is it good enough to get in (keeping in mind that perfect is sometimes the enemy of progress)?  I'd hate to see this totally stall.",
...
```json
{
    "body": "Would be awesome to have something working in a month or so before the next school year ;-)",
...
```json
{
    "body": "for the bot, instructions should be given here as follows:\n\napply only trac_10132_revised_sage-5.10.patch",
...
```json
{
    "body": "Hi Vincent, thanks for the review! I will look into the issue with the cached methods soon. A possible merge with sage-manifolds is definitely a good idea, and high on my todo-list, but I haven't had a chance to explore that code in depth just yet.",
...
```json
{
    "body": "Feel free to add `# known bug` to those plots...",
...
```json
{
    "body": "Attachment [trac_10132_revised_sage-5.13.beta2.patch](tarball://root/attachments/some-uuid/ticket10132/trac_10132_revised_sage-5.13.beta2.patch) by @jvkersch created at 2013-11-23 15:55:41",
    "created_at": "2013-11-23T15:55:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10132",
    "type": "issue_comment",
...
Attachment [trac_10132_revised_sage-5.13.beta2.patch](tarball://root/attachments/some-uuid/ticket10132/trac_10132_revised_sage-5.13.beta2.patch) by @jvkersch created at 2013-11-23 15:55:41
...
```json
{
    "body": "Thanks, I forgot about that one.",
