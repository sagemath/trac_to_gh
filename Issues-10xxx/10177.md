# Issue 10177: Gamma0 equivalence function for number field cusps returns wrong transformation matrix

archive/issues_010176.json:
```json
{
    "assignees": [],
    "body": "The transformation matrix returned by 'is_Gamma0_equivalent' for number field cusps is not in Gamma0(N). My fault (this is even wrong in the example I wrote for the documentation, sorry...):\n\n```\n  sage: k.<a> = NumberField(x^2 + 5)\n  sage: N = k.ideal(3)\n  sage: alpha = NFCusp(k, 3, a + 1)\n  sage: beta = NFCusp(k, 2, a - 3)\n  sage: alpha.is_Gamma0_equivalent(beta, N, Transformation=True)\n  (True, [2*a - 4, -3*a - 4, -5*a + 2, 3*a + 13])\n ```\nThe matrix is not in Gamma0(3)!\n\nIt's not difficult to compute the right matrix. I'll be happy to write a patch in a few weeks (after I submit my thesis). \n\nAssignee: **@craigcitro**\n\nCC:  @JohnCremona\n\nKeywords: **number field cusps**\n\nAuthor: **Maite Aranes**\n\nReviewer: **David Loeffler**\n\nMerged: **sage-4.6.1.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10177_\n\n",
    "closed_at": "2010-12-19T09:34:22Z",
    "created_at": "2010-10-27T11:34:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Gamma0 equivalence function for number field cusps returns wrong transformation matrix",
    "type": "issue",
    "updated_at": "2010-12-19T09:34:22Z",
    "url": "https://github.com/sagemath/sage/issues/10177",
    "user": "https://github.com/sagetrac-mtaranes"
}
```
The transformation matrix returned by 'is_Gamma0_equivalent' for number field cusps is not in Gamma0(N). My fault (this is even wrong in the example I wrote for the documentation, sorry...):

```
  sage: k.<a> = NumberField(x^2 + 5)
  sage: N = k.ideal(3)
  sage: alpha = NFCusp(k, 3, a + 1)
  sage: beta = NFCusp(k, 2, a - 3)
  sage: alpha.is_Gamma0_equivalent(beta, N, Transformation=True)
  (True, [2*a - 4, -3*a - 4, -5*a + 2, 3*a + 13])
 ```
The matrix is not in Gamma0(3)!

It's not difficult to compute the right matrix. I'll be happy to write a patch in a few weeks (after I submit my thesis). 

Assignee: **@craigcitro**

CC:  @JohnCremona

Keywords: **number field cusps**

Author: **Maite Aranes**

Reviewer: **David Loeffler**

Merged: **sage-4.6.1.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/10177_





---

archive/issue_events_121620.json:
```json
{
    "actor": "https://github.com/sagetrac-mtaranes",
    "created_at": "2010-10-27T11:34:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "component: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121620"
}
```



---

archive/issue_events_121621.json:
```json
{
    "actor": "https://github.com/sagetrac-mtaranes",
    "created_at": "2010-10-27T11:34:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121621"
}
```



---

archive/issue_events_121622.json:
```json
{
    "actor": "https://github.com/sagetrac-mtaranes",
    "created_at": "2010-10-27T11:34:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121622"
}
```



---

archive/issue_events_121623.json:
```json
{
    "actor": "https://github.com/sagetrac-mtaranes",
    "created_at": "2010-12-13T17:17:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121623"
}
```



---

archive/issue_comments_096117.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_10177_transf_matrix_fix.patch.gz](https://github.com/sagemath/sage/files/ticket10177/trac_10177_transf_matrix_fix.patch.gz)**\n\nHere is a patch, based on Sage 4.6.",
    "created_at": "2010-12-13T17:17:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10177#issuecomment-96117",
    "user": "https://github.com/sagetrac-mtaranes"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_10177_transf_matrix_fix.patch.gz](https://github.com/sagemath/sage/files/ticket10177/trac_10177_transf_matrix_fix.patch.gz)**

Here is a patch, based on Sage 4.6.



---

archive/issue_events_121624.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-16T13:14:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121624"
}
```



---

archive/issue_events_121625.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-16T13:14:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121625"
}
```



---

archive/issue_events_121626.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-16T13:14:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121626"
}
```



---

archive/issue_comments_096118.json:
```json
{
    "body": "Reviewer: **David Loeffler**",
    "created_at": "2010-12-16T13:14:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10177#issuecomment-96118",
    "user": "https://github.com/loefflerd"
}
```

Reviewer: **David Loeffler**



---

archive/issue_comments_096119.json:
```json
{
    "body": "Author: **Maite Aranes**",
    "created_at": "2010-12-16T13:14:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10177#issuecomment-96119",
    "user": "https://github.com/loefflerd"
}
```

Author: **Maite Aranes**



---

archive/issue_comments_096120.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks fine to me, and all doctests in sage/modular pass. (I set the milestone to 4.6.2, as 4.6.1 is in feature freeze, I believe).",
    "created_at": "2010-12-16T13:14:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10177#issuecomment-96120",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks fine to me, and all doctests in sage/modular pass. (I set the milestone to 4.6.2, as 4.6.1 is in feature freeze, I believe).



---

archive/issue_comments_096121.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nReplying to [@loefflerd](#comment%3A2):\n> Looks fine to me, and all doctests in sage/modular pass. (I set the milestone to 4.6.2, as 4.6.1 is in feature freeze, I believe).\n\nThanks, David -- it would quite nice to get this in before Maite's viva (Jan 27th)!",
    "created_at": "2010-12-16T20:45:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10177#issuecomment-96121",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:3" align="right">Comment 3</div>

Replying to [@loefflerd](#comment%3A2):
> Looks fine to me, and all doctests in sage/modular pass. (I set the milestone to 4.6.2, as 4.6.1 is in feature freeze, I believe).

Thanks, David -- it would quite nice to get this in before Maite's viva (Jan 27th)!



---

archive/issue_comments_096122.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nMilestone set back to 4.6.1 by special request from John Cremona.",
    "created_at": "2010-12-17T14:11:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10177#issuecomment-96122",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

Milestone set back to 4.6.1 by special request from John Cremona.



---

archive/issue_events_121627.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-17T14:11:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121627"
}
```



---

archive/issue_events_121628.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-17T14:11:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121628"
}
```



---

archive/issue_events_121629.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-19T09:34:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121629"
}
```



---

archive/issue_events_121630.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-19T09:34:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10177#event-121630"
}
```



---

archive/issue_comments_096123.json:
```json
{
    "body": "Merged: **sage-4.6.1.rc0**",
    "created_at": "2010-12-19T09:34:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10177#issuecomment-96123",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.1.rc0**
