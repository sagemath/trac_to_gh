...
```json
{
    "body": "<a id='comment:29'></a>\n`_construct_mutation_classes_by_`rank, `_construct_exceptional_mutation_`classes, and  `_save_exceptional_data_`dig6 depend on #10538.  Perhaps they could move into there?  (This was not detected by the doctesting because, um, there are no doctests for those procedures.)  \n\nAlso, I am a bit confused about what the role of the `_save_`... method is.  It's a private method, so it in principle shouldn't be called by the user, but it also doesn't seem to be called by the code in this patch or in 10538.",
