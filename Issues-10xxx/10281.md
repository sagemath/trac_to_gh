...
```json
{
    "body": "<a id='comment:1'></a>This is not just an issue with cyclotomic linear algebra.  I'm doing some computations of modular symbols spaces of weight 4 with trivial character, and just hit \"no previous prime\".  We need real 64-bit mod-p linear algebra to fix this issue.  We need more primes!",
...
<a id='comment:1'></a>This is not just an issue with cyclotomic linear algebra.  I'm doing some computations of modular symbols spaces of weight 4 with trivial character, and just hit "no previous prime".  We need real 64-bit mod-p linear algebra to fix this issue.  We need more primes!
...
<a id='comment:2'></a>This turned out to be easier to resolve than it might have otherwise been, since Burcin Erocal and Martin Albrecht had improved matrices modn so they can now handle a modulus up to 2^23.  However, they hadn't adapted the chinese remainder theorem infrastructure to take this into account. 
...
```json
{
    "body": "<a id='comment:4'></a>Note - the patch is bigger than expected since I fixed some of the formatting of docstrings in a file, just because.",
...
<a id='comment:4'></a>Note - the patch is bigger than expected since I fixed some of the formatting of docstrings in a file, just because.
...
<a id='comment:5'></a>Apply trac_10281-sage-5.0-beta9.patch
...
<a id='comment:6'></a>Looks good to me. The only comment I have is that the changes adding
...
<a id='comment:7'></a>Replying to [mraum](#comment%3A5):
> Apply trac_10281-sage-5.0-beta9.patch
> 
> (for the patchbot)
> 
...
<a id='comment:8'></a>I tested on 32-bit and it works, but *only* after making a 1-line change to the Linbox spkg (which isn't needed on 64-bit, evidently). 
...
<a id='comment:10'></a>I have tested the spkg, and it is completely ok. The commit message could be a bit more informative, but since it refers to the ticket, it's ok also.
...
```json
{
    "body": "<a id='comment:12'></a>Great! Let me know when you have made progress on #4968. I hope to have time to review it.",
...
<a id='comment:12'></a>Great! Let me know when you have made progress on #4968. I hope to have time to review it.
