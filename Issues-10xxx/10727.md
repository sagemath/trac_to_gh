# Issue 10727: 2d console matrix/function plotting

archive/issues_010664.json:
```json
{
    "assignees": [],
    "body": "It's possible to plot a matrix or a 2d function in the console as a colorful map of rectangular ascii characters. The attached code by Nicolas P. Rougier does this for a numpy array. I suggest to refactor this for Sage, by introducing a plot2d_console function that either visualizes a matrix-array or a 2d function. \n\nAssignee: **@jasongrout, @williamstein**\n\nCC:  nicolas.rougier@inria.fr\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10727_\n\n",
    "created_at": "2011-02-02T13:19:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "2d console matrix/function plotting",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/10727",
    "user": "https://github.com/haraldschilly"
}
```
It's possible to plot a matrix or a 2d function in the console as a colorful map of rectangular ascii characters. The attached code by Nicolas P. Rougier does this for a numpy array. I suggest to refactor this for Sage, by introducing a plot2d_console function that either visualizes a matrix-array or a 2d function. 

Assignee: **@jasongrout, @williamstein**

CC:  nicolas.rougier@inria.fr

_Issue created by migration from https://trac.sagemath.org/ticket/10727_





---

archive/issue_events_129201.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2011-02-02T13:19:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129201"
}
```



---

archive/issue_events_129202.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2011-02-02T13:19:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129202"
}
```



---

archive/issue_events_129203.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2011-02-02T13:19:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129203"
}
```



---

archive/issue_events_129204.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2011-02-02T13:19:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129204"
}
```



---

archive/issue_comments_105110.json:
```json
{
    "body": "original code by Nicolas P. Rougier",
    "created_at": "2011-02-02T13:20:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10727#issuecomment-105110",
    "user": "https://github.com/haraldschilly"
}
```

original code by Nicolas P. Rougier



---

archive/issue_comments_105111.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[numpy_imshow.py.gz](https://github.com/sagemath/sage/files/ticket10727/numpy_imshow.py.gz)**",
    "created_at": "2013-08-13T15:35:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10727#issuecomment-105111",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[numpy_imshow.py.gz](https://github.com/sagemath/sage/files/ticket10727/numpy_imshow.py.gz)**



---

archive/issue_events_129205.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129205"
}
```



---

archive/issue_events_129206.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129206"
}
```



---

archive/issue_events_129207.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129207"
}
```



---

archive/issue_events_129208.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129208"
}
```



---

archive/issue_events_129209.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129209"
}
```



---

archive/issue_events_129210.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129210"
}
```



---

archive/issue_events_129211.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129211"
}
```



---

archive/issue_events_129212.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129212"
}
```



---

archive/issue_events_129213.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10727",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10727#event-129213"
}
```
