# Issue 10642: Additional points in a documentation titles

archive/issues_010589.json:
```json
{
    "assignees": [],
    "body": "In the sage reference manual, some entries have trailing dots '.'. The patches remove these dots.\n\n---\n\nApply\n\n1. [attachment: trac_10642.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642.patch.gz)\n2. [attachment: trac_10642_new_dots_sage.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sage.patch.gz)\n\nto the **sage** repository.\n\nApply\n\n1. [attachment: trac_10642_new_dots_sagenb.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sagenb.patch.gz)\n\nto the **sagenb** repository.\n\n\nAssignee: **@hedtke**\n\nKeywords: **days28**\n\nAuthor: **Ivo Hedtke, Julian Rueth**\n\nReviewer: **Julian Rueth, Jason Grout**\n\nMerged: **sage-4.8.alpha5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10642_\n\n",
    "closed_at": "2011-12-17T09:12:09Z",
    "created_at": "2011-01-18T10:16:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Additional points in a documentation titles",
    "type": "issue",
    "updated_at": "2011-12-17T09:12:09Z",
    "url": "https://github.com/sagemath/sage/issues/10642",
    "user": "https://github.com/sagetrac-oguichard"
}
```
In the sage reference manual, some entries have trailing dots '.'. The patches remove these dots.

---

Apply

1. [attachment: trac_10642.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642.patch.gz)
2. [attachment: trac_10642_new_dots_sage.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sage.patch.gz)

to the **sage** repository.

Apply

1. [attachment: trac_10642_new_dots_sagenb.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sagenb.patch.gz)

to the **sagenb** repository.


Assignee: **@hedtke**

Keywords: **days28**

Author: **Ivo Hedtke, Julian Rueth**

Reviewer: **Julian Rueth, Jason Grout**

Merged: **sage-4.8.alpha5**

_Issue created by migration from https://trac.sagemath.org/ticket/10642_





---

archive/issue_events_128049.json:
```json
{
    "actor": "https://github.com/sagetrac-oguichard",
    "created_at": "2011-01-18T10:16:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128049"
}
```



---

archive/issue_events_128050.json:
```json
{
    "actor": "https://github.com/sagetrac-oguichard",
    "created_at": "2011-01-18T10:16:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128050"
}
```



---

archive/issue_events_128051.json:
```json
{
    "actor": "https://github.com/sagetrac-oguichard",
    "created_at": "2011-01-18T10:16:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128051"
}
```



---

archive/issue_events_128052.json:
```json
{
    "actor": "https://github.com/sagetrac-oguichard",
    "created_at": "2011-01-18T10:16:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128052"
}
```



---

archive/issue_events_128053.json:
```json
{
    "actor": "https://github.com/sagetrac-oguichard",
    "created_at": "2011-01-18T10:16:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128053"
}
```



---

archive/issue_comments_103936.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,4 @@\n An extremous point is present in the chapter 'Symbolic Computation' in part 'Symbolic Calculus' in Sage Reference Manual v4.6.\n \n+there are more such points\n+\n``````\n",
    "created_at": "2011-07-23T16:04:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103936",
    "user": "https://github.com/hedtke"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,4 @@
 An extremous point is present in the chapter 'Symbolic Computation' in part 'Symbolic Calculus' in Sage Reference Manual v4.6.
 
+there are more such points
+
``````




---

archive/issue_comments_103937.json:
```json
{
    "body": "Changed assignee from **days28** to **@hedtke**",
    "created_at": "2011-07-23T16:04:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103937",
    "user": "https://github.com/hedtke"
}
```

Changed assignee from **days28** to **@hedtke**



---

archive/issue_events_128054.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-23T16:04:48Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "title_is": "Additional points in a documentation titles",
    "title_was": "Additionnal point in a documentation title 'Symbolic Computation'",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128054"
}
```



---

archive/issue_events_128055.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-23T18:03:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128055"
}
```



---

archive/issue_comments_103938.json:
```json
{
    "body": "Author: **Ivo Hedtke**",
    "created_at": "2011-07-23T18:03:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103938",
    "user": "https://github.com/hedtke"
}
```

Author: **Ivo Hedtke**



---

archive/issue_comments_103939.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[trac_10642.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642.patch.gz)**",
    "created_at": "2011-07-23T18:03:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103939",
    "user": "https://github.com/hedtke"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[trac_10642.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642.patch.gz)**



---

archive/issue_comments_103940.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nIn the meantime some new dots have appeared. I'm updating the patch accordingly.",
    "created_at": "2011-10-18T12:48:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103940",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:3'>Comment 3:</a>
In the meantime some new dots have appeared. I'm updating the patch accordingly.



---

archive/issue_events_128056.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-10-18T12:48:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128056"
}
```



---

archive/issue_events_128057.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-10-18T12:48:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128057"
}
```



---

archive/issue_comments_103941.json:
```json
{
    "body": "Reviewer: **Julian Rueth**",
    "created_at": "2011-10-18T12:48:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103941",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rueth**



---

archive/issue_comments_103942.json:
```json
{
    "body": "Attachment: **[trac_10642_new_dots_sage.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sage.patch.gz)**\n\nremove trailing dots from the reference manual",
    "created_at": "2011-10-18T15:47:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103942",
    "user": "https://github.com/saraedum"
}
```

Attachment: **[trac_10642_new_dots_sage.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sage.patch.gz)**

remove trailing dots from the reference manual



---

archive/issue_comments_103943.json:
```json
{
    "body": "Attachment: **[trac_10642_new_dots_sagenb.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sagenb.patch.gz)**\n\nremove trailing dots from the reference manual",
    "created_at": "2011-10-18T15:48:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103943",
    "user": "https://github.com/saraedum"
}
```

Attachment: **[trac_10642_new_dots_sagenb.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sagenb.patch.gz)**

remove trailing dots from the reference manual



---

archive/issue_events_128058.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-10-18T15:52:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128058"
}
```



---

archive/issue_events_128059.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-10-18T15:52:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128059"
}
```



---

archive/issue_events_128060.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-10-18T15:52:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128060"
}
```



---

archive/issue_events_128061.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-10-18T15:52:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128061"
}
```



---

archive/issue_comments_103944.json:
```json
{
    "body": "Changed author from **Ivo Hedtke** to **Ivo Hedtke, Julian Rueth**",
    "created_at": "2011-10-18T15:52:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103944",
    "user": "https://github.com/saraedum"
}
```

Changed author from **Ivo Hedtke** to **Ivo Hedtke, Julian Rueth**



---

archive/issue_comments_103945.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,17 @@\n-An extremous point is present in the chapter 'Symbolic Computation' in part 'Symbolic Calculus' in Sage Reference Manual v4.6.\n+In the sage reference manual, some entries have trailing dots '.'. The patches remove these dots.\n \n-there are more such points\n+---\n \n+Apply\n+\n+1. [attachment: trac_10642.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642.patch.gz)\n+2. [attachment: trac_10642_new_dots_sage.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sage.patch.gz)\n+\n+to the **sage** repository.\n+\n+Apply\n+\n+1. [attachment: trac_10642_new_dots_sagenb.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sagenb.patch.gz)\n+\n+to the **sagenb** repository.\n+\n``````\n",
    "created_at": "2011-10-18T15:52:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103945",
    "user": "https://github.com/saraedum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,17 @@
-An extremous point is present in the chapter 'Symbolic Computation' in part 'Symbolic Calculus' in Sage Reference Manual v4.6.
+In the sage reference manual, some entries have trailing dots '.'. The patches remove these dots.
 
-there are more such points
+---
 
+Apply
+
+1. [attachment: trac_10642.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642.patch.gz)
+2. [attachment: trac_10642_new_dots_sage.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sage.patch.gz)
+
+to the **sage** repository.
+
+Apply
+
+1. [attachment: trac_10642_new_dots_sagenb.patch.gz](https://github.com/sagemath/sage/files/ticket10642/trac_10642_new_dots_sagenb.patch.gz)
+
+to the **sagenb** repository.
+
``````




---

archive/issue_events_128062.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-03T16:14:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128062"
}
```



---

archive/issue_comments_103946.json:
```json
{
    "body": "Milestone sage-4.7.3 deleted",
    "created_at": "2011-11-03T16:14:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103946",
    "user": "https://github.com/jdemeyer"
}
```

Milestone sage-4.7.3 deleted



---

archive/issue_events_128063.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-12-13T17:45:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128063"
}
```



---

archive/issue_events_128064.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-12-13T17:45:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128064"
}
```



---

archive/issue_comments_103947.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nLooks good; applies to 4.7.2.alpha3, and things look great.  Thanks!",
    "created_at": "2011-12-13T17:45:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103947",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:6'>Comment 6:</a>
Looks good; applies to 4.7.2.alpha3, and things look great.  Thanks!



---

archive/issue_events_128065.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-12-13T17:45:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128065"
}
```



---

archive/issue_comments_103948.json:
```json
{
    "body": "Changed reviewer from **Julian Rueth** to **Julian Rueth, Jason Grout**",
    "created_at": "2011-12-13T17:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103948",
    "user": "https://github.com/jasongrout"
}
```

Changed reviewer from **Julian Rueth** to **Julian Rueth, Jason Grout**



---

archive/issue_events_128066.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-17T09:12:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128066"
}
```



---

archive/issue_events_128067.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-17T09:12:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10642#event-128067"
}
```



---

archive/issue_comments_103949.json:
```json
{
    "body": "Merged: **sage-4.8.alpha5**",
    "created_at": "2011-12-17T09:12:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10642#issuecomment-103949",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.alpha5**
