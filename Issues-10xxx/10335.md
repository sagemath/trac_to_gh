# Issue 10335: Add domains for permutation groups

archive/issues_010334.json:
```json
{
    "assignees": [],
    "body": "Often, one wants to define a permutation group on a set other than ` {1,2,...,n} ` such as say ` {'a', 'b', 'c', 'd'} `.\n\nFor example,\n\n```\nsage: G = PermutationGroup([ [('c','d')], [('a','c')] ])\nsage: G.orbit('a')\n['a', 'c', 'd']\n```\n\n**Apply:**\n1.  [attachment: trac_10335-permgroup_domain-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-rebase.patch.gz)\n\nDepends on #10334\n\nAssignee: **joyner**\n\nCC:  sage-combinat jasonbhill @rbeezer @jdemeyer\n\nKeywords: **sd31**\n\nAuthor: **Mike Hansen, Jason Hill, David Loeffler**\n\nReviewer: **Robert Miller, Rob Beezer, Nicolas Borie, Nicolas M. Thi\u00e9ry**\n\nMerged: **sage-4.7.2.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10335_\n\n",
    "closed_at": "2011-09-27T17:43:21Z",
    "created_at": "2010-11-26T06:02:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add domains for permutation groups",
    "type": "issue",
    "updated_at": "2011-10-19T18:48:57Z",
    "url": "https://github.com/sagemath/sage/issues/10335",
    "user": "https://github.com/mwhansen"
}
```
Often, one wants to define a permutation group on a set other than ` {1,2,...,n} ` such as say ` {'a', 'b', 'c', 'd'} `.

For example,

```
sage: G = PermutationGroup([ [('c','d')], [('a','c')] ])
sage: G.orbit('a')
['a', 'c', 'd']
```

**Apply:**
1.  [attachment: trac_10335-permgroup_domain-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-rebase.patch.gz)

Depends on #10334

Assignee: **joyner**

CC:  sage-combinat jasonbhill @rbeezer @jdemeyer

Keywords: **sd31**

Author: **Mike Hansen, Jason Hill, David Loeffler**

Reviewer: **Robert Miller, Rob Beezer, Nicolas Borie, Nicolas M. Thiéry**

Merged: **sage-4.7.2.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/10335_





---

archive/issue_events_119844.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-11-26T06:02:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119844"
}
```



---

archive/issue_events_119845.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-11-26T06:02:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119845"
}
```



---

archive/issue_events_119846.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-11-26T06:02:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119846"
}
```



---

archive/issue_events_119847.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-11-26T06:02:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119847"
}
```



---

archive/issue_comments_098910.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis depends",
    "created_at": "2010-11-26T06:09:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98910",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'>Comment 1:</a>
This depends



---

archive/issue_comments_098911.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,3 +8,4 @@\n ['a', 'c', 'd']\n ```\n \n+Depends on #10334.\n``````\n",
    "created_at": "2010-11-26T06:09:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98911",
    "user": "https://github.com/mwhansen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,3 +8,4 @@
 ['a', 'c', 'd']
 ```
 
+Depends on #10334.
``````




---

archive/issue_events_119848.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-11-26T06:09:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119848"
}
```



---

archive/issue_comments_098912.json:
```json
{
    "body": "Reviewer: **Robert Miller**",
    "created_at": "2011-01-19T04:32:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98912",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **Robert Miller**



---

archive/issue_comments_098913.json:
```json
{
    "body": "Work Issues: **needs rebase**",
    "created_at": "2011-01-19T04:32:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98913",
    "user": "https://github.com/rlmill"
}
```

Work Issues: **needs rebase**



---

archive/issue_comments_098914.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nApplying to `sage-4.6.2.alpha0` on top of #10334, I get:\n\n```\napplying trac_10335-permgroup_domain-mh.patch\npatching file sage/groups/perm_gps/permgroup.py\nHunk #5 FAILED at 321\n1 out of 59 hunks FAILED -- saving rejects to file sage/groups/perm_gps/permgroup.py.rej\npatching file sage/groups/perm_gps/permgroup_named.py\nHunk #10 succeeded at 313 with fuzz 2 (offset 0 lines).\nHunk #11 succeeded at 315 with fuzz 1 (offset -49 lines).\npatch failed, unable to continue (try -v)\npatch failed, rejects left in working dir\nerrors during apply, please fix and refresh trac_10335-permgroup_domain-mh.patch\n```\n\nThe failure:\n\n```\n$ cat sage/groups/perm_gps/permgroup.py.rej\n--- permgroup.py\n+++ permgroup.py\n@@ -320,7 +322,7 @@\n         sage: G = PermutationGroup([[(1,2,3),(4,5)],[(3,4)]])\n         sage: TestSuite(G).run()\n     \"\"\"\n-    def __init__(self, gens=None, gap_group=None, canonicalize=True, category = None):\n+    def __init__(self, gens=None, gap_group=None, canonicalize=True, category=None, domain=None):\n         r\"\"\"\n         INPUT:\n\n```\n\n\nLooks trivial to fix, and I'm excited to see this feature in Sage. Thank you, Mike!",
    "created_at": "2011-01-19T04:32:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98914",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:3'>Comment 3:</a>
Applying to `sage-4.6.2.alpha0` on top of #10334, I get:

```
applying trac_10335-permgroup_domain-mh.patch
patching file sage/groups/perm_gps/permgroup.py
Hunk #5 FAILED at 321
1 out of 59 hunks FAILED -- saving rejects to file sage/groups/perm_gps/permgroup.py.rej
patching file sage/groups/perm_gps/permgroup_named.py
Hunk #10 succeeded at 313 with fuzz 2 (offset 0 lines).
Hunk #11 succeeded at 315 with fuzz 1 (offset -49 lines).
patch failed, unable to continue (try -v)
patch failed, rejects left in working dir
errors during apply, please fix and refresh trac_10335-permgroup_domain-mh.patch
```

The failure:

```
$ cat sage/groups/perm_gps/permgroup.py.rej
--- permgroup.py
+++ permgroup.py
@@ -320,7 +322,7 @@
         sage: G = PermutationGroup([[(1,2,3),(4,5)],[(3,4)]])
         sage: TestSuite(G).run()
     """
-    def __init__(self, gens=None, gap_group=None, canonicalize=True, category = None):
+    def __init__(self, gens=None, gap_group=None, canonicalize=True, category=None, domain=None):
         r"""
         INPUT:

```


Looks trivial to fix, and I'm excited to see this feature in Sage. Thank you, Mike!



---

archive/issue_comments_098915.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,4 @@\n ['a', 'c', 'd']\n ```\n \n-Depends on #10334.\n+Depends on #8359 and #10334.\n``````\n",
    "created_at": "2011-01-19T23:44:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98915",
    "user": "https://github.com/rlmill"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,4 @@
 ['a', 'c', 'd']
 ```
 
-Depends on #10334.
+Depends on #8359 and #10334.
``````




---

archive/issue_comments_098916.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThis was most likely a minor conflict with Rob's permgroup patches which were merged late in the 4.6.2 release cycle.\n\nOnly apply: trac_10335-permgroup_domain-mh.2.patch",
    "created_at": "2011-03-14T08:34:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98916",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:5'>Comment 5:</a>
This was most likely a minor conflict with Rob's permgroup patches which were merged late in the 4.6.2 release cycle.

Only apply: trac_10335-permgroup_domain-mh.2.patch



---

archive/issue_comments_098917.json:
```json
{
    "body": "Changed work issues from **needs rebase** to none",
    "created_at": "2011-03-14T08:34:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98917",
    "user": "https://github.com/nthiery"
}
```

Changed work issues from **needs rebase** to none



---

archive/issue_events_119849.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-03-14T13:01:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119849"
}
```



---

archive/issue_events_119850.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-03-14T13:01:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119850"
}
```



---

archive/issue_comments_098918.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAh, the test fail, and that's because the patch still did not commute perfectly with trac_8359-coxeter-groups-permutation-nt.patch. we need to have this patch add the category argument to `__init__` of permutation groups, and remove that part from 8359.\n\nMike: let me know if you can do it shortly, and then #8359 is fair game. Otherwise I'll handle it.",
    "created_at": "2011-03-14T13:01:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98918",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:6'>Comment 6:</a>
Ah, the test fail, and that's because the patch still did not commute perfectly with trac_8359-coxeter-groups-permutation-nt.patch. we need to have this patch add the category argument to `__init__` of permutation groups, and remove that part from 8359.

Mike: let me know if you can do it shortly, and then #8359 is fair game. Otherwise I'll handle it.



---

archive/issue_comments_098919.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,4 @@\n ['a', 'c', 'd']\n ```\n \n-Depends on #8359 and #10334.\n+Depends on #10334.\n``````\n",
    "created_at": "2011-03-16T10:22:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98919",
    "user": "https://github.com/mwhansen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,4 @@
 ['a', 'c', 'd']
 ```
 
-Depends on #8359 and #10334.
+Depends on #10334.
``````




---

archive/issue_comments_098920.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI've fixed the issue along with updating the patch at #8359.\n\nOnly apply: trac_10335-permgroup_domain-mh.patch",
    "created_at": "2011-03-16T10:22:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98920",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:8'>Comment 8:</a>
I've fixed the issue along with updating the patch at #8359.

Only apply: trac_10335-permgroup_domain-mh.patch



---

archive/issue_comments_098921.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThis looks real good, Mike.  I spent some time with this today.  I know its not done yet, but hopefully the following will be helpful.\n\nI applied the patch from sage-combinat for #8359 to 4.7.alpha1, as best I could tell (used patch from http://combinat.sagemath.org/hgwebdir.cgi/patches/rev/508d4942a6d2).  One hunk failure at sage/combinat/root_system/all.py.  Then #10334 had a hunk failure in sage/groups/perm_gps/permgroup.py and #10335 had a had failure in sage/categories/pushout.py.  All easy to fix by hand, and maybe a result of not getting the right thing from the sage-combinat server.\n\nPassed all tests in sage/groups/perm_gps, except two, so not too far off.  Both doctest failures look very minor, and a  result of routines I wrote that have been added recently.\n\n```\n**********************************************************************\nFile \"/sage/dev/devel/sage-main/sage/groups/perm_gps/permgroup.py\", line 1920:\n    sage: G.subgroups()\nExpected:\n    [Permutation Group with generators [()],\n     Permutation Group with generators [(2,3)],\n     Permutation Group with generators [(1,2)],\n     Permutation Group with generators [(1,3)],\n     Permutation Group with generators [(1,2,3)],\n     Permutation Group with generators [(1,3,2), (1,2)]]\nGot:\n    [Permutation Group with generators [()],\n     Permutation Group with generators [(2,3)],\n     Permutation Group with generators [(1,2)],\n     Permutation Group with generators [(1,3)],\n     Permutation Group with generators [(1,2,3)],\n     Permutation Group with generators [(1,2), (1,3,2)]]\n**********************************************************************\nFile \"/sage/dev/devel/sage-main/sage/groups/perm_gps/permgroup.py\", line 2082:\n    sage: A.cosets(S)\nExpected:\n    Traceback (most recent call last):\n    ...\n    ValueError: Subgroup of SymmetricGroup(3) generated by [(1,2)] is not a subgroup of AlternatingGroup(3)\nGot:\n    Traceback (most recent call last):\n      File \"/sage/dev/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/sage/dev/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/sage/dev/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_61[29]>\", line 1, in <module>\n        A.cosets(S)###line 2082:\n    sage: A.cosets(S)\n      File \"/sage/dev/local/lib/python/site-packages/sage/groups/perm_gps/permgroup.py\", line 2099, in cosets\n        raise ValueError(\"%s is not a subgroup of %s\" % (S, self))\n    ValueError: Subgroup of (Symmetric group of order 3! as a permutation group) generated by [(1,2)] is not a subgroup of Alternating group of order 3!/2 as a permutation group\n**********************************************************************\n```\n\nI think the stabilizer() routine needs some work.\n\n```\ntry:\n    postition = self._domain_to_gap[point]\nexcept KeyError:\n    return self.subgroup(gens=self.gens())\nreturn self.subgroup(gap_group=gap.Stabilizer(self, point))\n```\n\n\"position\" is mis-spelled as first defined, and in the return, I think \"point\" should be replaced with \"position\".  With these changes, this seems to work properly with a non-trivial domain in play.\n\nNot a comprehensive look, but hopefully of use.  Let me know what I can do to help (including assisting with documentation).  It'd be nice if there was a patch, or something, at #8359 explaining how best to apply that work.\n\nRob",
    "created_at": "2011-03-18T19:06:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98921",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:9'>Comment 9:</a>
This looks real good, Mike.  I spent some time with this today.  I know its not done yet, but hopefully the following will be helpful.

I applied the patch from sage-combinat for #8359 to 4.7.alpha1, as best I could tell (used patch from http://combinat.sagemath.org/hgwebdir.cgi/patches/rev/508d4942a6d2).  One hunk failure at sage/combinat/root_system/all.py.  Then #10334 had a hunk failure in sage/groups/perm_gps/permgroup.py and #10335 had a had failure in sage/categories/pushout.py.  All easy to fix by hand, and maybe a result of not getting the right thing from the sage-combinat server.

Passed all tests in sage/groups/perm_gps, except two, so not too far off.  Both doctest failures look very minor, and a  result of routines I wrote that have been added recently.

```
**********************************************************************
File "/sage/dev/devel/sage-main/sage/groups/perm_gps/permgroup.py", line 1920:
    sage: G.subgroups()
Expected:
    [Permutation Group with generators [()],
     Permutation Group with generators [(2,3)],
     Permutation Group with generators [(1,2)],
     Permutation Group with generators [(1,3)],
     Permutation Group with generators [(1,2,3)],
     Permutation Group with generators [(1,3,2), (1,2)]]
Got:
    [Permutation Group with generators [()],
     Permutation Group with generators [(2,3)],
     Permutation Group with generators [(1,2)],
     Permutation Group with generators [(1,3)],
     Permutation Group with generators [(1,2,3)],
     Permutation Group with generators [(1,2), (1,3,2)]]
**********************************************************************
File "/sage/dev/devel/sage-main/sage/groups/perm_gps/permgroup.py", line 2082:
    sage: A.cosets(S)
Expected:
    Traceback (most recent call last):
    ...
    ValueError: Subgroup of SymmetricGroup(3) generated by [(1,2)] is not a subgroup of AlternatingGroup(3)
Got:
    Traceback (most recent call last):
      File "/sage/dev/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/sage/dev/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/sage/dev/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_61[29]>", line 1, in <module>
        A.cosets(S)###line 2082:
    sage: A.cosets(S)
      File "/sage/dev/local/lib/python/site-packages/sage/groups/perm_gps/permgroup.py", line 2099, in cosets
        raise ValueError("%s is not a subgroup of %s" % (S, self))
    ValueError: Subgroup of (Symmetric group of order 3! as a permutation group) generated by [(1,2)] is not a subgroup of Alternating group of order 3!/2 as a permutation group
**********************************************************************
```

I think the stabilizer() routine needs some work.

```
try:
    postition = self._domain_to_gap[point]
except KeyError:
    return self.subgroup(gens=self.gens())
return self.subgroup(gap_group=gap.Stabilizer(self, point))
```

"position" is mis-spelled as first defined, and in the return, I think "point" should be replaced with "position".  With these changes, this seems to work properly with a non-trivial domain in play.

Not a comprehensive look, but hopefully of use.  Let me know what I can do to help (including assisting with documentation).  It'd be nice if there was a patch, or something, at #8359 explaining how best to apply that work.

Rob



---

archive/issue_comments_098922.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nHi Mike,\n\nFor the record: I rebased trac_10335-permgroup_domain-mh.patch on the sage-combinat server on top of #9949",
    "created_at": "2011-04-07T14:26:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98922",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:10'>Comment 10:</a>
Hi Mike,

For the record: I rebased trac_10335-permgroup_domain-mh.patch on the sage-combinat server on top of #9949



---

archive/issue_comments_098923.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nHi Mike!\n\nReplying to [@nthiery](#comment%3A10):\n> For the record: I rebased trac_10335-permgroup_domain-mh.patch on the sage-combinat server on top of #9949\n\nWith Rob we are both at Sage days 30, and would love to push permutation groups in. Rob is going to rebase #10334, and I'll finalize the review. #10335 itself needs rebase on 3.4.7, and has quite a few failing doctests, though most look harmless (it's just that the output of permutation groups have changed). Would there be any chance for you to work on this early this week?\n\nThanks!\n\nCheers,\n                   Nicolas",
    "created_at": "2011-05-02T19:02:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98923",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:11'>Comment 11:</a>
Hi Mike!

Replying to [@nthiery](#comment%3A10):
> For the record: I rebased trac_10335-permgroup_domain-mh.patch on the sage-combinat server on top of #9949

With Rob we are both at Sage days 30, and would love to push permutation groups in. Rob is going to rebase #10334, and I'll finalize the review. #10335 itself needs rebase on 3.4.7, and has quite a few failing doctests, though most look harmless (it's just that the output of permutation groups have changed). Would there be any chance for you to work on this early this week?

Thanks!

Cheers,
                   Nicolas



---

archive/issue_comments_098924.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nSure -- I'll try to get this taken care of tonight.",
    "created_at": "2011-05-04T02:10:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98924",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:12'>Comment 12:</a>
Sure -- I'll try to get this taken care of tonight.



---

archive/issue_comments_098925.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nI've fixed the doctest failures and updated the patch.",
    "created_at": "2011-05-29T05:33:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98925",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:13'>Comment 13:</a>
I've fixed the doctest failures and updated the patch.



---

archive/issue_events_119851.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-05-29T05:33:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119851"
}
```



---

archive/issue_events_119852.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-05-29T05:33:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119852"
}
```



---

archive/issue_events_119853.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-05-31T04:30:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119853"
}
```



---

archive/issue_events_119854.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-05-31T04:30:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119854"
}
```



---

archive/issue_comments_098926.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nI ran  `make ptestlong` and got quite a few failures.  I even tried it twice - not sure what is up.\n\n*  Many were in  sage/doc\n*  Many were simply the new version of `_repr_` for the symmetric group, and its subgroups\n*  The `latex()` method is putting a comma between the cycles within each generator.  Is this desired?\n\n```\nreturn \",\".join([\"(\" + \",\".join([latex(x) for x in cycle])+\")\" for cycle in self.cycle_tuples()])\n```\n    At a minimum it is causing doctest failures.\n* 73 failures in sage/rings/number_field/galois_group.py, likely all the same root cause\n\nSaw these in the patch itself:\n\n* a stray \"deprecated_function_alias\" in a docstring\n* <code>\\`\\`selfs\\`\\`</code> in a docstring",
    "created_at": "2011-05-31T04:30:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98926",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:14'>Comment 14:</a>
I ran  `make ptestlong` and got quite a few failures.  I even tried it twice - not sure what is up.

*  Many were in  sage/doc
*  Many were simply the new version of `_repr_` for the symmetric group, and its subgroups
*  The `latex()` method is putting a comma between the cycles within each generator.  Is this desired?

```
return ",".join(["(" + ",".join([latex(x) for x in cycle])+")" for cycle in self.cycle_tuples()])
```
    At a minimum it is causing doctest failures.
* 73 failures in sage/rings/number_field/galois_group.py, likely all the same root cause

Saw these in the patch itself:

* a stray "deprecated_function_alias" in a docstring
* <code>\`\`selfs\`\`</code> in a docstring



---

archive/issue_comments_098927.json:
```json
{
    "body": "Dependencies: **#10334**",
    "created_at": "2011-06-07T19:00:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98927",
    "user": "https://github.com/sagetrac-nborie"
}
```

Dependencies: **#10334**



---

archive/issue_comments_098928.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nI'm pretty sure that this patches addresses all of the above problems.\n\nApply trac_10335-permgroup_domain-mh.patch",
    "created_at": "2011-06-08T16:28:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98928",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:16'>Comment 16:</a>
I'm pretty sure that this patches addresses all of the above problems.

Apply trac_10335-permgroup_domain-mh.patch



---

archive/issue_events_119855.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-06-08T16:28:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119855"
}
```



---

archive/issue_events_119856.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-06-08T16:28:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119856"
}
```



---

archive/issue_comments_098929.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nFixed commit message.\n\nApply trac_10335-permgroup_domain-mh.patch",
    "created_at": "2011-06-08T18:59:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98929",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:17'>Comment 17:</a>
Fixed commit message.

Apply trac_10335-permgroup_domain-mh.patch



---

archive/issue_comments_098930.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nI was very close to set a positive review for this patch but I got this faillure :\n\n```\nnicolas@lancelot:/opt/sage/devel/sage-combinat$ sage -t sage/groups/perm_gps/permgroup.py \nsage -t  \"devel/sage-combinat/sage/groups/perm_gps/permgroup.py\"\n**********************************************************************\nFile \"/opt/sage/devel/sage-combinat/sage/groups/perm_gps/permgroup.py\", line 2550:\n    sage: G.isomorphism_to(H)\nExpected:\n    Permutation group morphism:\n      From: Permutation Group with generators [(2,3), (1,2,3)]\n      To:   Permutation Group with generators [(1,2,4), (1,4)]\n      Defn: [(2,3), (1,2,3)] -> [(2,4), (1,2,4)]\nGot:\n    Permutation group morphism:\n      From: Permutation Group with generators [(2,3), (1,2,3)]\n      To:   Permutation Group with generators [(1,2,4), (1,4)]\n      Defn: [(2,3), (1,2,3)] -> [(2,4), (1,4,2)]\n**********************************************************************\n1 items had failures:\n   1 of   9 in __main__.example_73\n```\nIs this a random test ? Is this test really unrandonized (sorry for such a word) ?\nI am running a 11.4 Ubuntu up to date on a macbook santa rosa 4,1. I got this faillure both using the sage combinat queue and a separate branch with only 10334 and 10335.\n\nAlso, in sage/groups/perm-gps/permgroup.py line 1119, there is still the micro typo : \npostition --> position\n\nAnyway, It is a very very nice work and improvement of permutation groups.",
    "created_at": "2011-06-08T20:00:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98930",
    "user": "https://github.com/sagetrac-nborie"
}
```

<a id='comment:18'>Comment 18:</a>
I was very close to set a positive review for this patch but I got this faillure :

```
nicolas@lancelot:/opt/sage/devel/sage-combinat$ sage -t sage/groups/perm_gps/permgroup.py 
sage -t  "devel/sage-combinat/sage/groups/perm_gps/permgroup.py"
**********************************************************************
File "/opt/sage/devel/sage-combinat/sage/groups/perm_gps/permgroup.py", line 2550:
    sage: G.isomorphism_to(H)
Expected:
    Permutation group morphism:
      From: Permutation Group with generators [(2,3), (1,2,3)]
      To:   Permutation Group with generators [(1,2,4), (1,4)]
      Defn: [(2,3), (1,2,3)] -> [(2,4), (1,2,4)]
Got:
    Permutation group morphism:
      From: Permutation Group with generators [(2,3), (1,2,3)]
      To:   Permutation Group with generators [(1,2,4), (1,4)]
      Defn: [(2,3), (1,2,3)] -> [(2,4), (1,4,2)]
**********************************************************************
1 items had failures:
   1 of   9 in __main__.example_73
```
Is this a random test ? Is this test really unrandonized (sorry for such a word) ?
I am running a 11.4 Ubuntu up to date on a macbook santa rosa 4,1. I got this faillure both using the sage combinat queue and a separate branch with only 10334 and 10335.

Also, in sage/groups/perm-gps/permgroup.py line 1119, there is still the micro typo : 
postition --> position

Anyway, It is a very very nice work and improvement of permutation groups.



---

archive/issue_comments_098931.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nThe failure you see is at #11427 .  I'll put up a new patch fixing the typo.",
    "created_at": "2011-06-08T20:03:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98931",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:19'>Comment 19:</a>
The failure you see is at #11427 .  I'll put up a new patch fixing the typo.



---

archive/issue_comments_098932.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nAttachment: **[trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)**\n\nNice shoot! I confirm that I use a lot the Gap database especially for Transitive Groups. The optional package was installed on my machine.\n\nThanks for your work and your rapidity for fixing this last typo.\n\nI give this patch a positive review.",
    "created_at": "2011-06-08T20:21:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98932",
    "user": "https://github.com/sagetrac-nborie"
}
```

<a id='comment:20'>Comment 20:</a>
Attachment: **[trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)**

Nice shoot! I confirm that I use a lot the Gap database especially for Transitive Groups. The optional package was installed on my machine.

Thanks for your work and your rapidity for fixing this last typo.

I give this patch a positive review.



---

archive/issue_comments_098933.json:
```json
{
    "body": "Changed reviewer from **Robert Miller** to **Robert Miller, Rob Beezer, Nicolas Borie**",
    "created_at": "2011-06-08T20:21:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98933",
    "user": "https://github.com/sagetrac-nborie"
}
```

Changed reviewer from **Robert Miller** to **Robert Miller, Rob Beezer, Nicolas Borie**



---

archive/issue_events_119857.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2011-06-08T20:21:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119857"
}
```



---

archive/issue_events_119858.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2011-06-08T20:21:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119858"
}
```



---

archive/issue_comments_098934.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,5 +7,3 @@\n sage: G.orbit('a')\n ['a', 'c', 'd']\n ```\n-\n-Depends on #10334.\n``````\n",
    "created_at": "2011-06-10T08:44:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98934",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,5 +7,3 @@
 sage: G.orbit('a')
 ['a', 'c', 'd']
 ```
-
-Depends on #10334.
``````




---

archive/issue_events_119859.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-10T08:44:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119859"
}
```



---

archive/issue_events_119860.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-10T08:44:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119860"
}
```



---

archive/issue_comments_098935.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nFor the record: when applying this patch with the sage-combinat queue on sage-4.7.2, I got an import loop, and used the extra patch to fix it.",
    "created_at": "2011-06-13T16:19:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98935",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:22'>Comment 22:</a>
For the record: when applying this patch with the sage-combinat queue on sage-4.7.2, I got an import loop, and used the extra patch to fix it.



---

archive/issue_comments_098936.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,3 +7,5 @@\n sage: G.orbit('a')\n ['a', 'c', 'd']\n ```\n+\n+\n``````\n",
    "created_at": "2011-06-13T16:19:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98936",
    "user": "https://github.com/nthiery"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,3 +7,5 @@
 sage: G.orbit('a')
 ['a', 'c', 'd']
 ```
+
+
``````




---

archive/issue_comments_098937.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nReplying to [@nthiery](#comment%3A22):\n> For the record: when applying this patch with the sage-combinat queue on sage-4.7.2, I got an import loop, and used the extra patch to fix it.\n\nOn 4.7.2?",
    "created_at": "2011-06-13T16:42:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98937",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:23'>Comment 23:</a>
Replying to [@nthiery](#comment%3A22):
> For the record: when applying this patch with the sage-combinat queue on sage-4.7.2, I got an import loop, and used the extra patch to fix it.

On 4.7.2?



---

archive/issue_comments_098938.json:
```json
{
    "body": "<a id='comment:24'>Comment 24:</a>\nReplying to [@mwhansen](#comment%3A23):\n> Replying to [@nthiery](#comment%3A22):\n> > For the record: when applying this patch with the sage-combinat queue on sage-4.7.2, I got an import loop, and used the extra patch to fix it.\n\n> \n> On 4.7.2?\n\nOops, 4.7.1 alpha2!\n\n Sage-Combinat is not that much in advance :-)",
    "created_at": "2011-06-13T17:24:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98938",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:24'>Comment 24:</a>
Replying to [@mwhansen](#comment%3A23):
> Replying to [@nthiery](#comment%3A22):
> > For the record: when applying this patch with the sage-combinat queue on sage-4.7.2, I got an import loop, and used the extra patch to fix it.

> 
> On 4.7.2?

Oops, 4.7.1 alpha2!

 Sage-Combinat is not that much in advance :-)



---

archive/issue_events_119861.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-14T17:31:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119861"
}
```



---

archive/issue_events_119862.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-14T17:31:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119862"
}
```



---

archive/issue_events_119863.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-14T17:31:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119863"
}
```



---

archive/issue_events_119864.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-14T17:31:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119864"
}
```



---

archive/issue_comments_098939.json:
```json
{
    "body": "<a id='comment:26'>Comment 26:</a>\nNew patch needs review.",
    "created_at": "2011-06-14T17:31:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98939",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:26'>Comment 26:</a>
New patch needs review.



---

archive/issue_comments_098940.json:
```json
{
    "body": "Changed keywords from **** to **sd31**",
    "created_at": "2011-06-14T19:52:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98940",
    "user": "https://github.com/rbeezer"
}
```

Changed keywords from **** to **sd31**



---

archive/issue_events_119865.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-06-14T19:52:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119865"
}
```



---

archive/issue_events_119866.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-06-14T19:52:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119866"
}
```



---

archive/issue_comments_098941.json:
```json
{
    "body": "<a id='comment:27'>Comment 27:</a>\nReplying to [@jdemeyer](#comment%3A26):\n> New patch needs review.\n\nAlmost passes all tests on 4.7.1.alpha2 and I get no import problems.\n\nBut the German documentation has old `_repr_` behavior.  Aargh.\n\n```\nsage -t  -long -force_lib ooks/devel/sage/doc/de/tutorial/tour_groups.rst # 1 doctests failed\nsage -t  -long -force_lib ooks/devel/sage/doc/de/tutorial/interfaces.rst # 1 doctests failed\n```\n\nPatch coming up, maybe.",
    "created_at": "2011-06-14T19:52:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98941",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:27'>Comment 27:</a>
Replying to [@jdemeyer](#comment%3A26):
> New patch needs review.

Almost passes all tests on 4.7.1.alpha2 and I get no import problems.

But the German documentation has old `_repr_` behavior.  Aargh.

```
sage -t  -long -force_lib ooks/devel/sage/doc/de/tutorial/tour_groups.rst # 1 doctests failed
sage -t  -long -force_lib ooks/devel/sage/doc/de/tutorial/interfaces.rst # 1 doctests failed
```

Patch coming up, maybe.



---

archive/issue_comments_098942.json:
```json
{
    "body": "Attachment: **[trac_10335_german_tutorial_doc.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335_german_tutorial_doc.patch.gz)**",
    "created_at": "2011-06-14T20:07:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98942",
    "user": "https://github.com/rbeezer"
}
```

Attachment: **[trac_10335_german_tutorial_doc.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335_german_tutorial_doc.patch.gz)**



---

archive/issue_events_119867.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-06-14T20:10:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119867"
}
```



---

archive/issue_events_119868.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-06-14T20:10:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119868"
}
```



---

archive/issue_comments_098943.json:
```json
{
    "body": "<a id='comment:28'>Comment 28:</a>\nGerman doc patch should solve above problems.  Gotta run - ticket needs \"apply\" block....",
    "created_at": "2011-06-14T20:10:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98943",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:28'>Comment 28:</a>
German doc patch should solve above problems.  Gotta run - ticket needs "apply" block....



---

archive/issue_comments_098944.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,7 @@\n ['a', 'c', 'd']\n ```\n \n-\n+**Apply:**\n+1.  [attachment: trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)\n+2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)\n+3.  [attachment: trac_10335_german_tutorial_doc.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335_german_tutorial_doc.patch.gz)\n``````\n",
    "created_at": "2011-06-15T03:34:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98944",
    "user": "https://github.com/rbeezer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,7 @@
 ['a', 'c', 'd']
 ```
 
-
+**Apply:**
+1.  [attachment: trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)
+2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)
+3.  [attachment: trac_10335_german_tutorial_doc.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335_german_tutorial_doc.patch.gz)
``````




---

archive/issue_comments_098945.json:
```json
{
    "body": "<a id='comment:30'>Comment 30:</a>\nNote that [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz) should have its commit message changed.",
    "created_at": "2011-06-15T15:26:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98945",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:30'>Comment 30:</a>
Note that [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz) should have its commit message changed.



---

archive/issue_events_119869.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-16T21:33:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119869"
}
```



---

archive/issue_events_119870.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-16T21:33:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119870"
}
```



---

archive/issue_comments_098946.json:
```json
{
    "body": "Merge only if needed",
    "created_at": "2011-06-16T23:48:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98946",
    "user": "https://github.com/nthiery"
}
```

Merge only if needed



---

archive/issue_comments_098947.json:
```json
{
    "body": "<a id='comment:32'>Comment 32:</a>\nAttachment: **[trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)**\n\nReplying to [@jdemeyer](#comment%3A30):\n> Note that [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz) should have its commit message changed.\n\nOops, done.",
    "created_at": "2011-06-16T23:48:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98947",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:32'>Comment 32:</a>
Attachment: **[trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)**

Replying to [@jdemeyer](#comment%3A30):
> Note that [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz) should have its commit message changed.

Oops, done.



---

archive/issue_events_119871.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-06-16T23:49:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119871"
}
```



---

archive/issue_events_119872.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-06-16T23:49:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119872"
}
```



---

archive/issue_comments_098948.json:
```json
{
    "body": "<a id='comment:33'>Comment 33:</a>\nReplying to [@rbeezer](#comment%3A28):\n> German doc patch should solve above problems.  Gotta run - ticket needs \"apply\" block....\n\nSounds good to me assuming the patch bot is happy.",
    "created_at": "2011-06-16T23:49:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98948",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:33'>Comment 33:</a>
Replying to [@rbeezer](#comment%3A28):
> German doc patch should solve above problems.  Gotta run - ticket needs "apply" block....

Sounds good to me assuming the patch bot is happy.



---

archive/issue_events_119873.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-19T09:01:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119873"
}
```



---

archive/issue_events_119874.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-19T09:01:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119874"
}
```



---

archive/issue_comments_098949.json:
```json
{
    "body": "<a id='comment:34'>Comment 34:</a>\nThe Russian tutorial (#9378) also needs to be fixed:\n\n```\nsage -t  -force_lib devel/sage/doc/ru/tutorial/interfaces.rst\n**********************************************************************\nFile \"/mnt/usb1/scratch/jdemeyer/merger/sage-4.7.2.alpha0/devel/sage-main/doc/ru/tutorial/interfaces.rst\", line 167:\n    sage: G.center()\nExpected:\n    Permutation Group with generators [()]\nGot:\n    Subgroup of (Permutation Group with generators [(3,4), (1,2,3)(4,5)]) generated by [()]\n**********************************************************************\n```\n\n```\nsage -t  -force_lib devel/sage/doc/ru/tutorial/tour_groups.rst\n**********************************************************************\nFile \"/mnt/usb1/scratch/jdemeyer/merger/sage-4.7.2.alpha0/devel/sage-main/doc/ru/tutorial/tour_groups.rst\", line 24:\n    sage: G.center()\nExpected:\n    Permutation Group with generators [()]\nGot:\n    Subgroup of (Permutation Group with generators [(3,4), (1,2,3)(4,5)]) generated by [()]\n**********************************************************************\n```",
    "created_at": "2011-06-19T09:01:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98949",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:34'>Comment 34:</a>
The Russian tutorial (#9378) also needs to be fixed:

```
sage -t  -force_lib devel/sage/doc/ru/tutorial/interfaces.rst
**********************************************************************
File "/mnt/usb1/scratch/jdemeyer/merger/sage-4.7.2.alpha0/devel/sage-main/doc/ru/tutorial/interfaces.rst", line 167:
    sage: G.center()
Expected:
    Permutation Group with generators [()]
Got:
    Subgroup of (Permutation Group with generators [(3,4), (1,2,3)(4,5)]) generated by [()]
**********************************************************************
```

```
sage -t  -force_lib devel/sage/doc/ru/tutorial/tour_groups.rst
**********************************************************************
File "/mnt/usb1/scratch/jdemeyer/merger/sage-4.7.2.alpha0/devel/sage-main/doc/ru/tutorial/tour_groups.rst", line 24:
    sage: G.center()
Expected:
    Permutation Group with generators [()]
Got:
    Subgroup of (Permutation Group with generators [(3,4), (1,2,3)(4,5)]) generated by [()]
**********************************************************************
```



---

archive/issue_comments_098950.json:
```json
{
    "body": "German and Russian language fixes combined",
    "created_at": "2011-06-20T20:57:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98950",
    "user": "https://github.com/rbeezer"
}
```

German and Russian language fixes combined



---

archive/issue_comments_098951.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,4 +11,4 @@\n **Apply:**\n 1.  [attachment: trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)\n 2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)\n-3.  [attachment: trac_10335_german_tutorial_doc.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335_german_tutorial_doc.patch.gz)\n+3.  [attachment: trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)\n``````\n",
    "created_at": "2011-06-20T21:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98951",
    "user": "https://github.com/rbeezer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,4 +11,4 @@
 **Apply:**
 1.  [attachment: trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)
 2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)
-3.  [attachment: trac_10335_german_tutorial_doc.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335_german_tutorial_doc.patch.gz)
+3.  [attachment: trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)
``````




---

archive/issue_comments_098952.json:
```json
{
    "body": "<a id='comment:35'>Comment 35:</a>\nAttachment: **[trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)**\n\nReplying to [@jdemeyer](#comment%3A34):\n> The Russian tutorial (#9378) also needs to be fixed:\n\nApplied #9378 and the two main patches here.  (I do not understand what \"merge if needed\" is suppose to mean - so I included that).  Rolled the Russian fixes up with the German fixes into one patch.\n\nPasses all tests in  doc  directory.\n\nCould we *please* review and merge this before anymore new languages surface?  ;-) ;-)\n\nRob",
    "created_at": "2011-06-20T21:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98952",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:35'>Comment 35:</a>
Attachment: **[trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)**

Replying to [@jdemeyer](#comment%3A34):
> The Russian tutorial (#9378) also needs to be fixed:

Applied #9378 and the two main patches here.  (I do not understand what "merge if needed" is suppose to mean - so I included that).  Rolled the Russian fixes up with the German fixes into one patch.

Passes all tests in  doc  directory.

Could we *please* review and merge this before anymore new languages surface?  ;-) ;-)

Rob



---

archive/issue_events_119875.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-06-20T21:05:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119875"
}
```



---

archive/issue_events_119876.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-06-20T21:05:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119876"
}
```



---

archive/issue_events_119877.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-06-22T11:18:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119877"
}
```



---

archive/issue_events_119878.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-06-22T11:18:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119878"
}
```



---

archive/issue_comments_098953.json:
```json
{
    "body": "<a id='comment:36'>Comment 36:</a>\nReplying to [@rbeezer](#comment%3A35):\n> Applied #9378 and the two main patches here.  (I do not understand what \"merge if needed\" is suppose to mean - so I included that). \n\nSee comment 23 above.\n\n> Rolled the Russian fixes up with the German fixes into one patch.\n> \n> Passes all tests in  doc  directory.\n> \n> Could we *please* review and merge this before anymore new languages surface?  ;-) ;-)\n\nI checked your new patch, and am happy with it. Back to positive review. Thanks!",
    "created_at": "2011-06-22T11:18:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98953",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:36'>Comment 36:</a>
Replying to [@rbeezer](#comment%3A35):
> Applied #9378 and the two main patches here.  (I do not understand what "merge if needed" is suppose to mean - so I included that). 

See comment 23 above.

> Rolled the Russian fixes up with the German fixes into one patch.
> 
> Passes all tests in  doc  directory.
> 
> Could we *please* review and merge this before anymore new languages surface?  ;-) ;-)

I checked your new patch, and am happy with it. Back to positive review. Thanks!



---

archive/issue_comments_098954.json:
```json
{
    "body": "Changed reviewer from **Robert Miller, Rob Beezer, Nicolas Borie** to **Robert Miller, Rob Beezer, Nicolas Borie, Nicolas M. Thi\u00e9ry**",
    "created_at": "2011-06-22T11:18:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98954",
    "user": "https://github.com/nthiery"
}
```

Changed reviewer from **Robert Miller, Rob Beezer, Nicolas Borie** to **Robert Miller, Rob Beezer, Nicolas Borie, Nicolas M. Thiéry**



---

archive/issue_events_119879.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-22T12:49:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119879"
}
```



---

archive/issue_events_119880.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-22T12:49:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119880"
}
```



---

archive/issue_comments_098955.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha0**",
    "created_at": "2011-07-22T12:49:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98955",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.2.alpha0**



---

archive/issue_comments_098956.json:
```json
{
    "body": "Changed merged from **sage-4.7.2.alpha0** to none",
    "created_at": "2011-08-01T11:38:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98956",
    "user": "https://github.com/jdemeyer"
}
```

Changed merged from **sage-4.7.2.alpha0** to none



---

archive/issue_comments_098957.json:
```json
{
    "body": "<a id='comment:38'>Comment 38:</a>\nUnfortunately, there is a doctest error on Solaris (to be more precise, the buildbot machine mark2):\n\n```\nsage -t -long  -force_lib devel/sage/sage/groups/perm_gps/permgroup.py\n**********************************************************************\nFile \"/home/buildbot/build/sage/mark-1/mark_full/build/sage-4.7.2.alpha0/devel/sage-main/sage/groups/perm_gps/permgroup.py\", line 811:\n    sage: G.gens_small()\nExpected:\n    [('a','b'), ('a','b','c')]\nGot:\n    [('b','c'), ('a','b','c')]\n**********************************************************************\n```",
    "created_at": "2011-08-01T11:38:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98957",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:38'>Comment 38:</a>
Unfortunately, there is a doctest error on Solaris (to be more precise, the buildbot machine mark2):

```
sage -t -long  -force_lib devel/sage/sage/groups/perm_gps/permgroup.py
**********************************************************************
File "/home/buildbot/build/sage/mark-1/mark_full/build/sage-4.7.2.alpha0/devel/sage-main/sage/groups/perm_gps/permgroup.py", line 811:
    sage: G.gens_small()
Expected:
    [('a','b'), ('a','b','c')]
Got:
    [('b','c'), ('a','b','c')]
**********************************************************************
```



---

archive/issue_comments_098958.json:
```json
{
    "body": "Work Issues: **Solaris doctest error**",
    "created_at": "2011-08-01T11:38:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98958",
    "user": "https://github.com/jdemeyer"
}
```

Work Issues: **Solaris doctest error**



---

archive/issue_comments_098959.json:
```json
{
    "body": "<a id='comment:39'>Comment 39:</a>\nWhat's the best way to test a fix on that machine?  Do I have to log into mark2 and do a full build or is there a way to access that build?",
    "created_at": "2011-08-01T18:05:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98959",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:39'>Comment 39:</a>
What's the best way to test a fix on that machine?  Do I have to log into mark2 and do a full build or is there a way to access that build?



---

archive/issue_comments_098960.json:
```json
{
    "body": "<a id='comment:40'>Comment 40:</a>\nPing.",
    "created_at": "2011-08-02T18:50:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98960",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:40'>Comment 40:</a>
Ping.



---

archive/issue_comments_098961.json:
```json
{
    "body": "<a id='comment:41'>Comment 41:</a>\njdemeyer:\n\n```\nWhat's the best way to test a fix on that machine? Do I have to log into mark2 and do a full build or is there a way to access that build?\n```",
    "created_at": "2011-08-16T19:27:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98961",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:41'>Comment 41:</a>
jdemeyer:

```
What's the best way to test a fix on that machine? Do I have to log into mark2 and do a full build or is there a way to access that build?
```



---

archive/issue_comments_098962.json:
```json
{
    "body": "Attachment: **[trac_10335-solaris_issue.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-solaris_issue.patch.gz)**\n\napply over previous patches",
    "created_at": "2011-08-25T09:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98962",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[trac_10335-solaris_issue.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-solaris_issue.patch.gz)**

apply over previous patches



---

archive/issue_comments_098963.json:
```json
{
    "body": "<a id='comment:42'>Comment 42:</a>\nI don't have access to a Solaris machine to test this on; but, looking at the code, it's clear that (a) this Sage function just wraps the corresponding one in Gap, (b) the answer returned on Solaris is just as much a valid answer as that returned on other systems, and (c) there is already a perfectly good doctest for that function.\n\nI strongly suggest that we change this doctest to something less sensitive, given that there is a huge pile of other code waiting on this. I have just uploaded a patch which does this.",
    "created_at": "2011-08-25T09:20:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98963",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:42'>Comment 42:</a>
I don't have access to a Solaris machine to test this on; but, looking at the code, it's clear that (a) this Sage function just wraps the corresponding one in Gap, (b) the answer returned on Solaris is just as much a valid answer as that returned on other systems, and (c) there is already a perfectly good doctest for that function.

I strongly suggest that we change this doctest to something less sensitive, given that there is a huge pile of other code waiting on this. I have just uploaded a patch which does this.



---

archive/issue_comments_098964.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -12,3 +12,4 @@\n 1.  [attachment: trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)\n 2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)\n 3.  [attachment: trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)\n+4.  [attachment: trac_10335-solaris_issue.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-solaris_issue.patch.gz)\n``````\n",
    "created_at": "2011-08-25T09:20:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98964",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -12,3 +12,4 @@
 1.  [attachment: trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)
 2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)
 3.  [attachment: trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)
+4.  [attachment: trac_10335-solaris_issue.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-solaris_issue.patch.gz)
``````




---

archive/issue_events_119881.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-08-25T09:20:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119881"
}
```



---

archive/issue_comments_098965.json:
```json
{
    "body": "apply over previous patches",
    "created_at": "2011-09-11T15:19:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98965",
    "user": "https://github.com/loefflerd"
}
```

apply over previous patches



---

archive/issue_comments_098966.json:
```json
{
    "body": "<a id='comment:43'>Comment 43:</a>\nAttachment: **[trac_10335-updates.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-updates.patch.gz)**\n\nThis has now bitrotted: several doctests fail in trivial ways on sage 4.7.2.alpha2. I've just uploaded a patch that fixes these.",
    "created_at": "2011-09-11T15:20:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98966",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:43'>Comment 43:</a>
Attachment: **[trac_10335-updates.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-updates.patch.gz)**

This has now bitrotted: several doctests fail in trivial ways on sage 4.7.2.alpha2. I've just uploaded a patch that fixes these.



---

archive/issue_comments_098967.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -13,3 +13,4 @@\n 2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)\n 3.  [attachment: trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)\n 4.  [attachment: trac_10335-solaris_issue.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-solaris_issue.patch.gz)\n+5.  [attachment: trac_10335-updates.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-updates.patch.gz)\n``````\n",
    "created_at": "2011-09-11T15:20:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98967",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -13,3 +13,4 @@
 2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)
 3.  [attachment: trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)
 4.  [attachment: trac_10335-solaris_issue.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-solaris_issue.patch.gz)
+5.  [attachment: trac_10335-updates.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-updates.patch.gz)
``````




---

archive/issue_comments_098968.json:
```json
{
    "body": "Changed work issues from **Solaris doctest error** to none",
    "created_at": "2011-09-18T08:09:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98968",
    "user": "https://github.com/loefflerd"
}
```

Changed work issues from **Solaris doctest error** to none



---

archive/issue_comments_098969.json:
```json
{
    "body": "<a id='comment:44'>Comment 44:</a>\nI've tested this on the Sun machine \"mark\" on skynet, and all doctests pass (except for a pre-existing error in `matrix_double_dense` which has nothing to do with this patch. Can someone please visually check that the two most recent patches [and [attachment: trac_10335-updates.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-updates.patch.gz)(https://github.com/sagemath/sage/files/ticket10335/9ed8d2fa5296d89b689770d6d2559b72.gz) are morally sound? I'd be really disappointed if this bitrotted yet again.",
    "created_at": "2011-09-18T08:09:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98969",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:44'>Comment 44:</a>
I've tested this on the Sun machine "mark" on skynet, and all doctests pass (except for a pre-existing error in `matrix_double_dense` which has nothing to do with this patch. Can someone please visually check that the two most recent patches [and [attachment: trac_10335-updates.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-updates.patch.gz)(https://github.com/sagemath/sage/files/ticket10335/9ed8d2fa5296d89b689770d6d2559b72.gz) are morally sound? I'd be really disappointed if this bitrotted yet again.



---

archive/issue_comments_098970.json:
```json
{
    "body": "<a id='comment:45'>Comment 45:</a>\nFWIW, the \"pre-existing error in `matrix_double_dense`\" is a known issue coming from #10795, which has apparently been de-merged from the next release for this reason.",
    "created_at": "2011-09-18T08:36:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98970",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:45'>Comment 45:</a>
FWIW, the "pre-existing error in `matrix_double_dense`" is a known issue coming from #10795, which has apparently been de-merged from the next release for this reason.



---

archive/issue_events_119882.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-09-18T16:15:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119882"
}
```



---

archive/issue_events_119883.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-09-18T16:15:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119883"
}
```



---

archive/issue_comments_098971.json:
```json
{
    "body": "<a id='comment:46'>Comment 46:</a>\nDavid's two recent patches look good to me, and update from a point where this had a positive review.\n\nApplies, builds, passes long tests on 4.7.2.alpha2 and 64-bit Ubuntu 11.04.  So, back to positive review with a hope it merges it OK, finally.",
    "created_at": "2011-09-18T16:15:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98971",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:46'>Comment 46:</a>
David's two recent patches look good to me, and update from a point where this had a positive review.

Applies, builds, passes long tests on 4.7.2.alpha2 and 64-bit Ubuntu 11.04.  So, back to positive review with a hope it merges it OK, finally.



---

archive/issue_comments_098972.json:
```json
{
    "body": "Changed author from **Mike Hansen** to **Mike Hansen, David Loeffler**",
    "created_at": "2011-09-18T16:15:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98972",
    "user": "https://github.com/rbeezer"
}
```

Changed author from **Mike Hansen** to **Mike Hansen, David Loeffler**



---

archive/issue_comments_098973.json:
```json
{
    "body": "<a id='comment:47'>Comment 47:</a>\nI'm not sure whether removing a doctest is the proper way to fix a doctest error... ;-)",
    "created_at": "2011-09-18T17:33:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98973",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:47'>Comment 47:</a>
I'm not sure whether removing a doctest is the proper way to fix a doctest error... ;-)



---

archive/issue_comments_098974.json:
```json
{
    "body": "<a id='comment:48'>Comment 48:</a>\nReplying to [@nexttime](#comment%3A47):\n> I'm not sure whether removing a doctest is the proper way to fix a doctest error... ;-)\n\nI agree.  But in this case, I agree with David's argument.  This patch implements a highly desirable feature with many doctests.  And if I understand right, GAP uses some probabilistic algorithms, so we could tie ourselves in knots trying to rewrite this doctest.\n\nMy understanding (and some grep'ing appears to confirm) is that this is a **new** test.  Would the patch have gone in without it if it had never appeared?  I think so.  If an **existing** test were failing, I'd be a lot less cavalier about dropping it.\n\nAnyway, my $0.02 worth.",
    "created_at": "2011-09-18T18:04:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98974",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:48'>Comment 48:</a>
Replying to [@nexttime](#comment%3A47):
> I'm not sure whether removing a doctest is the proper way to fix a doctest error... ;-)

I agree.  But in this case, I agree with David's argument.  This patch implements a highly desirable feature with many doctests.  And if I understand right, GAP uses some probabilistic algorithms, so we could tie ourselves in knots trying to rewrite this doctest.

My understanding (and some grep'ing appears to confirm) is that this is a **new** test.  Would the patch have gone in without it if it had never appeared?  I think so.  If an **existing** test were failing, I'd be a lot less cavalier about dropping it.

Anyway, my $0.02 worth.



---

archive/issue_comments_098975.json:
```json
{
    "body": "<a id='comment:49'>Comment 49:</a>\nI think the correct fix would to be to add one of the `current_randstate().set_seed_gap()` lines to the beginning of that function.",
    "created_at": "2011-09-18T18:10:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98975",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:49'>Comment 49:</a>
I think the correct fix would to be to add one of the `current_randstate().set_seed_gap()` lines to the beginning of that function.



---

archive/issue_comments_098976.json:
```json
{
    "body": "<a id='comment:50'>Comment 50:</a>\nReplying to [@mwhansen](#comment%3A49):\n> I think the correct fix would to be to add one of the `current_randstate().set_seed_gap()` lines to the beginning of that function.\n\nOr otherwise at least *tag* the doctest with `# random`, `# not tested`, `# platform-specific`(?) or whatever, since doing so *documents* the different behaviour.",
    "created_at": "2011-09-18T19:11:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98976",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:50'>Comment 50:</a>
Replying to [@mwhansen](#comment%3A49):
> I think the correct fix would to be to add one of the `current_randstate().set_seed_gap()` lines to the beginning of that function.

Or otherwise at least *tag* the doctest with `# random`, `# not tested`, `# platform-specific`(?) or whatever, since doing so *documents* the different behaviour.



---

archive/issue_comments_098977.json:
```json
{
    "body": "<a id='comment:51'>Comment 51:</a>\nI agree that in an ideal world it would be better to have another doctest for this function, but as Rob points out, there was no such doctest before this ticket anyway. So it seems absurd to hold up all of the other new functionality added by this ticket, and the two other positively-reviewed tickets depending on it, for such a reason. \n\nIf it matters that much, perhaps we should have a new ticket \"improve doctests for wrapper of Gap function xxx\".",
    "created_at": "2011-09-18T21:17:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98977",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:51'>Comment 51:</a>
I agree that in an ideal world it would be better to have another doctest for this function, but as Rob points out, there was no such doctest before this ticket anyway. So it seems absurd to hold up all of the other new functionality added by this ticket, and the two other positively-reviewed tickets depending on it, for such a reason. 

If it matters that much, perhaps we should have a new ticket "improve doctests for wrapper of Gap function xxx".



---

archive/issue_comments_098978.json:
```json
{
    "body": "<a id='comment:52'>Comment 52:</a>\nI agree with leif that adding a simple \"# random\" is an easy solution which at least documents the problem.",
    "created_at": "2011-09-19T09:25:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98978",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:52'>Comment 52:</a>
I agree with leif that adding a simple "# random" is an easy solution which at least documents the problem.



---

archive/issue_comments_098979.json:
```json
{
    "body": "<a id='comment:53'>Comment 53:</a>\nAaargh! Do what you like, just *MERGE IT SOMEHOW*! If we miss the boat with this one again, and consequently #11422, #11598, #5048, #11601 and #10546 end up still trapped in purgatory, I will have to think very seriously about whether working on Sage in future is a productive use of my time.",
    "created_at": "2011-09-19T09:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98979",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:53'>Comment 53:</a>
Aaargh! Do what you like, just *MERGE IT SOMEHOW*! If we miss the boat with this one again, and consequently #11422, #11598, #5048, #11601 and #10546 end up still trapped in purgatory, I will have to think very seriously about whether working on Sage in future is a productive use of my time.



---

archive/issue_comments_098980.json:
```json
{
    "body": "<a id='comment:54'>Comment 54:</a>\nReplying to [@loefflerd](#comment%3A53):\n> Aaargh! Do what you like, just *MERGE IT SOMEHOW*! If we miss the boat with this one again, and consequently #11422, #11598, #5048, #11601 and #10546 end up still trapped in purgatory, I will have to think very seriously about whether working on Sage in future is a productive use of my time.\n\nWell, to be honest, it's not my problem that this ticket bit-rottened. I started merging tickets \"officially\" into alpha3 about two weeks ago IIRC (including some you participated in); this ticket got positive review 20 hours ago.\n\nI agree that it's a pity, not just because other already positively reviewed tickets -- three of the other ones you mentioned still need review -- depend on this one, but as I told Rob yesterday, it would have to get rebased anyway, because of (at least) #11749, and I'm not going to back the latter out (or try to reapply it on top of tickets that haven't had positive review recently, again having to manually resolve conflicts and errors, some showing up only later during testing).\n\nWe haven't yet talked about the release plans for the final Sage 4.7.2, but I think there'll still be an alpha4 before we go into feature freeze, as there are other important things that still have to get fixed and should be solved in a final.\n\nSo IMHO it would be best to wait for the alpha3, hopefully released within the next days, and then rebase this ticket (and if necessary the others) on that.\n\n[I didn't touch the tickets having milestone \"sage-pending\" (including #11422 and #11598) at all, since I don't know why Jeroen put them there, although I have to admit I also didn't try to find out, mainly because of lack of time. I early did ping the authors and reviewers of all other tickets that were holding up other positively reviewed ones though, with some success.]\n\n\n\n\nW.r.t. the doctest with varying output:\n\nIt would IMHO be best to give the alternative, also correct result as an example, too, marking both `# not tested`, with an appropriate comment.\n\n(And perhaps add a real doctest which tests a property common to both.)",
    "created_at": "2011-09-19T13:04:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98980",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:54'>Comment 54:</a>
Replying to [@loefflerd](#comment%3A53):
> Aaargh! Do what you like, just *MERGE IT SOMEHOW*! If we miss the boat with this one again, and consequently #11422, #11598, #5048, #11601 and #10546 end up still trapped in purgatory, I will have to think very seriously about whether working on Sage in future is a productive use of my time.

Well, to be honest, it's not my problem that this ticket bit-rottened. I started merging tickets "officially" into alpha3 about two weeks ago IIRC (including some you participated in); this ticket got positive review 20 hours ago.

I agree that it's a pity, not just because other already positively reviewed tickets -- three of the other ones you mentioned still need review -- depend on this one, but as I told Rob yesterday, it would have to get rebased anyway, because of (at least) #11749, and I'm not going to back the latter out (or try to reapply it on top of tickets that haven't had positive review recently, again having to manually resolve conflicts and errors, some showing up only later during testing).

We haven't yet talked about the release plans for the final Sage 4.7.2, but I think there'll still be an alpha4 before we go into feature freeze, as there are other important things that still have to get fixed and should be solved in a final.

So IMHO it would be best to wait for the alpha3, hopefully released within the next days, and then rebase this ticket (and if necessary the others) on that.

[I didn't touch the tickets having milestone "sage-pending" (including #11422 and #11598) at all, since I don't know why Jeroen put them there, although I have to admit I also didn't try to find out, mainly because of lack of time. I early did ping the authors and reviewers of all other tickets that were holding up other positively reviewed ones though, with some success.]




W.r.t. the doctest with varying output:

It would IMHO be best to give the alternative, also correct result as an example, too, marking both `# not tested`, with an appropriate comment.

(And perhaps add a real doctest which tests a property common to both.)



---

archive/issue_comments_098981.json:
```json
{
    "body": "<a id='comment:55'>Comment 55:</a>\nReplying to [@nexttime](#comment%3A54):\n> I didn't touch the tickets having milestone \"sage-pending\" (including #11422 and #11598) at all, since I don't know why Jeroen put them there\n\nI put them there because of the dependency on this ticket, no other reason.",
    "created_at": "2011-09-19T13:22:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98981",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:55'>Comment 55:</a>
Replying to [@nexttime](#comment%3A54):
> I didn't touch the tickets having milestone "sage-pending" (including #11422 and #11598) at all, since I don't know why Jeroen put them there

I put them there because of the dependency on this ticket, no other reason.



---

archive/issue_comments_098982.json:
```json
{
    "body": "<a id='comment:56'>Comment 56:</a>\nReplying to [@loefflerd](#comment%3A53):\n> Aaargh! Do what you like, just *MERGE IT SOMEHOW*!\n\nLeif - thanks for the help with this one.  With classes starting, I really wasn't in a position to be chasing the evolving alpha, so I had to pass on that offer, but appreciate the willingness.\n\nDavid - How about the following?\n\n1.  Wait for alpha3 (which sounds big already).\n\n2.  I mark the disputed test `# random`.\n\n3.  I rebase on alpha3 in preparation for alpha4.\n\n4.  You review (2) and (3).\n\n5.  We quickly identify if the tickets in Permutation Group Purgatory needing changes/rebasing.\n\nLemme know and I'll carve out time to make it happen.\n\nRob",
    "created_at": "2011-09-19T14:40:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98982",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:56'>Comment 56:</a>
Replying to [@loefflerd](#comment%3A53):
> Aaargh! Do what you like, just *MERGE IT SOMEHOW*!

Leif - thanks for the help with this one.  With classes starting, I really wasn't in a position to be chasing the evolving alpha, so I had to pass on that offer, but appreciate the willingness.

David - How about the following?

1.  Wait for alpha3 (which sounds big already).

2.  I mark the disputed test `# random`.

3.  I rebase on alpha3 in preparation for alpha4.

4.  You review (2) and (3).

5.  We quickly identify if the tickets in Permutation Group Purgatory needing changes/rebasing.

Lemme know and I'll carve out time to make it happen.

Rob



---

archive/issue_comments_098983.json:
```json
{
    "body": "<a id='comment:57'>Comment 57:</a>\nThat sounds like a good solution -- thanks for volunteering to do (2) and (3). I can take care of (5). \n\nMy apologies, particularly to Leif, for my outburst earlier.",
    "created_at": "2011-09-19T14:51:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98983",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:57'>Comment 57:</a>
That sounds like a good solution -- thanks for volunteering to do (2) and (3). I can take care of (5). 

My apologies, particularly to Leif, for my outburst earlier.



---

archive/issue_events_119884.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-09-19T15:22:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119884"
}
```



---

archive/issue_events_119885.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-09-19T15:22:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119885"
}
```



---

archive/issue_comments_098984.json:
```json
{
    "body": "<a id='comment:58'>Comment 58:</a>\nReplying to [@loefflerd](#comment%3A57):\n> That sounds like a good solution -- thanks for volunteering to do (2) and (3). I can take care of (5). \n\nGreat!  We have a plan. Thanks for taking (5).  \n\nLeif and/or Jeroen - feel free to ping us if we need to move quickly on something.\n\nAt the ready,\nRob",
    "created_at": "2011-09-19T15:22:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98984",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:58'>Comment 58:</a>
Replying to [@loefflerd](#comment%3A57):
> That sounds like a good solution -- thanks for volunteering to do (2) and (3). I can take care of (5). 

Great!  We have a plan. Thanks for taking (5).  

Leif and/or Jeroen - feel free to ping us if we need to move quickly on something.

At the ready,
Rob



---

archive/issue_comments_098985.json:
```json
{
    "body": "<a id='comment:59'>Comment 59:</a>\nReplying to [@rbeezer](#comment%3A56):\n> 1.  Wait for alpha3 (which sounds big already). \n\n> [...] \n\n> 3.  I rebase on alpha3 in preparation for alpha4. \n\n> [...] \n\n> 5.  We quickly identify if the tickets in Permutation Group Purgatory needing changes / rebasing.\n\n\n\nFor the impatient, an updated tarball and source tree for upgrading can be found in\n\n [http://sage.math.washington.edu/home/leif/Sage/release/sage-4.7.2.alpha3-prerelease/](http://sage.math.washington.edu/home/leif/Sage/release/sage-4.7.2.alpha3-prerelease/).\n\nI don't expect further changes to the alpha3 release, except for a few only fixing some doctest errors due to numerical noise below\n* `sage/rings/polynomial/` and\n* `sage/matrix/`,\nand perhaps some clean-up like adding ticket numbers to commit messages lacking them, so it should be safe to (re)base patches on this version.",
    "created_at": "2011-09-20T12:47:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98985",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:59'>Comment 59:</a>
Replying to [@rbeezer](#comment%3A56):
> 1.  Wait for alpha3 (which sounds big already). 

> [...] 

> 3.  I rebase on alpha3 in preparation for alpha4. 

> [...] 

> 5.  We quickly identify if the tickets in Permutation Group Purgatory needing changes / rebasing.



For the impatient, an updated tarball and source tree for upgrading can be found in

 [http://sage.math.washington.edu/home/leif/Sage/release/sage-4.7.2.alpha3-prerelease/](http://sage.math.washington.edu/home/leif/Sage/release/sage-4.7.2.alpha3-prerelease/).

I don't expect further changes to the alpha3 release, except for a few only fixing some doctest errors due to numerical noise below
* `sage/rings/polynomial/` and
* `sage/matrix/`,
and perhaps some clean-up like adding ticket numbers to commit messages lacking them, so it should be safe to (re)base patches on this version.



---

archive/issue_comments_098986.json:
```json
{
    "body": "<a id='comment:60'>Comment 60:</a>\nReplying to [@nexttime](#comment%3A59):\n> For the impatient\n\nPerfect - thanks!  Building now, will tinker tonight.",
    "created_at": "2011-09-20T13:42:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98986",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:60'>Comment 60:</a>
Replying to [@nexttime](#comment%3A59):
> For the impatient

Perfect - thanks!  Building now, will tinker tonight.



---

archive/issue_comments_098987.json:
```json
{
    "body": "Attachment: **[trac_10335-permgroup_domain-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-rebase.patch.gz)**\n\npatch against 4.7.2.alpha3 prerelease",
    "created_at": "2011-09-20T18:28:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98987",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[trac_10335-permgroup_domain-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-rebase.patch.gz)**

patch against 4.7.2.alpha3 prerelease



---

archive/issue_comments_098988.json:
```json
{
    "body": "<a id='comment:61'>Comment 61:</a>\nI had a few minutes free this afternoon, so I've done a rebased patch myself. This is just the existing patches rebased and folded, but with the doctest in `permgroup.py` marked as random rather than deleted, and with examples of the output I got on a Linux and a Solaris machine.\n\nRob: any chance you could review this? Apologies if you'd already started rebasing this yourself as per our earlier agreement -- in that case feel free to ignore my patch and I'll review yours.\n\nLeif: you might be interested to hear that this prerelease builds and passes long doctests without problems on my 64-bit Linux machine -- no numerical noise or anything like that.",
    "created_at": "2011-09-20T18:33:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98988",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:61'>Comment 61:</a>
I had a few minutes free this afternoon, so I've done a rebased patch myself. This is just the existing patches rebased and folded, but with the doctest in `permgroup.py` marked as random rather than deleted, and with examples of the output I got on a Linux and a Solaris machine.

Rob: any chance you could review this? Apologies if you'd already started rebasing this yourself as per our earlier agreement -- in that case feel free to ignore my patch and I'll review yours.

Leif: you might be interested to hear that this prerelease builds and passes long doctests without problems on my 64-bit Linux machine -- no numerical noise or anything like that.



---

archive/issue_events_119886.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-09-20T18:33:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119886"
}
```



---

archive/issue_events_119887.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-09-20T18:33:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119887"
}
```



---

archive/issue_comments_098989.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,8 +9,4 @@\n ```\n \n **Apply:**\n-1.  [attachment: trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)\n-2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)\n-3.  [attachment: trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)\n-4.  [attachment: trac_10335-solaris_issue.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-solaris_issue.patch.gz)\n-5.  [attachment: trac_10335-updates.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-updates.patch.gz)\n+1.  [attachment: trac_10335-permgroup_domain-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-rebase.patch.gz)\n``````\n",
    "created_at": "2011-09-20T18:33:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98989",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,8 +9,4 @@
 ```
 
 **Apply:**
-1.  [attachment: trac_10335-permgroup_domain-mh.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-mh.patch.gz)
-2.  [attachment: trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-import_loop_fix-nt.patch.gz)
-3.  [attachment: trac_10335-tutorial-languages-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-tutorial-languages-fixes.patch.gz)
-4.  [attachment: trac_10335-solaris_issue.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-solaris_issue.patch.gz)
-5.  [attachment: trac_10335-updates.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-updates.patch.gz)
+1.  [attachment: trac_10335-permgroup_domain-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket10335/trac_10335-permgroup_domain-rebase.patch.gz)
``````




---

archive/issue_comments_098990.json:
```json
{
    "body": "<a id='comment:62'>Comment 62:</a>\nDavid - no problem, I'll do a review.  I have no free minutes today.  So tonight. ;-)\n\nProbably about 8-10 hours from now.  Thanks for hopping on this one.",
    "created_at": "2011-09-20T19:01:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98990",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:62'>Comment 62:</a>
David - no problem, I'll do a review.  I have no free minutes today.  So tonight. ;-)

Probably about 8-10 hours from now.  Thanks for hopping on this one.



---

archive/issue_comments_098991.json:
```json
{
    "body": "<a id='comment:63'>Comment 63:</a>\nReplying to [@loefflerd](#comment%3A61):\n> Leif: you might be interested to hear that this prerelease builds and passes long doctests without problems on my 64-bit Linux machine -- no numerical noise or anything like that.\n\nIf you also tell me what CPU you built on... (e.g. top of `/proc/cpuinfo`, i.e., only for the first core...)\n\nMight be interesting.",
    "created_at": "2011-09-20T22:44:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98991",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:63'>Comment 63:</a>
Replying to [@loefflerd](#comment%3A61):
> Leif: you might be interested to hear that this prerelease builds and passes long doctests without problems on my 64-bit Linux machine -- no numerical noise or anything like that.

If you also tell me what CPU you built on... (e.g. top of `/proc/cpuinfo`, i.e., only for the first core...)

Might be interesting.



---

archive/issue_comments_098992.json:
```json
{
    "body": "<a id='comment:64'>Comment 64:</a>\nReplying to [@rbeezer](#comment%3A62):\n> Probably about 8-10 hours from now.\n\nMy build was interrupted (long story) and now two files are giving segfaults when I doctest, with or without the patch.  Don't panic - likely entirely all my fault.  So anyway, I'm starting over and building overnight.  Stay tuned.\n\nRob",
    "created_at": "2011-09-21T04:12:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98992",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:64'>Comment 64:</a>
Replying to [@rbeezer](#comment%3A62):
> Probably about 8-10 hours from now.

My build was interrupted (long story) and now two files are giving segfaults when I doctest, with or without the patch.  Don't panic - likely entirely all my fault.  So anyway, I'm starting over and building overnight.  Stay tuned.

Rob



---

archive/issue_comments_098993.json:
```json
{
    "body": "<a id='comment:65'>Comment 65:</a>\nalpha3-prerelease segfaults during testing, without patch.  Errors, followed by system info.  I can follow up with more testing, if requested.\n\n\n```\nThe following tests failed:\n\n        sage -t  -long -force_lib devel/sage/sage/modular/modsym/ambient.py # 0 doctests failed\n        sage -t  -long -force_lib devel/sage/sage/modular/hecke/ambient_module.py # 0 doctests failed\n        sage -t  -long -force_lib devel/sage/sage/modular/hecke/hecke_operator.py # 0 doctests failed\n----------------------------------------------------------------------\nTotal time for all tests: 2701.6 seconds\nmake: *** [ptestlong] Error 128\n```\n\n```\nrob@lava:/sage/sage-4.7.2.alpha3-prerelease$ ./sage -t  -long -force_lib devel/sage/sage/modular/modsym/ambient.py\nsage -t -long -force_lib \"devel/sage/sage/modular/modsym/ambient.py\"\n/sage/sage-4.7.2.alpha3-prerelease/local/lib/libcsage.so(print_backtrace+0x31)[0x7fd8e335834f]\n/sage/sage-4.7.2.alpha3-prerelease/local/lib/libcsage.so(sigdie+0x14)[0x7fd8e3358381]\n/sage/sage-4.7.2.alpha3-prerelease/local/lib/libcsage.so(sage_signal_handler+0x20e)[0x7fd8e3357fb4]\n/lib/x86_64-linux-gnu/libpthread.so.0(+0xfc60)[0x7fd8e484fc60]\n/sage/sage-4.7.2.alpha3-prerelease/local/lib/libatlas.so(ATL_dJIK44x44x44TN44x44x0_a1_b0+0x9d)[0x7fd8da12c3ed]\n\n------------------------------------------------------------------------\nUnhandled SIGSEGV: A segmentation fault occurred in Sage.\nThis probably occurred because a *compiled* component of Sage has a bug\nin it and is not properly wrapped with sig_on(), sig_off(). You might\nwant to run Sage under gdb with 'sage -gdb' to debug this.\nSage will now terminate.\n------------------------------------------------------------------------\nSegmentation fault\n\n         [5.9 s]\n \n----------------------------------------------------------------------\nThe following tests failed:\n\n\n        sage -t -long -force_lib \"devel/sage/sage/modular/modsym/ambient.py\"\nTotal time for all tests: 5.9 seconds\n```\n\n```\nrob@lava:/sage/sage-4.7.2.alpha3-prerelease$ uname -a\nLinux lava 2.6.38-10-generic #46-Ubuntu SMP Tue Jun 28 15:07:17 UTC 2011 x86_64 x86_64 x86_64 GNU/Linux\n```\n\n```\nrob@lava:/sage/sage-4.7.2.alpha3-prerelease$ gcc -v\nUsing built-in specs.\nCOLLECT_GCC=gcc\nCOLLECT_LTO_WRAPPER=/usr/lib/x86_64-linux-gnu/gcc/x86_64-linux-gnu/4.5.2/lto-wrapper\nTarget: x86_64-linux-gnu\nConfigured with: ../src/configure -v --with-pkgversion='Ubuntu/Linaro 4.5.2-8ubuntu4' --with-bugurl=file:///usr/share/doc/gcc-4.5/README.Bugs --enable-languages=c,c++,fortran,objc,obj-c++ --prefix=/usr --program-suffix=-4.5 --enable-shared --enable-multiarch --with-multiarch-defaults=x86_64-linux-gnu --enable-linker-build-id --with-system-zlib --libexecdir=/usr/lib/x86_64-linux-gnu --without-included-gettext --enable-threads=posix --with-gxx-include-dir=/usr/include/c++/4.5 --libdir=/usr/lib/x86_64-linux-gnu --enable-nls --with-sysroot=/ --enable-clocale=gnu --enable-libstdcxx-debug --enable-libstdcxx-time=yes --enable-plugin --enable-gold --enable-ld=default --with-plugin-ld=ld.gold --enable-objc-gc --disable-werror --with-arch-32=i686 --with-tune=generic --enable-checking=release --build=x86_64-linux-gnu --host=x86_64-linux-gnu --target=x86_64-linux-gnu\nThread model: posix\ngcc version 4.5.2 (Ubuntu/Linaro 4.5.2-8ubuntu4) \n```\n\n```\nrob@lava:/sage/sage-4.7.2.alpha3-prerelease$ cat /proc/cpuinfo\nprocessor       : 0\nvendor_id       : GenuineIntel\ncpu family      : 6\nmodel           : 42\nmodel name      : Intel(R) Core(TM) i7-2600 CPU @ 3.40GHz\nstepping        : 7\ncpu MHz         : 1600.000\ncache size      : 8192 KB\nphysical id     : 0\nsiblings        : 8\ncore id         : 0\ncpu cores       : 4\napicid          : 0\ninitial apicid  : 0\nfpu             : yes\nfpu_exception   : yes\ncpuid level     : 13\nwp              : yes\nflags           : fpu vme de pse tsc msr pae mce cx8 apic sep mtrr pge mca cmov pat pse36 clflush dts acpi mmx fxsr sse sse2 ss ht tm pbe syscall nx rdtscp lm constant_tsc arch_perfmon pebs bts rep_good nopl xtopology nonstop_tsc aperfmperf pni pclmulqdq dtes64 monitor ds_cpl vmx smx est tm2 ssse3 cx16 xtpr pdcm sse4_1 sse4_2 x2apic popcnt aes xsave avx lahf_lm ida arat epb xsaveopt pln pts dts tpr_shadow vnmi flexpriority ept vpid\nbogomips        : 6822.05\nclflush size    : 64\ncache_alignment : 64\naddress sizes   : 36 bits physical, 48 bits virtual\npower management:\n```",
    "created_at": "2011-09-21T14:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98993",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:65'>Comment 65:</a>
alpha3-prerelease segfaults during testing, without patch.  Errors, followed by system info.  I can follow up with more testing, if requested.


```
The following tests failed:

        sage -t  -long -force_lib devel/sage/sage/modular/modsym/ambient.py # 0 doctests failed
        sage -t  -long -force_lib devel/sage/sage/modular/hecke/ambient_module.py # 0 doctests failed
        sage -t  -long -force_lib devel/sage/sage/modular/hecke/hecke_operator.py # 0 doctests failed
----------------------------------------------------------------------
Total time for all tests: 2701.6 seconds
make: *** [ptestlong] Error 128
```

```
rob@lava:/sage/sage-4.7.2.alpha3-prerelease$ ./sage -t  -long -force_lib devel/sage/sage/modular/modsym/ambient.py
sage -t -long -force_lib "devel/sage/sage/modular/modsym/ambient.py"
/sage/sage-4.7.2.alpha3-prerelease/local/lib/libcsage.so(print_backtrace+0x31)[0x7fd8e335834f]
/sage/sage-4.7.2.alpha3-prerelease/local/lib/libcsage.so(sigdie+0x14)[0x7fd8e3358381]
/sage/sage-4.7.2.alpha3-prerelease/local/lib/libcsage.so(sage_signal_handler+0x20e)[0x7fd8e3357fb4]
/lib/x86_64-linux-gnu/libpthread.so.0(+0xfc60)[0x7fd8e484fc60]
/sage/sage-4.7.2.alpha3-prerelease/local/lib/libatlas.so(ATL_dJIK44x44x44TN44x44x0_a1_b0+0x9d)[0x7fd8da12c3ed]

------------------------------------------------------------------------
Unhandled SIGSEGV: A segmentation fault occurred in Sage.
This probably occurred because a *compiled* component of Sage has a bug
in it and is not properly wrapped with sig_on(), sig_off(). You might
want to run Sage under gdb with 'sage -gdb' to debug this.
Sage will now terminate.
------------------------------------------------------------------------
Segmentation fault

         [5.9 s]
 
----------------------------------------------------------------------
The following tests failed:


        sage -t -long -force_lib "devel/sage/sage/modular/modsym/ambient.py"
Total time for all tests: 5.9 seconds
```

```
rob@lava:/sage/sage-4.7.2.alpha3-prerelease$ uname -a
Linux lava 2.6.38-10-generic #46-Ubuntu SMP Tue Jun 28 15:07:17 UTC 2011 x86_64 x86_64 x86_64 GNU/Linux
```

```
rob@lava:/sage/sage-4.7.2.alpha3-prerelease$ gcc -v
Using built-in specs.
COLLECT_GCC=gcc
COLLECT_LTO_WRAPPER=/usr/lib/x86_64-linux-gnu/gcc/x86_64-linux-gnu/4.5.2/lto-wrapper
Target: x86_64-linux-gnu
Configured with: ../src/configure -v --with-pkgversion='Ubuntu/Linaro 4.5.2-8ubuntu4' --with-bugurl=file:///usr/share/doc/gcc-4.5/README.Bugs --enable-languages=c,c++,fortran,objc,obj-c++ --prefix=/usr --program-suffix=-4.5 --enable-shared --enable-multiarch --with-multiarch-defaults=x86_64-linux-gnu --enable-linker-build-id --with-system-zlib --libexecdir=/usr/lib/x86_64-linux-gnu --without-included-gettext --enable-threads=posix --with-gxx-include-dir=/usr/include/c++/4.5 --libdir=/usr/lib/x86_64-linux-gnu --enable-nls --with-sysroot=/ --enable-clocale=gnu --enable-libstdcxx-debug --enable-libstdcxx-time=yes --enable-plugin --enable-gold --enable-ld=default --with-plugin-ld=ld.gold --enable-objc-gc --disable-werror --with-arch-32=i686 --with-tune=generic --enable-checking=release --build=x86_64-linux-gnu --host=x86_64-linux-gnu --target=x86_64-linux-gnu
Thread model: posix
gcc version 4.5.2 (Ubuntu/Linaro 4.5.2-8ubuntu4) 
```

```
rob@lava:/sage/sage-4.7.2.alpha3-prerelease$ cat /proc/cpuinfo
processor       : 0
vendor_id       : GenuineIntel
cpu family      : 6
model           : 42
model name      : Intel(R) Core(TM) i7-2600 CPU @ 3.40GHz
stepping        : 7
cpu MHz         : 1600.000
cache size      : 8192 KB
physical id     : 0
siblings        : 8
core id         : 0
cpu cores       : 4
apicid          : 0
initial apicid  : 0
fpu             : yes
fpu_exception   : yes
cpuid level     : 13
wp              : yes
flags           : fpu vme de pse tsc msr pae mce cx8 apic sep mtrr pge mca cmov pat pse36 clflush dts acpi mmx fxsr sse sse2 ss ht tm pbe syscall nx rdtscp lm constant_tsc arch_perfmon pebs bts rep_good nopl xtopology nonstop_tsc aperfmperf pni pclmulqdq dtes64 monitor ds_cpl vmx smx est tm2 ssse3 cx16 xtpr pdcm sse4_1 sse4_2 x2apic popcnt aes xsave avx lahf_lm ida arat epb xsaveopt pln pts dts tpr_shadow vnmi flexpriority ept vpid
bogomips        : 6822.05
clflush size    : 64
cache_alignment : 64
address sizes   : 36 bits physical, 48 bits virtual
power management:
```



---

archive/issue_comments_098994.json:
```json
{
    "body": "<a id='comment:66'>Comment 66:</a>\nReplying to [@rbeezer](#comment%3A65):\n> alpha3-prerelease segfaults during testing, without patch.  Errors, followed by system info.  I can follow up with more testing, if requested.\n\nThat's either due to Linaro, or, more likely, an ATLAS problem, since our version afaik doesn't yet support Sandy Bridge (second generation \"Core i\").\n\nYou could try using your system's ATLAS (may have to install it with your package manager first), by doing:\n\n```sh\n$ export SAGE_ATLAS_LIB=/usr/lib64/atlas # the directory; usual place on 64-bit Linux\n$ rm $SAGE_ROOT/spkg/installed/atlas-*\n$ cd $SAGE_ROOT\n$ env SAGE_UPGRADING=yes make # or \"... make build\"\n```\n\nThe `SAGE_UPGRADING=yes` is important to (really) rebuild all dependent packages as well.",
    "created_at": "2011-09-21T15:00:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98994",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:66'>Comment 66:</a>
Replying to [@rbeezer](#comment%3A65):
> alpha3-prerelease segfaults during testing, without patch.  Errors, followed by system info.  I can follow up with more testing, if requested.

That's either due to Linaro, or, more likely, an ATLAS problem, since our version afaik doesn't yet support Sandy Bridge (second generation "Core i").

You could try using your system's ATLAS (may have to install it with your package manager first), by doing:

```sh
$ export SAGE_ATLAS_LIB=/usr/lib64/atlas # the directory; usual place on 64-bit Linux
$ rm $SAGE_ROOT/spkg/installed/atlas-*
$ cd $SAGE_ROOT
$ env SAGE_UPGRADING=yes make # or "... make build"
```

The `SAGE_UPGRADING=yes` is important to (really) rebuild all dependent packages as well.



---

archive/issue_comments_098995.json:
```json
{
    "body": "<a id='comment:67'>Comment 67:</a>\nReplying to [@nexttime](#comment%3A66):\n> \n\n```sh\n$ export SAGE_ATLAS_LIB=/usr/lib64/atlas # the directory; usual place on 64-bit Linux\n...\n```\n\nMight be `/usr/lib/x86_64-linux-gnu/atlas/` on your system; you should check this before attempting to build... ;-)",
    "created_at": "2011-09-21T15:10:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98995",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:67'>Comment 67:</a>
Replying to [@nexttime](#comment%3A66):
> 

```sh
$ export SAGE_ATLAS_LIB=/usr/lib64/atlas # the directory; usual place on 64-bit Linux
...
```

Might be `/usr/lib/x86_64-linux-gnu/atlas/` on your system; you should check this before attempting to build... ;-)



---

archive/issue_comments_098996.json:
```json
{
    "body": "<a id='comment:68'>Comment 68:</a>\nThanks, Leif.  Ubuntu installs ATLAS into `/usr/lib64/atlas-base`.  Then setting up everything as you describe, suitably adjusted yields:\n\n```\nUnable to find one of liblapack, libcblas, libatlas or libf77blas\nin the directory /usr/lib64/atlas-base\nSet SAGE_ATLAS_LIB to the directory containing liblapack, libcblas,\nlibatlas and libf77blas (either .a or .so extensions) if you wish to\nuse existing ATLAS libraries. For more details, see\n```\n\nI've installed the packages `liblapack3gf, libblas3gf` but cannot see anything to provide libf77blas and do not know if I am getting libcblas.  Furthermore, these all go to different directories.\n\nAny advice?  My Sandy Bridge motherboard is about 8 months old.  Will anybody with a newer chip have to go through a drill like this?  Would Volker's ATLAS ticket be a better/workable approach?",
    "created_at": "2011-09-21T16:12:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98996",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:68'>Comment 68:</a>
Thanks, Leif.  Ubuntu installs ATLAS into `/usr/lib64/atlas-base`.  Then setting up everything as you describe, suitably adjusted yields:

```
Unable to find one of liblapack, libcblas, libatlas or libf77blas
in the directory /usr/lib64/atlas-base
Set SAGE_ATLAS_LIB to the directory containing liblapack, libcblas,
libatlas and libf77blas (either .a or .so extensions) if you wish to
use existing ATLAS libraries. For more details, see
```

I've installed the packages `liblapack3gf, libblas3gf` but cannot see anything to provide libf77blas and do not know if I am getting libcblas.  Furthermore, these all go to different directories.

Any advice?  My Sandy Bridge motherboard is about 8 months old.  Will anybody with a newer chip have to go through a drill like this?  Would Volker's ATLAS ticket be a better/workable approach?



---

archive/issue_comments_098997.json:
```json
{
    "body": "<a id='comment:69'>Comment 69:</a>\nReplying to [@rbeezer](#comment%3A68):\n> I've installed the packages `liblapack3gf, libblas3gf` but cannot see anything to provide libf77blas and do not know if I am getting libcblas.  Furthermore, these all go to different directories.\n\nI've asked Volker on the IRC for help. (Perhaps drop in if you can.)\n\nIf some libraries are simply in different directories, you could just create symbolic links to them in the `.../atlas-base/` directory. (Although there IMHO should<sup>TM</sup> be a directory containing all; are there some more self-contained ATLAS packages you could install?)\n\n\n\n\n\n> My Sandy Bridge motherboard is about 8 months old.\n\nThat shouldn't matter; Intel's bug just causes a SATA part of the chipset to die earlier.\n\nWhether AVX is available depends on your board (and chipset); if you have on-board graphics, AVX isn't usable.\n\n\n\n\n> Will anybody with a newer chip have to go through a drill like this?\n\nSimon King did successfully build the prerelease on a (Sandy Bridge) Core i3 with Debian x86_64; there only five doctests (in three files) failed due to numerical noise (the same doctests that failed on some other \"rare\" and older platforms).",
    "created_at": "2011-09-21T16:40:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98997",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:69'>Comment 69:</a>
Replying to [@rbeezer](#comment%3A68):
> I've installed the packages `liblapack3gf, libblas3gf` but cannot see anything to provide libf77blas and do not know if I am getting libcblas.  Furthermore, these all go to different directories.

I've asked Volker on the IRC for help. (Perhaps drop in if you can.)

If some libraries are simply in different directories, you could just create symbolic links to them in the `.../atlas-base/` directory. (Although there IMHO should<sup>TM</sup> be a directory containing all; are there some more self-contained ATLAS packages you could install?)





> My Sandy Bridge motherboard is about 8 months old.

That shouldn't matter; Intel's bug just causes a SATA part of the chipset to die earlier.

Whether AVX is available depends on your board (and chipset); if you have on-board graphics, AVX isn't usable.




> Will anybody with a newer chip have to go through a drill like this?

Simon King did successfully build the prerelease on a (Sandy Bridge) Core i3 with Debian x86_64; there only five doctests (in three files) failed due to numerical noise (the same doctests that failed on some other "rare" and older platforms).



---

archive/issue_comments_098998.json:
```json
{
    "body": "<a id='comment:70'>Comment 70:</a>\nThe following bug in Ubuntu launchpad says that the symlinks for the standard libraries are not created: https://bugs.launchpad.net/ubuntu/+source/atlas/+bug/769180  So you either create them manually (in a directory of your choice), or you compile ATLAS again. For the latter, you should probably try to set\n\n```sh\nexport SAGE_ATLAS_ARCH=Corei7,SSE3,SSE2,SSE1\n```\nThis works only with the new atlas spkg (4.7.2.alpha3+)",
    "created_at": "2011-09-21T16:45:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98998",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:70'>Comment 70:</a>
The following bug in Ubuntu launchpad says that the symlinks for the standard libraries are not created: https://bugs.launchpad.net/ubuntu/+source/atlas/+bug/769180  So you either create them manually (in a directory of your choice), or you compile ATLAS again. For the latter, you should probably try to set

```sh
export SAGE_ATLAS_ARCH=Corei7,SSE3,SSE2,SSE1
```
This works only with the new atlas spkg (4.7.2.alpha3+)



---

archive/issue_comments_098999.json:
```json
{
    "body": "<a id='comment:71'>Comment 71:</a>\nAVX is just the CPU instruction set extension du jour. The version of ATLAS that we are shipping knows nothing about AVX. All 2011 sandy bridge cpus have it, irrespective of on-board graphics.",
    "created_at": "2011-09-21T16:51:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-98999",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:71'>Comment 71:</a>
AVX is just the CPU instruction set extension du jour. The version of ATLAS that we are shipping knows nothing about AVX. All 2011 sandy bridge cpus have it, irrespective of on-board graphics.



---

archive/issue_comments_099000.json:
```json
{
    "body": "<a id='comment:72'>Comment 72:</a>\nReplying to [@vbraun](#comment%3A71):\n> AVX is just the CPU instruction set extension du jour. The version of ATLAS that we are shipping knows nothing about AVX.\n\nAha. Wasn't sure about that. But does 3.8.4 \"know\" Sandy Brigde already?\n\n\n\n\n> All 2011 sandy bridge cpus have it, irrespective of on-board graphics. \n\nThey have it, but you can only use it when the CPU doesn't also act as a GPU. And afaik that's hard-wired, i.e., you even cannot use it if your motherboard has on-board graphics you don't actually use.",
    "created_at": "2011-09-21T17:10:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99000",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:72'>Comment 72:</a>
Replying to [@vbraun](#comment%3A71):
> AVX is just the CPU instruction set extension du jour. The version of ATLAS that we are shipping knows nothing about AVX.

Aha. Wasn't sure about that. But does 3.8.4 "know" Sandy Brigde already?




> All 2011 sandy bridge cpus have it, irrespective of on-board graphics. 

They have it, but you can only use it when the CPU doesn't also act as a GPU. And afaik that's hard-wired, i.e., you even cannot use it if your motherboard has on-board graphics you don't actually use.



---

archive/issue_comments_099001.json:
```json
{
    "body": "<a id='comment:73'>Comment 73:</a>\nReplying to [@vbraun](#comment%3A70):\n> This works only with the new atlas spkg (4.7.2.alpha3+)\n\nI guess you meant alpha**2**+.\n\nApparently alpha2 in contrast did **not** segfault for Rob (on that machine), despite having **the same** ATLAS spkg. At least that's my understanding.",
    "created_at": "2011-09-21T17:13:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99001",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:73'>Comment 73:</a>
Replying to [@vbraun](#comment%3A70):
> This works only with the new atlas spkg (4.7.2.alpha3+)

I guess you meant alpha**2**+.

Apparently alpha2 in contrast did **not** segfault for Rob (on that machine), despite having **the same** ATLAS spkg. At least that's my understanding.



---

archive/issue_comments_099002.json:
```json
{
    "body": "<a id='comment:74'>Comment 74:</a>\nOh yes, it was merged in alpha2. \n\nThis is now completely OT, but my (Sandy Bridge) Thinkpad W520 uses the on-board GPU and has AVX:\n\n```sh\n\n[vbraun@volker-laptop-two ~]$ cat /proc/cpuinfo | head -24\nprocessor\t: 0\nvendor_id\t: GenuineIntel\ncpu family\t: 6\nmodel\t\t: 42\nmodel name\t: Intel(R) Core(TM) i7-2720QM CPU @ 2.20GHz\nstepping\t: 7\ncpu MHz\t\t: 2201.000\ncache size\t: 6144 KB\nphysical id\t: 0\nsiblings\t: 8\ncore id\t\t: 0\ncpu cores\t: 4\napicid\t\t: 0\ninitial apicid\t: 0\nfpu\t\t: yes\nfpu_exception\t: yes\ncpuid level\t: 13\nwp\t\t: yes\nflags\t\t: fpu vme de pse tsc msr pae mce cx8 apic sep mtrr pge mca cmov pat pse36 clflush dts acpi mmx fxsr sse sse2 ss ht tm pbe syscall nx rdtscp lm constant_tsc arch_perfmon pebs bts nopl xtopology nonstop_tsc aperfmperf pni pclmulqdq dtes64 monitor ds_cpl vmx smx est tm2 ssse3 cx16 xtpr pdcm sse4_1 sse4_2 x2apic popcnt aes xsave avx lahf_lm ida arat epb xsaveopt pln pts dts tpr_shadow vnmi flexpriority ept vpid\nbogomips\t: 4385.92\nclflush size\t: 64\ncache_alignment\t: 64\naddress sizes\t: 36 bits physical, 48 bits virtual\npower management:\n```",
    "created_at": "2011-09-21T17:16:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99002",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:74'>Comment 74:</a>
Oh yes, it was merged in alpha2. 

This is now completely OT, but my (Sandy Bridge) Thinkpad W520 uses the on-board GPU and has AVX:

```sh

[vbraun@volker-laptop-two ~]$ cat /proc/cpuinfo | head -24
processor	: 0
vendor_id	: GenuineIntel
cpu family	: 6
model		: 42
model name	: Intel(R) Core(TM) i7-2720QM CPU @ 2.20GHz
stepping	: 7
cpu MHz		: 2201.000
cache size	: 6144 KB
physical id	: 0
siblings	: 8
core id		: 0
cpu cores	: 4
apicid		: 0
initial apicid	: 0
fpu		: yes
fpu_exception	: yes
cpuid level	: 13
wp		: yes
flags		: fpu vme de pse tsc msr pae mce cx8 apic sep mtrr pge mca cmov pat pse36 clflush dts acpi mmx fxsr sse sse2 ss ht tm pbe syscall nx rdtscp lm constant_tsc arch_perfmon pebs bts nopl xtopology nonstop_tsc aperfmperf pni pclmulqdq dtes64 monitor ds_cpl vmx smx est tm2 ssse3 cx16 xtpr pdcm sse4_1 sse4_2 x2apic popcnt aes xsave avx lahf_lm ida arat epb xsaveopt pln pts dts tpr_shadow vnmi flexpriority ept vpid
bogomips	: 4385.92
clflush size	: 64
cache_alignment	: 64
address sizes	: 36 bits physical, 48 bits virtual
power management:
```



---

archive/issue_comments_099003.json:
```json
{
    "body": "<a id='comment:75'>Comment 75:</a>\nLeif and Volker,\n\nThanks for the quick help, and I apologize to the group theorists for diverting this ticket.  I've got to attend to some other things the rest of today, but I'll see if I can get the symlinks in place with guidance from that bug report.\n\nI have Sandy Bridge with no on-board graphics.  I have built and tested alpha2 - this is my first problem like this.\n\nRob",
    "created_at": "2011-09-21T17:27:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99003",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:75'>Comment 75:</a>
Leif and Volker,

Thanks for the quick help, and I apologize to the group theorists for diverting this ticket.  I've got to attend to some other things the rest of today, but I'll see if I can get the symlinks in place with guidance from that bug report.

I have Sandy Bridge with no on-board graphics.  I have built and tested alpha2 - this is my first problem like this.

Rob



---

archive/issue_comments_099004.json:
```json
{
    "body": "<a id='comment:76'>Comment 76:</a>\nReplying to [@rbeezer](#comment%3A75):\n> Leif and Volker,\n> \n> Thanks for the quick help, and I apologize to the group theorists for diverting this ticket.\n\nYep, we can continue the discussion on e.g. [this sage-release thread](http://groups.google.com/group/sage-release/browse_thread/thread/e52a7d9593d50da5#), or -- preferably -- a new one there.",
    "created_at": "2011-09-21T19:22:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99004",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:76'>Comment 76:</a>
Replying to [@rbeezer](#comment%3A75):
> Leif and Volker,
> 
> Thanks for the quick help, and I apologize to the group theorists for diverting this ticket.

Yep, we can continue the discussion on e.g. [this sage-release thread](http://groups.google.com/group/sage-release/browse_thread/thread/e52a7d9593d50da5#), or -- preferably -- a new one there.



---

archive/issue_comments_099005.json:
```json
{
    "body": "<a id='comment:77'>Comment 77:</a>\nalpha3 prerelease build is fixed and passes long tests, with and without this rebased patch.  To quote William \"merge that sucker\"!  In other words, positive review.\n\nSecond line of patch summary message credits Jason Hill - I'll add him to the authors, and Mike's name is still on the main patch.\n\nDavid - feel free to cc me on any of the follow-on tickets that might need review, especially if they are cosmetic changes.  I have not yet looked at them at all.\n\nLeif - thanks for your attention to this one and help with my build challenges.\n\nEverybody else - my apologies again for getting this ticket off the tracks.  I need to post a summary on sage-release.\n\nRob",
    "created_at": "2011-09-22T15:06:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99005",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:77'>Comment 77:</a>
alpha3 prerelease build is fixed and passes long tests, with and without this rebased patch.  To quote William "merge that sucker"!  In other words, positive review.

Second line of patch summary message credits Jason Hill - I'll add him to the authors, and Mike's name is still on the main patch.

David - feel free to cc me on any of the follow-on tickets that might need review, especially if they are cosmetic changes.  I have not yet looked at them at all.

Leif - thanks for your attention to this one and help with my build challenges.

Everybody else - my apologies again for getting this ticket off the tracks.  I need to post a summary on sage-release.

Rob



---

archive/issue_events_119888.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-09-22T15:06:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119888"
}
```



---

archive/issue_events_119889.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-09-22T15:06:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119889"
}
```



---

archive/issue_comments_099006.json:
```json
{
    "body": "Changed author from **Mike Hansen, David Loeffler** to **Mike Hansen, Jason Hill, David Loeffler**",
    "created_at": "2011-09-22T15:06:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99006",
    "user": "https://github.com/rbeezer"
}
```

Changed author from **Mike Hansen, David Loeffler** to **Mike Hansen, Jason Hill, David Loeffler**



---

archive/issue_comments_099007.json:
```json
{
    "body": "<a id='comment:78'>Comment 78:</a>\nHi Rob,\n\nThanks for the review! I have tested the two tickets that follow on directly from this (#11422 and #11598) and they apply fine and pass doctests, so no futher reviewing is needed; I'm going to move them back to the 4.7.2 milestone (from \"sage-pending\"). \n\nRegards, David",
    "created_at": "2011-09-22T15:22:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99007",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:78'>Comment 78:</a>
Hi Rob,

Thanks for the review! I have tested the two tickets that follow on directly from this (#11422 and #11598) and they apply fine and pass doctests, so no futher reviewing is needed; I'm going to move them back to the 4.7.2 milestone (from "sage-pending"). 

Regards, David



---

archive/issue_comments_099008.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha3**",
    "created_at": "2011-09-27T17:43:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99008",
    "user": "https://github.com/nexttime"
}
```

Merged: **sage-4.7.2.alpha3**



---

archive/issue_events_119890.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-27T17:43:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119890"
}
```



---

archive/issue_events_119891.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-27T17:43:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10335#event-119891"
}
```



---

archive/issue_comments_099009.json:
```json
{
    "body": "<a id='comment:80'>Comment 80:</a>\nHello and sorry to add a comment on a closed ticket. I have a patch which depend on it and the patchbot try to apply 2 patches instead one here.\n\napply trac_10335-permgroup_domain-rebase.patch",
    "created_at": "2011-10-19T18:48:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10335#issuecomment-99009",
    "user": "https://github.com/sagetrac-nborie"
}
```

<a id='comment:80'>Comment 80:</a>
Hello and sorry to add a comment on a closed ticket. I have a patch which depend on it and the patchbot try to apply 2 patches instead one here.

apply trac_10335-permgroup_domain-rebase.patch
