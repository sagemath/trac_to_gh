# Issue 10283: Fix reference in eliminate_linear_variables

archive/issues_010282.json:
```json
{
    "assignees": [],
    "body": "Stanislav Bulyin reported in private communication that the reference given in the documentation for `eliminate_linear_variables()` is wrong.\n\n**Apply:**\n\n1. [attachment: trac_10283.patch](https://github.com/sagemath/sage/files/ticket10283/trac_10283.patch.gz)\n2. [attachment: trac-10283_reviewer.patch](https://github.com/sagemath/sage/files/ticket10283/trac-10283_reviewer.patch.gz)\n\nAssignee: **@sagetrac-mvngu**\n\nAuthor: **Martin Albrecht**\n\nReviewer: **Minh Van Nguyen**\n\nMerged: **sage-4.6.1.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10283_\n\n",
    "closed_at": "2010-11-19T07:54:33Z",
    "created_at": "2010-11-17T17:04:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cryptography",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix reference in eliminate_linear_variables",
    "type": "issue",
    "updated_at": "2010-11-19T07:54:33Z",
    "url": "https://github.com/sagemath/sage/issues/10283",
    "user": "https://github.com/malb"
}
```
Stanislav Bulyin reported in private communication that the reference given in the documentation for `eliminate_linear_variables()` is wrong.

**Apply:**

1. [attachment: trac_10283.patch](https://github.com/sagemath/sage/files/ticket10283/trac_10283.patch.gz)
2. [attachment: trac-10283_reviewer.patch](https://github.com/sagemath/sage/files/ticket10283/trac-10283_reviewer.patch.gz)

Assignee: **@sagetrac-mvngu**

Author: **Martin Albrecht**

Reviewer: **Minh Van Nguyen**

Merged: **sage-4.6.1.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/10283_





---

archive/issue_events_123263.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-11-17T17:04:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10283#event-123263"
}
```



---

archive/issue_events_123264.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-11-17T17:04:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cryptography",
    "label_color": "0000ff",
    "label_name": "component: cryptography",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10283#event-123264"
}
```



---

archive/issue_events_123265.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-11-17T17:04:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10283#event-123265"
}
```



---

archive/issue_events_123266.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-11-17T17:04:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10283#event-123266"
}
```



---

archive/issue_comments_097934.json:
```json
{
    "body": "Attachment: **[trac_10283.patch.gz](https://github.com/sagemath/sage/files/ticket10283/trac_10283.patch.gz)**",
    "created_at": "2010-11-17T17:05:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10283#issuecomment-97934",
    "user": "https://github.com/malb"
}
```

Attachment: **[trac_10283.patch.gz](https://github.com/sagemath/sage/files/ticket10283/trac_10283.patch.gz)**



---

archive/issue_events_123267.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-11-17T17:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10283#event-123267"
}
```



---

archive/issue_comments_097935.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2010-11-18T11:40:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10283#issuecomment-97935",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_comments_097936.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,6 @@\n Stanislav Bulyin reported in private communication that the reference given in the documentation for `eliminate_linear_variables()` is wrong.\n+\n+**Apply:**\n+\n+1. [attachment: trac_10283.patch](https://github.com/sagemath/sage/files/ticket10283/trac_10283.patch.gz)\n+2. [attachment: trac-10283_reviewer.patch](https://github.com/sagemath/sage/files/ticket10283/trac-10283_reviewer.patch.gz)\n``````\n",
    "created_at": "2010-11-18T11:40:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10283#issuecomment-97936",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,6 @@
 Stanislav Bulyin reported in private communication that the reference given in the documentation for `eliminate_linear_variables()` is wrong.
+
+**Apply:**
+
+1. [attachment: trac_10283.patch](https://github.com/sagemath/sage/files/ticket10283/trac_10283.patch.gz)
+2. [attachment: trac-10283_reviewer.patch](https://github.com/sagemath/sage/files/ticket10283/trac-10283_reviewer.patch.gz)
``````




---

archive/issue_comments_097937.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAttachment: **[trac-10283_reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket10283/trac-10283_reviewer.patch.gz)**\n\nApplying [attachment: trac_10283.patch](https://github.com/sagemath/sage/files/ticket10283/trac_10283.patch.gz) to Sage 4.6.1.alpha1, building the HTML version of the reference manual results in the following warning:\n\n```\n/dev/shm/mvngu/sage-4.6.1.alpha1/devel/sage/doc/en/reference/sage/crypto/mq/mpolynomialsystem.rst:: WARNING: citation not found: CB07\n```\n\nAnd building the PDF version results in the following warning:\n\n```\nresolving references...\n/dev/shm/mvngu/sage-4.6.1.alpha1/devel/sage/doc/en/reference/index.rst:: WARNING: citation not found: CB07\n```\n\nThe warning is fixed in [attachment: trac-10283_reviewer.patch](https://github.com/sagemath/sage/files/ticket10283/trac-10283_reviewer.patch.gz), which needs reviewing.",
    "created_at": "2010-11-18T11:40:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10283#issuecomment-97937",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">Comment 2</div>

Attachment: **[trac-10283_reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket10283/trac-10283_reviewer.patch.gz)**

Applying [attachment: trac_10283.patch](https://github.com/sagemath/sage/files/ticket10283/trac_10283.patch.gz) to Sage 4.6.1.alpha1, building the HTML version of the reference manual results in the following warning:

```
/dev/shm/mvngu/sage-4.6.1.alpha1/devel/sage/doc/en/reference/sage/crypto/mq/mpolynomialsystem.rst:: WARNING: citation not found: CB07
```

And building the PDF version results in the following warning:

```
resolving references...
/dev/shm/mvngu/sage-4.6.1.alpha1/devel/sage/doc/en/reference/index.rst:: WARNING: citation not found: CB07
```

The warning is fixed in [attachment: trac-10283_reviewer.patch](https://github.com/sagemath/sage/files/ticket10283/trac-10283_reviewer.patch.gz), which needs reviewing.



---

archive/issue_comments_097938.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good, thanks for spotting that!",
    "created_at": "2010-11-18T11:48:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10283#issuecomment-97938",
    "user": "https://github.com/malb"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good, thanks for spotting that!



---

archive/issue_events_123268.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-11-18T11:48:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10283#event-123268"
}
```



---

archive/issue_events_123269.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-11-18T11:48:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10283#event-123269"
}
```



---

archive/issue_comments_097939.json:
```json
{
    "body": "Merged: **sage-4.6.1.alpha2**",
    "created_at": "2010-11-19T07:54:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10283#issuecomment-97939",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.1.alpha2**



---

archive/issue_events_123270.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-19T07:54:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10283#event-123270"
}
```



---

archive/issue_events_123271.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-19T07:54:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10283",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10283#event-123271"
}
```
