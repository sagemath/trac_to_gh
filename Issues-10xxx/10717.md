# Issue 10717: Prime typeset badly on plots

archive/issues_010655.json:
```json
{
    "assignees": [],
    "body": "When I try to put something like `$\\sigma'_0$` on plots, the prime looks\nvery big (see attached pdf). I found that `$\\sigma^'_0$` looks OK, but\nthere should be no need for such tricks, especially since this forms\nlooks bad in proper LaTeX.\n\n\nAssignee: **@jasongrout, @williamstein**\n\nKeywords: **sd40.5**\n\nUpstream: **Fixed upstream, in a later stable release.**\n\nReviewer: **Andrey Novoseltsev**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10717_\n\n",
    "closed_at": "2012-06-02T13:30:07Z",
    "created_at": "2011-01-30T18:17:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Prime typeset badly on plots",
    "type": "issue",
    "updated_at": "2012-06-02T13:30:07Z",
    "url": "https://github.com/sagemath/sage/issues/10717",
    "user": "https://github.com/novoselt"
}
```
When I try to put something like `$\sigma'_0$` on plots, the prime looks
very big (see attached pdf). I found that `$\sigma^'_0$` looks OK, but
there should be no need for such tricks, especially since this forms
looks bad in proper LaTeX.


Assignee: **@jasongrout, @williamstein**

Keywords: **sd40.5**

Upstream: **Fixed upstream, in a later stable release.**

Reviewer: **Andrey Novoseltsev**

_Issue created by migration from https://trac.sagemath.org/ticket/10717_





---

archive/issue_events_129039.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2011-01-30T18:17:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129039"
}
```



---

archive/issue_events_129040.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2011-01-30T18:17:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129040"
}
```



---

archive/issue_events_129041.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2011-01-30T18:17:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129041"
}
```



---

archive/issue_events_129042.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2011-01-30T18:17:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129042"
}
```



---

archive/issue_comments_104944.json:
```json
{
    "body": "Example of bad plotting (in the legend of the plot)",
    "created_at": "2011-01-30T18:17:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104944",
    "user": "https://github.com/novoselt"
}
```

Example of bad plotting (in the legend of the plot)



---

archive/issue_comments_104945.json:
```json
{
    "body": "Upstream: **Reported upstream. Little or no feedback.**",
    "created_at": "2011-02-01T17:44:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104945",
    "user": "https://github.com/jasongrout"
}
```

Upstream: **Reported upstream. Little or no feedback.**



---

archive/issue_comments_104946.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[Sigma_prime_--_Sage.pdf.gz](https://github.com/sagemath/sage/files/ticket10717/Sigma_prime_--_Sage.pdf.gz)**\n\nThis is a problem in matplotlib.  I've emailed the matplotlib list: http://sourceforge.net/mailarchive/forum.php?thread_name=4D484307.7010604%40creativetrax.com&forum_name=matplotlib-users",
    "created_at": "2011-02-01T17:44:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104946",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[Sigma_prime_--_Sage.pdf.gz](https://github.com/sagemath/sage/files/ticket10717/Sigma_prime_--_Sage.pdf.gz)**

This is a problem in matplotlib.  I've emailed the matplotlib list: http://sourceforge.net/mailarchive/forum.php?thread_name=4D484307.7010604%40creativetrax.com&forum_name=matplotlib-users



---

archive/issue_comments_104947.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nIncidentally, that discussion does show a workaround that doesn't fix the issue at the core.",
    "created_at": "2011-02-16T22:18:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104947",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'>Comment 2:</a>
Incidentally, that discussion does show a workaround that doesn't fix the issue at the core.



---

archive/issue_comments_104948.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nmatplotlib pull request filed on bugfix branch\n\nhttps://github.com/matplotlib/matplotlib/pull/54",
    "created_at": "2011-03-24T01:30:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104948",
    "user": "https://github.com/sagetrac-mdboom"
}
```

<a id='comment:3'>Comment 3:</a>
matplotlib pull request filed on bugfix branch

https://github.com/matplotlib/matplotlib/pull/54



---

archive/issue_comments_104949.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-When I try to put something like `$\\sigma'_0$` on plots, the prime looks very big (see attached pdf). I found that `$\\sigma^'_0$` looks OK, but there should be no need for such tricks, especially since this forms looks bad in proper LaTeX.\n+\n``````\n",
    "created_at": "2011-03-24T01:30:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104949",
    "user": "https://github.com/sagetrac-mdboom"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-When I try to put something like `$\sigma'_0$` on plots, the prime looks very big (see attached pdf). I found that `$\sigma^'_0$` looks OK, but there should be no need for such tricks, especially since this forms looks bad in proper LaTeX.
+
``````




---

archive/issue_comments_104950.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+When I try to put something like `$\\sigma'_0$` on plots, the prime looks\n+very big (see attached pdf). I found that `$\\sigma^'_0$` looks OK, but\n+there should be no need for such tricks, especially since this forms\n+looks bad in proper LaTeX.\n \n``````\n",
    "created_at": "2011-03-24T02:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104950",
    "user": "https://github.com/sagetrac-mdboom"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+When I try to put something like `$\sigma'_0$` on plots, the prime looks
+very big (see attached pdf). I found that `$\sigma^'_0$` looks OK, but
+there should be no need for such tricks, especially since this forms
+looks bad in proper LaTeX.
 
``````




---

archive/issue_comments_104951.json:
```json
{
    "body": "Changed keywords from **** to **sd40.5**",
    "created_at": "2012-05-24T22:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104951",
    "user": "https://github.com/novoselt"
}
```

Changed keywords from **** to **sd40.5**



---

archive/issue_comments_104952.json:
```json
{
    "body": "Changed upstream from **Reported upstream. Little or no feedback.** to **Fixed upstream, in a later stable release.**",
    "created_at": "2012-05-24T22:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104952",
    "user": "https://github.com/novoselt"
}
```

Changed upstream from **Reported upstream. Little or no feedback.** to **Fixed upstream, in a later stable release.**



---

archive/issue_comments_104953.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nIn Sage-5.1.beta0 everything looks fine!",
    "created_at": "2012-05-24T22:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104953",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:5'>Comment 5:</a>
In Sage-5.1.beta0 everything looks fine!



---

archive/issue_events_129043.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-24T22:19:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129043"
}
```



---

archive/issue_events_129044.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-24T22:19:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129044"
}
```



---

archive/issue_events_129045.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-24T22:19:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129045"
}
```



---

archive/issue_comments_104954.json:
```json
{
    "body": "Reviewer: **Andrey Novoseltsev**",
    "created_at": "2012-05-24T22:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10717#issuecomment-104954",
    "user": "https://github.com/novoselt"
}
```

Reviewer: **Andrey Novoseltsev**



---

archive/issue_events_129046.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-24T22:19:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129046"
}
```



---

archive/issue_events_129047.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-24T22:20:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129047"
}
```



---

archive/issue_events_129048.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-24T22:20:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129048"
}
```



---

archive/issue_events_129049.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T13:30:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129049"
}
```



---

archive/issue_events_129050.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T13:30:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129050"
}
```



---

archive/issue_events_129051.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T13:30:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10717",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10717#event-129051"
}
```
