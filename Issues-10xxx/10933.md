...
```json
{
    "body": "This isn't a magma issue per se.  I get a similar issue with gp:\n\n```\ndef foo(str):\n     time gp(str)\n\nfoo('1')\n```",
...
```json
{
    "body": "OK, investigated this for IPython. The issue is that it evaluates the expression *gp(s)* within the main user namespace, not within the function's internal namespace (where s lives). Off the top of my head, I can't think of a neat way for the time command to get the namespace from inside the function.",
...
```json
{
    "body": "I fixed this by making the preparse_ipython preparse the time just as it already happens in the notebook.",
    "created_at": "2011-03-22T17:09:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10933",
    "type": "issue_comment",
...
```json
{
    "body": "That's ok, but let us hope that is gets fixed in IPython soon.",
...
```json
{
    "body": "We're looking at a way to make %time evaluate things in the correct scope, but in the current ipython trunk, auto-magics (like \"time\" without the %) are only used in single-line inputs. So you'd have to do:\n\n```\ndef f(x):\n    %time gp(x)\n```",
...
```json
{
    "body": "I've written some simple code to make this work in IPython, if you want to test my branch here: https://github.com/ipython/ipython/pull/295",
