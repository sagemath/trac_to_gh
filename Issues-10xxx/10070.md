...
```json
{
    "body": "See also #10071 for how this would allow `step(hold=True)` to be evaluated post hoc.",
...
```json
{
    "body": "I opened an issue: https://github.com/pynac/pynac/issues/243",
...
```json
{
    "body": "Note that if you allow a separate `step()` then a `Function` in `src/sage/functions` would be needed because atm only `x.step()` works not `step(x)`.",
...
```json
{
    "body": "Of course in the final version the name is set back to `unit_step` after step in Pynac is named unit_step. Also the doctests need to be moved to the class docstring.",
...
```json
{
    "body": "I will try to include the relevant changes in the next Pynac version 0.7.6.",
...
```json
{
    "body": "Replying to [comment:35 mforets]:\n> we still have issues that have been raised in this thread that should be fixed (optional definition at 0, use in `integrate` fails, use in `desolve_rk4` fails) -- but i guess that should be a new ticket for `heaviside` only?\n\n\nAgreed.",
...
```json
{
    "body": "update docstring of `step`, add see also blocks.\n\n---\nNew commits:",
...
```json
{
    "body": "Your last commit will likely conflict with a similar change I made on #22838. Once you merge in those changes, you can set a positive review on my behalf (and add your name to the reviewers list).",
