# Issue 10565: another bug in symbolic sum

archive/issues_010512.json:
```json
{
    "body": "This was reported by Alexandre Casamayou:\n\n```\nsage: var('n')\nn\nsage: assume(x<0)\nsage: sum (x^n/(2*n+1), n, 0, infinity)\narctanh(sqrt(x))/sqrt(x)\n```\nIn the last result `x` should be `-x` since x is negative.\nThis is what we get if we replace `x` by `-y`:\n\n```\nsage: var('y')\ny\nsage: assume(y>0)\nsage: sum ((-y)^n/(2*n+1), n, 0, infinity)\narctan(sqrt(y))/sqrt(y)\n```\n\nAssignee: @burcin\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/10565\n\n",
    "closed_at": "2011-01-07T08:01:50Z",
    "created_at": "2011-01-06T21:04:23Z",
    "labels": [
        "component: calculus",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.6.1",
    "title": "another bug in symbolic sum",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10565",
    "user": "https://github.com/zimmermann6"
}
```
This was reported by Alexandre Casamayou:

```
sage: var('n')
n
sage: assume(x<0)
sage: sum (x^n/(2*n+1), n, 0, infinity)
arctanh(sqrt(x))/sqrt(x)
```
In the last result `x` should be `-x` since x is negative.
This is what we get if we replace `x` by `-y`:

```
sage: var('y')
y
sage: assume(y>0)
sage: sum ((-y)^n/(2*n+1), n, 0, infinity)
arctan(sqrt(y))/sqrt(y)
```

Assignee: @burcin

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/10565





---

archive/issue_comments_114877.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2011-01-07T08:01:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10565#issuecomment-114877",
    "user": "https://github.com/zimmermann6"
}
```

Resolution: invalid



---

archive/issue_events_027154.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-01-07T08:01:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10565",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10565#event-27154"
}
```



---

archive/issue_comments_114878.json:
```json
{
    "body": "<a id='comment:1'></a>I retract this, since the formula with `sqrt(x)` is in fact valid over the complex numbers,\nmoreover the formula with `sqrt(-x)` is wrong (consider `x=-0.9` as example).\n\nPaul",
    "created_at": "2011-01-07T08:01:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10565#issuecomment-114878",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:1'></a>I retract this, since the formula with `sqrt(x)` is in fact valid over the complex numbers,
moreover the formula with `sqrt(-x)` is wrong (consider `x=-0.9` as example).

Paul
