...
```json
{
    "body": "<a id='comment:2'></a>Question on 2. How are ids determined?",
...
<a id='comment:2'></a>Question on 2. How are ids determined?
...
<a id='comment:4'></a>Regarding 1 & 3:  I am making good progress on Sage scripts to compute all the data in all the old-style files (i.e. tables 1-8)  taking as input just the "short" version of Table 1 (with only the optimal curve from each class).  There is no reason why these would not also work on the S-W database curves.  The only assumption is that the input file contains lines like
...
```json
{
    "body": "<a id='comment:5'></a>Since you are already working on 1 and 3, I will start some version of 2. I am reading the lecture notes. But it might come down to simply converting Stein Watkins in a different format (as similar to yours as possible), but keep it as a separate database.",
...
<a id='comment:5'></a>Since you are already working on 1 and 3, I will start some version of 2. I am reading the lecture notes. But it might come down to simply converting Stein Watkins in a different format (as similar to yours as possible), but keep it as a separate database.
...
<a id='comment:6'></a>After reading the labeling section of the paper, I have come up with the following conclusion.
...
<a id='comment:7'></a>Is there any point in including SW curves of conductor < 130000 which are already in my database?
...
<a id='comment:8'></a>I actually thought about question 2 on Thursday.
...
<a id='comment:9'></a>This seems quite sensible to me -- but then perhaps I would say that, since the proposal is that my database is special and does not require any prefix, while everyone else's needs some qualification!
...
```json
{
    "body": "<a id='comment:10'></a>In light of #11587, can someone update the goals of this ticket?",
...
<a id='comment:10'></a>In light of #11587, can someone update the goals of this ticket?
...
```json
{
    "body": "<a id='comment:17'></a>I assume this ticket can be closed as \"invalid\" now, but perhaps John Cremona should confirm this.",
    "created_at": "2014-09-04T12:38:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10581",
    "type": "issue_comment",
...
<a id='comment:17'></a>I assume this ticket can be closed as "invalid" now, but perhaps John Cremona should confirm this.
...
```json
{
    "body": "<a id='comment:19'></a>I corrected item 1 since #11587 actually uses SQLITE.  As for 2 and 3, I think these are better left to the LMFDB project, where 3 is already done and work on 2 is ongoing.\n\nHence yes: close this ticket.",
...
<a id='comment:19'></a>I corrected item 1 since #11587 actually uses SQLITE.  As for 2 and 3, I think these are better left to the LMFDB project, where 3 is already done and work on 2 is ongoing.
