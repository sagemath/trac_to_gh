# Issue 10932: Fix typo in the multiple edges detection code of DiGraph

archive/issues_010854.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: DiGraph([(1, 2, 0), (1,2,1)])\nTraceback (most recent call last):\n  ...\n  File \"/opt/sage-4.6.2/local/lib/python2.6/site-packages/sage/graphs/digraph.py\", line 417, in __init__\n    (u in data[v])):\nKeyError: 2\n\nsage: DiGraph([(1, 2, 0)])\n------------------------------------------------------------\nTraceback (most recent call last):\n  ...\n  File \"/opt/sage-4.6.2/local/lib/python2.6/site-packages/sage/graphs/digraph.py\", line 417, in __init__\n    (u in data[v])):\nKeyError: 2\n\n```\n\nThe test should have been v in data[u]. I'll attach soon a patch doing this:\n\n```\n     sage: DiGraph([(1, 2, 0), (1,2,1)])\n     Multi-digraph on 2 vertices\n     sage: DiGraph([(1, 2, 0)])\n     Digraph on 2 vertices\n```\n\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  @nathanncohen\n\nAuthor: **Nicolas M. Thi\u00e9ry**\n\nReviewer: **Nathann Cohen**\n\nMerged: **sage-4.7.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10932_\n\n",
    "closed_at": "2011-03-18T19:18:30Z",
    "created_at": "2011-03-14T13:04:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix typo in the multiple edges detection code of DiGraph",
    "type": "issue",
    "updated_at": "2011-03-18T19:18:30Z",
    "url": "https://github.com/sagemath/sage/issues/10932",
    "user": "https://github.com/nthiery"
}
```

```
sage: DiGraph([(1, 2, 0), (1,2,1)])
Traceback (most recent call last):
  ...
  File "/opt/sage-4.6.2/local/lib/python2.6/site-packages/sage/graphs/digraph.py", line 417, in __init__
    (u in data[v])):
KeyError: 2

sage: DiGraph([(1, 2, 0)])
------------------------------------------------------------
Traceback (most recent call last):
  ...
  File "/opt/sage-4.6.2/local/lib/python2.6/site-packages/sage/graphs/digraph.py", line 417, in __init__
    (u in data[v])):
KeyError: 2

```

The test should have been v in data[u]. I'll attach soon a patch doing this:

```
     sage: DiGraph([(1, 2, 0), (1,2,1)])
     Multi-digraph on 2 vertices
     sage: DiGraph([(1, 2, 0)])
     Digraph on 2 vertices
```


Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  @nathanncohen

Author: **Nicolas M. Thiéry**

Reviewer: **Nathann Cohen**

Merged: **sage-4.7.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/10932_





---

archive/issue_events_127877.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-03-14T13:04:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127877"
}
```



---

archive/issue_events_127878.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-03-14T13:04:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127878"
}
```



---

archive/issue_events_127879.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-03-14T13:04:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127879"
}
```



---

archive/issue_events_127880.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-03-14T13:04:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127880"
}
```



---

archive/issue_events_127881.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-03-14T13:10:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127881"
}
```



---

archive/issue_comments_108345.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_10932-digraphs-multiple_edges_constructor-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10932/trac_10932-digraphs-multiple_edges_constructor-nt.patch.gz)**",
    "created_at": "2011-03-14T13:10:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10932#issuecomment-108345",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_10932-digraphs-multiple_edges_constructor-nt.patch.gz](https://github.com/sagemath/sage/files/ticket10932/trac_10932-digraphs-multiple_edges_constructor-nt.patch.gz)**



---

archive/issue_comments_108346.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBtw: pyflakes complains about ``choice`` not being defined on l. 602 of this file. But I leave that to someone else to investigate this further.",
    "created_at": "2011-03-14T13:18:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10932#issuecomment-108346",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:2'>Comment 2:</a>
Btw: pyflakes complains about ``choice`` not being defined on l. 602 of this file. But I leave that to someone else to investigate this further.



---

archive/issue_comments_108347.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,7 +7,7 @@\n     (u in data[v])):\n KeyError: 2\n \n-sage:             sage: DiGraph([(1, 2, 0)])\n+sage: DiGraph([(1, 2, 0)])\n ------------------------------------------------------------\n Traceback (most recent call last):\n   ...\n``````\n",
    "created_at": "2011-03-14T13:18:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10932#issuecomment-108347",
    "user": "https://github.com/nthiery"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,7 +7,7 @@
     (u in data[v])):
 KeyError: 2
 
-sage:             sage: DiGraph([(1, 2, 0)])
+sage: DiGraph([(1, 2, 0)])
 ------------------------------------------------------------
 Traceback (most recent call last):
   ...
``````




---

archive/issue_events_127882.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-03-14T14:16:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127882"
}
```



---

archive/issue_events_127883.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-03-14T14:16:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127883"
}
```



---

archive/issue_comments_108348.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI thought there would be a similar problem in graph.py, but the situation there is obviously more... symmetric `:-)`\n\nNathann",
    "created_at": "2011-03-14T14:16:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10932#issuecomment-108348",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'>Comment 3:</a>
I thought there would be a similar problem in graph.py, but the situation there is obviously more... symmetric `:-)`

Nathann



---

archive/issue_comments_108349.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nReplying to [@nathanncohen](#comment%3A3):\n> I thought there would be a similar problem in graph.py, but the situation there is obviously more... symmetric `:-)`\n> \n> Nathann\n\nThanks for the review!",
    "created_at": "2011-03-14T16:01:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10932#issuecomment-108349",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:4'>Comment 4:</a>
Replying to [@nathanncohen](#comment%3A3):
> I thought there would be a similar problem in graph.py, but the situation there is obviously more... symmetric `:-)`
> 
> Nathann

Thanks for the review!



---

archive/issue_events_127884.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-16T11:19:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127884"
}
```



---

archive/issue_comments_108350.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2011-03-18T13:52:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10932#issuecomment-108350",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_127885.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-18T19:18:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127885"
}
```



---

archive/issue_events_127886.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-18T19:18:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10932#event-127886"
}
```



---

archive/issue_comments_108351.json:
```json
{
    "body": "Merged: **sage-4.7.alpha2**",
    "created_at": "2011-03-18T19:18:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10932#issuecomment-108351",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha2**
