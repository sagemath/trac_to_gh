# Issue 10020: points(list of lists) doesn't work

archive/issues_010019.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/williamstein"
    ],
    "body": "This doesn't work:\n\n```\npts= [[0, 1], [1, 0], [1.5, 1],[2,1],[5,2],[6,3],[7,3]]\npoints(pts)\n```\n\nbut this does\n\n```\npts= [[0, 1], [1, 0], [1.5, 1],[2,1],[5,2],[6,3],[7,3]]\npoint(pts)\n```\n\nComponent: **graphics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10020_\n\n",
    "closed_at": "2010-09-27T22:59:47Z",
    "created_at": "2010-09-27T13:19:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/good%20first%20issue",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "points(list of lists) doesn't work",
    "type": "issue",
    "updated_at": "2010-09-27T22:59:47Z",
    "url": "https://github.com/sagemath/sage/issues/10020",
    "user": "https://github.com/jasongrout"
}
```
This doesn't work:

```
pts= [[0, 1], [1, 0], [1.5, 1],[2,1],[5,2],[6,3],[7,3]]
points(pts)
```

but this does

```
pts= [[0, 1], [1, 0], [1.5, 1],[2,1],[5,2],[6,3],[7,3]]
point(pts)
```

Component: **graphics**

_Issue created by migration from https://trac.sagemath.org/ticket/10020_





---

archive/issue_events_130728.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T13:19:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130728"
}
```



---

archive/issue_events_130729.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T13:19:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130729"
}
```



---

archive/issue_events_130730.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T13:19:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130730"
}
```



---

archive/issue_events_130731.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T13:19:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130731"
}
```



---

archive/issue_events_130732.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T13:19:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130732"
}
```



---

archive/issue_events_130733.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T13:19:48Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130733"
}
```



---

archive/issue_events_130734.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-09-27T13:19:48Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130734"
}
```



---

archive/issue_comments_091558.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAren't those the same statements?",
    "created_at": "2010-09-27T18:41:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10020#issuecomment-91558",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

Aren't those the same statements?



---

archive/issue_comments_091559.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\none is plural, the other singular.",
    "created_at": "2010-09-27T22:56:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10020#issuecomment-91559",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:2" align="right">comment:2</div>

one is plural, the other singular.



---

archive/issue_comments_091560.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nIt seems to work now.  I probably redefined \"points\" somewhere when I originally ran into it.\n\nYeah, Mike, I thought it was weird because I thought one was an alias of the other.",
    "created_at": "2010-09-27T22:59:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10020#issuecomment-91560",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:3" align="right">comment:3</div>

It seems to work now.  I probably redefined "points" somewhere when I originally ran into it.

Yeah, Mike, I thought it was weird because I thought one was an alias of the other.



---

archive/issue_events_130735.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T22:59:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130735"
}
```



---

archive/issue_events_130736.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T22:59:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130736"
}
```



---

archive/issue_events_130737.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T22:59:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130737"
}
```



---

archive/issue_events_130738.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-09-27T22:59:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10020",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10020#event-130738"
}
```
