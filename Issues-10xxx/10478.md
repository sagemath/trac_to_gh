...
<a id='comment:1'></a>Here's a patch which corrects the problem, but on my test machine, I'm getting doctest errors in `shapes2.py` just like those reported at #9088:
...
```json
{
    "body": "<a id='comment:2'></a>I'm marking this as \"needs review\" because it is ready for review -- I hope someone can explain the above test failures so I can fix them.",
...
<a id='comment:2'></a>I'm marking this as "needs review" because it is ready for review -- I hope someone can explain the above test failures so I can fix them.
...
```json
{
    "body": "<a id='comment:3'></a>The doctest failures are due to an internal counter used for uniqification. You can just change them to the new output, or \"texture...\"",
...
<a id='comment:3'></a>The doctest failures are due to an internal counter used for uniqification. You can just change them to the new output, or "texture..."
...
```json
{
    "body": "<a id='comment:4'></a>New patch up; I just used the new values for the textures to fix the doctests. Please review.",
...
<a id='comment:4'></a>New patch up; I just used the new values for the textures to fix the doctests. Please review.
