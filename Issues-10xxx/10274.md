# Issue 10274: Example coercion code is broken

archive/issues_010273.json:
```json
{
    "assignees": [],
    "body": "I noticed that the example code at http://sagemath.org/doc/reference/coercion.html\nis actually broken -- no surprise, since not tested = broken :-(.  \nThe fix is easy:\n\n   x = x._x\n\nshould be \n\n   x = x._value\n\nAssignee: **@robertwb**\n\nAuthor: **Robert Bradshaw**\n\nReviewer: **William Stein**\n\nMerged: **sage-4.6.1.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10274_\n\n",
    "closed_at": "2010-11-18T13:04:23Z",
    "created_at": "2010-11-15T17:34:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Example coercion code is broken",
    "type": "issue",
    "updated_at": "2010-11-18T13:04:23Z",
    "url": "https://github.com/sagemath/sage/issues/10274",
    "user": "https://github.com/williamstein"
}
```
I noticed that the example code at http://sagemath.org/doc/reference/coercion.html
is actually broken -- no surprise, since not tested = broken :-(.  
The fix is easy:

   x = x._x

should be 

   x = x._value

Assignee: **@robertwb**

Author: **Robert Bradshaw**

Reviewer: **William Stein**

Merged: **sage-4.6.1.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/10274_





---

archive/issue_events_123140.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-11-15T17:34:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10274#event-123140"
}
```



---

archive/issue_events_123141.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-11-15T17:34:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10274#event-123141"
}
```



---

archive/issue_events_123142.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-11-15T17:34:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10274#event-123142"
}
```



---

archive/issue_events_123143.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-11-15T17:34:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10274#event-123143"
}
```



---

archive/issue_comments_097820.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[10274-coercion-docs.patch.gz](https://github.com/sagemath/sage/files/ticket10274/10274-coercion-docs.patch.gz)**",
    "created_at": "2010-11-17T06:47:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10274#issuecomment-97820",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[10274-coercion-docs.patch.gz](https://github.com/sagemath/sage/files/ticket10274/10274-coercion-docs.patch.gz)**



---

archive/issue_events_123144.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-11-17T06:47:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10274#event-123144"
}
```



---

archive/issue_events_123145.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-11-17T06:47:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10274#event-123145"
}
```



---

archive/issue_events_123146.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-11-17T06:47:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10274#event-123146"
}
```



---

archive/issue_comments_097821.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2010-11-18T09:56:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10274#issuecomment-97821",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **William Stein**



---

archive/issue_comments_097822.json:
```json
{
    "body": "Author: **Robert Bradshaw**",
    "created_at": "2010-11-18T09:56:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10274#issuecomment-97822",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Robert Bradshaw**



---

archive/issue_events_123147.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-18T13:04:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10274#event-123147"
}
```



---

archive/issue_events_123148.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-18T13:04:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10274#event-123148"
}
```



---

archive/issue_comments_097823.json:
```json
{
    "body": "Merged: **sage-4.6.1.alpha2**",
    "created_at": "2010-11-18T13:04:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10274#issuecomment-97823",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.1.alpha2**
