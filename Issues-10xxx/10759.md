# Issue 10759: remove dead code in matrix/matrix_dense.pyx

archive/issues_010696.json:
```json
{
    "body": "while trying to improve the coverage of `matrix/matrix_dense.pyx`,\nwe noticed with Marc Fuentes that the function\n`_multiply_classical` from that file is dead code.\nIndeed, all instances of that function in other files are locally\ndefined, and it seems impossible to exhibit a test which calls that\nfunction. We therefore propose to remove that dead code.\n\nIndeed, after removing that function, all doctests still pass.\n\nAssignee: jason, was\n\nCC:  marc fuentes\n\nResolution: fixed\n\nAuthor: Marc Fuentes, Paul Zimmermann\n\nReviewer: Nathann Cohen\n\nMerged: sage-4.7.1.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/10759\n\n",
    "closed_at": "2011-05-25T12:52:38Z",
    "created_at": "2011-02-08T22:05:58Z",
    "labels": [
        "component: linear algebra",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.7.1",
    "title": "remove dead code in matrix/matrix_dense.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10759",
    "user": "https://github.com/zimmermann6"
}
```
while trying to improve the coverage of `matrix/matrix_dense.pyx`,
we noticed with Marc Fuentes that the function
`_multiply_classical` from that file is dead code.
Indeed, all instances of that function in other files are locally
defined, and it seems impossible to exhibit a test which calls that
function. We therefore propose to remove that dead code.

Indeed, after removing that function, all doctests still pass.

Assignee: jason, was

CC:  marc fuentes

Resolution: fixed

Author: Marc Fuentes, Paul Zimmermann

Reviewer: Nathann Cohen

Merged: sage-4.7.1.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/10759





---

archive/issue_comments_135992.json:
```json
{
    "body": "Attachment [trac_10759.patch](tarball://root/attachments/some-uuid/ticket10759/trac_10759.patch) by @zimmermann6 created at 2011-02-08 22:07:18",
    "created_at": "2011-02-08T22:07:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10759#issuecomment-135992",
    "user": "https://github.com/zimmermann6"
}
```

Attachment [trac_10759.patch](tarball://root/attachments/some-uuid/ticket10759/trac_10759.patch) by @zimmermann6 created at 2011-02-08 22:07:18



---

archive/issue_comments_135993.json:
```json
{
    "body": "Changing author from \"\" to \"Marc Fuentes, Paul Zimmermann\"",
    "created_at": "2011-02-08T22:07:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10759#issuecomment-135993",
    "user": "https://github.com/zimmermann6"
}
```

Changing author from "" to "Marc Fuentes, Paul Zimmermann"



---

archive/issue_comments_135994.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-02-08T22:07:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10759#issuecomment-135994",
    "user": "https://github.com/zimmermann6"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_135995.json:
```json
{
    "body": "<a id='comment:3'></a>Passes -testall -long, and it's a private method anyway... So I guess it's safe to let it go `:-)`\n\nNathann",
    "created_at": "2011-05-08T18:21:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10759#issuecomment-135995",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>Passes -testall -long, and it's a private method anyway... So I guess it's safe to let it go `:-)`

Nathann



---

archive/issue_comments_135996.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-05-08T18:21:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10759#issuecomment-135996",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_135997.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Nathann Cohen\"",
    "created_at": "2011-05-08T18:21:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10759#issuecomment-135997",
    "user": "https://github.com/nathanncohen"
}
```

Changing reviewer from "" to "Nathann Cohen"



---

archive/issue_events_027733.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-10T07:31:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "milestone": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10759#event-27733"
}
```



---

archive/issue_comments_135998.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-05-25T12:52:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10759#issuecomment-135998",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_027734.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-25T12:52:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10759#event-27734"
}
```



---

archive/issue_comments_135999.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.7.1.alpha1\"",
    "created_at": "2011-05-25T12:52:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10759#issuecomment-135999",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.7.1.alpha1"
