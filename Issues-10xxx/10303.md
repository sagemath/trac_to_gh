...
```json
{
    "body": "Attachment [trac_10303-scripts-SAGE64.patch](tarball://root/attachments/some-uuid/ticket10303/trac_10303-scripts-SAGE64.patch) by @jhpalmieri created at 2010-11-21 00:43:00",
    "created_at": "2010-11-21T00:43:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10303",
    "type": "issue_comment",
...
```json
{
    "body": "Like Peter on sage-devel, I think it's bad to restrict this SAGE64 to OS X and Solaris, as these two systems are certainly not the only ones which default to 32-bit, but can build 64-bit.",
...
```json
{
    "body": "Okay, here's a \"v3\" patch which allows the use of SAGE64 on any platform.  It also requires replacing spkg/install with the attached version (that's the same for v2 and v3).  I'm leaving v2 up here in case there is more debate about which approach is better: to allow SAGE64 on any system or not.",
...
```json
{
    "body": "Here's a new version of the v3 patch.",
...
```json
{
    "body": "Dave: I agree 100% that having files in `spkg/base` and in the `scripts` repo is a mess.  This has caused problems for me countless times.  I will now also handle `sage-check-64` specially, so it should work.",
...
```json
{
    "body": "The commit message of [attachment:trac_10303-scripts-SAGE64.v3.patch] should not be one long line.  Provide a short description on the first line (this will appear in `hg log`).  Any further info should go on following lines.",
...
```json
{
    "body": "Seriously?  You're holding up the patch because of this?  The release manager can easily fix an issue like this in about 2 minutes.  Instead, ten hours later, I'm going to post a new patch.",
    "created_at": "2010-12-03T00:34:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10303",
    "type": "issue_comment",
...
```json
{
    "body": "Trouble with `sage -ba`, see #10436.",
...
```json
{
    "body": "I don't think I have any time to work on this right now.  If no one else has time either, then the best solution might be to back it out; it was not very high priority to begin with.",
...
```json
{
    "body": "New patches here should perhaps be based on #9960 (which currently needs review, but hopefully gets merged soon before it further rottens - just rebased it to Sage 4.6.1.rc0.)\n\n(#9960 mainly patches `sage-spkg`, but also `sage-sage` and `sage-env`.)",
...
```json
{
    "body": "The status is, the patches here don't work with upgrades.  There was also some debate about the approach in general.  If someone else has the time and energy to work on it, please go ahead; it might be best to start over.",
...
```json
{
    "body": "For future reference: if the proposed fix works like the old one -- copying sage-check-64 to spkg/base -- then you must add sage-check-64 to the .hgignore file for the base repo.  See also #11073 for other issues dealing with the base repo.",
...
```json
{
    "body": "Here are some patches.  I believe these should also fix the problem at #10436.  (I think you can see that problem if you apply the scripts patch, but then revert just the file sage-build.  Then build the polybori spkg without SAGE64 being set.)",
    "created_at": "2011-03-29T04:45:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10303",
    "type": "issue_comment",
...
```json
{
    "body": "Changing milestone to 4.7.1 since it depends on #9960.",
    "created_at": "2011-05-12T15:10:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10303",
    "type": "issue_comment",
...
```json
{
    "body": "I think none of the issues mentioned are still relevant (some were fixed at #12789).  I'm going to close, but feel to protest and disagree.",
    "created_at": "2012-08-23T13:05:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10303",
    "type": "issue_comment",
