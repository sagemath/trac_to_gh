# Issue 10619: arange function for fractional ranges

archive/issues_010566.json:
```json
{
    "body": "This patch adds bindings for numpy's arange, and adds another algorithm that maintains precision. The advantage of these over Python's range is that they can accept fractional arguments.\n\nAssignee: @jasongrout\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/10619\n\n",
    "closed_at": "2011-01-15T03:09:13Z",
    "created_at": "2011-01-13T05:54:28Z",
    "labels": [
        "component: misc"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "arange function for fractional ranges",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10619",
    "user": "https://github.com/eviatarbach"
}
```
This patch adds bindings for numpy's arange, and adds another algorithm that maintains precision. The advantage of these over Python's range is that they can accept fractional arguments.

Assignee: @jasongrout

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/10619





---

archive/issue_comments_133335.json:
```json
{
    "body": "Attachment [15207.patch](tarball://root/attachments/some-uuid/ticket10619/15207.patch) by @eviatarbach created at 2011-01-13 06:06:41",
    "created_at": "2011-01-13T06:06:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10619#issuecomment-133335",
    "user": "https://github.com/eviatarbach"
}
```

Attachment [15207.patch](tarball://root/attachments/some-uuid/ticket10619/15207.patch) by @eviatarbach created at 2011-01-13 06:06:41



---

archive/issue_comments_133336.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-01-13T06:06:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10619#issuecomment-133336",
    "user": "https://github.com/eviatarbach"
}
```

Changing status from new to needs_review.



---

archive/issue_events_027335.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-15T03:09:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10619",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10619#event-27335"
}
```



---

archive/issue_comments_133337.json:
```json
{
    "body": "<a id='comment:2'></a>I found out about srange, so I will probably just add this patch to that.",
    "created_at": "2011-01-15T03:09:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10619#issuecomment-133337",
    "user": "https://github.com/eviatarbach"
}
```

<a id='comment:2'></a>I found out about srange, so I will probably just add this patch to that.



---

archive/issue_comments_133338.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2011-01-15T03:09:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10619#issuecomment-133338",
    "user": "https://github.com/eviatarbach"
}
```

Resolution: invalid



---

archive/issue_events_027336.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2011-01-16T17:37:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10619",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10619#event-27336"
}
```
