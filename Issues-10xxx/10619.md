# Issue 10619: arange function for fractional ranges

archive/issues_010566.json:
```json
{
    "assignees": [],
    "body": "This patch adds bindings for numpy's arange, and adds another algorithm that maintains precision. The advantage of these over Python's range is that they can accept fractional arguments.\n\nAssignee: **@jasongrout**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10619_\n\n",
    "closed_at": "2011-01-15T03:09:13Z",
    "created_at": "2011-01-13T05:54:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "arange function for fractional ranges",
    "type": "issue",
    "updated_at": "2011-01-16T17:37:35Z",
    "url": "https://github.com/sagemath/sage/issues/10619",
    "user": "https://github.com/eviatarbach"
}
```
This patch adds bindings for numpy's arange, and adds another algorithm that maintains precision. The advantage of these over Python's range is that they can accept fractional arguments.

Assignee: **@jasongrout**

_Issue created by migration from https://trac.sagemath.org/ticket/10619_





---

archive/issue_events_127707.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-13T05:54:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10619#event-127707"
}
```



---

archive/issue_events_127708.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-13T05:54:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10619#event-127708"
}
```



---

archive/issue_events_127709.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-13T05:54:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10619#event-127709"
}
```



---

archive/issue_comments_103487.json:
```json
{
    "body": "Attachment: **[15207.patch.gz](https://github.com/sagemath/sage/files/ticket10619/15207.patch.gz)**",
    "created_at": "2011-01-13T06:06:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10619#issuecomment-103487",
    "user": "https://github.com/eviatarbach"
}
```

Attachment: **[15207.patch.gz](https://github.com/sagemath/sage/files/ticket10619/15207.patch.gz)**



---

archive/issue_events_127710.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-13T06:06:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10619#event-127710"
}
```



---

archive/issue_events_127711.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-15T03:09:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10619#event-127711"
}
```



---

archive/issue_events_127712.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-15T03:09:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10619#event-127712"
}
```



---

archive/issue_comments_103488.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI found out about srange, so I will probably just add this patch to that.",
    "created_at": "2011-01-15T03:09:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10619#issuecomment-103488",
    "user": "https://github.com/eviatarbach"
}
```

<a id='comment:2'>Comment 2:</a>
I found out about srange, so I will probably just add this patch to that.



---

archive/issue_events_127713.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-15T03:09:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10619#event-127713"
}
```



---

archive/issue_events_127714.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-15T03:09:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10619#event-127714"
}
```



---

archive/issue_events_127715.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-01-16T17:37:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10619",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10619#event-127715"
}
```
