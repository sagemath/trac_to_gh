# Issue 10485: Thematic tutorial on affine KR crystals

archive/issues_010432.json:
```json
{
    "assignees": [],
    "body": "This builds on ticket #8442 and extends the thematic Lie tutorial for crystals and affine crystals.\n\nA copy of the html documentation built using \n\n```\nsage -docbuild thematic_tutorials html\n```\n\nis available here:\n\nhttp://www.math.ucdavis.edu/~anne/sage/lie/crystals.html\n\nhttp://www.math.ucdavis.edu/~anne/sage/lie/affine_crystals.html\n\n(which might not always be completely up to date).\n\n**Dependencies**: #10632, #10723\n\n**Apply** [attachment: trac_10485-thematic-tutorial-as.patch](https://github.com/sagemath/sage/files/ticket10485/trac_10485-thematic-tutorial-as.patch.gz)\n\nAssignee: **@anneschilling**\n\nCC:  @sagetrac-sage-combinat\n\nKeywords: **crystals, thematic tutorial**\n\nReviewer: **Daniel Bump**\n\nAuthor: **Anne Schilling**\n\nMerged: **sage-4.7.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10485_\n\n",
    "closed_at": "2011-03-24T20:39:18Z",
    "created_at": "2010-12-16T15:53:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Thematic tutorial on affine KR crystals",
    "type": "issue",
    "updated_at": "2023-01-14T16:14:31Z",
    "url": "https://github.com/sagemath/sage/issues/10485",
    "user": "https://github.com/anneschilling"
}
```
This builds on ticket #8442 and extends the thematic Lie tutorial for crystals and affine crystals.

A copy of the html documentation built using 

```
sage -docbuild thematic_tutorials html
```

is available here:

http://www.math.ucdavis.edu/~anne/sage/lie/crystals.html

http://www.math.ucdavis.edu/~anne/sage/lie/affine_crystals.html

(which might not always be completely up to date).

**Dependencies**: #10632, #10723

**Apply** [attachment: trac_10485-thematic-tutorial-as.patch](https://github.com/sagemath/sage/files/ticket10485/trac_10485-thematic-tutorial-as.patch.gz)

Assignee: **@anneschilling**

CC:  @sagetrac-sage-combinat

Keywords: **crystals, thematic tutorial**

Reviewer: **Daniel Bump**

Author: **Anne Schilling**

Merged: **sage-4.7.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/10485_





---

archive/issue_events_125538.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2010-12-16T15:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125538"
}
```



---

archive/issue_events_125539.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2010-12-16T15:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125539"
}
```



---

archive/issue_events_125540.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2010-12-16T15:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125540"
}
```



---

archive/issue_comments_100594.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,18 @@\n-The upcoming patch will contain a thematic tutorial on affine KR crystals. This is an enhancement of trac # 8442.\n+This builds on ticket #8422 and extends the thematic Lie tutorial for crystals and affine crystals.\n+\n+A copy of the html documentation built using \n+\n+```\n+sage -docbuild thematic_tutorials html\n+```\n+\n+is available here:\n+\n+http://www.math.ucdavis.edu/~anne/sage/lie/crystals.html\n+\n+http://www.math.ucdavis.edu/~anne/sage/lie/affine_crystals.html\n+\n+(which might not always be completely up to date).\n+\n+Depends on trac #10632.\n+\n``````\n",
    "created_at": "2011-02-04T13:44:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100594",
    "user": "https://github.com/anneschilling"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,18 @@
-The upcoming patch will contain a thematic tutorial on affine KR crystals. This is an enhancement of trac # 8442.
+This builds on ticket #8422 and extends the thematic Lie tutorial for crystals and affine crystals.
+
+A copy of the html documentation built using 
+
+```
+sage -docbuild thematic_tutorials html
+```
+
+is available here:
+
+http://www.math.ucdavis.edu/~anne/sage/lie/crystals.html
+
+http://www.math.ucdavis.edu/~anne/sage/lie/affine_crystals.html
+
+(which might not always be completely up to date).
+
+Depends on trac #10632.
+
``````




---

archive/issue_events_125541.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2011-02-04T13:44:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125541"
}
```



---

archive/issue_events_125542.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2011-02-04T13:44:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125542"
}
```



---

archive/issue_comments_100595.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nFixed broken doc test for new version of #10632.",
    "created_at": "2011-02-12T08:45:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100595",
    "user": "https://github.com/anneschilling"
}
```

<div id="comment:2" align="right">Comment 2</div>

Fixed broken doc test for new version of #10632.



---

archive/issue_events_125543.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2011-02-12T08:45:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125543"
}
```



---

archive/issue_events_125544.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2011-02-12T08:45:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125544"
}
```



---

archive/issue_comments_100596.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,5 +14,5 @@\n \n (which might not always be completely up to date).\n \n-Depends on trac #10632.\n+Depends on #10632\n \n``````\n",
    "created_at": "2011-03-10T08:00:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100596",
    "user": "https://github.com/anneschilling"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,5 +14,5 @@
 
 (which might not always be completely up to date).
 
-Depends on trac #10632.
+Depends on #10632
 
``````




---

archive/issue_comments_100597.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-This builds on ticket #8422 and extends the thematic Lie tutorial for crystals and affine crystals.\n+This builds on ticket #8442 and extends the thematic Lie tutorial for crystals and affine crystals.\n \n A copy of the html documentation built using \n \n``````\n",
    "created_at": "2011-03-10T22:36:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100597",
    "user": "https://github.com/anneschilling"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-This builds on ticket #8422 and extends the thematic Lie tutorial for crystals and affine crystals.
+This builds on ticket #8442 and extends the thematic Lie tutorial for crystals and affine crystals.
 
 A copy of the html documentation built using 
 
``````




---

archive/issue_comments_100598.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,5 +14,7 @@\n \n (which might not always be completely up to date).\n \n+This patch should be merged simultaneously with #10632.\n+\n Depends on #10632\n \n``````\n",
    "created_at": "2011-03-12T07:44:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100598",
    "user": "https://github.com/anneschilling"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,5 +14,7 @@
 
 (which might not always be completely up to date).
 
+This patch should be merged simultaneously with #10632.
+
 Depends on #10632
 
``````




---

archive/issue_comments_100599.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -18,3 +18,5 @@\n \n Depends on #10632\n \n+Depends on #10723\n+\n``````\n",
    "created_at": "2011-03-12T09:31:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100599",
    "user": "https://github.com/anneschilling"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -18,3 +18,5 @@
 
 Depends on #10632
 
+Depends on #10723
+
``````




---

archive/issue_comments_100600.json:
```json
{
    "body": "Changed assignee from **@sagetrac-sage-combinat** to **@anneschilling**",
    "created_at": "2011-03-12T09:31:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100600",
    "user": "https://github.com/anneschilling"
}
```

Changed assignee from **@sagetrac-sage-combinat** to **@anneschilling**



---

archive/issue_comments_100601.json:
```json
{
    "body": "#10485: acknowledge one NSF grant",
    "created_at": "2011-03-12T21:24:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100601",
    "user": "https://github.com/dwbump"
}
```

#10485: acknowledge one NSF grant



---

archive/issue_events_125545.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2011-03-12T21:28:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125545"
}
```



---

archive/issue_events_125546.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2011-03-12T21:28:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125546"
}
```



---

archive/issue_comments_100602.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nAttachment: **[trac_10485-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket10485/trac_10485-reviewer.patch.gz)**\n\nThis adds to the documentation of crystals in the Lie thematic tutorial. It must be merged simultaneously with #10632 (and after #10723). When all three patches #10723, #10632 and #10485 are merged, all doctests pass with sage-4.7.alpha1. The patches have been extensively reviewed in sage-combinat-devel and appropriately revised.\n\nThe reviewer patch adds acknowledgement of one NSF grant, which should have been cited in #8442.",
    "created_at": "2011-03-12T21:28:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100602",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:7" align="right">Comment 7</div>

Attachment: **[trac_10485-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket10485/trac_10485-reviewer.patch.gz)**

This adds to the documentation of crystals in the Lie thematic tutorial. It must be merged simultaneously with #10632 (and after #10723). When all three patches #10723, #10632 and #10485 are merged, all doctests pass with sage-4.7.alpha1. The patches have been extensively reviewed in sage-combinat-devel and appropriately revised.

The reviewer patch adds acknowledgement of one NSF grant, which should have been cited in #8442.



---

archive/issue_comments_100603.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nIgnore the reviewer patch since Anne Schilling added this info to the main patch.",
    "created_at": "2011-03-12T21:42:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100603",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:8" align="right">Comment 8</div>

Ignore the reviewer patch since Anne Schilling added this info to the main patch.



---

archive/issue_comments_100604.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -20,3 +20,4 @@\n \n Depends on #10723\n \n+The reviewer patch may be ignored since it has been incorporated in the main patch.\n``````\n",
    "created_at": "2011-03-12T21:43:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100604",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -20,3 +20,4 @@
 
 Depends on #10723
 
+The reviewer patch may be ignored since it has been incorporated in the main patch.
``````




---

archive/issue_comments_100605.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -20,4 +20,6 @@\n \n Depends on #10723\n \n+Apply trac_10485-thematic-tutorial-as.patch\n+\n The reviewer patch may be ignored since it has been incorporated in the main patch.\n``````\n",
    "created_at": "2011-03-14T13:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100605",
    "user": "https://github.com/anneschilling"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -20,4 +20,6 @@
 
 Depends on #10723
 
+Apply trac_10485-thematic-tutorial-as.patch
+
 The reviewer patch may be ignored since it has been incorporated in the main patch.
``````




---

archive/issue_comments_100606.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,12 +14,6 @@\n \n (which might not always be completely up to date).\n \n-This patch should be merged simultaneously with #10632.\n+**Dependencies**: #10632, #10723\n \n-Depends on #10632\n-\n-Depends on #10723\n-\n-Apply trac_10485-thematic-tutorial-as.patch\n-\n-The reviewer patch may be ignored since it has been incorporated in the main patch.\n+**Apply** [attachment: trac_10485-thematic-tutorial-as.patch](https://github.com/sagemath/sage/files/ticket10485/trac_10485-thematic-tutorial-as.patch.gz)\n``````\n",
    "created_at": "2011-03-17T19:33:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100606",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,12 +14,6 @@
 
 (which might not always be completely up to date).
 
-This patch should be merged simultaneously with #10632.
+**Dependencies**: #10632, #10723
 
-Depends on #10632
-
-Depends on #10723
-
-Apply trac_10485-thematic-tutorial-as.patch
-
-The reviewer patch may be ignored since it has been incorporated in the main patch.
+**Apply** [attachment: trac_10485-thematic-tutorial-as.patch](https://github.com/sagemath/sage/files/ticket10485/trac_10485-thematic-tutorial-as.patch.gz)
``````




---

archive/issue_comments_100607.json:
```json
{
    "body": "Merged: **sage-4.7.alpha2**",
    "created_at": "2011-03-18T13:40:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100607",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha2**



---

archive/issue_events_125547.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-18T13:40:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125547"
}
```



---

archive/issue_events_125548.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-18T13:40:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125548"
}
```



---

archive/issue_comments_100608.json:
```json
{
    "body": "Changed merged from **sage-4.7.alpha2** to none",
    "created_at": "2011-03-21T15:37:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100608",
    "user": "https://github.com/jdemeyer"
}
```

Changed merged from **sage-4.7.alpha2** to none



---

archive/issue_events_125549.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-21T15:37:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125549"
}
```



---

archive/issue_events_125550.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-21T15:37:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125550"
}
```



---

archive/issue_events_125551.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-21T15:37:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125551"
}
```



---

archive/issue_comments_100609.json:
```json
{
    "body": "Attachment: **[trac_10485-thematic-tutorial-as.patch.gz](https://github.com/sagemath/sage/files/ticket10485/trac_10485-thematic-tutorial-as.patch.gz)**",
    "created_at": "2011-03-21T21:03:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100609",
    "user": "https://github.com/anneschilling"
}
```

Attachment: **[trac_10485-thematic-tutorial-as.patch.gz](https://github.com/sagemath/sage/files/ticket10485/trac_10485-thematic-tutorial-as.patch.gz)**



---

archive/issue_comments_100610.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nI fixed the syntax for # optional in the thematic tutorial.\n\nAnne",
    "created_at": "2011-03-21T21:03:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100610",
    "user": "https://github.com/anneschilling"
}
```

<div id="comment:16" align="right">Comment 16</div>

I fixed the syntax for # optional in the thematic tutorial.

Anne



---

archive/issue_events_125552.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-24T20:39:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125552"
}
```



---

archive/issue_events_125553.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-24T20:39:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10485#event-125553"
}
```



---

archive/issue_comments_100611.json:
```json
{
    "body": "Merged: **sage-4.7.alpha3**",
    "created_at": "2011-03-24T20:39:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100611",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha3**



---

archive/issue_comments_100612.json:
```json
{
    "body": "Changed reviewer from **Dan Bump** to **Daniel Bump**",
    "created_at": "2011-05-03T13:25:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10485",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10485#issuecomment-100612",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Dan Bump** to **Daniel Bump**
