# Issue 10347: Implementation of is_(skew_)symmetrizable for matrices

archive/issues_010346.json:
```json
{
    "body": "matrices are lacking methods for testing (skew-)symmetrizability. Those methods are needed for the cluster algebra and quiver package, see Ticket #10298.\n\n**CC:**  franco.saliola@gmail.com hthomas@unb.ca\n\n**Keywords:** symmetrizable matrix\n\n**Author:** Christian Stump\n\n**Reviewer:** Hugh Thomas\n\n**Merged:** sage-5.0.beta5\n\nIssue created by migration from https://trac.sagemath.org/ticket/10347\n\n",
    "closed_at": "2012-02-22T10:44:25Z",
    "created_at": "2010-11-27T23:02:31Z",
    "labels": [
        "component: combinatorics",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "title": "Implementation of is_(skew_)symmetrizable for matrices",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10347",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```
matrices are lacking methods for testing (skew-)symmetrizability. Those methods are needed for the cluster algebra and quiver package, see Ticket #10298.

**CC:**  franco.saliola@gmail.com hthomas@unb.ca

**Keywords:** symmetrizable matrix

**Author:** Christian Stump

**Reviewer:** Hugh Thomas

**Merged:** sage-5.0.beta5

Issue created by migration from https://trac.sagemath.org/ticket/10347





---

archive/issue_events_079159.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-02-08T16:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "component: combinatorics",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79159"
}
```



---

archive/issue_events_079160.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-02-08T16:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79160"
}
```



---

archive/issue_comments_099161.json:
```json
{
    "body": "<a id='comment:2'></a>\nApply only trac_10347_is_skew_symmetrizable-cs.patch",
    "created_at": "2011-03-29T17:04:19Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99161",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:2'></a>
Apply only trac_10347_is_skew_symmetrizable-cs.patch



---

archive/issue_comments_099162.json:
```json
{
    "body": "<a id='comment:3'></a>\nYou should modify the header of your patch : add something like\n\n#10347 Implementation of is_(skew_)symmetrizable for matrices\n\nThen the buildbot will be more happy.",
    "created_at": "2011-06-01T14:13:37Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99162",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'></a>
You should modify the header of your patch : add something like

#10347 Implementation of is_(skew_)symmetrizable for matrices

Then the buildbot will be more happy.



---

archive/issue_comments_099163.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [chapoton](#comment%3A3):\n\n> Then the buildbot will be more happy.\n\ndone, thanks.",
    "created_at": "2011-06-08T21:14:09Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99163",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:4'></a>
Replying to [chapoton](#comment%3A3):

> Then the buildbot will be more happy.

done, thanks.



---

archive/issue_comments_099164.json:
```json
{
    "body": "**Reviewer:** hthomas@unb.ca",
    "created_at": "2011-06-09T21:50:43Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99164",
    "user": "https://github.com/hughrthomas"
}
```

**Reviewer:** hthomas@unb.ca



---

archive/issue_events_079161.json:
```json
{
    "actor": "https://github.com/hughrthomas",
    "created_at": "2011-06-09T21:50:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79161"
}
```



---

archive/issue_events_079162.json:
```json
{
    "actor": "https://github.com/hughrthomas",
    "created_at": "2011-06-09T21:50:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79162"
}
```



---

archive/issue_comments_099165.json:
```json
{
    "body": "<a id='comment:5'></a>\n* \n\n``` \nsage: M=matrix([[0,6],[0,0]]) \nsage: M.is_symmetrizable(return_diag=true)\n[1, 1] \n```\n\nThis M is not symmetrizable, and the returned diagonal matrix doesn't symmetrize it.\n\n* \n\n``` \nsage: M=matrix([2])\nsage: M.is_symmetrizable() \nFalse\n```\n\nThe code is checking that the diagonal entries are all zero even when testing for symmetrizability.\n\n* Zelevinsky spells his name with a final \"y\".",
    "created_at": "2011-06-09T21:50:43Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99165",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:5'></a>
* 

``` 
sage: M=matrix([[0,6],[0,0]]) 
sage: M.is_symmetrizable(return_diag=true)
[1, 1] 
```

This M is not symmetrizable, and the returned diagonal matrix doesn't symmetrize it.

* 

``` 
sage: M=matrix([2])
sage: M.is_symmetrizable() 
False
```

The code is checking that the diagonal entries are all zero even when testing for symmetrizability.

* Zelevinsky spells his name with a final "y".



---

archive/issue_events_079163.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-09T23:30:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79163"
}
```



---

archive/issue_events_079164.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-09T23:30:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79164"
}
```



---

archive/issue_comments_099166.json:
```json
{
    "body": "<a id='comment:6'></a>\nReplying to [hthomas](#comment%3A5):\n\nHi Hugh,\n\nThanks for testing the code!\n\n> * \n> \n> ``` \n> sage: M=matrix([[0,6],[0,0]]) \n> sage: M.is_symmetrizable(return_diag=true)\n> [1, 1] \n> ```\n> \n> This M is not symmetrizable, and the returned diagonal matrix doesn't symmetrize it.\n\nThe problem here was that the algorithm checks conditions on columns (in ``_travel_column``), but didn't check if an entry `i,j` is zero, that than `j,i` is zero as well. So the bottom left zero was read, but no check was made on the top right non-zero...\n\n> * \n> \n> ``` \n> sage: M=matrix([2])\n> sage: M.is_symmetrizable() \n> False\n> ```\n> \n> The code is checking that the diagonal entries are all zero even when testing for symmetrizability.\n\nI moved this piece of code, so it is only used for skew-symmetrizable matrices.\n\n> * Zelevinsky spells his name with a final \"y\".  \n\n:-)\n\nBest, Christian",
    "created_at": "2011-06-09T23:30:49Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99166",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:6'></a>
Replying to [hthomas](#comment%3A5):

Hi Hugh,

Thanks for testing the code!

> * 
> 
> ``` 
> sage: M=matrix([[0,6],[0,0]]) 
> sage: M.is_symmetrizable(return_diag=true)
> [1, 1] 
> ```
> 
> This M is not symmetrizable, and the returned diagonal matrix doesn't symmetrize it.

The problem here was that the algorithm checks conditions on columns (in ``_travel_column``), but didn't check if an entry `i,j` is zero, that than `j,i` is zero as well. So the bottom left zero was read, but no check was made on the top right non-zero...

> * 
> 
> ``` 
> sage: M=matrix([2])
> sage: M.is_symmetrizable() 
> False
> ```
> 
> The code is checking that the diagonal entries are all zero even when testing for symmetrizability.

I moved this piece of code, so it is only used for skew-symmetrizable matrices.

> * Zelevinsky spells his name with a final "y".  

:-)

Best, Christian



---

archive/issue_comments_099167.json:
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [stumpc5](#comment%3A4):\n> Replying to [chapoton](#comment%3A3):\n> \n> > Then the buildbot will be more happy.\n\n> \n> done, thanks.\n\nA second try -- hopefully, the buildbot accepts it this time...",
    "created_at": "2011-06-10T00:28:28Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99167",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:7'></a>
Replying to [stumpc5](#comment%3A4):
> Replying to [chapoton](#comment%3A3):
> 
> > Then the buildbot will be more happy.

> 
> done, thanks.

A second try -- hopefully, the buildbot accepts it this time...



---

archive/issue_comments_099168.json:
```json
{
    "body": "<a id='comment:8'></a>\nHello Christian,\n\nI guess the header is still bad. What did you do ? It seems that you modified the first line, and this is not good ! By hand ???\n\nI think you need only to use \"sage -hg qrefresh -e\" and to give a commit message in the editor that should open. Compare for instance with the header of #11311. But I do not feel very competent on the build bot, alas.. Maybe one should ask the question on the mailing-list.",
    "created_at": "2011-06-10T07:51:51Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99168",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'></a>
Hello Christian,

I guess the header is still bad. What did you do ? It seems that you modified the first line, and this is not good ! By hand ???

I think you need only to use "sage -hg qrefresh -e" and to give a commit message in the editor that should open. Compare for instance with the header of #11311. But I do not feel very competent on the build bot, alas.. Maybe one should ask the question on the mailing-list.



---

archive/issue_comments_099169.json:
```json
{
    "body": "<a id='comment:9'></a>\nReplying to [chapoton](#comment%3A8):\n> I guess the header is still bad. What did you do ? It seems that you modified the first line, and this is not good ! By hand ???\n> \n> I think you need only to use \"sage -hg qrefresh -e\" and to give a commit message in the editor that should open. Compare for instance with the header of #11311. But I do not feel very competent on the build bot, alas.. Maybe one should ask the question on the mailing-list.\n\nI did that now -- the only difference to the first version I sent yesterday is that there is no space between # and the number. Maybe that was my mistake...\n\nThanks for rechecking!",
    "created_at": "2011-06-10T11:02:26Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99169",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:9'></a>
Replying to [chapoton](#comment%3A8):
> I guess the header is still bad. What did you do ? It seems that you modified the first line, and this is not good ! By hand ???
> 
> I think you need only to use "sage -hg qrefresh -e" and to give a commit message in the editor that should open. Compare for instance with the header of #11311. But I do not feel very competent on the build bot, alas.. Maybe one should ask the question on the mailing-list.

I did that now -- the only difference to the first version I sent yesterday is that there is no space between # and the number. Maybe that was my mistake...

Thanks for rechecking!



---

archive/issue_comments_099170.json:
```json
{
    "body": "<a id='comment:10'></a>\nIf I understand it right, the test failures are not related to the patch...",
    "created_at": "2011-06-10T16:22:58Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99170",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:10'></a>
If I understand it right, the test failures are not related to the patch...



---

archive/issue_comments_099171.json:
```json
{
    "body": "**Changing reviewer** from \"hthomas@unb.ca\" to \"Hugh Thomas\".",
    "created_at": "2011-06-10T16:22:58Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99171",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

**Changing reviewer** from "hthomas@unb.ca" to "Hugh Thomas".



---

archive/issue_comments_099172.json:
```json
{
    "body": "<a id='comment:11'></a>\nHi Christian!\n\nBelow are all my comments.  The first one is something which should be fixed (but is trivial).  For the others, I don't know if any change needs to be made.  Suggestions occurred to me, but perhaps you had good reasons for doing things as you did, and in any case, it doesn't seem to me that these are likely to be routines that are time-critical.  I am perhaps over-thinking this process, because of its being the first time I am reviewing a patch.  \n\n* The description of the return value for is_skew_symmetrizable, is_skew_symmetric, and _check_symmetrizability when return_diag is true says that it returns the diagonal matrix, when in fact, it returns the diagonal of the diagonal matrix.  (I like the output as is, I just think the documentation should change.)\n\n* In _travel_column, technically, I think it makes more sense to check that i isn't in dict before getting self_ik and self_ki, and testing whether or not bool(self_ik) and bool(self_ki) agree -- if i is in dict, this has already been checked.  \n\n* When return_diag is set, I don't see the point of saying [ d[i] for i in sorted( d.keys() ) ].  Why not just say [ d[i] for i in range( self._ncols ) ]?\n\n* If the entries in the input matrix aren't in an integral domain, things will get ugly.  I realize that's far from the intended use case.  What's the protocol for something like that?  Maybe just that the docstring should say that the input is a matrix in an integral domain?  Or is it even necessary to do anything at all?\n\n* I don't really like the following, but, again, it's far from the intended use case (and it's not the fault of the code here):  \n\n```\nsage: R.<x>=PolynomialRing(QQ)\nsage: M=matrix([[1,x],[-x,1]])\nsage: M.is_symmetrizable(positive=true)\nTrue\nsage: M.is_symmetrizable(positive=true, return_diag=true)\n[1, -1]\n```\n\nI am happy to give a positive review as soon as you fix at least the first point above (assuming the tests, which are running on my computer right now, pass).  Or should I wait until the patchbot is also happy?  Unless you think someone else should also review it, since I'm pretty inexperienced.",
    "created_at": "2011-06-10T22:47:10Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99172",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:11'></a>
Hi Christian!

Below are all my comments.  The first one is something which should be fixed (but is trivial).  For the others, I don't know if any change needs to be made.  Suggestions occurred to me, but perhaps you had good reasons for doing things as you did, and in any case, it doesn't seem to me that these are likely to be routines that are time-critical.  I am perhaps over-thinking this process, because of its being the first time I am reviewing a patch.  

* The description of the return value for is_skew_symmetrizable, is_skew_symmetric, and _check_symmetrizability when return_diag is true says that it returns the diagonal matrix, when in fact, it returns the diagonal of the diagonal matrix.  (I like the output as is, I just think the documentation should change.)

* In _travel_column, technically, I think it makes more sense to check that i isn't in dict before getting self_ik and self_ki, and testing whether or not bool(self_ik) and bool(self_ki) agree -- if i is in dict, this has already been checked.  

* When return_diag is set, I don't see the point of saying [ d[i] for i in sorted( d.keys() ) ].  Why not just say [ d[i] for i in range( self._ncols ) ]?

* If the entries in the input matrix aren't in an integral domain, things will get ugly.  I realize that's far from the intended use case.  What's the protocol for something like that?  Maybe just that the docstring should say that the input is a matrix in an integral domain?  Or is it even necessary to do anything at all?

* I don't really like the following, but, again, it's far from the intended use case (and it's not the fault of the code here):  

```
sage: R.<x>=PolynomialRing(QQ)
sage: M=matrix([[1,x],[-x,1]])
sage: M.is_symmetrizable(positive=true)
True
sage: M.is_symmetrizable(positive=true, return_diag=true)
[1, -1]
```

I am happy to give a positive review as soon as you fix at least the first point above (assuming the tests, which are running on my computer right now, pass).  Or should I wait until the patchbot is also happy?  Unless you think someone else should also review it, since I'm pretty inexperienced.



---

archive/issue_events_079165.json:
```json
{
    "actor": "https://github.com/hughrthomas",
    "created_at": "2011-06-10T22:47:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79165"
}
```



---

archive/issue_events_079166.json:
```json
{
    "actor": "https://github.com/hughrthomas",
    "created_at": "2011-06-10T22:47:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79166"
}
```



---

archive/issue_events_079167.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-11T01:21:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79167"
}
```



---

archive/issue_events_079168.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-11T01:21:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79168"
}
```



---

archive/issue_comments_099173.json:
```json
{
    "body": "<a id='comment:13'></a>\nReplying to [hthomas](#comment%3A11):\n\nHi Hugh -- thanks for your comments!\n\n> * The description of the return value for is_skew_symmetrizable, is_skew_symmetric, and _check_symmetrizability when return_diag is true says that it returns the diagonal matrix, when in fact, it returns the diagonal of the diagonal matrix.  (I like the output as is, I just think the documentation should change.)\n\ndone.\n\n> * In _travel_column, technically, I think it makes more sense to check that i isn't in dict before getting self_ik and self_ki, and testing whether or not bool(self_ik) and bool(self_ki) agree -- if i is in dict, this has already been checked.  \n\ndone.\n\n> * When return_diag is set, I don't see the point of saying [ d[i] for i in sorted( d.keys() ) ].  Why not just say [ d[i] for i in range( self._ncols ) ]?\n\ndone.\n\n> * If the entries in the input matrix aren't in an integral domain, things will get ugly.  I realize that's far from the intended use case.  What's the protocol for something like that?  Maybe just that the docstring should say that the input is a matrix in an integral domain?  Or is it even necessary to do anything at all?\n\nI added a sentence saying that the input matrix should be over an integral domain (we actually need to be able to invert all nonzero entries we see in the matrix - but the methods are not meant to be used like that and no one is going to do so I guess). Also I added that the base ring must be ordered if `positive=True`.\n\n> * I don't really like the following, but, again, it's far from the intended use case\n\nThere are many methods which do only make sense in certain contexts, I don't really know how to avoid that...\n\n> Or should I wait until the patchbot is also happy?\n\nI hope it is now...\n\n> Unless you think someone else should also review it, since I'm pretty inexperienced.  \n\nIf someone wanna have a look that's fine, but you can also give it a positive review if no one is going to do so.\n\nThanks again! Christian",
    "created_at": "2011-06-11T01:21:21Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99173",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:13'></a>
Replying to [hthomas](#comment%3A11):

Hi Hugh -- thanks for your comments!

> * The description of the return value for is_skew_symmetrizable, is_skew_symmetric, and _check_symmetrizability when return_diag is true says that it returns the diagonal matrix, when in fact, it returns the diagonal of the diagonal matrix.  (I like the output as is, I just think the documentation should change.)

done.

> * In _travel_column, technically, I think it makes more sense to check that i isn't in dict before getting self_ik and self_ki, and testing whether or not bool(self_ik) and bool(self_ki) agree -- if i is in dict, this has already been checked.  

done.

> * When return_diag is set, I don't see the point of saying [ d[i] for i in sorted( d.keys() ) ].  Why not just say [ d[i] for i in range( self._ncols ) ]?

done.

> * If the entries in the input matrix aren't in an integral domain, things will get ugly.  I realize that's far from the intended use case.  What's the protocol for something like that?  Maybe just that the docstring should say that the input is a matrix in an integral domain?  Or is it even necessary to do anything at all?

I added a sentence saying that the input matrix should be over an integral domain (we actually need to be able to invert all nonzero entries we see in the matrix - but the methods are not meant to be used like that and no one is going to do so I guess). Also I added that the base ring must be ordered if `positive=True`.

> * I don't really like the following, but, again, it's far from the intended use case

There are many methods which do only make sense in certain contexts, I don't really know how to avoid that...

> Or should I wait until the patchbot is also happy?

I hope it is now...

> Unless you think someone else should also review it, since I'm pretty inexperienced.  

If someone wanna have a look that's fine, but you can also give it a positive review if no one is going to do so.

Thanks again! Christian



---

archive/issue_events_079169.json:
```json
{
    "actor": "https://github.com/hughrthomas",
    "created_at": "2011-06-11T16:00:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79169"
}
```



---

archive/issue_events_079170.json:
```json
{
    "actor": "https://github.com/hughrthomas",
    "created_at": "2011-06-11T16:00:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79170"
}
```



---

archive/issue_comments_099174.json:
```json
{
    "body": "<a id='comment:14'></a>\nHi Christian--\n\nOkay, looks good!\n\ncheers,\n\nHugh",
    "created_at": "2011-06-11T16:00:56Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99174",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:14'></a>
Hi Christian--

Okay, looks good!

cheers,

Hugh



---

archive/issue_comments_099175.json:
```json
{
    "body": "<a id='comment:15'></a>\nHi Christian--\n\nI was reading a bit more about what referees are supposed to do, and one thing is that they should be enforcing the convention that every function, even those beginning with an underscore, should have examples showing it in action.  (This is according to William Stein's blog post on refereeing.)  This isn't true of _travel_column.  Is this something that should be fixed, or should I not be taking what William Stein writes literally in this case?  \n\ncheers,\n\nHugh",
    "created_at": "2011-06-12T14:42:05Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99175",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:15'></a>
Hi Christian--

I was reading a bit more about what referees are supposed to do, and one thing is that they should be enforcing the convention that every function, even those beginning with an underscore, should have examples showing it in action.  (This is according to William Stein's blog post on refereeing.)  This isn't true of _travel_column.  Is this something that should be fixed, or should I not be taking what William Stein writes literally in this case?  

cheers,

Hugh



---

archive/issue_comments_099176.json:
```json
{
    "body": "<a id='comment:16'></a>\nReplying to [hthomas](#comment%3A15):\n\nHi Hugh,\n\n> I was reading a bit more about what referees are supposed to do, and one thing is that they should be enforcing the convention that every function, even those beginning with an underscore, should have examples showing it in action.  (This is according to William Stein's blog post on refereeing.)  This isn't true of _travel_column.  Is this something that should be fixed, or should I not be taking what William Stein writes literally in this case?  \n\nI don't really know -- I added an example, but as this function is not used anywhere but inside _check_symm..., the example doesn't give any inside to the reader. On the other hand, if someone brakes this function for whatever reason, then the doctest catches this. That's a huge plus!\n\nBest, Christian",
    "created_at": "2011-06-12T17:20:24Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99176",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:16'></a>
Replying to [hthomas](#comment%3A15):

Hi Hugh,

> I was reading a bit more about what referees are supposed to do, and one thing is that they should be enforcing the convention that every function, even those beginning with an underscore, should have examples showing it in action.  (This is according to William Stein's blog post on refereeing.)  This isn't true of _travel_column.  Is this something that should be fixed, or should I not be taking what William Stein writes literally in this case?  

I don't really know -- I added an example, but as this function is not used anywhere but inside _check_symm..., the example doesn't give any inside to the reader. On the other hand, if someone brakes this function for whatever reason, then the doctest catches this. That's a huge plus!

Best, Christian



---

archive/issue_events_079171.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-12T21:52:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79171"
}
```



---

archive/issue_events_079172.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-12T21:52:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79172"
}
```



---

archive/issue_comments_099177.json:
```json
{
    "body": "<a id='comment:17'></a>\nHi Hugh,\n\nwhat do you think about always forcing the entries in the diagonal matrix D to be positive? Otherwise, people might get confused as without this positivity condition the notion does not appear in the literature (does it?).\n\nBest, Christian",
    "created_at": "2011-06-12T21:52:27Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99177",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:17'></a>
Hi Hugh,

what do you think about always forcing the entries in the diagonal matrix D to be positive? Otherwise, people might get confused as without this positivity condition the notion does not appear in the literature (does it?).

Best, Christian



---

archive/issue_comments_099178.json:
```json
{
    "body": "<a id='comment:18'></a>\nHi Christian--\n\nYeah, on reflection, I think that's a good idea.  I'm pretty sure it's part of the standard definition of symmetrizable, too.  (Though I just checked, and wikipedia doesn't think so.)  Maybe you should just change the default setting of positive?  \n\ncheers,\n\nHugh",
    "created_at": "2011-06-12T22:07:19Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99178",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:18'></a>
Hi Christian--

Yeah, on reflection, I think that's a good idea.  I'm pretty sure it's part of the standard definition of symmetrizable, too.  (Though I just checked, and wikipedia doesn't think so.)  Maybe you should just change the default setting of positive?  

cheers,

Hugh



---

archive/issue_events_079173.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-12T22:29:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79173"
}
```



---

archive/issue_events_079174.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-12T22:29:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79174"
}
```



---

archive/issue_comments_099179.json:
```json
{
    "body": "<a id='comment:19'></a>\nReplying to [hthomas](#comment%3A18):\n\n> Yeah, on reflection, I think that's a good idea.  I'm pretty sure it's part of the standard definition of symmetrizable, too.  (Though I just checked, and wikipedia doesn't think so.)  Maybe you should just change the default setting of positive?  \n\ndone!\n\nNow as you write it, I remember that I checked the wiki page when I first wrote the method and saw that positivity is not assumed there. Maybe there is another context where symmetrizability is used. (Actually, our algorithm only works for integral domains with certain elements being invertible, while the general definition of symmetrizability doesn't assume that.)\n\nBest, Christian",
    "created_at": "2011-06-12T22:29:34Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99179",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:19'></a>
Replying to [hthomas](#comment%3A18):

> Yeah, on reflection, I think that's a good idea.  I'm pretty sure it's part of the standard definition of symmetrizable, too.  (Though I just checked, and wikipedia doesn't think so.)  Maybe you should just change the default setting of positive?  

done!

Now as you write it, I remember that I checked the wiki page when I first wrote the method and saw that positivity is not assumed there. Maybe there is another context where symmetrizability is used. (Actually, our algorithm only works for integral domains with certain elements being invertible, while the general definition of symmetrizability doesn't assume that.)

Best, Christian



---

archive/issue_events_079175.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-16T21:40:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "milestone": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79175"
}
```



---

archive/issue_events_079176.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/stumpc5",
    "created_at": "2011-06-16T21:40:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79176"
}
```



---

archive/issue_comments_099180.json:
```json
{
    "body": "<a id='comment:21'></a>\nFixed an issue with travel_column as described in #12408",
    "created_at": "2012-02-02T11:02:35Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99180",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:21'></a>
Fixed an issue with travel_column as described in #12408



---

archive/issue_comments_099181.json:
```json
{
    "body": "<a id='comment:22'></a>\nThe buildbot was unhappy because of some trailing whitespace, which I removed.",
    "created_at": "2012-02-11T20:51:26Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99181",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:22'></a>
The buildbot was unhappy because of some trailing whitespace, which I removed.



---

archive/issue_comments_099182.json:
```json
{
    "body": "<a id='comment:23'></a>\nI think the format of the patch might be correct now!  (I didn't know I should export it.)",
    "created_at": "2012-02-11T22:03:06Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99182",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:23'></a>
I think the format of the patch might be correct now!  (I didn't know I should export it.)



---

archive/issue_comments_099183.json:
```json
{
    "body": "<a id='comment:24'></a>\nThe docstrings for is_symmetrizable and is_skewsymmetrizable said that they took a boolean argument \"skew\" which is not true.  I have corrected this in my patch.",
    "created_at": "2012-02-14T16:43:31Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99183",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:24'></a>
The docstrings for is_symmetrizable and is_skewsymmetrizable said that they took a boolean argument "skew" which is not true.  I have corrected this in my patch.



---

archive/issue_comments_099184.json:
```json
{
    "body": "<a id='comment:25'></a>\nHi Hugh,\n\ndo you mind if I fold your 3 lines into the first patch? Do you see other issues, or is the patch ready to go soon?\n\nThanks, Christian",
    "created_at": "2012-02-14T16:51:57Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99184",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:25'></a>
Hi Hugh,

do you mind if I fold your 3 lines into the first patch? Do you see other issues, or is the patch ready to go soon?

Thanks, Christian



---

archive/issue_comments_099185.json:
```json
{
    "body": "<a id='comment:26'></a>\nHi Christian--\n\nI don't expect to find anything else, but on the other hand, I am still looking.  I will try to be fully done in a few days.  So it's up to you if you'd like to fold the patch in now.    \n\nI did just notice something else, though.  In the examples for \"is_skew_symmetrizable\", one of them actually calls is_symmetrizable, not is_skew_symmetrizable\".  This seems harmless but odd.  \n\ncheers,\n\nHugh",
    "created_at": "2012-02-14T17:07:54Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99185",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:26'></a>
Hi Christian--

I don't expect to find anything else, but on the other hand, I am still looking.  I will try to be fully done in a few days.  So it's up to you if you'd like to fold the patch in now.    

I did just notice something else, though.  In the examples for "is_skew_symmetrizable", one of them actually calls is_symmetrizable, not is_skew_symmetrizable".  This seems harmless but odd.  

cheers,

Hugh



---

archive/issue_comments_099186.json:
```json
{
    "body": "<a id='comment:27'></a>\nReplying to [hthomas](#comment%3A26):\n\nJust put whatever you find in your patch, and we then fold them when\nyou are done...\n\nBest, Christian",
    "created_at": "2012-02-14T17:13:53Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99186",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:27'></a>
Replying to [hthomas](#comment%3A26):

Just put whatever you find in your patch, and we then fold them when
you are done...

Best, Christian



---

archive/issue_comments_099187.json:
```json
{
    "body": "<a id='comment:28'></a>\nHi Christian--\n\nI made a couple of very minor changes to documentation.  \n\nThis is a positive review provided the patchbot is okay with it.  \n\nI have confused matters by uploading a second patch.  I meant it to replace my other patch.  I don't see how to remove attached patches, or I would fix this.  (Nor could I easily find an explanation in the documentation.)  How do you do it?\n\ncheers,\n\nHugh",
    "created_at": "2012-02-14T17:57:57Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99187",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:28'></a>
Hi Christian--

I made a couple of very minor changes to documentation.  

This is a positive review provided the patchbot is okay with it.  

I have confused matters by uploading a second patch.  I meant it to replace my other patch.  I don't see how to remove attached patches, or I would fix this.  (Nor could I easily find an explanation in the documentation.)  How do you do it?

cheers,

Hugh



---

archive/issue_comments_099188.json:
```json
{
    "body": "<a id='comment:29'></a>\nReplying to [hthomas](#comment%3A28):\n> I have confused matters by uploading a second patch.  I meant it to replace my other patch.  I don't see how to remove attached patches, or I would fix this.  (Nor could I easily find an explanation in the documentation.)  How do you do it?\n\nHi Hugh, it is not possible to delete files except for admins (as this is not reversible). I can do it, so let me know which files to delete.\n\nBest, Christian",
    "created_at": "2012-02-14T21:41:41Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99188",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:29'></a>
Replying to [hthomas](#comment%3A28):
> I have confused matters by uploading a second patch.  I meant it to replace my other patch.  I don't see how to remove attached patches, or I would fix this.  (Nor could I easily find an explanation in the documentation.)  How do you do it?

Hi Hugh, it is not possible to delete files except for admins (as this is not reversible). I can do it, so let me know which files to delete.

Best, Christian



---

archive/issue_comments_099189.json:
```json
{
    "body": "<a id='comment:30'></a>\nHi Christian--\n\nThanks for the clarification.  \n\nThe patch I added most recently (-ht.2.patch) should be folded in with your patch (if you're happy with the changes, all minor).  The older patch of mine (-ht.patch) can be thrown out.  (The changes in it are included also in -ht.2.patch.)\n\nThanks!\n\ncheers,\n\nHugh",
    "created_at": "2012-02-15T01:56:07Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99189",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:30'></a>
Hi Christian--

Thanks for the clarification.  

The patch I added most recently (-ht.2.patch) should be folded in with your patch (if you're happy with the changes, all minor).  The older patch of mine (-ht.patch) can be thrown out.  (The changes in it are included also in -ht.2.patch.)

Thanks!

cheers,

Hugh



---

archive/issue_comments_099190.json:
```json
{
    "body": "contains matrix methods is_symmetrizable, is_skew_symmetrizable, _check_symmetrizability, _travel_column",
    "created_at": "2012-02-15T07:59:19Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99190",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

contains matrix methods is_symmetrizable, is_skew_symmetrizable, _check_symmetrizability, _travel_column



---

archive/attachments_014049.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_10347_is_skew_symmetrizable-cs.patch",
    "asset_url": "tarball://root/attachments/ticket10347/trac_10347_is_skew_symmetrizable-cs.patch",
    "created_at": "2012-02-15T08:02:38Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket10347/trac_10347_is_skew_symmetrizable-cs.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```



---

archive/issue_comments_099191.json:
```json
{
    "body": "<a id='comment:31'></a>\n**Attachment:** [trac_10347_is_skew_symmetrizable-cs.patch](https://github.com/sagemath/sage/files/ticket10347/trac_10347_is_skew_symmetrizable-cs.patch)\n\nReplying to [hthomas](#comment%3A30):\n\nThanks Hugh for the work -- I folded both patches and add you as an author (which doesn't exclude you from being the reviewer). We might make it into 5.0 if you set the positive review.\n\nBest, Christian",
    "created_at": "2012-02-15T08:02:38Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99191",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

<a id='comment:31'></a>
**Attachment:** [trac_10347_is_skew_symmetrizable-cs.patch](https://github.com/sagemath/sage/files/ticket10347/trac_10347_is_skew_symmetrizable-cs.patch)

Replying to [hthomas](#comment%3A30):

Thanks Hugh for the work -- I folded both patches and add you as an author (which doesn't exclude you from being the reviewer). We might make it into 5.0 if you set the positive review.

Best, Christian



---

archive/issue_comments_099192.json:
```json
{
    "body": "**Changing author** from \"Christian Stump\" to \"Christian Stump, Hugh Thomas\".",
    "created_at": "2012-02-15T08:02:38Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99192",
    "user": "https://trac.sagemath.org/admin/accounts/users/stumpc5"
}
```

**Changing author** from "Christian Stump" to "Christian Stump, Hugh Thomas".



---

archive/issue_events_079177.json:
```json
{
    "actor": "https://github.com/hughrthomas",
    "created_at": "2012-02-15T12:21:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79177"
}
```



---

archive/issue_events_079178.json:
```json
{
    "actor": "https://github.com/hughrthomas",
    "created_at": "2012-02-15T12:21:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79178"
}
```



---

archive/issue_comments_099193.json:
```json
{
    "body": "**Changing author** from \"Christian Stump, Hugh Thomas\" to \"Christian Stump\".",
    "created_at": "2012-02-18T13:53:26Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99193",
    "user": "https://github.com/hughrthomas"
}
```

**Changing author** from "Christian Stump, Hugh Thomas" to "Christian Stump".



---

archive/issue_comments_099194.json:
```json
{
    "body": "<a id='comment:33'></a>\nHi Christian--\n\nI think I'm happier not claiming to have been an author -- I think everything I did is appropriately described by saying that I was the reviewer.  \n\ncheers,\n\nHugh",
    "created_at": "2012-02-18T13:53:26Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99194",
    "user": "https://github.com/hughrthomas"
}
```

<a id='comment:33'></a>
Hi Christian--

I think I'm happier not claiming to have been an author -- I think everything I did is appropriately described by saying that I was the reviewer.  

cheers,

Hugh



---

archive/issue_comments_099195.json:
```json
{
    "body": "**Merged:** sage-5.0.beta5",
    "created_at": "2012-02-22T10:44:25Z",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10347#issuecomment-99195",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.0.beta5



---

archive/issue_events_079179.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-22T10:44:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79179"
}
```



---

archive/issue_events_079180.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-22T10:44:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10347",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10347#event-79180"
}
```
