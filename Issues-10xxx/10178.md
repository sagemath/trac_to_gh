# Issue 10178: Printing html documentation cuts off a couple of inches on the right when the sidebar is not collapsed before printing

archive/issues_010177.json:
```json
{
    "assignees": [],
    "body": "If I try to print the html documentation,for example, here:\n\nhttp://sagemath.org/doc/tutorial/tour_rings.html\n\nThe (blank) sidebar takes up a couple of inches on the left, and the rightmost couple of inches is cut off.\n\nThis was reported by Dan Willis.\n\nAssignee: **mvngu**\n\nCC:  @jhpalmieri @qed777\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10178_\n\n",
    "created_at": "2010-10-27T13:19:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Printing html documentation cuts off a couple of inches on the right when the sidebar is not collapsed before printing",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/10178",
    "user": "https://github.com/jasongrout"
}
```
If I try to print the html documentation,for example, here:

http://sagemath.org/doc/tutorial/tour_rings.html

The (blank) sidebar takes up a couple of inches on the left, and the rightmost couple of inches is cut off.

This was reported by Dan Willis.

Assignee: **mvngu**

CC:  @jhpalmieri @qed777

_Issue created by migration from https://trac.sagemath.org/ticket/10178_





---

archive/issue_events_121628.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-10-27T13:19:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121628"
}
```



---

archive/issue_events_121629.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-10-27T13:19:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121629"
}
```



---

archive/issue_events_121630.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-10-27T13:19:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121630"
}
```



---

archive/issue_events_121631.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-10-27T13:19:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121631"
}
```



---

archive/issue_comments_096126.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI can confirm this by doing a print preview using Firefox. A work around is to collapse the side bar. You can do this by clicking on the slim bar that divides the side bar to the left and the documentation to the right. Then do a print preview again using Firefox; the reported problem should then be fixed. As this ticket now stands, I'm inclined to think it can be closed as invalid.",
    "created_at": "2010-10-27T14:22:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10178#issuecomment-96126",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:1'>Comment 1:</a>
I can confirm this by doing a print preview using Firefox. A work around is to collapse the side bar. You can do this by clicking on the slim bar that divides the side bar to the left and the documentation to the right. Then do a print preview again using Firefox; the reported problem should then be fixed. As this ticket now stands, I'm inclined to think it can be closed as invalid.



---

archive/issue_comments_096127.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nReplying to [mvngu](#comment%3A1):\n> I can confirm this by doing a print preview using Firefox. A work around is to collapse the side bar. You can do this by clicking on the slim bar that divides the side bar to the left and the documentation to the right. Then do a print preview again using Firefox; the reported problem should then be fixed. As this ticket now stands, I'm inclined to think it can be closed as invalid.\n\nI don't think it's invalid.  A new user will not know to collapse the sidebar to get sensible printing.  I bet most experienced users wouldn't know that either.\n\nI'll change the title to specify printing without collapsing the sidebar.  In reality, there should be a print stylesheet that hides that sidebar automatically.",
    "created_at": "2010-10-27T15:02:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10178#issuecomment-96127",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'>Comment 2:</a>
Replying to [mvngu](#comment%3A1):
> I can confirm this by doing a print preview using Firefox. A work around is to collapse the side bar. You can do this by clicking on the slim bar that divides the side bar to the left and the documentation to the right. Then do a print preview again using Firefox; the reported problem should then be fixed. As this ticket now stands, I'm inclined to think it can be closed as invalid.

I don't think it's invalid.  A new user will not know to collapse the sidebar to get sensible printing.  I bet most experienced users wouldn't know that either.

I'll change the title to specify printing without collapsing the sidebar.  In reality, there should be a print stylesheet that hides that sidebar automatically.



---

archive/issue_events_121632.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-10-27T15:02:51Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "title_is": "Printing html documentation cuts off a couple of inches on the right when the sidebar is not collapsed before printing",
    "title_was": "Printing html documentation cuts off a couple of inches on the right",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121632"
}
```



---

archive/issue_events_121633.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121633"
}
```



---

archive/issue_events_121634.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121634"
}
```



---

archive/issue_events_121635.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121635"
}
```



---

archive/issue_events_121636.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121636"
}
```



---

archive/issue_events_121637.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121637"
}
```



---

archive/issue_events_121638.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121638"
}
```



---

archive/issue_events_121639.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121639"
}
```



---

archive/issue_events_121640.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121640"
}
```



---

archive/issue_events_121641.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10178",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10178#event-121641"
}
```
