# Issue 10357: merge_vertices does not respect loops

archive/issues_010356.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/nathanncohen",
        "https://github.com/rlmill"
    ],
    "body": "This is a duplicate of #9807.\n\nIf a graph allows loops, then merge_vertices maybe should have an option to put a loop on the merged vertex if there was either an edge between the vertices, or maybe if the merged vertices formed a complete graph, so the following would make a loop in the graph:\n\n```\ng=graphs.PathGraph(2)\ng.allow_loops(True)\ng.merge_vertices([0,1])\ng.edges()\n```\n\n\nIn my case, I'm merging two vertices, so either case would be the same thing.\n\nReviewer: **Travis Scrimshaw**\n\nComponent: **graph theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10357_\n\n",
    "closed_at": "2019-12-30T20:43:46Z",
    "created_at": "2010-11-30T23:49:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "merge_vertices does not respect loops",
    "type": "issue",
    "updated_at": "2019-12-30T20:43:46Z",
    "url": "https://github.com/sagemath/sage/issues/10357",
    "user": "https://github.com/jasongrout"
}
```
This is a duplicate of #9807.

If a graph allows loops, then merge_vertices maybe should have an option to put a loop on the merged vertex if there was either an edge between the vertices, or maybe if the merged vertices formed a complete graph, so the following would make a loop in the graph:

```
g=graphs.PathGraph(2)
g.allow_loops(True)
g.merge_vertices([0,1])
g.edges()
```


In my case, I'm merging two vertices, so either case would be the same thing.

Reviewer: **Travis Scrimshaw**

Component: **graph theory**

_Issue created by migration from https://trac.sagemath.org/ticket/10357_





---

archive/issue_events_136370.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-11-30T23:49:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136370"
}
```



---

archive/issue_events_136371.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-11-30T23:49:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136371"
}
```



---

archive/issue_events_136372.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-11-30T23:49:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136372"
}
```



---

archive/issue_events_136373.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-11-30T23:49:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136373"
}
```



---

archive/issue_events_136374.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-11-30T23:49:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136374"
}
```



---

archive/issue_events_136375.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-11-30T23:49:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136375"
}
```



---

archive/issue_events_136376.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-11-30T23:49:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136376"
}
```



---

archive/issue_comments_097440.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,11 @@\n-If a graph allows loops, then merge_vertices maybe should have an option to put a loop on the merged vertex if there was either an edge between the vertices, or maybe if the merged vertices formed a complete graph.  \n+If a graph allows loops, then merge_vertices maybe should have an option to put a loop on the merged vertex if there was either an edge between the vertices, or maybe if the merged vertices formed a complete graph, so the following would make a loop in the graph:\n+\n+```\n+g=graphs.PathGraph(2)\n+g.allow_loops(True)\n+g.merge_vertices([0,1])\n+g.edges()\n+```\n+\n \n In my case, I'm merging two vertices, so either case would be the same thing.\n``````\n",
    "created_at": "2010-11-30T23:51:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10357#issuecomment-97440",
    "user": "https://github.com/jasongrout"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,11 @@
-If a graph allows loops, then merge_vertices maybe should have an option to put a loop on the merged vertex if there was either an edge between the vertices, or maybe if the merged vertices formed a complete graph.  
+If a graph allows loops, then merge_vertices maybe should have an option to put a loop on the merged vertex if there was either an edge between the vertices, or maybe if the merged vertices formed a complete graph, so the following would make a loop in the graph:
+
+```
+g=graphs.PathGraph(2)
+g.allow_loops(True)
+g.merge_vertices([0,1])
+g.edges()
+```
+
 
 In my case, I'm merging two vertices, so either case would be the same thing.
``````




---

archive/issue_comments_097441.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nIs this a duplicate of #9807?",
    "created_at": "2010-12-04T13:00:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10357#issuecomment-97441",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

Is this a duplicate of #9807?



---

archive/issue_events_136377.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-12-04T13:00:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136377"
}
```



---

archive/issue_comments_097442.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nYes. Also, see #7304.",
    "created_at": "2012-01-31T23:18:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10357#issuecomment-97442",
    "user": "https://github.com/sagetrac-brunellus"
}
```

<div id="comment:3" align="right">comment:3</div>

Yes. Also, see #7304.



---

archive/issue_events_136378.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136378"
}
```



---

archive/issue_events_136379.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136379"
}
```



---

archive/issue_events_136380.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136380"
}
```



---

archive/issue_events_136381.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136381"
}
```



---

archive/issue_events_136382.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136382"
}
```



---

archive/issue_events_136383.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136383"
}
```



---

archive/issue_events_136384.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136384"
}
```



---

archive/issue_events_136385.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136385"
}
```



---

archive/issue_comments_097443.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n+This is a duplicate of #9807.\n+\n If a graph allows loops, then merge_vertices maybe should have an option to put a loop on the merged vertex if there was either an edge between the vertices, or maybe if the merged vertices formed a complete graph, so the following would make a loop in the graph:\n \n ```\n``````\n",
    "created_at": "2019-12-19T13:09:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10357#issuecomment-97443",
    "user": "https://github.com/kliem"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
+This is a duplicate of #9807.
+
 If a graph allows loops, then merge_vertices maybe should have an option to put a loop on the merged vertex if there was either an edge between the vertices, or maybe if the merged vertices formed a complete graph, so the following would make a loop in the graph:
 
 ```
``````




---

archive/issue_events_136386.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-19T13:09:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136386"
}
```



---

archive/issue_events_136387.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-19T13:09:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136387"
}
```



---

archive/issue_comments_097444.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nThis is a duplicate of #9807.",
    "created_at": "2019-12-19T13:09:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10357#issuecomment-97444",
    "user": "https://github.com/kliem"
}
```

<div id="comment:8" align="right">comment:8</div>

This is a duplicate of #9807.



---

archive/issue_events_136388.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-19T13:09:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136388"
}
```



---

archive/issue_events_136389.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-19T13:09:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136389"
}
```



---

archive/issue_events_136390.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-19T13:09:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136390"
}
```



---

archive/issue_comments_097445.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nAgreed.",
    "created_at": "2019-12-27T16:59:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10357#issuecomment-97445",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">comment:9</div>

Agreed.



---

archive/issue_events_136391.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-12-27T16:59:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136391"
}
```



---

archive/issue_events_136392.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-12-27T16:59:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136392"
}
```



---

archive/issue_comments_097446.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-12-27T16:59:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10357#issuecomment-97446",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_136393.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-30T20:43:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136393"
}
```



---

archive/issue_events_136394.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-30T20:43:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136394"
}
```



---

archive/issue_events_136395.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-30T20:43:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10357",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10357#event-136395"
}
```
