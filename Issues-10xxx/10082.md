...
```json
{
    "body": "Has anyone else seen this problem, possibly on other machines?",
...
```json
{
    "body": "I've now also seen the problem in the description on menas.  For me, it seems the `math` module is installed, since I can run `./sage -python` and `import` it, at least, without problems.\n\nIs this a parallel build problem?  Reinstalling the package serially works.\n\nCould we do better than sleep for an arbitrary three seconds?",
