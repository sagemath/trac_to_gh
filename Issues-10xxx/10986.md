# Issue 10986: building ecl fails in case the installed etags is actually exuberant-ctags

archive/issues_010906.json:
```json
{
    "body": "In some debian installs (and derivatives such as ubuntu) the etags program is actually to ctags from exuberant-ctags. This is incompatible with what ecl expect. the symptoms are as follow:\n\n```\necho > TAGS\nif test \"xetags\" != \"x\"; then \\\n        srcfiles=`find \n/scratch/scratch/schilly/sage/sage-4.6.2/spkg/build/ecl-11.1.1/src/src/c \n/scratch/scratch/schilly/sage/sage-4.6.2/spkg/build/ecl-11.1.1/src/src/h \n-name '*.[chd]'` && \\\n        etags --language=c    -o TAGS $srcfiles && \\\n        etags --language=none -o TAGS --append \\\n              --regex='/@\\([-:*a-zA-z]+\\)/\\1/' \\\n              --regex='/@(defun \\([-:*a-zA-z]+\\)/\\1/' \\\n              $srcfiles; \\\n        fi\netags: relocation error: /usr/lib/libldap_r-2.4.so.2: symbol \n``` \nor\n\n```\necho > TAGS\nif test \"xetags\" != \"x\"; then \\\nsrcfiles=`find  \n/storage/strogdon/gentoo/var/tmp/portage/dev-lisp/ecls-11.1.1/work/ecl-11.1.1/src/c  \n/storage/strogdon/gentoo/var/tmp/portage/dev-lisp/ecls-11.1.1/work/ecl-11.1.1/src/h  \n-name '*.[chd]'` && \\\netags --language=c    -o TAGS $srcfiles && \\\netags --language=none -o TAGS --append \\\n       --regex='/@\\([-:*a-zA-z]+\\)/\\1/' \\\n       --regex='/@(defun \\([-:*a-zA-z]+\\)/\\1/' \\\n       $srcfiles; \\\nfi\netags: Warning: \"--language\" option is obsolete; use \"--language-force\"  \ninstead\netags: \"TAGS\" doesn't look like a tag file; I refuse to overwrite it.\n```\nThere is no way to disable the building of TAGS in the configuration and detecting that you have an etags program from emacs is complicated. See the following bug reports:\n* [http://bugs.gentoo.org/show_bug.cgi?id=357319](http://bugs.gentoo.org/show_bug.cgi?id=357319)\n* [http://sourceforge.net/tracker/?func=detail&aid=3202253&group_id=30035&atid=398053](http://sourceforge.net/tracker/?func=detail&aid=3202253&group_id=30035&atid=398053) (upstream)\n\nNew spkg: [http://spkg-upload.googlecode.com/files/ecl-11.1.1.p0.spkg](http://spkg-upload.googlecode.com/files/ecl-11.1.1.p0.spkg)\n\n\nAssignee: tbd\n\nCC:  @haraldschilly\n\nResolution: fixed\n\nAuthor: Fran\u00e7ois Bissey\n\nReviewer: Harald Schilly, Simon King\n\nUpstream: Reported upstream. Little or no feedback.\n\nMerged: sage-4.7.alpha5\n\nIssue created by migration from https://trac.sagemath.org/ticket/10986\n\n",
    "closed_at": "2011-04-13T15:17:57Z",
    "created_at": "2011-03-22T21:01:13Z",
    "labels": [
        "component: packages: standard",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.7",
    "title": "building ecl fails in case the installed etags is actually exuberant-ctags",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10986",
    "user": "https://github.com/kiwifb"
}
```
In some debian installs (and derivatives such as ubuntu) the etags program is actually to ctags from exuberant-ctags. This is incompatible with what ecl expect. the symptoms are as follow:

```
echo > TAGS
if test "xetags" != "x"; then \
        srcfiles=`find 
/scratch/scratch/schilly/sage/sage-4.6.2/spkg/build/ecl-11.1.1/src/src/c 
/scratch/scratch/schilly/sage/sage-4.6.2/spkg/build/ecl-11.1.1/src/src/h 
-name '*.[chd]'` && \
        etags --language=c    -o TAGS $srcfiles && \
        etags --language=none -o TAGS --append \
              --regex='/@\([-:*a-zA-z]+\)/\1/' \
              --regex='/@(defun \([-:*a-zA-z]+\)/\1/' \
              $srcfiles; \
        fi
etags: relocation error: /usr/lib/libldap_r-2.4.so.2: symbol 
``` 
or

```
echo > TAGS
if test "xetags" != "x"; then \
srcfiles=`find  
/storage/strogdon/gentoo/var/tmp/portage/dev-lisp/ecls-11.1.1/work/ecl-11.1.1/src/c  
/storage/strogdon/gentoo/var/tmp/portage/dev-lisp/ecls-11.1.1/work/ecl-11.1.1/src/h  
-name '*.[chd]'` && \
etags --language=c    -o TAGS $srcfiles && \
etags --language=none -o TAGS --append \
       --regex='/@\([-:*a-zA-z]+\)/\1/' \
       --regex='/@(defun \([-:*a-zA-z]+\)/\1/' \
       $srcfiles; \
fi
etags: Warning: "--language" option is obsolete; use "--language-force"  
instead
etags: "TAGS" doesn't look like a tag file; I refuse to overwrite it.
```
There is no way to disable the building of TAGS in the configuration and detecting that you have an etags program from emacs is complicated. See the following bug reports:
* [http://bugs.gentoo.org/show_bug.cgi?id=357319](http://bugs.gentoo.org/show_bug.cgi?id=357319)
* [http://sourceforge.net/tracker/?func=detail&aid=3202253&group_id=30035&atid=398053](http://sourceforge.net/tracker/?func=detail&aid=3202253&group_id=30035&atid=398053) (upstream)

New spkg: [http://spkg-upload.googlecode.com/files/ecl-11.1.1.p0.spkg](http://spkg-upload.googlecode.com/files/ecl-11.1.1.p0.spkg)


Assignee: tbd

CC:  @haraldschilly

Resolution: fixed

Author: François Bissey

Reviewer: Harald Schilly, Simon King

Upstream: Reported upstream. Little or no feedback.

Merged: sage-4.7.alpha5

Issue created by migration from https://trac.sagemath.org/ticket/10986





---

archive/issue_comments_141736.json:
```json
{
    "body": "<a id='comment:1'></a>Here is the new spkg. Basically I touch build/TAGS before make is called so that TAGS is never built. Bypassing any problem with a broken etags or ctags passing for etags.\nI will attach a patch for the reviewer with all the changes to the spkg.",
    "created_at": "2011-03-22T21:51:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141736",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:1'></a>Here is the new spkg. Basically I touch build/TAGS before make is called so that TAGS is never built. Bypassing any problem with a broken etags or ctags passing for etags.
I will attach a patch for the reviewer with all the changes to the spkg.



---

archive/issue_comments_141737.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -38,3 +38,5 @@\n * [http://bugs.gentoo.org/show_bug.cgi?id=357319](http://bugs.gentoo.org/show_bug.cgi?id=357319)\n * [http://sourceforge.net/tracker/?func=detail&aid=3202253&group_id=30035&atid=398053](http://sourceforge.net/tracker/?func=detail&aid=3202253&group_id=30035&atid=398053) (upstream)\n \n+New spkg: [http://spkg-upload.googlecode.com/files/ecl-11.1.1.p0.spkg](http://spkg-upload.googlecode.com/files/ecl-11.1.1.p0.spkg)\n+\n``````\n",
    "created_at": "2011-03-22T21:51:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141737",
    "user": "https://github.com/kiwifb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -38,3 +38,5 @@
 * [http://bugs.gentoo.org/show_bug.cgi?id=357319](http://bugs.gentoo.org/show_bug.cgi?id=357319)
 * [http://sourceforge.net/tracker/?func=detail&aid=3202253&group_id=30035&atid=398053](http://sourceforge.net/tracker/?func=detail&aid=3202253&group_id=30035&atid=398053) (upstream)
 
+New spkg: [http://spkg-upload.googlecode.com/files/ecl-11.1.1.p0.spkg](http://spkg-upload.googlecode.com/files/ecl-11.1.1.p0.spkg)
+
``````




---

archive/issue_comments_141738.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-03-22T21:51:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141738",
    "user": "https://github.com/kiwifb"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_141739.json:
```json
{
    "body": "summary of change patch, for the reviewer only",
    "created_at": "2011-03-22T21:52:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141739",
    "user": "https://github.com/kiwifb"
}
```

summary of change patch, for the reviewer only



---

archive/issue_comments_141740.json:
```json
{
    "body": "<a id='comment:2'></a>Attachment [trac_10986-spkg-upgrade.patch](tarball://root/attachments/some-uuid/ticket10986/trac_10986-spkg-upgrade.patch) by @kiwifb created at 2011-03-22 21:53:17\n\nThis is now ready for review.",
    "created_at": "2011-03-22T21:53:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141740",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:2'></a>Attachment [trac_10986-spkg-upgrade.patch](tarball://root/attachments/some-uuid/ticket10986/trac_10986-spkg-upgrade.patch) by @kiwifb created at 2011-03-22 21:53:17

This is now ready for review.



---

archive/issue_comments_141741.json:
```json
{
    "body": "<a id='comment:3'></a>nice, it just built successfully on the same machine where i reported the problem mentioned above!",
    "created_at": "2011-03-22T21:59:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141741",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:3'></a>nice, it just built successfully on the same machine where i reported the problem mentioned above!



---

archive/issue_comments_141742.json:
```json
{
    "body": "Changing author from \"\" to \"Fran\u00e7ois Bissey\"",
    "created_at": "2011-04-13T08:03:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141742",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "" to "François Bissey"



---

archive/issue_comments_141743.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-04-13T08:03:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141743",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_141744.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Harald Schilly\"",
    "created_at": "2011-04-13T08:03:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141744",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Harald Schilly"



---

archive/issue_comments_141745.json:
```json
{
    "body": "Changing reviewer from \"Harald Schilly\" to \"Harald Schilly, Simon King\"",
    "created_at": "2011-04-13T08:04:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141745",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "Harald Schilly" to "Harald Schilly, Simon King"



---

archive/issue_comments_141746.json:
```json
{
    "body": "<a id='comment:6'></a>Thank you for changing it into a positive review! I was just about to do that myself.",
    "created_at": "2011-04-13T08:23:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141746",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:6'></a>Thank you for changing it into a positive review! I was just about to do that myself.



---

archive/issue_events_028544.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-13T15:17:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10986#event-28544"
}
```



---

archive/issue_comments_141747.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-04-13T15:17:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141747",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_141748.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.7.alpha5\"",
    "created_at": "2011-04-13T15:17:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10986#issuecomment-141748",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.7.alpha5"
