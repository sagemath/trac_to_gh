...
<a id='comment:1'></a>The backtrace from the ticket description is not very clear (all those question marks). Here is more information:
...
```json
{
    "body": "<a id='comment:2'></a>By the way, I tried with other input values: `seed(a,b)` seems to result in a segfault, independent of the values a and b.",
...
<a id='comment:2'></a>By the way, I tried with other input values: `seed(a,b)` seems to result in a segfault, independent of the values a and b.
...
```json
{
    "body": "<a id='comment:3'></a>Now that's strange. I inserted print statements into the new `__cinit__` and the existing `__init__` and `__dealloc__` methods of sage.misc.randstate.randstate, namely printing `id(self)`. It turns out that, when doing `seed(a,b)`, `__dealloc__` is called on an object without `__cinit__` called previously.\n\nHow can that happen?",
...
<a id='comment:4'></a>More precisely: When I have
...
<a id='comment:7'></a>The patch applies to 5.0.beta14 and does the trick for me (I tried a variety of bogus inputs).
...
```json
{
    "body": "<a id='comment:11'></a>PS: Since `ZZ(...)` forwards its arguments to the init method of Integer, you obtain:\n\n```\nsage: ZZ(1,2,3,4,5,6,7,8,9)\nTraceback (most recent call last):\n...\nTypeError: __init__() takes at most 2 positional arguments (9 given)\n```",
...
<a id='comment:11'></a>PS: Since `ZZ(...)` forwards its arguments to the init method of Integer, you obtain:
...
```json
{
    "body": "<a id='comment:13'></a>Agree. In Cython, you **must** initialize C variables in `__cinit__` to a sane state, and destroy them in `__dealloc__`. There is no guarantee that `__init__` runs all the way or even runs at all.",
...
<a id='comment:13'></a>Agree. In Cython, you **must** initialize C variables in `__cinit__` to a sane state, and destroy them in `__dealloc__`. There is no guarantee that `__init__` runs all the way or even runs at all.
