...
```json
{
    "body": "I think that, in this case, the `__cmp__` of toric varieties should be strengthened so that `X1==X2` if and only if `X1 is X2`. Otherwise you have `X1==X2` but you would be unable to add divisors and cohomology classes. Moreover, these operations would yield a confusing error message since `X1._repr_()==X2._repr_()`.",
