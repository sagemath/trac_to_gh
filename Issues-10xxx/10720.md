...
```json
{
    "body": "<a id='comment:30'></a>Reproducing the example of [This is the Trac macro *comment:7[comment:7](#comment%3A7)* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#comment:7[comment:7](#comment%3A7)-macro)\n\n```\nsage: S.<t> = QQ[[]]\nsage: p = t.exp(30)\nsage: %time p1 = p.nth_root(1000)\nCPU times: user 11.7 ms, sys: 3.27 ms, total: 15 ms\nWall time: 13.8 ms\n```",
...
<a id='comment:30'></a>Reproducing the example of [This is the Trac macro *comment:7[comment:7](#comment%3A7)* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#comment:7[comment:7](#comment%3A7)-macro)
...
<a id='comment:36'></a>Replying to [chapoton](#comment%3A14):
> One related remark:
> 
> Thanks to ticket #14115, it is now easy to implement a general rational power by
> 
