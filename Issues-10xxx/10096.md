# Issue 10096: sha().an() assumes E is minimal.

archive/issues_010095.json:
```json
{
    "body": "I noticed the following problem with sha().an()\n\n```\nsage: E=EllipticCurve([1215*1216,0]) # non-minimal model\nsage: E.sha().an()\n6.00000000000000\nsage: E.minimal_model().sha().an()\n1.00000000000000\n```\n\nIt looks like sha().an() assumes that E is minimal. The extra factor of 6 seems to be coming from the real period.\n\n```\nsage: E.period_lattice().omega()\n0.106360349280819\nsage: E.minimal_model().period_lattice().omega()\n0.638162095684913\n```\n\nIt's probably unfair to call this a bug, but it could definitely lead people astray.\n\nAssignee: @JohnCremona\n\nCC:  @williamstein @rlmill @categorie\n\nKeywords: elliptic curves, sha, real period\n\nResolution: fixed\n\nAuthor: John Cremona\n\nReviewer: Aly Deines\n\nMerged: sage-4.6.1.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/10096\n\n",
    "closed_at": "2010-11-10T22:27:07Z",
    "created_at": "2010-10-07T20:42:47Z",
    "labels": [
        "component: elliptic curves",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.6.1",
    "title": "sha().an() assumes E is minimal.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10096",
    "user": "https://trac.sagemath.org/admin/accounts/users/weigandt"
}
```
I noticed the following problem with sha().an()

```
sage: E=EllipticCurve([1215*1216,0]) # non-minimal model
sage: E.sha().an()
6.00000000000000
sage: E.minimal_model().sha().an()
1.00000000000000
```

It looks like sha().an() assumes that E is minimal. The extra factor of 6 seems to be coming from the real period.

```
sage: E.period_lattice().omega()
0.106360349280819
sage: E.minimal_model().period_lattice().omega()
0.638162095684913
```

It's probably unfair to call this a bug, but it could definitely lead people astray.

Assignee: @JohnCremona

CC:  @williamstein @rlmill @categorie

Keywords: elliptic curves, sha, real period

Resolution: fixed

Author: John Cremona

Reviewer: Aly Deines

Merged: sage-4.6.1.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/10096





---

archive/issue_comments_122272.json:
```json
{
    "body": "Changing upstream from \"Reported upstream. Developers acknowledge bug.\" to \"N/A\"",
    "created_at": "2010-10-28T09:09:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122272",
    "user": "https://github.com/JohnCremona"
}
```

Changing upstream from "Reported upstream. Developers acknowledge bug." to "N/A"



---

archive/issue_comments_122273.json:
```json
{
    "body": "<a id='comment:1'></a>I found the problem here.  an() uses the minimal model when it can do the job itself, which is when the rank is at most 1, but when the rank is at least 2 it passes the work to an_numerical(), and that did not use the minimal model.\n\nI am preparing a patch which fixes this.  And as I suspect that other related functions also need the minimal model (notably the p-adic one) I am now caching the minimal model and changing lots of occurrences of self.E to self.Emin.\n\nWatch this space...",
    "created_at": "2010-10-28T09:09:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122273",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:1'></a>I found the problem here.  an() uses the minimal model when it can do the job itself, which is when the rank is at most 1, but when the rank is at least 2 it passes the work to an_numerical(), and that did not use the minimal model.

I am preparing a patch which fixes this.  And as I suspect that other related functions also need the minimal model (notably the p-adic one) I am now caching the minimal model and changing lots of occurrences of self.E to self.Emin.

Watch this space...



---

archive/issue_comments_122274.json:
```json
{
    "body": "Applies to 4.6.rc0",
    "created_at": "2010-10-28T09:33:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122274",
    "user": "https://github.com/JohnCremona"
}
```

Applies to 4.6.rc0



---

archive/issue_comments_122275.json:
```json
{
    "body": "Changing author from \"\" to \"John Cremona\"",
    "created_at": "2010-10-28T09:37:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122275",
    "user": "https://github.com/JohnCremona"
}
```

Changing author from "" to "John Cremona"



---

archive/issue_comments_122276.json:
```json
{
    "body": "<a id='comment:2'></a>Attachment [trac_10096-sha.patch](tarball://root/attachments/some-uuid/ticket10096/trac_10096-sha.patch) by @JohnCremona created at 2010-10-28 09:37:09\n\nThe patch fixes this.  the minimal model is computed on construction and used in place of the original (when the latter is not minimal).  New doctests show that the reported problem has gone away.\n\nIt would be helpful if someone could confirm that this is the correct thing to do also for the an_padic() function.  The doctests still pass, but I sust[ect that they are all with minimal curves anyway.  I was assuming (a) that the value of an_padic() was isomorphism-invariant, and (2) that using a minimal model would be at least as fast, possibly faster, than using a non-minimal model, even if both give the correct answer!",
    "created_at": "2010-10-28T09:37:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122276",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:2'></a>Attachment [trac_10096-sha.patch](tarball://root/attachments/some-uuid/ticket10096/trac_10096-sha.patch) by @JohnCremona created at 2010-10-28 09:37:09

The patch fixes this.  the minimal model is computed on construction and used in place of the original (when the latter is not minimal).  New doctests show that the reported problem has gone away.

It would be helpful if someone could confirm that this is the correct thing to do also for the an_padic() function.  The doctests still pass, but I sust[ect that they are all with minimal curves anyway.  I was assuming (a) that the value of an_padic() was isomorphism-invariant, and (2) that using a minimal model would be at least as fast, possibly faster, than using a non-minimal model, even if both give the correct answer!



---

archive/issue_comments_122277.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-10-28T09:37:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122277",
    "user": "https://github.com/JohnCremona"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_122278.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2010-11-03T06:16:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122278",
    "user": "https://github.com/adeines"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_122279.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Aly Deines\"",
    "created_at": "2010-11-03T10:48:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122279",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Aly Deines"



---

archive/issue_comments_122280.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.6.1.alpha1\"",
    "created_at": "2010-11-10T22:20:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122280",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.6.1.alpha1"



---

archive/issue_events_025749.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-10T22:27:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10096#event-25749"
}
```



---

archive/issue_comments_122281.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-11-10T22:27:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122281",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_122282.json:
```json
{
    "body": "Changing reviewer from \"Aly Deines\" to \"Alyson Deines\"",
    "created_at": "2011-01-13T03:33:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122282",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "Aly Deines" to "Alyson Deines"



---

archive/issue_comments_122283.json:
```json
{
    "body": "Changing reviewer from \"Alyson Deines\" to \"Aly Deines\"",
    "created_at": "2011-01-13T06:05:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10096#issuecomment-122283",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "Alyson Deines" to "Aly Deines"
