...
```json
{
    "body": "Mike, I think you misunderstood step 2).  I meant \"try to compile the PARI/GP package independently of Sage by doing the following steps:\"\n\n```\n$ tar xjf pari-2.4.3.alpha.p0.spkg\n$ cd pari-2.4.3.alpha.p0/src\n$ ./Configure\n$ make gp\n```",
...
```json
{
    "body": "Can you compile the PARI spkg with -DINLINE=inline added to CFLAGS and see if that works?",
...
```json
{
    "body": "Sorry, ignore the last post.  Since the problem is not with PARI itself, but with the sagelib modules, instead do the following: after the PARI spkg is installed, find the file `SAGE_ROOT/local/include/pari/paricfg.h`.\n\nTry adding `#define INLINE inline` there and try again (in any case, it would be useful if you would post the contents of that file).",
...
```json
{
    "body": "It was in the \"wrong\" order for csage, but after changing that, I still get the same error in building gen.dll.",
...
```json
{
    "body": "Do you have any tools similar to readelf to check that these symbols are in the pari dll that you have built?",
