# Issue 10196: ETS spkg updated to version 3.5.0

archive/issues_010195.json:
```json
{
    "body": "As I have told few days ago to Jaap I just wanted to upload my spkg and share it with Sage's community in order to everyone who needs it could install it, so this is to open the ticket, I am not any familiar with the procedure, hope it goes in this way, I am attaching it, the md5sum is the next:\ngeorge`@`manp-desktop:~/Documentos$ md5sum -b ets-3.5.0-20101024.p0.spkg \na742400ae39b84db79925d342ba74e33 *ets-3.5.0-20101024.p0.spkg\n\nAssignee: drkirkby\n\nCC:  @embray @jdemeyer @kiwifb\n\nKeywords: ETS-3.5.0 experimental spkg\n\nReviewer: Fran\u00e7ois Bissey\n\nAuthor: Jorge Sanchez\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/10196\n\n",
    "closed_at": "2019-06-15T12:40:53Z",
    "created_at": "2010-10-31T19:53:46Z",
    "labels": [
        "component: spkg-check",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "ETS spkg updated to version 3.5.0",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10196",
    "user": "Jorge Sanchez"
}
```
As I have told few days ago to Jaap I just wanted to upload my spkg and share it with Sage's community in order to everyone who needs it could install it, so this is to open the ticket, I am not any familiar with the procedure, hope it goes in this way, I am attaching it, the md5sum is the next:
george`@`manp-desktop:~/Documentos$ md5sum -b ets-3.5.0-20101024.p0.spkg 
a742400ae39b84db79925d342ba74e33 *ets-3.5.0-20101024.p0.spkg

Assignee: drkirkby

CC:  @embray @jdemeyer @kiwifb

Keywords: ETS-3.5.0 experimental spkg

Reviewer: François Bissey

Author: Jorge Sanchez

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/10196





---

archive/issue_comments_124739.json:
```json
{
    "body": "<a id='comment:1'></a>I tried to attach the file but it is larger than 2Mb it is around 50Mb.\n\nSorry, what can I do now?\n\nBest regards,\nJorge",
    "created_at": "2010-10-31T20:04:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10196#issuecomment-124739",
    "user": "Jorge Sanchez"
}
```

<a id='comment:1'></a>I tried to attach the file but it is larger than 2Mb it is around 50Mb.

Sorry, what can I do now?

Best regards,
Jorge



---

archive/issue_comments_124740.json:
```json
{
    "body": "<a id='comment:2'></a>obsolete, no activity in 9 years, can be closed",
    "created_at": "2019-06-15T07:35:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10196#issuecomment-124740",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'></a>obsolete, no activity in 9 years, can be closed



---

archive/issue_events_026126.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-15T07:35:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10196#event-26126"
}
```



---

archive/issue_comments_124741.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-06-15T07:35:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10196#issuecomment-124741",
    "user": "https://github.com/fchapoton"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_124742.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Fran\u00e7ois Bissey\"",
    "created_at": "2019-06-15T07:44:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10196#issuecomment-124742",
    "user": "https://github.com/kiwifb"
}
```

Changing reviewer from "" to "François Bissey"



---

archive/issue_comments_124743.json:
```json
{
    "body": "<a id='comment:3'></a>Certainly, there is not enough information to do anything about it in any case.",
    "created_at": "2019-06-15T07:44:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10196#issuecomment-124743",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:3'></a>Certainly, there is not enough information to do anything about it in any case.



---

archive/issue_comments_124744.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-06-15T07:44:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10196#issuecomment-124744",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_124745.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2019-06-15T12:40:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10196#issuecomment-124745",
    "user": "https://github.com/fchapoton"
}
```

Resolution: invalid



---

archive/issue_comments_124746.json:
```json
{
    "body": "<a id='comment:4'></a>close 3 old invalid tickets",
    "created_at": "2019-06-15T12:40:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10196#issuecomment-124746",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'></a>close 3 old invalid tickets



---

archive/issue_events_026127.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-15T12:40:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10196",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10196#event-26127"
}
```
