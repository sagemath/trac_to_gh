# Issue 10144: Update plot.py to 100% coverage

archive/issues_010143.json:
```json
{
    "assignees": [],
    "body": "sage/plot/plot.py is missing coverage for the important function `_plot` as well as for some GraphicsArray methods.  This is documention, but putting under graphics component since it requires good knowledge of this.\n\nAssignee: **@jasongrout, @williamstein**\n\nReviewer: **Karl-Dieter Crisman**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10144_\n\n",
    "closed_at": "2012-07-17T08:33:56Z",
    "created_at": "2010-10-19T01:48:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update plot.py to 100% coverage",
    "type": "issue",
    "updated_at": "2012-07-17T08:33:56Z",
    "url": "https://github.com/sagemath/sage/issues/10144",
    "user": "https://github.com/kcrisman"
}
```
sage/plot/plot.py is missing coverage for the important function `_plot` as well as for some GraphicsArray methods.  This is documention, but putting under graphics component since it requires good knowledge of this.

Assignee: **@jasongrout, @williamstein**

Reviewer: **Karl-Dieter Crisman**

_Issue created by migration from https://trac.sagemath.org/ticket/10144_





---

archive/issue_events_121090.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2010-10-19T01:48:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121090"
}
```



---

archive/issue_events_121091.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2010-10-19T01:48:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121091"
}
```



---

archive/issue_events_121092.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2010-10-19T01:48:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121092"
}
```



---

archive/issue_events_121093.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2010-10-19T01:48:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121093"
}
```



---

archive/issue_comments_095498.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\n\n```\n$ sage -coverage ~/Downloads/sage-5.1.beta6/devel/sage/sage/plot/plot.py\n----------------------------------------------------------------------\n/Users/.../sage-5.1.beta6/devel/sage/sage/plot/plot.py\nSCORE /Users/.../sage-5.1.beta6/devel/sage/sage/plot/plot.py: 100% (24 of 24)\n----------------------------------------------------------------------\n\n$ sage -coverage ~/Downloads/sage-5.1.beta6/devel/sage/sage/plot/graphics.py\n----------------------------------------------------------------------\n/Users/.../sage-5.1.beta6/devel/sage/sage/plot/graphics.py\nSCORE /Users/.../sage-5.1.beta6/devel/sage/sage/plot/graphics.py: 100% (53 of 53)\n\nPossibly wrong (function name doesn't occur in doctests):\n\t * _set_figsize_(self, ls):\n\n----------------------------------------------------------------------\n```\nThis was fixed by #12495.",
    "created_at": "2012-07-07T03:30:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10144#issuecomment-95498",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">Comment 1</div>


```
$ sage -coverage ~/Downloads/sage-5.1.beta6/devel/sage/sage/plot/plot.py
----------------------------------------------------------------------
/Users/.../sage-5.1.beta6/devel/sage/sage/plot/plot.py
SCORE /Users/.../sage-5.1.beta6/devel/sage/sage/plot/plot.py: 100% (24 of 24)
----------------------------------------------------------------------

$ sage -coverage ~/Downloads/sage-5.1.beta6/devel/sage/sage/plot/graphics.py
----------------------------------------------------------------------
/Users/.../sage-5.1.beta6/devel/sage/sage/plot/graphics.py
SCORE /Users/.../sage-5.1.beta6/devel/sage/sage/plot/graphics.py: 100% (53 of 53)

Possibly wrong (function name doesn't occur in doctests):
	 * _set_figsize_(self, ls):

----------------------------------------------------------------------
```
This was fixed by #12495.



---

archive/issue_comments_095499.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman**",
    "created_at": "2012-07-07T03:30:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10144#issuecomment-95499",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Karl-Dieter Crisman**



---

archive/issue_events_121094.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-07-07T03:30:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121094"
}
```



---

archive/issue_events_121095.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-07-07T03:30:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121095"
}
```



---

archive/issue_events_121096.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-07-07T03:30:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121096"
}
```



---

archive/issue_events_121097.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-07-07T03:30:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121097"
}
```



---

archive/issue_events_121098.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-07-07T03:30:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121098"
}
```



---

archive/issue_events_121099.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-07-07T03:30:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121099"
}
```



---

archive/issue_events_121100.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-17T08:33:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121100"
}
```



---

archive/issue_events_121101.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-17T08:33:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121101"
}
```



---

archive/issue_events_121102.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-17T08:33:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10144",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10144#event-121102"
}
```
