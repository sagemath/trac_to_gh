# Issue 10033: Make evaluation possible for 'hold' objects

archive/issues_010033.json:
```json
{
    "body": "Assignee: @burcin\n\nCC:  @eviatarbach @paulmasson\n\nSee #9879, where it is now possible to 'hold' a symbolic expression:\n\n```\nsage: a = (pi/12).tan(hold=True)\nsage: a\ntan(1/12*pi)\n```\n\nHowever, without going through Maxima and `a.simplify()`, it isn't clear how to get the actual answer for this.  Either by changing `simplify()` to try simplifying through Pynac first, or by adding something like an `a.eval()` method, we should make that possible without Maxima.\n\nIssue created by migration from https://trac.sagemath.org/ticket/10034\n\n",
    "closed_at": "2016-06-26T23:37:52Z",
    "created_at": "2010-09-29T18:59:28Z",
    "labels": [
        "component: symbolics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.10",
    "title": "Make evaluation possible for 'hold' objects",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10033",
    "user": "https://github.com/kcrisman"
}
```
Assignee: @burcin

CC:  @eviatarbach @paulmasson

See #9879, where it is now possible to 'hold' a symbolic expression:

```
sage: a = (pi/12).tan(hold=True)
sage: a
tan(1/12*pi)
```

However, without going through Maxima and `a.simplify()`, it isn't clear how to get the actual answer for this.  Either by changing `simplify()` to try simplifying through Pynac first, or by adding something like an `a.eval()` method, we should make that possible without Maxima.

Issue created by migration from https://trac.sagemath.org/ticket/10034





---

archive/issue_comments_100649.json:
```json
{
    "body": "This is related to #10071, which is about functions which can't even be evaluated using Maxima or `n()`.  This ticket is saying that one should be able to evaluate all held functions without using Maxima or `n()`, whether or not a function currently can be evaluated in that way or not.",
    "created_at": "2010-10-06T14:28:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100649",
    "user": "https://github.com/kcrisman"
}
```

This is related to #10071, which is about functions which can't even be evaluated using Maxima or `n()`.  This ticket is saying that one should be able to evaluate all held functions without using Maxima or `n()`, whether or not a function currently can be evaluated in that way or not.



---

archive/issue_comments_100650.json:
```json
{
    "body": "Here's a patch which makes evaluation possible, simply by walking the expression and evaluating all operations. It does not work for the functions in #10071, because the `.operator()` method doesn't work on them; I believe this is a separate issue for another ticket though.\n\nPatchbot apply trac_10034.patch",
    "created_at": "2013-07-03T02:50:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100650",
    "user": "https://github.com/eviatarbach"
}
```

Here's a patch which makes evaluation possible, simply by walking the expression and evaluating all operations. It does not work for the functions in #10071, because the `.operator()` method doesn't work on them; I believe this is a separate issue for another ticket though.

Patchbot apply trac_10034.patch



---

archive/issue_comments_100651.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-07-03T02:50:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100651",
    "user": "https://github.com/eviatarbach"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_100652.json:
```json
{
    "body": "Attachment [trac_10034.patch](tarball://root/attachments/some-uuid/ticket10034/trac_10034.patch) by @eviatarbach created at 2013-07-03 02:51:53",
    "created_at": "2013-07-03T02:51:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100652",
    "user": "https://github.com/eviatarbach"
}
```

Attachment [trac_10034.patch](tarball://root/attachments/some-uuid/ticket10034/trac_10034.patch) by @eviatarbach created at 2013-07-03 02:51:53



---

archive/issue_comments_100653.json:
```json
{
    "body": "Glance looks good.  Before I think more about this, a question - is `eval` the right name for this?  I know I'm the one who suggested it... but what do other eval methods do?  Also, I think there are a lot of examples which use `simplify` to evaluate these currently - maybe we could switch them to this (or add this, perhaps).  Yes, I agree that #10071 is fine not to try to handle here - that's why I opened #10071.",
    "created_at": "2013-07-03T16:05:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100653",
    "user": "https://github.com/kcrisman"
}
```

Glance looks good.  Before I think more about this, a question - is `eval` the right name for this?  I know I'm the one who suggested it... but what do other eval methods do?  Also, I think there are a lot of examples which use `simplify` to evaluate these currently - maybe we could switch them to this (or add this, perhaps).  Yes, I agree that #10071 is fine not to try to handle here - that's why I opened #10071.



---

archive/issue_comments_100654.json:
```json
{
    "body": "The `_eval_` method for symbolic functions defines what to do when the function is evaluated, and the `_eval_self` method for expressions tries to do numerical evaluation. Maybe a name like `unhold` would be better?\n\nAh right, I'll switch the examples to this. There's no reason why these expressions should be transferred to Maxima simply for evaluating an operation.",
    "created_at": "2013-07-03T18:10:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100654",
    "user": "https://github.com/eviatarbach"
}
```

The `_eval_` method for symbolic functions defines what to do when the function is evaluated, and the `_eval_self` method for expressions tries to do numerical evaluation. Maybe a name like `unhold` would be better?

Ah right, I'll switch the examples to this. There's no reason why these expressions should be transferred to Maxima simply for evaluating an operation.



---

archive/issue_comments_100655.json:
```json
{
    "body": "Ok.\n\nAs another remark (and you might hate me for this one), I could imagine someone wanting to evaluate *some* but not *all* of the held operations.   I think this is possible with your patch and judicious use of `op` and the tree, but at least adding an example of that would be helpful. Particularly in the `x * x + x * x` example, though, I wonder if it wouldn't be pretty annoying to simplify this to `2 * x * x` using this.  What do you think about such cases?",
    "created_at": "2013-07-03T18:26:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100655",
    "user": "https://github.com/kcrisman"
}
```

Ok.

As another remark (and you might hate me for this one), I could imagine someone wanting to evaluate *some* but not *all* of the held operations.   I think this is possible with your patch and judicious use of `op` and the tree, but at least adding an example of that would be helpful. Particularly in the `x * x + x * x` example, though, I wonder if it wouldn't be pretty annoying to simplify this to `2 * x * x` using this.  What do you think about such cases?



---

archive/issue_comments_100656.json:
```json
{
    "body": "I've thought about this. One option is to use pattern matching to specify the parts to evaluate, but I don't how the expression could be reconstructed afterwards.\n\nAnother option is to have an argument for providing a list of operators not to evaluate (I think it's better to have an argument to exclude rather than include, because it is difficult to find all the operators in Sage, while finding ones to exclude just involves searching the expression). Then for the `2 * x * x` example you could just add `operator.mul` to the excluded operators and it would work.",
    "created_at": "2013-07-03T18:42:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100656",
    "user": "https://github.com/eviatarbach"
}
```

I've thought about this. One option is to use pattern matching to specify the parts to evaluate, but I don't how the expression could be reconstructed afterwards.

Another option is to have an argument for providing a list of operators not to evaluate (I think it's better to have an argument to exclude rather than include, because it is difficult to find all the operators in Sage, while finding ones to exclude just involves searching the expression). Then for the `2 * x * x` example you could just add `operator.mul` to the excluded operators and it would work.



---

archive/issue_comments_100657.json:
```json
{
    "body": "Okay, the new patch renames `eval` to `unhold`, moves the examples to use the new method, and adds an `exclude` option. Excluding arithmetic operators doesn't yet work because of #14850.\n\nPatchbot apply trac_10034_2.patch",
    "created_at": "2013-07-03T22:15:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100657",
    "user": "https://github.com/eviatarbach"
}
```

Okay, the new patch renames `eval` to `unhold`, moves the examples to use the new method, and adds an `exclude` option. Excluding arithmetic operators doesn't yet work because of #14850.

Patchbot apply trac_10034_2.patch



---

archive/issue_comments_100658.json:
```json
{
    "body": "Attachment [trac_10034_2.patch](tarball://root/attachments/some-uuid/ticket10034/trac_10034_2.patch) by @eviatarbach created at 2013-07-03 22:15:31",
    "created_at": "2013-07-03T22:15:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100658",
    "user": "https://github.com/eviatarbach"
}
```

Attachment [trac_10034_2.patch](tarball://root/attachments/some-uuid/ticket10034/trac_10034_2.patch) by @eviatarbach created at 2013-07-03 22:15:31



---

archive/issue_comments_100659.json:
```json
{
    "body": "Actually, the way I implemented it is not correct, since if the length of the operands is over two it reduces the rest of the operands using the operator. This is the desired behaviour for the arithmetic operators, but not generally.",
    "created_at": "2013-07-05T18:38:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100659",
    "user": "https://github.com/eviatarbach"
}
```

Actually, the way I implemented it is not correct, since if the length of the operands is over two it reduces the rest of the operands using the operator. This is the desired behaviour for the arithmetic operators, but not generally.



---

archive/issue_comments_100660.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2013-07-05T18:38:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100660",
    "user": "https://github.com/eviatarbach"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_events_025452.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25452"
}
```



---

archive/issue_comments_100661.json:
```json
{
    "body": "Maybe this should also fix other places the hold stuff is shown, e.g. functions/trig.py.",
    "created_at": "2013-08-20T00:59:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100661",
    "user": "https://github.com/kcrisman"
}
```

Maybe this should also fix other places the hold stuff is shown, e.g. functions/trig.py.



---

archive/issue_events_025453.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25453"
}
```



---

archive/issue_events_025454.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25454"
}
```



---

archive/issue_events_025455.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25455"
}
```



---

archive/issue_events_025456.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25456"
}
```



---

archive/issue_events_025457.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25457"
}
```



---

archive/issue_events_025458.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25458"
}
```



---

archive/issue_comments_100662.json:
```json
{
    "body": "I don't think an `eval` member is right here. The end user would expect `.simplify_trig()` to work, and it does actually. The only problem the original submitter had was the Maxima overhead, so it boils down to a native implementation called from `simplify_trig()`.",
    "created_at": "2014-12-25T08:25:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100662",
    "user": "https://github.com/rwst"
}
```

I don't think an `eval` member is right here. The end user would expect `.simplify_trig()` to work, and it does actually. The only problem the original submitter had was the Maxima overhead, so it boils down to a native implementation called from `simplify_trig()`.



---

archive/issue_comments_100663.json:
```json
{
    "body": "What I wrote:\n> I don't think an `eval` member is right here. The end user would expect `.simplify_trig()` to work, and it does actually. The only problem the original submitter had was the Maxima overhead, so it boils down to a native implementation called from `simplify_trig()`.\n\nis of course nonsense. Every function that sends the held expression through Maxima unchanged would work because the hold status gets lost. The expansion happens in Pynac's `(function)::eval` so is already implemented outside Maxima.",
    "created_at": "2015-07-27T07:21:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100663",
    "user": "https://github.com/rwst"
}
```

What I wrote:
> I don't think an `eval` member is right here. The end user would expect `.simplify_trig()` to work, and it does actually. The only problem the original submitter had was the Maxima overhead, so it boils down to a native implementation called from `simplify_trig()`.

is of course nonsense. Every function that sends the held expression through Maxima unchanged would work because the hold status gets lost. The expansion happens in Pynac's `(function)::eval` so is already implemented outside Maxima.



---

archive/issue_comments_100664.json:
```json
{
    "body": "Previous title restored.",
    "created_at": "2015-10-22T15:21:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100664",
    "user": "https://github.com/rwst"
}
```

Previous title restored.



---

archive/issue_events_025459.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-10-23T14:15:11Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25459"
}
```



---

archive/issue_events_025460.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-10-23T14:15:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "milestone": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25460"
}
```



---

archive/issue_comments_100665.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2015-10-23T14:15:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100665",
    "user": "https://github.com/rwst"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_100666.json:
```json
{
    "body": "I used the convenient `ExpressionTreeWalker` that takes care of the caveats mentioned above.\n\n---\nNew commits:",
    "created_at": "2015-10-23T14:15:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100666",
    "user": "https://github.com/rwst"
}
```

I used the convenient `ExpressionTreeWalker` that takes care of the caveats mentioned above.

---
New commits:



---

archive/issue_comments_100667.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-06-13T08:04:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100667",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_100668.json:
```json
{
    "body": "Everything I tested functions just fine, including building documents. Ready for positive review.\n\nOne minor quibble: the exclude option references Trac #14850, which is a closed duplicate of #10169. Shouldn't that be updated?",
    "created_at": "2016-06-24T20:39:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100668",
    "user": "https://github.com/paulmasson"
}
```

Everything I tested functions just fine, including building documents. Ready for positive review.

One minor quibble: the exclude option references Trac #14850, which is a closed duplicate of #10169. Shouldn't that be updated?



---

archive/issue_comments_100669.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-06-25T06:47:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100669",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_100670.json:
```json
{
    "body": "Right. Thanks for the review. Please add your name to Reviewers: too.",
    "created_at": "2016-06-25T06:48:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100670",
    "user": "https://github.com/rwst"
}
```

Right. Thanks for the review. Please add your name to Reviewers: too.



---

archive/issue_comments_100671.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-06-25T19:40:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100671",
    "user": "https://github.com/paulmasson"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_025461.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-26T23:37:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10033#event-25461"
}
```



---

archive/issue_comments_100672.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2016-06-26T23:37:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10033#issuecomment-100672",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
