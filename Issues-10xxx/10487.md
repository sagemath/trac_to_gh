# Issue 10487: Clean up doctests in sage/rings/finite_rings/finite_field_ext_pari.py

archive/issues_010434.json:
```json
{
    "assignees": [],
    "body": "\n\nAssignee: **mvngu**\n\nAuthor: **Jeroen Demeyer**\n\nReviewer: **Gagan Sekhon**\n\nMerged: **sage-4.6.2.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10487_\n\n",
    "closed_at": "2011-02-16T08:50:18Z",
    "created_at": "2010-12-16T16:53:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean up doctests in sage/rings/finite_rings/finite_field_ext_pari.py",
    "type": "issue",
    "updated_at": "2011-02-16T08:50:18Z",
    "url": "https://github.com/sagemath/sage/issues/10487",
    "user": "https://github.com/jdemeyer"
}
```


Assignee: **mvngu**

Author: **Jeroen Demeyer**

Reviewer: **Gagan Sekhon**

Merged: **sage-4.6.2.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/10487_





---

archive/issue_events_121356.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-16T16:53:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121356"
}
```



---

archive/issue_events_121357.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-16T16:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121357"
}
```



---

archive/issue_events_121358.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-16T16:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121358"
}
```



---

archive/issue_events_121359.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-16T16:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121359"
}
```



---

archive/issue_comments_100620.json:
```json
{
    "body": "Attachment: **[10487.patch.gz](https://github.com/sagemath/sage/files/ticket10487/10487.patch.gz)**",
    "created_at": "2011-02-04T10:45:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10487#issuecomment-100620",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[10487.patch.gz](https://github.com/sagemath/sage/files/ticket10487/10487.patch.gz)**



---

archive/issue_events_121360.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-04T10:45:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121360"
}
```



---

archive/issue_events_121361.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-04T10:45:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121361"
}
```



---

archive/issue_comments_100621.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2011-02-04T10:45:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10487#issuecomment-100621",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_events_121362.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-04T10:45:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121362"
}
```



---

archive/issue_events_121363.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-04T10:45:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121363"
}
```



---

archive/issue_comments_100622.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nIt passed all tests, but I can figure out where the output html file is located. Given you changed doctest and documentation, that is where all the changes would happen.",
    "created_at": "2011-02-10T16:53:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10487#issuecomment-100622",
    "user": "https://github.com/sagetrac-gagansekhon"
}
```

<a id='comment:2'>Comment 2:</a>
It passed all tests, but I can figure out where the output html file is located. Given you changed doctest and documentation, that is where all the changes would happen.



---

archive/issue_comments_100623.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis file doesn't actually appear in the reference manual, so there is no HTML file.",
    "created_at": "2011-02-11T08:25:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10487#issuecomment-100623",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
This file doesn't actually appear in the reference manual, so there is no HTML file.



---

archive/issue_comments_100624.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nIt passed all test and it seems it does not appear in the reference manual",
    "created_at": "2011-02-14T05:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10487#issuecomment-100624",
    "user": "https://github.com/sagetrac-gagansekhon"
}
```

<a id='comment:4'>Comment 4:</a>
It passed all test and it seems it does not appear in the reference manual



---

archive/issue_events_121364.json:
```json
{
    "actor": "https://github.com/sagetrac-gagansekhon",
    "created_at": "2011-02-14T05:20:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121364"
}
```



---

archive/issue_events_121365.json:
```json
{
    "actor": "https://github.com/sagetrac-gagansekhon",
    "created_at": "2011-02-14T05:20:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121365"
}
```



---

archive/issue_comments_100625.json:
```json
{
    "body": "Reviewer: **Gagan Sekhon**",
    "created_at": "2011-02-14T05:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10487#issuecomment-100625",
    "user": "https://github.com/sagetrac-gagansekhon"
}
```

Reviewer: **Gagan Sekhon**



---

archive/issue_events_121366.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-16T08:50:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121366"
}
```



---

archive/issue_events_121367.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-16T08:50:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10487#event-121367"
}
```



---

archive/issue_comments_100626.json:
```json
{
    "body": "Merged: **sage-4.6.2.rc0**",
    "created_at": "2011-02-16T08:50:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10487",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10487#issuecomment-100626",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.2.rc0**
