# Issue 10139: Problem found in graphs.is_planar() when the graph has no edges.

archive/issues_010138.json:
```json
{
    "assignees": [],
    "body": "The is_planar method for graphs includes a set_embedding argument that saves the combinatorial embedding of the graph (if it's planar). In this example, it works properly:\n\nsage: g = graphs.WheelGraph(6)\nsage: g.is_planar(set_embedding=True)\nTrue\nsage: g.get_embedding()\n{0: [1, 5, 4, 3, 2], 1: [2, 5, 0], 2: [0, 3, 1], 3: [0, 4, 2], 4: [0, 5, 3], 5: [0, 1, 4]}\n\nHowever, is we apply this method to a graph with no edges, a problem arises:\n\nsage: g = graphs.EmptyGraph()\nsage: g.add_vertices(range(6))\nsage: g.is_planar(set_embedding=True)\n\n---\nAttributeError                            Traceback (most recent call last)\n\n/home/ers/<ipython console> in <module>()\n\n/usr/public/share/sage/local/lib/python2.6/site-packages/sage/graphs/generic_graph.pyc in is_planar(self, on_embedding, kuratowski, set_embedding, set_pos)\n   2438                     self._pos = G._pos\n   2439                 if set_embedding:\n-> 2440                     self._embedding = G._embedding\n   2441             return planar\n   2442 \n\nAttributeError: 'Graph' object has no attribute '_embedding'\n\nOnce we add an edge to this graph, all is well:\n\nsage: g.add_edge(0,1)\nsage: sage: g.is_planar(set_embedding=True)\nTrue\nsage: g.get_embedding()\n{0: [1], 1: [0], 2: [], 3: [], 4: [], 5: []}\n\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  brunellus\n\nReviewer: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10139_\n\n",
    "closed_at": "2012-01-13T09:00:19Z",
    "created_at": "2010-10-17T19:12:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Problem found in graphs.is_planar() when the graph has no edges.",
    "type": "issue",
    "updated_at": "2012-01-13T09:00:19Z",
    "url": "https://github.com/sagemath/sage/issues/10139",
    "user": "https://github.com/sagetrac-edward-scheinerman"
}
```
The is_planar method for graphs includes a set_embedding argument that saves the combinatorial embedding of the graph (if it's planar). In this example, it works properly:

sage: g = graphs.WheelGraph(6)
sage: g.is_planar(set_embedding=True)
True
sage: g.get_embedding()
{0: [1, 5, 4, 3, 2], 1: [2, 5, 0], 2: [0, 3, 1], 3: [0, 4, 2], 4: [0, 5, 3], 5: [0, 1, 4]}

However, is we apply this method to a graph with no edges, a problem arises:

sage: g = graphs.EmptyGraph()
sage: g.add_vertices(range(6))
sage: g.is_planar(set_embedding=True)

---
AttributeError                            Traceback (most recent call last)

/home/ers/<ipython console> in <module>()

/usr/public/share/sage/local/lib/python2.6/site-packages/sage/graphs/generic_graph.pyc in is_planar(self, on_embedding, kuratowski, set_embedding, set_pos)
   2438                     self._pos = G._pos
   2439                 if set_embedding:
-> 2440                     self._embedding = G._embedding
   2441             return planar
   2442 

AttributeError: 'Graph' object has no attribute '_embedding'

Once we add an edge to this graph, all is well:

sage: g.add_edge(0,1)
sage: sage: g.is_planar(set_embedding=True)
True
sage: g.get_embedding()
{0: [1], 1: [0], 2: [], 3: [], 4: [], 5: []}


Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  brunellus

Reviewer: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/10139_





---

archive/issue_events_121013.json:
```json
{
    "actor": "https://github.com/sagetrac-edward-scheinerman",
    "created_at": "2010-10-17T19:12:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121013"
}
```



---

archive/issue_events_121014.json:
```json
{
    "actor": "https://github.com/sagetrac-edward-scheinerman",
    "created_at": "2010-10-17T19:12:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121014"
}
```



---

archive/issue_events_121015.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-10-18T08:01:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121015"
}
```



---

archive/issue_comments_095395.json:
```json
{
    "body": "Assignee: **@jasongrout, @nathanncohen, @rlmill**",
    "created_at": "2010-10-18T08:01:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10139#issuecomment-95395",
    "user": "https://github.com/nathanncohen"
}
```

Assignee: **@jasongrout, @nathanncohen, @rlmill**



---

archive/issue_events_121016.json:
```json
{
    "actor": "https://github.com/sagetrac-brunellus",
    "created_at": "2011-12-31T03:33:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121016"
}
```



---

archive/issue_comments_095396.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThe patch from #12134 should solve this problem.",
    "created_at": "2011-12-31T03:33:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10139#issuecomment-95396",
    "user": "https://github.com/sagetrac-brunellus"
}
```

<a id='comment:3'>Comment 3:</a>
The patch from #12134 should solve this problem.



---

archive/issue_events_121017.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-12-31T09:59:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121017"
}
```



---

archive/issue_events_121018.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-12-31T09:59:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121018"
}
```



---

archive/issue_events_121019.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-12-31T09:59:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121019"
}
```



---

archive/issue_events_121020.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-12-31T09:59:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121020"
}
```



---

archive/issue_comments_095397.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2012-01-13T09:00:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10139#issuecomment-95397",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_121021.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-13T09:00:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121021"
}
```



---

archive/issue_events_121022.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-13T09:00:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121022"
}
```



---

archive/issue_events_121023.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-13T09:00:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10139",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10139#event-121023"
}
```
