# Issue 10332: isPositiveSemiDefinite not accessible

archive/issues_010331.json:
```json
{
    "body": "I would like to test rational matrices for semidefinite positiveness. There is such a function in linbox, apparently (isPositiveSemiDefinite, from is-positive-semidefinite.h), but it is not accessible from Sage.\n\nAssignee: jason, was\n\nCC:  @orlitzky @kliem @mwageringel @dimpase @rbeezer\n\nKeywords: psd, semidefinite positive\n\nBranch/Commit: 909c1e19c205fc8741c5d2934f10cd8969c7a507\n\nReviewer: Dima Pasechnik\n\nAuthor: Michael Orlitzky\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/10332\n\n",
    "closed_at": "2021-02-20T10:46:42Z",
    "created_at": "2010-11-25T22:10:03Z",
    "labels": [
        "component: linear algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.3",
    "title": "isPositiveSemiDefinite not accessible",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10332",
    "user": "https://trac.sagemath.org/admin/accounts/users/monniaux"
}
```
I would like to test rational matrices for semidefinite positiveness. There is such a function in linbox, apparently (isPositiveSemiDefinite, from is-positive-semidefinite.h), but it is not accessible from Sage.

Assignee: jason, was

CC:  @orlitzky @kliem @mwageringel @dimpase @rbeezer

Keywords: psd, semidefinite positive

Branch/Commit: 909c1e19c205fc8741c5d2934f10cd8969c7a507

Reviewer: Dima Pasechnik

Author: Michael Orlitzky

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/10332





---

archive/issue_comments_127413.json:
```json
{
    "body": "Changing component from linbox to linear algebra.",
    "created_at": "2013-03-28T22:50:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127413",
    "user": "https://github.com/jdemeyer"
}
```

Changing component from linbox to linear algebra.



---

archive/issue_comments_127414.json:
```json
{
    "body": "Changing assignee from @ClementPernet to jason, was.",
    "created_at": "2013-03-28T22:50:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127414",
    "user": "https://github.com/jdemeyer"
}
```

Changing assignee from @ClementPernet to jason, was.



---

archive/issue_comments_127415.json:
```json
{
    "body": "<a id='comment:2'></a>I went down a rabbit hole with the cholesky inverse and wound up here. The inverse of a positive-definite matrix can actually be computed a tiiiiny bit faster through the `indefinite_factorization` method, which returns a naive (suitable only in exact arithmetic) LDLT factorization.\n\nIf we modify the `indefinite_factorization` to pivot (put the zeros at the bottom of `D`), we could also perform an LDLT factorization for positive-SEMIdefinite matrices, making the goal of this ticket attainable.\n\nNote also that some people have asked for square roots of PSD matrices in #23424 and #25464. Right now that's only implemented for positive-definite matrices, due to the aforementioned limitation of the LDLT factorization, so we could potentially improve that too.",
    "created_at": "2020-09-26T02:27:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127415",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:2'></a>I went down a rabbit hole with the cholesky inverse and wound up here. The inverse of a positive-definite matrix can actually be computed a tiiiiny bit faster through the `indefinite_factorization` method, which returns a naive (suitable only in exact arithmetic) LDLT factorization.

If we modify the `indefinite_factorization` to pivot (put the zeros at the bottom of `D`), we could also perform an LDLT factorization for positive-SEMIdefinite matrices, making the goal of this ticket attainable.

Note also that some people have asked for square roots of PSD matrices in #23424 and #25464. Right now that's only implemented for positive-definite matrices, due to the aforementioned limitation of the LDLT factorization, so we could potentially improve that too.



---

archive/issue_comments_127416.json:
```json
{
    "body": "<a id='comment:3'></a>I'm slowly working on a pivoted LDLT factorization at\n\n  http://gitweb.michael.orlitzky.com/?p=sage.d.git;a=blob;f=mjo/ldlt.py\n\nAt some point I will have to carefully check (unless someone has a reference) how it can be used to determine positive-semidefiniteness, especially with complex matrices.",
    "created_at": "2020-09-29T15:26:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127416",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:3'></a>I'm slowly working on a pivoted LDLT factorization at

  http://gitweb.michael.orlitzky.com/?p=sage.d.git;a=blob;f=mjo/ldlt.py

At some point I will have to carefully check (unless someone has a reference) how it can be used to determine positive-semidefiniteness, especially with complex matrices.



---

archive/issue_comments_127417.json:
```json
{
    "body": "Changing branch from \"\" to \"u/mjo/ticket/10332\"",
    "created_at": "2020-10-07T21:21:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127417",
    "user": "https://github.com/orlitzky"
}
```

Changing branch from "" to "u/mjo/ticket/10332"



---

archive/issue_comments_127418.json:
```json
{
    "body": "Changing author from \"\" to \"Michael Orlitzky\"",
    "created_at": "2020-10-07T21:21:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127418",
    "user": "https://github.com/orlitzky"
}
```

Changing author from "" to "Michael Orlitzky"



---

archive/issue_comments_127419.json:
```json
{
    "body": "Changing commit from \"\" to \"b2b925e81cdfa95475a545d4e2b37bfad337edfb\"",
    "created_at": "2020-10-07T21:21:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127419",
    "user": "https://github.com/orlitzky"
}
```

Changing commit from "" to "b2b925e81cdfa95475a545d4e2b37bfad337edfb"



---

archive/issue_comments_127420.json:
```json
{
    "body": "<a id='comment:4'></a>This is nearing presentability.\n\nMy branch has a numerically stable block-LDLT factorization that works for indefinite matrices and in inexact arithmetic. The cited paper shows why we can use it to determine positive-semidefiniteness: each 2x2 diagonal block corresponds to one positive and one negative eigenvalue, and Sylvester's inertia theorem handles the rest.. I've added an `is_positive_semidefinite()` method for matrices based on that.\n\nCompared to `is_positive_definite()`, the new method...\n\n1. Doesn't distinguish between real/complex algorithms. We simply insist that the matrix be Hermitian (which is true of real symmetric matrices, of course), and always use `conjugate_transpose()`. This is not hugely expensive and cleans up the user interface a lot.\n2. Doesn't do any checks on the input matrix. So long as the matrix is Hermitian, you should be okay -- but you're in charge of checking that. Running `if not A.is_hermitian()...` on your own is a lot simpler than asking the method to check it for you, and then waiting to catch an exception. And when you want it to be fast, there's no need to disable the checks; it's already fast.\n\nThis could conceivably also be used to speed up `solve_left` and `solve_right` for Hermitian matrices.",
    "created_at": "2020-10-07T21:21:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127420",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:4'></a>This is nearing presentability.

My branch has a numerically stable block-LDLT factorization that works for indefinite matrices and in inexact arithmetic. The cited paper shows why we can use it to determine positive-semidefiniteness: each 2x2 diagonal block corresponds to one positive and one negative eigenvalue, and Sylvester's inertia theorem handles the rest.. I've added an `is_positive_semidefinite()` method for matrices based on that.

Compared to `is_positive_definite()`, the new method...

1. Doesn't distinguish between real/complex algorithms. We simply insist that the matrix be Hermitian (which is true of real symmetric matrices, of course), and always use `conjugate_transpose()`. This is not hugely expensive and cleans up the user interface a lot.
2. Doesn't do any checks on the input matrix. So long as the matrix is Hermitian, you should be okay -- but you're in charge of checking that. Running `if not A.is_hermitian()...` on your own is a lot simpler than asking the method to check it for you, and then waiting to catch an exception. And when you want it to be fast, there's no need to disable the checks; it's already fast.

This could conceivably also be used to speed up `solve_left` and `solve_right` for Hermitian matrices.



---

archive/issue_comments_127421.json:
```json
{
    "body": "<a id='comment:5'></a>Since I haven't changed anything in four months I guess this is ready for review.",
    "created_at": "2021-01-26T13:18:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127421",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:5'></a>Since I haven't changed anything in four months I guess this is ready for review.



---

archive/issue_comments_127422.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2021-01-26T13:18:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127422",
    "user": "https://github.com/orlitzky"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_127423.json:
```json
{
    "body": "<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:",
    "created_at": "2021-01-26T16:09:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127423",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:



---

archive/issue_comments_127424.json:
```json
{
    "body": "Changing commit from \"b2b925e81cdfa95475a545d4e2b37bfad337edfb\" to \"909c1e19c205fc8741c5d2934f10cd8969c7a507\"",
    "created_at": "2021-01-26T16:09:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127424",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "b2b925e81cdfa95475a545d4e2b37bfad337edfb" to "909c1e19c205fc8741c5d2934f10cd8969c7a507"



---

archive/issue_comments_127425.json:
```json
{
    "body": "<a id='comment:8'></a>Rebased onto develop.",
    "created_at": "2021-01-26T16:10:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127425",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:8'></a>Rebased onto develop.



---

archive/issue_comments_127426.json:
```json
{
    "body": "<a id='comment:9'></a>matrix2.pyx  is long overdue for splitting, it's 670K !!! (unless we aim for Guinness Book of Records, of course).",
    "created_at": "2021-01-27T10:06:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127426",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:9'></a>matrix2.pyx  is long overdue for splitting, it's 670K !!! (unless we aim for Guinness Book of Records, of course).



---

archive/issue_comments_127427.json:
```json
{
    "body": "<a id='comment:10'></a>otherwise, OK.",
    "created_at": "2021-01-27T11:39:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127427",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:10'></a>otherwise, OK.



---

archive/issue_comments_127428.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Dima Pasechnik\"",
    "created_at": "2021-01-27T11:39:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127428",
    "user": "https://github.com/dimpase"
}
```

Changing reviewer from "" to "Dima Pasechnik"



---

archive/issue_comments_127429.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2021-01-27T11:39:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127429",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_127430.json:
```json
{
    "body": "<a id='comment:11'></a>Thanks!",
    "created_at": "2021-01-27T13:29:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127430",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:11'></a>Thanks!



---

archive/issue_events_026567.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-01-30T10:17:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10332#event-26567"
}
```



---

archive/issue_comments_127431.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2021-02-20T10:46:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127431",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_026568.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-02-20T10:46:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10332#event-26568"
}
```



---

archive/issue_comments_127432.json:
```json
{
    "body": "Changing branch from \"u/mjo/ticket/10332\" to \"909c1e19c205fc8741c5d2934f10cd8969c7a507\"",
    "created_at": "2021-02-20T10:46:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10332#issuecomment-127432",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/mjo/ticket/10332" to "909c1e19c205fc8741c5d2934f10cd8969c7a507"
