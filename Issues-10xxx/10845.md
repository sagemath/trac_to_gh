# Issue 10845: fixed errors in documentation in two ETuple methods

archive/issues_010779.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "fixed errors in documentation in emax and emin ETuple methods\n\nComponent: **documentation**\n\nAuthor: **Mario Pernici**\n\nReviewer: **Ivo Hedtke**\n\nMerged: **sage-4.7.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10845_\n\n",
    "closed_at": "2011-07-27T13:40:48Z",
    "created_at": "2011-02-24T17:57:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fixed errors in documentation in two ETuple methods",
    "type": "issue",
    "updated_at": "2011-07-27T13:40:48Z",
    "url": "https://github.com/sagemath/sage/issues/10845",
    "user": "https://github.com/sagetrac-pernici"
}
```
fixed errors in documentation in emax and emin ETuple methods

Component: **documentation**

Author: **Mario Pernici**

Reviewer: **Ivo Hedtke**

Merged: **sage-4.7.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/10845_





---

archive/issue_events_143779.json:
```json
{
    "actor": "https://github.com/sagetrac-pernici",
    "created_at": "2011-02-24T17:57:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143779"
}
```



---

archive/issue_events_143780.json:
```json
{
    "actor": "https://github.com/sagetrac-pernici",
    "created_at": "2011-02-24T17:57:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143780"
}
```



---

archive/issue_events_143781.json:
```json
{
    "actor": "https://github.com/sagetrac-pernici",
    "created_at": "2011-02-24T17:57:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143781"
}
```



---

archive/issue_events_143782.json:
```json
{
    "actor": "https://github.com/sagetrac-pernici",
    "created_at": "2011-02-24T17:57:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143782"
}
```



---

archive/issue_events_143783.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-02-24T17:57:50Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "subject": "https://github.com/sagetrac-pernici",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143783"
}
```



---

archive/issue_comments_105067.json:
```json
{
    "body": "Attachment: **[trac_10845_etuple_doc.patch.gz](https://github.com/sagemath/sage/files/ticket10845/trac_10845_etuple_doc.patch.gz)**",
    "created_at": "2011-02-24T18:00:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10845#issuecomment-105067",
    "user": "https://github.com/sagetrac-pernici"
}
```

Attachment: **[trac_10845_etuple_doc.patch.gz](https://github.com/sagemath/sage/files/ticket10845/trac_10845_etuple_doc.patch.gz)**



---

archive/issue_events_143784.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-22T21:14:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143784"
}
```



---

archive/issue_comments_105068.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nOK.",
    "created_at": "2011-07-23T18:10:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10845#issuecomment-105068",
    "user": "https://github.com/hedtke"
}
```

<div id="comment:2" align="right">comment:2</div>

OK.



---

archive/issue_comments_105069.json:
```json
{
    "body": "Reviewer: **Ivo Hedtke**",
    "created_at": "2011-07-23T18:10:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10845#issuecomment-105069",
    "user": "https://github.com/hedtke"
}
```

Reviewer: **Ivo Hedtke**



---

archive/issue_events_143785.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-23T18:10:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143785"
}
```



---

archive/issue_events_143786.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-23T18:10:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143786"
}
```



---

archive/issue_events_143787.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:24:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143787"
}
```



---

archive/issue_events_143788.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:24:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143788"
}
```



---

archive/issue_comments_105070.json:
```json
{
    "body": "Changed author from **mario pernici** to **Mario Pernici**",
    "created_at": "2011-07-24T11:24:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10845#issuecomment-105070",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **mario pernici** to **Mario Pernici**



---

archive/issue_events_143789.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-27T13:40:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143789"
}
```



---

archive/issue_events_143790.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-27T13:40:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10845#event-143790"
}
```



---

archive/issue_comments_105071.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha0**",
    "created_at": "2011-07-27T13:40:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10845#issuecomment-105071",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.2.alpha0**
