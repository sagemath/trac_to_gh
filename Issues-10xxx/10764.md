# Issue 10764: Cleanup a few Cython warnings

archive/issues_010700.json:
```json
{
    "body": "See, e.g. the list of warnings at #10493.\n\n**Apply** attachment:trac_10764.patch\n\nAssignee: GeorgSWeber\n\nReviewer: Fran\u00e7ois Bissey, Andr\u00e9 Apitzsch\n\nAuthor: Fran\u00e7ois Bissey, Andr\u00e9 Apitzsch\n\nMerged: sage-5.2.beta0\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/10764\n\n",
    "closed_at": "2012-07-02T15:21:21Z",
    "created_at": "2011-02-10T03:16:36Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.2",
    "title": "Cleanup a few Cython warnings",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10764",
    "user": "https://github.com/robertwb"
}
```
See, e.g. the list of warnings at #10493.

**Apply** attachment:trac_10764.patch

Assignee: GeorgSWeber

Reviewer: François Bissey, André Apitzsch

Author: François Bissey, André Apitzsch

Merged: sage-5.2.beta0

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/10764





---

archive/issue_comments_136069.json:
```json
{
    "body": "<a id='comment:1'></a>I am starting the work by fixing the deprecation warnings. One of the easy bits :)",
    "created_at": "2011-02-16T09:36:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136069",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:1'></a>I am starting the work by fixing the deprecation warnings. One of the easy bits :)



---

archive/issue_events_027740.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2011-02-16T09:36:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "milestone": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10764#event-27740"
}
```



---

archive/issue_comments_136070.json:
```json
{
    "body": "<a id='comment:2'></a>I hadn't mentioned these in #10493 I assume we want to fix them as well:\n\n```\nwarning: sage/gsl/fft.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/gsl/gsl_array.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/modular/modsym/p1list.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/modules/free_module_element.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/modules/free_module_element.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/laurent_series_ring_element.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polydict.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_gf2x.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_zmod_flint.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_element.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_integer_dense_ntl.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_zz_pex.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_integer_dense_flint.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_modn_dense_ntl.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_modn_dense_ntl.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_modn_dense_ntl.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/structure/parent.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\nwarning: sage/rings/polynomial/polynomial_rational_flint.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead\n```",
    "created_at": "2011-03-13T22:47:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136070",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:2'></a>I hadn't mentioned these in #10493 I assume we want to fix them as well:

```
warning: sage/gsl/fft.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/gsl/gsl_array.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/modular/modsym/p1list.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/modules/free_module_element.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/modules/free_module_element.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/laurent_series_ring_element.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polydict.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_gf2x.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_zmod_flint.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_element.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_integer_dense_ntl.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_zz_pex.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_integer_dense_flint.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_modn_dense_ntl.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_modn_dense_ntl.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_modn_dense_ntl.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/structure/parent.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
warning: sage/rings/polynomial/polynomial_rational_flint.pyx:1:0: __getslice__, __setslice__, and __delslice__ are not supported by Python 3, use __getitem__, __setitem__, and __delitem__ instead
```



---

archive/issue_comments_136071.json:
```json
{
    "body": "<a id='comment:3'></a>I am looking at gsl/fft.pyx and some of the code is strange\n\n```\n    def __getitem__(self, size_t i):\n        if i < 0 or i >= self.n:\n            raise IndexError\n        cdef int j\n        j = 2*i\n        return self.data[2*i], self.data[2*i+1]\n\n    def __getslice__(self, Py_ssize_t i, Py_ssize_t j):\n        # Todo -- make this actually fast.\n        return list(self)[i:j]\n```\nThere is already a getitem, I don't think it would work with slices. But getitem and getslice look very different. One asks for size_t while the other asks for Py_ssize_t I assume one is deprecated. The code in getitem is very strange. j is declared, computed and never used, the code also return something different from you would naively expect. None of this seem to ever be called from anywhere else in sage.",
    "created_at": "2011-03-14T02:36:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136071",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:3'></a>I am looking at gsl/fft.pyx and some of the code is strange

```
    def __getitem__(self, size_t i):
        if i < 0 or i >= self.n:
            raise IndexError
        cdef int j
        j = 2*i
        return self.data[2*i], self.data[2*i+1]

    def __getslice__(self, Py_ssize_t i, Py_ssize_t j):
        # Todo -- make this actually fast.
        return list(self)[i:j]
```
There is already a getitem, I don't think it would work with slices. But getitem and getslice look very different. One asks for size_t while the other asks for Py_ssize_t I assume one is deprecated. The code in getitem is very strange. j is declared, computed and never used, the code also return something different from you would naively expect. None of this seem to ever be called from anywhere else in sage.



---

archive/issue_comments_136072.json:
```json
{
    "body": "<a id='comment:4'></a>The warning in sage/combinat/combinat_cython.pyx has been fixed as a side effect of #10258. Updated patch coming shortly.",
    "created_at": "2011-05-02T03:34:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136072",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:4'></a>The warning in sage/combinat/combinat_cython.pyx has been fixed as a side effect of #10258. Updated patch coming shortly.



---

archive/issue_comments_136073.json:
```json
{
    "body": "Fix deprecation warnings (updated after #10258)",
    "created_at": "2011-05-02T03:35:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136073",
    "user": "https://github.com/kiwifb"
}
```

Fix deprecation warnings (updated after #10258)



---

archive/issue_comments_136074.json:
```json
{
    "body": "Attachment [trac_10764-fix_deprecation_warning.patch](tarball://root/attachments/some-uuid/ticket10764/trac_10764-fix_deprecation_warning.patch) by @kiwifb created at 2011-05-02 04:04:00\n\nprevious patch broke a doctest in gen_interpreters.py.",
    "created_at": "2011-05-02T04:04:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136074",
    "user": "https://github.com/kiwifb"
}
```

Attachment [trac_10764-fix_deprecation_warning.patch](tarball://root/attachments/some-uuid/ticket10764/trac_10764-fix_deprecation_warning.patch) by @kiwifb created at 2011-05-02 04:04:00

previous patch broke a doctest in gen_interpreters.py.



---

archive/issue_comments_136075.json:
```json
{
    "body": "<a id='comment:5'></a>Attachment [trac_10764-fix-gen_interpreters_doctest.patch](tarball://root/attachments/some-uuid/ticket10764/trac_10764-fix-gen_interpreters_doctest.patch) by @a-andre created at 2011-11-15 15:22:15\n\nThe patch attached to #12041 should fix some warnings.",
    "created_at": "2011-11-15T15:22:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136075",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:5'></a>Attachment [trac_10764-fix-gen_interpreters_doctest.patch](tarball://root/attachments/some-uuid/ticket10764/trac_10764-fix-gen_interpreters_doctest.patch) by @a-andre created at 2011-11-15 15:22:15

The patch attached to #12041 should fix some warnings.



---

archive/issue_comments_136076.json:
```json
{
    "body": "<a id='comment:6'></a>Replying to [aapitzsch](#comment%3A5):\n> The patch attached to #12041 should fix some warnings.\n\n#12093 fixes the remaining ones. See also #12119.",
    "created_at": "2011-12-04T22:42:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136076",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:6'></a>Replying to [aapitzsch](#comment%3A5):
> The patch attached to #12041 should fix some warnings.

#12093 fixes the remaining ones. See also #12119.



---

archive/issue_comments_136077.json:
```json
{
    "body": "<a id='comment:7'></a>The other deprecation warnings are fixed, so this ticket can be reviewed.",
    "created_at": "2012-04-06T17:56:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136077",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:7'></a>The other deprecation warnings are fixed, so this ticket can be reviewed.



---

archive/issue_comments_136078.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-04-06T17:56:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136078",
    "user": "https://github.com/a-andre"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_136079.json:
```json
{
    "body": "Changing work_issues from \"\" to \"Rebase\"",
    "created_at": "2012-04-06T18:00:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136079",
    "user": "https://github.com/a-andre"
}
```

Changing work_issues from "" to "Rebase"



---

archive/issue_comments_136080.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-04-06T18:00:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136080",
    "user": "https://github.com/a-andre"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_136081.json:
```json
{
    "body": "<a id='comment:8'></a>Please rebase to current beta.",
    "created_at": "2012-04-06T18:00:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136081",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:8'></a>Please rebase to current beta.



---

archive/issue_comments_136082.json:
```json
{
    "body": "Attachment [trac_10764.patch](tarball://root/attachments/some-uuid/ticket10764/trac_10764.patch) by @a-andre created at 2012-06-24 14:21:35",
    "created_at": "2012-06-24T14:21:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136082",
    "user": "https://github.com/a-andre"
}
```

Attachment [trac_10764.patch](tarball://root/attachments/some-uuid/ticket10764/trac_10764.patch) by @a-andre created at 2012-06-24 14:21:35



---

archive/issue_comments_136083.json:
```json
{
    "body": "Changing work_issues from \"Rebase\" to \"\"",
    "created_at": "2012-06-24T14:23:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136083",
    "user": "https://github.com/a-andre"
}
```

Changing work_issues from "Rebase" to ""



---

archive/issue_comments_136084.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2012-06-24T14:23:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136084",
    "user": "https://github.com/a-andre"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_136085.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n-See, e.g. the list of warnings at #10493. \n+See, e.g. the list of warnings at #10493.\n+\n+**Apply** attachment:trac_10764.patch\n``````\n",
    "created_at": "2012-06-24T14:23:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136085",
    "user": "https://github.com/a-andre"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
-See, e.g. the list of warnings at #10493. 
+See, e.g. the list of warnings at #10493.
+
+**Apply** attachment:trac_10764.patch
``````




---

archive/issue_comments_136086.json:
```json
{
    "body": "<a id='comment:10'></a>patchbot: apply trac_10764.patch",
    "created_at": "2012-06-24T14:30:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136086",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:10'></a>patchbot: apply trac_10764.patch



---

archive/issue_comments_136087.json:
```json
{
    "body": "<a id='comment:11'></a>sorry to have been out of touch. I assume your patch is a rebase of my 2 patches, correct?",
    "created_at": "2012-06-25T09:29:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136087",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:11'></a>sorry to have been out of touch. I assume your patch is a rebase of my 2 patches, correct?



---

archive/issue_comments_136088.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [fbissey](#comment%3A11):\n> sorry to have been out of touch. I assume your patch is a rebase of my 2 patches, correct?\nYes. Additionally I removed some trailing white spaces.",
    "created_at": "2012-06-25T09:48:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136088",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:12'></a>Replying to [fbissey](#comment%3A11):
> sorry to have been out of touch. I assume your patch is a rebase of my 2 patches, correct?
Yes. Additionally I removed some trailing white spaces.



---

archive/issue_comments_136089.json:
```json
{
    "body": "<a id='comment:13'></a>I am putting this to positive review. I guess we agree on what needs doing and that it doesn't have side effects.",
    "created_at": "2012-06-26T10:30:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136089",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:13'></a>I am putting this to positive review. I guess we agree on what needs doing and that it doesn't have side effects.



---

archive/issue_comments_136090.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Fran\u00e7ois Bissey, Andr\u00e9 Apitzsch\"",
    "created_at": "2012-06-26T10:30:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136090",
    "user": "https://github.com/kiwifb"
}
```

Changing reviewer from "" to "François Bissey, André Apitzsch"



---

archive/issue_comments_136091.json:
```json
{
    "body": "Changing author from \"\" to \"Fran\u00e7ois Bissey, Andr\u00e9 Apitzsch\"",
    "created_at": "2012-06-26T10:30:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136091",
    "user": "https://github.com/kiwifb"
}
```

Changing author from "" to "François Bissey, André Apitzsch"



---

archive/issue_comments_136092.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-06-26T10:30:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136092",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_027741.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2012-06-26T10:30:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "milestone": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10764#event-27741"
}
```



---

archive/issue_events_027742.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2012-06-26T10:30:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "milestone": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10764#event-27742"
}
```



---

archive/issue_comments_136093.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-07-02T15:21:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136093",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_027743.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-02T15:21:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10764#event-27743"
}
```



---

archive/issue_comments_136094.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.2.beta0\"",
    "created_at": "2012-07-02T15:21:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10764#issuecomment-136094",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.2.beta0"
