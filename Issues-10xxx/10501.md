# Issue 10501: Deprecate adjoint in favor of adjugate

archive/issues_010448.json:
```json
{
    "body": "Matrix methods named `adjoint` and `_adjoint` are renamed `adjugate` and `_adjugate` and replacements are added that raise deprecation warnings.\n\nThis is part of the program at #10465.  \n\nAssignee: jason, was\n\nCC:  @williamstein mvngu kohel @tornaria @orlitzky\n\nKeywords: notation, linear algebra, adjugate, matrices, determinants\n\nBranch/Commit: bf44252756d2f663acc479c1a345c1913716a1f6\n\nReviewer: Darij Grinberg\n\nAuthor: Kwankyu Lee\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/10501\n\n",
    "closed_at": "2019-02-04T23:01:10Z",
    "created_at": "2010-12-20T05:19:46Z",
    "labels": [
        "component: linear algebra",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.7",
    "title": "Deprecate adjoint in favor of adjugate",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10501",
    "user": "https://github.com/rbeezer"
}
```
Matrix methods named `adjoint` and `_adjoint` are renamed `adjugate` and `_adjugate` and replacements are added that raise deprecation warnings.

This is part of the program at #10465.  

Assignee: jason, was

CC:  @williamstein mvngu kohel @tornaria @orlitzky

Keywords: notation, linear algebra, adjugate, matrices, determinants

Branch/Commit: bf44252756d2f663acc479c1a345c1913716a1f6

Reviewer: Darij Grinberg

Author: Kwankyu Lee

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/10501





---

archive/issue_comments_130066.json:
```json
{
    "body": "Attachment [trac_10501-deprecate-adjoint.patch](tarball://root/attachments/some-uuid/ticket10501/trac_10501-deprecate-adjoint.patch) by @rbeezer created at 2010-12-20 22:52:31",
    "created_at": "2010-12-20T22:52:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130066",
    "user": "https://github.com/rbeezer"
}
```

Attachment [trac_10501-deprecate-adjoint.patch](tarball://root/attachments/some-uuid/ticket10501/trac_10501-deprecate-adjoint.patch) by @rbeezer created at 2010-12-20 22:52:31



---

archive/issue_comments_130067.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-12-20T22:57:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130067",
    "user": "https://github.com/rbeezer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_130068.json:
```json
{
    "body": "<a id='comment:1'></a>\nThree files caused doctest errors on a full run with only the necessary changes in sage/matrix.  I've made changes in these other places to fix those failures, and the affected files now pass their tests.  I'm running the full suite right now.\n\nI've cc'ed folks who I think might be able to double-check that no complications have crept in.  If you want to sneak a quick look at the patch, here's a quick guide:\n\nMinh, David:  sage/crypto/classical.py,  inverse_key() for a Hill Cryptosystem\n\nGonzalo:  sage/quadratic_forms/quadratic_form_ternary_Tornaria.py,  adjoints of a form\n\nWilliam:  sage/quadratic_forms/quadratic_form.py,  adjoint_primitive()",
    "created_at": "2010-12-20T22:57:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130068",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:1'></a>
Three files caused doctest errors on a full run with only the necessary changes in sage/matrix.  I've made changes in these other places to fix those failures, and the affected files now pass their tests.  I'm running the full suite right now.

I've cc'ed folks who I think might be able to double-check that no complications have crept in.  If you want to sneak a quick look at the patch, here's a quick guide:

Minh, David:  sage/crypto/classical.py,  inverse_key() for a Hill Cryptosystem

Gonzalo:  sage/quadratic_forms/quadratic_form_ternary_Tornaria.py,  adjoints of a form

William:  sage/quadratic_forms/quadratic_form.py,  adjoint_primitive()



---

archive/issue_comments_130069.json:
```json
{
    "body": "<a id='comment:2'></a>\nI already stated some objections to this on the mailing list, but I'll repeat:\n\nOn deprecating \"adjoint\" meaning \"matrix of cofactors\"\n1. it's standard terminology and has meant this in sage for long\n2. \"adjugate\" is newer and (IMO) less standard terminology -- in particular it has no obvious translations\n\nOn using \"adjoint\" meaning \"conjugate transpose\"\n3. \"conjugate transpose\" is easy to say, and it's really what is meant\n4. the \"adjoint operator\" for a matrix seems ill-defined, because a matrix is not an operator but only a representation of an operator in some basis.\n\nMoreover, if there are two colliding usages of the name \"adjoint\", I would find it more reasonable to keep the usage that is already traditional in Sage.\n\nThe usage of \"adjoint\" is ubiquitous in relation to quadratic forms afaict (and, as John Cremona pointed out, is where the term originates with Gauss on ternary quadratic forms)\n\n---\n\nReference for \"Adjoint of a matrix\":\n\nBourbaki, Elements, book 2, chapter III, section 11, exercise 9:\n\n  The adjoint of a square matrix X of order n over A is the matrix X = (det (A'\")) of minors of A\" of order n \u2014 1.\n\n(Note that the term also shows at the index of terminology of the book)\n\nPS: searching for\n\n \"The adjoint of a square matrix\" bourbaki\n\nin books.google.com, yields the above passage.",
    "created_at": "2010-12-20T23:42:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130069",
    "user": "https://github.com/tornaria"
}
```

<a id='comment:2'></a>
I already stated some objections to this on the mailing list, but I'll repeat:

On deprecating "adjoint" meaning "matrix of cofactors"
1. it's standard terminology and has meant this in sage for long
2. "adjugate" is newer and (IMO) less standard terminology -- in particular it has no obvious translations

On using "adjoint" meaning "conjugate transpose"
3. "conjugate transpose" is easy to say, and it's really what is meant
4. the "adjoint operator" for a matrix seems ill-defined, because a matrix is not an operator but only a representation of an operator in some basis.

Moreover, if there are two colliding usages of the name "adjoint", I would find it more reasonable to keep the usage that is already traditional in Sage.

The usage of "adjoint" is ubiquitous in relation to quadratic forms afaict (and, as John Cremona pointed out, is where the term originates with Gauss on ternary quadratic forms)

---

Reference for "Adjoint of a matrix":

Bourbaki, Elements, book 2, chapter III, section 11, exercise 9:

  The adjoint of a square matrix X of order n over A is the matrix X = (det (A'")) of minors of A" of order n — 1.

(Note that the term also shows at the index of terminology of the book)

PS: searching for

 "The adjoint of a square matrix" bourbaki

in books.google.com, yields the above passage.



---

archive/issue_comments_130070.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2010-12-20T23:42:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130070",
    "user": "https://github.com/tornaria"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_130071.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [tornaria](#comment%3A2):\n\nHi Gonzalo,\n\nI certainly read your postings to the mailing list carefully and appreciated the points you raised.  However, I had not realized you were so opposed to the change.\n\nAfter some discussion, I asked 'Is there any objection to deprecating the current .adjoint() function (which returns a matrix of cofactors) and renaming it as the \"adjugate\"?'\n\nIt was not meant to be an official vote, but I got +1 replies from Grout, Cremona, Loeffler and Stein.  Dima P and Karl Crisman had earlier voiced support.  There were no objections stated once I asked the question carefully.  So I have been proceeding on the assumption that there was strong support.\n\nI do not believe I changed any of the names of the commands for quadratic forms, though I can see that causing confusion if the adjoint of a matrix becomes the conjugate transpose.\n\nI have written a patch (#10471) with the `conjugate_transpose()`, which I find a really clumsy command, but workable in the interim.  William has suggested a more general `adjoint` function, which I would need to think about some more, but maybe that does not help with any of your objections (sounds like maybe that is worse in your view).\n\nI have twice now taught a \"matrix analysis\" course and it seems to me that adjoint gets used regularly (but maybe not consistently) for the conjugate transpose.  I am in the middle of making a major push to add significant amount of Sage code to my introductory linear algebra text, which is going very nicely.  But I need to also fix my \"complex inner product\" since I defined it with the conjugation on the \"wrong\" half.  So I would really like to keep Sage, my text, and the word \"adjoint\" all consistent with each other when I get to that point in a few weeks.\n\nDo you have some suggestions for a way forward?\n\nThanks,\nRob",
    "created_at": "2010-12-21T00:52:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130071",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:3'></a>
Replying to [tornaria](#comment%3A2):

Hi Gonzalo,

I certainly read your postings to the mailing list carefully and appreciated the points you raised.  However, I had not realized you were so opposed to the change.

After some discussion, I asked 'Is there any objection to deprecating the current .adjoint() function (which returns a matrix of cofactors) and renaming it as the "adjugate"?'

It was not meant to be an official vote, but I got +1 replies from Grout, Cremona, Loeffler and Stein.  Dima P and Karl Crisman had earlier voiced support.  There were no objections stated once I asked the question carefully.  So I have been proceeding on the assumption that there was strong support.

I do not believe I changed any of the names of the commands for quadratic forms, though I can see that causing confusion if the adjoint of a matrix becomes the conjugate transpose.

I have written a patch (#10471) with the `conjugate_transpose()`, which I find a really clumsy command, but workable in the interim.  William has suggested a more general `adjoint` function, which I would need to think about some more, but maybe that does not help with any of your objections (sounds like maybe that is worse in your view).

I have twice now taught a "matrix analysis" course and it seems to me that adjoint gets used regularly (but maybe not consistently) for the conjugate transpose.  I am in the middle of making a major push to add significant amount of Sage code to my introductory linear algebra text, which is going very nicely.  But I need to also fix my "complex inner product" since I defined it with the conjugation on the "wrong" half.  So I would really like to keep Sage, my text, and the word "adjoint" all consistent with each other when I get to that point in a few weeks.

Do you have some suggestions for a way forward?

Thanks,
Rob



---

archive/issue_comments_130072.json:
```json
{
    "body": "<a id='comment:4'></a>\n+1 from me. I hit this today, and just checked a handful of books:\n\n* Atkinson, An Introduction to Numerical Analysis, 1989. Section 7.1.\n* Axler, Linear Algebra Done Right, 1997. Ch. 6.\n* Marcus & Minc, Introduction to Linear Algebra, 1988. Section 1.4.\n* Meyer, Matrix Analysis and Applied Linear Algebra, 2000. Section 3.2.\n* Rudin, Functional Analysis, 1991. Chapter 4.\n* Shilov, Linear Algebra, 1977. Section 7.6.\n\nAll of which use the \"new\" meaning. In the interest of fairness, I also found,\n\n* Edwards, Elementary Linear Algebra, 2000. Section 3.4.\n\nWhich uses the cofactor definition.",
    "created_at": "2012-12-03T17:56:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130072",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:4'></a>
+1 from me. I hit this today, and just checked a handful of books:

* Atkinson, An Introduction to Numerical Analysis, 1989. Section 7.1.
* Axler, Linear Algebra Done Right, 1997. Ch. 6.
* Marcus & Minc, Introduction to Linear Algebra, 1988. Section 1.4.
* Meyer, Matrix Analysis and Applied Linear Algebra, 2000. Section 3.2.
* Rudin, Functional Analysis, 1991. Chapter 4.
* Shilov, Linear Algebra, 1977. Section 7.6.

All of which use the "new" meaning. In the interest of fairness, I also found,

* Edwards, Elementary Linear Algebra, 2000. Section 3.4.

Which uses the cofactor definition.



---

archive/issue_comments_130073.json:
```json
{
    "body": "<a id='comment:5'></a>\nHmm. Given two completely different uses of the word \"adjoint\" in this situation, I wonder if the right solution is to avoid it completely (with a deprecation warning for a while). If we use the \"new\" meaning, there will still be people who type `A.adjoint()` expecting the old meaning, and vice versa. Something like `A.conjugate_transpose()` can be found by tab completion; is that good enough? Is `A.adjugate()` the right name for the other version?",
    "created_at": "2012-12-03T22:31:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130073",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>
Hmm. Given two completely different uses of the word "adjoint" in this situation, I wonder if the right solution is to avoid it completely (with a deprecation warning for a while). If we use the "new" meaning, there will still be people who type `A.adjoint()` expecting the old meaning, and vice versa. Something like `A.conjugate_transpose()` can be found by tab completion; is that good enough? Is `A.adjugate()` the right name for the other version?



---

archive/issue_comments_130074.json:
```json
{
    "body": "<a id='comment:6'></a>\nReplying to [jhpalmieri](#comment%3A5):\n> Hmm. Given two completely different uses of the word \"adjoint\" in this situation, I wonder if the right solution is to avoid it completely (with a deprecation warning for a while). If we use the \"new\" meaning, there will still be people who type `A.adjoint()` expecting the old meaning, and vice versa. Something like `A.conjugate_transpose()` can be found by tab completion; is that good enough? Is `A.adjugate()` the right name for the other version?\n\n\n\n\nDid someone seriously implement `m.conjugate_transpose()` as a shortcut for `m.conjugate().transpose()`? =)\n\nI never thought to look for another method, I just did the operations individually.\n\nFrom what I understand, the terms \"adjoint\" and \"adjunct\" come from higher algebra, most of which is over my head. If that's the case, books written after e.g. category theory became popular will probably gravitate towards the new terminology. Although it does suck to have to deprecate `adjoint`, give it a new name, and then give something else the old name.\n\nMost of us have access to math departments; maybe we could do a survey of people working in linear algebra? If the result is overwhelming, rename it.",
    "created_at": "2012-12-04T02:03:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130074",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:6'></a>
Replying to [jhpalmieri](#comment%3A5):
> Hmm. Given two completely different uses of the word "adjoint" in this situation, I wonder if the right solution is to avoid it completely (with a deprecation warning for a while). If we use the "new" meaning, there will still be people who type `A.adjoint()` expecting the old meaning, and vice versa. Something like `A.conjugate_transpose()` can be found by tab completion; is that good enough? Is `A.adjugate()` the right name for the other version?




Did someone seriously implement `m.conjugate_transpose()` as a shortcut for `m.conjugate().transpose()`? =)

I never thought to look for another method, I just did the operations individually.

From what I understand, the terms "adjoint" and "adjunct" come from higher algebra, most of which is over my head. If that's the case, books written after e.g. category theory became popular will probably gravitate towards the new terminology. Although it does suck to have to deprecate `adjoint`, give it a new name, and then give something else the old name.

Most of us have access to math departments; maybe we could do a survey of people working in linear algebra? If the result is overwhelming, rename it.



---

archive/issue_comments_130075.json:
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [mjo](#comment%3A6):\n> Did someone seriously implement `m.conjugate_transpose()` as a shortcut for `m.conjugate().transpose()`? =)\n\n\nYep, that was me. ;-) But the BDFL suggested it.  Required reading:\n\nhttps://groups.google.com/forum/?fromgroups=#!topic/sage-devel/YjImMWVVwo4\n\nYou will see a lot of support for changes.  You'll see one conscientious objector.  I dropped it.  If someone else wants to carry the torch, I'll have their back.\n\nRob",
    "created_at": "2012-12-04T04:29:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130075",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:7'></a>
Replying to [mjo](#comment%3A6):
> Did someone seriously implement `m.conjugate_transpose()` as a shortcut for `m.conjugate().transpose()`? =)


Yep, that was me. ;-) But the BDFL suggested it.  Required reading:

https://groups.google.com/forum/?fromgroups=#!topic/sage-devel/YjImMWVVwo4

You will see a lot of support for changes.  You'll see one conscientious objector.  I dropped it.  If someone else wants to carry the torch, I'll have their back.

Rob



---

archive/issue_comments_130076.json:
```json
{
    "body": "<a id='comment:8'></a>\n> > Did someone seriously implement `m.conjugate_transpose()` as a shortcut for `m.conjugate().transpose()`? =)\n\n> \n\nIt's not as bad as you think, because tab-completion doesn't work on `m.conjugate()`, though it would be awesome if Sage could magically know that...\n\nRob, so what does the latest version of your book do?",
    "created_at": "2012-12-12T03:58:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130076",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:8'></a>
> > Did someone seriously implement `m.conjugate_transpose()` as a shortcut for `m.conjugate().transpose()`? =)

> 

It's not as bad as you think, because tab-completion doesn't work on `m.conjugate()`, though it would be awesome if Sage could magically know that...

Rob, so what does the latest version of your book do?



---

archive/issue_comments_130077.json:
```json
{
    "body": "<a id='comment:9'></a>\nReplying to [kcrisman](#comment%3A8):\n> Rob, so what does the latest version of your book do?\n\n\nconjugate-transpose has always been called \"adjoint,\" in line with my experience teaching numerical linear algebra.  I even have my inner product conjugating the correct vector now.  ;-)\n\nSee: http://linear.ups.edu/html/section-MO.html#subsection-AM\n\nI almost never have need to reference the matrix of cofactors (proposed as adjugate here), but do use it one exercise about building a matrix inverse this way.\n\nSee:  Exercise PDM.T20 in http://linear.ups.edu/html/section-PDM.html\n\nRob",
    "created_at": "2012-12-12T17:31:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130077",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:9'></a>
Replying to [kcrisman](#comment%3A8):
> Rob, so what does the latest version of your book do?


conjugate-transpose has always been called "adjoint," in line with my experience teaching numerical linear algebra.  I even have my inner product conjugating the correct vector now.  ;-)

See: http://linear.ups.edu/html/section-MO.html#subsection-AM

I almost never have need to reference the matrix of cofactors (proposed as adjugate here), but do use it one exercise about building a matrix inverse this way.

See:  Exercise PDM.T20 in http://linear.ups.edu/html/section-PDM.html

Rob



---

archive/issue_events_026917.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26917"
}
```



---

archive/issue_events_026918.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26918"
}
```



---

archive/issue_events_026919.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26919"
}
```



---

archive/issue_events_026920.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26920"
}
```



---

archive/issue_events_026921.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26921"
}
```



---

archive/issue_events_026922.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26922"
}
```



---

archive/issue_events_026923.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26923"
}
```



---

archive/issue_events_026924.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2018-10-05T10:23:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26924"
}
```



---

archive/issue_events_026925.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2018-10-05T10:23:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26925"
}
```



---

archive/issue_comments_130078.json:
```json
{
    "body": "<a id='comment:14'></a>\nI want to revive this ticket. So here is the needed patch.\n\nOne thing not found in Rob's original patch is alias `adjoint_classical` of `adjugate`. The alias is used in quadratic form code in Sage.\n\n---\nNew commits:\n|                                                                                                                                          |                                                      |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------|\n|[0669f64](https://git.sagemath.org/sage.git/commit?id=0669f64d384d8b42838d590a41a6bf9fcb115979)|`Deprecate adjoint for adjugate and adjoint_classical`|",
    "created_at": "2018-10-05T10:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130078",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:14'></a>
I want to revive this ticket. So here is the needed patch.

One thing not found in Rob's original patch is alias `adjoint_classical` of `adjugate`. The alias is used in quadratic form code in Sage.

---
New commits:
|                                                                                                                                          |                                                      |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------|
|[0669f64](https://git.sagemath.org/sage.git/commit?id=0669f64d384d8b42838d590a41a6bf9fcb115979)|`Deprecate adjoint for adjugate and adjoint_classical`|



---

archive/issue_comments_130079.json:
```json
{
    "body": "Changing branch from \"\" to \"u/klee/10501\"",
    "created_at": "2018-10-05T10:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130079",
    "user": "https://github.com/kwankyu"
}
```

Changing branch from "" to "u/klee/10501"



---

archive/issue_comments_130080.json:
```json
{
    "body": "Changing author from \"\" to \"Kwankyu Lee\"",
    "created_at": "2018-10-05T10:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130080",
    "user": "https://github.com/kwankyu"
}
```

Changing author from "" to "Kwankyu Lee"



---

archive/issue_comments_130081.json:
```json
{
    "body": "Changing priority from major to minor.",
    "created_at": "2018-10-05T10:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130081",
    "user": "https://github.com/kwankyu"
}
```

Changing priority from major to minor.



---

archive/issue_comments_130082.json:
```json
{
    "body": "Changing commit from \"\" to \"0669f64d384d8b42838d590a41a6bf9fcb115979\"",
    "created_at": "2018-10-05T10:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130082",
    "user": "https://github.com/kwankyu"
}
```

Changing commit from "" to "0669f64d384d8b42838d590a41a6bf9fcb115979"



---

archive/issue_comments_130083.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-10-05T10:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130083",
    "user": "https://github.com/kwankyu"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_130084.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Matrix methods named `adjoint` and `_adjoint` are renamed `adjugate` and `_adjugate` and replacements are added that raise deprecation warnings.\n \n-This is part of the program at #10465.  Patch coming once tests conclude.\n+This is part of the program at #10465.  \n``````\n",
    "created_at": "2018-10-05T10:25:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130084",
    "user": "https://github.com/kwankyu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Matrix methods named `adjoint` and `_adjoint` are renamed `adjugate` and `_adjugate` and replacements are added that raise deprecation warnings.
 
-This is part of the program at #10465.  Patch coming once tests conclude.
+This is part of the program at #10465.  
``````




---

archive/issue_comments_130085.json:
```json
{
    "body": "<a id='comment:16'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                        |\n|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------|\n|[a61f618](https://git.sagemath.org/sage.git/commit/?id=a61f6184ab68ab69e94c62f0b2cda62ccac4b671)|`Merge branch 'develop'`|\n|[1a2f6a0](https://git.sagemath.org/sage.git/commit/?id=1a2f6a003c03ace089ac324b91c9ba11392974b0)|`properly deprecate adjoint`|\n|[43ab6df](https://git.sagemath.org/sage.git/commit/?id=43ab6df581d54a2d627c030443a9320c6a7dee8e)|`pyflakes check and fix exception messages style`|",
    "created_at": "2018-10-12T06:33:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130085",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:16'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                        |
|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------|
|[a61f618](https://git.sagemath.org/sage.git/commit/?id=a61f6184ab68ab69e94c62f0b2cda62ccac4b671)|`Merge branch 'develop'`|
|[1a2f6a0](https://git.sagemath.org/sage.git/commit/?id=1a2f6a003c03ace089ac324b91c9ba11392974b0)|`properly deprecate adjoint`|
|[43ab6df](https://git.sagemath.org/sage.git/commit/?id=43ab6df581d54a2d627c030443a9320c6a7dee8e)|`pyflakes check and fix exception messages style`|



---

archive/issue_comments_130086.json:
```json
{
    "body": "Changing commit from \"0669f64d384d8b42838d590a41a6bf9fcb115979\" to \"43ab6df581d54a2d627c030443a9320c6a7dee8e\"",
    "created_at": "2018-10-12T06:33:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130086",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "0669f64d384d8b42838d590a41a6bf9fcb115979" to "43ab6df581d54a2d627c030443a9320c6a7dee8e"



---

archive/issue_comments_130087.json:
```json
{
    "body": "Changing commit from \"43ab6df581d54a2d627c030443a9320c6a7dee8e\" to \"08eee094dd63965b8aadf3a38b2f3fe620910a9b\"",
    "created_at": "2018-10-12T07:07:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130087",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "43ab6df581d54a2d627c030443a9320c6a7dee8e" to "08eee094dd63965b8aadf3a38b2f3fe620910a9b"



---

archive/issue_comments_130088.json:
```json
{
    "body": "<a id='comment:17'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                 |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------|\n|[08eee09](https://git.sagemath.org/sage.git/commit/?id=08eee094dd63965b8aadf3a38b2f3fe620910a9b)|`import deprecated_function_alias on global level to avoid crash`|",
    "created_at": "2018-10-12T07:07:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130088",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:17'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                 |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------|
|[08eee09](https://git.sagemath.org/sage.git/commit/?id=08eee094dd63965b8aadf3a38b2f3fe620910a9b)|`import deprecated_function_alias on global level to avoid crash`|



---

archive/issue_comments_130089.json:
```json
{
    "body": "Changing commit from \"08eee094dd63965b8aadf3a38b2f3fe620910a9b\" to \"d1bbfb1bd29c9cfb688d24d9a71e92ce98148513\"",
    "created_at": "2018-10-13T02:59:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130089",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "08eee094dd63965b8aadf3a38b2f3fe620910a9b" to "d1bbfb1bd29c9cfb688d24d9a71e92ce98148513"



---

archive/issue_comments_130090.json:
```json
{
    "body": "<a id='comment:18'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                 |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|\n|[d1bbfb1](https://git.sagemath.org/sage.git/commit/?id=d1bbfb1bd29c9cfb688d24d9a71e92ce98148513)|`change more adjoint to adjugate`|",
    "created_at": "2018-10-13T02:59:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130090",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:18'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                 |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|
|[d1bbfb1](https://git.sagemath.org/sage.git/commit/?id=d1bbfb1bd29c9cfb688d24d9a71e92ce98148513)|`change more adjoint to adjugate`|



---

archive/issue_comments_130091.json:
```json
{
    "body": "<a id='comment:19'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                |\n|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------|\n|[2f15975](https://git.sagemath.org/sage.git/commit/?id=2f1597566c868739e748599aa5e8b2c2eb59775e)|`fix neglected doctest failures`|",
    "created_at": "2018-10-16T04:31:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130091",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:19'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                |
|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------|
|[2f15975](https://git.sagemath.org/sage.git/commit/?id=2f1597566c868739e748599aa5e8b2c2eb59775e)|`fix neglected doctest failures`|



---

archive/issue_comments_130092.json:
```json
{
    "body": "Changing commit from \"d1bbfb1bd29c9cfb688d24d9a71e92ce98148513\" to \"2f1597566c868739e748599aa5e8b2c2eb59775e\"",
    "created_at": "2018-10-16T04:31:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130092",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "d1bbfb1bd29c9cfb688d24d9a71e92ce98148513" to "2f1597566c868739e748599aa5e8b2c2eb59775e"



---

archive/issue_events_026926.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2018-10-29T00:41:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26926"
}
```



---

archive/issue_events_026927.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2018-10-29T00:41:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26927"
}
```



---

archive/issue_comments_130093.json:
```json
{
    "body": "<a id='comment:21'></a>\nI wholeheartedly agree with \"adjugate\". When I see \"adjoint\", I look up the definition. When I see \"adjugate\", I immediately know what is meant.\n\nPushed a little commit to improve the documentation. IMHO, this is an easy ticket to review: just run all doctests. If they work, then it's fine.\n\n---\nNew commits:\n|                                                                                                                                          |                                                                      |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------|\n|[64b9491](https://git.sagemath.org/sage.git/commit?id=64b9491bb88a5476c85fed35bd7b544c2e3d16d6)|`Merge branch 'u/klee/10501' of git://trac.sagemath.org/sage into adj`|\n|[b4994c4](https://git.sagemath.org/sage.git/commit?id=b4994c43854ebbece7186b7b7b5cd132873f9939)|`actually define the adjugate in the doc`|",
    "created_at": "2019-01-16T17:59:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130093",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:21'></a>
I wholeheartedly agree with "adjugate". When I see "adjoint", I look up the definition. When I see "adjugate", I immediately know what is meant.

Pushed a little commit to improve the documentation. IMHO, this is an easy ticket to review: just run all doctests. If they work, then it's fine.

---
New commits:
|                                                                                                                                          |                                                                      |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------|
|[64b9491](https://git.sagemath.org/sage.git/commit?id=64b9491bb88a5476c85fed35bd7b544c2e3d16d6)|`Merge branch 'u/klee/10501' of git://trac.sagemath.org/sage into adj`|
|[b4994c4](https://git.sagemath.org/sage.git/commit?id=b4994c43854ebbece7186b7b7b5cd132873f9939)|`actually define the adjugate in the doc`|



---

archive/issue_comments_130094.json:
```json
{
    "body": "Changing branch from \"u/klee/10501\" to \"public/ticket/10501\"",
    "created_at": "2019-01-16T17:59:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130094",
    "user": "https://github.com/darijgr"
}
```

Changing branch from "u/klee/10501" to "public/ticket/10501"



---

archive/issue_comments_130095.json:
```json
{
    "body": "Changing commit from \"2f1597566c868739e748599aa5e8b2c2eb59775e\" to \"b4994c43854ebbece7186b7b7b5cd132873f9939\"",
    "created_at": "2019-01-16T17:59:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130095",
    "user": "https://github.com/darijgr"
}
```

Changing commit from "2f1597566c868739e748599aa5e8b2c2eb59775e" to "b4994c43854ebbece7186b7b7b5cd132873f9939"



---

archive/issue_comments_130096.json:
```json
{
    "body": "Changing keywords from \"\" to \"notation, linear algebra, adjugate, matrices, determinants\".",
    "created_at": "2019-01-16T17:59:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130096",
    "user": "https://github.com/darijgr"
}
```

Changing keywords from "" to "notation, linear algebra, adjugate, matrices, determinants".



---

archive/issue_comments_130097.json:
```json
{
    "body": "<a id='comment:22'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                                                      |\n|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------|\n|[356285d](https://git.sagemath.org/sage.git/commit/?id=356285df8e0ede0f6d2ccc02559744004528b744)|`Deprecate adjoint for adjugate and adjoint_classical`|",
    "created_at": "2019-01-17T04:45:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130097",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:22'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                                                      |
|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------|
|[356285d](https://git.sagemath.org/sage.git/commit/?id=356285df8e0ede0f6d2ccc02559744004528b744)|`Deprecate adjoint for adjugate and adjoint_classical`|



---

archive/issue_comments_130098.json:
```json
{
    "body": "Changing commit from \"b4994c43854ebbece7186b7b7b5cd132873f9939\" to \"356285df8e0ede0f6d2ccc02559744004528b744\"",
    "created_at": "2019-01-17T04:45:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130098",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "b4994c43854ebbece7186b7b7b5cd132873f9939" to "356285df8e0ede0f6d2ccc02559744004528b744"



---

archive/issue_comments_130099.json:
```json
{
    "body": "<a id='comment:23'></a>\nSquashed and rebased to sage 8.6. In addition, refined some docstrings and comments.",
    "created_at": "2019-01-17T04:49:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130099",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:23'></a>
Squashed and rebased to sage 8.6. In addition, refined some docstrings and comments.



---

archive/issue_comments_130100.json:
```json
{
    "body": "<a id='comment:24'></a>\nIf klee and darij are both agreed that each other's respective contributions are sufficient, then you can jointly set to positive review (sometimes Authors are 1, 2 and Reviewers are 2, 1).\n\nI do wonder whether some of the py3 things like\n\n```diff\n-                raise TypeError(\"Oops!  The matrix must have \" + str(n) + \" rows. =(\")\n+                raise TypeError(\"the matrix must have {} rows\".format(n))\n```\nare necessary on this ticket, since it makes it a little more of a patch bomb.\n\nAlso, don't we typically test deprecations (and then remove when deprecation is done)?",
    "created_at": "2019-01-17T12:43:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130100",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:24'></a>
If klee and darij are both agreed that each other's respective contributions are sufficient, then you can jointly set to positive review (sometimes Authors are 1, 2 and Reviewers are 2, 1).

I do wonder whether some of the py3 things like

```diff
-                raise TypeError("Oops!  The matrix must have " + str(n) + " rows. =(")
+                raise TypeError("the matrix must have {} rows".format(n))
```
are necessary on this ticket, since it makes it a little more of a patch bomb.

Also, don't we typically test deprecations (and then remove when deprecation is done)?



---

archive/issue_comments_130101.json:
```json
{
    "body": "<a id='comment:25'></a>\nReplying to [kcrisman](#comment%3A24):\n> If klee and darij are both agreed that each other's respective contributions are sufficient, then you can jointly set to positive review (sometimes Authors are 1, 2 and Reviewers are 2, 1).\n\n\nPositive review on his part of the code. It is up to him to put his name to the Author field.\n\n> I do wonder whether some of the py3 things like\n> \n> ```diff\n> -                raise TypeError(\"Oops!  The matrix must have \" + str(n) + \" rows. =(\")\n> +                raise TypeError(\"the matrix must have {} rows\".format(n))\n> ```\n> are necessary on this ticket, since it makes it a little more of a patch bomb.\n\n\nNot necessary. But I think we don't need to freak away from making small improvements unrelated with the main issue of the ticket. Do we?\n\nOr do you regard the small changes as no improvements?  \n\n> Also, don't we typically test deprecations (and then remove when deprecation is done)?\n\n\nI didn't know. I don't know.",
    "created_at": "2019-01-17T14:16:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130101",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:25'></a>
Replying to [kcrisman](#comment%3A24):
> If klee and darij are both agreed that each other's respective contributions are sufficient, then you can jointly set to positive review (sometimes Authors are 1, 2 and Reviewers are 2, 1).


Positive review on his part of the code. It is up to him to put his name to the Author field.

> I do wonder whether some of the py3 things like
> 
> ```diff
> -                raise TypeError("Oops!  The matrix must have " + str(n) + " rows. =(")
> +                raise TypeError("the matrix must have {} rows".format(n))
> ```
> are necessary on this ticket, since it makes it a little more of a patch bomb.


Not necessary. But I think we don't need to freak away from making small improvements unrelated with the main issue of the ticket. Do we?

Or do you regard the small changes as no improvements?  

> Also, don't we typically test deprecations (and then remove when deprecation is done)?


I didn't know. I don't know.



---

archive/issue_comments_130102.json:
```json
{
    "body": "<a id='comment:26'></a>\n> Or do you regard the small changes as no improvements?  \n\n\nNo, it's just that it causes more opportunities for clashes with other tickets.\n\n> \n> > Also, don't we typically test deprecations (and then remove when deprecation is done)?\n\n> \n> I didn't know. I don't know. \n\n\nFor one of *many* examples, see (at least right now correct link) [this spot](https://github.com/sagemath/sage/blob/master/src/sage/tensor/coordinate_patch.py#L58)\n\n```\n            sage: x, y, z = var('x, y, z')\n            sage: S = CoordinatePatch((x, y, z)); S\n            doctest:...: DeprecationWarning: Use Manifold instead.\n            See http://trac.sagemath.org/24444 for details.\n            Open subset of R^3 with coordinates x, y, z\n```\nI guess this should be done for positive review.",
    "created_at": "2019-01-17T22:10:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130102",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:26'></a>
> Or do you regard the small changes as no improvements?  


No, it's just that it causes more opportunities for clashes with other tickets.

> 
> > Also, don't we typically test deprecations (and then remove when deprecation is done)?

> 
> I didn't know. I don't know. 


For one of *many* examples, see (at least right now correct link) [this spot](https://github.com/sagemath/sage/blob/master/src/sage/tensor/coordinate_patch.py#L58)

```
            sage: x, y, z = var('x, y, z')
            sage: S = CoordinatePatch((x, y, z)); S
            doctest:...: DeprecationWarning: Use Manifold instead.
            See http://trac.sagemath.org/24444 for details.
            Open subset of R^3 with coordinates x, y, z
```
I guess this should be done for positive review.



---

archive/issue_comments_130103.json:
```json
{
    "body": "<a id='comment:27'></a>\nSorry, I don't have time for this :/\n\nI could reread the rebased branch once the stress from the semester start has subsided, but I'm generally extremely short on time until September or so(?). Sorry.",
    "created_at": "2019-01-17T22:13:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130103",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:27'></a>
Sorry, I don't have time for this :/

I could reread the rebased branch once the stress from the semester start has subsided, but I'm generally extremely short on time until September or so(?). Sorry.



---

archive/issue_comments_130104.json:
```json
{
    "body": "<a id='comment:28'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                            |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|\n|[bf44252](https://git.sagemath.org/sage.git/commit/?id=bf44252756d2f663acc479c1a345c1913716a1f6)|`Add a test for deprecation`|",
    "created_at": "2019-01-18T02:20:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130104",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:28'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                            |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|
|[bf44252](https://git.sagemath.org/sage.git/commit/?id=bf44252756d2f663acc479c1a345c1913716a1f6)|`Add a test for deprecation`|



---

archive/issue_comments_130105.json:
```json
{
    "body": "Changing commit from \"356285df8e0ede0f6d2ccc02559744004528b744\" to \"bf44252756d2f663acc479c1a345c1913716a1f6\"",
    "created_at": "2019-01-18T02:20:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130105",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "356285df8e0ede0f6d2ccc02559744004528b744" to "bf44252756d2f663acc479c1a345c1913716a1f6"



---

archive/issue_comments_130106.json:
```json
{
    "body": "<a id='comment:29'></a>\nAnything else to do?",
    "created_at": "2019-01-22T11:57:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130106",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:29'></a>
Anything else to do?



---

archive/issue_events_026928.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2019-01-22T12:18:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26928"
}
```



---

archive/issue_events_026929.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2019-01-22T12:18:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26929"
}
```



---

archive/issue_comments_130107.json:
```json
{
    "body": "<a id='comment:30'></a>\nThe new version looks good to me. If complete doctests pass (anyone please check), please make this a positive_review.\n\n(I should not be listed as author; my changes were trivial.)",
    "created_at": "2019-01-22T12:18:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130107",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:30'></a>
The new version looks good to me. If complete doctests pass (anyone please check), please make this a positive_review.

(I should not be listed as author; my changes were trivial.)



---

archive/issue_comments_130108.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Darij Grinberg\"",
    "created_at": "2019-01-22T12:18:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130108",
    "user": "https://github.com/darijgr"
}
```

Changing reviewer from "" to "Darij Grinberg"



---

archive/issue_comments_130109.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-01-29T00:47:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130109",
    "user": "https://github.com/kwankyu"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_130110.json:
```json
{
    "body": "Changing branch from \"public/ticket/10501\" to \"bf44252756d2f663acc479c1a345c1913716a1f6\"",
    "created_at": "2019-02-04T23:01:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130110",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/ticket/10501" to "bf44252756d2f663acc479c1a345c1913716a1f6"



---

archive/issue_events_026930.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-02-04T23:01:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10501#event-26930"
}
```



---

archive/issue_comments_130111.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-02-04T23:01:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10501",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10501#issuecomment-130111",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
