# Issue 10121: cleaned lapack spkg metadata and added hgignore

archive/issues_010120.json:
```json
{
    "assignees": [],
    "body": "hgignore is  missing and and hg repo not clean, preventing working with hg queues. As well, SPKG out of date.\n\nAll this is fixed in \nhttp://boxen.math.washington.edu/home/dima/packages/lapack-20071123.p2.spkg\n \nReviewing this should be really trivial...\n\nCC:  @jasongrout\n\nAuthor: **Dmitrii Pasechnik**\n\nReviewer: **David Kirkby**\n\nMerged: **sage-4.6.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10121_\n\n",
    "closed_at": "2010-11-01T10:15:58Z",
    "created_at": "2010-10-12T04:17:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "cleaned lapack spkg metadata and added hgignore",
    "type": "issue",
    "updated_at": "2011-01-12T06:26:03Z",
    "url": "https://github.com/sagemath/sage/issues/10121",
    "user": "https://github.com/dimpase"
}
```
hgignore is  missing and and hg repo not clean, preventing working with hg queues. As well, SPKG out of date.

All this is fixed in 
http://boxen.math.washington.edu/home/dima/packages/lapack-20071123.p2.spkg
 
Reviewing this should be really trivial...

CC:  @jasongrout

Author: **Dmitrii Pasechnik**

Reviewer: **David Kirkby**

Merged: **sage-4.6.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/10121_





---

archive/issue_events_120746.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2010-10-12T04:17:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120746"
}
```



---

archive/issue_events_120747.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2010-10-12T04:17:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120747"
}
```



---

archive/issue_events_120748.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2010-10-12T04:17:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120748"
}
```



---

archive/issue_events_120749.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2010-10-12T04:17:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120749"
}
```



---

archive/issue_comments_095124.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI did not notice you had created this, but I made a ticket #10123 to update the package completely. The version in Sage is 3 years old. I should complete that update within the next 24 hours or so, and will of course add things like an `spkg-check` file, as the newer version has some self-tests. \n\nDave",
    "created_at": "2010-10-12T14:58:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10121#issuecomment-95124",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<a id='comment:2'>Comment 2:</a>
I did not notice you had created this, but I made a ticket #10123 to update the package completely. The version in Sage is 3 years old. I should complete that update within the next 24 hours or so, and will of course add things like an `spkg-check` file, as the newer version has some self-tests. 

Dave



---

archive/issue_comments_095125.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nHaving created a ticket to make a new version, I realise this is more complex than I thought, as the library names used by the new version are all different, so will take more work. \n\nThis looks good. Positive review.",
    "created_at": "2010-10-16T05:28:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10121#issuecomment-95125",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<a id='comment:3'>Comment 3:</a>
Having created a ticket to make a new version, I realise this is more complex than I thought, as the library names used by the new version are all different, so will take more work. 

This looks good. Positive review.



---

archive/issue_events_120750.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-10-16T05:28:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120750"
}
```



---

archive/issue_events_120751.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-10-16T05:34:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120751"
}
```



---

archive/issue_events_120752.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-10-16T05:34:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120752"
}
```



---

archive/issue_comments_095126.json:
```json
{
    "body": "Author: **Dima Pasechnik**",
    "created_at": "2010-10-16T05:34:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10121#issuecomment-95126",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Author: **Dima Pasechnik**



---

archive/issue_comments_095127.json:
```json
{
    "body": "Reviewer: **David Kirkby**",
    "created_at": "2010-10-16T05:34:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10121#issuecomment-95127",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Reviewer: **David Kirkby**



---

archive/issue_comments_095128.json:
```json
{
    "body": "Merged: **sage-4.6.1.alpha0**",
    "created_at": "2010-11-01T10:15:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10121#issuecomment-95128",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.1.alpha0**



---

archive/issue_events_120753.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-01T10:15:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120753"
}
```



---

archive/issue_events_120754.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-01T10:15:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120754"
}
```



---

archive/issue_events_120755.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-11-01T10:26:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120755"
}
```



---

archive/issue_events_120756.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-11-01T10:26:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10121#event-120756"
}
```



---

archive/issue_comments_095129.json:
```json
{
    "body": "Changed author from **Dima Pasechnik** to **Dmitrii Pasechnik**",
    "created_at": "2011-01-12T06:26:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10121#issuecomment-95129",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Dima Pasechnik** to **Dmitrii Pasechnik**
