# Issue 10626: Route sparse integer matrices to a working Smith form

archive/issues_010573.json:
```json
{
    "body": "Sparse integer matrices get sent to the generic Smith form routine, where they fail (see #10625).  They should instead get dense-ified\nand routed to the integer-only version.  This solves some failures on #10604.\n\n```\nsage: A = matrix(2, range(4), sparse=True)\nsage: A.smith_form()\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n\n/home/sage/sage-4.6.1.alpha2/devel/sage-main/<ipython console> in <module>()\n\n/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2.Matrix.smith_form (sage/matrix/matrix2.c:37477)()\n\n/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2._smith_onestep (sage/matrix/matrix2.c:42366)()\n\n/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2._smith_onestep (sage/matrix/matrix2.c:42181)()\n\n/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2._generic_clear_column (sage/matrix/matrix2.c:41049)()\n\n/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/rings/integer.so in sage.rings.integer.Integer.inverse_mod (sage/rings/integer.c:28740)()\n\n/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/rings/integer.so in sage.rings.integer.as_Integer (sage/rings/integer.c:6052)()\n\n/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/rings/integer.so in sage.rings.integer.Integer.__init__ (sage/rings/integer.c:7312)()\n\nTypeError: unable to coerce <class 'sage.rings.ideal.Ideal_pid'> to an integer\n```\n\nAssignee: jason, was\n\nCC:  @dandrake jthurber\n\nKeywords: Smith form\n\nResolution: fixed\n\nAuthor: Rob Beezer\n\nReviewer: David Loeffler\n\nMerged: sage-4.6.2.alpha3\n\nIssue created by migration from https://trac.sagemath.org/ticket/10626\n\n",
    "closed_at": "2011-01-27T09:14:00Z",
    "created_at": "2011-01-13T19:01:02Z",
    "labels": [
        "component: linear algebra",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.6.2",
    "title": "Route sparse integer matrices to a working Smith form",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10626",
    "user": "https://github.com/rbeezer"
}
```
Sparse integer matrices get sent to the generic Smith form routine, where they fail (see #10625).  They should instead get dense-ified
and routed to the integer-only version.  This solves some failures on #10604.

```
sage: A = matrix(2, range(4), sparse=True)
sage: A.smith_form()
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)

/home/sage/sage-4.6.1.alpha2/devel/sage-main/<ipython console> in <module>()

/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2.Matrix.smith_form (sage/matrix/matrix2.c:37477)()

/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2._smith_onestep (sage/matrix/matrix2.c:42366)()

/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2._smith_onestep (sage/matrix/matrix2.c:42181)()

/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2._generic_clear_column (sage/matrix/matrix2.c:41049)()

/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/rings/integer.so in sage.rings.integer.Integer.inverse_mod (sage/rings/integer.c:28740)()

/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/rings/integer.so in sage.rings.integer.as_Integer (sage/rings/integer.c:6052)()

/home/sage/sage-4.6.1.alpha2/local/lib/python2.6/site-packages/sage/rings/integer.so in sage.rings.integer.Integer.__init__ (sage/rings/integer.c:7312)()

TypeError: unable to coerce <class 'sage.rings.ideal.Ideal_pid'> to an integer
```

Assignee: jason, was

CC:  @dandrake jthurber

Keywords: Smith form

Resolution: fixed

Author: Rob Beezer

Reviewer: David Loeffler

Merged: sage-4.6.2.alpha3

Issue created by migration from https://trac.sagemath.org/ticket/10626





---

archive/issue_comments_133457.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-01-13T20:22:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10626#issuecomment-133457",
    "user": "https://github.com/rbeezer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_133458.json:
```json
{
    "body": "Changing author from \"\" to \"Rob Beezer\"",
    "created_at": "2011-01-13T20:22:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10626#issuecomment-133458",
    "user": "https://github.com/rbeezer"
}
```

Changing author from "" to "Rob Beezer"



---

archive/issue_comments_133459.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [trac_10626-smith-form-integer-sparse.patch](tarball://root/attachments/some-uuid/ticket10626/trac_10626-smith-form-integer-sparse.patch) by @rbeezer created at 2011-01-13 20:22:47\n\nPatch mirrors docstring for dense matrices and just ships off a dense version of the matrix to that code.  `matrix/matrix_integer_sparse.pyx` is not included into the documentation, so I've added that chore to my list.",
    "created_at": "2011-01-13T20:22:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10626#issuecomment-133459",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:1'></a>Attachment [trac_10626-smith-form-integer-sparse.patch](tarball://root/attachments/some-uuid/ticket10626/trac_10626-smith-form-integer-sparse.patch) by @rbeezer created at 2011-01-13 20:22:47

Patch mirrors docstring for dense matrices and just ships off a dense version of the matrix to that code.  `matrix/matrix_integer_sparse.pyx` is not included into the documentation, so I've added that chore to my list.



---

archive/issue_comments_133460.json:
```json
{
    "body": "<a id='comment:2'></a>I've just uploaded a patch at #10625, which makes the examples here work. That said, densifying a matrix and calculating Smith form of that using integer-specific routines is going to be much faster than calling a slow generic routine; so this patch should still be applied. Applies fine to 4.6.2.alpha0 and all doctests pass, so positive review.",
    "created_at": "2011-01-16T16:43:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10626#issuecomment-133460",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:2'></a>I've just uploaded a patch at #10625, which makes the examples here work. That said, densifying a matrix and calculating Smith form of that using integer-specific routines is going to be much faster than calling a slow generic routine; so this patch should still be applied. Applies fine to 4.6.2.alpha0 and all doctests pass, so positive review.



---

archive/issue_comments_133461.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-01-16T16:43:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10626#issuecomment-133461",
    "user": "https://github.com/loefflerd"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_133462.json:
```json
{
    "body": "Changing reviewer from \"\" to \"David Loeffler\"",
    "created_at": "2011-01-16T16:43:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10626#issuecomment-133462",
    "user": "https://github.com/loefflerd"
}
```

Changing reviewer from "" to "David Loeffler"



---

archive/issue_comments_133463.json:
```json
{
    "body": "Changing keywords from \"\" to \"Smith form\".",
    "created_at": "2011-01-16T16:43:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10626#issuecomment-133463",
    "user": "https://github.com/loefflerd"
}
```

Changing keywords from "" to "Smith form".



---

archive/issue_events_027355.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-27T09:14:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10626#event-27355"
}
```



---

archive/issue_comments_133464.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-01-27T09:14:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10626#issuecomment-133464",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_133465.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.6.2.alpha3\"",
    "created_at": "2011-01-27T09:14:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10626#issuecomment-133465",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.6.2.alpha3"
