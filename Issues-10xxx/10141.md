# Issue 10141: bug in printing trivial class groups (trivial to fix)

archive/issues_010140.json:
```json
{
    "assignees": [],
    "body": "Observe that the printing is wrong in the special case of class number 1:\n\n```\nsage: NumberField(x^2 + 1, 'i').class_group()\nClass group of order 1 with structure  of Number Field in i with defining polynomial x^2 + 1\nsage: \n```\n\nIn particular \"with structure  of Number Field ...\"  where there is something after \"structure\" missing. \n\nAssignee: **@loefflerd**\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10141_\n\n",
    "closed_at": "2011-10-09T10:51:25Z",
    "created_at": "2010-10-17T21:26:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bug in printing trivial class groups (trivial to fix)",
    "type": "issue",
    "updated_at": "2011-10-09T10:51:25Z",
    "url": "https://github.com/sagemath/sage/issues/10141",
    "user": "https://github.com/williamstein"
}
```
Observe that the printing is wrong in the special case of class number 1:

```
sage: NumberField(x^2 + 1, 'i').class_group()
Class group of order 1 with structure  of Number Field in i with defining polynomial x^2 + 1
sage: 
```

In particular "with structure  of Number Field ..."  where there is something after "structure" missing. 

Assignee: **@loefflerd**

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/10141_





---

archive/issue_events_121056.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-10-17T21:26:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10141#event-121056"
}
```



---

archive/issue_events_121057.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-10-17T21:26:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10141#event-121057"
}
```



---

archive/issue_events_121058.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-10-17T21:26:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10141#event-121058"
}
```



---

archive/issue_events_121059.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-10-17T21:27:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10141#event-121059"
}
```



---

archive/issue_events_121060.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-09T10:51:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10141#event-121060"
}
```



---

archive/issue_events_121061.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-09T10:51:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10141#event-121061"
}
```



---

archive/issue_events_121062.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-09T10:51:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10141#event-121062"
}
```



---

archive/issue_comments_095469.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2011-10-09T10:51:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10141#issuecomment-95469",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_121063.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-09T10:51:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10141#event-121063"
}
```



---

archive/issue_events_121064.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-09T10:51:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10141#event-121064"
}
```



---

archive/issue_comments_095470.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nFixed in sage-4.7.2.alpha3.",
    "created_at": "2011-10-09T10:51:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10141#issuecomment-95470",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

Fixed in sage-4.7.2.alpha3.
