# Issue 10511: Numbers with exponents mangled when sending to mathematica

archive/issues_010458.json:
```json
{
    "assignees": [],
    "body": "Oh dear:\n\n```\nsage: mathematica(1.e-4)\n0.0001\nsage: mathematica(1.e-5)\n0.00001\nsage: mathematica(1.e-6)\n-7 + 10.*e\n```\n\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10511_\n\n",
    "created_at": "2010-12-22T06:55:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Numbers with exponents mangled when sending to mathematica",
    "type": "issue",
    "updated_at": "2015-06-23T13:49:26Z",
    "url": "https://github.com/sagemath/sage/issues/10511",
    "user": "https://github.com/sagetrac-flawrence"
}
```
Oh dear:

```
sage: mathematica(1.e-4)
0.0001
sage: mathematica(1.e-5)
0.00001
sage: mathematica(1.e-6)
-7 + 10.*e
```


Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/10511_





---

archive/issue_events_125972.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-12-22T06:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10511",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10511#event-125972"
}
```



---

archive/issue_events_125973.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-12-22T06:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10511",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10511#event-125973"
}
```



---

archive/issue_events_125974.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-12-22T06:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10511",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10511#event-125974"
}
```



---

archive/issue_comments_101534.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis stems from the behaviour of _interface_init() in rings.real_mpfr.pyx.  I'm sure most of the other rings.X modules would give similar problems...",
    "created_at": "2010-12-22T08:24:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10511#issuecomment-101534",
    "user": "https://github.com/sagetrac-flawrence"
}
```

<div id="comment:1" align="right">Comment 1</div>

This stems from the behaviour of _interface_init() in rings.real_mpfr.pyx.  I'm sure most of the other rings.X modules would give similar problems...



---

archive/issue_events_125975.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-23T13:49:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10511",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10511#event-125975"
}
```



---

archive/issue_events_125976.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-23T13:49:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10511",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10511#event-125976"
}
```
