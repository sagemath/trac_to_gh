...
```json
{
    "body": "<a id='comment:6'></a>The failed doctest are related to the mulitple process doctesting. I get the same on my computer if I use multiple processes, but all tests pass with only a single process.\n\nI guess that has to do with reading the exceptional types from an external file, which cannot be accessed simultaneously from different processes. I add a sentence to the documentation...",
...
<a id='comment:6'></a>The failed doctest are related to the mulitple process doctesting. I get the same on my computer if I use multiple processes, but all tests pass with only a single process.
...
<a id='comment:7'></a>Is this dependent on anything other than #10527?  After applying that and this patch I get the following:
...
<a id='comment:8'></a>Replying to [JStarx](#comment%3A7):
> I think the patch was meant for 4.7.2, maybe it needs to be updated to work with the current version of sage?
...
<a id='comment:10'></a>Replying to [hthomas](#comment%3A9):
> Are the data files for the different mutation types really supposed to be part of the patch?  It seems to me that it's a fairly large quantity of data, so maybe it would be better to set things up so that an interested user can reconstruct it (which wouldn't be too hard, right?).  Or maybe I have the wrong idea about how receptive Sage is to including data like this.  Are there guidelines for this anywhere?  If not, maybe you could ask for feedback on sage-devel (or if you'd rather, I could)?  
...
```json
{
    "body": "<a id='comment:11'></a>I have added a review patch, but I haven't put anything in it yet.  \n\ncheers,\n\nHugh",
...
<a id='comment:11'></a>I have added a review patch, but I haven't put anything in it yet.  
...
```json
{
    "body": "<a id='comment:12'></a>I think that what I have now posted in the review patch for #10538 (on the combinat server) makes the patch compatible with the changes I have introduced in reviewing #10527.  (It also pointed out some mistakes in my \"corrections\" to #10527...)  In particular, all doctests are passing for me.  If you find anything that is not working, please let me know; I will try to get it so everything is running properly for Sage Days 40.",
...
<a id='comment:12'></a>I think that what I have now posted in the review patch for #10538 (on the combinat server) makes the patch compatible with the changes I have introduced in reviewing #10527.  (It also pointed out some mistakes in my "corrections" to #10527...)  In particular, all doctests are passing for me.  If you find anything that is not working, please let me know; I will try to get it so everything is running properly for Sage Days 40.
...
```json
{
    "body": "<a id='comment:16'></a>With an eye towards startup time, can all these new modules be imported on demand?",
...
<a id='comment:16'></a>With an eye towards startup time, can all these new modules be imported on demand?
...
<a id='comment:18'></a>This patch fixes a number of issues and also gives the user more flexibility.  Please see details below (especially 8c)
...
<a id='comment:20'></a>Replying to [stumpc5](#comment%3A17):
> Replying to [robertwb](#comment%3A16):
> > With an eye towards startup time, can all these new modules be imported on demand?
...
<a id='comment:22'></a>Replying to [gmoose05](#comment%3A18):
> This patch fixes a number of issues and also gives the user more flexibility.
...
<a id='comment:23'></a>Comments on your patch:
...
<a id='comment:24'></a>I just re-uploaded my patch.  Christian, thanks for catching those bugs.
...
<a id='comment:25'></a>Christian, the changes we discussed have now been essentially made:
...
```json
{
    "body": "<a id='comment:26'></a>I rebased the patches according to the new lazy import in #10527.",
...
<a id='comment:26'></a>I rebased the patches according to the new lazy import in #10527.
...
<a id='comment:27'></a>Recent changes to #10527 led to apply errors when my patch trac_10538-quiver-gm.patch is applied after the new #10527 patches.   I just uploaded a new version of my patch to eliminate these errors. 
...
```json
{
    "body": "<a id='comment:28'></a>Replying to [gmoose05](#comment%3A27):\n> Christian, please delete my old  (39.1 KB) patch and keep my (35.4 KB) patch in its place.  I no longer had permissions to replace my old patch.\n\n\nDone.",
...
<a id='comment:28'></a>Replying to [gmoose05](#comment%3A27):
> Christian, please delete my old  (39.1 KB) patch and keep my (35.4 KB) patch in its place.  I no longer had permissions to replace my old patch.
...
<a id='comment:30'></a>Just added an updated version of my patch.  
...
<a id='comment:31'></a>I made some small changes to the init() method.  In particular there was some undesirable behavior when 'data' was a list of edges or an empty digraph.  I added examples and tests which illustrate the changes.  When 'data' is a digraph it will now warn the user if loops are present rather than referring to the loops as two-cycles.  I also added an 'inplace' option (not default) to the principal_extension() method.
...
```json
{
    "body": "<a id='comment:32'></a>I uploaded a new version of the patch with all missing doctests added. Let's see what the patchbot says...",
...
<a id='comment:32'></a>I uploaded a new version of the patch with all missing doctests added. Let's see what the patchbot says...
...
```json
{
    "body": "<a id='comment:33'></a>Okay, I had another look and made some last minor changes in the documentation -- I think it is ready to go once we can convince the patchbot to give us a green light!",
...
<a id='comment:33'></a>Okay, I had another look and made some last minor changes in the documentation -- I think it is ready to go once we can convince the patchbot to give us a green light!
...
<a id='comment:34'></a>I did some reorganizing of the examples, is the placement of my colons correct?
...
<a id='comment:35'></a>Replying to [drupel](#comment%3A34):
> I did some reorganizing of the examples, is the placement of my colons correct?
...
```json
{
    "body": "<a id='comment:43'></a>There is a minor issue in qmu_save: it does not work whenever the quiver has 0<m!=n frozen vertices.\nPlease include the attached patch.",
...
<a id='comment:43'></a>There is a minor issue in qmu_save: it does not work whenever the quiver has 0<m!=n frozen vertices.
Please include the attached patch.
...
<a id='comment:44'></a>Replying to [etn40ff](#comment%3A43):
> There is a minor issue in qmu_save: it does not work whenever the quiver has 0<m!=n frozen vertices.
> Please include the attached patch.
