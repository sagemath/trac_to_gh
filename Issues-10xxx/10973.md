# Issue 10973: Integral points on elliptic curves over number fields

archive/issues_010894.json:
```json
{
    "body": "Assignee: @JohnCremona\n\nCC:  @JohnCremona gagansekhon @jbalakrishnan\n\nKeywords: sd32\n\nIncorporate work done by Rado Kirov and Jackie Anderson at Sage Days 22, based on Magma implementation by Cremona's student Nook.\n\nSee also #12095 (now tagged as duplicate).\n\nConverted to git by John Cremona, 2013-12-18\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/10973\n\n",
    "created_at": "2011-03-21T19:47:56Z",
    "labels": [
        "component: elliptic curves"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Integral points on elliptic curves over number fields",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/10973",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```
Assignee: @JohnCremona

CC:  @JohnCremona gagansekhon @jbalakrishnan

Keywords: sd32

Incorporate work done by Rado Kirov and Jackie Anderson at Sage Days 22, based on Magma implementation by Cremona's student Nook.

See also #12095 (now tagged as duplicate).

Converted to git by John Cremona, 2013-12-18


Issue created by migration from https://trac.sagemath.org/ticket/10973





---

archive/issue_comments_116417.json:
```json
{
    "body": "Attachment [intpts.sage](tarball://root/attachments/some-uuid/ticket10973/intpts.sage) by justin created at 2011-03-21 19:48:30",
    "created_at": "2011-03-21T19:48:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116417",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

Attachment [intpts.sage](tarball://root/attachments/some-uuid/ticket10973/intpts.sage) by justin created at 2011-03-21 19:48:30



---

archive/issue_comments_116418.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to enhancement.",
    "created_at": "2011-03-21T20:04:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116418",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

Changing type from PLEASE CHANGE to enhancement.



---

archive/issue_comments_116419.json:
```json
{
    "body": "Attachment [ell_int_points.py](tarball://root/attachments/some-uuid/ticket10973/ell_int_points.py) by justin created at 2011-03-22 17:32:21\n\nThis code goes into \"sage/schemes/elliptiic_curves\".  It works, but needs work.",
    "created_at": "2011-03-22T17:32:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116419",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

Attachment [ell_int_points.py](tarball://root/attachments/some-uuid/ticket10973/ell_int_points.py) by justin created at 2011-03-22 17:32:21

This code goes into "sage/schemes/elliptiic_curves".  It works, but needs work.



---

archive/issue_comments_116420.json:
```json
{
    "body": "New version of .py file, fixing a problem with bogus interaction with Maxima.",
    "created_at": "2011-03-22T17:33:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116420",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

New version of .py file, fixing a problem with bogus interaction with Maxima.



---

archive/issue_comments_116421.json:
```json
{
    "body": "Attachment [trac_10973](tarball://root/attachments/some-uuid/ticket10973/trac_10973) by gagansekhon created at 2011-03-23 02:22:15",
    "created_at": "2011-03-23T02:22:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116421",
    "user": "https://trac.sagemath.org/admin/accounts/users/gagansekhon"
}
```

Attachment [trac_10973](tarball://root/attachments/some-uuid/ticket10973/trac_10973) by gagansekhon created at 2011-03-23 02:22:15



---

archive/issue_comments_116422.json:
```json
{
    "body": "The patch trac_10973 should apply w/o problems against sage-4.7.alpha1.\n\nThe previous attachments are replaced by this one.",
    "created_at": "2011-03-23T04:50:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116422",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

The patch trac_10973 should apply w/o problems against sage-4.7.alpha1.

The previous attachments are replaced by this one.



---

archive/issue_comments_116423.json:
```json
{
    "body": "This patch is related to, and appears to fix, the problem described by #10152.",
    "created_at": "2011-03-25T00:23:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116423",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

This patch is related to, and appears to fix, the problem described by #10152.



---

archive/issue_comments_116424.json:
```json
{
    "body": "Attachment [trac_10973.2](tarball://root/attachments/some-uuid/ticket10973/trac_10973.2) by justin created at 2011-03-25 01:17:21\n\nUpdated patch with code to reject requests w/o generators; replaces previous patch of same name.",
    "created_at": "2011-03-25T01:17:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116424",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

Attachment [trac_10973.2](tarball://root/attachments/some-uuid/ticket10973/trac_10973.2) by justin created at 2011-03-25 01:17:21

Updated patch with code to reject requests w/o generators; replaces previous patch of same name.



---

archive/issue_comments_116425.json:
```json
{
    "body": "minor error in all.py, +from ell_int_points.py import *\n should be +from ell_int_points import *\nThe new patch that I will be uploading in a minute fixes that.",
    "created_at": "2011-03-25T19:08:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116425",
    "user": "https://trac.sagemath.org/admin/accounts/users/gagansekhon"
}
```

minor error in all.py, +from ell_int_points.py import *
 should be +from ell_int_points import *
The new patch that I will be uploading in a minute fixes that.



---

archive/issue_comments_116426.json:
```json
{
    "body": "Attachment [trac_10973.patch](tarball://root/attachments/some-uuid/ticket10973/trac_10973.patch) by gagansekhon created at 2011-03-25 19:08:46",
    "created_at": "2011-03-25T19:08:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116426",
    "user": "https://trac.sagemath.org/admin/accounts/users/gagansekhon"
}
```

Attachment [trac_10973.patch](tarball://root/attachments/some-uuid/ticket10973/trac_10973.patch) by gagansekhon created at 2011-03-25 19:08:46



---

archive/issue_comments_116427.json:
```json
{
    "body": "Attachment [trac_10973.2.patch](tarball://root/attachments/some-uuid/ticket10973/trac_10973.2.patch) by @adeines created at 2011-08-24 04:53:12\n\nThis replaces the previous patch",
    "created_at": "2011-08-24T04:53:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116427",
    "user": "https://github.com/adeines"
}
```

Attachment [trac_10973.2.patch](tarball://root/attachments/some-uuid/ticket10973/trac_10973.2.patch) by @adeines created at 2011-08-24 04:53:12

This replaces the previous patch



---

archive/issue_comments_116428.json:
```json
{
    "body": "The tests pass on all files except ell_ergos.py and constructor.py, specifically, the tests which call S_integral_points([]) are having issues.  \n\nAll examples from the paper referenced now pass as do all examples from #10152, so once this is implemented, this also fixes #10152.",
    "created_at": "2011-08-24T04:56:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116428",
    "user": "https://github.com/adeines"
}
```

The tests pass on all files except ell_ergos.py and constructor.py, specifically, the tests which call S_integral_points([]) are having issues.  

All examples from the paper referenced now pass as do all examples from #10152, so once this is implemented, this also fixes #10152.



---

archive/issue_comments_116429.json:
```json
{
    "body": "replaces previous patch",
    "created_at": "2011-08-24T05:15:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116429",
    "user": "https://github.com/adeines"
}
```

replaces previous patch



---

archive/issue_comments_116430.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-08-24T05:16:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116430",
    "user": "https://github.com/adeines"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_116431.json:
```json
{
    "body": "Attachment [trac_10973.3.patch](tarball://root/attachments/some-uuid/ticket10973/trac_10973.3.patch) by @adeines created at 2011-08-24 05:16:34\n\nThis should do it.  This is a small fix to the previous patch, it now passes the tests.",
    "created_at": "2011-08-24T05:16:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116431",
    "user": "https://github.com/adeines"
}
```

Attachment [trac_10973.3.patch](tarball://root/attachments/some-uuid/ticket10973/trac_10973.3.patch) by @adeines created at 2011-08-24 05:16:34

This should do it.  This is a small fix to the previous patch, it now passes the tests.



---

archive/issue_comments_116432.json:
```json
{
    "body": "Attachment [report.txt](tarball://root/attachments/some-uuid/ticket10973/report.txt) by @williamstein created at 2011-08-24 06:15:08\n\nThis is a list of 14 issues I had reading through trac_10973.3.patch.",
    "created_at": "2011-08-24T06:15:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116432",
    "user": "https://github.com/williamstein"
}
```

Attachment [report.txt](tarball://root/attachments/some-uuid/ticket10973/report.txt) by @williamstein created at 2011-08-24 06:15:08

This is a list of 14 issues I had reading through trac_10973.3.patch.



---

archive/issue_comments_116433.json:
```json
{
    "body": "Please see the rather long file report.txt that I posted with a bunch of issues I noticed when reading through trac_10973.3.patch.",
    "created_at": "2011-08-24T06:16:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116433",
    "user": "https://github.com/williamstein"
}
```

Please see the rather long file report.txt that I posted with a bunch of issues I noticed when reading through trac_10973.3.patch.



---

archive/issue_comments_116434.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2011-08-24T06:16:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116434",
    "user": "https://github.com/williamstein"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_116435.json:
```json
{
    "body": "Attachment [Trac10973.4.patch](tarball://root/attachments/some-uuid/ticket10973/Trac10973.4.patch) by @adeines created at 2011-08-24 21:50:22\n\nReplaces previous patch.",
    "created_at": "2011-08-24T21:50:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116435",
    "user": "https://github.com/adeines"
}
```

Attachment [Trac10973.4.patch](tarball://root/attachments/some-uuid/ticket10973/Trac10973.4.patch) by @adeines created at 2011-08-24 21:50:22

Replaces previous patch.



---

archive/issue_comments_116436.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2011-08-24T21:56:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116436",
    "user": "https://github.com/adeines"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_116437.json:
```json
{
    "body": "Trac10973.4.patch addresses everything in report.txt.\n\nThis returns the same result as the original integral_points over `Q` with all curves of conductor up to 1000 (and also agrees with Magma).  This also finds the missing points referred to in ticket #10152.\n\nWe compared times on all curves over `Q` of conductor less then 1000; the original implementation was usually 2 to 8 \ntimes faster, though in certain instances it did not find all integral points (this was ticket #10152).  So this is a bit of a slowdown.  \n\nIf `E` is a curve over `Q`, then E.integral_points() calls the method in ell_rational_points.py (the previous, faster, not always correct, method).",
    "created_at": "2011-08-24T21:56:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116437",
    "user": "https://github.com/adeines"
}
```

Trac10973.4.patch addresses everything in report.txt.

This returns the same result as the original integral_points over `Q` with all curves of conductor up to 1000 (and also agrees with Magma).  This also finds the missing points referred to in ticket #10152.

We compared times on all curves over `Q` of conductor less then 1000; the original implementation was usually 2 to 8 
times faster, though in certain instances it did not find all integral points (this was ticket #10152).  So this is a bit of a slowdown.  

If `E` is a curve over `Q`, then E.integral_points() calls the method in ell_rational_points.py (the previous, faster, not always correct, method).



---

archive/issue_comments_116438.json:
```json
{
    "body": "Attachment [Trac10973.5.patch](tarball://root/attachments/some-uuid/ticket10973/Trac10973.5.patch) by @adeines created at 2011-08-24 21:59:06\n\nReplaces previous patch, corrects Authors.",
    "created_at": "2011-08-24T21:59:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116438",
    "user": "https://github.com/adeines"
}
```

Attachment [Trac10973.5.patch](tarball://root/attachments/some-uuid/ticket10973/Trac10973.5.patch) by @adeines created at 2011-08-24 21:59:06

Replaces previous patch, corrects Authors.



---

archive/issue_comments_116439.json:
```json
{
    "body": "Replying to [comment:12 aly.deines]:\n> If `E` is a curve over `Q`, then E.integral_points() calls the \n> method in ell_rational_points.py (the previous, faster, not always correct, method).\n\n\nWhy?  Surely it would definitely be better to return a correct answer by default.   I can imagine leaving the old code in with an not-by-default option to call it and docs that mention it is faster.",
    "created_at": "2011-08-24T22:02:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116439",
    "user": "https://github.com/williamstein"
}
```

Replying to [comment:12 aly.deines]:
> If `E` is a curve over `Q`, then E.integral_points() calls the 
> method in ell_rational_points.py (the previous, faster, not always correct, method).


Why?  Surely it would definitely be better to return a correct answer by default.   I can imagine leaving the old code in with an not-by-default option to call it and docs that mention it is faster.



---

archive/issue_comments_116440.json:
```json
{
    "body": "Attachment [report2.txt](tarball://root/attachments/some-uuid/ticket10973/report2.txt) by @williamstein created at 2011-08-24 22:28:08\n\nsecond short report -- looking good!",
    "created_at": "2011-08-24T22:28:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116440",
    "user": "https://github.com/williamstein"
}
```

Attachment [report2.txt](tarball://root/attachments/some-uuid/ticket10973/report2.txt) by @williamstein created at 2011-08-24 22:28:08

second short report -- looking good!



---

archive/issue_comments_116441.json:
```json
{
    "body": "Here's a list of curves where your code and the latest (?) Magma V2.17-4 give different answers.  I checked these by hand, and Magma is clearly totally wrong (missing definitely integral points) in each case.  \n\n1264i1, 1328b1, 1656f1, 1848i1",
    "created_at": "2011-08-24T22:37:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116441",
    "user": "https://github.com/williamstein"
}
```

Here's a list of curves where your code and the latest (?) Magma V2.17-4 give different answers.  I checked these by hand, and Magma is clearly totally wrong (missing definitely integral points) in each case.  

1264i1, 1328b1, 1656f1, 1848i1



---

archive/issue_comments_116442.json:
```json
{
    "body": "Changing keywords from \"\" to \"sd32\".",
    "created_at": "2011-08-24T23:41:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116442",
    "user": "https://github.com/williamstein"
}
```

Changing keywords from "" to "sd32".



---

archive/issue_comments_116443.json:
```json
{
    "body": "replaces previous patch",
    "created_at": "2011-08-25T00:05:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116443",
    "user": "https://github.com/adeines"
}
```

replaces previous patch



---

archive/issue_comments_116444.json:
```json
{
    "body": "Attachment [Trac10973.6.patch](tarball://root/attachments/some-uuid/ticket10973/Trac10973.6.patch) by @JohnCremona created at 2011-11-23 11:52:47\n\nReplying to [comment:14 was]:\n> Here's a list of curves where your code and the latest (?) Magma V2.17-4 give different answers.  I checked these by hand, and Magma is clearly totally wrong (missing definitely integral points) in each case.  \n> \n> 1264i1, 1328b1, 1656f1, 1848i1\n\n\nUpdate.  Using Sage-4.7.2 and Magma V2.17-9, these all agree:\n\n```\nsage: E = EllipticCurve(\"1264i1\")\nsage: E.integral_points()\n[(2 : 0 : 1), (4 : 10 : 1), (18 : 80 : 1), (246404 : 122312970 : 1)]\nsage: magma(E).IntegralPoints()\n[ (2 : 0 : 1), (4 : -10 : 1), (18 : 80 : 1), (246404 : 122312970 : 1) ]\nsage: E = EllipticCurve(\"1328b1\")\nsage: E.integral_points()\n[(2 : 2 : 1), (4386 : 290438 : 1)]\nsage: magma(E).IntegralPoints()\n[ (2 : 2 : 1), (4386 : 290438 : 1) ]\nsage: E = EllipticCurve(\"1656f1\")\nsage: E.integral_points()\n[(3 : 0 : 1), (6 : 18 : 1), (27 : 144 : 1), (336678 : 195353910 : 1)]\nsage: magma(E).IntegralPoints()\n[ (3 : 0 : 1), (6 : -18 : 1), (27 : 144 : 1), (336678 : -195353910 : 1) ]\nsage: E = EllipticCurve(\"1848i1\")\nsage: E.integral_points()\n[(2 : 3 : 1), (3206 : 181557 : 1)]\nsage: magma(E).IntegralPoints()\n[ (2 : 3 : 1), (3206 : 181557 : 1) ]\n```\nThat is using vanilla Sage (i.e. without any patches from this ticket).\nI am about to do a more systematic test.",
    "created_at": "2011-11-23T11:52:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116444",
    "user": "https://github.com/JohnCremona"
}
```

Attachment [Trac10973.6.patch](tarball://root/attachments/some-uuid/ticket10973/Trac10973.6.patch) by @JohnCremona created at 2011-11-23 11:52:47

Replying to [comment:14 was]:
> Here's a list of curves where your code and the latest (?) Magma V2.17-4 give different answers.  I checked these by hand, and Magma is clearly totally wrong (missing definitely integral points) in each case.  
> 
> 1264i1, 1328b1, 1656f1, 1848i1


Update.  Using Sage-4.7.2 and Magma V2.17-9, these all agree:

```
sage: E = EllipticCurve("1264i1")
sage: E.integral_points()
[(2 : 0 : 1), (4 : 10 : 1), (18 : 80 : 1), (246404 : 122312970 : 1)]
sage: magma(E).IntegralPoints()
[ (2 : 0 : 1), (4 : -10 : 1), (18 : 80 : 1), (246404 : 122312970 : 1) ]
sage: E = EllipticCurve("1328b1")
sage: E.integral_points()
[(2 : 2 : 1), (4386 : 290438 : 1)]
sage: magma(E).IntegralPoints()
[ (2 : 2 : 1), (4386 : 290438 : 1) ]
sage: E = EllipticCurve("1656f1")
sage: E.integral_points()
[(3 : 0 : 1), (6 : 18 : 1), (27 : 144 : 1), (336678 : 195353910 : 1)]
sage: magma(E).IntegralPoints()
[ (3 : 0 : 1), (6 : -18 : 1), (27 : 144 : 1), (336678 : -195353910 : 1) ]
sage: E = EllipticCurve("1848i1")
sage: E.integral_points()
[(2 : 3 : 1), (3206 : 181557 : 1)]
sage: magma(E).IntegralPoints()
[ (2 : 3 : 1), (3206 : 181557 : 1) ]
```
That is using vanilla Sage (i.e. without any patches from this ticket).
I am about to do a more systematic test.



---

archive/issue_comments_116445.json:
```json
{
    "body": "I am testing this now.  It looks as if all the points raised in William's last report have been dealt with.\n\nIt looks odd to me to have the main integral_points function which is a method of the EllipticCurve_number_field class defined in a separate file, so that in ell_number_field.py you only see the line integral_points = integral_points.  I did not know that would work!  But, given that it does, it is cleaner.  I will check to see that the documentation is not thereby hidden.",
    "created_at": "2011-11-23T12:50:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116445",
    "user": "https://github.com/JohnCremona"
}
```

I am testing this now.  It looks as if all the points raised in William's last report have been dealt with.

It looks odd to me to have the main integral_points function which is a method of the EllipticCurve_number_field class defined in a separate file, so that in ell_number_field.py you only see the line integral_points = integral_points.  I did not know that would work!  But, given that it does, it is cleaner.  I will check to see that the documentation is not thereby hidden.



---

archive/issue_comments_116446.json:
```json
{
    "body": "Positive review.  In my reviewer's patch I fix  the following:\n\n0. Now applies to 4.7.2 with no fuzz.\n1. Added QQ to import list on line 332 of ell_number_field.py, otherwise testing ell_int_pts failed, but testing ell_number_field did not -- since the test for that function silverman_height_bounds was incomplete!  I fixed that too.\n2. Fixed a warning in docbuild by adding a blank line at line 69 of the same file.\n\nIt is now true that for an elliptic curve E over Q we have both the new E.silverman_height_bounds(), which gives lower and upper bounds, and the old E.silverman_height_bound(), which only gives one of them.  The latter can be deleted but that will require some tweaking of any code which uses it.  I have left that for another ticket.",
    "created_at": "2011-11-23T14:38:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116446",
    "user": "https://github.com/JohnCremona"
}
```

Positive review.  In my reviewer's patch I fix  the following:

0. Now applies to 4.7.2 with no fuzz.
1. Added QQ to import list on line 332 of ell_number_field.py, otherwise testing ell_int_pts failed, but testing ell_number_field did not -- since the test for that function silverman_height_bounds was incomplete!  I fixed that too.
2. Fixed a warning in docbuild by adding a blank line at line 69 of the same file.

It is now true that for an elliptic curve E over Q we have both the new E.silverman_height_bounds(), which gives lower and upper bounds, and the old E.silverman_height_bound(), which only gives one of them.  The latter can be deleted but that will require some tweaking of any code which uses it.  I have left that for another ticket.



---

archive/issue_comments_116447.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-11-23T14:38:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116447",
    "user": "https://github.com/JohnCremona"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_116448.json:
```json
{
    "body": "Replaces all previous: reviewer's patch",
    "created_at": "2011-11-23T14:39:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116448",
    "user": "https://github.com/JohnCremona"
}
```

Replaces all previous: reviewer's patch



---

archive/issue_comments_116449.json:
```json
{
    "body": "Attachment [Trac10973.7.patch](tarball://root/attachments/some-uuid/ticket10973/Trac10973.7.patch) by @JohnCremona created at 2011-11-23 16:15:10\n\nReplying to [comment:18 cremona]:\n> I am testing this now.  It looks as if all the points raised in William's last report have been dealt with.\n> \n\n\nThe first curve where Sage-4.7.2 fails to find the same number of integral points as Magma V2.17-9 is 2082a1 where Sage-4.7.2 finds x-coordinates -11,-2,4,13 but not 507525709 (!).  But the new version does find that last one.\n\nI will carry on testing, but there is no need to delay merging this (expect perhaps if I find a curve for which the new code fails to find all known points).",
    "created_at": "2011-11-23T16:15:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116449",
    "user": "https://github.com/JohnCremona"
}
```

Attachment [Trac10973.7.patch](tarball://root/attachments/some-uuid/ticket10973/Trac10973.7.patch) by @JohnCremona created at 2011-11-23 16:15:10

Replying to [comment:18 cremona]:
> I am testing this now.  It looks as if all the points raised in William's last report have been dealt with.
> 


The first curve where Sage-4.7.2 fails to find the same number of integral points as Magma V2.17-9 is 2082a1 where Sage-4.7.2 finds x-coordinates -11,-2,4,13 but not 507525709 (!).  But the new version does find that last one.

I will carry on testing, but there is no need to delay merging this (expect perhaps if I find a curve for which the new code fails to find all known points).



---

archive/issue_comments_116450.json:
```json
{
    "body": "Replying to [comment:20 cremona]:\n> Replying to [comment:18 cremona]:\n> > I am testing this now.  It looks as if all the points raised in William's last report have been dealt with.\n> > \n\n> \n> The first curve where Sage-4.7.2 fails to find the same number of integral points as Magma V2.17-9 is 2082a1 where Sage-4.7.2 finds x-coordinates -11,-2,4,13 but not 507525709 (!).  But the new version does find that last one.\n> \n> I will carry on testing, but there is no need to delay merging this (expect perhaps if I find a curve for which the new code fails to find all known points).\n\n\nFor all curves of conductor up to 10000, there are precisely 3 where Sage-4.7.2 misses integral points compared with Magma V2.17-9, namely 2082a1, 6104b1, 8470g1, 8470g2;  in each case just one point is missed.  And the new code agrees with Magma in all these cases.  Regarding timings, up to 10000 Sage-4.7.2 took 157m while Magma took 943m;  that may be an unfair comparison though since Sage took generators from the DB while Magma (I think) computed them from scratch.  I will redo this with the current patch applied.",
    "created_at": "2011-11-24T14:19:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116450",
    "user": "https://github.com/JohnCremona"
}
```

Replying to [comment:20 cremona]:
> Replying to [comment:18 cremona]:
> > I am testing this now.  It looks as if all the points raised in William's last report have been dealt with.
> > 

> 
> The first curve where Sage-4.7.2 fails to find the same number of integral points as Magma V2.17-9 is 2082a1 where Sage-4.7.2 finds x-coordinates -11,-2,4,13 but not 507525709 (!).  But the new version does find that last one.
> 
> I will carry on testing, but there is no need to delay merging this (expect perhaps if I find a curve for which the new code fails to find all known points).


For all curves of conductor up to 10000, there are precisely 3 where Sage-4.7.2 misses integral points compared with Magma V2.17-9, namely 2082a1, 6104b1, 8470g1, 8470g2;  in each case just one point is missed.  And the new code agrees with Magma in all these cases.  Regarding timings, up to 10000 Sage-4.7.2 took 157m while Magma took 943m;  that may be an unfair comparison though since Sage took generators from the DB while Magma (I think) computed them from scratch.  I will redo this with the current patch applied.



---

archive/issue_events_028457.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-26T12:44:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28457"
}
```



---

archive/issue_comments_116451.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2011-11-26T21:27:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116451",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_116452.json:
```json
{
    "body": "I unfortunately get several doctest failures (with sage-4.8.alpha2 + various patches).  Did you test this patch with the new PARI (merged in sage-4.8.alpha1)?  I am also afraid that there are speed regressions, I am seeing a timeout:\n\n```\nsage -t  -force_lib devel/sage/sage/schemes/elliptic_curves/ell_int_points.py\n**********************************************************************\nFile \"/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py\", line 12:\n    sage: E.integral_points()\nExpected:\n    [(-11 : -19 : 1), (-2 : 29 : 1), (4 : -16 : 1), (13 : -43 : 1), (507525709 : 11433453531221 : 1)]\nGot:\n    [(-11 : 29 : 1), (-2 : -28 : 1), (4 : 11 : 1), (13 : 29 : 1), (507525709 : -11433961056931 : 1)]\n**********************************************************************\nFile \"/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py\", line 22:\n    sage: E.integral_points()\nExpected:\n    [(-41 : -266 : 1), (-27 : 294 : 1), (-13 : 266 : 1), (19 : 74 : 1), (22\n    : -49 : 1), (27 : -6 : 1), (29 : -14 : 1), (43 : -154 : 1), (53 : 266 :\n    1), (71 : 490 : 1), (414 : -8379 : 1), (1639 : 66346 : 1), (1667 :\n    -68054 : 1), (23036693 : 110568192854 : 1)]\nGot:\n    [(-41 : 266 : 1), (-27 : 294 : 1), (-13 : -266 : 1), (19 : 74 : 1), (22 : -49 : 1), (27 : 6 : 1), (29 : -14 : 1), (43 : 154 : 1), (53 : 266 : 1), (71 : -490 : 1), (414 : 8379 : 1), (1639 : 66346 : 1), (1667 : 68054 : 1), (23036693 : 110568192854 : 1)]\n**********************************************************************\nFile \"/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py\", line 31:\n    sage: E.integral_points(algorithm = \"old\")\nExpected:\n    [(-1 : 0 : 1), (0 : 6 : 1), (3 : 12 : 1), (10 : 33 : 1), (15 : 56 : 1), (24 : 110 : 1), (43 : 264\n    : 1), (98 : 924 : 1), (395 : 7656 : 1)]\nGot:\n    [(-1 : 0 : 1), (0 : 6 : 1), (3 : 12 : 1), (10 : 33 : 1), (15 : 56 : 1), (24 : 110 : 1), (43 : 264 : 1), (98 : 924 : 1), (395 : 7656 : 1), (1681690 : 2179974753 : 1)]\n*********************************************************************\nFile \"/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py\", line 34:\n    sage: E.integral_points()\nExpected:\n    [(-1 : 0 : 1), (0 : -7 : 1), (3 : 12 : 1), (10 : -44 : 1), (15 : 56 :\n    1), (24 : 110 : 1), (43 : 264 : 1), (98 : -1023 : 1), (395 : -8052 : 1),\n    (1681690 : 2179974753 : 1)]\nGot:\n    [(-1 : 0 : 1), (0 : 6 : 1), (3 : -16 : 1), (10 : 33 : 1), (15 : -72 : 1), (24 : 110 : 1), (43 : -308 : 1), (98 : -1023 : 1), (395 : 76\n56 : 1), (1681690 : -2181656444 : 1)]\n**********************************************************************\nFile \"/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py\", line 46:\n    sage: E.integral_points()\nExpected:\n    [(-14 : 17 : 1), (-12 : -22 : 1), (-7 : 38 : 1), (-1 : 22 : 1), (0 : -18\n    : 1), (13 : -22 : 1), (14 : -32 : 1), (21 : 66 : 1), (33 : -192 : 1),\n    (44 : 257 : 1), (63 : 458 : 1), (98 : -1012 : 1), (175 : -2398 : 1),\n    (1533 : -60792 : 1), (1561 : 60896 : 1), (18888968 : -82103620687 : 1)]\nGot:\n    [(-14 : 17 : 1), (-12 : 33 : 1), (-7 : -32 : 1), (-1 : 22 : 1), (0 : -18 : 1), (13 : -22 : 1), (14 : 17 : 1), (21 : -88 : 1), (33 : -1\n92 : 1), (44 : -302 : 1), (63 : 458 : 1), (98 : -1012 : 1), (175 : 2222 : 1), (1533 : -60792 : 1), (1561 : 60896 : 1), (18888968 : -821036\n20687 : 1)]\n**********************************************************************\nFile \"/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py\", line 131:\n    sage: ellpts.abs_log_height(list(P2))\nExpected:\n    1.09861228866811\nGot:\n    3.21887582486820\n**********************************************************************\n```\n\n```\nThe following tests failed:\n\n        sage -t  -force_lib devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst # Time out\n        sage -t  -force_lib devel/sage/sage/schemes/elliptic_curves/ell_int_points.py # 6 doctests failed\n```",
    "created_at": "2011-11-26T21:27:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116452",
    "user": "https://github.com/jdemeyer"
}
```

I unfortunately get several doctest failures (with sage-4.8.alpha2 + various patches).  Did you test this patch with the new PARI (merged in sage-4.8.alpha1)?  I am also afraid that there are speed regressions, I am seeing a timeout:

```
sage -t  -force_lib devel/sage/sage/schemes/elliptic_curves/ell_int_points.py
**********************************************************************
File "/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py", line 12:
    sage: E.integral_points()
Expected:
    [(-11 : -19 : 1), (-2 : 29 : 1), (4 : -16 : 1), (13 : -43 : 1), (507525709 : 11433453531221 : 1)]
Got:
    [(-11 : 29 : 1), (-2 : -28 : 1), (4 : 11 : 1), (13 : 29 : 1), (507525709 : -11433961056931 : 1)]
**********************************************************************
File "/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py", line 22:
    sage: E.integral_points()
Expected:
    [(-41 : -266 : 1), (-27 : 294 : 1), (-13 : 266 : 1), (19 : 74 : 1), (22
    : -49 : 1), (27 : -6 : 1), (29 : -14 : 1), (43 : -154 : 1), (53 : 266 :
    1), (71 : 490 : 1), (414 : -8379 : 1), (1639 : 66346 : 1), (1667 :
    -68054 : 1), (23036693 : 110568192854 : 1)]
Got:
    [(-41 : 266 : 1), (-27 : 294 : 1), (-13 : -266 : 1), (19 : 74 : 1), (22 : -49 : 1), (27 : 6 : 1), (29 : -14 : 1), (43 : 154 : 1), (53 : 266 : 1), (71 : -490 : 1), (414 : 8379 : 1), (1639 : 66346 : 1), (1667 : 68054 : 1), (23036693 : 110568192854 : 1)]
**********************************************************************
File "/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py", line 31:
    sage: E.integral_points(algorithm = "old")
Expected:
    [(-1 : 0 : 1), (0 : 6 : 1), (3 : 12 : 1), (10 : 33 : 1), (15 : 56 : 1), (24 : 110 : 1), (43 : 264
    : 1), (98 : 924 : 1), (395 : 7656 : 1)]
Got:
    [(-1 : 0 : 1), (0 : 6 : 1), (3 : 12 : 1), (10 : 33 : 1), (15 : 56 : 1), (24 : 110 : 1), (43 : 264 : 1), (98 : 924 : 1), (395 : 7656 : 1), (1681690 : 2179974753 : 1)]
*********************************************************************
File "/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py", line 34:
    sage: E.integral_points()
Expected:
    [(-1 : 0 : 1), (0 : -7 : 1), (3 : 12 : 1), (10 : -44 : 1), (15 : 56 :
    1), (24 : 110 : 1), (43 : 264 : 1), (98 : -1023 : 1), (395 : -8052 : 1),
    (1681690 : 2179974753 : 1)]
Got:
    [(-1 : 0 : 1), (0 : 6 : 1), (3 : -16 : 1), (10 : 33 : 1), (15 : -72 : 1), (24 : 110 : 1), (43 : -308 : 1), (98 : -1023 : 1), (395 : 76
56 : 1), (1681690 : -2181656444 : 1)]
**********************************************************************
File "/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py", line 46:
    sage: E.integral_points()
Expected:
    [(-14 : 17 : 1), (-12 : -22 : 1), (-7 : 38 : 1), (-1 : 22 : 1), (0 : -18
    : 1), (13 : -22 : 1), (14 : -32 : 1), (21 : 66 : 1), (33 : -192 : 1),
    (44 : 257 : 1), (63 : 458 : 1), (98 : -1012 : 1), (175 : -2398 : 1),
    (1533 : -60792 : 1), (1561 : 60896 : 1), (18888968 : -82103620687 : 1)]
Got:
    [(-14 : 17 : 1), (-12 : 33 : 1), (-7 : -32 : 1), (-1 : 22 : 1), (0 : -18 : 1), (13 : -22 : 1), (14 : 17 : 1), (21 : -88 : 1), (33 : -1
92 : 1), (44 : -302 : 1), (63 : 458 : 1), (98 : -1012 : 1), (175 : 2222 : 1), (1533 : -60792 : 1), (1561 : 60896 : 1), (18888968 : -821036
20687 : 1)]
**********************************************************************
File "/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha3/devel/sage-main/sage/schemes/elliptic_curves/ell_int_points.py", line 131:
    sage: ellpts.abs_log_height(list(P2))
Expected:
    1.09861228866811
Got:
    3.21887582486820
**********************************************************************
```

```
The following tests failed:

        sage -t  -force_lib devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst # Time out
        sage -t  -force_lib devel/sage/sage/schemes/elliptic_curves/ell_int_points.py # 6 doctests failed
```



---

archive/issue_comments_116453.json:
```json
{
    "body": "Minor comment: could you format the copyright notice of the added file as shown in [http://sagemath.org/doc/developer/conventions.html#headings-of-sage-library-code-files](http://sagemath.org/doc/developer/conventions.html#headings-of-sage-library-code-files).  I am also very surprised to see William Stein in the copyright statement, he is not even an author of this ticket.",
    "created_at": "2011-11-26T21:33:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116453",
    "user": "https://github.com/jdemeyer"
}
```

Minor comment: could you format the copyright notice of the added file as shown in [http://sagemath.org/doc/developer/conventions.html#headings-of-sage-library-code-files](http://sagemath.org/doc/developer/conventions.html#headings-of-sage-library-code-files).  I am also very surprised to see William Stein in the copyright statement, he is not even an author of this ticket.



---

archive/issue_comments_116454.json:
```json
{
    "body": "Line 617: replace by `initQ = 10^118` :-)",
    "created_at": "2011-11-26T21:37:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116454",
    "user": "https://github.com/jdemeyer"
}
```

Line 617: replace by `initQ = 10^118` :-)



---

archive/issue_comments_116455.json:
```json
{
    "body": "If you word-wrap doctest results (which is a good idea), try to word-wrap them more logically, i.e. write\n\n```\n[(-41 : -266 : 1), (-27 : 294 : 1), (-13 : 266 : 1), (19 : 74 : 1),\n(22 : -49 : 1), (27 : -6 : 1), (29 : -14 : 1), (43 : -154 : 1),\n(53 : 266 : 1), (71 : 490 : 1), (414 : -8379 : 1), (1639 : 66346 : 1),\n(1667 : -68054 : 1), (23036693 : 110568192854 : 1)]\n```\ninstead of\n\n```\n[(-41 : -266 : 1), (-27 : 294 : 1), (-13 : 266 : 1), (19 : 74 : 1), (22 \n: -49 : 1), (27 : -6 : 1), (29 : -14 : 1), (43 : -154 : 1), (53 : 266 : \n1), (71 : 490 : 1), (414 : -8379 : 1), (1639 : 66346 : 1), (1667 : \n-68054 : 1), (23036693 : 110568192854 : 1)]\n```",
    "created_at": "2011-11-26T21:55:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116455",
    "user": "https://github.com/jdemeyer"
}
```

If you word-wrap doctest results (which is a good idea), try to word-wrap them more logically, i.e. write

```
[(-41 : -266 : 1), (-27 : 294 : 1), (-13 : 266 : 1), (19 : 74 : 1),
(22 : -49 : 1), (27 : -6 : 1), (29 : -14 : 1), (43 : -154 : 1),
(53 : 266 : 1), (71 : 490 : 1), (414 : -8379 : 1), (1639 : 66346 : 1),
(1667 : -68054 : 1), (23036693 : 110568192854 : 1)]
```
instead of

```
[(-41 : -266 : 1), (-27 : 294 : 1), (-13 : 266 : 1), (19 : 74 : 1), (22 
: -49 : 1), (27 : -6 : 1), (29 : -14 : 1), (43 : -154 : 1), (53 : 266 : 
1), (71 : 490 : 1), (414 : -8379 : 1), (1639 : 66346 : 1), (1667 : 
-68054 : 1), (23036693 : 110568192854 : 1)]
```



---

archive/issue_comments_116456.json:
```json
{
    "body": "In the documentation, you refer to ticket 10152.  This should be 10973 instead.",
    "created_at": "2011-11-26T21:58:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116456",
    "user": "https://github.com/jdemeyer"
}
```

In the documentation, you refer to ticket 10152.  This should be 10973 instead.



---

archive/issue_comments_116457.json:
```json
{
    "body": "Indentation at the top should probably be\n\n```\nr\"\"\"\nIntegral Points of Elliptic Curves over Number Fields\n\nThe following examples are from trac ticket #10152.  Previously Magma was finding ore \nintegral points on these curves than Sage.  We resolved this issue.\n\nEXAMPLES::\n\n    sage: E = EllipticCurve('2082a1')\n    sage: E.integral_points(algorithm = \"old\")\n    [(-11 : 29 : 1), (-2 : 29 : 1), (4 : 11 : 1), (13 : 29 : 1)]\n    sage: E.integral_points()\n    [(-11 : -19 : 1), (-2 : 29 : 1), (4 : -16 : 1), (13 : -43 : 1), (507525709 : 11433453531221 : 1)]\n    \n::\n\n    sage: ...\n```",
    "created_at": "2011-11-26T22:00:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116457",
    "user": "https://github.com/jdemeyer"
}
```

Indentation at the top should probably be

```
r"""
Integral Points of Elliptic Curves over Number Fields

The following examples are from trac ticket #10152.  Previously Magma was finding ore 
integral points on these curves than Sage.  We resolved this issue.

EXAMPLES::

    sage: E = EllipticCurve('2082a1')
    sage: E.integral_points(algorithm = "old")
    [(-11 : 29 : 1), (-2 : 29 : 1), (4 : 11 : 1), (13 : 29 : 1)]
    sage: E.integral_points()
    [(-11 : -19 : 1), (-2 : 29 : 1), (4 : -16 : 1), (13 : -43 : 1), (507525709 : 11433453531221 : 1)]
    
::

    sage: ...
```



---

archive/issue_comments_116458.json:
```json
{
    "body": "Since setting this to positive review I have done more testing and was about to revert it to \"needs work\" myself anyway:  in a test of all curves to conductor 10000 I found that the new code appears to hang on 9615d1.",
    "created_at": "2011-11-27T12:05:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116458",
    "user": "https://github.com/JohnCremona"
}
```

Since setting this to positive review I have done more testing and was about to revert it to "needs work" myself anyway:  in a test of all curves to conductor 10000 I found that the new code appears to hang on 9615d1.



---

archive/issue_events_028458.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-11-29T11:52:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28458"
}
```



---

archive/issue_comments_116459.json:
```json
{
    "body": "See comments at #12095",
    "created_at": "2011-11-29T11:52:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116459",
    "user": "https://github.com/JohnCremona"
}
```

See comments at #12095



---

archive/issue_comments_116460.json:
```json
{
    "body": "Did you intentionally remove the reviewer and milestone fields?  If you did, I don't understand why.",
    "created_at": "2011-11-29T13:26:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116460",
    "user": "https://github.com/jdemeyer"
}
```

Did you intentionally remove the reviewer and milestone fields?  If you did, I don't understand why.



---

archive/issue_comments_116461.json:
```json
{
    "body": "For the patchbot: apply Trac10973.7.patch",
    "created_at": "2012-04-18T21:36:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116461",
    "user": "https://github.com/novoselt"
}
```

For the patchbot: apply Trac10973.7.patch



---

archive/issue_comments_116462.json:
```json
{
    "body": "Replying to [comment:31 jdemeyer]:\n> Did you intentionally remove the reviewer and milestone fields?  If you did, I don't understand why.\n\n\nI did not (at least, not intentionally).",
    "created_at": "2012-08-28T09:18:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116462",
    "user": "https://github.com/JohnCremona"
}
```

Replying to [comment:31 jdemeyer]:
> Did you intentionally remove the reviewer and milestone fields?  If you did, I don't understand why.


I did not (at least, not intentionally).



---

archive/issue_comments_116463.json:
```json
{
    "body": "Noting that one effect of this code is to improve integral-point-finding over QQ (though it is still not always correct, see comment 29 above,  we should at least add a stopgap trigger for integral points over QQ, to alert the user to known issues.  Example: without this patch, \n\n```\nsage: E = EllipticCurve(\"2082a1\")\nsage: P = E.gens()[0]\nsage: E.integral_points()\n[(-11 : 29 : 1), (-2 : 29 : 1), (4 : 11 : 1), (13 : 29 : 1)]\nsage: 13*P\n(507525709 : 11433453531221 : 1)\n```",
    "created_at": "2012-09-20T12:11:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116463",
    "user": "https://github.com/JohnCremona"
}
```

Noting that one effect of this code is to improve integral-point-finding over QQ (though it is still not always correct, see comment 29 above,  we should at least add a stopgap trigger for integral points over QQ, to alert the user to known issues.  Example: without this patch, 

```
sage: E = EllipticCurve("2082a1")
sage: P = E.gens()[0]
sage: E.integral_points()
[(-11 : 29 : 1), (-2 : 29 : 1), (4 : 11 : 1), (13 : 29 : 1)]
sage: 13*P
(507525709 : 11433453531221 : 1)
```



---

archive/issue_comments_116464.json:
```json
{
    "body": "patchbot, apply only trac_10973_v8.patch",
    "created_at": "2013-08-20T14:54:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116464",
    "user": "https://github.com/fchapoton"
}
```

patchbot, apply only trac_10973_v8.patch



---

archive/issue_comments_116465.json:
```json
{
    "body": "9 doctests are failing in sage/schemes/elliptic_curves/ell_int_points.py\n\nthe failing test in _c3 seems to be a matter of precision only\n\nThe other tests must be checked mathematically.",
    "created_at": "2013-08-20T15:12:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116465",
    "user": "https://github.com/fchapoton"
}
```

9 doctests are failing in sage/schemes/elliptic_curves/ell_int_points.py

the failing test in _c3 seems to be a matter of precision only

The other tests must be checked mathematically.



---

archive/issue_comments_116466.json:
```json
{
    "body": "apply only trac_10973_v8.patch",
    "created_at": "2013-08-20T15:52:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116466",
    "user": "https://github.com/fchapoton"
}
```

apply only trac_10973_v8.patch



---

archive/issue_comments_116467.json:
```json
{
    "body": "This ticket needs care from someone who can check the failing doctests.",
    "created_at": "2013-08-29T16:36:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116467",
    "user": "https://github.com/fchapoton"
}
```

This ticket needs care from someone who can check the failing doctests.



---

archive/issue_comments_116468.json:
```json
{
    "body": "apply only trac_10973_v8.patch",
    "created_at": "2013-09-01T08:15:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116468",
    "user": "https://github.com/fchapoton"
}
```

apply only trac_10973_v8.patch



---

archive/issue_comments_116469.json:
```json
{
    "body": "I have corrected the (short) failing doctests, and inserted the file in the doc.\n\nThere may remain one (very) long doctest failing.",
    "created_at": "2013-09-01T09:12:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116469",
    "user": "https://github.com/fchapoton"
}
```

I have corrected the (short) failing doctests, and inserted the file in the doc.

There may remain one (very) long doctest failing.



---

archive/issue_comments_116470.json:
```json
{
    "body": "I have corrected the last failing doctest\n\napply only trac_10973_v8.patch\n\nNeeds review !",
    "created_at": "2013-09-01T11:51:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116470",
    "user": "https://github.com/fchapoton"
}
```

I have corrected the last failing doctest

apply only trac_10973_v8.patch

Needs review !



---

archive/issue_comments_116471.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2013-09-01T11:51:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116471",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_events_028459.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-09-01T11:51:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28459"
}
```



---

archive/issue_comments_116472.json:
```json
{
    "body": "I checked that the new patch applies fine and passes most tests (all tests in sage/schemes/elliptic_curves except one, see below).  I want to do more testing now, to test on a large range of conductors that (over QQ) we find the same integral points as Magma.  In any case, I intend to run the new function on all curve of conductor <300000 in order to correct the files at http://homepages.warwick.ac.uk/staff/J.E.Cremona/ftp/data/, so I may as well do all that as part of this review.\n\nThis seems to be simply a formatting difference:\n\n```\nFailed example:\n    E.integral_points(L = [E((4,-4)),E((0,-20)),E((-4,-28)),E((-89/5,-637*a/25))])  # long time\nExpected:\n    [(-12 , 4 , 1), (-8 , 28 , 1), (-7 , -29 , 1), (-4 , -28 , 1),\n    (0 , -20 , 1), (1 , 17 , 1), (4 , -4 , 1), (8 , 4 , 1), (9 , -11 , 1),\n    (12 , -28 , 1), (16 , 52 , 1), (25 , -115 , 1), (32 , 172 , 1),\n    (44 , 284 , 1), (56 , -412 , 1), (84 , -764 , 1), (148 , 1796 , 1),\n    (208 , 2996 , 1), (372 , -7172 , 1), (1368 , -50596 , 1),\n    (1624 , -65444 , 1), (3264 , 186476 , 1)]\nGot:\n    [(-12 : 4 : 1), (-8 : 28 : 1), (-7 : -29 : 1), (-4 : -28 : 1), (0 : -20 : 1), (1 : 17 : 1), (4 : -4 : 1), (8 : 4 : 1), (9 : -11 : 1), (12 : -28 : 1), (16 : 52 : 1), (25 : -115 : 1), (32 : 172 : 1), (44 : 284 : 1), (56 : -412 : 1), (84 : -764 : 1), (148 : 1796 : 1), (208 : 2996 : 1), (372 : -7172 : 1), (1368 : -50596 : 1), (1624 : -65444 : 1), (3264 : 186476 : 1)]\n**********************************************************************\n1 item had failures:\n   1 of  14 in sage.schemes.elliptic_curves.ell_int_points.integral_points\n    [150 tests, 1 failure, 325.30 s]\n```",
    "created_at": "2013-09-04T11:12:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116472",
    "user": "https://github.com/JohnCremona"
}
```

I checked that the new patch applies fine and passes most tests (all tests in sage/schemes/elliptic_curves except one, see below).  I want to do more testing now, to test on a large range of conductors that (over QQ) we find the same integral points as Magma.  In any case, I intend to run the new function on all curve of conductor <300000 in order to correct the files at http://homepages.warwick.ac.uk/staff/J.E.Cremona/ftp/data/, so I may as well do all that as part of this review.

This seems to be simply a formatting difference:

```
Failed example:
    E.integral_points(L = [E((4,-4)),E((0,-20)),E((-4,-28)),E((-89/5,-637*a/25))])  # long time
Expected:
    [(-12 , 4 , 1), (-8 , 28 , 1), (-7 , -29 , 1), (-4 , -28 , 1),
    (0 , -20 , 1), (1 , 17 , 1), (4 , -4 , 1), (8 , 4 , 1), (9 , -11 , 1),
    (12 , -28 , 1), (16 , 52 , 1), (25 , -115 , 1), (32 , 172 , 1),
    (44 , 284 , 1), (56 , -412 , 1), (84 , -764 , 1), (148 , 1796 , 1),
    (208 , 2996 , 1), (372 , -7172 , 1), (1368 , -50596 , 1),
    (1624 , -65444 , 1), (3264 , 186476 , 1)]
Got:
    [(-12 : 4 : 1), (-8 : 28 : 1), (-7 : -29 : 1), (-4 : -28 : 1), (0 : -20 : 1), (1 : 17 : 1), (4 : -4 : 1), (8 : 4 : 1), (9 : -11 : 1), (12 : -28 : 1), (16 : 52 : 1), (25 : -115 : 1), (32 : 172 : 1), (44 : 284 : 1), (56 : -412 : 1), (84 : -764 : 1), (148 : 1796 : 1), (208 : 2996 : 1), (372 : -7172 : 1), (1368 : -50596 : 1), (1624 : -65444 : 1), (3264 : 186476 : 1)]
**********************************************************************
1 item had failures:
   1 of  14 in sage.schemes.elliptic_curves.ell_int_points.integral_points
    [150 tests, 1 failure, 325.30 s]
```



---

archive/issue_comments_116473.json:
```json
{
    "body": "More seriously, \n\n```\nsage: E = EllipticCurve('9615d1)\nsage: E.integral_points()\n```\nseems to hang, thought I don't know why, while \n\n```\nsage: E = EllipticCurve('9615d1)\nsage: E.integral_points(algorithm=\"old\")\n```\nis fine.  I think that this will need investigating before this can be merged.  Hence \"needs work\".\n\nI systematically checked all curves with conductor < 10000 with Magma V2.19-8.  Note that thaere have been a lot of improvements in Magma's algorithm, due to Steve DOnelly but not published, since this ticket was opened.  Not just bugfixes; but only over Q.\n\nWith the old algorithm in Sage for N<10000 there are only 3 cases where Sage missed integral points.  Of these, 3 (6104b1, 8470g1 and 8470g2), are very easily fixed, simply by increasing the precision of the function `integral_points_with_bounded_mw_coeffs` from 100 to 200, thogh I have not done the necessary analysis to compute the actual precision required.  The other one is 2082a1, where the MW group is cyclic of rank 1 and the integral multiples of the generator P are n*P for n=1,2,3,4,13, but the bound computed by the algorithm is 12.  It should be possible to find that bug using a detailed analysis of the implementation, but I have not done that yet (though I did find one small bug, in the formula for mu there should be b2/12 and not b2).",
    "created_at": "2013-09-05T09:55:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116473",
    "user": "https://github.com/JohnCremona"
}
```

More seriously, 

```
sage: E = EllipticCurve('9615d1)
sage: E.integral_points()
```
seems to hang, thought I don't know why, while 

```
sage: E = EllipticCurve('9615d1)
sage: E.integral_points(algorithm="old")
```
is fine.  I think that this will need investigating before this can be merged.  Hence "needs work".

I systematically checked all curves with conductor < 10000 with Magma V2.19-8.  Note that thaere have been a lot of improvements in Magma's algorithm, due to Steve DOnelly but not published, since this ticket was opened.  Not just bugfixes; but only over Q.

With the old algorithm in Sage for N<10000 there are only 3 cases where Sage missed integral points.  Of these, 3 (6104b1, 8470g1 and 8470g2), are very easily fixed, simply by increasing the precision of the function `integral_points_with_bounded_mw_coeffs` from 100 to 200, thogh I have not done the necessary analysis to compute the actual precision required.  The other one is 2082a1, where the MW group is cyclic of rank 1 and the integral multiples of the generator P are n*P for n=1,2,3,4,13, but the bound computed by the algorithm is 12.  It should be possible to find that bug using a detailed analysis of the implementation, but I have not done that yet (though I did find one small bug, in the formula for mu there should be b2/12 and not b2).



---

archive/issue_comments_116474.json:
```json
{
    "body": "I have done some debugging, not yet complete but I'll be away for a while and wanted to record what I found so far.\n\n2082a1:   the old code is not nearly careful enough about computing elliptic logs and periods to high enough precision, and I have worked a bit on improving that, but in this example it reduces the coefficient bound first from `10^44` to 34 (that looks OK to me) and then to 12, which is not OK since the actual bound is 13.  And I can see what is done wrong:  in Cohen's notation you need a lower bound on `d(L,0)^2` which is greater than `T^2+Q`, and in this second step it is not;  the code makes an adjustment, which is not (as far as I can see) justified.  Instead one should increase the scaling parameter C.  I found that multiplying C by 100 is OK, but yields a worse bound on the coefficients, namely 56.  If one did check all 34 multiples of the generator (the rank is 1) then that would be OK.  But the new code here manages to get successive bounds `10^40`, 53, 18 and uses 18 which works better than 34 would have done!  The two implementations need further careful comparison.\n\n9615d1, which has rank 2.  The new code finds a bound of 162, and the apparent hanging happens when testing all multiples with coefficients up to that bound.  This is what is taking a very long time: there are 52812 points to check, and many will have huge height.  This is a good illustration of the benefit of using floating point at this stage, since on the same curve if you run `integral_points_with_bounded_mw_coeffs(E,E.gens(),162)` it finishes quickly (and finds all the points, namely the 15 which Magma also finds).  On the other hand the old code obtains a coefficient bound of only 18, which is good enough,  so it is necessary to find out why the new code does so much worse (bound 162).\n\nI don't feel prepared to remove the \"needs work\" tag just yet.",
    "created_at": "2013-09-06T11:27:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116474",
    "user": "https://github.com/JohnCremona"
}
```

I have done some debugging, not yet complete but I'll be away for a while and wanted to record what I found so far.

2082a1:   the old code is not nearly careful enough about computing elliptic logs and periods to high enough precision, and I have worked a bit on improving that, but in this example it reduces the coefficient bound first from `10^44` to 34 (that looks OK to me) and then to 12, which is not OK since the actual bound is 13.  And I can see what is done wrong:  in Cohen's notation you need a lower bound on `d(L,0)^2` which is greater than `T^2+Q`, and in this second step it is not;  the code makes an adjustment, which is not (as far as I can see) justified.  Instead one should increase the scaling parameter C.  I found that multiplying C by 100 is OK, but yields a worse bound on the coefficients, namely 56.  If one did check all 34 multiples of the generator (the rank is 1) then that would be OK.  But the new code here manages to get successive bounds `10^40`, 53, 18 and uses 18 which works better than 34 would have done!  The two implementations need further careful comparison.

9615d1, which has rank 2.  The new code finds a bound of 162, and the apparent hanging happens when testing all multiples with coefficients up to that bound.  This is what is taking a very long time: there are 52812 points to check, and many will have huge height.  This is a good illustration of the benefit of using floating point at this stage, since on the same curve if you run `integral_points_with_bounded_mw_coeffs(E,E.gens(),162)` it finishes quickly (and finds all the points, namely the 15 which Magma also finds).  On the other hand the old code obtains a coefficient bound of only 18, which is good enough,  so it is necessary to find out why the new code does so much worse (bound 162).

I don't feel prepared to remove the "needs work" tag just yet.



---

archive/issue_comments_116475.json:
```json
{
    "body": "I have made a new patch, correcting the one failing doctest",
    "created_at": "2013-10-19T19:49:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116475",
    "user": "https://github.com/fchapoton"
}
```

I have made a new patch, correcting the one failing doctest



---

archive/issue_comments_116476.json:
```json
{
    "body": "for the **patchbots**:\napply trac_10973_v8.patch",
    "created_at": "2013-10-22T11:45:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116476",
    "user": "https://github.com/fchapoton"
}
```

for the **patchbots**:
apply trac_10973_v8.patch



---

archive/issue_comments_116477.json:
```json
{
    "body": "replaces all previous patches ; rebased on 5.11",
    "created_at": "2013-10-23T18:25:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116477",
    "user": "https://github.com/fchapoton"
}
```

replaces all previous patches ; rebased on 5.11



---

archive/issue_comments_116478.json:
```json
{
    "body": "Attachment [trac_10973_v8.patch](tarball://root/attachments/some-uuid/ticket10973/trac_10973_v8.patch) by @fchapoton created at 2013-10-23 18:26:55\n\nI have made a new patch, that should work with 5.13.beta1\n\nfor the patchbots: apply trac_10973_v8.patch",
    "created_at": "2013-10-23T18:26:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116478",
    "user": "https://github.com/fchapoton"
}
```

Attachment [trac_10973_v8.patch](tarball://root/attachments/some-uuid/ticket10973/trac_10973_v8.patch) by @fchapoton created at 2013-10-23 18:26:55

I have made a new patch, that should work with 5.13.beta1

for the patchbots: apply trac_10973_v8.patch



---

archive/issue_comments_116479.json:
```json
{
    "body": "Last 10 new commits:",
    "created_at": "2013-12-18T10:16:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116479",
    "user": "https://github.com/JohnCremona"
}
```

Last 10 new commits:



---

archive/issue_comments_116480.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2013-12-18T10:16:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116480",
    "user": "https://github.com/JohnCremona"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_116481.json:
```json
{
    "body": "Why is this \"needs works\" ?",
    "created_at": "2013-12-20T09:55:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116481",
    "user": "https://github.com/fchapoton"
}
```

Why is this "needs works" ?



---

archive/issue_comments_116482.json:
```json
{
    "body": "Replying to [comment:50 chapoton]:\n> Why is this \"needs works\" ?\n\n\nBecause I am actively working on it!  Seriously, comparing in detail Smart's paper and book and Nook's account in his thesis, I have found two things: (1) Smart does not deal properly with torsion generators, Nook does and that is in this code but I think I might be able to do better; (2) more seriously, Smart derived an upper bound for `|x(P)^2\\psi(P)|` (relative to each embedding) which only valid for points with `|x(P)|` bounded below by some quantity (in the appropriate embedding), and this means that there are points, possible integral, which fail this condition and need to be considered separately.  Over Q that is easily done by searching on a finite x-interval, but over a general number field it is not clear to me that the \"missing\" points are in a finite region.  However,  I am working on a bound (which certainly exists) for `|x(P)^2\\psi(P)|` valid for all complex points with any embedding, and when that is done I will be changing some of the constants in the code.\n\nI am not expecting the output to be different in any of the examples (but who knows), but I cannot let this in when I have identified but not yet fixed this mathematical bug!",
    "created_at": "2013-12-20T10:49:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116482",
    "user": "https://github.com/JohnCremona"
}
```

Replying to [comment:50 chapoton]:
> Why is this "needs works" ?


Because I am actively working on it!  Seriously, comparing in detail Smart's paper and book and Nook's account in his thesis, I have found two things: (1) Smart does not deal properly with torsion generators, Nook does and that is in this code but I think I might be able to do better; (2) more seriously, Smart derived an upper bound for `|x(P)^2\psi(P)|` (relative to each embedding) which only valid for points with `|x(P)|` bounded below by some quantity (in the appropriate embedding), and this means that there are points, possible integral, which fail this condition and need to be considered separately.  Over Q that is easily done by searching on a finite x-interval, but over a general number field it is not clear to me that the "missing" points are in a finite region.  However,  I am working on a bound (which certainly exists) for `|x(P)^2\psi(P)|` valid for all complex points with any embedding, and when that is done I will be changing some of the constants in the code.

I am not expecting the output to be different in any of the examples (but who knows), but I cannot let this in when I have identified but not yet fixed this mathematical bug!



---

archive/issue_events_028460.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28460"
}
```



---

archive/issue_events_028461.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28461"
}
```



---

archive/issue_events_028462.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28462"
}
```



---

archive/issue_events_028463.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28463"
}
```



---

archive/issue_comments_116483.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2014-07-14T13:21:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116483",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_events_028464.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28464"
}
```



---

archive/issue_events_028465.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28465"
}
```



---

archive/issue_events_028466.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-09-11T20:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28466"
}
```



---

archive/issue_events_028467.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-09-11T20:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28467"
}
```



---

archive/issue_comments_116484.json:
```json
{
    "body": "rebased\n\n---\nNew commits:",
    "created_at": "2015-09-11T20:41:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116484",
    "user": "https://github.com/fchapoton"
}
```

rebased

---
New commits:



---

archive/issue_comments_116485.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-10-21T15:18:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116485",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_events_028468.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-21T15:19:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28468"
}
```



---

archive/issue_events_028469.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-21T15:19:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28469"
}
```



---

archive/issue_comments_116486.json:
```json
{
    "body": "New commits:",
    "created_at": "2016-02-18T09:48:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116486",
    "user": "https://github.com/JohnCremona"
}
```

New commits:



---

archive/issue_events_028470.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-05T12:18:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28470"
}
```



---

archive/issue_events_028471.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-05T12:18:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28471"
}
```



---

archive/issue_comments_116487.json:
```json
{
    "body": "rebased again\n\n---\nNew commits:",
    "created_at": "2016-08-05T12:18:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116487",
    "user": "https://github.com/fchapoton"
}
```

rebased again

---
New commits:



---

archive/issue_comments_116488.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-08-05T14:19:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116488",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_116489.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-12-15T19:21:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116489",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_116490.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-03-16T13:42:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116490",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_116491.json:
```json
{
    "body": "Thanks for keeping this updated.  I just have not been able to finish work on it (there are some mathematical issues).",
    "created_at": "2017-03-16T14:36:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116491",
    "user": "https://github.com/JohnCremona"
}
```

Thanks for keeping this updated.  I just have not been able to finish work on it (there are some mathematical issues).



---

archive/issue_comments_116492.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2018-08-22T18:57:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116492",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_116493.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2019-01-04T09:32:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116493",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_116494.json:
```json
{
    "body": "I have merged the current working branch in with 8.6.beta1 and resolved the conflicts.\n\nI also made the following test:  there are 207 curves in the database (conductors < 400000, curves defined over QQ) where the old E.integral_points() missed integral points, as I found out by running every curve through Magma's IntegralPoints function.  For every one of these the current version finds all the points (the same as Magma does).\n\nHowever I still think that this code is too slow over QQ and intend to make a new branch (and a new ticket #27015) to fix the problems over QQ.  When that is done I will get back to this branch.",
    "created_at": "2019-01-04T09:36:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116494",
    "user": "https://github.com/JohnCremona"
}
```

I have merged the current working branch in with 8.6.beta1 and resolved the conflicts.

I also made the following test:  there are 207 curves in the database (conductors < 400000, curves defined over QQ) where the old E.integral_points() missed integral points, as I found out by running every curve through Magma's IntegralPoints function.  For every one of these the current version finds all the points (the same as Magma does).

However I still think that this code is too slow over QQ and intend to make a new branch (and a new ticket #27015) to fix the problems over QQ.  When that is done I will get back to this branch.



---

archive/issue_comments_116495.json:
```json
{
    "body": "Correction to the above:  it was true that there were 207 curves whose lists of integral points at http://johncremona.github.io/ecdata/ (since updated) were incomplete, and hence also the integral points stored in the LMFDB (not yet updated).  However, in all but 3 of these cases (2082a1, 23808e1, 285928a1) the current Sage (8.5) finds all the points.  So previous bug fixes have helped.\n\nIn the negative direction there are some more discrepancies, since we now (using vanilla 8.5) miss x=20829482 on 1701d1 and x=176 and 179 on 1848f1.  (There will surely be more).  \n\nThe complete list of missed points with Sage 8.5 is: 12 points on 11 curves:\n\n 1701d1, 1848f1, 2082a1, 3510n1, 6514b1,\n 177211a1,\n 23808e1,\n 268740c1,\n 285928a1,\n 66432f1,\n 92554e1,\n\nmissing 1 point in each case except 2 points on 1848f1.\n\n#27015 will deal with this.",
    "created_at": "2019-01-04T12:13:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116495",
    "user": "https://github.com/JohnCremona"
}
```

Correction to the above:  it was true that there were 207 curves whose lists of integral points at http://johncremona.github.io/ecdata/ (since updated) were incomplete, and hence also the integral points stored in the LMFDB (not yet updated).  However, in all but 3 of these cases (2082a1, 23808e1, 285928a1) the current Sage (8.5) finds all the points.  So previous bug fixes have helped.

In the negative direction there are some more discrepancies, since we now (using vanilla 8.5) miss x=20829482 on 1701d1 and x=176 and 179 on 1848f1.  (There will surely be more).  

The complete list of missed points with Sage 8.5 is: 12 points on 11 curves:

 1701d1, 1848f1, 2082a1, 3510n1, 6514b1,
 177211a1,
 23808e1,
 268740c1,
 285928a1,
 66432f1,
 92554e1,

missing 1 point in each case except 2 points on 1848f1.

#27015 will deal with this.



---

archive/issue_comments_116496.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2019-02-19T19:36:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116496",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_116497.json:
```json
{
    "body": "So what's the status here ? Should we just fix the few failing doctests ?",
    "created_at": "2019-02-19T19:38:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116497",
    "user": "https://github.com/fchapoton"
}
```

So what's the status here ? Should we just fix the few failing doctests ?



---

archive/issue_events_028472.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-02-19T19:38:29Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28472"
}
```



---

archive/issue_events_028473.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-02-19T19:38:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28473"
}
```



---

archive/issue_comments_116498.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2019-02-20T08:26:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116498",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_116499.json:
```json
{
    "body": "I have a current branch called intptsQ which I made good progress on in December and will get back to.",
    "created_at": "2019-02-20T09:16:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116499",
    "user": "https://github.com/JohnCremona"
}
```

I have a current branch called intptsQ which I made good progress on in December and will get back to.



---

archive/issue_comments_116500.json:
```json
{
    "body": "Ticket retargeted after milestone closed (if you don't believe this ticket is appropriate for the Sage 8.8 release please retarget manually)",
    "created_at": "2019-03-25T10:56:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116500",
    "user": "https://github.com/embray"
}
```

Ticket retargeted after milestone closed (if you don't believe this ticket is appropriate for the Sage 8.8 release please retarget manually)



---

archive/issue_events_028474.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:56:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28474"
}
```



---

archive/issue_events_028475.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:56:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28475"
}
```



---

archive/issue_comments_116501.json:
```json
{
    "body": "Tickets still needing working or clarification should be moved to the next release milestone at the soonest (please feel free to revert if you think the ticket is close to being resolved).",
    "created_at": "2019-06-14T14:50:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116501",
    "user": "https://github.com/embray"
}
```

Tickets still needing working or clarification should be moved to the next release milestone at the soonest (please feel free to revert if you think the ticket is close to being resolved).



---

archive/issue_events_028476.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:50:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28476"
}
```



---

archive/issue_events_028477.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:50:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28477"
}
```



---

archive/issue_comments_116502.json:
```json
{
    "body": "Ticket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116502",
    "user": "https://github.com/embray"
}
```

Ticket retargeted after milestone closed



---

archive/issue_events_028478.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28478"
}
```



---

archive/issue_events_028479.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28479"
}
```



---

archive/issue_comments_116503.json:
```json
{
    "body": "Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116503",
    "user": "https://github.com/mkoeppe"
}
```

Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_028480.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28480"
}
```



---

archive/issue_events_028481.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28481"
}
```



---

archive/issue_events_028482.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28482"
}
```



---

archive/issue_events_028483.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28483"
}
```



---

archive/issue_comments_116504.json:
```json
{
    "body": "Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116504",
    "user": "https://github.com/mkoeppe"
}
```

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_028484.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28484"
}
```



---

archive/issue_events_028485.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28485"
}
```



---

archive/issue_events_028486.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28486"
}
```



---

archive/issue_events_028487.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28487"
}
```



---

archive/issue_comments_116505.json:
```json
{
    "body": "Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/10973#issuecomment-116505",
    "user": "https://github.com/mkoeppe"
}
```

Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_028488.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28488"
}
```



---

archive/issue_events_028489.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28489"
}
```



---

archive/issue_events_028490.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28490"
}
```



---

archive/issue_events_028491.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28491"
}
```



---

archive/issue_events_028492.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28492"
}
```



---

archive/issue_events_028493.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/10973",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/10973#event-28493"
}
```
