# Issue 31130: Latte error raising does not work

archive/issues_030893.json:
```json
{
    "assignees": [],
    "body": "From #31046:\n\n```\nsage: P = Polyhedron(rays=[[1,0],[0,1]])                                                                                                                                            \nsage: P._volume_latte()                                                                                                                                                             \n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n<ipython-input-2-abc8351118c1> in <module>\n----> 1 P._volume_latte()\n\n~/Applications/sage/local/lib/python3.8/site-packages/sage/geometry/polyhedron/base.py in _volume_latte(self, verbose, algorithm, **kwargs)\n   7888             raise ValueError(\"LattE integrale cannot be applied over inexact rings\")\n   7889         else:\n-> 7890             return integrate(self.cdd_Hrepresentation(), algorithm=algorithm, cdd=True, verbose=verbose, **kwargs)\n   7891 \n   7892     def _volume_normaliz(self, measure='induced'):\n\n~/Applications/sage/local/lib/python3.8/site-packages/sage/interfaces/latte.py in integrate(arg, polynomial, algorithm, raw_output, verbose, **kwds)\n    372             err = \", see error message above\"\n    373         else:\n--> 374             err = \":\\n\" + err\n    375         raise RuntimeError(\"LattE integrale program failed (exit code {})\".format(ret_code) + err.strip())\n    376 \n\nTypeError: can only concatenate str (not \"bytes\") to str\n```\n\nLatte probably raises an error here, which we fail to communicate. The `RuntimeError` was never tested.\n\nKeywords: **latte, runtime error**\n\nBranch: **[7f8815a](https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31130_\n\n",
    "closed_at": "2021-01-04T22:37:05Z",
    "created_at": "2020-12-29T09:33:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Latte error raising does not work",
    "type": "issue",
    "updated_at": "2021-03-05T08:02:24Z",
    "url": "https://github.com/sagemath/sage/issues/31130",
    "user": "https://github.com/kliem"
}
```
From #31046:

```
sage: P = Polyhedron(rays=[[1,0],[0,1]])                                                                                                                                            
sage: P._volume_latte()                                                                                                                                                             
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-2-abc8351118c1> in <module>
----> 1 P._volume_latte()

~/Applications/sage/local/lib/python3.8/site-packages/sage/geometry/polyhedron/base.py in _volume_latte(self, verbose, algorithm, **kwargs)
   7888             raise ValueError("LattE integrale cannot be applied over inexact rings")
   7889         else:
-> 7890             return integrate(self.cdd_Hrepresentation(), algorithm=algorithm, cdd=True, verbose=verbose, **kwargs)
   7891 
   7892     def _volume_normaliz(self, measure='induced'):

~/Applications/sage/local/lib/python3.8/site-packages/sage/interfaces/latte.py in integrate(arg, polynomial, algorithm, raw_output, verbose, **kwds)
    372             err = ", see error message above"
    373         else:
--> 374             err = ":\n" + err
    375         raise RuntimeError("LattE integrale program failed (exit code {})".format(ret_code) + err.strip())
    376 

TypeError: can only concatenate str (not "bytes") to str
```

Latte probably raises an error here, which we fail to communicate. The `RuntimeError` was never tested.

Keywords: **latte, runtime error**

Branch: **[7f8815a](https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d)**

Reviewer: **Matthias Koeppe**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/31130_





---

archive/issue_events_410958.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-12-29T09:33:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410958"
}
```



---

archive/issue_events_410959.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-12-29T09:33:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410959"
}
```



---

archive/issue_events_410960.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-12-29T09:33:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410960"
}
```



---

archive/issue_events_410961.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-12-29T09:33:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410961"
}
```



---

archive/issue_comments_501880.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9b2a548c6cb78aae9240be82f75c946475fde8f7\">9b2a548</a></td><td><code>raise correct runtime error on latte failure</code></td></tr></table>\n",
    "created_at": "2020-12-29T09:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501880",
    "user": "https://github.com/kliem"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9b2a548c6cb78aae9240be82f75c946475fde8f7">9b2a548</a></td><td><code>raise correct runtime error on latte failure</code></td></tr></table>




---

archive/issue_comments_501881.json:
```json
{
    "body": "Branch: **[public/31130](https://github.com/sagemath/sagetrac-mirror/tree/public/31130)**",
    "created_at": "2020-12-29T09:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501881",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/31130](https://github.com/sagemath/sagetrac-mirror/tree/public/31130)**



---

archive/issue_comments_501882.json:
```json
{
    "body": "Author: **Jonathan Kliem**",
    "created_at": "2020-12-29T09:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501882",
    "user": "https://github.com/kliem"
}
```

Author: **Jonathan Kliem**



---

archive/issue_comments_501883.json:
```json
{
    "body": "Commit: **[9b2a548](https://github.com/sagemath/sagetrac-mirror/commit/9b2a548c6cb78aae9240be82f75c946475fde8f7)**",
    "created_at": "2020-12-29T09:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501883",
    "user": "https://github.com/kliem"
}
```

Commit: **[9b2a548](https://github.com/sagemath/sagetrac-mirror/commit/9b2a548c6cb78aae9240be82f75c946475fde8f7)**



---

archive/issue_events_410962.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-12-29T09:56:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410962"
}
```



---

archive/issue_comments_501884.json:
```json
{
    "body": "Changed keywords from **** to **latte, runtime error**",
    "created_at": "2020-12-29T09:56:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501884",
    "user": "https://github.com/kliem"
}
```

Changed keywords from **** to **latte, runtime error**



---

archive/issue_comments_501885.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-12-29T18:17:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501885",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_410963.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-29T18:17:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410963"
}
```



---

archive/issue_events_410964.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-29T18:17:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410964"
}
```



---

archive/issue_comments_501886.json:
```json
{
    "body": "Changed commit from **[9b2a548](https://github.com/sagemath/sagetrac-mirror/commit/9b2a548c6cb78aae9240be82f75c946475fde8f7)** to **[7f8815a](https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d)**",
    "created_at": "2020-12-29T20:57:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501886",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9b2a548](https://github.com/sagemath/sagetrac-mirror/commit/9b2a548c6cb78aae9240be82f75c946475fde8f7)** to **[7f8815a](https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d)**



---

archive/issue_comments_501887.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d\">7f8815a</a></td><td><code>actually testing the bug</code></td></tr></table>\n",
    "created_at": "2020-12-29T20:57:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501887",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d">7f8815a</a></td><td><code>actually testing the bug</code></td></tr></table>




---

archive/issue_events_410965.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2020-12-29T20:57:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410965"
}
```



---

archive/issue_events_410966.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2020-12-29T20:57:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410966"
}
```



---

archive/issue_comments_501888.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThanks and sorry: The second doctest that I added did not contain an error message and therefore is working fine with and without this ticket.\n\nI replaced by the doctest from this ticket that includes an error message now (and wasn't working before).",
    "created_at": "2020-12-29T20:59:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501888",
    "user": "https://github.com/kliem"
}
```

<a id='comment:5'>Comment 5:</a>
Thanks and sorry: The second doctest that I added did not contain an error message and therefore is working fine with and without this ticket.

I replaced by the doctest from this ticket that includes an error message now (and wasn't working before).



---

archive/issue_events_410967.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-29T21:10:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410967"
}
```



---

archive/issue_events_410968.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-29T21:10:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410968"
}
```



---

archive/issue_comments_501889.json:
```json
{
    "body": "Changed branch from **[public/31130](https://github.com/sagemath/sagetrac-mirror/tree/public/31130)** to **[7f8815a](https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d)**",
    "created_at": "2021-01-04T22:37:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501889",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/31130](https://github.com/sagemath/sagetrac-mirror/tree/public/31130)** to **[7f8815a](https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d)**



---

archive/issue_events_410969.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-01-04T22:37:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410969"
}
```



---

archive/issue_events_410970.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "91f633b5f09f96f3cd352e735480ebc82cf8a63c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-01-04T22:37:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31130#event-410970"
}
```



---

archive/issue_comments_501890.json:
```json
{
    "body": "Changed commit from **[7f8815a](https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d)** to none",
    "created_at": "2021-03-05T07:54:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501890",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[7f8815a](https://github.com/sagemath/sagetrac-mirror/commit/7f8815a04e2d19d4469e835c0543307c702ff26d)** to none



---

archive/issue_comments_501891.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nwhat about #31046 now ? can it be closed ?",
    "created_at": "2021-03-05T07:54:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501891",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'>Comment 8:</a>
what about #31046 now ? can it be closed ?



---

archive/issue_comments_501892.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI don't think so. #31046 reports some doctest failures. But we don't know which ones, because there is no proper error message. Now that this ticket is closed, we can hope that we can a proper report of this message.\n\nWell, we could also close #31046 and wait until the error shows up again.",
    "created_at": "2021-03-05T08:02:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31130#issuecomment-501892",
    "user": "https://github.com/kliem"
}
```

<a id='comment:9'>Comment 9:</a>
I don't think so. #31046 reports some doctest failures. But we don't know which ones, because there is no proper error message. Now that this ticket is closed, we can hope that we can a proper report of this message.

Well, we could also close #31046 and wait until the error shows up again.
