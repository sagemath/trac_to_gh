# Issue 31732: ManifoldSubset: New methods declare_empty, declare_nonempty, is_empty, has_defined_points, open_cover_family

archive/issues_031495.json:
```json
{
    "assignees": [],
    "body": "A subset is declared empty by giving it (or any superset) an open cover that is an empty family.\n\nA subset is declared nonempty by defining a point on it (or on any subset).\n\n\nDepends on #31718\nDepends on #31727\n\nCC:  @egourgoulhon @mjungmath @tscrim\n\nBranch/Commit: **[359dde1](https://github.com/sagemath/sagetrac-mirror/commit/359dde1acb2c64a04341bb8d5ffeaa58c3d51dae)**\n\nReviewer: **Michael Jung**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31732_\n\n",
    "closed_at": "2021-06-21T21:17:29Z",
    "created_at": "2021-04-26T18:46:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ManifoldSubset: New methods declare_empty, declare_nonempty, is_empty, has_defined_points, open_cover_family",
    "type": "issue",
    "updated_at": "2021-06-21T21:17:29Z",
    "url": "https://github.com/sagemath/sage/issues/31732",
    "user": "https://github.com/mkoeppe"
}
```
A subset is declared empty by giving it (or any superset) an open cover that is an empty family.

A subset is declared nonempty by defining a point on it (or on any subset).


Depends on #31718
Depends on #31727

CC:  @egourgoulhon @mjungmath @tscrim

Branch/Commit: **[359dde1](https://github.com/sagemath/sagetrac-mirror/commit/359dde1acb2c64a04341bb8d5ffeaa58c3d51dae)**

Reviewer: **Michael Jung**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/31732_





---

archive/issue_events_419440.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:46:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419440"
}
```



---

archive/issue_events_419441.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:46:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
    "label_color": "0000ff",
    "label_name": "component: manifolds",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419441"
}
```



---

archive/issue_events_419442.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:46:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419442"
}
```



---

archive/issue_events_419443.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:46:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419443"
}
```



---

archive/issue_events_419444.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:53:16Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "title_is": "ManifoldSubset: New methods declare_empty, declare_nonempty, is_empty, has_defined_points, open_cover_family",
    "title_was": "ManifoldSubset: New methods declare_empty, declare_nonempty, is_empty, has_defined_points",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419444"
}
```



---

archive/issue_comments_513455.json:
```json
{
    "body": "Changed dependencies from **#31727** to **#31718**",
    "created_at": "2021-04-26T18:53:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513455",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#31727** to **#31718**



---

archive/issue_comments_513456.json:
```json
{
    "body": "Branch: **[u/mkoeppe/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family)**",
    "created_at": "2021-04-26T19:19:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513456",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family)**



---

archive/issue_comments_513457.json:
```json
{
    "body": "Commit: **[1b5e5ac](https://github.com/sagemath/sagetrac-mirror/commit/1b5e5ace5e5cc01820d4e85e08f81d1662e5511d)**",
    "created_at": "2021-04-26T19:22:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513457",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[1b5e5ac](https://github.com/sagemath/sagetrac-mirror/commit/1b5e5ace5e5cc01820d4e85e08f81d1662e5511d)**



---

archive/issue_comments_513458.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/78cc27a8e439c542c2f03958e8e0888b0551a979\">78cc27a</a></td><td><code>ManifoldSubset.open_covers: Change to generator, add optional arg 'trivial'; update uses</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e026e7a68095577277de1b32df0f8c446b594aa7\">e026e7a</a></td><td><code>ManifoldSubset.subset_digraph: Use open_covers method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5a072cf1c4041f98456d7967857d048e675128f9\">5a072cf</a></td><td><code>Merge #31718</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/83a9f47566a1025f02c1c286eb3c03046432f0d8\">83a9f47</a></td><td><code>ManifoldSubset.open_cover_family: New</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a18357584a8f36b9db51060e3a6577a424cc7535\">a183575</a></td><td><code>ManifoldSubset.{declare_empty,declare_nonempty,is_empty,has_defined_points}: New</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4ad562d8b15f65f45cc45848c3abfa10a088843a\">4ad562d</a></td><td><code>ManifoldSubset.{sub,super}set_{digraph,poset}: Add option 'points'</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cec7fa23dfdf81f5436b432d46b791f8c820938b\">cec7fa2</a></td><td><code>ManifoldSubset.open_covers: Add option supersets; use it to fix is_empty</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c40ec033530849699c387ad7def2d9ba7f6ad653\">c40ec03</a></td><td><code>ManifoldSubset.open_cover_family: Add option supersets</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a066387b0081ffbb2274cfd80c043491ec8cf1f8\">a066387</a></td><td><code>Fix doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1b5e5ace5e5cc01820d4e85e08f81d1662e5511d\">1b5e5ac</a></td><td><code>ManifoldSubset.declare_empty: Add plot</code></td></tr></table>\n",
    "created_at": "2021-04-26T19:22:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513458",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/78cc27a8e439c542c2f03958e8e0888b0551a979">78cc27a</a></td><td><code>ManifoldSubset.open_covers: Change to generator, add optional arg 'trivial'; update uses</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e026e7a68095577277de1b32df0f8c446b594aa7">e026e7a</a></td><td><code>ManifoldSubset.subset_digraph: Use open_covers method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5a072cf1c4041f98456d7967857d048e675128f9">5a072cf</a></td><td><code>Merge #31718</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/83a9f47566a1025f02c1c286eb3c03046432f0d8">83a9f47</a></td><td><code>ManifoldSubset.open_cover_family: New</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a18357584a8f36b9db51060e3a6577a424cc7535">a183575</a></td><td><code>ManifoldSubset.{declare_empty,declare_nonempty,is_empty,has_defined_points}: New</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4ad562d8b15f65f45cc45848c3abfa10a088843a">4ad562d</a></td><td><code>ManifoldSubset.{sub,super}set_{digraph,poset}: Add option 'points'</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cec7fa23dfdf81f5436b432d46b791f8c820938b">cec7fa2</a></td><td><code>ManifoldSubset.open_covers: Add option supersets; use it to fix is_empty</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c40ec033530849699c387ad7def2d9ba7f6ad653">c40ec03</a></td><td><code>ManifoldSubset.open_cover_family: Add option supersets</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a066387b0081ffbb2274cfd80c043491ec8cf1f8">a066387</a></td><td><code>Fix doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1b5e5ace5e5cc01820d4e85e08f81d1662e5511d">1b5e5ac</a></td><td><code>ManifoldSubset.declare_empty: Add plot</code></td></tr></table>




---

archive/issue_events_419445.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T19:22:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419445"
}
```



---

archive/issue_comments_513459.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI am not sure whether this is a good point to hook in, but long-term it would be nice to things like Cech cohomology. This construction needs \"good covers\" which are basically given by contractible intersections. Currently, we cannot determine a subset being contractible (or even connected). But perhaps this is something to consider at this point already?",
    "created_at": "2021-04-26T21:38:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513459",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:4" align="right">Comment 4</div>

I am not sure whether this is a good point to hook in, but long-term it would be nice to things like Cech cohomology. This construction needs "good covers" which are basically given by contractible intersections. Currently, we cannot determine a subset being contractible (or even connected). But perhaps this is something to consider at this point already?



---

archive/issue_comments_513460.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nAn idea to do things like this would be to try to break complicated domains into \"simple\" subsets (for which we can algorithmically verify that they are simply connected) and to then use algebraic topology (e.g., simplicial complexes) on the subsets. At this point, I don't know what properties other than emptiness/nonemptiness really needs to be encoded on the level of a single subset.",
    "created_at": "2021-04-26T22:54:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513460",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

An idea to do things like this would be to try to break complicated domains into "simple" subsets (for which we can algorithmically verify that they are simply connected) and to then use algebraic topology (e.g., simplicial complexes) on the subsets. At this point, I don't know what properties other than emptiness/nonemptiness really needs to be encoded on the level of a single subset.



---

archive/issue_comments_513461.json:
```json
{
    "body": "Changed commit from **[1b5e5ac](https://github.com/sagemath/sagetrac-mirror/commit/1b5e5ace5e5cc01820d4e85e08f81d1662e5511d)** to **[48b2700](https://github.com/sagemath/sagetrac-mirror/commit/48b2700e8e8990dbc3f9b358a6799e3c6b960854)**",
    "created_at": "2021-04-26T23:09:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513461",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1b5e5ac](https://github.com/sagemath/sagetrac-mirror/commit/1b5e5ace5e5cc01820d4e85e08f81d1662e5511d)** to **[48b2700](https://github.com/sagemath/sagetrac-mirror/commit/48b2700e8e8990dbc3f9b358a6799e3c6b960854)**



---

archive/issue_comments_513462.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/48b2700e8e8990dbc3f9b358a6799e3c6b960854\">48b2700</a></td><td><code>src/sage/manifolds/subset.py: Remove unused import, make doctest stable by using 'sorted'</code></td></tr></table>\n",
    "created_at": "2021-04-26T23:09:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513462",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/48b2700e8e8990dbc3f9b358a6799e3c6b960854">48b2700</a></td><td><code>src/sage/manifolds/subset.py: Remove unused import, make doctest stable by using 'sorted'</code></td></tr></table>




---

archive/issue_comments_513463.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nCould you please open a new metaticket that collects the rough ideas and updates your progress regarding subsets? I think it would make it easier to keep track, at least for me.",
    "created_at": "2021-04-27T09:20:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513463",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:7" align="right">Comment 7</div>

Could you please open a new metaticket that collects the rough ideas and updates your progress regarding subsets? I think it would make it easier to keep track, at least for me.



---

archive/issue_comments_513464.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nReplying to [@mjungmath](#comment%3A7):\n> Could you please open a new metaticket that collects the rough ideas and updates your progress regarding subsets? I think it would make it easier to keep track, at least for me.\n\nIf you don't plan a lot of such tickets, maybe the metaticket #30525 is sufficient.",
    "created_at": "2021-04-27T12:37:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513464",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:8" align="right">Comment 8</div>

Replying to [@mjungmath](#comment%3A7):
> Could you please open a new metaticket that collects the rough ideas and updates your progress regarding subsets? I think it would make it easier to keep track, at least for me.

If you don't plan a lot of such tickets, maybe the metaticket #30525 is sufficient.



---

archive/issue_comments_513465.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nMatthias already opened #31740 for that. It's a nice overview and as you can see, there are plenty of tickets concerning subsets already. Thanks Matthias!",
    "created_at": "2021-04-27T13:01:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513465",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:9" align="right">Comment 9</div>

Matthias already opened #31740 for that. It's a nice overview and as you can see, there are plenty of tickets concerning subsets already. Thanks Matthias!



---

archive/issue_comments_513466.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nMh, for some reason patchbot complains about coverage.",
    "created_at": "2021-04-27T13:05:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513466",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:10" align="right">Comment 10</div>

Mh, for some reason patchbot complains about coverage.



---

archive/issue_comments_513467.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nAh, `is_empty`, `declare_nonempty` and `has_defined_points` do not admit examples/tests.",
    "created_at": "2021-04-27T13:09:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513467",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:11" align="right">Comment 11</div>

Ah, `is_empty`, `declare_nonempty` and `has_defined_points` do not admit examples/tests.



---

archive/issue_comments_513468.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nReplying to [@mjungmath](#comment%3A9):\n> Matthias already opened #31740 for that. \n\nAh yes!\n>It's a nice overview and as you can see, there are plenty of tickets concerning subsets already. \n\nIndeed!\n>Thanks Matthias!\n\n+1",
    "created_at": "2021-04-27T14:56:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513468",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:12" align="right">Comment 12</div>

Replying to [@mjungmath](#comment%3A9):
> Matthias already opened #31740 for that. 

Ah yes!
>It's a nice overview and as you can see, there are plenty of tickets concerning subsets already. 

Indeed!
>Thanks Matthias!

+1



---

archive/issue_events_419446.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-27T18:19:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419446"
}
```



---

archive/issue_events_419447.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-27T18:19:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419447"
}
```



---

archive/issue_comments_513469.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1e6a05505661e19ad09df7716fc93f5a1a2dde4d\">1e6a055</a></td><td><code>is_empty, declare_nonempty, has_defined_points: Add doc, examples</code></td></tr></table>\n",
    "created_at": "2021-04-27T18:34:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513469",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:14" align="right">Comment 14</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1e6a05505661e19ad09df7716fc93f5a1a2dde4d">1e6a055</a></td><td><code>is_empty, declare_nonempty, has_defined_points: Add doc, examples</code></td></tr></table>




---

archive/issue_comments_513470.json:
```json
{
    "body": "Changed commit from **[48b2700](https://github.com/sagemath/sagetrac-mirror/commit/48b2700e8e8990dbc3f9b358a6799e3c6b960854)** to **[1e6a055](https://github.com/sagemath/sagetrac-mirror/commit/1e6a05505661e19ad09df7716fc93f5a1a2dde4d)**",
    "created_at": "2021-04-27T18:34:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513470",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[48b2700](https://github.com/sagemath/sagetrac-mirror/commit/48b2700e8e8990dbc3f9b358a6799e3c6b960854)** to **[1e6a055](https://github.com/sagemath/sagetrac-mirror/commit/1e6a05505661e19ad09df7716fc93f5a1a2dde4d)**



---

archive/issue_events_419448.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-27T18:35:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419448"
}
```



---

archive/issue_events_419449.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-27T18:35:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419449"
}
```



---

archive/issue_comments_513471.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nNo more coverage warnings.",
    "created_at": "2021-04-27T22:48:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513471",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:16" align="right">Comment 16</div>

No more coverage warnings.



---

archive/issue_comments_513472.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nThanks! Patchbot is also green.\n\nHow exactly is this ticket related to #31743? What exactly are the dependencies you have in mind?\n\nOther than that, looks good to me. Eric, what do you say?",
    "created_at": "2021-04-28T00:41:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513472",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:17" align="right">Comment 17</div>

Thanks! Patchbot is also green.

How exactly is this ticket related to #31743? What exactly are the dependencies you have in mind?

Other than that, looks good to me. Eric, what do you say?



---

archive/issue_comments_513473.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nReplying to [@mjungmath](#comment%3A17):\n> How exactly is this ticket related to #31743? What exactly are the dependencies you have in mind?\n\n#31743 will certainly depend on the present ticket.",
    "created_at": "2021-04-28T01:22:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513473",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:18" align="right">Comment 18</div>

Replying to [@mjungmath](#comment%3A17):
> How exactly is this ticket related to #31743? What exactly are the dependencies you have in mind?

#31743 will certainly depend on the present ticket.



---

archive/issue_comments_513474.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nA nice and important addition to the current code. Thank you! LGTM.",
    "created_at": "2021-04-28T13:55:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513474",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:19" align="right">Comment 19</div>

A nice and important addition to the current code. Thank you! LGTM.



---

archive/issue_events_419450.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-04-28T13:55:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419450"
}
```



---

archive/issue_events_419451.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-04-28T13:55:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419451"
}
```



---

archive/issue_comments_513475.json:
```json
{
    "body": "Reviewer: **Michael Jung**",
    "created_at": "2021-04-28T15:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513475",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Michael Jung**



---

archive/issue_comments_513476.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nThanks for the review!",
    "created_at": "2021-04-28T15:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513476",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:20" align="right">Comment 20</div>

Thanks for the review!



---

archive/issue_comments_513477.json:
```json
{
    "body": "Changed dependencies from **#31718** to **#31718, #31727**",
    "created_at": "2021-06-19T18:04:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513477",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#31718** to **#31718, #31727**



---

archive/issue_events_419452.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2021-06-19T18:04:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419452"
}
```



---

archive/issue_events_419453.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2021-06-19T18:04:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419453"
}
```



---

archive/issue_comments_513478.json:
```json
{
    "body": "Changed commit from **[1e6a055](https://github.com/sagemath/sagetrac-mirror/commit/1e6a05505661e19ad09df7716fc93f5a1a2dde4d)** to **[359dde1](https://github.com/sagemath/sagetrac-mirror/commit/359dde1acb2c64a04341bb8d5ffeaa58c3d51dae)**",
    "created_at": "2021-06-19T18:04:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513478",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1e6a055](https://github.com/sagemath/sagetrac-mirror/commit/1e6a05505661e19ad09df7716fc93f5a1a2dde4d)** to **[359dde1](https://github.com/sagemath/sagetrac-mirror/commit/359dde1acb2c64a04341bb8d5ffeaa58c3d51dae)**



---

archive/issue_comments_513479.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3c5555acd67b87e772acb7f22354a444cca16595\">3c5555a</a></td><td><code>Merge #31718</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/52a93f94de48c0fb7070f95b170cd218d7ddfc15\">52a93f9</a></td><td><code>ManifoldSubset.subset_family: New; deprecate .list_of_subsets</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7a22bb0838eccf995ac6dd9de4cdc44d07fee81f\">7a22bb0</a></td><td><code>ManifoldSubset.superset_family: New, use it in doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bdc07ffec4d821ca00e4a230aeb51d11dfef4322\">bdc07ff</a></td><td><code>Merge #31677</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11\">2b47b1b</a></td><td><code>ManifoldSubset.open_superset_family: New</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/359dde1acb2c64a04341bb8d5ffeaa58c3d51dae\">359dde1</a></td><td><code>Merge branch 't/31727/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets' into t/31732/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family</code></td></tr></table>\n",
    "created_at": "2021-06-19T18:04:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513479",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:22" align="right">Comment 22</div>

Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3c5555acd67b87e772acb7f22354a444cca16595">3c5555a</a></td><td><code>Merge #31718</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/52a93f94de48c0fb7070f95b170cd218d7ddfc15">52a93f9</a></td><td><code>ManifoldSubset.subset_family: New; deprecate .list_of_subsets</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7a22bb0838eccf995ac6dd9de4cdc44d07fee81f">7a22bb0</a></td><td><code>ManifoldSubset.superset_family: New, use it in doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bdc07ffec4d821ca00e4a230aeb51d11dfef4322">bdc07ff</a></td><td><code>Merge #31677</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11">2b47b1b</a></td><td><code>ManifoldSubset.open_superset_family: New</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/359dde1acb2c64a04341bb8d5ffeaa58c3d51dae">359dde1</a></td><td><code>Merge branch 't/31727/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets' into t/31732/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family</code></td></tr></table>




---

archive/issue_comments_513480.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">Comment 23</div>\n\nMerged #31727 to resolve a merge conflict",
    "created_at": "2021-06-19T18:05:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513480",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:23" align="right">Comment 23</div>

Merged #31727 to resolve a merge conflict



---

archive/issue_events_419454.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-19T18:05:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419454"
}
```



---

archive/issue_events_419455.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-19T18:05:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419455"
}
```



---

archive/issue_events_419456.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-06-21T21:17:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419456"
}
```



---

archive/issue_events_419457.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3a36b12e1325a4be72ba13db21f73056bf0a5dea",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-06-21T21:17:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31732#event-419457"
}
```



---

archive/issue_comments_513481.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family)** to **[359dde1](https://github.com/sagemath/sagetrac-mirror/commit/359dde1acb2c64a04341bb8d5ffeaa58c3d51dae)**",
    "created_at": "2021-06-21T21:17:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31732#issuecomment-513481",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/manifoldsubset__new_methods_declare_empty__declare_nonempty__is_empty__has_defined_points__open_cover_family)** to **[359dde1](https://github.com/sagemath/sagetrac-mirror/commit/359dde1acb2c64a04341bb8d5ffeaa58c3d51dae)**
