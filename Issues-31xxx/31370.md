# Issue 31370: Create a new module for morphic words

archive/issues_031133.json:
```json
{
    "assignees": [],
    "body": "The goal of this ticket is to create a new module for morphic words.\n\nAs a consequence, it will improve the following computations which take a lot of time:\n\n```\nsage: m = WordMorphism('a->ab,b->a')\nsage: w = m.fixed_point('a')\nsage: w\nword: abaababaabaababaababaabaababaabaababaaba...\nsage: %time w[1000]\nCPU times: user 1.45 ms, sys: 0 ns, total: 1.45 ms\nWall time: 1.45 ms\n'a'\nsage: %time w[10000]\nCPU times: user 82.9 ms, sys: 0 ns, total: 82.9 ms\nWall time: 82.1 ms\n'a'\nsage: %time w[100000]\nCPU times: user 5.19 s, sys: 6.26 ms, total: 5.2 s\nWall time: 5.19 s\n'b'\nsage: %time w[1000000]\nCPU times: user 12min 45s, sys: 93.4 ms, total: 12min 45s\nWall time: 12min 45s\n'a'\n```\n\n\nCC:  @fchapoton\n\nReviewer: **S\u00e9bastien Labb\u00e9, Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31370_\n\n",
    "closed_at": "2021-02-11T10:00:50Z",
    "created_at": "2021-02-09T13:47:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Create a new module for morphic words",
    "type": "issue",
    "updated_at": "2021-05-06T11:51:11Z",
    "url": "https://github.com/sagemath/sage/issues/31370",
    "user": "https://github.com/seblabbe"
}
```
The goal of this ticket is to create a new module for morphic words.

As a consequence, it will improve the following computations which take a lot of time:

```
sage: m = WordMorphism('a->ab,b->a')
sage: w = m.fixed_point('a')
sage: w
word: abaababaabaababaababaabaababaabaababaaba...
sage: %time w[1000]
CPU times: user 1.45 ms, sys: 0 ns, total: 1.45 ms
Wall time: 1.45 ms
'a'
sage: %time w[10000]
CPU times: user 82.9 ms, sys: 0 ns, total: 82.9 ms
Wall time: 82.1 ms
'a'
sage: %time w[100000]
CPU times: user 5.19 s, sys: 6.26 ms, total: 5.2 s
Wall time: 5.19 s
'b'
sage: %time w[1000000]
CPU times: user 12min 45s, sys: 93.4 ms, total: 12min 45s
Wall time: 12min 45s
'a'
```


CC:  @fchapoton

Reviewer: **Sébastien Labbé, Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/31370_





---

archive/issue_events_414101.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2021-02-09T13:47:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414101"
}
```



---

archive/issue_events_414102.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2021-02-09T13:47:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414102"
}
```



---

archive/issue_events_414103.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2021-02-09T13:47:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414103"
}
```



---

archive/issue_events_414104.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2021-02-09T13:47:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414104"
}
```



---

archive/issue_events_414105.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2021-02-11T09:34:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414105"
}
```



---

archive/issue_events_414106.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2021-02-11T09:34:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414106"
}
```



---

archive/issue_events_414107.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2021-02-11T09:34:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414107"
}
```



---

archive/issue_comments_506171.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nlet's close this one as duplicate of #31378.",
    "created_at": "2021-02-11T09:34:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31370#issuecomment-506171",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:1" align="right">Comment 1</div>

let's close this one as duplicate of #31378.



---

archive/issue_events_414108.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2021-02-11T09:34:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414108"
}
```



---

archive/issue_events_414109.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-11T10:00:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414109"
}
```



---

archive/issue_events_414110.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-11T10:00:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414110"
}
```



---

archive/issue_events_414111.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-11T10:00:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31370#event-414111"
}
```



---

archive/issue_comments_506172.json:
```json
{
    "body": "Reviewer: **S\u00e9bastien Labb\u00e9, Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-05-06T11:51:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31370#issuecomment-506172",
    "user": "https://github.com/slel"
}
```

Reviewer: **Sébastien Labbé, Frédéric Chapoton**
