# Issue 31374: Fix an if statement that is never called in normal_form

archive/issues_031137.json:
```json
{
    "assignees": [],
    "body": "In line 1483 in sage.quadratic_forms.genera.normal_form we have\n\n```\n        b = [[0,5], [2,7], [1,2], [0,3], [0,1,5], [1,2,7], [0,3,7], [0,1,3]]\n        if x in b:\n            w = W[-1]\n            if x == [3,7]:\n                w = W[0]\n```\nThe second if statement is never true due to a typo.\nIt should be `x == [0,3,7]`. \nActually the bug is cleaned up later so the output should be correct anyways.\n\nBranch/Commit: **[7d304d6](https://github.com/sagemath/sagetrac-mirror/commit/7d304d69f42b7ae930f8c91ab298e52427f0c24d)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Simon Brandhorst**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31374_\n\n",
    "closed_at": "2021-03-20T15:27:29Z",
    "created_at": "2021-02-10T10:14:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix an if statement that is never called in normal_form",
    "type": "issue",
    "updated_at": "2021-03-20T15:27:29Z",
    "url": "https://github.com/sagemath/sage/issues/31374",
    "user": "https://github.com/simonbrandhorst"
}
```
In line 1483 in sage.quadratic_forms.genera.normal_form we have

```
        b = [[0,5], [2,7], [1,2], [0,3], [0,1,5], [1,2,7], [0,3,7], [0,1,3]]
        if x in b:
            w = W[-1]
            if x == [3,7]:
                w = W[0]
```
The second if statement is never true due to a typo.
It should be `x == [0,3,7]`. 
Actually the bug is cleaned up later so the output should be correct anyways.

Branch/Commit: **[7d304d6](https://github.com/sagemath/sagetrac-mirror/commit/7d304d69f42b7ae930f8c91ab298e52427f0c24d)**

Reviewer: **Frédéric Chapoton**

Author: **Simon Brandhorst**

_Issue created by migration from https://trac.sagemath.org/ticket/31374_





---

archive/issue_events_414147.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2021-02-10T10:14:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414147"
}
```



---

archive/issue_events_414148.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2021-02-10T10:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
    "label_color": "0000ff",
    "label_name": "component: quadratic forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414148"
}
```



---

archive/issue_events_414149.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2021-02-10T10:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414149"
}
```



---

archive/issue_events_414150.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2021-02-10T10:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414150"
}
```



---

archive/issue_comments_506221.json:
```json
{
    "body": "Branch: **[u/sbrandhorst/fix_an_if_statement_that_is_never_called_in_normal_form](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/fix_an_if_statement_that_is_never_called_in_normal_form)**",
    "created_at": "2021-02-10T10:24:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506221",
    "user": "https://github.com/simonbrandhorst"
}
```

Branch: **[u/sbrandhorst/fix_an_if_statement_that_is_never_called_in_normal_form](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/fix_an_if_statement_that_is_never_called_in_normal_form)**



---

archive/issue_comments_506222.json:
```json
{
    "body": "Commit: **[89467ed](https://github.com/sagemath/sagetrac-mirror/commit/89467edba8a1dac0dd5192f1e78f13241daef5fb)**",
    "created_at": "2021-02-10T10:25:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506222",
    "user": "https://github.com/simonbrandhorst"
}
```

Commit: **[89467ed](https://github.com/sagemath/sagetrac-mirror/commit/89467edba8a1dac0dd5192f1e78f13241daef5fb)**



---

archive/issue_events_414151.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2021-02-10T10:25:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414151"
}
```



---

archive/issue_comments_506223.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/89467edba8a1dac0dd5192f1e78f13241daef5fb\">89467ed</a></td><td><code>clean up an if statement that is never called</code></td></tr></table>\n",
    "created_at": "2021-02-10T10:25:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506223",
    "user": "https://github.com/simonbrandhorst"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/89467edba8a1dac0dd5192f1e78f13241daef5fb">89467ed</a></td><td><code>clean up an if statement that is never called</code></td></tr></table>




---

archive/issue_events_414152.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-10T19:20:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414152"
}
```



---

archive/issue_events_414153.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-10T19:20:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414153"
}
```



---

archive/issue_comments_506224.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok, let it be",
    "created_at": "2021-02-10T19:20:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506224",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok, let it be



---

archive/issue_events_414154.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-03-09T00:03:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414154"
}
```



---

archive/issue_events_414155.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-03-09T00:03:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414155"
}
```



---

archive/issue_comments_506225.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReviewer name is missing",
    "created_at": "2021-03-09T00:03:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506225",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Reviewer name is missing



---

archive/issue_comments_506226.json:
```json
{
    "body": "Changed commit from **[89467ed](https://github.com/sagemath/sagetrac-mirror/commit/89467edba8a1dac0dd5192f1e78f13241daef5fb)** to **[7d304d6](https://github.com/sagemath/sagetrac-mirror/commit/7d304d69f42b7ae930f8c91ab298e52427f0c24d)**",
    "created_at": "2021-03-11T12:38:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506226",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[89467ed](https://github.com/sagemath/sagetrac-mirror/commit/89467edba8a1dac0dd5192f1e78f13241daef5fb)** to **[7d304d6](https://github.com/sagemath/sagetrac-mirror/commit/7d304d69f42b7ae930f8c91ab298e52427f0c24d)**



---

archive/issue_comments_506227.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7d304d69f42b7ae930f8c91ab298e52427f0c24d\">7d304d6</a></td><td><code>Merge branch 'develop' into t/31374/fix_an_if_statement_that_is_never_called_in_normal_form</code></td></tr></table>\n",
    "created_at": "2021-03-11T12:38:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506227",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7d304d69f42b7ae930f8c91ab298e52427f0c24d">7d304d6</a></td><td><code>Merge branch 'develop' into t/31374/fix_an_if_statement_that_is_never_called_in_normal_form</code></td></tr></table>




---

archive/issue_comments_506228.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nPlease add your name :-)",
    "created_at": "2021-03-11T12:39:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506228",
    "user": "https://github.com/simonbrandhorst"
}
```

<div id="comment:6" align="right">Comment 6</div>

Please add your name :-)



---

archive/issue_events_414156.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2021-03-11T12:39:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414156"
}
```



---

archive/issue_events_414157.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2021-03-11T12:39:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414157"
}
```



---

archive/issue_events_414158.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-03-11T13:50:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414158"
}
```



---

archive/issue_events_414159.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-03-11T13:50:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414159"
}
```



---

archive/issue_comments_506229.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-03-11T13:50:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506229",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_506230.json:
```json
{
    "body": "Changed branch from **[u/sbrandhorst/fix_an_if_statement_that_is_never_called_in_normal_form](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/fix_an_if_statement_that_is_never_called_in_normal_form)** to **[7d304d6](https://github.com/sagemath/sagetrac-mirror/commit/7d304d69f42b7ae930f8c91ab298e52427f0c24d)**",
    "created_at": "2021-03-20T15:27:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31374#issuecomment-506230",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/sbrandhorst/fix_an_if_statement_that_is_never_called_in_normal_form](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/fix_an_if_statement_that_is_never_called_in_normal_form)** to **[7d304d6](https://github.com/sagemath/sagetrac-mirror/commit/7d304d69f42b7ae930f8c91ab298e52427f0c24d)**



---

archive/issue_events_414160.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-03-20T15:27:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414160"
}
```



---

archive/issue_events_414161.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ab9ca9cf1bbbcd808720bff882c77660a3b1dc1b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-03-20T15:27:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31374",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31374#event-414161"
}
```
