# Issue 31385: Improve interface of list_packages

archive/issues_031148.json:
```json
{
    "assignees": [],
    "body": "(split out from #31013)\n\n... representing package info using a new `PackageInfo` class, which provides also dict-like access, maintaining compatibility with the previous interface in this way.\n\n\nCC:  @jhpalmieri @dimpase\n\nBranch: **[ebfb13c](https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640)**\n\nReviewer: **Matthias Koeppe, John Palmieri, Tobias Diez**\n\nAuthor: **Tobias Diez, Matthias Koeppe, John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31385_\n\n",
    "closed_at": "2021-10-31T22:21:19Z",
    "created_at": "2021-02-12T00:08:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve interface of list_packages",
    "type": "issue",
    "updated_at": "2021-11-16T20:50:26Z",
    "url": "https://github.com/sagemath/sage/issues/31385",
    "user": "https://github.com/mkoeppe"
}
```
(split out from #31013)

... representing package info using a new `PackageInfo` class, which provides also dict-like access, maintaining compatibility with the previous interface in this way.


CC:  @jhpalmieri @dimpase

Branch: **[ebfb13c](https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640)**

Reviewer: **Matthias Koeppe, John Palmieri, Tobias Diez**

Author: **Tobias Diez, Matthias Koeppe, John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/31385_





---

archive/issue_events_414309.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-12T00:08:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414309"
}
```



---

archive/issue_events_414310.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-12T00:08:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414310"
}
```



---

archive/issue_events_414311.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-12T00:08:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414311"
}
```



---

archive/issue_comments_506564.json:
```json
{
    "body": "Branch: **[u/mkoeppe/improve_interface_of_list_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/improve_interface_of_list_packages)**",
    "created_at": "2021-02-12T00:31:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506564",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/improve_interface_of_list_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/improve_interface_of_list_packages)**



---

archive/issue_events_414312.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-12T00:32:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414312"
}
```



---

archive/issue_comments_506565.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe, ...**",
    "created_at": "2021-02-12T00:32:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506565",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe, ...**



---

archive/issue_comments_506566.json:
```json
{
    "body": "Commit: **[7488fcd](https://github.com/sagemath/sagetrac-mirror/commit/7488fcd9cf57ea207d3a33ce96661dd0282d9546)**",
    "created_at": "2021-02-12T00:32:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506566",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[7488fcd](https://github.com/sagemath/sagetrac-mirror/commit/7488fcd9cf57ea207d3a33ce96661dd0282d9546)**



---

archive/issue_comments_506567.json:
```json
{
    "body": "Changed author from **Tobias Diez** to **Tobias Diez, Matthias Koeppe**",
    "created_at": "2021-02-12T00:32:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506567",
    "user": "https://github.com/mkoeppe"
}
```

Changed author from **Tobias Diez** to **Tobias Diez, Matthias Koeppe**



---

archive/issue_comments_506568.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/35642b351ffe3e423597ac75c6f6803a0aab76b2\">35642b3</a></td><td><code>sage.misc.packages, sage-list-packages: Use new class PackageInfo</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/687df80d881e9755689ac6e4fd7e4095e312d401\">687df80</a></td><td><code>sage.misc.package.PackageInfo: Improve wording of deprecation message, fix doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6bc70fc5f2c0dd5bbdd133289fc6c44c06b82e4c\">6bc70fc</a></td><td><code>sage.misc.package.list_packages: Align optional tags in doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7488fcd9cf57ea207d3a33ce96661dd0282d9546\">7488fcd</a></td><td><code>Code cleanup</code></td></tr></table>\n",
    "created_at": "2021-02-12T00:32:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506568",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/35642b351ffe3e423597ac75c6f6803a0aab76b2">35642b3</a></td><td><code>sage.misc.packages, sage-list-packages: Use new class PackageInfo</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/687df80d881e9755689ac6e4fd7e4095e312d401">687df80</a></td><td><code>sage.misc.package.PackageInfo: Improve wording of deprecation message, fix doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6bc70fc5f2c0dd5bbdd133289fc6c44c06b82e4c">6bc70fc</a></td><td><code>sage.misc.package.list_packages: Align optional tags in doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7488fcd9cf57ea207d3a33ce96661dd0282d9546">7488fcd</a></td><td><code>Code cleanup</code></td></tr></table>




---

archive/issue_comments_506569.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506569",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'>Comment 3:</a>
Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_414313.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414313"
}
```



---

archive/issue_events_414314.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414314"
}
```



---

archive/issue_comments_506570.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nStill needs review. Matthias, I'm happy with your changes - if that is sufficient, feel free to set it to positive review.",
    "created_at": "2021-05-18T07:45:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506570",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:4'>Comment 4:</a>
Still needs review. Matthias, I'm happy with your changes - if that is sufficient, feel free to set it to positive review.



---

archive/issue_comments_506571.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\ndoes not work with #31577 and #32036 - not sure which one to blame.",
    "created_at": "2021-06-25T19:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506571",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:6'>Comment 6:</a>
does not work with #31577 and #32036 - not sure which one to blame.



---

archive/issue_events_414315.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-25T19:52:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414315"
}
```



---

archive/issue_events_414316.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-25T19:52:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414316"
}
```



---

archive/issue_comments_506572.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nReplying to [@dimpase](#comment%3A6):\n> does not work with #31577 and #32036 - not sure which one to blame.\n\nWith this plus #32036, the Sage library fails to build for me:\n\n```\nTraceback (most recent call last):\n  File \"/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/setup.py\", line 79, in <module>\n    distributions += ['sage-{}'.format(pkg)\n  File \"/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/setup.py\", line 81, in <listcomp>\n    if is_package_installed_and_updated(pkg)]\n  File \"/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/sage_setup/optional_extension.py\", line 51, in is_package_installed_and_updated\n    condition = (pkginfo[\"installed_version\"] == pkginfo[\"remote_version\"])\n  File \"/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/sage/misc/package.py\", line 210, in __getitem__\n    from sage.misc.superseded import deprecation\n  File \"/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/sage/misc/superseded.py\", line 29, in <module>\n    from sage.misc.lazy_attribute import lazy_attribute\nModuleNotFoundError: No module named 'sage.misc.lazy_attribute'\n```\n(I don't think that #31577 is involved.)",
    "created_at": "2021-06-26T00:59:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506572",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:8'>Comment 8:</a>
Replying to [@dimpase](#comment%3A6):
> does not work with #31577 and #32036 - not sure which one to blame.

With this plus #32036, the Sage library fails to build for me:

```
Traceback (most recent call last):
  File "/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/setup.py", line 79, in <module>
    distributions += ['sage-{}'.format(pkg)
  File "/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/setup.py", line 81, in <listcomp>
    if is_package_installed_and_updated(pkg)]
  File "/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/sage_setup/optional_extension.py", line 51, in is_package_installed_and_updated
    condition = (pkginfo["installed_version"] == pkginfo["remote_version"])
  File "/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/sage/misc/package.py", line 210, in __getitem__
    from sage.misc.superseded import deprecation
  File "/Users/jpalmier/Desktop/Sage/sage_builds/TESTING/sage-9.4.beta3/build/pkgs/sagelib/src/sage/misc/superseded.py", line 29, in <module>
    from sage.misc.lazy_attribute import lazy_attribute
ModuleNotFoundError: No module named 'sage.misc.lazy_attribute'
```
(I don't think that #31577 is involved.)



---

archive/issue_comments_506573.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nReplying to [@jhpalmieri](#comment%3A8):\n> (I don't think that #31577 is involved.)\n\nOr maybe it is, on another machine I'm seeing the same error with #31577 + #31385, but not #32036.",
    "created_at": "2021-06-26T03:56:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506573",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:9'>Comment 9:</a>
Replying to [@jhpalmieri](#comment%3A8):
> (I don't think that #31577 is involved.)

Or maybe it is, on another machine I'm seeing the same error with #31577 + #31385, but not #32036.



---

archive/issue_comments_506574.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nMaybe these previous posts are red herrings, and the problem is this ticket doesn't work with the latest beta.",
    "created_at": "2021-06-30T17:37:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506574",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:10'>Comment 10:</a>
Maybe these previous posts are red herrings, and the problem is this ticket doesn't work with the latest beta.



---

archive/issue_comments_506575.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nThanks for testing it -- I'll revisit it after the next beta",
    "created_at": "2021-06-30T17:56:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506575",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:11'>Comment 11:</a>
Thanks for testing it -- I'll revisit it after the next beta



---

archive/issue_comments_506576.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nThe following seems to fix it for me, but I don't know if it's a good and/or safe change:\n\n```diff\ndiff --git a/src/sage_setup/optional_extension.py b/src/sage_setup/optional_extension.py\nindex 06586f1d39..f252849384 100644\n--- a/src/sage_setup/optional_extension.py\n+++ b/src/sage_setup/optional_extension.py\n@@ -48,7 +48,7 @@ def is_package_installed_and_updated(pkg):\n         # Might be an installed old-style package\n         condition = is_package_installed(pkg)\n     else:\n-        condition = (pkginfo[\"installed_version\"] == pkginfo[\"remote_version\"])\n+        condition = (pkginfo.installed_version == pkginfo.remote_version)\n     return condition\n \n \n```",
    "created_at": "2021-06-30T22:53:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506576",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'>Comment 12:</a>
The following seems to fix it for me, but I don't know if it's a good and/or safe change:

```diff
diff --git a/src/sage_setup/optional_extension.py b/src/sage_setup/optional_extension.py
index 06586f1d39..f252849384 100644
--- a/src/sage_setup/optional_extension.py
+++ b/src/sage_setup/optional_extension.py
@@ -48,7 +48,7 @@ def is_package_installed_and_updated(pkg):
         # Might be an installed old-style package
         condition = is_package_installed(pkg)
     else:
-        condition = (pkginfo["installed_version"] == pkginfo["remote_version"])
+        condition = (pkginfo.installed_version == pkginfo.remote_version)
     return condition
 
 
```



---

archive/issue_events_414317.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414317"
}
```



---

archive/issue_events_414318.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414318"
}
```



---

archive/issue_comments_506577.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nSetting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506577",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:13'>Comment 13:</a>
Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_comments_506578.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/improve_interface_of_list_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/improve_interface_of_list_packages)** to **[u/jhpalmieri/improve_interface_of_list_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/improve_interface_of_list_packages)**",
    "created_at": "2021-09-30T19:55:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506578",
    "user": "https://github.com/jhpalmieri"
}
```

Changed branch from **[u/mkoeppe/improve_interface_of_list_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/improve_interface_of_list_packages)** to **[u/jhpalmieri/improve_interface_of_list_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/improve_interface_of_list_packages)**



---

archive/issue_comments_506579.json:
```json
{
    "body": "Changed commit from **[7488fcd](https://github.com/sagemath/sagetrac-mirror/commit/7488fcd9cf57ea207d3a33ce96661dd0282d9546)** to **[ebfb13c](https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640)**",
    "created_at": "2021-09-30T19:56:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506579",
    "user": "https://github.com/jhpalmieri"
}
```

Changed commit from **[7488fcd](https://github.com/sagemath/sagetrac-mirror/commit/7488fcd9cf57ea207d3a33ce96661dd0282d9546)** to **[ebfb13c](https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640)**



---

archive/issue_events_414319.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-09-30T19:56:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414319"
}
```



---

archive/issue_events_414320.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-09-30T19:56:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414320"
}
```



---

archive/issue_comments_506580.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nIf my changes are okay, then I think we should merge it.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/de1a48c38bf33bc3dc4c2dc82770d6ad8c08c14f\">de1a48c</a></td><td><code>sage.misc.packages, sage-list-packages: Use new class PackageInfo</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ebc4f82361cb640446683ba22e155d80085ab005\">ebc4f82</a></td><td><code>sage.misc.package.PackageInfo: Improve wording of deprecation message, fix doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/18bcf003b88a98f6a33a98d2849a257b3ae9067b\">18bcf00</a></td><td><code>sage.misc.package.list_packages: Align optional tags in doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8b06dc85e95a2d23c52390d55f04baedd399aa3b\">8b06dc8</a></td><td><code>Code cleanup</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640\">ebfb13c</a></td><td><code>trac 31385: fix sage_setup to use the new package attributes</code></td></tr></table>\n",
    "created_at": "2021-09-30T19:56:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506580",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:15'>Comment 15:</a>
If my changes are okay, then I think we should merge it.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/de1a48c38bf33bc3dc4c2dc82770d6ad8c08c14f">de1a48c</a></td><td><code>sage.misc.packages, sage-list-packages: Use new class PackageInfo</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ebc4f82361cb640446683ba22e155d80085ab005">ebc4f82</a></td><td><code>sage.misc.package.PackageInfo: Improve wording of deprecation message, fix doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/18bcf003b88a98f6a33a98d2849a257b3ae9067b">18bcf00</a></td><td><code>sage.misc.package.list_packages: Align optional tags in doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8b06dc85e95a2d23c52390d55f04baedd399aa3b">8b06dc8</a></td><td><code>Code cleanup</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640">ebfb13c</a></td><td><code>trac 31385: fix sage_setup to use the new package attributes</code></td></tr></table>




---

archive/issue_comments_506581.json:
```json
{
    "body": "Changed reviewer from **Matthias Koeppe, ...** to **Matthias Koeppe, John Palmieri**",
    "created_at": "2021-09-30T19:56:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506581",
    "user": "https://github.com/jhpalmieri"
}
```

Changed reviewer from **Matthias Koeppe, ...** to **Matthias Koeppe, John Palmieri**



---

archive/issue_comments_506582.json:
```json
{
    "body": "Changed author from **Tobias Diez, Matthias Koeppe** to **Tobias Diez, Matthias Koeppe, John Palmieri**",
    "created_at": "2021-09-30T19:56:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506582",
    "user": "https://github.com/jhpalmieri"
}
```

Changed author from **Tobias Diez, Matthias Koeppe** to **Tobias Diez, Matthias Koeppe, John Palmieri**



---

archive/issue_events_414321.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2021-10-26T15:48:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414321"
}
```



---

archive/issue_events_414322.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2021-10-26T15:48:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414322"
}
```



---

archive/issue_comments_506583.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nThey are fine with me. Setting this to positive review as it seems that everyone reviewed everyone else's changes.",
    "created_at": "2021-10-26T15:48:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506583",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:16'>Comment 16:</a>
They are fine with me. Setting this to positive review as it seems that everyone reviewed everyone else's changes.



---

archive/issue_comments_506584.json:
```json
{
    "body": "Changed reviewer from **Matthias Koeppe, John Palmieri** to **Matthias Koeppe, John Palmieri, Tobias Diez**",
    "created_at": "2021-10-26T15:48:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506584",
    "user": "https://github.com/tobiasdiez"
}
```

Changed reviewer from **Matthias Koeppe, John Palmieri** to **Matthias Koeppe, John Palmieri, Tobias Diez**



---

archive/issue_comments_506585.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nYes, looks good to me. Thanks!",
    "created_at": "2021-10-26T17:39:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506585",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:17'>Comment 17:</a>
Yes, looks good to me. Thanks!



---

archive/issue_comments_506586.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/improve_interface_of_list_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/improve_interface_of_list_packages)** to **[ebfb13c](https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640)**",
    "created_at": "2021-10-31T22:21:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506586",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/improve_interface_of_list_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/improve_interface_of_list_packages)** to **[ebfb13c](https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640)**



---

archive/issue_events_414323.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-31T22:21:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414323"
}
```



---

archive/issue_events_414324.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "906387d5e4f4042d413b2c59b844db43e03499bc",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-31T22:21:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31385#event-414324"
}
```



---

archive/issue_comments_506587.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\n`sage --standard`, `sage --optional` and `sage --experimental` are broken in 9.5.beta6 (9 doctests failing in cmdline.py with `--optional=sage,internet`). Maybe this was caused by the changes made here. I created ticket #32885.",
    "created_at": "2021-11-16T20:50:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506587",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:19'>Comment 19:</a>
`sage --standard`, `sage --optional` and `sage --experimental` are broken in 9.5.beta6 (9 doctests failing in cmdline.py with `--optional=sage,internet`). Maybe this was caused by the changes made here. I created ticket #32885.



---

archive/issue_comments_506588.json:
```json
{
    "body": "Changed commit from **[ebfb13c](https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640)** to none",
    "created_at": "2021-11-16T20:50:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31385#issuecomment-506588",
    "user": "https://github.com/seblabbe"
}
```

Changed commit from **[ebfb13c](https://github.com/sagemath/sagetrac-mirror/commit/ebfb13c9868424e685a1462c1755137777a96640)** to none
