# Issue 31044: Migrate to established doc style

archive/issues_030807.json:
```json
{
    "body": "It's worth considering to migrate from Sage's custom documentation style to one of the more established/standarized ones such as:\n- [pep257](https://www.python.org/dev/peps/pep-0257/)\n- [numpy](https://numpydoc.readthedocs.io/en/latest/format.html)\n\n```python\ndef function(param1: int, param2: str) -> bool:\n    \"\"\"\n    Example function doing nothing.\n\n\n    Parameters\n    ----------\n    param1\n        The first parameter.\n    param2\n        The second parameter.\n        The description may span multiple lines. Following\n        lines should keep the indention.\n\n    Returns\n    -------\n    bool\n        True if successful, False otherwise.\n\n    Examples\n    --------\n    Examples should be written in doctest format, and should illustrate how\n    to use the function.\n\n    >>> print([i for i in example_generator(4)])\n    [0, 1, 2, 3]\n\n    \"\"\"\n```\n    \n    The return type must be duplicated in the docstring to comply with the NumPy docstring style.\n- [google](https://google.github.io/styleguide/pyguide.html#383-functions-and-methods)\n\n```python\ndef function(param1: int, param2: str) -> bool:\n    \"\"\"\n    Example function doing nothing.\n\n    Args:\n        param1: The first parameter.\n        param2: The second parameter.\n            The description may span multiple lines. Following\n            lines should be indented.\n\n    Returns:\n        The return value. True for success, False otherwise.\n\n    Examples:\n        Examples should be written in doctest format, and should illustrate how\n        to use the function.\n\n        >>> print([i for i in example_generator(4)])\n        [0, 1, 2, 3]\n\n    \"\"\"\n    }}}\n    \nMore examples for numpy and google can be found at https://www.sphinx-doc.org/en/master/usage/extensions/napoleon.html\n\nCC:  @mkoeppe @jhpalmieri\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/31044\n\n",
    "created_at": "2020-12-12T20:23:20Z",
    "labels": [
        "component: documentation"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-wishlist",
    "title": "Migrate to established doc style",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/31044",
    "user": "https://github.com/tobiasdiez"
}
```
It's worth considering to migrate from Sage's custom documentation style to one of the more established/standarized ones such as:
- [pep257](https://www.python.org/dev/peps/pep-0257/)
- [numpy](https://numpydoc.readthedocs.io/en/latest/format.html)

```python
def function(param1: int, param2: str) -> bool:
    """
    Example function doing nothing.


    Parameters
    ----------
    param1
        The first parameter.
    param2
        The second parameter.
        The description may span multiple lines. Following
        lines should keep the indention.

    Returns
    -------
    bool
        True if successful, False otherwise.

    Examples
    --------
    Examples should be written in doctest format, and should illustrate how
    to use the function.

    >>> print([i for i in example_generator(4)])
    [0, 1, 2, 3]

    """
```
    
    The return type must be duplicated in the docstring to comply with the NumPy docstring style.
- [google](https://google.github.io/styleguide/pyguide.html#383-functions-and-methods)

```python
def function(param1: int, param2: str) -> bool:
    """
    Example function doing nothing.

    Args:
        param1: The first parameter.
        param2: The second parameter.
            The description may span multiple lines. Following
            lines should be indented.

    Returns:
        The return value. True for success, False otherwise.

    Examples:
        Examples should be written in doctest format, and should illustrate how
        to use the function.

        >>> print([i for i in example_generator(4)])
        [0, 1, 2, 3]

    """
    }}}
    
More examples for numpy and google can be found at https://www.sphinx-doc.org/en/master/usage/extensions/napoleon.html

CC:  @mkoeppe @jhpalmieri

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/31044





---

archive/issue_comments_616006.json:
```json
{
    "body": "<a id='comment:1'></a>More examples here: https://www.sphinx-doc.org/en/master/examples.html\n\nI like that the inputs and outputs are highlighted in our style, whereas some others are more freeform. The numpy style appeals to me for this reason. The standard Python documentation is another model we could use, and its familiarity might be an advantage.\n\nWe are also not using the default Sphinx setup except in a few spots (like simplicial complexes: https://doc.sagemath.org/html/en/reference/topology/sage/topology/simplicial_complex.html#sage.topology.simplicial_complex.Simplex). We could head in this direction, too.",
    "created_at": "2022-02-03T23:58:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616006",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'></a>More examples here: https://www.sphinx-doc.org/en/master/examples.html

I like that the inputs and outputs are highlighted in our style, whereas some others are more freeform. The numpy style appeals to me for this reason. The standard Python documentation is another model we could use, and its familiarity might be an advantage.

We are also not using the default Sphinx setup except in a few spots (like simplicial complexes: https://doc.sagemath.org/html/en/reference/topology/sage/topology/simplicial_complex.html#sage.topology.simplicial_complex.Simplex). We could head in this direction, too.



---

archive/issue_comments_616007.json:
```json
{
    "body": "<a id='comment:2'></a>I agree, numpy is quite readable. It's a bit sad though that you have to specify the return type in the documentation string, so it's not enough to provide it as a typing annotation.\nGoogle doesn't have this disadvantage and also requires the grouping of paramaters and return value etc. NumPy style tends to require more vertical space, whereas Google style tends to use more horizontal space. I guess with modern widescreen monitors that might be an advantage of the google style.\n\nI've updated the ticket description with examples (essentially taken from the napoleon docs).\n\nWhat would be the right way to move forward with this? I guess at some point there should be a vote on the developer mailing list.",
    "created_at": "2022-02-04T22:46:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616007",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:2'></a>I agree, numpy is quite readable. It's a bit sad though that you have to specify the return type in the documentation string, so it's not enough to provide it as a typing annotation.
Google doesn't have this disadvantage and also requires the grouping of paramaters and return value etc. NumPy style tends to require more vertical space, whereas Google style tends to use more horizontal space. I guess with modern widescreen monitors that might be an advantage of the google style.

I've updated the ticket description with examples (essentially taken from the napoleon docs).

What would be the right way to move forward with this? I guess at some point there should be a vote on the developer mailing list.



---

archive/issue_comments_616008.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,64 @@\n It's worth considering to migrate from Sage's custom documentation style to one of the more established/standarized ones such as:\n - [pep257](https://www.python.org/dev/peps/pep-0257/)\n - [numpy](https://numpydoc.readthedocs.io/en/latest/format.html)\n-- [\u200bhttps://google.github.io/styleguide/pyguide.html#383-functions-and-methods google]\n+\n+```python\n+def function(param1: int, param2: str) -> bool:\n+    \"\"\"\n+    Example function doing nothing.\n+\n+\n+    Parameters\n+    ----------\n+    param1\n+        The first parameter.\n+    param2\n+        The second parameter.\n+        The description may span multiple lines. Following\n+        lines should keep the indention.\n+\n+    Returns\n+    -------\n+    bool\n+        True if successful, False otherwise.\n+\n+    Examples\n+    --------\n+    Examples should be written in doctest format, and should illustrate how\n+    to use the function.\n+\n+    >>> print([i for i in example_generator(4)])\n+    [0, 1, 2, 3]\n+\n+    \"\"\"\n+```\n+    \n+    The return type must be duplicated in the docstring to comply with the NumPy docstring style.\n+- [google](https://google.github.io/styleguide/pyguide.html#383-functions-and-methods)\n+\n+```python\n+def function(param1: int, param2: str) -> bool:\n+    \"\"\"\n+    Example function doing nothing.\n+\n+    Args:\n+        param1: The first parameter.\n+        param2: The second parameter.\n+            The description may span multiple lines. Following\n+            lines should be indented.\n+\n+    Returns:\n+        The return value. True for success, False otherwise.\n+\n+    Examples:\n+        Examples should be written in doctest format, and should illustrate how\n+        to use the function.\n+\n+        >>> print([i for i in example_generator(4)])\n+        [0, 1, 2, 3]\n+\n+    \"\"\"\n+    }}}\n+    \n+More examples for numpy and google can be found at https://sphinxcontrib-napoleon.readthedocs.io/en/latest/\n+\n``````\n",
    "created_at": "2022-02-04T22:46:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616008",
    "user": "https://github.com/tobiasdiez"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,64 @@
 It's worth considering to migrate from Sage's custom documentation style to one of the more established/standarized ones such as:
 - [pep257](https://www.python.org/dev/peps/pep-0257/)
 - [numpy](https://numpydoc.readthedocs.io/en/latest/format.html)
-- [​https://google.github.io/styleguide/pyguide.html#383-functions-and-methods google]
+
+```python
+def function(param1: int, param2: str) -> bool:
+    """
+    Example function doing nothing.
+
+
+    Parameters
+    ----------
+    param1
+        The first parameter.
+    param2
+        The second parameter.
+        The description may span multiple lines. Following
+        lines should keep the indention.
+
+    Returns
+    -------
+    bool
+        True if successful, False otherwise.
+
+    Examples
+    --------
+    Examples should be written in doctest format, and should illustrate how
+    to use the function.
+
+    >>> print([i for i in example_generator(4)])
+    [0, 1, 2, 3]
+
+    """
+```
+    
+    The return type must be duplicated in the docstring to comply with the NumPy docstring style.
+- [google](https://google.github.io/styleguide/pyguide.html#383-functions-and-methods)
+
+```python
+def function(param1: int, param2: str) -> bool:
+    """
+    Example function doing nothing.
+
+    Args:
+        param1: The first parameter.
+        param2: The second parameter.
+            The description may span multiple lines. Following
+            lines should be indented.
+
+    Returns:
+        The return value. True for success, False otherwise.
+
+    Examples:
+        Examples should be written in doctest format, and should illustrate how
+        to use the function.
+
+        >>> print([i for i in example_generator(4)])
+        [0, 1, 2, 3]
+
+    """
+    }}}
+    
+More examples for numpy and google can be found at https://sphinxcontrib-napoleon.readthedocs.io/en/latest/
+
``````




---

archive/issue_comments_616009.json:
```json
{
    "body": "<a id='comment:3'></a>I don't understand the quote \"Google style tends to be easier to read for short and simple docstrings, whereas NumPy style tends be easier to read for long and in-depth docstrings\" from the napoleon web page. I don't care that much about which is easier to read in raw form\u00a0\u2014\u00a0I care more about how they are rendered by Sphinx, and we should have a lot of control over that. Or am I missing something? If the Sphinx rendering varies for the various styles, then it would be great to have examples of the rendered documentation, not just the raw code.",
    "created_at": "2022-02-04T22:59:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616009",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'></a>I don't understand the quote "Google style tends to be easier to read for short and simple docstrings, whereas NumPy style tends be easier to read for long and in-depth docstrings" from the napoleon web page. I don't care that much about which is easier to read in raw form — I care more about how they are rendered by Sphinx, and we should have a lot of control over that. Or am I missing something? If the Sphinx rendering varies for the various styles, then it would be great to have examples of the rendered documentation, not just the raw code.



---

archive/issue_comments_616010.json:
```json
{
    "body": "<a id='comment:4'></a>Both are rendered exactly the same. Napoleon converts the google or numpy format into restructured text with the correct annotations (like `:param:`) and then you can feed it into [autodoc](https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html#module-sphinx.ext.autodoc), where it is processed by spinx and displayed correctly. So the decision google vs numpy vs annotated-restructured is only about which form we as the developer want to write.\n\nThe display/rendering format can indeed be customized very easily (in a second step, after it's properly parsed by `autodoc`).",
    "created_at": "2022-02-04T23:39:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616010",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:4'></a>Both are rendered exactly the same. Napoleon converts the google or numpy format into restructured text with the correct annotations (like `:param:`) and then you can feed it into [autodoc](https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html#module-sphinx.ext.autodoc), where it is processed by spinx and displayed correctly. So the decision google vs numpy vs annotated-restructured is only about which form we as the developer want to write.

The display/rendering format can indeed be customized very easily (in a second step, after it's properly parsed by `autodoc`).



---

archive/issue_comments_616011.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [gh-tobiasdiez](#comment%3A4):\n> Both are rendered exactly the same. Napoleon converts the google or numpy format into restructured text with the correct annotations (like `:param:`) and then you can feed it into [autodoc](https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html#module-sphinx.ext.autodoc), where it is processed by spinx and displayed correctly.\n\n\nWould it be an option to implement this for the style Sage currently uses? I would expect this to be more manageable than changing every docstring to a different style.",
    "created_at": "2022-02-05T11:02:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616011",
    "user": "https://github.com/mwageringel"
}
```

<a id='comment:5'></a>Replying to [gh-tobiasdiez](#comment%3A4):
> Both are rendered exactly the same. Napoleon converts the google or numpy format into restructured text with the correct annotations (like `:param:`) and then you can feed it into [autodoc](https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html#module-sphinx.ext.autodoc), where it is processed by spinx and displayed correctly.


Would it be an option to implement this for the style Sage currently uses? I would expect this to be more manageable than changing every docstring to a different style.



---

archive/issue_comments_616012.json:
```json
{
    "body": "<a id='comment:6'></a>Replying to [gh-mwageringel](#comment%3A5):\n> Replying to [gh-tobiasdiez](#comment%3A4):\n> > Both are rendered exactly the same. Napoleon converts the google or numpy format into restructured text with the correct annotations (like `:param:`) and then you can feed it into [autodoc](https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html#module-sphinx.ext.autodoc), where it is processed by spinx and displayed correctly.\n\n> \n> Would it be an option to implement this for the style Sage currently uses? I would expect this to be more manageable than changing every docstring to a different style.\n\n\nIt's certainly possible but napoleon's parsing is already quite involved and we would need to duplicate that.\n\nBy using a standard convention for docstrings we would also get all the other advantages that come with following standards like:\n- Easier onboarding of newcomers that don't need to learn a new syntax for docstrings.\n- IDEs interpret standard docstrings properly and provide the doc as well as type information in a nicely formatted way. Example:\n<img src=\"https://resources.jetbrains.com/help/img/idea/2021.3/docstring_example_10_quick_documentation.png\" 520px>\n- IDEs support generating docstrings in the right format. Example:\n<img src=\"https://github.com/NilsJPWerner/autoDocstring/raw/master/images/demo.gif\" 700px>\n- There exist proper linter for the docstring format like [pycodestyle](http://www.pydocstyle.org/en/stable/index.html). In contrast, our \"linter\" currently uses a couple of regex to check that its \"TESTS\" instead of \"TEST\".\n\nThe migration can also be very fluent; for example, if one introduces a new method or change an existing one then one uses a standard docstring format. Thus, over time the codebase gets slowly migrated.",
    "created_at": "2022-02-05T13:15:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616012",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:6'></a>Replying to [gh-mwageringel](#comment%3A5):
> Replying to [gh-tobiasdiez](#comment%3A4):
> > Both are rendered exactly the same. Napoleon converts the google or numpy format into restructured text with the correct annotations (like `:param:`) and then you can feed it into [autodoc](https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html#module-sphinx.ext.autodoc), where it is processed by spinx and displayed correctly.

> 
> Would it be an option to implement this for the style Sage currently uses? I would expect this to be more manageable than changing every docstring to a different style.


It's certainly possible but napoleon's parsing is already quite involved and we would need to duplicate that.

By using a standard convention for docstrings we would also get all the other advantages that come with following standards like:
- Easier onboarding of newcomers that don't need to learn a new syntax for docstrings.
- IDEs interpret standard docstrings properly and provide the doc as well as type information in a nicely formatted way. Example:
<img src="https://resources.jetbrains.com/help/img/idea/2021.3/docstring_example_10_quick_documentation.png" 520px>
- IDEs support generating docstrings in the right format. Example:
<img src="https://github.com/NilsJPWerner/autoDocstring/raw/master/images/demo.gif" 700px>
- There exist proper linter for the docstring format like [pycodestyle](http://www.pydocstyle.org/en/stable/index.html). In contrast, our "linter" currently uses a couple of regex to check that its "TESTS" instead of "TEST".

The migration can also be very fluent; for example, if one introduces a new method or change an existing one then one uses a standard docstring format. Thus, over time the codebase gets slowly migrated.



---

archive/issue_comments_616013.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -60,5 +60,4 @@\n     \"\"\"\n     }}}\n     \n-More examples for numpy and google can be found at https://sphinxcontrib-napoleon.readthedocs.io/en/latest/\n-\n+More examples for numpy and google can be found at https://www.sphinx-doc.org/en/master/usage/extensions/napoleon.html\n``````\n",
    "created_at": "2022-02-06T00:44:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616013",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -60,5 +60,4 @@
     """
     }}}
     
-More examples for numpy and google can be found at https://sphinxcontrib-napoleon.readthedocs.io/en/latest/
-
+More examples for numpy and google can be found at https://www.sphinx-doc.org/en/master/usage/extensions/napoleon.html
``````




---

archive/issue_comments_616014.json:
```json
{
    "body": "<a id='comment:8'></a>Related: #30884, #30893",
    "created_at": "2022-02-06T00:46:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616014",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:8'></a>Related: #30884, #30893



---

archive/issue_comments_616015.json:
```json
{
    "body": "<a id='comment:9'></a>Currently our \"TESTS\" section does not appear in the documentation unless `--include-tests-blocks` is passed to the docbuilder. How can we reproduce that with a different format? Maybe use `napoleon_custom_sections` to create a \"TESTS\" section?",
    "created_at": "2022-02-13T05:22:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616015",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:9'></a>Currently our "TESTS" section does not appear in the documentation unless `--include-tests-blocks` is passed to the docbuilder. How can we reproduce that with a different format? Maybe use `napoleon_custom_sections` to create a "TESTS" section?



---

archive/issue_comments_616016.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [jhpalmieri](#comment%3A9):\n> Currently our \"TESTS\" section does not appear in the documentation unless `--include-tests-blocks` is passed to the docbuilder. How can we reproduce that with a different format? Maybe use `napoleon_custom_sections` to create a \"TESTS\" section?\n\n\nYes, that should work.",
    "created_at": "2022-02-13T09:24:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616016",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:10'></a>Replying to [jhpalmieri](#comment%3A9):
> Currently our "TESTS" section does not appear in the documentation unless `--include-tests-blocks` is passed to the docbuilder. How can we reproduce that with a different format? Maybe use `napoleon_custom_sections` to create a "TESTS" section?


Yes, that should work.



---

archive/issue_comments_616017.json:
```json
{
    "body": "<a id='comment:11'></a>I think that a big obstacle to this is #30893. I tried adding `'sphinx.ext.napoleon'` to our Sphinx extensions and ran into various error messages.",
    "created_at": "2022-02-14T22:59:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616017",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:11'></a>I think that a big obstacle to this is #30893. I tried adding `'sphinx.ext.napoleon'` to our Sphinx extensions and ran into various error messages.



---

archive/issue_comments_616018.json:
```json
{
    "body": "<a id='comment:12'></a>Yes, this will probably require a few changes to our doc builder. I would be willing to work on this, but before starting this project we should decide if we indeed want to migrate to a different docstyle. Should I start a discussion on the dev list, or what would be a good process?",
    "created_at": "2022-02-15T12:19:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616018",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:12'></a>Yes, this will probably require a few changes to our doc builder. I would be willing to work on this, but before starting this project we should decide if we indeed want to migrate to a different docstyle. Should I start a discussion on the dev list, or what would be a good process?



---

archive/issue_comments_616019.json:
```json
{
    "body": "<a id='comment:13'></a>Ideally, we could start to use a new docstyle while the old one still functions, and then people could see samples within the Sage library of both. Regardless, the Sage community needs to make two choices: do we want to switch to a different default format, and if so, which one? \n\nSo maybe the right thing to do is to start a discussion on sage-devel about it. If there is general support, then the next steps would be to work on #30893, enable napoleon, and convert a few files to the new format. Maybe try to write a script to convert the current format to another one and apply it to parts of the Sage library, say one directory at a time.",
    "created_at": "2022-02-15T18:50:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31044#issuecomment-616019",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:13'></a>Ideally, we could start to use a new docstyle while the old one still functions, and then people could see samples within the Sage library of both. Regardless, the Sage community needs to make two choices: do we want to switch to a different default format, and if so, which one? 

So maybe the right thing to do is to start a discussion on sage-devel about it. If there is general support, then the next steps would be to work on #30893, enable napoleon, and convert a few files to the new format. Maybe try to write a script to convert the current format to another one and apply it to parts of the Sage library, say one directory at a time.
