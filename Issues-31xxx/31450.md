# Issue 31450: Add functionalities for GRS attack to linear_rank_metric.py

archive/issues_031213.json:
```json
{
    "assignees": [],
    "body": "To perform the GRS attack described by Gaborit et al. in \"On the complexity of the rank syndrome decoding problem\", functionalities need to be added to `AbstractLinearRankMetricCode` to:\n- Find an error vector of a certain rank.\n- Apply the canonical projection to a element in the extended field.\n- Find a subspace of the extended field of given dimension.\n- Apply the GRS algorithm.\n\nCC:  @MaaikevLeuken\n\nKeywords: **rank code**\n\nBranch/Commit: **[u/gh-MaaikevLeuken/add_low_rank_parity_check_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-MaaikevLeuken/add_low_rank_parity_check_codes) @ [a9a274e](https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66)**\n\nAuthor: **Maaike van Leuken**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31450_\n\n",
    "created_at": "2021-03-02T21:15:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add functionalities for GRS attack to linear_rank_metric.py",
    "type": "issue",
    "updated_at": "2022-08-31T05:25:02Z",
    "url": "https://github.com/sagemath/sage/issues/31450",
    "user": "https://github.com/MaaikevLeuken"
}
```
To perform the GRS attack described by Gaborit et al. in "On the complexity of the rank syndrome decoding problem", functionalities need to be added to `AbstractLinearRankMetricCode` to:
- Find an error vector of a certain rank.
- Apply the canonical projection to a element in the extended field.
- Find a subspace of the extended field of given dimension.
- Apply the GRS algorithm.

CC:  @MaaikevLeuken

Keywords: **rank code**

Branch/Commit: **[u/gh-MaaikevLeuken/add_low_rank_parity_check_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-MaaikevLeuken/add_low_rank_parity_check_codes) @ [a9a274e](https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66)**

Author: **Maaike van Leuken**

_Issue created by migration from https://trac.sagemath.org/ticket/31450_





---

archive/issue_events_415314.json:
```json
{
    "actor": "https://github.com/MaaikevLeuken",
    "created_at": "2021-03-02T21:15:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415314"
}
```



---

archive/issue_events_415315.json:
```json
{
    "actor": "https://github.com/MaaikevLeuken",
    "created_at": "2021-03-02T21:15:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
    "label_color": "0000ff",
    "label_name": "component: coding theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415315"
}
```



---

archive/issue_events_415316.json:
```json
{
    "actor": "https://github.com/MaaikevLeuken",
    "created_at": "2021-03-02T21:15:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415316"
}
```



---

archive/issue_events_415317.json:
```json
{
    "actor": "https://github.com/MaaikevLeuken",
    "created_at": "2021-03-02T21:15:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415317"
}
```



---

archive/issue_comments_508050.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31450#issuecomment-508050",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_415318.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415318"
}
```



---

archive/issue_events_415319.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415319"
}
```



---

archive/issue_comments_508051.json:
```json
{
    "body": "Commit: **[8c92768](https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33)**",
    "created_at": "2021-05-16T15:55:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31450#issuecomment-508051",
    "user": "https://github.com/MaaikevLeuken"
}
```

Commit: **[8c92768](https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33)**



---

archive/issue_comments_508052.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33\">8c92768</a></td><td><code>Added Low Rank Parity Check codes and added functionalities for linear_rank_metric</code></td></tr></table>\n",
    "created_at": "2021-05-16T15:55:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31450#issuecomment-508052",
    "user": "https://github.com/MaaikevLeuken"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33">8c92768</a></td><td><code>Added Low Rank Parity Check codes and added functionalities for linear_rank_metric</code></td></tr></table>




---

archive/issue_comments_508053.json:
```json
{
    "body": "Branch: **[u/gh-MaaikevLeuken/add_low_rank_parity_check_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-MaaikevLeuken/add_low_rank_parity_check_codes)**",
    "created_at": "2021-05-16T15:55:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31450#issuecomment-508053",
    "user": "https://github.com/MaaikevLeuken"
}
```

Branch: **[u/gh-MaaikevLeuken/add_low_rank_parity_check_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-MaaikevLeuken/add_low_rank_parity_check_codes)**



---

archive/issue_comments_508054.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66\">a9a274e</a></td><td><code>Improved formatting.</code></td></tr></table>\n",
    "created_at": "2021-05-17T12:08:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31450#issuecomment-508054",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66">a9a274e</a></td><td><code>Improved formatting.</code></td></tr></table>




---

archive/issue_comments_508055.json:
```json
{
    "body": "Changed commit from **[8c92768](https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33)** to **[a9a274e](https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66)**",
    "created_at": "2021-05-17T12:08:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31450#issuecomment-508055",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8c92768](https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33)** to **[a9a274e](https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66)**



---

archive/issue_events_415320.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415320"
}
```



---

archive/issue_events_415321.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415321"
}
```



---

archive/issue_events_415322.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415322"
}
```



---

archive/issue_events_415323.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415323"
}
```



---

archive/issue_events_415324.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415324"
}
```



---

archive/issue_events_415325.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415325"
}
```



---

archive/issue_events_415326.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415326"
}
```



---

archive/issue_events_415327.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31450",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31450#event-415327"
}
```
