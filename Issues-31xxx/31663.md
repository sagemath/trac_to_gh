# Issue 31663: build/pkgs/numpy/lapack_conf.py: Use #! for interpreter that is actually used

archive/issues_031426.json:
```json
{
    "assignees": [],
    "body": "(from [#29039 comment:78](https://github.com/sagemath/sage/issues/29039#comment:78))\n\nnumpy's `spkg-install.in` runs `python3 lapack_conf.py` (i.e., correctly, with the Sage `python3`), but `lapack_conf.py` confusingly indicates `#!/usr/bin/env sage-bootstrap-python` (which is the wrong Python; but in `spkg-install.in`, the script is not called by the shell, so this `#!` is not actually interpreted). \n\nWe remove this confusion in this ticket, but there is no functional change.\n\n\n\nCC:  @dimpase\n\nBranch/Commit: **[0ac27ff](https://github.com/sagemath/sagetrac-mirror/commit/0ac27ff147e7e5f1c41d83142894dc49869a171e)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31663_\n\n",
    "closed_at": "2021-07-23T20:11:18Z",
    "created_at": "2021-04-12T16:18:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "build/pkgs/numpy/lapack_conf.py: Use #! for interpreter that is actually used",
    "type": "issue",
    "updated_at": "2021-07-23T20:11:18Z",
    "url": "https://github.com/sagemath/sage/issues/31663",
    "user": "https://github.com/mkoeppe"
}
```
(from [#29039 comment:78](https://github.com/sagemath/sage/issues/29039#comment:78))

numpy's `spkg-install.in` runs `python3 lapack_conf.py` (i.e., correctly, with the Sage `python3`), but `lapack_conf.py` confusingly indicates `#!/usr/bin/env sage-bootstrap-python` (which is the wrong Python; but in `spkg-install.in`, the script is not called by the shell, so this `#!` is not actually interpreted). 

We remove this confusion in this ticket, but there is no functional change.



CC:  @dimpase

Branch/Commit: **[0ac27ff](https://github.com/sagemath/sagetrac-mirror/commit/0ac27ff147e7e5f1c41d83142894dc49869a171e)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/31663_





---

archive/issue_events_418521.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-12T16:18:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418521"
}
```



---

archive/issue_events_418522.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-12T16:18:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418522"
}
```



---

archive/issue_events_418523.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-12T16:18:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418523"
}
```



---

archive/issue_events_418524.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-12T16:18:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418524"
}
```



---

archive/issue_comments_512241.json:
```json
{
    "body": "Branch: **[u/mkoeppe/build_pkgs_numpy_lapack_conf_py__use____for_interpreter_that_is_actually_used](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build_pkgs_numpy_lapack_conf_py__use____for_interpreter_that_is_actually_used)**",
    "created_at": "2021-04-12T16:22:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31663#issuecomment-512241",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/build_pkgs_numpy_lapack_conf_py__use____for_interpreter_that_is_actually_used](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build_pkgs_numpy_lapack_conf_py__use____for_interpreter_that_is_actually_used)**



---

archive/issue_comments_512242.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-04-12T16:23:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31663#issuecomment-512242",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_512243.json:
```json
{
    "body": "Commit: **[0ac27ff](https://github.com/sagemath/sagetrac-mirror/commit/0ac27ff147e7e5f1c41d83142894dc49869a171e)**",
    "created_at": "2021-04-12T16:23:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31663#issuecomment-512243",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[0ac27ff](https://github.com/sagemath/sagetrac-mirror/commit/0ac27ff147e7e5f1c41d83142894dc49869a171e)**



---

archive/issue_events_418525.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-12T16:23:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418525"
}
```



---

archive/issue_comments_512244.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0ac27ff147e7e5f1c41d83142894dc49869a171e\">0ac27ff</a></td><td><code>build/pkgs/numpy/lapack_conf.py: Change #! line to use python3.  Decorative change only - spkg-install.in already explicitly uses python3 to call this script</code></td></tr></table>\n",
    "created_at": "2021-04-12T16:23:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31663#issuecomment-512244",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0ac27ff147e7e5f1c41d83142894dc49869a171e">0ac27ff</a></td><td><code>build/pkgs/numpy/lapack_conf.py: Change #! line to use python3.  Decorative change only - spkg-install.in already explicitly uses python3 to call this script</code></td></tr></table>




---

archive/issue_comments_512245.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2021-04-12T16:25:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31663#issuecomment-512245",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_512246.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nthanks!",
    "created_at": "2021-04-12T16:25:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31663#issuecomment-512246",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:3'>Comment 3:</a>
thanks!



---

archive/issue_events_418526.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-04-12T16:25:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418526"
}
```



---

archive/issue_events_418527.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-04-12T16:25:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418527"
}
```



---

archive/issue_events_418528.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T20:57:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418528"
}
```



---

archive/issue_events_418529.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T20:57:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418529"
}
```



---

archive/issue_comments_512247.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPromoting the oldest 5 positively reviewed tickets to \"major\"",
    "created_at": "2021-07-19T20:57:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31663#issuecomment-512247",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Promoting the oldest 5 positively reviewed tickets to "major"



---

archive/issue_events_418530.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-23T20:11:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418530"
}
```



---

archive/issue_events_418531.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "87e0b803a8ddeab1abf56d22a3a5b77f0cef4e96",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-07-23T20:11:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31663#event-418531"
}
```



---

archive/issue_comments_512248.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/build_pkgs_numpy_lapack_conf_py__use____for_interpreter_that_is_actually_used](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build_pkgs_numpy_lapack_conf_py__use____for_interpreter_that_is_actually_used)** to **[0ac27ff](https://github.com/sagemath/sagetrac-mirror/commit/0ac27ff147e7e5f1c41d83142894dc49869a171e)**",
    "created_at": "2021-07-23T20:11:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31663#issuecomment-512248",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/build_pkgs_numpy_lapack_conf_py__use____for_interpreter_that_is_actually_used](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build_pkgs_numpy_lapack_conf_py__use____for_interpreter_that_is_actually_used)** to **[0ac27ff](https://github.com/sagemath/sagetrac-mirror/commit/0ac27ff147e7e5f1c41d83142894dc49869a171e)**
