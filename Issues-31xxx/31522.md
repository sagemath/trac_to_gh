# Issue 31522: GH Actions: tox-optional.yml: Remove misleading error messages regarding optional script packages

archive/issues_031285.json:
```json
{
    "assignees": [],
    "body": "For example on `ubuntu-trusty-maximal` (https://github.com/mkoeppe/sage/runs/2135025659), \n`configure` correctly finds system `perl_cpan_polymake_prereq`\nbut then tests whether SPKG `perl_cpan_polymake_prereq` can be installed (it gives an error - by design).\n\nThis can be distracting in the GH Action logs.\n\n\nDepends on #31163\n\nCC:  @kliem @jhpalmieri\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31522_\n\n",
    "closed_at": "2022-09-01T02:30:35Z",
    "created_at": "2021-03-19T19:01:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "GH Actions: tox-optional.yml: Remove misleading error messages regarding optional script packages",
    "type": "issue",
    "updated_at": "2022-09-01T02:30:35Z",
    "url": "https://github.com/sagemath/sage/issues/31522",
    "user": "https://github.com/mkoeppe"
}
```
For example on `ubuntu-trusty-maximal` (https://github.com/mkoeppe/sage/runs/2135025659), 
`configure` correctly finds system `perl_cpan_polymake_prereq`
but then tests whether SPKG `perl_cpan_polymake_prereq` can be installed (it gives an error - by design).

This can be distracting in the GH Action logs.


Depends on #31163

CC:  @kliem @jhpalmieri

_Issue created by migration from https://trac.sagemath.org/ticket/31522_





---

archive/issue_events_416295.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-19T19:01:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416295"
}
```



---

archive/issue_events_416296.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-19T19:01:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416296"
}
```



---

archive/issue_events_416297.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-19T19:01:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416297"
}
```



---

archive/issue_events_416298.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-19T19:01:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416298"
}
```



---

archive/issue_comments_509235.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nA possible fix would be through #31163",
    "created_at": "2021-03-19T20:11:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31522#issuecomment-509235",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

A possible fix would be through #31163



---

archive/issue_comments_509236.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31522#issuecomment-509236",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_416299.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416299"
}
```



---

archive/issue_events_416300.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416300"
}
```



---

archive/issue_events_416301.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416301"
}
```



---

archive/issue_events_416302.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416302"
}
```



---

archive/issue_comments_509237.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nTaken care of by #31163",
    "created_at": "2021-09-18T19:23:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31522#issuecomment-509237",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Taken care of by #31163



---

archive/issue_comments_509238.json:
```json
{
    "body": "Dependencies: **#31163**",
    "created_at": "2021-09-18T19:23:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31522#issuecomment-509238",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#31163**



---

archive/issue_events_416303.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-18T19:23:59Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416303"
}
```



---

archive/issue_events_416304.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-18T19:23:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416304"
}
```



---

archive/issue_events_416305.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-18T19:23:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416305"
}
```



---

archive/issue_events_416306.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-18T19:23:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416306"
}
```



---

archive/issue_events_416307.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-08-06T20:42:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416307"
}
```



---

archive/issue_events_416308.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-08-06T20:42:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416308"
}
```



---

archive/issue_events_416309.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-01T02:30:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416309"
}
```



---

archive/issue_events_416310.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-01T02:30:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31522",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31522#event-416310"
}
```
