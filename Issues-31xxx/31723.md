# Issue 31723: Patterson Decoder for binary Goppa codes

archive/issues_031486.json:
```json
{
    "assignees": [],
    "body": "Decoder for binary Goppa codes whose defining polynomials are square free which uses Patterson decoding algorithm in order to correct errors in words.\nThe Patterson decoder makes use of the terminated extended Euclidean algorithm.\nFor further details see \n\nN. Patterson: The algebraic decoding of Goppa codes, IEEE Transactions on Information Theory 21.2 (1975), pp. 203-207.\n\nJohan S. H. Rosenkilde gave us a feedback on the first draft of this decoder and also suggested us possible reviewers. \nSo, we would like to thank him.\n\nCC:  @xcaruso @dimpase\n\nKeywords: **coding theory, goppa codes, patterson decoder**\n\nBranch/Commit: **[u/gh-giucesare/patterson_decoder](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/patterson_decoder) @ [e62cd8a](https://github.com/sagemath/sagetrac-mirror/commit/e62cd8a8b7b1af149b9d445d3b58535cc0702d35)**\n\nReviewer: **Xavier Caruso, Dima Pasechnik**\n\nAuthor: **Giuseppe Cesare, Ferdinando Zullo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31723_\n\n",
    "created_at": "2021-04-24T11:03:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20review"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Patterson Decoder for binary Goppa codes",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/31723",
    "user": "https://github.com/giucesare"
}
```
Decoder for binary Goppa codes whose defining polynomials are square free which uses Patterson decoding algorithm in order to correct errors in words.
The Patterson decoder makes use of the terminated extended Euclidean algorithm.
For further details see 

N. Patterson: The algebraic decoding of Goppa codes, IEEE Transactions on Information Theory 21.2 (1975), pp. 203-207.

Johan S. H. Rosenkilde gave us a feedback on the first draft of this decoder and also suggested us possible reviewers. 
So, we would like to thank him.

CC:  @xcaruso @dimpase

Keywords: **coding theory, goppa codes, patterson decoder**

Branch/Commit: **[u/gh-giucesare/patterson_decoder](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/patterson_decoder) @ [e62cd8a](https://github.com/sagemath/sagetrac-mirror/commit/e62cd8a8b7b1af149b9d445d3b58535cc0702d35)**

Reviewer: **Xavier Caruso, Dima Pasechnik**

Author: **Giuseppe Cesare, Ferdinando Zullo**

_Issue created by migration from https://trac.sagemath.org/ticket/31723_





---

archive/issue_events_419325.json:
```json
{
    "actor": "https://github.com/giucesare",
    "created_at": "2021-04-24T11:03:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419325"
}
```



---

archive/issue_events_419326.json:
```json
{
    "actor": "https://github.com/giucesare",
    "created_at": "2021-04-24T11:03:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
    "label_color": "0000ff",
    "label_name": "component: coding theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419326"
}
```



---

archive/issue_events_419327.json:
```json
{
    "actor": "https://github.com/giucesare",
    "created_at": "2021-04-24T11:03:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419327"
}
```



---

archive/issue_events_419328.json:
```json
{
    "actor": "https://github.com/giucesare",
    "created_at": "2021-04-24T11:03:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419328"
}
```



---

archive/issue_comments_513374.json:
```json
{
    "body": "Attachment: **[patterson_decoder.py.gz](https://github.com/sagemath/sage/files/ticket31723/patterson_decoder.py.gz)**\n\nPatterson Decoder",
    "created_at": "2021-04-24T11:04:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513374",
    "user": "https://github.com/giucesare"
}
```

Attachment: **[patterson_decoder.py.gz](https://github.com/sagemath/sage/files/ticket31723/patterson_decoder.py.gz)**

Patterson Decoder



---

archive/issue_comments_513375.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nCould you post a branch, rather than an attachment?",
    "created_at": "2021-04-24T18:52:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513375",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:1" align="right">Comment 1</div>

Could you post a branch, rather than an attachment?



---

archive/issue_comments_513376.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,8 +2,7 @@\n The Patterson decoder makes use of the terminated extended Euclidean algorithm.\n For further details see \n \n-N. Patterson: The algebraic decoding of Goppa codes, IEEE Transactions on\n-Information Theory 21.2 (1975), pp. 203-207.\n+N. Patterson: The algebraic decoding of Goppa codes, IEEE Transactions on Information Theory 21.2 (1975), pp. 203-207.\n \n Johan S. H. Rosenkilde gave us a feedback on the first draft of this decoder and also suggested us possible reviewers. \n So, we would like to thank him.\n``````\n",
    "created_at": "2021-04-26T15:41:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513376",
    "user": "https://github.com/giucesare"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,8 +2,7 @@
 The Patterson decoder makes use of the terminated extended Euclidean algorithm.
 For further details see 
 
-N. Patterson: The algebraic decoding of Goppa codes, IEEE Transactions on
-Information Theory 21.2 (1975), pp. 203-207.
+N. Patterson: The algebraic decoding of Goppa codes, IEEE Transactions on Information Theory 21.2 (1975), pp. 203-207.
 
 Johan S. H. Rosenkilde gave us a feedback on the first draft of this decoder and also suggested us possible reviewers. 
 So, we would like to thank him.
``````




---

archive/issue_comments_513377.json:
```json
{
    "body": "Branch: **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)**",
    "created_at": "2021-04-26T16:28:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513377",
    "user": "https://github.com/giucesare"
}
```

Branch: **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)**



---

archive/issue_comments_513378.json:
```json
{
    "body": "Changed branch from **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)** to none",
    "created_at": "2021-04-26T16:29:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513378",
    "user": "https://github.com/giucesare"
}
```

Changed branch from **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)** to none



---

archive/issue_comments_513379.json:
```json
{
    "body": "Branch: **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)**",
    "created_at": "2021-04-26T17:01:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513379",
    "user": "https://github.com/giucesare"
}
```

Branch: **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)**



---

archive/issue_comments_513380.json:
```json
{
    "body": "Changed branch from **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)** to none",
    "created_at": "2021-04-26T17:03:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513380",
    "user": "https://github.com/giucesare"
}
```

Changed branch from **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)** to none



---

archive/issue_comments_513381.json:
```json
{
    "body": "Branch: **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)**",
    "created_at": "2021-04-27T17:08:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513381",
    "user": "https://github.com/giucesare"
}
```

Branch: **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)**



---

archive/issue_comments_513382.json:
```json
{
    "body": "Changed branch from **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)** to none",
    "created_at": "2021-04-27T17:22:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513382",
    "user": "https://github.com/giucesare"
}
```

Changed branch from **[u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/31723-patterson_decoder_for_binary_goppa_codes)** to none



---

archive/issue_comments_513383.json:
```json
{
    "body": "Branch: **[u/gh-giucesare/patterson_decoder](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/patterson_decoder)**",
    "created_at": "2021-04-27T19:10:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513383",
    "user": "https://github.com/giucesare"
}
```

Branch: **[u/gh-giucesare/patterson_decoder](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-giucesare/patterson_decoder)**



---

archive/issue_comments_513384.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nafter many attempts maybe I did it correctly now... sorry, this is my first ticket.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e62cd8a8b7b1af149b9d445d3b58535cc0702d35\">e62cd8a</a></td><td><code>Decoder for binary Goppa codes whose defining polynomials are square free which uses Patterson decoding algorithm in order to correct errors in words.</code></td></tr></table>\n",
    "created_at": "2021-04-27T19:18:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513384",
    "user": "https://github.com/giucesare"
}
```

<div id="comment:10" align="right">Comment 10</div>

after many attempts maybe I did it correctly now... sorry, this is my first ticket.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e62cd8a8b7b1af149b9d445d3b58535cc0702d35">e62cd8a</a></td><td><code>Decoder for binary Goppa codes whose defining polynomials are square free which uses Patterson decoding algorithm in order to correct errors in words.</code></td></tr></table>




---

archive/issue_comments_513385.json:
```json
{
    "body": "Commit: **[e62cd8a](https://github.com/sagemath/sagetrac-mirror/commit/e62cd8a8b7b1af149b9d445d3b58535cc0702d35)**",
    "created_at": "2021-04-27T19:18:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513385",
    "user": "https://github.com/giucesare"
}
```

Commit: **[e62cd8a](https://github.com/sagemath/sagetrac-mirror/commit/e62cd8a8b7b1af149b9d445d3b58535cc0702d35)**



---

archive/issue_comments_513386.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThe method `_partial_xgcd_gen` is called only once; is it really useful to have a special method for this? (Something that makes more sense according to me is to have a general shared function for this task which could be defined for example in the class handling univariate polynomials; but, more simply, you can just put your code directly in the caller method `_decode_to_code_and_message`.)\n\nAnother general remark is that the documentation is not always correctly formatted.\nSee https://doc.sagemath.org/html/en/developer/coding_basics.html#the-docstring-of-a-function-content for details.\nIn addition you should probably add TESTS sections (in the documentation) for checking that errors are handled correctly.",
    "created_at": "2021-04-28T14:18:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513386",
    "user": "https://github.com/xcaruso"
}
```

<div id="comment:11" align="right">Comment 11</div>

The method `_partial_xgcd_gen` is called only once; is it really useful to have a special method for this? (Something that makes more sense according to me is to have a general shared function for this task which could be defined for example in the class handling univariate polynomials; but, more simply, you can just put your code directly in the caller method `_decode_to_code_and_message`.)

Another general remark is that the documentation is not always correctly formatted.
See https://doc.sagemath.org/html/en/developer/coding_basics.html#the-docstring-of-a-function-content for details.
In addition you should probably add TESTS sections (in the documentation) for checking that errors are handled correctly.



---

archive/issue_events_419329.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2021-04-28T14:18:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419329"
}
```



---

archive/issue_comments_513387.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nOk, we will make these changes then. Thank you.",
    "created_at": "2021-05-02T00:11:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513387",
    "user": "https://github.com/giucesare"
}
```

<div id="comment:12" align="right">Comment 12</div>

Ok, we will make these changes then. Thank you.



---

archive/issue_comments_513388.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nSetting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513388",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:13" align="right">Comment 13</div>

Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_419330.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419330"
}
```



---

archive/issue_events_419331.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419331"
}
```



---

archive/issue_events_419332.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419332"
}
```



---

archive/issue_events_419333.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419333"
}
```



---

archive/issue_comments_513389.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nStalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.",
    "created_at": "2021-12-18T19:53:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31723#issuecomment-513389",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:14" align="right">Comment 14</div>

Stalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.



---

archive/issue_events_419334.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-11T17:19:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419334"
}
```



---

archive/issue_events_419335.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-11T17:19:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419335"
}
```



---

archive/issue_events_419336.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419336"
}
```



---

archive/issue_events_419337.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31723",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31723#event-419337"
}
```
