...
```json
{
    "body": "This is required by giac, see #30537",
...
```json
{
    "body": "For the record, debian and therefore ubuntu have shipped `pari` with threads for a while now. But their pari is also statically linked. Do we have speed regression examples with the library as well as the executable?",
...
```json
{
    "body": "Perhaps giac can use single-threaded Pari, as well? It can test whether Pari is single- or multi-threaded, and use different code paths for these two scenarios.",
...
```json
{
    "body": "FTR, upstream pari recommends compiling gp with -lto to make up for the performance loss caused by multithreading. We've been doing that in the Arch package for a while.",
