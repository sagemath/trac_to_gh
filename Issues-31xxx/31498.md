...
<a id='comment:5'></a>what remains is to fix this weird error - something is emitting that string of `*` whcih overrides the test output. Not reproducible at `sage:` prompt:
...
<a id='comment:8'></a>another test to fix:
...
```json
{
    "body": "<a id='comment:12'></a>I won't lie, I am not quite ready for this in sage-on-gentoo. I have been under rather heavy stress so far this year and I am not doing as much as I want.",
...
<a id='comment:12'></a>I won't lie, I am not quite ready for this in sage-on-gentoo. I have been under rather heavy stress so far this year and I am not doing as much as I want.
...
<a id='comment:13'></a>Tell me about it ;-) 
...
<a id='comment:16'></a>There is a small merge conflict with #31489, and a failure
...
<a id='comment:18'></a>these test_packages stuff is a mess. We have two different functions
that both claim to `Return list of all installed packages.`,  `test_packages()`, and `all_installed_packages()` which differ in the output format - the former outputs `table` (sic!)
and the latter does not check whether these packages actually may be loaded (so it's largely meaningless).
...
```json
{
    "body": "<a id='comment:20'></a>I believe the test is fixed now.",
    "created_at": "2021-06-01T23:50:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31498",
    "type": "issue_comment",
...
<a id='comment:20'></a>I believe the test is fixed now.
...
```json
{
    "body": "<a id='comment:23'></a>OK, now it depends on #31489 and should cleanly merge: the only change from #31489 is done in[comment:22](#comment%3A22)",
...
<a id='comment:23'></a>OK, now it depends on #31489 and should cleanly merge: the only change from #31489 is done in[comment:22](#comment%3A22)
...
```json
{
    "body": "<a id='comment:24'></a>I have not merged the full branch of #31489 here, though.",
...
<a id='comment:24'></a>I have not merged the full branch of #31489 here, though.
...
```json
{
    "body": "<a id='comment:26'></a>rebased on 9.4.beta1",
...
<a id='comment:26'></a>rebased on 9.4.beta1
...
```json
{
    "body": "<a id='comment:28'></a>Let's try to get this into the next beta.",
    "created_at": "2021-06-11T03:46:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31498",
    "type": "issue_comment",
...
<a id='comment:28'></a>Let's try to get this into the next beta.
...
<a id='comment:30'></a>I am a bit dumbfounded by this upgrade in a way. There are strange particulars to it in sage-on-gentoo. I cannot build the doc.
...
```json
{
    "body": "<a id='comment:31'></a>Whats the output of `_get_gap_memory_pool_size_MB`? \n\nGAP needs swap due to how they reserve the memory pool.",
...
<a id='comment:31'></a>Whats the output of `_get_gap_memory_pool_size_MB`? 
