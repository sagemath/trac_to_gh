# Issue 31569: sage-spkg DESTDIR staging fails for Python packages when SAGE_LOCAL is not a physical pathname

archive/issues_031332.json:
```json
{
    "body": "For example, on macOS, there is the symlink `/var -> /private/var`, and so if `SAGE_LOCAL=/var/tmp/xyz`, the Python package when installing to the staging dir may resolve SAGE_LOCAL to the absolute physical pathname; but `sage-spkg` only copies from $(SAGE_DESTDIR)$(SAGE_LOCAL) and will miss the entire installation.\n\n\nCC:  @dimpase\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/31569\n\n",
    "created_at": "2021-03-27T07:16:06Z",
    "labels": [
        "component: build",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-wishlist",
    "title": "sage-spkg DESTDIR staging fails for Python packages when SAGE_LOCAL is not a physical pathname",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/31569",
    "user": "https://github.com/mkoeppe"
}
```
For example, on macOS, there is the symlink `/var -> /private/var`, and so if `SAGE_LOCAL=/var/tmp/xyz`, the Python package when installing to the staging dir may resolve SAGE_LOCAL to the absolute physical pathname; but `sage-spkg` only copies from $(SAGE_DESTDIR)$(SAGE_LOCAL) and will miss the entire installation.


CC:  @dimpase

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/31569





---

archive/issue_events_083000.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-04T18:58:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31569",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31569#event-83000"
}
```



---

archive/issue_comments_478299.json:
```json
{
    "body": "Changing priority from critical to minor.",
    "created_at": "2021-08-02T00:08:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31569#issuecomment-478299",
    "user": "https://github.com/mkoeppe"
}
```

Changing priority from critical to minor.



---

archive/issue_events_083001.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-02T00:08:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31569",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31569#event-83001"
}
```



---

archive/issue_events_083002.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-02T00:08:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31569",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31569#event-83002"
}
```



---

archive/issue_comments_478300.json:
```json
{
    "body": "<a id='comment:2'></a>After #29585 (Remove DESTDIR staging for Python packages), this general defect no longer affects Python packages; but the issue may reappear if the build system of some other (non-Python) package decides to resolve pathnames to physical.",
    "created_at": "2021-08-02T00:08:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31569#issuecomment-478300",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'></a>After #29585 (Remove DESTDIR staging for Python packages), this general defect no longer affects Python packages; but the issue may reappear if the build system of some other (non-Python) package decides to resolve pathnames to physical.
