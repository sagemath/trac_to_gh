# Issue 31749: MixedIntegerLinearProgram: New generator method 'variables'

archive/issues_031512.json:
```json
{
    "body": "We modify `MixedIntegerLinearProgram.new_variable` so that it remembers the variables.\n\nTo avoid reintroducing reference loops eliminated in #12616, we modify `MIPVariable` so that it no longer stores a reference to the MIP but rather to the `backend`, the `base_ring`, and the `linear_functions_parent`.\n\nA new method `M.variables()` yields all `MIPVariable` instances that have been created using `M.new_variable()` in the order of their creation.\n\n\nCC:  @yuan-zhou tmonteil @tscrim\n\nBranch: [u/mkoeppe/mixedintegerlinearprogram__new_generator_method__variables_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/mixedintegerlinearprogram__new_generator_method__variables_)\n\nStatus: new\n\nDependencies: #20656\n\nIssue created by migration from https://trac.sagemath.org/ticket/31749\n\n",
    "created_at": "2021-04-28T17:28:30Z",
    "labels": [
        "component: linear programming"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "MixedIntegerLinearProgram: New generator method 'variables'",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/31749",
    "user": "https://github.com/mkoeppe"
}
```
We modify `MixedIntegerLinearProgram.new_variable` so that it remembers the variables.

To avoid reintroducing reference loops eliminated in #12616, we modify `MIPVariable` so that it no longer stores a reference to the MIP but rather to the `backend`, the `base_ring`, and the `linear_functions_parent`.

A new method `M.variables()` yields all `MIPVariable` instances that have been created using `M.new_variable()` in the order of their creation.


CC:  @yuan-zhou tmonteil @tscrim

Branch: [u/mkoeppe/mixedintegerlinearprogram__new_generator_method__variables_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/mixedintegerlinearprogram__new_generator_method__variables_)

Status: new

Dependencies: #20656

Issue created by migration from https://trac.sagemath.org/ticket/31749





---

archive/issue_comments_631845.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,6 @@\n We modify `MixedIntegerLinearProgram.new_variable` so that it remembers the variables.\n+\n+To avoid reintroducing reference loops eliminated in #12616, we modify `MIPVariable` so that it no longer stores a reference to the MIP but rather to the `backend`, the `base_ring`, and the `linear_functions_parent`.\n \n A new method `M.variables()` yields all `MIPVariable` instances that have been created using `M.new_variable()` in the order of their creation.\n \n``````\n",
    "created_at": "2021-04-28T23:03:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31749#issuecomment-631845",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,6 @@
 We modify `MixedIntegerLinearProgram.new_variable` so that it remembers the variables.
+
+To avoid reintroducing reference loops eliminated in #12616, we modify `MIPVariable` so that it no longer stores a reference to the MIP but rather to the `backend`, the `base_ring`, and the `linear_functions_parent`.
 
 A new method `M.variables()` yields all `MIPVariable` instances that have been created using `M.new_variable()` in the order of their creation.
 
``````




---

archive/issue_comments_631846.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#20656\"",
    "created_at": "2021-04-29T03:46:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31749#issuecomment-631846",
    "user": "https://github.com/mkoeppe"
}
```

Changing dependencies from "" to "#20656"



---

archive/issue_comments_631847.json:
```json
{
    "body": "Changing branch from \"\" to \"u/mkoeppe/mixedintegerlinearprogram__new_generator_method__variables_\"",
    "created_at": "2021-04-29T05:25:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31749#issuecomment-631847",
    "user": "https://github.com/mkoeppe"
}
```

Changing branch from "" to "u/mkoeppe/mixedintegerlinearprogram__new_generator_method__variables_"



---

archive/issue_events_094282.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31749#event-94282"
}
```



---

archive/issue_events_094283.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31749#event-94283"
}
```



---

archive/issue_events_094284.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31749#event-94284"
}
```



---

archive/issue_events_094285.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31749#event-94285"
}
```



---

archive/issue_events_094286.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31749#event-94286"
}
```



---

archive/issue_events_094287.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31749#event-94287"
}
```



---

archive/issue_events_094288.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31749",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31749#event-94288"
}
```
