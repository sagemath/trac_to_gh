# Issue 31613: ClusterAlgebra.greedy_element bug and further improvements

archive/issues_031376.json:
```json
{
    "assignees": [],
    "body": "Currently `ClusterAlgebra.greedy_element` gives wrong answers when mixing initial and non-initial cluster variables.\n\nThe code in this ticket started as a fix to these issues and ended up in a retooling of the whole greedy and theta basis framework.\n\nWith the proposed changes the code should be able to compute (in rank 2) theta basis elements, F-polynomials, and decompose any element in terms of theta basis elements.\n\nCC:  @egunawan @sagetrac-gmoose05 @sagetrac-drupel\n\nKeywords: **Cluster Algebras, theta basis**\n\nBranch/Commit: **[2d8e44e](https://github.com/sagemath/sagetrac-mirror/commit/2d8e44ee466b796196a1173285db2661e11943f3)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Salvatore Stella**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31613_\n\n",
    "closed_at": "2021-06-06T13:18:28Z",
    "created_at": "2021-04-05T18:09:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ClusterAlgebra.greedy_element bug and further improvements",
    "type": "issue",
    "updated_at": "2021-06-06T13:18:28Z",
    "url": "https://github.com/sagemath/sage/issues/31613",
    "user": "https://github.com/etn40ff"
}
```
Currently `ClusterAlgebra.greedy_element` gives wrong answers when mixing initial and non-initial cluster variables.

The code in this ticket started as a fix to these issues and ended up in a retooling of the whole greedy and theta basis framework.

With the proposed changes the code should be able to compute (in rank 2) theta basis elements, F-polynomials, and decompose any element in terms of theta basis elements.

CC:  @egunawan @sagetrac-gmoose05 @sagetrac-drupel

Keywords: **Cluster Algebras, theta basis**

Branch/Commit: **[2d8e44e](https://github.com/sagemath/sagetrac-mirror/commit/2d8e44ee466b796196a1173285db2661e11943f3)**

Reviewer: **Frédéric Chapoton**

Author: **Salvatore Stella**

_Issue created by migration from https://trac.sagemath.org/ticket/31613_





---

archive/issue_events_417791.json:
```json
{
    "actor": "https://github.com/etn40ff",
    "created_at": "2021-04-05T18:09:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417791"
}
```



---

archive/issue_events_417792.json:
```json
{
    "actor": "https://github.com/etn40ff",
    "created_at": "2021-04-05T18:09:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417792"
}
```



---

archive/issue_events_417793.json:
```json
{
    "actor": "https://github.com/etn40ff",
    "created_at": "2021-04-05T18:09:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417793"
}
```



---

archive/issue_events_417794.json:
```json
{
    "actor": "https://github.com/etn40ff",
    "created_at": "2021-04-05T18:09:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417794"
}
```



---

archive/issue_events_417795.json:
```json
{
    "actor": "https://github.com/etn40ff",
    "created_at": "2021-04-05T18:09:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417795"
}
```



---

archive/issue_events_417796.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-04-05T18:48:56Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "title_is": "ClusterAlgebra.greedy_element bug and further improvements",
    "title_was": "ClusterAlgeba.greedy_element bug and further improvements",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417796"
}
```



---

archive/issue_comments_511424.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Currently `ClusterAlgeba.greedy_element` gives wrong answers when mixing initial and non-initial cluster variables.\n+Currently `ClusterAlgebra.greedy_element` gives wrong answers when mixing initial and non-initial cluster variables.\n \n The code in this ticket started as a fix to these issues and ended up in a retooling of the whole greedy and theta basis framework.\n \n``````\n",
    "created_at": "2021-04-05T18:48:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511424",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Currently `ClusterAlgeba.greedy_element` gives wrong answers when mixing initial and non-initial cluster variables.
+Currently `ClusterAlgebra.greedy_element` gives wrong answers when mixing initial and non-initial cluster variables.
 
 The code in this ticket started as a fix to these issues and ended up in a retooling of the whole greedy and theta basis framework.
 
``````




---

archive/issue_comments_511425.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\ntypo \"decoposition\"\n\nmissing empty line after EXAMPLES::\n\ntypo \"biliinear\"",
    "created_at": "2021-04-05T18:48:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511425",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

typo "decoposition"

missing empty line after EXAMPLES::

typo "biliinear"



---

archive/issue_comments_511426.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e608be536c58a666bdde1515da85d7032a98105a\">e608be5</a></td><td><code>cleanup</code></td></tr></table>\n",
    "created_at": "2021-04-05T19:04:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511426",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e608be536c58a666bdde1515da85d7032a98105a">e608be5</a></td><td><code>cleanup</code></td></tr></table>




---

archive/issue_comments_511427.json:
```json
{
    "body": "Changed commit from **[59a2633](https://github.com/sagemath/sagetrac-mirror/commit/59a2633341e84152ee1cd9a54ad76b8b6b10f4c7)** to **[e608be5](https://github.com/sagemath/sagetrac-mirror/commit/e608be536c58a666bdde1515da85d7032a98105a)**",
    "created_at": "2021-04-05T19:04:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511427",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[59a2633](https://github.com/sagemath/sagetrac-mirror/commit/59a2633341e84152ee1cd9a54ad76b8b6b10f4c7)** to **[e608be5](https://github.com/sagemath/sagetrac-mirror/commit/e608be536c58a666bdde1515da85d7032a98105a)**



---

archive/issue_comments_511428.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThank you Fr\u00e9d\u00e9ric\n\nS.",
    "created_at": "2021-04-05T19:05:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511428",
    "user": "https://github.com/etn40ff"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thank you Frédéric

S.



---

archive/issue_comments_511429.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nin `g_vector_to_d_vector(self, g)`, there is a superfluous \"return d\"",
    "created_at": "2021-04-06T06:35:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511429",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

in `g_vector_to_d_vector(self, g)`, there is a superfluous "return d"



---

archive/issue_comments_511430.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2d8e44ee466b796196a1173285db2661e11943f3\">2d8e44e</a></td><td><code>Remove leftorvers</code></td></tr></table>\n",
    "created_at": "2021-04-06T06:44:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511430",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2d8e44ee466b796196a1173285db2661e11943f3">2d8e44e</a></td><td><code>Remove leftorvers</code></td></tr></table>




---

archive/issue_comments_511431.json:
```json
{
    "body": "Changed commit from **[e608be5](https://github.com/sagemath/sagetrac-mirror/commit/e608be536c58a666bdde1515da85d7032a98105a)** to **[2d8e44e](https://github.com/sagemath/sagetrac-mirror/commit/2d8e44ee466b796196a1173285db2661e11943f3)**",
    "created_at": "2021-04-06T06:44:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511431",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e608be5](https://github.com/sagemath/sagetrac-mirror/commit/e608be536c58a666bdde1515da85d7032a98105a)** to **[2d8e44e](https://github.com/sagemath/sagetrac-mirror/commit/2d8e44ee466b796196a1173285db2661e11943f3)**



---

archive/issue_comments_511432.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@fchapoton](#comment%3A5):\n> in `g_vector_to_d_vector(self, g)`, there is a superfluous \"return d\"\n\nMy bad: I missed that from an older implementation while cleaning up.",
    "created_at": "2021-04-06T06:46:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511432",
    "user": "https://github.com/etn40ff"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@fchapoton](#comment%3A5):
> in `g_vector_to_d_vector(self, g)`, there is a superfluous "return d"

My bad: I missed that from an older implementation while cleaning up.



---

archive/issue_comments_511433.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nok, let it be. I have no stamina for a serious mathematical review.",
    "created_at": "2021-04-07T18:42:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511433",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">Comment 8</div>

ok, let it be. I have no stamina for a serious mathematical review.



---

archive/issue_comments_511434.json:
```json
{
    "body": "Author: **Salvatore Stella**",
    "created_at": "2021-04-07T18:42:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511434",
    "user": "https://github.com/fchapoton"
}
```

Author: **Salvatore Stella**



---

archive/issue_events_417797.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-04-07T18:42:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417797"
}
```



---

archive/issue_events_417798.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-04-07T18:42:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417798"
}
```



---

archive/issue_comments_511435.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-04-07T18:42:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511435",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_511436.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nTnks",
    "created_at": "2021-04-07T20:14:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511436",
    "user": "https://github.com/etn40ff"
}
```

<div id="comment:9" align="right">Comment 9</div>

Tnks



---

archive/issue_events_417799.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-05-10T11:05:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417799"
}
```



---

archive/issue_events_417800.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-05-10T11:05:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417800"
}
```



---

archive/issue_comments_511437.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nmilestone to 9.4, as 9.3 has been released",
    "created_at": "2021-05-10T11:05:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511437",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:10" align="right">Comment 10</div>

milestone to 9.4, as 9.3 has been released



---

archive/issue_comments_511438.json:
```json
{
    "body": "Changed branch from **[u/etn40ff/fix_greedy](https://github.com/sagemath/sagetrac-mirror/tree/u/etn40ff/fix_greedy)** to **[2d8e44e](https://github.com/sagemath/sagetrac-mirror/commit/2d8e44ee466b796196a1173285db2661e11943f3)**",
    "created_at": "2021-06-06T13:18:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31613#issuecomment-511438",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/etn40ff/fix_greedy](https://github.com/sagemath/sagetrac-mirror/tree/u/etn40ff/fix_greedy)** to **[2d8e44e](https://github.com/sagemath/sagetrac-mirror/commit/2d8e44ee466b796196a1173285db2661e11943f3)**



---

archive/issue_events_417801.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-06-06T13:18:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417801"
}
```



---

archive/issue_events_417802.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "749b0ad122ec06d7670ea741edd0e091d47a970a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-06-06T13:18:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31613",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31613#event-417802"
}
```
