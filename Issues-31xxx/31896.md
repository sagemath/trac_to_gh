# Issue 31896: Fix for field of definition functions

archive/issues_031659.json:
```json
{
    "assignees": [],
    "body": "Currently the following code fails:\n\n```\nA.<x> = AffineSpace(QQ,1)\nf = DynamicalSystem_affine(x^2 + x + 1)\nF = f.homogenize(1)\nF.field_of_definition_periodic(1)\n```\n\nAs the dehomogenization of F has x as its variable, not x0. Similar code fails for field_of_definition_critical and field_of_definition_preimage.\n\nWe implement a fix by using x for arithmetic, not x0.\n\nCC:  @bhutz\n\nKeywords: **gsoc2021**\n\nBranch/Commit: **[1007e4b](https://github.com/sagemath/sagetrac-mirror/commit/1007e4b0275cb3c67cf506adeec1d12095b41cbb)**\n\nReviewer: **Ben Hutz**\n\nAuthor: **Alexander Galarraga**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31896_\n\n",
    "closed_at": "2021-07-25T13:26:56Z",
    "created_at": "2021-06-03T15:48:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20dynamics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix for field of definition functions",
    "type": "issue",
    "updated_at": "2021-07-25T13:26:56Z",
    "url": "https://github.com/sagemath/sage/issues/31896",
    "user": "https://github.com/EnderWannabe"
}
```
Currently the following code fails:

```
A.<x> = AffineSpace(QQ,1)
f = DynamicalSystem_affine(x^2 + x + 1)
F = f.homogenize(1)
F.field_of_definition_periodic(1)
```

As the dehomogenization of F has x as its variable, not x0. Similar code fails for field_of_definition_critical and field_of_definition_preimage.

We implement a fix by using x for arithmetic, not x0.

CC:  @bhutz

Keywords: **gsoc2021**

Branch/Commit: **[1007e4b](https://github.com/sagemath/sagetrac-mirror/commit/1007e4b0275cb3c67cf506adeec1d12095b41cbb)**

Reviewer: **Ben Hutz**

Author: **Alexander Galarraga**

_Issue created by migration from https://trac.sagemath.org/ticket/31896_





---

archive/issue_events_407404.json:
```json
{
    "actor": "https://github.com/EnderWannabe",
    "created_at": "2021-06-03T15:48:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407404"
}
```



---

archive/issue_events_407405.json:
```json
{
    "actor": "https://github.com/EnderWannabe",
    "created_at": "2021-06-03T15:48:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20dynamics",
    "label_color": "0000ff",
    "label_name": "component: dynamics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407405"
}
```



---

archive/issue_events_407406.json:
```json
{
    "actor": "https://github.com/EnderWannabe",
    "created_at": "2021-06-03T15:48:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407406"
}
```



---

archive/issue_events_407407.json:
```json
{
    "actor": "https://github.com/EnderWannabe",
    "created_at": "2021-06-03T15:48:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407407"
}
```



---

archive/issue_comments_515735.json:
```json
{
    "body": "Branch: **[u/gh-EnderWannabe/field_of_definition_fix](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-EnderWannabe/field_of_definition_fix)**",
    "created_at": "2021-06-03T15:51:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31896#issuecomment-515735",
    "user": "https://github.com/EnderWannabe"
}
```

Branch: **[u/gh-EnderWannabe/field_of_definition_fix](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-EnderWannabe/field_of_definition_fix)**



---

archive/issue_comments_515736.json:
```json
{
    "body": "Commit: **[1007e4b](https://github.com/sagemath/sagetrac-mirror/commit/1007e4b0275cb3c67cf506adeec1d12095b41cbb)**",
    "created_at": "2021-06-04T14:17:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31896#issuecomment-515736",
    "user": "https://github.com/EnderWannabe"
}
```

Commit: **[1007e4b](https://github.com/sagemath/sagetrac-mirror/commit/1007e4b0275cb3c67cf506adeec1d12095b41cbb)**



---

archive/issue_comments_515737.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1007e4b0275cb3c67cf506adeec1d12095b41cbb\">1007e4b</a></td><td><code>31896: initial commit with fix and tests</code></td></tr></table>\n",
    "created_at": "2021-06-04T14:17:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31896#issuecomment-515737",
    "user": "https://github.com/EnderWannabe"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1007e4b0275cb3c67cf506adeec1d12095b41cbb">1007e4b</a></td><td><code>31896: initial commit with fix and tests</code></td></tr></table>




---

archive/issue_events_407408.json:
```json
{
    "actor": "https://github.com/EnderWannabe",
    "created_at": "2021-06-04T14:17:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407408"
}
```



---

archive/issue_comments_515738.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks good to me.",
    "created_at": "2021-06-08T18:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31896#issuecomment-515738",
    "user": "https://github.com/bhutz"
}
```

<a id='comment:3'>Comment 3:</a>
Looks good to me.



---

archive/issue_comments_515739.json:
```json
{
    "body": "Reviewer: **Ben Hutz**",
    "created_at": "2021-06-08T18:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31896#issuecomment-515739",
    "user": "https://github.com/bhutz"
}
```

Reviewer: **Ben Hutz**



---

archive/issue_events_407409.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2021-06-08T18:58:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407409"
}
```



---

archive/issue_events_407410.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2021-06-08T18:58:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407410"
}
```



---

archive/issue_comments_515740.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPromoting 5 tickets that fix defects to \"major\" so that they have a chance to get merged",
    "created_at": "2021-07-24T18:43:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31896#issuecomment-515740",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Promoting 5 tickets that fix defects to "major" so that they have a chance to get merged



---

archive/issue_events_407411.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-24T18:43:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407411"
}
```



---

archive/issue_events_407412.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-24T18:43:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407412"
}
```



---

archive/issue_comments_515741.json:
```json
{
    "body": "Changed branch from **[u/gh-EnderWannabe/field_of_definition_fix](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-EnderWannabe/field_of_definition_fix)** to **[1007e4b](https://github.com/sagemath/sagetrac-mirror/commit/1007e4b0275cb3c67cf506adeec1d12095b41cbb)**",
    "created_at": "2021-07-25T13:26:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31896#issuecomment-515741",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-EnderWannabe/field_of_definition_fix](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-EnderWannabe/field_of_definition_fix)** to **[1007e4b](https://github.com/sagemath/sagetrac-mirror/commit/1007e4b0275cb3c67cf506adeec1d12095b41cbb)**



---

archive/issue_events_407413.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-25T13:26:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407413"
}
```



---

archive/issue_events_407414.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d1fc4b6046bf385954625f4f0680e5e19c16c995",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-07-25T13:26:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31896",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31896#event-407414"
}
```
