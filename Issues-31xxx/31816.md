# Issue 31816: Get rid of sage-pip-uninstall

archive/issues_031579.json:
```json
{
    "assignees": [],
    "body": "This is a follow-up from #31423. We get rid of the outdated `sage-pip-uninstall` mechanism completely.\n\n\nCC:  @jhpalmieri @dimpase\n\nBranch/Commit: **[3b590a1](https://github.com/sagemath/sagetrac-mirror/commit/3b590a1b93cf88ece6911351c1a0b574feecb83e)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31816_\n\n",
    "closed_at": "2021-06-19T20:57:26Z",
    "created_at": "2021-05-12T02:27:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Get rid of sage-pip-uninstall",
    "type": "issue",
    "updated_at": "2021-06-19T20:57:26Z",
    "url": "https://github.com/sagemath/sage/issues/31816",
    "user": "https://github.com/mkoeppe"
}
```
This is a follow-up from #31423. We get rid of the outdated `sage-pip-uninstall` mechanism completely.


CC:  @jhpalmieri @dimpase

Branch/Commit: **[3b590a1](https://github.com/sagemath/sagetrac-mirror/commit/3b590a1b93cf88ece6911351c1a0b574feecb83e)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/31816_





---

archive/issue_events_420539.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-12T02:27:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31816#event-420539"
}
```



---

archive/issue_events_420540.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-12T02:27:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31816#event-420540"
}
```



---

archive/issue_events_420541.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-12T02:27:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31816#event-420541"
}
```



---

archive/issue_events_420542.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-12T02:27:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31816#event-420542"
}
```



---

archive/issue_comments_514594.json:
```json
{
    "body": "Branch: **[u/mkoeppe/get_rid_of_sage_pip_uninstall](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/get_rid_of_sage_pip_uninstall)**",
    "created_at": "2021-05-12T02:35:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31816#issuecomment-514594",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/get_rid_of_sage_pip_uninstall](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/get_rid_of_sage_pip_uninstall)**



---

archive/issue_events_420543.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-12T02:36:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31816#event-420543"
}
```



---

archive/issue_comments_514595.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3b590a1b93cf88ece6911351c1a0b574feecb83e\">3b590a1</a></td><td><code>build/bin/sage-dist-helpers (sdh_pip_install): No longer try to uninstall an old version first with sage-pip-uninstall</code></td></tr></table>\n",
    "created_at": "2021-05-12T02:36:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31816#issuecomment-514595",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3b590a1b93cf88ece6911351c1a0b574feecb83e">3b590a1</a></td><td><code>build/bin/sage-dist-helpers (sdh_pip_install): No longer try to uninstall an old version first with sage-pip-uninstall</code></td></tr></table>




---

archive/issue_comments_514596.json:
```json
{
    "body": "Commit: **[3b590a1](https://github.com/sagemath/sagetrac-mirror/commit/3b590a1b93cf88ece6911351c1a0b574feecb83e)**",
    "created_at": "2021-05-12T02:36:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31816#issuecomment-514596",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[3b590a1](https://github.com/sagemath/sagetrac-mirror/commit/3b590a1b93cf88ece6911351c1a0b574feecb83e)**



---

archive/issue_comments_514597.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-05-12T02:36:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31816#issuecomment-514597",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_420544.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-05-12T13:34:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31816#event-420544"
}
```



---

archive/issue_events_420545.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-05-12T13:34:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31816#event-420545"
}
```



---

archive/issue_comments_514598.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2021-05-12T13:34:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31816#issuecomment-514598",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_514599.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nlgtm",
    "created_at": "2021-05-12T13:34:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31816#issuecomment-514599",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

lgtm



---

archive/issue_comments_514600.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks!",
    "created_at": "2021-05-12T14:54:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31816#issuecomment-514600",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks!



---

archive/issue_comments_514601.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/get_rid_of_sage_pip_uninstall](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/get_rid_of_sage_pip_uninstall)** to **[3b590a1](https://github.com/sagemath/sagetrac-mirror/commit/3b590a1b93cf88ece6911351c1a0b574feecb83e)**",
    "created_at": "2021-06-19T20:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31816#issuecomment-514601",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/get_rid_of_sage_pip_uninstall](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/get_rid_of_sage_pip_uninstall)** to **[3b590a1](https://github.com/sagemath/sagetrac-mirror/commit/3b590a1b93cf88ece6911351c1a0b574feecb83e)**



---

archive/issue_events_420546.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-06-19T20:57:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31816#event-420546"
}
```



---

archive/issue_events_420547.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4a7ed9696ced64d4679a00edbf4223d4d5eefd3e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-06-19T20:57:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31816",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31816#event-420547"
}
```
