# Issue 31602: modularized relocatable wheel version of sage_conf

archive/issues_031365.json:
```json
{
    "assignees": [],
    "body": "Redesign of #31396 that does not need to put a huge wheel on PyPI\n\n- huge wheel `sage_root_with_var_tmp_symlink` and wheels for all Python package linking to libraries there are published on https://github.com/sagemath/sage-wheels/releases (no size limit)\n\n- `sage_conf` (sdist and wheel) declares install_requires that @ to the URLs within https://github.com/sagemath/sage-wheels/releases\n\n\n\nDepends on #31396\n\nBranch: **[u/mkoeppe/modularized_relocatable_wheel_sage_conf](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/modularized_relocatable_wheel_sage_conf)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31602_\n\n",
    "created_at": "2021-04-03T16:05:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "modularized relocatable wheel version of sage_conf",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/31602",
    "user": "https://github.com/mkoeppe"
}
```
Redesign of #31396 that does not need to put a huge wheel on PyPI

- huge wheel `sage_root_with_var_tmp_symlink` and wheels for all Python package linking to libraries there are published on https://github.com/sagemath/sage-wheels/releases (no size limit)

- `sage_conf` (sdist and wheel) declares install_requires that @ to the URLs within https://github.com/sagemath/sage-wheels/releases



Depends on #31396

Branch: **[u/mkoeppe/modularized_relocatable_wheel_sage_conf](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/modularized_relocatable_wheel_sage_conf)**

_Issue created by migration from https://trac.sagemath.org/ticket/31602_





---

archive/issue_events_417625.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-03T16:05:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417625"
}
```



---

archive/issue_events_417626.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-03T16:05:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417626"
}
```



---

archive/issue_events_417627.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-03T16:05:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417627"
}
```



---

archive/issue_events_417628.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-03T16:05:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417628"
}
```



---

archive/issue_comments_511365.json:
```json
{
    "body": "Branch: **[u/mkoeppe/modularized_relocatable_wheel_sage_conf](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/modularized_relocatable_wheel_sage_conf)**",
    "created_at": "2021-04-03T16:08:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31602#issuecomment-511365",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/modularized_relocatable_wheel_sage_conf](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/modularized_relocatable_wheel_sage_conf)**



---

archive/issue_comments_511366.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-04-07T19:29:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31602#issuecomment-511366",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_417629.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417629"
}
```



---

archive/issue_events_417630.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417630"
}
```



---

archive/issue_events_417631.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417631"
}
```



---

archive/issue_events_417632.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417632"
}
```



---

archive/issue_events_417633.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417633"
}
```



---

archive/issue_events_417634.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417634"
}
```



---

archive/issue_events_417635.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417635"
}
```



---

archive/issue_events_417636.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417636"
}
```



---

archive/issue_events_417637.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417637"
}
```



---

archive/issue_events_417638.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417638"
}
```



---

archive/issue_events_417639.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417639"
}
```



---

archive/issue_events_417640.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31602",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31602#event-417640"
}
```
