# Issue 31012: Adapt RandomChordalGraph to deprecation of sampling from sets in Python 3.9

archive/issues_030775.json:
```json
{
    "assignees": [],
    "body": "Python 3.9 deprecates the use `sample(population, k)` when `population` is a `set`. \n\n```\nsage: G = graphs.RandomChordalGraph(100)                                                                                            \n/Users/dcoudert/sage/local/lib/python3.9/site-packages/sage/misc/prandom.py:179: DeprecationWarning: Sampling from a set deprecated\nsince Python 3.9 and will be removed in a subsequent version.\n  return _pyrand().sample(population, k)\n```\nIt was used in `graphs.RandomChordalGraph(...)`, so we change that.\n\nCC:  @slel\n\nKeywords: **sample**\n\nBranch/Commit: **[f5bc021](https://github.com/sagemath/sagetrac-mirror/commit/f5bc021d734663fbe6901fbecad6165f3f8211fa)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31012_\n\n",
    "closed_at": "2020-12-06T12:49:11Z",
    "created_at": "2020-12-05T17:39:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Adapt RandomChordalGraph to deprecation of sampling from sets in Python 3.9",
    "type": "issue",
    "updated_at": "2020-12-06T12:49:11Z",
    "url": "https://github.com/sagemath/sage/issues/31012",
    "user": "https://github.com/dcoudert"
}
```
Python 3.9 deprecates the use `sample(population, k)` when `population` is a `set`. 

```
sage: G = graphs.RandomChordalGraph(100)                                                                                            
/Users/dcoudert/sage/local/lib/python3.9/site-packages/sage/misc/prandom.py:179: DeprecationWarning: Sampling from a set deprecated
since Python 3.9 and will be removed in a subsequent version.
  return _pyrand().sample(population, k)
```
It was used in `graphs.RandomChordalGraph(...)`, so we change that.

CC:  @slel

Keywords: **sample**

Branch/Commit: **[f5bc021](https://github.com/sagemath/sagetrac-mirror/commit/f5bc021d734663fbe6901fbecad6165f3f8211fa)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/31012_





---

archive/issue_events_409421.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-12-05T17:39:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409421"
}
```



---

archive/issue_events_409422.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-12-05T17:39:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409422"
}
```



---

archive/issue_events_409423.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-12-05T17:39:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409423"
}
```



---

archive/issue_events_409424.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-12-05T17:39:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409424"
}
```



---

archive/issue_comments_499983.json:
```json
{
    "body": "Commit: **[f5bc021](https://github.com/sagemath/sagetrac-mirror/commit/f5bc021d734663fbe6901fbecad6165f3f8211fa)**",
    "created_at": "2020-12-05T17:40:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31012#issuecomment-499983",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[f5bc021](https://github.com/sagemath/sagetrac-mirror/commit/f5bc021d734663fbe6901fbecad6165f3f8211fa)**



---

archive/issue_comments_499984.json:
```json
{
    "body": "Branch: **[public/graphs/31012_sample](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/31012_sample)**",
    "created_at": "2020-12-05T17:40:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31012#issuecomment-499984",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/graphs/31012_sample](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/31012_sample)**



---

archive/issue_comments_499985.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f5bc021d734663fbe6901fbecad6165f3f8211fa\">f5bc021</a></td><td><code>fix deprecation</code></td></tr></table>\n",
    "created_at": "2020-12-05T17:40:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31012#issuecomment-499985",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f5bc021d734663fbe6901fbecad6165f3f8211fa">f5bc021</a></td><td><code>fix deprecation</code></td></tr></table>




---

archive/issue_events_409425.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-12-05T17:40:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409425"
}
```



---

archive/issue_comments_499986.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok, merci",
    "created_at": "2020-12-05T20:03:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31012#issuecomment-499986",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok, merci



---

archive/issue_comments_499987.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2020-12-05T20:03:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31012#issuecomment-499987",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_409426.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-12-05T20:03:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409426"
}
```



---

archive/issue_events_409427.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-12-05T20:03:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409427"
}
```



---

archive/issue_events_409428.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-12-06T07:39:59Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "title_is": "Adapt RandomChordalGraph to deprecation of sampling from sets in Python 3.9",
    "title_was": "Modify RandomChordalGraph according a deprecation warning introduced in Python 3.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409428"
}
```



---

archive/issue_comments_499988.json:
```json
{
    "body": "Changed keywords from **** to **sample**",
    "created_at": "2020-12-06T07:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31012#issuecomment-499988",
    "user": "https://github.com/slel"
}
```

Changed keywords from **** to **sample**



---

archive/issue_comments_499989.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThank you. The description is much better this way.",
    "created_at": "2020-12-06T09:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31012#issuecomment-499989",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thank you. The description is much better this way.



---

archive/issue_events_409429.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-12-06T12:49:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409429"
}
```



---

archive/issue_events_409430.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "52b91c88ce119f5ec65a895a91fbed6044962f6f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-12-06T12:49:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31012#event-409430"
}
```



---

archive/issue_comments_499990.json:
```json
{
    "body": "Changed branch from **[public/graphs/31012_sample](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/31012_sample)** to **[f5bc021](https://github.com/sagemath/sagetrac-mirror/commit/f5bc021d734663fbe6901fbecad6165f3f8211fa)**",
    "created_at": "2020-12-06T12:49:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31012#issuecomment-499990",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/31012_sample](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/31012_sample)** to **[f5bc021](https://github.com/sagemath/sagetrac-mirror/commit/f5bc021d734663fbe6901fbecad6165f3f8211fa)**
