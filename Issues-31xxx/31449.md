# Issue 31449: Add basic functionalities to linear_rank_metric.py

archive/issues_031212.json:
```json
{
    "assignees": [],
    "body": "Certain important functionalities are missing in `AbstractLinearRankMetricCode`.\nA list with functionalities to be added:\n- Computing the support of a codeword. Both column and row support are desired.\n- Computing the relative distance of the code.\n- Computing the rank distribution of the code.\n- Finding random codeword of given rank.\n- Finding the minimal and maximal rank in the code.\n- Finding the characteristic of the code.\n- Rewriting elements in the extended field in some basis and recovering from it.\n\nCC:  @MaaikevLeuken\n\nKeywords: **rank code**\n\nBranch/Commit: **[u/gh-MaaikevLeuken/add_low_rank_parity_check_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-MaaikevLeuken/add_low_rank_parity_check_codes) @ [a9a274e](https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66)**\n\nAuthor: **Maaike van Leuken**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31449_\n\n",
    "created_at": "2021-03-02T21:08:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add basic functionalities to linear_rank_metric.py",
    "type": "issue",
    "updated_at": "2022-08-31T05:25:02Z",
    "url": "https://github.com/sagemath/sage/issues/31449",
    "user": "https://github.com/MaaikevLeuken"
}
```
Certain important functionalities are missing in `AbstractLinearRankMetricCode`.
A list with functionalities to be added:
- Computing the support of a codeword. Both column and row support are desired.
- Computing the relative distance of the code.
- Computing the rank distribution of the code.
- Finding random codeword of given rank.
- Finding the minimal and maximal rank in the code.
- Finding the characteristic of the code.
- Rewriting elements in the extended field in some basis and recovering from it.

CC:  @MaaikevLeuken

Keywords: **rank code**

Branch/Commit: **[u/gh-MaaikevLeuken/add_low_rank_parity_check_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-MaaikevLeuken/add_low_rank_parity_check_codes) @ [a9a274e](https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66)**

Author: **Maaike van Leuken**

_Issue created by migration from https://trac.sagemath.org/ticket/31449_





---

archive/issue_events_415300.json:
```json
{
    "actor": "https://github.com/MaaikevLeuken",
    "created_at": "2021-03-02T21:08:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415300"
}
```



---

archive/issue_events_415301.json:
```json
{
    "actor": "https://github.com/MaaikevLeuken",
    "created_at": "2021-03-02T21:08:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
    "label_color": "0000ff",
    "label_name": "component: coding theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415301"
}
```



---

archive/issue_events_415302.json:
```json
{
    "actor": "https://github.com/MaaikevLeuken",
    "created_at": "2021-03-02T21:08:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415302"
}
```



---

archive/issue_events_415303.json:
```json
{
    "actor": "https://github.com/MaaikevLeuken",
    "created_at": "2021-03-02T21:08:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415303"
}
```



---

archive/issue_comments_508043.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Certain important functionalities are missing in `LinearRankMetricCode`.\n+Certain important functionalities are missing in `AbstractLinearRankMetricCode`.\n A list with functionalities to be added:\n - Computing the support of a codeword. Both column and row support are desired.\n - Computing the relative distance of the code.\n``````\n",
    "created_at": "2021-03-02T21:15:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31449#issuecomment-508043",
    "user": "https://github.com/MaaikevLeuken"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Certain important functionalities are missing in `LinearRankMetricCode`.
+Certain important functionalities are missing in `AbstractLinearRankMetricCode`.
 A list with functionalities to be added:
 - Computing the support of a codeword. Both column and row support are desired.
 - Computing the relative distance of the code.
``````




---

archive/issue_events_415304.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415304"
}
```



---

archive/issue_events_415305.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415305"
}
```



---

archive/issue_comments_508044.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31449#issuecomment-508044",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_comments_508045.json:
```json
{
    "body": "Commit: **[8c92768](https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33)**",
    "created_at": "2021-05-16T15:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31449#issuecomment-508045",
    "user": "https://github.com/MaaikevLeuken"
}
```

Commit: **[8c92768](https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33)**



---

archive/issue_comments_508046.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33\">8c92768</a></td><td><code>Added Low Rank Parity Check codes and added functionalities for linear_rank_metric</code></td></tr></table>\n",
    "created_at": "2021-05-16T15:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31449#issuecomment-508046",
    "user": "https://github.com/MaaikevLeuken"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33">8c92768</a></td><td><code>Added Low Rank Parity Check codes and added functionalities for linear_rank_metric</code></td></tr></table>




---

archive/issue_comments_508047.json:
```json
{
    "body": "Branch: **[u/gh-MaaikevLeuken/add_low_rank_parity_check_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-MaaikevLeuken/add_low_rank_parity_check_codes)**",
    "created_at": "2021-05-16T15:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31449#issuecomment-508047",
    "user": "https://github.com/MaaikevLeuken"
}
```

Branch: **[u/gh-MaaikevLeuken/add_low_rank_parity_check_codes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-MaaikevLeuken/add_low_rank_parity_check_codes)**



---

archive/issue_comments_508048.json:
```json
{
    "body": "Changed commit from **[8c92768](https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33)** to **[a9a274e](https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66)**",
    "created_at": "2021-05-17T12:08:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31449#issuecomment-508048",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8c92768](https://github.com/sagemath/sagetrac-mirror/commit/8c927685ac8684f940a3aea0591a3dc85e819e33)** to **[a9a274e](https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66)**



---

archive/issue_comments_508049.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66\">a9a274e</a></td><td><code>Improved formatting.</code></td></tr></table>\n",
    "created_at": "2021-05-17T12:08:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31449#issuecomment-508049",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a9a274e70ef9d8eb63e40859e086f0c08f02ad66">a9a274e</a></td><td><code>Improved formatting.</code></td></tr></table>




---

archive/issue_events_415306.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415306"
}
```



---

archive/issue_events_415307.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415307"
}
```



---

archive/issue_events_415308.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415308"
}
```



---

archive/issue_events_415309.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415309"
}
```



---

archive/issue_events_415310.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415310"
}
```



---

archive/issue_events_415311.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415311"
}
```



---

archive/issue_events_415312.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415312"
}
```



---

archive/issue_events_415313.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31449",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31449#event-415313"
}
```
