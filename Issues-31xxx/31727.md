# Issue 31727: ManifoldSubset: Add methods subset_family, superset_family, open_superset_family; deprecate method list_of_subsets

archive/issues_031490.json:
```json
{
    "assignees": [],
    "body": "Follow-up from #31718.\n\nThis ticket proposes to deprecate `list_of_subsets` (note - no other methods in `sage.manifolds` follow this naming scheme) in favor of a new method `subset_family`, which prints in a more compact way, is hashable, and is sorted by name too.\n\nWe also add `superset_family` (previously the set of supersets was not exposed by a method).\n\nWe also add `open_superset_family` (to complement `open_superset` added in #31677).\n\n\nDepends on #31718\nDepends on #31677\n\nCC:  @egourgoulhon @mjungmath @tscrim\n\nBranch/Commit: **[2b47b1b](https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11)**\n\nReviewer: **Eric Gourgoulhon**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31727_\n\n",
    "closed_at": "2021-06-21T21:17:31Z",
    "created_at": "2021-04-25T18:14:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ManifoldSubset: Add methods subset_family, superset_family, open_superset_family; deprecate method list_of_subsets",
    "type": "issue",
    "updated_at": "2021-06-21T21:17:31Z",
    "url": "https://github.com/sagemath/sage/issues/31727",
    "user": "https://github.com/mkoeppe"
}
```
Follow-up from #31718.

This ticket proposes to deprecate `list_of_subsets` (note - no other methods in `sage.manifolds` follow this naming scheme) in favor of a new method `subset_family`, which prints in a more compact way, is hashable, and is sorted by name too.

We also add `superset_family` (previously the set of supersets was not exposed by a method).

We also add `open_superset_family` (to complement `open_superset` added in #31677).


Depends on #31718
Depends on #31677

CC:  @egourgoulhon @mjungmath @tscrim

Branch/Commit: **[2b47b1b](https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11)**

Reviewer: **Eric Gourgoulhon**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/31727_





---

archive/issue_events_419373.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-25T18:14:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419373"
}
```



---

archive/issue_events_419374.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-25T18:14:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
    "label_color": "0000ff",
    "label_name": "component: manifolds",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419374"
}
```



---

archive/issue_events_419375.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-25T18:14:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419375"
}
```



---

archive/issue_events_419376.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-25T18:14:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419376"
}
```



---

archive/issue_comments_513423.json:
```json
{
    "body": "Branch: **[u/mkoeppe/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets)**",
    "created_at": "2021-04-25T19:05:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513423",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets)**



---

archive/issue_comments_513424.json:
```json
{
    "body": "Commit: **[52a93f9](https://github.com/sagemath/sagetrac-mirror/commit/52a93f94de48c0fb7070f95b170cd218d7ddfc15)**",
    "created_at": "2021-04-25T19:06:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513424",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[52a93f9](https://github.com/sagemath/sagetrac-mirror/commit/52a93f94de48c0fb7070f95b170cd218d7ddfc15)**



---

archive/issue_comments_513425.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5d87ceef7ea74cd88701eb3a3eb728d6a994bd43\">5d87cee</a></td><td><code>Manifold{Object,Subset}FiniteFamily: Rename from FiniteManifold{Object,Subset}Family</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2f2ace2e9e9b0580d2b6d93eaa98d14bbc966f77\">2f2ace2</a></td><td><code>src/doc/en/reference/manifolds/manifold.rst: Add sage.manifolds.family</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1aff58a70849386432f336b5bba37e9545eed6eb\">1aff58a</a></td><td><code>Fix up docstring markup</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b922066d150a4bdc6e6ee9d52924a7d49f039946\">b922066</a></td><td><code>ManifoldSubsetFiniteFamily: If all subsets are open, include 'open' in repr</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/30271af51d78e4796f1be8b221088ceb2a71b2fa\">30271af</a></td><td><code>Fixup doctest</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/adac07acd0af399defe7819f0dc9f4cea8e1b27e\">adac07a</a></td><td><code>Merge #31680</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/78cc27a8e439c542c2f03958e8e0888b0551a979\">78cc27a</a></td><td><code>ManifoldSubset.open_covers: Change to generator, add optional arg 'trivial'; update uses</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e026e7a68095577277de1b32df0f8c446b594aa7\">e026e7a</a></td><td><code>ManifoldSubset.subset_digraph: Use open_covers method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3c5555acd67b87e772acb7f22354a444cca16595\">3c5555a</a></td><td><code>Merge #31718</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/52a93f94de48c0fb7070f95b170cd218d7ddfc15\">52a93f9</a></td><td><code>ManifoldSubset.subset_family: New; deprecate .list_of_subsets</code></td></tr></table>\n",
    "created_at": "2021-04-25T19:06:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513425",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5d87ceef7ea74cd88701eb3a3eb728d6a994bd43">5d87cee</a></td><td><code>Manifold{Object,Subset}FiniteFamily: Rename from FiniteManifold{Object,Subset}Family</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2f2ace2e9e9b0580d2b6d93eaa98d14bbc966f77">2f2ace2</a></td><td><code>src/doc/en/reference/manifolds/manifold.rst: Add sage.manifolds.family</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1aff58a70849386432f336b5bba37e9545eed6eb">1aff58a</a></td><td><code>Fix up docstring markup</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b922066d150a4bdc6e6ee9d52924a7d49f039946">b922066</a></td><td><code>ManifoldSubsetFiniteFamily: If all subsets are open, include 'open' in repr</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/30271af51d78e4796f1be8b221088ceb2a71b2fa">30271af</a></td><td><code>Fixup doctest</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/adac07acd0af399defe7819f0dc9f4cea8e1b27e">adac07a</a></td><td><code>Merge #31680</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/78cc27a8e439c542c2f03958e8e0888b0551a979">78cc27a</a></td><td><code>ManifoldSubset.open_covers: Change to generator, add optional arg 'trivial'; update uses</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e026e7a68095577277de1b32df0f8c446b594aa7">e026e7a</a></td><td><code>ManifoldSubset.subset_digraph: Use open_covers method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3c5555acd67b87e772acb7f22354a444cca16595">3c5555a</a></td><td><code>Merge #31718</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/52a93f94de48c0fb7070f95b170cd218d7ddfc15">52a93f9</a></td><td><code>ManifoldSubset.subset_family: New; deprecate .list_of_subsets</code></td></tr></table>




---

archive/issue_comments_513426.json:
```json
{
    "body": "Dependencies: **#31718**",
    "created_at": "2021-04-25T19:06:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513426",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#31718**



---

archive/issue_comments_513427.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f1bc8bc9cd3c49a4beb64a8a4b4ecbf04d7a3b37\">f1bc8bc</a></td><td><code>ManifoldSubset.superset_family: New, use it in doctests</code></td></tr></table>\n",
    "created_at": "2021-04-25T19:16:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513427",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f1bc8bc9cd3c49a4beb64a8a4b4ecbf04d7a3b37">f1bc8bc</a></td><td><code>ManifoldSubset.superset_family: New, use it in doctests</code></td></tr></table>




---

archive/issue_comments_513428.json:
```json
{
    "body": "Changed commit from **[52a93f9](https://github.com/sagemath/sagetrac-mirror/commit/52a93f94de48c0fb7070f95b170cd218d7ddfc15)** to **[f1bc8bc](https://github.com/sagemath/sagetrac-mirror/commit/f1bc8bc9cd3c49a4beb64a8a4b4ecbf04d7a3b37)**",
    "created_at": "2021-04-25T19:16:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513428",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[52a93f9](https://github.com/sagemath/sagetrac-mirror/commit/52a93f94de48c0fb7070f95b170cd218d7ddfc15)** to **[f1bc8bc](https://github.com/sagemath/sagetrac-mirror/commit/f1bc8bc9cd3c49a4beb64a8a4b4ecbf04d7a3b37)**



---

archive/issue_comments_513429.json:
```json
{
    "body": "Changed dependencies from **#31718** to **#31718, #31677**",
    "created_at": "2021-04-25T19:26:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513429",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#31718** to **#31718, #31677**



---

archive/issue_comments_513430.json:
```json
{
    "body": "Changed commit from **[f1bc8bc](https://github.com/sagemath/sagetrac-mirror/commit/f1bc8bc9cd3c49a4beb64a8a4b4ecbf04d7a3b37)** to **[59553f7](https://github.com/sagemath/sagetrac-mirror/commit/59553f7408ec98f32f18ee2cf7fc81703653bacd)**",
    "created_at": "2021-04-25T19:45:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513430",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f1bc8bc](https://github.com/sagemath/sagetrac-mirror/commit/f1bc8bc9cd3c49a4beb64a8a4b4ecbf04d7a3b37)** to **[59553f7](https://github.com/sagemath/sagetrac-mirror/commit/59553f7408ec98f32f18ee2cf7fc81703653bacd)**



---

archive/issue_comments_513431.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. Last 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ef1614aafe5cabdce11443809ca932125105253b\">ef1614a</a></td><td><code>Refactor Manifold.open_subset methods through new method _init_open_subset</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/265b0e26d3db8fea35eec87b5ef9497b8f0ddfab\">265b0e2</a></td><td><code>TopologicalManifold._init_open_subset: Add docstring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a93f7ed196b5a8ac98dadb0afed641c60a9827cf\">a93f7ed</a></td><td><code>TopologicalSubmanifold._init_open_subset: Add docstring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/58b43f7902296384dade8a617441e205956d9824\">58b43f7</a></td><td><code>DifferentiableManifold._init_open_subset: Add docstring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d54241202c54c606dee609ff71821d5ebdb0c930\">d542412</a></td><td><code>ManifoldSubset.open_supersets: New generator, use it to fix loops that assume open supersets</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c821a75beec3166aad45e72ce56dce64cb4e8c8a\">c821a75</a></td><td><code>{Topological,...}Manifold.open_subset: New optional parameter supersets</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b35eca14d542d604421cf82de0c24f9287e6b05f\">b35eca1</a></td><td><code>ManifoldSubset.{open_subset,_init_open_subset}: New</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/afe10a03784ea3c0c1863bad8eadb8c2617c1167\">afe10a0</a></td><td><code>Merge #31677</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cd1fa4da5745b5ed358405d0613aec17a976d30d\">cd1fa4d</a></td><td><code>ManifoldSubset.open_superset_family: New</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/59553f7408ec98f32f18ee2cf7fc81703653bacd\">59553f7</a></td><td><code>ManifoldSubset.open_cover_family: New</code></td></tr></table>\n",
    "created_at": "2021-04-25T19:45:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513431",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ef1614aafe5cabdce11443809ca932125105253b">ef1614a</a></td><td><code>Refactor Manifold.open_subset methods through new method _init_open_subset</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/265b0e26d3db8fea35eec87b5ef9497b8f0ddfab">265b0e2</a></td><td><code>TopologicalManifold._init_open_subset: Add docstring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a93f7ed196b5a8ac98dadb0afed641c60a9827cf">a93f7ed</a></td><td><code>TopologicalSubmanifold._init_open_subset: Add docstring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/58b43f7902296384dade8a617441e205956d9824">58b43f7</a></td><td><code>DifferentiableManifold._init_open_subset: Add docstring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d54241202c54c606dee609ff71821d5ebdb0c930">d542412</a></td><td><code>ManifoldSubset.open_supersets: New generator, use it to fix loops that assume open supersets</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c821a75beec3166aad45e72ce56dce64cb4e8c8a">c821a75</a></td><td><code>{Topological,...}Manifold.open_subset: New optional parameter supersets</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b35eca14d542d604421cf82de0c24f9287e6b05f">b35eca1</a></td><td><code>ManifoldSubset.{open_subset,_init_open_subset}: New</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/afe10a03784ea3c0c1863bad8eadb8c2617c1167">afe10a0</a></td><td><code>Merge #31677</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cd1fa4da5745b5ed358405d0613aec17a976d30d">cd1fa4d</a></td><td><code>ManifoldSubset.open_superset_family: New</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/59553f7408ec98f32f18ee2cf7fc81703653bacd">59553f7</a></td><td><code>ManifoldSubset.open_cover_family: New</code></td></tr></table>




---

archive/issue_events_419377.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-25T19:46:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419377"
}
```



---

archive/issue_events_419378.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-25T19:46:31Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "title_is": "ManifoldSubset: Add methods subset_family, superset_family, open_superset_family, open_cover_family; deprecate method list_of_subsets",
    "title_was": "ManifoldSubset: Add methods subset_family, superset_family, equal_subset_family; deprecate method list_of_subsets",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419378"
}
```



---

archive/issue_comments_513432.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,6 +4,5 @@\n \n We also add `superset_family` (previously the set of supersets was not exposed by a method).\n \n-We also add `equal_subset_family` - consisting of all subsets that are also supersets (see discussion in #30263).\n+We also add `open_superset_family` (to complement `open_superset` added in #31677) and `open_cover_family`.\n \n-\n``````\n",
    "created_at": "2021-04-25T19:46:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513432",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,6 +4,5 @@
 
 We also add `superset_family` (previously the set of supersets was not exposed by a method).
 
-We also add `equal_subset_family` - consisting of all subsets that are also supersets (see discussion in #30263).
+We also add `open_superset_family` (to complement `open_superset` added in #31677) and `open_cover_family`.
 
-
``````




---

archive/issue_comments_513433.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fc23784527c66e6287625a720edc12c85740ccb6\">fc23784</a></td><td><code>ManifoldSubset.open_cover_family: Fix up docstring</code></td></tr></table>\n",
    "created_at": "2021-04-25T20:29:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513433",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fc23784527c66e6287625a720edc12c85740ccb6">fc23784</a></td><td><code>ManifoldSubset.open_cover_family: Fix up docstring</code></td></tr></table>




---

archive/issue_comments_513434.json:
```json
{
    "body": "Changed commit from **[59553f7](https://github.com/sagemath/sagetrac-mirror/commit/59553f7408ec98f32f18ee2cf7fc81703653bacd)** to **[fc23784](https://github.com/sagemath/sagetrac-mirror/commit/fc23784527c66e6287625a720edc12c85740ccb6)**",
    "created_at": "2021-04-25T20:29:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513434",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[59553f7](https://github.com/sagemath/sagetrac-mirror/commit/59553f7408ec98f32f18ee2cf7fc81703653bacd)** to **[fc23784](https://github.com/sagemath/sagetrac-mirror/commit/fc23784527c66e6287625a720edc12c85740ccb6)**



---

archive/issue_events_419379.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:24:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419379"
}
```



---

archive/issue_events_419380.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:24:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419380"
}
```



---

archive/issue_comments_513435.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d5127e531ad364666bd2d27a3609e7f4af273698\">d5127e5</a></td><td><code>ManifoldSubset.open_cover_family: New</code></td></tr></table>\n",
    "created_at": "2021-04-26T18:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513435",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d5127e531ad364666bd2d27a3609e7f4af273698">d5127e5</a></td><td><code>ManifoldSubset.open_cover_family: New</code></td></tr></table>




---

archive/issue_comments_513436.json:
```json
{
    "body": "Changed commit from **[fc23784](https://github.com/sagemath/sagetrac-mirror/commit/fc23784527c66e6287625a720edc12c85740ccb6)** to **[d5127e5](https://github.com/sagemath/sagetrac-mirror/commit/d5127e531ad364666bd2d27a3609e7f4af273698)**",
    "created_at": "2021-04-26T18:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513436",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[fc23784](https://github.com/sagemath/sagetrac-mirror/commit/fc23784527c66e6287625a720edc12c85740ccb6)** to **[d5127e5](https://github.com/sagemath/sagetrac-mirror/commit/d5127e531ad364666bd2d27a3609e7f4af273698)**



---

archive/issue_events_419381.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:28:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419381"
}
```



---

archive/issue_events_419382.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:28:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419382"
}
```



---

archive/issue_comments_513437.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,5 +4,5 @@\n \n We also add `superset_family` (previously the set of supersets was not exposed by a method).\n \n-We also add `open_superset_family` (to complement `open_superset` added in #31677) and `open_cover_family`.\n+We also add `open_superset_family` (to complement `open_superset` added in #31677).\n \n``````\n",
    "created_at": "2021-04-26T18:50:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513437",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,5 +4,5 @@
 
 We also add `superset_family` (previously the set of supersets was not exposed by a method).
 
-We also add `open_superset_family` (to complement `open_superset` added in #31677) and `open_cover_family`.
+We also add `open_superset_family` (to complement `open_superset` added in #31677).
 
``````




---

archive/issue_events_419383.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-26T18:50:40Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "title_is": "ManifoldSubset: Add methods subset_family, superset_family, open_superset_family; deprecate method list_of_subsets",
    "title_was": "ManifoldSubset: Add methods subset_family, superset_family, open_superset_family, open_cover_family; deprecate method list_of_subsets",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419383"
}
```



---

archive/issue_comments_513438.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:",
    "created_at": "2021-04-26T18:51:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513438",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:



---

archive/issue_comments_513439.json:
```json
{
    "body": "Changed commit from **[d5127e5](https://github.com/sagemath/sagetrac-mirror/commit/d5127e531ad364666bd2d27a3609e7f4af273698)** to **[cd1fa4d](https://github.com/sagemath/sagetrac-mirror/commit/cd1fa4da5745b5ed358405d0613aec17a976d30d)**",
    "created_at": "2021-04-26T18:51:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513439",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d5127e5](https://github.com/sagemath/sagetrac-mirror/commit/d5127e531ad364666bd2d27a3609e7f4af273698)** to **[cd1fa4d](https://github.com/sagemath/sagetrac-mirror/commit/cd1fa4da5745b5ed358405d0613aec17a976d30d)**



---

archive/issue_comments_513440.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nRemoved the last commit; the addition of `open_cover_family` will now happen in #31732 instead.",
    "created_at": "2021-04-26T18:52:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513440",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:13" align="right">Comment 13</div>

Removed the last commit; the addition of `open_cover_family` will now happen in #31732 instead.



---

archive/issue_comments_513441.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nReady for review.",
    "created_at": "2021-04-26T23:10:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513441",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:14" align="right">Comment 14</div>

Ready for review.



---

archive/issue_comments_513442.json:
```json
{
    "body": "Changed commit from **[cd1fa4d](https://github.com/sagemath/sagetrac-mirror/commit/cd1fa4da5745b5ed358405d0613aec17a976d30d)** to **[2b47b1b](https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11)**",
    "created_at": "2021-04-26T23:46:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513442",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[cd1fa4d](https://github.com/sagemath/sagetrac-mirror/commit/cd1fa4da5745b5ed358405d0613aec17a976d30d)** to **[2b47b1b](https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11)**



---

archive/issue_comments_513443.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7a22bb0838eccf995ac6dd9de4cdc44d07fee81f\">7a22bb0</a></td><td><code>ManifoldSubset.superset_family: New, use it in doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bdc07ffec4d821ca00e4a230aeb51d11dfef4322\">bdc07ff</a></td><td><code>Merge #31677</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11\">2b47b1b</a></td><td><code>ManifoldSubset.open_superset_family: New</code></td></tr></table>\n",
    "created_at": "2021-04-26T23:46:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513443",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:15" align="right">Comment 15</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7a22bb0838eccf995ac6dd9de4cdc44d07fee81f">7a22bb0</a></td><td><code>ManifoldSubset.superset_family: New, use it in doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bdc07ffec4d821ca00e4a230aeb51d11dfef4322">bdc07ff</a></td><td><code>Merge #31677</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11">2b47b1b</a></td><td><code>ManifoldSubset.open_superset_family: New</code></td></tr></table>




---

archive/issue_comments_513444.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nLGTM. Thanks!",
    "created_at": "2021-05-02T18:46:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513444",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:16" align="right">Comment 16</div>

LGTM. Thanks!



---

archive/issue_events_419384.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2021-05-02T18:46:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419384"
}
```



---

archive/issue_events_419385.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2021-05-02T18:46:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419385"
}
```



---

archive/issue_comments_513445.json:
```json
{
    "body": "Reviewer: **Eric Gourgoulhon**",
    "created_at": "2021-05-02T18:46:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513445",
    "user": "https://github.com/egourgoulhon"
}
```

Reviewer: **Eric Gourgoulhon**



---

archive/issue_comments_513446.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nThanks for the review!",
    "created_at": "2021-05-02T19:06:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513446",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:18" align="right">Comment 18</div>

Thanks for the review!



---

archive/issue_comments_513447.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets)** to **[2b47b1b](https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11)**",
    "created_at": "2021-06-21T21:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31727#issuecomment-513447",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/manifoldsubset__add_methods_subset_family__superset_family__equal_subset_family__deprecate_method_list_of_subsets)** to **[2b47b1b](https://github.com/sagemath/sagetrac-mirror/commit/2b47b1b4573d19fc8c9201a379d255cc67631c11)**



---

archive/issue_events_419386.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-06-21T21:17:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419386"
}
```



---

archive/issue_events_419387.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fad657640d3c8eb99d1d4794cd9a9b2aa45c603f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-06-21T21:17:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31727",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31727#event-419387"
}
```
