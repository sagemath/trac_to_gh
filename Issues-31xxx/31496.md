# Issue 31496: Remove configure message "SAGE_SPKG_CONFIGURE_SAGELIB: command not found"

archive/issues_031259.json:
```json
{
    "assignees": [],
    "body": "Solved by moving the configure check from\n`build/pkgs/sagelib/spkg-configure.m4` to `configure.ac`.\n\n\nCritical for 9.3 as it will prevent 1000s of bug reports by concerned users\n\nCC:  @vbraun @slel @dimpase\n\nBranch/Commit: **[9aca813](https://github.com/sagemath/sagetrac-mirror/commit/9aca813dc8ffca146ea620be2082e2c4e3a746cb)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31496_\n\n",
    "closed_at": "2021-04-11T09:18:18Z",
    "created_at": "2021-03-15T06:54:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove configure message \"SAGE_SPKG_CONFIGURE_SAGELIB: command not found\"",
    "type": "issue",
    "updated_at": "2021-04-11T09:18:18Z",
    "url": "https://github.com/sagemath/sage/issues/31496",
    "user": "https://github.com/mkoeppe"
}
```
Solved by moving the configure check from
`build/pkgs/sagelib/spkg-configure.m4` to `configure.ac`.


Critical for 9.3 as it will prevent 1000s of bug reports by concerned users

CC:  @vbraun @slel @dimpase

Branch/Commit: **[9aca813](https://github.com/sagemath/sagetrac-mirror/commit/9aca813dc8ffca146ea620be2082e2c4e3a746cb)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/31496_





---

archive/issue_events_415950.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T06:54:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415950"
}
```



---

archive/issue_events_415951.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T06:54:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
    "label_color": "000080",
    "label_name": "component: build: configure",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415951"
}
```



---

archive/issue_events_415952.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T06:54:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415952"
}
```



---

archive/issue_events_415953.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T06:54:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415953"
}
```



---

archive/issue_comments_508664.json:
```json
{
    "body": "Branch: **[u/mkoeppe/remove_configure_message__sage_spkg_configure_sagelib__command_not_found_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_configure_message__sage_spkg_configure_sagelib__command_not_found_)**",
    "created_at": "2021-03-20T19:27:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508664",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/remove_configure_message__sage_spkg_configure_sagelib__command_not_found_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_configure_message__sage_spkg_configure_sagelib__command_not_found_)**



---

archive/issue_comments_508665.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-03-20T19:29:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508665",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_508666.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9aca813dc8ffca146ea620be2082e2c4e3a746cb\">9aca813</a></td><td><code>Move AC_ARG_ENABLE for --enable-editable to configure.ac, remove build/pkgs/sagelib/spkg-configure.m4</code></td></tr></table>\n",
    "created_at": "2021-03-20T19:29:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508666",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9aca813dc8ffca146ea620be2082e2c4e3a746cb">9aca813</a></td><td><code>Move AC_ARG_ENABLE for --enable-editable to configure.ac, remove build/pkgs/sagelib/spkg-configure.m4</code></td></tr></table>




---

archive/issue_events_415954.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-20T19:29:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415954"
}
```



---

archive/issue_comments_508667.json:
```json
{
    "body": "Commit: **[9aca813](https://github.com/sagemath/sagetrac-mirror/commit/9aca813dc8ffca146ea620be2082e2c4e3a746cb)**",
    "created_at": "2021-03-20T19:29:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508667",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[9aca813](https://github.com/sagemath/sagetrac-mirror/commit/9aca813dc8ffca146ea620be2082e2c4e3a746cb)**



---

archive/issue_events_415955.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-03-21T00:10:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415955"
}
```



---

archive/issue_events_415956.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-03-21T00:10:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415956"
}
```



---

archive/issue_comments_508668.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2021-03-21T00:10:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508668",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_508669.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nlgtm",
    "created_at": "2021-03-21T00:10:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508669",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

lgtm



---

archive/issue_comments_508670.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,2 @@\n-\n+Solved by moving the configure check from\n+`build/pkgs/sagelib/spkg-configure.m4` to `configure.ac`.\n``````\n",
    "created_at": "2021-03-21T00:13:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508670",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,2 @@
-
+Solved by moving the configure check from
+`build/pkgs/sagelib/spkg-configure.m4` to `configure.ac`.
``````




---

archive/issue_comments_508671.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThanks.",
    "created_at": "2021-03-21T01:09:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508671",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thanks.



---

archive/issue_comments_508672.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nNice. Hope this can get in the next beta.",
    "created_at": "2021-03-23T01:38:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508672",
    "user": "https://github.com/slel"
}
```

<div id="comment:6" align="right">Comment 6</div>

Nice. Hope this can get in the next beta.



---

archive/issue_comments_508673.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,5 @@\n Solved by moving the configure check from\n `build/pkgs/sagelib/spkg-configure.m4` to `configure.ac`.\n+\n+\n+Critical for 9.3 as it will prevent 1000s of bug reports by concerned users\n``````\n",
    "created_at": "2021-03-24T17:22:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508673",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,5 @@
 Solved by moving the configure check from
 `build/pkgs/sagelib/spkg-configure.m4` to `configure.ac`.
+
+
+Critical for 9.3 as it will prevent 1000s of bug reports by concerned users
``````




---

archive/issue_events_415957.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T17:22:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415957"
}
```



---

archive/issue_events_415958.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T17:22:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415958"
}
```



---

archive/issue_comments_508674.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nSetting priority to blocker to bring this ticket to the attention of the release bot.",
    "created_at": "2021-03-29T23:54:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508674",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Setting priority to blocker to bring this ticket to the attention of the release bot.



---

archive/issue_events_415959.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-29T23:54:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415959"
}
```



---

archive/issue_events_415960.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-29T23:54:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415960"
}
```



---

archive/issue_events_415961.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-04-11T09:18:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415961"
}
```



---

archive/issue_events_415962.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "78e8276fa3b31ed0cb3668960d21e116a6f066b6",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-04-11T09:18:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31496#event-415962"
}
```



---

archive/issue_comments_508675.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/remove_configure_message__sage_spkg_configure_sagelib__command_not_found_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_configure_message__sage_spkg_configure_sagelib__command_not_found_)** to **[9aca813](https://github.com/sagemath/sagetrac-mirror/commit/9aca813dc8ffca146ea620be2082e2c4e3a746cb)**",
    "created_at": "2021-04-11T09:18:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31496#issuecomment-508675",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/remove_configure_message__sage_spkg_configure_sagelib__command_not_found_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_configure_message__sage_spkg_configure_sagelib__command_not_found_)** to **[9aca813](https://github.com/sagemath/sagetrac-mirror/commit/9aca813dc8ffca146ea620be2082e2c4e3a746cb)**
