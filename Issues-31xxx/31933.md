...
<a id='comment:6'></a>Just some comments that got to me when scrolling through the current code (well being aware that it is work in progress):
...
<a id='comment:8'></a>Just some comments from my side at this point:
...
<a id='comment:10'></a>I had a look on the current version (4060b6c). If I am not mistaken, then items
...
<a id='comment:11'></a>Please send a short message or reply to the raised items once your branch is ready to be reviewed again. I agree with the points raised by Clemens; in particular when it comes to the comments concerning `_find_minimum_` (31 – 35).
...
```json
{
    "body": "<a id='comment:13'></a>Replying to [dkrenn](#comment%3A12):\n> 40. Maybe, in\n> \n> ```\n> -            return self._create_element_(data, coefficient)\n> +            else:\n> +                return self._create_element_(data, coefficient, **kwds)\n> ```\n> remove the `else` block of the `try` and simply return as before. The `else` block of a `try` is useful if there is a `finally` block because it is executed before this `finally` block. \n> \n> Also, all other code in the asymptotic package does not use it, so I suggest to be consistent. One reason for this choice is, that there is a clear, non-indented ending (the `return` above) of an indented block.\n\n\nI think this is 26 (but with more explanations). :-)",
...
<a id='comment:13'></a>Replying to [dkrenn](#comment%3A12):
> 40. Maybe, in
> 
> ```
> -            return self._create_element_(data, coefficient)
...
<a id='comment:16'></a>Items worked on and considered done by the latest commits: 14, 15, 19, 20, 22, 26, 29, 36, 39, 40.
...
<a id='comment:18'></a>Replying to [gh-thhagelmayer](#comment%3A16):
> Items worked on and considered done by the latest commits: 14, 15, 19, 20, 22, 26, 29, 36, 39, 40.
> 
...
<a id='comment:20'></a>45. The test you have added in 218d166 should assign x and y as the variables of the growth group `B`, like
    {{{ 
    sage: x, y = B('x'), B('y')
    }}}
    as the `NameError` currently thrown has nothing to do with the implementation.
...
<a id='comment:21'></a>23: This does not yet render properly: 
...
```json
{
    "body": "<a id='comment:23'></a>With the latest commit I consider following items to be open:\n\n8. example for `valid_from`\n\n16. `BTerm.can_absorb`: The `NotImplementedError` seems to be unnecessary because the following code should mostly work for arbitrary number of variables (but the keys of `valid_from` must agree, which should be checked somewhere). \n\n21. `BTerm.can_absorb`: I think that BTerms should be able to absorb exact terms. \n\n33. `MonomialGrowthElement._find_minimum_`: Currently, this is symmetric in self and other. That should not be the case: self must be at least other in order to allow for absorption. After 32., this should simply be a check whether self is at least 1. (see 30)",
    "created_at": "2021-07-08T17:47:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31933",
    "type": "issue_comment",
...
<a id='comment:23'></a>With the latest commit I consider following items to be open:
...
<a id='comment:24'></a>-   Concerning 24: Assume we have
    {{{
    #!python
    sage: B = GrowthGroup('x^ZZ * y^ZZ');
    sage: x, y = B('x'), B('y')
...
```json
{
    "body": "<a id='comment:25'></a>I will have a look at code of this ticket once the items above are processed; no comments at the moment to these items.",
...
<a id='comment:25'></a>I will have a look at code of this ticket once the items above are processed; no comments at the moment to these items.
...
<a id='comment:27'></a>I addressed items: `8, 21, 24, 27, 43, 47, 48, 49, 50, 51, 52, 53, 54 (33 ctd.), 55, 56, 57, 58`.
...
<a id='comment:28'></a>Thank you. Some remarks on previous items and a list of 5 items, most of which should be easily fixable.
...
<a id='comment:30'></a>FWIW, I just had a look at the problem with conversion. There are two questions that need to be answered:
...
```json
{
    "body": "<a id='comment:32'></a>I have addressed items `8, 59, 60, 61 and 62.`",
...
<a id='comment:32'></a>I have addressed items `8, 59, 60, 61 and 62.`
...
<a id='comment:34'></a>Thanks you for this code; it looks quite good already; I have a couple of minor comments:
...
<a id='comment:35'></a>Replying to [dkrenn](#comment%3A34):
> Thanks you for this code; it looks quite good already; I have a couple of minor comments:
> 
> ---
> New commits:
...
<a id='comment:36'></a>Replying to [behackl](#comment%3A30):
> FWIW, I just had a look at the problem with conversion. 
...
```json
{
    "body": "<a id='comment:37'></a>I did not check what from <= 63 is still open and what not.",
...
<a id='comment:37'></a>I did not check what from <= 63 is still open and what not.
...
<a id='comment:38'></a>Replying to [dkrenn](#comment%3A35):
...
```json
{
    "body": "<a id='comment:39'></a>Replying to [dkrenn](#comment%3A34):\n\n> 73. In `_create_element_`, is the idea of the test\n> \n> ```\n>     sage: BT(x^3, 4, 10)\n> ```\n>   to make sure, that there is no positional third argument? If yes, this will be impossible anyways after #32215. If not, what is its purpose?\n\n\nYes, that is the idea. Should the test be removed then?",
...
<a id='comment:39'></a>Replying to [dkrenn](#comment%3A34):
...
<a id='comment:40'></a>Replying to [dkrenn](#comment%3A34):
...
```json
{
    "body": "<a id='comment:42'></a>Replying to [gh-thhagelmayer](#comment%3A39):\n> Replying to [dkrenn](#comment%3A34):\n> \n> > 73. In `_create_element_`, is the idea of the test\n> > \n> > ```\n> >     sage: BT(x^3, 4, 10)\n> > ```\n> >   to make sure, that there is no positional third argument? If yes, this will be impossible anyways after #32215. If not, what is its purpose?\n\n> \n> Yes, that is the idea. Should the test be removed then?\n\n\nProbably the best, since it would be removed later anyways.",
...
<a id='comment:42'></a>Replying to [gh-thhagelmayer](#comment%3A39):
> Replying to [dkrenn](#comment%3A34):
> 
> > 73. In `_create_element_`, is the idea of the test
> > 
...
<a id='comment:43'></a>Replying to [gh-thhagelmayer](#comment%3A40):
> Replying to [dkrenn](#comment%3A34):
> 
> > 65. "Examples" at top of docstring of `BTerm`. Is there a particular reason for putting the examples at this unsusual position? If not, I suggest to move them to the EXAMPLES-section; then it can also be checked that they work properly.
...
<a id='comment:45'></a>Replying to [dkrenn](#comment%3A43):
> Replying to [gh-thhagelmayer](#comment%3A40):
> > Replying to [dkrenn](#comment%3A34):
> > 
> > > 65. "Examples" at top of docstring of `BTerm`. Is there a particular reason for putting the examples at this unsusual position? If not, I suggest to move them to the EXAMPLES-section; then it can also be checked that they work properly.
...
```json
{
    "body": "<a id='comment:47'></a>I worked on items `64-73` and merged #32153 into this ticket.\n\nA review would be appreciated.",
...
<a id='comment:47'></a>I worked on items `64-73` and merged #32153 into this ticket.
...
<a id='comment:49'></a>It seems that there are missing the reviewer commit done on `u/dkrenn/initial_support_for_bterms`:
...
<a id='comment:50'></a>Replying to [dkrenn](#comment%3A34):
> 67. Consider putting the experimental-warning in the docstring at the top of the file; maybe in the Various-section. This `FutureWarning` will otherwise be somewhere in the middle due to #32229.
...
<a id='comment:51'></a>Replying to [git](#comment%3A46):
> |                                                                                                                                           |                                                          |
> |-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------|
> |[b28061e](https://git.sagemath.org/sage.git/commit/?id=b28061ea9a06b1d503babbe8c0b2696e409f239d)|`Trac #31933. Addded latex representation to BTerm._repr_`|
...
<a id='comment:52'></a>Replying to [gh-thhagelmayer](#comment%3A45):
> > I see. This seems not quite easy to do, however. If there is code, it should be tested. We can make run the test directly there; this would give something like
> > 
> > ```
> >     We first explain what B-terms are. With
...
```json
{
    "body": "<a id='comment:54'></a>Thank you for your comments. I merged the missing reviewer commits and corrected the small issues.\n\n---\nNew commits:",
...
<a id='comment:54'></a>Thank you for your comments. I merged the missing reviewer commits and corrected the small issues.
...
<a id='comment:55'></a>Thank you for the changes. I had a look on the commits since my last comments;
we are all aware that conversion is postponed to #32229, but I keep
...
<a id='comment:57'></a>Thank you. I still have a few rather trivial comments.
...
```json
{
    "body": "<a id='comment:59'></a>I have worked on items `83, 84, 85, 86, 87, 88`.",
...
<a id='comment:59'></a>I have worked on items `83, 84, 85, 86, 87, 88`.
...
<a id='comment:60'></a>Thank you. I have no further comments at this point. 
...
<a id='comment:61'></a>Replying to [git](#comment%3A56):
> |                                                                                                                                           |                                                                                                                                    |
> |-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------|
> |[c8c8ec4](https://git.sagemath.org/sage.git/commit/?id=c8c8ec486fad105cc65e68b1889ff95575b49c97)|`Trac #31933. Item 76: revert GenericGrowthElement._find_minimum_ Input explanation introduced in 1c1ebd41. Add missing empty line.`|
...
<a id='comment:62'></a>Replying to [git](#comment%3A56):
> |                                                                                                                                           |                                                                                                             |
> |-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------|
> |[b8e7b86](https://git.sagemath.org/sage.git/commit/?id=b8e7b863bbf53fe4698b6d5131c18614c66c517b)|`Trac #31933. Item 82: Split examples block into three parts to have clearer explanations without scrolling.`|
...
```json
{
    "body": "<a id='comment:63'></a>Except for the two minor (almost trivial) comments 89 and 90, this ticket is fine for me.",
...
<a id='comment:63'></a>Except for the two minor (almost trivial) comments 89 and 90, this ticket is fine for me.
...
<a id='comment:64'></a>Just had a look at the code because of #32214 and found two more very small things:
...
<a id='comment:68'></a>I've reviewed the changes from when I last looked at this ticket up to now and have no further comments.
