# Issue 31673: MR53: Unbreak sagelib's setup.py install

archive/issues_031436.json:
```json
{
    "assignees": [],
    "body": "Mauricio Collares ([@collares](https://gitlab.com/sagemath/dev/tracmirror/collares)) opened a merge request at https://gitlab.com/sagemath/sage/-/merge_requests/53:\n\n---\n\n```markdown\nThe pattern of importing setuptools before distutils was already used in other files, but not in sagelib's setup.py. This caused errors of the form \"AttributeError: 'Extension' object has no attribute '_needs_stub'\" on my machine.\n\nI understand this install method is deprecated, but I believe this patch has essentially zero cost and can safely be added to Sage 9.3.\n```\n\n\n\n\n\nCC:  @dimpase\n\nBranch/Commit: **[994f57c](https://github.com/sagemath/sagetrac-mirror/commit/994f57c75fd5202fb3b5008b52f489463fa0b795)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Mauricio Collares**\n\nComponent: **build**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31673_\n\n",
    "closed_at": "2021-04-25T12:14:02Z",
    "created_at": "2021-04-15T15:59:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "MR53: Unbreak sagelib's setup.py install",
    "type": "issue",
    "updated_at": "2021-04-25T12:14:02Z",
    "url": "https://github.com/sagemath/sage/issues/31673",
    "user": "https://github.com/sagetrac-gitlab-bot"
}
```
Mauricio Collares ([@collares](https://gitlab.com/sagemath/dev/tracmirror/collares)) opened a merge request at https://gitlab.com/sagemath/sage/-/merge_requests/53:

---

```markdown
The pattern of importing setuptools before distutils was already used in other files, but not in sagelib's setup.py. This caused errors of the form "AttributeError: 'Extension' object has no attribute '_needs_stub'" on my machine.

I understand this install method is deprecated, but I believe this patch has essentially zero cost and can safely be added to Sage 9.3.
```





CC:  @dimpase

Branch/Commit: **[994f57c](https://github.com/sagemath/sagetrac-mirror/commit/994f57c75fd5202fb3b5008b52f489463fa0b795)**

Reviewer: **Matthias Koeppe**

Author: **Mauricio Collares**

Component: **build**

_Issue created by migration from https://trac.sagemath.org/ticket/31673_





---

archive/issue_events_436964.json:
```json
{
    "actor": "https://github.com/sagetrac-gitlab-bot",
    "created_at": "2021-04-15T15:59:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436964"
}
```



---

archive/issue_events_436965.json:
```json
{
    "actor": "https://github.com/sagetrac-gitlab-bot",
    "created_at": "2021-04-15T15:59:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436965"
}
```



---

archive/issue_events_436966.json:
```json
{
    "actor": "https://github.com/sagetrac-gitlab-bot",
    "created_at": "2021-04-15T15:59:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436966"
}
```



---

archive/issue_comments_509548.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-04-15T16:16:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31673#issuecomment-509548",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_436967.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T16:16:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436967"
}
```



---

archive/issue_events_436968.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T16:16:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436968"
}
```



---

archive/issue_events_436969.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T16:16:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "000080",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436969"
}
```



---

archive/issue_events_436970.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T16:16:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436970"
}
```



---

archive/issue_events_436971.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T16:16:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436971"
}
```



---

archive/issue_events_436972.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T16:16:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436972"
}
```



---

archive/issue_events_436973.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T16:16:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436973"
}
```



---

archive/issue_events_436974.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T16:16:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436974"
}
```



---

archive/issue_comments_509549.json:
```json
{
    "body": "Description changed:\n``````diff\n\n``````\n",
    "created_at": "2021-04-15T16:16:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31673#issuecomment-509549",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff

``````




---

archive/issue_events_436975.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-18T16:13:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436975"
}
```



---

archive/issue_events_436976.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-18T16:13:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436976"
}
```



---

archive/issue_comments_509550.json:
```json
{
    "body": "Changed branch from **[u/galois/mrs/53/setuppy-install](https://github.com/sagemath/sagetrac-mirror/tree/u/galois/mrs/53/setuppy-install)** to **[994f57c](https://github.com/sagemath/sagetrac-mirror/commit/994f57c75fd5202fb3b5008b52f489463fa0b795)**",
    "created_at": "2021-04-25T12:14:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31673#issuecomment-509550",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/galois/mrs/53/setuppy-install](https://github.com/sagemath/sagetrac-mirror/tree/u/galois/mrs/53/setuppy-install)** to **[994f57c](https://github.com/sagemath/sagetrac-mirror/commit/994f57c75fd5202fb3b5008b52f489463fa0b795)**



---

archive/issue_events_436977.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-04-25T12:14:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436977"
}
```



---

archive/issue_events_436978.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a0672c621b50d9bd326ba948fb2800e71d900e03",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-04-25T12:14:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31673",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31673#event-436978"
}
```
