...
```json
{
    "body": "This last commit is not very nice, but it's now way faster.",
...
```json
{
    "body": "I had to remove last commit because method `next_out_neighbor_unsafe` is not implemented in `static_sparse_backend.pyx`, and I don't know yet how to add such method to this backend.",
...
```json
{
    "body": "A swap problem is certainly also an implementation problem. If we can use less memory with better implementation that is certainly an improvement.",
...
```json
{
    "body": "I tried that but it's not compatible with `static_sparse_backend.pyx` (don't have `next_out_neighbor_unsafe`).\nAn option is to implement a specific BFS for `static_sparse_backend.pyx`.",
...
```json
{
    "body": "I only noticed now that what I suggested in comment:10 is what you have tried before on this ticket and then have fallen back.",
...
```json
{
    "body": "Then we should let this for another ticket. Further cleaning / improvements / implications in the backends may lead to other / faster solutions.",
...
```json
{
    "body": "I'm happy with the ticket.",
...
```json
{
    "body": "Inlining makes a difference. More than 10 percent for any of those test instances.\n\nPlease update the timings in the ticket description.",
...
```json
{
    "body": "We now have green bot. If you agree, we can set this ticket to positive review. We have significant improvements !",
