# Issue 31250: Upgrade: boost_cropped 1.78.0

archive/issues_031013.json:
```json
{
    "assignees": [],
    "body": "Boost is currently at 1.78.0 (https://repology.org/project/boost/versions).\n\nOur last upgrade was to boost/boost_cropped 1.66.0\nin #26597.\n\nThe full `boost` package was removed in #31575.\n\nRelated:\n\n- #25225: header-only, independently usable boost_cropped\n\n\nCC:  @kiwifb @dimpase @antonio-rojas\n\nKeywords: **upgrade, boost_cropped**\n\nReviewer: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31250_\n\n",
    "closed_at": "2023-01-01T03:09:33Z",
    "created_at": "2021-01-16T17:20:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Upgrade: boost_cropped 1.78.0",
    "type": "issue",
    "updated_at": "2023-01-01T03:09:33Z",
    "url": "https://github.com/sagemath/sage/issues/31250",
    "user": "https://github.com/mkoeppe"
}
```
Boost is currently at 1.78.0 (https://repology.org/project/boost/versions).

Our last upgrade was to boost/boost_cropped 1.66.0
in #26597.

The full `boost` package was removed in #31575.

Related:

- #25225: header-only, independently usable boost_cropped


CC:  @kiwifb @dimpase @antonio-rojas

Keywords: **upgrade, boost_cropped**

Reviewer: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/31250_





---

archive/issue_events_412433.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-16T17:20:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412433"
}
```



---

archive/issue_events_412434.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-16T17:20:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412434"
}
```



---

archive/issue_events_412435.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-16T17:20:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412435"
}
```



---

archive/issue_events_412436.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-16T17:20:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412436"
}
```



---

archive/issue_events_412437.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412437"
}
```



---

archive/issue_events_412438.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412438"
}
```



---

archive/issue_comments_503928.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503928",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'>Comment 1:</a>
Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_412439.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T02:58:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412439"
}
```



---

archive/issue_events_412440.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T02:58:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412440"
}
```



---

archive/issue_comments_503929.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,6 @@\n-Our version (1.66.0) is broken on several platforms, for example `ubuntu-groovy` (https://github.com/mkoeppe/sage/runs/1712141683) and on macOS.\n+Our version is 1.66.0.\n+Current is version 1.77.0 (https://repology.org/project/boost/versions)\n \n-Current is version 1.75.0, released December 11th, 2020. https://www.boost.org/\n+Last updated in 2018 in #26597. The full `boost` package was removed in #31575.\n \n-Last updated in 2018 in #26597\n \n-Part of Meta-ticket #29900: Fix optional and experimental packages\n``````\n",
    "created_at": "2021-09-23T15:26:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503929",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,6 @@
-Our version (1.66.0) is broken on several platforms, for example `ubuntu-groovy` (https://github.com/mkoeppe/sage/runs/1712141683) and on macOS.
+Our version is 1.66.0.
+Current is version 1.77.0 (https://repology.org/project/boost/versions)
 
-Current is version 1.75.0, released December 11th, 2020. https://www.boost.org/
+Last updated in 2018 in #26597. The full `boost` package was removed in #31575.
 
-Last updated in 2018 in #26597
 
-Part of Meta-ticket #29900: Fix optional and experimental packages
``````




---

archive/issue_events_412441.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-23T15:26:05Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "title_is": "boost_cropped: Update",
    "title_was": "boost, boost_cropped: Update",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412441"
}
```



---

archive/issue_comments_503930.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\ndo we really have to vendor (mini)boost at all?",
    "created_at": "2021-09-23T15:58:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503930",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:4'>Comment 4:</a>
do we really have to vendor (mini)boost at all?



---

archive/issue_comments_503931.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nYes, it is a dependency of a standard package (`brial`)",
    "created_at": "2021-09-23T16:12:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503931",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'>Comment 5:</a>
Yes, it is a dependency of a standard package (`brial`)



---

archive/issue_events_412442.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412442"
}
```



---

archive/issue_events_412443.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412443"
}
```



---

archive/issue_comments_503932.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nHow is `boost_cropped` produced?",
    "created_at": "2022-01-01T22:38:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503932",
    "user": "https://github.com/slel"
}
```

<a id='comment:7'>Comment 7:</a>
How is `boost_cropped` produced?



---

archive/issue_comments_503933.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,8 @@\n-Our version is 1.66.0.\n-Current is version 1.77.0 (https://repology.org/project/boost/versions)\n+Boost is currently at 1.78.0 (https://repology.org/project/boost/versions).\n \n-Last updated in 2018 in #26597. The full `boost` package was removed in #31575.\n+Our last upgrade was to boost/boost_cropped 1.66.0\n+in #26597.\n+\n+The full `boost` package was removed in #31575.\n \n \n``````\n",
    "created_at": "2022-01-01T22:38:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503933",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,8 @@
-Our version is 1.66.0.
-Current is version 1.77.0 (https://repology.org/project/boost/versions)
+Boost is currently at 1.78.0 (https://repology.org/project/boost/versions).
 
-Last updated in 2018 in #26597. The full `boost` package was removed in #31575.
+Our last upgrade was to boost/boost_cropped 1.66.0
+in #26597.
+
+The full `boost` package was removed in #31575.
 
 
``````




---

archive/issue_events_412444.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2022-01-01T22:38:10Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "title_is": "Upgrade: boost_cropped 1.78.0",
    "title_was": "boost_cropped: Update",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412444"
}
```



---

archive/issue_comments_503934.json:
```json
{
    "body": "Changed keywords from **** to **upgrade, boost_cropped**",
    "created_at": "2022-01-01T22:38:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503934",
    "user": "https://github.com/slel"
}
```

Changed keywords from **** to **upgrade, boost_cropped**



---

archive/issue_comments_503935.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,4 +5,7 @@\n \n The full `boost` package was removed in #31575.\n \n+Related:\n \n+- #25225: header-only, independently usable boost_cropped\n+\n``````\n",
    "created_at": "2022-01-01T22:40:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503935",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,4 +5,7 @@
 
 The full `boost` package was removed in #31575.
 
+Related:
 
+- #25225: header-only, independently usable boost_cropped
+
``````




---

archive/issue_events_412445.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-04T23:30:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412445"
}
```



---

archive/issue_events_412446.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-04T23:30:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412446"
}
```



---

archive/issue_events_412447.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412447"
}
```



---

archive/issue_events_412448.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412448"
}
```



---

archive/issue_events_412449.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-11-20T23:40:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412449"
}
```



---

archive/issue_events_412450.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-11-20T23:40:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412450"
}
```



---

archive/issue_events_412451.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-11-20T23:40:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412451"
}
```



---

archive/issue_comments_503936.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\ndup of #34748, which has a branch",
    "created_at": "2022-11-20T23:40:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503936",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:11'>Comment 11:</a>
dup of #34748, which has a branch



---

archive/issue_events_412452.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-11-20T23:40:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412452"
}
```



---

archive/issue_events_412453.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-11-21T10:43:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412453"
}
```



---

archive/issue_events_412454.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-11-21T10:43:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412454"
}
```



---

archive/issue_comments_503937.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nOK, missed that this was already open, sorry.",
    "created_at": "2022-11-21T10:43:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503937",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:12'>Comment 12:</a>
OK, missed that this was already open, sorry.



---

archive/issue_comments_503938.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2022-11-21T10:43:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31250#issuecomment-503938",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_412455.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-01T03:09:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412455"
}
```



---

archive/issue_events_412456.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-01T03:09:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31250",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31250#event-412456"
}
```
