# Issue 31181: Mutability class does not return is_immutable

archive/issues_030944.json:
```json
{
    "assignees": [],
    "body": "At the current stage, the mixin class `sage.structure.mutability.Mutability` does not return anything for `is_immutable`:\n\n\n```python\n    def is_immutable(self):\n        \"\"\"\n        ...\n        \"\"\"\n        self._is_immutable\n```\n\n\nCC:  @tscrim @mkoeppe\n\nBranch/Commit: **[5ecbf3a](https://github.com/sagemath/sagetrac-mirror/commit/5ecbf3a4896e2ccc10f3f16d4100e197de6a6fea)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Michael Jung**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31181_\n\n",
    "closed_at": "2021-01-24T10:37:49Z",
    "created_at": "2021-01-04T20:01:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Mutability class does not return is_immutable",
    "type": "issue",
    "updated_at": "2021-01-24T10:37:49Z",
    "url": "https://github.com/sagemath/sage/issues/31181",
    "user": "https://github.com/mjungmath"
}
```
At the current stage, the mixin class `sage.structure.mutability.Mutability` does not return anything for `is_immutable`:


```python
    def is_immutable(self):
        """
        ...
        """
        self._is_immutable
```


CC:  @tscrim @mkoeppe

Branch/Commit: **[5ecbf3a](https://github.com/sagemath/sagetrac-mirror/commit/5ecbf3a4896e2ccc10f3f16d4100e197de6a6fea)**

Reviewer: **Travis Scrimshaw**

Author: **Michael Jung**

_Issue created by migration from https://trac.sagemath.org/ticket/31181_





---

archive/issue_events_411569.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-01-04T20:01:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31181#event-411569"
}
```



---

archive/issue_events_411570.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-01-04T20:01:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31181#event-411570"
}
```



---

archive/issue_events_411571.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-01-04T20:01:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31181#event-411571"
}
```



---

archive/issue_events_411572.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-01-04T20:03:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31181#event-411572"
}
```



---

archive/issue_comments_502621.json:
```json
{
    "body": "Branch: **[u/gh-mjungmath/mutability_class_does_not_return_is_immutable](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mjungmath/mutability_class_does_not_return_is_immutable)**",
    "created_at": "2021-01-04T20:03:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31181#issuecomment-502621",
    "user": "https://github.com/mjungmath"
}
```

Branch: **[u/gh-mjungmath/mutability_class_does_not_return_is_immutable](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mjungmath/mutability_class_does_not_return_is_immutable)**



---

archive/issue_events_411573.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-01-05T05:57:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31181#event-411573"
}
```



---

archive/issue_events_411574.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-01-05T05:57:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31181#event-411574"
}
```



---

archive/issue_comments_502622.json:
```json
{
    "body": "Commit: **[5ecbf3a](https://github.com/sagemath/sagetrac-mirror/commit/5ecbf3a4896e2ccc10f3f16d4100e197de6a6fea)**",
    "created_at": "2021-01-05T05:57:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31181#issuecomment-502622",
    "user": "https://github.com/tscrim"
}
```

Commit: **[5ecbf3a](https://github.com/sagemath/sagetrac-mirror/commit/5ecbf3a4896e2ccc10f3f16d4100e197de6a6fea)**



---

archive/issue_comments_502623.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks like a simple oversight, but the doctest is not actually testing this function. Well, this will work though.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5ecbf3a4896e2ccc10f3f16d4100e197de6a6fea\">5ecbf3a</a></td><td><code>Trac #31181: return added</code></td></tr></table>\n",
    "created_at": "2021-01-05T05:57:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31181#issuecomment-502623",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
Looks like a simple oversight, but the doctest is not actually testing this function. Well, this will work though.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5ecbf3a4896e2ccc10f3f16d4100e197de6a6fea">5ecbf3a</a></td><td><code>Trac #31181: return added</code></td></tr></table>




---

archive/issue_comments_502624.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-01-05T05:57:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31181#issuecomment-502624",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_502625.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks for the quick review. Some doctests will be added with #30261 anyway. But first, we have to handle the pickling problem in #31182.",
    "created_at": "2021-01-06T10:37:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31181#issuecomment-502625",
    "user": "https://github.com/mjungmath"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks for the quick review. Some doctests will be added with #30261 anyway. But first, we have to handle the pickling problem in #31182.



---

archive/issue_comments_502626.json:
```json
{
    "body": "Changed branch from **[u/gh-mjungmath/mutability_class_does_not_return_is_immutable](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mjungmath/mutability_class_does_not_return_is_immutable)** to **[5ecbf3a](https://github.com/sagemath/sagetrac-mirror/commit/5ecbf3a4896e2ccc10f3f16d4100e197de6a6fea)**",
    "created_at": "2021-01-24T10:37:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31181#issuecomment-502626",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mjungmath/mutability_class_does_not_return_is_immutable](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mjungmath/mutability_class_does_not_return_is_immutable)** to **[5ecbf3a](https://github.com/sagemath/sagetrac-mirror/commit/5ecbf3a4896e2ccc10f3f16d4100e197de6a6fea)**



---

archive/issue_events_411575.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-01-24T10:37:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31181#event-411575"
}
```



---

archive/issue_events_411576.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "09b4dd28544ffb7f5994be142dc81ce2d8e8a23e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-01-24T10:37:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31181",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31181#event-411576"
}
```
