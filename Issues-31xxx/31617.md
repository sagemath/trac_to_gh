# Issue 31617: Follow-up: provide 3-sphere with additional features

archive/issues_031380.json:
```json
{
    "assignees": [],
    "body": "This is a follow-up of #30804. This ticket provides:\n\n- a Lie group structure of S^3 (?)\n- a global frame of S^3\n- Hopf coordinates\n\nCC:  @egourgoulhon @tscrim @tobiasdiez\n\nComponent: **manifolds**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31617_\n\n",
    "created_at": "2021-04-07T07:43:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20manifolds",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Follow-up: provide 3-sphere with additional features",
    "type": "issue",
    "updated_at": "2022-08-31T05:25:02Z",
    "url": "https://github.com/sagemath/sage/issues/31617",
    "user": "https://github.com/mjungmath"
}
```
This is a follow-up of #30804. This ticket provides:

- a Lie group structure of S^3 (?)
- a global frame of S^3
- Hopf coordinates

CC:  @egourgoulhon @tscrim @tobiasdiez

Component: **manifolds**

_Issue created by migration from https://trac.sagemath.org/ticket/31617_





---

archive/issue_events_436113.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-04-07T07:43:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436113"
}
```



---

archive/issue_events_436114.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-04-07T07:43:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20manifolds",
    "label_color": "0000ff",
    "label_name": "c: manifolds",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436114"
}
```



---

archive/issue_events_436115.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-04-07T07:43:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436115"
}
```



---

archive/issue_events_436116.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-04-07T07:43:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436116"
}
```



---

archive/issue_comments_508545.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n This is a follow-up of #30804. This ticket provides:\n \n+- a Lie group structure of S^3 (?)\n - a global frame of S^3\n - Hopf coordinates\n-- Lie group structure (?)\n``````\n",
    "created_at": "2021-04-07T07:49:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31617#issuecomment-508545",
    "user": "https://github.com/mjungmath"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
 This is a follow-up of #30804. This ticket provides:
 
+- a Lie group structure of S^3 (?)
 - a global frame of S^3
 - Hopf coordinates
-- Lie group structure (?)
``````




---

archive/issue_events_436117.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-04-07T07:49:16Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "title_is": "Follow-up: provide 3-sphere with additional features",
    "title_was": "Follow-up: S^3 as special example of spheres",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436117"
}
```



---

archive/issue_events_436118.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436118"
}
```



---

archive/issue_events_436119.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436119"
}
```



---

archive/issue_comments_508546.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-04-07T19:29:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31617#issuecomment-508546",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">comment:2</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_comments_508547.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nJust a reminder of some notebooks which might serve as starting points for this project:\n- Hopf coordinates: https://nbviewer.jupyter.org/github/sagemanifolds/SageManifolds/blob/master/Notebooks/SM_sphere_S3_Hopf.ipynb\n- Lie group structure: https://nbviewer.jupyter.org/github/sagemanifolds/SageManifolds/blob/master/Notebooks/SM_sphere_S3_vectors.ipynb\n- Riemannian structure: https://nbviewer.jupyter.org/github/sagemanifolds/SageManifolds/blob/master/Notebooks/SM_sphere_S3_metric.ipynb",
    "created_at": "2021-04-08T08:56:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31617#issuecomment-508547",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:3" align="right">comment:3</div>

Just a reminder of some notebooks which might serve as starting points for this project:
- Hopf coordinates: https://nbviewer.jupyter.org/github/sagemanifolds/SageManifolds/blob/master/Notebooks/SM_sphere_S3_Hopf.ipynb
- Lie group structure: https://nbviewer.jupyter.org/github/sagemanifolds/SageManifolds/blob/master/Notebooks/SM_sphere_S3_vectors.ipynb
- Riemannian structure: https://nbviewer.jupyter.org/github/sagemanifolds/SageManifolds/blob/master/Notebooks/SM_sphere_S3_metric.ipynb



---

archive/issue_comments_508548.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAlso, regarding Lie groups in Sage, at the moment, we have only nilpotent Lie groups (cf. https://doc.sagemath.org/html/en/reference/groups/sage/groups/lie_gps/nilpotent_lie_group.html), which have been implemented through #26344.",
    "created_at": "2021-04-08T09:02:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31617#issuecomment-508548",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:4" align="right">comment:4</div>

Also, regarding Lie groups in Sage, at the moment, we have only nilpotent Lie groups (cf. https://doc.sagemath.org/html/en/reference/groups/sage/groups/lie_gps/nilpotent_lie_group.html), which have been implemented through #26344.



---

archive/issue_events_436120.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-10T17:30:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436120"
}
```



---

archive/issue_events_436121.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-10T17:30:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436121"
}
```



---

archive/issue_events_436122.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436122"
}
```



---

archive/issue_events_436123.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436123"
}
```



---

archive/issue_events_436124.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436124"
}
```



---

archive/issue_events_436125.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436125"
}
```



---

archive/issue_events_436126.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436126"
}
```



---

archive/issue_events_436127.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31617",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31617#event-436127"
}
```
