...
<a id='comment:23'></a>Replying to [egourgoulhon](#comment%3A20):
> {{{
> sage: var('x y')
> (x, y)
> sage: s = x>0 & y<0
...
<a id='comment:28'></a>Replying to [charpent](#comment%3A25):
> * A `symbolic_not` is necessary. 
...
<a id='comment:38'></a>Replying to [charpent](#comment%3A36):
> Replying to [mkoeppe](#comment%3A34):
> > For extending `sage.misc.parser`, I would follow `sage.logic.logicparser` (even though I am not sure of its relevance), which uses `~` for logical 'not'. [...]
...
<a id='comment:39'></a>Replying to [charpent](#comment%3A36):
> `Not` can be unambiguous, and nine characters lighter than "symbolic_not" ; 
...
```json
{
    "body": "<a id='comment:44'></a>Yes, see[comment:10](#comment%3A10) -- I have not figured out maxima conversion -- hoping for help from the experts",
...
<a id='comment:44'></a>Yes, see[comment:10](#comment%3A10) -- I have not figured out maxima conversion -- hoping for help from the experts
...
<a id='comment:51'></a>Replying to [mkoeppe](#comment%3A45):
> {{{
> sage: maxima("(a>0) and (b<0)")                                                 
> a>0andb<0
> }}}
...
<a id='comment:67'></a>Replying to [mkoeppe](#comment%3A10):
> For converting to maxima, some help would be welcome.
...
<a id='comment:71'></a>Replying to [charpent](#comment%3A69):
> BTW :
> 
> ```
> sage: ~((a>0) & ((b<0) | (x!=0)))
