# Issue 31379: change CRLF to unix line feed in 3 files

archive/issues_031142.json:
```json
{
    "assignees": [],
    "body": "so that\n\n```\ngit grep -I $'\\r' src/\n```\nno longer return any result\n\nCC:  @JohnCremona @dimpase\n\nBranch/Commit: **[`831f792`](https://github.com/sagemath/sagetrac-mirror/commit/831f79254a6dee039f508d3a863da286c28ea883)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nComponent: **refactoring**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31379_\n\n",
    "closed_at": "2021-03-14T15:03:14Z",
    "created_at": "2021-02-11T09:55:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "change CRLF to unix line feed in 3 files",
    "type": "issue",
    "updated_at": "2021-03-14T15:03:14Z",
    "url": "https://github.com/sagemath/sage/issues/31379",
    "user": "https://github.com/fchapoton"
}
```
so that

```
git grep -I $'\r' src/
```
no longer return any result

CC:  @JohnCremona @dimpase

Branch/Commit: **[`831f792`](https://github.com/sagemath/sagetrac-mirror/commit/831f79254a6dee039f508d3a863da286c28ea883)**

Reviewer: **Matthias Koeppe**

Author: **Frédéric Chapoton**

Component: **refactoring**

_Issue created by migration from https://trac.sagemath.org/ticket/31379_





---

archive/issue_events_432508.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-11T09:55:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31379#event-432508"
}
```



---

archive/issue_events_432509.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-11T09:55:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31379#event-432509"
}
```



---

archive/issue_events_432510.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-11T09:55:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31379#event-432510"
}
```



---

archive/issue_events_432511.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-11T09:55:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31379#event-432511"
}
```



---

archive/issue_events_432512.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-02-11T09:56:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31379#event-432512"
}
```



---

archive/issue_comments_503543.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis was made by opening the first 2 files with emacs and clicking (at the bottom left of emacs windows) on the line-feed encoding symbol until it becomes unix.\n\nThe `__init__` was just emptied as it should be",
    "created_at": "2021-02-11T09:58:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31379#issuecomment-503543",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">comment:2</div>

This was made by opening the first 2 files with emacs and clicking (at the bottom left of emacs windows) on the line-feed encoding symbol until it becomes unix.

The `__init__` was just emptied as it should be



---

archive/issue_comments_503544.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-02-11T17:06:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31379#issuecomment-503544",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_432513.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-11T17:06:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31379#event-432513"
}
```



---

archive/issue_events_432514.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-11T17:06:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31379#event-432514"
}
```



---

archive/issue_events_432515.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-03-14T15:03:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31379#event-432515"
}
```



---

archive/issue_events_432516.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-03-14T15:03:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31379#event-432516"
}
```



---

archive/issue_comments_503545.json:
```json
{
    "body": "Changed branch from **[u/chapoton/no_more_CRLF](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/no_more_CRLF)** to **[`831f792`](https://github.com/sagemath/sagetrac-mirror/commit/831f79254a6dee039f508d3a863da286c28ea883)**",
    "created_at": "2021-03-14T15:03:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31379#issuecomment-503545",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/no_more_CRLF](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/no_more_CRLF)** to **[`831f792`](https://github.com/sagemath/sagetrac-mirror/commit/831f79254a6dee039f508d3a863da286c28ea883)**
