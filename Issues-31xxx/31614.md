# Issue 31614: Remove dead _cholesky_decomposition_ from matrix2.pyx

archive/issues_031377.json:
```json
{
    "assignees": [],
    "body": "There's an old unused cholesky implementation in `_cholesky_decomposition_()`. It can be deleted now.\n\n\nBranch/Commit: **[51489c4](https://github.com/sagemath/sagetrac-mirror/commit/51489c4857c114097f22cb14e54c47d599be7a93)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Michael Orlitzky**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31614_\n\n",
    "closed_at": "2021-06-06T13:18:26Z",
    "created_at": "2021-04-06T00:14:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove dead _cholesky_decomposition_ from matrix2.pyx",
    "type": "issue",
    "updated_at": "2021-06-06T13:18:26Z",
    "url": "https://github.com/sagemath/sage/issues/31614",
    "user": "https://github.com/orlitzky"
}
```
There's an old unused cholesky implementation in `_cholesky_decomposition_()`. It can be deleted now.


Branch/Commit: **[51489c4](https://github.com/sagemath/sagetrac-mirror/commit/51489c4857c114097f22cb14e54c47d599be7a93)**

Reviewer: **Vincent Delecroix**

Author: **Michael Orlitzky**

_Issue created by migration from https://trac.sagemath.org/ticket/31614_





---

archive/issue_events_417803.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-04-06T00:14:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417803"
}
```



---

archive/issue_events_417804.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-04-06T00:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417804"
}
```



---

archive/issue_events_417805.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-04-06T00:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417805"
}
```



---

archive/issue_events_417806.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-04-06T00:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417806"
}
```



---

archive/issue_events_417807.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-04-06T00:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417807"
}
```



---

archive/issue_events_417808.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417808"
}
```



---

archive/issue_events_417809.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417809"
}
```



---

archive/issue_comments_511439.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-04-07T19:29:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31614#issuecomment-511439",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_comments_511440.json:
```json
{
    "body": "Author: **Michael Orlitzky**",
    "created_at": "2021-04-08T13:49:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31614#issuecomment-511440",
    "user": "https://github.com/orlitzky"
}
```

Author: **Michael Orlitzky**



---

archive/issue_events_417810.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-04-08T13:49:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417810"
}
```



---

archive/issue_comments_511441.json:
```json
{
    "body": "Commit: **[51489c4](https://github.com/sagemath/sagetrac-mirror/commit/51489c4857c114097f22cb14e54c47d599be7a93)**",
    "created_at": "2021-04-08T13:49:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31614#issuecomment-511441",
    "user": "https://github.com/orlitzky"
}
```

Commit: **[51489c4](https://github.com/sagemath/sagetrac-mirror/commit/51489c4857c114097f22cb14e54c47d599be7a93)**



---

archive/issue_comments_511442.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/51489c4857c114097f22cb14e54c47d599be7a93\">51489c4</a></td><td><code>Trac #31614: drop old, unused _cholesky_decomposition_() method.</code></td></tr></table>\n",
    "created_at": "2021-04-08T13:49:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31614#issuecomment-511442",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/51489c4857c114097f22cb14e54c47d599be7a93">51489c4</a></td><td><code>Trac #31614: drop old, unused _cholesky_decomposition_() method.</code></td></tr></table>




---

archive/issue_comments_511443.json:
```json
{
    "body": "Branch: **[u/mjo/ticket/31614](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/31614)**",
    "created_at": "2021-04-08T13:49:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31614#issuecomment-511443",
    "user": "https://github.com/orlitzky"
}
```

Branch: **[u/mjo/ticket/31614](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/31614)**



---

archive/issue_events_417811.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2021-04-08T15:15:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417811"
}
```



---

archive/issue_events_417812.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2021-04-08T15:15:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417812"
}
```



---

archive/issue_comments_511444.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2021-04-08T15:15:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31614#issuecomment-511444",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_comments_511445.json:
```json
{
    "body": "Changed branch from **[u/mjo/ticket/31614](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/31614)** to **[51489c4](https://github.com/sagemath/sagetrac-mirror/commit/51489c4857c114097f22cb14e54c47d599be7a93)**",
    "created_at": "2021-06-06T13:18:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31614#issuecomment-511445",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mjo/ticket/31614](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/31614)** to **[51489c4](https://github.com/sagemath/sagetrac-mirror/commit/51489c4857c114097f22cb14e54c47d599be7a93)**



---

archive/issue_events_417813.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-06-06T13:18:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417813"
}
```



---

archive/issue_events_417814.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c95fe8492a01ee736a5d533b1db9d5816994a9db",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-06-06T13:18:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31614",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31614#event-417814"
}
```
