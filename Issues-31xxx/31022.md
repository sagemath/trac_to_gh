# Issue 31022: symmetrica: Add xz as dependency

archive/issues_030785.json:
```json
{
    "assignees": [],
    "body": "... just because its tarball is .xz\n\nhttps://github.com/sagemath/sage/runs/1507347585\n\n```\nsage-logger -p 'SAGE_CHECK=warn sage-spkg -y -o   symmetrica-3.0.1' '/Users/runner/work/sage/sage/logs/pkgs/symmetrica-3.0.1.log'\n[symmetrica-3.0.1] installing. Log file: /Users/runner/work/sage/sage/logs/pkgs/symmetrica-3.0.1.log\n  [symmetrica-3.0.1] error installing, exit status 1. End of log file:\n  [symmetrica-3.0.1]   Found local metadata for symmetrica-3.0.1\n  [symmetrica-3.0.1]   Using cached file /Users/runner/work/sage/sage/upstream/symmetrica-3.0.1.tar.xz\n  [symmetrica-3.0.1]   symmetrica-3.0.1\n  [symmetrica-3.0.1]   ====================================================\n  [symmetrica-3.0.1]   Setting up build directory for symmetrica-3.0.1\n  [symmetrica-3.0.1]   Error: Unknown file type: /Users/runner/work/sage/sage/upstream/symmetrica-3.0.1.tar.xz\n```\n\n\nCC:  @jhpalmieri @dimpase @orlitzky\n\nBranch/Commit: **[98fba8a](https://github.com/sagemath/sagetrac-mirror/commit/98fba8a1b01a5c8d6042df119e6669667c70df09)**\n\nReviewer: **Michael Orlitzky**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31022_\n\n",
    "closed_at": "2020-12-13T20:31:15Z",
    "created_at": "2020-12-07T05:57:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "symmetrica: Add xz as dependency",
    "type": "issue",
    "updated_at": "2020-12-13T20:31:15Z",
    "url": "https://github.com/sagemath/sage/issues/31022",
    "user": "https://github.com/mkoeppe"
}
```
... just because its tarball is .xz

https://github.com/sagemath/sage/runs/1507347585

```
sage-logger -p 'SAGE_CHECK=warn sage-spkg -y -o   symmetrica-3.0.1' '/Users/runner/work/sage/sage/logs/pkgs/symmetrica-3.0.1.log'
[symmetrica-3.0.1] installing. Log file: /Users/runner/work/sage/sage/logs/pkgs/symmetrica-3.0.1.log
  [symmetrica-3.0.1] error installing, exit status 1. End of log file:
  [symmetrica-3.0.1]   Found local metadata for symmetrica-3.0.1
  [symmetrica-3.0.1]   Using cached file /Users/runner/work/sage/sage/upstream/symmetrica-3.0.1.tar.xz
  [symmetrica-3.0.1]   symmetrica-3.0.1
  [symmetrica-3.0.1]   ====================================================
  [symmetrica-3.0.1]   Setting up build directory for symmetrica-3.0.1
  [symmetrica-3.0.1]   Error: Unknown file type: /Users/runner/work/sage/sage/upstream/symmetrica-3.0.1.tar.xz
```


CC:  @jhpalmieri @dimpase @orlitzky

Branch/Commit: **[98fba8a](https://github.com/sagemath/sagetrac-mirror/commit/98fba8a1b01a5c8d6042df119e6669667c70df09)**

Reviewer: **Michael Orlitzky**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/31022_





---

archive/issue_events_409588.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-07T05:57:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409588"
}
```



---

archive/issue_events_409589.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-07T05:57:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409589"
}
```



---

archive/issue_events_409590.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-07T05:57:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409590"
}
```



---

archive/issue_events_409591.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-07T05:57:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409591"
}
```



---

archive/issue_comments_500183.json:
```json
{
    "body": "Branch: **[u/mkoeppe/symmetrica__add_xz_as_dependency](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/symmetrica__add_xz_as_dependency)**",
    "created_at": "2020-12-07T06:00:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31022#issuecomment-500183",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/symmetrica__add_xz_as_dependency](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/symmetrica__add_xz_as_dependency)**



---

archive/issue_comments_500184.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-12-07T06:00:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31022#issuecomment-500184",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_500185.json:
```json
{
    "body": "Commit: **[98fba8a](https://github.com/sagemath/sagetrac-mirror/commit/98fba8a1b01a5c8d6042df119e6669667c70df09)**",
    "created_at": "2020-12-07T06:00:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31022#issuecomment-500185",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[98fba8a](https://github.com/sagemath/sagetrac-mirror/commit/98fba8a1b01a5c8d6042df119e6669667c70df09)**



---

archive/issue_events_409592.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-07T06:00:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409592"
}
```



---

archive/issue_events_409593.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-07T06:00:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409593"
}
```



---

archive/issue_events_409594.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-12-07T06:00:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409594"
}
```



---

archive/issue_comments_500186.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/98fba8a1b01a5c8d6042df119e6669667c70df09\">98fba8a</a></td><td><code>build/pkgs/symmetrica/dependencies: Add xz</code></td></tr></table>\n",
    "created_at": "2020-12-07T06:00:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31022#issuecomment-500186",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/98fba8a1b01a5c8d6042df119e6669667c70df09">98fba8a</a></td><td><code>build/pkgs/symmetrica/dependencies: Add xz</code></td></tr></table>




---

archive/issue_events_409595.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-12-07T11:32:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409595"
}
```



---

archive/issue_events_409596.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-12-07T11:32:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409596"
}
```



---

archive/issue_comments_500187.json:
```json
{
    "body": "Reviewer: **Michael Orlitzky**",
    "created_at": "2020-12-07T11:32:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31022#issuecomment-500187",
    "user": "https://github.com/orlitzky"
}
```

Reviewer: **Michael Orlitzky**



---

archive/issue_comments_500188.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks!",
    "created_at": "2020-12-07T19:07:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31022#issuecomment-500188",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks!



---

archive/issue_events_409597.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-12-13T20:31:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409597"
}
```



---

archive/issue_events_409598.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "77f7f815b809b1058755cf11570d324e028c8d3f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-12-13T20:31:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31022#event-409598"
}
```



---

archive/issue_comments_500189.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/symmetrica__add_xz_as_dependency](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/symmetrica__add_xz_as_dependency)** to **[98fba8a](https://github.com/sagemath/sagetrac-mirror/commit/98fba8a1b01a5c8d6042df119e6669667c70df09)**",
    "created_at": "2020-12-13T20:31:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31022",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31022#issuecomment-500189",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/symmetrica__add_xz_as_dependency](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/symmetrica__add_xz_as_dependency)** to **[98fba8a](https://github.com/sagemath/sagetrac-mirror/commit/98fba8a1b01a5c8d6042df119e6669667c70df09)**
