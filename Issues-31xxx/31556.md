# Issue 31556: Update libgd to 2.3.2

archive/issues_031319.json:
```json
{
    "assignees": [],
    "body": "Our `libgd` is outdated. https://repology.org/project/gd/versions\n\nWe make very limited use of `libgd` -- only in `{matrix,vector}_mod2_dense` for pickling matrices. Nevertheless it should be updated. (Why we configure it to use freetype is unclear.)\n\nPrevious tickets: #27825, #27970, #19721, #18293, #17334\n\n\nCC:  @dimpase @sagetrac-jpflori @kiwifb @antonio-rojas @slel\n\nKeywords: **upgrade, libgd**\n\nBranch/Commit: **[c20ea90](https://github.com/sagemath/sagetrac-mirror/commit/c20ea90c22774fbe22caf5ec66056b311bac27f5)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Samuel Leli\u00e8vre**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31556_\n\n",
    "closed_at": "2021-05-27T20:30:47Z",
    "created_at": "2021-03-25T02:58:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update libgd to 2.3.2",
    "type": "issue",
    "updated_at": "2021-05-27T20:30:47Z",
    "url": "https://github.com/sagemath/sage/issues/31556",
    "user": "https://github.com/mkoeppe"
}
```
Our `libgd` is outdated. https://repology.org/project/gd/versions

We make very limited use of `libgd` -- only in `{matrix,vector}_mod2_dense` for pickling matrices. Nevertheless it should be updated. (Why we configure it to use freetype is unclear.)

Previous tickets: #27825, #27970, #19721, #18293, #17334


CC:  @dimpase @sagetrac-jpflori @kiwifb @antonio-rojas @slel

Keywords: **upgrade, libgd**

Branch/Commit: **[c20ea90](https://github.com/sagemath/sagetrac-mirror/commit/c20ea90c22774fbe22caf5ec66056b311bac27f5)**

Reviewer: **Travis Scrimshaw**

Author: **Samuel Lelièvre**

_Issue created by migration from https://trac.sagemath.org/ticket/31556_





---

archive/issue_events_416842.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-25T02:58:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416842"
}
```



---

archive/issue_events_416843.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-25T02:58:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416843"
}
```



---

archive/issue_events_416844.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-25T02:58:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416844"
}
```



---

archive/issue_events_416845.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-25T02:58:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416845"
}
```



---

archive/issue_events_416846.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-25T02:58:30Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "title_is": "Update libgd to 2.3.2",
    "title_was": "Update libgd to 2.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416846"
}
```



---

archive/issue_comments_510052.json:
```json
{
    "body": "Author: **Samuel Leli\u00e8vre**",
    "created_at": "2021-04-13T20:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31556#issuecomment-510052",
    "user": "https://github.com/slel"
}
```

Author: **Samuel Lelièvre**



---

archive/issue_comments_510053.json:
```json
{
    "body": "Changed keywords from **** to **upgrade, libgd**",
    "created_at": "2021-04-13T20:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31556#issuecomment-510053",
    "user": "https://github.com/slel"
}
```

Changed keywords from **** to **upgrade, libgd**



---

archive/issue_comments_510054.json:
```json
{
    "body": "Branch: **[public/31556](https://github.com/sagemath/sagetrac-mirror/tree/public/31556)**",
    "created_at": "2021-04-13T20:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31556#issuecomment-510054",
    "user": "https://github.com/slel"
}
```

Branch: **[public/31556](https://github.com/sagemath/sagetrac-mirror/tree/public/31556)**



---

archive/issue_comments_510055.json:
```json
{
    "body": "Commit: **[c20ea90](https://github.com/sagemath/sagetrac-mirror/commit/c20ea90c22774fbe22caf5ec66056b311bac27f5)**",
    "created_at": "2021-04-13T20:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31556#issuecomment-510055",
    "user": "https://github.com/slel"
}
```

Commit: **[c20ea90](https://github.com/sagemath/sagetrac-mirror/commit/c20ea90c22774fbe22caf5ec66056b311bac27f5)**



---

archive/issue_comments_510056.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c20ea90c22774fbe22caf5ec66056b311bac27f5\">c20ea90</a></td><td><code>31556: Upgrade: libgd 2.3.2</code></td></tr></table>\n",
    "created_at": "2021-04-13T20:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31556#issuecomment-510056",
    "user": "https://github.com/slel"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c20ea90c22774fbe22caf5ec66056b311bac27f5">c20ea90</a></td><td><code>31556: Upgrade: libgd 2.3.2</code></td></tr></table>




---

archive/issue_events_416847.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-04-13T20:10:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416847"
}
```



---

archive/issue_comments_510057.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nUnless it is a blocker, the milestone should be 9.4. I am personally holding off putting those tickets to positive review until the release as rebasing will probably be requested. \n\nOtherwise the ticket is fairly trivial and get rid of patches (yeah!).",
    "created_at": "2021-04-13T20:58:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31556#issuecomment-510057",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:4" align="right">Comment 4</div>

Unless it is a blocker, the milestone should be 9.4. I am personally holding off putting those tickets to positive review until the release as rebasing will probably be requested. 

Otherwise the ticket is fairly trivial and get rid of patches (yeah!).



---

archive/issue_comments_510058.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-04-16T00:12:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31556#issuecomment-510058",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_416848.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-04-16T00:12:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416848"
}
```



---

archive/issue_events_416849.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-04-16T00:12:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416849"
}
```



---

archive/issue_comments_510059.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI agree that this should be put off to 9.4 since we are deep in the rc stage. However, this LGTM, so off to the buildbots when we look towards 9.4.beta0.",
    "created_at": "2021-04-16T00:12:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31556#issuecomment-510059",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

I agree that this should be put off to 9.4 since we are deep in the rc stage. However, this LGTM, so off to the buildbots when we look towards 9.4.beta0.



---

archive/issue_events_416850.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-04-16T00:12:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416850"
}
```



---

archive/issue_events_416851.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-04-16T00:12:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416851"
}
```



---

archive/issue_events_416852.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-05-27T20:30:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416852"
}
```



---

archive/issue_events_416853.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2cf65a584d363949aff5bf2e650d3566ed2b7a18",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-05-27T20:30:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31556#event-416853"
}
```



---

archive/issue_comments_510060.json:
```json
{
    "body": "Changed branch from **[public/31556](https://github.com/sagemath/sagetrac-mirror/tree/public/31556)** to **[c20ea90](https://github.com/sagemath/sagetrac-mirror/commit/c20ea90c22774fbe22caf5ec66056b311bac27f5)**",
    "created_at": "2021-05-27T20:30:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31556#issuecomment-510060",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/31556](https://github.com/sagemath/sagetrac-mirror/tree/public/31556)** to **[c20ea90](https://github.com/sagemath/sagetrac-mirror/commit/c20ea90c22774fbe22caf5ec66056b311bac27f5)**
