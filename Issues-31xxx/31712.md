...
<a id='comment:6'></a>Replying to [comment:1 dimpase]:
> On Gentoo `[abi:cxx11]` is there in givaro library, and it all works:
> 
> ```
> $ nm -D /usr/lib64/libgivaro.so.9 | c++filt | grep abi\:cxx11
...
```json
{
    "body": "<a id='comment:8'></a>On Gentoo, going back to `sci-libs/givaro-4.1.1-r1` fixed the issue (still all is good as in comment:1)",
...
<a id='comment:8'></a>On Gentoo, going back to `sci-libs/givaro-4.1.1-r1` fixed the issue (still all is good as in comment:1)
