# Issue 31905: Fix suitesparse / cvxopt path configuration for Apple Silicon, Alpine Linux

archive/issues_031668.json:
```json
{
    "assignees": [],
    "body": "I was able to successfully build Sage on M1 (see #30592) but had two main types of doctest errors.\n\n```\n> However, I did see some of this type as well in the sage/numerical tests: \n> \n> ImportError: dlopen(/Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so, 2): Symbol not found: _cholmod_l_allocate_dense \n> Referenced from: /Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so \n> Expected in: flat namespace \n> in /Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so \n\n\notool -L /Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so \n\n\n/Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so:\n\t/opt/homebrew/opt/openblas/lib/libopenblas.0.dylib (compatibility version 0.0.0, current version 0.0.0)\n\t/usr/lib/libSystem.B.dylib (compatibility version 1.0.0, current version 1292.100.5) \n```\nThis apparently requires a change in the setup.py (see [this sage-devel discussion](https://groups.google.com/g/sage-devel/c/6LldEeJu3ZA/m/HXp3lFwCCgAJ)):\n\n```\nif sys.platform.startswith(\"darwin\"): \nSUITESPARSE_LIB_DIR = '/usr/local/lib' \nSUITESPARSE_INC_DIR = '/usr/local/include' \n```\nto allow `/opt/homebrew`.\n\nCC:  @dimpase @mkoeppe @orlitzky @kiwifb\n\nUpstream: **Reported upstream. No feedback yet.**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31905_\n\n",
    "closed_at": "2023-01-01T03:09:33Z",
    "created_at": "2021-06-04T11:26:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix suitesparse / cvxopt path configuration for Apple Silicon, Alpine Linux",
    "type": "issue",
    "updated_at": "2023-01-01T03:09:33Z",
    "url": "https://github.com/sagemath/sage/issues/31905",
    "user": "https://github.com/kcrisman"
}
```
I was able to successfully build Sage on M1 (see #30592) but had two main types of doctest errors.

```
> However, I did see some of this type as well in the sage/numerical tests: 
> 
> ImportError: dlopen(/Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so, 2): Symbol not found: _cholmod_l_allocate_dense 
> Referenced from: /Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so 
> Expected in: flat namespace 
> in /Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so 


otool -L /Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so 


/Users/karl.crisman/Downloads/BrewSage/sage-9.4.beta0/local/lib/python3.9/site-packages/cvxopt/cholmod.cpython-39-darwin.so:
	/opt/homebrew/opt/openblas/lib/libopenblas.0.dylib (compatibility version 0.0.0, current version 0.0.0)
	/usr/lib/libSystem.B.dylib (compatibility version 1.0.0, current version 1292.100.5) 
```
This apparently requires a change in the setup.py (see [this sage-devel discussion](https://groups.google.com/g/sage-devel/c/6LldEeJu3ZA/m/HXp3lFwCCgAJ)):

```
if sys.platform.startswith("darwin"): 
SUITESPARSE_LIB_DIR = '/usr/local/lib' 
SUITESPARSE_INC_DIR = '/usr/local/include' 
```
to allow `/opt/homebrew`.

CC:  @dimpase @mkoeppe @orlitzky @kiwifb

Upstream: **Reported upstream. No feedback yet.**

_Issue created by migration from https://trac.sagemath.org/ticket/31905_





---

archive/issue_events_421652.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2021-06-04T11:26:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421652"
}
```



---

archive/issue_events_421653.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2021-06-04T11:26:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421653"
}
```



---

archive/issue_events_421654.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2021-06-04T11:26:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421654"
}
```



---

archive/issue_events_421655.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2021-06-04T11:26:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421655"
}
```



---

archive/issue_comments_516114.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe following diff works and fixes the bad doctests for me (except some unrelated ones for graphics). \n\n```diff\ndiff --git a/build/pkgs/cvxopt/patches/libsuitesparse_path.patch b/build/pkgs/cvxopt/patches/libsuitesparse_path.patch\nindex fc8908aa7f..2da0e1f36a 100644\n--- a/build/pkgs/cvxopt/patches/libsuitesparse_path.patch\n+++ b/build/pkgs/cvxopt/patches/libsuitesparse_path.patch\n@@ -1,10 +1,13 @@\n diff --git a/setup.py b/setup.py\n-index d312416..4fa14c4 100644\n --- a/setup.py\n +++ b/setup.py\n-@@ -58,9 +58,9 @@ if sys.platform.startswith(\"darwin\"):\n-     SUITESPARSE_LIB_DIR = '/usr/local/lib'\n-     SUITESPARSE_INC_DIR = '/usr/local/include'\n+@@ -56,11 +56,11 @@\n+ if sys.platform.startswith(\"darwin\"):\n+     # macOS\n+-    SUITESPARSE_LIB_DIR = '/usr/local/lib'\n+-    SUITESPARSE_INC_DIR = '/usr/local/include'\n++    SUITESPARSE_LIB_DIR = '/opt/homebrew/lib'\n++    SUITESPARSE_INC_DIR = '/opt/homebrew/include'\n  else:\n -    if glob(\"/usr/lib/x86_64-linux-gnu/libsuitesparse*\"):\n -        # Ubuntu/Debian\n```\nI am *not* currently in a position to make a proper branch with this testing setup, my apologies, and in any case it needs some additional logic to test for which version of MacOS we have and/or if homebrew is being used.",
    "created_at": "2021-06-04T11:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516114",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'>Comment 1:</a>
The following diff works and fixes the bad doctests for me (except some unrelated ones for graphics). 

```diff
diff --git a/build/pkgs/cvxopt/patches/libsuitesparse_path.patch b/build/pkgs/cvxopt/patches/libsuitesparse_path.patch
index fc8908aa7f..2da0e1f36a 100644
--- a/build/pkgs/cvxopt/patches/libsuitesparse_path.patch
+++ b/build/pkgs/cvxopt/patches/libsuitesparse_path.patch
@@ -1,10 +1,13 @@
 diff --git a/setup.py b/setup.py
-index d312416..4fa14c4 100644
 --- a/setup.py
 +++ b/setup.py
-@@ -58,9 +58,9 @@ if sys.platform.startswith("darwin"):
-     SUITESPARSE_LIB_DIR = '/usr/local/lib'
-     SUITESPARSE_INC_DIR = '/usr/local/include'
+@@ -56,11 +56,11 @@
+ if sys.platform.startswith("darwin"):
+     # macOS
+-    SUITESPARSE_LIB_DIR = '/usr/local/lib'
+-    SUITESPARSE_INC_DIR = '/usr/local/include'
++    SUITESPARSE_LIB_DIR = '/opt/homebrew/lib'
++    SUITESPARSE_INC_DIR = '/opt/homebrew/include'
  else:
 -    if glob("/usr/lib/x86_64-linux-gnu/libsuitesparse*"):
 -        # Ubuntu/Debian
```
I am *not* currently in a position to make a proper branch with this testing setup, my apologies, and in any case it needs some additional logic to test for which version of MacOS we have and/or if homebrew is being used.



---

archive/issue_comments_516115.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nUpstream report...",
    "created_at": "2021-06-04T18:28:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516115",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
Upstream report...



---

archive/issue_comments_516116.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\n> Upstream report...\n\nWill they see this as a bug? I already asked about whether to report upstream [here](https://groups.google.com/g/sage-devel/c/6LldEeJu3ZA/m/SIvNj5s-CgAJ).",
    "created_at": "2021-06-04T18:46:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516116",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'>Comment 3:</a>
> Upstream report...

Will they see this as a bug? I already asked about whether to report upstream [here](https://groups.google.com/g/sage-devel/c/6LldEeJu3ZA/m/SIvNj5s-CgAJ).



---

archive/issue_comments_516117.json:
```json
{
    "body": "Upstream: **Reported upstream. No feedback yet.**",
    "created_at": "2021-06-04T18:54:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516117",
    "user": "https://github.com/kcrisman"
}
```

Upstream: **Reported upstream. No feedback yet.**



---

archive/issue_comments_516118.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nhttps://github.com/cvxopt/cvxopt/issues/195",
    "created_at": "2021-06-04T18:54:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516118",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:4'>Comment 4:</a>
https://github.com/cvxopt/cvxopt/issues/195



---

archive/issue_events_421656.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421656"
}
```



---

archive/issue_events_421657.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421657"
}
```



---

archive/issue_comments_516119.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThese defaults in cvxopt's setup.py can be overridden using environment variables\n`CVXOPT_SUITESPARSE_INC_DIR`, `CVXOPT_SUITESPARSE_LIB_DIR`.\nWe already set these variables when we build our own `suitesparse` (then `SAGE_SUITESPARSE_PREFIX` is nonempty). \n\nOn macOS, we could just set these variables also (to something arbitrary) when system suitesparse is in use.\n\nSee also [#33083 comment:40](https://github.com/sagemath/sage/issues/33083#comment:40) - a related issue on `alpine`.",
    "created_at": "2022-01-17T07:32:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516119",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'>Comment 6:</a>
These defaults in cvxopt's setup.py can be overridden using environment variables
`CVXOPT_SUITESPARSE_INC_DIR`, `CVXOPT_SUITESPARSE_LIB_DIR`.
We already set these variables when we build our own `suitesparse` (then `SAGE_SUITESPARSE_PREFIX` is nonempty). 

On macOS, we could just set these variables also (to something arbitrary) when system suitesparse is in use.

See also [#33083 comment:40](https://github.com/sagemath/sage/issues/33083#comment:40) - a related issue on `alpine`.



---

archive/issue_events_421658.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-17T07:35:45Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "title_is": "Fix suitesparse / cvxopt path configuration for Apple Silicon, Alpine Linux",
    "title_was": "cvxopt links wrong M1 directory",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421658"
}
```



---

archive/issue_events_421659.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-28T23:11:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421659"
}
```



---

archive/issue_events_421660.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-28T23:11:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421660"
}
```



---

archive/issue_events_421661.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421661"
}
```



---

archive/issue_events_421662.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421662"
}
```



---

archive/issue_events_421663.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421663"
}
```



---

archive/issue_events_421664.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421664"
}
```



---

archive/issue_comments_516120.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nMaybe this is resolved? I can build Sage (including `cvxopt`) on an M2 computer using homebrew's suitesparse, and all tests pass. If that's the issue, perhaps it's solved. Or is it about using Sage's suitesparse + Sage's cvxopt?",
    "created_at": "2022-09-26T18:50:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516120",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:11'>Comment 11:</a>
Maybe this is resolved? I can build Sage (including `cvxopt`) on an M2 computer using homebrew's suitesparse, and all tests pass. If that's the issue, perhaps it's solved. Or is it about using Sage's suitesparse + Sage's cvxopt?



---

archive/issue_comments_516121.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nIt's plausible that the cvxopt update in #34150 may have fixed it, but I haven't checked",
    "created_at": "2022-09-26T19:27:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516121",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:12'>Comment 12:</a>
It's plausible that the cvxopt update in #34150 may have fixed it, but I haven't checked



---

archive/issue_comments_516122.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\n`setup.py` still says\n\n```\nif sys.platform.startswith(\"darwin\"):\n    # macOS\n    SUITESPARSE_LIB_DIR = '/usr/local/lib'\n    SUITESPARSE_INC_DIR = '/usr/local/include'\n```\nand `/opt` is not found anywhere in that file, and looking at the log file, I don't know how it's finding the suitesparse libraries.",
    "created_at": "2022-09-26T19:53:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516122",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:13'>Comment 13:</a>
`setup.py` still says

```
if sys.platform.startswith("darwin"):
    # macOS
    SUITESPARSE_LIB_DIR = '/usr/local/lib'
    SUITESPARSE_INC_DIR = '/usr/local/include'
```
and `/opt` is not found anywhere in that file, and looking at the log file, I don't know how it's finding the suitesparse libraries.



---

archive/issue_comments_516123.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nI propose closing this. Sage builds fine on my `M2`, using a homebrew installation of `suitesparse`.",
    "created_at": "2022-11-16T01:04:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516123",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:14'>Comment 14:</a>
I propose closing this. Sage builds fine on my `M2`, using a homebrew installation of `suitesparse`.



---

archive/issue_events_421665.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-11-16T01:04:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421665"
}
```



---

archive/issue_events_421666.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-11-16T01:04:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421666"
}
```



---

archive/issue_events_421667.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-11-16T01:04:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421667"
}
```



---

archive/issue_events_421668.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-11-16T01:04:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421668"
}
```



---

archive/issue_comments_516124.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nReplying to [John Palmieri](#comment%3A14):\n> I propose closing this. Sage builds fine on my `M2`, using a homebrew installation of `suitesparse`.\n\ncan one build Sage's `suitsparse` on M2, and use it to build `cvxopt`?\nIf so, we can close it.",
    "created_at": "2022-11-16T12:43:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516124",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:16'>Comment 16:</a>
Replying to [John Palmieri](#comment%3A14):
> I propose closing this. Sage builds fine on my `M2`, using a homebrew installation of `suitesparse`.

can one build Sage's `suitsparse` on M2, and use it to build `cvxopt`?
If so, we can close it.



---

archive/issue_comments_516125.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nReplying to [Dima Pasechnik](#comment%3A16):\n> Replying to [John Palmieri](#comment%3A14):\n> > I propose closing this. Sage builds fine on my `M2`, using a homebrew installation of `suitesparse`.\n\n> \n> can one build Sage's `suitsparse` on M2, and use it to build `cvxopt`?\n> If so, we can close it.\n\nI can build successfully using Sage's `suitesparse` and also homebrew's `suitesparse`, so setting to positive review.",
    "created_at": "2022-11-16T19:40:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31905#issuecomment-516125",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:17'>Comment 17:</a>
Replying to [Dima Pasechnik](#comment%3A16):
> Replying to [John Palmieri](#comment%3A14):
> > I propose closing this. Sage builds fine on my `M2`, using a homebrew installation of `suitesparse`.

> 
> can one build Sage's `suitsparse` on M2, and use it to build `cvxopt`?
> If so, we can close it.

I can build successfully using Sage's `suitesparse` and also homebrew's `suitesparse`, so setting to positive review.



---

archive/issue_events_421669.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-11-16T19:40:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421669"
}
```



---

archive/issue_events_421670.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-11-16T19:40:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421670"
}
```



---

archive/issue_events_421671.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-01T03:09:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421671"
}
```



---

archive/issue_events_421672.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-01T03:09:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31905",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31905#event-421672"
}
```
