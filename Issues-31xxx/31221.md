# Issue 31221: Alllow arbitrary folder as output directory of docbuild

archive/issues_030984.json:
```json
{
    "assignees": [],
    "body": "It was not possible to specify an arbitrary folder as the output of the docbuild, because the citation extension had an assertion that checked that the output directory was a subfolder of `SAGE_DOC`. This check is replaced by providing some default folder for the citations in case the output directory is not below `SAGE_DOC`. \n\nThis is allows to build the documentation by running `sphinx` manually, which is required by some tools (e.g. the restructuredtext extension for VS code provides a HTML preview of the currently edited rst file by running sphinx on it).\n\n\nI also used the opportunity to convert the code in question to use pathlib. \n\nCC:  @tscrim @nthiery @fchapoton @mkoeppe @jhpalmieri\n\nBranch: **[ad75421](https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Tobias Diez**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31221_\n\n",
    "closed_at": "2021-01-24T10:37:18Z",
    "created_at": "2021-01-10T16:07:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Alllow arbitrary folder as output directory of docbuild",
    "type": "issue",
    "updated_at": "2023-01-08T09:15:38Z",
    "url": "https://github.com/sagemath/sage/issues/31221",
    "user": "https://github.com/tobiasdiez"
}
```
It was not possible to specify an arbitrary folder as the output of the docbuild, because the citation extension had an assertion that checked that the output directory was a subfolder of `SAGE_DOC`. This check is replaced by providing some default folder for the citations in case the output directory is not below `SAGE_DOC`. 

This is allows to build the documentation by running `sphinx` manually, which is required by some tools (e.g. the restructuredtext extension for VS code provides a HTML preview of the currently edited rst file by running sphinx on it).


I also used the opportunity to convert the code in question to use pathlib. 

CC:  @tscrim @nthiery @fchapoton @mkoeppe @jhpalmieri

Branch: **[ad75421](https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477)**

Reviewer: **Matthias Koeppe**

Author: **Tobias Diez**

_Issue created by migration from https://trac.sagemath.org/ticket/31221_





---

archive/issue_events_412040.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2021-01-10T16:07:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31221#event-412040"
}
```



---

archive/issue_events_412041.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2021-01-10T16:07:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31221#event-412041"
}
```



---

archive/issue_events_412042.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2021-01-10T16:07:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31221#event-412042"
}
```



---

archive/issue_events_412043.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2021-01-10T16:07:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31221#event-412043"
}
```



---

archive/issue_comments_503190.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2021-01-10T16:08:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503190",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:1" align="right">Comment 1</div>

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_503191.json:
```json
{
    "body": "Commit: **[9d686f2](https://github.com/sagemath/sagetrac-mirror/commit/9d686f22f4cfb2c9553ea106f42ef81ff893b0aa)**",
    "created_at": "2021-01-10T16:08:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503191",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[9d686f2](https://github.com/sagemath/sagetrac-mirror/commit/9d686f22f4cfb2c9553ea106f42ef81ff893b0aa)**



---

archive/issue_comments_503192.json:
```json
{
    "body": "Changed commit from **[9d686f2](https://github.com/sagemath/sagetrac-mirror/commit/9d686f22f4cfb2c9553ea106f42ef81ff893b0aa)** to **[ad75421](https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477)**",
    "created_at": "2021-01-10T16:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503192",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9d686f2](https://github.com/sagemath/sagetrac-mirror/commit/9d686f22f4cfb2c9553ea106f42ef81ff893b0aa)** to **[ad75421](https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477)**



---

archive/issue_comments_503193.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477\">ad75421</a></td><td><code>Alllow arbitrary folder as output directory of docbuild</code></td></tr></table>\n",
    "created_at": "2021-01-10T16:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503193",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477">ad75421</a></td><td><code>Alllow arbitrary folder as output directory of docbuild</code></td></tr></table>




---

archive/issue_events_412044.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2021-01-10T16:09:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31221#event-412044"
}
```



---

archive/issue_comments_503194.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-It was not possible to specify an arbitrary folder as the output of the docbuild, because the citation extension had an assertion that checked that the output directory was a subfolder of \u0300 SAGE_DOC`. This check is replaced by providing some default folder for the citations in case the output directory is not below `SAGE_DOC`. \n+It was not possible to specify an arbitrary folder as the output of the docbuild, because the citation extension had an assertion that checked that the output directory was a subfolder of `SAGE_DOC`. This check is replaced by providing some default folder for the citations in case the output directory is not below `SAGE_DOC`. \n \n This is allows to build the documentation by running `sphinx` manually, which is required by some tools (e.g. the restructuredtext extension for VS code provides a HTML preview of the currently edited rst file by running sphinx on it).\n \n``````\n",
    "created_at": "2021-01-10T18:50:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503194",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-It was not possible to specify an arbitrary folder as the output of the docbuild, because the citation extension had an assertion that checked that the output directory was a subfolder of ̀ SAGE_DOC`. This check is replaced by providing some default folder for the citations in case the output directory is not below `SAGE_DOC`. 
+It was not possible to specify an arbitrary folder as the output of the docbuild, because the citation extension had an assertion that checked that the output directory was a subfolder of `SAGE_DOC`. This check is replaced by providing some default folder for the citations in case the output directory is not below `SAGE_DOC`. 
 
 This is allows to build the documentation by running `sphinx` manually, which is required by some tools (e.g. the restructuredtext extension for VS code provides a HTML preview of the currently edited rst file by running sphinx on it).
 
``````




---

archive/issue_comments_503195.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nPatchbot is green, looks fine to me.",
    "created_at": "2021-01-16T18:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503195",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Patchbot is green, looks fine to me.



---

archive/issue_events_412045.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-16T18:39:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31221#event-412045"
}
```



---

archive/issue_events_412046.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-16T18:39:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31221#event-412046"
}
```



---

archive/issue_comments_503196.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-01-16T18:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503196",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_503197.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThanks!",
    "created_at": "2021-01-16T19:19:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503197",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thanks!



---

archive/issue_events_412047.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-01-24T10:37:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31221#event-412047"
}
```



---

archive/issue_events_412048.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8101d780567ad3b651920397b4548eeaa7446f84",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-01-24T10:37:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31221#event-412048"
}
```



---

archive/issue_comments_503198.json:
```json
{
    "body": "Changed branch from **[public/documentation/allow-from-arbitrary-folder](https://github.com/sagemath/sagetrac-mirror/tree/public/documentation/allow-from-arbitrary-folder)** to **[ad75421](https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477)**",
    "created_at": "2021-01-24T10:37:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503198",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/documentation/allow-from-arbitrary-folder](https://github.com/sagemath/sagetrac-mirror/tree/public/documentation/allow-from-arbitrary-folder)** to **[ad75421](https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477)**



---

archive/issue_comments_503199.json:
```json
{
    "body": "Changed commit from **[ad75421](https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477)** to none",
    "created_at": "2023-01-08T09:15:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31221#issuecomment-503199",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[ad75421](https://github.com/sagemath/sagetrac-mirror/commit/ad75421920754cbc85e5a301e30e2c8d86046477)** to none
