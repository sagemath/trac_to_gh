...
<a id='comment:3'></a>The `sig_check()` outside the loop is unnecessary:
...
```json
{
    "body": "<a id='comment:9'></a>Thank you. This is quite a nice improvement.",
...
<a id='comment:9'></a>Thank you. This is quite a nice improvement.
...
<a id='comment:11'></a>On the kucalc buildbot:
...
<a id='comment:13'></a>This appears to be an issue with python 3.5 to 3.7 according to https://bugs.python.org/issue25150
...
```json
{
    "body": "<a id='comment:14'></a>Actually, that is a problem with cysignals that should be fixed there.",
...
<a id='comment:14'></a>Actually, that is a problem with cysignals that should be fixed there.
...
<a id='comment:16'></a>According to 
...
```json
{
    "body": "<a id='comment:18'></a>Back off to the builbots.",
    "created_at": "2021-02-01T00:55:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31245",
    "type": "issue_comment",
...
<a id='comment:18'></a>Back off to the builbots.
...
<a id='comment:19'></a>Build fails on various buildbots:
...
```json
{
    "body": "<a id='comment:22'></a>I don't think this is going to happen with 9.3 anymore, if this is waiting on a cysignals patch/update.",
...
<a id='comment:22'></a>I don't think this is going to happen with 9.3 anymore, if this is waiting on a cysignals patch/update.
...
<a id='comment:24'></a>This bug isn't too hard to reproduce actually:
...
```json
{
    "body": "<a id='comment:26'></a>This might need more work, as the flag `-fopenmp` isn't understood everywhere: https://github.com/kliem/sage/pull/40/checks?check_run_id=2076988665\n\nSo we should probably check out at configure time, whether our compiler supports this, similar to #27122.",
...
<a id='comment:26'></a>This might need more work, as the flag `-fopenmp` isn't understood everywhere: https://github.com/kliem/sage/pull/40/checks?check_run_id=2076988665
...
```json
{
    "body": "<a id='comment:30'></a>Tests are running here:\n\nhttps://github.com/kliem/sage/pull/40/checks",
...
<a id='comment:30'></a>Tests are running here:
...
```json
{
    "body": "<a id='comment:33'></a>Thanks again.",
...
<a id='comment:33'></a>Thanks again.
...
<a id='comment:37'></a>I'm not sure this was actually a merge conflict.
...
```json
{
    "body": "<a id='comment:38'></a>Works for me. Let's try this again...",
    "created_at": "2021-05-21T00:14:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31245",
    "type": "issue_comment",
...
<a id='comment:38'></a>Works for me. Let's try this again...
...
<a id='comment:42'></a>Segfaults reliably on OSX
...
<a id='comment:43'></a>Thanks Volker for the precise log.
...
<a id='comment:44'></a>I think I tracked it down. I assumed that
...
```json
{
    "body": "<a id='comment:45'></a>I really don't understand why this pops out just now and not earlier.",
...
<a id='comment:45'></a>I really don't understand why this pops out just now and not earlier.
...
```json
{
    "body": "<a id='comment:48'></a>Hopefully this will be the last iteration of this. Sorry for having to redo this a bunch of times Volker; I just haven't been able to locally recreate any of the issues.",
...
<a id='comment:48'></a>Hopefully this will be the last iteration of this. Sorry for having to redo this a bunch of times Volker; I just haven't been able to locally recreate any of the issues.
