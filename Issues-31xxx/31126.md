# Issue 31126: fix typos in calculus/

archive/issues_030889.json:
```json
{
    "assignees": [],
    "body": "plus a few pep8 details in the modified file\n\nCC:  @slel tmonteil @tscrim\n\nBranch/Commit: **[14fa073](https://github.com/sagemath/sagetrac-mirror/commit/14fa0737d6f91be9d4c5b1efa5fc4bdbbd6c2370)**\n\nReviewer: **David Coudert**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31126_\n\n",
    "closed_at": "2021-01-04T22:37:09Z",
    "created_at": "2020-12-28T20:36:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix typos in calculus/",
    "type": "issue",
    "updated_at": "2021-01-04T22:37:09Z",
    "url": "https://github.com/sagemath/sage/issues/31126",
    "user": "https://github.com/fchapoton"
}
```
plus a few pep8 details in the modified file

CC:  @slel tmonteil @tscrim

Branch/Commit: **[14fa073](https://github.com/sagemath/sagetrac-mirror/commit/14fa0737d6f91be9d4c5b1efa5fc4bdbbd6c2370)**

Reviewer: **David Coudert**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/31126_





---

archive/issue_events_410916.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-12-28T20:36:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31126#event-410916"
}
```



---

archive/issue_events_410917.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-12-28T20:36:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31126#event-410917"
}
```



---

archive/issue_events_410918.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-12-28T20:36:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31126#event-410918"
}
```



---

archive/issue_events_410919.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-12-28T20:36:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31126#event-410919"
}
```



---

archive/issue_events_410920.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-12-28T20:37:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31126#event-410920"
}
```



---

archive/issue_comments_501847.json:
```json
{
    "body": "Branch: **[u/chapoton/31126](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/31126)**",
    "created_at": "2020-12-28T20:37:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31126#issuecomment-501847",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/31126](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/31126)**



---

archive/issue_comments_501848.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nticket trivial et facile, je crois\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/14fa0737d6f91be9d4c5b1efa5fc4bdbbd6c2370\">14fa073</a></td><td><code>fix one typo in calculus/</code></td></tr></table>\n",
    "created_at": "2020-12-28T20:37:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31126#issuecomment-501848",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
ticket trivial et facile, je crois

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/14fa0737d6f91be9d4c5b1efa5fc4bdbbd6c2370">14fa073</a></td><td><code>fix one typo in calculus/</code></td></tr></table>




---

archive/issue_comments_501849.json:
```json
{
    "body": "Commit: **[14fa073](https://github.com/sagemath/sagetrac-mirror/commit/14fa0737d6f91be9d4c5b1efa5fc4bdbbd6c2370)**",
    "created_at": "2020-12-28T20:37:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31126#issuecomment-501849",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[14fa073](https://github.com/sagemath/sagetrac-mirror/commit/14fa0737d6f91be9d4c5b1efa5fc4bdbbd6c2370)**



---

archive/issue_comments_501850.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThe doctest error reported by the patchbot is not caused by the modifications done in this ticket. so LGTM.",
    "created_at": "2020-12-30T23:44:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31126#issuecomment-501850",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:3'>Comment 3:</a>
The doctest error reported by the patchbot is not caused by the modifications done in this ticket. so LGTM.



---

archive/issue_events_410921.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-12-30T23:44:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31126#event-410921"
}
```



---

archive/issue_events_410922.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-12-30T23:44:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31126#event-410922"
}
```



---

archive/issue_comments_501851.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2020-12-30T23:44:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31126#issuecomment-501851",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_comments_501852.json:
```json
{
    "body": "Changed branch from **[u/chapoton/31126](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/31126)** to **[14fa073](https://github.com/sagemath/sagetrac-mirror/commit/14fa0737d6f91be9d4c5b1efa5fc4bdbbd6c2370)**",
    "created_at": "2021-01-04T22:37:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31126#issuecomment-501852",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/31126](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/31126)** to **[14fa073](https://github.com/sagemath/sagetrac-mirror/commit/14fa0737d6f91be9d4c5b1efa5fc4bdbbd6c2370)**



---

archive/issue_events_410923.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-01-04T22:37:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31126#event-410923"
}
```



---

archive/issue_events_410924.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f69f8ca4e450286b3a9709d5d7b1562ff7254cb1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-01-04T22:37:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31126",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31126#event-410924"
}
```
