# Issue 31310: interactive_simplex_method: Customize LPRevisedDictionary for Vanderbei notation

archive/issues_031073.json:
```json
{
    "assignees": [],
    "body": "If style is set to Vanderbei, we change the notation for the data in the revised dictionary accordingly.\n\n\nDepends on #30809\n\nCC:  @yuan-zhou @novoselt\n\nBranch/Commit: **[u/mkoeppe/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation) @ [98083f0](https://github.com/sagemath/sagetrac-mirror/commit/98083f0bb674681615d0eb9223d0e5b2bf218dca)**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31310_\n\n",
    "created_at": "2021-01-31T08:59:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20review"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "interactive_simplex_method: Customize LPRevisedDictionary for Vanderbei notation",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/31310",
    "user": "https://github.com/mkoeppe"
}
```
If style is set to Vanderbei, we change the notation for the data in the revised dictionary accordingly.


Depends on #30809

CC:  @yuan-zhou @novoselt

Branch/Commit: **[u/mkoeppe/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation) @ [98083f0](https://github.com/sagemath/sagetrac-mirror/commit/98083f0bb674681615d0eb9223d0e5b2bf218dca)**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/31310_





---

archive/issue_events_413318.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-31T08:59:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413318"
}
```



---

archive/issue_events_413319.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-31T08:59:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
    "label_color": "0000ff",
    "label_name": "component: linear programming",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413319"
}
```



---

archive/issue_events_413320.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-31T08:59:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413320"
}
```



---

archive/issue_events_413321.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-31T08:59:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413321"
}
```



---

archive/issue_comments_504902.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis can be taken from https://github.com/mkoeppe/sage/commit/730c8032ae22b57b1c13d8bae67c0db08b3c924b",
    "created_at": "2021-01-31T09:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31310#issuecomment-504902",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'>Comment 1:</a>
This can be taken from https://github.com/mkoeppe/sage/commit/730c8032ae22b57b1c13d8bae67c0db08b3c924b



---

archive/issue_comments_504903.json:
```json
{
    "body": "Branch: **[u/mkoeppe/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation)**",
    "created_at": "2021-01-31T18:49:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31310#issuecomment-504903",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation)**



---

archive/issue_comments_504904.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d2bb22a901e9aa6a6d6b0a6d89e1b12be32f48df\">d2bb22a</a></td><td><code>src/sage/numerical/interactive_simplex_method.py (LPDictionary): Fix use of \\color with MathJaX</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/37bbf530f70a91a761079f58ab3e9ab815bac838\">37bbf53</a></td><td><code>src/sage/numerical/interactive_simplex_method.py (LPRevisedDictionary._latex_): Fix use of \\color for mathjax</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/69a9e38404c86ef3e280ce162322d69d5fd7d399\">69a9e38</a></td><td><code>interactive_simplex_method: Give better instructions how to view typeset problems/dictionaries</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/88d8e49870e35b2d7f73483839915b76dd9c7f34\">88d8e49</a></td><td><code>interactive_simplex_method: A bit more documentation for display typeset</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/489816f57ae5499d6ea6f81e0ca3c31853bc7109\">489816f</a></td><td><code>Merge branch 't/30809/interactive_simplex_method_fix_color_jupyter' into t/31310/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/98083f0bb674681615d0eb9223d0e5b2bf218dca\">98083f0</a></td><td><code>src/sage/numerical/interactive_simplex_method.py (LPRevisedDictionary._latex_): Customize for Vanderbei notation</code></td></tr></table>\n",
    "created_at": "2021-01-31T18:52:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31310#issuecomment-504904",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d2bb22a901e9aa6a6d6b0a6d89e1b12be32f48df">d2bb22a</a></td><td><code>src/sage/numerical/interactive_simplex_method.py (LPDictionary): Fix use of \color with MathJaX</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/37bbf530f70a91a761079f58ab3e9ab815bac838">37bbf53</a></td><td><code>src/sage/numerical/interactive_simplex_method.py (LPRevisedDictionary._latex_): Fix use of \color for mathjax</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/69a9e38404c86ef3e280ce162322d69d5fd7d399">69a9e38</a></td><td><code>interactive_simplex_method: Give better instructions how to view typeset problems/dictionaries</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/88d8e49870e35b2d7f73483839915b76dd9c7f34">88d8e49</a></td><td><code>interactive_simplex_method: A bit more documentation for display typeset</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/489816f57ae5499d6ea6f81e0ca3c31853bc7109">489816f</a></td><td><code>Merge branch 't/30809/interactive_simplex_method_fix_color_jupyter' into t/31310/interactive_simplex_method__customize_lpreviseddictionary_for_vanderbei_notation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/98083f0bb674681615d0eb9223d0e5b2bf218dca">98083f0</a></td><td><code>src/sage/numerical/interactive_simplex_method.py (LPRevisedDictionary._latex_): Customize for Vanderbei notation</code></td></tr></table>




---

archive/issue_comments_504905.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-01-31T18:52:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31310#issuecomment-504905",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_413322.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-31T18:52:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413322"
}
```



---

archive/issue_comments_504906.json:
```json
{
    "body": "Commit: **[98083f0](https://github.com/sagemath/sagetrac-mirror/commit/98083f0bb674681615d0eb9223d0e5b2bf218dca)**",
    "created_at": "2021-01-31T18:52:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31310#issuecomment-504906",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[98083f0](https://github.com/sagemath/sagetrac-mirror/commit/98083f0bb674681615d0eb9223d0e5b2bf218dca)**



---

archive/issue_comments_504907.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,2 @@\n+If style is set to Vanderbei, we change the notation for the data in the revised dictionary accordingly.\n \n``````\n",
    "created_at": "2021-01-31T18:52:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31310#issuecomment-504907",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,2 @@
+If style is set to Vanderbei, we change the notation for the data in the revised dictionary accordingly.
 
``````




---

archive/issue_events_413323.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413323"
}
```



---

archive/issue_events_413324.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413324"
}
```



---

archive/issue_comments_504908.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-15T22:07:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31310#issuecomment-504908",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_413325.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413325"
}
```



---

archive/issue_events_413326.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413326"
}
```



---

archive/issue_comments_504909.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nSetting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31310#issuecomment-504909",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'>Comment 5:</a>
Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_413327.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413327"
}
```



---

archive/issue_events_413328.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413328"
}
```



---

archive/issue_comments_504910.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nStalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.",
    "created_at": "2021-12-18T19:53:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31310#issuecomment-504910",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'>Comment 6:</a>
Stalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.



---

archive/issue_events_413329.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T17:54:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413329"
}
```



---

archive/issue_events_413330.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T17:54:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413330"
}
```



---

archive/issue_events_413331.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413331"
}
```



---

archive/issue_events_413332.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413332"
}
```



---

archive/issue_events_413333.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413333"
}
```



---

archive/issue_events_413334.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31310",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31310#event-413334"
}
```
