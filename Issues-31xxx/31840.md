# Issue 31840: perl_mongodb: Split out from perl_cpan_polymake_prereq

archive/issues_031603.json:
```json
{
    "assignees": [],
    "body": "This package has lots of Perl dependencies, is only an optional dependency of polymake (for PolyDB), and has recently been abandoned upstream.\n\n\n\nCC:  @yuan-zhou @kliem @dimpase @tscrim\n\nBranch/Commit: **[24c8003](https://github.com/sagemath/sagetrac-mirror/commit/24c80038bdb56340fc165e96510c41609450eaaa)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31840_\n\n",
    "closed_at": "2021-06-21T21:17:09Z",
    "created_at": "2021-05-19T20:23:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "perl_mongodb: Split out from perl_cpan_polymake_prereq",
    "type": "issue",
    "updated_at": "2021-06-21T21:17:09Z",
    "url": "https://github.com/sagemath/sage/issues/31840",
    "user": "https://github.com/mkoeppe"
}
```
This package has lots of Perl dependencies, is only an optional dependency of polymake (for PolyDB), and has recently been abandoned upstream.



CC:  @yuan-zhou @kliem @dimpase @tscrim

Branch/Commit: **[24c8003](https://github.com/sagemath/sagetrac-mirror/commit/24c80038bdb56340fc165e96510c41609450eaaa)**

Reviewer: **Travis Scrimshaw**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/31840_





---

archive/issue_events_420836.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-19T20:23:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420836"
}
```



---

archive/issue_events_420837.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-19T20:23:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "component: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420837"
}
```



---

archive/issue_events_420838.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-19T20:23:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420838"
}
```



---

archive/issue_events_420839.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-19T20:23:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420839"
}
```



---

archive/issue_comments_514922.json:
```json
{
    "body": "Branch: **[u/mkoeppe/perl_mongodb__split_out_from_perl_cpan_polymake_prereq](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/perl_mongodb__split_out_from_perl_cpan_polymake_prereq)**",
    "created_at": "2021-05-20T01:47:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31840#issuecomment-514922",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/perl_mongodb__split_out_from_perl_cpan_polymake_prereq](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/perl_mongodb__split_out_from_perl_cpan_polymake_prereq)**



---

archive/issue_comments_514923.json:
```json
{
    "body": "Commit: **[24c8003](https://github.com/sagemath/sagetrac-mirror/commit/24c80038bdb56340fc165e96510c41609450eaaa)**",
    "created_at": "2021-05-20T01:48:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31840#issuecomment-514923",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[24c8003](https://github.com/sagemath/sagetrac-mirror/commit/24c80038bdb56340fc165e96510c41609450eaaa)**



---

archive/issue_events_420840.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-20T01:48:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "component: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420840"
}
```



---

archive/issue_events_420841.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-20T01:48:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420841"
}
```



---

archive/issue_comments_514924.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/24c80038bdb56340fc165e96510c41609450eaaa\">24c8003</a></td><td><code>build/pkgs/mongodb: Split out from perl_cpan_polymake_prereq</code></td></tr></table>\n",
    "created_at": "2021-05-20T01:48:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31840#issuecomment-514924",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/24c80038bdb56340fc165e96510c41609450eaaa">24c8003</a></td><td><code>build/pkgs/mongodb: Split out from perl_cpan_polymake_prereq</code></td></tr></table>




---

archive/issue_events_420842.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-20T01:48:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420842"
}
```



---

archive/issue_comments_514925.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-05-20T01:48:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31840#issuecomment-514925",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_420843.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-05-23T06:41:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420843"
}
```



---

archive/issue_events_420844.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-05-23T06:41:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420844"
}
```



---

archive/issue_comments_514926.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-05-23T06:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31840#issuecomment-514926",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_514927.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2021-05-23T06:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31840#issuecomment-514927",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_514928.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks!",
    "created_at": "2021-05-23T18:24:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31840#issuecomment-514928",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks!



---

archive/issue_comments_514929.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/perl_mongodb__split_out_from_perl_cpan_polymake_prereq](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/perl_mongodb__split_out_from_perl_cpan_polymake_prereq)** to **[24c8003](https://github.com/sagemath/sagetrac-mirror/commit/24c80038bdb56340fc165e96510c41609450eaaa)**",
    "created_at": "2021-06-21T21:17:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31840#issuecomment-514929",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/perl_mongodb__split_out_from_perl_cpan_polymake_prereq](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/perl_mongodb__split_out_from_perl_cpan_polymake_prereq)** to **[24c8003](https://github.com/sagemath/sagetrac-mirror/commit/24c80038bdb56340fc165e96510c41609450eaaa)**



---

archive/issue_events_420845.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-06-21T21:17:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420845"
}
```



---

archive/issue_events_420846.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "390bddf5818e00538234455d47752dfff8379675",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-06-21T21:17:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31840",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31840#event-420846"
}
```
