# Issue 31925: Reorganize topology

archive/issues_031688.json:
```json
{
    "assignees": [],
    "body": "Proposal: create a directory `sage/topology` and move some of the files currently in `sage/homology`: move `cell_complex.py`, `cubical_complex.py`, `delta_complex.py`, and `simplicial_*.py`. Also move `examples.py` and rename it to `simplicial_complex_examples.py` or similar.\n\nCC:  @tscrim @yuan-zhou\n\nBranch: **[f641bbb](https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31925_\n\n",
    "closed_at": "2021-06-29T23:08:31Z",
    "created_at": "2021-06-07T18:46:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20topology",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Reorganize topology",
    "type": "issue",
    "updated_at": "2021-07-01T04:33:54Z",
    "url": "https://github.com/sagemath/sage/issues/31925",
    "user": "https://github.com/jhpalmieri"
}
```
Proposal: create a directory `sage/topology` and move some of the files currently in `sage/homology`: move `cell_complex.py`, `cubical_complex.py`, `delta_complex.py`, and `simplicial_*.py`. Also move `examples.py` and rename it to `simplicial_complex_examples.py` or similar.

CC:  @tscrim @yuan-zhou

Branch: **[f641bbb](https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde)**

Reviewer: **Matthias Koeppe**

Author: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/31925_





---

archive/issue_events_421958.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-06-07T18:46:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421958"
}
```



---

archive/issue_events_421959.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-06-07T18:46:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20topology",
    "label_color": "0000ff",
    "label_name": "component: algebraic topology",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421959"
}
```



---

archive/issue_events_421960.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-06-07T18:46:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421960"
}
```



---

archive/issue_events_421961.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-06-07T18:46:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421961"
}
```



---

archive/issue_comments_516624.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nHow do we do this and respect Sage's deprecation policy? Have deprecated versions of all of the files in `sage/homology` which just contain deprecation warnings and then import from the corresponding file in `sage/topology`? Is there another option?",
    "created_at": "2021-06-07T23:24:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516624",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">Comment 1</div>

How do we do this and respect Sage's deprecation policy? Have deprecated versions of all of the files in `sage/homology` which just contain deprecation warnings and then import from the corresponding file in `sage/topology`? Is there another option?



---

archive/issue_comments_516625.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI think your proposal is good. I believe we did that previously when moving files.",
    "created_at": "2021-06-08T00:43:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516625",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

I think your proposal is good. I believe we did that previously when moving files.



---

archive/issue_comments_516626.json:
```json
{
    "body": "Branch: **[u/jhpalmieri/reorg-topology](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/reorg-topology)**",
    "created_at": "2021-06-08T03:06:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516626",
    "user": "https://github.com/jhpalmieri"
}
```

Branch: **[u/jhpalmieri/reorg-topology](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/reorg-topology)**



---

archive/issue_comments_516627.json:
```json
{
    "body": "Author: **John Palmieri**",
    "created_at": "2021-06-08T03:07:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516627",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **John Palmieri**



---

archive/issue_comments_516628.json:
```json
{
    "body": "Commit: **[3e2c041](https://github.com/sagemath/sagetrac-mirror/commit/3e2c0417ed5a072067996fd767190f080313da28)**",
    "created_at": "2021-06-08T03:07:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516628",
    "user": "https://github.com/jhpalmieri"
}
```

Commit: **[3e2c041](https://github.com/sagemath/sagetrac-mirror/commit/3e2c0417ed5a072067996fd767190f080313da28)**



---

archive/issue_comments_516629.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOkay, let's try this. The two separate commits may be easier to review than the final result.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e1f4093f60c7349e740fc79302e9808057b05151\">e1f4093</a></td><td><code>trac 31925: move files into sage/topology</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3e2c0417ed5a072067996fd767190f080313da28\">3e2c041</a></td><td><code>trac 31925: add deprecated versions</code></td></tr></table>\n",
    "created_at": "2021-06-08T03:07:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516629",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:4" align="right">Comment 4</div>

Okay, let's try this. The two separate commits may be easier to review than the final result.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e1f4093f60c7349e740fc79302e9808057b05151">e1f4093</a></td><td><code>trac 31925: move files into sage/topology</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3e2c0417ed5a072067996fd767190f080313da28">3e2c041</a></td><td><code>trac 31925: add deprecated versions</code></td></tr></table>




---

archive/issue_events_421962.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-06-08T03:07:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421962"
}
```



---

archive/issue_comments_516630.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nDoctests passed for me after having done only commit [e1f4093](https://github.com/sagemath/sagetrac-mirror/commit/e1f4093f60c7349e740fc79302e9808057b05151), and in addition I searched for `sage.homology...`, so I think I caught all of the old references.",
    "created_at": "2021-06-08T03:09:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516630",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:5" align="right">Comment 5</div>

Doctests passed for me after having done only commit [e1f4093](https://github.com/sagemath/sagetrac-mirror/commit/e1f4093f60c7349e740fc79302e9808057b05151), and in addition I searched for `sage.homology...`, so I think I caught all of the old references.



---

archive/issue_comments_516631.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nmerge conflict with 9.4.beta3",
    "created_at": "2021-06-23T01:57:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516631",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

merge conflict with 9.4.beta3



---

archive/issue_events_421963.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-23T01:57:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421963"
}
```



---

archive/issue_events_421964.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-23T01:57:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421964"
}
```



---

archive/issue_comments_516632.json:
```json
{
    "body": "Changed commit from **[3e2c041](https://github.com/sagemath/sagetrac-mirror/commit/3e2c0417ed5a072067996fd767190f080313da28)** to **[f641bbb](https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde)**",
    "created_at": "2021-06-23T03:59:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516632",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3e2c041](https://github.com/sagemath/sagetrac-mirror/commit/3e2c0417ed5a072067996fd767190f080313da28)** to **[f641bbb](https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde)**



---

archive/issue_comments_516633.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde\">f641bbb</a></td><td><code>trac 31925: rebase to 9.4.beta3</code></td></tr></table>\n",
    "created_at": "2021-06-23T03:59:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516633",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde">f641bbb</a></td><td><code>trac 31925: rebase to 9.4.beta3</code></td></tr></table>




---

archive/issue_comments_516634.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nRebased.",
    "created_at": "2021-06-23T04:00:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516634",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:9" align="right">Comment 9</div>

Rebased.



---

archive/issue_events_421965.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-06-23T04:00:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421965"
}
```



---

archive/issue_events_421966.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-06-23T04:00:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421966"
}
```



---

archive/issue_comments_516635.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nLGTM.",
    "created_at": "2021-06-23T06:55:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516635",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:10" align="right">Comment 10</div>

LGTM.



---

archive/issue_events_421967.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-23T06:55:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421967"
}
```



---

archive/issue_events_421968.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-23T06:55:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421968"
}
```



---

archive/issue_comments_516636.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-06-23T06:55:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516636",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_516637.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nGreat, thank you!",
    "created_at": "2021-06-23T16:09:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516637",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:12" align="right">Comment 12</div>

Great, thank you!



---

archive/issue_comments_516638.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/reorg-topology](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/reorg-topology)** to **[f641bbb](https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde)**",
    "created_at": "2021-06-29T23:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516638",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/reorg-topology](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/reorg-topology)** to **[f641bbb](https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde)**



---

archive/issue_events_421969.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-06-29T23:08:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421969"
}
```



---

archive/issue_events_421970.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "90ee1b06556f723f021aa65b0b97b686f138cb86",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-06-29T23:08:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31925#event-421970"
}
```



---

archive/issue_comments_516639.json:
```json
{
    "body": "Changed commit from **[f641bbb](https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde)** to none",
    "created_at": "2021-07-01T04:33:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516639",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[f641bbb](https://github.com/sagemath/sagetrac-mirror/commit/f641bbb597fbf58103ed8c84867bab207a10ffde)** to none



---

archive/issue_comments_516640.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nTurns out this way of deprecating the classes is inconvenient when user code actually tries to subclass some of these classes. I think it would have been better to use `lazy_import` instead with a deprecation",
    "created_at": "2021-07-01T04:33:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31925",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31925#issuecomment-516640",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:14" align="right">Comment 14</div>

Turns out this way of deprecating the classes is inconvenient when user code actually tries to subclass some of these classes. I think it would have been better to use `lazy_import` instead with a deprecation
