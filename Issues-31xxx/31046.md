# Issue 31046: TypeError: can only concatenate str (not "bytes") to str in latte

archive/issues_030809.json:
```json
{
    "assignees": [],
    "body": "That branch is only hit sometimes (possibly random)\n\n```\n**********************************************************************\nFile \"src/sage/geometry/polyhedron/base.py\", line 7870, in sage.geometry.polyhedron.base.Polyhedron_base._volume_latte\nFailed example:\n    polytopes.cuboctahedron()._volume_latte(algorithm='cone-decompose') # optional - latte_int\nException raised:\n    Traceback (most recent call last):\n      File \"/var/lib/buildbot/slave/sage_git/build/local/lib/python3.8/site-packages/sage/doctest/forker.py\", line 714, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/var/lib/buildbot/slave/sage_git/build/local/lib/python3.8/site-packages/sage/doctest/forker.py\", line 1133, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.geometry.polyhedron.base.Polyhedron_base._volume_latte[5]>\", line 1, in <module>\n        polytopes.cuboctahedron()._volume_latte(algorithm='cone-decompose') # optional - latte_int\n      File \"/var/lib/buildbot/slave/sage_git/build/local/lib/python3.8/site-packages/sage/geometry/polyhedron/base.py\", line 7890, in _volume_latte\n        return integrate(self.cdd_Hrepresentation(), algorithm=algorithm, cdd=True, verbose=verbose, **kwargs)\n      File \"/var/lib/buildbot/slave/sage_git/build/local/lib/python3.8/site-packages/sage/interfaces/latte.py\", line 374, in integrate\n        err = \":\\n\" + err\n    TypeError: can only concatenate str (not \"bytes\") to str\n**********************************************************************\n1 item had failures:\n   1 of  10 in sage.geometry.polyhedron.base.Polyhedron_base._volume_latte\n    [1629 tests, 1 failure, 49.38 s]\n```\n\nDepends on #31130\n\nCC:  @kliem\n\n_Issue created by migration from https://trac.sagemath.org/ticket/31046_\n\n",
    "created_at": "2020-12-13T10:47:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "TypeError: can only concatenate str (not \"bytes\") to str in latte",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/31046",
    "user": "https://github.com/vbraun"
}
```
That branch is only hit sometimes (possibly random)

```
**********************************************************************
File "src/sage/geometry/polyhedron/base.py", line 7870, in sage.geometry.polyhedron.base.Polyhedron_base._volume_latte
Failed example:
    polytopes.cuboctahedron()._volume_latte(algorithm='cone-decompose') # optional - latte_int
Exception raised:
    Traceback (most recent call last):
      File "/var/lib/buildbot/slave/sage_git/build/local/lib/python3.8/site-packages/sage/doctest/forker.py", line 714, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/var/lib/buildbot/slave/sage_git/build/local/lib/python3.8/site-packages/sage/doctest/forker.py", line 1133, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.geometry.polyhedron.base.Polyhedron_base._volume_latte[5]>", line 1, in <module>
        polytopes.cuboctahedron()._volume_latte(algorithm='cone-decompose') # optional - latte_int
      File "/var/lib/buildbot/slave/sage_git/build/local/lib/python3.8/site-packages/sage/geometry/polyhedron/base.py", line 7890, in _volume_latte
        return integrate(self.cdd_Hrepresentation(), algorithm=algorithm, cdd=True, verbose=verbose, **kwargs)
      File "/var/lib/buildbot/slave/sage_git/build/local/lib/python3.8/site-packages/sage/interfaces/latte.py", line 374, in integrate
        err = ":\n" + err
    TypeError: can only concatenate str (not "bytes") to str
**********************************************************************
1 item had failures:
   1 of  10 in sage.geometry.polyhedron.base.Polyhedron_base._volume_latte
    [1629 tests, 1 failure, 49.38 s]
```

Depends on #31130

CC:  @kliem

_Issue created by migration from https://trac.sagemath.org/ticket/31046_





---

archive/issue_events_409854.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-12-13T10:47:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409854"
}
```



---

archive/issue_events_409855.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-12-13T10:47:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409855"
}
```



---

archive/issue_events_409856.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-12-13T10:47:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409856"
}
```



---

archive/issue_events_409857.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-12-13T10:47:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409857"
}
```



---

archive/issue_comments_500520.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis appears to be a two step process.\n\nFirst, the error raising of latte was never tested, see #31130.",
    "created_at": "2020-12-29T09:33:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31046#issuecomment-500520",
    "user": "https://github.com/kliem"
}
```

<div id="comment:1" align="right">Comment 1</div>

This appears to be a two step process.

First, the error raising of latte was never tested, see #31130.



---

archive/issue_comments_500521.json:
```json
{
    "body": "Dependencies: **#31130**",
    "created_at": "2020-12-29T09:57:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31046#issuecomment-500521",
    "user": "https://github.com/kliem"
}
```

Dependencies: **#31130**



---

archive/issue_events_409858.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T02:16:37Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409858"
}
```



---

archive/issue_events_409859.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-15T02:16:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409859"
}
```



---

archive/issue_events_409860.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409860"
}
```



---

archive/issue_events_409861.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409861"
}
```



---

archive/issue_events_409862.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409862"
}
```



---

archive/issue_events_409863.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409863"
}
```



---

archive/issue_events_409864.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409864"
}
```



---

archive/issue_events_409865.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409865"
}
```



---

archive/issue_events_409866.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409866"
}
```



---

archive/issue_events_409867.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31046",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31046#event-409867"
}
```
