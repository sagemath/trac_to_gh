# Issue 11971: PariError when creating a relative number field

archive/issues_011971.json:
```json
{
    "body": "Assignee: @loefflerd\n\n\n```\nsage: t = QQ['t'].gen() \nsage: K.<a> = QQ.extension(t^6 - 1/4*t^3 - 1/64)\nsage: s = K['s'].gen() \nsage: g = s^4 + 2*a^2*s^2 + a^4 + a\nsage: L.<b> = K.extension(g)\n```\n\n\nfails with PariError: inconsistent data (12)\n\nThe failure occurs in:\nrings.number_field.number_field_rel.pari_relative_polynomial().polisirreducible() \n\nIf, instead, one tries:\n\n```\nsage: L.<b> = K.extension(g, check=False)\n```\n\nit fails with PariError: (5)\n\nThis time it fails in:\npari_absolute_base_polynomial().rnfequation(self.pari_relative_polynomial(), 1)\n\nIssue created by migration from https://trac.sagemath.org/ticket/12143\n\n",
    "created_at": "2011-12-11T04:57:57Z",
    "labels": [
        "component: number fields",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "PariError when creating a relative number field",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11971",
    "user": "https://github.com/culler"
}
```
Assignee: @loefflerd


```
sage: t = QQ['t'].gen() 
sage: K.<a> = QQ.extension(t^6 - 1/4*t^3 - 1/64)
sage: s = K['s'].gen() 
sage: g = s^4 + 2*a^2*s^2 + a^4 + a
sage: L.<b> = K.extension(g)
```


fails with PariError: inconsistent data (12)

The failure occurs in:
rings.number_field.number_field_rel.pari_relative_polynomial().polisirreducible() 

If, instead, one tries:

```
sage: L.<b> = K.extension(g, check=False)
```

it fails with PariError: (5)

This time it fails in:
pari_absolute_base_polynomial().rnfequation(self.pari_relative_polynomial(), 1)

Issue created by migration from https://trac.sagemath.org/ticket/12143





---

archive/issue_comments_137554.json:
```json
{
    "body": "I see now that this is just another example of NumberField not working with polynomials that are not monic or not integral, which goes back 5 years to #252. So feel free to close the ticket.  Unfortunately I don't see any way to work around it in my project.  Too bad.",
    "created_at": "2011-12-11T05:17:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11971#issuecomment-137554",
    "user": "https://github.com/culler"
}
```

I see now that this is just another example of NumberField not working with polynomials that are not monic or not integral, which goes back 5 years to #252. So feel free to close the ticket.  Unfortunately I don't see any way to work around it in my project.  Too bad.



---

archive/issue_comments_137555.json:
```json
{
    "body": "Replying to [comment:1 culler]:\n\n> Unfortunately I don't see any way to work around it in my project.\n\nI think the following works:\n\n```\nsage: PQ.<t> = QQ[]\nsage: f = t^6 - 1/4*t^3 - 1/64\nsage: f = PQ(pari(f).polredabs())\nsage: K.<a> = QQ.extension(f)\nsage: s = K['s'].gen()\nsage: g = s^4 + 2*a^2*s^2 + a^4 + a\nsage: L.<b> = K.extension(g)\nsage: L\nNumber Field in b with defining polynomial s^4 + 2*a^2*s^2 + a^4 + a over its base field\nsage: K\nNumber Field in a with defining polynomial t^6 - 2*t^3 - 1\n```\n\n\nI think this technique could be used to sort out #252.  But there are many details to be got right.",
    "created_at": "2011-12-12T09:54:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11971#issuecomment-137555",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```

Replying to [comment:1 culler]:

> Unfortunately I don't see any way to work around it in my project.

I think the following works:

```
sage: PQ.<t> = QQ[]
sage: f = t^6 - 1/4*t^3 - 1/64
sage: f = PQ(pari(f).polredabs())
sage: K.<a> = QQ.extension(f)
sage: s = K['s'].gen()
sage: g = s^4 + 2*a^2*s^2 + a^4 + a
sage: L.<b> = K.extension(g)
sage: L
Number Field in b with defining polynomial s^4 + 2*a^2*s^2 + a^4 + a over its base field
sage: K
Number Field in a with defining polynomial t^6 - 2*t^3 - 1
```


I think this technique could be used to sort out #252.  But there are many details to be got right.



---

archive/issue_comments_137556.json:
```json
{
    "body": "Replying to [comment:2 fwclarke]:\n\nThanks!  That is very helpful.  It is not the whole story, I'm afraid, since I am working in a tower of extensions and the call pari(f) seems to fail if f does not have rational coefficients.  Still, it gets things off the ground and gives me a hint about where to look for the rest of the answer.",
    "created_at": "2011-12-12T22:32:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11971#issuecomment-137556",
    "user": "https://github.com/culler"
}
```

Replying to [comment:2 fwclarke]:

Thanks!  That is very helpful.  It is not the whole story, I'm afraid, since I am working in a tower of extensions and the call pari(f) seems to fail if f does not have rational coefficients.  Still, it gets things off the ground and gives me a hint about where to look for the rest of the answer.



---

archive/issue_comments_137557.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-06-14T13:59:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11971#issuecomment-137557",
    "user": "https://github.com/mstreng"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_137558.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-06-14T14:00:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11971#issuecomment-137558",
    "user": "https://github.com/mstreng"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_032284.json:
```json
{
    "actor": "https://github.com/mstreng",
    "created_at": "2013-06-14T14:00:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11971",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11971#event-32284"
}
```



---

archive/issue_events_032285.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-19T12:20:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11971",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11971#event-32285"
}
```



---

archive/issue_comments_137559.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2013-06-19T12:20:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11971#issuecomment-137559",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: duplicate
