# Issue 7789: Improve the arguments for the default type of a variable in MixedIntegerLinearProgram

archive/issues_007789.json:
```json
{
    "assignees": [],
    "body": "The help of `MixedIntegerLinearProgram.new_variable` shows a way to define a default type for new variables, but it uses the argument vtype and pre-defined constants (`MixedIntegerLinearProgram.__INTEGER` for example) which is really ugly.\n\nWe should accept things like :\n\n```\np.new_variable(boolean=True)\n```\nor\n\n```\np.new_variable(type=\"boolean\")\n```\n\nAssignee: **jkantor**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **Joni Syri**\n\nMerged: **sage-4.4.4.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7789_\n\n",
    "closed_at": "2010-06-07T04:51:52Z",
    "created_at": "2009-12-29T18:14:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve the arguments for the default type of a variable in MixedIntegerLinearProgram",
    "type": "issue",
    "updated_at": "2010-06-07T04:51:52Z",
    "url": "https://github.com/sagemath/sage/issues/7789",
    "user": "https://github.com/nathanncohen"
}
```
The help of `MixedIntegerLinearProgram.new_variable` shows a way to define a default type for new variables, but it uses the argument vtype and pre-defined constants (`MixedIntegerLinearProgram.__INTEGER` for example) which is really ugly.

We should accept things like :

```
p.new_variable(boolean=True)
```
or

```
p.new_variable(type="boolean")
```

Assignee: **jkantor**

Author: **Nathann Cohen**

Reviewer: **Joni Syri**

Merged: **sage-4.4.4.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/7789_





---

archive/issue_events_085001.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-29T18:14:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "milestone_number": null,
    "milestone_title": "sage-4.4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85001"
}
```



---

archive/issue_events_085002.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-29T18:14:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85002"
}
```



---

archive/issue_events_085003.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-29T18:14:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85003"
}
```



---

archive/issue_events_085004.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-29T18:14:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85004"
}
```



---

archive/issue_comments_060446.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nHere it is !",
    "created_at": "2010-01-16T16:31:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60446",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:1'>Comment 1:</a>
Here it is !



---

archive/issue_events_085005.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-01-16T16:31:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85005"
}
```



---

archive/issue_events_085006.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-03-17T05:14:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85006"
}
```



---

archive/issue_events_085007.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-03-17T05:14:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85007"
}
```



---

archive/issue_comments_060447.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI think this needs more examples.",
    "created_at": "2010-04-03T13:02:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60447",
    "user": "https://github.com/wdjoyner"
}
```

<a id='comment:3'>Comment 3:</a>
I think this needs more examples.



---

archive/issue_events_085008.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-04-03T13:02:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85008"
}
```



---

archive/issue_events_085009.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-04-03T13:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85009"
}
```



---

archive/issue_comments_060448.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI just updated the patch to add 2 lines of test .. I am sorry but I do not really know which kind of examples you expect there... :-/ \n\nPlease tell me and I'll add them immediately !!\n\nNathann",
    "created_at": "2010-04-03T19:07:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60448",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'>Comment 4:</a>
I just updated the patch to add 2 lines of test .. I am sorry but I do not really know which kind of examples you expect there... :-/ 

Please tell me and I'll add them immediately !!

Nathann



---

archive/issue_events_085010.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-03T19:07:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85010"
}
```



---

archive/issue_events_085011.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-03T19:07:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85011"
}
```



---

archive/issue_comments_060449.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-The help of MixedIntegerLinearProgram.new_variable shows a way to define a default type for new variables, but it uses the argument vtype and pre-defined constants (MixedIntegerLinearProgram.__INTEGER for example) which is really ugly.\n+The help of `MixedIntegerLinearProgram.new_variable` shows a way to define a default type for new variables, but it uses the argument vtype and pre-defined constants (`MixedIntegerLinearProgram.__INTEGER` for example) which is really ugly.\n \n We should accept things like :\n \n``````\n",
    "created_at": "2010-05-06T16:51:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60449",
    "user": "https://github.com/malb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-The help of MixedIntegerLinearProgram.new_variable shows a way to define a default type for new variables, but it uses the argument vtype and pre-defined constants (MixedIntegerLinearProgram.__INTEGER for example) which is really ugly.
+The help of `MixedIntegerLinearProgram.new_variable` shows a way to define a default type for new variables, but it uses the argument vtype and pre-defined constants (`MixedIntegerLinearProgram.__INTEGER` for example) which is really ugly.
 
 We should accept things like :
 
``````




---

archive/issue_comments_060450.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nYou current code allows `real=True` and `binary=True` and will quietly make a choice.",
    "created_at": "2010-05-06T16:52:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60450",
    "user": "https://github.com/malb"
}
```

<a id='comment:6'>Comment 6:</a>
You current code allows `real=True` and `binary=True` and will quietly make a choice.



---

archive/issue_comments_060451.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nFixed !",
    "created_at": "2010-05-06T17:15:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60451",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:7'>Comment 7:</a>
Fixed !



---

archive/issue_comments_060452.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nFew comments:\n\nI think parameter 'name' should be documented. I couldn't find out any place where changing this parameter would affect at all. Also show() method lists 'binary' type variables as boolean variables, which I find bit ugly. Fixing that might be out of scope of this ticket though, as 'binary' is used extensively in code and documentation.\n\nI'm not native English speaker so I might be wrong here, but I think this: \"An exception is raised when two types are required\" at the documentation should maybe say something like \"An exception is raised when two types are _supplied_\".",
    "created_at": "2010-05-13T20:21:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60452",
    "user": "https://github.com/sagetrac-jsyri"
}
```

<a id='comment:8'>Comment 8:</a>
Few comments:

I think parameter 'name' should be documented. I couldn't find out any place where changing this parameter would affect at all. Also show() method lists 'binary' type variables as boolean variables, which I find bit ugly. Fixing that might be out of scope of this ticket though, as 'binary' is used extensively in code and documentation.

I'm not native English speaker so I might be wrong here, but I think this: "An exception is raised when two types are required" at the documentation should maybe say something like "An exception is raised when two types are _supplied_".



---

archive/issue_events_085012.json:
```json
{
    "actor": "https://github.com/sagetrac-jsyri",
    "created_at": "2010-05-13T20:21:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85012"
}
```



---

archive/issue_events_085013.json:
```json
{
    "actor": "https://github.com/sagetrac-jsyri",
    "created_at": "2010-05-13T20:21:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85013"
}
```



---

archive/issue_events_085014.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-13T20:35:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85014"
}
```



---

archive/issue_events_085015.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-13T20:35:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85015"
}
```



---

archive/issue_comments_060453.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nSounds comments... Updated ! :-)\n\nNathann",
    "created_at": "2010-05-13T20:35:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60453",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:9'>Comment 9:</a>
Sounds comments... Updated ! :-)

Nathann



---

archive/issue_comments_060454.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nUnfortunately indentation at documentation for 'name' is off by one, so one more update required :-( Otherwise everything seems fine. I'm running doctest now, and if they come clean it'll get positive review after that last fix.",
    "created_at": "2010-05-13T21:18:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60454",
    "user": "https://github.com/sagetrac-jsyri"
}
```

<a id='comment:10'>Comment 10:</a>
Unfortunately indentation at documentation for 'name' is off by one, so one more update required :-( Otherwise everything seems fine. I'm running doctest now, and if they come clean it'll get positive review after that last fix.



---

archive/issue_events_085016.json:
```json
{
    "actor": "https://github.com/sagetrac-jsyri",
    "created_at": "2010-05-13T21:18:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85016"
}
```



---

archive/issue_events_085017.json:
```json
{
    "actor": "https://github.com/sagetrac-jsyri",
    "created_at": "2010-05-13T21:18:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85017"
}
```



---

archive/issue_events_085018.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-13T21:22:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85018"
}
```



---

archive/issue_events_085019.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-13T21:22:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85019"
}
```



---

archive/issue_comments_060455.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nAttachment: **[trac_7789.patch.gz](https://github.com/sagemath/sage/files/ticket7789/trac_7789.patch.gz)**\n\nDone.",
    "created_at": "2010-05-13T21:22:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60455",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:12'>Comment 12:</a>
Attachment: **[trac_7789.patch.gz](https://github.com/sagemath/sage/files/ticket7789/trac_7789.patch.gz)**

Done.



---

archive/issue_events_085020.json:
```json
{
    "actor": "https://github.com/sagetrac-jsyri",
    "created_at": "2010-05-14T07:53:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85020"
}
```



---

archive/issue_events_085021.json:
```json
{
    "actor": "https://github.com/sagetrac-jsyri",
    "created_at": "2010-05-14T07:53:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85021"
}
```



---

archive/issue_comments_060456.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nEverything seems to be in order. Positive review it is.",
    "created_at": "2010-05-14T07:53:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60456",
    "user": "https://github.com/sagetrac-jsyri"
}
```

<a id='comment:13'>Comment 13:</a>
Everything seems to be in order. Positive review it is.



---

archive/issue_comments_060457.json:
```json
{
    "body": "Reviewer: **Joni Syri**",
    "created_at": "2010-06-07T04:51:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60457",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Joni Syri**



---

archive/issue_comments_060458.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2010-06-07T04:51:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60458",
    "user": "https://github.com/mwhansen"
}
```

Author: **Nathann Cohen**



---

archive/issue_comments_060459.json:
```json
{
    "body": "Merged: **sage-4.4.4.alpha0**",
    "created_at": "2010-06-07T04:51:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7789#issuecomment-60459",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.4.4.alpha0**



---

archive/issue_events_085022.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-07T04:51:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85022"
}
```



---

archive/issue_events_085023.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-07T04:51:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7789",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7789#event-85023"
}
```
