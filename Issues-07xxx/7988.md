# Issue 7988: add testcc and testcxx to hgignore in scripts repo

archive/issues_007988.json:
```json
{
    "body": "\n\nAssignee: @aghitza\n\nResolution: fixed\n\nAuthor: Willem Jan Palenstijn\n\nReviewer: Robert Miller\n\nMerged: sage-4.3.1.rc1\n\nIssue created by migration from https://trac.sagemath.org/ticket/7988\n\n",
    "closed_at": "2010-01-19T05:07:05Z",
    "created_at": "2010-01-19T02:08:41Z",
    "labels": [
        "component: distribution",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.3.1",
    "title": "add testcc and testcxx to hgignore in scripts repo",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7988",
    "user": "https://github.com/rlmill"
}
```


Assignee: @aghitza

Resolution: fixed

Author: Willem Jan Palenstijn

Reviewer: Robert Miller

Merged: sage-4.3.1.rc1

Issue created by migration from https://trac.sagemath.org/ticket/7988





---

archive/issue_comments_081605.json:
```json
{
    "body": "Attachment [scripts_7988_hgignore.patch](tarball://root/attachments/some-uuid/ticket7988/scripts_7988_hgignore.patch) by @wjp created at 2010-01-19 02:10:31",
    "created_at": "2010-01-19T02:10:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81605",
    "user": "https://github.com/wjp"
}
```

Attachment [scripts_7988_hgignore.patch](tarball://root/attachments/some-uuid/ticket7988/scripts_7988_hgignore.patch) by @wjp created at 2010-01-19 02:10:31



---

archive/issue_comments_081606.json:
```json
{
    "body": "<a id='comment:1'></a>Trivial patch for the scripts repo attached.",
    "created_at": "2010-01-19T02:10:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81606",
    "user": "https://github.com/wjp"
}
```

<a id='comment:1'></a>Trivial patch for the scripts repo attached.



---

archive/issue_comments_081607.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-01-19T02:10:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81607",
    "user": "https://github.com/wjp"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_081608.json:
```json
{
    "body": "Changing author from \"\" to \"Willem Jan Palenstijn\"",
    "created_at": "2010-01-19T02:43:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81608",
    "user": "https://github.com/rlmill"
}
```

Changing author from "" to "Willem Jan Palenstijn"



---

archive/issue_comments_081609.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2010-01-19T02:43:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81609",
    "user": "https://github.com/rlmill"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_081610.json:
```json
{
    "body": "<a id='comment:2'></a>finally!",
    "created_at": "2010-01-19T02:43:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81610",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:2'></a>finally!



---

archive/issue_comments_081611.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Robert Miller\"",
    "created_at": "2010-01-19T02:43:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81611",
    "user": "https://github.com/rlmill"
}
```

Changing reviewer from "" to "Robert Miller"



---

archive/issue_comments_081612.json:
```json
{
    "body": "Changing component from algebra to distribution.",
    "created_at": "2010-01-19T04:45:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81612",
    "user": "https://github.com/aghitza"
}
```

Changing component from algebra to distribution.



---

archive/issue_events_019121.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-19T04:45:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "milestone": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7988#event-19121"
}
```



---

archive/issue_events_019122.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-01-19T05:07:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7988#event-19122"
}
```



---

archive/issue_comments_081613.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.3.1.rc1\"",
    "created_at": "2010-01-19T05:07:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81613",
    "user": "https://github.com/rlmill"
}
```

Changing merged from "" to "sage-4.3.1.rc1"



---

archive/issue_comments_081614.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-01-19T05:07:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7988#issuecomment-81614",
    "user": "https://github.com/rlmill"
}
```

Resolution: fixed
