# Issue 7839: Failure to coerce q^(-1) into its own LaurentPolynomialRing

archive/issues_007839.json:
```json
{
    "body": "Consider:\n\n```\nsage: P.<q> = LaurentPolynomialRing(QQ)\nsage: q in P\nTrue\nsage: P(q)\nq\nsage: q^(-1) in P\nTrue\nsage: P(q^(-1))\n```\n\nThe last statement raises an exception.\n\n**Assignee:** @aghitza\n\n**CC:**  sage-combinat\n\nIssue created by migration from https://trac.sagemath.org/ticket/7839\n\n",
    "closed_at": "2010-01-19T21:55:38Z",
    "created_at": "2010-01-04T04:47:29Z",
    "labels": [
        "component: algebra",
        "bug",
        "duplicate"
    ],
    "title": "Failure to coerce q^(-1) into its own LaurentPolynomialRing",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7839",
    "user": "https://github.com/dwbump"
}
```
Consider:

```
sage: P.<q> = LaurentPolynomialRing(QQ)
sage: q in P
True
sage: P(q)
q
sage: q^(-1) in P
True
sage: P(q^(-1))
```

The last statement raises an exception.

**Assignee:** @aghitza

**CC:**  sage-combinat

Issue created by migration from https://trac.sagemath.org/ticket/7839





---

archive/issue_comments_062699.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is a duplicate of #3617 which should be fixed soon.",
    "created_at": "2010-01-19T21:55:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7839#issuecomment-62699",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
This is a duplicate of #3617 which should be fixed soon.



---

archive/issue_events_055435.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-19T21:55:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7839",
    "milestone": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7839#event-55435"
}
```



---

archive/issue_events_055436.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-19T21:55:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/7839",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7839#event-55436"
}
```



---

archive/issue_events_055437.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-19T21:55:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7839",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7839#event-55437"
}
```



---

archive/issue_events_055438.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-19T21:55:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/7839",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7839#event-55438"
}
```
