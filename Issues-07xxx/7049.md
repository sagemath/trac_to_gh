# Issue 7049: GLPK, just minor changes

archive/issues_007049.json:
```json
{
    "assignees": [],
    "body": "Hello !!!\n\nBecause of #7012, some functions in numerical.mip had to be renamed, and this package had to be modified as it uses them.\n\nThese are the only changes contained in the package located on : ~ncohen/glpk-4.38.p2.spkg (sagemath).\n\nCC:  @sagetrac-mvngu\n\nComponent: **packages: optional**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **Minh Van Nguyen**\n\nMerged: **sage-4.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7049_\n\n",
    "closed_at": "2009-10-23T09:09:51Z",
    "created_at": "2009-09-28T09:29:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "GLPK, just minor changes",
    "type": "issue",
    "updated_at": "2009-10-23T09:09:51Z",
    "url": "https://github.com/sagemath/sage/issues/7049",
    "user": "https://github.com/nathanncohen"
}
```
Hello !!!

Because of #7012, some functions in numerical.mip had to be renamed, and this package had to be modified as it uses them.

These are the only changes contained in the package located on : ~ncohen/glpk-4.38.p2.spkg (sagemath).

CC:  @sagetrac-mvngu

Component: **packages: optional**

Author: **Nathann Cohen**

Reviewer: **Minh Van Nguyen**

Merged: **sage-4.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/7049_





---

archive/issue_events_085797.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-28T09:29:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85797"
}
```



---

archive/issue_events_085798.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-28T09:29:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "c: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85798"
}
```



---

archive/issue_events_085799.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-28T09:29:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85799"
}
```



---

archive/issue_events_085800.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-28T09:29:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85800"
}
```



---

archive/issue_events_085801.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-28T09:29:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85801"
}
```



---

archive/issue_comments_050605.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI just updated the SPKG. In ticket #7012, variable sense had been changed to maximization, and it had to be mentionned here.",
    "created_at": "2009-10-02T16:24:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7049#issuecomment-50605",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:1" align="right">comment:1</div>

I just updated the SPKG. In ticket #7012, variable sense had been changed to maximization, and it had to be mentionned here.



---

archive/issue_comments_050606.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2009-10-21T22:31:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7049#issuecomment-50606",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Nathann Cohen**



---

archive/issue_events_085802.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-10-21T22:31:59Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "title_is": "GLPK, just minor changes",
    "title_was": "[with SPKG] GLPK, just minor changes",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85802"
}
```



---

archive/issue_comments_050607.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2009-10-21T22:31:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7049#issuecomment-50607",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_events_085803.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-10-21T22:31:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85803"
}
```



---

archive/issue_events_085804.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-10-21T22:31:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85804"
}
```



---

archive/issue_comments_050608.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good. This should be merged in the optional packages repository.",
    "created_at": "2009-10-21T22:31:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7049#issuecomment-50608",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good. This should be merged in the optional packages repository.



---

archive/issue_events_085805.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-23T09:09:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85805"
}
```



---

archive/issue_events_085806.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-23T09:09:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7049#event-85806"
}
```



---

archive/issue_comments_050609.json:
```json
{
    "body": "Merged: **sage-4.2.alpha1**",
    "created_at": "2009-10-23T09:09:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7049#issuecomment-50609",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.alpha1**
