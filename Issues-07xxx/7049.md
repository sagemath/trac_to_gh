# Issue 7049: GLPK, just minor changes

archive/issues_007049.json:
```json
{
    "body": "Hello !!!\n\nBecause of #7012, some functions in numerical.mip had to be renamed, and this package had to be modified as it uses them.\n\nThese are the only changes contained in the package located on : ~ncohen/glpk-4.38.p2.spkg (sagemath).\n\nAssignee: tbd\n\nCC:  mvngu\n\nResolution: fixed\n\nAuthor: Nathann Cohen\n\nReviewer: Minh Van Nguyen\n\nMerged: sage-4.2.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/7049\n\n",
    "closed_at": "2009-10-23T09:09:51Z",
    "created_at": "2009-09-28T09:29:21Z",
    "labels": [
        "component: packages: optional",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.2",
    "title": "GLPK, just minor changes",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7049",
    "user": "https://github.com/nathanncohen"
}
```
Hello !!!

Because of #7012, some functions in numerical.mip had to be renamed, and this package had to be modified as it uses them.

These are the only changes contained in the package located on : ~ncohen/glpk-4.38.p2.spkg (sagemath).

Assignee: tbd

CC:  mvngu

Resolution: fixed

Author: Nathann Cohen

Reviewer: Minh Van Nguyen

Merged: sage-4.2.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/7049





---

archive/issue_comments_066960.json:
```json
{
    "body": "<a id='comment:1'></a>I just updated the SPKG. In ticket #7012, variable sense had been changed to maximization, and it had to be mentionned here.",
    "created_at": "2009-10-02T16:24:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7049#issuecomment-66960",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:1'></a>I just updated the SPKG. In ticket #7012, variable sense had been changed to maximization, and it had to be mentionned here.



---

archive/issue_comments_066961.json:
```json
{
    "body": "Changing author from \"\" to \"Nathann Cohen\"",
    "created_at": "2009-10-21T22:31:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7049#issuecomment-66961",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing author from "" to "Nathann Cohen"



---

archive/issue_comments_066962.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Minh Van Nguyen\"",
    "created_at": "2009-10-21T22:31:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7049#issuecomment-66962",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing reviewer from "" to "Minh Van Nguyen"



---

archive/issue_comments_066963.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2009-10-21T22:31:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7049#issuecomment-66963",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_066964.json:
```json
{
    "body": "<a id='comment:2'></a>Looks good. This should be merged in the optional packages repository.",
    "created_at": "2009-10-21T22:31:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7049#issuecomment-66964",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:2'></a>Looks good. This should be merged in the optional packages repository.



---

archive/issue_events_016618.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-23T09:09:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7049",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7049#event-16618"
}
```



---

archive/issue_comments_066965.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-10-23T09:09:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7049#issuecomment-66965",
    "user": "https://github.com/mwhansen"
}
```

Resolution: fixed



---

archive/issue_comments_066966.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.2.alpha1\"",
    "created_at": "2009-10-23T09:09:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7049#issuecomment-66966",
    "user": "https://github.com/mwhansen"
}
```

Changing merged from "" to "sage-4.2.alpha1"
