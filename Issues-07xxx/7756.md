# Issue 7756: Extra, unwanted text inserted in cells by shift-enter

archive/issues_007756.json:
```json
{
    "assignees": [],
    "body": "See [sage-notebook](http://groups.google.com/group/sage-notebook/browse_thread/thread/95aa96ca848ef135) and [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/47f6756f00df1f72/6f3dd9ce745832e4?#6f3dd9ce745832e4).\n\nAssignee: **@williamstein**\n\nAuthor: **Mitesh Patel**\n\nReviewer: **William Stein**\n\nMerged: **sagenb-0.4.8**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7756_\n\n",
    "closed_at": "2009-12-24T06:56:16Z",
    "created_at": "2009-12-24T06:18:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Extra, unwanted text inserted in cells by shift-enter",
    "type": "issue",
    "updated_at": "2009-12-29T14:40:40Z",
    "url": "https://github.com/sagemath/sage/issues/7756",
    "user": "https://github.com/qed777"
}
```
See [sage-notebook](http://groups.google.com/group/sage-notebook/browse_thread/thread/95aa96ca848ef135) and [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/47f6756f00df1f72/6f3dd9ce745832e4?#6f3dd9ce745832e4).

Assignee: **@williamstein**

Author: **Mitesh Patel**

Reviewer: **William Stein**

Merged: **sagenb-0.4.8**

_Issue created by migration from https://trac.sagemath.org/ticket/7756_





---

archive/issue_events_084595.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-12-24T06:18:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7756#event-84595"
}
```



---

archive/issue_events_084596.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-12-24T06:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7756#event-84596"
}
```



---

archive/issue_events_084597.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-12-24T06:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7756#event-84597"
}
```



---

archive/issue_events_084598.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-12-24T06:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7756#event-84598"
}
```



---

archive/issue_comments_060057.json:
```json
{
    "body": "Minimal version ripped from #7666. sagenb repo.",
    "created_at": "2009-12-24T06:22:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7756#issuecomment-60057",
    "user": "https://github.com/qed777"
}
```

Minimal version ripped from #7666. sagenb repo.



---

archive/issue_comments_060058.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_7756-extra_text_shift_enter.patch.gz](https://github.com/sagemath/sage/files/ticket7756/trac_7756-extra_text_shift_enter.patch.gz)**\n\nThe problem seems to be that the auto-indent code \"fires\" too often, e.g., when the shift key comes up before the enter key.  The [patch](https://github.com/sagemath/sage/files/ticket7756/trac_7756-extra_text_shift_enter.patch.gz) should fix this in Cr, FF, S, and O.  If it does not, please let me know!\n\nNote: I've extracted the patch here from the patch at #7666.",
    "created_at": "2009-12-24T06:32:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7756#issuecomment-60058",
    "user": "https://github.com/qed777"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_7756-extra_text_shift_enter.patch.gz](https://github.com/sagemath/sage/files/ticket7756/trac_7756-extra_text_shift_enter.patch.gz)**

The problem seems to be that the auto-indent code "fires" too often, e.g., when the shift key comes up before the enter key.  The [patch](https://github.com/sagemath/sage/files/ticket7756/trac_7756-extra_text_shift_enter.patch.gz) should fix this in Cr, FF, S, and O.  If it does not, please let me know!

Note: I've extracted the patch here from the patch at #7666.



---

archive/issue_comments_060059.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis does indeed seem to fix the problem for me.  Subtle.\n\nMerged into sagenb-0.4.8.",
    "created_at": "2009-12-24T06:56:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7756#issuecomment-60059",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'>Comment 2:</a>
This does indeed seem to fix the problem for me.  Subtle.

Merged into sagenb-0.4.8.



---

archive/issue_events_084599.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-24T06:56:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7756#event-84599"
}
```



---

archive/issue_comments_060060.json:
```json
{
    "body": "Merged: **sagenb-0.4.8**",
    "created_at": "2009-12-29T14:40:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7756#issuecomment-60060",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sagenb-0.4.8**



---

archive/issue_comments_060061.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2009-12-29T14:40:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7756#issuecomment-60061",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **William Stein**
