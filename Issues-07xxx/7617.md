# Issue 7617: include sagetex as a standard spkg

archive/issues_007617.json:
```json
{
    "assignees": [],
    "body": "From this [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/4c192cdc748c9b1c) thread, SageTeX has been voted to be a standard Sage spkg. The official maintainer for the \"next few years\" is Dan Drake.\n\nCC:  @dandrake @jhpalmieri\n\nComponent: **packages: standard**\n\nAuthor: **Dan Drake**\n\nReviewer: **William Stein, Mike Hansen, John Palmieri**\n\nMerged: **sage-4.3.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7617_\n\n",
    "closed_at": "2010-01-31T00:02:41Z",
    "created_at": "2009-12-07T22:20:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "include sagetex as a standard spkg",
    "type": "issue",
    "updated_at": "2010-03-07T13:20:15Z",
    "url": "https://github.com/sagemath/sage/issues/7617",
    "user": "https://github.com/williamstein"
}
```
From this [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/4c192cdc748c9b1c) thread, SageTeX has been voted to be a standard Sage spkg. The official maintainer for the "next few years" is Dan Drake.

CC:  @dandrake @jhpalmieri

Component: **packages: standard**

Author: **Dan Drake**

Reviewer: **William Stein, Mike Hansen, John Palmieri**

Merged: **sage-4.3.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/7617_





---

archive/issue_events_093019.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-07T22:20:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "milestone_number": null,
    "milestone_title": "sage-4.3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93019"
}
```



---

archive/issue_events_093020.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-07T22:20:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93020"
}
```



---

archive/issue_events_093021.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-07T22:20:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93021"
}
```



---

archive/issue_events_093022.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-07T22:20:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93022"
}
```



---

archive/issue_comments_056721.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1 @@\n-sagetex has been voted in to be a standard sage spkg. \n-\n-the official maintainer for the \"next few years\" is Dan Drake.\n+From this [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/4c192cdc748c9b1c) thread, SageTeX has been voted to be a standard Sage spkg. The official maintainer for the \"next few years\" is Dan Drake.\n``````\n",
    "created_at": "2009-12-07T22:38:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56721",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1 @@
-sagetex has been voted in to be a standard sage spkg. 
-
-the official maintainer for the "next few years" is Dan Drake.
+From this [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/4c192cdc748c9b1c) thread, SageTeX has been voted to be a standard Sage spkg. The official maintainer for the "next few years" is Dan Drake.
``````




---

archive/issue_comments_056722.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI don't know if this should be a separate ticket for the upgrade, but the current spkg on the website is 2.1.1, but I have a spkg for version 2.2.1 prepared: http://sage.math.washington.edu/home/drake/code/sage/st/sagetex-2.2.1.spkg",
    "created_at": "2009-12-08T02:57:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56722",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:3" align="right">comment:3</div>

I don't know if this should be a separate ticket for the upgrade, but the current spkg on the website is 2.1.1, but I have a spkg for version 2.2.1 prepared: http://sage.math.washington.edu/home/drake/code/sage/st/sagetex-2.2.1.spkg



---

archive/issue_events_093023.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-08T07:37:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93023"
}
```



---

archive/issue_comments_056723.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nHi,\n\nI'm curious how users will even *know* to do this:\n\n```\n:To use SageTeX, both Sage and LaTeX need to know about it. If you have\ninstalled this spkg, then Sage knows about SageTeX; now you need to make\nLaTeX aware of it. The easiest thing to do is to ...\n```\n\nthe only reason I know to do the things listed there so I can use SageTex is because I happen to have looked in the spkg after installing it and was looking at SPKG.txt.   When this package is standard in Sage, how will users know how to use and set it up?  Binaries won't even include that SPKG.txt file, since SAGE binaries don't include the contents of any spkg files. \n\nOne solution, which I think would be extremely appropriate, is for you to add a new section to the Sage tutorial explaining how to use and get going with SageTex.  It's such a killer feature, that I think this is reasonable and worth it.    Let's make SageTex one of the centerpieces of Sage!",
    "created_at": "2009-12-08T07:42:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56723",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:5" align="right">comment:5</div>

Hi,

I'm curious how users will even *know* to do this:

```
:To use SageTeX, both Sage and LaTeX need to know about it. If you have
installed this spkg, then Sage knows about SageTeX; now you need to make
LaTeX aware of it. The easiest thing to do is to ...
```

the only reason I know to do the things listed there so I can use SageTex is because I happen to have looked in the spkg after installing it and was looking at SPKG.txt.   When this package is standard in Sage, how will users know how to use and set it up?  Binaries won't even include that SPKG.txt file, since SAGE binaries don't include the contents of any spkg files. 

One solution, which I think would be extremely appropriate, is for you to add a new section to the Sage tutorial explaining how to use and get going with SageTex.  It's such a killer feature, that I think this is reasonable and worth it.    Let's make SageTex one of the centerpieces of Sage!



---

archive/issue_events_093024.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-08T07:42:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93024"
}
```



---

archive/issue_events_093025.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-08T07:42:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93025"
}
```



---

archive/issue_comments_056724.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nHi,\n\nWhy not make all this config stuff automatic?\n\n\"For Linux/Unix users, do\n\n```\n  cp -r $SAGE_ROOT/local/share/texmf/ $HOME/texmf/\n```\nFor OS X users with MacTeX:\n\n```\n  cp -r $SAGE_ROOT/local/share/texmf/ $HOME/Library/texmf/\n```\nAfter you've copied over the necessary files, you'll need to update\nTeX's database so that it can find them. Run \"`texhash $HOME/texmf`\"\n(replace the texmf directory as appropriate) to do this.\"\n\nI don't think the above is as easy as it could be...  I like to make things really easy, e.g., setting up a twisted webserver to server the Sage notebook app is `notebook()`.",
    "created_at": "2009-12-08T07:47:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56724",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:6" align="right">comment:6</div>

Hi,

Why not make all this config stuff automatic?

"For Linux/Unix users, do

```
  cp -r $SAGE_ROOT/local/share/texmf/ $HOME/texmf/
```
For OS X users with MacTeX:

```
  cp -r $SAGE_ROOT/local/share/texmf/ $HOME/Library/texmf/
```
After you've copied over the necessary files, you'll need to update
TeX's database so that it can find them. Run "`texhash $HOME/texmf`"
(replace the texmf directory as appropriate) to do this."

I don't think the above is as easy as it could be...  I like to make things really easy, e.g., setting up a twisted webserver to server the Sage notebook app is `notebook()`.



---

archive/issue_comments_056725.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@williamstein](#comment%3A6):\n> Why not make all this config stuff automatic?\n> \n> \"For Linux/Unix users, do\n> \n> ```\n>   cp -r $SAGE_ROOT/local/share/texmf/ $HOME/texmf/\n> ```\n> For OS X users with MacTeX:\n> \n> ```\n>   cp -r $SAGE_ROOT/local/share/texmf/ $HOME/Library/texmf/\n> ```\n> After you've copied over the necessary files, you'll need to update\n> TeX's database so that it can find them. Run \"`texhash $HOME/texmf`\"\n> (replace the texmf directory as appropriate) to do this.\"\n\nOne problem that I see is that there are different TeX distributions, and not all of them may automatically look in home directories for a texmf/ tree. Some users may also not want the default installation of Sage (which will now install SageTeX) to create those directories and files in their $HOME. \n\n> I don't think the above is as easy as it could be...  I like to make things really easy, e.g., setting up a twisted webserver to server the Sage notebook app is `notebook()`.\n\nI agree, it's not very simple, but we are now interacting not only with Sage (which is a large software system) but with TeX, another large -- and quite different -- software system. And unlike Sage, a TeX installation is supposed to be very stable and work the same way for a long, long time -- so I'm guessing we could run into very old systems.\n\nI'll look and see if there's a reasonably effective way to autodetect if $HOME/texmf will get picked up by TeX. I also like the idea of putting some basic installation instructions and pointers to the rest of the documentation into the tutorial.",
    "created_at": "2009-12-08T08:20:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56725",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@williamstein](#comment%3A6):
> Why not make all this config stuff automatic?
> 
> "For Linux/Unix users, do
> 
> ```
>   cp -r $SAGE_ROOT/local/share/texmf/ $HOME/texmf/
> ```
> For OS X users with MacTeX:
> 
> ```
>   cp -r $SAGE_ROOT/local/share/texmf/ $HOME/Library/texmf/
> ```
> After you've copied over the necessary files, you'll need to update
> TeX's database so that it can find them. Run "`texhash $HOME/texmf`"
> (replace the texmf directory as appropriate) to do this."

One problem that I see is that there are different TeX distributions, and not all of them may automatically look in home directories for a texmf/ tree. Some users may also not want the default installation of Sage (which will now install SageTeX) to create those directories and files in their $HOME. 

> I don't think the above is as easy as it could be...  I like to make things really easy, e.g., setting up a twisted webserver to server the Sage notebook app is `notebook()`.

I agree, it's not very simple, but we are now interacting not only with Sage (which is a large software system) but with TeX, another large -- and quite different -- software system. And unlike Sage, a TeX installation is supposed to be very stable and work the same way for a long, long time -- so I'm guessing we could run into very old systems.

I'll look and see if there's a reasonably effective way to autodetect if $HOME/texmf will get picked up by TeX. I also like the idea of putting some basic installation instructions and pointers to the rest of the documentation into the tutorial.



---

archive/issue_comments_056726.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nReplying to [@dandrake](#comment%3A7):\n> And unlike Sage, a TeX installation is supposed to be very stable and work the same way for a long, long time\n\nOops, I don't mean to say that Sage isn't supposed to be stable! Just that TeX moves very slowly, in stark contrast to Sage.",
    "created_at": "2009-12-08T08:35:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56726",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:8" align="right">comment:8</div>

Replying to [@dandrake](#comment%3A7):
> And unlike Sage, a TeX installation is supposed to be very stable and work the same way for a long, long time

Oops, I don't mean to say that Sage isn't supposed to be stable! Just that TeX moves very slowly, in stark contrast to Sage.



---

archive/issue_comments_056727.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nI see your points above.  I would like to see some specific plan though for making how to install SageTex be clearly documented with Sage (e.g., via the tutorial).  Otherwise, I don't understand how any non-super-advanced-and-lucky-user will figure out how to use it.",
    "created_at": "2009-12-09T14:12:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56727",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:9" align="right">comment:9</div>

I see your points above.  I would like to see some specific plan though for making how to install SageTex be clearly documented with Sage (e.g., via the tutorial).  Otherwise, I don't understand how any non-super-advanced-and-lucky-user will figure out how to use it.



---

archive/issue_comments_056728.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nHi,\n\nI've updated the optional sagetex spkg, since I tried it and it looks OK.\n\nWilliam",
    "created_at": "2009-12-09T14:13:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56728",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:10" align="right">comment:10</div>

Hi,

I've updated the optional sagetex spkg, since I tried it and it looks OK.

William



---

archive/issue_comments_056729.json:
```json
{
    "body": "a draft of additions to the installation guide",
    "created_at": "2009-12-11T09:31:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56729",
    "user": "https://github.com/dandrake"
}
```

a draft of additions to the installation guide



---

archive/issue_comments_056730.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nAttachment: **[sample_for_installation_guide.patch.gz](https://github.com/sagemath/sage/files/ticket7617/sample_for_installation_guide.patch.gz)**\n\nI've attached a draft of what we might add to the installation guide to help users, well, install SageTeX. I'd also like to add something to the reference manual, as users might after installation want to use SageTeX and not think of \"installing\" anything new (since their friends will tell them it's included in Sage). Mostly, I think we need to put pointers to the SageTeX documentation in several places. Where else should we mention it in the documentation?",
    "created_at": "2009-12-11T09:35:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56730",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:11" align="right">comment:11</div>

Attachment: **[sample_for_installation_guide.patch.gz](https://github.com/sagemath/sage/files/ticket7617/sample_for_installation_guide.patch.gz)**

I've attached a draft of what we might add to the installation guide to help users, well, install SageTeX. I'd also like to add something to the reference manual, as users might after installation want to use SageTeX and not think of "installing" anything new (since their friends will tell them it's included in Sage). Mostly, I think we need to put pointers to the SageTeX documentation in several places. Where else should we mention it in the documentation?



---

archive/issue_comments_056731.json:
```json
{
    "body": "Attachment: **[trac_7617.patch.gz](https://github.com/sagemath/sage/files/ticket7617/trac_7617.patch.gz)**\n\nadditions to the installation guide and tutorial",
    "created_at": "2009-12-22T08:23:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56731",
    "user": "https://github.com/dandrake"
}
```

Attachment: **[trac_7617.patch.gz](https://github.com/sagemath/sage/files/ticket7617/trac_7617.patch.gz)**

additions to the installation guide and tutorial



---

archive/issue_comments_056732.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThe attached patch (attachment:trac_7617.patch) adds instructions and basic help for installing and using SageTeX to the installation guide and tutorial.",
    "created_at": "2009-12-22T08:26:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56732",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:12" align="right">comment:12</div>

The attached patch (attachment:trac_7617.patch) adds instructions and basic help for installing and using SageTeX to the installation guide and tutorial.



---

archive/issue_events_093026.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2009-12-22T08:26:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93026"
}
```



---

archive/issue_events_093027.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2009-12-22T08:26:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93027"
}
```



---

archive/issue_comments_056733.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nI'm also going to change the section of the SageTeX documentation that discusses installation, since obviously that will now be different. That will involve a new spkg. Can/Should that be part of this ticket, or should it be a different one?",
    "created_at": "2009-12-23T00:54:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56733",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:13" align="right">comment:13</div>

I'm also going to change the section of the SageTeX documentation that discusses installation, since obviously that will now be different. That will involve a new spkg. Can/Should that be part of this ticket, or should it be a different one?



---

archive/issue_comments_056734.json:
```json
{
    "body": "Author: **Dan Drake**",
    "created_at": "2009-12-23T00:54:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56734",
    "user": "https://github.com/dandrake"
}
```

Author: **Dan Drake**



---

archive/issue_events_093028.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-24T20:15:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93028"
}
```



---

archive/issue_events_093029.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-24T20:15:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
    "label_color": "ff7700",
    "label_name": "p: critical / 2",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93029"
}
```



---

archive/issue_comments_056735.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nI've just read through all the patches and they look excellent.  I didn't test that the sphinx is valid or that the sagetex example actually works.  If somebody verifies that, then positive review. \n\n> Can/Should that be part of this ticket, or should it be a different one? \n\nIt would make sense to make it part of this one. \n\nI'm changing the priority on this to critical for sage-4.3.1. \n\n -- William",
    "created_at": "2009-12-24T20:15:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56735",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:14" align="right">comment:14</div>

I've just read through all the patches and they look excellent.  I didn't test that the sphinx is valid or that the sagetex example actually works.  If somebody verifies that, then positive review. 

> Can/Should that be part of this ticket, or should it be a different one? 

It would make sense to make it part of this one. 

I'm changing the priority on this to critical for sage-4.3.1. 

 -- William



---

archive/issue_comments_056736.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nThe markup in the patch looks good.",
    "created_at": "2009-12-27T18:44:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56736",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:15" align="right">comment:15</div>

The markup in the patch looks good.



---

archive/issue_events_093030.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-27T18:44:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93030"
}
```



---

archive/issue_events_093031.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-27T18:44:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93031"
}
```



---

archive/issue_comments_056737.json:
```json
{
    "body": "Reviewer: **William Stein, Mike Hansen**",
    "created_at": "2009-12-27T18:44:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56737",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **William Stein, Mike Hansen**



---

archive/issue_comments_056738.json:
```json
{
    "body": "Attachment: **[trac_7617.2.patch.gz](https://github.com/sagemath/sage/files/ticket7617/trac_7617.2.patch.gz)**\n\nadditions to the installation guide and tutorial",
    "created_at": "2009-12-28T08:34:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56738",
    "user": "https://github.com/dandrake"
}
```

Attachment: **[trac_7617.2.patch.gz](https://github.com/sagemath/sage/files/ticket7617/trac_7617.2.patch.gz)**

additions to the installation guide and tutorial



---

archive/issue_comments_056739.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nHrm, how can I switch this back to \"needs review\"? I added a bit on using TEXINPUTS.\n\nI'm working on a new spkg that will have an updated installation section.",
    "created_at": "2009-12-28T08:36:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56739",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:16" align="right">comment:16</div>

Hrm, how can I switch this back to "needs review"? I added a bit on using TEXINPUTS.

I'm working on a new spkg that will have an updated installation section.



---

archive/issue_events_093032.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-12-28T09:31:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93032"
}
```



---

archive/issue_events_093033.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-12-28T09:31:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93033"
}
```



---

archive/issue_comments_056740.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nYou have to make it \"needs work\" first.",
    "created_at": "2009-12-28T09:31:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56740",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:18" align="right">comment:18</div>

You have to make it "needs work" first.



---

archive/issue_events_093034.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-12-28T09:31:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93034"
}
```



---

archive/issue_events_093035.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-12-28T09:31:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93035"
}
```



---

archive/issue_comments_056741.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\nThanks for the help, Alex. Along with the patch [attachment: trac_7617.2.patch,](https://github.com/sagemath/sage/files/ticket7617/a27be8dc415d2fe1a02cb9a74b60e5ef.gz) I have a new spkg that reflects the new status as a standard spkg, available from http://sagenb.kaist.ac.kr/~drake/sagetex-2.2.3.spkg. Those two things need review.",
    "created_at": "2009-12-30T07:02:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56741",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:19" align="right">comment:19</div>

Thanks for the help, Alex. Along with the patch [attachment: trac_7617.2.patch,](https://github.com/sagemath/sage/files/ticket7617/a27be8dc415d2fe1a02cb9a74b60e5ef.gz) I have a new spkg that reflects the new status as a standard spkg, available from http://sagenb.kaist.ac.kr/~drake/sagetex-2.2.3.spkg. Those two things need review.



---

archive/issue_comments_056742.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nA few comments: overall, the documentation patch is good, but unfortunately it needs to be rebased.  Since you have to do that, I wonder if, in the reference manual, the file sagetex.rst needs to be in the new directory \"other\", or whether it should just be in the top ref manual directory.  Not a big deal.\n\nAlso, is it worth mentioning in the documentation that if you have several copies of sagetex.sty lying around, you need to make sure that the most recent one gets read first?  I just ran into this problem because I had installed an old version a while ago which was shadowing the more recent one.  Again, not a big deal, and if you think this should be done but don't want to bother now, it can be delayed to another ticket.\n\nFor a future version of sagetex, should sagetex print its version each time you latex the file, so that users can see what version is actually being used?\n\nThe spkg looks good, and seems to indicate in the right places that it's now a standard spkg.  Does anything else need to be checked?",
    "created_at": "2010-01-21T04:04:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56742",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:21" align="right">comment:21</div>

A few comments: overall, the documentation patch is good, but unfortunately it needs to be rebased.  Since you have to do that, I wonder if, in the reference manual, the file sagetex.rst needs to be in the new directory "other", or whether it should just be in the top ref manual directory.  Not a big deal.

Also, is it worth mentioning in the documentation that if you have several copies of sagetex.sty lying around, you need to make sure that the most recent one gets read first?  I just ran into this problem because I had installed an old version a while ago which was shadowing the more recent one.  Again, not a big deal, and if you think this should be done but don't want to bother now, it can be delayed to another ticket.

For a future version of sagetex, should sagetex print its version each time you latex the file, so that users can see what version is actually being used?

The spkg looks good, and seems to indicate in the right places that it's now a standard spkg.  Does anything else need to be checked?



---

archive/issue_comments_056743.json:
```json
{
    "body": "Changed reviewer from **William Stein, Mike Hansen** to **William Stein, Mike Hansen, John Palmieri**",
    "created_at": "2010-01-21T04:04:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56743",
    "user": "https://github.com/jhpalmieri"
}
```

Changed reviewer from **William Stein, Mike Hansen** to **William Stein, Mike Hansen, John Palmieri**



---

archive/issue_events_093036.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-01-21T04:04:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93036"
}
```



---

archive/issue_events_093037.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-01-21T04:04:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93037"
}
```



---

archive/issue_comments_056744.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nReplying to [@jhpalmieri](#comment%3A21):\n> A few comments: overall, the documentation patch is good, but unfortunately it needs to be rebased.  Since you have to do that, I wonder if, in the reference manual, the file sagetex.rst needs to be in the new directory \"other\", or whether it should just be in the top ref manual directory.  Not a big deal.\n\nI'll put it in the \"other\" directory. I just guessed when I put sagetex.rst where it is.\n\n> Also, is it worth mentioning in the documentation that if you have several copies of sagetex.sty lying around, you need to make sure that the most recent one gets read first?  I just ran into this problem because I had installed an old version a while ago which was shadowing the more recent one.  Again, not a big deal, and if you think this should be done but don't want to bother now, it can be delayed to another ticket.\n\nI'll add a bit about this. It's one of the biggest problems with SageTeX.\n\n> For a future version of sagetex, should sagetex print its version each time you latex the file, so that users can see what version is actually being used?\n\nI've thought about some kind of mechanism that would check for version mismatch, but don't have any easily-implementable ideas at the moment. I think this would be best in another ticket.\n\n> The spkg looks good, and seems to indicate in the right places that it's now a standard spkg.  Does anything else need to be checked?\n\nI don't think so. If you can install the spkg, follow the TeX installation directions, and successfully use it on a document, I think it's good to go.\n\nI have 4.3.1 compiled, so I'll rebase and adjust the documentation, and open a ticket for the version mismatch stuff.",
    "created_at": "2010-01-22T00:45:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56744",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:22" align="right">comment:22</div>

Replying to [@jhpalmieri](#comment%3A21):
> A few comments: overall, the documentation patch is good, but unfortunately it needs to be rebased.  Since you have to do that, I wonder if, in the reference manual, the file sagetex.rst needs to be in the new directory "other", or whether it should just be in the top ref manual directory.  Not a big deal.

I'll put it in the "other" directory. I just guessed when I put sagetex.rst where it is.

> Also, is it worth mentioning in the documentation that if you have several copies of sagetex.sty lying around, you need to make sure that the most recent one gets read first?  I just ran into this problem because I had installed an old version a while ago which was shadowing the more recent one.  Again, not a big deal, and if you think this should be done but don't want to bother now, it can be delayed to another ticket.

I'll add a bit about this. It's one of the biggest problems with SageTeX.

> For a future version of sagetex, should sagetex print its version each time you latex the file, so that users can see what version is actually being used?

I've thought about some kind of mechanism that would check for version mismatch, but don't have any easily-implementable ideas at the moment. I think this would be best in another ticket.

> The spkg looks good, and seems to indicate in the right places that it's now a standard spkg.  Does anything else need to be checked?

I don't think so. If you can install the spkg, follow the TeX installation directions, and successfully use it on a document, I think it's good to go.

I have 4.3.1 compiled, so I'll rebase and adjust the documentation, and open a ticket for the version mismatch stuff.



---

archive/issue_comments_056745.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">comment:23</div>\n\nThe \"detect version mismatch ticket\" is #8035.",
    "created_at": "2010-01-22T00:50:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56745",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:23" align="right">comment:23</div>

The "detect version mismatch ticket" is #8035.



---

archive/issue_comments_056746.json:
```json
{
    "body": "Attachment: **[trac_7617.3.patch.gz](https://github.com/sagemath/sage/files/ticket7617/trac_7617.3.patch.gz)**\n\nadditions to the installation guide and tutorial, v3. apply only this.",
    "created_at": "2010-01-22T05:21:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56746",
    "user": "https://github.com/dandrake"
}
```

Attachment: **[trac_7617.3.patch.gz](https://github.com/sagemath/sage/files/ticket7617/trac_7617.3.patch.gz)**

additions to the installation guide and tutorial, v3. apply only this.



---

archive/issue_events_093038.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-01-22T05:29:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93038"
}
```



---

archive/issue_events_093039.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-01-22T05:29:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93039"
}
```



---

archive/issue_comments_056747.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\nI've uploaded a new version of the documentation patch. It should apply to 4.3.1.\n\nFor the reference manual, I've kept sagetex.rst in the new \"other\" directory; when I put it in the root reference manual directory, Sphinx would try to look up a module named \"sagetex\" in the Sage library, get really confused, and overwrite sagetex.rst. I found that that didn't happen when I had the file in the \"other\" directory. If there's a better way to do this, let me know.\n\nI added some text to the installation guide warning about having stale copies of sagetex.sty around.",
    "created_at": "2010-01-22T05:29:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56747",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:24" align="right">comment:24</div>

I've uploaded a new version of the documentation patch. It should apply to 4.3.1.

For the reference manual, I've kept sagetex.rst in the new "other" directory; when I put it in the root reference manual directory, Sphinx would try to look up a module named "sagetex" in the Sage library, get really confused, and overwrite sagetex.rst. I found that that didn't happen when I had the file in the "other" directory. If there's a better way to do this, let me know.

I added some text to the installation guide warning about having stale copies of sagetex.sty around.



---

archive/issue_comments_056748.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nLooks good to me.  I guess the next thing to do is to add commands to TeXShop, Auc-TeX, etc., so they can easily run sage on the appropriate file...  :)",
    "created_at": "2010-01-22T06:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56748",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:25" align="right">comment:25</div>

Looks good to me.  I guess the next thing to do is to add commands to TeXShop, Auc-TeX, etc., so they can easily run sage on the appropriate file...  :)



---

archive/issue_events_093040.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-01-22T06:12:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93040"
}
```



---

archive/issue_events_093041.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-01-22T06:12:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93041"
}
```



---

archive/issue_comments_056749.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\nSee #8037 for a related ticket.",
    "created_at": "2010-01-22T06:18:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56749",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:26" align="right">comment:26</div>

See #8037 for a related ticket.



---

archive/issue_comments_056750.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nJust a minor comment: The spkg [sagetex-2.2.3.spkg](http://sagenb.kaist.ac.kr/~drake/sagetex-2.2.3.spkg) consists of SageTeX plus an MD5 check sum file `md5sum.check`, both compressed as `sagetex-2.2.3.spkg`. During the installation process, `sagetex-2.2.3.spkg` is uncompressed to `SAGE_PACKAGES/build/` and then installed, so that `SAGE_PACKAGES/build/` contains both `sagetex-2.2.3` and `md5sum.check`. After the installation is successful, the directory `SAGE_PACKAGES/build/sagetex-2.2.3/` is deleted, but `md5sum.check` still lingers under `SAGE_PACKAGES/build/`.\n\nIs `md5sum.check` meant to be a realization of the ideas at #329 specifically for the SageTeX spkg?",
    "created_at": "2010-01-24T23:37:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56750",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:27" align="right">comment:27</div>

Just a minor comment: The spkg [sagetex-2.2.3.spkg](http://sagenb.kaist.ac.kr/~drake/sagetex-2.2.3.spkg) consists of SageTeX plus an MD5 check sum file `md5sum.check`, both compressed as `sagetex-2.2.3.spkg`. During the installation process, `sagetex-2.2.3.spkg` is uncompressed to `SAGE_PACKAGES/build/` and then installed, so that `SAGE_PACKAGES/build/` contains both `sagetex-2.2.3` and `md5sum.check`. After the installation is successful, the directory `SAGE_PACKAGES/build/sagetex-2.2.3/` is deleted, but `md5sum.check` still lingers under `SAGE_PACKAGES/build/`.

Is `md5sum.check` meant to be a realization of the ideas at #329 specifically for the SageTeX spkg?



---

archive/issue_comments_056751.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">comment:28</div>\n\nReplying to [@sagetrac-mvngu](#comment%3A27):\n> Is `md5sum.check` meant to be a realization of the ideas at #329 specifically for the SageTeX spkg?\n\nYes. I was trying to be ahead of the curve and get my spkg compatible right away...but nothing has happened with that. Should I just remove that?",
    "created_at": "2010-01-25T00:17:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56751",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:28" align="right">comment:28</div>

Replying to [@sagetrac-mvngu](#comment%3A27):
> Is `md5sum.check` meant to be a realization of the ideas at #329 specifically for the SageTeX spkg?

Yes. I was trying to be ahead of the curve and get my spkg compatible right away...but nothing has happened with that. Should I just remove that?



---

archive/issue_comments_056752.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">comment:29</div>\n\nReplying to [@dandrake](#comment%3A28):\n> Should I just remove that?\n\nNo, I think you can leave the MD5 check sum file as is for now as a realization of the ideas at #329. The SageTeX spkg is the only package implementing the integrity check ideas at #329. I find the verification instructions at #329 to be very helpful, which make me think about an idea for implementing a patch to automatically verify the check sum in an spkg. But if other spkg's in the future also have a file named `md5sum.check`, it can be difficult to know which check file belongs to which package (because they're all given the same name). But if each spkg has its own integrity check file named, say, `spkg-name.md5` then `SAGE_PACKAGES/build` would be inundated with MD5 sum files (unless you delete them upon successful verification).",
    "created_at": "2010-01-25T00:41:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56752",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:29" align="right">comment:29</div>

Replying to [@dandrake](#comment%3A28):
> Should I just remove that?

No, I think you can leave the MD5 check sum file as is for now as a realization of the ideas at #329. The SageTeX spkg is the only package implementing the integrity check ideas at #329. I find the verification instructions at #329 to be very helpful, which make me think about an idea for implementing a patch to automatically verify the check sum in an spkg. But if other spkg's in the future also have a file named `md5sum.check`, it can be difficult to know which check file belongs to which package (because they're all given the same name). But if each spkg has its own integrity check file named, say, `spkg-name.md5` then `SAGE_PACKAGES/build` would be inundated with MD5 sum files (unless you delete them upon successful verification).



---

archive/issue_comments_056753.json:
```json
{
    "body": "<div id=\"comment:30\" align=\"right\">comment:30</div>\n\nReplying to [@sagetrac-mvngu](#comment%3A29):\n> But if other spkg's in the future also have a file named `md5sum.check`, it can be difficult to know which check file belongs to which package (because they're all given the same name). But if each spkg has its own integrity check file named, say, `spkg-name.md5` then `SAGE_PACKAGES/build` would be inundated with MD5 sum files (unless you delete them upon successful verification).\n\nUsing the spkg name sounds like a good idea. I think verifying file integrity is important enough, and the number of files created small enough, so that having all those files in SAGE_PACKAGES/build worth it.",
    "created_at": "2010-01-25T08:35:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56753",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:30" align="right">comment:30</div>

Replying to [@sagetrac-mvngu](#comment%3A29):
> But if other spkg's in the future also have a file named `md5sum.check`, it can be difficult to know which check file belongs to which package (because they're all given the same name). But if each spkg has its own integrity check file named, say, `spkg-name.md5` then `SAGE_PACKAGES/build` would be inundated with MD5 sum files (unless you delete them upon successful verification).

Using the spkg name sounds like a good idea. I think verifying file integrity is important enough, and the number of files created small enough, so that having all those files in SAGE_PACKAGES/build worth it.



---

archive/issue_comments_056754.json:
```json
{
    "body": "updated deps for SageTeX",
    "created_at": "2010-01-29T17:46:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56754",
    "user": "https://github.com/sagetrac-mvngu"
}
```

updated deps for SageTeX



---

archive/issue_comments_056755.json:
```json
{
    "body": "Attachment: **[deps.gz](https://github.com/sagemath/sage/files/ticket7617/deps.gz)**\n\nAttachment: **[deps.patch.gz](https://github.com/sagemath/sage/files/ticket7617/deps.patch.gz)**\n\ndifferences between deps in Sage 4.3.2.alpha0 and updated deps; don't apply",
    "created_at": "2010-01-29T17:47:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56755",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[deps.gz](https://github.com/sagemath/sage/files/ticket7617/deps.gz)**

Attachment: **[deps.patch.gz](https://github.com/sagemath/sage/files/ticket7617/deps.patch.gz)**

differences between deps in Sage 4.3.2.alpha0 and updated deps; don't apply



---

archive/issue_comments_056756.json:
```json
{
    "body": "Attachment: **[install.gz](https://github.com/sagemath/sage/files/ticket7617/install.gz)**\n\nupdated install for SageTeX",
    "created_at": "2010-01-29T17:47:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56756",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[install.gz](https://github.com/sagemath/sage/files/ticket7617/install.gz)**

updated install for SageTeX



---

archive/issue_comments_056757.json:
```json
{
    "body": "Attachment: **[install.patch.gz](https://github.com/sagemath/sage/files/ticket7617/install.patch.gz)**\n\ndifferences between install in Sage 4.3.2.alpha0 and updated install; don't apply",
    "created_at": "2010-01-29T17:48:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56757",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[install.patch.gz](https://github.com/sagemath/sage/files/ticket7617/install.patch.gz)**

differences between install in Sage 4.3.2.alpha0 and updated install; don't apply



---

archive/issue_comments_056758.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">comment:31</div>\n\nI have attached newer versions of the files `deps` and `install`, updated to take account of SageTeX being a standard spkg.",
    "created_at": "2010-01-29T17:49:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56758",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:31" align="right">comment:31</div>

I have attached newer versions of the files `deps` and `install`, updated to take account of SageTeX being a standard spkg.



---

archive/issue_events_093042.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-01-31T00:02:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93042"
}
```



---

archive/issue_events_093043.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-01-31T00:02:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7617#event-93043"
}
```



---

archive/issue_comments_056759.json:
```json
{
    "body": "<div id=\"comment:32\" align=\"right\">comment:32</div>\n\nMerged in this order: \n\n1. [trac_7617.3.patch](https://github.com/sagemath/sage/files/ticket7617/trac_7617.3.patch.gz) in the Sage library\n2. [install](https://github.com/sagemath/sage/files/ticket7617/install.gz) under `SAGE_ROOT/spkg/`\n3. [deps](https://github.com/sagemath/sage/files/ticket7617/deps.gz) under `SAGE_ROOT/spkg/standard/`\n4. [sagetex-2.2.3.spkg](http://sagenb.kaist.ac.kr/~drake/sagetex-2.2.3.spkg) in the standard spkg repository",
    "created_at": "2010-01-31T00:02:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56759",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:32" align="right">comment:32</div>

Merged in this order: 

1. [trac_7617.3.patch](https://github.com/sagemath/sage/files/ticket7617/trac_7617.3.patch.gz) in the Sage library
2. [install](https://github.com/sagemath/sage/files/ticket7617/install.gz) under `SAGE_ROOT/spkg/`
3. [deps](https://github.com/sagemath/sage/files/ticket7617/deps.gz) under `SAGE_ROOT/spkg/standard/`
4. [sagetex-2.2.3.spkg](http://sagenb.kaist.ac.kr/~drake/sagetex-2.2.3.spkg) in the standard spkg repository



---

archive/issue_comments_056760.json:
```json
{
    "body": "Merged: **sage-4.3.2.alpha1**",
    "created_at": "2010-01-31T00:02:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56760",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sage-4.3.2.alpha1**



---

archive/issue_comments_056761.json:
```json
{
    "body": "<div id=\"comment:33\" align=\"right\">comment:33</div>\n\nSee #8144 for a follow-up to this ticket.",
    "created_at": "2010-02-01T18:14:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56761",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:33" align="right">comment:33</div>

See #8144 for a follow-up to this ticket.



---

archive/issue_comments_056762.json:
```json
{
    "body": "<div id=\"comment:34\" align=\"right\">comment:34</div>\n\nReplying to [@sagetrac-mvngu](#comment%3A29):\n> No, I think you can leave the MD5 check sum file as is for now as a realization of the ideas at #329. The SageTeX spkg is the only package implementing the integrity check ideas at #329. \n\nNote I commented on #329 that I believe 'cksum' is better than md5, since\n\n* The command to compute an md5 checksum is called by different names on different systems. 'md5' and 'md5sum' are two I've come across. On Solaris one has to use 'digest -a md5 filename'\n* I suspect on some cut-down Linux distros, no such command exits - it is not part of POSIX. \n* In contrast, 'cksum' is specified by POSIX, it is portable across all platforms as POSIX specifies the algorithm, and further that whilst not quite such a good test as 'md5', the probability of getting a false result is less than 2.4 x 10^-10.",
    "created_at": "2010-03-07T13:20:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7617",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7617#issuecomment-56762",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:34" align="right">comment:34</div>

Replying to [@sagetrac-mvngu](#comment%3A29):
> No, I think you can leave the MD5 check sum file as is for now as a realization of the ideas at #329. The SageTeX spkg is the only package implementing the integrity check ideas at #329. 

Note I commented on #329 that I believe 'cksum' is better than md5, since

* The command to compute an md5 checksum is called by different names on different systems. 'md5' and 'md5sum' are two I've come across. On Solaris one has to use 'digest -a md5 filename'
* I suspect on some cut-down Linux distros, no such command exits - it is not part of POSIX. 
* In contrast, 'cksum' is specified by POSIX, it is portable across all platforms as POSIX specifies the algorithm, and further that whilst not quite such a good test as 'md5', the probability of getting a false result is less than 2.4 x 10^-10.
