# Issue 7245: sage -merge misses positively reviewed tickets

archive/issues_007245.json:
```json
{
    "assignees": [],
    "body": "The ` sage -merge ` script doesn't detect tickets as being marked positively reviewed if they don't have \"positive review\" in the title.\n\nCC:  @craigcitro\n\nKeywords: **merge apply ticket**\n\nComponent: **build**\n\nAuthor: **Mike Hansen**\n\nReviewer: **Dan Drake**\n\nMerged: **sage-4.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7245_\n\n",
    "closed_at": "2009-10-19T06:03:43Z",
    "created_at": "2009-10-19T05:28:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage -merge misses positively reviewed tickets",
    "type": "issue",
    "updated_at": "2009-10-19T06:03:43Z",
    "url": "https://github.com/sagemath/sage/issues/7245",
    "user": "https://github.com/mwhansen"
}
```
The ` sage -merge ` script doesn't detect tickets as being marked positively reviewed if they don't have "positive review" in the title.

CC:  @craigcitro

Keywords: **merge apply ticket**

Component: **build**

Author: **Mike Hansen**

Reviewer: **Dan Drake**

Merged: **sage-4.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/7245_





---

archive/issue_events_088407.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T05:28:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7245#event-88407"
}
```



---

archive/issue_events_088408.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T05:28:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "000080",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7245#event-88408"
}
```



---

archive/issue_events_088409.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T05:28:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7245#event-88409"
}
```



---

archive/issue_events_088410.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T05:28:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7245#event-88410"
}
```



---

archive/issue_comments_052234.json:
```json
{
    "body": "Attachment: **[trac_7245.patch.gz](https://github.com/sagemath/sage/files/ticket7245/trac_7245.patch.gz)**\n\nApply to the scripts repository",
    "created_at": "2009-10-19T05:30:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7245#issuecomment-52234",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_7245.patch.gz](https://github.com/sagemath/sage/files/ticket7245/trac_7245.patch.gz)**

Apply to the scripts repository



---

archive/issue_events_088411.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T05:30:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7245#event-88411"
}
```



---

archive/issue_comments_052235.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nCorrectly finds all tickets in the \"positive review\" report, and the code is just a little change to the regexps. Positive review.",
    "created_at": "2009-10-19T06:02:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7245#issuecomment-52235",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:2" align="right">comment:2</div>

Correctly finds all tickets in the "positive review" report, and the code is just a little change to the regexps. Positive review.



---

archive/issue_comments_052236.json:
```json
{
    "body": "Reviewer: **Dan Drake**",
    "created_at": "2009-10-19T06:02:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7245#issuecomment-52236",
    "user": "https://github.com/dandrake"
}
```

Reviewer: **Dan Drake**



---

archive/issue_events_088412.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2009-10-19T06:02:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7245#event-88412"
}
```



---

archive/issue_events_088413.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2009-10-19T06:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7245#event-88413"
}
```



---

archive/issue_comments_052237.json:
```json
{
    "body": "Merged: **sage-4.2.alpha1**",
    "created_at": "2009-10-19T06:03:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7245#issuecomment-52237",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.alpha1**



---

archive/issue_events_088414.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T06:03:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7245#event-88414"
}
```



---

archive/issue_events_088415.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T06:03:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7245",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7245#event-88415"
}
```
