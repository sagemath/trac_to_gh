# Issue 7879: Remove unnecessary signal handling for low prec mpfr operations.

archive/issues_007879.json:
```json
{
    "assignees": [],
    "body": "`_sig_on` and `_sig_off` are more expensive than computing the result itself.\n\n---\n\nApply only [attachment: trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz) to the Sage library.\n\nAssignee: **@aghitza**\n\nKeywords: **sd32**\n\nAuthor: **Robert Bradshaw**\n\nReviewer: **Alex Ghitza, Mariah Lenox, William Stein**\n\nMerged: **sage-4.7.2.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7879_\n\n",
    "closed_at": "2011-09-13T12:16:11Z",
    "created_at": "2010-01-09T19:31:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove unnecessary signal handling for low prec mpfr operations.",
    "type": "issue",
    "updated_at": "2011-09-13T12:16:11Z",
    "url": "https://github.com/sagemath/sage/issues/7879",
    "user": "https://github.com/robertwb"
}
```
`_sig_on` and `_sig_off` are more expensive than computing the result itself.

---

Apply only [attachment: trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz) to the Sage library.

Assignee: **@aghitza**

Keywords: **sd32**

Author: **Robert Bradshaw**

Reviewer: **Alex Ghitza, Mariah Lenox, William Stein**

Merged: **sage-4.7.2.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/7879_





---

archive/issue_events_088867.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-09T19:31:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88867"
}
```



---

archive/issue_events_088868.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-09T19:31:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88868"
}
```



---

archive/issue_events_088869.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-09T19:31:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88869"
}
```



---

archive/issue_events_088870.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-09T19:31:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88870"
}
```



---

archive/issue_events_088871.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-09T19:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88871"
}
```



---

archive/issue_comments_061631.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nSignal handling is omitted for functions/precisions that were clearly and uniformly in the millisecond range or less.",
    "created_at": "2010-01-09T19:38:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61631",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:1'>Comment 1:</a>
Signal handling is omitted for functions/precisions that were clearly and uniformly in the millisecond range or less.



---

archive/issue_comments_061632.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-`_sig_on` and {{{_sig_off}} are more expensive than computing the result itself. \n+`_sig_on` and `_sig_off` are more expensive than computing the result itself. \n``````\n",
    "created_at": "2010-01-09T19:38:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61632",
    "user": "https://github.com/robertwb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-`_sig_on` and {{{_sig_off}} are more expensive than computing the result itself. 
+`_sig_on` and `_sig_off` are more expensive than computing the result itself. 
``````




---

archive/issue_comments_061633.json:
```json
{
    "body": "Attachment: **[7879-RR-signal.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.patch.gz)**",
    "created_at": "2010-01-09T19:42:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61633",
    "user": "https://github.com/robertwb"
}
```

Attachment: **[7879-RR-signal.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.patch.gz)**



---

archive/issue_events_088872.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-20T10:24:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88872"
}
```



---

archive/issue_events_088873.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-20T10:24:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88873"
}
```



---

archive/issue_comments_061634.json:
```json
{
    "body": "Reviewer: **Alex Ghitza**",
    "created_at": "2010-01-20T10:24:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61634",
    "user": "https://github.com/aghitza"
}
```

Reviewer: **Alex Ghitza**



---

archive/issue_comments_061635.json:
```json
{
    "body": "Author: **Robert Bradshaw**",
    "created_at": "2010-01-20T10:24:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61635",
    "user": "https://github.com/aghitza"
}
```

Author: **Robert Bradshaw**



---

archive/issue_comments_061636.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis looks good.  I have one question: you are hardcoding threshold precisions after which the signals should be used, mostly 1000, but also 10000 in a couple of places.  Would it be better to have a global variable PREC_THRESHOLD set to 1000 at the top of the file, and then use it or 10*PREC_THRESHOLD where needed?  It would make it easier to change this in the future, since a search for it would be all that's needed.\n\nI'm marking this as needs_info.  If you think it's not worth the trouble, I'll put a positive review.",
    "created_at": "2010-01-20T10:24:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61636",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:3'>Comment 3:</a>
This looks good.  I have one question: you are hardcoding threshold precisions after which the signals should be used, mostly 1000, but also 10000 in a couple of places.  Would it be better to have a global variable PREC_THRESHOLD set to 1000 at the top of the file, and then use it or 10*PREC_THRESHOLD where needed?  It would make it easier to change this in the future, since a search for it would be all that's needed.

I'm marking this as needs_info.  If you think it's not worth the trouble, I'll put a positive review.



---

archive/issue_events_088874.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-19T16:10:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88874"
}
```



---

archive/issue_events_088875.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-19T16:10:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88875"
}
```



---

archive/issue_events_088876.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-19T16:10:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88876"
}
```



---

archive/issue_events_088877.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-19T16:10:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88877"
}
```



---

archive/issue_events_088878.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-19T16:10:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88878"
}
```



---

archive/issue_comments_061637.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI am proposing to close this ticket as invalid since #9678 makes `sig_on()` and `sig_off()` a lot faster.  In any case, if you want to keep the patch, it needs rebasing (`_sig_on` and `_sig_off` must be replaced by `sig_on()` and `sig_off()` but there are more patch conflicts).",
    "created_at": "2011-01-19T16:10:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61637",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
I am proposing to close this ticket as invalid since #9678 makes `sig_on()` and `sig_off()` a lot faster.  In any case, if you want to keep the patch, it needs rebasing (`_sig_on` and `_sig_off` must be replaced by `sig_on()` and `sig_off()` but there are more patch conflicts).



---

archive/issue_events_088879.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-17T13:19:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88879"
}
```



---

archive/issue_events_088880.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-17T13:19:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88880"
}
```



---

archive/issue_comments_061638.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nSince this ticket is marked sage-duplicate/invalid/wontfix plus has\nnot been active for 4 months, I believe this ticket can be closed.",
    "created_at": "2011-05-17T13:19:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61638",
    "user": "https://github.com/sagetrac-mariah"
}
```

<a id='comment:5'>Comment 5:</a>
Since this ticket is marked sage-duplicate/invalid/wontfix plus has
not been active for 4 months, I believe this ticket can be closed.



---

archive/issue_comments_061639.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nRobert, what is your opinion?  `sig_on()` and `sig_off()` became a lot faster since #9678, but they still take up some cycles.",
    "created_at": "2011-05-17T15:56:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61639",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
Robert, what is your opinion?  `sig_on()` and `sig_off()` became a lot faster since #9678, but they still take up some cycles.



---

archive/issue_events_088881.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-05-17T18:26:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88881"
}
```



---

archive/issue_comments_061640.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThanks for taking a look at this. I've been waiting for 4.7 to be released to re-base this, and think it'd still worth having. Perhaps I'll put PREC_THRESHOLD in there while I'm at it, and maybe do some benchmarking.",
    "created_at": "2011-05-17T18:26:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61640",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:7'>Comment 7:</a>
Thanks for taking a look at this. I've been waiting for 4.7 to be released to re-base this, and think it'd still worth having. Perhaps I'll put PREC_THRESHOLD in there while I'm at it, and maybe do some benchmarking.



---

archive/issue_events_088882.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-17T18:32:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88882"
}
```



---

archive/issue_events_088883.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-17T18:32:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88883"
}
```



---

archive/issue_comments_061641.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nReplying to [@robertwb](#comment%3A7):\n> and maybe do some benchmarking.\n\nIf you find useful ways to do benchmarks, please let me know. I might re-use those benchmarks to optimize `sig_on()` and `sig_off()`.",
    "created_at": "2011-05-17T20:33:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61641",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'>Comment 9:</a>
Replying to [@robertwb](#comment%3A7):
> and maybe do some benchmarking.

If you find useful ways to do benchmarks, please let me know. I might re-use those benchmarks to optimize `sig_on()` and `sig_off()`.



---

archive/issue_comments_061642.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nThe patch adds one unrelated doctest.  This doctest has been moved to #11344.",
    "created_at": "2011-05-17T20:34:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61642",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'>Comment 10:</a>
The patch adds one unrelated doctest.  This doctest has been moved to #11344.



---

archive/issue_comments_061643.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\n\n```\n----------------------------------------------------------------------\n| Sage Version 4.7, Release Date: 2011-05-23                         |\n| Type notebook() for the GUI, and license() for information.        |\n----------------------------------------------------------------------\nLoading Sage library. Current Mercurial branch is: bitrot\n\nsage: L = [RR(k) for k in range(1000)]\n\nsage: %timeit [a.sqrt() for a in L]\n 625 loops, best of 3: 569 \u00b5s per loop\n\nsage: %timeit [a.sqrt() for a in L]\n 625 loops, best of 3: 509 \u00b5s per loop\n\nsage: %timeit [a.sqrt() for a in L]\n 625 loops, best of 3: 513 \u00b5s per loop\n\nsage: timeit(\"[a.sqrt() for a in L]\", number=10000)\n10000 loops, best of 3: 519 \u00b5s per loop\n\nsage: \nExiting Sage (CPU time 0m19.94s, Wall time 1m9.75s).\nrobertwb@sage:/scratch/robertwb/sage-4.7/devel/sage-bitrot$ hg qpush\napplying 7879-RR-signal.patch\nnow at: 7879-RR-signal.patch\nrobertwb@sage:/scratch/robertwb/sage-4.7/devel/sage-bitrot$ ../../sage -br\n[...]\n----------------------------------------------------------------------\n| Sage Version 4.7, Release Date: 2011-05-23                         |\n| Type notebook() for the GUI, and license() for information.        |\n----------------------------------------------------------------------\nLoading Sage library. Current Mercurial branch is: bitrot\n\nsage: L = [RR(k) for k in range(1000)]\n\nsage: %timeit [a.sqrt() for a in L]\n 625 loops, best of 3: 498 \u00b5s per loop\n\nsage: %timeit [a.sqrt() for a in L]\n 625 loops, best of 3: 498 \u00b5s per loop\n\nsage: %timeit [a.sqrt() for a in L]\n 625 loops, best of 3: 496 \u00b5s per loop\n\nsage: timeit(\"[a.sqrt() for a in L]\", number=10000)\n10000 loops, best of 3: 499 \u00b5s per loop\n```\n\nLooks like about a 5% win (much smaller than I remember it being before).",
    "created_at": "2011-05-31T08:01:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61643",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:11'>Comment 11:</a>

```
----------------------------------------------------------------------
| Sage Version 4.7, Release Date: 2011-05-23                         |
| Type notebook() for the GUI, and license() for information.        |
----------------------------------------------------------------------
Loading Sage library. Current Mercurial branch is: bitrot

sage: L = [RR(k) for k in range(1000)]

sage: %timeit [a.sqrt() for a in L]
 625 loops, best of 3: 569 µs per loop

sage: %timeit [a.sqrt() for a in L]
 625 loops, best of 3: 509 µs per loop

sage: %timeit [a.sqrt() for a in L]
 625 loops, best of 3: 513 µs per loop

sage: timeit("[a.sqrt() for a in L]", number=10000)
10000 loops, best of 3: 519 µs per loop

sage: 
Exiting Sage (CPU time 0m19.94s, Wall time 1m9.75s).
robertwb@sage:/scratch/robertwb/sage-4.7/devel/sage-bitrot$ hg qpush
applying 7879-RR-signal.patch
now at: 7879-RR-signal.patch
robertwb@sage:/scratch/robertwb/sage-4.7/devel/sage-bitrot$ ../../sage -br
[...]
----------------------------------------------------------------------
| Sage Version 4.7, Release Date: 2011-05-23                         |
| Type notebook() for the GUI, and license() for information.        |
----------------------------------------------------------------------
Loading Sage library. Current Mercurial branch is: bitrot

sage: L = [RR(k) for k in range(1000)]

sage: %timeit [a.sqrt() for a in L]
 625 loops, best of 3: 498 µs per loop

sage: %timeit [a.sqrt() for a in L]
 625 loops, best of 3: 498 µs per loop

sage: %timeit [a.sqrt() for a in L]
 625 loops, best of 3: 496 µs per loop

sage: timeit("[a.sqrt() for a in L]", number=10000)
10000 loops, best of 3: 499 µs per loop
```

Looks like about a 5% win (much smaller than I remember it being before).



---

archive/issue_events_088884.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-05-31T08:01:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88884"
}
```



---

archive/issue_events_088885.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-05-31T08:01:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88885"
}
```



---

archive/issue_comments_061644.json:
```json
{
    "body": "Attachment: **[7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz)**\n\nRebased on 4.7",
    "created_at": "2011-05-31T08:01:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61644",
    "user": "https://github.com/robertwb"
}
```

Attachment: **[7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz)**

Rebased on 4.7



---

archive/issue_comments_061645.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\npatch [attachment: 7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz) applied to sage-4.7.1.alpha1.  Then did 'sage -b' followed by 'make testlong'.\nThe following test failed:\n\n```\neno% ./sage -t  -long -force_lib \"devel/sage/sage/rings/residue_field.pyx\"\nsage -t -long -force_lib \"devel/sage/sage/rings/residue_field.pyx\"\n**********************************************************************\nFile \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/devel/sage/sage/rings/residue_field.pyx\", line 670:\n    sage: I = QQ[3^(1/3)].factor(5)[1][0]; I\nException raised:\n    Traceback (most recent call last):\n      File \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_10[2]>\", line 1, in <module>\n        I = QQ[Integer(3)**(Integer(1)/Integer(3))].factor(Integer(5))[Integer(1)][Integer(0)]; I###line 670:\n    sage: I = QQ[3^(1/3)].factor(5)[1][0]; I\n      File \"ring.pyx\", line 167, in sage.rings.ring.Ring.__getitem__ (sage/rings/ring.c:2007)\n      File \"expression.pyx\", line 8287, in sage.symbolic.expression.Expression.minpoly (sage/symbolic/expression.cpp:31950)\n      File \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/lib/python/site-packages/sage/calculus/calculus.py\", line 925, in minpoly\n        if g(ex).simplify_trig().simplify_radical() == 0:\n      File \"expression.pyx\", line 6546, in sage.symbolic.expression.Expression.simplify_trig (sage/symbolic/expression.cpp:24657)\n      File \"expression.pyx\", line 460, in sage.symbolic.expression.Expression._maxima_ (sage/symbolic/expression.cpp:3674)\n      File \"sage_object.pyx\", line 429, in sage.structure.sage_object.SageObject._interface_ (sage/structure/sage_object.c:3451)\n      File \"lazy_import.pyx\", line 167, in sage.misc.lazy_import.LazyImport.__getattr__ (sage/misc/lazy_import.c:1353)\n      File \"lazy_import.pyx\", line 110, in sage.misc.lazy_import.LazyImport._get_object (sage/misc/lazy_import.c:1023)\n      File \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/lib/python/site-packages/sage/interfaces/maxima_lib.py\", line 152, in <module>\n        ecl_eval(\"#$%s$\"%l)\n      File \"ecl.pyx\", line 1224, in sage.libs.ecl.ecl_eval (sage/libs/ecl.c:6301)\n      File \"ecl.pyx\", line 1239, in sage.libs.ecl.ecl_eval (sage/libs/ecl.c:6252)\n      File \"ecl.pyx\", line 252, in sage.libs.ecl.ecl_safe_eval (sage/libs/ecl.c:2544)\n    RuntimeError: ECL says: THROW: The catch MACSYMA-QUIT is undefined.\n**********************************************************************\nFile \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/devel/sage/sage/rings/residue_field.pyx\", line 672:\n    sage: k = I.residue_field(); k\nException raised:\n    Traceback (most recent call last):\n      File \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_10[3]>\", line 1, in <module>\n        k = I.residue_field(); k###line 672:\n    sage: k = I.residue_field(); k\n      File \"element.pyx\", line 328, in sage.structure.element.Element.__getattr__ (sage/structure/element.c:2792)\n      File \"parent.pyx\", line 277, in sage.structure.parent.getattr_from_other_class (sage/structure/parent.c:2940)\n      File \"parent.pyx\", line 175, in sage.structure.parent.raise_attribute_error (sage/structure/parent.c:2709)\n    AttributeError: 'sage.symbolic.expression.Expression' object has no attribute 'residue_field'\n**********************************************************************\n```\n\netc.",
    "created_at": "2011-06-01T15:19:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61645",
    "user": "https://github.com/sagetrac-mariah"
}
```

<a id='comment:12'>Comment 12:</a>
patch [attachment: 7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz) applied to sage-4.7.1.alpha1.  Then did 'sage -b' followed by 'make testlong'.
The following test failed:

```
eno% ./sage -t  -long -force_lib "devel/sage/sage/rings/residue_field.pyx"
sage -t -long -force_lib "devel/sage/sage/rings/residue_field.pyx"
**********************************************************************
File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/devel/sage/sage/rings/residue_field.pyx", line 670:
    sage: I = QQ[3^(1/3)].factor(5)[1][0]; I
Exception raised:
    Traceback (most recent call last):
      File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_10[2]>", line 1, in <module>
        I = QQ[Integer(3)**(Integer(1)/Integer(3))].factor(Integer(5))[Integer(1)][Integer(0)]; I###line 670:
    sage: I = QQ[3^(1/3)].factor(5)[1][0]; I
      File "ring.pyx", line 167, in sage.rings.ring.Ring.__getitem__ (sage/rings/ring.c:2007)
      File "expression.pyx", line 8287, in sage.symbolic.expression.Expression.minpoly (sage/symbolic/expression.cpp:31950)
      File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/lib/python/site-packages/sage/calculus/calculus.py", line 925, in minpoly
        if g(ex).simplify_trig().simplify_radical() == 0:
      File "expression.pyx", line 6546, in sage.symbolic.expression.Expression.simplify_trig (sage/symbolic/expression.cpp:24657)
      File "expression.pyx", line 460, in sage.symbolic.expression.Expression._maxima_ (sage/symbolic/expression.cpp:3674)
      File "sage_object.pyx", line 429, in sage.structure.sage_object.SageObject._interface_ (sage/structure/sage_object.c:3451)
      File "lazy_import.pyx", line 167, in sage.misc.lazy_import.LazyImport.__getattr__ (sage/misc/lazy_import.c:1353)
      File "lazy_import.pyx", line 110, in sage.misc.lazy_import.LazyImport._get_object (sage/misc/lazy_import.c:1023)
      File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/lib/python/site-packages/sage/interfaces/maxima_lib.py", line 152, in <module>
        ecl_eval("#$%s$"%l)
      File "ecl.pyx", line 1224, in sage.libs.ecl.ecl_eval (sage/libs/ecl.c:6301)
      File "ecl.pyx", line 1239, in sage.libs.ecl.ecl_eval (sage/libs/ecl.c:6252)
      File "ecl.pyx", line 252, in sage.libs.ecl.ecl_safe_eval (sage/libs/ecl.c:2544)
    RuntimeError: ECL says: THROW: The catch MACSYMA-QUIT is undefined.
**********************************************************************
File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/devel/sage/sage/rings/residue_field.pyx", line 672:
    sage: k = I.residue_field(); k
Exception raised:
    Traceback (most recent call last):
      File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_10[3]>", line 1, in <module>
        k = I.residue_field(); k###line 672:
    sage: k = I.residue_field(); k
      File "element.pyx", line 328, in sage.structure.element.Element.__getattr__ (sage/structure/element.c:2792)
      File "parent.pyx", line 277, in sage.structure.parent.getattr_from_other_class (sage/structure/parent.c:2940)
      File "parent.pyx", line 175, in sage.structure.parent.raise_attribute_error (sage/structure/parent.c:2709)
    AttributeError: 'sage.symbolic.expression.Expression' object has no attribute 'residue_field'
**********************************************************************
```

etc.



---

archive/issue_events_088886.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-06-01T15:19:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88886"
}
```



---

archive/issue_events_088887.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-06-01T15:19:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88887"
}
```



---

archive/issue_comments_061646.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nReplying to [mariah](#comment%3A12):\n> patch [attachment: 7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz) applied to sage-4.7.1.alpha1.  Then did 'sage -b' followed by 'make testlong'.\n> The following test failed:\n> \n> ```\n> eno% ./sage -t  -long -force_lib \"devel/sage/sage/rings/residue_field.pyx\"\n> sage -t -long -force_lib \"devel/sage/sage/rings/residue_field.pyx\"\n> **********************************************************************\n> File \"/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/devel/sage/sage/rings/residue_field.pyx\", line 670:\n>     sage: I = QQ[3^(1/3)].factor(5)[1][0]; I\n> Exception raised:\n> ```\n\nThese errors don't occur for me with sage-4.7.2.alpha2.",
    "created_at": "2011-08-23T07:16:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61646",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:13'>Comment 13:</a>
Replying to [mariah](#comment%3A12):
> patch [attachment: 7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz) applied to sage-4.7.1.alpha1.  Then did 'sage -b' followed by 'make testlong'.
> The following test failed:
> 
> ```
> eno% ./sage -t  -long -force_lib "devel/sage/sage/rings/residue_field.pyx"
> sage -t -long -force_lib "devel/sage/sage/rings/residue_field.pyx"
> **********************************************************************
> File "/home/mariah/sage/sage-4.7.1.alpha1-x86_64-Linux-core2-fc-review-7879/devel/sage/sage/rings/residue_field.pyx", line 670:
>     sage: I = QQ[3^(1/3)].factor(5)[1][0]; I
> Exception raised:
> ```

These errors don't occur for me with sage-4.7.2.alpha2.



---

archive/issue_comments_061647.json:
```json
{
    "body": "Changed reviewer from **Alex Ghitza** to **Alex Ghitza, William Stein**",
    "created_at": "2011-08-23T07:16:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61647",
    "user": "https://github.com/williamstein"
}
```

Changed reviewer from **Alex Ghitza** to **Alex Ghitza, William Stein**



---

archive/issue_events_088888.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-08-23T07:24:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88888"
}
```



---

archive/issue_events_088889.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-08-23T07:24:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88889"
}
```



---

archive/issue_comments_061648.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nI did a full test of Sage on another machine too with 4.7.2.alpha2, and everything works.  Code looks good and addresses other people's issues...",
    "created_at": "2011-08-23T07:24:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61648",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:14'>Comment 14:</a>
I did a full test of Sage on another machine too with 4.7.2.alpha2, and everything works.  Code looks good and addresses other people's issues...



---

archive/issue_comments_061649.json:
```json
{
    "body": "Changed keywords from **** to **sd32**",
    "created_at": "2011-08-24T23:43:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61649",
    "user": "https://github.com/williamstein"
}
```

Changed keywords from **** to **sd32**



---

archive/issue_comments_061650.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n-`_sig_on` and `_sig_off` are more expensive than computing the result itself. \n+`_sig_on` and `_sig_off` are more expensive than computing the result itself.\n+\n+---\n+\n+Apply only [attachment: 7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz) to the Sage library.\n``````\n",
    "created_at": "2011-09-09T04:24:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61650",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
-`_sig_on` and `_sig_off` are more expensive than computing the result itself. 
+`_sig_on` and `_sig_off` are more expensive than computing the result itself.
+
+---
+
+Apply only [attachment: 7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz) to the Sage library.
``````




---

archive/issue_comments_061651.json:
```json
{
    "body": "Changed reviewer from **Alex Ghitza, William Stein** to **Alex Ghitza, Mariah Lenox, William Stein**",
    "created_at": "2011-09-09T04:24:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61651",
    "user": "https://github.com/nexttime"
}
```

Changed reviewer from **Alex Ghitza, William Stein** to **Alex Ghitza, Mariah Lenox, William Stein**



---

archive/issue_comments_061652.json:
```json
{
    "body": "Sage library patch. Robert's patch rebased to Sage 4.7.2.alpha2, because of fuzz 2.",
    "created_at": "2011-09-12T15:51:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61652",
    "user": "https://github.com/nexttime"
}
```

Sage library patch. Robert's patch rebased to Sage 4.7.2.alpha2, because of fuzz 2.



---

archive/issue_comments_061653.json:
```json
{
    "body": "Attachment: **[trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz)**\n\nAttachment: **[trac_7879-harmless_fuzz_2.diff.gz](https://github.com/sagemath/sage/files/ticket7879/trac_7879-harmless_fuzz_2.diff.gz)**\n\nDo not apply. Fuzz against Sage 4.7.2.alpha2. For reference only.",
    "created_at": "2011-09-12T15:52:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61653",
    "user": "https://github.com/nexttime"
}
```

Attachment: **[trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz)**

Attachment: **[trac_7879-harmless_fuzz_2.diff.gz](https://github.com/sagemath/sage/files/ticket7879/trac_7879-harmless_fuzz_2.diff.gz)**

Do not apply. Fuzz against Sage 4.7.2.alpha2. For reference only.



---

archive/issue_comments_061654.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n ---\n \n-Apply only [attachment: 7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz) to the Sage library.\n+Apply only [attachment: trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz) to the Sage library.\n``````\n",
    "created_at": "2011-09-12T15:58:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61654",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 ---
 
-Apply only [attachment: 7879-RR-signal.2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/7879-RR-signal.2.patch.gz) to the Sage library.
+Apply only [attachment: trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz](https://github.com/sagemath/sage/files/ticket7879/trac_7879-RR-signal.2.rebased_to_4.7.2.alpha2.patch.gz) to the Sage library.
``````




---

archive/issue_comments_061655.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nAttached a rebased patch because of fuzz.",
    "created_at": "2011-09-12T15:58:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61655",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:17'>Comment 17:</a>
Attached a rebased patch because of fuzz.



---

archive/issue_comments_061656.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha3**",
    "created_at": "2011-09-13T12:16:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7879#issuecomment-61656",
    "user": "https://github.com/nexttime"
}
```

Merged: **sage-4.7.2.alpha3**



---

archive/issue_events_088890.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-13T12:16:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88890"
}
```



---

archive/issue_events_088891.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-13T12:16:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7879",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7879#event-88891"
}
```
