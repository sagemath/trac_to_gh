# Issue 7393: Update ECL to the latest version

archive/issues_007393.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThe new spkg can be found at http://sage.math.washington.edu/home/mhansen/ecl-9.10.2-20091105cvs.p0.spkg\n\nThis should fix #7113.\n\nCC:  @sagetrac-drkirkby\n\nComponent: **packages: standard**\n\nAuthor: **Mike Hansen**\n\nReviewer: **William Stein**\n\nMerged: **sage-4.2.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7393_\n\n",
    "closed_at": "2009-11-06T07:21:18Z",
    "created_at": "2009-11-05T03:28:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update ECL to the latest version",
    "type": "issue",
    "updated_at": "2009-11-06T07:21:18Z",
    "url": "https://github.com/sagemath/sage/issues/7393",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>

The new spkg can be found at http://sage.math.washington.edu/home/mhansen/ecl-9.10.2-20091105cvs.p0.spkg

This should fix #7113.

CC:  @sagetrac-drkirkby

Component: **packages: standard**

Author: **Mike Hansen**

Reviewer: **William Stein**

Merged: **sage-4.2.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/7393_





---

archive/issue_events_089702.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-05T03:28:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "milestone_number": null,
    "milestone_title": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7393#event-89702"
}
```



---

archive/issue_events_089703.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-05T03:28:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7393#event-89703"
}
```



---

archive/issue_events_089704.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-05T03:28:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7393#event-89704"
}
```



---

archive/issue_events_089705.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-05T03:28:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7393#event-89705"
}
```



---

archive/issue_events_089706.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-05T03:29:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7393#event-89706"
}
```



---

archive/issue_comments_054077.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis *does* fix 7113.   Also, what you actually did looks good (in the spkg).  So positive review.",
    "created_at": "2009-11-06T05:49:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7393#issuecomment-54077",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

This *does* fix 7113.   Also, what you actually did looks good (in the spkg).  So positive review.



---

archive/issue_events_089707.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-06T05:49:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7393#event-89707"
}
```



---

archive/issue_events_089708.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-06T05:49:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7393#event-89708"
}
```



---

archive/issue_comments_054078.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2009-11-06T07:21:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7393#issuecomment-54078",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **William Stein**



---

archive/issue_comments_054079.json:
```json
{
    "body": "Merged: **sage-4.2.1.alpha0**",
    "created_at": "2009-11-06T07:21:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7393#issuecomment-54079",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.1.alpha0**



---

archive/issue_events_089709.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-06T07:21:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7393#event-89709"
}
```



---

archive/issue_events_089710.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-06T07:21:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7393",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7393#event-89710"
}
```
