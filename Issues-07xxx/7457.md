...
<a id='comment:2'></a>Can `self.cover_ring()` ever be non-Noetherian (with our current code)?
...
```json
{
    "body": "<a id='comment:3'></a>That's a very good point, I should definitely fix this.  I'll have `is_noetherian` return True if `cover_ring` is Noetherian, and raise a `NotImplementedError` otherwise.\n\nI'm marking this as needs_work, and I'll try to get to it soon.",
    "created_at": "2009-11-22T10:21:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7457",
    "type": "issue_comment",
...
<a id='comment:3'></a>That's a very good point, I should definitely fix this.  I'll have `is_noetherian` return True if `cover_ring` is Noetherian, and raise a `NotImplementedError` otherwise.
...
```json
{
    "body": "<a id='comment:4'></a>Alright, in the process of fixing this I added a few trivial methods to `InfinitePolynomialRing` and fixed a `NotImplementedError` being returned rather than raised in `ring.pyx`.",
...
<a id='comment:4'></a>Alright, in the process of fixing this I added a few trivial methods to `InfinitePolynomialRing` and fixed a `NotImplementedError` being returned rather than raised in `ring.pyx`.
...
<a id='comment:5'></a>Two comments: this is not your fault, but can you fix lines 39-40 of quotient_ring.py?  Right now it says
...
```json
{
    "body": "<a id='comment:6'></a>Here's a referee's patch to fix these two issues.  See #7532 for a follow-up.",
    "created_at": "2009-11-26T15:35:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7457",
    "type": "issue_comment",
...
<a id='comment:6'></a>Here's a referee's patch to fix these two issues.  See #7532 for a follow-up.
...
```json
{
    "body": "<a id='comment:8'></a>Wow, I look away for a couple of days and this is all done and fixed.  Thanks, John!",
...
<a id='comment:8'></a>Wow, I look away for a couple of days and this is all done and fixed.  Thanks, John!
