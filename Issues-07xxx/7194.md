...
<a id='comment:1'></a>I updated the patch. It now supports matrices as well :-).
I also fixes some problem when returning polynomials.
Cheers,
Michael
...
<a id='comment:3'></a>How to construct ring with parameters
...
<a id='comment:4'></a>how to make letter place
sage: from sage.libs.singular.function import lib, singular_function
sage: P.<x,y,z>=QQ[]
sage: lib("freegb.lib")
sage: make_letter_place_ring = singular_function("makeLetterplaceRing")
...
```json
{
    "body": "<a id='comment:6'></a>the latest patch supports intvec/intmat\nCheers,\nMichael",
    "created_at": "2009-10-15T11:18:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7194",
    "type": "issue_comment",
...
<a id='comment:6'></a>the latest patch supports intvec/intmat
Cheers,
Michael
...
```json
{
    "body": "<a id='comment:8'></a>Attachment [singular_list.5.patch](tarball://root/attachments/some-uuid/ticket7194/singular_list.5.patch) by PolyBoRi created at 2009-10-15 15:08:13\n\nthe recent version wraps vectors over P**n in both direction.\nNow, I am about hunting the modules.",
...
<a id='comment:8'></a>Attachment [singular_list.5.patch](tarball://root/attachments/some-uuid/ticket7194/singular_list.5.patch) by PolyBoRi created at 2009-10-15 15:08:13
...
<a id='comment:9'></a>The latest version supports also modules and free resolution objects.
...
```json
{
    "body": "<a id='comment:10'></a>Attachment [singular_list.9.patch](tarball://root/attachments/some-uuid/ticket7194/singular_list.9.patch) by @malb created at 2009-11-18 17:10:23\n\nA new SPKG installing `Singular/lists.h` is available here\n\n   http://sage.math.washington.edu/home/malb/spkgs/singular-3-1-0-4-20090818.p2.spkg",
...
<a id='comment:10'></a>Attachment [singular_list.9.patch](tarball://root/attachments/some-uuid/ticket7194/singular_list.9.patch) by @malb created at 2009-11-18 17:10:23
...
<a id='comment:11'></a>Attachment [singular_lists_referee.patch](tarball://root/attachments/some-uuid/ticket7194/singular_lists_referee.patch) by @malb created at 2009-11-18 17:45:30
...
```json
{
    "body": "<a id='comment:12'></a>Michael, Burcin, any takers for looking at my referee patch?",
    "created_at": "2009-12-01T16:39:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7194",
    "type": "issue_comment",
...
<a id='comment:12'></a>Michael, Burcin, any takers for looking at my referee patch?
...
```json
{
    "body": "<a id='comment:14'></a>I get all sorts of Cython errors in function.pyx about cdef functions not being declared.\n\nDid you try this on 4.3.alpha0?",
...
<a id='comment:14'></a>I get all sorts of Cython errors in function.pyx about cdef functions not being declared.
...
```json
{
    "body": "<a id='comment:15'></a>I fixed the compilation failures under 4.3, strange that it compiled with 4.2. The instructions remain the same:\n\n* install  http://sage.math.washington.edu/home/malb/spkgs/singular-3-1-0-4-20090818.p2.spkg \n* apply `singular_list.10.patch` (fixed)\n* apply `singular_lists_referee.patch`",
...
<a id='comment:15'></a>I fixed the compilation failures under 4.3, strange that it compiled with 4.2. The instructions remain the same:
...
```json
{
    "body": "<a id='comment:17'></a>Thanks to all, who helped to get that code into Sage (while I was in holidays).\nYou made me really happy :-).\n\nCheers,\nMichael",
...
<a id='comment:17'></a>Thanks to all, who helped to get that code into Sage (while I was in holidays).
You made me really happy :-).
