...
```json
{
    "body": "<a id='comment:8'></a>As pointed out in comment:1, this problem went away ten years ago. I also verified now on MacOS and `CoCalc`. The PR adds a doctest, so we can close this old ticket if the patchbots agree.\n\n---\nNew commits:",
...
<a id='comment:8'></a>As pointed out in comment:1, this problem went away ten years ago. I also verified now on MacOS and `CoCalc`. The PR adds a doctest, so we can close this old ticket if the patchbots agree.
