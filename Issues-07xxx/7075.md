# Issue 7075: Two last figure saves to wrong place

archive/issues_007075.json:
```json
{
    "assignees": [],
    "body": "There are two more instances in plot/plot of saving figures to the current directory as opposed to SAGE_TMP.\n\nAssignee: **@williamstein**\n\nCC:  @jhpalmieri mvngu\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7075_\n\n",
    "closed_at": "2009-09-30T15:01:06Z",
    "created_at": "2009-09-29T13:57:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Two last figure saves to wrong place",
    "type": "issue",
    "updated_at": "2009-09-30T15:01:06Z",
    "url": "https://github.com/sagemath/sage/issues/7075",
    "user": "https://github.com/kcrisman"
}
```
There are two more instances in plot/plot of saving figures to the current directory as opposed to SAGE_TMP.

Assignee: **@williamstein**

CC:  @jhpalmieri mvngu

_Issue created by migration from https://trac.sagemath.org/ticket/7075_





---

archive/issue_events_075810.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-29T13:57:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-75810"
}
```



---

archive/issue_events_075811.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-29T13:57:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-75811"
}
```



---

archive/issue_events_075812.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-29T13:57:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-75812"
}
```



---

archive/issue_comments_052331.json:
```json
{
    "body": "Based on 4.1.2.alpha4",
    "created_at": "2009-09-29T14:02:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-52331",
    "user": "https://github.com/kcrisman"
}
```

Based on 4.1.2.alpha4



---

archive/issue_comments_052332.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_7075-save-fig-smart.patch.gz](https://github.com/sagemath/sage/files/ticket7075/trac_7075-save-fig-smart.patch.gz)**\n\nTo release manager: this is a duplicate of #7059, please close.",
    "created_at": "2009-09-29T14:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-52332",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_7075-save-fig-smart.patch.gz](https://github.com/sagemath/sage/files/ticket7075/trac_7075-save-fig-smart.patch.gz)**

To release manager: this is a duplicate of #7059, please close.



---

archive/issue_comments_052333.json:
```json
{
    "body": "Author: **Karl-Dieter Crisman**",
    "created_at": "2009-09-29T14:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-52333",
    "user": "https://github.com/kcrisman"
}
```

Author: **Karl-Dieter Crisman**



---

archive/issue_comments_052334.json:
```json
{
    "body": "Changed author from **Karl-Dieter Crisman** to none",
    "created_at": "2009-09-29T14:05:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-52334",
    "user": "https://github.com/kcrisman"
}
```

Changed author from **Karl-Dieter Crisman** to none



---

archive/issue_comments_052335.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nTo Minh (if it's not October 1 where you are): please close.",
    "created_at": "2009-09-30T14:58:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-52335",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'>Comment 3:</a>
To Minh (if it's not October 1 where you are): please close.



---

archive/issue_events_075813.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T15:01:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-75813"
}
```



---

archive/issue_events_075814.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T15:01:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-75814"
}
```



---

archive/issue_events_075815.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T15:01:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-75815"
}
```



---

archive/issue_comments_052336.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nClosing this ticket as a duplicate of #7059.",
    "created_at": "2009-09-30T15:01:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-52336",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:4'>Comment 4:</a>
Closing this ticket as a duplicate of #7059.
