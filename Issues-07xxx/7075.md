# Issue 7075: Two last figure saves to wrong place

archive/issues_007075.json:
```json
{
    "body": "There are two more instances in plot/plot of saving figures to the current directory as opposed to SAGE_TMP.\n\nAssignee: @williamstein\n\nCC:  @jhpalmieri mvngu\n\nResolution: duplicate\n\nIssue created by migration from https://trac.sagemath.org/ticket/7075\n\n",
    "closed_at": "2009-09-30T15:01:06Z",
    "created_at": "2009-09-29T13:57:29Z",
    "labels": [
        "component: graphics",
        "trivial",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Two last figure saves to wrong place",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7075",
    "user": "https://github.com/kcrisman"
}
```
There are two more instances in plot/plot of saving figures to the current directory as opposed to SAGE_TMP.

Assignee: @williamstein

CC:  @jhpalmieri mvngu

Resolution: duplicate

Issue created by migration from https://trac.sagemath.org/ticket/7075





---

archive/issue_comments_067189.json:
```json
{
    "body": "Based on 4.1.2.alpha4",
    "created_at": "2009-09-29T14:02:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7075#issuecomment-67189",
    "user": "https://github.com/kcrisman"
}
```

Based on 4.1.2.alpha4



---

archive/issue_comments_067190.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [trac_7075-save-fig-smart.patch](tarball://root/attachments/some-uuid/ticket7075/trac_7075-save-fig-smart.patch) by @kcrisman created at 2009-09-29 14:05:38\n\nTo release manager: this is a duplicate of #7059, please close.",
    "created_at": "2009-09-29T14:05:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7075#issuecomment-67190",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'></a>Attachment [trac_7075-save-fig-smart.patch](tarball://root/attachments/some-uuid/ticket7075/trac_7075-save-fig-smart.patch) by @kcrisman created at 2009-09-29 14:05:38

To release manager: this is a duplicate of #7059, please close.



---

archive/issue_comments_067191.json:
```json
{
    "body": "Changing author from \"\" to \"Karl-Dieter Crisman\"",
    "created_at": "2009-09-29T14:05:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7075#issuecomment-67191",
    "user": "https://github.com/kcrisman"
}
```

Changing author from "" to "Karl-Dieter Crisman"



---

archive/issue_comments_067192.json:
```json
{
    "body": "Changing author from \"Karl-Dieter Crisman\" to \"\"",
    "created_at": "2009-09-29T14:05:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7075#issuecomment-67192",
    "user": "https://github.com/kcrisman"
}
```

Changing author from "Karl-Dieter Crisman" to ""



---

archive/issue_comments_067193.json:
```json
{
    "body": "<a id='comment:3'></a>To Minh (if it's not October 1 where you are): please close.",
    "created_at": "2009-09-30T14:58:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7075#issuecomment-67193",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>To Minh (if it's not October 1 where you are): please close.



---

archive/issue_events_016733.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-30T15:01:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7075",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7075#event-16733"
}
```



---

archive/issue_events_016734.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-30T15:01:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7075",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7075#event-16734"
}
```



---

archive/issue_comments_067194.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2009-09-30T15:01:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7075#issuecomment-67194",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Resolution: duplicate



---

archive/issue_comments_067195.json:
```json
{
    "body": "<a id='comment:4'></a>Closing this ticket as a duplicate of #7059.",
    "created_at": "2009-09-30T15:01:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7075#issuecomment-67195",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>Closing this ticket as a duplicate of #7059.
