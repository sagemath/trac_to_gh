# Issue 7679: constructions doc: Python functional programming for mathematicians

archive/issues_007679.json:
```json
{
    "assignees": [],
    "body": "Add a section to the [Constructions](http://www.sagemath.org/doc/constructions/) document with tips on Python functional programming.\n\nAssignee: **@sagetrac-mvngu**\n\nKeywords: **functional programming, constructions document**\n\nAuthor: **Minh Van Nguyen**\n\nReviewer: **John Cremona**\n\nMerged: **sage-4.3.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7679_\n\n",
    "closed_at": "2009-12-14T16:09:57Z",
    "created_at": "2009-12-14T02:21:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "constructions doc: Python functional programming for mathematicians",
    "type": "issue",
    "updated_at": "2009-12-14T16:09:57Z",
    "url": "https://github.com/sagemath/sage/issues/7679",
    "user": "https://github.com/sagetrac-mvngu"
}
```
Add a section to the [Constructions](http://www.sagemath.org/doc/constructions/) document with tips on Python functional programming.

Assignee: **@sagetrac-mvngu**

Keywords: **functional programming, constructions document**

Author: **Minh Van Nguyen**

Reviewer: **John Cremona**

Merged: **sage-4.3.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/7679_





---

archive/issue_events_086134.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-12-14T02:21:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7679#event-86134"
}
```



---

archive/issue_events_086135.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-12-14T02:21:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7679#event-86135"
}
```



---

archive/issue_events_086136.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-12-14T02:21:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7679#event-86136"
}
```



---

archive/issue_events_086137.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-12-14T02:21:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7679#event-86137"
}
```



---

archive/issue_comments_059170.json:
```json
{
    "body": "Attachment: **[trac_7679-functional-programming.patch.gz](https://github.com/sagemath/sage/files/ticket7679/trac_7679-functional-programming.patch.gz)**\n\nbased on Sage 4.3.rc0",
    "created_at": "2009-12-14T03:36:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7679#issuecomment-59170",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_7679-functional-programming.patch.gz](https://github.com/sagemath/sage/files/ticket7679/trac_7679-functional-programming.patch.gz)**

based on Sage 4.3.rc0



---

archive/issue_comments_059171.json:
```json
{
    "body": "Author: **Minh Van Nguyen**",
    "created_at": "2009-12-14T03:37:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7679#issuecomment-59171",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Minh Van Nguyen**



---

archive/issue_events_086138.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-12-14T03:37:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7679#event-86138"
}
```



---

archive/issue_comments_059172.json:
```json
{
    "body": "Changed keywords from **** to **functional programming, constructions document**",
    "created_at": "2009-12-14T03:37:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7679#issuecomment-59172",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed keywords from **** to **functional programming, constructions document**



---

archive/issue_comments_059173.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good, applies fine to 4.3.rc0, the new section complies fine, tests ok and looks good.",
    "created_at": "2009-12-14T09:23:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7679#issuecomment-59173",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good, applies fine to 4.3.rc0, the new section complies fine, tests ok and looks good.



---

archive/issue_events_086139.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-12-14T09:23:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7679#event-86139"
}
```



---

archive/issue_events_086140.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-12-14T09:23:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7679#event-86140"
}
```



---

archive/issue_comments_059174.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2009-12-14T16:09:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7679#issuecomment-59174",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **John Cremona**



---

archive/issue_comments_059175.json:
```json
{
    "body": "Merged: **sage-4.3.rc1**",
    "created_at": "2009-12-14T16:09:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7679#issuecomment-59175",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.rc1**



---

archive/issue_events_086141.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-14T16:09:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7679#event-86141"
}
```



---

archive/issue_events_086142.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-14T16:09:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7679",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7679#event-86142"
}
```
