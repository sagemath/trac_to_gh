# Issue 7967: DeprecationWarning for popen3 in hg_sage

archive/issues_007967.json:
```json
{
    "assignees": [],
    "body": "Using `hg_sage` is raising a `DeprecationWarning`:\n\n```\n/data/sage/sage-4.3.1.rc0/local/lib/python2.6/site-packages/sage/misc/hg.py:240: DeprecationWarning: os.popen3 is deprecated.  Use the subprocess module.\n  x = os.popen3(s)\n```\n\nReviewer: **Tim Dumol**\n\nAuthor: **Willem Jan Palenstijn**\n\nMerged: **sage-4.3.1.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7967_\n\n",
    "closed_at": "2010-01-19T00:55:56Z",
    "created_at": "2010-01-17T18:34:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "DeprecationWarning for popen3 in hg_sage",
    "type": "issue",
    "updated_at": "2016-10-31T16:49:32Z",
    "url": "https://github.com/sagemath/sage/issues/7967",
    "user": "https://github.com/wjp"
}
```
Using `hg_sage` is raising a `DeprecationWarning`:

```
/data/sage/sage-4.3.1.rc0/local/lib/python2.6/site-packages/sage/misc/hg.py:240: DeprecationWarning: os.popen3 is deprecated.  Use the subprocess module.
  x = os.popen3(s)
```

Reviewer: **Tim Dumol**

Author: **Willem Jan Palenstijn**

Merged: **sage-4.3.1.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/7967_





---

archive/issue_events_087299.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T18:34:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7967#event-87299"
}
```



---

archive/issue_events_087300.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T18:34:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7967#event-87300"
}
```



---

archive/issue_events_087301.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T18:34:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7967#event-87301"
}
```



---

archive/issue_events_087302.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T18:36:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7967#event-87302"
}
```



---

archive/issue_events_087303.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T18:36:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7967#event-87303"
}
```



---

archive/issue_comments_062604.json:
```json
{
    "body": "Reviewer: **Tim Dumol**",
    "created_at": "2010-01-17T19:19:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7967#issuecomment-62604",
    "user": "https://github.com/TimDumol"
}
```

Reviewer: **Tim Dumol**



---

archive/issue_comments_062605.json:
```json
{
    "body": "Author: **Willem Palenstijn**",
    "created_at": "2010-01-17T19:19:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7967#issuecomment-62605",
    "user": "https://github.com/TimDumol"
}
```

Author: **Willem Palenstijn**



---

archive/issue_comments_062606.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[7967_popen3.patch.gz](https://github.com/sagemath/sage/files/ticket7967/7967_popen3.patch.gz)**\n\nGood job; looks good.",
    "created_at": "2010-01-17T19:19:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7967#issuecomment-62606",
    "user": "https://github.com/TimDumol"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[7967_popen3.patch.gz](https://github.com/sagemath/sage/files/ticket7967/7967_popen3.patch.gz)**

Good job; looks good.



---

archive/issue_events_087304.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-01-17T19:19:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7967#event-87304"
}
```



---

archive/issue_events_087305.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-01-17T19:19:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7967#event-87305"
}
```



---

archive/issue_comments_062607.json:
```json
{
    "body": "Merged: **sage-4.3.1.rc1**",
    "created_at": "2010-01-19T00:55:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7967#issuecomment-62607",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.3.1.rc1**



---

archive/issue_events_087306.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-01-19T00:55:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7967#event-87306"
}
```



---

archive/issue_events_087307.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-01-19T00:55:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7967#event-87307"
}
```



---

archive/issue_comments_062608.json:
```json
{
    "body": "Changed author from **Willem Palenstijn** to **Willem Jan Palenstijn**",
    "created_at": "2016-10-31T16:49:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7967#issuecomment-62608",
    "user": "https://github.com/fchapoton"
}
```

Changed author from **Willem Palenstijn** to **Willem Jan Palenstijn**
