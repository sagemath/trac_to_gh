# Issue 7917: make gauss_sum() work for dirichlet characters when the base ring is CC

archive/issues_007917.json:
```json
{
    "body": "This is a pretty small change. We add three lines to gauss_sum_numerical() to make it work when the base ring is a complex field, and change gauss_sum() to call gauss_sum_numerical() when the base ring is a complex field.\n\nNote that it is actually much faster to compute the (approximate) gauss sum when the base ring is CC, as compared to when the base ring is a cyclotomic field.\n\n\n```\nsage: G = DirichletGroup(2981)\nsage: chi = G.0\nsage: timeit('chi.gauss_sum_numerical()')\n5 loops, best of 3: 1.82 s per loop\nsage: G = DirichletGroup(2981, ComplexField(200))\nsage: chi = G.0                                  \nsage: timeit('chi.gauss_sum_numerical()')        \n25 loops, best of 3: 23.5 ms per loop\n```\n\n\n**Assignee:** @williamstein\n\n**CC:**  @rishikesha\n\n**Keywords:** dirichlet character gauss sum\n\n**Reviewer:** Peter Bruin\n\nIssue created by migration from https://trac.sagemath.org/ticket/7917\n\n",
    "closed_at": "2015-09-12T13:58:21Z",
    "created_at": "2010-01-13T06:22:56Z",
    "labels": [
        "component: number theory",
        "enhancement",
        "invalid"
    ],
    "title": "make gauss_sum() work for dirichlet characters when the base ring is CC",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7917",
    "user": "https://trac.sagemath.org/admin/accounts/users/bober"
}
```
This is a pretty small change. We add three lines to gauss_sum_numerical() to make it work when the base ring is a complex field, and change gauss_sum() to call gauss_sum_numerical() when the base ring is a complex field.

Note that it is actually much faster to compute the (approximate) gauss sum when the base ring is CC, as compared to when the base ring is a cyclotomic field.


```
sage: G = DirichletGroup(2981)
sage: chi = G.0
sage: timeit('chi.gauss_sum_numerical()')
5 loops, best of 3: 1.82 s per loop
sage: G = DirichletGroup(2981, ComplexField(200))
sage: chi = G.0                                  
sage: timeit('chi.gauss_sum_numerical()')        
25 loops, best of 3: 23.5 ms per loop
```


**Assignee:** @williamstein

**CC:**  @rishikesha

**Keywords:** dirichlet character gauss sum

**Reviewer:** Peter Bruin

Issue created by migration from https://trac.sagemath.org/ticket/7917





---

archive/attachments_009693.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "gauss_sum_CC.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket7917/gauss_sum_CC.patch",
    "created_at": "2010-08-02T21:54:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/rishikesha"
}
```



---

archive/issue_comments_063603.json:
```json
{
    "body": "<a id='comment:1'></a>\n",
    "created_at": "2010-08-02T21:54:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7917#issuecomment-63603",
    "user": "https://github.com/rishikesha"
}
```

<a id='comment:1'></a>




---

archive/issue_comments_063604.json:
```json
{
    "body": "<a id='comment:2'></a>\nShould this be marked as needs review?",
    "created_at": "2012-09-17T05:37:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7917#issuecomment-63604",
    "user": "https://github.com/roed314"
}
```

<a id='comment:2'></a>
Should this be marked as needs review?



---

archive/issue_events_056200.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56200"
}
```



---

archive/issue_events_056201.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56201"
}
```



---

archive/issue_events_056202.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56202"
}
```



---

archive/issue_events_056203.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56203"
}
```



---

archive/issue_events_056204.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56204"
}
```



---

archive/issue_events_056205.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56205"
}
```



---

archive/issue_events_056206.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56206"
}
```



---

archive/issue_events_056207.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56207"
}
```



---

archive/issue_events_056208.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2015-08-19T14:28:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56208"
}
```



---

archive/issue_events_056209.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2015-08-19T14:28:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56209"
}
```



---

archive/issue_comments_063605.json:
```json
{
    "body": "**Changing author** from \"bober\" to \"\".",
    "created_at": "2015-08-19T14:28:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7917#issuecomment-63605",
    "user": "https://github.com/pjbruin"
}
```

**Changing author** from "bober" to "".



---

archive/issue_comments_063606.json:
```json
{
    "body": "<a id='comment:7'></a>\nThis is superseded by #7191; I propose to close this as a duplicate.  There is only one small problem, namely that in the lines\n\n```python\nif rings.is_ComplexField(K):\n    return self.gauss_sum_numerical(a=a)\n```\nthe argument `a` was omitted in #7191.  This will be fixed in #19056.",
    "created_at": "2015-08-19T14:28:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7917#issuecomment-63606",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:7'></a>
This is superseded by #7191; I propose to close this as a duplicate.  There is only one small problem, namely that in the lines

```python
if rings.is_ComplexField(K):
    return self.gauss_sum_numerical(a=a)
```
the argument `a` was omitted in #7191.  This will be fixed in #19056.



---

archive/issue_events_056210.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2015-08-19T14:28:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56210"
}
```



---

archive/issue_comments_063607.json:
```json
{
    "body": "**Reviewer:** Peter Bruin",
    "created_at": "2015-08-19T14:28:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7917#issuecomment-63607",
    "user": "https://github.com/pjbruin"
}
```

**Reviewer:** Peter Bruin



---

archive/issue_comments_063608.json:
```json
{
    "body": "<a id='comment:8'></a>\nNow that #19056 is closed, I see no reason to keep this ticket open.",
    "created_at": "2015-09-07T11:55:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7917#issuecomment-63608",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:8'></a>
Now that #19056 is closed, I see no reason to keep this ticket open.



---

archive/issue_events_056211.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2015-09-07T11:55:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56211"
}
```



---

archive/issue_events_056212.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2015-09-07T11:55:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56212"
}
```



---

archive/issue_events_056213.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T13:58:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56213"
}
```



---

archive/issue_events_056214.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T13:58:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7917",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7917#event-56214"
}
```
