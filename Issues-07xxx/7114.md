# Issue 7114: cryptanalysis of classical ciphers

archive/issues_007114.json:
```json
{
    "body": "Implement cryptanalytic techniques for the following classical ciphers:\n\n* Hill or matrix cipher\n* #7123 shift cipher\n* #7124 affine cipher\n* #15756 substitution cipher\n* transposition cipher\n* #7231 Vigenere cipher\n\nThis depends on #6841 and #7010 (in that order).\nAlso depends on #15772\n\n**CC:**  @rbeezer @rwst\n\n**Keywords:** cryptanalysis\n\nIssue created by migration from https://trac.sagemath.org/ticket/7114\n\n",
    "created_at": "2009-10-04T17:33:49Z",
    "labels": [
        "component: cryptography",
        "enhancement",
        "feature"
    ],
    "title": "cryptanalysis of classical ciphers",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7114",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
Implement cryptanalytic techniques for the following classical ciphers:

* Hill or matrix cipher
* #7123 shift cipher
* #7124 affine cipher
* #15756 substitution cipher
* transposition cipher
* #7231 Vigenere cipher

This depends on #6841 and #7010 (in that order).
Also depends on #15772

**CC:**  @rbeezer @rwst

**Keywords:** cryptanalysis

Issue created by migration from https://trac.sagemath.org/ticket/7114





---

archive/issue_comments_052732.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -5,3 +5,5 @@\n * substitution cipher\n * transposition cipher\n * Vigenere cipher\n+\n+This depends on #6841 and #7010 (in that order).\n``````\n",
    "created_at": "2009-10-05T10:14:11Z",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7114#issuecomment-52732",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -5,3 +5,5 @@
 * substitution cipher
 * transposition cipher
 * Vigenere cipher
+
+This depends on #6841 and #7010 (in that order).
``````




---

archive/issue_comments_052733.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,7 +1,7 @@\n Implement cryptanalytic techniques for the following classical ciphers:\n \n * Hill or matrix cipher\n-* shift cipher\n+* #7123 shift cipher\n * substitution cipher\n * transposition cipher\n * Vigenere cipher\n``````\n",
    "created_at": "2009-10-05T16:05:15Z",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7114#issuecomment-52733",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,7 +1,7 @@
 Implement cryptanalytic techniques for the following classical ciphers:
 
 * Hill or matrix cipher
-* shift cipher
+* #7123 shift cipher
 * substitution cipher
 * transposition cipher
 * Vigenere cipher
``````




---

archive/issue_comments_052734.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -2,6 +2,7 @@\n \n * Hill or matrix cipher\n * #7123 shift cipher\n+* #7124 affine cipher\n * substitution cipher\n * transposition cipher\n * Vigenere cipher\n``````\n",
    "created_at": "2009-10-05T16:14:26Z",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7114#issuecomment-52734",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -2,6 +2,7 @@
 
 * Hill or matrix cipher
 * #7123 shift cipher
+* #7124 affine cipher
 * substitution cipher
 * transposition cipher
 * Vigenere cipher
``````




---

archive/issue_comments_052735.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -5,6 +5,6 @@\n * #7124 affine cipher\n * substitution cipher\n * transposition cipher\n-* Vigenere cipher\n+* #7231 Vigenere cipher\n \n This depends on #6841 and #7010 (in that order).\n``````\n",
    "created_at": "2009-10-16T00:53:44Z",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7114#issuecomment-52735",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -5,6 +5,6 @@
 * #7124 affine cipher
 * substitution cipher
 * transposition cipher
-* Vigenere cipher
+* #7231 Vigenere cipher
 
 This depends on #6841 and #7010 (in that order).
``````




---

archive/issue_events_047591.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7114#event-47591"
}
```



---

archive/issue_events_047592.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7114#event-47592"
}
```



---

archive/issue_comments_052736.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -3,7 +3,7 @@\n * Hill or matrix cipher\n * #7123 shift cipher\n * #7124 affine cipher\n-* substitution cipher\n+* #15756 substitution cipher\n * transposition cipher\n * #7231 Vigenere cipher\n \n``````\n",
    "created_at": "2014-01-29T05:31:38Z",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7114#issuecomment-52736",
    "user": "https://github.com/rwst"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -3,7 +3,7 @@
 * Hill or matrix cipher
 * #7123 shift cipher
 * #7124 affine cipher
-* substitution cipher
+* #15756 substitution cipher
 * transposition cipher
 * #7231 Vigenere cipher
 
``````




---

archive/issue_events_047593.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7114#event-47593"
}
```



---

archive/issue_events_047594.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7114#event-47594"
}
```



---

archive/issue_comments_052737.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -8,3 +8,4 @@\n * #7231 Vigenere cipher\n \n This depends on #6841 and #7010 (in that order).\n+Also depends on #15772\n``````\n",
    "created_at": "2014-02-01T11:17:10Z",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7114#issuecomment-52737",
    "user": "https://github.com/rwst"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -8,3 +8,4 @@
 * #7231 Vigenere cipher
 
 This depends on #6841 and #7010 (in that order).
+Also depends on #15772
``````




---

archive/issue_events_047595.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-16T10:26:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7114",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7114#event-47595"
}
```
