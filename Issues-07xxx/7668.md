# Issue 7668: Play waveforms directly, like in Mathematica

archive/issues_007668.json:
```json
{
    "assignees": [],
    "body": "This was requested by a participant at the first Sage Education Day.  I'm putting this in notebook but I really can't think of any appropriate component.\n\n**Assignee:** @williamstein\n\n**CC:**  @jasongrout @fchapoton\n\nIssue created by migration from https://trac.sagemath.org/ticket/7668\n\n",
    "closed_at": "2020-09-01T15:50:16Z",
    "created_at": "2009-12-11T20:26:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Play waveforms directly, like in Mathematica",
    "type": "issue",
    "updated_at": "2020-09-01T23:05:37Z",
    "url": "https://github.com/sagemath/sage/issues/7668",
    "user": "https://github.com/kcrisman"
}
```
This was requested by a participant at the first Sage Education Day.  I'm putting this in notebook but I really can't think of any appropriate component.

**Assignee:** @williamstein

**CC:**  @jasongrout @fchapoton

Issue created by migration from https://trac.sagemath.org/ticket/7668





---

archive/issue_comments_059021.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis would not be terribly hard to do using the python wave module.  See this interact for a basic demo:\n\nhttp://wiki.sagemath.org/interact/misc#Hearingatrigonometricidentity\n\nIt would be good to wrap this functionality in a more user-friendly command.",
    "created_at": "2009-12-11T20:59:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7668#issuecomment-59021",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<a id='comment:1'></a>
This would not be terribly hard to do using the python wave module.  See this interact for a basic demo:

http://wiki.sagemath.org/interact/misc#Hearingatrigonometricidentity

It would be good to wrap this functionality in a more user-friendly command.



---

archive/issue_comments_059022.json:
```json
{
    "body": "<a id='comment:2'></a>\nAlso, don't forget about",
    "created_at": "2009-12-11T21:56:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7668#issuecomment-59022",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
Also, don't forget about



---

archive/issue_comments_059023.json:
```json
{
    "body": "<a id='comment:3'></a>\nI meant to say about the scipy signal processing stuff, but then thought better about it.\n\nI think this would fit well into a widget framework to represent objects in the notebook.",
    "created_at": "2009-12-11T21:57:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7668#issuecomment-59023",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'></a>
I meant to say about the scipy signal processing stuff, but then thought better about it.

I think this would fit well into a widget framework to represent objects in the notebook.



---

archive/issue_comments_059024.json:
```json
{
    "body": "<a id='comment:4'></a>\nHere is another nice example of this - [this ask.sagemath](http://ask.sagemath.org/question/2542/sound-how-is-it-done) question as well as [this sage-devel discussion](https://groups.google.com/forum/?fromgroups=#!topic/sage-devel/3-Y1RUFkq14).",
    "created_at": "2013-05-08T01:48:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7668#issuecomment-59024",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:4'></a>
Here is another nice example of this - [this ask.sagemath](http://ask.sagemath.org/question/2542/sound-how-is-it-done) question as well as [this sage-devel discussion](https://groups.google.com/forum/?fromgroups=#!topic/sage-devel/3-Y1RUFkq14).



---

archive/issue_events_052583.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7668#event-52583"
}
```



---

archive/issue_comments_059025.json:
```json
{
    "body": "<a id='comment:5'></a>\nProposing to close all sagenb tickets as outdated, so that all remaining open tickets in the notebook component are about the Jupyter notebook.",
    "created_at": "2020-08-18T00:36:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7668#issuecomment-59025",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>
Proposing to close all sagenb tickets as outdated, so that all remaining open tickets in the notebook component are about the Jupyter notebook.



---

archive/issue_events_052584.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-01T15:50:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7668#event-52584"
}
```



---

archive/issue_events_052585.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-01T15:50:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7668#event-52585"
}
```



---

archive/issue_comments_059026.json:
```json
{
    "body": "<a id='comment:7'></a>\nThis could conceivably be implemented in Jupyter as well.",
    "created_at": "2020-09-01T21:34:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7668#issuecomment-59026",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'></a>
This could conceivably be implemented in Jupyter as well.



---

archive/issue_comments_059027.json:
```json
{
    "body": "<a id='comment:8'></a>\n(And it certainly fits the Sage mission statement.)",
    "created_at": "2020-09-01T21:34:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7668#issuecomment-59027",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:8'></a>
(And it certainly fits the Sage mission statement.)



---

archive/issue_comments_059028.json:
```json
{
    "body": "<a id='comment:9'></a>\nAnd arguably works already in Jupyter: https://ipython.readthedocs.io/en/stable/api/generated/IPython.display.html#IPython.display.Audio\n\nThis is used in one of the most popular ipywidgets examples: https://github.com/jupyter-widgets/ipywidgets/blob/master/docs/source/examples/Beat%20Frequencies.ipynb",
    "created_at": "2020-09-01T22:37:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7668#issuecomment-59028",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:9'></a>
And arguably works already in Jupyter: https://ipython.readthedocs.io/en/stable/api/generated/IPython.display.html#IPython.display.Audio

This is used in one of the most popular ipywidgets examples: https://github.com/jupyter-widgets/ipywidgets/blob/master/docs/source/examples/Beat%20Frequencies.ipynb



---

archive/issue_events_052586.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2020-09-01T23:05:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "08517b",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7668#event-52586"
}
```



---

archive/issue_events_052587.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2020-09-01T23:05:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "08517b",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7668#event-52587"
}
```



---

archive/issue_comments_059029.json:
```json
{
    "body": "<a id='comment:10'></a>\nNice! Wow, I wasn't expecting a reply from one of the core Jupyter developers ;-) though I guess he WAS one of the original people on this ticket :-)\n\nMaybe this ticket can then just be adding this to a likely place in the documentation.",
    "created_at": "2020-09-01T23:05:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7668#issuecomment-59029",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:10'></a>
Nice! Wow, I wasn't expecting a reply from one of the core Jupyter developers ;-) though I guess he WAS one of the original people on this ticket :-)

Maybe this ticket can then just be adding this to a likely place in the documentation.
