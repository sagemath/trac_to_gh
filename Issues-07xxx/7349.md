# Issue 7349: SageNB -- Include fixes from #6848 (introspection)

archive/issues_007349.json:
```json
{
    "body": "The fixes from #6848 have not been included in `sagenb.misc.sageinspect`. This ports them to SageNB\n\n**Assignee:** boothby\n\n**CC:**  @williamstein @qed777\n\nIssue created by migration from https://trac.sagemath.org/ticket/7349\n\n",
    "closed_at": "2009-11-11T19:51:43Z",
    "created_at": "2009-10-29T15:26:22Z",
    "labels": [
        "component: notebook",
        "bug",
        "duplicate"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "title": "SageNB -- Include fixes from #6848 (introspection)",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7349",
    "user": "https://github.com/TimDumol"
}
```
The fixes from #6848 have not been included in `sagenb.misc.sageinspect`. This ports them to SageNB

**Assignee:** boothby

**CC:**  @williamstein @qed777

Issue created by migration from https://trac.sagemath.org/ticket/7349





---

archive/issue_comments_055114.json:
```json
{
    "body": "Ports fixes from #6848 to SageNB",
    "created_at": "2009-10-29T15:28:04Z",
    "issue": "https://github.com/sagemath/sage/issues/7349",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7349#issuecomment-55114",
    "user": "https://github.com/TimDumol"
}
```

Ports fixes from #6848 to SageNB



---

archive/attachments_008816.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7349-6848-port.patch",
    "asset_url": "tarball://root/attachments/ticket7349/trac_7349-6848-port.patch",
    "created_at": "2009-10-29T17:23:13Z",
    "issue": "https://github.com/sagemath/sage/issues/7349",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7349/trac_7349-6848-port.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_055115.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_7349-6848-port.patch](https://github.com/sagemath/sage/files/ticket7349/trac_7349-6848-port.patch) by @jhpalmieri created at 2009-10-29 17:23:13\n\nI think that #7316 is the same issue, but the patch is here, so we should close the other one as a duplicate.",
    "created_at": "2009-10-29T17:23:13Z",
    "issue": "https://github.com/sagemath/sage/issues/7349",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7349#issuecomment-55115",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'></a>
Attachment [trac_7349-6848-port.patch](https://github.com/sagemath/sage/files/ticket7349/trac_7349-6848-port.patch) by @jhpalmieri created at 2009-10-29 17:23:13

I think that #7316 is the same issue, but the patch is here, so we should close the other one as a duplicate.



---

archive/issue_comments_055116.json:
```json
{
    "body": "<a id='comment:2'></a>\nPlease close this ticket as a duplicate. #7316 fixes the doctests, which I have neglected to fix.",
    "created_at": "2009-10-29T17:59:57Z",
    "issue": "https://github.com/sagemath/sage/issues/7349",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7349#issuecomment-55116",
    "user": "https://github.com/TimDumol"
}
```

<a id='comment:2'></a>
Please close this ticket as a duplicate. #7316 fixes the doctests, which I have neglected to fix.



---

archive/issue_events_049550.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-11T19:51:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7349",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7349#event-49550"
}
```



---

archive/issue_events_049551.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-11T19:51:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7349",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7349#event-49551"
}
```



---

archive/issue_comments_055117.json:
```json
{
    "body": "<a id='comment:3'></a>\nThis is a dup of #7316.",
    "created_at": "2009-11-11T19:51:43Z",
    "issue": "https://github.com/sagemath/sage/issues/7349",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7349#issuecomment-55117",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
This is a dup of #7316.



---

archive/issue_events_049552.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-11T19:51:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7349",
    "milestone": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7349#event-49552"
}
```



---

archive/issue_events_049553.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-11T19:51:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7349",
    "milestone": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7349#event-49553"
}
```
