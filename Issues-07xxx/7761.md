...
```json
{
    "body": "<a id='comment:1'></a>These problems go away if one builds in 64-bit mode, by exporting SAGE64 to yes. However, CFLAGS must be passed properly to Python, otherwise the -m64 does not get added as a CFLAG. That was only happening on OS X. \n\nThis updated spkg file adds:\n\n```\nCC=\"$CC $CFLAGS\"\n```\non the end of the configure line. Then, as long as CFLAGS contains -m64, so the package will build with 64-bit support. \n\nhttp://boxen.math.washington.edu/home/kirkby/portability/python-2.6.2.p5/",
...
<a id='comment:1'></a>These problems go away if one builds in 64-bit mode, by exporting SAGE64 to yes. However, CFLAGS must be passed properly to Python, otherwise the -m64 does not get added as a CFLAG. That was only happening on OS X. 
...
<a id='comment:2'></a>On my Open Solaris build still fails:
...
<a id='comment:3'></a>That is very odd. I just used OpenSSL's default location, which is /usr/local/ssl. Python knows to look there. I manged to get the following all built now in 64-bit mode.  
...
<a id='comment:5'></a>> .rodata.str1.1 (merged string section) 0x53e            /usr/local/lib/libcrypto.a(bn_sqrt.o)
> ld: fatal: relocations remain against allocatable but non-writable sections
> collect2: ld returned 1 exit status
> 
> }}}
...
```json
{
    "body": "<a id='comment:6'></a>I'm sticking this to 'needs info' as I've noticed another problem in this package (incorrect usage of set -e), and might as well fix the other one at the same time.",
...
<a id='comment:6'></a>I'm sticking this to 'needs info' as I've noticed another problem in this package (incorrect usage of set -e), and might as well fix the other one at the same time.
...
<a id='comment:7'></a>I'm slowly making progress building p5 on my Open Solaris in VirtualBox :)
...
<a id='comment:8'></a>I finally got this going on Open Solaris in VirtualBox!
...
<a id='comment:9'></a>OK, I see you updated the spkg in
...
<a id='comment:11'></a>Updated package which 
...
<a id='comment:12'></a>On Open Solaris:
...
```json
{
    "body": "<a id='comment:13'></a>Too late for sage-3.4.1, but we'll still merge it ;-)",
    "created_at": "2010-01-13T06:10:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7761",
    "type": "issue_comment",
...
<a id='comment:13'></a>Too late for sage-3.4.1, but we'll still merge it ;-)
...
<a id='comment:15'></a>Does this still rely on the (unresolved) #7818? Distutils pulls CFLAGS out of the makefile, so I'm not convince that 
...
<a id='comment:17'></a>I'm just saying that the problems we have later on using setup.py to build extensions may have to do with the fact that Python is built with "covert" command line options that Distutils doesn't pick up. I figured this ticket needed to be re-opened 'cause if it never got in it's clearly shouldn't be closed. (At this point, we should probably make a p6 spkg.) I agree with the general goal behind #7818, but it has issues that I don't know how to fix, so shouldn't be a dependancy (unless that gets worked out first).
...
<a id='comment:18'></a>I can understand why this was not merged. I'm just in the process of updating #7818. I just need to double check everything. But the main change is that 
...
```json
{
    "body": "<a id='comment:20'></a>As a work around I changed Darwin to SunOS. This give a working cython!\n\nChecking other packages with python setup.py install.\n\nJaap",
...
<a id='comment:20'></a>As a work around I changed Darwin to SunOS. This give a working cython!
...
```json
{
    "body": "<a id='comment:21'></a>That seems fine. Whatever allows you to make progress. Specific issues can be sorted later. There are some quite interesting, and more complex tasks, such as #6028 to be solved. You might want to take a look at something more interesting, if you get fed up with adding -m64!\n\nDave",
...
<a id='comment:21'></a>That seems fine. Whatever allows you to make progress. Specific issues can be sorted later. There are some quite interesting, and more complex tasks, such as #6028 to be solved. You might want to take a look at something more interesting, if you get fed up with adding -m64!
...
<a id='comment:22'></a>Attachment [python-2.6.4.p6.patch](tarball://root/attachments/some-uuid/ticket7761/python-2.6.4.p6.patch) by @jaapspies created at 2010-02-23 14:33:49
...
<a id='comment:23'></a>I see no reason this should not work on Linux, but have you checked it? I know  you have a linux build of Sage. Assuming you have checked it on Linux, then I'm happy to give it a positive review. It looks fine to me. 
...
<a id='comment:24'></a>Replying to [drkirkby](#comment%3A23):
> I see no reason this should not work on Linux, but have you checked it? I know  you have a linux build of Sage. Assuming you have checked it on Linux, then I'm happy to give it a positive review. It looks fine to me. 
> 
> It does build on Solaris 10 in 32-bit mode on SPARC. I've not tested 64-bit mode, but there is currently no effort being put into a 64-bit port on SPARC, so there is no need for it to work there. 
> 
...
```json
{
    "body": "<a id='comment:25'></a>No problem. I think it would be ok, but there is no harm in checking. \n\nPositive review.",
    "created_at": "2010-02-23T18:36:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7761",
    "type": "issue_comment",
...
<a id='comment:25'></a>No problem. I think it would be ok, but there is no harm in checking. 
...
```json
{
    "body": "<a id='comment:26'></a>I'm taking myself off this as author, and adding myself as reviewer, as these changes by Jaap are his, and my earlier version was not working completely, so I feel justified in reviewing this. \n\nI'm going to delete the version I have on boxen, so not to cause any confusion. \n\nDave",
    "created_at": "2010-02-23T18:41:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7761",
    "type": "issue_comment",
...
<a id='comment:26'></a>I'm taking myself off this as author, and adding myself as reviewer, as these changes by Jaap are his, and my earlier version was not working completely, so I feel justified in reviewing this. 
...
```json
{
    "body": "<a id='comment:27'></a>I see this ticket is marked as: Ticket #7761 (positive_review defect: fixed)\n\nFixed? At some time it was merged, but reopened.\n\nJaap",
...
<a id='comment:27'></a>I see this ticket is marked as: Ticket #7761 (positive_review defect: fixed)
...
```json
{
    "body": "<a id='comment:28'></a>Yes, it got merged, but was changed by robertwb  from fixed to needs work. It relied on another patch (#7818) which itself caused problems by setting CFLAGS globally. So whilst my python version worked, the patch it relied on caused other problems. \n\nDave",
    "created_at": "2010-02-23T20:02:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7761",
    "type": "issue_comment",
...
<a id='comment:28'></a>Yes, it got merged, but was changed by robertwb  from fixed to needs work. It relied on another patch (#7818) which itself caused problems by setting CFLAGS globally. So whilst my python version worked, the patch it relied on caused other problems. 
...
```json
{
    "body": "<a id='comment:29'></a>Merged [python-2.6.4.p6.spkg](http://boxen.math.washington.edu/home/jsp/ports/python-2.6.4.p6.spkg) in the standard spkg repository.",
    "created_at": "2010-03-02T23:31:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7761",
    "type": "issue_comment",
...
<a id='comment:29'></a>Merged [python-2.6.4.p6.spkg](http://boxen.math.washington.edu/home/jsp/ports/python-2.6.4.p6.spkg) in the standard spkg repository.
