# Issue 7320: search_src and friends are case-sensitive

archive/issues_007320.json:
```json
{
    "body": "The functions `search_src`, `search_def`, and `search_src` are case-sensitive and have been for a while.  The documentation says that they're not.  This patch changes the documentation to reflect this, and adds one doctest to verify it.\n\n**Assignee:** @jhpalmieri\n\n**CC:**  mvngu\n\n**Author:** John Palmieri\n\n**Reviewer:** Karl-Dieter Crisman\n\n**Merged:** sage-4.2.1.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/7320\n\n",
    "closed_at": "2009-10-31T15:50:45Z",
    "created_at": "2009-10-27T05:15:26Z",
    "labels": [
        "component: misc",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "title": "search_src and friends are case-sensitive",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7320",
    "user": "https://github.com/jhpalmieri"
}
```
The functions `search_src`, `search_def`, and `search_src` are case-sensitive and have been for a while.  The documentation says that they're not.  This patch changes the documentation to reflect this, and adds one doctest to verify it.

**Assignee:** @jhpalmieri

**CC:**  mvngu

**Author:** John Palmieri

**Reviewer:** Karl-Dieter Crisman

**Merged:** sage-4.2.1.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/7320





---

archive/attachments_008769.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7320-case-sensitive.patch",
    "asset_url": "tarball://root/attachments/ticket7320/trac_7320-case-sensitive.patch",
    "created_at": "2009-10-27T05:15:54Z",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7320/trac_7320-case-sensitive.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_054770.json:
```json
{
    "body": "Attachment [trac_7320-case-sensitive.patch](https://github.com/sagemath/sage/files/ticket7320/trac_7320-case-sensitive.patch) by @jhpalmieri created at 2009-10-27 05:15:54",
    "created_at": "2009-10-27T05:15:54Z",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7320#issuecomment-54770",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment [trac_7320-case-sensitive.patch](https://github.com/sagemath/sage/files/ticket7320/trac_7320-case-sensitive.patch) by @jhpalmieri created at 2009-10-27 05:15:54



---

archive/issue_events_049280.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-10-27T05:16:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7320#event-49280"
}
```



---

archive/issue_events_049281.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-29T18:31:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7320#event-49281"
}
```



---

archive/issue_events_049282.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-29T18:31:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7320#event-49282"
}
```



---

archive/issue_comments_054771.json:
```json
{
    "body": "**Reviewer:** Karl-Dieter Crisman",
    "created_at": "2009-10-29T18:31:07Z",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7320#issuecomment-54771",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Karl-Dieter Crisman



---

archive/issue_comments_054772.json:
```json
{
    "body": "<a id='comment:2'></a>\nJust out of curiosity, is it possible to change the behavior?  Obviously that would be a different ticket.  That could be useful (or not).  Maybe a function that is, and another one that isn't...\n\nThe new patch is just making the notation for \"case-sensitive\" uniformly with hyphen, which seemed to be the majority of references in the doc; however, usage in general seems quite variable.  One word for the adjective seems right, though.  Maybe mvngu will have a comment, so I'm cc:ing him on this.\n\nI also added a little to the doctests to make sure we're really doctesting the right thing and compare the two possibilities.",
    "created_at": "2009-10-29T18:31:07Z",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7320#issuecomment-54772",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>
Just out of curiosity, is it possible to change the behavior?  Obviously that would be a different ticket.  That could be useful (or not).  Maybe a function that is, and another one that isn't...

The new patch is just making the notation for "case-sensitive" uniformly with hyphen, which seemed to be the majority of references in the doc; however, usage in general seems quite variable.  One word for the adjective seems right, though.  Maybe mvngu will have a comment, so I'm cc:ing him on this.

I also added a little to the doctests to make sure we're really doctesting the right thing and compare the two possibilities.



---

archive/issue_comments_054773.json:
```json
{
    "body": "Fixes a few things, adds doctest - use this",
    "created_at": "2009-10-29T18:31:54Z",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7320#issuecomment-54773",
    "user": "https://github.com/kcrisman"
}
```

Fixes a few things, adds doctest - use this



---

archive/attachments_008770.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7320-case-sensitive.2.patch",
    "asset_url": "tarball://root/attachments/ticket7320/trac_7320-case-sensitive.2.patch",
    "created_at": "2009-10-29T19:01:50Z",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7320/trac_7320-case-sensitive.2.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_054774.json:
```json
{
    "body": "<a id='comment:3'></a>\nAttachment [trac_7320-case-sensitive.2.patch](https://github.com/sagemath/sage/files/ticket7320/trac_7320-case-sensitive.2.patch) by @jhpalmieri created at 2009-10-29 19:01:50\n\nReplying to [kcrisman](#comment%3A2):\n> Just out of curiosity, is it possible to change the behavior?  Obviously that would be a different ticket.  That could be useful (or not).  Maybe a function that is, and another one that isn't...\n\n\nI think so: I think we can add a flag to the regular expression search to make it case-insensitive.  We could add a flag (like `ignore_case=False`) to the search functions so people could toggle this.  Of course, I haven't actually tried this, but the documentation for regular expression searches in Python suggests that it should be possible...",
    "created_at": "2009-10-29T19:01:50Z",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7320#issuecomment-54774",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'></a>
Attachment [trac_7320-case-sensitive.2.patch](https://github.com/sagemath/sage/files/ticket7320/trac_7320-case-sensitive.2.patch) by @jhpalmieri created at 2009-10-29 19:01:50

Replying to [kcrisman](#comment%3A2):
> Just out of curiosity, is it possible to change the behavior?  Obviously that would be a different ticket.  That could be useful (or not).  Maybe a function that is, and another one that isn't...


I think so: I think we can add a flag to the regular expression search to make it case-insensitive.  We could add a flag (like `ignore_case=False`) to the search functions so people could toggle this.  Of course, I haven't actually tried this, but the documentation for regular expression searches in Python suggests that it should be possible...



---

archive/issue_comments_054775.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [kcrisman](#comment%3A2):\n> Maybe mvngu will have a comment, so I'm cc:ing him on this.\n\n\nSometimes I want to search the source with case-sensitivity on. For example, I might be interested in knowing if the source has anything with the word \"Euler\" in it. As function and method names are lower-case, case-sensitivity search might not return the name of a function/method like \"euler_phi\". However, if the documentation for \"euler_phi\" has something like \"Euler phi function\" or \"Euler totient function\", then case-sensitivity search would pick up \"Euler\". Sometimes I want to do a case-insensitivity search. In that case (pun not intended), I would expect that both \"euler_phi\" and \"Euler\" be returned by the search. At the end of the day, one can make case-insensitivity search as default, but should also give people the option to do case-sensitivity search. My 2-cent.",
    "created_at": "2009-10-30T04:47:06Z",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7320#issuecomment-54775",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
Replying to [kcrisman](#comment%3A2):
> Maybe mvngu will have a comment, so I'm cc:ing him on this.


Sometimes I want to search the source with case-sensitivity on. For example, I might be interested in knowing if the source has anything with the word "Euler" in it. As function and method names are lower-case, case-sensitivity search might not return the name of a function/method like "euler_phi". However, if the documentation for "euler_phi" has something like "Euler phi function" or "Euler totient function", then case-sensitivity search would pick up "Euler". Sometimes I want to do a case-insensitivity search. In that case (pun not intended), I would expect that both "euler_phi" and "Euler" be returned by the search. At the end of the day, one can make case-insensitivity search as default, but should also give people the option to do case-sensitivity search. My 2-cent.



---

archive/issue_events_049283.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-31T15:50:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7320#event-49283"
}
```



---

archive/issue_events_049284.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-31T15:50:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7320#event-49284"
}
```



---

archive/issue_comments_054776.json:
```json
{
    "body": "**Merged:** sage-4.2.1.alpha0",
    "created_at": "2009-10-31T15:50:45Z",
    "issue": "https://github.com/sagemath/sage/issues/7320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7320#issuecomment-54776",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.2.1.alpha0
