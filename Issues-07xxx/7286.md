# Issue 7286: After installing sphinx-0.6.3.p1.spkg, error occurs during docbuild

archive/issues_007286.json:
```json
{
    "assignees": [],
    "body": "> 11:24 < williamstein> I tried \"sage -upgrade\" on a clean install (the systemwide one) on geom.math.\n\n> 11:24 < williamstein> It fails with:\n\n> 11:24 < williamstein>   File \"/usr/local/sage/local/lib/python2.6/site-packages/Sphinx-0.6.3-py2.6.egg/sphinx/environment.py\", line 204, in frompickle\n\n> 11:24 < williamstein>     env = pickle.load(picklefile)\n\n> 11:24 < williamstein> AttributeError: 'module' object has no attribute 'RedirStream'\n\nAssignee: **@sagetrac-mabshoff**\n\nAuthor: **Tim Dumol**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7286_\n\n",
    "closed_at": "2009-10-25T04:21:38Z",
    "created_at": "2009-10-25T03:53:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "After installing sphinx-0.6.3.p1.spkg, error occurs during docbuild",
    "type": "issue",
    "updated_at": "2009-10-29T07:12:07Z",
    "url": "https://github.com/sagemath/sage/issues/7286",
    "user": "https://github.com/TimDumol"
}
```
> 11:24 < williamstein> I tried "sage -upgrade" on a clean install (the systemwide one) on geom.math.

> 11:24 < williamstein> It fails with:

> 11:24 < williamstein>   File "/usr/local/sage/local/lib/python2.6/site-packages/Sphinx-0.6.3-py2.6.egg/sphinx/environment.py", line 204, in frompickle

> 11:24 < williamstein>     env = pickle.load(picklefile)

> 11:24 < williamstein> AttributeError: 'module' object has no attribute 'RedirStream'

Assignee: **@sagetrac-mabshoff**

Author: **Tim Dumol**

_Issue created by migration from https://trac.sagemath.org/ticket/7286_





---

archive/issue_events_080654.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-10-25T03:53:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "milestone_number": null,
    "milestone_title": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7286#event-80654"
}
```



---

archive/issue_events_080655.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-10-25T03:53:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7286#event-80655"
}
```



---

archive/issue_events_080656.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-10-25T03:53:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7286#event-80656"
}
```



---

archive/issue_events_080657.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-10-25T03:53:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7286#event-80657"
}
```



---

archive/issue_events_080658.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-10-25T03:58:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7286#event-80658"
}
```



---

archive/issue_comments_054257.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nHas the updated package: http://sage.math.washington.edu/home/timdumol/sphinx-0.6.3.p2.spkg",
    "created_at": "2009-10-25T03:58:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7286#issuecomment-54257",
    "user": "https://github.com/TimDumol"
}
```

<div id="comment:1" align="right">Comment 1</div>

Has the updated package: http://sage.math.washington.edu/home/timdumol/sphinx-0.6.3.p2.spkg



---

archive/issue_comments_054258.json:
```json
{
    "body": "Author: **Tim Dumol**",
    "created_at": "2009-10-25T03:58:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7286#issuecomment-54258",
    "user": "https://github.com/TimDumol"
}
```

Author: **Tim Dumol**



---

archive/issue_events_080659.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-25T04:21:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7286#event-80659"
}
```



---

archive/issue_events_080660.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-25T04:21:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7286#event-80660"
}
```



---

archive/issue_events_080661.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-25T04:21:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7286#event-80661"
}
```



---

archive/issue_events_080662.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-25T04:21:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7286",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7286#event-80662"
}
```
