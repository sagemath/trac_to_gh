# Issue 7081: sage -c "..." runs from the local/bin/ directory.  ugh

archive/issues_007081.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-cwitty"
    ],
    "body": "This is not good:\n\n```\nflat:sagenb wstein$ pwd\n/Users/wstein/sage/nb/sagenb\nflat:sagenb wstein$ sage -c \"print os.path.abspath('.')\"\n/Users/wstein/sage/build/64bit/sage/local/bin\n```\n\nIt should be when one runs \"sage -c\" that it runs in the *current* directory.  The actual behavior is very confusing.\n\nComponent: **misc**\n\nAuthor: **Tim Dumol**\n\nReviewer: **William Stein**\n\nMerged: **sage-4.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7081_\n\n",
    "closed_at": "2009-10-15T08:57:16Z",
    "created_at": "2009-09-30T09:10:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage -c \"...\" runs from the local/bin/ directory.  ugh",
    "type": "issue",
    "updated_at": "2009-10-15T08:57:16Z",
    "url": "https://github.com/sagemath/sage/issues/7081",
    "user": "https://github.com/williamstein"
}
```
This is not good:

```
flat:sagenb wstein$ pwd
/Users/wstein/sage/nb/sagenb
flat:sagenb wstein$ sage -c "print os.path.abspath('.')"
/Users/wstein/sage/build/64bit/sage/local/bin
```

It should be when one runs "sage -c" that it runs in the *current* directory.  The actual behavior is very confusing.

Component: **misc**

Author: **Tim Dumol**

Reviewer: **William Stein**

Merged: **sage-4.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/7081_





---

archive/issue_events_086222.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-30T09:10:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86222"
}
```



---

archive/issue_events_086223.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-30T09:10:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86223"
}
```



---

archive/issue_events_086224.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-30T09:10:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86224"
}
```



---

archive/issue_events_086225.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-30T09:10:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86225"
}
```



---

archive/issue_events_086226.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-09-30T09:10:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86226"
}
```



---

archive/issue_comments_050788.json:
```json
{
    "body": "Attachment: **[trac_7081-chdir-fix.patch.gz](https://github.com/sagemath/sage/files/ticket7081/trac_7081-chdir-fix.patch.gz)**\n\nApply to scripts repo.",
    "created_at": "2009-09-30T09:16:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7081#issuecomment-50788",
    "user": "https://github.com/TimDumol"
}
```

Attachment: **[trac_7081-chdir-fix.patch.gz](https://github.com/sagemath/sage/files/ticket7081/trac_7081-chdir-fix.patch.gz)**

Apply to scripts repo.



---

archive/issue_comments_050789.json:
```json
{
    "body": "Author: **Tim Dumol**",
    "created_at": "2009-09-30T09:17:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7081#issuecomment-50789",
    "user": "https://github.com/TimDumol"
}
```

Author: **Tim Dumol**



---

archive/issue_comments_050790.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nEditing $SAGE_LOCAL/bin/sage-eval to os.chdir(os.getenv('CUR')) before evalutation fixes the problem.",
    "created_at": "2009-09-30T09:17:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7081#issuecomment-50790",
    "user": "https://github.com/TimDumol"
}
```

<div id="comment:1" align="right">comment:1</div>

Editing $SAGE_LOCAL/bin/sage-eval to os.chdir(os.getenv('CUR')) before evalutation fixes the problem.



---

archive/issue_events_086227.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-09-30T09:17:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86227"
}
```



---

archive/issue_events_086228.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-09-30T09:17:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86228"
}
```



---

archive/issue_events_086229.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-30T09:19:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86229"
}
```



---

archive/issue_events_086230.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-30T09:19:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86230"
}
```



---

archive/issue_comments_050791.json:
```json
{
    "body": "Merged: **sage-4.2.alpha0**",
    "created_at": "2009-10-15T08:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7081#issuecomment-50791",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.alpha0**



---

archive/issue_events_086231.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-15T08:57:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86231"
}
```



---

archive/issue_events_086232.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-15T08:57:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7081#event-86232"
}
```



---

archive/issue_comments_050792.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2009-10-15T08:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7081#issuecomment-50792",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **William Stein**
