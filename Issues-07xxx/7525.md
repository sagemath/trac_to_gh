# Issue 7525: developer's guide: add information about editing python files in the sage library

archive/issues_007525.json:
```json
{
    "assignees": [],
    "body": "There have been a few questions on sage-support and sage-devel recently about which files to edit in the Sage library.  The attached patch tries to address this.  (It also deletes a paragraph which got included twice.)\n\n\nAssignee: **mvngu**\n\nReviewer: **Minh Van Nguyen**\n\nAuthor: **John Palmieri**\n\nMerged: **sage-4.3.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7525_\n\n",
    "closed_at": "2009-11-29T05:46:17Z",
    "created_at": "2009-11-24T18:11:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "developer's guide: add information about editing python files in the sage library",
    "type": "issue",
    "updated_at": "2017-07-19T08:52:40Z",
    "url": "https://github.com/sagemath/sage/issues/7525",
    "user": "https://github.com/jhpalmieri"
}
```
There have been a few questions on sage-support and sage-devel recently about which files to edit in the Sage library.  The attached patch tries to address this.  (It also deletes a paragraph which got included twice.)


Assignee: **mvngu**

Reviewer: **Minh Van Nguyen**

Author: **John Palmieri**

Merged: **sage-4.3.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/7525_





---

archive/issue_events_081433.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-11-24T18:11:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81433"
}
```



---

archive/issue_events_081434.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-11-24T18:11:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81434"
}
```



---

archive/issue_events_081435.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-11-24T18:11:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81435"
}
```



---

archive/issue_events_081436.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-11-24T18:11:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81436"
}
```



---

archive/issue_comments_057204.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_7525-developer.patch.gz](https://github.com/sagemath/sage/files/ticket7525/trac_7525-developer.patch.gz)**",
    "created_at": "2009-11-24T18:15:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-57204",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_7525-developer.patch.gz](https://github.com/sagemath/sage/files/ticket7525/trac_7525-developer.patch.gz)**



---

archive/issue_events_081437.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-11-24T18:15:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81437"
}
```



---

archive/issue_events_081438.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-25T05:30:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81438"
}
```



---

archive/issue_events_081439.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-25T05:30:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81439"
}
```



---

archive/issue_events_081440.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-25T05:30:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81440"
}
```



---

archive/issue_comments_057205.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2009-11-25T05:30:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-57205",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_comments_057206.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nApplied OK against Sage 4.3.alpha0 and rebuilding of the Developers' Guide went fine. The expository content of the patch `trac_7525-developer.patch` is reasonable. I can confirm that that patch also removes a duplicate section.",
    "created_at": "2009-11-25T05:30:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-57206",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:2'>Comment 2:</a>
Applied OK against Sage 4.3.alpha0 and rebuilding of the Developers' Guide went fine. The expository content of the patch `trac_7525-developer.patch` is reasonable. I can confirm that that patch also removes a duplicate section.



---

archive/issue_events_081441.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-29T05:46:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81441"
}
```



---

archive/issue_events_081442.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-29T05:46:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-81442"
}
```



---

archive/issue_comments_057207.json:
```json
{
    "body": "Merged: **sage-4.3.alpha1**",
    "created_at": "2009-11-29T05:46:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-57207",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.alpha1**



---

archive/issue_comments_057208.json:
```json
{
    "body": "Changed author from **John H. Palmieri** to **John Palmieri**",
    "created_at": "2017-07-19T08:52:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-57208",
    "user": "https://github.com/fchapoton"
}
```

Changed author from **John H. Palmieri** to **John Palmieri**



---

archive/issue_comments_057209.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nunique name please",
    "created_at": "2017-07-19T08:52:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-57209",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
unique name please
