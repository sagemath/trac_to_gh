# Issue 7742: add a compose function to sage

archive/issues_007742.json:
```json
{
    "assignees": [
        "https://github.com/aghitza"
    ],
    "body": "\n```\n\ndef compose(f, n, a):\n    \"\"\"\n    Return f(f(...f(a)...)), where the composition occurs n times.\n    \n    INPUT:\n        - `f` -- anything that is callable\n        - `n` -- a nonnegative integer\n        - `a` -- any input for `f`\n\n    OUTPUT:\n        result of composing `f` with itself `n` times and applying to `a`.\n\n    EXAMPLES::\n\n        sage: def f(x): return x^2 + 1\n        sage: x = var('x')\n        sage: compose(f, 3, x)\n        ((x^2 + 1)^2 + 1)^2 + 1\n    \"\"\"\n    n = Integer(n)\n    if n <= 0: return a\n    a = f(a)\n    for i in range(n-1): a = f(a)\n    return a\n\n```\nTo the release manager: apply only the last patch\n(trac_7742-add-compose-etc_v2.1.patch).\n\nComponent: **misc**\n\nAuthor: **Christopher Olah, Felix Lawrence**\n\nReviewer: **Paul Zimmermann**\n\nMerged: **sage-4.7.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7742_\n\n",
    "closed_at": "2011-03-17T19:22:27Z",
    "created_at": "2009-12-19T20:15:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add a compose function to sage",
    "type": "issue",
    "updated_at": "2011-03-17T19:22:27Z",
    "url": "https://github.com/sagemath/sage/issues/7742",
    "user": "https://github.com/williamstein"
}
```

```

def compose(f, n, a):
    """
    Return f(f(...f(a)...)), where the composition occurs n times.
    
    INPUT:
        - `f` -- anything that is callable
        - `n` -- a nonnegative integer
        - `a` -- any input for `f`

    OUTPUT:
        result of composing `f` with itself `n` times and applying to `a`.

    EXAMPLES::

        sage: def f(x): return x^2 + 1
        sage: x = var('x')
        sage: compose(f, 3, x)
        ((x^2 + 1)^2 + 1)^2 + 1
    """
    n = Integer(n)
    if n <= 0: return a
    a = f(a)
    for i in range(n-1): a = f(a)
    return a

```
To the release manager: apply only the last patch
(trac_7742-add-compose-etc_v2.1.patch).

Component: **misc**

Author: **Christopher Olah, Felix Lawrence**

Reviewer: **Paul Zimmermann**

Merged: **sage-4.7.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/7742_





---

archive/issue_events_096039.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-19T20:15:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96039"
}
```



---

archive/issue_events_096040.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-19T20:15:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96040"
}
```



---

archive/issue_events_096041.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-19T20:15:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96041"
}
```



---

archive/issue_events_096042.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-19T20:15:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96042"
}
```



---

archive/issue_events_096043.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-12-19T20:15:55Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96043"
}
```



---

archive/issue_comments_058135.json:
```json
{
    "body": "Replying to [ticket:7742 was]:\n>     n = Integer(n)\n\nConverting to int should be sufficient... Or the implementation below should'nt be very useful :-).\n\n>     if n <= 0: return a\n\nI'd rather sage to raise a `ValueError` if `n < 0` than returning silently a wrong value...\n\nFlorent",
    "created_at": "2009-12-19T22:13:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58135",
    "user": "https://github.com/hivert"
}
```

Replying to [ticket:7742 was]:
>     n = Integer(n)

Converting to int should be sufficient... Or the implementation below should'nt be very useful :-).

>     if n <= 0: return a

I'd rather sage to raise a `ValueError` if `n < 0` than returning silently a wrong value...

Florent



---

archive/issue_comments_058136.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nReplying to [@hivert](#comment:1):\n> Replying to [ticket:7742 was]:\n> >     n = Integer(n)\n\n> \n> Converting to int should be sufficient... Or the implementation below should'nt be very useful :-).\n> \n> >     if n <= 0: return a\n\n> \n> I'd rather sage to raise a `ValueError` if `n < 0` than returning silently a wrong value...\n> \n> Florent\n\nWe probably want to return an inverse function, if possible, when n<0... I'm also wondering if there is a generalisation of this to all real numbers.",
    "created_at": "2009-12-20T16:20:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58136",
    "user": "https://github.com/sagetrac-colah"
}
```

<div id="comment:2" align="right">comment:2</div>

Replying to [@hivert](#comment:1):
> Replying to [ticket:7742 was]:
> >     n = Integer(n)

> 
> Converting to int should be sufficient... Or the implementation below should'nt be very useful :-).
> 
> >     if n <= 0: return a

> 
> I'd rather sage to raise a `ValueError` if `n < 0` than returning silently a wrong value...
> 
> Florent

We probably want to return an inverse function, if possible, when n<0... I'm also wondering if there is a generalisation of this to all real numbers.



---

archive/issue_comments_058137.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\n\n```\nHi,\n\nThere is a function in Mathematica that does exactly what you want and it's called Nest. I've reimplemented it in python as general effort to port some functional tools that I like to python. Please check this link if you are interested on it:\n\nhttp://bitbucket.org/juliusc/functional/src/tip/functional/functional_mathematica.py\n\nHope this helps,\nCarlos\n```",
    "created_at": "2009-12-20T21:38:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58137",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>


```
Hi,

There is a function in Mathematica that does exactly what you want and it's called Nest. I've reimplemented it in python as general effort to port some functional tools that I like to python. Please check this link if you are interested on it:

http://bitbucket.org/juliusc/functional/src/tip/functional/functional_mathematica.py

Hope this helps,
Carlos
```



---

archive/issue_comments_058138.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nHi,\n\nThere is a function in Mathematica that does exactly what you want and it's called Nest. I've reimplemented it in python as general effort to port some functional tools that I like to python. Please check this link if you are interested on it:\n\nhttp://bitbucket.org/juliusc/functional/src/tip/functional/functional_mathematica.py\n\nHope this helps,\nCarlos",
    "created_at": "2009-12-20T21:38:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58138",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:4" align="right">comment:4</div>

Hi,

There is a function in Mathematica that does exactly what you want and it's called Nest. I've reimplemented it in python as general effort to port some functional tools that I like to python. Please check this link if you are interested on it:

http://bitbucket.org/juliusc/functional/src/tip/functional/functional_mathematica.py

Hope this helps,
Carlos



---

archive/issue_comments_058139.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\n> We probably want to return an inverse function, if possible, when n<0..\n\nNote that usually the inverse isn't available.",
    "created_at": "2009-12-20T21:38:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58139",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:5" align="right">comment:5</div>

> We probably want to return an inverse function, if possible, when n<0..

Note that usually the inverse isn't available.



---

archive/issue_comments_058140.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nWhat about doing it in log(n) compositions, like when you try to compute x^12 ? :-)\n\nNathann",
    "created_at": "2009-12-21T07:30:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58140",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:6" align="right">comment:6</div>

What about doing it in log(n) compositions, like when you try to compute x^12 ? :-)

Nathann



---

archive/issue_comments_058141.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@nathanncohen](#comment:6):\n> What about doing it in log(n) compositions, like when you try to compute x^12 ? :-)\n\nThat's an interesting idea.  For a Python function that will make absolutely no difference.  For a symbolic callable expression it would make a noticeable difference though.   So we should definitely think about it. \n\nI think the most important thing though is to implement something with a clear interface and definition now and get it into sage. Then we can make it faster whenever we want.",
    "created_at": "2009-12-21T08:02:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58141",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@nathanncohen](#comment:6):
> What about doing it in log(n) compositions, like when you try to compute x^12 ? :-)

That's an interesting idea.  For a Python function that will make absolutely no difference.  For a symbolic callable expression it would make a noticeable difference though.   So we should definitely think about it. 

I think the most important thing though is to implement something with a clear interface and definition now and get it into sage. Then we can make it faster whenever we want.



---

archive/issue_comments_058142.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nI completely agree ! That's what we are doing in graph theory : first implement, then try to think about how to optimize it :-)\n\nI this case it should not be too different from these few lines ( if I make none of my usual mistakes )\n\n```\nif n==0:\n    return \"identity function\"( no idea how it is called in Sage )\n\nif n%2 == 1:\n    ff = compose(f, (n-1)/2, a)\n    return f(ff(ff(a)))\nelse:\n    ff = compose(f, n/2, a)\n    return ff(ff(a))\n```\n\nBut perhaps the function \"compose\" should be a method of these symbolic callable expressions you mentionned, which would be called by this \"compose\" function : it may be interesting to compute symbolically the n^th composed of a function without evaluating it  :-)",
    "created_at": "2009-12-21T08:12:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58142",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:8" align="right">comment:8</div>

I completely agree ! That's what we are doing in graph theory : first implement, then try to think about how to optimize it :-)

I this case it should not be too different from these few lines ( if I make none of my usual mistakes )

```
if n==0:
    return "identity function"( no idea how it is called in Sage )

if n%2 == 1:
    ff = compose(f, (n-1)/2, a)
    return f(ff(ff(a)))
else:
    ff = compose(f, n/2, a)
    return ff(ff(a))
```

But perhaps the function "compose" should be a method of these symbolic callable expressions you mentionned, which would be called by this "compose" function : it may be interesting to compute symbolically the n^th composed of a function without evaluating it  :-)



---

archive/issue_comments_058143.json:
```json
{
    "body": "Attachment: **[compose.patch.gz](https://github.com/sagemath/sage/files/ticket7742/compose.patch.gz)**\n\nAdds the compose function.",
    "created_at": "2010-03-14T04:00:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58143",
    "user": "https://github.com/sagetrac-colah"
}
```

Attachment: **[compose.patch.gz](https://github.com/sagemath/sage/files/ticket7742/compose.patch.gz)**

Adds the compose function.



---

archive/issue_comments_058144.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nI ran into some problems producing the patch. On mhansen's suggestion, I added     \n{{from sage.rings.integer import Integer}}\nat the beginning of the function instead of the beginning of misc.py and it worked... But I don't know why and am concerned that it adds unnecessary overhead...",
    "created_at": "2010-03-14T04:03:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58144",
    "user": "https://github.com/sagetrac-colah"
}
```

<div id="comment:9" align="right">comment:9</div>

I ran into some problems producing the patch. On mhansen's suggestion, I added     
{{from sage.rings.integer import Integer}}
at the beginning of the function instead of the beginning of misc.py and it worked... But I don't know why and am concerned that it adds unnecessary overhead...



---

archive/issue_events_096044.json:
```json
{
    "actor": "https://github.com/sagetrac-colah",
    "created_at": "2010-03-14T04:04:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96044"
}
```



---

archive/issue_comments_058145.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nI have two comments: (1) the patch contains twice the new function, and (2) it would be nice to\nbe able to write `compose(f,x,n)` for a symbolic variable n (or any expression). Only when\nn is an integer would the \"nesting\" be evaluated. Comment (1) needs more work, comment (2) could\nbe a separate ticket, but if easy it would be nice to do it.\n\nAdditional comment: please rename the patch as trac_7742.patch.",
    "created_at": "2010-03-14T18:03:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58145",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:11" align="right">comment:11</div>

I have two comments: (1) the patch contains twice the new function, and (2) it would be nice to
be able to write `compose(f,x,n)` for a symbolic variable n (or any expression). Only when
n is an integer would the "nesting" be evaluated. Comment (1) needs more work, comment (2) could
be a separate ticket, but if easy it would be nice to do it.

Additional comment: please rename the patch as trac_7742.patch.



---

archive/issue_events_096045.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-03-14T18:03:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96045"
}
```



---

archive/issue_events_096046.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-03-14T18:03:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96046"
}
```



---

archive/issue_comments_058146.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThe new patch addresses many of the above concerns (other than symbolic n), and splits the functionality into three functions:\n\n* compose(f,g) which composes functions such that compose(f,g)(x) = f(g(x))\n\n* self_compose(f, n) which composes a function with itself such that self_compose(f,n)(x) = f(...(f(x))...)\n\n* nest(f, n, a) which effectively self_composes and evaluates (but does it slightly faster than a self_compose)\n\nQuestions:\n\n* compose(f,g)(x) = f(g(x)) - is this the most common convention among mathematicians? or should the RHS be g(f(x))?\n\n* Is (f, n, a) or (f, a, n) the best order of parameters for nest?\n\n* Is there somewhere more suitable than misc/misc.py for these functions?",
    "created_at": "2010-11-04T13:51:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58146",
    "user": "https://github.com/sagetrac-flawrence"
}
```

<div id="comment:12" align="right">comment:12</div>

The new patch addresses many of the above concerns (other than symbolic n), and splits the functionality into three functions:

* compose(f,g) which composes functions such that compose(f,g)(x) = f(g(x))

* self_compose(f, n) which composes a function with itself such that self_compose(f,n)(x) = f(...(f(x))...)

* nest(f, n, a) which effectively self_composes and evaluates (but does it slightly faster than a self_compose)

Questions:

* compose(f,g)(x) = f(g(x)) - is this the most common convention among mathematicians? or should the RHS be g(f(x))?

* Is (f, n, a) or (f, a, n) the best order of parameters for nest?

* Is there somewhere more suitable than misc/misc.py for these functions?



---

archive/issue_events_096047.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-11-04T13:51:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96047"
}
```



---

archive/issue_events_096048.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-11-04T13:51:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96048"
}
```



---

archive/issue_comments_058147.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nWhat version of Sage are you targeting?  I tried applying both of these (individually) to 4.6.0 and to 4.5.3 and couldn't apply them.",
    "created_at": "2010-11-16T22:50:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58147",
    "user": "https://github.com/sagetrac-jrp"
}
```

<div id="comment:13" align="right">comment:13</div>

What version of Sage are you targeting?  I tried applying both of these (individually) to 4.6.0 and to 4.5.3 and couldn't apply them.



---

archive/issue_events_096049.json:
```json
{
    "actor": "https://github.com/sagetrac-jrp",
    "created_at": "2010-11-16T22:50:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96049"
}
```



---

archive/issue_events_096050.json:
```json
{
    "actor": "https://github.com/sagetrac-jrp",
    "created_at": "2010-11-16T22:50:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96050"
}
```



---

archive/issue_comments_058148.json:
```json
{
    "body": "Attachment: **[trac_7742-add-compose-etc.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-add-compose-etc.patch.gz)**\n\nadds compose, self_compose and nest functions. actually replaces previous patch this time",
    "created_at": "2010-11-17T04:40:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58148",
    "user": "https://github.com/sagetrac-flawrence"
}
```

Attachment: **[trac_7742-add-compose-etc.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-add-compose-etc.patch.gz)**

adds compose, self_compose and nest functions. actually replaces previous patch this time



---

archive/issue_events_096051.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-11-17T04:45:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96051"
}
```



---

archive/issue_events_096052.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-11-17T04:45:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96052"
}
```



---

archive/issue_comments_058149.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nSorry, the second patch accidentally relied on the first (which needed rebasing).  I've fixed it and the second patch should now apply and work for both 4.6.0 and 4.6.1alpha0.",
    "created_at": "2010-11-17T04:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58149",
    "user": "https://github.com/sagetrac-flawrence"
}
```

<div id="comment:14" align="right">comment:14</div>

Sorry, the second patch accidentally relied on the first (which needed rebasing).  I've fixed it and the second patch should now apply and work for both 4.6.0 and 4.6.1alpha0.



---

archive/issue_comments_058150.json:
```json
{
    "body": "Author: **Christopher Olah, Felix Lawrence**",
    "created_at": "2010-11-17T11:51:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58150",
    "user": "https://github.com/zimmermann6"
}
```

Author: **Christopher Olah, Felix Lawrence**



---

archive/issue_events_096053.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-11-17T11:51:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96053"
}
```



---

archive/issue_events_096054.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-11-17T11:51:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96054"
}
```



---

archive/issue_comments_058151.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nThis needs more work, since the function `self_compose` fails for n=0:\n\n```\nsage: function('f');\nsage: h=self_compose(f,0)\n---------------------------------------------------------------------------\nValueError                                Traceback (most recent call last)\n\n/localdisk/tmp/sage-4.6/<ipython console> in <module>()\n\n/localdisk/tmp/sage-4.6/local/lib/python2.6/site-packages/sage/misc/misc.pyc in self_compose(f, n)\n    890     # all necessary powers of two (f, f^2, f^4), storing them in a list\n    891     fs = [f]\n--> 892     for i in xrange(int(floor(log(n, 2)))):\n    893         fs.append(compose(fs[i], fs[i]))\n    894     \n\nValueError: math domain error\n```\nPaul",
    "created_at": "2010-11-17T11:51:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58151",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:15" align="right">comment:15</div>

This needs more work, since the function `self_compose` fails for n=0:

```
sage: function('f');
sage: h=self_compose(f,0)
---------------------------------------------------------------------------
ValueError                                Traceback (most recent call last)

/localdisk/tmp/sage-4.6/<ipython console> in <module>()

/localdisk/tmp/sage-4.6/local/lib/python2.6/site-packages/sage/misc/misc.pyc in self_compose(f, n)
    890     # all necessary powers of two (f, f^2, f^4), storing them in a list
    891     fs = [f]
--> 892     for i in xrange(int(floor(log(n, 2)))):
    893         fs.append(compose(fs[i], fs[i]))
    894     

ValueError: math domain error
```
Paul



---

archive/issue_comments_058152.json:
```json
{
    "body": "Reviewer: **Paul Zimmermann**",
    "created_at": "2010-11-17T11:51:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58152",
    "user": "https://github.com/zimmermann6"
}
```

Reviewer: **Paul Zimmermann**



---

archive/issue_comments_058153.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nReplying to [@sagetrac-flawrence](#comment:12):\n\n> * compose(f,g)(x) = f(g(x)) - is this the most common convention among mathematicians? or should the RHS be g(f(x))?\n\ncompose(f,g) seems good to me.\n\n> * Is (f, n, a) or (f, a, n) the best order of parameters for nest?\n\nI prefer (f,n,a) since this is coherent with `self_compose(f,n)`.\n\n> * Is there somewhere more suitable than misc/misc.py for these functions?\n\nI don't know.\n\nAlso if a symbolic n is not allowed (yet) in `self_compose`, some type-checking\nshould be done:\n\n```\nsage: function('f')\nsage: var('m')\nsage: self_compose(f,m)(x)\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n\n/localdisk/tmp/sage-4.6/<ipython console> in <module>()\n\n/localdisk/tmp/sage-4.6/local/lib/python2.6/site-packages/sage/misc/misc.pyc in self_compose(f, n)\n    890     # all necessary powers of two (f, f^2, f^4), storing them in a list\n    891     fs = [f]\n--> 892     for i in xrange(int(floor(log(n, 2)))):\n    893         fs.append(compose(fs[i], fs[i]))\n    894     \n\n/localdisk/tmp/sage-4.6/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.__float__ (sage/symbolic/expression.cpp:5398)()\n\nTypeError: unable to simplify to float approximation\n```\n\nPaul",
    "created_at": "2010-11-17T11:58:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58153",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:16" align="right">comment:16</div>

Replying to [@sagetrac-flawrence](#comment:12):

> * compose(f,g)(x) = f(g(x)) - is this the most common convention among mathematicians? or should the RHS be g(f(x))?

compose(f,g) seems good to me.

> * Is (f, n, a) or (f, a, n) the best order of parameters for nest?

I prefer (f,n,a) since this is coherent with `self_compose(f,n)`.

> * Is there somewhere more suitable than misc/misc.py for these functions?

I don't know.

Also if a symbolic n is not allowed (yet) in `self_compose`, some type-checking
should be done:

```
sage: function('f')
sage: var('m')
sage: self_compose(f,m)(x)
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)

/localdisk/tmp/sage-4.6/<ipython console> in <module>()

/localdisk/tmp/sage-4.6/local/lib/python2.6/site-packages/sage/misc/misc.pyc in self_compose(f, n)
    890     # all necessary powers of two (f, f^2, f^4), storing them in a list
    891     fs = [f]
--> 892     for i in xrange(int(floor(log(n, 2)))):
    893         fs.append(compose(fs[i], fs[i]))
    894     

/localdisk/tmp/sage-4.6/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.__float__ (sage/symbolic/expression.cpp:5398)()

TypeError: unable to simplify to float approximation
```

Paul



---

archive/issue_comments_058154.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\nAt the moment the powers-of-two speed up isn't helping even in the symbolic case, because the implementation is to return lambda x: f(g(x)).  If both inputs to compose are symbolic, then I think we want to just return f(g(x)) instead.",
    "created_at": "2010-11-18T01:26:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58154",
    "user": "https://github.com/sagetrac-jrp"
}
```

<div id="comment:17" align="right">comment:17</div>

At the moment the powers-of-two speed up isn't helping even in the symbolic case, because the implementation is to return lambda x: f(g(x)).  If both inputs to compose are symbolic, then I think we want to just return f(g(x)) instead.



---

archive/issue_comments_058155.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nReplying to [@sagetrac-jrp](#comment:17):\n> At the moment the powers-of-two speed up isn't helping even in the symbolic case, because the implementation is to return lambda x: f(g(x)).  If both inputs to compose are symbolic, then I think we want to just return f(g(x)) instead.\n\nGiven that symbolic functions have named variables, is this the correct behavior?\n\n\nsage: f(x) = x+1\nsage: g(y) = y+1\nsage: compose(f,g)\ny + 2\nsage: compose(g,f)\nx + 2",
    "created_at": "2010-11-18T01:45:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58155",
    "user": "https://github.com/sagetrac-jrp"
}
```

<div id="comment:18" align="right">comment:18</div>

Replying to [@sagetrac-jrp](#comment:17):
> At the moment the powers-of-two speed up isn't helping even in the symbolic case, because the implementation is to return lambda x: f(g(x)).  If both inputs to compose are symbolic, then I think we want to just return f(g(x)) instead.

Given that symbolic functions have named variables, is this the correct behavior?


sage: f(x) = x+1
sage: g(y) = y+1
sage: compose(f,g)
y + 2
sage: compose(g,f)
x + 2



---

archive/issue_comments_058156.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\nReplying to [@sagetrac-jrp](#comment:18):\n> Replying to [@sagetrac-jrp](#comment:17):\n> > At the moment the powers-of-two speed up isn't helping even in the symbolic case, because the implementation is to return lambda x: f(g(x)).  If both inputs to compose are symbolic, then I think we want to just return f(g(x)) instead.\n\n> \n> Given that symbolic functions have named variables, is this the correct behavior?\n> \n> \n> sage: f(x) = x+1\n> sage: g(y) = y+1\n> sage: compose(f,g)\n> y + 2\n> sage: compose(g,f)\n> x + 2\n\nI don't actually know anything about symbolic functions in Sage.  As such, all doctests and examples in the patch are for regular python functions not symbolic functions.  The compose functions in the patch use lambda functions, which may or may not break symbolic functions.\n\nWith normal python functions (such as those in the doctest), the powers-of-two provides a major speedup for large n over the other techniques I tried, both for the function composition and for the evaluation of the resulting function (benchmarked using \"timeit\").  In fact, without a powers-of-two-type algorithm, the self_compose function regularly hit the recursion limit and crashed for large n (above 500 or 1000).  But I don't know if anyone is interested in large n cases.",
    "created_at": "2010-11-18T03:15:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58156",
    "user": "https://github.com/sagetrac-flawrence"
}
```

<div id="comment:19" align="right">comment:19</div>

Replying to [@sagetrac-jrp](#comment:18):
> Replying to [@sagetrac-jrp](#comment:17):
> > At the moment the powers-of-two speed up isn't helping even in the symbolic case, because the implementation is to return lambda x: f(g(x)).  If both inputs to compose are symbolic, then I think we want to just return f(g(x)) instead.

> 
> Given that symbolic functions have named variables, is this the correct behavior?
> 
> 
> sage: f(x) = x+1
> sage: g(y) = y+1
> sage: compose(f,g)
> y + 2
> sage: compose(g,f)
> x + 2

I don't actually know anything about symbolic functions in Sage.  As such, all doctests and examples in the patch are for regular python functions not symbolic functions.  The compose functions in the patch use lambda functions, which may or may not break symbolic functions.

With normal python functions (such as those in the doctest), the powers-of-two provides a major speedup for large n over the other techniques I tried, both for the function composition and for the evaluation of the resulting function (benchmarked using "timeit").  In fact, without a powers-of-two-type algorithm, the self_compose function regularly hit the recursion limit and crashed for large n (above 500 or 1000).  But I don't know if anyone is interested in large n cases.



---

archive/issue_comments_058157.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">comment:20</div>\n\nReplying to [@sagetrac-flawrence](#comment:19):\n\n> With normal python functions (such as those in the doctest), the powers-of-two provides a major speedup for large n over the other techniques I tried, both for the function composition and for the evaluation of the resulting function (benchmarked using \"timeit\").  In fact, without a powers-of-two-type algorithm, the self_compose function regularly hit the recursion limit and crashed for large n (above 500 or 1000).\n\nI ran the following:\n\n```\nsage: def f(x):\n....:     return x + 1\n....: \nsage: g = self_compose(f,10000)\nsage: g\n<function <lambda> at 0xb0aa80c>\nsage: timeit('g(0)')\n25 loops, best of 3: 9.41 ms per loop\nsage: timeit('nest(f,10000,0)')\n125 loops, best of 3: 6.35 ms per loop\n```\nSince nest just loops through xrange and doesn't use powers of two, maybe the slowdown before was due to the recursion.  But in any case, it definitely helps with the symbolic ones.\n\n> The compose functions in the patch use lambda functions, which may or may not break symbolic functions.\n\nI'm attaching a patch to check for symbolic functions and (hopefully) do the right thing.  I also make self_compose(f,0) return the identity.",
    "created_at": "2010-11-18T07:38:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58157",
    "user": "https://github.com/sagetrac-jrp"
}
```

<div id="comment:20" align="right">comment:20</div>

Replying to [@sagetrac-flawrence](#comment:19):

> With normal python functions (such as those in the doctest), the powers-of-two provides a major speedup for large n over the other techniques I tried, both for the function composition and for the evaluation of the resulting function (benchmarked using "timeit").  In fact, without a powers-of-two-type algorithm, the self_compose function regularly hit the recursion limit and crashed for large n (above 500 or 1000).

I ran the following:

```
sage: def f(x):
....:     return x + 1
....: 
sage: g = self_compose(f,10000)
sage: g
<function <lambda> at 0xb0aa80c>
sage: timeit('g(0)')
25 loops, best of 3: 9.41 ms per loop
sage: timeit('nest(f,10000,0)')
125 loops, best of 3: 6.35 ms per loop
```
Since nest just loops through xrange and doesn't use powers of two, maybe the slowdown before was due to the recursion.  But in any case, it definitely helps with the symbolic ones.

> The compose functions in the patch use lambda functions, which may or may not break symbolic functions.

I'm attaching a patch to check for symbolic functions and (hopefully) do the right thing.  I also make self_compose(f,0) return the identity.



---

archive/issue_comments_058158.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\n> Given that symbolic functions have named variables, is this the correct behavior?\n\n\n```\nsage: f(x) = x+1\nsage: g(y) = y+1\nsage: compose(f,g)\ny + 2\nsage: compose(g,f)\nx + 2\n```\n\n[please use { { { and } } } to quote Sage examples]\n\nI can't reproduce this. I get with the patch from Felix:\n\n```\nsage: f(x) = x+1\nsage: g(y) = y+1\nsage: compose(f,g)\n<function <lambda> at 0x1787c80>\nsage: compose(f,g)(x)\nx + 2\n```\nwhich seems ok to me.\n\nPaul",
    "created_at": "2010-11-18T07:49:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58158",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:21" align="right">comment:21</div>

> Given that symbolic functions have named variables, is this the correct behavior?


```
sage: f(x) = x+1
sage: g(y) = y+1
sage: compose(f,g)
y + 2
sage: compose(g,f)
x + 2
```

[please use { { { and } } } to quote Sage examples]

I can't reproduce this. I get with the patch from Felix:

```
sage: f(x) = x+1
sage: g(y) = y+1
sage: compose(f,g)
<function <lambda> at 0x1787c80>
sage: compose(f,g)(x)
x + 2
```
which seems ok to me.

Paul



---

archive/issue_comments_058159.json:
```json
{
    "body": "Special treatment of symbolic functions.",
    "created_at": "2010-11-18T19:10:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58159",
    "user": "https://github.com/sagetrac-jrp"
}
```

Special treatment of symbolic functions.



---

archive/issue_comments_058160.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nAttachment: **[trac_7742-symbolics.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-symbolics.patch.gz)**\n\nI realized that what really matters is the type of the first function in the composition - i.e., the type of g in compose(f,g).",
    "created_at": "2010-11-18T19:11:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58160",
    "user": "https://github.com/sagetrac-jrp"
}
```

<div id="comment:22" align="right">comment:22</div>

Attachment: **[trac_7742-symbolics.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-symbolics.patch.gz)**

I realized that what really matters is the type of the first function in the composition - i.e., the type of g in compose(f,g).



---

archive/issue_events_096055.json:
```json
{
    "actor": "https://github.com/sagetrac-jrp",
    "created_at": "2010-11-18T19:11:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96055"
}
```



---

archive/issue_events_096056.json:
```json
{
    "actor": "https://github.com/sagetrac-jrp",
    "created_at": "2010-11-18T19:11:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96056"
}
```



---

archive/issue_comments_058161.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">comment:23</div>\n\njrp, you didn't answer comment 21. What was wrong with Felix patch?\n\nPaul",
    "created_at": "2010-11-18T19:15:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58161",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:23" align="right">comment:23</div>

jrp, you didn't answer comment 21. What was wrong with Felix patch?

Paul



---

archive/issue_comments_058162.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\nPaul - I wanted it to automatically do the right thing when g is a symbolic function.  I agree that this isn't very important since we can already just call compose(f,g)(x).  More important is that with symbolics, now the powers-of-two optimization will work.  (I don't think it did before, but will think more on this).",
    "created_at": "2010-11-18T19:34:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58162",
    "user": "https://github.com/sagetrac-jrp"
}
```

<div id="comment:24" align="right">comment:24</div>

Paul - I wanted it to automatically do the right thing when g is a symbolic function.  I agree that this isn't very important since we can already just call compose(f,g)(x).  More important is that with symbolics, now the powers-of-two optimization will work.  (I don't think it did before, but will think more on this).



---

archive/issue_comments_058163.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nI am not very happy with the fact that when f is an expression, the code assumes it has exactly\none symbolic variable:\n\n```\nsage: f = 2\nsage: self_compose(f,0)\n<function <lambda> at 0x41031b8>\nsage: self_compose(f,1)\n2\nsage: self_compose(f,2)\n<function <lambda> at 0x7fa8ac2b0d70>\n```\nor\n\n```\nsage: f = x+y\nsage: self_compose(f,0)\nx\nsage: self_compose(f,1)\nx + y\nsage: self_compose(f,2)\nx + 2*y\n```\nConsider also:\n\n```\nsage: f = pi+1\nsage: self_compose(f,0)\n---------------------------------------------------------------------------\nIndexError                                Traceback (most recent call last)\n\n/tmp/cado-nfs-1.0-rc1/<ipython console> in <module>()\n\n/usr/local/sage-4.6/sage/local/lib/python2.6/site-packages/sage/misc/misc.pyc in self_compose(f, n)\n    911     if n == 0:\n    912         if type(f) == sage.symbolic.expression.Expression:\n--> 913             return f.parent()(f.variables()[0])\n    914         else:\n    915             return lambda x: x\n\nIndexError: tuple index out of range\n```\n\nI would prefer that symbolic expressions such as `f=x+1` are not allowed,\nand we should write instead `f(x)=x+1` or `f = lambda x : x+1`.\n\nPaul",
    "created_at": "2010-11-18T20:51:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58163",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:25" align="right">comment:25</div>

I am not very happy with the fact that when f is an expression, the code assumes it has exactly
one symbolic variable:

```
sage: f = 2
sage: self_compose(f,0)
<function <lambda> at 0x41031b8>
sage: self_compose(f,1)
2
sage: self_compose(f,2)
<function <lambda> at 0x7fa8ac2b0d70>
```
or

```
sage: f = x+y
sage: self_compose(f,0)
x
sage: self_compose(f,1)
x + y
sage: self_compose(f,2)
x + 2*y
```
Consider also:

```
sage: f = pi+1
sage: self_compose(f,0)
---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)

/tmp/cado-nfs-1.0-rc1/<ipython console> in <module>()

/usr/local/sage-4.6/sage/local/lib/python2.6/site-packages/sage/misc/misc.pyc in self_compose(f, n)
    911     if n == 0:
    912         if type(f) == sage.symbolic.expression.Expression:
--> 913             return f.parent()(f.variables()[0])
    914         else:
    915             return lambda x: x

IndexError: tuple index out of range
```

I would prefer that symbolic expressions such as `f=x+1` are not allowed,
and we should write instead `f(x)=x+1` or `f = lambda x : x+1`.

Paul



---

archive/issue_events_096057.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-11-18T20:51:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96057"
}
```



---

archive/issue_events_096058.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-11-18T20:51:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96058"
}
```



---

archive/issue_comments_058164.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\nReplying to [@zimmermann6](#comment:25):\n> I would prefer that symbolic expressions such as `f=x+1` are not allowed,\n> and we should write instead `f(x)=x+1` or `f = lambda x : x+1`.\n\nI now think it's best to leave compose,self_compose, and nest agnostic to the type of f; the user can supply anything callable and we won't attempt to turn it back into a symbolic function.\n\nThen as a separate issue, symbolic functions can grow an f.self_compose(n) function that plays nice with types.",
    "created_at": "2010-11-22T20:09:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58164",
    "user": "https://github.com/sagetrac-jrp"
}
```

<div id="comment:26" align="right">comment:26</div>

Replying to [@zimmermann6](#comment:25):
> I would prefer that symbolic expressions such as `f=x+1` are not allowed,
> and we should write instead `f(x)=x+1` or `f = lambda x : x+1`.

I now think it's best to leave compose,self_compose, and nest agnostic to the type of f; the user can supply anything callable and we won't attempt to turn it back into a symbolic function.

Then as a separate issue, symbolic functions can grow an f.self_compose(n) function that plays nice with types.



---

archive/issue_comments_058165.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nReplying to [@sagetrac-flawrence](#comment:19):\n> With normal python functions (such as those in the doctest), the powers-of-two provides a major speedup for large n over the other techniques I tried, both for the function composition and for the evaluation of the resulting function (benchmarked using \"timeit\").  In fact, without a powers-of-two-type algorithm, the self_compose function regularly hit the recursion limit and crashed for large n (above 500 or 1000).  But I don't know if anyone is interested in large n cases.\n\nWith normal python functions, I don't think there is any gain from powers of two.\n\nWith the self_compose from adds-compose-etc.patch:\n\n```\nsage: f = lambda x: x + 1\nsage: g = x + 1\nsage: f1 = self_compose(f,10000)\nsage: g1 = self_compose(g,10000)\nsage: timeit('self_compose(f,10000)')\n625 loops, best of 3: 66.6 \u00b5s per loop\nsage: timeit('self_compose(g,10000)')\n625 loops, best of 3: 66.9 \u00b5s per loop\nsage: timeit('f1(0)')                \n25 loops, best of 3: 9.32 ms per loop\nsage: timeit('g1(0)')\n5 loops, best of 3: 375 ms per loop\nsage: timeit('nest(f,10000,0)')\n125 loops, best of 3: 6.46 ms per loop\nsage: timeit('nest(g,10000,0)')\n5 loops, best of 3: 379 ms per loop\nsage: def new_self_compose(f,n):\n....:     return lambda a: nest(f,n,a)\n....: \nsage: timeit('new_self_compose(f,10000)')\n625 loops, best of 3: 1.17 \u00b5s per loop\nsage: timeit('new_self_compose(g,10000)')\n625 loops, best of 3: 2.68 \u00b5s per loop\n```\n\nSince self_compose is implemented via `lambda x: f(g(x))`, we have to eventually pass the argument through `f` the correct number of times.\n\nLet's move the powers of two, and the symbolics handling, into a method of symbolic functions.  Then the compose tools for python functions can have fast, dirt-simple implementations.",
    "created_at": "2010-11-22T20:16:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58165",
    "user": "https://github.com/sagetrac-jrp"
}
```

<div id="comment:27" align="right">comment:27</div>

Replying to [@sagetrac-flawrence](#comment:19):
> With normal python functions (such as those in the doctest), the powers-of-two provides a major speedup for large n over the other techniques I tried, both for the function composition and for the evaluation of the resulting function (benchmarked using "timeit").  In fact, without a powers-of-two-type algorithm, the self_compose function regularly hit the recursion limit and crashed for large n (above 500 or 1000).  But I don't know if anyone is interested in large n cases.

With normal python functions, I don't think there is any gain from powers of two.

With the self_compose from adds-compose-etc.patch:

```
sage: f = lambda x: x + 1
sage: g = x + 1
sage: f1 = self_compose(f,10000)
sage: g1 = self_compose(g,10000)
sage: timeit('self_compose(f,10000)')
625 loops, best of 3: 66.6 µs per loop
sage: timeit('self_compose(g,10000)')
625 loops, best of 3: 66.9 µs per loop
sage: timeit('f1(0)')                
25 loops, best of 3: 9.32 ms per loop
sage: timeit('g1(0)')
5 loops, best of 3: 375 ms per loop
sage: timeit('nest(f,10000,0)')
125 loops, best of 3: 6.46 ms per loop
sage: timeit('nest(g,10000,0)')
5 loops, best of 3: 379 ms per loop
sage: def new_self_compose(f,n):
....:     return lambda a: nest(f,n,a)
....: 
sage: timeit('new_self_compose(f,10000)')
625 loops, best of 3: 1.17 µs per loop
sage: timeit('new_self_compose(g,10000)')
625 loops, best of 3: 2.68 µs per loop
```

Since self_compose is implemented via `lambda x: f(g(x))`, we have to eventually pass the argument through `f` the correct number of times.

Let's move the powers of two, and the symbolics handling, into a method of symbolic functions.  Then the compose tools for python functions can have fast, dirt-simple implementations.



---

archive/issue_comments_058166.json:
```json
{
    "body": "Attachment: **[trac_7742-add-compose-etc_v2.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-add-compose-etc_v2.patch.gz)**\n\nadds compose, self_compose and nest functions. self_compose is a wrapper for nest.  a self-contained patch.",
    "created_at": "2010-11-23T04:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58166",
    "user": "https://github.com/sagetrac-flawrence"
}
```

Attachment: **[trac_7742-add-compose-etc_v2.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-add-compose-etc_v2.patch.gz)**

adds compose, self_compose and nest functions. self_compose is a wrapper for nest.  a self-contained patch.



---

archive/issue_events_096059.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-11-23T05:01:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96059"
}
```



---

archive/issue_events_096060.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-11-23T05:01:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96060"
}
```



---

archive/issue_events_096061.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-11-23T05:01:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96061"
}
```



---

archive/issue_comments_058167.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">comment:28</div>\n\nReplying to [@sagetrac-jrp](#comment:27):\n> With normal python functions, I don't think there is any gain from powers of two.\n> \n> With the self_compose from adds-compose-etc.patch:\n\n(snip)\n> Since self_compose is implemented via `lambda x: f(g(x))`, we have to eventually pass the argument through `f` the correct number of times.\n\nYes, `nest` is always faster than `self_compose` if you actually want to evaluate the function.  However the functions do different things: `nest` isn't really a compose function, since it returns f^n(x) not f^n.  `self_compose` is more useful than `nest` if you want to forget about f and only want to apply `f^n`.  In hindsight, `self_compose` should actually look like this, which I agree would be faster to construct and to evaluate than the powers of two method:\n\n```\n\ndef self_compose(f,n):\n    return lambda x: nest(f,n,x)\n``` \n> Let's move the powers of two, and the symbolics handling, into a method of symbolic functions.  Then the compose tools for python functions can have fast, dirt-simple implementations.\n\nI agree that we should scrap the powers of two, and not go out of our way to support symbolics in this ticket.  I'm uploading a patch that makes the change detailed above.  I'm not going to have any more time to work on this for at least a month, so someone else will have to finish the ticket if further changes are required.\n\nThe new patch works for n = 0, but still does not check whether n is symbolic.  I don't think this is the worst thing in the world: the documentation does state that n should be a nonnegative integer, and if it is symbolic then the error raised is appropriate IMO:\n\n```\n\nsage: _ = var('x y')\nsage: nest(sin,x,y)\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n\n/Applications/sage/devel/sage-dev/<ipython console> in <module>()\n\n/Applications/sage/local/lib/python2.6/site-packages/sage/misc/misc.pyc in nest(f, n, x)\n    939     \n    940     \"\"\"\n--> 941     for i in xrange(n):\n    942         x = f(x)\n    943     return x\n\n/Applications/sage/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.__int__ (sage/symbolic/expression.cpp:4375)()\n\n/Applications/sage/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.n (sage/symbolic/expression.cpp:17185)()\n\nTypeError: cannot evaluate symbolic expression numerically\n```",
    "created_at": "2010-11-23T05:01:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58167",
    "user": "https://github.com/sagetrac-flawrence"
}
```

<div id="comment:28" align="right">comment:28</div>

Replying to [@sagetrac-jrp](#comment:27):
> With normal python functions, I don't think there is any gain from powers of two.
> 
> With the self_compose from adds-compose-etc.patch:

(snip)
> Since self_compose is implemented via `lambda x: f(g(x))`, we have to eventually pass the argument through `f` the correct number of times.

Yes, `nest` is always faster than `self_compose` if you actually want to evaluate the function.  However the functions do different things: `nest` isn't really a compose function, since it returns f^n(x) not f^n.  `self_compose` is more useful than `nest` if you want to forget about f and only want to apply `f^n`.  In hindsight, `self_compose` should actually look like this, which I agree would be faster to construct and to evaluate than the powers of two method:

```

def self_compose(f,n):
    return lambda x: nest(f,n,x)
``` 
> Let's move the powers of two, and the symbolics handling, into a method of symbolic functions.  Then the compose tools for python functions can have fast, dirt-simple implementations.

I agree that we should scrap the powers of two, and not go out of our way to support symbolics in this ticket.  I'm uploading a patch that makes the change detailed above.  I'm not going to have any more time to work on this for at least a month, so someone else will have to finish the ticket if further changes are required.

The new patch works for n = 0, but still does not check whether n is symbolic.  I don't think this is the worst thing in the world: the documentation does state that n should be a nonnegative integer, and if it is symbolic then the error raised is appropriate IMO:

```

sage: _ = var('x y')
sage: nest(sin,x,y)
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)

/Applications/sage/devel/sage-dev/<ipython console> in <module>()

/Applications/sage/local/lib/python2.6/site-packages/sage/misc/misc.pyc in nest(f, n, x)
    939     
    940     """
--> 941     for i in xrange(n):
    942         x = f(x)
    943     return x

/Applications/sage/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.__int__ (sage/symbolic/expression.cpp:4375)()

/Applications/sage/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.n (sage/symbolic/expression.cpp:17185)()

TypeError: cannot evaluate symbolic expression numerically
```



---

archive/issue_events_096062.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-11-23T05:01:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96062"
}
```



---

archive/issue_events_096063.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2010-11-23T05:01:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96063"
}
```



---

archive/issue_events_096064.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-11-24T07:12:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96064"
}
```



---

archive/issue_events_096065.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-11-24T07:12:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96065"
}
```



---

archive/issue_events_096066.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-24T10:13:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96066"
}
```



---

archive/issue_events_096067.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-24T10:13:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96067"
}
```



---

archive/issue_comments_058168.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">comment:31</div>\n\nPlease state clearly in the ticket description which patches have to be applied.",
    "created_at": "2010-12-02T13:54:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58168",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:31" align="right">comment:31</div>

Please state clearly in the ticket description which patches have to be applied.



---

archive/issue_events_096068.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-02T13:54:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96068"
}
```



---

archive/issue_events_096069.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-02T13:54:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96069"
}
```



---

archive/issue_events_096070.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-12-03T22:36:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96070"
}
```



---

archive/issue_events_096071.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-12-03T22:36:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96071"
}
```



---

archive/issue_comments_058169.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -27,3 +27,5 @@\n     return a\n \n ```\n+To the release manager: apply only the last patch\n+(trac_7742-add-compose-etc_v2.patch).\n``````\n",
    "created_at": "2010-12-03T22:36:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58169",
    "user": "https://github.com/zimmermann6"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -27,3 +27,5 @@
     return a
 
 ```
+To the release manager: apply only the last patch
+(trac_7742-add-compose-etc_v2.patch).
``````




---

archive/issue_events_096072.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-12-03T22:36:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96072"
}
```



---

archive/issue_events_096073.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-12-03T22:36:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96073"
}
```



---

archive/issue_comments_058170.json:
```json
{
    "body": "<div id=\"comment:34\" align=\"right\">comment:34</div>\n\nThis needs to be rebased to sage-4.6.2.alpha2.  You might also want to check for conflicts with #8456 (so it might be safer to base your patch to sage-4.6.2.alpha2 + #8456).",
    "created_at": "2011-01-26T18:32:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58170",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:34" align="right">comment:34</div>

This needs to be rebased to sage-4.6.2.alpha2.  You might also want to check for conflicts with #8456 (so it might be safer to base your patch to sage-4.6.2.alpha2 + #8456).



---

archive/issue_comments_058171.json:
```json
{
    "body": "Work Issues: **rebase**",
    "created_at": "2011-01-26T18:32:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58171",
    "user": "https://github.com/jdemeyer"
}
```

Work Issues: **rebase**



---

archive/issue_events_096074.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-26T18:32:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96074"
}
```



---

archive/issue_events_096075.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-26T18:32:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96075"
}
```



---

archive/issue_comments_058172.json:
```json
{
    "body": "<div id=\"comment:35\" align=\"right\">comment:35</div>\n\ndoes anybody know why this patch with positive review was not included in 4.6.1?\n\nPaul",
    "created_at": "2011-01-26T20:33:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58172",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:35" align="right">comment:35</div>

does anybody know why this patch with positive review was not included in 4.6.1?

Paul



---

archive/issue_comments_058173.json:
```json
{
    "body": "<div id=\"comment:36\" align=\"right\">comment:36</div>\n\nReplying to [@zimmermann6](#comment:35):\n> does anybody know why this patch with positive review was not included in 4.6.1?\n\nI know because I was the one making that decision.\n\nMost likely, the 4.6.1 release cycle was in bug-fix-only status at the time when this patch got positive_review.  Unfortunately, 4.6.1 stayed in that status for quite a while (much longer than usual Sage releases) because there were a lot of changes in the Sage build scripts giving rise to many system-specific issues which had to be tracked down.",
    "created_at": "2011-01-26T21:42:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58173",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:36" align="right">comment:36</div>

Replying to [@zimmermann6](#comment:35):
> does anybody know why this patch with positive review was not included in 4.6.1?

I know because I was the one making that decision.

Most likely, the 4.6.1 release cycle was in bug-fix-only status at the time when this patch got positive_review.  Unfortunately, 4.6.1 stayed in that status for quite a while (much longer than usual Sage releases) because there were a lot of changes in the Sage build scripts giving rise to many system-specific issues which had to be tracked down.



---

archive/issue_comments_058174.json:
```json
{
    "body": "<div id=\"comment:37\" align=\"right\">comment:37</div>\n\nAnybody wants to look at this?...",
    "created_at": "2011-02-16T09:37:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58174",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:37" align="right">comment:37</div>

Anybody wants to look at this?...



---

archive/issue_events_096076.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-16T09:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96076"
}
```



---

archive/issue_events_096077.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-16T09:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96077"
}
```



---

archive/issue_comments_058175.json:
```json
{
    "body": "Attachment: **[trac_7742-add-compose-etc_v2_rebased.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-add-compose-etc_v2_rebased.patch.gz)**\n\nRebased for 4.6.2.rc0",
    "created_at": "2011-02-19T07:13:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58175",
    "user": "https://github.com/sagetrac-flawrence"
}
```

Attachment: **[trac_7742-add-compose-etc_v2_rebased.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-add-compose-etc_v2_rebased.patch.gz)**

Rebased for 4.6.2.rc0



---

archive/issue_comments_058176.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -28,4 +28,4 @@\n \n ```\n To the release manager: apply only the last patch\n-(trac_7742-add-compose-etc_v2.patch).\n+(trac_7742-add-compose-etc_v2_rebased.patch).\n``````\n",
    "created_at": "2011-02-19T07:16:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58176",
    "user": "https://github.com/sagetrac-flawrence"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -28,4 +28,4 @@
 
 ```
 To the release manager: apply only the last patch
-(trac_7742-add-compose-etc_v2.patch).
+(trac_7742-add-compose-etc_v2_rebased.patch).
``````




---

archive/issue_comments_058177.json:
```json
{
    "body": "Changed work issues from **rebase** to none",
    "created_at": "2011-02-19T07:16:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58177",
    "user": "https://github.com/sagetrac-flawrence"
}
```

Changed work issues from **rebase** to none



---

archive/issue_comments_058178.json:
```json
{
    "body": "<div id=\"comment:38\" align=\"right\">comment:38</div>\n\nOnly change since the positive review was updating the change to sage/misc/all.py",
    "created_at": "2011-02-19T07:16:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58178",
    "user": "https://github.com/sagetrac-flawrence"
}
```

<div id="comment:38" align="right">comment:38</div>

Only change since the positive review was updating the change to sage/misc/all.py



---

archive/issue_events_096078.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2011-02-19T07:16:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96078"
}
```



---

archive/issue_events_096079.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2011-02-19T07:16:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96079"
}
```



---

archive/issue_events_096080.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-02-19T08:48:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96080"
}
```



---

archive/issue_events_096081.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-02-19T08:48:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96081"
}
```



---

archive/issue_comments_058179.json:
```json
{
    "body": "Work Issues: **check argument type**",
    "created_at": "2011-02-19T08:48:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58179",
    "user": "https://github.com/zimmermann6"
}
```

Work Issues: **check argument type**



---

archive/issue_comments_058180.json:
```json
{
    "body": "<div id=\"comment:39\" align=\"right\">comment:39</div>\n\na minor point: it would be good to check that n is indeed a nonnegative integer in\n`self_compose` and `nest`, otherwise we can get strange results:\n\n```\nsage: self_compose(sin, -3)(x) \nx\nsage: self_compose(sin, x)(x)\n...\nTypeError: cannot evaluate symbolic expression numerically\n```\n\nPaul",
    "created_at": "2011-02-19T08:48:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58180",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:39" align="right">comment:39</div>

a minor point: it would be good to check that n is indeed a nonnegative integer in
`self_compose` and `nest`, otherwise we can get strange results:

```
sage: self_compose(sin, -3)(x) 
x
sage: self_compose(sin, x)(x)
...
TypeError: cannot evaluate symbolic expression numerically
```

Paul



---

archive/issue_comments_058181.json:
```json
{
    "body": "Attachment: **[trac_7742-add-compose-etc_v2.1.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-add-compose-etc_v2.1.patch.gz)**\n\nchecks that n is a nonnegative integer",
    "created_at": "2011-02-19T10:35:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58181",
    "user": "https://github.com/sagetrac-flawrence"
}
```

Attachment: **[trac_7742-add-compose-etc_v2.1.patch.gz](https://github.com/sagemath/sage/files/ticket7742/trac_7742-add-compose-etc_v2.1.patch.gz)**

checks that n is a nonnegative integer



---

archive/issue_events_096082.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2011-02-19T10:42:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96082"
}
```



---

archive/issue_events_096083.json:
```json
{
    "actor": "https://github.com/sagetrac-flawrence",
    "created_at": "2011-02-19T10:42:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96083"
}
```



---

archive/issue_comments_058182.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -28,4 +28,4 @@\n \n ```\n To the release manager: apply only the last patch\n-(trac_7742-add-compose-etc_v2_rebased.patch).\n+(trac_7742-add-compose-etc_v2.1.patch).\n``````\n",
    "created_at": "2011-02-19T10:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58182",
    "user": "https://github.com/sagetrac-flawrence"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -28,4 +28,4 @@
 
 ```
 To the release manager: apply only the last patch
-(trac_7742-add-compose-etc_v2_rebased.patch).
+(trac_7742-add-compose-etc_v2.1.patch).
``````




---

archive/issue_comments_058183.json:
```json
{
    "body": "<div id=\"comment:40\" align=\"right\">comment:40</div>\n\nReplying to [@zimmermann6](#comment:39):\n> a minor point: it would be good to check that n is indeed a nonnegative integer in\n> `self_compose` and `nest`, otherwise we can get strange results\n\nDone:\n\n```\nsage: self_compose(sin, -3)\n...\nValueError: n must be a nonnegative integer, not -3.\nsage: self_compose(sin, x)\n...\nTypeError: n (=x) must be of type (<type 'int'>, <type 'long'>, <type 'sage.rings.integer.Integer'>).\n```",
    "created_at": "2011-02-19T10:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58183",
    "user": "https://github.com/sagetrac-flawrence"
}
```

<div id="comment:40" align="right">comment:40</div>

Replying to [@zimmermann6](#comment:39):
> a minor point: it would be good to check that n is indeed a nonnegative integer in
> `self_compose` and `nest`, otherwise we can get strange results

Done:

```
sage: self_compose(sin, -3)
...
ValueError: n must be a nonnegative integer, not -3.
sage: self_compose(sin, x)
...
TypeError: n (=x) must be of type (<type 'int'>, <type 'long'>, <type 'sage.rings.integer.Integer'>).
```



---

archive/issue_comments_058184.json:
```json
{
    "body": "<div id=\"comment:41\" align=\"right\">comment:41</div>\n\npositive review, thank you Christoph and Felix for that nice work!\n\nPaul",
    "created_at": "2011-02-19T14:14:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58184",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:41" align="right">comment:41</div>

positive review, thank you Christoph and Felix for that nice work!

Paul



---

archive/issue_events_096084.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-02-19T14:14:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96084"
}
```



---

archive/issue_events_096085.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-02-19T14:14:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96085"
}
```



---

archive/issue_comments_058185.json:
```json
{
    "body": "Changed work issues from **check argument type** to none",
    "created_at": "2011-02-19T14:14:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58185",
    "user": "https://github.com/zimmermann6"
}
```

Changed work issues from **check argument type** to none



---

archive/issue_comments_058186.json:
```json
{
    "body": "<div id=\"comment:42\" align=\"right\">comment:42</div>\n\nand thank you for the extra rebase work, I hope this time the patch will be included soon...\n\nPaul",
    "created_at": "2011-02-19T14:16:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58186",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:42" align="right">comment:42</div>

and thank you for the extra rebase work, I hope this time the patch will be included soon...

Paul



---

archive/issue_comments_058187.json:
```json
{
    "body": "Merged: **sage-4.7.alpha2**",
    "created_at": "2011-03-17T19:22:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7742#issuecomment-58187",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha2**



---

archive/issue_events_096086.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-17T19:22:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96086"
}
```



---

archive/issue_events_096087.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-17T19:22:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7742",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7742#event-96087"
}
```
