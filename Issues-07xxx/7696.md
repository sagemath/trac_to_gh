# Issue 7696: zlib -- source spkg contains precompiled binary crap (.obj files)

archive/issues_007696.json:
```json
{
    "assignees": [],
    "body": "\n```\nzlib-1.2.3.p4/src/contrib/masmx86/inffas32.obj: 80386 COFF\nzlib-1.2.3.p4/src/contrib/masmx86/gvmat32.obj: 80386 COFF\nzlib-1.2.3.p4/src/contrib/masmx64/inffasx64.obj: ACB archive data\nzlib-1.2.3.p4/src/contrib/masmx64/gvmat64.obj: ACB archive data\n```\n\nDelete the above and it builds fine.  Similar directories don't have obj files.\n\nNote that the above is for Microsoft Windows anyways, so it's especially important we don't distribute their binary stuff!\n\nAuthor: **William Stein**\n\nReviewer: **Mike Hansen**\n\nMerged: **sage-4.3.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7696_\n\n",
    "closed_at": "2010-01-03T22:23:23Z",
    "created_at": "2009-12-16T00:54:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "zlib -- source spkg contains precompiled binary crap (.obj files)",
    "type": "issue",
    "updated_at": "2010-01-03T22:23:23Z",
    "url": "https://github.com/sagemath/sage/issues/7696",
    "user": "https://github.com/williamstein"
}
```

```
zlib-1.2.3.p4/src/contrib/masmx86/inffas32.obj: 80386 COFF
zlib-1.2.3.p4/src/contrib/masmx86/gvmat32.obj: 80386 COFF
zlib-1.2.3.p4/src/contrib/masmx64/inffasx64.obj: ACB archive data
zlib-1.2.3.p4/src/contrib/masmx64/gvmat64.obj: ACB archive data
```

Delete the above and it builds fine.  Similar directories don't have obj files.

Note that the above is for Microsoft Windows anyways, so it's especially important we don't distribute their binary stuff!

Author: **William Stein**

Reviewer: **Mike Hansen**

Merged: **sage-4.3.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/7696_





---

archive/issue_events_086359.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-16T00:54:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7696#event-86359"
}
```



---

archive/issue_events_086360.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-16T00:54:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7696#event-86360"
}
```



---

archive/issue_events_086361.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-16T00:54:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7696#event-86361"
}
```



---

archive/issue_events_086362.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-16T00:54:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7696#event-86362"
}
```



---

archive/issue_comments_059331.json:
```json
{
    "body": "Upstream: **Not yet reported upstream; Will do shortly.**",
    "created_at": "2009-12-16T00:54:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7696#issuecomment-59331",
    "user": "https://github.com/williamstein"
}
```

Upstream: **Not yet reported upstream; Will do shortly.**



---

archive/issue_comments_059332.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThe following new spkg deletes the binary crap *and* also greatly improves the SPKG.txt:\n\n    http://wstein.org/home/wstein/patches/zlib-1.2.3.p5.spkg",
    "created_at": "2009-12-18T06:17:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7696#issuecomment-59332",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">Comment 2</div>

The following new spkg deletes the binary crap *and* also greatly improves the SPKG.txt:

    http://wstein.org/home/wstein/patches/zlib-1.2.3.p5.spkg



---

archive/issue_events_086363.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-18T06:17:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7696#event-86363"
}
```



---

archive/issue_comments_059333.json:
```json
{
    "body": "Changed upstream from **Not yet reported upstream; Will do shortly.** to none",
    "created_at": "2009-12-27T16:09:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7696#issuecomment-59333",
    "user": "https://github.com/mwhansen"
}
```

Changed upstream from **Not yet reported upstream; Will do shortly.** to none



---

archive/issue_comments_059334.json:
```json
{
    "body": "Reviewer: **Mike Hansen**",
    "created_at": "2009-12-27T16:09:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7696#issuecomment-59334",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Mike Hansen**



---

archive/issue_comments_059335.json:
```json
{
    "body": "Author: **William Stein**",
    "created_at": "2009-12-27T16:09:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7696#issuecomment-59335",
    "user": "https://github.com/mwhansen"
}
```

Author: **William Stein**



---

archive/issue_comments_059336.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me.",
    "created_at": "2009-12-27T16:09:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7696#issuecomment-59336",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me.



---

archive/issue_events_086364.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-27T16:09:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7696#event-86364"
}
```



---

archive/issue_events_086365.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-27T16:09:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7696#event-86365"
}
```



---

archive/issue_comments_059337.json:
```json
{
    "body": "Merged: **sage-4.3.1.alpha0**",
    "created_at": "2010-01-03T22:23:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7696#issuecomment-59337",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.1.alpha0**



---

archive/issue_events_086366.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T22:23:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7696#event-86366"
}
```



---

archive/issue_events_086367.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T22:23:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7696",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7696#event-86367"
}
```
