# Issue 7025: givaro 3.2.13rc2 says GMP is not installed, even though MPIR is.

archive/issues_007025.json:
```json
{
    "body": "MPIR is supposed to be a substitute for GMP. When using the Sun compiler with sage-4.1.2.alpha2, givaro-3.2.13rc2 seems to think GMP is not installed, even though the substitute mpir is installed. See below. \n\nIt works on Solaris if gcc is used, which is a bit odd. I checked to see if the mpir had created the header files and libraries, and see that local/include/gmp.h and local/lib/libgmp.* do in fact exist, so mpir really has been installed. \n\n\n```\ngivaro-3.2.13rc2/src/examples/Polynomial/pol_eval.C\ngivaro-3.2.13rc2/src/examples/Polynomial/pol_factor.C\nFinished extraction\n****************************************************\nHost system\nuname -a:\nSunOS swan 5.10 Generic_139555-08 sun4u sparc SUNW,Sun-Blade-1000\n****************************************************\n****************************************************\nCC Version\n/opt/xxxsunstudio12.1/bin/cc -v\nusage: cc [ options] files.  Use 'cc -flags' for details\n****************************************************\nCopying updated gmp++.h\nchecking for a BSD-compatible install... /usr/local/bin/ginstall -c\nchecking whether build environment is sane... yes\nchecking for a thread-safe mkdir -p... ./install-sh -c -d\n<SNIP>\nchecking for GMP >= 3.1.1... not found\n*******************************************************************************\n ERROR: GMP not found!\n\n GMP version 3.1.1 or greater is required for this library to compile. Please\n make sure GMP is installed and specify its location with the option\n --with-gmp=<prefix> when running configure.\n*******************************************************************************\nError configuring givaro\n\nreal    0m22.266s\nuser    0m7.925s\nsys     0m13.148s\n```\n\nAssignee: tbd\n\nCC:  @dimpase\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/7025\n\n",
    "closed_at": "2020-08-22T07:15:11Z",
    "created_at": "2009-09-27T11:11:06Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "givaro 3.2.13rc2 says GMP is not installed, even though MPIR is.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7025",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```
MPIR is supposed to be a substitute for GMP. When using the Sun compiler with sage-4.1.2.alpha2, givaro-3.2.13rc2 seems to think GMP is not installed, even though the substitute mpir is installed. See below. 

It works on Solaris if gcc is used, which is a bit odd. I checked to see if the mpir had created the header files and libraries, and see that local/include/gmp.h and local/lib/libgmp.* do in fact exist, so mpir really has been installed. 


```
givaro-3.2.13rc2/src/examples/Polynomial/pol_eval.C
givaro-3.2.13rc2/src/examples/Polynomial/pol_factor.C
Finished extraction
****************************************************
Host system
uname -a:
SunOS swan 5.10 Generic_139555-08 sun4u sparc SUNW,Sun-Blade-1000
****************************************************
****************************************************
CC Version
/opt/xxxsunstudio12.1/bin/cc -v
usage: cc [ options] files.  Use 'cc -flags' for details
****************************************************
Copying updated gmp++.h
checking for a BSD-compatible install... /usr/local/bin/ginstall -c
checking whether build environment is sane... yes
checking for a thread-safe mkdir -p... ./install-sh -c -d
<SNIP>
checking for GMP >= 3.1.1... not found
*******************************************************************************
 ERROR: GMP not found!

 GMP version 3.1.1 or greater is required for this library to compile. Please
 make sure GMP is installed and specify its location with the option
 --with-gmp=<prefix> when running configure.
*******************************************************************************
Error configuring givaro

real    0m22.266s
user    0m7.925s
sys     0m13.148s
```

Assignee: tbd

CC:  @dimpase

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/7025





---

archive/issue_events_028412.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28412"
}
```



---

archive/issue_events_028413.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28413"
}
```



---

archive/issue_events_028414.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28414"
}
```



---

archive/issue_events_028415.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28415"
}
```



---

archive/issue_events_028416.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28416"
}
```



---

archive/issue_events_028417.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28417"
}
```



---

archive/issue_events_028418.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28418"
}
```



---

archive/issue_comments_065346.json:
```json
{
    "body": "<a id='comment:6'></a>\nOutdated spkg or build system ticket, should be closed",
    "created_at": "2020-08-17T16:38:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7025#issuecomment-65346",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'></a>
Outdated spkg or build system ticket, should be closed



---

archive/issue_events_028419.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-17T16:38:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28419"
}
```



---

archive/issue_events_028420.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-17T16:38:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28420"
}
```



---

archive/issue_comments_065347.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2020-08-17T16:38:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7025#issuecomment-65347",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_065348.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2020-08-22T07:15:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7025#issuecomment-65348",
    "user": "https://github.com/slel"
}
```

Resolution: invalid



---

archive/issue_events_028421.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-22T07:15:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7025#event-28421"
}
```



---

archive/issue_comments_065349.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2020-08-22T07:15:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7025",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7025#issuecomment-65349",
    "user": "https://github.com/slel"
}
```

Changing upstream from "" to "N/A"
