# Issue 7078: [with patch; fixed and applied] SageNB -- Introspection does not format output properly

archive/issues_007078.json:
```json
{
    "body": "* LaTeX macros are not applied\n* Sometimes, output is parsed twice.\n\nAssignee: boothby\n\nKeywords: sagenb notebook\n\nResolution: fixed\n\nAuthor: Tim Dumol\n\nIssue created by migration from https://trac.sagemath.org/ticket/7078\n\n",
    "closed_at": "2009-09-30T08:17:09Z",
    "created_at": "2009-09-30T04:51:26Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1.2",
    "title": "[with patch; fixed and applied] SageNB -- Introspection does not format output properly",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7078",
    "user": "https://github.com/TimDumol"
}
```
* LaTeX macros are not applied
* Sometimes, output is parsed twice.

Assignee: boothby

Keywords: sagenb notebook

Resolution: fixed

Author: Tim Dumol

Issue created by migration from https://trac.sagemath.org/ticket/7078





---

archive/issue_comments_065826.json:
```json
{
    "body": "Fixes the formatting problems.",
    "created_at": "2009-09-30T04:52:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7078#issuecomment-65826",
    "user": "https://github.com/TimDumol"
}
```

Fixes the formatting problems.



---

archive/issue_comments_065827.json:
```json
{
    "body": "Attachment [trac_7078-introspection-formatting](tarball://root/attachments/some-uuid/ticket7078/trac_7078-introspection-formatting) by @TimDumol created at 2009-09-30 06:11:35\n\ntiny change to previous patch's EMBEDDED_MODE setting. William Stein.",
    "created_at": "2009-09-30T06:11:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7078#issuecomment-65827",
    "user": "https://github.com/TimDumol"
}
```

Attachment [trac_7078-introspection-formatting](tarball://root/attachments/some-uuid/ticket7078/trac_7078-introspection-formatting) by @TimDumol created at 2009-09-30 06:11:35

tiny change to previous patch's EMBEDDED_MODE setting. William Stein.



---

archive/issue_comments_065828.json:
```json
{
    "body": "Attachment [trac_7078-EMBEDDED_MODE_setting_fix.patch](tarball://root/attachments/some-uuid/ticket7078/trac_7078-EMBEDDED_MODE_setting_fix.patch) by @TimDumol created at 2009-09-30 06:11:55\n\n#7078 Eliminated circular dependency in `sage.misc.misc`.",
    "created_at": "2009-09-30T06:11:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7078#issuecomment-65828",
    "user": "https://github.com/TimDumol"
}
```

Attachment [trac_7078-EMBEDDED_MODE_setting_fix.patch](tarball://root/attachments/some-uuid/ticket7078/trac_7078-EMBEDDED_MODE_setting_fix.patch) by @TimDumol created at 2009-09-30 06:11:55

#7078 Eliminated circular dependency in `sage.misc.misc`.



---

archive/issue_events_028709.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/7078",
    "rename": {
        "from": "SageNB -- Introspection does not format output properly",
        "to": "[with patch] SageNB -- Introspection does not format output properly"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7078#event-28709"
}
```



---

archive/issue_comments_065829.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_7078-minor-fix.patch](tarball://root/attachments/some-uuid/ticket7078/trac_7078-minor-fix.patch) by @williamstein created at 2009-09-30 06:50:13",
    "created_at": "2009-09-30T06:50:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7078#issuecomment-65829",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [trac_7078-minor-fix.patch](tarball://root/attachments/some-uuid/ticket7078/trac_7078-minor-fix.patch) by @williamstein created at 2009-09-30 06:50:13



---

archive/issue_comments_065830.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-09-30T08:17:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7078#issuecomment-65830",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed



---

archive/issue_events_028710.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-30T08:17:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7078",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7078#event-28710"
}
```



---

archive/issue_events_028711.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/7078",
    "rename": {
        "from": "[with patch] SageNB -- Introspection does not format output properly",
        "to": "[with patch; fixed and applied] SageNB -- Introspection does not format output properly"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7078#event-28711"
}
```



---

archive/issue_events_028712.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-30T08:17:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7078",
    "milestone": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7078#event-28712"
}
```
