...
<a id='comment:1'></a>Ultimately factorization over number fields are done by PARI.  Looking at g.factor?? you see that the factorization is ultimately handled by PARI after some manipulations to account for denominators in the defining polynomial for the number field.  So it's likely that the bug is actually in PARI.   But we'll see.  If I were debugging this I would put some print statements in the factor function to see exactly what is passed off to pari and see if pari is buggy.  If so, report upstream, after trying the latest svn version.
...
```json
{
    "body": "<a id='comment:3'></a>This might be the same as #7097?",
...
<a id='comment:3'></a>This might be the same as #7097?
...
<a id='comment:4'></a>It may well be that the short-term fix I put in at #7097 is not yet good enough.  [It is short-term since the latest version of pari have fixed some bugs which arose for non-monic polynomials, which is why the patch  I put in at #7097 made sure that pari was only called to factor monic ones.]
...
```json
{
    "body": "<a id='comment:5'></a>There's an SPKG for the new PARI at #8453 which fixes John's problem, but not the one reported in the summary.",
...
<a id='comment:5'></a>There's an SPKG for the new PARI at #8453 which fixes John's problem, but not the one reported in the summary.
...
<a id='comment:6'></a>The following script confirms that all is well using either nffactor() or factornf() in 
...
```json
{
    "body": "<a id='comment:7'></a>I have now checked that the work at #9343 completely fixes this bug.  So when that goes in, this can be changed.",
...
<a id='comment:7'></a>I have now checked that the work at #9343 completely fixes this bug.  So when that goes in, this can be changed.
...
```json
{
    "body": "<a id='comment:10'></a>I assume this is still fixed by #9343.  If not, please reopen this.",
    "created_at": "2010-09-10T10:49:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7736",
    "type": "issue_comment",
...
<a id='comment:10'></a>I assume this is still fixed by #9343.  If not, please reopen this.
