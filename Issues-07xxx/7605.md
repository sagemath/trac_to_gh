# Issue 7605: Graph.vertex_connectivity

archive/issues_007605.json:
```json
{
    "body": "As the title says, this patch implements the function vertex_connectivity for Graphs.\n\nYou could be in need of #7270 and GLPK from  http://sagemath.org/packages/optional/glpk-4.38.p4.spkg depending on the version of Sage you are using !!!\n\n\nAssignee: @rlmill\n\nResolution: fixed\n\nAuthor: Nathann Cohen\n\nReviewer: Robert Miller\n\nMerged: sage-4.3.rc1\n\nIssue created by migration from https://trac.sagemath.org/ticket/7605\n\n",
    "closed_at": "2009-12-19T20:27:22Z",
    "created_at": "2009-12-04T18:25:28Z",
    "labels": [
        "component: graph theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.3",
    "title": "Graph.vertex_connectivity",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7605",
    "user": "https://github.com/nathanncohen"
}
```
As the title says, this patch implements the function vertex_connectivity for Graphs.

You could be in need of #7270 and GLPK from  http://sagemath.org/packages/optional/glpk-4.38.p4.spkg depending on the version of Sage you are using !!!


Assignee: @rlmill

Resolution: fixed

Author: Nathann Cohen

Reviewer: Robert Miller

Merged: sage-4.3.rc1

Issue created by migration from https://trac.sagemath.org/ticket/7605





---

archive/issue_comments_075401.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2009-12-04T18:26:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75401",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_075402.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2009-12-15T17:54:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75402",
    "user": "https://github.com/rlmill"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_075403.json:
```json
{
    "body": "<a id='comment:2'></a>You need a doctest which illustrates the `sets` input option.",
    "created_at": "2009-12-15T17:54:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75403",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:2'></a>You need a doctest which illustrates the `sets` input option.



---

archive/issue_comments_075404.json:
```json
{
    "body": "Attachment [trac_7605.patch](tarball://root/attachments/some-uuid/ticket7605/trac_7605.patch) by @nathanncohen created at 2009-12-16 11:05:36",
    "created_at": "2009-12-16T11:05:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75404",
    "user": "https://github.com/nathanncohen"
}
```

Attachment [trac_7605.patch](tarball://root/attachments/some-uuid/ticket7605/trac_7605.patch) by @nathanncohen created at 2009-12-16 11:05:36



---

archive/issue_comments_075405.json:
```json
{
    "body": "<a id='comment:3'></a>Updated !",
    "created_at": "2009-12-16T11:05:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75405",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>Updated !



---

archive/issue_comments_075406.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2009-12-16T11:05:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75406",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_075407.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2009-12-16T18:29:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75407",
    "user": "https://github.com/rlmill"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_075408.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Robert Miller\"",
    "created_at": "2009-12-16T18:29:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75408",
    "user": "https://github.com/rlmill"
}
```

Changing reviewer from "" to "Robert Miller"



---

archive/issue_comments_075409.json:
```json
{
    "body": "Changing author from \"\" to \"Nathann Cohen\"",
    "created_at": "2009-12-16T18:29:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75409",
    "user": "https://github.com/rlmill"
}
```

Changing author from "" to "Nathann Cohen"



---

archive/issue_comments_075410.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-12-19T20:27:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75410",
    "user": "https://github.com/mwhansen"
}
```

Resolution: fixed



---

archive/issue_comments_075411.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.3.rc1\"",
    "created_at": "2009-12-19T20:27:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7605#issuecomment-75411",
    "user": "https://github.com/mwhansen"
}
```

Changing merged from "" to "sage-4.3.rc1"



---

archive/issue_events_018074.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-19T20:27:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7605#event-18074"
}
```



---

archive/issue_events_018075.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-20T07:43:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7605",
    "milestone": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7605#event-18075"
}
```
