# Issue 8870: Multiflow

archive/issues_008870.json:
```json
{
    "body": "Assignee: jason, ncohen, rlm\n\n* Requires #8364\n\nIssue created by migration from https://trac.sagemath.org/ticket/8870\n\n",
    "created_at": "2010-05-04T17:27:20Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.5",
    "title": "Multiflow",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/8870",
    "user": "https://github.com/nathanncohen"
}
```
Assignee: jason, ncohen, rlm

* Requires #8364

Issue created by migration from https://trac.sagemath.org/ticket/8870





---

archive/issue_comments_081405.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-05-08T16:53:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81405",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_081406.json:
```json
{
    "body": "Changing assignee from jason, ncohen, rlm to @nathanncohen.",
    "created_at": "2010-05-08T22:02:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81406",
    "user": "https://github.com/nathanncohen"
}
```

Changing assignee from jason, ncohen, rlm to @nathanncohen.



---

archive/issue_comments_081407.json:
```json
{
    "body": "In this patch, the method multi-commodity flow is defined. The code is also refactored as this method shares many common points with flow. The new code is more compact and (I hope) easier to understand ! :-)\n\nNathann",
    "created_at": "2010-05-08T22:18:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81407",
    "user": "https://github.com/nathanncohen"
}
```

In this patch, the method multi-commodity flow is defined. The code is also refactored as this method shares many common points with flow. The new code is more compact and (I hope) easier to understand ! :-)

Nathann



---

archive/issue_comments_081408.json:
```json
{
    "body": "1. Needs rebasing (applying on top of #9269, #8781, and #9230).\n\n2. `# optional` tags need to follow the correct format.",
    "created_at": "2010-06-21T20:40:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81408",
    "user": "https://github.com/rlmill"
}
```

1. Needs rebasing (applying on top of #9269, #8781, and #9230).

2. `# optional` tags need to follow the correct format.



---

archive/issue_comments_081409.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2010-06-21T20:40:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81409",
    "user": "https://github.com/rlmill"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_081410.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2010-06-21T21:06:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81410",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_081411.json:
```json
{
    "body": "Updated ! A nasty piece to rebase... ;-)\n\nNathann",
    "created_at": "2010-06-21T21:06:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81411",
    "user": "https://github.com/nathanncohen"
}
```

Updated ! A nasty piece to rebase... ;-)

Nathann



---

archive/issue_comments_081412.json:
```json
{
    "body": "Attachment [trac_8870.patch](tarball://root/attachments/some-uuid/ticket8870/trac_8870.patch) by @nathanncohen created at 2010-06-21 21:06:53",
    "created_at": "2010-06-21T21:06:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81412",
    "user": "https://github.com/nathanncohen"
}
```

Attachment [trac_8870.patch](tarball://root/attachments/some-uuid/ticket8870/trac_8870.patch) by @nathanncohen created at 2010-06-21 21:06:53



---

archive/issue_comments_081413.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2010-06-21T21:50:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81413",
    "user": "https://github.com/rlmill"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_081414.json:
```json
{
    "body": "Attachment [trac_8870-ref.patch](tarball://root/attachments/some-uuid/ticket8870/trac_8870-ref.patch) by @rlmill created at 2010-06-21 21:50:44\n\nLooks good to me. Apply both patches.",
    "created_at": "2010-06-21T21:50:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81414",
    "user": "https://github.com/rlmill"
}
```

Attachment [trac_8870-ref.patch](tarball://root/attachments/some-uuid/ticket8870/trac_8870-ref.patch) by @rlmill created at 2010-06-21 21:50:44

Looks good to me. Apply both patches.



---

archive/issue_comments_081415.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-06-29T16:46:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8870#issuecomment-81415",
    "user": "https://github.com/rlmill"
}
```

Resolution: fixed



---

archive/issue_events_009031.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-29T16:46:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/8870",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8870#event-9031"
}
```
