...
~/tmp/sage/build/pkgs» for p in *; do
 if [This is the Trac macro *! -f "$p"/package-version.txt * that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#! -f "$p"/package-version.txt -macro); then
  continue
 fi
 if [This is the Trac macro *-z "$* that was inherited from the migration called with arguments (<$p/package-version.txt)" )](https://trac.sagemath.org/wiki/WikiMacros#-z "$-macro); then
