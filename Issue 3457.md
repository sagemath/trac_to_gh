# Issue 3457: Install sqlalchemy_migrate

archive/issues_003457.json:
```json
{
    "body": "Assignee: cwitty\n\nKeywords: database\n\nWe need this for #3456 and #2318\n\nhttp://code.google.com/p/sqlalchemy-migrate/\n\nit provides tools to migrate databases / tables between versions.\n\nIssue created by migration from https://trac.sagemath.org/ticket/3457\n\n",
    "created_at": "2008-06-18T01:30:18Z",
    "labels": [
        "misc",
        "blocker",
        "bug"
    ],
    "title": "Install sqlalchemy_migrate",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3457",
    "user": "boothby"
}
```
Assignee: cwitty

Keywords: database

We need this for #3456 and #2318

http://code.google.com/p/sqlalchemy-migrate/

it provides tools to migrate databases / tables between versions.

Issue created by migration from https://trac.sagemath.org/ticket/3457





---

archive/issue_comments_024372.json:
```json
{
    "body": "Changing type from defect to enhancement.",
    "created_at": "2009-01-23T02:46:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3457#issuecomment-24372",
    "user": "AlexGhitza"
}
```

Changing type from defect to enhancement.



---

archive/issue_comments_024373.json:
```json
{
    "body": "What idiot marked this as a blocker?",
    "created_at": "2009-01-23T09:04:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3457#issuecomment-24373",
    "user": "boothby"
}
```

What idiot marked this as a blocker?



---

archive/issue_comments_024374.json:
```json
{
    "body": "Changing priority from blocker to trivial.",
    "created_at": "2009-01-23T09:04:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3457#issuecomment-24374",
    "user": "boothby"
}
```

Changing priority from blocker to trivial.



---

archive/issue_comments_024375.json:
```json
{
    "body": "Nice idea, but since #15593 suggests we will remove sqlalchemy and since we have never actually used sqlalchemy *anywhere* in Sage *and* since it's available with `pip`, this is (currently) invalid and should have a new ticket opened if and when it is *necessary*.",
    "created_at": "2015-01-06T15:19:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3457#issuecomment-24375",
    "user": "kcrisman"
}
```

Nice idea, but since #15593 suggests we will remove sqlalchemy and since we have never actually used sqlalchemy *anywhere* in Sage *and* since it's available with `pip`, this is (currently) invalid and should have a new ticket opened if and when it is *necessary*.



---

archive/issue_comments_024376.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-01-06T15:19:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3457#issuecomment-24376",
    "user": "kcrisman"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_024377.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-01-06T15:19:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3457#issuecomment-24377",
    "user": "kcrisman"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_024378.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2015-01-13T01:19:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3457#issuecomment-24378",
    "user": "vbraun"
}
```

Resolution: wontfix
