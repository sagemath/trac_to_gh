# Issue 31574: singular build failure with Sage 9.3 source

archive/issues_031574.json:
```json
{
    "body": "Sage 9.3 fails to compile from source on my laptop (Ubuntu 18.04). The error comes from Singular:\n\n```\nx86_64-pc-linux-gnu-g++: fatal error: cannot execute 'cc1plus': execvp: No such file or directory\n```\n\nI attach the Singular build log.\n\nIssue created by migration from https://trac.sagemath.org/ticket/31811\n\n",
    "created_at": "2021-05-11T08:57:42Z",
    "labels": [
        "packages: standard",
        "major",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "singular build failure with Sage 9.3 source",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/31574",
    "user": "zimmerma"
}
```
Sage 9.3 fails to compile from source on my laptop (Ubuntu 18.04). The error comes from Singular:

```
x86_64-pc-linux-gnu-g++: fatal error: cannot execute 'cc1plus': execvp: No such file or directory
```

I attach the Singular build log.

Issue created by migration from https://trac.sagemath.org/ticket/31811





---

archive/issue_comments_451177.json:
```json
{
    "body": "Attachment [singular-4.2.0p1+2021-04-06+sage.log](tarball://root/attachments/some-uuid/ticket31811/singular-4.2.0p1+2021-04-06+sage.log) by mkoeppe created at 2021-05-11 17:19:40\n\nWe also need the top-level `config.log`",
    "created_at": "2021-05-11T17:19:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31574#issuecomment-451177",
    "user": "mkoeppe"
}
```

Attachment [singular-4.2.0p1+2021-04-06+sage.log](tarball://root/attachments/some-uuid/ticket31811/singular-4.2.0p1+2021-04-06+sage.log) by mkoeppe created at 2021-05-11 17:19:40

We also need the top-level `config.log`



---

archive/issue_comments_451178.json:
```json
{
    "body": "We have tested thoroughly on `ubuntu-bionic`. Perhaps it's time for a fresh OS installation?\n\n\n```\n# ls -l /usr/bin/*g++*\nlrwxrwxrwx 1 root root       5 May 20  2019 /usr/bin/g++ -> g++-7\nlrwxrwxrwx 1 root root      22 Dec  4  2019 /usr/bin/g++-7 -> x86_64-linux-gnu-g++-7\nlrwxrwxrwx 1 root root       5 May 20  2019 /usr/bin/x86_64-linux-gnu-g++ -> g++-7\n-rwxr-xr-x 1 root root 1047488 Dec  4  2019 /usr/bin/x86_64-linux-gnu-g++-7\n```\n\nWhat does it look like on your system?",
    "created_at": "2021-05-11T17:39:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31574#issuecomment-451178",
    "user": "mkoeppe"
}
```

We have tested thoroughly on `ubuntu-bionic`. Perhaps it's time for a fresh OS installation?


```
# ls -l /usr/bin/*g++*
lrwxrwxrwx 1 root root       5 May 20  2019 /usr/bin/g++ -> g++-7
lrwxrwxrwx 1 root root      22 Dec  4  2019 /usr/bin/g++-7 -> x86_64-linux-gnu-g++-7
lrwxrwxrwx 1 root root       5 May 20  2019 /usr/bin/x86_64-linux-gnu-g++ -> g++-7
-rwxr-xr-x 1 root root 1047488 Dec  4  2019 /usr/bin/x86_64-linux-gnu-g++-7
```

What does it look like on your system?



---

archive/issue_comments_451179.json:
```json
{
    "body": "Attachment [config.log](tarball://root/attachments/some-uuid/ticket31811/config.log) by zimmerma created at 2021-05-12 05:30:14\n\nReplying to [comment:1 mkoeppe]:\n> We also need the top-level `config.log`\n\nit is attached now.",
    "created_at": "2021-05-12T05:30:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31574#issuecomment-451179",
    "user": "zimmerma"
}
```

Attachment [config.log](tarball://root/attachments/some-uuid/ticket31811/config.log) by zimmerma created at 2021-05-12 05:30:14

Replying to [comment:1 mkoeppe]:
> We also need the top-level `config.log`

it is attached now.



---

archive/issue_comments_451180.json:
```json
{
    "body": "I get the same output (apart from the additional clang++):\n\n```\nzimmerma@mure:/usr/local/sage-9.3$ ls -l /usr/bin/*g++*\nlrwxrwxrwx 1 root root      27 May 16  2018 /usr/bin/clang++ -> ../lib/llvm-6.0/bin/clang++\nlrwxrwxrwx 1 root root      27 Apr  6  2018 /usr/bin/clang++-6.0 -> ../lib/llvm-6.0/bin/clang++\nlrwxrwxrwx 1 root root       5 May 20  2019 /usr/bin/g++ -> g++-7\nlrwxrwxrwx 1 root root      22 Dec  4  2019 /usr/bin/g++-7 -> x86_64-linux-gnu-g++-7\nlrwxrwxrwx 1 root root       5 May 20  2019 /usr/bin/x86_64-linux-gnu-g++ -> g++-7\n-rwxr-xr-x 1 root root 1047488 Dec  4  2019 /usr/bin/x86_64-linux-gnu-g++-7\n```\n",
    "created_at": "2021-05-12T05:32:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31574#issuecomment-451180",
    "user": "zimmerma"
}
```

I get the same output (apart from the additional clang++):

```
zimmerma@mure:/usr/local/sage-9.3$ ls -l /usr/bin/*g++*
lrwxrwxrwx 1 root root      27 May 16  2018 /usr/bin/clang++ -> ../lib/llvm-6.0/bin/clang++
lrwxrwxrwx 1 root root      27 Apr  6  2018 /usr/bin/clang++-6.0 -> ../lib/llvm-6.0/bin/clang++
lrwxrwxrwx 1 root root       5 May 20  2019 /usr/bin/g++ -> g++-7
lrwxrwxrwx 1 root root      22 Dec  4  2019 /usr/bin/g++-7 -> x86_64-linux-gnu-g++-7
lrwxrwxrwx 1 root root       5 May 20  2019 /usr/bin/x86_64-linux-gnu-g++ -> g++-7
-rwxr-xr-x 1 root root 1047488 Dec  4  2019 /usr/bin/x86_64-linux-gnu-g++-7
```




---

archive/issue_comments_451181.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2021-05-12T06:04:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31574#issuecomment-451181",
    "user": "zimmerma"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_451182.json:
```json
{
    "body": "sorry my fault. I had another g++ installation somewhere in my path. We can close this.",
    "created_at": "2021-05-12T06:04:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31574#issuecomment-451182",
    "user": "zimmerma"
}
```

sorry my fault. I had another g++ installation somewhere in my path. We can close this.



---

archive/issue_comments_451183.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2021-05-12T14:49:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31574#issuecomment-451183",
    "user": "mkoeppe"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_451184.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2021-05-28T16:49:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31574#issuecomment-451184",
    "user": "chapoton"
}
```

Resolution: invalid
