# Issue 14299: Typos in the reference manual: GAP interface and Weyl group

archive/issues_014299.json:
```json
{
    "body": "Assignee: mvngu\n\nFollowing https://groups.google.com/forum/?fromgroups=#!topic/sage-devel/zijERXXw43M, here are two small and trivial fixes for the reference manual:\n\n* GAP interface misses the G in GAP.\n* Weyl group category links to a Wikipedia article that does not exist\n\nIssue created by migration from https://trac.sagemath.org/ticket/14503\n\n",
    "created_at": "2013-04-28T21:28:15Z",
    "labels": [
        "documentation",
        "trivial",
        "bug"
    ],
    "title": "Typos in the reference manual: GAP interface and Weyl group",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/14299",
    "user": "mathzeta2"
}
```
Assignee: mvngu

Following https://groups.google.com/forum/?fromgroups=#!topic/sage-devel/zijERXXw43M, here are two small and trivial fixes for the reference manual:

* GAP interface misses the G in GAP.
* Weyl group category links to a Wikipedia article that does not exist

Issue created by migration from https://trac.sagemath.org/ticket/14503





---

archive/issue_comments_180462.json:
```json
{
    "body": "Attachment [trac_14503_typos.patch](tarball://root/attachments/some-uuid/ticket14503/trac_14503_typos.patch) by mathzeta2 created at 2013-04-28 21:31:33",
    "created_at": "2013-04-28T21:31:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14299#issuecomment-180462",
    "user": "mathzeta2"
}
```

Attachment [trac_14503_typos.patch](tarball://root/attachments/some-uuid/ticket14503/trac_14503_typos.patch) by mathzeta2 created at 2013-04-28 21:31:33



---

archive/issue_comments_180463.json:
```json
{
    "body": "Makes sense, and it is not likely to do anything very bad `:-D`\n\nThis being said, could you replace the line containing the link with the following ?\n\n```\nSee the `Wikipedia page of Weyl Groups <Weyl_group>`.\n```\n\n\nThat's how we now refer to Wikipedia pages `:-)`\n\nThanks !\n\nNathann",
    "created_at": "2013-04-29T09:40:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14299#issuecomment-180463",
    "user": "ncohen"
}
```

Makes sense, and it is not likely to do anything very bad `:-D`

This being said, could you replace the line containing the link with the following ?

```
See the `Wikipedia page of Weyl Groups <Weyl_group>`.
```


That's how we now refer to Wikipedia pages `:-)`

Thanks !

Nathann



---

archive/issue_comments_180464.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-04-29T09:40:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14299#issuecomment-180464",
    "user": "ncohen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_180465.json:
```json
{
    "body": "Attachment [trac_14503_typos_wikipedia.patch](tarball://root/attachments/some-uuid/ticket14503/trac_14503_typos_wikipedia.patch) by mathzeta2 created at 2013-04-29 19:16:39\n\nReplying to [comment:1 ncohen]:\n> This being said, could you replace the line containing the link with the following ?\n> {{{\n> See the `Wikipedia page of Weyl Groups <Weyl_group>`.\n> }}}\n> \n> That's how we now refer to Wikipedia pages `:-)`\n\nAttached is an extra patch with proper Wikipedia links. In the process of learning Sage's ReST rules, I stumbled upon another broken Wikipedia link: [multicommodity flow](http://www.sagemath.org/doc/reference/graphs/sage/graphs/generic_graph.html#sage.graphs.generic_graph.GenericGraph.multicommodity_flow). I hope that piggybacking this ticket is OK.",
    "created_at": "2013-04-29T19:16:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14299#issuecomment-180465",
    "user": "mathzeta2"
}
```

Attachment [trac_14503_typos_wikipedia.patch](tarball://root/attachments/some-uuid/ticket14503/trac_14503_typos_wikipedia.patch) by mathzeta2 created at 2013-04-29 19:16:39

Replying to [comment:1 ncohen]:
> This being said, could you replace the line containing the link with the following ?
> {{{
> See the `Wikipedia page of Weyl Groups <Weyl_group>`.
> }}}
> 
> That's how we now refer to Wikipedia pages `:-)`

Attached is an extra patch with proper Wikipedia links. In the process of learning Sage's ReST rules, I stumbled upon another broken Wikipedia link: [multicommodity flow](http://www.sagemath.org/doc/reference/graphs/sage/graphs/generic_graph.html#sage.graphs.generic_graph.GenericGraph.multicommodity_flow). I hope that piggybacking this ticket is OK.



---

archive/issue_comments_180466.json:
```json
{
    "body": "Helloooooooooooooo !!\n\n> Attached is an extra patch with proper Wikipedia links. In the process of learning Sage's ReST rules, I stumbled upon another broken Wikipedia link: [multicommodity flow](http://www.sagemath.org/doc/reference/graphs/sage/graphs/generic_graph.html#sage.graphs.generic_graph.GenericGraph.multicommodity_flow). I hope that piggybacking this ticket is OK.\n\nGloops. The one from `generic_graph.py` is probably one of my mistakes `:-)`\n\nThank you for those patches ! There is no problem at all with having several patches on the same ticket. You just need to explicitly say in the ticket's description how they are to be applied in that case (just one, or the other, or both). I just did it, so that you can see how you can create links toward patches in a ticket `:-)`",
    "created_at": "2013-04-29T20:53:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14299#issuecomment-180466",
    "user": "ncohen"
}
```

Helloooooooooooooo !!

> Attached is an extra patch with proper Wikipedia links. In the process of learning Sage's ReST rules, I stumbled upon another broken Wikipedia link: [multicommodity flow](http://www.sagemath.org/doc/reference/graphs/sage/graphs/generic_graph.html#sage.graphs.generic_graph.GenericGraph.multicommodity_flow). I hope that piggybacking this ticket is OK.

Gloops. The one from `generic_graph.py` is probably one of my mistakes `:-)`

Thank you for those patches ! There is no problem at all with having several patches on the same ticket. You just need to explicitly say in the ticket's description how they are to be applied in that case (just one, or the other, or both). I just did it, so that you can see how you can create links toward patches in a ticket `:-)`



---

archive/issue_comments_180467.json:
```json
{
    "body": "Good to go !\n\nNathann",
    "created_at": "2013-04-29T20:54:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14299#issuecomment-180467",
    "user": "ncohen"
}
```

Good to go !

Nathann



---

archive/issue_comments_180468.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-04-29T20:54:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14299#issuecomment-180468",
    "user": "ncohen"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_180469.json:
```json
{
    "body": "Please fill in your real names as Author and Reviewer.",
    "created_at": "2013-04-30T09:53:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14299#issuecomment-180469",
    "user": "jdemeyer"
}
```

Please fill in your real names as Author and Reviewer.



---

archive/issue_comments_180470.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2013-05-07T09:07:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14299#issuecomment-180470",
    "user": "jdemeyer"
}
```

Resolution: fixed
