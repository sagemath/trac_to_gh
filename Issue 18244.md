...
```
$ grep "^[This is the Trac macro *:space:* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#:space:-macro)*def [a-zA-Z0-9_]*(" $(find . -name "*.py") | grep "copy="
...
```
e$ grep "^[This is the Trac macro *:space:* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#:space:-macro)*def [a-zA-Z0-9_]*(" $(find . -name "*.py") | grep "inplace="
