# Issue 33337: Comparing integers to infinity is slow

archive/issues_033100.json:
```json
{
    "body": "Currently, comparisons involving infinities can be slow:\n\n```\nsage: %timeit 2 > oo  # slow\n4.96 \u00b5s \u00b1 35.1 ns per loop (mean \u00b1 std. dev. of 7 runs, 100000 loops each)\nsage: %timeit oo > oo  # ok\n321 ns \u00b1 4.57 ns per loop (mean \u00b1 std. dev. of 7 runs, 1000000 loops each)\nsage: %timeit 1/2 > oo  # slow\n5.95 \u00b5s \u00b1 311 ns per loop (mean \u00b1 std. dev. of 7 runs, 100000 loops each)\nsage: o = RR(oo)\nsage: %timeit o > oo  # slow\n2.5 \u00b5s \u00b1 26.8 ns per loop (mean \u00b1 std. dev. of 7 runs, 100000 loops each)\n```\n\nIt's probably a good idea to implement `__richcmp__` for the infinity elements to special case a few of these obvious things.\n\nCC:  @roed314\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/33337\n\n",
    "created_at": "2022-02-13T19:19:46Z",
    "labels": [
        "component: basic arithmetic"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Comparing integers to infinity is slow",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/33337",
    "user": "https://github.com/saraedum"
}
```
Currently, comparisons involving infinities can be slow:

```
sage: %timeit 2 > oo  # slow
4.96 µs ± 35.1 ns per loop (mean ± std. dev. of 7 runs, 100000 loops each)
sage: %timeit oo > oo  # ok
321 ns ± 4.57 ns per loop (mean ± std. dev. of 7 runs, 1000000 loops each)
sage: %timeit 1/2 > oo  # slow
5.95 µs ± 311 ns per loop (mean ± std. dev. of 7 runs, 100000 loops each)
sage: o = RR(oo)
sage: %timeit o > oo  # slow
2.5 µs ± 26.8 ns per loop (mean ± std. dev. of 7 runs, 100000 loops each)
```

It's probably a good idea to implement `__richcmp__` for the infinity elements to special case a few of these obvious things.

CC:  @roed314

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/33337





---

archive/issue_comments_471268.json:
```json
{
    "body": "<a id='comment:1'></a>In fact, there is already a special method `_richcmp_` implemented for infinite elements.\n\nI think that the slowness comes from the fact that `__richcmp__` first coerces the arguments to the same parent and so creates `InfinityRing(2)`, which is more or less slow:\n\n```\nsage: %timeit _ = InfinityRing(2)\n2.9 \u00b5s \u00b1 17.2 ns per loop (mean \u00b1 std. dev. of 7 runs, 100000 loops each)\n```",
    "created_at": "2022-02-13T21:15:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33337#issuecomment-471268",
    "user": "https://github.com/xcaruso"
}
```

<a id='comment:1'></a>In fact, there is already a special method `_richcmp_` implemented for infinite elements.

I think that the slowness comes from the fact that `__richcmp__` first coerces the arguments to the same parent and so creates `InfinityRing(2)`, which is more or less slow:

```
sage: %timeit _ = InfinityRing(2)
2.9 µs ± 17.2 ns per loop (mean ± std. dev. of 7 runs, 100000 loops each)
```



---

archive/issue_comments_471269.json:
```json
{
    "body": "<a id='comment:2'></a>Exactly, that's why we should probably implement the double underscore method which does not do any coercion.",
    "created_at": "2022-02-13T21:40:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33337#issuecomment-471269",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:2'></a>Exactly, that's why we should probably implement the double underscore method which does not do any coercion.



---

archive/issue_comments_471270.json:
```json
{
    "body": "<a id='comment:3'></a>+1 on doing this. One hack around this that is fast is\n\n```\nsage: %timeit 2 < float(\"inf\")\n285 ns \u00b1 2.88 ns per loop (mean \u00b1 std. dev. of 7 runs, 1000000 loops each)\n```",
    "created_at": "2022-02-14T03:37:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33337#issuecomment-471270",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>+1 on doing this. One hack around this that is fast is

```
sage: %timeit 2 < float("inf")
285 ns ± 2.88 ns per loop (mean ± std. dev. of 7 runs, 1000000 loops each)
```



---

archive/issue_comments_471271.json:
```json
{
    "body": "<a id='comment:4'></a>Ah OK.\nI'm fine with doing this but I think that this solves only half of the problem because `2 > oo` calls `__richcmp__` on `2`.",
    "created_at": "2022-02-14T07:49:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33337#issuecomment-471271",
    "user": "https://github.com/xcaruso"
}
```

<a id='comment:4'></a>Ah OK.
I'm fine with doing this but I think that this solves only half of the problem because `2 > oo` calls `__richcmp__` on `2`.



---

archive/issue_events_087965.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:45:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/33337",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33337#event-87965"
}
```



---

archive/issue_events_087966.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/33337",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33337#event-87966"
}
```



---

archive/issue_events_087967.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/33337",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33337#event-87967"
}
```
