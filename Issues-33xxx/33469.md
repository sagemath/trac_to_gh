...
<a id='comment:3'></a>I hit presumably the same problem this morning in class with
students that wanted to build widgets in Sage.
...
```json
{
    "body": "<a id='comment:4'></a>Eric: does the workaround work for ipyml?\n\nCheers,",
...
<a id='comment:4'></a>Eric: does the workaround work for ipyml?
...
```json
{
    "body": "<a id='comment:7'></a>Great. So the next step is to implement the fix directly in Sage.\n\nI might not be able to do it in the next days, so anyone feel free\nto take the lead :-)",
...
<a id='comment:7'></a>Great. So the next step is to implement the fix directly in Sage.
...
```json
{
    "body": "<a id='comment:9'></a>Miracle: upgrading my sage was quick, and git trac still functional on my laptop.\nFirst submission in a loooong time ...\n\n---\nNew commits:",
...
<a id='comment:9'></a>Miracle: upgrading my sage was quick, and git trac still functional on my laptop.
First submission in a loooong time ...
...
<a id='comment:11'></a>Well, needs review I guess.
...
<a id='comment:14'></a>> But if the test does not test the patch, then I think it is of no use.
...
```json
{
    "body": "<a id='comment:20'></a>OK, let's move on. Thank you Nicolas for the fix!",
...
<a id='comment:20'></a>OK, let's move on. Thank you Nicolas for the fix!
