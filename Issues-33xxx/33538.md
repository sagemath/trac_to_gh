# Issue 33538: More doctests fix in sage_docbuild

archive/issues_033301.json:
```json
{
    "body": "A fix for a couple of doctest went missing from #33141.\n\n```\nsage -t --long --random-seed=22919418159437026337026400262276501942 /usr/lib/python3.10/site-packages/sage_docbuild/__init__.py\n**********************************************************************\nFile \"/usr/lib/python3.10/site-packages/sage_docbuild/__init__.py\", line 110, in sage_docbuild.builder_helper\nFailed example:\n    try:  # optional - sagemath_doc_html\n        build_many(build_ref_doc, [(\"docname\", \"en\", \"html\", {})])\n    except Exception as E:\n        \"Non-exception during docbuild: abort pool operation\" in str(E)\nExpected:\n    True\nGot:\n    False\n**********************************************************************\nFile \"/usr/lib/python3.10/site-packages/sage_docbuild/__init__.py\", line 211, in sage_docbuild.DocBuilder._doctrees_dir\nFailed example:\n    b._doctrees_dir()             # optional - sagemath_doc_html\nException raised:\n    Traceback (most recent call last):\n      File \"/usr/lib/python3.9/site-packages/sage/doctest/forker.py\", line 695, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/usr/lib/python3.9/site-packages/sage/doctest/forker.py\", line 1093, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage_docbuild.DocBuilder._doctrees_dir[2]>\", line 1, in <module>\n        b._doctrees_dir()             # optional - sagemath_doc_html\n      File \"/usr/lib/python3.9/site-packages/sage_docbuild/__init__.py\", line 210, in _doctrees_dir\n        os.makedirs(d, exist_ok=True)\n      File \"/usr/lib/python3.9/os.py\", line 215, in makedirs\n        makedirs(head, exist_ok=exist_ok)\n      File \"/usr/lib/python3.9/os.py\", line 215, in makedirs\n        makedirs(head, exist_ok=exist_ok)\n      File \"/usr/lib/python3.9/os.py\", line 225, in makedirs\n        mkdir(name, mode)\n    PermissionError: [Errno 13] Permission denied: '/usr/share/doc/sage-doc-9999/doctrees'\n```\nDoctesting this file in is made more robust, and distro friendly by allowing and making `SAGE_DOC` to be a random temporary directory at the start of the doctesting of the file.\n\nTo allow `SAGE_DOC` to be set randomly for doctesting we cannot let it be imported globally in the whole file. Functions that uses it need to import it explicitly. \n\n\nCC:  @jhpalmieri\n\nBranch/Commit: 35694089294172b03f09dca8e21039d3b9a1a88c\n\nReviewer: John Palmieri\n\nAuthor: Fran\u00e7ois Bissey\n\nDependencies: #33141\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/33538\n\n",
    "closed_at": "2022-04-02T10:53:13Z",
    "created_at": "2022-03-21T00:38:34Z",
    "labels": [
        "component: distribution",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.6",
    "title": "More doctests fix in sage_docbuild",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/33538",
    "user": "https://github.com/kiwifb"
}
```
A fix for a couple of doctest went missing from #33141.

```
sage -t --long --random-seed=22919418159437026337026400262276501942 /usr/lib/python3.10/site-packages/sage_docbuild/__init__.py
**********************************************************************
File "/usr/lib/python3.10/site-packages/sage_docbuild/__init__.py", line 110, in sage_docbuild.builder_helper
Failed example:
    try:  # optional - sagemath_doc_html
        build_many(build_ref_doc, [("docname", "en", "html", {})])
    except Exception as E:
        "Non-exception during docbuild: abort pool operation" in str(E)
Expected:
    True
Got:
    False
**********************************************************************
File "/usr/lib/python3.10/site-packages/sage_docbuild/__init__.py", line 211, in sage_docbuild.DocBuilder._doctrees_dir
Failed example:
    b._doctrees_dir()             # optional - sagemath_doc_html
Exception raised:
    Traceback (most recent call last):
      File "/usr/lib/python3.9/site-packages/sage/doctest/forker.py", line 695, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/usr/lib/python3.9/site-packages/sage/doctest/forker.py", line 1093, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage_docbuild.DocBuilder._doctrees_dir[2]>", line 1, in <module>
        b._doctrees_dir()             # optional - sagemath_doc_html
      File "/usr/lib/python3.9/site-packages/sage_docbuild/__init__.py", line 210, in _doctrees_dir
        os.makedirs(d, exist_ok=True)
      File "/usr/lib/python3.9/os.py", line 215, in makedirs
        makedirs(head, exist_ok=exist_ok)
      File "/usr/lib/python3.9/os.py", line 215, in makedirs
        makedirs(head, exist_ok=exist_ok)
      File "/usr/lib/python3.9/os.py", line 225, in makedirs
        mkdir(name, mode)
    PermissionError: [Errno 13] Permission denied: '/usr/share/doc/sage-doc-9999/doctrees'
```
Doctesting this file in is made more robust, and distro friendly by allowing and making `SAGE_DOC` to be a random temporary directory at the start of the doctesting of the file.

To allow `SAGE_DOC` to be set randomly for doctesting we cannot let it be imported globally in the whole file. Functions that uses it need to import it explicitly. 


CC:  @jhpalmieri

Branch/Commit: 35694089294172b03f09dca8e21039d3b9a1a88c

Reviewer: John Palmieri

Author: François Bissey

Dependencies: #33141

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/33538





---

archive/issue_comments_668524.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2022-03-21T00:39:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668524",
    "user": "https://github.com/kiwifb"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_668525.json:
```json
{
    "body": "<a id='comment:1'></a>New commits:",
    "created_at": "2022-03-21T00:39:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668525",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:1'></a>New commits:



---

archive/issue_comments_668526.json:
```json
{
    "body": "Changing branch from \"\" to \"u/fbissey/trac33538\"",
    "created_at": "2022-03-21T00:39:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668526",
    "user": "https://github.com/kiwifb"
}
```

Changing branch from "" to "u/fbissey/trac33538"



---

archive/issue_comments_668527.json:
```json
{
    "body": "Changing commit from \"\" to \"35694089294172b03f09dca8e21039d3b9a1a88c\"",
    "created_at": "2022-03-21T00:39:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668527",
    "user": "https://github.com/kiwifb"
}
```

Changing commit from "" to "35694089294172b03f09dca8e21039d3b9a1a88c"



---

archive/issue_comments_668528.json:
```json
{
    "body": "<a id='comment:3'></a>Would it be too clumsy to conditionally import `SAGE_DOC` at the top of the file if it is not yet defined? That would require the doctests to set the environment variable `SAGE_DOC` before importing from `sage_docbuild`, but at the moment there seems to be only one affected doctest (or at least `SAGE_DOC` is only defined in one testing block), while the current branch requires five new `from sage.env import SAGE_DOC` statements.",
    "created_at": "2022-03-26T23:26:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668528",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'></a>Would it be too clumsy to conditionally import `SAGE_DOC` at the top of the file if it is not yet defined? That would require the doctests to set the environment variable `SAGE_DOC` before importing from `sage_docbuild`, but at the moment there seems to be only one affected doctest (or at least `SAGE_DOC` is only defined in one testing block), while the current branch requires five new `from sage.env import SAGE_DOC` statements.



---

archive/issue_comments_668529.json:
```json
{
    "body": "<a id='comment:4'></a>I tried various approaches. I am not sure about importing conditionally. Would you look at os.getenv? While I only defined `SAGE_DOC` once, that definition is carried to all subsequent doctests. And it affects the results of at least another doctest apart from the obvious one, the one at line 211 as stated in the description.\n\nNow that I look at it with a bit of distance, there is one thing that could be interesting to try. Define `SAGE_DOC` in the doctest before the first import of `sage_docbuild` and see if that sticks.",
    "created_at": "2022-03-27T00:47:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668529",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:4'></a>I tried various approaches. I am not sure about importing conditionally. Would you look at os.getenv? While I only defined `SAGE_DOC` once, that definition is carried to all subsequent doctests. And it affects the results of at least another doctest apart from the obvious one, the one at line 211 as stated in the description.

Now that I look at it with a bit of distance, there is one thing that could be interesting to try. Define `SAGE_DOC` in the doctest before the first import of `sage_docbuild` and see if that sticks.



---

archive/issue_comments_668530.json:
```json
{
    "body": "<a id='comment:5'></a>Having thought about it more, I think it's too fragile to have doctests depend on whether `SAGE_DOC` is defined before the relevant `import` statements. Let's leave the branch as it is \u2014\u00a0I don't have any better ideas for how to fix this.\n\nTests pass on Sage built from scratch, and I trust that they fix problems, or at least don't cause any new ones, for distros.",
    "created_at": "2022-03-28T22:59:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668530",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>Having thought about it more, I think it's too fragile to have doctests depend on whether `SAGE_DOC` is defined before the relevant `import` statements. Let's leave the branch as it is — I don't have any better ideas for how to fix this.

Tests pass on Sage built from scratch, and I trust that they fix problems, or at least don't cause any new ones, for distros.



---

archive/issue_comments_668531.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-03-28T22:59:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668531",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_668532.json:
```json
{
    "body": "Changing reviewer from \"\" to \"John Palmieri\"",
    "created_at": "2022-03-28T22:59:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668532",
    "user": "https://github.com/jhpalmieri"
}
```

Changing reviewer from "" to "John Palmieri"



---

archive/issue_comments_668533.json:
```json
{
    "body": "<a id='comment:6'></a>Thank you.",
    "created_at": "2022-03-28T23:02:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668533",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:6'></a>Thank you.



---

archive/issue_events_088289.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-04-02T10:53:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33538#event-88289"
}
```



---

archive/issue_comments_668534.json:
```json
{
    "body": "Changing branch from \"u/fbissey/trac33538\" to \"35694089294172b03f09dca8e21039d3b9a1a88c\"",
    "created_at": "2022-04-02T10:53:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668534",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/fbissey/trac33538" to "35694089294172b03f09dca8e21039d3b9a1a88c"



---

archive/issue_comments_668535.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2022-04-02T10:53:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33538#issuecomment-668535",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
