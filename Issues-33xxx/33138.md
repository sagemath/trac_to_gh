# Issue 33138: Fix SAGE_FAT_BINARY regression for numpy ("Ignoring --build-option when building numpy using PEP 517")

archive/issues_032901.json:
```json
{
    "assignees": [],
    "body": "Seen in https://github.com/sagemath/sage/files/ticket33070/numpy-1.21.4.log\n\nThe change made as part of #32046, using `pip wheel --build-option ...` (instead of `setup.py bdist_wheel...`) seems to have broken the use of said build options when the package is a PEP 517 package (i.e., has `pyproject.toml`)\n\nAmong the packages found by `git grep sdh_prefix`, only **numpy** is \naffected (the others, `python_igraph`, `jupyter_jsmol`, `pillow`, do not have `pyproject.toml`).\n\nThe build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. This would explain the segfaults (again!) in the multi-stage Cygwin build, triggered by plots in doctests - https://github.com/sagemath/sage/runs/4768915607?check_suite_focus=true\n\n(Hence \"blocker\" for Sage 9.5.)\n\nAlso fixes a build problem on `alpine-standard` (#33083)\n\nCC:  @dimpase @kliem @jhpalmieri @culler\n\nBranch: **[455e1fa](https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33138_\n\n",
    "closed_at": "2022-01-12T23:52:06Z",
    "created_at": "2022-01-10T00:11:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix SAGE_FAT_BINARY regression for numpy (\"Ignoring --build-option when building numpy using PEP 517\")",
    "type": "issue",
    "updated_at": "2022-08-25T17:58:57Z",
    "url": "https://github.com/sagemath/sage/issues/33138",
    "user": "https://github.com/mkoeppe"
}
```
Seen in https://github.com/sagemath/sage/files/ticket33070/numpy-1.21.4.log

The change made as part of #32046, using `pip wheel --build-option ...` (instead of `setup.py bdist_wheel...`) seems to have broken the use of said build options when the package is a PEP 517 package (i.e., has `pyproject.toml`)

Among the packages found by `git grep sdh_prefix`, only **numpy** is 
affected (the others, `python_igraph`, `jupyter_jsmol`, `pillow`, do not have `pyproject.toml`).

The build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. This would explain the segfaults (again!) in the multi-stage Cygwin build, triggered by plots in doctests - https://github.com/sagemath/sage/runs/4768915607?check_suite_focus=true

(Hence "blocker" for Sage 9.5.)

Also fixes a build problem on `alpine-standard` (#33083)

CC:  @dimpase @kliem @jhpalmieri @culler

Branch: **[455e1fa](https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/33138_





---

archive/issue_events_437663.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-10T00:11:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437663"
}
```



---

archive/issue_events_437664.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-10T00:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437664"
}
```



---

archive/issue_events_437665.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-10T00:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437665"
}
```



---

archive/issue_events_437666.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-10T00:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437666"
}
```



---

archive/issue_comments_538519.json:
```json
{
    "body": "Branch: **[u/mkoeppe/ignoring___build_option_when_building_numpy_using_pep_517](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/ignoring___build_option_when_building_numpy_using_pep_517)**",
    "created_at": "2022-01-10T00:19:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538519",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/ignoring___build_option_when_building_numpy_using_pep_517](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/ignoring___build_option_when_building_numpy_using_pep_517)**



---

archive/issue_comments_538520.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d\">455e1fa</a></td><td><code>build/pkgs/numpy/spkg-install.in: Use pip wheel --no-use-pep517</code></td></tr></table>\n",
    "created_at": "2022-01-10T00:19:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538520",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d">455e1fa</a></td><td><code>build/pkgs/numpy/spkg-install.in: Use pip wheel --no-use-pep517</code></td></tr></table>




---

archive/issue_events_437667.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-10T00:19:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437667"
}
```



---

archive/issue_comments_538521.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,7 +3,7 @@\n The change made as part of #32046, using `pip wheel --build-option ...` (instead of `setup.py bdist_wheel...`) seems to have broken the use of said build options when the package is a PEP 517 package (i.e., has `pyproject.toml`)\n \n Among the packages found by `git grep sdh_prefix`, only **numpy** is \n-affected (the others, `python_igraph`, `jupyter_jsmol`, `pillow` (with do not have `pyproject.toml`).\n+affected (the others, `python_igraph`, `jupyter_jsmol`, `pillow`, do not have `pyproject.toml`).\n \n The build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. (Hence \"blocker\" for Sage 9.5.)\n \n``````\n",
    "created_at": "2022-01-10T00:19:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538521",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,7 +3,7 @@
 The change made as part of #32046, using `pip wheel --build-option ...` (instead of `setup.py bdist_wheel...`) seems to have broken the use of said build options when the package is a PEP 517 package (i.e., has `pyproject.toml`)
 
 Among the packages found by `git grep sdh_prefix`, only **numpy** is 
-affected (the others, `python_igraph`, `jupyter_jsmol`, `pillow` (with do not have `pyproject.toml`).
+affected (the others, `python_igraph`, `jupyter_jsmol`, `pillow`, do not have `pyproject.toml`).
 
 The build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. (Hence "blocker" for Sage 9.5.)
 
``````




---

archive/issue_comments_538522.json:
```json
{
    "body": "Commit: **[455e1fa](https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d)**",
    "created_at": "2022-01-10T00:19:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538522",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[455e1fa](https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d)**



---

archive/issue_comments_538523.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2022-01-10T00:19:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538523",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_437668.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-10T00:21:27Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "title_is": "Fix \"Ignoring --build-option when building numpy using PEP 517\"",
    "title_was": "Ignoring --build-option when building numpy using PEP 517",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437668"
}
```



---

archive/issue_comments_538524.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,3 +7,4 @@\n \n The build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. (Hence \"blocker\" for Sage 9.5.)\n \n+Also fixes a build problem on `alpine-standard` (#33083)\n``````\n",
    "created_at": "2022-01-10T02:29:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538524",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,3 +7,4 @@
 
 The build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. (Hence "blocker" for Sage 9.5.)
 
+Also fixes a build problem on `alpine-standard` (#33083)
``````




---

archive/issue_comments_538525.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nsee [#33083 comment:45](https://github.com/sagemath/sage/issues/33083#comment:45) - it throws an error, saying that\n`setup.py clean` is not supported.\n(this is actually a by-product of a failing build)",
    "created_at": "2022-01-10T12:44:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538525",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:5'>Comment 5:</a>
see [#33083 comment:45](https://github.com/sagemath/sage/issues/33083#comment:45) - it throws an error, saying that
`setup.py clean` is not supported.
(this is actually a by-product of a failing build)



---

archive/issue_comments_538526.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,6 +5,8 @@\n Among the packages found by `git grep sdh_prefix`, only **numpy** is \n affected (the others, `python_igraph`, `jupyter_jsmol`, `pillow`, do not have `pyproject.toml`).\n \n-The build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. (Hence \"blocker\" for Sage 9.5.)\n+The build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. This would explain the segfaults (again!) in the multi-stage Cygwin build, triggered by plots in doctests - https://github.com/sagemath/sage/runs/4768915607?check_suite_focus=true\n+\n+(Hence \"blocker\" for Sage 9.5.)\n \n Also fixes a build problem on `alpine-standard` (#33083)\n``````\n",
    "created_at": "2022-01-12T06:13:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538526",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,6 +5,8 @@
 Among the packages found by `git grep sdh_prefix`, only **numpy** is 
 affected (the others, `python_igraph`, `jupyter_jsmol`, `pillow`, do not have `pyproject.toml`).
 
-The build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. (Hence "blocker" for Sage 9.5.)
+The build options for numpy are in particular needed for `SAGE_FAT_BINARY=yes` builds. This would explain the segfaults (again!) in the multi-stage Cygwin build, triggered by plots in doctests - https://github.com/sagemath/sage/runs/4768915607?check_suite_focus=true
+
+(Hence "blocker" for Sage 9.5.)
 
 Also fixes a build problem on `alpine-standard` (#33083)
``````




---

archive/issue_events_437669.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-12T06:13:18Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "title_is": "Fix SAGE_FAT_BINARY regression for numpy (\"Ignoring --build-option when building numpy using PEP 517\")",
    "title_was": "Fix \"Ignoring --build-option when building numpy using PEP 517\"",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437669"
}
```



---

archive/issue_comments_538527.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2022-01-12T11:05:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538527",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_538528.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nOK, assuming the inability to build on Alpine in lxc (as reported on #33083) has nothing to do with this.",
    "created_at": "2022-01-12T11:05:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538528",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:8'>Comment 8:</a>
OK, assuming the inability to build on Alpine in lxc (as reported on #33083) has nothing to do with this.



---

archive/issue_events_437670.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-01-12T11:05:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437670"
}
```



---

archive/issue_events_437671.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-01-12T11:05:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437671"
}
```



---

archive/issue_comments_538529.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThanks.",
    "created_at": "2022-01-12T16:23:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538529",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:9'>Comment 9:</a>
Thanks.



---

archive/issue_events_437672.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-01-12T23:52:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437672"
}
```



---

archive/issue_events_437673.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "92bd27363784357ec92df4e7faf7795717e8c5e5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-01-12T23:52:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33138#event-437673"
}
```



---

archive/issue_comments_538530.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/ignoring___build_option_when_building_numpy_using_pep_517](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/ignoring___build_option_when_building_numpy_using_pep_517)** to **[455e1fa](https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d)**",
    "created_at": "2022-01-12T23:52:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538530",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/ignoring___build_option_when_building_numpy_using_pep_517](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/ignoring___build_option_when_building_numpy_using_pep_517)** to **[455e1fa](https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d)**



---

archive/issue_comments_538531.json:
```json
{
    "body": "Changed commit from **[455e1fa](https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d)** to none",
    "created_at": "2022-08-25T17:58:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538531",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[455e1fa](https://github.com/sagemath/sagetrac-mirror/commit/455e1fa3187386367b43d0e3869e5d39f5b8101d)** to none



---

archive/issue_comments_538532.json:
```json
{
    "body": "Replying to [ticket:33138 mkoeppe]:\n> Among the packages found by `git grep sdh_prefix`, only **numpy** is \n> affected (the others, `python_igraph`, `jupyter_jsmol`, `pillow`, do not have `pyproject.toml`).\n\nFollow-up for these packages in #34430, #34421, #34433.",
    "created_at": "2022-08-25T17:58:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33138#issuecomment-538532",
    "user": "https://github.com/mkoeppe"
}
```

Replying to [ticket:33138 mkoeppe]:
> Among the packages found by `git grep sdh_prefix`, only **numpy** is 
> affected (the others, `python_igraph`, `jupyter_jsmol`, `pillow`, do not have `pyproject.toml`).

Follow-up for these packages in #34430, #34421, #34433.
