...
```json
{
    "body": "Shouldn't this use `os.path.join`?",
...
```json
{
    "body": "it works for me on opensuse-tumblewheed with a system fplll, and it works on macOS with Sage's built fplll.\n\nHow do you get this error?",
...
```json
{
    "body": "if it's not a wrong branch for you, please post `BKZ.DEFAULT_STRATEGY*` values you get after \n\n```\nfrom fpylll import BKZ\n```",
...
```json
{
    "body": "So do we need to check for a half-installed fplll in `configure`?",
...
```json
{
    "body": "`fplll-devel` was not what was missing, `fplll` was",
