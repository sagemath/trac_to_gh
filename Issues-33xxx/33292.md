# Issue 33292: mark invlex ordering as global

archive/issues_033055.json:
```json
{
    "assignees": [],
    "body": "It corresponds to `rp` [in Singular](https://www.singular.uni-kl.de/Manual/4-3-0/sing_978.htm#SEC1035) which is global.\n\n```\nsage: t = TermOrder(\"invlex\")\nsage: t.is_global()\nFalse\nsage: t._singular_str\nrp\n```\n\nBranch/Commit: **[8731cb3](https://github.com/sagemath/sagetrac-mirror/commit/8731cb329480d567cc59cfbef5b90ecf7bcd718e)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33292_\n\n",
    "closed_at": "2022-03-03T22:19:38Z",
    "created_at": "2022-02-05T12:53:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "mark invlex ordering as global",
    "type": "issue",
    "updated_at": "2022-03-03T22:19:38Z",
    "url": "https://github.com/sagemath/sage/issues/33292",
    "user": "https://github.com/mwageringel"
}
```
It corresponds to `rp` [in Singular](https://www.singular.uni-kl.de/Manual/4-3-0/sing_978.htm#SEC1035) which is global.

```
sage: t = TermOrder("invlex")
sage: t.is_global()
False
sage: t._singular_str
rp
```

Branch/Commit: **[8731cb3](https://github.com/sagemath/sagetrac-mirror/commit/8731cb329480d567cc59cfbef5b90ecf7bcd718e)**

Reviewer: **Travis Scrimshaw**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/33292_





---

archive/issue_events_439566.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2022-02-05T12:53:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33292#event-439566"
}
```



---

archive/issue_events_439567.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2022-02-05T12:53:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33292#event-439567"
}
```



---

archive/issue_events_439568.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2022-02-05T12:53:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33292#event-439568"
}
```



---

archive/issue_events_439569.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2022-02-05T12:53:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33292#event-439569"
}
```



---

archive/issue_comments_541116.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8731cb329480d567cc59cfbef5b90ecf7bcd718e\">8731cb3</a></td><td><code>33292: mark invlex ordering as global</code></td></tr></table>\n",
    "created_at": "2022-02-05T13:07:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33292#issuecomment-541116",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8731cb329480d567cc59cfbef5b90ecf7bcd718e">8731cb3</a></td><td><code>33292: mark invlex ordering as global</code></td></tr></table>




---

archive/issue_comments_541117.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/33292](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/33292)**",
    "created_at": "2022-02-05T13:07:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33292#issuecomment-541117",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/33292](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/33292)**



---

archive/issue_comments_541118.json:
```json
{
    "body": "Commit: **[8731cb3](https://github.com/sagemath/sagetrac-mirror/commit/8731cb329480d567cc59cfbef5b90ecf7bcd718e)**",
    "created_at": "2022-02-05T13:07:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33292#issuecomment-541118",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[8731cb3](https://github.com/sagemath/sagetrac-mirror/commit/8731cb329480d567cc59cfbef5b90ecf7bcd718e)**



---

archive/issue_events_439570.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2022-02-05T13:07:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33292#event-439570"
}
```



---

archive/issue_comments_541119.json:
```json
{
    "body": "Author: **Markus Wageringel**",
    "created_at": "2022-02-05T13:07:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33292#issuecomment-541119",
    "user": "https://github.com/mwageringel"
}
```

Author: **Markus Wageringel**



---

archive/issue_events_439571.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-27T00:52:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33292#event-439571"
}
```



---

archive/issue_events_439572.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-27T00:52:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33292#event-439572"
}
```



---

archive/issue_comments_541120.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-02-27T00:52:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33292#issuecomment-541120",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_541121.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2022-02-27T00:52:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33292#issuecomment-541121",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_events_439573.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-03-03T22:19:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33292#event-439573"
}
```



---

archive/issue_events_439574.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "00c2f53ed91b2b5b4719eca8237e02a2cb8d01ce",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-03-03T22:19:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33292#event-439574"
}
```



---

archive/issue_comments_541122.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/33292](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/33292)** to **[8731cb3](https://github.com/sagemath/sagetrac-mirror/commit/8731cb329480d567cc59cfbef5b90ecf7bcd718e)**",
    "created_at": "2022-03-03T22:19:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33292#issuecomment-541122",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/33292](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/33292)** to **[8731cb3](https://github.com/sagemath/sagetrac-mirror/commit/8731cb329480d567cc59cfbef5b90ecf7bcd718e)**
