...
<a id='comment:53'></a>Replying to [comment:37 git]:
> Branch pushed to git repo; I updated commit sha1. New commits:
> |                                                                                                                                           |                                    |
> |-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|
> |[a379ea3](https://git.sagemath.org/sage.git/commit/?id=a379ea3a3d4020bf344bf8ed4c562018cacb661d)|`build/pkgs/cypari: Update to 2.1.3`|
...
```json
{
    "body": "<a id='comment:56'></a>See comment:35 - current changes here on the branch need to be conditionalized on python >= 3.11",
...
<a id='comment:56'></a>See comment:35 - current changes here on the branch need to be conditionalized on python >= 3.11
...
```json
{
    "body": "<a id='comment:57'></a>And comment:48 is still open - this requires major work",
...
<a id='comment:57'></a>And comment:48 is still open - this requires major work
...
<a id='comment:58'></a>Replying to [comment:53 Gonzalo Tornaría]:
> Replying to [comment:37 git]:
> > Branch pushed to git repo; I updated commit sha1. New commits:
> > |                                                                                                                                           |                                    |
> > |-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|
...
<a id='comment:59'></a>Replying to [comment:57 Matthias Köppe]:
> And comment:48 is still open - this requires major work
...
<a id='comment:60'></a>I upgraded cypari to 2.1.3 and fixed running tests (see comment:79:ticket:34537).
...
<a id='comment:61'></a>Replying to [comment:60 Gonzalo Tornaría]:
> So this seems to be comment:48. Somehow I thought you had sage running and only some tests would break...
...
<a id='comment:70'></a>Replying to [comment:35 Matthias Köppe]:
> I think we need to conditionalize this - `Py_SET_TYPE`, `Py_SET_SIZE` were added in 3.9, but we still support 3.8
...
<a id='comment:71'></a>Replying to [comment:70 Gonzalo Tornaría]:
> Replying to [comment:35 Matthias Köppe]:
> > I think we need to conditionalize this - `Py_SET_TYPE`, `Py_SET_SIZE` were added in 3.9, but we still support 3.8
...
<a id='comment:73'></a>Replying to [comment:71 Matthias Köppe]:
> Replying to [comment:70 Gonzalo Tornaría]:
> > Replying to [comment:35 Matthias Köppe]:
> > > I think we need to conditionalize this - `Py_SET_TYPE`, `Py_SET_SIZE` were added in 3.9, but we still support 3.8
...
<a id='comment:80'></a>Replying to [comment:35 Matthias Köppe]:
> I think we need to conditionalize this - `Py_SET_TYPE`, `Py_SET_SIZE` were added in 3.9, but we still support 3.8
...
<a id='comment:81'></a>Replying to [comment:78 Gonzalo Tornaría]:
> Just as a heads up, with the last version all doctests pass for me (both python 3.10 and 3.11) except for the following //ocassional// failure:
> 
> ```
> **********************************************************************
...
<a id='comment:84'></a>As per comment:80, something like the following should restore support for python 3.8, but I have not tested it. 
...
```json
{
    "body": "<a id='comment:96'></a>Is comment:50 resolved?",
...
<a id='comment:96'></a>Is comment:50 resolved?
...
<a id='comment:100'></a>Replying to [comment:99 Andrew]:
> a lot of "slated for removal" messages
...
<a id='comment:108'></a>Replying to [comment:99 Andrew]:
> a lot of "slated for removal" messages
...
<a id='comment:119'></a>Replying to [comment:117 Matthias Köppe]:
> This seems to come from `docutils`, which is upgraded in #34795 (please review).
...
<a id='comment:122'></a>Replying to [comment:121 Matthias Köppe]:
> The babel 2.11 upgrade is already here on the branch
...
<a id='comment:130'></a>Replying to [comment:128 Matthias Köppe]:
> Something else does need to be fixed on 32 bit though. After a successful `make build`, I get the following when starting `./sage`:
> 
> ```
> root@fd2f063d77c6:/sage# ./sage 
...
<a id='comment:144'></a>Replying to [comment:140 Matthias Köppe]:
> I'll also open an issue with docutils
