# Issue 33676: add .bit_length() method to Integer

archive/issues_033439.json:
```json
{
    "assignees": [],
    "body": "Python has `int.bit_length()`, but Sage has `Integer.nbits()`.\n\nIn this patch, we add `Integer.bit_length()` for compatibility with `int`.\n\nAmong other things, this change should make it easier to switch from Python to Sage mid-development, which currently requires replacing all calls to `.bit_length()` or adding casts to `int`.\n\nThis helps with\n\n- #29728: Meta-ticket: improve compatibility with the Python library and Python packages\n\nBranch: **[fb8f165](https://github.com/sagemath/sagetrac-mirror/commit/fb8f1650e190bc5e723eb6ffaee3ab9798a3bc3e)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Lorenz Panny**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33676_\n\n",
    "closed_at": "2022-05-22T09:57:46Z",
    "created_at": "2022-04-11T04:09:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add .bit_length() method to Integer",
    "type": "issue",
    "updated_at": "2022-05-31T16:09:04Z",
    "url": "https://github.com/sagemath/sage/issues/33676",
    "user": "https://github.com/yyyyx4"
}
```
Python has `int.bit_length()`, but Sage has `Integer.nbits()`.

In this patch, we add `Integer.bit_length()` for compatibility with `int`.

Among other things, this change should make it easier to switch from Python to Sage mid-development, which currently requires replacing all calls to `.bit_length()` or adding casts to `int`.

This helps with

- #29728: Meta-ticket: improve compatibility with the Python library and Python packages

Branch: **[fb8f165](https://github.com/sagemath/sagetrac-mirror/commit/fb8f1650e190bc5e723eb6ffaee3ab9798a3bc3e)**

Reviewer: **Frédéric Chapoton**

Author: **Lorenz Panny**

_Issue created by migration from https://trac.sagemath.org/ticket/33676_





---

archive/issue_events_443972.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-04-11T04:09:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443972"
}
```



---

archive/issue_events_443973.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-04-11T04:09:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443973"
}
```



---

archive/issue_events_443974.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-04-11T04:09:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443974"
}
```



---

archive/issue_events_443975.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-04-11T04:09:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443975"
}
```



---

archive/issue_events_443976.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-04-11T05:52:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443976"
}
```



---

archive/issue_events_443977.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-04-11T08:56:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443977"
}
```



---

archive/issue_events_443978.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-04-11T08:56:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443978"
}
```



---

archive/issue_comments_547486.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok, looks good",
    "created_at": "2022-04-11T08:56:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33676#issuecomment-547486",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok, looks good



---

archive/issue_comments_547487.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2022-04-11T08:56:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33676#issuecomment-547487",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_547488.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks!",
    "created_at": "2022-04-11T09:02:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33676#issuecomment-547488",
    "user": "https://github.com/yyyyx4"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks!



---

archive/issue_events_443979.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-04-26T08:16:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443979"
}
```



---

archive/issue_events_443980.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-04-26T08:16:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443980"
}
```



---

archive/issue_events_443981.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-05-22T09:57:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443981"
}
```



---

archive/issue_events_443982.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-05-22T09:57:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33676#event-443982"
}
```



---

archive/issue_comments_547489.json:
```json
{
    "body": "Changed branch from **[public/add_bit_length_method_to_Integer](https://github.com/sagemath/sagetrac-mirror/tree/public/add_bit_length_method_to_Integer)** to **[fb8f165](https://github.com/sagemath/sagetrac-mirror/commit/fb8f1650e190bc5e723eb6ffaee3ab9798a3bc3e)**",
    "created_at": "2022-05-22T09:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33676#issuecomment-547489",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/add_bit_length_method_to_Integer](https://github.com/sagemath/sagetrac-mirror/tree/public/add_bit_length_method_to_Integer)** to **[fb8f165](https://github.com/sagemath/sagetrac-mirror/commit/fb8f1650e190bc5e723eb6ffaee3ab9798a3bc3e)**



---

archive/issue_comments_547490.json:
```json
{
    "body": "Changed commit from **[fb8f165](https://github.com/sagemath/sagetrac-mirror/commit/fb8f1650e190bc5e723eb6ffaee3ab9798a3bc3e)** to none",
    "created_at": "2022-05-31T16:09:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33676#issuecomment-547490",
    "user": "https://github.com/slel"
}
```

Changed commit from **[fb8f165](https://github.com/sagemath/sagetrac-mirror/commit/fb8f1650e190bc5e723eb6ffaee3ab9798a3bc3e)** to none



---

archive/issue_comments_547491.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,3 +4,6 @@\n \n Among other things, this change should make it easier to switch from Python to Sage mid-development, which currently requires replacing all calls to `.bit_length()` or adding casts to `int`.\n \n+This helps with\n+\n+- #29728: Meta-ticket: improve compatibility with the Python library and Python packages\n``````\n",
    "created_at": "2022-05-31T16:09:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33676#issuecomment-547491",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,3 +4,6 @@
 
 Among other things, this change should make it easier to switch from Python to Sage mid-development, which currently requires replacing all calls to `.bit_length()` or adding casts to `int`.
 
+This helps with
+
+- #29728: Meta-ticket: improve compatibility with the Python library and Python packages
``````

