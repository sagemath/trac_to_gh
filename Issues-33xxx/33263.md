# Issue 33263: Add github action running on each push

archive/issues_033026.json:
```json
{
    "body": "Add a github action that runs on each push. Since it uses the most recent docker image as the base, the runtime is relatively short with about 2h. The idea is to get quick feedback on PRs without the need to wait until the patchbot picks it up. After this ticket is merged, we can add a badge in the ticket similar to the linter workflow.\n\nExample run: https://github.com/sagemath/sagetrac-mirror/actions/workflows/build.yml\n(it works but one doctest and the pytests are failing; I think there are already tickets for these issues)\n\nCC:  @mkoeppe @fchapoton\n\nBranch/Commit: 63298162ccec5c390018231b036a9dc5dd107e55\n\nReviewer: Matthias Koeppe\n\nAuthor: Tobias Diez\n\nDependencies: #33103\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/33263\n\n",
    "closed_at": "2022-02-16T23:57:16Z",
    "created_at": "2022-01-31T20:24:42Z",
    "labels": [
        "component: build",
        "critical"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.6",
    "title": "Add github action running on each push",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/33263",
    "user": "https://github.com/tobiasdiez"
}
```
Add a github action that runs on each push. Since it uses the most recent docker image as the base, the runtime is relatively short with about 2h. The idea is to get quick feedback on PRs without the need to wait until the patchbot picks it up. After this ticket is merged, we can add a badge in the ticket similar to the linter workflow.

Example run: https://github.com/sagemath/sagetrac-mirror/actions/workflows/build.yml
(it works but one doctest and the pytests are failing; I think there are already tickets for these issues)

CC:  @mkoeppe @fchapoton

Branch/Commit: 63298162ccec5c390018231b036a9dc5dd107e55

Reviewer: Matthias Koeppe

Author: Tobias Diez

Dependencies: #33103

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/33263





---

archive/issue_comments_663606.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2022-01-31T20:25:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663606",
    "user": "https://github.com/tobiasdiez"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_663607.json:
```json
{
    "body": "Changing commit from \"676799b3b45f2a85c3ce767d161d5660cde15788\" to \"1ab07550879f9d0f7d6fa626273a0b04eaabf9fa\"",
    "created_at": "2022-01-31T20:26:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663607",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "676799b3b45f2a85c3ce767d161d5660cde15788" to "1ab07550879f9d0f7d6fa626273a0b04eaabf9fa"



---

archive/issue_comments_663608.json:
```json
{
    "body": "<a id='comment:2'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2022-01-31T20:26:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663608",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:2'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_663609.json:
```json
{
    "body": "<a id='comment:3'></a>Why not just `configure --prefix=/sage/local`?",
    "created_at": "2022-01-31T22:13:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663609",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'></a>Why not just `configure --prefix=/sage/local`?



---

archive/issue_comments_663610.json:
```json
{
    "body": "<a id='comment:4'></a>Which jobs does it cancel?",
    "created_at": "2022-01-31T22:15:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663610",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'></a>Which jobs does it cancel?



---

archive/issue_comments_663611.json:
```json
{
    "body": "<a id='comment:5'></a>You may want to use #33233",
    "created_at": "2022-01-31T22:18:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663611",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>You may want to use #33233



---

archive/issue_comments_663612.json:
```json
{
    "body": "<a id='comment:6'></a>Replying to [mkoeppe](#comment%3A4):\n> Which jobs does it cancel?\n\nAlso perhaps the new built-in `concurrency` stuff can be used? https://docs.github.com/en/actions/using-workflows/workflow-syntax-for-github-actions#concurrency",
    "created_at": "2022-01-31T22:32:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663612",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'></a>Replying to [mkoeppe](#comment%3A4):
> Which jobs does it cancel?

Also perhaps the new built-in `concurrency` stuff can be used? https://docs.github.com/en/actions/using-workflows/workflow-syntax-for-github-actions#concurrency



---

archive/issue_comments_663613.json:
```json
{
    "body": "<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2022-02-01T13:29:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663613",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_663614.json:
```json
{
    "body": "Changing commit from \"1ab07550879f9d0f7d6fa626273a0b04eaabf9fa\" to \"d27ca7f2e5ae0483a226926aa11e69d6b4417bab\"",
    "created_at": "2022-02-01T13:29:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663614",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "1ab07550879f9d0f7d6fa626273a0b04eaabf9fa" to "d27ca7f2e5ae0483a226926aa11e69d6b4417bab"



---

archive/issue_comments_663615.json:
```json
{
    "body": "Changing commit from \"d27ca7f2e5ae0483a226926aa11e69d6b4417bab\" to \"4f5bcb9fe895728a5707a6cdaad7a9c5a49e9dab\"",
    "created_at": "2022-02-01T13:31:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663615",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "d27ca7f2e5ae0483a226926aa11e69d6b4417bab" to "4f5bcb9fe895728a5707a6cdaad7a9c5a49e9dab"



---

archive/issue_comments_663616.json:
```json
{
    "body": "<a id='comment:8'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2022-02-01T13:31:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663616",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:8'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_663617.json:
```json
{
    "body": "<a id='comment:9'></a>Thanks for the feedback. I've now used `--prefix` and `concurrency` (to cancel previous runs of the same workflow for the same branch).",
    "created_at": "2022-02-01T13:32:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663617",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:9'></a>Thanks for the feedback. I've now used `--prefix` and `concurrency` (to cancel previous runs of the same workflow for the same branch).



---

archive/issue_comments_663618.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [mkoeppe](#comment%3A5):\n> You may want to use #33233\n\n\nThe idea is that this workflow never fails (otherwise the branch is not merged). Thus the base line is \"always green\".",
    "created_at": "2022-02-01T13:33:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663618",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:10'></a>Replying to [mkoeppe](#comment%3A5):
> You may want to use #33233


The idea is that this workflow never fails (otherwise the branch is not merged). Thus the base line is "always green".



---

archive/issue_comments_663619.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [mkoeppe](#comment%3A3):\n> Why not just `configure --prefix=/sage/local`?\n\n\nBut now it seems to rebuild all python packages instead of using them from the docker image as before. Is this a bug in prefix or did I use it wrong?",
    "created_at": "2022-02-01T13:42:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663619",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:11'></a>Replying to [mkoeppe](#comment%3A3):
> Why not just `configure --prefix=/sage/local`?


But now it seems to rebuild all python packages instead of using them from the docker image as before. Is this a bug in prefix or did I use it wrong?



---

archive/issue_comments_663620.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n Add a github action that runs on each push. Since it uses the most recent docker image as the base, the runtime is relatively short with about 2h. The idea is to get quick feedback on PRs without the need to wait until the patchbot picks it up. After this ticket is merged, we can add a badge in the ticket similar to the linter workflow.\n \n-Example run: https://github.com/tobiasdiez/sage/actions/runs/1773704682\n+Example run: https://github.com/sagemath/sagetrac-mirror/actions/workflows/build.yml\n (it works but one doctest and the pytests are failing; I think there are already tickets for these issues)\n``````\n",
    "created_at": "2022-02-01T13:42:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663620",
    "user": "https://github.com/tobiasdiez"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
 Add a github action that runs on each push. Since it uses the most recent docker image as the base, the runtime is relatively short with about 2h. The idea is to get quick feedback on PRs without the need to wait until the patchbot picks it up. After this ticket is merged, we can add a badge in the ticket similar to the linter workflow.
 
-Example run: https://github.com/tobiasdiez/sage/actions/runs/1773704682
+Example run: https://github.com/sagemath/sagetrac-mirror/actions/workflows/build.yml
 (it works but one doctest and the pytests are failing; I think there are already tickets for these issues)
``````




---

archive/issue_comments_663621.json:
```json
{
    "body": "<a id='comment:13'></a>Sorry, it should be `configure --prefix=/sage/local --with-sage-venv`",
    "created_at": "2022-02-01T15:44:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663621",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:13'></a>Sorry, it should be `configure --prefix=/sage/local --with-sage-venv`



---

archive/issue_comments_663622.json:
```json
{
    "body": "<a id='comment:14'></a>Replying to [gh-tobiasdiez](#comment%3A10):\n> Replying to [mkoeppe](#comment%3A5):\n> > You may want to use #33233\n\n> \n> The idea is that this workflow never fails (otherwise the branch is not merged). Thus the base line is \"always green\".\n\n\nThe patchbot makes the same optimistic assumption, which is why it's broken so often...",
    "created_at": "2022-02-01T15:45:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663622",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:14'></a>Replying to [gh-tobiasdiez](#comment%3A10):
> Replying to [mkoeppe](#comment%3A5):
> > You may want to use #33233

> 
> The idea is that this workflow never fails (otherwise the branch is not merged). Thus the base line is "always green".


The patchbot makes the same optimistic assumption, which is why it's broken so often...



---

archive/issue_comments_663623.json:
```json
{
    "body": "<a id='comment:15'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2022-02-01T15:51:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663623",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:15'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_663624.json:
```json
{
    "body": "Changing commit from \"4f5bcb9fe895728a5707a6cdaad7a9c5a49e9dab\" to \"3b0e99f648280c6665e10104bd2794a2bdaabfd8\"",
    "created_at": "2022-02-01T15:51:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663624",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "4f5bcb9fe895728a5707a6cdaad7a9c5a49e9dab" to "3b0e99f648280c6665e10104bd2794a2bdaabfd8"



---

archive/issue_comments_663625.json:
```json
{
    "body": "<a id='comment:16'></a>Replying to [mkoeppe](#comment%3A13):\n> Sorry, it should be `configure --prefix=/sage/local --with-sage-venv`\n\n\nThanks, that worked well!",
    "created_at": "2022-02-01T16:39:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663625",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:16'></a>Replying to [mkoeppe](#comment%3A13):
> Sorry, it should be `configure --prefix=/sage/local --with-sage-venv`


Thanks, that worked well!



---

archive/issue_comments_663626.json:
```json
{
    "body": "<a id='comment:17'></a>Another idea, for a much faster build, would be to just work out of `/sage`, merging the current commit.",
    "created_at": "2022-02-01T20:32:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663626",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:17'></a>Another idea, for a much faster build, would be to just work out of `/sage`, merging the current commit.



---

archive/issue_comments_663627.json:
```json
{
    "body": "<a id='comment:18'></a>I think with a bit more than 1.5h runtime, it is already reasonably fast. The actual build also only takes about 35mins, the rest is spent on the tests. Further optimization in my opinion will be a trade-off between time savings and reduced reliability, because you then have to think about invalidation of the cythonize cache etc. The point of this ticket is to pursue a different approach than the patchbot and really start from a clean state. I would propose to first gain experience with this, before we implement more fancy caching mechanisms (and in this case I would also prefer github actions builtin cache mechanism).",
    "created_at": "2022-02-02T09:30:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663627",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:18'></a>I think with a bit more than 1.5h runtime, it is already reasonably fast. The actual build also only takes about 35mins, the rest is spent on the tests. Further optimization in my opinion will be a trade-off between time savings and reduced reliability, because you then have to think about invalidation of the cythonize cache etc. The point of this ticket is to pursue a different approach than the patchbot and really start from a clean state. I would propose to first gain experience with this, before we implement more fancy caching mechanisms (and in this case I would also prefer github actions builtin cache mechanism).



---

archive/issue_comments_663628.json:
```json
{
    "body": "<a id='comment:19'></a>Sure, sounds good.",
    "created_at": "2022-02-02T18:10:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663628",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:19'></a>Sure, sounds good.



---

archive/issue_comments_663629.json:
```json
{
    "body": "<a id='comment:20'></a>However I think it should only be run for branches on tickets set to `needs_review`. \nAfter making `git-trac` available (see #33222), you can use something like `git trac search --branch u/mkoeppe/upgrade_giac_to_1_7_0`\nand `git trac print 33263 | grep '^Status: needs_review'` for this",
    "created_at": "2022-02-02T18:21:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663629",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:20'></a>However I think it should only be run for branches on tickets set to `needs_review`. 
After making `git-trac` available (see #33222), you can use something like `git trac search --branch u/mkoeppe/upgrade_giac_to_1_7_0`
and `git trac print 33263 | grep '^Status: needs_review'` for this



---

archive/issue_comments_663630.json:
```json
{
    "body": "<a id='comment:21'></a>Hm... no, people set \"needs_review\" after a push, not before",
    "created_at": "2022-02-02T18:22:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663630",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:21'></a>Hm... no, people set "needs_review" after a push, not before



---

archive/issue_comments_663631.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#33103\"",
    "created_at": "2022-02-02T18:33:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663631",
    "user": "https://github.com/mkoeppe"
}
```

Changing dependencies from "" to "#33103"



---

archive/issue_comments_663632.json:
```json
{
    "body": "<a id='comment:22'></a>OK, let's just merge it and try it out. I'd suggest to set #33103 as a dependency and already put `dev` instead of `9.5` as image tag",
    "created_at": "2022-02-02T18:33:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663632",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:22'></a>OK, let's just merge it and try it out. I'd suggest to set #33103 as a dependency and already put `dev` instead of `9.5` as image tag



---

archive/issue_comments_663633.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Matthias Koeppe\"",
    "created_at": "2022-02-02T18:33:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663633",
    "user": "https://github.com/mkoeppe"
}
```

Changing reviewer from "" to "Matthias Koeppe"



---

archive/issue_comments_663634.json:
```json
{
    "body": "<a id='comment:23'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2022-02-03T10:53:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663634",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:23'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_663635.json:
```json
{
    "body": "Changing commit from \"3b0e99f648280c6665e10104bd2794a2bdaabfd8\" to \"63298162ccec5c390018231b036a9dc5dd107e55\"",
    "created_at": "2022-02-03T10:53:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663635",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "3b0e99f648280c6665e10104bd2794a2bdaabfd8" to "63298162ccec5c390018231b036a9dc5dd107e55"



---

archive/issue_comments_663636.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-02-03T10:56:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663636",
    "user": "https://github.com/tobiasdiez"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_663637.json:
```json
{
    "body": "<a id='comment:24'></a>Replying to [mkoeppe](#comment%3A22):\n> OK, let's just merge it and try it out. I'd suggest to set #33103 as a dependency and already put `dev` instead of `9.5` as image tag\n\n\nThanks for the review. It now uses `dev` and I set the ticket to positive review now (please intervene if I misunderstood you).",
    "created_at": "2022-02-03T10:56:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663637",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:24'></a>Replying to [mkoeppe](#comment%3A22):
> OK, let's just merge it and try it out. I'd suggest to set #33103 as a dependency and already put `dev` instead of `9.5` as image tag


Thanks for the review. It now uses `dev` and I set the ticket to positive review now (please intervene if I misunderstood you).



---

archive/issue_comments_663638.json:
```json
{
    "body": "<a id='comment:25'></a>Increasing priority as the badge is already added to the trac ticket (so that I don't have to login in the VM and change the config multiple times).",
    "created_at": "2022-02-14T20:23:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663638",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:25'></a>Increasing priority as the badge is already added to the trac ticket (so that I don't have to login in the VM and change the config multiple times).



---

archive/issue_comments_663639.json:
```json
{
    "body": "Changing priority from major to critical.",
    "created_at": "2022-02-14T20:23:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663639",
    "user": "https://github.com/tobiasdiez"
}
```

Changing priority from major to critical.



---

archive/issue_comments_663640.json:
```json
{
    "body": "Changing branch from \"public/build/github_build\" to \"63298162ccec5c390018231b036a9dc5dd107e55\"",
    "created_at": "2022-02-16T23:57:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663640",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/build/github_build" to "63298162ccec5c390018231b036a9dc5dd107e55"



---

archive/issue_comments_663641.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2022-02-16T23:57:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33263#issuecomment-663641",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_087832.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-02-16T23:57:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/33263",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33263#event-87832"
}
```
