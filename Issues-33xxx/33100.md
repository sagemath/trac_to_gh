...
<a id='comment:1'></a>This also fails for larger matrices, such as these linear combinations of positive definite matrices (for many seeds):
...
<a id='comment:2'></a>My first thought was that we should be able to delete `cholesky()` and `is_positive_definite()` in the subclass to obtain something possibly slower, but that actually works. There are two small problems though.
...
<a id='comment:4'></a>I agree with using the "naive" algorithm here and do not see why it would be more naive/less appropriate than the "orthonormal" one. For the orthonormal algorithm, no tolerance would be large enough to work for all such matrices that are Hermitian in an exact sense, when considered entrywise. Scipy 1.8.0 will also have a function [scipy.linalg.ishermitian](https://scipy.github.io/devdocs/reference/generated/scipy.linalg.ishermitian.html) which works like the naive algorithm, i.e. entrywise.
...
<a id='comment:5'></a>Replying to [gh-mwageringel](#comment%3A4):
> I agree with using the "naive" algorithm here and do not see why it would be more naive/less appropriate than the "orthonormal" one. For the orthonormal algorithm, no tolerance would be large enough to work for all such matrices that are Hermitian in an exact sense, when considered entrywise. Scipy 1.8.0 will also have a function [scipy.linalg.ishermitian](https://scipy.github.io/devdocs/reference/generated/scipy.linalg.ishermitian.html) which works like the naive algorithm, i.e. entrywise.
> 
> Replying to [mjo](#comment%3A2):
> > My first thought was that we should be able to delete `cholesky()` and `is_positive_definite()` in the subclass to obtain something possibly slower, but that actually works.
...
```json
{
    "body": "<a id='comment:8'></a>I left `is_positive_definite()` alone because `cholesky()` and `is_positive_definite()` cache one another in the subclass. Using the superclass for `is_positive_definite()` would cache a `block_ldlt()` factorization, but then it would not be used for `cholesky()`.",
...
<a id='comment:8'></a>I left `is_positive_definite()` alone because `cholesky()` and `is_positive_definite()` cache one another in the subclass. Using the superclass for `is_positive_definite()` would cache a `block_ldlt()` factorization, but then it would not be used for `cholesky()`.
...
```json
{
    "body": "<a id='comment:9'></a>Ok, this looks good to me then. Thank you for implementing this.",
    "created_at": "2022-01-04T11:35:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33100",
    "type": "issue_comment",
...
<a id='comment:9'></a>Ok, this looks good to me then. Thank you for implementing this.
...
```json
{
    "body": "<a id='comment:12'></a>(brought to you by https://github.com/sagemath/git-trac-command/pull/57)",
...
<a id='comment:12'></a>(brought to you by https://github.com/sagemath/git-trac-command/pull/57)
...
```json
{
    "body": "<a id='comment:14'></a>Thanks for the input! Yes, this can certainly be refined.",
...
<a id='comment:14'></a>Thanks for the input! Yes, this can certainly be refined.
...
```json
{
    "body": "<a id='comment:15'></a>Set milestone to sage-9.6 after Sage 9.5 release.",
    "created_at": "2022-01-30T15:44:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33100",
    "type": "issue_comment",
...
<a id='comment:15'></a>Set milestone to sage-9.6 after Sage 9.5 release.
...
<a id='comment:18'></a>Replying to [gh-mwageringel](#comment%3A17):
> I am not completely sure about theses changes. Square roots do not always exist in the same field, such as cyclotomic fields for which `sqrt` returns a result in `SR`. Granted, this probably did not work before either, as comparisons are broken in number fields (#20028).
...
```json
{
    "body": "<a id='comment:20'></a>Let's see what patchbot says about this one...",
...
<a id='comment:20'></a>Let's see what patchbot says about this one...
...
```json
{
    "body": "<a id='comment:21'></a>There is a merge failure, but otherwise this looks good to me, as long as the tests pass.",
    "created_at": "2022-02-14T21:02:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33100",
    "type": "issue_comment",
...
<a id='comment:21'></a>There is a merge failure, but otherwise this looks good to me, as long as the tests pass.
...
```json
{
    "body": "<a id='comment:23'></a>It rebased cleanly, I think I maybe it wanted me to drop the commits from #33107.",
...
<a id='comment:23'></a>It rebased cleanly, I think I maybe it wanted me to drop the commits from #33107.
