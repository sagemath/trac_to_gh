...
```json
{
    "body": "<a id='comment:2'></a>For our part, we already warn when the sysconfig flags contain `-I` or `-L`. We could do the same for `-Werror`.",
...
<a id='comment:2'></a>For our part, we already warn when the sysconfig flags contain `-I` or `-L`. We could do the same for `-Werror`.
...
```json
{
    "body": "<a id='comment:3'></a>Good idea.",
...
```json
{
    "body": "<a id='comment:4'></a>In contrast to the trouble with `-I` and `-L` (for which the sysconfig-provided flags are coming first and therefore cannot be overridden), in this case we can work around it by adding the `-Wno-error` to our flags for the affected extension modules...",
...
<a id='comment:4'></a>In contrast to the trouble with `-I` and `-L` (for which the sysconfig-provided flags are coming first and therefore cannot be overridden), in this case we can work around it by adding the `-Wno-error` to our flags for the affected extension modules...
...
```json
{
    "body": "<a id='comment:5'></a>in `src/sage/libs/linbox/fflas.pxd`",
...
<a id='comment:5'></a>in `src/sage/libs/linbox/fflas.pxd`
...
```json
{
    "body": "<a id='comment:6'></a>Ok, that's a better idea, I'm trying it.",
...
<a id='comment:6'></a>Ok, that's a better idea, I'm trying it.
...
```json
{
    "body": "Changing branch from \"\" to \"u/mkoeppe/opensuse_tumbleweed__sagelib_build_fails\"",
...
Changing branch from "" to "u/mkoeppe/opensuse_tumbleweed__sagelib_build_fails"
...
```json
{
    "body": "<a id='comment:8'></a>Need to check though if this flag works with ancient compilers that we still support\n\n---\nNew commits:",
...
<a id='comment:8'></a>Need to check though if this flag works with ancient compilers that we still support
...
```json
{
    "body": "Changing commit from \"\" to \"3fdfae973e4901a3ceb5aa2c5c341d02371226de\"",
    "created_at": "2022-01-12T20:27:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
...
Changing commit from "" to "3fdfae973e4901a3ceb5aa2c5c341d02371226de"
...
```json
{
    "body": "<a id='comment:9'></a>The upstream bug is already fixed in fflas-ffpack-2.4.3. Repology says that tumbleweed is at 2.5.0. I wonder what version the reporter is using? Maybe we just bump the minimum version in `spkg-configure.m4` up from 2.4.0 to 2.4.3. Again according to repology that shouldn't hurt anyone.",
...
<a id='comment:9'></a>The upstream bug is already fixed in fflas-ffpack-2.4.3. Repology says that tumbleweed is at 2.5.0. I wonder what version the reporter is using? Maybe we just bump the minimum version in `spkg-configure.m4` up from 2.4.0 to 2.4.3. Again according to repology that shouldn't hurt anyone.
...
<a id='comment:11'></a>Anyway, I've tried to clobber my sysconfig but the build still succeeded. My system header *is* missing the return value. So I wonder why it worked:
...
```json
{
    "body": "<a id='comment:12'></a>Perhaps a sufficiently smart compiler has optimized the issue away?",
...
<a id='comment:12'></a>Perhaps a sufficiently smart compiler has optimized the issue away?
...
```json
{
    "body": "<a id='comment:15'></a>I haven't figured out how to get cython to show me WTF it's actually doing yet, but watching `ps auxwww`, I can see that the sysconfig flags aren't being used while it's compiling pyx files.",
...
<a id='comment:15'></a>I haven't figured out how to get cython to show me WTF it's actually doing yet, but watching `ps auxwww`, I can see that the sysconfig flags aren't being used while it's compiling pyx files.
...
```json
{
    "body": "<a id='comment:16'></a>Is that using standard Sage Python packages, or using some system python packages?",
...
<a id='comment:16'></a>Is that using standard Sage Python packages, or using some system python packages?
...
```json
{
    "body": "<a id='comment:18'></a>It turns out that the python on Gentoo is already patched to avoid this very issue. Ugh.",
...
<a id='comment:18'></a>It turns out that the python on Gentoo is already patched to avoid this very issue. Ugh.
...
<a id='comment:19'></a>Rebuilt with the python3 SPKG, and now my sysconfig changes are visible. But somehow I'm able to compile the file that was causing the OP's problem, e.g.:
...
```json
{
    "body": "<a id='comment:20'></a>I would think it is sufficient to reproduce the error on the reported platform and confirm that the patch fixes it. Little point on trying to reproduce on a different platform.",
...
<a id='comment:20'></a>I would think it is sufficient to reproduce the error on the reported platform and confirm that the patch fixes it. Little point on trying to reproduce on a different platform.
...
```json
{
    ...
    "created_at": "2022-01-14T16:03:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
...
```json
{
    "body": "Changing commit from \"3fdfae973e4901a3ceb5aa2c5c341d02371226de\" to \"fa2e672ff402327cd97884a73412fc5ab68bd357\"",
    "created_at": "2022-01-14T17:56:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661451",
    "user": "https://github.com/orlitzky"
}
```

Changing commit from "3fdfae973e4901a3ceb5aa2c5c341d02371226de" to "fa2e672ff402327cd97884a73412fc5ab68bd357"
...
```json
{
    "body": "<a id='comment:22'></a>I finally got it to fail using the fflas_ffpack SPKG. Maybe it has something to do with the autotune, who knows. Anyway, the patch works. I added some comments at the top of the file. Feel free to positive review after checking it.",
    "created_at": "2022-01-14T17:56:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661452",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:22'></a>I finally got it to fail using the fflas_ffpack SPKG. Maybe it has something to do with the autotune, who knows. Anyway, the patch works. I added some comments at the top of the file. Feel free to positive review after checking it.
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661453",
    "user": "https://github.com/orlitzky"
}
```
...
```json
{
    "body": "Changing branch from \"u/mkoeppe/opensuse_tumbleweed__sagelib_build_fails\" to \"u/mjo/ticket/33153\"",
    "created_at": "2022-01-14T17:56:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661454",
    "user": "https://github.com/orlitzky"
}
```

Changing branch from "u/mkoeppe/opensuse_tumbleweed__sagelib_build_fails" to "u/mjo/ticket/33153"
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661455",
    "user": "https://github.com/mkoeppe"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661456",
    "user": "https://github.com/mkoeppe"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661457",
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33153#event-87594"
}
```
...
```json
{
    "body": "Changing branch from \"u/mjo/ticket/33153\" to \"fa2e672ff402327cd97884a73412fc5ab68bd357\"",
    "created_at": "2022-01-15T18:58:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661458",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/mjo/ticket/33153" to "fa2e672ff402327cd97884a73412fc5ab68bd357"
...
```json
{
    "body": "<a id='comment:25'></a>`opensuse` has marked https://bugzilla.suse.com/show_bug.cgi?id=1194605 wontfix.",
    "created_at": "2022-03-08T02:14:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661459",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:25'></a>`opensuse` has marked https://bugzilla.suse.com/show_bug.cgi?id=1194605 wontfix.
...
```json
{
    "body": "Changing commit from \"fa2e672ff402327cd97884a73412fc5ab68bd357\" to \"\"",
    "created_at": "2022-03-08T02:14:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33153#issuecomment-661460",
    "user": "https://github.com/mkoeppe"
}
```

Changing commit from "fa2e672ff402327cd97884a73412fc5ab68bd357" to ""
