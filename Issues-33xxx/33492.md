# Issue 33492: make sure "real" gcc is rejected on macOS

archive/issues_033255.json:
```json
{
    "assignees": [],
    "body": "This is prompted by a report on [sage-develop](https://groups.google.com/d/msgid/sage-devel/c007e475-9191-4803-b366-4f8b6b1938fbn%40googlegroups.com?utm_medium=email&utm_source=footer)\n\nIf gcc is actually real gcc on macOS, say installed in `/usr/local/bin`,\nthe build errors out on various C++ modules.\n\nCC:  @mkoeppe @kiwifb\n\nKeywords: **macOS, gcc**\n\nBranch/Commit: **[467b2f1](https://github.com/sagemath/sagetrac-mirror/commit/467b2f14cb16f7e075301110b3291a710d6d94b8)**\n\nReviewer: **Matthias Koeppe, Guillermo Moreno-Soc\u00edas**\n\nAuthor: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33492_\n\n",
    "closed_at": "2022-03-27T15:44:01Z",
    "created_at": "2022-03-12T11:33:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make sure \"real\" gcc is rejected on macOS",
    "type": "issue",
    "updated_at": "2022-03-27T15:44:01Z",
    "url": "https://github.com/sagemath/sage/issues/33492",
    "user": "https://github.com/dimpase"
}
```
This is prompted by a report on [sage-develop](https://groups.google.com/d/msgid/sage-devel/c007e475-9191-4803-b366-4f8b6b1938fbn%40googlegroups.com?utm_medium=email&utm_source=footer)

If gcc is actually real gcc on macOS, say installed in `/usr/local/bin`,
the build errors out on various C++ modules.

CC:  @mkoeppe @kiwifb

Keywords: **macOS, gcc**

Branch/Commit: **[467b2f1](https://github.com/sagemath/sagetrac-mirror/commit/467b2f14cb16f7e075301110b3291a710d6d94b8)**

Reviewer: **Matthias Koeppe, Guillermo Moreno-Socías**

Author: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/33492_





---

archive/issue_events_441745.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-03-12T11:33:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33492#event-441745"
}
```



---

archive/issue_events_441746.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-03-12T11:33:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
    "label_color": "000080",
    "label_name": "component: build: configure",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33492#event-441746"
}
```



---

archive/issue_events_441747.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-03-12T11:33:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33492#event-441747"
}
```



---

archive/issue_events_441748.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-03-12T11:33:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33492#event-441748"
}
```



---

archive/issue_comments_544015.json:
```json
{
    "body": "Commit: **[467b2f1](https://github.com/sagemath/sagetrac-mirror/commit/467b2f14cb16f7e075301110b3291a710d6d94b8)**",
    "created_at": "2022-03-12T13:13:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544015",
    "user": "https://github.com/dimpase"
}
```

Commit: **[467b2f1](https://github.com/sagemath/sagetrac-mirror/commit/467b2f14cb16f7e075301110b3291a710d6d94b8)**



---

archive/issue_comments_544016.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/467b2f14cb16f7e075301110b3291a710d6d94b8\">467b2f1</a></td><td><code>reject \"real\" gcc on macOS</code></td></tr></table>\n",
    "created_at": "2022-03-12T13:13:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544016",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/467b2f14cb16f7e075301110b3291a710d6d94b8">467b2f1</a></td><td><code>reject "real" gcc on macOS</code></td></tr></table>




---

archive/issue_comments_544017.json:
```json
{
    "body": "Branch: **[u/dimpase/configure/no_real_gcc_on_macos](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/configure/no_real_gcc_on_macos)**",
    "created_at": "2022-03-12T13:13:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544017",
    "user": "https://github.com/dimpase"
}
```

Branch: **[u/dimpase/configure/no_real_gcc_on_macos](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/configure/no_real_gcc_on_macos)**



---

archive/issue_comments_544018.json:
```json
{
    "body": "Author: **Dima Pasechnik**",
    "created_at": "2022-03-12T13:13:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544018",
    "user": "https://github.com/dimpase"
}
```

Author: **Dima Pasechnik**



---

archive/issue_events_441749.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-03-12T13:13:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33492#event-441749"
}
```



---

archive/issue_comments_544019.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n This is prompted by a report on [sage-develop](https://groups.google.com/d/msgid/sage-devel/c007e475-9191-4803-b366-4f8b6b1938fbn%40googlegroups.com?utm_medium=email&utm_source=footer)\n \n-If gcc is actually real gcc on macOS, say installed in /usr/local/bin\n+If gcc is actually real gcc on macOS, say installed in `/usr/local/bin`,\n the build errors out on various C++ modules.\n``````\n",
    "created_at": "2022-03-12T13:13:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544019",
    "user": "https://github.com/dimpase"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
 This is prompted by a report on [sage-develop](https://groups.google.com/d/msgid/sage-devel/c007e475-9191-4803-b366-4f8b6b1938fbn%40googlegroups.com?utm_medium=email&utm_source=footer)
 
-If gcc is actually real gcc on macOS, say installed in /usr/local/bin
+If gcc is actually real gcc on macOS, say installed in `/usr/local/bin`,
 the build errors out on various C++ modules.
``````




---

archive/issue_comments_544020.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\ntests are run on [#33512 comment:3](https://github.com/sagemath/sage/issues/33512#comment:3)",
    "created_at": "2022-03-18T09:19:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544020",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

tests are run on [#33512 comment:3](https://github.com/sagemath/sage/issues/33512#comment:3)



---

archive/issue_comments_544021.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nStill running but looking fine.",
    "created_at": "2022-03-18T17:26:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544021",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Still running but looking fine.



---

archive/issue_events_441750.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-18T17:26:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33492#event-441750"
}
```



---

archive/issue_events_441751.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-18T17:26:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33492#event-441751"
}
```



---

archive/issue_comments_544022.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2022-03-18T17:37:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544022",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_544023.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nFWIW, Positive review on Apple Silicon M1 Macs with Homebrew up to date, both macOS 11.6.5 (Big Sur) with Xcode 13.2.1 and macOS 12.3 (Monterey) with Xcode 13.3.",
    "created_at": "2022-03-20T19:25:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544023",
    "user": "https://github.com/GMS103"
}
```

<div id="comment:6" align="right">Comment 6</div>

FWIW, Positive review on Apple Silicon M1 Macs with Homebrew up to date, both macOS 11.6.5 (Big Sur) with Xcode 13.2.1 and macOS 12.3 (Monterey) with Xcode 13.3.



---

archive/issue_comments_544024.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks for testing this!",
    "created_at": "2022-03-20T19:39:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544024",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks for testing this!



---

archive/issue_comments_544025.json:
```json
{
    "body": "Changed reviewer from **Matthias Koeppe** to **Matthias Koeppe, Guillermo Moreno-Soc\u00edas**",
    "created_at": "2022-03-20T19:39:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544025",
    "user": "https://github.com/mkoeppe"
}
```

Changed reviewer from **Matthias Koeppe** to **Matthias Koeppe, Guillermo Moreno-Socías**



---

archive/issue_events_441752.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-03-27T15:44:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33492#event-441752"
}
```



---

archive/issue_events_441753.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cedd9bbb57990fb3bdb1b0f2a34cc78de86e83ab",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-03-27T15:44:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33492#event-441753"
}
```



---

archive/issue_comments_544026.json:
```json
{
    "body": "Changed branch from **[u/dimpase/configure/no_real_gcc_on_macos](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/configure/no_real_gcc_on_macos)** to **[467b2f1](https://github.com/sagemath/sagetrac-mirror/commit/467b2f14cb16f7e075301110b3291a710d6d94b8)**",
    "created_at": "2022-03-27T15:44:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33492#issuecomment-544026",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dimpase/configure/no_real_gcc_on_macos](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/configure/no_real_gcc_on_macos)** to **[467b2f1](https://github.com/sagemath/sagetrac-mirror/commit/467b2f14cb16f7e075301110b3291a710d6d94b8)**
