...
```json
{
    "body": "<a id='comment:3'></a>I fixed the doctest in `src/sage/symbolic/expression.pyx` (by adding `forget()`), so a doctest fails now, because of the problem in the ticket description. (Sorry, I forgot to do \"git commit\" before my first push, but I think the branch is ok now.)\n\n---\nNew commits:",
    "created_at": "2022-04-12T19:57:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33698",
    "type": "issue_comment",
...
<a id='comment:3'></a>I fixed the doctest in `src/sage/symbolic/expression.pyx` (by adding `forget()`), so a doctest fails now, because of the problem in the ticket description. (Sorry, I forgot to do "git commit" before my first push, but I think the branch is ok now.)
