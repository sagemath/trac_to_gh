# Issue 33063: notebook: update to 6.4.10 to fix deprecation warning

archive/issues_032826.json:
```json
{
    "assignees": [],
    "body": "While doctesting `src/sage/repl/ipython_kernel/install.py`:\n    \n\n```\nDeprecationWarning: invalid escape sequence '\\s'\n```\n    \nThe solution is to make the regex a raw string.\n\nUpstream: https://github.com/jupyter/notebook/pull/6253\n\nWe update to the latest in the 6.4.x series, 6.4.10, which contains the fix.\n\n\nCC:  @dimpase @egourgoulhon\n\nBranch/Commit: **[67537dd](https://github.com/sagemath/sagetrac-mirror/commit/67537dd869de01173ace0b008b21c1630b9afb75)**\n\nUpstream: **Fixed upstream, in a later stable release.**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33063_\n\n",
    "closed_at": "2022-04-10T13:13:03Z",
    "created_at": "2021-12-21T19:08:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook: update to 6.4.10 to fix deprecation warning",
    "type": "issue",
    "updated_at": "2022-04-10T13:13:03Z",
    "url": "https://github.com/sagemath/sage/issues/33063",
    "user": "https://github.com/tornaria"
}
```
While doctesting `src/sage/repl/ipython_kernel/install.py`:
    

```
DeprecationWarning: invalid escape sequence '\s'
```
    
The solution is to make the regex a raw string.

Upstream: https://github.com/jupyter/notebook/pull/6253

We update to the latest in the 6.4.x series, 6.4.10, which contains the fix.


CC:  @dimpase @egourgoulhon

Branch/Commit: **[67537dd](https://github.com/sagemath/sagetrac-mirror/commit/67537dd869de01173ace0b008b21c1630b9afb75)**

Upstream: **Fixed upstream, in a later stable release.**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/33063_





---

archive/issue_events_436640.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2021-12-21T19:08:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436640"
}
```



---

archive/issue_events_436641.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2021-12-21T19:08:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436641"
}
```



---

archive/issue_events_436642.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2021-12-21T19:08:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436642"
}
```



---

archive/issue_events_436643.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2021-12-21T19:08:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436643"
}
```



---

archive/issue_events_436644.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2021-12-21T19:09:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436644"
}
```



---

archive/issue_events_436645.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-28T23:09:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436645"
}
```



---

archive/issue_events_436646.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-28T23:09:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436646"
}
```



---

archive/issue_comments_536888.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nOn the one hand, this looks fine to me. But I also see that this has now been fixed upstream in 6.4.7 (https://github.com/jupyter/notebook/releases/tag/v6.4.7) and it would make sense to update from 6.4.6 to 6.4.7 or 6.4.8.",
    "created_at": "2022-03-29T14:56:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536888",
    "user": "https://github.com/davidlowryduda"
}
```

<div id="comment:3" align="right">Comment 3</div>

On the one hand, this looks fine to me. But I also see that this has now been fixed upstream in 6.4.7 (https://github.com/jupyter/notebook/releases/tag/v6.4.7) and it would make sense to update from 6.4.6 to 6.4.7 or 6.4.8.



---

archive/issue_events_436647.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-29T18:11:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436647"
}
```



---

archive/issue_events_436648.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-29T18:11:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436648"
}
```



---

archive/issue_comments_536889.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI agree, updating to the latest in the 6.4.x series is better",
    "created_at": "2022-03-29T18:11:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536889",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

I agree, updating to the latest in the 6.4.x series is better



---

archive/issue_comments_536890.json:
```json
{
    "body": "Changed branch from **[u/tornaria/notebook-deprecation-patch](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/notebook-deprecation-patch)** to **[u/mkoeppe/notebook-deprecation-patch](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/notebook-deprecation-patch)**",
    "created_at": "2022-04-01T16:55:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536890",
    "user": "https://github.com/mkoeppe"
}
```

Changed branch from **[u/tornaria/notebook-deprecation-patch](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/notebook-deprecation-patch)** to **[u/mkoeppe/notebook-deprecation-patch](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/notebook-deprecation-patch)**



---

archive/issue_events_436649.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T16:57:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436649"
}
```



---

archive/issue_events_436650.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T16:57:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436650"
}
```



---

archive/issue_comments_536891.json:
```json
{
    "body": "Changed author from **Gonzalo Tornar\u00eda** to **Matthias Koeppe**",
    "created_at": "2022-04-01T16:57:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536891",
    "user": "https://github.com/mkoeppe"
}
```

Changed author from **Gonzalo Tornaría** to **Matthias Koeppe**



---

archive/issue_comments_536892.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,3 +8,6 @@\n The solution is to make the regex a raw string.\n \n Upstream: https://github.com/jupyter/notebook/pull/6253\n+\n+We update to the latest in the 6.4.x series, 6.4.10, which contains the fix.\n+\n``````\n",
    "created_at": "2022-04-01T16:57:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536892",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,3 +8,6 @@
 The solution is to make the regex a raw string.
 
 Upstream: https://github.com/jupyter/notebook/pull/6253
+
+We update to the latest in the 6.4.x series, 6.4.10, which contains the fix.
+
``````




---

archive/issue_comments_536893.json:
```json
{
    "body": "Changed commit from **[966926e](https://github.com/sagemath/sagetrac-mirror/commit/966926e3db5ea06b962fac386fa3c7223f9a14fb)** to **[67537dd](https://github.com/sagemath/sagetrac-mirror/commit/67537dd869de01173ace0b008b21c1630b9afb75)**",
    "created_at": "2022-04-01T16:57:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536893",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[966926e](https://github.com/sagemath/sagetrac-mirror/commit/966926e3db5ea06b962fac386fa3c7223f9a14fb)** to **[67537dd](https://github.com/sagemath/sagetrac-mirror/commit/67537dd869de01173ace0b008b21c1630b9afb75)**



---

archive/issue_comments_536894.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/67537dd869de01173ace0b008b21c1630b9afb75\">67537dd</a></td><td><code>build/pkgs/notebook: Update to 6.4.10</code></td></tr></table>\n",
    "created_at": "2022-04-01T16:57:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536894",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/67537dd869de01173ace0b008b21c1630b9afb75">67537dd</a></td><td><code>build/pkgs/notebook: Update to 6.4.10</code></td></tr></table>




---

archive/issue_events_436651.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T16:57:07Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "title_is": "notebook: update to 6.4.10 to fix deprecation warning",
    "title_was": "notebook: fix deprecation warning",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436651"
}
```



---

archive/issue_comments_536895.json:
```json
{
    "body": "Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**",
    "created_at": "2022-04-08T05:56:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536895",
    "user": "https://github.com/mkoeppe"
}
```

Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**



---

archive/issue_comments_536896.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2022-04-08T08:57:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536896",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_536897.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nlgtm",
    "created_at": "2022-04-08T08:57:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536897",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:8" align="right">Comment 8</div>

lgtm



---

archive/issue_events_436652.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-04-08T08:57:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436652"
}
```



---

archive/issue_events_436653.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-04-08T08:57:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436653"
}
```



---

archive/issue_comments_536898.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/notebook-deprecation-patch](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/notebook-deprecation-patch)** to **[67537dd](https://github.com/sagemath/sagetrac-mirror/commit/67537dd869de01173ace0b008b21c1630b9afb75)**",
    "created_at": "2022-04-10T13:13:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33063#issuecomment-536898",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/notebook-deprecation-patch](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/notebook-deprecation-patch)** to **[67537dd](https://github.com/sagemath/sagetrac-mirror/commit/67537dd869de01173ace0b008b21c1630b9afb75)**



---

archive/issue_events_436654.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-04-10T13:13:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436654"
}
```



---

archive/issue_events_436655.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6df7b36536c036fd06dfa695b9e2ef8857009eb4",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-04-10T13:13:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33063",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33063#event-436655"
}
```
