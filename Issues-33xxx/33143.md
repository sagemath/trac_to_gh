# Issue 33143: Matplotlib update breaks build on debian buster

archive/issues_032906.json:
```json
{
    "body": "#31580 or #33040 broke sage build on debian buster:\n\n```\n[matplotlib-3.5.1]   gcc -DNDEBUG -g -fwrapv -O2 -Wall -march=native -O3 -g -Wdate-time -D_FORTIFY_SOURCE=2 -fPIC -DMPL_DEVNULL=/dev/null -DPY_ARRAY_UNIQUE_SYMBOL=MPL_matplotlib__$\nhull_ARRAY_API -DNPY_NO_DEPRECATED_API=NPY_1_7_API_VERSION -D__STDC_FORMAT_MACROS=1 -I/srv/public/kliem/sage/local/var/lib/sage/venv-python3.7/lib/python3.7/site-packages/numpy/co$\ne/include -I/srv/public/kliem/sage/local/var/lib/sage/venv-python3.7/include -I/usr/include/python3.7m -c src/qhull_wrap.cpp -o build/temp.linux-x86_64-3.7/matplotlib._qhull/src/q$\null_wrap.o                                                                                                                                                                          \n[matplotlib-3.5.1]   src/qhull_wrap.cpp:19:10: fatal error: libqhull_r/qhull_ra.h: No such file or directory                                                                        \n[matplotlib-3.5.1]    #include \"libqhull_r/qhull_ra.h\"  \n```\n\n**CC:**  @dimpase @kiwifb @mkoeppe @egourgoulhon\n\n**Branch/Commit:** [9de0e06395253164bb04b22d2daf5aebf8df2000](https://github.com/sagemath/sagetrac-mirror/commit/9de0e06395253164bb04b22d2daf5aebf8df2000)\n\n**Reviewer:** Jonathan Kliem\n\n**Author:** Matthias Koeppe\n\nIssue created by migration from https://trac.sagemath.org/ticket/33143\n\n",
    "closed_at": "2022-01-12T17:17:49Z",
    "created_at": "2022-01-10T16:36:52Z",
    "labels": [
        "component: build",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "title": "Matplotlib update breaks build on debian buster",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/33143",
    "user": "https://github.com/kliem"
}
```
#31580 or #33040 broke sage build on debian buster:

```
[matplotlib-3.5.1]   gcc -DNDEBUG -g -fwrapv -O2 -Wall -march=native -O3 -g -Wdate-time -D_FORTIFY_SOURCE=2 -fPIC -DMPL_DEVNULL=/dev/null -DPY_ARRAY_UNIQUE_SYMBOL=MPL_matplotlib__$
hull_ARRAY_API -DNPY_NO_DEPRECATED_API=NPY_1_7_API_VERSION -D__STDC_FORMAT_MACROS=1 -I/srv/public/kliem/sage/local/var/lib/sage/venv-python3.7/lib/python3.7/site-packages/numpy/co$
e/include -I/srv/public/kliem/sage/local/var/lib/sage/venv-python3.7/include -I/usr/include/python3.7m -c src/qhull_wrap.cpp -o build/temp.linux-x86_64-3.7/matplotlib._qhull/src/q$
ull_wrap.o                                                                                                                                                                          
[matplotlib-3.5.1]   src/qhull_wrap.cpp:19:10: fatal error: libqhull_r/qhull_ra.h: No such file or directory                                                                        
[matplotlib-3.5.1]    #include "libqhull_r/qhull_ra.h"  
```

**CC:**  @dimpase @kiwifb @mkoeppe @egourgoulhon

**Branch/Commit:** [9de0e06395253164bb04b22d2daf5aebf8df2000](https://github.com/sagemath/sagetrac-mirror/commit/9de0e06395253164bb04b22d2daf5aebf8df2000)

**Reviewer:** Jonathan Kliem

**Author:** Matthias Koeppe

Issue created by migration from https://trac.sagemath.org/ticket/33143





---

archive/issue_comments_538302.json:
```json
{
    "body": "<a id='comment:2'></a>\nconfig.log please",
    "created_at": "2022-01-10T18:16:16Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538302",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'></a>
config.log please



---

archive/issue_comments_538303.json:
```json
{
    "body": "<a id='comment:3'></a>\nhttps://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/aZCzjiXMAQAJ",
    "created_at": "2022-01-10T18:24:14Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538303",
    "user": "https://github.com/kliem"
}
```

<a id='comment:3'></a>
https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/aZCzjiXMAQAJ



---

archive/issue_comments_538304.json:
```json
{
    "body": "<a id='comment:4'></a>\nIs `qhull` installed properly? The only thing I can think of otherwise is that the compilation line includes path to the venv but not to `$SAGE_LOCAL/include` where the library should be found.",
    "created_at": "2022-01-10T18:24:37Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538304",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:4'></a>
Is `qhull` installed properly? The only thing I can think of otherwise is that the compilation line includes path to the venv but not to `$SAGE_LOCAL/include` where the library should be found.



---

archive/issue_comments_538305.json:
```json
{
    "body": "<a id='comment:5'></a>\nReplying to [gh-kliem](#comment%3A3):\n> https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/aZCzjiXMAQAJ\n\nFrom there:\n\n```\nconfigure:38313: checking for qhull\nconfigure:38331: found /usr/bin/qhull\nconfigure:38343: result: /usr/bin/qhull\nconfigure:38359: checking is qhull's version good enough?\nconfigure:38482: will use system package and not install SPKG qhull\n```\n\nLooks like `build/pkgs/qhull/spkg-configure.m4` is missing the \"else\" branch in AX_COMPARE_VERSION",
    "created_at": "2022-01-10T18:31:47Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538305",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>
Replying to [gh-kliem](#comment%3A3):
> https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/aZCzjiXMAQAJ

From there:

```
configure:38313: checking for qhull
configure:38331: found /usr/bin/qhull
configure:38343: result: /usr/bin/qhull
configure:38359: checking is qhull's version good enough?
configure:38482: will use system package and not install SPKG qhull
```

Looks like `build/pkgs/qhull/spkg-configure.m4` is missing the "else" branch in AX_COMPARE_VERSION



---

archive/issue_comments_538306.json:
```json
{
    "body": "**Author:** Matthias Koeppe",
    "created_at": "2022-01-10T18:32:15Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538306",
    "user": "https://github.com/mkoeppe"
}
```

**Author:** Matthias Koeppe



---

archive/issue_comments_538307.json:
```json
{
    "body": "**Branch:** [u/mkoeppe/matplotlib_update_breaks_build_on_debian_buster](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/matplotlib_update_breaks_build_on_debian_buster)",
    "created_at": "2022-01-10T18:38:23Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538307",
    "user": "https://github.com/mkoeppe"
}
```

**Branch:** [u/mkoeppe/matplotlib_update_breaks_build_on_debian_buster](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/matplotlib_update_breaks_build_on_debian_buster)



---

archive/issue_events_297241.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-10T18:38:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33143#event-297241"
}
```



---

archive/issue_comments_538308.json:
```json
{
    "body": "<a id='comment:8'></a>\nTry this please\n\n---\n**New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9de0e06395253164bb04b22d2daf5aebf8df2000\">9de0e06</a></td><td><code>build/pkgs/qhull/spkg-configure.m4: Fix test for library</code></td></tr></table>\n",
    "created_at": "2022-01-10T18:38:36Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538308",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:8'></a>
Try this please

---
**New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9de0e06395253164bb04b22d2daf5aebf8df2000">9de0e06</a></td><td><code>build/pkgs/qhull/spkg-configure.m4: Fix test for library</code></td></tr></table>




---

archive/issue_comments_538309.json:
```json
{
    "body": "**Commit:** [9de0e06395253164bb04b22d2daf5aebf8df2000](https://github.com/sagemath/sagetrac-mirror/commit/9de0e06395253164bb04b22d2daf5aebf8df2000)",
    "created_at": "2022-01-10T18:38:36Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538309",
    "user": "https://github.com/mkoeppe"
}
```

**Commit:** [9de0e06395253164bb04b22d2daf5aebf8df2000](https://github.com/sagemath/sagetrac-mirror/commit/9de0e06395253164bb04b22d2daf5aebf8df2000)



---

archive/issue_comments_538310.json:
```json
{
    "body": "<a id='comment:9'></a>\nThanks for the quick fix. This seems to be the problem.",
    "created_at": "2022-01-10T18:44:38Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538310",
    "user": "https://github.com/kliem"
}
```

<a id='comment:9'></a>
Thanks for the quick fix. This seems to be the problem.



---

archive/issue_comments_538311.json:
```json
{
    "body": "**Reviewer:** Jonathan Kliem",
    "created_at": "2022-01-10T18:44:38Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538311",
    "user": "https://github.com/kliem"
}
```

**Reviewer:** Jonathan Kliem



---

archive/issue_events_297242.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2022-01-10T18:44:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33143#event-297242"
}
```



---

archive/issue_events_297243.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2022-01-10T18:44:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33143#event-297243"
}
```



---

archive/issue_comments_538312.json:
```json
{
    "body": "**Changing branch** from \"[u/mkoeppe/matplotlib_update_breaks_build_on_debian_buster](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/matplotlib_update_breaks_build_on_debian_buster)\" to \"[9de0e06395253164bb04b22d2daf5aebf8df2000](https://github.com/sagemath/sagetrac-mirror/commit/9de0e06395253164bb04b22d2daf5aebf8df2000)\".",
    "created_at": "2022-01-12T17:17:49Z",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33143#issuecomment-538312",
    "user": "https://github.com/vbraun"
}
```

**Changing branch** from "[u/mkoeppe/matplotlib_update_breaks_build_on_debian_buster](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/matplotlib_update_breaks_build_on_debian_buster)" to "[9de0e06395253164bb04b22d2daf5aebf8df2000](https://github.com/sagemath/sagetrac-mirror/commit/9de0e06395253164bb04b22d2daf5aebf8df2000)".



---

archive/issue_events_297244.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-01-12T17:17:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33143#event-297244"
}
```



---

archive/issue_events_297245.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cee501534dfedf966b04cfe35b3cd55300a36db6",
    "created_at": "2022-01-12T17:17:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33143",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33143#event-297245"
}
```
