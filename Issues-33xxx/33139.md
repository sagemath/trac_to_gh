# Issue 33139: Fix sagemath_doc_html build failure on Cygwin

archive/issues_032902.json:
```json
{
    "body": "On sage-release, report on a failure in Cygwin\n(from a fresh git clone):\n\n- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/oLwrXx6yAQAJ\n\nAlso observed on Linux, where `make distclean` fixed it:\n\n- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ\n- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ\n\nLet us deal with the Cygwin failure here,\nand with the Linux failure at #33488.\n\nCC:  @fchapoton @mkoeppe @seblabbe @slel\n\nKeywords: cygwin\n\nBranch/Commit: 3aa3f09ea35d9acc124c9148df8a3cf43cb4acbd\n\nReviewer: Ethan Lin\n\nAuthor: Tobias Diez\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/33139\n\n",
    "closed_at": "2022-04-02T10:53:32Z",
    "created_at": "2022-01-10T06:46:53Z",
    "labels": [
        "component: packages: standard",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.6",
    "title": "Fix sagemath_doc_html build failure on Cygwin",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/33139",
    "user": "https://github.com/slel"
}
```
On sage-release, report on a failure in Cygwin
(from a fresh git clone):

- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/oLwrXx6yAQAJ

Also observed on Linux, where `make distclean` fixed it:

- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ
- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ

Let us deal with the Cygwin failure here,
and with the Linux failure at #33488.

CC:  @fchapoton @mkoeppe @seblabbe @slel

Keywords: cygwin

Branch/Commit: 3aa3f09ea35d9acc124c9148df8a3cf43cb4acbd

Reviewer: Ethan Lin

Author: Tobias Diez

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/33139





---

archive/issue_comments_661068.json:
```json
{
    "body": "<a id='comment:1'></a>Also observed on Linux, where `make distclean` fixed it.\n\n- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ\n- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ\n\nSo this ticket's component is not \"porting: Cygwin\".\n\nMaybe \"build\" or \"documentation\" or \"packages: standard\"?",
    "created_at": "2022-01-10T22:44:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661068",
    "user": "https://github.com/slel"
}
```

<a id='comment:1'></a>Also observed on Linux, where `make distclean` fixed it.

- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ
- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ

So this ticket's component is not "porting: Cygwin".

Maybe "build" or "documentation" or "packages: standard"?



---

archive/issue_comments_661069.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,8 @@\n-As reported on sage-release:\n+On sage-release, report on a failure in Cygwin:\n \n - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/oLwrXx6yAQAJ\n+\n+Also observed on Linux, where `make distclean` fixed it:\n+\n+- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ\n+- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ\n``````\n",
    "created_at": "2022-01-10T22:44:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661069",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,8 @@
-As reported on sage-release:
+On sage-release, report on a failure in Cygwin:
 
 - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/oLwrXx6yAQAJ
+
+Also observed on Linux, where `make distclean` fixed it:
+
+- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ
+- https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ
``````




---

archive/issue_comments_661070.json:
```json
{
    "body": "Changing component from porting: Cygwin to packages: standard.",
    "created_at": "2022-01-10T22:44:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661070",
    "user": "https://github.com/slel"
}
```

Changing component from porting: Cygwin to packages: standard.



---

archive/issue_comments_661071.json:
```json
{
    "body": "Changing keywords from \"cygwin\" to \"\".",
    "created_at": "2022-01-10T23:22:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661071",
    "user": "https://github.com/slel"
}
```

Changing keywords from "cygwin" to "".



---

archive/issue_comments_661072.json:
```json
{
    "body": "<a id='comment:2'></a>See alsoticket: ticket:33142#comment:4#comment:33142 and patchbot reports for #33142:\n\n- https://patchbot.sagemath.org/ticket/33142/",
    "created_at": "2022-01-10T23:22:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661072",
    "user": "https://github.com/slel"
}
```

<a id='comment:2'></a>See alsoticket: ticket:33142#comment:4#comment:33142 and patchbot reports for #33142:

- https://patchbot.sagemath.org/ticket/33142/



---

archive/issue_comments_661073.json:
```json
{
    "body": "<a id='comment:3'></a>I think this is \"only\" an incremental docbuild failure, probably nothing to be fixed here\n\nFrom-scratch build of Sage on Cygwin on GH Actions succeeded in https://github.com/sagemath/sage/runs/4768915824?check_suite_focus=true - including documentation",
    "created_at": "2022-01-12T06:11:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661073",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'></a>I think this is "only" an incremental docbuild failure, probably nothing to be fixed here

From-scratch build of Sage on Cygwin on GH Actions succeeded in https://github.com/sagemath/sage/runs/4768915824?check_suite_focus=true - including documentation



---

archive/issue_comments_661074.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,5 @@\n-On sage-release, report on a failure in Cygwin:\n+On sage-release, report on a failure in Cygwin\n+(from a fresh git clone):\n \n - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/oLwrXx6yAQAJ\n \n``````\n",
    "created_at": "2022-01-12T07:55:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661074",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,5 @@
-On sage-release, report on a failure in Cygwin:
+On sage-release, report on a failure in Cygwin
+(from a fresh git clone):
 
 - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/oLwrXx6yAQAJ
 
``````




---

archive/issue_comments_661075.json:
```json
{
    "body": "<a id='comment:4'></a>In my case it was a fresh git clone. I'll try again.",
    "created_at": "2022-01-12T07:55:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661075",
    "user": "https://github.com/slel"
}
```

<a id='comment:4'></a>In my case it was a fresh git clone. I'll try again.



---

archive/issue_comments_661076.json:
```json
{
    "body": "<a id='comment:5'></a>There are persistent problems with patchbots. Somebody needs to fix them. Maybe the people that introduced \"make doc-uninstall\" can pick up the task ?",
    "created_at": "2022-01-14T20:28:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661076",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'></a>There are persistent problems with patchbots. Somebody needs to fix them. Maybe the people that introduced "make doc-uninstall" can pick up the task ?



---

archive/issue_comments_661077.json:
```json
{
    "body": "Changing keywords from \"\" to \"cygwin\".",
    "created_at": "2022-01-15T08:48:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661077",
    "user": "https://github.com/slel"
}
```

Changing keywords from "" to "cygwin".



---

archive/issue_comments_661078.json:
```json
{
    "body": "<a id='comment:6'></a>New report on sage-release for Sage 9.5.rc1:\n\n- https://groups.google.com/g/sage-release/c/8ctwsUGl6LQ/m/CeIBwxSBBwAJ",
    "created_at": "2022-01-15T08:48:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661078",
    "user": "https://github.com/slel"
}
```

<a id='comment:6'></a>New report on sage-release for Sage 9.5.rc1:

- https://groups.google.com/g/sage-release/c/8ctwsUGl6LQ/m/CeIBwxSBBwAJ



---

archive/issue_comments_661079.json:
```json
{
    "body": "Changing priority from major to critical.",
    "created_at": "2022-01-15T08:48:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661079",
    "user": "https://github.com/slel"
}
```

Changing priority from major to critical.



---

archive/issue_comments_661080.json:
```json
{
    "body": "<a id='comment:7'></a>Happened again on Cygwin and Debian as reported at\n\n- https://groups.google.com/g/sage-release/c/1YV0dNtEZz4/m/T8IAplFRAgAJ",
    "created_at": "2022-01-22T13:06:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661080",
    "user": "https://github.com/slel"
}
```

<a id='comment:7'></a>Happened again on Cygwin and Debian as reported at

- https://groups.google.com/g/sage-release/c/1YV0dNtEZz4/m/T8IAplFRAgAJ



---

archive/issue_comments_661081.json:
```json
{
    "body": "<a id='comment:8'></a>This failed on Cygwin for all releases from\nSage 9.5.beta7 to Sage 9.5.rc4 for me.",
    "created_at": "2022-01-26T16:29:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661081",
    "user": "https://github.com/slel"
}
```

<a id='comment:8'></a>This failed on Cygwin for all releases from
Sage 9.5.beta7 to Sage 9.5.rc4 for me.



---

archive/issue_comments_661082.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,3 +7,7 @@\n \n - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ\n - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ\n+\n+Possibly related to #32789, see\n+\n+- https://groups.google.com/g/sage-release/c/FXpakLURP_0/m/rzFhVu5nBgAJ\n``````\n",
    "created_at": "2022-01-26T16:35:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661082",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,3 +7,7 @@
 
 - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ
 - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ
+
+Possibly related to #32789, see
+
+- https://groups.google.com/g/sage-release/c/FXpakLURP_0/m/rzFhVu5nBgAJ
``````




---

archive/issue_comments_661083.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,6 +8,4 @@\n - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ\n - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ\n \n-Possibly related to #32789, see\n \n-- https://groups.google.com/g/sage-release/c/FXpakLURP_0/m/rzFhVu5nBgAJ\n``````\n",
    "created_at": "2022-01-27T14:30:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661083",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,6 +8,4 @@
 - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ
 - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ
 
-Possibly related to #32789, see
 
-- https://groups.google.com/g/sage-release/c/FXpakLURP_0/m/rzFhVu5nBgAJ
``````




---

archive/issue_comments_661084.json:
```json
{
    "body": "<a id='comment:10'></a>A link with #32789 was initially suspected, but then ruled out, see:\n\n- https://groups.google.com/g/sage-release/c/FXpakLURP_0/m/rzFhVu5nBgAJ\n-ticket: ticket:32954#comment:8#comment:32954\n\nFor Sage 9.5.rc4, the file `logs/pkgs/sagemath_doc_html-none.log`\nis as follows, up to replacing:\n\n- `INVENTORY_REF`: `local/share/doc/sage/inventory/en/reference`\n- `DOCTREES_REF`: `local/share/doc/sage/doctrees/en/reference`\n- `SITE_PKGS`: `local/var/lib/sage/venv-python3.9/lib/python3.9/site-packages`\n- `SRC_REF`: `src/doc/en/reference`\n\n```\n[reference] building [inventory]: targets for 1 source files that are out of date\n[reference] updating environment: [new config] 1 added, 0 changed, 0 removed\n[reference] The inventory files are in INVENTORY_REF/references.\nBuild finished. The built documents can be found in INVENTORY_REF/references\n[spkg     ] building [inventory]: targets for 354 source files that are out of date\n[spkg     ] updating environment: [new config] 354 added, 0 changed, 0 removed\n[spkg     ] The inventory files are in INVENTORY_REF/spkg.\nBuild finished. The built documents can be found in INVENTORY_REF/spkg\n[manifolds] building [inventory]: targets for 84 source files that are out of date\n[manifolds] updating environment: [new config] 84 added, 0 changed, 0 removed\n[manifolds] The inventory files are in INVENTORY_REF/manifolds.\nBuild finished. The built documents can be found in INVENTORY_REF/manifolds\n[polynomia] building [inventory]: targets for 62 source files that are out of date\n[polynomia] updating environment: [new config] 62 added, 0 changed, 0 removed\n[polynomia] The inventory files are in INVENTORY_REF/polynomial_rings.\nBuild finished. The built documents can be found in INVENTORY_REF/polynomial_rings\n[repl     ] building [inventory]: targets for 36 source files that are out of date\n[repl     ] updating environment: [new config] 36 added, 0 changed, 0 removed\n[repl     ] The inventory files are in INVENTORY_REF/repl.\nBuild finished. The built documents can be found in INVENTORY_REF/repl\n[algebras ] building [inventory]: targets for 99 source files that are out of date\n[algebras ] updating environment: [new config] 99 added, 0 changed, 0 removed\n[tensor_fr] building [inventory]: targets for 19 source files that are out of date\n[tensor_fr] updating environment: [new config] 19 added, 0 changed, 0 removed\n[tensor_fr] The inventory files are in INVENTORY_REF/tensor_free_modules.\nBuild finished. The built documents can be found in INVENTORY_REF/tensor_free_modules\n[combinat ] building [inventory]: targets for 369 source files that are out of date\n[combinat ] updating environment: [new config] 369 added, 0 changed, 0 removed\n[dynamics ] building [inventory]: targets for 16 source files that are out of date\n[dynamics ] updating environment: [new config] 16 added, 0 changed, 0 removed\n[dynamics ] The inventory files are in INVENTORY_REF/dynamics.\nBuild finished. The built documents can be found in INVENTORY_REF/dynamics\n[plot3d   ] building [inventory]: targets for 20 source files that are out of date\n[plot3d   ] updating environment: [new config] 20 added, 0 changed, 0 removed\n[plot3d   ] The inventory files are in INVENTORY_REF/plot3d.\nBuild finished. The built documents can be found in INVENTORY_REF/plot3d\n...\n[structure] building [inventory]: targets for 32 source files that are out of date\n[structure] updating environment: [new config] 32 added, 0 changed, 0 removed\n[structure] The inventory files are in INVENTORY_REF/structure.\nBuild finished. The built documents can be found in INVENTORY_REF/structure\n[valuation] building [inventory]: targets for 14 source files that are out of date\n[valuation] updating environment: [new config] 14 added, 0 changed, 0 removed\n[valuation] The inventory files are in INVENTORY_REF/valuations.\nBuild finished. The built documents can be found in INVENTORY_REF/valuations\n[reference] building [inventory]: targets for 1 source files that are out of date\n[reference] updating environment: [new config] 1 added, 0 changed, 0 removed\n[reference] Merging environment/index file\n[reference]     algebras:\n[reference]     arithgroup: 1 todos, 12 index, 0 citations, 11 modules\n[reference]     arithmetic_curves: 12 todos, 59 index, 2 citations, 58 modules\n[reference]     asymptotic: 6 todos, 8 index, 0 citations, 7 modules\n[reference]     calculus: 2 todos, 38 index, 0 citations, 37 modules\n[reference]     categories:\n[reference]     coding:\n[reference]     coercion: 0 todos, 8 index, 0 citations, 7 modules\n[reference]     combinat:\n[reference]     constants: 0 todos, 3 index, 0 citations, 2 modules\n[reference]     cpython: 0 todos, 8 index, 0 citations, 7 modules\n...\n[reference]     tensor_free_modules: 2 todos, 19 index, 0 citations, 15 modules\n[reference]     topology: 0 todos, 14 index, 0 citations, 13 modules\n[reference]     valuations: 1 todos, 14 index, 0 citations, 13 modules\n[reference] ... done (184 todos, 1670 index, 1434 citations, 1281 modules)\n[reference] WARNING: Unable to fetch DOCTREES_REF/algebras/environment.pickle\n[reference] WARNING: Unable to fetch DOCTREES_REF/categories/environment.pickle\n[reference] WARNING: Unable to fetch DOCTREES_REF/coding/environment.pickle\n[reference] WARNING: Unable to fetch DOCTREES_REF/combinat/environment.pickle\n[reference] WARNING: Unable to fetch DOCTREES_REF/graphs/environment.pickle\n[reference] WARNING: Unable to fetch DOCTREES_REF/groups/environment.pickle\n[reference] WARNING: Unable to fetch DOCTREES_REF/knots/environment.pickle\n[reference] WARNING: Unable to fetch DOCTREES_REF/libs/environment.pickle\n[reference] preparing documents... skipping loading of indexes... done\n[reference] SRC_REF/index.rst:35: WARNING: unknown document: categories/index\n[reference] SRC_REF/index.rst:77: WARNING: unknown document: groups/index\n[reference] SRC_REF/index.rst:80: WARNING: unknown document: algebras/index\n[reference] SRC_REF/index.rst:85: WARNING: unknown document: combinat/index\n[reference] SRC_REF/index.rst:86: WARNING: unknown document: graphs/index\n[reference] SRC_REF/index.rst:90: WARNING: unknown document: coding/index\n[reference] SRC_REF/index.rst:105: WARNING: unknown document: knots/index\n[reference] SRC_REF/index.rst:156: WARNING: unknown document: libs/index\n[reference] The inventory files are in INVENTORY_REF.\nError building the documentation.\nTraceback (most recent call last):\n  File \"/usr/lib/python3.9/runpy.py\", line 197, in _run_module_as_main\n    return _run_code(code, main_globals, None,\n  File \"/usr/lib/python3.9/runpy.py\", line 87, in _run_code\n    exec(code, run_globals)\n  File \"SITE_PKGS/sage_docbuild/__main__.py\", line 2, in <module>\n    main()\n  File \"SITE_PKGS/sage_docbuild/__init__.py\", line 1729, in main\n    builder()\n  File \"SITE_PKGS/sage_docbuild/__init__.py\", line 137, in f\n    runsphinx()\n  File \"SITE_PKGS/sage_docbuild/sphinxbuild.py\", line 323, in runsphinx\n    sys.stderr.raise_errors()\n  File \"SITE_PKGS/sage_docbuild/sphinxbuild.py\", line 258, in raise_errors\n    raise OSError(self._error)\nOSError: WARNING: Unable to fetch DOCTREES_REF/algebras/environment.pickle \n\n    Note: incremental documentation builds sometimes cause spurious\n    error messages. To be certain that these are real errors, run\n    \"make doc-clean doc-uninstall\" first and try again.\nmake[6]: *** [Makefile:20: doc-inventory--reference_top] Error 1\nmake[5]: *** [Makefile:38: doc-inventory-reference] Error 2\n```\n\nWhat are the `environment.pickle` files\nin `local/share/doc/sage/doctrees/en/reference/*`?\n\nWhy are some of them missing?",
    "created_at": "2022-01-27T14:30:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661084",
    "user": "https://github.com/slel"
}
```

<a id='comment:10'></a>A link with #32789 was initially suspected, but then ruled out, see:

- https://groups.google.com/g/sage-release/c/FXpakLURP_0/m/rzFhVu5nBgAJ
-ticket: ticket:32954#comment:8#comment:32954

For Sage 9.5.rc4, the file `logs/pkgs/sagemath_doc_html-none.log`
is as follows, up to replacing:

- `INVENTORY_REF`: `local/share/doc/sage/inventory/en/reference`
- `DOCTREES_REF`: `local/share/doc/sage/doctrees/en/reference`
- `SITE_PKGS`: `local/var/lib/sage/venv-python3.9/lib/python3.9/site-packages`
- `SRC_REF`: `src/doc/en/reference`

```
[reference] building [inventory]: targets for 1 source files that are out of date
[reference] updating environment: [new config] 1 added, 0 changed, 0 removed
[reference] The inventory files are in INVENTORY_REF/references.
Build finished. The built documents can be found in INVENTORY_REF/references
[spkg     ] building [inventory]: targets for 354 source files that are out of date
[spkg     ] updating environment: [new config] 354 added, 0 changed, 0 removed
[spkg     ] The inventory files are in INVENTORY_REF/spkg.
Build finished. The built documents can be found in INVENTORY_REF/spkg
[manifolds] building [inventory]: targets for 84 source files that are out of date
[manifolds] updating environment: [new config] 84 added, 0 changed, 0 removed
[manifolds] The inventory files are in INVENTORY_REF/manifolds.
Build finished. The built documents can be found in INVENTORY_REF/manifolds
[polynomia] building [inventory]: targets for 62 source files that are out of date
[polynomia] updating environment: [new config] 62 added, 0 changed, 0 removed
[polynomia] The inventory files are in INVENTORY_REF/polynomial_rings.
Build finished. The built documents can be found in INVENTORY_REF/polynomial_rings
[repl     ] building [inventory]: targets for 36 source files that are out of date
[repl     ] updating environment: [new config] 36 added, 0 changed, 0 removed
[repl     ] The inventory files are in INVENTORY_REF/repl.
Build finished. The built documents can be found in INVENTORY_REF/repl
[algebras ] building [inventory]: targets for 99 source files that are out of date
[algebras ] updating environment: [new config] 99 added, 0 changed, 0 removed
[tensor_fr] building [inventory]: targets for 19 source files that are out of date
[tensor_fr] updating environment: [new config] 19 added, 0 changed, 0 removed
[tensor_fr] The inventory files are in INVENTORY_REF/tensor_free_modules.
Build finished. The built documents can be found in INVENTORY_REF/tensor_free_modules
[combinat ] building [inventory]: targets for 369 source files that are out of date
[combinat ] updating environment: [new config] 369 added, 0 changed, 0 removed
[dynamics ] building [inventory]: targets for 16 source files that are out of date
[dynamics ] updating environment: [new config] 16 added, 0 changed, 0 removed
[dynamics ] The inventory files are in INVENTORY_REF/dynamics.
Build finished. The built documents can be found in INVENTORY_REF/dynamics
[plot3d   ] building [inventory]: targets for 20 source files that are out of date
[plot3d   ] updating environment: [new config] 20 added, 0 changed, 0 removed
[plot3d   ] The inventory files are in INVENTORY_REF/plot3d.
Build finished. The built documents can be found in INVENTORY_REF/plot3d
...
[structure] building [inventory]: targets for 32 source files that are out of date
[structure] updating environment: [new config] 32 added, 0 changed, 0 removed
[structure] The inventory files are in INVENTORY_REF/structure.
Build finished. The built documents can be found in INVENTORY_REF/structure
[valuation] building [inventory]: targets for 14 source files that are out of date
[valuation] updating environment: [new config] 14 added, 0 changed, 0 removed
[valuation] The inventory files are in INVENTORY_REF/valuations.
Build finished. The built documents can be found in INVENTORY_REF/valuations
[reference] building [inventory]: targets for 1 source files that are out of date
[reference] updating environment: [new config] 1 added, 0 changed, 0 removed
[reference] Merging environment/index file
[reference]     algebras:
[reference]     arithgroup: 1 todos, 12 index, 0 citations, 11 modules
[reference]     arithmetic_curves: 12 todos, 59 index, 2 citations, 58 modules
[reference]     asymptotic: 6 todos, 8 index, 0 citations, 7 modules
[reference]     calculus: 2 todos, 38 index, 0 citations, 37 modules
[reference]     categories:
[reference]     coding:
[reference]     coercion: 0 todos, 8 index, 0 citations, 7 modules
[reference]     combinat:
[reference]     constants: 0 todos, 3 index, 0 citations, 2 modules
[reference]     cpython: 0 todos, 8 index, 0 citations, 7 modules
...
[reference]     tensor_free_modules: 2 todos, 19 index, 0 citations, 15 modules
[reference]     topology: 0 todos, 14 index, 0 citations, 13 modules
[reference]     valuations: 1 todos, 14 index, 0 citations, 13 modules
[reference] ... done (184 todos, 1670 index, 1434 citations, 1281 modules)
[reference] WARNING: Unable to fetch DOCTREES_REF/algebras/environment.pickle
[reference] WARNING: Unable to fetch DOCTREES_REF/categories/environment.pickle
[reference] WARNING: Unable to fetch DOCTREES_REF/coding/environment.pickle
[reference] WARNING: Unable to fetch DOCTREES_REF/combinat/environment.pickle
[reference] WARNING: Unable to fetch DOCTREES_REF/graphs/environment.pickle
[reference] WARNING: Unable to fetch DOCTREES_REF/groups/environment.pickle
[reference] WARNING: Unable to fetch DOCTREES_REF/knots/environment.pickle
[reference] WARNING: Unable to fetch DOCTREES_REF/libs/environment.pickle
[reference] preparing documents... skipping loading of indexes... done
[reference] SRC_REF/index.rst:35: WARNING: unknown document: categories/index
[reference] SRC_REF/index.rst:77: WARNING: unknown document: groups/index
[reference] SRC_REF/index.rst:80: WARNING: unknown document: algebras/index
[reference] SRC_REF/index.rst:85: WARNING: unknown document: combinat/index
[reference] SRC_REF/index.rst:86: WARNING: unknown document: graphs/index
[reference] SRC_REF/index.rst:90: WARNING: unknown document: coding/index
[reference] SRC_REF/index.rst:105: WARNING: unknown document: knots/index
[reference] SRC_REF/index.rst:156: WARNING: unknown document: libs/index
[reference] The inventory files are in INVENTORY_REF.
Error building the documentation.
Traceback (most recent call last):
  File "/usr/lib/python3.9/runpy.py", line 197, in _run_module_as_main
    return _run_code(code, main_globals, None,
  File "/usr/lib/python3.9/runpy.py", line 87, in _run_code
    exec(code, run_globals)
  File "SITE_PKGS/sage_docbuild/__main__.py", line 2, in <module>
    main()
  File "SITE_PKGS/sage_docbuild/__init__.py", line 1729, in main
    builder()
  File "SITE_PKGS/sage_docbuild/__init__.py", line 137, in f
    runsphinx()
  File "SITE_PKGS/sage_docbuild/sphinxbuild.py", line 323, in runsphinx
    sys.stderr.raise_errors()
  File "SITE_PKGS/sage_docbuild/sphinxbuild.py", line 258, in raise_errors
    raise OSError(self._error)
OSError: WARNING: Unable to fetch DOCTREES_REF/algebras/environment.pickle 

    Note: incremental documentation builds sometimes cause spurious
    error messages. To be certain that these are real errors, run
    "make doc-clean doc-uninstall" first and try again.
make[6]: *** [Makefile:20: doc-inventory--reference_top] Error 1
make[5]: *** [Makefile:38: doc-inventory-reference] Error 2
```

What are the `environment.pickle` files
in `local/share/doc/sage/doctrees/en/reference/*`?

Why are some of them missing?



---

archive/issue_events_087563.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-01-29T08:58:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33139#event-87563"
}
```



---

archive/issue_comments_661085.json:
```json
{
    "body": "<a id='comment:11'></a>bump to 9.6",
    "created_at": "2022-01-29T08:58:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661085",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:11'></a>bump to 9.6



---

archive/issue_comments_661086.json:
```json
{
    "body": "<a id='comment:12'></a>This breaks my development versions of Sage\non Cygwin, Debian, macOS, Ubuntu.\n\nI agree Sage 9.5 needs to get out though,\nand I have no clue on solving this failure.",
    "created_at": "2022-01-29T15:23:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661086",
    "user": "https://github.com/slel"
}
```

<a id='comment:12'></a>This breaks my development versions of Sage
on Cygwin, Debian, macOS, Ubuntu.

I agree Sage 9.5 needs to get out though,
and I have no clue on solving this failure.



---

archive/issue_comments_661087.json:
```json
{
    "body": "<a id='comment:13'></a>~~This blocked merging a positively reviewed ticket,\nseeticket: ticket:32505#comment:83#comment:32505.~~\n\nOops, no it did not. Something else caused an error\nin sagemath_doc_html there.",
    "created_at": "2022-01-31T08:51:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661087",
    "user": "https://github.com/slel"
}
```

<a id='comment:13'></a>~~This blocked merging a positively reviewed ticket,
seeticket: ticket:32505#comment:83#comment:32505.~~

Oops, no it did not. Something else caused an error
in sagemath_doc_html there.



---

archive/issue_comments_661088.json:
```json
{
    "body": "<a id='comment:14'></a>I hit the same issue (on WSL2, tried to clean the doc build and error persisted). What helped for me was to demote the warning (which is then treated as an error) to a debug message.\nThe build seems to succeed without any issues without the pickled env file.\n\nA very similar issue occurs also on the github workflow added in #31415.\nThere, sphinx (not the sage code invoking sphinx) reports the following messages:\n\n```\n[sagemath_doc_html-none] [spkg     ] loading pickled environment... failed\n[sagemath_doc_html-none] [spkg     ] failed: source directory has changed\n[sagemath_doc_html-none] [spkg     ] building [inventory]: targets for 355 source files that are out of date\n[sagemath_doc_html-none] [spkg     ] updating environment: [new config] 355 added, 0 changed, 0 removed\n```\nBut since there is no \"Warning\" prefix, these are then not upgraded to errors.\n\nSee for example, https://github.com/sagemath/sagetrac-mirror/runs/5112648667?check_suite_focus=true\n\n---\nNew commits:",
    "created_at": "2022-02-08T19:40:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661088",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:14'></a>I hit the same issue (on WSL2, tried to clean the doc build and error persisted). What helped for me was to demote the warning (which is then treated as an error) to a debug message.
The build seems to succeed without any issues without the pickled env file.

A very similar issue occurs also on the github workflow added in #31415.
There, sphinx (not the sage code invoking sphinx) reports the following messages:

```
[sagemath_doc_html-none] [spkg     ] loading pickled environment... failed
[sagemath_doc_html-none] [spkg     ] failed: source directory has changed
[sagemath_doc_html-none] [spkg     ] building [inventory]: targets for 355 source files that are out of date
[sagemath_doc_html-none] [spkg     ] updating environment: [new config] 355 added, 0 changed, 0 removed
```
But since there is no "Warning" prefix, these are then not upgraded to errors.

See for example, https://github.com/sagemath/sagetrac-mirror/runs/5112648667?check_suite_focus=true

---
New commits:



---

archive/issue_comments_661089.json:
```json
{
    "body": "Changing author from \"\" to \"Tobias Diez\"",
    "created_at": "2022-02-08T19:40:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661089",
    "user": "https://github.com/tobiasdiez"
}
```

Changing author from "" to "Tobias Diez"



---

archive/issue_comments_661090.json:
```json
{
    "body": "Changing commit from \"\" to \"3aa3f09ea35d9acc124c9148df8a3cf43cb4acbd\"",
    "created_at": "2022-02-08T19:40:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661090",
    "user": "https://github.com/tobiasdiez"
}
```

Changing commit from "" to "3aa3f09ea35d9acc124c9148df8a3cf43cb4acbd"



---

archive/issue_comments_661091.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2022-02-08T19:40:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661091",
    "user": "https://github.com/tobiasdiez"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_661092.json:
```json
{
    "body": "Changing branch from \"\" to \"public/docs/pickled_env\"",
    "created_at": "2022-02-08T19:40:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661092",
    "user": "https://github.com/tobiasdiez"
}
```

Changing branch from "" to "public/docs/pickled_env"



---

archive/issue_comments_661093.json:
```json
{
    "body": "<a id='comment:15'></a>Let us deal with the Cygwin failure here,\nand with the Linux failure at #33488.",
    "created_at": "2022-03-10T04:58:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661093",
    "user": "https://github.com/slel"
}
```

<a id='comment:15'></a>Let us deal with the Cygwin failure here,
and with the Linux failure at #33488.



---

archive/issue_comments_661094.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,5 @@\n - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ\n - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ\n \n-\n+Let us deal with the Cygwin failure here,\n+and with the Linux failure at #33488.\n``````\n",
    "created_at": "2022-03-10T04:58:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661094",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,5 @@
 - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/4iALmgfJAQAJ
 - https://groups.google.com/g/sage-release/c/gTNMEVbzb6M/m/k152YqXPAQAJ
 
-
+Let us deal with the Cygwin failure here,
+and with the Linux failure at #33488.
``````




---

archive/issue_comments_661095.json:
```json
{
    "body": "<a id='comment:16'></a>It looks like #33488 is about a different error, though, or not?\n\nWhat you think about the fix here in this ticket? It solves the issue for me also on Ubuntu.",
    "created_at": "2022-03-10T17:41:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661095",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:16'></a>It looks like #33488 is about a different error, though, or not?

What you think about the fix here in this ticket? It solves the issue for me also on Ubuntu.



---

archive/issue_comments_661096.json:
```json
{
    "body": "<a id='comment:17'></a>Can I be notified of this as well?",
    "created_at": "2022-03-10T20:50:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661096",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

<a id='comment:17'></a>Can I be notified of this as well?



---

archive/issue_comments_661097.json:
```json
{
    "body": "<a id='comment:18'></a>So on the latest version of sage, 9.6.beta5, the sagemath_doc_html_none did successfully build :) and now I have a functioning version of sage!!",
    "created_at": "2022-03-21T20:43:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661097",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

<a id='comment:18'></a>So on the latest version of sage, 9.6.beta5, the sagemath_doc_html_none did successfully build :) and now I have a functioning version of sage!!



---

archive/issue_comments_661098.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-03-21T20:45:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661098",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_661099.json:
```json
{
    "body": "Changing reviewer from \"\" to \"gh-WindowsSystemAdmin\"",
    "created_at": "2022-03-21T20:53:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661099",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

Changing reviewer from "" to "gh-WindowsSystemAdmin"



---

archive/issue_comments_661100.json:
```json
{
    "body": "<a id='comment:21'></a>Thanks for the review!",
    "created_at": "2022-03-21T20:57:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661100",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:21'></a>Thanks for the review!



---

archive/issue_comments_661101.json:
```json
{
    "body": "<a id='comment:22'></a>Full reviewer name please",
    "created_at": "2022-03-21T21:25:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661101",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:22'></a>Full reviewer name please



---

archive/issue_comments_661102.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2022-03-21T23:41:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661102",
    "user": "https://github.com/vbraun"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_661103.json:
```json
{
    "body": "<a id='comment:23'></a>Merge failure on top of:\n\n6dbf0930e7 Trac #30845: Fixes for the conda-for-Sage-developers installation method, add GH Actions workflow\n\n95307a68b5 Trac #33522: pythran: Update to 0.11.0\n\n3afe1c712a Trac #33440: sage.libs.singular: Do not fail if the Singular binary is not in PATH\n\n3cc2873986 Trac #33437: Add Lehrer-Solomon symmetric function\n\n6e77b3fd5b Trac #33416: Colormaps and contours for complex_plot\n\na636498287 Trac #33407: sage -t --installed\n\nff2baed67c Trac #33396: removing some useless ...: in the doc\n\ndce5f06cf9 Trac #33233: sage -t --baseline-stats-path\n\nf06a7f845e Trac #33170: Update doctests for compatibility with ipython 8.x\n\n2fbf359176 Trac #33156: fix wrong results & huge slowdown due to broken caching in .multiplication_by_m()\n\nb8d91ace4f Trac #33141: Fix doctests in sage_setup and sage_docbuild for sage-on-distribution\n\nf30a4c8b06 Trac #32942: Introduce `align_latex` option that allows latex displays aligned left or centered\n\n39de62bca0 Trac #32429: Typos\n\nd79b00d98a Trac #30078: Check for duplicate hyperplanes in arrangements over any base ring\n\na633cce6f8 Trac #33532: Doctest failure when SAGE_LIB doesn't match `.../site-packages`\n\n9873ee5a57 Trac #33361: configure: Handle the case of SAGE_LOCAL = a system directory with gcc better\n\ndc311dcd82 Trac #32975: Improve doctest interaction with pytest\n\n0e4f121222 Trac #33471: Add pictures to various 2d graphics manual pages\n\n5b20998e56 Trac #33487: Package CyLP\n\ndf50acb8cc Trac #33477: Conversion problem between multivariate Laurent polynomial ring and its field of fraction\n\n3c80055420 Trac #33475: deTeX document titles of sage documentation\n\n5b872c9d08 Trac #30717: add \"How to cite SageMath\" to FAQ\n\na61c15ecd6 Trac #29631: spkg-configure.m4 for linbox\n\n55a711e3d6 Updated [SageMath](SageMath) version to 9.6.beta5\n\n\n\nreviewer 'gh-WindowsSystemAdmin' does not look right",
    "created_at": "2022-03-21T23:41:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661103",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:23'></a>Merge failure on top of:

6dbf0930e7 Trac #30845: Fixes for the conda-for-Sage-developers installation method, add GH Actions workflow

95307a68b5 Trac #33522: pythran: Update to 0.11.0

3afe1c712a Trac #33440: sage.libs.singular: Do not fail if the Singular binary is not in PATH

3cc2873986 Trac #33437: Add Lehrer-Solomon symmetric function

6e77b3fd5b Trac #33416: Colormaps and contours for complex_plot

a636498287 Trac #33407: sage -t --installed

ff2baed67c Trac #33396: removing some useless ...: in the doc

dce5f06cf9 Trac #33233: sage -t --baseline-stats-path

f06a7f845e Trac #33170: Update doctests for compatibility with ipython 8.x

2fbf359176 Trac #33156: fix wrong results & huge slowdown due to broken caching in .multiplication_by_m()

b8d91ace4f Trac #33141: Fix doctests in sage_setup and sage_docbuild for sage-on-distribution

f30a4c8b06 Trac #32942: Introduce `align_latex` option that allows latex displays aligned left or centered

39de62bca0 Trac #32429: Typos

d79b00d98a Trac #30078: Check for duplicate hyperplanes in arrangements over any base ring

a633cce6f8 Trac #33532: Doctest failure when SAGE_LIB doesn't match `.../site-packages`

9873ee5a57 Trac #33361: configure: Handle the case of SAGE_LOCAL = a system directory with gcc better

dc311dcd82 Trac #32975: Improve doctest interaction with pytest

0e4f121222 Trac #33471: Add pictures to various 2d graphics manual pages

5b20998e56 Trac #33487: Package CyLP

df50acb8cc Trac #33477: Conversion problem between multivariate Laurent polynomial ring and its field of fraction

3c80055420 Trac #33475: deTeX document titles of sage documentation

5b872c9d08 Trac #30717: add "How to cite SageMath" to FAQ

a61c15ecd6 Trac #29631: spkg-configure.m4 for linbox

55a711e3d6 Updated [SageMath](SageMath) version to 9.6.beta5



reviewer 'gh-WindowsSystemAdmin' does not look right



---

archive/issue_comments_661104.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2022-03-25T16:42:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661104",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_661105.json:
```json
{
    "body": "<a id='comment:25'></a>How do I change my username?",
    "created_at": "2022-03-25T20:53:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661105",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

<a id='comment:25'></a>How do I change my username?



---

archive/issue_comments_661106.json:
```json
{
    "body": "<a id='comment:26'></a>Your username is fine. Type in your real name\nin the \"Reviewers\" field.",
    "created_at": "2022-03-25T23:14:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661106",
    "user": "https://github.com/slel"
}
```

<a id='comment:26'></a>Your username is fine. Type in your real name
in the "Reviewers" field.



---

archive/issue_comments_661107.json:
```json
{
    "body": "<a id='comment:27'></a>k",
    "created_at": "2022-03-26T17:13:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661107",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

<a id='comment:27'></a>k



---

archive/issue_comments_661108.json:
```json
{
    "body": "Changing reviewer from \"gh-WindowsSystemAdmin\" to \"Ethan Lin\"",
    "created_at": "2022-03-26T17:14:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661108",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

Changing reviewer from "gh-WindowsSystemAdmin" to "Ethan Lin"



---

archive/issue_comments_661109.json:
```json
{
    "body": "<a id='comment:29'></a>Thank you. And now you can set to positive review!",
    "created_at": "2022-03-26T20:44:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661109",
    "user": "https://github.com/slel"
}
```

<a id='comment:29'></a>Thank you. And now you can set to positive review!



---

archive/issue_comments_661110.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-03-28T15:14:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661110",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_087564.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-04-02T10:53:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33139#event-87564"
}
```



---

archive/issue_comments_661111.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2022-04-02T10:53:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661111",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_661112.json:
```json
{
    "body": "Changing branch from \"public/docs/pickled_env\" to \"3aa3f09ea35d9acc124c9148df8a3cf43cb4acbd\"",
    "created_at": "2022-04-02T10:53:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33139#issuecomment-661112",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/docs/pickled_env" to "3aa3f09ea35d9acc124c9148df8a3cf43cb4acbd"
