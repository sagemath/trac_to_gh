# Issue 33887: Fix map_reduce doctest, part 2

archive/issues_033650.json:
```json
{
    "assignees": [],
    "body": "Follow up from #33834. The github workflow still sometimes fails, since the tasks are started in a different order, see eg https://github.com/sagemath/sagetrac-mirror/runs/6543006608?check_suite_focus=true\n\n**CC:**  @dimpase @mkoeppe @fchapoton\n\n**Branch/Commit:** [92c97c9f1cda8fbd53c2373d5cfedb3a374021b1](https://github.com/sagemath/sagetrac-mirror/commit/92c97c9f1cda8fbd53c2373d5cfedb3a374021b1)\n\n**Reviewer:** Matthias Koeppe\n\n**Author:** Tobias Diez\n\nIssue created by migration from https://trac.sagemath.org/ticket/33887\n\n",
    "closed_at": "2022-05-26T22:49:46Z",
    "created_at": "2022-05-23T20:04:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20misc",
        "https://github.com/sagemath/sage/labels/critical",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix map_reduce doctest, part 2",
    "type": "issue",
    "updated_at": "2022-05-26T22:49:46Z",
    "url": "https://github.com/sagemath/sage/issues/33887",
    "user": "https://github.com/tobiasdiez"
}
```
Follow up from #33834. The github workflow still sometimes fails, since the tasks are started in a different order, see eg https://github.com/sagemath/sagetrac-mirror/runs/6543006608?check_suite_focus=true

**CC:**  @dimpase @mkoeppe @fchapoton

**Branch/Commit:** [92c97c9f1cda8fbd53c2373d5cfedb3a374021b1](https://github.com/sagemath/sagetrac-mirror/commit/92c97c9f1cda8fbd53c2373d5cfedb3a374021b1)

**Reviewer:** Matthias Koeppe

**Author:** Tobias Diez

Issue created by migration from https://trac.sagemath.org/ticket/33887





---

archive/issue_events_409522.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2022-05-23T20:04:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33887#event-409522"
}
```



---

archive/issue_events_409523.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2022-05-23T20:04:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20misc",
    "label_color": "08517b",
    "label_name": "component: misc",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33887#event-409523"
}
```



---

archive/issue_events_409524.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2022-05-23T20:04:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "label": "https://github.com/sagemath/sage/labels/critical",
    "label_color": "ff0000",
    "label_name": "critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33887#event-409524"
}
```



---

archive/issue_events_409525.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2022-05-23T20:04:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33887#event-409525"
}
```



---

archive/issue_events_409526.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2022-05-23T20:04:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33887#event-409526"
}
```



---

archive/issue_comments_550733.json:
```json
{
    "body": "**Reviewer:** Matthias Koeppe",
    "created_at": "2022-05-24T18:39:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33887#issuecomment-550733",
    "user": "https://github.com/mkoeppe"
}
```

**Reviewer:** Matthias Koeppe



---

archive/issue_events_409527.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-24T18:39:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33887#event-409527"
}
```



---

archive/issue_events_409528.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-24T18:39:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33887#event-409528"
}
```



---

archive/issue_comments_550734.json:
```json
{
    "body": "<a id='comment:2'>**Comment 2:**</a>\nLGTM",
    "created_at": "2022-05-24T18:39:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33887#issuecomment-550734",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>**Comment 2:**</a>
LGTM



---

archive/issue_comments_550735.json:
```json
{
    "body": "<a id='comment:3'>**Comment 3:**</a>\nThanks!",
    "created_at": "2022-05-25T19:44:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33887#issuecomment-550735",
    "user": "https://github.com/tobiasdiez"
}
```

<a id='comment:3'>**Comment 3:**</a>
Thanks!



---

archive/issue_comments_550736.json:
```json
{
    "body": "**Changing branch** from \"[public/misc/fix_map_reduce_test_two](https://github.com/sagemath/sagetrac-mirror/tree/public/misc/fix_map_reduce_test_two)\" to \"[92c97c9f1cda8fbd53c2373d5cfedb3a374021b1](https://github.com/sagemath/sagetrac-mirror/commit/92c97c9f1cda8fbd53c2373d5cfedb3a374021b1)\".",
    "created_at": "2022-05-26T22:49:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33887#issuecomment-550736",
    "user": "https://github.com/vbraun"
}
```

**Changing branch** from "[public/misc/fix_map_reduce_test_two](https://github.com/sagemath/sagetrac-mirror/tree/public/misc/fix_map_reduce_test_two)" to "[92c97c9f1cda8fbd53c2373d5cfedb3a374021b1](https://github.com/sagemath/sagetrac-mirror/commit/92c97c9f1cda8fbd53c2373d5cfedb3a374021b1)".



---

archive/issue_events_409529.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-05-26T22:49:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33887#event-409529"
}
```



---

archive/issue_events_409530.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7c5c452d36b3a73ac15cf2b8e79d49edf6720cb2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-05-26T22:49:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33887",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33887#event-409530"
}
```
