# Issue 33357: Random set partition with fixed block sizes does not respect the constraint and other improvements

archive/issues_033120.json:
```json
{
    "assignees": [],
    "body": "We have\n\n```\nsage: SP = SetPartitions(10, [4,3,2,1])\nsage: SP.random_element()\n{{1, 2, 4, 5, 7, 8}, {3, 6}, {9}, {10}}\n```\nThis should not be the case. We provide a random sampling that appears to be fairly efficient and uniform.\n\nWe also improve the efficiently of the `__contains__` test and allow unsorted inputs for the block sizes.\n\nCC:  @fchapoton @darijgr @mantepse\n\nKeywords: **set partitions**\n\nBranch/Commit: **[b110cbb](https://github.com/sagemath/sagetrac-mirror/commit/b110cbb78051125549eef3e95da72cf2dddd27a5)**\n\nReviewer: **Martin Rubey**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33357_\n\n",
    "closed_at": "2022-02-21T21:55:41Z",
    "created_at": "2022-02-16T03:03:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Random set partition with fixed block sizes does not respect the constraint and other improvements",
    "type": "issue",
    "updated_at": "2022-02-21T21:55:41Z",
    "url": "https://github.com/sagemath/sage/issues/33357",
    "user": "https://github.com/tscrim"
}
```
We have

```
sage: SP = SetPartitions(10, [4,3,2,1])
sage: SP.random_element()
{{1, 2, 4, 5, 7, 8}, {3, 6}, {9}, {10}}
```
This should not be the case. We provide a random sampling that appears to be fairly efficient and uniform.

We also improve the efficiently of the `__contains__` test and allow unsorted inputs for the block sizes.

CC:  @fchapoton @darijgr @mantepse

Keywords: **set partitions**

Branch/Commit: **[b110cbb](https://github.com/sagemath/sagetrac-mirror/commit/b110cbb78051125549eef3e95da72cf2dddd27a5)**

Reviewer: **Martin Rubey**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/33357_





---

archive/issue_events_440253.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-16T03:03:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440253"
}
```



---

archive/issue_events_440254.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-16T03:03:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440254"
}
```



---

archive/issue_events_440255.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-16T03:03:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440255"
}
```



---

archive/issue_events_440256.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-16T03:03:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440256"
}
```



---

archive/issue_comments_542159.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,4 +5,6 @@\n sage: SP.random_element()\n {{1, 2, 4, 5, 7, 8}, {3, 6}, {9}, {10}}\n ```\n-This should not be the case.\n+This should not be the case. We provide a random sampling that appears to be fairly efficient and uniform.\n+\n+We also improve the efficiently of the `__contains__` test and allow unsorted inputs for the block sizes.\n``````\n",
    "created_at": "2022-02-16T03:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542159",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,4 +5,6 @@
 sage: SP.random_element()
 {{1, 2, 4, 5, 7, 8}, {3, 6}, {9}, {10}}
 ```
-This should not be the case.
+This should not be the case. We provide a random sampling that appears to be fairly efficient and uniform.
+
+We also improve the efficiently of the `__contains__` test and allow unsorted inputs for the block sizes.
``````




---

archive/issue_comments_542160.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2022-02-16T03:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542160",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_comments_542161.json:
```json
{
    "body": "Branch: **[public/combinat/random_set_partitions_block_sizes-33357](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/random_set_partitions_block_sizes-33357)**",
    "created_at": "2022-02-16T03:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542161",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/combinat/random_set_partitions_block_sizes-33357](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/random_set_partitions_block_sizes-33357)**



---

archive/issue_events_440257.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-16T03:50:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440257"
}
```



---

archive/issue_events_440258.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-16T03:50:57Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "title_is": "Random set partition with fixed block sizes does not respect the constraint and other improvements",
    "title_was": "Random set partition with fixed block sizes does not respect the constraint",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440258"
}
```



---

archive/issue_comments_542162.json:
```json
{
    "body": "Changed keywords from **** to **set partitions**",
    "created_at": "2022-02-16T03:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542162",
    "user": "https://github.com/tscrim"
}
```

Changed keywords from **** to **set partitions**



---

archive/issue_comments_542163.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e914110e0a6cd48eccd5b9e0736c6a9e5525b18a\">e914110</a></td><td><code>Implement an algorithm to generate a random set partition with fixed block sizes.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b110cbb78051125549eef3e95da72cf2dddd27a5\">b110cbb</a></td><td><code>Changing doctests and some other improvements to set partitions.</code></td></tr></table>\n",
    "created_at": "2022-02-16T03:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542163",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e914110e0a6cd48eccd5b9e0736c6a9e5525b18a">e914110</a></td><td><code>Implement an algorithm to generate a random set partition with fixed block sizes.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b110cbb78051125549eef3e95da72cf2dddd27a5">b110cbb</a></td><td><code>Changing doctests and some other improvements to set partitions.</code></td></tr></table>




---

archive/issue_comments_542164.json:
```json
{
    "body": "Commit: **[b110cbb](https://github.com/sagemath/sagetrac-mirror/commit/b110cbb78051125549eef3e95da72cf2dddd27a5)**",
    "created_at": "2022-02-16T03:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542164",
    "user": "https://github.com/tscrim"
}
```

Commit: **[b110cbb](https://github.com/sagemath/sagetrac-mirror/commit/b110cbb78051125549eef3e95da72cf2dddd27a5)**



---

archive/issue_comments_542165.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nExcellent, thank you!\n\nI am split on the issue whether `SetPartitions` should sort its `part` argument, so let it be.",
    "created_at": "2022-02-16T07:09:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542165",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:2'>Comment 2:</a>
Excellent, thank you!

I am split on the issue whether `SetPartitions` should sort its `part` argument, so let it be.



---

archive/issue_comments_542166.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nReplying to [@mantepse](#comment%3A2):\n> Excellent, thank you!\n> \n> I am split on the issue whether `SetPartitions` should sort its `part` argument, so let it be.\n\nI thought about this for a bit too. I don't see any harm in it, and it will likely improve the usability (or at least remove a surprise for a more casual user). I couldn't quite think of a reasonable scenario when passing things out of order would be considered a typo-type bug (if I have understood your comment correctly on sage-devel).",
    "created_at": "2022-02-16T07:13:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542166",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
Replying to [@mantepse](#comment%3A2):
> Excellent, thank you!
> 
> I am split on the issue whether `SetPartitions` should sort its `part` argument, so let it be.

I thought about this for a bit too. I don't see any harm in it, and it will likely improve the usability (or at least remove a surprise for a more casual user). I couldn't quite think of a reasonable scenario when passing things out of order would be considered a typo-type bug (if I have understood your comment correctly on sage-devel).



---

archive/issue_comments_542167.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAlso, green bot (morally). Should I interpret [comment:2](#comment%3A2) as a positive review?",
    "created_at": "2022-02-16T07:13:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542167",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Also, green bot (morally). Should I interpret [comment:2](#comment%3A2) as a positive review?



---

archive/issue_events_440259.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-02-16T07:24:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440259"
}
```



---

archive/issue_events_440260.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-02-16T07:24:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440260"
}
```



---

archive/issue_comments_542168.json:
```json
{
    "body": "Reviewer: **Martin Rubey**",
    "created_at": "2022-02-16T07:24:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542168",
    "user": "https://github.com/mantepse"
}
```

Reviewer: **Martin Rubey**



---

archive/issue_comments_542169.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\n(I tested uniformity experimentally, just to make sure)",
    "created_at": "2022-02-16T07:25:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542169",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:6'>Comment 6:</a>
(I tested uniformity experimentally, just to make sure)



---

archive/issue_events_440261.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-02-16T07:25:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440261"
}
```



---

archive/issue_events_440262.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-02-16T07:25:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440262"
}
```



---

archive/issue_comments_542170.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nsorry, misclicked.  LGTM, you'd say :-)",
    "created_at": "2022-02-16T07:25:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542170",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:8'>Comment 8:</a>
sorry, misclicked.  LGTM, you'd say :-)



---

archive/issue_comments_542171.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nAh, okay. I was a bit confused if you had possibly instead meant \"let it be\" as \"don't change it.\" `:)` Thank you for the review!",
    "created_at": "2022-02-16T07:26:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542171",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:9'>Comment 9:</a>
Ah, okay. I was a bit confused if you had possibly instead meant "let it be" as "don't change it." `:)` Thank you for the review!



---

archive/issue_comments_542172.json:
```json
{
    "body": "Changed branch from **[public/combinat/random_set_partitions_block_sizes-33357](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/random_set_partitions_block_sizes-33357)** to **[b110cbb](https://github.com/sagemath/sagetrac-mirror/commit/b110cbb78051125549eef3e95da72cf2dddd27a5)**",
    "created_at": "2022-02-21T21:55:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33357#issuecomment-542172",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/combinat/random_set_partitions_block_sizes-33357](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/random_set_partitions_block_sizes-33357)** to **[b110cbb](https://github.com/sagemath/sagetrac-mirror/commit/b110cbb78051125549eef3e95da72cf2dddd27a5)**



---

archive/issue_events_440263.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-02-21T21:55:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440263"
}
```



---

archive/issue_events_440264.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7abda14ee4b9aa028cf34bc7997023b1284266da",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-02-21T21:55:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33357",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33357#event-440264"
}
```
