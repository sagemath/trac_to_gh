# Issue 33678: Make representation integers exportable and small improvements to backend ppl

archive/issues_033441.json:
```json
{
    "assignees": [],
    "body": "In `backend_ppl.py` we currently manually copy the integers representing the different representation types in `representation.py`. In this ticket we change this to an import. This is easier to read and less error prone.\n\nIn addition we outsource to abstract methods to create a ppl generator resp. constraint system.\n\nCC:  @tscrim @yuan-zhou @mkoeppe\n\nBranch/Commit: **[94bd1ee](https://github.com/sagemath/sagetrac-mirror/commit/94bd1ee515bed34817ba9a593eb1bc14dcb6e15b)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33678_\n\n",
    "closed_at": "2022-05-22T09:57:44Z",
    "created_at": "2022-04-11T08:53:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make representation integers exportable and small improvements to backend ppl",
    "type": "issue",
    "updated_at": "2022-05-22T09:57:44Z",
    "url": "https://github.com/sagemath/sage/issues/33678",
    "user": "https://github.com/kliem"
}
```
In `backend_ppl.py` we currently manually copy the integers representing the different representation types in `representation.py`. In this ticket we change this to an import. This is easier to read and less error prone.

In addition we outsource to abstract methods to create a ppl generator resp. constraint system.

CC:  @tscrim @yuan-zhou @mkoeppe

Branch/Commit: **[94bd1ee](https://github.com/sagemath/sagetrac-mirror/commit/94bd1ee515bed34817ba9a593eb1bc14dcb6e15b)**

Reviewer: **Travis Scrimshaw**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/33678_





---

archive/issue_events_443996.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2022-04-11T08:53:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-443996"
}
```



---

archive/issue_events_443997.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2022-04-11T08:53:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-443997"
}
```



---

archive/issue_events_443998.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2022-04-11T08:53:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-443998"
}
```



---

archive/issue_events_443999.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2022-04-11T08:53:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-443999"
}
```



---

archive/issue_comments_547503.json:
```json
{
    "body": "Branch: **[public/33678](https://github.com/sagemath/sagetrac-mirror/tree/public/33678)**",
    "created_at": "2022-04-11T09:12:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33678#issuecomment-547503",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/33678](https://github.com/sagemath/sagetrac-mirror/tree/public/33678)**



---

archive/issue_comments_547504.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6e8740ff4717ef85e2357f82cc73eb1114dbc81a\">6e8740f</a></td><td><code>use types directly from representation.py</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/94bd1ee515bed34817ba9a593eb1bc14dcb6e15b\">94bd1ee</a></td><td><code>outsource creating of ppl generator/constraint system</code></td></tr></table>\n",
    "created_at": "2022-04-11T09:12:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33678#issuecomment-547504",
    "user": "https://github.com/kliem"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6e8740ff4717ef85e2357f82cc73eb1114dbc81a">6e8740f</a></td><td><code>use types directly from representation.py</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/94bd1ee515bed34817ba9a593eb1bc14dcb6e15b">94bd1ee</a></td><td><code>outsource creating of ppl generator/constraint system</code></td></tr></table>




---

archive/issue_events_444000.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2022-04-11T09:12:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-444000"
}
```



---

archive/issue_comments_547505.json:
```json
{
    "body": "Commit: **[94bd1ee](https://github.com/sagemath/sagetrac-mirror/commit/94bd1ee515bed34817ba9a593eb1bc14dcb6e15b)**",
    "created_at": "2022-04-11T09:12:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33678#issuecomment-547505",
    "user": "https://github.com/kliem"
}
```

Commit: **[94bd1ee](https://github.com/sagemath/sagetrac-mirror/commit/94bd1ee515bed34817ba9a593eb1bc14dcb6e15b)**



---

archive/issue_events_444001.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-11T17:19:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-444001"
}
```



---

archive/issue_events_444002.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-11T17:19:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-444002"
}
```



---

archive/issue_comments_547506.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-04-11T22:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33678#issuecomment-547506",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_444003.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-04-11T22:55:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-444003"
}
```



---

archive/issue_events_444004.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-04-11T22:55:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-444004"
}
```



---

archive/issue_comments_547507.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nPersonally, I am not a fan of the `@staticmethod`, but since that is what is done for other conversion functions, let it be so.",
    "created_at": "2022-04-11T22:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33678#issuecomment-547507",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
Personally, I am not a fan of the `@staticmethod`, but since that is what is done for other conversion functions, let it be so.



---

archive/issue_comments_547508.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThank you.",
    "created_at": "2022-04-12T06:48:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33678#issuecomment-547508",
    "user": "https://github.com/kliem"
}
```

<a id='comment:4'>Comment 4:</a>
Thank you.



---

archive/issue_events_444005.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-05-22T09:57:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-444005"
}
```



---

archive/issue_events_444006.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cb9d6aa3a9a2540c19f88e0cdfe232de6eb6f335",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-05-22T09:57:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33678#event-444006"
}
```



---

archive/issue_comments_547509.json:
```json
{
    "body": "Changed branch from **[public/33678](https://github.com/sagemath/sagetrac-mirror/tree/public/33678)** to **[94bd1ee](https://github.com/sagemath/sagetrac-mirror/commit/94bd1ee515bed34817ba9a593eb1bc14dcb6e15b)**",
    "created_at": "2022-05-22T09:57:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33678#issuecomment-547509",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/33678](https://github.com/sagemath/sagetrac-mirror/tree/public/33678)** to **[94bd1ee](https://github.com/sagemath/sagetrac-mirror/commit/94bd1ee515bed34817ba9a593eb1bc14dcb6e15b)**
