# Issue 33959: some improvements in sandpile.py

archive/issues_033722.json:
```json
{
    "assignees": [],
    "body": "We try to avoid as much as possible operations like `[g.vertices().index(v) for v in g.vertices()]`.\n\nA serious improvement would be to avoid using `.vertices()` and to accept any ordering of the vertices, but this requires extra changes in graph methods such as `.laplacian_matrix()`.\n\nDepends on #33954\n\nCC:  @fchapoton\n\nBranch/Commit: **[ddda2f4](https://github.com/sagemath/sagetrac-mirror/commit/ddda2f46316b586aff6fdce5b4b43eaa56184e76)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33959_\n\n",
    "closed_at": "2022-06-12T12:16:43Z",
    "created_at": "2022-06-06T14:55:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "some improvements in sandpile.py",
    "type": "issue",
    "updated_at": "2022-06-12T12:16:43Z",
    "url": "https://github.com/sagemath/sage/issues/33959",
    "user": "https://github.com/dcoudert"
}
```
We try to avoid as much as possible operations like `[g.vertices().index(v) for v in g.vertices()]`.

A serious improvement would be to avoid using `.vertices()` and to accept any ordering of the vertices, but this requires extra changes in graph methods such as `.laplacian_matrix()`.

Depends on #33954

CC:  @fchapoton

Branch/Commit: **[ddda2f4](https://github.com/sagemath/sagetrac-mirror/commit/ddda2f46316b586aff6fdce5b4b43eaa56184e76)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/33959_





---

archive/issue_events_447082.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-06T14:55:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447082"
}
```



---

archive/issue_events_447083.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-06T14:55:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447083"
}
```



---

archive/issue_events_447084.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-06T14:55:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447084"
}
```



---

archive/issue_events_447085.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-06T14:55:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447085"
}
```



---

archive/issue_events_447086.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-06T14:57:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447086"
}
```



---

archive/issue_events_447087.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-06T14:57:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447087"
}
```



---

archive/issue_events_447088.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-06T14:57:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447088"
}
```



---

archive/issue_comments_552049.json:
```json
{
    "body": "Commit: **[905a6bb](https://github.com/sagemath/sagetrac-mirror/commit/905a6bb15b1e8e41b074e35658df397902bc9e75)**",
    "created_at": "2022-06-06T14:57:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33959#issuecomment-552049",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[905a6bb](https://github.com/sagemath/sagetrac-mirror/commit/905a6bb15b1e8e41b074e35658df397902bc9e75)**



---

archive/issue_comments_552050.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0adf8cb96be21d41da5dc02eedb61178319f1a4e\">0adf8cb</a></td><td><code>partial pep cleaning of sandpiles/sandpile</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eb8340ec7fdfb6a87c161c350a0eef372037fb79\">eb8340e</a></td><td><code>a few more changes in sandpile</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/905a6bb15b1e8e41b074e35658df397902bc9e75\">905a6bb</a></td><td><code>trac #33959: some care in sandpile.py</code></td></tr></table>\n",
    "created_at": "2022-06-06T14:57:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33959#issuecomment-552050",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0adf8cb96be21d41da5dc02eedb61178319f1a4e">0adf8cb</a></td><td><code>partial pep cleaning of sandpiles/sandpile</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eb8340ec7fdfb6a87c161c350a0eef372037fb79">eb8340e</a></td><td><code>a few more changes in sandpile</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/905a6bb15b1e8e41b074e35658df397902bc9e75">905a6bb</a></td><td><code>trac #33959: some care in sandpile.py</code></td></tr></table>




---

archive/issue_comments_552051.json:
```json
{
    "body": "Branch: **[public/sandpile/33959](https://github.com/sagemath/sagetrac-mirror/tree/public/sandpile/33959)**",
    "created_at": "2022-06-06T14:57:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33959#issuecomment-552051",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/sandpile/33959](https://github.com/sagemath/sagetrac-mirror/tree/public/sandpile/33959)**



---

archive/issue_comments_552052.json:
```json
{
    "body": "Changed commit from **[905a6bb](https://github.com/sagemath/sagetrac-mirror/commit/905a6bb15b1e8e41b074e35658df397902bc9e75)** to **[ddda2f4](https://github.com/sagemath/sagetrac-mirror/commit/ddda2f46316b586aff6fdce5b4b43eaa56184e76)**",
    "created_at": "2022-06-06T15:07:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33959#issuecomment-552052",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[905a6bb](https://github.com/sagemath/sagetrac-mirror/commit/905a6bb15b1e8e41b074e35658df397902bc9e75)** to **[ddda2f4](https://github.com/sagemath/sagetrac-mirror/commit/ddda2f46316b586aff6fdce5b4b43eaa56184e76)**



---

archive/issue_comments_552053.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ddda2f46316b586aff6fdce5b4b43eaa56184e76\">ddda2f4</a></td><td><code>trac #33959: another change</code></td></tr></table>\n",
    "created_at": "2022-06-06T15:07:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33959#issuecomment-552053",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ddda2f46316b586aff6fdce5b4b43eaa56184e76">ddda2f4</a></td><td><code>trac #33959: another change</code></td></tr></table>




---

archive/issue_comments_552054.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2022-06-06T19:23:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33959#issuecomment-552054",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_552055.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nok, ca me semble raisonnable",
    "created_at": "2022-06-06T19:23:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33959#issuecomment-552055",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

ok, ca me semble raisonnable



---

archive/issue_events_447089.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-06-06T19:23:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447089"
}
```



---

archive/issue_events_447090.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-06-06T19:23:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447090"
}
```



---

archive/issue_comments_552056.json:
```json
{
    "body": "Changed branch from **[public/sandpile/33959](https://github.com/sagemath/sagetrac-mirror/tree/public/sandpile/33959)** to **[ddda2f4](https://github.com/sagemath/sagetrac-mirror/commit/ddda2f46316b586aff6fdce5b4b43eaa56184e76)**",
    "created_at": "2022-06-12T12:16:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33959#issuecomment-552056",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/sandpile/33959](https://github.com/sagemath/sagetrac-mirror/tree/public/sandpile/33959)** to **[ddda2f4](https://github.com/sagemath/sagetrac-mirror/commit/ddda2f46316b586aff6fdce5b4b43eaa56184e76)**



---

archive/issue_events_447091.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-06-12T12:16:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447091"
}
```



---

archive/issue_events_447092.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "93619a48357eb1d35a2071cab3e372414aab0248",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-06-12T12:16:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33959",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33959#event-447092"
}
```
