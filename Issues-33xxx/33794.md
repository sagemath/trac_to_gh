# Issue 33794: modules/fp_graded/morphism.py test failure

archive/issues_033557.json:
```json
{
    "body": "Happened when doctesting with `--long --all` and 64 threads, both on x86_64 and on aarch64. Possibly related to #30680.\n\n```\nsage -t --long --random-seed=339659929997122211039564167058187187237 /nix/store/bfvc3kipmpmapbikj4zjlgdnglv5hrx2-sage-src-9.6.rc3/src/sage/modules/fp_graded/morphism.py\n**********************************************************************\nFile \"/nix/store/bfvc3kipmpmapbikj4zjlgdnglv5hrx2-sage-src-9.6.rc3/src/sage/modules/fp_graded/morphism.py\", line 1571, in sage.modules.fp_graded.morphism.FPModuleMorphism.image\nFailed example:\n    K.is_injective()  # long time\nException raised:\n    Traceback (most recent call last):\n      File \"/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/doctest/forker.py\", line 695, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/doctest/forker.py\", line 1101, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.modules.fp_graded.morphism.FPModuleMorphism.image[11]>\", line 1, in <module>\n        K.is_injective()  # long time\n      File \"/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/morphism.py\", line 122, in is_injective\n        finite_algebra = SteenrodAlgebra_generic(algebra.prime(), profile=self.profile())\n      File \"/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/morphism.py\", line 90, in profile\n        return enveloping_profile_elements(elements,\n      File \"/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/profile.py\", line 132, in enveloping_profile_elements\n        profiles = [profile_elt(x) for x in alist if x != 0]\n      File \"/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/profile.py\", line 132, in <listcomp>\n        profiles = [profile_elt(x) for x in alist if x != 0]\n      File \"/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/profile.py\", line 82, in profile_elt\n        minprofile = [max(0, n.exact_log(char) + 1) for n in elt]\n      File \"/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/profile.py\", line 82, in <listcomp>\n        minprofile = [max(0, n.exact_log(char) + 1) for n in elt]\n    AttributeError: 'int' object has no attribute 'exact_log'\n```\n\nCC:  @jhpalmieri @tscrim\n\nBranch/Commit: 0b4860129e0f686594667be1ad3a3e1073478a56\n\nReviewer: Travis Scrimshaw\n\nAuthor: John Palmieri\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/33794\n\n",
    "closed_at": "2022-05-22T16:32:00Z",
    "created_at": "2022-05-04T11:24:15Z",
    "labels": [
        "component: algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.7",
    "title": "modules/fp_graded/morphism.py test failure",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/33794",
    "user": "https://github.com/collares"
}
```
Happened when doctesting with `--long --all` and 64 threads, both on x86_64 and on aarch64. Possibly related to #30680.

```
sage -t --long --random-seed=339659929997122211039564167058187187237 /nix/store/bfvc3kipmpmapbikj4zjlgdnglv5hrx2-sage-src-9.6.rc3/src/sage/modules/fp_graded/morphism.py
**********************************************************************
File "/nix/store/bfvc3kipmpmapbikj4zjlgdnglv5hrx2-sage-src-9.6.rc3/src/sage/modules/fp_graded/morphism.py", line 1571, in sage.modules.fp_graded.morphism.FPModuleMorphism.image
Failed example:
    K.is_injective()  # long time
Exception raised:
    Traceback (most recent call last):
      File "/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/doctest/forker.py", line 695, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/doctest/forker.py", line 1101, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.modules.fp_graded.morphism.FPModuleMorphism.image[11]>", line 1, in <module>
        K.is_injective()  # long time
      File "/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/morphism.py", line 122, in is_injective
        finite_algebra = SteenrodAlgebra_generic(algebra.prime(), profile=self.profile())
      File "/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/morphism.py", line 90, in profile
        return enveloping_profile_elements(elements,
      File "/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/profile.py", line 132, in enveloping_profile_elements
        profiles = [profile_elt(x) for x in alist if x != 0]
      File "/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/profile.py", line 132, in <listcomp>
        profiles = [profile_elt(x) for x in alist if x != 0]
      File "/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/profile.py", line 82, in profile_elt
        minprofile = [max(0, n.exact_log(char) + 1) for n in elt]
      File "/nix/store/s6xgkv83xziazw85fvay1zqigfd972jb-python3-3.9.12-env/lib/python3.9/site-packages/sage/modules/fp_graded/steenrod/profile.py", line 82, in <listcomp>
        minprofile = [max(0, n.exact_log(char) + 1) for n in elt]
    AttributeError: 'int' object has no attribute 'exact_log'
```

CC:  @jhpalmieri @tscrim

Branch/Commit: 0b4860129e0f686594667be1ad3a3e1073478a56

Reviewer: Travis Scrimshaw

Author: John Palmieri

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/33794





---

archive/issue_comments_673939.json:
```json
{
    "body": "<a id='comment:1'></a>works fine for me on ubuntu 20.04:\n\n```\nDoctesting 1 file.\nsage -t --long --random-seed=339659929997122211039564167058187187237 src/sage/modules/fp_graded/morphism.py\n    [432 tests, 17.66 s]\n----------------------------------------------------------------------\nAll tests passed!\n----------------------------------------------------------------------\n```",
    "created_at": "2022-05-04T16:17:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673939",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'></a>works fine for me on ubuntu 20.04:

```
Doctesting 1 file.
sage -t --long --random-seed=339659929997122211039564167058187187237 src/sage/modules/fp_graded/morphism.py
    [432 tests, 17.66 s]
----------------------------------------------------------------------
All tests passed!
----------------------------------------------------------------------
```



---

archive/issue_comments_673940.json:
```json
{
    "body": "<a id='comment:2'></a>We can make this change, just to be on the safe side:\n\n```diff\ndiff --git a/src/sage/modules/fp_graded/steenrod/profile.py b/src/sage/modules/fp_graded/steenrod/profile.py\nindex 5dd91e1b26..0e30b4b413 100755\n--- a/src/sage/modules/fp_graded/steenrod/profile.py\n+++ b/src/sage/modules/fp_graded/steenrod/profile.py\n@@ -79,7 +79,7 @@ def profile_elt(elt, char=2):\n         pass\n \n     if char == 2:\n-        minprofile = [max(0, n.exact_log(char) + 1) for n in elt]\n+        minprofile = [max(0, ZZ(n).exact_log(char) + 1) for n in elt]\n         return find_min_profile(minprofile, char)\n \n     # odd primes:\n```\n`n` should already be a Sage integer, but it doesn't hurt to make sure.",
    "created_at": "2022-05-04T17:54:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673940",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>We can make this change, just to be on the safe side:

```diff
diff --git a/src/sage/modules/fp_graded/steenrod/profile.py b/src/sage/modules/fp_graded/steenrod/profile.py
index 5dd91e1b26..0e30b4b413 100755
--- a/src/sage/modules/fp_graded/steenrod/profile.py
+++ b/src/sage/modules/fp_graded/steenrod/profile.py
@@ -79,7 +79,7 @@ def profile_elt(elt, char=2):
         pass
 
     if char == 2:
-        minprofile = [max(0, n.exact_log(char) + 1) for n in elt]
+        minprofile = [max(0, ZZ(n).exact_log(char) + 1) for n in elt]
         return find_min_profile(minprofile, char)
 
     # odd primes:
```
`n` should already be a Sage integer, but it doesn't hurt to make sure.



---

archive/issue_comments_673941.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jhpalmieri/trac33794\"",
    "created_at": "2022-05-04T18:00:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673941",
    "user": "https://github.com/jhpalmieri"
}
```

Changing branch from "" to "u/jhpalmieri/trac33794"



---

archive/issue_comments_673942.json:
```json
{
    "body": "Changing commit from \"\" to \"0b4860129e0f686594667be1ad3a3e1073478a56\"",
    "created_at": "2022-05-04T19:37:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673942",
    "user": "https://github.com/jhpalmieri"
}
```

Changing commit from "" to "0b4860129e0f686594667be1ad3a3e1073478a56"



---

archive/issue_comments_673943.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2022-05-04T19:37:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673943",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_673944.json:
```json
{
    "body": "Changing author from \"\" to \"John Palmieri\"",
    "created_at": "2022-05-04T19:37:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673944",
    "user": "https://github.com/jhpalmieri"
}
```

Changing author from "" to "John Palmieri"



---

archive/issue_comments_673945.json:
```json
{
    "body": "<a id='comment:4'></a>New commits:",
    "created_at": "2022-05-04T19:37:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673945",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'></a>New commits:



---

archive/issue_comments_673946.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2022-05-05T00:01:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673946",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_673947.json:
```json
{
    "body": "<a id='comment:5'></a>My guess is that it is not machine dependent but order dependent on the doctests: Something is getting cached with an `int` Instead of an integer, which makes its way into that part of the code. While it doesn\u2019t hurt to do this, it might cover up a slightly deeper problem with not converting inputs to the proper parent (it might also be possible to make this, say, an element of `QQ` instead).\n\nAnyways, for now, green bot => positive review.",
    "created_at": "2022-05-05T00:01:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673947",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'></a>My guess is that it is not machine dependent but order dependent on the doctests: Something is getting cached with an `int` Instead of an integer, which makes its way into that part of the code. While it doesn’t hurt to do this, it might cover up a slightly deeper problem with not converting inputs to the proper parent (it might also be possible to make this, say, an element of `QQ` instead).

Anyways, for now, green bot => positive review.



---

archive/issue_comments_673948.json:
```json
{
    "body": "<a id='comment:6'></a>It might be a sign of a deeper problem, but without more information, for example a repeatable failure, I have no idea how to diagnose it.",
    "created_at": "2022-05-05T03:03:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673948",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:6'></a>It might be a sign of a deeper problem, but without more information, for example a repeatable failure, I have no idea how to diagnose it.



---

archive/issue_comments_673949.json:
```json
{
    "body": "<a id='comment:7'></a>The initial failure happened on NixOS CI on builder that has 64 cores. I reproduced this on the first try locally (on x86_64) by running `sage -t --long -p 64 --all`, even though my laptop only has 8 cores; 16GB RAM was enough to run 64 threads until the `morphism.py` failure.",
    "created_at": "2022-05-05T12:12:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673949",
    "user": "https://github.com/collares"
}
```

<a id='comment:7'></a>The initial failure happened on NixOS CI on builder that has 64 cores. I reproduced this on the first try locally (on x86_64) by running `sage -t --long -p 64 --all`, even though my laptop only has 8 cores; 16GB RAM was enough to run 64 threads until the `morphism.py` failure.



---

archive/issue_comments_673950.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Possibly related to #30680:\n+Happened when doctesting with `--long --all` and 64 threads, both on x86_64 and on aarch64. Possibly related to #30680.\n \n ```\n sage -t --long --random-seed=339659929997122211039564167058187187237 /nix/store/bfvc3kipmpmapbikj4zjlgdnglv5hrx2-sage-src-9.6.rc3/src/sage/modules/fp_graded/morphism.py\n``````\n",
    "created_at": "2022-05-05T12:12:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673950",
    "user": "https://github.com/collares"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Possibly related to #30680:
+Happened when doctesting with `--long --all` and 64 threads, both on x86_64 and on aarch64. Possibly related to #30680.
 
 ```
 sage -t --long --random-seed=339659929997122211039564167058187187237 /nix/store/bfvc3kipmpmapbikj4zjlgdnglv5hrx2-sage-src-9.6.rc3/src/sage/modules/fp_graded/morphism.py
``````




---

archive/issue_comments_673951.json:
```json
{
    "body": "<a id='comment:8'></a>Well, since I cannot reproduce the original error, I can only say that this definitely should fix it. Tests pass for me, so I am going to set a positive review.\n\ncomment:7: Strange. Perhaps it is related to the number of cores somehow? That would be surprising to me. Do you get the failure when running it more locally, say, on this file or with the folder specifically?",
    "created_at": "2022-05-06T01:19:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673951",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:8'></a>Well, since I cannot reproduce the original error, I can only say that this definitely should fix it. Tests pass for me, so I am going to set a positive review.

comment:7: Strange. Perhaps it is related to the number of cores somehow? That would be surprising to me. Do you get the failure when running it more locally, say, on this file or with the folder specifically?



---

archive/issue_comments_673952.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-05-06T01:19:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673952",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_673953.json:
```json
{
    "body": "<a id='comment:9'></a>comment:8: I unfortunately have no good way of reproducing this. Running the test suite just on the `modules` directory is not enough to reproduce it. I don't know if this is to be trusted, since I don't believe the bug is 100% deterministic.\n\nI noticed something while browsing the code: `modules/fp_graded/module.py:vector_presentation` and `modules/fp_graded/free_module.py:vector_presentation` both have the ``@`cached_method` decorator. This is not concerning per se, but in both cases there's `__getitem__ = vector_presentation` line in the corresponding files. Python does lookups differently for special methods, and `misc/cachefunc.pyx` has to handle them separately because of this (https://github.com/sagemath/sagetrac-mirror/blob/master/src/sage/misc/cachefunc.pyx?h=develop&id=0b4860129e0f686594667be1ad3a3e1073478a56#n3092); I worry that the `__getitem__ = vector_presentation` line bypasses the check done by the decorator. I know nothing about the surrounding code, so I don't know how plausible the above theory is. In any case, I will try removing the ``@`cached_method` decorator for those two methods and see if it helps on NixOS CI.\n\n(A similar problem might affect the line `__bool__ = is_zero` in `modules/fp_graded/morphism.py`, but this shouldn't be relevant for the test failure because this `__bool__` instance is unused in the testsuite.)",
    "created_at": "2022-05-09T15:36:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673953",
    "user": "https://github.com/collares"
}
```

<a id='comment:9'></a>comment:8: I unfortunately have no good way of reproducing this. Running the test suite just on the `modules` directory is not enough to reproduce it. I don't know if this is to be trusted, since I don't believe the bug is 100% deterministic.

I noticed something while browsing the code: `modules/fp_graded/module.py:vector_presentation` and `modules/fp_graded/free_module.py:vector_presentation` both have the ``@`cached_method` decorator. This is not concerning per se, but in both cases there's `__getitem__ = vector_presentation` line in the corresponding files. Python does lookups differently for special methods, and `misc/cachefunc.pyx` has to handle them separately because of this (https://github.com/sagemath/sagetrac-mirror/blob/master/src/sage/misc/cachefunc.pyx?h=develop&id=0b4860129e0f686594667be1ad3a3e1073478a56#n3092); I worry that the `__getitem__ = vector_presentation` line bypasses the check done by the decorator. I know nothing about the surrounding code, so I don't know how plausible the above theory is. In any case, I will try removing the ``@`cached_method` decorator for those two methods and see if it helps on NixOS CI.

(A similar problem might affect the line `__bool__ = is_zero` in `modules/fp_graded/morphism.py`, but this shouldn't be relevant for the test failure because this `__bool__` instance is unused in the testsuite.)



---

archive/issue_comments_673954.json:
```json
{
    "body": "<a id='comment:10'></a>For what it's worth, `__getitem__` barely gets used. I changed the line `__getitem__ = vector_presentation` to\n\n```python\n    def __getitem__(self, n, verbose=False):\n        print('hello')\n        return self.vector_presentation(n, verbose)\n```\nand there was only one doctest failure when I tested `modules/fp_graded/`:\n\n```\nFile \"src/sage/modules/fp_graded/module.py\", line 826, in sage.modules.fp_graded.module.FPModule.vector_presentation\nFailed example:\n    M[4].dimension()\nExpected:\n    3\nGot:\n    hello\n    3\n```",
    "created_at": "2022-05-09T15:53:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673954",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:10'></a>For what it's worth, `__getitem__` barely gets used. I changed the line `__getitem__ = vector_presentation` to

```python
    def __getitem__(self, n, verbose=False):
        print('hello')
        return self.vector_presentation(n, verbose)
```
and there was only one doctest failure when I tested `modules/fp_graded/`:

```
File "src/sage/modules/fp_graded/module.py", line 826, in sage.modules.fp_graded.module.FPModule.vector_presentation
Failed example:
    M[4].dimension()
Expected:
    3
Got:
    hello
    3
```



---

archive/issue_comments_673955.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [gh-collares](#comment%3A9):\n> comment:8: I unfortunately have no good way of reproducing this. Running the test suite just on the `modules` directory is not enough to reproduce it. I don't know if this is to be trusted, since I don't believe the bug is 100% deterministic.\n\n\nThat makes it seem like it is some kind of caching issue with an `int` getting leaked in somehow through some other code path. Anyways, not serious enough to warrant a deep dive into the code to find it IMO.\n\n>  Python does lookups differently for special methods, and `misc/cachefunc.pyx` has to handle them separately because of this (https://github.com/sagemath/sagetrac-mirror/blob/master/src/sage/misc/cachefunc.pyx?h=develop&id=0b4860129e0f686594667be1ad3a3e1073478a56#n3092); I worry that the `__getitem__ = vector_presentation` line bypasses the check done by the decorator.\n\n\nI don\u2019t think it bypasses it, but this is easy to verify similar to John\u2019s test in[comment:10](#comment%3A10).\n\n> I know nothing about the surrounding code, so I don't know how plausible the above theory is. In any case, I will try removing the ``@`cached_method` decorator for those two methods and see if it helps on NixOS CI.\n\n\nI don\u2019t think that is relevant to the bugs at hand as I cannot see any relation. There could potentially be a separate issue with the caching not being done as it should, but that is a performance issue and not a true bug IMO.",
    "created_at": "2022-05-10T02:44:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673955",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:11'></a>Replying to [gh-collares](#comment%3A9):
> comment:8: I unfortunately have no good way of reproducing this. Running the test suite just on the `modules` directory is not enough to reproduce it. I don't know if this is to be trusted, since I don't believe the bug is 100% deterministic.


That makes it seem like it is some kind of caching issue with an `int` getting leaked in somehow through some other code path. Anyways, not serious enough to warrant a deep dive into the code to find it IMO.

>  Python does lookups differently for special methods, and `misc/cachefunc.pyx` has to handle them separately because of this (https://github.com/sagemath/sagetrac-mirror/blob/master/src/sage/misc/cachefunc.pyx?h=develop&id=0b4860129e0f686594667be1ad3a3e1073478a56#n3092); I worry that the `__getitem__ = vector_presentation` line bypasses the check done by the decorator.


I don’t think it bypasses it, but this is easy to verify similar to John’s test in[comment:10](#comment%3A10).

> I know nothing about the surrounding code, so I don't know how plausible the above theory is. In any case, I will try removing the ``@`cached_method` decorator for those two methods and see if it helps on NixOS CI.


I don’t think that is relevant to the bugs at hand as I cannot see any relation. There could potentially be a separate issue with the caching not being done as it should, but that is a performance issue and not a true bug IMO.



---

archive/issue_comments_673956.json:
```json
{
    "body": "<a id='comment:12'></a>You two are absolutely right, of course! Thanks for the explanation. Besides, making such a specific guess without having a way to reproduce the failure was clearly a bad idea. \n\nFortunately, I now have a way to do reproduce the bug! Applying the following patch, the bug shows up every time when running `sage -t --long src/sage/modules/fp_graded/morphism.py`:\n\n```diff\ndiff --git a/src/sage/modules/fp_graded/steenrod/morphism.py b/src/sage/modules/fp_graded/steenrod/morphism.py\nindex d55b51faf4..b0cf86c587 100755\n--- a/src/sage/modules/fp_graded/steenrod/morphism.py\n+++ b/src/sage/modules/fp_graded/steenrod/morphism.py\n@@ -82,11 +82,16 @@ class SteenrodFPModuleMorphism(FPModuleMorphism):\n         def _flatten(f):\n             return [c for value in f for c in value.dense_coefficient_list()]\n \n+        import gc\n+        n = gc.collect()\n+\n         elements = (_flatten(self.domain().relations())\n                     + _flatten(self.codomain().relations())\n                     + _flatten(self.values()))\n         elements = [a for a in elements if a not in (0, 1)]\n \n         return enveloping_profile_elements(elements,\n                                            char=self.base_ring().characteristic())\n \ndiff --git a/src/sage/structure/unique_representation.py b/src/sage/structure/unique_representation.py\nindex 5d8d4ad758..4afccaec88 100644\n--- a/src/sage/structure/unique_representation.py\n+++ b/src/sage/structure/unique_representation.py\n@@ -989,7 +989,7 @@ class CachedRepresentation(metaclass=ClasscallMetaclass):\n     :meth:`__init__<object.__init__>`.\n     \"\"\"\n \n-    @weak_cached_function(cache=128)  # automatically a staticmethod\n+    @weak_cached_function(cache=0)  # automatically a staticmethod\n     def __classcall__(cls, *args, **options):\n         \"\"\"\n         Construct a new object of this class or reuse an existing one.\n\n```",
    "created_at": "2022-05-17T03:20:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673956",
    "user": "https://github.com/collares"
}
```

<a id='comment:12'></a>You two are absolutely right, of course! Thanks for the explanation. Besides, making such a specific guess without having a way to reproduce the failure was clearly a bad idea. 

Fortunately, I now have a way to do reproduce the bug! Applying the following patch, the bug shows up every time when running `sage -t --long src/sage/modules/fp_graded/morphism.py`:

```diff
diff --git a/src/sage/modules/fp_graded/steenrod/morphism.py b/src/sage/modules/fp_graded/steenrod/morphism.py
index d55b51faf4..b0cf86c587 100755
--- a/src/sage/modules/fp_graded/steenrod/morphism.py
+++ b/src/sage/modules/fp_graded/steenrod/morphism.py
@@ -82,11 +82,16 @@ class SteenrodFPModuleMorphism(FPModuleMorphism):
         def _flatten(f):
             return [c for value in f for c in value.dense_coefficient_list()]
 
+        import gc
+        n = gc.collect()
+
         elements = (_flatten(self.domain().relations())
                     + _flatten(self.codomain().relations())
                     + _flatten(self.values()))
         elements = [a for a in elements if a not in (0, 1)]
 
         return enveloping_profile_elements(elements,
                                            char=self.base_ring().characteristic())
 
diff --git a/src/sage/structure/unique_representation.py b/src/sage/structure/unique_representation.py
index 5d8d4ad758..4afccaec88 100644
--- a/src/sage/structure/unique_representation.py
+++ b/src/sage/structure/unique_representation.py
@@ -989,7 +989,7 @@ class CachedRepresentation(metaclass=ClasscallMetaclass):
     :meth:`__init__<object.__init__>`.
     """
 
-    @weak_cached_function(cache=128)  # automatically a staticmethod
+    @weak_cached_function(cache=0)  # automatically a staticmethod
     def __classcall__(cls, *args, **options):
         """
         Construct a new object of this class or reuse an existing one.

```



---

archive/issue_comments_673957.json:
```json
{
    "body": "Changing branch from \"u/jhpalmieri/trac33794\" to \"0b4860129e0f686594667be1ad3a3e1073478a56\"",
    "created_at": "2022-05-22T16:32:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673957",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/jhpalmieri/trac33794" to "0b4860129e0f686594667be1ad3a3e1073478a56"



---

archive/issue_events_088773.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-05-22T16:32:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33794#event-88773"
}
```



---

archive/issue_comments_673958.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2022-05-22T16:32:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33794#issuecomment-673958",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
