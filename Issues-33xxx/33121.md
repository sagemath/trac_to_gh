...
<a id='comment:2'></a>Discrete log also seems to be faster with PARI/GP (x3 on this example)
...
```json
{
    "body": "Changing commit from \"\" to \"208b2c073beb5669478e94f2581e61560f145c18\"",
...
Changing commit from "" to "208b2c073beb5669478e94f2581e61560f145c18"
...
<a id='comment:3'></a>Alright, here's a branch. Its primary goal was to call `elllog` in `.discrete_log()`, but there's a little complication: PARI doesn't actually guarantee that `elllog` terminates when `Q` is not a multiple of `P`.
Hence, I added a Weil pairing check to ensure this before calling into PARI, and changed `.weil_pairing()` to use PARI as well. (The old implementation remains accessible via an optional `algorithm=` argument.)
...
```json
{
    "body": "Changing branch from \"\" to \"public/use_pari_elllog_and_ellweilpairing\"",
    "created_at": "2022-01-10T11:22:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33121",
    "type": "issue_comment",
...
Changing branch from "" to "public/use_pari_elllog_and_ellweilpairing"
...
<a id='comment:5'></a>In the `znlog()` documentation, near the bottom, it says:
...
```json
{
    "body": "<a id='comment:6'></a>Does the Weil pairing guarantees colinearity or the fact that Q is a multiple of P ? (not very familiar with EC myself)",
...
<a id='comment:6'></a>Does the Weil pairing guarantees colinearity or the fact that Q is a multiple of P ? (not very familiar with EC myself)
...
```json
{
    "body": "<a id='comment:7'></a>More precisely, there could be a denominator issue. The two following questions are different : \n`Q` in `ZZ P` versus `Q` in `K P` (where `K` is the ground field).",
...
<a id='comment:7'></a>More precisely, there could be a denominator issue. The two following questions are different : 
`Q` in `ZZ P` versus `Q` in `K P` (where `K` is the ground field).
...
<a id='comment:8'></a>You are right that "`Q in ℚ·P`" can be an issue, but in this case it isn't.
...
<a id='comment:9'></a>Replying to [lorenz](#comment%3A3):
> While benchmarking, I also noticed that scalar multiplications in Sage are significantly (5×?) slower than PARI's `ellmul`, but changing that is a little more intricate, hence left for future work.
...
```json
{
    ...
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33121#event-87512"
}
```
...
```json
{
    "body": "Changing commit from \"208b2c073beb5669478e94f2581e61560f145c18\" to \"c79b74a34ef9b3c48431421efc5f1b872c3f44c2\"",
    "created_at": "2022-01-30T11:50:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33121",
    "type": "issue_comment",
...
Changing commit from "208b2c073beb5669478e94f2581e61560f145c18" to "c79b74a34ef9b3c48431421efc5f1b872c3f44c2"
...
```json
{
    "body": "<a id='comment:13'></a>Thanks, good catch. PARI doesn't seem to like pairings over infinite fields.",
...
<a id='comment:13'></a>Thanks, good catch. PARI doesn't seem to like pairings over infinite fields.
...
```json
{
    "body": "<a id='comment:14'></a>According to the patchbot (and sage documentation) `Returns` should be `Return` in the docstring of `discrete_log`.",
...
<a id='comment:14'></a>According to the patchbot (and sage documentation) `Returns` should be `Return` in the docstring of `discrete_log`.
...
```json
{
    "body": "Changing commit from \"c79b74a34ef9b3c48431421efc5f1b872c3f44c2\" to \"c7626f6d209c4161c2c0abb05364cf4d9ba68c6c\"",
...
Changing commit from "c79b74a34ef9b3c48431421efc5f1b872c3f44c2" to "c7626f6d209c4161c2c0abb05364cf4d9ba68c6c"
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/33121",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33121#event-87513"
}
```
...
```json
{
    "body": "Changing branch from \"public/use_pari_elllog_and_ellweilpairing\" to \"c7626f6d209c4161c2c0abb05364cf4d9ba68c6c\"",
    "created_at": "2022-02-13T10:17:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33121",
    "type": "issue_comment",
...
Changing branch from "public/use_pari_elllog_and_ellweilpairing" to "c7626f6d209c4161c2c0abb05364cf4d9ba68c6c"
