# Issue 33439: archlinux-minimal: scipy build fails

archive/issues_033202.json:
```json
{
    "body": "\n```\n  /sage/local/bin/gfortran -Wall -g -Wall -g -shared -shared -Wl,-rpath-link,/sage/local/lib -L/sage/local/lib -Wl,-rpath,/sage/local/lib -Wl,-rpath-link,/sage/local/lib -L/sage/local/lib -Wl,-rpath,/sage/local/lib build/temp.linux-x86_64-3.10/scipy/integrate/_quadpackmodule.o -L/sage/local/lib -L/sage/local/lib/../lib -L/sage/local/lib/../lib -L/usr/lib -Lbuild/temp.linux-x86_64-3.10 -lquadpack -lmach -lopenblas -lopenblas -lgfortran -o build/lib.linux-x86_64-3.10/scipy/integrate/_quadpack.cpython-310-x86_64-linux-gnu.so -Wl,--version-script=build/temp.linux-x86_64-3.10/link-version-scipy.integrate._quadpack.map\n  lto1: fatal error: bytecode stream in file 'build/temp.linux-x86_64-3.10/scipy/integrate/_quadpackmodule.o' generated with LTO version 11.0 instead of the expected 9.3\n  compilation terminated.\n  lto-wrapper: fatal error: /sage/local/bin/gfortran returned 1 exit status\n  compilation terminated.\n  /usr/sbin/ld: error: lto-wrapper failed\n  collect2: error: ld returned 1 exit status\n```\nhttps://github.com/sagemath/sage/runs/5353145933\n\n\nCC:  @antonio-rojas\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/33439\n\n",
    "created_at": "2022-03-01T18:57:23Z",
    "labels": [
        "component: packages: standard",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "archlinux-minimal: scipy build fails",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/33439",
    "user": "https://github.com/mkoeppe"
}
```

```
  /sage/local/bin/gfortran -Wall -g -Wall -g -shared -shared -Wl,-rpath-link,/sage/local/lib -L/sage/local/lib -Wl,-rpath,/sage/local/lib -Wl,-rpath-link,/sage/local/lib -L/sage/local/lib -Wl,-rpath,/sage/local/lib build/temp.linux-x86_64-3.10/scipy/integrate/_quadpackmodule.o -L/sage/local/lib -L/sage/local/lib/../lib -L/sage/local/lib/../lib -L/usr/lib -Lbuild/temp.linux-x86_64-3.10 -lquadpack -lmach -lopenblas -lopenblas -lgfortran -o build/lib.linux-x86_64-3.10/scipy/integrate/_quadpack.cpython-310-x86_64-linux-gnu.so -Wl,--version-script=build/temp.linux-x86_64-3.10/link-version-scipy.integrate._quadpack.map
  lto1: fatal error: bytecode stream in file 'build/temp.linux-x86_64-3.10/scipy/integrate/_quadpackmodule.o' generated with LTO version 11.0 instead of the expected 9.3
  compilation terminated.
  lto-wrapper: fatal error: /sage/local/bin/gfortran returned 1 exit status
  compilation terminated.
  /usr/sbin/ld: error: lto-wrapper failed
  collect2: error: ld returned 1 exit status
```
https://github.com/sagemath/sage/runs/5353145933


CC:  @antonio-rojas

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/33439





---

archive/issue_comments_666657.json:
```json
{
    "body": "<a id='comment:1'></a>I certainly wouldn't expect mismatched versions of gcc and gfortran to work together. They should both be taken from the system or both compiled from source.",
    "created_at": "2022-03-01T19:19:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33439#issuecomment-666657",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:1'></a>I certainly wouldn't expect mismatched versions of gcc and gfortran to work together. They should both be taken from the system or both compiled from source.



---

archive/issue_events_088146.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/33439",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33439#event-88146"
}
```



---

archive/issue_events_088147.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/33439",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33439#event-88147"
}
```



---

archive/issue_events_088148.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/33439",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33439#event-88148"
}
```
