# Issue 33858: Further parallelization  for fundamental group of curves.

archive/issues_033621.json:
```json
{
    "assignees": [],
    "body": "The computation of the fundamental group of algebraic plane curves is done by computing the braid monodromy (which already takes advantage of parallelization), and then computing the action of the braids over the free group.\n\nThis second step can take non negligible time in some cases. With this simple change, it is computed in parallel too, which can save some time.\n\nCC:  @fchapoton @tscrim\n\nBranch/Commit: **[8953d53](https://github.com/sagemath/sagetrac-mirror/commit/8953d5397487655156f83598dc1e0bc3930992d4)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Miguel Marco**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33858_\n\n",
    "closed_at": "2022-05-24T22:44:53Z",
    "created_at": "2022-05-16T21:24:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Further parallelization  for fundamental group of curves.",
    "type": "issue",
    "updated_at": "2022-05-24T22:44:53Z",
    "url": "https://github.com/sagemath/sage/issues/33858",
    "user": "https://github.com/miguelmarco"
}
```
The computation of the fundamental group of algebraic plane curves is done by computing the braid monodromy (which already takes advantage of parallelization), and then computing the action of the braids over the free group.

This second step can take non negligible time in some cases. With this simple change, it is computed in parallel too, which can save some time.

CC:  @fchapoton @tscrim

Branch/Commit: **[8953d53](https://github.com/sagemath/sagetrac-mirror/commit/8953d5397487655156f83598dc1e0bc3930992d4)**

Reviewer: **Travis Scrimshaw**

Author: **Miguel Marco**

_Issue created by migration from https://trac.sagemath.org/ticket/33858_





---

archive/issue_events_446034.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2022-05-16T21:24:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33858#event-446034"
}
```



---

archive/issue_events_446035.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2022-05-16T21:24:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33858#event-446035"
}
```



---

archive/issue_events_446036.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2022-05-16T21:24:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33858#event-446036"
}
```



---

archive/issue_events_446037.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2022-05-16T21:24:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33858#event-446037"
}
```



---

archive/issue_comments_550727.json:
```json
{
    "body": "Branch: **[u/mmarco/further_parallelization__for_fundamental_group_of_curves_](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/further_parallelization__for_fundamental_group_of_curves_)**",
    "created_at": "2022-05-16T21:29:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33858#issuecomment-550727",
    "user": "https://github.com/miguelmarco"
}
```

Branch: **[u/mmarco/further_parallelization__for_fundamental_group_of_curves_](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/further_parallelization__for_fundamental_group_of_curves_)**



---

archive/issue_comments_550728.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8953d5397487655156f83598dc1e0bc3930992d4\">8953d53</a></td><td><code>Parallelize action of braids on free group gens</code></td></tr></table>\n",
    "created_at": "2022-05-16T21:29:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33858#issuecomment-550728",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8953d5397487655156f83598dc1e0bc3930992d4">8953d53</a></td><td><code>Parallelize action of braids on free group gens</code></td></tr></table>




---

archive/issue_events_446038.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2022-05-16T21:29:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33858#event-446038"
}
```



---

archive/issue_comments_550729.json:
```json
{
    "body": "Commit: **[8953d53](https://github.com/sagemath/sagetrac-mirror/commit/8953d5397487655156f83598dc1e0bc3930992d4)**",
    "created_at": "2022-05-16T21:29:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33858#issuecomment-550729",
    "user": "https://github.com/miguelmarco"
}
```

Commit: **[8953d53](https://github.com/sagemath/sagetrac-mirror/commit/8953d5397487655156f83598dc1e0bc3930992d4)**



---

archive/issue_events_446039.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-05-16T22:06:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33858#event-446039"
}
```



---

archive/issue_events_446040.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-05-16T22:06:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33858#event-446040"
}
```



---

archive/issue_comments_550730.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2022-05-16T22:06:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33858#issuecomment-550730",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_550731.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-05-16T22:06:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33858#issuecomment-550731",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_550732.json:
```json
{
    "body": "Changed branch from **[u/mmarco/further_parallelization__for_fundamental_group_of_curves_](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/further_parallelization__for_fundamental_group_of_curves_)** to **[8953d53](https://github.com/sagemath/sagetrac-mirror/commit/8953d5397487655156f83598dc1e0bc3930992d4)**",
    "created_at": "2022-05-24T22:44:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33858#issuecomment-550732",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmarco/further_parallelization__for_fundamental_group_of_curves_](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/further_parallelization__for_fundamental_group_of_curves_)** to **[8953d53](https://github.com/sagemath/sagetrac-mirror/commit/8953d5397487655156f83598dc1e0bc3930992d4)**



---

archive/issue_events_446041.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-05-24T22:44:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33858#event-446041"
}
```



---

archive/issue_events_446042.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7de926230ca0b48ebd022b7019e7946794ba5934",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-05-24T22:44:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33858",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33858#event-446042"
}
```
