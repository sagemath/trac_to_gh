# Issue 33848: System GIAC kills process 1 in ctrl_c_signal_handler

archive/issues_033611.json:
```json
{
    "body": "The bug fixed in #33706 by patching our GIAC package still happens in 9.6.rc3 on systems where the system giac is used: `ubuntu-jammy-standard` (https://github.com/sagemath/sage/runs/6236167662, 1.7.0.39+dfsg2-1build2) and `debian-sid-standard`.\n\n\nCC:  parisse tmonteil @kiwifb\n\nUpstream: Reported upstream. No feedback yet.\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/33848\n\n",
    "created_at": "2022-05-13T15:39:52Z",
    "labels": [
        "component: packages: standard",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "System GIAC kills process 1 in ctrl_c_signal_handler",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/33848",
    "user": "https://github.com/mkoeppe"
}
```
The bug fixed in #33706 by patching our GIAC package still happens in 9.6.rc3 on systems where the system giac is used: `ubuntu-jammy-standard` (https://github.com/sagemath/sage/runs/6236167662, 1.7.0.39+dfsg2-1build2) and `debian-sid-standard`.


CC:  parisse tmonteil @kiwifb

Upstream: Reported upstream. No feedback yet.

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/33848





---

archive/issue_events_088838.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/33848",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33848#event-88838"
}
```



---

archive/issue_comments_477891.json:
```json
{
    "body": "<a id='comment:3'></a>`@`parisse, could you take a look whether our patch https://github.com/sagemath/sage/blob/develop/build/pkgs/giac/patches/0001-src-global.cc-Do-not-send-SIGINT-to-process-1.patch would be acceptable to merge in giac?",
    "created_at": "2022-11-24T20:21:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33848#issuecomment-477891",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'></a>`@`parisse, could you take a look whether our patch https://github.com/sagemath/sage/blob/develop/build/pkgs/giac/patches/0001-src-global.cc-Do-not-send-SIGINT-to-process-1.patch would be acceptable to merge in giac?
