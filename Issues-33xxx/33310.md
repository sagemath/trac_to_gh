# Issue 33310: Add support to save RealBalls

archive/issues_033073.json:
```json
{
    "assignees": [],
    "body": "Saving RealBalls (and analogously ComplexBalls) is currently not supported (tested on SageMath/9.4):\n\n```\nsage: tmp = RBF(pi)\nsage: save(tmp,\"test\")\n...\nAttributeError: 'sage.rings.real_arb.RealBall' object has no attribute '__dict__'\n```\nOne current workaround (though not very satisfying) is to convert the elements to RealIntervalFields before saving them.\n\nKeywords: **arb, RealBallField**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33310_\n\n",
    "closed_at": "2022-04-19T08:47:59Z",
    "created_at": "2022-02-07T10:48:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20pickling",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add support to save RealBalls",
    "type": "issue",
    "updated_at": "2022-04-19T08:47:59Z",
    "url": "https://github.com/sagemath/sage/issues/33310",
    "user": "https://github.com/David-Berghaus"
}
```
Saving RealBalls (and analogously ComplexBalls) is currently not supported (tested on SageMath/9.4):

```
sage: tmp = RBF(pi)
sage: save(tmp,"test")
...
AttributeError: 'sage.rings.real_arb.RealBall' object has no attribute '__dict__'
```
One current workaround (though not very satisfying) is to convert the elements to RealIntervalFields before saving them.

Keywords: **arb, RealBallField**

_Issue created by migration from https://trac.sagemath.org/ticket/33310_





---

archive/issue_events_439755.json:
```json
{
    "actor": "https://github.com/David-Berghaus",
    "created_at": "2022-02-07T10:48:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439755"
}
```



---

archive/issue_events_439756.json:
```json
{
    "actor": "https://github.com/David-Berghaus",
    "created_at": "2022-02-07T10:48:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20pickling",
    "label_color": "000080",
    "label_name": "component: pickling",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439756"
}
```



---

archive/issue_events_439757.json:
```json
{
    "actor": "https://github.com/David-Berghaus",
    "created_at": "2022-02-07T10:48:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439757"
}
```



---

archive/issue_events_439758.json:
```json
{
    "actor": "https://github.com/David-Berghaus",
    "created_at": "2022-02-07T10:48:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439758"
}
```



---

archive/issue_events_439759.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-17T19:59:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439759"
}
```



---

archive/issue_events_439760.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-17T19:59:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439760"
}
```



---

archive/issue_events_439761.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2022-04-19T06:59:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439761"
}
```



---

archive/issue_comments_541472.json:
```json
{
    "body": "Replying to [ticket:33310 gh-David-Berghaus]:\n> Saving RealBalls (and analogously ComplexBalls) is currently not supported (tested on SageMath/9.4):\n\nYes it is, since #32990.",
    "created_at": "2022-04-19T06:59:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33310#issuecomment-541472",
    "user": "https://github.com/mezzarobba"
}
```

Replying to [ticket:33310 gh-David-Berghaus]:
> Saving RealBalls (and analogously ComplexBalls) is currently not supported (tested on SageMath/9.4):

Yes it is, since #32990.



---

archive/issue_events_439762.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2022-04-19T06:59:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439762"
}
```



---

archive/issue_events_439763.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2022-04-19T06:59:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439763"
}
```



---

archive/issue_events_439764.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2022-04-19T06:59:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439764"
}
```



---

archive/issue_events_439765.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-04-19T08:47:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439765"
}
```



---

archive/issue_events_439766.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-04-19T08:47:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33310",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33310#event-439766"
}
```
