...
```json
{
    "body": "I think if we include #33531 it can join the list of less urgent stuff to fix about `pytest`.",
...
```json
{
    "body": "I just realised it may make more sense to just disable `pytest` for `sage -t --installed`. Running `pytest` relies on the presence of `src/conftest.py` which is not installed (not sure it would make sense to install it). So, a system install of sage would run `pytest` without fixtures and ignore list - unless you manually point to a copy.",
...
```json
{
    "body": "I'm not sure about the exact use case of `--installed` but if you completely disable pytest in this situation then users no longer run the existing pytests (like the ones for the symplectic form).",
...
```json
{
    "body": "`--installed` is for running against a sage install as opposed to running on the source. pytest default on running on source. As for your latest suggestion, in the normal use case for this ticket, `SAGE_SRC` is usually pointing to `SAGE_LIB` and `tox.ini` may not be available.",
...
```json
{
    "body": "Setting #31924 as a dependency because it modifies `src/bin/sage-runtests` in the same place that needs modifying here.",
...
```json
{
    "body": "Need to get on with that ticket. Not sure how long it will take me to get to it, I am trying to diagnose flaky RAM on my main dev machine. Compilations breaking in random places is a good signal of issue.",
...
```json
{
    "body": "There are two major use cases for `--installed`: In downstream distributions and for testing modularized distribution packages. For the latter, an example is #32601. In this context, there is no problem with providing the pytest configuration, as the source is available.",
...
```json
{
    "body": "Here's an attempt at a solution following comment:5\n\n---\nNew commits:",
    "created_at": "2022-03-25T18:46:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33521",
    "type": "issue_comment",
...
```json
{
    "body": "I have been busy the last couple of weeks and completely forgotten about this ticket. I will try to do a proper review later today.",
...
```json
{
    "body": "It works as expected for me in sage-on-gentoo. pytest are not run on system install lacking the right configuration files anymore.",
