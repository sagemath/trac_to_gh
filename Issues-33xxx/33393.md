# Issue 33393: Implement Krawtchouk, Meixner, and Hahn polynomials

archive/issues_033156.json:
```json
{
    "assignees": [],
    "body": "These are three classes of orthogonal polynomials in\nthe [Askey scheme](https://en.wikipedia.org/wiki/Askey_scheme).\nWe should provide an implementation based on\ntheir definition as hypergeometric functions.\n\nThis ticket provides just a basic implementation with\nno particular reason for the normalization choices.\n\nCC:  @dimpase @sagetrac-tmonteil @ferihr @fchapoton @mezzarobba @slel\n\nKeywords: **orthogonal polynomials**\n\nBranch/Commit: **[9b40ec4](https://github.com/sagemath/sagetrac-mirror/commit/9b40ec430c42a89dcd5b24ac35e2e3eb7d3d411a)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33393_\n\n",
    "closed_at": "2022-03-30T22:33:09Z",
    "created_at": "2022-02-21T08:33:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement Krawtchouk, Meixner, and Hahn polynomials",
    "type": "issue",
    "updated_at": "2022-03-30T22:33:09Z",
    "url": "https://github.com/sagemath/sage/issues/33393",
    "user": "https://github.com/tscrim"
}
```
These are three classes of orthogonal polynomials in
the [Askey scheme](https://en.wikipedia.org/wiki/Askey_scheme).
We should provide an implementation based on
their definition as hypergeometric functions.

This ticket provides just a basic implementation with
no particular reason for the normalization choices.

CC:  @dimpase @sagetrac-tmonteil @ferihr @fchapoton @mezzarobba @slel

Keywords: **orthogonal polynomials**

Branch/Commit: **[9b40ec4](https://github.com/sagemath/sagetrac-mirror/commit/9b40ec430c42a89dcd5b24ac35e2e3eb7d3d411a)**

Reviewer: **Frédéric Chapoton**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/33393_





---

archive/issue_events_440655.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-21T08:33:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440655"
}
```



---

archive/issue_events_440656.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-21T08:33:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440656"
}
```



---

archive/issue_events_440657.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-21T08:33:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440657"
}
```



---

archive/issue_events_440658.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-21T08:33:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440658"
}
```



---

archive/issue_events_440659.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-02-21T08:42:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440659"
}
```



---

archive/issue_comments_542636.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAn implementation of the Krawtchouk polynomials appear in `coding/delsarte_bounds.py`. I am cc-ing those people as they might be interested (at least for consistency such as normalization differences and/or possible cross-references). This code is likely slower as it is more general but I haven't checked.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9b5f2e1b74d9d5ec6bc54084fc221bd4c3c4412c\">9b5f2e1</a></td><td><code>Cleanup functions/orthogonal_polys.py.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6dca17377a0a54e401389d5e2f1782c90d0f60df\">6dca173</a></td><td><code>Adding krawtchouk polynomials.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cb7a54c6351bfe6d11b89df77945bffdd2d057f4\">cb7a54c</a></td><td><code>Adding Meixner polynomials; fixing some details with Krawtchouk.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9399347d1250fe19abe04a071f34d567835e468c\">9399347</a></td><td><code>Implement Hahn polynomials.</code></td></tr></table>\n",
    "created_at": "2022-02-21T08:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542636",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

An implementation of the Krawtchouk polynomials appear in `coding/delsarte_bounds.py`. I am cc-ing those people as they might be interested (at least for consistency such as normalization differences and/or possible cross-references). This code is likely slower as it is more general but I haven't checked.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9b5f2e1b74d9d5ec6bc54084fc221bd4c3c4412c">9b5f2e1</a></td><td><code>Cleanup functions/orthogonal_polys.py.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6dca17377a0a54e401389d5e2f1782c90d0f60df">6dca173</a></td><td><code>Adding krawtchouk polynomials.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cb7a54c6351bfe6d11b89df77945bffdd2d057f4">cb7a54c</a></td><td><code>Adding Meixner polynomials; fixing some details with Krawtchouk.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9399347d1250fe19abe04a071f34d567835e468c">9399347</a></td><td><code>Implement Hahn polynomials.</code></td></tr></table>




---

archive/issue_comments_542637.json:
```json
{
    "body": "Branch: **[public/symbolics/more_orthogonal_polys-33393](https://github.com/sagemath/sagetrac-mirror/tree/public/symbolics/more_orthogonal_polys-33393)**",
    "created_at": "2022-02-21T08:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542637",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/symbolics/more_orthogonal_polys-33393](https://github.com/sagemath/sagetrac-mirror/tree/public/symbolics/more_orthogonal_polys-33393)**



---

archive/issue_comments_542638.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n These are three classes of orthogonal polynomials in the Askey scheme. We should provide an implementation based on their definition as hypergeometric functions.\n+\n+This ticket provides just a basic implementation with no particular reason for the normalization choices.\n``````\n",
    "created_at": "2022-02-21T08:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542638",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 These are three classes of orthogonal polynomials in the Askey scheme. We should provide an implementation based on their definition as hypergeometric functions.
+
+This ticket provides just a basic implementation with no particular reason for the normalization choices.
``````




---

archive/issue_comments_542639.json:
```json
{
    "body": "Commit: **[9399347](https://github.com/sagemath/sagetrac-mirror/commit/9399347d1250fe19abe04a071f34d567835e468c)**",
    "created_at": "2022-02-21T08:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542639",
    "user": "https://github.com/tscrim"
}
```

Commit: **[9399347](https://github.com/sagemath/sagetrac-mirror/commit/9399347d1250fe19abe04a071f34d567835e468c)**



---

archive/issue_comments_542640.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nthe \"eval_recursive\" should be cached (in parent ?), don't you think ?",
    "created_at": "2022-02-21T10:50:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542640",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

the "eval_recursive" should be cached (in parent ?), don't you think ?



---

archive/issue_comments_542641.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThere isn't really a parent per se. Since the input could be widely varied, it doesn't make so much sense to cache it (and could quick eat up memory). There might be some benefit to caching the resulting (generic) polynomial, but this would likely need a bit of refactoring.",
    "created_at": "2022-02-22T00:31:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542641",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

There isn't really a parent per se. Since the input could be widely varied, it doesn't make so much sense to cache it (and could quick eat up memory). There might be some benefit to caching the resulting (generic) polynomial, but this would likely need a bit of refactoring.



---

archive/issue_comments_542642.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,7 @@\n-These are three classes of orthogonal polynomials in the Askey scheme. We should provide an implementation based on their definition as hypergeometric functions.\n+These are three classes of orthogonal polynomials in\n+the [Askey scheme](https://en.wikipedia.org/wiki/Askey_scheme).\n+We should provide an implementation based on\n+their definition as hypergeometric functions.\n \n-This ticket provides just a basic implementation with no particular reason for the normalization choices.\n+This ticket provides just a basic implementation with\n+no particular reason for the normalization choices.\n``````\n",
    "created_at": "2022-02-23T11:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542642",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,7 @@
-These are three classes of orthogonal polynomials in the Askey scheme. We should provide an implementation based on their definition as hypergeometric functions.
+These are three classes of orthogonal polynomials in
+the [Askey scheme](https://en.wikipedia.org/wiki/Askey_scheme).
+We should provide an implementation based on
+their definition as hypergeometric functions.
 
-This ticket provides just a basic implementation with no particular reason for the normalization choices.
+This ticket provides just a basic implementation with
+no particular reason for the normalization choices.
``````




---

archive/issue_comments_542643.json:
```json
{
    "body": "Changed commit from **[9399347](https://github.com/sagemath/sagetrac-mirror/commit/9399347d1250fe19abe04a071f34d567835e468c)** to **[b9f1eef](https://github.com/sagemath/sagetrac-mirror/commit/b9f1eeff221b572330dec1fdc2d02bacd9f4f454)**",
    "created_at": "2022-02-27T07:51:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542643",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9399347](https://github.com/sagemath/sagetrac-mirror/commit/9399347d1250fe19abe04a071f34d567835e468c)** to **[b9f1eef](https://github.com/sagemath/sagetrac-mirror/commit/b9f1eeff221b572330dec1fdc2d02bacd9f4f454)**



---

archive/issue_comments_542644.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b9f1eeff221b572330dec1fdc2d02bacd9f4f454\">b9f1eef</a></td><td><code>eval_recursive() is a method, not a function.</code></td></tr></table>\n",
    "created_at": "2022-02-27T07:51:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542644",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b9f1eeff221b572330dec1fdc2d02bacd9f4f454">b9f1eef</a></td><td><code>eval_recursive() is a method, not a function.</code></td></tr></table>




---

archive/issue_comments_542645.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThis fixes the pyflakes issues reported by the otherwise green patchbot.",
    "created_at": "2022-02-27T07:53:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542645",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

This fixes the pyflakes issues reported by the otherwise green patchbot.



---

archive/issue_comments_542646.json:
```json
{
    "body": "Changed commit from **[b9f1eef](https://github.com/sagemath/sagetrac-mirror/commit/b9f1eeff221b572330dec1fdc2d02bacd9f4f454)** to **[cc6feca](https://github.com/sagemath/sagetrac-mirror/commit/cc6fecab74defb53882fc659bf82d3cf37bbab4c)**",
    "created_at": "2022-03-11T00:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542646",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b9f1eef](https://github.com/sagemath/sagetrac-mirror/commit/b9f1eeff221b572330dec1fdc2d02bacd9f4f454)** to **[cc6feca](https://github.com/sagemath/sagetrac-mirror/commit/cc6fecab74defb53882fc659bf82d3cf37bbab4c)**



---

archive/issue_comments_542647.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f3e2af0efca5bab5894f74208c58ddc024098689\">f3e2af0</a></td><td><code>Merge branch 'public/symbolics/more_orthogonal_polys-33393' of https://github.com/sagemath/sagetrac-mirror into public/symbolics/more_orthogonal_polys-33393</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cc6fecab74defb53882fc659bf82d3cf37bbab4c\">cc6feca</a></td><td><code>Some little documentation tweaks and improvements.</code></td></tr></table>\n",
    "created_at": "2022-03-11T00:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542647",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f3e2af0efca5bab5894f74208c58ddc024098689">f3e2af0</a></td><td><code>Merge branch 'public/symbolics/more_orthogonal_polys-33393' of https://github.com/sagemath/sagetrac-mirror into public/symbolics/more_orthogonal_polys-33393</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cc6fecab74defb53882fc659bf82d3cf37bbab4c">cc6feca</a></td><td><code>Some little documentation tweaks and improvements.</code></td></tr></table>




---

archive/issue_comments_542648.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2c45db3ee76bd4b631dfc9845889528a6be0c450\">2c45db3</a></td><td><code>Some additional document improvements, including some more history.</code></td></tr></table>\n",
    "created_at": "2022-03-11T04:05:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542648",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2c45db3ee76bd4b631dfc9845889528a6be0c450">2c45db3</a></td><td><code>Some additional document improvements, including some more history.</code></td></tr></table>




---

archive/issue_comments_542649.json:
```json
{
    "body": "Changed commit from **[cc6feca](https://github.com/sagemath/sagetrac-mirror/commit/cc6fecab74defb53882fc659bf82d3cf37bbab4c)** to **[2c45db3](https://github.com/sagemath/sagetrac-mirror/commit/2c45db3ee76bd4b631dfc9845889528a6be0c450)**",
    "created_at": "2022-03-11T04:05:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542649",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[cc6feca](https://github.com/sagemath/sagetrac-mirror/commit/cc6fecab74defb53882fc659bf82d3cf37bbab4c)** to **[2c45db3](https://github.com/sagemath/sagetrac-mirror/commit/2c45db3ee76bd4b631dfc9845889528a6be0c450)**



---

archive/issue_comments_542650.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nPatchbot is (morally) green.",
    "created_at": "2022-03-14T07:12:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542650",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">Comment 9</div>

Patchbot is (morally) green.



---

archive/issue_comments_542651.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nok\n\nmaybe one could add cross-references from and to the Krawtchouk polynomials in coding ?",
    "created_at": "2022-03-14T19:25:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542651",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:10" align="right">Comment 10</div>

ok

maybe one could add cross-references from and to the Krawtchouk polynomials in coding ?



---

archive/issue_comments_542652.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nReplying to [@fchapoton](#comment%3A10):\n> ok\n> \n> maybe one could add cross-references from and to the Krawtchouk polynomials in coding ?\n\nperhaps on a follow-up ticket?",
    "created_at": "2022-03-14T22:54:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542652",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:11" align="right">Comment 11</div>

Replying to [@fchapoton](#comment%3A10):
> ok
> 
> maybe one could add cross-references from and to the Krawtchouk polynomials in coding ?

perhaps on a follow-up ticket?



---

archive/issue_comments_542653.json:
```json
{
    "body": "Changed commit from **[2c45db3](https://github.com/sagemath/sagetrac-mirror/commit/2c45db3ee76bd4b631dfc9845889528a6be0c450)** to **[b0154c5](https://github.com/sagemath/sagetrac-mirror/commit/b0154c572bc4ad797a9457fd424401335e6bda19)**",
    "created_at": "2022-03-15T00:00:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542653",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2c45db3](https://github.com/sagemath/sagetrac-mirror/commit/2c45db3ee76bd4b631dfc9845889528a6be0c450)** to **[b0154c5](https://github.com/sagemath/sagetrac-mirror/commit/b0154c572bc4ad797a9457fd424401335e6bda19)**



---

archive/issue_comments_542654.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7a20625542ba5ad574bce6f23267a4fbbc533da6\">7a20625</a></td><td><code>Merge branch 'develop' into public/symbolics/more_orthogonal_polys-33393</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b0154c572bc4ad797a9457fd424401335e6bda19\">b0154c5</a></td><td><code>Adding crossrefs and a bit more documentation.</code></td></tr></table>\n",
    "created_at": "2022-03-15T00:00:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542654",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7a20625542ba5ad574bce6f23267a4fbbc533da6">7a20625</a></td><td><code>Merge branch 'develop' into public/symbolics/more_orthogonal_polys-33393</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b0154c572bc4ad797a9457fd424401335e6bda19">b0154c5</a></td><td><code>Adding crossrefs and a bit more documentation.</code></td></tr></table>




---

archive/issue_comments_542655.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nI think that is a good idea to have crossrefs. I have added them along with an explicit test checking the relationship between the conventions.",
    "created_at": "2022-03-15T00:03:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542655",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:13" align="right">Comment 13</div>

I think that is a good idea to have crossrefs. I have added them along with an explicit test checking the relationship between the conventions.



---

archive/issue_comments_542656.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nok, thenl let's go",
    "created_at": "2022-03-15T07:12:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542656",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:14" align="right">Comment 14</div>

ok, thenl let's go



---

archive/issue_comments_542657.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2022-03-15T07:12:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542657",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_440660.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-15T07:12:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440660"
}
```



---

archive/issue_events_440661.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-15T07:12:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440661"
}
```



---

archive/issue_comments_542658.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nThank you.",
    "created_at": "2022-03-15T07:12:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542658",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:15" align="right">Comment 15</div>

Thank you.



---

archive/issue_comments_542659.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\n\n```\n[sagemath_doc_pdf-none] ? \n[sagemath_doc_pdf-none] ! Emergency stop.\n[sagemath_doc_pdf-none]  ...                                              \n[sagemath_doc_pdf-none]                                                   \n[sagemath_doc_pdf-none] l.4558 They are named after Pafnuty Chebyshev (\u0427\n[sagemath_doc_pdf-none]                                                  \u0435\u0431\u044b\u0448\u0451\u0432 1821\\sphinxhyp...\n[sagemath_doc_pdf-none] \n```",
    "created_at": "2022-03-20T19:29:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542659",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:16" align="right">Comment 16</div>


```
[sagemath_doc_pdf-none] ? 
[sagemath_doc_pdf-none] ! Emergency stop.
[sagemath_doc_pdf-none]  ...                                              
[sagemath_doc_pdf-none]                                                   
[sagemath_doc_pdf-none] l.4558 They are named after Pafnuty Chebyshev (Ч
[sagemath_doc_pdf-none]                                                  ебышёв 1821\sphinxhyp...
[sagemath_doc_pdf-none] 
```



---

archive/issue_events_440662.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-03-20T19:29:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440662"
}
```



---

archive/issue_events_440663.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-03-20T19:29:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440663"
}
```



---

archive/issue_comments_542660.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nIt looks like the pdf doc doesn\u2019t like Cyrillic so much; I thought it would be fine with the Russian tutorial. Do you know how to get that to work? If not, then we can just remove it.",
    "created_at": "2022-03-21T01:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542660",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:17" align="right">Comment 17</div>

It looks like the pdf doc doesn’t like Cyrillic so much; I thought it would be fine with the Russian tutorial. Do you know how to get that to work? If not, then we can just remove it.



---

archive/issue_comments_542661.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\njust remove cyrillic",
    "created_at": "2022-03-21T08:34:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542661",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:18" align="right">Comment 18</div>

just remove cyrillic



---

archive/issue_comments_542662.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9b40ec430c42a89dcd5b24ac35e2e3eb7d3d411a\">9b40ec4</a></td><td><code>Remove Cyrillic from doc so pdf will compile.</code></td></tr></table>\n",
    "created_at": "2022-03-24T23:13:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542662",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:19" align="right">Comment 19</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9b40ec430c42a89dcd5b24ac35e2e3eb7d3d411a">9b40ec4</a></td><td><code>Remove Cyrillic from doc so pdf will compile.</code></td></tr></table>




---

archive/issue_comments_542663.json:
```json
{
    "body": "Changed commit from **[b0154c5](https://github.com/sagemath/sagetrac-mirror/commit/b0154c572bc4ad797a9457fd424401335e6bda19)** to **[9b40ec4](https://github.com/sagemath/sagetrac-mirror/commit/9b40ec430c42a89dcd5b24ac35e2e3eb7d3d411a)**",
    "created_at": "2022-03-24T23:13:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542663",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b0154c5](https://github.com/sagemath/sagetrac-mirror/commit/b0154c572bc4ad797a9457fd424401335e6bda19)** to **[9b40ec4](https://github.com/sagemath/sagetrac-mirror/commit/9b40ec430c42a89dcd5b24ac35e2e3eb7d3d411a)**



---

archive/issue_comments_542664.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nRemoved.",
    "created_at": "2022-03-24T23:13:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542664",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:20" align="right">Comment 20</div>

Removed.



---

archive/issue_events_440664.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-03-25T07:52:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440664"
}
```



---

archive/issue_events_440665.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-03-25T07:52:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440665"
}
```



---

archive/issue_comments_542665.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\ndid you check that pdf doc compiles ?",
    "created_at": "2022-03-25T10:39:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542665",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:22" align="right">Comment 22</div>

did you check that pdf doc compiles ?



---

archive/issue_comments_542666.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">Comment 23</div>\n\nYes, the pdf doc compiles for me.",
    "created_at": "2022-03-27T04:58:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542666",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:23" align="right">Comment 23</div>

Yes, the pdf doc compiles for me.



---

archive/issue_events_440666.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-27T07:11:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440666"
}
```



---

archive/issue_events_440667.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-27T07:11:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440667"
}
```



---

archive/issue_comments_542667.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">Comment 24</div>\n\nThanks, then let's move on",
    "created_at": "2022-03-27T07:11:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542667",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:24" align="right">Comment 24</div>

Thanks, then let's move on



---

archive/issue_comments_542668.json:
```json
{
    "body": "Changed branch from **[public/symbolics/more_orthogonal_polys-33393](https://github.com/sagemath/sagetrac-mirror/tree/public/symbolics/more_orthogonal_polys-33393)** to **[9b40ec4](https://github.com/sagemath/sagetrac-mirror/commit/9b40ec430c42a89dcd5b24ac35e2e3eb7d3d411a)**",
    "created_at": "2022-03-30T22:33:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33393#issuecomment-542668",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/symbolics/more_orthogonal_polys-33393](https://github.com/sagemath/sagetrac-mirror/tree/public/symbolics/more_orthogonal_polys-33393)** to **[9b40ec4](https://github.com/sagemath/sagetrac-mirror/commit/9b40ec430c42a89dcd5b24ac35e2e3eb7d3d411a)**



---

archive/issue_events_440668.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-03-30T22:33:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440668"
}
```



---

archive/issue_events_440669.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b11471b30b876a6ca657e0c14d565e72d201dcef",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-03-30T22:33:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33393",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33393#event-440669"
}
```
