...
```json
{
    "body": "This is a duplicate of #33050, but with more information.",
...
```json
{
    "body": "It's very weird and I can reproduce it now. We hit a similar issue in #33092; see the discussion starting around comment 45. Does using `sage -t --serial` fix the problem?",
...
```json
{
    "body": "Using `--serial` worked fine exactly once. Every subsequent run hangs even with `--serial`",
