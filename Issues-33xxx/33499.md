# Issue 33499: implement inverse_of_unit on polynomials

archive/issues_033262.json:
```json
{
    "assignees": [],
    "body": "Even though `is_unit` and `is_nilpotent` are correctly implemented the various implementations of `inverse_of_unit` are either wrong or not doing their job\n\n```\nsage: R.<x,y> = Zmod(4)[]\nsage: u = 1 + 2*x + 2*y**2\nsage: u.is_unit()\nTrue\nsage: (u * u).is_one()\nTrue\nsage: u.inverse_of_unit()\nTraceback (most recent call last):\n...\nArithmeticError: Element is not a unit.\n```\n\nCC:  @DavidAyotte\n\nBranch/Commit: **[u/vdelecroix/33499](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/33499) @ [02c2d4d](https://github.com/sagemath/sagetrac-mirror/commit/02c2d4df9b3c649aedf8293b43955beb454e6541)**\n\nReviewer: **David Ayotte**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33499_\n\n",
    "created_at": "2022-03-14T08:55:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "implement inverse_of_unit on polynomials",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/33499",
    "user": "https://github.com/videlec"
}
```
Even though `is_unit` and `is_nilpotent` are correctly implemented the various implementations of `inverse_of_unit` are either wrong or not doing their job

```
sage: R.<x,y> = Zmod(4)[]
sage: u = 1 + 2*x + 2*y**2
sage: u.is_unit()
True
sage: (u * u).is_one()
True
sage: u.inverse_of_unit()
Traceback (most recent call last):
...
ArithmeticError: Element is not a unit.
```

CC:  @DavidAyotte

Branch/Commit: **[u/vdelecroix/33499](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/33499) @ [02c2d4d](https://github.com/sagemath/sagetrac-mirror/commit/02c2d4df9b3c649aedf8293b43955beb454e6541)**

Reviewer: **David Ayotte**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/33499_





---

archive/issue_events_441819.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-03-14T08:55:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441819"
}
```



---

archive/issue_events_441820.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-03-14T08:55:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441820"
}
```



---

archive/issue_events_441821.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-03-14T08:55:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441821"
}
```



---

archive/issue_events_441822.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-03-14T08:55:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441822"
}
```



---

archive/issue_comments_544146.json:
```json
{
    "body": "Author: **Vincent Delecroix**",
    "created_at": "2022-03-14T14:13:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544146",
    "user": "https://github.com/videlec"
}
```

Author: **Vincent Delecroix**



---

archive/issue_comments_544147.json:
```json
{
    "body": "Branch: **[u/vdelecroix/33499](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/33499)**",
    "created_at": "2022-03-14T14:13:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544147",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/33499](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/33499)**



---

archive/issue_comments_544148.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f57c141aacf4e2439c96f94927946b83e7e33ae2\">f57c141</a></td><td><code>33499: fix inverse_of_unit for polynomials</code></td></tr></table>\n",
    "created_at": "2022-03-14T14:13:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544148",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f57c141aacf4e2439c96f94927946b83e7e33ae2">f57c141</a></td><td><code>33499: fix inverse_of_unit for polynomials</code></td></tr></table>




---

archive/issue_events_441823.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-03-14T14:13:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441823"
}
```



---

archive/issue_comments_544149.json:
```json
{
    "body": "Commit: **[f57c141](https://github.com/sagemath/sagetrac-mirror/commit/f57c141aacf4e2439c96f94927946b83e7e33ae2)**",
    "created_at": "2022-03-14T14:13:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544149",
    "user": "https://github.com/videlec"
}
```

Commit: **[f57c141](https://github.com/sagemath/sagetrac-mirror/commit/f57c141aacf4e2439c96f94927946b83e7e33ae2)**



---

archive/issue_comments_544150.json:
```json
{
    "body": "Reviewer: **David Ayotte**",
    "created_at": "2022-03-14T17:33:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544150",
    "user": "https://github.com/DavidAyotte"
}
```

Reviewer: **David Ayotte**



---

archive/issue_comments_544151.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nHello Vincent, thank you for this fix. Minor comment: according to the reviewer's checklist (https://doc.sagemath.org/html/en/developer/reviewer_checklist.html) you should mention the number of this ticket in the doctests saying that you fixed the bug you found.\n\nAnyhow, everything looks good. I will wait for the result of the doctest for the rest.",
    "created_at": "2022-03-14T17:33:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544151",
    "user": "https://github.com/DavidAyotte"
}
```

<a id='comment:2'>Comment 2:</a>
Hello Vincent, thank you for this fix. Minor comment: according to the reviewer's checklist (https://doc.sagemath.org/html/en/developer/reviewer_checklist.html) you should mention the number of this ticket in the doctests saying that you fixed the bug you found.

Anyhow, everything looks good. I will wait for the result of the doctest for the rest.



---

archive/issue_comments_544152.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThe three failed doctests are not related to this ticket.",
    "created_at": "2022-03-14T18:01:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544152",
    "user": "https://github.com/DavidAyotte"
}
```

<a id='comment:3'>Comment 3:</a>
The three failed doctests are not related to this ticket.



---

archive/issue_comments_544153.json:
```json
{
    "body": "Changed commit from **[f57c141](https://github.com/sagemath/sagetrac-mirror/commit/f57c141aacf4e2439c96f94927946b83e7e33ae2)** to **[02c2d4d](https://github.com/sagemath/sagetrac-mirror/commit/02c2d4df9b3c649aedf8293b43955beb454e6541)**",
    "created_at": "2022-03-14T20:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544153",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f57c141](https://github.com/sagemath/sagetrac-mirror/commit/f57c141aacf4e2439c96f94927946b83e7e33ae2)** to **[02c2d4d](https://github.com/sagemath/sagetrac-mirror/commit/02c2d4df9b3c649aedf8293b43955beb454e6541)**



---

archive/issue_comments_544154.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/02c2d4df9b3c649aedf8293b43955beb454e6541\">02c2d4d</a></td><td><code>33499: add a specific test mentioning the ticket number</code></td></tr></table>\n",
    "created_at": "2022-03-14T20:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544154",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/02c2d4df9b3c649aedf8293b43955beb454e6541">02c2d4d</a></td><td><code>33499: add a specific test mentioning the ticket number</code></td></tr></table>




---

archive/issue_events_441824.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-03-14T20:01:21Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "title_is": "implement inverse_of_unit on polynomials",
    "title_was": "implement inverse_of_unit on multivariate polynomials",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441824"
}
```



---

archive/issue_events_441825.json:
```json
{
    "actor": "https://github.com/DavidAyotte",
    "created_at": "2022-03-14T20:04:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441825"
}
```



---

archive/issue_events_441826.json:
```json
{
    "actor": "https://github.com/DavidAyotte",
    "created_at": "2022-03-14T20:04:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441826"
}
```



---

archive/issue_events_441827.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-03-26T21:49:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441827"
}
```



---

archive/issue_events_441828.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-03-26T21:49:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441828"
}
```



---

archive/issue_comments_544155.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThis ticket causes timeouts in\n\n```\nsage -t --long --random-seed=114734410077030599852047573071790577692 src/sage/rings/valuation/augmented_valuation.py  # Timed out\n```",
    "created_at": "2022-03-26T21:49:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33499#issuecomment-544155",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:7'>Comment 7:</a>
This ticket causes timeouts in

```
sage -t --long --random-seed=114734410077030599852047573071790577692 src/sage/rings/valuation/augmented_valuation.py  # Timed out
```



---

archive/issue_events_441829.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-17T19:59:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441829"
}
```



---

archive/issue_events_441830.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-17T19:59:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441830"
}
```



---

archive/issue_events_441831.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441831"
}
```



---

archive/issue_events_441832.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33499",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33499#event-441832"
}
```
