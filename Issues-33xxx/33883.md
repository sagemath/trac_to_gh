# Issue 33883: Typo in Steenrod module thematic tutorial

archive/issues_033646.json:
```json
{
    "assignees": [],
    "body": "We replace the incorrect `ker(f)` by `ker(q)` here:\n\n```diff\n which means that the indeterminacy of choosing a lift for\n `f \\in \\operatorname{Hom}_A(L, Hko)` is represented by an element in\n-`\\operatorname{Hom}_A(L,\\ker(f))`.  Therefore, we can proceed to count the\n+`\\operatorname{Hom}_A(L,\\ker(q))`.  Therefore, we can proceed to count the\n number of lifts by computing this vector space of homomorphisms::\n```\n\nCC:  @sverre320 @sagetrac-kvanwoerden @tscrim @rrbruner\n\nBranch/Commit: **[8a05da7](https://github.com/sagemath/sagetrac-mirror/commit/8a05da7661ba110e428d4661589b251642441440)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33883_\n\n",
    "closed_at": "2022-05-26T22:49:13Z",
    "created_at": "2022-05-22T20:46:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Typo in Steenrod module thematic tutorial",
    "type": "issue",
    "updated_at": "2022-05-26T22:49:13Z",
    "url": "https://github.com/sagemath/sage/issues/33883",
    "user": "https://github.com/jhpalmieri"
}
```
We replace the incorrect `ker(f)` by `ker(q)` here:

```diff
 which means that the indeterminacy of choosing a lift for
 `f \in \operatorname{Hom}_A(L, Hko)` is represented by an element in
-`\operatorname{Hom}_A(L,\ker(f))`.  Therefore, we can proceed to count the
+`\operatorname{Hom}_A(L,\ker(q))`.  Therefore, we can proceed to count the
 number of lifts by computing this vector space of homomorphisms::
```

CC:  @sverre320 @sagetrac-kvanwoerden @tscrim @rrbruner

Branch/Commit: **[8a05da7](https://github.com/sagemath/sagetrac-mirror/commit/8a05da7661ba110e428d4661589b251642441440)**

Reviewer: **Travis Scrimshaw**

Author: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/33883_





---

archive/issue_events_446320.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-05-22T20:46:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446320"
}
```



---

archive/issue_events_446321.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-05-22T20:46:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446321"
}
```



---

archive/issue_comments_551185.json:
```json
{
    "body": "Branch: **[u/jhpalmieri/steenrod-typo](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/steenrod-typo)**",
    "created_at": "2022-05-22T20:47:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33883#issuecomment-551185",
    "user": "https://github.com/jhpalmieri"
}
```

Branch: **[u/jhpalmieri/steenrod-typo](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/steenrod-typo)**



---

archive/issue_comments_551186.json:
```json
{
    "body": "Commit: **[8a05da7](https://github.com/sagemath/sagetrac-mirror/commit/8a05da7661ba110e428d4661589b251642441440)**",
    "created_at": "2022-05-22T20:50:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33883#issuecomment-551186",
    "user": "https://github.com/jhpalmieri"
}
```

Commit: **[8a05da7](https://github.com/sagemath/sagetrac-mirror/commit/8a05da7661ba110e428d4661589b251642441440)**



---

archive/issue_events_446322.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-05-22T20:50:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446322"
}
```



---

archive/issue_events_446323.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-05-22T20:50:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446323"
}
```



---

archive/issue_events_446324.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-05-22T20:50:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446324"
}
```



---

archive/issue_events_446325.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-05-22T20:50:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446325"
}
```



---

archive/issue_comments_551187.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8a05da7661ba110e428d4661589b251642441440\">8a05da7</a></td><td><code>trac 33883: typo in thematic_tutorials/steenrod_algebra_modules.rst</code></td></tr></table>\n",
    "created_at": "2022-05-22T20:50:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33883#issuecomment-551187",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8a05da7661ba110e428d4661589b251642441440">8a05da7</a></td><td><code>trac 33883: typo in thematic_tutorials/steenrod_algebra_modules.rst</code></td></tr></table>




---

archive/issue_events_446326.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-05-22T20:50:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446326"
}
```



---

archive/issue_events_446327.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-05-22T22:35:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446327"
}
```



---

archive/issue_events_446328.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-05-22T22:35:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446328"
}
```



---

archive/issue_comments_551188.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-05-22T22:35:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33883#issuecomment-551188",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_551189.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLGTM.",
    "created_at": "2022-05-22T22:35:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33883#issuecomment-551189",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

LGTM.



---

archive/issue_comments_551190.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThanks!",
    "created_at": "2022-05-22T23:53:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33883#issuecomment-551190",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thanks!



---

archive/issue_comments_551191.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,9 @@\n+We replace the incorrect `ker(f)` by `ker(q)` here:\n \n+```diff\n+ which means that the indeterminacy of choosing a lift for\n+ `f \\in \\operatorname{Hom}_A(L, Hko)` is represented by an element in\n+-`\\operatorname{Hom}_A(L,\\ker(f))`.  Therefore, we can proceed to count the\n++`\\operatorname{Hom}_A(L,\\ker(q))`.  Therefore, we can proceed to count the\n+ number of lifts by computing this vector space of homomorphisms::\n+```\n``````\n",
    "created_at": "2022-05-24T10:35:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33883#issuecomment-551191",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,9 @@
+We replace the incorrect `ker(f)` by `ker(q)` here:
 
+```diff
+ which means that the indeterminacy of choosing a lift for
+ `f \in \operatorname{Hom}_A(L, Hko)` is represented by an element in
+-`\operatorname{Hom}_A(L,\ker(f))`.  Therefore, we can proceed to count the
++`\operatorname{Hom}_A(L,\ker(q))`.  Therefore, we can proceed to count the
+ number of lifts by computing this vector space of homomorphisms::
+```
``````




---

archive/issue_events_446329.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-05-26T22:49:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446329"
}
```



---

archive/issue_events_446330.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8501c7398215556ff4a4c8b05c6699949c8dfca3",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-05-26T22:49:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33883#event-446330"
}
```



---

archive/issue_comments_551192.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/steenrod-typo](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/steenrod-typo)** to **[8a05da7](https://github.com/sagemath/sagetrac-mirror/commit/8a05da7661ba110e428d4661589b251642441440)**",
    "created_at": "2022-05-26T22:49:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33883#issuecomment-551192",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/steenrod-typo](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/steenrod-typo)** to **[8a05da7](https://github.com/sagemath/sagetrac-mirror/commit/8a05da7661ba110e428d4661589b251642441440)**
