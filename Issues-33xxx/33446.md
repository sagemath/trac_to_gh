...
```json
{
    "body": "I'd guess there'a potential for a regression here on Linux systems that have multiple versions of the shared library installed.",
...
```json
{
    "body": "Why isn't `libgap.so` installed with the rest of the package? You need it for `$CC ... -lgap` to work, so I think it should be part of the \"normal\" installation.",
...
```json
{
    "body": "I also see that the gap library is versioned upstream, so if the distro package is installing it as `libgap.so.0.0.0` on purpose, that makes any theorizing about multiple versions extra pointless.",
...
```json
{
    "body": "In fact one way to get the soname for a library may be using `ctypes.util.find_library()` but I'm not sure it would do the right thing when multiple libraries are available:\n\n```\nsage: import ctypes.util\nsage: ctypes.util.find_library(\"gap\")\n'libgap.so.0.0.0'\nsage: import sage.misc.compat\nsage: sage.misc.compat.find_library(\"gap\")\n'libgap.so.0.0.0'\n```",
...
```json
{
    "body": "If we are so worried about mixing up versions because of potential incompatibility, surely we should lock on a specific version of the library in the first place? Playing the devil's advocate there.",
...
```json
{
    "body": "That's interesting. In Gentoo, I explicitly patch packages like `io` to link to `libgap.so`. In fact I even patched all the package tooling from `gap` like `gac` and `Makefile.gappkg` so packages that use those do not need to be patched as much as possible and still get linked with `libgap`.\n\nSounds to me like you are telling me, I can give the hack the boot on Gentoo if I want too.",
...
```json
{
    "body": "Anything that get rid of the `GAP_SO` hack is a good thing (TM). If someone could test that it works on OS X we should go for the latest. Although, we can apply all the separate measures that can fix it in a single drop to make sure the thing is killed.",
...
```json
{
    "body": "Going down that route, I think we should just eliminate `GAP_SO` and `LIBSINGULAR_PATH`, which means removing them from `env.py` and dealing with this strange bit in `sage/interfaces/gap_workspace.py`\n\n```\n    if GAP_SO:\n        h = hashlib.sha1(GAP_SO.encode('utf-8')).hexdigest()\n    else:\n        h = 'unknown'\n    return os.path.join(dir, '%s-%s-%s' % (system, name, h))\n```\nI am sure when can find something else to hash.",
...
```json
{
    "body": "The current branch seems to work well on macOS (x86_64) with singular from SPKG.",
...
```json
{
    "body": "Could you rebase this on GAP 4.12 - the update has been positively reviewed in #34391 ?",
...
```json
{
    "body": "Rebased on top of #34391. I added a commit so `sysinfo.gap` is searched both in `GAP_LIB_DIR` and in `GAP_SHARE_DIR`. Also: use the content of those variables instead of `SAGE_LOCAL`.",
...
```json
{
    "body": "let's get #34391 in first - it's a bit hard to review now.",
...
```json
{
    "body": "looks good - also tested on macOS",
    "created_at": "2022-12-09T11:02:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33446",
    "type": "issue_comment",
