# Issue 33300: Allow large exponents in polynomial rings

archive/issues_033063.json:
```json
{
    "assignees": [],
    "body": "\n```\nOverflowError                             Traceback (most recent call last)\n<ipython-input-37-db1c8bc53ef7> in <module>\n----> 1 ff = x**Integer(Integer(2818407171672390179701902257344061561))\n      2 ff\n\n/var/tmp/sage-9.4-current/local/lib/python3.9/site-packages/sage/rings/polynomial/polynomial_modn_dense_ntl.pyx in sage.rings.polynomial.polynomial_modn_dense_ntl.Polynomial_dense_modn_ntl_ZZ.__pow__ (build/cythonized/sage/rings/polynomial/polynomial_modn_dense_ntl.cpp:19729)()\n   1408         \"\"\"\n   1409         cdef bint recip = 0, do_sig\n-> 1410         cdef long e = ee\n   1411         if e != ee:\n   1412             raise TypeError(\"Only integral powers defined.\")\n\nOverflowError: Python int too large to convert to C long\n```\n\nComponent: **algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33300_\n\n",
    "created_at": "2022-02-06T08:20:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/feature"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Allow large exponents in polynomial rings",
    "type": "issue",
    "updated_at": "2022-02-07T06:45:54Z",
    "url": "https://github.com/sagemath/sage/issues/33300",
    "user": "https://github.com/lyciumlee"
}
```

```
OverflowError                             Traceback (most recent call last)
<ipython-input-37-db1c8bc53ef7> in <module>
----> 1 ff = x**Integer(Integer(2818407171672390179701902257344061561))
      2 ff

/var/tmp/sage-9.4-current/local/lib/python3.9/site-packages/sage/rings/polynomial/polynomial_modn_dense_ntl.pyx in sage.rings.polynomial.polynomial_modn_dense_ntl.Polynomial_dense_modn_ntl_ZZ.__pow__ (build/cythonized/sage/rings/polynomial/polynomial_modn_dense_ntl.cpp:19729)()
   1408         """
   1409         cdef bint recip = 0, do_sig
-> 1410         cdef long e = ee
   1411         if e != ee:
   1412             raise TypeError("Only integral powers defined.")

OverflowError: Python int too large to convert to C long
```

Component: **algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/33300_





---

archive/issue_events_457967.json:
```json
{
    "actor": "https://github.com/lyciumlee",
    "created_at": "2022-02-06T08:20:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33300",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33300#event-457967"
}
```



---

archive/issue_events_457968.json:
```json
{
    "actor": "https://github.com/lyciumlee",
    "created_at": "2022-02-06T08:20:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33300",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33300#event-457968"
}
```



---

archive/issue_events_457969.json:
```json
{
    "actor": "https://github.com/lyciumlee",
    "created_at": "2022-02-06T08:20:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33300",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33300#event-457969"
}
```



---

archive/issue_events_457970.json:
```json
{
    "actor": "https://github.com/lyciumlee",
    "created_at": "2022-02-06T08:20:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33300",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33300#event-457970"
}
```



---

archive/issue_comments_538302.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,8 @@\n+\n+```\n OverflowError                             Traceback (most recent call last)\n <ipython-input-37-db1c8bc53ef7> in <module>\n-\n----\n+----> 1 ff = x**Integer(Integer(2818407171672390179701902257344061561))\n       2 ff\n \n /var/tmp/sage-9.4-current/local/lib/python3.9/site-packages/sage/rings/polynomial/polynomial_modn_dense_ntl.pyx in sage.rings.polynomial.polynomial_modn_dense_ntl.Polynomial_dense_modn_ntl_ZZ.__pow__ (build/cythonized/sage/rings/polynomial/polynomial_modn_dense_ntl.cpp:19729)()\n@@ -12,3 +13,4 @@\n    1412             raise TypeError(\"Only integral powers defined.\")\n \n OverflowError: Python int too large to convert to C long\n+```\n``````\n",
    "created_at": "2022-02-06T17:37:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33300#issuecomment-538302",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,8 @@
+
+```
 OverflowError                             Traceback (most recent call last)
 <ipython-input-37-db1c8bc53ef7> in <module>
-
----
+----> 1 ff = x**Integer(Integer(2818407171672390179701902257344061561))
       2 ff
 
 /var/tmp/sage-9.4-current/local/lib/python3.9/site-packages/sage/rings/polynomial/polynomial_modn_dense_ntl.pyx in sage.rings.polynomial.polynomial_modn_dense_ntl.Polynomial_dense_modn_ntl_ZZ.__pow__ (build/cythonized/sage/rings/polynomial/polynomial_modn_dense_ntl.cpp:19729)()
@@ -12,3 +13,4 @@
    1412             raise TypeError("Only integral powers defined.")
 
 OverflowError: Python int too large to convert to C long
+```
``````




---

archive/issue_comments_538303.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nHow did you define `x` before running `ff = x^...`?\n\nWhat computation is intended?\n\nSparse polynomials might be made to accept such\nlarge exponents; dense polynomials likely not.",
    "created_at": "2022-02-06T17:37:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33300#issuecomment-538303",
    "user": "https://github.com/slel"
}
```

<div id="comment:1" align="right">comment:1</div>

How did you define `x` before running `ff = x^...`?

What computation is intended?

Sparse polynomials might be made to accept such
large exponents; dense polynomials likely not.



---

archive/issue_events_457971.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2022-02-06T17:37:28Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/33300",
    "title_is": "Allow large exponents in polynomial rings",
    "title_was": "Could you please let polynomial ring exponent support large number?",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33300#event-457971"
}
```



---

archive/issue_comments_538304.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\n\n```\nN = \nc1 = \nc2 = \ne = 0xd4088c345ced64cbbf8444321ef2af8b\n\nR = Zmod(N)\nPR.<x> = PolynomialRing(R)\npol = c2-(c1-x)^5\n#\nff = x^e\nprint(ff % pol)\n```\nI want get ff moduled by pol polynomial, I know that I can use quotient_ring to achieve it. But could you please let it worked directly.",
    "created_at": "2022-02-07T06:44:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33300#issuecomment-538304",
    "user": "https://github.com/lyciumlee"
}
```

<div id="comment:2" align="right">comment:2</div>


```
N = 
c1 = 
c2 = 
e = 0xd4088c345ced64cbbf8444321ef2af8b

R = Zmod(N)
PR.<x> = PolynomialRing(R)
pol = c2-(c1-x)^5
#
ff = x^e
print(ff % pol)
```
I want get ff moduled by pol polynomial, I know that I can use quotient_ring to achieve it. But could you please let it worked directly.



---

archive/issue_comments_538305.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nWhen e is a small number sagemath will work.",
    "created_at": "2022-02-07T06:45:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33300#issuecomment-538305",
    "user": "https://github.com/lyciumlee"
}
```

<div id="comment:3" align="right">comment:3</div>

When e is a small number sagemath will work.
