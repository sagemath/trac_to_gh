# Issue 33128: SyntaxWarning when byte-compiling sagetexparse.py

archive/issues_032891.json:
```json
{
    "assignees": [],
    "body": "\n```\n.../share/texmf/tex/latex/sagetex/sagetexparse.py:135: SyntaxWarning: \"is not\" with a literal. Did you mean \"!=\"?\n  if t.format is not '':\n```\n\nThis is [already fixed upstream](https://github.com/sagemath/sagetex/commit/3a22234a28e4850be169d98fba78d90369a4723e#diff-0b17ad5d149a4c8e23b977c2da6375d92582ee11347d8f5a6b9e365ca9399d28L499).\n\nDepends on #32887\n\nCC:  @dimpase\n\nBranch/Commit: **[u/tornaria/sagetex-fix-warning](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/sagetex-fix-warning) @ [53e4d68](https://github.com/sagemath/sagetrac-mirror/commit/53e4d68ca3ba0d3b29fe78dbd99fa927fa4091b6)**\n\nReviewer: **Kwankyu Lee**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33128_\n\n",
    "closed_at": "2022-11-14T19:36:43Z",
    "created_at": "2022-01-07T17:34:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SyntaxWarning when byte-compiling sagetexparse.py",
    "type": "issue",
    "updated_at": "2022-11-14T19:36:43Z",
    "url": "https://github.com/sagemath/sage/issues/33128",
    "user": "https://github.com/tornaria"
}
```

```
.../share/texmf/tex/latex/sagetex/sagetexparse.py:135: SyntaxWarning: "is not" with a literal. Did you mean "!="?
  if t.format is not '':
```

This is [already fixed upstream](https://github.com/sagemath/sagetex/commit/3a22234a28e4850be169d98fba78d90369a4723e#diff-0b17ad5d149a4c8e23b977c2da6375d92582ee11347d8f5a6b9e365ca9399d28L499).

Depends on #32887

CC:  @dimpase

Branch/Commit: **[u/tornaria/sagetex-fix-warning](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/sagetex-fix-warning) @ [53e4d68](https://github.com/sagemath/sagetrac-mirror/commit/53e4d68ca3ba0d3b29fe78dbd99fa927fa4091b6)**

Reviewer: **Kwankyu Lee**

_Issue created by migration from https://trac.sagemath.org/ticket/33128_





---

archive/issue_events_437526.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2022-01-07T17:34:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437526"
}
```



---

archive/issue_events_437527.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2022-01-07T17:34:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437527"
}
```



---

archive/issue_events_437528.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2022-01-07T17:34:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437528"
}
```



---

archive/issue_events_437529.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2022-01-07T17:34:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437529"
}
```



---

archive/issue_events_437530.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2022-01-07T17:35:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437530"
}
```



---

archive/issue_comments_538337.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/53e4d68ca3ba0d3b29fe78dbd99fa927fa4091b6\">53e4d68</a></td><td><code>sagetex: Fix a warning when byte-compiling sagetexparse.py</code></td></tr></table>\n",
    "created_at": "2022-01-07T17:35:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538337",
    "user": "https://github.com/tornaria"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/53e4d68ca3ba0d3b29fe78dbd99fa927fa4091b6">53e4d68</a></td><td><code>sagetex: Fix a warning when byte-compiling sagetexparse.py</code></td></tr></table>




---

archive/issue_comments_538338.json:
```json
{
    "body": "Author: **Gonzalo Tornar\u00eda**",
    "created_at": "2022-01-07T17:35:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538338",
    "user": "https://github.com/tornaria"
}
```

Author: **Gonzalo Tornaría**



---

archive/issue_comments_538339.json:
```json
{
    "body": "Branch: **[u/tornaria/sagetex-fix-warning](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/sagetex-fix-warning)**",
    "created_at": "2022-01-07T17:35:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538339",
    "user": "https://github.com/tornaria"
}
```

Branch: **[u/tornaria/sagetex-fix-warning](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/sagetex-fix-warning)**



---

archive/issue_comments_538340.json:
```json
{
    "body": "Commit: **[53e4d68](https://github.com/sagemath/sagetrac-mirror/commit/53e4d68ca3ba0d3b29fe78dbd99fa927fa4091b6)**",
    "created_at": "2022-01-07T17:35:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538340",
    "user": "https://github.com/tornaria"
}
```

Commit: **[53e4d68](https://github.com/sagemath/sagetrac-mirror/commit/53e4d68ca3ba0d3b29fe78dbd99fa927fa4091b6)**



---

archive/issue_comments_538341.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nwe should rather go ahead with updating sagetex to 3.6.\n\nI hope to be able to have a look at it again next week.",
    "created_at": "2022-01-07T20:12:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538341",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:3'>Comment 3:</a>
we should rather go ahead with updating sagetex to 3.6.

I hope to be able to have a look at it again next week.



---

archive/issue_events_437531.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-28T23:03:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437531"
}
```



---

archive/issue_events_437532.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-28T23:03:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437532"
}
```



---

archive/issue_events_437533.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437533"
}
```



---

archive/issue_events_437534.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437534"
}
```



---

archive/issue_events_437535.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437535"
}
```



---

archive/issue_events_437536.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437536"
}
```



---

archive/issue_comments_538342.json:
```json
{
    "body": "Dependencies: **#32887**",
    "created_at": "2022-09-26T00:05:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538342",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#32887**



---

archive/issue_comments_538343.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI think this outdated after the update done in #32887.",
    "created_at": "2022-09-26T00:05:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538343",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'>Comment 7:</a>
I think this outdated after the update done in #32887.



---

archive/issue_events_437537.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-26T00:05:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437537"
}
```



---

archive/issue_events_437538.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-26T00:05:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437538"
}
```



---

archive/issue_events_437539.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-26T00:05:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437539"
}
```



---

archive/issue_comments_538344.json:
```json
{
    "body": "Changed author from **Gonzalo Tornar\u00eda** to none",
    "created_at": "2022-09-26T00:05:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538344",
    "user": "https://github.com/mkoeppe"
}
```

Changed author from **Gonzalo Tornaría** to none



---

archive/issue_comments_538345.json:
```json
{
    "body": "Reviewer: **Kwankyu Lee**",
    "created_at": "2022-10-14T21:00:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538345",
    "user": "https://github.com/kwankyu"
}
```

Reviewer: **Kwankyu Lee**



---

archive/issue_events_437540.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-10-14T21:00:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437540"
}
```



---

archive/issue_events_437541.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-10-14T21:00:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437541"
}
```



---

archive/issue_comments_538346.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nYes. It's already fixed.",
    "created_at": "2022-10-14T21:00:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33128#issuecomment-538346",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:8'>Comment 8:</a>
Yes. It's already fixed.



---

archive/issue_events_437542.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-11-14T19:36:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437542"
}
```



---

archive/issue_events_437543.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-11-14T19:36:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33128",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33128#event-437543"
}
```
