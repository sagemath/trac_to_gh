...
```json
{
    "body": "<a id='comment:2'></a>The individual commits should be easier to understand than the overall diff.\n\n---\nNew commits:",
...
<a id='comment:2'></a>The individual commits should be easier to understand than the overall diff.
...
<a id='comment:5'></a>I am not sure I entirely like moving something from Cython that could be called a lot to a Python function. I understand it would be easier to maintain, but I am not sure it is worth the potential speed tradeoff since this is a fairly important part of Sage. This isn't a strong opinion, but it is something to consider.
...
```json
{
    "body": "<a id='comment:10'></a>Patchbot is morally green (though I'm not sure what's up with the one failure\u2009\u2014\u2009it appears to be somewhat consistent, but is probably unrelated to these changes?).\n\nI applied two more small tweaks to hopefully make the patchbot plugins happy too.",
...
<a id='comment:10'></a>Patchbot is morally green (though I'm not sure what's up with the one failure — it appears to be somewhat consistent, but is probably unrelated to these changes?).
...
<a id='comment:12'></a>Merge failure on top of:
...
```json
{
    "body": "<a id='comment:14'></a>Trivial merge, but let's see what the patchbot says.",
...
<a id='comment:14'></a>Trivial merge, but let's see what the patchbot says.
...
```json
{
    "body": "<a id='comment:15'></a>Morally green.",
    "created_at": "2022-03-01T12:05:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33319",
    "type": "issue_comment",
...
<a id='comment:15'></a>Morally green.
