# Issue 33677: Fix typos in MixedIntegerLinearProgram backends

archive/issues_033440.json:
```json
{
    "assignees": [],
    "body": "The methods `variable_upper_bound` and `variable_lower_bound` has argument value=False by default; their docstring mis-stated as \"When set to `None` (default)\".\n\nCC:  @mkoeppe\n\nKeywords: **lp**\n\nBranch/Commit: **[b91ab9f](https://github.com/sagemath/sagetrac-mirror/commit/b91ab9fbe905cba2c7cde0822b50c824f7eec857)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Yuan Zhou**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33677_\n\n",
    "closed_at": "2022-08-30T19:03:24Z",
    "created_at": "2022-04-11T07:15:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix typos in MixedIntegerLinearProgram backends",
    "type": "issue",
    "updated_at": "2022-08-30T19:03:24Z",
    "url": "https://github.com/sagemath/sage/issues/33677",
    "user": "https://github.com/yuan-zhou"
}
```
The methods `variable_upper_bound` and `variable_lower_bound` has argument value=False by default; their docstring mis-stated as "When set to `None` (default)".

CC:  @mkoeppe

Keywords: **lp**

Branch/Commit: **[b91ab9f](https://github.com/sagemath/sagetrac-mirror/commit/b91ab9fbe905cba2c7cde0822b50c824f7eec857)**

Reviewer: **Matthias Koeppe**

Author: **Yuan Zhou**

_Issue created by migration from https://trac.sagemath.org/ticket/33677_





---

archive/issue_events_443983.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-04-11T07:15:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443983"
}
```



---

archive/issue_events_443984.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-04-11T07:15:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443984"
}
```



---

archive/issue_events_443985.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-04-11T07:15:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443985"
}
```



---

archive/issue_comments_547492.json:
```json
{
    "body": "Branch: **[u/yzh/fix_typos_in_mixedintegerlinearprogram_backends](https://github.com/sagemath/sagetrac-mirror/tree/u/yzh/fix_typos_in_mixedintegerlinearprogram_backends)**",
    "created_at": "2022-04-11T07:24:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33677#issuecomment-547492",
    "user": "https://github.com/yuan-zhou"
}
```

Branch: **[u/yzh/fix_typos_in_mixedintegerlinearprogram_backends](https://github.com/sagemath/sagetrac-mirror/tree/u/yzh/fix_typos_in_mixedintegerlinearprogram_backends)**



---

archive/issue_events_443986.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-04-11T07:24:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443986"
}
```



---

archive/issue_comments_547493.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d39af6d126b465b59a264d1cbf40acef7b149be7\">d39af6d</a></td><td><code>typo in docstings value=False by default in variable_upper_bound etc.</code></td></tr></table>\n",
    "created_at": "2022-04-11T07:24:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33677#issuecomment-547493",
    "user": "https://github.com/yuan-zhou"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d39af6d126b465b59a264d1cbf40acef7b149be7">d39af6d</a></td><td><code>typo in docstings value=False by default in variable_upper_bound etc.</code></td></tr></table>




---

archive/issue_comments_547494.json:
```json
{
    "body": "Commit: **[d39af6d](https://github.com/sagemath/sagetrac-mirror/commit/d39af6d126b465b59a264d1cbf40acef7b149be7)**",
    "created_at": "2022-04-11T07:24:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33677#issuecomment-547494",
    "user": "https://github.com/yuan-zhou"
}
```

Commit: **[d39af6d](https://github.com/sagemath/sagetrac-mirror/commit/d39af6d126b465b59a264d1cbf40acef7b149be7)**



---

archive/issue_events_443987.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-04-11T07:25:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443987"
}
```



---

archive/issue_events_443988.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-04-11T07:25:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443988"
}
```



---

archive/issue_events_443989.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-04-11T07:25:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443989"
}
```



---

archive/issue_events_443990.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-17T19:59:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443990"
}
```



---

archive/issue_events_443991.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-17T19:59:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443991"
}
```



---

archive/issue_comments_547495.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nwhat's the status here?",
    "created_at": "2022-08-26T00:20:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33677#issuecomment-547495",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

what's the status here?



---

archive/issue_events_443992.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-08-26T00:29:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443992"
}
```



---

archive/issue_events_443993.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-08-26T00:29:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443993"
}
```



---

archive/issue_events_443994.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-08-26T00:29:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443994"
}
```



---

archive/issue_events_443995.json:
```json
{
    "actor": "https://github.com/yuan-zhou",
    "created_at": "2022-08-26T00:29:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443995"
}
```



---

archive/issue_comments_547496.json:
```json
{
    "body": "Changed branch from **[u/yzh/fix_typos_in_mixedintegerlinearprogram_backends](https://github.com/sagemath/sagetrac-mirror/tree/u/yzh/fix_typos_in_mixedintegerlinearprogram_backends)** to **[u/mkoeppe/fix_typos_in_mixedintegerlinearprogram_backends](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/fix_typos_in_mixedintegerlinearprogram_backends)**",
    "created_at": "2022-08-26T00:35:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33677#issuecomment-547496",
    "user": "https://github.com/mkoeppe"
}
```

Changed branch from **[u/yzh/fix_typos_in_mixedintegerlinearprogram_backends](https://github.com/sagemath/sagetrac-mirror/tree/u/yzh/fix_typos_in_mixedintegerlinearprogram_backends)** to **[u/mkoeppe/fix_typos_in_mixedintegerlinearprogram_backends](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/fix_typos_in_mixedintegerlinearprogram_backends)**



---

archive/issue_comments_547497.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nrebased\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b91ab9fbe905cba2c7cde0822b50c824f7eec857\">b91ab9f</a></td><td><code>typo in docstings value=False by default in variable_upper_bound etc.</code></td></tr></table>\n",
    "created_at": "2022-08-26T00:36:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33677#issuecomment-547497",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

rebased

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b91ab9fbe905cba2c7cde0822b50c824f7eec857">b91ab9f</a></td><td><code>typo in docstings value=False by default in variable_upper_bound etc.</code></td></tr></table>




---

archive/issue_comments_547498.json:
```json
{
    "body": "Changed commit from **[d39af6d](https://github.com/sagemath/sagetrac-mirror/commit/d39af6d126b465b59a264d1cbf40acef7b149be7)** to **[b91ab9f](https://github.com/sagemath/sagetrac-mirror/commit/b91ab9fbe905cba2c7cde0822b50c824f7eec857)**",
    "created_at": "2022-08-26T00:36:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33677#issuecomment-547498",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[d39af6d](https://github.com/sagemath/sagetrac-mirror/commit/d39af6d126b465b59a264d1cbf40acef7b149be7)** to **[b91ab9f](https://github.com/sagemath/sagetrac-mirror/commit/b91ab9fbe905cba2c7cde0822b50c824f7eec857)**



---

archive/issue_events_443996.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-26T03:41:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443996"
}
```



---

archive/issue_events_443997.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-26T03:41:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443997"
}
```



---

archive/issue_comments_547499.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2022-08-26T03:41:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33677#issuecomment-547499",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_443998.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-08-30T19:03:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443998"
}
```



---

archive/issue_events_443999.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7b69f459d6cffd3cd7c9560562b8d8fce09a8f13",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-08-30T19:03:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33677#event-443999"
}
```



---

archive/issue_comments_547500.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/fix_typos_in_mixedintegerlinearprogram_backends](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/fix_typos_in_mixedintegerlinearprogram_backends)** to **[b91ab9f](https://github.com/sagemath/sagetrac-mirror/commit/b91ab9fbe905cba2c7cde0822b50c824f7eec857)**",
    "created_at": "2022-08-30T19:03:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33677#issuecomment-547500",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/fix_typos_in_mixedintegerlinearprogram_backends](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/fix_typos_in_mixedintegerlinearprogram_backends)** to **[b91ab9f](https://github.com/sagemath/sagetrac-mirror/commit/b91ab9fbe905cba2c7cde0822b50c824f7eec857)**
