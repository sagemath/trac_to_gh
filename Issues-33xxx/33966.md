# Issue 33966: KnotInfo interface returns wrong braid_notation for the unknot

archive/issues_033729.json:
```json
{
    "assignees": [],
    "body": "The original database does not provide a value for the `braid_notation` of the unknot. Thus this value has been set individually in the interface, but unfortunately this value corresponds to a disjoint union of two unknots. This bug remained unnoticed because the construction of a link from a braid in Sage erases all additional unknots.\n\n\nBranch/Commit: **[99345e1](https://github.com/sagemath/sagetrac-mirror/commit/99345e14d8182d09ac8da35a53af78964d77c2e8)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Sebastian Oehms**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/33966_\n\n",
    "closed_at": "2022-06-19T15:24:21Z",
    "created_at": "2022-06-09T16:40:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "KnotInfo interface returns wrong braid_notation for the unknot",
    "type": "issue",
    "updated_at": "2022-06-19T15:24:21Z",
    "url": "https://github.com/sagemath/sage/issues/33966",
    "user": "https://github.com/soehms"
}
```
The original database does not provide a value for the `braid_notation` of the unknot. Thus this value has been set individually in the interface, but unfortunately this value corresponds to a disjoint union of two unknots. This bug remained unnoticed because the construction of a link from a braid in Sage erases all additional unknots.


Branch/Commit: **[99345e1](https://github.com/sagemath/sagetrac-mirror/commit/99345e14d8182d09ac8da35a53af78964d77c2e8)**

Reviewer: **Travis Scrimshaw**

Author: **Sebastian Oehms**

_Issue created by migration from https://trac.sagemath.org/ticket/33966_





---

archive/issue_events_447147.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2022-06-09T16:40:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33966#event-447147"
}
```



---

archive/issue_events_447148.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2022-06-09T16:40:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33966#event-447148"
}
```



---

archive/issue_events_447149.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2022-06-09T16:40:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33966#event-447149"
}
```



---

archive/issue_events_447150.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2022-06-09T16:40:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33966#event-447150"
}
```



---

archive/issue_comments_552109.json:
```json
{
    "body": "Branch: **[u/soehms/fix_unknot_braid_notation_knotinfo_33966](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/fix_unknot_braid_notation_knotinfo_33966)**",
    "created_at": "2022-06-09T16:45:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33966#issuecomment-552109",
    "user": "https://github.com/soehms"
}
```

Branch: **[u/soehms/fix_unknot_braid_notation_knotinfo_33966](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/fix_unknot_braid_notation_knotinfo_33966)**



---

archive/issue_events_447151.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2022-06-09T16:46:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33966#event-447151"
}
```



---

archive/issue_comments_552110.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/99345e14d8182d09ac8da35a53af78964d77c2e8\">99345e1</a></td><td><code>33966: initial</code></td></tr></table>\n",
    "created_at": "2022-06-09T16:46:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33966#issuecomment-552110",
    "user": "https://github.com/soehms"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/99345e14d8182d09ac8da35a53af78964d77c2e8">99345e1</a></td><td><code>33966: initial</code></td></tr></table>




---

archive/issue_comments_552111.json:
```json
{
    "body": "Commit: **[99345e1](https://github.com/sagemath/sagetrac-mirror/commit/99345e14d8182d09ac8da35a53af78964d77c2e8)**",
    "created_at": "2022-06-09T16:46:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33966#issuecomment-552111",
    "user": "https://github.com/soehms"
}
```

Commit: **[99345e1](https://github.com/sagemath/sagetrac-mirror/commit/99345e14d8182d09ac8da35a53af78964d77c2e8)**



---

archive/issue_comments_552112.json:
```json
{
    "body": "Author: **Sebastian Oehms**",
    "created_at": "2022-06-09T16:46:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33966#issuecomment-552112",
    "user": "https://github.com/soehms"
}
```

Author: **Sebastian Oehms**



---

archive/issue_comments_552113.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-06-16T00:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33966#issuecomment-552113",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_447152.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-06-16T00:57:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33966#event-447152"
}
```



---

archive/issue_events_447153.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-06-16T00:57:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33966#event-447153"
}
```



---

archive/issue_comments_552114.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2022-06-16T00:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33966#issuecomment-552114",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_552115.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks",
    "created_at": "2022-06-17T09:32:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33966#issuecomment-552115",
    "user": "https://github.com/soehms"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks



---

archive/issue_comments_552116.json:
```json
{
    "body": "Changed branch from **[u/soehms/fix_unknot_braid_notation_knotinfo_33966](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/fix_unknot_braid_notation_knotinfo_33966)** to **[99345e1](https://github.com/sagemath/sagetrac-mirror/commit/99345e14d8182d09ac8da35a53af78964d77c2e8)**",
    "created_at": "2022-06-19T15:24:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/33966#issuecomment-552116",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/soehms/fix_unknot_braid_notation_knotinfo_33966](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/fix_unknot_braid_notation_knotinfo_33966)** to **[99345e1](https://github.com/sagemath/sagetrac-mirror/commit/99345e14d8182d09ac8da35a53af78964d77c2e8)**



---

archive/issue_events_447154.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-06-19T15:24:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33966#event-447154"
}
```



---

archive/issue_events_447155.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e79f6ff49a340a63ebdb9027ad7470220b25ffa9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-06-19T15:24:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/33966",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/33966#event-447155"
}
```
