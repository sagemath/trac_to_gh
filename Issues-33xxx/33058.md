...
```json
{
    "body": "<a id='comment:1'></a>`round(random(), 5)` is returning an element of `RDF`, so this should either (a) have a tolerance associated with it, or (b) be using an exact ring.\n\nMoreover, this doctest shouldn't be repeating the same test 20 times... it looks like there are four other tests in this file doing the same thing.",
...
<a id='comment:1'></a>`round(random(), 5)` is returning an element of `RDF`, so this should either (a) have a tolerance associated with it, or (b) be using an exact ring.
...
<a id='comment:3'></a>Typo: `on a random graphs`
...
<a id='comment:5'></a>Sorry, there is actually one more typo here:
