...
```json
{
    "body": "<a id='comment:1'></a>no -- you can't combine command line options like that.  this isn't a bug\nbut a not implemented yet issue.",
...
<a id='comment:1'></a>no -- you can't combine command line options like that.  this isn't a bug
but a not implemented yet issue.
...
```json
{
    "body": "<a id='comment:2'></a>This should be fixable, but the long term goal is to do a proper rewrite of the command line options.\n\nCheers,\n\nMichael",
...
<a id='comment:2'></a>This should be fixable, but the long term goal is to do a proper rewrite of the command line options.
...
```json
{
    "body": "<a id='comment:5'></a>See also #180 for a bunch of related failures due to the option parsing being dumb :o\n\nCheers,\n\nMichael",
...
<a id='comment:5'></a>See also #180 for a bunch of related failures due to the option parsing being dumb :o
...
```json
{
    "body": "<a id='comment:7'></a>Note that sage -bn now builds, then does notebook, though of course it doesn't fix the issue here.",
...
<a id='comment:7'></a>Note that sage -bn now builds, then does notebook, though of course it doesn't fix the issue here.
...
<a id='comment:8'></a>Attachment [trac_21-sagenb.patch](tarball://root/attachments/some-uuid/ticket21/trac_21-sagenb.patch) by @jhpalmieri created at 2010-03-19 15:50:37
...
<a id='comment:10'></a>I've marked this as "needs review", but it might need work.  In the previously cited thread from sage-devel, there was the following suggestion:
...
<a id='comment:11'></a>Attachment [sage.diff](tarball://root/attachments/some-uuid/ticket21/sage.diff) by @jhpalmieri created at 2010-03-19 21:22:45
...
```json
{
    "body": "<a id='comment:12'></a>Note that Python 2.7 will include the [argparse](http://docs.python.org/dev/library/argparse.html) module, which might be easier to use than optparse.",
...
<a id='comment:12'></a>Note that Python 2.7 will include the [argparse](http://docs.python.org/dev/library/argparse.html) module, which might be easier to use than optparse.
...
```json
{
    "body": "<a id='comment:13'></a>Wow, this is really fantastic.",
    "created_at": "2010-06-03T04:33:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21",
    "type": "issue_comment",
...
<a id='comment:13'></a>Wow, this is really fantastic.
...
<a id='comment:15'></a>I'm changing this back to needs review.  I realized when trying to apply it that I had got confused about how to even apply this.
...
```json
{
    "body": "<a id='comment:16'></a>Attachment [trac_21-sage.patch](tarball://root/attachments/some-uuid/ticket21/trac_21-sage.patch) by @jhpalmieri created at 2010-06-03 06:04:06\n\n> John, can you please post clear directions about how to use the patches attached to this ticket?\n\n\nSorry, some parts needed rebasing.  I think it's okay now.  I've modified the summary with instructions for how to apply the patches.",
...
<a id='comment:16'></a>Attachment [trac_21-sage.patch](tarball://root/attachments/some-uuid/ticket21/trac_21-sage.patch) by @jhpalmieri created at 2010-06-03 06:04:06
...
```json
{
    "body": "<a id='comment:18'></a>It appears like sage-sage.py managed to get itself into 4.4.3 (William, could this have happened when you were trying to apply it). Also, sage-apply-ticket has greatly changed since the patches were posted, so I am changing this back to needs work.",
...
<a id='comment:18'></a>It appears like sage-sage.py managed to get itself into 4.4.3 (William, could this have happened when you were trying to apply it). Also, sage-apply-ticket has greatly changed since the patches were posted, so I am changing this back to needs work.
...
```json
{
    "body": "<a id='comment:19'></a>As far as I know, the file sage-sage.py was accidentally added in 4.4.3, but the release manager should probably delete it: it's not used anywhere.\n\nI don't have the time to rebase it right now, so if anyone else wants to work it, that would be great.",
...
<a id='comment:19'></a>As far as I know, the file sage-sage.py was accidentally added in 4.4.3, but the release manager should probably delete it: it's not used anywhere.
...
<a id='comment:20'></a>I've uploaded a patch rebased on 4.6.1.alpha1 (I also added in #8654 while I was at it). One thing I noted during rebasing it is that -tp no longer works (and hence -btp which was added since 4.4.3), I don't know optparse well enough to come up with a solution (if we want to provide one). The documentation patches will need to be rebased as well.
...
```json
{
    "body": "<a id='comment:22'></a>ohanar mentioned on #8654 that this should be rewritten (or modified) to use argparse as optparse is deprecated in Python 2.7, so I'm adding #9958 to the dependencies because argparse is shipped with 2.7. Alternatively we could make an spkg for argparse.",
    "created_at": "2011-11-01T14:49:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21",
    "type": "issue_comment",
...
<a id='comment:22'></a>ohanar mentioned on #8654 that this should be rewritten (or modified) to use argparse as optparse is deprecated in Python 2.7, so I'm adding #9958 to the dependencies because argparse is shipped with 2.7. Alternatively we could make an spkg for argparse.
...
```json
{
    "body": "<a id='comment:23'></a>I think waiting until 2.7 is fine.",
...
<a id='comment:23'></a>I think waiting until 2.7 is fine.
...
<a id='comment:24'></a>Can we edit the title and description of this ticket to reflect what we're actually trying to do here? (I would, but preferably someone who has been working on it should do it.) The goal is to completely replace the command line handler (currently a shell script) with a Python script which uses argparse for extensibility, right?
...
```json
{
    "body": "<a id='comment:26'></a>People CC'd to this ticket probably already know this, but the above mentioned #9958 is merged, so we can use Python 2.7 default modules such as argparse now.",
...
<a id='comment:26'></a>People CC'd to this ticket probably already know this, but the above mentioned #9958 is merged, so we can use Python 2.7 default modules such as argparse now.
...
<a id='comment:27'></a>Replying to [kini](#comment%3A26):
> People CC'd to this ticket probably already know this, but the above mentioned #9958 is merged, so we can use Python 2.7 default modules such as argparse now.
...
<a id='comment:28'></a>Capital! :) As it's a large project maybe you could post WIPs once in a while?
...
<a id='comment:29'></a>Replying to [kini](#comment%3A28):
> Capital! :) As it's a large project maybe you could post WIPs once in a while?
...
```json
{
    "body": "<a id='comment:30'></a>We're going to try to get this in as a Sage 6.0 goal along with #13015.",
    "created_at": "2012-05-25T20:16:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21",
    "type": "issue_comment",
...
<a id='comment:30'></a>We're going to try to get this in as a Sage 6.0 goal along with #13015.
...
<a id='comment:31'></a>patchbot: apply 21-scripts-4.6.1.alpha1.patch
...
```json
{
    "body": "<a id='comment:33'></a>Going with `argparse` is probably a good idea, but I would still like a special \"pre-parser\" in `bash` to handle to options\n\n```\nsage --sh\nsage -i\nsage -f\n```\n(or whatever version they will become)\n\nI think these must be available from the start, before Python has been built.",
...
<a id='comment:33'></a>Going with `argparse` is probably a good idea, but I would still like a special "pre-parser" in `bash` to handle to options
...
```json
{
    "body": "<a id='comment:34'></a>ohanar and I talked about this a bit at SD40.5. What do you think about having a totally separate executable (rather than a preparser) called `sage-sh`? Stuff like the current `sage -gap` could be called rapidly with `sage-sh -c gap`, or slowly via python with `sage gap` or what-have-you.",
...
<a id='comment:34'></a>ohanar and I talked about this a bit at SD40.5. What do you think about having a totally separate executable (rather than a preparser) called `sage-sh`? Stuff like the current `sage -gap` could be called rapidly with `sage-sh -c gap`, or slowly via python with `sage gap` or what-have-you.
...
<a id='comment:35'></a>William, among others, as alluded to [above](#comment%3A11), was pretty insistent that commands like `sage --gp` execute quickly, without the overhead that starting Python entails. Hence the approach in my patches on this ticket. If you want to insist that people use `sage-sh -c gp` instead, I don't think it can get a positive review unless the approach gets a positive response on sage-devel.
...
<a id='comment:36'></a>Rather than building on your patches on this ticket, we're proposing completely rewriting the command line interface of Sage, and that includes changing invocation methods. That's why we've set the milestone to sage-6.0 - the idea is to tie it with the layout restructuring and new development interface that will come with the git transition (#13015). We figured that the git transition is a big enough change to warrant a major version bump (though that might end up being sage-7.0 or whatever, depending on the timeline).
...
```json
{
    "body": "<a id='comment:37'></a>Oh, I understood that you were starting from scratch. I was responding to your proposal about `sage-sh`, suggesting that not all users would be happy with that solution.",
...
<a id='comment:37'></a>Oh, I understood that you were starting from scratch. I was responding to your proposal about `sage-sh`, suggesting that not all users would be happy with that solution.
...
```json
{
    "body": "<a id='comment:38'></a>If you need a separate `bash` script anyway to parse some options like `--sh`, then it's a trivial exercise to parse more options (like `--gp`) in that same script.",
...
<a id='comment:38'></a>If you need a separate `bash` script anyway to parse some options like `--sh`, then it's a trivial exercise to parse more options (like `--gp`) in that same script.
...
<a id='comment:39'></a>jdemeyer: I think you misunderstand. Currently, `sage` is a bash script, which checks some options, then starts up Python to handle the rest (separately in various different code branches, no less). What we're proposing is the following scenario:
...
<a id='comment:40'></a>Doesn't this contradict?
...
<a id='comment:41'></a>1) That comment is 3 years old and I imagine it's up for discussion by now.
...
<a id='comment:42'></a>OK I see.  So we would have two "top-level" scripts then:
`./sage` (Python-based) and `./sage-sh` (`bash`-based).  Yes, that would be fine for me.
...
<a id='comment:46'></a>Replying to [kini](#comment%3A45):
> Right, we might have a very thin bash wrapper that loads `sage-env` (which will be a bash script) before the main Python script. Or, since `sage-env` ideally should just set up environment variables and do nothing else (right?), we could turn it into a config file that was read independently by `sage-sh` and by `sage`. This would also allow us to rely less on environment variables for random things seemingly unrelated to the shell.
...
<a id='comment:48'></a>Replying to [kini](#comment%3A45):
> Or, since `sage-env` ideally should just set up environment variables and do nothing else
...
```json
{
    "body": "<a id='comment:53'></a>The command line interface continues to evolve; can someone (who cares) give a summary of what still would be needed? [comment:24](#comment%3A24) still hasn't been resolved, and[comment:33](#comment%3A33) (as well as quick, non-Python-starting, use of `sage -maxima` and friends) seems quite relevant.",
...
<a id='comment:53'></a>The command line interface continues to evolve; can someone (who cares) give a summary of what still would be needed? [comment:24](#comment%3A24) still hasn't been resolved, and[comment:33](#comment%3A33) (as well as quick, non-Python-starting, use of `sage -maxima` and friends) seems quite relevant.
...
<a id='comment:55'></a>Still reading up on this ticket, and don't have any comments to add yet to the existing discussion.  But one question I have in general: Is anyone opposed at all to the idea of creating a new sub-command based interface, more like git, than the slightly unusual interface that uses single-character flags for subcommands?  E.g. replace `sage -t` with `sage test`.  Yes, it's more to type, but only by two characters, and is much less unusual.  The old `-t` could still be supported very easily for backwards compatibility, but perhaps with a deprecation warning.
...
<a id='comment:56'></a>William Stein and I were just talking about this idea yesterday. Something like this?
...
<a id='comment:57'></a>Yes, something quite like that.
...
```json
{
    "body": "<a id='comment:58'></a>(I should add, that's a very nice mock-up of what such an interface would look like, so thank you for that.)",
...
<a id='comment:58'></a>(I should add, that's a very nice mock-up of what such an interface would look like, so thank you for that.)
...
<a id='comment:60'></a>We should also think to what extent the build system should be exposed under the `sage` command. For example, we now have
...
```json
{
    "body": "<a id='comment:61'></a>Needless to say, many people don't even know the subtle differences between the above commands.",
...
<a id='comment:61'></a>Needless to say, many people don't even know the subtle differences between the above commands.
...
<a id='comment:62'></a>Replying to [jdemeyer](#comment%3A59):
> Replying to [embray](#comment%3A55):
> > E.g. replace `sage -t` with `sage test`.
...
```json
{
    "body": "<a id='comment:66'></a>Strong -1 \"wontfix\" for the idea of making an incompatible change toward using \"subcommands\" after 14 years of the existence of the `sage` script.",
...
<a id='comment:66'></a>Strong -1 "wontfix" for the idea of making an incompatible change toward using "subcommands" after 14 years of the existence of the `sage` script.
...
<a id='comment:67'></a>I think the current interface of the `sage` script is pretty clumsy and dated by modern standards.  It can and should be made more user-friendly.  Just as one example of an advantage of subcommands is it makes the help documentation vastly more digestible.  I wouldn't propose changing it without maintaining backwards compatibility though.
...
<a id='comment:68'></a>+1 from me to changing the sage script to support subcommands, especially if we can somehow do it in a way that preserves compatibility with the current parsing.  I wonder to what extent the following is possible:
...
<a id='comment:69'></a>Since all of the sage script's current "subcommands" (e.g. `sage -b`, `sage -i`) all start with hyphens, and new subcommands with be non-hyphenated, I think it would work to support both without too much ambiguity but I'd be interested in a counter-example.  `sage <filename.{py,sage}>` would still work since no sub-command would be confused with a runnable script filename.
...
```json
{
    "body": "<a id='comment:71'></a>> I might still just write it as a shell script.\n\n\n+1\n\nI should have just said \"in my experience, structuring command line parsing code as subcommands (implemented in any language) can result in very nice modular code.\"",
    "created_at": "2020-09-01T13:39:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21#issuecomment-164",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:71'></a>> I might still just write it as a shell script.
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/21",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21#issuecomment-165",
    "user": "https://github.com/mkoeppe"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/21",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21#issuecomment-168",
    "user": "https://github.com/nbruin"
}
```
...
```json
{
    "body": "<a id='comment:76'></a>Replying to [nbruin](#comment%3A75):\n> The traditional reason for having dashes in front of options/subcommands is to remove ambiguity from `sage notebook` (to run the file notebook) and `sage notebook` (to start the notebook). For that reason, I think we can only have subcommands for `sage` if `sage <file>` would have no meaning. I don't think we can discard this main function of `sage`. I think with jupyter, where there is one VERY common use, it's already a mistake to go with a subcommand design.\n> \n> Extrapolating from that, I think that using subcommands for the `sage` script is also the wrong fit. It works well with `git`, but users really interact differently with git.\n\n\nI fully agree.",
    "created_at": "2020-09-04T17:48:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21#issuecomment-169",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:76'></a>Replying to [nbruin](#comment%3A75):
> The traditional reason for having dashes in front of options/subcommands is to remove ambiguity from `sage notebook` (to run the file notebook) and `sage notebook` (to start the notebook). For that reason, I think we can only have subcommands for `sage` if `sage <file>` would have no meaning. I don't think we can discard this main function of `sage`. I think with jupyter, where there is one VERY common use, it's already a mistake to go with a subcommand design.
> 
> Extrapolating from that, I think that using subcommands for the `sage` script is also the wrong fit. It works well with `git`, but users really interact differently with git.
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/21",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21#issuecomment-170",
    "user": "https://github.com/kini"
}
```
