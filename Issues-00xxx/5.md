# Issue 5: full keyboard mode for notebook

archive/issues_000005.json:
```json
{
    "body": "Currently I need to switch between my keyboard and mouse a lot to operate the notebook, especially when using it for development. For example, I would like to be able to restart the server without going for the mouse. There should be a way of assigning keyboard shortcuts.\n\n\nAssignee: boothby\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/5\n\n",
    "closed_at": "2008-03-16T13:49:15Z",
    "created_at": "2006-09-12T00:45:22Z",
    "labels": [
        "component: notebook",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "full keyboard mode for notebook",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```
Currently I need to switch between my keyboard and mouse a lot to operate the notebook, especially when using it for development. For example, I would like to be able to restart the server without going for the mouse. There should be a way of assigning keyboard shortcuts.


Assignee: boothby

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/5





---

archive/issue_comments_000016.json:
```json
{
    "body": "Changing component from basic arithmetic to notebook.",
    "created_at": "2007-08-22T19:41:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5#issuecomment-16",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing component from basic arithmetic to notebook.



---

archive/issue_comments_000017.json:
```json
{
    "body": "Changing assignee from somebody to boothby.",
    "created_at": "2007-08-22T19:41:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5#issuecomment-17",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing assignee from somebody to boothby.



---

archive/issue_events_000005.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-09-11T02:41:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5",
    "milestone": "sage-feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5#event-5"
}
```



---

archive/issue_comments_000018.json:
```json
{
    "body": "<a id='comment:3'></a>Apparently I'm the only person who has ever requested this, and I don't particularly want it any more anyway, so marking this ticket invalid.",
    "created_at": "2008-03-16T13:49:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5#issuecomment-18",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```

<a id='comment:3'></a>Apparently I'm the only person who has ever requested this, and I don't particularly want it any more anyway, so marking this ticket invalid.



---

archive/issue_events_000006.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/dmharvey",
    "created_at": "2008-03-16T13:49:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5#event-6"
}
```



---

archive/issue_comments_000019.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2008-03-16T13:49:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5#issuecomment-19",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```

Resolution: invalid



---

archive/issue_events_000007.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T17:43:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5",
    "milestone": "sage-feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5#event-7"
}
```
