# Issue 497: add minpoly command to finite field elements

archive/issues_000497.json:
```json
{
    "body": "Make it so a.minpoly() works when a is in a finite field.\n\nAssignee: @pdenapo\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/497\n\n",
    "closed_at": "2007-09-02T05:49:22Z",
    "created_at": "2007-08-27T22:15:51Z",
    "labels": [
        "component: basic arithmetic"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.4",
    "title": "add minpoly command to finite field elements",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/497",
    "user": "https://github.com/williamstein"
}
```
Make it so a.minpoly() works when a is in a finite field.

Assignee: @pdenapo

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/497





---

archive/issue_comments_002475.json:
```json
{
    "body": "Changing assignee from pablo to @pdenapo.",
    "created_at": "2007-09-01T21:02:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/497#issuecomment-2475",
    "user": "https://github.com/pdenapo"
}
```

Changing assignee from pablo to @pdenapo.



---

archive/issue_comments_002476.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-09-01T21:02:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/497#issuecomment-2476",
    "user": "https://github.com/pdenapo"
}
```

Changing status from new to assigned.



---

archive/issue_comments_002477.json:
```json
{
    "body": "<a id='comment:2'></a>Attachment [minpoly_FiniteFieldElement.patch](tarball://root/attachments/some-uuid/ticket497/minpoly_FiniteFieldElement.patch) by @pdenapo created at 2007-09-01 21:08:37\n\nHere is an updated version of my patch. It also adds a default variable name 'x' to charpoly.",
    "created_at": "2007-09-01T21:08:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/497#issuecomment-2477",
    "user": "https://github.com/pdenapo"
}
```

<a id='comment:2'></a>Attachment [minpoly_FiniteFieldElement.patch](tarball://root/attachments/some-uuid/ticket497/minpoly_FiniteFieldElement.patch) by @pdenapo created at 2007-09-01 21:08:37

Here is an updated version of my patch. It also adds a default variable name 'x' to charpoly.



---

archive/issue_comments_002478.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-09-02T05:49:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/497#issuecomment-2478",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed



---

archive/issue_events_001268.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-02T05:49:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/497",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/497#event-1268"
}
```



---

archive/issue_events_001269.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-02T05:49:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/497",
    "milestone": "sage-2.8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/497#event-1269"
}
```
