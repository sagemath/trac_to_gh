# Issue 647: [with patch] create a quaternion algebra element class

archive/issues_000647.json:
```json
{
    "body": "Assignee: somebody\n\nFrom Alex Ghitza:\n\n```\nI fiddled a bit more with the quaternion algebras, this time with\nelements.  This is still warm-up for doing something more serious with\norders and ideals, starting with implementing the code by\nRodriguez-Villegas et al.\n```\n\nSee attached patch.\n\nIssue created by migration from https://trac.sagemath.org/ticket/647\n\n",
    "closed_at": "2007-09-20T23:48:15Z",
    "created_at": "2007-09-13T14:07:10Z",
    "labels": [
        "component: basic arithmetic"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.5",
    "title": "[with patch] create a quaternion algebra element class",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/647",
    "user": "https://github.com/williamstein"
}
```
Assignee: somebody

From Alex Ghitza:

```
I fiddled a bit more with the quaternion algebras, this time with
elements.  This is still warm-up for doing something more serious with
orders and ideals, starting with implementing the code by
Rodriguez-Villegas et al.
```

See attached patch.

Issue created by migration from https://trac.sagemath.org/ticket/647





---

archive/issue_comments_003349.json:
```json
{
    "body": "Attachment [quatalgelm.hg](tarball://root/attachments/some-uuid/ticket647/quatalgelm.hg) by @williamstein created at 2007-09-13 14:07:24",
    "created_at": "2007-09-13T14:07:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/647#issuecomment-3349",
    "user": "https://github.com/williamstein"
}
```

Attachment [quatalgelm.hg](tarball://root/attachments/some-uuid/ticket647/quatalgelm.hg) by @williamstein created at 2007-09-13 14:07:24



---

archive/issue_comments_003350.json:
```json
{
    "body": "The main thing that needs to happen to close this ticket is that 1-2 people need to look at this patch\nand say \"yep, looks good\".",
    "created_at": "2007-09-13T14:08:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/647#issuecomment-3350",
    "user": "https://github.com/williamstein"
}
```

The main thing that needs to happen to close this ticket is that 1-2 people need to look at this patch
and say "yep, looks good".



---

archive/issue_comments_003351.json:
```json
{
    "body": "OK, I just looked at it, and it *has* no doctests at all for the new functions it introduces.  Please add these.",
    "created_at": "2007-09-13T14:08:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/647#issuecomment-3351",
    "user": "https://github.com/williamstein"
}
```

OK, I just looked at it, and it *has* no doctests at all for the new functions it introduces.  Please add these.



---

archive/issue_comments_003352.json:
```json
{
    "body": "Attachment [newquatalgelm.hg](tarball://root/attachments/some-uuid/ticket647/newquatalgelm.hg) by @aghitza created at 2007-09-20 15:39:17\n\nI inserted doctests and attached the new version newquatalgelm.hg.",
    "created_at": "2007-09-20T15:39:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/647#issuecomment-3352",
    "user": "https://github.com/aghitza"
}
```

Attachment [newquatalgelm.hg](tarball://root/attachments/some-uuid/ticket647/newquatalgelm.hg) by @aghitza created at 2007-09-20 15:39:17

I inserted doctests and attached the new version newquatalgelm.hg.



---

archive/issue_comments_003353.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-09-20T23:48:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/647#issuecomment-3353",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed



---

archive/issue_events_001727.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-20T23:48:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/647",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/647#event-1727"
}
```
