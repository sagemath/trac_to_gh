# Issue 138: HTML reference documentation incorrect links

archive/issues_000138.json:
```json
{
    "assignees": [],
    "body": "There seems to be a problem with the SAGE reference manual (at least the one that is on the web).\n\nWhen I go to:\nhttp://sage.math.washington.edu/SAGE/doc/html/ref/index.html\n\nAnd click on, for example, the link called \"Matrices and Spaces of Matrices\", I am brought instead to the root page for \"Algebras\":\nhttp://sage.math.washington.edu/SAGE/doc/html/ref/node214.html\n\nClicking on other links results in arriving at similarly undersired pages.\n\nAssignee: **@williamstein**\n\nKeywords: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/138_\n\n",
    "closed_at": "2007-01-12T23:25:07Z",
    "created_at": "2006-10-19T03:07:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "HTML reference documentation incorrect links",
    "type": "issue",
    "updated_at": "2007-01-12T23:25:07Z",
    "url": "https://github.com/sagemath/sage/issues/138",
    "user": "https://github.com/bobmoretti"
}
```
There seems to be a problem with the SAGE reference manual (at least the one that is on the web).

When I go to:
http://sage.math.washington.edu/SAGE/doc/html/ref/index.html

And click on, for example, the link called "Matrices and Spaces of Matrices", I am brought instead to the root page for "Algebras":
http://sage.math.washington.edu/SAGE/doc/html/ref/node214.html

Clicking on other links results in arriving at similarly undersired pages.

Assignee: **@williamstein**

Keywords: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/138_





---

archive/issue_events_000907.json:
```json
{
    "actor": "https://github.com/bobmoretti",
    "created_at": "2006-10-19T03:07:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/138",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/138#event-907"
}
```



---

archive/issue_events_000908.json:
```json
{
    "actor": "https://github.com/bobmoretti",
    "created_at": "2006-10-19T03:07:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/138",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/138#event-908"
}
```



---

archive/issue_events_000909.json:
```json
{
    "actor": "https://github.com/bobmoretti",
    "created_at": "2006-10-19T03:07:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/138",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/138#event-909"
}
```



---

archive/issue_comments_000477.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis problem no longer exists.",
    "created_at": "2007-01-12T23:25:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/138#issuecomment-477",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">Comment 1</div>

This problem no longer exists.



---

archive/issue_events_000910.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-12T23:25:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/138",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/138#event-910"
}
```
