# Issue 685: Make Eisenstein series code handle eisenstein series with character

archive/issues_000685.json:
```json
{
    "body": "The eisenstein_series_qexp function currently only handles eisenstein series for level 1. It wouldn't be hard to make this handle eisenstein series for higher level and with character. I'm going to take care of this soon, but this is here in case I forget. ;)\n\n**Assignee:** @craigcitro\n\nIssue created by migration from https://trac.sagemath.org/ticket/685\n\n",
    "closed_at": "2014-06-06T11:01:15Z",
    "created_at": "2007-09-18T05:14:14Z",
    "labels": [
        "component: modular forms",
        "minor",
        "enhancement",
        "invalid"
    ],
    "title": "Make Eisenstein series code handle eisenstein series with character",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/685",
    "user": "https://github.com/craigcitro"
}
```
The eisenstein_series_qexp function currently only handles eisenstein series for level 1. It wouldn't be hard to make this handle eisenstein series for higher level and with character. I'm going to take care of this soon, but this is here in case I forget. ;)

**Assignee:** @craigcitro

Issue created by migration from https://trac.sagemath.org/ticket/685





---

archive/issue_events_003055.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-18T09:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "milestone": "sage-2.10.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3055"
}
```



---

archive/issue_events_003056.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-18T09:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "milestone": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3056"
}
```



---

archive/issue_comments_002615.json:
```json
{
    "body": "<a id='comment:3'></a>\nCraig,\n\nisn't this implemented by now?\n\nCheers,\n\nMichael",
    "created_at": "2008-11-05T18:56:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/685#issuecomment-2615",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Craig,

isn't this implemented by now?

Cheers,

Michael



---

archive/issue_events_003057.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3057"
}
```



---

archive/issue_events_003058.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3058"
}
```



---

archive/issue_events_003059.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3059"
}
```



---

archive/issue_events_003060.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3060"
}
```



---

archive/issue_events_003061.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-04-24T20:23:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3061"
}
```



---

archive/issue_events_003062.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-04-24T20:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3062"
}
```



---

archive/issue_comments_002616.json:
```json
{
    "body": "<a id='comment:6'></a>\nDo we want any more functionality than in the following example?\n\n```\nsage: G=DirichletGroup(4)\nsage: chi=G[1]\nsage: E=EisensteinForms(chi,1)\nsage: f=E.eisenstein_series()[0]\nsage: f.q_expansion(40)\n1/4 + q + q^2 + q^4 + 2*q^5 + q^8 + q^9 + 2*q^10 + 2*q^13 + q^16 + 2*q^17 + q^18 + 2*q^20 + 3*q^25 + 2*q^26 + 2*q^29 + q^32 + 2*q^34 + q^36 + 2*q^37 + O(q^40)\n```\nIf not, maybe we can close this as fixed/wontfix/whatever_is_appropriate.",
    "created_at": "2014-04-24T20:23:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/685#issuecomment-2616",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:6'></a>
Do we want any more functionality than in the following example?

```
sage: G=DirichletGroup(4)
sage: chi=G[1]
sage: E=EisensteinForms(chi,1)
sage: f=E.eisenstein_series()[0]
sage: f.q_expansion(40)
1/4 + q + q^2 + q^4 + 2*q^5 + q^8 + q^9 + 2*q^10 + 2*q^13 + q^16 + 2*q^17 + q^18 + 2*q^20 + 3*q^25 + 2*q^26 + 2*q^29 + q^32 + 2*q^34 + q^36 + 2*q^37 + O(q^40)
```
If not, maybe we can close this as fixed/wontfix/whatever_is_appropriate.



---

archive/issue_comments_002617.json:
```json
{
    "body": "**Upstream:** N/A",
    "created_at": "2014-04-24T20:23:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/685#issuecomment-2617",
    "user": "https://github.com/pjbruin"
}
```

**Upstream:** N/A



---

archive/issue_events_003063.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-04-24T20:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3063"
}
```



---

archive/issue_comments_002618.json:
```json
{
    "body": "<a id='comment:7'></a>\nYes, that leaves nothing to desire.",
    "created_at": "2014-06-06T06:18:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/685#issuecomment-2618",
    "user": "https://github.com/rwst"
}
```

<a id='comment:7'></a>
Yes, that leaves nothing to desire.



---

archive/issue_events_003064.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-06-06T06:18:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3064"
}
```



---

archive/issue_events_003065.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-06-06T06:18:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3065"
}
```



---

archive/issue_events_003066.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-06T11:01:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3066"
}
```



---

archive/issue_events_003067.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-06T11:01:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/685",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/685#event-3067"
}
```
