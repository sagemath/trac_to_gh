# Issue 922: bug in prime_powers

archive/issues_000922.json:
```json
{
    "assignees": [],
    "body": "Inconsistent types:\n\n```\nsage: vv = prime_powers(10)\nsage: type(vv[0])\n<type 'int'>\nsage: type(vv[1])\n<type 'sage.rings.integer.Integer'>\n```\n\nFreebie for bug day on Saturday :-)\n\nAssignee: **@robertwb**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/922_\n\n",
    "closed_at": "2007-10-21T00:55:34Z",
    "created_at": "2007-10-18T18:35:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bug in prime_powers",
    "type": "issue",
    "updated_at": "2007-10-21T00:55:34Z",
    "url": "https://github.com/sagemath/sage/issues/922",
    "user": "https://github.com/williamstein"
}
```
Inconsistent types:

```
sage: vv = prime_powers(10)
sage: type(vv[0])
<type 'int'>
sage: type(vv[1])
<type 'sage.rings.integer.Integer'>
```

Freebie for bug day on Saturday :-)

Assignee: **@robertwb**

_Issue created by migration from https://trac.sagemath.org/ticket/922_





---

archive/issue_events_007802.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-18T18:35:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/922",
    "milestone_number": null,
    "milestone_title": "sage-2.8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/922#event-7802"
}
```



---

archive/issue_events_007803.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-18T18:35:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/922",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/922#event-7803"
}
```



---

archive/issue_events_007804.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-18T18:35:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/922",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/922#event-7804"
}
```



---

archive/issue_events_007805.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-18T18:35:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/922",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/922#event-7805"
}
```



---

archive/issue_comments_004300.json:
```json
{
    "body": "Assignee: **@robertwb**",
    "created_at": "2007-10-20T19:21:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/922",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/922#issuecomment-4300",
    "user": "https://github.com/robertwb"
}
```

Assignee: **@robertwb**



---

archive/issue_comments_004301.json:
```json
{
    "body": "Attachment: **[922.patch.gz](https://github.com/sagemath/sage/files/ticket922/922.patch.gz)**",
    "created_at": "2007-10-20T19:40:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/922",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/922#issuecomment-4301",
    "user": "https://github.com/robertwb"
}
```

Attachment: **[922.patch.gz](https://github.com/sagemath/sage/files/ticket922/922.patch.gz)**



---

archive/issue_events_007806.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-21T00:55:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/922",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/922#event-7806"
}
```
