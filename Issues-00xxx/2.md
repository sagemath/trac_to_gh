...
<a id='comment:1'></a>A thought: PIDs live in a fairly small space, so it might be worth doing some kind of check to verify that the process corresponding to the PID is a SAGE notebook process.
...
```json
{
    "body": "<a id='comment:3'></a>One could end up with an NFS filesystem and notebooks running on distinct machines. If the port listened on is only a local one on machine A then sending a request from machine B will not confirm that the process is still running.\n\nThis is a bad thing to happen, but not unlikely if someone has sage installed in their homedir and moves from machine A to machine B.",
...
<a id='comment:3'></a>One could end up with an NFS filesystem and notebooks running on distinct machines. If the port listened on is only a local one on machine A then sending a request from machine B will not confirm that the process is still running.
...
```json
{
    "body": "<a id='comment:5'></a>This was fixed decades ago!",
...
<a id='comment:5'></a>This was fixed decades ago!
