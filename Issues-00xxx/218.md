# Issue 218: optimize/implement matrix multimodular algorithms

archive/issues_000218.json:
```json
{
    "assignees": [],
    "body": "\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/218\n\n",
    "closed_at": "2008-03-21T02:36:03Z",
    "created_at": "2007-01-25T18:59:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "optimize/implement matrix multimodular algorithms",
    "type": "issue",
    "updated_at": "2008-03-21T02:36:03Z",
    "url": "https://github.com/sagemath/sage/issues/218",
    "user": "https://github.com/williamstein"
}
```


**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/218





---

archive/issue_events_001343.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T18:59:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "milestone_number": null,
    "milestone_title": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1343"
}
```



---

archive/issue_events_001344.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T18:59:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "08517b",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1344"
}
```



---

archive/issue_events_001345.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T18:59:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1345"
}
```



---

archive/issue_events_001346.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-10T02:58:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1346"
}
```



---

archive/issue_events_001347.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-21T13:52:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "milestone_number": null,
    "milestone_title": "sage-2.8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1347"
}
```



---

archive/issue_comments_000707.json:
```json
{
    "body": "<a id='comment:2'>**Comment 2:**</a>\nHmm, the implementation is certainly there, but the question is how much this can be optimized. Being more specific what needs/should be done would also be helpful.\n\nCheers,\n\nMichael",
    "created_at": "2007-10-21T13:52:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/218#issuecomment-707",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>**Comment 2:**</a>
Hmm, the implementation is certainly there, but the question is how much this can be optimized. Being more specific what needs/should be done would also be helpful.

Cheers,

Michael



---

archive/issue_comments_000708.json:
```json
{
    "body": "<a id='comment:3'>**Comment 3:**</a>\nThis is too unspecific for a current milestone, pushing it back to sage-wishlist.",
    "created_at": "2007-10-23T21:24:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/218#issuecomment-708",
    "user": "https://github.com/malb"
}
```

<a id='comment:3'>**Comment 3:**</a>
This is too unspecific for a current milestone, pushing it back to sage-wishlist.



---

archive/issue_events_001348.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-10-23T21:24:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "milestone_number": null,
    "milestone_title": "sage-2.8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1348"
}
```



---

archive/issue_events_001349.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-10-23T21:24:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "label": "https://github.com/sagemath/sage/labels/wishlist",
    "label_color": "008080",
    "label_name": "wishlist",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1349"
}
```



---

archive/issue_events_001350.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-21T02:36:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1350"
}
```



---

archive/issue_comments_000709.json:
```json
{
    "body": "<a id='comment:4'>**Comment 4:**</a>\n\n```\n[03:05] <mabshoff> wstein: I think #218 is invalid since it is way to unspecific.\n[03:06] <wstein> We already did that long ago.\n[03:06] <wstein> Robertwb did most of the work.\n[03:06] <mabshoff> ok, close it?\n[03:06] <wstein> Yes.  And credit it to Robertwb :-)\n[03:06] <wstein> (and david harvey and me)\n[03:07] <mabshoff> close it as fixed against the current milestone mentioning it was done a while ago.\n[03:07] <mabshoff> Actually, I will just paste this part of the log ;)\n```\n\nCheers,\n\nMichael",
    "created_at": "2008-03-21T02:36:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/218#issuecomment-709",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:4'>**Comment 4:**</a>

```
[03:05] <mabshoff> wstein: I think #218 is invalid since it is way to unspecific.
[03:06] <wstein> We already did that long ago.
[03:06] <wstein> Robertwb did most of the work.
[03:06] <mabshoff> ok, close it?
[03:06] <wstein> Yes.  And credit it to Robertwb :-)
[03:06] <wstein> (and david harvey and me)
[03:07] <mabshoff> close it as fixed against the current milestone mentioning it was done a while ago.
[03:07] <mabshoff> Actually, I will just paste this part of the log ;)
```

Cheers,

Michael



---

archive/issue_events_001351.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-21T02:36:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "milestone_number": null,
    "milestone_title": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1351"
}
```



---

archive/issue_events_001352.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-21T02:36:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/218",
    "label": "https://github.com/sagemath/sage/labels/wishlist",
    "label_color": "008080",
    "label_name": "wishlist",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/218#event-1352"
}
```
