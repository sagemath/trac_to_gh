# Issue 811: get Andrew Sutherland's insanely fast hyperelliptic curve pointing into SAGE

archive/issues_000811.json:
```json
{
    "assignees": [],
    "body": "get Andrew Sutherland's insanely fast hyperelliptic curve pointing into SAGE (see also #793)\n\n\nAssignee: **mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/811_\n\n",
    "closed_at": "2007-10-21T21:26:16Z",
    "created_at": "2007-10-03T16:49:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "get Andrew Sutherland's insanely fast hyperelliptic curve pointing into SAGE",
    "type": "issue",
    "updated_at": "2007-10-21T21:26:16Z",
    "url": "https://github.com/sagemath/sage/issues/811",
    "user": "https://github.com/sagetrac-dmharvey"
}
```
get Andrew Sutherland's insanely fast hyperelliptic curve pointing into SAGE (see also #793)


Assignee: **mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/811_





---

archive/issue_events_006894.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-03T16:49:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/811#event-6894"
}
```



---

archive/issue_events_006895.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-03T16:49:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/811#event-6895"
}
```



---

archive/issue_events_006896.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-03T16:49:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/811#event-6896"
}
```



---

archive/issue_comments_003700.json:
```json
{
    "body": "Changed assignee from **@williamstein** to **mabshoff**",
    "created_at": "2007-10-04T19:54:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/811#issuecomment-3700",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Changed assignee from **@williamstein** to **mabshoff**



---

archive/issue_events_006897.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-04T19:54:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/811#event-6897"
}
```



---

archive/issue_comments_003701.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nDrew, William and I have been talking about various approaches and we are hoping that at least the basic integration will be done before SD6.\n\nCheers,\n\nMichael",
    "created_at": "2007-10-04T19:54:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/811#issuecomment-3701",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Drew, William and I have been talking about various approaches and we are hoping that at least the basic integration will be done before SD6.

Cheers,

Michael



---

archive/issue_events_006898.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-21T21:25:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/811#event-6898"
}
```



---

archive/issue_comments_003702.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nDupe of #965 - which has more info and a link to the newly GPLed code.\n\nCheers,\n\nMichael",
    "created_at": "2007-10-21T21:25:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/811#issuecomment-3702",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Dupe of #965 - which has more info and a link to the newly GPLed code.

Cheers,

Michael



---

archive/issue_events_006899.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-21T21:25:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/811#event-6899"
}
```



---

archive/issue_events_006900.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-21T21:26:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/811#event-6900"
}
```



---

archive/issue_events_006901.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-21T21:26:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/811",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/811#event-6901"
}
```
