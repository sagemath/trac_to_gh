# Issue 284: m4r -- include Gregory Bard's method of four russians C library in SAGE

archive/issues_000284.json:
```json
{
    "assignees": [],
    "body": "Implement matrices mod 2 in SAGE via Bard's data structure, and use this to get really fast echelon form mod 2 (etc.)\n\n**Assignee:** martin albrecht\n\nIssue created by migration from https://trac.sagemath.org/ticket/284\n\n",
    "closed_at": "2007-08-09T21:40:01Z",
    "created_at": "2007-02-24T02:31:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "m4r -- include Gregory Bard's method of four russians C library in SAGE",
    "type": "issue",
    "updated_at": "2007-08-09T21:40:01Z",
    "url": "https://github.com/sagemath/sage/issues/284",
    "user": "https://github.com/williamstein"
}
```
Implement matrices mod 2 in SAGE via Bard's data structure, and use this to get really fast echelon form mod 2 (etc.)

**Assignee:** martin albrecht

Issue created by migration from https://trac.sagemath.org/ticket/284





---

archive/issue_events_001069.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-02-24T02:31:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/284",
    "milestone_number": null,
    "milestone_title": "sage-2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/284#event-1069"
}
```



---

archive/issue_events_001070.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-02-24T02:31:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/284",
    "milestone_number": null,
    "milestone_title": "sage-2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/284#event-1070"
}
```



---

archive/issue_events_001071.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-02-24T02:31:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/284",
    "milestone_number": null,
    "milestone_title": "sage-2.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/284#event-1071"
}
```



---

archive/issue_comments_001031.json:
```json
{
    "body": "<a id='comment:3'></a>\nresolved a while ago.",
    "created_at": "2007-08-09T21:40:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/284#issuecomment-1031",
    "user": "https://github.com/malb"
}
```

<a id='comment:3'></a>
resolved a while ago.



---

archive/issue_events_001072.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-08-09T21:40:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/284",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/284#event-1072"
}
```
