# Issue 913: [with patch] real_roots code fails if polynomial obviously has no roots

archive/issues_000913.json:
```json
{
    "body": "The following test case will loop forever:\n\n```\nsage: x = polygen(ZZ)\nsage: (x^2 + 1).real_root_intervals()\n```\n\n\nAssignee: cwitty\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/913\n\n",
    "closed_at": "2007-10-21T00:49:39Z",
    "created_at": "2007-10-17T15:38:15Z",
    "labels": [
        "component: numerical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.8",
    "title": "[with patch] real_roots code fails if polynomial obviously has no roots",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/913",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
The following test case will loop forever:

```
sage: x = polygen(ZZ)
sage: (x^2 + 1).real_root_intervals()
```


Assignee: cwitty

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/913





---

archive/issue_comments_005876.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [7002.patch](tarball://root/attachments/some-uuid/ticket913/7002.patch) by cwitty created at 2007-10-20 17:19:19\n\nFixed.",
    "created_at": "2007-10-20T17:19:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/913",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/913#issuecomment-5876",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:1'></a>
Attachment [7002.patch](tarball://root/attachments/some-uuid/ticket913/7002.patch) by cwitty created at 2007-10-20 17:19:19

Fixed.



---

archive/issue_events_003098.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/913",
    "rename": {
        "from": "real_roots code fails if polynomial obviously has no roots",
        "to": "[with patch] real_roots code fails if polynomial obviously has no roots"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/913#event-3098"
}
```



---

archive/issue_events_003099.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-21T00:49:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/913",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/913#event-3099"
}
```



---

archive/issue_comments_005877.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-21T00:49:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/913",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/913#issuecomment-5877",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed
