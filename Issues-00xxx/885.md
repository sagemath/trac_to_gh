# Issue 885: 2.8.7-alpha0: doctest failure in rings/morphism.pyx (loads/dumps)

archive/issues_000885.json:
```json
{
    "body": "On sage.math:\n\n```\nFile \"morphism.pyx\", line 312:\n    sage: c == loads(dumps(c))\nExpected:\n    True\nGot:\n    False\n```\n\n**Assignee:** failure\n\n**Resolution:** fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/885\n\n",
    "closed_at": "2007-10-14T22:56:00Z",
    "created_at": "2007-10-13T20:23:36Z",
    "labels": [
        "component: doctest coverage",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.7",
    "title": "2.8.7-alpha0: doctest failure in rings/morphism.pyx (loads/dumps)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/885",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
On sage.math:

```
File "morphism.pyx", line 312:
    sage: c == loads(dumps(c))
Expected:
    True
Got:
    False
```

**Assignee:** failure

**Resolution:** fixed

Issue created by migration from https://trac.sagemath.org/ticket/885





---

archive/attachments_000556.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "6931.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket885/6931.patch",
    "created_at": "2007-10-13T22:35:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/885",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```



---

archive/issue_comments_005890.json:
```json
{
    "body": "<a id='comment:1'></a>\nChanged class RingHomomorphism_im_gens to use `_cmp_c_`impl (this is necessary to make Python subclasses compare correctly).",
    "created_at": "2007-10-13T22:35:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/885#issuecomment-5890",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:1'></a>
Changed class RingHomomorphism_im_gens to use `_cmp_c_`impl (this is necessary to make Python subclasses compare correctly).



---

archive/issue_events_002708.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-14T22:56:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/885",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/885#event-2708"
}
```



---

archive/issue_comments_005891.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2007-10-14T22:56:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/885#issuecomment-5891",
    "user": "https://github.com/williamstein"
}
```

**Resolution:** fixed
