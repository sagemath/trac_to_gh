# Issue 885: 2.8.7-alpha0: doctest failure in rings/morphism.pyx (loads/dumps)

archive/issues_000885.json:
```json
{
    "assignees": [],
    "body": "On sage.math:\n\n```\nFile \"morphism.pyx\", line 312:\n    sage: c == loads(dumps(c))\nExpected:\n    True\nGot:\n    False\n```\n\n**Assignee:** failure\n\nIssue created by migration from https://trac.sagemath.org/ticket/885\n\n",
    "closed_at": "2007-10-14T22:56:00Z",
    "created_at": "2007-10-13T20:23:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20coverage",
        "https://github.com/sagemath/sage/labels/blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "2.8.7-alpha0: doctest failure in rings/morphism.pyx (loads/dumps)",
    "type": "issue",
    "updated_at": "2007-10-14T22:56:00Z",
    "url": "https://github.com/sagemath/sage/issues/885",
    "user": "https://github.com/sagetrac-cwitty"
}
```
On sage.math:

```
File "morphism.pyx", line 312:
    sage: c == loads(dumps(c))
Expected:
    True
Got:
    False
```

**Assignee:** failure

Issue created by migration from https://trac.sagemath.org/ticket/885





---

archive/issue_events_006936.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-13T20:23:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/885",
    "milestone_number": null,
    "milestone_title": "sage-2.8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/885#event-6936"
}
```



---

archive/issue_events_006937.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-13T20:23:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/885",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20coverage",
    "label_color": "08517b",
    "label_name": "component: doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/885#event-6937"
}
```



---

archive/issue_events_006938.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-13T20:23:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/885",
    "label": "https://github.com/sagemath/sage/labels/blocker",
    "label_color": "ff0000",
    "label_name": "blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/885#event-6938"
}
```



---

archive/issue_events_006939.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-13T20:23:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/885",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/885#event-6939"
}
```



---

archive/issue_comments_004170.json:
```json
{
    "body": "<a id='comment:1'>**Comment 1:**</a>\n**Attachment:** [6931.patch.gz](https://github.com/sagemath/sage/files/ticket885/6931.patch.gz)\n\nChanged class RingHomomorphism_im_gens to use _cmp_c_impl (this is necessary to make Python subclasses compare correctly).",
    "created_at": "2007-10-13T22:35:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/885#issuecomment-4170",
    "user": "https://github.com/sagetrac-cwitty"
}
```

<a id='comment:1'>**Comment 1:**</a>
**Attachment:** [6931.patch.gz](https://github.com/sagemath/sage/files/ticket885/6931.patch.gz)

Changed class RingHomomorphism_im_gens to use _cmp_c_impl (this is necessary to make Python subclasses compare correctly).



---

archive/issue_events_006940.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-14T22:56:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/885",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/885#event-6940"
}
```
