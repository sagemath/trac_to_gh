# Issue 198: Improve modular symbols and modular forms.

archive/issues_000198.json:
```json
{
    "assignees": [],
    "body": "*  Some algorithms haven't been completely implemented.  (Some do things I never implemented in MAGMA.)\n    - finish modular forms algorithms\n    - enable modular forms doctests and test and fix\n    - finish implementing modular symbols factorization in general (eisenstein part is currently not fully implemented)\n    - computation of period mapping\n    - more testing, examples, etc., for modular symbos for GammaH\n     \n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/198\n\n",
    "closed_at": "2007-10-24T02:54:08Z",
    "created_at": "2007-01-19T09:59:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve modular symbols and modular forms.",
    "type": "issue",
    "updated_at": "2007-10-24T02:54:13Z",
    "url": "https://github.com/sagemath/sage/issues/198",
    "user": "https://github.com/williamstein"
}
```
*  Some algorithms haven't been completely implemented.  (Some do things I never implemented in MAGMA.)
    - finish modular forms algorithms
    - enable modular forms doctests and test and fix
    - finish implementing modular symbols factorization in general (eisenstein part is currently not fully implemented)
    - computation of period mapping
    - more testing, examples, etc., for modular symbos for GammaH
     

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/198





---

archive/issue_events_001221.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-19T09:59:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
    "label_color": "08517b",
    "label_name": "component: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/198#event-1221"
}
```



---

archive/issue_events_001222.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-19T09:59:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/198#event-1222"
}
```



---

archive/issue_events_001223.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-19T09:59:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "008080",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/198#event-1223"
}
```



---

archive/issue_events_001224.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-10T02:40:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "milestone_number": null,
    "milestone_title": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/198#event-1224"
}
```



---

archive/issue_comments_000653.json:
```json
{
    "body": "<a id='comment:1'>**Comment 1:**</a>\nThis ticket seems like a high priority item. It is a little stale and William ought to update what has happened. If there are only small issues left we should close this ticket and open individual tickets for the remaining items.\n\nCheers,\n\nMichael",
    "created_at": "2007-09-10T02:40:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/198#issuecomment-653",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>**Comment 1:**</a>
This ticket seems like a high priority item. It is a little stale and William ought to update what has happened. If there are only small issues left we should close this ticket and open individual tickets for the remaining items.

Cheers,

Michael



---

archive/issue_events_001225.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-21T13:50:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "milestone_number": null,
    "milestone_title": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/198#event-1225"
}
```



---

archive/issue_events_001226.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-21T13:50:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "milestone_number": null,
    "milestone_title": "sage-2.8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/198#event-1226"
}
```



---

archive/issue_comments_000654.json:
```json
{
    "body": "<a id='comment:3'>**Comment 3:**</a>\nThis will be replaced by many smaller tickets.",
    "created_at": "2007-10-24T02:54:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/198#issuecomment-654",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'>**Comment 3:**</a>
This will be replaced by many smaller tickets.



---

archive/issue_events_001227.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-24T02:54:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/198#event-1227"
}
```



---

archive/issue_events_001228.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-24T02:54:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/198",
    "milestone_number": null,
    "milestone_title": "sage-2.8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/198#event-1228"
}
```
