# Issue 338: In doc browser make hyper links to referenced code in SAGE library

archive/issues_000338.json:
```json
{
    "body": "For example:\nOn page http://sagenb.com/doc_browser?/const/?node72.html, reference to sage/modular/dims.py would be a hyper link to the actual code.\nOn page http://sagenb.com/doc_browser?/prog/?node15.html, reference to SAGE_ROOT/devel/sage/sage/graphs/graph.py would be a hyper link to the actual code.\n\n**Assignee:** TimothyClemans\n\nIssue created by migration from https://trac.sagemath.org/ticket/338\n\n",
    "closed_at": "2018-05-18T17:16:26Z",
    "created_at": "2007-03-31T17:42:35Z",
    "labels": [
        "component: notebook",
        "minor",
        "enhancement",
        "wontfix"
    ],
    "title": "In doc browser make hyper links to referenced code in SAGE library",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/338",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```
For example:
On page http://sagenb.com/doc_browser?/const/?node72.html, reference to sage/modular/dims.py would be a hyper link to the actual code.
On page http://sagenb.com/doc_browser?/prog/?node15.html, reference to SAGE_ROOT/devel/sage/sage/graphs/graph.py would be a hyper link to the actual code.

**Assignee:** TimothyClemans

Issue created by migration from https://trac.sagemath.org/ticket/338





---

archive/issue_comments_001253.json:
```json
{
    "body": "**Changing assignee** from boothby to TimothyClemans.",
    "created_at": "2007-03-31T18:40:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/338#issuecomment-1253",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

**Changing assignee** from boothby to TimothyClemans.



---

archive/issue_events_001321.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-09-11T00:51:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "label": "feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/338#event-1321"
}
```



---

archive/issue_comments_001254.json:
```json
{
    "body": "<a id='comment:3'></a>\nThe URLs in the ticket description don't exist.",
    "created_at": "2012-06-23T09:18:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/338#issuecomment-1254",
    "user": "https://github.com/kini"
}
```

<a id='comment:3'></a>
The URLs in the ticket description don't exist.



---

archive/issue_events_001322.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-23T09:18:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/338#event-1322"
}
```



---

archive/issue_comments_001255.json:
```json
{
    "body": "<a id='comment:4'></a>\nThis can be closed.  Keshav, I think this is WELL before you were aware of Sage - it dates from the days when there was no login process of any kind, if I'm not mistaken.  Ah, those were the days ;-)\n\nAnyway, the real issue is that although within each piece of the Sage documentation it's easy to create hyperlinks, but not between different sets (e.g. the developer guide and the reference manual).  For instance, the first example is referring to [here](http://sagenb.org/doc/static/constructions/modular_forms.html?highlight=modular#genus-formulas).  But that has been discussed before, and this is too vague and outdated to be able to really doable.\n\nOr what do you think?  Could this ticket be repurposed to the actual problem, and is that worthwhile?",
    "created_at": "2013-01-29T20:48:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/338#issuecomment-1255",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:4'></a>
This can be closed.  Keshav, I think this is WELL before you were aware of Sage - it dates from the days when there was no login process of any kind, if I'm not mistaken.  Ah, those were the days ;-)

Anyway, the real issue is that although within each piece of the Sage documentation it's easy to create hyperlinks, but not between different sets (e.g. the developer guide and the reference manual).  For instance, the first example is referring to [here](http://sagenb.org/doc/static/constructions/modular_forms.html?highlight=modular#genus-formulas).  But that has been discussed before, and this is too vague and outdated to be able to really doable.

Or what do you think?  Could this ticket be repurposed to the actual problem, and is that worthwhile?



---

archive/issue_events_001323.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-04-17T07:44:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/338#event-1323"
}
```



---

archive/issue_events_001324.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-04-17T07:44:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/338#event-1324"
}
```



---

archive/issue_events_001325.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-04-17T07:44:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "label": "feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/338#event-1325"
}
```



---

archive/issue_events_001326.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/338#event-1326"
}
```



---

archive/issue_events_001327.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/338#event-1327"
}
```



---

archive/issue_events_001328.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/338#event-1328"
}
```



---

archive/issue_comments_001256.json:
```json
{
    "body": "<a id='comment:6'></a>\nclosing positively reviewed duplicates",
    "created_at": "2018-05-18T17:16:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/338#issuecomment-1256",
    "user": "https://github.com/videlec"
}
```

<a id='comment:6'></a>
closing positively reviewed duplicates
