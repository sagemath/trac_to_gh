# Issue 984: sage-2.8.9.rc1: sage-banner is empty

archive/issues_000984.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "sage is missing its startup message in 2.8.9.rc1:\n\n```\ncwitty@magnetar:~/sage-2.8.9.rc1$ ./sage\n\nsage: \n```\n\n\nComponent: **user interface**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/984_\n\n",
    "closed_at": "2007-10-27T05:17:37Z",
    "created_at": "2007-10-25T00:50:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage-2.8.9.rc1: sage-banner is empty",
    "type": "issue",
    "updated_at": "2007-10-27T05:17:37Z",
    "url": "https://github.com/sagemath/sage/issues/984",
    "user": "https://github.com/sagetrac-cwitty"
}
```
sage is missing its startup message in 2.8.9.rc1:

```
cwitty@magnetar:~/sage-2.8.9.rc1$ ./sage

sage: 
```


Component: **user interface**

_Issue created by migration from https://trac.sagemath.org/ticket/984_





---

archive/issue_events_009460.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-25T00:50:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "milestone_number": null,
    "milestone_title": "sage-2.8.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/984#event-9460"
}
```



---

archive/issue_events_009461.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-25T00:50:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/984#event-9461"
}
```



---

archive/issue_events_009462.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-25T00:50:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/984#event-9462"
}
```



---

archive/issue_events_009463.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-25T00:50:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/984#event-9463"
}
```



---

archive/issue_events_009464.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-25T00:50:42Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "subject": "https://github.com/sagetrac-cwitty",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/984#event-9464"
}
```



---

archive/issue_comments_004316.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nWhen I do sage -sdist even using that binary I get that a banner\nis created.  I banner certainly gets created when I do -sdist from\nmy sage install.  So it will be in the released version.",
    "created_at": "2007-10-25T07:56:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/984#issuecomment-4316",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

When I do sage -sdist even using that binary I get that a banner
is created.  I banner certainly gets created when I do -sdist from
my sage install.  So it will be in the released version.



---

archive/issue_comments_004317.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nSo this ought to be close against 2.8.9 then.\n\nMichael",
    "created_at": "2007-10-25T10:48:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/984#issuecomment-4317",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

So this ought to be close against 2.8.9 then.

Michael



---

archive/issue_events_009465.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-27T05:17:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "milestone_number": null,
    "milestone_title": "sage-2.8.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/984#event-9465"
}
```



---

archive/issue_events_009466.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-27T05:17:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "milestone_number": null,
    "milestone_title": "sage-2.8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/984#event-9466"
}
```



---

archive/issue_comments_004318.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nFixed in final 2.8.9 and verified with local build.\n\nCheers,\n\nMichael",
    "created_at": "2007-10-27T05:17:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/984#issuecomment-4318",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Fixed in final 2.8.9 and verified with local build.

Cheers,

Michael



---

archive/issue_events_009467.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-27T05:17:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/984",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/984#event-9467"
}
```
