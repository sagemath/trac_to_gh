# Issue 90: SAGE build process -- immediately quite if GCC < 3 installed

archive/issues_000090.json:
```json
{
    "assignees": [],
    "body": "SAGE can't build with GCC 2.9x, so it shouldn't try.  \n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/90_\n\n",
    "closed_at": "2007-08-09T21:49:11Z",
    "created_at": "2006-09-27T17:33:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SAGE build process -- immediately quite if GCC < 3 installed",
    "type": "issue",
    "updated_at": "2007-08-09T21:49:11Z",
    "url": "https://github.com/sagemath/sage/issues/90",
    "user": "https://github.com/williamstein"
}
```
SAGE can't build with GCC 2.9x, so it shouldn't try.  

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/90_





---

archive/issue_events_000629.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-27T17:33:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/90",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/90#event-629"
}
```



---

archive/issue_events_000630.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-27T17:33:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/90",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/90#event-630"
}
```



---

archive/issue_events_000631.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-27T17:33:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/90",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/90#event-631"
}
```



---

archive/issue_events_000632.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-08-09T21:49:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/90",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/90#event-632"
}
```



---

archive/issue_comments_000330.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nfixed.",
    "created_at": "2007-08-09T21:49:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/90",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/90#issuecomment-330",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'>Comment 1:</a>
fixed.
