# Issue 892: [with patch] sage -t doesn't test .pxi files

archive/issues_000892.json:
```json
{
    "body": "\n\nAssignee: failure\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/892\n\n",
    "closed_at": "2007-10-14T05:16:03Z",
    "created_at": "2007-10-13T20:48:14Z",
    "labels": [
        "component: doctest coverage",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.7",
    "title": "[with patch] sage -t doesn't test .pxi files",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/892",
    "user": "https://github.com/mwhansen"
}
```


Assignee: failure

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/892





---

archive/issue_comments_005766.json:
```json
{
    "body": "Attachment [892.patch](tarball://root/attachments/some-uuid/ticket892/892.patch) by @mwhansen created at 2007-10-13 20:52:27",
    "created_at": "2007-10-13T20:52:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/892#issuecomment-5766",
    "user": "https://github.com/mwhansen"
}
```

Attachment [892.patch](tarball://root/attachments/some-uuid/ticket892/892.patch) by @mwhansen created at 2007-10-13 20:52:27



---

archive/issue_events_003024.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/892",
    "rename": {
        "from": "sage -t doesn't test .pxi files",
        "to": "[with patch] sage -t doesn't test .pxi files"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/892#event-3024"
}
```



---

archive/issue_events_003025.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-14T05:16:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/892",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/892#event-3025"
}
```



---

archive/issue_comments_005767.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-14T05:16:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/892#issuecomment-5767",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed
