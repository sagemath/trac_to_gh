# Issue 89: another p-adic division bug

archive/issues_000089.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: (1 + O(5^2)) / (1 + O(5))\n 1 + O(5^2)\n```\n\nClearly the answer should be instead\n\n```\n 1 + O(5)\n```\n\n\nAssignee: **dmharvey**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/89_\n\n",
    "closed_at": "2006-10-10T23:58:17Z",
    "created_at": "2006-09-27T13:34:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "another p-adic division bug",
    "type": "issue",
    "updated_at": "2006-10-10T23:58:17Z",
    "url": "https://github.com/sagemath/sage/issues/89",
    "user": "https://github.com/sagetrac-dmharvey"
}
```

```
sage: (1 + O(5^2)) / (1 + O(5))
 1 + O(5^2)
```

Clearly the answer should be instead

```
 1 + O(5)
```


Assignee: **dmharvey**

_Issue created by migration from https://trac.sagemath.org/ticket/89_





---

archive/issue_events_000625.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2006-09-27T13:34:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/89",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/89#event-625"
}
```



---

archive/issue_events_000626.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2006-09-27T13:34:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/89",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/89#event-626"
}
```



---

archive/issue_events_000627.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2006-09-27T13:34:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/89",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/89#event-627"
}
```



---

archive/issue_comments_000328.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nActually the problem is apparently with `__invert__`:\n\n```\nsage: (1 + O(5)).__invert__()\n 1 + O(5^20)\n```",
    "created_at": "2006-09-27T13:37:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/89",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/89#issuecomment-328",
    "user": "https://github.com/sagetrac-dmharvey"
}
```

<a id='comment:1'>Comment 1:</a>
Actually the problem is apparently with `__invert__`:

```
sage: (1 + O(5)).__invert__()
 1 + O(5^20)
```



---

archive/issue_comments_000329.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nfixed in sage 1.4",
    "created_at": "2006-10-10T23:58:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/89",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/89#issuecomment-329",
    "user": "https://github.com/sagetrac-dmharvey"
}
```

<a id='comment:2'>Comment 2:</a>
fixed in sage 1.4



---

archive/issue_events_000628.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2006-10-10T23:58:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/89",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/89#event-628"
}
```
