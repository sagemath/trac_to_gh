# Issue 867: [with patch] support for rook monoid and planar rook monoid (as well as their diagram/partition algebras) and a few other misc. fixes

archive/issues_000867.json:
```json
{
    "body": "\n\nAssignee: @mwhansen\n\nCC:  sage-combinat\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/867\n\n",
    "closed_at": "2007-10-13T07:55:16Z",
    "created_at": "2007-10-12T23:18:36Z",
    "labels": [
        "component: combinatorics",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.7",
    "title": "[with patch] support for rook monoid and planar rook monoid (as well as their diagram/partition algebras) and a few other misc. fixes",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/867",
    "user": "https://github.com/mwhansen"
}
```


Assignee: @mwhansen

CC:  sage-combinat

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/867





---

archive/issue_comments_005623.json:
```json
{
    "body": "Attachment [6504.patch](tarball://root/attachments/some-uuid/ticket867/6504.patch) by @mwhansen created at 2007-10-12 23:18:49",
    "created_at": "2007-10-12T23:18:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/867#issuecomment-5623",
    "user": "https://github.com/mwhansen"
}
```

Attachment [6504.patch](tarball://root/attachments/some-uuid/ticket867/6504.patch) by @mwhansen created at 2007-10-12 23:18:49



---

archive/issue_events_002952.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/867",
    "rename": {
        "from": "[with patch] support for rook monoid and planar rook monoid (as well as their diagram/partition algebras)",
        "to": "[with patch] support for rook monoid and planar rook monoid (as well as their diagram/partition algebras) and a few other misc. fixes"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/867#event-2952"
}
```



---

archive/issue_comments_005624.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-10-13T00:12:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/867#issuecomment-5624",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_events_002953.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T07:55:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/867",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/867#event-2953"
}
```



---

archive/issue_comments_005625.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-13T07:55:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/867#issuecomment-5625",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed
