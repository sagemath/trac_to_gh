# Issue 623: New Cython

archive/issues_000623.json:
```json
{
    "body": "Update the Cython package to the latest. \n\nAssignee: @robertwb\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/623\n\n",
    "closed_at": "2007-10-20T18:14:55Z",
    "created_at": "2007-09-07T19:43:40Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.8",
    "title": "New Cython",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/623",
    "user": "https://github.com/robertwb"
}
```
Update the Cython package to the latest. 

Assignee: @robertwb

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/623





---

archive/issue_comments_003189.json:
```json
{
    "body": "Changing assignee from @williamstein to @robertwb.",
    "created_at": "2007-09-07T19:43:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3189",
    "user": "https://github.com/robertwb"
}
```

Changing assignee from @williamstein to @robertwb.



---

archive/issue_comments_003190.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-09-07T19:43:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3190",
    "user": "https://github.com/robertwb"
}
```

Changing status from new to assigned.



---

archive/issue_events_001660.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-09-09T18:14:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "milestone": "sage-2.8.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/623#event-1660"
}
```



---

archive/issue_events_001661.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-09-12T04:25:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "milestone": "sage-2.8.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/623#event-1661"
}
```



---

archive/issue_events_001662.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-09-12T04:25:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/623#event-1662"
}
```



---

archive/issue_comments_003191.json:
```json
{
    "body": "<a id='comment:4'></a>See http://sage.math.washington.edu/home/robertwb/coerce/",
    "created_at": "2007-09-12T04:29:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3191",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:4'></a>See http://sage.math.washington.edu/home/robertwb/coerce/



---

archive/issue_comments_003192.json:
```json
{
    "body": "<a id='comment:5'></a>This ought to be closed. For the next Cython release the subject line should be more specific.\n\nCheers,\n\nMichael",
    "created_at": "2007-10-19T18:38:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3192",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>This ought to be closed. For the next Cython release the subject line should be more specific.

Cheers,

Michael



---

archive/issue_events_001663.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-10-19T18:38:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/623#event-1663"
}
```



---

archive/issue_events_001664.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-10-19T18:38:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "milestone": "sage-2.8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/623#event-1664"
}
```



---

archive/issue_comments_003193.json:
```json
{
    "body": "<a id='comment:6'></a>I'm closing this, since the directory above points to an old version of cython.",
    "created_at": "2007-10-20T18:14:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3193",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:6'></a>I'm closing this, since the directory above points to an old version of cython.



---

archive/issue_events_001665.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-20T18:14:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/623#event-1665"
}
```



---

archive/issue_comments_003194.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-20T18:14:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3194",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed
