# Issue 27: Move the function dual_eigenvector in sage/hecke/module.py to

archive/issues_000027.json:
```json
{
    "assignees": [],
    "body": "\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/27\n\n",
    "closed_at": "2008-03-16T07:48:07Z",
    "created_at": "2006-09-12T23:24:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Move the function dual_eigenvector in sage/hecke/module.py to",
    "type": "issue",
    "updated_at": "2008-03-16T07:57:24Z",
    "url": "https://github.com/sagemath/sage/issues/27",
    "user": "https://github.com/williamstein"
}
```


**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/27





---

archive/issue_events_000116.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-11T02:17:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27#event-116"
}
```



---

archive/issue_events_000117.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-01-27T05:08:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "08517b",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27#event-117"
}
```



---

archive/issue_events_000118.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-01-27T05:08:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
    "label_color": "08517b",
    "label_name": "component: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27#event-118"
}
```



---

archive/issue_comments_000147.json:
```json
{
    "body": "**Assignee:** @williamstein",
    "created_at": "2008-01-27T05:08:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27#issuecomment-147",
    "user": "https://github.com/aghitza"
}
```

**Assignee:** @williamstein



---

archive/issue_events_000119.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-16T07:48:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27#event-119"
}
```



---

archive/issue_comments_000148.json:
```json
{
    "body": "<a id='comment:3'></a>\nwtf?!",
    "created_at": "2008-03-16T07:48:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27#issuecomment-148",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
wtf?!



---

archive/issue_events_000120.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-16T07:57:24Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27",
    "milestone_number": null,
    "milestone_title": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27#event-120"
}
```
