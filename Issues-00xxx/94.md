# Issue 94: mercurial -- need easy way to safely link directly to source browser

archive/issues_000094.json:
```json
{
    "assignees": [],
    "body": "David Harvey requested this.  There's all kinds of weird url encoding to\nget to the source, which I don't understand, so possibly we need to improve\nthe cgi-bin script.   \n\n\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/94_\n\n",
    "closed_at": "2006-09-29T14:52:25Z",
    "created_at": "2006-09-28T17:45:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "mercurial -- need easy way to safely link directly to source browser",
    "type": "issue",
    "updated_at": "2006-09-29T14:52:25Z",
    "url": "https://github.com/sagemath/sage/issues/94",
    "user": "https://github.com/williamstein"
}
```
David Harvey requested this.  There's all kinds of weird url encoding to
get to the source, which I don't understand, so possibly we need to improve
the cgi-bin script.   



Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/94_





---

archive/issue_events_000641.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-28T17:45:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/94",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/94#event-641"
}
```



---

archive/issue_events_000642.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-28T17:45:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/94",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/94#event-642"
}
```



---

archive/issue_events_000643.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-28T17:45:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/94",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/94#event-643"
}
```



---

archive/issue_events_000644.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-29T14:52:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/94",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/94#event-644"
}
```



---

archive/issue_comments_000335.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI asked on the mercurial mailing list, and got the following prompt reply:\n \n\n```\nhttp://sage.math.washington.edu/sage/hg/sage-main?cmd=manifest;manifest=-1;path=/sage/\n ```\nDavid Harvey",
    "created_at": "2006-09-29T14:52:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/94",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/94#issuecomment-335",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'>Comment 1:</a>
I asked on the mercurial mailing list, and got the following prompt reply:
 

```
http://sage.math.washington.edu/sage/hg/sage-main?cmd=manifest;manifest=-1;path=/sage/
 ```
David Harvey
