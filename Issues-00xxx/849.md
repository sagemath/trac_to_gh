# Issue 849: [with patch] [tested by cwitty] Cython 0.9.6.7

archive/issues_000849.json:
```json
{
    "body": "New release of Cython at http://cython.org/cython-0.9.6.7.spkg . Everything builds and all doctests pass with 2.8.6 and patch below. \n\nThere will probably be another release soon (merging in latest Pyrex release, no promise on how long that will take) but this has the overrideable cdef methods (now rdef) that would be good to start taking advantage of. \n\n\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/849\n\n",
    "closed_at": "2007-10-13T07:24:49Z",
    "created_at": "2007-10-11T10:16:14Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.7",
    "title": "[with patch] [tested by cwitty] Cython 0.9.6.7",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/849",
    "user": "https://github.com/robertwb"
}
```
New release of Cython at http://cython.org/cython-0.9.6.7.spkg . Everything builds and all doctests pass with 2.8.6 and patch below. 

There will probably be another release soon (merging in latest Pyrex release, no promise on how long that will take) but this has the overrideable cdef methods (now rdef) that would be good to start taking advantage of. 



Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/849





---

archive/issue_comments_005508.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [sage-cython-fixes.hg](tarball://root/attachments/some-uuid/ticket849/sage-cython-fixes.hg) by mabshoff created at 2007-10-11 13:40:14",
    "created_at": "2007-10-11T13:40:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/849",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/849#issuecomment-5508",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Attachment [sage-cython-fixes.hg](tarball://root/attachments/some-uuid/ticket849/sage-cython-fixes.hg) by mabshoff created at 2007-10-11 13:40:14



---

archive/issue_events_002895.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/849",
    "rename": {
        "from": "Cython 0.9.6.7",
        "to": "[with patch] Cython 0.9.6.7"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/849#event-2895"
}
```



---

archive/issue_events_002896.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-10-11T13:40:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/849",
    "milestone": "sage-2.8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/849#event-2896"
}
```



---

archive/issue_events_002897.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/849",
    "rename": {
        "from": "[with patch] Cython 0.9.6.7",
        "to": "[with patch] [tested by cwitty] Cython 0.9.6.7"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/849#event-2897"
}
```



---

archive/issue_events_002898.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T07:24:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/849",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/849#event-2898"
}
```



---

archive/issue_comments_005509.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-13T07:24:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/849",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/849#issuecomment-5509",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed
