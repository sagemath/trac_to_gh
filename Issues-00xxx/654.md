# Issue 654: add callgrind support for vagrinding sage

archive/issues_000654.json:
```json
{
    "assignees": [],
    "body": "Callgrind is quite usefull and was the only of the four major valgrind tools not yet supported. The patch at \n\nhttp://fsmath.mathematik.uni-dortmund.de/~mabshoff/patches/Sage-2.8.4.2-add-callgrind-support.patch\n\nchanges that.\n\nCheers,\n\nMichael\n\nAssignee: **mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/654_\n\n",
    "closed_at": "2007-09-14T18:56:47Z",
    "created_at": "2007-09-14T06:30:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add callgrind support for vagrinding sage",
    "type": "issue",
    "updated_at": "2007-09-14T18:56:47Z",
    "url": "https://github.com/sagemath/sage/issues/654",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
Callgrind is quite usefull and was the only of the four major valgrind tools not yet supported. The patch at 

http://fsmath.mathematik.uni-dortmund.de/~mabshoff/patches/Sage-2.8.4.2-add-callgrind-support.patch

changes that.

Cheers,

Michael

Assignee: **mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/654_





---

archive/issue_events_005313.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-14T06:30:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/654",
    "milestone_number": null,
    "milestone_title": "sage-2.8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/654#event-5313"
}
```



---

archive/issue_events_005314.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-14T06:30:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/654",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/654#event-5314"
}
```



---

archive/issue_events_005315.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-14T06:30:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/654",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/654#event-5315"
}
```



---

archive/issue_events_005316.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-14T06:30:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/654",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/654#event-5316"
}
```



---

archive/issue_events_005317.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-14T06:37:14Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/654",
    "title_is": "add callgrind support for vagrinding sage",
    "title_was": "all callgrind support for vagrinding sage",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/654#event-5317"
}
```



---

archive/issue_comments_002459.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nimported for sage-2.8.4.3.",
    "created_at": "2007-09-14T18:56:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/654#issuecomment-2459",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'>Comment 2:</a>
imported for sage-2.8.4.3.



---

archive/issue_events_005318.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-14T18:56:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/654",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/654#event-5318"
}
```
