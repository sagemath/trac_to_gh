# Issue 320: RealField.is_field() is broken

archive/issues_000320.json:
```json
{
    "body": "```\nsage: RealField.is_field()\n---------------------------------------------------------------------------\n<type 'exceptions.TypeError'>             Traceback (most recent call last)\n\n/home/dmharvey/gauss-manin-de/<ipython console> in <module>()\n\n<type 'exceptions.TypeError'>: descriptor 'is_field' of 'sage.rings.ring.Field' object needs an argument\n\n```\n\n\nAssignee: somebody\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/320\n\n",
    "closed_at": "2007-03-21T22:49:19Z",
    "created_at": "2007-03-14T04:58:31Z",
    "labels": [
        "component: basic arithmetic",
        "minor",
        "bug"
    ],
    "title": "RealField.is_field() is broken",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/320",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```
```
sage: RealField.is_field()
---------------------------------------------------------------------------
<type 'exceptions.TypeError'>             Traceback (most recent call last)

/home/dmharvey/gauss-manin-de/<ipython console> in <module>()

<type 'exceptions.TypeError'>: descriptor 'is_field' of 'sage.rings.ring.Field' object needs an argument

```


Assignee: somebody

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/320





---

archive/issue_comments_001691.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2007-03-21T22:49:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/320#issuecomment-1691",
    "user": "https://github.com/williamstein"
}
```

Resolution: invalid



---

archive/issue_comments_001692.json:
```json
{
    "body": "<a id='comment:1'></a>This doesn't make any sense.  RealField is a function that returns a real field of given\nprecision.  You can't call a method on it.  You mean, e.g., \n\n   RealField(53).is_field(),\n\nwhich isn't broken.",
    "created_at": "2007-03-21T22:49:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/320#issuecomment-1692",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>This doesn't make any sense.  RealField is a function that returns a real field of given
precision.  You can't call a method on it.  You mean, e.g., 

   RealField(53).is_field(),

which isn't broken.



---

archive/issue_events_000746.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-03-21T22:49:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/320",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/320#event-746"
}
```
