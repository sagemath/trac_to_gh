# Issue 970: [with patch] ntl.GF2E

archive/issues_000970.json:
```json
{
    "body": "The attached patches implement a partial rewrite of ntl.GF2E to reflect the design decisions made for the ntl interface. \n\nThis patch depends on #966 to be applied.\n\nAssignee: somebody\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/970\n\n",
    "closed_at": "2007-10-23T21:12:46Z",
    "created_at": "2007-10-22T22:42:35Z",
    "labels": [
        "component: basic arithmetic",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.9",
    "title": "[with patch] ntl.GF2E",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/970",
    "user": "https://github.com/malb"
}
```
The attached patches implement a partial rewrite of ntl.GF2E to reflect the design decisions made for the ntl interface. 

This patch depends on #966 to be applied.

Assignee: somebody

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/970





---

archive/issue_comments_006209.json:
```json
{
    "body": "Attachment [ntl_GF2E.patch](tarball://root/attachments/some-uuid/ticket970/ntl_GF2E.patch) by @malb created at 2007-10-22 22:42:53",
    "created_at": "2007-10-22T22:42:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/970",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/970#issuecomment-6209",
    "user": "https://github.com/malb"
}
```

Attachment [ntl_GF2E.patch](tarball://root/attachments/some-uuid/ticket970/ntl_GF2E.patch) by @malb created at 2007-10-22 22:42:53



---

archive/issue_events_003305.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/970",
    "rename": {
        "from": "ntl.GF2E",
        "to": "[with patch] ntl.GF2E"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/970#event-3305"
}
```



---

archive/issue_events_003306.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-10-23T21:12:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/970",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/970#event-3306"
}
```



---

archive/issue_comments_006210.json:
```json
{
    "body": "<a id='comment:2'></a>\napplied to 2.8.9.alpha0",
    "created_at": "2007-10-23T21:12:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/970",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/970#issuecomment-6210",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'></a>
applied to 2.8.9.alpha0



---

archive/issue_comments_006211.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-23T21:12:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/970",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/970#issuecomment-6211",
    "user": "https://github.com/malb"
}
```

Resolution: fixed
