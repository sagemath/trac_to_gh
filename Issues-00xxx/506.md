# Issue 506: [with positive review] add %timeit support to the notebook

archive/issues_000506.json:
```json
{
    "body": "Like in Ipython, it would be good if the notebook had support for using %timeit to time\nblocks of code. \n\n**Assignee:** @williamstein\n\n**CC:**  jason-sage@creativetrax.com\n\n**Resolution:** fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/506\n\n",
    "closed_at": "2008-02-15T22:00:30Z",
    "created_at": "2007-08-29T02:47:47Z",
    "labels": [
        "component: notebook",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "[with positive review] add %timeit support to the notebook",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/506",
    "user": "https://github.com/williamstein"
}
```
Like in Ipython, it would be good if the notebook had support for using %timeit to time
blocks of code. 

**Assignee:** @williamstein

**CC:**  jason-sage@creativetrax.com

**Resolution:** fixed

Issue created by migration from https://trac.sagemath.org/ticket/506





---

archive/issue_comments_002684.json:
```json
{
    "body": "**Changing assignee** from boothby to @williamstein.",
    "created_at": "2008-02-15T05:09:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2684",
    "user": "https://github.com/williamstein"
}
```

**Changing assignee** from boothby to @williamstein.



---

archive/issue_comments_002685.json:
```json
{
    "body": "**Changing status** from new to assigned.",
    "created_at": "2008-02-15T05:09:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2685",
    "user": "https://github.com/williamstein"
}
```

**Changing status** from new to assigned.



---

archive/attachments_000169.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac-506_timeit.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket506/trac-506_timeit.patch",
    "created_at": "2008-02-15T06:34:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_002686.json:
```json
{
    "body": "<a id='comment:3'></a>\nIn addition to adding the requested functionality there is now a command timeit that one can also use from the command line, which correctly uses the preparser.",
    "created_at": "2008-02-15T06:34:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2686",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
In addition to adding the requested functionality there is now a command timeit that one can also use from the command line, which correctly uses the preparser.



---

archive/issue_comments_002687.json:
```json
{
    "body": "**Changing status** from new to needs_review.",
    "created_at": "2008-02-15T06:34:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2687",
    "user": "https://github.com/williamstein"
}
```

**Changing status** from new to needs_review.



---

archive/attachments_000170.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac-506-part2_of_2.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket506/trac-506-part2_of_2.patch",
    "created_at": "2008-02-15T06:39:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_002688.json:
```json
{
    "body": "apply both this and trac-506.ptch",
    "created_at": "2008-02-15T06:39:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2688",
    "user": "https://github.com/williamstein"
}
```

apply both this and trac-506.ptch



---

archive/issue_comments_002689.json:
```json
{
    "body": "<a id='comment:4'></a>\nLooks good to me.",
    "created_at": "2008-02-15T06:46:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2689",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:4'></a>
Looks good to me.



---

archive/issue_comments_002690.json:
```json
{
    "body": "**Changing status** from needs_review to positive_review.",
    "created_at": "2008-02-15T06:46:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2690",
    "user": "https://github.com/mwhansen"
}
```

**Changing status** from needs_review to positive_review.



---

archive/issue_comments_002691.json:
```json
{
    "body": "<a id='comment:5'></a>\nLooks good to me too.",
    "created_at": "2008-02-15T07:02:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2691",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:5'></a>
Looks good to me too.



---

archive/issue_events_001414.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "rename": {
        "from": "add %timeit support to the notebook",
        "to": "[with positive review] add %timeit support to the notebook"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/506#event-1414"
}
```



---

archive/issue_comments_002692.json:
```json
{
    "body": "<a id='comment:6'></a>\nMerged in Sage 2.10.2.alpha1",
    "created_at": "2008-02-15T22:00:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2692",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
Merged in Sage 2.10.2.alpha1



---

archive/issue_comments_002693.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2008-02-15T22:00:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/506#issuecomment-2693",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Resolution:** fixed



---

archive/issue_events_001415.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-15T22:00:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/506",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/506#event-1415"
}
```
