# Issue 701: port srange to Cython for speed

archive/issues_000701.json:
```json
{
    "body": "William complained about srange being slow several times now. Let's fix it.\n\nIssue created by migration from https://trac.sagemath.org/ticket/701\n\n",
    "closed_at": "2007-10-04T03:16:54Z",
    "created_at": "2007-09-20T10:31:27Z",
    "labels": [
        "component: basic arithmetic",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.6",
    "title": "port srange to Cython for speed",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/701",
    "user": "https://github.com/malb"
}
```
William complained about srange being slow several times now. Let's fix it.

Issue created by migration from https://trac.sagemath.org/ticket/701





---

archive/issue_events_003142.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-21T00:00:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/701#event-3142"
}
```



---

archive/attachments_000333.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "srange.hg",
    "asset_url": "tarball://root/attachments/some-uuid/ticket701/srange.hg",
    "created_at": "2007-09-21T00:00:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.hg",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_002713.json:
```json
{
    "body": "<a id='comment:1'></a>\n",
    "created_at": "2007-09-21T00:00:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/701#issuecomment-2713",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>




---

archive/issue_events_003143.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-21T00:01:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/701#event-3143"
}
```



---

archive/issue_events_003144.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-21T00:01:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "milestone": "sage-2.8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/701#event-3144"
}
```



---

archive/issue_comments_002714.json:
```json
{
    "body": "<a id='comment:3'></a>\nThe srange function with include_endpoint=True\ndoes not include the endpoint in some cases:\n\nsage: srange(1.0, 5.0, include_endpoint=True)\n\n[1.00000000000000, 2.00000000000000, 3.00000000000000, 4.00000000000000]",
    "created_at": "2007-09-21T14:28:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/701#issuecomment-2714",
    "user": "https://github.com/jaapspies"
}
```

<a id='comment:3'></a>
The srange function with include_endpoint=True
does not include the endpoint in some cases:

sage: srange(1.0, 5.0, include_endpoint=True)

[1.00000000000000, 2.00000000000000, 3.00000000000000, 4.00000000000000]



---

archive/attachments_000334.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "srange-fixes.hg",
    "asset_url": "tarball://root/attachments/some-uuid/ticket701/srange-fixes.hg",
    "created_at": "2007-09-21T18:46:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.hg",
    "user": "https://github.com/robertwb"
}
```



---

archive/issue_comments_002715.json:
```json
{
    "body": "",
    "created_at": "2007-09-21T18:46:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/701#issuecomment-2715",
    "user": "https://github.com/robertwb"
}
```





---

archive/issue_events_003145.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-21T18:49:35Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "rename": {
        "from": "port srange to Cython for speed",
        "to": "port srange to Cython for speed [patch attached]"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/701#event-3145"
}
```



---

archive/issue_events_003146.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-21T18:49:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "milestone": "sage-2.8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/701#event-3146"
}
```



---

archive/issue_events_003147.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-21T18:49:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "milestone": "sage-2.8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/701#event-3147"
}
```



---

archive/issue_events_003148.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-09-25T08:16:02Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "rename": {
        "from": "port srange to Cython for speed [patch attached]",
        "to": "port srange to Cython for speed"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/701#event-3148"
}
```



---

archive/issue_events_003149.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-04T03:16:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/701",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/701#event-3149"
}
```
