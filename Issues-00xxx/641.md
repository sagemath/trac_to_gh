# Issue 641: from sage.all import * for spyx files/cython notebook cells

archive/issues_000641.json:
```json
{
    "assignees": [],
    "body": "In spyx files, and cython blocks of the notebook, all names in sage.all are available (one doesn't need to import Integer, et al.) \n\nThis means that miss-spelled/non-existent names will not be caught at compile time though. \n\n\nAssignee: **@robertwb**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/641_\n\n",
    "closed_at": "2007-09-15T21:04:59Z",
    "created_at": "2007-09-12T04:48:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "from sage.all import * for spyx files/cython notebook cells",
    "type": "issue",
    "updated_at": "2007-09-15T21:04:59Z",
    "url": "https://github.com/sagemath/sage/issues/641",
    "user": "https://github.com/robertwb"
}
```
In spyx files, and cython blocks of the notebook, all names in sage.all are available (one doesn't need to import Integer, et al.) 

This means that miss-spelled/non-existent names will not be caught at compile time though. 


Assignee: **@robertwb**

_Issue created by migration from https://trac.sagemath.org/ticket/641_





---

archive/issue_events_005182.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-09-12T04:48:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "milestone_number": null,
    "milestone_title": "sage-2.8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/641#event-5182"
}
```



---

archive/issue_events_005183.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-09-12T04:48:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/641#event-5183"
}
```



---

archive/issue_events_005184.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-09-12T04:48:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/641#event-5184"
}
```



---

archive/issue_events_005185.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-09-12T04:48:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/641#event-5185"
}
```



---

archive/issue_comments_002397.json:
```json
{
    "body": "Changed assignee from **boothby** to **@robertwb**",
    "created_at": "2007-09-12T04:51:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/641#issuecomment-2397",
    "user": "https://github.com/robertwb"
}
```

Changed assignee from **boothby** to **@robertwb**



---

archive/issue_comments_002398.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[cython-sage-all.patch.gz](https://github.com/sagemath/sage/files/ticket641/cython-sage-all.patch.gz)**",
    "created_at": "2007-09-12T04:51:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/641#issuecomment-2398",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[cython-sage-all.patch.gz](https://github.com/sagemath/sage/files/ticket641/cython-sage-all.patch.gz)**



---

archive/issue_events_005186.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-15T19:19:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/641#event-5186"
}
```



---

archive/issue_events_005187.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-15T19:19:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "milestone_number": null,
    "milestone_title": "sage-2.8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/641#event-5187"
}
```



---

archive/issue_events_005188.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-15T21:04:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/641#event-5188"
}
```



---

archive/issue_comments_002399.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThis is frickin' awesome.  In sage-2.8.4.3.  Done.",
    "created_at": "2007-09-15T21:04:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/641#issuecomment-2399",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:5'>Comment 5:</a>
This is frickin' awesome.  In sage-2.8.4.3.  Done.
