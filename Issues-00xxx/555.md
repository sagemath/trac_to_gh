...
```json
{
    "body": "<a id='comment:2'></a>I'm pretty sure this is fixed. Basically, the problem is that scons doesn't want to look at a remote file (in our case $SAGE_ROOT/local/lib/libcsage.dylib) and see if it's up to date. The fix: just force it to copy the file over every time. Not the most elegant, but it works.\n\nhg bundle is attached.",
...
<a id='comment:2'></a>I'm pretty sure this is fixed. Basically, the problem is that scons doesn't want to look at a remote file (in our case $SAGE_ROOT/local/lib/libcsage.dylib) and see if it's up to date. The fix: just force it to copy the file over every time. Not the most elegant, but it works.
...
```json
{
    "body": "<a id='comment:3'></a>I can't apply this patch no matter what I try...",
...
<a id='comment:3'></a>I can't apply this patch no matter what I try...
...
<a id='comment:4'></a>Attachment [total_trac_555.hg](tarball://root/attachments/some-uuid/ticket555/total_trac_555.hg) by @craigcitro created at 2007-09-03 07:02:01
...
```json
{
    "body": "<a id='comment:5'></a>I've pushed this all into the official repo.",
...
<a id='comment:5'></a>I've pushed this all into the official repo.
