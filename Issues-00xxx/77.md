# Issue 77: 'attach' command doesn't show up with TABbing

archive/issues_000077.json:
```json
{
    "assignees": [],
    "body": "sage: atta[TAB]ched_files\n\nAnother indication that 'attach' is not a first-class citizen.\n\n\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/77_\n\n",
    "closed_at": "2006-09-23T01:00:12Z",
    "created_at": "2006-09-22T22:29:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "'attach' command doesn't show up with TABbing",
    "type": "issue",
    "updated_at": "2006-09-23T01:00:12Z",
    "url": "https://github.com/sagemath/sage/issues/77",
    "user": "Justin Walker (justin@mac.com)"
}
```
sage: atta[TAB]ched_files

Another indication that 'attach' is not a first-class citizen.



Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/77_





---

archive/issue_events_000584.json:
```json
{
    "actor": null,
    "created_at": "2006-09-22T22:29:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/77",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/77#event-584"
}
```



---

archive/issue_events_000585.json:
```json
{
    "actor": null,
    "created_at": "2006-09-22T22:29:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/77",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/77#event-585"
}
```



---

archive/issue_events_000586.json:
```json
{
    "actor": null,
    "created_at": "2006-09-22T22:29:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/77",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/77#event-586"
}
```



---

archive/issue_comments_000266.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nclosed with #76",
    "created_at": "2006-09-23T01:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/77",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/77#issuecomment-266",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

closed with #76



---

archive/issue_events_000587.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-23T01:00:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/77",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/77#event-587"
}
```
