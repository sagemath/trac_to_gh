# Issue 451: flintqs Solaris build fixes

archive/issues_000451.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "flintqs-20070505 uses linux-ism for types. In lanzos.h add\n\n#ifdef __sun\n#define u_int32_t unsigned int\n#define u_int64_t unsigned long long\n#endif \n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/451_\n\n",
    "closed_at": "2007-09-04T11:51:45Z",
    "created_at": "2007-08-19T07:55:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "flintqs Solaris build fixes",
    "type": "issue",
    "updated_at": "2007-09-04T11:51:45Z",
    "url": "https://github.com/sagemath/sage/issues/451",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
flintqs-20070505 uses linux-ism for types. In lanzos.h add

#ifdef __sun
#define u_int32_t unsigned int
#define u_int64_t unsigned long long
#endif 

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/451_





---

archive/issue_events_003830.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-19T07:55:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3830"
}
```



---

archive/issue_events_003831.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-19T07:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3831"
}
```



---

archive/issue_events_003832.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-19T07:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3832"
}
```



---

archive/issue_events_003833.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-19T07:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3833"
}
```



---

archive/issue_events_003834.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-19T07:55:51Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3834"
}
```



---

archive/issue_events_003835.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-21T12:31:37Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3835"
}
```



---

archive/issue_events_003836.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-21T12:31:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3836"
}
```



---

archive/issue_comments_001581.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis has been fixed upstream by Bill some time before Sage 2.8.3\n\nCheers,\n\nMichael",
    "created_at": "2007-09-04T11:51:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/451#issuecomment-1581",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

This has been fixed upstream by Bill some time before Sage 2.8.3

Cheers,

Michael



---

archive/issue_events_003837.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-04T11:51:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3837"
}
```



---

archive/issue_events_003838.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-04T11:51:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3838"
}
```



---

archive/issue_events_003839.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-04T11:51:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/451",
    "milestone_number": null,
    "milestone_title": "sage-2.8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/451#event-3839"
}
```
