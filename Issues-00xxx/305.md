# Issue 305: weird pickling bug in padics

archive/issues_000305.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: K = pAdicField(5)\nsage: x = K(0)\nsage: y = loads(dumps(x))\nsage: x.parent().print_prec()\n Infinity\nsage: y.parent().print_prec()\n <class 'sage.rings.padic_field.pAdicField_generic'>\n```\n\n\n_Issue created by migration from https://trac.sagemath.org/ticket/305_\n\n",
    "closed_at": "2007-03-06T22:22:57Z",
    "created_at": "2007-03-03T14:57:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "weird pickling bug in padics",
    "type": "issue",
    "updated_at": "2007-03-06T22:22:57Z",
    "url": "https://github.com/sagemath/sage/issues/305",
    "user": "https://github.com/sagetrac-dmharvey"
}
```

```
sage: K = pAdicField(5)
sage: x = K(0)
sage: y = loads(dumps(x))
sage: x.parent().print_prec()
 Infinity
sage: y.parent().print_prec()
 <class 'sage.rings.padic_field.pAdicField_generic'>
```


_Issue created by migration from https://trac.sagemath.org/ticket/305_





---

archive/issue_events_002261.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-03-03T14:57:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/305",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/305#event-2261"
}
```



---

archive/issue_events_002262.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-03-03T14:57:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/305",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/305#event-2262"
}
```



---

archive/issue_events_002263.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-03-03T14:57:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/305",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/305#event-2263"
}
```



---

archive/issue_comments_001094.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nTwo more examples of weirdness:\n\n```\nsage: K = pAdicField(5)\nsage: x = K(0)\nsage: x.prec()\n Infinity\nsage: y = loads(dumps(x))\nsage: y.prec()\n <class 'sage.rings.padic_field.pAdicField_generic'>\n```\n\nand\n\n```\nsage: K = pAdicField(5)\nsage: x = K(42)\nsage: x.prec()\n Infinity\nsage: y = loads(dumps(x))\nsage: y.prec()\n '_pAdicField_generic__p'\n```",
    "created_at": "2007-03-03T15:09:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/305#issuecomment-1094",
    "user": "https://github.com/sagetrac-dmharvey"
}
```

<a id='comment:1'>Comment 1:</a>
Two more examples of weirdness:

```
sage: K = pAdicField(5)
sage: x = K(0)
sage: x.prec()
 Infinity
sage: y = loads(dumps(x))
sage: y.prec()
 <class 'sage.rings.padic_field.pAdicField_generic'>
```

and

```
sage: K = pAdicField(5)
sage: x = K(42)
sage: x.prec()
 Infinity
sage: y = loads(dumps(x))
sage: y.prec()
 '_pAdicField_generic__p'
```



---

archive/issue_comments_001095.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis is fixed by David Roe's new p-adics in SAGE-2.3.",
    "created_at": "2007-03-06T22:22:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/305#issuecomment-1095",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'>Comment 2:</a>
This is fixed by David Roe's new p-adics in SAGE-2.3.



---

archive/issue_events_002264.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-03-06T22:22:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/305",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/305#event-2264"
}
```
