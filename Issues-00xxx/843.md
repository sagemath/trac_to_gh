# Issue 843: fast quadratic field elements

archive/issues_000843.json:
```json
{
    "assignees": [],
    "body": "This patch fixes several bugs in Robert's fast quadratic field element code, and SWITCHES THEM ON.\n\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/843_\n\n",
    "closed_at": "2007-10-13T06:35:26Z",
    "created_at": "2007-10-10T01:16:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fast quadratic field elements",
    "type": "issue",
    "updated_at": "2007-10-13T06:35:26Z",
    "url": "https://github.com/sagemath/sage/issues/843",
    "user": "https://github.com/sagetrac-dmharvey"
}
```
This patch fixes several bugs in Robert's fast quadratic field element code, and SWITCHES THEM ON.


Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/843_





---

archive/issue_events_008127.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-10T01:16:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/843",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/843#event-8127"
}
```



---

archive/issue_events_008128.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-10T01:16:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/843",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/843#event-8128"
}
```



---

archive/issue_events_008129.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-10T01:16:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/843",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/843#event-8129"
}
```



---

archive/issue_comments_003764.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[quadratic.hg.gz](https://github.com/sagemath/sage/files/ticket843/quadratic.hg.gz)**",
    "created_at": "2007-10-10T01:41:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/843#issuecomment-3764",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[quadratic.hg.gz](https://github.com/sagemath/sage/files/ticket843/quadratic.hg.gz)**



---

archive/issue_events_008130.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-10T01:41:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/843",
    "milestone_number": null,
    "milestone_title": "sage-2.8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/843#event-8130"
}
```



---

archive/issue_events_008131.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T06:35:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/843",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/843#event-8131"
}
```
