# Issue 699: [with patch] implement computation of twisted winding elements (a technical thing involving modular symbols)

archive/issues_000699.json:
```json
{
    "body": "Craig just did this.  Now I need to test it out, etc. \n\nAssignee: @craigcitro\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/699\n\n",
    "closed_at": "2007-10-21T03:14:53Z",
    "created_at": "2007-09-19T23:59:54Z",
    "labels": [
        "component: modular forms"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.8",
    "title": "[with patch] implement computation of twisted winding elements (a technical thing involving modular symbols)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/699",
    "user": "https://github.com/williamstein"
}
```
Craig just did this.  Now I need to test it out, etc. 

Assignee: @craigcitro

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/699





---

archive/issue_comments_003764.json:
```json
{
    "body": "Attachment [winding_elem.hg](tarball://root/attachments/some-uuid/ticket699/winding_elem.hg) by @williamstein created at 2007-09-20 00:00:04",
    "created_at": "2007-09-20T00:00:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/699#issuecomment-3764",
    "user": "https://github.com/williamstein"
}
```

Attachment [winding_elem.hg](tarball://root/attachments/some-uuid/ticket699/winding_elem.hg) by @williamstein created at 2007-09-20 00:00:04



---

archive/issue_comments_003765.json:
```json
{
    "body": "Changing assignee from craig citro to @craigcitro.",
    "created_at": "2007-10-02T00:34:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/699#issuecomment-3765",
    "user": "https://github.com/craigcitro"
}
```

Changing assignee from craig citro to @craigcitro.



---

archive/issue_comments_003766.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-10-02T00:34:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/699#issuecomment-3766",
    "user": "https://github.com/craigcitro"
}
```

Changing status from new to assigned.



---

archive/issue_events_002215.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2007-10-12T22:04:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "milestone": "sage-2.8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/699#event-2215"
}
```



---

archive/issue_events_002216.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "rename": {
        "from": "implement computation of twisted winding elements (a technical thing involving modular symbols)",
        "to": "[with patch] implement computation of twisted winding elements (a technical thing involving modular symbols)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/699#event-2216"
}
```



---

archive/issue_comments_003767.json:
```json
{
    "body": "<a id='comment:3'></a>\nI'm changing this to 2.8.7, because (1) it's done, and (2) I'm going to use it for something else soon (see trac ticket 812).",
    "created_at": "2007-10-12T22:04:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/699#issuecomment-3767",
    "user": "https://github.com/craigcitro"
}
```

<a id='comment:3'></a>
I'm changing this to 2.8.7, because (1) it's done, and (2) I'm going to use it for something else soon (see trac ticket 812).



---

archive/issue_events_002217.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T07:53:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "milestone": "sage-2.8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/699#event-2217"
}
```



---

archive/issue_events_002218.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T07:53:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "milestone": "sage-2.8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/699#event-2218"
}
```



---

archive/issue_comments_003768.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-21T03:14:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/699#issuecomment-3768",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed



---

archive/issue_events_002219.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-21T03:14:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/699",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/699#event-2219"
}
```
