# Issue 860: dsage -- update to fix a bunch of "rough edges"

archive/issues_000860.json:
```json
{
    "assignees": [],
    "body": "\n```\nthe rough edges that William mentioned.\n\nhttp://sage.math.washington.edu/home/yqiang/dsage_latest.hg\n\nThere is documentation on how to do this here:\n\nhttp://www.sagemath.org/doc/html/ref/module-sage.misc.hg.html\n\nLet me know if you need any help doing this. \n```\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/860_\n\n",
    "closed_at": "2007-10-13T01:38:55Z",
    "created_at": "2007-10-12T06:05:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "dsage -- update to fix a bunch of \"rough edges\"",
    "type": "issue",
    "updated_at": "2007-10-13T01:38:55Z",
    "url": "https://github.com/sagemath/sage/issues/860",
    "user": "https://github.com/williamstein"
}
```

```
the rough edges that William mentioned.

http://sage.math.washington.edu/home/yqiang/dsage_latest.hg

There is documentation on how to do this here:

http://www.sagemath.org/doc/html/ref/module-sage.misc.hg.html

Let me know if you need any help doing this. 
```

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/860_





---

archive/issue_events_007368.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-12T06:05:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/860",
    "milestone_number": null,
    "milestone_title": "sage-2.8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/860#event-7368"
}
```



---

archive/issue_events_007369.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-12T06:05:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/860",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/860#event-7369"
}
```



---

archive/issue_events_007370.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-12T06:05:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/860",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/860#event-7370"
}
```



---

archive/issue_events_007371.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-12T06:05:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/860",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/860#event-7371"
}
```



---

archive/issue_comments_004066.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nHmm, I don't see any patch. Did it get merged already?\n\nCheers,\n\nMichael",
    "created_at": "2007-10-12T16:24:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/860#issuecomment-4066",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:4'>Comment 4:</a>
Hmm, I don't see any patch. Did it get merged already?

Cheers,

Michael



---

archive/issue_events_007372.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T01:38:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/860",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/860#event-7372"
}
```
