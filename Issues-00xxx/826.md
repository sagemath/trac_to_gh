# Issue 826: [with patch] [tested by cwitty] Coercing rational function elements (i.e. fraction field) to symbolic ring

archive/issues_000826.json:
```json
{
    "body": "Currently, there is no coercion from a fraction of polynomials to the symbolic ring, but there is for polynomials.  I added coercion for the fraction field elements in the attached patch.\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/826\n\n",
    "closed_at": "2007-10-13T07:30:16Z",
    "created_at": "2007-10-04T19:29:08Z",
    "labels": [
        "component: calculus",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.7",
    "title": "[with patch] [tested by cwitty] Coercing rational function elements (i.e. fraction field) to symbolic ring",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/826",
    "user": "https://trac.sagemath.org/admin/accounts/users/jbmohler"
}
```
Currently, there is no coercion from a fraction of polynomials to the symbolic ring, but there is for polynomials.  I added coercion for the fraction field elements in the attached patch.

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/826





---

archive/issue_comments_005377.json:
```json
{
    "body": "the patch!",
    "created_at": "2007-10-04T19:29:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/826",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/826#issuecomment-5377",
    "user": "https://trac.sagemath.org/admin/accounts/users/jbmohler"
}
```

the patch!



---

archive/issue_events_002811.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-10-04T19:30:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/826",
    "milestone": "sage-2.8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/826#event-2811"
}
```



---

archive/issue_comments_005378.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [symbolic_coercion.hg](tarball://root/attachments/some-uuid/ticket826/symbolic_coercion.hg) by mabshoff created at 2007-10-04 19:30:06",
    "created_at": "2007-10-04T19:30:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/826",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/826#issuecomment-5378",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Attachment [symbolic_coercion.hg](tarball://root/attachments/some-uuid/ticket826/symbolic_coercion.hg) by mabshoff created at 2007-10-04 19:30:06



---

archive/issue_events_002812.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/826",
    "rename": {
        "from": "Coercing rational function elements (i.e. fraction field) to symbolic ring",
        "to": "[with patch] Coercing rational function elements (i.e. fraction field) to symbolic ring"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/826#event-2812"
}
```



---

archive/issue_events_002813.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/826",
    "rename": {
        "from": "[with patch] Coercing rational function elements (i.e. fraction field) to symbolic ring",
        "to": "[with patch] [tested by cwitty] Coercing rational function elements (i.e. fraction field) to symbolic ring"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/826#event-2813"
}
```



---

archive/issue_events_002814.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T07:30:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/826",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/826#event-2814"
}
```



---

archive/issue_comments_005379.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-13T07:30:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/826",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/826#issuecomment-5379",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed
