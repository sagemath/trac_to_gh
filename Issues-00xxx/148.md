# Issue 148: links in HTML version don't work

archive/issues_000148.json:
```json
{
    "body": "links in a notebook saved to html point to localhost -- they should just be relative anyway.\n\n**Assignee:** boothby\n\nIssue created by migration from https://trac.sagemath.org/ticket/148\n\n",
    "closed_at": "2007-01-22T01:43:54Z",
    "created_at": "2006-10-23T16:26:43Z",
    "labels": [
        "component: notebook",
        "trivial",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-1.8",
    "title": "links in HTML version don't work",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/148",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```
links in a notebook saved to html point to localhost -- they should just be relative anyway.

**Assignee:** boothby

Issue created by migration from https://trac.sagemath.org/ticket/148





---

archive/issue_events_000476.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2006-10-23T16:27:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/148",
    "label": "component: algebraic geometry",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/148#event-476"
}
```



---

archive/issue_events_000477.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2006-10-23T16:27:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/148",
    "label": "component: notebook",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/148#event-477"
}
```



---

archive/issue_comments_000497.json:
```json
{
    "body": "**Changing assignee** from @williamstein to boothby.",
    "created_at": "2006-10-23T16:27:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/148#issuecomment-497",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

**Changing assignee** from @williamstein to boothby.



---

archive/issue_events_000478.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2006-10-23T16:27:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/148",
    "label": "trivial",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/148#event-478"
}
```



---

archive/issue_events_000479.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-19T09:57:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/148",
    "milestone": "sage-1.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/148#event-479"
}
```



---

archive/issue_comments_000498.json:
```json
{
    "body": "<a id='comment:3'></a>\nthis simply isn't true.  I tried this both with print mode and\ndirectly from a worksheet with this in edit mode followed by\nio cells.  \n\n```\n\n<a href=\"/trac92\">trac92</a>\n\n\n```\n\n\nIn each case the link does not go to localhost.  It's absolute. \nAnyway, I do not understand this bug report at all.",
    "created_at": "2007-01-19T11:32:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/148#issuecomment-498",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
this simply isn't true.  I tried this both with print mode and
directly from a worksheet with this in edit mode followed by
io cells.  

```

<a href="/trac92">trac92</a>


```


In each case the link does not go to localhost.  It's absolute. 
Anyway, I do not understand this bug report at all.



---

archive/issue_events_000480.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-22T01:43:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/148",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/148#event-480"
}
```



---

archive/issue_comments_000499.json:
```json
{
    "body": "<a id='comment:4'></a>\nNot a problem anymore...",
    "created_at": "2007-01-22T01:43:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/148#issuecomment-499",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:4'></a>
Not a problem anymore...
