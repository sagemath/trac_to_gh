...
```json
{
    "body": "<a id='comment:1'></a>The expression `x^8+x+1` is now a symbolic expression, not a polynomial, so the `change_ring` method is no longer valid.\n\nIn any case, the bug appears to be fixed now, so I'm closing this ticket.",
...
<a id='comment:1'></a>The expression `x^8+x+1` is now a symbolic expression, not a polynomial, so the `change_ring` method is no longer valid.
