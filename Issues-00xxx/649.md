# Issue 649: [with patch, positive review] create a special symbolic matrix data type

archive/issues_000649.json:
```json
{
    "body": "It should have:\n* simplify_* methods\n* underlying data should be a pointer to a maxima matrix object, so that matrix operations are very fast.\n\n\nThis should also resolve a bug reported by Kate Minola on 20070914 with doctesting \n\n     sage -t devel/sage-main/sage/plot/plot3d/transform.pyx\n\nAssignee: @mwhansen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/649\n\n",
    "closed_at": "2007-12-22T18:37:41Z",
    "created_at": "2007-09-13T18:44:26Z",
    "labels": [
        "component: calculus"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.9.1",
    "title": "[with patch, positive review] create a special symbolic matrix data type",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/649",
    "user": "https://github.com/robertwb"
}
```
It should have:
* simplify_* methods
* underlying data should be a pointer to a maxima matrix object, so that matrix operations are very fast.


This should also resolve a bug reported by Kate Minola on 20070914 with doctesting 

     sage -t devel/sage-main/sage/plot/plot3d/transform.pyx

Assignee: @mwhansen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/649





---

archive/issue_events_002037.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-09-13T19:42:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "milestone": "sage-feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/649#event-2037"
}
```



---

archive/issue_comments_003474.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,8 @@\n+It should have:\n+* simplify_* methods\n+* underlying data should be a pointer to a maxima matrix object, so that matrix operations are very fast.\n \n+\n+This should also resolve a bug reported by Kate Minola on 20070914 with doctesting \n+\n+     sage -t devel/sage-main/sage/plot/plot3d/transform.pyx\n``````\n",
    "created_at": "2007-09-14T17:31:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/649#issuecomment-3474",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,8 @@
+It should have:
+* simplify_* methods
+* underlying data should be a pointer to a maxima matrix object, so that matrix operations are very fast.
 
+
+This should also resolve a bug reported by Kate Minola on 20070914 with doctesting 
+
+     sage -t devel/sage-main/sage/plot/plot3d/transform.pyx
``````




---

archive/issue_events_002038.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "rename": {
        "from": "symbolic matrices need simplify_* methods",
        "to": "create a special symbolic matrix data type"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/649#event-2038"
}
```



---

archive/issue_events_002039.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-14T17:31:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "milestone": "sage-feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/649#event-2039"
}
```



---

archive/issue_events_002040.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-14T17:31:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/649#event-2040"
}
```



---

archive/issue_comments_003475.json:
```json
{
    "body": "Changing assignee from @williamstein to @mwhansen.",
    "created_at": "2007-10-19T18:43:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/649#issuecomment-3475",
    "user": "https://github.com/mwhansen"
}
```

Changing assignee from @williamstein to @mwhansen.



---

archive/issue_comments_003476.json:
```json
{
    "body": "<a id='comment:5'></a>\nAttachment [649-symbolic-matrices.hg](tarball://root/attachments/some-uuid/ticket649/649-symbolic-matrices.hg) by @robertwb created at 2007-12-20 10:47:34\n\nI've posted a preliminary bundle. Something's up with the eigenvalues command however.",
    "created_at": "2007-12-20T10:47:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/649#issuecomment-3476",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:5'></a>
Attachment [649-symbolic-matrices.hg](tarball://root/attachments/some-uuid/ticket649/649-symbolic-matrices.hg) by @robertwb created at 2007-12-20 10:47:34

I've posted a preliminary bundle. Something's up with the eigenvalues command however.



---

archive/issue_events_002041.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-12-20T10:47:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/649#event-2041"
}
```



---

archive/issue_events_002042.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-12-20T10:47:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "milestone": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/649#event-2042"
}
```



---

archive/issue_comments_003477.json:
```json
{
    "body": "Attachment [649.patch](tarball://root/attachments/some-uuid/ticket649/649.patch) by @mwhansen created at 2007-12-22 18:18:00",
    "created_at": "2007-12-22T18:18:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/649#issuecomment-3477",
    "user": "https://github.com/mwhansen"
}
```

Attachment [649.patch](tarball://root/attachments/some-uuid/ticket649/649.patch) by @mwhansen created at 2007-12-22 18:18:00



---

archive/issue_comments_003478.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-12-22T18:19:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/649#issuecomment-3478",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_comments_003479.json:
```json
{
    "body": "<a id='comment:6'></a>\nPatch attached (which unintentionally includes the changes from Robert's bundle).",
    "created_at": "2007-12-22T18:19:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/649#issuecomment-3479",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:6'></a>
Patch attached (which unintentionally includes the changes from Robert's bundle).



---

archive/issue_events_002043.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "rename": {
        "from": "create a special symbolic matrix data type",
        "to": "[with patch] create a special symbolic matrix data type"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/649#event-2043"
}
```



---

archive/issue_comments_003480.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-12-22T18:37:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/649#issuecomment-3480",
    "user": "https://github.com/rlmill"
}
```

Resolution: fixed



---

archive/issue_events_002044.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "rename": {
        "from": "[with patch] create a special symbolic matrix data type",
        "to": "[with patch, positive review] create a special symbolic matrix data type"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/649#event-2044"
}
```



---

archive/issue_comments_003481.json:
```json
{
    "body": "<a id='comment:7'></a>\nmerged in 2.9.1 rc0",
    "created_at": "2007-12-22T18:37:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/649#issuecomment-3481",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:7'></a>
merged in 2.9.1 rc0



---

archive/issue_events_002045.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-22T18:37:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/649",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/649#event-2045"
}
```
