# Issue 117: Trac: Can't view tickets I created

archive/issues_000117.json:
```json
{
    "assignees": [],
    "body": "Subject line says it all.  Is  there a way that I missed?\n\nThis is not a SAGE problem.\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/117_\n\n",
    "closed_at": "2007-01-08T19:04:10Z",
    "created_at": "2006-10-07T20:27:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Trac: Can't view tickets I created",
    "type": "issue",
    "updated_at": "2007-01-08T19:04:10Z",
    "url": "https://github.com/sagemath/sage/issues/117",
    "user": "https://github.com/sagetrac-justin"
}
```
Subject line says it all.  Is  there a way that I missed?

This is not a SAGE problem.

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/117_





---

archive/issue_events_000781.json:
```json
{
    "actor": "https://github.com/sagetrac-justin",
    "created_at": "2006-10-07T20:27:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/117",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/117#event-781"
}
```



---

archive/issue_events_000782.json:
```json
{
    "actor": "https://github.com/sagetrac-justin",
    "created_at": "2006-10-07T20:27:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/117",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/117#event-782"
}
```



---

archive/issue_events_000783.json:
```json
{
    "actor": "https://github.com/sagetrac-justin",
    "created_at": "2006-10-07T20:27:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/117",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/117#event-783"
}
```



---

archive/issue_comments_000397.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI know I can search for 'justin', but then I get false positives, like tickets containing \"adjusting\".",
    "created_at": "2006-10-07T20:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/117",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/117#issuecomment-397",
    "user": "https://github.com/sagetrac-justin"
}
```

<a id='comment:1'>Comment 1:</a>
I know I can search for 'justin', but then I get false positives, like tickets containing "adjusting".



---

archive/issue_comments_000398.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,3 @@\n Subject line says it all.  Is  there a way that I missed?\n \n+This is not a SAGE problem.\n``````\n",
    "created_at": "2007-01-08T19:04:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/117",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/117#issuecomment-398",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,3 @@
 Subject line says it all.  Is  there a way that I missed?
 
+This is not a SAGE problem.
``````




---

archive/issue_events_000784.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-08T19:04:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/117",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/117#event-784"
}
```
