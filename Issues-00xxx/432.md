# Issue 432: finish implementing deleting the trash (emptying it) in the sage notebook.

archive/issues_000432.json:
```json
{
    "body": "I just never got around to implementing it.\n\n**Assignee:** boothby\n\nIssue created by migration from https://trac.sagemath.org/ticket/432\n\n",
    "closed_at": "2008-03-17T04:01:40Z",
    "created_at": "2007-08-16T09:43:33Z",
    "labels": [
        "component: notebook",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.4",
    "title": "finish implementing deleting the trash (emptying it) in the sage notebook.",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/432",
    "user": "https://github.com/williamstein"
}
```
I just never got around to implementing it.

**Assignee:** boothby

Issue created by migration from https://trac.sagemath.org/ticket/432





---

archive/issue_events_001765.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-08-16T09:43:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "label": "component: algebraic geometry",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1765"
}
```



---

archive/issue_events_001766.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-08-16T09:43:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "label": "component: notebook",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1766"
}
```



---

archive/issue_comments_001618.json:
```json
{
    "body": "**Changing assignee** from @williamstein to boothby.",
    "created_at": "2007-08-16T09:43:41Z",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/432#issuecomment-1618",
    "user": "https://github.com/williamstein"
}
```

**Changing assignee** from @williamstein to boothby.



---

archive/issue_events_001767.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-09-10T02:46:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "milestone": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1767"
}
```



---

archive/issue_comments_001619.json:
```json
{
    "body": "<a id='comment:3'></a>\nJust a remark from a user:\n\n```\n\n> But, when I archived all trash content, it hangs... Closed Firefox,\n> VMware Player, reboot - everything was to-o-o-oo slow and buggy,\n> almost any loading of saved file caused an error.\n> \n> I ended up deleting all sage files and unpacking them again from\n> distribution ZIP. Now everything is fast again.\n```",
    "created_at": "2007-12-13T18:46:56Z",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/432#issuecomment-1619",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
Just a remark from a user:

```

> But, when I archived all trash content, it hangs... Closed Firefox,
> VMware Player, reboot - everything was to-o-o-oo slow and buggy,
> almost any loading of saved file caused an error.
> 
> I ended up deleting all sage files and unpacking them again from
> distribution ZIP. Now everything is fast again.
```



---

archive/issue_events_001768.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-18T10:00:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "milestone": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1768"
}
```



---

archive/attachments_000138.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage-432.patch",
    "asset_url": "tarball://root/attachments/ticket432/sage-432.patch",
    "created_at": "2008-03-13T15:56:56Z",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket432/sage-432.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_001620.json:
```json
{
    "body": "<a id='comment:5'></a>\nAttachment [sage-432.patch](https://github.com/sagemath/sage/files/ticket432/sage-432.patch) by @williamstein created at 2008-03-13 15:56:56\n\nI finally implemented this.  And it's with 100% doctest coverage of new functions, which is a new thing for the notebook (to have any docs at all).",
    "created_at": "2008-03-13T15:56:56Z",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/432#issuecomment-1620",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:5'></a>
Attachment [sage-432.patch](https://github.com/sagemath/sage/files/ticket432/sage-432.patch) by @williamstein created at 2008-03-13 15:56:56

I finally implemented this.  And it's with 100% doctest coverage of new functions, which is a new thing for the notebook (to have any docs at all).



---

archive/issue_events_001769.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-13T15:56:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1769"
}
```



---

archive/issue_events_001770.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-13T15:56:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "milestone": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1770"
}
```



---

archive/issue_events_001771.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-13T15:56:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "milestone": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1771"
}
```



---

archive/issue_events_001772.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-03-16T18:11:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1772"
}
```



---

archive/issue_events_001773.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-03-16T18:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1773"
}
```



---

archive/issue_comments_001621.json:
```json
{
    "body": "<a id='comment:6'></a>\nWorks for me.",
    "created_at": "2008-03-16T18:11:31Z",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/432#issuecomment-1621",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

<a id='comment:6'></a>
Works for me.



---

archive/issue_events_001774.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-17T04:01:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1774"
}
```



---

archive/issue_events_001775.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-17T04:01:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/432#event-1775"
}
```



---

archive/issue_comments_001622.json:
```json
{
    "body": "<a id='comment:7'></a>\nMerged in Sage 2.10.4.final",
    "created_at": "2008-03-17T04:01:40Z",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/432#issuecomment-1622",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:7'></a>
Merged in Sage 2.10.4.final



---

archive/issue_comments_001623.json:
```json
{
    "body": "<a id='comment:8'></a>\nTried the patch on 2.10.3 and still had some problems with it - worked sometimes but not others.\nInstalled 2.10.4.final which includes the patch and it now works fine. Could this be some\ninteraction with other patches?\nAnyway - seems fine under 2.10.4 now.",
    "created_at": "2008-03-18T11:46:42Z",
    "issue": "https://github.com/sagemath/sage/issues/432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/432#issuecomment-1623",
    "user": "https://trac.sagemath.org/admin/accounts/users/bill.p"
}
```

<a id='comment:8'></a>
Tried the patch on 2.10.3 and still had some problems with it - worked sometimes but not others.
Installed 2.10.4.final which includes the patch and it now works fine. Could this be some
interaction with other patches?
Anyway - seems fine under 2.10.4 now.
