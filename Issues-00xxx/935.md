# Issue 935: fix problem where some files don't show up in distributions (missing in MANIFEST.in)

archive/issues_000935.json:
```json
{
    "body": "Well, the title says it all. Patch is against 2.8.7.\n\nCheers,\n\nMichael\n\nAssignee: cwitty\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/935\n\n",
    "closed_at": "2007-10-28T21:40:17Z",
    "created_at": "2007-10-19T22:56:39Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.10",
    "title": "fix problem where some files don't show up in distributions (missing in MANIFEST.in)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/935",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Well, the title says it all. Patch is against 2.8.7.

Cheers,

Michael

Assignee: cwitty

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/935





---

archive/issue_comments_005979.json:
```json
{
    "body": "Attachment [sage-2.8.7-remove-deleted-files-from-repo.patch](tarball://root/attachments/some-uuid/ticket935/sage-2.8.7-remove-deleted-files-from-repo.patch) by mabshoff created at 2007-10-19 22:57:11",
    "created_at": "2007-10-19T22:57:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5979",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [sage-2.8.7-remove-deleted-files-from-repo.patch](tarball://root/attachments/some-uuid/ticket935/sage-2.8.7-remove-deleted-files-from-repo.patch) by mabshoff created at 2007-10-19 22:57:11



---

archive/issue_events_003167.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "rename": {
        "from": "remove no longer present files from the sage repo",
        "to": "[with patch] remove no longer present files from the sage repo"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/935#event-3167"
}
```



---

archive/issue_comments_005980.json:
```json
{
    "body": "Changing assignee from @williamstein to mabshoff.",
    "created_at": "2007-10-20T06:14:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5980",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing assignee from @williamstein to mabshoff.



---

archive/issue_comments_005981.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-10-20T06:14:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5981",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_005982.json:
```json
{
    "body": "<a id='comment:3'></a>\n\n```\n[21:20] <wstein> #935 -- reject.\n[21:20] <mabshoff> Why?\n[21:20] <wstein> Those files *should* be in sage.\n[21:20] <mabshoff> So why aren't they in the repo?\n[21:20] <wstein> probably they didn't get included because of a mistake in MANIFEST.in.\n[21:20] <mabshoff> ok\n[21:21] <mabshoff> So mark #935 as invalid?\n[21:21] <wstein> at least the one that doctests cvxopt, scipy, definitely should be remade.\n[21:21] <mabshoff> That is another ticket.\n[21:21] <wstein> It's not invalid, it's just that the fix isn't exactly right.\n```",
    "created_at": "2007-10-20T19:25:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5982",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>

```
[21:20] <wstein> #935 -- reject.
[21:20] <mabshoff> Why?
[21:20] <wstein> Those files *should* be in sage.
[21:20] <mabshoff> So why aren't they in the repo?
[21:20] <wstein> probably they didn't get included because of a mistake in MANIFEST.in.
[21:20] <mabshoff> ok
[21:21] <mabshoff> So mark #935 as invalid?
[21:21] <wstein> at least the one that doctests cvxopt, scipy, definitely should be remade.
[21:21] <mabshoff> That is another ticket.
[21:21] <wstein> It's not invalid, it's just that the fix isn't exactly right.
```



---

archive/issue_events_003168.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "rename": {
        "from": "[with patch] remove no longer present files from the sage repo",
        "to": "[with patch] fix supposedly missing files in sage repo"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/935#event-3168"
}
```



---

archive/issue_events_003169.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-21T01:05:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/935#event-3169"
}
```



---

archive/issue_comments_005983.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2007-10-21T01:05:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5983",
    "user": "https://github.com/williamstein"
}
```

Resolution: invalid



---

archive/issue_comments_005984.json:
```json
{
    "body": "Resolution changed from invalid to ",
    "created_at": "2007-10-28T16:07:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5984",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Resolution changed from invalid to 



---

archive/issue_comments_005985.json:
```json
{
    "body": "Changing status from closed to reopened.",
    "created_at": "2007-10-28T16:07:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5985",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Changing status from closed to reopened.



---

archive/issue_events_003170.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/cwitty",
    "created_at": "2007-10-28T16:07:51Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/935#event-3170"
}
```



---

archive/issue_events_003171.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "rename": {
        "from": "[with patch] fix supposedly missing files in sage repo",
        "to": "fix problem where some files don't show up in distributions (missing in MANIFEST.in)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/935#event-3171"
}
```



---

archive/issue_events_003172.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/cwitty",
    "created_at": "2007-10-28T16:07:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "milestone": "sage-2.8.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/935#event-3172"
}
```



---

archive/issue_comments_005986.json:
```json
{
    "body": "Changing status from reopened to new.",
    "created_at": "2007-10-28T16:07:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5986",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Changing status from reopened to new.



---

archive/issue_comments_005987.json:
```json
{
    "body": "Changing assignee from mabshoff to cwitty.",
    "created_at": "2007-10-28T16:07:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5987",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Changing assignee from mabshoff to cwitty.



---

archive/issue_comments_005988.json:
```json
{
    "body": "Attachment [935.patch](tarball://root/attachments/some-uuid/ticket935/935.patch) by cwitty created at 2007-10-28 17:48:58",
    "created_at": "2007-10-28T17:48:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5988",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Attachment [935.patch](tarball://root/attachments/some-uuid/ticket935/935.patch) by cwitty created at 2007-10-28 17:48:58



---

archive/issue_comments_005989.json:
```json
{
    "body": "<a id='comment:7'></a>\nI think the attached patch should fix this problem.  (I will close the ticket once I have actually built a distribution and checked that it worked.)",
    "created_at": "2007-10-28T17:49:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5989",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:7'></a>
I think the attached patch should fix this problem.  (I will close the ticket once I have actually built a distribution and checked that it worked.)



---

archive/issue_events_003173.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/cwitty",
    "created_at": "2007-10-28T21:40:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/935#event-3173"
}
```



---

archive/issue_comments_005990.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-28T21:40:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/935#issuecomment-5990",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Resolution: fixed
