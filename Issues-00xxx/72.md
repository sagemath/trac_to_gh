# Issue 72: trac doesn't handle control keys well

archive/issues_000072.json:
```json
{
    "body": "I don't know that this belongs here, but: emacs key bindings are hard-wired in my lizard brain, and I've noticed that not all control characters work: try ^F.\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/72\n\n",
    "closed_at": "2007-09-14T02:58:25Z",
    "created_at": "2006-09-20T21:43:29Z",
    "labels": [
        "component: user interface",
        "trivial",
        "bug",
        "invalid"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.5",
    "title": "trac doesn't handle control keys well",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/72",
    "user": "Justin Walker (justin@mac.com)"
}
```
I don't know that this belongs here, but: emacs key bindings are hard-wired in my lizard brain, and I've noticed that not all control characters work: try ^F.

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/72





---

archive/issue_comments_000286.json:
```json
{
    "body": "<a id='comment:1'></a>\nHmmm... that \"^F\" is meant to be \"CTRL-F\".",
    "created_at": "2006-09-20T21:44:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/72#issuecomment-286",
    "user": "Justin Walker (justin@mac.com)"
}
```

<a id='comment:1'></a>
Hmmm... that "^F" is meant to be "CTRL-F".



---

archive/issue_comments_000287.json:
```json
{
    "body": "<a id='comment:2'></a>\nIs this a SAGE problem?",
    "created_at": "2006-09-21T01:25:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/72#issuecomment-287",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
Is this a SAGE problem?



---

archive/issue_events_000288.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/justin",
    "created_at": "2006-10-07T23:16:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "label": "component: basic arithmetic",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/72#event-288"
}
```



---

archive/issue_events_000289.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/justin",
    "created_at": "2006-10-07T23:16:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "label": "component: user interface",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/72#event-289"
}
```



---

archive/issue_comments_000288.json:
```json
{
    "body": "**Assignee:** @williamstein",
    "created_at": "2006-10-07T23:16:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/72#issuecomment-288",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

**Assignee:** @williamstein



---

archive/issue_comments_000289.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [was](#comment%3A2):\n> Is this a SAGE problem? \n\n\nNo clue.  I was hoping an expert here would know.  This could be a problem in SAGE, browser, Trac.",
    "created_at": "2006-10-07T23:16:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/72#issuecomment-289",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

<a id='comment:3'></a>
Replying to [was](#comment%3A2):
> Is this a SAGE problem? 


No clue.  I was hoping an expert here would know.  This could be a problem in SAGE, browser, Trac.



---

archive/issue_events_000290.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-09-11T00:52:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "label": "feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/72#event-290"
}
```



---

archive/issue_events_000291.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-14T02:58:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "label": "invalid",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/72#event-291"
}
```



---

archive/issue_comments_000290.json:
```json
{
    "body": "<a id='comment:5'></a>\nThis is not a SAGE issue.",
    "created_at": "2007-09-14T02:58:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/72#issuecomment-290",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:5'></a>
This is not a SAGE issue.



---

archive/issue_events_000292.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-14T02:58:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "milestone": "sage-2.8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/72#event-292"
}
```



---

archive/issue_events_000293.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-14T02:58:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "label": "feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/72#event-293"
}
```



---

archive/issue_events_000294.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-14T02:58:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/72",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/72#event-294"
}
```
