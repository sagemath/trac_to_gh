...
```json
{
    "body": "<a id='comment:3'></a>Control-c works fine now (tested on linux and osx).  Maybe this was fixed by the other control-c related fixes that Gonzalo Tornaria and I made at SD5.",
...
<a id='comment:3'></a>Control-c works fine now (tested on linux and osx).  Maybe this was fixed by the other control-c related fixes that Gonzalo Tornaria and I made at SD5.
...
<a id='comment:4'></a>I found this ancient ticket while looking for anything mentioning kummer_surface.  With Sage 5.10 it does not work at all:
