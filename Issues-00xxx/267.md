# Issue 267: implement "sage -bt"

archive/issues_000267.json:
```json
{
    "assignees": [],
    "body": "`sage -bt filename` could run a `sage -b` and then run doctests on the indicated file\n\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/267\n\n",
    "closed_at": "2007-02-25T02:48:25Z",
    "created_at": "2007-02-17T20:59:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "implement \"sage -bt\"",
    "type": "issue",
    "updated_at": "2007-02-25T16:08:16Z",
    "url": "https://github.com/sagemath/sage/issues/267",
    "user": "https://github.com/sagetrac-dmharvey"
}
```
`sage -bt filename` could run a `sage -b` and then run doctests on the indicated file


**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/267





---

archive/issue_events_001730.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-02-17T20:59:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/267",
    "milestone_number": null,
    "milestone_title": "sage-2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/267#event-1730"
}
```



---

archive/issue_events_001731.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-02-17T20:59:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/267",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "08517b",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/267#event-1731"
}
```



---

archive/issue_events_001732.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-02-17T20:59:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/267",
    "label": "https://github.com/sagemath/sage/labels/minor",
    "label_color": "ff0000",
    "label_name": "minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/267#event-1732"
}
```



---

archive/issue_events_001733.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-02-17T20:59:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/267",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/267#event-1733"
}
```



---

archive/issue_comments_000947.json:
```json
{
    "body": "**Attachment:** [ncalexan-scripts-clone.hg.gz](https://github.com/sagemath/sage/files/ticket267/ncalexan-scripts-clone.hg.gz)",
    "created_at": "2007-02-25T02:43:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/267#issuecomment-947",
    "user": "https://github.com/ncalexan"
}
```

**Attachment:** [ncalexan-scripts-clone.hg.gz](https://github.com/sagemath/sage/files/ticket267/ncalexan-scripts-clone.hg.gz)



---

archive/issue_events_001734.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2007-02-25T02:48:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/267",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/267#event-1734"
}
```



---

archive/issue_comments_000948.json:
```json
{
    "body": "<a id='comment:1'>**Comment 1:**</a>\nAdds options \"-bt\" and \"-btnew\" for \"build current and test/test new\".\n\nAdds option \"-r clone\" for \"switch to and run clone\".\n\nChanges cloning so that \"-clone existing\" switches to \"existing\" clone.",
    "created_at": "2007-02-25T02:48:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/267#issuecomment-948",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:1'>**Comment 1:**</a>
Adds options "-bt" and "-btnew" for "build current and test/test new".

Adds option "-r clone" for "switch to and run clone".

Changes cloning so that "-clone existing" switches to "existing" clone.



---

archive/issue_events_001735.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-02-25T16:08:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/267",
    "milestone_number": null,
    "milestone_title": "sage-2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/267#event-1735"
}
```
