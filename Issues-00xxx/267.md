# Issue 267: implement "sage -bt"

archive/issues_000267.json:
```json
{
    "body": "`sage -bt filename` could run a `sage -b` and then run doctests on the indicated file\n\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/267\n\n",
    "closed_at": "2007-02-25T02:48:25Z",
    "created_at": "2007-02-17T20:59:16Z",
    "labels": [
        "component: user interface",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.2",
    "title": "implement \"sage -bt\"",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/267",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```
`sage -bt filename` could run a `sage -b` and then run doctests on the indicated file


Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/267





---

archive/issue_comments_001392.json:
```json
{
    "body": "Attachment [ncalexan-scripts-clone.hg](tarball://root/attachments/some-uuid/ticket267/ncalexan-scripts-clone.hg) by @ncalexan created at 2007-02-25 02:43:43",
    "created_at": "2007-02-25T02:43:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/267#issuecomment-1392",
    "user": "https://github.com/ncalexan"
}
```

Attachment [ncalexan-scripts-clone.hg](tarball://root/attachments/some-uuid/ticket267/ncalexan-scripts-clone.hg) by @ncalexan created at 2007-02-25 02:43:43



---

archive/issue_comments_001393.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-02-25T02:48:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/267#issuecomment-1393",
    "user": "https://github.com/ncalexan"
}
```

Resolution: fixed



---

archive/issue_events_000586.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2007-02-25T02:48:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/267",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/267#event-586"
}
```



---

archive/issue_comments_001394.json:
```json
{
    "body": "<a id='comment:1'></a>Adds options \"-bt\" and \"-btnew\" for \"build current and test/test new\".\n\nAdds option \"-r clone\" for \"switch to and run clone\".\n\nChanges cloning so that \"-clone existing\" switches to \"existing\" clone.",
    "created_at": "2007-02-25T02:48:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/267#issuecomment-1394",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:1'></a>Adds options "-bt" and "-btnew" for "build current and test/test new".

Adds option "-r clone" for "switch to and run clone".

Changes cloning so that "-clone existing" switches to "existing" clone.



---

archive/issue_events_000587.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-02-25T16:08:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/267",
    "milestone": "sage-2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/267#event-587"
}
```
