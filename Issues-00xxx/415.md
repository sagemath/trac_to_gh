# Issue 415: ZZ.random_element(0)

archive/issues_000415.json:
```json
{
    "assignees": [],
    "body": "ZZ.random_element(0)\n\nproduces an uncaught FPE and hence crashes sage.\n\n_Issue created by migration from https://trac.sagemath.org/ticket/415_\n\n",
    "closed_at": "2007-08-09T21:29:57Z",
    "created_at": "2007-08-09T19:53:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ZZ.random_element(0)",
    "type": "issue",
    "updated_at": "2007-08-09T21:29:57Z",
    "url": "https://github.com/sagemath/sage/issues/415",
    "user": "https://github.com/nbruin"
}
```
ZZ.random_element(0)

produces an uncaught FPE and hence crashes sage.

_Issue created by migration from https://trac.sagemath.org/ticket/415_





---

archive/issue_events_003172.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2007-08-09T19:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/415#event-3172"
}
```



---

archive/issue_events_003173.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2007-08-09T19:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/415#event-3173"
}
```



---

archive/issue_events_003174.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2007-08-09T19:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/415#event-3174"
}
```



---

archive/issue_comments_001523.json:
```json
{
    "body": "hg patch",
    "created_at": "2007-08-09T21:29:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/415#issuecomment-1523",
    "user": "https://github.com/malb"
}
```

hg patch



---

archive/issue_comments_001524.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_ticket_415.patch.gz](https://github.com/sagemath/sage/files/ticket415/trac_ticket_415.patch.gz)**\n\nfixed in attached patch.",
    "created_at": "2007-08-09T21:29:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/415#issuecomment-1524",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_ticket_415.patch.gz](https://github.com/sagemath/sage/files/ticket415/trac_ticket_415.patch.gz)**

fixed in attached patch.



---

archive/issue_events_003175.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-08-09T21:29:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/415#event-3175"
}
```
