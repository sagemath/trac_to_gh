# Issue 415: ZZ.random_element(0)

archive/issues_000415.json:
```json
{
    "assignees": [],
    "body": "ZZ.random_element(0)\n\nproduces an uncaught FPE and hence crashes sage.\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/415_\n\n",
    "closed_at": "2007-08-09T21:29:57Z",
    "created_at": "2007-08-09T19:53:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ZZ.random_element(0)",
    "type": "issue",
    "updated_at": "2007-08-09T21:29:57Z",
    "url": "https://github.com/sagemath/sage/issues/415",
    "user": "https://github.com/nbruin"
}
```
ZZ.random_element(0)

produces an uncaught FPE and hence crashes sage.

Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/415_





---

archive/issue_events_003384.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2007-08-09T19:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/415#event-3384"
}
```



---

archive/issue_events_003385.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2007-08-09T19:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
    "label_color": "ff7700",
    "label_name": "p: critical / 2",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/415#event-3385"
}
```



---

archive/issue_events_003386.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2007-08-09T19:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/415#event-3386"
}
```



---

archive/issue_comments_001436.json:
```json
{
    "body": "hg patch",
    "created_at": "2007-08-09T21:29:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/415#issuecomment-1436",
    "user": "https://github.com/malb"
}
```

hg patch



---

archive/issue_comments_001437.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_ticket_415.patch.gz](https://github.com/sagemath/sage/files/ticket415/trac_ticket_415.patch.gz)**\n\nfixed in attached patch.",
    "created_at": "2007-08-09T21:29:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/415#issuecomment-1437",
    "user": "https://github.com/malb"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_ticket_415.patch.gz](https://github.com/sagemath/sage/files/ticket415/trac_ticket_415.patch.gz)**

fixed in attached patch.



---

archive/issue_events_003387.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-08-09T21:29:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/415",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/415#event-3387"
}
```
