# Issue 74: switch to mercurial source control manager.

archive/issues_000074.json:
```json
{
    "assignees": [],
    "body": "Why: Because SAGE is huge and darcs doesn't scale sufficiently well.  \nMercurial does scale to the task, though is less capable.\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/74_\n\n",
    "closed_at": "2006-09-21T10:58:54Z",
    "created_at": "2006-09-21T01:30:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "switch to mercurial source control manager.",
    "type": "issue",
    "updated_at": "2006-09-21T10:58:54Z",
    "url": "https://github.com/sagemath/sage/issues/74",
    "user": "https://github.com/williamstein"
}
```
Why: Because SAGE is huge and darcs doesn't scale sufficiently well.  
Mercurial does scale to the task, though is less capable.

Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/74_





---

archive/issue_events_000569.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-21T01:30:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/74",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/74#event-569"
}
```



---

archive/issue_events_000570.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-21T01:30:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/74",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/74#event-570"
}
```



---

archive/issue_events_000571.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-21T01:30:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/74",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/74#event-571"
}
```



---

archive/issue_comments_000260.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is now finished for the source code.  \nWhat remains:\n\n* write hg_source, etc., python interfaces\n* move doc, extcode, and scripts into mercurial.",
    "created_at": "2006-09-21T09:47:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/74",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/74#issuecomment-260",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

This is now finished for the source code.  
What remains:

* write hg_source, etc., python interfaces
* move doc, extcode, and scripts into mercurial.



---

archive/issue_events_000572.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-21T10:58:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/74",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/74#event-572"
}
```
