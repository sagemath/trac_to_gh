# Issue 728: sage_inspect broken

archive/issues_000728.json:
```json
{
    "assignees": [],
    "body": "This is SAGE 2.8.5:\n\n```\nsage: A = random_matrix(ZZ,3,3)\nsage: A.lll?\nTraceback (most recent call last):\n  File \"<stdin>\", line 1, in <module>\n ... File \"/usr/local/sage-2.8.1/local/lib/python2.5/site-packages/sage/misc/sageinspect.py\", line 252, in sage_getargspec\n    args, varargs, varkw = inspect.getargs(func_obj.func_code)\nUnboundLocalError: local variable 'func_obj' referenced before assignment\nsage: e = 1\nsage: e.additive_order?\n\n  File \"<stdin>\", line 1, in <module>\n ... File \"/usr/local/sage-2.8.1/local/lib/python2.5/site-packages/sage/misc/sageinspect.py\", line 252, in sage_getargspec\n    args, varargs, varkw = inspect.getargs(func_obj.func_code)\nUnboundLocalError: local variable 'func_obj' referenced before assignment\n```\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/728_\n\n",
    "closed_at": "2007-10-04T18:44:20Z",
    "created_at": "2007-09-21T13:38:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage_inspect broken",
    "type": "issue",
    "updated_at": "2007-10-04T18:44:20Z",
    "url": "https://github.com/sagemath/sage/issues/728",
    "user": "https://github.com/malb"
}
```
This is SAGE 2.8.5:

```
sage: A = random_matrix(ZZ,3,3)
sage: A.lll?
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
 ... File "/usr/local/sage-2.8.1/local/lib/python2.5/site-packages/sage/misc/sageinspect.py", line 252, in sage_getargspec
    args, varargs, varkw = inspect.getargs(func_obj.func_code)
UnboundLocalError: local variable 'func_obj' referenced before assignment
sage: e = 1
sage: e.additive_order?

  File "<stdin>", line 1, in <module>
 ... File "/usr/local/sage-2.8.1/local/lib/python2.5/site-packages/sage/misc/sageinspect.py", line 252, in sage_getargspec
    args, varargs, varkw = inspect.getargs(func_obj.func_code)
UnboundLocalError: local variable 'func_obj' referenced before assignment
```

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/728_





---

archive/issue_events_006047.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-09-21T13:38:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/728",
    "milestone_number": null,
    "milestone_title": "sage-2.8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/728#event-6047"
}
```



---

archive/issue_events_006048.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-09-21T13:38:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/728",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/728#event-6048"
}
```



---

archive/issue_events_006049.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-09-21T13:38:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/728",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/728#event-6049"
}
```



---

archive/issue_events_006050.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-09-21T13:38:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/728",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/728#event-6050"
}
```



---

archive/issue_comments_003220.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nWhat platform do these errors occur on?\n\nBoth of the examples work fine for me on 64-bit Linux.",
    "created_at": "2007-09-26T20:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/728#issuecomment-3220",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'>Comment 1:</a>
What platform do these errors occur on?

Both of the examples work fine for me on 64-bit Linux.



---

archive/issue_events_006051.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-04T18:44:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/728",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/728#event-6051"
}
```



---

archive/issue_comments_003221.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI fixed this; it happened on os x.",
    "created_at": "2007-10-04T18:44:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/728#issuecomment-3221",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'>Comment 2:</a>
I fixed this; it happened on os x.
