# Issue 869: [tested by cwitty] graphs: bipartite graph functions

archive/issues_000869.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "Here are a few functions wrapped from the bipartite functions in networkx.\n\nKeywords: **graphs**\n\nComponent: **combinatorics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/869_\n\n",
    "closed_at": "2007-10-13T06:26:42Z",
    "created_at": "2007-10-13T00:45:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[tested by cwitty] graphs: bipartite graph functions",
    "type": "issue",
    "updated_at": "2007-10-13T06:26:42Z",
    "url": "https://github.com/sagemath/sage/issues/869",
    "user": "https://github.com/jasongrout"
}
```
Here are a few functions wrapped from the bipartite functions in networkx.

Keywords: **graphs**

Component: **combinatorics**

_Issue created by migration from https://trac.sagemath.org/ticket/869_





---

archive/issue_events_008347.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-10-13T00:45:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/869",
    "milestone_number": null,
    "milestone_title": "sage-2.8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/869#event-8347"
}
```



---

archive/issue_events_008348.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-10-13T00:45:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/869",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/869#event-8348"
}
```



---

archive/issue_events_008349.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-10-13T00:45:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/869",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/869#event-8349"
}
```



---

archive/issue_events_008350.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-10-13T00:45:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/869",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/869#event-8350"
}
```



---

archive/issue_events_008351.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T00:45:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/869",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/869#event-8351"
}
```



---

archive/issue_comments_003869.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[bipartite.patch.gz](https://github.com/sagemath/sage/files/ticket869/bipartite.patch.gz)**",
    "created_at": "2007-10-13T05:23:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/869#issuecomment-3869",
    "user": "https://github.com/sagetrac-cwitty"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[bipartite.patch.gz](https://github.com/sagemath/sage/files/ticket869/bipartite.patch.gz)**



---

archive/issue_events_008352.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-13T05:23:50Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/869",
    "title_is": "[tested by cwitty] graphs: bipartite graph functions",
    "title_was": "graphs: bipartite graph functions",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/869#event-8352"
}
```



---

archive/issue_comments_003870.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nRobert Miller also says this \"definitely\" looks good.",
    "created_at": "2007-10-13T06:25:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/869#issuecomment-3870",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

Robert Miller also says this "definitely" looks good.



---

archive/issue_events_008353.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T06:26:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/869",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/869#event-8353"
}
```
