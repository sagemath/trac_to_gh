# Issue 869: [with patch] [tested by cwitty] graphs: bipartite graph functions

archive/issues_000869.json:
```json
{
    "body": "Here are a few functions wrapped from the bipartite functions in networkx.\n\nAssignee: @williamstein\n\nKeywords: graphs\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/869\n\n",
    "closed_at": "2007-10-13T06:26:42Z",
    "created_at": "2007-10-13T00:45:10Z",
    "labels": [
        "component: combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.7",
    "title": "[with patch] [tested by cwitty] graphs: bipartite graph functions",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/869",
    "user": "https://github.com/jasongrout"
}
```
Here are a few functions wrapped from the bipartite functions in networkx.

Assignee: @williamstein

Keywords: graphs

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/869





---

archive/issue_comments_005628.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [bipartite.patch](tarball://root/attachments/some-uuid/ticket869/bipartite.patch) by cwitty created at 2007-10-13 05:23:50",
    "created_at": "2007-10-13T05:23:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/869#issuecomment-5628",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:1'></a>
Attachment [bipartite.patch](tarball://root/attachments/some-uuid/ticket869/bipartite.patch) by cwitty created at 2007-10-13 05:23:50



---

archive/issue_events_002955.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/869",
    "rename": {
        "from": "[with patch] graphs: bipartite graph functions",
        "to": "[with patch] [tested by cwitty] graphs: bipartite graph functions"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/869#event-2955"
}
```



---

archive/issue_comments_005629.json:
```json
{
    "body": "<a id='comment:2'></a>\nRobert Miller also says this \"definitely\" looks good.",
    "created_at": "2007-10-13T06:25:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/869#issuecomment-5629",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
Robert Miller also says this "definitely" looks good.



---

archive/issue_comments_005630.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-13T06:26:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/869#issuecomment-5630",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed



---

archive/issue_events_002956.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T06:26:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/869",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/869#event-2956"
}
```
