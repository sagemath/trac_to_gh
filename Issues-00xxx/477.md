# Issue 477: break up gnucryptolib.spkg into separate spkgs

archive/issues_000477.json:
```json
{
    "assignees": [],
    "body": "This is done to avoid confusion (i.e. where is gnutls.spkg ? I can't fint it), but also to catch build errors. For details see also http://groups.google.com/group/sage-devel/browse_thread/thread/af701e5f8d428ad2\n\nCheers,\n\nMichael\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/477_\n\n",
    "closed_at": "2007-08-29T03:09:45Z",
    "created_at": "2007-08-22T06:46:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "break up gnucryptolib.spkg into separate spkgs",
    "type": "issue",
    "updated_at": "2007-08-29T03:09:45Z",
    "url": "https://github.com/sagemath/sage/issues/477",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
This is done to avoid confusion (i.e. where is gnutls.spkg ? I can't fint it), but also to catch build errors. For details see also http://groups.google.com/group/sage-devel/browse_thread/thread/af701e5f8d428ad2

Cheers,

Michael

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/477_





---

archive/issue_events_003728.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-22T06:46:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/477",
    "milestone_number": null,
    "milestone_title": "sage-2.8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/477#event-3728"
}
```



---

archive/issue_events_003729.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-22T06:46:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/477",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/477#event-3729"
}
```



---

archive/issue_events_003730.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-22T06:46:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/477",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/477#event-3730"
}
```



---

archive/issue_events_003731.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-08-22T06:46:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/477",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/477#event-3731"
}
```



---

archive/issue_comments_001765.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\ndone for sage-2.8.3.",
    "created_at": "2007-08-29T03:09:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/477",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/477#issuecomment-1765",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'>Comment 1:</a>
done for sage-2.8.3.



---

archive/issue_events_003732.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-08-29T03:09:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/477",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/477#event-3732"
}
```
