# Issue 222: field mutability issue

archive/issues_000222.json:
```json
{
    "assignees": [],
    "body": "\n```\ncwitty: Yuck:\n[10:48pm] cwitty: sage: RS = RealField(sci_not=True)\n[10:48pm] cwitty: sage: R == RS\n[10:48pm] cwitty: sage: RS.scientific_notation(False)\n[10:48pm] cwitty: sage: RR == RS\n[10:48pm] cwitty: (Oops... second line should be \"RR == RS\")\n[10:49pm] cwitty: Second line prints False, fourth line prints True.  Shouldn't fields be immutable?\n```\n\n_Issue created by migration from https://trac.sagemath.org/ticket/222_\n\n",
    "closed_at": "2007-10-21T02:36:57Z",
    "created_at": "2007-01-26T07:09:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "field mutability issue",
    "type": "issue",
    "updated_at": "2007-10-22T00:03:54Z",
    "url": "https://github.com/sagemath/sage/issues/222",
    "user": "https://github.com/williamstein"
}
```

```
cwitty: Yuck:
[10:48pm] cwitty: sage: RS = RealField(sci_not=True)
[10:48pm] cwitty: sage: R == RS
[10:48pm] cwitty: sage: RS.scientific_notation(False)
[10:48pm] cwitty: sage: RR == RS
[10:48pm] cwitty: (Oops... second line should be "RR == RS")
[10:49pm] cwitty: Second line prints False, fourth line prints True.  Shouldn't fields be immutable?
```

_Issue created by migration from https://trac.sagemath.org/ticket/222_





---

archive/issue_events_001506.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-26T07:09:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "milestone_number": null,
    "milestone_title": "sage-2.8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/222#event-1506"
}
```



---

archive/issue_events_001507.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-26T07:09:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/222#event-1507"
}
```



---

archive/issue_events_001508.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-26T07:09:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/222#event-1508"
}
```



---

archive/issue_events_001509.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-26T07:09:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/222#event-1509"
}
```



---

archive/issue_events_001510.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-11T02:06:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/222#event-1510"
}
```



---

archive/issue_comments_000716.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\n\n```\n\n\n```",
    "created_at": "2007-10-21T02:36:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/222#issuecomment-716",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'>Comment 2:</a>

```


```



---

archive/issue_events_001511.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-21T02:36:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/222#event-1511"
}
```



---

archive/issue_comments_000717.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAttachment: **[trac222.patch.gz](https://github.com/sagemath/sage/files/ticket222/trac222.patch.gz)**",
    "created_at": "2007-10-22T00:03:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/222#issuecomment-717",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Attachment: **[trac222.patch.gz](https://github.com/sagemath/sage/files/ticket222/trac222.patch.gz)**



---

archive/issue_events_001512.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-22T00:03:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/222#event-1512"
}
```



---

archive/issue_events_001513.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-22T00:03:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/222",
    "milestone_number": null,
    "milestone_title": "sage-2.8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/222#event-1513"
}
```
