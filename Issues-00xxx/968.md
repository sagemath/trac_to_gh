# Issue 968: graph_isom: memory management, legibility

archive/issues_000968.json:
```json
{
    "assignees": [],
    "body": "These are a few changesets to increase readability and eliminate some pointer arithmetic in memory allocation.\n\nAssignee: **@rlmill**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/968_\n\n",
    "closed_at": "2007-10-23T22:18:11Z",
    "created_at": "2007-10-22T01:22:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "graph_isom: memory management, legibility",
    "type": "issue",
    "updated_at": "2007-10-23T22:18:11Z",
    "url": "https://github.com/sagemath/sage/issues/968",
    "user": "https://github.com/rlmill"
}
```
These are a few changesets to increase readability and eliminate some pointer arithmetic in memory allocation.

Assignee: **@rlmill**

_Issue created by migration from https://trac.sagemath.org/ticket/968_





---

archive/issue_events_008210.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-10-22T01:22:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "milestone_number": null,
    "milestone_title": "sage-2.8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/968#event-8210"
}
```



---

archive/issue_events_008211.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-10-22T01:22:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/968#event-8211"
}
```



---

archive/issue_events_008212.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-10-22T01:22:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/968#event-8212"
}
```



---

archive/issue_events_008213.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-10-22T01:22:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/968#event-8213"
}
```



---

archive/issue_events_008214.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-10-22T01:23:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/968#event-8214"
}
```



---

archive/issue_events_008215.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-10-22T01:23:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/968#event-8215"
}
```



---

archive/issue_comments_004492.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[mem_graph_isom.hg.gz](https://github.com/sagemath/sage/files/ticket968/mem_graph_isom.hg.gz)**",
    "created_at": "2007-10-22T01:23:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/968#issuecomment-4492",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[mem_graph_isom.hg.gz](https://github.com/sagemath/sage/files/ticket968/mem_graph_isom.hg.gz)**



---

archive/issue_comments_004493.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI should mention I found another bug like #939 when I cleaned up the allocation, too.",
    "created_at": "2007-10-22T01:24:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/968#issuecomment-4493",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:2'>Comment 2:</a>
I should mention I found another bug like #939 when I cleaned up the allocation, too.



---

archive/issue_comments_004494.json:
```json
{
    "body": "Attachment: **[7091.patch.gz](https://github.com/sagemath/sage/files/ticket968/7091.patch.gz)**\n\nThis should be patched on top of the bundle...",
    "created_at": "2007-10-22T01:45:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/968#issuecomment-4494",
    "user": "https://github.com/rlmill"
}
```

Attachment: **[7091.patch.gz](https://github.com/sagemath/sage/files/ticket968/7091.patch.gz)**

This should be patched on top of the bundle...



---

archive/issue_comments_004495.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\napplied to 2.8.9.alpha0",
    "created_at": "2007-10-23T22:18:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/968#issuecomment-4495",
    "user": "https://github.com/malb"
}
```

<a id='comment:4'>Comment 4:</a>
applied to 2.8.9.alpha0



---

archive/issue_events_008216.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2007-10-23T22:18:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/968",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/968#event-8216"
}
```
