# Issue 179: recent worksheets -- in notebook display

archive/issues_000179.json:
```json
{
    "assignees": [],
    "body": "In the notebook it would be good to be able to see recent worksheets, \nand also *active* worksheets.\n\nRecent: via cookies.\n\nActive: Last 5 worksheets changed.\n\n\nAssignee: **boothby**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/179_\n\n",
    "closed_at": "2008-03-17T04:21:40Z",
    "created_at": "2006-12-08T23:15:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "recent worksheets -- in notebook display",
    "type": "issue",
    "updated_at": "2008-03-17T04:25:10Z",
    "url": "https://github.com/sagemath/sage/issues/179",
    "user": "https://github.com/williamstein"
}
```
In the notebook it would be good to be able to see recent worksheets, 
and also *active* worksheets.

Recent: via cookies.

Active: Last 5 worksheets changed.


Assignee: **boothby**

_Issue created by migration from https://trac.sagemath.org/ticket/179_





---

archive/issue_events_001188.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-12-08T23:15:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "milestone_number": null,
    "milestone_title": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/179#event-1188"
}
```



---

archive/issue_events_001189.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-12-08T23:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/179#event-1189"
}
```



---

archive/issue_events_001190.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-12-08T23:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/179#event-1190"
}
```



---

archive/issue_events_001191.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-12-08T23:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/179#event-1191"
}
```



---

archive/issue_events_001192.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-10T02:41:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "milestone_number": null,
    "milestone_title": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/179#event-1192"
}
```



---

archive/issue_comments_000604.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nHow about this? Any progress? Are we going to implement this?\n\nCheers,\n\nMichael",
    "created_at": "2007-09-10T02:41:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/179#issuecomment-604",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
How about this? Any progress? Are we going to implement this?

Cheers,

Michael



---

archive/issue_events_001193.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2008-03-17T04:21:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/179#event-1193"
}
```



---

archive/issue_comments_000605.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis has been done for months... worksheets are dislayed last-edited first.",
    "created_at": "2008-03-17T04:21:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/179#issuecomment-605",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:2'>Comment 2:</a>
This has been done for months... worksheets are dislayed last-edited first.



---

archive/issue_events_001194.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-17T04:25:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "milestone_number": null,
    "milestone_title": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/179#event-1194"
}
```



---

archive/issue_events_001195.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-17T04:25:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/179",
    "milestone_number": null,
    "milestone_title": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/179#event-1195"
}
```
