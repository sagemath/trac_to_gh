# Issue 605: GMP build -- get the gmp spkg in shape again (in particular, make sure patches are properly applied)

archive/issues_000605.json:
```json
{
    "assignees": [],
    "body": "\n\nAssignee: **@williamstein, dmharvey**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/605_\n\n",
    "closed_at": "2007-09-07T07:22:40Z",
    "created_at": "2007-09-06T22:17:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "GMP build -- get the gmp spkg in shape again (in particular, make sure patches are properly applied)",
    "type": "issue",
    "updated_at": "2007-09-07T07:22:40Z",
    "url": "https://github.com/sagemath/sage/issues/605",
    "user": "https://github.com/williamstein"
}
```


Assignee: **@williamstein, dmharvey**

_Issue created by migration from https://trac.sagemath.org/ticket/605_





---

archive/issue_events_004886.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-06T22:17:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/605",
    "milestone_number": null,
    "milestone_title": "sage-2.8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/605#event-4886"
}
```



---

archive/issue_events_004887.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-06T22:17:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/605",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/605#event-4887"
}
```



---

archive/issue_events_004888.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-06T22:17:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/605",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/605#event-4888"
}
```



---

archive/issue_events_004889.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-06T22:17:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/605",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/605#event-4889"
}
```



---

archive/issue_comments_002255.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nhopefully this is fixed by the `fastgcd.hg` patch on ticket #424",
    "created_at": "2007-09-07T01:24:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/605#issuecomment-2255",
    "user": "https://github.com/sagetrac-dmharvey"
}
```

<a id='comment:1'>Comment 1:</a>
hopefully this is fixed by the `fastgcd.hg` patch on ticket #424



---

archive/issue_events_004890.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-07T07:22:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/605",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/605#event-4890"
}
```
