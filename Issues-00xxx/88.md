# Issue 88: start writing "How To Make Pyrex FAST"

archive/issues_000088.json:
```json
{
    "body": "I want to start writing a document on the wiki explaining various ways to make Pyrex code even faster than it already is. Each technique should have examples, including the generated C code and some timing experiments. We've started discovering quite a few things, and it would be helpful for new developers to be able to learn these techniques quickly.\n\n\nAssignee: somebody\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/88\n\n",
    "closed_at": "2008-02-27T11:08:55Z",
    "created_at": "2006-09-26T22:31:30Z",
    "labels": [
        "component: basic arithmetic"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "start writing \"How To Make Pyrex FAST\"",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/88",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```
I want to start writing a document on the wiki explaining various ways to make Pyrex code even faster than it already is. Each technique should have examples, including the generated C code and some timing experiments. We've started discovering quite a few things, and it would be helpful for new developers to be able to learn these techniques quickly.


Assignee: somebody

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/88





---

archive/issue_comments_000472.json:
```json
{
    "body": "Changing type from defect to task.",
    "created_at": "2006-09-26T22:31:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/88",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/88#issuecomment-472",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```

Changing type from defect to task.



---

archive/issue_comments_000473.json:
```json
{
    "body": "Attachment [pyrex_guide.tex](tarball://root/attachments/some-uuid/ticket88/pyrex_guide.tex) by @williamstein created at 2006-09-27 17:34:26\n\nHere's a work in progress that is relevant...",
    "created_at": "2006-09-27T17:34:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/88",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/88#issuecomment-473",
    "user": "https://github.com/williamstein"
}
```

Attachment [pyrex_guide.tex](tarball://root/attachments/some-uuid/ticket88/pyrex_guide.tex) by @williamstein created at 2006-09-27 17:34:26

Here's a work in progress that is relevant...



---

archive/issue_comments_000474.json:
```json
{
    "body": "<a id='comment:2'></a>I noticed that \n* Wrapping C++ Code * section in ` pyrex_guide.tex `  is empty. Lenard Lindstrom has a well-written post about writing C++ code in Pyrex that I've gone back to many times for clarity. The url is: http://lists.copyleft.no/pipermail/pyrex/2006-April/001704.html . Could it be included?",
    "created_at": "2007-03-12T05:54:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/88",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/88#issuecomment-474",
    "user": "https://github.com/dfdeshom"
}
```

<a id='comment:2'></a>I noticed that 
* Wrapping C++ Code * section in ` pyrex_guide.tex `  is empty. Lenard Lindstrom has a well-written post about writing C++ code in Pyrex that I've gone back to many times for clarity. The url is: http://lists.copyleft.no/pipermail/pyrex/2006-April/001704.html . Could it be included?



---

archive/issue_comments_000475.json:
```json
{
    "body": "Changing assignee from dmharvey to somebody.",
    "created_at": "2007-09-28T04:05:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/88",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/88#issuecomment-475",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```

Changing assignee from dmharvey to somebody.



---

archive/issue_comments_000476.json:
```json
{
    "body": "<a id='comment:5'></a>Is this still a valid ticket? Certainly the tex is outdated. Also, this should be done over at the Cython project, shouldn't it?",
    "created_at": "2008-02-26T23:58:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/88",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/88#issuecomment-476",
    "user": "https://github.com/malb"
}
```

<a id='comment:5'></a>Is this still a valid ticket? Certainly the tex is outdated. Also, this should be done over at the Cython project, shouldn't it?



---

archive/issue_comments_000477.json:
```json
{
    "body": "<a id='comment:6'></a>Yeah, I think since the Cython project now exists, this ticket should probably be marked invalid.",
    "created_at": "2008-02-27T00:23:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/88",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/88#issuecomment-477",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```

<a id='comment:6'></a>Yeah, I think since the Cython project now exists, this ticket should probably be marked invalid.



---

archive/issue_comments_000478.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2008-02-27T11:08:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/88",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/88#issuecomment-478",
    "user": "https://github.com/malb"
}
```

Resolution: invalid



---

archive/issue_comments_000479.json:
```json
{
    "body": "<a id='comment:7'></a>So lets do it ... one ticket less to worry about :-)",
    "created_at": "2008-02-27T11:08:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/88",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/88#issuecomment-479",
    "user": "https://github.com/malb"
}
```

<a id='comment:7'></a>So lets do it ... one ticket less to worry about :-)



---

archive/issue_events_000183.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-02-27T11:08:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/88",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/88#event-183"
}
```
