# Issue 871: [with patch] [tested by cwitty] graphs: circulant graph creating function

archive/issues_000871.json:
```json
{
    "body": "Patch to graph_generators to create circulant graphs.\n\nAssignee: @williamstein\n\nKeywords: graphs\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/871\n\n",
    "closed_at": "2007-10-13T07:56:29Z",
    "created_at": "2007-10-13T03:07:15Z",
    "labels": [
        "component: combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.7",
    "title": "[with patch] [tested by cwitty] graphs: circulant graph creating function",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/871",
    "user": "https://github.com/jasongrout"
}
```
Patch to graph_generators to create circulant graphs.

Assignee: @williamstein

Keywords: graphs

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/871





---

archive/issue_comments_005636.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [circulant-graph.patch](tarball://root/attachments/some-uuid/ticket871/circulant-graph.patch) by cwitty created at 2007-10-13 05:27:28",
    "created_at": "2007-10-13T05:27:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/871#issuecomment-5636",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:1'></a>
Attachment [circulant-graph.patch](tarball://root/attachments/some-uuid/ticket871/circulant-graph.patch) by cwitty created at 2007-10-13 05:27:28



---

archive/issue_events_002959.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/871",
    "rename": {
        "from": "[with patch] graphs: circulant graph creating function",
        "to": "[with patch] [tested by cwitty] graphs: circulant graph creating function"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/871#event-2959"
}
```



---

archive/issue_events_002960.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T07:56:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/871",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/871#event-2960"
}
```



---

archive/issue_comments_005637.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-13T07:56:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/871#issuecomment-5637",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed
