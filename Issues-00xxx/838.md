# Issue 838: doctest runner should share SAGE initialization using fork()

archive/issues_000838.json:
```json
{
    "assignees": [],
    "body": "On my machine, every file that gets doctested takes 0.8s for initializing SAGE (before it even gets to running the tests).  It would be nice if this overhead could be eliminated somehow; and I think it might be possible, by having a single process do the SAGE initialization, then fork() before doctesting a particular file.  (This sort of architecture might also make it easier to implement #639.)\n\nAssignee: **failure**\n\nCC:  @garyfurnish\n\nReviewer: **David Roe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/838_\n\n",
    "closed_at": "2013-02-08T13:55:52Z",
    "created_at": "2007-10-07T17:03:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "doctest runner should share SAGE initialization using fork()",
    "type": "issue",
    "updated_at": "2013-02-08T13:55:52Z",
    "url": "https://github.com/sagemath/sage/issues/838",
    "user": "https://github.com/sagetrac-cwitty"
}
```
On my machine, every file that gets doctested takes 0.8s for initializing SAGE (before it even gets to running the tests).  It would be nice if this overhead could be eliminated somehow; and I think it might be possible, by having a single process do the SAGE initialization, then fork() before doctesting a particular file.  (This sort of architecture might also make it easier to implement #639.)

Assignee: **failure**

CC:  @garyfurnish

Reviewer: **David Roe**

_Issue created by migration from https://trac.sagemath.org/ticket/838_





---

archive/issue_events_007191.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-07T17:03:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/838",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/838#event-7191"
}
```



---

archive/issue_events_007192.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-07T17:03:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/838",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/838#event-7192"
}
```



---

archive/issue_events_007193.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-07T17:03:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/838",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/838#event-7193"
}
```



---

archive/issue_comments_003947.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis is not a dupe of #4699, but seems closely related since -tp 1 would basically work that way.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-05T04:51:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/838#issuecomment-3947",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
This is not a dupe of #4699, but seems closely related since -tp 1 would basically work that way.

Cheers,

Michael



---

archive/issue_comments_003948.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis will be resolved by #12415.",
    "created_at": "2012-02-06T03:50:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/838#issuecomment-3948",
    "user": "https://github.com/roed314"
}
```

<a id='comment:2'>Comment 2:</a>
This will be resolved by #12415.



---

archive/issue_comments_003949.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2013-02-08T13:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/838#issuecomment-3949",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **David Roe**



---

archive/issue_events_007194.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-08T13:55:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/838",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/838#event-7194"
}
```



---

archive/issue_events_007195.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-08T13:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/838",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/838#event-7195"
}
```
