# Issue 678: Solaris 10: fix scipy compiler selection

archive/issues_000678.json:
```json
{
    "body": "On Solaris scipy looks for \n\n* g77\n* f77\n* f90\n\nbut fails to detect either g95 or gfortran. It should look for those two, but a quick fix is usually to symbolically link gfortran or g95 as g77.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\n**Resolution:** fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/678\n\n",
    "closed_at": "2008-07-29T17:27:38Z",
    "created_at": "2007-09-17T05:22:27Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.6",
    "title": "Solaris 10: fix scipy compiler selection",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/678",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
On Solaris scipy looks for 

* g77
* f77
* f90

but fails to detect either g95 or gfortran. It should look for those two, but a quick fix is usually to symbolically link gfortran or g95 as g77.

Cheers,

Michael

**Assignee:** mabshoff

**Resolution:** fixed

Issue created by migration from https://trac.sagemath.org/ticket/678





---

archive/issue_comments_003699.json:
```json
{
    "body": "**Changing status** from new to assigned.",
    "created_at": "2007-09-17T05:50:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/678#issuecomment-3699",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Changing status** from new to assigned.



---

archive/issue_comments_003700.json:
```json
{
    "body": "<a id='comment:2'></a>\nI know how the scipy chooses fortran compilers (its actually in numpy's distutils/fcompiler).\n\nI could probably help with this if I had access to a solaris box.",
    "created_at": "2007-11-18T09:02:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/678#issuecomment-3700",
    "user": "https://trac.sagemath.org/admin/accounts/users/jkantor"
}
```

<a id='comment:2'></a>
I know how the scipy chooses fortran compilers (its actually in numpy's distutils/fcompiler).

I could probably help with this if I had access to a solaris box.



---

archive/issue_events_001983.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "rename": {
        "from": "Solaris 10: fix scipy compiler selection",
        "to": "[probably fixed] Solaris 10: fix scipy compiler selection"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/678#event-1983"
}
```



---

archive/issue_comments_003701.json:
```json
{
    "body": "<a id='comment:3'></a>\nI believe this has been fixed. I will investigate this.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-04T14:15:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/678#issuecomment-3701",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
I believe this has been fixed. I will investigate this.

Cheers,

Michael



---

archive/issue_events_001984.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-04T14:15:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/678#event-1984"
}
```



---

archive/issue_events_001985.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-03-12T04:38:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/678#event-1985"
}
```



---

archive/issue_events_001986.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-03-12T04:38:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "milestone": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/678#event-1986"
}
```



---

archive/issue_events_001987.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-29T17:27:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "milestone": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/678#event-1987"
}
```



---

archive/issue_events_001988.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-29T17:27:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "milestone": "sage-3.0.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/678#event-1988"
}
```



---

archive/issue_comments_003702.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2008-07-29T17:27:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/678#issuecomment-3702",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Resolution:** fixed



---

archive/issue_events_001989.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "rename": {
        "from": "[probably fixed] Solaris 10: fix scipy compiler selection",
        "to": "Solaris 10: fix scipy compiler selection"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/678#event-1989"
}
```



---

archive/issue_comments_003703.json:
```json
{
    "body": "<a id='comment:5'></a>\nBy introducing sage_fortran this issue has been fixed.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-29T17:27:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/678#issuecomment-3703",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
By introducing sage_fortran this issue has been fixed.

Cheers,

Michael



---

archive/issue_events_001990.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-29T17:27:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/678",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/678#event-1990"
}
```
