# Issue 480: Make Sage work with SELinux on Linux

archive/issues_000480.json:
```json
{
    "body": "Fedora Core 7 has SELinux turned on per default. Sage should support running when SELinux is activated. \n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nCC:  @williamstein @vbraun\n\nKeywords: sd40.5\n\nReviewer: Mike Hansen\n\nResolution: worksforme\n\nIssue created by migration from https://trac.sagemath.org/ticket/480\n\n",
    "closed_at": "2012-08-01T12:23:55Z",
    "created_at": "2007-08-22T21:37:55Z",
    "labels": [
        "component: distribution"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Make Sage work with SELinux on Linux",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/480",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Fedora Core 7 has SELinux turned on per default. Sage should support running when SELinux is activated. 

Cheers,

Michael

Assignee: mabshoff

CC:  @williamstein @vbraun

Keywords: sd40.5

Reviewer: Mike Hansen

Resolution: worksforme

Issue created by migration from https://trac.sagemath.org/ticket/480





---

archive/issue_comments_002628.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-08-22T21:38:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2628",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_events_001222.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-06T20:00:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "milestone": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/480#event-1222"
}
```



---

archive/issue_comments_002629.json:
```json
{
    "body": "<a id='comment:2'></a>From http://www.ittvis.com/services/techtip.asp?ttid=3092:\n\n```\nTo rectify this issue, you can either:\n\n    * Change the default security context for IDL by issuing the command:\n\n      chcon -t texrel_shlib_t /usr/local/rsi/idl_6.1/bin/bin.linux.x86/*.so\n```\n\nCheers,\n\nM ichael",
    "created_at": "2007-12-06T20:00:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2629",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>From http://www.ittvis.com/services/techtip.asp?ttid=3092:

```
To rectify this issue, you can either:

    * Change the default security context for IDL by issuing the command:

      chcon -t texrel_shlib_t /usr/local/rsi/idl_6.1/bin/bin.linux.x86/*.so
```

Cheers,

M ichael



---

archive/issue_comments_002630.json:
```json
{
    "body": "<a id='comment:3'></a>\n```\nOn my CENTOS 5 system, I found the SELinux errors can be avoided by setting SELinux to \"permissive\" rather than \"enforced.\"  \"Disabled\" was not necessary.  Lots of software seems to be affected by the new tighted SELinux settings, including Mathematica.\nThanks for your great work on SAGE!\n```",
    "created_at": "2007-12-24T06:27:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2630",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
```
On my CENTOS 5 system, I found the SELinux errors can be avoided by setting SELinux to "permissive" rather than "enforced."  "Disabled" was not necessary.  Lots of software seems to be affected by the new tighted SELinux settings, including Mathematica.
Thanks for your great work on SAGE!
```



---

archive/issue_comments_002631.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2012-05-28T07:15:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2631",
    "user": "https://github.com/mwhansen"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_002632.json:
```json
{
    "body": "<a id='comment:4'></a>Is this still valid?",
    "created_at": "2012-05-28T07:15:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2632",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:4'></a>Is this still valid?



---

archive/issue_comments_002633.json:
```json
{
    "body": "Changing keywords from \"\" to \"sd40.5\".",
    "created_at": "2012-05-28T07:15:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2633",
    "user": "https://github.com/mwhansen"
}
```

Changing keywords from "" to "sd40.5".



---

archive/issue_comments_002634.json:
```json
{
    "body": "Changing status from new to needs_info.",
    "created_at": "2012-05-28T07:15:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2634",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to needs_info.



---

archive/issue_events_001223.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2012-08-01T06:43:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "milestone": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/480#event-1223"
}
```



---

archive/issue_events_001224.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2012-08-01T06:43:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/480#event-1224"
}
```



---

archive/issue_comments_002635.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2012-08-01T06:43:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2635",
    "user": "https://github.com/mwhansen"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_002636.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Mike Hansen\"",
    "created_at": "2012-08-01T06:43:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2636",
    "user": "https://github.com/mwhansen"
}
```

Changing reviewer from "" to "Mike Hansen"



---

archive/issue_comments_002637.json:
```json
{
    "body": "<a id='comment:6'></a>I think we can closed this as invalid since SELinux has been incorporated to the 2.6 kernel for quite awhile now.",
    "created_at": "2012-08-01T06:43:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2637",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:6'></a>I think we can closed this as invalid since SELinux has been incorporated to the 2.6 kernel for quite awhile now.



---

archive/issue_comments_002638.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-08-01T06:44:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2638",
    "user": "https://github.com/mwhansen"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_001225.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-01T12:23:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/480#event-1225"
}
```



---

archive/issue_comments_002639.json:
```json
{
    "body": "Resolution: worksforme",
    "created_at": "2012-08-01T12:23:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/480#issuecomment-2639",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: worksforme
