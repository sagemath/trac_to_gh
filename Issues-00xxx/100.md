# Issue 100: sage doctest system -- when it tests a pyrex file it still describes it as a .py file

archive/issues_000100.json:
```json
{
    "assignees": [],
    "body": "Example:\n\n```\nFile \"element.py\", line 507:\n    sage: f.mod( (x - y, y - z) )\nException raised:\n    Traceback (most recent call last):\n```\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/100_\n\n",
    "closed_at": "2007-01-13T02:19:31Z",
    "created_at": "2006-09-30T21:47:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage doctest system -- when it tests a pyrex file it still describes it as a .py file",
    "type": "issue",
    "updated_at": "2007-01-13T02:19:31Z",
    "url": "https://github.com/sagemath/sage/issues/100",
    "user": "https://github.com/williamstein"
}
```
Example:

```
File "element.py", line 507:
    sage: f.mod( (x - y, y - z) )
Exception raised:
    Traceback (most recent call last):
```

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/100_





---

archive/issue_events_000668.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-30T21:47:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/100",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/100#event-668"
}
```



---

archive/issue_events_000669.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-30T21:47:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/100",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/100#event-669"
}
```



---

archive/issue_events_000670.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-09-30T21:47:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/100",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/100#event-670"
}
```



---

archive/issue_comments_000348.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nfixed",
    "created_at": "2007-01-13T02:19:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/100#issuecomment-348",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">Comment 1</div>

fixed



---

archive/issue_events_000671.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-13T02:19:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/100",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/100#event-671"
}
```
