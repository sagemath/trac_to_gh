...
```json
{
    "body": "<a id='comment:4'></a>I am not sure if there is anything to fix here. Opinions?",
...
<a id='comment:4'></a>I am not sure if there is anything to fix here. Opinions?
...
```json
{
    "body": "<a id='comment:5'></a>> I am not sure if there is anything to fix here. Opinions?\n\n\nYes, we need to \"make the output of version() much different.\"",
...
<a id='comment:5'></a>> I am not sure if there is anything to fix here. Opinions?
...
```json
{
    "body": "<a id='comment:7'></a>See #2039, to which I've just added a patch. Does this do what you want?\n\n  John",
...
<a id='comment:7'></a>See #2039, to which I've just added a patch. Does this do what you want?
...
<a id='comment:9'></a>In fact: banner_text() which is used for sage-banner uses version to construct the string printed at Sage startup:
...
```json
{
    "body": "<a id='comment:10'></a>I'm not sure I understand.  SAGE_ROOT/local/bin/sage-banner seems to be responsible for the startup message.  Is this file autogenerated from banner_text() somehow?  If I change banner_text(), the startup message is not affected.\n\n(However, I still don't quite see the point of this ticket, and would have no objections to invalidating it.)\n\n  John",
...
<a id='comment:10'></a>I'm not sure I understand.  SAGE_ROOT/local/bin/sage-banner seems to be responsible for the startup message.  Is this file autogenerated from banner_text() somehow?  If I change banner_text(), the startup message is not affected.
...
```json
{
    "body": "<a id='comment:12'></a>William agreed via Google chat, so this is invalidated.\n\nCheers,\n\nMichael",
    "created_at": "2008-10-31T21:08:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/80",
    "type": "issue_comment",
...
<a id='comment:12'></a>William agreed via Google chat, so this is invalidated.
