...
<a id='comment:1'></a>GMP doesn't have mpz_t --> long long functions, so it can only deal with the first 32 bits of the integer on 32 bit platforms. At most I need the 33rd bit, so this patch lowers the bound to use gmp in these cases.
