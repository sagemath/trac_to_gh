# Issue 810: gens_reduced for general ideals

archive/issues_000810.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "It's annoying that number field ideals have a `gens_reduced()` method, but ideals of ZZ do not. This patch fixes this by adding a `gens_reduced()` method to the base ideal class, whose default implementation just calls `gens()`.\n\n\nComponent: **algebraic geometry**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/810_\n\n",
    "closed_at": "2007-10-04T15:15:23Z",
    "created_at": "2007-10-03T16:39:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "gens_reduced for general ideals",
    "type": "issue",
    "updated_at": "2007-10-04T15:15:23Z",
    "url": "https://github.com/sagemath/sage/issues/810",
    "user": "https://github.com/sagetrac-dmharvey"
}
```
It's annoying that number field ideals have a `gens_reduced()` method, but ideals of ZZ do not. This patch fixes this by adding a `gens_reduced()` method to the base ideal class, whose default implementation just calls `gens()`.


Component: **algebraic geometry**

_Issue created by migration from https://trac.sagemath.org/ticket/810_





---

archive/issue_events_007728.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-03T16:39:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/810",
    "milestone_number": null,
    "milestone_title": "sage-2.8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/810#event-7728"
}
```



---

archive/issue_events_007729.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-03T16:39:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/810",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "c: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/810#event-7729"
}
```



---

archive/issue_events_007730.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-03T16:39:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/810",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/810#event-7730"
}
```



---

archive/issue_events_007731.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2007-10-03T16:39:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/810",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/810#event-7731"
}
```



---

archive/issue_events_007732.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-03T16:39:57Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/810",
    "subject": "https://github.com/sagetrac-dmharvey",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/810#event-7732"
}
```



---

archive/issue_comments_003490.json:
```json
{
    "body": "Attachment: **[gens-reduced.hg.gz](https://github.com/sagemath/sage/files/ticket810/gens-reduced.hg.gz)**",
    "created_at": "2007-10-03T16:40:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/810#issuecomment-3490",
    "user": "https://github.com/sagetrac-dmharvey"
}
```

Attachment: **[gens-reduced.hg.gz](https://github.com/sagemath/sage/files/ticket810/gens-reduced.hg.gz)**



---

archive/issue_events_007733.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-04T15:15:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/810",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/810#event-7733"
}
```
