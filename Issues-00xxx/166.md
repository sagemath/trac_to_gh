# Issue 166: incorrect wrapping for modular arithmetic negation

archive/issues_000166.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\n\n```\nsage: -Mod(0, 5)\n 5\n```\n\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/166_\n\n",
    "closed_at": "2006-11-06T07:44:42Z",
    "created_at": "2006-11-01T22:55:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "incorrect wrapping for modular arithmetic negation",
    "type": "issue",
    "updated_at": "2006-11-06T07:44:42Z",
    "url": "https://github.com/sagemath/sage/issues/166",
    "user": "https://github.com/sagetrac-dmharvey"
}
```
<div id="comment:0"></div>


```
sage: -Mod(0, 5)
 5
```


Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/166_





---

archive/issue_events_001164.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2006-11-01T22:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/166",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/166#event-1164"
}
```



---

archive/issue_events_001165.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2006-11-01T22:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/166",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/166#event-1165"
}
```



---

archive/issue_events_001166.json:
```json
{
    "actor": "https://github.com/sagetrac-dmharvey",
    "created_at": "2006-11-01T22:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/166",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/166#event-1166"
}
```



---

archive/issue_comments_000511.json:
```json
{
    "body": "Attachment: **[1788.patch.gz](https://github.com/sagemath/sage/files/ticket166/1788.patch.gz)**\n\npatch that fixes the bug",
    "created_at": "2006-11-06T07:44:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/166#issuecomment-511",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[1788.patch.gz](https://github.com/sagemath/sage/files/ticket166/1788.patch.gz)**

patch that fixes the bug



---

archive/issue_events_001167.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-11-06T07:44:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/166",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/166#event-1167"
}
```



---

archive/issue_comments_000512.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI fixed this.  See attached patch.",
    "created_at": "2006-11-06T07:44:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/166#issuecomment-512",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

I fixed this.  See attached patch.
