# Issue 24: notebook -- prompts mess up implicit printing

archive/issues_000024.json:
```json
{
    "body": "in sage notebook, if there are prompts, e.g., \"sage: 2\", then\nno implicit printing of last result.  It should still.\n\nAssignee: somebody\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/24\n\n",
    "closed_at": "2007-01-13T01:58:11Z",
    "created_at": "2006-09-12T23:23:14Z",
    "labels": [
        "component: basic arithmetic",
        "minor",
        "bug"
    ],
    "title": "notebook -- prompts mess up implicit printing",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24",
    "user": "https://github.com/williamstein"
}
```
in sage notebook, if there are prompts, e.g., "sage: 2", then
no implicit printing of last result.  It should still.

Assignee: somebody

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/24





---

archive/issue_comments_000189.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-01-13T01:58:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24#issuecomment-189",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed



---

archive/issue_events_000044.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-13T01:58:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24#event-44"
}
```



---

archive/issue_comments_000190.json:
```json
{
    "body": "<a id='comment:1'></a>This now works fine.",
    "created_at": "2007-01-13T01:58:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24#issuecomment-190",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>This now works fine.
