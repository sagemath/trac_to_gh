# Issue 848: [with patch, positive review] Using strings for infinity

archive/issues_000848.json:
```json
{
    "body": "sage/gsl/integration.pyx uses the strings 'inf' and '-inf' rather than the infinity element for its bounds. \n\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/848\n\n",
    "closed_at": "2008-04-14T23:56:26Z",
    "created_at": "2007-10-11T09:02:45Z",
    "labels": [
        "component: numerical",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[with patch, positive review] Using strings for infinity",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/848",
    "user": "https://github.com/robertwb"
}
```
sage/gsl/integration.pyx uses the strings 'inf' and '-inf' rather than the infinity element for its bounds. 


Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/848





---

archive/issue_events_002891.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-10-11T10:08:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/848",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/848#event-2891"
}
```



---

archive/issue_comments_005504.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [848-gsl_infinity.patch](tarball://root/attachments/some-uuid/ticket848/848-gsl_infinity.patch) by @aghitza created at 2008-04-13 03:38:44\n\nSee the attached patch.",
    "created_at": "2008-04-13T03:38:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/848#issuecomment-5504",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:2'></a>
Attachment [848-gsl_infinity.patch](tarball://root/attachments/some-uuid/ticket848/848-gsl_infinity.patch) by @aghitza created at 2008-04-13 03:38:44

See the attached patch.



---

archive/issue_events_002892.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/848",
    "rename": {
        "from": "Using strings for infinity",
        "to": "[with patch, needs review] Using strings for infinity"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/848#event-2892"
}
```



---

archive/issue_comments_005505.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good to me.  All tests in gsl/ pass.",
    "created_at": "2008-04-14T22:52:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/848#issuecomment-5505",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
Looks good to me.  All tests in gsl/ pass.



---

archive/issue_events_002893.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/848",
    "rename": {
        "from": "[with patch, needs review] Using strings for infinity",
        "to": "[with patch, positive review] Using strings for infinity"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/848#event-2893"
}
```



---

archive/issue_comments_005506.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-14T23:56:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/848#issuecomment-5506",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_005507.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.alpha5",
    "created_at": "2008-04-14T23:56:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/848#issuecomment-5507",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.alpha5



---

archive/issue_events_002894.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-14T23:56:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/848",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/848#event-2894"
}
```
