# Issue 185: Firefox + Xorg (Linux) take way too much cputime while waiting for results from the notebook server

archive/issues_000185.json:
```json
{
    "body": "On my P4 so as my P3 system whenever I undertake a long calculation using the notebook the polling for results takes way too much cputime. I guess it's caused by a very high polling frequency. top output follows:\n\n```\n XXXX martin    25   0  125m  36m 9.8m R   80  2.4   1:13.44 python\n XXXX root      16   0  285m 176m 2860 R   36 11.6 204:18.81 Xorg\n XXXX martin    15   0 95860  43m  19m S   30  2.8   3:50.28 firefox-bin\n```\nHow to reproduce: Calculate something difficult and monitor top.\n\n**Assignee:** boothby\n\nIssue created by migration from https://trac.sagemath.org/ticket/185\n\n",
    "closed_at": "2008-03-17T04:24:47Z",
    "created_at": "2006-12-18T00:00:39Z",
    "labels": [
        "component: notebook",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.4",
    "title": "Firefox + Xorg (Linux) take way too much cputime while waiting for results from the notebook server",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/185",
    "user": "https://github.com/malb"
}
```
On my P4 so as my P3 system whenever I undertake a long calculation using the notebook the polling for results takes way too much cputime. I guess it's caused by a very high polling frequency. top output follows:

```
 XXXX martin    25   0  125m  36m 9.8m R   80  2.4   1:13.44 python
 XXXX root      16   0  285m 176m 2860 R   36 11.6 204:18.81 Xorg
 XXXX martin    15   0 95860  43m  19m S   30  2.8   3:50.28 firefox-bin
```
How to reproduce: Calculate something difficult and monitor top.

**Assignee:** boothby

Issue created by migration from https://trac.sagemath.org/ticket/185





---

archive/issue_events_000606.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-19T09:41:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "label": "bug",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-606"
}
```



---

archive/issue_events_000607.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-19T09:41:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "label": "enhancement",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-607"
}
```



---

archive/issue_events_000608.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-19T09:57:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "milestone": "sage-1.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-608"
}
```



---

archive/issue_comments_000616.json:
```json
{
    "body": "<a id='comment:3'></a>\nPlease investigate the issue. The orignial milestone was 1.8 so it has been a while.\n\nCheers,\n\nMichael",
    "created_at": "2007-09-10T02:37:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/185#issuecomment-616",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Please investigate the issue. The orignial milestone was 1.8 so it has been a while.

Cheers,

Michael



---

archive/issue_events_000609.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-09-10T02:37:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "milestone": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-609"
}
```



---

archive/issue_comments_000617.json:
```json
{
    "body": "<a id='comment:4'></a>\nThe issues has not been resolved, an truncated exponential backoff was discussed here:\n\nhttp://groups.google.com/group/sage-devel/browse_thread/thread/f8f76d438163e546/110cf0c908689635?",
    "created_at": "2008-01-17T12:50:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/185#issuecomment-617",
    "user": "https://github.com/malb"
}
```

<a id='comment:4'></a>
The issues has not been resolved, an truncated exponential backoff was discussed here:

http://groups.google.com/group/sage-devel/browse_thread/thread/f8f76d438163e546/110cf0c908689635?



---

archive/issue_events_000610.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-03-16T21:22:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-610"
}
```



---

archive/issue_comments_000618.json:
```json
{
    "body": "<a id='comment:5'></a>\nI've implemented something like exponential falloff.  Polling slows down gradually; after about 30 seconds, poll delay is 5 seconds.",
    "created_at": "2008-03-16T21:22:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/185#issuecomment-618",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

<a id='comment:5'></a>
I've implemented something like exponential falloff.  Polling slows down gradually; after about 30 seconds, poll delay is 5 seconds.



---

archive/attachments_000044.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "185-smartpoll.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket185/185-smartpoll.patch",
    "created_at": "2008-03-16T21:23:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```



---

archive/issue_comments_000619.json:
```json
{
    "body": "",
    "created_at": "2008-03-16T21:23:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/185#issuecomment-619",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```





---

archive/issue_events_000611.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-03-16T21:24:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "milestone": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-611"
}
```



---

archive/issue_events_000612.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-03-16T21:24:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "milestone": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-612"
}
```



---

archive/issue_comments_000620.json:
```json
{
    "body": "<a id='comment:7'></a>\nIt works fine for me; the code makes sense.",
    "created_at": "2008-03-17T04:20:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/185#issuecomment-620",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:7'></a>
It works fine for me; the code makes sense.



---

archive/issue_events_000613.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-17T04:20:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-613"
}
```



---

archive/issue_events_000614.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-17T04:20:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-614"
}
```



---

archive/issue_events_000615.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-17T04:24:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-615"
}
```



---

archive/issue_events_000616.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-17T04:24:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/185#event-616"
}
```



---

archive/issue_comments_000621.json:
```json
{
    "body": "<a id='comment:8'></a>\nMerged in Sage 2.10.4.final",
    "created_at": "2008-03-17T04:24:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/185#issuecomment-621",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:8'></a>
Merged in Sage 2.10.4.final
