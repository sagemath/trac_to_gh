# Issue 221: generic laurent series over arbitrary ring in Cython

archive/issues_000221.json:
```json
{
    "assignees": [],
    "body": "\n\nIssue created by migration from https://trac.sagemath.org/ticket/221\n\n",
    "closed_at": "2008-11-14T08:44:11Z",
    "created_at": "2007-01-25T19:02:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "generic laurent series over arbitrary ring in Cython",
    "type": "issue",
    "updated_at": "2008-11-14T08:44:11Z",
    "url": "https://github.com/sagemath/sage/issues/221",
    "user": "https://github.com/williamstein"
}
```


Issue created by migration from https://trac.sagemath.org/ticket/221





---

archive/issue_events_001365.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T19:02:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "milestone_number": null,
    "milestone_title": "sage-2.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1365"
}
```



---

archive/issue_events_001366.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T19:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "08517b",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1366"
}
```



---

archive/issue_events_001367.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T19:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1367"
}
```



---

archive/issue_events_001368.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-10T02:58:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1368"
}
```



---

archive/issue_events_001369.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-21T13:53:30Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "rename": {
        "from": "SageX: generic laurent series over arbitrary ring",
        "to": "generic laurent series over arbitrary ring in Cython"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1369"
}
```



---

archive/issue_events_001370.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-11-14T08:44:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "milestone_number": null,
    "milestone_title": "sage-2.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1370"
}
```



---

archive/issue_events_001371.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-11-14T08:44:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1371"
}
```



---

archive/issue_comments_000714.json:
```json
{
    "body": "<a id='comment:3'>**Comment 3:**</a>\nI think this has already been done as evidenced by sage/rings/laurent_series*.pyx.  This was done around Sage 2.4.2 -- changeset 4157.",
    "created_at": "2008-11-14T08:44:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/221#issuecomment-714",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'>**Comment 3:**</a>
I think this has already been done as evidenced by sage/rings/laurent_series*.pyx.  This was done around Sage 2.4.2 -- changeset 4157.
