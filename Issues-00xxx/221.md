# Issue 221: generic laurent series over arbitrary ring in Cython

archive/issues_000221.json:
```json
{
    "assignees": [],
    "body": "\n\n_Issue created by migration from https://trac.sagemath.org/ticket/221_\n\n",
    "closed_at": "2008-11-14T08:44:11Z",
    "created_at": "2007-01-25T19:02:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "generic laurent series over arbitrary ring in Cython",
    "type": "issue",
    "updated_at": "2008-11-14T08:44:11Z",
    "url": "https://github.com/sagemath/sage/issues/221",
    "user": "https://github.com/williamstein"
}
```


_Issue created by migration from https://trac.sagemath.org/ticket/221_





---

archive/issue_events_001498.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T19:02:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "milestone_number": null,
    "milestone_title": "sage-2.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1498"
}
```



---

archive/issue_events_001499.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T19:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1499"
}
```



---

archive/issue_events_001500.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T19:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1500"
}
```



---

archive/issue_events_001501.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-01-25T19:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1501"
}
```



---

archive/issue_events_001502.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-10T02:58:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1502"
}
```



---

archive/issue_events_001503.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-10-21T13:53:30Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "title_is": "generic laurent series over arbitrary ring in Cython",
    "title_was": "SageX: generic laurent series over arbitrary ring",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1503"
}
```



---

archive/issue_events_001504.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-11-14T08:44:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "milestone_number": null,
    "milestone_title": "sage-2.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1504"
}
```



---

archive/issue_events_001505.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-11-14T08:44:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/221#event-1505"
}
```



---

archive/issue_comments_000715.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI think this has already been done as evidenced by sage/rings/laurent_series*.pyx.  This was done around Sage 2.4.2 -- changeset 4157.",
    "created_at": "2008-11-14T08:44:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/221#issuecomment-715",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'>Comment 3:</a>
I think this has already been done as evidenced by sage/rings/laurent_series*.pyx.  This was done around Sage 2.4.2 -- changeset 4157.
