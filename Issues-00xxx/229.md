# Issue 229: make it so all objects pickled now can be loaded in all future versions of SAGE

archive/issues_000229.json:
```json
{
    "assignees": [],
    "body": "PLAN\n\n\nMake it so pickled objects can always be reloaded. To do this, ever single class *must* have a couple example pickled objects, and the doctest has to simply load them and make sure it works.   Make this a requirement for all SAGE classes (!).\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/229\n\n",
    "closed_at": "2008-08-02T03:04:38Z",
    "created_at": "2007-01-29T19:03:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make it so all objects pickled now can be loaded in all future versions of SAGE",
    "type": "issue",
    "updated_at": "2008-08-02T03:04:38Z",
    "url": "https://github.com/sagemath/sage/issues/229",
    "user": "https://github.com/williamstein"
}
```
PLAN


Make it so pickled objects can always be reloaded. To do this, ever single class *must* have a couple example pickled objects, and the doctest has to simply load them and make sure it works.   Make this a requirement for all SAGE classes (!).

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/229





---

archive/issue_events_000772.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-09-10T02:56:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/229",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/229#event-772"
}
```



---

archive/issue_events_000773.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-02T03:04:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/229",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/229#event-773"
}
```



---

archive/issue_comments_000730.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis ticket has been resolved by the \"pickle jar\".\n\nCheers,\n\nMichael",
    "created_at": "2008-08-02T03:04:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/229#issuecomment-730",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'></a>
This ticket has been resolved by the "pickle jar".

Cheers,

Michael



---

archive/issue_events_000774.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-02T03:04:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/229",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/229#event-774"
}
```
