# Issue 863: trac -- make it not crash, or when it does, make it autorestart!!

archive/issues_000863.json:
```json
{
    "body": "\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/863\n\n",
    "closed_at": "2007-10-19T21:00:20Z",
    "created_at": "2007-10-12T19:04:35Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.8",
    "title": "trac -- make it not crash, or when it does, make it autorestart!!",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/863",
    "user": "https://github.com/williamstein"
}
```


**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/863





---

archive/issue_events_004169.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-12T19:04:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/863",
    "label": "component: algebraic geometry",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/863#event-4169"
}
```



---

archive/issue_events_004170.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-12T19:04:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/863",
    "label": "component: packages",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/863#event-4170"
}
```



---

archive/issue_events_004171.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-19T21:00:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/863",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/863#event-4171"
}
```



---

archive/issue_events_004172.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-19T21:00:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/863",
    "milestone": "sage-2.8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/863#event-4172"
}
```



---

archive/issue_events_004173.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-19T21:00:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/863",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/863#event-4173"
}
```
