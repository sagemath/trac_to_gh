...
<a id='comment:3'></a>I think at the least that the Py_None *must* be NULL instead.  I have
crashes because of exactly this, and found that changing the Py_None
to NULL stopped those crashes.  But presumably one should do 4 and 5
as well so that the weakref stuff actually works (rather than just
not crashing).
...
<a id='comment:4'></a>From Greg Ewing:
...
```json
{
    "body": "<a id='comment:5'></a>I applied the patch from Peter Johnson, which implements weakref support.",
...
<a id='comment:5'></a>I applied the patch from Peter Johnson, which implements weakref support.
