# Issue 158: missing sage-make_relative's

archive/issues_000158.json:
```json
{
    "assignees": [],
    "body": "sage-make_relative needs to be called after any \"sage -i\".  E.g., installing pyrex\nthen moving sage results in a broken sage.\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/158_\n\n",
    "closed_at": "2006-11-06T07:52:28Z",
    "created_at": "2006-10-27T21:02:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "missing sage-make_relative's",
    "type": "issue",
    "updated_at": "2006-11-06T07:52:28Z",
    "url": "https://github.com/sagemath/sage/issues/158",
    "user": "https://github.com/williamstein"
}
```
sage-make_relative needs to be called after any "sage -i".  E.g., installing pyrex
then moving sage results in a broken sage.

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/158_





---

archive/issue_events_001017.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-10-27T21:02:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/158",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/158#event-1017"
}
```



---

archive/issue_events_001018.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-10-27T21:02:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/158",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/158#event-1018"
}
```



---

archive/issue_events_001019.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-10-27T21:02:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/158",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/158#event-1019"
}
```



---

archive/issue_comments_000521.json:
```json
{
    "body": "Attachment: **[94.patch.gz](https://github.com/sagemath/sage/files/ticket158/94.patch.gz)**",
    "created_at": "2006-11-06T07:52:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/158#issuecomment-521",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[94.patch.gz](https://github.com/sagemath/sage/files/ticket158/94.patch.gz)**



---

archive/issue_events_001020.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2006-11-06T07:52:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/158",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/158#event-1020"
}
```



---

archive/issue_comments_000522.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nfixed -- was an easy addition to sage-spkg",
    "created_at": "2006-11-06T07:52:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/158#issuecomment-522",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'>Comment 1:</a>
fixed -- was an easy addition to sage-spkg
