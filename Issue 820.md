# Issue 820: more sage -coverage improvements

archive/issues_000820.json:
```json
{
    "body": "Assignee: @craigcitro\n\nKeywords: coverage\n\nThere are situations where functions aren't easy to doctest, and shouldn't have this counted against the file's coverage. Think about the right way to implement this.\n\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/820\n\n",
    "created_at": "2007-10-04T03:27:23Z",
    "labels": [
        "component: documentation",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "more sage -coverage improvements",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/820",
    "user": "https://github.com/craigcitro"
}
```
Assignee: @craigcitro

Keywords: coverage

There are situations where functions aren't easy to doctest, and shouldn't have this counted against the file's coverage. Think about the right way to implement this.



Issue created by migration from https://trac.sagemath.org/ticket/820





---

archive/issue_comments_005071.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-10-04T03:27:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/820#issuecomment-5071",
    "user": "https://github.com/craigcitro"
}
```

Changing status from new to assigned.



---

archive/issue_comments_005072.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2008-03-29T02:33:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/820#issuecomment-5072",
    "user": "https://github.com/mwhansen"
}
```

Resolution: invalid



---

archive/issue_events_000931.json:
```json
{
    "actor": "@mwhansen",
    "created_at": "2008-03-29T02:33:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/820",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/820#event-931"
}
```



---

archive/issue_comments_005073.json:
```json
{
    "body": "Too vague for a ticket.  If you have a more specific problem to address, then open a new ticket.",
    "created_at": "2008-03-29T02:33:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/820#issuecomment-5073",
    "user": "https://github.com/mwhansen"
}
```

Too vague for a ticket.  If you have a more specific problem to address, then open a new ticket.
