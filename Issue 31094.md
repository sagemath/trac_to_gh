# Issue 31094: Make .variety() work for ideals of univariate polynomial rings

archive/issues_031094.json:
```json
{
    "body": "CC:  @slel\n\nKeywords: ideal, variety, polynomial\n\nSage 9.2:\n\n```\nsage: PR = PolynomialRing(QQ, ['x'])\nsage: I = ideal(PR('x^2 - 4'))\nsage: print(I.variety())\n\nTraceback (most recent call last):\n  File \"crash.sage.py\", line 8, in <module>\n    print(I.variety())\n  File \"sage/structure/element.pyx\", line 493, in sage.structure.element.Element.__getattr__ (build/cythonized/sage/structure/element.c:4703)\n  File \"sage/structure/element.pyx\", line 506, in sage.structure.element.Element.getattr_from_category (build/cythonized/sage/structure/element.c:4815)\n  File \"sage/cpython/getattr.pyx\", line 372, in sage.cpython.getattr.getattr_from_other_class (build/cythonized/sage/cpython/getattr.c:2619)\nAttributeError: 'Ideal_1poly_field' object has no attribute 'variety'\n\nAnd I had another, funkier error attempting the same thing from the notebook.\n\n\n/opt/SageMath/9.2/local/lib/python3.8/site-packages/sage/structure/element.pyx in sage.structure.element.Element.__getattr__ (build/cythonized/sage/structure/element.c:4703)()\n    491             AttributeError: 'LeftZeroSemigroup_with_category.element_class' object has no attribute 'blah_blah'\n    492         \"\"\"\n--> 493         return self.getattr_from_category(name)\n    494 \n    495     cdef getattr_from_category(self, name):\n\n/opt/SageMath/9.2/local/lib/python3.8/site-packages/sage/structure/element.pyx in sage.structure.element.Element.getattr_from_category (build/cythonized/sage/structure/element.c:4815)()\n    504         else:\n    505             cls = P._abstract_element_class\n--> 506         return getattr_from_other_class(self, cls, name)\n    507 \n    508     def __dir__(self):\n```\n\nIssue created by migration from https://trac.sagemath.org/ticket/31331\n\n",
    "created_at": "2021-02-03T18:55:31Z",
    "labels": [
        "component: commutative algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Make .variety() work for ideals of univariate polynomial rings",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/31094",
    "user": "https://trac.sagemath.org/admin/accounts/users/monniaux"
}
```
CC:  @slel

Keywords: ideal, variety, polynomial

Sage 9.2:

```
sage: PR = PolynomialRing(QQ, ['x'])
sage: I = ideal(PR('x^2 - 4'))
sage: print(I.variety())

Traceback (most recent call last):
  File "crash.sage.py", line 8, in <module>
    print(I.variety())
  File "sage/structure/element.pyx", line 493, in sage.structure.element.Element.__getattr__ (build/cythonized/sage/structure/element.c:4703)
  File "sage/structure/element.pyx", line 506, in sage.structure.element.Element.getattr_from_category (build/cythonized/sage/structure/element.c:4815)
  File "sage/cpython/getattr.pyx", line 372, in sage.cpython.getattr.getattr_from_other_class (build/cythonized/sage/cpython/getattr.c:2619)
AttributeError: 'Ideal_1poly_field' object has no attribute 'variety'

And I had another, funkier error attempting the same thing from the notebook.


/opt/SageMath/9.2/local/lib/python3.8/site-packages/sage/structure/element.pyx in sage.structure.element.Element.__getattr__ (build/cythonized/sage/structure/element.c:4703)()
    491             AttributeError: 'LeftZeroSemigroup_with_category.element_class' object has no attribute 'blah_blah'
    492         """
--> 493         return self.getattr_from_category(name)
    494 
    495     cdef getattr_from_category(self, name):

/opt/SageMath/9.2/local/lib/python3.8/site-packages/sage/structure/element.pyx in sage.structure.element.Element.getattr_from_category (build/cythonized/sage/structure/element.c:4815)()
    504         else:
    505             cls = P._abstract_element_class
--> 506         return getattr_from_other_class(self, cls, name)
    507 
    508     def __dir__(self):
```

Issue created by migration from https://trac.sagemath.org/ticket/31331





---

archive/issue_comments_443648.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to enhancement.",
    "created_at": "2021-02-05T20:10:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31094#issuecomment-443648",
    "user": "https://github.com/slel"
}
```

Changing type from PLEASE CHANGE to enhancement.



---

archive/issue_comments_443649.json:
```json
{
    "body": "Changing keywords from \"\" to \"ideal, variety, polynomial\".",
    "created_at": "2021-02-05T20:10:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31094#issuecomment-443649",
    "user": "https://github.com/slel"
}
```

Changing keywords from "" to "ideal, variety, polynomial".



---

archive/issue_comments_443650.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to commutative algebra.",
    "created_at": "2021-02-05T20:10:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31094#issuecomment-443650",
    "user": "https://github.com/slel"
}
```

Changing component from PLEASE CHANGE to commutative algebra.



---

archive/issue_events_082053.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31094#event-82053"
}
```



---

archive/issue_comments_443651.json:
```json
{
    "body": "Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/31094#issuecomment-443651",
    "user": "https://github.com/mkoeppe"
}
```

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_082054.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31094#event-82054"
}
```



---

archive/issue_events_082055.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31094#event-82055"
}
```



---

archive/issue_events_082056.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31094#event-82056"
}
```



---

archive/issue_events_082057.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31094#event-82057"
}
```



---

archive/issue_events_082058.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31094#event-82058"
}
```



---

archive/issue_events_082059.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31094#event-82059"
}
```



---

archive/issue_events_082060.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31094#event-82060"
}
```



---

archive/issue_events_082061.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/31094",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/31094#event-82061"
}
```
