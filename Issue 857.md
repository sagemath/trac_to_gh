# Issue 857: update info about cython in license() output

archive/issues_000857.json:
```json
{
    "body": "Assignee: tba\n\nIn 2.8.6, license() still talks about pyrex/sagex, not cython\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/857\n\n",
    "created_at": "2007-10-12T03:12:23Z",
    "labels": [
        "documentation",
        "major",
        "enhancement"
    ],
    "title": "update info about cython in license() output",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/857",
    "user": "dmharvey"
}
```
Assignee: tba

In 2.8.6, license() still talks about pyrex/sagex, not cython


Issue created by migration from https://trac.sagemath.org/ticket/857





---

archive/issue_comments_005306.json:
```json
{
    "body": "This ought to be easily done during Bug Day 4.\n\nCheers,\n\nMichael",
    "created_at": "2007-10-19T19:26:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/857",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/857#issuecomment-5306",
    "user": "mabshoff"
}
```

This ought to be easily done during Bug Day 4.

Cheers,

Michael



---

archive/issue_comments_005307.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-10-19T19:26:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/857",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/857#issuecomment-5307",
    "user": "mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_005308.json:
```json
{
    "body": "Changing assignee from tba to mabshoff.",
    "created_at": "2007-10-19T19:26:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/857",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/857#issuecomment-5308",
    "user": "mabshoff"
}
```

Changing assignee from tba to mabshoff.



---

archive/issue_comments_005309.json:
```json
{
    "body": "Attachment",
    "created_at": "2007-12-22T11:37:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/857",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/857#issuecomment-5309",
    "user": "mabshoff"
}
```

Attachment



---

archive/issue_comments_005310.json:
```json
{
    "body": "This is COPYING.txt from 2.9.1.alpha3, where I made the following changes:\n\n* rename pyrex/sagex to cython\n* add flint\n* update singular's license section to the lastest version shipped with 3-0-4\n\nCheers,\n\nMichael",
    "created_at": "2007-12-22T11:39:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/857",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/857#issuecomment-5310",
    "user": "mabshoff"
}
```

This is COPYING.txt from 2.9.1.alpha3, where I made the following changes:

* rename pyrex/sagex to cython
* add flint
* update singular's license section to the lastest version shipped with 3-0-4

Cheers,

Michael



---

archive/issue_comments_005311.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-12-22T17:38:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/857",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/857#issuecomment-5311",
    "user": "rlm"
}
```

Resolution: fixed



---

archive/issue_comments_005312.json:
```json
{
    "body": "I updated the sagex/pyrex description part also, and moved it to its place. I added a description for flint.",
    "created_at": "2007-12-22T17:38:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/857",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/857#issuecomment-5312",
    "user": "rlm"
}
```

I updated the sagex/pyrex description part also, and moved it to its place. I added a description for flint.
