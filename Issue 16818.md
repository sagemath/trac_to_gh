...
```
sage: RCP = RiggedConfigurations(['A',7,2], [This is the Trac macro *3,1* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#3,1-macro))
sage: ascii_art(RCP.module_generators[0].f_string([3,4]))
(/)  (/)  1[ ]1  -1[ ]-1
sage: RCP = RiggedConfigurations(['C',4,1], [This is the Trac macro *3,1* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#3,1-macro))
sage: ascii_art(RCP.module_generators[0].f_string([3,4]))
(/)  (/)  0[ ]0  -1[ ]-1
```
