# Issue 7469: SageNB - Add new tests regarding publishing and editing published worksheets.

archive/issues_007469.json:
```json
{
    "body": "Assignee: boothby\n\nCC:  @qed777\n\nMore tests is always good, and will be useful in the future.\n\nIssue created by migration from https://trac.sagemath.org/ticket/7469\n\n",
    "created_at": "2009-11-15T07:32:36Z",
    "labels": [
        "notebook",
        "major",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "SageNB - Add new tests regarding publishing and editing published worksheets.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7469",
    "user": "@TimDumol"
}
```
Assignee: boothby

CC:  @qed777

More tests is always good, and will be useful in the future.

Issue created by migration from https://trac.sagemath.org/ticket/7469





---

archive/issue_comments_062913.json:
```json
{
    "body": "Attachment [trac_7469-new-publishing-tests.patch](tarball://root/attachments/some-uuid/ticket7469/trac_7469-new-publishing-tests.patch) by @TimDumol created at 2009-11-15 07:33:38\n\nThis adds an additional test. Depends on #7468.",
    "created_at": "2009-11-15T07:33:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62913",
    "user": "@TimDumol"
}
```

Attachment [trac_7469-new-publishing-tests.patch](tarball://root/attachments/some-uuid/ticket7469/trac_7469-new-publishing-tests.patch) by @TimDumol created at 2009-11-15 07:33:38

This adds an additional test. Depends on #7468.



---

archive/issue_comments_062914.json:
```json
{
    "body": "Here's one patch -- it depends on #7468. Please feel free to add more.",
    "created_at": "2009-11-15T07:36:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62914",
    "user": "@TimDumol"
}
```

Here's one patch -- it depends on #7468. Please feel free to add more.



---

archive/issue_comments_062915.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-07-16T17:25:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62915",
    "user": "cwitty"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_062916.json:
```json
{
    "body": "timdumol, I'm assuming you forgot to set this as needing review.  If you feel it's not ready for review, you should set it to needs_work.",
    "created_at": "2010-07-16T17:25:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62916",
    "user": "cwitty"
}
```

timdumol, I'm assuming you forgot to set this as needing review.  If you feel it's not ready for review, you should set it to needs_work.



---

archive/issue_comments_062917.json:
```json
{
    "body": "Since #8961 is now merged, should we close this ticket?",
    "created_at": "2010-07-16T21:09:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62917",
    "user": "@qed777"
}
```

Since #8961 is now merged, should we close this ticket?



---

archive/issue_comments_062918.json:
```json
{
    "body": "Please fill in your real name as Author.",
    "created_at": "2012-07-27T20:42:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62918",
    "user": "@jdemeyer"
}
```

Please fill in your real name as Author.



---

archive/issue_comments_062919.json:
```json
{
    "body": "This patch has undergone major bitrot, and will be completely useless once the new UI branch is merged in. Can this please be closed?",
    "created_at": "2012-07-28T04:09:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62919",
    "user": "@TimDumol"
}
```

This patch has undergone major bitrot, and will be completely useless once the new UI branch is merged in. Can this please be closed?



---

archive/issue_comments_062920.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2013-06-18T20:15:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62920",
    "user": "@kcrisman"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_062921.json:
```json
{
    "body": "totally obsolete",
    "created_at": "2019-12-19T11:04:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62921",
    "user": "@fchapoton"
}
```

totally obsolete



---

archive/issue_comments_062922.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2019-12-19T11:04:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7469#issuecomment-62922",
    "user": "@fchapoton"
}
```

Resolution: invalid
