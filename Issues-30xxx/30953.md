# Issue 30953: Implicit line continuation in callable symbolic expression

archive/issues_030716.json:
```json
{
    "assignees": [],
    "body": "Reported in [#30928 comment:19](https://github.com/sagemath/sage/issues/30928#comment:19)\n\n```python\nsage: f(x) = (x +  \n....:         1)                                                                                    \n  File \"<ipython-input-10-ae23cb631161>\", line 1\n    __tmp__=var(\"x\"); f = symbolic_expression((x + ).function(x)\n                                                   ^\nSyntaxError: invalid syntax\n```\n\n`sage.repl.preparse.preparse_calculus` needs to be modified to accommodate implicit line continuations such as inside parentheses, brackets, and braces.\n\nCC:  @strogdon @egourgoulhon @kliem @slel\n\nKeywords: **line continuation symbolic expression preparser parentheses**\n\nWork Issues: **close #4545?**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30953_\n\n",
    "closed_at": "2021-10-04T23:44:13Z",
    "created_at": "2020-11-23T18:12:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implicit line continuation in callable symbolic expression",
    "type": "issue",
    "updated_at": "2021-10-04T23:44:13Z",
    "url": "https://github.com/sagemath/sage/issues/30953",
    "user": "https://github.com/jcamp0x2a"
}
```
Reported in [#30928 comment:19](https://github.com/sagemath/sage/issues/30928#comment:19)

```python
sage: f(x) = (x +  
....:         1)                                                                                    
  File "<ipython-input-10-ae23cb631161>", line 1
    __tmp__=var("x"); f = symbolic_expression((x + ).function(x)
                                                   ^
SyntaxError: invalid syntax
```

`sage.repl.preparse.preparse_calculus` needs to be modified to accommodate implicit line continuations such as inside parentheses, brackets, and braces.

CC:  @strogdon @egourgoulhon @kliem @slel

Keywords: **line continuation symbolic expression preparser parentheses**

Work Issues: **close #4545?**

_Issue created by migration from https://trac.sagemath.org/ticket/30953_





---

archive/issue_events_408642.json:
```json
{
    "actor": "https://github.com/jcamp0x2a",
    "created_at": "2020-11-23T18:12:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408642"
}
```



---

archive/issue_events_408643.json:
```json
{
    "actor": "https://github.com/jcamp0x2a",
    "created_at": "2020-11-23T18:12:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408643"
}
```



---

archive/issue_events_408644.json:
```json
{
    "actor": "https://github.com/jcamp0x2a",
    "created_at": "2020-11-23T18:12:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408644"
}
```



---

archive/issue_events_408645.json:
```json
{
    "actor": "https://github.com/jcamp0x2a",
    "created_at": "2020-11-23T18:12:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408645"
}
```



---

archive/issue_comments_499001.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI've just been struck by a silent failure (!) version of this bug. A minimal version is\n\n```\nsage: f(x) = 1/2*(1 + x \n....:             + x^2)                                                                            \nsage: f                                                                                             \nx |--> x^2 + 1/2*x + 1/2\n```\nThis issue is present in Sage 9.1, 9.2 and 9.3.beta3.",
    "created_at": "2020-12-14T20:20:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499001",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:3'>Comment 3:</a>
I've just been struck by a silent failure (!) version of this bug. A minimal version is

```
sage: f(x) = 1/2*(1 + x 
....:             + x^2)                                                                            
sage: f                                                                                             
x |--> x^2 + 1/2*x + 1/2
```
This issue is present in Sage 9.1, 9.2 and 9.3.beta3.



---

archive/issue_comments_499002.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBecause this is a silent failure in an elementary operation.",
    "created_at": "2020-12-14T20:22:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499002",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:4'>Comment 4:</a>
Because this is a silent failure in an elementary operation.



---

archive/issue_events_408646.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2020-12-14T20:22:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408646"
}
```



---

archive/issue_events_408647.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2020-12-14T20:22:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408647"
}
```



---

archive/issue_events_408648.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-18T16:17:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408648"
}
```



---

archive/issue_events_408649.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-18T16:17:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408649"
}
```



---

archive/issue_events_408650.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408650"
}
```



---

archive/issue_events_408651.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408651"
}
```



---

archive/issue_events_408652.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-09-16T14:49:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408652"
}
```



---

archive/issue_comments_499003.json:
```json
{
    "body": "Branch: **[public/30953](https://github.com/sagemath/sagetrac-mirror/tree/public/30953)**",
    "created_at": "2021-09-16T14:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499003",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/30953](https://github.com/sagemath/sagetrac-mirror/tree/public/30953)**



---

archive/issue_comments_499004.json:
```json
{
    "body": "Author: **Jonathan Kliem**",
    "created_at": "2021-09-16T14:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499004",
    "user": "https://github.com/kliem"
}
```

Author: **Jonathan Kliem**



---

archive/issue_comments_499005.json:
```json
{
    "body": "Dependencies: **#31951**",
    "created_at": "2021-09-16T14:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499005",
    "user": "https://github.com/kliem"
}
```

Dependencies: **#31951**



---

archive/issue_comments_499006.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nOnly works in the interactive shell so far and not when parsing `*.sage` files.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d9c4e95f8db6ad82291717e822ace7cd68971e51\">d9c4e95</a></td><td><code>fix completeness check for sage special syntax</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/72c89b14e7adf88cc188e43bf9b3899c4996f2b9\">72c89b1</a></td><td><code>preparse implicit line continuations correctly in interactive shell</code></td></tr></table>\n",
    "created_at": "2021-09-16T14:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499006",
    "user": "https://github.com/kliem"
}
```

<a id='comment:8'>Comment 8:</a>
Only works in the interactive shell so far and not when parsing `*.sage` files.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d9c4e95f8db6ad82291717e822ace7cd68971e51">d9c4e95</a></td><td><code>fix completeness check for sage special syntax</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/72c89b14e7adf88cc188e43bf9b3899c4996f2b9">72c89b1</a></td><td><code>preparse implicit line continuations correctly in interactive shell</code></td></tr></table>




---

archive/issue_comments_499007.json:
```json
{
    "body": "Commit: **[72c89b1](https://github.com/sagemath/sagetrac-mirror/commit/72c89b14e7adf88cc188e43bf9b3899c4996f2b9)**",
    "created_at": "2021-09-16T14:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499007",
    "user": "https://github.com/kliem"
}
```

Commit: **[72c89b1](https://github.com/sagemath/sagetrac-mirror/commit/72c89b14e7adf88cc188e43bf9b3899c4996f2b9)**



---

archive/issue_events_408653.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-09-21T15:30:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408653"
}
```



---

archive/issue_events_408654.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-09-21T15:30:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408654"
}
```



---

archive/issue_comments_499008.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI'm reworking the entire `preparse` now.\n\nThere is so much basic stuff that is just not working, e.g. line continuation for\n\n```\nsage: def foo():\n....:     a = 2312334432134.factor()\n```",
    "created_at": "2021-09-21T15:30:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499008",
    "user": "https://github.com/kliem"
}
```

<a id='comment:9'>Comment 9:</a>
I'm reworking the entire `preparse` now.

There is so much basic stuff that is just not working, e.g. line continuation for

```
sage: def foo():
....:     a = 2312334432134.factor()
```



---

archive/issue_comments_499009.json:
```json
{
    "body": "Work Issues: **close #4545?**",
    "created_at": "2021-09-23T04:33:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499009",
    "user": "https://github.com/kliem"
}
```

Work Issues: **close #4545?**



---

archive/issue_comments_499010.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nIn [#31951 comment:42](https://github.com/sagemath/sage/issues/31951#comment:42) I suggest a simple fix for #31951 and doing the more elaborate work here.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9e7c480c43ef491f92d90c4eb2775f98b6045ba8\">9e7c480</a></td><td><code>rework Sages input preparse, such that the syntax is valid after applying cleanup and token transformers</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db167ddc3c35b6ddbb61f97c7233c2f6d18d005e\">db167dd</a></td><td><code>correct ticket number</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9a5362a31c63d53c16cb62c33b990e91308aa114\">9a5362a</a></td><td><code>Stylistic fixes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b4655134f36d64649b5b7676add2c986e2a02766\">b465513</a></td><td><code>Transfer doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3d8655472435a8af62658b884d9599c60037e5f1\">3d86554</a></td><td><code>Revert deprecations</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fe36d2eb8473849d87bc1b36e458d05a7db60996\">fe36d2e</a></td><td><code>Merge branch 'develop' into trac31951</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5a7b172568acad1f2ea868608139a946df73a65e\">5a7b172</a></td><td><code>Remove unnecessary preparse_ipython</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bc8187324de61ff24581d665d4254214133d96f2\">bc81873</a></td><td><code>Fix a doctest failure</code></td></tr></table>\n",
    "created_at": "2021-09-27T21:51:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499010",
    "user": "https://github.com/kliem"
}
```

<a id='comment:11'>Comment 11:</a>
In [#31951 comment:42](https://github.com/sagemath/sage/issues/31951#comment:42) I suggest a simple fix for #31951 and doing the more elaborate work here.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9e7c480c43ef491f92d90c4eb2775f98b6045ba8">9e7c480</a></td><td><code>rework Sages input preparse, such that the syntax is valid after applying cleanup and token transformers</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db167ddc3c35b6ddbb61f97c7233c2f6d18d005e">db167dd</a></td><td><code>correct ticket number</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9a5362a31c63d53c16cb62c33b990e91308aa114">9a5362a</a></td><td><code>Stylistic fixes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b4655134f36d64649b5b7676add2c986e2a02766">b465513</a></td><td><code>Transfer doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3d8655472435a8af62658b884d9599c60037e5f1">3d86554</a></td><td><code>Revert deprecations</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fe36d2eb8473849d87bc1b36e458d05a7db60996">fe36d2e</a></td><td><code>Merge branch 'develop' into trac31951</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5a7b172568acad1f2ea868608139a946df73a65e">5a7b172</a></td><td><code>Remove unnecessary preparse_ipython</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bc8187324de61ff24581d665d4254214133d96f2">bc81873</a></td><td><code>Fix a doctest failure</code></td></tr></table>




---

archive/issue_comments_499011.json:
```json
{
    "body": "Changed branch from **[public/30953](https://github.com/sagemath/sagetrac-mirror/tree/public/30953)** to **[public/31951](https://github.com/sagemath/sagetrac-mirror/tree/public/31951)**",
    "created_at": "2021-09-27T21:51:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499011",
    "user": "https://github.com/kliem"
}
```

Changed branch from **[public/30953](https://github.com/sagemath/sagetrac-mirror/tree/public/30953)** to **[public/31951](https://github.com/sagemath/sagetrac-mirror/tree/public/31951)**



---

archive/issue_comments_499012.json:
```json
{
    "body": "Changed commit from **[72c89b1](https://github.com/sagemath/sagetrac-mirror/commit/72c89b14e7adf88cc188e43bf9b3899c4996f2b9)** to **[bc81873](https://github.com/sagemath/sagetrac-mirror/commit/bc8187324de61ff24581d665d4254214133d96f2)**",
    "created_at": "2021-09-27T21:51:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499012",
    "user": "https://github.com/kliem"
}
```

Changed commit from **[72c89b1](https://github.com/sagemath/sagetrac-mirror/commit/72c89b14e7adf88cc188e43bf9b3899c4996f2b9)** to **[bc81873](https://github.com/sagemath/sagetrac-mirror/commit/bc8187324de61ff24581d665d4254214133d96f2)**



---

archive/issue_comments_499013.json:
```json
{
    "body": "Changed author from **Jonathan Kliem** to **Jonathan Kliem, Kwankyu Lee**",
    "created_at": "2021-09-27T21:52:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499013",
    "user": "https://github.com/kliem"
}
```

Changed author from **Jonathan Kliem** to **Jonathan Kliem, Kwankyu Lee**



---

archive/issue_comments_499014.json:
```json
{
    "body": "Changed branch from **[public/31951](https://github.com/sagemath/sagetrac-mirror/tree/public/31951)** to **[public/30953-new](https://github.com/sagemath/sagetrac-mirror/tree/public/30953-new)**",
    "created_at": "2021-09-27T21:56:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499014",
    "user": "https://github.com/kliem"
}
```

Changed branch from **[public/31951](https://github.com/sagemath/sagetrac-mirror/tree/public/31951)** to **[public/30953-new](https://github.com/sagemath/sagetrac-mirror/tree/public/30953-new)**



---

archive/issue_comments_499015.json:
```json
{
    "body": "Changed commit from **[bc81873](https://github.com/sagemath/sagetrac-mirror/commit/bc8187324de61ff24581d665d4254214133d96f2)** to **[e34168c](https://github.com/sagemath/sagetrac-mirror/commit/e34168c0d17ee8a340570be2d5fa9c9f5674d071)**",
    "created_at": "2021-09-27T21:56:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499015",
    "user": "https://github.com/kliem"
}
```

Changed commit from **[bc81873](https://github.com/sagemath/sagetrac-mirror/commit/bc8187324de61ff24581d665d4254214133d96f2)** to **[e34168c](https://github.com/sagemath/sagetrac-mirror/commit/e34168c0d17ee8a340570be2d5fa9c9f5674d071)**



---

archive/issue_comments_499016.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nRenamed the branch and added to commits.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f7dbbcfd415a77b3c0d5a8185204c28e1bffe6d8\">f7dbbcf</a></td><td><code>do not deprecate InterfaceShellTransformer</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e34168c0d17ee8a340570be2d5fa9c9f5674d071\">e34168c</a></td><td><code>make doctest stable</code></td></tr></table>\n",
    "created_at": "2021-09-27T21:56:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499016",
    "user": "https://github.com/kliem"
}
```

<a id='comment:13'>Comment 13:</a>
Renamed the branch and added to commits.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f7dbbcfd415a77b3c0d5a8185204c28e1bffe6d8">f7dbbcf</a></td><td><code>do not deprecate InterfaceShellTransformer</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e34168c0d17ee8a340570be2d5fa9c9f5674d071">e34168c</a></td><td><code>make doctest stable</code></td></tr></table>




---

archive/issue_comments_499017.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nFixed in #31951.",
    "created_at": "2021-09-28T16:19:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499017",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:14'>Comment 14:</a>
Fixed in #31951.



---

archive/issue_comments_499018.json:
```json
{
    "body": "Changed author from **Jonathan Kliem, Kwankyu Lee** to none",
    "created_at": "2021-09-28T16:19:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499018",
    "user": "https://github.com/kwankyu"
}
```

Changed author from **Jonathan Kliem, Kwankyu Lee** to none



---

archive/issue_events_408655.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2021-09-28T16:19:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408655"
}
```



---

archive/issue_events_408656.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2021-09-28T16:19:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408656"
}
```



---

archive/issue_events_408657.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2021-09-28T16:19:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408657"
}
```



---

archive/issue_comments_499019.json:
```json
{
    "body": "Changed commit from **[e34168c](https://github.com/sagemath/sagetrac-mirror/commit/e34168c0d17ee8a340570be2d5fa9c9f5674d071)** to none",
    "created_at": "2021-09-28T16:19:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499019",
    "user": "https://github.com/kwankyu"
}
```

Changed commit from **[e34168c](https://github.com/sagemath/sagetrac-mirror/commit/e34168c0d17ee8a340570be2d5fa9c9f5674d071)** to none



---

archive/issue_events_408658.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2021-09-28T16:19:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408658"
}
```



---

archive/issue_events_408659.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2021-09-28T16:19:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408659"
}
```



---

archive/issue_comments_499020.json:
```json
{
    "body": "Changed branch from **[public/30953-new](https://github.com/sagemath/sagetrac-mirror/tree/public/30953-new)** to none",
    "created_at": "2021-09-28T16:19:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499020",
    "user": "https://github.com/kwankyu"
}
```

Changed branch from **[public/30953-new](https://github.com/sagemath/sagetrac-mirror/tree/public/30953-new)** to none



---

archive/issue_comments_499021.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nThis is fixed by #31951.",
    "created_at": "2021-10-01T06:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499021",
    "user": "https://github.com/kliem"
}
```

<a id='comment:15'>Comment 15:</a>
This is fixed by #31951.



---

archive/issue_events_408660.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-10-01T06:31:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408660"
}
```



---

archive/issue_events_408661.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-10-01T06:31:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408661"
}
```



---

archive/issue_comments_499022.json:
```json
{
    "body": "Changed dependencies from **#31951** to none",
    "created_at": "2021-10-01T06:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30953#issuecomment-499022",
    "user": "https://github.com/kliem"
}
```

Changed dependencies from **#31951** to none



---

archive/issue_events_408662.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T23:44:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408662"
}
```



---

archive/issue_events_408663.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T23:44:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30953",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30953#event-408663"
}
```
