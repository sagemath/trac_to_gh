# Issue 30404: Add pyright and pycodestyle checks as Github actions

archive/issues_030167.json:
```json
{
    "assignees": [],
    "body": "Currently, the linters (pycodestyle, pyflakes, ...) run as plugins of patchbot. This has a few disadvantages: a) the lint results depend on the version of the tools installed by the owner of the patchbot b) it takes quite some time until the results are available and c) you need people to run the patchbot.\n\nThis ticket adds pyright, relint and pycodestyle checks as Github actions. They run really fast (about 2min), have reliable results as the version of the linters can be controlled and uses Github server instead of people's machines.\n\nAlthough only those rules are activated that are also run by the patchbot, the pycodestyle and relint checks currently fail with a lot of errors. See https://github.com/tobiasdiez/sage/actions/runs/352364668. Thus, making these tests pass requires more work by the sage community.\n\nWhat is left to do is to add the integration of the github action output to trac - #30877\n\nCC:  @saraedum @mkoeppe @fchapoton @mjungmath @embray @slel @dimpase\n\nKeywords: **code style, lint**\n\nBranch/Commit: **[51828da](https://github.com/sagemath/sagetrac-mirror/commit/51828da28a18b1c0d161f09a91417abd9f652088)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Tobias Diez**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30404_\n\n",
    "closed_at": "2020-11-20T22:15:09Z",
    "created_at": "2020-08-20T11:33:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add pyright and pycodestyle checks as Github actions",
    "type": "issue",
    "updated_at": "2020-11-20T22:15:09Z",
    "url": "https://github.com/sagemath/sage/issues/30404",
    "user": "https://github.com/tobiasdiez"
}
```
Currently, the linters (pycodestyle, pyflakes, ...) run as plugins of patchbot. This has a few disadvantages: a) the lint results depend on the version of the tools installed by the owner of the patchbot b) it takes quite some time until the results are available and c) you need people to run the patchbot.

This ticket adds pyright, relint and pycodestyle checks as Github actions. They run really fast (about 2min), have reliable results as the version of the linters can be controlled and uses Github server instead of people's machines.

Although only those rules are activated that are also run by the patchbot, the pycodestyle and relint checks currently fail with a lot of errors. See https://github.com/tobiasdiez/sage/actions/runs/352364668. Thus, making these tests pass requires more work by the sage community.

What is left to do is to add the integration of the github action output to trac - #30877

CC:  @saraedum @mkoeppe @fchapoton @mjungmath @embray @slel @dimpase

Keywords: **code style, lint**

Branch/Commit: **[51828da](https://github.com/sagemath/sagetrac-mirror/commit/51828da28a18b1c0d161f09a91417abd9f652088)**

Reviewer: **Matthias Koeppe**

Author: **Tobias Diez**

_Issue created by migration from https://trac.sagemath.org/ticket/30404_





---

archive/issue_events_400793.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-08-20T11:33:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400793"
}
```



---

archive/issue_events_400794.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-08-20T11:33:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400794"
}
```



---

archive/issue_events_400795.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-08-20T11:33:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400795"
}
```



---

archive/issue_comments_487074.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nOption A: I think this is politically impossible. I know that some of the contributors here have (or at least had) strong feelings about github. That likely did not improve with Microsoft buying them.\n\nOption B*: Every branch here is already mirrored on [GitLab](../wiki/GitLab). I believe what's missing here is better docs: https://gitlab.com/sagemath/dev/trac\n\nOption C*: You could add the linter to [GitLab](../wiki/GitLab) CI. If I remember embray's comments correctly it's really easy to write a trac plugin that shows the gitlab CI status here on trac.",
    "created_at": "2020-08-20T11:59:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487074",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:1" align="right">Comment 1</div>

Option A: I think this is politically impossible. I know that some of the contributors here have (or at least had) strong feelings about github. That likely did not improve with Microsoft buying them.

Option B*: Every branch here is already mirrored on [GitLab](../wiki/GitLab). I believe what's missing here is better docs: https://gitlab.com/sagemath/dev/trac

Option C*: You could add the linter to [GitLab](../wiki/GitLab) CI. If I remember embray's comments correctly it's really easy to write a trac plugin that shows the gitlab CI status here on trac.



---

archive/issue_comments_487075.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNote there's also https://github.com/sagemath/sagetrac-mirror - but I don't know at what intervals it is mirrored.",
    "created_at": "2020-08-20T12:44:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487075",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

Note there's also https://github.com/sagemath/sagetrac-mirror - but I don't know at what intervals it is mirrored.



---

archive/issue_comments_487076.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nPerfect @mkoeppe! In this case, the integration should be fairly straightforward and amounts to adding the adding a link to the badge, by\n\n\n```\n<a href=\"https://github.com/sagemath/sagetrac-mirror/actions?query=workflow%3ALint+branch%3BRANCH\"><img src=\"https://github.com/sagemath/sagetrac-mirror/workflows/Lint/badge.svg?branch=BRANCH&event=push\"/></a>\n```\n\n\nGiving something similar to\n\n![](https://github.com/tobiasdiez/sage/workflows/Lint/badge.svg?branch=patch-2&event=push)\n\nwith a link to https://github.com/tobiasdiez/sage/actions?query=workflow%3ALint+branch%3Apatch-2\n(its sadly not possible to directly link to the workflow output)\n\nThis leaves the following todo list:\n- Restrict current github actions to master and develop branch\n- Activate gitub actions in sagetrac-mirror\n- Add the link to the badge to trac\n\nI can do the first task. Could somebody else then do point 2 and 3?",
    "created_at": "2020-08-20T13:33:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487076",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:3" align="right">Comment 3</div>

Perfect @mkoeppe! In this case, the integration should be fairly straightforward and amounts to adding the adding a link to the badge, by


```
<a href="https://github.com/sagemath/sagetrac-mirror/actions?query=workflow%3ALint+branch%3BRANCH"><img src="https://github.com/sagemath/sagetrac-mirror/workflows/Lint/badge.svg?branch=BRANCH&event=push"/></a>
```


Giving something similar to

![](https://github.com/tobiasdiez/sage/workflows/Lint/badge.svg?branch=patch-2&event=push)

with a link to https://github.com/tobiasdiez/sage/actions?query=workflow%3ALint+branch%3Apatch-2
(its sadly not possible to directly link to the workflow output)

This leaves the following todo list:
- Restrict current github actions to master and develop branch
- Activate gitub actions in sagetrac-mirror
- Add the link to the badge to trac

I can do the first task. Could somebody else then do point 2 and 3?



---

archive/issue_comments_487077.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nMy suggestion is to first focus on local operation. For example, finding a place for the  pycodestyle configuration in the Sage source tree (this should not be configured in  `.github/workflows/`, nor in the patchbot sources!) -- perhaps this should go to `src/tox.ini`, and then provide corresponding documentation on how to run it and why. Next, change the patchbot so that it uses the configuration from that file. This will make sure that things don't fall out of sync.",
    "created_at": "2020-08-20T13:35:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487077",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

My suggestion is to first focus on local operation. For example, finding a place for the  pycodestyle configuration in the Sage source tree (this should not be configured in  `.github/workflows/`, nor in the patchbot sources!) -- perhaps this should go to `src/tox.ini`, and then provide corresponding documentation on how to run it and why. Next, change the patchbot so that it uses the configuration from that file. This will make sure that things don't fall out of sync.



---

archive/issue_comments_487078.json:
```json
{
    "body": "Changed keywords from none to **code style, lint**",
    "created_at": "2020-08-20T14:28:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487078",
    "user": "https://github.com/slel"
}
```

Changed keywords from none to **code style, lint**



---

archive/issue_events_400796.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-20T14:28:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400796"
}
```



---

archive/issue_comments_487079.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nMoving the config to src/tox.ini is done in #30408.\n\nShould I continue with the above plan and restrict the current github workflows to the master & develop branch?",
    "created_at": "2020-08-20T18:58:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487079",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:7" align="right">Comment 7</div>

Moving the config to src/tox.ini is done in #30408.

Should I continue with the above plan and restrict the current github workflows to the master & develop branch?



---

archive/issue_comments_487080.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nTake a look at the other workflows from my portability tests. They are configured to run on every push to a tag and on every pull request. That's I think also the right granularity for the new workflow",
    "created_at": "2020-08-21T00:18:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487080",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Take a look at the other workflows from my portability tests. They are configured to run on every push to a tag and on every pull request. That's I think also the right granularity for the new workflow



---

archive/issue_comments_487081.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nReplying to [@tobiasdiez](#comment%3A7):\n> Moving the config to src/tox.ini is done in #30408.\n\nYou could make #30408 a dependency of this ticket & merge in the branch",
    "created_at": "2020-08-21T00:19:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487081",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:9" align="right">Comment 9</div>

Replying to [@tobiasdiez](#comment%3A7):
> Moving the config to src/tox.ini is done in #30408.

You could make #30408 a dependency of this ticket & merge in the branch



---

archive/issue_comments_487082.json:
```json
{
    "body": "Branch: **[public/build/lintGithub](https://github.com/sagemath/sagetrac-mirror/tree/public/build/lintGithub)**",
    "created_at": "2020-08-22T11:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487082",
    "user": "https://github.com/tobiasdiez"
}
```

Branch: **[public/build/lintGithub](https://github.com/sagemath/sagetrac-mirror/tree/public/build/lintGithub)**



---

archive/issue_events_400797.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-08-22T11:12:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400797"
}
```



---

archive/issue_comments_487083.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nI've now changed the workflow to use tox, which is enabled by #30408. Moreover, I realized based on the above comment by mkoeppe that the other Github actions are already restricted to only run on pushes with tags. Thus, they don't run on normal pushes to sagetrac-mirror, so no further action needed there.\n\n\n\n\n> They are configured to run on every push to a tag and on every pull request. That's I think also the right granularity for the new workflow\n\nI think the lint workflow should run on every push and pull request. In this way, the developer gets immediate feedback, and this is similar to the patchbot that also runs on every \"push\".\nMoreover, they run very quickly so there is no problem in running them on every push.\n\nSo this leaves the following todo's.\n- Peer-review this ticket\n- Activate gitub actions in sagetrac-mirror\n- Merge the dependencies #30408, #30361\n- Double check that the workflow runs successful on sagetrac-mirror\n- Merge\n- Add the link to the badge to trac as described above\n\nAfterwards, the workflow can be extended to include further linters. It might also be good to later add a basic build & doctest workflow that runs on every push.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2e88f614ec248e40ef76d76b61231f3c5f76af74\">2e88f61</a></td><td><code>Add lint workflow</code></td></tr></table>\n",
    "created_at": "2020-08-22T11:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487083",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:10" align="right">Comment 10</div>

I've now changed the workflow to use tox, which is enabled by #30408. Moreover, I realized based on the above comment by mkoeppe that the other Github actions are already restricted to only run on pushes with tags. Thus, they don't run on normal pushes to sagetrac-mirror, so no further action needed there.




> They are configured to run on every push to a tag and on every pull request. That's I think also the right granularity for the new workflow

I think the lint workflow should run on every push and pull request. In this way, the developer gets immediate feedback, and this is similar to the patchbot that also runs on every "push".
Moreover, they run very quickly so there is no problem in running them on every push.

So this leaves the following todo's.
- Peer-review this ticket
- Activate gitub actions in sagetrac-mirror
- Merge the dependencies #30408, #30361
- Double check that the workflow runs successful on sagetrac-mirror
- Merge
- Add the link to the badge to trac as described above

Afterwards, the workflow can be extended to include further linters. It might also be good to later add a basic build & doctest workflow that runs on every push.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2e88f614ec248e40ef76d76b61231f3c5f76af74">2e88f61</a></td><td><code>Add lint workflow</code></td></tr></table>




---

archive/issue_comments_487084.json:
```json
{
    "body": "Dependencies: **#30408, #30361**",
    "created_at": "2020-08-22T11:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487084",
    "user": "https://github.com/tobiasdiez"
}
```

Dependencies: **#30408, #30361**



---

archive/issue_comments_487085.json:
```json
{
    "body": "Commit: **[2e88f61](https://github.com/sagemath/sagetrac-mirror/commit/2e88f614ec248e40ef76d76b61231f3c5f76af74)**",
    "created_at": "2020-08-22T11:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487085",
    "user": "https://github.com/tobiasdiez"
}
```

Commit: **[2e88f61](https://github.com/sagemath/sagetrac-mirror/commit/2e88f614ec248e40ef76d76b61231f3c5f76af74)**



---

archive/issue_comments_487086.json:
```json
{
    "body": "Work Issues: **merge dependencies**",
    "created_at": "2020-09-05T18:49:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487086",
    "user": "https://github.com/mkoeppe"
}
```

Work Issues: **merge dependencies**



---

archive/issue_comments_487087.json:
```json
{
    "body": "Author: **Tobias Diez**",
    "created_at": "2020-09-05T18:49:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487087",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Tobias Diez**



---

archive/issue_events_400798.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T18:49:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400798"
}
```



---

archive/issue_events_400799.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T18:49:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400799"
}
```



---

archive/issue_comments_487088.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\n@mkoeppe, what should I do here? The dependencies are rather indirect (it works without #30361, it just tests more code). Moreover, this github action is currently nowhere invoked and hence not checked / tested....",
    "created_at": "2020-09-05T20:09:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487088",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:12" align="right">Comment 12</div>

@mkoeppe, what should I do here? The dependencies are rather indirect (it works without #30361, it just tests more code). Moreover, this github action is currently nowhere invoked and hence not checked / tested....



---

archive/issue_comments_487089.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nTicket description: \"The pyright check currently fails, as there is no configuration file\"",
    "created_at": "2020-09-05T20:13:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487089",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:13" align="right">Comment 13</div>

Ticket description: "The pyright check currently fails, as there is no configuration file"



---

archive/issue_comments_487090.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nYes, but even with the configuration files added the github workflow fails as there is no typing information yet(and the one added in #29775 is not complete) and pycodestyle reports over 600 errors as well.\n\nFixing these points requires more effort from the whole sage community.\n\nThe aim of this ticket is to add the github actions so that one has a consistent check, which at some point in the future hopefully turns green.",
    "created_at": "2020-09-06T11:36:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487090",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:14" align="right">Comment 14</div>

Yes, but even with the configuration files added the github workflow fails as there is no typing information yet(and the one added in #29775 is not complete) and pycodestyle reports over 600 errors as well.

Fixing these points requires more effort from the whole sage community.

The aim of this ticket is to add the github actions so that one has a consistent check, which at some point in the future hopefully turns green.



---

archive/issue_events_400800.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-09-06T11:36:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400800"
}
```



---

archive/issue_events_400801.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-09-06T11:36:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400801"
}
```



---

archive/issue_events_400802.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-09-18T21:52:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400802"
}
```



---

archive/issue_events_400803.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-09-18T21:52:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400803"
}
```



---

archive/issue_comments_487091.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nShould invoke relint as well (added in #30467).",
    "created_at": "2020-09-18T21:52:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487091",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:15" align="right">Comment 15</div>

Should invoke relint as well (added in #30467).



---

archive/issue_comments_487092.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nCould just run all tox environments...",
    "created_at": "2020-09-18T21:54:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487092",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:16" align="right">Comment 16</div>

Could just run all tox environments...



---

archive/issue_events_400804.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400804"
}
```



---

archive/issue_events_400805.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400805"
}
```



---

archive/issue_comments_487093.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e3751dec25712fed85ed9dbe5ffe581a73e935bb\">e3751de</a></td><td><code>Merge branch 'develop' of git://github.com/sagemath/sage into public/build/lintGithub</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/51828da28a18b1c0d161f09a91417abd9f652088\">51828da</a></td><td><code>Add relint</code></td></tr></table>\n",
    "created_at": "2020-11-08T14:21:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487093",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:18" align="right">Comment 18</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e3751dec25712fed85ed9dbe5ffe581a73e935bb">e3751de</a></td><td><code>Merge branch 'develop' of git://github.com/sagemath/sage into public/build/lintGithub</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/51828da28a18b1c0d161f09a91417abd9f652088">51828da</a></td><td><code>Add relint</code></td></tr></table>




---

archive/issue_comments_487094.json:
```json
{
    "body": "Changed commit from **[2e88f61](https://github.com/sagemath/sagetrac-mirror/commit/2e88f614ec248e40ef76d76b61231f3c5f76af74)** to **[51828da](https://github.com/sagemath/sagetrac-mirror/commit/51828da28a18b1c0d161f09a91417abd9f652088)**",
    "created_at": "2020-11-08T14:21:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487094",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2e88f61](https://github.com/sagemath/sagetrac-mirror/commit/2e88f614ec248e40ef76d76b61231f3c5f76af74)** to **[51828da](https://github.com/sagemath/sagetrac-mirror/commit/51828da28a18b1c0d161f09a91417abd9f652088)**



---

archive/issue_events_400806.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-08T14:31:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400806"
}
```



---

archive/issue_events_400807.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-08T14:31:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400807"
}
```



---

archive/issue_comments_487095.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,16 +1,20 @@\n Currently, the linters (pycodestyle, pyflakes, ...) run as plugins of patchbot. This has a few disadvantages: a) the lint results depend on the version of the tools installed by the owner of the patchbot b) it takes quite some time until the results are available and c) you need people to run the patchbot.\n \n-At https://github.com/sagemath/sage/pull/95 I added the pyright and pycodestyle checks as Github actions. They run really fast (about 10min), have reliable results as the version of the linters can be controlled and uses Github server instead of people's machines.\n+This ticket adds pyright, relint and pycodestyle checks as Github actions. They run really fast (about 2min), have reliable results as the version of the linters can be controlled and uses Github server instead of people's machines.\n \n-The pyright check currently fails, as there is no configuration file (which is added in #30361). The pycodestyle check also fails as there are currently over 600 errors that for some reason were missed by the patchbot (see https://github.com/tobiasdiez/sage/runs/1007221345?check_suite_focus=true) (I only activated those rules that are also run by the patchbot).\n+Although only those rules are activated that are also run by the patchbot, the pycodestyle and relint checks currently fail with a lot of errors. See https://github.com/tobiasdiez/sage/actions/runs/352364668. Thus, making these tests pass requires more work by the sage community.\n \n-**Question 1:** What should be done about these errors?\n+What is left to do is to add the integration of the github action output to trac. That can be done as follows:\n+1. Sync trac branches to sagetrac-mirror (by post-commit hook)\n+2. Activate github actions in the sagetrac-mirror github repo\n+3. Add the following code to either the patchbot output or to the trac ticket header (next to the patchbot button)\n \n-**Question 2:** What is the best way apply the Github actions to new code contributions?\n+```\n+<a href=\"https://github.com/sagemath/sagetrac-mirror/actions?query=workflow%3ALint+branch%3BRANCH\"><img src=\"https://github.com/sagemath/sagetrac-mirror/workflows/Lint/badge.svg?branch=BRANCH&event=push\"/></a>\n+```\n+Here BRANCH needs to be replaced by the branch.\n+Giving something similar to\n \n-*Option A:* Switch from trac to pull requests via Github (refs #30363)\n+![](https://github.com/tobiasdiez/sage/workflows/Lint/badge.svg?branch=patch-2&event=push)\n \n-*Option B:* Mirror every branch on trac to Github (it's not clear to me how to best integrate the results from the Github actions then here on trac; one option would be to use the Github API to create pull requests automatically for each trac ticket)\n-\n-*Option C:* Integrate the Github actions directly in the patchbot (how?)\n-\n+As this requires rights to the github repo and trac config, I cannot do this and have to leave it to somebody else.\n``````\n",
    "created_at": "2020-11-08T14:31:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487095",
    "user": "https://github.com/tobiasdiez"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,16 +1,20 @@
 Currently, the linters (pycodestyle, pyflakes, ...) run as plugins of patchbot. This has a few disadvantages: a) the lint results depend on the version of the tools installed by the owner of the patchbot b) it takes quite some time until the results are available and c) you need people to run the patchbot.
 
-At https://github.com/sagemath/sage/pull/95 I added the pyright and pycodestyle checks as Github actions. They run really fast (about 10min), have reliable results as the version of the linters can be controlled and uses Github server instead of people's machines.
+This ticket adds pyright, relint and pycodestyle checks as Github actions. They run really fast (about 2min), have reliable results as the version of the linters can be controlled and uses Github server instead of people's machines.
 
-The pyright check currently fails, as there is no configuration file (which is added in #30361). The pycodestyle check also fails as there are currently over 600 errors that for some reason were missed by the patchbot (see https://github.com/tobiasdiez/sage/runs/1007221345?check_suite_focus=true) (I only activated those rules that are also run by the patchbot).
+Although only those rules are activated that are also run by the patchbot, the pycodestyle and relint checks currently fail with a lot of errors. See https://github.com/tobiasdiez/sage/actions/runs/352364668. Thus, making these tests pass requires more work by the sage community.
 
-**Question 1:** What should be done about these errors?
+What is left to do is to add the integration of the github action output to trac. That can be done as follows:
+1. Sync trac branches to sagetrac-mirror (by post-commit hook)
+2. Activate github actions in the sagetrac-mirror github repo
+3. Add the following code to either the patchbot output or to the trac ticket header (next to the patchbot button)
 
-**Question 2:** What is the best way apply the Github actions to new code contributions?
+```
+<a href="https://github.com/sagemath/sagetrac-mirror/actions?query=workflow%3ALint+branch%3BRANCH"><img src="https://github.com/sagemath/sagetrac-mirror/workflows/Lint/badge.svg?branch=BRANCH&event=push"/></a>
+```
+Here BRANCH needs to be replaced by the branch.
+Giving something similar to
 
-*Option A:* Switch from trac to pull requests via Github (refs #30363)
+![](https://github.com/tobiasdiez/sage/workflows/Lint/badge.svg?branch=patch-2&event=push)
 
-*Option B:* Mirror every branch on trac to Github (it's not clear to me how to best integrate the results from the Github actions then here on trac; one option would be to use the Github API to create pull requests automatically for each trac ticket)
-
-*Option C:* Integrate the Github actions directly in the patchbot (how?)
-
+As this requires rights to the github repo and trac config, I cannot do this and have to leave it to somebody else.
``````




---

archive/issue_comments_487096.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nI've now added relint. This is now ready for review.",
    "created_at": "2020-11-08T14:31:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487096",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:19" align="right">Comment 19</div>

I've now added relint. This is now ready for review.



---

archive/issue_comments_487097.json:
```json
{
    "body": "Changed dependencies from **#30408, #30361** to none",
    "created_at": "2020-11-09T04:03:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487097",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#30408, #30361** to none



---

archive/issue_comments_487098.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,17 +4,4 @@\n \n Although only those rules are activated that are also run by the patchbot, the pycodestyle and relint checks currently fail with a lot of errors. See https://github.com/tobiasdiez/sage/actions/runs/352364668. Thus, making these tests pass requires more work by the sage community.\n \n-What is left to do is to add the integration of the github action output to trac. That can be done as follows:\n-1. Sync trac branches to sagetrac-mirror (by post-commit hook)\n-2. Activate github actions in the sagetrac-mirror github repo\n-3. Add the following code to either the patchbot output or to the trac ticket header (next to the patchbot button)\n-\n-```\n-<a href=\"https://github.com/sagemath/sagetrac-mirror/actions?query=workflow%3ALint+branch%3BRANCH\"><img src=\"https://github.com/sagemath/sagetrac-mirror/workflows/Lint/badge.svg?branch=BRANCH&event=push\"/></a>\n-```\n-Here BRANCH needs to be replaced by the branch.\n-Giving something similar to\n-\n-![](https://github.com/tobiasdiez/sage/workflows/Lint/badge.svg?branch=patch-2&event=push)\n-\n-As this requires rights to the github repo and trac config, I cannot do this and have to leave it to somebody else.\n+What is left to do is to add the integration of the github action output to trac - #30877\n``````\n",
    "created_at": "2020-11-09T04:03:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487098",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,17 +4,4 @@
 
 Although only those rules are activated that are also run by the patchbot, the pycodestyle and relint checks currently fail with a lot of errors. See https://github.com/tobiasdiez/sage/actions/runs/352364668. Thus, making these tests pass requires more work by the sage community.
 
-What is left to do is to add the integration of the github action output to trac. That can be done as follows:
-1. Sync trac branches to sagetrac-mirror (by post-commit hook)
-2. Activate github actions in the sagetrac-mirror github repo
-3. Add the following code to either the patchbot output or to the trac ticket header (next to the patchbot button)
-
-```
-<a href="https://github.com/sagemath/sagetrac-mirror/actions?query=workflow%3ALint+branch%3BRANCH"><img src="https://github.com/sagemath/sagetrac-mirror/workflows/Lint/badge.svg?branch=BRANCH&event=push"/></a>
-```
-Here BRANCH needs to be replaced by the branch.
-Giving something similar to
-
-![](https://github.com/tobiasdiez/sage/workflows/Lint/badge.svg?branch=patch-2&event=push)
-
-As this requires rights to the github repo and trac config, I cannot do this and have to leave it to somebody else.
+What is left to do is to add the integration of the github action output to trac - #30877
``````




---

archive/issue_events_400808.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-09T04:03:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400808"
}
```



---

archive/issue_events_400809.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-09T04:03:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400809"
}
```



---

archive/issue_comments_487099.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nThis seems to work well. \n\nI have moved the part of the ticket description that describes Trac integration to the new ticket #30877. But actually, given that the new push mirror for Trac ended up using GitLab instead of GitHub (see #30736), you should probably look into creating [GitLab](../wiki/GitLab) pipelines for the same checks.",
    "created_at": "2020-11-09T04:03:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487099",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:20" align="right">Comment 20</div>

This seems to work well. 

I have moved the part of the ticket description that describes Trac integration to the new ticket #30877. But actually, given that the new push mirror for Trac ended up using GitLab instead of GitHub (see #30736), you should probably look into creating [GitLab](../wiki/GitLab) pipelines for the same checks.



---

archive/issue_comments_487100.json:
```json
{
    "body": "Changed work issues from **merge dependencies** to none",
    "created_at": "2020-11-09T04:03:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487100",
    "user": "https://github.com/mkoeppe"
}
```

Changed work issues from **merge dependencies** to none



---

archive/issue_comments_487101.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nThanks for the review!\n\nI think you can make a good argument to have both Gitlab as well as Github as push mirrors.",
    "created_at": "2020-11-09T11:22:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487101",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:21" align="right">Comment 21</div>

Thanks for the review!

I think you can make a good argument to have both Gitlab as well as Github as push mirrors.



---

archive/issue_comments_487102.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nreviewer name missing",
    "created_at": "2020-11-09T14:36:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487102",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:22" align="right">Comment 22</div>

reviewer name missing



---

archive/issue_comments_487103.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-11-09T16:56:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487103",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_487104.json:
```json
{
    "body": "Changed branch from **[public/build/lintGithub](https://github.com/sagemath/sagetrac-mirror/tree/public/build/lintGithub)** to **[51828da](https://github.com/sagemath/sagetrac-mirror/commit/51828da28a18b1c0d161f09a91417abd9f652088)**",
    "created_at": "2020-11-20T22:15:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30404#issuecomment-487104",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/build/lintGithub](https://github.com/sagemath/sagetrac-mirror/tree/public/build/lintGithub)** to **[51828da](https://github.com/sagemath/sagetrac-mirror/commit/51828da28a18b1c0d161f09a91417abd9f652088)**



---

archive/issue_events_400810.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-11-20T22:15:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400810"
}
```



---

archive/issue_events_400811.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7863f17c2713b1cad23fa878ae9dafc22d70d6eb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-11-20T22:15:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30404",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30404#event-400811"
}
```
