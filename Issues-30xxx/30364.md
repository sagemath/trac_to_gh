# Issue 30364: Doctests using system brial crash (segfault) on debian-sid, -bullseye

archive/issues_030127.json:
```json
{
    "assignees": [],
    "body": "`debian-sid` (https://github.com/mkoeppe/sage/runs/985563463) and `debian-bullseye`:\n\n```\nsage -t --random-seed=0 src/doc/en/constructions/algebraic_geometry.rst  # 1 doctest failed\nsage -t --random-seed=0 src/doc/en/developer/coding_in_other.rst  # 1 doctest failed\nsage -t --random-seed=0 src/doc/en/reference/sat/index.rst  # Killed due to segmentation fault\nsage -t --random-seed=0 src/sage/crypto/mq/sr.py  # Killed due to abort\nsage -t --random-seed=0 src/sage/modules/fg_pid/fgp_module.py  # 3 doctests failed\nsage -t --random-seed=0 src/sage/modules/free_module_morphism.py  # 3 doctests failed\nsage -t --random-seed=0 src/sage/rings/complex_arb.pyx  # 6 doctests failed\nsage -t --random-seed=0 src/sage/rings/polynomial/multi_polynomial_sequence.py  # Killed due to segmentation fault\nsage -t --random-seed=0 src/sage/rings/polynomial/pbori.pyx  # Killed due to segmentation fault\nsage -t --random-seed=0 src/sage/rings/polynomial/polynomial_ring_constructor.py  # Killed due to abort\nsage -t --random-seed=0 src/sage/rings/real_arb.pyx  # 2 doctests failed\nsage -t --random-seed=0 src/sage/sat/boolean_polynomials.py  # Killed due to segmentation fault\nsage -t --random-seed=0 src/sage/sat/solvers/dimacs.py  # Killed due to segmentation fault\nsage -t --random-seed=0 src/sage/schemes/hyperelliptic_curves/monsky_washnitzer.py  # Killed due to abort\n```\n\nPer previous tickets (#29490, #29792), we avoid brial 1.2.5 for the same reason. \n\nHere it is 1.2.8 that crashes, illustrating once more why version number based tests are not robust enough.\n\n\n\n\n\n\nCC:  @kiwifb @dimpase @orlitzky @tobihan @sagetrac-Snark\n\nReviewer: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30364_\n\n",
    "closed_at": "2020-10-11T16:43:58Z",
    "created_at": "2020-08-15T00:28:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Doctests using system brial crash (segfault) on debian-sid, -bullseye",
    "type": "issue",
    "updated_at": "2020-10-11T16:43:58Z",
    "url": "https://github.com/sagemath/sage/issues/30364",
    "user": "https://github.com/mkoeppe"
}
```
`debian-sid` (https://github.com/mkoeppe/sage/runs/985563463) and `debian-bullseye`:

```
sage -t --random-seed=0 src/doc/en/constructions/algebraic_geometry.rst  # 1 doctest failed
sage -t --random-seed=0 src/doc/en/developer/coding_in_other.rst  # 1 doctest failed
sage -t --random-seed=0 src/doc/en/reference/sat/index.rst  # Killed due to segmentation fault
sage -t --random-seed=0 src/sage/crypto/mq/sr.py  # Killed due to abort
sage -t --random-seed=0 src/sage/modules/fg_pid/fgp_module.py  # 3 doctests failed
sage -t --random-seed=0 src/sage/modules/free_module_morphism.py  # 3 doctests failed
sage -t --random-seed=0 src/sage/rings/complex_arb.pyx  # 6 doctests failed
sage -t --random-seed=0 src/sage/rings/polynomial/multi_polynomial_sequence.py  # Killed due to segmentation fault
sage -t --random-seed=0 src/sage/rings/polynomial/pbori.pyx  # Killed due to segmentation fault
sage -t --random-seed=0 src/sage/rings/polynomial/polynomial_ring_constructor.py  # Killed due to abort
sage -t --random-seed=0 src/sage/rings/real_arb.pyx  # 2 doctests failed
sage -t --random-seed=0 src/sage/sat/boolean_polynomials.py  # Killed due to segmentation fault
sage -t --random-seed=0 src/sage/sat/solvers/dimacs.py  # Killed due to segmentation fault
sage -t --random-seed=0 src/sage/schemes/hyperelliptic_curves/monsky_washnitzer.py  # Killed due to abort
```

Per previous tickets (#29490, #29792), we avoid brial 1.2.5 for the same reason. 

Here it is 1.2.8 that crashes, illustrating once more why version number based tests are not robust enough.






CC:  @kiwifb @dimpase @orlitzky @tobihan @sagetrac-Snark

Reviewer: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/30364_





---

archive/issue_events_400143.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-15T00:28:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400143"
}
```



---

archive/issue_events_400144.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-15T00:28:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400144"
}
```



---

archive/issue_events_400145.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-15T00:28:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400145"
}
```



---

archive/issue_events_400146.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-15T00:28:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400146"
}
```



---

archive/issue_events_400147.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-08-15T07:07:59Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "title_is": "Doctests using system brial crash (segfault) on debian-sid, -bullseye",
    "title_was": "Doctests using system brial crash (segfault) on debian-sid, -buster",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400147"
}
```



---

archive/issue_comments_485521.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\ncc debian people",
    "created_at": "2020-08-15T07:07:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30364#issuecomment-485521",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:1" align="right">Comment 1</div>

cc debian people



---

archive/issue_comments_485522.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nan explicit test on 1.2.5 was rather unclear how to accomplish, thus I opted for banning a version.",
    "created_at": "2020-08-15T07:10:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30364#issuecomment-485522",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:2" align="right">Comment 2</div>

an explicit test on 1.2.5 was rather unclear how to accomplish, thus I opted for banning a version.



---

archive/issue_events_400148.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-09-08T07:46:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400148"
}
```



---

archive/issue_events_400149.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-09-08T07:46:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400149"
}
```



---

archive/issue_comments_485523.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nMaybe we should have positive lists instead of negative lists, so by default we don't use unknown system versions? Either way, sid/bullseye are not (and will not be in the near future)  released versions -> not a blocker",
    "created_at": "2020-09-08T07:46:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30364#issuecomment-485523",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Maybe we should have positive lists instead of negative lists, so by default we don't use unknown system versions? Either way, sid/bullseye are not (and will not be in the near future)  released versions -> not a blocker



---

archive/issue_events_400150.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-09T01:10:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400150"
}
```



---

archive/issue_events_400151.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-09T01:10:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400151"
}
```



---

archive/issue_events_400152.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-09T01:10:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400152"
}
```



---

archive/issue_events_400153.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-09T01:10:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400153"
}
```



---

archive/issue_comments_485524.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThe segfaults seem to have disappeared in more recent runs (https://groups.google.com/d/msg/sage-release/hobZzw74Rv0/0CkgcJYjCgAJ), so perhaps this was just a Debian bug.",
    "created_at": "2020-09-09T01:10:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30364#issuecomment-485524",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

The segfaults seem to have disappeared in more recent runs (https://groups.google.com/d/msg/sage-release/hobZzw74Rv0/0CkgcJYjCgAJ), so perhaps this was just a Debian bug.



---

archive/issue_events_400154.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-09-09T06:12:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400154"
}
```



---

archive/issue_events_400155.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-09-09T06:12:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400155"
}
```



---

archive/issue_comments_485525.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-09-09T06:12:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30364#issuecomment-485525",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_400156.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-10-11T16:43:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400156"
}
```



---

archive/issue_events_400157.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-10-11T16:43:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400157"
}
```



---

archive/issue_events_400158.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-10-11T16:43:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30364",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30364#event-400158"
}
```
