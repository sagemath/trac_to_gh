# Issue 30023: Build Windows installer on GH Actions

archive/issues_029786.json:
```json
{
    "assignees": [],
    "body": "... as discussed in https://github.com/sagemath/sage-windows/issues/45\n\n\n\nDepends on #29537\n\nCC:  @embray @slel\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30023_\n\n",
    "created_at": "2020-06-29T18:55:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/pending"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Build Windows installer on GH Actions",
    "type": "issue",
    "updated_at": "2022-08-25T20:19:16Z",
    "url": "https://github.com/sagemath/sage/issues/30023",
    "user": "https://github.com/mkoeppe"
}
```
... as discussed in https://github.com/sagemath/sage-windows/issues/45



Depends on #29537

CC:  @embray @slel

_Issue created by migration from https://trac.sagemath.org/ticket/30023_





---

archive/issue_events_395181.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-29T18:55:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395181"
}
```



---

archive/issue_events_395182.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-29T18:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395182"
}
```



---

archive/issue_events_395183.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-29T18:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395183"
}
```



---

archive/issue_events_395184.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-29T18:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395184"
}
```



---

archive/issue_comments_478183.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis could be `cygwin-stage-iv-e` unless you want some optional packages in it, which are built in stages `iv-b` to `iv-d`, in which case it should be stage `v`.",
    "created_at": "2020-06-29T19:00:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30023#issuecomment-478183",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

This could be `cygwin-stage-iv-e` unless you want some optional packages in it, which are built in stages `iv-b` to `iv-d`, in which case it should be stage `v`.



---

archive/issue_comments_478184.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAccording to https://github.com/actions/virtual-environments/blob/master/images/win/Windows2019-Readme.md, InnoSetup 6.0.5 comes preinstalled",
    "created_at": "2020-06-30T00:27:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30023#issuecomment-478184",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

According to https://github.com/actions/virtual-environments/blob/master/images/win/Windows2019-Readme.md, InnoSetup 6.0.5 comes preinstalled



---

archive/issue_events_395185.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395185"
}
```



---

archive/issue_events_395186.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395186"
}
```



---

archive/issue_comments_478185.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLet's get this ready for Sage 9.3?",
    "created_at": "2021-03-15T21:59:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30023#issuecomment-478185",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Let's get this ready for Sage 9.3?



---

archive/issue_comments_478186.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI don't know if I'll have time before then to really make sure this is working, but after 9.3 (or even sooner) we can at least start testing it on betas?",
    "created_at": "2021-03-17T15:48:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30023#issuecomment-478186",
    "user": "https://github.com/embray"
}
```

<div id="comment:5" align="right">Comment 5</div>

I don't know if I'll have time before then to really make sure this is working, but after 9.3 (or even sooner) we can at least start testing it on betas?



---

archive/issue_comments_478187.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-04-07T19:29:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30023#issuecomment-478187",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_395187.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395187"
}
```



---

archive/issue_events_395188.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395188"
}
```



---

archive/issue_events_395189.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395189"
}
```



---

archive/issue_events_395190.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395190"
}
```



---

archive/issue_events_395191.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395191"
}
```



---

archive/issue_events_395192.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395192"
}
```



---

archive/issue_events_395193.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395193"
}
```



---

archive/issue_events_395194.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395194"
}
```



---

archive/issue_events_395195.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-25T20:19:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395195"
}
```



---

archive/issue_events_395196.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-25T20:19:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30023",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30023#event-395196"
}
```
