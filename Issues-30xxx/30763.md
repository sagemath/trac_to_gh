# Issue 30763: improve setting global options to None

archive/issues_030526.json:
```json
{
    "assignees": [],
    "body": "The `__call__` method of global options is used as getter and setter. This ticket improves this method to properly handle values of `None`.\n\nFor example:\n\n```python\nsage: from sage.structure.global_options import GlobalOptions\nsage: class config(GlobalOptions):\n....:     size = dict(default=42,\n....:                 description='integer or None',\n....:                 checker=lambda val: val is None or val >= 0)\nsage: config.size(None)  # this should set the option to `None`\n42\n```\n(See #30755 for an actual example where `None` is used as a value.)\n\nThis can be used as a workaround instead:\n\n```python\nsage: config.size = None\nsage: config.size() is None\nTrue\n```\n\nThis ticket makes `config.size(None)` work as a setter. Additionally, a deprecation is added for the keyword-argument syntax `config.size(value=None)`, as it is not usually used and this will simplify the implementation a little.\n\n\nBranch/Commit: **[4c08256](https://github.com/sagemath/sagetrac-mirror/commit/4c08256824fe4df325e5f391de2d3f9546a8f201)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30763_\n\n",
    "closed_at": "2022-02-16T23:56:50Z",
    "created_at": "2020-10-13T17:52:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve setting global options to None",
    "type": "issue",
    "updated_at": "2022-02-16T23:56:50Z",
    "url": "https://github.com/sagemath/sage/issues/30763",
    "user": "https://github.com/mwageringel"
}
```
The `__call__` method of global options is used as getter and setter. This ticket improves this method to properly handle values of `None`.

For example:

```python
sage: from sage.structure.global_options import GlobalOptions
sage: class config(GlobalOptions):
....:     size = dict(default=42,
....:                 description='integer or None',
....:                 checker=lambda val: val is None or val >= 0)
sage: config.size(None)  # this should set the option to `None`
42
```
(See #30755 for an actual example where `None` is used as a value.)

This can be used as a workaround instead:

```python
sage: config.size = None
sage: config.size() is None
True
```

This ticket makes `config.size(None)` work as a setter. Additionally, a deprecation is added for the keyword-argument syntax `config.size(value=None)`, as it is not usually used and this will simplify the implementation a little.


Branch/Commit: **[4c08256](https://github.com/sagemath/sagetrac-mirror/commit/4c08256824fe4df325e5f391de2d3f9546a8f201)**

Reviewer: **Vincent Delecroix**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/30763_





---

archive/issue_events_406038.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-10-13T17:52:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406038"
}
```



---

archive/issue_events_406039.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-10-13T17:52:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406039"
}
```



---

archive/issue_events_406040.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-10-13T17:52:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406040"
}
```



---

archive/issue_comments_494720.json:
```json
{
    "body": "Author: **Markus Wageringel**",
    "created_at": "2020-10-13T17:56:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494720",
    "user": "https://github.com/mwageringel"
}
```

Author: **Markus Wageringel**



---

archive/issue_comments_494721.json:
```json
{
    "body": "Commit: **[4c08256](https://github.com/sagemath/sagetrac-mirror/commit/4c08256824fe4df325e5f391de2d3f9546a8f201)**",
    "created_at": "2020-10-13T17:56:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494721",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[4c08256](https://github.com/sagemath/sagetrac-mirror/commit/4c08256824fe4df325e5f391de2d3f9546a8f201)**



---

archive/issue_comments_494722.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4c08256824fe4df325e5f391de2d3f9546a8f201\">4c08256</a></td><td><code>30763: allow setting global options to None</code></td></tr></table>\n",
    "created_at": "2020-10-13T17:56:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494722",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4c08256824fe4df325e5f391de2d3f9546a8f201">4c08256</a></td><td><code>30763: allow setting global options to None</code></td></tr></table>




---

archive/issue_comments_494723.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/30763](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/30763)**",
    "created_at": "2020-10-13T17:56:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494723",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/30763](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/30763)**



---

archive/issue_events_406041.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-10-13T17:56:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406041"
}
```



---

archive/issue_comments_494724.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nWouldn't it be simpler to just use a unique object instead of `None` as the default argument?  Then no change of the interface is necessary",
    "created_at": "2020-11-30T05:38:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494724",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

Wouldn't it be simpler to just use a unique object instead of `None` as the default argument?  Then no change of the interface is necessary



---

archive/issue_comments_494725.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nWell, yes, but I think that it is common to use `None` as a default value/no preferred choice. For example, in Sage, this is the case for the [preferences of the display manager](https://gitlab.com/sagemath/sage/-/blob/020bd59ec28717bfab9af44d2231c53da1ff99f1/src/sage/repl/rich_output/preferences.py#L7-9) (though, these are distinct from `GlobalOptions`).\n\nNote that, in the example from the description, `config.size()` still keeps working as a getter, so the interface does not really change.",
    "created_at": "2020-11-30T17:48:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494725",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:3" align="right">Comment 3</div>

Well, yes, but I think that it is common to use `None` as a default value/no preferred choice. For example, in Sage, this is the case for the [preferences of the display manager](https://gitlab.com/sagemath/sage/-/blob/020bd59ec28717bfab9af44d2231c53da1ff99f1/src/sage/repl/rich_output/preferences.py#L7-9) (though, these are distinct from `GlobalOptions`).

Note that, in the example from the description, `config.size()` still keeps working as a getter, so the interface does not really change.



---

archive/issue_events_406042.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406042"
}
```



---

archive/issue_events_406043.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406043"
}
```



---

archive/issue_comments_494726.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-15T22:07:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494726",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_comments_494727.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nSetting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494727",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_406044.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406044"
}
```



---

archive/issue_events_406045.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406045"
}
```



---

archive/issue_events_406046.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406046"
}
```



---

archive/issue_events_406047.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406047"
}
```



---

archive/issue_comments_494728.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nStalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.",
    "created_at": "2021-12-18T19:53:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494728",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Stalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.



---

archive/issue_comments_494729.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThe proposed change is reasonable and simplifies (actually will simplify in the future) the implementation of `__call__`.",
    "created_at": "2021-12-29T11:43:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494729",
    "user": "https://github.com/videlec"
}
```

<div id="comment:7" align="right">Comment 7</div>

The proposed change is reasonable and simplifies (actually will simplify in the future) the implementation of `__call__`.



---

archive/issue_events_406048.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2021-12-29T11:43:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406048"
}
```



---

archive/issue_events_406049.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2021-12-29T11:43:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406049"
}
```



---

archive/issue_comments_494730.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2021-12-29T11:43:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494730",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_events_406050.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-02-16T23:56:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406050"
}
```



---

archive/issue_events_406051.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cd31b18f040b95683ce121b51c43e30708616101",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-02-16T23:56:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30763#event-406051"
}
```



---

archive/issue_comments_494731.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/30763](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/30763)** to **[4c08256](https://github.com/sagemath/sagetrac-mirror/commit/4c08256824fe4df325e5f391de2d3f9546a8f201)**",
    "created_at": "2022-02-16T23:56:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30763#issuecomment-494731",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/30763](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/30763)** to **[4c08256](https://github.com/sagemath/sagetrac-mirror/commit/4c08256824fe4df325e5f391de2d3f9546a8f201)**
