# Issue 30322: Deprecate import of classes Sigma, Euler_Phi, Moebius to the global namespace

archive/issues_030085.json:
```json
{
    "assignees": [],
    "body": "They are classes that implement the global functions `sigma`, `euler_phi`, `moebius`. There is no reason why the classes also need to be in the global namespace.\n\nIn this ticket, we deprecate the import.\n\n\nCC:  @DaveWitteMorris @slel @fchapoton\n\nBranch/Commit: **[4b8d207](https://github.com/sagemath/sagetrac-mirror/commit/4b8d207c9ab98019c8cfe74d74fbc9226aa70d6c)**\n\nReviewer: **Markus Wageringel**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30322_\n\n",
    "closed_at": "2020-08-23T16:27:57Z",
    "created_at": "2020-08-09T16:36:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Deprecate import of classes Sigma, Euler_Phi, Moebius to the global namespace",
    "type": "issue",
    "updated_at": "2020-08-23T16:27:57Z",
    "url": "https://github.com/sagemath/sage/issues/30322",
    "user": "https://github.com/mkoeppe"
}
```
They are classes that implement the global functions `sigma`, `euler_phi`, `moebius`. There is no reason why the classes also need to be in the global namespace.

In this ticket, we deprecate the import.


CC:  @DaveWitteMorris @slel @fchapoton

Branch/Commit: **[4b8d207](https://github.com/sagemath/sagetrac-mirror/commit/4b8d207c9ab98019c8cfe74d74fbc9226aa70d6c)**

Reviewer: **Markus Wageringel**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/30322_





---

archive/issue_events_399474.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-09T16:36:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399474"
}
```



---

archive/issue_events_399475.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-09T16:36:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399475"
}
```



---

archive/issue_events_399476.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-09T16:36:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399476"
}
```



---

archive/issue_events_399477.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-09T16:36:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399477"
}
```



---

archive/issue_comments_484244.json:
```json
{
    "body": "Branch: **[u/mkoeppe/deprecate_import_of_classes_sigma__euler_phi__moebius_to_the_global_namespace](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/deprecate_import_of_classes_sigma__euler_phi__moebius_to_the_global_namespace)**",
    "created_at": "2020-08-09T16:42:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484244",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/deprecate_import_of_classes_sigma__euler_phi__moebius_to_the_global_namespace](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/deprecate_import_of_classes_sigma__euler_phi__moebius_to_the_global_namespace)**



---

archive/issue_comments_484245.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c1ad4e2d28dbb47154e93ecd96da943a887b88be\">c1ad4e2</a></td><td><code>sage.arith.all: Deprecate import of classes Sigma, Euler_Phi, Moebius to the global namespace</code></td></tr></table>\n",
    "created_at": "2020-08-09T16:42:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484245",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c1ad4e2d28dbb47154e93ecd96da943a887b88be">c1ad4e2</a></td><td><code>sage.arith.all: Deprecate import of classes Sigma, Euler_Phi, Moebius to the global namespace</code></td></tr></table>




---

archive/issue_comments_484246.json:
```json
{
    "body": "Commit: **[c1ad4e2](https://github.com/sagemath/sagetrac-mirror/commit/c1ad4e2d28dbb47154e93ecd96da943a887b88be)**",
    "created_at": "2020-08-09T16:42:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484246",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[c1ad4e2](https://github.com/sagemath/sagetrac-mirror/commit/c1ad4e2d28dbb47154e93ecd96da943a887b88be)**



---

archive/issue_comments_484247.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-08-09T16:42:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484247",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_399478.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-09T16:42:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399478"
}
```



---

archive/issue_events_399479.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-13T18:37:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399479"
}
```



---

archive/issue_events_399480.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-13T18:37:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399480"
}
```



---

archive/issue_comments_484248.json:
```json
{
    "body": "Reviewer: **Markus Wageringel**",
    "created_at": "2020-08-13T18:37:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484248",
    "user": "https://github.com/mwageringel"
}
```

Reviewer: **Markus Wageringel**



---

archive/issue_comments_484249.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nSome doctests are failing.",
    "created_at": "2020-08-13T18:37:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484249",
    "user": "https://github.com/mwageringel"
}
```

<a id='comment:3'>Comment 3:</a>
Some doctests are failing.



---

archive/issue_comments_484250.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4b8d207c9ab98019c8cfe74d74fbc9226aa70d6c\">4b8d207</a></td><td><code>Replace uses of deprecated imports Euler_Phi, Moebius</code></td></tr></table>\n",
    "created_at": "2020-08-13T22:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484250",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4b8d207c9ab98019c8cfe74d74fbc9226aa70d6c">4b8d207</a></td><td><code>Replace uses of deprecated imports Euler_Phi, Moebius</code></td></tr></table>




---

archive/issue_comments_484251.json:
```json
{
    "body": "Changed commit from **[c1ad4e2](https://github.com/sagemath/sagetrac-mirror/commit/c1ad4e2d28dbb47154e93ecd96da943a887b88be)** to **[4b8d207](https://github.com/sagemath/sagetrac-mirror/commit/4b8d207c9ab98019c8cfe74d74fbc9226aa70d6c)**",
    "created_at": "2020-08-13T22:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484251",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c1ad4e2](https://github.com/sagemath/sagetrac-mirror/commit/c1ad4e2d28dbb47154e93ecd96da943a887b88be)** to **[4b8d207](https://github.com/sagemath/sagetrac-mirror/commit/4b8d207c9ab98019c8cfe74d74fbc9226aa70d6c)**



---

archive/issue_events_399481.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T22:00:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399481"
}
```



---

archive/issue_events_399482.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T22:00:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399482"
}
```



---

archive/issue_events_399483.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-14T08:34:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399483"
}
```



---

archive/issue_events_399484.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-14T08:34:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399484"
}
```



---

archive/issue_comments_484252.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThanks!",
    "created_at": "2020-08-14T11:40:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484252",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'>Comment 7:</a>
Thanks!



---

archive/issue_comments_484253.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/deprecate_import_of_classes_sigma__euler_phi__moebius_to_the_global_namespace](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/deprecate_import_of_classes_sigma__euler_phi__moebius_to_the_global_namespace)** to **[4b8d207](https://github.com/sagemath/sagetrac-mirror/commit/4b8d207c9ab98019c8cfe74d74fbc9226aa70d6c)**",
    "created_at": "2020-08-23T16:27:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30322#issuecomment-484253",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/deprecate_import_of_classes_sigma__euler_phi__moebius_to_the_global_namespace](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/deprecate_import_of_classes_sigma__euler_phi__moebius_to_the_global_namespace)** to **[4b8d207](https://github.com/sagemath/sagetrac-mirror/commit/4b8d207c9ab98019c8cfe74d74fbc9226aa70d6c)**



---

archive/issue_events_399485.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-08-23T16:27:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399485"
}
```



---

archive/issue_events_399486.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9746d70e4a18caa1804932018be7f5ef5d1c2356",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-08-23T16:27:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30322",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30322#event-399486"
}
```
