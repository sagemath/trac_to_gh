# Issue 30458: Outsource some functions in bit_vector_operations.cc

archive/issues_030221.json:
```json
{
    "assignees": [],
    "body": "We simplify obtaining the inclusion maximal faces in the function\n`get_next_level` by outsourcing to new function `is_contained_in_one`.\n\nIn addition we implement `contains_one` for #30040.\n\nThis ticket merges cleanly with #30435.\n\nKeywords: **code duplication, combinatorial polyhedron**\n\nBranch/Commit: **[5311cdf](https://github.com/sagemath/sagetrac-mirror/commit/5311cdf89f1ae38c4d8b8a458b2071fa4f9f44b9)**\n\nReviewer: **Travis Scrimshaw, Samuel Leli\u00e8vre**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30458_\n\n",
    "closed_at": "2020-11-07T16:23:01Z",
    "created_at": "2020-08-28T14:19:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Outsource some functions in bit_vector_operations.cc",
    "type": "issue",
    "updated_at": "2020-11-07T16:23:01Z",
    "url": "https://github.com/sagemath/sage/issues/30458",
    "user": "https://github.com/kliem"
}
```
We simplify obtaining the inclusion maximal faces in the function
`get_next_level` by outsourcing to new function `is_contained_in_one`.

In addition we implement `contains_one` for #30040.

This ticket merges cleanly with #30435.

Keywords: **code duplication, combinatorial polyhedron**

Branch/Commit: **[5311cdf](https://github.com/sagemath/sagetrac-mirror/commit/5311cdf89f1ae38c4d8b8a458b2071fa4f9f44b9)**

Reviewer: **Travis Scrimshaw, Samuel Lelièvre**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/30458_





---

archive/issue_events_401594.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-08-28T14:19:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401594"
}
```



---

archive/issue_events_401595.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-08-28T14:19:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401595"
}
```



---

archive/issue_events_401596.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-08-28T14:19:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401596"
}
```



---

archive/issue_events_401597.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-08-28T14:19:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401597"
}
```



---

archive/issue_comments_488207.json:
```json
{
    "body": "Branch: **[u/gh-kliem/outsource_inclusion_maximal](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-kliem/outsource_inclusion_maximal)**",
    "created_at": "2020-08-28T14:19:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488207",
    "user": "https://github.com/kliem"
}
```

Branch: **[u/gh-kliem/outsource_inclusion_maximal](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-kliem/outsource_inclusion_maximal)**



---

archive/issue_comments_488208.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/20a39b679f660e6b59ba7db4f7c302ab454fe28b\">20a39b6</a></td><td><code>outsource inclusion maximal</code></td></tr></table>\n",
    "created_at": "2020-08-28T14:19:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488208",
    "user": "https://github.com/kliem"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/20a39b679f660e6b59ba7db4f7c302ab454fe28b">20a39b6</a></td><td><code>outsource inclusion maximal</code></td></tr></table>




---

archive/issue_events_401598.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-08-28T14:19:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401598"
}
```



---

archive/issue_comments_488209.json:
```json
{
    "body": "Commit: **[20a39b6](https://github.com/sagemath/sagetrac-mirror/commit/20a39b679f660e6b59ba7db4f7c302ab454fe28b)**",
    "created_at": "2020-08-28T14:19:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488209",
    "user": "https://github.com/kliem"
}
```

Commit: **[20a39b6](https://github.com/sagemath/sagetrac-mirror/commit/20a39b679f660e6b59ba7db4f7c302ab454fe28b)**



---

archive/issue_comments_488210.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b672fcaf65e2219644166a6ed5d8e29c13b4d67d\">b672fca</a></td><td><code>removed redundant function</code></td></tr></table>\n",
    "created_at": "2020-08-28T14:24:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488210",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b672fcaf65e2219644166a6ed5d8e29c13b4d67d">b672fca</a></td><td><code>removed redundant function</code></td></tr></table>




---

archive/issue_comments_488211.json:
```json
{
    "body": "Changed commit from **[20a39b6](https://github.com/sagemath/sagetrac-mirror/commit/20a39b679f660e6b59ba7db4f7c302ab454fe28b)** to **[b672fca](https://github.com/sagemath/sagetrac-mirror/commit/b672fcaf65e2219644166a6ed5d8e29c13b4d67d)**",
    "created_at": "2020-08-28T14:24:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488211",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[20a39b6](https://github.com/sagemath/sagetrac-mirror/commit/20a39b679f660e6b59ba7db4f7c302ab454fe28b)** to **[b672fca](https://github.com/sagemath/sagetrac-mirror/commit/b672fcaf65e2219644166a6ed5d8e29c13b4d67d)**



---

archive/issue_comments_488212.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-08-30T04:20:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488212",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_401599.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-08-30T04:20:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401599"
}
```



---

archive/issue_events_401600.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-08-30T04:20:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401600"
}
```



---

archive/issue_comments_488213.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2020-08-30T04:20:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488213",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_488214.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThank you.",
    "created_at": "2020-08-30T05:06:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488214",
    "user": "https://github.com/kliem"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thank you.



---

archive/issue_events_401601.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-30T13:27:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401601"
}
```



---

archive/issue_events_401602.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-30T13:27:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401602"
}
```



---

archive/issue_comments_488215.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThe functions `inline int is_contained_in_one` (with or without `skip`)\nhave this fine description:\n\n```\n     Return whether ``face`` is contained in one of ``faces``.\n```\n\nDescriptions for the functions `contains_one` (with or without `skip`)\nhave a problem:\n\n```diff\n inline int contains_one(uint64_t *face, uint64_t **faces, size_t n_faces, size_t face_length){\n     /*\n-    Return whether ``face`` contains in one of ``faces``.\n+    Return whether ``face`` contains one of ``faces``.\n     */\n```\n\n```diff\n inline int contains_one(uint64_t *face, uint64_t **faces, size_t n_faces, size_t face_length, size_t skip){\n     /*\n-    Return whether ``face`` is contains in one of ``faces``.\n+    Return whether ``face`` contains one of ``faces``.\n \n     Skips ``faces[skip]``.\n     */\n```",
    "created_at": "2020-08-30T13:27:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488215",
    "user": "https://github.com/slel"
}
```

<div id="comment:5" align="right">Comment 5</div>

The functions `inline int is_contained_in_one` (with or without `skip`)
have this fine description:

```
     Return whether ``face`` is contained in one of ``faces``.
```

Descriptions for the functions `contains_one` (with or without `skip`)
have a problem:

```diff
 inline int contains_one(uint64_t *face, uint64_t **faces, size_t n_faces, size_t face_length){
     /*
-    Return whether ``face`` contains in one of ``faces``.
+    Return whether ``face`` contains one of ``faces``.
     */
```

```diff
 inline int contains_one(uint64_t *face, uint64_t **faces, size_t n_faces, size_t face_length, size_t skip){
     /*
-    Return whether ``face`` is contains in one of ``faces``.
+    Return whether ``face`` contains one of ``faces``.
 
     Skips ``faces[skip]``.
     */
```



---

archive/issue_comments_488216.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b2ce23e9fafbb530e5c3deaaefb8205cc9b66852\">b2ce23e</a></td><td><code>grammar</code></td></tr></table>\n",
    "created_at": "2020-08-30T14:54:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488216",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b2ce23e9fafbb530e5c3deaaefb8205cc9b66852">b2ce23e</a></td><td><code>grammar</code></td></tr></table>




---

archive/issue_comments_488217.json:
```json
{
    "body": "Changed commit from **[b672fca](https://github.com/sagemath/sagetrac-mirror/commit/b672fcaf65e2219644166a6ed5d8e29c13b4d67d)** to **[b2ce23e](https://github.com/sagemath/sagetrac-mirror/commit/b2ce23e9fafbb530e5c3deaaefb8205cc9b66852)**",
    "created_at": "2020-08-30T14:54:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488217",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b672fca](https://github.com/sagemath/sagetrac-mirror/commit/b672fcaf65e2219644166a6ed5d8e29c13b4d67d)** to **[b2ce23e](https://github.com/sagemath/sagetrac-mirror/commit/b2ce23e9fafbb530e5c3deaaefb8205cc9b66852)**



---

archive/issue_events_401603.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-08-30T14:56:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401603"
}
```



---

archive/issue_events_401604.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-08-30T14:56:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401604"
}
```



---

archive/issue_events_401605.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-30T16:17:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401605"
}
```



---

archive/issue_events_401606.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-30T16:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401606"
}
```



---

archive/issue_comments_488218.json:
```json
{
    "body": "Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Samuel Leli\u00e8vre**",
    "created_at": "2020-08-30T16:17:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488218",
    "user": "https://github.com/slel"
}
```

Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Samuel Lelièvre**



---

archive/issue_comments_488219.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThanks again.",
    "created_at": "2020-08-30T16:32:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488219",
    "user": "https://github.com/kliem"
}
```

<div id="comment:9" align="right">Comment 9</div>

Thanks again.



---

archive/issue_comments_488220.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7c970e3dab865b1df378e046f8da86cd09de5ae0\">7c970e3</a></td><td><code>outsource inclusion maximal</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/92fb29c1ebe5ab9728f9ce68c7d3a89178cc0fc3\">92fb29c</a></td><td><code>removed redundant function</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5311cdf89f1ae38c4d8b8a458b2071fa4f9f44b9\">5311cdf</a></td><td><code>grammar</code></td></tr></table>\n",
    "created_at": "2020-09-09T08:02:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488220",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7c970e3dab865b1df378e046f8da86cd09de5ae0">7c970e3</a></td><td><code>outsource inclusion maximal</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/92fb29c1ebe5ab9728f9ce68c7d3a89178cc0fc3">92fb29c</a></td><td><code>removed redundant function</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5311cdf89f1ae38c4d8b8a458b2071fa4f9f44b9">5311cdf</a></td><td><code>grammar</code></td></tr></table>




---

archive/issue_events_401607.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2020-09-09T08:02:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401607"
}
```



---

archive/issue_events_401608.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2020-09-09T08:02:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401608"
}
```



---

archive/issue_comments_488221.json:
```json
{
    "body": "Changed commit from **[b2ce23e](https://github.com/sagemath/sagetrac-mirror/commit/b2ce23e9fafbb530e5c3deaaefb8205cc9b66852)** to **[5311cdf](https://github.com/sagemath/sagetrac-mirror/commit/5311cdf89f1ae38c4d8b8a458b2071fa4f9f44b9)**",
    "created_at": "2020-09-09T08:02:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488221",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b2ce23e](https://github.com/sagemath/sagetrac-mirror/commit/b2ce23e9fafbb530e5c3deaaefb8205cc9b66852)** to **[5311cdf](https://github.com/sagemath/sagetrac-mirror/commit/5311cdf89f1ae38c4d8b8a458b2071fa4f9f44b9)**



---

archive/issue_events_401609.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-09-09T08:03:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401609"
}
```



---

archive/issue_events_401610.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-09-09T08:03:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401610"
}
```



---

archive/issue_comments_488222.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nRebased.",
    "created_at": "2020-09-09T08:03:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488222",
    "user": "https://github.com/kliem"
}
```

<div id="comment:11" align="right">Comment 11</div>

Rebased.



---

archive/issue_events_401611.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401611"
}
```



---

archive/issue_events_401612.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401612"
}
```



---

archive/issue_comments_488223.json:
```json
{
    "body": "Changed branch from **[u/gh-kliem/outsource_inclusion_maximal](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-kliem/outsource_inclusion_maximal)** to **[5311cdf](https://github.com/sagemath/sagetrac-mirror/commit/5311cdf89f1ae38c4d8b8a458b2071fa4f9f44b9)**",
    "created_at": "2020-11-07T16:23:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30458#issuecomment-488223",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-kliem/outsource_inclusion_maximal](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-kliem/outsource_inclusion_maximal)** to **[5311cdf](https://github.com/sagemath/sagetrac-mirror/commit/5311cdf89f1ae38c4d8b8a458b2071fa4f9f44b9)**



---

archive/issue_events_401613.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-11-07T16:23:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401613"
}
```



---

archive/issue_events_401614.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b74b62e57ed766aa31701ea626141f5acda6384e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-11-07T16:23:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30458",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30458#event-401614"
}
```
