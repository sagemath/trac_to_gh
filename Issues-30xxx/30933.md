# Issue 30933: GH Actions: Repair upload of docker images to docker.pkg.github.com

archive/issues_030696.json:
```json
{
    "assignees": [],
    "body": "These docker images are built by the portability CI workflows for Linux.\n\nHowever, refactoring of the workflow had broken the upload to `docker.pkg.github.com`. (Although the login to `docker.pkg.github.com` succeeds, the push fails with `no basic auth credentials` - see https://github.com/mkoeppe/sage/runs/1414148202?check_suite_focus=true)\n\n\n\n\n\nCC:  @kliem @dimpase @tobiasdiez\n\nBranch/Commit: **[21afe51](https://github.com/sagemath/sagetrac-mirror/commit/21afe511f1e903647102127204f47c2d29e04fef)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30933_\n\n",
    "closed_at": "2022-01-30T11:38:05Z",
    "created_at": "2020-11-17T21:40:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "GH Actions: Repair upload of docker images to docker.pkg.github.com",
    "type": "issue",
    "updated_at": "2022-01-30T11:38:05Z",
    "url": "https://github.com/sagemath/sage/issues/30933",
    "user": "https://github.com/mkoeppe"
}
```
These docker images are built by the portability CI workflows for Linux.

However, refactoring of the workflow had broken the upload to `docker.pkg.github.com`. (Although the login to `docker.pkg.github.com` succeeds, the push fails with `no basic auth credentials` - see https://github.com/mkoeppe/sage/runs/1414148202?check_suite_focus=true)





CC:  @kliem @dimpase @tobiasdiez

Branch/Commit: **[21afe51](https://github.com/sagemath/sagetrac-mirror/commit/21afe511f1e903647102127204f47c2d29e04fef)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/30933_





---

archive/issue_events_408367.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-17T21:40:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408367"
}
```



---

archive/issue_events_408368.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-17T21:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408368"
}
```



---

archive/issue_events_408369.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-17T21:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408369"
}
```



---

archive/issue_events_408370.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-17T21:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408370"
}
```



---

archive/issue_comments_498636.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis is possibly because `tox` does not pass on `$HOME` - and `docker login` stores the password in `/home/runner/.docker/config.json`.",
    "created_at": "2020-11-17T21:45:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498636",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

This is possibly because `tox` does not pass on `$HOME` - and `docker login` stores the password in `/home/runner/.docker/config.json`.



---

archive/issue_comments_498637.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\n`docker login` recommends to configure a credential helper - \nhttps://docs.docker.com/engine/reference/commandline/login/#credentials-store",
    "created_at": "2020-11-17T21:47:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498637",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

`docker login` recommends to configure a credential helper - 
https://docs.docker.com/engine/reference/commandline/login/#credentials-store



---

archive/issue_comments_498638.json:
```json
{
    "body": "Branch: **[u/mkoeppe/gh_actions__repair_upload_of_docker_images](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/gh_actions__repair_upload_of_docker_images)**",
    "created_at": "2020-11-18T06:15:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498638",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/gh_actions__repair_upload_of_docker_images](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/gh_actions__repair_upload_of_docker_images)**



---

archive/issue_comments_498639.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-11-18T06:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498639",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_498640.json:
```json
{
    "body": "Commit: **[1bb3b33](https://github.com/sagemath/sagetrac-mirror/commit/1bb3b3349c751dbeb0d376b29a45c1c55088df85)**",
    "created_at": "2020-11-18T06:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498640",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[1bb3b33](https://github.com/sagemath/sagetrac-mirror/commit/1bb3b3349c751dbeb0d376b29a45c1c55088df85)**



---

archive/issue_comments_498641.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e8afd96ab2c466ecd433c9170e49ee7ec248a1fd\">e8afd96</a></td><td><code>.github/workflows/tox*.yml: Use new method to set environment variable for next GH Actions steps</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1bb3b3349c751dbeb0d376b29a45c1c55088df85\">1bb3b33</a></td><td><code>.github/workflows/tox*.yml: Install pass for docker push</code></td></tr></table>\n",
    "created_at": "2020-11-18T06:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498641",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e8afd96ab2c466ecd433c9170e49ee7ec248a1fd">e8afd96</a></td><td><code>.github/workflows/tox*.yml: Use new method to set environment variable for next GH Actions steps</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1bb3b3349c751dbeb0d376b29a45c1c55088df85">1bb3b33</a></td><td><code>.github/workflows/tox*.yml: Install pass for docker push</code></td></tr></table>




---

archive/issue_comments_498642.json:
```json
{
    "body": "Reviewer: **https://github.com/mkoeppe/sage/actions/runs/369629393**",
    "created_at": "2020-11-18T06:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498642",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **https://github.com/mkoeppe/sage/actions/runs/369629393**



---

archive/issue_comments_498643.json:
```json
{
    "body": "Changed reviewer from **https://github.com/mkoeppe/sage/actions/runs/369629393** to none",
    "created_at": "2020-11-18T06:26:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498643",
    "user": "https://github.com/mkoeppe"
}
```

Changed reviewer from **https://github.com/mkoeppe/sage/actions/runs/369629393** to none



---

archive/issue_events_408371.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408371"
}
```



---

archive/issue_events_408372.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408372"
}
```



---

archive/issue_comments_498644.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498644",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_408373.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408373"
}
```



---

archive/issue_events_408374.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408374"
}
```



---

archive/issue_events_408375.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-27T01:03:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408375"
}
```



---

archive/issue_events_408376.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-27T01:03:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408376"
}
```



---

archive/issue_events_408377.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-31T21:18:36Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "title_is": "GH Actions: Repair upload of docker images to docker.pkg.github.com",
    "title_was": "GH Actions: Repair upload of docker images",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408377"
}
```



---

archive/issue_comments_498645.json:
```json
{
    "body": "Changed commit from **[1bb3b33](https://github.com/sagemath/sagetrac-mirror/commit/1bb3b3349c751dbeb0d376b29a45c1c55088df85)** to **[0644259](https://github.com/sagemath/sagetrac-mirror/commit/064425907b0d7f909a779420062c83923008b808)**",
    "created_at": "2021-12-31T21:49:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498645",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1bb3b33](https://github.com/sagemath/sagetrac-mirror/commit/1bb3b3349c751dbeb0d376b29a45c1c55088df85)** to **[0644259](https://github.com/sagemath/sagetrac-mirror/commit/064425907b0d7f909a779420062c83923008b808)**



---

archive/issue_comments_498646.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/064425907b0d7f909a779420062c83923008b808\">0644259</a></td><td><code>.github/workflows/tox*.yml, tox.ini: Pass ddocker.pkg.github.com credentials via DOCKER_CONFIG_FILE</code></td></tr></table>\n",
    "created_at": "2021-12-31T21:49:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498646",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/064425907b0d7f909a779420062c83923008b808">0644259</a></td><td><code>.github/workflows/tox*.yml, tox.ini: Pass ddocker.pkg.github.com credentials via DOCKER_CONFIG_FILE</code></td></tr></table>




---

archive/issue_comments_498647.json:
```json
{
    "body": "Changed dependencies from **#30733** to none",
    "created_at": "2021-12-31T21:56:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498647",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#30733** to none



---

archive/issue_events_408378.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-31T21:56:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408378"
}
```



---

archive/issue_events_408379.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-31T21:56:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408379"
}
```



---

archive/issue_events_408380.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-31T21:56:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408380"
}
```



---

archive/issue_comments_498648.json:
```json
{
    "body": "Reviewer: **https://github.com/mkoeppe/sage/actions/runs/1641828444**",
    "created_at": "2021-12-31T21:56:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498648",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **https://github.com/mkoeppe/sage/actions/runs/1641828444**



---

archive/issue_comments_498649.json:
```json
{
    "body": "Changed commit from **[0644259](https://github.com/sagemath/sagetrac-mirror/commit/064425907b0d7f909a779420062c83923008b808)** to **[21afe51](https://github.com/sagemath/sagetrac-mirror/commit/21afe511f1e903647102127204f47c2d29e04fef)**",
    "created_at": "2021-12-31T23:53:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498649",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0644259](https://github.com/sagemath/sagetrac-mirror/commit/064425907b0d7f909a779420062c83923008b808)** to **[21afe51](https://github.com/sagemath/sagetrac-mirror/commit/21afe511f1e903647102127204f47c2d29e04fef)**



---

archive/issue_comments_498650.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/21afe511f1e903647102127204f47c2d29e04fef\">21afe51</a></td><td><code>.github/workflows/tox*.yml, tox.ini: Pass ddocker.pkg.github.com credentials via DOCKER_CONFIG_FILE</code></td></tr></table>\n",
    "created_at": "2021-12-31T23:53:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498650",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/21afe511f1e903647102127204f47c2d29e04fef">21afe51</a></td><td><code>.github/workflows/tox*.yml, tox.ini: Pass ddocker.pkg.github.com credentials via DOCKER_CONFIG_FILE</code></td></tr></table>




---

archive/issue_comments_498651.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,7 @@\n-Even after #30733, although the login to `docker.pkg.github.com` succeeds, the push fails with `no basic auth credentials` - see https://github.com/mkoeppe/sage/runs/1414148202?check_suite_focus=true\n+These docker images are built by the portability CI workflows for Linux.\n+\n+However, refactoring of the workflow had broken the upload to `docker.pkg.github.com`. (Although the login to `docker.pkg.github.com` succeeds, the push fails with `no basic auth credentials` - see https://github.com/mkoeppe/sage/runs/1414148202?check_suite_focus=true)\n \n \n+\n+\n``````\n",
    "created_at": "2022-01-01T00:05:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498651",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,7 @@
-Even after #30733, although the login to `docker.pkg.github.com` succeeds, the push fails with `no basic auth credentials` - see https://github.com/mkoeppe/sage/runs/1414148202?check_suite_focus=true
+These docker images are built by the portability CI workflows for Linux.
+
+However, refactoring of the workflow had broken the upload to `docker.pkg.github.com`. (Although the login to `docker.pkg.github.com` succeeds, the push fails with `no basic auth credentials` - see https://github.com/mkoeppe/sage/runs/1414148202?check_suite_focus=true)
 
 
+
+
``````




---

archive/issue_comments_498652.json:
```json
{
    "body": "Changed reviewer from **https://github.com/mkoeppe/sage/actions/runs/1641828444** to **https://github.com/mkoeppe/sage/runs/4676511083?check_suite_focus=true**",
    "created_at": "2022-01-01T00:05:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498652",
    "user": "https://github.com/mkoeppe"
}
```

Changed reviewer from **https://github.com/mkoeppe/sage/actions/runs/1641828444** to **https://github.com/mkoeppe/sage/runs/4676511083?check_suite_focus=true**



---

archive/issue_comments_498653.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nThis fixes the problem.\n\nFor example, https://github.com/mkoeppe/sage/runs/4676511083?check_suite_focus=true has just uploaded the image https://github.com/mkoeppe/sage/pkgs/container/sage%2Fsage-docker-ubuntu-trusty-minimal-configured",
    "created_at": "2022-01-01T00:05:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498653",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:13" align="right">Comment 13</div>

This fixes the problem.

For example, https://github.com/mkoeppe/sage/runs/4676511083?check_suite_focus=true has just uploaded the image https://github.com/mkoeppe/sage/pkgs/container/sage%2Fsage-docker-ubuntu-trusty-minimal-configured



---

archive/issue_comments_498654.json:
```json
{
    "body": "Changed reviewer from **https://github.com/mkoeppe/sage/runs/4676511083?check_suite_focus=true** to **Dima Pasechnik**",
    "created_at": "2022-01-01T00:43:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498654",
    "user": "https://github.com/dimpase"
}
```

Changed reviewer from **https://github.com/mkoeppe/sage/runs/4676511083?check_suite_focus=true** to **Dima Pasechnik**



---

archive/issue_events_408381.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-01-01T00:43:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408381"
}
```



---

archive/issue_events_408382.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-01-01T00:43:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408382"
}
```



---

archive/issue_comments_498655.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nOK, Happy New Year :-)",
    "created_at": "2022-01-01T00:43:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498655",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:14" align="right">Comment 14</div>

OK, Happy New Year :-)



---

archive/issue_comments_498656.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nThanks Dima, happy new year to you too!",
    "created_at": "2022-01-01T01:05:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498656",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:15" align="right">Comment 15</div>

Thanks Dima, happy new year to you too!



---

archive/issue_events_408383.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-13T18:56:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408383"
}
```



---

archive/issue_events_408384.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-13T18:56:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408384"
}
```



---

archive/issue_events_408385.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-27T21:48:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408385"
}
```



---

archive/issue_events_408386.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-01-27T21:48:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408386"
}
```



---

archive/issue_events_408387.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-01-30T11:38:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408387"
}
```



---

archive/issue_events_408388.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fb599d00f37dc36e8b4c0485d62a3227b0cc1ed0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-01-30T11:38:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30933#event-408388"
}
```



---

archive/issue_comments_498657.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/gh_actions__repair_upload_of_docker_images](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/gh_actions__repair_upload_of_docker_images)** to **[21afe51](https://github.com/sagemath/sagetrac-mirror/commit/21afe511f1e903647102127204f47c2d29e04fef)**",
    "created_at": "2022-01-30T11:38:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30933#issuecomment-498657",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/gh_actions__repair_upload_of_docker_images](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/gh_actions__repair_upload_of_docker_images)** to **[21afe51](https://github.com/sagemath/sagetrac-mirror/commit/21afe511f1e903647102127204f47c2d29e04fef)**
