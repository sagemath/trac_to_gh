...
<a id='comment:9'></a>Replying to [strogdon](#comment%3A7):
> With these changes
> 
> ```
> #!diff
...
<a id='comment:17'></a>Replying to [gh-Ivo-Maffei](#comment%3A16):
> Replying to [dimpase](#comment%3A15):
> > there is no need to construct the incidence graph. In the regular+uniform  case, i.e. if (s,t) are defined, just check that the collinearity graph (aka block graph) is s.r.g. with the correct parameters.
...
<a id='comment:22'></a>Replying to [strogdon](#comment%3A21):
...
<a id='comment:25'></a>Replying to [strogdon](#comment%3A7):
> With these changes
> 
> ```
> #!diff
...
<a id='comment:28'></a>Replying to [chapoton](#comment%3A27):
> I have added a commit on top of the public branch, just adding the 3 suggested known bug tags 
...
<a id='comment:31'></a>Replying to [gh-Ivo-Maffei](#comment%3A30):
> Replying to [strogdon](#comment%3A28):
> > Is the `def test()` function needed?
