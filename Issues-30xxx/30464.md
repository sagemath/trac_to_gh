# Issue 30464: Repair "sage -docbuild"

archive/issues_030227.json:
```json
{
    "assignees": [],
    "body": "\n```\n./sage -docbuild file=src/sage/geometry/polyhedron/base.py html\nError building the documentation.\n\n\n    base_dir = os.path.join(DOT_SAGE, 'docbuild', module_name)\nNameError: name 'DOT_SAGE' is not defined\n```\nThis was probably broken in #29111 \n\nCC:  @jhpalmieri @kiwifb @tscrim @dimpase\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30464_\n\n",
    "closed_at": "2020-08-29T06:22:57Z",
    "created_at": "2020-08-29T00:38:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Repair \"sage -docbuild\"",
    "type": "issue",
    "updated_at": "2020-08-29T06:22:57Z",
    "url": "https://github.com/sagemath/sage/issues/30464",
    "user": "https://github.com/mkoeppe"
}
```

```
./sage -docbuild file=src/sage/geometry/polyhedron/base.py html
Error building the documentation.


    base_dir = os.path.join(DOT_SAGE, 'docbuild', module_name)
NameError: name 'DOT_SAGE' is not defined
```
This was probably broken in #29111 

CC:  @jhpalmieri @kiwifb @tscrim @dimpase

_Issue created by migration from https://trac.sagemath.org/ticket/30464_





---

archive/issue_events_401683.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T00:38:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401683"
}
```



---

archive/issue_events_401684.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T00:38:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401684"
}
```



---

archive/issue_events_401685.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T00:38:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401685"
}
```



---

archive/issue_events_401686.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T00:38:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401686"
}
```



---

archive/issue_comments_488373.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nSorry, this is a dup of #29651",
    "created_at": "2020-08-29T00:40:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30464#issuecomment-488373",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

Sorry, this is a dup of #29651



---

archive/issue_events_401687.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T00:40:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401687"
}
```



---

archive/issue_events_401688.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T00:40:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401688"
}
```



---

archive/issue_events_401689.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T00:40:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401689"
}
```



---

archive/issue_events_401690.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T00:40:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401690"
}
```



---

archive/issue_events_401691.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-29T06:22:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401691"
}
```



---

archive/issue_events_401692.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-29T06:22:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401692"
}
```



---

archive/issue_events_401693.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-29T06:22:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30464#event-401693"
}
```



---

archive/issue_comments_488374.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nalso broken on linux, by the way",
    "created_at": "2020-08-29T06:22:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30464#issuecomment-488374",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

also broken on linux, by the way
