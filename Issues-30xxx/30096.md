# Issue 30096: sage.tensor.modules: Add backends using TensorFlow Core, PyTorch, SymPy

archive/issues_029859.json:
```json
{
    "assignees": [],
    "body": "Tensors from `sage.tensor` are stored as `sage.tensor.modules.Components`, which is a dictionary with index tuples as keys.\n\nWe propose to create additional backends for numerical coordinate rings:\n- https://www.tensorflow.org/guide/tensor\n- https://pytorch.org/docs/stable/tensors.html\n\nThey provide efficient storage and GPU-accelerated computations for numerical tensors.\n\nFor SymPy, see #31946\n\n\nCC:  @egourgoulhon\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30096_\n\n",
    "created_at": "2020-07-09T02:16:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.tensor.modules: Add backends using TensorFlow Core, PyTorch, SymPy",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/30096",
    "user": "https://github.com/mkoeppe"
}
```
Tensors from `sage.tensor` are stored as `sage.tensor.modules.Components`, which is a dictionary with index tuples as keys.

We propose to create additional backends for numerical coordinate rings:
- https://www.tensorflow.org/guide/tensor
- https://pytorch.org/docs/stable/tensors.html

They provide efficient storage and GPU-accelerated computations for numerical tensors.

For SymPy, see #31946


CC:  @egourgoulhon

_Issue created by migration from https://trac.sagemath.org/ticket/30096_





---

archive/issue_events_396185.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-09T02:16:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396185"
}
```



---

archive/issue_events_396186.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-09T02:16:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396186"
}
```



---

archive/issue_events_396187.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-09T02:16:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396187"
}
```



---

archive/issue_events_396188.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-09T02:16:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396188"
}
```



---

archive/issue_events_396189.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396189"
}
```



---

archive/issue_events_396190.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396190"
}
```



---

archive/issue_events_396191.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396191"
}
```



---

archive/issue_events_396192.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396192"
}
```



---

archive/issue_comments_479491.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30096#issuecomment-479491",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_comments_479492.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,9 +1,10 @@\n Tensors from `sage.tensor` are stored as `sage.tensor.modules.Components`, which is a dictionary with index tuples as keys.\n \n-We propose to create additional backends.\n+We propose to create additional backends for numerical coordinate rings:\n - https://www.tensorflow.org/guide/tensor\n - https://pytorch.org/docs/stable/tensors.html\n \n-They provide efficient storage and GPU-accelerated computations.\n+They provide efficient storage and GPU-accelerated computations for numerical tensors.\n \n+For SymPy, see #31946\n \n``````\n",
    "created_at": "2021-06-10T03:37:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30096#issuecomment-479492",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,9 +1,10 @@
 Tensors from `sage.tensor` are stored as `sage.tensor.modules.Components`, which is a dictionary with index tuples as keys.
 
-We propose to create additional backends.
+We propose to create additional backends for numerical coordinate rings:
 - https://www.tensorflow.org/guide/tensor
 - https://pytorch.org/docs/stable/tensors.html
 
-They provide efficient storage and GPU-accelerated computations.
+They provide efficient storage and GPU-accelerated computations for numerical tensors.
 
+For SymPy, see #31946
 
``````




---

archive/issue_events_396193.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-10T03:37:00Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "title_is": "sage.tensor.modules: Add backends using TensorFlow Core, PyTorch, SymPy",
    "title_was": "sage.tensor.modules: Add backends using TensorFlow Core and PyTorch",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396193"
}
```



---

archive/issue_events_396194.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396194"
}
```



---

archive/issue_events_396195.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396195"
}
```



---

archive/issue_events_396196.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396196"
}
```



---

archive/issue_events_396197.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396197"
}
```



---

archive/issue_events_396198.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396198"
}
```



---

archive/issue_events_396199.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396199"
}
```



---

archive/issue_events_396200.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396200"
}
```



---

archive/issue_events_396201.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396201"
}
```



---

archive/issue_events_396202.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396202"
}
```



---

archive/issue_events_396203.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30096",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30096#event-396203"
}
```
