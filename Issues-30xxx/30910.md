# Issue 30910: GH Actions: Fixes for WSL workflow

archive/issues_030673.json:
```json
{
    "assignees": [],
    "body": "Like the other workflows, it now runs only on tags and pushes to branch having \"wsl\" in the name (to ease testing if the workflow has to be changed).\n\nCurrently it dies if a system package does not exist - even for optional packages.\nThis is fixed with this ticket.\nhttps://github.com/tobiasdiez/sage/runs/1405834030?check_suite_focus=true\n\n\n\n\nDepends on #30923\n\nCC:  @tobiasdiez\n\nBranch/Commit: **[3ccae02](https://github.com/sagemath/sagetrac-mirror/commit/3ccae0258309b682d6429cafbc656e90894d6ebe)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Tobias Diez**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30910_\n\n",
    "closed_at": "2020-11-29T11:58:03Z",
    "created_at": "2020-11-13T21:15:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "GH Actions: Fixes for WSL workflow",
    "type": "issue",
    "updated_at": "2020-11-29T11:58:03Z",
    "url": "https://github.com/sagemath/sage/issues/30910",
    "user": "https://github.com/mkoeppe"
}
```
Like the other workflows, it now runs only on tags and pushes to branch having "wsl" in the name (to ease testing if the workflow has to be changed).

Currently it dies if a system package does not exist - even for optional packages.
This is fixed with this ticket.
https://github.com/tobiasdiez/sage/runs/1405834030?check_suite_focus=true




Depends on #30923

CC:  @tobiasdiez

Branch/Commit: **[3ccae02](https://github.com/sagemath/sagetrac-mirror/commit/3ccae0258309b682d6429cafbc656e90894d6ebe)**

Reviewer: **Matthias Koeppe**

Author: **Tobias Diez**

_Issue created by migration from https://trac.sagemath.org/ticket/30910_





---

archive/issue_events_408059.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-13T21:15:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408059"
}
```



---

archive/issue_events_408060.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-13T21:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408060"
}
```



---

archive/issue_events_408061.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-13T21:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408061"
}
```



---

archive/issue_events_408062.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-13T21:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408062"
}
```



---

archive/issue_events_408063.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-16T10:52:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408063"
}
```



---

archive/issue_events_408064.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-16T10:52:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408064"
}
```



---

archive/issue_comments_498041.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/942149ee76a97628046042b7255a2d31630e7721\">942149e</a></td><td><code>Add confirmation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/75ecd11e878e34a4c508d2f983b6d7527e689f94\">75ecd11</a></td><td><code>tox.ini (local-sudo): Also use ...-bootstrap.txt</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0dc79eb4b542a11aa8c226ca8cf9f0e2306698eb\">0dc79eb</a></td><td><code>build/bin/sage-print-system-package-command: Handle --no-install-recommends, --yes for systems for which write-dockerfile.sh knows these flags</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/050dcb8275b3a7e0db4c7ae3503428107bb5bf17\">050dcb8</a></td><td><code>tox.ini (local-sudo): Use --yes --no-install-recommends</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c8fbe0beaec3a404d330aaebdf37186e9c6becec\">c8fbe0b</a></td><td><code>tox.ini (local-sudo): Ignore errors when IGNORE_MISSING_SYSTEM_PACKAGES=yes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ff34897cac35061afd348cc08a6559f95f406f14\">ff34897</a></td><td><code>tox.ini (local): Guess the package system if it is not provided as a factor</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e27ff19a5969919ceb43fe910fc8d43cfb0c7bf0\">e27ff19</a></td><td><code>Merge branch 'u/mkoeppe/tox_ini__add_environments_local_sudo_ubuntu_standard__etc_' of git://trac.sagemath.org/sage into public/build/fixwsl</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6ff45c41ce97531cc845456941a982c9b583d5f7\">6ff45c4</a></td><td><code>Cleanup workflow</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/53481440a24a3914430dcbf1cf4e19e4aab5267c\">5348144</a></td><td><code>Replace texlive-generic-extra</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0fd029f1e2b1e8384d14995c361f90e331b784b9\">0fd029f</a></td><td><code>Narrow down events that trigger the workflow</code></td></tr></table>\n",
    "created_at": "2020-11-16T10:52:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498041",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:1" align="right">Comment 1</div>

Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/942149ee76a97628046042b7255a2d31630e7721">942149e</a></td><td><code>Add confirmation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/75ecd11e878e34a4c508d2f983b6d7527e689f94">75ecd11</a></td><td><code>tox.ini (local-sudo): Also use ...-bootstrap.txt</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0dc79eb4b542a11aa8c226ca8cf9f0e2306698eb">0dc79eb</a></td><td><code>build/bin/sage-print-system-package-command: Handle --no-install-recommends, --yes for systems for which write-dockerfile.sh knows these flags</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/050dcb8275b3a7e0db4c7ae3503428107bb5bf17">050dcb8</a></td><td><code>tox.ini (local-sudo): Use --yes --no-install-recommends</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c8fbe0beaec3a404d330aaebdf37186e9c6becec">c8fbe0b</a></td><td><code>tox.ini (local-sudo): Ignore errors when IGNORE_MISSING_SYSTEM_PACKAGES=yes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ff34897cac35061afd348cc08a6559f95f406f14">ff34897</a></td><td><code>tox.ini (local): Guess the package system if it is not provided as a factor</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e27ff19a5969919ceb43fe910fc8d43cfb0c7bf0">e27ff19</a></td><td><code>Merge branch 'u/mkoeppe/tox_ini__add_environments_local_sudo_ubuntu_standard__etc_' of git://trac.sagemath.org/sage into public/build/fixwsl</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6ff45c41ce97531cc845456941a982c9b583d5f7">6ff45c4</a></td><td><code>Cleanup workflow</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/53481440a24a3914430dcbf1cf4e19e4aab5267c">5348144</a></td><td><code>Replace texlive-generic-extra</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0fd029f1e2b1e8384d14995c361f90e331b784b9">0fd029f</a></td><td><code>Narrow down events that trigger the workflow</code></td></tr></table>




---

archive/issue_comments_498042.json:
```json
{
    "body": "Commit: **[0fd029f](https://github.com/sagemath/sagetrac-mirror/commit/0fd029f1e2b1e8384d14995c361f90e331b784b9)**",
    "created_at": "2020-11-16T10:52:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498042",
    "user": "https://github.com/tobiasdiez"
}
```

Commit: **[0fd029f](https://github.com/sagemath/sagetrac-mirror/commit/0fd029f1e2b1e8384d14995c361f90e331b784b9)**



---

archive/issue_comments_498043.json:
```json
{
    "body": "Branch: **[public/build/fixwsl](https://github.com/sagemath/sagetrac-mirror/tree/public/build/fixwsl)**",
    "created_at": "2020-11-16T10:52:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498043",
    "user": "https://github.com/tobiasdiez"
}
```

Branch: **[public/build/fixwsl](https://github.com/sagemath/sagetrac-mirror/tree/public/build/fixwsl)**



---

archive/issue_comments_498044.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,8 @@\n-It should only run on tags, like the other workflows\n+Like the other workflows, it now runs only on tags and pushes to branch having \"wsl\" in the name (to ease testing if the workflow has to be changed).\n \n-Currently it dies if a system package does not exist - even for optional packages - https://github.com/sagemath/sage/runs/1368397973?check_suite_focus=true\n+Currently it dies if a system package does not exist - even for optional packages.\n+This is fixed with this ticket.\n+https://github.com/tobiasdiez/sage/runs/1405834030?check_suite_focus=true\n \n \n \n-\n``````\n",
    "created_at": "2020-11-16T10:52:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498044",
    "user": "https://github.com/tobiasdiez"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,8 @@
-It should only run on tags, like the other workflows
+Like the other workflows, it now runs only on tags and pushes to branch having "wsl" in the name (to ease testing if the workflow has to be changed).
 
-Currently it dies if a system package does not exist - even for optional packages - https://github.com/sagemath/sage/runs/1368397973?check_suite_focus=true
+Currently it dies if a system package does not exist - even for optional packages.
+This is fixed with this ticket.
+https://github.com/tobiasdiez/sage/runs/1405834030?check_suite_focus=true
 
 
 
-
``````




---

archive/issue_comments_498045.json:
```json
{
    "body": "Dependencies: **#30923**",
    "created_at": "2020-11-16T10:52:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498045",
    "user": "https://github.com/tobiasdiez"
}
```

Dependencies: **#30923**



---

archive/issue_events_408065.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-16T10:52:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408065"
}
```



---

archive/issue_comments_498046.json:
```json
{
    "body": "Author: **Tobias Diez**",
    "created_at": "2020-11-16T10:52:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498046",
    "user": "https://github.com/tobiasdiez"
}
```

Author: **Tobias Diez**



---

archive/issue_comments_498047.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI don't think `texlive-plain-generic` is the right package. We use latex, not plain tex",
    "created_at": "2020-11-16T20:06:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498047",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

I don't think `texlive-plain-generic` is the right package. We use latex, not plain tex



---

archive/issue_comments_498048.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-11-16T20:07:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498048",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_498049.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI'm also not sure, but looking at https://packages.ubuntu.com/bionic/texlive-generic-extra it looks like `texlive-generic-extra` is a (deprecated) wrapper around `texlive-plain-generic`.\n\n(The dependency might actually be there not for the pdf/latex docs but for the html docs, which requires bibtex to generate the aux.)",
    "created_at": "2020-11-16T20:13:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498049",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:4" align="right">Comment 4</div>

I'm also not sure, but looking at https://packages.ubuntu.com/bionic/texlive-generic-extra it looks like `texlive-generic-extra` is a (deprecated) wrapper around `texlive-plain-generic`.

(The dependency might actually be there not for the pdf/latex docs but for the html docs, which requires bibtex to generate the aux.)



---

archive/issue_comments_498050.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI think I would go for `texlive-latex-extra` - https://packages.ubuntu.com/groovy/texlive-latex-extra",
    "created_at": "2020-11-16T20:17:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498050",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

I think I would go for `texlive-latex-extra` - https://packages.ubuntu.com/groovy/texlive-latex-extra



---

archive/issue_comments_498051.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3ccae0258309b682d6429cafbc656e90894d6ebe\">3ccae02</a></td><td><code>build/pkgs/_recommended/distros/debian.txt: Update latex package</code></td></tr></table>\n",
    "created_at": "2020-11-17T02:27:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498051",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3ccae0258309b682d6429cafbc656e90894d6ebe">3ccae02</a></td><td><code>build/pkgs/_recommended/distros/debian.txt: Update latex package</code></td></tr></table>




---

archive/issue_comments_498052.json:
```json
{
    "body": "Changed commit from **[0fd029f](https://github.com/sagemath/sagetrac-mirror/commit/0fd029f1e2b1e8384d14995c361f90e331b784b9)** to **[3ccae02](https://github.com/sagemath/sagetrac-mirror/commit/3ccae0258309b682d6429cafbc656e90894d6ebe)**",
    "created_at": "2020-11-17T02:27:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498052",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0fd029f](https://github.com/sagemath/sagetrac-mirror/commit/0fd029f1e2b1e8384d14995c361f90e331b784b9)** to **[3ccae02](https://github.com/sagemath/sagetrac-mirror/commit/3ccae0258309b682d6429cafbc656e90894d6ebe)**



---

archive/issue_events_408066.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-17T02:31:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408066"
}
```



---

archive/issue_events_408067.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-11-17T02:31:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408067"
}
```



---

archive/issue_comments_498053.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThanks!",
    "created_at": "2020-11-20T19:41:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498053",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:8" align="right">Comment 8</div>

Thanks!



---

archive/issue_events_408068.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-11-29T11:58:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408068"
}
```



---

archive/issue_events_408069.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "0372dd96333abda072650cd2bcc5afaa584f20f5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-11-29T11:58:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30910#event-408069"
}
```



---

archive/issue_comments_498054.json:
```json
{
    "body": "Changed branch from **[public/build/fixwsl](https://github.com/sagemath/sagetrac-mirror/tree/public/build/fixwsl)** to **[3ccae02](https://github.com/sagemath/sagetrac-mirror/commit/3ccae0258309b682d6429cafbc656e90894d6ebe)**",
    "created_at": "2020-11-29T11:58:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30910#issuecomment-498054",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/build/fixwsl](https://github.com/sagemath/sagetrac-mirror/tree/public/build/fixwsl)** to **[3ccae02](https://github.com/sagemath/sagetrac-mirror/commit/3ccae0258309b682d6429cafbc656e90894d6ebe)**
