# Issue 30344: Update 4ti2

archive/issues_030107.json:
```json
{
    "assignees": [],
    "body": "Previous/related tickets:\n\n- #30887: spkg-configure.m4 for 4ti2\n- #29193: Optional package 4ti2 test failures on Cygwin\n- #19693: Update 4ti2 to version 1.6.7\n- #18325: Support more 4ti2 features\n- #14843: Minor improvements to 4ti2 interface\n- #5489: Add an interface for 4ti2 to Sage\n\n\nCC:  @slel\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30344_\n\n",
    "created_at": "2020-08-12T19:21:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update 4ti2",
    "type": "issue",
    "updated_at": "2022-08-06T19:55:19Z",
    "url": "https://github.com/sagemath/sage/issues/30344",
    "user": "https://github.com/mkoeppe"
}
```
Previous/related tickets:

- #30887: spkg-configure.m4 for 4ti2
- #29193: Optional package 4ti2 test failures on Cygwin
- #19693: Update 4ti2 to version 1.6.7
- #18325: Support more 4ti2 features
- #14843: Minor improvements to 4ti2 interface
- #5489: Add an interface for 4ti2 to Sage


CC:  @slel

_Issue created by migration from https://trac.sagemath.org/ticket/30344_





---

archive/issue_events_399802.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-12T19:21:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399802"
}
```



---

archive/issue_events_399803.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-12T19:21:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399803"
}
```



---

archive/issue_events_399804.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-12T19:21:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399804"
}
```



---

archive/issue_events_399805.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-12T19:21:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399805"
}
```



---

archive/issue_comments_484688.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nTicket #19693, merged in Sage 7.0.beta0, upgraded to 4ti2 1.6.7, released 2015-12-12.\n\nLet us upgrade to 4ti2 1.6.9, released 2018-08-30, the latest release as of 2020-08-13.",
    "created_at": "2020-08-13T20:27:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30344#issuecomment-484688",
    "user": "https://github.com/slel"
}
```

<div id="comment:1" align="right">Comment 1</div>

Ticket #19693, merged in Sage 7.0.beta0, upgraded to 4ti2 1.6.7, released 2015-12-12.

Let us upgrade to 4ti2 1.6.9, released 2018-08-30, the latest release as of 2020-08-13.



---

archive/issue_comments_484689.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nWhat should `upstream_url` be set to in `build/pkgs/4ti2/checksums.ini`?",
    "created_at": "2020-08-19T22:49:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30344#issuecomment-484689",
    "user": "https://github.com/slel"
}
```

<div id="comment:2" align="right">Comment 2</div>

What should `upstream_url` be set to in `build/pkgs/4ti2/checksums.ini`?



---

archive/issue_comments_484690.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI first need to make a new 4ti2 release...",
    "created_at": "2020-08-19T23:04:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30344#issuecomment-484690",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

I first need to make a new 4ti2 release...



---

archive/issue_events_399806.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399806"
}
```



---

archive/issue_events_399807.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399807"
}
```



---

archive/issue_comments_484691.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,3 +10,4 @@\n   #18325\n - #14843 (Minor improvements to 4ti2 interface)\n   #14843\n+- #30887 spkg-configure.m4 for 4ti2\n``````\n",
    "created_at": "2020-11-11T01:25:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30344#issuecomment-484691",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,3 +10,4 @@
   #18325
 - #14843 (Minor improvements to 4ti2 interface)
   #14843
+- #30887 spkg-configure.m4 for 4ti2
``````




---

archive/issue_comments_484692.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30344#issuecomment-484692",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_399808.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399808"
}
```



---

archive/issue_events_399809.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399809"
}
```



---

archive/issue_comments_484693.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,13 +1,9 @@\n+Previous/related tickets:\n \n-Previous/related tickets:\n-- #5489 (Add an interface for 4ti2 to Sage)\n-  #5489\n-- #29193 (Optional package 4ti2 test failures on Cygwin)\n-  #29193\n-- #19693 (Update 4ti2 to version 1.6.7)\n-  #19693\n-- #18325 (Add support for unsupported 4ti2 features to sage.interfaces.four_ti_2) \u2013 Sage\n-  #18325\n-- #14843 (Minor improvements to 4ti2 interface)\n-  #14843\n-- #30887 spkg-configure.m4 for 4ti2\n+- #30887: spkg-configure.m4 for 4ti2\n+- #29193: Optional package 4ti2 test failures on Cygwin\n+- #19693: Update 4ti2 to version 1.6.7\n+- #18325: Support more 4ti2 features\n+- #14843: Minor improvements to 4ti2 interface\n+- #5489: Add an interface for 4ti2 to Sage\n+\n``````\n",
    "created_at": "2021-05-11T12:35:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30344#issuecomment-484693",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,13 +1,9 @@
+Previous/related tickets:
 
-Previous/related tickets:
-- #5489 (Add an interface for 4ti2 to Sage)
-  #5489
-- #29193 (Optional package 4ti2 test failures on Cygwin)
-  #29193
-- #19693 (Update 4ti2 to version 1.6.7)
-  #19693
-- #18325 (Add support for unsupported 4ti2 features to sage.interfaces.four_ti_2) – Sage
-  #18325
-- #14843 (Minor improvements to 4ti2 interface)
-  #14843
-- #30887 spkg-configure.m4 for 4ti2
+- #30887: spkg-configure.m4 for 4ti2
+- #29193: Optional package 4ti2 test failures on Cygwin
+- #19693: Update 4ti2 to version 1.6.7
+- #18325: Support more 4ti2 features
+- #14843: Minor improvements to 4ti2 interface
+- #5489: Add an interface for 4ti2 to Sage
+
``````




---

archive/issue_events_399810.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T02:58:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399810"
}
```



---

archive/issue_events_399811.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T02:58:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399811"
}
```



---

archive/issue_events_399812.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399812"
}
```



---

archive/issue_events_399813.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399813"
}
```



---

archive/issue_events_399814.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399814"
}
```



---

archive/issue_events_399815.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399815"
}
```



---

archive/issue_events_399816.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-06T19:55:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399816"
}
```



---

archive/issue_events_399817.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-06T19:55:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30344",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30344#event-399817"
}
```
