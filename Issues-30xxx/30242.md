# Issue 30242: Make ExtPowerFreeModule a quotient of TensorFreeModule

archive/issues_030005.json:
```json
{
    "assignees": [],
    "body": "(split out from #30169)\n\nWe give `ExtPowerFreeModule` the additional structure of a quotient module of `TensorFreeModule`.\n\nFor degrees 0 and 1 (for which this class is not instantiated by `FiniteRankFreeModule.exterior_power`), taking the quotient is trivial but we still set it up as a quotient for didactical/uniformity/formal reasons.\n\n\n\nCC:  @egourgoulhon @tscrim @mjungmath\n\nBranch/Commit: **[u/mkoeppe/make_extpowerfreemodule_a_quotient_of_tensorfreemodule](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/make_extpowerfreemodule_a_quotient_of_tensorfreemodule) @ [8734151](https://github.com/sagemath/sagetrac-mirror/commit/87341516d539c2f0cd164b2502b939cbd0608b24)**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30242_\n\n",
    "created_at": "2020-07-28T20:05:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make ExtPowerFreeModule a quotient of TensorFreeModule",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/30242",
    "user": "https://github.com/mkoeppe"
}
```
(split out from #30169)

We give `ExtPowerFreeModule` the additional structure of a quotient module of `TensorFreeModule`.

For degrees 0 and 1 (for which this class is not instantiated by `FiniteRankFreeModule.exterior_power`), taking the quotient is trivial but we still set it up as a quotient for didactical/uniformity/formal reasons.



CC:  @egourgoulhon @tscrim @mjungmath

Branch/Commit: **[u/mkoeppe/make_extpowerfreemodule_a_quotient_of_tensorfreemodule](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/make_extpowerfreemodule_a_quotient_of_tensorfreemodule) @ [8734151](https://github.com/sagemath/sagetrac-mirror/commit/87341516d539c2f0cd164b2502b939cbd0608b24)**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/30242_





---

archive/issue_events_398324.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-28T20:05:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398324"
}
```



---

archive/issue_events_398325.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-28T20:05:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398325"
}
```



---

archive/issue_events_398326.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-28T20:05:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398326"
}
```



---

archive/issue_events_398327.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-28T20:05:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398327"
}
```



---

archive/issue_comments_482551.json:
```json
{
    "body": "Branch: **[u/mkoeppe/make_extpowerfreemodule_a_quotient_of_tensorfreemodule](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/make_extpowerfreemodule_a_quotient_of_tensorfreemodule)**",
    "created_at": "2020-07-28T20:23:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482551",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/make_extpowerfreemodule_a_quotient_of_tensorfreemodule](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/make_extpowerfreemodule_a_quotient_of_tensorfreemodule)**



---

archive/issue_comments_482552.json:
```json
{
    "body": "Commit: **[44d6c93](https://github.com/sagemath/sagetrac-mirror/commit/44d6c93e8b0885b7fda016ff90954f29e16367ad)**",
    "created_at": "2020-07-28T23:36:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482552",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[44d6c93](https://github.com/sagemath/sagetrac-mirror/commit/44d6c93e8b0885b7fda016ff90954f29e16367ad)**



---

archive/issue_comments_482553.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0869a10e02936f5b4548f9de4d5f682a60fe92f5\">0869a10</a></td><td><code>ExtPowerFreeModule, ExtPowerDualFreeModule._repr_: Simplify by using the ordinal_str method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/344a7938b625d3b1b723c617569b01049125d653\">344a793</a></td><td><code>ExtPowerFreeModule, ExtPowerDualFreeModule.__init__: Remove duplicate registration of self in the base module, uniqueness check</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/12e7da1eaa179f40abe447a2ecf0299ac5cb2f60\">12e7da1</a></td><td><code>ExtPowerDualFreeModule: Document the 0th power</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/44d6c93e8b0885b7fda016ff90954f29e16367ad\">44d6c93</a></td><td><code>ExtPowerFreeModule: Make a quotient of a tensor module even for degree < 2</code></td></tr></table>\n",
    "created_at": "2020-07-28T23:36:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482553",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0869a10e02936f5b4548f9de4d5f682a60fe92f5">0869a10</a></td><td><code>ExtPowerFreeModule, ExtPowerDualFreeModule._repr_: Simplify by using the ordinal_str method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/344a7938b625d3b1b723c617569b01049125d653">344a793</a></td><td><code>ExtPowerFreeModule, ExtPowerDualFreeModule.__init__: Remove duplicate registration of self in the base module, uniqueness check</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/12e7da1eaa179f40abe447a2ecf0299ac5cb2f60">12e7da1</a></td><td><code>ExtPowerDualFreeModule: Document the 0th power</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/44d6c93e8b0885b7fda016ff90954f29e16367ad">44d6c93</a></td><td><code>ExtPowerFreeModule: Make a quotient of a tensor module even for degree < 2</code></td></tr></table>




---

archive/issue_comments_482554.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a59eb9cc470b79d1993ae83eee25a9a7f34efbb9\">a59eb9c</a></td><td><code>TensorFreeModule: Compute the correct category for type (0, 0)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8339de48eebfe4e5565b50ae1d194e18384b9a26\">8339de4</a></td><td><code>TensorFreeModule.__init__: Remove duplicate registration of self in the base module, uniqueness check</code></td></tr></table>\n",
    "created_at": "2020-07-29T00:25:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482554",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a59eb9cc470b79d1993ae83eee25a9a7f34efbb9">a59eb9c</a></td><td><code>TensorFreeModule: Compute the correct category for type (0, 0)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8339de48eebfe4e5565b50ae1d194e18384b9a26">8339de4</a></td><td><code>TensorFreeModule.__init__: Remove duplicate registration of self in the base module, uniqueness check</code></td></tr></table>




---

archive/issue_comments_482555.json:
```json
{
    "body": "Changed commit from **[44d6c93](https://github.com/sagemath/sagetrac-mirror/commit/44d6c93e8b0885b7fda016ff90954f29e16367ad)** to **[8339de4](https://github.com/sagemath/sagetrac-mirror/commit/8339de48eebfe4e5565b50ae1d194e18384b9a26)**",
    "created_at": "2020-07-29T00:25:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482555",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[44d6c93](https://github.com/sagemath/sagetrac-mirror/commit/44d6c93e8b0885b7fda016ff90954f29e16367ad)** to **[8339de4](https://github.com/sagemath/sagetrac-mirror/commit/8339de48eebfe4e5565b50ae1d194e18384b9a26)**



---

archive/issue_comments_482556.json:
```json
{
    "body": "Changed commit from **[8339de4](https://github.com/sagemath/sagetrac-mirror/commit/8339de48eebfe4e5565b50ae1d194e18384b9a26)** to **[8734151](https://github.com/sagemath/sagetrac-mirror/commit/87341516d539c2f0cd164b2502b939cbd0608b24)**",
    "created_at": "2020-07-29T01:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482556",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8339de4](https://github.com/sagemath/sagetrac-mirror/commit/8339de48eebfe4e5565b50ae1d194e18384b9a26)** to **[8734151](https://github.com/sagemath/sagetrac-mirror/commit/87341516d539c2f0cd164b2502b939cbd0608b24)**



---

archive/issue_comments_482557.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/14171b9b14bf4a872435d5a695a51d8150091241\">14171b9</a></td><td><code>FreeModuleBasis.__init__: Update all modules in the set fmodule._all_modules</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/87341516d539c2f0cd164b2502b939cbd0608b24\">8734151</a></td><td><code>FiniteRankFreeModule: Move all module identifications to methods exterior_power, dual_exterior_power, tensor_module</code></td></tr></table>\n",
    "created_at": "2020-07-29T01:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482557",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/14171b9b14bf4a872435d5a695a51d8150091241">14171b9</a></td><td><code>FreeModuleBasis.__init__: Update all modules in the set fmodule._all_modules</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/87341516d539c2f0cd164b2502b939cbd0608b24">8734151</a></td><td><code>FiniteRankFreeModule: Move all module identifications to methods exterior_power, dual_exterior_power, tensor_module</code></td></tr></table>




---

archive/issue_comments_482558.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nShouldn't #30251 be a dependency of this ticket?",
    "created_at": "2020-07-30T13:07:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482558",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:5" align="right">Comment 5</div>

Shouldn't #30251 be a dependency of this ticket?



---

archive/issue_comments_482559.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nYes, I will rebase this branch on top of it.",
    "created_at": "2020-07-30T15:34:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482559",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Yes, I will rebase this branch on top of it.



---

archive/issue_events_398328.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398328"
}
```



---

archive/issue_events_398329.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398329"
}
```



---

archive/issue_events_398330.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398330"
}
```



---

archive/issue_events_398331.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398331"
}
```



---

archive/issue_comments_482560.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30242#issuecomment-482560",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_398332.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398332"
}
```



---

archive/issue_events_398333.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398333"
}
```



---

archive/issue_events_398334.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398334"
}
```



---

archive/issue_events_398335.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398335"
}
```



---

archive/issue_events_398336.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398336"
}
```



---

archive/issue_events_398337.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398337"
}
```



---

archive/issue_events_398338.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398338"
}
```



---

archive/issue_events_398339.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398339"
}
```



---

archive/issue_events_398340.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398340"
}
```



---

archive/issue_events_398341.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30242",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30242#event-398341"
}
```
