...
```json
{
    "body": "ok, they changed the output format.\nThis is certainly easy to take care of.",
...
```json
{
    "body": "Many test failures are caused by pari having changed the normal Smith form algorithm, which now gives different transition matrices. Most of them are still correct, but it also has some unfortunate side effects:\n\n```\nsage: V=ZZ^2\nsage: W=span([[1,0],[0,6]],ZZ)\nsage: M=V/W\nsage: a=M[1]; a\n(1)\nsage: a.lift()\n(0, -1)\n```",
...
```json
{
    "body": "On Tuesday (November 3) from *14h to 17h* (UTC+2) there is a virtual PARI/GP workshop. I intend to discuss with Bill the cypari2 release and the adaptation in [SageMath](SageMath). Anybody interested is more than welcome. Just send me a private e-mail so that I can forward the announcement.",
...
```json
{
    "body": "To my mind, fixing Sage `gcd/lcm` makes more sense.",
...
```json
{
    "body": "Related: the functions `gcd/lcm` are not commutative\n\n```\nsage: type(lcm(2, pari(3)))\n<class 'sage.rings.integer.Integer'>\nsage: type(lcm(pari(3), 2))\n<class 'cypari2.gen.Gen'>\n```",
...
```json
{
    "body": "(Rebased on top of #30849 dropping modification to gcd/lcm when poldegree is involved)",
...
```json
{
    "body": "Any reason you're including the deprecation warnings for `Strchr` and `besseln` instead of replacing the functions themselves (with `strchr` and `bessely`)? Do we want to keep compatibility with older pari?",
...
```json
{
    "body": "PARI doc says: `rnfkummer` is deprecated use `bnrclassfield`",
...
```json
{
    "body": "Using sage 9.2 with the patch from this ticket, when spkg-configure is checking whether the pari 2.13 Debian package is usable, the check\n\n```\nchecking whether bnfisunit bug of pari 2.11.3 is fixed...\n```\n\nnow returns no. The check passed with Debian's pari 2.11.4.",
...
```json
{
    "body": "`build/pkgs/cypari/checksums.ini` needs `upstream_url`.\nAlso, perhaps the spkg should be renamed to `cypari2`?",
...
```json
{
    "body": "Here is the upstream bug report for the timeout, with an answer from Bill Allombert:\nhttps://pari.math.u-bordeaux.fr/cgi-bin/bugreport.cgi?bug=2259",
...
```json
{
    "body": "I decided to split cypari2 upgrade which is ready to go #31029.",
...
```json
{
    "body": "I don't know if it can be useful, but archlinux has a patch called `sagemath-pari-2.13.patch` here: https://aur.archlinux.org/cgit/aur.git/tree/?h=sagemath-git and debian too here: https://sources.debian.org/patches/sagemath/9.2-2/ (links from #31016)",
...
```json
{
    "body": "Can you merge https://git.sagemath.org/sage.git/commit?id=ad702bf59ea7372ce12c37f53a18662cd9baee0b so that system pari-2.13 is accepted?",
...
```json
{
    "body": "Yes, this still needs a lot of work. In particular, someone needs to look into comment:13",
...
```json
{
    "body": "The patch in the last comment should be in branch [u/tornaria/pari-snf](https://git.sagemath.org/sage.git/commit?id=u/tornaria/pari-snf), commit [f893a25](https://git.sagemath.org/sage.git/commit?id=f893a259238ecc11924d8c577dac161646243e00).",
...
```json
{
    "body": "Is the plan to drop support for system PARI < 2.13.1? Then `spkg-configure.m4` will need changing",
...
```json
{
    "body": "I think it's OK to drop older PARIs. Several Linux distros (Debian, Gentoo, arch (I guess)) and Homebrew already carry PARI 2.13. \n\nIt seems that only Conda is behind. Isuru, is Conda's PARI update in the works?",
...
```json
{
    "body": "Do you know whether the error `isirreducible: symbol not found` that I get in #31392 is related to this ticket?",
...
```json
{
    "body": "what is the status here? still \"needs work\"?",
...
```json
{
    "body": "I will have a look at the sandpile.",
...
```json
{
    "body": "Nothing to worry about in sandpile. All come from the change in Smith form and are valid answers.",
...
```json
{
    "body": "Replying to [comment:106 arojas]:\n> Replying to [comment:105 vdelecroix]:\n> \n> > And this is very bad: pari does not return the generator (the second item in the last output is empty). In the current sage version we have\n\n> \n> Do we actually need the generator here? From `number_field_ideal.py`:\n> \n> ```\n>         # Call bnfisprincipal().\n>         # If gens_needed, use flag=3 which will insist on computing\n>         # the generator.  Otherwise, use flag=1, where the generator\n>         # may or may not be computed.\n>         v = bnf.bnfisprincipal(self.pari_hnf(), 3 if gens_needed else 1)\n> ```\n> So it looks like sage is trying to find a generator even if it is not requested. According to the `bnfisprincipal` docs, this makes the computation unnecessarily complex:\n> \n> ```\n>    * 1: If set, outputs [e,t] as explained above, otherwise returns\n>      only e, which is much easier to compute.\n> ```\n> Should we change this to use the flag 0 when a generator is not needed?\n\n\nGood suggestion. Done in `58b84fd`.",
...
```json
{
    "body": "Haha: calling the compact form and expanding it works :) It is worth a report to PARI/GP.",
...
```json
{
    "body": "Great, only the Simon 2-descent breaks doctest now. I suggest that we leave it aside and triggers a warning anytime it is called. At the same time I would send a message on sage-devel/sage-nt so that interested people if any can fix it.",
...
```json
{
    "body": "Actually, the changes are clear regressions, but given the specification of `simon_two_descent` the output is correct in all cases.",
...
```json
{
    "body": "I will fix `spkg-configure.m4` and add check for `rnfdisc`.",
...
```json
{
    "body": "This ticket adds a Pari patch in build/pkgs/pari to fix https://pari.math.u-bordeaux.fr/cgi-bin/bugreport.cgi?bug=2284, which causes the failure above.",
...
```json
{
    "body": "Ah, OK, this explains - I'm trying this with 2.13.1 from Gentoo. So this means it's not detected by spkg-configure,m4, this bug.",
...
```json
{
    "body": "add a test for the latest patch in spkg-configure",
...
```json
{
    "body": "good point about SAGE_CHECK.",
...
```json
{
    "body": "and of course the full patch does not apply cleanly to 2.13.1, grrr...",
...
```json
{
    "body": "I think src/test/in/rnf needs to be added to the PARI patch too (like in https://github.com/NixOS/nixpkgs/blob/fb58ee7fd4baa7d49bdb8de8662d6ad20bf3bd19/pkgs/applications/science/math/pari/rnfdisc.patch).",
...
```json
{
    "body": "Ah, nevermind, I see you did remove the new test's output. My bad.",
...
```json
{
    "body": "Moreover, few lines above in the code, the tests involve a `# random` tag...",
...
```json
{
    "body": "Oops sorry, a tag `#random` was missing; it's actually weird that this test used to pass earlier.",
...
```json
{
    "body": "IMHO the difference in RNG values on different platforms here may be ignored.",
...
```json
{
    "body": "Well, the test `a == b` below does this checking (precisely, equality is checked at the default halting precision of the parent, which is 40 in the example).",
...
```json
{
    "body": "Looks harmless to me. I gather from the discussions above that the default generators for multiplicative groups of non-prime finite fields have been changed, and that would inevitably lead to changes in the output of these doctests (without affecting the mathematical validity of the calculations).",
...
```json
{
    "body": "The characters returned by the doctest are the same characters as they were before -- just notated differently. So the warning is no more or less problematic than it was before.",
...
```json
{
    "body": "OK, thanks, we'll be able to proceed then.",
...
```json
{
    "body": "as we bump the Pari version to 2.13+, we should remove tests for bugs fixed earlier.",
...
```json
{
    "body": "I'll do some digging and see whether this is Pari's fault, or a bug in the local components code. Quite likely it's the latter -- the `ZeroDivisionError` lines look rather fishy.",
...
```json
{
    "body": "As to the crash, it's hard to say what's wrong, maybe you build Pari/GP in an unusual way, such as using MPIR (one should not)?",
...
```json
{
    "body": "I didn't do anything \"unusual\" as far as I'm aware. I'm not using MPIR but the configure script is picking up my system-wide GMP library. I'm trying a rebuild forcing sage to build its own GMP (but that requires rebuilding about 50% of Sage so it will take a while).\n\n(By the way: if MPIR is deprecated, why is it supported by the configure script?)\n\nAs for your previous comment: I don't think this command has an exact equivalent in \"pure\" Pari. It looks like the problem is (once again) caused by changes to Smith normal form algorithms.",
...
```json
{
    "body": "I now tested for  crash on macOS 11.4 and on Fedora 32 Linux, no crash. Will do one more different system, just in case.",
...
```json
{
    "body": "also on Gentoo Linux I don't get a crash as reported in comment:171 - so I am inclined to see it as a one-time irreproducible non-bug.",
...
```json
{
    "body": "in any event, please post the top-level config.log and more info on the crash(es), a backtrace, preferably. Does it happen in Pari?",
...
```json
{
    "body": "It's not a tigerlake cpu (not that new). GCC version is 9.3.0. The operating system is Ubuntu 20.04 LTS, currently the most recent Ubuntu long-term-support version.\n\nHere's the config.log file:\n\nhttps://pastebin.com/3Kc7RNP0\n\nHere's the traceback:\n\nhttps://pastebin.com/bcW8GHVg",
...
```json
{
    "body": "I can confirm that the fix in comment:180 does the job, great! I'll add it and the rest of fixes, should be ready to go then, hopefully.",
...
```json
{
    "body": "Confirming this branch eliminates the error I was seeing in PARI/GP's qfminim(), due to a bug in PARI/GP that's since been corrected in that libraries source code. My elliptic curve code is now running properly again.\n\n---\nNew commits:",
...
```json
{
    "body": "This had a positive review before the issues with local components came up, and I'm confident those are fixed, so I think we can reinstate the positive review.",
...
```json
{
    "body": "With 9.4.b3 and pari 2.13, my patchbot petitbonum has now two errors in src/sage/modules/fg_pid/fgp_module.py\n\nhttps://patchbot.sagemath.org/ticket/0/",
...
```json
{
    "body": "I believe this will be fixed with #29978.",
...
```json
{
    "body": "No, #29978 does not fix the problem.",
...
```json
{
    "body": "I think the fix is quite trivial. Apparently with `reduce=False` things really are random. There is already one test in line 1`288 that is marked as random because of this. So yes, this could be done in a follow up of #29978.",
...
```json
{
    "body": "See #32048 for a possible fix.",
...
```json
{
    "body": "Let's move the discussion to #29977.",
