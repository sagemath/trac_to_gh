# Issue 30640: cygwin-standard: Segfault building doc-html

archive/issues_030403.json:
```json
{
    "assignees": [],
    "body": "https://github.com/mkoeppe/sage/runs/1148742719\n\n```\n[dochtml] error installing, exit status 1. End of log file:\n  [dochtml]   [spkg     ] updating environment: [new config] 272 added, 0 changed, 0 removed\n  [dochtml]   [polynomia] building [inventory]: targets for 62 source files that are out of date\n  [dochtml]   [polynomia] updating environment: [new config] 62 added, 0 changed, 0 removed\n  [dochtml]   [manifolds] building [inventory]: targets for 78 source files that are out of date\n  [dochtml]   [manifolds] updating environment: [new config] 78 added, 0 changed, 0 removed\n  [dochtml]   [spkg     ] The inventory files are in local/share/doc/sage/inventory/en/reference/spkg.\n  [dochtml]   Build finished. The built documents can be found in /cygdrive/d/a/sage/sage/local/share/doc/sage/inventory/en/reference/spkg\n  [dochtml]   ------------------------------------------------------------------------\n  [dochtml]   Unhandled SIGSEGV: A segmentation fault occurred.\n  [dochtml]   This probably occurred because a *compiled* module has a bug\n  [dochtml]   in it and is not properly wrapped with sig_on(), sig_off().\n  [dochtml]   Python will now terminate.\n  [dochtml]   ------------------------------------------------------------------------\n  [dochtml]   Error building the documentation.\n```\n\n**CC:**  @embray @slel\n\n**Reviewer:** Samuel Leli\u00e8vre\n\nIssue created by migration from https://trac.sagemath.org/ticket/30640\n\n",
    "closed_at": "2021-08-21T06:03:17Z",
    "created_at": "2020-09-23T04:36:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "cygwin-standard: Segfault building doc-html",
    "type": "issue",
    "updated_at": "2021-08-21T06:03:17Z",
    "url": "https://github.com/sagemath/sage/issues/30640",
    "user": "https://github.com/mkoeppe"
}
```
https://github.com/mkoeppe/sage/runs/1148742719

```
[dochtml] error installing, exit status 1. End of log file:
  [dochtml]   [spkg     ] updating environment: [new config] 272 added, 0 changed, 0 removed
  [dochtml]   [polynomia] building [inventory]: targets for 62 source files that are out of date
  [dochtml]   [polynomia] updating environment: [new config] 62 added, 0 changed, 0 removed
  [dochtml]   [manifolds] building [inventory]: targets for 78 source files that are out of date
  [dochtml]   [manifolds] updating environment: [new config] 78 added, 0 changed, 0 removed
  [dochtml]   [spkg     ] The inventory files are in local/share/doc/sage/inventory/en/reference/spkg.
  [dochtml]   Build finished. The built documents can be found in /cygdrive/d/a/sage/sage/local/share/doc/sage/inventory/en/reference/spkg
  [dochtml]   ------------------------------------------------------------------------
  [dochtml]   Unhandled SIGSEGV: A segmentation fault occurred.
  [dochtml]   This probably occurred because a *compiled* module has a bug
  [dochtml]   in it and is not properly wrapped with sig_on(), sig_off().
  [dochtml]   Python will now terminate.
  [dochtml]   ------------------------------------------------------------------------
  [dochtml]   Error building the documentation.
```

**CC:**  @embray @slel

**Reviewer:** Samuel Lelièvre

Issue created by migration from https://trac.sagemath.org/ticket/30640





---

archive/issue_events_371260.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-23T04:36:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "08517b",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371260"
}
```



---

archive/issue_events_371261.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-23T04:36:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "label": "https://github.com/sagemath/sage/labels/critical",
    "label_color": "ff0000",
    "label_name": "critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371261"
}
```



---

archive/issue_events_371262.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-23T04:36:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371262"
}
```



---

archive/issue_events_371263.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-23T04:36:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "008080",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371263"
}
```



---

archive/issue_events_371264.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371264"
}
```



---

archive/issue_events_371265.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371265"
}
```



---

archive/issue_events_371266.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-26T16:20:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371266"
}
```



---

archive/issue_comments_491816.json:
```json
{
    "body": "<a id='comment:2'>**Comment 2:**</a>\noutdated, we can't even get this far in the GH Actions until the numpy-related non-portability of the build is addressed.",
    "created_at": "2021-03-26T16:20:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30640#issuecomment-491816",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>**Comment 2:**</a>
outdated, we can't even get this far in the GH Actions until the numpy-related non-portability of the build is addressed.



---

archive/issue_events_371267.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-26T16:20:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "label": "https://github.com/sagemath/sage/labels/critical",
    "label_color": "ff0000",
    "label_name": "critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371267"
}
```



---

archive/issue_events_371268.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-06T18:16:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371268"
}
```



---

archive/issue_comments_491817.json:
```json
{
    "body": "<a id='comment:4'>**Comment 4:**</a>\nSage 9.3 and Sage 9.4.rc2 both build\nfor me on Cygwin, including doc-html.",
    "created_at": "2021-08-19T22:30:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30640#issuecomment-491817",
    "user": "https://github.com/slel"
}
```

<a id='comment:4'>**Comment 4:**</a>
Sage 9.3 and Sage 9.4.rc2 both build
for me on Cygwin, including doc-html.



---

archive/issue_events_371269.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-19T22:30:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371269"
}
```



---

archive/issue_events_371270.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-19T22:30:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371270"
}
```



---

archive/issue_comments_491818.json:
```json
{
    "body": "**Reviewer:** Samuel Leli\u00e8vre",
    "created_at": "2021-08-19T22:30:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30640#issuecomment-491818",
    "user": "https://github.com/slel"
}
```

**Reviewer:** Samuel Lelièvre



---

archive/issue_comments_491819.json:
```json
{
    "body": "<a id='comment:5'>**Comment 5:**</a>\nAnd in any case this was most likely related to the numpy architecture issue.",
    "created_at": "2021-08-20T11:15:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30640#issuecomment-491819",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'>**Comment 5:**</a>
And in any case this was most likely related to the numpy architecture issue.



---

archive/issue_events_371271.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-21T06:03:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371271"
}
```



---

archive/issue_events_371272.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-21T06:03:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30640",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30640#event-371272"
}
```
