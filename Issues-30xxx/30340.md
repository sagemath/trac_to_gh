# Issue 30340: tox.yml (dist): Do not fail when the tag already exists

archive/issues_030103.json:
```json
{
    "assignees": [],
    "body": "This currently blocks the automatic testing of the `local-macos-nohomebrew` platform in https://github.com/sagemath/sage/actions on releases.\n\nhttps://github.com/sagemath/sage/runs/968872297\n\n```\nCreating upstream/configure-415221a9a83e234a253168f82ff598a5c00fdf1a.tar.gz...\nUpdating checksum of configure (tarball configure-415221a9a83e234a253168f82ff598a5c00fdf1a.tar.gz)\n[detached HEAD 484d9ccb5a] Updated SageMath version to 9.2.beta8\n 2 files changed, 4 insertions(+), 4 deletions(-)\nfatal: tag '9.2.beta8' already exists\n```\n\n\n\nCC:  @kliem @vbraun\n\nBranch/Commit: **[2740aad](https://github.com/sagemath/sagetrac-mirror/commit/2740aad0db4e6b372ef9c7eb19f5bfac1875f9f7)**\n\nReviewer: **Jonathan Kliem**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30340_\n\n",
    "closed_at": "2020-09-15T22:01:06Z",
    "created_at": "2020-08-12T01:22:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tox.yml (dist): Do not fail when the tag already exists",
    "type": "issue",
    "updated_at": "2020-09-15T22:01:06Z",
    "url": "https://github.com/sagemath/sage/issues/30340",
    "user": "https://github.com/mkoeppe"
}
```
This currently blocks the automatic testing of the `local-macos-nohomebrew` platform in https://github.com/sagemath/sage/actions on releases.

https://github.com/sagemath/sage/runs/968872297

```
Creating upstream/configure-415221a9a83e234a253168f82ff598a5c00fdf1a.tar.gz...
Updating checksum of configure (tarball configure-415221a9a83e234a253168f82ff598a5c00fdf1a.tar.gz)
[detached HEAD 484d9ccb5a] Updated SageMath version to 9.2.beta8
 2 files changed, 4 insertions(+), 4 deletions(-)
fatal: tag '9.2.beta8' already exists
```



CC:  @kliem @vbraun

Branch/Commit: **[2740aad](https://github.com/sagemath/sagetrac-mirror/commit/2740aad0db4e6b372ef9c7eb19f5bfac1875f9f7)**

Reviewer: **Jonathan Kliem**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/30340_





---

archive/issue_events_399740.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-12T01:22:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399740"
}
```



---

archive/issue_events_399741.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-12T01:22:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399741"
}
```



---

archive/issue_events_399742.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-12T01:22:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399742"
}
```



---

archive/issue_events_399743.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-12T01:22:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399743"
}
```



---

archive/issue_comments_484598.json:
```json
{
    "body": "Branch: **[u/mkoeppe/tox_yml__dist___do_not_fail_when_the_tag_already_exists](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/tox_yml__dist___do_not_fail_when_the_tag_already_exists)**",
    "created_at": "2020-08-29T17:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30340#issuecomment-484598",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/tox_yml__dist___do_not_fail_when_the_tag_already_exists](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/tox_yml__dist___do_not_fail_when_the_tag_already_exists)**



---

archive/issue_events_399744.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T17:56:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399744"
}
```



---

archive/issue_comments_484599.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-08-29T17:56:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30340#issuecomment-484599",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_399745.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T17:56:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399745"
}
```



---

archive/issue_events_399746.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T17:56:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399746"
}
```



---

archive/issue_comments_484600.json:
```json
{
    "body": "Commit: **[2740aad](https://github.com/sagemath/sagetrac-mirror/commit/2740aad0db4e6b372ef9c7eb19f5bfac1875f9f7)**",
    "created_at": "2020-08-29T17:56:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30340#issuecomment-484600",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[2740aad](https://github.com/sagemath/sagetrac-mirror/commit/2740aad0db4e6b372ef9c7eb19f5bfac1875f9f7)**



---

archive/issue_comments_484601.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2740aad0db4e6b372ef9c7eb19f5bfac1875f9f7\">2740aad</a></td><td><code>.github/workflows/tox.yml (dist): Do not fail when the tag already exists</code></td></tr></table>\n",
    "created_at": "2020-08-29T17:56:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30340#issuecomment-484601",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2740aad0db4e6b372ef9c7eb19f5bfac1875f9f7">2740aad</a></td><td><code>.github/workflows/tox.yml (dist): Do not fail when the tag already exists</code></td></tr></table>




---

archive/issue_comments_484602.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n+This currently blocks the automatic testing of the `local-macos-nohomebrew` platform in https://github.com/sagemath/sage/actions on releases.\n+\n https://github.com/sagemath/sage/runs/968872297\n \n ```\n``````\n",
    "created_at": "2020-08-29T17:56:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30340#issuecomment-484602",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
+This currently blocks the automatic testing of the `local-macos-nohomebrew` platform in https://github.com/sagemath/sage/actions on releases.
+
 https://github.com/sagemath/sage/runs/968872297
 
 ```
``````




---

archive/issue_comments_484603.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2020-09-04T13:01:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30340#issuecomment-484603",
    "user": "https://github.com/kliem"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_484604.json:
```json
{
    "body": "Reviewer: **Jonathan Kliem**",
    "created_at": "2020-09-04T13:01:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30340#issuecomment-484604",
    "user": "https://github.com/kliem"
}
```

Reviewer: **Jonathan Kliem**



---

archive/issue_events_399747.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-09-04T13:01:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399747"
}
```



---

archive/issue_events_399748.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-09-04T13:01:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399748"
}
```



---

archive/issue_comments_484605.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks.",
    "created_at": "2020-09-04T17:33:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30340#issuecomment-484605",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks.



---

archive/issue_comments_484606.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/tox_yml__dist___do_not_fail_when_the_tag_already_exists](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/tox_yml__dist___do_not_fail_when_the_tag_already_exists)** to **[2740aad](https://github.com/sagemath/sagetrac-mirror/commit/2740aad0db4e6b372ef9c7eb19f5bfac1875f9f7)**",
    "created_at": "2020-09-15T22:01:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30340#issuecomment-484606",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/tox_yml__dist___do_not_fail_when_the_tag_already_exists](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/tox_yml__dist___do_not_fail_when_the_tag_already_exists)** to **[2740aad](https://github.com/sagemath/sagetrac-mirror/commit/2740aad0db4e6b372ef9c7eb19f5bfac1875f9f7)**



---

archive/issue_events_399749.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-09-15T22:01:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399749"
}
```



---

archive/issue_events_399750.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cd65699176ee31678c8bba9a830965f1997a3670",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-09-15T22:01:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30340",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30340#event-399750"
}
```
