# Issue 30983: Fix tick label rendering in 2D plots, eg 4.0e6 displays as 4.0

archive/issues_030746.json:
```json
{
    "body": "Reported at\n\n- [Ask Sage question 54361: Plot ratio of Bessel functions](https://ask.sagemath.org/question/54361)\n\n- [Ask Sage question 54462: Some 2-d plot questions](https://ask.sagemath.org/question/54462)\n\n- #31447: `list_plot` labels don't indicate exponential notation by default\n\nCC:  @egourgoulhon @jcamp0x2a @slel\n\nKeywords: plot2d, tick labels\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/30983\n\n",
    "created_at": "2020-11-30T15:46:46Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Fix tick label rendering in 2D plots, eg 4.0e6 displays as 4.0",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/30983",
    "user": "https://github.com/slel"
}
```
Reported at

- [Ask Sage question 54361: Plot ratio of Bessel functions](https://ask.sagemath.org/question/54361)

- [Ask Sage question 54462: Some 2-d plot questions](https://ask.sagemath.org/question/54462)

- #31447: `list_plot` labels don't indicate exponential notation by default

CC:  @egourgoulhon @jcamp0x2a @slel

Keywords: plot2d, tick labels

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/30983





---

archive/issue_comments_614685.json:
```json
{
    "body": "<a id='comment:1'></a>Related:\n\n- [Stack Overflow: matplotlib: format axis offset-values to whole numbers or specific number](https://stackoverflow.com/q/3677368)",
    "created_at": "2020-12-01T15:37:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30983#issuecomment-614685",
    "user": "https://github.com/slel"
}
```

<a id='comment:1'></a>Related:

- [Stack Overflow: matplotlib: format axis offset-values to whole numbers or specific number](https://stackoverflow.com/q/3677368)



---

archive/issue_events_080903.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30983#event-80903"
}
```



---

archive/issue_comments_614686.json:
```json
{
    "body": "<a id='comment:2'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30983#issuecomment-614686",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_comments_614687.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,8 +1,10 @@\n Reported at\n \n-- [Ask Sage question 54361: Plot ration of Bessel functions](https://ask.sagemath.org/question/54361)\n+- [Ask Sage question 54361: Plot ratio of Bessel functions](https://ask.sagemath.org/question/54361)\n \n - [Ask Sage question 54462: Some 2-d plot questions](https://ask.sagemath.org/question/54462)\n+\n+- #31447: `list_plot` labels don't indicate exponential notation by default\n \n Comment: 1\n \n``````\n",
    "created_at": "2021-03-03T14:23:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30983#issuecomment-614687",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,8 +1,10 @@
 Reported at
 
-- [Ask Sage question 54361: Plot ration of Bessel functions](https://ask.sagemath.org/question/54361)
+- [Ask Sage question 54361: Plot ratio of Bessel functions](https://ask.sagemath.org/question/54361)
 
 - [Ask Sage question 54462: Some 2-d plot questions](https://ask.sagemath.org/question/54462)
+
+- #31447: `list_plot` labels don't indicate exponential notation by default
 
 Comment: 1
 
``````




---

archive/issue_events_080904.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30983#event-80904"
}
```



---

archive/issue_events_080905.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30983#event-80905"
}
```



---

archive/issue_events_080906.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30983#event-80906"
}
```



---

archive/issue_events_080907.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30983#event-80907"
}
```



---

archive/issue_events_080908.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30983#event-80908"
}
```



---

archive/issue_events_080909.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30983#event-80909"
}
```



---

archive/issue_comments_614688.json:
```json
{
    "body": "<a id='comment:7'></a>I believe this is a feature of \"new\" matplotlib.  See [matplotlib](https://matplotlib.org/stable/api/ticker_api.html#matplotlib.ticker.ScalarFormatter) and in particular the \"Notes\" and the [figure](https://matplotlib.org/stable/api/ticker_api-1.png) which clearly shows this.  See also ticket:7964 and [this sage-devel discussion](https://groups.google.com/g/sage-devel/c/s39WysaG0fI).",
    "created_at": "2022-07-27T13:34:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30983#issuecomment-614688",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'></a>I believe this is a feature of "new" matplotlib.  See [matplotlib](https://matplotlib.org/stable/api/ticker_api.html#matplotlib.ticker.ScalarFormatter) and in particular the "Notes" and the [figure](https://matplotlib.org/stable/api/ticker_api-1.png) which clearly shows this.  See also ticket:7964 and [this sage-devel discussion](https://groups.google.com/g/sage-devel/c/s39WysaG0fI).



---

archive/issue_comments_614689.json:
```json
{
    "body": "<a id='comment:8'></a>> - [Stack Overflow: matplotlib: format axis offset-values to whole numbers or specific number](https://stackoverflow.com/q/3677368)\n\n\nNot just related, but probably something that can fix this, once we fix that the 1e6 stuff is gone completely anyway (related to #7964, again).",
    "created_at": "2022-07-27T13:35:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30983#issuecomment-614689",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:8'></a>> - [Stack Overflow: matplotlib: format axis offset-values to whole numbers or specific number](https://stackoverflow.com/q/3677368)


Not just related, but probably something that can fix this, once we fix that the 1e6 stuff is gone completely anyway (related to #7964, again).



---

archive/issue_events_080910.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30983#event-80910"
}
```



---

archive/issue_events_080911.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30983",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30983#event-80911"
}
```
