# Issue 30037: Added a construction for BIBDs from symmetric BIBDs

archive/issues_029800.json:
```json
{
    "assignees": [],
    "body": "Given a symmetric `(v, k, lambd)`-BIBD one can build a `(v-k, k-lambd, lambd)`-BIBD by removing a block from the former.\n\nFor more on BIBDs see https://en.wikipedia.org/wiki/Block_design#Definition_of_a_BIBD_(or_2-design)\n\nDepends on #29959\n\nCC:  @dimpase\n\nKeywords: **bibd**\n\nBranch/Commit: **[aac95be](https://github.com/sagemath/sagetrac-mirror/commit/aac95beaa3de54a990b06be27b1ca7a529884634)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Ivo Maffei**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30037_\n\n",
    "closed_at": "2020-07-10T19:33:32Z",
    "created_at": "2020-07-01T09:33:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorial%20designs",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Added a construction for BIBDs from symmetric BIBDs",
    "type": "issue",
    "updated_at": "2020-07-10T19:33:32Z",
    "url": "https://github.com/sagemath/sage/issues/30037",
    "user": "https://github.com/Ivo-Maffei"
}
```
Given a symmetric `(v, k, lambd)`-BIBD one can build a `(v-k, k-lambd, lambd)`-BIBD by removing a block from the former.

For more on BIBDs see https://en.wikipedia.org/wiki/Block_design#Definition_of_a_BIBD_(or_2-design)

Depends on #29959

CC:  @dimpase

Keywords: **bibd**

Branch/Commit: **[aac95be](https://github.com/sagemath/sagetrac-mirror/commit/aac95beaa3de54a990b06be27b1ca7a529884634)**

Reviewer: **Dima Pasechnik**

Author: **Ivo Maffei**

_Issue created by migration from https://trac.sagemath.org/ticket/30037_





---

archive/issue_events_395347.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-07-01T09:33:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30037#event-395347"
}
```



---

archive/issue_events_395348.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-07-01T09:33:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorial%20designs",
    "label_color": "0000ff",
    "label_name": "component: combinatorial designs",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30037#event-395348"
}
```



---

archive/issue_events_395349.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-07-01T09:33:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30037#event-395349"
}
```



---

archive/issue_events_395350.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-07-01T09:33:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30037#event-395350"
}
```



---

archive/issue_comments_478360.json:
```json
{
    "body": "Changed commit from **[e608ef8](https://github.com/sagemath/sagetrac-mirror/commit/e608ef8346e58e62b0b9eb76358f110511ec5451)** to **[aac95be](https://github.com/sagemath/sagetrac-mirror/commit/aac95beaa3de54a990b06be27b1ca7a529884634)**",
    "created_at": "2020-07-05T13:36:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30037#issuecomment-478360",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e608ef8](https://github.com/sagemath/sagetrac-mirror/commit/e608ef8346e58e62b0b9eb76358f110511ec5451)** to **[aac95be](https://github.com/sagemath/sagetrac-mirror/commit/aac95beaa3de54a990b06be27b1ca7a529884634)**



---

archive/issue_comments_478361.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a60769317c4d0a0d601270ee1123318cebee8564\">a607693</a></td><td><code>changed docstring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7c4042ce49c57a76c7ad1defbfac36efcf2a8eca\">7c4042c</a></td><td><code>Merge branch 'bibd' into recursive_bibd</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aac95beaa3de54a990b06be27b1ca7a529884634\">aac95be</a></td><td><code>fixed bug; added tests</code></td></tr></table>\n",
    "created_at": "2020-07-05T13:36:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30037#issuecomment-478361",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:1'>Comment 1:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a60769317c4d0a0d601270ee1123318cebee8564">a607693</a></td><td><code>changed docstring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7c4042ce49c57a76c7ad1defbfac36efcf2a8eca">7c4042c</a></td><td><code>Merge branch 'bibd' into recursive_bibd</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aac95beaa3de54a990b06be27b1ca7a529884634">aac95be</a></td><td><code>fixed bug; added tests</code></td></tr></table>




---

archive/issue_events_395351.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-07-05T13:37:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30037#event-395351"
}
```



---

archive/issue_comments_478362.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nto test this recursive builder, it would be nice to have a (maybe internal) function to build all the 2-designs known to Sage with at  most v points and with lambda in a given range.",
    "created_at": "2020-07-07T16:01:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30037#issuecomment-478362",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:3'>Comment 3:</a>
to test this recursive builder, it would be nice to have a (maybe internal) function to build all the 2-designs known to Sage with at  most v points and with lambda in a given range.



---

archive/issue_comments_478363.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-07-08T10:36:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30037#issuecomment-478363",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_478364.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPlease do what I propose in [comment:3](#comment%3A3) on another ticket.\nAs an example of such a test, cf. `_check_database()` in `sage.graphs.strongly_regular_db`.",
    "created_at": "2020-07-08T10:36:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30037#issuecomment-478364",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:4'>Comment 4:</a>
Please do what I propose in [comment:3](#comment%3A3) on another ticket.
As an example of such a test, cf. `_check_database()` in `sage.graphs.strongly_regular_db`.



---

archive/issue_events_395352.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-07-08T10:36:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30037#event-395352"
}
```



---

archive/issue_events_395353.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-07-08T10:36:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30037#event-395353"
}
```



---

archive/issue_events_395354.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-07-10T19:33:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30037#event-395354"
}
```



---

archive/issue_events_395355.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "13cfd670488d02c3e28e8433c8c91da68a0cb555",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-07-10T19:33:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30037#event-395355"
}
```



---

archive/issue_comments_478365.json:
```json
{
    "body": "Changed branch from **[u/gh-Ivo-Maffei/recursive_bibd](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Ivo-Maffei/recursive_bibd)** to **[aac95be](https://github.com/sagemath/sagetrac-mirror/commit/aac95beaa3de54a990b06be27b1ca7a529884634)**",
    "created_at": "2020-07-10T19:33:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30037",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30037#issuecomment-478365",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-Ivo-Maffei/recursive_bibd](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Ivo-Maffei/recursive_bibd)** to **[aac95be](https://github.com/sagemath/sagetrac-mirror/commit/aac95beaa3de54a990b06be27b1ca7a529884634)**
