...
<a id='comment:7'></a>Replying to [gh-wdjoyner](#comment%3A3):
> I'm trying to compile sage-9.2.rc0 on big sur with xcode 12.0.1. 
> 
> I ran sage -f openblas. Here's the tail of that:
> 
...
<a id='comment:10'></a>Sorry, I led us on a bit of a wild goose chase for the past few comments. Back in[comment:4](#comment%3A4), I said the right thing:
...
<a id='comment:17'></a>Replying to [jhpalmieri](#comment%3A10):
> Sorry, I led us on a bit of a wild goose chase for the past few comments. Back in[comment:4](#comment%3A4), I said the right thing:
> 
> ```
> I could try building Sage's mpfr and gmp.
...
<a id='comment:25'></a>I don't know how to set these variables in such a way to get Sage to build. (This is with the changes listed in[comment:22](#comment%3A22).)
...
<a id='comment:48'></a>Replying to [jhpalmieri](#comment%3A36):
> After running `make pillow`, I tried `make numpy` and got an unexpected error:
> 
> ```
> Package 'numpy' is currently not installed
...
<a id='comment:54'></a>Replying to [jhpalmieri](#comment%3A51):
> Replying to [jhpalmieri](#comment%3A50):
> > I [opened an issue](https://github.com/numpy/numpy/issues/18093) with `numpy` for a fix as was done with `cffi`.
