# Issue 30392: Cygwin: can't build Numpy 1.19.1 with GCC < 8.4

archive/issues_030155.json:
```json
{
    "body": "Since #29766 I cannot build Numpy 1.19.1 on Cygwin, specifically with AVX enabled, due to this issue in Numpy (actually caused by a bug in GCC < 8.4): https://github.com/numpy/numpy/issues/16290\n\n\nI've been meaning to work on this issue anyways, so I'll see if I can find a clean workaround.  If not, it also works to pass some CFLAGS to the Numpy build as noted on the upstream issues.\n\n\nhttps://cygwin.com/packages/summary/gcc-core.html\n\nKeywords: cygwin numpy\n\nResolution: worksforme\n\nIssue created by migration from https://trac.sagemath.org/ticket/30392\n\n",
    "closed_at": "2021-03-29T07:52:05Z",
    "created_at": "2020-08-19T14:24:47Z",
    "labels": [
        "component: porting: cygwin",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Cygwin: can't build Numpy 1.19.1 with GCC < 8.4",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/30392",
    "user": "https://github.com/embray"
}
```
Since #29766 I cannot build Numpy 1.19.1 on Cygwin, specifically with AVX enabled, due to this issue in Numpy (actually caused by a bug in GCC < 8.4): https://github.com/numpy/numpy/issues/16290


I've been meaning to work on this issue anyways, so I'll see if I can find a clean workaround.  If not, it also works to pass some CFLAGS to the Numpy build as noted on the upstream issues.


https://cygwin.com/packages/summary/gcc-core.html

Keywords: cygwin numpy

Resolution: worksforme

Issue created by migration from https://trac.sagemath.org/ticket/30392





---

archive/issue_comments_458152.json:
```json
{
    "body": "Description changed:\n```diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Since #29766 I cannot build Numpy 1.9.1 on Cygwin, specifically with AVX enabled, due to this issue in Numpy (actually caused by a bug in GCC): https://github.com/numpy/numpy/issues/16290\n+Since #29766 I cannot build Numpy 1.19.1 on Cygwin, specifically with AVX enabled, due to this issue in Numpy (actually caused by a bug in GCC): https://github.com/numpy/numpy/issues/16290\n \n \n I've been meaning to work on this issue anyways, so I'll see if I can find a clean workaround.  If not, it also works to pass some CFLAGS to the Numpy build as noted on the upstream issues.\n```\n",
    "created_at": "2020-08-19T17:23:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458152",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
```diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Since #29766 I cannot build Numpy 1.9.1 on Cygwin, specifically with AVX enabled, due to this issue in Numpy (actually caused by a bug in GCC): https://github.com/numpy/numpy/issues/16290
+Since #29766 I cannot build Numpy 1.19.1 on Cygwin, specifically with AVX enabled, due to this issue in Numpy (actually caused by a bug in GCC): https://github.com/numpy/numpy/issues/16290
 
 
 I've been meaning to work on this issue anyways, so I'll see if I can find a clean workaround.  If not, it also works to pass some CFLAGS to the Numpy build as noted on the upstream issues.
```




---

archive/issue_comments_458153.json:
```json
{
    "body": "<a id='comment:2'></a>Yes, this is important to fix. Unfortunately the CI infrastructure does not catch this because we cannot control the processor variant that will run the job. For example, ~~https://github.com/sagemath/sage/runs/969113336?check_suite_focus=true~~ https://github.com/sagemath/sage/runs/1002630008 has built numpy successfully.\n\nWith which compiler are you observing this failure?",
    "created_at": "2020-08-19T17:28:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458153",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'></a>Yes, this is important to fix. Unfortunately the CI infrastructure does not catch this because we cannot control the processor variant that will run the job. For example, ~~https://github.com/sagemath/sage/runs/969113336?check_suite_focus=true~~ https://github.com/sagemath/sage/runs/1002630008 has built numpy successfully.

With which compiler are you observing this failure?



---

archive/issue_comments_458154.json:
```json
{
    "body": "<a id='comment:3'></a>The discussion at https://github.com/numpy/numpy/issues/14787 seems to suggest that nothing needs to be fixed with current compilers. Do you still experience the build problem?",
    "created_at": "2020-08-31T13:50:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458154",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'></a>The discussion at https://github.com/numpy/numpy/issues/14787 seems to suggest that nothing needs to be fixed with current compilers. Do you still experience the build problem?



---

archive/issue_comments_458155.json:
```json
{
    "body": "<a id='comment:4'></a>It is a problem with GCC<8.4.\n\nI started on a patch to Numpy the week before last but hit a snag before I was able to finish it.  Then I went on vacation :P\n\nAn ugly but acceptable workaround is also to set\n\n```\nCFLAGS=-ffixed-xmm16 -ffixed-xmm17 -ffixed-xmm18 -ffixed-xmm19 -ffixed-xmm20 -ffixed-xmm21 -ffixed-xmm22 -ffixed-xmm23 -ffixed-xmm24 -ffixed-xmm25 -ffixed-xmm26 -ffixed-xmm27 -ffixed-xmm28 -ffixed-xmm29 -ffixed-xmm30 -ffixed-xmm31\n```\n\nwhen compiling Numpy.",
    "created_at": "2020-08-31T14:37:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458155",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'></a>It is a problem with GCC<8.4.

I started on a patch to Numpy the week before last but hit a snag before I was able to finish it.  Then I went on vacation :P

An ugly but acceptable workaround is also to set

```
CFLAGS=-ffixed-xmm16 -ffixed-xmm17 -ffixed-xmm18 -ffixed-xmm19 -ffixed-xmm20 -ffixed-xmm21 -ffixed-xmm22 -ffixed-xmm23 -ffixed-xmm24 -ffixed-xmm25 -ffixed-xmm26 -ffixed-xmm27 -ffixed-xmm28 -ffixed-xmm29 -ffixed-xmm30 -ffixed-xmm31
```

when compiling Numpy.



---

archive/issue_comments_458156.json:
```json
{
    "body": "<a id='comment:5'></a>Current cygwin has gcc 10.2. \n\nI would suggest to limit support of a from-source build of Sage on Cygwin to the latest Cygwin version and encourage users to update Cygwin.\n\n(Does cygwin even have a supported way to install an older version of the distribution?)",
    "created_at": "2020-08-31T15:03:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458156",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>Current cygwin has gcc 10.2. 

I would suggest to limit support of a from-source build of Sage on Cygwin to the latest Cygwin version and encourage users to update Cygwin.

(Does cygwin even have a supported way to install an older version of the distribution?)



---

archive/issue_comments_458157.json:
```json
{
    "body": "Description changed:\n```diff\n--- \n+++ \n@@ -1,4 +1,10 @@\n+Since #29766 I cannot build Numpy 1.19.1 on Cygwin, specifically with AVX enabled, due to this issue in Numpy (actually caused by a bug in GCC < 8.4): https://github.com/numpy/numpy/issues/16290\n \n+\n+I've been meaning to work on this issue anyways, so I'll see if I can find a clean workaround.  If not, it also works to pass some CFLAGS to the Numpy build as noted on the upstream issues.\n+\n+\n+https://cygwin.com/packages/summary/gcc-core.html\n \n Summary: Cygwin: can't build Numpy 1.9.1\n \n```\n",
    "created_at": "2020-08-31T15:04:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458157",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
```diff
--- 
+++ 
@@ -1,4 +1,10 @@
+Since #29766 I cannot build Numpy 1.19.1 on Cygwin, specifically with AVX enabled, due to this issue in Numpy (actually caused by a bug in GCC < 8.4): https://github.com/numpy/numpy/issues/16290
 
+
+I've been meaning to work on this issue anyways, so I'll see if I can find a clean workaround.  If not, it also works to pass some CFLAGS to the Numpy build as noted on the upstream issues.
+
+
+https://cygwin.com/packages/summary/gcc-core.html
 
 Summary: Cygwin: can't build Numpy 1.9.1
 
```




---

archive/issue_events_078802.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30392#event-78802"
}
```



---

archive/issue_comments_458158.json:
```json
{
    "body": "<a id='comment:10'></a>We can close this one as outdated",
    "created_at": "2021-03-26T16:21:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458158",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'></a>We can close this one as outdated



---

archive/issue_comments_458159.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2021-03-26T16:21:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458159",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_events_078803.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-26T16:21:37Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30392#event-78803"
}
```



---

archive/issue_events_078804.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-26T16:21:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30392#event-78804"
}
```



---

archive/issue_events_078805.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2021-03-29T07:52:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30392#event-78805"
}
```



---

archive/issue_comments_458160.json:
```json
{
    "body": "<a id='comment:11'></a>I believe so, yes.  Most Cygwin installs are not on GCC<8.4 by default, and I have also fixed the issue in Numpy.",
    "created_at": "2021-03-29T07:52:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458160",
    "user": "https://github.com/embray"
}
```

<a id='comment:11'></a>I believe so, yes.  Most Cygwin installs are not on GCC<8.4 by default, and I have also fixed the issue in Numpy.



---

archive/issue_comments_458161.json:
```json
{
    "body": "Resolution: worksforme",
    "created_at": "2021-03-29T07:52:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30392#issuecomment-458161",
    "user": "https://github.com/embray"
}
```

Resolution: worksforme
