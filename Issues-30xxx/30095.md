# Issue 30095: sage.tensor.modules: Add SymPowerFreeModule

archive/issues_029858.json:
```json
{
    "assignees": [],
    "body": "`FiniteRankFreeModule` defines a method `exterior_power`, creating an instance of `ExtPowerFreeModule`.\n\nWe propose to add a method `symmetric_power` and class `SymPowerFreeModule`. (More generally, we could have submodules of tensors with prescribed symmetries; this is currently only available on the level of elements, not parents.)\n\nThey will be submodules of `TensorFreeModule`s. (#30169 makes `ExtPowerFreeModule`s quotients.)\n\nThe existing method `FiniteRankFreeModule.sym_bilinear_form` might be changed to produce elements of it for p=2.\n\n\n\nDepends on #30229\n\nCC:  @egourgoulhon @dimpase @yuan-zhou @tscrim @mjungmath\n\nReviewer: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30095_\n\n",
    "closed_at": "2022-08-28T09:02:18Z",
    "created_at": "2020-07-08T22:54:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.tensor.modules: Add SymPowerFreeModule",
    "type": "issue",
    "updated_at": "2022-08-28T09:02:18Z",
    "url": "https://github.com/sagemath/sage/issues/30095",
    "user": "https://github.com/mkoeppe"
}
```
`FiniteRankFreeModule` defines a method `exterior_power`, creating an instance of `ExtPowerFreeModule`.

We propose to add a method `symmetric_power` and class `SymPowerFreeModule`. (More generally, we could have submodules of tensors with prescribed symmetries; this is currently only available on the level of elements, not parents.)

They will be submodules of `TensorFreeModule`s. (#30169 makes `ExtPowerFreeModule`s quotients.)

The existing method `FiniteRankFreeModule.sym_bilinear_form` might be changed to produce elements of it for p=2.



Depends on #30229

CC:  @egourgoulhon @dimpase @yuan-zhou @tscrim @mjungmath

Reviewer: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/30095_





---

archive/issue_events_396162.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T22:54:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396162"
}
```



---

archive/issue_events_396163.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T22:54:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396163"
}
```



---

archive/issue_events_396164.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T22:54:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396164"
}
```



---

archive/issue_events_396165.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T22:54:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396165"
}
```



---

archive/issue_comments_479481.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,6 +2,6 @@\n \n We propose to add a method `symmetric_power` and class `SymPowerFreeModule`.\n \n-The existing method `ExtPowerFreeModule.sym_bilinear_form` might be changed to produce elements of it for p=2.\n+The existing method `FiniteRankFreeModule.sym_bilinear_form` might be changed to produce elements of it for p=2.\n \n \n``````\n",
    "created_at": "2020-07-09T01:45:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479481",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,6 +2,6 @@
 
 We propose to add a method `symmetric_power` and class `SymPowerFreeModule`.
 
-The existing method `ExtPowerFreeModule.sym_bilinear_form` might be changed to produce elements of it for p=2.
+The existing method `FiniteRankFreeModule.sym_bilinear_form` might be changed to produce elements of it for p=2.
 
 
``````




---

archive/issue_comments_479482.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,6 +2,8 @@\n \n We propose to add a method `symmetric_power` and class `SymPowerFreeModule`.\n \n+They will be submodules of `TensorFreeModule`s. (#30169 makes `ExtPowerFreeModule`s quotients.)\n+\n The existing method `FiniteRankFreeModule.sym_bilinear_form` might be changed to produce elements of it for p=2.\n \n \n``````\n",
    "created_at": "2020-07-28T12:08:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479482",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,6 +2,8 @@
 
 We propose to add a method `symmetric_power` and class `SymPowerFreeModule`.
 
+They will be submodules of `TensorFreeModule`s. (#30169 makes `ExtPowerFreeModule`s quotients.)
+
 The existing method `FiniteRankFreeModule.sym_bilinear_form` might be changed to produce elements of it for p=2.
 
 
``````




---

archive/issue_comments_479483.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n `FiniteRankFreeModule` defines a method `exterior_power`, creating an instance of `ExtPowerFreeModule`.\n \n-We propose to add a method `symmetric_power` and class `SymPowerFreeModule`.\n+We propose to add a method `symmetric_power` and class `SymPowerFreeModule`. (More generally, we could have submodules of tensors with prescribed symmetries; this is currently only available on the level of elements, not parents.)\n \n They will be submodules of `TensorFreeModule`s. (#30169 makes `ExtPowerFreeModule`s quotients.)\n \n``````\n",
    "created_at": "2020-07-30T02:54:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479483",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 `FiniteRankFreeModule` defines a method `exterior_power`, creating an instance of `ExtPowerFreeModule`.
 
-We propose to add a method `symmetric_power` and class `SymPowerFreeModule`.
+We propose to add a method `symmetric_power` and class `SymPowerFreeModule`. (More generally, we could have submodules of tensors with prescribed symmetries; this is currently only available on the level of elements, not parents.)
 
 They will be submodules of `TensorFreeModule`s. (#30169 makes `ExtPowerFreeModule`s quotients.)
 
``````




---

archive/issue_comments_479484.json:
```json
{
    "body": "Dependencies: **#30229**",
    "created_at": "2020-08-02T05:53:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479484",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#30229**



---

archive/issue_comments_479485.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nI think one has to be very careful here, especially with the last proposal. If the base field has not characteristic zero, as described also on [Wikipedia](https://en.wikipedia.org/wiki/Symmetric_algebra#Relationship_with_symmetric_tensors), a symmetric bilinear form cannot be identified with an element of the symmetric algebra.\n\n**Addendum:** I just noticed, we are not even in the manifold setting.",
    "created_at": "2020-08-03T11:18:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479485",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:7" align="right">Comment 7</div>

I think one has to be very careful here, especially with the last proposal. If the base field has not characteristic zero, as described also on [Wikipedia](https://en.wikipedia.org/wiki/Symmetric_algebra#Relationship_with_symmetric_tensors), a symmetric bilinear form cannot be identified with an element of the symmetric algebra.

**Addendum:** I just noticed, we are not even in the manifold setting.



---

archive/issue_comments_479486.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThanks. Yes, we'll have to be careful",
    "created_at": "2020-08-03T17:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479486",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Thanks. Yes, we'll have to be careful



---

archive/issue_events_396166.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396166"
}
```



---

archive/issue_events_396167.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396167"
}
```



---

archive/issue_events_396168.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396168"
}
```



---

archive/issue_events_396169.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396169"
}
```



---

archive/issue_comments_479487.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479487",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:10" align="right">Comment 10</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_396170.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396170"
}
```



---

archive/issue_events_396171.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396171"
}
```



---

archive/issue_events_396172.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396172"
}
```



---

archive/issue_events_396173.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396173"
}
```



---

archive/issue_events_396174.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396174"
}
```



---

archive/issue_events_396175.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396175"
}
```



---

archive/issue_events_396176.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-28T04:33:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396176"
}
```



---

archive/issue_comments_479488.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nDone as part of #30229",
    "created_at": "2022-08-28T04:33:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479488",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:14" align="right">Comment 14</div>

Done as part of #30229



---

archive/issue_events_396177.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-28T04:33:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396177"
}
```



---

archive/issue_events_396178.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-28T04:33:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396178"
}
```



---

archive/issue_events_396179.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-28T04:33:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396179"
}
```



---

archive/issue_comments_479489.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2022-08-28T08:56:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479489",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_396180.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-08-28T08:56:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396180"
}
```



---

archive/issue_events_396181.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-08-28T08:56:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396181"
}
```



---

archive/issue_events_396182.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-08-28T09:02:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396182"
}
```



---

archive/issue_events_396183.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-08-28T09:02:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396183"
}
```



---

archive/issue_events_396184.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-08-28T09:02:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30095#event-396184"
}
```



---

archive/issue_comments_479490.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nclosing as duplicate, to test the new trac machine",
    "created_at": "2022-08-28T09:02:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30095#issuecomment-479490",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:16" align="right">Comment 16</div>

closing as duplicate, to test the new trac machine
