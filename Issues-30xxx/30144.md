...
```
Currently, there are reliability issues on [GitHub](../wikis/GitHub) Actions, builds time out or die for no apparent reason, and then no docker images are pushed. 
...
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,5 +1,7 @@\n-Currently, some builds on [GitHub](../wikis/GitHub) Actions time out and then no docker images are pushed. \n+Currently, there are reliability issues on [GitHub](../wikis/GitHub) Actions, builds time out or die for no apparent reason, and then no docker images are pushed. \n \n We integrate this code into `tox.ini` so that the pushing happens after each of the steps defined by `DOCKER_TARGETS`.\n \n+Then one can pull an intermediate image and continue some tests locally.\n \n+\n``````\n",
    "created_at": "2020-08-12T22:40:04Z",
    "issue": "https://github.com/sagemath/sage/issues/30144",
    "type": "issue_comment",
