# Issue 30297: Elliptic curve lift_x with extend for p-adics needs names argument

archive/issues_030060.json:
```json
{
    "assignees": [],
    "body": "Currently\n\n```\n            sage: K = Qp(5)\n            sage: E = EllipticCurve([K(0), K(1)])\n            sage: E.lift_x(1, extend=True)\n```\nfails as the `extension` method for p-adic fields takes prec as a second argument, so add the argument name `names` explicitly.\n\nKeywords: **padics, elliptic curves**\n\nBranch/Commit: **[43d5d33](https://github.com/sagemath/sagetrac-mirror/commit/43d5d33ad310ed9e67ec29088e39e43763dde0bf)**\n\nReviewer: **Markus Wageringel**\n\nAuthor: **Alex J. Best**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30297_\n\n",
    "closed_at": "2020-08-23T17:50:57Z",
    "created_at": "2020-08-05T20:46:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Elliptic curve lift_x with extend for p-adics needs names argument",
    "type": "issue",
    "updated_at": "2020-08-23T17:50:57Z",
    "url": "https://github.com/sagemath/sage/issues/30297",
    "user": "https://github.com/alexjbest"
}
```
Currently

```
            sage: K = Qp(5)
            sage: E = EllipticCurve([K(0), K(1)])
            sage: E.lift_x(1, extend=True)
```
fails as the `extension` method for p-adic fields takes prec as a second argument, so add the argument name `names` explicitly.

Keywords: **padics, elliptic curves**

Branch/Commit: **[43d5d33](https://github.com/sagemath/sagetrac-mirror/commit/43d5d33ad310ed9e67ec29088e39e43763dde0bf)**

Reviewer: **Markus Wageringel**

Author: **Alex J. Best**

_Issue created by migration from https://trac.sagemath.org/ticket/30297_





---

archive/issue_events_399086.json:
```json
{
    "actor": "https://github.com/alexjbest",
    "created_at": "2020-08-05T20:46:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30297#event-399086"
}
```



---

archive/issue_events_399087.json:
```json
{
    "actor": "https://github.com/alexjbest",
    "created_at": "2020-08-05T20:46:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30297#event-399087"
}
```



---

archive/issue_events_399088.json:
```json
{
    "actor": "https://github.com/alexjbest",
    "created_at": "2020-08-05T20:46:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30297#event-399088"
}
```



---

archive/issue_events_399089.json:
```json
{
    "actor": "https://github.com/alexjbest",
    "created_at": "2020-08-05T20:46:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30297#event-399089"
}
```



---

archive/issue_comments_483605.json:
```json
{
    "body": "Branch: **[u/alexjbest/padic-extension-lift](https://github.com/sagemath/sagetrac-mirror/tree/u/alexjbest/padic-extension-lift)**",
    "created_at": "2020-08-05T20:47:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30297#issuecomment-483605",
    "user": "https://github.com/alexjbest"
}
```

Branch: **[u/alexjbest/padic-extension-lift](https://github.com/sagemath/sagetrac-mirror/tree/u/alexjbest/padic-extension-lift)**



---

archive/issue_comments_483606.json:
```json
{
    "body": "Commit: **[3c31df5](https://github.com/sagemath/sagetrac-mirror/commit/3c31df5df9baffb0fd92b69a912c4861e255e55f)**",
    "created_at": "2020-08-05T20:47:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30297#issuecomment-483606",
    "user": "https://github.com/alexjbest"
}
```

Commit: **[3c31df5](https://github.com/sagemath/sagetrac-mirror/commit/3c31df5df9baffb0fd92b69a912c4861e255e55f)**



---

archive/issue_events_399090.json:
```json
{
    "actor": "https://github.com/alexjbest",
    "created_at": "2020-08-05T20:47:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30297#event-399090"
}
```



---

archive/issue_comments_483607.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3c31df5df9baffb0fd92b69a912c4861e255e55f\">3c31df5</a></td><td><code>add names argument explicitly</code></td></tr></table>\n",
    "created_at": "2020-08-05T20:47:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30297#issuecomment-483607",
    "user": "https://github.com/alexjbest"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3c31df5df9baffb0fd92b69a912c4861e255e55f">3c31df5</a></td><td><code>add names argument explicitly</code></td></tr></table>




---

archive/issue_comments_483608.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/43d5d33ad310ed9e67ec29088e39e43763dde0bf\">43d5d33</a></td><td><code>add ticket number</code></td></tr></table>\n",
    "created_at": "2020-08-05T20:48:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30297#issuecomment-483608",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/43d5d33ad310ed9e67ec29088e39e43763dde0bf">43d5d33</a></td><td><code>add ticket number</code></td></tr></table>




---

archive/issue_comments_483609.json:
```json
{
    "body": "Changed commit from **[3c31df5](https://github.com/sagemath/sagetrac-mirror/commit/3c31df5df9baffb0fd92b69a912c4861e255e55f)** to **[43d5d33](https://github.com/sagemath/sagetrac-mirror/commit/43d5d33ad310ed9e67ec29088e39e43763dde0bf)**",
    "created_at": "2020-08-05T20:48:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30297#issuecomment-483609",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3c31df5](https://github.com/sagemath/sagetrac-mirror/commit/3c31df5df9baffb0fd92b69a912c4861e255e55f)** to **[43d5d33](https://github.com/sagemath/sagetrac-mirror/commit/43d5d33ad310ed9e67ec29088e39e43763dde0bf)**



---

archive/issue_events_399091.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-13T19:43:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30297#event-399091"
}
```



---

archive/issue_events_399092.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-13T19:43:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30297#event-399092"
}
```



---

archive/issue_comments_483610.json:
```json
{
    "body": "Reviewer: **Markus Wageringel**",
    "created_at": "2020-08-13T19:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30297#issuecomment-483610",
    "user": "https://github.com/mwageringel"
}
```

Reviewer: **Markus Wageringel**



---

archive/issue_comments_483611.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOk, this looks good to me.",
    "created_at": "2020-08-13T19:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30297#issuecomment-483611",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:4" align="right">Comment 4</div>

Ok, this looks good to me.



---

archive/issue_events_399093.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-08-23T17:50:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30297#event-399093"
}
```



---

archive/issue_events_399094.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4bca77e5a032ebac41e598b80313859c48c4792b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-08-23T17:50:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30297#event-399094"
}
```



---

archive/issue_comments_483612.json:
```json
{
    "body": "Changed branch from **[u/alexjbest/padic-extension-lift](https://github.com/sagemath/sagetrac-mirror/tree/u/alexjbest/padic-extension-lift)** to **[43d5d33](https://github.com/sagemath/sagetrac-mirror/commit/43d5d33ad310ed9e67ec29088e39e43763dde0bf)**",
    "created_at": "2020-08-23T17:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30297#issuecomment-483612",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/alexjbest/padic-extension-lift](https://github.com/sagemath/sagetrac-mirror/tree/u/alexjbest/padic-extension-lift)** to **[43d5d33](https://github.com/sagemath/sagetrac-mirror/commit/43d5d33ad310ed9e67ec29088e39e43763dde0bf)**
