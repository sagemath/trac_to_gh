...
```json
{
    "body": "<a id='comment:3'></a>\nI wonder a little bit if `OrderedDict` is really the best data structure. It feels like it really should be a `set`, but I guess `OrderedDict` has more guaranteed consistency across different sessions. Note that iterating over a `set` is actually faster:\n\n```\nsage: from collections import OrderedDict\nsage: d = OrderedDict()\nsage: for i in range(10):\n....:     d[i] = None\nsage: x = set(range(10))\n\nsage: def test_iter(X):\n....:     for i in X:\n....:         pass\n....: def test_access(X):\n....:     for i in range(20):\n....:         if i in X:\n....:             pass\n\nsage: %timeit test_iter(d)\n1000000 loops, best of 5: 293 ns per loop\nsage: %timeit test_iter(x)\n10000000 loops, best of 5: 173 ns per loop\n\nsage: %timeit test_access(d)\n1000000 loops, best of 5: 786 ns per loop\nsage: %timeit test_access(x)\n1000000 loops, best of 5: 761 ns per loop\n```\nAlthough this micro-optimization probably doesn't matter too much.\n\n---\nNew commits:\n|                                                                                                                                          |                                                                                                   |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------|\n|[134da39](https://git.sagemath.org/sage.git/commit?id=134da3942bf306fb2d484046f324c3fb067d9f20)|`sage.symbolic.assumptions.GenericDeclaration.assume: Validate against cached valid features first`|\n|[8b31261](https://git.sagemath.org/sage.git/commit?id=8b31261d0be43e63bccc72a59cae67510ecf6a24)|`sage.symbolic.assumptions.GenericDeclaration: Make it a UniqueRepresentation`|\n|[51ea23d](https://git.sagemath.org/sage.git/commit?id=51ea23d9a4d75d66ddef7b1670d14e5324909183)|`sage.symbolic.assumptions.GenericDeclaration.assume: Check first if already in _assumptions`|\n|[9119e82](https://git.sagemath.org/sage.git/commit?id=9119e82f95e36847383ca29a9d5a96376c35fde0)|`sage.symbolic.assumptions._assumptions: Change from list to OrderDict`|\n|[cda81ff](https://git.sagemath.org/sage.git/commit?id=cda81ff3e8a54f4ec09fecd04a0673866736940a)|`sage.symbolic.assumptions.GenericDeclaration: Make it hashable by inheriting comparisons from UniqueRepresentation`|",
    "created_at": "2020-07-07T01:06:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30074",
    "type": "issue_comment",
...
```json
{
    "body": "<a id='comment:10'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                             |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------|\n|[642836d](https://git.sagemath.org/sage.git/commit/?id=642836dc080f10de1da5a502ccb8bad19113c46b)|`sage.symbolic.assumptions: Use dict instead of OrderedDict for _assumptions`|",
...
```json
{
    "body": "<a id='comment:16'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                     |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------|\n|[9e0cbed](https://git.sagemath.org/sage.git/commit/?id=9e0cbed2ca26f85160ad363c2b298cb109822ffe)|`src/sage/symbolic/assumptions.py: Fix style warning`|",
...
```json
{
    "body": "<a id='comment:19'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                        |\n|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|\n|[4adcb20](https://git.sagemath.org/sage.git/commit/?id=4adcb204aca95916a01535ba4c3a98ec0d13d39d)|`src/sage/symbolic/assumptions.py: Remove unused import`|",
