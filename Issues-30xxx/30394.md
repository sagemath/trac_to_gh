...
```json
{
    "body": "also, the current construction for antipodal graphs, `G = self.distance_graph(self.diameter())`, contains all the vertices of `G`. Is it correct according the definition of antipodal graphs ?",
...
```json
{
    "body": "to get the same vertex set, it suffices to use `Graph([G, E], ...)`.",
...
```json
{
    "body": "let us limit this ticket to the distance regular case, and deal with a general case on another ticket.",
...
```json
{
    "body": "I pushed minor corrections. In particular, for the mapping int -> cliques, we can simply use a list.",
...
```json
{
    "body": "Nothing major, just a conflict with #30355",
