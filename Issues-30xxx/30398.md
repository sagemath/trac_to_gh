...
<a id='comment:1'></a>I like the idea :-) We are indeed reaching the time where sage-explorer
could reach prime time. It has little dependencies and is unlikely
to break anything except itself, so it does not  add much technical debt
to Sage. Also it's the kind of tool that is useful if you have it right
under hand. So yeah, standard package makes sense. Maybe after a test
...
```json
{
    "body": "<a id='comment:2'></a>Does it work when the Sage jupyter kernel is used with a system notebook server? (see #30306)",
...
<a id='comment:2'></a>Does it work when the Sage jupyter kernel is used with a system notebook server? (see #30306)
