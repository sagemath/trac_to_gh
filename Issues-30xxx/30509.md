# Issue 30509: Graphs: Faster implementation for HalfCube

archive/issues_030272.json:
```json
{
    "assignees": [],
    "body": "Implementation of a faster method to construct the graph `HalfCube`\n\nDepends on #30329\n\nCC:  @dimpase @dcoudert\n\nBranch/Commit: **[898fbee](https://github.com/sagemath/sagetrac-mirror/commit/898fbeed7386582544f49f680b03cd3853985fbe)**\n\nReviewer: **David Coudert**\n\nAuthor: **Ivo Maffei**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30509_\n\n",
    "closed_at": "2020-09-23T21:27:31Z",
    "created_at": "2020-09-05T12:09:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Graphs: Faster implementation for HalfCube",
    "type": "issue",
    "updated_at": "2020-09-23T21:27:31Z",
    "url": "https://github.com/sagemath/sage/issues/30509",
    "user": "https://github.com/Ivo-Maffei"
}
```
Implementation of a faster method to construct the graph `HalfCube`

Depends on #30329

CC:  @dimpase @dcoudert

Branch/Commit: **[898fbee](https://github.com/sagemath/sagetrac-mirror/commit/898fbeed7386582544f49f680b03cd3853985fbe)**

Reviewer: **David Coudert**

Author: **Ivo Maffei**

_Issue created by migration from https://trac.sagemath.org/ticket/30509_





---

archive/issue_events_402335.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-09-05T12:09:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30509#event-402335"
}
```



---

archive/issue_events_402336.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-09-05T12:09:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30509#event-402336"
}
```



---

archive/issue_events_402337.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-09-05T12:09:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30509#event-402337"
}
```



---

archive/issue_events_402338.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-09-05T12:09:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30509#event-402338"
}
```



---

archive/issue_comments_489291.json:
```json
{
    "body": "Dependencies: **#30329**",
    "created_at": "2020-09-05T12:14:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489291",
    "user": "https://github.com/Ivo-Maffei"
}
```

Dependencies: **#30329**



---

archive/issue_comments_489292.json:
```json
{
    "body": "Branch: **[public/graphs/30509](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/30509)**",
    "created_at": "2020-09-05T12:14:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489292",
    "user": "https://github.com/Ivo-Maffei"
}
```

Branch: **[public/graphs/30509](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/30509)**



---

archive/issue_comments_489293.json:
```json
{
    "body": "Commit: **[3024980](https://github.com/sagemath/sagetrac-mirror/commit/3024980467f73b6397ad70788aecabeea76d8770)**",
    "created_at": "2020-09-05T12:14:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489293",
    "user": "https://github.com/Ivo-Maffei"
}
```

Commit: **[3024980](https://github.com/sagemath/sagetrac-mirror/commit/3024980467f73b6397ad70788aecabeea76d8770)**



---

archive/issue_events_402339.json:
```json
{
    "actor": "https://github.com/Ivo-Maffei",
    "created_at": "2020-09-05T12:14:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30509#event-402339"
}
```



---

archive/issue_comments_489294.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nTo convert the vertices of `CubeGraph` to integers one can use `G.relabel(perm=int)` as the vertices are strings and `int(str)` will convert `str` to its integer representation.\n\nHowever, doing so causes an overflow error (not sure why) when we iterate over `G`.\nOne can simply iterate over `range(2**(n-1))`, but I think the construction in this branch is even faster as everything happens in Cython without calling `CubeGraph`.\n\n---\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/97f045062e412beaeaeec5abf2bcceb9030140e7\">97f0450</a></td><td><code>Merge branch 'develop' into t/30312</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a1861d7e67604f3b9cd7944d10f48356ef99b55\">2a1861d</a></td><td><code>Merge branch 't/30312' into t/30329</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5344f35562db78a3456124f2af0b7ffbd2a47d5a\">5344f35</a></td><td><code>added long time flags and fixed van Lint name</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9e69f0dc82fe102a853bb3cefe6c874c094227c2\">9e69f0d</a></td><td><code>missing # long time</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/49cdcc1c74dfef9f4a422d0175c211fcfd214309\">49cdcc1</a></td><td><code>Merge branch 't/30312' into t/30329</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ea30040d9d0726b18435cb94793d61bf525c5b5e\">ea30040</a></td><td><code>added long time flags</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4bcdb3ce885b32356fcf66fa7a14a1c9f940d572\">4bcdb3c</a></td><td><code>typos and TeX fixes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/feabf4fd56996341180d1eacf0dbf859e8d426c8\">feabf4f</a></td><td><code>better index section, uniform naming</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1fdcad9f649697e6a6e5fa168518df6a2152b21c\">1fdcad9</a></td><td><code>fixed typo</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3024980467f73b6397ad70788aecabeea76d8770\">3024980</a></td><td><code>faster implementation of HalfCube</code></td></tr></table>\n",
    "created_at": "2020-09-05T12:14:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489294",
    "user": "https://github.com/Ivo-Maffei"
}
```

<div id="comment:1" align="right">Comment 1</div>

To convert the vertices of `CubeGraph` to integers one can use `G.relabel(perm=int)` as the vertices are strings and `int(str)` will convert `str` to its integer representation.

However, doing so causes an overflow error (not sure why) when we iterate over `G`.
One can simply iterate over `range(2**(n-1))`, but I think the construction in this branch is even faster as everything happens in Cython without calling `CubeGraph`.

---
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/97f045062e412beaeaeec5abf2bcceb9030140e7">97f0450</a></td><td><code>Merge branch 'develop' into t/30312</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a1861d7e67604f3b9cd7944d10f48356ef99b55">2a1861d</a></td><td><code>Merge branch 't/30312' into t/30329</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5344f35562db78a3456124f2af0b7ffbd2a47d5a">5344f35</a></td><td><code>added long time flags and fixed van Lint name</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9e69f0dc82fe102a853bb3cefe6c874c094227c2">9e69f0d</a></td><td><code>missing # long time</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/49cdcc1c74dfef9f4a422d0175c211fcfd214309">49cdcc1</a></td><td><code>Merge branch 't/30312' into t/30329</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ea30040d9d0726b18435cb94793d61bf525c5b5e">ea30040</a></td><td><code>added long time flags</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4bcdb3ce885b32356fcf66fa7a14a1c9f940d572">4bcdb3c</a></td><td><code>typos and TeX fixes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/feabf4fd56996341180d1eacf0dbf859e8d426c8">feabf4f</a></td><td><code>better index section, uniform naming</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1fdcad9f649697e6a6e5fa168518df6a2152b21c">1fdcad9</a></td><td><code>fixed typo</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3024980467f73b6397ad70788aecabeea76d8770">3024980</a></td><td><code>faster implementation of HalfCube</code></td></tr></table>




---

archive/issue_comments_489295.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nIt's of course faster to not call `CubeGraph`. The drawback is to lose the placement of vertices done in `CubeGraph` (which is certainly the most expensive part of the method).\n\na minor improvement:\n\n```diff\n-    G = Graph(E, format='list_of_edges')\n+    G = Graph([range(2**(n - 1)), E], format='vertices_and_edges')\n```\nalso, may be it's enough to put the `sig_check` before the `for i...` loop ?",
    "created_at": "2020-09-05T12:21:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489295",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:2" align="right">Comment 2</div>

It's of course faster to not call `CubeGraph`. The drawback is to lose the placement of vertices done in `CubeGraph` (which is certainly the most expensive part of the method).

a minor improvement:

```diff
-    G = Graph(E, format='list_of_edges')
+    G = Graph([range(2**(n - 1)), E], format='vertices_and_edges')
```
also, may be it's enough to put the `sig_check` before the `for i...` loop ?



---

archive/issue_comments_489296.json:
```json
{
    "body": "Changed commit from **[3024980](https://github.com/sagemath/sagetrac-mirror/commit/3024980467f73b6397ad70788aecabeea76d8770)** to **[898fbee](https://github.com/sagemath/sagetrac-mirror/commit/898fbeed7386582544f49f680b03cd3853985fbe)**",
    "created_at": "2020-09-06T10:21:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489296",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3024980](https://github.com/sagemath/sagetrac-mirror/commit/3024980467f73b6397ad70788aecabeea76d8770)** to **[898fbee](https://github.com/sagemath/sagetrac-mirror/commit/898fbeed7386582544f49f680b03cd3853985fbe)**



---

archive/issue_comments_489297.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/898fbeed7386582544f49f680b03cd3853985fbe\">898fbee</a></td><td><code>added positions to HalfCube</code></td></tr></table>\n",
    "created_at": "2020-09-06T10:21:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489297",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/898fbeed7386582544f49f680b03cd3853985fbe">898fbee</a></td><td><code>added positions to HalfCube</code></td></tr></table>




---

archive/issue_events_402340.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-09-06T12:58:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30509#event-402340"
}
```



---

archive/issue_events_402341.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-09-06T12:58:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30509#event-402341"
}
```



---

archive/issue_comments_489298.json:
```json
{
    "body": "Author: **Ivo Maffei**",
    "created_at": "2020-09-06T12:58:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489298",
    "user": "https://github.com/dcoudert"
}
```

Author: **Ivo Maffei**



---

archive/issue_comments_489299.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2020-09-06T12:58:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489299",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_comments_489300.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLGTM, and the plots are nice.",
    "created_at": "2020-09-06T12:58:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489300",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:4" align="right">Comment 4</div>

LGTM, and the plots are nice.



---

archive/issue_comments_489301.json:
```json
{
    "body": "Changed branch from **[public/graphs/30509](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/30509)** to **[898fbee](https://github.com/sagemath/sagetrac-mirror/commit/898fbeed7386582544f49f680b03cd3853985fbe)**",
    "created_at": "2020-09-23T21:27:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30509#issuecomment-489301",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/30509](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/30509)** to **[898fbee](https://github.com/sagemath/sagetrac-mirror/commit/898fbeed7386582544f49f680b03cd3853985fbe)**



---

archive/issue_events_402342.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-09-23T21:27:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30509#event-402342"
}
```



---

archive/issue_events_402343.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "16b10897433a03c7c021b7d04ab3fef07a25e88f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-09-23T21:27:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30509",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30509#event-402343"
}
```
