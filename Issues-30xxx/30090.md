# Issue 30090: Implemented test function for BIBDs

archive/issues_029853.json:
```json
{
    "body": "Implemented a function to test the recursive construction given in the method `balanced_incomplete_block_design`.\n\nCC:  @dimpase\n\nKeywords: bibd\n\nAuthor: Ivo Maffei\n\nBranch: u/gh-Ivo-Maffei/bibd_tests\n\nStatus: new\n\nDependencies: #30037\n\nCommit: 675378180b8201601fcfa15226a0a2dcd7c7a2f1\n\nIssue created by migration from https://trac.sagemath.org/ticket/30090\n\n",
    "created_at": "2020-07-08T14:12:07Z",
    "labels": [
        "component: combinatorial designs"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Implemented test function for BIBDs",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/30090",
    "user": "https://github.com/Ivo-Maffei"
}
```
Implemented a function to test the recursive construction given in the method `balanced_incomplete_block_design`.

CC:  @dimpase

Keywords: bibd

Author: Ivo Maffei

Branch: u/gh-Ivo-Maffei/bibd_tests

Status: new

Dependencies: #30037

Commit: 675378180b8201601fcfa15226a0a2dcd7c7a2f1

Issue created by migration from https://trac.sagemath.org/ticket/30090





---

archive/issue_comments_592095.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis function takes in a maximum value for `v` and a range for `lambda` and then tests all possible BIBDs that can arise from the recursive construction.\nI'm not sure if this is what you meant, or if you were thinking at a more general function.",
    "created_at": "2020-07-08T14:14:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30090#issuecomment-592095",
    "user": "https://github.com/Ivo-Maffei"
}
```

<a id='comment:1'></a>
This function takes in a maximum value for `v` and a range for `lambda` and then tests all possible BIBDs that can arise from the recursive construction.
I'm not sure if this is what you meant, or if you were thinking at a more general function.



---

archive/issue_comments_592096.json:
```json
{
    "body": "<a id='comment:2'></a>\nI meant testing every construction available, as if you build a full database of (v,k,l) for which Sage does/does not know how to build a design.",
    "created_at": "2020-07-08T14:39:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30090#issuecomment-592096",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:2'></a>
I meant testing every construction available, as if you build a full database of (v,k,l) for which Sage does/does not know how to build a design.



---

archive/issue_comments_592097.json:
```json
{
    "body": "Changing commit from \"5982cc82749c77d5193de24a8b4a50c4a05361ce\" to \"675378180b8201601fcfa15226a0a2dcd7c7a2f1\"",
    "created_at": "2020-07-09T21:27:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30090#issuecomment-592097",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "5982cc82749c77d5193de24a8b4a50c4a05361ce" to "675378180b8201601fcfa15226a0a2dcd7c7a2f1"



---

archive/issue_comments_592098.json:
```json
{
    "body": "<a id='comment:3'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                                |\n|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------|\n|[aa99d3e](https://github.com/sagemath/sagetrac-mirror/commit/aa99d3e8899fd989fd43894ef659fa47c66b4afe)|`added a recursive construction`|\n|[d137495](https://github.com/sagemath/sagetrac-mirror/commit/d137495406b17534b9e4b3f858152a15b01ea8d1)|`fixed bug; added tests`|\n|[f689565](https://github.com/sagemath/sagetrac-mirror/commit/f6895657cc07c184442348c1488aaa157575ed4c)|`added function to test recursive construction`|\n|[6753781](https://github.com/sagemath/sagetrac-mirror/commit/675378180b8201601fcfa15226a0a2dcd7c7a2f1)|`skeleton of test function`|",
    "created_at": "2020-07-09T21:27:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30090#issuecomment-592098",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:3'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                                |
|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------|
|[aa99d3e](https://github.com/sagemath/sagetrac-mirror/commit/aa99d3e8899fd989fd43894ef659fa47c66b4afe)|`added a recursive construction`|
|[d137495](https://github.com/sagemath/sagetrac-mirror/commit/d137495406b17534b9e4b3f858152a15b01ea8d1)|`fixed bug; added tests`|
|[f689565](https://github.com/sagemath/sagetrac-mirror/commit/f6895657cc07c184442348c1488aaa157575ed4c)|`added function to test recursive construction`|
|[6753781](https://github.com/sagemath/sagetrac-mirror/commit/675378180b8201601fcfa15226a0a2dcd7c7a2f1)|`skeleton of test function`|



---

archive/issue_comments_592099.json:
```json
{
    "body": "<a id='comment:4'></a>\nI rebased the branch to Sage 9.2.beta4.\nI know the docstring is wrong and the function now doesn't work (see #30102), but I would like to know if this is a good structure for the function.\n\nThe idea is to test whether Sage can build the BIBDs Sage claims to be able to build, then to test that there are not parameters that Sage is able to build but claims it can't.\n\nIt should be a quite comprehensive test, but it feels like the second part is going to be hard to maintain.\nI'm not sure that's the best approach. I'm not sure how much I should trust sage and the existing code and how much needs to be checked in the test function.",
    "created_at": "2020-07-09T21:37:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30090#issuecomment-592099",
    "user": "https://github.com/Ivo-Maffei"
}
```

<a id='comment:4'></a>
I rebased the branch to Sage 9.2.beta4.
I know the docstring is wrong and the function now doesn't work (see #30102), but I would like to know if this is a good structure for the function.

The idea is to test whether Sage can build the BIBDs Sage claims to be able to build, then to test that there are not parameters that Sage is able to build but claims it can't.

It should be a quite comprehensive test, but it feels like the second part is going to be hard to maintain.
I'm not sure that's the best approach. I'm not sure how much I should trust sage and the existing code and how much needs to be checked in the test function.



---

archive/issue_events_087350.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87350"
}
```



---

archive/issue_events_087351.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87351"
}
```



---

archive/issue_events_087352.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87352"
}
```



---

archive/issue_comments_592100.json:
```json
{
    "body": "<a id='comment:6'></a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30090#issuecomment-592100",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'></a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_087353.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87353"
}
```



---

archive/issue_events_087354.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87354"
}
```



---

archive/issue_events_087355.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87355"
}
```



---

archive/issue_events_087356.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87356"
}
```



---

archive/issue_events_087357.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87357"
}
```



---

archive/issue_events_087358.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87358"
}
```



---

archive/issue_events_087359.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87359"
}
```



---

archive/issue_events_087360.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30090",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30090#event-87360"
}
```
