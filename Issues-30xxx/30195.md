...
```json
{
    "body": "Branch is 1 commit on top of #29929, which also needs review\n\n---\nLast 10 new commits:",
    "created_at": "2020-07-22T04:08:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30195",
    "type": "issue_comment",
...
```json
{
    "body": "They should work, just add them to tox.yml for testing. I haven't tested them widely yet, only one run of `docker-manylinux-2014-standard` on my computer. Best to test together with #30173",
...
```json
{
    "body": "I'll look into the tox config error first.",
...
```json
{
    "body": "On `manylinux-2010-minimal` (https://github.com/kliem/sage/runs/899346056), building python3 fails with `./python: error while loading shared libraries: libcrypt.so.2: cannot open shared object file: No such file or directory` (related to #29012?)\n... but actually it was intended to use the installed python3 in `/opt`. Looks like I need to make adjustments for `-2010`.",
...
```json
{
    "body": "When you have a chance, please test this version again. See above on my comments on the supported tox environments. Also note the images for non-x86 archs can't work on Linux Docker (and thus on GH Actions) because they are not multi-arch images. They work on macOS Docker Desktop because they are running inside of an emulator.",
...
```json
{
    "body": "I'm getting configure failures with minimal. Will this be a subject of this ticket or of another one?\n\nOtherwise it's looking fine.",
...
```json
{
    "body": "`minimal` with `python3.7` cannot work because python3 dependencies such as sqlite3 are not available from the system and therefore it cannot use system python3.",
