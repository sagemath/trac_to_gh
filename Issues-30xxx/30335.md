# Issue 30335: openblas: clear out .pc symlinks before installation

archive/issues_030098.json:
```json
{
    "assignees": [],
    "body": "When switching from system BLAS to SPKG openblas, the following errors have been reported:\n\n```\nCopying package files from temporary location /Users/yzh/sagecopies/sage/worktree-2020/local/var/tmp/sage/build/openblas-0.3.9/inst to /Users/yzh/sagecopies/sage/worktree-2020/local\ncp: /Users/yzh/sagecopies/sage/worktree-2020/local/./lib/pkgconfig/cblas.pc: Permission denied\ncp: /Users/yzh/sagecopies/sage/worktree-2020/local/./lib/pkgconfig/blas.pc: Permission denied\ncp: /Users/yzh/sagecopies/sage/worktree-2020/local/./lib/pkgconfig/lapack.pc: Permission denied\n************************************************************************\nError copying files for openblas-0.3.9.\n```\n\nThe correct fix is described in [#29585 comment:9](https://github.com/sagemath/sage/issues/29585#comment:9)\nand #29387 + #29386\n\n\nThis ticket is a quick workaround for Sage 9.2.\n\n\n\nCC:  @yuan-zhou @jhpalmieri @dimpase @embray\n\nBranch: **[29b1ff8](https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30335_\n\n",
    "closed_at": "2020-08-14T22:23:50Z",
    "created_at": "2020-08-11T15:29:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "openblas: clear out .pc symlinks before installation",
    "type": "issue",
    "updated_at": "2020-08-31T16:31:04Z",
    "url": "https://github.com/sagemath/sage/issues/30335",
    "user": "https://github.com/mkoeppe"
}
```
When switching from system BLAS to SPKG openblas, the following errors have been reported:

```
Copying package files from temporary location /Users/yzh/sagecopies/sage/worktree-2020/local/var/tmp/sage/build/openblas-0.3.9/inst to /Users/yzh/sagecopies/sage/worktree-2020/local
cp: /Users/yzh/sagecopies/sage/worktree-2020/local/./lib/pkgconfig/cblas.pc: Permission denied
cp: /Users/yzh/sagecopies/sage/worktree-2020/local/./lib/pkgconfig/blas.pc: Permission denied
cp: /Users/yzh/sagecopies/sage/worktree-2020/local/./lib/pkgconfig/lapack.pc: Permission denied
************************************************************************
Error copying files for openblas-0.3.9.
```

The correct fix is described in [#29585 comment:9](https://github.com/sagemath/sage/issues/29585#comment:9)
and #29387 + #29386


This ticket is a quick workaround for Sage 9.2.



CC:  @yuan-zhou @jhpalmieri @dimpase @embray

Branch: **[29b1ff8](https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/30335_





---

archive/issue_events_386005.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-11T15:29:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30335#event-386005"
}
```



---

archive/issue_events_386006.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-11T15:29:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30335#event-386006"
}
```



---

archive/issue_events_386007.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-11T15:29:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30335#event-386007"
}
```



---

archive/issue_events_386008.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-11T15:29:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30335#event-386008"
}
```



---

archive/issue_comments_484526.json:
```json
{
    "body": "Branch: **[u/mkoeppe/openblas__clear_out__pc_symlinks_before_installation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/openblas__clear_out__pc_symlinks_before_installation)**",
    "created_at": "2020-08-11T15:34:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484526",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/openblas__clear_out__pc_symlinks_before_installation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/openblas__clear_out__pc_symlinks_before_installation)**



---

archive/issue_comments_484527.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4\">29b1ff8</a></td><td><code>build/pkgs/openblas/spkg-install.in: Clear out possible .pc symlinks before installation</code></td></tr></table>\n",
    "created_at": "2020-08-11T16:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484527",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4">29b1ff8</a></td><td><code>build/pkgs/openblas/spkg-install.in: Clear out possible .pc symlinks before installation</code></td></tr></table>




---

archive/issue_comments_484528.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-08-11T16:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484528",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_484529.json:
```json
{
    "body": "Commit: **[29b1ff8](https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4)**",
    "created_at": "2020-08-11T16:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484529",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[29b1ff8](https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4)**



---

archive/issue_events_386009.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-11T16:00:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30335#event-386009"
}
```



---

archive/issue_comments_484530.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-08-11T16:14:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484530",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_386010.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-08-11T16:14:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30335#event-386010"
}
```



---

archive/issue_events_386011.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-08-11T16:14:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30335#event-386011"
}
```



---

archive/issue_comments_484531.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nlgtm",
    "created_at": "2020-08-11T16:14:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484531",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:3'>Comment 3:</a>
lgtm



---

archive/issue_comments_484532.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks!",
    "created_at": "2020-08-11T17:11:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484532",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks!



---

archive/issue_comments_484533.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/openblas__clear_out__pc_symlinks_before_installation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/openblas__clear_out__pc_symlinks_before_installation)** to **[29b1ff8](https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4)**",
    "created_at": "2020-08-14T22:23:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484533",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/openblas__clear_out__pc_symlinks_before_installation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/openblas__clear_out__pc_symlinks_before_installation)** to **[29b1ff8](https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4)**



---

archive/issue_events_386012.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-08-14T22:23:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30335#event-386012"
}
```



---

archive/issue_events_386013.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "04c64492a6cf4bbdac0c1f58782cb1fb620e4457",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-08-14T22:23:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30335#event-386013"
}
```



---

archive/issue_comments_484534.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAh, no, spkg-install scripts should **not** make changes to `$SAGE_LOCAL`.  If it's absolutely necessary to you can do that in an `spkg-preinst` script.",
    "created_at": "2020-08-31T15:46:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484534",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'>Comment 6:</a>
Ah, no, spkg-install scripts should **not** make changes to `$SAGE_LOCAL`.  If it's absolutely necessary to you can do that in an `spkg-preinst` script.



---

archive/issue_comments_484535.json:
```json
{
    "body": "Changed commit from **[29b1ff8](https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4)** to none",
    "created_at": "2020-08-31T15:46:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484535",
    "user": "https://github.com/embray"
}
```

Changed commit from **[29b1ff8](https://github.com/sagemath/sagetrac-mirror/commit/29b1ff8f2c4ee85a3661da3160e340d3d404f6f4)** to none



---

archive/issue_comments_484536.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nFollow-up: #30480",
    "created_at": "2020-08-31T16:31:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30335#issuecomment-484536",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'>Comment 7:</a>
Follow-up: #30480
