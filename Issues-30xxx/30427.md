...
```json
{
    "body": "Some fix should be easy. Just use `try ... except ... finally` to properly clean the list at failure.\n\nHowever, I don't understand entirely how `cdef list` is supposed to behave.",
...
```json
{
    "body": "So apparently the exit code of cython still figures out that all those `<PyObject *>` that we created and appended to the list must be deleted. However, once we interrupt once in our sage session, this changes permanently.",
