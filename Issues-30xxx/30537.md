...
<a id='comment:18'></a>I can no longer reproduce the crashes mentioned in comment:1 with 1.6.0-25. The tests still fail, though
...
<a id='comment:21'></a>Replying to [comment:20 arojas]:
> Replying to [comment:19 fbissey]:
> > I still have the crash in giac's testsuite with 1.6.0.25.
...
<a id='comment:29'></a>Replying to [comment:28 parisse]:
> Well, I do not check --disable-gui, and there is indeed a small change to make in icas.cc to be able to compile, at the beginning of main
> 
> 
> ```
...
<a id='comment:31'></a>Replying to [comment:30 parisse]:
> No, I have now added a --disable-micropy option in configure, will be in 1.6.0-43, and isalready available here if you want to check
> [https://www-fourier.univ-grenoble-alpes.fr/~parisse/giac/giac-1.6.0.tar.bz2](https://www-fourier.univ-grenoble-alpes.fr/~parisse/giac/giac-1.6.0.tar.bz2)
...
```json
{
    "body": "<a id='comment:44'></a>The test failures at comment:18 are still not addressed AFAICS",
...
<a id='comment:44'></a>The test failures at comment:18 are still not addressed AFAICS
...
<a id='comment:64'></a>Replying to [comment:63 parisse]:
> 2/ pari: what should I add in configure.ac?
...
<a id='comment:66'></a>Replying to [comment:63 parisse]:
> 3/ gbasis/eliminate: maybe, you can modify your test and add gbasis as additional argument to your command. By default, eliminate does not compute a gbasis if there is only 1 variable to eliminate, resultant is more efficient.
> [https://dev.geogebra.org/trac/changeset/69735/](https://dev.geogebra.org/trac/changeset/69735/)
...
<a id='comment:67'></a>Replying to [comment:65 parisse]:
> But I don't know how to adapt this inside my configure.ac. Current code slice is
> 
> 
> ```
...
<a id='comment:72'></a>Replying to [comment:70 zimmerma]:
> on Ubuntu 18.04 (gcc 10.2.0) the compilation of Sage 9.2 fails on x86_64 while building giac-1.5.0.87, but the same error (internal compiler error) occurs with vanilla giac-1.6.0:
...
<a id='comment:88'></a>Replying to [comment:85 mkoeppe]:
> Don't we still want to support system giac 1.5? 
> Then we can't just change the doctest like this.
...
```json
{
    "body": "<a id='comment:91'></a>Replying to [comment:90 fbissey]:\n> do we still support some 1.5.0?\n\n\nI think we should",
...
<a id='comment:91'></a>Replying to [comment:90 fbissey]:
> do we still support some 1.5.0?
