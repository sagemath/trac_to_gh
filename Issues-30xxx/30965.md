# Issue 30965: fix precision in Padé method of power series

archive/issues_030728.json:
```json
{
    "assignees": [],
    "body": "as suggested by https://ask.sagemath.org/question/54385/power-series-method-pademn-off-by-1/\n\nCC:  @slel\n\nBranch/Commit: **[598298c](https://github.com/sagemath/sagetrac-mirror/commit/598298c3348273b92ef8f2c402581f15b0c12943)**\n\nReviewer: **Samuel Leli\u00e8vre**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30965_\n\n",
    "closed_at": "2020-12-05T22:12:55Z",
    "created_at": "2020-11-26T08:16:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix precision in Pad\u00e9 method of power series",
    "type": "issue",
    "updated_at": "2020-12-05T22:12:55Z",
    "url": "https://github.com/sagemath/sage/issues/30965",
    "user": "https://github.com/fchapoton"
}
```
as suggested by https://ask.sagemath.org/question/54385/power-series-method-pademn-off-by-1/

CC:  @slel

Branch/Commit: **[598298c](https://github.com/sagemath/sagetrac-mirror/commit/598298c3348273b92ef8f2c402581f15b0c12943)**

Reviewer: **Samuel Lelièvre**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/30965_





---

archive/issue_events_408806.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-11-26T08:16:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30965#event-408806"
}
```



---

archive/issue_events_408807.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-11-26T08:16:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30965#event-408807"
}
```



---

archive/issue_events_408808.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-11-26T08:16:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30965#event-408808"
}
```



---

archive/issue_events_408809.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-11-26T08:16:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30965#event-408809"
}
```



---

archive/issue_comments_499173.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/598298c3348273b92ef8f2c402581f15b0c12943\">598298c</a></td><td><code>allow lower precision in Pad\u00e9</code></td></tr></table>\n",
    "created_at": "2020-11-26T08:17:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30965#issuecomment-499173",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/598298c3348273b92ef8f2c402581f15b0c12943">598298c</a></td><td><code>allow lower precision in Padé</code></td></tr></table>




---

archive/issue_events_408810.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-11-26T08:17:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30965#event-408810"
}
```



---

archive/issue_comments_499174.json:
```json
{
    "body": "Commit: **[598298c](https://github.com/sagemath/sagetrac-mirror/commit/598298c3348273b92ef8f2c402581f15b0c12943)**",
    "created_at": "2020-11-26T08:17:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30965#issuecomment-499174",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[598298c](https://github.com/sagemath/sagetrac-mirror/commit/598298c3348273b92ef8f2c402581f15b0c12943)**



---

archive/issue_comments_499175.json:
```json
{
    "body": "Branch: **[u/chapoton/30965](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/30965)**",
    "created_at": "2020-11-26T08:17:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30965#issuecomment-499175",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/30965](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/30965)**



---

archive/issue_comments_499176.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\ngreen bot, please review, easy one",
    "created_at": "2020-11-26T12:55:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30965#issuecomment-499176",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

green bot, please review, easy one



---

archive/issue_comments_499177.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMerci Fr\u00e9d\u00e9ric.",
    "created_at": "2020-11-26T14:23:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30965#issuecomment-499177",
    "user": "https://github.com/slel"
}
```

<div id="comment:3" align="right">Comment 3</div>

Merci Frédéric.



---

archive/issue_comments_499178.json:
```json
{
    "body": "Reviewer: **Samuel Leli\u00e8vre**",
    "created_at": "2020-11-26T14:23:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30965#issuecomment-499178",
    "user": "https://github.com/slel"
}
```

Reviewer: **Samuel Lelièvre**



---

archive/issue_events_408811.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-11-26T14:23:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30965#event-408811"
}
```



---

archive/issue_events_408812.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-11-26T14:23:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30965#event-408812"
}
```



---

archive/issue_events_408813.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-12-05T22:12:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30965#event-408813"
}
```



---

archive/issue_events_408814.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c3d9bf77a6ed47cf768a7712d710767f09fb8993",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-12-05T22:12:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30965#event-408814"
}
```



---

archive/issue_comments_499179.json:
```json
{
    "body": "Changed branch from **[u/chapoton/30965](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/30965)** to **[598298c](https://github.com/sagemath/sagetrac-mirror/commit/598298c3348273b92ef8f2c402581f15b0c12943)**",
    "created_at": "2020-12-05T22:12:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30965#issuecomment-499179",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/30965](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/30965)** to **[598298c](https://github.com/sagemath/sagetrac-mirror/commit/598298c3348273b92ef8f2c402581f15b0c12943)**
