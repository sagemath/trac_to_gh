# Issue 30623: Cleanup for spkg-install scripts

archive/issues_030386.json:
```json
{
    "assignees": [],
    "body": "A lot of this boilerplate can be replaced by `sdh_guard`\n\n```\n$ git grep 'undefined.*exiting' build/pkgs/\nbuild/pkgs/attrs/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/benzene/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/buckygen/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/certifi/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/database_cremona_ellcurve/spkg-install.in:    echo >&2 \"SAGE_SHARE undefined ... exiting\"\nbuild/pkgs/database_jones_numfield/spkg-install.in:    echo >&2 \"SAGE_SHARE undefined ... exiting\"\nbuild/pkgs/database_mutation_class/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined database_mutation_class-1.0 exiting\"\nbuild/pkgs/database_stein_watkins/spkg-install.in:    echo >&2 \"SAGE_SHARE undefined ... exiting\"\nbuild/pkgs/fpylll/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/gap3/spkg-src.in:    echo >&2 \"Error - SAGE_ROOT undefined ... exiting\"\nbuild/pkgs/giac/spkg-src:   echo \"SAGE_LOCAL undefined ... exiting\";\nbuild/pkgs/glpk/spkg-src:    echo >&2 \"Error - SAGE_ROOT undefined ... exiting\"\nbuild/pkgs/importlib_metadata/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/jsonschema/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/jupyterlab_widgets/spkg-install:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/markupsafe/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/mistune/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/mpir/spkg-src:    echo >&2 \"SAGE_DISTFILES undefined ... exiting\"\nbuild/pkgs/nodejs/spkg-install:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/openssl/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/pyrsistent/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/python_igraph/spkg-src:   echo >&2 \"\\$SAGE_ROOT or \\$SAGE_DISTFILES undefined ... exiting\";\nbuild/pkgs/sagelib/spkg-src:    echo >&2 \"Error - SAGE_ROOT undefined ... exiting\"\nbuild/pkgs/surf/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/termcap/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/vcversioner/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\nbuild/pkgs/zipp/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n```\n\nCC:  @jhpalmieri\n\nKeywords: **sd111**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30623_\n\n",
    "created_at": "2020-09-21T06:18:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cleanup for spkg-install scripts",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/30623",
    "user": "https://github.com/mkoeppe"
}
```
A lot of this boilerplate can be replaced by `sdh_guard`

```
$ git grep 'undefined.*exiting' build/pkgs/
build/pkgs/attrs/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/benzene/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/buckygen/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/certifi/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/database_cremona_ellcurve/spkg-install.in:    echo >&2 "SAGE_SHARE undefined ... exiting"
build/pkgs/database_jones_numfield/spkg-install.in:    echo >&2 "SAGE_SHARE undefined ... exiting"
build/pkgs/database_mutation_class/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined database_mutation_class-1.0 exiting"
build/pkgs/database_stein_watkins/spkg-install.in:    echo >&2 "SAGE_SHARE undefined ... exiting"
build/pkgs/fpylll/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/gap3/spkg-src.in:    echo >&2 "Error - SAGE_ROOT undefined ... exiting"
build/pkgs/giac/spkg-src:   echo "SAGE_LOCAL undefined ... exiting";
build/pkgs/glpk/spkg-src:    echo >&2 "Error - SAGE_ROOT undefined ... exiting"
build/pkgs/importlib_metadata/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/jsonschema/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/jupyterlab_widgets/spkg-install:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/markupsafe/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/mistune/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/mpir/spkg-src:    echo >&2 "SAGE_DISTFILES undefined ... exiting"
build/pkgs/nodejs/spkg-install:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/openssl/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/pyrsistent/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/python_igraph/spkg-src:   echo >&2 "\$SAGE_ROOT or \$SAGE_DISTFILES undefined ... exiting";
build/pkgs/sagelib/spkg-src:    echo >&2 "Error - SAGE_ROOT undefined ... exiting"
build/pkgs/surf/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/termcap/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/vcversioner/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
build/pkgs/zipp/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
```

CC:  @jhpalmieri

Keywords: **sd111**

_Issue created by migration from https://trac.sagemath.org/ticket/30623_





---

archive/issue_events_403922.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-21T06:18:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403922"
}
```



---

archive/issue_events_403923.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-21T06:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403923"
}
```



---

archive/issue_events_403924.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-21T06:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403924"
}
```



---

archive/issue_events_403925.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-21T06:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403925"
}
```



---

archive/issue_comments_491709.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,32 @@\n+A lot of this boilerplate can be replaced by `sdh_guard`\n \n+```\n+$ git grep 'undefined.*exiting' build/pkgs/\n+build/pkgs/attrs/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/benzene/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/buckygen/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/certifi/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/database_cremona_ellcurve/spkg-install.in:    echo >&2 \"SAGE_SHARE undefined ... exiting\"\n+build/pkgs/database_jones_numfield/spkg-install.in:    echo >&2 \"SAGE_SHARE undefined ... exiting\"\n+build/pkgs/database_mutation_class/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined database_mutation_class-1.0 exiting\"\n+build/pkgs/database_stein_watkins/spkg-install.in:    echo >&2 \"SAGE_SHARE undefined ... exiting\"\n+build/pkgs/fpylll/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/gap3/spkg-src.in:    echo >&2 \"Error - SAGE_ROOT undefined ... exiting\"\n+build/pkgs/giac/spkg-src:   echo \"SAGE_LOCAL undefined ... exiting\";\n+build/pkgs/glpk/spkg-src:    echo >&2 \"Error - SAGE_ROOT undefined ... exiting\"\n+build/pkgs/importlib_metadata/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/jsonschema/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/jupyterlab_widgets/spkg-install:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/markupsafe/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/mistune/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/mpir/spkg-src:    echo >&2 \"SAGE_DISTFILES undefined ... exiting\"\n+build/pkgs/nodejs/spkg-install:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/openssl/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/pyrsistent/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/python_igraph/spkg-src:   echo >&2 \"\\$SAGE_ROOT or \\$SAGE_DISTFILES undefined ... exiting\";\n+build/pkgs/sagelib/spkg-src:    echo >&2 \"Error - SAGE_ROOT undefined ... exiting\"\n+build/pkgs/surf/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/termcap/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/vcversioner/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+build/pkgs/zipp/spkg-install.in:    echo >&2 \"SAGE_LOCAL undefined ... exiting\"\n+```\n``````\n",
    "created_at": "2020-09-21T06:21:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30623#issuecomment-491709",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,32 @@
+A lot of this boilerplate can be replaced by `sdh_guard`
 
+```
+$ git grep 'undefined.*exiting' build/pkgs/
+build/pkgs/attrs/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/benzene/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/buckygen/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/certifi/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/database_cremona_ellcurve/spkg-install.in:    echo >&2 "SAGE_SHARE undefined ... exiting"
+build/pkgs/database_jones_numfield/spkg-install.in:    echo >&2 "SAGE_SHARE undefined ... exiting"
+build/pkgs/database_mutation_class/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined database_mutation_class-1.0 exiting"
+build/pkgs/database_stein_watkins/spkg-install.in:    echo >&2 "SAGE_SHARE undefined ... exiting"
+build/pkgs/fpylll/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/gap3/spkg-src.in:    echo >&2 "Error - SAGE_ROOT undefined ... exiting"
+build/pkgs/giac/spkg-src:   echo "SAGE_LOCAL undefined ... exiting";
+build/pkgs/glpk/spkg-src:    echo >&2 "Error - SAGE_ROOT undefined ... exiting"
+build/pkgs/importlib_metadata/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/jsonschema/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/jupyterlab_widgets/spkg-install:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/markupsafe/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/mistune/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/mpir/spkg-src:    echo >&2 "SAGE_DISTFILES undefined ... exiting"
+build/pkgs/nodejs/spkg-install:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/openssl/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/pyrsistent/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/python_igraph/spkg-src:   echo >&2 "\$SAGE_ROOT or \$SAGE_DISTFILES undefined ... exiting";
+build/pkgs/sagelib/spkg-src:    echo >&2 "Error - SAGE_ROOT undefined ... exiting"
+build/pkgs/surf/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/termcap/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/vcversioner/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+build/pkgs/zipp/spkg-install.in:    echo >&2 "SAGE_LOCAL undefined ... exiting"
+```
``````




---

archive/issue_events_403926.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403926"
}
```



---

archive/issue_events_403927.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403927"
}
```



---

archive/issue_comments_491710.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nHoping we can make progress on this ticket this week - https://wiki.sagemath.org/days111",
    "created_at": "2020-12-06T18:17:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30623#issuecomment-491710",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Hoping we can make progress on this ticket this week - https://wiki.sagemath.org/days111



---

archive/issue_comments_491711.json:
```json
{
    "body": "Changed keywords from none to **sd111**",
    "created_at": "2020-12-06T18:17:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30623#issuecomment-491711",
    "user": "https://github.com/mkoeppe"
}
```

Changed keywords from none to **sd111**



---

archive/issue_events_403928.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403928"
}
```



---

archive/issue_events_403929.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403929"
}
```



---

archive/issue_comments_491712.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30623#issuecomment-491712",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_403930.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403930"
}
```



---

archive/issue_events_403931.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403931"
}
```



---

archive/issue_events_403932.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403932"
}
```



---

archive/issue_events_403933.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403933"
}
```



---

archive/issue_events_403934.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:45:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403934"
}
```



---

archive/issue_events_403935.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:45:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403935"
}
```



---

archive/issue_events_403936.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403936"
}
```



---

archive/issue_events_403937.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403937"
}
```



---

archive/issue_events_403938.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403938"
}
```



---

archive/issue_events_403939.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30623",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30623#event-403939"
}
```
