# Issue 30276: Phased permutation groups

archive/issues_030039.json:
```json
{
    "assignees": [],
    "body": "Mathematica uses \"phased permutations\" to express tensor symmetries.\n\nA cycle of length k is labeled with a kth root of unity.\n\n\nhttp://reference.wolframcloud.com/language/tutorial/TensorSymmetries.html\n\n\nThis generalizes the symmetries that `sage.tensor` can currently express, which are products of full symmetric groups (where the transpositions in the antisymmetries are labeled with -1).\n\nWe represent it as a matrix group in GL_n,\nand also provide a method that computes its representation as a subgroup of `GL(T^{k,l)M)`. \n\n\nRelated reference: https://arxiv.org/pdf/2007.08056.pdf\n\nCC:  @tscrim @egourgoulhon @mjungmath @LBrunswic @mwageringel @dimpase @Ivo-Maffei\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30276_\n\n",
    "created_at": "2020-08-02T22:41:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Phased permutation groups",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/30276",
    "user": "https://github.com/mkoeppe"
}
```
Mathematica uses "phased permutations" to express tensor symmetries.

A cycle of length k is labeled with a kth root of unity.


http://reference.wolframcloud.com/language/tutorial/TensorSymmetries.html


This generalizes the symmetries that `sage.tensor` can currently express, which are products of full symmetric groups (where the transpositions in the antisymmetries are labeled with -1).

We represent it as a matrix group in GL_n,
and also provide a method that computes its representation as a subgroup of `GL(T^{k,l)M)`. 


Related reference: https://arxiv.org/pdf/2007.08056.pdf

CC:  @tscrim @egourgoulhon @mjungmath @LBrunswic @mwageringel @dimpase @Ivo-Maffei

_Issue created by migration from https://trac.sagemath.org/ticket/30276_





---

archive/issue_events_398811.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-02T22:41:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398811"
}
```



---

archive/issue_events_398812.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-02T22:41:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398812"
}
```



---

archive/issue_events_398813.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-02T22:41:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398813"
}
```



---

archive/issue_events_398814.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-02T22:41:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398814"
}
```



---

archive/issue_comments_483253.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,4 +9,4 @@\n This generalizes the symmetries that `sage.tensor` can currently express, which are products of full symmetric groups (where the transpositions in the antisymmetries are labeled with -1).\n \n \n-\n+Related reference: https://arxiv.org/pdf/2007.08056.pdf\n``````\n",
    "created_at": "2020-08-02T23:04:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30276#issuecomment-483253",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,4 +9,4 @@
 This generalizes the symmetries that `sage.tensor` can currently express, which are products of full symmetric groups (where the transpositions in the antisymmetries are labeled with -1).
 
 
-
+Related reference: https://arxiv.org/pdf/2007.08056.pdf
``````




---

archive/issue_comments_483254.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,5 +8,8 @@\n \n This generalizes the symmetries that `sage.tensor` can currently express, which are products of full symmetric groups (where the transpositions in the antisymmetries are labeled with -1).\n \n+We represent it as a matrix group in GL_n,\n+and also provide a method that computes its representation as a subgroup of `GL(T^{k,l)M)`. \n+\n \n Related reference: https://arxiv.org/pdf/2007.08056.pdf\n``````\n",
    "created_at": "2020-08-02T23:31:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30276#issuecomment-483254",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,5 +8,8 @@
 
 This generalizes the symmetries that `sage.tensor` can currently express, which are products of full symmetric groups (where the transpositions in the antisymmetries are labeled with -1).
 
+We represent it as a matrix group in GL_n,
+and also provide a method that computes its representation as a subgroup of `GL(T^{k,l)M)`. 
+
 
 Related reference: https://arxiv.org/pdf/2007.08056.pdf
``````




---

archive/issue_comments_483255.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nthis seems to generalise to cyclic groups only, no?",
    "created_at": "2020-08-11T15:13:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30276#issuecomment-483255",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:5" align="right">Comment 5</div>

this seems to generalise to cyclic groups only, no?



---

archive/issue_comments_483256.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\neach generator is a cycle...",
    "created_at": "2020-08-11T15:16:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30276#issuecomment-483256",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

each generator is a cycle...



---

archive/issue_comments_483257.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\neach generator is a product of cycles, in full generality. Then, I think these things are called monomial groups, \"phased\" comes from physics people not taking algebra classes :-)",
    "created_at": "2020-08-11T16:21:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30276#issuecomment-483257",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:7" align="right">Comment 7</div>

each generator is a product of cycles, in full generality. Then, I think these things are called monomial groups, "phased" comes from physics people not taking algebra classes :-)



---

archive/issue_comments_483258.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nYes, it's a Wolfram-ism, I think",
    "created_at": "2020-08-11T16:55:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30276#issuecomment-483258",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Yes, it's a Wolfram-ism, I think



---

archive/issue_comments_483259.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nWould these be a generalization of `ColoredPermutations`, where each element of `{1, ..., n}` can have its own distinct cycle length?",
    "created_at": "2020-08-11T22:50:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30276#issuecomment-483259",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">Comment 9</div>

Would these be a generalization of `ColoredPermutations`, where each element of `{1, ..., n}` can have its own distinct cycle length?



---

archive/issue_comments_483260.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nyes, but with some kind of compatibility relation, I guess.",
    "created_at": "2020-08-12T00:39:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30276#issuecomment-483260",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:10" align="right">Comment 10</div>

yes, but with some kind of compatibility relation, I guess.



---

archive/issue_events_398815.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398815"
}
```



---

archive/issue_events_398816.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398816"
}
```



---

archive/issue_events_398817.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398817"
}
```



---

archive/issue_events_398818.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398818"
}
```



---

archive/issue_comments_483261.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30276#issuecomment-483261",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:12" align="right">Comment 12</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_398819.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398819"
}
```



---

archive/issue_events_398820.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398820"
}
```



---

archive/issue_events_398821.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398821"
}
```



---

archive/issue_events_398822.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398822"
}
```



---

archive/issue_events_398823.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398823"
}
```



---

archive/issue_events_398824.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398824"
}
```



---

archive/issue_events_398825.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398825"
}
```



---

archive/issue_events_398826.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398826"
}
```



---

archive/issue_events_398827.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398827"
}
```



---

archive/issue_events_398828.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30276",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30276#event-398828"
}
```
