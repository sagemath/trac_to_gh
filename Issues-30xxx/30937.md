# Issue 30937: Improve error message for jmol

archive/issues_030700.json:
```json
{
    "assignees": [],
    "body": "Currently, if something goes wrong during the export of the image, the error message contains a reference to a temporary file. This is not very helpful, especially if the error occurs in a CI environment where one cannot check the temporary file for the error message. With this ticket, the content of the error message file is printed leading to messages like\n\n```\nRuntimeError: Jmol failed to create file /github/home/.sage/temp/ed71d2cc7e60/53202/dir_u4obyp15/preview.png: Error: Unable to access jarfile /__w/sage/sage/local/share/jmol/JmolData.jar\n```\n\nCC:  @fchapoton @embray @jcamp0x2a\n\nBranch/Commit: **[`34eb0d3`](https://github.com/sagemath/sagetrac-mirror/commit/34eb0d38df96829db1e471c92599af5f474dcabe)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Tobias Diez**\n\nComponent: **interfaces**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30937_\n\n",
    "closed_at": "2020-11-22T15:05:19Z",
    "created_at": "2020-11-18T10:40:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve error message for jmol",
    "type": "issue",
    "updated_at": "2020-11-22T15:05:19Z",
    "url": "https://github.com/sagemath/sage/issues/30937",
    "user": "https://github.com/tobiasdiez"
}
```
Currently, if something goes wrong during the export of the image, the error message contains a reference to a temporary file. This is not very helpful, especially if the error occurs in a CI environment where one cannot check the temporary file for the error message. With this ticket, the content of the error message file is printed leading to messages like

```
RuntimeError: Jmol failed to create file /github/home/.sage/temp/ed71d2cc7e60/53202/dir_u4obyp15/preview.png: Error: Unable to access jarfile /__w/sage/sage/local/share/jmol/JmolData.jar
```

CC:  @fchapoton @embray @jcamp0x2a

Branch/Commit: **[`34eb0d3`](https://github.com/sagemath/sagetrac-mirror/commit/34eb0d38df96829db1e471c92599af5f474dcabe)**

Reviewer: **Travis Scrimshaw**

Author: **Tobias Diez**

Component: **interfaces**

_Issue created by migration from https://trac.sagemath.org/ticket/30937_





---

archive/issue_events_426696.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-18T10:40:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30937#event-426696"
}
```



---

archive/issue_events_426697.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-18T10:40:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "c: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30937#event-426697"
}
```



---

archive/issue_events_426698.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-18T10:40:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30937#event-426698"
}
```



---

archive/issue_comments_495785.json:
```json
{
    "body": "Author: **Tobias Diez**",
    "created_at": "2020-11-18T10:41:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30937#issuecomment-495785",
    "user": "https://github.com/tobiasdiez"
}
```

Author: **Tobias Diez**



---

archive/issue_events_426699.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-18T10:41:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30937#event-426699"
}
```



---

archive/issue_events_426700.json:
```json
{
    "actor": "https://github.com/tobiasdiez",
    "created_at": "2020-11-18T10:45:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30937#event-426700"
}
```



---

archive/issue_comments_495786.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nSounds like a very reasonable thing to do.",
    "created_at": "2020-11-19T23:22:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30937#issuecomment-495786",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">comment:4</div>

Sounds like a very reasonable thing to do.



---

archive/issue_events_426701.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-11-19T23:22:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30937#event-426701"
}
```



---

archive/issue_events_426702.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-11-19T23:22:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30937#event-426702"
}
```



---

archive/issue_comments_495787.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-11-19T23:22:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30937#issuecomment-495787",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_495788.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThanks for the review!",
    "created_at": "2020-11-20T09:32:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30937#issuecomment-495788",
    "user": "https://github.com/tobiasdiez"
}
```

<div id="comment:5" align="right">comment:5</div>

Thanks for the review!



---

archive/issue_comments_495789.json:
```json
{
    "body": "Changed branch from **[public/interfaces/jmolError](https://github.com/sagemath/sagetrac-mirror/tree/public/interfaces/jmolError)** to **[`34eb0d3`](https://github.com/sagemath/sagetrac-mirror/commit/34eb0d38df96829db1e471c92599af5f474dcabe)**",
    "created_at": "2020-11-22T15:05:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30937#issuecomment-495789",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/interfaces/jmolError](https://github.com/sagemath/sagetrac-mirror/tree/public/interfaces/jmolError)** to **[`34eb0d3`](https://github.com/sagemath/sagetrac-mirror/commit/34eb0d38df96829db1e471c92599af5f474dcabe)**



---

archive/issue_events_426703.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-11-22T15:05:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30937#event-426703"
}
```



---

archive/issue_events_426704.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7adc2a9160300f818e3beba0ab8dd0ad894ba7d0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-11-22T15:05:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30937",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30937#event-426704"
}
```
