# Issue 30572: Remove indirect typecasts when calling bitset.pxi

archive/issues_030335.json:
```json
{
    "assignees": [],
    "body": "This ticket makes typecasts to functions in `data_structures/bitset.pxi` explicit.\n\nThis allows [fused types](https://cython.readthedocs.io/en/latest/src/userguide/fusedtypes.html,) as signatures of those functions. E.g.\n\n`cdef inline void bitset_add(bitset_t bits, mp_bitcnt_t n)` -> `cdef inline void bitset_add(fused_bitset_t bits, mp_bitcnt_t n)`. Fused functions do not allow calling this function with signature `(bitset_t, PyObject)`. (`(bitset_t, int)` etc. are ok though.)\n\nThis way, we can use most of the code in `bitset.pxi` for `CombinatorialPolyhedron`, while easily optimizing it for the special situation.\n\nKeywords: **bitset, typecasting, fused types**\n\nBranch/Commit: **[2c296ab](https://github.com/sagemath/sagetrac-mirror/commit/2c296ab44fbcd62db51da5ab2325e57efd379148)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jonathan Kliem**\n\nComponent: **cython**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30572_\n\n",
    "closed_at": "2020-09-27T09:09:28Z",
    "created_at": "2020-09-14T15:05:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20cython",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove indirect typecasts when calling bitset.pxi",
    "type": "issue",
    "updated_at": "2020-09-27T16:18:03Z",
    "url": "https://github.com/sagemath/sage/issues/30572",
    "user": "https://github.com/kliem"
}
```
This ticket makes typecasts to functions in `data_structures/bitset.pxi` explicit.

This allows [fused types](https://cython.readthedocs.io/en/latest/src/userguide/fusedtypes.html,) as signatures of those functions. E.g.

`cdef inline void bitset_add(bitset_t bits, mp_bitcnt_t n)` -> `cdef inline void bitset_add(fused_bitset_t bits, mp_bitcnt_t n)`. Fused functions do not allow calling this function with signature `(bitset_t, PyObject)`. (`(bitset_t, int)` etc. are ok though.)

This way, we can use most of the code in `bitset.pxi` for `CombinatorialPolyhedron`, while easily optimizing it for the special situation.

Keywords: **bitset, typecasting, fused types**

Branch/Commit: **[2c296ab](https://github.com/sagemath/sagetrac-mirror/commit/2c296ab44fbcd62db51da5ab2325e57efd379148)**

Reviewer: **Travis Scrimshaw**

Author: **Jonathan Kliem**

Component: **cython**

_Issue created by migration from https://trac.sagemath.org/ticket/30572_





---

archive/issue_events_421479.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-09-14T15:05:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421479"
}
```



---

archive/issue_events_421480.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-09-14T15:05:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20cython",
    "label_color": "000080",
    "label_name": "c: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421480"
}
```



---

archive/issue_events_421481.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-09-14T15:05:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421481"
}
```



---

archive/issue_events_421482.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-09-14T15:05:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421482"
}
```



---

archive/issue_events_421483.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-09-14T15:05:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421483"
}
```



---

archive/issue_comments_487798.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,6 +2,6 @@\n \n This allows [fused types](https://cython.readthedocs.io/en/latest/src/userguide/fusedtypes.html,) as signatures of those functions. E.g.\n \n-`cdef inline void bitset_add(bitset_t bits, mp_bitcnt_t n)` -> `cdef inline void bitset_add(fused_bitset_t bits, mp_bitcnt_t n)`. Fused functions do not allow from `PyObject`. (`int` to `size_t` etc. is ok though.)\n+`cdef inline void bitset_add(bitset_t bits, mp_bitcnt_t n)` -> `cdef inline void bitset_add(fused_bitset_t bits, mp_bitcnt_t n)`. Fused functions do not allow calling this function with signature `(bitset_t, PyObject)`. (`(bitset_t, int)` etc. are ok though.)\n \n This way, we can use most of the code in `bitset.pxi` for `CombinatorialPolyhedron`, while easily optimizing it for the special situation.\n``````\n",
    "created_at": "2020-09-14T18:14:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30572#issuecomment-487798",
    "user": "https://github.com/kliem"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,6 +2,6 @@
 
 This allows [fused types](https://cython.readthedocs.io/en/latest/src/userguide/fusedtypes.html,) as signatures of those functions. E.g.
 
-`cdef inline void bitset_add(bitset_t bits, mp_bitcnt_t n)` -> `cdef inline void bitset_add(fused_bitset_t bits, mp_bitcnt_t n)`. Fused functions do not allow from `PyObject`. (`int` to `size_t` etc. is ok though.)
+`cdef inline void bitset_add(bitset_t bits, mp_bitcnt_t n)` -> `cdef inline void bitset_add(fused_bitset_t bits, mp_bitcnt_t n)`. Fused functions do not allow calling this function with signature `(bitset_t, PyObject)`. (`(bitset_t, int)` etc. are ok though.)
 
 This way, we can use most of the code in `bitset.pxi` for `CombinatorialPolyhedron`, while easily optimizing it for the special situation.
``````




---

archive/issue_events_421484.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-09-17T00:02:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421484"
}
```



---

archive/issue_events_421485.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-09-17T00:02:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421485"
}
```



---

archive/issue_comments_487799.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLGTM.",
    "created_at": "2020-09-17T00:02:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30572#issuecomment-487799",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">comment:3</div>

LGTM.



---

archive/issue_comments_487800.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-09-17T00:02:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30572#issuecomment-487800",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_487801.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThank you.",
    "created_at": "2020-09-17T03:42:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30572#issuecomment-487801",
    "user": "https://github.com/kliem"
}
```

<div id="comment:4" align="right">comment:4</div>

Thank you.



---

archive/issue_comments_487802.json:
```json
{
    "body": "Changed branch from **[u/gh-kliem/remove_some_indirect_typecasts_to_bitset](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-kliem/remove_some_indirect_typecasts_to_bitset)** to **[2c296ab](https://github.com/sagemath/sagetrac-mirror/commit/2c296ab44fbcd62db51da5ab2325e57efd379148)**",
    "created_at": "2020-09-27T09:09:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30572#issuecomment-487802",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-kliem/remove_some_indirect_typecasts_to_bitset](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-kliem/remove_some_indirect_typecasts_to_bitset)** to **[2c296ab](https://github.com/sagemath/sagetrac-mirror/commit/2c296ab44fbcd62db51da5ab2325e57efd379148)**



---

archive/issue_events_421486.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-09-27T09:09:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421486"
}
```



---

archive/issue_events_421487.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "de8e68e534523fb8b11de476c5722da2ce83fa26",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-09-27T09:09:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421487"
}
```



---

archive/issue_events_421488.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-27T16:18:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421488"
}
```



---

archive/issue_events_421489.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-27T16:18:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30572",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30572#event-421489"
}
```
