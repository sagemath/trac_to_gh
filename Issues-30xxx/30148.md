# Issue 30148: fix "make download"

archive/issues_029911.json:
```json
{
    "assignees": [],
    "body": "Noticed while doing something else:\n\n```\n$ make download\nexport SAGE_ROOT=$(pwd) && \\\nexport PATH=$SAGE_ROOT/src/bin:$PATH && \\\n./src/bin/sage-download-upstream\n./src/bin/sage-download-upstream: line 6: sage-spkg: command not found\n./src/bin/sage-download-upstream: line 6: sage-spkg: command not found\n...\nmake: *** [Makefile:69: download] Error 127\n```\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30148_\n\n",
    "closed_at": "2020-07-15T06:41:35Z",
    "created_at": "2020-07-15T01:12:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix \"make download\"",
    "type": "issue",
    "updated_at": "2020-07-15T06:41:35Z",
    "url": "https://github.com/sagemath/sage/issues/30148",
    "user": "https://github.com/orlitzky"
}
```
Noticed while doing something else:

```
$ make download
export SAGE_ROOT=$(pwd) && \
export PATH=$SAGE_ROOT/src/bin:$PATH && \
./src/bin/sage-download-upstream
./src/bin/sage-download-upstream: line 6: sage-spkg: command not found
./src/bin/sage-download-upstream: line 6: sage-spkg: command not found
...
make: *** [Makefile:69: download] Error 127
```

_Issue created by migration from https://trac.sagemath.org/ticket/30148_





---

archive/issue_events_396903.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:12:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396903"
}
```



---

archive/issue_events_396904.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:12:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396904"
}
```



---

archive/issue_events_396905.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:12:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396905"
}
```



---

archive/issue_events_396906.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:12:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396906"
}
```



---

archive/issue_comments_480374.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\ndup of #29896",
    "created_at": "2020-07-15T01:14:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30148#issuecomment-480374",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

dup of #29896



---

archive/issue_events_396907.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:19:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396907"
}
```



---

archive/issue_events_396908.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:19:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396908"
}
```



---

archive/issue_events_396909.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:19:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396909"
}
```



---

archive/issue_comments_480375.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nSorry, I don't know how to work trac.",
    "created_at": "2020-07-15T01:19:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30148#issuecomment-480375",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:2" align="right">Comment 2</div>

Sorry, I don't know how to work trac.



---

archive/issue_events_396910.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:19:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396910"
}
```



---

archive/issue_events_396911.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:19:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396911"
}
```



---

archive/issue_events_396912.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-15T01:19:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396912"
}
```



---

archive/issue_events_396913.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-07-15T06:41:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396913"
}
```



---

archive/issue_events_396914.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-07-15T06:41:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396914"
}
```



---

archive/issue_events_396915.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-07-15T06:41:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30148",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30148#event-396915"
}
```
