# Issue 30415: Remove pyflakes?

archive/issues_030178.json:
```json
{
    "body": "With #30361 and #30404 pyright is added as a static type checker. Besides type checking, it also includes almost all checks that are performed by pyflakes. \n\nAs far as I can see, the differences are as follows:\n- Pyright excels at type checking\n- Pyflakes has no support for type checking, and indeed is not even able to handle basic typing syntax (this would need among others https://github.com/PyCQA/pyflakes/pull/530, which seems to be stale) \n- Pyflakes checks for basic validity of the arguments to \"format\" and \"print\", this is not supported by pyright \n\nSo it depends on how important the print and format validation are. Maybe they are also covered by lgtm?\n\nResources:\n- List of checks of pyright: https://github.com/microsoft/pyright/blob/master/docs/configuration.md#type-check-diagnostics-settings\n- List of checks of pyflakes: https://flake8.pycqa.org/en/latest/user/error-codes.html\n\n\nDepending on the outcome, the stub documentation of pyflakes added in #30361 needs be extended or removed.\n\nCC:  @mkoeppe @fchapoton @mjungmath @kliem\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/30415\n\n",
    "created_at": "2020-08-21T17:29:50Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Remove pyflakes?",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/30415",
    "user": "https://github.com/tobiasdiez"
}
```
With #30361 and #30404 pyright is added as a static type checker. Besides type checking, it also includes almost all checks that are performed by pyflakes. 

As far as I can see, the differences are as follows:
- Pyright excels at type checking
- Pyflakes has no support for type checking, and indeed is not even able to handle basic typing syntax (this would need among others https://github.com/PyCQA/pyflakes/pull/530, which seems to be stale) 
- Pyflakes checks for basic validity of the arguments to "format" and "print", this is not supported by pyright 

So it depends on how important the print and format validation are. Maybe they are also covered by lgtm?

Resources:
- List of checks of pyright: https://github.com/microsoft/pyright/blob/master/docs/configuration.md#type-check-diagnostics-settings
- List of checks of pyflakes: https://flake8.pycqa.org/en/latest/user/error-codes.html


Depending on the outcome, the stub documentation of pyflakes added in #30361 needs be extended or removed.

CC:  @mkoeppe @fchapoton @mjungmath @kliem

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/30415





---

archive/issue_comments_600450.json:
```json
{
    "body": "<a id='comment:1'></a>`flake8` (https://pypi.org/project/flake8/) also seems to have some valuable plugins:\n- https://pypi.org/project/flake8-rst-docstrings/\n- https://pypi.org/project/flake8-docstrings/  (uses https://github.com/pycqa/pydocstyle)",
    "created_at": "2020-08-30T02:39:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30415#issuecomment-600450",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'></a>`flake8` (https://pypi.org/project/flake8/) also seems to have some valuable plugins:
- https://pypi.org/project/flake8-rst-docstrings/
- https://pypi.org/project/flake8-docstrings/  (uses https://github.com/pycqa/pydocstyle)



---

archive/issue_comments_600451.json:
```json
{
    "body": "<a id='comment:2'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30415#issuecomment-600451",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_078871.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-78871"
}
```



---

archive/issue_events_078872.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-78872"
}
```



---

archive/issue_events_078873.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-78873"
}
```



---

archive/issue_events_078874.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-78874"
}
```



---

archive/issue_events_078875.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-78875"
}
```



---

archive/issue_events_078876.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-07T00:07:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-78876"
}
```



---

archive/issue_events_078877.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-07T00:07:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-78877"
}
```



---

archive/issue_events_078878.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-78878"
}
```



---

archive/issue_events_078879.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-78879"
}
```
