# Issue 30961: Fix valid input types for NumberField element constructor

archive/issues_030724.json:
```json
{
    "assignees": [],
    "body": "Before this ticket, lists of elements with arbitrary parents were accepted:\n\n```\nsage: x = SR.var('x')\nsage: QQi = i.parent()\nsage: QQi((x, x))\n(I + 1)*x\n```\n\nHowever, some lists that arguably should be accepted for consistency were not:\n\n```\nsage: QQi(\"1\")\n1\nsage: QQi((\"1\", \"2\"))\n...\nTypeError: can't multiply sequence by non-int of type 'sage.rings.number_field.number_field_element_quadratic.NumberFieldElement_gaussian'\n```\n\nAdditionally, lists and vector inputs behaved differently for no clear reason:\n\n```\nsage: QQi(RR(1))\n1\nsage: QQi((RR(1), RR(2)))\n1.00000000000000 + 2.00000000000000*I\nsage: _.parent()\nComplex Field with 53 bits of precision\nsage: QQi(vector((RR(1), RR(2))))\n...\nTypeError: unable to convert (1.00000000000000, 2.00000000000000) to Number Field in I with defining polynomial x^2 + 1 with I = 1*I\n```\n\nBranch/Commit: **[780ed33](https://github.com/sagemath/sagetrac-mirror/commit/780ed337ce09cacc3bbedf2fc5cfe6bb82ec40f5)**\n\nReviewer: **Samuel Leli\u00e8vre**\n\nAuthor: **Marc Mezzarobba**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30961_\n\n",
    "closed_at": "2020-12-06T12:49:24Z",
    "created_at": "2020-11-25T15:17:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix valid input types for NumberField element constructor",
    "type": "issue",
    "updated_at": "2020-12-06T12:49:24Z",
    "url": "https://github.com/sagemath/sage/issues/30961",
    "user": "https://github.com/mezzarobba"
}
```
Before this ticket, lists of elements with arbitrary parents were accepted:

```
sage: x = SR.var('x')
sage: QQi = i.parent()
sage: QQi((x, x))
(I + 1)*x
```

However, some lists that arguably should be accepted for consistency were not:

```
sage: QQi("1")
1
sage: QQi(("1", "2"))
...
TypeError: can't multiply sequence by non-int of type 'sage.rings.number_field.number_field_element_quadratic.NumberFieldElement_gaussian'
```

Additionally, lists and vector inputs behaved differently for no clear reason:

```
sage: QQi(RR(1))
1
sage: QQi((RR(1), RR(2)))
1.00000000000000 + 2.00000000000000*I
sage: _.parent()
Complex Field with 53 bits of precision
sage: QQi(vector((RR(1), RR(2))))
...
TypeError: unable to convert (1.00000000000000, 2.00000000000000) to Number Field in I with defining polynomial x^2 + 1 with I = 1*I
```

Branch/Commit: **[780ed33](https://github.com/sagemath/sagetrac-mirror/commit/780ed337ce09cacc3bbedf2fc5cfe6bb82ec40f5)**

Reviewer: **Samuel Lelièvre**

Author: **Marc Mezzarobba**

_Issue created by migration from https://trac.sagemath.org/ticket/30961_





---

archive/issue_events_408751.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2020-11-25T15:17:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408751"
}
```



---

archive/issue_events_408752.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2020-11-25T15:17:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408752"
}
```



---

archive/issue_events_408753.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2020-11-25T15:17:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408753"
}
```



---

archive/issue_events_408754.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2020-11-25T15:17:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408754"
}
```



---

archive/issue_comments_499099.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/780ed337ce09cacc3bbedf2fc5cfe6bb82ec40f5\">780ed33</a></td><td><code>#30961 fix construction of number field elts from lists/vectors</code></td></tr></table>\n",
    "created_at": "2020-11-25T15:25:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30961#issuecomment-499099",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/780ed337ce09cacc3bbedf2fc5cfe6bb82ec40f5">780ed33</a></td><td><code>#30961 fix construction of number field elts from lists/vectors</code></td></tr></table>




---

archive/issue_comments_499100.json:
```json
{
    "body": "Branch: **[u/mmezzarobba/30961-fix_nf_elt_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/30961-fix_nf_elt_constructor)**",
    "created_at": "2020-11-25T15:25:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30961#issuecomment-499100",
    "user": "https://github.com/mezzarobba"
}
```

Branch: **[u/mmezzarobba/30961-fix_nf_elt_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/30961-fix_nf_elt_constructor)**



---

archive/issue_events_408755.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2020-11-25T15:25:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408755"
}
```



---

archive/issue_comments_499101.json:
```json
{
    "body": "Commit: **[780ed33](https://github.com/sagemath/sagetrac-mirror/commit/780ed337ce09cacc3bbedf2fc5cfe6bb82ec40f5)**",
    "created_at": "2020-11-25T15:25:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30961#issuecomment-499101",
    "user": "https://github.com/mezzarobba"
}
```

Commit: **[780ed33](https://github.com/sagemath/sagetrac-mirror/commit/780ed337ce09cacc3bbedf2fc5cfe6bb82ec40f5)**



---

archive/issue_comments_499102.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThese are welcome fixes.",
    "created_at": "2020-12-01T12:47:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30961#issuecomment-499102",
    "user": "https://github.com/slel"
}
```

<div id="comment:2" align="right">Comment 2</div>

These are welcome fixes.



---

archive/issue_comments_499103.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,13 +1,13 @@\n-Lists of elements with arbitrary parents are accepted:\n+Before this ticket, lists of elements with arbitrary parents were accepted:\n \n ```\n sage: x = SR.var('x')\n sage: QQi = i.parent()\n-sage: QQi((x,x))\n+sage: QQi((x, x))\n (I + 1)*x\n ```\n \n-However, some lists that arguably should be accepted for consistency are not:\n+However, some lists that arguably should be accepted for consistency were not:\n \n ```\n sage: QQi(\"1\")\n@@ -17,7 +17,7 @@\n TypeError: can't multiply sequence by non-int of type 'sage.rings.number_field.number_field_element_quadratic.NumberFieldElement_gaussian'\n ```\n \n-Additionally, lists and vector inputs behave differently for no clear reason:\n+Additionally, lists and vector inputs behaved differently for no clear reason:\n \n ```\n sage: QQi(RR(1))\n``````\n",
    "created_at": "2020-12-01T12:47:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30961#issuecomment-499103",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,13 +1,13 @@
-Lists of elements with arbitrary parents are accepted:
+Before this ticket, lists of elements with arbitrary parents were accepted:
 
 ```
 sage: x = SR.var('x')
 sage: QQi = i.parent()
-sage: QQi((x,x))
+sage: QQi((x, x))
 (I + 1)*x
 ```
 
-However, some lists that arguably should be accepted for consistency are not:
+However, some lists that arguably should be accepted for consistency were not:
 
 ```
 sage: QQi("1")
@@ -17,7 +17,7 @@
 TypeError: can't multiply sequence by non-int of type 'sage.rings.number_field.number_field_element_quadratic.NumberFieldElement_gaussian'
 ```
 
-Additionally, lists and vector inputs behave differently for no clear reason:
+Additionally, lists and vector inputs behaved differently for no clear reason:
 
 ```
 sage: QQi(RR(1))
``````




---

archive/issue_events_408756.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-12-01T12:47:47Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "title_is": "Fix valid input types for NumberField element constructor",
    "title_was": "multiple bugs in NumberField element constructor",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408756"
}
```



---

archive/issue_events_408757.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-12-01T12:47:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408757"
}
```



---

archive/issue_events_408758.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-12-01T12:47:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408758"
}
```



---

archive/issue_comments_499104.json:
```json
{
    "body": "Reviewer: **Samuel Leli\u00e8vre**",
    "created_at": "2020-12-01T12:47:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30961#issuecomment-499104",
    "user": "https://github.com/slel"
}
```

Reviewer: **Samuel Lelièvre**



---

archive/issue_comments_499105.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nmerci !",
    "created_at": "2020-12-01T13:18:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30961#issuecomment-499105",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:3" align="right">Comment 3</div>

merci !



---

archive/issue_events_408759.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-12-06T12:49:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408759"
}
```



---

archive/issue_events_408760.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3fb2f3eab931d9b2b0ebd2f25d9f3967cd7bf2e4",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-12-06T12:49:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30961#event-408760"
}
```



---

archive/issue_comments_499106.json:
```json
{
    "body": "Changed branch from **[u/mmezzarobba/30961-fix_nf_elt_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/30961-fix_nf_elt_constructor)** to **[780ed33](https://github.com/sagemath/sagetrac-mirror/commit/780ed337ce09cacc3bbedf2fc5cfe6bb82ec40f5)**",
    "created_at": "2020-12-06T12:49:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30961#issuecomment-499106",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmezzarobba/30961-fix_nf_elt_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/30961-fix_nf_elt_constructor)** to **[780ed33](https://github.com/sagemath/sagetrac-mirror/commit/780ed337ce09cacc3bbedf2fc5cfe6bb82ec40f5)**
