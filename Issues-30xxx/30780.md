# Issue 30780: sage_setup: Separate installing and cleaning

archive/issues_030543.json:
```json
{
    "assignees": [],
    "body": "This is preparation for #28925.\n\n\nDepends on #30779\n\nCC:  @tobiasdiez @dimpase @jhpalmieri\n\nBranch: **[ad9402f](https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30780_\n\n",
    "closed_at": "2020-11-22T15:06:39Z",
    "created_at": "2020-10-16T22:13:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage_setup: Separate installing and cleaning",
    "type": "issue",
    "updated_at": "2020-12-01T19:01:30Z",
    "url": "https://github.com/sagemath/sage/issues/30780",
    "user": "https://github.com/mkoeppe"
}
```
This is preparation for #28925.


Depends on #30779

CC:  @tobiasdiez @dimpase @jhpalmieri

Branch: **[ad9402f](https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/30780_





---

archive/issue_events_406287.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-16T22:13:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30780#event-406287"
}
```



---

archive/issue_events_406288.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-16T22:13:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30780#event-406288"
}
```



---

archive/issue_events_406289.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-16T22:13:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30780#event-406289"
}
```



---

archive/issue_events_406290.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-16T22:13:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30780#event-406290"
}
```



---

archive/issue_comments_495221.json:
```json
{
    "body": "Branch: **[u/mkoeppe/sage_setup__separate_installing_and_cleaning](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_setup__separate_installing_and_cleaning)**",
    "created_at": "2020-10-16T22:17:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495221",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/sage_setup__separate_installing_and_cleaning](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_setup__separate_installing_and_cleaning)**



---

archive/issue_events_406291.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-16T22:18:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30780#event-406291"
}
```



---

archive/issue_comments_495222.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4f565179d1e87d3cd14b0e76c508fd9d7bbfc5a8\">4f56517</a></td><td><code>Duplicate src/setup.py</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/79801309c0aa7e455db183a9865a57b8f49ad2b5\">7980130</a></td><td><code>Merge branch 't/30779/duplicate_src_setup_py' into t/30780/sage_setup__separate_installing_and_cleaning</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9b9562827f92371c91109fc62380fecf7af8f65d\">9b95628</a></td><td><code>sage_setup.command.sage_install.sage_install: Rename to sage_install_and_clean, update use it in build/pkgs/sagelib/src/setup.py; refactor through new class sage_install</code></td></tr></table>\n",
    "created_at": "2020-10-16T22:18:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495222",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4f565179d1e87d3cd14b0e76c508fd9d7bbfc5a8">4f56517</a></td><td><code>Duplicate src/setup.py</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/79801309c0aa7e455db183a9865a57b8f49ad2b5">7980130</a></td><td><code>Merge branch 't/30779/duplicate_src_setup_py' into t/30780/sage_setup__separate_installing_and_cleaning</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9b9562827f92371c91109fc62380fecf7af8f65d">9b95628</a></td><td><code>sage_setup.command.sage_install.sage_install: Rename to sage_install_and_clean, update use it in build/pkgs/sagelib/src/setup.py; refactor through new class sage_install</code></td></tr></table>




---

archive/issue_comments_495223.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-10-16T22:18:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495223",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_495224.json:
```json
{
    "body": "Commit: **[9b95628](https://github.com/sagemath/sagetrac-mirror/commit/9b9562827f92371c91109fc62380fecf7af8f65d)**",
    "created_at": "2020-10-16T22:18:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495224",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[9b95628](https://github.com/sagemath/sagetrac-mirror/commit/9b9562827f92371c91109fc62380fecf7af8f65d)**



---

archive/issue_comments_495225.json:
```json
{
    "body": "Changed commit from **[9b95628](https://github.com/sagemath/sagetrac-mirror/commit/9b9562827f92371c91109fc62380fecf7af8f65d)** to **[ad9402f](https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a)**",
    "created_at": "2020-10-17T02:18:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495225",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9b95628](https://github.com/sagemath/sagetrac-mirror/commit/9b9562827f92371c91109fc62380fecf7af8f65d)** to **[ad9402f](https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a)**



---

archive/issue_comments_495226.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a\">ad9402f</a></td><td><code>sage_setup.command.sage_install: Fixup: Remove cleaning code (moved to sage_install_and_clean)</code></td></tr></table>\n",
    "created_at": "2020-10-17T02:18:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495226",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a">ad9402f</a></td><td><code>sage_setup.command.sage_install: Fixup: Remove cleaning code (moved to sage_install_and_clean)</code></td></tr></table>




---

archive/issue_events_406292.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-11-11T09:09:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30780#event-406292"
}
```



---

archive/issue_events_406293.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-11-11T09:09:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30780#event-406293"
}
```



---

archive/issue_comments_495227.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-11-11T09:09:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495227",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_495228.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nok",
    "created_at": "2020-11-11T09:09:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495228",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:7" align="right">Comment 7</div>

ok



---

archive/issue_comments_495229.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThanks",
    "created_at": "2020-11-11T16:53:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495229",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Thanks



---

archive/issue_events_406294.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-11-22T15:06:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30780#event-406294"
}
```



---

archive/issue_events_406295.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "496b3f6d08cd3f1090b52aba51cfa724cb529622",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-11-22T15:06:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30780#event-406295"
}
```



---

archive/issue_comments_495230.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/sage_setup__separate_installing_and_cleaning](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_setup__separate_installing_and_cleaning)** to **[ad9402f](https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a)**",
    "created_at": "2020-11-22T15:06:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495230",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/sage_setup__separate_installing_and_cleaning](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_setup__separate_installing_and_cleaning)** to **[ad9402f](https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a)**



---

archive/issue_comments_495231.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nWhy did this include a whole entire duplicate of `setup.py`?",
    "created_at": "2020-12-01T10:45:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495231",
    "user": "https://github.com/embray"
}
```

<div id="comment:10" align="right">Comment 10</div>

Why did this include a whole entire duplicate of `setup.py`?



---

archive/issue_comments_495232.json:
```json
{
    "body": "Changed commit from **[ad9402f](https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a)** to none",
    "created_at": "2020-12-01T10:45:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495232",
    "user": "https://github.com/embray"
}
```

Changed commit from **[ad9402f](https://github.com/sagemath/sagetrac-mirror/commit/ad9402f606204d6dd9a454b9b055bed63247d97a)** to none



---

archive/issue_comments_495233.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThe duplication was done in #30779, a dependency of this ticket. This is preparing for Tobias Diez's work for editable installs (`setup.py develop`) in #30371 out of `src/` and the work in #29705 regarding modularization, which is taking place in `build/pkgs/sagelib/src`. At some point they will be reconciled again, but for the time being it simplifies development to keep the two copies separate.",
    "created_at": "2020-12-01T19:01:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30780#issuecomment-495233",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:11" align="right">Comment 11</div>

The duplication was done in #30779, a dependency of this ticket. This is preparing for Tobias Diez's work for editable installs (`setup.py develop`) in #30371 out of `src/` and the work in #29705 regarding modularization, which is taking place in `build/pkgs/sagelib/src`. At some point they will be reconciled again, but for the time being it simplifies development to keep the two copies separate.
