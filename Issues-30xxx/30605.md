# Issue 30605: improve cone containment tests

archive/issues_030368.json:
```json
{
    "assignees": [],
    "body": "If `K` is a polyhedral cone, the test `x in K` only supports `x` with rational coordinates. It is desirable in many cases to perform the same test with irrational numbers as well.\n\nThis will simplify the implementation in ticket #29169.\n\n\nCC:  @kliem @jplab @novoselt\n\nBranch/Commit: **[7ea2dd8](https://github.com/sagemath/sagetrac-mirror/commit/7ea2dd880b8a3047a05be8bb1d521b48e134cf73)**\n\nReviewer: **Jonathan Kliem**\n\nAuthor: **Michael Orlitzky**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30605_\n\n",
    "closed_at": "2020-09-30T22:29:56Z",
    "created_at": "2020-09-18T20:33:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve cone containment tests",
    "type": "issue",
    "updated_at": "2020-09-30T22:29:56Z",
    "url": "https://github.com/sagemath/sage/issues/30605",
    "user": "https://github.com/orlitzky"
}
```
If `K` is a polyhedral cone, the test `x in K` only supports `x` with rational coordinates. It is desirable in many cases to perform the same test with irrational numbers as well.

This will simplify the implementation in ticket #29169.


CC:  @kliem @jplab @novoselt

Branch/Commit: **[7ea2dd8](https://github.com/sagemath/sagetrac-mirror/commit/7ea2dd880b8a3047a05be8bb1d521b48e134cf73)**

Reviewer: **Jonathan Kliem**

Author: **Michael Orlitzky**

_Issue created by migration from https://trac.sagemath.org/ticket/30605_





---

archive/issue_events_403685.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-09-18T20:33:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30605#event-403685"
}
```



---

archive/issue_events_403686.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-09-18T20:33:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30605#event-403686"
}
```



---

archive/issue_events_403687.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-09-18T20:33:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30605#event-403687"
}
```



---

archive/issue_events_403688.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-09-18T20:33:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30605#event-403688"
}
```



---

archive/issue_comments_491318.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nI was thinking what should be done with reals and other non-exact things, but I think it does not matter - as long as exact ones are handled correctly, it is user's responsibility to be aware that non-exact things may give wrong results without extra care.",
    "created_at": "2020-09-19T03:11:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491318",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:1" align="right">Comment 1</div>

I was thinking what should be done with reals and other non-exact things, but I think it does not matter - as long as exact ones are handled correctly, it is user's responsibility to be aware that non-exact things may give wrong results without extra care.



---

archive/issue_comments_491319.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThe difficulty is never where you think it will be...\n\n```\nsage: bool(I >= 0)                                                              \nTrue\n```",
    "created_at": "2020-09-19T03:38:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491319",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:2" align="right">Comment 2</div>

The difficulty is never where you think it will be...

```
sage: bool(I >= 0)                                                              
True
```



---

archive/issue_comments_491320.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMaybe this suffices?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cf365166f6af3edaed08895fafcbc7305f70e467\">cf36516</a></td><td><code>Trac #30605: try AA before RR in cone._ambient_space_point().</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/276db84b14e0004da821f4af729b1fb3de53c52f\">276db84</a></td><td><code>Trac #30605: catch more coercion failures in cone._ambient_space_point().</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/507affb163b2113e1b77d6858efaf65e2170a46d\">507affb</a></td><td><code>Trac #30605: factor out repetition in cone._ambient_space_point().</code></td></tr></table>\n",
    "created_at": "2020-09-19T12:39:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491320",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:3" align="right">Comment 3</div>

Maybe this suffices?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cf365166f6af3edaed08895fafcbc7305f70e467">cf36516</a></td><td><code>Trac #30605: try AA before RR in cone._ambient_space_point().</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/276db84b14e0004da821f4af729b1fb3de53c52f">276db84</a></td><td><code>Trac #30605: catch more coercion failures in cone._ambient_space_point().</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/507affb163b2113e1b77d6858efaf65e2170a46d">507affb</a></td><td><code>Trac #30605: factor out repetition in cone._ambient_space_point().</code></td></tr></table>




---

archive/issue_comments_491321.json:
```json
{
    "body": "Branch: **[u/mjo/ticket/30605](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/30605)**",
    "created_at": "2020-09-19T12:39:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491321",
    "user": "https://github.com/orlitzky"
}
```

Branch: **[u/mjo/ticket/30605](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/30605)**



---

archive/issue_comments_491322.json:
```json
{
    "body": "Commit: **[507affb](https://github.com/sagemath/sagetrac-mirror/commit/507affb163b2113e1b77d6858efaf65e2170a46d)**",
    "created_at": "2020-09-19T12:39:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491322",
    "user": "https://github.com/orlitzky"
}
```

Commit: **[507affb](https://github.com/sagemath/sagetrac-mirror/commit/507affb163b2113e1b77d6858efaf65e2170a46d)**



---

archive/issue_events_403689.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-09-19T12:39:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30605#event-403689"
}
```



---

archive/issue_comments_491323.json:
```json
{
    "body": "Author: **Michael Orlitzky**",
    "created_at": "2020-09-19T12:40:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491323",
    "user": "https://github.com/orlitzky"
}
```

Author: **Michael Orlitzky**



---

archive/issue_comments_491324.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/09c2fa3903f4e6f1e5d06bc1fbcbad519521cf8e\">09c2fa3</a></td><td><code>Trac #30605: update polyhedral cone containment documentation.</code></td></tr></table>\n",
    "created_at": "2020-09-19T15:33:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491324",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/09c2fa3903f4e6f1e5d06bc1fbcbad519521cf8e">09c2fa3</a></td><td><code>Trac #30605: update polyhedral cone containment documentation.</code></td></tr></table>




---

archive/issue_comments_491325.json:
```json
{
    "body": "Changed commit from **[507affb](https://github.com/sagemath/sagetrac-mirror/commit/507affb163b2113e1b77d6858efaf65e2170a46d)** to **[09c2fa3](https://github.com/sagemath/sagetrac-mirror/commit/09c2fa3903f4e6f1e5d06bc1fbcbad519521cf8e)**",
    "created_at": "2020-09-19T15:33:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491325",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[507affb](https://github.com/sagemath/sagetrac-mirror/commit/507affb163b2113e1b77d6858efaf65e2170a46d)** to **[09c2fa3](https://github.com/sagemath/sagetrac-mirror/commit/09c2fa3903f4e6f1e5d06bc1fbcbad519521cf8e)**



---

archive/issue_comments_491326.json:
```json
{
    "body": "Reviewer: **Jonathan Kliem**",
    "created_at": "2020-09-23T06:44:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491326",
    "user": "https://github.com/kliem"
}
```

Reviewer: **Jonathan Kliem**



---

archive/issue_comments_491327.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nCould you follow the recommendation of `pycodestyle` and do one command per line, e.g.\n\n```diff\n-        if p is not None: return p\n+        if p is not None:\n+            return p\n```\n\nor\n\n```diff\n-        try: return L.base_extend(ring)(data)\n-        except TypeError: pass\n-        except ValueError as ex:\n-            if str(ex).startswith(\"Cannot coerce\"): pass\n+        try:\n+            return L.base_extend(ring)(data)\n+        except TypeError:\n+            pass\n+        except ValueError as ex:\n+            if str(ex).startswith(\"Cannot coerce\"):\n+                pass\n```\n\nI think this make it easier to read.\n\nOnce done, you can set in on positive review on my behalf.",
    "created_at": "2020-09-23T06:44:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491327",
    "user": "https://github.com/kliem"
}
```

<div id="comment:6" align="right">Comment 6</div>

Could you follow the recommendation of `pycodestyle` and do one command per line, e.g.

```diff
-        if p is not None: return p
+        if p is not None:
+            return p
```

or

```diff
-        try: return L.base_extend(ring)(data)
-        except TypeError: pass
-        except ValueError as ex:
-            if str(ex).startswith("Cannot coerce"): pass
+        try:
+            return L.base_extend(ring)(data)
+        except TypeError:
+            pass
+        except ValueError as ex:
+            if str(ex).startswith("Cannot coerce"):
+                pass
```

I think this make it easier to read.

Once done, you can set in on positive review on my behalf.



---

archive/issue_comments_491328.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7ea2dd880b8a3047a05be8bb1d521b48e134cf73\">7ea2dd8</a></td><td><code>Trac #30605: reformat according to pycodestyle.</code></td></tr></table>\n",
    "created_at": "2020-09-23T21:28:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491328",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7ea2dd880b8a3047a05be8bb1d521b48e134cf73">7ea2dd8</a></td><td><code>Trac #30605: reformat according to pycodestyle.</code></td></tr></table>




---

archive/issue_comments_491329.json:
```json
{
    "body": "Changed commit from **[09c2fa3](https://github.com/sagemath/sagetrac-mirror/commit/09c2fa3903f4e6f1e5d06bc1fbcbad519521cf8e)** to **[7ea2dd8](https://github.com/sagemath/sagetrac-mirror/commit/7ea2dd880b8a3047a05be8bb1d521b48e134cf73)**",
    "created_at": "2020-09-23T21:28:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491329",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[09c2fa3](https://github.com/sagemath/sagetrac-mirror/commit/09c2fa3903f4e6f1e5d06bc1fbcbad519521cf8e)** to **[7ea2dd8](https://github.com/sagemath/sagetrac-mirror/commit/7ea2dd880b8a3047a05be8bb1d521b48e134cf73)**



---

archive/issue_events_403690.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-09-23T21:30:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30605#event-403690"
}
```



---

archive/issue_events_403691.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-09-23T21:30:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30605#event-403691"
}
```



---

archive/issue_comments_491330.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThanks! I fixed the indentation in one final (separate) commit if you want to take a look. But pycodestyle is happy now.",
    "created_at": "2020-09-23T21:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491330",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:8" align="right">Comment 8</div>

Thanks! I fixed the indentation in one final (separate) commit if you want to take a look. But pycodestyle is happy now.



---

archive/issue_comments_491331.json:
```json
{
    "body": "Changed branch from **[u/mjo/ticket/30605](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/30605)** to **[7ea2dd8](https://github.com/sagemath/sagetrac-mirror/commit/7ea2dd880b8a3047a05be8bb1d521b48e134cf73)**",
    "created_at": "2020-09-30T22:29:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30605#issuecomment-491331",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mjo/ticket/30605](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/30605)** to **[7ea2dd8](https://github.com/sagemath/sagetrac-mirror/commit/7ea2dd880b8a3047a05be8bb1d521b48e134cf73)**



---

archive/issue_events_403692.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-09-30T22:29:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30605#event-403692"
}
```



---

archive/issue_events_403693.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ed7c4a3ee0757863298aa7a459c77c000b2764b5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-09-30T22:29:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30605",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30605#event-403693"
}
```
