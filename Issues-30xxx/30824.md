# Issue 30824: More Flexible Coordinate Treatment for EuclideanSpace

archive/issues_030587.json:
```json
{
    "assignees": [],
    "body": "See #30189.\n\nCurrently, each coordinate has its own attribute in `EuclideanSpace`. I propose a more general setup using dictionaries. Morever, the user should be able to input their own coordinate initialization method if he/she wants to.\n\nCC:  @egourgoulhon @tscrim\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30824_\n\n",
    "created_at": "2020-10-25T12:36:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "More Flexible Coordinate Treatment for EuclideanSpace",
    "type": "issue",
    "updated_at": "2022-08-31T02:51:13Z",
    "url": "https://github.com/sagemath/sage/issues/30824",
    "user": "https://github.com/mjungmath"
}
```
See #30189.

Currently, each coordinate has its own attribute in `EuclideanSpace`. I propose a more general setup using dictionaries. Morever, the user should be able to input their own coordinate initialization method if he/she wants to.

CC:  @egourgoulhon @tscrim

_Issue created by migration from https://trac.sagemath.org/ticket/30824_





---

archive/issue_events_406900.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2020-10-25T12:36:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406900"
}
```



---

archive/issue_events_406901.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2020-10-25T12:36:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406901"
}
```



---

archive/issue_events_406902.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2020-10-25T12:40:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406902"
}
```



---

archive/issue_comments_496202.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Currently, each coordinate has its own attribute in `EuclideanSpace`. I propose a more general setup using dictionaries. Morever, the user should be able to input their own coordinate initialization method if he/she wants to.\n``````\n",
    "created_at": "2020-10-25T12:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30824#issuecomment-496202",
    "user": "https://github.com/mjungmath"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Currently, each coordinate has its own attribute in `EuclideanSpace`. I propose a more general setup using dictionaries. Morever, the user should be able to input their own coordinate initialization method if he/she wants to.
``````




---

archive/issue_events_406903.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2020-10-25T12:40:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
    "label_color": "0000ff",
    "label_name": "component: manifolds",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406903"
}
```



---

archive/issue_comments_496203.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+See #30189.\n+\n Currently, each coordinate has its own attribute in `EuclideanSpace`. I propose a more general setup using dictionaries. Morever, the user should be able to input their own coordinate initialization method if he/she wants to.\n``````\n",
    "created_at": "2020-10-25T13:18:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30824#issuecomment-496203",
    "user": "https://github.com/mjungmath"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+See #30189.
+
 Currently, each coordinate has its own attribute in `EuclideanSpace`. I propose a more general setup using dictionaries. Morever, the user should be able to input their own coordinate initialization method if he/she wants to.
``````




---

archive/issue_comments_496204.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30824#issuecomment-496204",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_406904.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406904"
}
```



---

archive/issue_events_406905.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406905"
}
```



---

archive/issue_events_406906.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406906"
}
```



---

archive/issue_events_406907.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406907"
}
```



---

archive/issue_events_406908.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406908"
}
```



---

archive/issue_events_406909.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406909"
}
```



---

archive/issue_events_406910.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406910"
}
```



---

archive/issue_events_406911.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406911"
}
```



---

archive/issue_events_406912.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406912"
}
```



---

archive/issue_events_406913.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30824",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30824#event-406913"
}
```
