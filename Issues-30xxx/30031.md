# Issue 30031: sage.numerical.backends.generic_sdp_backend: Allow solver = a user-defined subclass of GenericSDPBackend

archive/issues_029794.json:
```json
{
    "assignees": [],
    "body": "... in analogy to #28914.\n\n(However, in contrast to #28914, the solver must be a subclass of `GenericSDPBackend`.)\n\nCC:  @dimpase @yuan-zhou\n\nBranch/Commit: **[51dd905](https://github.com/sagemath/sagetrac-mirror/commit/51dd905b51e8c3c6134430e4774fa3c90839e549)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30031_\n\n",
    "closed_at": "2020-09-01T23:00:00Z",
    "created_at": "2020-06-30T18:52:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.numerical.backends.generic_sdp_backend: Allow solver = a user-defined subclass of GenericSDPBackend",
    "type": "issue",
    "updated_at": "2020-09-01T23:00:00Z",
    "url": "https://github.com/sagemath/sage/issues/30031",
    "user": "https://github.com/mkoeppe"
}
```
... in analogy to #28914.

(However, in contrast to #28914, the solver must be a subclass of `GenericSDPBackend`.)

CC:  @dimpase @yuan-zhou

Branch/Commit: **[51dd905](https://github.com/sagemath/sagetrac-mirror/commit/51dd905b51e8c3c6134430e4774fa3c90839e549)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/30031_





---

archive/issue_events_395277.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-30T18:52:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395277"
}
```



---

archive/issue_events_395278.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-30T18:52:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395278"
}
```



---

archive/issue_events_395279.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-30T18:52:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395279"
}
```



---

archive/issue_events_395280.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-30T18:52:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395280"
}
```



---

archive/issue_comments_478238.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-06-30T19:15:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478238",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_478239.json:
```json
{
    "body": "Branch: **[u/mkoeppe/sage_numerical_backends_generic_sdp_backend__allow_solver___a_callable](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_numerical_backends_generic_sdp_backend__allow_solver___a_callable)**",
    "created_at": "2020-06-30T19:41:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478239",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/sage_numerical_backends_generic_sdp_backend__allow_solver___a_callable](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_numerical_backends_generic_sdp_backend__allow_solver___a_callable)**



---

archive/issue_comments_478240.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ff3d42516ddff823808886a403d74172d62e6e7b\">ff3d425</a></td><td><code>sage.numerical.backends.generic_sdp_backend: Allow solver = a callable</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3d1cca70c2726643c9c3886540bec69a2d5bf687\">3d1cca7</a></td><td><code>sage.numerical.sdp, sage.numerical.backends.generic_sdp_backend: Allow duck typing of SDP backends</code></td></tr></table>\n",
    "created_at": "2020-06-30T20:04:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478240",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ff3d42516ddff823808886a403d74172d62e6e7b">ff3d425</a></td><td><code>sage.numerical.backends.generic_sdp_backend: Allow solver = a callable</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3d1cca70c2726643c9c3886540bec69a2d5bf687">3d1cca7</a></td><td><code>sage.numerical.sdp, sage.numerical.backends.generic_sdp_backend: Allow duck typing of SDP backends</code></td></tr></table>




---

archive/issue_events_395281.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-30T20:04:22Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "title_is": "sage.numerical.backends.generic_sdp_backend: Allow solver = a user-defined subclass of GenericSDPBackend",
    "title_was": "sage.numerical.backends.generic_sdp_backend: Allow solver = a callable",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395281"
}
```



---

archive/issue_comments_478241.json:
```json
{
    "body": "Commit: **[3d1cca7](https://github.com/sagemath/sagetrac-mirror/commit/3d1cca70c2726643c9c3886540bec69a2d5bf687)**",
    "created_at": "2020-06-30T20:04:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478241",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[3d1cca7](https://github.com/sagemath/sagetrac-mirror/commit/3d1cca70c2726643c9c3886540bec69a2d5bf687)**



---

archive/issue_comments_478242.json:
```json
{
    "body": "Changed commit from **[3d1cca7](https://github.com/sagemath/sagetrac-mirror/commit/3d1cca70c2726643c9c3886540bec69a2d5bf687)** to **[51dd905](https://github.com/sagemath/sagetrac-mirror/commit/51dd905b51e8c3c6134430e4774fa3c90839e549)**",
    "created_at": "2020-06-30T20:13:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478242",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3d1cca7](https://github.com/sagemath/sagetrac-mirror/commit/3d1cca70c2726643c9c3886540bec69a2d5bf687)** to **[51dd905](https://github.com/sagemath/sagetrac-mirror/commit/51dd905b51e8c3c6134430e4774fa3c90839e549)**



---

archive/issue_comments_478243.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/51dd905b51e8c3c6134430e4774fa3c90839e549\">51dd905</a></td><td><code>sage.numerical.backends.generic_sdp_backend: Allow solver = a user-defined subclass of GenericSDPBackend</code></td></tr></table>\n",
    "created_at": "2020-06-30T20:13:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478243",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/51dd905b51e8c3c6134430e4774fa3c90839e549">51dd905</a></td><td><code>sage.numerical.backends.generic_sdp_backend: Allow solver = a user-defined subclass of GenericSDPBackend</code></td></tr></table>




---

archive/issue_events_395282.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-30T20:14:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395282"
}
```



---

archive/issue_comments_478244.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n ... in analogy to #28914.\n \n-\n+(However, in contrast to #28914, the solver must be a subclass of `GenericSDPBackend`.)\n``````\n",
    "created_at": "2020-06-30T20:14:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478244",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 ... in analogy to #28914.
 
-
+(However, in contrast to #28914, the solver must be a subclass of `GenericSDPBackend`.)
``````




---

archive/issue_comments_478245.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nas a concrete example of using this, could we have csdp (an optional sage spkg) as a backend this way?\n\nit does even have a dynamic library available.",
    "created_at": "2020-07-01T11:18:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478245",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:6" align="right">Comment 6</div>

as a concrete example of using this, could we have csdp (an optional sage spkg) as a backend this way?

it does even have a dynamic library available.



---

archive/issue_comments_478246.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThe doctests already provide a concrete (but useless) example.\n\nBefore spending effort on sage-specific SDP backend development, let's survey what kind of interfacing routes for SDP solvers are available. Either by extending  \nMeta-ticket #26511 (Use Python optimization interfaces: PuLP, Pyomo, cylp...), which is currently LP/MILP-centric, or a separate meta ticket for SDP solvers.",
    "created_at": "2020-07-02T19:31:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478246",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">Comment 7</div>

The doctests already provide a concrete (but useless) example.

Before spending effort on sage-specific SDP backend development, let's survey what kind of interfacing routes for SDP solvers are available. Either by extending  
Meta-ticket #26511 (Use Python optimization interfaces: PuLP, Pyomo, cylp...), which is currently LP/MILP-centric, or a separate meta ticket for SDP solvers.



---

archive/issue_comments_478247.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nAlso see #30049 (`MatrixSDPBackend`).",
    "created_at": "2020-07-02T19:38:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478247",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Also see #30049 (`MatrixSDPBackend`).



---

archive/issue_comments_478248.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nLet's get this in please",
    "created_at": "2020-08-24T17:31:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478248",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:9" align="right">Comment 9</div>

Let's get this in please



---

archive/issue_events_395283.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-08-26T23:08:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395283"
}
```



---

archive/issue_events_395284.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-08-26T23:08:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395284"
}
```



---

archive/issue_comments_478249.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-08-26T23:08:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478249",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_478250.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nlgtm.",
    "created_at": "2020-08-26T23:08:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478250",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:10" align="right">Comment 10</div>

lgtm.



---

archive/issue_comments_478251.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThanks!",
    "created_at": "2020-08-26T23:26:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478251",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:11" align="right">Comment 11</div>

Thanks!



---

archive/issue_events_395285.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-09-01T23:00:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395285"
}
```



---

archive/issue_events_395286.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7fe518dc20ddfaa6e8d1df20c5ba9476e2ee653a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-09-01T23:00:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30031#event-395286"
}
```



---

archive/issue_comments_478252.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/sage_numerical_backends_generic_sdp_backend__allow_solver___a_callable](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_numerical_backends_generic_sdp_backend__allow_solver___a_callable)** to **[51dd905](https://github.com/sagemath/sagetrac-mirror/commit/51dd905b51e8c3c6134430e4774fa3c90839e549)**",
    "created_at": "2020-09-01T23:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30031#issuecomment-478252",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/sage_numerical_backends_generic_sdp_backend__allow_solver___a_callable](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_numerical_backends_generic_sdp_backend__allow_solver___a_callable)** to **[51dd905](https://github.com/sagemath/sagetrac-mirror/commit/51dd905b51e8c3c6134430e4774fa3c90839e549)**
