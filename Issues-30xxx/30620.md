...
<a id='comment:1'></a>From [#26410](https://trac.sagemath.org/ticket/26410#comment:12) as asked by Joshua Campbell:
...
```json
{
    "body": "<a id='comment:2'></a>If it comes to maintaining a separate non-modular set of files, I am committed to providing `OrbitControls.js` at the very least. I'm also hoping to prod Mr.doob into adding some basic controls to the main library.",
...
<a id='comment:2'></a>If it comes to maintaining a separate non-modular set of files, I am committed to providing `OrbitControls.js` at the very least. I'm also hoping to prod Mr.doob into adding some basic controls to the main library.
...
<a id='comment:8'></a>Joshua, keep in mind that browsers can only display a limited number of WebGL instances at once, typically 16. A notebook, being a browser, will lose instances once that limit is exceeded. There is a way to fix that using IntersectionObserver, but that would need to be done outside of Sage.
...
<a id='comment:10'></a>Joshua, I'm attaching a preliminary version of the minified build to this ticket so you can experiment with it. The build depends on features recently added to r121, so we'll wait for that release before updating things here. The build requires having Node/npm installed and goes like this:
...
<a id='comment:11'></a>Attachment [three.min.js](tarball://root/attachments/some-uuid/ticket30620/three.min.js) by @mkoeppe created at 2020-09-26 00:26:06
...
```json
{
    "body": "<a id='comment:13'></a>In general I think it is best to have a source repositories for sources, not build artifacts, and to publish build artifacts using the distribution infrastructure that is standard for the language used. \n\nTo make the build process convenient and transparent, one can use GitHub Actions to automate the process. https://docs.github.com/en/free-pro-team`@`latest/actions/guides/publishing-nodejs-packages",
...
<a id='comment:13'></a>In general I think it is best to have a source repositories for sources, not build artifacts, and to publish build artifacts using the distribution infrastructure that is standard for the language used. 
...
```json
{
    "body": "<a id='comment:14'></a>Upstream uses tags like `r120`, and you could be using `r120-sage` or something like this.",
...
<a id='comment:14'></a>Upstream uses tags like `r120`, and you could be using `r120-sage` or something like this.
...
<a id='comment:15'></a>Replying to [paulmasson](#comment%3A10):
> Joshua, I'm attaching a preliminary version of the minified build to this ticket so you can experiment with it. The build depends on features recently added to r121, so we'll wait for that release before updating things here. The build requires having Node/npm installed and goes like this:
> 
> * perform a shallow clone of the target branch with 
> 
...
<a id='comment:16'></a>Replying to [gh-jcamp0x2a](#comment%3A15):
> Replying to [paulmasson](#comment%3A10):
> > Joshua, I'm attaching a preliminary version of the minified build to this ticket so you can experiment with it. The build depends on features recently added to r121, so we'll wait for that release before updating things here. The build requires having Node/npm installed and goes like this:
> > 
> > * perform a shallow clone of the target branch with 
...
<a id='comment:17'></a>Replying to [mkoeppe](#comment%3A13):
> In general I think it is best to have a source repositories for sources, not build artifacts, and to publish build artifacts using the distribution infrastructure that is standard for the language used. 
> 
> To make the build process convenient and transparent, one can use GitHub Actions to automate the process. https://docs.github.com/en/free-pro-team`@`latest/actions/guides/publishing-nodejs-packages
> 
...
<a id='comment:18'></a>Replying to [paulmasson](#comment%3A16):
> Yeah, I overlooked `Line2`, so I've edited the instructions above. The Closure compiler is already gone from `r121dev` so you're building against `r120`. Since you've got a build I'll remove my attachment. Bet your final build is twice what it should be, since that was only fixed in the current `dev` branch...
...
<a id='comment:19'></a>Replying to [gh-jcamp0x2a](#comment%3A18):
> Replying to [paulmasson](#comment%3A16):
> > Yeah, I overlooked `Line2`, so I've edited the instructions above. The Closure compiler is already gone from `r121dev` so you're building against `r120`. Since you've got a build I'll remove my attachment. Bet your final build is twice what it should be, since that was only fixed in the current `dev` branch...
...
<a id='comment:20'></a>Replying to [paulmasson](#comment%3A12):
> Replying to [mkoeppe](#comment%3A11):
> > From the recipe - adding the 5 lines to `src/Three.js` - does this not just mean that you want to maintain a git branch with these lines added and merge in the new upstream releases when they become available?
...
<a id='comment:22'></a>I have some naive questions. 
- It seems that deployment is intended to be done via cdn.jsdelivr.net; do you intend to also provide local deployment for use in Sage?
- Since this is a js package, wouldn't people want to be able to install it with npm?
...
<a id='comment:25'></a>Replying to [gh-jcamp0x2a](#comment%3A18):
> Replying to [paulmasson](#comment%3A16):
...
```json
{
    "body": "<a id='comment:26'></a>I've added a comment to https://github.com/paulmasson/threejs-sage/issues/1",
...
<a id='comment:26'></a>I've added a comment to https://github.com/paulmasson/threejs-sage/issues/1
