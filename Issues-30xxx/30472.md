# Issue 30472: Change remaining all-caps SAGE to Sage

archive/issues_030235.json:
```json
{
    "assignees": [],
    "body": "Originally Sage was called SAGE (in all-caps).\n\nMany code comments, docstrings or error messages\nstill have the all-caps version.\n\nThis ticket is to fix that.\n\nTo locate all-caps `SAGE` not preceded or followed\nby underscore, change to the Sage root folder and run:\n\n```bash\n$ git grep '\\([^_]\\|^\\)SAGE\\([^_]\\|$\\)' src\n```\n\nThis leaves alone environment variables\nsuch as `DOT_SAGE` or `SAGE_SRC` etc.\n\n\nCC:  @fchapoton @slel\n\nKeywords: **typo**\n\nBranch/Commit: **[a71d230](https://github.com/sagemath/sagetrac-mirror/commit/a71d2302c06c612430d01ebb7354f80b47a5b3c4)**\n\nReviewer: **Samuel Leli\u00e8vre**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/30472_\n\n",
    "closed_at": "2020-10-31T18:08:07Z",
    "created_at": "2020-08-30T14:51:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Change remaining all-caps SAGE to Sage",
    "type": "issue",
    "updated_at": "2020-10-31T18:08:07Z",
    "url": "https://github.com/sagemath/sage/issues/30472",
    "user": "https://github.com/slel"
}
```
Originally Sage was called SAGE (in all-caps).

Many code comments, docstrings or error messages
still have the all-caps version.

This ticket is to fix that.

To locate all-caps `SAGE` not preceded or followed
by underscore, change to the Sage root folder and run:

```bash
$ git grep '\([^_]\|^\)SAGE\([^_]\|$\)' src
```

This leaves alone environment variables
such as `DOT_SAGE` or `SAGE_SRC` etc.


CC:  @fchapoton @slel

Keywords: **typo**

Branch/Commit: **[a71d230](https://github.com/sagemath/sagetrac-mirror/commit/a71d2302c06c612430d01ebb7354f80b47a5b3c4)**

Reviewer: **Samuel Lelièvre**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/30472_





---

archive/issue_events_401790.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-30T14:51:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401790"
}
```



---

archive/issue_events_401791.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-30T14:51:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401791"
}
```



---

archive/issue_events_401792.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-30T14:51:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401792"
}
```



---

archive/issue_events_401793.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-30T14:51:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401793"
}
```



---

archive/issue_events_401794.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-30T14:51:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401794"
}
```



---

archive/issue_comments_488500.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nCan someone suggest a better regular expression to\n\n- also avoid `MESSAGE`, `USAGE`, `SAGEDIR`, `$SAGE`\n- not capture adjacent characters",
    "created_at": "2020-08-30T14:55:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488500",
    "user": "https://github.com/slel"
}
```

<div id="comment:1" align="right">Comment 1</div>

Can someone suggest a better regular expression to

- also avoid `MESSAGE`, `USAGE`, `SAGEDIR`, `$SAGE`
- not capture adjacent characters



---

archive/issue_comments_488501.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\n\n```\ngit grep -w SAGE src/sage\n```\n(words only)",
    "created_at": "2020-08-30T17:33:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488501",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>


```
git grep -w SAGE src/sage
```
(words only)



---

archive/issue_comments_488502.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nWe may want to add these patterns to those in #30467... so that also future code is checked against them and these bad patterns do not creep back in.",
    "created_at": "2020-08-30T18:19:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488502",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

We may want to add these patterns to those in #30467... so that also future code is checked against them and these bad patterns do not creep back in.



---

archive/issue_comments_488503.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nUse `\\b` for word boundaries",
    "created_at": "2020-08-30T18:19:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488503",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Use `\b` for word boundaries



---

archive/issue_events_401795.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-31T16:51:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401795"
}
```



---

archive/issue_comments_488504.json:
```json
{
    "body": "Branch: **[public/ticket/30472](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/30472)**",
    "created_at": "2020-08-31T16:51:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488504",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[public/ticket/30472](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/30472)**



---

archive/issue_comments_488505.json:
```json
{
    "body": "Commit: **[68f5e40](https://github.com/sagemath/sagetrac-mirror/commit/68f5e403070494387edb5881e49a516844ff3203)**",
    "created_at": "2020-08-31T16:51:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488505",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[68f5e40](https://github.com/sagemath/sagetrac-mirror/commit/68f5e403070494387edb5881e49a516844ff3203)**



---

archive/issue_comments_488506.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nfirst try\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/68f5e403070494387edb5881e49a516844ff3203\">68f5e40</a></td><td><code>change some SAGE to Sage</code></td></tr></table>\n",
    "created_at": "2020-08-31T16:51:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488506",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

first try

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/68f5e403070494387edb5881e49a516844ff3203">68f5e40</a></td><td><code>change some SAGE to Sage</code></td></tr></table>




---

archive/issue_comments_488507.json:
```json
{
    "body": "Changed commit from **[68f5e40](https://github.com/sagemath/sagetrac-mirror/commit/68f5e403070494387edb5881e49a516844ff3203)** to **[7dacd7c](https://github.com/sagemath/sagetrac-mirror/commit/7dacd7c8e765ee8e901bbe4335c2e0f54cff0466)**",
    "created_at": "2020-08-31T16:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488507",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[68f5e40](https://github.com/sagemath/sagetrac-mirror/commit/68f5e403070494387edb5881e49a516844ff3203)** to **[7dacd7c](https://github.com/sagemath/sagetrac-mirror/commit/7dacd7c8e765ee8e901bbe4335c2e0f54cff0466)**



---

archive/issue_comments_488508.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7dacd7c8e765ee8e901bbe4335c2e0f54cff0466\">7dacd7c</a></td><td><code>more SAGE to Sage</code></td></tr></table>\n",
    "created_at": "2020-08-31T16:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488508",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7dacd7c8e765ee8e901bbe4335c2e0f54cff0466">7dacd7c</a></td><td><code>more SAGE to Sage</code></td></tr></table>




---

archive/issue_comments_488509.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nIn `src/sage/schemes/plane_conics/all.py` and elsewhere, while we are at it:\n\n```diff\n-#   Sage: System for Algebra and Geometry Experimentation\n+#   Sage: Open Source Mathematical Software\n```\n\nIn `src/sage/matroids/linear_matroid.pyx` maybe un-all-caps this comment:\n\n```diff\n-        # REMNANT OF THE OLD CODE THAT WAS NOT YET TRANSLATED\n-        # TO Sage'S GRAPH ISOMORPHISM. POTENTIAL SPEEDUP?\n+        # Remnant of the old code that was not yet translated\n+        # to Sage's graph isomorphism. Potential speedup?\n```\n\nIn `src/sage/symbolic/todo.txt`, properly capitalize other software names too?\n\n```diff\n-* Maybe change Sage's Ginac to make a call to a cython gcd function, then use\n-singular, since singular's gcd over QQ is much better than ginac's, I think,\n-and ginac *only* does GCD over QQ.  Actually, just make everything in normal.cpp\n-be implemented via Singular, probably...\n+* Maybe change Sage's GiNaC to make a call to a Cython gcd function, then use\n+  Singular, since Singular's gcd over QQ is much better than GiNaC's, I think,\n+  and GiNaC *only* does gcd over QQ.  Actually, just make everything in\n+  normal.cpp be implemented via Singular, probably...\n```\n\nThat todo file should really go, see #30482.",
    "created_at": "2020-08-31T23:49:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488509",
    "user": "https://github.com/slel"
}
```

<div id="comment:7" align="right">Comment 7</div>

In `src/sage/schemes/plane_conics/all.py` and elsewhere, while we are at it:

```diff
-#   Sage: System for Algebra and Geometry Experimentation
+#   Sage: Open Source Mathematical Software
```

In `src/sage/matroids/linear_matroid.pyx` maybe un-all-caps this comment:

```diff
-        # REMNANT OF THE OLD CODE THAT WAS NOT YET TRANSLATED
-        # TO Sage'S GRAPH ISOMORPHISM. POTENTIAL SPEEDUP?
+        # Remnant of the old code that was not yet translated
+        # to Sage's graph isomorphism. Potential speedup?
```

In `src/sage/symbolic/todo.txt`, properly capitalize other software names too?

```diff
-* Maybe change Sage's Ginac to make a call to a cython gcd function, then use
-singular, since singular's gcd over QQ is much better than ginac's, I think,
-and ginac *only* does GCD over QQ.  Actually, just make everything in normal.cpp
-be implemented via Singular, probably...
+* Maybe change Sage's GiNaC to make a call to a Cython gcd function, then use
+  Singular, since Singular's gcd over QQ is much better than GiNaC's, I think,
+  and GiNaC *only* does gcd over QQ.  Actually, just make everything in
+  normal.cpp be implemented via Singular, probably...
```

That todo file should really go, see #30482.



---

archive/issue_comments_488510.json:
```json
{
    "body": "Changed commit from **[7dacd7c](https://github.com/sagemath/sagetrac-mirror/commit/7dacd7c8e765ee8e901bbe4335c2e0f54cff0466)** to **[a71d230](https://github.com/sagemath/sagetrac-mirror/commit/a71d2302c06c612430d01ebb7354f80b47a5b3c4)**",
    "created_at": "2020-09-01T10:00:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488510",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7dacd7c](https://github.com/sagemath/sagetrac-mirror/commit/7dacd7c8e765ee8e901bbe4335c2e0f54cff0466)** to **[a71d230](https://github.com/sagemath/sagetrac-mirror/commit/a71d2302c06c612430d01ebb7354f80b47a5b3c4)**



---

archive/issue_comments_488511.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/46f0b3353746e76b3d0fa17e87899acdf43460fc\">46f0b33</a></td><td><code>massive refresh of banners</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a71d2302c06c612430d01ebb7354f80b47a5b3c4\">a71d230</a></td><td><code>various details</code></td></tr></table>\n",
    "created_at": "2020-09-01T10:00:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488511",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/46f0b3353746e76b3d0fa17e87899acdf43460fc">46f0b33</a></td><td><code>massive refresh of banners</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a71d2302c06c612430d01ebb7354f80b47a5b3c4">a71d230</a></td><td><code>various details</code></td></tr></table>




---

archive/issue_comments_488512.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\ndone",
    "created_at": "2020-09-01T10:01:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488512",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:9" align="right">Comment 9</div>

done



---

archive/issue_comments_488513.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nThanks!",
    "created_at": "2020-09-01T12:53:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488513",
    "user": "https://github.com/slel"
}
```

<div id="comment:10" align="right">Comment 10</div>

Thanks!



---

archive/issue_events_401796.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-09-01T12:53:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401796"
}
```



---

archive/issue_events_401797.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-09-01T12:53:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401797"
}
```



---

archive/issue_comments_488514.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2020-09-01T12:53:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488514",
    "user": "https://github.com/slel"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_488515.json:
```json
{
    "body": "Reviewer: **Samuel Leli\u00e8vre**",
    "created_at": "2020-09-01T12:53:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488515",
    "user": "https://github.com/slel"
}
```

Reviewer: **Samuel Lelièvre**



---

archive/issue_events_401798.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-10-11T16:38:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401798"
}
```



---

archive/issue_events_401799.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-10-11T16:38:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401799"
}
```



---

archive/issue_comments_488516.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nMarking critical since it's been so long this name change happened.\n\nI really want this to be in Sage 9.2 so we turn that page.",
    "created_at": "2020-10-11T16:38:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488516",
    "user": "https://github.com/slel"
}
```

<div id="comment:11" align="right">Comment 11</div>

Marking critical since it's been so long this name change happened.

I really want this to be in Sage 9.2 so we turn that page.



---

archive/issue_events_401800.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401800"
}
```



---

archive/issue_events_401801.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401801"
}
```



---

archive/issue_comments_488517.json:
```json
{
    "body": "Changed branch from **[public/ticket/30472](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/30472)** to **[a71d230](https://github.com/sagemath/sagetrac-mirror/commit/a71d2302c06c612430d01ebb7354f80b47a5b3c4)**",
    "created_at": "2020-10-31T18:08:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/30472#issuecomment-488517",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/ticket/30472](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/30472)** to **[a71d230](https://github.com/sagemath/sagetrac-mirror/commit/a71d2302c06c612430d01ebb7354f80b47a5b3c4)**



---

archive/issue_events_401802.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-10-31T18:08:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401802"
}
```



---

archive/issue_events_401803.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9c1eef89864487d13a677ef61d916bb3d9f4e4ff",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-10-31T18:08:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/30472",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/30472#event-401803"
}
```
