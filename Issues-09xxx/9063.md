...
```json
{
    "body": "<a id='comment:1'></a>If the element of the ring is zero or the base_ring does not have a denominator function, the code returned the \"wrong 1\", either int 1 or polynomial 1. The patch correct this.",
...
<a id='comment:1'></a>If the element of the ring is zero or the base_ring does not have a denominator function, the code returned the "wrong 1", either int 1 or polynomial 1. The patch correct this.
...
```json
{
    "body": "<a id='comment:2'></a>Attachment [trac_9063.patch](tarball://root/attachments/some-uuid/ticket9063/trac_9063.patch) by @lftabera created at 2010-06-24 11:26:48\n\nCorrected a typo in documentation",
...
<a id='comment:2'></a>Attachment [trac_9063.patch](tarball://root/attachments/some-uuid/ticket9063/trac_9063.patch) by @lftabera created at 2010-06-24 11:26:48
...
<a id='comment:4'></a>This looks good to me.  It applied ok to 4.5.3.alpha1, and all (long) tests pass, except for one:
...
<a id='comment:6'></a>Replying to [lftabera](#comment%3A5):
> Well, 
> 
> The issue of adding a lcm function for finite fields has some problems associated:
> 
...
```json
{
    "body": "<a id='comment:7'></a>I have added a new bug dealing with the issue of lcm. So this bug now depends on #9819",
...
<a id='comment:7'></a>I have added a new bug dealing with the issue of lcm. So this bug now depends on #9819
...
<a id='comment:10'></a>Attachment [weak-popof-form.patch](tarball://root/attachments/some-uuid/ticket9063/weak-popof-form.patch) by @lftabera created at 2010-09-03 11:37:47
...
<a id='comment:11'></a>AFAICT, the `weak_popov_form` function clears the denominators to work over the polynomial ring instead of the fraction field. When a matrix over the polynomial ring is given, it seems inefficient to first coerce all entries to the fraction field, then find the denominator `1` and go back to the polynomials we were given.
...
```json
{
    "body": "<a id='comment:12'></a>Attachment [weak-popof-form.2.patch](tarball://root/attachments/some-uuid/ticket9063/weak-popof-form.2.patch) by @lftabera created at 2010-10-08 16:08:52\n\nI have added a new patch to weak_popov_form so it does not perform any cleaning of denominadors when the matrix has polynomial entries.\n\nApply:\n\n\ntrac_9063.patch\n\nweak-popof-form.2.patch\n\nIn any order",
...
<a id='comment:12'></a>Attachment [weak-popof-form.2.patch](tarball://root/attachments/some-uuid/ticket9063/weak-popof-form.2.patch) by @lftabera created at 2010-10-08 16:08:52
...
<a id='comment:14'></a>failed on coverage for both files 
...
```json
{
    "body": "<a id='comment:15'></a>Sorry, I was following developers guide, which stated if coverage is less than 100% than it needs work. However, given the functions that you changed are not the ones where coverage failed, I put it back to needs_review. I will be running other test now",
    "created_at": "2011-01-08T02:58:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9063",
    "type": "issue_comment",
...
<a id='comment:15'></a>Sorry, I was following developers guide, which stated if coverage is less than 100% than it needs work. However, given the functions that you changed are not the ones where coverage failed, I put it back to needs_review. I will be running other test now
