...
```json
{
    "body": "<a id='comment:1'></a>I've attached a trial patch that uses `cd \"$SAGE_PACKAGES/build/\"`.",
    "created_at": "2010-07-07T05:40:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9444",
    "type": "issue_comment",
...
<a id='comment:1'></a>I've attached a trial patch that uses `cd "$SAGE_PACKAGES/build/"`.
...
<a id='comment:2'></a>This looks good and I'm confident it would fix this rather annoying bug. 
...
<a id='comment:3'></a>I think that this message only appears on Solaris; at least, that's been my experience.  On sage.math, for example, I can do this:
...
```json
{
    "body": "<a id='comment:4'></a>mpatel: I assume this is ready for review?",
...
<a id='comment:4'></a>mpatel: I assume this is ready for review?
...
<a id='comment:5'></a>I'm ok with the patch, though we could replicate
...
```json
{
    "body": "<a id='comment:6'></a>I'm happy too. My initial concern was that the directory was not getting deleted on any operating system, in which case deleting it could be dangerous. But now I realise it gets deleted on Linux, then clearly there is no risk. \n\nI'm 100% happy with this. \n\nDave",
...
<a id='comment:6'></a>I'm happy too. My initial concern was that the directory was not getting deleted on any operating system, in which case deleting it could be dangerous. But now I realise it gets deleted on Linux, then clearly there is no risk. 
...
```json
{
    "body": "<a id='comment:7'></a>Just for the record: Besides other things, I'd like to have something like `$SAGE_SPKG_KEEP_SRC` (or `SAGE_KEEP_BUILT_SPKGS`) for developers, because `-s` is not available for whole builds (with `make`).",
...
<a id='comment:7'></a>Just for the record: Besides other things, I'd like to have something like `$SAGE_SPKG_KEEP_SRC` (or `SAGE_KEEP_BUILT_SPKGS`) for developers, because `-s` is not available for whole builds (with `make`).
...
<a id='comment:8'></a>Replying to [jhpalmieri](#comment%3A4):
> mpatel: I assume this is ready for review?
...
```json
{
    "body": "<a id='comment:9'></a>No, everything has worked fine.",
...
<a id='comment:9'></a>No, everything has worked fine.
...
```json
{
    "body": "<a id='comment:11'></a>Why is the milestone for this set to 5.0? Do we really need to wait that long to fix this bug, given it already has positive review and is one of the simplest possible bug fixes. \n\nDave",
...
<a id='comment:11'></a>Why is the milestone for this set to 5.0? Do we really need to wait that long to fix this bug, given it already has positive review and is one of the simplest possible bug fixes. 
