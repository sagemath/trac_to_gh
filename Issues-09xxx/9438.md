# Issue 9438: IntegerMod_int.log has side-effect in Pari

archive/issues_009438.json:
```json
{
    "body": "Motivated by a bug hunt for #2420, I found:\n\n```\nsage: R.<a, b> = QQ[]\nsage: b._pari_()\nb\nsage: GF(7)(5).log()\n5\nsage: b._pari_()\nMod(3, 7)\n```\n\nThe reason is that in the `log` method, the string\n\n```\n'b=Mod(%s,%s); if(znorder(b)!=eulerphi(%s),-1,znlog(%s,b))'%(b, self.__modulus.sageInteger,\n                                                                           self.__modulus.sageInteger, self)\n```\nis evaluated in `pari`, so that afterwards `pari('b')` isn't doing what it should.\n\nSince this bug is triggered whenever a GAP representation of a finite field element is created, I mark this ticket \"critical\". I hope \"basic arithmetic\" is the right component.\n\nAssignee: @aghitza\n\nKeywords: log finite field pari\n\nResolution: fixed\n\nAuthor: Simon King\n\nReviewer: David Loeffler\n\nMerged: sage-4.5.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/9438\n\n",
    "closed_at": "2010-07-20T09:17:28Z",
    "created_at": "2010-07-06T15:34:02Z",
    "labels": [
        "component: basic arithmetic",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.5.2",
    "title": "IntegerMod_int.log has side-effect in Pari",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9438",
    "user": "https://github.com/simon-king-jena"
}
```
Motivated by a bug hunt for #2420, I found:

```
sage: R.<a, b> = QQ[]
sage: b._pari_()
b
sage: GF(7)(5).log()
5
sage: b._pari_()
Mod(3, 7)
```

The reason is that in the `log` method, the string

```
'b=Mod(%s,%s); if(znorder(b)!=eulerphi(%s),-1,znlog(%s,b))'%(b, self.__modulus.sageInteger,
                                                                           self.__modulus.sageInteger, self)
```
is evaluated in `pari`, so that afterwards `pari('b')` isn't doing what it should.

Since this bug is triggered whenever a GAP representation of a finite field element is created, I mark this ticket "critical". I hope "basic arithmetic" is the right component.

Assignee: @aghitza

Keywords: log finite field pari

Resolution: fixed

Author: Simon King

Reviewer: David Loeffler

Merged: sage-4.5.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/9438





---

archive/issue_comments_107819.json:
```json
{
    "body": "<a id='comment:1'></a>The patch seems to fix the bug. The string that is evaluated by PARI defines a variable 'b' and uses it exactly twice. The definition of 'b' is not so long: It is `b=Mod(m,n)`. OK, m and n can be large numbers. But still, I think the cleanest solution is to expand the definition and insert `Mod(m,n)` twice in the string.\n\nsage -t \"devel/sage/sage/rings/finite_rings/integer_mod.pyx\" passes, I now do sage -testall\n\nOf course, I added a doctest against the bug.",
    "created_at": "2010-07-06T17:00:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107819",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:1'></a>The patch seems to fix the bug. The string that is evaluated by PARI defines a variable 'b' and uses it exactly twice. The definition of 'b' is not so long: It is `b=Mod(m,n)`. OK, m and n can be large numbers. But still, I think the cleanest solution is to expand the definition and insert `Mod(m,n)` twice in the string.

sage -t "devel/sage/sage/rings/finite_rings/integer_mod.pyx" passes, I now do sage -testall

Of course, I added a doctest against the bug.



---

archive/issue_comments_107820.json:
```json
{
    "body": "Changing author from \"\" to \"Simon King\"",
    "created_at": "2010-07-06T17:00:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107820",
    "user": "https://github.com/simon-king-jena"
}
```

Changing author from "" to "Simon King"



---

archive/issue_comments_107821.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-07-06T17:00:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107821",
    "user": "https://github.com/simon-king-jena"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_107822.json:
```json
{
    "body": "<a id='comment:2'></a>`sage -testall` passes!",
    "created_at": "2010-07-06T18:35:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107822",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:2'></a>`sage -testall` passes!



---

archive/issue_comments_107823.json:
```json
{
    "body": "<a id='comment:3'></a>This modifies exactly the same code as #9205 (which already has a positive review). Sadly I didn't spot this problem when I wrote that patch. Can you check to see if your patch applies OK on top of #9205, and if not, rebase it?\n\nDavid",
    "created_at": "2010-07-06T19:36:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107823",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:3'></a>This modifies exactly the same code as #9205 (which already has a positive review). Sadly I didn't spot this problem when I wrote that patch. Can you check to see if your patch applies OK on top of #9205, and if not, rebase it?

David



---

archive/issue_comments_107824.json:
```json
{
    "body": "Attachment [trac_9438_IntegerMod_log_vs_PARI.patch](tarball://root/attachments/some-uuid/ticket9438/trac_9438_IntegerMod_log_vs_PARI.patch) by @simon-king-jena created at 2010-07-07 11:48:43\n\nFixes a side effect of log on PARI",
    "created_at": "2010-07-07T11:48:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107824",
    "user": "https://github.com/simon-king-jena"
}
```

Attachment [trac_9438_IntegerMod_log_vs_PARI.patch](tarball://root/attachments/some-uuid/ticket9438/trac_9438_IntegerMod_log_vs_PARI.patch) by @simon-king-jena created at 2010-07-07 11:48:43

Fixes a side effect of log on PARI



---

archive/issue_comments_107825.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [comment:3 davidloeffler]:\n> This modifies exactly the same code as #9205 (which already has a positive review). Sadly I \n> didn't spot this problem when I wrote that patch. Can you check to see if your patch applies OK \n> on top of #9205, and if not, rebase it?\n\n\nIt did not apply on top of #9205. So, I replaced the original patch by one that is to be applied after the two patches from #9205.\n\nCheers,\n\nSimon",
    "created_at": "2010-07-07T11:50:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107825",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:4'></a>Replying to [comment:3 davidloeffler]:
> This modifies exactly the same code as #9205 (which already has a positive review). Sadly I 
> didn't spot this problem when I wrote that patch. Can you check to see if your patch applies OK 
> on top of #9205, and if not, rebase it?


It did not apply on top of #9205. So, I replaced the original patch by one that is to be applied after the two patches from #9205.

Cheers,

Simon



---

archive/issue_comments_107826.json:
```json
{
    "body": "Changing reviewer from \"\" to \"David Loeffler\"",
    "created_at": "2010-07-07T15:10:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107826",
    "user": "https://github.com/loefflerd"
}
```

Changing reviewer from "" to "David Loeffler"



---

archive/issue_comments_107827.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2010-07-07T15:10:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107827",
    "user": "https://github.com/loefflerd"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_107828.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-07-20T09:17:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107828",
    "user": "https://github.com/qed777"
}
```

Resolution: fixed



---

archive/issue_events_023330.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-20T09:17:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9438#event-23330"
}
```



---

archive/issue_comments_107829.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.5.2.alpha0\"",
    "created_at": "2010-07-20T09:17:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9438#issuecomment-107829",
    "user": "https://github.com/qed777"
}
```

Changing merged from "" to "sage-4.5.2.alpha0"
