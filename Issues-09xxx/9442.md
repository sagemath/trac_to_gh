# Issue 9442: reference manual issues for 4.5.alpha4

archive/issues_009442.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "The attached patch fixes the two warnings when building the ref manual in Sage 4.5.alpha4:\n\n```\n/Applications/sage_builds/sage-4.5.alpha4/devel/sage/doc/en/reference/graphs.rst:4: (WARNING/2) toctree references unknown document u'sage/graphs/pq_trees.py'\n/Applications/sage_builds/sage-4.5.alpha4/local/lib/python2.6/site-packages/sage/schemes/elliptic_curves/sha_tate.py:docstring of sage.schemes.elliptic_curves.sha_tate.Sha.bound_kato:12: (WARNING/2) Definition list ends without a blank line; unexpected unindent.\n```\n\n\nComponent: **documentation**\n\nAuthor: **John Palmieri**\n\nReviewer: **Mitesh Patel**\n\nMerged: **sage-4.5.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9442_\n\n",
    "closed_at": "2010-07-08T19:09:31Z",
    "created_at": "2010-07-06T22:41:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "reference manual issues for 4.5.alpha4",
    "type": "issue",
    "updated_at": "2010-07-08T19:09:31Z",
    "url": "https://github.com/sagemath/sage/issues/9442",
    "user": "https://github.com/jhpalmieri"
}
```
The attached patch fixes the two warnings when building the ref manual in Sage 4.5.alpha4:

```
/Applications/sage_builds/sage-4.5.alpha4/devel/sage/doc/en/reference/graphs.rst:4: (WARNING/2) toctree references unknown document u'sage/graphs/pq_trees.py'
/Applications/sage_builds/sage-4.5.alpha4/local/lib/python2.6/site-packages/sage/schemes/elliptic_curves/sha_tate.py:docstring of sage.schemes.elliptic_curves.sha_tate.Sha.bound_kato:12: (WARNING/2) Definition list ends without a blank line; unexpected unindent.
```


Component: **documentation**

Author: **John Palmieri**

Reviewer: **Mitesh Patel**

Merged: **sage-4.5.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/9442_





---

archive/issue_events_121328.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-07-06T22:41:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "milestone_number": null,
    "milestone_title": "sage-4.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121328"
}
```



---

archive/issue_events_121329.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-07-06T22:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121329"
}
```



---

archive/issue_events_121330.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-07-06T22:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121330"
}
```



---

archive/issue_events_121331.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-07-06T22:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121331"
}
```



---

archive/issue_events_121332.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-07-06T22:41:15Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "subject": "https://github.com/jhpalmieri",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121332"
}
```



---

archive/issue_comments_081299.json:
```json
{
    "body": "Attachment: **[trac_9442-ref-manual.patch.gz](https://github.com/sagemath/sage/files/ticket9442/trac_9442-ref-manual.patch.gz)**",
    "created_at": "2010-07-06T22:43:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9442#issuecomment-81299",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[trac_9442-ref-manual.patch.gz](https://github.com/sagemath/sage/files/ticket9442/trac_9442-ref-manual.patch.gz)**



---

archive/issue_comments_081300.json:
```json
{
    "body": "Reviewer: **Mitesh Patel**",
    "created_at": "2010-07-07T02:39:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9442#issuecomment-81300",
    "user": "https://github.com/qed777"
}
```

Reviewer: **Mitesh Patel**



---

archive/issue_events_121333.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-07T02:39:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121333"
}
```



---

archive/issue_comments_081301.json:
```json
{
    "body": "Author: **John Palmieri**",
    "created_at": "2010-07-07T02:39:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9442#issuecomment-81301",
    "user": "https://github.com/qed777"
}
```

Author: **John Palmieri**



---

archive/issue_events_121334.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-07T02:39:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121334"
}
```



---

archive/issue_events_121335.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-07T02:39:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121335"
}
```



---

archive/issue_comments_081302.json:
```json
{
    "body": "Merged: **sage-4.5.rc0**",
    "created_at": "2010-07-08T19:09:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9442#issuecomment-81302",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.5.rc0**



---

archive/issue_events_121336.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-07-08T19:09:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121336"
}
```



---

archive/issue_events_121337.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-07-08T19:09:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9442",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9442#event-121337"
}
```
