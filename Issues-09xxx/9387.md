...
```json
{
    "body": "<a id='comment:1'></a>Added patch with the method tamagawa_numbers(), essentially a duplication of the code for the rational case.",
...
<a id='comment:1'></a>Added patch with the method tamagawa_numbers(), essentially a duplication of the code for the rational case.
...
```json
{
    "body": "<a id='comment:2'></a>Updated the patch with a corrected doctest (run and passed).",
...
<a id='comment:2'></a>Updated the patch with a corrected doctest (run and passed).
...
```json
{
    "body": "<a id='comment:4'></a>That's a good idea.  Justin, all that's needed is (a) delete the version in ell_rational_field, (b) make sure that the code in ell_number_field works over Q (say by moving the old doctest into the new function -- it should have examples over Q and over another field.\n\nThere might be other functions like this.  If you notice any, make a ticket!",
...
<a id='comment:4'></a>That's a good idea.  Justin, all that's needed is (a) delete the version in ell_rational_field, (b) make sure that the code in ell_number_field works over Q (say by moving the old doctest into the new function -- it should have examples over Q and over another field.
...
```json
{
    "body": "<a id='comment:7'></a>OK, that's weird.  Turns out I popped when I should have pushed, so I was testing unmodified code.  I'll be back.",
...
<a id='comment:7'></a>OK, that's weird.  Turns out I popped when I should have pushed, so I was testing unmodified code.  I'll be back.
...
```json
{
    "body": "<a id='comment:8'></a>New patch, replacing previous one.  This time, with some luck, I verified the patch against both 4.4.4 and 4.5.a1.\n\nComments, brickbats, scotch all welcome.",
...
<a id='comment:8'></a>New patch, replacing previous one.  This time, with some luck, I verified the patch against both 4.4.4 and 4.5.a1.
