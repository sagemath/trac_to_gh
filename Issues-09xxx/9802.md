# Issue 9802: Add random diagonalizable matrix to matrix/constructor.py

archive/issues_009801.json:
```json
{
    "body": "This depends on #9720, so first apply the v3 patch from there.  This method generates random diagonalizable matrices whose eigenvectors, if computed by hand, have only integer values. This routine is designed as educational tool, generating exam and homework problems, and problem generating interacts.\n\n**Assignee:** @jasongrout, @williamstein\n\n**CC:**  @rbeezer @wdjoyner\n\n**Resolution:** fixed\n\n**Author:** Billy Wonderly\n\n**Reviewer:** David Joyner\n\n**Merged:** sage-4.6.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/9802\n\n",
    "closed_at": "2010-09-15T09:53:42Z",
    "created_at": "2010-08-25T18:45:05Z",
    "labels": [
        "component: linear algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.6",
    "title": "Add random diagonalizable matrix to matrix/constructor.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9802",
    "user": "https://trac.sagemath.org/admin/accounts/users/bwonderly"
}
```
This depends on #9720, so first apply the v3 patch from there.  This method generates random diagonalizable matrices whose eigenvectors, if computed by hand, have only integer values. This routine is designed as educational tool, generating exam and homework problems, and problem generating interacts.

**Assignee:** @jasongrout, @williamstein

**CC:**  @rbeezer @wdjoyner

**Resolution:** fixed

**Author:** Billy Wonderly

**Reviewer:** David Joyner

**Merged:** sage-4.6.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/9802





---

archive/attachments_012647.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_9802-random-diagonalizable-matrix.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket9802/trac_9802-random-diagonalizable-matrix.patch",
    "created_at": "2010-08-25T18:47:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/bwonderly"
}
```



---

archive/issue_comments_121492.json:
```json
{
    "body": "<a id='comment:1'></a>\n",
    "created_at": "2010-08-25T18:47:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121492",
    "user": "https://trac.sagemath.org/admin/accounts/users/bwonderly"
}
```

<a id='comment:1'></a>




---

archive/issue_comments_121493.json:
```json
{
    "body": "**Changing status** from new to needs_review.",
    "created_at": "2010-08-25T18:47:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121493",
    "user": "https://trac.sagemath.org/admin/accounts/users/bwonderly"
}
```

**Changing status** from new to needs_review.



---

archive/attachments_012648.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_9802-random-diagonalizable-matrix-v2.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket9802/trac_9802-random-diagonalizable-matrix-v2.patch",
    "created_at": "2010-08-28T02:39:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/bwonderly"
}
```



---

archive/issue_comments_121494.json:
```json
{
    "body": "revised to fit generalization of random_matrix constructor. Apply v4 from #9720 and v2 from #9803 and go from there. This patch is independent from #9754, but will be rebased as soon as either one gets a positive review",
    "created_at": "2010-08-28T02:39:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121494",
    "user": "https://trac.sagemath.org/admin/accounts/users/bwonderly"
}
```

revised to fit generalization of random_matrix constructor. Apply v4 from #9720 and v2 from #9803 and go from there. This patch is independent from #9754, but will be rebased as soon as either one gets a positive review



---

archive/issue_comments_121495.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe v2 patch is independent of #9754. There are revisions to the documentation of the random_diagonalizable_matrix routine, as well as to the random_matrix routine.  The code is revised to fit with the generalization of the random_matrix constructor.  First apply v4 from #9720, and then v2 from #9803 and go from there.",
    "created_at": "2010-08-28T17:39:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121495",
    "user": "https://trac.sagemath.org/admin/accounts/users/bwonderly"
}
```

<a id='comment:2'></a>
The v2 patch is independent of #9754. There are revisions to the documentation of the random_diagonalizable_matrix routine, as well as to the random_matrix routine.  The code is revised to fit with the generalization of the random_matrix constructor.  First apply v4 from #9720, and then v2 from #9803 and go from there.



---

archive/issue_comments_121496.json:
```json
{
    "body": "<a id='comment:3'></a>\nThis (with the other patches, as indicated above) applied fine to 4.5.1 and passed sage -testall.\n\nPositive review, as far as I am concerned (and will be useful for me teaching linear algebra later in the semester!). Perhaps Mike Hansen should have the final say?\n\nThanks Rob and bwonderly!",
    "created_at": "2010-08-28T21:58:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121496",
    "user": "https://github.com/wdjoyner"
}
```

<a id='comment:3'></a>
This (with the other patches, as indicated above) applied fine to 4.5.1 and passed sage -testall.

Positive review, as far as I am concerned (and will be useful for me teaching linear algebra later in the semester!). Perhaps Mike Hansen should have the final say?

Thanks Rob and bwonderly!



---

archive/issue_comments_121497.json:
```json
{
    "body": "**Reviewer:** David Joyner",
    "created_at": "2010-09-02T02:20:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121497",
    "user": "https://github.com/rbeezer"
}
```

**Reviewer:** David Joyner



---

archive/issue_comments_121498.json:
```json
{
    "body": "**Changing status** from needs_review to positive_review.",
    "created_at": "2010-09-02T02:20:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121498",
    "user": "https://github.com/rbeezer"
}
```

**Changing status** from needs_review to positive_review.



---

archive/issue_comments_121499.json:
```json
{
    "body": "<a id='comment:4'></a>\nMike Hansen looked in on #9803, so I've marked this as ready to go based on comments above.  Thanks, David.",
    "created_at": "2010-09-02T02:20:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121499",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:4'></a>
Mike Hansen looked in on #9803, so I've marked this as ready to go based on comments above.  Thanks, David.



---

archive/issue_comments_121500.json:
```json
{
    "body": "<a id='comment:5'></a>\n## Release Manager\n\n#9720, #9803, #9802, #9754 is each dependent on the predecessor, merge in this\norder.",
    "created_at": "2010-09-03T06:28:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121500",
    "user": "https://trac.sagemath.org/admin/accounts/users/bwonderly"
}
```

<a id='comment:5'></a>
## Release Manager

#9720, #9803, #9802, #9754 is each dependent on the predecessor, merge in this
order.



---

archive/issue_comments_121501.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2010-09-15T09:53:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121501",
    "user": "https://github.com/qed777"
}
```

**Resolution:** fixed



---

archive/issue_events_029825.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-09-15T09:53:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9802#event-29825"
}
```



---

archive/issue_comments_121502.json:
```json
{
    "body": "**Merged:** sage-4.6.alpha1",
    "created_at": "2010-09-15T09:53:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9802#issuecomment-121502",
    "user": "https://github.com/qed777"
}
```

**Merged:** sage-4.6.alpha1
