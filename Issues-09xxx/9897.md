# Issue 9897: pari-2.4.3.svn-12577 fails to build on itanium with gcc 4.5.1

archive/issues_009896.json:
```json
{
    "body": "On the skynet machines cleo (ia64-Linux-rhel) and iras (ia64-Linux-suse), the PARI spkg in 4.6.alpha0 triggers a **bug in gcc 4.5.1**:\n\n```\ngcc  -c -O3 -Wall -fno-strict-aliasing -fomit-frame-pointer  -O3 -g   -I. -I../src/headers -fPIC -o mpqs.o ../src/modules/mpqs.c\n../src/modules/krasner.c: In function 'GetRamifiedPol': \n../src/modules/krasner.c:878:1: error: unrecognizable insn: \n(insn:TI 7910 7861 7937 509 (parallel [ \n            (set (reg:DI 134 f6) \n                (asm_operands:DI (\"xma.hu %0 = %2, %3, f0 \n        ;; \n        xma.l %1 = %2, %3, f0\") (\"=&f\") 0 [ \n                        (reg:DI 135 f7) \n                        (reg/v:DI 130 f2 [orig:1756 pmodg ] [1756]) \n                    ] \n                     [ \n                        (asm_input:DI (\"f\") (null):0) \n                        (asm_input:DI (\"f\") (null):0) \n                    ] \n                     [] ../src/modules/krasner.c:878)) \n            (set (reg:DI 135 f7) \n                (asm_operands:DI (\"xma.hu %0 = %2, %3, f0 \n        ;; \n        xma.l %1 = %2, %3, f0\") (\"=f\") 1 [ \n                        (reg:DI 135 f7) \n                        (reg/v:DI 130 f2 [orig:1756 pmodg ] [1756]) \n                    ] \n                     [ \n                        (asm_input:DI (\"f\") (null):0) \n                        (asm_input:DI (\"f\") (null):0) \n                    ] \n                     [] ../src/modules/krasner.c:878)) \n        ]) -1 (nil)) \n../src/modules/krasner.c:878:1: internal compiler error: in \nget_attr_first_insn, at config/ia64/itanium2.md:1909 \nPlease submit a full bug report, \nwith preprocessed source if appropriate. \nSee <http://gcc.gnu.org/bugs.html> for instructions. \nmake[3]: *** [krasner.o] Error 1\n```\n\nReported upstream: [http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044)\n\nFixed by #10572.\n\n**CC:**  @nexttime @jdemeyer\n\n**Upstream:** Reported upstream. Little or no feedback.\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/9897\n\n",
    "closed_at": "2010-12-10T14:15:12Z",
    "created_at": "2010-09-11T06:08:39Z",
    "labels": [
        "component: packages: standard",
        "critical",
        "bug",
        "duplicate"
    ],
    "title": "pari-2.4.3.svn-12577 fails to build on itanium with gcc 4.5.1",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9897",
    "user": "https://github.com/jhpalmieri"
}
```
On the skynet machines cleo (ia64-Linux-rhel) and iras (ia64-Linux-suse), the PARI spkg in 4.6.alpha0 triggers a **bug in gcc 4.5.1**:

```
gcc  -c -O3 -Wall -fno-strict-aliasing -fomit-frame-pointer  -O3 -g   -I. -I../src/headers -fPIC -o mpqs.o ../src/modules/mpqs.c
../src/modules/krasner.c: In function 'GetRamifiedPol': 
../src/modules/krasner.c:878:1: error: unrecognizable insn: 
(insn:TI 7910 7861 7937 509 (parallel [ 
            (set (reg:DI 134 f6) 
                (asm_operands:DI ("xma.hu %0 = %2, %3, f0 
        ;; 
        xma.l %1 = %2, %3, f0") ("=&f") 0 [ 
                        (reg:DI 135 f7) 
                        (reg/v:DI 130 f2 [orig:1756 pmodg ] [1756]) 
                    ] 
                     [ 
                        (asm_input:DI ("f") (null):0) 
                        (asm_input:DI ("f") (null):0) 
                    ] 
                     [] ../src/modules/krasner.c:878)) 
            (set (reg:DI 135 f7) 
                (asm_operands:DI ("xma.hu %0 = %2, %3, f0 
        ;; 
        xma.l %1 = %2, %3, f0") ("=f") 1 [ 
                        (reg:DI 135 f7) 
                        (reg/v:DI 130 f2 [orig:1756 pmodg ] [1756]) 
                    ] 
                     [ 
                        (asm_input:DI ("f") (null):0) 
                        (asm_input:DI ("f") (null):0) 
                    ] 
                     [] ../src/modules/krasner.c:878)) 
        ]) -1 (nil)) 
../src/modules/krasner.c:878:1: internal compiler error: in 
get_attr_first_insn, at config/ia64/itanium2.md:1909 
Please submit a full bug report, 
with preprocessed source if appropriate. 
See <http://gcc.gnu.org/bugs.html> for instructions. 
make[3]: *** [krasner.o] Error 1
```

Reported upstream: [http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044)

Fixed by #10572.

**CC:**  @nexttime @jdemeyer

**Upstream:** Reported upstream. Little or no feedback.

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/9897





---

archive/issue_comments_091149.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-On the skynet machines cleo (ia64-Linux-rhel) and iras (ia64-Linux-suse), each using gcc 4.5.1, the PARI spkg in 4.6.alpha0 fails to build:\n+On the skynet machines cleo (ia64-Linux-rhel) and iras (ia64-Linux-suse), the PARI spkg in 4.6.alpha0 triggers a **bug in gcc 4.5.1**:\n \n ```\n gcc  -c -O3 -Wall -fno-strict-aliasing -fomit-frame-pointer  -O3 -g   -I. -I../src/headers -fPIC -o mpqs.o ../src/modules/mpqs.c\n``````\n",
    "created_at": "2010-09-19T08:32:48Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91149",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-On the skynet machines cleo (ia64-Linux-rhel) and iras (ia64-Linux-suse), each using gcc 4.5.1, the PARI spkg in 4.6.alpha0 fails to build:
+On the skynet machines cleo (ia64-Linux-rhel) and iras (ia64-Linux-suse), the PARI spkg in 4.6.alpha0 triggers a **bug in gcc 4.5.1**:
 
 ```
 gcc  -c -O3 -Wall -fno-strict-aliasing -fomit-frame-pointer  -O3 -g   -I. -I../src/headers -fPIC -o mpqs.o ../src/modules/mpqs.c
``````




---

archive/issue_comments_091150.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is clearly a gcc bug.",
    "created_at": "2010-09-19T08:32:48Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91150",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
This is clearly a gcc bug.



---

archive/issue_comments_091151.json:
```json
{
    "body": "<a id='comment:3'></a>\nWith cleo's default GCC 4.1.2 20080704 (Red Hat 4.1.2-44), which I \"enabled\" by not sourcing `/usr/local/skynet_bash_profile` in `~/.bash_profile`, a standalone build succeeds but some tests fail:\n\n```\n$ tar xjf pari-2.4.3.svn-12577.p5.spkg\n$ cd pari-2.4.3.svn-12577.p5/src\n$ ./Configure --graphic=none\n$ make test-all\n[...]\n* Testing zn    for gp-sta..TIME=7      for gp-dyn..TIME=3\n+++ [BUG] Total bench for gp-sta is 496414\n+++ [BUG] Total bench for gp-dyn is 554345\n\nPROBLEMS WERE NOTED. The following files list them in diff format:\nDirectory: /home/mpatel/cleo/pari-2.4.3.svn-12577.p5/src/Olinux-ia64\n        ellglobalred-sta.dif\n        ellsea-sta.dif\n        galois-sta.dif\n        ellglobalred-dyn.dif\n        ellsea-dyn.dif\n        galois-dyn.dif\nmake[1]: *** [test-all] Error 1\nmake[1]: Leaving directory `/home/mpatel/cleo/pari-2.4.3.svn-12577.p5/src/Olinux-ia64'\nmake: *** [test-all] Error 2\n```\nI've put copies of these .dif files [here](http://sage.math.washington.edu/home/mpatel/trac/9897/).  But I get the same failures on sage.math.",
    "created_at": "2010-09-25T10:46:01Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91151",
    "user": "https://github.com/qed777"
}
```

<a id='comment:3'></a>
With cleo's default GCC 4.1.2 20080704 (Red Hat 4.1.2-44), which I "enabled" by not sourcing `/usr/local/skynet_bash_profile` in `~/.bash_profile`, a standalone build succeeds but some tests fail:

```
$ tar xjf pari-2.4.3.svn-12577.p5.spkg
$ cd pari-2.4.3.svn-12577.p5/src
$ ./Configure --graphic=none
$ make test-all
[...]
* Testing zn    for gp-sta..TIME=7      for gp-dyn..TIME=3
+++ [BUG] Total bench for gp-sta is 496414
+++ [BUG] Total bench for gp-dyn is 554345

PROBLEMS WERE NOTED. The following files list them in diff format:
Directory: /home/mpatel/cleo/pari-2.4.3.svn-12577.p5/src/Olinux-ia64
        ellglobalred-sta.dif
        ellsea-sta.dif
        galois-sta.dif
        ellglobalred-dyn.dif
        ellsea-dyn.dif
        galois-dyn.dif
make[1]: *** [test-all] Error 1
make[1]: Leaving directory `/home/mpatel/cleo/pari-2.4.3.svn-12577.p5/src/Olinux-ia64'
make: *** [test-all] Error 2
```
I've put copies of these .dif files [here](http://sage.math.washington.edu/home/mpatel/trac/9897/).  But I get the same failures on sage.math.



---

archive/issue_comments_091152.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [mpatel](#comment%3A3):\n> With cleo's default GCC 4.1.2 20080704 (Red Hat 4.1.2-44), which I \"enabled\" by not sourcing `/usr/local/skynet_bash_profile` in `~/.bash_profile`, a standalone build succeeds but some tests fail:\n> \n\n```\n...\nPROBLEMS WERE NOTED. The following files list them in diff format:\nDirectory: /home/mpatel/cleo/pari-2.4.3.svn-12577.p5/src/Olinux-ia64\n        ellglobalred-sta.dif\n        ellsea-sta.dif\n        galois-sta.dif\n        ellglobalred-dyn.dif\n        ellsea-dyn.dif\n        galois-dyn.dif\n...\n```\n> I've put copies of these .dif files [here](http://sage.math.washington.edu/home/mpatel/trac/9897/).  But I get the same failures on sage.math.\n\nThese 2*3 tests *must* fail, because they depend on data files which aren't (yet) present / installed at that point. (We don't ship the ellglobalred data at all since it's 14 MB, and therefore patch `config/get_tests`.)",
    "created_at": "2010-09-25T13:09:43Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91152",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'></a>
Replying to [mpatel](#comment%3A3):
> With cleo's default GCC 4.1.2 20080704 (Red Hat 4.1.2-44), which I "enabled" by not sourcing `/usr/local/skynet_bash_profile` in `~/.bash_profile`, a standalone build succeeds but some tests fail:
> 

```
...
PROBLEMS WERE NOTED. The following files list them in diff format:
Directory: /home/mpatel/cleo/pari-2.4.3.svn-12577.p5/src/Olinux-ia64
        ellglobalred-sta.dif
        ellsea-sta.dif
        galois-sta.dif
        ellglobalred-dyn.dif
        ellsea-dyn.dif
        galois-dyn.dif
...
```
> I've put copies of these .dif files [here](http://sage.math.washington.edu/home/mpatel/trac/9897/).  But I get the same failures on sage.math.

These 2*3 tests *must* fail, because they depend on data files which aren't (yet) present / installed at that point. (We don't ship the ellglobalred data at all since it's 14 MB, and therefore patch `config/get_tests`.)



---

archive/issue_comments_091153.json:
```json
{
    "body": "<a id='comment:5'></a>\n(You could temporarily patch `src/config/get_tests` with `patches/get_tests.patch`, run `src/Configure` with also e.g. `--prefix=/tmp/pari` and `make install-data` before running `make test-all`. I'm not sure if you also have to `make gp` and `make install` before running `make install-data`.)",
    "created_at": "2010-09-25T13:32:26Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91153",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:5'></a>
(You could temporarily patch `src/config/get_tests` with `patches/get_tests.patch`, run `src/Configure` with also e.g. `--prefix=/tmp/pari` and `make install-data` before running `make test-all`. I'm not sure if you also have to `make gp` and `make install` before running `make install-data`.)



---

archive/issue_comments_091154.json:
```json
{
    "body": "<a id='comment:6'></a>\nReplying to [leif](#comment%3A5):\n> I'm not sure if you also have to `make gp` and `make install` before running `make install-data`.\n\nYou don't need to do that, i.e you can simply run `make install-data && make test-all` after copying `patches/files/get_tests` to `src/config` and configuring with some temporary installation directory.",
    "created_at": "2010-09-25T13:54:03Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91154",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:6'></a>
Replying to [leif](#comment%3A5):
> I'm not sure if you also have to `make gp` and `make install` before running `make install-data`.

You don't need to do that, i.e you can simply run `make install-data && make test-all` after copying `patches/files/get_tests` to `src/config` and configuring with some temporary installation directory.



---

archive/issue_comments_091155.json:
```json
{
    "body": "<a id='comment:7'></a>\nAre there any plans to report this upstream to gcc?",
    "created_at": "2010-10-02T08:26:32Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91155",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>
Are there any plans to report this upstream to gcc?



---

archive/issue_comments_091156.json:
```json
{
    "body": "<a id='comment:8'></a>\nThe problem above seems very similar to [this one](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=31684).",
    "created_at": "2010-10-02T08:35:00Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91156",
    "user": "https://github.com/qed777"
}
```

<a id='comment:8'></a>
The problem above seems very similar to [this one](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=31684).



---

archive/issue_comments_091157.json:
```json
{
    "body": "<a id='comment:9'></a>\nReplying to [mpatel](#comment%3A8):\n> The problem above seems very similar to [this one](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=31684).\n\nExcept that was marked as \"RESOLVED FIXED\" back in January 2008. Another bug report should be created. The gcc developers require the preprocessed file - see http://gcc.gnu.org/bugs/\n\nAs another matter, it would be good if the Pari developers could add a test target that tests everything except those parts of Pari needing the extra databases, which are not part of the main Pari distribution. Someone here probably knows the best person to ask. That is not a bug as such, but a request for an enhancement. If that was done, it would remove the need for a patch in Sage. \n\nDave",
    "created_at": "2010-10-02T10:30:39Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91157",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

<a id='comment:9'></a>
Replying to [mpatel](#comment%3A8):
> The problem above seems very similar to [this one](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=31684).

Except that was marked as "RESOLVED FIXED" back in January 2008. Another bug report should be created. The gcc developers require the preprocessed file - see http://gcc.gnu.org/bugs/

As another matter, it would be good if the Pari developers could add a test target that tests everything except those parts of Pari needing the extra databases, which are not part of the main Pari distribution. Someone here probably knows the best person to ask. That is not a bug as such, but a request for an enhancement. If that was done, it would remove the need for a patch in Sage. 

Dave



---

archive/issue_comments_091158.json:
```json
{
    "body": "<a id='comment:10'></a>\nReplying to [drkirkby](#comment%3A9):\n> As another matter, it would be good if the Pari developers could add a test target that tests everything except those parts of Pari needing the extra databases, which are not part of the main Pari distribution. Someone here probably knows the best person to ask. That is not a bug as such, but a request for an enhancement. If that was done, it would remove the need for a patch in Sage. \n\nI have posed the question at [http://pari.math.u-bordeaux.fr/archives/pari-dev-1010/msg00000.html](http://pari.math.u-bordeaux.fr/archives/pari-dev-1010/msg00000.html)",
    "created_at": "2010-10-02T17:07:16Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91158",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>
Replying to [drkirkby](#comment%3A9):
> As another matter, it would be good if the Pari developers could add a test target that tests everything except those parts of Pari needing the extra databases, which are not part of the main Pari distribution. Someone here probably knows the best person to ask. That is not a bug as such, but a request for an enhancement. If that was done, it would remove the need for a patch in Sage. 

I have posed the question at [http://pari.math.u-bordeaux.fr/archives/pari-dev-1010/msg00000.html](http://pari.math.u-bordeaux.fr/archives/pari-dev-1010/msg00000.html)



---

archive/issue_comments_091159.json:
```json
{
    "body": "<a id='comment:11'></a>\nSage 4.6.alpha3 [compiles on cleo with the default compiler](http://build.sagemath.org/sage/builders/cleo%20full/builds/11) (gcc version 4.1.2 20080704 (Red Hat 4.1.2-44) and passes the tests (except for known blockers).\n\nWith the default on iras (gcc version 4.1.2 20070115 (prerelease) (SUSE Linux)), I had problems installing Python (see #10082), IPython, PolyBoRi, and R.  I don't know which of these, if any, stems from the build configuration.",
    "created_at": "2010-10-09T05:28:29Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91159",
    "user": "https://github.com/qed777"
}
```

<a id='comment:11'></a>
Sage 4.6.alpha3 [compiles on cleo with the default compiler](http://build.sagemath.org/sage/builders/cleo%20full/builds/11) (gcc version 4.1.2 20080704 (Red Hat 4.1.2-44) and passes the tests (except for known blockers).

With the default on iras (gcc version 4.1.2 20070115 (prerelease) (SUSE Linux)), I had problems installing Python (see #10082), IPython, PolyBoRi, and R.  I don't know which of these, if any, stems from the build configuration.



---

archive/issue_events_074174.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-10-09T05:30:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "label": "blocker",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9897#event-74174"
}
```



---

archive/issue_events_074175.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-10-09T05:30:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "label": "critical",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9897#event-74175"
}
```



---

archive/issue_comments_091160.json:
```json
{
    "body": "<a id='comment:13'></a>\nCan we isolate a possible test case from `krasner.c`?",
    "created_at": "2010-10-09T05:35:32Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91160",
    "user": "https://github.com/qed777"
}
```

<a id='comment:13'></a>
Can we isolate a possible test case from `krasner.c`?



---

archive/issue_comments_091161.json:
```json
{
    "body": "<a id='comment:14'></a>\nReplying to [mpatel](#comment%3A13):\n> Can we isolate a possible test case from `krasner.c`?\n\nI'm willing to have a look at it if somebody can arrange access for me to a machine with this problem.",
    "created_at": "2010-10-09T07:51:34Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91161",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:14'></a>
Replying to [mpatel](#comment%3A13):
> Can we isolate a possible test case from `krasner.c`?

I'm willing to have a look at it if somebody can arrange access for me to a machine with this problem.



---

archive/issue_comments_091162.json:
```json
{
    "body": "<a id='comment:15'></a>\nReplying to [jdemeyer](#comment%3A14):\n> Replying to [mpatel](#comment%3A13):\n> > Can we isolate a possible test case from `krasner.c`?\n\n> \n> I'm willing to have a look at it if somebody can arrange access for me to a machine with this problem.\n\nI'm not sure about whether William is in charge of the Skynet cluster, but he can ask the Skynet administrator Mariah Lenox to give you an account.",
    "created_at": "2010-10-09T07:56:55Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91162",
    "user": "https://github.com/qed777"
}
```

<a id='comment:15'></a>
Replying to [jdemeyer](#comment%3A14):
> Replying to [mpatel](#comment%3A13):
> > Can we isolate a possible test case from `krasner.c`?

> 
> I'm willing to have a look at it if somebody can arrange access for me to a machine with this problem.

I'm not sure about whether William is in charge of the Skynet cluster, but he can ask the Skynet administrator Mariah Lenox to give you an account.



---

archive/issue_comments_091163.json:
```json
{
    "body": "<a id='comment:16'></a>\nConfirmed, even with vanilla PARI 2.4.3.\nCompiling with -O2 instead of -O3 works.",
    "created_at": "2010-10-15T08:51:58Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91163",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:16'></a>
Confirmed, even with vanilla PARI 2.4.3.
Compiling with -O2 instead of -O3 works.



---

archive/issue_comments_091164.json:
```json
{
    "body": "<a id='comment:17'></a>\nCould it be related to this: http://gcc.gnu.org/bugzilla/show_bug.cgi?id=43603",
    "created_at": "2010-10-15T09:10:38Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91164",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:17'></a>
Could it be related to this: http://gcc.gnu.org/bugzilla/show_bug.cgi?id=43603



---

archive/issue_comments_091165.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -38,3 +38,4 @@\n make[3]: *** [krasner.o] Error 1\n ```\n \n+Reported upstream: [http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044)\n``````\n",
    "created_at": "2010-10-16T12:37:49Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91165",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -38,3 +38,4 @@
 make[3]: *** [krasner.o] Error 1
 ```
 
+Reported upstream: [http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044)
``````




---

archive/issue_comments_091166.json:
```json
{
    "body": "**Upstream:** Reported upstream. Little or no feedback.",
    "created_at": "2010-10-16T12:37:49Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91166",
    "user": "https://github.com/jdemeyer"
}
```

**Upstream:** Reported upstream. Little or no feedback.



---

archive/issue_comments_091167.json:
```json
{
    "body": "Minimal testcase exhibiting the bug",
    "created_at": "2010-10-16T12:38:15Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91167",
    "user": "https://github.com/jdemeyer"
}
```

Minimal testcase exhibiting the bug



---

archive/attachments_013320.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "krasner.i",
    "asset_url": "tarball://root/attachments/ticket9897/krasner.i",
    "created_at": "2010-10-16T22:50:38Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket9897/krasner.i",
    "user": "https://github.com/qed777"
}
```



---

archive/issue_comments_091168.json:
```json
{
    "body": "<a id='comment:19'></a>\n**Attachment:** [krasner.i](https://github.com/sagemath/sage/files/ticket9897/krasner.i)\n\nCool.  Thanks!",
    "created_at": "2010-10-16T22:50:38Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91168",
    "user": "https://github.com/qed777"
}
```

<a id='comment:19'></a>
**Attachment:** [krasner.i](https://github.com/sagemath/sage/files/ticket9897/krasner.i)

Cool.  Thanks!



---

archive/issue_comments_091169.json:
```json
{
    "body": "<a id='comment:20'></a>\nReplying to [jdemeyer](#comment%3A16):\n> Confirmed, even with vanilla PARI 2.4.3.\n> Compiling with -O2 instead of -O3 works.\n\nI've successfully built 4.6.alpha3 on cleo and iras with `SAGE_DEBUG=yes`, which turns off `-O3` for at least PARI.",
    "created_at": "2010-10-17T05:50:11Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91169",
    "user": "https://github.com/qed777"
}
```

<a id='comment:20'></a>
Replying to [jdemeyer](#comment%3A16):
> Confirmed, even with vanilla PARI 2.4.3.
> Compiling with -O2 instead of -O3 works.

I've successfully built 4.6.alpha3 on cleo and iras with `SAGE_DEBUG=yes`, which turns off `-O3` for at least PARI.



---

archive/issue_comments_091170.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2010-12-10T14:15:12Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91170",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_074176.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-10T14:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9897#event-74176"
}
```



---

archive/issue_comments_091171.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -39,3 +39,5 @@\n ```\n \n Reported upstream: [http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044)\n+\n+Fixed by #10430.\n``````\n",
    "created_at": "2010-12-10T14:15:12Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91171",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -39,3 +39,5 @@
 ```
 
 Reported upstream: [http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044)
+
+Fixed by #10430.
``````




---

archive/issue_events_074177.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-10T14:15:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "milestone": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9897#event-74177"
}
```



---

archive/issue_events_074178.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-10T14:15:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9897#event-74178"
}
```



---

archive/issue_comments_091172.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -40,4 +40,4 @@\n \n Reported upstream: [http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044)\n \n-Fixed by #10430.\n+Fixed by #10572.\n``````\n",
    "created_at": "2011-01-19T22:31:30Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91172",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -40,4 +40,4 @@
 
 Reported upstream: [http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044](http://gcc.gnu.org/bugzilla/show_bug.cgi?id=46044)
 
-Fixed by #10430.
+Fixed by #10572.
``````




---

archive/issue_comments_091173.json:
```json
{
    "body": "<a id='comment:23'></a>\nIsn't that overkill to use (and wait for) #10572 rather than adding three lines in spkg-install to pass -O2 on ia64 and some GCC versions?\nI just encountered the same bug two years later with the default GCC 4.4.5 installed there and that Sage accepted.\n(This just made me think the problem on Cygwin with ECL and GCC 4.6.3 (but not 4.7.2) reported at http://gcc.gnu.org/bugzilla/show_bug.cgi?id=52061 which I encountered as well could be dealt with by just dropping the optimizaiton level.)",
    "created_at": "2013-03-01T10:09:27Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91173",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

<a id='comment:23'></a>
Isn't that overkill to use (and wait for) #10572 rather than adding three lines in spkg-install to pass -O2 on ia64 and some GCC versions?
I just encountered the same bug two years later with the default GCC 4.4.5 installed there and that Sage accepted.
(This just made me think the problem on Cygwin with ECL and GCC 4.6.3 (but not 4.7.2) reported at http://gcc.gnu.org/bugzilla/show_bug.cgi?id=52061 which I encountered as well could be dealt with by just dropping the optimizaiton level.)



---

archive/issue_comments_091174.json:
```json
{
    "body": "<a id='comment:24'></a>\nReplying to [jpflori](#comment%3A23):\n> I just encountered the same bug two years later with the default GCC 4.4.5 installed there and that Sage accepted.\n\nAre you really sure it's the same bug? What happens when you compile [attachment:krasner.i](https://github.com/sagemath/sage/files/ticket9897/krasner.i) with `-O3`?\n\nI would prefer to fix the `spkg/install` script which detects GCC versions and decides whether or not to build GCC.",
    "created_at": "2013-03-06T12:34:59Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91174",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:24'></a>
Replying to [jpflori](#comment%3A23):
> I just encountered the same bug two years later with the default GCC 4.4.5 installed there and that Sage accepted.

Are you really sure it's the same bug? What happens when you compile [attachment:krasner.i](https://github.com/sagemath/sage/files/ticket9897/krasner.i) with `-O3`?

I would prefer to fix the `spkg/install` script which detects GCC versions and decides whether or not to build GCC.



---

archive/issue_comments_091175.json:
```json
{
    "body": "<a id='comment:25'></a>\nReplying to [jdemeyer](#comment%3A24):\n> Replying to [jpflori](#comment%3A23):\n> > I just encountered the same bug two years later with the default GCC 4.4.5 installed there and that Sage accepted.\n\n> Are you really sure it's the same bug? What happens when you compile [attachment:krasner.i](https://github.com/sagemath/sage/files/ticket9897/krasner.i) with `-O3`?\n\nIt happens when compiling krasner.c so I think its the same bug, see https://github.com/sagemath/sage/issues/10508#comment:319.\n> \n> I would prefer to fix the `spkg/install` script which detects GCC versions and decides whether or not to build GCC.",
    "created_at": "2013-03-14T17:00:24Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91175",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

<a id='comment:25'></a>
Replying to [jdemeyer](#comment%3A24):
> Replying to [jpflori](#comment%3A23):
> > I just encountered the same bug two years later with the default GCC 4.4.5 installed there and that Sage accepted.

> Are you really sure it's the same bug? What happens when you compile [attachment:krasner.i](https://github.com/sagemath/sage/files/ticket9897/krasner.i) with `-O3`?

It happens when compiling krasner.c so I think its the same bug, see https://github.com/sagemath/sage/issues/10508#comment:319.
> 
> I would prefer to fix the `spkg/install` script which detects GCC versions and decides whether or not to build GCC.



---

archive/issue_comments_091176.json:
```json
{
    "body": "<a id='comment:26'></a>\nSee #14378.",
    "created_at": "2013-03-29T04:07:29Z",
    "issue": "https://github.com/sagemath/sage/issues/9897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9897#issuecomment-91176",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:26'></a>
See #14378.
