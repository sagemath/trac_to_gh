# Issue 9506: include libSingular error messages in exceptions

archive/issues_009506.json:
```json
{
    "assignees": [],
    "body": "Like this:\n\n\n\n```\nsage: P.<e,d,c,b,a> = PolynomialRing(QQ,5,order='lex')\nsage: I = sage.rings.ideal.Cyclic(P)\n\nsage: triangL = sage.libs.singular.ff.triang__lib.triangL\nsage: _ = triangL(I)\nTraceback (most recent call last):\n...\nRuntimeError: Error in Singular function call 'triangL':\n The input is no groebner basis.\n leaving triang.lib::triangL\n```\n\nAssignee: **@aghitza**\n\nAuthor: **Martin Albrecht**\n\nReviewer: **William Stein**\n\nMerged: **sage-4.5.3.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9506_\n\n",
    "closed_at": "2010-08-15T08:02:52Z",
    "created_at": "2010-07-15T12:49:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "include libSingular error messages in exceptions",
    "type": "issue",
    "updated_at": "2010-08-15T08:27:16Z",
    "url": "https://github.com/sagemath/sage/issues/9506",
    "user": "https://github.com/malb"
}
```
Like this:



```
sage: P.<e,d,c,b,a> = PolynomialRing(QQ,5,order='lex')
sage: I = sage.rings.ideal.Cyclic(P)

sage: triangL = sage.libs.singular.ff.triang__lib.triangL
sage: _ = triangL(I)
Traceback (most recent call last):
...
RuntimeError: Error in Singular function call 'triangL':
 The input is no groebner basis.
 leaving triang.lib::triangL
```

Assignee: **@aghitza**

Author: **Martin Albrecht**

Reviewer: **William Stein**

Merged: **sage-4.5.3.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/9506_





---

archive/issue_events_111528.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-07-15T12:49:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "milestone_number": null,
    "milestone_title": "sage-4.5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111528"
}
```



---

archive/issue_events_111529.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-07-15T12:49:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111529"
}
```



---

archive/issue_events_111530.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-07-15T12:49:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111530"
}
```



---

archive/issue_events_111531.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-07-15T12:49:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111531"
}
```



---

archive/issue_events_111532.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-07-15T12:50:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111532"
}
```



---

archive/issue_comments_084010.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe attached patch requires #4499 and a patch to Singular available at http://www.singular.uni-kl.de:8002/trac/ticket/244 which will be available in the next Singular release.",
    "created_at": "2010-07-15T12:50:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9506#issuecomment-84010",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'>Comment 1:</a>
The attached patch requires #4499 and a patch to Singular available at http://www.singular.uni-kl.de:8002/trac/ticket/244 which will be available in the next Singular release.



---

archive/issue_comments_084011.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nArgh, that's \u00a0#9499 and not#4499.",
    "created_at": "2010-07-15T12:51:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9506#issuecomment-84011",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'>Comment 2:</a>
Argh, that's  #9499 and not#4499.



---

archive/issue_comments_084012.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAttachment: **[trac_9506_error_msg.patch.gz](https://github.com/sagemath/sage/files/ticket9506/trac_9506_error_msg.patch.gz)**\n\nThis patch depends on\u00a0#8059.",
    "created_at": "2010-07-15T17:20:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9506#issuecomment-84012",
    "user": "https://github.com/malb"
}
```

<a id='comment:3'>Comment 3:</a>
Attachment: **[trac_9506_error_msg.patch.gz](https://github.com/sagemath/sage/files/ticket9506/trac_9506_error_msg.patch.gz)**

This patch depends on #8059.



---

archive/issue_events_111533.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-07-20T09:08:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111533"
}
```



---

archive/issue_events_111534.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-07-20T09:08:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111534"
}
```



---

archive/issue_events_111535.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-07-20T09:27:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111535"
}
```



---

archive/issue_events_111536.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-07-20T09:27:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111536"
}
```



---

archive/issue_comments_084013.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLooks good.  And I reviewed this informally before when I stress tested it for my application and found it didn't work (for several iterations).  But I think this is good.",
    "created_at": "2010-07-20T09:27:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9506#issuecomment-84013",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:5'>Comment 5:</a>
Looks good.  And I reviewed this informally before when I stress tested it for my application and found it didn't work (for several iterations).  But I think this is good.



---

archive/issue_comments_084014.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2010-08-15T07:00:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9506#issuecomment-84014",
    "user": "https://github.com/qed777"
}
```

Reviewer: **William Stein**



---

archive/issue_events_111537.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-15T08:02:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111537"
}
```



---

archive/issue_events_111538.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-15T08:02:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9506#event-111538"
}
```



---

archive/issue_comments_084015.json:
```json
{
    "body": "Merged: **sage-4.5.3.alpha0**",
    "created_at": "2010-08-15T08:02:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9506#issuecomment-84015",
    "user": "https://github.com/qed777"
}
```

Merged: **sage-4.5.3.alpha0**



---

archive/issue_comments_084016.json:
```json
{
    "body": "Changed merged from **sage-4.5.3.alpha0** to **sage-4.5.3.alpha1**",
    "created_at": "2010-08-15T08:27:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9506#issuecomment-84016",
    "user": "https://github.com/qed777"
}
```

Changed merged from **sage-4.5.3.alpha0** to **sage-4.5.3.alpha1**
