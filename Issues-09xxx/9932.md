# Issue 9932: Make PyWavelets an experimental package

archive/issues_009931.json:
```json
{
    "assignees": [],
    "body": "PyWavelets is a Python wavelet transforms module that can do:\n\n* 1D and 2D Forward and Inverse Discrete Wavelet Transform (DWT and IDWT)\n* 1D and 2D Stationary Wavelet Transform (Undecimated Wavelet Transform)\n* 1D and 2D Wavelet Packet decomposition and reconstruction\n* Computing Approximations of wavelet and scaling functions\n* Over seventy built-in wavelet filters and support for custom wavelets\n* Single and double precision calculations\n* Results compatibility with Matlab Wavelet Toolbox (tm)\n\nDocumentation and links to more resources is available online\nat http://www.pybytes.com/pywavelets/.\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9932_\n\n",
    "closed_at": "2016-02-23T22:59:46Z",
    "created_at": "2010-09-17T10:48:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make PyWavelets an experimental package",
    "type": "issue",
    "updated_at": "2016-02-23T22:59:46Z",
    "url": "https://github.com/sagemath/sage/issues/9932",
    "user": "https://github.com/sagetrac-mhampton"
}
```
PyWavelets is a Python wavelet transforms module that can do:

* 1D and 2D Forward and Inverse Discrete Wavelet Transform (DWT and IDWT)
* 1D and 2D Stationary Wavelet Transform (Undecimated Wavelet Transform)
* 1D and 2D Wavelet Packet decomposition and reconstruction
* Computing Approximations of wavelet and scaling functions
* Over seventy built-in wavelet filters and support for custom wavelets
* Single and double precision calculations
* Results compatibility with Matlab Wavelet Toolbox (tm)

Documentation and links to more resources is available online
at http://www.pybytes.com/pywavelets/.

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/9932_





---

archive/issue_events_117773.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2010-09-17T10:48:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "component: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117773"
}
```



---

archive/issue_events_117774.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2010-09-17T10:48:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117774"
}
```



---

archive/issue_events_117775.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2010-09-17T10:48:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117775"
}
```



---

archive/issue_comments_091697.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nFirst attempt is up at:\n\n[http://sage.math.washington.edu/home/mhampton/PyWavelets-0.2.0.p0.spkg](http://sage.math.washington.edu/home/mhampton/PyWavelets-0.2.0.p0.spkg)",
    "created_at": "2010-09-17T10:49:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9932#issuecomment-91697",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:1" align="right">Comment 1</div>

First attempt is up at:

[http://sage.math.washington.edu/home/mhampton/PyWavelets-0.2.0.p0.spkg](http://sage.math.washington.edu/home/mhampton/PyWavelets-0.2.0.p0.spkg)



---

archive/issue_comments_091698.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nOld-style packages are obsolete.",
    "created_at": "2016-01-22T10:52:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9932#issuecomment-91698",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

Old-style packages are obsolete.



---

archive/issue_comments_091699.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2016-01-22T10:52:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9932#issuecomment-91699",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_117776.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-22T10:52:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117776"
}
```



---

archive/issue_events_117777.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-22T10:52:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117777"
}
```



---

archive/issue_events_117778.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-22T10:52:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117778"
}
```



---

archive/issue_events_117779.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-22T10:53:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117779"
}
```



---

archive/issue_events_117780.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-22T10:53:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117780"
}
```



---

archive/issue_events_117781.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-23T22:59:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117781"
}
```



---

archive/issue_events_117782.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-23T22:59:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117782"
}
```



---

archive/issue_events_117783.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-23T22:59:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-117783"
}
```
