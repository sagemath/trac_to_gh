...
```json
{
    "body": "<a id='comment:5'></a>\nI'm testing this now on 4.5.2.rc0.  Can you say why you included `_sig_`off ?  (I do not claim to understand how Sage/pari signal handling works.)  Without this patch there is a return with no `_sig_`off (unless the call to new_gen does that.)",
...
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [cremona](#comment%3A5):\n> I'm testing this now on 4.5.2.rc0.  Can you say why you included `_sig_`off ?  (I do not claim to understand how Sage/pari signal handling works.)  Without this patch there is a return with no `_sig_`off (unless the call to new_gen does that.)\n\n\nIt's exactly as you say, new_gen() calls `_sig_`off.",
