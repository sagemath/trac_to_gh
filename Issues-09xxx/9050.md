# Issue 9050: libntl.dll should be libntl.dll.a on Cygwin

archive/issues_009050.json:
```json
{
    "assignees": [],
    "body": "This is so that Cygwin will find the shared library before it finds the static library; otherwise, there are lots of segfaults in Sage under Cygwin\n\nComponent: **porting: Cygwin**\n\nAuthor: **Mike Hansen**\n\nReviewer: **William Stein**\n\nMerged: **sage-4.4.3.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9050_\n\n",
    "closed_at": "2010-05-26T01:08:21Z",
    "created_at": "2010-05-25T22:54:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "libntl.dll should be libntl.dll.a on Cygwin",
    "type": "issue",
    "updated_at": "2010-05-27T04:28:21Z",
    "url": "https://github.com/sagemath/sage/issues/9050",
    "user": "https://github.com/mwhansen"
}
```
This is so that Cygwin will find the shared library before it finds the static library; otherwise, there are lots of segfaults in Sage under Cygwin

Component: **porting: Cygwin**

Author: **Mike Hansen**

Reviewer: **William Stein**

Merged: **sage-4.4.3.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/9050_





---

archive/issue_events_115173.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-25T22:54:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "c: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9050#event-115173"
}
```



---

archive/issue_events_115174.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-25T22:54:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9050#event-115174"
}
```



---

archive/issue_events_115175.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-25T22:54:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9050#event-115175"
}
```



---

archive/issue_events_115176.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-25T22:57:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9050#event-115176"
}
```



---

archive/issue_comments_074712.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThere is an spkg for this at http://sage.math.washington.edu/home/mhansen/ntl-5.4.2.p12.spkg",
    "created_at": "2010-05-25T22:57:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9050#issuecomment-74712",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

There is an spkg for this at http://sage.math.washington.edu/home/mhansen/ntl-5.4.2.p12.spkg



---

archive/issue_comments_074713.json:
```json
{
    "body": "Reviewer: **wstein**",
    "created_at": "2010-05-26T01:08:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9050#issuecomment-74713",
    "user": "https://github.com/williamstein"
}
```

Reviewer: **wstein**



---

archive/issue_events_115177.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-26T01:08:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9050#event-115177"
}
```



---

archive/issue_events_115178.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-26T01:08:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9050#event-115178"
}
```



---

archive/issue_comments_074714.json:
```json
{
    "body": "Merged: **4.4.3.alpha0**",
    "created_at": "2010-05-26T01:08:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9050#issuecomment-74714",
    "user": "https://github.com/williamstein"
}
```

Merged: **4.4.3.alpha0**



---

archive/issue_comments_074715.json:
```json
{
    "body": "Changed merged from **4.4.3.alpha0** to **sage-4.4.3.alpha0**",
    "created_at": "2010-05-27T04:28:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9050#issuecomment-74715",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **4.4.3.alpha0** to **sage-4.4.3.alpha0**



---

archive/issue_comments_074716.json:
```json
{
    "body": "Changed reviewer from **wstein** to **William Stein**",
    "created_at": "2010-05-27T04:28:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9050#issuecomment-74716",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed reviewer from **wstein** to **William Stein**



---

archive/issue_events_115179.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-05-27T04:28:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9050",
    "milestone_number": null,
    "milestone_title": "sage-4.4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9050#event-115179"
}
```
