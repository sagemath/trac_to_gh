...
```json
{
    "body": "<a id='comment:1'></a>Please see [comment:ticket:8306:73 John Palmieri's comment] (or [attachment:ticket:8306:deps-deps-new.diff this diff]) at #8306 for some other suggestions.",
...
<a id='comment:1'></a>Please see [comment:ticket:8306:73 John Palmieri's comment] (or [attachment:ticket:8306:deps-deps-new.diff this diff]) at #8306 for some other suggestions.
...
<a id='comment:2'></a>The attached `deps` incorporates suggestions from [Carl Hansen](http://groups.google.com/group/sage-devel/browse_thread/thread/d15b668609983181/268094db15092f85?#268094db15092f85), [David Kirkby](http://groups.google.com/group/sage-devel/browse_thread/thread/7763c990fdc1d5ac#), and [comment:ticket:8306:73 John Palmieri].
...
<a id='comment:3'></a>#9351, which has positive review, makes Sagetex dependant on both gap and Sage, since you need a working Sage in order that Sagetex can be tested with SAGE_CHECK=yes. So the 'deps' file attached to this ticket would need that dependency updating.
...
<a id='comment:4'></a>I really want sage-4.5 to be a better release, so I'm adding this to the blockers. Just like the issue that #9431 is exposing, this is too egregous to delay.
...
<a id='comment:6'></a>See the comment about Maxima needing to be reinstalled above. 
...
```json
{
    "body": "<a id='comment:7'></a>Is this as good a place as any to work out dependencies for glpk?  (See [http://trac.sagemath.org/sage_trac/ticket/9312#comment:24](http://trac.sagemath.org/sage_trac/ticket/9312#comment:24).)",
...
<a id='comment:7'></a>Is this as good a place as any to work out dependencies for glpk?  (See [http://trac.sagemath.org/sage_trac/ticket/9312#comment:24](http://trac.sagemath.org/sage_trac/ticket/9312#comment:24).)
...
```json
{
    "body": "<a id='comment:9'></a>Here are rebased versions, and they also include making cython a prerequisite for glpk.",
...
<a id='comment:9'></a>Here are rebased versions, and they also include making cython a prerequisite for glpk.
...
```json
{
    "body": "<a id='comment:13'></a>Make sure to merge \"deps-new\", not \"deps\".",
...
<a id='comment:13'></a>Make sure to merge "deps-new", not "deps".
...
<a id='comment:17'></a>I apologize for my late reply.
