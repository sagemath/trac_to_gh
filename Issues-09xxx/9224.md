...
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nI've attached a first take on an interface for a Sage `Doctester` class.  Please see [this comment](https://github.com/sagemath/sage/issues/9225#comment:4) at #9225 for possible uses.  I'm not sure whether to put this in the Sage library (under `misc/`) or in `SAGE_LOCAL/bin`.  But I think we'll need to add `sage-doctest` to the unification efforts.\n\nThoughts?",
