# Issue 9494: marked mpoly_all() as not tested

archive/issues_009494.json:
```json
{
    "body": "Demo at SD23.5, also increases coverage!\n\n---\n\nApply [attachment:trac_9494-doctest_mpoly_all.2.patch] to the Sage library.\n\n\nAssignee: mvngu\n\nKeywords: sd32\n\nBranch/Commit: [6f109fad8d6bf24f2622c3f11ca4104445dbe57a](https://github.com/sagemath/sagetrac-mirror/commit/6f109fad8d6bf24f2622c3f11ca4104445dbe57a)\n\nReviewer: Rob Beezer, Benjamin Jones, Ralf Stephan\n\nAuthor: Robert Miller\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/9494\n\n",
    "closed_at": "2014-02-23T07:46:26Z",
    "created_at": "2010-07-14T09:28:21Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.2",
    "title": "marked mpoly_all() as not tested",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9494",
    "user": "https://trac.sagemath.org/admin/accounts/users/demosd235"
}
```
Demo at SD23.5, also increases coverage!

---

Apply [attachment:trac_9494-doctest_mpoly_all.2.patch] to the Sage library.


Assignee: mvngu

Keywords: sd32

Branch/Commit: [6f109fad8d6bf24f2622c3f11ca4104445dbe57a](https://github.com/sagemath/sagetrac-mirror/commit/6f109fad8d6bf24f2622c3f11ca4104445dbe57a)

Reviewer: Rob Beezer, Benjamin Jones, Ralf Stephan

Author: Robert Miller

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/9494





---

archive/issue_comments_115127.json:
```json
{
    "body": "<a id='comment:1'></a>\nAdd a comment to a ticket here.",
    "created_at": "2010-07-14T09:33:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115127",
    "user": "https://trac.sagemath.org/admin/accounts/users/demosd235"
}
```

<a id='comment:1'></a>
Add a comment to a ticket here.



---

archive/issue_comments_115128.json:
```json
{
    "body": "<a id='comment:2'></a>\nIs this ready yet?",
    "created_at": "2010-07-14T09:33:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115128",
    "user": "https://trac.sagemath.org/admin/accounts/users/demosd235"
}
```

<a id='comment:2'></a>
Is this ready yet?



---

archive/issue_comments_115129.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-07-14T09:33:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115129",
    "user": "https://trac.sagemath.org/admin/accounts/users/demosd235"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_115130.json:
```json
{
    "body": "<a id='comment:3'></a>\nAccording to http://www.sagemath.org/doc/developer/conventions.html#docstring-markup-with-rest-and-sphinx it should be `EXAMPLES:` instead of `EXAMPLE::`.\n\nIt's enough to put only 3 dots between `<BLANKLINE>` and `System` but this is okay as well.",
    "created_at": "2010-12-01T12:51:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115130",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:3'></a>
According to http://www.sagemath.org/doc/developer/conventions.html#docstring-markup-with-rest-and-sphinx it should be `EXAMPLES:` instead of `EXAMPLE::`.

It's enough to put only 3 dots between `<BLANKLINE>` and `System` but this is okay as well.



---

archive/issue_comments_115131.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2011-05-26T15:43:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115131",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_115132.json:
```json
{
    "body": "Changing status from needs_work to needs_info.",
    "created_at": "2011-08-22T07:09:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115132",
    "user": "https://github.com/rbeezer"
}
```

Changing status from needs_work to needs_info.



---

archive/issue_comments_115133.json:
```json
{
    "body": "<a id='comment:5'></a>\nIf an \"EXAMPLES\" block just has one example, then it is OK to drop the S.  And if there is no prose before the test, then a double-colon can be used to indicate verbatim text next.\n\nThere are excess ... but they cause no harm.  This ticket looks good to me.  Am I missing something?  It looks ready for a positive review to me.\n\nI am working on this as a high-priority ticket for Bug Days 32.\n\nRob",
    "created_at": "2011-08-22T07:09:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115133",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:5'></a>
If an "EXAMPLES" block just has one example, then it is OK to drop the S.  And if there is no prose before the test, then a double-colon can be used to indicate verbatim text next.

There are excess ... but they cause no harm.  This ticket looks good to me.  Am I missing something?  It looks ready for a positive review to me.

I am working on this as a high-priority ticket for Bug Days 32.

Rob



---

archive/issue_comments_115134.json:
```json
{
    "body": "Reviewer: rbeezer, benjaminfjones",
    "created_at": "2011-08-23T00:46:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115134",
    "user": "https://github.com/benjaminfjones"
}
```

Reviewer: rbeezer, benjaminfjones



---

archive/issue_comments_115135.json:
```json
{
    "body": "<a id='comment:6'></a>\nThe patch looks good to me and passes doctests:\n\n```\n\nbjones@sage:~/sage/sage-4.7.1/devel/sage$ ../../sage -t sage/tests/benchmark.py \nsage -t  \"devel/sage-9494/sage/tests/benchmark.py\"          \n\t [8.6 s]\n \n----------------------------------------------------------------------\nAll tests passed!\nTotal time for all tests: 8.6 seconds\n```\n\nI'm giving it a positive review.",
    "created_at": "2011-08-23T00:46:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115135",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:6'></a>
The patch looks good to me and passes doctests:

```

bjones@sage:~/sage/sage-4.7.1/devel/sage$ ../../sage -t sage/tests/benchmark.py 
sage -t  "devel/sage-9494/sage/tests/benchmark.py"          
	 [8.6 s]
 
----------------------------------------------------------------------
All tests passed!
Total time for all tests: 8.6 seconds
```

I'm giving it a positive review.



---

archive/issue_comments_115136.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2011-08-23T00:47:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115136",
    "user": "https://github.com/benjaminfjones"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_115137.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-08-23T00:47:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115137",
    "user": "https://github.com/benjaminfjones"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_115138.json:
```json
{
    "body": "Changing reviewer from \"rbeezer, benjaminfjones\" to \"Rob Beezer, Benjamin Jones\"",
    "created_at": "2011-08-23T20:49:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115138",
    "user": "https://github.com/benjaminfjones"
}
```

Changing reviewer from "rbeezer, benjaminfjones" to "Rob Beezer, Benjamin Jones"



---

archive/issue_comments_115139.json:
```json
{
    "body": "Changing keywords from \"\" to \"sd32\".",
    "created_at": "2011-08-24T23:45:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115139",
    "user": "https://github.com/williamstein"
}
```

Changing keywords from "" to "sd32".



---

archive/issue_comments_115140.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,6 @@\n Demo at SD23.5, also increases coverage!\n+\n+---\n+\n+Apply [attachment:trac_9494-doctest_mpoly_all.patch] to the Sage library.\n+\n``````\n",
    "created_at": "2011-09-09T00:54:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115140",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,6 @@
 Demo at SD23.5, also increases coverage!
+
+---
+
+Apply [attachment:trac_9494-doctest_mpoly_all.patch] to the Sage library.
+
``````




---

archive/issue_comments_115141.json:
```json
{
    "body": "Work_Issues: Fix `mpoly_all()` or mark the doctest `# not tested`.",
    "created_at": "2011-09-16T06:38:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115141",
    "user": "https://github.com/nexttime"
}
```

Work_Issues: Fix `mpoly_all()` or mark the doctest `# not tested`.



---

archive/issue_comments_115142.json:
```json
{
    "body": "<a id='comment:12'></a>\nActually, `mpoly_all()` needs work, since it is apparently unable to interrupt (e.g.) Mathematica a second time.\n\nOn redhawk, where `/home` still isn't mounted, Mathematica is installed but fails to startup (prompting the user), such that I get:\n\n```sh\nleif@redhawk:/$ time sage -c \"from sage.tests.benchmark import mpoly_all; mpoly_all()\" \n\n\n\nCompute (x_0 + ... + x_99) * (x_100 + ... + x_200) over Rational Field (use singular for Sage mult.)\n  System      min         avg         max         trials          cpu or wall\n* sage        0.049767    0.049767    0.049767    1                  0.049767w\nUnable to start magma\ncomputation timed out because alarm was set for 60 seconds\nUnable to start macaulay2 because the command 'M2 --no-debug --no-readline --silent -e 'ZZ#{Standard,Core#\"private dictionary\"#\"InputPrompt\"} = lineno -> \"_EGAS_ : \";ZZ#{Standard,Core#\"private dictionary\"#\"InputContinuationPrompt\"} = lineno -> \"_EGAS_ : \";printWidth = 0;lineNumber = 10^9;'' failed.\n\n\n\n\nCompute (x_0 + ... + x_199) * (x_200 + ... + x_400) over Rational Field (use singular for Sage mult.)\n  System      min         avg         max         trials          cpu or wall\n* sage        0.089736    0.089736    0.089736    1                  0.089736w\nUnable to start magma\nInterrupting Mathematica...\n```\nwhich does not terminate.\n\nWhat is this doctest supposed to \"test\" at all?\n\nThe output is random, so it doesn't really test anything, except perhaps which external programs are installed, but that's nothing subject to a doctest, so IMHO this ticket should either be closed as invalid, or the \"doctest\" (i.e., the example) should be marked `# not tested`, at least until `mpoly_all()` got fixed.",
    "created_at": "2011-09-16T06:38:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115142",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:12'></a>
Actually, `mpoly_all()` needs work, since it is apparently unable to interrupt (e.g.) Mathematica a second time.

On redhawk, where `/home` still isn't mounted, Mathematica is installed but fails to startup (prompting the user), such that I get:

```sh
leif@redhawk:/$ time sage -c "from sage.tests.benchmark import mpoly_all; mpoly_all()" 



Compute (x_0 + ... + x_99) * (x_100 + ... + x_200) over Rational Field (use singular for Sage mult.)
  System      min         avg         max         trials          cpu or wall
* sage        0.049767    0.049767    0.049767    1                  0.049767w
Unable to start magma
computation timed out because alarm was set for 60 seconds
Unable to start macaulay2 because the command 'M2 --no-debug --no-readline --silent -e 'ZZ#{Standard,Core#"private dictionary"#"InputPrompt"} = lineno -> "_EGAS_ : ";ZZ#{Standard,Core#"private dictionary"#"InputContinuationPrompt"} = lineno -> "_EGAS_ : ";printWidth = 0;lineNumber = 10^9;'' failed.




Compute (x_0 + ... + x_199) * (x_200 + ... + x_400) over Rational Field (use singular for Sage mult.)
  System      min         avg         max         trials          cpu or wall
* sage        0.089736    0.089736    0.089736    1                  0.089736w
Unable to start magma
Interrupting Mathematica...
```
which does not terminate.

What is this doctest supposed to "test" at all?

The output is random, so it doesn't really test anything, except perhaps which external programs are installed, but that's nothing subject to a doctest, so IMHO this ticket should either be closed as invalid, or the "doctest" (i.e., the example) should be marked `# not tested`, at least until `mpoly_all()` got fixed.



---

archive/issue_comments_115143.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2011-09-16T06:38:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115143",
    "user": "https://github.com/nexttime"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_115144.json:
```json
{
    "body": "Attachment [trac_9494-doctest_mpoly_all.2.patch](tarball://root/attachments/some-uuid/ticket9494/trac_9494-doctest_mpoly_all.2.patch) by @benjaminfjones created at 2012-04-07 23:51:39\n\nmarked mpoly_all() as not tested",
    "created_at": "2012-04-07T23:51:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115144",
    "user": "https://github.com/benjaminfjones"
}
```

Attachment [trac_9494-doctest_mpoly_all.2.patch](tarball://root/attachments/some-uuid/ticket9494/trac_9494-doctest_mpoly_all.2.patch) by @benjaminfjones created at 2012-04-07 23:51:39

marked mpoly_all() as not tested



---

archive/issue_comments_115145.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,5 +2,5 @@\n \n ---\n \n-Apply [attachment:trac_9494-doctest_mpoly_all.patch] to the Sage library.\n+Apply [attachment:trac_9494-doctest_mpoly_all.2.patch] to the Sage library.\n \n``````\n",
    "created_at": "2012-04-07T23:52:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115145",
    "user": "https://github.com/benjaminfjones"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,5 +2,5 @@
 
 ---
 
-Apply [attachment:trac_9494-doctest_mpoly_all.patch] to the Sage library.
+Apply [attachment:trac_9494-doctest_mpoly_all.2.patch] to the Sage library.
 
``````




---

archive/issue_comments_115146.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2012-04-07T23:52:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115146",
    "user": "https://github.com/benjaminfjones"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_115147.json:
```json
{
    "body": "Changing work_issues from \"Fix `mpoly_all()` or mark the doctest `# not tested`.\" to \"\"",
    "created_at": "2012-04-07T23:52:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115147",
    "user": "https://github.com/benjaminfjones"
}
```

Changing work_issues from "Fix `mpoly_all()` or mark the doctest `# not tested`." to ""



---

archive/issue_comments_115148.json:
```json
{
    "body": "<a id='comment:14'></a>\nI marked `mpoly_all()` as `# not tested`. The new patch applied cleanly to 5.0.beta12 and passes tests on `sage.math.washington.edu` where Mma is installed. I'm running tests on redhawk now to double check.",
    "created_at": "2012-04-07T23:53:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115148",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:14'></a>
I marked `mpoly_all()` as `# not tested`. The new patch applied cleanly to 5.0.beta12 and passes tests on `sage.math.washington.edu` where Mma is installed. I'm running tests on redhawk now to double check.



---

archive/issue_comments_115149.json:
```json
{
    "body": "<a id='comment:15'></a>\nThere seems to still be an issue with the Mma installation on redhawk and the `mpoly_all()` test is still unable to interrupt the crashed process all the time:\n\n```\nCompute (x_1 + 2*x_2 + 3*x_3 + ... + 128*x_128) * (129 * x_129 + ... + 257*x_257) over Rational Field (use singular for Sage mult.)\n  System      min         avg         max         trials          cpu or wall\n* sage        0.049826    0.049826    0.049826    1                  0.049826w\nUnable to start magma\nMathematica crashed -- automatically restarting.\ncomputation timed out because alarm was set for 60 seconds\nUnable to start macaulay2 because the command 'M2 --no-debug --no-readline --silent -e 'ZZ#{Standard,Core#\"private dictionary\"#\"InputPrompt\"} = lineno -> \"_EGAS_ : \";ZZ#{Standard,Core#\"private dictionary\"#\"InputContinuationPrompt\"} = lineno -> \"_EGAS_ : \";printWidth = 0;lineNumber = 10^9;'' failed.\n\n\n\n\nCompute (x_1 + 2*x_2 + 3*x_3 + ... + 256*x_256) * (257 * x_257 + ... + 513*x_513) over Rational Field (use singular for Sage mult.)\n  System      min         avg         max         trials          cpu or wall\n* sage        0.149662    0.149662    0.149662    1                  0.149662w\nUnable to start magma\nInterrupting Mathematica...\n^C\nUnable to start macaulay2 because the command 'M2 --no-debug --no-readline --silent -e 'ZZ#{Standard,Core#\"private dictionary\"#\"InputPrompt\"} = lineno -> \"_EGAS_ : \";ZZ#{Standard,Core#\"private dictionary\"#\"InputContinuationPrompt\"} = lineno -> \"_EGAS_ : \";printWidth = 0;lineNumber = 10^9;'' failed.\n```\n\nBut I'd say that's an issue for another ticket.",
    "created_at": "2012-04-08T04:06:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115149",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:15'></a>
There seems to still be an issue with the Mma installation on redhawk and the `mpoly_all()` test is still unable to interrupt the crashed process all the time:

```
Compute (x_1 + 2*x_2 + 3*x_3 + ... + 128*x_128) * (129 * x_129 + ... + 257*x_257) over Rational Field (use singular for Sage mult.)
  System      min         avg         max         trials          cpu or wall
* sage        0.049826    0.049826    0.049826    1                  0.049826w
Unable to start magma
Mathematica crashed -- automatically restarting.
computation timed out because alarm was set for 60 seconds
Unable to start macaulay2 because the command 'M2 --no-debug --no-readline --silent -e 'ZZ#{Standard,Core#"private dictionary"#"InputPrompt"} = lineno -> "_EGAS_ : ";ZZ#{Standard,Core#"private dictionary"#"InputContinuationPrompt"} = lineno -> "_EGAS_ : ";printWidth = 0;lineNumber = 10^9;'' failed.




Compute (x_1 + 2*x_2 + 3*x_3 + ... + 256*x_256) * (257 * x_257 + ... + 513*x_513) over Rational Field (use singular for Sage mult.)
  System      min         avg         max         trials          cpu or wall
* sage        0.149662    0.149662    0.149662    1                  0.149662w
Unable to start magma
Interrupting Mathematica...
^C
Unable to start macaulay2 because the command 'M2 --no-debug --no-readline --silent -e 'ZZ#{Standard,Core#"private dictionary"#"InputPrompt"} = lineno -> "_EGAS_ : ";ZZ#{Standard,Core#"private dictionary"#"InputContinuationPrompt"} = lineno -> "_EGAS_ : ";printWidth = 0;lineNumber = 10^9;'' failed.
```

But I'd say that's an issue for another ticket.



---

archive/issue_events_028712.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9494#event-28712"
}
```



---

archive/issue_events_028713.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9494#event-28713"
}
```



---

archive/issue_events_028714.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9494#event-28714"
}
```



---

archive/issue_comments_115150.json:
```json
{
    "body": "Branch: [u/rws/ticket/9494](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/ticket/9494)",
    "created_at": "2014-02-14T16:11:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115150",
    "user": "https://github.com/rwst"
}
```

Branch: [u/rws/ticket/9494](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/ticket/9494)



---

archive/issue_comments_115151.json:
```json
{
    "body": "<a id='comment:19'></a>\nMoved patch to git.\n\n---\nNew commits:\n|                                                                                                                                          |                                              |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------|\n|[1803137](https://github.com/sagemath/sagetrac-mirror/commit/1803137ae4c88a1e6015e5b58498947c906c92af)|`trac #9494: marked mpoly_all() as not tested`|",
    "created_at": "2014-02-14T16:13:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115151",
    "user": "https://github.com/rwst"
}
```

<a id='comment:19'></a>
Moved patch to git.

---
New commits:
|                                                                                                                                          |                                              |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------|
|[1803137](https://github.com/sagemath/sagetrac-mirror/commit/1803137ae4c88a1e6015e5b58498947c906c92af)|`trac #9494: marked mpoly_all() as not tested`|



---

archive/issue_comments_115152.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-02-14T16:13:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115152",
    "user": "https://github.com/rwst"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_115153.json:
```json
{
    "body": "Commit: [1803137ae4c88a1e6015e5b58498947c906c92af](https://github.com/sagemath/sagetrac-mirror/commit/1803137ae4c88a1e6015e5b58498947c906c92af)",
    "created_at": "2014-02-14T16:13:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115153",
    "user": "https://github.com/rwst"
}
```

Commit: [1803137ae4c88a1e6015e5b58498947c906c92af](https://github.com/sagemath/sagetrac-mirror/commit/1803137ae4c88a1e6015e5b58498947c906c92af)



---

archive/issue_comments_115154.json:
```json
{
    "body": "Changing reviewer from \"Rob Beezer, Benjamin Jones\" to \"Rob Beezer, Benjamin Jones, Ralf Stephan\"",
    "created_at": "2014-02-14T16:13:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115154",
    "user": "https://github.com/rwst"
}
```

Changing reviewer from "Rob Beezer, Benjamin Jones" to "Rob Beezer, Benjamin Jones, Ralf Stephan"



---

archive/issue_events_028715.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "rename": {
        "from": "Add a doctest to benchmark.py",
        "to": "marked mpoly_all() as not tested"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9494#event-28715"
}
```



---

archive/issue_comments_115155.json:
```json
{
    "body": "<a id='comment:20'></a>\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n|                                                                                                                                          |                                         |\n|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|\n|[6f109fa](https://github.com/sagemath/sagetrac-mirror/commit/6f109fad8d6bf24f2622c3f11ca4104445dbe57a)|`Merge branch 'develop' into ticket/9494`|",
    "created_at": "2014-02-20T17:23:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115155",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:20'></a>
Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
|                                                                                                                                          |                                         |
|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|
|[6f109fa](https://github.com/sagemath/sagetrac-mirror/commit/6f109fad8d6bf24f2622c3f11ca4104445dbe57a)|`Merge branch 'develop' into ticket/9494`|



---

archive/issue_comments_115156.json:
```json
{
    "body": "Changing commit from \"[1803137ae4c88a1e6015e5b58498947c906c92af](https://github.com/sagemath/sagetrac-mirror/commit/1803137ae4c88a1e6015e5b58498947c906c92af)\" to \"[6f109fad8d6bf24f2622c3f11ca4104445dbe57a](https://github.com/sagemath/sagetrac-mirror/commit/6f109fad8d6bf24f2622c3f11ca4104445dbe57a)\"",
    "created_at": "2014-02-20T17:23:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115156",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "[1803137ae4c88a1e6015e5b58498947c906c92af](https://github.com/sagemath/sagetrac-mirror/commit/1803137ae4c88a1e6015e5b58498947c906c92af)" to "[6f109fad8d6bf24f2622c3f11ca4104445dbe57a](https://github.com/sagemath/sagetrac-mirror/commit/6f109fad8d6bf24f2622c3f11ca4104445dbe57a)"



---

archive/issue_comments_115157.json:
```json
{
    "body": "Changing status from positive_review to needs_review.",
    "created_at": "2014-02-20T17:23:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115157",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing status from positive_review to needs_review.



---

archive/issue_comments_115158.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-02-20T17:45:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115158",
    "user": "https://github.com/rwst"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_115159.json:
```json
{
    "body": "Changing branch from \"[u/rws/ticket/9494](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/ticket/9494)\" to \"[6f109fad8d6bf24f2622c3f11ca4104445dbe57a](https://github.com/sagemath/sagetrac-mirror/commit/6f109fad8d6bf24f2622c3f11ca4104445dbe57a)\"",
    "created_at": "2014-02-23T07:46:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115159",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "[u/rws/ticket/9494](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/ticket/9494)" to "[6f109fad8d6bf24f2622c3f11ca4104445dbe57a](https://github.com/sagemath/sagetrac-mirror/commit/6f109fad8d6bf24f2622c3f11ca4104445dbe57a)"



---

archive/issue_events_028716.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-23T07:46:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9494#event-28716"
}
```



---

archive/issue_comments_115160.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2014-02-23T07:46:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9494#issuecomment-115160",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
