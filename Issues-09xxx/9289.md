# Issue 9289: Implement Puiseux polynomials

archive/issues_009289.json:
```json
{
    "assignees": [],
    "body": "Implement the ring of Puiseux polynomials. Those are usual\npolynomials, except that exponents can be any rational number.\n\n```\n   sage: S = PolynomialRing(QQ, ['a','b','c']); S\n   Multivariate Puiseux Polynomial Ring in a, b, c over Rational Field\n```\n\nOperations: roots of monomials, ...\n\nSee also: http://fr.wikipedia.org/wiki/Th%C3%A9or%C3%A8me_de_Puiseux, http://en.wikipedia.org/wiki/Puiseux_series\n\n\nAssignee: **@aghitza**\n\nCC:  sage-combinat\n\nKeywords: **Puiseux**\n\nBranch/Commit: **[public/9289](https://github.com/sagemath/sagetrac-mirror/tree/public/9289) @ [336e58b](https://github.com/sagemath/sagetrac-mirror/commit/336e58b484852a6f89ba2b813c2d334574297455)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9289_\n\n",
    "created_at": "2010-06-21T07:34:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wishlist%20item",
        "https://github.com/sagemath/sage/labels/needs%20info"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement Puiseux polynomials",
    "type": "issue",
    "updated_at": "2018-07-05T14:39:36Z",
    "url": "https://github.com/sagemath/sage/issues/9289",
    "user": "https://github.com/nthiery"
}
```
Implement the ring of Puiseux polynomials. Those are usual
polynomials, except that exponents can be any rational number.

```
   sage: S = PolynomialRing(QQ, ['a','b','c']); S
   Multivariate Puiseux Polynomial Ring in a, b, c over Rational Field
```

Operations: roots of monomials, ...

See also: http://fr.wikipedia.org/wiki/Th%C3%A9or%C3%A8me_de_Puiseux, http://en.wikipedia.org/wiki/Puiseux_series


Assignee: **@aghitza**

CC:  sage-combinat

Keywords: **Puiseux**

Branch/Commit: **[public/9289](https://github.com/sagemath/sagetrac-mirror/tree/public/9289) @ [336e58b](https://github.com/sagemath/sagetrac-mirror/commit/336e58b484852a6f89ba2b813c2d334574297455)**

_Issue created by migration from https://trac.sagemath.org/ticket/9289_





---

archive/issue_events_108313.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-06-21T07:34:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108313"
}
```



---

archive/issue_events_108314.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-06-21T07:34:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108314"
}
```



---

archive/issue_events_108315.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-06-21T07:34:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108315"
}
```



---

archive/issue_events_108316.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-06-21T07:34:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108316"
}
```



---

archive/issue_comments_080177.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,6 +6,7 @@\n    Multivariate Puiseux Polynomial Ring in a, b, c over Rational Field\n ```\n \n+Operations: roots of monomials, ...\n \n See also: http://fr.wikipedia.org/wiki/Th%C3%A9or%C3%A8me_de_Puiseux, http://en.wikipedia.org/wiki/Puiseux_series\n \n``````\n",
    "created_at": "2010-06-21T07:36:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9289#issuecomment-80177",
    "user": "https://github.com/nthiery"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,6 +6,7 @@
    Multivariate Puiseux Polynomial Ring in a, b, c over Rational Field
 ```
 
+Operations: roots of monomials, ...
 
 See also: http://fr.wikipedia.org/wiki/Th%C3%A9or%C3%A8me_de_Puiseux, http://en.wikipedia.org/wiki/Puiseux_series
 
``````




---

archive/issue_comments_080178.json:
```json
{
    "body": "Changed keywords from **** to **Puiseux**",
    "created_at": "2014-03-06T15:25:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9289#issuecomment-80178",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **Puiseux**



---

archive/issue_events_108317.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-16T16:18:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108317"
}
```



---

archive/issue_events_108318.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-16T16:18:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108318"
}
```



---

archive/issue_events_108319.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-16T16:18:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108319"
}
```



---

archive/issue_events_108320.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-16T16:18:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108320"
}
```



---

archive/issue_comments_080179.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNever mind. I thought this was a duplicate.",
    "created_at": "2014-03-16T16:18:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9289#issuecomment-80179",
    "user": "https://github.com/rwst"
}
```

<a id='comment:3'>Comment 3:</a>
Never mind. I thought this was a duplicate.



---

archive/issue_events_108321.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-16T16:20:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108321"
}
```



---

archive/issue_events_108322.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-16T16:20:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108322"
}
```



---

archive/issue_events_108323.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-16T16:20:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9289#event-108323"
}
```



---

archive/issue_comments_080180.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nSee also #4618.",
    "created_at": "2015-02-12T15:01:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9289#issuecomment-80180",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:5'>Comment 5:</a>
See also #4618.



---

archive/issue_comments_080181.json:
```json
{
    "body": "Branch: **[public/9289](https://github.com/sagemath/sagetrac-mirror/tree/public/9289)**",
    "created_at": "2018-07-05T14:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9289#issuecomment-80181",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[public/9289](https://github.com/sagemath/sagetrac-mirror/tree/public/9289)**



---

archive/issue_comments_080182.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/336e58b484852a6f89ba2b813c2d334574297455\">336e58b</a></td><td><code>very rough and sketchy tentative of Puiseux polynomials</code></td></tr></table>\n",
    "created_at": "2018-07-05T14:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9289#issuecomment-80182",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'>Comment 6:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/336e58b484852a6f89ba2b813c2d334574297455">336e58b</a></td><td><code>very rough and sketchy tentative of Puiseux polynomials</code></td></tr></table>




---

archive/issue_comments_080183.json:
```json
{
    "body": "Commit: **[336e58b](https://github.com/sagemath/sagetrac-mirror/commit/336e58b484852a6f89ba2b813c2d334574297455)**",
    "created_at": "2018-07-05T14:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9289",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9289#issuecomment-80183",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[336e58b](https://github.com/sagemath/sagetrac-mirror/commit/336e58b484852a6f89ba2b813c2d334574297455)**
