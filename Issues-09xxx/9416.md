# Issue 9416: elliptic curve E.__call__ is not fully equivalent to E.point, specifically for check flag

archive/issues_009416.json:
```json
{
    "assignees": [],
    "body": "For example, the flag check=False can only be used with E.point()\n\n```\nsage: emb = K2.places()[0]\nsage: E_complex = EllipticCurve(CC,map(emb, E2point.a_invariants()))\nsage: P = L2[-1]\nsage: print 'Exact arithmetic:', P in E2\nExact arithmetic: True\nsage: P_complex = map(emb,P)\nsage: E_complex(P_complex, check=False)\nTypeError: __call__() got an unexpected keyword argument 'check'\n\nsage: E_complex.point(P_complex, check=False)\n(-10.0000000000000 + 5.65685424949238*I : 28.0000000000000 + 25.4558441227157*I : 1.00000000000000)\n```\n\nAssignee: **@JohnCremona**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9416_\n\n",
    "created_at": "2010-07-03T03:00:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "elliptic curve E.__call__ is not fully equivalent to E.point, specifically for check flag",
    "type": "issue",
    "updated_at": "2010-07-03T03:01:32Z",
    "url": "https://github.com/sagemath/sage/issues/9416",
    "user": "https://github.com/rkirov"
}
```
For example, the flag check=False can only be used with E.point()

```
sage: emb = K2.places()[0]
sage: E_complex = EllipticCurve(CC,map(emb, E2point.a_invariants()))
sage: P = L2[-1]
sage: print 'Exact arithmetic:', P in E2
Exact arithmetic: True
sage: P_complex = map(emb,P)
sage: E_complex(P_complex, check=False)
TypeError: __call__() got an unexpected keyword argument 'check'

sage: E_complex.point(P_complex, check=False)
(-10.0000000000000 + 5.65685424949238*I : 28.0000000000000 + 25.4558441227157*I : 1.00000000000000)
```

Assignee: **@JohnCremona**

_Issue created by migration from https://trac.sagemath.org/ticket/9416_





---

archive/issue_events_110099.json:
```json
{
    "actor": "https://github.com/rkirov",
    "created_at": "2010-07-03T03:00:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9416",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9416#event-110099"
}
```



---

archive/issue_events_110100.json:
```json
{
    "actor": "https://github.com/rkirov",
    "created_at": "2010-07-03T03:00:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9416",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9416#event-110100"
}
```



---

archive/issue_events_110101.json:
```json
{
    "actor": "https://github.com/rkirov",
    "created_at": "2010-07-03T03:00:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9416",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9416#event-110101"
}
```



---

archive/issue_comments_082417.json:
```json
{
    "body": "Changed assignee from **@aghitza** to **@JohnCremona**",
    "created_at": "2010-07-03T03:01:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9416",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9416#issuecomment-82417",
    "user": "https://github.com/rkirov"
}
```

Changed assignee from **@aghitza** to **@JohnCremona**



---

archive/issue_events_110102.json:
```json
{
    "actor": "https://github.com/rkirov",
    "created_at": "2010-07-03T03:01:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9416",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9416#event-110102"
}
```



---

archive/issue_events_110103.json:
```json
{
    "actor": "https://github.com/rkirov",
    "created_at": "2010-07-03T03:01:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9416",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9416#event-110103"
}
```



---

archive/issue_events_110104.json:
```json
{
    "actor": "https://github.com/rkirov",
    "created_at": "2010-07-03T03:01:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9416",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9416#event-110104"
}
```



---

archive/issue_events_110105.json:
```json
{
    "actor": "https://github.com/rkirov",
    "created_at": "2010-07-03T03:01:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9416",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9416#event-110105"
}
```
