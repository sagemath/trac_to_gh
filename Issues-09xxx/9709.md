# Issue 9709: Crypto lattice basis generator (update)

archive/issues_009709.json:
```json
{
    "assignees": [],
    "body": "A bugfix/efficiency update for the new lattice basis generator. \n\nSpecifically, the explanations and generation of dual lattices has been improved.\n\nAssignee: **mvngu**\n\nKeywords: **cryptography, lattices**\n\nAuthor: **Richard Lindner**\n\nReviewer: **Martin Albrecht**\n\nMerged: **sage-4.6.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9709_\n\n",
    "closed_at": "2010-09-15T11:13:21Z",
    "created_at": "2010-08-09T14:09:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cryptography",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Crypto lattice basis generator (update)",
    "type": "issue",
    "updated_at": "2010-09-15T11:13:21Z",
    "url": "https://github.com/sagemath/sage/issues/9709",
    "user": "https://github.com/sagetrac-rlindner"
}
```
A bugfix/efficiency update for the new lattice basis generator. 

Specifically, the explanations and generation of dual lattices has been improved.

Assignee: **mvngu**

Keywords: **cryptography, lattices**

Author: **Richard Lindner**

Reviewer: **Martin Albrecht**

Merged: **sage-4.6.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/9709_





---

archive/issue_events_114505.json:
```json
{
    "actor": "https://github.com/sagetrac-rlindner",
    "created_at": "2010-08-09T14:09:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9709#event-114505"
}
```



---

archive/issue_events_114506.json:
```json
{
    "actor": "https://github.com/sagetrac-rlindner",
    "created_at": "2010-08-09T14:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cryptography",
    "label_color": "0000ff",
    "label_name": "component: cryptography",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9709#event-114506"
}
```



---

archive/issue_events_114507.json:
```json
{
    "actor": "https://github.com/sagetrac-rlindner",
    "created_at": "2010-08-09T14:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9709#event-114507"
}
```



---

archive/issue_events_114508.json:
```json
{
    "actor": "https://github.com/sagetrac-rlindner",
    "created_at": "2010-08-09T14:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9709#event-114508"
}
```



---

archive/issue_events_114509.json:
```json
{
    "actor": "https://github.com/sagetrac-rlindner",
    "created_at": "2010-08-09T14:16:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9709#event-114509"
}
```



---

archive/issue_comments_087381.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[14803.patch.gz](https://github.com/sagemath/sage/files/ticket9709/14803.patch.gz)**\n\nThis is an update following the discussion in ticket !#9496",
    "created_at": "2010-08-09T14:16:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9709#issuecomment-87381",
    "user": "https://github.com/sagetrac-rlindner"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[14803.patch.gz](https://github.com/sagemath/sage/files/ticket9709/14803.patch.gz)**

This is an update following the discussion in ticket !#9496



---

archive/issue_comments_087382.json:
```json
{
    "body": "Reviewer: **Martin Albrecht**",
    "created_at": "2010-08-11T15:14:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9709#issuecomment-87382",
    "user": "https://github.com/malb"
}
```

Reviewer: **Martin Albrecht**



---

archive/issue_events_114510.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-08-11T15:14:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9709#event-114510"
}
```



---

archive/issue_events_114511.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-08-11T15:14:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9709#event-114511"
}
```



---

archive/issue_comments_087383.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPatch looks good, doctests pass",
    "created_at": "2010-08-11T15:14:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9709#issuecomment-87383",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'>Comment 2:</a>
Patch looks good, doctests pass



---

archive/issue_comments_087384.json:
```json
{
    "body": "Merged: **sage-4.6.alpha1**",
    "created_at": "2010-09-15T11:13:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9709#issuecomment-87384",
    "user": "https://github.com/qed777"
}
```

Merged: **sage-4.6.alpha1**



---

archive/issue_events_114512.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-09-15T11:13:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9709#event-114512"
}
```



---

archive/issue_events_114513.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-09-15T11:13:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9709",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9709#event-114513"
}
```
