# Issue 9983: boehm_gc fails to build properly on AIX 5.3

archive/issues_009982.json:
```json
{
    "body": "Using the following system: \n\n* IBM [RS/6000 7025 F50](http://publib.boulder.ibm.com/infocenter/pseries/v5r3/index.jsp?topic=/com.ibm.pseries.doc/hardware_docs/rs6000_7025f50series.htm)\n* 4 x 332 MHz 32-bit PowerPC CPUs\n* 3 GB RAM\n* A fair wide mixture of disks sizes (3 x 9 GB, 1 x 18 GB, 2 x 36 GB and 1 x 73 GB)\n* AIX 5.3 (A POSIX certified operating system)\n* gcc 4.2.4 downloaded from [pware](http://pware.hvcc.edu/)\n* DDS-4 tape drive \n\nboehm_gc-7.1.p6 fails to build properly on AIX. See attached log of the build. I'm not sure yet if this is an upstream problem on a Sage problem. \n\n**Assignee:** drkirkby\n\n**CC:**  @fchapoton\n\n**Upstream:** None of the above - read trac for reasoning.\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/9983\n\n",
    "closed_at": "2016-08-30T13:32:25Z",
    "created_at": "2010-09-23T20:26:50Z",
    "labels": [
        "component: porting: aix or hp-ux",
        "minor",
        "bug",
        "wontfix"
    ],
    "title": "boehm_gc fails to build properly on AIX 5.3",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9983",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```
Using the following system: 

* IBM [RS/6000 7025 F50](http://publib.boulder.ibm.com/infocenter/pseries/v5r3/index.jsp?topic=/com.ibm.pseries.doc/hardware_docs/rs6000_7025f50series.htm)
* 4 x 332 MHz 32-bit PowerPC CPUs
* 3 GB RAM
* A fair wide mixture of disks sizes (3 x 9 GB, 1 x 18 GB, 2 x 36 GB and 1 x 73 GB)
* AIX 5.3 (A POSIX certified operating system)
* gcc 4.2.4 downloaded from [pware](http://pware.hvcc.edu/)
* DDS-4 tape drive 

boehm_gc-7.1.p6 fails to build properly on AIX. See attached log of the build. I'm not sure yet if this is an upstream problem on a Sage problem. 

**Assignee:** drkirkby

**CC:**  @fchapoton

**Upstream:** None of the above - read trac for reasoning.

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/9983





---

archive/issue_comments_094706.json:
```json
{
    "body": "Log of a failed build on an RS/6000 running AIX 5.3",
    "created_at": "2010-09-23T20:27:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9983#issuecomment-94706",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

Log of a failed build on an RS/6000 running AIX 5.3



---

archive/issue_events_076959.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-08-30T04:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9983#event-76959"
}
```



---

archive/attachments_013027.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "boehm_gc-7.1.p6.log",
    "asset_url": "tarball://root/attachments/some-uuid/ticket9983/boehm_gc-7.1.p6.log",
    "created_at": "2016-08-30T04:43:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.log",
    "user": "https://github.com/jm58660"
}
```



---

archive/issue_comments_094707.json:
```json
{
    "body": "<a id='comment:1'></a>\nEOL for AIX 5.3 was four years ago, and nobody seems to care about this ticket. So I suggest we close this as wontfix.",
    "created_at": "2016-08-30T04:43:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9983#issuecomment-94707",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:1'></a>
EOL for AIX 5.3 was four years ago, and nobody seems to care about this ticket. So I suggest we close this as wontfix.



---

archive/issue_events_076960.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-08-30T04:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9983#event-76960"
}
```



---

archive/issue_events_076961.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-30T07:14:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9983#event-76961"
}
```



---

archive/issue_events_076962.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-30T07:14:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9983#event-76962"
}
```



---

archive/issue_comments_094708.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2016-08-30T07:14:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9983#issuecomment-94708",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_comments_094709.json:
```json
{
    "body": "<a id='comment:3'></a>\nShould we mass-close all tickets about AIX?",
    "created_at": "2016-08-30T09:58:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9983#issuecomment-94709",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>
Should we mass-close all tickets about AIX?



---

archive/issue_comments_094710.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [jdemeyer](#comment%3A3):\n> Should we mass-close all tickets about AIX?\n\n\nIs anyone using [SageMath](SageMath) on AIX?\n\nAt least #9993 (\"Singular fails to build on AIX 5.3\") has no value, as it just tells an error. But  #9990 (\"Pari fails to build on AIX\") contains some discussion. But OTOH also closed tickets can be searched.",
    "created_at": "2016-08-30T10:06:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9983#issuecomment-94710",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:4'></a>
Replying to [jdemeyer](#comment%3A3):
> Should we mass-close all tickets about AIX?


Is anyone using [SageMath](SageMath) on AIX?

At least #9993 ("Singular fails to build on AIX 5.3") has no value, as it just tells an error. But  #9990 ("Pari fails to build on AIX") contains some discussion. But OTOH also closed tickets can be searched.



---

archive/issue_comments_094711.json:
```json
{
    "body": "<a id='comment:5'></a>\nDetermined to be invalid/duplicate/wontfix (closing as \"wontfix\" as a catch-all resolution).",
    "created_at": "2016-08-30T13:32:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9983#issuecomment-94711",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>
Determined to be invalid/duplicate/wontfix (closing as "wontfix" as a catch-all resolution).



---

archive/issue_events_076963.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9983#event-76963"
}
```



---

archive/issue_events_076964.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9983#event-76964"
}
```



---

archive/issue_events_076965.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9983",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9983#event-76965"
}
```
