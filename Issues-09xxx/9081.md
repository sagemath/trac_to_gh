# Issue 9081: Transistion better between two halves of the developer's walkthrough

archive/issues_009081.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "Discussions at Sage Days 21 suggest that the two halves of the walkthrough (clones, then queues) could be delineated more clearly.  This transistion text does that, and suggests starting with clones and moving to queues when clones start to feel cumbersome.\n\nCC:  @sagetrac-mvngu @williamstein\n\nComponent: **documentation**\n\nAuthor: **Rob Beezer**\n\nReviewer: **Minh Van Nguyen**\n\nMerged: **sage-4.4.4.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9081_\n\n",
    "closed_at": "2010-06-06T00:47:10Z",
    "created_at": "2010-05-29T06:34:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Transistion better between two halves of the developer's walkthrough",
    "type": "issue",
    "updated_at": "2010-06-06T00:47:10Z",
    "url": "https://github.com/sagemath/sage/issues/9081",
    "user": "https://github.com/rbeezer"
}
```
Discussions at Sage Days 21 suggest that the two halves of the walkthrough (clones, then queues) could be delineated more clearly.  This transistion text does that, and suggests starting with clones and moving to queues when clones start to feel cumbersome.

CC:  @sagetrac-mvngu @williamstein

Component: **documentation**

Author: **Rob Beezer**

Reviewer: **Minh Van Nguyen**

Merged: **sage-4.4.4.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/9081_





---

archive/issue_events_115682.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-05-29T06:34:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "milestone_number": null,
    "milestone_title": "sage-4.4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115682"
}
```



---

archive/issue_events_115683.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-05-29T06:34:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115683"
}
```



---

archive/issue_events_115684.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-05-29T06:34:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115684"
}
```



---

archive/issue_events_115685.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-05-29T06:34:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115685"
}
```



---

archive/issue_events_115686.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-05-29T06:34:06Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115686"
}
```



---

archive/issue_events_115687.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-05-29T06:37:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115687"
}
```



---

archive/issue_comments_075193.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_9081-devloper-walkthrough-transition.patch.gz](https://github.com/sagemath/sage/files/ticket9081/trac_9081-devloper-walkthrough-transition.patch.gz)**",
    "created_at": "2010-05-29T06:37:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9081#issuecomment-75193",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_9081-devloper-walkthrough-transition.patch.gz](https://github.com/sagemath/sage/files/ticket9081/trac_9081-devloper-walkthrough-transition.patch.gz)**



---

archive/issue_comments_075194.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2010-05-29T10:52:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9081#issuecomment-75194",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_comments_075195.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good to me.",
    "created_at": "2010-05-29T10:52:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9081#issuecomment-75195",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good to me.



---

archive/issue_events_115688.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-05-29T10:52:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115688"
}
```



---

archive/issue_events_115689.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-05-29T10:52:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115689"
}
```



---

archive/issue_comments_075196.json:
```json
{
    "body": "Merged: **sage-4.4.4.alpha0**",
    "created_at": "2010-06-06T00:47:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9081#issuecomment-75196",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.4.4.alpha0**



---

archive/issue_events_115690.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T00:47:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115690"
}
```



---

archive/issue_events_115691.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T00:47:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9081",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9081#event-115691"
}
```
