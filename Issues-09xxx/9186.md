# Issue 9186: Update R's spkg-install for building multiple spkgs in parallel

archive/issues_009186.json:
```json
{
    "assignees": [],
    "body": "To build R with `SAGE_PARALLEL_SPKG_BUILD=\"yes\"` on Mac OS X, we need to add, e.g.,\n\n```sh\nMAKEFLAGS=\nexport MAKEFLAGS\n```\nto the \"make install\" part of the package's `spkg-install`.\n\nPlease see #8306 about building spkgs in parallel.  For `MAKEFLAGS`, see [the GNU Make manual](http://www.gnu.org/software/make/manual/html_node/Options_002fRecursion.html).\n\nCC:  @sagetrac-drkirkby @jhpalmieri @nexttime\n\nAuthor: **Mitesh Patel**\n\nReviewer: **David Kirkby**\n\nMerged: **sage-4.5.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9186_\n\n",
    "closed_at": "2010-06-25T15:47:23Z",
    "created_at": "2010-06-08T08:42:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update R's spkg-install for building multiple spkgs in parallel",
    "type": "issue",
    "updated_at": "2010-06-25T15:47:23Z",
    "url": "https://github.com/sagemath/sage/issues/9186",
    "user": "https://github.com/qed777"
}
```
To build R with `SAGE_PARALLEL_SPKG_BUILD="yes"` on Mac OS X, we need to add, e.g.,

```sh
MAKEFLAGS=
export MAKEFLAGS
```
to the "make install" part of the package's `spkg-install`.

Please see #8306 about building spkgs in parallel.  For `MAKEFLAGS`, see [the GNU Make manual](http://www.gnu.org/software/make/manual/html_node/Options_002fRecursion.html).

CC:  @sagetrac-drkirkby @jhpalmieri @nexttime

Author: **Mitesh Patel**

Reviewer: **David Kirkby**

Merged: **sage-4.5.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/9186_





---

archive/issue_events_106946.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-06-08T08:42:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106946"
}
```



---

archive/issue_events_106947.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-06-08T08:42:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106947"
}
```



---

archive/issue_events_106948.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-06-08T08:42:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106948"
}
```



---

archive/issue_events_106949.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-06-08T08:42:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106949"
}
```



---

archive/issue_comments_078626.json:
```json
{
    "body": "spkg patch.  Set empty `MAKEFLAGS` for installation.",
    "created_at": "2010-06-09T02:22:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9186#issuecomment-78626",
    "user": "https://github.com/qed777"
}
```

spkg patch.  Set empty `MAKEFLAGS` for installation.



---

archive/issue_events_106950.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-06-09T02:25:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106950"
}
```



---

archive/issue_comments_078627.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_9186-r_makeflags.patch.gz](https://github.com/sagemath/sage/files/ticket9186/trac_9186-r_makeflags.patch.gz)**\n\nI've put a new spkg at\n\n* http://sage.math.washington.edu/home/mpatel/trac/9186/r-2.10.1.p2.spkg",
    "created_at": "2010-06-09T02:25:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9186#issuecomment-78627",
    "user": "https://github.com/qed777"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_9186-r_makeflags.patch.gz](https://github.com/sagemath/sage/files/ticket9186/trac_9186-r_makeflags.patch.gz)**

I've put a new spkg at

* http://sage.math.washington.edu/home/mpatel/trac/9186/r-2.10.1.p2.spkg



---

archive/issue_comments_078628.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nPositive review. It is extreamly simple, looks good and I've tested it on Solaris 10 on an old Sun Blade 2000 SPARC. \n\n```\nreal    21m53.863s\nuser    17m29.388s\nsys     3m22.428s\nSuccessfully installed r-2.10.1.p2\nNow cleaning up tmp files.\nrm: Cannot remove any directory in the path of the current working directory\n/export/home/drkirkby/sage-4.4.4/spkg/build/r-2.10.1.p2\nMaking Sage/Python scripts relocatable...\nMaking script relocatable\nFinished installing r-2.10.1.p2.spkg\n```\n\n**I really hope your code for building packages in parallel gets into Sage asap. It could make a huge difference to build times. I'll see if I can get some interest in pushing this up the priority list!**",
    "created_at": "2010-06-24T17:00:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9186#issuecomment-78628",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:2" align="right">Comment 2</div>

Positive review. It is extreamly simple, looks good and I've tested it on Solaris 10 on an old Sun Blade 2000 SPARC. 

```
real    21m53.863s
user    17m29.388s
sys     3m22.428s
Successfully installed r-2.10.1.p2
Now cleaning up tmp files.
rm: Cannot remove any directory in the path of the current working directory
/export/home/drkirkby/sage-4.4.4/spkg/build/r-2.10.1.p2
Making Sage/Python scripts relocatable...
Making script relocatable
Finished installing r-2.10.1.p2.spkg
```

**I really hope your code for building packages in parallel gets into Sage asap. It could make a huge difference to build times. I'll see if I can get some interest in pushing this up the priority list!**



---

archive/issue_events_106951.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-06-24T17:00:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106951"
}
```



---

archive/issue_events_106952.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-06-24T17:00:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106952"
}
```



---

archive/issue_events_106953.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-25T15:47:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106953"
}
```



---

archive/issue_events_106954.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-25T15:47:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106954"
}
```



---

archive/issue_comments_078629.json:
```json
{
    "body": "Merged: **sage-4.5.alpha0**",
    "created_at": "2010-06-25T15:47:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9186#issuecomment-78629",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.5.alpha0**



---

archive/issue_comments_078630.json:
```json
{
    "body": "Reviewer: **David Kirkby**",
    "created_at": "2010-06-25T15:47:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9186#issuecomment-78630",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **David Kirkby**



---

archive/issue_events_106955.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-25T15:47:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106955"
}
```



---

archive/issue_events_106956.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-25T15:47:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9186",
    "milestone_number": null,
    "milestone_title": "sage-4.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9186#event-106956"
}
```
