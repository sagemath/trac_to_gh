# Issue 9137: Infinite konqueror windows when opening notebook

archive/issues_009137.json:
```json
{
    "assignees": [],
    "body": "This is to make sage users aware of a bug that apparently exists in KDE and not in SAGE.\n\n**Behavior:**  On certain system setups, when starting SAGE notebook, konqueror goes haywire and starts opening up an infinite number of windows in the taskbar (none of the windows will show themselves).  Konqueror will continue to open new windows until Xorg is killed.\n\n**References:** [https://bugs.kde.org/show_bug.cgi?id=234620](https://bugs.kde.org/show_bug.cgi?id=234620) (includes screenshots of the problem)\n\n**Workaround:** 1) use ` notebook(open_viewer=False) ` and manually open browser to notebook port  2) Use a browser other than konqueror (you can change the defualt browser in KDE system settings).\n\n**Conclusion:** As stated above, this appears to be a bug in KDE as it affects other software as well.  If you see this bug, please contact the KDE developers.\n\nCC:  @jasongrout @mwhansen @williamstein\n\nKeywords: **konqueror, infinite, windows**\n\nComponent: **notebook**\n\nAuthor: **ryan**\n\nUpstream: **Fixed upstream, in a later stable release.**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9137_\n\n",
    "closed_at": "2010-08-28T19:24:33Z",
    "created_at": "2010-06-03T23:45:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Infinite konqueror windows when opening notebook",
    "type": "issue",
    "updated_at": "2010-08-28T19:24:33Z",
    "url": "https://github.com/sagemath/sage/issues/9137",
    "user": "https://github.com/sagetrac-ryan"
}
```
This is to make sage users aware of a bug that apparently exists in KDE and not in SAGE.

**Behavior:**  On certain system setups, when starting SAGE notebook, konqueror goes haywire and starts opening up an infinite number of windows in the taskbar (none of the windows will show themselves).  Konqueror will continue to open new windows until Xorg is killed.

**References:** [https://bugs.kde.org/show_bug.cgi?id=234620](https://bugs.kde.org/show_bug.cgi?id=234620) (includes screenshots of the problem)

**Workaround:** 1) use ` notebook(open_viewer=False) ` and manually open browser to notebook port  2) Use a browser other than konqueror (you can change the defualt browser in KDE system settings).

**Conclusion:** As stated above, this appears to be a bug in KDE as it affects other software as well.  If you see this bug, please contact the KDE developers.

CC:  @jasongrout @mwhansen @williamstein

Keywords: **konqueror, infinite, windows**

Component: **notebook**

Author: **ryan**

Upstream: **Fixed upstream, in a later stable release.**

_Issue created by migration from https://trac.sagemath.org/ticket/9137_





---

archive/issue_events_116597.json:
```json
{
    "actor": "https://github.com/sagetrac-ryan",
    "created_at": "2010-06-03T23:45:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "milestone_number": null,
    "milestone_title": "sage-4.5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116597"
}
```



---

archive/issue_events_116598.json:
```json
{
    "actor": "https://github.com/sagetrac-ryan",
    "created_at": "2010-06-03T23:45:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116598"
}
```



---

archive/issue_events_116599.json:
```json
{
    "actor": "https://github.com/sagetrac-ryan",
    "created_at": "2010-06-03T23:45:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116599"
}
```



---

archive/issue_events_116600.json:
```json
{
    "actor": "https://github.com/sagetrac-ryan",
    "created_at": "2010-06-03T23:45:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116600"
}
```



---

archive/issue_events_116601.json:
```json
{
    "actor": "https://github.com/sagetrac-ryan",
    "created_at": "2010-06-03T23:46:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116601"
}
```



---

archive/issue_comments_076076.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nthis bug is closed in KDE 4.5",
    "created_at": "2010-08-28T18:24:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9137#issuecomment-76076",
    "user": "https://github.com/sagetrac-ryan"
}
```

<div id="comment:3" align="right">comment:3</div>

this bug is closed in KDE 4.5



---

archive/issue_comments_076077.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\n[https://bugs.kde.org/show_bug.cgi?id=240677](https://bugs.kde.org/show_bug.cgi?id=240677)\n\nHere is the KDE bug ticket.",
    "created_at": "2010-08-28T18:25:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9137#issuecomment-76077",
    "user": "https://github.com/sagetrac-ryan"
}
```

<div id="comment:4" align="right">comment:4</div>

[https://bugs.kde.org/show_bug.cgi?id=240677](https://bugs.kde.org/show_bug.cgi?id=240677)

Here is the KDE bug ticket.



---

archive/issue_comments_076078.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI'm not sure who the current release manager is, but this ticket should be closed (see comments above).",
    "created_at": "2010-08-28T19:20:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9137#issuecomment-76078",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:5" align="right">comment:5</div>

I'm not sure who the current release manager is, but this ticket should be closed (see comments above).



---

archive/issue_events_116602.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-08-28T19:24:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116602"
}
```



---

archive/issue_events_116603.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-08-28T19:24:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116603"
}
```



---

archive/issue_events_116604.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-08-28T19:24:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "milestone_number": null,
    "milestone_title": "sage-4.5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116604"
}
```



---

archive/issue_events_116605.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-08-28T19:24:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116605"
}
```



---

archive/issue_events_116606.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-08-28T19:24:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9137#event-116606"
}
```



---

archive/issue_comments_076079.json:
```json
{
    "body": "Changed upstream from **Workaround found; Bug reported upstream.** to **Fixed upstream, in a later stable release.**",
    "created_at": "2010-08-28T19:24:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9137",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9137#issuecomment-76079",
    "user": "https://github.com/mwhansen"
}
```

Changed upstream from **Workaround found; Bug reported upstream.** to **Fixed upstream, in a later stable release.**
