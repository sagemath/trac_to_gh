# Issue 9014: Implement a repr_prod

archive/issues_009014.json:
```json
{
    "assignees": [],
    "body": "As for linear combinaison, implement a method in misc 'repr_prod' to represent products. \n\n```\ndef repr_prodcomb(symbols, coeffs, is_latex=False):\n    r\"\"\"\n    Compute a string representation of a product combination of some\n    formal symbols\n    \"\"\"\n    bla bla bla\n\nsage: repr_prodcomb(['a','b','c'], [1,-2,-3])\n'a * b^-2 * c^-3'\nsage: repr_prodcomb([],[])\n'1'\n```\n\nThere is already a such function in sage/rings/polynomial/polynomial_element_generic.py\n\nWith categories will come FreeGroups(Gens), FreeMonoid(Gens), Free... Such futurs features motivate the need of this function.\n\nAssignee: **@jasongrout**\n\nCC:  @nthiery\n\nKeywords: **repr product**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9014_\n\n",
    "created_at": "2010-05-22T09:29:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement a repr_prod",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/9014",
    "user": "https://github.com/sagetrac-nborie"
}
```
As for linear combinaison, implement a method in misc 'repr_prod' to represent products. 

```
def repr_prodcomb(symbols, coeffs, is_latex=False):
    r"""
    Compute a string representation of a product combination of some
    formal symbols
    """
    bla bla bla

sage: repr_prodcomb(['a','b','c'], [1,-2,-3])
'a * b^-2 * c^-3'
sage: repr_prodcomb([],[])
'1'
```

There is already a such function in sage/rings/polynomial/polynomial_element_generic.py

With categories will come FreeGroups(Gens), FreeMonoid(Gens), Free... Such futurs features motivate the need of this function.

Assignee: **@jasongrout**

CC:  @nthiery

Keywords: **repr product**

_Issue created by migration from https://trac.sagemath.org/ticket/9014_





---

archive/issue_events_104417.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-05-22T09:29:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104417"
}
```



---

archive/issue_events_104418.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-05-22T09:29:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104418"
}
```



---

archive/issue_events_104419.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-05-22T09:29:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104419"
}
```



---

archive/issue_comments_076104.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nFeel free to sugest a better name (still sorry for my english...) and a better description.",
    "created_at": "2010-05-22T09:30:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9014#issuecomment-76104",
    "user": "https://github.com/sagetrac-nborie"
}
```

<div id="comment:1" align="right">Comment 1</div>

Feel free to sugest a better name (still sorry for my english...) and a better description.



---

archive/issue_events_104420.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104420"
}
```



---

archive/issue_events_104421.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104421"
}
```



---

archive/issue_events_104422.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104422"
}
```



---

archive/issue_events_104423.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104423"
}
```



---

archive/issue_events_104424.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104424"
}
```



---

archive/issue_events_104425.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104425"
}
```



---

archive/issue_events_104426.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104426"
}
```



---

archive/issue_events_104427.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104427"
}
```



---

archive/issue_events_104428.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9014",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9014#event-104428"
}
```
