# Issue 9145: Expand math symbols available in documentation, remove doc/common/macros.tex

archive/issues_009145.json:
```json
{
    "body": "See discussion at\n\nhttp://groups.google.com/group/sage-devel/browse_thread/thread/d62ea229829048f7\n\n**Assignee:** mvngu\n\n**CC:**  @jhpalmieri\n\n**Author:** Rob Beezer\n\n**Reviewer:** Minh Van Nguyen\n\n**Merged:** sage-4.4.4.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/9145\n\n",
    "closed_at": "2010-06-06T00:44:00Z",
    "created_at": "2010-06-05T04:03:42Z",
    "labels": [
        "component: documentation",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.4",
    "title": "Expand math symbols available in documentation, remove doc/common/macros.tex",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9145",
    "user": "https://github.com/rbeezer"
}
```
See discussion at

http://groups.google.com/group/sage-devel/browse_thread/thread/d62ea229829048f7

**Assignee:** mvngu

**CC:**  @jhpalmieri

**Author:** Rob Beezer

**Reviewer:** Minh Van Nguyen

**Merged:** sage-4.4.4.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/9145





---

archive/issue_comments_078089.json:
```json
{
    "body": "**Author:** Rob Beezer",
    "created_at": "2010-06-05T04:14:02Z",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9145#issuecomment-78089",
    "user": "https://github.com/rbeezer"
}
```

**Author:** Rob Beezer



---

archive/issue_events_066625.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-06-05T04:14:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9145#event-66625"
}
```



---

archive/attachments_011540.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_9145_math_symbols_docs.patch",
    "asset_url": "tarball://root/attachments/ticket9145/trac_9145_math_symbols_docs.patch",
    "created_at": "2010-06-05T04:14:02Z",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket9145/trac_9145_math_symbols_docs.patch",
    "user": "https://github.com/rbeezer"
}
```



---

archive/issue_comments_078090.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_9145_math_symbols_docs.patch](https://github.com/sagemath/sage/files/ticket9145/trac_9145_math_symbols_docs.patch) by @rbeezer created at 2010-06-05 04:14:02\n\nPatch removes doc/common/macros.tex, replaces amsfonts by amssymb in latex preamble string of doc/common/conf.py.\n\n4.4.3.alpha3 HTMl and PDF documentation all build without halting when this patch is applied and a limited survey indicates they look good as well.",
    "created_at": "2010-06-05T04:14:02Z",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9145#issuecomment-78090",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:1'></a>
Attachment [trac_9145_math_symbols_docs.patch](https://github.com/sagemath/sage/files/ticket9145/trac_9145_math_symbols_docs.patch) by @rbeezer created at 2010-06-05 04:14:02

Patch removes doc/common/macros.tex, replaces amsfonts by amssymb in latex preamble string of doc/common/conf.py.

4.4.3.alpha3 HTMl and PDF documentation all build without halting when this patch is applied and a limited survey indicates they look good as well.



---

archive/issue_comments_078091.json:
```json
{
    "body": "**Reviewer:** Minh Van Nguyen",
    "created_at": "2010-06-05T21:44:52Z",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9145#issuecomment-78091",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Reviewer:** Minh Van Nguyen



---

archive/issue_events_066626.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-06-05T21:44:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9145#event-66626"
}
```



---

archive/issue_events_066627.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-06-05T21:44:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9145#event-66627"
}
```



---

archive/issue_comments_078092.json:
```json
{
    "body": "<a id='comment:2'></a>\nWithout the patch, when building the PDF version of the reference manual for Sage 4.4.3, I got:\n\n```\n! Undefined control sequence.\nl.217693 ...als}(\\mathrm{pos}_{i+1})]_{1 \\leqslant \n                                                   i \\leqslant n}$\n```\nNote that the HTML version built OK. With the patch, the PDF version built successfully on these machines:\n\n* sage.math, Ubuntu 8.04.4 LTS, with latex and pdflatex\n* bsd.math, Mac OS X 10.6.3, with latex and pdflatex\n\nBut failed on these machines:\n\n* eno.skynet, Fedora 12, no latex or pdflatex\n* rh.math, Ubuntu 10.04 LTS, no latex or pdflatex\n* rosemary.math, Red Hat Enterprise Linux Server 5.5, with latex and pdflatex, but I got the following error when building the PDF version:\n\n ```\n ! LaTeX Error: File `utf8x.def' not found.\n ```\n This also happens without the patch. The issue is more likely due to the LaTeX installation on rosemary.math.\n\nAs for `doc/common/macros.tex`, that file was during the days when Sage's documentation followed how Python did it. Since switching over to Sphinx, we don't need that file any more.",
    "created_at": "2010-06-05T21:44:52Z",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9145#issuecomment-78092",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:2'></a>
Without the patch, when building the PDF version of the reference manual for Sage 4.4.3, I got:

```
! Undefined control sequence.
l.217693 ...als}(\mathrm{pos}_{i+1})]_{1 \leqslant 
                                                   i \leqslant n}$
```
Note that the HTML version built OK. With the patch, the PDF version built successfully on these machines:

* sage.math, Ubuntu 8.04.4 LTS, with latex and pdflatex
* bsd.math, Mac OS X 10.6.3, with latex and pdflatex

But failed on these machines:

* eno.skynet, Fedora 12, no latex or pdflatex
* rh.math, Ubuntu 10.04 LTS, no latex or pdflatex
* rosemary.math, Red Hat Enterprise Linux Server 5.5, with latex and pdflatex, but I got the following error when building the PDF version:

 ```
 ! LaTeX Error: File `utf8x.def' not found.
 ```
 This also happens without the patch. The issue is more likely due to the LaTeX installation on rosemary.math.

As for `doc/common/macros.tex`, that file was during the days when Sage's documentation followed how Python did it. Since switching over to Sphinx, we don't need that file any more.



---

archive/issue_comments_078093.json:
```json
{
    "body": "**Merged:** sage-4.4.4.alpha0",
    "created_at": "2010-06-06T00:44:00Z",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9145#issuecomment-78093",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.4.4.alpha0



---

archive/issue_events_066628.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T00:44:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9145#event-66628"
}
```



---

archive/issue_events_066629.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T00:44:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9145",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9145#event-66629"
}
```
