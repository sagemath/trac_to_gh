# Issue 9851: sage -pkg may hang if there are many uncommitted changes in the package

archive/issues_009850.json:
```json
{
    "assignees": [],
    "body": "At [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/c008a74ae2b3af95), I reported the following problem.\n\n- Take a folder with many uncommitted changes in a Mercurial repository. [Example](http://www.google.com/url?sa=D&q=http://sage.math.washington.edu/home/SimonKing/Cohomology/p_group_cohomology-2.1.alpha0.tar.gz&usg=AFQjCNE362g2kL77GzI-7T5RaOBx92X0KQ).\n- Run `sage -pkg` on it.\n- It will hang forever while it is attempted to read from `stderr` of a subprocess.\n\nMitesh Patel pointed to the solution: One should use `communicate` rather than stdout/stdin/stderr!\n\nI don't know how to doctest the issue; but I have a patch that solves the problem.\n\nCC:  @qed777\n\nKeywords: **sage-pkg, stderr hangs**\n\nAuthor: **Simon King**\n\nReviewer: **Mitesh Patel**\n\nMerged: **sage-4.6.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9851_\n\n",
    "closed_at": "2010-09-15T11:13:07Z",
    "created_at": "2010-09-03T09:51:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage -pkg may hang if there are many uncommitted changes in the package",
    "type": "issue",
    "updated_at": "2010-09-15T11:13:07Z",
    "url": "https://github.com/sagemath/sage/issues/9851",
    "user": "https://github.com/simon-king-jena"
}
```
At [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/c008a74ae2b3af95), I reported the following problem.

- Take a folder with many uncommitted changes in a Mercurial repository. [Example](http://www.google.com/url?sa=D&q=http://sage.math.washington.edu/home/SimonKing/Cohomology/p_group_cohomology-2.1.alpha0.tar.gz&usg=AFQjCNE362g2kL77GzI-7T5RaOBx92X0KQ).
- Run `sage -pkg` on it.
- It will hang forever while it is attempted to read from `stderr` of a subprocess.

Mitesh Patel pointed to the solution: One should use `communicate` rather than stdout/stdin/stderr!

I don't know how to doctest the issue; but I have a patch that solves the problem.

CC:  @qed777

Keywords: **sage-pkg, stderr hangs**

Author: **Simon King**

Reviewer: **Mitesh Patel**

Merged: **sage-4.6.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/9851_





---

archive/issue_events_116579.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2010-09-03T09:51:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9851#event-116579"
}
```



---

archive/issue_events_116580.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2010-09-03T09:51:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9851#event-116580"
}
```



---

archive/issue_events_116581.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2010-09-03T09:51:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9851#event-116581"
}
```



---

archive/issue_events_116582.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2010-09-03T09:51:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9851#event-116582"
}
```



---

archive/issue_comments_089973.json:
```json
{
    "body": "Replace p.stderr.read()/p.stdout.read() by p.communicate() in the sage-pkg script",
    "created_at": "2010-09-03T09:55:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9851#issuecomment-89973",
    "user": "https://github.com/simon-king-jena"
}
```

Replace p.stderr.read()/p.stdout.read() by p.communicate() in the sage-pkg script



---

archive/issue_events_116583.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2010-09-03T09:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9851#event-116583"
}
```



---

archive/issue_comments_089974.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[9851-sage-pkg.patch.gz](https://github.com/sagemath/sage/files/ticket9851/9851-sage-pkg.patch.gz)**",
    "created_at": "2010-09-03T09:57:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9851#issuecomment-89974",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[9851-sage-pkg.patch.gz](https://github.com/sagemath/sage/files/ticket9851/9851-sage-pkg.patch.gz)**



---

archive/issue_events_116584.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-09-03T23:07:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9851#event-116584"
}
```



---

archive/issue_events_116585.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-09-03T23:07:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9851#event-116585"
}
```



---

archive/issue_comments_089975.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks for tracking this down and fixing it!",
    "created_at": "2010-09-03T23:07:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9851#issuecomment-89975",
    "user": "https://github.com/qed777"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks for tracking this down and fixing it!



---

archive/issue_comments_089976.json:
```json
{
    "body": "Reviewer: **Mitesh Patel**",
    "created_at": "2010-09-03T23:07:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9851#issuecomment-89976",
    "user": "https://github.com/qed777"
}
```

Reviewer: **Mitesh Patel**



---

archive/issue_comments_089977.json:
```json
{
    "body": "Merged: **sage-4.6.alpha1**",
    "created_at": "2010-09-15T11:13:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9851#issuecomment-89977",
    "user": "https://github.com/qed777"
}
```

Merged: **sage-4.6.alpha1**



---

archive/issue_events_116586.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-09-15T11:13:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9851#event-116586"
}
```



---

archive/issue_events_116587.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-09-15T11:13:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9851",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9851#event-116587"
}
```
