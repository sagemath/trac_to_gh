# Issue 9512: Sage Source Editor

archive/issues_009512.json:
```json
{
    "assignees": [],
    "body": "Add an \"Edit this page\" link at the bottom of source files (/src/...) where one can edit the file.\n\nOnly `user_type(username) == 'admin'` can edit the files.\n\nAssignee: **@jasongrout, @williamstein**\n\nCC:  boothby acleone cwitty @fchapoton\n\nReviewer: **Alex Leone**\n\nAuthor: **Tom Boothby**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9512_\n\n",
    "closed_at": "2020-09-11T10:41:57Z",
    "created_at": "2010-07-15T20:07:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Sage Source Editor",
    "type": "issue",
    "updated_at": "2020-09-11T10:41:57Z",
    "url": "https://github.com/sagemath/sage/issues/9512",
    "user": "https://github.com/sagetrac-acleone"
}
```
Add an "Edit this page" link at the bottom of source files (/src/...) where one can edit the file.

Only `user_type(username) == 'admin'` can edit the files.

Assignee: **@jasongrout, @williamstein**

CC:  boothby acleone cwitty @fchapoton

Reviewer: **Alex Leone**

Author: **Tom Boothby**

_Issue created by migration from https://trac.sagemath.org/ticket/9512_





---

archive/issue_events_108033.json:
```json
{
    "actor": "https://github.com/sagetrac-acleone",
    "created_at": "2010-07-15T20:07:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108033"
}
```



---

archive/issue_events_108034.json:
```json
{
    "actor": "https://github.com/sagetrac-acleone",
    "created_at": "2010-07-15T20:07:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108034"
}
```



---

archive/issue_events_108035.json:
```json
{
    "actor": "https://github.com/sagetrac-acleone",
    "created_at": "2010-07-15T20:07:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108035"
}
```



---

archive/issue_comments_084139.json:
```json
{
    "body": "Attachment: **[9512_source_editor.patch.gz](https://github.com/sagemath/sage/files/ticket9512/9512_source_editor.patch.gz)**",
    "created_at": "2010-07-15T20:08:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9512#issuecomment-84139",
    "user": "https://github.com/sagetrac-boothby"
}
```

Attachment: **[9512_source_editor.patch.gz](https://github.com/sagemath/sage/files/ticket9512/9512_source_editor.patch.gz)**



---

archive/issue_events_108036.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2010-07-15T20:08:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108036"
}
```



---

archive/issue_events_108037.json:
```json
{
    "actor": "https://github.com/sagetrac-acleone",
    "created_at": "2010-07-15T20:10:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108037"
}
```



---

archive/issue_events_108038.json:
```json
{
    "actor": "https://github.com/sagetrac-acleone",
    "created_at": "2010-07-15T20:10:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108038"
}
```



---

archive/issue_comments_084140.json:
```json
{
    "body": "Reviewer: **Alex Leone**",
    "created_at": "2010-07-15T20:10:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9512#issuecomment-84140",
    "user": "https://github.com/sagetrac-acleone"
}
```

Reviewer: **Alex Leone**



---

archive/issue_comments_084141.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI'm added the commit string\n\n```\n#9512: Sage source editor.  Tom Boothby\n```\nfor the patch I'm merging into SageNB 0.8.2 (#9572).",
    "created_at": "2010-07-23T06:59:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9512#issuecomment-84141",
    "user": "https://github.com/qed777"
}
```

<a id='comment:3'>Comment 3:</a>
I'm added the commit string

```
#9512: Sage source editor.  Tom Boothby
```
for the patch I'm merging into SageNB 0.8.2 (#9572).



---

archive/issue_comments_084142.json:
```json
{
    "body": "Merged: **sagenb-0.8.2**",
    "created_at": "2010-07-23T07:18:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9512#issuecomment-84142",
    "user": "https://github.com/qed777"
}
```

Merged: **sagenb-0.8.2**



---

archive/issue_events_108039.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-23T07:18:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108039"
}
```



---

archive/issue_events_108040.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-23T07:18:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108040"
}
```



---

archive/issue_comments_084143.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\n[comment:ticket:9572:6 C. Witty's comment] at #9572:\n> Actually, I found a bug: the \"source editor\" feature (#9512) converts line endings from Unix to DOS (so once you've edited the file, mercurial thinks every line has changed).\n\n> Given the total non-discoverability of #9512, I'm not sure this bug is worth holding up the new spkg; I'll let somebody else decide that.\n\nI also see this behavior.  I'm reopening this ticket, changing its status to \"needs work,\" and removing it, for now, from #9572's SageNB 0.8.2.",
    "created_at": "2010-07-25T07:28:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9512#issuecomment-84143",
    "user": "https://github.com/qed777"
}
```

<a id='comment:5'>Comment 5:</a>
[comment:ticket:9572:6 C. Witty's comment] at #9572:
> Actually, I found a bug: the "source editor" feature (#9512) converts line endings from Unix to DOS (so once you've edited the file, mercurial thinks every line has changed).

> Given the total non-discoverability of #9512, I'm not sure this bug is worth holding up the new spkg; I'll let somebody else decide that.

I also see this behavior.  I'm reopening this ticket, changing its status to "needs work," and removing it, for now, from #9572's SageNB 0.8.2.



---

archive/issue_comments_084144.json:
```json
{
    "body": "Changed merged from **sagenb-0.8.2** to none",
    "created_at": "2010-07-25T07:28:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9512#issuecomment-84144",
    "user": "https://github.com/qed777"
}
```

Changed merged from **sagenb-0.8.2** to none



---

archive/issue_events_108041.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-25T07:28:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108041"
}
```



---

archive/issue_events_108042.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108042"
}
```



---

archive/issue_events_108043.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108043"
}
```



---

archive/issue_events_108044.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108044"
}
```



---

archive/issue_events_108045.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108045"
}
```



---

archive/issue_events_108046.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108046"
}
```



---

archive/issue_events_108047.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108047"
}
```



---

archive/issue_events_108048.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108048"
}
```



---

archive/issue_events_108049.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108049"
}
```



---

archive/issue_comments_084145.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nProposing to close all sagenb tickets as outdated, so that all remaining open tickets in the notebook component are about the Jupyter notebook.",
    "created_at": "2020-08-18T00:36:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9512#issuecomment-84145",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:13'>Comment 13:</a>
Proposing to close all sagenb tickets as outdated, so that all remaining open tickets in the notebook component are about the Jupyter notebook.



---

archive/issue_events_108050.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108050"
}
```



---

archive/issue_events_108051.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108051"
}
```



---

archive/issue_events_108052.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108052"
}
```



---

archive/issue_events_108053.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-11T10:41:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108053"
}
```



---

archive/issue_events_108054.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-11T10:41:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9512",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9512#event-108054"
}
```
