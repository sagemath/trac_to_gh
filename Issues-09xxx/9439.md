# Issue 9439: hyperbolic geometry

archive/issues_009439.json:
```json
{
    "assignees": [],
    "body": "Implementation of two conformal models of hyperbolic geometry (half plane, disc) and actions of their isometry groups.\n\nThe actual file is almost complete for working with the hyperbolic plane as the following will plot a hyperbolic triangle\n\n```\nsage: HH\nHyperbolic half plane\nsage: HH(0)\nBoundary point 0\nsage: p = HH.polygon([CC(0), CC(1), CC(2,2)])\nsage: p.plot(face_color='red').show(aspect_ratio=1)\n```\nThere are more examples in the file.\n\nApply:\n* [attachment: trac_9439-hyperbolic_geometry.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry.patch.gz)\n* [attachment: trac_9439-hyperbolic_geometry_review_fc.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry_review_fc.patch.gz)\n\nAssignee: **@videlec**\n\nCC:  @kcrisman @pjbruin\n\nKeywords: **hyperbolic geometry, Poincare disc, upper half plane, Beltrami-Klein, hyperboloid model, sd35, days64**\n\nBranch: **[82cf89e](https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94)**\n\nReviewer: **Johan Bosman, Travis Scrimshaw, Greg Laun, Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Vincent Delecroix, Martin Raum, Greg Laun, Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9439_\n\n",
    "closed_at": "2015-03-19T03:17:35Z",
    "created_at": "2010-07-06T16:13:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "hyperbolic geometry",
    "type": "issue",
    "updated_at": "2015-03-20T21:16:41Z",
    "url": "https://github.com/sagemath/sage/issues/9439",
    "user": "https://github.com/videlec"
}
```
Implementation of two conformal models of hyperbolic geometry (half plane, disc) and actions of their isometry groups.

The actual file is almost complete for working with the hyperbolic plane as the following will plot a hyperbolic triangle

```
sage: HH
Hyperbolic half plane
sage: HH(0)
Boundary point 0
sage: p = HH.polygon([CC(0), CC(1), CC(2,2)])
sage: p.plot(face_color='red').show(aspect_ratio=1)
```
There are more examples in the file.

Apply:
* [attachment: trac_9439-hyperbolic_geometry.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry.patch.gz)
* [attachment: trac_9439-hyperbolic_geometry_review_fc.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry_review_fc.patch.gz)

Assignee: **@videlec**

CC:  @kcrisman @pjbruin

Keywords: **hyperbolic geometry, Poincare disc, upper half plane, Beltrami-Klein, hyperboloid model, sd35, days64**

Branch: **[82cf89e](https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94)**

Reviewer: **Johan Bosman, Travis Scrimshaw, Greg Laun, Frédéric Chapoton**

Author: **Vincent Delecroix, Martin Raum, Greg Laun, Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/9439_





---

archive/issue_events_106974.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-07-06T16:13:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106974"
}
```



---

archive/issue_events_106975.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-07-06T16:13:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106975"
}
```



---

archive/issue_events_106976.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-07-06T16:13:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106976"
}
```



---

archive/issue_events_106977.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-07-06T16:13:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106977"
}
```



---

archive/issue_comments_083025.json:
```json
{
    "body": "Attachment: **[trac_9439-hyperbolic geometry.py.gz](https://github.com/sagemath/sage/files/ticket9439/f41e5ddfa81119d3ad5857e009b9847d.gz)**\n\ndraft of hyperbolic geometry space",
    "created_at": "2010-07-06T16:16:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83025",
    "user": "https://github.com/videlec"
}
```

Attachment: **[trac_9439-hyperbolic geometry.py.gz](https://github.com/sagemath/sage/files/ticket9439/f41e5ddfa81119d3ad5857e009b9847d.gz)**

draft of hyperbolic geometry space



---

archive/issue_comments_083026.json:
```json
{
    "body": "Changed assignee from **mhampton** to **@videlec**",
    "created_at": "2010-07-06T16:17:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83026",
    "user": "https://github.com/videlec"
}
```

Changed assignee from **mhampton** to **@videlec**



---

archive/issue_comments_083027.json:
```json
{
    "body": "Attachment: **[trac_9439-hyperbolic_geometry.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry.patch.gz)**\n\nthis patch contains the previous one",
    "created_at": "2010-07-20T17:58:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83027",
    "user": "https://github.com/videlec"
}
```

Attachment: **[trac_9439-hyperbolic_geometry.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry.patch.gz)**

this patch contains the previous one



---

archive/issue_comments_083028.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,9 +1,14 @@\n-Implementation of three conformal models for hyperbolic geometry (half plane, disc, hyperboloid) with actions of their isometry groups.\n+Implementation of two conformal models of hyperbolic geometry (half plane, disc) and actions of their isometry groups.\n \n The actual file is almost complete for working with the hyperbolic plane as the following will plot a hyperbolic triangle\n \n ```\n-sage: HH.polygon(CC(0), CC(1), CC(2,2)).plot(face_color='red')\n+sage: HH\n+Hyperbolic half plane\n+sage: HH(0)\n+Boundary point 0\n+sage: p = HH.polygon([CC(0), CC(1), CC(2,2)])\n+sage: p.plot(face_color='red').show(aspect_ratio=1)\n ```\n There are more examples in the file.\n \n``````\n",
    "created_at": "2010-07-20T18:00:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83028",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,9 +1,14 @@
-Implementation of three conformal models for hyperbolic geometry (half plane, disc, hyperboloid) with actions of their isometry groups.
+Implementation of two conformal models of hyperbolic geometry (half plane, disc) and actions of their isometry groups.
 
 The actual file is almost complete for working with the hyperbolic plane as the following will plot a hyperbolic triangle
 
 ```
-sage: HH.polygon(CC(0), CC(1), CC(2,2)).plot(face_color='red')
+sage: HH
+Hyperbolic half plane
+sage: HH(0)
+Boundary point 0
+sage: p = HH.polygon([CC(0), CC(1), CC(2,2)])
+sage: p.plot(face_color='red').show(aspect_ratio=1)
 ```
 There are more examples in the file.
 
``````




---

archive/issue_comments_083029.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\n*ping* :)\n\nAre you planning on finishing this?  It would be very good to have an upper half plane implementation.  \n\nThere are a few things that need to be improved though.  Accessing attributes directly (e.g. with spam._value) is not good.  Please use accessor methods instead (i.e. a method named value() that returns _value); this improves the separation of interface and implementation.\n\nNear the real line, the hyperbolic distance becomes become HUGE compared to the Euclidean distance.  Representing a point as a complex number thus leads to numeric instability.  It is therefore better to implement a point by a pair of a matrix ((a,b),(c,d)) and a complex number z (thus representing (az+b)/(cz+d)).",
    "created_at": "2011-12-10T13:34:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83029",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

<a id='comment:3'>Comment 3:</a>
*ping* :)

Are you planning on finishing this?  It would be very good to have an upper half plane implementation.  

There are a few things that need to be improved though.  Accessing attributes directly (e.g. with spam._value) is not good.  Please use accessor methods instead (i.e. a method named value() that returns _value); this improves the separation of interface and implementation.

Near the real line, the hyperbolic distance becomes become HUGE compared to the Euclidean distance.  Representing a point as a complex number thus leads to numeric instability.  It is therefore better to implement a point by a pair of a matrix ((a,b),(c,d)) and a complex number z (thus representing (az+b)/(cz+d)).



---

archive/issue_comments_083030.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,8 +11,3 @@\n sage: p.plot(face_color='red').show(aspect_ratio=1)\n ```\n There are more examples in the file.\n-\n-\n-Depandancy:\n-\n-* #9076: plot arc of circles\n``````\n",
    "created_at": "2011-12-10T13:34:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83030",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,8 +11,3 @@
 sage: p.plot(face_color='red').show(aspect_ratio=1)
 ```
 There are more examples in the file.
-
-
-Depandancy:
-
-* #9076: plot arc of circles
``````




---

archive/issue_events_106978.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-10T13:34:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106978"
}
```



---

archive/issue_comments_083031.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\n> Are you planning on finishing this?  It would be very good to have an upper half plane implementation.  \n\nYes. But if you have time and motivation, go on. But there are problems (see below)\n\n> There are a few things that need to be improved though.  Accessing attributes directly (e.g. with spam._value) is not good.  Please use accessor methods instead (i.e. a method named value() that returns _value); this improves the separation of interface and implementation.\n\nCan be done.\n\n> Near the real line, the hyperbolic distance becomes become HUGE compared to the Euclidean distance.  Representing a point as a complex number thus leads to numeric instability.  It is therefore better to implement a point by a pair of a matrix ((a,b),(c,d)) and a complex number z (thus representing (az+b)/(cz+d)).\n\nI agree on the fact that near the real line it is unstable but disagree on the fact that we need a 5 dimensional object (an element of SL(2,R) and a complex number) to record a 2 dimensional object (a point in the half plane). The best option would be to store only the SL(2,R) matrix m such that the point is the image by z of the point i. Two matrices give the same point iff they are congruent modulo SO(2).\n\nProblems\n\n---\n\n1) The main problem with that project is about of action by matrices. It would be natural that matrices act on the upper half plane. But there are many instances\n* element of ArithmeticSubgroup (SL(2,Z)) which is implemented\n* element of groups SL(2,R) or GL(2,R) or ...\n* a matrix with real coefficient\n* ...\nI had trouble with the coercion system in order to be able to use any of the type above.\n\n2) In the actual implementation, the geodesics that pass to infinity in the half plane model have an arbitrary maximum height. I did not find a good way to draw them depending on what is asked in the final .show(). In the same veine, a circle with radius 1 very near the boundary should not be drawn as it is less than one pixel...",
    "created_at": "2011-12-10T19:12:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83031",
    "user": "https://github.com/videlec"
}
```

<a id='comment:4'>Comment 4:</a>
> Are you planning on finishing this?  It would be very good to have an upper half plane implementation.  

Yes. But if you have time and motivation, go on. But there are problems (see below)

> There are a few things that need to be improved though.  Accessing attributes directly (e.g. with spam._value) is not good.  Please use accessor methods instead (i.e. a method named value() that returns _value); this improves the separation of interface and implementation.

Can be done.

> Near the real line, the hyperbolic distance becomes become HUGE compared to the Euclidean distance.  Representing a point as a complex number thus leads to numeric instability.  It is therefore better to implement a point by a pair of a matrix ((a,b),(c,d)) and a complex number z (thus representing (az+b)/(cz+d)).

I agree on the fact that near the real line it is unstable but disagree on the fact that we need a 5 dimensional object (an element of SL(2,R) and a complex number) to record a 2 dimensional object (a point in the half plane). The best option would be to store only the SL(2,R) matrix m such that the point is the image by z of the point i. Two matrices give the same point iff they are congruent modulo SO(2).

Problems

---

1) The main problem with that project is about of action by matrices. It would be natural that matrices act on the upper half plane. But there are many instances
* element of ArithmeticSubgroup (SL(2,Z)) which is implemented
* element of groups SL(2,R) or GL(2,R) or ...
* a matrix with real coefficient
* ...
I had trouble with the coercion system in order to be able to use any of the type above.

2) In the actual implementation, the geodesics that pass to infinity in the half plane model have an arbitrary maximum height. I did not find a good way to draw them depending on what is asked in the final .show(). In the same veine, a circle with radius 1 very near the boundary should not be drawn as it is less than one pixel...



---

archive/issue_comments_083032.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nReplying to [@videlec](#comment%3A4):\n> \n> I agree on the fact that near the real line it is unstable but disagree on the fact that we need a 5 dimensional object (an element of SL(2,R) and a complex number) to record a 2 dimensional object (a point in the half plane). The best option would be to store only the SL(2,R) matrix m such that the point is the image by z of the point i. Two matrices give the same point iff they are congruent modulo SO(2).\n> \n\nOkay.  Another possibility is to have the matrix ((a, b), (c, d)) in SL_2(ZZ) and the complex number z in the standard fundamental domain for this group.  Or use both representations (and allow oneself to convert between them).",
    "created_at": "2011-12-10T21:08:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83032",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

<a id='comment:5'>Comment 5:</a>
Replying to [@videlec](#comment%3A4):
> 
> I agree on the fact that near the real line it is unstable but disagree on the fact that we need a 5 dimensional object (an element of SL(2,R) and a complex number) to record a 2 dimensional object (a point in the half plane). The best option would be to store only the SL(2,R) matrix m such that the point is the image by z of the point i. Two matrices give the same point iff they are congruent modulo SO(2).
> 

Okay.  Another possibility is to have the matrix ((a, b), (c, d)) in SL_2(ZZ) and the complex number z in the standard fundamental domain for this group.  Or use both representations (and allow oneself to convert between them).



---

archive/issue_comments_083033.json:
```json
{
    "body": "Attachment: **[trac_9439-hyperbolic_2space.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_2space.patch.gz)**\n\nA sketch for hyperbolic 2-space implementation.",
    "created_at": "2011-12-20T15:50:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83033",
    "user": "https://github.com/sagetrac-mraum"
}
```

Attachment: **[trac_9439-hyperbolic_2space.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_2space.patch.gz)**

A sketch for hyperbolic 2-space implementation.



---

archive/issue_comments_083034.json:
```json
{
    "body": "Attachment: **[trac_9439-hyperbolic_2space.2.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_2space.2.patch.gz)**\n\nSame patch with .py~ files removed",
    "created_at": "2011-12-20T16:13:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83034",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Attachment: **[trac_9439-hyperbolic_2space.2.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_2space.2.patch.gz)**

Same patch with .py~ files removed



---

archive/issue_comments_083035.json:
```json
{
    "body": "Changed keywords from **hyperbolic geometry, Poincare disc, upper half plane** to **hyperbolic geometry, Poincare disc, upper half plane, sd35**",
    "created_at": "2011-12-20T16:22:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83035",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Changed keywords from **hyperbolic geometry, Poincare disc, upper half plane** to **hyperbolic geometry, Poincare disc, upper half plane, sd35**



---

archive/issue_comments_083036.json:
```json
{
    "body": "Attachment: **[trac_9439-hyperbolic_upper_half_plane.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_upper_half_plane.patch.gz)**\n\nimplementations of geodesics, triangles, and polygons",
    "created_at": "2011-12-20T18:06:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83036",
    "user": "https://github.com/sagetrac-mraum"
}
```

Attachment: **[trac_9439-hyperbolic_upper_half_plane.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_upper_half_plane.patch.gz)**

implementations of geodesics, triangles, and polygons



---

archive/issue_comments_083037.json:
```json
{
    "body": "Attachment: **[trac_9439_with_points.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_with_points.patch.gz)**\n\nContains (and thus replaces) previous patch",
    "created_at": "2011-12-20T23:25:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83037",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Attachment: **[trac_9439_with_points.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_with_points.patch.gz)**

Contains (and thus replaces) previous patch



---

archive/issue_comments_083038.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThe patch I've just uploaded makes Sage import HH at startup.  I propose that we clean up all the mess first (including all the doctest failures) before we add further functionality.",
    "created_at": "2011-12-20T23:28:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83038",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

<a id='comment:7'>Comment 7:</a>
The patch I've just uploaded makes Sage import HH at startup.  I propose that we clean up all the mess first (including all the doctest failures) before we add further functionality.



---

archive/issue_comments_083039.json:
```json
{
    "body": "Attachment: **[trac_9439_with_points.2.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_with_points.2.patch.gz)**\n\nSecond attempt; hopefully the point file is added. :).",
    "created_at": "2011-12-21T12:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83039",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Attachment: **[trac_9439_with_points.2.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_with_points.2.patch.gz)**

Second attempt; hopefully the point file is added. :).



---

archive/issue_comments_083040.json:
```json
{
    "body": "Apparently not, so let's just upload the file. :P.",
    "created_at": "2011-12-21T12:58:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83040",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Apparently not, so let's just upload the file. :P.



---

archive/issue_comments_083041.json:
```json
{
    "body": "Attachment: **[upper_half_plane_point.py.gz](https://github.com/sagemath/sage/files/ticket9439/upper_half_plane_point.py.gz)**",
    "created_at": "2011-12-21T20:57:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83041",
    "user": "https://github.com/sagetrac-mraum"
}
```

Attachment: **[upper_half_plane_point.py.gz](https://github.com/sagemath/sage/files/ticket9439/upper_half_plane_point.py.gz)**



---

archive/issue_comments_083042.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nAttachment: **[trac_9439_hyperbolic_space.tar.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_hyperbolic_space.tar.gz)**\n\nI simply uploaded all current files, because I messed up my HG status.\n\nI couldn't fix the problem with conversion into CC, which is now #12216. I replaced it by the method toCC.\n\nI haven't checked whether triangles work, but I am exhausted. Plotting polygons works and it should be easy to fill in the tests, that are all stubs for the moment.",
    "created_at": "2011-12-21T20:59:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83042",
    "user": "https://github.com/sagetrac-mraum"
}
```

<a id='comment:8'>Comment 8:</a>
Attachment: **[trac_9439_hyperbolic_space.tar.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_hyperbolic_space.tar.gz)**

I simply uploaded all current files, because I messed up my HG status.

I couldn't fix the problem with conversion into CC, which is now #12216. I replaced it by the method toCC.

I haven't checked whether triangles work, but I am exhausted. Plotting polygons works and it should be easy to fill in the tests, that are all stubs for the moment.



---

archive/issue_comments_083043.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThis looks neat: I just wanted to encourage you guys to keep working on it.  I just had a request for plotting fundamental domains of congruence subgroups....",
    "created_at": "2012-07-20T19:48:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83043",
    "user": "https://github.com/roed314"
}
```

<a id='comment:9'>Comment 9:</a>
This looks neat: I just wanted to encourage you guys to keep working on it.  I just had a request for plotting fundamental domains of congruence subgroups....



---

archive/issue_comments_083044.json:
```json
{
    "body": "Reviewer: **Johan Bosman**",
    "created_at": "2012-07-20T21:26:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83044",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Johan Bosman**



---

archive/issue_comments_083045.json:
```json
{
    "body": "Changed author from **vdelecroix** to **Vincent Delecroix, Martin Raum**",
    "created_at": "2012-07-20T21:26:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83045",
    "user": "https://github.com/kcrisman"
}
```

Changed author from **vdelecroix** to **Vincent Delecroix, Martin Raum**



---

archive/issue_comments_083046.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nReplying to [@roed314](#comment%3A9):\n> This looks neat: I just wanted to encourage you guys to keep working on it.  I just had a request for plotting fundamental domains of congruence subgroups....\n\nI just notice that it is yet possible to draw fundamental domain for congruence subgroups using Farey symbols (#11709, integrated since sage-5.0).\n\n```\nsage: FareySymbol(Gamma(3)).fundamental_domain()\n```",
    "created_at": "2012-07-22T00:51:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83046",
    "user": "https://github.com/videlec"
}
```

<a id='comment:11'>Comment 11:</a>
Replying to [@roed314](#comment%3A9):
> This looks neat: I just wanted to encourage you guys to keep working on it.  I just had a request for plotting fundamental domains of congruence subgroups....

I just notice that it is yet possible to draw fundamental domain for congruence subgroups using Farey symbols (#11709, integrated since sage-5.0).

```
sage: FareySymbol(Gamma(3)).fundamental_domain()
```



---

archive/issue_comments_083047.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nI have a working implementation of hyperbolic two-space in sage that includes the upper half plane, Poincare disk, Klein disk, and hyperboloid model. For each model, geodesics, points, and isometry groups are implemented. \n\nConversion between each model to another is implemented*, as are a number of useful calculation routines.  For example, geodesics can compute the corresponding reflection, two geodesics can compute their common perpendiculars, isometries can factor themselves into products of reflections.\n\nThe code originated as a series of Mathematica notebooks from Bill Goldman's lab at UMD.  We had some students port it to a sage script, which was then fleshed out.  I have refrained from publishing it here because I'm completely refactoring into something resembling readable code.  It's taken me a few month longer than I thought it would to refactor, so I wanted to write a note here so that efforts aren't redoubled.\n\nIs there a best course of action as far as posting code goes?  I think that merging the patches posted here and my code won't be too bad.  Should I finish refactoring my code first, and then post it?  Should I post the original script?  In the original code, there are lots of test and all tests pass, so it's usable.  Should I post the bit of refactoring I've done so far?\n\n*Actually, not quite.  I don't have a great way to go from SO(2,1) to SL(2,R). So this makes converting from the hyperboloid model to the other models somewhat tricky.",
    "created_at": "2012-10-07T14:52:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83047",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:12'>Comment 12:</a>
I have a working implementation of hyperbolic two-space in sage that includes the upper half plane, Poincare disk, Klein disk, and hyperboloid model. For each model, geodesics, points, and isometry groups are implemented. 

Conversion between each model to another is implemented*, as are a number of useful calculation routines.  For example, geodesics can compute the corresponding reflection, two geodesics can compute their common perpendiculars, isometries can factor themselves into products of reflections.

The code originated as a series of Mathematica notebooks from Bill Goldman's lab at UMD.  We had some students port it to a sage script, which was then fleshed out.  I have refrained from publishing it here because I'm completely refactoring into something resembling readable code.  It's taken me a few month longer than I thought it would to refactor, so I wanted to write a note here so that efforts aren't redoubled.

Is there a best course of action as far as posting code goes?  I think that merging the patches posted here and my code won't be too bad.  Should I finish refactoring my code first, and then post it?  Should I post the original script?  In the original code, there are lots of test and all tests pass, so it's usable.  Should I post the bit of refactoring I've done so far?

*Actually, not quite.  I don't have a great way to go from SO(2,1) to SL(2,R). So this makes converting from the hyperboloid model to the other models somewhat tricky.



---

archive/issue_comments_083048.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nHi Greg,\n\nYou should either post your code here or provide a link. If your code does work, we will definitely use it !\n\nNote that several things should be merged\n* the different code in that ticket\n* fundamental domain of FareySymbol\n* your code\n\nBest,\nVincent",
    "created_at": "2012-10-08T09:21:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83048",
    "user": "https://github.com/videlec"
}
```

<a id='comment:13'>Comment 13:</a>
Hi Greg,

You should either post your code here or provide a link. If your code does work, we will definitely use it !

Note that several things should be merged
* the different code in that ticket
* fundamental domain of FareySymbol
* your code

Best,
Vincent



---

archive/issue_comments_083049.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nOkay great.  I'll post it by the end of the week.  That should be enough time to clean up the remaining refactoring (which will greatly improve the ability to merge the code).",
    "created_at": "2012-10-08T12:32:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83049",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:14'>Comment 14:</a>
Okay great.  I'll post it by the end of the week.  That should be enough time to clean up the remaining refactoring (which will greatly improve the ability to merge the code).



---

archive/issue_comments_083050.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nObviously it's been longer than a week.  I had to attend a long conference and then hurricanes etc.  It will be at the very least another week before I get it up.",
    "created_at": "2012-11-02T19:11:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83050",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:15'>Comment 15:</a>
Obviously it's been longer than a week.  I had to attend a long conference and then hurricanes etc.  It will be at the very least another week before I get it up.



---

archive/issue_comments_083051.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,3 +11,6 @@\n sage: p.plot(face_color='red').show(aspect_ratio=1)\n ```\n There are more examples in the file.\n+\n+Apply:\n+* [attachment: trac_9439-hyperbolic_geometry.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry.patch.gz)\n``````\n",
    "created_at": "2013-03-07T15:18:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83051",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,3 +11,6 @@
 sage: p.plot(face_color='red').show(aspect_ratio=1)
 ```
 There are more examples in the file.
+
+Apply:
+* [attachment: trac_9439-hyperbolic_geometry.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry.patch.gz)
``````




---

archive/issue_comments_083052.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nWhat a mess ! can some of you say what patches have to be applied and in which order ?\n\nLet me try with just one patch:\n\napply trac_9439-hyperbolic_geometry.patch",
    "created_at": "2013-03-07T15:18:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83052",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:16'>Comment 16:</a>
What a mess ! can some of you say what patches have to be applied and in which order ?

Let me try with just one patch:

apply trac_9439-hyperbolic_geometry.patch



---

archive/issue_comments_083053.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nA review patch, so that tests pass. But there lacks a lot of documentation !",
    "created_at": "2013-03-15T21:55:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83053",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:17'>Comment 17:</a>
A review patch, so that tests pass. But there lacks a lot of documentation !



---

archive/issue_comments_083054.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,3 +14,4 @@\n \n Apply:\n * [attachment: trac_9439-hyperbolic_geometry.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry.patch.gz)\n+* [attachment: trac_9439-hyperbolic_geometry_review_fc.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry_review_fc.patch.gz)\n``````\n",
    "created_at": "2013-03-15T21:57:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83054",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,3 +14,4 @@
 
 Apply:
 * [attachment: trac_9439-hyperbolic_geometry.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry.patch.gz)
+* [attachment: trac_9439-hyperbolic_geometry_review_fc.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry_review_fc.patch.gz)
``````




---

archive/issue_comments_083055.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\n50% coverage",
    "created_at": "2013-06-05T19:37:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83055",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:19'>Comment 19:</a>
50% coverage



---

archive/issue_comments_083056.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nAttachment: **[trac_9439-hyperbolic_geometry_review_fc.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry_review_fc.patch.gz)**\n\n77% coverage",
    "created_at": "2013-06-10T19:38:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83056",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:20'>Comment 20:</a>
Attachment: **[trac_9439-hyperbolic_geometry_review_fc.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439-hyperbolic_geometry_review_fc.patch.gz)**

77% coverage



---

archive/issue_comments_083057.json:
```json
{
    "body": "Attachment: **[hyperbolic_space.patch.gz](https://github.com/sagemath/sage/files/ticket9439/hyperbolic_space.patch.gz)**\n\nalternative implementation of hyperbolic space",
    "created_at": "2013-06-26T06:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83057",
    "user": "https://github.com/greglaun"
}
```

Attachment: **[hyperbolic_space.patch.gz](https://github.com/sagemath/sage/files/ticket9439/hyperbolic_space.patch.gz)**

alternative implementation of hyperbolic space



---

archive/issue_comments_083058.json:
```json
{
    "body": "demonstration of the hyperbolic geometry functionality",
    "created_at": "2013-06-26T06:44:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83058",
    "user": "https://github.com/greglaun"
}
```

demonstration of the hyperbolic geometry functionality



---

archive/issue_comments_083059.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nAttachment: **[hyp_demo.sage.gz](https://github.com/sagemath/sage/files/ticket9439/hyp_demo.sage.gz)**\n\nI atteched the long-overdue patch that I mentioned 9 months ago.  Sorry for the delay.  The patch has the following positive properties:\n* Hyperbolic point, geodesic, and isometry objects are implemented for each model.\n* Upper half plane, Poincare disk, Klein disk, and hyperboloid model all implemented.\n* Round-trip conversion among models works.  E.g. converting from the upper half plane to the hyperboloid and then back to the half plane gives the same point (up to numerical precision).  This was harder than might first be apparent since there are so many isomorphisms to choose from and they all have to play well together.\n* 100% test coverage, all tests pass\nAlso note the following negative things:\n* Points are not yet implemented as (point, isometry) pairs as suggested in Comment 3.\n* My handling of numerical computations could probably be significantly improved, as can the overall organization.  There may be features that are unnecessary or are vestigial from earlier versions.\n* Symbolic computations can take an incredibly long time.  I'm not sure if this is my fault (e.g. I should write functions to deal with this) or simply a drawback of allowing symbolic computations.\nI intend to comb through the previously attached patches and merge what I can.  I have also attached a filecalled hyp_demo.sage that demos the functionality of the implementation.",
    "created_at": "2013-06-26T06:53:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83059",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:21'>Comment 21:</a>
Attachment: **[hyp_demo.sage.gz](https://github.com/sagemath/sage/files/ticket9439/hyp_demo.sage.gz)**

I atteched the long-overdue patch that I mentioned 9 months ago.  Sorry for the delay.  The patch has the following positive properties:
* Hyperbolic point, geodesic, and isometry objects are implemented for each model.
* Upper half plane, Poincare disk, Klein disk, and hyperboloid model all implemented.
* Round-trip conversion among models works.  E.g. converting from the upper half plane to the hyperboloid and then back to the half plane gives the same point (up to numerical precision).  This was harder than might first be apparent since there are so many isomorphisms to choose from and they all have to play well together.
* 100% test coverage, all tests pass
Also note the following negative things:
* Points are not yet implemented as (point, isometry) pairs as suggested in Comment 3.
* My handling of numerical computations could probably be significantly improved, as can the overall organization.  There may be features that are unnecessary or are vestigial from earlier versions.
* Symbolic computations can take an incredibly long time.  I'm not sure if this is my fault (e.g. I should write functions to deal with this) or simply a drawback of allowing symbolic computations.
I intend to comb through the previously attached patches and merge what I can.  I have also attached a filecalled hyp_demo.sage that demos the functionality of the implementation.



---

archive/issue_comments_083060.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nReplying to [@greglaun](#comment%3A21):\n> I atteched the long-overdue patch that I mentioned 9 months ago.  Sorry for the delay.  The patch has the following positive properties:\n\nGreat! Good job!\n\n> Also note the following negative things:\n> * Points are not yet implemented as (point, isometry) pairs as suggested in Comment 3.\n\nThis was a *bad* suggestion! It's great that you avoid it.\n\n> * My handling of numerical computations could probably be significantly improved, as can the overall organization.  There may be features that are unnecessary or are vestigial from earlier versions.\n\nI will have a look as soon as possible.\n\n> * Symbolic computations can take an incredibly long time.  I'm not sure if this is my fault (e.g. I should write functions to deal with this) or simply a drawback of allowing symbolic computations.\n> I intend to comb through the previously attached patches and merge what I can.  I have also attached a filecalled hyp_demo.sage that demos the functionality of the implementation.\n\nidem.\n\nTwo points:\n\n- an important feature that you seem to avoid is the unit tangent bundle of the hyperbolic plane which is isomorphic to PSL(2,R). One great thing would be to have another object `TangentVector` (with a matrix as data). The action of PSL(2,R) on the unit tangent bundle is then just matrix multiplication. \n- your example worksheet should definitely be a thematic tutorial for the Sage documentation\n\nI suggest that we open two new tickets for those features.\n\nThanks again for your work on this. I am starting the review and will be back with more technical remarks shortly.",
    "created_at": "2013-06-26T14:32:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83060",
    "user": "https://github.com/videlec"
}
```

<a id='comment:22'>Comment 22:</a>
Replying to [@greglaun](#comment%3A21):
> I atteched the long-overdue patch that I mentioned 9 months ago.  Sorry for the delay.  The patch has the following positive properties:

Great! Good job!

> Also note the following negative things:
> * Points are not yet implemented as (point, isometry) pairs as suggested in Comment 3.

This was a *bad* suggestion! It's great that you avoid it.

> * My handling of numerical computations could probably be significantly improved, as can the overall organization.  There may be features that are unnecessary or are vestigial from earlier versions.

I will have a look as soon as possible.

> * Symbolic computations can take an incredibly long time.  I'm not sure if this is my fault (e.g. I should write functions to deal with this) or simply a drawback of allowing symbolic computations.
> I intend to comb through the previously attached patches and merge what I can.  I have also attached a filecalled hyp_demo.sage that demos the functionality of the implementation.

idem.

Two points:

- an important feature that you seem to avoid is the unit tangent bundle of the hyperbolic plane which is isomorphic to PSL(2,R). One great thing would be to have another object `TangentVector` (with a matrix as data). The action of PSL(2,R) on the unit tangent bundle is then just matrix multiplication. 
- your example worksheet should definitely be a thematic tutorial for the Sage documentation

I suggest that we open two new tickets for those features.

Thanks again for your work on this. I am starting the review and will be back with more technical remarks shortly.



---

archive/issue_comments_083061.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\n> Two points:\n> \n> - an important feature that you seem to avoid is the unit tangent bundle of the hyperbolic plane which is isomorphic to PSL(2,R). One great thing would be to have another object `TangentVector` (with a matrix as data). The action of PSL(2,R) on the unit tangent bundle is then just matrix multiplication. \n> - your example worksheet should definitely be a thematic tutorial for the Sage documentation\n\nThanks, I'll look into both of these soon.  I have some code for working in Minkowski (2,1) space that I want to use to implement the hyperboloid model in the Lie algebra sl(2,R) since that's what I use in my own research.  Several of the functions are for working in SL(2,R)/PSL(2,R).  I can take a look at that and see what can be appropriated for use in a TangentVector object.",
    "created_at": "2013-06-26T18:04:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83061",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:23'>Comment 23:</a>
> Two points:
> 
> - an important feature that you seem to avoid is the unit tangent bundle of the hyperbolic plane which is isomorphic to PSL(2,R). One great thing would be to have another object `TangentVector` (with a matrix as data). The action of PSL(2,R) on the unit tangent bundle is then just matrix multiplication. 
> - your example worksheet should definitely be a thematic tutorial for the Sage documentation

Thanks, I'll look into both of these soon.  I have some code for working in Minkowski (2,1) space that I want to use to implement the hyperboloid model in the Lie algebra sl(2,R) since that's what I use in my own research.  Several of the functions are for working in SL(2,R)/PSL(2,R).  I can take a look at that and see what can be appropriated for use in a TangentVector object.



---

archive/issue_comments_083062.json:
```json
{
    "body": "<a id='comment:24'>Comment 24:</a>\nHi Greg,\n\nThere are many nice features in the patch but I did not start to play with. You should start by a big clean:\n\n1) there should be no trailing whitespace\n\n2) the organization of each file should be a header which consists of a string with authors the copyright statement and then the code. There should not be several headers and copyright statements.\n\n3) all methods and functions should be commented and doctested. Running `sage -coverage` gives\n\n```\nSCORE hyperbolic_object.py: 10.0% (1 of 10)\n\nMissing documentation:\n     * line 40: def to_model(self, model)\n     * line 49: def to_UHP(self)\n     * line 52: def uhp_representation(self)\n     * line 55: def model_representation(self)\n     * line 58: def representation_in_model(self, model=None)\n     * line 96: def to_model(self, model, **options)\n     * line 108: def graphics_options(self)\n\nMissing doctests:\n     * line 33: def __init__(self, args, **graphics_options)\n     * line 88: def __init__(self, args, **graphics_options)\n```\n\n4) the files should be properly included in the sage documentation\n\nNow, more serious issues\n\n5) the objects from your module must be lazy imported and not imported in the global namespace (in order to not slow down sage startup)\n\n6) the method `show` should not return a graphics object but should rather shows it! You could rename it `plot`.\n\n7) I agree that it is misleading but `CC` in Sage is not the set of complex number! In particular the test `x in CC` does not answer to the question \"does my object `x` modelizes some complex number ?\". Precisely `CC` is the set of floating point complex number with 53 bits of precision. But is it on purpose that you want `x in CC` as coordinates for a point in the upper half plane ?\n\n8) The string representation \"Hyperbolic point whose representation in the Upper Half Plane is +Infinity\" is definitely too long! Why not \"Point in HH +Infinity\".\n\nAnd finally a design question (which perhaps should be thought of first)\n\n9) I think that the fact of being conformal or bounded etc is not a property of a HyperbolicObject but rather a property of the underlying model. You decided to not design a class for each model, was it on purpose? Such a class may contain all these property. As you see, in my initial patch, it was possible to write\n\n```\nsage: HH\nHyperbolic plane\nsage: HH(0)\nBoundary point 0\n```\nI would prefer to have a dedicated class for each model and be able to construct point/geodesic/polygons from the class (via for example ``HH.point(data)``, ``HH.geodesic(data)``, etc). You may also move the `random_element` methods and the various conversions between the models into this parent class.",
    "created_at": "2013-06-26T19:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83062",
    "user": "https://github.com/videlec"
}
```

<a id='comment:24'>Comment 24:</a>
Hi Greg,

There are many nice features in the patch but I did not start to play with. You should start by a big clean:

1) there should be no trailing whitespace

2) the organization of each file should be a header which consists of a string with authors the copyright statement and then the code. There should not be several headers and copyright statements.

3) all methods and functions should be commented and doctested. Running `sage -coverage` gives

```
SCORE hyperbolic_object.py: 10.0% (1 of 10)

Missing documentation:
     * line 40: def to_model(self, model)
     * line 49: def to_UHP(self)
     * line 52: def uhp_representation(self)
     * line 55: def model_representation(self)
     * line 58: def representation_in_model(self, model=None)
     * line 96: def to_model(self, model, **options)
     * line 108: def graphics_options(self)

Missing doctests:
     * line 33: def __init__(self, args, **graphics_options)
     * line 88: def __init__(self, args, **graphics_options)
```

4) the files should be properly included in the sage documentation

Now, more serious issues

5) the objects from your module must be lazy imported and not imported in the global namespace (in order to not slow down sage startup)

6) the method `show` should not return a graphics object but should rather shows it! You could rename it `plot`.

7) I agree that it is misleading but `CC` in Sage is not the set of complex number! In particular the test `x in CC` does not answer to the question "does my object `x` modelizes some complex number ?". Precisely `CC` is the set of floating point complex number with 53 bits of precision. But is it on purpose that you want `x in CC` as coordinates for a point in the upper half plane ?

8) The string representation "Hyperbolic point whose representation in the Upper Half Plane is +Infinity" is definitely too long! Why not "Point in HH +Infinity".

And finally a design question (which perhaps should be thought of first)

9) I think that the fact of being conformal or bounded etc is not a property of a HyperbolicObject but rather a property of the underlying model. You decided to not design a class for each model, was it on purpose? Such a class may contain all these property. As you see, in my initial patch, it was possible to write

```
sage: HH
Hyperbolic plane
sage: HH(0)
Boundary point 0
```
I would prefer to have a dedicated class for each model and be able to construct point/geodesic/polygons from the class (via for example ``HH.point(data)``, ``HH.geodesic(data)``, etc). You may also move the `random_element` methods and the various conversions between the models into this parent class.



---

archive/issue_comments_083063.json:
```json
{
    "body": "<a id='comment:25'>Comment 25:</a>\nThanks for the feedback!  I didn't know exactly what to do about tests in what was intended to be an abstract class (hyperbeolic_object) that shouldn't be instantiated.  I see now that there are other abstract classes in sage and they all have poper doctests, so I'll go ahead and add those in.  That should be very simple, as should the rest of the cleanup.\n\nThe other issues you mentioned are all things I thought might be problems, so I've given them all a little bit of thought already\n\nReplying to [@videlec](#comment%3A24):\n\n\n> 4) the files should be properly included in the sage documentation\n\nOkay, will do.\n\n> Now, more serious issues\n> \n> 5) the objects from your module must be lazy imported and not imported in the global namespace (in order to not slow down sage startup)\n\nOkay, I will change this.\n\n> \n> 6) the method `show` should not return a graphics object but should rather shows it! You could rename it `plot`.\n\nI'm actually not quite sure what the difference is.  Do you mean that show should make a system call to the default viewer?  Is it not sufficient to call a method that makes that call for me?  I've noticed in one other module that plot() is implemented and show() is an alias for plot.  Would this be a workaround?   \n\n> 7) I agree that it is misleading but `CC` in Sage is not the set of complex number! In particular the test `x in CC` does not answer to the question \"does my object `x` modelizes some complex number ?\". Precisely `CC` is the set of floating point complex number with 53 bits of precision. But is it on purpose that you want `x in CC` as coordinates for a point in the upper half plane ?\n\nThis is a hack. I wanted a function that returned True for things that can be converted into floating point complex numbers and \"2 + I in CC\" returns True even though \"2 + I\" is a symbolic expression.  I didn't want to test more explicitly for symbolic complex numbers because I don't know the symbolic system well enough to be sure to catch all possible cases.  And I didn't want to have a test condition that relied on the symbolic apparatus because via profiling I found many cases in which calling functions that were in the symbolics library slowed everything to a crawl.  So \"x in CC\" quickly checks whether something is the right type of object to have \"imag()\" called on it sensibly.  I more than welcome suggestions on how to solve this problem more elegantly without slowing down code.  As an example, the functions _clean_points and _shorten_symbolic are both very ugly to my tastes, but I wrote each to address specific issues that arose in profiling and their impact is significant.  In a similar way, I find 'x in CC' to be unfortunate but useful.\n\n> 8) The string representation \"Hyperbolic point whose representation in the Upper Half Plane is +Infinity\" is definitely too long! Why not \"Point in HH +Infinity\".\n\nThe long strings were a request, and I agree they're much too long.  I'll go ahead and change them.  I'd like them to contain information about the model, though so that there is less confusion when performing conversion. \n \n> And finally a design question (which perhaps should be thought of first)\n> \n> 9) I think that the fact of being conformal or bounded etc is not a property of a HyperbolicObject but rather a property of the underlying model. You decided to not design a class for each model, was it on purpose? Such a class may contain all these property. As you see, in my initial patch, it was possible to write\n> \n> ```\n> sage: HH\n> Hyperbolic plane\n> sage: HH(0)\n> Boundary point 0\n> ```\n> I would prefer to have a dedicated class for each model and be able to construct point/geodesic/polygons from the class (via for example ``HH.point(data)``, ``HH.geodesic(data)``, etc). You may also move the `random_element` methods and the various conversions between the models into this parent class.\n\nI think your way of doing it sounds  much better than mine. I had actually been planning on looking more closely at your structure and applying it to mine as a next step.  I'll look into this starting tomorrow.",
    "created_at": "2013-06-27T04:35:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83063",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:25'>Comment 25:</a>
Thanks for the feedback!  I didn't know exactly what to do about tests in what was intended to be an abstract class (hyperbeolic_object) that shouldn't be instantiated.  I see now that there are other abstract classes in sage and they all have poper doctests, so I'll go ahead and add those in.  That should be very simple, as should the rest of the cleanup.

The other issues you mentioned are all things I thought might be problems, so I've given them all a little bit of thought already

Replying to [@videlec](#comment%3A24):


> 4) the files should be properly included in the sage documentation

Okay, will do.

> Now, more serious issues
> 
> 5) the objects from your module must be lazy imported and not imported in the global namespace (in order to not slow down sage startup)

Okay, I will change this.

> 
> 6) the method `show` should not return a graphics object but should rather shows it! You could rename it `plot`.

I'm actually not quite sure what the difference is.  Do you mean that show should make a system call to the default viewer?  Is it not sufficient to call a method that makes that call for me?  I've noticed in one other module that plot() is implemented and show() is an alias for plot.  Would this be a workaround?   

> 7) I agree that it is misleading but `CC` in Sage is not the set of complex number! In particular the test `x in CC` does not answer to the question "does my object `x` modelizes some complex number ?". Precisely `CC` is the set of floating point complex number with 53 bits of precision. But is it on purpose that you want `x in CC` as coordinates for a point in the upper half plane ?

This is a hack. I wanted a function that returned True for things that can be converted into floating point complex numbers and "2 + I in CC" returns True even though "2 + I" is a symbolic expression.  I didn't want to test more explicitly for symbolic complex numbers because I don't know the symbolic system well enough to be sure to catch all possible cases.  And I didn't want to have a test condition that relied on the symbolic apparatus because via profiling I found many cases in which calling functions that were in the symbolics library slowed everything to a crawl.  So "x in CC" quickly checks whether something is the right type of object to have "imag()" called on it sensibly.  I more than welcome suggestions on how to solve this problem more elegantly without slowing down code.  As an example, the functions _clean_points and _shorten_symbolic are both very ugly to my tastes, but I wrote each to address specific issues that arose in profiling and their impact is significant.  In a similar way, I find 'x in CC' to be unfortunate but useful.

> 8) The string representation "Hyperbolic point whose representation in the Upper Half Plane is +Infinity" is definitely too long! Why not "Point in HH +Infinity".

The long strings were a request, and I agree they're much too long.  I'll go ahead and change them.  I'd like them to contain information about the model, though so that there is less confusion when performing conversion. 
 
> And finally a design question (which perhaps should be thought of first)
> 
> 9) I think that the fact of being conformal or bounded etc is not a property of a HyperbolicObject but rather a property of the underlying model. You decided to not design a class for each model, was it on purpose? Such a class may contain all these property. As you see, in my initial patch, it was possible to write
> 
> ```
> sage: HH
> Hyperbolic plane
> sage: HH(0)
> Boundary point 0
> ```
> I would prefer to have a dedicated class for each model and be able to construct point/geodesic/polygons from the class (via for example ``HH.point(data)``, ``HH.geodesic(data)``, etc). You may also move the `random_element` methods and the various conversions between the models into this parent class.

I think your way of doing it sounds  much better than mine. I had actually been planning on looking more closely at your structure and applying it to mine as a next step.  I'll look into this starting tomorrow.



---

archive/issue_comments_083064.json:
```json
{
    "body": "<a id='comment:26'>Comment 26:</a>\nHi Vincent,\n\nI've addressed issues 1-4, and I was hoping you wouldn't mind giving me some guidance about how to handle the remaining issues.  Specifically, I have the following questions:\n\n1) Should I post a patch for the fixes of 1-4?  Or should I wait until I fix the remaining issues?  More generally, should I err on the side of posting more patches (so more people can help) or posting only ones that seem more significant?\n\n2) I like the structure of your code, and I aim to emulate it.  Does it make sense for me to combine your class structure with the features in mine and post it in skeletal form before fixing all of the functions?  For example, I could post a class diagram, or just an outline of the modules with the guts removed. Or is this step likely to be more work with little payoff?  I would ideally like the structure to be as democratically decided as possible.\n\n3) I find your usage of HyperbolicPlane for the upper half plane to be confusing since the HyperbolicDisc is also topologically a plane.  I'm used to referring to both as the hyperbolic plane, and I typically differentiate them by specifying which model of the plane.  I have been using the abbreviations UHP, PD, KM, and HM which were just decided by fiat to be short and convenient.  Is there anything you dislike about this approach to naming?  I worry that HyperbolicUHP is too cryptic, but HyperbolicUpperHalfPlane is too long.",
    "created_at": "2013-07-02T21:25:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83064",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:26'>Comment 26:</a>
Hi Vincent,

I've addressed issues 1-4, and I was hoping you wouldn't mind giving me some guidance about how to handle the remaining issues.  Specifically, I have the following questions:

1) Should I post a patch for the fixes of 1-4?  Or should I wait until I fix the remaining issues?  More generally, should I err on the side of posting more patches (so more people can help) or posting only ones that seem more significant?

2) I like the structure of your code, and I aim to emulate it.  Does it make sense for me to combine your class structure with the features in mine and post it in skeletal form before fixing all of the functions?  For example, I could post a class diagram, or just an outline of the modules with the guts removed. Or is this step likely to be more work with little payoff?  I would ideally like the structure to be as democratically decided as possible.

3) I find your usage of HyperbolicPlane for the upper half plane to be confusing since the HyperbolicDisc is also topologically a plane.  I'm used to referring to both as the hyperbolic plane, and I typically differentiate them by specifying which model of the plane.  I have been using the abbreviations UHP, PD, KM, and HM which were just decided by fiat to be short and convenient.  Is there anything you dislike about this approach to naming?  I worry that HyperbolicUHP is too cryptic, but HyperbolicUpperHalfPlane is too long.



---

archive/issue_comments_083065.json:
```json
{
    "body": "<a id='comment:27'>Comment 27:</a>\nHi,\n\nReplying to [@greglaun](#comment%3A26):\n> I've addressed issues 1-4, and I was hoping you wouldn't mind giving me some guidance about how to handle the remaining issues.  Specifically, I have the following questions:\n> \n> 1) Should I post a patch for the fixes of 1-4?  Or should I wait until I fix the remaining issues?  More generally, should I err on the side of posting more patches (so more people can help) or posting only ones that seem more significant?\n\nNot necessarily. You can fold your two patches into one and post only the update version of your previous patch (to fold two patches `hg qfold`).\n \n> 2) I like the structure of your code, and I aim to emulate it.  Does it make sense for me to combine your class structure with the features in mine and post it in skeletal form before fixing all of the functions?  For example, I could post a class diagram, or just an outline of the modules with the guts removed. Or is this step likely to be more work with little payoff?  I would ideally like the structure to be as democratically decided as possible.\n\nYour code definitely contains much more material than mine. If you like better the structure I drafted you are free to reuse it. If you post anything on trac it is always better that it is working code.\n\nIn my opinion, as you are currently working on that project, you may choose the datastructure you prefer and indicate **clearly** in the documentation the concept, why did you choose that design and why did you discard the other ones.\n\n> 3) I find your usage of HyperbolicPlane for the upper half plane to be confusing since the HyperbolicDisc is also topologically a plane. \n\nI agree. It was just because I always find HH and DD in textbooks and did not ask myself more questions.\n\n> I'm used to referring to both as the hyperbolic plane, and I typically differentiate them by specifying which model of the plane.  I have been using the abbreviations UHP, PD, KM, and HM which were just decided by fiat to be short and convenient.  Is there anything you dislike about this approach to naming?  I worry that HyperbolicUHP is too cryptic, but HyperbolicUpperHalfPlane is too long.\n\nI like better UHP, PD, KM and HM but I think they should not be imported in the standard namespace as such (because it is not clear how the user may find them). What about something like:\n\n```\nsage: hyperbolic_geometry.upper_half_plane()\nThe upper half plane\nsage: hyperbolic_geometry.UHP\nThe upper half plane\netc\n```\nThat way, we may use tab completion. But perhaps, \"hyperbolic_geometry\" is a bit too long. If somebody want it in the global namespace it is still possible to do\n\n```\nsage: from sage.XXX.hyperbolic_geometry import *\nsage: UHP\nThe upper half plane\n```\n\nVincent",
    "created_at": "2013-07-03T13:32:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83065",
    "user": "https://github.com/videlec"
}
```

<a id='comment:27'>Comment 27:</a>
Hi,

Replying to [@greglaun](#comment%3A26):
> I've addressed issues 1-4, and I was hoping you wouldn't mind giving me some guidance about how to handle the remaining issues.  Specifically, I have the following questions:
> 
> 1) Should I post a patch for the fixes of 1-4?  Or should I wait until I fix the remaining issues?  More generally, should I err on the side of posting more patches (so more people can help) or posting only ones that seem more significant?

Not necessarily. You can fold your two patches into one and post only the update version of your previous patch (to fold two patches `hg qfold`).
 
> 2) I like the structure of your code, and I aim to emulate it.  Does it make sense for me to combine your class structure with the features in mine and post it in skeletal form before fixing all of the functions?  For example, I could post a class diagram, or just an outline of the modules with the guts removed. Or is this step likely to be more work with little payoff?  I would ideally like the structure to be as democratically decided as possible.

Your code definitely contains much more material than mine. If you like better the structure I drafted you are free to reuse it. If you post anything on trac it is always better that it is working code.

In my opinion, as you are currently working on that project, you may choose the datastructure you prefer and indicate **clearly** in the documentation the concept, why did you choose that design and why did you discard the other ones.

> 3) I find your usage of HyperbolicPlane for the upper half plane to be confusing since the HyperbolicDisc is also topologically a plane. 

I agree. It was just because I always find HH and DD in textbooks and did not ask myself more questions.

> I'm used to referring to both as the hyperbolic plane, and I typically differentiate them by specifying which model of the plane.  I have been using the abbreviations UHP, PD, KM, and HM which were just decided by fiat to be short and convenient.  Is there anything you dislike about this approach to naming?  I worry that HyperbolicUHP is too cryptic, but HyperbolicUpperHalfPlane is too long.

I like better UHP, PD, KM and HM but I think they should not be imported in the standard namespace as such (because it is not clear how the user may find them). What about something like:

```
sage: hyperbolic_geometry.upper_half_plane()
The upper half plane
sage: hyperbolic_geometry.UHP
The upper half plane
etc
```
That way, we may use tab completion. But perhaps, "hyperbolic_geometry" is a bit too long. If somebody want it in the global namespace it is still possible to do

```
sage: from sage.XXX.hyperbolic_geometry import *
sage: UHP
The upper half plane
```

Vincent



---

archive/issue_comments_083066.json:
```json
{
    "body": "<a id='comment:28'>Comment 28:</a>\nHi, just to give an update, I've completely revamped the class and module structure in a way that I like much better.  Right now only the basics of the classes are implemented, but those basics are tested and everything is in working order.  I'm now starting to add more functionality.",
    "created_at": "2013-07-30T19:44:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83066",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:28'>Comment 28:</a>
Hi, just to give an update, I've completely revamped the class and module structure in a way that I like much better.  Right now only the basics of the classes are implemented, but those basics are tested and everything is in working order.  I'm now starting to add more functionality.



---

archive/issue_events_106979.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106979"
}
```



---

archive/issue_events_106980.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106980"
}
```



---

archive/issue_comments_083067.json:
```json
{
    "body": "Updated hyperbolic space implementation.",
    "created_at": "2013-09-10T02:52:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83067",
    "user": "https://github.com/greglaun"
}
```

Updated hyperbolic space implementation.



---

archive/issue_comments_083068.json:
```json
{
    "body": "<a id='comment:30'>Comment 30:</a>\nAttachment: **[trac_9439_hyperbolic_space.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_hyperbolic_space.patch.gz)**\n\nI just attached a new version.  This one allows things like UHP.point(0) or KM.point((1/2, 1/2)), and similarly UHP.geodesic([start, finish]), and UHP.isometry(some_matrix).  \n\nThe class structure is somewhat complicated, but it's designed to make it easy to implement new models of 2D and 3D geometry.  Here's how it works:\n* The hyperbolic_model module contains data about each individual model, such as its name, the name of its isometry group, and dictionaries of maps to convert points and isometries to other models and so on.\n* the hyperbolic_point, hyperbolic_bdry_point, hyperbolic_geodesic, and hyperbolic_isometry modules all do what they sound like.  The methods in these modules are meant to be as general and abstract as possible.  Ideally they should barely have to be touched to implement new models of hyperbolic space.\n* hyperbolic_methods implements any methods that involve doing computations with actual numbers.  These methods have their own module because they are more likely to change if it turns out some computation is not efficient.  Also any model of hyperbolic geometry can choose to do computations in another model and convert the results.  In the current patch, every model does its computations in the upper half plane and then converts back to the desired model.  \n* hyperbolic_factory and model_factory are just factory patterns that make the whole abstract structure work.\n* finally, hyperbolic_interface is the module that contains the objects UHP, HM, KM, and PD that allow operations like UHP.point(1 + I).  These are just more pleasant user interfaces to methods in other classes.\n\nTo implement another 2D model, one mainly has to put the model information in hyperbolic_model and write down maps to all of the other models.  To implement a 3D model, one also has to implement hyperbolic_methods for at least one model (say the upper half space model).  In both cases there is a tiny amount of book keeping that must also be done in updating the factory and other modules with one or two lines of code.",
    "created_at": "2013-09-10T03:13:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83068",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:30'>Comment 30:</a>
Attachment: **[trac_9439_hyperbolic_space.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_hyperbolic_space.patch.gz)**

I just attached a new version.  This one allows things like UHP.point(0) or KM.point((1/2, 1/2)), and similarly UHP.geodesic([start, finish]), and UHP.isometry(some_matrix).  

The class structure is somewhat complicated, but it's designed to make it easy to implement new models of 2D and 3D geometry.  Here's how it works:
* The hyperbolic_model module contains data about each individual model, such as its name, the name of its isometry group, and dictionaries of maps to convert points and isometries to other models and so on.
* the hyperbolic_point, hyperbolic_bdry_point, hyperbolic_geodesic, and hyperbolic_isometry modules all do what they sound like.  The methods in these modules are meant to be as general and abstract as possible.  Ideally they should barely have to be touched to implement new models of hyperbolic space.
* hyperbolic_methods implements any methods that involve doing computations with actual numbers.  These methods have their own module because they are more likely to change if it turns out some computation is not efficient.  Also any model of hyperbolic geometry can choose to do computations in another model and convert the results.  In the current patch, every model does its computations in the upper half plane and then converts back to the desired model.  
* hyperbolic_factory and model_factory are just factory patterns that make the whole abstract structure work.
* finally, hyperbolic_interface is the module that contains the objects UHP, HM, KM, and PD that allow operations like UHP.point(1 + I).  These are just more pleasant user interfaces to methods in other classes.

To implement another 2D model, one mainly has to put the model information in hyperbolic_model and write down maps to all of the other models.  To implement a 3D model, one also has to implement hyperbolic_methods for at least one model (say the upper half space model).  In both cases there is a tiny amount of book keeping that must also be done in updating the factory and other modules with one or two lines of code.



---

archive/issue_comments_083069.json:
```json
{
    "body": "Updated demo of hyperbolic space.",
    "created_at": "2013-09-10T03:17:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83069",
    "user": "https://github.com/greglaun"
}
```

Updated demo of hyperbolic space.



---

archive/issue_comments_083070.json:
```json
{
    "body": "<a id='comment:31'>Comment 31:</a>\nAttachment: **[hyp_demo.2.sage.gz](https://github.com/sagemath/sage/files/ticket9439/hyp_demo.2.sage.gz)**\n\nI had occasion yesterday to use my most recent patch for actual computations that involve a lot of model changing, and I realized it's shamefully buggy/nonfunctioning.  I fixed a few of the obvious bugs but I'll add more tests before I repost to make sure that I haven't introduced other bugs.",
    "created_at": "2013-09-19T13:23:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83070",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:31'>Comment 31:</a>
Attachment: **[hyp_demo.2.sage.gz](https://github.com/sagemath/sage/files/ticket9439/hyp_demo.2.sage.gz)**

I had occasion yesterday to use my most recent patch for actual computations that involve a lot of model changing, and I realized it's shamefully buggy/nonfunctioning.  I fixed a few of the obvious bugs but I'll add more tests before I repost to make sure that I haven't introduced other bugs.



---

archive/issue_comments_083071.json:
```json
{
    "body": "Bug fixes and some new features.",
    "created_at": "2013-10-03T18:20:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83071",
    "user": "https://github.com/greglaun"
}
```

Bug fixes and some new features.



---

archive/issue_comments_083072.json:
```json
{
    "body": "Branch: **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)**",
    "created_at": "2013-11-21T00:50:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83072",
    "user": "https://github.com/greglaun"
}
```

Branch: **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)**



---

archive/issue_comments_083073.json:
```json
{
    "body": "<a id='comment:32'>Comment 32:</a>\nAttachment: **[trac_9439_10_03_2013.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_10_03_2013.patch.gz)**",
    "created_at": "2013-11-21T00:50:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83073",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:32'>Comment 32:</a>
Attachment: **[trac_9439_10_03_2013.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_10_03_2013.patch.gz)**



---

archive/issue_comments_083074.json:
```json
{
    "body": "Changed keywords from **hyperbolic geometry, Poincare disc, upper half plane, sd35** to **hyperbolic geometry, Poincare disc, upper half plane, Beltrami-Klein, hyperboloid model, sd35**",
    "created_at": "2013-11-21T00:50:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83074",
    "user": "https://github.com/greglaun"
}
```

Changed keywords from **hyperbolic geometry, Poincare disc, upper half plane, sd35** to **hyperbolic geometry, Poincare disc, upper half plane, Beltrami-Klein, hyperboloid model, sd35**



---

archive/issue_comments_083075.json:
```json
{
    "body": "Changed author from **Vincent Delecroix, Martin Raum** to **Vincent Delecroix, Martin Raum, Greg Laun**",
    "created_at": "2013-11-21T00:50:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83075",
    "user": "https://github.com/greglaun"
}
```

Changed author from **Vincent Delecroix, Martin Raum** to **Vincent Delecroix, Martin Raum, Greg Laun**



---

archive/issue_comments_083076.json:
```json
{
    "body": "Changed branch from **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)** to none",
    "created_at": "2013-11-21T01:15:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83076",
    "user": "https://github.com/greglaun"
}
```

Changed branch from **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)** to none



---

archive/issue_comments_083077.json:
```json
{
    "body": "This patch contains the previous ones as well as some changes to docstrings.",
    "created_at": "2013-11-21T01:17:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83077",
    "user": "https://github.com/greglaun"
}
```

This patch contains the previous ones as well as some changes to docstrings.



---

archive/issue_comments_083078.json:
```json
{
    "body": "<a id='comment:34'>Comment 34:</a>\nAttachment: **[trac_9439_11_20_2013.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_11_20_2013.patch.gz)**",
    "created_at": "2013-11-21T01:17:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83078",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:34'>Comment 34:</a>
Attachment: **[trac_9439_11_20_2013.patch.gz](https://github.com/sagemath/sage/files/ticket9439/trac_9439_11_20_2013.patch.gz)**



---

archive/issue_events_106981.json:
```json
{
    "actor": "https://github.com/greglaun",
    "created_at": "2013-11-21T01:17:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106981"
}
```



---

archive/issue_comments_083079.json:
```json
{
    "body": "<a id='comment:35'>Comment 35:</a>\nI uploaded a new patch.  It fixes some docbuild errors that I didn't catch before.  I am marking it as needs_review because I'd like to get some feedback on it.\n\nThere are several things I'd like to add, including\n* support for polygons, circles, and horocycles\n* the ability to return the SageManifolds manifold associated with each model\n* some models of hyperbolic 3 space\nBut I'd like each of these to have their own bug report/feature request so before I do work on them I'd like to get the current patch reviewed.\n\nIf it's easier for reviewers, I can try to submit a bare-bones patch (maybe with just the upper half plane model) to reduce the number of lines of code and then add everything else in incremental patches.",
    "created_at": "2013-11-21T01:23:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83079",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:35'>Comment 35:</a>
I uploaded a new patch.  It fixes some docbuild errors that I didn't catch before.  I am marking it as needs_review because I'd like to get some feedback on it.

There are several things I'd like to add, including
* support for polygons, circles, and horocycles
* the ability to return the SageManifolds manifold associated with each model
* some models of hyperbolic 3 space
But I'd like each of these to have their own bug report/feature request so before I do work on them I'd like to get the current patch reviewed.

If it's easier for reviewers, I can try to submit a bare-bones patch (maybe with just the upper half plane model) to reduce the number of lines of code and then add everything else in incremental patches.



---

archive/issue_comments_083080.json:
```json
{
    "body": "Branch: **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)**",
    "created_at": "2013-11-21T04:14:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83080",
    "user": "https://github.com/greglaun"
}
```

Branch: **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)**



---

archive/issue_comments_083081.json:
```json
{
    "body": "<a id='comment:37'>Comment 37:</a>\nNew commits:\n<table><tr><td>[bda7f78](https://github.com/sagemath/sagetrac-mirror/commit/bda7f78)</td><td><code>Re-commit of patches already on trac.</code></td></tr></table>\n",
    "created_at": "2013-11-21T16:23:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83081",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:37'>Comment 37:</a>
New commits:
<table><tr><td>[bda7f78](https://github.com/sagemath/sagetrac-mirror/commit/bda7f78)</td><td><code>Re-commit of patches already on trac.</code></td></tr></table>




---

archive/issue_comments_083082.json:
```json
{
    "body": "Commit: **[bda7f78](https://github.com/sagemath/sagetrac-mirror/commit/bda7f78dc5471984885bbbcc9f535412142dfe84)**",
    "created_at": "2013-11-21T16:23:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83082",
    "user": "https://github.com/pjbruin"
}
```

Commit: **[bda7f78](https://github.com/sagemath/sagetrac-mirror/commit/bda7f78dc5471984885bbbcc9f535412142dfe84)**



---

archive/issue_events_106982.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106982"
}
```



---

archive/issue_events_106983.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106983"
}
```



---

archive/issue_comments_083083.json:
```json
{
    "body": "<a id='comment:39'>Comment 39:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4675f7ecf61fa150974a07273d16892695b1bac0\">4675f7e</a></td><td><code>Fixed bug in fixed_point_set in hyperbolic_methods.  For UHP matrices, the base_ring is now always changed to RDF before eigenvector computations.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/556f3f7718fdd82e23245f65868d55ee506e45f7\">556f3f7</a></td><td><code>In the function fixed_point in the module hyperbolic methods for UHP (SL2R) matrices, catch an index</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/22773f0ef46e236ab2bd7a86a090957e01e67fc9\">22773f0</a></td><td><code>Fixed a bug in the drawing of complete geodesics in the Poincare disk model.</code></td></tr></table>\n",
    "created_at": "2014-02-28T02:37:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83083",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:39'>Comment 39:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4675f7ecf61fa150974a07273d16892695b1bac0">4675f7e</a></td><td><code>Fixed bug in fixed_point_set in hyperbolic_methods.  For UHP matrices, the base_ring is now always changed to RDF before eigenvector computations.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/556f3f7718fdd82e23245f65868d55ee506e45f7">556f3f7</a></td><td><code>In the function fixed_point in the module hyperbolic methods for UHP (SL2R) matrices, catch an index</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/22773f0ef46e236ab2bd7a86a090957e01e67fc9">22773f0</a></td><td><code>Fixed a bug in the drawing of complete geodesics in the Poincare disk model.</code></td></tr></table>




---

archive/issue_comments_083084.json:
```json
{
    "body": "Changed commit from **[bda7f78](https://github.com/sagemath/sagetrac-mirror/commit/bda7f78dc5471984885bbbcc9f535412142dfe84)** to **[22773f0](https://github.com/sagemath/sagetrac-mirror/commit/22773f0ef46e236ab2bd7a86a090957e01e67fc9)**",
    "created_at": "2014-02-28T02:37:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83084",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[bda7f78](https://github.com/sagemath/sagetrac-mirror/commit/bda7f78dc5471984885bbbcc9f535412142dfe84)** to **[22773f0](https://github.com/sagemath/sagetrac-mirror/commit/22773f0ef46e236ab2bd7a86a090957e01e67fc9)**



---

archive/issue_comments_083085.json:
```json
{
    "body": "<a id='comment:40'>Comment 40:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c04c17cbdc4a5cb20591176200a5ba7a49a2a379\">c04c17c</a></td><td><code>Fixed bug in orientation-reversing isometries.</code></td></tr></table>\n",
    "created_at": "2014-03-16T20:01:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83085",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:40'>Comment 40:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c04c17cbdc4a5cb20591176200a5ba7a49a2a379">c04c17c</a></td><td><code>Fixed bug in orientation-reversing isometries.</code></td></tr></table>




---

archive/issue_comments_083086.json:
```json
{
    "body": "Changed commit from **[22773f0](https://github.com/sagemath/sagetrac-mirror/commit/22773f0ef46e236ab2bd7a86a090957e01e67fc9)** to **[c04c17c](https://github.com/sagemath/sagetrac-mirror/commit/c04c17cbdc4a5cb20591176200a5ba7a49a2a379)**",
    "created_at": "2014-03-16T20:01:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83086",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[22773f0](https://github.com/sagemath/sagetrac-mirror/commit/22773f0ef46e236ab2bd7a86a090957e01e67fc9)** to **[c04c17c](https://github.com/sagemath/sagetrac-mirror/commit/c04c17cbdc4a5cb20591176200a5ba7a49a2a379)**



---

archive/issue_comments_083087.json:
```json
{
    "body": "<a id='comment:41'>Comment 41:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/354158f495a556c599cb6170276658762a0118c3\">354158f</a></td><td><code>Points are held as vectors rather than as tuples.  This avoids some unwanted side effects.</code></td></tr></table>\n",
    "created_at": "2014-03-23T22:11:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83087",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:41'>Comment 41:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/354158f495a556c599cb6170276658762a0118c3">354158f</a></td><td><code>Points are held as vectors rather than as tuples.  This avoids some unwanted side effects.</code></td></tr></table>




---

archive/issue_comments_083088.json:
```json
{
    "body": "Changed commit from **[c04c17c](https://github.com/sagemath/sagetrac-mirror/commit/c04c17cbdc4a5cb20591176200a5ba7a49a2a379)** to **[354158f](https://github.com/sagemath/sagetrac-mirror/commit/354158f495a556c599cb6170276658762a0118c3)**",
    "created_at": "2014-03-23T22:11:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83088",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c04c17c](https://github.com/sagemath/sagetrac-mirror/commit/c04c17cbdc4a5cb20591176200a5ba7a49a2a379)** to **[354158f](https://github.com/sagemath/sagetrac-mirror/commit/354158f495a556c599cb6170276658762a0118c3)**



---

archive/issue_events_106984.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106984"
}
```



---

archive/issue_events_106985.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106985"
}
```



---

archive/issue_events_106986.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-16T14:05:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106986"
}
```



---

archive/issue_events_106987.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-16T14:05:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106987"
}
```



---

archive/issue_comments_083089.json:
```json
{
    "body": "<a id='comment:43'>Comment 43:</a>\npatchbot fails with `make doc`:\n\n```\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/index.rst:47: ERROR: Unexpected indentation.\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/index.rst:56: WARNING: Literal block ends without a blank line; unexpected unindent.\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/index.rst:56: SEVERE: Missing matching underline for section title overline.\n[geometry ] =======\n[geometry ] sage/geometry/hyperplane_arrangement/arrangement\n[geometry ] sage/geometry/hyperplane_arrangement/library\n[geometry ] /scratch/sage/local/lib/python2.7/site-packages/sage/geometry/hyperbolic_space/hyperbolic_model.py:docstring of sage.geometry.hyperbolic_space.hyperbolic_model:14: ERROR: Unexpected indentation.\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_bdry_point.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_geodesic.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_interface.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_isometry.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_methods.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_model.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_point.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperplane_arrangement/affine_subspace.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperplane_arrangement/arrangement.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperplane_arrangement/hyperplane.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperplane_arrangement/library.rst:: WARNING: document isn't included in any toctree\n[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/linear_expression.rst:: WARNING: document isn't included in any toctree\nError building the documentation.\nTraceback (most recent call last):\n  File \"/scratch/sage/src/doc/common/builder.py\", line 1477, in <module>\n    getattr(get_builder(name), type)()\n  File \"/scratch/sage/src/doc/common/builder.py\", line 276, in _wrapper\n    getattr(get_builder(document), 'inventory')(*args, **kwds)\n  File \"/scratch/sage/src/doc/common/builder.py\", line 487, in _wrapper\n    x.get(99999)\n  File \"/scratch/sage/local/lib/python/multiprocessing/pool.py\", line 554, in get\n    raise self._value\nOSError: [geometry ] /scratch/sage/src/doc/en/reference/geometry/index.rst:47: ERROR: Unexpected indentation.\n\nmake: *** [doc-html] Error 1\n```",
    "created_at": "2014-05-16T14:05:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83089",
    "user": "https://github.com/rwst"
}
```

<a id='comment:43'>Comment 43:</a>
patchbot fails with `make doc`:

```
[geometry ] /scratch/sage/src/doc/en/reference/geometry/index.rst:47: ERROR: Unexpected indentation.
[geometry ] /scratch/sage/src/doc/en/reference/geometry/index.rst:56: WARNING: Literal block ends without a blank line; unexpected unindent.
[geometry ] /scratch/sage/src/doc/en/reference/geometry/index.rst:56: SEVERE: Missing matching underline for section title overline.
[geometry ] =======
[geometry ] sage/geometry/hyperplane_arrangement/arrangement
[geometry ] sage/geometry/hyperplane_arrangement/library
[geometry ] /scratch/sage/local/lib/python2.7/site-packages/sage/geometry/hyperbolic_space/hyperbolic_model.py:docstring of sage.geometry.hyperbolic_space.hyperbolic_model:14: ERROR: Unexpected indentation.
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_bdry_point.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_geodesic.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_interface.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_isometry.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_methods.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_model.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperbolic_space/hyperbolic_point.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperplane_arrangement/affine_subspace.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperplane_arrangement/arrangement.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperplane_arrangement/hyperplane.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/hyperplane_arrangement/library.rst:: WARNING: document isn't included in any toctree
[geometry ] /scratch/sage/src/doc/en/reference/geometry/sage/geometry/linear_expression.rst:: WARNING: document isn't included in any toctree
Error building the documentation.
Traceback (most recent call last):
  File "/scratch/sage/src/doc/common/builder.py", line 1477, in <module>
    getattr(get_builder(name), type)()
  File "/scratch/sage/src/doc/common/builder.py", line 276, in _wrapper
    getattr(get_builder(document), 'inventory')(*args, **kwds)
  File "/scratch/sage/src/doc/common/builder.py", line 487, in _wrapper
    x.get(99999)
  File "/scratch/sage/local/lib/python/multiprocessing/pool.py", line 554, in get
    raise self._value
OSError: [geometry ] /scratch/sage/src/doc/en/reference/geometry/index.rst:47: ERROR: Unexpected indentation.

make: *** [doc-html] Error 1
```



---

archive/issue_comments_083090.json:
```json
{
    "body": "<a id='comment:44'>Comment 44:</a>\nhere is new branch, with working doc. I have made small corrections in the doc.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d6ed4dcdba175839a59230ef5c58d071ada86e7f\">d6ed4dc</a></td><td><code>Merge branch 'u/glaun/ticket/9439' of ssh://trac.sagemath.org:22/sage into 9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7371810de8f1fc99cfa9cc230a2892a695ab9d99\">7371810</a></td><td><code>trac #9439 work on doc</code></td></tr></table>\n",
    "created_at": "2014-06-25T19:47:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83090",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:44'>Comment 44:</a>
here is new branch, with working doc. I have made small corrections in the doc.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d6ed4dcdba175839a59230ef5c58d071ada86e7f">d6ed4dc</a></td><td><code>Merge branch 'u/glaun/ticket/9439' of ssh://trac.sagemath.org:22/sage into 9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7371810de8f1fc99cfa9cc230a2892a695ab9d99">7371810</a></td><td><code>trac #9439 work on doc</code></td></tr></table>




---

archive/issue_events_106988.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-06-25T19:47:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106988"
}
```



---

archive/issue_events_106989.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-06-25T19:47:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106989"
}
```



---

archive/issue_comments_083091.json:
```json
{
    "body": "Changed commit from **[354158f](https://github.com/sagemath/sagetrac-mirror/commit/354158f495a556c599cb6170276658762a0118c3)** to **[7371810](https://github.com/sagemath/sagetrac-mirror/commit/7371810de8f1fc99cfa9cc230a2892a695ab9d99)**",
    "created_at": "2014-06-25T19:47:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83091",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[354158f](https://github.com/sagemath/sagetrac-mirror/commit/354158f495a556c599cb6170276658762a0118c3)** to **[7371810](https://github.com/sagemath/sagetrac-mirror/commit/7371810de8f1fc99cfa9cc230a2892a695ab9d99)**



---

archive/issue_comments_083092.json:
```json
{
    "body": "Changed branch from **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)** to **[public/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/9439)**",
    "created_at": "2014-06-25T19:47:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83092",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)** to **[public/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/9439)**



---

archive/issue_comments_083093.json:
```json
{
    "body": "Changed commit from **[7371810](https://github.com/sagemath/sagetrac-mirror/commit/7371810de8f1fc99cfa9cc230a2892a695ab9d99)** to **[fe22a75](https://github.com/sagemath/sagetrac-mirror/commit/fe22a753e12f7779c4ab70dd733a917915fa84f4)**",
    "created_at": "2014-06-26T04:23:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83093",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7371810](https://github.com/sagemath/sagetrac-mirror/commit/7371810de8f1fc99cfa9cc230a2892a695ab9d99)** to **[fe22a75](https://github.com/sagemath/sagetrac-mirror/commit/fe22a753e12f7779c4ab70dd733a917915fa84f4)**



---

archive/issue_comments_083094.json:
```json
{
    "body": "<a id='comment:45'>Comment 45:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/23181c4c7a4248fb5fce1c799d01f8e1bc7cd54c\">23181c4</a></td><td><code>Merge branch 'u/glaun/ticket/9439' of trac.sagemath.org:sage into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/24fc77c947722ac90a530c4e0984ab41b5063513\">24fc77c</a></td><td><code>First pass of review changes.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a64983a7d7b70d708719bc5cf3013f9e05e4719\">2a64983</a></td><td><code>Some additional review changes.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/95dd29e51a3e4ceda0de604b2f31a8423ae1dd99\">95dd29e</a></td><td><code>Merge branch 'develop' into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f1f8302b8547537c7227cf7bc1836a55d1df569f\">f1f8302</a></td><td><code>Merge branch 'develop' into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/656680f86c90212a386803654602862c1129e5ff\">656680f</a></td><td><code>Merge branch 'develop' into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/802377954e8c1f5a6d69381102324bdf1c4930a8\">8023779</a></td><td><code>Merge branch 'public/ticket/9439' of trac.sagemath.org:sage into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fe22a753e12f7779c4ab70dd733a917915fa84f4\">fe22a75</a></td><td><code>Some tweaks from the merge.</code></td></tr></table>\n",
    "created_at": "2014-06-26T04:23:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83094",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:45'>Comment 45:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/23181c4c7a4248fb5fce1c799d01f8e1bc7cd54c">23181c4</a></td><td><code>Merge branch 'u/glaun/ticket/9439' of trac.sagemath.org:sage into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/24fc77c947722ac90a530c4e0984ab41b5063513">24fc77c</a></td><td><code>First pass of review changes.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a64983a7d7b70d708719bc5cf3013f9e05e4719">2a64983</a></td><td><code>Some additional review changes.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/95dd29e51a3e4ceda0de604b2f31a8423ae1dd99">95dd29e</a></td><td><code>Merge branch 'develop' into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f1f8302b8547537c7227cf7bc1836a55d1df569f">f1f8302</a></td><td><code>Merge branch 'develop' into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/656680f86c90212a386803654602862c1129e5ff">656680f</a></td><td><code>Merge branch 'develop' into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/802377954e8c1f5a6d69381102324bdf1c4930a8">8023779</a></td><td><code>Merge branch 'public/ticket/9439' of trac.sagemath.org:sage into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fe22a753e12f7779c4ab70dd733a917915fa84f4">fe22a75</a></td><td><code>Some tweaks from the merge.</code></td></tr></table>




---

archive/issue_comments_083095.json:
```json
{
    "body": "<a id='comment:46'>Comment 46:</a>\nI've made some convention fixes and cleanup. I also have the following comments:\n\n- I'm not completely sure the framework that is currently there shouldn't be restructured as abstract base classes as you're not creating an army of hyperbolic spaces, just one in each type with data from a \"database\". Moreover there seems to be a lot of redundancy.\n\n- I think too much is getting imported into the global namespace (ex. `UHP`) which could have potential conflicts and in many ways is a strange acronym. It would probably be better to have them located via `HyperbolicSpace` (which would be a factory function or the base class with a `__classcall__`) and/or a catalog import `hyperbolic_space`.\n\nI will have to think more about the first one. The second should be done at some point unless there is a very good argument otherwise IMO.",
    "created_at": "2014-06-26T04:32:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83095",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:46'>Comment 46:</a>
I've made some convention fixes and cleanup. I also have the following comments:

- I'm not completely sure the framework that is currently there shouldn't be restructured as abstract base classes as you're not creating an army of hyperbolic spaces, just one in each type with data from a "database". Moreover there seems to be a lot of redundancy.

- I think too much is getting imported into the global namespace (ex. `UHP`) which could have potential conflicts and in many ways is a strange acronym. It would probably be better to have them located via `HyperbolicSpace` (which would be a factory function or the base class with a `__classcall__`) and/or a catalog import `hyperbolic_space`.

I will have to think more about the first one. The second should be done at some point unless there is a very good argument otherwise IMO.



---

archive/issue_comments_083096.json:
```json
{
    "body": "<a id='comment:47'>Comment 47:</a>\nI'm glad to see other people working on this ticket!  I tried to pull the updates and rebuild sage, but it won't build. It may be a few days before I have time to debug the build process and get back to work on this ticket.\n\nIn response to your points, I'm fine with a restructure as long as we talk it through first.  \n\n* Its design now uses abstract base classes for geodesics, isometries, and points.  In the current organization, the \"database\" of facts about the underlying space are in hyperbolic_model, divided by model.  The \"database\" of calculation methods are in hyperbolic_methods, of which only the methods in the upper half plane are implemented.  I think these should be separate because it allows someone to implement a model without ever having to see the details of how calculations are done. I suppose we could put the database somewhere else, like in a dictionary or database if that's wise.  I don't have strong opinions about that.  Is the organization unclear?  Let me know if you have specific ideas in mind, or even if you just want to discuss overall design philosophy.\n\n* I wasn't aware of `__classcall__` when I wrote it.  I was basically just going for doing a clean design.  It seems like `__classcall__` might be a modularity violation?  I'll look into it more.\n\n* It sounds like you're objecting to importing the interfaces into the global namespace.  I'm fine with getting rid of those imports.  We could do something like\n\n```\nUHP = HyperbolicPlane(\"upper half plane\")\nUHP.point(I)\n```\nand then only import HyperbolicPlane.  Is that the sort of thing you had in mind?  Or maybe something like\n\n```\np = HyperbolicPoint(I, \"upper half plane\") # Create point 0 + I.\n```",
    "created_at": "2014-06-28T12:26:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83096",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:47'>Comment 47:</a>
I'm glad to see other people working on this ticket!  I tried to pull the updates and rebuild sage, but it won't build. It may be a few days before I have time to debug the build process and get back to work on this ticket.

In response to your points, I'm fine with a restructure as long as we talk it through first.  

* Its design now uses abstract base classes for geodesics, isometries, and points.  In the current organization, the "database" of facts about the underlying space are in hyperbolic_model, divided by model.  The "database" of calculation methods are in hyperbolic_methods, of which only the methods in the upper half plane are implemented.  I think these should be separate because it allows someone to implement a model without ever having to see the details of how calculations are done. I suppose we could put the database somewhere else, like in a dictionary or database if that's wise.  I don't have strong opinions about that.  Is the organization unclear?  Let me know if you have specific ideas in mind, or even if you just want to discuss overall design philosophy.

* I wasn't aware of `__classcall__` when I wrote it.  I was basically just going for doing a clean design.  It seems like `__classcall__` might be a modularity violation?  I'll look into it more.

* It sounds like you're objecting to importing the interfaces into the global namespace.  I'm fine with getting rid of those imports.  We could do something like

```
UHP = HyperbolicPlane("upper half plane")
UHP.point(I)
```
and then only import HyperbolicPlane.  Is that the sort of thing you had in mind?  Or maybe something like

```
p = HyperbolicPoint(I, "upper half plane") # Create point 0 + I.
```



---

archive/issue_comments_083097.json:
```json
{
    "body": "Changed commit from **[fe22a75](https://github.com/sagemath/sagetrac-mirror/commit/fe22a753e12f7779c4ab70dd733a917915fa84f4)** to **[3a04339](https://github.com/sagemath/sagetrac-mirror/commit/3a043392d7b6c21a186303c4990204e06da00a6c)**",
    "created_at": "2014-07-29T20:21:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83097",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[fe22a75](https://github.com/sagemath/sagetrac-mirror/commit/fe22a753e12f7779c4ab70dd733a917915fa84f4)** to **[3a04339](https://github.com/sagemath/sagetrac-mirror/commit/3a043392d7b6c21a186303c4990204e06da00a6c)**



---

archive/issue_comments_083098.json:
```json
{
    "body": "<a id='comment:48'>Comment 48:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3a043392d7b6c21a186303c4990204e06da00a6c\">3a04339</a></td><td><code>Fixed failing test.</code></td></tr></table>\n",
    "created_at": "2014-07-29T20:21:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83098",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:48'>Comment 48:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3a043392d7b6c21a186303c4990204e06da00a6c">3a04339</a></td><td><code>Fixed failing test.</code></td></tr></table>




---

archive/issue_events_106990.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106990"
}
```



---

archive/issue_events_106991.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106991"
}
```



---

archive/issue_comments_083099.json:
```json
{
    "body": "Changed commit from **[3a04339](https://github.com/sagemath/sagetrac-mirror/commit/3a043392d7b6c21a186303c4990204e06da00a6c)** to **[58947b5](https://github.com/sagemath/sagetrac-mirror/commit/58947b55679d1bd4b5b7c3d0804094653a17e830)**",
    "created_at": "2014-08-20T08:39:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83099",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3a04339](https://github.com/sagemath/sagetrac-mirror/commit/3a043392d7b6c21a186303c4990204e06da00a6c)** to **[58947b5](https://github.com/sagemath/sagetrac-mirror/commit/58947b55679d1bd4b5b7c3d0804094653a17e830)**



---

archive/issue_comments_083100.json:
```json
{
    "body": "<a id='comment:50'>Comment 50:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7aeb3e63a9116eeb4ea59b54c79a49a0c594fada\">7aeb3e6</a></td><td><code>Merge branch 'public/ticket/9439' of ssh://trac.sagemath.org:22/sage into 9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/58947b55679d1bd4b5b7c3d0804094653a17e830\">58947b5</a></td><td><code>trac #9439 fixing index file</code></td></tr></table>\n",
    "created_at": "2014-08-20T08:39:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83100",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:50'>Comment 50:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7aeb3e63a9116eeb4ea59b54c79a49a0c594fada">7aeb3e6</a></td><td><code>Merge branch 'public/ticket/9439' of ssh://trac.sagemath.org:22/sage into 9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/58947b55679d1bd4b5b7c3d0804094653a17e830">58947b5</a></td><td><code>trac #9439 fixing index file</code></td></tr></table>




---

archive/issue_comments_083101.json:
```json
{
    "body": "Changed commit from **[58947b5](https://github.com/sagemath/sagetrac-mirror/commit/58947b55679d1bd4b5b7c3d0804094653a17e830)** to **[b7fa036](https://github.com/sagemath/sagetrac-mirror/commit/b7fa036b162e09cc5500c0fc67633dcc2b9e6786)**",
    "created_at": "2014-08-31T19:58:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83101",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[58947b5](https://github.com/sagemath/sagetrac-mirror/commit/58947b55679d1bd4b5b7c3d0804094653a17e830)** to **[b7fa036](https://github.com/sagemath/sagetrac-mirror/commit/b7fa036b162e09cc5500c0fc67633dcc2b9e6786)**



---

archive/issue_comments_083102.json:
```json
{
    "body": "<a id='comment:51'>Comment 51:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/22f83bc4b3dc628334c1be2ec2da2ce675da4395\">22f83bc</a></td><td><code>Merge branch 'public/ticket/9439' of ssh://trac.sagemath.org:22/sage into 9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b7fa036b162e09cc5500c0fc67633dcc2b9e6786\">b7fa036</a></td><td><code>trac #9439 make sure that doc builds + minor doc corrections</code></td></tr></table>\n",
    "created_at": "2014-08-31T19:58:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83102",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:51'>Comment 51:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/22f83bc4b3dc628334c1be2ec2da2ce675da4395">22f83bc</a></td><td><code>Merge branch 'public/ticket/9439' of ssh://trac.sagemath.org:22/sage into 9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b7fa036b162e09cc5500c0fc67633dcc2b9e6786">b7fa036</a></td><td><code>trac #9439 make sure that doc builds + minor doc corrections</code></td></tr></table>




---

archive/issue_comments_083103.json:
```json
{
    "body": "Changed author from **Vincent Delecroix, Martin Raum, Greg Laun** to **Vincent Delecroix, Martin Raum, Greg Laun, Travis Scrimshaw**",
    "created_at": "2014-09-14T07:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83103",
    "user": "https://github.com/tscrim"
}
```

Changed author from **Vincent Delecroix, Martin Raum, Greg Laun** to **Vincent Delecroix, Martin Raum, Greg Laun, Travis Scrimshaw**



---

archive/issue_comments_083104.json:
```json
{
    "body": "<a id='comment:52'>Comment 52:</a>\nOkay, so I've done a major refactoring in which I move everything into Sage's category/parent/element framework. In doing so, I figured out why all of the `@classmethod` bothered me (beyond the somewhat maze-like path of methods); it was emulating a singleton pattern using classes rather than instances. To me, this is a bad practice, so I'm now using `UniqueRepresentation` (although this would benefit from switching to #15247). So here's the other major points and structure.\n\n* The models are parents using the `WithRealizations` framework, and the points are the elements. I've setup the maps between the models are coercions and have methods to also translate the geodesics and isometries. Geodesics are just `SageObject`'s, but isometries are morphisms.\n* I've changed the naming of some of the methods to better reflect their operation (in particular, `orientation_preserving` to `preserves_orientation`).\n* I've kept most of the ease of implementing new models but by using the coercions instead of the methods class.\n* I've implemented a custom hash for isometries since compared equal but had different hashes.\n* More strict handling of points vs. coordinates and isometries vs. matrices. This makes the programmers/users take more care about where things belong and what one can do with them. This adds a little more burden of wrapping and unwrapping, but it shouldn't make things much (significant) difference in timings (I didn't check). Yet I would argue this is a better way of doing things.\n* Removed the `**graphic_options` from things like `perpendicular_bisector` because it seemed out of place and makes it easier to keep things consistent (plus 2 distinct steps for distinct operations).\n* There is only one big lazy import needed, and that is for `HyperbolicPlane`, which is the global entry point. I've added a method `HyperbolicSpace` for the future, but I didn't import it into the global namespace.\n\nQuestions:\n\n* If given an isometry, does one ask if it \"is orientation preserving\" or \"preserves orientation\"?\n* Should we keep with the `get_*` (ex. `get_point`) or just drop it to `*` (ex. `point` resp.)? I'm somewhat in favor of the way it is now, but I don't have a strong opinion.\n\nTodo for positive review:\n\n* Add doctests to remaining `hyperbolic_coercion.py` methods.\n* Fix doctest failures coming from `_to_std_geod`. I'm pretty sure it is not suppose to return an isometry (which is what causes the doctest failures). Please advise.\n\nTodos for the future:\n\n* Make custom endosets for each model with the isometries as elements and coercions implemented once homsets work with the coersion model (#14279 and possibly others).\n* Implement a category for metric spaces.\n\nI was quite impressed with the code and the interactions between everything and hope we can get more of (hyperbolic) geometry into Sage. So please test out, look over, and play with the refactored version (making sure everything works as before).\n\nPS - Phew, finally had some time to sit down to think and work through this. Sorry it took so long.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a77a115a2031646a47884db894cc5804e248716c\">a77a115</a></td><td><code>Merge branch 'public/ticket/9439' of trac.sagemath.org:sage into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/87f703a2769d26d739c60861e63d0870b34a5635\">87f703a</a></td><td><code>Some more trivial cleanup.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2759b74b3ba6d87c0c3b1cab81fd509d76b552d4\">2759b74</a></td><td><code>Changed global interface to HyperbolicPlane.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0572434edc03d53f81b087d106fcc5584de248c5\">0572434</a></td><td><code>About 85% of the refactoring is done and basic functionality is working.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b203a5390284a678a4cf79c44f3f3c368c1a9fe9\">b203a53</a></td><td><code>Some more refactoring and starting to fix doctests.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5747604de49da1b61dac10f7b42eae3e11f6c3e2\">5747604</a></td><td><code>Merge branch 'develop' into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/42f8a7d4b2a1942a558d86cc31fd6106b8bed92f\">42f8a7d</a></td><td><code>More cleanup work.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/872e453d3b274ee165e9041b4ba0d5751f377f7c\">872e453</a></td><td><code>Finished refactoring and most doctest pass.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/37287f35acfced2696dff3126e36cba8578a96f4\">37287f3</a></td><td><code>Some more doctest and bug fixes.</code></td></tr></table>\n",
    "created_at": "2014-09-14T07:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83104",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:52'>Comment 52:</a>
Okay, so I've done a major refactoring in which I move everything into Sage's category/parent/element framework. In doing so, I figured out why all of the `@classmethod` bothered me (beyond the somewhat maze-like path of methods); it was emulating a singleton pattern using classes rather than instances. To me, this is a bad practice, so I'm now using `UniqueRepresentation` (although this would benefit from switching to #15247). So here's the other major points and structure.

* The models are parents using the `WithRealizations` framework, and the points are the elements. I've setup the maps between the models are coercions and have methods to also translate the geodesics and isometries. Geodesics are just `SageObject`'s, but isometries are morphisms.
* I've changed the naming of some of the methods to better reflect their operation (in particular, `orientation_preserving` to `preserves_orientation`).
* I've kept most of the ease of implementing new models but by using the coercions instead of the methods class.
* I've implemented a custom hash for isometries since compared equal but had different hashes.
* More strict handling of points vs. coordinates and isometries vs. matrices. This makes the programmers/users take more care about where things belong and what one can do with them. This adds a little more burden of wrapping and unwrapping, but it shouldn't make things much (significant) difference in timings (I didn't check). Yet I would argue this is a better way of doing things.
* Removed the `**graphic_options` from things like `perpendicular_bisector` because it seemed out of place and makes it easier to keep things consistent (plus 2 distinct steps for distinct operations).
* There is only one big lazy import needed, and that is for `HyperbolicPlane`, which is the global entry point. I've added a method `HyperbolicSpace` for the future, but I didn't import it into the global namespace.

Questions:

* If given an isometry, does one ask if it "is orientation preserving" or "preserves orientation"?
* Should we keep with the `get_*` (ex. `get_point`) or just drop it to `*` (ex. `point` resp.)? I'm somewhat in favor of the way it is now, but I don't have a strong opinion.

Todo for positive review:

* Add doctests to remaining `hyperbolic_coercion.py` methods.
* Fix doctest failures coming from `_to_std_geod`. I'm pretty sure it is not suppose to return an isometry (which is what causes the doctest failures). Please advise.

Todos for the future:

* Make custom endosets for each model with the isometries as elements and coercions implemented once homsets work with the coersion model (#14279 and possibly others).
* Implement a category for metric spaces.

I was quite impressed with the code and the interactions between everything and hope we can get more of (hyperbolic) geometry into Sage. So please test out, look over, and play with the refactored version (making sure everything works as before).

PS - Phew, finally had some time to sit down to think and work through this. Sorry it took so long.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a77a115a2031646a47884db894cc5804e248716c">a77a115</a></td><td><code>Merge branch 'public/ticket/9439' of trac.sagemath.org:sage into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/87f703a2769d26d739c60861e63d0870b34a5635">87f703a</a></td><td><code>Some more trivial cleanup.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2759b74b3ba6d87c0c3b1cab81fd509d76b552d4">2759b74</a></td><td><code>Changed global interface to HyperbolicPlane.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0572434edc03d53f81b087d106fcc5584de248c5">0572434</a></td><td><code>About 85% of the refactoring is done and basic functionality is working.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b203a5390284a678a4cf79c44f3f3c368c1a9fe9">b203a53</a></td><td><code>Some more refactoring and starting to fix doctests.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5747604de49da1b61dac10f7b42eae3e11f6c3e2">5747604</a></td><td><code>Merge branch 'develop' into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/42f8a7d4b2a1942a558d86cc31fd6106b8bed92f">42f8a7d</a></td><td><code>More cleanup work.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/872e453d3b274ee165e9041b4ba0d5751f377f7c">872e453</a></td><td><code>Finished refactoring and most doctest pass.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/37287f35acfced2696dff3126e36cba8578a96f4">37287f3</a></td><td><code>Some more doctest and bug fixes.</code></td></tr></table>




---

archive/issue_comments_083105.json:
```json
{
    "body": "Changed branch from **[public/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/9439)** to **[public/geometry/hyperbolic-9439](https://github.com/sagemath/sagetrac-mirror/tree/public/geometry/hyperbolic-9439)**",
    "created_at": "2014-09-14T07:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83105",
    "user": "https://github.com/tscrim"
}
```

Changed branch from **[public/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/9439)** to **[public/geometry/hyperbolic-9439](https://github.com/sagemath/sagetrac-mirror/tree/public/geometry/hyperbolic-9439)**



---

archive/issue_events_106992.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-09-14T07:17:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106992"
}
```



---

archive/issue_events_106993.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-09-14T07:17:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106993"
}
```



---

archive/issue_comments_083106.json:
```json
{
    "body": "Changed reviewer from **Johan Bosman** to **Johan Bosman, Travis Scrimshaw**",
    "created_at": "2014-09-14T07:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83106",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Johan Bosman** to **Johan Bosman, Travis Scrimshaw**



---

archive/issue_comments_083107.json:
```json
{
    "body": "Changed commit from **[b7fa036](https://github.com/sagemath/sagetrac-mirror/commit/b7fa036b162e09cc5500c0fc67633dcc2b9e6786)** to **[37287f3](https://github.com/sagemath/sagetrac-mirror/commit/37287f35acfced2696dff3126e36cba8578a96f4)**",
    "created_at": "2014-09-14T07:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83107",
    "user": "https://github.com/tscrim"
}
```

Changed commit from **[b7fa036](https://github.com/sagemath/sagetrac-mirror/commit/b7fa036b162e09cc5500c0fc67633dcc2b9e6786)** to **[37287f3](https://github.com/sagemath/sagetrac-mirror/commit/37287f35acfced2696dff3126e36cba8578a96f4)**



---

archive/issue_comments_083108.json:
```json
{
    "body": "Changed branch from **[public/geometry/hyperbolic-9439](https://github.com/sagemath/sagetrac-mirror/tree/public/geometry/hyperbolic-9439)** to **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)**",
    "created_at": "2014-09-30T01:07:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83108",
    "user": "https://github.com/greglaun"
}
```

Changed branch from **[public/geometry/hyperbolic-9439](https://github.com/sagemath/sagetrac-mirror/tree/public/geometry/hyperbolic-9439)** to **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)**



---

archive/issue_comments_083109.json:
```json
{
    "body": "Changed commit from **[37287f3](https://github.com/sagemath/sagetrac-mirror/commit/37287f35acfced2696dff3126e36cba8578a96f4)** to **[222c2ad](https://github.com/sagemath/sagetrac-mirror/commit/222c2ad0611bc609c7a24f934a8fe6cef5cc6792)**",
    "created_at": "2014-09-30T01:10:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83109",
    "user": "https://github.com/greglaun"
}
```

Changed commit from **[37287f3](https://github.com/sagemath/sagetrac-mirror/commit/37287f35acfced2696dff3126e36cba8578a96f4)** to **[222c2ad](https://github.com/sagemath/sagetrac-mirror/commit/222c2ad0611bc609c7a24f934a8fe6cef5cc6792)**



---

archive/issue_comments_083110.json:
```json
{
    "body": "<a id='comment:54'>Comment 54:</a>\nThanks for your help on the code.  I'm glad you were able to get things working with the sage parent/element/etc framework.  I wasn't quite sure how to do that.\n\nThere are still bugs that I know about and have fixed in other branches, but which haven't been merged into this branch yet.  I'll hopefully have time to commit a patch that fixes the remaining bugs that I know about in the next week or two.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c612e31909f31f2dedd641e6fa10fee94635ea8c\">c612e31</a></td><td><code>Fixed two or three test failures in hyperbolic_geodesic.py.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/222c2ad0611bc609c7a24f934a8fe6cef5cc6792\">222c2ad</a></td><td><code>All tests now pass for hyperbolic_geodesic.py.  In particular, _to_std_geod tests are fixed.</code></td></tr></table>\n",
    "created_at": "2014-09-30T01:10:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83110",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:54'>Comment 54:</a>
Thanks for your help on the code.  I'm glad you were able to get things working with the sage parent/element/etc framework.  I wasn't quite sure how to do that.

There are still bugs that I know about and have fixed in other branches, but which haven't been merged into this branch yet.  I'll hopefully have time to commit a patch that fixes the remaining bugs that I know about in the next week or two.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c612e31909f31f2dedd641e6fa10fee94635ea8c">c612e31</a></td><td><code>Fixed two or three test failures in hyperbolic_geodesic.py.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/222c2ad0611bc609c7a24f934a8fe6cef5cc6792">222c2ad</a></td><td><code>All tests now pass for hyperbolic_geodesic.py.  In particular, _to_std_geod tests are fixed.</code></td></tr></table>




---

archive/issue_comments_083111.json:
```json
{
    "body": "<a id='comment:55'>Comment 55:</a>\nReplying to [@greglaun](#comment%3A54):\n> Thanks for your help on the code.  I'm glad you were able to get things working with the sage parent/element/etc framework.  I wasn't quite sure how to do that.\n\nNot a problem. I hope a lot of the design is clear and makes sense. There's still a general question of what to do with subsets (here geodesics) of a given set (the entire plane) in terms of parents (so we can induce coercions on the subsets [geodesics]), but that is a separate, quite large, and probably thorny issue that we can't solve here.\n\n> There are still bugs that I know about and have fixed in other branches, but which haven't been merged into this branch yet.  I'll hopefully have time to commit a patch that fixes the remaining bugs that I know about in the next week or two.\n\nLet me know as things get merged in and I can review those changes so we can get this included into Sage.\n\nFYI, a typo got introduced in `c612e31`:\n\n```diff\n@@ -528,7 +528,7 @@ class HyperbolicGeodesic(SageObject):\n          \"\"\"\n          return self._cached_geodesic.reflection_involution().to_model(self._model)\n\n-     def common_perpendicular(self, other):\n+     def common_perpendicula(self, other):\n          r\"\"\"\n          Return the unique hyperbolic geodesic perpendicular to two given\n          geodesics, if such a geodesic exists. If none exists, raise a\n```",
    "created_at": "2014-09-30T04:23:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83111",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:55'>Comment 55:</a>
Replying to [@greglaun](#comment%3A54):
> Thanks for your help on the code.  I'm glad you were able to get things working with the sage parent/element/etc framework.  I wasn't quite sure how to do that.

Not a problem. I hope a lot of the design is clear and makes sense. There's still a general question of what to do with subsets (here geodesics) of a given set (the entire plane) in terms of parents (so we can induce coercions on the subsets [geodesics]), but that is a separate, quite large, and probably thorny issue that we can't solve here.

> There are still bugs that I know about and have fixed in other branches, but which haven't been merged into this branch yet.  I'll hopefully have time to commit a patch that fixes the remaining bugs that I know about in the next week or two.

Let me know as things get merged in and I can review those changes so we can get this included into Sage.

FYI, a typo got introduced in `c612e31`:

```diff
@@ -528,7 +528,7 @@ class HyperbolicGeodesic(SageObject):
          """
          return self._cached_geodesic.reflection_involution().to_model(self._model)

-     def common_perpendicular(self, other):
+     def common_perpendicula(self, other):
          r"""
          Return the unique hyperbolic geodesic perpendicular to two given
          geodesics, if such a geodesic exists. If none exists, raise a
```



---

archive/issue_comments_083112.json:
```json
{
    "body": "<a id='comment:56'>Comment 56:</a>\nAhh, thanks for finding the typo!\n\nThe design is very clear, and it's nice to read someone's improvement of your code.  I plan to print out the code and make sure it makes sense from a bird's eye view, and to address the issue of what to do with subsets.  But that may not happen until a few weeks from now.\n\nAt present, I'm writing up a paper that uses the code, which means I'll be looking a lot at individual lines of code but not so much at organization.  Once I eliminate the remaining bugs that I've run into, I'll get to work on making the patch suitable for inclusion in Sage.\n\nThere are also some functions whose names I have always disliked -- such as _to_std_geod -- but for which I haven't yet found suitable replacements.  I'll try to clean that up too, when I'm working on making sure the parent/element architecture does everything we need it to.",
    "created_at": "2014-09-30T14:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83112",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:56'>Comment 56:</a>
Ahh, thanks for finding the typo!

The design is very clear, and it's nice to read someone's improvement of your code.  I plan to print out the code and make sure it makes sense from a bird's eye view, and to address the issue of what to do with subsets.  But that may not happen until a few weeks from now.

At present, I'm writing up a paper that uses the code, which means I'll be looking a lot at individual lines of code but not so much at organization.  Once I eliminate the remaining bugs that I've run into, I'll get to work on making the patch suitable for inclusion in Sage.

There are also some functions whose names I have always disliked -- such as _to_std_geod -- but for which I haven't yet found suitable replacements.  I'll try to clean that up too, when I'm working on making sure the parent/element architecture does everything we need it to.



---

archive/issue_comments_083113.json:
```json
{
    "body": "<a id='comment:57'>Comment 57:</a>\nSo what's left to review or what other bugs have you come across? Unfortunately we won't get this in 6.4, but I feel like this is very close to being done.\n\nAlso don't worry about dealing with subsets. As I mentioned, there are more general Sage design decisions that will need to be made and what we currently have is sufficient.",
    "created_at": "2014-11-03T03:17:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83113",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:57'>Comment 57:</a>
So what's left to review or what other bugs have you come across? Unfortunately we won't get this in 6.4, but I feel like this is very close to being done.

Also don't worry about dealing with subsets. As I mentioned, there are more general Sage design decisions that will need to be made and what we currently have is sufficient.



---

archive/issue_comments_083114.json:
```json
{
    "body": "Changed commit from **[222c2ad](https://github.com/sagemath/sagetrac-mirror/commit/222c2ad0611bc609c7a24f934a8fe6cef5cc6792)** to **[a1c4496](https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8)**",
    "created_at": "2014-11-03T15:15:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83114",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[222c2ad](https://github.com/sagemath/sagetrac-mirror/commit/222c2ad0611bc609c7a24f934a8fe6cef5cc6792)** to **[a1c4496](https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8)**



---

archive/issue_comments_083115.json:
```json
{
    "body": "<a id='comment:58'>Comment 58:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b558754ec2a6ca610ed14d1831e35002384c483c\">b558754</a></td><td><code>Fixed a bug in isometry_from_fixed_points.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/45392f410cf0167d30400acf14365dbe1a1c7772\">45392f4</a></td><td><code>Partial progress toward getting reflection_involution working properly with coercion UHP -> PD.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1f8d109f32f891e450905d75fe6bf081382316eb\">1f8d109</a></td><td><code>May need to roll this one back.  Uncertain if it's an improvement.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6d7c46ddcf8e6f495e8b0e8856b3a28098576db6\">6d7c46d</a></td><td><code>Fixed associativity issue with PD orientation-preserving and orientation-reversing isometries.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/81ecbb069f88098e38e88d590f1708bab089cd1e\">81ecbb0</a></td><td><code>Fixed test failures caused by the previous bug fixes.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8\">a1c4496</a></td><td><code>Added tests to hyperbolic_isometry.py.</code></td></tr></table>\n",
    "created_at": "2014-11-03T15:15:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83115",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:58'>Comment 58:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b558754ec2a6ca610ed14d1831e35002384c483c">b558754</a></td><td><code>Fixed a bug in isometry_from_fixed_points.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/45392f410cf0167d30400acf14365dbe1a1c7772">45392f4</a></td><td><code>Partial progress toward getting reflection_involution working properly with coercion UHP -> PD.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1f8d109f32f891e450905d75fe6bf081382316eb">1f8d109</a></td><td><code>May need to roll this one back.  Uncertain if it's an improvement.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6d7c46ddcf8e6f495e8b0e8856b3a28098576db6">6d7c46d</a></td><td><code>Fixed associativity issue with PD orientation-preserving and orientation-reversing isometries.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/81ecbb069f88098e38e88d590f1708bab089cd1e">81ecbb0</a></td><td><code>Fixed test failures caused by the previous bug fixes.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8">a1c4496</a></td><td><code>Added tests to hyperbolic_isometry.py.</code></td></tr></table>




---

archive/issue_comments_083116.json:
```json
{
    "body": "<a id='comment:59'>Comment 59:</a>\nSorry to have disappeared for a while.  I had an insane schedule for a few weeks.  I just pushed the latest code I have.  It passes all tests and has 100% coverage.\n\nI fixed the one major bug that I knew about.  The bug had to do with orientation-reversing isometries in the two models that use SO(2,1); namely, the hyperboloid model and the Klein disk.  I solved the problem by doing isometry multiplication in the upeer half plane model.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b558754ec2a6ca610ed14d1831e35002384c483c\">b558754</a></td><td><code>Fixed a bug in isometry_from_fixed_points.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/45392f410cf0167d30400acf14365dbe1a1c7772\">45392f4</a></td><td><code>Partial progress toward getting reflection_involution working properly with coercion UHP -> PD.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1f8d109f32f891e450905d75fe6bf081382316eb\">1f8d109</a></td><td><code>May need to roll this one back.  Uncertain if it's an improvement.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6d7c46ddcf8e6f495e8b0e8856b3a28098576db6\">6d7c46d</a></td><td><code>Fixed associativity issue with PD orientation-preserving and orientation-reversing isometries.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/81ecbb069f88098e38e88d590f1708bab089cd1e\">81ecbb0</a></td><td><code>Fixed test failures caused by the previous bug fixes.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8\">a1c4496</a></td><td><code>Added tests to hyperbolic_isometry.py.</code></td></tr></table>\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b558754ec2a6ca610ed14d1831e35002384c483c\">b558754</a></td><td><code>Fixed a bug in isometry_from_fixed_points.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/45392f410cf0167d30400acf14365dbe1a1c7772\">45392f4</a></td><td><code>Partial progress toward getting reflection_involution working properly with coercion UHP -> PD.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1f8d109f32f891e450905d75fe6bf081382316eb\">1f8d109</a></td><td><code>May need to roll this one back.  Uncertain if it's an improvement.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6d7c46ddcf8e6f495e8b0e8856b3a28098576db6\">6d7c46d</a></td><td><code>Fixed associativity issue with PD orientation-preserving and orientation-reversing isometries.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/81ecbb069f88098e38e88d590f1708bab089cd1e\">81ecbb0</a></td><td><code>Fixed test failures caused by the previous bug fixes.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8\">a1c4496</a></td><td><code>Added tests to hyperbolic_isometry.py.</code></td></tr></table>\n",
    "created_at": "2014-11-03T15:17:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83116",
    "user": "https://github.com/greglaun"
}
```

<a id='comment:59'>Comment 59:</a>
Sorry to have disappeared for a while.  I had an insane schedule for a few weeks.  I just pushed the latest code I have.  It passes all tests and has 100% coverage.

I fixed the one major bug that I knew about.  The bug had to do with orientation-reversing isometries in the two models that use SO(2,1); namely, the hyperboloid model and the Klein disk.  I solved the problem by doing isometry multiplication in the upeer half plane model.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b558754ec2a6ca610ed14d1831e35002384c483c">b558754</a></td><td><code>Fixed a bug in isometry_from_fixed_points.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/45392f410cf0167d30400acf14365dbe1a1c7772">45392f4</a></td><td><code>Partial progress toward getting reflection_involution working properly with coercion UHP -> PD.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1f8d109f32f891e450905d75fe6bf081382316eb">1f8d109</a></td><td><code>May need to roll this one back.  Uncertain if it's an improvement.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6d7c46ddcf8e6f495e8b0e8856b3a28098576db6">6d7c46d</a></td><td><code>Fixed associativity issue with PD orientation-preserving and orientation-reversing isometries.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/81ecbb069f88098e38e88d590f1708bab089cd1e">81ecbb0</a></td><td><code>Fixed test failures caused by the previous bug fixes.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8">a1c4496</a></td><td><code>Added tests to hyperbolic_isometry.py.</code></td></tr></table>

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b558754ec2a6ca610ed14d1831e35002384c483c">b558754</a></td><td><code>Fixed a bug in isometry_from_fixed_points.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/45392f410cf0167d30400acf14365dbe1a1c7772">45392f4</a></td><td><code>Partial progress toward getting reflection_involution working properly with coercion UHP -> PD.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1f8d109f32f891e450905d75fe6bf081382316eb">1f8d109</a></td><td><code>May need to roll this one back.  Uncertain if it's an improvement.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6d7c46ddcf8e6f495e8b0e8856b3a28098576db6">6d7c46d</a></td><td><code>Fixed associativity issue with PD orientation-preserving and orientation-reversing isometries.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/81ecbb069f88098e38e88d590f1708bab089cd1e">81ecbb0</a></td><td><code>Fixed test failures caused by the previous bug fixes.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8">a1c4496</a></td><td><code>Added tests to hyperbolic_isometry.py.</code></td></tr></table>




---

archive/issue_events_106994.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-11-03T19:25:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106994"
}
```



---

archive/issue_events_106995.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-11-03T19:25:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106995"
}
```



---

archive/issue_comments_083117.json:
```json
{
    "body": "Changed commit from **[a1c4496](https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8)** to **[c5dfd89](https://github.com/sagemath/sagetrac-mirror/commit/c5dfd89e923df732ffb868dcf2a22f48b5d32a44)**",
    "created_at": "2014-11-03T19:25:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83117",
    "user": "https://github.com/tscrim"
}
```

Changed commit from **[a1c4496](https://github.com/sagemath/sagetrac-mirror/commit/a1c4496109c119d3d44cacf31b6c87108dab1ba8)** to **[c5dfd89](https://github.com/sagemath/sagetrac-mirror/commit/c5dfd89e923df732ffb868dcf2a22f48b5d32a44)**



---

archive/issue_comments_083118.json:
```json
{
    "body": "Changed reviewer from **Johan Bosman, Travis Scrimshaw** to **Johan Bosman, Travis Scrimshaw, Greg Laun**",
    "created_at": "2014-11-03T19:25:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83118",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Johan Bosman, Travis Scrimshaw** to **Johan Bosman, Travis Scrimshaw, Greg Laun**



---

archive/issue_comments_083119.json:
```json
{
    "body": "<a id='comment:60'>Comment 60:</a>\nNo worries; I completely understand (and will likely have to do the same thing shortly).\n\nSo, I've updated it to the latest develop version, fixed (trivial) doctests from that, and added full coverage to `hyperbolic_coercion.py` (which I should've done in the first place). If there are no obvious major bugs, then you can set a positive review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c6eb9a0df27e96850f0f2c402ef93482102e25dc\">c6eb9a0</a></td><td><code>Merge branch 'u/glaun/ticket/9439' of trac.sagemath.org:sage into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c5dfd89e923df732ffb868dcf2a22f48b5d32a44\">c5dfd89</a></td><td><code>Final fixes and touchups.</code></td></tr></table>\n",
    "created_at": "2014-11-03T19:25:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83119",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:60'>Comment 60:</a>
No worries; I completely understand (and will likely have to do the same thing shortly).

So, I've updated it to the latest develop version, fixed (trivial) doctests from that, and added full coverage to `hyperbolic_coercion.py` (which I should've done in the first place). If there are no obvious major bugs, then you can set a positive review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c6eb9a0df27e96850f0f2c402ef93482102e25dc">c6eb9a0</a></td><td><code>Merge branch 'u/glaun/ticket/9439' of trac.sagemath.org:sage into public/geometry/hyperbolic-9439</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c5dfd89e923df732ffb868dcf2a22f48b5d32a44">c5dfd89</a></td><td><code>Final fixes and touchups.</code></td></tr></table>




---

archive/issue_comments_083120.json:
```json
{
    "body": "Changed branch from **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)** to **[public/geometry/hyperbolic-9439](https://github.com/sagemath/sagetrac-mirror/tree/public/geometry/hyperbolic-9439)**",
    "created_at": "2014-11-03T19:25:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83120",
    "user": "https://github.com/tscrim"
}
```

Changed branch from **[u/glaun/ticket/9439](https://github.com/sagemath/sagetrac-mirror/tree/u/glaun/ticket/9439)** to **[public/geometry/hyperbolic-9439](https://github.com/sagemath/sagetrac-mirror/tree/public/geometry/hyperbolic-9439)**



---

archive/issue_comments_083121.json:
```json
{
    "body": "<a id='comment:61'>Comment 61:</a>\nPlease remove the trailing whitespaces in:\n- `hyperbolic_geodesic.py`\n- `hyperbolic_model.py`",
    "created_at": "2014-11-07T17:01:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83121",
    "user": "https://github.com/videlec"
}
```

<a id='comment:61'>Comment 61:</a>
Please remove the trailing whitespaces in:
- `hyperbolic_geodesic.py`
- `hyperbolic_model.py`



---

archive/issue_comments_083122.json:
```json
{
    "body": "<a id='comment:62'>Comment 62:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/015086df733e2c0a5bab26962aa9d0ff2ca08161\">015086d</a></td><td><code>trac #9439 whitespace removal plus other cosmetic cleanup</code></td></tr></table>\n",
    "created_at": "2014-11-07T19:39:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83122",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:62'>Comment 62:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/015086df733e2c0a5bab26962aa9d0ff2ca08161">015086d</a></td><td><code>trac #9439 whitespace removal plus other cosmetic cleanup</code></td></tr></table>




---

archive/issue_comments_083123.json:
```json
{
    "body": "Changed commit from **[c5dfd89](https://github.com/sagemath/sagetrac-mirror/commit/c5dfd89e923df732ffb868dcf2a22f48b5d32a44)** to **[015086d](https://github.com/sagemath/sagetrac-mirror/commit/015086df733e2c0a5bab26962aa9d0ff2ca08161)**",
    "created_at": "2014-11-07T19:39:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83123",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c5dfd89](https://github.com/sagemath/sagetrac-mirror/commit/c5dfd89e923df732ffb868dcf2a22f48b5d32a44)** to **[015086d](https://github.com/sagemath/sagetrac-mirror/commit/015086df733e2c0a5bab26962aa9d0ff2ca08161)**



---

archive/issue_comments_083124.json:
```json
{
    "body": "<a id='comment:63'>Comment 63:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ef79bb7533500e8fa84faeca2197350e46e999df\">ef79bb7</a></td><td><code>trac #9439 correct doc index</code></td></tr></table>\n",
    "created_at": "2014-11-07T20:16:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83124",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:63'>Comment 63:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ef79bb7533500e8fa84faeca2197350e46e999df">ef79bb7</a></td><td><code>trac #9439 correct doc index</code></td></tr></table>




---

archive/issue_comments_083125.json:
```json
{
    "body": "Changed commit from **[015086d](https://github.com/sagemath/sagetrac-mirror/commit/015086df733e2c0a5bab26962aa9d0ff2ca08161)** to **[ef79bb7](https://github.com/sagemath/sagetrac-mirror/commit/ef79bb7533500e8fa84faeca2197350e46e999df)**",
    "created_at": "2014-11-07T20:16:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83125",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[015086d](https://github.com/sagemath/sagetrac-mirror/commit/015086df733e2c0a5bab26962aa9d0ff2ca08161)** to **[ef79bb7](https://github.com/sagemath/sagetrac-mirror/commit/ef79bb7533500e8fa84faeca2197350e46e999df)**



---

archive/issue_comments_083126.json:
```json
{
    "body": "<a id='comment:64'>Comment 64:</a>\nThanks Frederic. I'm happy with your changes. So then I believe the only thing left is for someone to review c5dfd89.",
    "created_at": "2014-11-14T19:26:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83126",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:64'>Comment 64:</a>
Thanks Frederic. I'm happy with your changes. So then I believe the only thing left is for someone to review c5dfd89.



---

archive/issue_comments_083127.json:
```json
{
    "body": "Changed reviewer from **Johan Bosman, Travis Scrimshaw, Greg Laun** to **Johan Bosman, Travis Scrimshaw, Greg Laun, Frederic Chapoton**",
    "created_at": "2014-11-14T19:26:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83127",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Johan Bosman, Travis Scrimshaw, Greg Laun** to **Johan Bosman, Travis Scrimshaw, Greg Laun, Frederic Chapoton**



---

archive/issue_comments_083128.json:
```json
{
    "body": "<a id='comment:65'>Comment 65:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b49f9f8efff32b8881f5a626b0b5522fba3c600b\">b49f9f8</a></td><td><code>Merge branch 'public/geometry/hyperbolic-9439' into 6.6.b5</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8789fdce0cd66dd71243be5f6466b2e5951cc9ce\">8789fdc</a></td><td><code>trac #9439 correct wrong use of abs, plus some pep8 details</code></td></tr></table>\n",
    "created_at": "2015-03-15T20:04:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83128",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:65'>Comment 65:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b49f9f8efff32b8881f5a626b0b5522fba3c600b">b49f9f8</a></td><td><code>Merge branch 'public/geometry/hyperbolic-9439' into 6.6.b5</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8789fdce0cd66dd71243be5f6466b2e5951cc9ce">8789fdc</a></td><td><code>trac #9439 correct wrong use of abs, plus some pep8 details</code></td></tr></table>




---

archive/issue_comments_083129.json:
```json
{
    "body": "Changed commit from **[ef79bb7](https://github.com/sagemath/sagetrac-mirror/commit/ef79bb7533500e8fa84faeca2197350e46e999df)** to **[8789fdc](https://github.com/sagemath/sagetrac-mirror/commit/8789fdce0cd66dd71243be5f6466b2e5951cc9ce)**",
    "created_at": "2015-03-15T20:04:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83129",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ef79bb7](https://github.com/sagemath/sagetrac-mirror/commit/ef79bb7533500e8fa84faeca2197350e46e999df)** to **[8789fdc](https://github.com/sagemath/sagetrac-mirror/commit/8789fdce0cd66dd71243be5f6466b2e5951cc9ce)**



---

archive/issue_comments_083130.json:
```json
{
    "body": "Changed commit from **[8789fdc](https://github.com/sagemath/sagetrac-mirror/commit/8789fdce0cd66dd71243be5f6466b2e5951cc9ce)** to **[82cf89e](https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94)**",
    "created_at": "2015-03-15T20:40:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83130",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8789fdc](https://github.com/sagemath/sagetrac-mirror/commit/8789fdce0cd66dd71243be5f6466b2e5951cc9ce)** to **[82cf89e](https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94)**



---

archive/issue_comments_083131.json:
```json
{
    "body": "<a id='comment:66'>Comment 66:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94\">82cf89e</a></td><td><code>trac #9439 fine tuning the doc</code></td></tr></table>\n",
    "created_at": "2015-03-15T20:40:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83131",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:66'>Comment 66:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94">82cf89e</a></td><td><code>trac #9439 fine tuning the doc</code></td></tr></table>




---

archive/issue_comments_083132.json:
```json
{
    "body": "<a id='comment:67'>Comment 67:</a>\nI am a bit worried by the fact that some tests are longer than 1s, see\n\n```\nsage -bt --long --warn-long src/sage/geometry/hyperbolic_space/*.py\n```\nOtherwise, things look good.",
    "created_at": "2015-03-16T20:30:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83132",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:67'>Comment 67:</a>
I am a bit worried by the fact that some tests are longer than 1s, see

```
sage -bt --long --warn-long src/sage/geometry/hyperbolic_space/*.py
```
Otherwise, things look good.



---

archive/issue_comments_083133.json:
```json
{
    "body": "Changed reviewer from **Johan Bosman, Travis Scrimshaw, Greg Laun, Frederic Chapoton** to **Johan Bosman, Travis Scrimshaw, Greg Laun, Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2015-03-16T22:13:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83133",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Johan Bosman, Travis Scrimshaw, Greg Laun, Frederic Chapoton** to **Johan Bosman, Travis Scrimshaw, Greg Laun, Frédéric Chapoton**



---

archive/issue_comments_083134.json:
```json
{
    "body": "<a id='comment:68'>Comment 68:</a>\nSome of those breakage of pep8 was to make it more readable, but I'm not opposed to those changes. However I am happy overall with your changes, and I'm treating your comments as positive review on the previous part. Also it's okay for long tests to be longer than 1s (at least, you're testing them), but if they were not marked as `# long time`, then my philosophy is if they are longer than 2-3s on my machine, then I mark them as long.\n\nTherefore I'm going to set this to a positive review.",
    "created_at": "2015-03-16T22:13:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83134",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:68'>Comment 68:</a>
Some of those breakage of pep8 was to make it more readable, but I'm not opposed to those changes. However I am happy overall with your changes, and I'm treating your comments as positive review on the previous part. Also it's okay for long tests to be longer than 1s (at least, you're testing them), but if they were not marked as `# long time`, then my philosophy is if they are longer than 2-3s on my machine, then I mark them as long.

Therefore I'm going to set this to a positive review.



---

archive/issue_events_106996.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-03-16T22:13:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106996"
}
```



---

archive/issue_events_106997.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-03-16T22:13:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106997"
}
```



---

archive/issue_events_106998.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-19T03:17:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106998"
}
```



---

archive/issue_events_106999.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ccf9d16115e0d5453fa548d6739e709e38241144",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-03-19T03:17:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9439#event-106999"
}
```



---

archive/issue_comments_083135.json:
```json
{
    "body": "Changed branch from **[public/geometry/hyperbolic-9439](https://github.com/sagemath/sagetrac-mirror/tree/public/geometry/hyperbolic-9439)** to **[82cf89e](https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94)**",
    "created_at": "2015-03-19T03:17:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83135",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/geometry/hyperbolic-9439](https://github.com/sagemath/sagetrac-mirror/tree/public/geometry/hyperbolic-9439)** to **[82cf89e](https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94)**



---

archive/issue_comments_083136.json:
```json
{
    "body": "Changed commit from **[82cf89e](https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94)** to none",
    "created_at": "2015-03-19T05:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83136",
    "user": "https://github.com/tscrim"
}
```

Changed commit from **[82cf89e](https://github.com/sagemath/sagetrac-mirror/commit/82cf89e367b6f95cf44fdfb7b040974d4e247f94)** to none



---

archive/issue_comments_083137.json:
```json
{
    "body": "Changed keywords from **hyperbolic geometry, Poincare disc, upper half plane, Beltrami-Klein, hyperboloid model, sd35** to **hyperbolic geometry, Poincare disc, upper half plane, Beltrami-Klein, hyperboloid model, sd35, days64**",
    "created_at": "2015-03-19T05:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83137",
    "user": "https://github.com/tscrim"
}
```

Changed keywords from **hyperbolic geometry, Poincare disc, upper half plane, Beltrami-Klein, hyperboloid model, sd35** to **hyperbolic geometry, Poincare disc, upper half plane, Beltrami-Klein, hyperboloid model, sd35, days64**



---

archive/issue_comments_083138.json:
```json
{
    "body": "<a id='comment:71'>Comment 71:</a>\nFirst off: It's very nice to see the ticket closed. :-)\n\nJust a side remark: I noticed that complex embeddings are used a lot for points (be it by using imag for boundary points or CC). Unfortunately this doesn't work for relative number fields (no support for specifying default embeddings yet). I use those to do exact calculations with hyperbolic fixed points of hecke triangle group elements. I guess I'm hoping that relative number fields will be improved. :-)",
    "created_at": "2015-03-20T20:18:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83138",
    "user": "https://github.com/jjermann"
}
```

<a id='comment:71'>Comment 71:</a>
First off: It's very nice to see the ticket closed. :-)

Just a side remark: I noticed that complex embeddings are used a lot for points (be it by using imag for boundary points or CC). Unfortunately this doesn't work for relative number fields (no support for specifying default embeddings yet). I use those to do exact calculations with hyperbolic fixed points of hecke triangle group elements. I guess I'm hoping that relative number fields will be improved. :-)



---

archive/issue_comments_083139.json:
```json
{
    "body": "<a id='comment:72'>Comment 72:</a>\nI think it would be good to have as a follow-up being able to specify which field you want to work over (for example, the benefit of being able to specify precision). However I felt that this was a very good step forward to include into Sage (and I don't care so much about having a perfect solution at the beginning but going in steps towards the optimal solution, but perhaps I'm in the minority...).",
    "created_at": "2015-03-20T20:47:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83139",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:72'>Comment 72:</a>
I think it would be good to have as a follow-up being able to specify which field you want to work over (for example, the benefit of being able to specify precision). However I felt that this was a very good step forward to include into Sage (and I don't care so much about having a perfect solution at the beginning but going in steps towards the optimal solution, but perhaps I'm in the minority...).



---

archive/issue_comments_083140.json:
```json
{
    "body": "<a id='comment:73'>Comment 73:</a>\nOh, I very much agree with going in steps and I'm glad the ticket got closed, good job! :-)\n\nAlso: The issue I mentioned is much more about (relative) number fields than this implementation (i.e. I'm off-topic and should probably have sent this to sage-devel).\n\nI guess one of the main issues is that (relative) number fields in sage are mostly \"designed\" without a fixed choice of embedding (\"they are not viewed as part of complex numbers / hyperbolic plane\"). Still they are quite important when doing arithmetic/number theory/exact calculations...\n\nEven in the absolute case a fixed field needs to be choosen for the default embedding. To make it\nwork in general one has to use something that coerces into many fields, e.g. AA/QQbar.\nSide question: Conceptually how would one organize/order the embeddings in a meaningful way? (I'm not sure how they are ordered at the moment.)\n\nThe issue with number fields also comes up when dealing with isometries/matrices over number fields and the group action...\n\n\nRegards\nJonas",
    "created_at": "2015-03-20T21:11:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83140",
    "user": "https://github.com/jjermann"
}
```

<a id='comment:73'>Comment 73:</a>
Oh, I very much agree with going in steps and I'm glad the ticket got closed, good job! :-)

Also: The issue I mentioned is much more about (relative) number fields than this implementation (i.e. I'm off-topic and should probably have sent this to sage-devel).

I guess one of the main issues is that (relative) number fields in sage are mostly "designed" without a fixed choice of embedding ("they are not viewed as part of complex numbers / hyperbolic plane"). Still they are quite important when doing arithmetic/number theory/exact calculations...

Even in the absolute case a fixed field needs to be choosen for the default embedding. To make it
work in general one has to use something that coerces into many fields, e.g. AA/QQbar.
Side question: Conceptually how would one organize/order the embeddings in a meaningful way? (I'm not sure how they are ordered at the moment.)

The issue with number fields also comes up when dealing with isometries/matrices over number fields and the group action...


Regards
Jonas



---

archive/issue_comments_083141.json:
```json
{
    "body": "<a id='comment:74'>Comment 74:</a>\nI don't think I can answer many of those questions because my knowledge in that part of math (and definitely of Sage) is likely insufficient. I think moving this to sage-devel is the best way to get the answers you're after.",
    "created_at": "2015-03-20T21:16:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9439#issuecomment-83141",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:74'>Comment 74:</a>
I don't think I can answer many of those questions because my knowledge in that part of math (and definitely of Sage) is likely insufficient. I think moving this to sage-devel is the best way to get the answers you're after.
