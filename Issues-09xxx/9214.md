# Issue 9214: Bring doc coverage of combinat/lyndon_word.py to 100%

archive/issues_009214.json:
```json
{
    "body": "\n\n**Assignee:** @seblabbe\n\n**CC:**  sage-combinat\n\n**Author:** S\u00e9bastien Labb\u00e9\n\n**Reviewer:** Franco Saliola, Minh Van Nguyen\n\n**Merged:** sage-4.4.4.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/9214\n\n",
    "closed_at": "2010-06-11T19:45:00Z",
    "created_at": "2010-06-11T10:51:04Z",
    "labels": [
        "component: documentation",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.4",
    "title": "Bring doc coverage of combinat/lyndon_word.py to 100%",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9214",
    "user": "https://github.com/seblabbe"
}
```


**Assignee:** @seblabbe

**CC:**  sage-combinat

**Author:** Sébastien Labbé

**Reviewer:** Franco Saliola, Minh Van Nguyen

**Merged:** sage-4.4.4.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/9214





---

archive/attachments_011615.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_9214-doc_lyndon_word-sl.patch",
    "asset_url": "tarball://root/attachments/ticket9214/trac_9214-doc_lyndon_word-sl.patch",
    "created_at": "2010-06-11T11:10:23Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket9214/trac_9214-doc_lyndon_word-sl.patch",
    "user": "https://github.com/seblabbe"
}
```



---

archive/issue_comments_078967.json:
```json
{
    "body": "Attachment [trac_9214-doc_lyndon_word-sl.patch](https://github.com/sagemath/sage/files/ticket9214/trac_9214-doc_lyndon_word-sl.patch) by @seblabbe created at 2010-06-11 11:10:23",
    "created_at": "2010-06-11T11:10:23Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78967",
    "user": "https://github.com/seblabbe"
}
```

Attachment [trac_9214-doc_lyndon_word-sl.patch](https://github.com/sagemath/sage/files/ticket9214/trac_9214-doc_lyndon_word-sl.patch) by @seblabbe created at 2010-06-11 11:10:23



---

archive/issue_events_067261.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-06-11T11:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67261"
}
```



---

archive/issue_comments_078968.json:
```json
{
    "body": "**Author:** S\u00e9bastien Labb\u00e9",
    "created_at": "2010-06-11T11:53:02Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78968",
    "user": "https://github.com/seblabbe"
}
```

**Author:** Sébastien Labbé



---

archive/issue_comments_078969.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me (reviewed against sage-4.4.3).",
    "created_at": "2010-06-11T13:02:29Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78969",
    "user": "https://github.com/saliola"
}
```

<a id='comment:2'></a>
Looks good to me (reviewed against sage-4.4.3).



---

archive/issue_comments_078970.json:
```json
{
    "body": "**Reviewer:** Franco Saloila",
    "created_at": "2010-06-11T13:02:29Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78970",
    "user": "https://github.com/saliola"
}
```

**Reviewer:** Franco Saloila



---

archive/issue_events_067262.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-06-11T13:02:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67262"
}
```



---

archive/issue_events_067263.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-06-11T13:02:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67263"
}
```



---

archive/attachments_011616.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_9214-reviewer.patch",
    "asset_url": "tarball://root/attachments/ticket9214/trac_9214-reviewer.patch",
    "created_at": "2010-06-11T19:41:53Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket9214/trac_9214-reviewer.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```



---

archive/issue_comments_078971.json:
```json
{
    "body": "Attachment [trac_9214-reviewer.patch](https://github.com/sagemath/sage/files/ticket9214/trac_9214-reviewer.patch) by mvngu created at 2010-06-11 19:41:53",
    "created_at": "2010-06-11T19:41:53Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78971",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Attachment [trac_9214-reviewer.patch](https://github.com/sagemath/sage/files/ticket9214/trac_9214-reviewer.patch) by mvngu created at 2010-06-11 19:41:53



---

archive/issue_events_067264.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-06-11T19:42:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67264"
}
```



---

archive/issue_events_067265.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-06-11T19:42:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67265"
}
```



---

archive/issue_events_067266.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-06-11T19:44:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67266"
}
```



---

archive/issue_events_067267.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-06-11T19:44:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67267"
}
```



---

archive/issue_comments_078972.json:
```json
{
    "body": "<a id='comment:4'></a>\nThis needs some typo fixes introduced by slabbe's patch. See the reviewer patch for such fixes. So only my reviewer patch needs another pair of eyes to look over it. If it's OK, then the whole ticket is good for a positive review.",
    "created_at": "2010-06-11T19:44:15Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78972",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
This needs some typo fixes introduced by slabbe's patch. See the reviewer patch for such fixes. So only my reviewer patch needs another pair of eyes to look over it. If it's OK, then the whole ticket is good for a positive review.



---

archive/issue_comments_078973.json:
```json
{
    "body": "**Changing reviewer** from \"Franco Saloila\" to \"Franco Saloila, Minh Van Nguyen\".",
    "created_at": "2010-06-11T19:44:15Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78973",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Changing reviewer** from "Franco Saloila" to "Franco Saloila, Minh Van Nguyen".



---

archive/issue_events_067268.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-11T19:44:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67268"
}
```



---

archive/issue_events_067269.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-11T19:44:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67269"
}
```



---

archive/issue_comments_078974.json:
```json
{
    "body": "<a id='comment:5'></a>\nLooks good.",
    "created_at": "2010-06-11T19:44:51Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78974",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:5'></a>
Looks good.



---

archive/issue_comments_078975.json:
```json
{
    "body": "**Merged:** sage-4.4.4.alpha1",
    "created_at": "2010-06-11T19:45:00Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78975",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.4.4.alpha1



---

archive/issue_events_067270.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-11T19:45:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67270"
}
```



---

archive/issue_events_067271.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-11T19:45:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9214#event-67271"
}
```



---

archive/issue_comments_078976.json:
```json
{
    "body": "**Changing reviewer** from \"Franco Saloila, Minh Van Nguyen\" to \"Franco Saliola, Minh Van Nguyen\".",
    "created_at": "2010-06-24T07:44:50Z",
    "issue": "https://github.com/sagemath/sage/issues/9214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9214#issuecomment-78976",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Changing reviewer** from "Franco Saloila, Minh Van Nguyen" to "Franco Saliola, Minh Van Nguyen".
