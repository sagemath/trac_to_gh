# Issue 9298: Memory leak in libsingular polynomial evaluation

archive/issues_009298.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: R.<A,B,C>=QQ[]\nsage: print get_memory_usage()\n180.0390625\nsage: for i in xrange(10000): v = A(1,8,9) # leaks\n....: \nsage: print get_memory_usage()\n181.5390625\nsage: for i in xrange(10000): v = A(1,8,9.0) # good\n....: \nsage: print get_memory_usage()\n181.5390625\nsage: for i in xrange(10000): v = A(1/2,1/3,1/4) # leaks\n....: \nsage: print get_memory_usage()\n183.5390625\n```\n\nAssignee: **@malb**\n\nCC:  @sagetrac-PolyBoRi @alexanderdreyer @sagetrac-jpflori\n\nReviewer: **Charles Bouillaguet**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9298_\n\n",
    "closed_at": "2013-01-10T09:27:12Z",
    "created_at": "2010-06-21T17:50:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20memleak",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Memory leak in libsingular polynomial evaluation",
    "type": "issue",
    "updated_at": "2013-01-10T09:27:12Z",
    "url": "https://github.com/sagemath/sage/issues/9298",
    "user": "https://github.com/robertwb"
}
```

```
sage: R.<A,B,C>=QQ[]
sage: print get_memory_usage()
180.0390625
sage: for i in xrange(10000): v = A(1,8,9) # leaks
....: 
sage: print get_memory_usage()
181.5390625
sage: for i in xrange(10000): v = A(1,8,9.0) # good
....: 
sage: print get_memory_usage()
181.5390625
sage: for i in xrange(10000): v = A(1/2,1/3,1/4) # leaks
....: 
sage: print get_memory_usage()
183.5390625
```

Assignee: **@malb**

CC:  @sagetrac-PolyBoRi @alexanderdreyer @sagetrac-jpflori

Reviewer: **Charles Bouillaguet**

_Issue created by migration from https://trac.sagemath.org/ticket/9298_





---

archive/issue_events_108414.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-06-21T17:50:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108414"
}
```



---

archive/issue_events_108415.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-06-21T17:50:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20memleak",
    "label_color": "000080",
    "label_name": "component: memleak",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108415"
}
```



---

archive/issue_events_108416.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-06-21T17:50:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108416"
}
```



---

archive/issue_events_108417.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-06-21T17:50:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108417"
}
```



---

archive/issue_comments_080280.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThere's a new version of Singular likely to be merged soon (#8059). I'd retest then.\n\nDave",
    "created_at": "2010-08-10T18:40:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9298#issuecomment-80280",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:1" align="right">Comment 1</div>

There's a new version of Singular likely to be merged soon (#8059). I'd retest then.

Dave



---

archive/issue_events_108418.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2013-01-02T14:57:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108418"
}
```



---

archive/issue_events_108419.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2013-01-02T14:57:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108419"
}
```



---

archive/issue_events_108420.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2013-01-02T14:57:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108420"
}
```



---

archive/issue_comments_080281.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nIndeed, the leak seems fixed. This ticket can be closed.",
    "created_at": "2013-01-02T14:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9298#issuecomment-80281",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

<div id="comment:4" align="right">Comment 4</div>

Indeed, the leak seems fixed. This ticket can be closed.



---

archive/issue_comments_080282.json:
```json
{
    "body": "Assignee: **@malb**",
    "created_at": "2013-01-02T14:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9298#issuecomment-80282",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

Assignee: **@malb**



---

archive/issue_events_108421.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2013-01-02T14:58:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108421"
}
```



---

archive/issue_comments_080283.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nSince the ticker cannot easily be doctested, and I cannot reproduce the leak, I suggest we close it.",
    "created_at": "2013-01-04T13:42:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9298#issuecomment-80283",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

<div id="comment:6" align="right">Comment 6</div>

Since the ticker cannot easily be doctested, and I cannot reproduce the leak, I suggest we close it.



---

archive/issue_events_108422.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2013-01-04T13:42:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108422"
}
```



---

archive/issue_events_108423.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2013-01-04T13:42:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108423"
}
```



---

archive/issue_comments_080284.json:
```json
{
    "body": "Reviewer: **Charles Bouillaguet**",
    "created_at": "2013-01-04T16:56:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9298#issuecomment-80284",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

Reviewer: **Charles Bouillaguet**



---

archive/issue_events_108424.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-10T09:27:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108424"
}
```



---

archive/issue_events_108425.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-10T09:27:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108425"
}
```



---

archive/issue_events_108426.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-10T09:27:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9298",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9298#event-108426"
}
```
