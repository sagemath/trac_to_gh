# Issue 9222: improve doctest coverage of databases/conway.py

archive/issues_009222.json:
```json
{
    "assignees": [],
    "body": "As of sage-4.4.3:\n\n```\n----------------------------------------------------------------------\nconway.py\nERROR: Please add a `TestSuite(s).run()` doctest.\nSCORE conway.py: 0% (0 of 7)\n\nMissing documentation:\n * _init(self):\n * __repr__(self):\n * polynomial(self, p, n):\n * has_polynomial(self, p, n):\n * primes(self):\n * degrees(self, p):\n\nMissing doctests:\n * __init__(self, read_only=True):\n\n----------------------------------------------------------------------\n```\n\n\nCC:  @sagetrac-mvngu\n\nKeywords: **conway polynomial database**\n\nReviewer: **David Loeffler**\n\nAuthor: **Alex Ghitza**\n\nMerged: **sage-4.5.2.alpha1**\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9222_\n\n",
    "closed_at": "2010-07-22T07:46:53Z",
    "created_at": "2010-06-12T07:09:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve doctest coverage of databases/conway.py",
    "type": "issue",
    "updated_at": "2015-09-06T16:29:53Z",
    "url": "https://github.com/sagemath/sage/issues/9222",
    "user": "https://github.com/aghitza"
}
```
As of sage-4.4.3:

```
----------------------------------------------------------------------
conway.py
ERROR: Please add a `TestSuite(s).run()` doctest.
SCORE conway.py: 0% (0 of 7)

Missing documentation:
 * _init(self):
 * __repr__(self):
 * polynomial(self, p, n):
 * has_polynomial(self, p, n):
 * primes(self):
 * degrees(self, p):

Missing doctests:
 * __init__(self, read_only=True):

----------------------------------------------------------------------
```


CC:  @sagetrac-mvngu

Keywords: **conway polynomial database**

Reviewer: **David Loeffler**

Author: **Alex Ghitza**

Merged: **sage-4.5.2.alpha1**

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/9222_





---

archive/issue_events_117967.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-06-12T07:09:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "milestone_number": null,
    "milestone_title": "sage-4.5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117967"
}
```



---

archive/issue_events_117968.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-06-12T07:09:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117968"
}
```



---

archive/issue_events_117969.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-06-12T07:09:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117969"
}
```



---

archive/issue_events_117970.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-06-12T07:09:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117970"
}
```



---

archive/issue_comments_077330.json:
```json
{
    "body": "Author: **Alex Ghitza**",
    "created_at": "2010-06-12T08:11:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9222#issuecomment-77330",
    "user": "https://github.com/aghitza"
}
```

Author: **Alex Ghitza**



---

archive/issue_events_117971.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-06-12T08:11:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117971"
}
```



---

archive/issue_comments_077331.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAfter the patch:\n\n```\nERROR: Please add a `TestSuite(s).run()` doctest.\nSCORE conway.py: 85% (6 of 7)\n\nMissing documentation:\n\t * _init(self):\n```\n\nI'm not convinced a `TestSuite` test makes sense here (and I have tried to put one in and got a pickling-related error).  Also, I'm not sure what `_init(self)` is meant to be doing, so I'm leaving it alone for now.",
    "created_at": "2010-06-12T08:11:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9222#issuecomment-77331",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:2" align="right">comment:2</div>

After the patch:

```
ERROR: Please add a `TestSuite(s).run()` doctest.
SCORE conway.py: 85% (6 of 7)

Missing documentation:
	 * _init(self):
```

I'm not convinced a `TestSuite` test makes sense here (and I have tried to put one in and got a pickling-related error).  Also, I'm not sure what `_init(self)` is meant to be doing, so I'm leaving it alone for now.



---

archive/issue_comments_077332.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks fine to me. Tests pass and the documentation builds OK. I agree that the _init method isn't something one can reasonably doctest!",
    "created_at": "2010-06-14T10:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9222#issuecomment-77332",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks fine to me. Tests pass and the documentation builds OK. I agree that the _init method isn't something one can reasonably doctest!



---

archive/issue_events_117972.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-06-14T10:32:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117972"
}
```



---

archive/issue_events_117973.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-06-14T10:32:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117973"
}
```



---

archive/issue_comments_077333.json:
```json
{
    "body": "Reviewer: **David Loeffler**",
    "created_at": "2010-06-29T08:00:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9222#issuecomment-77333",
    "user": "https://github.com/loefflerd"
}
```

Reviewer: **David Loeffler**



---

archive/issue_events_117974.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-07-22T02:31:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117974"
}
```



---

archive/issue_events_117975.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-07-22T02:31:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117975"
}
```



---

archive/issue_comments_077334.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nPlease include ticket numbers in the commit strings of your patches!",
    "created_at": "2010-07-22T02:31:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9222#issuecomment-77334",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:5" align="right">comment:5</div>

Please include ticket numbers in the commit strings of your patches!



---

archive/issue_events_117976.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-07-22T03:12:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117976"
}
```



---

archive/issue_events_117977.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-07-22T03:12:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117977"
}
```



---

archive/issue_comments_077335.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nAttachment: **[trac_9222.patch.gz](https://github.com/sagemath/sage/files/ticket9222/trac_9222.patch.gz)**\n\nDone.",
    "created_at": "2010-07-22T03:12:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9222#issuecomment-77335",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:6" align="right">comment:6</div>

Attachment: **[trac_9222.patch.gz](https://github.com/sagemath/sage/files/ticket9222/trac_9222.patch.gz)**

Done.



---

archive/issue_comments_077336.json:
```json
{
    "body": "Merged: **sage-4.5.2.alpha1**",
    "created_at": "2010-07-22T07:46:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9222#issuecomment-77336",
    "user": "https://github.com/dandrake"
}
```

Merged: **sage-4.5.2.alpha1**



---

archive/issue_comments_077337.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@aghitza](#comment:6):\n> Done.\n\nThanks!",
    "created_at": "2010-07-22T07:46:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9222#issuecomment-77337",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@aghitza](#comment:6):
> Done.

Thanks!



---

archive/issue_events_117978.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-07-22T07:46:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117978"
}
```



---

archive/issue_events_117979.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-07-22T07:46:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9222#event-117979"
}
```



---

archive/issue_comments_077338.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,16 +7,15 @@\n SCORE conway.py: 0% (0 of 7)\n \n Missing documentation:\n-\t * _init(self):\n-\t * __repr__(self):\n-\t * polynomial(self, p, n):\n-\t * has_polynomial(self, p, n):\n-\t * primes(self):\n-\t * degrees(self, p):\n-\n+ * _init(self):\n+ * __repr__(self):\n+ * polynomial(self, p, n):\n+ * has_polynomial(self, p, n):\n+ * primes(self):\n+ * degrees(self, p):\n \n Missing doctests:\n-\t * __init__(self, read_only=True):\n+ * __init__(self, read_only=True):\n \n ----------------------------------------------------------------------\n ```\n``````\n",
    "created_at": "2015-09-06T16:29:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9222#issuecomment-77338",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,16 +7,15 @@
 SCORE conway.py: 0% (0 of 7)
 
 Missing documentation:
-	 * _init(self):
-	 * __repr__(self):
-	 * polynomial(self, p, n):
-	 * has_polynomial(self, p, n):
-	 * primes(self):
-	 * degrees(self, p):
-
+ * _init(self):
+ * __repr__(self):
+ * polynomial(self, p, n):
+ * has_polynomial(self, p, n):
+ * primes(self):
+ * degrees(self, p):
 
 Missing doctests:
-	 * __init__(self, read_only=True):
+ * __init__(self, read_only=True):
 
 ----------------------------------------------------------------------
 ```
``````

