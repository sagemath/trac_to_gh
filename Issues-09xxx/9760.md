...
<a id='comment:1'></a>I expect ATLAS would be used for this test, but has that been 100% confirmed? 
...
<a id='comment:2'></a>For the example starting around line 6393 of `sage/matrix/matrix2.pyx`, I get this on t2:
...
```json
{
    "body": "<a id='comment:3'></a>To clarify:  \"...and that we cannot take the returned imaginary parts **here** as other than \"small\"?\"",
...
<a id='comment:3'></a>To clarify:  "...and that we cannot take the returned imaginary parts **here** as other than "small"?"
...
```json
{
    "body": "<a id='comment:7'></a>Let's make adding \"abs\" a separate ticket that we can include early in the 4.6 series.",
...
<a id='comment:7'></a>Let's make adding "abs" a separate ticket that we can include early in the 4.6 series.
...
```json
{
    "body": "<a id='comment:8'></a>Okay, here's a patch, replacing 1 by 1.1.",
...
<a id='comment:8'></a>Okay, here's a patch, replacing 1 by 1.1.
...
```json
{
    "body": "<a id='comment:9'></a>See #9765 for the \"abs\" issue.",
...
<a id='comment:9'></a>See #9765 for the "abs" issue.
...
```json
{
    "body": "<a id='comment:10'></a>Okay, I've now turned at least part of my mind on.  The file \"....pyx\" should be called \"....patch\".  I've attached a properly named file, identical to the first one.",
...
<a id='comment:10'></a>Okay, I've now turned at least part of my mind on.  The file "....pyx" should be called "....patch".  I've attached a properly named file, identical to the first one.
...
<a id='comment:11'></a>I was just about to write this below, and when I tried to submit it, you had beaten me to it. I think you have basically confirmed what I thought. 
...
```json
{
    "body": "<a id='comment:16'></a>NB although this has a positive review, there;s a problem with it fixed by a oatch at #9765.  Would it not be better to change the patch here?",
...
<a id='comment:16'></a>NB although this has a positive review, there;s a problem with it fixed by a oatch at #9765.  Would it not be better to change the patch here?
