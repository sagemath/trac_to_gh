# Issue 9234: Bring doc coverage of plot/plot3d/texture.py to 100%

archive/issues_009234.json:
```json
{
    "body": "from 30% (3/10)\n\nAssignee: @seblabbe\n\nCC:  sage-combinat\n\nResolution: fixed\n\nAuthor: S\u00e9bastien Labb\u00e9\n\nReviewer: Karl-Dieter Crisman, John Palmieri\n\nMerged: sage-4.5.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/9234\n\n",
    "closed_at": "2010-07-21T10:03:40Z",
    "created_at": "2010-06-14T09:50:06Z",
    "labels": [
        "component: documentation"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.5.2",
    "title": "Bring doc coverage of plot/plot3d/texture.py to 100%",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9234",
    "user": "https://github.com/seblabbe"
}
```
from 30% (3/10)

Assignee: @seblabbe

CC:  sage-combinat

Resolution: fixed

Author: Sébastien Labbé

Reviewer: Karl-Dieter Crisman, John Palmieri

Merged: sage-4.5.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/9234





---

archive/issue_comments_103262.json:
```json
{
    "body": "Attachment [trac_9234_doc_texture-sl.patch](tarball://root/attachments/some-uuid/ticket9234/trac_9234_doc_texture-sl.patch) by @seblabbe created at 2010-06-14 09:52:18",
    "created_at": "2010-06-14T09:52:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103262",
    "user": "https://github.com/seblabbe"
}
```

Attachment [trac_9234_doc_texture-sl.patch](tarball://root/attachments/some-uuid/ticket9234/trac_9234_doc_texture-sl.patch) by @seblabbe created at 2010-06-14 09:52:18



---

archive/issue_comments_103263.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-06-14T09:54:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103263",
    "user": "https://github.com/seblabbe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_103264.json:
```json
{
    "body": "Attachment [trac_9234-reviewer.patch](tarball://root/attachments/some-uuid/ticket9234/trac_9234-reviewer.patch) by @kcrisman created at 2010-06-18 19:30:57\n\nApply after initial patch",
    "created_at": "2010-06-18T19:30:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103264",
    "user": "https://github.com/kcrisman"
}
```

Attachment [trac_9234-reviewer.patch](tarball://root/attachments/some-uuid/ticket9234/trac_9234-reviewer.patch) by @kcrisman created at 2010-06-18 19:30:57

Apply after initial patch



---

archive/issue_comments_103265.json:
```json
{
    "body": "<a id='comment:2'></a>Looks nice!  This corrects a few minor typos, adds some examples at the top of the module, adds the document to the reference manual, and one or two other things.  The reviewer patch is all that needs to be checked now; try the command (from within the Sage directory)\n\n```\n./sage -docbuild reference html\n```\nto create the changed manual, after building the reviewer patch.",
    "created_at": "2010-06-18T19:32:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103265",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>Looks nice!  This corrects a few minor typos, adds some examples at the top of the module, adds the document to the reference manual, and one or two other things.  The reviewer patch is all that needs to be checked now; try the command (from within the Sage directory)

```
./sage -docbuild reference html
```
to create the changed manual, after building the reviewer patch.



---

archive/issue_comments_103266.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Karl-Dieter Crisman\"",
    "created_at": "2010-06-18T19:32:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103266",
    "user": "https://github.com/kcrisman"
}
```

Changing reviewer from "" to "Karl-Dieter Crisman"



---

archive/issue_comments_103267.json:
```json
{
    "body": "<a id='comment:3'></a>Reviewer patch looks good.",
    "created_at": "2010-06-21T19:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103267",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'></a>Reviewer patch looks good.



---

archive/issue_comments_103268.json:
```json
{
    "body": "Changing reviewer from \"Karl-Dieter Crisman\" to \"Karl-Dieter Crisman, John Palmieri\"",
    "created_at": "2010-06-21T19:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103268",
    "user": "https://github.com/jhpalmieri"
}
```

Changing reviewer from "Karl-Dieter Crisman" to "Karl-Dieter Crisman, John Palmieri"



---

archive/issue_comments_103269.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2010-06-21T19:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103269",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_022733.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-21T10:03:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9234#event-22733"
}
```



---

archive/issue_comments_103270.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-07-21T10:03:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103270",
    "user": "https://github.com/qed777"
}
```

Resolution: fixed



---

archive/issue_comments_103271.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.5.2.alpha0\"",
    "created_at": "2010-07-21T10:03:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9234#issuecomment-103271",
    "user": "https://github.com/qed777"
}
```

Changing merged from "" to "sage-4.5.2.alpha0"
