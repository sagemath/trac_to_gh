# Issue 9984: ECM fails to build on AIX 5.3 - A warning about linking a static library against a shared library

archive/issues_009983.json:
```json
{
    "body": "Using the following system: \n\n* IBM [RS/6000 7025 F50](http://publib.boulder.ibm.com/infocenter/pseries/v5r3/index.jsp?topic=/com.ibm.pseries.doc/hardware_docs/rs6000_7025f50series.htm)\n* 4 x 332 MHz 32-bit PowerPC CPUs\n* 3 GB RAM\n* A fair wide mixture of disks sizes (3 x 9 GB, 1 x 18 GB, 2 x 36 GB and 1 x 73 GB)\n* AIX 5.3 (A POSIX certified operating system)\n* gcc 4.2.4 downloaded from [pware](http://pware.hvcc.edu/)\n* DDS-4 tape drive \n\nECL fails to build properly. See the attached log. A rather obvious part of the failure message is:\n\n\n```\n*** Warning: Linking the shared library libecm.la against the\n*** static library /home/users/drkirkby/sage-4.6.alpha1/local/lib/libgmp.a is not portable!\n```\n\n\nAssignee: drkirkby\n\nCC:  @fchapoton\n\nUpstream: None of the above - read trac for reasoning.\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/9984\n\n",
    "closed_at": "2020-06-25T13:33:14Z",
    "created_at": "2010-09-23T20:34:12Z",
    "labels": [
        "component: porting: aix or hp-ux",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "ECM fails to build on AIX 5.3 - A warning about linking a static library against a shared library",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9984",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```
Using the following system: 

* IBM [RS/6000 7025 F50](http://publib.boulder.ibm.com/infocenter/pseries/v5r3/index.jsp?topic=/com.ibm.pseries.doc/hardware_docs/rs6000_7025f50series.htm)
* 4 x 332 MHz 32-bit PowerPC CPUs
* 3 GB RAM
* A fair wide mixture of disks sizes (3 x 9 GB, 1 x 18 GB, 2 x 36 GB and 1 x 73 GB)
* AIX 5.3 (A POSIX certified operating system)
* gcc 4.2.4 downloaded from [pware](http://pware.hvcc.edu/)
* DDS-4 tape drive 

ECL fails to build properly. See the attached log. A rather obvious part of the failure message is:


```
*** Warning: Linking the shared library libecm.la against the
*** static library /home/users/drkirkby/sage-4.6.alpha1/local/lib/libgmp.a is not portable!
```


Assignee: drkirkby

CC:  @fchapoton

Upstream: None of the above - read trac for reasoning.

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/9984





---

archive/issue_comments_120266.json:
```json
{
    "body": "Build failure observed on an RS/6000 running AIX 5.3",
    "created_at": "2010-09-23T20:35:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9984#issuecomment-120266",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

Build failure observed on an RS/6000 running AIX 5.3



---

archive/issue_events_025228.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25228"
}
```



---

archive/issue_comments_120267.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [ecm-6.2.1.p2.log](tarball://root/attachments/some-uuid/ticket9984/ecm-6.2.1.p2.log) by @jdemeyer created at 2013-08-13 15:35:53",
    "created_at": "2013-08-13T15:35:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9984#issuecomment-120267",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>Attachment [ecm-6.2.1.p2.log](tarball://root/attachments/some-uuid/ticket9984/ecm-6.2.1.p2.log) by @jdemeyer created at 2013-08-13 15:35:53



---

archive/issue_events_025229.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25229"
}
```



---

archive/issue_events_025230.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25230"
}
```



---

archive/issue_events_025231.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25231"
}
```



---

archive/issue_events_025232.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25232"
}
```



---

archive/issue_events_025233.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25233"
}
```



---

archive/issue_events_025234.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25234"
}
```



---

archive/issue_events_025235.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-15T18:39:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25235"
}
```



---

archive/issue_events_025236.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-15T18:39:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25236"
}
```



---

archive/issue_comments_120268.json:
```json
{
    "body": "<a id='comment:5'></a>I don't believe anyone's been maintaining support for AIX or HP-UX for some time.  Putting in sage-wishlist for now in case there is still a desire for it out there, otherwise these tickets should be closed (most of them are probably no longer relevant in any case but I have no obvious way to check this).",
    "created_at": "2019-01-15T18:39:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9984#issuecomment-120268",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>I don't believe anyone's been maintaining support for AIX or HP-UX for some time.  Putting in sage-wishlist for now in case there is still a desire for it out there, otherwise these tickets should be closed (most of them are probably no longer relevant in any case but I have no obvious way to check this).



---

archive/issue_comments_120269.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2020-06-23T21:26:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9984#issuecomment-120269",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_120270.json:
```json
{
    "body": "<a id='comment:6'></a>We should close this ticket as outdated.",
    "created_at": "2020-06-23T21:26:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9984#issuecomment-120270",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'></a>We should close this ticket as outdated.



---

archive/issue_events_025237.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-23T21:26:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25237"
}
```



---

archive/issue_events_025238.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-23T21:26:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25238"
}
```



---

archive/issue_events_025239.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-06-25T13:33:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9984#event-25239"
}
```



---

archive/issue_comments_120271.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2020-06-25T13:33:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9984#issuecomment-120271",
    "user": "https://github.com/fchapoton"
}
```

Resolution: invalid
