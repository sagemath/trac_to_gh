...
<a id='comment:2'></a>OK, tested on Debian Linux amd64, and on MacOSX PPC with gcc4.2.
The change seems to be more ideological than adding more functionality/bugfixing.
It would be nice if someone more versed in Sage had a look, whether this is not something 
alien ideologically...
...
```json
{
    "body": "<a id='comment:3'></a>indeed... and most importantly, it may have consisted in replacing \"return\" by \"yield\", if only those were available in Cython `:-p`\n\nNathann",
...
<a id='comment:3'></a>indeed... and most importantly, it may have consisted in replacing "return" by "yield", if only those were available in Cython `:-p`
