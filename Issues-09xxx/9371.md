# Issue 9371: Implement E.two_torsion_rank() over number fields

archive/issues_009371.json:
```json
{
    "body": "The function E.two_torsion_rank() can easily be made to work over number fields. The current implementation over QQ calls E.torsion_subgroup() and makes nontrivial use of Mazur's torsion theorem. This should be more efficient and more general by considering the 2-division polynomial.\n\nAssignee: weigandt\n\nCC:  @JohnCremona\n\nKeywords: elliptic curves, two torsion rank\n\nResolution: fixed\n\nAuthor: Jamie Weigandt, Aly Deines\n\nReviewer: John Cremona, Gagan Sekhon\n\nMerged: sage-4.7.alpha5\n\nIssue created by migration from https://trac.sagemath.org/ticket/9371\n\n",
    "closed_at": "2011-04-12T08:04:54Z",
    "created_at": "2010-06-29T04:12:52Z",
    "labels": [
        "component: elliptic curves"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.7",
    "title": "Implement E.two_torsion_rank() over number fields",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9371",
    "user": "https://trac.sagemath.org/admin/accounts/users/weigandt"
}
```
The function E.two_torsion_rank() can easily be made to work over number fields. The current implementation over QQ calls E.torsion_subgroup() and makes nontrivial use of Mazur's torsion theorem. This should be more efficient and more general by considering the 2-division polynomial.

Assignee: weigandt

CC:  @JohnCremona

Keywords: elliptic curves, two torsion rank

Resolution: fixed

Author: Jamie Weigandt, Aly Deines

Reviewer: John Cremona, Gagan Sekhon

Merged: sage-4.7.alpha5

Issue created by migration from https://trac.sagemath.org/ticket/9371





---

archive/issue_comments_106222.json:
```json
{
    "body": "Changing author from \"weigandt\" to \"Jamie Weigandt\"",
    "created_at": "2010-06-29T22:27:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106222",
    "user": "https://trac.sagemath.org/admin/accounts/users/weigandt"
}
```

Changing author from "weigandt" to "Jamie Weigandt"



---

archive/issue_comments_106223.json:
```json
{
    "body": "Extend E.two_torsion_rank() method to number fields. Applies to 4.4.4",
    "created_at": "2010-07-01T18:36:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106223",
    "user": "https://trac.sagemath.org/admin/accounts/users/weigandt"
}
```

Extend E.two_torsion_rank() method to number fields. Applies to 4.4.4



---

archive/issue_comments_106224.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-07-01T18:37:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106224",
    "user": "https://trac.sagemath.org/admin/accounts/users/weigandt"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_106225.json:
```json
{
    "body": "<a id='comment:2'></a>Attachment [trac_9371_two_torsion_rank.patch](tarball://root/attachments/some-uuid/ticket9371/trac_9371_two_torsion_rank.patch) by weigandt created at 2010-07-01 18:37:27",
    "created_at": "2010-07-01T18:37:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106225",
    "user": "https://trac.sagemath.org/admin/accounts/users/weigandt"
}
```

<a id='comment:2'></a>Attachment [trac_9371_two_torsion_rank.patch](tarball://root/attachments/some-uuid/ticket9371/trac_9371_two_torsion_rank.patch) by weigandt created at 2010-07-01 18:37:27



---

archive/issue_comments_106226.json:
```json
{
    "body": "<a id='comment:3'></a>Looks good: a better method and more general.   However:  why not move the function all the way up to ell_field?  There's no reason at all why the same code would not work over any field of char. not 2, and even in char. 2 (where the result is at most 0 or 1 for supersingular/ordinary curves, but so what).\n\nIf you do that, add extra doctests over (say) finite fields.\nWhile you are at it, one thing about the docstring could be improved:  the short description should fit on one line, so cut it after E(K), and put the rest into a separate ALGORITHM block.\n \n\"Needs work\" sounds negative, so let me elaborate: this is good and with a tiny amount of work would be very good!",
    "created_at": "2010-07-05T16:47:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106226",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:3'></a>Looks good: a better method and more general.   However:  why not move the function all the way up to ell_field?  There's no reason at all why the same code would not work over any field of char. not 2, and even in char. 2 (where the result is at most 0 or 1 for supersingular/ordinary curves, but so what).

If you do that, add extra doctests over (say) finite fields.
While you are at it, one thing about the docstring could be improved:  the short description should fit on one line, so cut it after E(K), and put the rest into a separate ALGORITHM block.
 
"Needs work" sounds negative, so let me elaborate: this is good and with a tiny amount of work would be very good!



---

archive/issue_comments_106227.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2010-07-05T16:47:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106227",
    "user": "https://github.com/JohnCremona"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_106228.json:
```json
{
    "body": "Changing reviewer from \"\" to \"John Cremona\"",
    "created_at": "2010-07-05T16:47:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106228",
    "user": "https://github.com/JohnCremona"
}
```

Changing reviewer from "" to "John Cremona"



---

archive/issue_comments_106229.json:
```json
{
    "body": "new patch, replaced old one, applies to 4.6.2",
    "created_at": "2011-03-23T01:41:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106229",
    "user": "https://trac.sagemath.org/admin/accounts/users/weigandt"
}
```

new patch, replaced old one, applies to 4.6.2



---

archive/issue_comments_106230.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2011-03-23T01:42:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106230",
    "user": "https://trac.sagemath.org/admin/accounts/users/weigandt"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_106231.json:
```json
{
    "body": "<a id='comment:4'></a>Attachment [trac_9371_field_two_torsion.patch](tarball://root/attachments/some-uuid/ticket9371/trac_9371_field_two_torsion.patch) by weigandt created at 2011-03-23 01:42:01",
    "created_at": "2011-03-23T01:42:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106231",
    "user": "https://trac.sagemath.org/admin/accounts/users/weigandt"
}
```

<a id='comment:4'></a>Attachment [trac_9371_field_two_torsion.patch](tarball://root/attachments/some-uuid/ticket9371/trac_9371_field_two_torsion.patch) by weigandt created at 2011-03-23 01:42:01



---

archive/issue_comments_106232.json:
```json
{
    "body": "<a id='comment:5'></a>I think there should be at least 2 more different examples like you had before. Either you can add then I can review or I can add and we will need to find a new reviewer",
    "created_at": "2011-03-23T16:29:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106232",
    "user": "https://trac.sagemath.org/admin/accounts/users/gagansekhon"
}
```

<a id='comment:5'></a>I think there should be at least 2 more different examples like you had before. Either you can add then I can review or I can add and we will need to find a new reviewer



---

archive/issue_comments_106233.json:
```json
{
    "body": "added documentation, replaces previous patch",
    "created_at": "2011-03-23T17:17:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106233",
    "user": "https://github.com/adeines"
}
```

added documentation, replaces previous patch



---

archive/issue_comments_106234.json:
```json
{
    "body": "Changing author from \"Jamie Weigandt\" to \"Jamie Weigandt, Aly.deines\"",
    "created_at": "2011-03-23T22:06:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106234",
    "user": "https://trac.sagemath.org/admin/accounts/users/gagansekhon"
}
```

Changing author from "Jamie Weigandt" to "Jamie Weigandt, Aly.deines"



---

archive/issue_comments_106235.json:
```json
{
    "body": "Changing reviewer from \"John Cremona\" to \"John Cremona, Gagan Sekhon\"",
    "created_at": "2011-03-23T22:06:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106235",
    "user": "https://trac.sagemath.org/admin/accounts/users/gagansekhon"
}
```

Changing reviewer from "John Cremona" to "John Cremona, Gagan Sekhon"



---

archive/issue_comments_106236.json:
```json
{
    "body": "<a id='comment:6'></a>Attachment [trac_9371_field_two_torsion.2.patch](tarball://root/attachments/some-uuid/ticket9371/trac_9371_field_two_torsion.2.patch) by gagansekhon created at 2011-03-23 22:06:25\n\nit initially failed sage/interface/r.py, but once I ran it separately and it worked.",
    "created_at": "2011-03-23T22:06:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106236",
    "user": "https://trac.sagemath.org/admin/accounts/users/gagansekhon"
}
```

<a id='comment:6'></a>Attachment [trac_9371_field_two_torsion.2.patch](tarball://root/attachments/some-uuid/ticket9371/trac_9371_field_two_torsion.2.patch) by gagansekhon created at 2011-03-23 22:06:25

it initially failed sage/interface/r.py, but once I ran it separately and it worked.



---

archive/issue_comments_106237.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-03-23T22:06:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106237",
    "user": "https://trac.sagemath.org/admin/accounts/users/gagansekhon"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_106238.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-04-12T08:04:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106238",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_106239.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.7.alpha5\"",
    "created_at": "2011-04-12T08:04:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106239",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.7.alpha5"



---

archive/issue_events_023113.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-12T08:04:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9371#event-23113"
}
```



---

archive/issue_comments_106240.json:
```json
{
    "body": "Changing author from \"Jamie Weigandt, Aly.deines\" to \"Jamie Weigandt, Aly Deines\"",
    "created_at": "2011-05-03T13:28:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9371",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9371#issuecomment-106240",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "Jamie Weigandt, Aly.deines" to "Jamie Weigandt, Aly Deines"
