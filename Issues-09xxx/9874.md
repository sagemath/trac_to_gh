# Issue 9874: add a function for the derivative of ceil and floor

archive/issues_009873.json:
```json
{
    "assignees": [],
    "body": "We should define a new symbolic function for the derivative of `ceil` or `floor`. \n\nIn Maple:\n\n```\n> diff(floor(x),x);\n                                  floor(1, x)\n> diff(floor(x),x,x);\n                                  floor(1, x)\n> diff(ceil(x),x,x); \n                                  floor(1, x)\n> eval(diff(ceil(x),x,x),x=1.5);\n                                       0\n> eval(diff(ceil(x),x,x),x=0);  \nError, (in floor) floor is not differentiable at integers\n```\n\nAssignee: **@burcin**\n\nCC:  ktkohl @rwst @kcrisman @pelegm\n\nKeywords: **sd35.5**\n\nBranch/Commit: **[u/ktkohl/add_a_function_for_the_derivative_of_ceil_and_floor](https://github.com/sagemath/sagetrac-mirror/tree/u/ktkohl/add_a_function_for_the_derivative_of_ceil_and_floor) @ [8013b9f](https://github.com/sagemath/sagetrac-mirror/commit/8013b9f89b3670847007f6d3560252035a899638)**\n\nAuthor: **Karen Kohl**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9874_\n\n",
    "created_at": "2010-09-08T11:41:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add a function for the derivative of ceil and floor",
    "type": "issue",
    "updated_at": "2022-12-29T01:32:56Z",
    "url": "https://github.com/sagemath/sage/issues/9874",
    "user": "https://github.com/burcin"
}
```
We should define a new symbolic function for the derivative of `ceil` or `floor`. 

In Maple:

```
> diff(floor(x),x);
                                  floor(1, x)
> diff(floor(x),x,x);
                                  floor(1, x)
> diff(ceil(x),x,x); 
                                  floor(1, x)
> eval(diff(ceil(x),x,x),x=1.5);
                                       0
> eval(diff(ceil(x),x,x),x=0);  
Error, (in floor) floor is not differentiable at integers
```

Assignee: **@burcin**

CC:  ktkohl @rwst @kcrisman @pelegm

Keywords: **sd35.5**

Branch/Commit: **[u/ktkohl/add_a_function_for_the_derivative_of_ceil_and_floor](https://github.com/sagemath/sagetrac-mirror/tree/u/ktkohl/add_a_function_for_the_derivative_of_ceil_and_floor) @ [8013b9f](https://github.com/sagemath/sagetrac-mirror/commit/8013b9f89b3670847007f6d3560252035a899638)**

Author: **Karen Kohl**

_Issue created by migration from https://trac.sagemath.org/ticket/9874_





---

archive/issue_events_116907.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-09-08T11:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116907"
}
```



---

archive/issue_events_116908.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-09-08T11:41:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116908"
}
```



---

archive/issue_events_116909.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-09-08T11:41:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116909"
}
```



---

archive/issue_events_116910.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-09-08T11:41:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116910"
}
```



---

archive/issue_comments_090444.json:
```json
{
    "body": "Changed keywords from **** to **sd35.5**",
    "created_at": "2012-01-12T14:48:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9874#issuecomment-90444",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Changed keywords from **** to **sd35.5**



---

archive/issue_events_116911.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116911"
}
```



---

archive/issue_events_116912.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116912"
}
```



---

archive/issue_events_116913.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116913"
}
```



---

archive/issue_events_116914.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116914"
}
```



---

archive/issue_events_116915.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116915"
}
```



---

archive/issue_events_116916.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116916"
}
```



---

archive/issue_events_116917.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116917"
}
```



---

archive/issue_events_116918.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116918"
}
```



---

archive/issue_comments_090445.json:
```json
{
    "body": "Author: **ktkohl**",
    "created_at": "2015-05-24T23:04:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9874#issuecomment-90445",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Author: **ktkohl**



---

archive/issue_comments_090446.json:
```json
{
    "body": "Branch: **[u/ktkohl/add_a_function_for_the_derivative_of_ceil_and_floor](https://github.com/sagemath/sagetrac-mirror/tree/u/ktkohl/add_a_function_for_the_derivative_of_ceil_and_floor)**",
    "created_at": "2015-05-25T18:42:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9874#issuecomment-90446",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Branch: **[u/ktkohl/add_a_function_for_the_derivative_of_ceil_and_floor](https://github.com/sagemath/sagetrac-mirror/tree/u/ktkohl/add_a_function_for_the_derivative_of_ceil_and_floor)**



---

archive/issue_comments_090447.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nThese give the right answer but also a runtime error:\n\n```\nsage: floor(x).derivative().subs(x=1.5)\nException RuntimeError: 'maximum recursion depth exceeded while calling a Python object' in 'sage.structure.parent.good_as_coerce_domain' ignored\n0\nsage: ceil(x).derivative().subs(x=1.5)\nException RuntimeError: 'maximum recursion depth exceeded while calling a Python object' in 'sage.structure.parent.good_as_coerce_domain' ignored\n0\n```\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8013b9f89b3670847007f6d3560252035a899638\">8013b9f</a></td><td><code>Added ceiling and floor functions to allow 2nd argument for derivative.</code></td></tr></table>\n",
    "created_at": "2015-05-25T18:44:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9874#issuecomment-90447",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

<a id='comment:8'>Comment 8:</a>
These give the right answer but also a runtime error:

```
sage: floor(x).derivative().subs(x=1.5)
Exception RuntimeError: 'maximum recursion depth exceeded while calling a Python object' in 'sage.structure.parent.good_as_coerce_domain' ignored
0
sage: ceil(x).derivative().subs(x=1.5)
Exception RuntimeError: 'maximum recursion depth exceeded while calling a Python object' in 'sage.structure.parent.good_as_coerce_domain' ignored
0
```

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8013b9f89b3670847007f6d3560252035a899638">8013b9f</a></td><td><code>Added ceiling and floor functions to allow 2nd argument for derivative.</code></td></tr></table>




---

archive/issue_comments_090448.json:
```json
{
    "body": "Commit: **[8013b9f](https://github.com/sagemath/sagetrac-mirror/commit/8013b9f89b3670847007f6d3560252035a899638)**",
    "created_at": "2015-05-25T18:44:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9874#issuecomment-90448",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Commit: **[8013b9f](https://github.com/sagemath/sagetrac-mirror/commit/8013b9f89b3670847007f6d3560252035a899638)**



---

archive/issue_comments_090449.json:
```json
{
    "body": "Changed author from **ktkohl** to **Karen Kohl**",
    "created_at": "2015-05-25T18:45:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9874#issuecomment-90449",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Changed author from **ktkohl** to **Karen Kohl**



---

archive/issue_comments_090450.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nJust a quick note that there are empty examples and tests fields but you probably know that.",
    "created_at": "2015-05-26T06:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9874#issuecomment-90450",
    "user": "https://github.com/rwst"
}
```

<a id='comment:11'>Comment 11:</a>
Just a quick note that there are empty examples and tests fields but you probably know that.



---

archive/issue_events_116919.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-05-26T06:11:29Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116919"
}
```



---

archive/issue_events_116920.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-05-26T06:11:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116920"
}
```



---

archive/issue_events_116921.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:32:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9874",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9874#event-116921"
}
```
