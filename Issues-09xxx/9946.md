...
```json
{
    "body": "Fran\u00e7ois has posted a patch at #9733 for the libfac problem.  We could merge it into a new singular-3-1-1-4.p3.spkg here.",
...
```json
{
    "body": "Patch attached with a slightly more descriptive name.",
...
```json
{
    "body": "P.S.: For `deps`, I'd prefer *generating* it from *formal* (dependency) specs given in (or for) the spkgs, like every proper package system / installer does. These usually also include specs of the required *versions*.\n\nSorry, a bit off-ticket-topic.",
