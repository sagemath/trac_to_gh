...
```json
{
    "body": "<a id='comment:1'></a>Attachment [readline-6.0.p2.spkg.patch](tarball://root/attachments/some-uuid/ticket9530/readline-6.0.p2.spkg.patch) by cwitty created at 2010-07-17 21:36:55\n\nIs it actually worth fixing the workarounds?  It seems like upgrading readline (#9523) wouldn't be much more work, and then we could probably just remove the workarounds.",
...
<a id='comment:1'></a>Attachment [readline-6.0.p2.spkg.patch](tarball://root/attachments/some-uuid/ticket9530/readline-6.0.p2.spkg.patch) by cwitty created at 2010-07-17 21:36:55
...
<a id='comment:2'></a>It might fix the problem for now. And when the system readline is upgraded again, everything is broken again.
...
<a id='comment:3'></a>What if you used symlinks to the native libreadline instead of copying it?
...
```json
{
    "body": "<a id='comment:4'></a>The formatting got messed up.\u00a0 Here is an example:\n\nThis:\n\ncp -a /lib64/libreadline.so.6* \"$SAGE_LOCAL\"/lib \n\nbecomes:\n\nln -s /lib64/libreadline.so.6* \"$SAGE_LOCAL\"/lib",
...
<a id='comment:4'></a>The formatting got messed up.  Here is an example:
...
<a id='comment:6'></a>Since the update for readline has been pushed back to at least 4.6.1, can we get this workaround in 4.6?  We have a patch that fixes the problem, is small, and is low-risk.  With the switch from 4.5 to 4.6 I think it is important that it successfully compiles on significant distributions like openSUSE and Arch.
...
<a id='comment:7'></a>Though the patch is not a Mercurial changeset...
...
```json
{
    "body": "<a id='comment:8'></a>I was thinking this made more as temporary workaround until 9523 render the workarounds unnecessary.\n\nI should add that I tried this patch and it worked.",
...
<a id='comment:8'></a>I was thinking this made more as temporary workaround until 9523 render the workarounds unnecessary.
...
<a id='comment:9'></a>Well, the milestone of #9523 has been set to Sage 4.6.1.
...
<a id='comment:11'></a>Replying to [TheBlackCat](#comment%3A10):
> Replying to [leif](#comment%3A9):
> > Well, the milestone of #9523 has been set to Sage 4.6.1.
...
<a id='comment:13'></a>Replying to [TheBlackCat](#comment%3A10):
> Replying to [leif](#comment%3A9):
> > Well, the milestone of #9523 has been set to Sage 4.6.1.
...
<a id='comment:14'></a>Replying to [drkirkby](#comment%3A13):
> However, in my opinion we should not hold a release of Sage just because on Linux distro has bought at yet another backwards incompatible version.
...
<a id='comment:16'></a>Replying to [baechler](#comment%3A14):
> Replying to [leif](#comment%3A12):
> > ... and also a patched new spkg. I do not even know the name of the author, baechler.
...
<a id='comment:17'></a>Replying to [baechler](#comment%3A14):
> Replying to [drkirkby](#comment%3A13):
> > However, in my opinion we should not hold a release of Sage just because on Linux distro has bought at yet another backwards incompatible version.
...
<a id='comment:18'></a>Replying to [leif](#comment%3A16):
> Replying to [TheBlackCat](#comment%3A15):
> > Further, a fix is available, and has been for over 3 months now (shortly after openSUSE 11.3 was released). The fix was published several weeks before sage 4.5.2 was released, over a month and a half before 4.5.3.
...
<a id='comment:19'></a>Replying to [TheBlackCat](#comment%3A18):
> Replying to [leif](#comment%3A16):
> > Unfortunately, the ticket hadn't been set to "needs review", there are apparently only a few Sage *developers* dealing with OpenSuSE, 
...
```json
{
    "body": "<a id='comment:20'></a>s/#9530/#9523/",
...
```json
{
    "body": "<a id='comment:23'></a>Would a `readline-6.0.p3.spkg` with the attached patch definitely fix the broken builds on Arch Linux and/or openSUSE, at least in the short term?  I'm willing to merge this into 4.6.",
...
<a id='comment:23'></a>Would a `readline-6.0.p3.spkg` with the attached patch definitely fix the broken builds on Arch Linux and/or openSUSE, at least in the short term?  I'm willing to merge this into 4.6.
...
```json
{
    "body": "<a id='comment:24'></a>See the description for a link to a p3 spkg with Thomas' patch.  I'll attach a Mercurial spkg patch shortly.  Note: I have not tested the new package.  Please do this!",
    "created_at": "2010-10-25T02:20:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9530",
    "type": "issue_comment",
...
<a id='comment:24'></a>See the description for a link to a p3 spkg with Thomas' patch.  I'll attach a Mercurial spkg patch shortly.  Note: I have not tested the new package.  Please do this!
...
```json
{
    "body": "<a id='comment:25'></a>Attachment [trac_9530-spkg_update_openSUSE_Arch_workarounds.patch](tarball://root/attachments/some-uuid/ticket9530/trac_9530-spkg_update_openSUSE_Arch_workarounds.patch) by TheBlackCat created at 2010-10-25 02:41:30\n\nThis should work at least through openSUSE 11.4.  The earliest it could be broken is an openSUSE release labeled 12.0.  That will not happen until at least 8 months after the release of 11.4, which is in March, so at least a year total.",
...
<a id='comment:25'></a>Attachment [trac_9530-spkg_update_openSUSE_Arch_workarounds.patch](tarball://root/attachments/some-uuid/ticket9530/trac_9530-spkg_update_openSUSE_Arch_workarounds.patch) by TheBlackCat created at 2010-10-25 02:41:30
...
<a id='comment:26'></a>Well, the current patch(es) will only work if a *development* version of readline is already installed.
...
```json
{
    "body": "<a id='comment:27'></a>P.S.: Anyway, probably better than what we currently have in Sage (the p2).",
...
<a id='comment:27'></a>P.S.: Anyway, probably better than what we currently have in Sage (the p2).
...
<a id='comment:28'></a>I  believe the `cp -a` will only be executed on Linux, so it's not a massive problem here, but note that is note that `-a` is not an option defined by POSIX, so is unportable and will break on some systems. Was there a good reason for using `-a`? Unless there was, it would be good to get into the habbit of using a more portable alternative - I suspect `-p` would have done the job here. 
...
<a id='comment:29'></a>drkirkby, cp -a is a rough equivalent to cp -dpR - as -d is (according to your post) not portable, this might be problematic, as -d copies symlinks as symlinks, instead of copying the underlying file (this is what I intended there).
...
<a id='comment:30'></a>Replying to [baechler](#comment%3A29):
> drkirkby, cp -a is a rough equivalent to cp -dpR - as -d is (according to your post) not portable, this might be problematic, as -d copies symlinks as symlinks, instead of copying the underlying file (this is what I intended there).
...
<a id='comment:31'></a>Replying to [baechler](#comment%3A29):
> 2) I am thinking to extend this test further: Instead of testing for a particular distribution, test whether libreadline.so.6 is installed and the development headers are present - and in those cases, never build libreadline. This way, only systems lacking libreadline or providing the old libreadline.so.5 would have to build libreadline, all others would use the system's library.
...
<a id='comment:32'></a>Replying to [leif](#comment%3A31):
> The situation with readline and bash is even more specific, since it's IMHO a bad idea to dynamically link the system shell against it, which is only the case on OpenSuSE and Arch.
...
<a id='comment:33'></a>Replying to [baechler](#comment%3A32):
> Replying to [leif](#comment%3A31):
> > The situation with readline and bash is even more specific, since it's IMHO a bad idea to dynamically link the system shell against it, which is only the case on OpenSuSE and Arch.
...
<a id='comment:34'></a>Replying to [drkirkby](#comment%3A28):
> I  believe the `cp -a` will only be executed on Linux, so it's not a massive problem here, but note that is note that `-a` is not an option defined by POSIX, so is unportable and will break on some systems. Was there a good reason for using `-a`? Unless there was, it would be good to get into the habbit of using a more portable alternative
...
<a id='comment:35'></a>Replying to [leif](#comment%3A34):
> Also, I think `ln -s ...` should be `ln -snf ...` (or at least `ln -sf ...`).
...
<a id='comment:36'></a>Can someone (Thomas?) please update a patch or the spkg with the suggested improvements?
...
<a id='comment:37'></a>Replying to [mpatel](#comment%3A36):
> The main remaining potential blockers to releasing 4.6 are this ticket and the other problem Florent Hivert reported on [sage-release](http://groups.google.com/group/sage-release/browse_thread/thread/bb636656e2153332) (cf. [comment:ticket:10097:16 comment 16ff] at #10097).
> 
> I'll investigate the latter later today.
...
<a id='comment:38'></a>Attachment [trac_9530-spkg_update_openSUSE_Arch_workarounds.2.patch](tarball://root/attachments/some-uuid/ticket9530/trac_9530-spkg_update_openSUSE_Arch_workarounds.2.patch) by @qed777 created at 2010-10-28 02:28:56
...
<a id='comment:39'></a>Replying to [mpatel](#comment%3A38):
> I've updated the patch to use `uname -m` and `ln -snf`.  I've left `cp -a` in it, since the enclosing block is Linux-specific.
...
<a id='comment:40'></a>Attachment [trac_9530-old_p3_vs_p4_reviewer.patch](tarball://root/attachments/some-uuid/ticket9530/trac_9530-old_p3_vs_p4_reviewer.patch) by drkirkby created at 2010-10-28 03:45:53
...
```json
{
    "body": "<a id='comment:42'></a>I'm happy to go with Leif's patch and make a p4 with it.  Leif, do you want to remove `-n` from the `ln` commands?",
...
<a id='comment:42'></a>I'm happy to go with Leif's patch and make a p4 with it.  Leif, do you want to remove `-n` from the `ln` commands?
...
<a id='comment:43'></a>**New alternate spkg: http://spkg-upload.googlecode.com/files/readline-6.0.p4.spkg**
...
<a id='comment:44'></a>Replying to [mpatel](#comment%3A42):
> I'm happy to go with Leif's patch and make a p4 with it.  Leif, do you want to remove `-n` from the `ln` commands?
...
<a id='comment:46'></a>I just got this on openSUSE 11.3
...
<a id='comment:48'></a>Undoubtadly unrelated, but I can't get ATLAS to build, in the virtual machine:
...
<a id='comment:49'></a>Replying to [drkirkby](#comment%3A46):
> I'll follow the instructions.
...
```json
{
    "body": "<a id='comment:50'></a>Hmmm, though it doesn't hurt here, we don't need the `-n` when creating the symbolic links to the libraries. It's only relevant for directories.\n\nBut I'm not going to change the spkg again; there's always a #9523...",
...
<a id='comment:50'></a>Hmmm, though it doesn't hurt here, we don't need the `-n` when creating the symbolic links to the libraries. It's only relevant for directories.
...
<a id='comment:51'></a>Replying to [leif](#comment%3A49):
> Replying to [drkirkby](#comment%3A46):
> > I'll follow the instructions.
...
<a id='comment:52'></a>Replying to [drkirkby](#comment%3A51):
> BTW, I don't see the need to copy the readline files to $SAGE_LOCAL - why not create links? It saves disk space and CPU cycles. 
...
<a id='comment:53'></a>Replying to [drkirkby](#comment%3A48):
...
<a id='comment:54'></a>Replying to [mpatel](#comment%3A53):
> Replying to [drkirkby](#comment%3A48):
> 
> > Undoubtadly unrelated, but I can't get ATLAS to build, in the virtual machine: ... so I can't check this myself.
...
<a id='comment:55'></a>Despite being unable to check this fully, I'm going to give it a positive review. 
...
<a id='comment:56'></a>Replying to [mpatel](#comment%3A53):
> Replying to [drkirkby](#comment%3A48):
> 
> > Undoubtedly unrelated, but I can't get ATLAS to build, in the virtual machine: ... so I can't check this myself.
...
<a id='comment:57'></a>Replying to [drkirkby](#comment%3A55):
> I'm sure Leif has checked it builds on his Linux.
...
<a id='comment:58'></a>Replying to [leif](#comment%3A57):
> Replying to [drkirkby](#comment%3A55):
...
```json
{
    "body": "<a id='comment:59'></a>Formality.",
