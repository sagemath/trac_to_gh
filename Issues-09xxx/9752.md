# Issue 9752: sorting of number field elements is broken

archive/issues_009752.json:
```json
{
    "assignees": [],
    "body": "The design of number field elements (and other aspects of sage) assumes that cmp defines a total ordering, which of course doesn't respect the algebraic field structure.   Unfortunately, the actual implementation (in number_field_element.pyx) is buggy and doesn't define a total ordering.   Look at the code and you'll see.  Or just look at this example:\n\n```\nsage: L.<b> = NumberField(x^3-10001)\nsage: b+1 < L(1667)\nFalse\nsage: L(1667) < b+1\nFalse\n```\n\n\n\nI think the best correct implementation of cmp should be one that is efficient and *also* agrees with the lexicographic ordering of elements based on their representation as a polynomial in the generator of the number field.   I did implement this as part of the patch bomb #9541.  The point of the present ticket is to \"backport\" something like this out of #9541, or implement a new fix from scratch.  This is motivated by #9400. \n\nAssignee: **@loefflerd**\n\nReviewer: **William Stein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9752_\n\n",
    "closed_at": "2011-03-21T01:17:13Z",
    "created_at": "2010-08-15T17:46:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sorting of number field elements is broken",
    "type": "issue",
    "updated_at": "2011-03-21T08:33:58Z",
    "url": "https://github.com/sagemath/sage/issues/9752",
    "user": "https://github.com/williamstein"
}
```
The design of number field elements (and other aspects of sage) assumes that cmp defines a total ordering, which of course doesn't respect the algebraic field structure.   Unfortunately, the actual implementation (in number_field_element.pyx) is buggy and doesn't define a total ordering.   Look at the code and you'll see.  Or just look at this example:

```
sage: L.<b> = NumberField(x^3-10001)
sage: b+1 < L(1667)
False
sage: L(1667) < b+1
False
```



I think the best correct implementation of cmp should be one that is efficient and *also* agrees with the lexicographic ordering of elements based on their representation as a polynomial in the generator of the number field.   I did implement this as part of the patch bomb #9541.  The point of the present ticket is to "backport" something like this out of #9541, or implement a new fix from scratch.  This is motivated by #9400. 

Assignee: **@loefflerd**

Reviewer: **William Stein**

_Issue created by migration from https://trac.sagemath.org/ticket/9752_





---

archive/issue_events_115161.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-08-15T17:46:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9752#event-115161"
}
```



---

archive/issue_events_115162.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-08-15T17:46:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9752#event-115162"
}
```



---

archive/issue_events_115163.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-08-15T17:46:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9752#event-115163"
}
```



---

archive/issue_events_115164.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-08-15T17:46:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9752#event-115164"
}
```



---

archive/issue_comments_088204.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nSee also #6132, there is some code to compare number field elements.",
    "created_at": "2010-09-20T19:26:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9752#issuecomment-88204",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'>Comment 1:</a>
See also #6132, there is some code to compare number field elements.



---

archive/issue_events_115165.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-21T01:17:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9752#event-115165"
}
```



---

archive/issue_events_115166.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-21T01:17:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9752#event-115166"
}
```



---

archive/issue_events_115167.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-21T01:17:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9752#event-115167"
}
```



---

archive/issue_comments_088205.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI'm closing this as a dup of #6132.",
    "created_at": "2011-03-21T01:17:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9752#issuecomment-88205",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'>Comment 2:</a>
I'm closing this as a dup of #6132.



---

archive/issue_comments_088206.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2011-03-21T08:33:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9752#issuecomment-88206",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **William Stein**



---

archive/issue_events_115168.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-21T08:33:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9752",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9752#event-115168"
}
```
