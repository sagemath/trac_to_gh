# Issue 9453: Implement Aurifeuillian factorization

archive/issues_009453.json:
```json
{
    "body": "Implement Aurifeuillian factorization of integers, see\nhttp://mathworld.wolfram.com/AurifeuilleanFactorization.html\n\nAssignee: tbd\n\nResolution: fixed\n\nDependencies: #5945, #10623\n\nAuthor: Andr\u00e9 Apitzsch\n\nReviewer: Mariah Lenox\n\nMerged: sage-4.7.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/9453\n\n",
    "closed_at": "2011-07-22T17:06:00Z",
    "created_at": "2010-07-08T14:12:32Z",
    "labels": [
        "component: factorization",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.7.2",
    "title": "Implement Aurifeuillian factorization",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9453",
    "user": "https://github.com/jdemeyer"
}
```
Implement Aurifeuillian factorization of integers, see
http://mathworld.wolfram.com/AurifeuilleanFactorization.html

Assignee: tbd

Resolution: fixed

Dependencies: #5945, #10623

Author: André Apitzsch

Reviewer: Mariah Lenox

Merged: sage-4.7.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/9453





---

archive/issue_comments_108127.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-12-08T10:06:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108127",
    "user": "https://github.com/a-andre"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_108128.json:
```json
{
    "body": "<a id='comment:1'></a>#5945 has to be applied first because of the new factorint.pyx module.",
    "created_at": "2010-12-08T10:06:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108128",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:1'></a>#5945 has to be applied first because of the new factorint.pyx module.



---

archive/issue_comments_108129.json:
```json
{
    "body": "<a id='comment:2'></a>Obviously, it should be implemented in general, not only for bases 2,3 and 5...",
    "created_at": "2010-12-08T10:16:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108129",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>Obviously, it should be implemented in general, not only for bases 2,3 and 5...



---

archive/issue_comments_108130.json:
```json
{
    "body": "Changing author from \"Jeroen Demeyer\" to \"\"",
    "created_at": "2010-12-08T10:16:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108130",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "Jeroen Demeyer" to ""



---

archive/issue_comments_108131.json:
```json
{
    "body": "<a id='comment:3'></a>Depends on #5945\n\nHere is a more general version.",
    "created_at": "2010-12-22T16:55:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108131",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:3'></a>Depends on #5945

Here is a more general version.



---

archive/issue_comments_108132.json:
```json
{
    "body": "Attachment [trac_9453_aurifeuillian_factorization.patch](tarball://root/attachments/some-uuid/ticket9453/trac_9453_aurifeuillian_factorization.patch) by @a-andre created at 2011-01-17 13:41:43\n\nDepends on #5945 and #10623",
    "created_at": "2011-01-17T13:41:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108132",
    "user": "https://github.com/a-andre"
}
```

Attachment [trac_9453_aurifeuillian_factorization.patch](tarball://root/attachments/some-uuid/ticket9453/trac_9453_aurifeuillian_factorization.patch) by @a-andre created at 2011-01-17 13:41:43

Depends on #5945 and #10623



---

archive/issue_comments_108133.json:
```json
{
    "body": "Changing author from \"\" to \"Andr\u00e9 Apitzsch\"",
    "created_at": "2011-01-19T14:11:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108133",
    "user": "https://github.com/a-andre"
}
```

Changing author from "" to "André Apitzsch"



---

archive/issue_comments_108134.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#5945, #10623\"",
    "created_at": "2011-05-11T09:54:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108134",
    "user": "https://github.com/a-andre"
}
```

Changing dependencies from "" to "#5945, #10623"



---

archive/issue_comments_108135.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-06-15T14:18:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108135",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_108136.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Mariah Lenox\"",
    "created_at": "2011-06-15T14:18:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108136",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

Changing reviewer from "" to "Mariah Lenox"



---

archive/issue_comments_108137.json:
```json
{
    "body": "<a id='comment:6'></a>Applied patch to sage-4.7.1.alpha2 and did 'make testlong'.  All tests passed.  Positive review!",
    "created_at": "2011-06-15T14:18:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108137",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

<a id='comment:6'></a>Applied patch to sage-4.7.1.alpha2 and did 'make testlong'.  All tests passed.  Positive review!



---

archive/issue_events_023378.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-16T21:34:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9453#event-23378"
}
```



---

archive/issue_comments_108138.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-07-22T17:06:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108138",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_023379.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-22T17:06:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9453#event-23379"
}
```



---

archive/issue_comments_108139.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.7.2.alpha0\"",
    "created_at": "2011-07-22T17:06:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9453#issuecomment-108139",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.7.2.alpha0"
