...
```json
{
    "body": "<a id='comment:3'></a>Would you like to make an spkg with current Fricas?\nI'll make sure to review it, etc.\nThanks!",
...
<a id='comment:3'></a>Would you like to make an spkg with current Fricas?
I'll make sure to review it, etc.
Thanks!
...
```json
{
    "body": "<a id='comment:4'></a>for the record, fricas-1.0.9 spkg fails to install in Sage 4.8 on x86_64 Linux (Debian).",
...
<a id='comment:4'></a>for the record, fricas-1.0.9 spkg fails to install in Sage 4.8 on x86_64 Linux (Debian).
...
<a id='comment:5'></a>Why are you considering 1.0.9? Above you find a link to 1.1.0. and
FriCAS progressed to 1.1.5.
...
<a id='comment:6'></a>Replying to [hemmecke](#comment%3A5):
> Why are you considering 1.0.9? Above you find a link to 1.1.0. and
> FriCAS progressed to 1.1.5.
...
<a id='comment:16'></a>I wrote the spkg-install and it at least compile on my computer... If anybody want to test it
- download the source tarball `fricas-1.2.4-full.tar.bz2`, move it to `$SAGE_ROOT/upstream` and rename it `fricas-1.2.4.tar.bz2`
- switch to the git branch provided here
- run `sage -i fricas` and then `make` (from `$SAGE_ROOT`)
...
```json
{
    "body": "<a id='comment:19'></a>Looks like it work -> needs review.",
...
<a id='comment:19'></a>Looks like it work -> needs review.
...
```json
{
    "body": "<a id='comment:21'></a>I have added a few '#optional - fricas' so that the tests pass both with '-optional=fricas' and without, in 'interfaces/fricas.py'\n\n---\nNew commits:",
...
<a id='comment:21'></a>I have added a few '#optional - fricas' so that the tests pass both with '-optional=fricas' and without, in 'interfaces/fricas.py'
...
```json
{
    "body": "<a id='comment:22'></a>Looks good to me. If somebody else can confirm, this seems to be good to go.",
...
<a id='comment:22'></a>Looks good to me. If somebody else can confirm, this seems to be good to go.
...
```json
{
    "body": "<a id='comment:23'></a>Wow, fricas has Puiseux series. Builds fine. Passes all tests in `interfaces`, `rings`, and `structure`.",
...
<a id='comment:23'></a>Wow, fricas has Puiseux series. Builds fine. Passes all tests in `interfaces`, `rings`, and `structure`.
...
<a id='comment:24'></a>1. See also http://fricas.github.io
...
<a id='comment:25'></a>Replying to [hemmecke](#comment%3A24):
> 1. See also http://fricas.github.io
...
<a id='comment:27'></a>Replying to [hemmecke](#comment%3A26):
> Replying to [vdelecroix](#comment%3A25):
>  
> > > 2. What exactly is the problem with fricas-aldor on 64bit machines?
...
```json
{
    "body": "<a id='comment:28'></a>See #9427 for a follow-up ticket on integration.",
...
<a id='comment:28'></a>See #9427 for a follow-up ticket on integration.
...
```json
{
    "body": "<a id='comment:29'></a>No $`@`#$ sourceforge links the next time, please.",
...
<a id='comment:29'></a>No $`@`#$ sourceforge links the next time, please.
...
```json
{
    "body": "<a id='comment:31'></a>FYI: optional doctests don't pass, so the package was moved to experimental.",
...
<a id='comment:31'></a>FYI: optional doctests don't pass, so the package was moved to experimental.
