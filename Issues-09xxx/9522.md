# Issue 9522: MPIR: Don't check SAGE_CHECK in spkg-install

archive/issues_009522.json:
```json
{
    "assignees": [],
    "body": "From the end of MPIR's `spkg-install`:\n\n```sh\nif [ \"$SAGE_CHECK\" = \"yes\" ]; then\n    cd ..; ./spkg-check\nfi\n```\nWe should remove this, since `SAGE_LOCAL/bin/sage-spkg` already does this check:\n\n```sh\n    cd $BASEDIR\n    if [ \"$SAGE_CHECK\" != \"\" -a -f spkg-check ]; then\n        echo \"Running the test suite.\"\n        chmod +x spkg-check\n        ./spkg-check\n        if [ $? -ne 0 ]; then\n```\n\nDepends on #8664\n\nReviewer: **Leif Leonhardy**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9522_\n\n",
    "closed_at": "2011-09-29T23:46:49Z",
    "created_at": "2010-07-17T01:02:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "MPIR: Don't check SAGE_CHECK in spkg-install",
    "type": "issue",
    "updated_at": "2011-09-29T23:46:49Z",
    "url": "https://github.com/sagemath/sage/issues/9522",
    "user": "https://github.com/qed777"
}
```
From the end of MPIR's `spkg-install`:

```sh
if [ "$SAGE_CHECK" = "yes" ]; then
    cd ..; ./spkg-check
fi
```
We should remove this, since `SAGE_LOCAL/bin/sage-spkg` already does this check:

```sh
    cd $BASEDIR
    if [ "$SAGE_CHECK" != "" -a -f spkg-check ]; then
        echo "Running the test suite."
        chmod +x spkg-check
        ./spkg-check
        if [ $? -ne 0 ]; then
```

Depends on #8664

Reviewer: **Leif Leonhardy**

_Issue created by migration from https://trac.sagemath.org/ticket/9522_





---

archive/issue_events_111771.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-17T01:02:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9522#event-111771"
}
```



---

archive/issue_events_111772.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-17T01:02:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9522#event-111772"
}
```



---

archive/issue_events_111773.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-17T01:02:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9522#event-111773"
}
```



---

archive/issue_events_111774.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-17T01:02:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9522#event-111774"
}
```



---

archive/issue_comments_084196.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThere's already [a ticket for updating MPIR to version 2.1.1](https://github.com/sagemath/sage/issues/8664) (currently needing review), which apparently is aware of *this* ticket.\n\nI though haven't checked if Mike deleted the superfluous test suite invocation in `spkg-install`.",
    "created_at": "2010-08-26T21:02:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9522#issuecomment-84196",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:1" align="right">Comment 1</div>

There's already [a ticket for updating MPIR to version 2.1.1](https://github.com/sagemath/sage/issues/8664) (currently needing review), which apparently is aware of *this* ticket.

I though haven't checked if Mike deleted the superfluous test suite invocation in `spkg-install`.



---

archive/issue_comments_084197.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nReplying to [@nexttime](#comment%3A1):\n> I though haven't checked if Mike deleted the superfluous test suite invocation in `spkg-install`.\n\nDone, he *did* remove it. So **this ticket can be closed as duplicate as soon as #8664 gets merged**.",
    "created_at": "2010-08-26T21:24:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9522#issuecomment-84197",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:2" align="right">Comment 2</div>

Replying to [@nexttime](#comment%3A1):
> I though haven't checked if Mike deleted the superfluous test suite invocation in `spkg-install`.

Done, he *did* remove it. So **this ticket can be closed as duplicate as soon as #8664 gets merged**.



---

archive/issue_comments_084198.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI think there's a good argument for running the mpir test suite every time (i.e. from spkg-install), as it has historically caught several compiler bugs. \n\nIt's obviously pointless running it twice. \n\nDave",
    "created_at": "2010-09-21T19:45:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9522#issuecomment-84198",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:3" align="right">Comment 3</div>

I think there's a good argument for running the mpir test suite every time (i.e. from spkg-install), as it has historically caught several compiler bugs. 

It's obviously pointless running it twice. 

Dave



---

archive/issue_comments_084199.json:
```json
{
    "body": "Reviewer: **Leif Leonhardy**",
    "created_at": "2011-09-29T23:46:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9522#issuecomment-84199",
    "user": "https://github.com/nexttime"
}
```

Reviewer: **Leif Leonhardy**



---

archive/issue_events_111775.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-29T23:46:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9522#event-111775"
}
```



---

archive/issue_events_111776.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-29T23:46:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9522#event-111776"
}
```



---

archive/issue_events_111777.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-29T23:46:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9522#event-111777"
}
```



---

archive/issue_comments_084200.json:
```json
{
    "body": "Dependencies: **#8664**",
    "created_at": "2011-09-29T23:46:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9522#issuecomment-84200",
    "user": "https://github.com/nexttime"
}
```

Dependencies: **#8664**



---

archive/issue_events_111778.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-29T23:46:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9522",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9522#event-111778"
}
```
