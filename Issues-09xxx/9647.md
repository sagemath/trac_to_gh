# Issue 9647: instructive example for mip.pyx

archive/issues_009647.json:
```json
{
    "assignees": [],
    "body": "Add an example from sage-support to the head of mip.pyx [see here](http://groups.google.com/group/sage-support/browse_thread/thread/e5ebaf0f6f657ab2).\n\nAssignee: **@jasongrout, @sagetrac-jkantor**\n\nCC:  @kcrisman @nathanncohen\n\nAuthor: **Harald Schilly**\n\nReviewer: **Nathann Cohen**\n\nMerged: **sage-4.5.3.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9647_\n\n",
    "closed_at": "2010-08-09T09:40:18Z",
    "created_at": "2010-07-30T17:06:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "instructive example for mip.pyx",
    "type": "issue",
    "updated_at": "2010-08-09T09:40:18Z",
    "url": "https://github.com/sagemath/sage/issues/9647",
    "user": "https://github.com/haraldschilly"
}
```
Add an example from sage-support to the head of mip.pyx [see here](http://groups.google.com/group/sage-support/browse_thread/thread/e5ebaf0f6f657ab2).

Assignee: **@jasongrout, @sagetrac-jkantor**

CC:  @kcrisman @nathanncohen

Author: **Harald Schilly**

Reviewer: **Nathann Cohen**

Merged: **sage-4.5.3.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/9647_





---

archive/issue_events_113516.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-30T17:06:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "milestone_number": null,
    "milestone_title": "sage-4.5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113516"
}
```



---

archive/issue_events_113517.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-30T17:06:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113517"
}
```



---

archive/issue_events_113518.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-30T17:06:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113518"
}
```



---

archive/issue_events_113519.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-30T17:06:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113519"
}
```



---

archive/issue_events_113520.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-30T17:19:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113520"
}
```



---

archive/issue_comments_086330.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI can't comment on the correctness, obviously, but if the formatting works out right, this is exactly the kind of thing I was envisioning.  Great documentation and explanation - I hope someone reviews it soon!",
    "created_at": "2010-07-30T19:13:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86330",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:2" align="right">Comment 2</div>

I can't comment on the correctness, obviously, but if the formatting works out right, this is exactly the kind of thing I was envisioning.  Great documentation and explanation - I hope someone reviews it soon!



---

archive/issue_comments_086331.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI've added a general introduction chapter, but suddenly the <code>:meth:~\\`Class.method\\`</code> references to methods no longer work. I don't know why.",
    "created_at": "2010-07-30T21:48:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86331",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:3" align="right">Comment 3</div>

I've added a general introduction chapter, but suddenly the <code>:meth:~\`Class.method\`</code> references to methods no longer work. I don't know why.



---

archive/issue_events_113521.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-30T21:48:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113521"
}
```



---

archive/issue_events_113522.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-30T21:48:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113522"
}
```



---

archive/issue_events_113523.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-31T12:11:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113523"
}
```



---

archive/issue_events_113524.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-31T12:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113524"
}
```



---

archive/issue_comments_086332.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nHello !!! Everything seems nice, but could you please add in the example one or two inequalities ? All the constraints in this case are equalities, which is a bit unusual with LP... Anyway people would understand that it is indeed possible to write an equality with two inequalities, while the opposite is not true... :-)\n\nNathann",
    "created_at": "2010-07-31T14:28:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86332",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:5" align="right">Comment 5</div>

Hello !!! Everything seems nice, but could you please add in the example one or two inequalities ? All the constraints in this case are equalities, which is a bit unusual with LP... Anyway people would understand that it is indeed possible to write an equality with two inequalities, while the opposite is not true... :-)

Nathann



---

archive/issue_comments_086333.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nw_3 >= 1 is an inequality, but it's probably not explicit enough... wait a sec.",
    "created_at": "2010-07-31T14:29:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86333",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:6" align="right">Comment 6</div>

w_3 >= 1 is an inequality, but it's probably not explicit enough... wait a sec.



---

archive/issue_comments_086334.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nHow about now? I've intentionally avoided to set the lower bound of w_3 to 1 and instead used an inequality just because of that. I think that's ok now.",
    "created_at": "2010-07-31T14:34:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86334",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:7" align="right">Comment 7</div>

How about now? I've intentionally avoided to set the lower bound of w_3 to 1 and instead used an inequality just because of that. I think that's ok now.



---

archive/issue_comments_086335.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nHmmm.. I don't want an inequality \"on principle\"... I thought it would be nice for the users to see among your system of equalities some inequality... When they look at this system of equations they can think \"ok, my own system fits\". If they only see equalities and don't read the full text, they may not notice it is possible... Well, it's up to you ! If you don't this it sound, just say so, your ticket already dserves a positive review :-)\n\nNathann",
    "created_at": "2010-07-31T14:42:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86335",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:8" align="right">Comment 8</div>

Hmmm.. I don't want an inequality "on principle"... I thought it would be nice for the users to see among your system of equalities some inequality... When they look at this system of equations they can think "ok, my own system fits". If they only see equalities and don't read the full text, they may not notice it is possible... Well, it's up to you ! If you don't this it sound, just say so, your ticket already dserves a positive review :-)

Nathann



---

archive/issue_comments_086336.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nAttachment: **[9647-example-MILP.patch.gz](https://github.com/sagemath/sage/files/ticket9647/9647-example-MILP.patch.gz)**\n\nok, you are probably right.",
    "created_at": "2010-07-31T14:54:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86336",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:9" align="right">Comment 9</div>

Attachment: **[9647-example-MILP.patch.gz](https://github.com/sagemath/sage/files/ticket9647/9647-example-MILP.patch.gz)**

ok, you are probably right.



---

archive/issue_comments_086337.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nSorryyyyyy !! I know it feels a bit artificial.... With a bit of luck we should have a proper tutorial for LP in Sage soon anyway... Thank you for your paaaaaatch !!! :-)\n\nNathann",
    "created_at": "2010-07-31T14:57:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86337",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:10" align="right">Comment 10</div>

Sorryyyyyy !! I know it feels a bit artificial.... With a bit of luck we should have a proper tutorial for LP in Sage soon anyway... Thank you for your paaaaaatch !!! :-)

Nathann



---

archive/issue_events_113525.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-07-31T14:57:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113525"
}
```



---

archive/issue_events_113526.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-07-31T14:57:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113526"
}
```



---

archive/issue_comments_086338.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2010-08-09T09:40:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86338",
    "user": "https://github.com/qed777"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_086339.json:
```json
{
    "body": "Author: **Harald Schilly**",
    "created_at": "2010-08-09T09:40:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86339",
    "user": "https://github.com/qed777"
}
```

Author: **Harald Schilly**



---

archive/issue_comments_086340.json:
```json
{
    "body": "Merged: **sage-4.5.3.alpha0**",
    "created_at": "2010-08-09T09:40:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9647#issuecomment-86340",
    "user": "https://github.com/qed777"
}
```

Merged: **sage-4.5.3.alpha0**



---

archive/issue_events_113527.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-09T09:40:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113527"
}
```



---

archive/issue_events_113528.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-09T09:40:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9647",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9647#event-113528"
}
```
