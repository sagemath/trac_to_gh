# Issue 9561: Docbuild warnings caused by #9219

archive/issues_009561.json:
```json
{
    "assignees": [],
    "body": "The forthcoming 4.5.2.alpha0 includes #9219, which appears to cause the following docbuild warnings:\n\n```\ndocstring of sage.stats.hmm.chmm.GaussianHiddenMarkovModel:14: (WARNING/2) Literal block expected; none found.\ndocstring of sage.stats.hmm.chmm.GaussianHiddenMarkovModel.viterbi:20: (WARNING/2) Literal block expected; none found.\ndocstring of sage.stats.hmm.distributions.Distribution.prob:13: (WARNING/2) Literal block expected; none found.\ndocstring of sage.stats.hmm.distributions.Distribution.sample:14: (WARNING/2) Literal block expected; none found.\n```\n\nAssignee: **@sagetrac-mvngu**\n\nCC:  @kcrisman @sagetrac-mvngu\n\nAuthor: **Karl-Dieter Crisman**\n\nReviewer: **John Palmieri**\n\nMerged: **sage-4.5.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9561_\n\n",
    "closed_at": "2010-07-23T02:29:55Z",
    "created_at": "2010-07-21T10:01:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Docbuild warnings caused by #9219",
    "type": "issue",
    "updated_at": "2010-07-23T02:29:55Z",
    "url": "https://github.com/sagemath/sage/issues/9561",
    "user": "https://github.com/qed777"
}
```
The forthcoming 4.5.2.alpha0 includes #9219, which appears to cause the following docbuild warnings:

```
docstring of sage.stats.hmm.chmm.GaussianHiddenMarkovModel:14: (WARNING/2) Literal block expected; none found.
docstring of sage.stats.hmm.chmm.GaussianHiddenMarkovModel.viterbi:20: (WARNING/2) Literal block expected; none found.
docstring of sage.stats.hmm.distributions.Distribution.prob:13: (WARNING/2) Literal block expected; none found.
docstring of sage.stats.hmm.distributions.Distribution.sample:14: (WARNING/2) Literal block expected; none found.
```

Assignee: **@sagetrac-mvngu**

CC:  @kcrisman @sagetrac-mvngu

Author: **Karl-Dieter Crisman**

Reviewer: **John Palmieri**

Merged: **sage-4.5.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/9561_





---

archive/issue_events_112300.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-21T10:01:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "milestone_number": null,
    "milestone_title": "sage-4.5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9561#event-112300"
}
```



---

archive/issue_events_112301.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-21T10:01:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9561#event-112301"
}
```



---

archive/issue_events_112302.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-21T10:01:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9561#event-112302"
}
```



---

archive/issue_events_112303.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-07-21T10:01:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9561#event-112303"
}
```



---

archive/issue_comments_084853.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis was caused by someone getting too excited about \n\n```\nEXAMPLES::\n```\nwhich does indeed often introduce actual examples, but now is more likely to have more text before the code itself.  Should be very easy review.",
    "created_at": "2010-07-22T14:15:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9561#issuecomment-84853",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">Comment 1</div>

This was caused by someone getting too excited about 

```
EXAMPLES::
```
which does indeed often introduce actual examples, but now is more likely to have more text before the code itself.  Should be very easy review.



---

archive/issue_comments_084854.json:
```json
{
    "body": "Attachment: **[trac_9561-stats-rst-warnings.patch.gz](https://github.com/sagemath/sage/files/ticket9561/trac_9561-stats-rst-warnings.patch.gz)**\n\nBased on 4.5.1 plus relevant patches",
    "created_at": "2010-07-22T14:17:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9561#issuecomment-84854",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[trac_9561-stats-rst-warnings.patch.gz](https://github.com/sagemath/sage/files/ticket9561/trac_9561-stats-rst-warnings.patch.gz)**

Based on 4.5.1 plus relevant patches



---

archive/issue_comments_084855.json:
```json
{
    "body": "Author: **Karl-Dieter Crisman**",
    "created_at": "2010-07-22T14:18:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9561#issuecomment-84855",
    "user": "https://github.com/kcrisman"
}
```

Author: **Karl-Dieter Crisman**



---

archive/issue_events_112304.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2010-07-22T14:18:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9561#event-112304"
}
```



---

archive/issue_comments_084856.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nEither do 4.5.1 and patches from #9218 and #9219, or base on 4.5.2.alpha0.  This should remove the warnings.",
    "created_at": "2010-07-22T14:18:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9561#issuecomment-84856",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:2" align="right">Comment 2</div>

Either do 4.5.1 and patches from #9218 and #9219, or base on 4.5.2.alpha0.  This should remove the warnings.



---

archive/issue_comments_084857.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2010-07-23T00:32:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9561#issuecomment-84857",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_events_112305.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-07-23T00:32:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9561#event-112305"
}
```



---

archive/issue_events_112306.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-07-23T00:32:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9561#event-112306"
}
```



---

archive/issue_events_112307.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-07-23T02:29:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9561#event-112307"
}
```



---

archive/issue_events_112308.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-07-23T02:29:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9561#event-112308"
}
```



---

archive/issue_comments_084858.json:
```json
{
    "body": "Merged: **sage-4.5.2.alpha1**",
    "created_at": "2010-07-23T02:29:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9561#issuecomment-84858",
    "user": "https://github.com/dandrake"
}
```

Merged: **sage-4.5.2.alpha1**
