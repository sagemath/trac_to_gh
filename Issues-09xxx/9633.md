...
```json
{
    "body": "<a id='comment:4'></a>Two points: (1) I think \"P = parent(x)\" is simpler, if I'm reading sage.structure.parent correctly.  (2) Doctest? `:^)`",
...
<a id='comment:4'></a>Two points: (1) I think "P = parent(x)" is simpler, if I'm reading sage.structure.parent correctly.  (2) Doctest? `:^)`
...
```json
{
    "body": "<a id='comment:5'></a>Good points. :).",
...
```json
{
    "body": "<a id='comment:6'></a>Certainly looks good!   Interesting that we didn't catch that when we put it in, even though it explicitly has 'float' in the previous version :( \n\nCurrently running tests in case there was something subtle about `x.parent()` that was different from `parent(x)`, though I can't see what that would be ...",
...
<a id='comment:6'></a>Certainly looks good!   Interesting that we didn't catch that when we put it in, even though it explicitly has 'float' in the previous version :( 
...
```json
{
    "body": "<a id='comment:7'></a>Pass :)  Good catch.",
    "created_at": "2011-04-25T16:41:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9633",
    "type": "issue_comment",
...
<a id='comment:7'></a>Pass :)  Good catch.
