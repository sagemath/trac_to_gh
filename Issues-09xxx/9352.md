# Issue 9352: givaro spkg: trivial typo in spkg-check

archive/issues_009352.json:
```json
{
    "assignees": [],
    "body": "The file spkg-check in the givaro spkg prints the following if testing fails: \"Error while running the R testsuite ... exiting\".  The new spkg changes \"R\" to \"Givaro\".\n\n\nCC:  drkirkby\n\nAuthor: **John Palmieri**\n\nReviewer: **David Kirkby**\n\nMerged: **sage-4.5.3.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9352_\n\n",
    "closed_at": "2010-08-09T09:35:55Z",
    "created_at": "2010-06-27T17:18:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20spkg-check",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "givaro spkg: trivial typo in spkg-check",
    "type": "issue",
    "updated_at": "2011-12-12T09:16:08Z",
    "url": "https://github.com/sagemath/sage/issues/9352",
    "user": "https://github.com/jhpalmieri"
}
```
The file spkg-check in the givaro spkg prints the following if testing fails: "Error while running the R testsuite ... exiting".  The new spkg changes "R" to "Givaro".


CC:  drkirkby

Author: **John Palmieri**

Reviewer: **David Kirkby**

Merged: **sage-4.5.3.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/9352_





---

archive/issue_events_105683.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-06-27T17:18:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "milestone_number": null,
    "milestone_title": "sage-4.5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105683"
}
```



---

archive/issue_events_105684.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-06-27T17:18:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20spkg-check",
    "label_color": "000080",
    "label_name": "component: spkg-check",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105684"
}
```



---

archive/issue_events_105685.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-06-27T17:18:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105685"
}
```



---

archive/issue_events_105686.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-06-27T17:18:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105686"
}
```



---

archive/issue_comments_081514.json:
```json
{
    "body": "Attachment: **[givaro.patch.gz](https://github.com/sagemath/sage/files/ticket9352/givaro.patch.gz)**",
    "created_at": "2010-06-27T17:20:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9352#issuecomment-81514",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[givaro.patch.gz](https://github.com/sagemath/sage/files/ticket9352/givaro.patch.gz)**



---

archive/issue_comments_081515.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI've attaching the output from \"hg diff\".  The new spkg is here: [http://sage.math.washington.edu/home/palmieri/SPKG/givaro-3.2.13rc2.p2.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/givaro-3.2.13rc2.p2.spkg).",
    "created_at": "2010-06-27T17:22:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9352#issuecomment-81515",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'>Comment 1:</a>
I've attaching the output from "hg diff".  The new spkg is here: [http://sage.math.washington.edu/home/palmieri/SPKG/givaro-3.2.13rc2.p2.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/givaro-3.2.13rc2.p2.spkg).



---

archive/issue_events_105687.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-06-27T17:22:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105687"
}
```



---

archive/issue_events_105688.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-06-27T17:43:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105688"
}
```



---

archive/issue_events_105689.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-06-27T17:43:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105689"
}
```



---

archive/issue_comments_081516.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nPositive review of course. \n\nDave",
    "created_at": "2010-06-27T17:43:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9352#issuecomment-81516",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<a id='comment:3'>Comment 3:</a>
Positive review of course. 

Dave



---

archive/issue_comments_081517.json:
```json
{
    "body": "Merged: **sage-4.5.3.alpha0**",
    "created_at": "2010-08-09T09:35:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9352#issuecomment-81517",
    "user": "https://github.com/qed777"
}
```

Merged: **sage-4.5.3.alpha0**



---

archive/issue_events_105690.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-09T09:35:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105690"
}
```



---

archive/issue_events_105691.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-09T09:35:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105691"
}
```



---

archive/issue_events_105692.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-09T09:35:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "milestone_number": null,
    "milestone_title": "sage-4.5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9352#event-105692"
}
```



---

archive/issue_comments_081518.json:
```json
{
    "body": "Reviewer: **David Kirkby**",
    "created_at": "2010-08-09T09:35:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9352#issuecomment-81518",
    "user": "https://github.com/qed777"
}
```

Reviewer: **David Kirkby**



---

archive/issue_comments_081519.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nCould it be that the version number really should have been\n\n```\ngivaro-3.2.13rc1.p2.spkg\n```\ninstead of\n\n```\ngivaro-3.2.13rc2.p2.spkg\n}}}?",
    "created_at": "2011-12-12T09:16:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9352#issuecomment-81519",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'>Comment 5:</a>
Could it be that the version number really should have been

```
givaro-3.2.13rc1.p2.spkg
```
instead of

```
givaro-3.2.13rc2.p2.spkg
}}}?
