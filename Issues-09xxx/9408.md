# Issue 9408: relativize in number fields is broken

archive/issues_009408.json:
```json
{
    "body": "Does not work due to denominators:\n\n```\nsage: K.<a> = NumberField(x^4-4*x^3+12*x^2-16*x+8)\nsage: L.<u,v> = K.relativize(3*a**3 - 9*a**2 + 24*a -16)  # OK\nsage: L2.<u2,v2> = K.relativize((3/4)*a**3 - (9/4)*a**2 + 6*a -4)\n```\n\nSimpler example:\n\n```\nsage: L.<a,b> = QQ[i].relativize(1) # OK\nsage: L.<a,b> = QQ[i].relativize(1/2)\n...\nTypeError: Unable to coerce number field defined by non-integral polynomial to PARI.\n```\n\nThis is in some sense a duplicate of #252.\n\n**Assignee:** @loefflerd\n\n**Keywords:** relativize\n\n**Dependencies:** #252\n\n**Reviewer:** Luis Felipe Tabera Alonso\n\nIssue created by migration from https://trac.sagemath.org/ticket/9408\n\n",
    "closed_at": "2011-11-15T09:21:27Z",
    "created_at": "2010-07-02T16:04:23Z",
    "labels": [
        "component: number fields",
        "bug",
        "duplicate"
    ],
    "title": "relativize in number fields is broken",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9408",
    "user": "https://github.com/lftabera"
}
```
Does not work due to denominators:

```
sage: K.<a> = NumberField(x^4-4*x^3+12*x^2-16*x+8)
sage: L.<u,v> = K.relativize(3*a**3 - 9*a**2 + 24*a -16)  # OK
sage: L2.<u2,v2> = K.relativize((3/4)*a**3 - (9/4)*a**2 + 6*a -4)
```

Simpler example:

```
sage: L.<a,b> = QQ[i].relativize(1) # OK
sage: L.<a,b> = QQ[i].relativize(1/2)
...
TypeError: Unable to coerce number field defined by non-integral polynomial to PARI.
```

This is in some sense a duplicate of #252.

**Assignee:** @loefflerd

**Keywords:** relativize

**Dependencies:** #252

**Reviewer:** Luis Felipe Tabera Alonso

Issue created by migration from https://trac.sagemath.org/ticket/9408





---

archive/issue_comments_083914.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis ticket should be closed as duplicate of #252",
    "created_at": "2010-07-06T10:58:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9408#issuecomment-83914",
    "user": "https://github.com/lftabera"
}
```

<a id='comment:1'></a>
This ticket should be closed as duplicate of #252



---

archive/issue_events_070691.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2010-07-06T10:58:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70691"
}
```



---

archive/issue_events_070692.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2011-06-07T08:58:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70692"
}
```



---

archive/issue_events_070693.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2011-06-07T08:58:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70693"
}
```



---

archive/issue_events_070694.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-15T12:50:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70694"
}
```



---

archive/issue_events_070695.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-15T12:50:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70695"
}
```



---

archive/issue_comments_083915.json:
```json
{
    "body": "**Dependencies:** #252",
    "created_at": "2011-10-15T12:50:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9408#issuecomment-83915",
    "user": "https://github.com/jdemeyer"
}
```

**Dependencies:** #252



---

archive/issue_comments_083916.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,16 +1,18 @@\n-Does not work due to (maybe) denominators\n+Does not work due to denominators:\n \n ```\n sage: K.<a> = NumberField(x^4-4*x^3+12*x^2-16*x+8)\n-sage: L.<u,v> = K.relativize(3*a**3 - 9*a**2 + 24*a -16)\n-sage:#This seems OK\n+sage: L.<u,v> = K.relativize(3*a**3 - 9*a**2 + 24*a -16)  # OK\n sage: L2.<u2,v2> = K.relativize((3/4)*a**3 - (9/4)*a**2 + 6*a -4)\n ```\n-PariError:  (8)\n \n-Simpler example\n+Simpler example:\n \n ```\n-sage: L.<a,b> = QQ[i].relativize(1) #Ok\n-sage: L.<a,b> = QQ[i].relativize(1/2) #PariError\n+sage: L.<a,b> = QQ[i].relativize(1) # OK\n+sage: L.<a,b> = QQ[i].relativize(1/2)\n+...\n+TypeError: Unable to coerce number field defined by non-integral polynomial to PARI.\n ```\n+\n+This is in some sense a duplicate of #252.\n``````\n",
    "created_at": "2011-10-15T12:50:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9408#issuecomment-83916",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,16 +1,18 @@
-Does not work due to (maybe) denominators
+Does not work due to denominators:
 
 ```
 sage: K.<a> = NumberField(x^4-4*x^3+12*x^2-16*x+8)
-sage: L.<u,v> = K.relativize(3*a**3 - 9*a**2 + 24*a -16)
-sage:#This seems OK
+sage: L.<u,v> = K.relativize(3*a**3 - 9*a**2 + 24*a -16)  # OK
 sage: L2.<u2,v2> = K.relativize((3/4)*a**3 - (9/4)*a**2 + 6*a -4)
 ```
-PariError:  (8)
 
-Simpler example
+Simpler example:
 
 ```
-sage: L.<a,b> = QQ[i].relativize(1) #Ok
-sage: L.<a,b> = QQ[i].relativize(1/2) #PariError
+sage: L.<a,b> = QQ[i].relativize(1) # OK
+sage: L.<a,b> = QQ[i].relativize(1/2)
+...
+TypeError: Unable to coerce number field defined by non-integral polynomial to PARI.
 ```
+
+This is in some sense a duplicate of #252.
``````




---

archive/issue_comments_083917.json:
```json
{
    "body": "**Reviewer:** Luis Felipe Tabera Alonso",
    "created_at": "2011-10-15T12:50:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9408#issuecomment-83917",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Luis Felipe Tabera Alonso



---

archive/issue_events_070696.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-15T12:50:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70696"
}
```



---

archive/issue_events_070697.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-15T12:50:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70697"
}
```



---

archive/issue_events_070698.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-15T09:21:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70698"
}
```



---

archive/issue_events_070699.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-15T09:21:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70699"
}
```



---

archive/issue_events_070700.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-15T09:21:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/9408",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9408#event-70700"
}
```
