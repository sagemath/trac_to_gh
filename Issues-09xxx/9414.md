# Issue 9414: make the rational number field consistent with other number fields

archive/issues_009414.json:
```json
{
    "body": "Currently QQ behaves different than a generic number field. This forces number theory functions to treat QQ separately, which is inconvenient.\n\n```\nK = QQ\nI = K.ideal(7)\n```\n\nThis creates ideal that does not have the functions I.denominator, I.numerator, I.prime_ideals() ... which a fractional ideal in a number field should have\n\n```\nK.<a> = NumberField(x^2+2)\nI = K.ideal(7)\n```\n\nSimilarly, QQ.places() is not implemented; it should return the one infinite place for Q. Although there seems to be QQ.embeddings().\n\n```\nQQ.places()\n```\n\nAssignee: @loefflerd\n\nKeywords: number field, rationals\n\nResolution: duplicate\n\nReviewer: Maarten Derickx\n\nIssue created by migration from https://trac.sagemath.org/ticket/9414\n\n",
    "closed_at": "2011-02-16T09:36:18Z",
    "created_at": "2010-07-03T02:38:31Z",
    "labels": [
        "component: number fields",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "make the rational number field consistent with other number fields",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9414",
    "user": "https://trac.sagemath.org/admin/accounts/users/rkirov"
}
```
Currently QQ behaves different than a generic number field. This forces number theory functions to treat QQ separately, which is inconvenient.

```
K = QQ
I = K.ideal(7)
```

This creates ideal that does not have the functions I.denominator, I.numerator, I.prime_ideals() ... which a fractional ideal in a number field should have

```
K.<a> = NumberField(x^2+2)
I = K.ideal(7)
```

Similarly, QQ.places() is not implemented; it should return the one infinite place for Q. Although there seems to be QQ.embeddings().

```
QQ.places()
```

Assignee: @loefflerd

Keywords: number field, rationals

Resolution: duplicate

Reviewer: Maarten Derickx

Issue created by migration from https://trac.sagemath.org/ticket/9414





---

archive/issue_comments_107077.json:
```json
{
    "body": "<a id='comment:2'></a>This is a duplicate of #7596. I'm putting it as positive review so that someone with the right abilities will see it an close this as duplicate ticket.",
    "created_at": "2011-02-10T14:07:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9414",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9414#issuecomment-107077",
    "user": "https://github.com/koffie"
}
```

<a id='comment:2'></a>This is a duplicate of #7596. I'm putting it as positive review so that someone with the right abilities will see it an close this as duplicate ticket.



---

archive/issue_comments_107078.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-02-10T14:07:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9414",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9414#issuecomment-107078",
    "user": "https://github.com/koffie"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_107079.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-02-10T14:08:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9414",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9414#issuecomment-107079",
    "user": "https://github.com/koffie"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_023248.json:
```json
{
    "actor": "https://github.com/koffie",
    "created_at": "2011-02-10T14:09:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/9414",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9414#event-23248"
}
```



---

archive/issue_comments_107080.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2011-02-16T09:36:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9414",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9414#issuecomment-107080",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: duplicate



---

archive/issue_events_023249.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-16T09:36:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9414",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9414#event-23249"
}
```



---

archive/issue_comments_107081.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Maarten Derickx\"",
    "created_at": "2011-02-16T09:36:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9414",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9414#issuecomment-107081",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Maarten Derickx"
