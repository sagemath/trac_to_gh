# Issue 623: New Cython

archive/issues_000623.json:
```json
{
    "body": "Assignee: @williamstein\n\nUpdate the Cython package to the latest. \n\nIssue created by migration from https://trac.sagemath.org/ticket/623\n\n",
    "created_at": "2007-09-07T19:43:40Z",
    "labels": [
        "packages: standard",
        "major",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.8",
    "title": "New Cython",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/623",
    "user": "@robertwb"
}
```
Assignee: @williamstein

Update the Cython package to the latest. 

Issue created by migration from https://trac.sagemath.org/ticket/623





---

archive/issue_comments_003202.json:
```json
{
    "body": "Changing assignee from @williamstein to @robertwb.",
    "created_at": "2007-09-07T19:43:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3202",
    "user": "@robertwb"
}
```

Changing assignee from @williamstein to @robertwb.



---

archive/issue_comments_003203.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-09-07T19:43:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3203",
    "user": "@robertwb"
}
```

Changing status from new to assigned.



---

archive/issue_comments_003204.json:
```json
{
    "body": "See http://sage.math.washington.edu/home/robertwb/coerce/",
    "created_at": "2007-09-12T04:29:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3204",
    "user": "@robertwb"
}
```

See http://sage.math.washington.edu/home/robertwb/coerce/



---

archive/issue_comments_003205.json:
```json
{
    "body": "This ought to be closed. For the next Cython release the subject line should be more specific.\n\nCheers,\n\nMichael",
    "created_at": "2007-10-19T18:38:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3205",
    "user": "mabshoff"
}
```

This ought to be closed. For the next Cython release the subject line should be more specific.

Cheers,

Michael



---

archive/issue_comments_003206.json:
```json
{
    "body": "I'm closing this, since the directory above points to an old version of cython.",
    "created_at": "2007-10-20T18:14:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3206",
    "user": "@williamstein"
}
```

I'm closing this, since the directory above points to an old version of cython.



---

archive/issue_comments_003207.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-10-20T18:14:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/623#issuecomment-3207",
    "user": "@williamstein"
}
```

Resolution: fixed
