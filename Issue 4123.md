# Issue 4123: [with patch, positive review] source $SAGE_ROOT/toolchain/toolchain-env if it exists

archive/issues_004123.json:
```json
{
    "body": "Assignee: mabshoff\n\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/4123\n\n",
    "closed_at": "2008-09-15T11:45:56Z",
    "created_at": "2008-09-15T01:09:50Z",
    "labels": [
        "component: distribution",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] source $SAGE_ROOT/toolchain/toolchain-env if it exists",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4123",
    "user": "https://github.com/rlmill"
}
```
Assignee: mabshoff



Issue created by migration from https://trac.sagemath.org/ticket/4123





---

archive/issue_comments_029836.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-09-15T10:51:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4123#issuecomment-29836",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_029837.json:
```json
{
    "body": "If $SAGE_ROOT/toolchain/toolchain-env exists we should source it from sage-env so that using a custom toolchain (for example for Solaris, Linux MIPS64, Itanioum) is trivial.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-15T10:51:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4123#issuecomment-29837",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

If $SAGE_ROOT/toolchain/toolchain-env exists we should source it from sage-env so that using a custom toolchain (for example for Solaris, Linux MIPS64, Itanioum) is trivial.

Cheers,

Michael



---

archive/issue_comments_029838.json:
```json
{
    "body": "Attachment [trac_4123.patch](tarball://root/attachments/some-uuid/ticket4123/trac_4123.patch) by mabshoff created at 2008-09-15 11:43:42",
    "created_at": "2008-09-15T11:43:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4123#issuecomment-29838",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [trac_4123.patch](tarball://root/attachments/some-uuid/ticket4123/trac_4123.patch) by mabshoff created at 2008-09-15 11:43:42



---

archive/issue_comments_029839.json:
```json
{
    "body": "Looks good.",
    "created_at": "2008-09-15T11:45:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4123#issuecomment-29839",
    "user": "https://github.com/rlmill"
}
```

Looks good.



---

archive/issue_comments_029840.json:
```json
{
    "body": "Merged in Sage 3.1.2.rc4",
    "created_at": "2008-09-15T11:45:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4123#issuecomment-29840",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Merged in Sage 3.1.2.rc4



---

archive/issue_events_009398.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-15T11:45:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4123",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4123#event-9398"
}
```



---

archive/issue_comments_029841.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-15T11:45:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4123#issuecomment-29841",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
