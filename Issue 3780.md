# Issue 3780: [with patch, depends on #3324

archive/issues_003780.json:
```json
{
    "body": "Assignee: @malb\n\nKeywords: m4ri\n\nadd fast col_swap method for dense matrices over GF(2).\n\nIssue created by migration from https://trac.sagemath.org/ticket/3780\n\n",
    "created_at": "2008-08-06T16:28:22Z",
    "labels": [
        "linear algebra",
        "major",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, depends on #3324",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3780",
    "user": "@malb"
}
```
Assignee: @malb

Keywords: m4ri

add fast col_swap method for dense matrices over GF(2).

Issue created by migration from https://trac.sagemath.org/ticket/3780





---

archive/issue_comments_026877.json:
```json
{
    "body": "Attachment [m4ri_col_swap.patch](tarball://root/attachments/some-uuid/ticket3780/m4ri_col_swap.patch) by @malb created at 2008-08-06 16:29:12",
    "created_at": "2008-08-06T16:29:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-26877",
    "user": "@malb"
}
```

Attachment [m4ri_col_swap.patch](tarball://root/attachments/some-uuid/ticket3780/m4ri_col_swap.patch) by @malb created at 2008-08-06 16:29:12



---

archive/issue_comments_026878.json:
```json
{
    "body": "I haven't tested this, but it looks fine. If it works, apply.",
    "created_at": "2008-08-31T00:11:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-26878",
    "user": "@rlmill"
}
```

I haven't tested this, but it looks fine. If it works, apply.



---

archive/issue_comments_026879.json:
```json
{
    "body": "...other than the missing output from the last doctest! :-[",
    "created_at": "2008-08-31T00:21:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-26879",
    "user": "@rlmill"
}
```

...other than the missing output from the last doctest! :-[



---

archive/issue_comments_026880.json:
```json
{
    "body": "Note that the doctests only pass with #3376 applied, where the missing output that rlm mentions above is actually added :)\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T00:34:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-26880",
    "user": "mabshoff"
}
```

Note that the doctests only pass with #3376 applied, where the missing output that rlm mentions above is actually added :)

Cheers,

Michael



---

archive/issue_comments_026881.json:
```json
{
    "body": "Merged in Sage 3.1.2.alpha3",
    "created_at": "2008-08-31T00:53:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-26881",
    "user": "mabshoff"
}
```

Merged in Sage 3.1.2.alpha3



---

archive/issue_comments_026882.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-31T00:53:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-26882",
    "user": "mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_026883.json:
```json
{
    "body": "Note that the patch attached to this ticket is a diff. I did commit it in Martin's name.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T03:55:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-26883",
    "user": "mabshoff"
}
```

Note that the patch attached to this ticket is a diff. I did commit it in Martin's name.

Cheers,

Michael
