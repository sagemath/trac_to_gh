# Issue 28155: dynamic_class: do not copy over Python 3's new __weakref__ class attribute

archive/issues_028155.json:
```json
{
    "body": "CC:  chapoton tscrim embray\n\nPython3 introduce a new attribute `__weakref__` in the datastructure for classes in addition to `__dict__`. As such, this attribute should not be copied over by `dynamic_class`. Otherwise we get errors, typically upon introspection:\n```\n    sage: from inspect import getmembers\n    sage: c = EllipticCurve([0,0,1,-1,0])\n    sage: getmembers(c)\n    ---------------------------------------------------------------------------\n    TypeError                                 Traceback (most recent call last)\n    <ipython-input-5-bef5f5512029> in <module>()\n    ----> 1 getmembers(c)\n\n    /opt/sage-git/local/lib/python3.7/inspect.py in getmembers(object, predicate)\n    339         # looking in the __dict__.\n    340         try:\n--> 341             value = getattr(object, key)\n    342             # handle the duplicate key\n    343             if key in processed:\n\n    TypeError: descriptor '__weakref__' for 'Sets.ParentMethods' objects doesn't apply to 'EllipticCurve_rational_field_with_category' object\n```\nThe same error can be triggered by calling directly `c.__weakref__`.\n\nThis can be reduced down to:\n```\n        sage: class A:\n        ....:     pass\n        sage: A.__weakref__\n        <attribute '__weakref__' of 'A' objects>\n        sage: Foo1 = sage.structure.dynamic_class.dynamic_class(\"Foo\", (), A)\n        sage: \"__weakref__\" in Foo1.__dict__\n        True\n```\n(should be False).\n\nThis ticket fixes this.\n\nIssue created by migration from https://trac.sagemath.org/ticket/28392\n\n",
    "created_at": "2019-08-23T17:29:00Z",
    "labels": [
        "python3",
        "major",
        "bug"
    ],
    "title": "dynamic_class: do not copy over Python 3's new __weakref__ class attribute",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28155",
    "user": "nthiery"
}
```
CC:  chapoton tscrim embray

Python3 introduce a new attribute `__weakref__` in the datastructure for classes in addition to `__dict__`. As such, this attribute should not be copied over by `dynamic_class`. Otherwise we get errors, typically upon introspection:
```
    sage: from inspect import getmembers
    sage: c = EllipticCurve([0,0,1,-1,0])
    sage: getmembers(c)
    ---------------------------------------------------------------------------
    TypeError                                 Traceback (most recent call last)
    <ipython-input-5-bef5f5512029> in <module>()
    ----> 1 getmembers(c)

    /opt/sage-git/local/lib/python3.7/inspect.py in getmembers(object, predicate)
    339         # looking in the __dict__.
    340         try:
--> 341             value = getattr(object, key)
    342             # handle the duplicate key
    343             if key in processed:

    TypeError: descriptor '__weakref__' for 'Sets.ParentMethods' objects doesn't apply to 'EllipticCurve_rational_field_with_category' object
```
The same error can be triggered by calling directly `c.__weakref__`.

This can be reduced down to:
```
        sage: class A:
        ....:     pass
        sage: A.__weakref__
        <attribute '__weakref__' of 'A' objects>
        sage: Foo1 = sage.structure.dynamic_class.dynamic_class("Foo", (), A)
        sage: "__weakref__" in Foo1.__dict__
        True
```
(should be False).

This ticket fixes this.

Issue created by migration from https://trac.sagemath.org/ticket/28392





---

archive/issue_comments_397581.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-08-23T21:06:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28155",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28155#issuecomment-397581",
    "user": "nthiery"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_397582.json:
```json
{
    "body": "New commits:",
    "created_at": "2019-08-23T21:06:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28155",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28155#issuecomment-397582",
    "user": "nthiery"
}
```

New commits:



---

archive/issue_comments_397583.json:
```json
{
    "body": "LGTM",
    "created_at": "2019-09-18T13:21:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28155",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28155#issuecomment-397583",
    "user": "embray"
}
```

LGTM



---

archive/issue_comments_397584.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-09-18T13:21:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28155",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28155#issuecomment-397584",
    "user": "embray"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_397585.json:
```json
{
    "body": "(FWIW `__weakref__` is not new, just this error, but the fact it was being copied is a mistake)",
    "created_at": "2019-09-18T13:26:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28155",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28155#issuecomment-397585",
    "user": "embray"
}
```

(FWIW `__weakref__` is not new, just this error, but the fact it was being copied is a mistake)



---

archive/issue_comments_397586.json:
```json
{
    "body": "moving milestone to 9.0 (after release of 8.9)",
    "created_at": "2019-09-30T08:12:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28155",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28155#issuecomment-397586",
    "user": "chapoton"
}
```

moving milestone to 9.0 (after release of 8.9)



---

archive/issue_comments_397587.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-10-03T17:57:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28155",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28155#issuecomment-397587",
    "user": "vbraun"
}
```

Resolution: fixed
