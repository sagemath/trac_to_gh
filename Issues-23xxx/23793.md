# Issue 23793: Doctest: Bug in symbolic GCD computations involving complex I

archive/issues_023556.json:
```json
{
    "assignees": [],
    "body": "For example `gcd(I-I*x, x^2-1)` will raise a runtime error (because `I` cannot be converted to Singular by Pynac). \n\nDepends on #23950\n\nBranch/Commit: **[638a14b](https://github.com/sagemath/sagetrac-mirror/commit/638a14bdc30f8613b8263271aa7f9a56f908c3d3)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23793_\n\n",
    "closed_at": "2017-10-29T10:32:01Z",
    "created_at": "2017-09-06T14:42:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Doctest: Bug in symbolic GCD computations involving complex I",
    "type": "issue",
    "updated_at": "2017-10-29T10:32:01Z",
    "url": "https://github.com/sagemath/sage/issues/23793",
    "user": "https://github.com/rwst"
}
```
For example `gcd(I-I*x, x^2-1)` will raise a runtime error (because `I` cannot be converted to Singular by Pynac). 

Depends on #23950

Branch/Commit: **[638a14b](https://github.com/sagemath/sagetrac-mirror/commit/638a14bdc30f8613b8263271aa7f9a56f908c3d3)**

Reviewer: **Travis Scrimshaw**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/23793_





---

archive/issue_events_312546.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-09-06T14:42:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312546"
}
```



---

archive/issue_events_312547.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-09-06T14:42:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312547"
}
```



---

archive/issue_events_312548.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-09-06T14:42:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312548"
}
```



---

archive/issue_events_312549.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-09-06T14:42:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312549"
}
```



---

archive/issue_comments_361536.json:
```json
{
    "body": "Branch: **[u/rws/fall_back_to_maxima_with_some_symbolic_gcd_computations](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/fall_back_to_maxima_with_some_symbolic_gcd_computations)**",
    "created_at": "2017-09-06T14:45:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361536",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/fall_back_to_maxima_with_some_symbolic_gcd_computations](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/fall_back_to_maxima_with_some_symbolic_gcd_computations)**



---

archive/issue_events_312550.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-09-06T14:45:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312550"
}
```



---

archive/issue_comments_361537.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2017-09-06T14:45:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361537",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_361538.json:
```json
{
    "body": "Commit: **[dd43864](https://github.com/sagemath/sagetrac-mirror/commit/dd43864a3c7e3cab30f630adcb4c2f45f26b3b67)**",
    "created_at": "2017-09-06T14:45:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361538",
    "user": "https://github.com/rwst"
}
```

Commit: **[dd43864](https://github.com/sagemath/sagetrac-mirror/commit/dd43864a3c7e3cab30f630adcb4c2f45f26b3b67)**



---

archive/issue_comments_361539.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dd43864a3c7e3cab30f630adcb4c2f45f26b3b67\">dd43864</a></td><td><code>23793: Fall back to Maxima with some symbolic GCD computations</code></td></tr></table>\n",
    "created_at": "2017-09-06T14:45:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361539",
    "user": "https://github.com/rwst"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dd43864a3c7e3cab30f630adcb4c2f45f26b3b67">dd43864</a></td><td><code>23793: Fall back to Maxima with some symbolic GCD computations</code></td></tr></table>




---

archive/issue_comments_361540.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-For example `gcd(I-I*x, x^2-1)` will raise a runtime error (because `I` cannot be converted to Singular by Pynac). Maxima can solve this case and possibly others that give a runtime error.\n+For example `gcd(I-I*x, x^2-1)` will raise a runtime error (because `I` cannot be converted to Singular by Pynac). \n``````\n",
    "created_at": "2017-10-02T06:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361540",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-For example `gcd(I-I*x, x^2-1)` will raise a runtime error (because `I` cannot be converted to Singular by Pynac). Maxima can solve this case and possibly others that give a runtime error.
+For example `gcd(I-I*x, x^2-1)` will raise a runtime error (because `I` cannot be converted to Singular by Pynac). 
``````




---

archive/issue_comments_361541.json:
```json
{
    "body": "Changed branch from **[u/rws/fall_back_to_maxima_with_some_symbolic_gcd_computations](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/fall_back_to_maxima_with_some_symbolic_gcd_computations)** to none",
    "created_at": "2017-10-02T06:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361541",
    "user": "https://github.com/rwst"
}
```

Changed branch from **[u/rws/fall_back_to_maxima_with_some_symbolic_gcd_computations](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/fall_back_to_maxima_with_some_symbolic_gcd_computations)** to none



---

archive/issue_events_312551.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-02T06:05:11Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "title_is": "Doctest: Bug in symbolic GCD computations involving complex I",
    "title_was": "Fall back to Maxima with some symbolic GCD computations",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312551"
}
```



---

archive/issue_comments_361542.json:
```json
{
    "body": "Changed author from **Ralf Stephan** to none",
    "created_at": "2017-10-02T06:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361542",
    "user": "https://github.com/rwst"
}
```

Changed author from **Ralf Stephan** to none



---

archive/issue_comments_361543.json:
```json
{
    "body": "Changed commit from **[dd43864](https://github.com/sagemath/sagetrac-mirror/commit/dd43864a3c7e3cab30f630adcb4c2f45f26b3b67)** to none",
    "created_at": "2017-10-02T06:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361543",
    "user": "https://github.com/rwst"
}
```

Changed commit from **[dd43864](https://github.com/sagemath/sagetrac-mirror/commit/dd43864a3c7e3cab30f630adcb4c2f45f26b3b67)** to none



---

archive/issue_events_312552.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-02T06:05:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312552"
}
```



---

archive/issue_events_312553.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-02T06:05:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312553"
}
```



---

archive/issue_comments_361544.json:
```json
{
    "body": "Dependencies: **#23950**",
    "created_at": "2017-10-02T06:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361544",
    "user": "https://github.com/rwst"
}
```

Dependencies: **#23950**



---

archive/issue_comments_361545.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nActually this is resolved in Pynac-0.7.12 and just needs a doctest.",
    "created_at": "2017-10-02T06:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361545",
    "user": "https://github.com/rwst"
}
```

<a id='comment:3'>Comment 3:</a>
Actually this is resolved in Pynac-0.7.12 and just needs a doctest.



---

archive/issue_comments_361546.json:
```json
{
    "body": "Branch: **[u/rws/23793](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/23793)**",
    "created_at": "2017-10-24T14:59:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361546",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/23793](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/23793)**



---

archive/issue_comments_361547.json:
```json
{
    "body": "Commit: **[638a14b](https://github.com/sagemath/sagetrac-mirror/commit/638a14bdc30f8613b8263271aa7f9a56f908c3d3)**",
    "created_at": "2017-10-24T14:59:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361547",
    "user": "https://github.com/rwst"
}
```

Commit: **[638a14b](https://github.com/sagemath/sagetrac-mirror/commit/638a14bdc30f8613b8263271aa7f9a56f908c3d3)**



---

archive/issue_comments_361548.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/638a14bdc30f8613b8263271aa7f9a56f908c3d3\">638a14b</a></td><td><code>23793: Doctest fix for GCD bug</code></td></tr></table>\n",
    "created_at": "2017-10-24T14:59:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361548",
    "user": "https://github.com/rwst"
}
```

<a id='comment:5'>Comment 5:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/638a14bdc30f8613b8263271aa7f9a56f908c3d3">638a14b</a></td><td><code>23793: Doctest fix for GCD bug</code></td></tr></table>




---

archive/issue_comments_361549.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2017-10-24T14:59:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361549",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_events_312554.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-24T14:59:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312554"
}
```



---

archive/issue_events_312555.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-24T14:59:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312555"
}
```



---

archive/issue_comments_361550.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nLGTM.",
    "created_at": "2017-10-25T00:31:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361550",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
LGTM.



---

archive/issue_events_312556.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-10-25T00:31:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312556"
}
```



---

archive/issue_events_312557.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-10-25T00:31:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312557"
}
```



---

archive/issue_comments_361551.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-10-25T00:31:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361551",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_361552.json:
```json
{
    "body": "Changed branch from **[u/rws/23793](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/23793)** to **[638a14b](https://github.com/sagemath/sagetrac-mirror/commit/638a14bdc30f8613b8263271aa7f9a56f908c3d3)**",
    "created_at": "2017-10-29T10:32:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23793#issuecomment-361552",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/23793](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/23793)** to **[638a14b](https://github.com/sagemath/sagetrac-mirror/commit/638a14bdc30f8613b8263271aa7f9a56f908c3d3)**



---

archive/issue_events_312558.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-29T10:32:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312558"
}
```



---

archive/issue_events_312559.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9898df4172d17b56cd7b4c62baa005148aedb1a3",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-10-29T10:32:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23793",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23793#event-312559"
}
```
