# Issue 23997: installed_packages(False) gives wrong version for Python packages with patches

archive/issues_023760.json:
```json
{
    "assignees": [],
    "body": "Sage uses a convention to append `.pNUM` to indicate patched or updated packages. However, because of the logic in `installed_packages(False)`, the version number returned by `installed_packages(False)` is the version given by `pip` instead of the Sage version number.\n\nThis causes in particular that optional doctests are not run for such packages. At the moment, the only package with this problem and which has optional doctests is `dot2tex`.\n\nCC:  @koffie\n\nBranch/Commit: **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23997_\n\n",
    "closed_at": "2017-10-15T09:22:03Z",
    "created_at": "2017-10-09T12:33:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "installed_packages(False) gives wrong version for Python packages with patches",
    "type": "issue",
    "updated_at": "2017-10-15T09:22:03Z",
    "url": "https://github.com/sagemath/sage/issues/23997",
    "user": "https://github.com/jdemeyer"
}
```
Sage uses a convention to append `.pNUM` to indicate patched or updated packages. However, because of the logic in `installed_packages(False)`, the version number returned by `installed_packages(False)` is the version given by `pip` instead of the Sage version number.

This causes in particular that optional doctests are not run for such packages. At the moment, the only package with this problem and which has optional doctests is `dot2tex`.

CC:  @koffie

Branch/Commit: **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/23997_





---

archive/issue_events_315218.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-09T12:33:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315218"
}
```



---

archive/issue_events_315219.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-09T12:33:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315219"
}
```



---

archive/issue_events_315220.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-09T12:33:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315220"
}
```



---

archive/issue_comments_365238.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Sage uses a convention to append `.pNUM` to indicate patched or updated packages. However, because of the logic in `installed_packages(False)`, the version number returned by `installed_packages(False)` is the version given by `pip` instead of the Sage version number.\n \n-This causes in particular that optional doctests are not run for such packages.\n+This causes in particular that optional doctests are not run for such packages. At the moment, the only package with this problem and which has optional doctests is `dot2tex`.\n``````\n",
    "created_at": "2017-10-09T12:37:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365238",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Sage uses a convention to append `.pNUM` to indicate patched or updated packages. However, because of the logic in `installed_packages(False)`, the version number returned by `installed_packages(False)` is the version given by `pip` instead of the Sage version number.
 
-This causes in particular that optional doctests are not run for such packages.
+This causes in particular that optional doctests are not run for such packages. At the moment, the only package with this problem and which has optional doctests is `dot2tex`.
``````




---

archive/issue_comments_365239.json:
```json
{
    "body": "Branch: **[u/jdemeyer/installed_packages_false__gives_wrong_version_for_python_packages_with_patches](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/installed_packages_false__gives_wrong_version_for_python_packages_with_patches)**",
    "created_at": "2017-10-09T13:53:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365239",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/installed_packages_false__gives_wrong_version_for_python_packages_with_patches](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/installed_packages_false__gives_wrong_version_for_python_packages_with_patches)**



---

archive/issue_comments_365240.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce\">8b4b305</a></td><td><code>Sage package version should override pip package version</code></td></tr></table>\n",
    "created_at": "2017-10-09T13:53:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365240",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce">8b4b305</a></td><td><code>Sage package version should override pip package version</code></td></tr></table>




---

archive/issue_events_315221.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-09T13:53:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315221"
}
```



---

archive/issue_comments_365241.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2017-10-09T13:53:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365241",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_365242.json:
```json
{
    "body": "Commit: **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)**",
    "created_at": "2017-10-09T13:53:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365242",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)**



---

archive/issue_comments_365243.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nMakes sense to me.",
    "created_at": "2017-10-09T15:15:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365243",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

Makes sense to me.



---

archive/issue_comments_365244.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-10-09T15:15:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365244",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_315222.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-10-09T15:15:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315222"
}
```



---

archive/issue_events_315223.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-10-09T15:15:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315223"
}
```



---

archive/issue_comments_365245.json:
```json
{
    "body": "Changed commit from **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)** to **[c5ad9c5](https://github.com/sagemath/sagetrac-mirror/commit/c5ad9c5cf928cb65046e76c9d633e30879874780)**",
    "created_at": "2017-10-09T19:23:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365245",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)** to **[c5ad9c5](https://github.com/sagemath/sagetrac-mirror/commit/c5ad9c5cf928cb65046e76c9d633e30879874780)**



---

archive/issue_events_315224.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2017-10-09T19:23:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315224"
}
```



---

archive/issue_events_315225.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2017-10-09T19:23:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315225"
}
```



---

archive/issue_comments_365246.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c5ad9c5cf928cb65046e76c9d633e30879874780\">c5ad9c5</a></td><td><code>Remove \"optional - dot2tex graphviz\" where it is not needed</code></td></tr></table>\n",
    "created_at": "2017-10-09T19:23:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365246",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c5ad9c5cf928cb65046e76c9d633e30879874780">c5ad9c5</a></td><td><code>Remove "optional - dot2tex graphviz" where it is not needed</code></td></tr></table>




---

archive/issue_comments_365247.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:",
    "created_at": "2017-10-09T19:24:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365247",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:



---

archive/issue_comments_365248.json:
```json
{
    "body": "Changed commit from **[c5ad9c5](https://github.com/sagemath/sagetrac-mirror/commit/c5ad9c5cf928cb65046e76c9d633e30879874780)** to **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)**",
    "created_at": "2017-10-09T19:24:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365248",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c5ad9c5](https://github.com/sagemath/sagetrac-mirror/commit/c5ad9c5cf928cb65046e76c9d633e30879874780)** to **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)**



---

archive/issue_events_315226.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-09T19:25:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315226"
}
```



---

archive/issue_events_315227.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-09T19:25:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315227"
}
```



---

archive/issue_comments_365249.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nAccidentally committed on the wrong branch. Fixed it.",
    "created_at": "2017-10-09T19:25:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365249",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:8" align="right">Comment 8</div>

Accidentally committed on the wrong branch. Fixed it.



---

archive/issue_events_315228.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-15T09:22:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315228"
}
```



---

archive/issue_events_315229.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fce8fd00d43aa17b548512d3b3e772f11f0460c9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-10-15T09:22:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23997#event-315229"
}
```



---

archive/issue_comments_365250.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/installed_packages_false__gives_wrong_version_for_python_packages_with_patches](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/installed_packages_false__gives_wrong_version_for_python_packages_with_patches)** to **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)**",
    "created_at": "2017-10-15T09:22:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23997#issuecomment-365250",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/installed_packages_false__gives_wrong_version_for_python_packages_with_patches](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/installed_packages_false__gives_wrong_version_for_python_packages_with_patches)** to **[8b4b305](https://github.com/sagemath/sagetrac-mirror/commit/8b4b3052f340736fbf551f6ae45cfb53a0492bce)**
