# Issue 23354: numerical noise in libs/lcalc/lcalc_Lfuntion.pyx with clang-4 and icc

archive/issues_023117.json:
```json
{
    "assignees": [],
    "body": "One doctest in `libs/lcalc/lcalc_Lfuntion.pyx` fails because of excessive numerical noise when sage is compiled with clang >= 4.0 or icc (17.04).\n\nclang-4.0.1\n\n```\nFile \"src/sage/libs/lcalc/lcalc_Lfunction.pyx\", line 188, in sage.libs.lcalc.lcalc_Lfunction.Lfunction.hardy_z_function\nFailed example:\n    L.hardy_z_function(0)\nExpected:\n    0.7939675904771...\nGot:\n    0.793967590477206 - 4.03312243040125e-17*I\n```\nicc:\n\n```\nFile \"src/sage/libs/lcalc/lcalc_Lfunction.pyx\", line 188, in sage.libs.lcalc.lcalc_Lfunction.Lfunction.hardy_z_function\nFailed example:\n    L.hardy_z_function(0)\nExpected:\n    0.7939675904771...\nGot:\n    0.793967590477208 - 4.03312243040124e-17*I\n```\n\n\nCC:  @dimpase\n\nBranch: **[6ec19b5](https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Fran\u00e7ois Bissey**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23354_\n\n",
    "closed_at": "2017-07-26T22:12:48Z",
    "created_at": "2017-07-03T02:14:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "numerical noise in libs/lcalc/lcalc_Lfuntion.pyx with clang-4 and icc",
    "type": "issue",
    "updated_at": "2017-12-08T14:57:09Z",
    "url": "https://github.com/sagemath/sage/issues/23354",
    "user": "https://github.com/kiwifb"
}
```
One doctest in `libs/lcalc/lcalc_Lfuntion.pyx` fails because of excessive numerical noise when sage is compiled with clang >= 4.0 or icc (17.04).

clang-4.0.1

```
File "src/sage/libs/lcalc/lcalc_Lfunction.pyx", line 188, in sage.libs.lcalc.lcalc_Lfunction.Lfunction.hardy_z_function
Failed example:
    L.hardy_z_function(0)
Expected:
    0.7939675904771...
Got:
    0.793967590477206 - 4.03312243040125e-17*I
```
icc:

```
File "src/sage/libs/lcalc/lcalc_Lfunction.pyx", line 188, in sage.libs.lcalc.lcalc_Lfunction.Lfunction.hardy_z_function
Failed example:
    L.hardy_z_function(0)
Expected:
    0.7939675904771...
Got:
    0.793967590477208 - 4.03312243040124e-17*I
```


CC:  @dimpase

Branch: **[6ec19b5](https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f)**

Reviewer: **Dima Pasechnik**

Author: **François Bissey**

_Issue created by migration from https://trac.sagemath.org/ticket/23354_





---

archive/issue_events_307197.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2017-07-03T02:14:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307197"
}
```



---

archive/issue_events_307198.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2017-07-03T02:14:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307198"
}
```



---

archive/issue_events_307199.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2017-07-03T02:14:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307199"
}
```



---

archive/issue_events_307200.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2017-07-03T02:14:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307200"
}
```



---

archive/issue_comments_353419.json:
```json
{
    "body": "Branch: **[u/fbissey/lcalc_numerical_noise](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/lcalc_numerical_noise)**",
    "created_at": "2017-07-03T02:17:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353419",
    "user": "https://github.com/kiwifb"
}
```

Branch: **[u/fbissey/lcalc_numerical_noise](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/lcalc_numerical_noise)**



---

archive/issue_comments_353420.json:
```json
{
    "body": "Commit: **[7de256c](https://github.com/sagemath/sagetrac-mirror/commit/7de256c24f2c63df50b667a32d7c4969524693af)**",
    "created_at": "2017-07-03T02:17:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353420",
    "user": "https://github.com/kiwifb"
}
```

Commit: **[7de256c](https://github.com/sagemath/sagetrac-mirror/commit/7de256c24f2c63df50b667a32d7c4969524693af)**



---

archive/issue_comments_353421.json:
```json
{
    "body": "Author: **Francois Bissey**",
    "created_at": "2017-07-03T02:17:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353421",
    "user": "https://github.com/kiwifb"
}
```

Author: **Francois Bissey**



---

archive/issue_events_307201.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2017-07-03T02:17:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307201"
}
```



---

archive/issue_comments_353422.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNo commit on the branch.",
    "created_at": "2017-07-03T05:41:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353422",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

No commit on the branch.



---

archive/issue_events_307202.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-03T05:41:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307202"
}
```



---

archive/issue_events_307203.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-03T05:41:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307203"
}
```



---

archive/issue_comments_353423.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f\">6ec19b5</a></td><td><code>fix numerical noise</code></td></tr></table>\n",
    "created_at": "2017-07-03T06:28:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353423",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f">6ec19b5</a></td><td><code>fix numerical noise</code></td></tr></table>




---

archive/issue_comments_353424.json:
```json
{
    "body": "Changed commit from **[7de256c](https://github.com/sagemath/sagetrac-mirror/commit/7de256c24f2c63df50b667a32d7c4969524693af)** to **[6ec19b5](https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f)**",
    "created_at": "2017-07-03T06:28:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353424",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7de256c](https://github.com/sagemath/sagetrac-mirror/commit/7de256c24f2c63df50b667a32d7c4969524693af)** to **[6ec19b5](https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f)**



---

archive/issue_comments_353425.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOoops. There is now.",
    "created_at": "2017-07-03T06:28:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353425",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:4" align="right">Comment 4</div>

Ooops. There is now.



---

archive/issue_comments_353426.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThe question is whether we should rather remove `-ffast-math` from the flags---which probably will fix this noise by itself. I haven't tried this yet.",
    "created_at": "2017-07-03T10:05:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353426",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:5" align="right">Comment 5</div>

The question is whether we should rather remove `-ffast-math` from the flags---which probably will fix this noise by itself. I haven't tried this yet.



---

archive/issue_comments_353427.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nIf we have the flag for gcc as well, it is not obvious which way things will go. All three may converge to something slightly different again.",
    "created_at": "2017-07-03T10:09:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353427",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:6" align="right">Comment 6</div>

If we have the flag for gcc as well, it is not obvious which way things will go. All three may converge to something slightly different again.



---

archive/issue_comments_353428.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nOK.",
    "created_at": "2017-07-03T10:17:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353428",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:7" align="right">Comment 7</div>

OK.



---

archive/issue_events_307204.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-07-03T10:17:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307204"
}
```



---

archive/issue_events_307205.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-07-03T10:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307205"
}
```



---

archive/issue_comments_353429.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2017-07-03T10:17:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353429",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_307206.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-26T22:12:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307206"
}
```



---

archive/issue_events_307207.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "beda263d3d0fcfbe2a488422a6dd55fd667c9bc5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-07-26T22:12:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23354#event-307207"
}
```



---

archive/issue_comments_353430.json:
```json
{
    "body": "Changed branch from **[u/fbissey/lcalc_numerical_noise](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/lcalc_numerical_noise)** to **[6ec19b5](https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f)**",
    "created_at": "2017-07-26T22:12:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353430",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/fbissey/lcalc_numerical_noise](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/lcalc_numerical_noise)** to **[6ec19b5](https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f)**



---

archive/issue_comments_353431.json:
```json
{
    "body": "Changed author from **Francois Bissey** to **Fran\u00e7ois Bissey**",
    "created_at": "2017-12-08T14:57:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353431",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Francois Bissey** to **François Bissey**



---

archive/issue_comments_353432.json:
```json
{
    "body": "Changed commit from **[6ec19b5](https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f)** to none",
    "created_at": "2017-12-08T14:57:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23354#issuecomment-353432",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[6ec19b5](https://github.com/sagemath/sagetrac-mirror/commit/6ec19b5c462c5a8d058a251364ab6512c3d1b03f)** to none
