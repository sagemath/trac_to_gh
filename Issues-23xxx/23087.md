...
```json
{
    "body": "Just to get a better idea what's going on might be helpful if, instead of testing `sage: 'In [1]: [False, True]' in output`, the test output was something resembling the expected contents of `output` (with ellipses as needed).",
...
```json
{
    "body": "I updated the test to be a little more informative (in particular, if the test fails we'll get what the actual output was).  However, on my last test full test run this test did not fail.\n\nI think the update to the test is good to have in its own right, but I'm going to try a few more times to see if I can get it to fail.\n\n---\nNew commits:",
...
```json
{
    "body": "Just realized that it's not strictly necessary to get this merged. I can just cherry-pick the patch for nixos and wait for our buildservers (which rebuild sage often) to stumble about it again. I don't like abusing the buildservers like this, but seems better than just disabling the test.",
