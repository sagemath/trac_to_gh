# Issue 23578: Force make doc-clean on upgrade

archive/issues_023341.json:
```json
{
    "assignees": [],
    "body": "Literally **every** time I upgrade sage, the build throws an error on building the documents. This is on sage originally built from source, and upgraded using sage -upgrade over the network.\n\nNow the fix is easy, just run make doc-clean and re-run the upgrade, but would it be possible to make the default behaviour of sage -upgrade to run make doc-clean before doing anything else?\n\nThis would also help those first time builders who hit this and don't know how to proceed.\n\nKeywords: **upgrade doc-clean**\n\nAuthor: **Joal Heagney**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23578_\n\n",
    "closed_at": "2017-12-12T08:23:33Z",
    "created_at": "2017-08-03T19:47:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Force make doc-clean on upgrade",
    "type": "issue",
    "updated_at": "2017-12-12T08:23:33Z",
    "url": "https://github.com/sagemath/sage/issues/23578",
    "user": "https://github.com/sagetrac-JoalHeagney"
}
```
Literally **every** time I upgrade sage, the build throws an error on building the documents. This is on sage originally built from source, and upgraded using sage -upgrade over the network.

Now the fix is easy, just run make doc-clean and re-run the upgrade, but would it be possible to make the default behaviour of sage -upgrade to run make doc-clean before doing anything else?

This would also help those first time builders who hit this and don't know how to proceed.

Keywords: **upgrade doc-clean**

Author: **Joal Heagney**

_Issue created by migration from https://trac.sagemath.org/ticket/23578_





---

archive/issue_events_310008.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2017-08-03T19:47:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310008"
}
```



---

archive/issue_events_310009.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2017-08-03T19:47:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310009"
}
```



---

archive/issue_events_310010.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2017-08-03T19:47:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310010"
}
```



---

archive/issue_events_310011.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2017-08-03T19:47:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310011"
}
```



---

archive/issue_comments_357505.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI think this is a duplicate of #19882. See also #22588 which tries to solve the same problem with a different approach.",
    "created_at": "2017-08-03T20:23:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23578#issuecomment-357505",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'>Comment 1:</a>
I think this is a duplicate of #19882. See also #22588 which tries to solve the same problem with a different approach.



---

archive/issue_comments_357506.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\n*sigh* Yeah I think it is. Well at least we've got some nice looking patches in those other tickets. Should I mark this as duplicate?",
    "created_at": "2017-08-03T22:26:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23578#issuecomment-357506",
    "user": "https://github.com/sagetrac-JoalHeagney"
}
```

<a id='comment:2'>Comment 2:</a>
*sigh* Yeah I think it is. Well at least we've got some nice looking patches in those other tickets. Should I mark this as duplicate?



---

archive/issue_events_310012.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2017-08-03T22:28:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310012"
}
```



---

archive/issue_events_310013.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2017-08-03T22:28:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310013"
}
```



---

archive/issue_events_310014.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-08-03T23:58:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310014"
}
```



---

archive/issue_events_310015.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-08-03T23:58:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310015"
}
```



---

archive/issue_events_310016.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-08-03T23:58:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310016"
}
```



---

archive/issue_events_310017.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-08-03T23:58:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310017"
}
```



---

archive/issue_events_310018.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-08-03T23:58:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310018"
}
```



---

archive/issue_events_310019.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-08-03T23:58:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310019"
}
```



---

archive/issue_events_310020.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310020"
}
```



---

archive/issue_events_310021.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310021"
}
```



---

archive/issue_events_310022.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23578",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23578#event-310022"
}
```
