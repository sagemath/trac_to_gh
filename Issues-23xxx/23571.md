# Issue 23571: Veronese embeddings for projective subschemes

archive/issues_023334.json:
```json
{
    "assignees": [],
    "body": "Implement a function to return the degree d Veronese embedding of a projective subscheme.\n\nDepends on #23434\n\nCC:  @bhutz\n\nBranch/Commit: **[5bec3ad](https://github.com/sagemath/sagetrac-mirror/commit/5bec3ad8b10bd727715ab7df4dae198753e82e2f)**\n\nReviewer: **Ben Hutz**\n\nAuthor: **Grayson Jorgenson**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23571_\n\n",
    "closed_at": "2017-09-26T06:57:46Z",
    "created_at": "2017-08-02T02:47:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Veronese embeddings for projective subschemes",
    "type": "issue",
    "updated_at": "2017-09-26T06:57:46Z",
    "url": "https://github.com/sagemath/sage/issues/23571",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```
Implement a function to return the degree d Veronese embedding of a projective subscheme.

Depends on #23434

CC:  @bhutz

Branch/Commit: **[5bec3ad](https://github.com/sagemath/sagetrac-mirror/commit/5bec3ad8b10bd727715ab7df4dae198753e82e2f)**

Reviewer: **Ben Hutz**

Author: **Grayson Jorgenson**

_Issue created by migration from https://trac.sagemath.org/ticket/23571_





---

archive/issue_events_309868.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2017-08-02T02:47:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309868"
}
```



---

archive/issue_events_309869.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2017-08-02T02:47:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309869"
}
```



---

archive/issue_events_309870.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2017-08-02T02:47:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309870"
}
```



---

archive/issue_events_309871.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2017-08-02T02:47:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309871"
}
```



---

archive/issue_comments_357209.json:
```json
{
    "body": "Commit: **[f17c8d3](https://github.com/sagemath/sagetrac-mirror/commit/f17c8d3daba84ace58b8bc665e49a9487b6197ad)**",
    "created_at": "2017-08-07T19:30:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357209",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

Commit: **[f17c8d3](https://github.com/sagemath/sagetrac-mirror/commit/f17c8d3daba84ace58b8bc665e49a9487b6197ad)**



---

archive/issue_comments_357210.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dd44cc8c67ffbec61d9ed176b0211092cd2cc5b4\">dd44cc8</a></td><td><code>23571: implement Veronese embedding functionality</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/604287c6439b89908f41c755e030ff325a99f145\">604287c</a></td><td><code>23234: move subscheme code to resp. folders</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a2ddbdf943620c72f8007e5f9976bf2a439268ff\">a2ddbdf</a></td><td><code>Merge branch 8.0.rc2 into subscheme_files</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aeeae927e00eb88a27651509b6015a02b5099ef4\">aeeae92</a></td><td><code>Merge branch 8.1.beta0 into t/23434/subscheme_files</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7c583b8b01853abb015e6b96c335c8700f968dbc\">7c583b8</a></td><td><code>Merge branch 8.1.beta1 into t/23434/subscheme_files</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ccadd12ffab86cec2176fdf466a4b83f78d798df\">ccadd12</a></td><td><code>Merge branch 'u/bhutz/subscheme_files' of git://trac.sagemath.org/sage into ticket/23571</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f17c8d3daba84ace58b8bc665e49a9487b6197ad\">f17c8d3</a></td><td><code>23571: exclude trivial case d = 0 which does not give an embedding</code></td></tr></table>\n",
    "created_at": "2017-08-07T19:30:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357210",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dd44cc8c67ffbec61d9ed176b0211092cd2cc5b4">dd44cc8</a></td><td><code>23571: implement Veronese embedding functionality</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/604287c6439b89908f41c755e030ff325a99f145">604287c</a></td><td><code>23234: move subscheme code to resp. folders</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a2ddbdf943620c72f8007e5f9976bf2a439268ff">a2ddbdf</a></td><td><code>Merge branch 8.0.rc2 into subscheme_files</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aeeae927e00eb88a27651509b6015a02b5099ef4">aeeae92</a></td><td><code>Merge branch 8.1.beta0 into t/23434/subscheme_files</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7c583b8b01853abb015e6b96c335c8700f968dbc">7c583b8</a></td><td><code>Merge branch 8.1.beta1 into t/23434/subscheme_files</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ccadd12ffab86cec2176fdf466a4b83f78d798df">ccadd12</a></td><td><code>Merge branch 'u/bhutz/subscheme_files' of git://trac.sagemath.org/sage into ticket/23571</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f17c8d3daba84ace58b8bc665e49a9487b6197ad">f17c8d3</a></td><td><code>23571: exclude trivial case d = 0 which does not give an embedding</code></td></tr></table>




---

archive/issue_comments_357211.json:
```json
{
    "body": "Branch: **[u/gjorgenson/ticket/23571](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/23571)**",
    "created_at": "2017-08-07T19:30:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357211",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

Branch: **[u/gjorgenson/ticket/23571](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/23571)**



---

archive/issue_events_309872.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2017-08-07T19:57:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309872"
}
```



---

archive/issue_comments_357212.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI implemented the Veronese embedding functionality in two functions. One at the level of projective spaces and one for subschemes which returns the map between the ambient spaces restricted to the subscheme and its image (using the existing _forward_image functionality for projective subschemes). I also merged with #23434 to preemptively address the conflict.",
    "created_at": "2017-08-07T19:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357212",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

<a id='comment:2'>Comment 2:</a>
I implemented the Veronese embedding functionality in two functions. One at the level of projective spaces and one for subschemes which returns the map between the ambient spaces restricted to the subscheme and its image (using the existing _forward_image functionality for projective subschemes). I also merged with #23434 to preemptively address the conflict.



---

archive/issue_events_309873.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2017-08-09T16:05:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309873"
}
```



---

archive/issue_events_309874.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2017-08-09T16:05:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309874"
}
```



---

archive/issue_comments_357213.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis looks good except for a very minor issue:\n\n- the two error messages lines 1153, 1662 do not have anything to fill in the %s.",
    "created_at": "2017-08-09T16:05:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357213",
    "user": "https://github.com/bhutz"
}
```

<a id='comment:3'>Comment 3:</a>
This looks good except for a very minor issue:

- the two error messages lines 1153, 1662 do not have anything to fill in the %s.



---

archive/issue_comments_357214.json:
```json
{
    "body": "Reviewer: **Ben Hutz**",
    "created_at": "2017-08-09T16:05:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357214",
    "user": "https://github.com/bhutz"
}
```

Reviewer: **Ben Hutz**



---

archive/issue_comments_357215.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5bec3ad8b10bd727715ab7df4dae198753e82e2f\">5bec3ad</a></td><td><code>23571: fixed error messages in projective space veronese_embedding</code></td></tr></table>\n",
    "created_at": "2017-08-09T17:24:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357215",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5bec3ad8b10bd727715ab7df4dae198753e82e2f">5bec3ad</a></td><td><code>23571: fixed error messages in projective space veronese_embedding</code></td></tr></table>




---

archive/issue_comments_357216.json:
```json
{
    "body": "Changed commit from **[f17c8d3](https://github.com/sagemath/sagetrac-mirror/commit/f17c8d3daba84ace58b8bc665e49a9487b6197ad)** to **[5bec3ad](https://github.com/sagemath/sagetrac-mirror/commit/5bec3ad8b10bd727715ab7df4dae198753e82e2f)**",
    "created_at": "2017-08-09T17:24:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357216",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f17c8d3](https://github.com/sagemath/sagetrac-mirror/commit/f17c8d3daba84ace58b8bc665e49a9487b6197ad)** to **[5bec3ad](https://github.com/sagemath/sagetrac-mirror/commit/5bec3ad8b10bd727715ab7df4dae198753e82e2f)**



---

archive/issue_comments_357217.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThanks, fixed.",
    "created_at": "2017-08-09T17:27:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357217",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

<a id='comment:5'>Comment 5:</a>
Thanks, fixed.



---

archive/issue_events_309875.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2017-08-09T17:27:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309875"
}
```



---

archive/issue_events_309876.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2017-08-09T17:27:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309876"
}
```



---

archive/issue_events_309877.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2017-08-09T19:24:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309877"
}
```



---

archive/issue_events_309878.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2017-08-09T19:24:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309878"
}
```



---

archive/issue_events_309879.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-09-26T06:57:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309879"
}
```



---

archive/issue_events_309880.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "79935595b6d3a47cf1ded2ae9e90afeeb1d9e156",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-09-26T06:57:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23571#event-309880"
}
```



---

archive/issue_comments_357218.json:
```json
{
    "body": "Changed branch from **[u/gjorgenson/ticket/23571](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/23571)** to **[5bec3ad](https://github.com/sagemath/sagetrac-mirror/commit/5bec3ad8b10bd727715ab7df4dae198753e82e2f)**",
    "created_at": "2017-09-26T06:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23571#issuecomment-357218",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gjorgenson/ticket/23571](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/23571)** to **[5bec3ad](https://github.com/sagemath/sagetrac-mirror/commit/5bec3ad8b10bd727715ab7df4dae198753e82e2f)**
