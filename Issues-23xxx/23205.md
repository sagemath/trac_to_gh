# Issue 23205: Do not require surjectivity to be implemented for composite injectivity

archive/issues_022968.json:
```json
{
    "assignees": [],
    "body": "Currently, this fails even though all morphisms are known to be injective.\n\n```\nsage: QQ.hom(QQbar)*ZZ.hom(QQ)\n\nComposite map:\n  From: Integer Ring\n  To:   Algebraic Field\n  Defn:   Ring Coercion morphism:\n          From: Integer Ring\n          To:   Rational Field\n        then\n          Ring Coercion morphism:\n          From: Rational Field\n          To:   Algebraic Field\nsage: _.is_injective()\n```\n\nKeywords: **sd86.5**\n\nBranch/Commit: **[b21c370](https://github.com/sagemath/sagetrac-mirror/commit/b21c370a4a3501d1ce35463e535fceaa258ccc83)**\n\nReviewer: **David Roe**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23205_\n\n",
    "closed_at": "2017-06-12T18:01:43Z",
    "created_at": "2017-06-10T00:12:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Do not require surjectivity to be implemented for composite injectivity",
    "type": "issue",
    "updated_at": "2017-06-12T18:01:43Z",
    "url": "https://github.com/sagemath/sage/issues/23205",
    "user": "https://github.com/saraedum"
}
```
Currently, this fails even though all morphisms are known to be injective.

```
sage: QQ.hom(QQbar)*ZZ.hom(QQ)

Composite map:
  From: Integer Ring
  To:   Algebraic Field
  Defn:   Ring Coercion morphism:
          From: Integer Ring
          To:   Rational Field
        then
          Ring Coercion morphism:
          From: Rational Field
          To:   Algebraic Field
sage: _.is_injective()
```

Keywords: **sd86.5**

Branch/Commit: **[b21c370](https://github.com/sagemath/sagetrac-mirror/commit/b21c370a4a3501d1ce35463e535fceaa258ccc83)**

Reviewer: **David Roe**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/23205_





---

archive/issue_events_305402.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-10T00:12:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23205#event-305402"
}
```



---

archive/issue_events_305403.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-10T00:12:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23205#event-305403"
}
```



---

archive/issue_events_305404.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-10T00:12:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23205#event-305404"
}
```



---

archive/issue_events_305405.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-10T00:12:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23205#event-305405"
}
```



---

archive/issue_comments_350823.json:
```json
{
    "body": "Branch: **[u/saraedum/do_not_require_surjectivity_to_be_implemented_for_composite_injectivity](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/do_not_require_surjectivity_to_be_implemented_for_composite_injectivity)**",
    "created_at": "2017-06-10T01:07:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23205#issuecomment-350823",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/do_not_require_surjectivity_to_be_implemented_for_composite_injectivity](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/do_not_require_surjectivity_to_be_implemented_for_composite_injectivity)**



---

archive/issue_comments_350824.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b21c370a4a3501d1ce35463e535fceaa258ccc83\">b21c370</a></td><td><code>Do not require is_surjective/is_injective to be implemented on as many morphisms to decide injectivity/surjectivity of composite maps</code></td></tr></table>\n",
    "created_at": "2017-06-10T01:07:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23205#issuecomment-350824",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b21c370a4a3501d1ce35463e535fceaa258ccc83">b21c370</a></td><td><code>Do not require is_surjective/is_injective to be implemented on as many morphisms to decide injectivity/surjectivity of composite maps</code></td></tr></table>




---

archive/issue_comments_350825.json:
```json
{
    "body": "Commit: **[b21c370](https://github.com/sagemath/sagetrac-mirror/commit/b21c370a4a3501d1ce35463e535fceaa258ccc83)**",
    "created_at": "2017-06-10T01:07:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23205#issuecomment-350825",
    "user": "https://github.com/saraedum"
}
```

Commit: **[b21c370](https://github.com/sagemath/sagetrac-mirror/commit/b21c370a4a3501d1ce35463e535fceaa258ccc83)**



---

archive/issue_events_305406.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-10T01:07:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23205#event-305406"
}
```



---

archive/issue_events_305407.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-06-10T17:29:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23205#event-305407"
}
```



---

archive/issue_events_305408.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-06-10T17:29:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23205#event-305408"
}
```



---

archive/issue_comments_350826.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2017-06-10T17:29:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23205#issuecomment-350826",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe**



---

archive/issue_comments_350827.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAll tests pass; looks good.",
    "created_at": "2017-06-10T17:29:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23205#issuecomment-350827",
    "user": "https://github.com/roed314"
}
```

<div id="comment:3" align="right">Comment 3</div>

All tests pass; looks good.



---

archive/issue_events_305409.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-12T18:01:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23205#event-305409"
}
```



---

archive/issue_events_305410.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "39d57353f9bd39e36ac3bc1114196e5633fef14c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-06-12T18:01:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23205#event-305410"
}
```



---

archive/issue_comments_350828.json:
```json
{
    "body": "Changed branch from **[u/saraedum/do_not_require_surjectivity_to_be_implemented_for_composite_injectivity](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/do_not_require_surjectivity_to_be_implemented_for_composite_injectivity)** to **[b21c370](https://github.com/sagemath/sagetrac-mirror/commit/b21c370a4a3501d1ce35463e535fceaa258ccc83)**",
    "created_at": "2017-06-12T18:01:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23205#issuecomment-350828",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/do_not_require_surjectivity_to_be_implemented_for_composite_injectivity](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/do_not_require_surjectivity_to_be_implemented_for_composite_injectivity)** to **[b21c370](https://github.com/sagemath/sagetrac-mirror/commit/b21c370a4a3501d1ce35463e535fceaa258ccc83)**
