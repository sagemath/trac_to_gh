# Issue 23188: p-adic rings embed into their fraction fields

archive/issues_022951.json:
```json
{
    "assignees": [],
    "body": "Currently, this fails:\n\n```\nsage: R.<a> = ZqCA(9)\nsage: R.fraction_field().coerce_map_from(R).is_injective()\nNotImplementedError\n```\n\nCC:  @roed314\n\nKeywords: **sd86.5**\n\nBranch/Commit: **[2deaa0f](https://github.com/sagemath/sagetrac-mirror/commit/2deaa0fedc581c87b35f99b9362d4bfd831d4a26)**\n\nReviewer: **David Roe**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23188_\n\n",
    "closed_at": "2017-06-15T23:09:52Z",
    "created_at": "2017-06-09T06:48:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "p-adic rings embed into their fraction fields",
    "type": "issue",
    "updated_at": "2017-06-15T23:09:52Z",
    "url": "https://github.com/sagemath/sage/issues/23188",
    "user": "https://github.com/saraedum"
}
```
Currently, this fails:

```
sage: R.<a> = ZqCA(9)
sage: R.fraction_field().coerce_map_from(R).is_injective()
NotImplementedError
```

CC:  @roed314

Keywords: **sd86.5**

Branch/Commit: **[2deaa0f](https://github.com/sagemath/sagetrac-mirror/commit/2deaa0fedc581c87b35f99b9362d4bfd831d4a26)**

Reviewer: **David Roe**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/23188_





---

archive/issue_events_305183.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-09T06:48:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23188#event-305183"
}
```



---

archive/issue_events_305184.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-09T06:48:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23188#event-305184"
}
```



---

archive/issue_events_305185.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-09T06:48:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23188#event-305185"
}
```



---

archive/issue_events_305186.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-09T06:48:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23188#event-305186"
}
```



---

archive/issue_comments_350490.json:
```json
{
    "body": "Branch: **[u/saraedum/p_adic_rings_embed_into_their_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/p_adic_rings_embed_into_their_fraction_fields)**",
    "created_at": "2017-06-09T21:00:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23188#issuecomment-350490",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/p_adic_rings_embed_into_their_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/p_adic_rings_embed_into_their_fraction_fields)**



---

archive/issue_comments_350491.json:
```json
{
    "body": "Commit: **[2deaa0f](https://github.com/sagemath/sagetrac-mirror/commit/2deaa0fedc581c87b35f99b9362d4bfd831d4a26)**",
    "created_at": "2017-06-09T21:03:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23188#issuecomment-350491",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[2deaa0f](https://github.com/sagemath/sagetrac-mirror/commit/2deaa0fedc581c87b35f99b9362d4bfd831d4a26)**



---

archive/issue_comments_350492.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2deaa0fedc581c87b35f99b9362d4bfd831d4a26\">2deaa0f</a></td><td><code>Implement is_injective/is_surjective for inclusions of p-adic rings into their fraction fields</code></td></tr></table>\n",
    "created_at": "2017-06-09T21:03:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23188#issuecomment-350492",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2deaa0fedc581c87b35f99b9362d4bfd831d4a26">2deaa0f</a></td><td><code>Implement is_injective/is_surjective for inclusions of p-adic rings into their fraction fields</code></td></tr></table>




---

archive/issue_comments_350493.json:
```json
{
    "body": "Author: **Julian R\u00fcth**",
    "created_at": "2017-06-09T21:06:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23188#issuecomment-350493",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rüth**



---

archive/issue_events_305187.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-09T21:06:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23188#event-305187"
}
```



---

archive/issue_comments_350494.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nSeems fine to me.",
    "created_at": "2017-06-09T21:20:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23188#issuecomment-350494",
    "user": "https://github.com/roed314"
}
```

<div id="comment:4" align="right">Comment 4</div>

Seems fine to me.



---

archive/issue_comments_350495.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2017-06-09T21:20:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23188#issuecomment-350495",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe**



---

archive/issue_events_305188.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-06-09T21:20:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23188#event-305188"
}
```



---

archive/issue_events_305189.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-06-09T21:20:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23188#event-305189"
}
```



---

archive/issue_comments_350496.json:
```json
{
    "body": "Changed branch from **[u/saraedum/p_adic_rings_embed_into_their_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/p_adic_rings_embed_into_their_fraction_fields)** to **[2deaa0f](https://github.com/sagemath/sagetrac-mirror/commit/2deaa0fedc581c87b35f99b9362d4bfd831d4a26)**",
    "created_at": "2017-06-15T23:09:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23188#issuecomment-350496",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/p_adic_rings_embed_into_their_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/p_adic_rings_embed_into_their_fraction_fields)** to **[2deaa0f](https://github.com/sagemath/sagetrac-mirror/commit/2deaa0fedc581c87b35f99b9362d4bfd831d4a26)**



---

archive/issue_events_305190.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-15T23:09:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23188#event-305190"
}
```



---

archive/issue_events_305191.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6838221872b5b87236405f1b9583495b3bc1580e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-06-15T23:09:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23188",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23188#event-305191"
}
```
