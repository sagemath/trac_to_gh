# Issue 23769: src/bin/sage-starts should be moved to build/bin

archive/issues_023532.json:
```json
{
    "assignees": [],
    "body": "`src/bin/sage-starts` uses `build/bin/sage-logger` (writing into `$SAGE_ROOT/logs`)  and therefore belong to sage-the-distribution rather than sagelib. It should not be installed in `$SAGE_LOCAL/bin`.\n\n(It could as well be eliminated and merged into `build/make/deps` (the only place from which it is called); this is where many invocations of `build/bin/sage-logger` happen and this one could as well, for consistency.)\n\nPart of #21559. See also #21510.\n\nCC:  @jdemeyer @kiwifb @jhpalmieri @embray @dimpase\n\nBranch/Commit: **[23ffe1d](https://github.com/sagemath/sagetrac-mirror/commit/23ffe1d9ee6d38980a889b49ad838edeb33b0130)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23769_\n\n",
    "closed_at": "2020-03-25T22:48:48Z",
    "created_at": "2017-08-31T23:44:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "src/bin/sage-starts should be moved to build/bin",
    "type": "issue",
    "updated_at": "2020-03-25T22:48:48Z",
    "url": "https://github.com/sagemath/sage/issues/23769",
    "user": "https://github.com/mkoeppe"
}
```
`src/bin/sage-starts` uses `build/bin/sage-logger` (writing into `$SAGE_ROOT/logs`)  and therefore belong to sage-the-distribution rather than sagelib. It should not be installed in `$SAGE_LOCAL/bin`.

(It could as well be eliminated and merged into `build/make/deps` (the only place from which it is called); this is where many invocations of `build/bin/sage-logger` happen and this one could as well, for consistency.)

Part of #21559. See also #21510.

CC:  @jdemeyer @kiwifb @jhpalmieri @embray @dimpase

Branch/Commit: **[23ffe1d](https://github.com/sagemath/sagetrac-mirror/commit/23ffe1d9ee6d38980a889b49ad838edeb33b0130)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/23769_





---

archive/issue_events_312319.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2017-08-31T23:44:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312319"
}
```



---

archive/issue_events_312320.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2017-08-31T23:44:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312320"
}
```



---

archive/issue_events_312321.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2017-08-31T23:44:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312321"
}
```



---

archive/issue_events_312322.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2017-08-31T23:44:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312322"
}
```



---

archive/issue_comments_361214.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI agree.  IIUC the only purpose of this file is to prevent make from building some targets if sage didn't build properly.",
    "created_at": "2017-09-01T09:11:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361214",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
I agree.  IIUC the only purpose of this file is to prevent make from building some targets if sage didn't build properly.



---

archive/issue_comments_361215.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nReplying to [@embray](#comment%3A1):\n> IIUC the only purpose of this file is to prevent make from building some targets if sage didn't build properly.\n\nNot quite. I think the purpose is to do a very basic check that Sage actually works. I remember cases in the past where Sage seemed to build fine but doing `./sage` would fail. These days, Sage is also used by the build system, so that is less likely. Still, it is good to leave that basic check in place.",
    "created_at": "2017-09-01T12:29:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361215",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
Replying to [@embray](#comment%3A1):
> IIUC the only purpose of this file is to prevent make from building some targets if sage didn't build properly.

Not quite. I think the purpose is to do a very basic check that Sage actually works. I remember cases in the past where Sage seemed to build fine but doing `./sage` would fail. These days, Sage is also used by the build system, so that is less likely. Still, it is good to leave that basic check in place.



---

archive/issue_comments_361216.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,3 +2,4 @@\n \n (It could as well be eliminated and merged into `build/make/deps` (the only place from which it is called); this is where many invocations of `build/bin/sage-logger` happen and this one could as well, for consistency.)\n \n+See also #21510.\n``````\n",
    "created_at": "2017-09-01T15:56:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361216",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,3 +2,4 @@
 
 (It could as well be eliminated and merged into `build/make/deps` (the only place from which it is called); this is where many invocations of `build/bin/sage-logger` happen and this one could as well, for consistency.)
 
+See also #21510.
``````




---

archive/issue_comments_361217.json:
```json
{
    "body": "Dependencies: **#21559**",
    "created_at": "2020-03-03T00:23:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361217",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#21559**



---

archive/issue_comments_361218.json:
```json
{
    "body": "Branch: **[u/mkoeppe/src_bin_sage_starts_should_be_moved_to_build_bin_or_merged_into_build_make_deps](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/src_bin_sage_starts_should_be_moved_to_build_bin_or_merged_into_build_make_deps)**",
    "created_at": "2020-03-20T20:30:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361218",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/src_bin_sage_starts_should_be_moved_to_build_bin_or_merged_into_build_make_deps](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/src_bin_sage_starts_should_be_moved_to_build_bin_or_merged_into_build_make_deps)**



---

archive/issue_comments_361219.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/23ffe1d9ee6d38980a889b49ad838edeb33b0130\">23ffe1d</a></td><td><code>Move src/bin/sage-starts to build/bin/</code></td></tr></table>\n",
    "created_at": "2020-03-20T20:31:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361219",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'>Comment 6:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/23ffe1d9ee6d38980a889b49ad838edeb33b0130">23ffe1d</a></td><td><code>Move src/bin/sage-starts to build/bin/</code></td></tr></table>




---

archive/issue_events_312323.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-20T20:31:54Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "title_is": "src/bin/sage-starts should be moved to build/bin",
    "title_was": "src/bin/sage-starts should be moved to build/bin or merged into build/make/deps",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312323"
}
```



---

archive/issue_events_312324.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-20T20:31:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312324"
}
```



---

archive/issue_comments_361220.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-03-20T20:31:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361220",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_361221.json:
```json
{
    "body": "Commit: **[23ffe1d](https://github.com/sagemath/sagetrac-mirror/commit/23ffe1d9ee6d38980a889b49ad838edeb33b0130)**",
    "created_at": "2020-03-20T20:31:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361221",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[23ffe1d](https://github.com/sagemath/sagetrac-mirror/commit/23ffe1d9ee6d38980a889b49ad838edeb33b0130)**



---

archive/issue_events_312325.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-20T20:31:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312325"
}
```



---

archive/issue_events_312326.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-20T20:31:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312326"
}
```



---

archive/issue_comments_361222.json:
```json
{
    "body": "Changed dependencies from **#21559** to none",
    "created_at": "2020-03-20T20:32:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361222",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#21559** to none



---

archive/issue_comments_361223.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n (It could as well be eliminated and merged into `build/make/deps` (the only place from which it is called); this is where many invocations of `build/bin/sage-logger` happen and this one could as well, for consistency.)\n \n-See also #21510.\n+Part of #21559. See also #21510.\n``````\n",
    "created_at": "2020-03-20T20:40:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361223",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 (It could as well be eliminated and merged into `build/make/deps` (the only place from which it is called); this is where many invocations of `build/bin/sage-logger` happen and this one could as well, for consistency.)
 
-See also #21510.
+Part of #21559. See also #21510.
``````




---

archive/issue_comments_361224.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nlgtm",
    "created_at": "2020-03-21T16:05:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361224",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:9'>Comment 9:</a>
lgtm



---

archive/issue_events_312327.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-21T16:05:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312327"
}
```



---

archive/issue_events_312328.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-21T16:05:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312328"
}
```



---

archive/issue_comments_361225.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-03-21T16:05:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361225",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_361226.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nThanks!",
    "created_at": "2020-03-21T16:16:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361226",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'>Comment 10:</a>
Thanks!



---

archive/issue_events_312329.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-03-25T22:48:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312329"
}
```



---

archive/issue_events_312330.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "675d301be6d38ce5650e2214201ad92d2ee80557",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-03-25T22:48:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23769#event-312330"
}
```



---

archive/issue_comments_361227.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/src_bin_sage_starts_should_be_moved_to_build_bin_or_merged_into_build_make_deps](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/src_bin_sage_starts_should_be_moved_to_build_bin_or_merged_into_build_make_deps)** to **[23ffe1d](https://github.com/sagemath/sagetrac-mirror/commit/23ffe1d9ee6d38980a889b49ad838edeb33b0130)**",
    "created_at": "2020-03-25T22:48:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23769",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23769#issuecomment-361227",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/src_bin_sage_starts_should_be_moved_to_build_bin_or_merged_into_build_make_deps](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/src_bin_sage_starts_should_be_moved_to_build_bin_or_merged_into_build_make_deps)** to **[23ffe1d](https://github.com/sagemath/sagetrac-mirror/commit/23ffe1d9ee6d38980a889b49ad838edeb33b0130)**
