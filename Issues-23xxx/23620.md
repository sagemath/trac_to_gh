# Issue 23620: gcd has wrong parent

archive/issues_023383.json:
```json
{
    "assignees": [],
    "body": "Currently, the parent of a gcd might be wrong when the gcd of polynomials is just the gcd of its contents:\n\n```\nsage: R.<x> = ZpFM(2)[]\nsage: f = 2*x + 2\nsage: g = 4*x + 2\nsage: f.gcd(g).parent() is R\nFalse # parent is ZpFM(2)\n```\n\nKeywords: **gcd**\n\nBranch/Commit: **[ada3927](https://github.com/sagemath/sagetrac-mirror/commit/ada3927e391c2f2945cc0a6269ac859ab85735ea)**\n\nReviewer: **David Roe**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23620_\n\n",
    "closed_at": "2017-08-17T20:33:21Z",
    "created_at": "2017-08-14T13:50:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "gcd has wrong parent",
    "type": "issue",
    "updated_at": "2017-08-17T20:33:21Z",
    "url": "https://github.com/sagemath/sage/issues/23620",
    "user": "https://github.com/saraedum"
}
```
Currently, the parent of a gcd might be wrong when the gcd of polynomials is just the gcd of its contents:

```
sage: R.<x> = ZpFM(2)[]
sage: f = 2*x + 2
sage: g = 4*x + 2
sage: f.gcd(g).parent() is R
False # parent is ZpFM(2)
```

Keywords: **gcd**

Branch/Commit: **[ada3927](https://github.com/sagemath/sagetrac-mirror/commit/ada3927e391c2f2945cc0a6269ac859ab85735ea)**

Reviewer: **David Roe**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/23620_





---

archive/issue_events_310540.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-14T13:50:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23620#event-310540"
}
```



---

archive/issue_events_310541.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-14T13:50:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23620#event-310541"
}
```



---

archive/issue_events_310542.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-14T13:50:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23620#event-310542"
}
```



---

archive/issue_events_310543.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-14T13:50:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23620#event-310543"
}
```



---

archive/issue_comments_358230.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,4 @@\n+Currently, the parent of a gcd might be wrong when the gcd of polynomials is just the gcd of its contents:\n \n ```\n sage: R.<x> = ZpFM(2)[]\n``````\n",
    "created_at": "2017-08-14T13:52:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23620#issuecomment-358230",
    "user": "https://github.com/saraedum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,4 @@
+Currently, the parent of a gcd might be wrong when the gcd of polynomials is just the gcd of its contents:
 
 ```
 sage: R.<x> = ZpFM(2)[]
``````




---

archive/issue_events_310544.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-14T13:52:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23620#event-310544"
}
```



---

archive/issue_comments_358231.json:
```json
{
    "body": "Branch: **[u/saraedum/gcd_has_wrong_parent](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/gcd_has_wrong_parent)**",
    "created_at": "2017-08-14T14:07:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23620#issuecomment-358231",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/gcd_has_wrong_parent](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/gcd_has_wrong_parent)**



---

archive/issue_comments_358232.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2017-08-15T11:25:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23620#issuecomment-358232",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe**



---

archive/issue_comments_358233.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ada3927e391c2f2945cc0a6269ac859ab85735ea\">ada3927</a></td><td><code>gcd(f,g) has the same parent as f and g</code></td></tr></table>\n",
    "created_at": "2017-08-15T11:25:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23620#issuecomment-358233",
    "user": "https://github.com/roed314"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ada3927e391c2f2945cc0a6269ac859ab85735ea">ada3927</a></td><td><code>gcd(f,g) has the same parent as f and g</code></td></tr></table>




---

archive/issue_events_310545.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-08-15T11:25:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23620#event-310545"
}
```



---

archive/issue_events_310546.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-08-15T11:25:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23620#event-310546"
}
```



---

archive/issue_comments_358234.json:
```json
{
    "body": "Commit: **[ada3927](https://github.com/sagemath/sagetrac-mirror/commit/ada3927e391c2f2945cc0a6269ac859ab85735ea)**",
    "created_at": "2017-08-15T11:25:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23620#issuecomment-358234",
    "user": "https://github.com/roed314"
}
```

Commit: **[ada3927](https://github.com/sagemath/sagetrac-mirror/commit/ada3927e391c2f2945cc0a6269ac859ab85735ea)**



---

archive/issue_comments_358235.json:
```json
{
    "body": "Changed branch from **[u/saraedum/gcd_has_wrong_parent](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/gcd_has_wrong_parent)** to **[ada3927](https://github.com/sagemath/sagetrac-mirror/commit/ada3927e391c2f2945cc0a6269ac859ab85735ea)**",
    "created_at": "2017-08-17T20:33:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23620#issuecomment-358235",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/gcd_has_wrong_parent](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/gcd_has_wrong_parent)** to **[ada3927](https://github.com/sagemath/sagetrac-mirror/commit/ada3927e391c2f2945cc0a6269ac859ab85735ea)**



---

archive/issue_events_310547.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-08-17T20:33:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23620#event-310547"
}
```



---

archive/issue_events_310548.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "689096eed843fc9f656ea2c7cce6c281251d7ba6",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-08-17T20:33:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23620",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23620#event-310548"
}
```
