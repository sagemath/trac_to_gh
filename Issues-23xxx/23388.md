# Issue 23388: faster floor for number field elements

archive/issues_023151.json:
```json
{
    "assignees": [],
    "body": "Before\n\n```\nsage: K.<cbrt2> = NumberField(x^3 - 2, embedding=AA(2)**(1/3))\nsage: %time l = [(m*cbrt2 - n).floor() for m in range(200) for n in range(200)]\nCPU times: user 29 s, sys: 59.3 ms, total: 29.1 s\nWall time: 29 s\n```\nAfter\n\n```\nsage: %time l = [(m*cbrt2 - n).floor() for m in range(200) for n in range(200)]\nCPU times: user 825 ms, sys: 66.9 ms, total: 892 ms\nWall time: 800 ms\n```\n\nBranch/Commit: **[0146c86](https://github.com/sagemath/sagetrac-mirror/commit/0146c864d25d7f5f711d83a6c67a2e352a54ad15)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23388_\n\n",
    "closed_at": "2017-07-26T22:12:19Z",
    "created_at": "2017-07-07T22:21:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "faster floor for number field elements",
    "type": "issue",
    "updated_at": "2017-07-26T22:12:19Z",
    "url": "https://github.com/sagemath/sage/issues/23388",
    "user": "https://github.com/videlec"
}
```
Before

```
sage: K.<cbrt2> = NumberField(x^3 - 2, embedding=AA(2)**(1/3))
sage: %time l = [(m*cbrt2 - n).floor() for m in range(200) for n in range(200)]
CPU times: user 29 s, sys: 59.3 ms, total: 29.1 s
Wall time: 29 s
```
After

```
sage: %time l = [(m*cbrt2 - n).floor() for m in range(200) for n in range(200)]
CPU times: user 825 ms, sys: 66.9 ms, total: 892 ms
Wall time: 800 ms
```

Branch/Commit: **[0146c86](https://github.com/sagemath/sagetrac-mirror/commit/0146c864d25d7f5f711d83a6c67a2e352a54ad15)**

Reviewer: **Travis Scrimshaw**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/23388_





---

archive/issue_events_307567.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-07-07T22:21:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23388#event-307567"
}
```



---

archive/issue_events_307568.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-07-07T22:21:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23388#event-307568"
}
```



---

archive/issue_events_307569.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-07-07T22:21:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23388#event-307569"
}
```



---

archive/issue_events_307570.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-07-07T22:21:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23388#event-307570"
}
```



---

archive/issue_comments_353869.json:
```json
{
    "body": "Branch: **[u/vdelecroix/23388](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/23388)**",
    "created_at": "2017-07-07T22:25:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23388#issuecomment-353869",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/23388](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/23388)**



---

archive/issue_comments_353870.json:
```json
{
    "body": "Commit: **[0146c86](https://github.com/sagemath/sagetrac-mirror/commit/0146c864d25d7f5f711d83a6c67a2e352a54ad15)**",
    "created_at": "2017-07-07T22:25:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23388#issuecomment-353870",
    "user": "https://github.com/videlec"
}
```

Commit: **[0146c86](https://github.com/sagemath/sagetrac-mirror/commit/0146c864d25d7f5f711d83a6c67a2e352a54ad15)**



---

archive/issue_comments_353871.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0146c864d25d7f5f711d83a6c67a2e352a54ad15\">0146c86</a></td><td><code>23388: faster floor for nf element</code></td></tr></table>\n",
    "created_at": "2017-07-07T22:25:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23388#issuecomment-353871",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0146c864d25d7f5f711d83a6c67a2e352a54ad15">0146c86</a></td><td><code>23388: faster floor for nf element</code></td></tr></table>




---

archive/issue_events_307571.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-07-07T22:25:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23388#event-307571"
}
```



---

archive/issue_comments_353872.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-07-14T00:00:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23388#issuecomment-353872",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_307572.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-14T00:00:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23388#event-307572"
}
```



---

archive/issue_events_307573.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-14T00:00:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23388#event-307573"
}
```



---

archive/issue_comments_353873.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2017-07-14T00:00:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23388#issuecomment-353873",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_comments_353874.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks!",
    "created_at": "2017-07-14T13:41:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23388#issuecomment-353874",
    "user": "https://github.com/videlec"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks!



---

archive/issue_events_307574.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-26T22:12:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23388#event-307574"
}
```



---

archive/issue_events_307575.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "138166d2ab491b98ff50281b26f33df7faa66909",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-07-26T22:12:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23388#event-307575"
}
```



---

archive/issue_comments_353875.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/23388](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/23388)** to **[0146c86](https://github.com/sagemath/sagetrac-mirror/commit/0146c864d25d7f5f711d83a6c67a2e352a54ad15)**",
    "created_at": "2017-07-26T22:12:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23388#issuecomment-353875",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/23388](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/23388)** to **[0146c86](https://github.com/sagemath/sagetrac-mirror/commit/0146c864d25d7f5f711d83a6c67a2e352a54ad15)**
