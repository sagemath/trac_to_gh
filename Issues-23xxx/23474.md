# Issue 23474: Make is_galois work for higher degree number fields

archive/issues_023237.json:
```json
{
    "assignees": [],
    "body": "Currently, the method is_galois only works in smaller degrees since it has pari compute the Galois group of the field.  This can be fixed by counting automorphisms of the field instead.\n\nKeywords: **sd87**\n\nBranch/Commit: **[5bff739](https://github.com/sagemath/sagetrac-mirror/commit/5bff739054816859a323b6c99e0e146f9d71d30a)**\n\nReviewer: **Marc Masdeu, David Roe**\n\nAuthor: **John Jones**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23474_\n\n",
    "closed_at": "2017-08-05T23:27:34Z",
    "created_at": "2017-07-19T20:04:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make is_galois work for higher degree number fields",
    "type": "issue",
    "updated_at": "2017-08-05T23:27:34Z",
    "url": "https://github.com/sagemath/sage/issues/23474",
    "user": "https://github.com/sagetrac-jwj"
}
```
Currently, the method is_galois only works in smaller degrees since it has pari compute the Galois group of the field.  This can be fixed by counting automorphisms of the field instead.

Keywords: **sd87**

Branch/Commit: **[5bff739](https://github.com/sagemath/sagetrac-mirror/commit/5bff739054816859a323b6c99e0e146f9d71d30a)**

Reviewer: **Marc Masdeu, David Roe**

Author: **John Jones**

_Issue created by migration from https://trac.sagemath.org/ticket/23474_





---

archive/issue_events_308667.json:
```json
{
    "actor": "https://github.com/sagetrac-jwj",
    "created_at": "2017-07-19T20:04:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308667"
}
```



---

archive/issue_events_308668.json:
```json
{
    "actor": "https://github.com/sagetrac-jwj",
    "created_at": "2017-07-19T20:04:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308668"
}
```



---

archive/issue_events_308669.json:
```json
{
    "actor": "https://github.com/sagetrac-jwj",
    "created_at": "2017-07-19T20:04:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308669"
}
```



---

archive/issue_events_308670.json:
```json
{
    "actor": "https://github.com/sagetrac-jwj",
    "created_at": "2017-07-19T20:04:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308670"
}
```



---

archive/issue_comments_355418.json:
```json
{
    "body": "Branch: **[u/jwj/abelian](https://github.com/sagemath/sagetrac-mirror/tree/u/jwj/abelian)**",
    "created_at": "2017-07-19T20:06:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355418",
    "user": "https://github.com/sagetrac-jwj"
}
```

Branch: **[u/jwj/abelian](https://github.com/sagemath/sagetrac-mirror/tree/u/jwj/abelian)**



---

archive/issue_comments_355419.json:
```json
{
    "body": "Commit: **[3a3a5f1](https://github.com/sagemath/sagetrac-mirror/commit/3a3a5f15639e5bcae109fd8a338a38acfd453368)**",
    "created_at": "2017-07-19T20:08:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355419",
    "user": "https://github.com/sagetrac-jwj"
}
```

Commit: **[3a3a5f1](https://github.com/sagemath/sagetrac-mirror/commit/3a3a5f15639e5bcae109fd8a338a38acfd453368)**



---

archive/issue_comments_355420.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3a3a5f15639e5bcae109fd8a338a38acfd453368\">3a3a5f1</a></td><td><code>Make is_galois for number fields work in higher degrees (beyond</code></td></tr></table>\n",
    "created_at": "2017-07-19T20:08:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355420",
    "user": "https://github.com/sagetrac-jwj"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3a3a5f15639e5bcae109fd8a338a38acfd453368">3a3a5f1</a></td><td><code>Make is_galois for number fields work in higher degrees (beyond</code></td></tr></table>




---

archive/issue_events_308671.json:
```json
{
    "actor": "https://github.com/sagetrac-jwj",
    "created_at": "2017-07-19T20:08:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308671"
}
```



---

archive/issue_comments_355421.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1c8d050818d3e7bb2c5149e82c346ddb20c9ec0e\">1c8d050</a></td><td><code>Use old method for is_galois when the degree is small enough for speed</code></td></tr></table>\n",
    "created_at": "2017-07-19T22:15:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355421",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1c8d050818d3e7bb2c5149e82c346ddb20c9ec0e">1c8d050</a></td><td><code>Use old method for is_galois when the degree is small enough for speed</code></td></tr></table>




---

archive/issue_comments_355422.json:
```json
{
    "body": "Changed commit from **[3a3a5f1](https://github.com/sagemath/sagetrac-mirror/commit/3a3a5f15639e5bcae109fd8a338a38acfd453368)** to **[1c8d050](https://github.com/sagemath/sagetrac-mirror/commit/1c8d050818d3e7bb2c5149e82c346ddb20c9ec0e)**",
    "created_at": "2017-07-19T22:15:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355422",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3a3a5f1](https://github.com/sagemath/sagetrac-mirror/commit/3a3a5f15639e5bcae109fd8a338a38acfd453368)** to **[1c8d050](https://github.com/sagemath/sagetrac-mirror/commit/1c8d050818d3e7bb2c5149e82c346ddb20c9ec0e)**



---

archive/issue_comments_355423.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nTests pass. All the (three) lines of code seem sensible, so positive review.",
    "created_at": "2017-07-20T02:33:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355423",
    "user": "https://github.com/mmasdeu"
}
```

<div id="comment:4" align="right">Comment 4</div>

Tests pass. All the (three) lines of code seem sensible, so positive review.



---

archive/issue_events_308672.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2017-07-20T14:00:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308672"
}
```



---

archive/issue_events_308673.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2017-07-20T14:00:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308673"
}
```



---

archive/issue_comments_355424.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nreviewer name ?",
    "created_at": "2017-07-23T07:46:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355424",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

reviewer name ?



---

archive/issue_comments_355425.json:
```json
{
    "body": "Reviewer: **Marc Masdeu**",
    "created_at": "2017-07-23T15:44:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355425",
    "user": "https://github.com/mmasdeu"
}
```

Reviewer: **Marc Masdeu**



---

archive/issue_events_308674.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-31T20:17:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308674"
}
```



---

archive/issue_events_308675.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-31T20:17:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308675"
}
```



---

archive/issue_comments_355426.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\n\n```\nsage -t --long src/sage/schemes/elliptic_curves/gal_reps.py  # Timed out\n```\nbecause \n\n```\nEllipticCurve([1,-1,0,-107,-379]).galois_representation().image_type(7)\n```\nhangs...",
    "created_at": "2017-07-31T20:17:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355426",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:8" align="right">Comment 8</div>


```
sage -t --long src/sage/schemes/elliptic_curves/gal_reps.py  # Timed out
```
because 

```
EllipticCurve([1,-1,0,-107,-379]).galois_representation().image_type(7)
```
hangs...



---

archive/issue_comments_355427.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5bff739054816859a323b6c99e0e146f9d71d30a\">5bff739</a></td><td><code>Check degree of division polynomial before trying to compute its Galois group.</code></td></tr></table>\n",
    "created_at": "2017-08-02T22:04:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355427",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5bff739054816859a323b6c99e0e146f9d71d30a">5bff739</a></td><td><code>Check degree of division polynomial before trying to compute its Galois group.</code></td></tr></table>




---

archive/issue_comments_355428.json:
```json
{
    "body": "Changed commit from **[1c8d050](https://github.com/sagemath/sagetrac-mirror/commit/1c8d050818d3e7bb2c5149e82c346ddb20c9ec0e)** to **[5bff739](https://github.com/sagemath/sagetrac-mirror/commit/5bff739054816859a323b6c99e0e146f9d71d30a)**",
    "created_at": "2017-08-02T22:04:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355428",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1c8d050](https://github.com/sagemath/sagetrac-mirror/commit/1c8d050818d3e7bb2c5149e82c346ddb20c9ec0e)** to **[5bff739](https://github.com/sagemath/sagetrac-mirror/commit/5bff739054816859a323b6c99e0e146f9d71d30a)**



---

archive/issue_events_308676.json:
```json
{
    "actor": "https://github.com/sagetrac-jwj",
    "created_at": "2017-08-02T22:09:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308676"
}
```



---

archive/issue_events_308677.json:
```json
{
    "actor": "https://github.com/sagetrac-jwj",
    "created_at": "2017-08-02T22:09:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308677"
}
```



---

archive/issue_comments_355429.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nAdded degree check before attempting to compute the Galois group of a division polynomial.  The problem with Galois representations of elliptic curves relied on galois_group() to give up if the degree was greater than 11 (it explicitly used pari).  Now image_type does the check before calling galois_group.",
    "created_at": "2017-08-02T22:09:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355429",
    "user": "https://github.com/sagetrac-jwj"
}
```

<div id="comment:10" align="right">Comment 10</div>

Added degree check before attempting to compute the Galois group of a division polynomial.  The problem with Galois representations of elliptic curves relied on galois_group() to give up if the degree was greater than 11 (it explicitly used pari).  Now image_type does the check before calling galois_group.



---

archive/issue_comments_355430.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nTests pass and it looks good.",
    "created_at": "2017-08-04T06:17:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355430",
    "user": "https://github.com/roed314"
}
```

<div id="comment:11" align="right">Comment 11</div>

Tests pass and it looks good.



---

archive/issue_events_308678.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-08-04T06:17:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308678"
}
```



---

archive/issue_events_308679.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-08-04T06:17:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308679"
}
```



---

archive/issue_comments_355431.json:
```json
{
    "body": "Changed reviewer from **Marc Masdeu** to **Marc Masdeu, David Roe**",
    "created_at": "2017-08-04T06:17:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355431",
    "user": "https://github.com/roed314"
}
```

Changed reviewer from **Marc Masdeu** to **Marc Masdeu, David Roe**



---

archive/issue_events_308680.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-08-05T23:27:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308680"
}
```



---

archive/issue_events_308681.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5f40e0986562e693931d2e778937362b83e03054",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-08-05T23:27:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23474#event-308681"
}
```



---

archive/issue_comments_355432.json:
```json
{
    "body": "Changed branch from **[u/jwj/abelian](https://github.com/sagemath/sagetrac-mirror/tree/u/jwj/abelian)** to **[5bff739](https://github.com/sagemath/sagetrac-mirror/commit/5bff739054816859a323b6c99e0e146f9d71d30a)**",
    "created_at": "2017-08-05T23:27:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23474#issuecomment-355432",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jwj/abelian](https://github.com/sagemath/sagetrac-mirror/tree/u/jwj/abelian)** to **[5bff739](https://github.com/sagemath/sagetrac-mirror/commit/5bff739054816859a323b6c99e0e146f9d71d30a)**
