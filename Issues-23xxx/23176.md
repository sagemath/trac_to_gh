# Issue 23176: Stop using cysignals .pxi files (part 7)

archive/issues_022939.json:
```json
{
    "assignees": [],
    "body": "Follow-up to #22806 and #22896: we should stop using the deprecated files `cysignals/memory.pxi` and `cysignals/signals.pxi`.\n\nGiven the large number of files which need to be changed, this is done in parts.\n\n---\n\nThis was split off from #23127 after some issues were found.\n\nCC:  @tscrim\n\nBranch/Commit: **[3aedda5](https://github.com/sagemath/sagetrac-mirror/commit/3aedda5e574df4be7379674343dddcaf2eb91318)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23176_\n\n",
    "closed_at": "2017-06-12T18:01:58Z",
    "created_at": "2017-06-08T07:53:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Stop using cysignals .pxi files (part 7)",
    "type": "issue",
    "updated_at": "2017-06-12T18:01:58Z",
    "url": "https://github.com/sagemath/sage/issues/23176",
    "user": "https://github.com/jdemeyer"
}
```
Follow-up to #22806 and #22896: we should stop using the deprecated files `cysignals/memory.pxi` and `cysignals/signals.pxi`.

Given the large number of files which need to be changed, this is done in parts.

---

This was split off from #23127 after some issues were found.

CC:  @tscrim

Branch/Commit: **[3aedda5](https://github.com/sagemath/sagetrac-mirror/commit/3aedda5e574df4be7379674343dddcaf2eb91318)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/23176_





---

archive/issue_events_305016.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-08T07:53:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23176#event-305016"
}
```



---

archive/issue_events_305017.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-08T07:53:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23176#event-305017"
}
```



---

archive/issue_events_305018.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-08T07:53:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23176#event-305018"
}
```



---

archive/issue_events_305019.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-08T07:53:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23176#event-305019"
}
```



---

archive/issue_comments_350209.json:
```json
{
    "body": "Branch: **[u/jdemeyer/stop_using_cysignals__pxi_files__part_7_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_cysignals__pxi_files__part_7_)**",
    "created_at": "2017-06-08T07:55:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23176#issuecomment-350209",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/stop_using_cysignals__pxi_files__part_7_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_cysignals__pxi_files__part_7_)**



---

archive/issue_comments_350210.json:
```json
{
    "body": "Commit: **[3aedda5](https://github.com/sagemath/sagetrac-mirror/commit/3aedda5e574df4be7379674343dddcaf2eb91318)**",
    "created_at": "2017-06-08T07:56:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23176#issuecomment-350210",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[3aedda5](https://github.com/sagemath/sagetrac-mirror/commit/3aedda5e574df4be7379674343dddcaf2eb91318)**



---

archive/issue_events_305020.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-08T07:56:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23176#event-305020"
}
```



---

archive/issue_comments_350211.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3aedda5e574df4be7379674343dddcaf2eb91318\">3aedda5</a></td><td><code>Stop using cysignals .pxi files (part 7)</code></td></tr></table>\n",
    "created_at": "2017-06-08T07:56:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23176#issuecomment-350211",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3aedda5e574df4be7379674343dddcaf2eb91318">3aedda5</a></td><td><code>Stop using cysignals .pxi files (part 7)</code></td></tr></table>




---

archive/issue_events_305021.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-08T07:56:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23176#event-305021"
}
```



---

archive/issue_events_305022.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-08T07:56:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23176#event-305022"
}
```



---

archive/issue_comments_350212.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/stop_using_cysignals__pxi_files__part_7_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_cysignals__pxi_files__part_7_)** to **[3aedda5](https://github.com/sagemath/sagetrac-mirror/commit/3aedda5e574df4be7379674343dddcaf2eb91318)**",
    "created_at": "2017-06-12T18:01:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23176#issuecomment-350212",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/stop_using_cysignals__pxi_files__part_7_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_cysignals__pxi_files__part_7_)** to **[3aedda5](https://github.com/sagemath/sagetrac-mirror/commit/3aedda5e574df4be7379674343dddcaf2eb91318)**



---

archive/issue_events_305023.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-12T18:01:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23176#event-305023"
}
```



---

archive/issue_events_305024.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "05d24d6a8f1445b44e3b42afeea80f3bf2d35b10",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-06-12T18:01:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23176",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23176#event-305024"
}
```
