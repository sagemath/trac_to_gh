# Issue 23918: Fix cases where exception is returned

archive/issues_023681.json:
```json
{
    "assignees": [],
    "body": "Functions normally do not *return* exceptions, so `return NotImplementedError` is almost certainly a mistake.\n\nCC:  @tscrim\n\nBranch/Commit: **[fdc151d](https://github.com/sagemath/sagetrac-mirror/commit/fdc151ddd89ce7091aebef56453057c75b0f4cc2)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton, Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23918_\n\n",
    "closed_at": "2017-09-24T13:04:04Z",
    "created_at": "2017-09-22T10:27:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix cases where exception is returned",
    "type": "issue",
    "updated_at": "2017-09-24T13:04:04Z",
    "url": "https://github.com/sagemath/sage/issues/23918",
    "user": "https://github.com/jdemeyer"
}
```
Functions normally do not *return* exceptions, so `return NotImplementedError` is almost certainly a mistake.

CC:  @tscrim

Branch/Commit: **[fdc151d](https://github.com/sagemath/sagetrac-mirror/commit/fdc151ddd89ce7091aebef56453057c75b0f4cc2)**

Reviewer: **Frédéric Chapoton, Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/23918_





---

archive/issue_events_314111.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T10:27:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23918#event-314111"
}
```



---

archive/issue_events_314112.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T10:27:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23918#event-314112"
}
```



---

archive/issue_events_314113.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T10:27:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23918#event-314113"
}
```



---

archive/issue_events_314114.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T10:33:11Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "title_is": "Fix cases where exception is returned",
    "title_was": "Fix cases where exception is returned instead of raised",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23918#event-314114"
}
```



---

archive/issue_comments_363668.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Functions normally do not *return* exceptions, so `return NotImplementedError` is almost certainly a mistake.\n``````\n",
    "created_at": "2017-09-22T10:33:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23918#issuecomment-363668",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Functions normally do not *return* exceptions, so `return NotImplementedError` is almost certainly a mistake.
``````




---

archive/issue_comments_363669.json:
```json
{
    "body": "Branch: **[u/jdemeyer/fix_cases_where_exception_is_returned_instead_of_raised](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_cases_where_exception_is_returned_instead_of_raised)**",
    "created_at": "2017-09-22T10:35:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23918#issuecomment-363669",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/fix_cases_where_exception_is_returned_instead_of_raised](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_cases_where_exception_is_returned_instead_of_raised)**



---

archive/issue_events_314115.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T10:35:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23918#event-314115"
}
```



---

archive/issue_comments_363670.json:
```json
{
    "body": "Commit: **[fdc151d](https://github.com/sagemath/sagetrac-mirror/commit/fdc151ddd89ce7091aebef56453057c75b0f4cc2)**",
    "created_at": "2017-09-22T10:35:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23918#issuecomment-363670",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[fdc151d](https://github.com/sagemath/sagetrac-mirror/commit/fdc151ddd89ce7091aebef56453057c75b0f4cc2)**



---

archive/issue_comments_363671.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fdc151ddd89ce7091aebef56453057c75b0f4cc2\">fdc151d</a></td><td><code>Do not return exceptions by mistake</code></td></tr></table>\n",
    "created_at": "2017-09-22T10:35:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23918#issuecomment-363671",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fdc151ddd89ce7091aebef56453057c75b0f4cc2">fdc151d</a></td><td><code>Do not return exceptions by mistake</code></td></tr></table>




---

archive/issue_comments_363672.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\npositive review if bot gives a green light.",
    "created_at": "2017-09-22T14:28:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23918#issuecomment-363672",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
positive review if bot gives a green light.



---

archive/issue_comments_363673.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLGTM.",
    "created_at": "2017-09-22T18:20:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23918#issuecomment-363673",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
LGTM.



---

archive/issue_events_314116.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-09-22T18:20:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23918#event-314116"
}
```



---

archive/issue_events_314117.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-09-22T18:20:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23918#event-314117"
}
```



---

archive/issue_comments_363674.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-09-22T18:20:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23918#issuecomment-363674",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_363675.json:
```json
{
    "body": "Changed reviewer from **Travis Scrimshaw** to **Fr\u00e9d\u00e9ric Chapoton, Travis Scrimshaw**",
    "created_at": "2017-09-22T18:20:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23918#issuecomment-363675",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Travis Scrimshaw** to **Frédéric Chapoton, Travis Scrimshaw**



---

archive/issue_comments_363676.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/fix_cases_where_exception_is_returned_instead_of_raised](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_cases_where_exception_is_returned_instead_of_raised)** to **[fdc151d](https://github.com/sagemath/sagetrac-mirror/commit/fdc151ddd89ce7091aebef56453057c75b0f4cc2)**",
    "created_at": "2017-09-24T13:04:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23918#issuecomment-363676",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/fix_cases_where_exception_is_returned_instead_of_raised](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_cases_where_exception_is_returned_instead_of_raised)** to **[fdc151d](https://github.com/sagemath/sagetrac-mirror/commit/fdc151ddd89ce7091aebef56453057c75b0f4cc2)**



---

archive/issue_events_314118.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-09-24T13:04:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23918#event-314118"
}
```



---

archive/issue_events_314119.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "90fcfd0f8d53e419f19a14fc442e3be001348fa3",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-09-24T13:04:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23918",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23918#event-314119"
}
```
