# Issue 23121: Stop using cysignals .pxi files (part 4)

archive/issues_022884.json:
```json
{
    "assignees": [],
    "body": "Follow-up to #22806 and #22896: we should stop using the deprecated files `cysignals/memory.pxi` and `cysignals/signals.pxi`.\n\nGiven the large number of files which need to be changed, this is done in parts.\n\nBranch/Commit: **[9caf02f](https://github.com/sagemath/sagetrac-mirror/commit/9caf02f07d17c193b5e79b636e29712ed1d46699)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23121_\n\n",
    "closed_at": "2017-06-07T20:13:12Z",
    "created_at": "2017-06-01T10:14:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Stop using cysignals .pxi files (part 4)",
    "type": "issue",
    "updated_at": "2017-06-07T20:13:12Z",
    "url": "https://github.com/sagemath/sage/issues/23121",
    "user": "https://github.com/jdemeyer"
}
```
Follow-up to #22806 and #22896: we should stop using the deprecated files `cysignals/memory.pxi` and `cysignals/signals.pxi`.

Given the large number of files which need to be changed, this is done in parts.

Branch/Commit: **[9caf02f](https://github.com/sagemath/sagetrac-mirror/commit/9caf02f07d17c193b5e79b636e29712ed1d46699)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/23121_





---

archive/issue_events_304336.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-01T10:14:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23121#event-304336"
}
```



---

archive/issue_events_304337.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-01T10:14:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23121#event-304337"
}
```



---

archive/issue_events_304338.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-01T10:14:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23121#event-304338"
}
```



---

archive/issue_events_304339.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-01T10:14:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23121#event-304339"
}
```



---

archive/issue_comments_349198.json:
```json
{
    "body": "Branch: **[u/jdemeyer/stop_using_cysignals__pxi_files__part_4_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_cysignals__pxi_files__part_4_)**",
    "created_at": "2017-06-01T13:20:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23121#issuecomment-349198",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/stop_using_cysignals__pxi_files__part_4_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_cysignals__pxi_files__part_4_)**



---

archive/issue_comments_349199.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9caf02f07d17c193b5e79b636e29712ed1d46699\">9caf02f</a></td><td><code>Stop using cysignals .pxi files (part 4)</code></td></tr></table>\n",
    "created_at": "2017-06-01T13:20:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23121#issuecomment-349199",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9caf02f07d17c193b5e79b636e29712ed1d46699">9caf02f</a></td><td><code>Stop using cysignals .pxi files (part 4)</code></td></tr></table>




---

archive/issue_comments_349200.json:
```json
{
    "body": "Commit: **[9caf02f](https://github.com/sagemath/sagetrac-mirror/commit/9caf02f07d17c193b5e79b636e29712ed1d46699)**",
    "created_at": "2017-06-01T13:20:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23121#issuecomment-349200",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[9caf02f](https://github.com/sagemath/sagetrac-mirror/commit/9caf02f07d17c193b5e79b636e29712ed1d46699)**



---

archive/issue_events_304340.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-01T13:20:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23121#event-304340"
}
```



---

archive/issue_events_304341.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-04T00:32:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23121#event-304341"
}
```



---

archive/issue_events_304342.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-04T00:32:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23121#event-304342"
}
```



---

archive/issue_comments_349201.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLGTM.",
    "created_at": "2017-06-04T00:32:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23121#issuecomment-349201",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
LGTM.



---

archive/issue_comments_349202.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-06-04T00:32:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23121#issuecomment-349202",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_349203.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/stop_using_cysignals__pxi_files__part_4_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_cysignals__pxi_files__part_4_)** to **[9caf02f](https://github.com/sagemath/sagetrac-mirror/commit/9caf02f07d17c193b5e79b636e29712ed1d46699)**",
    "created_at": "2017-06-07T20:13:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23121#issuecomment-349203",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/stop_using_cysignals__pxi_files__part_4_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_cysignals__pxi_files__part_4_)** to **[9caf02f](https://github.com/sagemath/sagetrac-mirror/commit/9caf02f07d17c193b5e79b636e29712ed1d46699)**



---

archive/issue_events_304343.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-07T20:13:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23121#event-304343"
}
```



---

archive/issue_events_304344.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7a39f63583f80c4ce6862daadecf94176e54bbb7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-06-07T20:13:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23121",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23121#event-304344"
}
```
