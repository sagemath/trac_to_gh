# Issue 23300: non-absolute import in BasisExchangeMatroid

archive/issues_023063.json:
```json
{
    "assignees": [],
    "body": "The following should not give an error:\n\n```\nsage: N = Matroid(field=GF(3), reduced_matrix=[[1,1],[1,-1]])\nsage: M = matroids.Uniform(2,4)\nsage: N._isomorphism(M)\n```\nThe problem is a non-absolute import statement.\n\nCC:  Rudi yomcat zgershkoff\n\nKeywords: **import, isomorphism, matroids**\n\nBranch: **[b443da7](https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d)**\n\nReviewer: **Zach Gershkoff, Travis Scrimshaw**\n\nAuthor: **Stefan van Zwam**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23300_\n\n",
    "closed_at": "2017-07-26T22:13:11Z",
    "created_at": "2017-06-21T16:59:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "non-absolute import in BasisExchangeMatroid",
    "type": "issue",
    "updated_at": "2017-10-24T15:09:14Z",
    "url": "https://github.com/sagemath/sage/issues/23300",
    "user": "https://github.com/sagetrac-Stefan"
}
```
The following should not give an error:

```
sage: N = Matroid(field=GF(3), reduced_matrix=[[1,1],[1,-1]])
sage: M = matroids.Uniform(2,4)
sage: N._isomorphism(M)
```
The problem is a non-absolute import statement.

CC:  Rudi yomcat zgershkoff

Keywords: **import, isomorphism, matroids**

Branch: **[b443da7](https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d)**

Reviewer: **Zach Gershkoff, Travis Scrimshaw**

Author: **Stefan van Zwam**

_Issue created by migration from https://trac.sagemath.org/ticket/23300_





---

archive/issue_events_306434.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-06-21T16:59:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306434"
}
```



---

archive/issue_events_306435.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-06-21T16:59:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
    "label_color": "0000ff",
    "label_name": "component: matroid theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306435"
}
```



---

archive/issue_events_306436.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-06-21T16:59:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306436"
}
```



---

archive/issue_events_306437.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-06-21T16:59:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306437"
}
```



---

archive/issue_comments_352259.json:
```json
{
    "body": "Branch: **[u/Stefan/non_absolute_import_in_basisexchangematroid](https://github.com/sagemath/sagetrac-mirror/tree/u/Stefan/non_absolute_import_in_basisexchangematroid)**",
    "created_at": "2017-06-21T17:23:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352259",
    "user": "https://github.com/sagetrac-Stefan"
}
```

Branch: **[u/Stefan/non_absolute_import_in_basisexchangematroid](https://github.com/sagemath/sagetrac-mirror/tree/u/Stefan/non_absolute_import_in_basisexchangematroid)**



---

archive/issue_comments_352260.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nFixed this. Turns out another instance occurred in the _is_isomorphism method in the same file, which I also fixed.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/221bf3025abab9508edaf3afebfd211ab814ee5f\">221bf30</a></td><td><code>fixed import statements and added tests</code></td></tr></table>\n",
    "created_at": "2017-06-21T17:24:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352260",
    "user": "https://github.com/sagetrac-Stefan"
}
```

<a id='comment:2'>Comment 2:</a>
Fixed this. Turns out another instance occurred in the _is_isomorphism method in the same file, which I also fixed.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/221bf3025abab9508edaf3afebfd211ab814ee5f">221bf30</a></td><td><code>fixed import statements and added tests</code></td></tr></table>




---

archive/issue_comments_352261.json:
```json
{
    "body": "Commit: **[221bf30](https://github.com/sagemath/sagetrac-mirror/commit/221bf3025abab9508edaf3afebfd211ab814ee5f)**",
    "created_at": "2017-06-21T17:24:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352261",
    "user": "https://github.com/sagetrac-Stefan"
}
```

Commit: **[221bf30](https://github.com/sagemath/sagetrac-mirror/commit/221bf3025abab9508edaf3afebfd211ab814ee5f)**



---

archive/issue_comments_352262.json:
```json
{
    "body": "Author: **Stefan van Zwam**",
    "created_at": "2017-06-21T17:24:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352262",
    "user": "https://github.com/sagetrac-Stefan"
}
```

Author: **Stefan van Zwam**



---

archive/issue_events_306438.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-06-21T17:24:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306438"
}
```



---

archive/issue_events_306439.json:
```json
{
    "actor": "https://github.com/sagetrac-zgershkoff",
    "created_at": "2017-06-22T19:28:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306439"
}
```



---

archive/issue_events_306440.json:
```json
{
    "actor": "https://github.com/sagetrac-zgershkoff",
    "created_at": "2017-06-22T19:28:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306440"
}
```



---

archive/issue_comments_352263.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThe methods work without errors when I compare basis exchange matroids to rank matroids, circuit closure matroids, and prototype graphic matroids.",
    "created_at": "2017-06-22T19:28:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352263",
    "user": "https://github.com/sagetrac-zgershkoff"
}
```

<a id='comment:3'>Comment 3:</a>
The methods work without errors when I compare basis exchange matroids to rank matroids, circuit closure matroids, and prototype graphic matroids.



---

archive/issue_comments_352264.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nReviewer name.\n\nHowever, I would say a better (well, at least cleaner IMO) idiom is to just import what you need\n\n```diff\n-            from . import basis_matroid\n-            ot = basis_matroid.BasisMatroid(other)\n+            from .basis_matroid import BasisMatroid\n+            ot = BasisMatroid(other)\n```",
    "created_at": "2017-06-22T23:39:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352264",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Reviewer name.

However, I would say a better (well, at least cleaner IMO) idiom is to just import what you need

```diff
-            from . import basis_matroid
-            ot = basis_matroid.BasisMatroid(other)
+            from .basis_matroid import BasisMatroid
+            ot = BasisMatroid(other)
```



---

archive/issue_events_306441.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-22T23:39:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306441"
}
```



---

archive/issue_events_306442.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-22T23:39:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306442"
}
```



---

archive/issue_comments_352265.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d\">b443da7</a></td><td><code>modified imports according to trac comment</code></td></tr></table>\n",
    "created_at": "2017-06-28T18:34:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352265",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d">b443da7</a></td><td><code>modified imports according to trac comment</code></td></tr></table>




---

archive/issue_comments_352266.json:
```json
{
    "body": "Changed commit from **[221bf30](https://github.com/sagemath/sagetrac-mirror/commit/221bf3025abab9508edaf3afebfd211ab814ee5f)** to **[b443da7](https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d)**",
    "created_at": "2017-06-28T18:34:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352266",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[221bf30](https://github.com/sagemath/sagetrac-mirror/commit/221bf3025abab9508edaf3afebfd211ab814ee5f)** to **[b443da7](https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d)**



---

archive/issue_events_306443.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-06-28T18:34:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306443"
}
```



---

archive/issue_events_306444.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-06-28T18:34:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306444"
}
```



---

archive/issue_comments_352267.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nFollowed Travis' suggestion",
    "created_at": "2017-06-28T18:34:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352267",
    "user": "https://github.com/sagetrac-Stefan"
}
```

<a id='comment:6'>Comment 6:</a>
Followed Travis' suggestion



---

archive/issue_events_306445.json:
```json
{
    "actor": "https://github.com/sagetrac-zgershkoff",
    "created_at": "2017-06-29T23:54:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306445"
}
```



---

archive/issue_events_306446.json:
```json
{
    "actor": "https://github.com/sagetrac-zgershkoff",
    "created_at": "2017-06-29T23:54:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306446"
}
```



---

archive/issue_comments_352268.json:
```json
{
    "body": "Reviewer: **Zachary Gershkoff, Travis Scrimshaw**",
    "created_at": "2017-06-29T23:54:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352268",
    "user": "https://github.com/sagetrac-zgershkoff"
}
```

Reviewer: **Zachary Gershkoff, Travis Scrimshaw**



---

archive/issue_comments_352269.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nStill works.",
    "created_at": "2017-06-29T23:54:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352269",
    "user": "https://github.com/sagetrac-zgershkoff"
}
```

<a id='comment:7'>Comment 7:</a>
Still works.



---

archive/issue_comments_352270.json:
```json
{
    "body": "Changed branch from **[u/Stefan/non_absolute_import_in_basisexchangematroid](https://github.com/sagemath/sagetrac-mirror/tree/u/Stefan/non_absolute_import_in_basisexchangematroid)** to **[b443da7](https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d)**",
    "created_at": "2017-07-26T22:13:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352270",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/Stefan/non_absolute_import_in_basisexchangematroid](https://github.com/sagemath/sagetrac-mirror/tree/u/Stefan/non_absolute_import_in_basisexchangematroid)** to **[b443da7](https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d)**



---

archive/issue_events_306447.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-26T22:13:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306447"
}
```



---

archive/issue_events_306448.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9f435bc886b5e405a356d076d8bc8440b59c1594",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-07-26T22:13:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23300#event-306448"
}
```



---

archive/issue_comments_352271.json:
```json
{
    "body": "Changed commit from **[b443da7](https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d)** to none",
    "created_at": "2017-10-24T15:09:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352271",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[b443da7](https://github.com/sagemath/sagetrac-mirror/commit/b443da71a73159fa2cf364f2f0da54b4000ccb6d)** to none



---

archive/issue_comments_352272.json:
```json
{
    "body": "Changed reviewer from **Zachary Gershkoff, Travis Scrimshaw** to **Zach Gershkoff, Travis Scrimshaw**",
    "created_at": "2017-10-24T15:09:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23300#issuecomment-352272",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Zachary Gershkoff, Travis Scrimshaw** to **Zach Gershkoff, Travis Scrimshaw**
