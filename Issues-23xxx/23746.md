# Issue 23746: Performance regression in span() for vectors over GF(2)

archive/issues_023509.json:
```json
{
    "body": "Calculating the span of a large number of vectors in a high-dimension vector space over GF(2) has gotten a lot slower in recent versions of sage. This can be demonstrated by the following code:\n\n```\nimport time\n\nset_random_seed(0)\nn = 1000\nv = [ random_vector(GF(2), n) for i in xrange(n) ]\n\nc1 = time.clock()\nspan(v, GF(2))\nprint time.clock()-c1\n```\n\nIn Sage 7.4, this took around ~0.7s (on my laptop), whereas in Sage 8.0, it takes ~12s. This performance regression was introduced by #21746. The reason is that the new logic in `Vector_mod2_dense.__init__()` ends up calculating the modulus of an IntegerMod_int object rather than a native C int, which turns out to be a lot slower. Profiling the above code reveals that in Sage 8.0 it indeed spends most of its time in `IntegerMod_int.__mod__`.\n\nCC:  simonking @jdemeyer\n\nKeywords: span, Vector_mod2_dense, IntegerMod_int\n\nBranch: u/dominic.else/performance_regression_in_span___for_vectors_over_gf_2_\n\nStatus: new\n\nCommit: 403ece1d71d97f4eaf2c5ae2ff948c3aa31d1d79\n\nIssue created by migration from https://trac.sagemath.org/ticket/23746\n\n",
    "created_at": "2017-08-29T18:39:02Z",
    "labels": [
        "component: performance",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.1",
    "title": "Performance regression in span() for vectors over GF(2)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/23746",
    "user": "https://trac.sagemath.org/admin/accounts/users/dominic.else"
}
```
Calculating the span of a large number of vectors in a high-dimension vector space over GF(2) has gotten a lot slower in recent versions of sage. This can be demonstrated by the following code:

```
import time

set_random_seed(0)
n = 1000
v = [ random_vector(GF(2), n) for i in xrange(n) ]

c1 = time.clock()
span(v, GF(2))
print time.clock()-c1
```

In Sage 7.4, this took around ~0.7s (on my laptop), whereas in Sage 8.0, it takes ~12s. This performance regression was introduced by #21746. The reason is that the new logic in `Vector_mod2_dense.__init__()` ends up calculating the modulus of an IntegerMod_int object rather than a native C int, which turns out to be a lot slower. Profiling the above code reveals that in Sage 8.0 it indeed spends most of its time in `IntegerMod_int.__mod__`.

CC:  simonking @jdemeyer

Keywords: span, Vector_mod2_dense, IntegerMod_int

Branch: u/dominic.else/performance_regression_in_span___for_vectors_over_gf_2_

Status: new

Commit: 403ece1d71d97f4eaf2c5ae2ff948c3aa31d1d79

Issue created by migration from https://trac.sagemath.org/ticket/23746





---

archive/issue_comments_448060.json:
```json
{
    "body": "Changing branch from \"\" to \"u/dominic.else/performance_regression_in_span___for_vectors_over_gf_2_\"",
    "created_at": "2017-08-29T18:54:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23746",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23746#issuecomment-448060",
    "user": "https://trac.sagemath.org/admin/accounts/users/dominic.else"
}
```

Changing branch from "" to "u/dominic.else/performance_regression_in_span___for_vectors_over_gf_2_"



---

archive/issue_comments_448061.json:
```json
{
    "body": "Changing commit from \"\" to \"403ece1d71d97f4eaf2c5ae2ff948c3aa31d1d79\"",
    "created_at": "2017-08-29T18:56:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23746",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23746#issuecomment-448061",
    "user": "https://trac.sagemath.org/admin/accounts/users/dominic.else"
}
```

Changing commit from "" to "403ece1d71d97f4eaf2c5ae2ff948c3aa31d1d79"



---

archive/issue_comments_448062.json:
```json
{
    "body": "<a id='comment:2'></a>The fix I uploaded sidesteps the problem by converting an IntegerMod_int object to a Python int first before taking the modulus, restoring the original performance. A better solution might be to make `IntegerMod_int.__mod__()` faster.\n\n---\nNew commits:",
    "created_at": "2017-08-29T18:56:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23746",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23746#issuecomment-448062",
    "user": "https://trac.sagemath.org/admin/accounts/users/dominic.else"
}
```

<a id='comment:2'></a>The fix I uploaded sidesteps the problem by converting an IntegerMod_int object to a Python int first before taking the modulus, restoring the original performance. A better solution might be to make `IntegerMod_int.__mod__()` faster.

---
New commits:



---

archive/issue_comments_448063.json:
```json
{
    "body": "Changing keywords from \"span, Vector_mod2_dense\" to \"span, Vector_mod2_dense, IntegerMod_int\".",
    "created_at": "2017-08-29T19:02:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23746",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23746#issuecomment-448063",
    "user": "https://trac.sagemath.org/admin/accounts/users/dominic.else"
}
```

Changing keywords from "span, Vector_mod2_dense" to "span, Vector_mod2_dense, IntegerMod_int".



---

archive/issue_comments_448064.json:
```json
{
    "body": "<a id='comment:5'></a>\n```python\n    def __mod__(self, modulus):\n        if not isinstance(self, IntegerMod_abstract):\n            # something % Mod(x,y) makes no sense\n            return NotImplemented\n        from .integer_mod_ring import IntegerModRing\n        R = IntegerModRing(modulus)\n        if (<Element>self)._parent._IntegerModRing_generic__order % R.order():\n            raise ArithmeticError(f\"reduction modulo {modulus!r} not defined\")\n        return R(self)\n```\nThe import should not be done inside of the method. Is `R.order()` not the same as `modulus`? But in any case, I do think that using the modulus of a native C int should be used.",
    "created_at": "2017-08-30T09:05:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23746",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23746#issuecomment-448064",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:5'></a>
```python
    def __mod__(self, modulus):
        if not isinstance(self, IntegerMod_abstract):
            # something % Mod(x,y) makes no sense
            return NotImplemented
        from .integer_mod_ring import IntegerModRing
        R = IntegerModRing(modulus)
        if (<Element>self)._parent._IntegerModRing_generic__order % R.order():
            raise ArithmeticError(f"reduction modulo {modulus!r} not defined")
        return R(self)
```
The import should not be done inside of the method. Is `R.order()` not the same as `modulus`? But in any case, I do think that using the modulus of a native C int should be used.



---

archive/issue_comments_448065.json:
```json
{
    "body": "<a id='comment:6'></a>Note that the semantic is different:\n\n```\nsage: x = GF(3)(2)\nsage: type(x)\n<type 'sage.rings.finite_rings.integer_mod.IntegerMod_int'>\nsage: x\n2\nsage: x%2\n---------------------------------------------------------------------------\nArithmeticError                           Traceback (most recent call last)\n<ipython-input-4-d92b2f50ba44> in <module>()\n----> 1 x%Integer(2)\n\n/home/king/Sage/git/sage/src/sage/rings/finite_rings/integer_mod.pyx in sage.rings.finite_rings.integer_mod.IntegerMod_abstract.__mod__ (build/cythonized/sage/rings/finite_rings/integer_mod.c:6386)()\n    392         R = IntegerModRing(modulus)\n    393         if (<Element>self)._parent._IntegerModRing_generic__order % R.order():\n--> 394             raise ArithmeticError(f\"reduction modulo {modulus!r} not defined\")\n    395         return R(self)\n    396 \n\nArithmeticError: reduction modulo 2 not defined\nsage: int(x)%2\n0\n```\nNote that the error does not depend on the specific element x, but only depends on the parent of x. Is it guaranteed in the vector code that all elements belong to the same parent? If they do, then the check on the parent should of course be done only once per vector, not once per element.",
    "created_at": "2017-08-30T09:12:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23746",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23746#issuecomment-448065",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:6'></a>Note that the semantic is different:

```
sage: x = GF(3)(2)
sage: type(x)
<type 'sage.rings.finite_rings.integer_mod.IntegerMod_int'>
sage: x
2
sage: x%2
---------------------------------------------------------------------------
ArithmeticError                           Traceback (most recent call last)
<ipython-input-4-d92b2f50ba44> in <module>()
----> 1 x%Integer(2)

/home/king/Sage/git/sage/src/sage/rings/finite_rings/integer_mod.pyx in sage.rings.finite_rings.integer_mod.IntegerMod_abstract.__mod__ (build/cythonized/sage/rings/finite_rings/integer_mod.c:6386)()
    392         R = IntegerModRing(modulus)
    393         if (<Element>self)._parent._IntegerModRing_generic__order % R.order():
--> 394             raise ArithmeticError(f"reduction modulo {modulus!r} not defined")
    395         return R(self)
    396 

ArithmeticError: reduction modulo 2 not defined
sage: int(x)%2
0
```
Note that the error does not depend on the specific element x, but only depends on the parent of x. Is it guaranteed in the vector code that all elements belong to the same parent? If they do, then the check on the parent should of course be done only once per vector, not once per element.



---

archive/issue_comments_448066.json:
```json
{
    "body": "<a id='comment:7'></a>They should all belong to the same parent.",
    "created_at": "2017-08-30T13:48:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23746",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23746#issuecomment-448066",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:7'></a>They should all belong to the same parent.
