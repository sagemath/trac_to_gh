# Issue 23591: py3: more absolute imports in pyx files

archive/issues_023354.json:
```json
{
    "assignees": [],
    "body": "sequel of #23589, part of part #22808\n\nchosen partially with the help of\n\ngrep -L \"absolute_import\" $(git grep -l \"^import [^s\\.]\" *.pyx)\n\nBranch/Commit: **[9bcc9ec](https://github.com/sagemath/sagetrac-mirror/commit/9bcc9ec5f495b65f557b82cbfe033835f2d8b309)**\n\nReviewer: **John Palmieri**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23591_\n\n",
    "closed_at": "2017-08-11T18:17:35Z",
    "created_at": "2017-08-07T20:11:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: more absolute imports in pyx files",
    "type": "issue",
    "updated_at": "2017-08-11T18:17:35Z",
    "url": "https://github.com/sagemath/sage/issues/23591",
    "user": "https://github.com/fchapoton"
}
```
sequel of #23589, part of part #22808

chosen partially with the help of

grep -L "absolute_import" $(git grep -l "^import [^s\.]" *.pyx)

Branch/Commit: **[9bcc9ec](https://github.com/sagemath/sagetrac-mirror/commit/9bcc9ec5f495b65f557b82cbfe033835f2d8b309)**

Reviewer: **John Palmieri**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/23591_





---

archive/issue_events_310185.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-07T20:11:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23591#event-310185"
}
```



---

archive/issue_events_310186.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-07T20:11:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23591#event-310186"
}
```



---

archive/issue_events_310187.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-07T20:11:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23591#event-310187"
}
```



---

archive/issue_events_310188.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-07T20:11:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23591#event-310188"
}
```



---

archive/issue_events_310189.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-07T20:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23591#event-310189"
}
```



---

archive/issue_comments_357738.json:
```json
{
    "body": "Commit: **[9bcc9ec](https://github.com/sagemath/sagetrac-mirror/commit/9bcc9ec5f495b65f557b82cbfe033835f2d8b309)**",
    "created_at": "2017-08-07T20:11:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23591#issuecomment-357738",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[9bcc9ec](https://github.com/sagemath/sagetrac-mirror/commit/9bcc9ec5f495b65f557b82cbfe033835f2d8b309)**



---

archive/issue_comments_357739.json:
```json
{
    "body": "Branch: **[u/chapoton/23591](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/23591)**",
    "created_at": "2017-08-07T20:11:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23591#issuecomment-357739",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/23591](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/23591)**



---

archive/issue_comments_357740.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9bcc9ec5f495b65f557b82cbfe033835f2d8b309\">9bcc9ec</a></td><td><code>more absolute_import in pyx files</code></td></tr></table>\n",
    "created_at": "2017-08-07T20:11:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23591#issuecomment-357740",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9bcc9ec5f495b65f557b82cbfe033835f2d8b309">9bcc9ec</a></td><td><code>more absolute_import in pyx files</code></td></tr></table>




---

archive/issue_comments_357741.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks good to me. If all tests pass, positive review.",
    "created_at": "2017-08-07T20:26:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23591#issuecomment-357741",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'>Comment 2:</a>
Looks good to me. If all tests pass, positive review.



---

archive/issue_comments_357742.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2017-08-07T20:26:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23591#issuecomment-357742",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_events_310190.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-08T06:13:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23591#event-310190"
}
```



---

archive/issue_events_310191.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-08T06:13:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23591#event-310191"
}
```



---

archive/issue_comments_357743.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThanks. As patchbot is morally green, I am setting to positive.",
    "created_at": "2017-08-08T06:13:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23591#issuecomment-357743",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
Thanks. As patchbot is morally green, I am setting to positive.



---

archive/issue_comments_357744.json:
```json
{
    "body": "Changed branch from **[u/chapoton/23591](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/23591)** to **[9bcc9ec](https://github.com/sagemath/sagetrac-mirror/commit/9bcc9ec5f495b65f557b82cbfe033835f2d8b309)**",
    "created_at": "2017-08-11T18:17:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23591#issuecomment-357744",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/23591](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/23591)** to **[9bcc9ec](https://github.com/sagemath/sagetrac-mirror/commit/9bcc9ec5f495b65f557b82cbfe033835f2d8b309)**



---

archive/issue_events_310192.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-08-11T18:17:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23591#event-310192"
}
```



---

archive/issue_events_310193.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f8edd55e536741c6022a59b5b5c753a08b01b2ba",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-08-11T18:17:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23591",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23591#event-310193"
}
```
