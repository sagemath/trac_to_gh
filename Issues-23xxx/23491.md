# Issue 23491: Don't use MethodType to assign methods to a class

archive/issues_023254.json:
```json
{
    "body": "Unbound methods have been removed in Py3 ; see\n\nhttp://python-future.org/what_else.html#binding-a-method-to-a-class\n\nSee also:\n\nhttps://pythonhosted.org/six/#six.create_bound_method\n\nThis also causes problems in pyx files. For example\n\n```\n/home/chapoton/sage3/src/sage/rings/padics/qadic_flint_CR.pyx in init sage.rings.padics.qadic_flint_CR (/home/chapoton/sage3/src/build/cythonized/sage/rings/padics/qadic_flint_CR.c:38543)()\n     12         \"\"\"\n     13         Initialization.\n     14 \n     15         EXAMPLES::\n     16 \n     17             sage: R.<a> = ZqCR(125)\n     18             sage: type(R.prime_pow)\n     19             <type 'sage.rings.padics.qadic_flint_CR.PowComputer_'>\n     20             sage: R.prime_pow._prec_type\n     21             'capped-rel'\n     22         \"\"\"\n     23         self._prec_type = 'capped-rel'\n     24         PowComputer_flint_unram.__init__(self, prime, cache_limit, prec_cap, ram_prec_cap, in_field, poly)\n     25 \n     26 cdef class qAdicCappedRelativeElement(CRElement):\n---> 27     frobenius = MethodType(frobenius_unram, None, qAdicCappedRelativeElement)\n        global frobenius = undefined\n        global MethodType = <class 'method'>\n        global frobenius_unram = <built-in function frobenius_unram>\n        global qAdicCappedRelativeElement = <class 'sage.rings.padics.qadic_flint_CR.qAdicCappedRelativeElement'>\n     28     trace = MethodType(trace_unram, None, qAdicCappedRelativeElement)\n     29     norm = MethodType(norm_unram, None, qAdicCappedRelativeElement)\n     30 \n     31     def matrix_mod_pn(self):\n     32         \"\"\"\n     33         Returns the matrix of right multiplication by the element on\n     34         the power basis `1, x, x^2, \\ldots, x^{d-1}` for this\n     35         extension field.  Thus the *rows* of this matrix give the\n     36         images of each of the `x^i`.  The entries of the matrices are\n     37         IntegerMod elements, defined modulo ``p^(self.absprec() / e)``.\n     38 \n     39         Raises an error if ``self`` has negative valuation.\n     40 \n     41         EXAMPLES::\n     42 \n\nTypeError: method expected 2 arguments, got 3\n\n```\n\nCC:  @jdemeyer\n\nBranch/Commit: a2035749bd093d6b27a303bbf3e11f6eabfc23c3\n\nReviewer: Fr\u00e9d\u00e9ric Chapoton\n\nAuthor: Jeroen Demeyer\n\nDependencies: #23360, #23494\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/23491\n\n",
    "closed_at": "2017-08-11T18:17:53Z",
    "created_at": "2017-07-20T10:10:10Z",
    "labels": [
        "component: python3",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.1",
    "title": "Don't use MethodType to assign methods to a class",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/23491",
    "user": "https://github.com/fchapoton"
}
```
Unbound methods have been removed in Py3 ; see

http://python-future.org/what_else.html#binding-a-method-to-a-class

See also:

https://pythonhosted.org/six/#six.create_bound_method

This also causes problems in pyx files. For example

```
/home/chapoton/sage3/src/sage/rings/padics/qadic_flint_CR.pyx in init sage.rings.padics.qadic_flint_CR (/home/chapoton/sage3/src/build/cythonized/sage/rings/padics/qadic_flint_CR.c:38543)()
     12         """
     13         Initialization.
     14 
     15         EXAMPLES::
     16 
     17             sage: R.<a> = ZqCR(125)
     18             sage: type(R.prime_pow)
     19             <type 'sage.rings.padics.qadic_flint_CR.PowComputer_'>
     20             sage: R.prime_pow._prec_type
     21             'capped-rel'
     22         """
     23         self._prec_type = 'capped-rel'
     24         PowComputer_flint_unram.__init__(self, prime, cache_limit, prec_cap, ram_prec_cap, in_field, poly)
     25 
     26 cdef class qAdicCappedRelativeElement(CRElement):
---> 27     frobenius = MethodType(frobenius_unram, None, qAdicCappedRelativeElement)
        global frobenius = undefined
        global MethodType = <class 'method'>
        global frobenius_unram = <built-in function frobenius_unram>
        global qAdicCappedRelativeElement = <class 'sage.rings.padics.qadic_flint_CR.qAdicCappedRelativeElement'>
     28     trace = MethodType(trace_unram, None, qAdicCappedRelativeElement)
     29     norm = MethodType(norm_unram, None, qAdicCappedRelativeElement)
     30 
     31     def matrix_mod_pn(self):
     32         """
     33         Returns the matrix of right multiplication by the element on
     34         the power basis `1, x, x^2, \ldots, x^{d-1}` for this
     35         extension field.  Thus the *rows* of this matrix give the
     36         images of each of the `x^i`.  The entries of the matrices are
     37         IntegerMod elements, defined modulo ``p^(self.absprec() / e)``.
     38 
     39         Raises an error if ``self`` has negative valuation.
     40 
     41         EXAMPLES::
     42 

TypeError: method expected 2 arguments, got 3

```

CC:  @jdemeyer

Branch/Commit: a2035749bd093d6b27a303bbf3e11f6eabfc23c3

Reviewer: Frédéric Chapoton

Author: Jeroen Demeyer

Dependencies: #23360, #23494

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/23491





---

archive/issue_comments_441931.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to defect.",
    "created_at": "2017-07-20T11:30:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441931",
    "user": "https://github.com/jdemeyer"
}
```

Changing type from PLEASE CHANGE to defect.



---

archive/issue_comments_441932.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to python3.",
    "created_at": "2017-07-20T11:30:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441932",
    "user": "https://github.com/jdemeyer"
}
```

Changing component from PLEASE CHANGE to python3.



---

archive/issue_comments_441933.json:
```json
{
    "body": "Changing author from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2017-07-20T11:30:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441933",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "" to "Jeroen Demeyer"



---

archive/issue_comments_441934.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,10 @@\n Unbound methods have been removed in Py3 ; see\n \n http://python-future.org/what_else.html#binding-a-method-to-a-class\n+\n+See also:\n+\n+https://pythonhosted.org/six/#six.create_bound_method\n \n This also causes problems in pyx files. For example\n \n``````\n",
    "created_at": "2017-07-20T12:10:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441934",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,10 @@
 Unbound methods have been removed in Py3 ; see
 
 http://python-future.org/what_else.html#binding-a-method-to-a-class
+
+See also:
+
+https://pythonhosted.org/six/#six.create_bound_method
 
 This also causes problems in pyx files. For example
 
``````




---

archive/issue_comments_441935.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#23360\"",
    "created_at": "2017-07-20T12:19:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441935",
    "user": "https://github.com/jdemeyer"
}
```

Changing dependencies from "" to "#23360"



---

archive/issue_comments_441936.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jdemeyer/py3__change_of_behaviour_of_bound___unbound_methods_and_methodtype\"",
    "created_at": "2017-07-20T12:22:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441936",
    "user": "https://github.com/jdemeyer"
}
```

Changing branch from "" to "u/jdemeyer/py3__change_of_behaviour_of_bound___unbound_methods_and_methodtype"



---

archive/issue_comments_441937.json:
```json
{
    "body": "<a id='comment:5'></a>New commits:",
    "created_at": "2017-07-20T12:28:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441937",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'></a>New commits:



---

archive/issue_comments_441938.json:
```json
{
    "body": "Changing commit from \"\" to \"0c588a7b7e581595052c86ae6e68aac150318d33\"",
    "created_at": "2017-07-20T12:28:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441938",
    "user": "https://github.com/jdemeyer"
}
```

Changing commit from "" to "0c588a7b7e581595052c86ae6e68aac150318d33"



---

archive/issue_comments_441939.json:
```json
{
    "body": "Changing dependencies from \"#23360\" to \"#23360, #23494\"",
    "created_at": "2017-07-20T12:28:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441939",
    "user": "https://github.com/jdemeyer"
}
```

Changing dependencies from "#23360" to "#23360, #23494"



---

archive/issue_comments_441940.json:
```json
{
    "body": "<a id='comment:6'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:",
    "created_at": "2017-07-20T13:45:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441940",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:



---

archive/issue_comments_441941.json:
```json
{
    "body": "Changing commit from \"0c588a7b7e581595052c86ae6e68aac150318d33\" to \"44084cdcbca46c40c989c7a49828f2e149328e66\"",
    "created_at": "2017-07-20T13:45:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441941",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "0c588a7b7e581595052c86ae6e68aac150318d33" to "44084cdcbca46c40c989c7a49828f2e149328e66"



---

archive/issue_comments_441942.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-07-20T13:47:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441942",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_441943.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2017-07-31T16:11:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441943",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_441944.json:
```json
{
    "body": "<a id='comment:8'></a>does not apply",
    "created_at": "2017-07-31T16:11:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441944",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'></a>does not apply



---

archive/issue_comments_441945.json:
```json
{
    "body": "<a id='comment:9'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-08-01T11:13:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441945",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:9'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_441946.json:
```json
{
    "body": "Changing commit from \"44084cdcbca46c40c989c7a49828f2e149328e66\" to \"a2035749bd093d6b27a303bbf3e11f6eabfc23c3\"",
    "created_at": "2017-08-01T11:13:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441946",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "44084cdcbca46c40c989c7a49828f2e149328e66" to "a2035749bd093d6b27a303bbf3e11f6eabfc23c3"



---

archive/issue_comments_441947.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2017-08-01T11:14:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441947",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_441948.json:
```json
{
    "body": "<a id='comment:11'></a>one failing doctest in\n\nsage -t --long src/sage/misc/sageinspect.py",
    "created_at": "2017-08-01T17:07:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441948",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:11'></a>one failing doctest in

sage -t --long src/sage/misc/sageinspect.py



---

archive/issue_comments_441949.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [chapoton](#comment%3A11):\n> one failing doctest in\n> \n> sage -t --long src/sage/misc/sageinspect.py\n\n\nNot really, that's because of the dependency #23494 and it's fixed in that ticket.",
    "created_at": "2017-08-01T17:53:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441949",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'></a>Replying to [chapoton](#comment%3A11):
> one failing doctest in
> 
> sage -t --long src/sage/misc/sageinspect.py


Not really, that's because of the dependency #23494 and it's fixed in that ticket.



---

archive/issue_comments_441950.json:
```json
{
    "body": "<a id='comment:13'></a>Now that #23494 is merged in `develop`, the failing doctest is fixed.",
    "created_at": "2017-08-07T10:09:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441950",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'></a>Now that #23494 is merged in `develop`, the failing doctest is fixed.



---

archive/issue_comments_441951.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Fr\u00e9d\u00e9ric Chapoton\"",
    "created_at": "2017-08-07T10:14:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441951",
    "user": "https://github.com/fchapoton"
}
```

Changing reviewer from "" to "Frédéric Chapoton"



---

archive/issue_comments_441952.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-08-07T10:14:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441952",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_441953.json:
```json
{
    "body": "<a id='comment:14'></a>ok, looks good. Let it be.",
    "created_at": "2017-08-07T10:14:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441953",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'></a>ok, looks good. Let it be.



---

archive/issue_comments_441954.json:
```json
{
    "body": "Changing branch from \"u/jdemeyer/py3__change_of_behaviour_of_bound___unbound_methods_and_methodtype\" to \"a2035749bd093d6b27a303bbf3e11f6eabfc23c3\"",
    "created_at": "2017-08-11T18:17:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441954",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/jdemeyer/py3__change_of_behaviour_of_bound___unbound_methods_and_methodtype" to "a2035749bd093d6b27a303bbf3e11f6eabfc23c3"



---

archive/issue_comments_441955.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2017-08-11T18:17:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23491#issuecomment-441955",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_060271.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-08-11T18:17:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/23491",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/23491#event-60271"
}
```
