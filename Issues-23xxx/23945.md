# Issue 23945: Add test for Weil polynomials

archive/issues_023708.json:
```json
{
    "assignees": [],
    "body": "We are adding a function that tests whether a given polynomial is a Weil polynomial for a given prime power q, i.e., whether its complex roots all have absolute value `sqrt(q)`.\n\nDepends on #23947\n\nKeywords: **Weil polynomial, sd91**\n\nBranch/Commit: **[cdcc8cf](https://github.com/sagemath/sagetrac-mirror/commit/cdcc8cfec1008982b0a91411fb70585062d8fbed)**\n\nReviewer: **Kiran Kedlaya, David Roe**\n\nAuthor: **David Zureick-Brown**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23945_\n\n",
    "closed_at": "2017-10-22T05:51:53Z",
    "created_at": "2017-09-30T17:19:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add test for Weil polynomials",
    "type": "issue",
    "updated_at": "2017-10-22T05:51:53Z",
    "url": "https://github.com/sagemath/sage/issues/23945",
    "user": "https://github.com/kedlaya"
}
```
We are adding a function that tests whether a given polynomial is a Weil polynomial for a given prime power q, i.e., whether its complex roots all have absolute value `sqrt(q)`.

Depends on #23947

Keywords: **Weil polynomial, sd91**

Branch/Commit: **[cdcc8cf](https://github.com/sagemath/sagetrac-mirror/commit/cdcc8cfec1008982b0a91411fb70585062d8fbed)**

Reviewer: **Kiran Kedlaya, David Roe**

Author: **David Zureick-Brown**

_Issue created by migration from https://trac.sagemath.org/ticket/23945_





---

archive/issue_events_314470.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-09-30T17:19:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314470"
}
```



---

archive/issue_events_314471.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-09-30T17:19:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314471"
}
```



---

archive/issue_events_314472.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-09-30T17:19:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314472"
}
```



---

archive/issue_events_314473.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-09-30T17:19:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314473"
}
```



---

archive/issue_comments_364107.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nOne easy way to do this would be to combine the \"inverse reciprocal transform\" from #23947 with the existing `all_roots_in_interval` method of polynomials (which wraps Pari's implementation of Sturm sequences).",
    "created_at": "2017-09-30T23:37:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364107",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:1'>Comment 1:</a>
One easy way to do this would be to combine the "inverse reciprocal transform" from #23947 with the existing `all_roots_in_interval` method of polynomials (which wraps Pari's implementation of Sturm sequences).



---

archive/issue_comments_364108.json:
```json
{
    "body": "Dependencies: **#23947**",
    "created_at": "2017-09-30T23:37:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364108",
    "user": "https://github.com/kedlaya"
}
```

Dependencies: **#23947**



---

archive/issue_comments_364109.json:
```json
{
    "body": "Branch: **[u/dzb/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/dzb/add_test_for_weil_polynomials)**",
    "created_at": "2017-10-01T22:50:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364109",
    "user": "https://github.com/sagetrac-dzb"
}
```

Branch: **[u/dzb/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/dzb/add_test_for_weil_polynomials)**



---

archive/issue_comments_364110.json:
```json
{
    "body": "Commit: **[4000398](https://github.com/sagemath/sagetrac-mirror/commit/4000398c7927e552e9d01e329d9218e230a89e83)**",
    "created_at": "2017-10-01T22:51:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364110",
    "user": "https://github.com/sagetrac-dzb"
}
```

Commit: **[4000398](https://github.com/sagemath/sagetrac-mirror/commit/4000398c7927e552e9d01e329d9218e230a89e83)**



---

archive/issue_events_314474.json:
```json
{
    "actor": "https://github.com/sagetrac-dzb",
    "created_at": "2017-10-01T22:51:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314474"
}
```



---

archive/issue_comments_364111.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nImplemented `is_weil_polynomial`\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b59dffe8539db248ce71c730184ef8c5f43d1544\">b59dffe</a></td><td><code>Implement reciprocal transform, inverse reciprocal transform</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/314b45fea06ecce0567d73a18ca2e78e80549e6d\">314b45f</a></td><td><code>Rename inverse transform as trace polynomial</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4000398c7927e552e9d01e329d9218e230a89e83\">4000398</a></td><td><code>Implemented is_weil_polynomial</code></td></tr></table>\n",
    "created_at": "2017-10-01T22:51:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364111",
    "user": "https://github.com/sagetrac-dzb"
}
```

<a id='comment:3'>Comment 3:</a>
Implemented `is_weil_polynomial`

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b59dffe8539db248ce71c730184ef8c5f43d1544">b59dffe</a></td><td><code>Implement reciprocal transform, inverse reciprocal transform</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/314b45fea06ecce0567d73a18ca2e78e80549e6d">314b45f</a></td><td><code>Rename inverse transform as trace polynomial</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4000398c7927e552e9d01e329d9218e230a89e83">4000398</a></td><td><code>Implemented is_weil_polynomial</code></td></tr></table>




---

archive/issue_comments_364112.json:
```json
{
    "body": "Author: **David Zureick-Brown**",
    "created_at": "2017-10-01T22:51:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364112",
    "user": "https://github.com/sagetrac-dzb"
}
```

Author: **David Zureick-Brown**



---

archive/issue_comments_364113.json:
```json
{
    "body": "Changed branch from **[u/dzb/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/dzb/add_test_for_weil_polynomials)** to **[u/roed/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/add_test_for_weil_polynomials)**",
    "created_at": "2017-10-01T23:46:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364113",
    "user": "https://github.com/roed314"
}
```

Changed branch from **[u/dzb/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/dzb/add_test_for_weil_polynomials)** to **[u/roed/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/add_test_for_weil_polynomials)**



---

archive/issue_comments_364114.json:
```json
{
    "body": "Changed commit from **[4000398](https://github.com/sagemath/sagetrac-mirror/commit/4000398c7927e552e9d01e329d9218e230a89e83)** to **[ead122d](https://github.com/sagemath/sagetrac-mirror/commit/ead122d8e4bc3b946af2aed1863ce2c3aec879d5)**",
    "created_at": "2017-10-01T23:47:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364114",
    "user": "https://github.com/roed314"
}
```

Changed commit from **[4000398](https://github.com/sagemath/sagetrac-mirror/commit/4000398c7927e552e9d01e329d9218e230a89e83)** to **[ead122d](https://github.com/sagemath/sagetrac-mirror/commit/ead122d8e4bc3b946af2aed1863ce2c3aec879d5)**



---

archive/issue_comments_364115.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nMade changes; if Kiran is happy then we can give this a positive review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/33ea68b55432b9030ab0e08c8264db2f1d92b1c2\">33ea68b</a></td><td><code>Small fixes to docstring, add comment to code, use try/except on trace_polynomial rather than duplicating code</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c5af2d4fe2845b73d47f1ced95fac03eef8aa1af\">c5af2d4</a></td><td><code>Fix docstring formatting, add doctests, fix error in cofactor of trace_polynomial</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ead122d8e4bc3b946af2aed1863ce2c3aec879d5\">ead122d</a></td><td><code>Merge branch 't/23947/reciprocal_transform' into t/23945/add_test_for_weil_polynomials</code></td></tr></table>\n",
    "created_at": "2017-10-01T23:47:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364115",
    "user": "https://github.com/roed314"
}
```

<a id='comment:5'>Comment 5:</a>
Made changes; if Kiran is happy then we can give this a positive review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/33ea68b55432b9030ab0e08c8264db2f1d92b1c2">33ea68b</a></td><td><code>Small fixes to docstring, add comment to code, use try/except on trace_polynomial rather than duplicating code</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c5af2d4fe2845b73d47f1ced95fac03eef8aa1af">c5af2d4</a></td><td><code>Fix docstring formatting, add doctests, fix error in cofactor of trace_polynomial</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ead122d8e4bc3b946af2aed1863ce2c3aec879d5">ead122d</a></td><td><code>Merge branch 't/23947/reciprocal_transform' into t/23945/add_test_for_weil_polynomials</code></td></tr></table>




---

archive/issue_comments_364116.json:
```json
{
    "body": "Changed author from **David Zureick-Brown** to **Kiran Kedlaya, David Zureick-Brown**",
    "created_at": "2017-10-02T00:29:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364116",
    "user": "https://github.com/kedlaya"
}
```

Changed author from **David Zureick-Brown** to **Kiran Kedlaya, David Zureick-Brown**



---

archive/issue_comments_364117.json:
```json
{
    "body": "Reviewer: **Kiran Kedlaya, David Roe**",
    "created_at": "2017-10-02T00:34:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364117",
    "user": "https://github.com/kedlaya"
}
```

Reviewer: **Kiran Kedlaya, David Roe**



---

archive/issue_events_314475.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-02T00:34:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314475"
}
```



---

archive/issue_events_314476.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-02T00:34:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314476"
}
```



---

archive/issue_comments_364118.json:
```json
{
    "body": "Changed author from **Kiran Kedlaya, David Zureick-Brown** to **David Zureick-Brown**",
    "created_at": "2017-10-02T00:34:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364118",
    "user": "https://github.com/kedlaya"
}
```

Changed author from **Kiran Kedlaya, David Zureick-Brown** to **David Zureick-Brown**



---

archive/issue_comments_364119.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nLooks good to me.",
    "created_at": "2017-10-02T00:34:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364119",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:7'>Comment 7:</a>
Looks good to me.



---

archive/issue_events_314477.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-02T03:13:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314477"
}
```



---

archive/issue_events_314478.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-02T03:13:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314478"
}
```



---

archive/issue_comments_364120.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nSorry, backing out the positive review due to some failing doctests. More soon.",
    "created_at": "2017-10-02T03:13:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364120",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:8'>Comment 8:</a>
Sorry, backing out the positive review due to some failing doctests. More soon.



---

archive/issue_comments_364121.json:
```json
{
    "body": "Changed branch from **[u/roed/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/add_test_for_weil_polynomials)** to **[u/kedlaya/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/add_test_for_weil_polynomials)**",
    "created_at": "2017-10-02T03:32:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364121",
    "user": "https://github.com/kedlaya"
}
```

Changed branch from **[u/roed/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/add_test_for_weil_polynomials)** to **[u/kedlaya/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/add_test_for_weil_polynomials)**



---

archive/issue_comments_364122.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nI fixed the issue with the doctests. Also, I don't like redefining `self` within a method definition.\n\nAll tests now pass on k8s. Since I touched the code last, someone other than me should check my work and then set a positive review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f8b93d5835e1fa0867ae31d972a1448ceb6db889\">f8b93d5</a></td><td><code>Correct call syntax for trace_polynomial</code></td></tr></table>\n",
    "created_at": "2017-10-02T03:32:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364122",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:10'>Comment 10:</a>
I fixed the issue with the doctests. Also, I don't like redefining `self` within a method definition.

All tests now pass on k8s. Since I touched the code last, someone other than me should check my work and then set a positive review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f8b93d5835e1fa0867ae31d972a1448ceb6db889">f8b93d5</a></td><td><code>Correct call syntax for trace_polynomial</code></td></tr></table>




---

archive/issue_comments_364123.json:
```json
{
    "body": "Changed commit from **[ead122d](https://github.com/sagemath/sagetrac-mirror/commit/ead122d8e4bc3b946af2aed1863ce2c3aec879d5)** to **[f8b93d5](https://github.com/sagemath/sagetrac-mirror/commit/f8b93d5835e1fa0867ae31d972a1448ceb6db889)**",
    "created_at": "2017-10-02T03:32:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364123",
    "user": "https://github.com/kedlaya"
}
```

Changed commit from **[ead122d](https://github.com/sagemath/sagetrac-mirror/commit/ead122d8e4bc3b946af2aed1863ce2c3aec879d5)** to **[f8b93d5](https://github.com/sagemath/sagetrac-mirror/commit/f8b93d5835e1fa0867ae31d972a1448ceb6db889)**



---

archive/issue_events_314479.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-02T03:32:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314479"
}
```



---

archive/issue_events_314480.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-02T03:32:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314480"
}
```



---

archive/issue_comments_364124.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nLooks good to me.  Positive review once tests pass.",
    "created_at": "2017-10-02T03:44:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364124",
    "user": "https://github.com/roed314"
}
```

<a id='comment:11'>Comment 11:</a>
Looks good to me.  Positive review once tests pass.



---

archive/issue_events_314481.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-10-02T17:24:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314481"
}
```



---

archive/issue_events_314482.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-10-02T17:24:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314482"
}
```



---

archive/issue_comments_364125.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nDavid Zureick-Brown isn't a trusted author, so patchbot won't run.  I ran tests on k8s with no failures.  Positive review.",
    "created_at": "2017-10-02T17:24:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364125",
    "user": "https://github.com/roed314"
}
```

<a id='comment:12'>Comment 12:</a>
David Zureick-Brown isn't a trusted author, so patchbot won't run.  I ran tests on k8s with no failures.  Positive review.



---

archive/issue_events_314483.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-07T21:19:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314483"
}
```



---

archive/issue_events_314484.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-07T21:19:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314484"
}
```



---

archive/issue_comments_364126.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\n\n```\n[dochtml] OSError: [polynomia] docstring of sage.rings.polynomial.polynomial_element.Polynomial.is_weil_polynomial:10: WARNING: Bullet list ends without a blank line; unexpected unindent.\n```",
    "created_at": "2017-10-07T21:19:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364126",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:13'>Comment 13:</a>

```
[dochtml] OSError: [polynomia] docstring of sage.rings.polynomial.polynomial_element.Polynomial.is_weil_polynomial:10: WARNING: Bullet list ends without a blank line; unexpected unindent.
```



---

archive/issue_comments_364127.json:
```json
{
    "body": "Changed commit from **[f8b93d5](https://github.com/sagemath/sagetrac-mirror/commit/f8b93d5835e1fa0867ae31d972a1448ceb6db889)** to **[87721f9](https://github.com/sagemath/sagetrac-mirror/commit/87721f94c74bf5a180d2c1fdf65671ebab6f8c97)**",
    "created_at": "2017-10-08T03:32:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364127",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f8b93d5](https://github.com/sagemath/sagetrac-mirror/commit/f8b93d5835e1fa0867ae31d972a1448ceb6db889)** to **[87721f9](https://github.com/sagemath/sagetrac-mirror/commit/87721f94c74bf5a180d2c1fdf65671ebab6f8c97)**



---

archive/issue_comments_364128.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/87721f94c74bf5a180d2c1fdf65671ebab6f8c97\">87721f9</a></td><td><code>Correct docstring formatting</code></td></tr></table>\n",
    "created_at": "2017-10-08T03:32:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364128",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:14'>Comment 14:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/87721f94c74bf5a180d2c1fdf65671ebab6f8c97">87721f9</a></td><td><code>Correct docstring formatting</code></td></tr></table>




---

archive/issue_comments_364129.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nLooks like there was some dodgy formatting in the docstring. Let's see if I managed to fix it.",
    "created_at": "2017-10-08T03:41:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364129",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:15'>Comment 15:</a>
Looks like there was some dodgy formatting in the docstring. Let's see if I managed to fix it.



---

archive/issue_events_314485.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-08T03:41:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314485"
}
```



---

archive/issue_events_314486.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-08T03:41:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314486"
}
```



---

archive/issue_events_314487.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-10-09T00:48:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314487"
}
```



---

archive/issue_events_314488.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-10-09T00:48:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314488"
}
```



---

archive/issue_comments_364130.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nSeems fine, though I think it's better if True has double backticks around it.",
    "created_at": "2017-10-09T00:48:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364130",
    "user": "https://github.com/roed314"
}
```

<a id='comment:16'>Comment 16:</a>
Seems fine, though I think it's better if True has double backticks around it.



---

archive/issue_comments_364131.json:
```json
{
    "body": "Changed commit from **[87721f9](https://github.com/sagemath/sagetrac-mirror/commit/87721f94c74bf5a180d2c1fdf65671ebab6f8c97)** to **[cdcc8cf](https://github.com/sagemath/sagetrac-mirror/commit/cdcc8cfec1008982b0a91411fb70585062d8fbed)**",
    "created_at": "2017-10-09T00:53:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364131",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[87721f9](https://github.com/sagemath/sagetrac-mirror/commit/87721f94c74bf5a180d2c1fdf65671ebab6f8c97)** to **[cdcc8cf](https://github.com/sagemath/sagetrac-mirror/commit/cdcc8cfec1008982b0a91411fb70585062d8fbed)**



---

archive/issue_events_314489.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2017-10-09T00:53:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314489"
}
```



---

archive/issue_events_314490.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2017-10-09T00:53:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314490"
}
```



---

archive/issue_comments_364132.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cdcc8cfec1008982b0a91411fb70585062d8fbed\">cdcc8cf</a></td><td><code>Add backticks to True in docstring</code></td></tr></table>\n",
    "created_at": "2017-10-09T00:53:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364132",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:17'>Comment 17:</a>
Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cdcc8cfec1008982b0a91411fb70585062d8fbed">cdcc8cf</a></td><td><code>Add backticks to True in docstring</code></td></tr></table>




---

archive/issue_comments_364133.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nI added the double backticks around True in the second line. In the one-sentence summary, I think the convention is not to use them.",
    "created_at": "2017-10-09T00:54:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364133",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:18'>Comment 18:</a>
I added the double backticks around True in the second line. In the one-sentence summary, I think the convention is not to use them.



---

archive/issue_comments_364134.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nGood enough, though I think the convention would be `Return whether this is a Weil polynomial` rather than `Return True if this is a Weil polynomial`.",
    "created_at": "2017-10-10T00:54:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364134",
    "user": "https://github.com/roed314"
}
```

<a id='comment:19'>Comment 19:</a>
Good enough, though I think the convention would be `Return whether this is a Weil polynomial` rather than `Return True if this is a Weil polynomial`.



---

archive/issue_events_314491.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-10-10T00:54:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314491"
}
```



---

archive/issue_events_314492.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-10-10T00:54:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314492"
}
```



---

archive/issue_comments_364135.json:
```json
{
    "body": "Changed branch from **[u/kedlaya/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/add_test_for_weil_polynomials)** to **[cdcc8cf](https://github.com/sagemath/sagetrac-mirror/commit/cdcc8cfec1008982b0a91411fb70585062d8fbed)**",
    "created_at": "2017-10-22T05:51:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23945#issuecomment-364135",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/kedlaya/add_test_for_weil_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/add_test_for_weil_polynomials)** to **[cdcc8cf](https://github.com/sagemath/sagetrac-mirror/commit/cdcc8cfec1008982b0a91411fb70585062d8fbed)**



---

archive/issue_events_314493.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-22T05:51:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314493"
}
```



---

archive/issue_events_314494.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ba0802131ade2e7a796ce7f14cfdfb16e0739cba",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-10-22T05:51:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23945",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23945#event-314494"
}
```
