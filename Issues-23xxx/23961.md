...
```json
{
    "body": "beware : there remains 2 failing doctests. I have copied the code from the pre-Lie case, but something seems to go wrong..",
...
```json
{
    "body": "I have added a simple check in the init of binary trees.",
...
```json
{
    "body": "Works for me, but I think we should add a test in the `BinaryTree.__init__` about the infinite loop with a `QQ` element. I would set a positive review otherwise if this is ready for review.",
...
```json
{
    "body": "I have added the doctest. Patchbot was green before this addition.",
...
```json
{
    "body": "I expanded the documentation about that test a little bit more. If my change is good, then positive review.",
