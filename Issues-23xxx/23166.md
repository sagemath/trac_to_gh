# Issue 23166: Conversion from a Function Field to its underlying Polynomial Ring

archive/issues_022929.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: K.<x> = FunctionField(QQ)\nsage: K._ring(x)\nTraceback:\n...\nValueError: only constants can be converted into the constant base field but x is not a constant\n```\n\nKeywords: **sd86.5**\n\nBranch/Commit: **[3d74661](https://github.com/sagemath/sagetrac-mirror/commit/3d74661ce4e1bc9cdb1abeb248f884ff5690a635)**\n\nReviewer: **Julian R\u00fcth**\n\nAuthor: **David Roe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23166_\n\n",
    "closed_at": "2017-06-11T09:13:08Z",
    "created_at": "2017-06-07T20:30:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Conversion from a Function Field to its underlying Polynomial Ring",
    "type": "issue",
    "updated_at": "2017-06-11T09:13:08Z",
    "url": "https://github.com/sagemath/sage/issues/23166",
    "user": "https://github.com/saraedum"
}
```

```
sage: K.<x> = FunctionField(QQ)
sage: K._ring(x)
Traceback:
...
ValueError: only constants can be converted into the constant base field but x is not a constant
```

Keywords: **sd86.5**

Branch/Commit: **[3d74661](https://github.com/sagemath/sagetrac-mirror/commit/3d74661ce4e1bc9cdb1abeb248f884ff5690a635)**

Reviewer: **Julian Rüth**

Author: **David Roe**

_Issue created by migration from https://trac.sagemath.org/ticket/23166_





---

archive/issue_events_304900.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-07T20:30:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23166#event-304900"
}
```



---

archive/issue_events_304901.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-07T20:30:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23166#event-304901"
}
```



---

archive/issue_events_304902.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-07T20:30:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23166#event-304902"
}
```



---

archive/issue_events_304903.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-07T20:30:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23166#event-304903"
}
```



---

archive/issue_comments_350044.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,8 @@\n \n+```\n+sage: K.<x> = FunctionField(QQ)\n+sage: K._ring(x)\n+Traceback:\n+...\n+ValueError: only constants can be converted into the constant base field but x is not a constant\n+```\n``````\n",
    "created_at": "2017-06-07T20:33:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23166#issuecomment-350044",
    "user": "https://github.com/roed314"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,8 @@
 
+```
+sage: K.<x> = FunctionField(QQ)
+sage: K._ring(x)
+Traceback:
+...
+ValueError: only constants can be converted into the constant base field but x is not a constant
+```
``````




---

archive/issue_comments_350045.json:
```json
{
    "body": "Branch: **[u/roed/conversion_from_a_function_field_to_its_underlying_polynomial_ring](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/conversion_from_a_function_field_to_its_underlying_polynomial_ring)**",
    "created_at": "2017-06-07T21:50:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23166#issuecomment-350045",
    "user": "https://github.com/roed314"
}
```

Branch: **[u/roed/conversion_from_a_function_field_to_its_underlying_polynomial_ring](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/conversion_from_a_function_field_to_its_underlying_polynomial_ring)**



---

archive/issue_comments_350046.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3d74661ce4e1bc9cdb1abeb248f884ff5690a635\">3d74661</a></td><td><code>Adding conversion from rational function fields to their ring of integers</code></td></tr></table>\n",
    "created_at": "2017-06-07T21:50:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23166#issuecomment-350046",
    "user": "https://github.com/roed314"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3d74661ce4e1bc9cdb1abeb248f884ff5690a635">3d74661</a></td><td><code>Adding conversion from rational function fields to their ring of integers</code></td></tr></table>




---

archive/issue_events_304904.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-06-07T21:50:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23166#event-304904"
}
```



---

archive/issue_comments_350047.json:
```json
{
    "body": "Commit: **[3d74661](https://github.com/sagemath/sagetrac-mirror/commit/3d74661ce4e1bc9cdb1abeb248f884ff5690a635)**",
    "created_at": "2017-06-07T21:50:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23166#issuecomment-350047",
    "user": "https://github.com/roed314"
}
```

Commit: **[3d74661](https://github.com/sagemath/sagetrac-mirror/commit/3d74661ce4e1bc9cdb1abeb248f884ff5690a635)**



---

archive/issue_events_304905.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-07T21:53:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23166#event-304905"
}
```



---

archive/issue_events_304906.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-07T21:53:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23166#event-304906"
}
```



---

archive/issue_comments_350048.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2017-06-07T21:53:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23166#issuecomment-350048",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_comments_350049.json:
```json
{
    "body": "Author: **David Roe**",
    "created_at": "2017-06-07T21:54:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23166#issuecomment-350049",
    "user": "https://github.com/saraedum"
}
```

Author: **David Roe**



---

archive/issue_events_304907.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-11T09:13:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23166#event-304907"
}
```



---

archive/issue_events_304908.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "554fe30b1769e6ac2b5d327fd68b11fc488f633c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-06-11T09:13:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23166#event-304908"
}
```



---

archive/issue_comments_350050.json:
```json
{
    "body": "Changed branch from **[u/roed/conversion_from_a_function_field_to_its_underlying_polynomial_ring](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/conversion_from_a_function_field_to_its_underlying_polynomial_ring)** to **[3d74661](https://github.com/sagemath/sagetrac-mirror/commit/3d74661ce4e1bc9cdb1abeb248f884ff5690a635)**",
    "created_at": "2017-06-11T09:13:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23166#issuecomment-350050",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/roed/conversion_from_a_function_field_to_its_underlying_polynomial_ring](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/conversion_from_a_function_field_to_its_underlying_polynomial_ring)** to **[3d74661](https://github.com/sagemath/sagetrac-mirror/commit/3d74661ce4e1bc9cdb1abeb248f884ff5690a635)**
