# Issue 23631: conversion of multivariate polynomial (over a number field) back to its base ring fails

archive/issues_023394.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: K.<s, t> = QQ.extension([x^2-2, x^2-3])\nsage: R.<y, z> = K[]\nsage: D.<d, e> = R[]\nsage: R(D(y))\n```\nfails with `TypeError: not a constant polynomial`\n\n```\nsage: K.<s, t> = QQ.extension([x^2-2, x^2-3])\nsage: R = PolynomialRing(K, 'xG_0, xG_1, xH_0')\nsage: D.<d, e> = R[]\nsage: R(D(R.0))\n```\nfails with `TypeError: unable to convert 'xG_0' to a rational`\n\nCC:  @tscrim\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23631_\n\n",
    "created_at": "2017-08-17T13:14:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "conversion of multivariate polynomial (over a number field) back to its base ring fails",
    "type": "issue",
    "updated_at": "2022-12-29T01:40:01Z",
    "url": "https://github.com/sagemath/sage/issues/23631",
    "user": "https://github.com/dkrenn"
}
```

```
sage: K.<s, t> = QQ.extension([x^2-2, x^2-3])
sage: R.<y, z> = K[]
sage: D.<d, e> = R[]
sage: R(D(y))
```
fails with `TypeError: not a constant polynomial`

```
sage: K.<s, t> = QQ.extension([x^2-2, x^2-3])
sage: R = PolynomialRing(K, 'xG_0, xG_1, xH_0')
sage: D.<d, e> = R[]
sage: R(D(R.0))
```
fails with `TypeError: unable to convert 'xG_0' to a rational`

CC:  @tscrim

_Issue created by migration from https://trac.sagemath.org/ticket/23631_





---

archive/issue_events_310675.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2017-08-17T13:14:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23631",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23631#event-310675"
}
```



---

archive/issue_events_310676.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2017-08-17T13:14:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23631",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23631#event-310676"
}
```



---

archive/issue_events_310677.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2017-08-17T13:14:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23631",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23631#event-310677"
}
```



---

archive/issue_events_310678.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2017-08-17T13:14:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23631",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23631#event-310678"
}
```



---

archive/issue_comments_358380.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nMaybe the same / a similar issue as #23629, but from the outside behavior it looks different; thus a new ticket.",
    "created_at": "2017-08-17T13:15:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23631#issuecomment-358380",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:1" align="right">Comment 1</div>

Maybe the same / a similar issue as #23629, but from the outside behavior it looks different; thus a new ticket.



---

archive/issue_events_310679.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:40:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23631",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23631#event-310679"
}
```
