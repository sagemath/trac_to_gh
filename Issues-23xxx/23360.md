# Issue 23360: Upgrade to Cython 0.26

archive/issues_023123.json:
```json
{
    "assignees": [],
    "body": "This allows us to use an unpatched version of Cython again.\n\n**Tarball**: https://pypi.python.org/packages/10/d5/753d2cb5073a9f4329d1ffed1de30b0458821780af8fdd8ba1ad5adb6f62/Cython-0.26.tar.gz\n\nCC:  @embray\n\nBranch/Commit: **[44030be](https://github.com/sagemath/sagetrac-mirror/commit/44030be5598fe725d747dd4abe3a664da37e37f2)**\n\nReviewer: **Erik Bray**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23360_\n\n",
    "closed_at": "2017-07-26T22:12:44Z",
    "created_at": "2017-07-04T08:29:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Upgrade to Cython 0.26",
    "type": "issue",
    "updated_at": "2017-07-26T22:12:44Z",
    "url": "https://github.com/sagemath/sage/issues/23360",
    "user": "https://github.com/jdemeyer"
}
```
This allows us to use an unpatched version of Cython again.

**Tarball**: https://pypi.python.org/packages/10/d5/753d2cb5073a9f4329d1ffed1de30b0458821780af8fdd8ba1ad5adb6f62/Cython-0.26.tar.gz

CC:  @embray

Branch/Commit: **[44030be](https://github.com/sagemath/sagetrac-mirror/commit/44030be5598fe725d747dd4abe3a664da37e37f2)**

Reviewer: **Erik Bray**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/23360_





---

archive/issue_events_307256.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T08:29:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23360#event-307256"
}
```



---

archive/issue_events_307257.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T08:29:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23360#event-307257"
}
```



---

archive/issue_events_307258.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T08:29:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23360#event-307258"
}
```



---

archive/issue_events_307259.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T08:29:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23360#event-307259"
}
```



---

archive/issue_comments_353498.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+Currently, this is just a beta but it's good to test now.\n+\n **Tarball**: https://github.com/cython/cython/archive/0.26b0.zip\n``````\n",
    "created_at": "2017-07-04T08:32:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353498",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+Currently, this is just a beta but it's good to test now.
+
 **Tarball**: https://github.com/cython/cython/archive/0.26b0.zip
``````




---

archive/issue_comments_353499.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,7 @@\n Currently, this is just a beta but it's good to test now.\n \n+Regressions:\n+- https://github.com/cython/cython/issues/1744\n+- https://github.com/cython/cython/pull/1758\n+\n **Tarball**: https://github.com/cython/cython/archive/0.26b0.zip\n``````\n",
    "created_at": "2017-07-04T08:56:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353499",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,7 @@
 Currently, this is just a beta but it's good to test now.
 
+Regressions:
+- https://github.com/cython/cython/issues/1744
+- https://github.com/cython/cython/pull/1758
+
 **Tarball**: https://github.com/cython/cython/archive/0.26b0.zip
``````




---

archive/issue_comments_353500.json:
```json
{
    "body": "Branch: **[u/jdemeyer/upgrade_to_cython_0_26](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/upgrade_to_cython_0_26)**",
    "created_at": "2017-07-04T09:00:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353500",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/upgrade_to_cython_0_26](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/upgrade_to_cython_0_26)**



---

archive/issue_comments_353501.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNeeds review?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/325763f79d5894b6cea92bbb599461cb053f9fab\">325763f</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>\n",
    "created_at": "2017-07-04T12:19:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353501",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

Needs review?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/325763f79d5894b6cea92bbb599461cb053f9fab">325763f</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>




---

archive/issue_comments_353502.json:
```json
{
    "body": "Commit: **[325763f](https://github.com/sagemath/sagetrac-mirror/commit/325763f79d5894b6cea92bbb599461cb053f9fab)**",
    "created_at": "2017-07-04T12:19:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353502",
    "user": "https://github.com/embray"
}
```

Commit: **[325763f](https://github.com/sagemath/sagetrac-mirror/commit/325763f79d5894b6cea92bbb599461cb053f9fab)**



---

archive/issue_comments_353503.json:
```json
{
    "body": "Changed commit from **[325763f](https://github.com/sagemath/sagetrac-mirror/commit/325763f79d5894b6cea92bbb599461cb053f9fab)** to **[b263b70](https://github.com/sagemath/sagetrac-mirror/commit/b263b70bde2eae0554f13a47c0378b5e0486911d)**",
    "created_at": "2017-07-04T12:33:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353503",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[325763f](https://github.com/sagemath/sagetrac-mirror/commit/325763f79d5894b6cea92bbb599461cb053f9fab)** to **[b263b70](https://github.com/sagemath/sagetrac-mirror/commit/b263b70bde2eae0554f13a47c0378b5e0486911d)**



---

archive/issue_comments_353504.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b263b70bde2eae0554f13a47c0378b5e0486911d\">b263b70</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>\n",
    "created_at": "2017-07-04T12:33:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353504",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b263b70bde2eae0554f13a47c0378b5e0486911d">b263b70</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>




---

archive/issue_comments_353505.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@embray](#comment%3A4):\n> Needs review?\n\nNo, Cython 0.26 is not released yet. This is just for testing the beta version now.",
    "created_at": "2017-07-04T12:42:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353505",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@embray](#comment%3A4):
> Needs review?

No, Cython 0.26 is not released yet. This is just for testing the beta version now.



---

archive/issue_comments_353506.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,7 @@\n Currently, this is just a beta but it's good to test now.\n \n-Regressions:\n+Regressions (fixed):\n - https://github.com/cython/cython/issues/1744\n - https://github.com/cython/cython/pull/1758\n \n-**Tarball**: https://github.com/cython/cython/archive/0.26b0.zip\n+**Tarball**: https://github.com/cython/cython/archive/0.26b2.zip\n``````\n",
    "created_at": "2017-07-12T07:57:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353506",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,7 @@
 Currently, this is just a beta but it's good to test now.
 
-Regressions:
+Regressions (fixed):
 - https://github.com/cython/cython/issues/1744
 - https://github.com/cython/cython/pull/1758
 
-**Tarball**: https://github.com/cython/cython/archive/0.26b0.zip
+**Tarball**: https://github.com/cython/cython/archive/0.26b2.zip
``````




---

archive/issue_comments_353507.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,5 @@\n Currently, this is just a beta but it's good to test now.\n \n-Regressions (fixed):\n-- https://github.com/cython/cython/issues/1744\n-- https://github.com/cython/cython/pull/1758\n+This allows us to use an unpatched version of Cython again.\n \n **Tarball**: https://github.com/cython/cython/archive/0.26b2.zip\n``````\n",
    "created_at": "2017-07-12T08:08:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353507",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,5 @@
 Currently, this is just a beta but it's good to test now.
 
-Regressions (fixed):
-- https://github.com/cython/cython/issues/1744
-- https://github.com/cython/cython/pull/1758
+This allows us to use an unpatched version of Cython again.
 
 **Tarball**: https://github.com/cython/cython/archive/0.26b2.zip
``````




---

archive/issue_comments_353508.json:
```json
{
    "body": "Changed commit from **[b263b70](https://github.com/sagemath/sagetrac-mirror/commit/b263b70bde2eae0554f13a47c0378b5e0486911d)** to **[68b11ba](https://github.com/sagemath/sagetrac-mirror/commit/68b11ba83397383ec3153729cbbbe11d39fcc35b)**",
    "created_at": "2017-07-12T08:09:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353508",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b263b70](https://github.com/sagemath/sagetrac-mirror/commit/b263b70bde2eae0554f13a47c0378b5e0486911d)** to **[68b11ba](https://github.com/sagemath/sagetrac-mirror/commit/68b11ba83397383ec3153729cbbbe11d39fcc35b)**



---

archive/issue_comments_353509.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/68b11ba83397383ec3153729cbbbe11d39fcc35b\">68b11ba</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>\n",
    "created_at": "2017-07-12T08:09:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353509",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/68b11ba83397383ec3153729cbbbe11d39fcc35b">68b11ba</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>




---

archive/issue_comments_353510.json:
```json
{
    "body": "Changed commit from **[68b11ba](https://github.com/sagemath/sagetrac-mirror/commit/68b11ba83397383ec3153729cbbbe11d39fcc35b)** to **[5528784](https://github.com/sagemath/sagetrac-mirror/commit/552878474e1d6f03031d27f24d81963aa73ff972)**",
    "created_at": "2017-07-12T11:53:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353510",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[68b11ba](https://github.com/sagemath/sagetrac-mirror/commit/68b11ba83397383ec3153729cbbbe11d39fcc35b)** to **[5528784](https://github.com/sagemath/sagetrac-mirror/commit/552878474e1d6f03031d27f24d81963aa73ff972)**



---

archive/issue_comments_353511.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/552878474e1d6f03031d27f24d81963aa73ff972\">5528784</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>\n",
    "created_at": "2017-07-12T11:53:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353511",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/552878474e1d6f03031d27f24d81963aa73ff972">5528784</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>




---

archive/issue_comments_353512.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n This allows us to use an unpatched version of Cython again.\n \n-**Tarball**: https://github.com/cython/cython/archive/0.26b2.zip\n+**Tarball**: http://bradshawfamily.net/cython/Cython-0.26rc1.tar.gz\n``````\n",
    "created_at": "2017-07-15T09:41:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353512",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 This allows us to use an unpatched version of Cython again.
 
-**Tarball**: https://github.com/cython/cython/archive/0.26b2.zip
+**Tarball**: http://bradshawfamily.net/cython/Cython-0.26rc1.tar.gz
``````




---

archive/issue_comments_353513.json:
```json
{
    "body": "Changed commit from **[5528784](https://github.com/sagemath/sagetrac-mirror/commit/552878474e1d6f03031d27f24d81963aa73ff972)** to **[c6126ff](https://github.com/sagemath/sagetrac-mirror/commit/c6126ff59b694e775307a057077c33a2aeca7976)**",
    "created_at": "2017-07-15T09:45:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353513",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[5528784](https://github.com/sagemath/sagetrac-mirror/commit/552878474e1d6f03031d27f24d81963aa73ff972)** to **[c6126ff](https://github.com/sagemath/sagetrac-mirror/commit/c6126ff59b694e775307a057077c33a2aeca7976)**



---

archive/issue_comments_353514.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c6126ff59b694e775307a057077c33a2aeca7976\">c6126ff</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>\n",
    "created_at": "2017-07-15T09:45:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353514",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c6126ff59b694e775307a057077c33a2aeca7976">c6126ff</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>




---

archive/issue_comments_353515.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n This allows us to use an unpatched version of Cython again.\n \n-**Tarball**: http://bradshawfamily.net/cython/Cython-0.26rc1.tar.gz\n+**Tarball**: https://github.com/cython/cython/archive/0.26rc2.zip\n``````\n",
    "created_at": "2017-07-19T08:08:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353515",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 This allows us to use an unpatched version of Cython again.
 
-**Tarball**: http://bradshawfamily.net/cython/Cython-0.26rc1.tar.gz
+**Tarball**: https://github.com/cython/cython/archive/0.26rc2.zip
``````




---

archive/issue_comments_353516.json:
```json
{
    "body": "Changed commit from **[c6126ff](https://github.com/sagemath/sagetrac-mirror/commit/c6126ff59b694e775307a057077c33a2aeca7976)** to **[ce5a03f](https://github.com/sagemath/sagetrac-mirror/commit/ce5a03fd6241cccf2516888ef4285099d0dbb353)**",
    "created_at": "2017-07-19T11:59:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353516",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c6126ff](https://github.com/sagemath/sagetrac-mirror/commit/c6126ff59b694e775307a057077c33a2aeca7976)** to **[ce5a03f](https://github.com/sagemath/sagetrac-mirror/commit/ce5a03fd6241cccf2516888ef4285099d0dbb353)**



---

archive/issue_comments_353517.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ce5a03fd6241cccf2516888ef4285099d0dbb353\">ce5a03f</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>\n",
    "created_at": "2017-07-19T11:59:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353517",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:14" align="right">Comment 14</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ce5a03fd6241cccf2516888ef4285099d0dbb353">ce5a03f</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>




---

archive/issue_comments_353518.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,3 @@\n-Currently, this is just a beta but it's good to test now.\n-\n This allows us to use an unpatched version of Cython again.\n \n-**Tarball**: https://github.com/cython/cython/archive/0.26rc2.zip\n+**Tarball**: https://pypi.python.org/packages/10/d5/753d2cb5073a9f4329d1ffed1de30b0458821780af8fdd8ba1ad5adb6f62/Cython-0.26.tar.gz\n``````\n",
    "created_at": "2017-07-20T08:23:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353518",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,3 @@
-Currently, this is just a beta but it's good to test now.
-
 This allows us to use an unpatched version of Cython again.
 
-**Tarball**: https://github.com/cython/cython/archive/0.26rc2.zip
+**Tarball**: https://pypi.python.org/packages/10/d5/753d2cb5073a9f4329d1ffed1de30b0458821780af8fdd8ba1ad5adb6f62/Cython-0.26.tar.gz
``````




---

archive/issue_comments_353519.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/44030be5598fe725d747dd4abe3a664da37e37f2\">44030be</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>\n",
    "created_at": "2017-07-20T08:26:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353519",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:16" align="right">Comment 16</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/44030be5598fe725d747dd4abe3a664da37e37f2">44030be</a></td><td><code>Upgrade to Cython 0.26</code></td></tr></table>




---

archive/issue_comments_353520.json:
```json
{
    "body": "Changed commit from **[ce5a03f](https://github.com/sagemath/sagetrac-mirror/commit/ce5a03fd6241cccf2516888ef4285099d0dbb353)** to **[44030be](https://github.com/sagemath/sagetrac-mirror/commit/44030be5598fe725d747dd4abe3a664da37e37f2)**",
    "created_at": "2017-07-20T08:26:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353520",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ce5a03f](https://github.com/sagemath/sagetrac-mirror/commit/ce5a03fd6241cccf2516888ef4285099d0dbb353)** to **[44030be](https://github.com/sagemath/sagetrac-mirror/commit/44030be5598fe725d747dd4abe3a664da37e37f2)**



---

archive/issue_events_307260.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-20T08:33:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23360#event-307260"
}
```



---

archive/issue_comments_353521.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nI launched the test suite on quasar (Ubuntu x86_64). Build was successfull.",
    "created_at": "2017-07-20T09:59:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353521",
    "user": "https://github.com/videlec"
}
```

<div id="comment:18" align="right">Comment 18</div>

I launched the test suite on quasar (Ubuntu x86_64). Build was successfull.



---

archive/issue_comments_353522.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nReplying to [@videlec](#comment%3A18):\n> I launched the test suite on quasar (Ubuntu x86_64). Build was successfull.\n\nAnd all tests pass.",
    "created_at": "2017-07-20T14:42:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353522",
    "user": "https://github.com/videlec"
}
```

<div id="comment:19" align="right">Comment 19</div>

Replying to [@videlec](#comment%3A18):
> I launched the test suite on quasar (Ubuntu x86_64). Build was successfull.

And all tests pass.



---

archive/issue_comments_353523.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nWindows patchbot just got to it.",
    "created_at": "2017-07-20T16:23:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353523",
    "user": "https://github.com/embray"
}
```

<div id="comment:20" align="right">Comment 20</div>

Windows patchbot just got to it.



---

archive/issue_comments_353524.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nPatchbots will do nothing for this kind of spkg ticket.",
    "created_at": "2017-07-20T16:30:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353524",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:21" align="right">Comment 21</div>

Patchbots will do nothing for this kind of spkg ticket.



---

archive/issue_comments_353525.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nAh, I keep forgetting that.  Why is that?  You'd think one would want them to test new packages?",
    "created_at": "2017-07-20T16:32:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353525",
    "user": "https://github.com/embray"
}
```

<div id="comment:22" align="right">Comment 22</div>

Ah, I keep forgetting that.  Why is that?  You'd think one would want them to test new packages?



---

archive/issue_comments_353526.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">Comment 23</div>\n\nFor safety reason, and because not so many people care about enhancing the patchbots. I have done my share.",
    "created_at": "2017-07-20T16:33:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353526",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:23" align="right">Comment 23</div>

For safety reason, and because not so many people care about enhancing the patchbots. I have done my share.



---

archive/issue_comments_353527.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">Comment 24</div>\n\nIf there were safety issues with an upstream package wouldn't we want to know that in the patchbots first, before just declaring them \"fine\" and then pushing them on all users?",
    "created_at": "2017-07-20T16:38:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353527",
    "user": "https://github.com/embray"
}
```

<div id="comment:24" align="right">Comment 24</div>

If there were safety issues with an upstream package wouldn't we want to know that in the patchbots first, before just declaring them "fine" and then pushing them on all users?



---

archive/issue_comments_353528.json:
```json
{
    "body": "Reviewer: **Erik Bray**",
    "created_at": "2017-07-20T17:16:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353528",
    "user": "https://github.com/embray"
}
```

Reviewer: **Erik Bray**



---

archive/issue_events_307261.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-20T17:16:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23360#event-307261"
}
```



---

archive/issue_events_307262.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-20T17:16:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23360#event-307262"
}
```



---

archive/issue_comments_353529.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">Comment 25</div>\n\nCompiles fine on Cygwin.",
    "created_at": "2017-07-20T17:16:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353529",
    "user": "https://github.com/embray"
}
```

<div id="comment:25" align="right">Comment 25</div>

Compiles fine on Cygwin.



---

archive/issue_events_307263.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-26T22:12:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23360#event-307263"
}
```



---

archive/issue_events_307264.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4ade915b057633218254bc15e0019d78350fbc17",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-07-26T22:12:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23360#event-307264"
}
```



---

archive/issue_comments_353530.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/upgrade_to_cython_0_26](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/upgrade_to_cython_0_26)** to **[44030be](https://github.com/sagemath/sagetrac-mirror/commit/44030be5598fe725d747dd4abe3a664da37e37f2)**",
    "created_at": "2017-07-26T22:12:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23360#issuecomment-353530",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/upgrade_to_cython_0_26](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/upgrade_to_cython_0_26)** to **[44030be](https://github.com/sagemath/sagetrac-mirror/commit/44030be5598fe725d747dd4abe3a664da37e37f2)**
