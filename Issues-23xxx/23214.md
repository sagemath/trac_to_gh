...
```json
{
    "body": "<a id='comment:46'></a>\nI get 5 errors on OSX:\n\n```\nsage -t --long src/sage/combinat/designs/ext_rep.py  # Killed due to abort\nsage -t --long src/sage/doctest/external.py  # Killed due to abort\nsage -t --long src/sage/matrix/matrix_modn_sparse.pyx  # 3 doctests failed\n```\n\n```\nmatrix_modn_sparse.pyx}}} errors are the sames as the patchbot ones and the two others are not ticket dependent (see Justin C. Walker message in [https://groups.google.com/forum/?fromgroups#!topic/sage-release/yiJTgNhjB6o Sage 8.5.beta1 released] ).",
