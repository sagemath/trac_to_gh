# Issue 23135: Doctest: Conjugate does not distribute over a sum

archive/issues_022898.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: x = var('x', domain='real') \nsage: conjugate(x+I)\nx + I\nsage: conjugate(x)+conjugate(I)\nx - I\n```\n\n(tested on top of #23134)\n\nReported recently in ask.sagemath, [a problem with variables in real domains](https://ask.sagemath.org/question/37766/a-problem-with-variables-in-real-domains/)\n\nDepends on #23325\n\nCC:  @rwst\n\nKeywords: **assumptions, complex numbers**\n\nBranch/Commit: **[8cef176](https://github.com/sagemath/sagetrac-mirror/commit/8cef17614aefcdbd928915e2b39c5111acfa9a3d)**\n\nReviewer: **Marcelo Forets**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23135_\n\n",
    "closed_at": "2017-09-10T22:05:33Z",
    "created_at": "2017-06-03T20:39:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Doctest: Conjugate does not distribute over a sum",
    "type": "issue",
    "updated_at": "2017-09-10T22:05:33Z",
    "url": "https://github.com/sagemath/sage/issues/23135",
    "user": "https://github.com/mforets"
}
```

```
sage: x = var('x', domain='real') 
sage: conjugate(x+I)
x + I
sage: conjugate(x)+conjugate(I)
x - I
```

(tested on top of #23134)

Reported recently in ask.sagemath, [a problem with variables in real domains](https://ask.sagemath.org/question/37766/a-problem-with-variables-in-real-domains/)

Depends on #23325

CC:  @rwst

Keywords: **assumptions, complex numbers**

Branch/Commit: **[8cef176](https://github.com/sagemath/sagetrac-mirror/commit/8cef17614aefcdbd928915e2b39c5111acfa9a3d)**

Reviewer: **Marcelo Forets**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/23135_





---

archive/issue_events_293565.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-06-03T20:39:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293565"
}
```



---

archive/issue_events_293566.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-06-03T20:39:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293566"
}
```



---

archive/issue_events_293567.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-06-03T20:39:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293567"
}
```



---

archive/issue_events_293568.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-06-03T20:39:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293568"
}
```



---

archive/issue_comments_349398.json:
```json
{
    "body": "Upstream: **Fixed upstream, in a later stable release.**",
    "created_at": "2017-06-04T09:01:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349398",
    "user": "https://github.com/rwst"
}
```

Upstream: **Fixed upstream, in a later stable release.**



---

archive/issue_comments_349399.json:
```json
{
    "body": "Dependencies: **pynac-0.7.9**",
    "created_at": "2017-06-04T09:01:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349399",
    "user": "https://github.com/rwst"
}
```

Dependencies: **pynac-0.7.9**



---

archive/issue_comments_349400.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nA rarely visited part of the Pynac code tried to be too smart. Fixed in pynac master.",
    "created_at": "2017-06-04T09:01:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349400",
    "user": "https://github.com/rwst"
}
```

<a id='comment:1'>Comment 1:</a>
A rarely visited part of the Pynac code tried to be too smart. Fixed in pynac master.



---

archive/issue_comments_349401.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nSo will this problem be solved once [SageMath](../wiki/SageMath) is using pynac master? If so should we close this ticket?",
    "created_at": "2017-06-05T21:46:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349401",
    "user": "https://github.com/sagetrac-hwsmith"
}
```

<a id='comment:2'>Comment 2:</a>
So will this problem be solved once [SageMath](../wiki/SageMath) is using pynac master? If so should we close this ticket?



---

archive/issue_comments_349402.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nReplying to [hwsmith](#comment%3A2):\n> So will this problem be solved once [SageMath](../wiki/SageMath) is using pynac master? If so should we close this ticket?\n\nThe dependency on \"pynac-0.7.9\" will be replaced with the Pynac upgrade ticket number after that version was released. This ticket should add doctests for the issue. Usually its title is then prepended with \"Doctest:\" to give a hint to potential reviewers.",
    "created_at": "2017-06-06T06:11:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349402",
    "user": "https://github.com/rwst"
}
```

<a id='comment:3'>Comment 3:</a>
Replying to [hwsmith](#comment%3A2):
> So will this problem be solved once [SageMath](../wiki/SageMath) is using pynac master? If so should we close this ticket?

The dependency on "pynac-0.7.9" will be replaced with the Pynac upgrade ticket number after that version was released. This ticket should add doctests for the issue. Usually its title is then prepended with "Doctest:" to give a hint to potential reviewers.



---

archive/issue_comments_349403.json:
```json
{
    "body": "Changed dependencies from **pynac-0.7.9** to **#23325**",
    "created_at": "2017-06-26T07:51:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349403",
    "user": "https://github.com/rwst"
}
```

Changed dependencies from **pynac-0.7.9** to **#23325**



---

archive/issue_comments_349404.json:
```json
{
    "body": "Changed upstream from **Fixed upstream, in a later stable release.** to none",
    "created_at": "2017-06-26T07:51:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349404",
    "user": "https://github.com/rwst"
}
```

Changed upstream from **Fixed upstream, in a later stable release.** to none



---

archive/issue_comments_349405.json:
```json
{
    "body": "Branch: **[u/rws/conjugate_does_not_distribute_over_a_sum](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/conjugate_does_not_distribute_over_a_sum)**",
    "created_at": "2017-09-06T05:21:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349405",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/conjugate_does_not_distribute_over_a_sum](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/conjugate_does_not_distribute_over_a_sum)**



---

archive/issue_events_293569.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-09-06T05:22:26Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "title_is": "Doctest: Conjugate does not distribute over a sum",
    "title_was": "Conjugate does not distribute over a sum",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293569"
}
```



---

archive/issue_comments_349406.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8cef17614aefcdbd928915e2b39c5111acfa9a3d\">8cef176</a></td><td><code>23135: Doctest: Conjugate does not distribute over a sum</code></td></tr></table>\n",
    "created_at": "2017-09-06T05:22:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349406",
    "user": "https://github.com/rwst"
}
```

<a id='comment:6'>Comment 6:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8cef17614aefcdbd928915e2b39c5111acfa9a3d">8cef176</a></td><td><code>23135: Doctest: Conjugate does not distribute over a sum</code></td></tr></table>




---

archive/issue_comments_349407.json:
```json
{
    "body": "Commit: **[8cef176](https://github.com/sagemath/sagetrac-mirror/commit/8cef17614aefcdbd928915e2b39c5111acfa9a3d)**",
    "created_at": "2017-09-06T05:22:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349407",
    "user": "https://github.com/rwst"
}
```

Commit: **[8cef176](https://github.com/sagemath/sagetrac-mirror/commit/8cef17614aefcdbd928915e2b39c5111acfa9a3d)**



---

archive/issue_events_293570.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-09-06T05:22:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293570"
}
```



---

archive/issue_comments_349408.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2017-09-06T05:22:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349408",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_349409.json:
```json
{
    "body": "Reviewer: **Marcelo Forets**",
    "created_at": "2017-09-06T11:44:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349409",
    "user": "https://github.com/mforets"
}
```

Reviewer: **Marcelo Forets**



---

archive/issue_events_293571.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-09-06T11:44:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293571"
}
```



---

archive/issue_events_293572.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-09-06T11:44:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293572"
}
```



---

archive/issue_comments_349410.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nreviewed. thank you for fixing this!",
    "created_at": "2017-09-06T11:44:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349410",
    "user": "https://github.com/mforets"
}
```

<a id='comment:7'>Comment 7:</a>
reviewed. thank you for fixing this!



---

archive/issue_comments_349411.json:
```json
{
    "body": "Changed branch from **[u/rws/conjugate_does_not_distribute_over_a_sum](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/conjugate_does_not_distribute_over_a_sum)** to **[8cef176](https://github.com/sagemath/sagetrac-mirror/commit/8cef17614aefcdbd928915e2b39c5111acfa9a3d)**",
    "created_at": "2017-09-10T22:05:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23135#issuecomment-349411",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/conjugate_does_not_distribute_over_a_sum](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/conjugate_does_not_distribute_over_a_sum)** to **[8cef176](https://github.com/sagemath/sagetrac-mirror/commit/8cef17614aefcdbd928915e2b39c5111acfa9a3d)**



---

archive/issue_events_293573.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-09-10T22:05:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293573"
}
```



---

archive/issue_events_293574.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b45ee2c5cac9350cfc925bf14250c79b6b0bca68",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-09-10T22:05:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23135",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23135#event-293574"
}
```
