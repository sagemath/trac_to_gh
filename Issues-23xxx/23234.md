# Issue 23234: Remove precision when printing fixed modulus p-adic elements

archive/issues_022997.json:
```json
{
    "assignees": [],
    "body": "Currently, fixed modulus elements always print a precision\n\n```\nsage: ZpFM(5)(1)\n1 + O(5^20)\n```\nFor consistency with floating point p-adics and to save screen real estate, I think that they should not print their precision.\n\n```\nsage: a = ZpFM(5)(1); a\n1\nsage: a.precision_absolute()\n20\n```\n\nKeywords: **sd87, padicIMA**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23234_\n\n",
    "closed_at": "2018-11-08T16:11:39Z",
    "created_at": "2017-06-14T04:49:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove precision when printing fixed modulus p-adic elements",
    "type": "issue",
    "updated_at": "2018-11-08T16:11:39Z",
    "url": "https://github.com/sagemath/sage/issues/23234",
    "user": "https://github.com/roed314"
}
```
Currently, fixed modulus elements always print a precision

```
sage: ZpFM(5)(1)
1 + O(5^20)
```
For consistency with floating point p-adics and to save screen real estate, I think that they should not print their precision.

```
sage: a = ZpFM(5)(1); a
1
sage: a.precision_absolute()
20
```

Keywords: **sd87, padicIMA**

_Issue created by migration from https://trac.sagemath.org/ticket/23234_





---

archive/issue_events_305767.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-06-14T04:49:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305767"
}
```



---

archive/issue_events_305768.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-06-14T04:49:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305768"
}
```



---

archive/issue_events_305769.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-06-14T04:49:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305769"
}
```



---

archive/issue_comments_351505.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nFWIW, I agree :-)",
    "created_at": "2018-02-26T13:55:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23234#issuecomment-351505",
    "user": "https://github.com/xcaruso"
}
```

<div id="comment:1" align="right">Comment 1</div>

FWIW, I agree :-)



---

archive/issue_comments_351506.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nSee ticket #24843",
    "created_at": "2018-02-28T13:44:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23234#issuecomment-351506",
    "user": "https://github.com/xcaruso"
}
```

<div id="comment:2" align="right">Comment 2</div>

See ticket #24843



---

archive/issue_comments_351507.json:
```json
{
    "body": "Changed keywords from **sd87** to **sd87, padicIMA**",
    "created_at": "2018-07-22T20:07:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23234#issuecomment-351507",
    "user": "https://github.com/roed314"
}
```

Changed keywords from **sd87** to **sd87, padicIMA**



---

archive/issue_events_305770.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-07-25T19:36:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305770"
}
```



---

archive/issue_events_305771.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-07-25T19:36:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305771"
}
```



---

archive/issue_events_305772.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-07-25T19:36:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305772"
}
```



---

archive/issue_events_305773.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-07-25T19:36:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305773"
}
```



---

archive/issue_events_305774.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-07-25T19:36:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305774"
}
```



---

archive/issue_events_305775.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-07-25T19:36:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305775"
}
```



---

archive/issue_events_305776.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:11:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305776"
}
```



---

archive/issue_events_305777.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:11:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305777"
}
```



---

archive/issue_events_305778.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:11:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23234",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23234#event-305778"
}
```
