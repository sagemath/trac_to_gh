# Issue 23480: Make the Krasner's Lemma check in p-adic factory actually do something

archive/issues_023243.json:
```json
{
    "assignees": [],
    "body": "When you create a p-adic extension from an inexact polynomial, Sage should ensure that there is enough precision to determine the resulting extension up to isomorphism.\n\nDepends on #21869\n\nCC:  @saraedum\n\nKeywords: **sd87, padicIMA, padicBordeaux**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23480_\n\n",
    "created_at": "2017-07-19T22:46:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make the Krasner's Lemma check in p-adic factory actually do something",
    "type": "issue",
    "updated_at": "2022-12-29T01:40:01Z",
    "url": "https://github.com/sagemath/sage/issues/23480",
    "user": "https://github.com/roed314"
}
```
When you create a p-adic extension from an inexact polynomial, Sage should ensure that there is enough precision to determine the resulting extension up to isomorphism.

Depends on #21869

CC:  @saraedum

Keywords: **sd87, padicIMA, padicBordeaux**

_Issue created by migration from https://trac.sagemath.org/ticket/23480_





---

archive/issue_events_308735.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-19T22:46:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-308735"
}
```



---

archive/issue_events_308736.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-19T22:46:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-308736"
}
```



---

archive/issue_events_308737.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-19T22:46:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-308737"
}
```



---

archive/issue_events_308738.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-19T22:46:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-308738"
}
```



---

archive/issue_comments_355521.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis is implemented here: https://github.com/saraedum/completion/blob/master/completion.py#L904",
    "created_at": "2017-07-19T23:37:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23480#issuecomment-355521",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:1'>Comment 1:</a>
This is implemented here: https://github.com/saraedum/completion/blob/master/completion.py#L904



---

archive/issue_comments_355522.json:
```json
{
    "body": "Changed keywords from **** to **sd87**",
    "created_at": "2017-07-20T20:14:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23480#issuecomment-355522",
    "user": "https://github.com/roed314"
}
```

Changed keywords from **** to **sd87**



---

archive/issue_comments_355523.json:
```json
{
    "body": "Changed keywords from **sd87** to **sd87, padicIMA**",
    "created_at": "2018-07-22T20:23:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23480#issuecomment-355523",
    "user": "https://github.com/roed314"
}
```

Changed keywords from **sd87** to **sd87, padicIMA**



---

archive/issue_comments_355524.json:
```json
{
    "body": "Changed keywords from **sd87, padicIMA** to **sd87, padicIMA, padicBordeaux**",
    "created_at": "2019-09-07T12:46:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23480#issuecomment-355524",
    "user": "https://github.com/roed314"
}
```

Changed keywords from **sd87, padicIMA** to **sd87, padicIMA, padicBordeaux**



---

archive/issue_events_308739.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:40:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-308739"
}
```
