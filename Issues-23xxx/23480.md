# Issue 23480: Make the Krasner's Lemma check in p-adic factory actually do something

archive/issues_023243.json:
```json
{
    "assignees": [],
    "body": "When you create a p-adic extension from an inexact polynomial, Sage should ensure that there is enough precision to determine the resulting extension up to isomorphism.\n\nDepends on #21869\n\nCC:  @saraedum\n\nKeywords: **sd87, padicIMA, padicBordeaux**\n\nComponent: **padics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23480_\n\n",
    "created_at": "2017-07-19T22:46:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20padics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make the Krasner's Lemma check in p-adic factory actually do something",
    "type": "issue",
    "updated_at": "2022-12-29T01:40:01Z",
    "url": "https://github.com/sagemath/sage/issues/23480",
    "user": "https://github.com/roed314"
}
```
When you create a p-adic extension from an inexact polynomial, Sage should ensure that there is enough precision to determine the resulting extension up to isomorphism.

Depends on #21869

CC:  @saraedum

Keywords: **sd87, padicIMA, padicBordeaux**

Component: **padics**

_Issue created by migration from https://trac.sagemath.org/ticket/23480_





---

archive/issue_events_326740.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-19T22:46:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-326740"
}
```



---

archive/issue_events_326741.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-19T22:46:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20padics",
    "label_color": "0000ff",
    "label_name": "c: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-326741"
}
```



---

archive/issue_events_326742.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-19T22:46:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-326742"
}
```



---

archive/issue_events_326743.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-19T22:46:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-326743"
}
```



---

archive/issue_comments_352882.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is implemented here: https://github.com/saraedum/completion/blob/master/completion.py#L904",
    "created_at": "2017-07-19T23:37:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23480#issuecomment-352882",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:1" align="right">comment:1</div>

This is implemented here: https://github.com/saraedum/completion/blob/master/completion.py#L904



---

archive/issue_comments_352883.json:
```json
{
    "body": "Changed keywords from none to **sd87**",
    "created_at": "2017-07-20T20:14:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23480#issuecomment-352883",
    "user": "https://github.com/roed314"
}
```

Changed keywords from none to **sd87**



---

archive/issue_comments_352884.json:
```json
{
    "body": "Changed keywords from **sd87** to **sd87, padicIMA**",
    "created_at": "2018-07-22T20:23:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23480#issuecomment-352884",
    "user": "https://github.com/roed314"
}
```

Changed keywords from **sd87** to **sd87, padicIMA**



---

archive/issue_comments_352885.json:
```json
{
    "body": "Changed keywords from **sd87, padicIMA** to **sd87, padicIMA, padicBordeaux**",
    "created_at": "2019-09-07T12:46:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23480#issuecomment-352885",
    "user": "https://github.com/roed314"
}
```

Changed keywords from **sd87, padicIMA** to **sd87, padicIMA, padicBordeaux**



---

archive/issue_events_326744.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:40:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23480",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23480#event-326744"
}
```
