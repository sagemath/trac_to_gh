# Issue 23433: Swap x and y in `AbstractLinearCode.weight_enumerator`

archive/issues_023196.json:
```json
{
    "assignees": [],
    "body": "Currently `x` and `y` have interchanged roles according to the documentation.\n\nCC:  @dimpase @wdjoyner\n\nBranch/Commit: **[31cdafc](https://github.com/sagemath/sagetrac-mirror/commit/31cdafc2c69e10bfb9b5cb163e9438883505ab68)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Johan Rosenkilde**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23433_\n\n",
    "closed_at": "2017-07-26T22:11:55Z",
    "created_at": "2017-07-14T09:34:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Swap x and y in `AbstractLinearCode.weight_enumerator`",
    "type": "issue",
    "updated_at": "2017-07-26T22:11:55Z",
    "url": "https://github.com/sagemath/sage/issues/23433",
    "user": "https://github.com/johanrosenkilde"
}
```
Currently `x` and `y` have interchanged roles according to the documentation.

CC:  @dimpase @wdjoyner

Branch/Commit: **[31cdafc](https://github.com/sagemath/sagetrac-mirror/commit/31cdafc2c69e10bfb9b5cb163e9438883505ab68)**

Reviewer: **Dima Pasechnik**

Author: **Johan Rosenkilde**

_Issue created by migration from https://trac.sagemath.org/ticket/23433_





---

archive/issue_events_308156.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2017-07-14T09:34:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23433#event-308156"
}
```



---

archive/issue_events_308157.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2017-07-14T09:34:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
    "label_color": "0000ff",
    "label_name": "component: coding theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23433#event-308157"
}
```



---

archive/issue_events_308158.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2017-07-14T09:34:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23433#event-308158"
}
```



---

archive/issue_events_308159.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2017-07-14T09:34:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23433#event-308159"
}
```



---

archive/issue_comments_354744.json:
```json
{
    "body": "Branch: **[u/jsrn/23433_weight_enumerator_xy](https://github.com/sagemath/sagetrac-mirror/tree/u/jsrn/23433_weight_enumerator_xy)**",
    "created_at": "2017-07-14T10:07:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23433#issuecomment-354744",
    "user": "https://github.com/johanrosenkilde"
}
```

Branch: **[u/jsrn/23433_weight_enumerator_xy](https://github.com/sagemath/sagetrac-mirror/tree/u/jsrn/23433_weight_enumerator_xy)**



---

archive/issue_events_308160.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2017-07-14T10:10:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23433#event-308160"
}
```



---

archive/issue_comments_354745.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI've made the swap. This caused doctests to fail in `AbstractLinearCode.zeta_function` since it relied on the order of x and y, so I modified that implementation so doc-tests pass again.\n\nI've also removed a special `weight_enumerator()` from GRS codes: this was **already** removed in #21576, and I have no idea how it got back! Git blame didn't tell me anything useful.\n\nLastly, I've added some tests which demonstrate that the special `weight_distribution()` for GRS codes and Golay codes actually correspond to that returned by the generic algorithm. These tests were sorely missing before.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1fee505c3e8d324d8bd66d1df7a4ec9219c085d4\">1fee505</a></td><td><code>Fix x,y swap in AbstractLinearCode.weight_enumerator</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/31cdafc2c69e10bfb9b5cb163e9438883505ab68\">31cdafc</a></td><td><code>Removed grs.weight_enumerator (again?! see #21576). Tests for special weight_distributions</code></td></tr></table>\n",
    "created_at": "2017-07-14T10:10:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23433#issuecomment-354745",
    "user": "https://github.com/johanrosenkilde"
}
```

<div id="comment:2" align="right">Comment 2</div>

I've made the swap. This caused doctests to fail in `AbstractLinearCode.zeta_function` since it relied on the order of x and y, so I modified that implementation so doc-tests pass again.

I've also removed a special `weight_enumerator()` from GRS codes: this was **already** removed in #21576, and I have no idea how it got back! Git blame didn't tell me anything useful.

Lastly, I've added some tests which demonstrate that the special `weight_distribution()` for GRS codes and Golay codes actually correspond to that returned by the generic algorithm. These tests were sorely missing before.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1fee505c3e8d324d8bd66d1df7a4ec9219c085d4">1fee505</a></td><td><code>Fix x,y swap in AbstractLinearCode.weight_enumerator</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/31cdafc2c69e10bfb9b5cb163e9438883505ab68">31cdafc</a></td><td><code>Removed grs.weight_enumerator (again?! see #21576). Tests for special weight_distributions</code></td></tr></table>




---

archive/issue_comments_354746.json:
```json
{
    "body": "Commit: **[31cdafc](https://github.com/sagemath/sagetrac-mirror/commit/31cdafc2c69e10bfb9b5cb163e9438883505ab68)**",
    "created_at": "2017-07-14T10:10:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23433#issuecomment-354746",
    "user": "https://github.com/johanrosenkilde"
}
```

Commit: **[31cdafc](https://github.com/sagemath/sagetrac-mirror/commit/31cdafc2c69e10bfb9b5cb163e9438883505ab68)**



---

archive/issue_comments_354747.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2017-07-18T13:55:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23433#issuecomment-354747",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_354748.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nlooks good to me, thanks!",
    "created_at": "2017-07-18T13:55:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23433#issuecomment-354748",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

looks good to me, thanks!



---

archive/issue_events_308161.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-07-18T13:55:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23433#event-308161"
}
```



---

archive/issue_events_308162.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-07-18T13:55:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23433#event-308162"
}
```



---

archive/issue_comments_354749.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks!",
    "created_at": "2017-07-20T09:28:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23433#issuecomment-354749",
    "user": "https://github.com/johanrosenkilde"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks!



---

archive/issue_comments_354750.json:
```json
{
    "body": "Changed branch from **[u/jsrn/23433_weight_enumerator_xy](https://github.com/sagemath/sagetrac-mirror/tree/u/jsrn/23433_weight_enumerator_xy)** to **[31cdafc](https://github.com/sagemath/sagetrac-mirror/commit/31cdafc2c69e10bfb9b5cb163e9438883505ab68)**",
    "created_at": "2017-07-26T22:11:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23433#issuecomment-354750",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jsrn/23433_weight_enumerator_xy](https://github.com/sagemath/sagetrac-mirror/tree/u/jsrn/23433_weight_enumerator_xy)** to **[31cdafc](https://github.com/sagemath/sagetrac-mirror/commit/31cdafc2c69e10bfb9b5cb163e9438883505ab68)**



---

archive/issue_events_308163.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-26T22:11:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23433#event-308163"
}
```



---

archive/issue_events_308164.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a8071818098c318b56a92cc4ea826f61d9b94c16",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-07-26T22:11:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23433",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23433#event-308164"
}
```
