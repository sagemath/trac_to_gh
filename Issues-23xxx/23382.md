# Issue 23382: Clean up matroid constructor

archive/issues_023145.json:
```json
{
    "assignees": [],
    "body": "Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Simplify the code and add more doctests for errors.\n\nI'm particularly proud of this:\n\n```\n 2 files changed, 235 insertions(+), 256 deletions(-)\n```\nThis is -21 lines despite adding several doctests.\n\nBranch/Commit: **[23a7260](https://github.com/sagemath/sagetrac-mirror/commit/23a7260e164e8adc1c37fc531f6cd32155501717)**\n\nReviewer: **Stefan van Zwam**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23382_\n\n",
    "closed_at": "2017-07-26T22:12:28Z",
    "created_at": "2017-07-06T13:37:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean up matroid constructor",
    "type": "issue",
    "updated_at": "2017-07-26T22:12:28Z",
    "url": "https://github.com/sagemath/sage/issues/23382",
    "user": "https://github.com/jdemeyer"
}
```
Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Simplify the code and add more doctests for errors.

I'm particularly proud of this:

```
 2 files changed, 235 insertions(+), 256 deletions(-)
```
This is -21 lines despite adding several doctests.

Branch/Commit: **[23a7260](https://github.com/sagemath/sagetrac-mirror/commit/23a7260e164e8adc1c37fc531f6cd32155501717)**

Reviewer: **Stefan van Zwam**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/23382_





---

archive/issue_events_307497.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-06T13:37:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23382#event-307497"
}
```



---

archive/issue_events_307498.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-06T13:37:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
    "label_color": "0000ff",
    "label_name": "component: matroid theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23382#event-307498"
}
```



---

archive/issue_events_307499.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-06T13:37:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23382#event-307499"
}
```



---

archive/issue_events_307500.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-06T13:37:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23382#event-307500"
}
```



---

archive/issue_comments_353801.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Some basic clean up of the matroid constructor. Also add more doctests for errors.\n+Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Also add more doctests for errors.\n``````\n",
    "created_at": "2017-07-06T14:06:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23382#issuecomment-353801",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Some basic clean up of the matroid constructor. Also add more doctests for errors.
+Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Also add more doctests for errors.
``````




---

archive/issue_comments_353802.json:
```json
{
    "body": "Branch: **[u/jdemeyer/clean_up_matroid_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/clean_up_matroid_constructor)**",
    "created_at": "2017-07-06T15:27:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23382#issuecomment-353802",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/clean_up_matroid_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/clean_up_matroid_constructor)**



---

archive/issue_events_307501.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-06T15:29:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23382#event-307501"
}
```



---

archive/issue_comments_353803.json:
```json
{
    "body": "Commit: **[23a7260](https://github.com/sagemath/sagetrac-mirror/commit/23a7260e164e8adc1c37fc531f6cd32155501717)**",
    "created_at": "2017-07-06T15:29:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23382#issuecomment-353803",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[23a7260](https://github.com/sagemath/sagetrac-mirror/commit/23a7260e164e8adc1c37fc531f6cd32155501717)**



---

archive/issue_comments_353804.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/23a7260e164e8adc1c37fc531f6cd32155501717\">23a7260</a></td><td><code>Clean up matroid constructor</code></td></tr></table>\n",
    "created_at": "2017-07-06T15:29:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23382#issuecomment-353804",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/23a7260e164e8adc1c37fc531f6cd32155501717">23a7260</a></td><td><code>Clean up matroid constructor</code></td></tr></table>




---

archive/issue_comments_353805.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,8 @@\n Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Also add more doctests for errors.\n+\n+I'm particularly proud of this:\n+\n+```\n+ 2 files changed, 235 insertions(+), 256 deletions(-)\n+```\n+This is -21 lines despite adding several doctests.\n``````\n",
    "created_at": "2017-07-06T15:31:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23382#issuecomment-353805",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,8 @@
 Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Also add more doctests for errors.
+
+I'm particularly proud of this:
+
+```
+ 2 files changed, 235 insertions(+), 256 deletions(-)
+```
+This is -21 lines despite adding several doctests.
``````




---

archive/issue_comments_353806.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Also add more doctests for errors.\n+Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Simplify the code and add more doctests for errors.\n \n I'm particularly proud of this:\n \n``````\n",
    "created_at": "2017-07-07T08:47:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23382#issuecomment-353806",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Also add more doctests for errors.
+Some basic clean up of the matroid constructor. In particular, use normal variables like `groundset` instead of `kwds['groundset']`. Simplify the code and add more doctests for errors.
 
 I'm particularly proud of this:
 
``````




---

archive/issue_comments_353807.json:
```json
{
    "body": "Reviewer: **Stefan van Zwam**",
    "created_at": "2017-07-09T23:21:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23382#issuecomment-353807",
    "user": "https://github.com/sagetrac-Stefan"
}
```

Reviewer: **Stefan van Zwam**



---

archive/issue_events_307502.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-07-09T23:21:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23382#event-307502"
}
```



---

archive/issue_events_307503.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-07-09T23:21:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23382#event-307503"
}
```



---

archive/issue_comments_353808.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nClean code, all doctests pass. Nice work! It gets my positive review.",
    "created_at": "2017-07-09T23:22:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23382#issuecomment-353808",
    "user": "https://github.com/sagetrac-Stefan"
}
```

<a id='comment:7'>Comment 7:</a>
Clean code, all doctests pass. Nice work! It gets my positive review.



---

archive/issue_events_307504.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-26T22:12:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23382#event-307504"
}
```



---

archive/issue_events_307505.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d4445e33c85104526fdb7d10c35ac46aeb28d15b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-07-26T22:12:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23382#event-307505"
}
```



---

archive/issue_comments_353809.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/clean_up_matroid_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/clean_up_matroid_constructor)** to **[23a7260](https://github.com/sagemath/sagetrac-mirror/commit/23a7260e164e8adc1c37fc531f6cd32155501717)**",
    "created_at": "2017-07-26T22:12:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23382",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23382#issuecomment-353809",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/clean_up_matroid_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/clean_up_matroid_constructor)** to **[23a7260](https://github.com/sagemath/sagetrac-mirror/commit/23a7260e164e8adc1c37fc531f6cd32155501717)**
