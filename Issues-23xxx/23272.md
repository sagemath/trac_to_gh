...
```json
{
    "body": "I'm setting the status to needs review mainly to get people's attention. I assume there will be things that need to be changed.",
    "created_at": "2017-06-18T18:38:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23272",
    "type": "issue_comment",
...
```json
{
    "body": "I don't really like that we are just guessing `ATOM` as a fall-back when the build failed. I would prefer to just let the failed build be failed (possibly with an error message suggesting `OPENBLAS_CONFIGURE=\"TARGET=ATOM\"` or something).\n\nAlso, if you add a patch, you should bump the version number of openblas (from `0.2.19` to `0.2.19.p0`).",
...
```json
{
    "body": "PS: the patchlevel should be increased 0.2.19.p0 -> 0.2.19.p1 to trigger recompilation.",
...
```json
{
    "body": "Upgrading should also get rid of us of all the current patches except for `openblas-0.2.19-SMP_conditional.patch` (because the PR for it is still sitting on my HD - real life happened and it got forgotten).",
...
```json
{
    "body": "alive and kicking: https://github.com/xianyi/OpenBLAS/commits/develop",
...
```json
{
    "body": "Once the ticket is positively reviewed and merged, a copy of the tarball will be put on sagemath mirrors by the release manager.",
...
```json
{
    "body": "OK, I gather it's the official openblas release tarball (md5sum is right). I've modified the ticket description accordingly.",
...
```json
{
    "body": "You might want to wait till 0.2.21. See https://github.com/xianyi/OpenBLAS/issues/1258#issuecomment-320223121",
...
```json
{
    "body": "Release seems to be tardy. Possibly because they were moving to a 0.3.0 release. We could adopt https://github.com/xianyi/OpenBLAS/commit/63cfa32691680505e6b9daf0997755178ddd3144 instead.",
...
```json
{
    "body": "Ok, so according to the authors, v0.2.21 is likely not going to be coming out soon and that the only change is one patch which was added to master. Instead of rezipping everything, I've decided to just add this patch into our patches directory and upload that directly.\n\nEverything is working on my end and this ticket is now ready for review.",
...
```json
{
    "body": "I think this needs updating for openblas-0.2.19-cygwin.patch",
...
```json
{
    "body": "I had removed all of the patches except for one. If the cygwin patch is required I can re-add it in.",
...
```json
{
    "body": "It was recently added; please merge in the latest beta",
...
```json
{
    "body": "I just mean the latest develop branch, e.g. on trac: `git remote update trac && git merge trac/develop`",
...
```json
{
    "body": "I kept having problems merging in the cygwin patch (kept failing on my build for some reason) I fixed it and have merged again with the latest develop. It should be ready for testing now.",
...
```json
{
    "body": "OK, I'll increase the tolerance to 2e-03 there.",
...
```json
{
    "body": "This fixed the build under error I was having on Ubuntu 17.10. The /proc/cpuinfo identifies the CPU as:\n\nIntel(R) Core(TM) i7-7500U CPU `@` 2.70GHz",
...
```json
{
    "body": "(Minor fix to ticket description formatting.)",
