...
```json
{
    "body": "Here is a first implementation\n\n---\nNew commits:",
...
```json
{
    "body": "It looks like my implementation is similar to Martin's.  I think that adding features such as scalar product and change basis are for another ticket, but I agree that they would be useful.",
    "created_at": "2017-07-24T12:46:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23446",
    "type": "issue_comment",
...
```json
{
    "body": "Do you think it would be possible to also support the following?\n\n```\nsage: s = SymmetricFunctions(QQ).s()\nsage: X = tensor([s([1]), s([])])\nsage: Y = tensor([s([]), s([1])]);\nsage: X(s[2])\ns[2] # s[]\nsage: Y(s[2])\n1/2*s[] # s[1, 1] + 1/2*s[] # s[2] - 1/2*s[1, 1] # s[] + 1/2*s[2] # s[]\n```",
...
```json
{
    "body": "I would just implement a `TensorProducts` category for the Sym bases with the appropriate methods in the `ParentMethods`.",
...
```json
{
    "body": "Oh, I'm very sorry, that was autofill, probably!  Thanks for correcting!",
