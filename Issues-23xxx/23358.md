# Issue 23358: Don't embed rpaths during the build

archive/issues_023121.json:
```json
{
    "assignees": [],
    "body": "As per #22509, `DESTDIR` is useful in the cases where the build process needs to hard-code `--prefix` paths in the build output. For example, embedded rpaths. This is not only useful but vital for binary distros who can't write to /usr during a build.\n\nIt's less important - and one can get away with only supporting `--prefix` - if the build does not embed these paths. However nowadays `DESTDIR` is a common standard, so it's good to support it regardless.\n\nSince Sage 8.0 currently does not support DESTDIR (and neither did previous versions) I have to carry the following patch in Debian. It would be technically unnecessary after DESTDIR support is introduced, however in Debian we also have a policy to avoid rpath if possible.\n\nEven disregarding Debian, there is no particular reason for Sage to go embedding rpaths in its binaries, it prevents SAGE_LOCAL from being moved to different places around the filesystem. AFAICS, no other part of Sage involves embedding build paths, and one can freely move a SAGE_LOCAL around after applying this patch.\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23358_\n\n",
    "closed_at": "2017-07-04T12:44:57Z",
    "created_at": "2017-07-03T22:16:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Don't embed rpaths during the build",
    "type": "issue",
    "updated_at": "2017-07-04T12:44:57Z",
    "url": "https://github.com/sagemath/sage/issues/23358",
    "user": "https://github.com/infinity0"
}
```
As per #22509, `DESTDIR` is useful in the cases where the build process needs to hard-code `--prefix` paths in the build output. For example, embedded rpaths. This is not only useful but vital for binary distros who can't write to /usr during a build.

It's less important - and one can get away with only supporting `--prefix` - if the build does not embed these paths. However nowadays `DESTDIR` is a common standard, so it's good to support it regardless.

Since Sage 8.0 currently does not support DESTDIR (and neither did previous versions) I have to carry the following patch in Debian. It would be technically unnecessary after DESTDIR support is introduced, however in Debian we also have a policy to avoid rpath if possible.

Even disregarding Debian, there is no particular reason for Sage to go embedding rpaths in its binaries, it prevents SAGE_LOCAL from being moved to different places around the filesystem. AFAICS, no other part of Sage involves embedding build paths, and one can freely move a SAGE_LOCAL around after applying this patch.

_Issue created by migration from https://trac.sagemath.org/ticket/23358_





---

archive/issue_events_307234.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:16:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307234"
}
```



---

archive/issue_events_307235.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:16:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307235"
}
```



---

archive/issue_events_307236.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:25:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307236"
}
```



---

archive/issue_events_307237.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:25:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307237"
}
```



---

archive/issue_events_307238.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:25:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307238"
}
```



---

archive/issue_comments_353486.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,7 @@\n+As per #22509, `DESTDIR` is useful in the cases where the build process needs to hard-code `--prefix` paths in the build output. For example, embedded rpaths. This is not only useful but vital for binary distros who can't write to /usr during a build.\n \n+It's less important - and one can get away with only supporting `--prefix` - if the build does not embed these paths. However nowadays `DESTDIR` is a common standard, so it's good to support it regardless.\n+\n+Since Sage 8.0 currently does not support DESTDIR (and neither did previous versions) I have to carry the following patch in Debian. It would be technically unnecessary after DESTDIR support is introduced, however in Debian we also have a policy to avoid rpath if possible.\n+\n+Even disregarding Debian, there is no particular reason for Sage to go embedding rpaths in its binaries, it prevents SAGE_LOCAL from being moved to different places around the filesystem. AFAICS, no other part of Sage involves embedding build paths, and one can freely move a SAGE_LOCAL around after applying this patch.\n``````\n",
    "created_at": "2017-07-03T22:25:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23358#issuecomment-353486",
    "user": "https://github.com/infinity0"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,7 @@
+As per #22509, `DESTDIR` is useful in the cases where the build process needs to hard-code `--prefix` paths in the build output. For example, embedded rpaths. This is not only useful but vital for binary distros who can't write to /usr during a build.
 
+It's less important - and one can get away with only supporting `--prefix` - if the build does not embed these paths. However nowadays `DESTDIR` is a common standard, so it's good to support it regardless.
+
+Since Sage 8.0 currently does not support DESTDIR (and neither did previous versions) I have to carry the following patch in Debian. It would be technically unnecessary after DESTDIR support is introduced, however in Debian we also have a policy to avoid rpath if possible.
+
+Even disregarding Debian, there is no particular reason for Sage to go embedding rpaths in its binaries, it prevents SAGE_LOCAL from being moved to different places around the filesystem. AFAICS, no other part of Sage involves embedding build paths, and one can freely move a SAGE_LOCAL around after applying this patch.
``````




---

archive/issue_events_307239.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:25:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307239"
}
```



---

archive/issue_events_307240.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:25:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307240"
}
```



---

archive/issue_comments_353487.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nSorry, please delete this ticket, I messed up when running `git trac`. The correct ticket is #23359.",
    "created_at": "2017-07-03T22:26:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23358#issuecomment-353487",
    "user": "https://github.com/infinity0"
}
```

<div id="comment:2" align="right">Comment 2</div>

Sorry, please delete this ticket, I messed up when running `git trac`. The correct ticket is #23359.



---

archive/issue_events_307241.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:26:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307241"
}
```



---

archive/issue_events_307242.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:26:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307242"
}
```



---

archive/issue_events_307243.json:
```json
{
    "actor": "https://github.com/infinity0",
    "created_at": "2017-07-03T22:26:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307243"
}
```



---

archive/issue_events_307244.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T12:44:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307244"
}
```



---

archive/issue_events_307245.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T12:44:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23358",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23358#event-307245"
}
```
