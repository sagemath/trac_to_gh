# Issue 23523: Fix subtraction of affine/loop Lie algebra elements

archive/issues_023286.json:
```json
{
    "assignees": [],
    "body": "The `axpy` input was switched, so subtraction was backwards:\n\n```\nsage: L = lie_algebras.Affine(QQ, ['A',1,1])\nsage: e1,f1,h1,e0,f0,c,d = list(L.lie_algebra_generators())\nsage: e1 - c\n(-E[alpha[1]])#t^0 + -1*c\n```\n\n\nCC:  @bsalisbury1 @fchapoton\n\nKeywords: **lie algebras**\n\nBranch/Commit: **[1e56d9a](https://github.com/sagemath/sagetrac-mirror/commit/1e56d9a47bac3a7ab2de1bf20a0e56f72f0f79f1)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23523_\n\n",
    "closed_at": "2017-07-29T19:27:05Z",
    "created_at": "2017-07-23T03:17:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix subtraction of affine/loop Lie algebra elements",
    "type": "issue",
    "updated_at": "2017-07-29T19:27:05Z",
    "url": "https://github.com/sagemath/sage/issues/23523",
    "user": "https://github.com/tscrim"
}
```
The `axpy` input was switched, so subtraction was backwards:

```
sage: L = lie_algebras.Affine(QQ, ['A',1,1])
sage: e1,f1,h1,e0,f0,c,d = list(L.lie_algebra_generators())
sage: e1 - c
(-E[alpha[1]])#t^0 + -1*c
```


CC:  @bsalisbury1 @fchapoton

Keywords: **lie algebras**

Branch/Commit: **[1e56d9a](https://github.com/sagemath/sagetrac-mirror/commit/1e56d9a47bac3a7ab2de1bf20a0e56f72f0f79f1)**

Reviewer: **Frédéric Chapoton**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/23523_





---

archive/issue_events_309293.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-23T03:17:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23523#event-309293"
}
```



---

archive/issue_events_309294.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-23T03:17:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23523#event-309294"
}
```



---

archive/issue_events_309295.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-23T03:17:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23523#event-309295"
}
```



---

archive/issue_events_309296.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-23T03:17:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23523#event-309296"
}
```



---

archive/issue_comments_356480.json:
```json
{
    "body": "Branch: **[public/lie_algebras/loop_addition-23523](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/loop_addition-23523)**",
    "created_at": "2017-07-23T03:19:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23523#issuecomment-356480",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/lie_algebras/loop_addition-23523](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/loop_addition-23523)**



---

archive/issue_comments_356481.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,9 @@\n-The `axpy` input was switched, so subtraction was backwards.\n+The `axpy` input was switched, so subtraction was backwards:\n+\n+```\n+sage: L = lie_algebras.Affine(QQ, ['A',1,1])\n+sage: e1,f1,h1,e0,f0,c,d = list(L.lie_algebra_generators())\n+sage: e1 - c\n+(-E[alpha[1]])#t^0 + -1*c\n+```\n+\n``````\n",
    "created_at": "2017-07-23T03:19:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23523#issuecomment-356481",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,9 @@
-The `axpy` input was switched, so subtraction was backwards.
+The `axpy` input was switched, so subtraction was backwards:
+
+```
+sage: L = lie_algebras.Affine(QQ, ['A',1,1])
+sage: e1,f1,h1,e0,f0,c,d = list(L.lie_algebra_generators())
+sage: e1 - c
+(-E[alpha[1]])#t^0 + -1*c
+```
+
``````




---

archive/issue_comments_356482.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\n*facepalm*\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1e56d9a47bac3a7ab2de1bf20a0e56f72f0f79f1\">1e56d9a</a></td><td><code>Fixing subtraction for loop/affine Lie algebra elements.</code></td></tr></table>\n",
    "created_at": "2017-07-23T03:19:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23523#issuecomment-356482",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
*facepalm*

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1e56d9a47bac3a7ab2de1bf20a0e56f72f0f79f1">1e56d9a</a></td><td><code>Fixing subtraction for loop/affine Lie algebra elements.</code></td></tr></table>




---

archive/issue_events_309297.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-23T03:19:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23523#event-309297"
}
```



---

archive/issue_comments_356483.json:
```json
{
    "body": "Commit: **[1e56d9a](https://github.com/sagemath/sagetrac-mirror/commit/1e56d9a47bac3a7ab2de1bf20a0e56f72f0f79f1)**",
    "created_at": "2017-07-23T03:19:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23523#issuecomment-356483",
    "user": "https://github.com/tscrim"
}
```

Commit: **[1e56d9a](https://github.com/sagemath/sagetrac-mirror/commit/1e56d9a47bac3a7ab2de1bf20a0e56f72f0f79f1)**



---

archive/issue_comments_356484.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2017-07-23T07:12:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23523#issuecomment-356484",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_309298.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-07-23T07:12:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23523#event-309298"
}
```



---

archive/issue_events_309299.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-07-23T07:12:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23523#event-309299"
}
```



---

archive/issue_comments_356485.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nok, let it be",
    "created_at": "2017-07-23T07:12:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23523#issuecomment-356485",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
ok, let it be



---

archive/issue_comments_356486.json:
```json
{
    "body": "Changed branch from **[public/lie_algebras/loop_addition-23523](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/loop_addition-23523)** to **[1e56d9a](https://github.com/sagemath/sagetrac-mirror/commit/1e56d9a47bac3a7ab2de1bf20a0e56f72f0f79f1)**",
    "created_at": "2017-07-29T19:27:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23523#issuecomment-356486",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/lie_algebras/loop_addition-23523](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/loop_addition-23523)** to **[1e56d9a](https://github.com/sagemath/sagetrac-mirror/commit/1e56d9a47bac3a7ab2de1bf20a0e56f72f0f79f1)**



---

archive/issue_events_309300.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-29T19:27:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23523#event-309300"
}
```



---

archive/issue_events_309301.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "175e067fb6950537cd84894488e9693be9d926f2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-07-29T19:27:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23523",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23523#event-309301"
}
```
