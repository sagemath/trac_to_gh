# Issue 23291: Fix segfault in PCRE when using the JIT compiler feature on Cygwin

archive/issues_023054.json:
```json
{
    "assignees": [],
    "body": "This bug in PCRE on Cygwin causes R to crash during building; see [#23067 comment:10](https://github.com/sagemath/sage/issues/23067#comment:10) and subsequent comments.\n\nKeywords: **windows cygwin r pcre**\n\nBranch/Commit: **[5ac9c28](https://github.com/sagemath/sagetrac-mirror/commit/5ac9c28e65366771c7ca4bce2f2b42b183b53264)**\n\nReviewer: **Jean-Pierre Flori**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23291_\n\n",
    "closed_at": "2017-06-22T07:23:43Z",
    "created_at": "2017-06-21T08:45:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix segfault in PCRE when using the JIT compiler feature on Cygwin",
    "type": "issue",
    "updated_at": "2017-06-22T07:23:43Z",
    "url": "https://github.com/sagemath/sage/issues/23291",
    "user": "https://github.com/embray"
}
```
This bug in PCRE on Cygwin causes R to crash during building; see [#23067 comment:10](https://github.com/sagemath/sage/issues/23067#comment:10) and subsequent comments.

Keywords: **windows cygwin r pcre**

Branch/Commit: **[5ac9c28](https://github.com/sagemath/sagetrac-mirror/commit/5ac9c28e65366771c7ca4bce2f2b42b183b53264)**

Reviewer: **Jean-Pierre Flori**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/23291_





---

archive/issue_events_306347.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-06-21T08:45:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23291#event-306347"
}
```



---

archive/issue_events_306348.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-06-21T08:45:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23291#event-306348"
}
```



---

archive/issue_events_306349.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-06-21T08:45:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23291#event-306349"
}
```



---

archive/issue_events_306350.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-06-21T08:45:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23291#event-306350"
}
```



---

archive/issue_comments_352166.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5ac9c28e65366771c7ca4bce2f2b42b183b53264\">5ac9c28</a></td><td><code>Add patch to fix segfault in PCRE when using the JIT compiler feature on Cygwin</code></td></tr></table>\n",
    "created_at": "2017-06-21T08:51:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23291#issuecomment-352166",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5ac9c28e65366771c7ca4bce2f2b42b183b53264">5ac9c28</a></td><td><code>Add patch to fix segfault in PCRE when using the JIT compiler feature on Cygwin</code></td></tr></table>




---

archive/issue_comments_352167.json:
```json
{
    "body": "Author: **Erik Bray**",
    "created_at": "2017-06-21T08:51:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23291#issuecomment-352167",
    "user": "https://github.com/embray"
}
```

Author: **Erik Bray**



---

archive/issue_events_306351.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-06-21T08:51:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23291#event-306351"
}
```



---

archive/issue_comments_352168.json:
```json
{
    "body": "Commit: **[5ac9c28](https://github.com/sagemath/sagetrac-mirror/commit/5ac9c28e65366771c7ca4bce2f2b42b183b53264)**",
    "created_at": "2017-06-21T08:51:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23291#issuecomment-352168",
    "user": "https://github.com/embray"
}
```

Commit: **[5ac9c28](https://github.com/sagemath/sagetrac-mirror/commit/5ac9c28e65366771c7ca4bce2f2b42b183b53264)**



---

archive/issue_comments_352169.json:
```json
{
    "body": "Branch: **[u/embray/cygwin/ticket-23291](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/ticket-23291)**",
    "created_at": "2017-06-21T08:51:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23291#issuecomment-352169",
    "user": "https://github.com/embray"
}
```

Branch: **[u/embray/cygwin/ticket-23291](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/ticket-23291)**



---

archive/issue_comments_352170.json:
```json
{
    "body": "Changed keywords from **** to **windows cygwin r pcre**",
    "created_at": "2017-06-21T08:51:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23291#issuecomment-352170",
    "user": "https://github.com/embray"
}
```

Changed keywords from **** to **windows cygwin r pcre**



---

archive/issue_events_306352.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2017-06-21T08:55:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23291#event-306352"
}
```



---

archive/issue_events_306353.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2017-06-21T08:55:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23291#event-306353"
}
```



---

archive/issue_comments_352171.json:
```json
{
    "body": "Reviewer: **Jean-Pierre Flori**",
    "created_at": "2017-06-21T08:55:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23291#issuecomment-352171",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Reviewer: **Jean-Pierre Flori**



---

archive/issue_comments_352172.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPatch looks sensible and windows/cygwin affecting only.",
    "created_at": "2017-06-21T08:55:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23291#issuecomment-352172",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:2'>Comment 2:</a>
Patch looks sensible and windows/cygwin affecting only.



---

archive/issue_events_306354.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-22T07:23:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23291#event-306354"
}
```



---

archive/issue_events_306355.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "82814739a66ac3b071d9ac72d8f668e30f44140a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-06-22T07:23:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23291#event-306355"
}
```



---

archive/issue_comments_352173.json:
```json
{
    "body": "Changed branch from **[u/embray/cygwin/ticket-23291](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/ticket-23291)** to **[5ac9c28](https://github.com/sagemath/sagetrac-mirror/commit/5ac9c28e65366771c7ca4bce2f2b42b183b53264)**",
    "created_at": "2017-06-22T07:23:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23291#issuecomment-352173",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/cygwin/ticket-23291](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/ticket-23291)** to **[5ac9c28](https://github.com/sagemath/sagetrac-mirror/commit/5ac9c28e65366771c7ca4bce2f2b42b183b53264)**
