# Issue 23359: Don't embed rpaths during the build

archive/issues_023122.json:
```json
{
    "body": "As per #22509, `DESTDIR` is useful in the cases where the build process needs to hard-code `--prefix` paths in the build output. For example, embedded rpaths. This is not only useful but vital for binary distros who can't write to /usr during a build.\n\nIt's less important - and one can get away with only supporting `--prefix` - if the build does not embed these paths. However nowadays `DESTDIR` is a common standard, so it's good to support it regardless.\n\nSince Sage 8.0 currently does not support DESTDIR (and neither did previous versions) I have to carry the following patch in Debian. It would be technically unnecessary after DESTDIR support is introduced, however in Debian we also have a policy to avoid rpath if possible.\n\nEven disregarding Debian, there is no particular reason for Sage to go embedding rpaths in its binaries, it prevents SAGE_LOCAL from being moved to different places around the filesystem. AFAICS, no other part of Sage involves embedding build paths, and one can freely move a SAGE_LOCAL around after applying this patch.\n\n\nAuthor: Ximin Luo\n\nBranch: u/infinity0/don_t_embed_rpaths_during_the_build\n\nStatus: needs_info\n\nCommit: d3d0f46c33eb5cd230247a30ecef4e7ea33b70b5\n\nIssue created by migration from https://trac.sagemath.org/ticket/23359\n\n",
    "created_at": "2017-07-03T22:17:44Z",
    "labels": [
        "component: build",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.0",
    "title": "Don't embed rpaths during the build",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/23359",
    "user": "https://github.com/infinity0"
}
```
As per #22509, `DESTDIR` is useful in the cases where the build process needs to hard-code `--prefix` paths in the build output. For example, embedded rpaths. This is not only useful but vital for binary distros who can't write to /usr during a build.

It's less important - and one can get away with only supporting `--prefix` - if the build does not embed these paths. However nowadays `DESTDIR` is a common standard, so it's good to support it regardless.

Since Sage 8.0 currently does not support DESTDIR (and neither did previous versions) I have to carry the following patch in Debian. It would be technically unnecessary after DESTDIR support is introduced, however in Debian we also have a policy to avoid rpath if possible.

Even disregarding Debian, there is no particular reason for Sage to go embedding rpaths in its binaries, it prevents SAGE_LOCAL from being moved to different places around the filesystem. AFAICS, no other part of Sage involves embedding build paths, and one can freely move a SAGE_LOCAL around after applying this patch.


Author: Ximin Luo

Branch: u/infinity0/don_t_embed_rpaths_during_the_build

Status: needs_info

Commit: d3d0f46c33eb5cd230247a30ecef4e7ea33b70b5

Issue created by migration from https://trac.sagemath.org/ticket/23359





---

archive/issue_comments_439015.json:
```json
{
    "body": "Changing branch from \"\" to \"u/infinity0/don_t_embed_rpaths_during_the_build\"",
    "created_at": "2017-07-03T22:21:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439015",
    "user": "https://github.com/infinity0"
}
```

Changing branch from "" to "u/infinity0/don_t_embed_rpaths_during_the_build"



---

archive/issue_comments_439016.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-07-03T22:25:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439016",
    "user": "https://github.com/infinity0"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_439017.json:
```json
{
    "body": "<a id='comment:2'></a>New commits:",
    "created_at": "2017-07-03T22:25:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439017",
    "user": "https://github.com/infinity0"
}
```

<a id='comment:2'></a>New commits:



---

archive/issue_comments_439018.json:
```json
{
    "body": "Changing priority from major to minor.",
    "created_at": "2017-07-03T22:25:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439018",
    "user": "https://github.com/infinity0"
}
```

Changing priority from major to minor.



---

archive/issue_comments_439019.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to enhancement.",
    "created_at": "2017-07-03T22:25:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439019",
    "user": "https://github.com/infinity0"
}
```

Changing type from PLEASE CHANGE to enhancement.



---

archive/issue_comments_439020.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to build.",
    "created_at": "2017-07-03T22:25:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439020",
    "user": "https://github.com/infinity0"
}
```

Changing component from PLEASE CHANGE to build.



---

archive/issue_comments_439021.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,8 @@\n+As per #22509, `DESTDIR` is useful in the cases where the build process needs to hard-code `--prefix` paths in the build output. For example, embedded rpaths. This is not only useful but vital for binary distros who can't write to /usr during a build.\n \n+It's less important - and one can get away with only supporting `--prefix` - if the build does not embed these paths. However nowadays `DESTDIR` is a common standard, so it's good to support it regardless.\n+\n+Since Sage 8.0 currently does not support DESTDIR (and neither did previous versions) I have to carry the following patch in Debian. It would be technically unnecessary after DESTDIR support is introduced, however in Debian we also have a policy to avoid rpath if possible.\n+\n+Even disregarding Debian, there is no particular reason for Sage to go embedding rpaths in its binaries, it prevents SAGE_LOCAL from being moved to different places around the filesystem. AFAICS, no other part of Sage involves embedding build paths, and one can freely move a SAGE_LOCAL around after applying this patch.\n+\n``````\n",
    "created_at": "2017-07-03T22:25:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439021",
    "user": "https://github.com/infinity0"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,8 @@
+As per #22509, `DESTDIR` is useful in the cases where the build process needs to hard-code `--prefix` paths in the build output. For example, embedded rpaths. This is not only useful but vital for binary distros who can't write to /usr during a build.
 
+It's less important - and one can get away with only supporting `--prefix` - if the build does not embed these paths. However nowadays `DESTDIR` is a common standard, so it's good to support it regardless.
+
+Since Sage 8.0 currently does not support DESTDIR (and neither did previous versions) I have to carry the following patch in Debian. It would be technically unnecessary after DESTDIR support is introduced, however in Debian we also have a policy to avoid rpath if possible.
+
+Even disregarding Debian, there is no particular reason for Sage to go embedding rpaths in its binaries, it prevents SAGE_LOCAL from being moved to different places around the filesystem. AFAICS, no other part of Sage involves embedding build paths, and one can freely move a SAGE_LOCAL around after applying this patch.
+
``````




---

archive/issue_comments_439022.json:
```json
{
    "body": "Changing commit from \"\" to \"d3d0f46c33eb5cd230247a30ecef4e7ea33b70b5\"",
    "created_at": "2017-07-03T22:25:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439022",
    "user": "https://github.com/infinity0"
}
```

Changing commit from "" to "d3d0f46c33eb5cd230247a30ecef4e7ea33b70b5"



---

archive/issue_comments_439023.json:
```json
{
    "body": "Changing author from \"\" to \"Ximin Luo\"",
    "created_at": "2017-07-03T22:26:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439023",
    "user": "https://github.com/infinity0"
}
```

Changing author from "" to "Ximin Luo"



---

archive/issue_comments_439024.json:
```json
{
    "body": "<a id='comment:4'></a>Thou shall not use sage's provided `sage-env` in sage-on-distro. Seriously don't :) I am even wondering why I contribute to it.\n\nSetting rpath is sage as an arbitrary prefixed package's way to find its own stuff. Historically sage was using `LD_LIBRARY_PATH` or equivalent. Which was also set in `sage-env`.... To be clear if you remove the rpath and sage is not installed in system path, which is what happens for vanilla installs, someone will have to put `LD_LIBRARY_PATH` back.\n\nWhich goes back to my point. My recommendation for fellow distros is just bring your own `sage-env`, don't burden yourself with that bag of seemingly random stuff (it used to be worse much worse). It is just toxic to distros in this shape. May be if we rebuilt it from scratch using autotools it would be more palatable but that's much more than your small change here.",
    "created_at": "2017-07-03T23:36:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439024",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:4'></a>Thou shall not use sage's provided `sage-env` in sage-on-distro. Seriously don't :) I am even wondering why I contribute to it.

Setting rpath is sage as an arbitrary prefixed package's way to find its own stuff. Historically sage was using `LD_LIBRARY_PATH` or equivalent. Which was also set in `sage-env`.... To be clear if you remove the rpath and sage is not installed in system path, which is what happens for vanilla installs, someone will have to put `LD_LIBRARY_PATH` back.

Which goes back to my point. My recommendation for fellow distros is just bring your own `sage-env`, don't burden yourself with that bag of seemingly random stuff (it used to be worse much worse). It is just toxic to distros in this shape. May be if we rebuilt it from scratch using autotools it would be more palatable but that's much more than your small change here.



---

archive/issue_comments_439025.json:
```json
{
    "body": "<a id='comment:5'></a>Now Looking more closely I see you re-introduce `LD_LIBRARY_PATH` (which is linux only, use DYLD_LIBRARY_PATH on OS X please). You could say the debate between rpath and LD_LIBRARY_PATH has already happened and a shift from one to the other was made.\n\nSo you want them to switch back... I am not sure what to say to you about that.",
    "created_at": "2017-07-03T23:57:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439025",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:5'></a>Now Looking more closely I see you re-introduce `LD_LIBRARY_PATH` (which is linux only, use DYLD_LIBRARY_PATH on OS X please). You could say the debate between rpath and LD_LIBRARY_PATH has already happened and a shift from one to the other was made.

So you want them to switch back... I am not sure what to say to you about that.



---

archive/issue_comments_439026.json:
```json
{
    "body": "<a id='comment:6'></a>We don't ship Sage's own `sage-env`, we took that tip off Gentoo a while ago. :) However we build with it for various reasons. For example, Sage needs access to `SAGE_SRC` during the build, and various other things we don't put in our own `sage-env`.\n\nIt seems [OpenBSD](https://man.openbsd.org/ldconfig) and [FreeBSD](https://www.freebsd.org/cgi/man.cgi?query=ld.so) do support `LD_LIBRARY_PATH`. I'm not sure when they added this, perhaps FreeBSD means Mac OS X now also supports it. Do you have a link to any threads on this topic? Maybe it's a good time for Sage to revisit this issue.\n\nAs I said, the main advantage is that `SAGE_LOCAL` can be moved around. It's also useful for reproducible builds, which last time I checked, Sage was very close to achieving - there was only some Cython issue with embedded paths.",
    "created_at": "2017-07-04T07:32:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439026",
    "user": "https://github.com/infinity0"
}
```

<a id='comment:6'></a>We don't ship Sage's own `sage-env`, we took that tip off Gentoo a while ago. :) However we build with it for various reasons. For example, Sage needs access to `SAGE_SRC` during the build, and various other things we don't put in our own `sage-env`.

It seems [OpenBSD](https://man.openbsd.org/ldconfig) and [FreeBSD](https://www.freebsd.org/cgi/man.cgi?query=ld.so) do support `LD_LIBRARY_PATH`. I'm not sure when they added this, perhaps FreeBSD means Mac OS X now also supports it. Do you have a link to any threads on this topic? Maybe it's a good time for Sage to revisit this issue.

As I said, the main advantage is that `SAGE_LOCAL` can be moved around. It's also useful for reproducible builds, which last time I checked, Sage was very close to achieving - there was only some Cython issue with embedded paths.



---

archive/issue_comments_439027.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2017-07-04T07:56:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439027",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_439028.json:
```json
{
    "body": "<a id='comment:7'></a>There have been many discussions in the past about this... I don't know much about dynamic linking myself, but I do know that there are reasons why Sage uses rpaths.\n\nAt least this ticket should be discussed on sage-devel.",
    "created_at": "2017-07-04T07:56:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439028",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>There have been many discussions in the past about this... I don't know much about dynamic linking myself, but I do know that there are reasons why Sage uses rpaths.

At least this ticket should be discussed on sage-devel.



---

archive/issue_comments_439029.json:
```json
{
    "body": "<a id='comment:8'></a>Replying to [infinity0](#comment%3A6):\n> We don't ship Sage's own `sage-env`, we took that tip off Gentoo a while ago. :) However we build with it for various reasons. For example, Sage needs access to `SAGE_SRC` during the build, and various other things we don't put in our own `sage-env`.\n> \n\n\nYou can use `sage-env` for that but really during the build of \"sagelib\" most thing are coming from `sage/env.py`. Those value can be overridden by the environment which is the main reason my ebuild has the following section\n\n```\n\texport SAGE_LOCAL=\"${EPREFIX}\"/usr\n\texport SAGE_ROOT=`pwd`/..\n\texport SAGE_SRC=`pwd`\n\texport SAGE_ETC=`pwd`/bin\n\texport SAGE_DOC=`pwd`/build_doc\n\texport SAGE_DOC_SRC=`pwd`/doc\n\texport SAGE_DOC_MATHJAX=yes\n\texport VARTEXFONTS=\"${T}\"/fonts\n\texport SAGE_VERSION=${PV}\n\texport SAGE_NUM_THREADS=$(makeopts_jobs)\n```\n`pwd` in that context is the `src` folder from the sage tarball. The build takes its cues from there.\n\nOtherwise I agree with Jeroen. A flip back of this magnitude has to be discussed on sage-devel. Having an experience on HPC and various package managers which build software stack on top on an OS I fall heavily in the rpath camp. No solution to this problem is 100% perfect, but rpath now does it for me. Sage as a software stack falls in that category. This is of course different from the \"sagelib\" component itself, which should be free to have a life independent of the sage packaging system.",
    "created_at": "2017-07-04T08:24:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23359#issuecomment-439029",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:8'></a>Replying to [infinity0](#comment%3A6):
> We don't ship Sage's own `sage-env`, we took that tip off Gentoo a while ago. :) However we build with it for various reasons. For example, Sage needs access to `SAGE_SRC` during the build, and various other things we don't put in our own `sage-env`.
> 


You can use `sage-env` for that but really during the build of "sagelib" most thing are coming from `sage/env.py`. Those value can be overridden by the environment which is the main reason my ebuild has the following section

```
	export SAGE_LOCAL="${EPREFIX}"/usr
	export SAGE_ROOT=`pwd`/..
	export SAGE_SRC=`pwd`
	export SAGE_ETC=`pwd`/bin
	export SAGE_DOC=`pwd`/build_doc
	export SAGE_DOC_SRC=`pwd`/doc
	export SAGE_DOC_MATHJAX=yes
	export VARTEXFONTS="${T}"/fonts
	export SAGE_VERSION=${PV}
	export SAGE_NUM_THREADS=$(makeopts_jobs)
```
`pwd` in that context is the `src` folder from the sage tarball. The build takes its cues from there.

Otherwise I agree with Jeroen. A flip back of this magnitude has to be discussed on sage-devel. Having an experience on HPC and various package managers which build software stack on top on an OS I fall heavily in the rpath camp. No solution to this problem is 100% perfect, but rpath now does it for me. Sage as a software stack falls in that category. This is of course different from the "sagelib" component itself, which should be free to have a life independent of the sage packaging system.
