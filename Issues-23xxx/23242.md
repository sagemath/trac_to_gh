# Issue 23242: Allow `make ptest*` to honor SAGE_NUM_THREADS

archive/issues_023005.json:
```json
{
    "body": "This change simply passes the value of `SAGE_NUM_THREADS` for the `-p` flag when running `make ptest` and similar targets.\n\nReviewer: Travis Scrimshaw\n\nAuthor: Erik Bray\n\nBranch: u/embray/build/ptest_num_threads\n\nCommit: bbe187b3ef71be9546187ed750cf8750561f1156\n\nResolution: worksforme\n\nIssue created by migration from https://trac.sagemath.org/ticket/23242\n\n",
    "closed_at": "2017-06-15T15:47:25Z",
    "created_at": "2017-06-15T08:08:45Z",
    "labels": [
        "component: build",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Allow `make ptest*` to honor SAGE_NUM_THREADS",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/23242",
    "user": "https://github.com/embray"
}
```
This change simply passes the value of `SAGE_NUM_THREADS` for the `-p` flag when running `make ptest` and similar targets.

Reviewer: Travis Scrimshaw

Author: Erik Bray

Branch: u/embray/build/ptest_num_threads

Commit: bbe187b3ef71be9546187ed750cf8750561f1156

Resolution: worksforme

Issue created by migration from https://trac.sagemath.org/ticket/23242





---

archive/issue_comments_436638.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-06-15T08:08:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436638",
    "user": "https://github.com/embray"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_436639.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2017-06-15T08:14:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436639",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_436640.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-06-15T08:14:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436640",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_436641.json:
```json
{
    "body": "<a id='comment:2'></a>Sounds reasonable, especially since there is no other way to pass such a parameter IIRC.",
    "created_at": "2017-06-15T08:14:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436641",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'></a>Sounds reasonable, especially since there is no other way to pass such a parameter IIRC.



---

archive/issue_comments_436642.json:
```json
{
    "body": "<a id='comment:3'></a>Which problem does this fix? `SAGE_NUM_THREADS` already works without this patch.",
    "created_at": "2017-06-15T13:19:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436642",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>Which problem does this fix? `SAGE_NUM_THREADS` already works without this patch.



---

archive/issue_comments_436643.json:
```json
{
    "body": "Changing status from positive_review to needs_info.",
    "created_at": "2017-06-15T13:19:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436643",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from positive_review to needs_info.



---

archive/issue_comments_436644.json:
```json
{
    "body": "<a id='comment:5'></a>Well it looks to me like this is all a bit of a mess really.  According to the docs:\n\n> `SAGE_NUM_THREADS` - if set to a number, then when building the documentation, parallel doctesting, or running `sage -b`, use this many threads. If this is not set, then determine the number of threads using the value of the `MAKE` (see above) or `MAKEFLAGS` environment variables. If none of these specifies a number of jobs, use one thread (except for parallel testing: there we use a default of the number of CPU cores, with a maximum of 8 and a minimum of 2).\n\n\nBut in reality `sage-runtests` doesn't use any of this, and instead checks `SAGE_NUM_THREADS_PARALLEL` which is set in `sage-env` using the `sage-num-threads.py` script (???).  `SAGE_NUM_THREADS_PARALLEL` isn't documented anywhere, much less as the way to set the default number of processes to use for sage-runtests -p`.\n\nI don't understand what the distinction between `SAGE_NUM_THREADS` and `SAGE_NUM_THREADS_PARALLEL` is supposed to be (the use of \"threads\" is a bit misleading too, though you could at least argue that each process has a main thread...).\n\nPoint being, there's no well-documented way to control how many processes to use when running `make ptest`, and this adds at least *a* way that's more or less consistent with what's documented.",
    "created_at": "2017-06-15T14:04:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436644",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>Well it looks to me like this is all a bit of a mess really.  According to the docs:

> `SAGE_NUM_THREADS` - if set to a number, then when building the documentation, parallel doctesting, or running `sage -b`, use this many threads. If this is not set, then determine the number of threads using the value of the `MAKE` (see above) or `MAKEFLAGS` environment variables. If none of these specifies a number of jobs, use one thread (except for parallel testing: there we use a default of the number of CPU cores, with a maximum of 8 and a minimum of 2).


But in reality `sage-runtests` doesn't use any of this, and instead checks `SAGE_NUM_THREADS_PARALLEL` which is set in `sage-env` using the `sage-num-threads.py` script (???).  `SAGE_NUM_THREADS_PARALLEL` isn't documented anywhere, much less as the way to set the default number of processes to use for sage-runtests -p`.

I don't understand what the distinction between `SAGE_NUM_THREADS` and `SAGE_NUM_THREADS_PARALLEL` is supposed to be (the use of "threads" is a bit misleading too, though you could at least argue that each process has a main thread...).

Point being, there's no well-documented way to control how many processes to use when running `make ptest`, and this adds at least *a* way that's more or less consistent with what's documented.



---

archive/issue_comments_436645.json:
```json
{
    "body": "Resolution: worksforme",
    "created_at": "2017-06-15T15:47:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436645",
    "user": "https://github.com/embray"
}
```

Resolution: worksforme



---

archive/issue_comments_436646.json:
```json
{
    "body": "<a id='comment:6'></a>I gave it another try without this patch, and it looks like this isn't needed after all as Jeroen suggested.  The current behavior *is* right.  Convoluted and hard to trace, but it does seem to work.",
    "created_at": "2017-06-15T15:47:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436646",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'></a>I gave it another try without this patch, and it looks like this isn't needed after all as Jeroen suggested.  The current behavior *is* right.  Convoluted and hard to trace, but it does seem to work.



---

archive/issue_events_059938.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-06-15T15:47:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/23242#event-59938"
}
```



---

archive/issue_events_059939.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-06-15T15:47:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/23242#event-59939"
}
```



---

archive/issue_comments_436647.json:
```json
{
    "body": "<a id='comment:7'></a>Replying to [embray](#comment%3A6):\n> Convoluted and hard to trace\n\n\nI don't think it's so bad. The script `sage-num-threads.py` is where all decisions regarding number of threads/processes are made. Other parts of Sage (like the doctester) use the output of that script. I think the script itself is reasonably documented.",
    "created_at": "2017-06-16T07:49:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436647",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>Replying to [embray](#comment%3A6):
> Convoluted and hard to trace


I don't think it's so bad. The script `sage-num-threads.py` is where all decisions regarding number of threads/processes are made. Other parts of Sage (like the doctester) use the output of that script. I think the script itself is reasonably documented.



---

archive/issue_comments_436648.json:
```json
{
    "body": "<a id='comment:8'></a>I think the script itself is not so bad, but it's not obvious if you don't know about it how these values are getting set.  There's a disconnect between the docs (e.g. \"use `SAGE_NUM_THREADS`\") and things like the implementation of the test runner (\"default to `SAGE_NUM_THREADS_PARALLEL`\"--which leads to \"where the heck did that come from?).  That is, just reading the code doesn't suggest any obvious path for this value is actually getting set because it happens extrinsically when setting up `sage-env`.\n\nNow that I understand, I don't think it's so bad either.  Just not obvious.",
    "created_at": "2017-06-16T08:43:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23242#issuecomment-436648",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>I think the script itself is not so bad, but it's not obvious if you don't know about it how these values are getting set.  There's a disconnect between the docs (e.g. "use `SAGE_NUM_THREADS`") and things like the implementation of the test runner ("default to `SAGE_NUM_THREADS_PARALLEL`"--which leads to "where the heck did that come from?).  That is, just reading the code doesn't suggest any obvious path for this value is actually getting set because it happens extrinsically when setting up `sage-env`.

Now that I understand, I don't think it's so bad either.  Just not obvious.
