# Issue 23152: Add 'is_separable' to function field class

archive/issues_022915.json:
```json
{
    "assignees": [],
    "body": "Often necessary to check if a function field is separable before running algorithms (ie. whether it is worthwhile to look for an integral basis).\n\nKeywords: **sd86.5**\n\nBranch: **[3cbf43f](https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f)**\n\nReviewer: **\u00c9douard Rousseau, Julian R\u00fcth**\n\nAuthor: **Dean Bisogno**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23152_\n\n",
    "closed_at": "2017-06-12T18:02:00Z",
    "created_at": "2017-06-06T17:54:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add 'is_separable' to function field class",
    "type": "issue",
    "updated_at": "2017-10-24T15:03:37Z",
    "url": "https://github.com/sagemath/sage/issues/23152",
    "user": "https://github.com/sagetrac-dbisogno"
}
```
Often necessary to check if a function field is separable before running algorithms (ie. whether it is worthwhile to look for an integral basis).

Keywords: **sd86.5**

Branch: **[3cbf43f](https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f)**

Reviewer: **Édouard Rousseau, Julian Rüth**

Author: **Dean Bisogno**

_Issue created by migration from https://trac.sagemath.org/ticket/23152_





---

archive/issue_events_304714.json:
```json
{
    "actor": "https://github.com/sagetrac-dbisogno",
    "created_at": "2017-06-06T17:54:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304714"
}
```



---

archive/issue_events_304715.json:
```json
{
    "actor": "https://github.com/sagetrac-dbisogno",
    "created_at": "2017-06-06T17:54:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304715"
}
```



---

archive/issue_events_304716.json:
```json
{
    "actor": "https://github.com/sagetrac-dbisogno",
    "created_at": "2017-06-06T17:54:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304716"
}
```



---

archive/issue_events_304717.json:
```json
{
    "actor": "https://github.com/sagetrac-dbisogno",
    "created_at": "2017-06-06T17:54:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304717"
}
```



---

archive/issue_comments_349772.json:
```json
{
    "body": "Branch: **[u/dbisogno/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/dbisogno/add__is_separable__to_function_field_class)**",
    "created_at": "2017-06-06T18:56:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349772",
    "user": "https://github.com/sagetrac-dbisogno"
}
```

Branch: **[u/dbisogno/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/dbisogno/add__is_separable__to_function_field_class)**



---

archive/issue_events_304718.json:
```json
{
    "actor": "https://github.com/sagetrac-dbisogno",
    "created_at": "2017-06-06T19:57:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304718"
}
```



---

archive/issue_comments_349773.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Often necessary to check if a function field is separable before running algorithms (ie. finding an integral basis).\n+Often necessary to check if a function field is separable before running algorithms (ie. whether it is worthwhile to look for an integral basis).\n``````\n",
    "created_at": "2017-06-06T19:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349773",
    "user": "https://github.com/sagetrac-dbisogno"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Often necessary to check if a function field is separable before running algorithms (ie. finding an integral basis).
+Often necessary to check if a function field is separable before running algorithms (ie. whether it is worthwhile to look for an integral basis).
``````




---

archive/issue_comments_349774.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4bba9ae92a1d9e298ae722f4281d5de8db77b5ac\">4bba9ae</a></td><td><code>is_separable method added to function field class</code></td></tr></table>\n",
    "created_at": "2017-06-06T19:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349774",
    "user": "https://github.com/sagetrac-dbisogno"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4bba9ae92a1d9e298ae722f4281d5de8db77b5ac">4bba9ae</a></td><td><code>is_separable method added to function field class</code></td></tr></table>




---

archive/issue_comments_349775.json:
```json
{
    "body": "Commit: **[4bba9ae](https://github.com/sagemath/sagetrac-mirror/commit/4bba9ae92a1d9e298ae722f4281d5de8db77b5ac)**",
    "created_at": "2017-06-06T19:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349775",
    "user": "https://github.com/sagetrac-dbisogno"
}
```

Commit: **[4bba9ae](https://github.com/sagemath/sagetrac-mirror/commit/4bba9ae92a1d9e298ae722f4281d5de8db77b5ac)**



---

archive/issue_comments_349776.json:
```json
{
    "body": "Changed keywords from **** to **sd86.5**",
    "created_at": "2017-06-06T20:22:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349776",
    "user": "https://github.com/sagetrac-dbisogno"
}
```

Changed keywords from **** to **sd86.5**



---

archive/issue_comments_349777.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/444fb594bd6470dfbfbaaf32cc6243e57705970b\">444fb59</a></td><td><code>INPUT and OUTPUT blocks added to documentation.</code></td></tr></table>\n",
    "created_at": "2017-06-06T21:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349777",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/444fb594bd6470dfbfbaaf32cc6243e57705970b">444fb59</a></td><td><code>INPUT and OUTPUT blocks added to documentation.</code></td></tr></table>




---

archive/issue_comments_349778.json:
```json
{
    "body": "Changed commit from **[4bba9ae](https://github.com/sagemath/sagetrac-mirror/commit/4bba9ae92a1d9e298ae722f4281d5de8db77b5ac)** to **[444fb59](https://github.com/sagemath/sagetrac-mirror/commit/444fb594bd6470dfbfbaaf32cc6243e57705970b)**",
    "created_at": "2017-06-06T21:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349778",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4bba9ae](https://github.com/sagemath/sagetrac-mirror/commit/4bba9ae92a1d9e298ae722f4281d5de8db77b5ac)** to **[444fb59](https://github.com/sagemath/sagetrac-mirror/commit/444fb594bd6470dfbfbaaf32cc6243e57705970b)**



---

archive/issue_comments_349779.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI went through the reviewer's check list and it looks fine to me.\n\nEdouard",
    "created_at": "2017-06-06T21:36:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349779",
    "user": "https://github.com/sagetrac-erousseau"
}
```

<a id='comment:5'>Comment 5:</a>
I went through the reviewer's check list and it looks fine to me.

Edouard



---

archive/issue_comments_349780.json:
```json
{
    "body": "Reviewer: **Edouard Rousseau**",
    "created_at": "2017-06-06T21:36:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349780",
    "user": "https://github.com/sagetrac-erousseau"
}
```

Reviewer: **Edouard Rousseau**



---

archive/issue_events_304719.json:
```json
{
    "actor": "https://github.com/sagetrac-erousseau",
    "created_at": "2017-06-06T21:36:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304719"
}
```



---

archive/issue_events_304720.json:
```json
{
    "actor": "https://github.com/sagetrac-erousseau",
    "created_at": "2017-06-06T21:36:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304720"
}
```



---

archive/issue_comments_349781.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nDocumentation doesn't build",
    "created_at": "2017-06-10T14:56:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349781",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:6'>Comment 6:</a>
Documentation doesn't build



---

archive/issue_events_304721.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-10T14:56:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304721"
}
```



---

archive/issue_events_304722.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-10T14:56:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304722"
}
```



---

archive/issue_comments_349782.json:
```json
{
    "body": "Changed reviewer from **Edouard Rousseau** to **Edouard Rousseau, Julian R\u00fcth**",
    "created_at": "2017-06-10T16:53:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349782",
    "user": "https://github.com/saraedum"
}
```

Changed reviewer from **Edouard Rousseau** to **Edouard Rousseau, Julian Rüth**



---

archive/issue_events_304723.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-10T16:53:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304723"
}
```



---

archive/issue_events_304724.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-10T16:53:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304724"
}
```



---

archive/issue_comments_349783.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThe `sage:` blocks were not indented correctly, and the math was not wrapped in single backticks (to use LaTeX style rendering.)\nEdouard or Dean, if you do not object to my changes, feel free to set this back to positive review.",
    "created_at": "2017-06-10T16:53:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349783",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:7'>Comment 7:</a>
The `sage:` blocks were not indented correctly, and the math was not wrapped in single backticks (to use LaTeX style rendering.)
Edouard or Dean, if you do not object to my changes, feel free to set this back to positive review.



---

archive/issue_comments_349784.json:
```json
{
    "body": "Changed branch from **[u/dbisogno/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/dbisogno/add__is_separable__to_function_field_class)** to **[u/saraedum/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/add__is_separable__to_function_field_class)**",
    "created_at": "2017-06-10T16:54:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349784",
    "user": "https://github.com/saraedum"
}
```

Changed branch from **[u/dbisogno/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/dbisogno/add__is_separable__to_function_field_class)** to **[u/saraedum/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/add__is_separable__to_function_field_class)**



---

archive/issue_comments_349785.json:
```json
{
    "body": "Changed branch from **[u/saraedum/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/add__is_separable__to_function_field_class)** to **[u/dbisogno/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/dbisogno/add__is_separable__to_function_field_class)**",
    "created_at": "2017-06-10T16:55:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349785",
    "user": "https://github.com/saraedum"
}
```

Changed branch from **[u/saraedum/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/add__is_separable__to_function_field_class)** to **[u/dbisogno/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/dbisogno/add__is_separable__to_function_field_class)**



---

archive/issue_comments_349786.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI also took out the INPUT/OUTPUT blocks. They do not add anything that is not stated elsewhere already, so it's probably better to just leave them out. Feel free to put them back in, if you want them in.",
    "created_at": "2017-06-10T16:55:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349786",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:9'>Comment 9:</a>
I also took out the INPUT/OUTPUT blocks. They do not add anything that is not stated elsewhere already, so it's probably better to just leave them out. Feel free to put them back in, if you want them in.



---

archive/issue_comments_349787.json:
```json
{
    "body": "Changed branch from **[u/dbisogno/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/dbisogno/add__is_separable__to_function_field_class)** to **[u/saraedum/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/add__is_separable__to_function_field_class)**",
    "created_at": "2017-06-10T16:56:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349787",
    "user": "https://github.com/saraedum"
}
```

Changed branch from **[u/dbisogno/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/dbisogno/add__is_separable__to_function_field_class)** to **[u/saraedum/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/add__is_separable__to_function_field_class)**



---

archive/issue_events_304725.json:
```json
{
    "actor": "https://github.com/sagetrac-erousseau",
    "created_at": "2017-06-10T17:15:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304725"
}
```



---

archive/issue_events_304726.json:
```json
{
    "actor": "https://github.com/sagetrac-erousseau",
    "created_at": "2017-06-10T17:15:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304726"
}
```



---

archive/issue_comments_349788.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nI do not object.\n\nEdouard\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f\">3cbf43f</a></td><td><code>Fix docbuilding errors</code></td></tr></table>\n",
    "created_at": "2017-06-10T17:15:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349788",
    "user": "https://github.com/sagetrac-erousseau"
}
```

<a id='comment:11'>Comment 11:</a>
I do not object.

Edouard

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f">3cbf43f</a></td><td><code>Fix docbuilding errors</code></td></tr></table>




---

archive/issue_comments_349789.json:
```json
{
    "body": "Changed commit from **[444fb59](https://github.com/sagemath/sagetrac-mirror/commit/444fb594bd6470dfbfbaaf32cc6243e57705970b)** to **[3cbf43f](https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f)**",
    "created_at": "2017-06-10T17:15:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349789",
    "user": "https://github.com/sagetrac-erousseau"
}
```

Changed commit from **[444fb59](https://github.com/sagemath/sagetrac-mirror/commit/444fb594bd6470dfbfbaaf32cc6243e57705970b)** to **[3cbf43f](https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f)**



---

archive/issue_events_304727.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-12T00:37:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304727"
}
```



---

archive/issue_events_304728.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-12T00:37:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304728"
}
```



---

archive/issue_comments_349790.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nI just realized that #16564 also introduces this. The implementation there is more general and apparently you need to normalize the defining polynomial to get this right?",
    "created_at": "2017-06-12T00:42:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349790",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:13'>Comment 13:</a>
I just realized that #16564 also introduces this. The implementation there is more general and apparently you need to normalize the defining polynomial to get this right?



---

archive/issue_events_304729.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-12T00:42:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304729"
}
```



---

archive/issue_events_304730.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-12T00:42:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304730"
}
```



---

archive/issue_comments_349791.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nSorry for the noise. Your implementation is fine. I adapted mine to not do the normalization.",
    "created_at": "2017-06-12T00:49:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349791",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:15'>Comment 15:</a>
Sorry for the noise. Your implementation is fine. I adapted mine to not do the normalization.



---

archive/issue_events_304731.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-12T00:49:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304731"
}
```



---

archive/issue_events_304732.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-06-12T00:49:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304732"
}
```



---

archive/issue_comments_349792.json:
```json
{
    "body": "Changed branch from **[u/saraedum/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/add__is_separable__to_function_field_class)** to **[3cbf43f](https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f)**",
    "created_at": "2017-06-12T18:02:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349792",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/add__is_separable__to_function_field_class](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/add__is_separable__to_function_field_class)** to **[3cbf43f](https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f)**



---

archive/issue_events_304733.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-12T18:02:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304733"
}
```



---

archive/issue_events_304734.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8d191b6b17a38cfb472eb58fe652bed272c34bd9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-06-12T18:02:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23152#event-304734"
}
```



---

archive/issue_comments_349793.json:
```json
{
    "body": "Changed reviewer from **Edouard Rousseau, Julian R\u00fcth** to **\u00c9douard Rousseau, Julian R\u00fcth**",
    "created_at": "2017-10-24T15:03:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349793",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Edouard Rousseau, Julian Rüth** to **Édouard Rousseau, Julian Rüth**



---

archive/issue_comments_349794.json:
```json
{
    "body": "Changed commit from **[3cbf43f](https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f)** to none",
    "created_at": "2017-10-24T15:03:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23152",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23152#issuecomment-349794",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[3cbf43f](https://github.com/sagemath/sagetrac-mirror/commit/3cbf43fc6d219172833d9bf0a5da676bcae2976f)** to none
