# Issue 23904: Do not import cvxopt globally

archive/issues_023667.json:
```json
{
    "assignees": [],
    "body": "There is no need to `import cvxopt` at the top level in the cvxopt backends since it is already imported locally.\n\nBranch/Commit: **[dd04b0e](https://github.com/sagemath/sagetrac-mirror/commit/dd04b0e2cc6a9167c8197c433ccdf95cdf7af3ad)**\n\nReviewer: **Maarten Derickx**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23904_\n\n",
    "closed_at": "2017-09-26T06:57:54Z",
    "created_at": "2017-09-20T09:07:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Do not import cvxopt globally",
    "type": "issue",
    "updated_at": "2017-09-26T06:57:54Z",
    "url": "https://github.com/sagemath/sage/issues/23904",
    "user": "https://github.com/jdemeyer"
}
```
There is no need to `import cvxopt` at the top level in the cvxopt backends since it is already imported locally.

Branch/Commit: **[dd04b0e](https://github.com/sagemath/sagetrac-mirror/commit/dd04b0e2cc6a9167c8197c433ccdf95cdf7af3ad)**

Reviewer: **Maarten Derickx**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/23904_





---

archive/issue_events_313932.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-20T09:07:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23904#event-313932"
}
```



---

archive/issue_events_313933.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-20T09:07:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
    "label_color": "0000ff",
    "label_name": "component: linear programming",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23904#event-313933"
}
```



---

archive/issue_events_313934.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-20T09:07:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23904#event-313934"
}
```



---

archive/issue_events_313935.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-20T09:07:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23904#event-313935"
}
```



---

archive/issue_comments_363434.json:
```json
{
    "body": "Branch: **[u/jdemeyer/do_not_import_cvxopt_globally](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/do_not_import_cvxopt_globally)**",
    "created_at": "2017-09-20T09:08:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23904#issuecomment-363434",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/do_not_import_cvxopt_globally](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/do_not_import_cvxopt_globally)**



---

archive/issue_events_313936.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-20T09:09:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23904#event-313936"
}
```



---

archive/issue_comments_363435.json:
```json
{
    "body": "Commit: **[dd04b0e](https://github.com/sagemath/sagetrac-mirror/commit/dd04b0e2cc6a9167c8197c433ccdf95cdf7af3ad)**",
    "created_at": "2017-09-20T09:09:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23904#issuecomment-363435",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[dd04b0e](https://github.com/sagemath/sagetrac-mirror/commit/dd04b0e2cc6a9167c8197c433ccdf95cdf7af3ad)**



---

archive/issue_comments_363436.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dd04b0e2cc6a9167c8197c433ccdf95cdf7af3ad\">dd04b0e</a></td><td><code>Do not import cvxopt globally</code></td></tr></table>\n",
    "created_at": "2017-09-20T09:09:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23904#issuecomment-363436",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dd04b0e2cc6a9167c8197c433ccdf95cdf7af3ad">dd04b0e</a></td><td><code>Do not import cvxopt globally</code></td></tr></table>




---

archive/issue_comments_363437.json:
```json
{
    "body": "Reviewer: **Maarten Derickx**",
    "created_at": "2017-09-25T22:25:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23904#issuecomment-363437",
    "user": "https://github.com/koffie"
}
```

Reviewer: **Maarten Derickx**



---

archive/issue_events_313937.json:
```json
{
    "actor": "https://github.com/koffie",
    "created_at": "2017-09-25T22:25:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23904#event-313937"
}
```



---

archive/issue_events_313938.json:
```json
{
    "actor": "https://github.com/koffie",
    "created_at": "2017-09-25T22:25:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23904#event-313938"
}
```



---

archive/issue_comments_363438.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me.",
    "created_at": "2017-09-25T22:25:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23904#issuecomment-363438",
    "user": "https://github.com/koffie"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me.



---

archive/issue_comments_363439.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/do_not_import_cvxopt_globally](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/do_not_import_cvxopt_globally)** to **[dd04b0e](https://github.com/sagemath/sagetrac-mirror/commit/dd04b0e2cc6a9167c8197c433ccdf95cdf7af3ad)**",
    "created_at": "2017-09-26T06:57:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23904#issuecomment-363439",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/do_not_import_cvxopt_globally](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/do_not_import_cvxopt_globally)** to **[dd04b0e](https://github.com/sagemath/sagetrac-mirror/commit/dd04b0e2cc6a9167c8197c433ccdf95cdf7af3ad)**



---

archive/issue_events_313939.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-09-26T06:57:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23904#event-313939"
}
```



---

archive/issue_events_313940.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "50ea249b908da82968b82aa9a3baf897aef80198",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-09-26T06:57:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23904",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23904#event-313940"
}
```
