# Issue 23750: Fix _richcmp_ in morphism.pyx

archive/issues_023513.json:
```json
{
    "assignees": [],
    "body": "#23485 mistakenly used `def _richcmp_` instead of `cpdef _richmp_`.\n\nCC:  @saraedum @roed314\n\nBranch/Commit: **[f1c779f](https://github.com/sagemath/sagetrac-mirror/commit/f1c779f7d07346deebefa13760ad37c9baf23353)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23750_\n\n",
    "closed_at": "2017-09-04T06:11:16Z",
    "created_at": "2017-08-30T08:48:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix _richcmp_ in morphism.pyx",
    "type": "issue",
    "updated_at": "2017-09-04T06:11:16Z",
    "url": "https://github.com/sagemath/sage/issues/23750",
    "user": "https://github.com/jdemeyer"
}
```
#23485 mistakenly used `def _richcmp_` instead of `cpdef _richmp_`.

CC:  @saraedum @roed314

Branch/Commit: **[f1c779f](https://github.com/sagemath/sagetrac-mirror/commit/f1c779f7d07346deebefa13760ad37c9baf23353)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/23750_





---

archive/issue_events_312125.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-08-30T08:48:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23750#event-312125"
}
```



---

archive/issue_events_312126.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-08-30T08:48:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23750#event-312126"
}
```



---

archive/issue_events_312127.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-08-30T08:48:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23750#event-312127"
}
```



---

archive/issue_events_312128.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-08-30T08:48:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23750#event-312128"
}
```



---

archive/issue_comments_360959.json:
```json
{
    "body": "Branch: **[u/jdemeyer/fix__richcmp__in_morphism_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix__richcmp__in_morphism_pyx)**",
    "created_at": "2017-08-30T15:12:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23750#issuecomment-360959",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/fix__richcmp__in_morphism_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix__richcmp__in_morphism_pyx)**



---

archive/issue_comments_360960.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f1c779f7d07346deebefa13760ad37c9baf23353\">f1c779f</a></td><td><code>Fix `_richcmp_` signature</code></td></tr></table>\n",
    "created_at": "2017-08-30T15:12:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23750#issuecomment-360960",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f1c779f7d07346deebefa13760ad37c9baf23353">f1c779f</a></td><td><code>Fix `_richcmp_` signature</code></td></tr></table>




---

archive/issue_events_312129.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-08-30T15:12:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23750#event-312129"
}
```



---

archive/issue_comments_360961.json:
```json
{
    "body": "Commit: **[f1c779f](https://github.com/sagemath/sagetrac-mirror/commit/f1c779f7d07346deebefa13760ad37c9baf23353)**",
    "created_at": "2017-08-30T15:12:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23750#issuecomment-360961",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[f1c779f](https://github.com/sagemath/sagetrac-mirror/commit/f1c779f7d07346deebefa13760ad37c9baf23353)**



---

archive/issue_comments_360962.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2017-08-30T15:12:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23750#issuecomment-360962",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_360963.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI believe these also need to be declared in the pxd file, correct?",
    "created_at": "2017-08-30T15:52:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23750#issuecomment-360963",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
I believe these also need to be declared in the pxd file, correct?



---

archive/issue_comments_360964.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nReplying to [@tscrim](#comment%3A3):\n> I believe these also need to be declared in the pxd file, correct?\n\nNo because it's already declared in `element.pxd`",
    "created_at": "2017-08-30T20:22:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23750#issuecomment-360964",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
Replying to [@tscrim](#comment%3A3):
> I believe these also need to be declared in the pxd file, correct?

No because it's already declared in `element.pxd`



---

archive/issue_comments_360965.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-08-30T20:28:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23750#issuecomment-360965",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_360966.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nOkay. LGTM.",
    "created_at": "2017-08-30T20:28:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23750#issuecomment-360966",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
Okay. LGTM.



---

archive/issue_events_312130.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-08-30T20:28:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23750#event-312130"
}
```



---

archive/issue_events_312131.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-08-30T20:28:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23750#event-312131"
}
```



---

archive/issue_events_312132.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-09-04T06:11:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23750#event-312132"
}
```



---

archive/issue_events_312133.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "10a21623fb891e34f5becb9bae37571ac969c7d2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-09-04T06:11:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23750#event-312133"
}
```



---

archive/issue_comments_360967.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/fix__richcmp__in_morphism_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix__richcmp__in_morphism_pyx)** to **[f1c779f](https://github.com/sagemath/sagetrac-mirror/commit/f1c779f7d07346deebefa13760ad37c9baf23353)**",
    "created_at": "2017-09-04T06:11:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23750",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23750#issuecomment-360967",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/fix__richcmp__in_morphism_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix__richcmp__in_morphism_pyx)** to **[f1c779f](https://github.com/sagemath/sagetrac-mirror/commit/f1c779f7d07346deebefa13760ad37c9baf23353)**
