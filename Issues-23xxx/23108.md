# Issue 23108: Add an example to expand_trig

archive/issues_022871.json:
```json
{
    "assignees": [],
    "body": "Add an example showing expressions do not get trig-expanded if one does not expand them first, in general:\n\n```\nsage: (x, k1, k2) = var('x, k1, k2')\nsage: cos((k1-k2)*x).expand_trig()\ncos((k1 - k2)*x)\nsage: cos((k1-k2)*x).expand().expand_trig()\ncos(k1*x)*cos(k2*x) + sin(k1*x)*sin(k2*x)\n```\n\nThis behavior is stated in the function's description (`For best results, self should already be expanded.`.) and this ticket proposes to illustrate it.\n\nCC:  @rwst\n\nKeywords: **documentation, trigonometric**\n\nBranch/Commit: **[6be6fe7](https://github.com/sagemath/sagetrac-mirror/commit/6be6fe73c08e38154a8c1ad00f3c959924a96efb)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Marcelo Forets**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23108_\n\n",
    "closed_at": "2017-06-07T20:13:17Z",
    "created_at": "2017-05-31T06:48:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add an example to expand_trig",
    "type": "issue",
    "updated_at": "2017-06-07T20:13:17Z",
    "url": "https://github.com/sagemath/sage/issues/23108",
    "user": "https://github.com/mforets"
}
```
Add an example showing expressions do not get trig-expanded if one does not expand them first, in general:

```
sage: (x, k1, k2) = var('x, k1, k2')
sage: cos((k1-k2)*x).expand_trig()
cos((k1 - k2)*x)
sage: cos((k1-k2)*x).expand().expand_trig()
cos(k1*x)*cos(k2*x) + sin(k1*x)*sin(k2*x)
```

This behavior is stated in the function's description (`For best results, self should already be expanded.`.) and this ticket proposes to illustrate it.

CC:  @rwst

Keywords: **documentation, trigonometric**

Branch/Commit: **[6be6fe7](https://github.com/sagemath/sagetrac-mirror/commit/6be6fe73c08e38154a8c1ad00f3c959924a96efb)**

Reviewer: **Travis Scrimshaw**

Author: **Marcelo Forets**

_Issue created by migration from https://trac.sagemath.org/ticket/23108_





---

archive/issue_events_304175.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-05-31T06:48:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23108#event-304175"
}
```



---

archive/issue_events_304176.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-05-31T06:48:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23108#event-304176"
}
```



---

archive/issue_events_304177.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-05-31T06:48:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23108#event-304177"
}
```



---

archive/issue_events_304178.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-05-31T06:48:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23108#event-304178"
}
```



---

archive/issue_events_304179.json:
```json
{
    "actor": "https://github.com/mforets",
    "created_at": "2017-05-31T06:52:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23108#event-304179"
}
```



---

archive/issue_comments_348783.json:
```json
{
    "body": "Commit: **[6be6fe7](https://github.com/sagemath/sagetrac-mirror/commit/6be6fe73c08e38154a8c1ad00f3c959924a96efb)**",
    "created_at": "2017-05-31T06:52:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23108#issuecomment-348783",
    "user": "https://github.com/mforets"
}
```

Commit: **[6be6fe7](https://github.com/sagemath/sagetrac-mirror/commit/6be6fe73c08e38154a8c1ad00f3c959924a96efb)**



---

archive/issue_comments_348784.json:
```json
{
    "body": "Author: **Marcelo Forets**",
    "created_at": "2017-05-31T06:52:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23108#issuecomment-348784",
    "user": "https://github.com/mforets"
}
```

Author: **Marcelo Forets**



---

archive/issue_comments_348785.json:
```json
{
    "body": "Branch: **[u/mforets/23108](https://github.com/sagemath/sagetrac-mirror/tree/u/mforets/23108)**",
    "created_at": "2017-05-31T06:52:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23108#issuecomment-348785",
    "user": "https://github.com/mforets"
}
```

Branch: **[u/mforets/23108](https://github.com/sagemath/sagetrac-mirror/tree/u/mforets/23108)**



---

archive/issue_comments_348786.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6be6fe73c08e38154a8c1ad00f3c959924a96efb\">6be6fe7</a></td><td><code>add cos example to expand_trig</code></td></tr></table>\n",
    "created_at": "2017-05-31T06:52:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23108#issuecomment-348786",
    "user": "https://github.com/mforets"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6be6fe73c08e38154a8c1ad00f3c959924a96efb">6be6fe7</a></td><td><code>add cos example to expand_trig</code></td></tr></table>




---

archive/issue_comments_348787.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\n+1 for illustrative examples.",
    "created_at": "2017-05-31T15:16:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23108#issuecomment-348787",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

+1 for illustrative examples.



---

archive/issue_comments_348788.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-05-31T15:16:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23108#issuecomment-348788",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_304180.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-31T15:16:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23108#event-304180"
}
```



---

archive/issue_events_304181.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-31T15:16:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23108#event-304181"
}
```



---

archive/issue_comments_348789.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nthanks for the feedback.",
    "created_at": "2017-05-31T15:42:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23108#issuecomment-348789",
    "user": "https://github.com/mforets"
}
```

<div id="comment:3" align="right">Comment 3</div>

thanks for the feedback.



---

archive/issue_comments_348790.json:
```json
{
    "body": "Changed branch from **[u/mforets/23108](https://github.com/sagemath/sagetrac-mirror/tree/u/mforets/23108)** to **[6be6fe7](https://github.com/sagemath/sagetrac-mirror/commit/6be6fe73c08e38154a8c1ad00f3c959924a96efb)**",
    "created_at": "2017-06-07T20:13:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23108#issuecomment-348790",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mforets/23108](https://github.com/sagemath/sagetrac-mirror/tree/u/mforets/23108)** to **[6be6fe7](https://github.com/sagemath/sagetrac-mirror/commit/6be6fe73c08e38154a8c1ad00f3c959924a96efb)**



---

archive/issue_events_304182.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-07T20:13:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23108#event-304182"
}
```



---

archive/issue_events_304183.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "597542d00f6e2161e54fda988f4e01462aa42728",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-06-07T20:13:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23108",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23108#event-304183"
}
```
