# Issue 23248: sympy behavior depends on gmpy2 installation

archive/issues_023011.json:
```json
{
    "assignees": [],
    "body": "When gmpy2 is installed one gets doctest failures in `sage/calculus/calculus.py` and `symbolic/integration/integral.py`.\n\n- Sympy issue: https://github.com/sympy/sympy/issues/12753\n- Sympy pull request: https://github.com/sympy/sympy/pull/12784\n\n\nCC:  @vinklein\n\nKeywords: **thursdaysbdx**\n\nBranch/Commit: **[edb4b4a](https://github.com/sagemath/sagetrac-mirror/commit/edb4b4a9587e62a45a5581e482cec551712aa5f5)**\n\nUpstream: **Reported upstream. No feedback yet.**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23248_\n\n",
    "closed_at": "2017-06-25T15:45:12Z",
    "created_at": "2017-06-15T15:26:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sympy behavior depends on gmpy2 installation",
    "type": "issue",
    "updated_at": "2017-06-25T15:45:12Z",
    "url": "https://github.com/sagemath/sage/issues/23248",
    "user": "https://github.com/videlec"
}
```
When gmpy2 is installed one gets doctest failures in `sage/calculus/calculus.py` and `symbolic/integration/integral.py`.

- Sympy issue: https://github.com/sympy/sympy/issues/12753
- Sympy pull request: https://github.com/sympy/sympy/pull/12784


CC:  @vinklein

Keywords: **thursdaysbdx**

Branch/Commit: **[edb4b4a](https://github.com/sagemath/sagetrac-mirror/commit/edb4b4a9587e62a45a5581e482cec551712aa5f5)**

Upstream: **Reported upstream. No feedback yet.**

Reviewer: **Travis Scrimshaw**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/23248_





---

archive/issue_events_305894.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-06-15T15:26:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23248#event-305894"
}
```



---

archive/issue_events_305895.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-06-15T15:26:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23248#event-305895"
}
```



---

archive/issue_events_305896.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-06-15T15:26:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23248#event-305896"
}
```



---

archive/issue_events_305897.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-06-15T15:26:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23248#event-305897"
}
```



---

archive/issue_comments_351617.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n When gmpy2 is installed one gets doctest failures in `sage/calculus/calculus.py` and `symbolic/integration/integral.py`.\n+\n+Sympy issue: https://github.com/sympy/sympy/issues/12753\n``````\n",
    "created_at": "2017-06-15T15:28:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351617",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 When gmpy2 is installed one gets doctest failures in `sage/calculus/calculus.py` and `symbolic/integration/integral.py`.
+
+Sympy issue: https://github.com/sympy/sympy/issues/12753
``````




---

archive/issue_comments_351618.json:
```json
{
    "body": "Changed upstream from **Not yet reported upstream; Will do shortly.** to **Reported upstream. No feedback yet.**",
    "created_at": "2017-06-15T15:28:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351618",
    "user": "https://github.com/videlec"
}
```

Changed upstream from **Not yet reported upstream; Will do shortly.** to **Reported upstream. No feedback yet.**



---

archive/issue_comments_351619.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n When gmpy2 is installed one gets doctest failures in `sage/calculus/calculus.py` and `symbolic/integration/integral.py`.\n \n-Sympy issue: https://github.com/sympy/sympy/issues/12753\n+- Sympy issue: https://github.com/sympy/sympy/issues/12753\n+- Sympy pull request: https://github.com/sympy/sympy/pull/12784\n+\n``````\n",
    "created_at": "2017-06-22T15:58:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351619",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 When gmpy2 is installed one gets doctest failures in `sage/calculus/calculus.py` and `symbolic/integration/integral.py`.
 
-Sympy issue: https://github.com/sympy/sympy/issues/12753
+- Sympy issue: https://github.com/sympy/sympy/issues/12753
+- Sympy pull request: https://github.com/sympy/sympy/pull/12784
+
``````




---

archive/issue_events_305898.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-06-22T16:01:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23248#event-305898"
}
```



---

archive/issue_comments_351620.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/310c2954dee2b3574b116ab8436fd5615494e4e9\">310c295</a></td><td><code>23248: backport a patch to sympy</code></td></tr></table>\n",
    "created_at": "2017-06-22T16:01:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351620",
    "user": "https://github.com/videlec"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/310c2954dee2b3574b116ab8436fd5615494e4e9">310c295</a></td><td><code>23248: backport a patch to sympy</code></td></tr></table>




---

archive/issue_comments_351621.json:
```json
{
    "body": "Branch: **[u/vdelecroix/23248](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/23248)**",
    "created_at": "2017-06-22T16:01:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351621",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/23248](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/23248)**



---

archive/issue_comments_351622.json:
```json
{
    "body": "Commit: **[310c295](https://github.com/sagemath/sagetrac-mirror/commit/310c2954dee2b3574b116ab8436fd5615494e4e9)**",
    "created_at": "2017-06-22T16:01:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351622",
    "user": "https://github.com/videlec"
}
```

Commit: **[310c295](https://github.com/sagemath/sagetrac-mirror/commit/310c2954dee2b3574b116ab8436fd5615494e4e9)**



---

archive/issue_comments_351623.json:
```json
{
    "body": "Changed commit from **[310c295](https://github.com/sagemath/sagetrac-mirror/commit/310c2954dee2b3574b116ab8436fd5615494e4e9)** to **[edb4b4a](https://github.com/sagemath/sagetrac-mirror/commit/edb4b4a9587e62a45a5581e482cec551712aa5f5)**",
    "created_at": "2017-06-22T16:02:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351623",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[310c295](https://github.com/sagemath/sagetrac-mirror/commit/310c2954dee2b3574b116ab8436fd5615494e4e9)** to **[edb4b4a](https://github.com/sagemath/sagetrac-mirror/commit/edb4b4a9587e62a45a5581e482cec551712aa5f5)**



---

archive/issue_comments_351624.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/edb4b4a9587e62a45a5581e482cec551712aa5f5\">edb4b4a</a></td><td><code>23248: bump sympy package version</code></td></tr></table>\n",
    "created_at": "2017-06-22T16:02:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351624",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/edb4b4a9587e62a45a5581e482cec551712aa5f5">edb4b4a</a></td><td><code>23248: bump sympy package version</code></td></tr></table>




---

archive/issue_comments_351625.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nIf upstream is happy, how could we refuse. :)",
    "created_at": "2017-06-22T23:55:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351625",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
If upstream is happy, how could we refuse. :)



---

archive/issue_events_305899.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-22T23:55:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23248#event-305899"
}
```



---

archive/issue_events_305900.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-22T23:55:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23248#event-305900"
}
```



---

archive/issue_comments_351626.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-06-22T23:55:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351626",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_305901.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-25T15:45:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23248#event-305901"
}
```



---

archive/issue_events_305902.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "31268b9613af576fb943abfdf955c54fb0685791",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-06-25T15:45:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23248#event-305902"
}
```



---

archive/issue_comments_351627.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/23248](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/23248)** to **[edb4b4a](https://github.com/sagemath/sagetrac-mirror/commit/edb4b4a9587e62a45a5581e482cec551712aa5f5)**",
    "created_at": "2017-06-25T15:45:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23248#issuecomment-351627",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/23248](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/23248)** to **[edb4b4a](https://github.com/sagemath/sagetrac-mirror/commit/edb4b4a9587e62a45a5581e482cec551712aa5f5)**
