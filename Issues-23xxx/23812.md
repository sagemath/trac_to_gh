# Issue 23812: py3: let the repr of numbers be a string

archive/issues_023575.json:
```json
{
    "assignees": [],
    "body": "and not forced to be \"bytes\"\n\nreplaced by #24223\n\nAssignee: **@embray**\n\nCC:  @embray @jdemeyer\n\nKeywords: **unicode**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23812_\n\n",
    "closed_at": "2018-01-10T15:24:13Z",
    "created_at": "2017-09-09T11:21:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: let the repr of numbers be a string",
    "type": "issue",
    "updated_at": "2018-06-27T19:01:49Z",
    "url": "https://github.com/sagemath/sage/issues/23812",
    "user": "https://github.com/fchapoton"
}
```
and not forced to be "bytes"

replaced by #24223

Assignee: **@embray**

CC:  @embray @jdemeyer

Keywords: **unicode**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/23812_





---

archive/issue_events_312841.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-09-09T11:21:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312841"
}
```



---

archive/issue_events_312842.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-09-09T11:21:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312842"
}
```



---

archive/issue_events_312843.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-09-09T11:21:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312843"
}
```



---

archive/issue_events_312844.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-09-09T11:21:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312844"
}
```



---

archive/issue_comments_361892.json:
```json
{
    "body": "Commit: **[f34394d](https://github.com/sagemath/sagetrac-mirror/commit/f34394d84ed07c4ed492479136fed334abd14b19)**",
    "created_at": "2017-09-09T11:22:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361892",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[f34394d](https://github.com/sagemath/sagetrac-mirror/commit/f34394d84ed07c4ed492479136fed334abd14b19)**



---

archive/issue_events_312845.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-09-09T11:22:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312845"
}
```



---

archive/issue_comments_361893.json:
```json
{
    "body": "Branch: **[u/chapoton/23812](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/23812)**",
    "created_at": "2017-09-09T11:22:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361893",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/23812](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/23812)**



---

archive/issue_comments_361894.json:
```json
{
    "body": "Changed commit from **[f34394d](https://github.com/sagemath/sagetrac-mirror/commit/f34394d84ed07c4ed492479136fed334abd14b19)** to **[f093217](https://github.com/sagemath/sagetrac-mirror/commit/f0932175fb6523b5baaa831c379e9ac82037960f)**",
    "created_at": "2017-09-09T11:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361894",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f34394d](https://github.com/sagemath/sagetrac-mirror/commit/f34394d84ed07c4ed492479136fed334abd14b19)** to **[f093217](https://github.com/sagemath/sagetrac-mirror/commit/f0932175fb6523b5baaa831c379e9ac82037960f)**



---

archive/issue_comments_361895.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f0932175fb6523b5baaa831c379e9ac82037960f\">f093217</a></td><td><code>py3: str repr for integers</code></td></tr></table>\n",
    "created_at": "2017-09-09T11:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361895",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f0932175fb6523b5baaa831c379e9ac82037960f">f093217</a></td><td><code>py3: str repr for integers</code></td></tr></table>




---

archive/issue_comments_361896.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI wonder if we should have a general function to convert from `bytes` to `str` and back in Sage. For example, like in cypari2: https://github.com/defeo/cypari2/blob/master/cypari2/string_utils.pyx",
    "created_at": "2017-09-10T08:22:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361896",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
I wonder if we should have a general function to convert from `bytes` to `str` and back in Sage. For example, like in cypari2: https://github.com/defeo/cypari2/blob/master/cypari2/string_utils.pyx



---

archive/issue_comments_361897.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nIt seems that we also need to change the repr of rational numbers..",
    "created_at": "2017-09-10T09:59:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361897",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
It seems that we also need to change the repr of rational numbers..



---

archive/issue_comments_361898.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nping ?",
    "created_at": "2017-09-11T18:54:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361898",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
ping ?



---

archive/issue_comments_361899.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAs I said, I think we should have a general solution instead of this.",
    "created_at": "2017-09-12T10:10:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361899",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
As I said, I think we should have a general solution instead of this.



---

archive/issue_events_312846.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-12T10:10:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312846"
}
```



---

archive/issue_events_312847.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-12T10:10:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312847"
}
```



---

archive/issue_comments_361900.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThere is\n\n```\nsrc/sage/misc/six.py:def u(x):\n```\nbut this is not doing exactly that.",
    "created_at": "2017-09-12T10:11:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361900",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:7'>Comment 7:</a>
There is

```
src/sage/misc/six.py:def u(x):
```
but this is not doing exactly that.



---

archive/issue_comments_361901.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI don't think this is the right answer.  There a dozens of places where a similar conversion is needed.  There needs to be a generic function for this kind of thing.",
    "created_at": "2017-09-12T20:06:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361901",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'>Comment 8:</a>
I don't think this is the right answer.  There a dozens of places where a similar conversion is needed.  There needs to be a generic function for this kind of thing.



---

archive/issue_comments_361902.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThis is maybe not the definitive word. If you feel you can do better quickly, please do.\n\nNethertheless, there are not so many casting to bytes:\n\n```\ngit grep -c \"<bytes>\" *.pyx\nsrc/sage/libs/ecl.pyx:2\nsrc/sage/libs/homfly.pyx:1\nsrc/sage/libs/pynac/pynac.pyx:1\nsrc/sage/numerical/backends/cvxopt_backend.pyx:1\nsrc/sage/numerical/backends/cvxopt_sdp_backend.pyx:1\nsrc/sage/numerical/backends/ppl_backend.pyx:1\nsrc/sage/rings/integer.pyx:1\n```\nand integer.pyx is certainly the most important place where we need to correct the casting to bytes.",
    "created_at": "2017-09-13T06:44:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361902",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'>Comment 9:</a>
This is maybe not the definitive word. If you feel you can do better quickly, please do.

Nethertheless, there are not so many casting to bytes:

```
git grep -c "<bytes>" *.pyx
src/sage/libs/ecl.pyx:2
src/sage/libs/homfly.pyx:1
src/sage/libs/pynac/pynac.pyx:1
src/sage/numerical/backends/cvxopt_backend.pyx:1
src/sage/numerical/backends/cvxopt_sdp_backend.pyx:1
src/sage/numerical/backends/ppl_backend.pyx:1
src/sage/rings/integer.pyx:1
```
and integer.pyx is certainly the most important place where we need to correct the casting to bytes.



---

archive/issue_comments_361903.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nReplying to [@fchapoton](#comment%3A9):\n> Nethertheless, there are not so many casting to bytes:\n\nI think most of the time, the `bytes` is implicit because it's just what corresponds to `char *` in C. For example, in code like\n\n```\ndef f():\n    cdef char* cstr = ....\n    return cstr\n```\nCython will automatically convert `cstr` to Python `bytes`. Since Sage deals with a lot of C libraries, I would guess that this happens in a lot of places.",
    "created_at": "2017-09-13T09:07:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361903",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'>Comment 10:</a>
Replying to [@fchapoton](#comment%3A9):
> Nethertheless, there are not so many casting to bytes:

I think most of the time, the `bytes` is implicit because it's just what corresponds to `char *` in C. For example, in code like

```
def f():
    cdef char* cstr = ....
    return cstr
```
Cython will automatically convert `cstr` to Python `bytes`. Since Sage deals with a lot of C libraries, I would guess that this happens in a lot of places.



---

archive/issue_comments_361904.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/60782ff0b93612e180b1adb727764580c1ae3a83\">60782ff</a></td><td><code>Merge branch 'u/chapoton/23812' in 8.1.b5</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6f8a6cf6ba7e8483ce0d9d5d3e7b9e070f931a71\">6f8a6cf</a></td><td><code>trac 23812 also for rationals</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f0bafbb67e5ef9412d9780104af897f04198d407\">f0bafbb</a></td><td><code>Merge branch 'u/chapoton/23812' of trac.sagemath.org:sage into 23812</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8cb8955b728424334732c0624bcda34128c37df1\">8cb8955</a></td><td><code>Merge branch '23812' into integer branch</code></td></tr></table>\n",
    "created_at": "2017-09-13T16:07:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361904",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/60782ff0b93612e180b1adb727764580c1ae3a83">60782ff</a></td><td><code>Merge branch 'u/chapoton/23812' in 8.1.b5</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6f8a6cf6ba7e8483ce0d9d5d3e7b9e070f931a71">6f8a6cf</a></td><td><code>trac 23812 also for rationals</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f0bafbb67e5ef9412d9780104af897f04198d407">f0bafbb</a></td><td><code>Merge branch 'u/chapoton/23812' of trac.sagemath.org:sage into 23812</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8cb8955b728424334732c0624bcda34128c37df1">8cb8955</a></td><td><code>Merge branch '23812' into integer branch</code></td></tr></table>




---

archive/issue_comments_361905.json:
```json
{
    "body": "Changed commit from **[f093217](https://github.com/sagemath/sagetrac-mirror/commit/f0932175fb6523b5baaa831c379e9ac82037960f)** to **[8cb8955](https://github.com/sagemath/sagetrac-mirror/commit/8cb8955b728424334732c0624bcda34128c37df1)**",
    "created_at": "2017-09-13T16:07:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361905",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f093217](https://github.com/sagemath/sagetrac-mirror/commit/f0932175fb6523b5baaa831c379e9ac82037960f)** to **[8cb8955](https://github.com/sagemath/sagetrac-mirror/commit/8cb8955b728424334732c0624bcda34128c37df1)**



---

archive/issue_comments_361906.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ad2879024b7f296eefb1ec54e91dcd162ea7fc4f\">ad28790</a></td><td><code>more utf8 compatibility</code></td></tr></table>\n",
    "created_at": "2017-09-13T16:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361906",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:12'>Comment 12:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ad2879024b7f296eefb1ec54e91dcd162ea7fc4f">ad28790</a></td><td><code>more utf8 compatibility</code></td></tr></table>




---

archive/issue_comments_361907.json:
```json
{
    "body": "Changed commit from **[8cb8955](https://github.com/sagemath/sagetrac-mirror/commit/8cb8955b728424334732c0624bcda34128c37df1)** to **[ad28790](https://github.com/sagemath/sagetrac-mirror/commit/ad2879024b7f296eefb1ec54e91dcd162ea7fc4f)**",
    "created_at": "2017-09-13T16:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361907",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8cb8955](https://github.com/sagemath/sagetrac-mirror/commit/8cb8955b728424334732c0624bcda34128c37df1)** to **[ad28790](https://github.com/sagemath/sagetrac-mirror/commit/ad2879024b7f296eefb1ec54e91dcd162ea7fc4f)**



---

archive/issue_comments_361908.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nwhat do you think of the new branch ?",
    "created_at": "2017-09-13T16:58:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361908",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:13'>Comment 13:</a>
what do you think of the new branch ?



---

archive/issue_events_312848.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-09-13T16:58:20Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "title_is": "py3: let the repr of numbers be a string",
    "title_was": "py3: let the repr of integers be a string",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312848"
}
```



---

archive/issue_comments_361909.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nsee also #23648",
    "created_at": "2017-09-14T08:24:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361909",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'>Comment 14:</a>
see also #23648



---

archive/issue_comments_361910.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nYeah, this really needs some opposite of `string_to_bytes()` from #23648.  This isn't a great approach.  And although in the vast majority of cases we can assume that the return values in these cases will be in ASCII, it's a bad pattern to repeat, because if anything happens to *not* return ASCII using this pattern you'll get something like this on Python 2:\n\n```\n>>> str(b'\\xe2\\x98\\x83'.decode('utf8'))\nTraceback (most recent call last):\n  File \"<stdin>\", line 1, in <module>\nUnicodeEncodeError: 'ascii' codec can't encode character u'\\u2603' in position 0: ordinal not in range(128)\n```",
    "created_at": "2017-09-15T15:22:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361910",
    "user": "https://github.com/embray"
}
```

<a id='comment:15'>Comment 15:</a>
Yeah, this really needs some opposite of `string_to_bytes()` from #23648.  This isn't a great approach.  And although in the vast majority of cases we can assume that the return values in these cases will be in ASCII, it's a bad pattern to repeat, because if anything happens to *not* return ASCII using this pattern you'll get something like this on Python 2:

```
>>> str(b'\xe2\x98\x83'.decode('utf8'))
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
UnicodeEncodeError: 'ascii' codec can't encode character u'\u2603' in position 0: ordinal not in range(128)
```



---

archive/issue_comments_361911.json:
```json
{
    "body": "Changed keywords from **** to **unicode**",
    "created_at": "2017-10-18T16:59:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361911",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **unicode**



---

archive/issue_comments_361912.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nI would appreciate if Erik or another expert of unicode issues works on the matter here and all related tickets dealing with unicode adaptations for python3. I understand when one tells me that my solutions are not good, but please then propose another branch.",
    "created_at": "2017-10-22T19:07:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361912",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:17'>Comment 17:</a>
I would appreciate if Erik or another expert of unicode issues works on the matter here and all related tickets dealing with unicode adaptations for python3. I understand when one tells me that my solutions are not good, but please then propose another branch.



---

archive/issue_comments_361913.json:
```json
{
    "body": "Assignee: **@embray**",
    "created_at": "2017-10-23T10:10:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361913",
    "user": "https://github.com/embray"
}
```

Assignee: **@embray**



---

archive/issue_comments_361914.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nReplying to [@fchapoton](#comment%3A17):\n> I would appreciate if Erik or another expert of unicode issues works on the matter here and all related tickets dealing with unicode adaptations for python3. I understand when one tells me that my solutions are not good, but please then propose another branch.\n> \n\nI hear you-- I need to get my Python 3 build working again but I do want to help with this stuff.  Please feel free to assign such tickets to me.",
    "created_at": "2017-10-23T10:10:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361914",
    "user": "https://github.com/embray"
}
```

<a id='comment:18'>Comment 18:</a>
Replying to [@fchapoton](#comment%3A17):
> I would appreciate if Erik or another expert of unicode issues works on the matter here and all related tickets dealing with unicode adaptations for python3. I understand when one tells me that my solutions are not good, but please then propose another branch.
> 

I hear you-- I need to get my Python 3 build working again but I do want to help with this stuff.  Please feel free to assign such tickets to me.



---

archive/issue_comments_361915.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nI have created a new ticket that just wants to introduce the conversion tools: #24186.",
    "created_at": "2017-11-10T12:45:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361915",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:19'>Comment 19:</a>
I have created a new ticket that just wants to introduce the conversion tools: #24186.



---

archive/issue_comments_361916.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nOops, this patch is now for some reason deleting all of rationals.pyx?\n\nI'm going to see if I can rework this with my new string conversion functions.",
    "created_at": "2017-11-15T08:10:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361916",
    "user": "https://github.com/embray"
}
```

<a id='comment:20'>Comment 20:</a>
Oops, this patch is now for some reason deleting all of rationals.pyx?

I'm going to see if I can rework this with my new string conversion functions.



---

archive/issue_comments_361917.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nReplying to [@embray](#comment%3A20):\n> Oops, this patch is now for some reason deleting all of rationals.pyx?\n\nprobably a known bug in trac git plugin\n\n> I'm going to see if I can rework this with my new string conversion functions.\n\nok. Beware also of #24136",
    "created_at": "2017-11-15T08:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361917",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:21'>Comment 21:</a>
Replying to [@embray](#comment%3A20):
> Oops, this patch is now for some reason deleting all of rationals.pyx?

probably a known bug in trac git plugin

> I'm going to see if I can rework this with my new string conversion functions.

ok. Beware also of #24136



---

archive/issue_events_312849.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:27:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312849"
}
```



---

archive/issue_events_312850.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:27:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312850"
}
```



---

archive/issue_comments_361918.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n and not forced to be \"bytes\"\n+\n+replaced by #24223\n``````\n",
    "created_at": "2018-01-05T12:31:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361918",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 and not forced to be "bytes"
+
+replaced by #24223
``````




---

archive/issue_events_312851.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-05T12:31:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312851"
}
```



---

archive/issue_events_312852.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-05T12:31:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312852"
}
```



---

archive/issue_events_312853.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-05T12:31:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312853"
}
```



---

archive/issue_events_312854.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-05T12:31:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312854"
}
```



---

archive/issue_events_312855.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-05T12:31:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312855"
}
```



---

archive/issue_comments_361919.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nlet us get rid of this one as duplicate",
    "created_at": "2018-01-05T12:31:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361919",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:23'>Comment 23:</a>
let us get rid of this one as duplicate



---

archive/issue_comments_361920.json:
```json
{
    "body": "<a id='comment:24'>Comment 24:</a>\nping ? any objection to close as invalid ?",
    "created_at": "2018-01-10T14:27:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361920",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:24'>Comment 24:</a>
ping ? any objection to close as invalid ?



---

archive/issue_events_312856.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-10T15:24:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312856"
}
```



---

archive/issue_events_312857.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-10T15:24:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312857"
}
```



---

archive/issue_events_312858.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-10T15:24:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23812#event-312858"
}
```



---

archive/issue_comments_361921.json:
```json
{
    "body": "Changed branch from **[u/chapoton/23812](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/23812)** to none",
    "created_at": "2018-06-27T19:01:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361921",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/chapoton/23812](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/23812)** to none



---

archive/issue_comments_361922.json:
```json
{
    "body": "Changed commit from **[ad28790](https://github.com/sagemath/sagetrac-mirror/commit/ad2879024b7f296eefb1ec54e91dcd162ea7fc4f)** to none",
    "created_at": "2018-06-27T19:01:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23812#issuecomment-361922",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[ad28790](https://github.com/sagemath/sagetrac-mirror/commit/ad2879024b7f296eefb1ec54e91dcd162ea7fc4f)** to none
