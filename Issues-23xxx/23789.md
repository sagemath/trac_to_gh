...
```json
{
    "body": "That should only happen if we use `AC_PROG_CC_C99` in `configure.ac`.",
...
```json
{
    "body": "The easiest solution is probably to save the value of `$CC` before the test and then restore it.",
...
```json
{
    "body": "I'm currently working on some other things. Feel free to implement this, otherwise I'll do it later.",
...
```json
{
    "body": "I do not have the compiler to test this. It will probably need its own configure tarball. But some testing first.",
...
```json
{
    "body": "Thanks for taking over. I was busy with \"bedtime stories\" for my son :)",
