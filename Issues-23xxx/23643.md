# Issue 23643: Stopgap for factorization over fraction fields

archive/issues_023406.json:
```json
{
    "assignees": [],
    "body": "Stopgap warning for #23642.\n\nReviewer: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23643_\n\n",
    "closed_at": "2017-11-13T14:20:37Z",
    "created_at": "2017-08-18T11:43:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Stopgap for factorization over fraction fields",
    "type": "issue",
    "updated_at": "2017-11-13T14:44:20Z",
    "url": "https://github.com/sagemath/sage/issues/23643",
    "user": "https://github.com/saraedum"
}
```
Stopgap warning for #23642.

Reviewer: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/23643_





---

archive/issue_events_310845.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-18T11:43:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310845"
}
```



---

archive/issue_events_310846.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-18T11:43:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310846"
}
```



---

archive/issue_events_310847.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-18T11:43:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310847"
}
```



---

archive/issue_events_310848.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-18T11:43:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310848"
}
```



---

archive/issue_comments_358577.json:
```json
{
    "body": "Branch: **[u/saraedum/stopgap_for_factorization_over_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/stopgap_for_factorization_over_fraction_fields)**",
    "created_at": "2017-08-18T12:03:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358577",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/stopgap_for_factorization_over_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/stopgap_for_factorization_over_fraction_fields)**



---

archive/issue_comments_358578.json:
```json
{
    "body": "Changed branch from **[u/saraedum/stopgap_for_factorization_over_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/stopgap_for_factorization_over_fraction_fields)** to none",
    "created_at": "2017-08-18T12:07:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358578",
    "user": "https://github.com/saraedum"
}
```

Changed branch from **[u/saraedum/stopgap_for_factorization_over_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/stopgap_for_factorization_over_fraction_fields)** to none



---

archive/issue_events_310849.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-08-18T12:07:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310849"
}
```



---

archive/issue_comments_358579.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNow it prints:\n\n```\nsage: sage: k.<a> = GF(9)\n....: sage: K = k['t'].fraction_field()\n....: sage: R.<x> = K[]\n....: sage: f = x^3 + a\n....: sage: f.factor()\n....: \n/home/jule/proj/sage/sage/local/lib/python2.7/site-packages/IPython/core/interactiveshell.py:2881:\n********************************************************************************\nFactorization over some quotients can be incorrect.\nThis issue is being tracked at https://github.com/sagemath/sage/issues/23642.\n********************************************************************************\nx^3 + a\n```",
    "created_at": "2017-08-18T12:07:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358579",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:2'>Comment 2:</a>
Now it prints:

```
sage: sage: k.<a> = GF(9)
....: sage: K = k['t'].fraction_field()
....: sage: R.<x> = K[]
....: sage: f = x^3 + a
....: sage: f.factor()
....: 
/home/jule/proj/sage/sage/local/lib/python2.7/site-packages/IPython/core/interactiveshell.py:2881:
********************************************************************************
Factorization over some quotients can be incorrect.
This issue is being tracked at https://github.com/sagemath/sage/issues/23642.
********************************************************************************
x^3 + a
```



---

archive/issue_comments_358580.json:
```json
{
    "body": "Commit: **[4b88524](https://github.com/sagemath/sagetrac-mirror/commit/4b8852443e89cc89b82fb6aa00e5059a93cf0294)**",
    "created_at": "2017-09-20T16:24:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358580",
    "user": "https://github.com/saraedum"
}
```

Commit: **[4b88524](https://github.com/sagemath/sagetrac-mirror/commit/4b8852443e89cc89b82fb6aa00e5059a93cf0294)**



---

archive/issue_comments_358581.json:
```json
{
    "body": "Branch: **[u/saraedum/stopgap_for_factorization_over_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/stopgap_for_factorization_over_fraction_fields)**",
    "created_at": "2017-09-20T16:24:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358581",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/stopgap_for_factorization_over_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/stopgap_for_factorization_over_fraction_fields)**



---

archive/issue_comments_358582.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4b8852443e89cc89b82fb6aa00e5059a93cf0294\">4b88524</a></td><td><code>Stopgap warning for 23642</code></td></tr></table>\n",
    "created_at": "2017-09-20T16:24:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358582",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4b8852443e89cc89b82fb6aa00e5059a93cf0294">4b88524</a></td><td><code>Stopgap warning for 23642</code></td></tr></table>




---

archive/issue_comments_358583.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nSince #23642 has positive review, I think this can be closed as won't fix.",
    "created_at": "2017-10-04T17:22:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358583",
    "user": "https://github.com/roed314"
}
```

<a id='comment:4'>Comment 4:</a>
Since #23642 has positive review, I think this can be closed as won't fix.



---

archive/issue_events_310850.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-10-04T17:22:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310850"
}
```



---

archive/issue_events_310851.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-10-04T17:22:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310851"
}
```



---

archive/issue_events_310852.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-10-04T17:22:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310852"
}
```



---

archive/issue_comments_358584.json:
```json
{
    "body": "Changed branch from **[u/saraedum/stopgap_for_factorization_over_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/stopgap_for_factorization_over_fraction_fields)** to none",
    "created_at": "2017-10-04T17:22:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358584",
    "user": "https://github.com/roed314"
}
```

Changed branch from **[u/saraedum/stopgap_for_factorization_over_fraction_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/stopgap_for_factorization_over_fraction_fields)** to none



---

archive/issue_comments_358585.json:
```json
{
    "body": "Changed commit from **[4b88524](https://github.com/sagemath/sagetrac-mirror/commit/4b8852443e89cc89b82fb6aa00e5059a93cf0294)** to none",
    "created_at": "2017-10-04T17:22:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358585",
    "user": "https://github.com/roed314"
}
```

Changed commit from **[4b88524](https://github.com/sagemath/sagetrac-mirror/commit/4b8852443e89cc89b82fb6aa00e5059a93cf0294)** to none



---

archive/issue_events_310853.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-09T11:28:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310853"
}
```



---

archive/issue_events_310854.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-09T11:28:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310854"
}
```



---

archive/issue_comments_358586.json:
```json
{
    "body": "Changed author from **Julian R\u00fcth** to none",
    "created_at": "2017-11-13T14:20:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358586",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Julian Rüth** to none



---

archive/issue_comments_358587.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2017-11-13T14:20:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23643#issuecomment-358587",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_events_310855.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-13T14:20:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310855"
}
```



---

archive/issue_events_310856.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-13T14:20:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310856"
}
```



---

archive/issue_events_310857.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-11-13T14:44:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23643",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23643#event-310857"
}
```
