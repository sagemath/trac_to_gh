# Issue 23154: Divisor composition in the Jacobian of genus 2 hyperelliptic curves

archive/issues_022917.json:
```json
{
    "assignees": [],
    "body": "A generic implementation for divisor composition using Cantor's algorithm is currently used. There are more efficient formulas described in field operations that we intend to implement in the genus 2 setting.\n\nCC:  @roed314\n\nKeywords: **sd86.5**\n\nBranch/Commit: **[u/slindner/g2](https://github.com/sagemath/sagetrac-mirror/tree/u/slindner/g2) @ [9261a35](https://github.com/sagemath/sagetrac-mirror/commit/9261a35201e1868e1c04a16557841543f7aedd82)**\n\nAuthor: **Mike Jacobson, Sebastian Lindner**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23154_\n\n",
    "created_at": "2017-06-07T00:07:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Divisor composition in the Jacobian of genus 2 hyperelliptic curves",
    "type": "issue",
    "updated_at": "2022-12-29T01:39:18Z",
    "url": "https://github.com/sagemath/sage/issues/23154",
    "user": "https://github.com/sagetrac-slindner"
}
```
A generic implementation for divisor composition using Cantor's algorithm is currently used. There are more efficient formulas described in field operations that we intend to implement in the genus 2 setting.

CC:  @roed314

Keywords: **sd86.5**

Branch/Commit: **[u/slindner/g2](https://github.com/sagemath/sagetrac-mirror/tree/u/slindner/g2) @ [9261a35](https://github.com/sagemath/sagetrac-mirror/commit/9261a35201e1868e1c04a16557841543f7aedd82)**

Author: **Mike Jacobson, Sebastian Lindner**

_Issue created by migration from https://trac.sagemath.org/ticket/23154_





---

archive/issue_events_304747.json:
```json
{
    "actor": "https://github.com/sagetrac-slindner",
    "created_at": "2017-06-07T00:07:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23154#event-304747"
}
```



---

archive/issue_events_304748.json:
```json
{
    "actor": "https://github.com/sagetrac-slindner",
    "created_at": "2017-06-07T00:07:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23154#event-304748"
}
```



---

archive/issue_events_304749.json:
```json
{
    "actor": "https://github.com/sagetrac-slindner",
    "created_at": "2017-06-07T00:07:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23154#event-304749"
}
```



---

archive/issue_events_304750.json:
```json
{
    "actor": "https://github.com/sagetrac-slindner",
    "created_at": "2017-06-07T00:07:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23154#event-304750"
}
```



---

archive/issue_comments_349828.json:
```json
{
    "body": "Branch: **[u/slindner/g2](https://github.com/sagemath/sagetrac-mirror/tree/u/slindner/g2)**",
    "created_at": "2017-06-09T20:25:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23154#issuecomment-349828",
    "user": "https://github.com/sagetrac-slindner"
}
```

Branch: **[u/slindner/g2](https://github.com/sagemath/sagetrac-mirror/tree/u/slindner/g2)**



---

archive/issue_comments_349829.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/97ce09789787ef83f4fafd626e8e222af19965a5\">97ce097</a></td><td><code>Updated class with object oriented behaviour, reduction is only done on initialization from polynomilas, moved recursive calls of reduction into while loop</code></td></tr></table>\n",
    "created_at": "2017-06-12T02:09:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23154#issuecomment-349829",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/97ce09789787ef83f4fafd626e8e222af19965a5">97ce097</a></td><td><code>Updated class with object oriented behaviour, reduction is only done on initialization from polynomilas, moved recursive calls of reduction into while loop</code></td></tr></table>




---

archive/issue_comments_349830.json:
```json
{
    "body": "Commit: **[97ce097](https://github.com/sagemath/sagetrac-mirror/commit/97ce09789787ef83f4fafd626e8e222af19965a5)**",
    "created_at": "2017-06-12T02:09:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23154#issuecomment-349830",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[97ce097](https://github.com/sagemath/sagetrac-mirror/commit/97ce09789787ef83f4fafd626e8e222af19965a5)**



---

archive/issue_comments_349831.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2897a19530c629ddb08952f17a3101529ff121b3\">2897a19</a></td><td><code>Changed while loop location for cantors reduction</code></td></tr></table>\n",
    "created_at": "2017-06-13T15:57:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23154#issuecomment-349831",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2897a19530c629ddb08952f17a3101529ff121b3">2897a19</a></td><td><code>Changed while loop location for cantors reduction</code></td></tr></table>




---

archive/issue_comments_349832.json:
```json
{
    "body": "Changed commit from **[97ce097](https://github.com/sagemath/sagetrac-mirror/commit/97ce09789787ef83f4fafd626e8e222af19965a5)** to **[2897a19](https://github.com/sagemath/sagetrac-mirror/commit/2897a19530c629ddb08952f17a3101529ff121b3)**",
    "created_at": "2017-06-13T15:57:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23154#issuecomment-349832",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[97ce097](https://github.com/sagemath/sagetrac-mirror/commit/97ce09789787ef83f4fafd626e8e222af19965a5)** to **[2897a19](https://github.com/sagemath/sagetrac-mirror/commit/2897a19530c629ddb08952f17a3101529ff121b3)**



---

archive/issue_comments_349833.json:
```json
{
    "body": "Changed commit from **[2897a19](https://github.com/sagemath/sagetrac-mirror/commit/2897a19530c629ddb08952f17a3101529ff121b3)** to **[a17ec61](https://github.com/sagemath/sagetrac-mirror/commit/a17ec61ce952d9990d84eee46bd4b6fb96418a18)**",
    "created_at": "2017-06-14T05:43:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23154#issuecomment-349833",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2897a19](https://github.com/sagemath/sagetrac-mirror/commit/2897a19530c629ddb08952f17a3101529ff121b3)** to **[a17ec61](https://github.com/sagemath/sagetrac-mirror/commit/a17ec61ce952d9990d84eee46bd4b6fb96418a18)**



---

archive/issue_comments_349834.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a17ec61ce952d9990d84eee46bd4b6fb96418a18\">a17ec61</a></td><td><code>Resolved issue where check for reduction is made even when using formulas, completely encapsulated polynomial arithmetic.</code></td></tr></table>\n",
    "created_at": "2017-06-14T05:43:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23154#issuecomment-349834",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a17ec61ce952d9990d84eee46bd4b6fb96418a18">a17ec61</a></td><td><code>Resolved issue where check for reduction is made even when using formulas, completely encapsulated polynomial arithmetic.</code></td></tr></table>




---

archive/issue_comments_349835.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9261a35201e1868e1c04a16557841543f7aedd82\">9261a35</a></td><td><code>In `_init_`, a check for proper Mumford polynomials and reduction is only initiated when creating a divisor by describing the polynomials manually. `_add_` now encapsulates reduction after composition in all cases.</code></td></tr></table>\n",
    "created_at": "2017-06-15T13:44:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23154#issuecomment-349835",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9261a35201e1868e1c04a16557841543f7aedd82">9261a35</a></td><td><code>In `_init_`, a check for proper Mumford polynomials and reduction is only initiated when creating a divisor by describing the polynomials manually. `_add_` now encapsulates reduction after composition in all cases.</code></td></tr></table>




---

archive/issue_comments_349836.json:
```json
{
    "body": "Changed commit from **[a17ec61](https://github.com/sagemath/sagetrac-mirror/commit/a17ec61ce952d9990d84eee46bd4b6fb96418a18)** to **[9261a35](https://github.com/sagemath/sagetrac-mirror/commit/9261a35201e1868e1c04a16557841543f7aedd82)**",
    "created_at": "2017-06-15T13:44:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23154#issuecomment-349836",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a17ec61](https://github.com/sagemath/sagetrac-mirror/commit/a17ec61ce952d9990d84eee46bd4b6fb96418a18)** to **[9261a35](https://github.com/sagemath/sagetrac-mirror/commit/9261a35201e1868e1c04a16557841543f7aedd82)**



---

archive/issue_events_304751.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:39:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23154",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23154#event-304751"
}
```
