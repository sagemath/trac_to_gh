# Issue 23376: Equality testing of genera of quadratic forms over ZZ changes the genus and produces false results

archive/issues_023139.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: D4 = QuadraticForm(Matrix(ZZ,4,4,[2,0,0,-1,0,2,0,-1,0,0,2,-1,-1,-1,-1,2]))\nsage: G = D4.global_genus_symbol()\nsage: sage.quadratic_forms.genera.genus.is_GlobalGenus(G)\nTrue\nsage: G == copy(G)\nTrue\nsage: sage.quadratic_forms.genera.genus.is_GlobalGenus(G)\nFalse\n```\n\nThe reason is that various functions unintentionally modify their input. Since a genus consists of lists containing lists, functions do just copy the lists but not deepcopy.\n\nFor example sage.quadratic_forms.genera.genus.canonical_2_adic_reduction modifies its input which does not seem to be intended.\n\nKeywords: **sd87, Genus, sd91**\n\nBranch: **[24a5b0b](https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c)**\n\nReviewer: **Anthony V\u00e1rilly-Alvarado, Jen Berg**\n\nAuthor: **Simon Brandhorst**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23376_\n\n",
    "closed_at": "2017-10-05T06:54:12Z",
    "created_at": "2017-07-06T07:21:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Equality testing of genera of quadratic forms over ZZ changes the genus and produces false results",
    "type": "issue",
    "updated_at": "2017-12-08T14:47:46Z",
    "url": "https://github.com/sagemath/sage/issues/23376",
    "user": "https://github.com/simonbrandhorst"
}
```

```
sage: D4 = QuadraticForm(Matrix(ZZ,4,4,[2,0,0,-1,0,2,0,-1,0,0,2,-1,-1,-1,-1,2]))
sage: G = D4.global_genus_symbol()
sage: sage.quadratic_forms.genera.genus.is_GlobalGenus(G)
True
sage: G == copy(G)
True
sage: sage.quadratic_forms.genera.genus.is_GlobalGenus(G)
False
```

The reason is that various functions unintentionally modify their input. Since a genus consists of lists containing lists, functions do just copy the lists but not deepcopy.

For example sage.quadratic_forms.genera.genus.canonical_2_adic_reduction modifies its input which does not seem to be intended.

Keywords: **sd87, Genus, sd91**

Branch: **[24a5b0b](https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c)**

Reviewer: **Anthony Várilly-Alvarado, Jen Berg**

Author: **Simon Brandhorst**

_Issue created by migration from https://trac.sagemath.org/ticket/23376_





---

archive/issue_events_307419.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2017-07-06T07:21:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307419"
}
```



---

archive/issue_events_307420.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2017-07-06T07:21:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
    "label_color": "0000ff",
    "label_name": "component: quadratic forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307420"
}
```



---

archive/issue_events_307421.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2017-07-06T07:21:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307421"
}
```



---

archive/issue_events_307422.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2017-07-06T07:21:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307422"
}
```



---

archive/issue_comments_353714.json:
```json
{
    "body": "Branch: **[u/sbrandhorst/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results)**",
    "created_at": "2017-07-07T14:16:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353714",
    "user": "https://github.com/simonbrandhorst"
}
```

Branch: **[u/sbrandhorst/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results)**



---

archive/issue_comments_353715.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/88bdf3c89c5522df62d0035cd3d8826755d451d2\">88bdf3c</a></td><td><code>Fixed two bugs in the genus class</code></td></tr></table>\n",
    "created_at": "2017-07-07T14:17:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353715",
    "user": "https://github.com/simonbrandhorst"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/88bdf3c89c5522df62d0035cd3d8826755d451d2">88bdf3c</a></td><td><code>Fixed two bugs in the genus class</code></td></tr></table>




---

archive/issue_events_307423.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2017-07-07T14:17:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307423"
}
```



---

archive/issue_comments_353716.json:
```json
{
    "body": "Commit: **[88bdf3c](https://github.com/sagemath/sagetrac-mirror/commit/88bdf3c89c5522df62d0035cd3d8826755d451d2)**",
    "created_at": "2017-07-07T14:17:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353716",
    "user": "https://github.com/simonbrandhorst"
}
```

Commit: **[88bdf3c](https://github.com/sagemath/sagetrac-mirror/commit/88bdf3c89c5522df62d0035cd3d8826755d451d2)**



---

archive/issue_events_307424.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2017-07-08T22:26:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307424"
}
```



---

archive/issue_events_307425.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2017-07-08T22:26:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307425"
}
```



---

archive/issue_comments_353717.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8e62210714bf1bcc426733eab9848bd3ff230c23\">8e62210</a></td><td><code>Seems that is_2_adic_genus did not assume the input to be canonical but instead did not reduce the determinants mod 8. Changed that.</code></td></tr></table>\n",
    "created_at": "2017-07-10T06:12:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353717",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8e62210714bf1bcc426733eab9848bd3ff230c23">8e62210</a></td><td><code>Seems that is_2_adic_genus did not assume the input to be canonical but instead did not reduce the determinants mod 8. Changed that.</code></td></tr></table>




---

archive/issue_comments_353718.json:
```json
{
    "body": "Changed commit from **[88bdf3c](https://github.com/sagemath/sagetrac-mirror/commit/88bdf3c89c5522df62d0035cd3d8826755d451d2)** to **[8e62210](https://github.com/sagemath/sagetrac-mirror/commit/8e62210714bf1bcc426733eab9848bd3ff230c23)**",
    "created_at": "2017-07-10T06:12:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353718",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[88bdf3c](https://github.com/sagemath/sagetrac-mirror/commit/88bdf3c89c5522df62d0035cd3d8826755d451d2)** to **[8e62210](https://github.com/sagemath/sagetrac-mirror/commit/8e62210714bf1bcc426733eab9848bd3ff230c23)**



---

archive/issue_events_307426.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2017-07-10T07:22:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307426"
}
```



---

archive/issue_events_307427.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2017-07-10T07:22:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307427"
}
```



---

archive/issue_comments_353719.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\n(reformatting)",
    "created_at": "2017-07-13T17:49:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353719",
    "user": "https://github.com/videlec"
}
```

<div id="comment:6" align="right">Comment 6</div>

(reformatting)



---

archive/issue_comments_353720.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,16 +1,15 @@\n-sage: D4=QuadraticForm(Matrix(ZZ,4,4,[2,0,0,-1,0,2,0,-1,0,0,2,-1,-1,-1,-1,2]))\n-sage: G=D4.global_genus_symbol()\n+\n+```\n+sage: D4 = QuadraticForm(Matrix(ZZ,4,4,[2,0,0,-1,0,2,0,-1,0,0,2,-1,-1,-1,-1,2]))\n+sage: G = D4.global_genus_symbol()\n sage: sage.quadratic_forms.genera.genus.is_GlobalGenus(G)\n True\n-sage: G==copy(G)\n+sage: G == copy(G)\n True\n sage: sage.quadratic_forms.genera.genus.is_GlobalGenus(G)\n False\n+```\n \n-The reason is that various functions unintentionally modify their input.\n-Since a genus consists of lists containing lists, functions do just copy the lists but not deepcopy. \n+The reason is that various functions unintentionally modify their input. Since a genus consists of lists containing lists, functions do just copy the lists but not deepcopy.\n \n-For example \n-sage.quadratic_forms.genera.genus.canonical_2_adic_reduction\n-\n-modifies its input which does not seem to be intended.\n+For example sage.quadratic_forms.genera.genus.canonical_2_adic_reduction modifies its input which does not seem to be intended.\n``````\n",
    "created_at": "2017-07-13T17:49:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353720",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,16 +1,15 @@
-sage: D4=QuadraticForm(Matrix(ZZ,4,4,[2,0,0,-1,0,2,0,-1,0,0,2,-1,-1,-1,-1,2]))
-sage: G=D4.global_genus_symbol()
+
+```
+sage: D4 = QuadraticForm(Matrix(ZZ,4,4,[2,0,0,-1,0,2,0,-1,0,0,2,-1,-1,-1,-1,2]))
+sage: G = D4.global_genus_symbol()
 sage: sage.quadratic_forms.genera.genus.is_GlobalGenus(G)
 True
-sage: G==copy(G)
+sage: G == copy(G)
 True
 sage: sage.quadratic_forms.genera.genus.is_GlobalGenus(G)
 False
+```
 
-The reason is that various functions unintentionally modify their input.
-Since a genus consists of lists containing lists, functions do just copy the lists but not deepcopy. 
+The reason is that various functions unintentionally modify their input. Since a genus consists of lists containing lists, functions do just copy the lists but not deepcopy.
 
-For example 
-sage.quadratic_forms.genera.genus.canonical_2_adic_reduction
-
-modifies its input which does not seem to be intended.
+For example sage.quadratic_forms.genera.genus.canonical_2_adic_reduction modifies its input which does not seem to be intended.
``````




---

archive/issue_comments_353721.json:
```json
{
    "body": "Changed keywords from **Genus** to **Genus, sd87**",
    "created_at": "2017-07-18T18:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353721",
    "user": "https://github.com/simonbrandhorst"
}
```

Changed keywords from **Genus** to **Genus, sd87**



---

archive/issue_comments_353722.json:
```json
{
    "body": "Changed keywords from **Genus, sd87** to **Genus, sd91**",
    "created_at": "2017-08-17T13:52:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353722",
    "user": "https://github.com/simonbrandhorst"
}
```

Changed keywords from **Genus, sd87** to **Genus, sd91**



---

archive/issue_comments_353723.json:
```json
{
    "body": "Changed keywords from **Genus, sd91** to **sd87, Genus, sd91**",
    "created_at": "2017-09-04T20:07:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353723",
    "user": "https://github.com/edgarcosta"
}
```

Changed keywords from **Genus, sd91** to **sd87, Genus, sd91**



---

archive/issue_comments_353724.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nYou should leave the sd87 keyword there.",
    "created_at": "2017-09-04T20:07:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353724",
    "user": "https://github.com/edgarcosta"
}
```

<div id="comment:9" align="right">Comment 9</div>

You should leave the sd87 keyword there.



---

archive/issue_comments_353725.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nDoesn't follow the doctest conventions; needs empty line after TESTS::",
    "created_at": "2017-09-30T17:22:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353725",
    "user": "https://github.com/sagetrac-jensberg"
}
```

<div id="comment:10" align="right">Comment 10</div>

Doesn't follow the doctest conventions; needs empty line after TESTS::



---

archive/issue_events_307428.json:
```json
{
    "actor": "https://github.com/sagetrac-jensberg",
    "created_at": "2017-09-30T17:22:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307428"
}
```



---

archive/issue_events_307429.json:
```json
{
    "actor": "https://github.com/sagetrac-jensberg",
    "created_at": "2017-09-30T17:22:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307429"
}
```



---

archive/issue_comments_353726.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a7cf87ed4801bf3c7b18f440de166733c49aade4\">a7cf87e</a></td><td><code>Merge branch 'u/bhutz/projective_infinity' of git://trac.sagemath.org/sage into t/23376/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c\">24a5b0b</a></td><td><code>Doctest formatting.</code></td></tr></table>\n",
    "created_at": "2017-09-30T17:30:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353726",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:11" align="right">Comment 11</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a7cf87ed4801bf3c7b18f440de166733c49aade4">a7cf87e</a></td><td><code>Merge branch 'u/bhutz/projective_infinity' of git://trac.sagemath.org/sage into t/23376/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c">24a5b0b</a></td><td><code>Doctest formatting.</code></td></tr></table>




---

archive/issue_comments_353727.json:
```json
{
    "body": "Changed commit from **[8e62210](https://github.com/sagemath/sagetrac-mirror/commit/8e62210714bf1bcc426733eab9848bd3ff230c23)** to **[24a5b0b](https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c)**",
    "created_at": "2017-09-30T17:30:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353727",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8e62210](https://github.com/sagemath/sagetrac-mirror/commit/8e62210714bf1bcc426733eab9848bd3ff230c23)** to **[24a5b0b](https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c)**



---

archive/issue_comments_353728.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nFix looks good now.  Passes all doc tests and documentation builds.",
    "created_at": "2017-09-30T20:23:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353728",
    "user": "https://github.com/sagetrac-avarilly"
}
```

<div id="comment:12" align="right">Comment 12</div>

Fix looks good now.  Passes all doc tests and documentation builds.



---

archive/issue_events_307430.json:
```json
{
    "actor": "https://github.com/sagetrac-avarilly",
    "created_at": "2017-09-30T20:23:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307430"
}
```



---

archive/issue_events_307431.json:
```json
{
    "actor": "https://github.com/sagetrac-avarilly",
    "created_at": "2017-09-30T20:23:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307431"
}
```



---

archive/issue_comments_353729.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nReviewer name is missing...",
    "created_at": "2017-10-03T18:34:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353729",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:13" align="right">Comment 13</div>

Reviewer name is missing...



---

archive/issue_events_307432.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-03T18:34:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307432"
}
```



---

archive/issue_events_307433.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-03T18:34:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307433"
}
```



---

archive/issue_comments_353730.json:
```json
{
    "body": "Reviewer: **Anthony Varilly-Alvarado, Jen Berg**",
    "created_at": "2017-10-03T18:47:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353730",
    "user": "https://github.com/sagetrac-jensberg"
}
```

Reviewer: **Anthony Varilly-Alvarado, Jen Berg**



---

archive/issue_events_307434.json:
```json
{
    "actor": "https://github.com/sagetrac-jensberg",
    "created_at": "2017-10-03T18:47:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307434"
}
```



---

archive/issue_events_307435.json:
```json
{
    "actor": "https://github.com/sagetrac-jensberg",
    "created_at": "2017-10-03T18:47:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307435"
}
```



---

archive/issue_comments_353731.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nAdded reviewer names.",
    "created_at": "2017-10-03T18:47:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353731",
    "user": "https://github.com/sagetrac-jensberg"
}
```

<div id="comment:14" align="right">Comment 14</div>

Added reviewer names.



---

archive/issue_events_307436.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-05T06:54:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307436"
}
```



---

archive/issue_events_307437.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f8b263fb537d92c8ba9673cf19df0caf4a98f37e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-10-05T06:54:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23376#event-307437"
}
```



---

archive/issue_comments_353732.json:
```json
{
    "body": "Changed branch from **[u/sbrandhorst/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results)** to **[24a5b0b](https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c)**",
    "created_at": "2017-10-05T06:54:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353732",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/sbrandhorst/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/equality_testing_of_genera_of_quadratic_forms_over_zz_changes_the_genus_and_produces_false_results)** to **[24a5b0b](https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c)**



---

archive/issue_comments_353733.json:
```json
{
    "body": "Changed commit from **[24a5b0b](https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c)** to none",
    "created_at": "2017-12-08T14:47:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353733",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[24a5b0b](https://github.com/sagemath/sagetrac-mirror/commit/24a5b0bcc3f31e1dcf08ce88c0c709578a8bbf7c)** to none



---

archive/issue_comments_353734.json:
```json
{
    "body": "Changed reviewer from **Anthony Varilly-Alvarado, Jen Berg** to **Anthony V\u00e1rilly-Alvarado, Jen Berg**",
    "created_at": "2017-12-08T14:47:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23376#issuecomment-353734",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Anthony Varilly-Alvarado, Jen Berg** to **Anthony Várilly-Alvarado, Jen Berg**
