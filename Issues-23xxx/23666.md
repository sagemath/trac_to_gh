# Issue 23666: Unlinkable documentation files

archive/issues_023429.json:
```json
{
    "assignees": [],
    "body": "There are two files that refer to documents up the tree without any problem:\n* `src/doc/en/prep/Quickstarts/Multivariable-Calculus.rst` links `<../Calculus>`\n* `src/doc/en/prep/Quickstarts/Interact.rst` links to `<../Programming>`\n\nbut it was not possible for me to compile a link to `<../polynomial_rings/index>` included in the file `src/doc/en/reference/calculus/index.rst` nor a similar link upwards.\n\nCondition: it must be compiled with `make` after `make doc-clean`\n\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23666_\n\n",
    "created_at": "2017-08-21T15:48:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Unlinkable documentation files",
    "type": "issue",
    "updated_at": "2022-12-29T01:40:01Z",
    "url": "https://github.com/sagemath/sage/issues/23666",
    "user": "https://github.com/rwst"
}
```
There are two files that refer to documents up the tree without any problem:
* `src/doc/en/prep/Quickstarts/Multivariable-Calculus.rst` links `<../Calculus>`
* `src/doc/en/prep/Quickstarts/Interact.rst` links to `<../Programming>`

but it was not possible for me to compile a link to `<../polynomial_rings/index>` included in the file `src/doc/en/reference/calculus/index.rst` nor a similar link upwards.

Condition: it must be compiled with `make` after `make doc-clean`


_Issue created by migration from https://trac.sagemath.org/ticket/23666_





---

archive/issue_events_311102.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-08-21T15:48:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23666",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23666#event-311102"
}
```



---

archive/issue_events_311103.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-08-21T15:48:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23666",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23666#event-311103"
}
```



---

archive/issue_events_311104.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-08-21T15:48:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23666",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23666#event-311104"
}
```



---

archive/issue_events_311105.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-08-21T15:48:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23666",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23666#event-311105"
}
```



---

archive/issue_comments_358928.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nWhat works is creating an RST link, i.e. a file in the current directory containing `.. include:: ../file`. This does not explain why referring to `../file` directly works in `en/prep`.",
    "created_at": "2018-02-28T09:38:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23666#issuecomment-358928",
    "user": "https://github.com/rwst"
}
```

<div id="comment:1" align="right">Comment 1</div>

What works is creating an RST link, i.e. a file in the current directory containing `.. include:: ../file`. This does not explain why referring to `../file` directly works in `en/prep`.



---

archive/issue_events_311106.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:40:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23666",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23666#event-311106"
}
```
