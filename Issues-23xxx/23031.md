# Issue 23031: Fix ECM 7.0 build on Cygwin

archive/issues_022794.json:
```json
{
    "assignees": [],
    "body": "For some reason, if ECM detects that GMP is built as a shared library (DLL) on Windows, it refuses to build unless the flags `--enable-shared --disable-static` are given explicitly.  I don't think (?) there's any need for a static build anyways so using the flags as suggested should suffice.\n\nKeywords: **windows cygwin ecm**\n\nBranch/Commit: **[39700ce](https://github.com/sagemath/sagetrac-mirror/commit/39700cedf1994bd63687ac2b83f69208625d4b33)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/23031_\n\n",
    "closed_at": "2017-05-21T21:51:59Z",
    "created_at": "2017-05-19T12:18:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix ECM 7.0 build on Cygwin",
    "type": "issue",
    "updated_at": "2017-05-21T21:51:59Z",
    "url": "https://github.com/sagemath/sage/issues/23031",
    "user": "https://github.com/embray"
}
```
For some reason, if ECM detects that GMP is built as a shared library (DLL) on Windows, it refuses to build unless the flags `--enable-shared --disable-static` are given explicitly.  I don't think (?) there's any need for a static build anyways so using the flags as suggested should suffice.

Keywords: **windows cygwin ecm**

Branch/Commit: **[39700ce](https://github.com/sagemath/sagetrac-mirror/commit/39700cedf1994bd63687ac2b83f69208625d4b33)**

Reviewer: **Travis Scrimshaw**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/23031_





---

archive/issue_events_303234.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-05-19T12:18:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23031#event-303234"
}
```



---

archive/issue_events_303235.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-05-19T12:18:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23031#event-303235"
}
```



---

archive/issue_events_303236.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-05-19T12:18:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23031#event-303236"
}
```



---

archive/issue_events_303237.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-05-19T12:18:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23031#event-303237"
}
```



---

archive/issue_events_303238.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-05-19T12:18:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23031#event-303238"
}
```



---

archive/issue_comments_347510.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-05-20T20:43:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23031#issuecomment-347510",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_347511.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWorks for me on Cygwin. Off to the buildbots.",
    "created_at": "2017-05-20T20:43:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23031#issuecomment-347511",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
Works for me on Cygwin. Off to the buildbots.



---

archive/issue_events_303239.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-20T20:43:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23031#event-303239"
}
```



---

archive/issue_events_303240.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-20T20:43:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23031#event-303240"
}
```



---

archive/issue_events_303241.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-05-21T21:51:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23031#event-303241"
}
```



---

archive/issue_events_303242.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-05-21T21:51:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23031#event-303242"
}
```



---

archive/issue_comments_347512.json:
```json
{
    "body": "Changed branch from **[u/embray/cygwin/ecm-7.0](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/ecm-7.0)** to **[39700ce](https://github.com/sagemath/sagetrac-mirror/commit/39700cedf1994bd63687ac2b83f69208625d4b33)**",
    "created_at": "2017-05-21T21:51:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/23031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23031#issuecomment-347512",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/cygwin/ecm-7.0](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/ecm-7.0)** to **[39700ce](https://github.com/sagemath/sagetrac-mirror/commit/39700cedf1994bd63687ac2b83f69208625d4b33)**
