# Issue 23782: fricas output and sage conversion bug

archive/issues_023545.json:
```json
{
    "body": "In 'SageMath version 8.1.beta0, Release Date: 2017-07-29' I have the following behaviour, (at least with FriCAS 1.2.5)\n\n```\ndef elementary(k, j):\n    \"\"\"\n    sage: f = fricas.guessRat([elementary(5,j) for j in range(15)])[0]; f\n    10      9      8      7       6      5       4      3       2\n    3n   - 25n  + 50n  + 62n  - 229n  - 25n  + 320n  - 12n  - 144n\n    ---------------------------------------------------------------\n                                 11520\n\n    sage: print fricas.get(f._name)\n      10      9      8      7       6      5       4      3       2\n    3n   - 25n  + 50n  + 62n  - 229n  - 25n  + 320n  - 12n  - 144n\n    ---------------------------------------------------------------\n                                 11520\n\n    sage: f.sage()\n    NotImplementedError: The translation of the FriCAS Expression \"(3*n^10+-25*n^9+50*n^8+62*n^7+-229*n^6+-25*n^5+320*n^4+-12*n^3+-144*n^2)/11520 to sage is not yet implemented.\n    \"\"\"\n    return stirling_number1(j+1, j+1-k)\n```\nNote the bad indentation in the second line and the weird quotation mark in the third.\n\nFixing #22525 would probably fix this, too.\n\nCC:  simonking @fchapoton @rwst\n\nKeywords: FriCAS\n\nBranch/Commit: 2521f3a9556ea763d418df9e04b36e84b6137cb3\n\nReviewer: Ralf Stephan\n\nAuthor: Martin Rubey\n\nDependencies: #22525\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/23782\n\n",
    "closed_at": "2017-10-01T00:19:04Z",
    "created_at": "2017-09-04T19:24:04Z",
    "labels": [
        "component: interfaces: optional",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.1",
    "title": "fricas output and sage conversion bug",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/23782",
    "user": "https://github.com/mantepse"
}
```
In 'SageMath version 8.1.beta0, Release Date: 2017-07-29' I have the following behaviour, (at least with FriCAS 1.2.5)

```
def elementary(k, j):
    """
    sage: f = fricas.guessRat([elementary(5,j) for j in range(15)])[0]; f
    10      9      8      7       6      5       4      3       2
    3n   - 25n  + 50n  + 62n  - 229n  - 25n  + 320n  - 12n  - 144n
    ---------------------------------------------------------------
                                 11520

    sage: print fricas.get(f._name)
      10      9      8      7       6      5       4      3       2
    3n   - 25n  + 50n  + 62n  - 229n  - 25n  + 320n  - 12n  - 144n
    ---------------------------------------------------------------
                                 11520

    sage: f.sage()
    NotImplementedError: The translation of the FriCAS Expression "(3*n^10+-25*n^9+50*n^8+62*n^7+-229*n^6+-25*n^5+320*n^4+-12*n^3+-144*n^2)/11520 to sage is not yet implemented.
    """
    return stirling_number1(j+1, j+1-k)
```
Note the bad indentation in the second line and the weird quotation mark in the third.

Fixing #22525 would probably fix this, too.

CC:  simonking @fchapoton @rwst

Keywords: FriCAS

Branch/Commit: 2521f3a9556ea763d418df9e04b36e84b6137cb3

Reviewer: Ralf Stephan

Author: Martin Rubey

Dependencies: #22525

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/23782





---

archive/issue_comments_448668.json:
```json
{
    "body": "Changing keywords from \"\" to \"FriCAS\".",
    "created_at": "2017-09-05T06:13:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448668",
    "user": "https://github.com/fchapoton"
}
```

Changing keywords from "" to "FriCAS".



---

archive/issue_comments_448669.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis seems to work after #21377",
    "created_at": "2017-09-06T11:45:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448669",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'></a>
This seems to work after #21377



---

archive/issue_comments_448670.json:
```json
{
    "body": "<a id='comment:3'></a>\nAfter #21377 I still get the bad indentation, but the error is gone.  So I vote to close...\n\nI'll put the remark concerning the bad indentation on #21377.",
    "created_at": "2017-09-06T16:31:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448670",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:3'></a>
After #21377 I still get the bad indentation, but the error is gone.  So I vote to close...

I'll put the remark concerning the bad indentation on #21377.



---

archive/issue_comments_448671.json:
```json
{
    "body": "<a id='comment:4'></a>\nI just realised that I had a sage 7.5.beta3 lying around:\n\nthe display bug was introduced **after** 7.5.beta3 - the bug with the quotation mark existed **already** in 7.5.beta3!",
    "created_at": "2017-09-12T06:16:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448671",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:4'></a>
I just realised that I had a sage 7.5.beta3 lying around:

the display bug was introduced **after** 7.5.beta3 - the bug with the quotation mark existed **already** in 7.5.beta3!



---

archive/issue_comments_448672.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to defect.",
    "created_at": "2017-09-12T06:16:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448672",
    "user": "https://github.com/mantepse"
}
```

Changing type from PLEASE CHANGE to defect.



---

archive/issue_comments_448673.json:
```json
{
    "body": "<a id='comment:5'></a>\nMoreover, there is no change in the code of `fricas.py` between sage 7.5.beta3 and 8.0.beta12, which is the first version I have on my computer where the bug appears.\n\nI have no idea what could have changed - I'm afraid I need to bisect.",
    "created_at": "2017-09-12T06:24:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448673",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:5'></a>
Moreover, there is no change in the code of `fricas.py` between sage 7.5.beta3 and 8.0.beta12, which is the first version I have on my computer where the bug appears.

I have no idea what could have changed - I'm afraid I need to bisect.



---

archive/issue_comments_448674.json:
```json
{
    "body": "<a id='comment:6'></a>\n\n```\nsage:  fricas('(1 + sqrt(2))^5')\n\n+-+\n29\\|2  + 41\nsage: a= fricas('(1 + sqrt(2))^5')\nsage: print a\n+-+\n29\\|2  + 41\nsage: print fricas.get(a._name)\n   +-+\n29\\|2  + 41\nsage: repr(a)\n'+-+\\r\\n29\\\\|2  + 41'\nsage: fricas.get(a._name)\n'   +-+\\r\\n29\\\\|2  + 41'\n```",
    "created_at": "2017-09-12T06:33:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448674",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:6'></a>

```
sage:  fricas('(1 + sqrt(2))^5')

+-+
29\|2  + 41
sage: a= fricas('(1 + sqrt(2))^5')
sage: print a
+-+
29\|2  + 41
sage: print fricas.get(a._name)
   +-+
29\|2  + 41
sage: repr(a)
'+-+\r\n29\\|2  + 41'
sage: fricas.get(a._name)
'   +-+\r\n29\\|2  + 41'
```



---

archive/issue_comments_448675.json:
```json
{
    "body": "<a id='comment:7'></a>\nOK, I found it:\n\n```\ncommit 5082c96e79a15b34779a0c9b0b43d652cdade31c\nAuthor: Simon King <simon.king@uni-jena.de>\nDate:   Tue Mar 14 14:53:36 2017 +0100\n\n    Use single underscore _repr_ in interfaces, add more documentation concerning eval and get\n```\nintroduced this change in #22501:\n\n```\n+    def _repr_(self):\n+        \"\"\"\n+        Default implementation of a helper method for string representation.\n...\n+        \"\"\"\n+        P = self.parent()\n+        try:\n+            if self._get_using_file:\n+                return P.get_using_file(self._name).strip()\n+        except AttributeError:\n+            return self.parent().get(self._name).strip()\n```\n\nI think the strip should go away.  The other solution is to override it in `fricas.py`, but I really cannot think of a situation where we really want the `strip`.  Output will often be ascii-art in interfaces, and this is a default implementation.",
    "created_at": "2017-09-12T06:43:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448675",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:7'></a>
OK, I found it:

```
commit 5082c96e79a15b34779a0c9b0b43d652cdade31c
Author: Simon King <simon.king@uni-jena.de>
Date:   Tue Mar 14 14:53:36 2017 +0100

    Use single underscore _repr_ in interfaces, add more documentation concerning eval and get
```
introduced this change in #22501:

```
+    def _repr_(self):
+        """
+        Default implementation of a helper method for string representation.
...
+        """
+        P = self.parent()
+        try:
+            if self._get_using_file:
+                return P.get_using_file(self._name).strip()
+        except AttributeError:
+            return self.parent().get(self._name).strip()
```

I think the strip should go away.  The other solution is to override it in `fricas.py`, but I really cannot think of a situation where we really want the `strip`.  Output will often be ascii-art in interfaces, and this is a default implementation.



---

archive/issue_comments_448676.json:
```json
{
    "body": "<a id='comment:8'></a>\nI want to add that I commented on #22501 that fricas tests would be passing.  This means that I made a mistake then, so the blame is on me.",
    "created_at": "2017-09-12T06:59:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448676",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:8'></a>
I want to add that I commented on #22501 that fricas tests would be passing.  This means that I made a mistake then, so the blame is on me.



---

archive/issue_comments_448677.json:
```json
{
    "body": "Changing branch from \"\" to \"u/mantepse/fricas_output_and_sage_conversion_bug\"",
    "created_at": "2017-09-12T08:21:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448677",
    "user": "https://github.com/mantepse"
}
```

Changing branch from "" to "u/mantepse/fricas_output_and_sage_conversion_bug"



---

archive/issue_comments_448678.json:
```json
{
    "body": "Changing commit from \"\" to \"20f1d4b42bfc840406e0db132a8970dbf561ff61\"",
    "created_at": "2017-09-12T08:22:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448678",
    "user": "https://github.com/mantepse"
}
```

Changing commit from "" to "20f1d4b42bfc840406e0db132a8970dbf561ff61"



---

archive/issue_comments_448679.json:
```json
{
    "body": "Changing author from \"\" to \"Martin Rubey\"",
    "created_at": "2017-09-12T08:22:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448679",
    "user": "https://github.com/mantepse"
}
```

Changing author from "" to "Martin Rubey"



---

archive/issue_comments_448680.json:
```json
{
    "body": "<a id='comment:10'></a>\nNew commits:\n|                                                                                                                                          |                                                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|\n|[20f1d4b](https://git.sagemath.org/sage.git/commit?id=20f1d4b42bfc840406e0db132a8970dbf561ff61)|`do not strip output in default InterfaceElement._repr_`|",
    "created_at": "2017-09-12T08:22:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448680",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:10'></a>
New commits:
|                                                                                                                                          |                                                        |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|
|[20f1d4b](https://git.sagemath.org/sage.git/commit?id=20f1d4b42bfc840406e0db132a8970dbf561ff61)|`do not strip output in default InterfaceElement._repr_`|



---

archive/issue_comments_448681.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-09-12T08:22:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448681",
    "user": "https://github.com/mantepse"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_448682.json:
```json
{
    "body": "<a id='comment:11'></a>\nI would suggest to use rstrip instead of strip.",
    "created_at": "2017-09-12T08:27:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448682",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:11'></a>
I would suggest to use rstrip instead of strip.



---

archive/issue_comments_448683.json:
```json
{
    "body": "<a id='comment:12'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                               |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|\n|[5e72d43](https://git.sagemath.org/sage.git/commit/?id=5e72d438cbce766972d66af7516b851c82a62ca7)|`add a test for the weird quotation mark issue`|",
    "created_at": "2017-09-12T08:30:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448683",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:12'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                               |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|
|[5e72d43](https://git.sagemath.org/sage.git/commit/?id=5e72d438cbce766972d66af7516b851c82a62ca7)|`add a test for the weird quotation mark issue`|



---

archive/issue_comments_448684.json:
```json
{
    "body": "Changing commit from \"20f1d4b42bfc840406e0db132a8970dbf561ff61\" to \"5e72d438cbce766972d66af7516b851c82a62ca7\"",
    "created_at": "2017-09-12T08:30:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448684",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "20f1d4b42bfc840406e0db132a8970dbf561ff61" to "5e72d438cbce766972d66af7516b851c82a62ca7"



---

archive/issue_comments_448685.json:
```json
{
    "body": "<a id='comment:13'></a>\nReplying to [chapoton](#comment%3A11):\n> I would suggest to use rstrip instead of strip.\n\n\nI agree that this is a sensible possibility, but I'm not completely sure.\n\nCan you think of any surprising results with rstrip?",
    "created_at": "2017-09-12T08:32:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448685",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:13'></a>
Replying to [chapoton](#comment%3A11):
> I would suggest to use rstrip instead of strip.


I agree that this is a sensible possibility, but I'm not completely sure.

Can you think of any surprising results with rstrip?



---

archive/issue_comments_448686.json:
```json
{
    "body": "Changing commit from \"5e72d438cbce766972d66af7516b851c82a62ca7\" to \"4520a6e734c35ce816bfcc0e7e8b6c5b0b3f6c7c\"",
    "created_at": "2017-09-12T09:37:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448686",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "5e72d438cbce766972d66af7516b851c82a62ca7" to "4520a6e734c35ce816bfcc0e7e8b6c5b0b3f6c7c"



---

archive/issue_comments_448687.json:
```json
{
    "body": "<a id='comment:14'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                                      |\n|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|\n|[4520a6e](https://git.sagemath.org/sage.git/commit/?id=4520a6e734c35ce816bfcc0e7e8b6c5b0b3f6c7c)|`use rstrip in _repr_ (instead of original strip and instead of nothing), adapt tests`|",
    "created_at": "2017-09-12T09:37:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448687",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:14'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                                      |
|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
|[4520a6e](https://git.sagemath.org/sage.git/commit/?id=4520a6e734c35ce816bfcc0e7e8b6c5b0b3f6c7c)|`use rstrip in _repr_ (instead of original strip and instead of nothing), adapt tests`|



---

archive/issue_comments_448688.json:
```json
{
    "body": "<a id='comment:15'></a>\nI tested fricas, lie and gap3.  Unfortunately, gap3 is failing, but I do not know yet whether the failures are related to this ticket.",
    "created_at": "2017-09-12T09:39:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448688",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:15'></a>
I tested fricas, lie and gap3.  Unfortunately, gap3 is failing, but I do not know yet whether the failures are related to this ticket.



---

archive/issue_comments_448689.json:
```json
{
    "body": "<a id='comment:16'></a>\nI get the same failures in all other versions of sage I have lying around...  So I think that this is really ready to go.  Please review!",
    "created_at": "2017-09-12T09:44:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448689",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:16'></a>
I get the same failures in all other versions of sage I have lying around...  So I think that this is really ready to go.  Please review!



---

archive/issue_comments_448690.json:
```json
{
    "body": "<a id='comment:17'></a>\n`Check that #23782 is fixed:`\n\nshould be\n\n`Check that :trac:`23782` is fixed::`",
    "created_at": "2017-09-12T09:53:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448690",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:17'></a>
`Check that #23782 is fixed:`

should be

`Check that :trac:`23782` is fixed::`



---

archive/issue_comments_448691.json:
```json
{
    "body": "<a id='comment:18'></a>\nnote also that \"atan\" will be converted to \"arctan\" after #22525",
    "created_at": "2017-09-12T10:09:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448691",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:18'></a>
note also that "atan" will be converted to "arctan" after #22525



---

archive/issue_comments_448692.json:
```json
{
    "body": "Changing commit from \"4520a6e734c35ce816bfcc0e7e8b6c5b0b3f6c7c\" to \"b0ab79f9b3da0cd5862a9438e6f0d34b7947c4d0\"",
    "created_at": "2017-09-12T10:13:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448692",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "4520a6e734c35ce816bfcc0e7e8b6c5b0b3f6c7c" to "b0ab79f9b3da0cd5862a9438e6f0d34b7947c4d0"



---

archive/issue_comments_448693.json:
```json
{
    "body": "<a id='comment:19'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                       |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|\n|[b0ab79f](https://git.sagemath.org/sage.git/commit/?id=b0ab79f9b3da0cd5862a9438e6f0d34b7947c4d0)|`fix reference to trac`|",
    "created_at": "2017-09-12T10:13:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448693",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:19'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                       |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|
|[b0ab79f](https://git.sagemath.org/sage.git/commit/?id=b0ab79f9b3da0cd5862a9438e6f0d34b7947c4d0)|`fix reference to trac`|



---

archive/issue_comments_448694.json:
```json
{
    "body": "<a id='comment:20'></a>\nReplying to [chapoton](#comment%3A18):\n> note also that \"atan\" will be converted to \"arctan\" after #22525\n\n\nyes, I just looked at it, and have a branch that only adds some testcases - I would set it to positive review there and make this one a dependency.  Or would you prefer it the other way round?",
    "created_at": "2017-09-12T10:14:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448694",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:20'></a>
Replying to [chapoton](#comment%3A18):
> note also that "atan" will be converted to "arctan" after #22525


yes, I just looked at it, and have a branch that only adds some testcases - I would set it to positive review there and make this one a dependency.  Or would you prefer it the other way round?



---

archive/issue_comments_448695.json:
```json
{
    "body": "<a id='comment:21'></a>\nThe branch here is slightly more problematic, because you change the behaviour of all the interfaces and this needs serious checking. The other one should be easier and go first, imho.",
    "created_at": "2017-09-12T10:18:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448695",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:21'></a>
The branch here is slightly more problematic, because you change the behaviour of all the interfaces and this needs serious checking. The other one should be easier and go first, imho.



---

archive/issue_comments_448696.json:
```json
{
    "body": "<a id='comment:22'></a>\nI agree.  So let's do the other one first.  I have problems with checking out right now, though.",
    "created_at": "2017-09-12T10:19:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448696",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:22'></a>
I agree.  So let's do the other one first.  I have problems with checking out right now, though.



---

archive/issue_comments_448697.json:
```json
{
    "body": "<a id='comment:23'></a>\nI'll undo the modified line in integral.py after recompiling.",
    "created_at": "2017-09-12T10:34:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448697",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:23'></a>
I'll undo the modified line in integral.py after recompiling.



---

archive/issue_comments_448698.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#22525\"",
    "created_at": "2017-09-12T10:34:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448698",
    "user": "https://github.com/mantepse"
}
```

Changing dependencies from "" to "#22525"



---

archive/issue_comments_448699.json:
```json
{
    "body": "<a id='comment:24'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                           |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------|\n|[c4a44f6](https://git.sagemath.org/sage.git/commit/?id=c4a44f633790effdff1d4123e8a7a7c3155e9c9e)|`undo edit of test output - this is fixed by #22525 anyway`|",
    "created_at": "2017-09-12T11:48:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448699",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:24'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                           |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------|
|[c4a44f6](https://git.sagemath.org/sage.git/commit/?id=c4a44f633790effdff1d4123e8a7a7c3155e9c9e)|`undo edit of test output - this is fixed by #22525 anyway`|



---

archive/issue_comments_448700.json:
```json
{
    "body": "Changing commit from \"b0ab79f9b3da0cd5862a9438e6f0d34b7947c4d0\" to \"c4a44f633790effdff1d4123e8a7a7c3155e9c9e\"",
    "created_at": "2017-09-12T11:48:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448700",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "b0ab79f9b3da0cd5862a9438e6f0d34b7947c4d0" to "c4a44f633790effdff1d4123e8a7a7c3155e9c9e"



---

archive/issue_comments_448701.json:
```json
{
    "body": "<a id='comment:25'></a>\nI think this is ready now.",
    "created_at": "2017-09-12T11:49:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448701",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:25'></a>
I think this is ready now.



---

archive/issue_comments_448702.json:
```json
{
    "body": "<a id='comment:26'></a>\nThere will be a merge conflict - I guess it's easiest to fix it when #22525 is merged.",
    "created_at": "2017-09-12T11:55:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448702",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:26'></a>
There will be a merge conflict - I guess it's easiest to fix it when #22525 is merged.



---

archive/issue_comments_448703.json:
```json
{
    "body": "<a id='comment:27'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                           |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------|\n|[2521f3a](https://git.sagemath.org/sage.git/commit/?id=2521f3a9556ea763d418df9e04b36e84b6137cb3)|`Merge branch 'develop' into t/23782/fricas_output_and_sage_conversion_bug`|",
    "created_at": "2017-09-22T06:24:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448703",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:27'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                           |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------|
|[2521f3a](https://git.sagemath.org/sage.git/commit/?id=2521f3a9556ea763d418df9e04b36e84b6137cb3)|`Merge branch 'develop' into t/23782/fricas_output_and_sage_conversion_bug`|



---

archive/issue_comments_448704.json:
```json
{
    "body": "Changing commit from \"c4a44f633790effdff1d4123e8a7a7c3155e9c9e\" to \"2521f3a9556ea763d418df9e04b36e84b6137cb3\"",
    "created_at": "2017-09-22T06:24:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448704",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "c4a44f633790effdff1d4123e8a7a7c3155e9c9e" to "2521f3a9556ea763d418df9e04b36e84b6137cb3"



---

archive/issue_comments_448705.json:
```json
{
    "body": "<a id='comment:29'></a>\nping?",
    "created_at": "2017-09-26T07:22:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448705",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:29'></a>
ping?



---

archive/issue_comments_448706.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2017-09-26T07:39:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448706",
    "user": "https://github.com/rwst"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_448707.json:
```json
{
    "body": "<a id='comment:30'></a>\nUsing --optional=fricas I get\n\n`sage -t --warn-long 40.1 src/sage/interfaces/fricas.py  # 3 doctests failed`",
    "created_at": "2017-09-26T07:39:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448707",
    "user": "https://github.com/rwst"
}
```

<a id='comment:30'></a>
Using --optional=fricas I get

`sage -t --warn-long 40.1 src/sage/interfaces/fricas.py  # 3 doctests failed`



---

archive/issue_comments_448708.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Ralf Stephan\"",
    "created_at": "2017-09-26T07:39:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448708",
    "user": "https://github.com/rwst"
}
```

Changing reviewer from "" to "Ralf Stephan"



---

archive/issue_comments_448709.json:
```json
{
    "body": "<a id='comment:31'></a>\nI was told to use\n`sage -t --optional fricas,sage src/sage/interfaces/fricas.py`",
    "created_at": "2017-09-26T08:00:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448709",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:31'></a>
I was told to use
`sage -t --optional fricas,sage src/sage/interfaces/fricas.py`



---

archive/issue_comments_448710.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2017-09-26T08:30:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448710",
    "user": "https://github.com/mantepse"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_448711.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-09-26T13:09:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448711",
    "user": "https://github.com/rwst"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_448712.json:
```json
{
    "body": "<a id='comment:33'></a>\nAlright, interesting, thanks. It passes now and looks good.",
    "created_at": "2017-09-26T13:09:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448712",
    "user": "https://github.com/rwst"
}
```

<a id='comment:33'></a>
Alright, interesting, thanks. It passes now and looks good.



---

archive/issue_comments_448713.json:
```json
{
    "body": "<a id='comment:34'></a>\nThank you!",
    "created_at": "2017-09-26T13:59:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448713",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:34'></a>
Thank you!



---

archive/issue_comments_448714.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2017-10-01T00:19:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448714",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_448715.json:
```json
{
    "body": "Changing branch from \"u/mantepse/fricas_output_and_sage_conversion_bug\" to \"2521f3a9556ea763d418df9e04b36e84b6137cb3\"",
    "created_at": "2017-10-01T00:19:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23782#issuecomment-448715",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/mantepse/fricas_output_and_sage_conversion_bug" to "2521f3a9556ea763d418df9e04b36e84b6137cb3"



---

archive/issue_events_060660.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-01T00:19:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/23782",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/23782#event-60660"
}
```
