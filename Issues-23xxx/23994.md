...
```json
{
    "body": "Now it looks good to me too ;)",
...
```json
{
    "body": "see discussion at \u200bhttps://groups.google.com/forum/#!topic/sage-devel/56sS2HnsOIg",
...
```json
{
    "body": "Should we consider that as back to positive review?",
...
```json
{
    "body": "`graphs.CycleGraph(4).is_eulerian(path=True)` => `False`.\n\nWhat?",
...
```json
{
    "body": "However, that is a separate issue for its own ticket.",
...
```json
{
    "body": "The best way to avoid confusion is to have different methods with clear names and with seealso blocks pointing to other methods.",
