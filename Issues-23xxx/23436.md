# Issue 23436: sum/product of immutable matrices is not immutable

archive/issues_023199.json:
```json
{
    "body": "\n```\nsage: M = Matrix([[1,2],[3,4]]); M.set_immutable()\nsage: N = Matrix([[2,2],[4,4]]); N.set_immutable()\nsage: (M + N).is_immutable()\nFalse\nsage: (M * N).is_immutable()\nFalse\n```\nThis is counter-intuitive. The result should again be immutable.\n\nCC:  @galipnik\n\nKeywords: matrix\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/23436\n\n",
    "created_at": "2017-07-14T15:25:46Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.1",
    "title": "sum/product of immutable matrices is not immutable",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/23436",
    "user": "https://github.com/dkrenn"
}
```

```
sage: M = Matrix([[1,2],[3,4]]); M.set_immutable()
sage: N = Matrix([[2,2],[4,4]]); N.set_immutable()
sage: (M + N).is_immutable()
False
sage: (M * N).is_immutable()
False
```
This is counter-intuitive. The result should again be immutable.

CC:  @galipnik

Keywords: matrix

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/23436





---

archive/issue_comments_440671.json:
```json
{
    "body": "<a id='comment:2'></a>For reference, this is how Python deals with it (`frozenset` being an immutable version of `set`):\n\n```\n>>> frozenset([1,2,3]) | frozenset([4,5,6])\nfrozenset({1, 2, 3, 4, 5, 6})\n>>> frozenset([1,2,3]) | set([4,5,6])\nfrozenset({1, 2, 3, 4, 5, 6})\n>>> set([1,2,3]) | frozenset([4,5,6])\n{1, 2, 3, 4, 5, 6}\n>>> set([1,2,3]) | set([4,5,6])\n{1, 2, 3, 4, 5, 6}\n```\n\nSo the mutability of `A | B` is taken from the mutability of `A`.",
    "created_at": "2017-07-15T08:33:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23436#issuecomment-440671",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>For reference, this is how Python deals with it (`frozenset` being an immutable version of `set`):

```
>>> frozenset([1,2,3]) | frozenset([4,5,6])
frozenset({1, 2, 3, 4, 5, 6})
>>> frozenset([1,2,3]) | set([4,5,6])
frozenset({1, 2, 3, 4, 5, 6})
>>> set([1,2,3]) | frozenset([4,5,6])
{1, 2, 3, 4, 5, 6}
>>> set([1,2,3]) | set([4,5,6])
{1, 2, 3, 4, 5, 6}
```

So the mutability of `A | B` is taken from the mutability of `A`.



---

archive/issue_comments_440672.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [jdemeyer](#comment%3A2):\n> For reference, this is how Python deals with it (`frozenset` being an immutable version of `set`):\n> \n> ```\n> >>> frozenset([1,2,3]) | frozenset([4,5,6])\n> frozenset({1, 2, 3, 4, 5, 6})\n> >>> frozenset([1,2,3]) | set([4,5,6])\n> frozenset({1, 2, 3, 4, 5, 6})\n> >>> set([1,2,3]) | frozenset([4,5,6])\n> {1, 2, 3, 4, 5, 6}\n> >>> set([1,2,3]) | set([4,5,6])\n> {1, 2, 3, 4, 5, 6}\n> ```\n> \n> So the mutability of `A | B` is taken from the mutability of `A`.\n\n\nIs this the desired behavior or should it be that if either is immutable than the resultant should be as well? For example, this is how networkx handles a graph product of a undirected and directed graph (in either order).",
    "created_at": "2017-07-25T17:20:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/23436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/23436#issuecomment-440672",
    "user": "https://trac.sagemath.org/admin/accounts/users/mcognetta"
}
```

<a id='comment:3'></a>Replying to [jdemeyer](#comment%3A2):
> For reference, this is how Python deals with it (`frozenset` being an immutable version of `set`):
> 
> ```
> >>> frozenset([1,2,3]) | frozenset([4,5,6])
> frozenset({1, 2, 3, 4, 5, 6})
> >>> frozenset([1,2,3]) | set([4,5,6])
> frozenset({1, 2, 3, 4, 5, 6})
> >>> set([1,2,3]) | frozenset([4,5,6])
> {1, 2, 3, 4, 5, 6}
> >>> set([1,2,3]) | set([4,5,6])
> {1, 2, 3, 4, 5, 6}
> ```
> 
> So the mutability of `A | B` is taken from the mutability of `A`.


Is this the desired behavior or should it be that if either is immutable than the resultant should be as well? For example, this is how networkx handles a graph product of a undirected and directed graph (in either order).
