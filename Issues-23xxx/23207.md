...
```json
{
    "body": "<a id='comment:10'></a>The commit in[comment:9](#comment%3A9) gets rid of some `six.iteritems` and `six.itervalues` since the lack of iterator is not crucial in the Python2 version; `.items()` and `.values()` suffice: no significant loss of performance and this makes the `python3_py` plugin of the patchbot happy ;-).",
...
<a id='comment:10'></a>The commit in[comment:9](#comment%3A9) gets rid of some `six.iteritems` and `six.itervalues` since the lack of iterator is not crucial in the Python2 version; `.items()` and `.values()` suffice: no significant loss of performance and this makes the `python3_py` plugin of the patchbot happy ;-).
