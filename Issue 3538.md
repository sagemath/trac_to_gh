# Issue 3538: [with patch; not ready for review] create a sage <--> Ruby interface

archive/issues_003538.json:
```json
{
    "body": "Assignee: was\n\nCreate an interface between Sage and Ruby. \n\nJosh Kantor and I wrote the attached one in 5 minutes and it worked the first try. \n\nWe haven't added any doctests and haven't implemented completions.\n\nIssue created by migration from https://trac.sagemath.org/ticket/3538\n\n",
    "created_at": "2008-06-30T14:20:06Z",
    "labels": [
        "interfaces",
        "major",
        "enhancement"
    ],
    "title": "[with patch; not ready for review] create a sage <--> Ruby interface",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3538",
    "user": "was"
}
```
Assignee: was

Create an interface between Sage and Ruby. 

Josh Kantor and I wrote the attached one in 5 minutes and it worked the first try. 

We haven't added any doctests and haven't implemented completions.

Issue created by migration from https://trac.sagemath.org/ticket/3538





---

archive/issue_comments_024998.json:
```json
{
    "body": "Attachment [sage-3538.patch](tarball://root/attachments/some-uuid/ticket3538/sage-3538.patch) by was created at 2008-06-30 14:20:40",
    "created_at": "2008-06-30T14:20:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-24998",
    "user": "was"
}
```

Attachment [sage-3538.patch](tarball://root/attachments/some-uuid/ticket3538/sage-3538.patch) by was created at 2008-06-30 14:20:40



---

archive/issue_comments_024999.json:
```json
{
    "body": "Changing assignee from was to mhansen.",
    "created_at": "2008-07-06T18:37:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-24999",
    "user": "was"
}
```

Changing assignee from was to mhansen.



---

archive/issue_comments_025000.json:
```json
{
    "body": "Since Mike owns this he might as well be the editor :)\n\nCheers,\n\nMichael",
    "created_at": "2008-07-07T03:30:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25000",
    "user": "mabshoff"
}
```

Since Mike owns this he might as well be the editor :)

Cheers,

Michael



---

archive/issue_comments_025001.json:
```json
{
    "body": "Changing keywords from \"\" to \"editor_mhansen\".",
    "created_at": "2008-07-07T03:30:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25001",
    "user": "mabshoff"
}
```

Changing keywords from "" to "editor_mhansen".



---

archive/issue_comments_025002.json:
```json
{
    "body": "New commits:",
    "created_at": "2014-07-18T21:09:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25002",
    "user": "chapoton"
}
```

New commits:



---

archive/issue_comments_025003.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-04-19T09:41:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25003",
    "user": "git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_025004.json:
```json
{
    "body": "This is clearly needing a lot of work. And documentation and doctests also",
    "created_at": "2015-04-19T09:42:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25004",
    "user": "chapoton"
}
```

This is clearly needing a lot of work. And documentation and doctests also



---

archive/issue_comments_025005.json:
```json
{
    "body": "Changing keywords from \"editor_mhansen\" to \"Ruby\".",
    "created_at": "2015-04-19T09:42:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25005",
    "user": "chapoton"
}
```

Changing keywords from "editor_mhansen" to "Ruby".



---

archive/issue_comments_025006.json:
```json
{
    "body": "Changing component from interfaces to interfaces: optional.",
    "created_at": "2015-06-23T13:49:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25006",
    "user": "jdemeyer"
}
```

Changing component from interfaces to interfaces: optional.



---

archive/issue_comments_025007.json:
```json
{
    "body": "I removed the utf-8 from the title, otherwise it is a mess with the patchbot.",
    "created_at": "2015-08-19T10:26:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25007",
    "user": "vdelecroix"
}
```

I removed the utf-8 from the title, otherwise it is a mess with the patchbot.



---

archive/issue_comments_025008.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-03-01T11:10:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25008",
    "user": "git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_025009.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-05-19T12:20:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25009",
    "user": "git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_025010.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-06-10T18:18:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3538#issuecomment-25010",
    "user": "git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:
