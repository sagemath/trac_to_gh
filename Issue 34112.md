# Issue 34112: SPKGs did not find equivalent system packages: Ubuntu 22.04.1 LTS

archive/issues_034112.json:
```json
{
    "body": "The following issue kept coming (after double checking that I had the packages installed as described)\n\n```\nchecking that generated files are newer than configure... done\nconfigure: creating ./config.status\nconfigure:\n\n    notice: the following SPKGs did not find equivalent system packages:\n\n        fflas_ffpack fplll givaro linbox lrcalc primecount qhull   4ti2 coxeter3 libsemigroups polymake _recommended\n        \nchecking for the package system in use... debian\nconfigure:\n\n    hint: installing the following system packages, if not\n    already present, is recommended and may avoid having to\n    build them (though some may have to be built anyway):\n\n      $ sudo apt-get update \n      $ sudo apt-get install  fflas-ffpack libfplll-dev libgivaro-dev liblinbox-dev liblrcalc-dev libqhull-dev\n\nconfigure:\n\n    hint: installing the following system packages, if not\n    already present, may provide additional optional features:\n\n      $ sudo apt-get update \n      $ sudo apt-get install  4ti2 polymake libpolymake-dev default-jdk libavdevice-dev\n```\n\nIssue created by migration from https://trac.sagemath.org/ticket/34349\n\n",
    "created_at": "2022-08-12T09:02:56Z",
    "labels": [
        "build: configure",
        "major",
        "bug"
    ],
    "title": "SPKGs did not find equivalent system packages: Ubuntu 22.04.1 LTS",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/34112",
    "user": "@heindel"
}
```
The following issue kept coming (after double checking that I had the packages installed as described)

```
checking that generated files are newer than configure... done
configure: creating ./config.status
configure:

    notice: the following SPKGs did not find equivalent system packages:

        fflas_ffpack fplll givaro linbox lrcalc primecount qhull   4ti2 coxeter3 libsemigroups polymake _recommended
        
checking for the package system in use... debian
configure:

    hint: installing the following system packages, if not
    already present, is recommended and may avoid having to
    build them (though some may have to be built anyway):

      $ sudo apt-get update 
      $ sudo apt-get install  fflas-ffpack libfplll-dev libgivaro-dev liblinbox-dev liblrcalc-dev libqhull-dev

configure:

    hint: installing the following system packages, if not
    already present, may provide additional optional features:

      $ sudo apt-get update 
      $ sudo apt-get install  4ti2 polymake libpolymake-dev default-jdk libavdevice-dev
```

Issue created by migration from https://trac.sagemath.org/ticket/34349





---

archive/issue_comments_483965.json:
```json
{
    "body": "Attachment [output.txt](tarball://root/attachments/some-uuid/ticket34349/output.txt) by @heindel created at 2022-08-12 09:06:34\n\nthe output",
    "created_at": "2022-08-12T09:06:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34112#issuecomment-483965",
    "user": "@heindel"
}
```

Attachment [output.txt](tarball://root/attachments/some-uuid/ticket34349/output.txt) by @heindel created at 2022-08-12 09:06:34

the output



---

archive/issue_comments_483966.json:
```json
{
    "body": "list of installed packages",
    "created_at": "2022-08-12T09:07:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34112#issuecomment-483966",
    "user": "@heindel"
}
```

list of installed packages



---

archive/issue_comments_483967.json:
```json
{
    "body": "Attachment [installedPackages.txt](tarball://root/attachments/some-uuid/ticket34349/installedPackages.txt) by slelievre created at 2022-08-12 19:32:41\n\nThanks for the report!\n\nOur suggestion mechanism is not perfect.\n\nThe phrasing (\"if not already present\", \"may\")\ntries to convey that.\n\nPropose to close as wontfix, unless someone\ncan suggest a concrete improvement.",
    "created_at": "2022-08-12T19:32:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34112#issuecomment-483967",
    "user": "slelievre"
}
```

Attachment [installedPackages.txt](tarball://root/attachments/some-uuid/ticket34349/installedPackages.txt) by slelievre created at 2022-08-12 19:32:41

Thanks for the report!

Our suggestion mechanism is not perfect.

The phrasing ("if not already present", "may")
tries to convey that.

Propose to close as wontfix, unless someone
can suggest a concrete improvement.



---

archive/issue_comments_483968.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2022-08-12T19:32:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34112#issuecomment-483968",
    "user": "slelievre"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_483969.json:
```json
{
    "body": "output.txt contains all the answers. For example:\n\n```\nChecking whether SageMath should install SPKG givaro...\n326\tchecking whether any of gmp is installed as or will be installed as SPKG... no\n327\tchecking for givaro-config... /usr/bin/givaro-config\n328\tchecking is givaro's version acceptable? ... no, too new\n329\tconfigure: no suitable system package found for SPKG givaro\n```\n",
    "created_at": "2022-08-12T19:41:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34112#issuecomment-483969",
    "user": "mkoeppe"
}
```

output.txt contains all the answers. For example:

```
Checking whether SageMath should install SPKG givaro...
326	checking whether any of gmp is installed as or will be installed as SPKG... no
327	checking for givaro-config... /usr/bin/givaro-config
328	checking is givaro's version acceptable? ... no, too new
329	configure: no suitable system package found for SPKG givaro
```




---

archive/issue_comments_483970.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-08-12T19:41:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34112#issuecomment-483970",
    "user": "mkoeppe"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_483971.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2022-09-01T02:30:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34112#issuecomment-483971",
    "user": "mkoeppe"
}
```

Resolution: invalid
