# Issue 4291: [with patch, needs review] Fix problem with IPython and init.sage

archive/issues_004291.json:
```json
{
    "body": "Assignee: mabshoff\n\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/4291\n\n",
    "created_at": "2008-10-15T10:48:58Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.4",
    "title": "[with patch, needs review] Fix problem with IPython and init.sage",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4291",
    "user": "https://github.com/mwhansen"
}
```
Assignee: mabshoff



Issue created by migration from https://trac.sagemath.org/ticket/4291





---

archive/issue_comments_031349.json:
```json
{
    "body": "Attachment [trac_4291.patch](tarball://root/attachments/some-uuid/ticket4291/trac_4291.patch) by @zimmermann6 created at 2008-10-15 14:20:22\n\nI tried it with a fresh 3.1.3 installation, and it works, both with and without an init.sage file.",
    "created_at": "2008-10-15T14:20:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4291#issuecomment-31349",
    "user": "https://github.com/zimmermann6"
}
```

Attachment [trac_4291.patch](tarball://root/attachments/some-uuid/ticket4291/trac_4291.patch) by @zimmermann6 created at 2008-10-15 14:20:22

I tried it with a fresh 3.1.3 installation, and it works, both with and without an init.sage file.



---

archive/issue_comments_031350.json:
```json
{
    "body": "Merged in Sage 3.1.4.\n\nCheers,\n\nMichael",
    "created_at": "2008-10-15T15:41:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4291#issuecomment-31350",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Merged in Sage 3.1.4.

Cheers,

Michael



---

archive/issue_comments_031351.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-10-15T15:41:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4291#issuecomment-31351",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_004535.json:
```json
{
    "actor": "mabshoff",
    "created_at": "2008-10-15T15:41:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4291",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4291#event-4535"
}
```
