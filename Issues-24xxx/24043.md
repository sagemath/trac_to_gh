# Issue 24043: Test if a (di)graph is self-complementary

archive/issues_023806.json:
```json
{
    "assignees": [],
    "body": "A simple (di)graph is self-complementary if it is isomorphic to its complement. \n\nA self-complementary graph is connected and traceable (i.e. it contains a hamiltonian path).\n\nCC:  @jm58660\n\nBranch/Commit: **[2e5c840](https://github.com/sagemath/sagetrac-mirror/commit/2e5c840e7c18c9be50680d75ab6a60e1cc1f85d1)**\n\nReviewer: **Jori M\u00e4ntysalo**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24043_\n\n",
    "closed_at": "2017-10-21T19:01:19Z",
    "created_at": "2017-10-14T11:07:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Test if a (di)graph is self-complementary",
    "type": "issue",
    "updated_at": "2017-10-21T19:01:19Z",
    "url": "https://github.com/sagemath/sage/issues/24043",
    "user": "https://github.com/dcoudert"
}
```
A simple (di)graph is self-complementary if it is isomorphic to its complement. 

A self-complementary graph is connected and traceable (i.e. it contains a hamiltonian path).

CC:  @jm58660

Branch/Commit: **[2e5c840](https://github.com/sagemath/sagetrac-mirror/commit/2e5c840e7c18c9be50680d75ab6a60e1cc1f85d1)**

Reviewer: **Jori Mäntysalo**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/24043_





---

archive/issue_events_315786.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2017-10-14T11:07:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315786"
}
```



---

archive/issue_events_315787.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2017-10-14T11:07:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315787"
}
```



---

archive/issue_events_315788.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2017-10-14T11:07:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315788"
}
```



---

archive/issue_events_315789.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2017-10-14T11:07:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315789"
}
```



---

archive/issue_events_315790.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2017-10-14T11:22:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315790"
}
```



---

archive/issue_comments_365973.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis could be interesting for testing traceable graphs.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c9a836846a011378123233f68543e1e09514ed3d\">c9a8368</a></td><td><code>trac #24043: add method is_self_complementary for (di)graphs</code></td></tr></table>\n",
    "created_at": "2017-10-14T11:22:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365973",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

This could be interesting for testing traceable graphs.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c9a836846a011378123233f68543e1e09514ed3d">c9a8368</a></td><td><code>trac #24043: add method is_self_complementary for (di)graphs</code></td></tr></table>




---

archive/issue_comments_365974.json:
```json
{
    "body": "Branch: **[u/dcoudert/24043](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/24043)**",
    "created_at": "2017-10-14T11:22:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365974",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[u/dcoudert/24043](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/24043)**



---

archive/issue_comments_365975.json:
```json
{
    "body": "Commit: **[c9a8368](https://github.com/sagemath/sagetrac-mirror/commit/c9a836846a011378123233f68543e1e09514ed3d)**",
    "created_at": "2017-10-14T11:22:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365975",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[c9a8368](https://github.com/sagemath/sagetrac-mirror/commit/c9a836846a011378123233f68543e1e09514ed3d)**



---

archive/issue_comments_365976.json:
```json
{
    "body": "Changed commit from **[c9a8368](https://github.com/sagemath/sagetrac-mirror/commit/c9a836846a011378123233f68543e1e09514ed3d)** to **[e17e0d1](https://github.com/sagemath/sagetrac-mirror/commit/e17e0d123ca53e61777c0f8e514f2f041bac84b6)**",
    "created_at": "2017-10-14T13:52:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365976",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c9a8368](https://github.com/sagemath/sagetrac-mirror/commit/c9a836846a011378123233f68543e1e09514ed3d)** to **[e17e0d1](https://github.com/sagemath/sagetrac-mirror/commit/e17e0d123ca53e61777c0f8e514f2f041bac84b6)**



---

archive/issue_comments_365977.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e17e0d123ca53e61777c0f8e514f2f041bac84b6\">e17e0d1</a></td><td><code>trac #24043: move seealso block after examples</code></td></tr></table>\n",
    "created_at": "2017-10-14T13:52:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365977",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e17e0d123ca53e61777c0f8e514f2f041bac84b6">e17e0d1</a></td><td><code>trac #24043: move seealso block after examples</code></td></tr></table>




---

archive/issue_comments_365978.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThe patchbot was complaining on the order of blocks. I try to follow [guidelines](http://doc.sagemath.org/html/en/developer/coding_basics.html#the-docstring-of-a-function-content).",
    "created_at": "2017-10-14T13:54:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365978",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:3" align="right">Comment 3</div>

The patchbot was complaining on the order of blocks. I try to follow [guidelines](http://doc.sagemath.org/html/en/developer/coding_basics.html#the-docstring-of-a-function-content).



---

archive/issue_events_315791.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-10-14T14:52:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315791"
}
```



---

archive/issue_events_315792.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-10-14T14:52:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315792"
}
```



---

archive/issue_comments_365979.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThis seems to be good addition... but bikeshedding follows.\n\nIMO every Boolean-valued function should have at least one example outputting `False` (and one `True`). This function has both, but quite implicitly said. I am not sure if the basic example should have `for`-loop at all -- maybe just `PathGraph(4)` and something else.\n\nAs there has been problems with corner cases I suggest adding `TESTS` section with empty and single-vertex graphs and digraphs for testing. Also some test should have \"right\" number of edges but fail to be self-complementary.\n\n`SEEALSO` blocks are mostly used for linking other functions in Sage. This kind of brokes the convention.\n\nI would write \"Check whether the graph is self-complementary.\" instead of \"Check whether self is self-complementary\". At least add the dot after sentence.",
    "created_at": "2017-10-14T14:52:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365979",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:4" align="right">Comment 4</div>

This seems to be good addition... but bikeshedding follows.

IMO every Boolean-valued function should have at least one example outputting `False` (and one `True`). This function has both, but quite implicitly said. I am not sure if the basic example should have `for`-loop at all -- maybe just `PathGraph(4)` and something else.

As there has been problems with corner cases I suggest adding `TESTS` section with empty and single-vertex graphs and digraphs for testing. Also some test should have "right" number of edges but fail to be self-complementary.

`SEEALSO` blocks are mostly used for linking other functions in Sage. This kind of brokes the convention.

I would write "Check whether the graph is self-complementary." instead of "Check whether self is self-complementary". At least add the dot after sentence.



---

archive/issue_comments_365980.json:
```json
{
    "body": "Reviewer: **Jori M\u00e4ntysalo**",
    "created_at": "2017-10-14T14:52:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365980",
    "user": "https://github.com/jm58660"
}
```

Reviewer: **Jori Mäntysalo**



---

archive/issue_comments_365981.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nAlso the last example on `graphs.PaleyGraph` could be changed to use this.",
    "created_at": "2017-10-14T16:28:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365981",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:5" align="right">Comment 5</div>

Also the last example on `graphs.PaleyGraph` could be changed to use this.



---

archive/issue_comments_365982.json:
```json
{
    "body": "Changed commit from **[e17e0d1](https://github.com/sagemath/sagetrac-mirror/commit/e17e0d123ca53e61777c0f8e514f2f041bac84b6)** to **[2e5c840](https://github.com/sagemath/sagetrac-mirror/commit/2e5c840e7c18c9be50680d75ab6a60e1cc1f85d1)**",
    "created_at": "2017-10-14T18:12:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365982",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e17e0d1](https://github.com/sagemath/sagetrac-mirror/commit/e17e0d123ca53e61777c0f8e514f2f041bac84b6)** to **[2e5c840](https://github.com/sagemath/sagetrac-mirror/commit/2e5c840e7c18c9be50680d75ab6a60e1cc1f85d1)**



---

archive/issue_comments_365983.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e43b5f165846b1b1966800b779ddf3e9a9c1c79c\">e43b5f1</a></td><td><code>trac #24043: more tests and better examples</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2e5c840e7c18c9be50680d75ab6a60e1cc1f85d1\">2e5c840</a></td><td><code>trac #24043: use the method in the test of PaleyGraph</code></td></tr></table>\n",
    "created_at": "2017-10-14T18:12:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365983",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e43b5f165846b1b1966800b779ddf3e9a9c1c79c">e43b5f1</a></td><td><code>trac #24043: more tests and better examples</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2e5c840e7c18c9be50680d75ab6a60e1cc1f85d1">2e5c840</a></td><td><code>trac #24043: use the method in the test of PaleyGraph</code></td></tr></table>




---

archive/issue_comments_365984.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nI have implemented all your comments. Thank you.",
    "created_at": "2017-10-14T18:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365984",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:7" align="right">Comment 7</div>

I have implemented all your comments. Thank you.



---

archive/issue_events_315793.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2017-10-14T18:21:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315793"
}
```



---

archive/issue_events_315794.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2017-10-14T18:21:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315794"
}
```



---

archive/issue_comments_365985.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThis is OK. Thanks.\n\n(Sidenote: I was thinking should it be self-complementary or selfcomplementary. Noticed that posets have `is_selfdual` and liner codes have `is_self_dual`. Duh.)",
    "created_at": "2017-10-14T19:20:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365985",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:9" align="right">Comment 9</div>

This is OK. Thanks.

(Sidenote: I was thinking should it be self-complementary or selfcomplementary. Noticed that posets have `is_selfdual` and liner codes have `is_self_dual`. Duh.)



---

archive/issue_events_315795.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-10-14T19:20:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315795"
}
```



---

archive/issue_events_315796.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-10-14T19:20:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315796"
}
```



---

archive/issue_comments_365986.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\ngood question. I'm in favour of `self_complementary` that I find easier to read.",
    "created_at": "2017-10-15T09:06:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365986",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:10" align="right">Comment 10</div>

good question. I'm in favour of `self_complementary` that I find easier to read.



---

archive/issue_events_315797.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-21T19:01:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315797"
}
```



---

archive/issue_events_315798.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "dbb1b38febc9a0efe4c7ff29c2d347ca2af48d4d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-10-21T19:01:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24043#event-315798"
}
```



---

archive/issue_comments_365987.json:
```json
{
    "body": "Changed branch from **[u/dcoudert/24043](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/24043)** to **[2e5c840](https://github.com/sagemath/sagetrac-mirror/commit/2e5c840e7c18c9be50680d75ab6a60e1cc1f85d1)**",
    "created_at": "2017-10-21T19:01:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24043#issuecomment-365987",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dcoudert/24043](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/24043)** to **[2e5c840](https://github.com/sagemath/sagetrac-mirror/commit/2e5c840e7c18c9be50680d75ab6a60e1cc1f85d1)**
