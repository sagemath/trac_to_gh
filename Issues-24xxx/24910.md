# Issue 24910: MacLane approximants for polynomial with non-integral roots

archive/issues_024673.json:
```json
{
    "assignees": [],
    "body": "Currently, the MacLane algorithm requires as input a polynomial G which is both monic and integral (with respect to the base valuation). Relaxing this condition would allow to approximate the irreducible factors with integral roots of an arbitrary polynomial G.\n\n```\nsage: R.<x> = QQ[]\nsage: v = QQ.valuation(2)\nsage: R.<x> = QQ[]\nsage: f = 1/2*x^2 + 1/3*x + 1\nsage: v.mac_lane_approximants(f)\nValueError: G must be integral\n```\n\nMigrated from https://github.com/saraedum/mac_lane/issues/5.\n\nCC:  swewers\n\nKeywords: **maclane, valuations**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24910_\n\n",
    "created_at": "2018-03-06T13:26:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "MacLane approximants for polynomial with non-integral roots",
    "type": "issue",
    "updated_at": "2022-12-29T01:40:48Z",
    "url": "https://github.com/sagemath/sage/issues/24910",
    "user": "https://github.com/saraedum"
}
```
Currently, the MacLane algorithm requires as input a polynomial G which is both monic and integral (with respect to the base valuation). Relaxing this condition would allow to approximate the irreducible factors with integral roots of an arbitrary polynomial G.

```
sage: R.<x> = QQ[]
sage: v = QQ.valuation(2)
sage: R.<x> = QQ[]
sage: f = 1/2*x^2 + 1/3*x + 1
sage: v.mac_lane_approximants(f)
ValueError: G must be integral
```

Migrated from https://github.com/saraedum/mac_lane/issues/5.

CC:  swewers

Keywords: **maclane, valuations**

_Issue created by migration from https://trac.sagemath.org/ticket/24910_





---

archive/issue_events_326621.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-06T13:26:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24910",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24910#event-326621"
}
```



---

archive/issue_events_326622.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-06T13:26:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24910",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24910#event-326622"
}
```



---

archive/issue_events_326623.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-06T13:26:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24910",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24910#event-326623"
}
```



---

archive/issue_events_326624.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-06T13:26:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24910",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24910#event-326624"
}
```



---

archive/issue_comments_381818.json:
```json
{
    "body": "Changed keywords from **** to **maclane, valuations**",
    "created_at": "2018-03-06T13:27:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24910#issuecomment-381818",
    "user": "https://github.com/saraedum"
}
```

Changed keywords from **** to **maclane, valuations**



---

archive/issue_events_326625.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:40:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24910",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24910#event-326625"
}
```
