...
```json
{
    "body": "<a id='comment:3'></a>No doctests need to be adapted for this release. Please review.",
    "created_at": "2017-11-22T06:56:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24262",
    "type": "issue_comment",
...
<a id='comment:3'></a>No doctests need to be adapted for this release. Please review.
...
<a id='comment:4'></a>On Debian 8.1.rc2 + #24240, compiles without difficulty.
...
<a id='comment:8'></a>Replying to [rws](#comment%3A5):
> Replying to [charpent](#comment%3A4):
...
```json
{
    "body": "<a id='comment:9'></a>Confirmed with `--long`.",
...
<a id='comment:9'></a>Confirmed with `--long`.
...
```json
{
    "body": "<a id='comment:10'></a>Okay the reason is a regression in `subs`, minimal case: `sage: (1+1/x).subs({x:1/x}) ... 1/x + 1`. We try to add a patch here with doctest.",
...
<a id='comment:10'></a>Okay the reason is a regression in `subs`, minimal case: `sage: (1+1/x).subs({x:1/x}) ... 1/x + 1`. We try to add a patch here with doctest.
...
```json
{
    "body": "<a id='comment:12'></a>A blocker is a ticket that reports/fixes an existing bug in Sage. This is a ticket that potentially introduces a severe bug, but with the right patch, it becomes a normal ticket. The sky does not fall down.",
...
<a id='comment:12'></a>A blocker is a ticket that reports/fixes an existing bug in Sage. This is a ticket that potentially introduces a severe bug, but with the right patch, it becomes a normal ticket. The sky does not fall down.
...
<a id='comment:15'></a>This fixes the issue. My ptestlong passes but you might want to test again too.
...
<a id='comment:16'></a>Doesn't smell good.. On top of 8.0.rc3+#24240, I get :
...
<a id='comment:17'></a>Replying to [charpent](#comment%3A16):
> Doesn't smell good.. On top of 8.0.rc3+#24240, I get :
...
```json
{
    "body": "<a id='comment:18'></a>It doesn't look like you updated your copy of the ticket branch, e.g.. git trac pull.24262.",
...
<a id='comment:18'></a>It doesn't look like you updated your copy of the ticket branch, e.g.. git trac pull.24262.
...
```json
{
    "body": "<a id='comment:20'></a>The usual gotcha: I forgot to bump the patchlevel, so Pynac was not recompiled when you started `make`.",
...
<a id='comment:20'></a>The usual gotcha: I forgot to bump the patchlevel, so Pynac was not recompiled when you started `make`.
...
<a id='comment:23'></a>Smells better :
...
```json
{
    "body": "<a id='comment:24'></a>On top of 8.0.rc3+#24240, passes `ptestlong` with no error whatsoever.\n\n==>`positive_review`",
    "created_at": "2017-11-27T09:48:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24262",
    "type": "issue_comment",
...
<a id='comment:24'></a>On top of 8.0.rc3+#24240, passes `ptestlong` with no error whatsoever.
...
```json
{
    "body": "<a id='comment:26'></a>We might as well upgrade to 0.7.14 in one go. I'll prepare #24329 as soon as 8.2.beta1 is out.",
...
<a id='comment:26'></a>We might as well upgrade to 0.7.14 in one go. I'll prepare #24329 as soon as 8.2.beta1 is out.
...
```json
{
    "body": "<a id='comment:27'></a>Duplicate of #24329.",
    "created_at": "2017-12-11T13:56:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24262",
    "type": "issue_comment",
...
<a id='comment:27'></a>Duplicate of #24329.
