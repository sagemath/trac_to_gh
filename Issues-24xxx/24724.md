# Issue 24724: Capture warning/errors messages in sage.misc.cython

archive/issues_024487.json:
```json
{
    "assignees": [],
    "body": "Cython allows redirecting warning/error messages. Instead of sending errors to stderr, they should be raised as exception.\n\nDepends on #24722\n\nBranch/Commit: **[52982f3](https://github.com/sagemath/sagetrac-mirror/commit/52982f36465228f4c02951e6f090b3e061be8650)**\n\nReviewer: **Fran\u00e7ois Bissey**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24724_\n\n",
    "closed_at": "2018-02-20T23:08:20Z",
    "created_at": "2018-02-13T14:03:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Capture warning/errors messages in sage.misc.cython",
    "type": "issue",
    "updated_at": "2018-02-20T23:08:20Z",
    "url": "https://github.com/sagemath/sage/issues/24724",
    "user": "https://github.com/jdemeyer"
}
```
Cython allows redirecting warning/error messages. Instead of sending errors to stderr, they should be raised as exception.

Depends on #24722

Branch/Commit: **[52982f3](https://github.com/sagemath/sagetrac-mirror/commit/52982f36465228f4c02951e6f090b3e061be8650)**

Reviewer: **François Bissey**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24724_





---

archive/issue_events_324256.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-13T14:03:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24724#event-324256"
}
```



---

archive/issue_events_324257.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-13T14:03:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24724#event-324257"
}
```



---

archive/issue_events_324258.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-13T14:03:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24724#event-324258"
}
```



---

archive/issue_events_324259.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-13T14:03:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24724#event-324259"
}
```



---

archive/issue_comments_378513.json:
```json
{
    "body": "Branch: **[u/jdemeyer/capture_warning_errors_messages_in_sage_misc_cython](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/capture_warning_errors_messages_in_sage_misc_cython)**",
    "created_at": "2018-02-13T14:37:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24724#issuecomment-378513",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/capture_warning_errors_messages_in_sage_misc_cython](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/capture_warning_errors_messages_in_sage_misc_cython)**



---

archive/issue_events_324260.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-13T14:37:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24724#event-324260"
}
```



---

archive/issue_comments_378514.json:
```json
{
    "body": "Commit: **[52982f3](https://github.com/sagemath/sagetrac-mirror/commit/52982f36465228f4c02951e6f090b3e061be8650)**",
    "created_at": "2018-02-13T14:37:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24724#issuecomment-378514",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[52982f3](https://github.com/sagemath/sagetrac-mirror/commit/52982f36465228f4c02951e6f090b3e061be8650)**



---

archive/issue_comments_378515.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f47ed48f4716e44fb18e46f52b4c5c67da9ad3a2\">f47ed48</a></td><td><code>Cleanup in sage.misc.cython</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/52982f36465228f4c02951e6f090b3e061be8650\">52982f3</a></td><td><code>Capture Cython warning/errors messages</code></td></tr></table>\n",
    "created_at": "2018-02-13T14:37:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24724#issuecomment-378515",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f47ed48f4716e44fb18e46f52b4c5c67da9ad3a2">f47ed48</a></td><td><code>Cleanup in sage.misc.cython</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/52982f36465228f4c02951e6f090b3e061be8650">52982f3</a></td><td><code>Capture Cython warning/errors messages</code></td></tr></table>




---

archive/issue_events_324261.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2018-02-18T21:32:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24724#event-324261"
}
```



---

archive/issue_events_324262.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2018-02-18T21:32:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24724#event-324262"
}
```



---

archive/issue_comments_378516.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAll the patchbot failures seem unrelated to this ticket so let's move it forward.",
    "created_at": "2018-02-18T21:32:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24724#issuecomment-378516",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:3" align="right">Comment 3</div>

All the patchbot failures seem unrelated to this ticket so let's move it forward.



---

archive/issue_comments_378517.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2018-02-18T21:32:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24724#issuecomment-378517",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_events_324263.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-20T23:08:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24724#event-324263"
}
```



---

archive/issue_events_324264.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ba0d124149c6df9d5b7b3686387926556e00b95a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-20T23:08:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24724#event-324264"
}
```



---

archive/issue_comments_378518.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/capture_warning_errors_messages_in_sage_misc_cython](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/capture_warning_errors_messages_in_sage_misc_cython)** to **[52982f3](https://github.com/sagemath/sagetrac-mirror/commit/52982f36465228f4c02951e6f090b3e061be8650)**",
    "created_at": "2018-02-20T23:08:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24724#issuecomment-378518",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/capture_warning_errors_messages_in_sage_misc_cython](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/capture_warning_errors_messages_in_sage_misc_cython)** to **[52982f3](https://github.com/sagemath/sagetrac-mirror/commit/52982f36465228f4c02951e6f090b3e061be8650)**
