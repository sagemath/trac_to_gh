# Issue 24369: upgrade arb to 2.12.0

archive/issues_024132.json:
```json
{
    "assignees": [],
    "body": "arb fails to pass self-tests on x86_64 skylake. This is fixed in arb 2.12.0.\n\n(The new version fails to pass self-tests on my 32bit VM, while it did not before, see #24661.)\n\ntarball here: https://github.com/fredrik-johansson/arb/archive/2.12.0.tar.gz\n\nCC:  @fredrik-johansson @kiwifb\n\nKeywords: **sdl**\n\nBranch: **[c793138](https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377)**\n\nUpstream: **Fixed upstream, in a later stable release.**\n\nReviewer: **Thierry Monteil, Marc Mezzarobba, Fredrik Johansson**\n\nAuthor: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24369_\n\n",
    "closed_at": "2018-02-09T23:48:07Z",
    "created_at": "2017-12-11T18:45:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "upgrade arb to 2.12.0",
    "type": "issue",
    "updated_at": "2019-08-27T19:45:19Z",
    "url": "https://github.com/sagemath/sage/issues/24369",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
arb fails to pass self-tests on x86_64 skylake. This is fixed in arb 2.12.0.

(The new version fails to pass self-tests on my 32bit VM, while it did not before, see #24661.)

tarball here: https://github.com/fredrik-johansson/arb/archive/2.12.0.tar.gz

CC:  @fredrik-johansson @kiwifb

Keywords: **sdl**

Branch: **[c793138](https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377)**

Upstream: **Fixed upstream, in a later stable release.**

Reviewer: **Thierry Monteil, Marc Mezzarobba, Fredrik Johansson**

Author: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/24369_





---

archive/issue_events_319832.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2017-12-11T18:45:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319832"
}
```



---

archive/issue_events_319833.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2017-12-11T18:45:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319833"
}
```



---

archive/issue_events_319834.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2017-12-11T18:45:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319834"
}
```



---

archive/issue_events_319835.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2017-12-11T18:45:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319835"
}
```



---

archive/issue_comments_371623.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[arb-2.11.1.p0.log](https://github.com/sagemath/sage/files/ticket24369/arb-2.11.1.p0.log)**",
    "created_at": "2017-12-11T18:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371623",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[arb-2.11.1.p0.log](https://github.com/sagemath/sage/files/ticket24369/arb-2.11.1.p0.log)**



---

archive/issue_comments_371624.json:
```json
{
    "body": "Upstream: **Reported upstream. No feedback yet.**",
    "created_at": "2017-12-11T18:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371624",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Upstream: **Reported upstream. No feedback yet.**



---

archive/issue_comments_371625.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nDoes upgrading to 2.12 solve the issue?",
    "created_at": "2017-12-11T20:08:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371625",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:2" align="right">Comment 2</div>

Does upgrading to 2.12 solve the issue?



---

archive/issue_comments_371626.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBy the way, the same error on x86_64 (a skylake CPU, Linux)\n\n```\n[arb-2.11.1.p0] gauss_period_minpoly....make[3]: *** [../Makefile.subdirs:84: \n../build/arb_fmpz_poly/test/t-gauss_period_minpoly_RUN] Floating point exception\n```\n\nI'll investigate the upgrade option...",
    "created_at": "2017-12-11T23:05:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371626",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

By the way, the same error on x86_64 (a skylake CPU, Linux)

```
[arb-2.11.1.p0] gauss_period_minpoly....make[3]: *** [../Makefile.subdirs:84: 
../build/arb_fmpz_poly/test/t-gauss_period_minpoly_RUN] Floating point exception
```

I'll investigate the upgrade option...



---

archive/issue_comments_371627.json:
```json
{
    "body": "Author: **Dima Pasechnik**",
    "created_at": "2017-12-12T00:02:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371627",
    "user": "https://github.com/dimpase"
}
```

Author: **Dima Pasechnik**



---

archive/issue_comments_371628.json:
```json
{
    "body": "Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**",
    "created_at": "2017-12-12T00:02:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371628",
    "user": "https://github.com/dimpase"
}
```

Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**



---

archive/issue_comments_371629.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,5 @@\n arb fails to pass self-tests on my 32bit VM, while it did not before. Log attached.\n \n+Fixed in arb 2.12.0.\n+\n+tarball here: https://github.com/fredrik-johansson/arb/archive/2.12.0.tar.gz\n``````\n",
    "created_at": "2017-12-12T00:02:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371629",
    "user": "https://github.com/dimpase"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,5 @@
 arb fails to pass self-tests on my 32bit VM, while it did not before. Log attached.
 
+Fixed in arb 2.12.0.
+
+tarball here: https://github.com/fredrik-johansson/arb/archive/2.12.0.tar.gz
``````




---

archive/issue_comments_371630.json:
```json
{
    "body": "Commit: **[c793138](https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377)**",
    "created_at": "2017-12-12T00:02:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371630",
    "user": "https://github.com/dimpase"
}
```

Commit: **[c793138](https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377)**



---

archive/issue_events_319836.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-12-12T00:02:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319836"
}
```



---

archive/issue_comments_371631.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nmildly tested on x86_64, fixed numerical noise (which almost uniformly went down :-)).\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377\">c793138</a></td><td><code>update arb to 2.12.0, doctest fixes, and #24369</code></td></tr></table>\n",
    "created_at": "2017-12-12T00:02:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371631",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:4" align="right">Comment 4</div>

mildly tested on x86_64, fixed numerical noise (which almost uniformly went down :-)).

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377">c793138</a></td><td><code>update arb to 2.12.0, doctest fixes, and #24369</code></td></tr></table>




---

archive/issue_comments_371632.json:
```json
{
    "body": "Branch: **[u/dimpase/arb212](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/arb212)**",
    "created_at": "2017-12-12T00:02:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371632",
    "user": "https://github.com/dimpase"
}
```

Branch: **[u/dimpase/arb212](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/arb212)**



---

archive/issue_comments_371633.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nNot a good time for me. I won't be able to give you a review before the 3rd or 4rth of January.",
    "created_at": "2017-12-29T09:04:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371633",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:6" align="right">Comment 6</div>

Not a good time for me. I won't be able to give you a review before the 3rd or 4rth of January.



---

archive/issue_events_319837.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2017-12-30T01:19:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319837"
}
```



---

archive/issue_events_319838.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2017-12-30T01:19:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319838"
}
```



---

archive/issue_comments_371634.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nWith 2.12.0, i still have one test failure on 32bit system:\n\n```\nl_vec_hurwitz....\n L value differ\nL(1/2, 1) single = (0 + 0j)  +/-  (0, 0j)\nL(1/2, 1) multi = (-0.30909754859579756426 + 0j)  +/-  (2.54e-23, 0j)\n\n(-0.3090975486 + 0j)  +/-  (2.54e-23, 0j)\n(0.7888523833 - 0.1836680212j)  +/-  (2.02e-23, 1.54e-23j)\n(0.4432168971 - 0.236477911j)  +/-  (1.99e-23, 1.32e-23j)\n(1.648021942 + 0.03777565741j)  +/-  (1.55e-23, 8.22e-24j)\n(0.6185977148 + 0.146695673j)  +/-  (2.08e-23, 1.32e-23j)\n(0.5039871966 - 0.8215955013j)  +/-  (2.02e-23, 1.71e-23j)\n(0.750432146 + 0j)  +/-  (2.54e-23, 0j)\n(0.5039871966 + 0.8215955013j)  +/-  (2.02e-23, 1.71e-23j)\n(0.6185977148 - 0.146695673j)  +/-  (2.08e-23, 1.32e-23j)\n(1.648021942 - 0.03777565741j)  +/-  (1.55e-23, 8.22e-24j)\n(0.4432168971 + 0.236477911j)  +/-  (1.99e-23, 1.32e-23j)\n(0.7888523833 + 0.1836680212j)  +/-  (2.02e-23, 1.54e-23j)\n\n\n../Makefile.subdirs:84: recipe for target '../build/acb_dirichlet/test/t-l_vec_hurwitz_RUN' failed\n```\n\nSee also the attached log.",
    "created_at": "2017-12-30T01:19:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371634",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:7" align="right">Comment 7</div>

With 2.12.0, i still have one test failure on 32bit system:

```
l_vec_hurwitz....
 L value differ
L(1/2, 1) single = (0 + 0j)  +/-  (0, 0j)
L(1/2, 1) multi = (-0.30909754859579756426 + 0j)  +/-  (2.54e-23, 0j)

(-0.3090975486 + 0j)  +/-  (2.54e-23, 0j)
(0.7888523833 - 0.1836680212j)  +/-  (2.02e-23, 1.54e-23j)
(0.4432168971 - 0.236477911j)  +/-  (1.99e-23, 1.32e-23j)
(1.648021942 + 0.03777565741j)  +/-  (1.55e-23, 8.22e-24j)
(0.6185977148 + 0.146695673j)  +/-  (2.08e-23, 1.32e-23j)
(0.5039871966 - 0.8215955013j)  +/-  (2.02e-23, 1.71e-23j)
(0.750432146 + 0j)  +/-  (2.54e-23, 0j)
(0.5039871966 + 0.8215955013j)  +/-  (2.02e-23, 1.71e-23j)
(0.6185977148 - 0.146695673j)  +/-  (2.08e-23, 1.32e-23j)
(1.648021942 - 0.03777565741j)  +/-  (1.55e-23, 8.22e-24j)
(0.4432168971 + 0.236477911j)  +/-  (1.99e-23, 1.32e-23j)
(0.7888523833 + 0.1836680212j)  +/-  (2.02e-23, 1.54e-23j)


../Makefile.subdirs:84: recipe for target '../build/acb_dirichlet/test/t-l_vec_hurwitz_RUN' failed
```

See also the attached log.



---

archive/issue_comments_371635.json:
```json
{
    "body": "Reviewer: **Thierry Monteil**",
    "created_at": "2017-12-30T01:19:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371635",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Reviewer: **Thierry Monteil**



---

archive/issue_comments_371636.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nAttachment: **[arb-2.12.0.p0.log](https://github.com/sagemath/sage/files/ticket24369/arb-2.12.0.p0.log)**\n\nwhile I can confirm that this is reproducible on a \"real\" x86 (arando buildbot)\n\n```\n[arb-2.12.0.p0] l_vec_hurwitz....\n[arb-2.12.0.p0]  L value differ\n[arb-2.12.0.p0] L(1/2, 1) single = (0 + 0j)  +/-  (0, 0j)\n[arb-2.12.0.p0] L(1/2, 1) multi = (-0.30909754859579756426 + 0j)  +/-  (2.54e-23, 0j)\n[arb-2.12.0.p0]\n[arb-2.12.0.p0] (-0.3090975486 + 0j)  +/-  (2.54e-23, 0j)\n[arb-2.12.0.p0] (0.7888523833 - 0.1836680212j)  +/-  (2.02e-23, 1.54e-23j)\n[arb-2.12.0.p0] (0.4432168971 - 0.236477911j)  +/-  (1.99e-23, 1.32e-23j)\n[arb-2.12.0.p0] (1.648021942 + 0.03777565741j)  +/-  (1.55e-23, 8.22e-24j)\n[arb-2.12.0.p0] (0.6185977148 + 0.146695673j)  +/-  (2.08e-23, 1.32e-23j)\n[arb-2.12.0.p0] (0.5039871966 - 0.8215955013j)  +/-  (2.02e-23, 1.71e-23j)\n[arb-2.12.0.p0] (0.750432146 + 0j)  +/-  (2.54e-23, 0j)\n[arb-2.12.0.p0] (0.5039871966 + 0.8215955013j)  +/-  (2.02e-23, 1.71e-23j)\n[arb-2.12.0.p0] (0.6185977148 - 0.146695673j)  +/-  (2.08e-23, 1.32e-23j)\n[arb-2.12.0.p0] (1.648021942 - 0.03777565741j)  +/-  (1.55e-23, 8.22e-24j)\n[arb-2.12.0.p0] (0.4432168971 + 0.236477911j)  +/-  (1.99e-23, 1.32e-23j)\n[arb-2.12.0.p0] (0.7888523833 + 0.1836680212j)  +/-  (2.02e-23, 1.54e-23j)\n[arb-2.12.0.p0]\n[arb-2.12.0.p0]\n[arb-2.12.0.p0] make[3]: *** [../build/acb_dirichlet/test/t-l_vec_hurwitz_RUN] Aborted (core dumped)\n```\nI still insist that we should upgrade, as the previous version also does not pass all the self-tests, albeit on a different (and much more popular) platform, x86_64 (if it is skylake- it seems to work OK on nechalem).\n\nSee the attached log.",
    "created_at": "2018-01-20T12:13:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371636",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:8" align="right">Comment 8</div>

Attachment: **[arb-2.12.0.p0.log](https://github.com/sagemath/sage/files/ticket24369/arb-2.12.0.p0.log)**

while I can confirm that this is reproducible on a "real" x86 (arando buildbot)

```
[arb-2.12.0.p0] l_vec_hurwitz....
[arb-2.12.0.p0]  L value differ
[arb-2.12.0.p0] L(1/2, 1) single = (0 + 0j)  +/-  (0, 0j)
[arb-2.12.0.p0] L(1/2, 1) multi = (-0.30909754859579756426 + 0j)  +/-  (2.54e-23, 0j)
[arb-2.12.0.p0]
[arb-2.12.0.p0] (-0.3090975486 + 0j)  +/-  (2.54e-23, 0j)
[arb-2.12.0.p0] (0.7888523833 - 0.1836680212j)  +/-  (2.02e-23, 1.54e-23j)
[arb-2.12.0.p0] (0.4432168971 - 0.236477911j)  +/-  (1.99e-23, 1.32e-23j)
[arb-2.12.0.p0] (1.648021942 + 0.03777565741j)  +/-  (1.55e-23, 8.22e-24j)
[arb-2.12.0.p0] (0.6185977148 + 0.146695673j)  +/-  (2.08e-23, 1.32e-23j)
[arb-2.12.0.p0] (0.5039871966 - 0.8215955013j)  +/-  (2.02e-23, 1.71e-23j)
[arb-2.12.0.p0] (0.750432146 + 0j)  +/-  (2.54e-23, 0j)
[arb-2.12.0.p0] (0.5039871966 + 0.8215955013j)  +/-  (2.02e-23, 1.71e-23j)
[arb-2.12.0.p0] (0.6185977148 - 0.146695673j)  +/-  (2.08e-23, 1.32e-23j)
[arb-2.12.0.p0] (1.648021942 - 0.03777565741j)  +/-  (1.55e-23, 8.22e-24j)
[arb-2.12.0.p0] (0.4432168971 + 0.236477911j)  +/-  (1.99e-23, 1.32e-23j)
[arb-2.12.0.p0] (0.7888523833 + 0.1836680212j)  +/-  (2.02e-23, 1.54e-23j)
[arb-2.12.0.p0]
[arb-2.12.0.p0]
[arb-2.12.0.p0] make[3]: *** [../build/acb_dirichlet/test/t-l_vec_hurwitz_RUN] Aborted (core dumped)
```
I still insist that we should upgrade, as the previous version also does not pass all the self-tests, albeit on a different (and much more popular) platform, x86_64 (if it is skylake- it seems to work OK on nechalem).

See the attached log.



---

archive/issue_comments_371637.json:
```json
{
    "body": "Attachment: **[arb-2.11.1.p0.2.log](https://github.com/sagemath/sage/files/ticket24369/arb-2.11.1.p0.2.log)**\n\nfailure on skylake",
    "created_at": "2018-01-20T12:17:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371637",
    "user": "https://github.com/dimpase"
}
```

Attachment: **[arb-2.11.1.p0.2.log](https://github.com/sagemath/sage/files/ticket24369/arb-2.11.1.p0.2.log)**

failure on skylake



---

archive/issue_comments_371638.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n-arb fails to pass self-tests on my 32bit VM, while it did not before. Log attached.\n+arb fails to pass self-tests on my 32bit VM, while it did not before. Log attached. As well, it fails to pass self-tests on x86_64 skylake.\n \n-Fixed in arb 2.12.0.\n+The latter is fixed in arb 2.12.0.\n \n tarball here: https://github.com/fredrik-johansson/arb/archive/2.12.0.tar.gz\n``````\n",
    "created_at": "2018-01-20T12:19:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371638",
    "user": "https://github.com/dimpase"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
-arb fails to pass self-tests on my 32bit VM, while it did not before. Log attached.
+arb fails to pass self-tests on my 32bit VM, while it did not before. Log attached. As well, it fails to pass self-tests on x86_64 skylake.
 
-Fixed in arb 2.12.0.
+The latter is fixed in arb 2.12.0.
 
 tarball here: https://github.com/fredrik-johansson/arb/archive/2.12.0.tar.gz
``````




---

archive/issue_events_319839.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-01-20T12:19:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319839"
}
```



---

archive/issue_events_319840.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-01-20T12:19:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319840"
}
```



---

archive/issue_comments_371639.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nJust to make sure, your commit remove the whole of `src/sage/rings/complex_arb.pyx` but not the corresponding `pxd` file? are you sure?",
    "created_at": "2018-01-23T02:01:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371639",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:10" align="right">Comment 10</div>

Just to make sure, your commit remove the whole of `src/sage/rings/complex_arb.pyx` but not the corresponding `pxd` file? are you sure?



---

archive/issue_comments_371640.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nReplying to [@kiwifb](#comment%3A10):\n> Just to make sure, your commit remove the whole of `src/sage/rings/complex_arb.pyx` but not the corresponding `pxd` file? are you sure?\n\nthis is a trac git viewer bug. If you look at the commit in comment 4, it would make sense.",
    "created_at": "2018-01-23T08:25:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371640",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:11" align="right">Comment 11</div>

Replying to [@kiwifb](#comment%3A10):
> Just to make sure, your commit remove the whole of `src/sage/rings/complex_arb.pyx` but not the corresponding `pxd` file? are you sure?

this is a trac git viewer bug. If you look at the commit in comment 4, it would make sense.



---

archive/issue_comments_371641.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nReplying to [@dimpase](#comment%3A11):\n> Replying to [@kiwifb](#comment%3A10):\n> > Just to make sure, your commit remove the whole of `src/sage/rings/complex_arb.pyx` but not the corresponding `pxd` file? are you sure?\n\n> \n> this is a trac git viewer bug. If you look at the commit in comment 4, it would make sense.\n\nNope, commit in comment 4 looks just the same.",
    "created_at": "2018-01-23T08:27:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371641",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:12" align="right">Comment 12</div>

Replying to [@dimpase](#comment%3A11):
> Replying to [@kiwifb](#comment%3A10):
> > Just to make sure, your commit remove the whole of `src/sage/rings/complex_arb.pyx` but not the corresponding `pxd` file? are you sure?

> 
> this is a trac git viewer bug. If you look at the commit in comment 4, it would make sense.

Nope, commit in comment 4 looks just the same.



---

archive/issue_comments_371642.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nHuh?\nhttps://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377\n\n(it could be it is different for you). To be sure, use git, \nor look here:\nhttps://github.com/sagemath/sagetrac-mirror/compare/u/dimpase/arb212",
    "created_at": "2018-01-23T09:11:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371642",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:13" align="right">Comment 13</div>

Huh?
https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377

(it could be it is different for you). To be sure, use git, 
or look here:
https://github.com/sagemath/sagetrac-mirror/compare/u/dimpase/arb212



---

archive/issue_comments_371643.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nThis is how it looks to me:\n\n```\n\nauthor\tDima Pasechnik <dimpase@gmail.com>\t2017-12-11 23:58:06 +0000\ncommitter\tDima Pasechnik <dimpase@gmail.com>\t2017-12-11 23:58:06 +0000\ncommit\tc79313811851584c5b6732ea12da8b40d04b0377 (patch)\ntree\t94c8774bc96b6b9e5386f9e5502ee60db3de0eb2\nparent\tUpdated SageMath version to 8.1 (diff)\nupdate arb to 2.12.0, doctest fixes, and #24369u/dimpase/arb212\nDiffstat\n-rw-r--r--\tbuild/pkgs/arb/checksums.ini\t6\t\n-rw-r--r--\tbuild/pkgs/arb/package-version.txt\t2\t\n-rw-r--r--\tsrc/sage/rings/complex_arb.pyx\t14\t\n-rw-r--r--\tsrc/sage/rings/real_arb.pyx\t4\t\n4 files changed, 13 insertions, 13 deletions\ndiff --git a/build/pkgs/arb/checksums.ini b/build/pkgs/arb/checksums.ini\nindex 78909d5..1924ee0 100644\n--- a/build/pkgs/arb/checksums.ini\n+++ b/build/pkgs/arb/checksums.ini\n@@ -1,4 +1,4 @@\n tarball=arb-VERSION.tar.gz\n-sha1=2f06bfb433cdaecde0e824c5e638094fd666a0d1\n-md5=d63cdd1147104790826c93bc8651104f\n-cksum=2745482665\n+sha1=27476d0529e48a07d92da90bd0fb80dd18f443e3\n+md5=733285d9705d10b8024e551ffa81952f\n+cksum=2391183744\ndiff --git a/build/pkgs/arb/package-version.txt b/build/pkgs/arb/package-version.txt\nindex 99993ff..c8810e9 100644\n--- a/build/pkgs/arb/package-version.txt\n+++ b/build/pkgs/arb/package-version.txt\n@@ -1 +1 @@\n-2.11.1.p0\n+2.12.0.p0\ndiff --git a/src/sage/rings/complex_arb.pyx b/src/sage/rings/complex_arb.pyx\nindex ee048fb..71fad24 100644\n--- a/src/sage/rings/complex_arb.pyx\n+++ b/src/sage/rings/complex_arb.pyx\n@@ -2603,7 +2603,7 @@ cdef class ComplexBall(RingElement):\n         EXAMPLES::\n \n             sage: CBF(0, -1).agm1()\n-            [0.5990701173678 +/- 1.15e-14] + [-0.5990701173678 +/- 1.19e-14]*I\n+            [0.599070117367796 +/- 3.9...e-16] + [-0.599070117367796 +/- 5.5...e-16]*I\n         \"\"\"\n         cdef ComplexBall res = self._new()\n         if _do_sig(prec(self)): sig_on()\n@@ -3318,9 +3318,9 @@ cdef class ComplexBall(RingElement):\n              [0.002473055794309 +/- 5.01e-16] + [0.003859554040267 +/- 4.45e-16]*I,\n              [-0.01299087561709 +/- 4.72e-15] + [0.00725027521915 +/- 4.32e-15]*I]\n             sage: (z + 3 + 4*tau).elliptic_p(tau, 3)\n-            [[-3.2892099677271 +/- 2.29e-14] + [-0.00036737673029 +/- 8.58e-15]*I,\n-             [0.002473055794 +/- 6.59e-13] + [0.003859554040 +/- 6.17e-13]*I,\n-             [-0.0129908756 +/- 3.39e-11] + [0.0072502752 +/- 3.60e-11]*I]\n+            [[-3.28920996772709 +/- 8.4...e-15] + [-0.00036737673029 +/- 4.1...e-15]*I,\n+             [0.0024730557943 +/- 6.6...e-14] + [0.0038595540403 +/- 8.8...e-14]*I,\n+             [-0.01299087562 +/- 5.6...e-12] + [0.00725027522 +/- 3.5...e-12]*I]\n \n         \"\"\"\n         cdef ComplexBall my_tau = self._parent.coerce(tau)\n@@ -3356,7 +3356,7 @@ cdef class ComplexBall(RingElement):\n         EXAMPLES::\n \n             sage: CBF(2,3).elliptic_k()\n-            [1.0429132919285 +/- 3.65e-14] + [0.6296824723086 +/- 6.15e-14]*I\n+            [1.04291329192852 +/- 5.9...e-15] + [0.62968247230864 +/- 3.4...e-15]*I\n \n         \"\"\"\n         cdef ComplexBall result = self._new()\n@@ -3373,7 +3373,7 @@ cdef class ComplexBall(RingElement):\n         EXAMPLES::\n \n             sage: CBF(2,3).elliptic_e()\n-            [1.472797144959 +/- 4.82e-13] + [-1.231604783936 +/- 1.25e-13]*I\n+            [1.472797144959 +/- 4.5...e-13] + [-1.231604783936 +/- 9.5...e-14]*I\n \n         \"\"\"\n         cdef ComplexBall result = self._new()\n@@ -3520,7 +3520,7 @@ cdef class ComplexBall(RingElement):\n         EXAMPLES::\n \n             sage: CBF(1/2).legendre_P(5)\n-            0.08984375000000000\n+            [0.08984375000000000 +/- 4.5...e-18]\n             sage: CBF(1,2).legendre_P(CBF(2,3), CBF(0,1))\n             [0.10996180744364 +/- 7.45e-15] + [0.14312767804055 +/- 8.38e-15]*I\n             sage: CBF(-10).legendre_P(5, 325/100)\ndiff --git a/src/sage/rings/real_arb.pyx b/src/sage/rings/real_arb.pyx\nindex aa12ae0..9aad425 100644\n--- a/src/sage/rings/real_arb.pyx\n+++ b/src/sage/rings/real_arb.pyx\n@@ -3451,7 +3451,7 @@ cdef class RealBall(RingElement):\n             sage: RBF(1/2).polylog(1)\n             [0.6931471805599 +/- 5.02e-14]\n             sage: RBF(1/3).polylog(1/2)\n-            [0.44210883528067 +/- 6.75e-15]\n+            [0.44210883528067 +/- 6.7...e-15]\n             sage: RBF(1/3).polylog(RLF(pi))\n             [0.34728895057225 +/- 5.51e-15]\n \n@@ -3551,7 +3551,7 @@ cdef class RealBall(RingElement):\n             sage: RBF(1).agm(1)\n             1.000000000000000\n             sage: RBF(sqrt(2)).agm(1)^(-1)\n-            [0.83462684167407 +/- 4.31e-15]\n+            [0.83462684167407 +/- 3.9...e-15]\n         \"\"\"\n         cdef RealBall other_as_ball\n         cdef RealBall res = self._new()\n```",
    "created_at": "2018-01-23T09:12:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371643",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:14" align="right">Comment 14</div>

This is how it looks to me:

```

author	Dima Pasechnik <dimpase@gmail.com>	2017-12-11 23:58:06 +0000
committer	Dima Pasechnik <dimpase@gmail.com>	2017-12-11 23:58:06 +0000
commit	c79313811851584c5b6732ea12da8b40d04b0377 (patch)
tree	94c8774bc96b6b9e5386f9e5502ee60db3de0eb2
parent	Updated SageMath version to 8.1 (diff)
update arb to 2.12.0, doctest fixes, and #24369u/dimpase/arb212
Diffstat
-rw-r--r--	build/pkgs/arb/checksums.ini	6	
-rw-r--r--	build/pkgs/arb/package-version.txt	2	
-rw-r--r--	src/sage/rings/complex_arb.pyx	14	
-rw-r--r--	src/sage/rings/real_arb.pyx	4	
4 files changed, 13 insertions, 13 deletions
diff --git a/build/pkgs/arb/checksums.ini b/build/pkgs/arb/checksums.ini
index 78909d5..1924ee0 100644
--- a/build/pkgs/arb/checksums.ini
+++ b/build/pkgs/arb/checksums.ini
@@ -1,4 +1,4 @@
 tarball=arb-VERSION.tar.gz
-sha1=2f06bfb433cdaecde0e824c5e638094fd666a0d1
-md5=d63cdd1147104790826c93bc8651104f
-cksum=2745482665
+sha1=27476d0529e48a07d92da90bd0fb80dd18f443e3
+md5=733285d9705d10b8024e551ffa81952f
+cksum=2391183744
diff --git a/build/pkgs/arb/package-version.txt b/build/pkgs/arb/package-version.txt
index 99993ff..c8810e9 100644
--- a/build/pkgs/arb/package-version.txt
+++ b/build/pkgs/arb/package-version.txt
@@ -1 +1 @@
-2.11.1.p0
+2.12.0.p0
diff --git a/src/sage/rings/complex_arb.pyx b/src/sage/rings/complex_arb.pyx
index ee048fb..71fad24 100644
--- a/src/sage/rings/complex_arb.pyx
+++ b/src/sage/rings/complex_arb.pyx
@@ -2603,7 +2603,7 @@ cdef class ComplexBall(RingElement):
         EXAMPLES::
 
             sage: CBF(0, -1).agm1()
-            [0.5990701173678 +/- 1.15e-14] + [-0.5990701173678 +/- 1.19e-14]*I
+            [0.599070117367796 +/- 3.9...e-16] + [-0.599070117367796 +/- 5.5...e-16]*I
         """
         cdef ComplexBall res = self._new()
         if _do_sig(prec(self)): sig_on()
@@ -3318,9 +3318,9 @@ cdef class ComplexBall(RingElement):
              [0.002473055794309 +/- 5.01e-16] + [0.003859554040267 +/- 4.45e-16]*I,
              [-0.01299087561709 +/- 4.72e-15] + [0.00725027521915 +/- 4.32e-15]*I]
             sage: (z + 3 + 4*tau).elliptic_p(tau, 3)
-            [[-3.2892099677271 +/- 2.29e-14] + [-0.00036737673029 +/- 8.58e-15]*I,
-             [0.002473055794 +/- 6.59e-13] + [0.003859554040 +/- 6.17e-13]*I,
-             [-0.0129908756 +/- 3.39e-11] + [0.0072502752 +/- 3.60e-11]*I]
+            [[-3.28920996772709 +/- 8.4...e-15] + [-0.00036737673029 +/- 4.1...e-15]*I,
+             [0.0024730557943 +/- 6.6...e-14] + [0.0038595540403 +/- 8.8...e-14]*I,
+             [-0.01299087562 +/- 5.6...e-12] + [0.00725027522 +/- 3.5...e-12]*I]
 
         """
         cdef ComplexBall my_tau = self._parent.coerce(tau)
@@ -3356,7 +3356,7 @@ cdef class ComplexBall(RingElement):
         EXAMPLES::
 
             sage: CBF(2,3).elliptic_k()
-            [1.0429132919285 +/- 3.65e-14] + [0.6296824723086 +/- 6.15e-14]*I
+            [1.04291329192852 +/- 5.9...e-15] + [0.62968247230864 +/- 3.4...e-15]*I
 
         """
         cdef ComplexBall result = self._new()
@@ -3373,7 +3373,7 @@ cdef class ComplexBall(RingElement):
         EXAMPLES::
 
             sage: CBF(2,3).elliptic_e()
-            [1.472797144959 +/- 4.82e-13] + [-1.231604783936 +/- 1.25e-13]*I
+            [1.472797144959 +/- 4.5...e-13] + [-1.231604783936 +/- 9.5...e-14]*I
 
         """
         cdef ComplexBall result = self._new()
@@ -3520,7 +3520,7 @@ cdef class ComplexBall(RingElement):
         EXAMPLES::
 
             sage: CBF(1/2).legendre_P(5)
-            0.08984375000000000
+            [0.08984375000000000 +/- 4.5...e-18]
             sage: CBF(1,2).legendre_P(CBF(2,3), CBF(0,1))
             [0.10996180744364 +/- 7.45e-15] + [0.14312767804055 +/- 8.38e-15]*I
             sage: CBF(-10).legendre_P(5, 325/100)
diff --git a/src/sage/rings/real_arb.pyx b/src/sage/rings/real_arb.pyx
index aa12ae0..9aad425 100644
--- a/src/sage/rings/real_arb.pyx
+++ b/src/sage/rings/real_arb.pyx
@@ -3451,7 +3451,7 @@ cdef class RealBall(RingElement):
             sage: RBF(1/2).polylog(1)
             [0.6931471805599 +/- 5.02e-14]
             sage: RBF(1/3).polylog(1/2)
-            [0.44210883528067 +/- 6.75e-15]
+            [0.44210883528067 +/- 6.7...e-15]
             sage: RBF(1/3).polylog(RLF(pi))
             [0.34728895057225 +/- 5.51e-15]
 
@@ -3551,7 +3551,7 @@ cdef class RealBall(RingElement):
             sage: RBF(1).agm(1)
             1.000000000000000
             sage: RBF(sqrt(2)).agm(1)^(-1)
-            [0.83462684167407 +/- 4.31e-15]
+            [0.83462684167407 +/- 3.9...e-15]
         """
         cdef RealBall other_as_ball
         cdef RealBall res = self._new()
```



---

archive/issue_comments_371644.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nOh yes that's right. I did something wrong there.",
    "created_at": "2018-01-23T09:13:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371644",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:15" align="right">Comment 15</div>

Oh yes that's right. I did something wrong there.



---

archive/issue_comments_371645.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nI was going to give this a positive review but re-reading things I am getting concerned that the new arb\u015b tests fail systematically on 32bits machine. Is it a correct assertion?",
    "created_at": "2018-02-01T20:55:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371645",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:16" align="right">Comment 16</div>

I was going to give this a positive review but re-reading things I am getting concerned that the new arbś tests fail systematically on 32bits machine. Is it a correct assertion?



---

archive/issue_comments_371646.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nReplying to [@kiwifb](#comment%3A16):\n> I was going to give this a positive review but re-reading things I am getting concerned that the new arb\u015b tests fail systematically on 32bits machine. Is it a correct assertion?\n\nYes. I am OK to either open another ticket for the upgrade, or another for the 32bit issue, but in any case, please do not just close that one without opening another ticket, since the issue is not solved (or only partially).",
    "created_at": "2018-02-01T21:14:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371646",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:17" align="right">Comment 17</div>

Replying to [@kiwifb](#comment%3A16):
> I was going to give this a positive review but re-reading things I am getting concerned that the new arbś tests fail systematically on 32bits machine. Is it a correct assertion?

Yes. I am OK to either open another ticket for the upgrade, or another for the 32bit issue, but in any case, please do not just close that one without opening another ticket, since the issue is not solved (or only partially).



---

archive/issue_comments_371647.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nThis upgrade is an improvement. Surely the 32-bit issue is not fully solved (the new bug is in new code as far as I can see), but this should go to a followup ticket.",
    "created_at": "2018-02-01T22:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371647",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:18" align="right">Comment 18</div>

This upgrade is an improvement. Surely the 32-bit issue is not fully solved (the new bug is in new code as far as I can see), but this should go to a followup ticket.



---

archive/issue_comments_371648.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nYes, the 32-bit bug is almost certainly in new code and should not be a concern for Sage.",
    "created_at": "2018-02-02T09:11:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371648",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:19" align="right">Comment 19</div>

Yes, the 32-bit bug is almost certainly in new code and should not be a concern for Sage.



---

archive/issue_comments_371649.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nThe follow-up ticket is here: #24661\n\nLet us finally be done with this one.",
    "created_at": "2018-02-05T17:49:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371649",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:20" align="right">Comment 20</div>

The follow-up ticket is here: #24661

Let us finally be done with this one.



---

archive/issue_comments_371650.json:
```json
{
    "body": "Changed reviewer from **Thierry Monteil** to **Thierry Monteil, Marc Mezzarobba, Fredrik Johansson**",
    "created_at": "2018-02-07T16:51:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371650",
    "user": "https://github.com/mezzarobba"
}
```

Changed reviewer from **Thierry Monteil** to **Thierry Monteil, Marc Mezzarobba, Fredrik Johansson**



---

archive/issue_comments_371651.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n-arb fails to pass self-tests on my 32bit VM, while it did not before. Log attached. As well, it fails to pass self-tests on x86_64 skylake.\n+arb fails to pass self-tests on x86_64 skylake. This is fixed in arb 2.12.0.\n \n-The latter is fixed in arb 2.12.0.\n+(The new version fails to pass self-tests on my 32bit VM, while it did not before, see #24661.)\n \n tarball here: https://github.com/fredrik-johansson/arb/archive/2.12.0.tar.gz\n``````\n",
    "created_at": "2018-02-07T16:51:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371651",
    "user": "https://github.com/mezzarobba"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
-arb fails to pass self-tests on my 32bit VM, while it did not before. Log attached. As well, it fails to pass self-tests on x86_64 skylake.
+arb fails to pass self-tests on x86_64 skylake. This is fixed in arb 2.12.0.
 
-The latter is fixed in arb 2.12.0.
+(The new version fails to pass self-tests on my 32bit VM, while it did not before, see #24661.)
 
 tarball here: https://github.com/fredrik-johansson/arb/archive/2.12.0.tar.gz
``````




---

archive/issue_events_319841.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-02-07T16:51:21Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "title_is": "upgrade arb to 2.12.0",
    "title_was": "arb fails to pass self-tests",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319841"
}
```



---

archive/issue_events_319842.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-02-07T16:51:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319842"
}
```



---

archive/issue_events_319843.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-02-07T16:51:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319843"
}
```



---

archive/issue_comments_371652.json:
```json
{
    "body": "Changed branch from **[u/dimpase/arb212](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/arb212)** to **[c793138](https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377)**",
    "created_at": "2018-02-09T23:48:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371652",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dimpase/arb212](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/arb212)** to **[c793138](https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377)**



---

archive/issue_events_319844.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-09T23:48:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319844"
}
```



---

archive/issue_events_319845.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "66f944cbd3eb3c42d2915cce9446809886d4a62e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-09T23:48:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24369#event-319845"
}
```



---

archive/issue_comments_371653.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">Comment 23</div>\n\nThis ticket has been closed, but as far as I can see, it's not been merged into the develop branch. Is that intentional?\n\nApologies if I'm missing something obvious.\n\n(Edit: I don't know why trac says I deleted the commit.)",
    "created_at": "2018-02-15T23:25:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371653",
    "user": "https://github.com/Konrad127123"
}
```

<div id="comment:23" align="right">Comment 23</div>

This ticket has been closed, but as far as I can see, it's not been merged into the develop branch. Is that intentional?

Apologies if I'm missing something obvious.

(Edit: I don't know why trac says I deleted the commit.)



---

archive/issue_comments_371654.json:
```json
{
    "body": "Changed commit from **[c793138](https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377)** to none",
    "created_at": "2018-02-15T23:25:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371654",
    "user": "https://github.com/Konrad127123"
}
```

Changed commit from **[c793138](https://github.com/sagemath/sagetrac-mirror/commit/c79313811851584c5b6732ea12da8b40d04b0377)** to none



---

archive/issue_comments_371655.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">Comment 24</div>\n\nReplying to [@Konrad127123](#comment%3A23):\n> This ticket has been closed, but as far as I can see, it's not been merged into the develop branch. Is that intentional?\n> \n> Apologies if I'm missing something obvious.\n\nI think it will be in the next beta (it has been merged in a sort of trunk, and this happened later than the latest ticket in the current beta(5))\n\n> \n> (Edit: I don't know why trac says I deleted the commit.)\n\nthis is just a quirk of the interface.\nIn fact the branch u/dimpase/arb212 is still there if you need it, e.g.\nfrom the mirror:\n\nhttps://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/arb212",
    "created_at": "2018-02-15T23:46:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371655",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:24" align="right">Comment 24</div>

Replying to [@Konrad127123](#comment%3A23):
> This ticket has been closed, but as far as I can see, it's not been merged into the develop branch. Is that intentional?
> 
> Apologies if I'm missing something obvious.

I think it will be in the next beta (it has been merged in a sort of trunk, and this happened later than the latest ticket in the current beta(5))

> 
> (Edit: I don't know why trac says I deleted the commit.)

this is just a quirk of the interface.
In fact the branch u/dimpase/arb212 is still there if you need it, e.g.
from the mirror:

https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/arb212



---

archive/issue_comments_371656.json:
```json
{
    "body": "Changed keywords from none to **sdl**",
    "created_at": "2019-08-27T19:45:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24369#issuecomment-371656",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Changed keywords from none to **sdl**
