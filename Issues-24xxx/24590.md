# Issue 24590: bug in inverse_image of module homomorphism

archive/issues_024353.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: A = FreeQuadraticModule(ZZ,1,matrix([2]))\nsage: f = A.Hom(A).an_element()\nsage: f.inverse_image(A)\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n....\nTypeError: ambient vector spaces must be equal\n\n```\n\nThe source of the bug is that `free_modules` are different from `free_quadratic_modules`.\nThus matrix methods like `row_span` return `free_modules` but not `free_quadratic_modules`.\n\nBranch/Commit: **[d7fa6ec](https://github.com/sagemath/sagetrac-mirror/commit/d7fa6ec8008f47c9420131756c168aaeeac0f1ba)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Simon Brandhorst**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24590_\n\n",
    "closed_at": "2018-02-09T08:03:56Z",
    "created_at": "2018-01-24T13:25:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bug in inverse_image of module homomorphism",
    "type": "issue",
    "updated_at": "2018-02-09T08:03:56Z",
    "url": "https://github.com/sagemath/sage/issues/24590",
    "user": "https://github.com/simonbrandhorst"
}
```

```
sage: A = FreeQuadraticModule(ZZ,1,matrix([2]))
sage: f = A.Hom(A).an_element()
sage: f.inverse_image(A)
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
....
TypeError: ambient vector spaces must be equal

```

The source of the bug is that `free_modules` are different from `free_quadratic_modules`.
Thus matrix methods like `row_span` return `free_modules` but not `free_quadratic_modules`.

Branch/Commit: **[d7fa6ec](https://github.com/sagemath/sagetrac-mirror/commit/d7fa6ec8008f47c9420131756c168aaeeac0f1ba)**

Reviewer: **Travis Scrimshaw**

Author: **Simon Brandhorst**

_Issue created by migration from https://trac.sagemath.org/ticket/24590_





---

archive/issue_events_322520.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-01-24T13:25:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24590#event-322520"
}
```



---

archive/issue_events_322521.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-01-24T13:25:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24590#event-322521"
}
```



---

archive/issue_events_322522.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-01-24T13:25:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24590#event-322522"
}
```



---

archive/issue_events_322523.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-01-24T13:25:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24590#event-322523"
}
```



---

archive/issue_comments_375409.json:
```json
{
    "body": "Branch: **[u/sbrandhorst/bug_in_inverse_image_of_module_homomorphism](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/bug_in_inverse_image_of_module_homomorphism)**",
    "created_at": "2018-01-24T13:44:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24590#issuecomment-375409",
    "user": "https://github.com/simonbrandhorst"
}
```

Branch: **[u/sbrandhorst/bug_in_inverse_image_of_module_homomorphism](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/bug_in_inverse_image_of_module_homomorphism)**



---

archive/issue_comments_375410.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,4 +11,4 @@\n ```\n \n The source of the bug is that `free_modules` are different from `free_quadratic_modules`.\n-Thus matrix methods like `row_span` return `free_modules` but not free_quadratic_modules`.\n+Thus matrix methods like `row_span` return `free_modules` but not `free_quadratic_modules`.\n``````\n",
    "created_at": "2018-01-24T13:46:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24590#issuecomment-375410",
    "user": "https://github.com/simonbrandhorst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,4 +11,4 @@
 ```
 
 The source of the bug is that `free_modules` are different from `free_quadratic_modules`.
-Thus matrix methods like `row_span` return `free_modules` but not free_quadratic_modules`.
+Thus matrix methods like `row_span` return `free_modules` but not `free_quadratic_modules`.
``````




---

archive/issue_comments_375411.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d7fa6ec8008f47c9420131756c168aaeeac0f1ba\">d7fa6ec</a></td><td><code>use domain.submodule instead of row_module</code></td></tr></table>\n",
    "created_at": "2018-01-24T13:46:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24590#issuecomment-375411",
    "user": "https://github.com/simonbrandhorst"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d7fa6ec8008f47c9420131756c168aaeeac0f1ba">d7fa6ec</a></td><td><code>use domain.submodule instead of row_module</code></td></tr></table>




---

archive/issue_comments_375412.json:
```json
{
    "body": "Commit: **[d7fa6ec](https://github.com/sagemath/sagetrac-mirror/commit/d7fa6ec8008f47c9420131756c168aaeeac0f1ba)**",
    "created_at": "2018-01-24T13:46:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24590#issuecomment-375412",
    "user": "https://github.com/simonbrandhorst"
}
```

Commit: **[d7fa6ec](https://github.com/sagemath/sagetrac-mirror/commit/d7fa6ec8008f47c9420131756c168aaeeac0f1ba)**



---

archive/issue_events_322524.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-01-24T13:46:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24590#event-322524"
}
```



---

archive/issue_events_322525.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-02-04T21:27:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24590#event-322525"
}
```



---

archive/issue_events_322526.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-02-04T21:27:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24590#event-322526"
}
```



---

archive/issue_comments_375413.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLGTM.",
    "created_at": "2018-02-04T21:27:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24590#issuecomment-375413",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
LGTM.



---

archive/issue_comments_375414.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-02-04T21:27:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24590#issuecomment-375414",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_375415.json:
```json
{
    "body": "Changed branch from **[u/sbrandhorst/bug_in_inverse_image_of_module_homomorphism](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/bug_in_inverse_image_of_module_homomorphism)** to **[d7fa6ec](https://github.com/sagemath/sagetrac-mirror/commit/d7fa6ec8008f47c9420131756c168aaeeac0f1ba)**",
    "created_at": "2018-02-09T08:03:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24590#issuecomment-375415",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/sbrandhorst/bug_in_inverse_image_of_module_homomorphism](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/bug_in_inverse_image_of_module_homomorphism)** to **[d7fa6ec](https://github.com/sagemath/sagetrac-mirror/commit/d7fa6ec8008f47c9420131756c168aaeeac0f1ba)**



---

archive/issue_events_322527.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-09T08:03:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24590#event-322527"
}
```



---

archive/issue_events_322528.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ac14d82c6f0c0f96c7872ddfa8942a2708433fd2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-09T08:03:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24590",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24590#event-322528"
}
```
