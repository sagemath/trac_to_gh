# Issue 24478: Random "Maxima crashed -- automatically restarting" failure

archive/issues_024241.json:
```json
{
    "assignees": [],
    "body": "This occasionally happens on the buildbot (seen on multiple machines):\n\n```\n**********************************************************************\nFile \"src/sage/interfaces/maxima.py\", line 1227, in sage.interfaces.maxima.MaximaElementFunction.__init__\nFailed example:\n    g = m.function('x,y','x+y^9')\nExpected nothing\nGot:\n    Maxima crashed -- automatically restarting.\n**********************************************************************\n1 item had failures:\n   1 of   8 in sage.interfaces.maxima.MaximaElementFunction.__init__\n    [182 tests, 1 failure, 29.76 s]\n----------------------------------------------------------------------\nsage -t --long src/sage/interfaces/maxima.py  # 1 doctest failed\n----------------------------------------------------------------------\n```\n\nKeywords: **random_fail**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24478_\n\n",
    "closed_at": "2021-09-10T17:33:19Z",
    "created_at": "2018-01-06T11:28:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Random \"Maxima crashed -- automatically restarting\" failure",
    "type": "issue",
    "updated_at": "2021-09-10T17:33:19Z",
    "url": "https://github.com/sagemath/sage/issues/24478",
    "user": "https://github.com/vbraun"
}
```
This occasionally happens on the buildbot (seen on multiple machines):

```
**********************************************************************
File "src/sage/interfaces/maxima.py", line 1227, in sage.interfaces.maxima.MaximaElementFunction.__init__
Failed example:
    g = m.function('x,y','x+y^9')
Expected nothing
Got:
    Maxima crashed -- automatically restarting.
**********************************************************************
1 item had failures:
   1 of   8 in sage.interfaces.maxima.MaximaElementFunction.__init__
    [182 tests, 1 failure, 29.76 s]
----------------------------------------------------------------------
sage -t --long src/sage/interfaces/maxima.py  # 1 doctest failed
----------------------------------------------------------------------
```

Keywords: **random_fail**

_Issue created by migration from https://trac.sagemath.org/ticket/24478_





---

archive/issue_events_321078.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-01-06T11:28:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321078"
}
```



---

archive/issue_events_321079.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-01-06T11:28:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321079"
}
```



---

archive/issue_events_321080.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-01-06T11:28:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321080"
}
```



---

archive/issue_events_321081.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-01-06T11:28:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321081"
}
```



---

archive/issue_events_321082.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-07T08:59:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321082"
}
```



---

archive/issue_events_321083.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-07T08:59:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321083"
}
```



---

archive/issue_events_321084.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-07T08:59:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321084"
}
```



---

archive/issue_events_321085.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-07T08:59:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321085"
}
```



---

archive/issue_comments_373255.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\noutdated, should close",
    "created_at": "2021-09-07T08:59:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24478#issuecomment-373255",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'>Comment 1:</a>
outdated, should close



---

archive/issue_events_321086.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2021-09-10T11:21:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321086"
}
```



---

archive/issue_events_321087.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2021-09-10T11:21:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321087"
}
```



---

archive/issue_comments_373256.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI agree",
    "created_at": "2021-09-10T11:21:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24478#issuecomment-373256",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:2'>Comment 2:</a>
I agree



---

archive/issue_events_321088.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T17:33:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321088"
}
```



---

archive/issue_events_321089.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T17:33:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24478",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24478#event-321089"
}
```
