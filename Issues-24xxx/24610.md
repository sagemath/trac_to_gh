# Issue 24610: Change richcmp() to a cpdef inline function

archive/issues_024373.json:
```json
{
    "assignees": [],
    "body": "Fix this Cython warning:\n\n```\n[sagelib-8.2.beta4] warning: sage/structure/richcmp.pyx:61:0: Overriding cdef method with def method.\n```\n\nBranch/Commit: **[9e3e338](https://github.com/sagemath/sagetrac-mirror/commit/9e3e33874326f119479346c20edac973d891e31f)**\n\nReviewer: **Marc Mezzarobba**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24610_\n\n",
    "closed_at": "2018-02-02T12:05:41Z",
    "created_at": "2018-01-29T21:04:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Change richcmp() to a cpdef inline function",
    "type": "issue",
    "updated_at": "2018-02-02T12:05:41Z",
    "url": "https://github.com/sagemath/sage/issues/24610",
    "user": "https://github.com/jdemeyer"
}
```
Fix this Cython warning:

```
[sagelib-8.2.beta4] warning: sage/structure/richcmp.pyx:61:0: Overriding cdef method with def method.
```

Branch/Commit: **[9e3e338](https://github.com/sagemath/sagetrac-mirror/commit/9e3e33874326f119479346c20edac973d891e31f)**

Reviewer: **Marc Mezzarobba**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24610_





---

archive/issue_events_322757.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-29T21:04:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24610#event-322757"
}
```



---

archive/issue_events_322758.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-29T21:04:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24610#event-322758"
}
```



---

archive/issue_events_322759.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-29T21:04:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24610#event-322759"
}
```



---

archive/issue_events_322760.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-29T21:04:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24610#event-322760"
}
```



---

archive/issue_comments_375819.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+Fix this Cython warning:\n \n+```\n+[sagelib-8.2.beta4] warning: sage/structure/richcmp.pyx:61:0: Overriding cdef method with def method.\n+```\n``````\n",
    "created_at": "2018-01-29T21:09:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24610#issuecomment-375819",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+Fix this Cython warning:
 
+```
+[sagelib-8.2.beta4] warning: sage/structure/richcmp.pyx:61:0: Overriding cdef method with def method.
+```
``````




---

archive/issue_comments_375820.json:
```json
{
    "body": "Branch: **[u/jdemeyer/change_richcmp___to_a_cpdef_inline_function](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_richcmp___to_a_cpdef_inline_function)**",
    "created_at": "2018-01-29T21:19:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24610#issuecomment-375820",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/change_richcmp___to_a_cpdef_inline_function](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_richcmp___to_a_cpdef_inline_function)**



---

archive/issue_events_322761.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-29T21:20:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24610#event-322761"
}
```



---

archive/issue_comments_375821.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9e3e33874326f119479346c20edac973d891e31f\">9e3e338</a></td><td><code>Change richcmp() to a cpdef inline function</code></td></tr></table>\n",
    "created_at": "2018-01-29T21:20:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24610#issuecomment-375821",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9e3e33874326f119479346c20edac973d891e31f">9e3e338</a></td><td><code>Change richcmp() to a cpdef inline function</code></td></tr></table>




---

archive/issue_comments_375822.json:
```json
{
    "body": "Commit: **[9e3e338](https://github.com/sagemath/sagetrac-mirror/commit/9e3e33874326f119479346c20edac973d891e31f)**",
    "created_at": "2018-01-29T21:20:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24610#issuecomment-375822",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[9e3e338](https://github.com/sagemath/sagetrac-mirror/commit/9e3e33874326f119479346c20edac973d891e31f)**



---

archive/issue_comments_375823.json:
```json
{
    "body": "Reviewer: **Marc Mezzarobba**",
    "created_at": "2018-01-31T08:39:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24610#issuecomment-375823",
    "user": "https://github.com/mezzarobba"
}
```

Reviewer: **Marc Mezzarobba**



---

archive/issue_events_322762.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-01-31T08:39:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24610#event-322762"
}
```



---

archive/issue_events_322763.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-01-31T08:39:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24610#event-322763"
}
```



---

archive/issue_comments_375824.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/change_richcmp___to_a_cpdef_inline_function](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_richcmp___to_a_cpdef_inline_function)** to **[9e3e338](https://github.com/sagemath/sagetrac-mirror/commit/9e3e33874326f119479346c20edac973d891e31f)**",
    "created_at": "2018-02-02T12:05:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24610#issuecomment-375824",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/change_richcmp___to_a_cpdef_inline_function](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_richcmp___to_a_cpdef_inline_function)** to **[9e3e338](https://github.com/sagemath/sagetrac-mirror/commit/9e3e33874326f119479346c20edac973d891e31f)**



---

archive/issue_events_322764.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-02T12:05:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24610#event-322764"
}
```



---

archive/issue_events_322765.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b200dbae270f0ced94e4e4ff3f24aff1ecf45984",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-02T12:05:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24610",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24610#event-322765"
}
```
