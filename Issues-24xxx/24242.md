# Issue 24242: Absolute import when importing string

archive/issues_024005.json:
```json
{
    "assignees": [],
    "body": "A few Sage modules import the Python module `string`. Since this name is so generic, it might conflict with submodules inside Sage (this happened in #24222). So it's better to use `from `__future__` import absolute_import`.\n\nBranch/Commit: **[55a76ce](https://github.com/sagemath/sagetrac-mirror/commit/55a76ce77c3b9464d74ab5c2af4dac3cd8a5efcd)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24242_\n\n",
    "closed_at": "2017-12-11T01:01:53Z",
    "created_at": "2017-11-19T16:31:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Absolute import when importing string",
    "type": "issue",
    "updated_at": "2017-12-11T01:01:53Z",
    "url": "https://github.com/sagemath/sage/issues/24242",
    "user": "https://github.com/jdemeyer"
}
```
A few Sage modules import the Python module `string`. Since this name is so generic, it might conflict with submodules inside Sage (this happened in #24222). So it's better to use `from `__future__` import absolute_import`.

Branch/Commit: **[55a76ce](https://github.com/sagemath/sagetrac-mirror/commit/55a76ce77c3b9464d74ab5c2af4dac3cd8a5efcd)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24242_





---

archive/issue_events_318281.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-19T16:31:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24242#event-318281"
}
```



---

archive/issue_events_318282.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-19T16:31:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24242#event-318282"
}
```



---

archive/issue_events_318283.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-19T16:31:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24242#event-318283"
}
```



---

archive/issue_comments_369567.json:
```json
{
    "body": "Branch: **[u/jdemeyer/absolute_import_when_importing_string](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/absolute_import_when_importing_string)**",
    "created_at": "2017-11-19T16:35:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24242#issuecomment-369567",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/absolute_import_when_importing_string](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/absolute_import_when_importing_string)**



---

archive/issue_comments_369568.json:
```json
{
    "body": "Commit: **[55a76ce](https://github.com/sagemath/sagetrac-mirror/commit/55a76ce77c3b9464d74ab5c2af4dac3cd8a5efcd)**",
    "created_at": "2017-11-19T16:35:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24242#issuecomment-369568",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[55a76ce](https://github.com/sagemath/sagetrac-mirror/commit/55a76ce77c3b9464d74ab5c2af4dac3cd8a5efcd)**



---

archive/issue_events_318284.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-19T16:35:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24242#event-318284"
}
```



---

archive/issue_comments_369569.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/55a76ce77c3b9464d74ab5c2af4dac3cd8a5efcd\">55a76ce</a></td><td><code>Add absolute_import in a few places</code></td></tr></table>\n",
    "created_at": "2017-11-19T16:35:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24242#issuecomment-369569",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/55a76ce77c3b9464d74ab5c2af4dac3cd8a5efcd">55a76ce</a></td><td><code>Add absolute_import in a few places</code></td></tr></table>




---

archive/issue_comments_369570.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-A few modules import the Python module `string`. Since this name is so generic, it might conflict with submodules inside Sage (this happened in #24222). So it's better to use `from `__future__` import absolute_import`.\n+A few Sage modules import the Python module `string`. Since this name is so generic, it might conflict with submodules inside Sage (this happened in #24222). So it's better to use `from `__future__` import absolute_import`.\n``````\n",
    "created_at": "2017-11-19T16:35:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24242#issuecomment-369570",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-A few modules import the Python module `string`. Since this name is so generic, it might conflict with submodules inside Sage (this happened in #24222). So it's better to use `from `__future__` import absolute_import`.
+A few Sage modules import the Python module `string`. Since this name is so generic, it might conflict with submodules inside Sage (this happened in #24222). So it's better to use `from `__future__` import absolute_import`.
``````




---

archive/issue_events_318285.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-19T21:29:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24242#event-318285"
}
```



---

archive/issue_events_318286.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-19T21:29:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24242#event-318286"
}
```



---

archive/issue_comments_369571.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-11-19T21:29:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24242#issuecomment-369571",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_369572.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2017-11-19T21:29:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24242#issuecomment-369572",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_369573.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI thought at some point we had agreed to just put `from `__future__` import absolute_import` in every module.  It is *much better* anyways than the old import semantics.",
    "created_at": "2017-11-20T09:08:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24242#issuecomment-369573",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

I thought at some point we had agreed to just put `from `__future__` import absolute_import` in every module.  It is *much better* anyways than the old import semantics.



---

archive/issue_events_318287.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-11T01:01:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24242#event-318287"
}
```



---

archive/issue_events_318288.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "778522d7125e77c27351a548fc508e813e460e0e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-11T01:01:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24242#event-318288"
}
```



---

archive/issue_comments_369574.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/absolute_import_when_importing_string](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/absolute_import_when_importing_string)** to **[55a76ce](https://github.com/sagemath/sagetrac-mirror/commit/55a76ce77c3b9464d74ab5c2af4dac3cd8a5efcd)**",
    "created_at": "2017-12-11T01:01:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24242#issuecomment-369574",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/absolute_import_when_importing_string](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/absolute_import_when_importing_string)** to **[55a76ce](https://github.com/sagemath/sagetrac-mirror/commit/55a76ce77c3b9464d74ab5c2af4dac3cd8a5efcd)**
