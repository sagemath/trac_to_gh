# Issue 24624: Stop using new_t_POL_from_int_star from cypari2

archive/issues_024387.json:
```json
{
    "assignees": [],
    "body": "As part of the cypari2 cleanup, stop using the awkward `new_t_POL_from_int_star` function.\n\nCC:  @videlec @defeo\n\nBranch/Commit: **[9440ff4](https://github.com/sagemath/sagetrac-mirror/commit/9440ff4f3e83f14cbf7f58a78837e5292c94e69c)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24624_\n\n",
    "closed_at": "2018-02-02T12:05:31Z",
    "created_at": "2018-01-31T13:39:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Stop using new_t_POL_from_int_star from cypari2",
    "type": "issue",
    "updated_at": "2018-02-02T12:05:31Z",
    "url": "https://github.com/sagemath/sage/issues/24624",
    "user": "https://github.com/jdemeyer"
}
```
As part of the cypari2 cleanup, stop using the awkward `new_t_POL_from_int_star` function.

CC:  @videlec @defeo

Branch/Commit: **[9440ff4](https://github.com/sagemath/sagetrac-mirror/commit/9440ff4f3e83f14cbf7f58a78837e5292c94e69c)**

Reviewer: **Vincent Delecroix**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24624_





---

archive/issue_events_322908.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-31T13:39:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24624#event-322908"
}
```



---

archive/issue_events_322909.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-31T13:39:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24624#event-322909"
}
```



---

archive/issue_events_322910.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-31T13:39:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24624#event-322910"
}
```



---

archive/issue_events_322911.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-31T13:39:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24624#event-322911"
}
```



---

archive/issue_comments_376090.json:
```json
{
    "body": "Branch: **[u/jdemeyer/stop_using_new_t_pol_from_int_star_from_cypari2](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_new_t_pol_from_int_star_from_cypari2)**",
    "created_at": "2018-01-31T14:04:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24624#issuecomment-376090",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/stop_using_new_t_pol_from_int_star_from_cypari2](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_new_t_pol_from_int_star_from_cypari2)**



---

archive/issue_events_322912.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-31T14:12:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24624#event-322912"
}
```



---

archive/issue_comments_376091.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9440ff4f3e83f14cbf7f58a78837e5292c94e69c\">9440ff4</a></td><td><code>Move new_t_POL_from_int_star from cypari2 to Sage</code></td></tr></table>\n",
    "created_at": "2018-01-31T14:12:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24624#issuecomment-376091",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9440ff4f3e83f14cbf7f58a78837e5292c94e69c">9440ff4</a></td><td><code>Move new_t_POL_from_int_star from cypari2 to Sage</code></td></tr></table>




---

archive/issue_comments_376092.json:
```json
{
    "body": "Commit: **[9440ff4](https://github.com/sagemath/sagetrac-mirror/commit/9440ff4f3e83f14cbf7f58a78837e5292c94e69c)**",
    "created_at": "2018-01-31T14:12:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24624#issuecomment-376092",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[9440ff4](https://github.com/sagemath/sagetrac-mirror/commit/9440ff4f3e83f14cbf7f58a78837e5292c94e69c)**



---

archive/issue_events_322913.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-02-01T08:42:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24624#event-322913"
}
```



---

archive/issue_events_322914.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-02-01T08:42:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24624#event-322914"
}
```



---

archive/issue_comments_376093.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2018-02-01T08:42:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24624#issuecomment-376093",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_events_322915.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-02T12:05:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24624#event-322915"
}
```



---

archive/issue_events_322916.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d842b25062b1801559b5ddbb312bd6c9a06d013e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-02T12:05:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24624#event-322916"
}
```



---

archive/issue_comments_376094.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/stop_using_new_t_pol_from_int_star_from_cypari2](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_new_t_pol_from_int_star_from_cypari2)** to **[9440ff4](https://github.com/sagemath/sagetrac-mirror/commit/9440ff4f3e83f14cbf7f58a78837e5292c94e69c)**",
    "created_at": "2018-02-02T12:05:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24624#issuecomment-376094",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/stop_using_new_t_pol_from_int_star_from_cypari2](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/stop_using_new_t_pol_from_int_star_from_cypari2)** to **[9440ff4](https://github.com/sagemath/sagetrac-mirror/commit/9440ff4f3e83f14cbf7f58a78837e5292c94e69c)**
