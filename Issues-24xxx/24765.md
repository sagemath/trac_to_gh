# Issue 24765: GAP fails its own test on SPARC Solaris 11

archive/issues_024528.json:
```json
{
    "assignees": [],
    "body": "running the usual \"testinstall.tst\" for GAP fails here:\n\n```\ngap> HashKeyBag([1,2,3],0,1,-1) <> HashKeyWholeBag([1,2,3],0);\nBus Error (core dumped)\n```\n\nSee https://github.com/gap-system/gap/issues/2191\n\nAlignment  problem, see\nhttps://github.com/gap-system/gap/pull/2196\n\nCC:  @jdemeyer @dimpase\n\nBranch/Commit: **[u/dimpase/sparc/gap_align](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/sparc/gap_align) @ [b9179e9](https://github.com/sagemath/sagetrac-mirror/commit/b9179e94a3d75efca288d90b46bfcaa3b56df361)**\n\nUpstream: **Fixed upstream, in a later stable release.**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24765_\n\n",
    "closed_at": "2020-07-10T07:27:00Z",
    "created_at": "2018-02-17T22:11:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20solaris",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "GAP fails its own test on SPARC Solaris 11",
    "type": "issue",
    "updated_at": "2021-01-21T10:39:14Z",
    "url": "https://github.com/sagemath/sage/issues/24765",
    "user": "https://github.com/dimpase"
}
```
running the usual "testinstall.tst" for GAP fails here:

```
gap> HashKeyBag([1,2,3],0,1,-1) <> HashKeyWholeBag([1,2,3],0);
Bus Error (core dumped)
```

See https://github.com/gap-system/gap/issues/2191

Alignment  problem, see
https://github.com/gap-system/gap/pull/2196

CC:  @jdemeyer @dimpase

Branch/Commit: **[u/dimpase/sparc/gap_align](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/sparc/gap_align) @ [b9179e9](https://github.com/sagemath/sagetrac-mirror/commit/b9179e94a3d75efca288d90b46bfcaa3b56df361)**

Upstream: **Fixed upstream, in a later stable release.**

Reviewer: **Dima Pasechnik**

Author: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/24765_





---

archive/issue_events_324811.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-02-17T22:11:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324811"
}
```



---

archive/issue_events_324812.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-02-17T22:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20solaris",
    "label_color": "000080",
    "label_name": "component: porting: solaris",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324812"
}
```



---

archive/issue_events_324813.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-02-17T22:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324813"
}
```



---

archive/issue_events_324814.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-02-17T22:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324814"
}
```



---

archive/issue_comments_379296.json:
```json
{
    "body": "Author: **Dima Pasechnik**",
    "created_at": "2018-02-19T14:21:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379296",
    "user": "https://github.com/dimpase"
}
```

Author: **Dima Pasechnik**



---

archive/issue_comments_379297.json:
```json
{
    "body": "Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**",
    "created_at": "2018-02-19T14:21:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379297",
    "user": "https://github.com/dimpase"
}
```

Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**



---

archive/issue_comments_379298.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nsee https://github.com/gap-system/gap/pull/2195 for a fix",
    "created_at": "2018-02-19T14:21:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379298",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:1" align="right">Comment 1</div>

see https://github.com/gap-system/gap/pull/2195 for a fix



---

archive/issue_comments_379299.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nthere are more issues:\n\nhttps://github.com/gap-system/gap/issues/2193\nand\nhttps://github.com/gap-system/gap/issues/2194",
    "created_at": "2018-02-19T19:53:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379299",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:2" align="right">Comment 2</div>

there are more issues:

https://github.com/gap-system/gap/issues/2193
and
https://github.com/gap-system/gap/issues/2194



---

archive/issue_comments_379300.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,4 +7,5 @@\n \n See https://github.com/gap-system/gap/issues/2191\n \n-Probably a big-endianness problem.\n+Alignment  problem, see\n+https://github.com/gap-system/gap/pull/2196\n``````\n",
    "created_at": "2018-03-31T09:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379300",
    "user": "https://github.com/dimpase"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,4 +7,5 @@
 
 See https://github.com/gap-system/gap/issues/2191
 
-Probably a big-endianness problem.
+Alignment  problem, see
+https://github.com/gap-system/gap/pull/2196
``````




---

archive/issue_comments_379301.json:
```json
{
    "body": "Commit: **[b9179e9](https://github.com/sagemath/sagetrac-mirror/commit/b9179e94a3d75efca288d90b46bfcaa3b56df361)**",
    "created_at": "2018-03-31T09:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379301",
    "user": "https://github.com/dimpase"
}
```

Commit: **[b9179e9](https://github.com/sagemath/sagetrac-mirror/commit/b9179e94a3d75efca288d90b46bfcaa3b56df361)**



---

archive/issue_comments_379302.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b9179e94a3d75efca288d90b46bfcaa3b56df361\">b9179e9</a></td><td><code>backport of alignment fix for (lib)gap</code></td></tr></table>\n",
    "created_at": "2018-03-31T09:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379302",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b9179e94a3d75efca288d90b46bfcaa3b56df361">b9179e9</a></td><td><code>backport of alignment fix for (lib)gap</code></td></tr></table>




---

archive/issue_comments_379303.json:
```json
{
    "body": "Branch: **[u/dimpase/sparc/gap_align](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/sparc/gap_align)**",
    "created_at": "2018-03-31T09:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379303",
    "user": "https://github.com/dimpase"
}
```

Branch: **[u/dimpase/sparc/gap_align](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/sparc/gap_align)**



---

archive/issue_comments_379304.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nWith this patch, on SPARC Solaris tests in src/sage/interfaces/gap_workspace.py, src/sage/interfaces/gap.py and src/sage/libs/gap/ pass.",
    "created_at": "2018-03-31T19:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379304",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:4" align="right">Comment 4</div>

With this patch, on SPARC Solaris tests in src/sage/interfaces/gap_workspace.py, src/sage/interfaces/gap.py and src/sage/libs/gap/ pass.



---

archive/issue_events_324815.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-03-31T19:40:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324815"
}
```



---

archive/issue_comments_379305.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis does not solve all of GAP's problems on SPARC Solaris, but it at least gets rid of segfaults.",
    "created_at": "2018-03-31T19:41:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379305",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:5" align="right">Comment 5</div>

This does not solve all of GAP's problems on SPARC Solaris, but it at least gets rid of segfaults.



---

archive/issue_events_324816.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:51:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324816"
}
```



---

archive/issue_events_324817.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:51:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324817"
}
```



---

archive/issue_events_324818.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:51:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324818"
}
```



---

archive/issue_comments_379306.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nOutdated, should be closed",
    "created_at": "2020-07-08T16:51:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379306",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Outdated, should be closed



---

archive/issue_events_324819.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-07-08T19:29:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324819"
}
```



---

archive/issue_events_324820.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-07-08T19:29:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324820"
}
```



---

archive/issue_comments_379307.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-07-08T19:29:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379307",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_324821.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-07-10T07:27:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324821"
}
```



---

archive/issue_events_324822.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-07-10T07:27:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24765#event-324822"
}
```



---

archive/issue_comments_379308.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nwe now have access to Solaris 11.4 (on Sparc), if anyone wants to give it a try...",
    "created_at": "2021-01-21T10:39:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24765#issuecomment-379308",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:9" align="right">Comment 9</div>

we now have access to Solaris 11.4 (on Sparc), if anyone wants to give it a try...
