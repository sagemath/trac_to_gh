# Issue 24248: Fix isinstance(x, int) calls in element.pyx

archive/issues_024011.json:
```json
{
    "assignees": [],
    "body": "\n\nDepends on #24244\n\nBranch/Commit: **[9c579c7](https://github.com/sagemath/sagetrac-mirror/commit/9c579c7ed1cf09f567c0452352b65d524ad66371)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24248_\n\n",
    "closed_at": "2017-12-13T17:38:15Z",
    "created_at": "2017-11-20T09:44:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix isinstance(x, int) calls in element.pyx",
    "type": "issue",
    "updated_at": "2017-12-13T17:38:15Z",
    "url": "https://github.com/sagemath/sage/issues/24248",
    "user": "https://github.com/jdemeyer"
}
```


Depends on #24244

Branch/Commit: **[9c579c7](https://github.com/sagemath/sagetrac-mirror/commit/9c579c7ed1cf09f567c0452352b65d524ad66371)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24248_





---

archive/issue_events_318360.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-20T09:44:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318360"
}
```



---

archive/issue_events_318361.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-20T09:44:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318361"
}
```



---

archive/issue_events_318362.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-20T09:44:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318362"
}
```



---

archive/issue_events_318363.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-20T09:44:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318363"
}
```



---

archive/issue_comments_369717.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/24248](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/24248)**",
    "created_at": "2017-11-20T09:57:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24248#issuecomment-369717",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/24248](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/24248)**



---

archive/issue_comments_369718.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b1ea838fc07baf8d6247a15f8db067ade9596e4a\">b1ea838</a></td><td><code>New functions integer_check_long() and integer_check_long_py()</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/46b6c25e146e51dca04e228b65625eae347c8045\">46b6c25</a></td><td><code>Fix isinstance(x, int) calls in element.pyx</code></td></tr></table>\n",
    "created_at": "2017-11-20T10:21:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24248#issuecomment-369718",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b1ea838fc07baf8d6247a15f8db067ade9596e4a">b1ea838</a></td><td><code>New functions integer_check_long() and integer_check_long_py()</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/46b6c25e146e51dca04e228b65625eae347c8045">46b6c25</a></td><td><code>Fix isinstance(x, int) calls in element.pyx</code></td></tr></table>




---

archive/issue_comments_369719.json:
```json
{
    "body": "Commit: **[46b6c25](https://github.com/sagemath/sagetrac-mirror/commit/46b6c25e146e51dca04e228b65625eae347c8045)**",
    "created_at": "2017-11-20T10:21:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24248#issuecomment-369719",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[46b6c25](https://github.com/sagemath/sagetrac-mirror/commit/46b6c25e146e51dca04e228b65625eae347c8045)**



---

archive/issue_events_318364.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-20T10:36:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318364"
}
```



---

archive/issue_events_318365.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-20T16:30:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318365"
}
```



---

archive/issue_events_318366.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-20T16:30:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318366"
}
```



---

archive/issue_comments_369720.json:
```json
{
    "body": "Changed commit from **[46b6c25](https://github.com/sagemath/sagetrac-mirror/commit/46b6c25e146e51dca04e228b65625eae347c8045)** to **[9c579c7](https://github.com/sagemath/sagetrac-mirror/commit/9c579c7ed1cf09f567c0452352b65d524ad66371)**",
    "created_at": "2017-11-20T18:14:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24248#issuecomment-369720",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[46b6c25](https://github.com/sagemath/sagetrac-mirror/commit/46b6c25e146e51dca04e228b65625eae347c8045)** to **[9c579c7](https://github.com/sagemath/sagetrac-mirror/commit/9c579c7ed1cf09f567c0452352b65d524ad66371)**



---

archive/issue_comments_369721.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bb114c91736f48e5854613ae438377ed7f032b7e\">bb114c9</a></td><td><code>Fake Integer interface</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4d76bc133b7b93cf3a3baaad782caedee692ae0a\">4d76bc1</a></td><td><code>New functions integer_check_long() and integer_check_long_py()</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9c579c7ed1cf09f567c0452352b65d524ad66371\">9c579c7</a></td><td><code>Fix isinstance(x, int) calls in element.pyx</code></td></tr></table>\n",
    "created_at": "2017-11-20T18:14:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24248#issuecomment-369721",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bb114c91736f48e5854613ae438377ed7f032b7e">bb114c9</a></td><td><code>Fake Integer interface</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4d76bc133b7b93cf3a3baaad782caedee692ae0a">4d76bc1</a></td><td><code>New functions integer_check_long() and integer_check_long_py()</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9c579c7ed1cf09f567c0452352b65d524ad66371">9c579c7</a></td><td><code>Fix isinstance(x, int) calls in element.pyx</code></td></tr></table>




---

archive/issue_events_318367.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-20T18:56:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318367"
}
```



---

archive/issue_events_318368.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-20T18:56:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318368"
}
```



---

archive/issue_comments_369722.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nLGTM modulo #24244.",
    "created_at": "2017-11-20T23:48:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24248#issuecomment-369722",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:7" align="right">Comment 7</div>

LGTM modulo #24244.



---

archive/issue_comments_369723.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-11-20T23:48:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24248#issuecomment-369723",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_318369.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-20T23:48:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318369"
}
```



---

archive/issue_events_318370.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-20T23:48:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318370"
}
```



---

archive/issue_comments_369724.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/24248](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/24248)** to **[9c579c7](https://github.com/sagemath/sagetrac-mirror/commit/9c579c7ed1cf09f567c0452352b65d524ad66371)**",
    "created_at": "2017-12-13T17:38:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24248#issuecomment-369724",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/24248](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/24248)** to **[9c579c7](https://github.com/sagemath/sagetrac-mirror/commit/9c579c7ed1cf09f567c0452352b65d524ad66371)**



---

archive/issue_events_318371.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-13T17:38:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318371"
}
```



---

archive/issue_events_318372.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d1f349904c812fc0f54210ffa1ffbf2acabdc825",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-13T17:38:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24248",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24248#event-318372"
}
```
