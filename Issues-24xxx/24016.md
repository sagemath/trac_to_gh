# Issue 24016: Expose methods to create/destroy FLINT vectors

archive/issues_023779.json:
```json
{
    "assignees": [],
    "body": "This addressess a point relevant for #23946: for some reason, the FLINT library interface doesn't currently expose the allocation/deallocation methods for `fmpz` vectors. The modification is literally a two-line patch.\n\nBranch/Commit: **[0fc52cb](https://github.com/sagemath/sagetrac-mirror/commit/0fc52cb301179d6d8f5230a2119fc8d88964f54d)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Kiran Kedlaya**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24016_\n\n",
    "closed_at": "2017-10-22T17:23:40Z",
    "created_at": "2017-10-12T05:09:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Expose methods to create/destroy FLINT vectors",
    "type": "issue",
    "updated_at": "2017-10-22T17:23:40Z",
    "url": "https://github.com/sagemath/sage/issues/24016",
    "user": "https://github.com/kedlaya"
}
```
This addressess a point relevant for #23946: for some reason, the FLINT library interface doesn't currently expose the allocation/deallocation methods for `fmpz` vectors. The modification is literally a two-line patch.

Branch/Commit: **[0fc52cb](https://github.com/sagemath/sagetrac-mirror/commit/0fc52cb301179d6d8f5230a2119fc8d88964f54d)**

Reviewer: **Jeroen Demeyer**

Author: **Kiran Kedlaya**

_Issue created by migration from https://trac.sagemath.org/ticket/24016_





---

archive/issue_events_315429.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-12T05:09:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24016#event-315429"
}
```



---

archive/issue_events_315430.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-12T05:09:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24016#event-315430"
}
```



---

archive/issue_events_315431.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-12T05:09:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24016#event-315431"
}
```



---

archive/issue_events_315432.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-12T05:09:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24016#event-315432"
}
```



---

archive/issue_comments_365469.json:
```json
{
    "body": "Branch: **[u/kedlaya/expose_methods_to_create_destroy_flint_vectors](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/expose_methods_to_create_destroy_flint_vectors)**",
    "created_at": "2017-10-12T05:11:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24016#issuecomment-365469",
    "user": "https://github.com/kedlaya"
}
```

Branch: **[u/kedlaya/expose_methods_to_create_destroy_flint_vectors](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/expose_methods_to_create_destroy_flint_vectors)**



---

archive/issue_events_315433.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2017-10-12T05:57:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24016#event-315433"
}
```



---

archive/issue_comments_365470.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAs promised, a two-line patch.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0fc52cb301179d6d8f5230a2119fc8d88964f54d\">0fc52cb</a></td><td><code>Expose allocation/deallocation of FLINT fmpz vectors</code></td></tr></table>\n",
    "created_at": "2017-10-12T05:57:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24016#issuecomment-365470",
    "user": "https://github.com/kedlaya"
}
```

<div id="comment:2" align="right">Comment 2</div>

As promised, a two-line patch.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0fc52cb301179d6d8f5230a2119fc8d88964f54d">0fc52cb</a></td><td><code>Expose allocation/deallocation of FLINT fmpz vectors</code></td></tr></table>




---

archive/issue_comments_365471.json:
```json
{
    "body": "Commit: **[0fc52cb](https://github.com/sagemath/sagetrac-mirror/commit/0fc52cb301179d6d8f5230a2119fc8d88964f54d)**",
    "created_at": "2017-10-12T05:57:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24016#issuecomment-365471",
    "user": "https://github.com/kedlaya"
}
```

Commit: **[0fc52cb](https://github.com/sagemath/sagetrac-mirror/commit/0fc52cb301179d6d8f5230a2119fc8d88964f54d)**



---

archive/issue_comments_365472.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2017-10-12T12:16:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24016#issuecomment-365472",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_315434.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-12T12:16:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24016#event-315434"
}
```



---

archive/issue_events_315435.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-10-12T12:16:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24016#event-315435"
}
```



---

archive/issue_comments_365473.json:
```json
{
    "body": "Author: **Kiran Kedlaya**",
    "created_at": "2017-10-12T12:16:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24016#issuecomment-365473",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Kiran Kedlaya**



---

archive/issue_comments_365474.json:
```json
{
    "body": "Changed branch from **[u/kedlaya/expose_methods_to_create_destroy_flint_vectors](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/expose_methods_to_create_destroy_flint_vectors)** to **[0fc52cb](https://github.com/sagemath/sagetrac-mirror/commit/0fc52cb301179d6d8f5230a2119fc8d88964f54d)**",
    "created_at": "2017-10-22T17:23:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24016#issuecomment-365474",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/kedlaya/expose_methods_to_create_destroy_flint_vectors](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/expose_methods_to_create_destroy_flint_vectors)** to **[0fc52cb](https://github.com/sagemath/sagetrac-mirror/commit/0fc52cb301179d6d8f5230a2119fc8d88964f54d)**



---

archive/issue_events_315436.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-10-22T17:23:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24016#event-315436"
}
```



---

archive/issue_events_315437.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a2cd05e67336bfb2b2a6919d038f4f4e397323f9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-10-22T17:23:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24016",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24016#event-315437"
}
```
