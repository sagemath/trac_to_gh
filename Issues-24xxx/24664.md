# Issue 24664: Implement Artin groups for finite type

archive/issues_024427.json:
```json
{
    "body": "We can adapt the `Braid.left_normal_form` into a general method for finite type Artin groups. In the process, we introduce an ABC `ArtinGroup` and make both `BraidGroup_class` and `RightAngledArtinGroup` as subclasses (and similarly for element classes). We fix a few bugs along the way as well (comparisons of RAAG elements are broken)\n\nCC:  @videlec @jhpalmieri @dimpase @miguelmarco @kcrisman @amitjamadagni fugelde\n\nKeywords: braid, Artin\n\nBranch/Commit: 97d235a4c973ef20cea57a1766dec2200610e2f3\n\nReviewer: Miguel Marco\n\nAuthor: Travis Scrimshaw\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/24664\n\n",
    "closed_at": "2018-02-09T23:47:13Z",
    "created_at": "2018-02-06T02:25:54Z",
    "labels": [
        "component: group theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.2",
    "title": "Implement Artin groups for finite type",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24664",
    "user": "https://github.com/tscrim"
}
```
We can adapt the `Braid.left_normal_form` into a general method for finite type Artin groups. In the process, we introduce an ABC `ArtinGroup` and make both `BraidGroup_class` and `RightAngledArtinGroup` as subclasses (and similarly for element classes). We fix a few bugs along the way as well (comparisons of RAAG elements are broken)

CC:  @videlec @jhpalmieri @dimpase @miguelmarco @kcrisman @amitjamadagni fugelde

Keywords: braid, Artin

Branch/Commit: 97d235a4c973ef20cea57a1766dec2200610e2f3

Reviewer: Miguel Marco

Author: Travis Scrimshaw

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/24664





---

archive/issue_comments_468195.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-02-06T02:47:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468195",
    "user": "https://github.com/tscrim"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_468196.json:
```json
{
    "body": "<a id='comment:1'></a>\nNew commits:\n|                                                                                                                                          |                                         |\n|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|\n|[7ca51b4](https://git.sagemath.org/sage.git/commit?id=7ca51b43ffdafc0585b01cfe8e15906b3d957743)|`Initial implementation of Artin groups.`|\n|[365e0a0](https://git.sagemath.org/sage.git/commit?id=365e0a03056902f5361c835a05308ce8b9224c16)|`Making RAAG's compatible and fixing some bugs.`|\n|[43ea78e](https://git.sagemath.org/sage.git/commit?id=43ea78e6b51bf93154b0425a3869739546750f24)|`Subclass braid group (elements) from Artin group (elements). Fix some bugs.`|",
    "created_at": "2018-02-06T02:47:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468196",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'></a>
New commits:
|                                                                                                                                          |                                         |
|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|
|[7ca51b4](https://git.sagemath.org/sage.git/commit?id=7ca51b43ffdafc0585b01cfe8e15906b3d957743)|`Initial implementation of Artin groups.`|
|[365e0a0](https://git.sagemath.org/sage.git/commit?id=365e0a03056902f5361c835a05308ce8b9224c16)|`Making RAAG's compatible and fixing some bugs.`|
|[43ea78e](https://git.sagemath.org/sage.git/commit?id=43ea78e6b51bf93154b0425a3869739546750f24)|`Subclass braid group (elements) from Artin group (elements). Fix some bugs.`|



---

archive/issue_comments_468197.json:
```json
{
    "body": "Changing commit from \"\" to \"43ea78e6b51bf93154b0425a3869739546750f24\"",
    "created_at": "2018-02-06T02:47:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468197",
    "user": "https://github.com/tscrim"
}
```

Changing commit from "" to "43ea78e6b51bf93154b0425a3869739546750f24"



---

archive/issue_comments_468198.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-We can adapt the `Braid.left_normal_form` into a general method for finite type Artin groups. In the process, we introduce an ABC `ArtinGroup` and make both `BraidGroup_class` and `RightAngledArtinGroup` as subclasses (and similarly for element classes).\n+We can adapt the `Braid.left_normal_form` into a general method for finite type Artin groups. In the process, we introduce an ABC `ArtinGroup` and make both `BraidGroup_class` and `RightAngledArtinGroup` as subclasses (and similarly for element classes). We fix a few bugs along the way as well (comparisons of RAAG elements are broken)\n``````\n",
    "created_at": "2018-02-06T02:47:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468198",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-We can adapt the `Braid.left_normal_form` into a general method for finite type Artin groups. In the process, we introduce an ABC `ArtinGroup` and make both `BraidGroup_class` and `RightAngledArtinGroup` as subclasses (and similarly for element classes).
+We can adapt the `Braid.left_normal_form` into a general method for finite type Artin groups. In the process, we introduce an ABC `ArtinGroup` and make both `BraidGroup_class` and `RightAngledArtinGroup` as subclasses (and similarly for element classes). We fix a few bugs along the way as well (comparisons of RAAG elements are broken)
``````




---

archive/issue_comments_468199.json:
```json
{
    "body": "Changing branch from \"\" to \"public/groups/artin_groups-24664\"",
    "created_at": "2018-02-06T02:47:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468199",
    "user": "https://github.com/tscrim"
}
```

Changing branch from "" to "public/groups/artin_groups-24664"



---

archive/issue_comments_468200.json:
```json
{
    "body": "Changing keywords from \"\" to \"braid, Artin\".",
    "created_at": "2018-02-06T02:47:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468200",
    "user": "https://github.com/tscrim"
}
```

Changing keywords from "" to "braid, Artin".



---

archive/issue_comments_468201.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is a nice addition. \n\nHave you compared the speed of the new implementation of left normal form with the previous one for braids?",
    "created_at": "2018-02-06T09:11:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468201",
    "user": "https://github.com/miguelmarco"
}
```

<a id='comment:2'></a>
This is a nice addition. 

Have you compared the speed of the new implementation of left normal form with the previous one for braids?



---

archive/issue_comments_468202.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [mmarco](#comment%3A2):\n> This is a nice addition. \n\n\nThank you.\n\n> Have you compared the speed of the new implementation of left normal form with the previous one for braids?\n\n\nNo, I have not, but I think it should be the same since it is the same algorithm as before.",
    "created_at": "2018-02-06T09:25:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468202",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>
Replying to [mmarco](#comment%3A2):
> This is a nice addition. 


Thank you.

> Have you compared the speed of the new implementation of left normal form with the previous one for braids?


No, I have not, but I think it should be the same since it is the same algorithm as before.



---

archive/issue_comments_468203.json:
```json
{
    "body": "<a id='comment:4'></a>\nActually, there is a speed gain, but only because I am now using `simple_reflections`, which does not recreate the s<sub>i</sub> used in the algorithm:\n\n```\nsage: B = BraidGroup(5)\nsage: b = B([1,4,-3,-2,3,1,-4,2,1,2,1,2,3,-2,1,-2,-1,3,4,-3,-4])\nsage: %timeit b._left_normal_form_coxeter()\n100 loops, best of 3: 7.8 ms per loop\nsage: bp = B([1,4,-3])\nsage: %timeit bp._left_normal_form_coxeter()\n1000 loops, best of 3: 534 \u00b5s per loop\n```\nvs old\n\n```\nsage: B = BraidGroup(5)\nsage: b = B([1,4,-3,-2,3,1,-4,2,1,2,1,2,3,-2,1,-2,-1,3,4,-3,-4])\nsage: %timeit b._left_normal_form_perm_()\n100 loops, best of 3: 15.1 ms per loop\nsage: bp = B([1,4,-3])\nsage: %timeit bp._left_normal_form_perm_()\n1000 loops, best of 3: 609 \u00b5s per loop\n```",
    "created_at": "2018-02-07T00:07:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468203",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'></a>
Actually, there is a speed gain, but only because I am now using `simple_reflections`, which does not recreate the s<sub>i</sub> used in the algorithm:

```
sage: B = BraidGroup(5)
sage: b = B([1,4,-3,-2,3,1,-4,2,1,2,1,2,3,-2,1,-2,-1,3,4,-3,-4])
sage: %timeit b._left_normal_form_coxeter()
100 loops, best of 3: 7.8 ms per loop
sage: bp = B([1,4,-3])
sage: %timeit bp._left_normal_form_coxeter()
1000 loops, best of 3: 534 µs per loop
```
vs old

```
sage: B = BraidGroup(5)
sage: b = B([1,4,-3,-2,3,1,-4,2,1,2,1,2,3,-2,1,-2,-1,3,4,-3,-4])
sage: %timeit b._left_normal_form_perm_()
100 loops, best of 3: 15.1 ms per loop
sage: bp = B([1,4,-3])
sage: %timeit bp._left_normal_form_perm_()
1000 loops, best of 3: 609 µs per loop
```



---

archive/issue_comments_468204.json:
```json
{
    "body": "<a id='comment:5'></a>\nI think there is an error in the doctest of `ArtinGroup.coxeter_matrix()`. It claims to return the coxeter type, but it should be the coxeter matrix, right?",
    "created_at": "2018-02-07T14:22:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468204",
    "user": "https://github.com/miguelmarco"
}
```

<a id='comment:5'></a>
I think there is an error in the doctest of `ArtinGroup.coxeter_matrix()`. It claims to return the coxeter type, but it should be the coxeter matrix, right?



---

archive/issue_comments_468205.json:
```json
{
    "body": "<a id='comment:6'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                 |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------|\n|[97d235a](https://git.sagemath.org/sage.git/commit/?id=97d235a4c973ef20cea57a1766dec2200610e2f3)|`Fixing typo type -> matrix in coxeter_matrix().`|",
    "created_at": "2018-02-07T22:41:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468205",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                 |
|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------|
|[97d235a](https://git.sagemath.org/sage.git/commit/?id=97d235a4c973ef20cea57a1766dec2200610e2f3)|`Fixing typo type -> matrix in coxeter_matrix().`|



---

archive/issue_comments_468206.json:
```json
{
    "body": "Changing commit from \"43ea78e6b51bf93154b0425a3869739546750f24\" to \"97d235a4c973ef20cea57a1766dec2200610e2f3\"",
    "created_at": "2018-02-07T22:41:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468206",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "43ea78e6b51bf93154b0425a3869739546750f24" to "97d235a4c973ef20cea57a1766dec2200610e2f3"



---

archive/issue_comments_468207.json:
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [mmarco](#comment%3A5):\n> I think there is an error in the doctest of `ArtinGroup.coxeter_matrix()`. It claims to return the coxeter type, but it should be the coxeter matrix, right?\n\n\nYes, indeed there is an error in the docstring. Fixed.",
    "created_at": "2018-02-07T22:41:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468207",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:7'></a>
Replying to [mmarco](#comment%3A5):
> I think there is an error in the doctest of `ArtinGroup.coxeter_matrix()`. It claims to return the coxeter type, but it should be the coxeter matrix, right?


Yes, indeed there is an error in the docstring. Fixed.



---

archive/issue_comments_468208.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-02-08T08:29:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468208",
    "user": "https://github.com/miguelmarco"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_468209.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Miguel Marco\"",
    "created_at": "2018-02-08T08:29:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468209",
    "user": "https://github.com/miguelmarco"
}
```

Changing reviewer from "" to "Miguel Marco"



---

archive/issue_comments_468210.json:
```json
{
    "body": "Changing branch from \"public/groups/artin_groups-24664\" to \"97d235a4c973ef20cea57a1766dec2200610e2f3\"",
    "created_at": "2018-02-09T23:47:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468210",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/groups/artin_groups-24664" to "97d235a4c973ef20cea57a1766dec2200610e2f3"



---

archive/issue_comments_468211.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-02-09T23:47:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24664#issuecomment-468211",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_062097.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-09T23:47:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24664",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24664#event-62097"
}
```
