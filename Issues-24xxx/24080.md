# Issue 24080: Indexing for symbolic expressions

archive/issues_023843.json:
```json
{
    "assignees": [],
    "body": "Any expression has a number `>=0` of operands. For convenience this ticket implements indexing `ex[i]` as a shortcut for calling `ex.operands()[i]`.\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24080_\n\n",
    "closed_at": "2017-12-12T08:23:33Z",
    "created_at": "2017-10-20T14:55:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Indexing for symbolic expressions",
    "type": "issue",
    "updated_at": "2017-12-12T08:23:33Z",
    "url": "https://github.com/sagemath/sage/issues/24080",
    "user": "https://github.com/rwst"
}
```
Any expression has a number `>=0` of operands. For convenience this ticket implements indexing `ex[i]` as a shortcut for calling `ex.operands()[i]`.

_Issue created by migration from https://trac.sagemath.org/ticket/24080_





---

archive/issue_events_316217.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-20T14:55:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316217"
}
```



---

archive/issue_events_316218.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-20T14:55:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316218"
}
```



---

archive/issue_events_316219.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-20T14:55:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316219"
}
```



---

archive/issue_events_316220.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-20T14:55:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316220"
}
```



---

archive/issue_comments_366499.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Any expression has a number `>=0` of operands. For convenience this ticket implements indexing `ex[i]` as a shortcut for calling `ex.operands[i]`.\n+Any expression has a number `>=0` of operands. For convenience this ticket implements indexing `ex[i]` as a shortcut for calling `ex.operands()[i]`.\n``````\n",
    "created_at": "2017-10-20T14:56:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366499",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Any expression has a number `>=0` of operands. For convenience this ticket implements indexing `ex[i]` as a shortcut for calling `ex.operands[i]`.
+Any expression has a number `>=0` of operands. For convenience this ticket implements indexing `ex[i]` as a shortcut for calling `ex.operands()[i]`.
``````




---

archive/issue_comments_366500.json:
```json
{
    "body": "Branch: **[u/rws/indexing_for_symbolic_expressions](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/indexing_for_symbolic_expressions)**",
    "created_at": "2017-10-20T15:25:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366500",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/indexing_for_symbolic_expressions](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/indexing_for_symbolic_expressions)**



---

archive/issue_events_316221.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-20T15:26:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316221"
}
```



---

archive/issue_comments_366501.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c2039fd66b468d0d97ac1a51f4b87be4c7703b39\">c2039fd</a></td><td><code>24080: Indexing for symbolic expressions</code></td></tr></table>\n",
    "created_at": "2017-10-20T15:26:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366501",
    "user": "https://github.com/rwst"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c2039fd66b468d0d97ac1a51f4b87be4c7703b39">c2039fd</a></td><td><code>24080: Indexing for symbolic expressions</code></td></tr></table>




---

archive/issue_comments_366502.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2017-10-20T15:26:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366502",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_366503.json:
```json
{
    "body": "Commit: **[c2039fd](https://github.com/sagemath/sagetrac-mirror/commit/c2039fd66b468d0d97ac1a51f4b87be4c7703b39)**",
    "created_at": "2017-10-20T15:26:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366503",
    "user": "https://github.com/rwst"
}
```

Commit: **[c2039fd](https://github.com/sagemath/sagetrac-mirror/commit/c2039fd66b468d0d97ac1a51f4b87be4c7703b39)**



---

archive/issue_comments_366504.json:
```json
{
    "body": "Changed commit from **[c2039fd](https://github.com/sagemath/sagetrac-mirror/commit/c2039fd66b468d0d97ac1a51f4b87be4c7703b39)** to **[b9b6f22](https://github.com/sagemath/sagetrac-mirror/commit/b9b6f226cea6d98e63ed36e4e5ccfd6deb467ae0)**",
    "created_at": "2017-10-20T15:54:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366504",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c2039fd](https://github.com/sagemath/sagetrac-mirror/commit/c2039fd66b468d0d97ac1a51f4b87be4c7703b39)** to **[b9b6f22](https://github.com/sagemath/sagetrac-mirror/commit/b9b6f226cea6d98e63ed36e4e5ccfd6deb467ae0)**



---

archive/issue_comments_366505.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b9b6f226cea6d98e63ed36e4e5ccfd6deb467ae0\">b9b6f22</a></td><td><code>24080: fix doctest</code></td></tr></table>\n",
    "created_at": "2017-10-20T15:54:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366505",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b9b6f226cea6d98e63ed36e4e5ccfd6deb467ae0">b9b6f22</a></td><td><code>24080: fix doctest</code></td></tr></table>




---

archive/issue_comments_366506.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\n\n```\nsage -t --long --warn-long 46.5 src/sage/functions/hypergeometric.py  # 1 doctest failed\nsage -t --long --warn-long 46.5 src/sage/functions/min_max.py  # 2 doctests failed\nsage -t --long --warn-long 46.5 src/sage/rings/complex_double.pyx  # 3 doctests failed\nsage -t --long --warn-long 46.5 src/sage/calculus/interpolators.pyx  # 4 doctests failed\nsage -t --long --warn-long 46.5 src/sage/calculus/desolvers.py  # 2 doctests failed\nsage -t --long --warn-long 46.5 src/sage/structure/coerce.pyx  # 1 doctest failed\nsage -t --long --warn-long 46.5 src/sage/matrix/special.py  # 2 doctests failed\nsage -t --long --warn-long 46.5 src/sage/plot/graphics.py  # 2 doctests failed\nsage -t --long --warn-long 46.5 src/sage/plot/misc.py  # 1 doctest failed\nsage -t --long --warn-long 46.5 src/sage/plot/plot.py  # 4 doctests failed\nsage -t --long --warn-long 46.5 src/sage/plot/matrix_plot.py  # 1 doctest failed\nsage -t --long --warn-long 46.5 src/sage/symbolic/function.pyx  # 3 doctests failed\nsage -t --long --warn-long 46.5 src/sage/numerical/optimize.py  # 3 doctests failed\nsage -t --long --warn-long 46.5 src/doc/en/prep/Calculus.rst  # 1 doctest failed\n```",
    "created_at": "2017-10-21T05:12:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366506",
    "user": "https://github.com/rwst"
}
```

<a id='comment:5'>Comment 5:</a>

```
sage -t --long --warn-long 46.5 src/sage/functions/hypergeometric.py  # 1 doctest failed
sage -t --long --warn-long 46.5 src/sage/functions/min_max.py  # 2 doctests failed
sage -t --long --warn-long 46.5 src/sage/rings/complex_double.pyx  # 3 doctests failed
sage -t --long --warn-long 46.5 src/sage/calculus/interpolators.pyx  # 4 doctests failed
sage -t --long --warn-long 46.5 src/sage/calculus/desolvers.py  # 2 doctests failed
sage -t --long --warn-long 46.5 src/sage/structure/coerce.pyx  # 1 doctest failed
sage -t --long --warn-long 46.5 src/sage/matrix/special.py  # 2 doctests failed
sage -t --long --warn-long 46.5 src/sage/plot/graphics.py  # 2 doctests failed
sage -t --long --warn-long 46.5 src/sage/plot/misc.py  # 1 doctest failed
sage -t --long --warn-long 46.5 src/sage/plot/plot.py  # 4 doctests failed
sage -t --long --warn-long 46.5 src/sage/plot/matrix_plot.py  # 1 doctest failed
sage -t --long --warn-long 46.5 src/sage/symbolic/function.pyx  # 3 doctests failed
sage -t --long --warn-long 46.5 src/sage/numerical/optimize.py  # 3 doctests failed
sage -t --long --warn-long 46.5 src/doc/en/prep/Calculus.rst  # 1 doctest failed
```



---

archive/issue_events_316222.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-21T05:12:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316222"
}
```



---

archive/issue_events_316223.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-21T05:12:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316223"
}
```



---

archive/issue_comments_366507.json:
```json
{
    "body": "Changed commit from **[b9b6f22](https://github.com/sagemath/sagetrac-mirror/commit/b9b6f226cea6d98e63ed36e4e5ccfd6deb467ae0)** to none",
    "created_at": "2017-11-18T16:09:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366507",
    "user": "https://github.com/rwst"
}
```

Changed commit from **[b9b6f22](https://github.com/sagemath/sagetrac-mirror/commit/b9b6f226cea6d98e63ed36e4e5ccfd6deb467ae0)** to none



---

archive/issue_events_316224.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-18T16:09:31Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316224"
}
```



---

archive/issue_events_316225.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-18T16:09:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316225"
}
```



---

archive/issue_events_316226.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-18T16:09:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316226"
}
```



---

archive/issue_comments_366508.json:
```json
{
    "body": "Changed author from **Ralf Stephan** to none",
    "created_at": "2017-11-18T16:09:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366508",
    "user": "https://github.com/rwst"
}
```

Changed author from **Ralf Stephan** to none



---

archive/issue_comments_366509.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nI think now it's superfluous because `ex.op()` does the same and it's short enough.",
    "created_at": "2017-11-18T16:09:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366509",
    "user": "https://github.com/rwst"
}
```

<a id='comment:6'>Comment 6:</a>
I think now it's superfluous because `ex.op()` does the same and it's short enough.



---

archive/issue_events_316227.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-18T16:09:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316227"
}
```



---

archive/issue_events_316228.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-18T16:09:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316228"
}
```



---

archive/issue_comments_366510.json:
```json
{
    "body": "Changed branch from **[u/rws/indexing_for_symbolic_expressions](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/indexing_for_symbolic_expressions)** to none",
    "created_at": "2017-11-18T16:09:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24080#issuecomment-366510",
    "user": "https://github.com/rwst"
}
```

Changed branch from **[u/rws/indexing_for_symbolic_expressions](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/indexing_for_symbolic_expressions)** to none



---

archive/issue_events_316229.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316229"
}
```



---

archive/issue_events_316230.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316230"
}
```



---

archive/issue_events_316231.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24080",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24080#event-316231"
}
```
