# Issue 24906: "non-identical C method 'add_variable' not redeclared" warings building InteractiveLPBackend and  GurobiBackend

archive/issues_024669.json:
```json
{
    "assignees": [],
    "body": "\n```\n[sagelib-8.2.beta7] warning: sage/numerical/backends/gurobi_backend.pyx:90:10: Compatible but non-identical C method 'add_variable' not redeclared in definition part of extension type 'GurobiBackend'.  This may cause incorrect vtables to be generated.\n[sagelib-8.2.beta7] warning: sage/numerical/backends/generic_backend.pxd:13:26: Previous declaration is here\n[sagelib-8.2.beta7] [329/489] Cythonizing sage/numerical/backends/interactivelp_backend.pyx\n[sagelib-8.2.beta7] warning: sage/numerical/backends/interactivelp_backend.pyx:179:10: Compatible but non-identical C method 'add_variable' not redeclared in definition part of extension type 'InteractiveLPBackend'.  This may cause incorrect vtables to be generated.\n[sagelib-8.2.beta7] warning: sage/numerical/backends/generic_backend.pxd:13:26: Previous declaration is here\n```\n\nCC:  @mkoeppe\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24906_\n\n",
    "closed_at": "2018-03-13T08:42:39Z",
    "created_at": "2018-03-06T07:17:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "\"non-identical C method 'add_variable' not redeclared\" warings building InteractiveLPBackend and  GurobiBackend",
    "type": "issue",
    "updated_at": "2018-03-13T08:42:39Z",
    "url": "https://github.com/sagemath/sage/issues/24906",
    "user": "https://github.com/dimpase"
}
```

```
[sagelib-8.2.beta7] warning: sage/numerical/backends/gurobi_backend.pyx:90:10: Compatible but non-identical C method 'add_variable' not redeclared in definition part of extension type 'GurobiBackend'.  This may cause incorrect vtables to be generated.
[sagelib-8.2.beta7] warning: sage/numerical/backends/generic_backend.pxd:13:26: Previous declaration is here
[sagelib-8.2.beta7] [329/489] Cythonizing sage/numerical/backends/interactivelp_backend.pyx
[sagelib-8.2.beta7] warning: sage/numerical/backends/interactivelp_backend.pyx:179:10: Compatible but non-identical C method 'add_variable' not redeclared in definition part of extension type 'InteractiveLPBackend'.  This may cause incorrect vtables to be generated.
[sagelib-8.2.beta7] warning: sage/numerical/backends/generic_backend.pxd:13:26: Previous declaration is here
```

CC:  @mkoeppe

_Issue created by migration from https://trac.sagemath.org/ticket/24906_





---

archive/issue_events_326576.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-03-06T07:17:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326576"
}
```



---

archive/issue_events_326577.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-03-06T07:17:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
    "label_color": "0000ff",
    "label_name": "component: linear programming",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326577"
}
```



---

archive/issue_events_326578.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-03-06T07:17:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326578"
}
```



---

archive/issue_events_326579.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-03-06T07:17:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326579"
}
```



---

archive/issue_comments_381723.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nnot sure whether these can be ignored.",
    "created_at": "2018-03-06T07:18:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24906#issuecomment-381723",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:1" align="right">Comment 1</div>

not sure whether these can be ignored.



---

archive/issue_comments_381724.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis is more or less a duplicate of #20324. If you want, we could fix the Cython issue without fixing `add_variable` more generally. But #20324 has bigger plans for fixing `add_variable` which should also fix the Cython warnings.",
    "created_at": "2018-03-06T09:39:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24906#issuecomment-381724",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

This is more or less a duplicate of #20324. If you want, we could fix the Cython issue without fixing `add_variable` more generally. But #20324 has bigger plans for fixing `add_variable` which should also fix the Cython warnings.



---

archive/issue_comments_381725.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nDo you plan to fix this? If not, we might as well let it be and hope that somebody will address it in #20324.",
    "created_at": "2018-03-12T19:33:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24906#issuecomment-381725",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

Do you plan to fix this? If not, we might as well let it be and hope that somebody will address it in #20324.



---

archive/issue_comments_381726.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nmy hands are full a.t.m.",
    "created_at": "2018-03-13T01:12:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24906#issuecomment-381726",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:4" align="right">Comment 4</div>

my hands are full a.t.m.



---

archive/issue_events_326580.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-03-13T01:12:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326580"
}
```



---

archive/issue_events_326581.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-03-13T01:12:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326581"
}
```



---

archive/issue_events_326582.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T08:42:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326582"
}
```



---

archive/issue_events_326583.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T08:42:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326583"
}
```



---

archive/issue_events_326584.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T08:42:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326584"
}
```



---

archive/issue_events_326585.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T08:42:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326585"
}
```



---

archive/issue_events_326586.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T08:42:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24906",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24906#event-326586"
}
```
