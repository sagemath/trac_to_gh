# Issue 24532: Replace lcalc by PARI

archive/issues_024295.json:
```json
{
    "assignees": [],
    "body": "\n\nCC:  @embray\n\nBranch/Commit: **[u/jdemeyer/replace_lcalc_by_pari](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/replace_lcalc_by_pari) @ [57769fa](https://github.com/sagemath/sagetrac-mirror/commit/57769fae12341d2c73ec59efe2314f1f5e4cf0cd)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24532_\n\n",
    "closed_at": "2018-01-15T16:30:40Z",
    "created_at": "2018-01-12T15:02:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Replace lcalc by PARI",
    "type": "issue",
    "updated_at": "2018-01-16T18:33:19Z",
    "url": "https://github.com/sagemath/sage/issues/24532",
    "user": "https://github.com/jdemeyer"
}
```


CC:  @embray

Branch/Commit: **[u/jdemeyer/replace_lcalc_by_pari](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/replace_lcalc_by_pari) @ [57769fa](https://github.com/sagemath/sagetrac-mirror/commit/57769fae12341d2c73ec59efe2314f1f5e4cf0cd)**

_Issue created by migration from https://trac.sagemath.org/ticket/24532_





---

archive/issue_events_321778.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-12T15:02:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24532#event-321778"
}
```



---

archive/issue_events_321779.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-12T15:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24532#event-321779"
}
```



---

archive/issue_events_321780.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-12T15:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24532#event-321780"
}
```



---

archive/issue_events_321781.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-12T15:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24532#event-321781"
}
```



---

archive/issue_comments_374160.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nTests are showing that (at least for some use cases) PARI is slower than lcalc. Part of the reason is that lcalc has optimizations for specific L-functions while PARI works in much larger generality.\n\nSo replacing lcalc is a bad idea.",
    "created_at": "2018-01-15T16:30:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24532#issuecomment-374160",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

Tests are showing that (at least for some use cases) PARI is slower than lcalc. Part of the reason is that lcalc has optimizations for specific L-functions while PARI works in much larger generality.

So replacing lcalc is a bad idea.



---

archive/issue_events_321782.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-15T16:30:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24532#event-321782"
}
```



---

archive/issue_events_321783.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-15T16:30:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24532#event-321783"
}
```



---

archive/issue_events_321784.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-15T16:30:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24532#event-321784"
}
```



---

archive/issue_events_321785.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-15T16:30:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24532#event-321785"
}
```



---

archive/issue_comments_374161.json:
```json
{
    "body": "Branch: **[u/jdemeyer/replace_lcalc_by_pari](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/replace_lcalc_by_pari)**",
    "created_at": "2018-01-15T16:30:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24532#issuecomment-374161",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/replace_lcalc_by_pari](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/replace_lcalc_by_pari)**



---

archive/issue_comments_374162.json:
```json
{
    "body": "Commit: **[57769fa](https://github.com/sagemath/sagetrac-mirror/commit/57769fae12341d2c73ec59efe2314f1f5e4cf0cd)**",
    "created_at": "2018-01-15T16:31:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24532#issuecomment-374162",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[57769fa](https://github.com/sagemath/sagetrac-mirror/commit/57769fae12341d2c73ec59efe2314f1f5e4cf0cd)**



---

archive/issue_comments_374163.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI'm still pushing the branch in case somebody wants to play with it.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/57769fae12341d2c73ec59efe2314f1f5e4cf0cd\">57769fa</a></td><td><code>Deprecate lcalc; use PARI instead</code></td></tr></table>\n",
    "created_at": "2018-01-15T16:31:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24532#issuecomment-374163",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

I'm still pushing the branch in case somebody wants to play with it.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/57769fae12341d2c73ec59efe2314f1f5e4cf0cd">57769fa</a></td><td><code>Deprecate lcalc; use PARI instead</code></td></tr></table>




---

archive/issue_comments_374164.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThat's a shame. `lcalc` will probably become a higher and higher maintenance cost in the future - unless upstream `lcalc` pick up maintenance again.",
    "created_at": "2018-01-15T19:30:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24532#issuecomment-374164",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:5" align="right">Comment 5</div>

That's a shame. `lcalc` will probably become a higher and higher maintenance cost in the future - unless upstream `lcalc` pick up maintenance again.



---

archive/issue_comments_374165.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI don't know how lcalc works at all, but maybe said optimizations can be ported upstream to PARI?",
    "created_at": "2018-01-16T16:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24532#issuecomment-374165",
    "user": "https://github.com/embray"
}
```

<div id="comment:6" align="right">Comment 6</div>

I don't know how lcalc works at all, but maybe said optimizations can be ported upstream to PARI?



---

archive/issue_comments_374166.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nlcalc is not a trivial package :-) The PARI developers are aware of lcalc and what it does. So, if it would be easy, they would already have done it.",
    "created_at": "2018-01-16T18:33:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24532#issuecomment-374166",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">Comment 7</div>

lcalc is not a trivial package :-) The PARI developers are aware of lcalc and what it does. So, if it would be easy, they would already have done it.
