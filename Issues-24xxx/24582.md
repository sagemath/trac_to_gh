# Issue 24582: py3: fixes to sage.structure.sage_object

archive/issues_024345.json:
```json
{
    "body": "Along with some related fixes to `sage.misc.explain_pickle` (which otherwise still has many failures on Python 3 due mostly to string/bytes differences).\n\nMost of these fixes have to do with pickling, especially minor differences in the API of the `pickle` module.\n\nCC:  @jdemeyer\n\nBranch/Commit: d0f4a71867a8acf5056f188192a3633b607c9347\n\nReviewer: Fr\u00e9d\u00e9ric Chapoton\n\nAuthor: Erik Bray\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/24582\n\n",
    "closed_at": "2018-06-29T22:34:05Z",
    "created_at": "2018-01-22T13:12:40Z",
    "labels": [
        "component: python3",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.3",
    "title": "py3: fixes to sage.structure.sage_object",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24582",
    "user": "https://github.com/embray"
}
```
Along with some related fixes to `sage.misc.explain_pickle` (which otherwise still has many failures on Python 3 due mostly to string/bytes differences).

Most of these fixes have to do with pickling, especially minor differences in the API of the `pickle` module.

CC:  @jdemeyer

Branch/Commit: d0f4a71867a8acf5056f188192a3633b607c9347

Reviewer: Frédéric Chapoton

Author: Erik Bray

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/24582





---

archive/issue_comments_465729.json:
```json
{
    "body": "Changing commit from \"\" to \"b9bec315533c5a7a7d600090c21699310a1a78ca\"",
    "created_at": "2018-01-22T13:12:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465729",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "" to "b9bec315533c5a7a7d600090c21699310a1a78ca"



---

archive/issue_comments_465730.json:
```json
{
    "body": "<a id='comment:1'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                 |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|\n|[cdb7cdf](https://git.sagemath.org/sage.git/commit/?id=cdb7cdffa417c27bec8cbc2a2efa91d59c8ba42a)|`Some pickling/unpickling fixes:`|\n|[5c16381](https://git.sagemath.org/sage.git/commit/?id=5c163810ec7a4dcfe4bcf51cb5df14bc4b31ac4f)|`Fix several tests in this module.`|\n|[3be9d86](https://git.sagemath.org/sage.git/commit/?id=3be9d8687e71e3a5e47b53b4d3ee7d96a2417884)|`miscellaneous cleanup; some PEP-8`|\n|[b9bec31](https://git.sagemath.org/sage.git/commit/?id=b9bec315533c5a7a7d600090c21699310a1a78ca)|`SageUnpickler didn't work for Python since cPickle.Unpickle is not a class`|",
    "created_at": "2018-01-22T13:12:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465730",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:1'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                 |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|
|[cdb7cdf](https://git.sagemath.org/sage.git/commit/?id=cdb7cdffa417c27bec8cbc2a2efa91d59c8ba42a)|`Some pickling/unpickling fixes:`|
|[5c16381](https://git.sagemath.org/sage.git/commit/?id=5c163810ec7a4dcfe4bcf51cb5df14bc4b31ac4f)|`Fix several tests in this module.`|
|[3be9d86](https://git.sagemath.org/sage.git/commit/?id=3be9d8687e71e3a5e47b53b4d3ee7d96a2417884)|`miscellaneous cleanup; some PEP-8`|
|[b9bec31](https://git.sagemath.org/sage.git/commit/?id=b9bec315533c5a7a7d600090c21699310a1a78ca)|`SageUnpickler didn't work for Python since cPickle.Unpickle is not a class`|



---

archive/issue_comments_465731.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-01-22T13:14:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465731",
    "user": "https://github.com/embray"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_465732.json:
```json
{
    "body": "<a id='comment:4'></a>\nI haven't looked in detail, but I wonder if `SageUnpickler` could be written to work for both Python 2 and Python 3.",
    "created_at": "2018-02-13T19:35:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465732",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>
I haven't looked in detail, but I wonder if `SageUnpickler` could be written to work for both Python 2 and Python 3.



---

archive/issue_comments_465733.json:
```json
{
    "body": "<a id='comment:5'></a>\ngreen bot",
    "created_at": "2018-02-14T08:47:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465733",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'></a>
green bot



---

archive/issue_comments_465734.json:
```json
{
    "body": "<a id='comment:6'></a>\nReplying to [jdemeyer](#comment%3A4):\n> I haven't looked in detail, but I wonder if `SageUnpickler` could be written to work for both Python 2 and Python 3.\n\n\n*probably* not, since in Python 2 the `cPickle.Unpickler` class has some oddities, and in particular its `find_globals` attribute has some strange behavior.  But I agree it might be worth a try...",
    "created_at": "2018-02-14T15:46:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465734",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'></a>
Replying to [jdemeyer](#comment%3A4):
> I haven't looked in detail, but I wonder if `SageUnpickler` could be written to work for both Python 2 and Python 3.


*probably* not, since in Python 2 the `cPickle.Unpickler` class has some oddities, and in particular its `find_globals` attribute has some strange behavior.  But I agree it might be worth a try...



---

archive/issue_comments_465735.json:
```json
{
    "body": "<a id='comment:7'></a>\nOh I remember now, the only reason the tests for `SageUnpickler` even work on Python 2 is that it's subclassing the pure Python implementation in that case, which is less strange but also slower.\n\nBut perhaps I could make a single `SageUnpickler` class that delegates to the preferred underlying implementation.",
    "created_at": "2018-02-14T16:05:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465735",
    "user": "https://github.com/embray"
}
```

<a id='comment:7'></a>
Oh I remember now, the only reason the tests for `SageUnpickler` even work on Python 2 is that it's subclassing the pure Python implementation in that case, which is less strange but also slower.

But perhaps I could make a single `SageUnpickler` class that delegates to the preferred underlying implementation.



---

archive/issue_comments_465736.json:
```json
{
    "body": "<a id='comment:8'></a>\nMaybe it is time to start moving forward here again ?",
    "created_at": "2018-03-11T12:38:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465736",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'></a>
Maybe it is time to start moving forward here again ?



---

archive/issue_comments_465737.json:
```json
{
    "body": "<a id='comment:9'></a>\nIn python3-sage, trying to run `./sage -t src/sage/combinat/necklace.py`:\n\n```\nFile \"src/sage/combinat/necklace.py\", line 104, in sage.combinat.necklace.Necklaces_evaluation.__init__\nFailed example:\n    N == loads(dumps(N))\nException raised:\n    Traceback (most recent call last):\n      File \"sage/structure/sage_object.pyx\", line 1456, in sage.structure.sage_object.unpickle_global (build/cythonized/sage/structure/sage_object.c:15713)\n    ModuleNotFoundError: No module named '__builtin__'\n\n    During handling of the above exception, another exception occurred:\n\n    Traceback (most recent call last):\n      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py\", line 548, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py\", line 958, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.combinat.necklace.Necklaces_evaluation.__init__[1]>\", line 1, in <module>\n        N == loads(dumps(N))\n      File \"sage/structure/sage_object.pyx\", line 1557, in sage.structure.sage_object.loads (build/cythonized/sage/structure/sage_object.c:16726)\n      File \"sage/structure/sage_object.pyx\", line 1504, in sage.structure.sage_object.SageUnpickler.find_class (build/cythonized/sage/structure/sage_object.c:15991)\n      File \"sage/structure/sage_object.pyx\", line 1458, in sage.structure.sage_object.unpickle_global (build/cythonized/sage/structure/sage_object.c:15759)\n      File \"sage/structure/sage_object.pyx\", line 1445, in sage.structure.sage_object.unpickle_global.error (build/cythonized/sage/structure/sage_object.c:15321)\n    ImportError: cannot import getattr from __builtin__, call register_unpickle_override('__builtin__', 'getattr', ...) to fix this\n```",
    "created_at": "2018-03-11T12:40:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465737",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'></a>
In python3-sage, trying to run `./sage -t src/sage/combinat/necklace.py`:

```
File "src/sage/combinat/necklace.py", line 104, in sage.combinat.necklace.Necklaces_evaluation.__init__
Failed example:
    N == loads(dumps(N))
Exception raised:
    Traceback (most recent call last):
      File "sage/structure/sage_object.pyx", line 1456, in sage.structure.sage_object.unpickle_global (build/cythonized/sage/structure/sage_object.c:15713)
    ModuleNotFoundError: No module named '__builtin__'

    During handling of the above exception, another exception occurred:

    Traceback (most recent call last):
      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py", line 548, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py", line 958, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.combinat.necklace.Necklaces_evaluation.__init__[1]>", line 1, in <module>
        N == loads(dumps(N))
      File "sage/structure/sage_object.pyx", line 1557, in sage.structure.sage_object.loads (build/cythonized/sage/structure/sage_object.c:16726)
      File "sage/structure/sage_object.pyx", line 1504, in sage.structure.sage_object.SageUnpickler.find_class (build/cythonized/sage/structure/sage_object.c:15991)
      File "sage/structure/sage_object.pyx", line 1458, in sage.structure.sage_object.unpickle_global (build/cythonized/sage/structure/sage_object.c:15759)
      File "sage/structure/sage_object.pyx", line 1445, in sage.structure.sage_object.unpickle_global.error (build/cythonized/sage/structure/sage_object.c:15321)
    ImportError: cannot import getattr from __builtin__, call register_unpickle_override('__builtin__', 'getattr', ...) to fix this
```



---

archive/issue_comments_465738.json:
```json
{
    "body": "<a id='comment:10'></a>\nI think I fixed that issue, but I forget what the cause was (I don't think it's related to this ticket).\n\nI started working on a `SageUnpickler` class that could work on Python 2 and 3 but it was a bit tricky, and I got sidetracked.  I'll take another shot at it though--it's certainly doable.",
    "created_at": "2018-03-12T14:08:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465738",
    "user": "https://github.com/embray"
}
```

<a id='comment:10'></a>
I think I fixed that issue, but I forget what the cause was (I don't think it's related to this ticket).

I started working on a `SageUnpickler` class that could work on Python 2 and 3 but it was a bit tricky, and I got sidetracked.  I'll take another shot at it though--it's certainly doable.



---

archive/issue_comments_465739.json:
```json
{
    "body": "<a id='comment:11'></a>\nReplying to [embray](#comment%3A10):\n> I think I fixed that issue, but I forget what the cause was (I don't think it's related to this ticket).\n\n\nI remember now. It *is* related to this.  On Python 3 we generally want to pass `fix_imports=False`.  This is a trick that makes pickles created on Python 3 loadable on Python 2.  Problem is, then it *isn't* loadable on Python 3.  So at least by default we want to disable that (though we may want to accept it as an option on Python 3...)",
    "created_at": "2018-03-12T14:13:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465739",
    "user": "https://github.com/embray"
}
```

<a id='comment:11'></a>
Replying to [embray](#comment%3A10):
> I think I fixed that issue, but I forget what the cause was (I don't think it's related to this ticket).


I remember now. It *is* related to this.  On Python 3 we generally want to pass `fix_imports=False`.  This is a trick that makes pickles created on Python 3 loadable on Python 2.  Problem is, then it *isn't* loadable on Python 3.  So at least by default we want to disable that (though we may want to accept it as an option on Python 3...)



---

archive/issue_comments_465740.json:
```json
{
    "body": "<a id='comment:12'></a>\nThat, or we just use the most recent pickle protocol on Python 3--if a pickle only needs to be loadable on Python 3 then there's no reason to use protocol version=2.  But maybe we should add an option on Sage's `dumps()` option to make pickles compatible with Python 2, in which case it would use `protocol=2` and `fix_imports=True`.",
    "created_at": "2018-03-12T14:15:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465740",
    "user": "https://github.com/embray"
}
```

<a id='comment:12'></a>
That, or we just use the most recent pickle protocol on Python 3--if a pickle only needs to be loadable on Python 3 then there's no reason to use protocol version=2.  But maybe we should add an option on Sage's `dumps()` option to make pickles compatible with Python 2, in which case it would use `protocol=2` and `fix_imports=True`.



---

archive/issue_comments_465741.json:
```json
{
    "body": "<a id='comment:13'></a>\nAlso, is there a reason the pickle-related functions live in `sage.structure.sage_object`?  Although the `SageObject` class has `.dumps()` and `.loads()` methods those are just wrappers for the `dumps()` and `loads()` functions.\n\nISTM the pickle-specific utilities should live elsewhere.",
    "created_at": "2018-03-12T14:21:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465741",
    "user": "https://github.com/embray"
}
```

<a id='comment:13'></a>
Also, is there a reason the pickle-related functions live in `sage.structure.sage_object`?  Although the `SageObject` class has `.dumps()` and `.loads()` methods those are just wrappers for the `dumps()` and `loads()` functions.

ISTM the pickle-specific utilities should live elsewhere.



---

archive/issue_comments_465742.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#25153\"",
    "created_at": "2018-04-12T15:27:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465742",
    "user": "https://github.com/embray"
}
```

Changing dependencies from "" to "#25153"



---

archive/issue_comments_465743.json:
```json
{
    "body": "<a id='comment:14'></a>\nFurther work on this should probably incorporate #25153, if that ticket is accepted.",
    "created_at": "2018-04-12T15:27:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465743",
    "user": "https://github.com/embray"
}
```

<a id='comment:14'></a>
Further work on this should probably incorporate #25153, if that ticket is accepted.



---

archive/issue_events_061964.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24582#event-61964"
}
```



---

archive/issue_comments_465744.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-06-04T06:49:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465744",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_465745.json:
```json
{
    "body": "<a id='comment:16'></a>\nbranch does not apply",
    "created_at": "2018-06-04T06:49:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465745",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:16'></a>
branch does not apply



---

archive/issue_comments_465746.json:
```json
{
    "body": "Changing dependencies from \"#25153\" to \"\"",
    "created_at": "2018-06-11T18:12:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465746",
    "user": "https://github.com/fchapoton"
}
```

Changing dependencies from "#25153" to ""



---

archive/issue_comments_465747.json:
```json
{
    "body": "<a id='comment:18'></a>\nMaybe this one should be given a rather high priority, no ?\n\nI see many pickling failures in the python3 doctests.",
    "created_at": "2018-06-20T18:25:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465747",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:18'></a>
Maybe this one should be given a rather high priority, no ?

I see many pickling failures in the python3 doctests.



---

archive/issue_comments_465748.json:
```json
{
    "body": "<a id='comment:19'></a>\nYes, I keep picking up work on this at like...the end of the day, getting distracted by something else, and forgetting it again.  I need to re-implement the `SageUnpickler` class to work on both Python 2 and 3, which is not hard, I just...have to do it.\n\nNow that #25153 is closed that helps too.",
    "created_at": "2018-06-21T12:57:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465748",
    "user": "https://github.com/embray"
}
```

<a id='comment:19'></a>
Yes, I keep picking up work on this at like...the end of the day, getting distracted by something else, and forgetting it again.  I need to re-implement the `SageUnpickler` class to work on both Python 2 and 3, which is not hard, I just...have to do it.

Now that #25153 is closed that helps too.



---

archive/issue_comments_465749.json:
```json
{
    "body": "Changing commit from \"b9bec315533c5a7a7d600090c21699310a1a78ca\" to \"bbcd4b2be8e533da0ecf7e89f30921b8184e1e94\"",
    "created_at": "2018-06-22T09:10:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465749",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "b9bec315533c5a7a7d600090c21699310a1a78ca" to "bbcd4b2be8e533da0ecf7e89f30921b8184e1e94"



---

archive/issue_comments_465750.json:
```json
{
    "body": "<a id='comment:20'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                                 |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|\n|[9a1e512](https://git.sagemath.org/sage.git/commit/?id=9a1e51214c3b3e57193b4d266a93c183cb22a558)|`Some pickling/unpickling fixes:`|\n|[1f7bab6](https://git.sagemath.org/sage.git/commit/?id=1f7bab6ada6cd7bc9f544675d6edefa055c354da)|`Fix several tests in this module.`|\n|[dee1576](https://git.sagemath.org/sage.git/commit/?id=dee1576f8e3f531a86665e5bc63ffd4a94de12fe)|`miscellaneous cleanup; some PEP-8`|\n|[bbcd4b2](https://git.sagemath.org/sage.git/commit/?id=bbcd4b2be8e533da0ecf7e89f30921b8184e1e94)|`SageUnpickler didn't work for Python since cPickle.Unpickle is not a class`|",
    "created_at": "2018-06-22T09:10:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465750",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:20'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                                 |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|
|[9a1e512](https://git.sagemath.org/sage.git/commit/?id=9a1e51214c3b3e57193b4d266a93c183cb22a558)|`Some pickling/unpickling fixes:`|
|[1f7bab6](https://git.sagemath.org/sage.git/commit/?id=1f7bab6ada6cd7bc9f544675d6edefa055c354da)|`Fix several tests in this module.`|
|[dee1576](https://git.sagemath.org/sage.git/commit/?id=dee1576f8e3f531a86665e5bc63ffd4a94de12fe)|`miscellaneous cleanup; some PEP-8`|
|[bbcd4b2](https://git.sagemath.org/sage.git/commit/?id=bbcd4b2be8e533da0ecf7e89f30921b8184e1e94)|`SageUnpickler didn't work for Python since cPickle.Unpickle is not a class`|



---

archive/issue_comments_465751.json:
```json
{
    "body": "<a id='comment:21'></a>\nSo far just rebased on #25153.  Going to look into refactoring `SageUnpickler` now.",
    "created_at": "2018-06-22T09:11:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465751",
    "user": "https://github.com/embray"
}
```

<a id='comment:21'></a>
So far just rebased on #25153.  Going to look into refactoring `SageUnpickler` now.



---

archive/issue_comments_465752.json:
```json
{
    "body": "Changing commit from \"bbcd4b2be8e533da0ecf7e89f30921b8184e1e94\" to \"1bbf4573b61fdd034db9a9b4276153817f58d377\"",
    "created_at": "2018-06-22T09:44:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465752",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "bbcd4b2be8e533da0ecf7e89f30921b8184e1e94" to "1bbf4573b61fdd034db9a9b4276153817f58d377"



---

archive/issue_comments_465753.json:
```json
{
    "body": "<a id='comment:22'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                                 |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|\n|[6873576](https://git.sagemath.org/sage.git/commit/?id=687357681bd019c0a08662b17f606837ba315222)|`Some pickling/unpickling fixes:`|\n|[deaab79](https://git.sagemath.org/sage.git/commit/?id=deaab79198c0ef3a82095a48351a9a500d8466b6)|`Fix several tests in this module.`|\n|[faf0c86](https://git.sagemath.org/sage.git/commit/?id=faf0c86642ca4dfe01f58449ce86e26ab580845a)|`miscellaneous cleanup; some PEP-8`|\n|[1bbf457](https://git.sagemath.org/sage.git/commit/?id=1bbf4573b61fdd034db9a9b4276153817f58d377)|`SageUnpickler didn't work for Python since cPickle.Unpickle is not a class`|",
    "created_at": "2018-06-22T09:44:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465753",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:22'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                                 |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|
|[6873576](https://git.sagemath.org/sage.git/commit/?id=687357681bd019c0a08662b17f606837ba315222)|`Some pickling/unpickling fixes:`|
|[deaab79](https://git.sagemath.org/sage.git/commit/?id=deaab79198c0ef3a82095a48351a9a500d8466b6)|`Fix several tests in this module.`|
|[faf0c86](https://git.sagemath.org/sage.git/commit/?id=faf0c86642ca4dfe01f58449ce86e26ab580845a)|`miscellaneous cleanup; some PEP-8`|
|[1bbf457](https://git.sagemath.org/sage.git/commit/?id=1bbf4573b61fdd034db9a9b4276153817f58d377)|`SageUnpickler didn't work for Python since cPickle.Unpickle is not a class`|



---

archive/issue_comments_465754.json:
```json
{
    "body": "<a id='comment:23'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                                   |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------|\n|[25321fc](https://git.sagemath.org/sage.git/commit/?id=25321fc59e3bec97c0727d59c854211e10ce9b56)|`miscellaneous cleanup; some PEP-8`|\n|[90c8361](https://git.sagemath.org/sage.git/commit/?id=90c8361ace22f95c50edc42a78b2faa37662fa62)|`SageUnpickler didn't work for Python since cPickle.Unpickle is not a class`|\n|[615b75e](https://git.sagemath.org/sage.git/commit/?id=615b75e567508e54f343d0e63bfeb3f82aa89a97)|`Repackaged Sage-specific pickling/unpickling behavior in SagePickle and`|\n|[0014f29](https://git.sagemath.org/sage.git/commit/?id=0014f29a346244db089f9ce9fb2b486d4c0f94d3)|`on Python 2 the return values of these methods is the Pickler object itself which, while useful for chaining`|",
    "created_at": "2018-06-22T13:55:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465754",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:23'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                                   |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------|
|[25321fc](https://git.sagemath.org/sage.git/commit/?id=25321fc59e3bec97c0727d59c854211e10ce9b56)|`miscellaneous cleanup; some PEP-8`|
|[90c8361](https://git.sagemath.org/sage.git/commit/?id=90c8361ace22f95c50edc42a78b2faa37662fa62)|`SageUnpickler didn't work for Python since cPickle.Unpickle is not a class`|
|[615b75e](https://git.sagemath.org/sage.git/commit/?id=615b75e567508e54f343d0e63bfeb3f82aa89a97)|`Repackaged Sage-specific pickling/unpickling behavior in SagePickle and`|
|[0014f29](https://git.sagemath.org/sage.git/commit/?id=0014f29a346244db089f9ce9fb2b486d4c0f94d3)|`on Python 2 the return values of these methods is the Pickler object itself which, while useful for chaining`|



---

archive/issue_comments_465755.json:
```json
{
    "body": "Changing commit from \"1bbf4573b61fdd034db9a9b4276153817f58d377\" to \"0014f29a346244db089f9ce9fb2b486d4c0f94d3\"",
    "created_at": "2018-06-22T13:55:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465755",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "1bbf4573b61fdd034db9a9b4276153817f58d377" to "0014f29a346244db089f9ce9fb2b486d4c0f94d3"



---

archive/issue_comments_465756.json:
```json
{
    "body": "<a id='comment:24'></a>\nOk, I reworked `SageUnpickler` so that it works on both Python 2 and 3 (most of the Python-specific differences are pushed down to a `_BaseUnpickler` class that bracketed by an `IF PY_MAJOR_VERSION == 2` statement in the Cython code).\n\nFor symmetry's sake I also added a `SagePickler` class which encapsulates the default pickling behavior we want (for now, always use pickle protocol 2, with `fix_imports=True` on Python 3).  The intent is to have the best possible pickle compatibility between Python 2 and 3.  This is hard to test though--to do so we might have to make a few sample pickles (enough to test things like import fixups and str/bytes issues but not as many as comprehensive as the old picklejar) on both Python 2 and 3 and ensure that they can be loaded.",
    "created_at": "2018-06-22T14:01:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465756",
    "user": "https://github.com/embray"
}
```

<a id='comment:24'></a>
Ok, I reworked `SageUnpickler` so that it works on both Python 2 and 3 (most of the Python-specific differences are pushed down to a `_BaseUnpickler` class that bracketed by an `IF PY_MAJOR_VERSION == 2` statement in the Cython code).

For symmetry's sake I also added a `SagePickler` class which encapsulates the default pickling behavior we want (for now, always use pickle protocol 2, with `fix_imports=True` on Python 3).  The intent is to have the best possible pickle compatibility between Python 2 and 3.  This is hard to test though--to do so we might have to make a few sample pickles (enough to test things like import fixups and str/bytes issues but not as many as comprehensive as the old picklejar) on both Python 2 and 3 and ensure that they can be loaded.



---

archive/issue_comments_465757.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-06-22T14:01:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465757",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_465758.json:
```json
{
    "body": "<a id='comment:25'></a>\ndoc does not build\n\n```\ndocstring of sage.misc.persist.SageUnpickler:21: WARNING: Inline interpreted text or phrase reference start-string without end-string.\n```\n\nmaybe because of\n\n```\n+      `pickle.Unpickler constructor.\n```",
    "created_at": "2018-06-25T08:45:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465758",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:25'></a>
doc does not build

```
docstring of sage.misc.persist.SageUnpickler:21: WARNING: Inline interpreted text or phrase reference start-string without end-string.
```

maybe because of

```
+      `pickle.Unpickler constructor.
```



---

archive/issue_comments_465759.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-06-25T08:45:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465759",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_465760.json:
```json
{
    "body": "<a id='comment:26'></a>\nI was afraid of that.  I need to get better about testing the doc build when I add any significant quantity of docstrings...",
    "created_at": "2018-06-25T09:16:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465760",
    "user": "https://github.com/embray"
}
```

<a id='comment:26'></a>
I was afraid of that.  I need to get better about testing the doc build when I add any significant quantity of docstrings...



---

archive/issue_comments_465761.json:
```json
{
    "body": "<a id='comment:27'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                                                                                                              |\n|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|\n|[259ea80](https://git.sagemath.org/sage.git/commit/?id=259ea80aa5a5c6c9f9d3a6fb9f8ad5e901155f3f)|`on Python 2 the return values of these methods is the Pickler object itself which, while useful for chaining`|",
    "created_at": "2018-06-25T09:21:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465761",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:27'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                                                                                                              |
|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
|[259ea80](https://git.sagemath.org/sage.git/commit/?id=259ea80aa5a5c6c9f9d3a6fb9f8ad5e901155f3f)|`on Python 2 the return values of these methods is the Pickler object itself which, while useful for chaining`|



---

archive/issue_comments_465762.json:
```json
{
    "body": "Changing commit from \"0014f29a346244db089f9ce9fb2b486d4c0f94d3\" to \"259ea80aa5a5c6c9f9d3a6fb9f8ad5e901155f3f\"",
    "created_at": "2018-06-25T09:21:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465762",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "0014f29a346244db089f9ce9fb2b486d4c0f94d3" to "259ea80aa5a5c6c9f9d3a6fb9f8ad5e901155f3f"



---

archive/issue_comments_465763.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-06-25T10:03:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465763",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_465764.json:
```json
{
    "body": "<a id='comment:28'></a>\nDocs work now.",
    "created_at": "2018-06-25T10:03:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465764",
    "user": "https://github.com/embray"
}
```

<a id='comment:28'></a>
Docs work now.



---

archive/issue_comments_465765.json:
```json
{
    "body": "<a id='comment:29'></a>\npyflakes plugin is not happy",
    "created_at": "2018-06-25T12:57:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465765",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:29'></a>
pyflakes plugin is not happy



---

archive/issue_comments_465766.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-06-25T14:54:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465766",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_465767.json:
```json
{
    "body": "Changing commit from \"259ea80aa5a5c6c9f9d3a6fb9f8ad5e901155f3f\" to \"94f12b4b87e80088fee0d2b4408932e7887c4148\"",
    "created_at": "2018-06-25T15:43:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465767",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "259ea80aa5a5c6c9f9d3a6fb9f8ad5e901155f3f" to "94f12b4b87e80088fee0d2b4408932e7887c4148"



---

archive/issue_comments_465768.json:
```json
{
    "body": "<a id='comment:31'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                       |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|\n|[94f12b4](https://git.sagemath.org/sage.git/commit/?id=94f12b4b87e80088fee0d2b4408932e7887c4148)|`removed unused import`|",
    "created_at": "2018-06-25T15:43:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465768",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:31'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                       |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|
|[94f12b4](https://git.sagemath.org/sage.git/commit/?id=94f12b4b87e80088fee0d2b4408932e7887c4148)|`removed unused import`|



---

archive/issue_comments_465769.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-06-25T15:44:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465769",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_465770.json:
```json
{
    "body": "<a id='comment:33'></a>\nDo we know if this fixes some/most/all of the python3 pickling issues ?",
    "created_at": "2018-06-26T07:19:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465770",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:33'></a>
Do we know if this fixes some/most/all of the python3 pickling issues ?



---

archive/issue_comments_465771.json:
```json
{
    "body": "<a id='comment:34'></a>\nThis does not fix the pickle issue with python 3, though the error changes. \nMy touchstone file is `./sage -t src/sage/combinat/interval_posets.py`\nand there:\n\n```\n   Failure in _test_elements\n    The following tests failed: _test_elements\n      Failure in _test_pickling:\n      Traceback (most recent call last):\n        File \"sage/misc/persist.pyx\", line 540, in sage.misc.persist.unpickle_global (build/cythonized/sage/misc/persist.c:5187)\n          __import__(module)\n      ModuleNotFoundError: No module named '__builtin__'\n    <BLANKLINE>\n      During handling of the above exception, another exception occurred:\n    <BLANKLINE>\n      Traceback (most recent call last):\n        File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/misc/sage_unittest.py\", line 294, in run\n          test_method(tester = tester)\n        File \"sage/structure/sage_object.pyx\", line 683, in sage.structure.sage_object.SageObject._test_pickling (build/cythonized/sage/structure/sage_object.c:5024)\n          tester.assertEqual(loads(dumps(self)), self)\n        File \"sage/misc/persist.pyx\", line 960, in sage.misc.persist.loads (build/cythonized/sage/misc/persist.c:7648)\n          return unpickler.load()\n        File \"sage/misc/persist.pyx\", line 736, in sage.misc.persist._BaseUnpickler.find_class (build/cythonized/sage/misc/persist.c:6386)\n          return unpickle_global(module, name)\n        File \"sage/misc/persist.pyx\", line 542, in sage.misc.persist.unpickle_global (build/cythonized/sage/misc/persist.c:5232)\n          error()\n        File \"sage/misc/persist.pyx\", line 529, in sage.misc.persist.unpickle_global.error (build/cythonized/sage/misc/persist.c:4797)\n          raise ImportError(\"cannot import {1} from {0}, call \"\n      ImportError: cannot import getattr from __builtin__, call register_unpickle_override('__builtin__', 'getattr', ...) to fix this\n```",
    "created_at": "2018-06-27T07:00:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465771",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:34'></a>
This does not fix the pickle issue with python 3, though the error changes. 
My touchstone file is `./sage -t src/sage/combinat/interval_posets.py`
and there:

```
   Failure in _test_elements
    The following tests failed: _test_elements
      Failure in _test_pickling:
      Traceback (most recent call last):
        File "sage/misc/persist.pyx", line 540, in sage.misc.persist.unpickle_global (build/cythonized/sage/misc/persist.c:5187)
          __import__(module)
      ModuleNotFoundError: No module named '__builtin__'
    <BLANKLINE>
      During handling of the above exception, another exception occurred:
    <BLANKLINE>
      Traceback (most recent call last):
        File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/misc/sage_unittest.py", line 294, in run
          test_method(tester = tester)
        File "sage/structure/sage_object.pyx", line 683, in sage.structure.sage_object.SageObject._test_pickling (build/cythonized/sage/structure/sage_object.c:5024)
          tester.assertEqual(loads(dumps(self)), self)
        File "sage/misc/persist.pyx", line 960, in sage.misc.persist.loads (build/cythonized/sage/misc/persist.c:7648)
          return unpickler.load()
        File "sage/misc/persist.pyx", line 736, in sage.misc.persist._BaseUnpickler.find_class (build/cythonized/sage/misc/persist.c:6386)
          return unpickle_global(module, name)
        File "sage/misc/persist.pyx", line 542, in sage.misc.persist.unpickle_global (build/cythonized/sage/misc/persist.c:5232)
          error()
        File "sage/misc/persist.pyx", line 529, in sage.misc.persist.unpickle_global.error (build/cythonized/sage/misc/persist.c:4797)
          raise ImportError("cannot import {1} from {0}, call "
      ImportError: cannot import getattr from __builtin__, call register_unpickle_override('__builtin__', 'getattr', ...) to fix this
```



---

archive/issue_comments_465772.json:
```json
{
    "body": "<a id='comment:35'></a>\nThis certainly lays the groundwork to fix issues like that, but I'll have a look at whatever's going on there.\n\nThe above issue is that it's creating pickles with Python 2 backwards-compatibility (the `fix_imports=True` argument to `pickle.dumps`).  However, the reverse should also work when loading such pickles on Python 3.",
    "created_at": "2018-06-27T12:14:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465772",
    "user": "https://github.com/embray"
}
```

<a id='comment:35'></a>
This certainly lays the groundwork to fix issues like that, but I'll have a look at whatever's going on there.

The above issue is that it's creating pickles with Python 2 backwards-compatibility (the `fix_imports=True` argument to `pickle.dumps`).  However, the reverse should also work when loading such pickles on Python 3.



---

archive/issue_comments_465773.json:
```json
{
    "body": "<a id='comment:36'></a>\nAhah, I think I see the problem.  The `fix_imports` stuff is actually implemented as the base implementation of `find_class` which we outright override.  For it to work we still need to call the base class's `find_class`.",
    "created_at": "2018-06-27T12:27:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465773",
    "user": "https://github.com/embray"
}
```

<a id='comment:36'></a>
Ahah, I think I see the problem.  The `fix_imports` stuff is actually implemented as the base implementation of `find_class` which we outright override.  For it to work we still need to call the base class's `find_class`.



---

archive/issue_comments_465774.json:
```json
{
    "body": "<a id='comment:37'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                                                                                                                                      |\n|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|\n|[d0f4a71](https://git.sagemath.org/sage.git/commit/?id=d0f4a71867a8acf5056f188192a3633b607c9347)|`turns out the Unpickler.find_class method in the base class actually does something on Python 3: It's where all the fix_imports stuff is impemented, so must call it to support that`|",
    "created_at": "2018-06-27T13:25:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465774",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:37'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                                                                                                                                      |
|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|[d0f4a71](https://git.sagemath.org/sage.git/commit/?id=d0f4a71867a8acf5056f188192a3633b607c9347)|`turns out the Unpickler.find_class method in the base class actually does something on Python 3: It's where all the fix_imports stuff is impemented, so must call it to support that`|



---

archive/issue_comments_465775.json:
```json
{
    "body": "Changing commit from \"94f12b4b87e80088fee0d2b4408932e7887c4148\" to \"d0f4a71867a8acf5056f188192a3633b607c9347\"",
    "created_at": "2018-06-27T13:25:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465775",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "94f12b4b87e80088fee0d2b4408932e7887c4148" to "d0f4a71867a8acf5056f188192a3633b607c9347"



---

archive/issue_comments_465776.json:
```json
{
    "body": "<a id='comment:38'></a>\nThis should fix it.  I confirmed that `./sage -t src/sage/combinat/interval_posets.py` works (as should other tests plagued by the same problem).",
    "created_at": "2018-06-27T13:25:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465776",
    "user": "https://github.com/embray"
}
```

<a id='comment:38'></a>
This should fix it.  I confirmed that `./sage -t src/sage/combinat/interval_posets.py` works (as should other tests plagued by the same problem).



---

archive/issue_comments_465777.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Fr\u00e9d\u00e9ric Chapoton\"",
    "created_at": "2018-06-27T19:35:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465777",
    "user": "https://github.com/fchapoton"
}
```

Changing reviewer from "" to "Frédéric Chapoton"



---

archive/issue_comments_465778.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-06-27T19:35:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465778",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_465779.json:
```json
{
    "body": "<a id='comment:39'></a>\nI am ready to give it a try.",
    "created_at": "2018-06-27T19:35:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465779",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:39'></a>
I am ready to give it a try.



---

archive/issue_comments_465780.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-06-29T22:34:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465780",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_465781.json:
```json
{
    "body": "Changing branch from \"u/embray/python3/sage-structure-sage_object\" to \"d0f4a71867a8acf5056f188192a3633b607c9347\"",
    "created_at": "2018-06-29T22:34:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24582#issuecomment-465781",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/embray/python3/sage-structure-sage_object" to "d0f4a71867a8acf5056f188192a3633b607c9347"



---

archive/issue_events_061965.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-06-29T22:34:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24582",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24582#event-61965"
}
```
