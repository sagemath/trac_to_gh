# Issue 24852: Fix bytes/str in sage.data_structures.bitset

archive/issues_024615.json:
```json
{
    "body": "\n\nAssignee: @embray\n\nCC:  @embray\n\nBranch/Commit: b6c068a087581db0cecf2956b3041156edc7ac36\n\nReviewer: Fr\u00e9d\u00e9ric Chapoton\n\nAuthor: Erik Bray\n\nDependencies: #26397\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/24852\n\n",
    "closed_at": "2018-10-06T17:13:15Z",
    "created_at": "2018-02-27T09:33:53Z",
    "labels": [
        "component: python3",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.4",
    "title": "Fix bytes/str in sage.data_structures.bitset",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24852",
    "user": "https://github.com/jdemeyer"
}
```


Assignee: @embray

CC:  @embray

Branch/Commit: b6c068a087581db0cecf2956b3041156edc7ac36

Reviewer: Frédéric Chapoton

Author: Erik Bray

Dependencies: #26397

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/24852





---

archive/issue_comments_472207.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jdemeyer/fix_bytes_str_in_sage_data_structures_bitset\"",
    "created_at": "2018-02-27T09:34:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472207",
    "user": "https://github.com/jdemeyer"
}
```

Changing branch from "" to "u/jdemeyer/fix_bytes_str_in_sage_data_structures_bitset"



---

archive/issue_comments_472208.json:
```json
{
    "body": "Changing commit from \"\" to \"59ad1fb956faade593e7dfe30b080ddae9456fe0\"",
    "created_at": "2018-02-27T09:36:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472208",
    "user": "https://github.com/jdemeyer"
}
```

Changing commit from "" to "59ad1fb956faade593e7dfe30b080ddae9456fe0"



---

archive/issue_comments_472209.json:
```json
{
    "body": "<a id='comment:2'></a>\nIt looks like `bitset_from_str` is meant to be a low-level C function. Wouldn't it be better to fix *calls* to `bitset_from_str` instead?\n\n---\nNew commits:\n|                                                                                                                                          |                                                   |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------|\n|[59ad1fb](https://git.sagemath.org/sage.git/commit?id=59ad1fb956faade593e7dfe30b080ddae9456fe0)|`py3: Fix bytes/str in sage.data_structures.bitset`|",
    "created_at": "2018-02-27T09:36:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472209",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
It looks like `bitset_from_str` is meant to be a low-level C function. Wouldn't it be better to fix *calls* to `bitset_from_str` instead?

---
New commits:
|                                                                                                                                          |                                                   |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------|
|[59ad1fb](https://git.sagemath.org/sage.git/commit?id=59ad1fb956faade593e7dfe30b080ddae9456fe0)|`py3: Fix bytes/str in sage.data_structures.bitset`|



---

archive/issue_comments_472210.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-02-27T09:36:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472210",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_472211.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-02-27T09:36:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472211",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_472212.json:
```json
{
    "body": "<a id='comment:5'></a>\nYes, I see your point. Let me see how many of those there actually are...",
    "created_at": "2018-02-27T09:38:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472212",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>
Yes, I see your point. Let me see how many of those there actually are...



---

archive/issue_comments_472213.json:
```json
{
    "body": "<a id='comment:6'></a>\nNo, it's only used in bitset.pyx, and the way I use `str_to_bytes` here makes sense.",
    "created_at": "2018-02-27T09:42:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472213",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'></a>
No, it's only used in bitset.pyx, and the way I use `str_to_bytes` here makes sense.



---

archive/issue_comments_472214.json:
```json
{
    "body": "<a id='comment:7'></a>\nThere are one or two cases in bitset.pyx where it's explicitly being used on bytes (e.g. in `__setstate__`, and other cases where it's being used on `str` (in the 2/3-compatibility sense) where using `str_to_bytes` in `bitset_from_str` makes sense.\n\nI think what might make sense, for symmetry's sake, might be to rename the original `bitset_from_str` to `bitset_from_char`  (as in `char *`) and then make `bitset_from_str` a wrapper around that which uses `str_to_bytes`.",
    "created_at": "2018-02-27T09:46:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472214",
    "user": "https://github.com/embray"
}
```

<a id='comment:7'></a>
There are one or two cases in bitset.pyx where it's explicitly being used on bytes (e.g. in `__setstate__`, and other cases where it's being used on `str` (in the 2/3-compatibility sense) where using `str_to_bytes` in `bitset_from_str` makes sense.

I think what might make sense, for symmetry's sake, might be to rename the original `bitset_from_str` to `bitset_from_char`  (as in `char *`) and then make `bitset_from_str` a wrapper around that which uses `str_to_bytes`.



---

archive/issue_comments_472215.json:
```json
{
    "body": "<a id='comment:8'></a>\nOn second thought, `__setstate__` is fine since `__getstate__` also returns `str`.  Though I don't know why it would dump/load as strings when it could just use binary.",
    "created_at": "2018-02-27T11:53:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472215",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>
On second thought, `__setstate__` is fine since `__getstate__` also returns `str`.  Though I don't know why it would dump/load as strings when it could just use binary.



---

archive/issue_events_062450.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-05-14T12:22:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24852#event-62450"
}
```



---

archive/issue_comments_472216.json:
```json
{
    "body": "Set assignee to @embray.",
    "created_at": "2018-05-14T16:12:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472216",
    "user": "https://github.com/embray"
}
```

Set assignee to @embray.



---

archive/issue_comments_472217.json:
```json
{
    "body": "<a id='comment:11'></a>\nwhat is the status of this one ?",
    "created_at": "2018-06-13T16:09:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472217",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:11'></a>
what is the status of this one ?



---

archive/issue_comments_472218.json:
```json
{
    "body": "<a id='comment:12'></a>\nI still think that it needs to be fixed.",
    "created_at": "2018-06-14T05:38:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472218",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'></a>
I still think that it needs to be fixed.



---

archive/issue_comments_472219.json:
```json
{
    "body": "<a id='comment:13'></a>\nReplying to [embray](#comment%3A7):\n> rename the original `bitset_from_str` to `bitset_from_char`  (as in `char *`)\n\n\nBikeshed: I find it a bit strange to use `char` to refer to `char *`. Better would be `cstr` or `charptr` or `chararray` or whatever.",
    "created_at": "2018-06-14T05:39:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472219",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'></a>
Replying to [embray](#comment%3A7):
> rename the original `bitset_from_str` to `bitset_from_char`  (as in `char *`)


Bikeshed: I find it a bit strange to use `char` to refer to `char *`. Better would be `cstr` or `charptr` or `chararray` or whatever.



---

archive/issue_comments_472220.json:
```json
{
    "body": "<a id='comment:14'></a>\nReplying to [jdemeyer](#comment%3A13):\n> Replying to [embray](#comment%3A7):\n> > rename the original `bitset_from_str` to `bitset_from_char`  (as in `char *`)\n\n> \n> Bikeshed: I find it a bit strange to use `char` to refer to `char *`. Better would be `cstr` or `charptr` or `chararray` or whatever.\n\n\nI have the same uneasiness about it but couldn't think of a better name at the time.  I think for now I'll keep just \"char\" for consistency's sake, unless you also want to rename `char_to_str`.\n\nI still believe just \"char\" is clear enough in context, though I do like \"cstr\".",
    "created_at": "2018-06-22T15:15:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472220",
    "user": "https://github.com/embray"
}
```

<a id='comment:14'></a>
Replying to [jdemeyer](#comment%3A13):
> Replying to [embray](#comment%3A7):
> > rename the original `bitset_from_str` to `bitset_from_char`  (as in `char *`)

> 
> Bikeshed: I find it a bit strange to use `char` to refer to `char *`. Better would be `cstr` or `charptr` or `chararray` or whatever.


I have the same uneasiness about it but couldn't think of a better name at the time.  I think for now I'll keep just "char" for consistency's sake, unless you also want to rename `char_to_str`.

I still believe just "char" is clear enough in context, though I do like "cstr".



---

archive/issue_events_062451.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24852#event-62451"
}
```



---

archive/issue_events_062452.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24852#event-62452"
}
```



---

archive/issue_comments_472221.json:
```json
{
    "body": "<a id='comment:15'></a>\nupdate milestone 8.3 -> 8.4",
    "created_at": "2018-08-03T19:20:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472221",
    "user": "https://github.com/videlec"
}
```

<a id='comment:15'></a>
update milestone 8.3 -> 8.4



---

archive/issue_comments_472222.json:
```json
{
    "body": "<a id='comment:16'></a>\nWhat's the status here ? can we move on ?",
    "created_at": "2018-09-11T14:29:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472222",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:16'></a>
What's the status here ? can we move on ?



---

archive/issue_comments_472223.json:
```json
{
    "body": "<a id='comment:17'></a>\nI think this is still worth fixing:\n\n> I think what might make sense, for symmetry's sake, might be to rename the original `bitset_from_str` to `bitset_from_char`  (as in `char *`) and then make `bitset_from_str` a wrapper around that which uses `str_to_bytes`.\n\n\nThere's also the matter of Jeroen's bikeshed about the name, but I think I addressed that above.",
    "created_at": "2018-09-11T14:55:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472223",
    "user": "https://github.com/embray"
}
```

<a id='comment:17'></a>
I think this is still worth fixing:

> I think what might make sense, for symmetry's sake, might be to rename the original `bitset_from_str` to `bitset_from_char`  (as in `char *`) and then make `bitset_from_str` a wrapper around that which uses `str_to_bytes`.


There's also the matter of Jeroen's bikeshed about the name, but I think I addressed that above.



---

archive/issue_comments_472224.json:
```json
{
    "body": "<a id='comment:18'></a>\nmany doctests failures with python2... essentially totally broken",
    "created_at": "2018-10-03T13:38:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472224",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:18'></a>
many doctests failures with python2... essentially totally broken



---

archive/issue_comments_472225.json:
```json
{
    "body": "<a id='comment:19'></a>\nOkay. Would you like to fix it, taking into account my last comment?  I can still do it too, it just hasn't been high priority.",
    "created_at": "2018-10-03T15:25:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472225",
    "user": "https://github.com/embray"
}
```

<a id='comment:19'></a>
Okay. Would you like to fix it, taking into account my last comment?  I can still do it too, it just hasn't been high priority.



---

archive/issue_comments_472226.json:
```json
{
    "body": "<a id='comment:20'></a>\nI have no idea what to do here.. So I am not able to do it myself, sorry.",
    "created_at": "2018-10-03T15:31:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472226",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:20'></a>
I have no idea what to do here.. So I am not able to do it myself, sorry.



---

archive/issue_comments_472227.json:
```json
{
    "body": "<a id='comment:21'></a>\nOk, but I think you underestimate yourself.  It's mainly just a simple str/bytes conversion issue.\n\nThe existing function maed `bitset_from_str` originally took a `char *` as its argument, which won't make sense for Python 3 str, so I changed it to accept a generic `object` instead and use `str_to_bytes` on it.\n\nJeroen's suggestion was that we keep the original implementation of `bitset_from_str`, but rename it to something like `bitset_from_char`, keeping the original `char *` interface, then add a new `bitset_from_str` which on Python 2 can easily just be an easily for `bitset_from_str`, and on Python 3 would call `str_to_bytes` and pass that to `bitset_from_char`.  Just for example.",
    "created_at": "2018-10-03T15:49:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472227",
    "user": "https://github.com/embray"
}
```

<a id='comment:21'></a>
Ok, but I think you underestimate yourself.  It's mainly just a simple str/bytes conversion issue.

The existing function maed `bitset_from_str` originally took a `char *` as its argument, which won't make sense for Python 3 str, so I changed it to accept a generic `object` instead and use `str_to_bytes` on it.

Jeroen's suggestion was that we keep the original implementation of `bitset_from_str`, but rename it to something like `bitset_from_char`, keeping the original `char *` interface, then add a new `bitset_from_str` which on Python 2 can easily just be an easily for `bitset_from_str`, and on Python 3 would call `str_to_bytes` and pass that to `bitset_from_char`.  Just for example.



---

archive/issue_comments_472228.json:
```json
{
    "body": "<a id='comment:22'></a>\nWell, I've got 30 minutes to kill. So I'll take a look at it now.",
    "created_at": "2018-10-04T10:02:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472228",
    "user": "https://github.com/embray"
}
```

<a id='comment:22'></a>
Well, I've got 30 minutes to kill. So I'll take a look at it now.



---

archive/issue_comments_472229.json:
```json
{
    "body": "<a id='comment:23'></a>\nReworked this a bit more.  In addition to the original changes from this branch, I also updated `__getstate__` and `__setstate__` to work on bytes instead of str, and I renamed the original `bitset_string` function to `bitset_bytes`, but also added a new `bitset_string` that returns a `str` on Python 3.\n\nI think everything should work on Python 2 as well.  I tested all the modules that use this class directly, at least.\n\n---\nNew commits:\n|                                                                                                                                          |                                                   |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------|\n|[b6c068a](https://git.sagemath.org/sage.git/commit?id=b6c068a087581db0cecf2956b3041156edc7ac36)|`py3: Fix bytes/str in sage.data_structures.bitset`|",
    "created_at": "2018-10-04T13:14:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472229",
    "user": "https://github.com/embray"
}
```

<a id='comment:23'></a>
Reworked this a bit more.  In addition to the original changes from this branch, I also updated `__getstate__` and `__setstate__` to work on bytes instead of str, and I renamed the original `bitset_string` function to `bitset_bytes`, but also added a new `bitset_string` that returns a `str` on Python 3.

I think everything should work on Python 2 as well.  I tested all the modules that use this class directly, at least.

---
New commits:
|                                                                                                                                          |                                                   |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------|
|[b6c068a](https://git.sagemath.org/sage.git/commit?id=b6c068a087581db0cecf2956b3041156edc7ac36)|`py3: Fix bytes/str in sage.data_structures.bitset`|



---

archive/issue_comments_472230.json:
```json
{
    "body": "Changing branch from \"u/jdemeyer/fix_bytes_str_in_sage_data_structures_bitset\" to \"u/embray/python3/ticket-24852\"",
    "created_at": "2018-10-04T13:14:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472230",
    "user": "https://github.com/embray"
}
```

Changing branch from "u/jdemeyer/fix_bytes_str_in_sage_data_structures_bitset" to "u/embray/python3/ticket-24852"



---

archive/issue_comments_472231.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-10-04T13:14:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472231",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_472232.json:
```json
{
    "body": "Changing commit from \"59ad1fb956faade593e7dfe30b080ddae9456fe0\" to \"b6c068a087581db0cecf2956b3041156edc7ac36\"",
    "created_at": "2018-10-04T13:14:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472232",
    "user": "https://github.com/embray"
}
```

Changing commit from "59ad1fb956faade593e7dfe30b080ddae9456fe0" to "b6c068a087581db0cecf2956b3041156edc7ac36"



---

archive/issue_comments_472233.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Fr\u00e9d\u00e9ric Chapoton\"",
    "created_at": "2018-10-04T16:14:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472233",
    "user": "https://github.com/fchapoton"
}
```

Changing reviewer from "" to "Frédéric Chapoton"



---

archive/issue_comments_472234.json:
```json
{
    "body": "<a id='comment:24'></a>\nok, then. Thanks a lot.",
    "created_at": "2018-10-04T16:14:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472234",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:24'></a>
ok, then. Thanks a lot.



---

archive/issue_comments_472235.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-10-04T16:14:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472235",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_472236.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#26397\"",
    "created_at": "2018-10-04T16:14:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472236",
    "user": "https://github.com/fchapoton"
}
```

Changing dependencies from "" to "#26397"



---

archive/issue_comments_472237.json:
```json
{
    "body": "Changing branch from \"u/embray/python3/ticket-24852\" to \"b6c068a087581db0cecf2956b3041156edc7ac36\"",
    "created_at": "2018-10-06T17:13:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472237",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/embray/python3/ticket-24852" to "b6c068a087581db0cecf2956b3041156edc7ac36"



---

archive/issue_events_062453.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-10-06T17:13:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24852#event-62453"
}
```



---

archive/issue_comments_472238.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-10-06T17:13:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24852#issuecomment-472238",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
