# Issue 24855: Workflows in CircleCI

archive/issues_024618.json:
```json
{
    "assignees": [],
    "body": "Now that the docker images are relatively small, we could pass them on in a workspace and use workflows to make this more similar to [GitLab](../wiki/GitLab) CI.\n\nDepends on #24655\n\nReviewer: **Julian R\u00fcth**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24855_\n\n",
    "closed_at": "2018-11-08T16:07:48Z",
    "created_at": "2018-02-27T10:38:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Workflows in CircleCI",
    "type": "issue",
    "updated_at": "2018-11-08T16:07:48Z",
    "url": "https://github.com/sagemath/sage/issues/24855",
    "user": "https://github.com/saraedum"
}
```
Now that the docker images are relatively small, we could pass them on in a workspace and use workflows to make this more similar to [GitLab](../wiki/GitLab) CI.

Depends on #24655

Reviewer: **Julian Rüth**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/24855_





---

archive/issue_events_325946.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-02-27T10:38:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325946"
}
```



---

archive/issue_events_325947.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-02-27T10:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325947"
}
```



---

archive/issue_events_325948.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-02-27T10:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325948"
}
```



---

archive/issue_events_325949.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-02-27T10:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325949"
}
```



---

archive/issue_comments_380787.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2018-05-30T15:55:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24855#issuecomment-380787",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_comments_380788.json:
```json
{
    "body": "Author: **Julian R\u00fcth**",
    "created_at": "2018-05-30T15:55:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24855#issuecomment-380788",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rüth**



---

archive/issue_events_325950.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-30T15:55:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325950"
}
```



---

archive/issue_comments_380789.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nWe are already using workflows in #24655 now. Passing images on does not work as the dev images exceed CirceCI's space limits.",
    "created_at": "2018-05-30T15:55:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24855#issuecomment-380789",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:1" align="right">Comment 1</div>

We are already using workflows in #24655 now. Passing images on does not work as the dev images exceed CirceCI's space limits.



---

archive/issue_events_325951.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-30T15:55:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325951"
}
```



---

archive/issue_events_325952.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-30T15:55:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325952"
}
```



---

archive/issue_events_325953.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-30T15:55:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325953"
}
```



---

archive/issue_events_325954.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-30T15:56:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325954"
}
```



---

archive/issue_events_325955.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-30T15:56:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325955"
}
```



---

archive/issue_events_325956.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:07:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325956"
}
```



---

archive/issue_events_325957.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:07:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325957"
}
```



---

archive/issue_events_325958.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:07:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24855",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24855#event-325958"
}
```
