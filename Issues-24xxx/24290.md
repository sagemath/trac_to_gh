# Issue 24290: Make simplifications in SymPy calculus on manifolds rely on SymPy only

archive/issues_024053.json:
```json
{
    "assignees": [],
    "body": "#22801 introduced SymPy as an optional engine for symbolic calculus on manifolds. However, not all simplifications are performed by SymPy; in particular those depending on assumptions are still treated via Maxima. \n\nDepends on #22801\n\nCC:  @man74cio @rll2021 @rwst\n\nKeywords: **manifolds calculus sympy**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24290_\n\n",
    "created_at": "2017-11-28T08:40:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make simplifications in SymPy calculus on manifolds rely on SymPy only",
    "type": "issue",
    "updated_at": "2022-12-29T01:40:48Z",
    "url": "https://github.com/sagemath/sage/issues/24290",
    "user": "https://github.com/egourgoulhon"
}
```
#22801 introduced SymPy as an optional engine for symbolic calculus on manifolds. However, not all simplifications are performed by SymPy; in particular those depending on assumptions are still treated via Maxima. 

Depends on #22801

CC:  @man74cio @rll2021 @rwst

Keywords: **manifolds calculus sympy**

_Issue created by migration from https://trac.sagemath.org/ticket/24290_





---

archive/issue_events_318914.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-318914"
}
```



---

archive/issue_events_318915.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-318915"
}
```



---

archive/issue_events_318916.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-318916"
}
```



---

archive/issue_events_318917.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-318917"
}
```



---

archive/issue_comments_370440.json:
```json
{
    "body": "Dependencies: **#22801**",
    "created_at": "2017-11-28T08:44:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24290#issuecomment-370440",
    "user": "https://github.com/egourgoulhon"
}
```

Dependencies: **#22801**



---

archive/issue_events_318918.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T09:25:53Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "title_is": "Make simplifications in SymPy calculus on manifolds rely on SymPy only",
    "title_was": "Make simplifications in SymPy calculus on manifolds depend on SymPy only",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-318918"
}
```



---

archive/issue_comments_370441.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBasically, one needs to pass Sage's `assumptions()` to SymPy and use them via `refine` (cf.  http://docs.sympy.org/latest/modules/assumptions/refine.html) or maybe via the context manager `assuming` (cf. http://docs.sympy.org/latest/modules/assumptions/assume.html).",
    "created_at": "2017-11-28T09:25:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24290#issuecomment-370441",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:2'>Comment 2:</a>
Basically, one needs to pass Sage's `assumptions()` to SymPy and use them via `refine` (cf.  http://docs.sympy.org/latest/modules/assumptions/refine.html) or maybe via the context manager `assuming` (cf. http://docs.sympy.org/latest/modules/assumptions/assume.html).



---

archive/issue_events_318919.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:40:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-318919"
}
```
