# Issue 24160: Improve speed of dominated_partitions

archive/issues_023923.json:
```json
{
    "assignees": [],
    "body": "Use the iterator that returns lists and check the dominance condition on that to avoid the (relatively) costly creation of a `Partition`.\n\nThis gives ~35% speedup:\n\n```\nsage: P = Partition([7,6,5,4,3,2,1])\nsage: %timeit P.dominated_partitions()\n10 loops, best of 3: 26.5 ms per loop\n```\nversus old\n\n```\nsage: P = Partition([7,6,5,4,3,2,1])\nsage: %timeit P.dominated_partitions()\n10 loops, best of 3: 37.3 ms per loop\n```\n\nCC:  @sagetrac-sage-combinat @darijgr @anneschilling @fchapoton\n\nKeywords: **partitions**\n\nBranch/Commit: **[4545d51](https://github.com/sagemath/sagetrac-mirror/commit/4545d51cc377eac0e31d4ef2c5b67e6e5accde64)**\n\nReviewer: **Darij Grinberg**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24160_\n\n",
    "closed_at": "2017-12-11T01:03:07Z",
    "created_at": "2017-11-06T03:43:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve speed of dominated_partitions",
    "type": "issue",
    "updated_at": "2017-12-11T01:03:07Z",
    "url": "https://github.com/sagemath/sage/issues/24160",
    "user": "https://github.com/tscrim"
}
```
Use the iterator that returns lists and check the dominance condition on that to avoid the (relatively) costly creation of a `Partition`.

This gives ~35% speedup:

```
sage: P = Partition([7,6,5,4,3,2,1])
sage: %timeit P.dominated_partitions()
10 loops, best of 3: 26.5 ms per loop
```
versus old

```
sage: P = Partition([7,6,5,4,3,2,1])
sage: %timeit P.dominated_partitions()
10 loops, best of 3: 37.3 ms per loop
```

CC:  @sagetrac-sage-combinat @darijgr @anneschilling @fchapoton

Keywords: **partitions**

Branch/Commit: **[4545d51](https://github.com/sagemath/sagetrac-mirror/commit/4545d51cc377eac0e31d4ef2c5b67e6e5accde64)**

Reviewer: **Darij Grinberg**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/24160_





---

archive/issue_events_317219.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-06T03:43:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24160#event-317219"
}
```



---

archive/issue_events_317220.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-06T03:43:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24160#event-317220"
}
```



---

archive/issue_events_317221.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-06T03:43:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24160#event-317221"
}
```



---

archive/issue_events_317222.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-06T03:43:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24160#event-317222"
}
```



---

archive/issue_comments_368151.json:
```json
{
    "body": "Branch: **[public/combiant/speedup_dominated_partitions-24160](https://github.com/sagemath/sagetrac-mirror/tree/public/combiant/speedup_dominated_partitions-24160)**",
    "created_at": "2017-11-06T03:45:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368151",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/combiant/speedup_dominated_partitions-24160](https://github.com/sagemath/sagetrac-mirror/tree/public/combiant/speedup_dominated_partitions-24160)**



---

archive/issue_comments_368152.json:
```json
{
    "body": "Commit: **[90a46ba](https://github.com/sagemath/sagetrac-mirror/commit/90a46baeb5a78b6b7b8d18e85ebe7a179009d3f3)**",
    "created_at": "2017-11-06T03:45:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368152",
    "user": "https://github.com/tscrim"
}
```

Commit: **[90a46ba](https://github.com/sagemath/sagetrac-mirror/commit/90a46baeb5a78b6b7b8d18e85ebe7a179009d3f3)**



---

archive/issue_events_317223.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-06T03:45:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24160#event-317223"
}
```



---

archive/issue_comments_368153.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nUnless we can implement box-sliding in a fast way, we will not beat the na\u00efve way for a reasonable partition size.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/90a46baeb5a78b6b7b8d18e85ebe7a179009d3f3\">90a46ba</a></td><td><code>Avoid creating Partition objects when possible.</code></td></tr></table>\n",
    "created_at": "2017-11-06T03:45:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368153",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

Unless we can implement box-sliding in a fast way, we will not beat the naïve way for a reasonable partition size.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/90a46baeb5a78b6b7b8d18e85ebe7a179009d3f3">90a46ba</a></td><td><code>Avoid creating Partition objects when possible.</code></td></tr></table>




---

archive/issue_comments_368154.json:
```json
{
    "body": "Changed branch from **[public/combiant/speedup_dominated_partitions-24160](https://github.com/sagemath/sagetrac-mirror/tree/public/combiant/speedup_dominated_partitions-24160)** to **[public/combinat/speedup_dominated_partitions-24160](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/speedup_dominated_partitions-24160)**",
    "created_at": "2017-11-06T03:48:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368154",
    "user": "https://github.com/tscrim"
}
```

Changed branch from **[public/combiant/speedup_dominated_partitions-24160](https://github.com/sagemath/sagetrac-mirror/tree/public/combiant/speedup_dominated_partitions-24160)** to **[public/combinat/speedup_dominated_partitions-24160](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/speedup_dominated_partitions-24160)**



---

archive/issue_comments_368155.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM!",
    "created_at": "2017-11-06T05:08:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368155",
    "user": "https://github.com/darijgr"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM!



---

archive/issue_comments_368156.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nPS. In the implementation of `dominates`, you can replace\n\n```\n        min_length = min(len(p1), len(p2))\n        if min_length == 0:\n            return len(p1) >= len(p2)\n```\nby\n\n```\n        min_length = min(len(p1), len(p2))\n        if min_length == 0:\n            return len(p2) == 0\n```\nNot sure if it's worth it :)",
    "created_at": "2017-11-06T05:10:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368156",
    "user": "https://github.com/darijgr"
}
```

<div id="comment:4" align="right">Comment 4</div>

PS. In the implementation of `dominates`, you can replace

```
        min_length = min(len(p1), len(p2))
        if min_length == 0:
            return len(p1) >= len(p2)
```
by

```
        min_length = min(len(p1), len(p2))
        if min_length == 0:
            return len(p2) == 0
```
Not sure if it's worth it :)



---

archive/issue_comments_368157.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nActually, perhaps more relevant: Why does it `            return len(p1) >= len(p2)` in one case and `        return bool(sum(p1) >= sum(p2))` in the other?",
    "created_at": "2017-11-06T05:11:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368157",
    "user": "https://github.com/darijgr"
}
```

<div id="comment:5" align="right">Comment 5</div>

Actually, perhaps more relevant: Why does it `            return len(p1) >= len(p2)` in one case and `        return bool(sum(p1) >= sum(p2))` in the other?



---

archive/issue_comments_368158.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4545d51cc377eac0e31d4ef2c5b67e6e5accde64\">4545d51</a></td><td><code>Some details in dominates.</code></td></tr></table>\n",
    "created_at": "2017-11-06T05:16:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368158",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4545d51cc377eac0e31d4ef2c5b67e6e5accde64">4545d51</a></td><td><code>Some details in dominates.</code></td></tr></table>




---

archive/issue_comments_368159.json:
```json
{
    "body": "Changed commit from **[90a46ba](https://github.com/sagemath/sagetrac-mirror/commit/90a46baeb5a78b6b7b8d18e85ebe7a179009d3f3)** to **[4545d51](https://github.com/sagemath/sagetrac-mirror/commit/4545d51cc377eac0e31d4ef2c5b67e6e5accde64)**",
    "created_at": "2017-11-06T05:16:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368159",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[90a46ba](https://github.com/sagemath/sagetrac-mirror/commit/90a46baeb5a78b6b7b8d18e85ebe7a179009d3f3)** to **[4545d51](https://github.com/sagemath/sagetrac-mirror/commit/4545d51cc377eac0e31d4ef2c5b67e6e5accde64)**



---

archive/issue_comments_368160.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nIt is a nice micro-optimization at least. As for [comment:5](#comment%3A5), Jason Bandlow was the last one to touch that line in Oct. 2008. I've removed it.",
    "created_at": "2017-11-06T05:18:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368160",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:7" align="right">Comment 7</div>

It is a nice micro-optimization at least. As for [comment:5](#comment%3A5), Jason Bandlow was the last one to touch that line in Oct. 2008. I've removed it.



---

archive/issue_comments_368161.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nIt's a positive review then!",
    "created_at": "2017-11-06T05:21:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368161",
    "user": "https://github.com/darijgr"
}
```

<div id="comment:8" align="right">Comment 8</div>

It's a positive review then!



---

archive/issue_comments_368162.json:
```json
{
    "body": "Changed keywords from **** to **partitions**",
    "created_at": "2017-11-06T05:22:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368162",
    "user": "https://github.com/darijgr"
}
```

Changed keywords from **** to **partitions**



---

archive/issue_events_317224.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2017-11-06T05:22:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24160#event-317224"
}
```



---

archive/issue_events_317225.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2017-11-06T05:22:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24160#event-317225"
}
```



---

archive/issue_comments_368163.json:
```json
{
    "body": "Reviewer: **Darij Grinberg**",
    "created_at": "2017-11-06T05:22:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368163",
    "user": "https://github.com/darijgr"
}
```

Reviewer: **Darij Grinberg**



---

archive/issue_comments_368164.json:
```json
{
    "body": "Changed branch from **[public/combinat/speedup_dominated_partitions-24160](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/speedup_dominated_partitions-24160)** to **[4545d51](https://github.com/sagemath/sagetrac-mirror/commit/4545d51cc377eac0e31d4ef2c5b67e6e5accde64)**",
    "created_at": "2017-12-11T01:03:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24160#issuecomment-368164",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/combinat/speedup_dominated_partitions-24160](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/speedup_dominated_partitions-24160)** to **[4545d51](https://github.com/sagemath/sagetrac-mirror/commit/4545d51cc377eac0e31d4ef2c5b67e6e5accde64)**



---

archive/issue_events_317226.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-11T01:03:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24160#event-317226"
}
```



---

archive/issue_events_317227.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "75bddf67d17dd18c2a7f3b195309e2829f11105e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-11T01:03:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24160",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24160#event-317227"
}
```
