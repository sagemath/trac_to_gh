...
```json
{
    "body": "Doesn't look like they do, but it doesn't stop Debian or other distributions to package it.",
...
```json
{
    "body": "Use `sage-python23` to run Python scripts.",
...
```json
{
    "body": "And this should probably depend on `$(PYTHON)`.",
...
```json
{
    "body": "There is some filename mismatch between `ninja-1.8.2.tar.gz` and `ninja_build-1.8.2.tar.gz`",
...
```json
{
    "body": "Seems to work fine. However, Python seems to be used only for build system, so Python should be an order-only dependency (after the `|` symbol).\n\nPositive review if you make that change.",
