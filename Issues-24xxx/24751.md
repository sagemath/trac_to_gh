# Issue 24751: py3: bug in pynac when evaluating functions

archive/issues_024514.json:
```json
{
    "body": "Keywords: pynac\n\nThere are several cases like:\n\n```\nsage: arcsinh(z)\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n<ipython-input-2-fbfc0c968329> in <module>()\n----> 1 arcsinh(z)\n\n/home/embray/src/sagemath/sage-python3/local/lib/python3.6/site-packages/sage/symbolic/function.pyx in sage.symbolic.function.BuiltinFunction.__call__ (build/cythonized/sage/symbolic/function.cpp:11719)()\n    988\n    989             if callable(method):\n--> 990                 res = method()\n    991\n    992         if res is None:\n\n/home/embray/src/sagemath/sage-python3/local/lib/python3.6/site-packages/sage/symbolic/expression.pyx in sage.symbolic.expression.Expression.arcsinh (build/cythonized/sage/symbolic/expression.cpp:49022)()\n   8588         \"\"\"\n   8589         return new_Expression_from_GEx(self._parent,\n-> 8590                 g_hold_wrapper(g_asinh, self._gobj, hold))\n   8591\n   8592     def arccosh(self, hold=False):\n\nTypeError: attribute name must be string, not 'bytes'\n```\n\nUpstream PRs: https://github.com/pynac/pynac/pull/306 https://github.com/pynac/pynac/pull/308\n\nIssue created by migration from https://trac.sagemath.org/ticket/24751\n\n",
    "closed_at": "2018-07-10T13:45:21Z",
    "created_at": "2018-02-16T13:16:57Z",
    "labels": [
        "component: python3",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "py3: bug in pynac when evaluating functions",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24751",
    "user": "https://github.com/embray"
}
```
Keywords: pynac

There are several cases like:

```
sage: arcsinh(z)
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-2-fbfc0c968329> in <module>()
----> 1 arcsinh(z)

/home/embray/src/sagemath/sage-python3/local/lib/python3.6/site-packages/sage/symbolic/function.pyx in sage.symbolic.function.BuiltinFunction.__call__ (build/cythonized/sage/symbolic/function.cpp:11719)()
    988
    989             if callable(method):
--> 990                 res = method()
    991
    992         if res is None:

/home/embray/src/sagemath/sage-python3/local/lib/python3.6/site-packages/sage/symbolic/expression.pyx in sage.symbolic.expression.Expression.arcsinh (build/cythonized/sage/symbolic/expression.cpp:49022)()
   8588         """
   8589         return new_Expression_from_GEx(self._parent,
-> 8590                 g_hold_wrapper(g_asinh, self._gobj, hold))
   8591
   8592     def arccosh(self, hold=False):

TypeError: attribute name must be string, not 'bytes'
```

Upstream PRs: https://github.com/pynac/pynac/pull/306 https://github.com/pynac/pynac/pull/308

Issue created by migration from https://trac.sagemath.org/ticket/24751





---

archive/issue_events_062249.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-03-14T15:50:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/24751",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24751#event-62249"
}
```



---

archive/issue_comments_343584.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-03-14T15:50:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24751#issuecomment-343584",
    "user": "https://github.com/fchapoton"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_343585.json:
```json
{
    "body": "This seems to be no longer an issue..",
    "created_at": "2018-03-14T15:50:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24751#issuecomment-343585",
    "user": "https://github.com/fchapoton"
}
```

This seems to be no longer an issue..



---

archive/issue_comments_343586.json:
```json
{
    "body": "I wouldn't say it's no longer an issue, but it should be fixed by #24838.",
    "created_at": "2018-03-14T16:25:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24751#issuecomment-343586",
    "user": "https://github.com/embray"
}
```

I wouldn't say it's no longer an issue, but it should be fixed by #24838.



---

archive/issue_comments_343587.json:
```json
{
    "body": "It seems to be fixed now, that's what I mean.. I am using a python3 branch + only a few more tickets on top of it.",
    "created_at": "2018-03-14T16:26:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24751#issuecomment-343587",
    "user": "https://github.com/fchapoton"
}
```

It seems to be fixed now, that's what I mean.. I am using a python3 branch + only a few more tickets on top of it.



---

archive/issue_comments_343588.json:
```json
{
    "body": "this should be closed now, please",
    "created_at": "2018-07-04T07:38:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24751#issuecomment-343588",
    "user": "https://github.com/fchapoton"
}
```

this should be closed now, please



---

archive/issue_comments_343589.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-07-04T07:38:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24751#issuecomment-343589",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_062250.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-10T13:45:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24751",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24751#event-62250"
}
```



---

archive/issue_comments_343590.json:
```json
{
    "body": "Resolution: worksforme",
    "created_at": "2018-07-10T13:45:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24751#issuecomment-343590",
    "user": "https://github.com/embray"
}
```

Resolution: worksforme
