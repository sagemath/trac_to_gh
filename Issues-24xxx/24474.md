# Issue 24474: Run all tests in sage/combinat/root_system

archive/issues_024237.json:
```json
{
    "assignees": [],
    "body": "Some tests in `sage/combinat/root_system/type_A.py` and `sage/combinat/root_system/type_G.py` are not run because they are added by manipulating `__doc__`. Don't do that!\n\nCC:  @nthiery @tscrim\n\nBranch/Commit: **[a84cbe9](https://github.com/sagemath/sagetrac-mirror/commit/a84cbe9f789ea39d0b7e06863dc89d4857513906)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24474_\n\n",
    "closed_at": "2018-01-13T11:52:10Z",
    "created_at": "2018-01-05T10:23:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Run all tests in sage/combinat/root_system",
    "type": "issue",
    "updated_at": "2018-01-13T11:52:10Z",
    "url": "https://github.com/sagemath/sage/issues/24474",
    "user": "https://github.com/jdemeyer"
}
```
Some tests in `sage/combinat/root_system/type_A.py` and `sage/combinat/root_system/type_G.py` are not run because they are added by manipulating `__doc__`. Don't do that!

CC:  @nthiery @tscrim

Branch/Commit: **[a84cbe9](https://github.com/sagemath/sagetrac-mirror/commit/a84cbe9f789ea39d0b7e06863dc89d4857513906)**

Reviewer: **Frédéric Chapoton**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24474_





---

archive/issue_events_321033.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-05T10:23:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24474#event-321033"
}
```



---

archive/issue_events_321034.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-05T10:23:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24474#event-321034"
}
```



---

archive/issue_events_321035.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-05T10:23:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24474#event-321035"
}
```



---

archive/issue_events_321036.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-05T10:23:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24474#event-321036"
}
```



---

archive/issue_comments_373208.json:
```json
{
    "body": "Branch: **[u/jdemeyer/run_all_tests_in_sage_combinat_root_system](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/run_all_tests_in_sage_combinat_root_system)**",
    "created_at": "2018-01-05T10:31:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24474#issuecomment-373208",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/run_all_tests_in_sage_combinat_root_system](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/run_all_tests_in_sage_combinat_root_system)**



---

archive/issue_events_321037.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-05T10:34:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24474#event-321037"
}
```



---

archive/issue_comments_373209.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a84cbe9f789ea39d0b7e06863dc89d4857513906\">a84cbe9</a></td><td><code>Run all tests in sage/combinat/root_system</code></td></tr></table>\n",
    "created_at": "2018-01-05T10:34:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24474#issuecomment-373209",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a84cbe9f789ea39d0b7e06863dc89d4857513906">a84cbe9</a></td><td><code>Run all tests in sage/combinat/root_system</code></td></tr></table>




---

archive/issue_comments_373210.json:
```json
{
    "body": "Commit: **[a84cbe9](https://github.com/sagemath/sagetrac-mirror/commit/a84cbe9f789ea39d0b7e06863dc89d4857513906)**",
    "created_at": "2018-01-05T10:34:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24474#issuecomment-373210",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[a84cbe9](https://github.com/sagemath/sagetrac-mirror/commit/a84cbe9f789ea39d0b7e06863dc89d4857513906)**



---

archive/issue_comments_373211.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-01-05T12:10:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24474#issuecomment-373211",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_321038.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-05T12:10:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24474#event-321038"
}
```



---

archive/issue_events_321039.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-05T12:10:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24474#event-321039"
}
```



---

archive/issue_comments_373212.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok, thanks",
    "created_at": "2018-01-05T12:10:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24474#issuecomment-373212",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok, thanks



---

archive/issue_comments_373213.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/run_all_tests_in_sage_combinat_root_system](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/run_all_tests_in_sage_combinat_root_system)** to **[a84cbe9](https://github.com/sagemath/sagetrac-mirror/commit/a84cbe9f789ea39d0b7e06863dc89d4857513906)**",
    "created_at": "2018-01-13T11:52:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24474#issuecomment-373213",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/run_all_tests_in_sage_combinat_root_system](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/run_all_tests_in_sage_combinat_root_system)** to **[a84cbe9](https://github.com/sagemath/sagetrac-mirror/commit/a84cbe9f789ea39d0b7e06863dc89d4857513906)**



---

archive/issue_events_321040.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-01-13T11:52:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24474#event-321040"
}
```



---

archive/issue_events_321041.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "38f7b8559a3fc4356d46ceb08b40dbcac6d96028",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-01-13T11:52:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24474",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24474#event-321041"
}
```
