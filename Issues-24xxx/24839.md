# Issue 24839: py3: an issue about dict iterators

archive/issues_024602.json:
```json
{
    "body": "some sums complain when using python3\n\n```\nFile \"src/sage/combinat/crystals/multisegments.py\", line 432, in sage.combinat.crystals.multisegments.InfinityCrystalOfMultisegments.Element.phi\nFailed example:\n    mg.f(1).phi(0)\nException raised:\n    Traceback (most recent call last):\n      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py\", line 557, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py\", line 967, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.combinat.crystals.multisegments.InfinityCrystalOfMultisegments.Element.phi[5]>\", line 1, in <module>\n        mg.f(Integer(1)).phi(Integer(0))\n      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/combinat/crystals/multisegments.py\", line 436, in phi\n        return self.epsilon(i) + self.weight().scalar(h[i])\n      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/combinat/root_system/weight_space.py\", line 506, in scalar\n        return BR.sum(self[i] * c for (i, c) in lambdacheck)\n    TypeError: iter() returned non-iterator of type 'dict_items'\n```\n\nCC:  @jdemeyer @embray\n\nBranch/Commit: bc7a60c9b4ba16d92a52df09f6930d5424652fb1\n\nReviewer: Erik Bray\n\nAuthor: Fr\u00e9d\u00e9ric Chapoton\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/24839\n\n",
    "closed_at": "2018-05-09T09:49:51Z",
    "created_at": "2018-02-26T14:08:20Z",
    "labels": [
        "component: python3"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.2",
    "title": "py3: an issue about dict iterators",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24839",
    "user": "https://github.com/fchapoton"
}
```
some sums complain when using python3

```
File "src/sage/combinat/crystals/multisegments.py", line 432, in sage.combinat.crystals.multisegments.InfinityCrystalOfMultisegments.Element.phi
Failed example:
    mg.f(1).phi(0)
Exception raised:
    Traceback (most recent call last):
      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py", line 557, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py", line 967, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.combinat.crystals.multisegments.InfinityCrystalOfMultisegments.Element.phi[5]>", line 1, in <module>
        mg.f(Integer(1)).phi(Integer(0))
      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/combinat/crystals/multisegments.py", line 436, in phi
        return self.epsilon(i) + self.weight().scalar(h[i])
      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/combinat/root_system/weight_space.py", line 506, in scalar
        return BR.sum(self[i] * c for (i, c) in lambdacheck)
    TypeError: iter() returned non-iterator of type 'dict_items'
```

CC:  @jdemeyer @embray

Branch/Commit: bc7a60c9b4ba16d92a52df09f6930d5424652fb1

Reviewer: Erik Bray

Author: Frédéric Chapoton

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/24839





---

archive/issue_comments_471909.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-02-26T14:09:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471909",
    "user": "https://github.com/fchapoton"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_471910.json:
```json
{
    "body": "<a id='comment:1'></a>New commits:",
    "created_at": "2018-02-26T14:09:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471910",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'></a>New commits:



---

archive/issue_comments_471911.json:
```json
{
    "body": "Changing commit from \"\" to \"52eee770d21b90ea9616c9d213f73a7da7504612\"",
    "created_at": "2018-02-26T14:09:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471911",
    "user": "https://github.com/fchapoton"
}
```

Changing commit from "" to "52eee770d21b90ea9616c9d213f73a7da7504612"



---

archive/issue_comments_471912.json:
```json
{
    "body": "Changing branch from \"\" to \"u/chapoton/24839\"",
    "created_at": "2018-02-26T14:09:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471912",
    "user": "https://github.com/fchapoton"
}
```

Changing branch from "" to "u/chapoton/24839"



---

archive/issue_comments_471913.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-02-26T14:10:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471913",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_471914.json:
```json
{
    "body": "<a id='comment:2'></a>this is not fixing the issue..",
    "created_at": "2018-02-26T14:10:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471914",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'></a>this is not fixing the issue..



---

archive/issue_comments_471915.json:
```json
{
    "body": "<a id='comment:3'></a>\n```",
    "created_at": "2018-02-26T14:12:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471915",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'></a>
```



---

archive/issue_comments_471916.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,23 @@\n-changing some sums that cause issue when using python3\n+some sums complain when using python3\n+\n+```\n+File \"src/sage/combinat/crystals/multisegments.py\", line 432, in sage.combinat.crystals.multisegments.InfinityCrystalOfMultisegments.Element.phi\n+Failed example:\n+    mg.f(1).phi(0)\n+Exception raised:\n+    Traceback (most recent call last):\n+      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py\", line 557, in _run\n+        self.compile_and_execute(example, compiler, test.globs)\n+      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py\", line 967, in compile_and_execute\n+        exec(compiled, globs)\n+      File \"<doctest sage.combinat.crystals.multisegments.InfinityCrystalOfMultisegments.Element.phi[5]>\", line 1, in <module>\n+        mg.f(Integer(1)).phi(Integer(0))\n+      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/combinat/crystals/multisegments.py\", line 436, in phi\n+        return self.epsilon(i) + self.weight().scalar(h[i])\n+      File \"/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/combinat/root_system/weight_space.py\", line 506, in scalar\n+        return BR.sum(self[i] * c for (i, c) in lambdacheck)\n+    TypeError: iter() returned non-iterator of type 'dict_items'\n+```\n \n Author: Fr\u00e9d\u00e9ric Chapoton\n \n``````\n",
    "created_at": "2018-02-26T14:12:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471916",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,23 @@
-changing some sums that cause issue when using python3
+some sums complain when using python3
+
+```
+File "src/sage/combinat/crystals/multisegments.py", line 432, in sage.combinat.crystals.multisegments.InfinityCrystalOfMultisegments.Element.phi
+Failed example:
+    mg.f(1).phi(0)
+Exception raised:
+    Traceback (most recent call last):
+      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py", line 557, in _run
+        self.compile_and_execute(example, compiler, test.globs)
+      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/doctest/forker.py", line 967, in compile_and_execute
+        exec(compiled, globs)
+      File "<doctest sage.combinat.crystals.multisegments.InfinityCrystalOfMultisegments.Element.phi[5]>", line 1, in <module>
+        mg.f(Integer(1)).phi(Integer(0))
+      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/combinat/crystals/multisegments.py", line 436, in phi
+        return self.epsilon(i) + self.weight().scalar(h[i])
+      File "/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/combinat/root_system/weight_space.py", line 506, in scalar
+        return BR.sum(self[i] * c for (i, c) in lambdacheck)
+    TypeError: iter() returned non-iterator of type 'dict_items'
+```
 
 Author: Frédéric Chapoton
 
``````




---

archive/issue_comments_471917.json:
```json
{
    "body": "Changing status from needs_work to needs_info.",
    "created_at": "2018-03-14T16:33:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471917",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_work to needs_info.



---

archive/issue_comments_471918.json:
```json
{
    "body": "<a id='comment:4'></a>Similar issue here under python3:\n\n```\nsage: d = DisjointSet(4)\nsage: list(d)\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n<ipython-input-2-d0c0da2e41ad> in <module>()\n----> 1 list(d)\n\nTypeError: iter() returned non-iterator of type 'dict_values'\n```\nwhere the code is\n\n```\nreturn (<dict?>self.root_to_elements_dict()).itervalues()\n```\nthat under python2 returns\n\n```\nsage: d = DisjointSet(4)\nsage: iter(d)\n<dictionary-valueiterator object at 0x7f0306d3f578>\n```",
    "created_at": "2018-03-14T16:33:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471918",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'></a>Similar issue here under python3:

```
sage: d = DisjointSet(4)
sage: list(d)
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-2-d0c0da2e41ad> in <module>()
----> 1 list(d)

TypeError: iter() returned non-iterator of type 'dict_values'
```
where the code is

```
return (<dict?>self.root_to_elements_dict()).itervalues()
```
that under python2 returns

```
sage: d = DisjointSet(4)
sage: iter(d)
<dictionary-valueiterator object at 0x7f0306d3f578>
```



---

archive/issue_comments_471919.json:
```json
{
    "body": "Changing component from combinatorics to python3.",
    "created_at": "2018-03-14T16:36:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471919",
    "user": "https://github.com/fchapoton"
}
```

Changing component from combinatorics to python3.



---

archive/issue_comments_471920.json:
```json
{
    "body": "<a id='comment:7'></a>Sigh... I guess it has to be `iter(dict.itervalues())` or `yield from dict.itervalues()` but I guess that the former will be faster.",
    "created_at": "2018-03-14T19:20:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471920",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>Sigh... I guess it has to be `iter(dict.itervalues())` or `yield from dict.itervalues()` but I guess that the former will be faster.



---

archive/issue_comments_471921.json:
```json
{
    "body": "<a id='comment:8'></a>Eurk. Ugly enough, indeed. No hope that Cython could do that for us ?\n\nHow to find all instances of this problem ? Maybe by looking for pyx files containing both `__iter__` and `.iter[a-z]` ?",
    "created_at": "2018-03-14T19:45:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471921",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'></a>Eurk. Ugly enough, indeed. No hope that Cython could do that for us ?

How to find all instances of this problem ? Maybe by looking for pyx files containing both `__iter__` and `.iter[a-z]` ?



---

archive/issue_comments_471922.json:
```json
{
    "body": "<a id='comment:9'></a>Potential list\n\n```\ncrypto/boolean_function.pyx (\u2713)\nlibs/gap/element.pyx\nalgebras/letterplace/free_algebra_element_letterplace.pyx\nalgebras/lie_algebras/lie_algebra_element.pyx\nplot/plot3d/index_face_set.pyx\next/fast_eval.pyx\nnumerical/linear_functions.pyx\nmisc/weak_dict.pyx\nrings/polynomial/polydict.pyx\nrings/polynomial/laurent_polynomial.pyx\nrings/polynomial/pbori.pyx\nrings/polynomial/multi_polynomial.pyx\nrings/polynomial/polynomial_element.pyx\nrings/power_series_mpoly.pyx\nsets/disjoint_set.pyx (todo)\nquivers/algebra_elements.pyx (\u2713)\nmodules/with_basis/indexed_element.pyx\nsymbolic/expression.pyx\n```\n\nfound using\n\n```\ngrep -l \"def.*__iter__\" $(grep -l -R \"\\.iter[ivk]\" --include=*.pyx)\n```",
    "created_at": "2018-03-14T19:57:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471922",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'></a>Potential list

```
crypto/boolean_function.pyx (✓)
libs/gap/element.pyx
algebras/letterplace/free_algebra_element_letterplace.pyx
algebras/lie_algebras/lie_algebra_element.pyx
plot/plot3d/index_face_set.pyx
ext/fast_eval.pyx
numerical/linear_functions.pyx
misc/weak_dict.pyx
rings/polynomial/polydict.pyx
rings/polynomial/laurent_polynomial.pyx
rings/polynomial/pbori.pyx
rings/polynomial/multi_polynomial.pyx
rings/polynomial/polynomial_element.pyx
rings/power_series_mpoly.pyx
sets/disjoint_set.pyx (todo)
quivers/algebra_elements.pyx (✓)
modules/with_basis/indexed_element.pyx
symbolic/expression.pyx
```

found using

```
grep -l "def.*__iter__" $(grep -l -R "\.iter[ivk]" --include=*.pyx)
```



---

archive/issue_comments_471923.json:
```json
{
    "body": "<a id='comment:10'></a>There is no need to fix everything on one ticket. In any case, doctests will show what is broken.",
    "created_at": "2018-03-14T20:01:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471923",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>There is no need to fix everything on one ticket. In any case, doctests will show what is broken.



---

archive/issue_comments_471924.json:
```json
{
    "body": "<a id='comment:11'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:",
    "created_at": "2018-03-14T20:15:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471924",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:11'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:



---

archive/issue_comments_471925.json:
```json
{
    "body": "Changing commit from \"52eee770d21b90ea9616c9d213f73a7da7504612\" to \"bc7a60c9b4ba16d92a52df09f6930d5424652fb1\"",
    "created_at": "2018-03-14T20:15:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471925",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "52eee770d21b90ea9616c9d213f73a7da7504612" to "bc7a60c9b4ba16d92a52df09f6930d5424652fb1"



---

archive/issue_comments_471926.json:
```json
{
    "body": "<a id='comment:12'></a>ok, let us content ourselves here with 3 changes",
    "created_at": "2018-03-14T20:16:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471926",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:12'></a>ok, let us content ourselves here with 3 changes



---

archive/issue_comments_471927.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2018-03-14T20:16:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471927",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_471928.json:
```json
{
    "body": "<a id='comment:13'></a>bot is green",
    "created_at": "2018-03-15T08:06:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471928",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:13'></a>bot is green



---

archive/issue_comments_471929.json:
```json
{
    "body": "<a id='comment:14'></a>ping ?",
    "created_at": "2018-03-28T13:32:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471929",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'></a>ping ?



---

archive/issue_comments_471930.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-03-28T14:15:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471930",
    "user": "https://github.com/embray"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_471931.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Erik Bray\"",
    "created_at": "2018-03-28T14:15:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471931",
    "user": "https://github.com/embray"
}
```

Changing reviewer from "" to "Erik Bray"



---

archive/issue_comments_471932.json:
```json
{
    "body": "Changing branch from \"u/chapoton/24839\" to \"bc7a60c9b4ba16d92a52df09f6930d5424652fb1\"",
    "created_at": "2018-05-09T09:49:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471932",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/chapoton/24839" to "bc7a60c9b4ba16d92a52df09f6930d5424652fb1"



---

archive/issue_events_062430.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-09T09:49:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24839#event-62430"
}
```



---

archive/issue_comments_471933.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-05-09T09:49:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24839#issuecomment-471933",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
