# Issue 24361: Coxeter3 does not pass self checks

archive/issues_024124.json:
```json
{
    "body": "On my 32bit VM, coxeter3 does not pass self checks. Quickly looking at the log, it seems that it does not use the C++ that was built with Sage (`export SAGE_INSTALL_GCC='yes'`), but the one (older) shipped with the system. Log attached.\n\n\n\nCC:  @tscrim\n\nKeywords: sdl\n\nResolution: worksforme\n\nIssue created by migration from https://trac.sagemath.org/ticket/24361\n\n",
    "closed_at": "2018-02-26T14:23:52Z",
    "created_at": "2017-12-10T12:49:49Z",
    "labels": [
        "component: packages: optional",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Coxeter3 does not pass self checks",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24361",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```
On my 32bit VM, coxeter3 does not pass self checks. Quickly looking at the log, it seems that it does not use the C++ that was built with Sage (`export SAGE_INSTALL_GCC='yes'`), but the one (older) shipped with the system. Log attached.



CC:  @tscrim

Keywords: sdl

Resolution: worksforme

Issue created by migration from https://trac.sagemath.org/ticket/24361





---

archive/issue_comments_461222.json:
```json
{
    "body": "Attachment [coxeter3-1.1.log](tarball://root/attachments/some-uuid/ticket24361/coxeter3-1.1.log) by tmonteil created at 2017-12-10 12:50:07",
    "created_at": "2017-12-10T12:50:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24361#issuecomment-461222",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Attachment [coxeter3-1.1.log](tarball://root/attachments/some-uuid/ticket24361/coxeter3-1.1.log) by tmonteil created at 2017-12-10 12:50:07



---

archive/issue_comments_461223.json:
```json
{
    "body": "<a id='comment:1'></a>I don't manage to reproduce this.",
    "created_at": "2018-02-10T09:16:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24361#issuecomment-461223",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>I don't manage to reproduce this.



---

archive/issue_comments_461224.json:
```json
{
    "body": "Resolution: worksforme",
    "created_at": "2018-02-26T14:23:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24361#issuecomment-461224",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: worksforme



---

archive/issue_events_061596.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-26T14:23:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/24361",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24361#event-61596"
}
```



---

archive/issue_events_061597.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-26T14:23:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24361",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24361#event-61597"
}
```



---

archive/issue_comments_461225.json:
```json
{
    "body": "Changing keywords from \"\" to \"sdl\".",
    "created_at": "2019-08-27T19:45:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24361#issuecomment-461225",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Changing keywords from "" to "sdl".
