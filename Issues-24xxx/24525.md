# Issue 24525: deprecate is_RealNumber, is_AlgebraicNumber, is_RealIntervalFieldElement, ...

archive/issues_024288.json:
```json
{
    "assignees": [],
    "body": "We deprecate `is_RealNumber`, `is_ComplexNumber`, `is_AlgebraicNumber`, `is_RealIntervalFieldElement`, ...\n\n`git grep 'is_.*Number('` and `git grep -E 'is_(Real|Complex).*Element[(]'` only shows a small number of uses.\n\nWe replace all uses either by \n- `isinstance` tests with the element class (if the module providing it is imported already) or\n- `isinstance` tests of the parent with an abc defined in `sage.rings.abc`\n\nSee also:\n- #24457\n- discussion in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/wEzb0awmyN0)\n\n\nDepends on #32566\n\nCC:  @tscrim\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24525_\n\n",
    "created_at": "2018-01-11T18:47:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "deprecate is_RealNumber, is_AlgebraicNumber, is_RealIntervalFieldElement, ...",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/24525",
    "user": "https://github.com/videlec"
}
```
We deprecate `is_RealNumber`, `is_ComplexNumber`, `is_AlgebraicNumber`, `is_RealIntervalFieldElement`, ...

`git grep 'is_.*Number('` and `git grep -E 'is_(Real|Complex).*Element[(]'` only shows a small number of uses.

We replace all uses either by 
- `isinstance` tests with the element class (if the module providing it is imported already) or
- `isinstance` tests of the parent with an abc defined in `sage.rings.abc`

See also:
- #24457
- discussion in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/wEzb0awmyN0)


Depends on #32566

CC:  @tscrim

_Issue created by migration from https://trac.sagemath.org/ticket/24525_





---

archive/issue_events_321690.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-11T18:47:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321690"
}
```



---

archive/issue_events_321691.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-11T18:47:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321691"
}
```



---

archive/issue_events_321692.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-11T18:47:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321692"
}
```



---

archive/issue_events_321693.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-11T18:47:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321693"
}
```



---

archive/issue_comments_374067.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+As part of #24457 we deprecate `is_RealField` and `is_RealNumber` in favor of `isinstance`.\n \n+See also the discussion in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/wEzb0awmyN0)\n``````\n",
    "created_at": "2018-01-11T19:08:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24525#issuecomment-374067",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+As part of #24457 we deprecate `is_RealField` and `is_RealNumber` in favor of `isinstance`.
 
+See also the discussion in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/wEzb0awmyN0)
``````




---

archive/issue_comments_374068.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\n#32610 takes care of `is_RealField`",
    "created_at": "2021-10-02T19:08:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24525#issuecomment-374068",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

#32610 takes care of `is_RealField`



---

archive/issue_events_321694.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-03T18:20:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321694"
}
```



---

archive/issue_events_321695.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-03T18:20:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321695"
}
```



---

archive/issue_comments_374069.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,12 @@\n-As part of #24457 we deprecate `is_RealField` and `is_RealNumber` in favor of `isinstance`.\n+We deprecate `is_RealNumber`, `is_ComplexNumber`, `is_AlgebraicNumber`, `is_RealIntervalFieldElement`, ...\n \n-See also the discussion in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/wEzb0awmyN0)\n+`git grep 'is_.*Number('` and `git grep -E 'is_(Real|Complex).*Element[(]'` only shows a small number of uses.\n+\n+We replace all uses either by \n+- `isinstance` tests with the element class (if the module providing it is imported already) or\n+- `isinstance` tests of the parent with an abc defined in `sage.rings.abc`\n+\n+See also:\n+- #24457\n+- discussion in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/wEzb0awmyN0)\n+\n``````\n",
    "created_at": "2021-10-03T18:20:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24525#issuecomment-374069",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,12 @@
-As part of #24457 we deprecate `is_RealField` and `is_RealNumber` in favor of `isinstance`.
+We deprecate `is_RealNumber`, `is_ComplexNumber`, `is_AlgebraicNumber`, `is_RealIntervalFieldElement`, ...
 
-See also the discussion in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/wEzb0awmyN0)
+`git grep 'is_.*Number('` and `git grep -E 'is_(Real|Complex).*Element[(]'` only shows a small number of uses.
+
+We replace all uses either by 
+- `isinstance` tests with the element class (if the module providing it is imported already) or
+- `isinstance` tests of the parent with an abc defined in `sage.rings.abc`
+
+See also:
+- #24457
+- discussion in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/wEzb0awmyN0)
+
``````




---

archive/issue_events_321696.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-03T18:20:56Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "title_is": "deprecate is_RealNumber, is_AlgebraicNumber, is_RealIntervalFieldElement, ...",
    "title_was": "deprecate is_RealField / is_RealNumber",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321696"
}
```



---

archive/issue_comments_374070.json:
```json
{
    "body": "Changed dependencies from **#24524** to **#32566**",
    "created_at": "2021-10-03T18:20:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24525#issuecomment-374070",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#24524** to **#32566**



---

archive/issue_events_321697.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321697"
}
```



---

archive/issue_events_321698.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321698"
}
```



---

archive/issue_events_321699.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321699"
}
```



---

archive/issue_events_321700.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321700"
}
```



---

archive/issue_events_321701.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321701"
}
```



---

archive/issue_events_321702.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321702"
}
```



---

archive/issue_events_321703.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321703"
}
```



---

archive/issue_events_321704.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24525",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24525#event-321704"
}
```
