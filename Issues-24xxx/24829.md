# Issue 24829: zero_matrix() should pass the correct zero

archive/issues_024592.json:
```json
{
    "assignees": [],
    "body": "The method `zero_matrix()` passes `coerce=False` to the matrix class constructor. If so, it should pass the correct zero of the base ring, not `int(0)`.\n\nThis bug causes segfaults in #24742 because the `coerce` flag is honored in more cases.\n\nCC:  @tscrim\n\nBranch/Commit: **[17cd4fb](https://github.com/sagemath/sagetrac-mirror/commit/17cd4fb1caba6ade402ad542abbd14c05a20ea09)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24829_\n\n",
    "closed_at": "2018-02-25T19:42:10Z",
    "created_at": "2018-02-23T21:52:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "zero_matrix() should pass the correct zero",
    "type": "issue",
    "updated_at": "2018-02-25T19:42:10Z",
    "url": "https://github.com/sagemath/sage/issues/24829",
    "user": "https://github.com/jdemeyer"
}
```
The method `zero_matrix()` passes `coerce=False` to the matrix class constructor. If so, it should pass the correct zero of the base ring, not `int(0)`.

This bug causes segfaults in #24742 because the `coerce` flag is honored in more cases.

CC:  @tscrim

Branch/Commit: **[17cd4fb](https://github.com/sagemath/sagetrac-mirror/commit/17cd4fb1caba6ade402ad542abbd14c05a20ea09)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24829_





---

archive/issue_events_325597.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-23T21:52:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24829#event-325597"
}
```



---

archive/issue_events_325598.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-23T21:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24829#event-325598"
}
```



---

archive/issue_events_325599.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-23T21:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24829#event-325599"
}
```



---

archive/issue_events_325600.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-23T21:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24829#event-325600"
}
```



---

archive/issue_comments_380137.json:
```json
{
    "body": "Branch: **[u/jdemeyer/zero_matrix___should_pass_the_correct_zero](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/zero_matrix___should_pass_the_correct_zero)**",
    "created_at": "2018-02-23T21:54:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24829#issuecomment-380137",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/zero_matrix___should_pass_the_correct_zero](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/zero_matrix___should_pass_the_correct_zero)**



---

archive/issue_comments_380138.json:
```json
{
    "body": "Commit: **[17cd4fb](https://github.com/sagemath/sagetrac-mirror/commit/17cd4fb1caba6ade402ad542abbd14c05a20ea09)**",
    "created_at": "2018-02-23T21:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24829#issuecomment-380138",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[17cd4fb](https://github.com/sagemath/sagetrac-mirror/commit/17cd4fb1caba6ade402ad542abbd14c05a20ea09)**



---

archive/issue_comments_380139.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/17cd4fb1caba6ade402ad542abbd14c05a20ea09\">17cd4fb</a></td><td><code>zero_matrix() should pass the correct zero</code></td></tr></table>\n",
    "created_at": "2018-02-23T21:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24829#issuecomment-380139",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/17cd4fb1caba6ade402ad542abbd14c05a20ea09">17cd4fb</a></td><td><code>zero_matrix() should pass the correct zero</code></td></tr></table>




---

archive/issue_events_325601.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-23T21:57:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24829#event-325601"
}
```



---

archive/issue_comments_380140.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2018-02-23T22:30:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24829#issuecomment-380140",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_380141.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-02-23T22:30:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24829#issuecomment-380141",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_325602.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-02-23T22:30:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24829#event-325602"
}
```



---

archive/issue_events_325603.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-02-23T22:30:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24829#event-325603"
}
```



---

archive/issue_events_325604.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-25T19:42:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24829#event-325604"
}
```



---

archive/issue_events_325605.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "87d791e0aa60e4bc6bb5c4ecfcbfa4bc041d2aab",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-25T19:42:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24829#event-325605"
}
```



---

archive/issue_comments_380142.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/zero_matrix___should_pass_the_correct_zero](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/zero_matrix___should_pass_the_correct_zero)** to **[17cd4fb](https://github.com/sagemath/sagetrac-mirror/commit/17cd4fb1caba6ade402ad542abbd14c05a20ea09)**",
    "created_at": "2018-02-25T19:42:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24829#issuecomment-380142",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/zero_matrix___should_pass_the_correct_zero](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/zero_matrix___should_pass_the_correct_zero)** to **[17cd4fb](https://github.com/sagemath/sagetrac-mirror/commit/17cd4fb1caba6ade402ad542abbd14c05a20ea09)**
