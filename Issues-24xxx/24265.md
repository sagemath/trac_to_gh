# Issue 24265: py3: fixes to sage.misc.fpickle module

archive/issues_024028.json:
```json
{
    "assignees": [],
    "body": "Miscellaneous fixes to the `sage.misc.fpickle` module for Python 3 support.\n\nI confirmed that all the module's tests passed on Python 3.  This also fixed some errors in tests that relied on this indirectly (e.g. through pickling of methods).\n\nBranch/Commit: **[67dd604](https://github.com/sagemath/sagetrac-mirror/commit/67dd604e78cbe255118f3366cec6bf0aa2d08183)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24265_\n\n",
    "closed_at": "2017-12-13T17:38:12Z",
    "created_at": "2017-11-22T13:05:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fixes to sage.misc.fpickle module",
    "type": "issue",
    "updated_at": "2017-12-13T17:38:12Z",
    "url": "https://github.com/sagemath/sage/issues/24265",
    "user": "https://github.com/embray"
}
```
Miscellaneous fixes to the `sage.misc.fpickle` module for Python 3 support.

I confirmed that all the module's tests passed on Python 3.  This also fixed some errors in tests that relied on this indirectly (e.g. through pickling of methods).

Branch/Commit: **[67dd604](https://github.com/sagemath/sagetrac-mirror/commit/67dd604e78cbe255118f3366cec6bf0aa2d08183)**

Reviewer: **Frédéric Chapoton**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/24265_





---

archive/issue_events_318604.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-22T13:05:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24265#event-318604"
}
```



---

archive/issue_events_318605.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-22T13:05:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24265#event-318605"
}
```



---

archive/issue_events_318606.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-22T13:05:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24265#event-318606"
}
```



---

archive/issue_events_318607.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-22T13:05:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24265#event-318607"
}
```



---

archive/issue_events_318608.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-22T13:06:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24265#event-318608"
}
```



---

archive/issue_comments_370026.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks good, but shall we keep the call to the attribute \"im_name\" ? I thought it also disappeared in py3.",
    "created_at": "2017-11-22T20:43:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24265#issuecomment-370026",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
Looks good, but shall we keep the call to the attribute "im_name" ? I thought it also disappeared in py3.



---

archive/issue_comments_370027.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis code isn't accessing an `im_name` attribute anywhere that I can see.  It's just using `im_name` as a variable name.",
    "created_at": "2017-11-23T09:47:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24265#issuecomment-370027",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'>Comment 3:</a>
This code isn't accessing an `im_name` attribute anywhere that I can see.  It's just using `im_name` as a variable name.



---

archive/issue_events_318609.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-23T12:42:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24265#event-318609"
}
```



---

archive/issue_events_318610.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-23T12:42:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24265#event-318610"
}
```



---

archive/issue_comments_370028.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nok, then",
    "created_at": "2017-11-23T12:42:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24265#issuecomment-370028",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
ok, then



---

archive/issue_comments_370029.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2017-11-23T12:42:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24265#issuecomment-370029",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_370030.json:
```json
{
    "body": "Changed branch from **[u/embray/python3/fpickle-fixes](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/fpickle-fixes)** to **[67dd604](https://github.com/sagemath/sagetrac-mirror/commit/67dd604e78cbe255118f3366cec6bf0aa2d08183)**",
    "created_at": "2017-12-13T17:38:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24265#issuecomment-370030",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/python3/fpickle-fixes](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/fpickle-fixes)** to **[67dd604](https://github.com/sagemath/sagetrac-mirror/commit/67dd604e78cbe255118f3366cec6bf0aa2d08183)**



---

archive/issue_events_318611.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-13T17:38:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24265#event-318611"
}
```



---

archive/issue_events_318612.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c79bf6ed78d1293456853bdf5eca75ed6501c54f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-13T17:38:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24265",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24265#event-318612"
}
```
