# Issue 24597: Properly set LD_LIBRARY_PATH in spkg-build for python2/3 on SunOS

archive/issues_024360.json:
```json
{
    "assignees": [],
    "body": "this is to allow Solaris build to go through, see sage-devel\n[post](https://groups.google.com/d/msg/sage-devel/DbpVwIuERHU/P70ADfLyAQAJ)\n\nPart of #24596\n\nCC:  @embray @jdemeyer\n\nBranch/Commit: **[364ec35](https://github.com/sagemath/sagetrac-mirror/commit/364ec35f1cbdab920e52c44341b8c19c8183b818)**\n\nReviewer: **Erik Bray**\n\nAuthor: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24597_\n\n",
    "closed_at": "2018-02-02T12:05:51Z",
    "created_at": "2018-01-26T16:21:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Properly set LD_LIBRARY_PATH in spkg-build for python2/3 on SunOS",
    "type": "issue",
    "updated_at": "2018-02-02T12:05:51Z",
    "url": "https://github.com/sagemath/sage/issues/24597",
    "user": "https://github.com/dimpase"
}
```
this is to allow Solaris build to go through, see sage-devel
[post](https://groups.google.com/d/msg/sage-devel/DbpVwIuERHU/P70ADfLyAQAJ)

Part of #24596

CC:  @embray @jdemeyer

Branch/Commit: **[364ec35](https://github.com/sagemath/sagetrac-mirror/commit/364ec35f1cbdab920e52c44341b8c19c8183b818)**

Reviewer: **Erik Bray**

Author: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/24597_





---

archive/issue_events_322633.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-01-26T16:21:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322633"
}
```



---

archive/issue_events_322634.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-01-26T16:21:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322634"
}
```



---

archive/issue_events_322635.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-01-26T16:21:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322635"
}
```



---

archive/issue_events_322636.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-01-26T16:21:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322636"
}
```



---

archive/issue_events_322637.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-26T16:26:52Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "title_is": "Properly set LD_LIBRARY_PATH in spkg-build for python2/3 on SunOS",
    "title_was": "properly unset LD_LIBRARY_PATH in spkg-build for python2/3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322637"
}
```



---

archive/issue_events_322638.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2018-01-26T16:36:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322638"
}
```



---

archive/issue_comments_375680.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/364ec35f1cbdab920e52c44341b8c19c8183b818\">364ec35</a></td><td><code>LD_LIBRARY_PATH=\".\" everywhere except OSX</code></td></tr></table>\n",
    "created_at": "2018-01-26T16:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24597#issuecomment-375680",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/364ec35f1cbdab920e52c44341b8c19c8183b818">364ec35</a></td><td><code>LD_LIBRARY_PATH="." everywhere except OSX</code></td></tr></table>




---

archive/issue_comments_375681.json:
```json
{
    "body": "Branch: **[u/dimpase/ldlibpath](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/ldlibpath)**",
    "created_at": "2018-01-26T16:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24597#issuecomment-375681",
    "user": "https://github.com/dimpase"
}
```

Branch: **[u/dimpase/ldlibpath](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/ldlibpath)**



---

archive/issue_comments_375682.json:
```json
{
    "body": "Commit: **[364ec35](https://github.com/sagemath/sagetrac-mirror/commit/364ec35f1cbdab920e52c44341b8c19c8183b818)**",
    "created_at": "2018-01-26T16:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24597#issuecomment-375682",
    "user": "https://github.com/dimpase"
}
```

Commit: **[364ec35](https://github.com/sagemath/sagetrac-mirror/commit/364ec35f1cbdab920e52c44341b8c19c8183b818)**



---

archive/issue_comments_375683.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,4 @@\n this is to allow Solaris build to go through, see sage-devel\n [post](https://groups.google.com/d/msg/sage-devel/DbpVwIuERHU/P70ADfLyAQAJ)\n+\n+Part of #24596\n``````\n",
    "created_at": "2018-01-26T16:39:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24597#issuecomment-375683",
    "user": "https://github.com/dimpase"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,4 @@
 this is to allow Solaris build to go through, see sage-devel
 [post](https://groups.google.com/d/msg/sage-devel/DbpVwIuERHU/P70ADfLyAQAJ)
+
+Part of #24596
``````




---

archive/issue_events_322639.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-30T12:27:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322639"
}
```



---

archive/issue_events_322640.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-30T12:27:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322640"
}
```



---

archive/issue_comments_375684.json:
```json
{
    "body": "Reviewer: **Erik Bray**",
    "created_at": "2018-01-30T12:27:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24597#issuecomment-375684",
    "user": "https://github.com/embray"
}
```

Reviewer: **Erik Bray**



---

archive/issue_comments_375685.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLooks good to me.  Setting `LD_LIBRARY_PATH` on Cygwin is mostly pointless (it only affects `dlopen()`) but it should be harmless all the same.",
    "created_at": "2018-01-30T12:27:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24597#issuecomment-375685",
    "user": "https://github.com/embray"
}
```

<div id="comment:5" align="right">Comment 5</div>

Looks good to me.  Setting `LD_LIBRARY_PATH` on Cygwin is mostly pointless (it only affects `dlopen()`) but it should be harmless all the same.



---

archive/issue_comments_375686.json:
```json
{
    "body": "Changed branch from **[u/dimpase/ldlibpath](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/ldlibpath)** to **[364ec35](https://github.com/sagemath/sagetrac-mirror/commit/364ec35f1cbdab920e52c44341b8c19c8183b818)**",
    "created_at": "2018-02-02T12:05:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24597#issuecomment-375686",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dimpase/ldlibpath](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/ldlibpath)** to **[364ec35](https://github.com/sagemath/sagetrac-mirror/commit/364ec35f1cbdab920e52c44341b8c19c8183b818)**



---

archive/issue_events_322641.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-02T12:05:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322641"
}
```



---

archive/issue_events_322642.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e7fe097bf30e96940ae21101591b31ca5b2cf6b0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-02T12:05:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24597",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24597#event-322642"
}
```
