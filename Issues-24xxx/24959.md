# Issue 24959: slightly faster tamari_pred

archive/issues_024722.json:
```json
{
    "assignees": [],
    "body": "\n\nBranch/Commit: **[a0ef3cf](https://github.com/sagemath/sagetrac-mirror/commit/a0ef3cfa0cd1defdb83646f15394a0d7caea67d3)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24959_\n\n",
    "closed_at": "2018-03-22T19:22:38Z",
    "created_at": "2018-03-12T16:58:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "slightly faster tamari_pred",
    "type": "issue",
    "updated_at": "2018-03-22T19:22:38Z",
    "url": "https://github.com/sagemath/sage/issues/24959",
    "user": "https://github.com/fchapoton"
}
```


Branch/Commit: **[a0ef3cf](https://github.com/sagemath/sagetrac-mirror/commit/a0ef3cfa0cd1defdb83646f15394a0d7caea67d3)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/24959_





---

archive/issue_events_327243.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-03-12T16:58:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24959#event-327243"
}
```



---

archive/issue_events_327244.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-03-12T16:58:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24959#event-327244"
}
```



---

archive/issue_events_327245.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-03-12T16:58:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24959#event-327245"
}
```



---

archive/issue_events_327246.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-03-12T16:58:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24959#event-327246"
}
```



---

archive/issue_comments_382743.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nBefore\n\n```\nsage: %timeit [t.tamari_pred() for t in BinaryTrees(7)]\n10 loops, best of 3: 49.8 ms per loop\n```\n\nAfter\n\n```\nsage: %timeit [t.tamari_pred() for t in BinaryTrees(7)]\n10 loops, best of 3: 41.9 ms per loop\n```",
    "created_at": "2018-03-12T16:59:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24959#issuecomment-382743",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

Before

```
sage: %timeit [t.tamari_pred() for t in BinaryTrees(7)]
10 loops, best of 3: 49.8 ms per loop
```

After

```
sage: %timeit [t.tamari_pred() for t in BinaryTrees(7)]
10 loops, best of 3: 41.9 ms per loop
```



---

archive/issue_comments_382744.json:
```json
{
    "body": "Branch: **[u/chapoton/24959](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/24959)**",
    "created_at": "2018-03-12T17:00:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24959#issuecomment-382744",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/24959](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/24959)**



---

archive/issue_events_327247.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-03-12T17:00:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24959#event-327247"
}
```



---

archive/issue_comments_382745.json:
```json
{
    "body": "Commit: **[a0ef3cf](https://github.com/sagemath/sagetrac-mirror/commit/a0ef3cfa0cd1defdb83646f15394a0d7caea67d3)**",
    "created_at": "2018-03-12T17:00:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24959#issuecomment-382745",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[a0ef3cf](https://github.com/sagemath/sagetrac-mirror/commit/a0ef3cfa0cd1defdb83646f15394a0d7caea67d3)**



---

archive/issue_comments_382746.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a0ef3cfa0cd1defdb83646f15394a0d7caea67d3\">a0ef3cf</a></td><td><code>trac 24959 slightly better tamari_pred</code></td></tr></table>\n",
    "created_at": "2018-03-12T17:00:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24959#issuecomment-382746",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a0ef3cfa0cd1defdb83646f15394a0d7caea67d3">a0ef3cf</a></td><td><code>trac 24959 slightly better tamari_pred</code></td></tr></table>




---

archive/issue_events_327248.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-03-13T00:17:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24959#event-327248"
}
```



---

archive/issue_events_327249.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-03-13T00:17:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24959#event-327249"
}
```



---

archive/issue_comments_382747.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2018-03-13T00:17:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24959#issuecomment-382747",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_382748.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-03-13T00:17:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24959#issuecomment-382748",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_382749.json:
```json
{
    "body": "Changed branch from **[u/chapoton/24959](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/24959)** to **[a0ef3cf](https://github.com/sagemath/sagetrac-mirror/commit/a0ef3cfa0cd1defdb83646f15394a0d7caea67d3)**",
    "created_at": "2018-03-22T19:22:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24959#issuecomment-382749",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/24959](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/24959)** to **[a0ef3cf](https://github.com/sagemath/sagetrac-mirror/commit/a0ef3cfa0cd1defdb83646f15394a0d7caea67d3)**



---

archive/issue_events_327250.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-22T19:22:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24959#event-327250"
}
```



---

archive/issue_events_327251.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "43c5a4ab691b3de2bcec74d11d3fd20bb838988e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-03-22T19:22:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24959",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24959#event-327251"
}
```
