# Issue 24284: Exclude some functions with arg restrictions from random testing

archive/issues_024047.json:
```json
{
    "assignees": [],
    "body": "Rare doctest errors can happen in `random_expression` if functions are selected that raise errors on wrong argument type. This ticket excludes all (up to now) known such functions from being used in `random_expression`.\n\nBranch/Commit: **[6559c5e](https://github.com/sagemath/sagetrac-mirror/commit/6559c5edcd92b640d6b2c74e0cf670d6cbc4d3fb)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24284_\n\n",
    "closed_at": "2017-12-22T23:29:51Z",
    "created_at": "2017-11-27T06:58:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Exclude some functions with arg restrictions from random testing",
    "type": "issue",
    "updated_at": "2017-12-22T23:29:51Z",
    "url": "https://github.com/sagemath/sage/issues/24284",
    "user": "https://github.com/rwst"
}
```
Rare doctest errors can happen in `random_expression` if functions are selected that raise errors on wrong argument type. This ticket excludes all (up to now) known such functions from being used in `random_expression`.

Branch/Commit: **[6559c5e](https://github.com/sagemath/sagetrac-mirror/commit/6559c5edcd92b640d6b2c74e0cf670d6cbc4d3fb)**

Reviewer: **Volker Braun**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/24284_





---

archive/issue_events_318845.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-27T06:58:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318845"
}
```



---

archive/issue_events_318846.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-27T06:58:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318846"
}
```



---

archive/issue_events_318847.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-27T06:58:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318847"
}
```



---

archive/issue_events_318848.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-27T06:58:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318848"
}
```



---

archive/issue_comments_370343.json:
```json
{
    "body": "Branch: **[u/rws/exclude_some_functions_with_arg_restrictions_from_random_testing](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/exclude_some_functions_with_arg_restrictions_from_random_testing)**",
    "created_at": "2017-11-27T07:03:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24284#issuecomment-370343",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/exclude_some_functions_with_arg_restrictions_from_random_testing](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/exclude_some_functions_with_arg_restrictions_from_random_testing)**



---

archive/issue_events_318849.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-11-27T07:05:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318849"
}
```



---

archive/issue_comments_370344.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2017-11-27T07:05:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24284#issuecomment-370344",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_370345.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6559c5edcd92b640d6b2c74e0cf670d6cbc4d3fb\">6559c5e</a></td><td><code>24282: Exclude some functions with arg restrictions from random testing</code></td></tr></table>\n",
    "created_at": "2017-11-27T07:05:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24284#issuecomment-370345",
    "user": "https://github.com/rwst"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6559c5edcd92b640d6b2c74e0cf670d6cbc4d3fb">6559c5e</a></td><td><code>24282: Exclude some functions with arg restrictions from random testing</code></td></tr></table>




---

archive/issue_comments_370346.json:
```json
{
    "body": "Commit: **[6559c5e](https://github.com/sagemath/sagetrac-mirror/commit/6559c5edcd92b640d6b2c74e0cf670d6cbc4d3fb)**",
    "created_at": "2017-11-27T07:05:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24284#issuecomment-370346",
    "user": "https://github.com/rwst"
}
```

Commit: **[6559c5e](https://github.com/sagemath/sagetrac-mirror/commit/6559c5edcd92b640d6b2c74e0cf670d6cbc4d3fb)**



---

archive/issue_events_318850.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-18T08:06:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318850"
}
```



---

archive/issue_events_318851.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-18T08:06:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318851"
}
```



---

archive/issue_events_318852.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-18T09:57:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318852"
}
```



---

archive/issue_events_318853.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-18T09:57:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318853"
}
```



---

archive/issue_comments_370347.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2017-12-18T09:57:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24284#issuecomment-370347",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_318854.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-22T23:29:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318854"
}
```



---

archive/issue_events_318855.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f08412d9cfaea5d329c80c4caa8027bdd193a676",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-22T23:29:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24284#event-318855"
}
```



---

archive/issue_comments_370348.json:
```json
{
    "body": "Changed branch from **[u/rws/exclude_some_functions_with_arg_restrictions_from_random_testing](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/exclude_some_functions_with_arg_restrictions_from_random_testing)** to **[6559c5e](https://github.com/sagemath/sagetrac-mirror/commit/6559c5edcd92b640d6b2c74e0cf670d6cbc4d3fb)**",
    "created_at": "2017-12-22T23:29:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24284#issuecomment-370348",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/exclude_some_functions_with_arg_restrictions_from_random_testing](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/exclude_some_functions_with_arg_restrictions_from_random_testing)** to **[6559c5e](https://github.com/sagemath/sagetrac-mirror/commit/6559c5edcd92b640d6b2c74e0cf670d6cbc4d3fb)**
