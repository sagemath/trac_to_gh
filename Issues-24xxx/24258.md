# Issue 24258: py3: support <type '...'> expected output in doctests on Python 3

archive/issues_024021.json:
```json
{
    "assignees": [],
    "body": "On Python 3 all classes, including built-in types, are represented as `<class '...'>` instead of `<type '...'>` as Python 2 uses for built-in types.\n\nThis normalizes the expected doctest output on Python 3 to account for this difference.\n\nWhenever Sage switches to Python 3 as a default it would be simple to also reverse the sense of this normalization, and update the doctests to expect `<class '...'>`.\n\nThis replaces #24228, #24229, #24230, and #24233.\n\nCC:  @fchapoton\n\nBranch: **[514ed93](https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24258_\n\n",
    "closed_at": "2018-01-18T18:09:22Z",
    "created_at": "2017-11-21T12:25:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: support <type '...'> expected output in doctests on Python 3",
    "type": "issue",
    "updated_at": "2018-09-07T10:18:37Z",
    "url": "https://github.com/sagemath/sage/issues/24258",
    "user": "https://github.com/embray"
}
```
On Python 3 all classes, including built-in types, are represented as `<class '...'>` instead of `<type '...'>` as Python 2 uses for built-in types.

This normalizes the expected doctest output on Python 3 to account for this difference.

Whenever Sage switches to Python 3 as a default it would be simple to also reverse the sense of this normalization, and update the doctests to expect `<class '...'>`.

This replaces #24228, #24229, #24230, and #24233.

CC:  @fchapoton

Branch: **[514ed93](https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9)**

Reviewer: **Frédéric Chapoton**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/24258_





---

archive/issue_events_318474.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-21T12:25:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318474"
}
```



---

archive/issue_events_318475.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-21T12:25:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318475"
}
```



---

archive/issue_events_318476.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-21T12:25:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318476"
}
```



---

archive/issue_events_318477.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-21T12:25:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318477"
}
```



---

archive/issue_comments_369838.json:
```json
{
    "body": "Dependencies: **#24257**",
    "created_at": "2017-11-21T12:26:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369838",
    "user": "https://github.com/embray"
}
```

Dependencies: **#24257**



---

archive/issue_events_318478.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-11-21T12:26:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318478"
}
```



---

archive/issue_comments_369839.json:
```json
{
    "body": "Replying to [ticket:24258 embray]:\n> Whenever Sage switches to Python 3 as a default it would be simple to also reverse the sense of this normalization, and update the doctests to expect `<class '...'>`.\n\nWouldn't it make sense to do that now, i.e. to replace `<type ` by `<class `? That would be more future-proof.",
    "created_at": "2017-11-22T08:07:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369839",
    "user": "https://github.com/jdemeyer"
}
```

Replying to [ticket:24258 embray]:
> Whenever Sage switches to Python 3 as a default it would be simple to also reverse the sense of this normalization, and update the doctests to expect `<class '...'>`.

Wouldn't it make sense to do that now, i.e. to replace `<type ` by `<class `? That would be more future-proof.



---

archive/issue_comments_369840.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReplying to [@jdemeyer](#comment%3A3):\n> Replying to [ticket:24258 embray]:\n> > Whenever Sage switches to Python 3 as a default it would be simple to also reverse the sense of this normalization, and update the doctests to expect `<class '...'>`.\n\n> \n> Wouldn't it make sense to do that now, i.e. to replace `<type ` by `<class `? That would be more future-proof.\n\n*nod* that was my original plan, but I decided:\n\na) This would be a less disruptive change for now--just a small change in one module vs. a small change in many\n\nb) For the time being Python 2 is still the default for Sage, so changing all the existing tests (particularly \"EXAMPLE\" docs) could confuse readers.",
    "created_at": "2017-11-22T12:52:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369840",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

Replying to [@jdemeyer](#comment%3A3):
> Replying to [ticket:24258 embray]:
> > Whenever Sage switches to Python 3 as a default it would be simple to also reverse the sense of this normalization, and update the doctests to expect `<class '...'>`.

> 
> Wouldn't it make sense to do that now, i.e. to replace `<type ` by `<class `? That would be more future-proof.

*nod* that was my original plan, but I decided:

a) This would be a less disruptive change for now--just a small change in one module vs. a small change in many

b) For the time being Python 2 is still the default for Sage, so changing all the existing tests (particularly "EXAMPLE" docs) could confuse readers.



---

archive/issue_events_318479.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-28T10:51:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318479"
}
```



---

archive/issue_events_318480.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-28T10:51:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318480"
}
```



---

archive/issue_comments_369841.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLots of breakage on the patchbot.",
    "created_at": "2017-11-28T10:51:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369841",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">Comment 5</div>

Lots of breakage on the patchbot.



---

archive/issue_comments_369842.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@embray](#comment%3A4):\n\n> a) This would be a less disruptive change for now--just a small change in one module vs. a small change in many\n\nIf you plan a disruptive change anyway when we switch to Python 3, that argument makes little sense. Why is a disruptive change later better than a disruptive change now.\n\n> b) For the time being Python 2 is still the default for Sage, so changing all the existing tests (particularly \"EXAMPLE\" docs) could confuse readers.\n\nFair enough.",
    "created_at": "2017-11-28T10:56:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369842",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@embray](#comment%3A4):

> a) This would be a less disruptive change for now--just a small change in one module vs. a small change in many

If you plan a disruptive change anyway when we switch to Python 3, that argument makes little sense. Why is a disruptive change later better than a disruptive change now.

> b) For the time being Python 2 is still the default for Sage, so changing all the existing tests (particularly "EXAMPLE" docs) could confuse readers.

Fair enough.



---

archive/issue_comments_369843.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nI believe most of the (relevant) failures are actually due to #24257, where they've since been fixed.  I can rebase this on top of that....",
    "created_at": "2017-11-28T11:06:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369843",
    "user": "https://github.com/embray"
}
```

<div id="comment:7" align="right">Comment 7</div>

I believe most of the (relevant) failures are actually due to #24257, where they've since been fixed.  I can rebase this on top of that....



---

archive/issue_comments_369844.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6a9e6cbd7d6b02cc11b37b8beddfb8ed2f5a55fe\">6a9e6cb</a></td><td><code>Remove 'from `__future__` import unicode_literals from some modules.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1add357c497b6630fb4a5c022f4caec6318a5ee0\">1add357</a></td><td><code>Add a doctest output fixup to replace <type '...'> with <class '...'> so that</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e12b67fda63bf721cf519c4553c5abef0e8553ef\">e12b67f</a></td><td><code>Fix docstring--the normalization is currently not performed on Python 2</code></td></tr></table>\n",
    "created_at": "2017-11-28T11:12:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369844",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6a9e6cbd7d6b02cc11b37b8beddfb8ed2f5a55fe">6a9e6cb</a></td><td><code>Remove 'from `__future__` import unicode_literals from some modules.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1add357c497b6630fb4a5c022f4caec6318a5ee0">1add357</a></td><td><code>Add a doctest output fixup to replace <type '...'> with <class '...'> so that</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e12b67fda63bf721cf519c4553c5abef0e8553ef">e12b67f</a></td><td><code>Fix docstring--the normalization is currently not performed on Python 2</code></td></tr></table>




---

archive/issue_comments_369845.json:
```json
{
    "body": "Changed commit from **[b5ba2ed](https://github.com/sagemath/sagetrac-mirror/commit/b5ba2ede2997b247d88389623adde2e0524580cc)** to **[e12b67f](https://github.com/sagemath/sagetrac-mirror/commit/e12b67fda63bf721cf519c4553c5abef0e8553ef)**",
    "created_at": "2017-11-28T11:12:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369845",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b5ba2ed](https://github.com/sagemath/sagetrac-mirror/commit/b5ba2ede2997b247d88389623adde2e0524580cc)** to **[e12b67f](https://github.com/sagemath/sagetrac-mirror/commit/e12b67fda63bf721cf519c4553c5abef0e8553ef)**



---

archive/issue_comments_369846.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nA similar, though not exactly the same difference I've noticed is that on Python 3 unbound methods of classes (which are just normal functions) are repr'd with their `__qualname__` which includes the class name.  E.g.\n\n```\n>>> class Foo:\n...     def a(self): pass\n...\n>>> Foo.a\n<function Foo.a at 0x7fe3d04c9b20>\n```\n\nSo some tests that happen to rely on such output fail on python 3.\n\nThe tests could be rewritten, but it might be nice to automatically account for this difference.  In principle there's a little advantage to having the `__qualname__` in the output since we can also also see in what class the function was defined, but in the meantime we don't lose anything over what we already have by ignoring it.\n\nI could either add that to this ticket, or add a new one.  It might be worth adding to this one since the same issue applies to nested classes.",
    "created_at": "2017-11-28T11:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369846",
    "user": "https://github.com/embray"
}
```

<div id="comment:9" align="right">Comment 9</div>

A similar, though not exactly the same difference I've noticed is that on Python 3 unbound methods of classes (which are just normal functions) are repr'd with their `__qualname__` which includes the class name.  E.g.

```
>>> class Foo:
...     def a(self): pass
...
>>> Foo.a
<function Foo.a at 0x7fe3d04c9b20>
```

So some tests that happen to rely on such output fail on python 3.

The tests could be rewritten, but it might be nice to automatically account for this difference.  In principle there's a little advantage to having the `__qualname__` in the output since we can also also see in what class the function was defined, but in the meantime we don't lose anything over what we already have by ignoring it.

I could either add that to this ticket, or add a new one.  It might be worth adding to this one since the same issue applies to nested classes.



---

archive/issue_comments_369847.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nReplying to [@embray](#comment%3A9):\n> I could either add that to this ticket, or add a new one.  It might be worth adding to this one since the same issue applies to nested classes.\n\nI implemented a workaround for this, but in implementation it's distinct enough from this issue that I'll just open a separate ticket for this once I'm confident that it's ready.",
    "created_at": "2017-11-28T13:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369847",
    "user": "https://github.com/embray"
}
```

<div id="comment:10" align="right">Comment 10</div>

Replying to [@embray](#comment%3A9):
> I could either add that to this ticket, or add a new one.  It might be worth adding to this one since the same issue applies to nested classes.

I implemented a workaround for this, but in implementation it's distinct enough from this issue that I'll just open a separate ticket for this once I'm confident that it's ready.



---

archive/issue_comments_369848.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nI don't think this actually needs any new work--it's been working fine for me on my python 3 branch for some time.  The previous patchbot failures were due to #24257 which is now closed.",
    "created_at": "2018-01-11T12:22:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369848",
    "user": "https://github.com/embray"
}
```

<div id="comment:11" align="right">Comment 11</div>

I don't think this actually needs any new work--it's been working fine for me on my python 3 branch for some time.  The previous patchbot failures were due to #24257 which is now closed.



---

archive/issue_events_318481.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-11T12:22:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318481"
}
```



---

archive/issue_events_318482.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-11T12:22:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318482"
}
```



---

archive/issue_comments_369849.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nWould be nice to get this done--I have a number of other test output normalizations that I've added since this one.",
    "created_at": "2018-01-11T12:23:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369849",
    "user": "https://github.com/embray"
}
```

<div id="comment:12" align="right">Comment 12</div>

Would be nice to get this done--I have a number of other test output normalizations that I've added since this one.



---

archive/issue_comments_369850.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nPlease add a test showing how `normalize_type_repr` works when given a more complicated expression. Something like:\n\n```\nsage: L = [Integer, float]\nsage: normalize_type_repr(repr(L))\n```",
    "created_at": "2018-01-11T13:13:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369850",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:13" align="right">Comment 13</div>

Please add a test showing how `normalize_type_repr` works when given a more complicated expression. Something like:

```
sage: L = [Integer, float]
sage: normalize_type_repr(repr(L))
```



---

archive/issue_comments_369851.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nSure--let me make sure that actually works in the first place (it should--I think I've already encountered some examples like that)",
    "created_at": "2018-01-11T14:14:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369851",
    "user": "https://github.com/embray"
}
```

<div id="comment:14" align="right">Comment 14</div>

Sure--let me make sure that actually works in the first place (it should--I think I've already encountered some examples like that)



---

archive/issue_events_318483.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-11T14:14:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318483"
}
```



---

archive/issue_events_318484.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-11T14:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318484"
}
```



---

archive/issue_comments_369852.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6760db0bc5406c4c36d131dad4d928e31f806008\">6760db0</a></td><td><code>Add an additional test for normalize_type_repr</code></td></tr></table>\n",
    "created_at": "2018-01-11T15:47:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369852",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:16" align="right">Comment 16</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6760db0bc5406c4c36d131dad4d928e31f806008">6760db0</a></td><td><code>Add an additional test for normalize_type_repr</code></td></tr></table>




---

archive/issue_comments_369853.json:
```json
{
    "body": "Changed commit from **[e12b67f](https://github.com/sagemath/sagetrac-mirror/commit/e12b67fda63bf721cf519c4553c5abef0e8553ef)** to **[6760db0](https://github.com/sagemath/sagetrac-mirror/commit/6760db0bc5406c4c36d131dad4d928e31f806008)**",
    "created_at": "2018-01-11T15:47:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369853",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e12b67f](https://github.com/sagemath/sagetrac-mirror/commit/e12b67fda63bf721cf519c4553c5abef0e8553ef)** to **[6760db0](https://github.com/sagemath/sagetrac-mirror/commit/6760db0bc5406c4c36d131dad4d928e31f806008)**



---

archive/issue_events_318485.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-11T15:47:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318485"
}
```



---

archive/issue_events_318486.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-11T15:47:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318486"
}
```



---

archive/issue_events_318487.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-12T09:23:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318487"
}
```



---

archive/issue_events_318488.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-01-12T09:23:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318488"
}
```



---

archive/issue_comments_369854.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nPatchbot complains about docbuild.",
    "created_at": "2018-01-12T09:23:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369854",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:18" align="right">Comment 18</div>

Patchbot complains about docbuild.



---

archive/issue_comments_369855.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nIf I understand things correctly, this ticket ensures that both the following doctests would pass on Python 3:\n\n```\nsage: int\n<class 'int'>\n```\nand\n\n```\nsage: int\n<type 'int'>\n```\nbut on Python 2 only\n\n```\nsage: int\n<type 'int'>\n```\nwould be accepted. Is that right?\n\nI would very much prefer to accept `<class 'int'>` on Python 2 too. That way, we could at least make *new* doctests more future-proof by using `<class` instead of `<type`.",
    "created_at": "2018-01-12T09:26:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369855",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:19" align="right">Comment 19</div>

If I understand things correctly, this ticket ensures that both the following doctests would pass on Python 3:

```
sage: int
<class 'int'>
```
and

```
sage: int
<type 'int'>
```
but on Python 2 only

```
sage: int
<type 'int'>
```
would be accepted. Is that right?

I would very much prefer to accept `<class 'int'>` on Python 2 too. That way, we could at least make *new* doctests more future-proof by using `<class` instead of `<type`.



---

archive/issue_comments_369856.json:
```json
{
    "body": "Changed commit from **[6760db0](https://github.com/sagemath/sagetrac-mirror/commit/6760db0bc5406c4c36d131dad4d928e31f806008)** to **[514ed93](https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9)**",
    "created_at": "2018-01-16T09:39:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369856",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[6760db0](https://github.com/sagemath/sagetrac-mirror/commit/6760db0bc5406c4c36d131dad4d928e31f806008)** to **[514ed93](https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9)**



---

archive/issue_comments_369857.json:
```json
{
    "body": "Changed dependencies from **#24257** to none",
    "created_at": "2018-01-16T09:39:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369857",
    "user": "https://github.com/fchapoton"
}
```

Changed dependencies from **#24257** to none



---

archive/issue_comments_369858.json:
```json
{
    "body": "Changed branch from **[u/embray/python3/type-repr-fixups](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/type-repr-fixups)** to **[public/python3/type-repr-fixups](https://github.com/sagemath/sagetrac-mirror/tree/public/python3/type-repr-fixups)**",
    "created_at": "2018-01-16T09:39:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369858",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/embray/python3/type-repr-fixups](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/type-repr-fixups)** to **[public/python3/type-repr-fixups](https://github.com/sagemath/sagetrac-mirror/tree/public/python3/type-repr-fixups)**



---

archive/issue_comments_369859.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nI fixed the doc\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/476a27b1fe10c95b9e9c69e95d23a73933757cef\">476a27b</a></td><td><code>Merge branch 'u/embray/python3/type-repr-fixups' in 8.2.b2</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9\">514ed93</a></td><td><code>trac 24258 make doc build</code></td></tr></table>\n",
    "created_at": "2018-01-16T09:39:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369859",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:20" align="right">Comment 20</div>

I fixed the doc

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/476a27b1fe10c95b9e9c69e95d23a73933757cef">476a27b</a></td><td><code>Merge branch 'u/embray/python3/type-repr-fixups' in 8.2.b2</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9">514ed93</a></td><td><code>trac 24258 make doc build</code></td></tr></table>




---

archive/issue_comments_369860.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nReplying to [@jdemeyer](#comment%3A19):\n> If I understand things correctly, this ticket ensures that both the following doctests would pass on Python 3:\n> \n> ```\n> sage: int\n> <class 'int'>\n> ```\n> and\n> \n> ```\n> sage: int\n> <type 'int'>\n> ```\n> but on Python 2 only\n> \n> ```\n> sage: int\n> <type 'int'>\n> ```\n> would be accepted. Is that right?\n> \n> I would very much prefer to accept `<class 'int'>` on Python 2 too. That way, we could at least make *new* doctests more future-proof by using `<class` instead of `<type`.\n\nWe can easily make new doctests future-proof by simply using `<... 'int'>`. There is a patchbot plugin that checks for that.",
    "created_at": "2018-01-16T10:15:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369860",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:21" align="right">Comment 21</div>

Replying to [@jdemeyer](#comment%3A19):
> If I understand things correctly, this ticket ensures that both the following doctests would pass on Python 3:
> 
> ```
> sage: int
> <class 'int'>
> ```
> and
> 
> ```
> sage: int
> <type 'int'>
> ```
> but on Python 2 only
> 
> ```
> sage: int
> <type 'int'>
> ```
> would be accepted. Is that right?
> 
> I would very much prefer to accept `<class 'int'>` on Python 2 too. That way, we could at least make *new* doctests more future-proof by using `<class` instead of `<type`.

We can easily make new doctests future-proof by simply using `<... 'int'>`. There is a patchbot plugin that checks for that.



---

archive/issue_events_318489.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-16T14:01:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318489"
}
```



---

archive/issue_events_318490.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-16T14:01:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318490"
}
```



---

archive/issue_comments_369861.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nReplying to [@jdemeyer](#comment%3A19):\n> I would very much prefer to accept `<class 'int'>` on Python 2 too. That way, we could at least make *new* doctests more future-proof by using `<class` instead of `<type`.\n\nI thought I already addressed this, but it doesn't make enormous sense to write new doctests against Python 3-style output as long as Sage's default python remains python 2--that's what the documentation should be written against.  When Sage switches to python 3 as the default python, then it makes sense to make mass replacements on the documentation too, and not until then.\n\nI wouldn't be opposed to making the output checker flexible in both cases, but I don't think there's any good reason to right now either.",
    "created_at": "2018-01-16T14:01:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369861",
    "user": "https://github.com/embray"
}
```

<div id="comment:22" align="right">Comment 22</div>

Replying to [@jdemeyer](#comment%3A19):
> I would very much prefer to accept `<class 'int'>` on Python 2 too. That way, we could at least make *new* doctests more future-proof by using `<class` instead of `<type`.

I thought I already addressed this, but it doesn't make enormous sense to write new doctests against Python 3-style output as long as Sage's default python remains python 2--that's what the documentation should be written against.  When Sage switches to python 3 as the default python, then it makes sense to make mass replacements on the documentation too, and not until then.

I wouldn't be opposed to making the output checker flexible in both cases, but I don't think there's any good reason to right now either.



---

archive/issue_comments_369862.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-01-16T16:44:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369862",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_318491.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-01-18T18:09:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318491"
}
```



---

archive/issue_events_318492.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5502506514e85fcb738af6cb5794136d1f7f3149",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-01-18T18:09:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24258#event-318492"
}
```



---

archive/issue_comments_369863.json:
```json
{
    "body": "Changed branch from **[public/python3/type-repr-fixups](https://github.com/sagemath/sagetrac-mirror/tree/public/python3/type-repr-fixups)** to **[514ed93](https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9)**",
    "created_at": "2018-01-18T18:09:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369863",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/python3/type-repr-fixups](https://github.com/sagemath/sagetrac-mirror/tree/public/python3/type-repr-fixups)** to **[514ed93](https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9)**



---

archive/issue_comments_369864.json:
```json
{
    "body": "Changed commit from **[514ed93](https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9)** to none",
    "created_at": "2018-09-07T09:50:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369864",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[514ed93](https://github.com/sagemath/sagetrac-mirror/commit/514ed9369f12f760b6e714468f45b3ce6c7b20a9)** to none



---

archive/issue_comments_369865.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">Comment 25</div>\n\nThis seems not to work on\n\n```\nsage -t --long src/sage/rings/universal_cyclotomic_field.py  # 1 doctest failed\n```\nwith sage 8.4.b4.",
    "created_at": "2018-09-07T09:50:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369865",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:25" align="right">Comment 25</div>

This seems not to work on

```
sage -t --long src/sage/rings/universal_cyclotomic_field.py  # 1 doctest failed
```
with sage 8.4.b4.



---

archive/issue_comments_369866.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">Comment 26</div>\n\nReplying to [@fchapoton](#comment%3A25):\n> This seems not to work on\n> \n> ```\n> sage -t --long src/sage/rings/universal_cyclotomic_field.py  # 1 doctest failed\n> ```\n> with sage 8.4.b4.\n\nIt's because the `<type ...>` repr is split onto multiple lines, which is not supported.  You could either change the regexp to be multi-line (but this introduces more complications), or just fix that test to not split `<type ...>` across lines.",
    "created_at": "2018-09-07T10:18:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24258#issuecomment-369866",
    "user": "https://github.com/embray"
}
```

<div id="comment:26" align="right">Comment 26</div>

Replying to [@fchapoton](#comment%3A25):
> This seems not to work on
> 
> ```
> sage -t --long src/sage/rings/universal_cyclotomic_field.py  # 1 doctest failed
> ```
> with sage 8.4.b4.

It's because the `<type ...>` repr is split onto multiple lines, which is not supported.  You could either change the regexp to be multi-line (but this introduces more complications), or just fix that test to not split `<type ...>` across lines.
