# Issue 24583: py3: additional fixes to sage.interfaces

archive/issues_024346.json:
```json
{
    "assignees": [],
    "body": "In #24269 I made some initial fixes to `sage.interfaces` just to get a minimum of functionality working.\n\nThis adds on top of that some additional fixes to `sage.interfaces.interface` and `sage.interfaces.expect` that were needed in getting the rest of the `sage.interfaces` tests passing.\n\nI'll follow up with additional tickets with fixes for the program-specific interfaces.  \n\nDepends on #24269\n\nBranch/Commit: **[u/embray/python3/sage-interfaces](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/sage-interfaces) @ [d169089](https://github.com/sagemath/sagetrac-mirror/commit/d169089100381f19af40476b075e9c65d3e7bcc5)**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24583_\n\n",
    "closed_at": "2018-05-21T15:10:44Z",
    "created_at": "2018-01-22T16:30:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: additional fixes to sage.interfaces",
    "type": "issue",
    "updated_at": "2018-05-21T15:10:44Z",
    "url": "https://github.com/sagemath/sage/issues/24583",
    "user": "https://github.com/embray"
}
```
In #24269 I made some initial fixes to `sage.interfaces` just to get a minimum of functionality working.

This adds on top of that some additional fixes to `sage.interfaces.interface` and `sage.interfaces.expect` that were needed in getting the rest of the `sage.interfaces` tests passing.

I'll follow up with additional tickets with fixes for the program-specific interfaces.  

Depends on #24269

Branch/Commit: **[u/embray/python3/sage-interfaces](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/sage-interfaces) @ [d169089](https://github.com/sagemath/sagetrac-mirror/commit/d169089100381f19af40476b075e9c65d3e7bcc5)**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/24583_





---

archive/issue_events_322427.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-22T16:30:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322427"
}
```



---

archive/issue_events_322428.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-22T16:30:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322428"
}
```



---

archive/issue_events_322429.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-22T16:30:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322429"
}
```



---

archive/issue_events_322430.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-22T16:30:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322430"
}
```



---

archive/issue_events_322431.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-22T16:30:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322431"
}
```



---

archive/issue_comments_375285.json:
```json
{
    "body": "Changed commit from **[bd6a331](https://github.com/sagemath/sagetrac-mirror/commit/bd6a3315b77df272b07e3d646cec0a54b9d77b69)** to **[2181b3e](https://github.com/sagemath/sagetrac-mirror/commit/2181b3eba6114f902f3599dbb76ad7d068478520)**",
    "created_at": "2018-02-19T17:54:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24583#issuecomment-375285",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[bd6a331](https://github.com/sagemath/sagetrac-mirror/commit/bd6a3315b77df272b07e3d646cec0a54b9d77b69)** to **[2181b3e](https://github.com/sagemath/sagetrac-mirror/commit/2181b3eba6114f902f3599dbb76ad7d068478520)**



---

archive/issue_comments_375286.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/afb764397db3bbe00743efbbe43daa465eafd921\">afb7643</a></td><td><code>Update SagePtyProcess and the Maxima and GAP pexpect interfaces to handle bytes roughly appropriately on Python 3</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2181b3eba6114f902f3599dbb76ad7d068478520\">2181b3e</a></td><td><code>py3: additional minor str/bytes fixes to the sage.interfaces framework</code></td></tr></table>\n",
    "created_at": "2018-02-19T17:54:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24583#issuecomment-375286",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/afb764397db3bbe00743efbbe43daa465eafd921">afb7643</a></td><td><code>Update SagePtyProcess and the Maxima and GAP pexpect interfaces to handle bytes roughly appropriately on Python 3</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2181b3eba6114f902f3599dbb76ad7d068478520">2181b3e</a></td><td><code>py3: additional minor str/bytes fixes to the sage.interfaces framework</code></td></tr></table>




---

archive/issue_events_322432.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322432"
}
```



---

archive/issue_events_322433.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322433"
}
```



---

archive/issue_events_322434.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-21T14:45:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322434"
}
```



---

archive/issue_events_322435.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-21T14:45:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322435"
}
```



---

archive/issue_comments_375287.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThis probably needs to be rebased on current #24269",
    "created_at": "2018-05-21T14:45:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24583#issuecomment-375287",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'>Comment 4:</a>
This probably needs to be rebased on current #24269



---

archive/issue_comments_375288.json:
```json
{
    "body": "Changed commit from **[2181b3e](https://github.com/sagemath/sagetrac-mirror/commit/2181b3eba6114f902f3599dbb76ad7d068478520)** to **[d169089](https://github.com/sagemath/sagetrac-mirror/commit/d169089100381f19af40476b075e9c65d3e7bcc5)**",
    "created_at": "2018-05-21T15:09:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24583#issuecomment-375288",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2181b3e](https://github.com/sagemath/sagetrac-mirror/commit/2181b3eba6114f902f3599dbb76ad7d068478520)** to **[d169089](https://github.com/sagemath/sagetrac-mirror/commit/d169089100381f19af40476b075e9c65d3e7bcc5)**



---

archive/issue_comments_375289.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3f08a183661403ffde313b97b224536dffe025ab\">3f08a18</a></td><td><code>Also supply a wrapper around spawn.readline that incorporates bytes_to_str</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6c8a6f038d2fbab5c25743ba17327f5727e474b5\">6c8a6f0</a></td><td><code>Update SagePtyProcess and the Maxima and GAP pexpect interfaces to handle bytes roughly appropriately on Python 3</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4a6a72afe589b0813a2b5fa3d56e5d808b17bc2c\">4a6a72a</a></td><td><code>just pyflakes fix</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3d4bcc594280000fe5efa3391717d2abd974dec7\">3d4bcc5</a></td><td><code>undo import removal</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/25409dc6bf6c74fc5c0563305007feb8d3e4f8ea\">25409dc</a></td><td><code>the base class now provides the expectation that self._prompt is already bytes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d169089100381f19af40476b075e9c65d3e7bcc5\">d169089</a></td><td><code>Additional Python 3 fixes to the Maxima interface:</code></td></tr></table>\n",
    "created_at": "2018-05-21T15:09:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24583#issuecomment-375289",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3f08a183661403ffde313b97b224536dffe025ab">3f08a18</a></td><td><code>Also supply a wrapper around spawn.readline that incorporates bytes_to_str</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6c8a6f038d2fbab5c25743ba17327f5727e474b5">6c8a6f0</a></td><td><code>Update SagePtyProcess and the Maxima and GAP pexpect interfaces to handle bytes roughly appropriately on Python 3</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4a6a72afe589b0813a2b5fa3d56e5d808b17bc2c">4a6a72a</a></td><td><code>just pyflakes fix</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3d4bcc594280000fe5efa3391717d2abd974dec7">3d4bcc5</a></td><td><code>undo import removal</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/25409dc6bf6c74fc5c0563305007feb8d3e4f8ea">25409dc</a></td><td><code>the base class now provides the expectation that self._prompt is already bytes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d169089100381f19af40476b075e9c65d3e7bcc5">d169089</a></td><td><code>Additional Python 3 fixes to the Maxima interface:</code></td></tr></table>




---

archive/issue_events_322436.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-21T15:10:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322436"
}
```



---

archive/issue_events_322437.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-21T15:10:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322437"
}
```



---

archive/issue_events_322438.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-21T15:10:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322438"
}
```



---

archive/issue_events_322439.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-21T15:10:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322439"
}
```



---

archive/issue_events_322440.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-21T15:10:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24583#event-322440"
}
```



---

archive/issue_comments_375290.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nApparently all that's left of this ticket that isn't already merged is a few more minor fixes for the maxima interface, so I'll just add that to #24269 and close this ticket.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3f08a183661403ffde313b97b224536dffe025ab\">3f08a18</a></td><td><code>Also supply a wrapper around spawn.readline that incorporates bytes_to_str</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6c8a6f038d2fbab5c25743ba17327f5727e474b5\">6c8a6f0</a></td><td><code>Update SagePtyProcess and the Maxima and GAP pexpect interfaces to handle bytes roughly appropriately on Python 3</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4a6a72afe589b0813a2b5fa3d56e5d808b17bc2c\">4a6a72a</a></td><td><code>just pyflakes fix</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3d4bcc594280000fe5efa3391717d2abd974dec7\">3d4bcc5</a></td><td><code>undo import removal</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/25409dc6bf6c74fc5c0563305007feb8d3e4f8ea\">25409dc</a></td><td><code>the base class now provides the expectation that self._prompt is already bytes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d169089100381f19af40476b075e9c65d3e7bcc5\">d169089</a></td><td><code>Additional Python 3 fixes to the Maxima interface:</code></td></tr></table>\n",
    "created_at": "2018-05-21T15:10:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24583#issuecomment-375290",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'>Comment 6:</a>
Apparently all that's left of this ticket that isn't already merged is a few more minor fixes for the maxima interface, so I'll just add that to #24269 and close this ticket.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3f08a183661403ffde313b97b224536dffe025ab">3f08a18</a></td><td><code>Also supply a wrapper around spawn.readline that incorporates bytes_to_str</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6c8a6f038d2fbab5c25743ba17327f5727e474b5">6c8a6f0</a></td><td><code>Update SagePtyProcess and the Maxima and GAP pexpect interfaces to handle bytes roughly appropriately on Python 3</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4a6a72afe589b0813a2b5fa3d56e5d808b17bc2c">4a6a72a</a></td><td><code>just pyflakes fix</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3d4bcc594280000fe5efa3391717d2abd974dec7">3d4bcc5</a></td><td><code>undo import removal</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/25409dc6bf6c74fc5c0563305007feb8d3e4f8ea">25409dc</a></td><td><code>the base class now provides the expectation that self._prompt is already bytes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d169089100381f19af40476b075e9c65d3e7bcc5">d169089</a></td><td><code>Additional Python 3 fixes to the Maxima interface:</code></td></tr></table>

