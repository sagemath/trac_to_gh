...
<a id='comment:3'></a>This is the basic idea. But this leads to a million (well, a lot) doctest failures of the form
...
```json
{
    "body": "<a id='comment:4'></a>Setting to needs_review just for the patchbot.",
    "created_at": "2018-02-12T13:49:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24715",
    "type": "issue_comment",
...
<a id='comment:4'></a>Setting to needs_review just for the patchbot.
...
<a id='comment:6'></a>Interesting fact: `combinat` already uses dynamic classes for almost everything. Only doctest failures in `combinat`:
...
<a id='comment:7'></a>From a quick look at the current patchbot reports:
...
<a id='comment:9'></a>I think this would cause speed regression in every `cpdef` call. Whenever a type is subclasses, `cpdef` must take the slow codepath in order to check if it's been overridden. I haven't checked if cython actually does that, but this should even be the case if the subclass is an extension type, since the `cpdef` on the super doesn't know what kind of shenanigans have been played (the subtype might have grown an instance dict and the method might be overridden by a function in there).
