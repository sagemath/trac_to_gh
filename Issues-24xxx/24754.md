# Issue 24754: Clean up EvaluationMethods

archive/issues_024517.json:
```json
{
    "assignees": [],
    "body": "Due to the strange way how `get_dynamic_class_for_function()` is implemented, all methods inside a `class EvaluationMethods` take a superfluous `cls` argument. We can fix this and at the same time make it Python 3 compatible.\n\nBranch/Commit: **[1d796e7](https://github.com/sagemath/sagetrac-mirror/commit/1d796e77e5772bb4b1716816a40d852d63f103b3)**\n\nReviewer: **Erik Bray**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24754_\n\n",
    "closed_at": "2018-03-21T06:18:47Z",
    "created_at": "2018-02-16T14:32:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean up EvaluationMethods",
    "type": "issue",
    "updated_at": "2018-03-21T06:18:47Z",
    "url": "https://github.com/sagemath/sage/issues/24754",
    "user": "https://github.com/embray"
}
```
Due to the strange way how `get_dynamic_class_for_function()` is implemented, all methods inside a `class EvaluationMethods` take a superfluous `cls` argument. We can fix this and at the same time make it Python 3 compatible.

Branch/Commit: **[1d796e7](https://github.com/sagemath/sagetrac-mirror/commit/1d796e77e5772bb4b1716816a40d852d63f103b3)**

Reviewer: **Erik Bray**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24754_





---

archive/issue_events_324643.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-16T14:32:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324643"
}
```



---

archive/issue_events_324644.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-16T14:32:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324644"
}
```



---

archive/issue_events_324645.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-16T14:32:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324645"
}
```



---

archive/issue_events_324646.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-16T14:32:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324646"
}
```



---

archive/issue_comments_379054.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fbb0c27ae914909886a3d489f7fea2a6c90d3ee6\">fbb0c27</a></td><td><code>Fix creation of Expression_with_dynamic_methods types on Python 3</code></td></tr></table>\n",
    "created_at": "2018-02-16T14:32:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379054",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:1" align="right">Comment 1</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fbb0c27ae914909886a3d489f7fea2a6c90d3ee6">fbb0c27</a></td><td><code>Fix creation of Expression_with_dynamic_methods types on Python 3</code></td></tr></table>




---

archive/issue_comments_379055.json:
```json
{
    "body": "Commit: **[fbb0c27](https://github.com/sagemath/sagetrac-mirror/commit/fbb0c27ae914909886a3d489f7fea2a6c90d3ee6)**",
    "created_at": "2018-02-16T14:32:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379055",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[fbb0c27](https://github.com/sagemath/sagetrac-mirror/commit/fbb0c27ae914909886a3d489f7fea2a6c90d3ee6)**



---

archive/issue_comments_379056.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fbb0c27ae914909886a3d489f7fea2a6c90d3ee6\">fbb0c27</a></td><td><code>Fix creation of Expression_with_dynamic_methods types on Python 3</code></td></tr></table>\n",
    "created_at": "2018-02-16T14:32:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379056",
    "user": "https://github.com/embray"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fbb0c27ae914909886a3d489f7fea2a6c90d3ee6">fbb0c27</a></td><td><code>Fix creation of Expression_with_dynamic_methods types on Python 3</code></td></tr></table>




---

archive/issue_comments_379057.json:
```json
{
    "body": "Dependencies: **u/embray/python3/sage-symbolic-expression/ismethod**",
    "created_at": "2018-02-16T14:32:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379057",
    "user": "https://github.com/embray"
}
```

Dependencies: **u/embray/python3/sage-symbolic-expression/ismethod**



---

archive/issue_events_324647.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-16T14:32:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324647"
}
```



---

archive/issue_comments_379058.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNot sure how that wound up there.",
    "created_at": "2018-02-16T14:33:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379058",
    "user": "https://github.com/embray"
}
```

<div id="comment:3" align="right">Comment 3</div>

Not sure how that wound up there.



---

archive/issue_comments_379059.json:
```json
{
    "body": "Changed dependencies from **u/embray/python3/sage-symbolic-expression/ismethod** to none",
    "created_at": "2018-02-16T14:33:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379059",
    "user": "https://github.com/embray"
}
```

Changed dependencies from **u/embray/python3/sage-symbolic-expression/ismethod** to none



---

archive/issue_comments_379060.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nWould `callable()` work?",
    "created_at": "2018-03-08T16:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379060",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

Would `callable()` work?



---

archive/issue_comments_379061.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nIt might but callable is also rather broad and could potentially pick up things that were not intended here.",
    "created_at": "2018-03-08T17:22:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379061",
    "user": "https://github.com/embray"
}
```

<div id="comment:5" align="right">Comment 5</div>

It might but callable is also rather broad and could potentially pick up things that were not intended here.



---

archive/issue_comments_379062.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nIt seems to me that the intention of the code to catch all \"normal\" attributes of the class, but not special attributes like `__repr__`. So maybe excluding all `__dunder__` attributes is the correct solution?",
    "created_at": "2018-03-13T15:02:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379062",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">Comment 6</div>

It seems to me that the intention of the code to catch all "normal" attributes of the class, but not special attributes like `__repr__`. So maybe excluding all `__dunder__` attributes is the correct solution?



---

archive/issue_events_324648.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T16:13:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324648"
}
```



---

archive/issue_events_324649.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T16:13:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324649"
}
```



---

archive/issue_comments_379063.json:
```json
{
    "body": "Changed author from **Erik Bray** to **Jeroen Demeyer**",
    "created_at": "2018-03-13T16:13:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379063",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Erik Bray** to **Jeroen Demeyer**



---

archive/issue_events_324650.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T16:13:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324650"
}
```



---

archive/issue_events_324651.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T16:13:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324651"
}
```



---

archive/issue_events_324652.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T16:13:57Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "title_is": "Clean up EvaluationMethods",
    "title_was": "py3: fix issue in sage.symbolic.expression with creating dynamic function classes",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324652"
}
```



---

archive/issue_comments_379064.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-I'm not completely happy with this solution but it gets the job done.\n+Due to the strange way how `get_dynamic_class_for_function()` is implemented, all methods inside a `class EvaluationMethods` take a superfluous `cls` argument. We can fix this and at the same time make it Python 3 compatible.\n``````\n",
    "created_at": "2018-03-13T16:13:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379064",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-I'm not completely happy with this solution but it gets the job done.
+Due to the strange way how `get_dynamic_class_for_function()` is implemented, all methods inside a `class EvaluationMethods` take a superfluous `cls` argument. We can fix this and at the same time make it Python 3 compatible.
``````




---

archive/issue_comments_379065.json:
```json
{
    "body": "Changed branch from **[u/embray/python3/sage-symbolic-expression/ismethod](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/sage-symbolic-expression/ismethod)** to **[u/jdemeyer/python3/sage-symbolic-expression/ismethod](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/python3/sage-symbolic-expression/ismethod)**",
    "created_at": "2018-03-13T16:29:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379065",
    "user": "https://github.com/jdemeyer"
}
```

Changed branch from **[u/embray/python3/sage-symbolic-expression/ismethod](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/sage-symbolic-expression/ismethod)** to **[u/jdemeyer/python3/sage-symbolic-expression/ismethod](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/python3/sage-symbolic-expression/ismethod)**



---

archive/issue_comments_379066.json:
```json
{
    "body": "Changed commit from **[fbb0c27](https://github.com/sagemath/sagetrac-mirror/commit/fbb0c27ae914909886a3d489f7fea2a6c90d3ee6)** to **[36f1be2](https://github.com/sagemath/sagetrac-mirror/commit/36f1be24d98a9ae04e52fd9700681688d802b673)**",
    "created_at": "2018-03-13T16:29:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379066",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[fbb0c27](https://github.com/sagemath/sagetrac-mirror/commit/fbb0c27ae914909886a3d489f7fea2a6c90d3ee6)** to **[36f1be2](https://github.com/sagemath/sagetrac-mirror/commit/36f1be24d98a9ae04e52fd9700681688d802b673)**



---

archive/issue_comments_379067.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nI think that this is a simpler and better solution.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/36f1be24d98a9ae04e52fd9700681688d802b673\">36f1be2</a></td><td><code>Clean up EvaluationMethods</code></td></tr></table>\n",
    "created_at": "2018-03-13T16:29:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379067",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:9" align="right">Comment 9</div>

I think that this is a simpler and better solution.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/36f1be24d98a9ae04e52fd9700681688d802b673">36f1be2</a></td><td><code>Clean up EvaluationMethods</code></td></tr></table>




---

archive/issue_events_324653.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T16:29:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324653"
}
```



---

archive/issue_events_324654.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-13T16:29:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324654"
}
```



---

archive/issue_events_324655.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-13T19:49:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324655"
}
```



---

archive/issue_events_324656.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-13T19:49:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324656"
}
```



---

archive/issue_comments_379068.json:
```json
{
    "body": "Reviewer: **Erik Bray**",
    "created_at": "2018-03-13T19:49:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379068",
    "user": "https://github.com/embray"
}
```

Reviewer: **Erik Bray**



---

archive/issue_comments_379069.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nOkay. It bothers me a bit that `eval_on_operands` is being moved and becoming a private function. I agree with the change; it just speaks to Sage's lack of a consistent \"public API\".  In this case it always should have been a private function.",
    "created_at": "2018-03-13T19:49:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379069",
    "user": "https://github.com/embray"
}
```

<div id="comment:10" align="right">Comment 10</div>

Okay. It bothers me a bit that `eval_on_operands` is being moved and becoming a private function. I agree with the change; it just speaks to Sage's lack of a consistent "public API".  In this case it always should have been a private function.



---

archive/issue_comments_379070.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nSlight nitpick that you can fix only if you feel like:\n\n```\n+                            [Expression], eval_methods, prepend_cls_bases=False)\n```\n\nwhile using a list for the bases certainly works, it's just kind of unidomatic, as opposed to using a tuple (since internally it's stored as a tuple).  But for the purposes of the constructor it doesn't really matter...",
    "created_at": "2018-03-13T19:54:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379070",
    "user": "https://github.com/embray"
}
```

<div id="comment:11" align="right">Comment 11</div>

Slight nitpick that you can fix only if you feel like:

```
+                            [Expression], eval_methods, prepend_cls_bases=False)
```

while using a list for the bases certainly works, it's just kind of unidomatic, as opposed to using a tuple (since internally it's stored as a tuple).  But for the purposes of the constructor it doesn't really matter...



---

archive/issue_comments_379071.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1d796e77e5772bb4b1716816a40d852d63f103b3\">1d796e7</a></td><td><code>Clean up EvaluationMethods</code></td></tr></table>\n",
    "created_at": "2018-03-14T09:48:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379071",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1d796e77e5772bb4b1716816a40d852d63f103b3">1d796e7</a></td><td><code>Clean up EvaluationMethods</code></td></tr></table>




---

archive/issue_events_324657.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2018-03-14T09:48:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324657"
}
```



---

archive/issue_events_324658.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2018-03-14T09:48:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324658"
}
```



---

archive/issue_comments_379072.json:
```json
{
    "body": "Changed commit from **[36f1be2](https://github.com/sagemath/sagetrac-mirror/commit/36f1be24d98a9ae04e52fd9700681688d802b673)** to **[1d796e7](https://github.com/sagemath/sagetrac-mirror/commit/1d796e77e5772bb4b1716816a40d852d63f103b3)**",
    "created_at": "2018-03-14T09:48:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379072",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[36f1be2](https://github.com/sagemath/sagetrac-mirror/commit/36f1be24d98a9ae04e52fd9700681688d802b673)** to **[1d796e7](https://github.com/sagemath/sagetrac-mirror/commit/1d796e77e5772bb4b1716816a40d852d63f103b3)**



---

archive/issue_comments_379073.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nReplying to [@embray](#comment%3A11):\n> Slight nitpick that you can fix only if you feel like:\n> \n> ```\n> +                            [Expression], eval_methods, prepend_cls_bases=False)\n> ```\n> \n> while using a list for the bases certainly works, it's just kind of unidomatic, as opposed to using a tuple (since internally it's stored as a tuple).\n\nDone.",
    "created_at": "2018-03-14T09:50:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379073",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:13" align="right">Comment 13</div>

Replying to [@embray](#comment%3A11):
> Slight nitpick that you can fix only if you feel like:
> 
> ```
> +                            [Expression], eval_methods, prepend_cls_bases=False)
> ```
> 
> while using a list for the bases certainly works, it's just kind of unidomatic, as opposed to using a tuple (since internally it's stored as a tuple).

Done.



---

archive/issue_events_324659.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-14T09:50:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324659"
}
```



---

archive/issue_events_324660.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-14T09:50:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324660"
}
```



---

archive/issue_events_324661.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-21T06:18:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324661"
}
```



---

archive/issue_events_324662.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3d65cf93d2f698b2fc5532ab042e9b847c45c977",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-03-21T06:18:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24754#event-324662"
}
```



---

archive/issue_comments_379074.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/python3/sage-symbolic-expression/ismethod](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/python3/sage-symbolic-expression/ismethod)** to **[1d796e7](https://github.com/sagemath/sagetrac-mirror/commit/1d796e77e5772bb4b1716816a40d852d63f103b3)**",
    "created_at": "2018-03-21T06:18:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24754#issuecomment-379074",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/python3/sage-symbolic-expression/ismethod](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/python3/sage-symbolic-expression/ismethod)** to **[1d796e7](https://github.com/sagemath/sagetrac-mirror/commit/1d796e77e5772bb4b1716816a40d852d63f103b3)**
