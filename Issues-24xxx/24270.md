# Issue 24270: Fix compiler warnings involving debug.refine_category_hash_check

archive/issues_024033.json:
```json
{
    "assignees": [],
    "body": "My compiler is not able to prove that the value of `debug.refine_category_hash_check` does not change during the call `Parent._refine_category_`. This gives `-Wmaybe-uninitialized` warnings.\n\nI am working regularly on this file and this warning is always distracting me.\n\nCC:  @tscrim\n\nBranch/Commit: **[8dfff41](https://github.com/sagemath/sagetrac-mirror/commit/8dfff41fe8fe4e5e23a7b950bf47f67854b3158b)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24270_\n\n",
    "closed_at": "2017-12-13T17:38:07Z",
    "created_at": "2017-11-23T09:15:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix compiler warnings involving debug.refine_category_hash_check",
    "type": "issue",
    "updated_at": "2017-12-13T17:38:07Z",
    "url": "https://github.com/sagemath/sage/issues/24270",
    "user": "https://github.com/jdemeyer"
}
```
My compiler is not able to prove that the value of `debug.refine_category_hash_check` does not change during the call `Parent._refine_category_`. This gives `-Wmaybe-uninitialized` warnings.

I am working regularly on this file and this warning is always distracting me.

CC:  @tscrim

Branch/Commit: **[8dfff41](https://github.com/sagemath/sagetrac-mirror/commit/8dfff41fe8fe4e5e23a7b950bf47f67854b3158b)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24270_





---

archive/issue_events_307208.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-23T09:15:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24270#event-307208"
}
```



---

archive/issue_events_307209.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-23T09:15:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24270#event-307209"
}
```



---

archive/issue_events_307210.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-23T09:15:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24270#event-307210"
}
```



---

archive/issue_comments_370126.json:
```json
{
    "body": "Branch: **[u/jdemeyer/fix_compiler_warnings_involving_debug_refine_category_hash_check](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_compiler_warnings_involving_debug_refine_category_hash_check)**",
    "created_at": "2017-11-23T09:28:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24270#issuecomment-370126",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/fix_compiler_warnings_involving_debug_refine_category_hash_check](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_compiler_warnings_involving_debug_refine_category_hash_check)**



---

archive/issue_events_307211.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-23T09:29:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24270#event-307211"
}
```



---

archive/issue_comments_370127.json:
```json
{
    "body": "Commit: **[8dfff41](https://github.com/sagemath/sagetrac-mirror/commit/8dfff41fe8fe4e5e23a7b950bf47f67854b3158b)**",
    "created_at": "2017-11-23T09:29:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24270#issuecomment-370127",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[8dfff41](https://github.com/sagemath/sagetrac-mirror/commit/8dfff41fe8fe4e5e23a7b950bf47f67854b3158b)**



---

archive/issue_comments_370128.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8dfff41fe8fe4e5e23a7b950bf47f67854b3158b\">8dfff41</a></td><td><code>Fix compiler warnings involving debug.refine_category_hash_check</code></td></tr></table>\n",
    "created_at": "2017-11-23T09:29:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24270#issuecomment-370128",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8dfff41fe8fe4e5e23a7b950bf47f67854b3158b">8dfff41</a></td><td><code>Fix compiler warnings involving debug.refine_category_hash_check</code></td></tr></table>




---

archive/issue_comments_370129.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n My compiler is not able to prove that the value of `debug.refine_category_hash_check` does not change during the call `Parent._refine_category_`. This gives `-Wmaybe-uninitialized` warnings.\n+\n+I am working regularly on this file and this warning is always distracting me.\n``````\n",
    "created_at": "2017-12-08T17:02:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24270#issuecomment-370129",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 My compiler is not able to prove that the value of `debug.refine_category_hash_check` does not change during the call `Parent._refine_category_`. This gives `-Wmaybe-uninitialized` warnings.
+
+I am working regularly on this file and this warning is always distracting me.
``````




---

archive/issue_events_307212.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-11T03:28:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24270#event-307212"
}
```



---

archive/issue_events_307213.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-11T03:28:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24270#event-307213"
}
```



---

archive/issue_comments_370130.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLGTM.",
    "created_at": "2017-12-11T03:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24270#issuecomment-370130",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
LGTM.



---

archive/issue_comments_370131.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-12-11T03:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24270#issuecomment-370131",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_307214.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-13T17:38:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24270#event-307214"
}
```



---

archive/issue_events_307215.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e5cbdd835202dae389189760c1bfca1c021c77a2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-13T17:38:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24270#event-307215"
}
```



---

archive/issue_comments_370132.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/fix_compiler_warnings_involving_debug_refine_category_hash_check](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_compiler_warnings_involving_debug_refine_category_hash_check)** to **[8dfff41](https://github.com/sagemath/sagetrac-mirror/commit/8dfff41fe8fe4e5e23a7b950bf47f67854b3158b)**",
    "created_at": "2017-12-13T17:38:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24270#issuecomment-370132",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/fix_compiler_warnings_involving_debug_refine_category_hash_check](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_compiler_warnings_involving_debug_refine_category_hash_check)** to **[8dfff41](https://github.com/sagemath/sagetrac-mirror/commit/8dfff41fe8fe4e5e23a7b950bf47f67854b3158b)**
