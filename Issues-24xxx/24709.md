# Issue 24709: Random failure in RealField_class._coerce_map_from_

archive/issues_024472.json:
```json
{
    "assignees": [],
    "body": "\n```\nFile \"src/sage/rings/real_mpfr.pyx\", line 713, in sage.rings.real_mpfr.RealField_class._coerce_map_from_\nFailed example:\n    RR['x'].get_action(ZZ)\nExpected:\n    Right scalar multiplication by Integer Ring on Univariate Polynomial Ring in x over Real Field with 53 bits of precision\nGot:\n    <repr(<sage.structure.coerce_actions.RightModuleAction at 0x153843e50>) failed: RuntimeError: This action acted on a set that became garbage collected>\n**********************************************************************\n1 item had failures:\n   1 of  18 in sage.rings.real_mpfr.RealField_class._coerce_map_from_\n    [1002 tests, 1 failure, 14.57 s]\n```\n\nKeywords: **random_fail**\n\nBranch/Commit: **[7f404f9](https://github.com/sagemath/sagetrac-mirror/commit/7f404f96714d4707878696b5515cbb13e34437cf)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24709_\n\n",
    "closed_at": "2018-02-13T08:10:47Z",
    "created_at": "2018-02-11T13:02:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Random failure in RealField_class._coerce_map_from_",
    "type": "issue",
    "updated_at": "2018-02-13T08:10:47Z",
    "url": "https://github.com/sagemath/sage/issues/24709",
    "user": "https://github.com/vbraun"
}
```

```
File "src/sage/rings/real_mpfr.pyx", line 713, in sage.rings.real_mpfr.RealField_class._coerce_map_from_
Failed example:
    RR['x'].get_action(ZZ)
Expected:
    Right scalar multiplication by Integer Ring on Univariate Polynomial Ring in x over Real Field with 53 bits of precision
Got:
    <repr(<sage.structure.coerce_actions.RightModuleAction at 0x153843e50>) failed: RuntimeError: This action acted on a set that became garbage collected>
**********************************************************************
1 item had failures:
   1 of  18 in sage.rings.real_mpfr.RealField_class._coerce_map_from_
    [1002 tests, 1 failure, 14.57 s]
```

Keywords: **random_fail**

Branch/Commit: **[7f404f9](https://github.com/sagemath/sagetrac-mirror/commit/7f404f96714d4707878696b5515cbb13e34437cf)**

Reviewer: **Jeroen Demeyer**

Author: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/24709_





---

archive/issue_events_324087.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-11T13:02:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24709#event-324087"
}
```



---

archive/issue_events_324088.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-11T13:02:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24709#event-324088"
}
```



---

archive/issue_events_324089.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-11T13:02:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24709#event-324089"
}
```



---

archive/issue_events_324090.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-11T13:02:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24709#event-324090"
}
```



---

archive/issue_comments_378291.json:
```json
{
    "body": "Branch: **[u/vbraun/random_failure_in_realfield_class__coerce_map_from_](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/random_failure_in_realfield_class__coerce_map_from_)**",
    "created_at": "2018-02-11T13:08:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24709#issuecomment-378291",
    "user": "https://github.com/vbraun"
}
```

Branch: **[u/vbraun/random_failure_in_realfield_class__coerce_map_from_](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/random_failure_in_realfield_class__coerce_map_from_)**



---

archive/issue_comments_378292.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2018-02-11T13:09:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24709#issuecomment-378292",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_378293.json:
```json
{
    "body": "Commit: **[7f404f9](https://github.com/sagemath/sagetrac-mirror/commit/7f404f96714d4707878696b5515cbb13e34437cf)**",
    "created_at": "2018-02-11T13:09:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24709#issuecomment-378293",
    "user": "https://github.com/vbraun"
}
```

Commit: **[7f404f9](https://github.com/sagemath/sagetrac-mirror/commit/7f404f96714d4707878696b5515cbb13e34437cf)**



---

archive/issue_events_324091.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-11T13:09:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24709#event-324091"
}
```



---

archive/issue_comments_378294.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7f404f96714d4707878696b5515cbb13e34437cf\">7f404f9</a></td><td><code>Hold reference to avoid garbage collection action of domain</code></td></tr></table>\n",
    "created_at": "2018-02-11T13:09:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24709#issuecomment-378294",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7f404f96714d4707878696b5515cbb13e34437cf">7f404f9</a></td><td><code>Hold reference to avoid garbage collection action of domain</code></td></tr></table>




---

archive/issue_events_324092.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-11T19:58:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24709#event-324092"
}
```



---

archive/issue_events_324093.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-11T19:58:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24709#event-324093"
}
```



---

archive/issue_comments_378295.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2018-02-11T19:58:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24709#issuecomment-378295",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_324094.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-13T08:10:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24709#event-324094"
}
```



---

archive/issue_events_324095.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "0fad24c606846a483ed41d6e803ef363da184762",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-13T08:10:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24709#event-324095"
}
```



---

archive/issue_comments_378296.json:
```json
{
    "body": "Changed branch from **[u/vbraun/random_failure_in_realfield_class__coerce_map_from_](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/random_failure_in_realfield_class__coerce_map_from_)** to **[7f404f9](https://github.com/sagemath/sagetrac-mirror/commit/7f404f96714d4707878696b5515cbb13e34437cf)**",
    "created_at": "2018-02-13T08:10:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24709#issuecomment-378296",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vbraun/random_failure_in_realfield_class__coerce_map_from_](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/random_failure_in_realfield_class__coerce_map_from_)** to **[7f404f9](https://github.com/sagemath/sagetrac-mirror/commit/7f404f96714d4707878696b5515cbb13e34437cf)**
