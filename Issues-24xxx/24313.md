...
```json
{
    "body": "Instead of the `encoding` argument, I would rather allow arbitrary `**kwds`.",
...
```json
{
    "body": "Also again, I'll note, that `TemporaryFile` does not accept all the arguments that `open` does in the first place (in particular it does not accept an argument for encoding error handling--which I would consider an upstream defect).",
...
```json
{
    "body": "Looks good to me. Jeroen, your opinion ?",
...
```json
{
    "body": "the import of locale seems no longer needed",
...
```json
{
    "body": "I added 2 tiny fixes. If you agree, you (Erik or Fr\u00e9d\u00e9ric) can set it to positive_review.",
