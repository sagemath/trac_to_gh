...
```json
{
    "body": "`Piecewise` is a lazy import, but that's not the cause:\n\n```\nsage: f1(x) = 1\nsage: f2(x) = 1 - x\nsage: Piecewise = Piecewise._get_object()\nsage: f = Piecewise([[(0,1),f1],[(1,2),f2]])\n/usr/local/src/sage-config/src/bin/sage-ipython:1: DeprecationWarning: use lower-case piecewise instead\nSee http://trac.sagemath.org/14801 for details.\n  #!/usr/bin/env sage-python23\nsage: f = Piecewise([[(0,1),f1],[(1,2),f2]])\n/usr/local/src/sage-config/src/bin/sage-ipython:1: DeprecationWarning: use lower-case piecewise instead\nSee http://trac.sagemath.org/14801 for details.\n  #!/usr/bin/env sage-python23\n```",
...
```json
{
    "body": "I'd still like to know why this specific warning behaves differently from other very similar warnings.",
...
```json
{
    "body": "The file `piecewise_old.py` has been removed by #26865, so this ticket should probably either be updated with another example in its description or be closed as \"invalid/wontfix/worksforme\".",
...
```json
{
    "body": "the `catch_warnings()` context manager is not meant to work in multithreaded environment: https://docs.python.org/3/library/warnings.html\n\nthis looks like it's basically doomed to be broken for our doctests - see e.g. #28329\nwhere this resurfaced.",
...
```json
{
    "body": "This does not explain how the interference from #28329 (i.e. https://git.sagemath.org/sage.git/commit/?id=6192e54fe6894b5c444e1a2cb7d0adeec1d94348) can make a difference here.\nBecause it should have been totally safe in a single-threaded environment!",
...
```json
{
    "body": "No, it explains it exactly.  The original change used warnings.catch_warnings(), and if that code happened to be called from anywhere in that module it would reset the warnings registry for that module and cause warnings that were previously shown to be shown again instead of hidden.\n\nAgain, threads have nothing to do with it.  It is a single-threaded environment.",
...
```json
{
    "body": "`catch_warnings()` has been called in context, in `with` block. Are you saying this changes global state? Then the Python docs on it are wrong.",
