# Issue 24916: Fix __richcmp__ calls with reversed operands

archive/issues_024679.json:
```json
{
    "assignees": [],
    "body": "This code in `coerce.pyx` has always been wrong:\n\n```\n         if y_is_Element:\n             if (<Element>y)._parent.get_flag(Parent_richcmp_element_without_coercion):\n                return Py_TYPE(y).tp_richcompare(x, y, op)\n```\nbecause the `self` argument of `__richcmp__` should be of the correct type. So the last line should be replaced by\n\n```\n                return Py_TYPE(y).tp_richcompare(y, x, revop(op))\n```\nbut this caused some breakage in `linear_functions.pyx` that needed to be fixed.\n\nBranch/Commit: **[ef5813a](https://github.com/sagemath/sagetrac-mirror/commit/ef5813a9d2f72e91cde914e243981f5354196ea8)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24916_\n\n",
    "closed_at": "2018-03-19T07:57:51Z",
    "created_at": "2018-03-06T14:45:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix __richcmp__ calls with reversed operands",
    "type": "issue",
    "updated_at": "2018-03-19T07:57:51Z",
    "url": "https://github.com/sagemath/sage/issues/24916",
    "user": "https://github.com/jdemeyer"
}
```
This code in `coerce.pyx` has always been wrong:

```
         if y_is_Element:
             if (<Element>y)._parent.get_flag(Parent_richcmp_element_without_coercion):
                return Py_TYPE(y).tp_richcompare(x, y, op)
```
because the `self` argument of `__richcmp__` should be of the correct type. So the last line should be replaced by

```
                return Py_TYPE(y).tp_richcompare(y, x, revop(op))
```
but this caused some breakage in `linear_functions.pyx` that needed to be fixed.

Branch/Commit: **[ef5813a](https://github.com/sagemath/sagetrac-mirror/commit/ef5813a9d2f72e91cde914e243981f5354196ea8)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24916_





---

archive/issue_events_326659.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-06T14:45:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24916#event-326659"
}
```



---

archive/issue_events_326660.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-06T14:45:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24916#event-326660"
}
```



---

archive/issue_events_326661.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-06T14:45:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24916#event-326661"
}
```



---

archive/issue_events_326662.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-06T14:45:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24916#event-326662"
}
```



---

archive/issue_comments_381824.json:
```json
{
    "body": "Branch: **[u/jdemeyer/fix___richcmp___calls_with_reversed_operands](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix___richcmp___calls_with_reversed_operands)**",
    "created_at": "2018-03-06T14:45:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24916#issuecomment-381824",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/fix___richcmp___calls_with_reversed_operands](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix___richcmp___calls_with_reversed_operands)**



---

archive/issue_comments_381825.json:
```json
{
    "body": "Commit: **[ef5813a](https://github.com/sagemath/sagetrac-mirror/commit/ef5813a9d2f72e91cde914e243981f5354196ea8)**",
    "created_at": "2018-03-06T14:49:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24916#issuecomment-381825",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[ef5813a](https://github.com/sagemath/sagetrac-mirror/commit/ef5813a9d2f72e91cde914e243981f5354196ea8)**



---

archive/issue_events_326663.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-06T14:49:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24916#event-326663"
}
```



---

archive/issue_comments_381826.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,13 @@\n+This code in `coerce.pyx` has always been wrong:\n \n+```\n+         if y_is_Element:\n+             if (<Element>y)._parent.get_flag(Parent_richcmp_element_without_coercion):\n+                return Py_TYPE(y).tp_richcompare(x, y, op)\n+```\n+because the `self` argument of `__richcmp__` should be of the correct type. So the last line should be replaced by\n+\n+```\n+                return Py_TYPE(y).tp_richcompare(y, x, revop(op))\n+```\n+but this caused some breakage in `linear_functions.pyx` that needed to be fixed.\n``````\n",
    "created_at": "2018-03-06T14:49:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24916#issuecomment-381826",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,13 @@
+This code in `coerce.pyx` has always been wrong:
 
+```
+         if y_is_Element:
+             if (<Element>y)._parent.get_flag(Parent_richcmp_element_without_coercion):
+                return Py_TYPE(y).tp_richcompare(x, y, op)
+```
+because the `self` argument of `__richcmp__` should be of the correct type. So the last line should be replaced by
+
+```
+                return Py_TYPE(y).tp_richcompare(y, x, revop(op))
+```
+but this caused some breakage in `linear_functions.pyx` that needed to be fixed.
``````




---

archive/issue_comments_381827.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ef5813a9d2f72e91cde914e243981f5354196ea8\">ef5813a</a></td><td><code>Fix `__richcmp__` calls with reversed operands</code></td></tr></table>\n",
    "created_at": "2018-03-06T14:49:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24916#issuecomment-381827",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ef5813a9d2f72e91cde914e243981f5354196ea8">ef5813a</a></td><td><code>Fix `__richcmp__` calls with reversed operands</code></td></tr></table>




---

archive/issue_comments_381828.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-03-06T22:18:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24916#issuecomment-381828",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_381829.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIf the patchbot comes back green, then positive review.",
    "created_at": "2018-03-06T22:18:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24916#issuecomment-381829",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

If the patchbot comes back green, then positive review.



---

archive/issue_events_326664.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-08T09:45:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24916#event-326664"
}
```



---

archive/issue_events_326665.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-08T09:45:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24916#event-326665"
}
```



---

archive/issue_events_326666.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-19T07:57:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24916#event-326666"
}
```



---

archive/issue_events_326667.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "49f0a7e066fe11caa01cf5902c272632b58e12eb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-03-19T07:57:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24916#event-326667"
}
```



---

archive/issue_comments_381830.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/fix___richcmp___calls_with_reversed_operands](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix___richcmp___calls_with_reversed_operands)** to **[ef5813a](https://github.com/sagemath/sagetrac-mirror/commit/ef5813a9d2f72e91cde914e243981f5354196ea8)**",
    "created_at": "2018-03-19T07:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24916#issuecomment-381830",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/fix___richcmp___calls_with_reversed_operands](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix___richcmp___calls_with_reversed_operands)** to **[ef5813a](https://github.com/sagemath/sagetrac-mirror/commit/ef5813a9d2f72e91cde914e243981f5354196ea8)**
