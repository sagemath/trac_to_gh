# Issue 24267: Fix newton_sqrt() method

archive/issues_024030.json:
```json
{
    "assignees": [],
    "body": "The code does some strange things, like checking whether a variable name equals `\"a\"`. If it does, it tries to raise a string to an integer power.\n\nThis code was introduced in #7927.\n\nIn all doctests, the variable name is indeed `\"a\"` so we just cleanup the code with that assumption.\n\nCC:  @jbalakrishnan @robertwb @kedlaya\n\nBranch/Commit: **[6d1e840](https://github.com/sagemath/sagetrac-mirror/commit/6d1e84077c5fc96113cc07911bd2bdbf9493c691)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24267_\n\n",
    "closed_at": "2017-12-13T17:38:10Z",
    "created_at": "2017-11-22T14:03:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix newton_sqrt() method",
    "type": "issue",
    "updated_at": "2017-12-13T17:38:10Z",
    "url": "https://github.com/sagemath/sage/issues/24267",
    "user": "https://github.com/jdemeyer"
}
```
The code does some strange things, like checking whether a variable name equals `"a"`. If it does, it tries to raise a string to an integer power.

This code was introduced in #7927.

In all doctests, the variable name is indeed `"a"` so we just cleanup the code with that assumption.

CC:  @jbalakrishnan @robertwb @kedlaya

Branch/Commit: **[6d1e840](https://github.com/sagemath/sagetrac-mirror/commit/6d1e84077c5fc96113cc07911bd2bdbf9493c691)**

Reviewer: **Frédéric Chapoton**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/24267_





---

archive/issue_events_307178.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-22T14:03:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24267#event-307178"
}
```



---

archive/issue_events_307179.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-22T14:03:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24267#event-307179"
}
```



---

archive/issue_events_307180.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-22T14:03:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24267#event-307180"
}
```



---

archive/issue_events_307181.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-22T14:03:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24267#event-307181"
}
```



---

archive/issue_comments_370055.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2017-11-22T14:04:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370055",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_370056.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n The code does some strange things, like depending on a variable name, raising a string to a power...\n \n+This code was introduced in #7927.\n+\n It turns out that the `try:` block always raises an exception, so we can just remove it.\n``````\n",
    "created_at": "2017-11-22T14:04:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370056",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 The code does some strange things, like depending on a variable name, raising a string to a power...
 
+This code was introduced in #7927.
+
 It turns out that the `try:` block always raises an exception, so we can just remove it.
``````




---

archive/issue_comments_370057.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n-The code does some strange things, like depending on a variable name, raising a string to a power...\n+The code does some strange things, like checking whether a variable name equals `\"a\"`. If it does, it tries to raise a string to a integer power.\n \n This code was introduced in #7927.\n \n-It turns out that the `try:` block always raises an exception, so we can just remove it.\n+In all doctests, the variable name is indeed `\"a\"` so we just cleanup the code with that assumption.\n``````\n",
    "created_at": "2017-11-22T14:10:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370057",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
-The code does some strange things, like depending on a variable name, raising a string to a power...
+The code does some strange things, like checking whether a variable name equals `"a"`. If it does, it tries to raise a string to a integer power.
 
 This code was introduced in #7927.
 
-It turns out that the `try:` block always raises an exception, so we can just remove it.
+In all doctests, the variable name is indeed `"a"` so we just cleanup the code with that assumption.
``````




---

archive/issue_comments_370058.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-The code does some strange things, like checking whether a variable name equals `\"a\"`. If it does, it tries to raise a string to a integer power.\n+The code does some strange things, like checking whether a variable name equals `\"a\"`. If it does, it tries to raise a string to an integer power.\n \n This code was introduced in #7927.\n \n``````\n",
    "created_at": "2017-11-22T14:10:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370058",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-The code does some strange things, like checking whether a variable name equals `"a"`. If it does, it tries to raise a string to a integer power.
+The code does some strange things, like checking whether a variable name equals `"a"`. If it does, it tries to raise a string to an integer power.
 
 This code was introduced in #7927.
 
``````




---

archive/issue_comments_370059.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/24267](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/24267)**",
    "created_at": "2017-11-22T14:12:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370059",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/24267](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/24267)**



---

archive/issue_comments_370060.json:
```json
{
    "body": "Commit: **[6d1e840](https://github.com/sagemath/sagetrac-mirror/commit/6d1e84077c5fc96113cc07911bd2bdbf9493c691)**",
    "created_at": "2017-11-22T14:17:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370060",
    "user": "https://github.com/kedlaya"
}
```

Commit: **[6d1e840](https://github.com/sagemath/sagetrac-mirror/commit/6d1e84077c5fc96113cc07911bd2bdbf9493c691)**



---

archive/issue_comments_370061.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6d1e84077c5fc96113cc07911bd2bdbf9493c691\">6d1e840</a></td><td><code>Fix newton_sqrt() method</code></td></tr></table>\n",
    "created_at": "2017-11-22T14:17:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370061",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:5'>Comment 5:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6d1e84077c5fc96113cc07911bd2bdbf9493c691">6d1e840</a></td><td><code>Fix newton_sqrt() method</code></td></tr></table>




---

archive/issue_events_307182.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-22T14:56:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24267#event-307182"
}
```



---

archive/issue_comments_370062.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBefore returning `z`, you no longer adjust the precision.  Shouldn't you still add `O(x^prec)`?",
    "created_at": "2017-11-30T02:41:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370062",
    "user": "https://github.com/roed314"
}
```

<a id='comment:7'>Comment 7:</a>
Before returning `z`, you no longer adjust the precision.  Shouldn't you still add `O(x^prec)`?



---

archive/issue_comments_370063.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nReplying to [@roed314](#comment%3A7):\n> Before returning `z`, you no longer adjust the precision.\n\nThe old code didn't do that either, although it gave that impression. `x` is the string(!) `\"a\"`, so the code tried to add `O(\"a\" ** prec)` which obviously failed.\n\nI am just replacing that by the `except:` branch.",
    "created_at": "2017-11-30T06:48:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370063",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'>Comment 8:</a>
Replying to [@roed314](#comment%3A7):
> Before returning `z`, you no longer adjust the precision.

The old code didn't do that either, although it gave that impression. `x` is the string(!) `"a"`, so the code tried to add `O("a" ** prec)` which obviously failed.

I am just replacing that by the `except:` branch.



---

archive/issue_comments_370064.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThat being said, if you want to improve the code in a way which doesn't involve arithmetic with strings and which still passes doctests, that is fine for me.",
    "created_at": "2017-11-30T06:49:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370064",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'>Comment 9:</a>
That being said, if you want to improve the code in a way which doesn't involve arithmetic with strings and which still passes doctests, that is fine for me.



---

archive/issue_events_307183.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-12-06T12:22:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24267#event-307183"
}
```



---

archive/issue_events_307184.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-12-06T12:22:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24267#event-307184"
}
```



---

archive/issue_comments_370065.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nok",
    "created_at": "2017-12-06T12:22:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370065",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:10'>Comment 10:</a>
ok



---

archive/issue_comments_370066.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2017-12-06T12:22:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370066",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_307185.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-13T17:38:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24267#event-307185"
}
```



---

archive/issue_events_307186.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a2dffffce091dc4dd1a4616245d0aba43bd7b53d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-13T17:38:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24267#event-307186"
}
```



---

archive/issue_comments_370067.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/24267](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/24267)** to **[6d1e840](https://github.com/sagemath/sagetrac-mirror/commit/6d1e84077c5fc96113cc07911bd2bdbf9493c691)**",
    "created_at": "2017-12-13T17:38:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24267#issuecomment-370067",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/24267](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/24267)** to **[6d1e840](https://github.com/sagemath/sagetrac-mirror/commit/6d1e84077c5fc96113cc07911bd2bdbf9493c691)**
