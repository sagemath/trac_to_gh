# Issue 24175: Optimizations to generating rigged configurations

archive/issues_023938.json:
```json
{
    "assignees": [],
    "body": "We get more speed by\n\n- Faster method to compute `phi` and `_calc_vacancy_number` for `i = oo`.\n- Cythonization of `RiggedPartition`.\n\nNote #24141 is not a strict dependency, but it does take those optimizations of the Kleber tree further.\n\nDepends on #24141\n\nCC:  sage-combinat @anneschilling @bsalisbury1\n\nKeywords: **rigged configurations, crystals**\n\nBranch: **[65df5a1](https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047)**\n\nReviewer: **Ben Salisbury**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24175_\n\n",
    "closed_at": "2018-02-03T21:22:01Z",
    "created_at": "2017-11-08T05:55:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/performance",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Optimizations to generating rigged configurations",
    "type": "issue",
    "updated_at": "2023-01-08T10:35:26Z",
    "url": "https://github.com/sagemath/sage/issues/24175",
    "user": "https://github.com/tscrim"
}
```
We get more speed by

- Faster method to compute `phi` and `_calc_vacancy_number` for `i = oo`.
- Cythonization of `RiggedPartition`.

Note #24141 is not a strict dependency, but it does take those optimizations of the Kleber tree further.

Depends on #24141

CC:  sage-combinat @anneschilling @bsalisbury1

Keywords: **rigged configurations, crystals**

Branch: **[65df5a1](https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047)**

Reviewer: **Ben Salisbury**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/24175_





---

archive/issue_events_306017.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-08T05:55:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24175#event-306017"
}
```



---

archive/issue_events_306018.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-08T05:55:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "label": "https://github.com/sagemath/sage/labels/performance",
    "label_color": "696969",
    "label_name": "performance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24175#event-306018"
}
```



---

archive/issue_events_306019.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-08T05:55:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24175#event-306019"
}
```



---

archive/issue_events_306020.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-08T05:55:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24175#event-306020"
}
```



---

archive/issue_comments_368429.json:
```json
{
    "body": "Commit: **[65df5a1](https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047)**",
    "created_at": "2017-11-08T06:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24175#issuecomment-368429",
    "user": "https://github.com/tscrim"
}
```

Commit: **[65df5a1](https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047)**



---

archive/issue_events_306021.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-08T06:01:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24175#event-306021"
}
```



---

archive/issue_comments_368430.json:
```json
{
    "body": "Branch: **[public/combinat/optimize_rc_generation-24175](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/optimize_rc_generation-24175)**",
    "created_at": "2017-11-08T06:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24175#issuecomment-368430",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/combinat/optimize_rc_generation-24175](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/optimize_rc_generation-24175)**



---

archive/issue_comments_368431.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\n\n```\nsage: RC = RiggedConfigurations(['F',4,1], [[3,9]])\nsage: K = RC.kleber_tree()\nsage: %time len(RC.module_generators)\nCPU times: user 3.4 s, sys: 92 ms, total: 3.49 s\nWall time: 3.32 s\n71009\nsage: %time L = [mg.weight() for mg in RC.module_generators]\nCPU times: user 4.82 s, sys: 56 ms, total: 4.87 s\nWall time: 4.78 s\n```\nvs 8.1.beta9 with #24141:\n\n```\nsage: %time len(RC.module_generators)\nCPU times: user 4.12 s, sys: 80 ms, total: 4.2 s\nWall time: 4.18 s\n71009\nsage: %time L = [mg.weight() for mg in RC.module_generators]\nCPU times: user 7.42 s, sys: 52 ms, total: 7.47 s\nWall time: 7.42 s\n```\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/82bc1fb522d3bedabdb639debf88f75d73d60d09\">82bc1fb</a></td><td><code>Initial optimization to avoid polytopes for small number of possible choices.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047\">65df5a1</a></td><td><code>Faster _calc_vacancy_number and phi. Cythonized RiggedPartition.</code></td></tr></table>\n",
    "created_at": "2017-11-08T06:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24175#issuecomment-368431",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>

```
sage: RC = RiggedConfigurations(['F',4,1], [[3,9]])
sage: K = RC.kleber_tree()
sage: %time len(RC.module_generators)
CPU times: user 3.4 s, sys: 92 ms, total: 3.49 s
Wall time: 3.32 s
71009
sage: %time L = [mg.weight() for mg in RC.module_generators]
CPU times: user 4.82 s, sys: 56 ms, total: 4.87 s
Wall time: 4.78 s
```
vs 8.1.beta9 with #24141:

```
sage: %time len(RC.module_generators)
CPU times: user 4.12 s, sys: 80 ms, total: 4.2 s
Wall time: 4.18 s
71009
sage: %time L = [mg.weight() for mg in RC.module_generators]
CPU times: user 7.42 s, sys: 52 ms, total: 7.47 s
Wall time: 7.42 s
```

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/82bc1fb522d3bedabdb639debf88f75d73d60d09">82bc1fb</a></td><td><code>Initial optimization to avoid polytopes for small number of possible choices.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047">65df5a1</a></td><td><code>Faster _calc_vacancy_number and phi. Cythonized RiggedPartition.</code></td></tr></table>




---

archive/issue_events_306022.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2018-02-03T00:32:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24175#event-306022"
}
```



---

archive/issue_events_306023.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2018-02-03T00:32:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24175#event-306023"
}
```



---

archive/issue_comments_368432.json:
```json
{
    "body": "Reviewer: **Ben Salisbury**",
    "created_at": "2018-02-03T00:32:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24175#issuecomment-368432",
    "user": "https://github.com/bsalisbury1"
}
```

Reviewer: **Ben Salisbury**



---

archive/issue_comments_368433.json:
```json
{
    "body": "Changed branch from **[public/combinat/optimize_rc_generation-24175](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/optimize_rc_generation-24175)** to **[65df5a1](https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047)**",
    "created_at": "2018-02-03T21:22:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24175#issuecomment-368433",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/combinat/optimize_rc_generation-24175](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/optimize_rc_generation-24175)** to **[65df5a1](https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047)**



---

archive/issue_events_306024.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-03T21:22:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24175#event-306024"
}
```



---

archive/issue_events_306025.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2d1615636ee16330df97367d7e93cc49c6cb3ac8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-03T21:22:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24175#event-306025"
}
```



---

archive/issue_comments_368434.json:
```json
{
    "body": "Changed commit from **[65df5a1](https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047)** to none",
    "created_at": "2023-01-08T10:35:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24175",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24175#issuecomment-368434",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[65df5a1](https://github.com/sagemath/sagetrac-mirror/commit/65df5a1a720fb949838ad045aa4a526a3631f047)** to none
