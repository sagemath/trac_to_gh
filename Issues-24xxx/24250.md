# Issue 24250: py3: do not use mathjax from inside sagenb

archive/issues_024013.json:
```json
{
    "assignees": [],
    "body": "currently, there is\n\n```\n/home/chapoton/sage3/src/doc/common/conf.py in <module>()\n    221 \n    222     from pkg_resources import Requirement, working_set\n--> 223     sagenb_path = working_set.find(Requirement.parse('sagenb')).location\n    224     mathjax_relative = os.path.join('sagenb','data','mathjax')\n    225 \nAttributeError: 'NoneType' object has no attribute 'location'\n```\nThis error happens in a python3-built-sage, where sagenb is not installed.\n\nBranch: **[8a6e110](https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8)**\n\nReviewer: **John Palmieri**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24250_\n\n",
    "closed_at": "2017-12-13T17:38:33Z",
    "created_at": "2017-11-20T15:06:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: do not use mathjax from inside sagenb",
    "type": "issue",
    "updated_at": "2017-12-15T12:03:09Z",
    "url": "https://github.com/sagemath/sage/issues/24250",
    "user": "https://github.com/fchapoton"
}
```
currently, there is

```
/home/chapoton/sage3/src/doc/common/conf.py in <module>()
    221 
    222     from pkg_resources import Requirement, working_set
--> 223     sagenb_path = working_set.find(Requirement.parse('sagenb')).location
    224     mathjax_relative = os.path.join('sagenb','data','mathjax')
    225 
AttributeError: 'NoneType' object has no attribute 'location'
```
This error happens in a python3-built-sage, where sagenb is not installed.

Branch: **[8a6e110](https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8)**

Reviewer: **John Palmieri**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/24250_





---

archive/issue_events_318388.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-20T15:06:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318388"
}
```



---

archive/issue_events_318389.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-20T15:06:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318389"
}
```



---

archive/issue_events_318390.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-20T15:06:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318390"
}
```



---

archive/issue_events_318391.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-20T15:06:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318391"
}
```



---

archive/issue_comments_369748.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,6 +7,6 @@\n --> 223     sagenb_path = working_set.find(Requirement.parse('sagenb')).location\n     224     mathjax_relative = os.path.join('sagenb','data','mathjax')\n     225 \n-\n AttributeError: 'NoneType' object has no attribute 'location'\n ```\n+This error happens in a python3-built-sage, where sagenb is not installed.\n``````\n",
    "created_at": "2017-11-20T15:07:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369748",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,6 +7,6 @@
 --> 223     sagenb_path = working_set.find(Requirement.parse('sagenb')).location
     224     mathjax_relative = os.path.join('sagenb','data','mathjax')
     225 
-
 AttributeError: 'NoneType' object has no attribute 'location'
 ```
+This error happens in a python3-built-sage, where sagenb is not installed.
``````




---

archive/issue_comments_369749.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nShould be easy since `mathjax` is a Sage package now.",
    "created_at": "2017-11-20T16:15:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369749",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
Should be easy since `mathjax` is a Sage package now.



---

archive/issue_comments_369750.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nok, but where to find the mathjax directory ?\n\n**EDIT** : Found it, in sage/local/share/mathjax/",
    "created_at": "2017-11-20T16:16:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369750",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
ok, but where to find the mathjax directory ?

**EDIT** : Found it, in sage/local/share/mathjax/



---

archive/issue_comments_369751.json:
```json
{
    "body": "Branch: **[u/chapoton/24250](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/24250)**",
    "created_at": "2017-11-20T17:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369751",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/24250](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/24250)**



---

archive/issue_comments_369752.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8\">8a6e110</a></td><td><code>py 24250 mathjax without sagenb</code></td></tr></table>\n",
    "created_at": "2017-11-20T17:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369752",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8">8a6e110</a></td><td><code>py 24250 mathjax without sagenb</code></td></tr></table>




---

archive/issue_events_318392.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-20T17:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318392"
}
```



---

archive/issue_comments_369753.json:
```json
{
    "body": "Commit: **[8a6e110](https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8)**",
    "created_at": "2017-11-20T17:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369753",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[8a6e110](https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8)**



---

archive/issue_comments_369754.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\ngreen bot, please review",
    "created_at": "2017-11-20T21:08:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369754",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
green bot, please review



---

archive/issue_comments_369755.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2017-11-20T23:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369755",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_369756.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2017-11-20T23:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369756",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_events_318393.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-11-20T23:39:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318393"
}
```



---

archive/issue_events_318394.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-11-20T23:39:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318394"
}
```



---

archive/issue_events_318395.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-11-20T23:39:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318395"
}
```



---

archive/issue_events_318396.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2017-11-20T23:39:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318396"
}
```



---

archive/issue_comments_369757.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThe changes make sense. I'm changing the milestone to 8.2: this is not crucial for the 8.1 release.",
    "created_at": "2017-11-20T23:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369757",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:6'>Comment 6:</a>
The changes make sense. I'm changing the milestone to 8.2: this is not crucial for the 8.1 release.



---

archive/issue_comments_369758.json:
```json
{
    "body": "Changed branch from **[u/chapoton/24250](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/24250)** to **[8a6e110](https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8)**",
    "created_at": "2017-12-13T17:38:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369758",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/24250](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/24250)** to **[8a6e110](https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8)**



---

archive/issue_events_318397.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-13T17:38:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318397"
}
```



---

archive/issue_events_318398.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "91b2e224ddf5f71e4001882bd36adb652f06f7b7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-13T17:38:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24250#event-318398"
}
```



---

archive/issue_comments_369759.json:
```json
{
    "body": "Changed commit from **[8a6e110](https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8)** to none",
    "created_at": "2017-12-15T12:03:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369759",
    "user": "https://github.com/embray"
}
```

Changed commit from **[8a6e110](https://github.com/sagemath/sagetrac-mirror/commit/8a6e1107658af89f0cc3e66107d7ea4d455755a8)** to none



---

archive/issue_comments_369760.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nAh, thanks for fixing this. I didn't find it before since it wasn't in the \"python3\" component, but this should save some hassle.",
    "created_at": "2017-12-15T12:03:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24250#issuecomment-369760",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'>Comment 8:</a>
Ah, thanks for fixing this. I didn't find it before since it wasn't in the "python3" component, but this should save some hassle.
