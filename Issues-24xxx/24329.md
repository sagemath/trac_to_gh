# Issue 24329: Upgrade to pynac-0.7.14

archive/issues_024092.json:
```json
{
    "assignees": [],
    "body": "In 0.7.14:\n* fix subs regression (#24262)\n* improve inexact numerics (#24299, #24317)\n* `(x<sup>-a)</sup>-b --> (x<sup>a)</sup>b`, with `a,b numeric > 0`\n* fix `mul` of infinities (#24327)\n* implement `has_function()` (#24283)\n* use arb for inverse trig/hyperbolic funs\n* internally rename `tgamma` to `gamma` (affects print order)\n\nIn 0.7.13:\n* fix `ex.coefficients()` bugs (#23545, #24147)\n* fix interface to Singular GCD (#23845)\n* fix compiler warnings\n* `asin`/`atan` enhancements (#24211)\n* `set_of_all()` function (#24176)\n* upgrade `ax_cxx_compile_stdcxx_11.m4`\n\nhttps://github.com/pynac/pynac/releases/download/pynac-0.7.14/pynac-0.7.14.tar.bz2\n\nCC:  @kiwifb\n\nBranch/Commit: **[ad68aeb](https://github.com/sagemath/sagetrac-mirror/commit/ad68aeb0da6d58bc719d96b54fcc26e04ea9f34f)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24329_\n\n",
    "closed_at": "2017-12-18T19:39:19Z",
    "created_at": "2017-12-06T08:44:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Upgrade to pynac-0.7.14",
    "type": "issue",
    "updated_at": "2017-12-18T19:39:19Z",
    "url": "https://github.com/sagemath/sage/issues/24329",
    "user": "https://github.com/rwst"
}
```
In 0.7.14:
* fix subs regression (#24262)
* improve inexact numerics (#24299, #24317)
* `(x<sup>-a)</sup>-b --> (x<sup>a)</sup>b`, with `a,b numeric > 0`
* fix `mul` of infinities (#24327)
* implement `has_function()` (#24283)
* use arb for inverse trig/hyperbolic funs
* internally rename `tgamma` to `gamma` (affects print order)

In 0.7.13:
* fix `ex.coefficients()` bugs (#23545, #24147)
* fix interface to Singular GCD (#23845)
* fix compiler warnings
* `asin`/`atan` enhancements (#24211)
* `set_of_all()` function (#24176)
* upgrade `ax_cxx_compile_stdcxx_11.m4`

https://github.com/pynac/pynac/releases/download/pynac-0.7.14/pynac-0.7.14.tar.bz2

CC:  @kiwifb

Branch/Commit: **[ad68aeb](https://github.com/sagemath/sagetrac-mirror/commit/ad68aeb0da6d58bc719d96b54fcc26e04ea9f34f)**

Reviewer: **Travis Scrimshaw**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/24329_





---

archive/issue_events_319362.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-06T08:44:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24329#event-319362"
}
```



---

archive/issue_events_319363.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-06T08:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24329#event-319363"
}
```



---

archive/issue_events_319364.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-06T08:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24329#event-319364"
}
```



---

archive/issue_events_319365.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-06T08:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24329#event-319365"
}
```



---

archive/issue_comments_370962.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,10 +1,10 @@\n In 0.7.14:\n * fix subs regression (#24262)\n * improve inexact numerics (#24299, #24317)\n-* (x<sup>-a)</sup>-b --> (x<sup>a)</sup>b, with a,b numeric > 0\n-* fix mul of infinities (#24327)\n-* implement has_function() (#24283)\n+* `(x<sup>-a)</sup>-b --> (x<sup>a)</sup>b`, with `a,b numeric > 0`\n+* fix `mul` of infinities (#24327)\n+* implement `has_function()` (#24283)\n * use arb for inverse trig/hyperbolic funs\n-* internally rename tgamma to gamma (affects print order)\n+* internally rename `tgamma` to `gamma` (affects print order)\n \n https://github.com/pynac/pynac/releases/download/pynac-0.7.14/pynac-0.7.14.tar.bz2\n``````\n",
    "created_at": "2017-12-06T08:45:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370962",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,10 +1,10 @@
 In 0.7.14:
 * fix subs regression (#24262)
 * improve inexact numerics (#24299, #24317)
-* (x<sup>-a)</sup>-b --> (x<sup>a)</sup>b, with a,b numeric > 0
-* fix mul of infinities (#24327)
-* implement has_function() (#24283)
+* `(x<sup>-a)</sup>-b --> (x<sup>a)</sup>b`, with `a,b numeric > 0`
+* fix `mul` of infinities (#24327)
+* implement `has_function()` (#24283)
 * use arb for inverse trig/hyperbolic funs
-* internally rename tgamma to gamma (affects print order)
+* internally rename `tgamma` to `gamma` (affects print order)
 
 https://github.com/pynac/pynac/releases/download/pynac-0.7.14/pynac-0.7.14.tar.bz2
``````




---

archive/issue_comments_370963.json:
```json
{
    "body": "Dependencies: **#24318**",
    "created_at": "2017-12-06T08:48:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370963",
    "user": "https://github.com/rwst"
}
```

Dependencies: **#24318**



---

archive/issue_comments_370964.json:
```json
{
    "body": "Branch: **[u/rws/upgrade_to_pynac_0_7_14](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_to_pynac_0_7_14)**",
    "created_at": "2017-12-06T09:06:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370964",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/upgrade_to_pynac_0_7_14](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_to_pynac_0_7_14)**



---

archive/issue_comments_370965.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9252fbd8d1bf8c8c651742c185ae2f527fbee962\">9252fbd</a></td><td><code>24329: doctest additions, adaptations</code></td></tr></table>\n",
    "created_at": "2017-12-06T14:19:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370965",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9252fbd8d1bf8c8c651742c185ae2f527fbee962">9252fbd</a></td><td><code>24329: doctest additions, adaptations</code></td></tr></table>




---

archive/issue_comments_370966.json:
```json
{
    "body": "Commit: **[9252fbd](https://github.com/sagemath/sagetrac-mirror/commit/9252fbd8d1bf8c8c651742c185ae2f527fbee962)**",
    "created_at": "2017-12-06T14:19:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370966",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[9252fbd](https://github.com/sagemath/sagetrac-mirror/commit/9252fbd8d1bf8c8c651742c185ae2f527fbee962)**



---

archive/issue_events_319366.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-11T13:55:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24329#event-319366"
}
```



---

archive/issue_comments_370967.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,4 +7,12 @@\n * use arb for inverse trig/hyperbolic funs\n * internally rename `tgamma` to `gamma` (affects print order)\n \n+In 0.7.13:\n+* fix `ex.coefficients()` bugs (#23545, #24147)\n+* fix interface to Singular GCD (#23845)\n+* fix compiler warnings\n+* `asin`/`atan` enhancements (#24211)\n+* `set_of_all()` function (#24176)\n+* upgrade `ax_cxx_compile_stdcxx_11.m4`\n+\n https://github.com/pynac/pynac/releases/download/pynac-0.7.14/pynac-0.7.14.tar.bz2\n``````\n",
    "created_at": "2017-12-11T13:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370967",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,4 +7,12 @@
 * use arb for inverse trig/hyperbolic funs
 * internally rename `tgamma` to `gamma` (affects print order)
 
+In 0.7.13:
+* fix `ex.coefficients()` bugs (#23545, #24147)
+* fix interface to Singular GCD (#23845)
+* fix compiler warnings
+* `asin`/`atan` enhancements (#24211)
+* `set_of_all()` function (#24176)
+* upgrade `ax_cxx_compile_stdcxx_11.m4`
+
 https://github.com/pynac/pynac/releases/download/pynac-0.7.14/pynac-0.7.14.tar.bz2
``````




---

archive/issue_comments_370968.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nDouble upgrade since 0.7.13 doesn't make it into 8.2.beta1.",
    "created_at": "2017-12-11T13:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370968",
    "user": "https://github.com/rwst"
}
```

<div id="comment:6" align="right">Comment 6</div>

Double upgrade since 0.7.13 doesn't make it into 8.2.beta1.



---

archive/issue_comments_370969.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ad68aeb0da6d58bc719d96b54fcc26e04ea9f34f\">ad68aeb</a></td><td><code>Merge branch 'develop' into t/24329/upgrade_to_pynac_0_7_14</code></td></tr></table>\n",
    "created_at": "2017-12-14T14:24:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370969",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ad68aeb0da6d58bc719d96b54fcc26e04ea9f34f">ad68aeb</a></td><td><code>Merge branch 'develop' into t/24329/upgrade_to_pynac_0_7_14</code></td></tr></table>




---

archive/issue_comments_370970.json:
```json
{
    "body": "Changed commit from **[9252fbd](https://github.com/sagemath/sagetrac-mirror/commit/9252fbd8d1bf8c8c651742c185ae2f527fbee962)** to **[ad68aeb](https://github.com/sagemath/sagetrac-mirror/commit/ad68aeb0da6d58bc719d96b54fcc26e04ea9f34f)**",
    "created_at": "2017-12-14T14:24:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370970",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9252fbd](https://github.com/sagemath/sagetrac-mirror/commit/9252fbd8d1bf8c8c651742c185ae2f527fbee962)** to **[ad68aeb](https://github.com/sagemath/sagetrac-mirror/commit/ad68aeb0da6d58bc719d96b54fcc26e04ea9f34f)**



---

archive/issue_comments_370971.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2017-12-14T14:25:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370971",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_370972.json:
```json
{
    "body": "Changed dependencies from **#24318** to none",
    "created_at": "2017-12-14T14:25:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370972",
    "user": "https://github.com/rwst"
}
```

Changed dependencies from **#24318** to none



---

archive/issue_comments_370973.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBuildbots, come forth. :P",
    "created_at": "2017-12-15T05:06:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370973",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">Comment 9</div>

Buildbots, come forth. :P



---

archive/issue_events_319367.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-15T05:06:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24329#event-319367"
}
```



---

archive/issue_events_319368.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-15T05:06:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24329#event-319368"
}
```



---

archive/issue_comments_370974.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-12-15T05:06:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370974",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_319369.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-18T19:39:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24329#event-319369"
}
```



---

archive/issue_events_319370.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e6fb1cf2a4f035b1e2c7cab1d67077d82a7c9cf1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-18T19:39:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24329#event-319370"
}
```



---

archive/issue_comments_370975.json:
```json
{
    "body": "Changed branch from **[u/rws/upgrade_to_pynac_0_7_14](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_to_pynac_0_7_14)** to **[ad68aeb](https://github.com/sagemath/sagetrac-mirror/commit/ad68aeb0da6d58bc719d96b54fcc26e04ea9f34f)**",
    "created_at": "2017-12-18T19:39:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24329#issuecomment-370975",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/upgrade_to_pynac_0_7_14](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_to_pynac_0_7_14)** to **[ad68aeb](https://github.com/sagemath/sagetrac-mirror/commit/ad68aeb0da6d58bc719d96b54fcc26e04ea9f34f)**
