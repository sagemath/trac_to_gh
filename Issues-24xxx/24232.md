...
```json
{
    "body": "<a id='comment:17'></a>The last commit gets rid of any string manipulation in `SimplifySqrtReal` and `SimplifyAbsTrig`, following the suggestion in[comment:13](#comment%3A13).",
...
<a id='comment:17'></a>The last commit gets rid of any string manipulation in `SimplifySqrtReal` and `SimplifyAbsTrig`, following the suggestion in[comment:13](#comment%3A13).
...
<a id='comment:19'></a>Replying to [rllozes](#comment%3A18):
> Builds and tests cleanly atop 8.1.rc0 on an OpenSUSE 42.2 system.
> So far, randomly selected worksheets complete successfully and correctly.
> 
