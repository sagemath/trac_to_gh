# Issue 24391: DynkinDiagram.marked_nodes doesn't work

archive/issues_024154.json:
```json
{
    "assignees": [],
    "body": "\n```\nD = DynkinDiagram(\"A3\")\nD.marked_nodes([1,3])\n```\n\nreturns \n\n\n```\nTraceback (most recent call last):\n  File \"/cocalc/lib/python2.7/site-packages/smc_sagews/sage_server.py\", line 1013, in execute\n    exec compile(block+'\\n', '', 'single') in namespace, locals\n  File \"\", line 1, in <module>\n  File \"/cocalc/lib/python2.7/site-packages/smc_sagews/sage_salvus.py\", line 3596, in displayhook\n    _system_sys_displayhook(obj)\n  File \"sage/structure/sage_object.pyx\", line 237, in sage.structure.sage_object.SageObject.__repr__ (/ext/sage/sage-8.0/src/build/cythonized/sage/structure/sage_object.c:2642)\n    result = repr_func()\n  File \"/ext/sage/sage-8.0/local/lib/python2.7/site-packages/sage/combinat/root_system/type_marked.py\", line 165, in _repr_\n    return self._type._repr_(compact = compact)+\" with nodes {} marked\".format(self._marked_nodes)\nTypeError: _repr_() got an unexpected keyword argument 'compact'\n```\n\n\nCC:  @sagetrac-sage-combinat @fchapoton @nthiery\n\nBranch/Commit: **[504f127](https://github.com/sagemath/sagetrac-mirror/commit/504f12773ecb7c21d9c97c5cab73b300b61ba9cd)**\n\nReviewer: **V\u00edt Tu\u010dek**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24391_\n\n",
    "closed_at": "2018-02-22T17:40:49Z",
    "created_at": "2017-12-16T14:03:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "DynkinDiagram.marked_nodes doesn't work",
    "type": "issue",
    "updated_at": "2018-02-22T17:40:49Z",
    "url": "https://github.com/sagemath/sage/issues/24391",
    "user": "https://github.com/vit-tucek"
}
```

```
D = DynkinDiagram("A3")
D.marked_nodes([1,3])
```

returns 


```
Traceback (most recent call last):
  File "/cocalc/lib/python2.7/site-packages/smc_sagews/sage_server.py", line 1013, in execute
    exec compile(block+'\n', '', 'single') in namespace, locals
  File "", line 1, in <module>
  File "/cocalc/lib/python2.7/site-packages/smc_sagews/sage_salvus.py", line 3596, in displayhook
    _system_sys_displayhook(obj)
  File "sage/structure/sage_object.pyx", line 237, in sage.structure.sage_object.SageObject.__repr__ (/ext/sage/sage-8.0/src/build/cythonized/sage/structure/sage_object.c:2642)
    result = repr_func()
  File "/ext/sage/sage-8.0/local/lib/python2.7/site-packages/sage/combinat/root_system/type_marked.py", line 165, in _repr_
    return self._type._repr_(compact = compact)+" with nodes {} marked".format(self._marked_nodes)
TypeError: _repr_() got an unexpected keyword argument 'compact'
```


CC:  @sagetrac-sage-combinat @fchapoton @nthiery

Branch/Commit: **[504f127](https://github.com/sagemath/sagetrac-mirror/commit/504f12773ecb7c21d9c97c5cab73b300b61ba9cd)**

Reviewer: **Vít Tuček**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/24391_





---

archive/issue_events_320083.json:
```json
{
    "actor": "https://github.com/vit-tucek",
    "created_at": "2017-12-16T14:03:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24391#event-320083"
}
```



---

archive/issue_events_320084.json:
```json
{
    "actor": "https://github.com/vit-tucek",
    "created_at": "2017-12-16T14:03:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24391#event-320084"
}
```



---

archive/issue_events_320085.json:
```json
{
    "actor": "https://github.com/vit-tucek",
    "created_at": "2017-12-16T14:03:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24391#event-320085"
}
```



---

archive/issue_events_320086.json:
```json
{
    "actor": "https://github.com/vit-tucek",
    "created_at": "2017-12-16T14:03:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24391#event-320086"
}
```



---

archive/issue_comments_372122.json:
```json
{
    "body": "Commit: **[504f127](https://github.com/sagemath/sagetrac-mirror/commit/504f12773ecb7c21d9c97c5cab73b300b61ba9cd)**",
    "created_at": "2017-12-18T02:55:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372122",
    "user": "https://github.com/tscrim"
}
```

Commit: **[504f127](https://github.com/sagemath/sagetrac-mirror/commit/504f12773ecb7c21d9c97c5cab73b300b61ba9cd)**



---

archive/issue_events_320087.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-18T02:55:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24391#event-320087"
}
```



---

archive/issue_comments_372123.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/504f12773ecb7c21d9c97c5cab73b300b61ba9cd\">504f127</a></td><td><code>Fixing `_repr_` for marked types.</code></td></tr></table>\n",
    "created_at": "2017-12-18T02:55:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372123",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/504f12773ecb7c21d9c97c5cab73b300b61ba9cd">504f127</a></td><td><code>Fixing `_repr_` for marked types.</code></td></tr></table>




---

archive/issue_comments_372124.json:
```json
{
    "body": "Branch: **[public/combinat/fix_repr_marked_dynkin_diagram-24391](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_repr_marked_dynkin_diagram-24391)**",
    "created_at": "2017-12-18T02:55:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372124",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/combinat/fix_repr_marked_dynkin_diagram-24391](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_repr_marked_dynkin_diagram-24391)**



---

archive/issue_comments_372125.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2017-12-18T02:55:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372125",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_comments_372126.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nWhy do you catch `TypeError` exception?",
    "created_at": "2018-02-20T17:47:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372126",
    "user": "https://github.com/vit-tucek"
}
```

<div id="comment:3" align="right">Comment 3</div>

Why do you catch `TypeError` exception?



---

archive/issue_comments_372127.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBecause that is what happens when you pass unexpected keyword arguments as noted in the ticket description. Admittedly, the proposed fix is a slight hack, but IIRC, there were some issues with adding a `compact` argument to all possible inputs and this is not an unpythonic solution.",
    "created_at": "2018-02-20T22:41:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372127",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

Because that is what happens when you pass unexpected keyword arguments as noted in the ticket description. Admittedly, the proposed fix is a slight hack, but IIRC, there were some issues with adding a `compact` argument to all possible inputs and this is not an unpythonic solution.



---

archive/issue_comments_372128.json:
```json
{
    "body": "Reviewer: **V\u00edt Tu\u010dek**",
    "created_at": "2018-02-21T08:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372128",
    "user": "https://github.com/vit-tucek"
}
```

Reviewer: **Vít Tuček**



---

archive/issue_comments_372129.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nOh yeah. Sorry. Long day.",
    "created_at": "2018-02-21T08:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372129",
    "user": "https://github.com/vit-tucek"
}
```

<div id="comment:5" align="right">Comment 5</div>

Oh yeah. Sorry. Long day.



---

archive/issue_events_320088.json:
```json
{
    "actor": "https://github.com/vit-tucek",
    "created_at": "2018-02-21T08:51:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24391#event-320088"
}
```



---

archive/issue_events_320089.json:
```json
{
    "actor": "https://github.com/vit-tucek",
    "created_at": "2018-02-21T08:51:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24391#event-320089"
}
```



---

archive/issue_comments_372130.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nNo problem. Thanks for the review.",
    "created_at": "2018-02-21T11:01:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372130",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

No problem. Thanks for the review.



---

archive/issue_events_320090.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-22T17:40:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24391#event-320090"
}
```



---

archive/issue_events_320091.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "196d3cc383daa2a36f7f7ecd68f562f0540de8b1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-22T17:40:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24391#event-320091"
}
```



---

archive/issue_comments_372131.json:
```json
{
    "body": "Changed branch from **[public/combinat/fix_repr_marked_dynkin_diagram-24391](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_repr_marked_dynkin_diagram-24391)** to **[504f127](https://github.com/sagemath/sagetrac-mirror/commit/504f12773ecb7c21d9c97c5cab73b300b61ba9cd)**",
    "created_at": "2018-02-22T17:40:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24391",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24391#issuecomment-372131",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/combinat/fix_repr_marked_dynkin_diagram-24391](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_repr_marked_dynkin_diagram-24391)** to **[504f127](https://github.com/sagemath/sagetrac-mirror/commit/504f12773ecb7c21d9c97c5cab73b300b61ba9cd)**
