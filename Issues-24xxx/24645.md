# Issue 24645: Support for spkg-postinst scripts

archive/issues_024408.json:
```json
{
    "body": "This is a change separated out from #22509, to add support for `spkg-postinst` scripts in SPKGs.  These are run after `spkg-install` and, in the case of packages that support `$SAGE_DESTDIR`, after the package's main files have been copied into `$SAGE_LOCAL`.\n\nThis allows for post-installation steps that are easier to perform once the package has been installed into its runtime context.  It *may* be used to generate additional files which are not tracked.  In this it would be good to have an `spkg-postrm` script that cleans up any such files, but for now that is omitted since we don't have proper package uninstallation yet (this was added in #22510 which still needs to be updated).\n\nBranch/Commit: a1b1c166196a63f1e06bad0939cae60b74fde8df\n\nReviewer: Julian R\u00fcth\n\nAuthor: Erik Bray\n\nDependencies: #24644\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/24645\n\n",
    "closed_at": "2018-05-08T17:26:25Z",
    "created_at": "2018-02-02T13:03:31Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.3",
    "title": "Support for spkg-postinst scripts",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24645",
    "user": "https://github.com/embray"
}
```
This is a change separated out from #22509, to add support for `spkg-postinst` scripts in SPKGs.  These are run after `spkg-install` and, in the case of packages that support `$SAGE_DESTDIR`, after the package's main files have been copied into `$SAGE_LOCAL`.

This allows for post-installation steps that are easier to perform once the package has been installed into its runtime context.  It *may* be used to generate additional files which are not tracked.  In this it would be good to have an `spkg-postrm` script that cleans up any such files, but for now that is omitted since we don't have proper package uninstallation yet (this was added in #22510 which still needs to be updated).

Branch/Commit: a1b1c166196a63f1e06bad0939cae60b74fde8df

Reviewer: Julian Rüth

Author: Erik Bray

Dependencies: #24644

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/24645





---

archive/issue_comments_467235.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-02-02T13:03:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467235",
    "user": "https://github.com/embray"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_467236.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#24644\"",
    "created_at": "2018-02-02T13:49:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467236",
    "user": "https://github.com/embray"
}
```

Changing dependencies from "" to "#24644"



---

archive/issue_comments_467237.json:
```json
{
    "body": "Changing type from defect to enhancement.",
    "created_at": "2018-02-02T13:50:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467237",
    "user": "https://github.com/embray"
}
```

Changing type from defect to enhancement.



---

archive/issue_comments_467238.json:
```json
{
    "body": "<a id='comment:4'></a>\nNote: #25139 now provides `spkg-postrm` and `spkg-prerm` capabilities.",
    "created_at": "2018-04-18T09:43:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467238",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'></a>
Note: #25139 now provides `spkg-postrm` and `spkg-prerm` capabilities.



---

archive/issue_comments_467239.json:
```json
{
    "body": "<a id='comment:5'></a>\n`additionall` is a typo I think.",
    "created_at": "2018-04-21T18:59:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467239",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:5'></a>
`additionall` is a typo I think.



---

archive/issue_comments_467240.json:
```json
{
    "body": "<a id='comment:6'></a>\nCould we add a ticket number to the TODO that tracks this?",
    "created_at": "2018-04-21T18:59:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467240",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:6'></a>
Could we add a ticket number to the TODO that tracks this?



---

archive/issue_comments_467241.json:
```json
{
    "body": "<a id='comment:7'></a>\nOtherwise, feel free to set this to positive review.",
    "created_at": "2018-04-21T19:00:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467241",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:7'></a>
Otherwise, feel free to set this to positive review.



---

archive/issue_comments_467242.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Julian R\u00fcth\"",
    "created_at": "2018-04-21T19:00:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467242",
    "user": "https://github.com/saraedum"
}
```

Changing reviewer from "" to "Julian Rüth"



---

archive/issue_comments_467243.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-04-23T12:11:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467243",
    "user": "https://github.com/embray"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_467244.json:
```json
{
    "body": "<a id='comment:8'></a>\nIn #25139 I actually removed the TODO comment, since there's a ticket for that now.  I forgot that was originally introduced in this ticket.  I'll just clean that up and set positive review.",
    "created_at": "2018-04-23T12:11:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467244",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>
In #25139 I actually removed the TODO comment, since there's a ticket for that now.  I forgot that was originally introduced in this ticket.  I'll just clean that up and set positive review.



---

archive/issue_comments_467245.json:
```json
{
    "body": "<a id='comment:9'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                                                                                    |\n|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------|\n|[4c66f35](https://git.sagemath.org/sage.git/commit/?id=4c66f35c2ba7835bdf2cdde253508889ceda98f3)|`Added the ability for packages to have a post-install script called spkg-postinst.`|\n|[168eaeb](https://git.sagemath.org/sage.git/commit/?id=168eaebfc7beb348cd6d3afd542091a9b43e45e6)|`The postinst script should probably be run with SAGE_SUDO if it is set`|\n|[9499342](https://git.sagemath.org/sage.git/commit/?id=9499342c7f8486243706bfeb93c71e2d7240ddf1)|`Add a paragraph to the docs about spkg-postinst`|",
    "created_at": "2018-04-26T08:50:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467245",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:9'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                                                                                    |
|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------|
|[4c66f35](https://git.sagemath.org/sage.git/commit/?id=4c66f35c2ba7835bdf2cdde253508889ceda98f3)|`Added the ability for packages to have a post-install script called spkg-postinst.`|
|[168eaeb](https://git.sagemath.org/sage.git/commit/?id=168eaebfc7beb348cd6d3afd542091a9b43e45e6)|`The postinst script should probably be run with SAGE_SUDO if it is set`|
|[9499342](https://git.sagemath.org/sage.git/commit/?id=9499342c7f8486243706bfeb93c71e2d7240ddf1)|`Add a paragraph to the docs about spkg-postinst`|



---

archive/issue_comments_467246.json:
```json
{
    "body": "Changing commit from \"828fdf7837c1e8f127a95053e8ae150abb087932\" to \"9499342c7f8486243706bfeb93c71e2d7240ddf1\"",
    "created_at": "2018-04-26T08:50:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467246",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "828fdf7837c1e8f127a95053e8ae150abb087932" to "9499342c7f8486243706bfeb93c71e2d7240ddf1"



---

archive/issue_comments_467247.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2018-04-26T09:11:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467247",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_467248.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2018-04-26T09:24:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467248",
    "user": "https://github.com/embray"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_467249.json:
```json
{
    "body": "<a id='comment:11'></a>\nFrom #25139, I see there's still an unresolved issue here: When running post-install scripts the `$SAGE_DESTDIR` variable should *not* be set anymore (or even unset).\n\nBy design, the post-install scripts run after the package files have already been moved into the real `$SAGE_LOCAL`, so any commands run in the post-install scripts should not use `$SAGE_DESTDIR`.",
    "created_at": "2018-04-26T09:24:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467249",
    "user": "https://github.com/embray"
}
```

<a id='comment:11'></a>
From #25139, I see there's still an unresolved issue here: When running post-install scripts the `$SAGE_DESTDIR` variable should *not* be set anymore (or even unset).

By design, the post-install scripts run after the package files have already been moved into the real `$SAGE_LOCAL`, so any commands run in the post-install scripts should not use `$SAGE_DESTDIR`.



---

archive/issue_comments_467250.json:
```json
{
    "body": "<a id='comment:12'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                                                                                 |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------|\n|[a1b1c16](https://git.sagemath.org/sage.git/commit/?id=a1b1c166196a63f1e06bad0939cae60b74fde8df)|`Ensure SAGE_DESTDIR and SAGE_DESTDIR_LOCAL are unset once they are no longer needed (in particular, before the postinst script)`|",
    "created_at": "2018-04-26T10:05:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467250",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:12'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                                                                                 |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------|
|[a1b1c16](https://git.sagemath.org/sage.git/commit/?id=a1b1c166196a63f1e06bad0939cae60b74fde8df)|`Ensure SAGE_DESTDIR and SAGE_DESTDIR_LOCAL are unset once they are no longer needed (in particular, before the postinst script)`|



---

archive/issue_comments_467251.json:
```json
{
    "body": "Changing commit from \"9499342c7f8486243706bfeb93c71e2d7240ddf1\" to \"a1b1c166196a63f1e06bad0939cae60b74fde8df\"",
    "created_at": "2018-04-26T10:05:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467251",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "9499342c7f8486243706bfeb93c71e2d7240ddf1" to "a1b1c166196a63f1e06bad0939cae60b74fde8df"



---

archive/issue_comments_467252.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2018-04-26T10:06:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467252",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_467253.json:
```json
{
    "body": "<a id='comment:13'></a>\nOk, I confirmed that this fixed the issue in #25139, so I'll set back to positive review unless anyone has an objection.",
    "created_at": "2018-04-26T10:06:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467253",
    "user": "https://github.com/embray"
}
```

<a id='comment:13'></a>
Ok, I confirmed that this fixed the issue in #25139, so I'll set back to positive review unless anyone has an objection.



---

archive/issue_events_062060.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:11:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24645#event-62060"
}
```



---

archive/issue_comments_467254.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-05-08T17:26:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467254",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_467255.json:
```json
{
    "body": "Changing branch from \"u/embray/build/spkg-postinst\" to \"a1b1c166196a63f1e06bad0939cae60b74fde8df\"",
    "created_at": "2018-05-08T17:26:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24645#issuecomment-467255",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/embray/build/spkg-postinst" to "a1b1c166196a63f1e06bad0939cae60b74fde8df"



---

archive/issue_events_062061.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-08T17:26:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24645",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24645#event-62061"
}
```
