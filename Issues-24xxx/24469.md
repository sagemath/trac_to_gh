# Issue 24469: Make extracting information about a PARI group less painful

archive/issues_024232.json:
```json
{
    "assignees": [],
    "body": "Requested by John Cremona on sage-devel:\n- https://groups.google.com/d/topic/sage-devel/xEXdPgg2kfM/discussion\n\nsee [issue 51 in cypari2](https://github.com/defeo/cypari2/issues/51).\n\nCC:  @JohnCremona @slel\n\nKeywords: **PARI group, Galois group**\n\nUpstream: **Reported upstream. Developers acknowledge bug.**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24469_\n\n",
    "created_at": "2018-01-04T00:43:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wishlist%20item"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make extracting information about a PARI group less painful",
    "type": "issue",
    "updated_at": "2019-05-20T10:46:00Z",
    "url": "https://github.com/sagemath/sage/issues/24469",
    "user": "https://github.com/slel"
}
```
Requested by John Cremona on sage-devel:
- https://groups.google.com/d/topic/sage-devel/xEXdPgg2kfM/discussion

see [issue 51 in cypari2](https://github.com/defeo/cypari2/issues/51).

CC:  @JohnCremona @slel

Keywords: **PARI group, Galois group**

Upstream: **Reported upstream. Developers acknowledge bug.**

_Issue created by migration from https://trac.sagemath.org/ticket/24469_





---

archive/issue_events_320989.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-01-04T00:43:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24469#event-320989"
}
```



---

archive/issue_events_320990.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-01-04T00:43:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24469#event-320990"
}
```



---

archive/issue_events_320991.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-01-04T00:43:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24469#event-320991"
}
```



---

archive/issue_events_320992.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-01-04T00:43:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24469#event-320992"
}
```



---

archive/issue_comments_373175.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nI think this should be addressed by [upstream cypari2](https://github.com/defeo/cypari2). The point is that GP provides shortcut to access some members\n\n```\n$ gp\n? K = bnfinit(x^3 - 2);\n? bnf = bnfinit(x^3 - 2);\n? bnf.nf     /* = bnf[7] */\n%3 = [x^3 - 2, [1, 1], -108, 1, ...]\n? bnf.clgp   /* = bnf[8][1] */\n%4 = [1, [], []]\n```\n(these do not exist in the PARI library). The same shortcuts should be available on the `Gen` type\n\n```\nsage: K = pari.bnfinit('x^3 - 2')\nsage: K.nf\nTraceback (most recent call last):\n...\nAttributeError: 'cypari2.gen.Gen' object has no attribute 'nf'\n```",
    "created_at": "2018-01-04T10:18:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24469#issuecomment-373175",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

I think this should be addressed by [upstream cypari2](https://github.com/defeo/cypari2). The point is that GP provides shortcut to access some members

```
$ gp
? K = bnfinit(x^3 - 2);
? bnf = bnfinit(x^3 - 2);
? bnf.nf     /* = bnf[7] */
%3 = [x^3 - 2, [1, 1], -108, 1, ...]
? bnf.clgp   /* = bnf[8][1] */
%4 = [1, [], []]
```
(these do not exist in the PARI library). The same shortcuts should be available on the `Gen` type

```
sage: K = pari.bnfinit('x^3 - 2')
sage: K.nf
Traceback (most recent call last):
...
AttributeError: 'cypari2.gen.Gen' object has no attribute 'nf'
```



---

archive/issue_comments_373176.json:
```json
{
    "body": "Upstream: **Reported upstream. Developers acknowledge bug.**",
    "created_at": "2018-01-04T16:29:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24469#issuecomment-373176",
    "user": "https://github.com/videlec"
}
```

Upstream: **Reported upstream. Developers acknowledge bug.**



---

archive/issue_comments_373177.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,4 @@\n Requested by John Cremona on sage-devel:\n - https://groups.google.com/d/topic/sage-devel/xEXdPgg2kfM/discussion\n+\n+see [issue 51 in cypari2](https://github.com/defeo/cypari2/issues/51).\n``````\n",
    "created_at": "2018-01-04T16:29:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24469#issuecomment-373177",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,4 @@
 Requested by John Cremona on sage-devel:
 - https://groups.google.com/d/topic/sage-devel/xEXdPgg2kfM/discussion
+
+see [issue 51 in cypari2](https://github.com/defeo/cypari2/issues/51).
``````




---

archive/issue_events_320993.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-04T16:29:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24469#event-320993"
}
```



---

archive/issue_events_320994.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-04T16:29:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24469#event-320994"
}
```



---

archive/issue_comments_373178.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThe bnf example is not quite relevant here, is it?  The components of the Galois group are not currently accessible by such shortcuts (as far as I know).",
    "created_at": "2018-01-07T11:49:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24469#issuecomment-373178",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:4" align="right">Comment 4</div>

The bnf example is not quite relevant here, is it?  The components of the Galois group are not currently accessible by such shortcuts (as far as I know).



---

archive/issue_events_320995.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-05-20T10:46:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24469#event-320995"
}
```



---

archive/issue_events_320996.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-05-20T10:46:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24469#event-320996"
}
```



---

archive/issue_comments_373179.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nChange milestone to sage-wishlist for a few tickets whose previous milestone is closed.",
    "created_at": "2019-05-20T10:46:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24469",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24469#issuecomment-373179",
    "user": "https://github.com/slel"
}
```

<div id="comment:5" align="right">Comment 5</div>

Change milestone to sage-wishlist for a few tickets whose previous milestone is closed.
