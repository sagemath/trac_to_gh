# Issue 24777: Chromatic number of the empty graph by MILP

archive/issues_024540.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: G = Graph()\nsage: G.chromatic_number(algorithm=\"DLX\")\n0\nsage: G.chromatic_number(algorithm=\"MILP\")\n1\nsage: G.chromatic_number(algorithm=\"CP\")\n0\n```\n\n\nCC:  @dcoudert\n\nBranch/Commit: **[76d3db3](https://github.com/sagemath/sagetrac-mirror/commit/76d3db370f587988c9122f33282497eb128c50f8)**\n\nReviewer: **Jori M\u00e4ntysalo**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24777_\n\n",
    "closed_at": "2018-02-21T18:55:24Z",
    "created_at": "2018-02-19T14:53:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Chromatic number of the empty graph by MILP",
    "type": "issue",
    "updated_at": "2018-02-21T18:55:24Z",
    "url": "https://github.com/sagemath/sage/issues/24777",
    "user": "https://github.com/jm58660"
}
```

```
sage: G = Graph()
sage: G.chromatic_number(algorithm="DLX")
0
sage: G.chromatic_number(algorithm="MILP")
1
sage: G.chromatic_number(algorithm="CP")
0
```


CC:  @dcoudert

Branch/Commit: **[76d3db3](https://github.com/sagemath/sagetrac-mirror/commit/76d3db370f587988c9122f33282497eb128c50f8)**

Reviewer: **Jori Mäntysalo**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/24777_





---

archive/issue_events_324958.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-19T14:53:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324958"
}
```



---

archive/issue_events_324959.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-19T14:53:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324959"
}
```



---

archive/issue_events_324960.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-19T14:53:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324960"
}
```



---

archive/issue_events_324961.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-19T14:53:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324961"
}
```



---

archive/issue_comments_379445.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nShould be enough.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/76d3db370f587988c9122f33282497eb128c50f8\">76d3db3</a></td><td><code>trac #24777: special case of empty graph</code></td></tr></table>\n",
    "created_at": "2018-02-19T16:50:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24777#issuecomment-379445",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

Should be enough.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/76d3db370f587988c9122f33282497eb128c50f8">76d3db3</a></td><td><code>trac #24777: special case of empty graph</code></td></tr></table>




---

archive/issue_comments_379446.json:
```json
{
    "body": "Author: **David Coudert**",
    "created_at": "2018-02-19T16:50:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24777#issuecomment-379446",
    "user": "https://github.com/dcoudert"
}
```

Author: **David Coudert**



---

archive/issue_comments_379447.json:
```json
{
    "body": "Branch: **[public/24777_vertex_coloring](https://github.com/sagemath/sagetrac-mirror/tree/public/24777_vertex_coloring)**",
    "created_at": "2018-02-19T16:50:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24777#issuecomment-379447",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/24777_vertex_coloring](https://github.com/sagemath/sagetrac-mirror/tree/public/24777_vertex_coloring)**



---

archive/issue_events_324962.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-02-19T16:50:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324962"
}
```



---

archive/issue_comments_379448.json:
```json
{
    "body": "Commit: **[76d3db3](https://github.com/sagemath/sagetrac-mirror/commit/76d3db370f587988c9122f33282497eb128c50f8)**",
    "created_at": "2018-02-19T16:50:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24777#issuecomment-379448",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[76d3db3](https://github.com/sagemath/sagetrac-mirror/commit/76d3db370f587988c9122f33282497eb128c50f8)**



---

archive/issue_events_324963.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-02-19T16:51:44Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "title_is": "Chromatic number of the empty graph by MILP",
    "title_was": "Chromatic index of the empty graph by MILP",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324963"
}
```



---

archive/issue_events_324964.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-20T08:42:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324964"
}
```



---

archive/issue_events_324965.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-20T08:42:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324965"
}
```



---

archive/issue_comments_379449.json:
```json
{
    "body": "Reviewer: **Jori M\u00e4ntysalo**",
    "created_at": "2018-02-20T08:42:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24777#issuecomment-379449",
    "user": "https://github.com/jm58660"
}
```

Reviewer: **Jori Mäntysalo**



---

archive/issue_comments_379450.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nWorks. Thanks.",
    "created_at": "2018-02-20T08:42:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24777#issuecomment-379450",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:3" align="right">Comment 3</div>

Works. Thanks.



---

archive/issue_events_324966.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-21T18:55:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324966"
}
```



---

archive/issue_events_324967.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d61e7dedd5ec82b52da58ec3aa34b36cf5eb59a8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-21T18:55:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24777#event-324967"
}
```



---

archive/issue_comments_379451.json:
```json
{
    "body": "Changed branch from **[public/24777_vertex_coloring](https://github.com/sagemath/sagetrac-mirror/tree/public/24777_vertex_coloring)** to **[76d3db3](https://github.com/sagemath/sagetrac-mirror/commit/76d3db370f587988c9122f33282497eb128c50f8)**",
    "created_at": "2018-02-21T18:55:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24777",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24777#issuecomment-379451",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/24777_vertex_coloring](https://github.com/sagemath/sagetrac-mirror/tree/public/24777_vertex_coloring)** to **[76d3db3](https://github.com/sagemath/sagetrac-mirror/commit/76d3db370f587988c9122f33282497eb128c50f8)**
