# Issue 24621: Fix conversions to CBF

archive/issues_024384.json:
```json
{
    "assignees": [],
    "body": "#24285 breaks some conversions to CBF, e.g.\n\n```\nsage: CBF(NumberField(polygen(QQ, 'y')^3 + 20, 'a', embedding=CC(1.35,2.35)).gen())\n...\nTypeError: unable to convert a to a ComplexBall\n```\n\nBranch/Commit: **[8474ea3](https://github.com/sagemath/sagetrac-mirror/commit/8474ea3c302e87aceb0b6362ce9e183e3ebf6b1a)**\n\nReviewer: **Vinent Delecroix**\n\nAuthor: **Marc Mezzarobba**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24621_\n\n",
    "closed_at": "2018-02-09T23:47:28Z",
    "created_at": "2018-01-31T09:34:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix conversions to CBF",
    "type": "issue",
    "updated_at": "2018-02-09T23:47:28Z",
    "url": "https://github.com/sagemath/sage/issues/24621",
    "user": "https://github.com/mezzarobba"
}
```
#24285 breaks some conversions to CBF, e.g.

```
sage: CBF(NumberField(polygen(QQ, 'y')^3 + 20, 'a', embedding=CC(1.35,2.35)).gen())
...
TypeError: unable to convert a to a ComplexBall
```

Branch/Commit: **[8474ea3](https://github.com/sagemath/sagetrac-mirror/commit/8474ea3c302e87aceb0b6362ce9e183e3ebf6b1a)**

Reviewer: **Vinent Delecroix**

Author: **Marc Mezzarobba**

_Issue created by migration from https://trac.sagemath.org/ticket/24621_





---

archive/issue_events_322879.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-01-31T09:34:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24621#event-322879"
}
```



---

archive/issue_events_322880.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-01-31T09:34:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24621#event-322880"
}
```



---

archive/issue_events_322881.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-01-31T09:34:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24621#event-322881"
}
```



---

archive/issue_events_322882.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-01-31T09:34:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24621#event-322882"
}
```



---

archive/issue_comments_375986.json:
```json
{
    "body": "Branch: **[u/mmezzarobba/ticket/24621](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/ticket/24621)**",
    "created_at": "2018-01-31T09:43:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24621#issuecomment-375986",
    "user": "https://github.com/mezzarobba"
}
```

Branch: **[u/mmezzarobba/ticket/24621](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/ticket/24621)**



---

archive/issue_comments_375987.json:
```json
{
    "body": "Commit: **[8474ea3](https://github.com/sagemath/sagetrac-mirror/commit/8474ea3c302e87aceb0b6362ce9e183e3ebf6b1a)**",
    "created_at": "2018-01-31T09:43:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24621#issuecomment-375987",
    "user": "https://github.com/mezzarobba"
}
```

Commit: **[8474ea3](https://github.com/sagemath/sagetrac-mirror/commit/8474ea3c302e87aceb0b6362ce9e183e3ebf6b1a)**



---

archive/issue_events_322883.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-01-31T09:43:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24621#event-322883"
}
```



---

archive/issue_comments_375988.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8474ea3c302e87aceb0b6362ce9e183e3ebf6b1a\">8474ea3</a></td><td><code>Fix up 75c75e9 (#24285)</code></td></tr></table>\n",
    "created_at": "2018-01-31T09:43:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24621#issuecomment-375988",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8474ea3c302e87aceb0b6362ce9e183e3ebf6b1a">8474ea3</a></td><td><code>Fix up 75c75e9 (#24285)</code></td></tr></table>




---

archive/issue_events_322884.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-02-06T08:19:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24621#event-322884"
}
```



---

archive/issue_events_322885.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-02-06T08:19:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24621#event-322885"
}
```



---

archive/issue_comments_375989.json:
```json
{
    "body": "Reviewer: **Vinent Delecroix**",
    "created_at": "2018-02-06T08:19:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24621#issuecomment-375989",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vinent Delecroix**



---

archive/issue_events_322886.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-09T23:47:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24621#event-322886"
}
```



---

archive/issue_events_322887.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a6df142fd9aaf0e0a0ecda2b92afe4a78970d89b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-09T23:47:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24621#event-322887"
}
```



---

archive/issue_comments_375990.json:
```json
{
    "body": "Changed branch from **[u/mmezzarobba/ticket/24621](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/ticket/24621)** to **[8474ea3](https://github.com/sagemath/sagetrac-mirror/commit/8474ea3c302e87aceb0b6362ce9e183e3ebf6b1a)**",
    "created_at": "2018-02-09T23:47:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24621#issuecomment-375990",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmezzarobba/ticket/24621](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/ticket/24621)** to **[8474ea3](https://github.com/sagemath/sagetrac-mirror/commit/8474ea3c302e87aceb0b6362ce9e183e3ebf6b1a)**
