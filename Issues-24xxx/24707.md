# Issue 24707: fpylll ignores --stdlib directive

archive/issues_024470.json:
```json
{
    "body": "If \"`--stdlib=libc++`\" is given in the CXX environment var, fpylll will add it only with compilations not when linking. This leads to libstdc++ being linked and import errors with fpylll.\n\nThis is the result from `sage -f fpylll` with `CXX=clang --stdlib=libc++`:\n\n```\nralf@ark:~/sage> ldd /home/ralf/sage/local/lib/python2.7/site-packages/fpylll/fplll/enumeration.so\n\tlinux-vdso.so.1 (0x00007ffd248da000)\n\tlibgmp.so.23 => /home/ralf/sage/local/lib/libgmp.so.23 (0x00007fe2f3bee000)\n\tlibmpfr.so.6 => /home/ralf/sage/local/lib/libmpfr.so.6 (0x00007fe2f3977000)\n\tlibfplll.so.4 => /home/ralf/sage/local/lib/libfplll.so.4 (0x00007fe2f3138000)\n\tlibpython2.7.so.1.0 => /home/ralf/sage/local/lib/libpython2.7.so.1.0 (0x00007fe2f2d4e000)\n\tlibpari-gmp-2.10.so.0 => /home/ralf/sage/local/lib/libpari-gmp-2.10.so.0 (0x00007fe2f226f000)\n\tlibstdc++.so.6 => /usr/lib64/libstdc++.so.6 (0x00007fe2f1ee6000)\n\tlibm.so.6 => /lib64/libm.so.6 (0x00007fe2f1be9000)\n\tlibgcc_s.so.1 => /lib64/libgcc_s.so.1 (0x00007fe2f19d2000)\n\tlibpthread.so.0 => /lib64/libpthread.so.0 (0x00007fe2f17b5000)\n\tlibc.so.6 => /lib64/libc.so.6 (0x00007fe2f1414000)\n\tlibc++.so.1 => /usr/lib64/libc++.so.1 (0x00007fe2f41d2000)\n\tlibc++abi.so.1 => /usr/lib64/libc++abi.so.1 (0x00007fe2f418f000)\n\tlibdl.so.2 => /lib64/libdl.so.2 (0x00007fe2f1210000)\n\tlibutil.so.1 => /lib64/libutil.so.1 (0x00007fe2f100d000)\n\t/lib64/ld-linux-x86-64.so.2 (0x00007fe2f40a3000)\n```\nIn the attached build log:\n\n```\n    clang++ -pthread -shared -L/home/ralf/sage/local/lib -Wl,-rpath,/home/ralf/s\nage/local/lib -L/home/ralf/sage/local/lib/python/lib -L/home/ralf/sage/local/lib\n -L/home/ralf/sage/local/lib/python/lib -L/home/ralf/sage/local/lib -L/home/ralf\n/sage/local/lib -Wl,-rpath,/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib\n/python/lib -L/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib/python/lib -\nL/home/ralf/sage/local/lib build/temp.linux-x86_64-2.7/build/src/fpylll/fplll/en\numeration.o -L/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib -lgmp -lmpfr\n -lfplll -lpython2.7 -o build/lib.linux-x86_64-2.7/fpylll/fplll/enumeration.so -\nlpari -std=c++11\n```\n\nCC:  @malb\n\nUpstream: Reported upstream. No feedback yet.\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/24707\n\n",
    "closed_at": "2018-02-12T09:27:23Z",
    "created_at": "2018-02-11T08:18:47Z",
    "labels": [
        "component: packages: standard",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "fpylll ignores --stdlib directive",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24707",
    "user": "https://github.com/rwst"
}
```
If "`--stdlib=libc++`" is given in the CXX environment var, fpylll will add it only with compilations not when linking. This leads to libstdc++ being linked and import errors with fpylll.

This is the result from `sage -f fpylll` with `CXX=clang --stdlib=libc++`:

```
ralf@ark:~/sage> ldd /home/ralf/sage/local/lib/python2.7/site-packages/fpylll/fplll/enumeration.so
	linux-vdso.so.1 (0x00007ffd248da000)
	libgmp.so.23 => /home/ralf/sage/local/lib/libgmp.so.23 (0x00007fe2f3bee000)
	libmpfr.so.6 => /home/ralf/sage/local/lib/libmpfr.so.6 (0x00007fe2f3977000)
	libfplll.so.4 => /home/ralf/sage/local/lib/libfplll.so.4 (0x00007fe2f3138000)
	libpython2.7.so.1.0 => /home/ralf/sage/local/lib/libpython2.7.so.1.0 (0x00007fe2f2d4e000)
	libpari-gmp-2.10.so.0 => /home/ralf/sage/local/lib/libpari-gmp-2.10.so.0 (0x00007fe2f226f000)
	libstdc++.so.6 => /usr/lib64/libstdc++.so.6 (0x00007fe2f1ee6000)
	libm.so.6 => /lib64/libm.so.6 (0x00007fe2f1be9000)
	libgcc_s.so.1 => /lib64/libgcc_s.so.1 (0x00007fe2f19d2000)
	libpthread.so.0 => /lib64/libpthread.so.0 (0x00007fe2f17b5000)
	libc.so.6 => /lib64/libc.so.6 (0x00007fe2f1414000)
	libc++.so.1 => /usr/lib64/libc++.so.1 (0x00007fe2f41d2000)
	libc++abi.so.1 => /usr/lib64/libc++abi.so.1 (0x00007fe2f418f000)
	libdl.so.2 => /lib64/libdl.so.2 (0x00007fe2f1210000)
	libutil.so.1 => /lib64/libutil.so.1 (0x00007fe2f100d000)
	/lib64/ld-linux-x86-64.so.2 (0x00007fe2f40a3000)
```
In the attached build log:

```
    clang++ -pthread -shared -L/home/ralf/sage/local/lib -Wl,-rpath,/home/ralf/s
age/local/lib -L/home/ralf/sage/local/lib/python/lib -L/home/ralf/sage/local/lib
 -L/home/ralf/sage/local/lib/python/lib -L/home/ralf/sage/local/lib -L/home/ralf
/sage/local/lib -Wl,-rpath,/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib
/python/lib -L/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib/python/lib -
L/home/ralf/sage/local/lib build/temp.linux-x86_64-2.7/build/src/fpylll/fplll/en
umeration.o -L/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib -lgmp -lmpfr
 -lfplll -lpython2.7 -o build/lib.linux-x86_64-2.7/fpylll/fplll/enumeration.so -
lpari -std=c++11
```

CC:  @malb

Upstream: Reported upstream. No feedback yet.

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/24707





---

archive/issue_comments_469298.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-If \"`--stdlib=libc++`\" is given in the CXX environment var, fpylll will add it only with C compilations not C++. This leads to libstdc++ being linked and import errors with fpylll.\n+If \"`--stdlib=libc++`\" is given in the CXX environment var, fpylll will add it only with compilations not when linking. This leads to libstdc++ being linked and import errors with fpylll.\n``````\n",
    "created_at": "2018-02-11T08:27:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469298",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-If "`--stdlib=libc++`" is given in the CXX environment var, fpylll will add it only with C compilations not C++. This leads to libstdc++ being linked and import errors with fpylll.
+If "`--stdlib=libc++`" is given in the CXX environment var, fpylll will add it only with compilations not when linking. This leads to libstdc++ being linked and import errors with fpylll.
``````




---

archive/issue_comments_469299.json:
```json
{
    "body": "<a id='comment:2'></a>Can you provide a build log please.",
    "created_at": "2018-02-11T08:38:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469299",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:2'></a>Can you provide a build log please.



---

archive/issue_comments_469300.json:
```json
{
    "body": "<a id='comment:3'></a>Is it fplll-5.2.0.p1.log? In other words, is fpylll a typo, or a different package?\n\nAnyway, I posted a fplll log resulting from building Sage with clange [here](http://users.minet.uni-jena.de/cohomology/logs/fplll-5.2.0.p1.log).",
    "created_at": "2018-02-11T08:45:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469300",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:3'></a>Is it fplll-5.2.0.p1.log? In other words, is fpylll a typo, or a different package?

Anyway, I posted a fplll log resulting from building Sage with clange [here](http://users.minet.uni-jena.de/cohomology/logs/fplll-5.2.0.p1.log).



---

archive/issue_comments_469301.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,36 @@\n If \"`--stdlib=libc++`\" is given in the CXX environment var, fpylll will add it only with compilations not when linking. This leads to libstdc++ being linked and import errors with fpylll.\n+\n+This is the result from `sage -f fpylll` with `CXX=clang --stdlib=libc++`:\n+\n+```\n+ralf@ark:~/sage> ldd /home/ralf/sage/local/lib/python2.7/site-packages/fpylll/fplll/enumeration.so\n+\tlinux-vdso.so.1 (0x00007ffd248da000)\n+\tlibgmp.so.23 => /home/ralf/sage/local/lib/libgmp.so.23 (0x00007fe2f3bee000)\n+\tlibmpfr.so.6 => /home/ralf/sage/local/lib/libmpfr.so.6 (0x00007fe2f3977000)\n+\tlibfplll.so.4 => /home/ralf/sage/local/lib/libfplll.so.4 (0x00007fe2f3138000)\n+\tlibpython2.7.so.1.0 => /home/ralf/sage/local/lib/libpython2.7.so.1.0 (0x00007fe2f2d4e000)\n+\tlibpari-gmp-2.10.so.0 => /home/ralf/sage/local/lib/libpari-gmp-2.10.so.0 (0x00007fe2f226f000)\n+\tlibstdc++.so.6 => /usr/lib64/libstdc++.so.6 (0x00007fe2f1ee6000)\n+\tlibm.so.6 => /lib64/libm.so.6 (0x00007fe2f1be9000)\n+\tlibgcc_s.so.1 => /lib64/libgcc_s.so.1 (0x00007fe2f19d2000)\n+\tlibpthread.so.0 => /lib64/libpthread.so.0 (0x00007fe2f17b5000)\n+\tlibc.so.6 => /lib64/libc.so.6 (0x00007fe2f1414000)\n+\tlibc++.so.1 => /usr/lib64/libc++.so.1 (0x00007fe2f41d2000)\n+\tlibc++abi.so.1 => /usr/lib64/libc++abi.so.1 (0x00007fe2f418f000)\n+\tlibdl.so.2 => /lib64/libdl.so.2 (0x00007fe2f1210000)\n+\tlibutil.so.1 => /lib64/libutil.so.1 (0x00007fe2f100d000)\n+\t/lib64/ld-linux-x86-64.so.2 (0x00007fe2f40a3000)\n+```\n+In the attached build log:\n+\n+```\n+    clang++ -pthread -shared -L/home/ralf/sage/local/lib -Wl,-rpath,/home/ralf/s\n+age/local/lib -L/home/ralf/sage/local/lib/python/lib -L/home/ralf/sage/local/lib\n+ -L/home/ralf/sage/local/lib/python/lib -L/home/ralf/sage/local/lib -L/home/ralf\n+/sage/local/lib -Wl,-rpath,/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib\n+/python/lib -L/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib/python/lib -\n+L/home/ralf/sage/local/lib build/temp.linux-x86_64-2.7/build/src/fpylll/fplll/en\n+umeration.o -L/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib -lgmp -lmpfr\n+ -lfplll -lpython2.7 -o build/lib.linux-x86_64-2.7/fpylll/fplll/enumeration.so -\n+lpari -std=c++11\n+```\n``````\n",
    "created_at": "2018-02-11T09:10:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469301",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,36 @@
 If "`--stdlib=libc++`" is given in the CXX environment var, fpylll will add it only with compilations not when linking. This leads to libstdc++ being linked and import errors with fpylll.
+
+This is the result from `sage -f fpylll` with `CXX=clang --stdlib=libc++`:
+
+```
+ralf@ark:~/sage> ldd /home/ralf/sage/local/lib/python2.7/site-packages/fpylll/fplll/enumeration.so
+	linux-vdso.so.1 (0x00007ffd248da000)
+	libgmp.so.23 => /home/ralf/sage/local/lib/libgmp.so.23 (0x00007fe2f3bee000)
+	libmpfr.so.6 => /home/ralf/sage/local/lib/libmpfr.so.6 (0x00007fe2f3977000)
+	libfplll.so.4 => /home/ralf/sage/local/lib/libfplll.so.4 (0x00007fe2f3138000)
+	libpython2.7.so.1.0 => /home/ralf/sage/local/lib/libpython2.7.so.1.0 (0x00007fe2f2d4e000)
+	libpari-gmp-2.10.so.0 => /home/ralf/sage/local/lib/libpari-gmp-2.10.so.0 (0x00007fe2f226f000)
+	libstdc++.so.6 => /usr/lib64/libstdc++.so.6 (0x00007fe2f1ee6000)
+	libm.so.6 => /lib64/libm.so.6 (0x00007fe2f1be9000)
+	libgcc_s.so.1 => /lib64/libgcc_s.so.1 (0x00007fe2f19d2000)
+	libpthread.so.0 => /lib64/libpthread.so.0 (0x00007fe2f17b5000)
+	libc.so.6 => /lib64/libc.so.6 (0x00007fe2f1414000)
+	libc++.so.1 => /usr/lib64/libc++.so.1 (0x00007fe2f41d2000)
+	libc++abi.so.1 => /usr/lib64/libc++abi.so.1 (0x00007fe2f418f000)
+	libdl.so.2 => /lib64/libdl.so.2 (0x00007fe2f1210000)
+	libutil.so.1 => /lib64/libutil.so.1 (0x00007fe2f100d000)
+	/lib64/ld-linux-x86-64.so.2 (0x00007fe2f40a3000)
+```
+In the attached build log:
+
+```
+    clang++ -pthread -shared -L/home/ralf/sage/local/lib -Wl,-rpath,/home/ralf/s
+age/local/lib -L/home/ralf/sage/local/lib/python/lib -L/home/ralf/sage/local/lib
+ -L/home/ralf/sage/local/lib/python/lib -L/home/ralf/sage/local/lib -L/home/ralf
+/sage/local/lib -Wl,-rpath,/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib
+/python/lib -L/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib/python/lib -
+L/home/ralf/sage/local/lib build/temp.linux-x86_64-2.7/build/src/fpylll/fplll/en
+umeration.o -L/home/ralf/sage/local/lib -L/home/ralf/sage/local/lib -lgmp -lmpfr
+ -lfplll -lpython2.7 -o build/lib.linux-x86_64-2.7/fpylll/fplll/enumeration.so -
+lpari -std=c++11
+```
``````




---

archive/issue_comments_469302.json:
```json
{
    "body": "<a id='comment:5'></a>Attachment [fpylll-0.3.0dev.p0.log](tarball://root/attachments/some-uuid/ticket24707/fpylll-0.3.0dev.p0.log) by @dimpase created at 2018-02-11 09:46:48",
    "created_at": "2018-02-11T09:46:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469302",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:5'></a>Attachment [fpylll-0.3.0dev.p0.log](tarball://root/attachments/some-uuid/ticket24707/fpylll-0.3.0dev.p0.log) by @dimpase created at 2018-02-11 09:46:48



---

archive/issue_comments_469303.json:
```json
{
    "body": "<a id='comment:6'></a>reported upstream: https://github.com/fplll/fpylll/issues/115",
    "created_at": "2018-02-11T10:03:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469303",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:6'></a>reported upstream: https://github.com/fplll/fpylll/issues/115



---

archive/issue_comments_469304.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"Reported upstream. No feedback yet.\"",
    "created_at": "2018-02-11T10:03:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469304",
    "user": "https://github.com/dimpase"
}
```

Changing upstream from "N/A" to "Reported upstream. No feedback yet."



---

archive/issue_comments_469305.json:
```json
{
    "body": "<a id='comment:7'></a>While fpylll seems to build (on my machine, with `CC=clang CXX=\"clang++ -I/usr/include/libcxxabi/ --stdlib=libc++\" make`), I got a build error in ratpoints and opened #24708 for it.",
    "created_at": "2018-02-11T10:37:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469305",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:7'></a>While fpylll seems to build (on my machine, with `CC=clang CXX="clang++ -I/usr/include/libcxxabi/ --stdlib=libc++" make`), I got a build error in ratpoints and opened #24708 for it.



---

archive/issue_comments_469306.json:
```json
{
    "body": "<a id='comment:8'></a>Shouldn't those parameters be passed as `CXXFLAGS`/`LDFLAGS`?",
    "created_at": "2018-02-11T12:02:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469306",
    "user": "https://github.com/malb"
}
```

<a id='comment:8'></a>Shouldn't those parameters be passed as `CXXFLAGS`/`LDFLAGS`?



---

archive/issue_comments_469307.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [malb](#comment%3A8):\n> Shouldn't those parameters be passed as `CXXFLAGS`/`LDFLAGS`?\n\n\nI think so, yes. At least that is their usage, as I see it now---`stdlib=libc++` as compile flag to get the right include files, and `-lc++ --stdlib=libc++` to get the file to link and prevent libstdc++ from being linked.",
    "created_at": "2018-02-11T13:48:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469307",
    "user": "https://github.com/rwst"
}
```

<a id='comment:9'></a>Replying to [malb](#comment%3A8):
> Shouldn't those parameters be passed as `CXXFLAGS`/`LDFLAGS`?


I think so, yes. At least that is their usage, as I see it now---`stdlib=libc++` as compile flag to get the right include files, and `-lc++ --stdlib=libc++` to get the file to link and prevent libstdc++ from being linked.



---

archive/issue_comments_469308.json:
```json
{
    "body": "<a id='comment:10'></a>However CXX may need to be set to `clang++ --stdlib=libc++` as well. I see a lot of compile commands in the logs ignoring CXXFLAGS (ppl, gfan...)",
    "created_at": "2018-02-11T14:04:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469308",
    "user": "https://github.com/rwst"
}
```

<a id='comment:10'></a>However CXX may need to be set to `clang++ --stdlib=libc++` as well. I see a lot of compile commands in the logs ignoring CXXFLAGS (ppl, gfan...)



---

archive/issue_comments_469309.json:
```json
{
    "body": "<a id='comment:11'></a>Thanks, does fpylll also fail to pass through `CXXFLAGS` and `LDFLAGS`?",
    "created_at": "2018-02-11T15:14:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469309",
    "user": "https://github.com/malb"
}
```

<a id='comment:11'></a>Thanks, does fpylll also fail to pass through `CXXFLAGS` and `LDFLAGS`?



---

archive/issue_comments_469310.json:
```json
{
    "body": "<a id='comment:12'></a>See also #24710. We need `--stdlib=libc++` in the CXXFLAGS but this makes gfortran fail.",
    "created_at": "2018-02-11T15:18:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469310",
    "user": "https://github.com/rwst"
}
```

<a id='comment:12'></a>See also #24710. We need `--stdlib=libc++` in the CXXFLAGS but this makes gfortran fail.



---

archive/issue_comments_469311.json:
```json
{
    "body": "<a id='comment:13'></a>Correction, we need it in LDFLAGS. I'm a bit frustrated.",
    "created_at": "2018-02-11T15:20:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469311",
    "user": "https://github.com/rwst"
}
```

<a id='comment:13'></a>Correction, we need it in LDFLAGS. I'm a bit frustrated.



---

archive/issue_events_062181.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-02-11T16:47:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24707#event-62181"
}
```



---

archive/issue_comments_469312.json:
```json
{
    "body": "<a id='comment:14'></a>Replying to [dimpase](#comment%3A6):\n> reported upstream: https://github.com/fplll/fpylll/issues/115\n\n\nThanks. I explained there that there is now a solution, i.e. to put the string in LDFLAGS.",
    "created_at": "2018-02-11T16:47:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469312",
    "user": "https://github.com/rwst"
}
```

<a id='comment:14'></a>Replying to [dimpase](#comment%3A6):
> reported upstream: https://github.com/fplll/fpylll/issues/115


Thanks. I explained there that there is now a solution, i.e. to put the string in LDFLAGS.



---

archive/issue_comments_469313.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-02-11T16:47:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469313",
    "user": "https://github.com/rwst"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_469314.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-02-11T16:48:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469314",
    "user": "https://github.com/rwst"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_469315.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2018-02-12T09:27:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24707#issuecomment-469315",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: invalid



---

archive/issue_events_062182.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-02-12T09:27:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24707",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24707#event-62182"
}
```
