# Issue 24559: py3: better backwards-compatibility for Python longs

archive/issues_024322.json:
```json
{
    "assignees": [],
    "body": "This does three little things to better support `long`-related functionality, especially in the doctests.  This allows a lot of tests to pass on Python 3 that wouldn't otherwise, or would need to be skipped and/or have Python 3 specific copies:\n\n* The Sage preparser supports Python 2 style long literals, like `42L`.  These are just treated as normal ints on Python 3 (we could also consider adding a deprecation warning for this usage).  I actually wish Python itself had done this (just as they restored backwards-compat for `u''` literals--that they didn't I think is just that explicit `long`s is relatively uncommon.\n\n* The *doctest* parser has a fixup for recognizing long literals in the expected output of a test, and treating them as normal ints on Python 3.  This is probably the weakest part of the patch in terms of implementation--it's very na\u00efve and I can imagine the possibility of false positives, though it is still pretty unlikely.  Open to better ideas on this.\n\n* In `sage.all` I added, on Python 3 only, a new global called `long` which is just a function that wraps `int()` and (except in the tests, for now) raises a `DeprecationWarning` for using `long()`.  This still allows a lot of tests to work.  I deliberately made it a simple function, and not a class, as the latter would tempt too many errors with code like `isinstance(x, long)`.\n\nBranch/Commit: **[e3b8148](https://github.com/sagemath/sagetrac-mirror/commit/e3b8148c764377ac86fa532432676ca357e3f56b)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24559_\n\n",
    "closed_at": "2018-05-08T17:26:44Z",
    "created_at": "2018-01-18T10:19:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: better backwards-compatibility for Python longs",
    "type": "issue",
    "updated_at": "2018-05-08T17:26:44Z",
    "url": "https://github.com/sagemath/sage/issues/24559",
    "user": "https://github.com/embray"
}
```
This does three little things to better support `long`-related functionality, especially in the doctests.  This allows a lot of tests to pass on Python 3 that wouldn't otherwise, or would need to be skipped and/or have Python 3 specific copies:

* The Sage preparser supports Python 2 style long literals, like `42L`.  These are just treated as normal ints on Python 3 (we could also consider adding a deprecation warning for this usage).  I actually wish Python itself had done this (just as they restored backwards-compat for `u''` literals--that they didn't I think is just that explicit `long`s is relatively uncommon.

* The *doctest* parser has a fixup for recognizing long literals in the expected output of a test, and treating them as normal ints on Python 3.  This is probably the weakest part of the patch in terms of implementation--it's very naïve and I can imagine the possibility of false positives, though it is still pretty unlikely.  Open to better ideas on this.

* In `sage.all` I added, on Python 3 only, a new global called `long` which is just a function that wraps `int()` and (except in the tests, for now) raises a `DeprecationWarning` for using `long()`.  This still allows a lot of tests to work.  I deliberately made it a simple function, and not a class, as the latter would tempt too many errors with code like `isinstance(x, long)`.

Branch/Commit: **[e3b8148](https://github.com/sagemath/sagetrac-mirror/commit/e3b8148c764377ac86fa532432676ca357e3f56b)**

Reviewer: **Jeroen Demeyer**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/24559_





---

archive/issue_events_322096.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-18T10:19:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322096"
}
```



---

archive/issue_events_322097.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-18T10:19:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322097"
}
```



---

archive/issue_events_322098.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-18T10:19:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322098"
}
```



---

archive/issue_events_322099.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-18T10:19:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322099"
}
```



---

archive/issue_comments_374620.json:
```json
{
    "body": "Dependencies: **#24258**",
    "created_at": "2018-01-18T10:20:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374620",
    "user": "https://github.com/embray"
}
```

Dependencies: **#24258**



---

archive/issue_events_322100.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-18T10:20:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322100"
}
```



---

archive/issue_comments_374621.json:
```json
{
    "body": "Changed commit from **[55f7a64](https://github.com/sagemath/sagetrac-mirror/commit/55f7a642ae60f371eed22c4aa4550cc8a30ecb78)** to **[e3c14f3](https://github.com/sagemath/sagetrac-mirror/commit/e3c14f32ac26b184520ee530b3e4de754971a004)**",
    "created_at": "2018-01-19T15:21:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374621",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[55f7a64](https://github.com/sagemath/sagetrac-mirror/commit/55f7a642ae60f371eed22c4aa4550cc8a30ecb78)** to **[e3c14f3](https://github.com/sagemath/sagetrac-mirror/commit/e3c14f32ac26b184520ee530b3e4de754971a004)**



---

archive/issue_comments_374622.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e3c14f32ac26b184520ee530b3e4de754971a004\">e3c14f3</a></td><td><code>Properly handle cases like '1rL'</code></td></tr></table>\n",
    "created_at": "2018-01-19T15:21:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374622",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e3c14f32ac26b184520ee530b3e4de754971a004">e3c14f3</a></td><td><code>Properly handle cases like '1rL'</code></td></tr></table>




---

archive/issue_comments_374623.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI don't really with the condition deprecation depending on `DOCTEST_MODE`. Doctests should be as close as possible to a normal interactive session. I'm fine with the `long = int` alias, but then I would remove the deprecation.",
    "created_at": "2018-01-23T09:52:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374623",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

I don't really with the condition deprecation depending on `DOCTEST_MODE`. Doctests should be as close as possible to a normal interactive session. I'm fine with the `long = int` alias, but then I would remove the deprecation.



---

archive/issue_comments_374624.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReplying to [@jdemeyer](#comment%3A3):\n> I don't really with the condition deprecation depending on `DOCTEST_MODE`. Doctests should be as close as possible to a normal interactive session. I'm fine with the `long = int` alias, but then I would remove the deprecation.\n\nThis was added very deliberately in part to help with porting the doctests, so as to allow the great many doctests that use `long()` to continue working for now without interruption (while at the same time giving a useful warning for user code).\n\nMy intention, though not explicitly stated, was to later (once we want to switch Sage to Python 3) remove the condition so that it's easy to find and fix/remove such tests later.",
    "created_at": "2018-01-23T11:15:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374624",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

Replying to [@jdemeyer](#comment%3A3):
> I don't really with the condition deprecation depending on `DOCTEST_MODE`. Doctests should be as close as possible to a normal interactive session. I'm fine with the `long = int` alias, but then I would remove the deprecation.

This was added very deliberately in part to help with porting the doctests, so as to allow the great many doctests that use `long()` to continue working for now without interruption (while at the same time giving a useful warning for user code).

My intention, though not explicitly stated, was to later (once we want to switch Sage to Python 3) remove the condition so that it's easy to find and fix/remove such tests later.



---

archive/issue_comments_374625.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nReplying to [@embray](#comment%3A4):\n> My intention, though not explicitly stated, was to later (once we want to switch Sage to Python 3) remove the condition so that it's easy to find and fix/remove such tests later.\n\nI think it's better to just enable the alias now without any deprecation and then later add an unconditional deprecation. Since Sage doesn't really support Python 3 for the moment, the deprecation won't have much effect anyway.",
    "created_at": "2018-01-24T16:14:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374625",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">Comment 5</div>

Replying to [@embray](#comment%3A4):
> My intention, though not explicitly stated, was to later (once we want to switch Sage to Python 3) remove the condition so that it's easy to find and fix/remove such tests later.

I think it's better to just enable the alias now without any deprecation and then later add an unconditional deprecation. Since Sage doesn't really support Python 3 for the moment, the deprecation won't have much effect anyway.



---

archive/issue_comments_374626.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nWhy do you think that's better? Why is this wrong?",
    "created_at": "2018-01-30T17:38:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374626",
    "user": "https://github.com/embray"
}
```

<div id="comment:6" align="right">Comment 6</div>

Why do you think that's better? Why is this wrong?



---

archive/issue_comments_374627.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThere's a difference between \"supporting Python 3\" and \"having Python 3 as the default Sage\".  I think in some version--hopefully before long (I'm running pretty low on obvious errors to fix, and most of the problems I'm seeing are minor issues in doctests) there will be a Sage that supports Python 3.\n\nFor backwards compatibility we'll want to keep Python 2 as the default for some time, but I would definitely encourage users to move to Python 3 sooner rather than later.  To that end, I envision that soon Sage will have Python 2 and Python 3 binary releases, with some emphasis on getting people to use the latter.  To that end it will be good to help as much as possible with transitioning.\n\nThat said, there's also a question of when to switch the default in the docs over.  If the docs are Python 3 then I think there should be no `long()` without a deprecation warning.\n\nIn other words, I'd be willing to change it, but it depends a lot on bigger plans regarding how Sage will transition to Python 3 that aren't particularly settled. But I feel right now that this currently with an eye toward being as helpful as possible for transitioning.",
    "created_at": "2018-01-30T17:47:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374627",
    "user": "https://github.com/embray"
}
```

<div id="comment:7" align="right">Comment 7</div>

There's a difference between "supporting Python 3" and "having Python 3 as the default Sage".  I think in some version--hopefully before long (I'm running pretty low on obvious errors to fix, and most of the problems I'm seeing are minor issues in doctests) there will be a Sage that supports Python 3.

For backwards compatibility we'll want to keep Python 2 as the default for some time, but I would definitely encourage users to move to Python 3 sooner rather than later.  To that end, I envision that soon Sage will have Python 2 and Python 3 binary releases, with some emphasis on getting people to use the latter.  To that end it will be good to help as much as possible with transitioning.

That said, there's also a question of when to switch the default in the docs over.  If the docs are Python 3 then I think there should be no `long()` without a deprecation warning.

In other words, I'd be willing to change it, but it depends a lot on bigger plans regarding how Sage will transition to Python 3 that aren't particularly settled. But I feel right now that this currently with an eye toward being as helpful as possible for transitioning.



---

archive/issue_comments_374628.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nReplying to [@embray](#comment%3A7):\n> If the docs are Python 3 then I think there should be no `long()` without a deprecation warning.\n\nThen it should be the same in doctests too. I'm fine with a deprecation or no deprecation either way, but I am protesting the condition on `DOCTEST_MODE`.",
    "created_at": "2018-01-31T10:27:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374628",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:8" align="right">Comment 8</div>

Replying to [@embray](#comment%3A7):
> If the docs are Python 3 then I think there should be no `long()` without a deprecation warning.

Then it should be the same in doctests too. I'm fine with a deprecation or no deprecation either way, but I am protesting the condition on `DOCTEST_MODE`.



---

archive/issue_comments_374629.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nI don't know *why*. It seems to me the whole point of having `DOCTEST_MODE` flag is to control behavior in the context of doctests.  Obviously this should be used sparingly, but one can always find special cases.\n\nThat said, I took a step back from this and rethought it a bit.  I thought of adding a `long()` in `sage.all` as sort of a helper for transition to Python 3.  That said, the lack of `long()` in Python 3 is *hardly* the biggest difference, and users aren't often in the habit of using the `long` built-in very much anyways (especially not in Sage where it's almost entirely unnecessary).\n\nSo maybe adding this helper isn't that useful anyways.\n\nSo what if we only injected it into the globals when running the doctests, just temporarily, no deprecation warning or anything.  Later, when we want to start moving the tests to be more Python 3-compatible, we just remove it and get NameErrors everywhere the tests try to use it.",
    "created_at": "2018-01-31T14:27:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374629",
    "user": "https://github.com/embray"
}
```

<div id="comment:9" align="right">Comment 9</div>

I don't know *why*. It seems to me the whole point of having `DOCTEST_MODE` flag is to control behavior in the context of doctests.  Obviously this should be used sparingly, but one can always find special cases.

That said, I took a step back from this and rethought it a bit.  I thought of adding a `long()` in `sage.all` as sort of a helper for transition to Python 3.  That said, the lack of `long()` in Python 3 is *hardly* the biggest difference, and users aren't often in the habit of using the `long` built-in very much anyways (especially not in Sage where it's almost entirely unnecessary).

So maybe adding this helper isn't that useful anyways.

So what if we only injected it into the globals when running the doctests, just temporarily, no deprecation warning or anything.  Later, when we want to start moving the tests to be more Python 3-compatible, we just remove it and get NameErrors everywhere the tests try to use it.



---

archive/issue_comments_374630.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nOK, I'll stop fighting here. I don't really agree, but if you want to add `long` on Python 3 if `DOCTEST_MODE` without deprecation, please go ahead.",
    "created_at": "2018-02-01T19:49:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374630",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:10" align="right">Comment 10</div>

OK, I'll stop fighting here. I don't really agree, but if you want to add `long` on Python 3 if `DOCTEST_MODE` without deprecation, please go ahead.



---

archive/issue_comments_374631.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThanks--though what do you think about my other proposal of just injecting `long` for the purpose of the doctests *only*?  I'm personally leaning toward it as a preference.\n\nI understand your feeling that the doctests should reflect what a real user sees.  The problem with Sage is that the only tests (with a few exceptions) are the doctests. And for the purposes of porting to Python 3 I don't think that philosophy really applies.  It's more important just that the tests are passing so that we can narrow down on real bugs (until we make Python 3 the default, in which case yes we don't want to special case anything in the doctests if possible).",
    "created_at": "2018-02-02T12:32:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374631",
    "user": "https://github.com/embray"
}
```

<div id="comment:11" align="right">Comment 11</div>

Thanks--though what do you think about my other proposal of just injecting `long` for the purpose of the doctests *only*?  I'm personally leaning toward it as a preference.

I understand your feeling that the doctests should reflect what a real user sees.  The problem with Sage is that the only tests (with a few exceptions) are the doctests. And for the purposes of porting to Python 3 I don't think that philosophy really applies.  It's more important just that the tests are passing so that we can narrow down on real bugs (until we make Python 3 the default, in which case yes we don't want to special case anything in the doctests if possible).



---

archive/issue_comments_374632.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nReplying to [@embray](#comment%3A11):\n> Thanks--though what do you think about my other proposal of just injecting `long` for the purpose of the doctests *only*?  I'm personally leaning toward it as a preference.\n\nGo for it.",
    "created_at": "2018-02-03T19:43:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374632",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:12" align="right">Comment 12</div>

Replying to [@embray](#comment%3A11):
> Thanks--though what do you think about my other proposal of just injecting `long` for the purpose of the doctests *only*?  I'm personally leaning toward it as a preference.

Go for it.



---

archive/issue_events_322101.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-02-12T16:51:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322101"
}
```



---

archive/issue_events_322102.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-02-12T16:51:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322102"
}
```



---

archive/issue_comments_374633.json:
```json
{
    "body": "Changed dependencies from **#24258** to none",
    "created_at": "2018-02-12T16:51:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374633",
    "user": "https://github.com/fchapoton"
}
```

Changed dependencies from **#24258** to none



---

archive/issue_events_322103.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-23T14:29:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322103"
}
```



---

archive/issue_events_322104.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-23T14:29:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322104"
}
```



---

archive/issue_comments_374634.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nWhy needs work?\n\nTh recent patchbot failures on this ticket are all a joke--they're broken patchbots.  This one passed with the current branch on this ticket, and isn't a broken patchbot: https://patchbot.sagemath.org/log/24559/Ubuntu/16.04/x86_64/3.13.0-123-generic/sagemath-patchbot-docker/2018-02-04%2014:11:23",
    "created_at": "2018-02-23T14:29:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374634",
    "user": "https://github.com/embray"
}
```

<div id="comment:14" align="right">Comment 14</div>

Why needs work?

Th recent patchbot failures on this ticket are all a joke--they're broken patchbots.  This one passed with the current branch on this ticket, and isn't a broken patchbot: https://patchbot.sagemath.org/log/24559/Ubuntu/16.04/x86_64/3.13.0-123-generic/sagemath-patchbot-docker/2018-02-04%2014:11:23



---

archive/issue_events_322105.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-23T14:30:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322105"
}
```



---

archive/issue_events_322106.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-23T14:30:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322106"
}
```



---

archive/issue_comments_374635.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nIgnore me--I *did* do the proposed work, but it seems I haven't pushed it to the upstream branch yet...",
    "created_at": "2018-02-23T14:30:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374635",
    "user": "https://github.com/embray"
}
```

<div id="comment:15" align="right">Comment 15</div>

Ignore me--I *did* do the proposed work, but it seems I haven't pushed it to the upstream branch yet...



---

archive/issue_comments_374636.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. Last 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/12ff3be331475a61fa4925b5e89898409c7f732d\">12ff3be</a></td><td><code>Fix various minor encoding issues</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bf6cf97ada80f3d70a07cd416a53acd2453b6d88\">bf6cf97</a></td><td><code>Correct some encoding issues on Python 2.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4b92b47403df0b0ae371d846dcba13b88a368d98\">4b92b47</a></td><td><code>A few other miscellaneous fixes to the doctests tests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ed249101a327c4deb035f50385874cf5b902a050\">ed24910</a></td><td><code>Various fixes intended to ensure that opened files are closed explictly</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/47e464dda9aee8cb43651568ef721ee054cbab04\">47e464d</a></td><td><code>Use the restore_atexit context manager for this code to work on Python 3,</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8995cce69565686e5fcdeff991a3a9dae0d96f05\">8995cce</a></td><td><code>On Python 3, allow Python 2-style long literals (they are just treated as normal int literals)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3a6d2fcb18e0327db8771253230186729140a23b\">3a6d2fc</a></td><td><code>Doctest normalization for Python 2 long literals.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/22c35443b11b2b2371d10d131d7af881d9162349\">22c3544</a></td><td><code>On Python 3 add a 'long' built-in to Sage that acts as a replacement for the Python 2 'long' built-in at least for type conversions.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/45450f3b8bc12cac16d9f24f5420c6ba71697bd2\">45450f3</a></td><td><code>Properly handle cases like '1rL'</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/036a788b50a1b538c8fbd5bdb5a4ca7d07ba4a2e\">036a788</a></td><td><code>py3: instead of providing a user-visible 'long' alias, just inject it into the globals for doctests, just as a transitional measure for the doctests only</code></td></tr></table>\n",
    "created_at": "2018-02-23T15:37:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374636",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:16" align="right">Comment 16</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/12ff3be331475a61fa4925b5e89898409c7f732d">12ff3be</a></td><td><code>Fix various minor encoding issues</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bf6cf97ada80f3d70a07cd416a53acd2453b6d88">bf6cf97</a></td><td><code>Correct some encoding issues on Python 2.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4b92b47403df0b0ae371d846dcba13b88a368d98">4b92b47</a></td><td><code>A few other miscellaneous fixes to the doctests tests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ed249101a327c4deb035f50385874cf5b902a050">ed24910</a></td><td><code>Various fixes intended to ensure that opened files are closed explictly</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/47e464dda9aee8cb43651568ef721ee054cbab04">47e464d</a></td><td><code>Use the restore_atexit context manager for this code to work on Python 3,</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8995cce69565686e5fcdeff991a3a9dae0d96f05">8995cce</a></td><td><code>On Python 3, allow Python 2-style long literals (they are just treated as normal int literals)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3a6d2fcb18e0327db8771253230186729140a23b">3a6d2fc</a></td><td><code>Doctest normalization for Python 2 long literals.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/22c35443b11b2b2371d10d131d7af881d9162349">22c3544</a></td><td><code>On Python 3 add a 'long' built-in to Sage that acts as a replacement for the Python 2 'long' built-in at least for type conversions.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/45450f3b8bc12cac16d9f24f5420c6ba71697bd2">45450f3</a></td><td><code>Properly handle cases like '1rL'</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/036a788b50a1b538c8fbd5bdb5a4ca7d07ba4a2e">036a788</a></td><td><code>py3: instead of providing a user-visible 'long' alias, just inject it into the globals for doctests, just as a transitional measure for the doctests only</code></td></tr></table>




---

archive/issue_comments_374637.json:
```json
{
    "body": "Changed commit from **[e3c14f3](https://github.com/sagemath/sagetrac-mirror/commit/e3c14f32ac26b184520ee530b3e4de754971a004)** to **[036a788](https://github.com/sagemath/sagetrac-mirror/commit/036a788b50a1b538c8fbd5bdb5a4ca7d07ba4a2e)**",
    "created_at": "2018-02-23T15:37:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374637",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e3c14f3](https://github.com/sagemath/sagetrac-mirror/commit/e3c14f32ac26b184520ee530b3e4de754971a004)** to **[036a788](https://github.com/sagemath/sagetrac-mirror/commit/036a788b50a1b538c8fbd5bdb5a4ca7d07ba4a2e)**



---

archive/issue_comments_374638.json:
```json
{
    "body": "Dependencies: **#24343**",
    "created_at": "2018-02-23T15:39:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374638",
    "user": "https://github.com/embray"
}
```

Dependencies: **#24343**



---

archive/issue_comments_374639.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nThis depends on #24343, but the new approach to handling `long(...)` can be see in the latest commit.\n\nNow, instead of providing a user-visible replacement for `long` (which as I've already remarked is probably not that useful anyways, or at least a slippery-slope to providing other replacements for old Python 2 builtins), it just injects it into the globals for doctests.  This should be seen purely as a transitional measure.\n\n---\n|                                                                                                                                                 |                                                                                                                                                           |\n|-------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|\n|[036a788](https://github.com/sagemath/sagetrac-mirror/commit/036a788b50a1b538c8fbd5bdb5a4ca7d07ba4a2e)|`py3: instead of providing a user-visible 'long' alias, just inject it into the globals for doctests, just as a transitional measure for the doctests only`|",
    "created_at": "2018-02-23T15:39:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374639",
    "user": "https://github.com/embray"
}
```

<div id="comment:17" align="right">Comment 17</div>

This depends on #24343, but the new approach to handling `long(...)` can be see in the latest commit.

Now, instead of providing a user-visible replacement for `long` (which as I've already remarked is probably not that useful anyways, or at least a slippery-slope to providing other replacements for old Python 2 builtins), it just injects it into the globals for doctests.  This should be seen purely as a transitional measure.

---
|                                                                                                                                                 |                                                                                                                                                           |
|-------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
|[036a788](https://github.com/sagemath/sagetrac-mirror/commit/036a788b50a1b538c8fbd5bdb5a4ca7d07ba4a2e)|`py3: instead of providing a user-visible 'long' alias, just inject it into the globals for doctests, just as a transitional measure for the doctests only`|



---

archive/issue_events_322107.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-23T15:39:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322107"
}
```



---

archive/issue_events_322108.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-02-23T15:39:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322108"
}
```



---

archive/issue_comments_374640.json:
```json
{
    "body": "Changed dependencies from **#24343** to **#24343, #24922**",
    "created_at": "2018-03-13T16:31:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374640",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **#24343** to **#24343, #24922**



---

archive/issue_comments_374641.json:
```json
{
    "body": "Changed branch from **[u/embray/python3/long-literals](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/long-literals)** to **[u/jdemeyer/python3/long-literals](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/python3/long-literals)**",
    "created_at": "2018-03-13T16:35:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374641",
    "user": "https://github.com/jdemeyer"
}
```

Changed branch from **[u/embray/python3/long-literals](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/long-literals)** to **[u/jdemeyer/python3/long-literals](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/python3/long-literals)**



---

archive/issue_comments_374642.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nRebased\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/af86e5c72b3157112648d878a03a5fde2f508319\">af86e5c</a></td><td><code>Various fixes intended to ensure that opened files are closed explictly where</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/436ed266f954ecf03f894b9c6539cca353a68ad6\">436ed26</a></td><td><code>A few other miscellaneous fixes to the doctests tests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e56690147cd2cc1987312232fff9088f25c153df\">e566901</a></td><td><code>Use the restore_atexit context manager for this code to work on Python 3,</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6fd29121b05be59dd65e69004c8f7794a6e7956c\">6fd2912</a></td><td><code>Add run option to restore_atexit context</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/46017b65c382daa67bb392add344dab379393fa7\">46017b6</a></td><td><code>Use restore_atexit(run=True) in doctest framework</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/13f433cbfd91cd09d2415305b3c54859ef1e983c\">13f433c</a></td><td><code>make this exception message more flexible to the exact point in the Python interpreter where it occurred</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4a60b3d6c0adea7e38255c3187f9bed256240585\">4a60b3d</a></td><td><code>Merge commit '436ed266f954ecf03f894b9c6539cca353a68ad6'; commit '13f433cbfd91cd09d2415305b3c54859ef1e983c' into HEAD</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9b24fb6c1a6561f9c5b1a34c75f550f0bb01640f\">9b24fb6</a></td><td><code>On Python 3, allow Python 2-style long literals (they are just treated as normal int literals)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/917851779eae32332ad1dfbbf1069a03ced11c95\">9178517</a></td><td><code>Doctest normalization for Python 2 long literals.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/875149800aaa3d0660cf0d7c3789da0542748611\">8751498</a></td><td><code>py3: instead of providing a user-visible 'long' alias, just inject it into the globals for doctests, just as a transitional measure for the doctests only</code></td></tr></table>\n",
    "created_at": "2018-03-13T16:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374642",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:20" align="right">Comment 20</div>

Rebased

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/af86e5c72b3157112648d878a03a5fde2f508319">af86e5c</a></td><td><code>Various fixes intended to ensure that opened files are closed explictly where</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/436ed266f954ecf03f894b9c6539cca353a68ad6">436ed26</a></td><td><code>A few other miscellaneous fixes to the doctests tests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e56690147cd2cc1987312232fff9088f25c153df">e566901</a></td><td><code>Use the restore_atexit context manager for this code to work on Python 3,</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6fd29121b05be59dd65e69004c8f7794a6e7956c">6fd2912</a></td><td><code>Add run option to restore_atexit context</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/46017b65c382daa67bb392add344dab379393fa7">46017b6</a></td><td><code>Use restore_atexit(run=True) in doctest framework</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/13f433cbfd91cd09d2415305b3c54859ef1e983c">13f433c</a></td><td><code>make this exception message more flexible to the exact point in the Python interpreter where it occurred</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4a60b3d6c0adea7e38255c3187f9bed256240585">4a60b3d</a></td><td><code>Merge commit '436ed266f954ecf03f894b9c6539cca353a68ad6'; commit '13f433cbfd91cd09d2415305b3c54859ef1e983c' into HEAD</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9b24fb6c1a6561f9c5b1a34c75f550f0bb01640f">9b24fb6</a></td><td><code>On Python 3, allow Python 2-style long literals (they are just treated as normal int literals)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/917851779eae32332ad1dfbbf1069a03ced11c95">9178517</a></td><td><code>Doctest normalization for Python 2 long literals.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/875149800aaa3d0660cf0d7c3789da0542748611">8751498</a></td><td><code>py3: instead of providing a user-visible 'long' alias, just inject it into the globals for doctests, just as a transitional measure for the doctests only</code></td></tr></table>




---

archive/issue_comments_374643.json:
```json
{
    "body": "Changed commit from **[036a788](https://github.com/sagemath/sagetrac-mirror/commit/036a788b50a1b538c8fbd5bdb5a4ca7d07ba4a2e)** to **[8751498](https://github.com/sagemath/sagetrac-mirror/commit/875149800aaa3d0660cf0d7c3789da0542748611)**",
    "created_at": "2018-03-13T16:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374643",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[036a788](https://github.com/sagemath/sagetrac-mirror/commit/036a788b50a1b538c8fbd5bdb5a4ca7d07ba4a2e)** to **[8751498](https://github.com/sagemath/sagetrac-mirror/commit/875149800aaa3d0660cf0d7c3789da0542748611)**



---

archive/issue_comments_374644.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nJeroen, I see you rebased this, but does that constitute a \"review\"?\n\nIIRC, the only open question on this ticket had to do with my plan for aliasing `long` in Python 3.  Originally I was adding an alias in the global namespace for all users (that would raise a deprecation warning).  But now I've gone back on that, and am only injecting `long` as an alias for `int` into the globals for doctests.\n\nMy experience with testing this, so far, has been that it allows a lot of tests to pass on Python 3 that wouldn't otherwise, with few exceptions, and that it's a decent transitional approach.\n\nThe `extra_globals` dict I added could prove useful for a few other things too.",
    "created_at": "2018-03-27T08:41:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374644",
    "user": "https://github.com/embray"
}
```

<div id="comment:21" align="right">Comment 21</div>

Jeroen, I see you rebased this, but does that constitute a "review"?

IIRC, the only open question on this ticket had to do with my plan for aliasing `long` in Python 3.  Originally I was adding an alias in the global namespace for all users (that would raise a deprecation warning).  But now I've gone back on that, and am only injecting `long` as an alias for `int` into the globals for doctests.

My experience with testing this, so far, has been that it allows a lot of tests to pass on Python 3 that wouldn't otherwise, with few exceptions, and that it's a decent transitional approach.

The `extra_globals` dict I added could prove useful for a few other things too.



---

archive/issue_comments_374645.json:
```json
{
    "body": "Changed commit from **[8751498](https://github.com/sagemath/sagetrac-mirror/commit/875149800aaa3d0660cf0d7c3789da0542748611)** to **[e3b8148](https://github.com/sagemath/sagetrac-mirror/commit/e3b8148c764377ac86fa532432676ca357e3f56b)**",
    "created_at": "2018-03-29T08:30:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374645",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8751498](https://github.com/sagemath/sagetrac-mirror/commit/875149800aaa3d0660cf0d7c3789da0542748611)** to **[e3b8148](https://github.com/sagemath/sagetrac-mirror/commit/e3b8148c764377ac86fa532432676ca357e3f56b)**



---

archive/issue_comments_374646.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8cbe1b36fbfce9157d5ce8cdf813644162ba47f9\">8cbe1b3</a></td><td><code>On Python 3, allow Python 2-style long literals (they are just treated as normal int literals)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5f0133323e1b2cf1e40654b91776be42c2783818\">5f01333</a></td><td><code>Doctest normalization for Python 2 long literals.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e3b8148c764377ac86fa532432676ca357e3f56b\">e3b8148</a></td><td><code>py3: instead of providing a user-visible 'long' alias, just inject it into the globals for doctests, just as a transitional measure for the doctests only</code></td></tr></table>\n",
    "created_at": "2018-03-29T08:30:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374646",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:22" align="right">Comment 22</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8cbe1b36fbfce9157d5ce8cdf813644162ba47f9">8cbe1b3</a></td><td><code>On Python 3, allow Python 2-style long literals (they are just treated as normal int literals)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5f0133323e1b2cf1e40654b91776be42c2783818">5f01333</a></td><td><code>Doctest normalization for Python 2 long literals.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e3b8148c764377ac86fa532432676ca357e3f56b">e3b8148</a></td><td><code>py3: instead of providing a user-visible 'long' alias, just inject it into the globals for doctests, just as a transitional measure for the doctests only</code></td></tr></table>




---

archive/issue_comments_374647.json:
```json
{
    "body": "Changed dependencies from **#24343, #24922** to none",
    "created_at": "2018-03-29T08:32:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374647",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **#24343, #24922** to none



---

archive/issue_comments_374648.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">Comment 24</div>\n\nI don't like this syntax:\n\n```\n    \"\"\"\n    Extra objects to place in the global namespace in which tests are run.\n    Normally this should be empty but there are special cases where it may\n    be useful.\n\n    In particular, on Python 3 add ``long`` as an alias for ``int`` so that\n    tests that use the ``long`` built-in (of which there are many) still pass.\n    We do this so that the test suite can run on Python 3 while Python 2 is\n    still the default.\n    \"\"\"\n```\nIt looks too much like a docstring but it isn't. Use a `# comment` instead.",
    "created_at": "2018-03-29T08:39:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374648",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:24" align="right">Comment 24</div>

I don't like this syntax:

```
    """
    Extra objects to place in the global namespace in which tests are run.
    Normally this should be empty but there are special cases where it may
    be useful.

    In particular, on Python 3 add ``long`` as an alias for ``int`` so that
    tests that use the ``long`` built-in (of which there are many) still pass.
    We do this so that the test suite can run on Python 3 while Python 2 is
    still the default.
    """
```
It looks too much like a docstring but it isn't. Use a `# comment` instead.



---

archive/issue_comments_374649.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">Comment 25</div>\n\nThat's standard syntax in Sphinx for documenting class attributes.",
    "created_at": "2018-03-29T08:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374649",
    "user": "https://github.com/embray"
}
```

<div id="comment:25" align="right">Comment 25</div>

That's standard syntax in Sphinx for documenting class attributes.



---

archive/issue_comments_374650.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">Comment 26</div>\n\nReplying to [@embray](#comment%3A25):\n> That's standard syntax in Sphinx for documenting class attributes.\n\nAccording to the [Sphinx documentation](http://www.sphinx-doc.org/en/stable/ext/autodoc.html#directive-autoattribute), Sphinx autodoc also supports the special comment syntax `#:` for this. And in fact, Sphinx itself uses that syntax.\n\nYou may think that there is no harm in using strings-which-are-not-docstrings like that, but the problem is that it teaches a bad habit. In particular the following does **not** contain a doctest despite looking like it does:\n\n```\nclass X(object):\n    attr = \"something\"\n    \"\"\"\n    EXAMPLES::\n        \n        sage: 1 + 1\n        3\n    \"\"\"\n```",
    "created_at": "2018-03-29T09:12:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374650",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:26" align="right">Comment 26</div>

Replying to [@embray](#comment%3A25):
> That's standard syntax in Sphinx for documenting class attributes.

According to the [Sphinx documentation](http://www.sphinx-doc.org/en/stable/ext/autodoc.html#directive-autoattribute), Sphinx autodoc also supports the special comment syntax `#:` for this. And in fact, Sphinx itself uses that syntax.

You may think that there is no harm in using strings-which-are-not-docstrings like that, but the problem is that it teaches a bad habit. In particular the following does **not** contain a doctest despite looking like it does:

```
class X(object):
    attr = "something"
    """
    EXAMPLES::
        
        sage: 1 + 1
        3
    """
```



---

archive/issue_comments_374651.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2018-03-29T09:42:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374651",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_322109.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-29T09:42:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322109"
}
```



---

archive/issue_events_322110.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-29T09:42:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322110"
}
```



---

archive/issue_comments_374652.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">Comment 28</div>\n\nI don't think it's a bad habit.  It's perfectly acceptable syntax and I think preferable from the Sphinx perspective over the special comment syntax.  If there *were* a notion in the Python language of attaching a docstring to a particular class attribute that's probably how I'd write it (unfortunately the concept doesn't quite fit in with how the language works, though personally I can think of ways Python might use it).\n\nI think your concern is more a peculiarity of Sage, which overemphasizes doctests in the first place (or, perhaps if this really a problem, the doctest parser should be modified to look in triple-quoted strings anywhere in a class or module body--I think that might make sense anyways).",
    "created_at": "2018-03-30T13:37:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374652",
    "user": "https://github.com/embray"
}
```

<div id="comment:28" align="right">Comment 28</div>

I don't think it's a bad habit.  It's perfectly acceptable syntax and I think preferable from the Sphinx perspective over the special comment syntax.  If there *were* a notion in the Python language of attaching a docstring to a particular class attribute that's probably how I'd write it (unfortunately the concept doesn't quite fit in with how the language works, though personally I can think of ways Python might use it).

I think your concern is more a peculiarity of Sage, which overemphasizes doctests in the first place (or, perhaps if this really a problem, the doctest parser should be modified to look in triple-quoted strings anywhere in a class or module body--I think that might make sense anyways).



---

archive/issue_events_322111.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322111"
}
```



---

archive/issue_events_322112.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322112"
}
```



---

archive/issue_events_322113.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-08T17:26:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322113"
}
```



---

archive/issue_events_322114.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "af11d43628b07c9a92cf2659ed92d9ea3c165700",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-08T17:26:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24559#event-322114"
}
```



---

archive/issue_comments_374653.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/python3/long-literals](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/python3/long-literals)** to **[e3b8148](https://github.com/sagemath/sagetrac-mirror/commit/e3b8148c764377ac86fa532432676ca357e3f56b)**",
    "created_at": "2018-05-08T17:26:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24559",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24559#issuecomment-374653",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/python3/long-literals](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/python3/long-literals)** to **[e3b8148](https://github.com/sagemath/sagetrac-mirror/commit/e3b8148c764377ac86fa532432676ca357e3f56b)**
