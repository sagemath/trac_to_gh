# Issue 24727: Graphs, check for algorithm

archive/issues_024490.json:
```json
{
    "assignees": [],
    "body": "Try\n\n```\ng = graphs.PetersenGraph()\ng.is_chordal(algorithm='junk')\ng.canonical_label(algorithm=0)\n```\n\nCC:  @dcoudert\n\nBranch/Commit: **[cd25d3e](https://github.com/sagemath/sagetrac-mirror/commit/cd25d3eaeb474e308c0839925b8c5b53d5834d24)**\n\nReviewer: **David Coudert**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24727_\n\n",
    "closed_at": "2018-02-18T12:52:01Z",
    "created_at": "2018-02-13T19:56:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Graphs, check for algorithm",
    "type": "issue",
    "updated_at": "2018-02-18T12:52:01Z",
    "url": "https://github.com/sagemath/sage/issues/24727",
    "user": "https://github.com/jm58660"
}
```
Try

```
g = graphs.PetersenGraph()
g.is_chordal(algorithm='junk')
g.canonical_label(algorithm=0)
```

CC:  @dcoudert

Branch/Commit: **[cd25d3e](https://github.com/sagemath/sagetrac-mirror/commit/cd25d3eaeb474e308c0839925b8c5b53d5834d24)**

Reviewer: **David Coudert**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/24727_





---

archive/issue_events_324288.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-13T19:56:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24727#event-324288"
}
```



---

archive/issue_events_324289.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-13T19:56:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24727#event-324289"
}
```



---

archive/issue_events_324290.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-13T19:56:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24727#event-324290"
}
```



---

archive/issue_events_324291.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-13T19:56:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24727#event-324291"
}
```



---

archive/issue_comments_378539.json:
```json
{
    "body": "Branch: **[u/jmantysalo/graphs__check_for_algorithm](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/graphs__check_for_algorithm)**",
    "created_at": "2018-02-14T06:35:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24727#issuecomment-378539",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/graphs__check_for_algorithm](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/graphs__check_for_algorithm)**



---

archive/issue_events_324292.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-02-14T06:36:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24727#event-324292"
}
```



---

archive/issue_comments_378540.json:
```json
{
    "body": "Author: **Jori M\u00e4ntysalo**",
    "created_at": "2018-02-14T06:36:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24727#issuecomment-378540",
    "user": "https://github.com/jm58660"
}
```

Author: **Jori Mäntysalo**



---

archive/issue_comments_378541.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cd25d3eaeb474e308c0839925b8c5b53d5834d24\">cd25d3e</a></td><td><code>Add two test for parameter value.</code></td></tr></table>\n",
    "created_at": "2018-02-14T06:36:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24727#issuecomment-378541",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cd25d3eaeb474e308c0839925b8c5b53d5834d24">cd25d3e</a></td><td><code>Add two test for parameter value.</code></td></tr></table>




---

archive/issue_comments_378542.json:
```json
{
    "body": "Commit: **[cd25d3e](https://github.com/sagemath/sagetrac-mirror/commit/cd25d3eaeb474e308c0839925b8c5b53d5834d24)**",
    "created_at": "2018-02-14T06:36:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24727#issuecomment-378542",
    "user": "https://github.com/jm58660"
}
```

Commit: **[cd25d3e](https://github.com/sagemath/sagetrac-mirror/commit/cd25d3eaeb474e308c0839925b8c5b53d5834d24)**



---

archive/issue_comments_378543.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2018-02-14T07:38:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24727#issuecomment-378543",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_comments_378544.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nGood to go.",
    "created_at": "2018-02-14T07:38:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24727#issuecomment-378544",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:3'>Comment 3:</a>
Good to go.



---

archive/issue_events_324293.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-02-14T07:38:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24727#event-324293"
}
```



---

archive/issue_events_324294.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-02-14T07:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24727#event-324294"
}
```



---

archive/issue_comments_378545.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/graphs__check_for_algorithm](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/graphs__check_for_algorithm)** to **[cd25d3e](https://github.com/sagemath/sagetrac-mirror/commit/cd25d3eaeb474e308c0839925b8c5b53d5834d24)**",
    "created_at": "2018-02-18T12:52:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24727#issuecomment-378545",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/graphs__check_for_algorithm](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/graphs__check_for_algorithm)** to **[cd25d3e](https://github.com/sagemath/sagetrac-mirror/commit/cd25d3eaeb474e308c0839925b8c5b53d5834d24)**



---

archive/issue_events_324295.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-18T12:52:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24727#event-324295"
}
```



---

archive/issue_events_324296.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c66161549b249a5aa0a0e206fc8936ad33379111",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-02-18T12:52:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24727",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24727#event-324296"
}
```
