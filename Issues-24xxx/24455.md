# Issue 24455: _integer_ for QQbar and QQ raise different errors.

archive/issues_024218.json:
```json
{
    "body": "\n```\nsage: ZZ(1/2)\n```\nfails and (correctly) raises a TypeError while\n\n```\nsage: ZZ(QQbar(sqrt(17)))\n```\n\nfails and (incorrectly) raises a ValueError. \n\nThis has several knock on effects, in particular vector(ZZ, [list of algebraic numbers]) all of whoms try / except blocks are set up to catch a TypeError in the event that one of the elements fails to be coerced to an integer. This in turn causes Polyhedron([(AA(sqrt(17)),)]).integral_points() to fail as this ValueError is never caught.\n\nKeywords: TypeError, ValueError, ZZ, QQbar\n\nBranch: u/mcbell/_integer__for_qqbar_and_qq_raise_different_errors_\n\nStatus: new\n\nCommit: b4df150c788cad61752ce513c726215c23013970\n\nIssue created by migration from https://trac.sagemath.org/ticket/24455\n\n",
    "created_at": "2018-01-01T15:23:03Z",
    "labels": [
        "component: number fields",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.2",
    "title": "_integer_ for QQbar and QQ raise different errors.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24455",
    "user": "https://trac.sagemath.org/admin/accounts/users/mcbell"
}
```

```
sage: ZZ(1/2)
```
fails and (correctly) raises a TypeError while

```
sage: ZZ(QQbar(sqrt(17)))
```

fails and (incorrectly) raises a ValueError. 

This has several knock on effects, in particular vector(ZZ, [list of algebraic numbers]) all of whoms try / except blocks are set up to catch a TypeError in the event that one of the elements fails to be coerced to an integer. This in turn causes Polyhedron([(AA(sqrt(17)),)]).integral_points() to fail as this ValueError is never caught.

Keywords: TypeError, ValueError, ZZ, QQbar

Branch: u/mcbell/_integer__for_qqbar_and_qq_raise_different_errors_

Status: new

Commit: b4df150c788cad61752ce513c726215c23013970

Issue created by migration from https://trac.sagemath.org/ticket/24455





---

archive/issue_comments_462862.json:
```json
{
    "body": "<a id='comment:1'></a>I believe that `ValueError` for `ZZ(1/2)` is the correct one. The main reason is that the following **conversion** should work `ZZ(QQ(3))`. Hence, it is not due to the type of the object but its value.\n\nNote that the following **coercion** should fail with a `TypeError` `ZZ.coerce(QQ(3))`.",
    "created_at": "2018-01-01T16:16:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24455#issuecomment-462862",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'></a>I believe that `ValueError` for `ZZ(1/2)` is the correct one. The main reason is that the following **conversion** should work `ZZ(QQ(3))`. Hence, it is not due to the type of the object but its value.

Note that the following **coercion** should fail with a `TypeError` `ZZ.coerce(QQ(3))`.



---

archive/issue_comments_462863.json:
```json
{
    "body": "<a id='comment:2'></a>Replying to [vdelecroix](#comment%3A1):\n> I believe that `ValueError` for `ZZ(1/2)` is the correct one. The main reason is that the following **conversion** should work `ZZ(QQ(3))`. Hence, it is not due to the type of the object but its value.\n> \n> Note that the following **coercion** should fail with a `TypeError` `ZZ.coerce(QQ(3))`.\n\n\nHmm, so does that mean that the Sequence initialiser in sage/structure/sequence.pyc should catch more types of errors since conversion can also fail due to a ValueError?\n\n```\n    455             for i in range(len(x)):\n    456                 try:\n    457                     x[i] = universe(x[i])\n    458                 except TypeError:\n    459                     raise TypeError(\"unable to convert {} to an element of {}\"\n```",
    "created_at": "2018-01-01T18:06:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24455#issuecomment-462863",
    "user": "https://trac.sagemath.org/admin/accounts/users/mcbell"
}
```

<a id='comment:2'></a>Replying to [vdelecroix](#comment%3A1):
> I believe that `ValueError` for `ZZ(1/2)` is the correct one. The main reason is that the following **conversion** should work `ZZ(QQ(3))`. Hence, it is not due to the type of the object but its value.
> 
> Note that the following **coercion** should fail with a `TypeError` `ZZ.coerce(QQ(3))`.


Hmm, so does that mean that the Sequence initialiser in sage/structure/sequence.pyc should catch more types of errors since conversion can also fail due to a ValueError?

```
    455             for i in range(len(x)):
    456                 try:
    457                     x[i] = universe(x[i])
    458                 except TypeError:
    459                     raise TypeError("unable to convert {} to an element of {}"
```



---

archive/issue_comments_462864.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [mcbell](#comment%3A2):\n> Replying to [vdelecroix](#comment%3A1):\n> > I believe that `ValueError` for `ZZ(1/2)` is the correct one. The main reason is that the following **conversion** should work `ZZ(QQ(3))`. Hence, it is not due to the type of the object but its value.\n> > \n> > Note that the following **coercion** should fail with a `TypeError` `ZZ.coerce(QQ(3))`.\n\n> \n> Hmm, so does that mean that the Sequence initialiser in sage/structure/sequence.pyc should catch more types of errors since conversion can also fail due to a ValueError?\n> \n> \n> ```\n>     455             for i in range(len(x)):\n>     456                 try:\n>     457                     x[i] = universe(x[i])\n>     458                 except TypeError:\n>     459                     raise TypeError(\"unable to convert {} to an element of {}\"\n> ```\n\n\nTwo possibilities\n\n1. (line 458) `except TypeError` -> `except (TypeError, ValueError)`\n2. (line 457) `x[i] = universe.coerce(x[i])`\n\nI do prefer the second one since coercion is stricter and more predictible.",
    "created_at": "2018-01-01T18:09:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24455#issuecomment-462864",
    "user": "https://github.com/videlec"
}
```

<a id='comment:3'></a>Replying to [mcbell](#comment%3A2):
> Replying to [vdelecroix](#comment%3A1):
> > I believe that `ValueError` for `ZZ(1/2)` is the correct one. The main reason is that the following **conversion** should work `ZZ(QQ(3))`. Hence, it is not due to the type of the object but its value.
> > 
> > Note that the following **coercion** should fail with a `TypeError` `ZZ.coerce(QQ(3))`.

> 
> Hmm, so does that mean that the Sequence initialiser in sage/structure/sequence.pyc should catch more types of errors since conversion can also fail due to a ValueError?
> 
> 
> ```
>     455             for i in range(len(x)):
>     456                 try:
>     457                     x[i] = universe(x[i])
>     458                 except TypeError:
>     459                     raise TypeError("unable to convert {} to an element of {}"
> ```


Two possibilities

1. (line 458) `except TypeError` -> `except (TypeError, ValueError)`
2. (line 457) `x[i] = universe.coerce(x[i])`

I do prefer the second one since coercion is stricter and more predictible.



---

archive/issue_comments_462865.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [vdelecroix](#comment%3A3):\n> Two possibilities\n> \n> 1. (line 458) `except TypeError` -> `except (TypeError, ValueError)`\n> 2. (line 457) `x[i] = universe.coerce(x[i])`\n> \n> I do prefer the second one since coercion is stricter and more predictible.\n\n\nI'm not sure that we can use the second option since we still want something like:\n\n```\nvector(ZZ, [7, 3/1])\n```\nto succeed. However I think the first would work perfectly well.",
    "created_at": "2018-01-01T20:29:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24455#issuecomment-462865",
    "user": "https://trac.sagemath.org/admin/accounts/users/mcbell"
}
```

<a id='comment:4'></a>Replying to [vdelecroix](#comment%3A3):
> Two possibilities
> 
> 1. (line 458) `except TypeError` -> `except (TypeError, ValueError)`
> 2. (line 457) `x[i] = universe.coerce(x[i])`
> 
> I do prefer the second one since coercion is stricter and more predictible.


I'm not sure that we can use the second option since we still want something like:

```
vector(ZZ, [7, 3/1])
```
to succeed. However I think the first would work perfectly well.



---

archive/issue_comments_462866.json:
```json
{
    "body": "Changing branch from \"\" to \"u/mcbell/_integer__for_qqbar_and_qq_raise_different_errors_\"",
    "created_at": "2018-01-02T10:42:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24455#issuecomment-462866",
    "user": "https://trac.sagemath.org/admin/accounts/users/mcbell"
}
```

Changing branch from "" to "u/mcbell/_integer__for_qqbar_and_qq_raise_different_errors_"



---

archive/issue_comments_462867.json:
```json
{
    "body": "<a id='comment:6'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2018-01-02T10:44:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24455#issuecomment-462867",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_462868.json:
```json
{
    "body": "Changing commit from \"\" to \"b4df150c788cad61752ce513c726215c23013970\"",
    "created_at": "2018-01-02T10:44:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24455#issuecomment-462868",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "" to "b4df150c788cad61752ce513c726215c23013970"



---

archive/issue_comments_462869.json:
```json
{
    "body": "<a id='comment:7'></a>This is not the right fix. `_integer_` should `raise TypeError` instead.",
    "created_at": "2018-01-02T10:59:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24455#issuecomment-462869",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>This is not the right fix. `_integer_` should `raise TypeError` instead.



---

archive/issue_comments_462870.json:
```json
{
    "body": "<a id='comment:8'></a>Replying to [jdemeyer](#comment%3A7):\n> This is not the right fix. `_integer_` should `raise TypeError` instead.\n\n\nSo I certainly agree that Rational._integer_ (which raises a TypeError) and AlgebraicNumber._integer_ (which raises a ValueError) should raise the same error. However I'm not sure why AlgebraicNumber._integer_ should be changed to raise a TypeError. I would have thought that a ValueError (https://docs.python.org/2/library/exceptions.html#exceptions.ValueError):\n\n> Raised when a built-in operation or function receives an argument that has the right type but an inappropriate value, and the situation is not described by a more precise exception such as IndexError.\n\n\nwould be the appropriate error for both, similar to how `int('foo')` raises a ValueError.",
    "created_at": "2018-01-02T11:23:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24455#issuecomment-462870",
    "user": "https://trac.sagemath.org/admin/accounts/users/mcbell"
}
```

<a id='comment:8'></a>Replying to [jdemeyer](#comment%3A7):
> This is not the right fix. `_integer_` should `raise TypeError` instead.


So I certainly agree that Rational._integer_ (which raises a TypeError) and AlgebraicNumber._integer_ (which raises a ValueError) should raise the same error. However I'm not sure why AlgebraicNumber._integer_ should be changed to raise a TypeError. I would have thought that a ValueError (https://docs.python.org/2/library/exceptions.html#exceptions.ValueError):

> Raised when a built-in operation or function receives an argument that has the right type but an inappropriate value, and the situation is not described by a more precise exception such as IndexError.


would be the appropriate error for both, similar to how `int('foo')` raises a ValueError.
