# Issue 24694: gfortran should not be installed if the gcc package is installed

archive/issues_024457.json:
```json
{
    "body": "`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed.\n\nThe same should be done for gfortran: it should not be installed when the GCC Sage package is already installed. There is a check that gfortran is not installed if we need to install GCC. But there is no such check if GCC was already installed earlier.\n\nIn the `configure.ac` script, this can be tested by `test x$SAGE_INSTALL_GCC = xexists`\n\nCC:  @kiwifb\n\nBranch/Commit: 9e58f1f8f6fb0db84fec3fdc01b32e32dcf8700b\n\nReviewer: Jeroen Demeyer\n\nAuthor: Fran\u00e7ois Bissey\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/24694\n\n",
    "closed_at": "2018-02-13T08:10:55Z",
    "created_at": "2018-02-09T13:07:37Z",
    "labels": [
        "component: build: configure",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.2",
    "title": "gfortran should not be installed if the gcc package is installed",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24694",
    "user": "https://github.com/jdemeyer"
}
```
`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed.

The same should be done for gfortran: it should not be installed when the GCC Sage package is already installed. There is a check that gfortran is not installed if we need to install GCC. But there is no such check if GCC was already installed earlier.

In the `configure.ac` script, this can be tested by `test x$SAGE_INSTALL_GCC = xexists`

CC:  @kiwifb

Branch/Commit: 9e58f1f8f6fb0db84fec3fdc01b32e32dcf8700b

Reviewer: Jeroen Demeyer

Author: François Bissey

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/24694





---

archive/issue_comments_468953.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed. The same should be done for gfortran: it should not be installed when the GCC Sage package is already installed.\n+`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed. The same should be done for gfortran: it should not be installed when the GCC Sage package is already installed. In the `configure.ac` script, this can be tested by `test x$SAGE_INSTALL_GCC = xexists`\n``````\n",
    "created_at": "2018-02-09T13:12:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468953",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed. The same should be done for gfortran: it should not be installed when the GCC Sage package is already installed.
+`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed. The same should be done for gfortran: it should not be installed when the GCC Sage package is already installed. In the `configure.ac` script, this can be tested by `test x$SAGE_INSTALL_GCC = xexists`
``````




---

archive/issue_comments_468954.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n-`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed. The same should be done for gfortran: it should not be installed when the GCC Sage package is already installed. In the `configure.ac` script, this can be tested by `test x$SAGE_INSTALL_GCC = xexists`\n+`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed.\n+\n+The same should be done for gfortran: it should not be installed when the GCC Sage package is already installed. There is a check that gfortran is not installed if we need to install GCC. But there is no such check if GCC was already installed earlier.\n+\n+In the `configure.ac` script, this can be tested by `test x$SAGE_INSTALL_GCC = xexists`\n``````\n",
    "created_at": "2018-02-09T13:26:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468954",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
-`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed. The same should be done for gfortran: it should not be installed when the GCC Sage package is already installed. In the `configure.ac` script, this can be tested by `test x$SAGE_INSTALL_GCC = xexists`
+`configure.ac` makes sure not to install GCC when the GCC Sage package is already installed.
+
+The same should be done for gfortran: it should not be installed when the GCC Sage package is already installed. There is a check that gfortran is not installed if we need to install GCC. But there is no such check if GCC was already installed earlier.
+
+In the `configure.ac` script, this can be tested by `test x$SAGE_INSTALL_GCC = xexists`
``````




---

archive/issue_comments_468955.json:
```json
{
    "body": "Changing branch from \"\" to \"u/fbissey/gcc_installed\"",
    "created_at": "2018-02-10T04:00:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468955",
    "user": "https://github.com/kiwifb"
}
```

Changing branch from "" to "u/fbissey/gcc_installed"



---

archive/issue_comments_468956.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-02-10T04:00:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468956",
    "user": "https://github.com/kiwifb"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_468957.json:
```json
{
    "body": "<a id='comment:3'></a>\nSomething like this branch then. Do we need to provide a new configure tarball?\n\n---\nNew commits:\n|                                                                                                                                          |                                                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|\n|[0120349](https://git.sagemath.org/sage.git/commit?id=012034994fb8be786dd39435bfe46114f849aa64)|`check if gcc is installed before setting need_gfortran`|\n|[fb83388](https://git.sagemath.org/sage.git/commit?id=fb833882a9cca8bd8e19b049987bc954fe43782a)|`Add comment`|",
    "created_at": "2018-02-10T04:00:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468957",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:3'></a>
Something like this branch then. Do we need to provide a new configure tarball?

---
New commits:
|                                                                                                                                          |                                                        |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|
|[0120349](https://git.sagemath.org/sage.git/commit?id=012034994fb8be786dd39435bfe46114f849aa64)|`check if gcc is installed before setting need_gfortran`|
|[fb83388](https://git.sagemath.org/sage.git/commit?id=fb833882a9cca8bd8e19b049987bc954fe43782a)|`Add comment`|



---

archive/issue_comments_468958.json:
```json
{
    "body": "Changing commit from \"\" to \"fb833882a9cca8bd8e19b049987bc954fe43782a\"",
    "created_at": "2018-02-10T04:00:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468958",
    "user": "https://github.com/kiwifb"
}
```

Changing commit from "" to "fb833882a9cca8bd8e19b049987bc954fe43782a"



---

archive/issue_comments_468959.json:
```json
{
    "body": "Changing author from \"\" to \"Fran\u00e7ois Bissey\"",
    "created_at": "2018-02-10T04:00:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468959",
    "user": "https://github.com/kiwifb"
}
```

Changing author from "" to "François Bissey"



---

archive/issue_comments_468960.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-02-10T09:12:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468960",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_468961.json:
```json
{
    "body": "<a id='comment:4'></a>\nIn the ticket description I wrote `test x$SAGE_INSTALL_GCC = xexists` and I meant exactly what I wrote.",
    "created_at": "2018-02-10T09:12:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468961",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>
In the ticket description I wrote `test x$SAGE_INSTALL_GCC = xexists` and I meant exactly what I wrote.



---

archive/issue_comments_468962.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2018-02-10T09:12:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468962",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_comments_468963.json:
```json
{
    "body": "<a id='comment:5'></a>\nBoth are needed, I am not fussy about doing it in a cascade rather than a \"or\" clause.",
    "created_at": "2018-02-10T09:16:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468963",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:5'></a>
Both are needed, I am not fussy about doing it in a cascade rather than a "or" clause.



---

archive/issue_comments_468964.json:
```json
{
    "body": "<a id='comment:6'></a>\nOK, please do the following: **carefully** read what you wrote and what I proposed in the ticket description. It's not the same.",
    "created_at": "2018-02-10T11:25:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468964",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>
OK, please do the following: **carefully** read what you wrote and what I proposed in the ticket description. It's not the same.



---

archive/issue_comments_468965.json:
```json
{
    "body": "<a id='comment:7'></a>\nS!**t. Thanks for the patience.",
    "created_at": "2018-02-10T11:34:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468965",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:7'></a>
S!**t. Thanks for the patience.



---

archive/issue_comments_468966.json:
```json
{
    "body": "<a id='comment:8'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                                                                   |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------|\n|[9e58f1f](https://git.sagemath.org/sage.git/commit/?id=9e58f1f8f6fb0db84fec3fdc01b32e32dcf8700b)|`check if gcc is installed before setting need_to_install_gfortran`|",
    "created_at": "2018-02-11T02:30:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468966",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:8'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                                                                   |
|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------|
|[9e58f1f](https://git.sagemath.org/sage.git/commit/?id=9e58f1f8f6fb0db84fec3fdc01b32e32dcf8700b)|`check if gcc is installed before setting need_to_install_gfortran`|



---

archive/issue_comments_468967.json:
```json
{
    "body": "Changing commit from \"fb833882a9cca8bd8e19b049987bc954fe43782a\" to \"9e58f1f8f6fb0db84fec3fdc01b32e32dcf8700b\"",
    "created_at": "2018-02-11T02:30:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468967",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "fb833882a9cca8bd8e19b049987bc954fe43782a" to "9e58f1f8f6fb0db84fec3fdc01b32e32dcf8700b"



---

archive/issue_comments_468968.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-02-11T02:32:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468968",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_468969.json:
```json
{
    "body": "<a id='comment:10'></a>\nLet's see if this works.",
    "created_at": "2018-02-12T09:52:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468969",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>
Let's see if this works.



---

archive/issue_comments_468970.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-02-12T09:52:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468970",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_468971.json:
```json
{
    "body": "Changing branch from \"u/fbissey/gcc_installed\" to \"9e58f1f8f6fb0db84fec3fdc01b32e32dcf8700b\"",
    "created_at": "2018-02-13T08:10:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468971",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/fbissey/gcc_installed" to "9e58f1f8f6fb0db84fec3fdc01b32e32dcf8700b"



---

archive/issue_comments_468972.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-02-13T08:10:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24694#issuecomment-468972",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_062163.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-02-13T08:10:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24694",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24694#event-62163"
}
```
