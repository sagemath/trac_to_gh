# Issue 24644: Use $SAGE_SUDO when copying files from SAGE_DESTDIR to SAGE_LOCAL

archive/issues_024407.json:
```json
{
    "body": "Minor followup to #24106, which didn't support the `SAGE_SUDO` variable properly.\n\nBranch/Commit: 5a23d19e72159e4231e1b1a6391b7e9ec04cd1c6\n\nReviewer: Julian R\u00fcth\n\nAuthor: Erik Bray\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/24644\n\n",
    "closed_at": "2018-05-08T17:26:27Z",
    "created_at": "2018-02-02T12:02:13Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.3",
    "title": "Use $SAGE_SUDO when copying files from SAGE_DESTDIR to SAGE_LOCAL",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/24644",
    "user": "https://github.com/embray"
}
```
Minor followup to #24106, which didn't support the `SAGE_SUDO` variable properly.

Branch/Commit: 5a23d19e72159e4231e1b1a6391b7e9ec04cd1c6

Reviewer: Julian Rüth

Author: Erik Bray

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/24644





---

archive/issue_comments_467215.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-02-02T12:02:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467215",
    "user": "https://github.com/embray"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_467216.json:
```json
{
    "body": "<a id='comment:2'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                             |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|\n|[56b9905](https://git.sagemath.org/sage.git/commit/?id=56b9905bc57b4f038653c17e8105a846a75d044e)|`Don't use SAGE_SUDO in these helpers if SAGE_DESTDIR is set`|",
    "created_at": "2018-02-02T12:11:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467216",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:2'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                             |
|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|
|[56b9905](https://git.sagemath.org/sage.git/commit/?id=56b9905bc57b4f038653c17e8105a846a75d044e)|`Don't use SAGE_SUDO in these helpers if SAGE_DESTDIR is set`|



---

archive/issue_comments_467217.json:
```json
{
    "body": "Changing commit from \"45127cba14768900e1da138252664ba8cc71024c\" to \"56b9905bc57b4f038653c17e8105a846a75d044e\"",
    "created_at": "2018-02-02T12:11:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467217",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "45127cba14768900e1da138252664ba8cc71024c" to "56b9905bc57b4f038653c17e8105a846a75d044e"



---

archive/issue_comments_467218.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2018-02-02T12:20:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467218",
    "user": "https://github.com/embray"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_467219.json:
```json
{
    "body": "<a id='comment:4'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                                                             |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|\n|[5a23d19](https://git.sagemath.org/sage.git/commit/?id=5a23d19e72159e4231e1b1a6391b7e9ec04cd1c6)|`Don't use SAGE_SUDO in these helpers if SAGE_DESTDIR is set`|",
    "created_at": "2018-02-02T12:23:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467219",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                                                             |
|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|
|[5a23d19](https://git.sagemath.org/sage.git/commit/?id=5a23d19e72159e4231e1b1a6391b7e9ec04cd1c6)|`Don't use SAGE_SUDO in these helpers if SAGE_DESTDIR is set`|



---

archive/issue_comments_467220.json:
```json
{
    "body": "Changing commit from \"56b9905bc57b4f038653c17e8105a846a75d044e\" to \"5a23d19e72159e4231e1b1a6391b7e9ec04cd1c6\"",
    "created_at": "2018-02-02T12:23:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467220",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "56b9905bc57b4f038653c17e8105a846a75d044e" to "5a23d19e72159e4231e1b1a6391b7e9ec04cd1c6"



---

archive/issue_comments_467221.json:
```json
{
    "body": "<a id='comment:5'></a>\n(I was doing some JavaScript earlier and got some wires crossed)",
    "created_at": "2018-02-02T12:24:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467221",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>
(I was doing some JavaScript earlier and got some wires crossed)



---

archive/issue_comments_467222.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-02-02T12:24:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467222",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_467223.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2018-02-03T09:14:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467223",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_467224.json:
```json
{
    "body": "<a id='comment:7'></a>\nIf this is really meant for installations as \"root\", then the installation in `DESTDIR` should already be done under the \"sudo\" user. So I think that this patch actually reverses existing correct behaviour.\n\n=> Proposal: close as wontfix.",
    "created_at": "2018-02-03T09:14:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467224",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>
If this is really meant for installations as "root", then the installation in `DESTDIR` should already be done under the "sudo" user. So I think that this patch actually reverses existing correct behaviour.

=> Proposal: close as wontfix.



---

archive/issue_comments_467225.json:
```json
{
    "body": "<a id='comment:8'></a>\nNo, I was wrong: we should keep `SAGE_SUDO` for the `DESTDIR` installation and then also add it for moving the files to `SAGE_LOCAL`.",
    "created_at": "2018-02-03T09:29:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467225",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>
No, I was wrong: we should keep `SAGE_SUDO` for the `DESTDIR` installation and then also add it for moving the files to `SAGE_LOCAL`.



---

archive/issue_comments_467226.json:
```json
{
    "body": "<a id='comment:9'></a>\nI'm not sure about that.  In the staged installation, the install to `DESTDIR` step should be treated as a \"build\" step, and doesn't need to have root privileges to run.  I don't know what makes you think it should be done as root.\n\nOn Debian, fakeroot is provided and I believe install to the `DESTDIR` is done using fakeroot.  But in Debian's case that's because it's being used to build binary tarballs, and you want the files in the resulting tarball to be owned by \"root\" (but fakeroot allows you to do this without *actually* having to gain root privileges on the system you're building on).  For Sage's purposes this isn't necessary because we're not building binary tarballs--at least for now it's just a step needed for easily building a file manifest.\n\nSo only the actual install into `SAGE_LOCAL` should be done with elevated privileges, if any.",
    "created_at": "2018-02-05T10:33:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467226",
    "user": "https://github.com/embray"
}
```

<a id='comment:9'></a>
I'm not sure about that.  In the staged installation, the install to `DESTDIR` step should be treated as a "build" step, and doesn't need to have root privileges to run.  I don't know what makes you think it should be done as root.

On Debian, fakeroot is provided and I believe install to the `DESTDIR` is done using fakeroot.  But in Debian's case that's because it's being used to build binary tarballs, and you want the files in the resulting tarball to be owned by "root" (but fakeroot allows you to do this without *actually* having to gain root privileges on the system you're building on).  For Sage's purposes this isn't necessary because we're not building binary tarballs--at least for now it's just a step needed for easily building a file manifest.

So only the actual install into `SAGE_LOCAL` should be done with elevated privileges, if any.



---

archive/issue_comments_467227.json:
```json
{
    "body": "<a id='comment:10'></a>\nWell, it looks like maybe there are some other problems with root installs.  For example, if you try to run something like `./configure --prefix=/opt/sage` where `/opt/sage` is owned by root, it will fail because the `configure` script tries to create some directories under `$SAGE_LOCAL`.\n\nBut if you run `sudo ./configure` (which really, you shouldn't have to in the first place) it fails because of `AC_CHECK_ROOT`.",
    "created_at": "2018-02-05T12:47:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467227",
    "user": "https://github.com/embray"
}
```

<a id='comment:10'></a>
Well, it looks like maybe there are some other problems with root installs.  For example, if you try to run something like `./configure --prefix=/opt/sage` where `/opt/sage` is owned by root, it will fail because the `configure` script tries to create some directories under `$SAGE_LOCAL`.

But if you run `sudo ./configure` (which really, you shouldn't have to in the first place) it fails because of `AC_CHECK_ROOT`.



---

archive/issue_comments_467228.json:
```json
{
    "body": "<a id='comment:11'></a>\nReplying to [embray](#comment%3A10):\n> Well, it looks like maybe there are some other problems with root installs.  For example, if you try to run something like `./configure --prefix=/opt/sage` where `/opt/sage` is owned by root, it will fail because the `configure` script tries to create some directories under `$SAGE_LOCAL`.\n\n\nThis is addressed by #21532.",
    "created_at": "2018-03-05T23:15:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467228",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:11'></a>
Replying to [embray](#comment%3A10):
> Well, it looks like maybe there are some other problems with root installs.  For example, if you try to run something like `./configure --prefix=/opt/sage` where `/opt/sage` is owned by root, it will fail because the `configure` script tries to create some directories under `$SAGE_LOCAL`.


This is addressed by #21532.



---

archive/issue_comments_467229.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2018-04-05T10:24:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467229",
    "user": "https://github.com/embray"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_467230.json:
```json
{
    "body": "<a id='comment:13'></a>\nLatest patchbot shows one failure due to the banner issue (#25056) but otherwise ths should be good I think.  I think this approach really makes the most sense for the time being.",
    "created_at": "2018-04-10T16:26:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467230",
    "user": "https://github.com/embray"
}
```

<a id='comment:13'></a>
Latest patchbot shows one failure due to the banner issue (#25056) but otherwise ths should be good I think.  I think this approach really makes the most sense for the time being.



---

archive/issue_comments_467231.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Julian R\u00fcth\"",
    "created_at": "2018-04-10T22:49:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467231",
    "user": "https://github.com/saraedum"
}
```

Changing reviewer from "" to "Julian Rüth"



---

archive/issue_comments_467232.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-04-10T22:49:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467232",
    "user": "https://github.com/saraedum"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_062058.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24644#event-62058"
}
```



---

archive/issue_comments_467233.json:
```json
{
    "body": "Changing branch from \"u/embray/build/destdir-sudo\" to \"5a23d19e72159e4231e1b1a6391b7e9ec04cd1c6\"",
    "created_at": "2018-05-08T17:26:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467233",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/embray/build/destdir-sudo" to "5a23d19e72159e4231e1b1a6391b7e9ec04cd1c6"



---

archive/issue_events_062059.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-08T17:26:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/24644#event-62059"
}
```



---

archive/issue_comments_467234.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-05-08T17:26:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/24644#issuecomment-467234",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
