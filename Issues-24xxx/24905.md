...
```json
{
    "body": "`ninja_build` should be an order-only dependency (after the `|` symbol)",
    "created_at": "2018-04-06T12:14:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24905",
    "type": "issue_comment",
...
```json
{
    "body": "All other packages have `$(MP_LIBRARY)` in their dependencies instead of `$(INST)/$(SAGE_MP_LIBRARY)`. I guess this should be changed.",
...
```json
{
    "body": "This upgrade seems to break the whole polymake interface. Next step: Compare what's happening on the wire (pexpect log) between the two polymake versions.",
...
```json
{
    "body": "For the error with Slurp, merge #24152 (needs review) and then re-read the polymake SPKG.txt regarding the Perl libraries that need to be installed.",
...
```json
{
    "body": "Merged the dependency to #24152.\n\n---\nNew commits:",
...
```json
{
    "body": "I accidently erased the patch to compile with gcc7 form a previous ticket... I do not have gcc7 on my computer... But I guess it would be good to check it again anyways on this new release...",
...
```json
{
    "body": "It seems that the tests now pass.\n\nReady for review!",
...
```json
{
    "body": "`@`MKoeppe: could you eventually check that the above works on OSX machine?\n\nBest,",
...
```json
{
    "body": "This ticket with polymake 3.3 (before last push) compile on ubuntu 16.04.",
...
```json
{
    "body": "I tried to compile on macOS Mojave and it failed. I attached my log file.",
...
```json
{
    "body": "Upgrading to 3.4 now. Builds OK on Mac OS Mojave (see updated instructions for prerequisites!)\n\nMany test failures from \n`./sage -t --optional=sage,polymake src/sage/interfaces/polymake.py `",
...
```json
{
    "body": "polymake patch sent upstream.",
...
```json
{
    "body": "Attachment [polymake-3.4.log](tarball://root/attachments/some-uuid/ticket24905/polymake-3.4.log) by @mkoeppe created at 2019-04-27 08:30:29\n\nSee the updated Perl module prerequisites by typing \"sage -info polymake\".",
...
```json
{
    "body": "It compiled on Arch Linux x86_64 (5.0.9-arch1-1-ARCH). All relevant tests pass except for three.\nhttps://trac.sagemath.org/attachment/ticket/24905/polymake.log",
...
```json
{
    "body": "polymake 3.3 worked fine. I didn't check with polymake 3.4.",
...
```json
{
    "body": "When I run polymake in the sage subshell I get a compilation error upon running ` $p = rand_sphere<4,20>;`.",
...
```json
{
    "body": "Run a `sage distclean` and reinstall sage from scratch. Installing polymake also broke my sage somehow a some point. Reinstalling worked for me.",
...
```json
{
    "body": "I already did it. Now `openssl` isn't working again. It will take a while until I have a working version of sage on this machine again.\n\nReplying to [comment:107 jipilab]:\n> Run a `sage distclean` and reinstall sage from scratch. Installing polymake also broke my sage somehow a some point. Reinstalling worked for me.",
...
```json
{
    "body": "polymake 3.4 on macOS Mojave (built within sage, but also the official DMG from the polymake website) dies if one presses `^C` in quick succession during a computation such as the following:\n\n```\n  $p = cube(20);\n  $p->VOLUME;\n```",
...
```json
{
    "body": "I can't seem to reproduce a failure in the tests for `src/sage/rings/integer.pyx`.",
...
```json
{
    "body": "please provide a precise link to the tarball. The link in the description leads to source tarball, minimal tarball, etc...",
...
```json
{
    "body": "The wget command line has the URL.",
...
```json
{
    "body": "why does one need `libncurses-dev`? Sage provides ncurses, and as soon as #27277 is in, it should be able to use system's (n)curses.",
...
```json
{
    "body": "OK - the previous instruction didn't work if polymake was already installed. (Surely, the fact that it's an experimental package only makes it more complicated).",
...
```json
{
    "body": "some errors, please have a look at mis-posted #27763#comment:9\nand \nhttps://trac.sagemath.org/ticket/27763#comment:11",
...
```json
{
    "body": "Very strange. I don't think the order of these annotations makes a difference",
...
```json
{
    "body": "The doctesting framework is a pile of .... - so I'm not surprised that on different platforms it behaves differently. E.g. we see doctests passing with `--serial` options and mysteriously crashing without it.",
...
```json
{
    "body": "oops, sorry, I keep pushing into wrong branches these days! Please save your old branch...",
...
```json
{
    "body": "specifically, Matthias, could you force-push your last #24905#comment:133 ?",
...
```json
{
    "body": "On https://github.com/sagemath/sagetrac-mirror/tree/public/upgrade_polymake_to_version_3_2r4\n\nwe have everything till comment:101 - so I can recreate the thing from history here... <facepalm>",
...
```json
{
    "body": "OK, this looks like a correctly recovered branch to me.",
    "created_at": "2019-05-08T10:57:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24905",
    "type": "issue_comment",
...
```json
{
    "body": "On Debian I also had to install libterm-readkey-perl",
...
```json
{
    "body": "I restored the old branch and cherry-picked your \"update docs\" commit",
...
```json
{
    "body": "could you also mention libterm-readline-gnu-perl as a package to install on !Debian/Ubuntu?",
...
```json
{
    "body": "Perhaps it's time to change this package from \"experimental\" to \"optional\"?",
...
```json
{
    "body": "Building 72 more Perl packages now :-) Why is polymake itself not on CPAN?",
...
```json
{
    "body": "one last nitpick `yum install perl-Term-ReadLine-Gnu` should be added to Fedora instructions",
...
```json
{
    "body": "Follow-up ticket: Fix polymake 3.4 lrslib detection (#27803).",
...
```json
{
    "body": "It's known to the internet that !Fedora/Centos perform a surgery on its Perl installation resulting in breakages like in comment:163 (missing `/usr/share/perl5/ExtUtils/xsubpp`; of course `xsubpp` is present, it's only not in this locaton, hardcoded somewhere...)",
...
```json
{
    "body": "Too late, a previous version was released in 8.8.beta5. Move any new code to a separate ticket",
    "created_at": "2019-05-12T21:33:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/24905",
    "type": "issue_comment",
