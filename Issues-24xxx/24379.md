# Issue 24379: Matplotlib on Python 2 bugs out when there are non-ASCII characters in user's home directory

archive/issues_024142.json:
```json
{
    "assignees": [],
    "body": "The `matplotlib/__init__.py` module contains `from `__future__` import unicode_literals` which is often inadvisable, as it can run afoul of some of Python's own unicode bugs.\n\nIn particular at one point it calls `os.path.expanduser('~')`, where because of `unicode_literals` this becomes `os.path.expanduser(u'~')`.  This does not take care to decode the results of `os.environ['HOME']` (assuming it is ASCII), and ends up concatenating a unicode string with a non-unicode string, which means trying to first decoded the non-unicode string with the default (ASCII) encoding.\n\nThis is a bug in matplotlib but it affects Sage users, particularly on Windows where non-ASCII usernames are more common (may also affect OSX users): https://ask.sagemath.org/question/40131/sagemath-80-using-windows-installer\n\n**Upstream report:** https://github.com/matplotlib/matplotlib/issues/10017\n\nDepends on #25702\n\nCC:  @slel @sagetrac-epalezzato\n\nKeywords: **matplotlib, days94**\n\nUpstream: **Fixed upstream, in a later stable release.**\n\nReviewer: **Erik Bray**\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24379_\n\n",
    "closed_at": "2018-10-31T12:07:09Z",
    "created_at": "2017-12-15T10:20:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Matplotlib on Python 2 bugs out when there are non-ASCII characters in user's home directory",
    "type": "issue",
    "updated_at": "2018-11-01T13:43:05Z",
    "url": "https://github.com/sagemath/sage/issues/24379",
    "user": "https://github.com/embray"
}
```
The `matplotlib/__init__.py` module contains `from `__future__` import unicode_literals` which is often inadvisable, as it can run afoul of some of Python's own unicode bugs.

In particular at one point it calls `os.path.expanduser('~')`, where because of `unicode_literals` this becomes `os.path.expanduser(u'~')`.  This does not take care to decode the results of `os.environ['HOME']` (assuming it is ASCII), and ends up concatenating a unicode string with a non-unicode string, which means trying to first decoded the non-unicode string with the default (ASCII) encoding.

This is a bug in matplotlib but it affects Sage users, particularly on Windows where non-ASCII usernames are more common (may also affect OSX users): https://ask.sagemath.org/question/40131/sagemath-80-using-windows-installer

**Upstream report:** https://github.com/matplotlib/matplotlib/issues/10017

Depends on #25702

CC:  @slel @sagetrac-epalezzato

Keywords: **matplotlib, days94**

Upstream: **Fixed upstream, in a later stable release.**

Reviewer: **Erik Bray**

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/24379_





---

archive/issue_events_337987.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-15T10:20:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337987"
}
```



---

archive/issue_events_337988.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-15T10:20:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337988"
}
```



---

archive/issue_events_337989.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-15T10:20:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337989"
}
```



---

archive/issue_events_337990.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-15T10:20:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337990"
}
```



---

archive/issue_comments_369323.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nIt looks like it is still present in the latest release (2.1.0), for which I have been working on an upgrade at #23696.\n\n```\nfrom __future__ import (absolute_import, division, print_function,\n                        unicode_literals)\n```",
    "created_at": "2017-12-15T18:03:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369323",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:1" align="right">comment:1</div>

It looks like it is still present in the latest release (2.1.0), for which I have been working on an upgrade at #23696.

```
from __future__ import (absolute_import, division, print_function,
                        unicode_literals)
```



---

archive/issue_comments_369324.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,5 @@\n In particular at one point it calls `os.path.expanduser('~')`, where because of `unicode_literals` this becomes `os.path.expanduser(u'~')`.  This does not take care to decode the results of `os.environ['HOME']` (assuming it is ASCII), and ends up concatenating a unicode string with a non-unicode string, which means trying to first decoded the non-unicode string with the default (ASCII) encoding.\n \n This is a bug in matplotlib but it affects Sage users, particularly on Windows where non-ASCII usernames are more common (may also affect OSX users): https://ask.sagemath.org/question/40131/sagemath-80-using-windows-installer\n+\n+**Upstream report:** https://github.com/matplotlib/matplotlib/issues/10017\n``````\n",
    "created_at": "2017-12-19T10:42:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369324",
    "user": "https://github.com/embray"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,5 @@
 In particular at one point it calls `os.path.expanduser('~')`, where because of `unicode_literals` this becomes `os.path.expanduser(u'~')`.  This does not take care to decode the results of `os.environ['HOME']` (assuming it is ASCII), and ends up concatenating a unicode string with a non-unicode string, which means trying to first decoded the non-unicode string with the default (ASCII) encoding.
 
 This is a bug in matplotlib but it affects Sage users, particularly on Windows where non-ASCII usernames are more common (may also affect OSX users): https://ask.sagemath.org/question/40131/sagemath-80-using-windows-installer
+
+**Upstream report:** https://github.com/matplotlib/matplotlib/issues/10017
``````




---

archive/issue_comments_369325.json:
```json
{
    "body": "Changed upstream from **Not yet reported upstream; Will do shortly.** to **Reported upstream. No feedback yet.**",
    "created_at": "2017-12-19T10:42:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369325",
    "user": "https://github.com/embray"
}
```

Changed upstream from **Not yet reported upstream; Will do shortly.** to **Reported upstream. No feedback yet.**



---

archive/issue_events_337991.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337991"
}
```



---

archive/issue_events_337992.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337992"
}
```



---

archive/issue_comments_369326.json:
```json
{
    "body": "Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**",
    "created_at": "2018-06-29T14:12:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369326",
    "user": "https://github.com/sagetrac-epalezzato"
}
```

Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**



---

archive/issue_comments_369327.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis should be solved by upgrading to matplotlib 2.2.2 which has the upstream fix.",
    "created_at": "2018-06-29T14:12:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369327",
    "user": "https://github.com/sagetrac-epalezzato"
}
```

<div id="comment:4" align="right">comment:4</div>

This should be solved by upgrading to matplotlib 2.2.2 which has the upstream fix.



---

archive/issue_comments_369328.json:
```json
{
    "body": "Changed keywords from none to **matplotlib, days94**",
    "created_at": "2018-06-29T14:12:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369328",
    "user": "https://github.com/sagetrac-epalezzato"
}
```

Changed keywords from none to **matplotlib, days94**



---

archive/issue_comments_369329.json:
```json
{
    "body": "Dependencies: **#25702**",
    "created_at": "2018-06-29T14:12:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369329",
    "user": "https://github.com/sagetrac-epalezzato"
}
```

Dependencies: **#25702**



---

archive/issue_events_337993.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:56:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337993"
}
```



---

archive/issue_events_337994.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:56:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337994"
}
```



---

archive/issue_events_337995.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337995"
}
```



---

archive/issue_events_337996.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337996"
}
```



---

archive/issue_comments_369330.json:
```json
{
    "body": "Author: **Fran\u00e7ois Bissey**",
    "created_at": "2018-10-31T00:48:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369330",
    "user": "https://github.com/kiwifb"
}
```

Author: **François Bissey**



---

archive/issue_events_337997.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2018-10-31T00:48:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337997"
}
```



---

archive/issue_comments_369331.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nThis is arguably fixed by #25702 (upgrade to MPL 2.2.2 with the fix).",
    "created_at": "2018-10-31T00:48:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369331",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:7" align="right">comment:7</div>

This is arguably fixed by #25702 (upgrade to MPL 2.2.2 with the fix).



---

archive/issue_events_337998.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2018-10-31T00:48:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337998"
}
```



---

archive/issue_events_337999.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2018-10-31T00:48:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-337999"
}
```



---

archive/issue_events_338000.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2018-10-31T00:48:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-338000"
}
```



---

archive/issue_events_338001.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-31T12:07:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-338001"
}
```



---

archive/issue_events_338002.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-31T12:07:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-338002"
}
```



---

archive/issue_comments_369332.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nYep; IIRC this was fixed, thanks for the triaging.",
    "created_at": "2018-10-31T12:07:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369332",
    "user": "https://github.com/embray"
}
```

<div id="comment:8" align="right">comment:8</div>

Yep; IIRC this was fixed, thanks for the triaging.



---

archive/issue_events_338003.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-31T12:07:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24379#event-338003"
}
```



---

archive/issue_comments_369333.json:
```json
{
    "body": "Changed author from **Fran\u00e7ois Bissey** to none",
    "created_at": "2018-11-01T13:43:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369333",
    "user": "https://github.com/slel"
}
```

Changed author from **François Bissey** to none



---

archive/issue_comments_369334.json:
```json
{
    "body": "Reviewer: **Erik Bray**",
    "created_at": "2018-11-01T13:43:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24379#issuecomment-369334",
    "user": "https://github.com/slel"
}
```

Reviewer: **Erik Bray**
