# Issue 24303: warn not in misc

archive/issues_024066.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: G = Graphics()\nsage: G = line2d([(0,0), (1,1)])\nsage: G.legend(True)\nsage: G.show()\n...\n.../graphics.pyc in\n   2661             leg = subplot.legend(prop=prop, **lopts)\n   2662             if leg is None:\n-> 2663                 sage.misc.misc.warn(\"legend requested but no items are labeled\")\n   2664             else:\n   2665                 # color\nAttributeError: 'module' object has no attribute 'warn'\n```\n\nThis is the kind of things that should have been in a doctest!!\n\nKeywords: **bug, thursdaysbdx**\n\nReviewer: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24303_\n\n",
    "closed_at": "2020-08-14T13:10:09Z",
    "created_at": "2017-11-30T11:06:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "warn not in misc",
    "type": "issue",
    "updated_at": "2020-08-14T13:10:09Z",
    "url": "https://github.com/sagemath/sage/issues/24303",
    "user": "https://github.com/videlec"
}
```

```
sage: G = Graphics()
sage: G = line2d([(0,0), (1,1)])
sage: G.legend(True)
sage: G.show()
...
.../graphics.pyc in
   2661             leg = subplot.legend(prop=prop, **lopts)
   2662             if leg is None:
-> 2663                 sage.misc.misc.warn("legend requested but no items are labeled")
   2664             else:
   2665                 # color
AttributeError: 'module' object has no attribute 'warn'
```

This is the kind of things that should have been in a doctest!!

Keywords: **bug, thursdaysbdx**

Reviewer: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/24303_





---

archive/issue_events_319057.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-11-30T11:06:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319057"
}
```



---

archive/issue_events_319058.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-11-30T11:06:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319058"
}
```



---

archive/issue_events_319059.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-11-30T11:06:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319059"
}
```



---

archive/issue_events_319060.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-11-30T11:06:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319060"
}
```



---

archive/issue_events_319061.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:28:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319061"
}
```



---

archive/issue_comments_370586.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nFixed in #29895",
    "created_at": "2020-07-08T16:28:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24303#issuecomment-370586",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

Fixed in #29895



---

archive/issue_events_319062.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:28:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319062"
}
```



---

archive/issue_events_319063.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:28:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319063"
}
```



---

archive/issue_events_319064.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:28:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319064"
}
```



---

archive/issue_events_319065.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-13T20:29:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319065"
}
```



---

archive/issue_events_319066.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-13T20:29:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319066"
}
```



---

archive/issue_comments_370587.json:
```json
{
    "body": "Reviewer: **Markus Wageringel**",
    "created_at": "2020-08-13T20:29:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24303#issuecomment-370587",
    "user": "https://github.com/mwageringel"
}
```

Reviewer: **Markus Wageringel**



---

archive/issue_events_319067.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-14T13:10:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319067"
}
```



---

archive/issue_events_319068.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-14T13:10:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319068"
}
```



---

archive/issue_events_319069.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-14T13:10:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24303",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24303#event-319069"
}
```
