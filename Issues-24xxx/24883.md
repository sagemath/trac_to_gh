# Issue 24883: Endless symbolic computation

archive/issues_024646.json:
```json
{
    "assignees": [],
    "body": "I tried calculating \n\n```\n\na=e^(I*pi/4)+1\nb=1-e^(I*pi/4)\na*b\n\n```\n\nand both expressions a*b and a/b don't stop computing. \n\nI tried both on sage-8.1 for Windows and on sage-8.2.beta6 on Ubuntu (native Ubuntu desktop on Windows10).\n\nDepends on #24838\n\nCC:  @rwst @videlec\n\nKeywords: **days93, days94**\n\nBranch/Commit: **[4d0c51e](https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2)**\n\nReviewer: **Ralf Stephan, Travis Scrimshaw**\n\nAuthor: **Irene Pasquinelli**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24883_\n\n",
    "closed_at": "2018-08-06T15:13:09Z",
    "created_at": "2018-03-02T09:45:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Endless symbolic computation",
    "type": "issue",
    "updated_at": "2018-08-06T15:13:09Z",
    "url": "https://github.com/sagemath/sage/issues/24883",
    "user": "https://github.com/sagetrac-ipasquinelli"
}
```
I tried calculating 

```

a=e^(I*pi/4)+1
b=1-e^(I*pi/4)
a*b

```

and both expressions a*b and a/b don't stop computing. 

I tried both on sage-8.1 for Windows and on sage-8.2.beta6 on Ubuntu (native Ubuntu desktop on Windows10).

Depends on #24838

CC:  @rwst @videlec

Keywords: **days93, days94**

Branch/Commit: **[4d0c51e](https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2)**

Reviewer: **Ralf Stephan, Travis Scrimshaw**

Author: **Irene Pasquinelli**

_Issue created by migration from https://trac.sagemath.org/ticket/24883_





---

archive/issue_events_326232.json:
```json
{
    "actor": "https://github.com/sagetrac-ipasquinelli",
    "created_at": "2018-03-02T09:45:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326232"
}
```



---

archive/issue_events_326233.json:
```json
{
    "actor": "https://github.com/sagetrac-ipasquinelli",
    "created_at": "2018-03-02T09:45:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326233"
}
```



---

archive/issue_events_326234.json:
```json
{
    "actor": "https://github.com/sagetrac-ipasquinelli",
    "created_at": "2018-03-02T09:45:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326234"
}
```



---

archive/issue_events_326235.json:
```json
{
    "actor": "https://github.com/sagetrac-ipasquinelli",
    "created_at": "2018-03-02T09:45:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326235"
}
```



---

archive/issue_comments_381066.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nCould you add your architecture/sage version in the ticket description? I can confirm the behavior on archlinux with compiled sage-8.2.beta6.",
    "created_at": "2018-03-02T09:47:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381066",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

Could you add your architecture/sage version in the ticket description? I can confirm the behavior on archlinux with compiled sage-8.2.beta6.



---

archive/issue_comments_381067.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAlternative computation that terminates:\n\n```\nUCF = UniversalCyclotomicField()\na = UCF.zeta(8) + 1\nb = 1 - UCF.zeta(8)\na * b\n```",
    "created_at": "2018-03-02T09:48:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381067",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">Comment 2</div>

Alternative computation that terminates:

```
UCF = UniversalCyclotomicField()
a = UCF.zeta(8) + 1
b = 1 - UCF.zeta(8)
a * b
```



---

archive/issue_comments_381068.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,3 +9,5 @@\n ```\n \n and both expressions a*b and a/b don't stop computing. \n+\n+I tried both on sage-8.1 for Windows and on sage-8.2.beta6 on Ubuntu (native Ubuntu desktop on Windows10).\n``````\n",
    "created_at": "2018-03-02T09:59:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381068",
    "user": "https://github.com/sagetrac-ipasquinelli"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,3 +9,5 @@
 ```
 
 and both expressions a*b and a/b don't stop computing. 
+
+I tried both on sage-8.1 for Windows and on sage-8.2.beta6 on Ubuntu (native Ubuntu desktop on Windows10).
``````




---

archive/issue_comments_381069.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI can confirm it on archlinux with compiled sage-8.2.beta6, sage-8.2.beta5 and sage-8.1.\nHowever, with the sagemath 8.1-11 from the archlinux community repository it does work.\n\n\n```\nsage: a = 1 + e^(I*pi/4)\nsage: b = 1 - e^(I*pi/4)\nsage: a*b\n1/4*((I + 1)*sqrt(2) - 2)*(-(I + 1)*sqrt(2) - 2)\nsage: a/b\n-1/2*(-(I + 1)*sqrt(2) - 2)/(-(1/2*I + 1/2)*sqrt(2) + 1)\n\n```",
    "created_at": "2018-03-02T13:19:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381069",
    "user": "https://github.com/sagetrac-sdrewitz"
}
```

<div id="comment:4" align="right">Comment 4</div>

I can confirm it on archlinux with compiled sage-8.2.beta6, sage-8.2.beta5 and sage-8.1.
However, with the sagemath 8.1-11 from the archlinux community repository it does work.


```
sage: a = 1 + e^(I*pi/4)
sage: b = 1 - e^(I*pi/4)
sage: a*b
1/4*((I + 1)*sqrt(2) - 2)*(-(I + 1)*sqrt(2) - 2)
sage: a/b
-1/2*(-(I + 1)*sqrt(2) - 2)/(-(1/2*I + 1/2)*sqrt(2) + 1)

```



---

archive/issue_comments_381070.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nConfirmed in beta6. The loop is in Pynac. Thanks for the report.",
    "created_at": "2018-03-02T13:45:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381070",
    "user": "https://github.com/rwst"
}
```

<div id="comment:5" align="right">Comment 5</div>

Confirmed in beta6. The loop is in Pynac. Thanks for the report.



---

archive/issue_comments_381071.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nActually there were changes in pynac-0.7.17 that appear to have fixed it. With #24838:\n\n```\nsage:  sage: a = 1 + e^(I*pi/4)\n....:  sage: b = 1 - e^(I*pi/4)\n....:  sage: a*b\n....: \n1/4*((I + 1)*sqrt(2) - 2)*(-(I + 1)*sqrt(2) - 2)\n```\nWe might doctest this in this ticket, though.",
    "created_at": "2018-03-02T14:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381071",
    "user": "https://github.com/rwst"
}
```

<div id="comment:6" align="right">Comment 6</div>

Actually there were changes in pynac-0.7.17 that appear to have fixed it. With #24838:

```
sage:  sage: a = 1 + e^(I*pi/4)
....:  sage: b = 1 - e^(I*pi/4)
....:  sage: a*b
....: 
1/4*((I + 1)*sqrt(2) - 2)*(-(I + 1)*sqrt(2) - 2)
```
We might doctest this in this ticket, though.



---

archive/issue_comments_381072.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@rwst](#comment%3A6):\n> Actually there were changes in pynac-0.7.17 that appear to have fixed it. With #24838:\n\nCool\n\n> We might doctest this in this ticket, though.\n\n+1",
    "created_at": "2018-03-02T14:20:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381072",
    "user": "https://github.com/videlec"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@rwst](#comment%3A6):
> Actually there were changes in pynac-0.7.17 that appear to have fixed it. With #24838:

Cool

> We might doctest this in this ticket, though.

+1



---

archive/issue_comments_381073.json:
```json
{
    "body": "Author: **Irene Pasquinelli**",
    "created_at": "2018-03-02T14:23:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381073",
    "user": "https://github.com/videlec"
}
```

Author: **Irene Pasquinelli**



---

archive/issue_comments_381074.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nRalf, Irene is working on this ticket (she is learning how to develop). We will have a branch in a minute. Thanks for pointing #24838.",
    "created_at": "2018-03-02T14:23:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381074",
    "user": "https://github.com/videlec"
}
```

<div id="comment:8" align="right">Comment 8</div>

Ralf, Irene is working on this ticket (she is learning how to develop). We will have a branch in a minute. Thanks for pointing #24838.



---

archive/issue_comments_381075.json:
```json
{
    "body": "Branch: **[u/ipasquinelli/24883](https://github.com/sagemath/sagetrac-mirror/tree/u/ipasquinelli/24883)**",
    "created_at": "2018-03-02T15:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381075",
    "user": "https://github.com/sagetrac-ipasquinelli"
}
```

Branch: **[u/ipasquinelli/24883](https://github.com/sagemath/sagetrac-mirror/tree/u/ipasquinelli/24883)**



---

archive/issue_events_326236.json:
```json
{
    "actor": "https://github.com/sagetrac-ipasquinelli",
    "created_at": "2018-03-02T15:30:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326236"
}
```



---

archive/issue_comments_381076.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e470685fb618bb3df3e67d786127a930b2a28287\">e470685</a></td><td><code>version / chkum / rm patch</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d3511ce1cbd35b9d625b0f4215ee014e6cc99931\">d3511ce</a></td><td><code>24838: doctest fixes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/efe5f145b805b60d72a12eaf9c57a5109e0c786a\">efe5f14</a></td><td><code>24883: adding doctest for symbolic</code></td></tr></table>\n",
    "created_at": "2018-03-02T15:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381076",
    "user": "https://github.com/sagetrac-ipasquinelli"
}
```

<div id="comment:9" align="right">Comment 9</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e470685fb618bb3df3e67d786127a930b2a28287">e470685</a></td><td><code>version / chkum / rm patch</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d3511ce1cbd35b9d625b0f4215ee014e6cc99931">d3511ce</a></td><td><code>24838: doctest fixes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/efe5f145b805b60d72a12eaf9c57a5109e0c786a">efe5f14</a></td><td><code>24883: adding doctest for symbolic</code></td></tr></table>




---

archive/issue_comments_381077.json:
```json
{
    "body": "Commit: **[efe5f14](https://github.com/sagemath/sagetrac-mirror/commit/efe5f145b805b60d72a12eaf9c57a5109e0c786a)**",
    "created_at": "2018-03-02T15:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381077",
    "user": "https://github.com/sagetrac-ipasquinelli"
}
```

Commit: **[efe5f14](https://github.com/sagemath/sagetrac-mirror/commit/efe5f145b805b60d72a12eaf9c57a5109e0c786a)**



---

archive/issue_comments_381078.json:
```json
{
    "body": "Dependencies: **#24838**",
    "created_at": "2018-03-02T15:31:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381078",
    "user": "https://github.com/sagetrac-ipasquinelli"
}
```

Dependencies: **#24838**



---

archive/issue_comments_381079.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2018-03-03T08:42:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381079",
    "user": "https://github.com/rwst"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_comments_381080.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nI think this is fine, but we may have to wait for setting positive until #24838 gets positive.",
    "created_at": "2018-03-03T08:42:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381080",
    "user": "https://github.com/rwst"
}
```

<div id="comment:11" align="right">Comment 11</div>

I think this is fine, but we may have to wait for setting positive until #24838 gets positive.



---

archive/issue_comments_381081.json:
```json
{
    "body": "Changed branch from **[u/ipasquinelli/24883](https://github.com/sagemath/sagetrac-mirror/tree/u/ipasquinelli/24883)** to **[public/24883](https://github.com/sagemath/sagetrac-mirror/tree/public/24883)**",
    "created_at": "2018-07-01T06:14:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381081",
    "user": "https://github.com/rwst"
}
```

Changed branch from **[u/ipasquinelli/24883](https://github.com/sagemath/sagetrac-mirror/tree/u/ipasquinelli/24883)** to **[public/24883](https://github.com/sagemath/sagetrac-mirror/tree/public/24883)**



---

archive/issue_comments_381082.json:
```json
{
    "body": "Changed commit from **[efe5f14](https://github.com/sagemath/sagetrac-mirror/commit/efe5f145b805b60d72a12eaf9c57a5109e0c786a)** to **[699c0a2](https://github.com/sagemath/sagetrac-mirror/commit/699c0a2215c0b2472a70170b89947d28021deb05)**",
    "created_at": "2018-07-01T06:56:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381082",
    "user": "https://github.com/rwst"
}
```

Changed commit from **[efe5f14](https://github.com/sagemath/sagetrac-mirror/commit/efe5f145b805b60d72a12eaf9c57a5109e0c786a)** to **[699c0a2](https://github.com/sagemath/sagetrac-mirror/commit/699c0a2215c0b2472a70170b89947d28021deb05)**



---

archive/issue_comments_381083.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nOkay, the dependency #24838 is now in the develop branch. Your branch no longer merges because there were further changes in the dependency. Instead of merging them I created a fresh branch with only your commit (using `git cherry-pick`).\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/699c0a2215c0b2472a70170b89947d28021deb05\">699c0a2</a></td><td><code>24883: adding doctest for symbolic</code></td></tr></table>\n",
    "created_at": "2018-07-01T06:56:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381083",
    "user": "https://github.com/rwst"
}
```

<div id="comment:13" align="right">Comment 13</div>

Okay, the dependency #24838 is now in the develop branch. Your branch no longer merges because there were further changes in the dependency. Instead of merging them I created a fresh branch with only your commit (using `git cherry-pick`).

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/699c0a2215c0b2472a70170b89947d28021deb05">699c0a2</a></td><td><code>24883: adding doctest for symbolic</code></td></tr></table>




---

archive/issue_comments_381084.json:
```json
{
    "body": "Changed keywords from **days93** to **days93, days94**",
    "created_at": "2018-07-01T07:20:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381084",
    "user": "https://github.com/tscrim"
}
```

Changed keywords from **days93** to **days93, days94**



---

archive/issue_comments_381085.json:
```json
{
    "body": "Changed reviewer from **Ralf Stephan** to **Ralf Stephan, Travis Scrimshaw**",
    "created_at": "2018-07-01T07:20:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381085",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Ralf Stephan** to **Ralf Stephan, Travis Scrimshaw**



---

archive/issue_events_326237.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-01T07:20:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326237"
}
```



---

archive/issue_events_326238.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-01T07:20:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326238"
}
```



---

archive/issue_events_326239.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-01T07:20:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326239"
}
```



---

archive/issue_events_326240.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-01T07:20:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326240"
}
```



---

archive/issue_comments_381086.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nLGTM.",
    "created_at": "2018-07-01T07:20:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381086",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:14" align="right">Comment 14</div>

LGTM.



---

archive/issue_comments_381087.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nsee patchbot",
    "created_at": "2018-07-05T22:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381087",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:15" align="right">Comment 15</div>

see patchbot



---

archive/issue_events_326241.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-07-05T22:19:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326241"
}
```



---

archive/issue_events_326242.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-07-05T22:19:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326242"
}
```



---

archive/issue_comments_381088.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nProblem actually comes from an earlier doctest:\n\n```\nsage: e = x+1 <= x-2\n```\n(I tested this by copy/pasting the doctest).\nSo we probably need to use `exp`.",
    "created_at": "2018-07-05T23:09:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381088",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:16" align="right">Comment 16</div>

Problem actually comes from an earlier doctest:

```
sage: e = x+1 <= x-2
```
(I tested this by copy/pasting the doctest).
So we probably need to use `exp`.



---

archive/issue_comments_381089.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cb24f9a42dbddb869f40479b513e21c52d1200ae\">cb24f9a</a></td><td><code>24883: improve usage of ambigous symbol in doctest</code></td></tr></table>\n",
    "created_at": "2018-07-06T05:35:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381089",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:17" align="right">Comment 17</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cb24f9a42dbddb869f40479b513e21c52d1200ae">cb24f9a</a></td><td><code>24883: improve usage of ambigous symbol in doctest</code></td></tr></table>




---

archive/issue_comments_381090.json:
```json
{
    "body": "Changed commit from **[699c0a2](https://github.com/sagemath/sagetrac-mirror/commit/699c0a2215c0b2472a70170b89947d28021deb05)** to **[cb24f9a](https://github.com/sagemath/sagetrac-mirror/commit/cb24f9a42dbddb869f40479b513e21c52d1200ae)**",
    "created_at": "2018-07-06T05:35:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381090",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[699c0a2](https://github.com/sagemath/sagetrac-mirror/commit/699c0a2215c0b2472a70170b89947d28021deb05)** to **[cb24f9a](https://github.com/sagemath/sagetrac-mirror/commit/cb24f9a42dbddb869f40479b513e21c52d1200ae)**



---

archive/issue_events_326243.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-07-06T05:36:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326243"
}
```



---

archive/issue_events_326244.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-07-06T05:36:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326244"
}
```



---

archive/issue_comments_381091.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nupdate milestone 8.3 -> 8.4",
    "created_at": "2018-08-03T19:20:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381091",
    "user": "https://github.com/videlec"
}
```

<div id="comment:19" align="right">Comment 19</div>

update milestone 8.3 -> 8.4



---

archive/issue_events_326245.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326245"
}
```



---

archive/issue_events_326246.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326246"
}
```



---

archive/issue_comments_381092.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nForgot about this. LGTM.",
    "created_at": "2018-08-04T07:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381092",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:20" align="right">Comment 20</div>

Forgot about this. LGTM.



---

archive/issue_events_326247.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-08-04T07:43:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326247"
}
```



---

archive/issue_events_326248.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-08-04T07:43:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326248"
}
```



---

archive/issue_events_326249.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2018-08-04T11:01:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326249"
}
```



---

archive/issue_events_326250.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2018-08-04T11:01:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326250"
}
```



---

archive/issue_comments_381093.json:
```json
{
    "body": "Changed commit from **[cb24f9a](https://github.com/sagemath/sagetrac-mirror/commit/cb24f9a42dbddb869f40479b513e21c52d1200ae)** to **[4d0c51e](https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2)**",
    "created_at": "2018-08-04T11:01:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381093",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[cb24f9a](https://github.com/sagemath/sagetrac-mirror/commit/cb24f9a42dbddb869f40479b513e21c52d1200ae)** to **[4d0c51e](https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2)**



---

archive/issue_comments_381094.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2\">4d0c51e</a></td><td><code>Merge with sage-8.3</code></td></tr></table>\n",
    "created_at": "2018-08-04T11:01:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381094",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:21" align="right">Comment 21</div>

Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2">4d0c51e</a></td><td><code>Merge with sage-8.3</code></td></tr></table>




---

archive/issue_comments_381095.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nReplying to [@sagetrac-git](#comment%3A21):\n> Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2\">4d0c51e</a></td><td><code>Merge with sage-8.3</code></td></tr></table>\n\nI fixed a trivial merge conflict with 8.3.",
    "created_at": "2018-08-04T11:02:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381095",
    "user": "https://github.com/bryangingechen"
}
```

<div id="comment:22" align="right">Comment 22</div>

Replying to [@sagetrac-git](#comment%3A21):
> Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2">4d0c51e</a></td><td><code>Merge with sage-8.3</code></td></tr></table>

I fixed a trivial merge conflict with 8.3.



---

archive/issue_events_326251.json:
```json
{
    "actor": "https://github.com/bryangingechen",
    "created_at": "2018-08-04T11:02:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326251"
}
```



---

archive/issue_events_326252.json:
```json
{
    "actor": "https://github.com/bryangingechen",
    "created_at": "2018-08-04T11:02:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326252"
}
```



---

archive/issue_comments_381096.json:
```json
{
    "body": "Changed branch from **[public/24883](https://github.com/sagemath/sagetrac-mirror/tree/public/24883)** to **[4d0c51e](https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2)**",
    "created_at": "2018-08-06T15:13:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24883#issuecomment-381096",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/24883](https://github.com/sagemath/sagetrac-mirror/tree/public/24883)** to **[4d0c51e](https://github.com/sagemath/sagetrac-mirror/commit/4d0c51eab2f477499b6f7c64b5e7d179f8063cf2)**



---

archive/issue_events_326253.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-06T15:13:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326253"
}
```



---

archive/issue_events_326254.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4664730ffe85259c42ab2aec73b819e676e93522",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-08-06T15:13:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24883",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24883#event-326254"
}
```
