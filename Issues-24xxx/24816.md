# Issue 24816: Sums of products of arb balls

archive/issues_024579.json:
```json
{
    "assignees": [],
    "body": "This ticket adds a simple way to compute sums of products of real or complex balls from Python with reduced overhead. For example:\n\n```\nsage: Pol.<x> = RBF[]\nsage: foo = list((1 + x/3)^10)\nsage: %timeit RBF._sum_of_products((c,c,c) for c in foo)\n100000 loops, best of 3: 7.67 \u00b5s per loop\nsage: %timeit sum(c*c*c for c in foo)\nThe slowest run took 6.92 times longer than the fastest. This could mean that an intermediate result is being cached.\n100000 loops, best of 3: 14.9 \u00b5s per loop\n```\n(The new methods are _private for now, because we may want to change the interface later, especially if we generalize it to other parents.)\n\nBranch/Commit: **[953e73f](https://github.com/sagemath/sagetrac-mirror/commit/953e73fc9bd9d6addd74f1a0cbd4c12d82db507a)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Marc Mezzarobba**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24816_\n\n",
    "closed_at": "2018-03-22T19:22:51Z",
    "created_at": "2018-02-22T12:27:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Sums of products of arb balls",
    "type": "issue",
    "updated_at": "2018-03-22T19:22:51Z",
    "url": "https://github.com/sagemath/sage/issues/24816",
    "user": "https://github.com/mezzarobba"
}
```
This ticket adds a simple way to compute sums of products of real or complex balls from Python with reduced overhead. For example:

```
sage: Pol.<x> = RBF[]
sage: foo = list((1 + x/3)^10)
sage: %timeit RBF._sum_of_products((c,c,c) for c in foo)
100000 loops, best of 3: 7.67 µs per loop
sage: %timeit sum(c*c*c for c in foo)
The slowest run took 6.92 times longer than the fastest. This could mean that an intermediate result is being cached.
100000 loops, best of 3: 14.9 µs per loop
```
(The new methods are _private for now, because we may want to change the interface later, especially if we generalize it to other parents.)

Branch/Commit: **[953e73f](https://github.com/sagemath/sagetrac-mirror/commit/953e73fc9bd9d6addd74f1a0cbd4c12d82db507a)**

Reviewer: **Travis Scrimshaw**

Author: **Marc Mezzarobba**

_Issue created by migration from https://trac.sagemath.org/ticket/24816_





---

archive/issue_events_325462.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-02-22T12:27:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24816#event-325462"
}
```



---

archive/issue_events_325463.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-02-22T12:27:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24816#event-325463"
}
```



---

archive/issue_events_325464.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-02-22T12:27:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24816#event-325464"
}
```



---

archive/issue_events_325465.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-02-22T12:27:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24816#event-325465"
}
```



---

archive/issue_events_325466.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-02-22T12:28:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24816#event-325466"
}
```



---

archive/issue_comments_379960.json:
```json
{
    "body": "Changed commit from **[30d78fa](https://github.com/sagemath/sagetrac-mirror/commit/30d78faf98cfca8e6a3bc4b7da788525dd1aeb93)** to **[dcd4d1f](https://github.com/sagemath/sagetrac-mirror/commit/dcd4d1f56b10104bf0b6d929d086125bdfcf0884)**",
    "created_at": "2018-03-03T11:49:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24816#issuecomment-379960",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[30d78fa](https://github.com/sagemath/sagetrac-mirror/commit/30d78faf98cfca8e6a3bc4b7da788525dd1aeb93)** to **[dcd4d1f](https://github.com/sagemath/sagetrac-mirror/commit/dcd4d1f56b10104bf0b6d929d086125bdfcf0884)**



---

archive/issue_comments_379961.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dcd4d1f56b10104bf0b6d929d086125bdfcf0884\">dcd4d1f</a></td><td><code>sum of products of arb balls</code></td></tr></table>\n",
    "created_at": "2018-03-03T11:49:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24816#issuecomment-379961",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dcd4d1f56b10104bf0b6d929d086125bdfcf0884">dcd4d1f</a></td><td><code>sum of products of arb balls</code></td></tr></table>




---

archive/issue_comments_379962.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nrebased on develop",
    "created_at": "2018-03-03T11:50:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24816#issuecomment-379962",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:3'>Comment 3:</a>
rebased on develop



---

archive/issue_comments_379963.json:
```json
{
    "body": "Changed commit from **[dcd4d1f](https://github.com/sagemath/sagetrac-mirror/commit/dcd4d1f56b10104bf0b6d929d086125bdfcf0884)** to **[953e73f](https://github.com/sagemath/sagetrac-mirror/commit/953e73fc9bd9d6addd74f1a0cbd4c12d82db507a)**",
    "created_at": "2018-03-20T09:25:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24816#issuecomment-379963",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[dcd4d1f](https://github.com/sagemath/sagetrac-mirror/commit/dcd4d1f56b10104bf0b6d929d086125bdfcf0884)** to **[953e73f](https://github.com/sagemath/sagetrac-mirror/commit/953e73fc9bd9d6addd74f1a0cbd4c12d82db507a)**



---

archive/issue_comments_379964.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/953e73fc9bd9d6addd74f1a0cbd4c12d82db507a\">953e73f</a></td><td><code>sum of products of arb balls</code></td></tr></table>\n",
    "created_at": "2018-03-20T09:25:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24816#issuecomment-379964",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/953e73fc9bd9d6addd74f1a0cbd4c12d82db507a">953e73f</a></td><td><code>sum of products of arb balls</code></td></tr></table>




---

archive/issue_comments_379965.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLGTM.",
    "created_at": "2018-03-20T11:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24816#issuecomment-379965",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
LGTM.



---

archive/issue_comments_379966.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-03-20T11:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24816#issuecomment-379966",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_325467.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-03-20T11:51:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24816#event-325467"
}
```



---

archive/issue_events_325468.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-03-20T11:51:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24816#event-325468"
}
```



---

archive/issue_comments_379967.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThanks!",
    "created_at": "2018-03-20T12:23:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24816#issuecomment-379967",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:6'>Comment 6:</a>
Thanks!



---

archive/issue_events_325469.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-03-22T19:22:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24816#event-325469"
}
```



---

archive/issue_events_325470.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d81ba7d5b5ba390b06af472f879bd5b2308b7329",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-03-22T19:22:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24816#event-325470"
}
```



---

archive/issue_comments_379968.json:
```json
{
    "body": "Changed branch from **[u/mmezzarobba/arb_sum_of_products](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/arb_sum_of_products)** to **[953e73f](https://github.com/sagemath/sagetrac-mirror/commit/953e73fc9bd9d6addd74f1a0cbd4c12d82db507a)**",
    "created_at": "2018-03-22T19:22:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24816#issuecomment-379968",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmezzarobba/arb_sum_of_products](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/arb_sum_of_products)** to **[953e73f](https://github.com/sagemath/sagetrac-mirror/commit/953e73fc9bd9d6addd74f1a0cbd4c12d82db507a)**
