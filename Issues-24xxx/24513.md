# Issue 24513: difference_family should return trooleans when existence=True

archive/issues_024276.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: designs.difference_family(3,2,1)   # fine\n(Ring of integers modulo 3, [[1, 2]])\nsage: designs.difference_family(3,2,1, existence=True) # should be True\n(Ring of integers modulo 3, [[1, 2]])\n```\n\nCC:  @rwst\n\nKeywords: **thursdaysbdx**\n\nBranch/Commit: **[a6f0ded](https://github.com/sagemath/sagetrac-mirror/commit/a6f0dedfc146719fff8617d11340d38d43f64ccb)**\n\nReviewer: **Ralf Stephan**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24513_\n\n",
    "closed_at": "2018-01-15T22:29:15Z",
    "created_at": "2018-01-11T11:55:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "difference_family should return trooleans when existence=True",
    "type": "issue",
    "updated_at": "2018-01-15T22:29:15Z",
    "url": "https://github.com/sagemath/sage/issues/24513",
    "user": "https://github.com/videlec"
}
```

```
sage: designs.difference_family(3,2,1)   # fine
(Ring of integers modulo 3, [[1, 2]])
sage: designs.difference_family(3,2,1, existence=True) # should be True
(Ring of integers modulo 3, [[1, 2]])
```

CC:  @rwst

Keywords: **thursdaysbdx**

Branch/Commit: **[a6f0ded](https://github.com/sagemath/sagetrac-mirror/commit/a6f0dedfc146719fff8617d11340d38d43f64ccb)**

Reviewer: **Ralf Stephan**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/24513_





---

archive/issue_events_321525.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-11T11:55:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24513#event-321525"
}
```



---

archive/issue_events_321526.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-11T11:55:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24513#event-321526"
}
```



---

archive/issue_events_321527.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-11T11:55:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24513#event-321527"
}
```



---

archive/issue_events_321528.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-11T11:55:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24513#event-321528"
}
```



---

archive/issue_comments_373828.json:
```json
{
    "body": "Changed keywords from **** to **thursdaysbdx**",
    "created_at": "2018-01-11T11:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373828",
    "user": "https://github.com/videlec"
}
```

Changed keywords from **** to **thursdaysbdx**



---

archive/issue_comments_373829.json:
```json
{
    "body": "Author: **Vincent Delecroix**",
    "created_at": "2018-01-11T11:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373829",
    "user": "https://github.com/videlec"
}
```

Author: **Vincent Delecroix**



---

archive/issue_comments_373830.json:
```json
{
    "body": "Commit: **[2c39704](https://github.com/sagemath/sagetrac-mirror/commit/2c39704b45fc41cb3e4dc9355248fb9051810a79)**",
    "created_at": "2018-01-11T12:08:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373830",
    "user": "https://github.com/videlec"
}
```

Commit: **[2c39704](https://github.com/sagemath/sagetrac-mirror/commit/2c39704b45fc41cb3e4dc9355248fb9051810a79)**



---

archive/issue_comments_373831.json:
```json
{
    "body": "Branch: **[u/vdelecroix/24513](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/24513)**",
    "created_at": "2018-01-11T12:08:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373831",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/24513](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/24513)**



---

archive/issue_events_321529.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-01-11T12:08:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24513#event-321529"
}
```



---

archive/issue_comments_373832.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2c39704b45fc41cb3e4dc9355248fb9051810a79\">2c39704</a></td><td><code>24513: fix difference family</code></td></tr></table>\n",
    "created_at": "2018-01-11T12:08:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373832",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2c39704b45fc41cb3e4dc9355248fb9051810a79">2c39704</a></td><td><code>24513: fix difference family</code></td></tr></table>




---

archive/issue_comments_373833.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5fb997cdf00db8fdb9f535adbb0d317c7edab36e\">5fb997c</a></td><td><code>24513: fix difference family</code></td></tr></table>\n",
    "created_at": "2018-01-11T12:10:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373833",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5fb997cdf00db8fdb9f535adbb0d317c7edab36e">5fb997c</a></td><td><code>24513: fix difference family</code></td></tr></table>




---

archive/issue_comments_373834.json:
```json
{
    "body": "Changed commit from **[2c39704](https://github.com/sagemath/sagetrac-mirror/commit/2c39704b45fc41cb3e4dc9355248fb9051810a79)** to **[5fb997c](https://github.com/sagemath/sagetrac-mirror/commit/5fb997cdf00db8fdb9f535adbb0d317c7edab36e)**",
    "created_at": "2018-01-11T12:10:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373834",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2c39704](https://github.com/sagemath/sagetrac-mirror/commit/2c39704b45fc41cb3e4dc9355248fb9051810a79)** to **[5fb997c](https://github.com/sagemath/sagetrac-mirror/commit/5fb997cdf00db8fdb9f535adbb0d317c7edab36e)**



---

archive/issue_comments_373835.json:
```json
{
    "body": "Changed commit from **[5fb997c](https://github.com/sagemath/sagetrac-mirror/commit/5fb997cdf00db8fdb9f535adbb0d317c7edab36e)** to **[f1ed582](https://github.com/sagemath/sagetrac-mirror/commit/f1ed582e094eaa4f8a264f419f221958666cd2fa)**",
    "created_at": "2018-01-11T12:11:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373835",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[5fb997c](https://github.com/sagemath/sagetrac-mirror/commit/5fb997cdf00db8fdb9f535adbb0d317c7edab36e)** to **[f1ed582](https://github.com/sagemath/sagetrac-mirror/commit/f1ed582e094eaa4f8a264f419f221958666cd2fa)**



---

archive/issue_comments_373836.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f1ed582e094eaa4f8a264f419f221958666cd2fa\">f1ed582</a></td><td><code>24513: fix difference family</code></td></tr></table>\n",
    "created_at": "2018-01-11T12:11:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373836",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f1ed582e094eaa4f8a264f419f221958666cd2fa">f1ed582</a></td><td><code>24513: fix difference family</code></td></tr></table>




---

archive/issue_comments_373837.json:
```json
{
    "body": "Changed commit from **[f1ed582](https://github.com/sagemath/sagetrac-mirror/commit/f1ed582e094eaa4f8a264f419f221958666cd2fa)** to **[a6f0ded](https://github.com/sagemath/sagetrac-mirror/commit/a6f0dedfc146719fff8617d11340d38d43f64ccb)**",
    "created_at": "2018-01-11T12:11:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373837",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f1ed582](https://github.com/sagemath/sagetrac-mirror/commit/f1ed582e094eaa4f8a264f419f221958666cd2fa)** to **[a6f0ded](https://github.com/sagemath/sagetrac-mirror/commit/a6f0dedfc146719fff8617d11340d38d43f64ccb)**



---

archive/issue_comments_373838.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a6f0dedfc146719fff8617d11340d38d43f64ccb\">a6f0ded</a></td><td><code>24513: fix difference family</code></td></tr></table>\n",
    "created_at": "2018-01-11T12:11:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373838",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a6f0dedfc146719fff8617d11340d38d43f64ccb">a6f0ded</a></td><td><code>24513: fix difference family</code></td></tr></table>




---

archive/issue_comments_373839.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nLGTM. Patchbot fails are unrelated.",
    "created_at": "2018-01-11T14:58:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373839",
    "user": "https://github.com/rwst"
}
```

<div id="comment:6" align="right">Comment 6</div>

LGTM. Patchbot fails are unrelated.



---

archive/issue_comments_373840.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2018-01-11T14:58:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373840",
    "user": "https://github.com/rwst"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_events_321530.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-01-11T14:58:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24513#event-321530"
}
```



---

archive/issue_events_321531.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-01-11T14:58:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24513#event-321531"
}
```



---

archive/issue_comments_373841.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/24513](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/24513)** to **[a6f0ded](https://github.com/sagemath/sagetrac-mirror/commit/a6f0dedfc146719fff8617d11340d38d43f64ccb)**",
    "created_at": "2018-01-15T22:29:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24513#issuecomment-373841",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/24513](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/24513)** to **[a6f0ded](https://github.com/sagemath/sagetrac-mirror/commit/a6f0dedfc146719fff8617d11340d38d43f64ccb)**



---

archive/issue_events_321532.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-01-15T22:29:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24513#event-321532"
}
```



---

archive/issue_events_321533.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "21486182547657a464b8d62e12a2b48f2c86148d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-01-15T22:29:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24513",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24513#event-321533"
}
```
