# Issue 27527: Wrong import statement for SageObject in the doc

archive/issues_027290.json:
```json
{
    "assignees": [],
    "body": "In `src/doc/en/developer/coding_in_python.rst`, there is\n\n```\nfrom sage.ext.sage_object import SageObject\n```\n\nwhich is wrong and should be\n\n```\nfrom sage.structure.sage_object import SageObject\n```\n\nCC:  @sagetrac-tmonteil\n\nBranch/Commit: **[fe82bba](https://github.com/sagemath/sagetrac-mirror/commit/fe82bbadc9cb00ff4c6b0f19f982d16dc83b513a)**\n\nReviewer: **Thierry Monteil**\n\nAuthor: **Martin P\u00e9pin**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27527_\n\n",
    "closed_at": "2019-03-25T19:43:16Z",
    "created_at": "2019-03-21T18:04:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Wrong import statement for SageObject in the doc",
    "type": "issue",
    "updated_at": "2019-03-25T19:43:16Z",
    "url": "https://github.com/sagemath/sage/issues/27527",
    "user": "https://github.com/Kerl13"
}
```
In `src/doc/en/developer/coding_in_python.rst`, there is

```
from sage.ext.sage_object import SageObject
```

which is wrong and should be

```
from sage.structure.sage_object import SageObject
```

CC:  @sagetrac-tmonteil

Branch/Commit: **[fe82bba](https://github.com/sagemath/sagetrac-mirror/commit/fe82bbadc9cb00ff4c6b0f19f982d16dc83b513a)**

Reviewer: **Thierry Monteil**

Author: **Martin Pépin**

_Issue created by migration from https://trac.sagemath.org/ticket/27527_





---

archive/issue_events_359793.json:
```json
{
    "actor": "https://github.com/Kerl13",
    "created_at": "2019-03-21T18:04:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27527#event-359793"
}
```



---

archive/issue_events_359794.json:
```json
{
    "actor": "https://github.com/Kerl13",
    "created_at": "2019-03-21T18:04:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27527#event-359794"
}
```



---

archive/issue_events_359795.json:
```json
{
    "actor": "https://github.com/Kerl13",
    "created_at": "2019-03-21T18:04:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27527#event-359795"
}
```



---

archive/issue_events_359796.json:
```json
{
    "actor": "https://github.com/Kerl13",
    "created_at": "2019-03-21T18:04:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27527#event-359796"
}
```



---

archive/issue_comments_429276.json:
```json
{
    "body": "Branch: **[u/gh-Kerl13/wrong_import_statement_for_sageobject_in_the_doc](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Kerl13/wrong_import_statement_for_sageobject_in_the_doc)**",
    "created_at": "2019-03-21T18:11:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27527#issuecomment-429276",
    "user": "https://github.com/Kerl13"
}
```

Branch: **[u/gh-Kerl13/wrong_import_statement_for_sageobject_in_the_doc](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Kerl13/wrong_import_statement_for_sageobject_in_the_doc)**



---

archive/issue_comments_429277.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fe82bbadc9cb00ff4c6b0f19f982d16dc83b513a\">fe82bba</a></td><td><code>trac #27527: Fix import statement in the doc</code></td></tr></table>\n",
    "created_at": "2019-03-21T18:14:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27527#issuecomment-429277",
    "user": "https://github.com/Kerl13"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fe82bbadc9cb00ff4c6b0f19f982d16dc83b513a">fe82bba</a></td><td><code>trac #27527: Fix import statement in the doc</code></td></tr></table>




---

archive/issue_events_359797.json:
```json
{
    "actor": "https://github.com/Kerl13",
    "created_at": "2019-03-21T18:14:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27527#event-359797"
}
```



---

archive/issue_comments_429278.json:
```json
{
    "body": "Commit: **[fe82bba](https://github.com/sagemath/sagetrac-mirror/commit/fe82bbadc9cb00ff4c6b0f19f982d16dc83b513a)**",
    "created_at": "2019-03-21T18:14:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27527#issuecomment-429278",
    "user": "https://github.com/Kerl13"
}
```

Commit: **[fe82bba](https://github.com/sagemath/sagetrac-mirror/commit/fe82bbadc9cb00ff4c6b0f19f982d16dc83b513a)**



---

archive/issue_events_359798.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2019-03-21T18:15:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27527#event-359798"
}
```



---

archive/issue_events_359799.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2019-03-21T18:15:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27527#event-359799"
}
```



---

archive/issue_comments_429279.json:
```json
{
    "body": "Reviewer: **Thierry Monteil**",
    "created_at": "2019-03-21T18:15:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27527#issuecomment-429279",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Reviewer: **Thierry Monteil**



---

archive/issue_comments_429280.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2019-03-21T18:15:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27527#issuecomment-429280",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_events_359800.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-03-25T19:43:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27527#event-359800"
}
```



---

archive/issue_events_359801.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "290b8115c8bbc2587176199366e66ddf54ed1368",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-03-25T19:43:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27527#event-359801"
}
```



---

archive/issue_comments_429281.json:
```json
{
    "body": "Changed branch from **[u/gh-Kerl13/wrong_import_statement_for_sageobject_in_the_doc](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Kerl13/wrong_import_statement_for_sageobject_in_the_doc)** to **[fe82bba](https://github.com/sagemath/sagetrac-mirror/commit/fe82bbadc9cb00ff4c6b0f19f982d16dc83b513a)**",
    "created_at": "2019-03-25T19:43:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27527#issuecomment-429281",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-Kerl13/wrong_import_statement_for_sageobject_in_the_doc](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Kerl13/wrong_import_statement_for_sageobject_in_the_doc)** to **[fe82bba](https://github.com/sagemath/sagetrac-mirror/commit/fe82bbadc9cb00ff4c6b0f19f982d16dc83b513a)**
