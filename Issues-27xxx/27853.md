# Issue 27853: polymake: Install gcc if system gcc older than 5.1

archive/issues_027616.json:
```json
{
    "body": "\n\nCC:  @dimpase\n\nReviewer: Dima Pasechnik\n\nBranch: u/mkoeppe/polymake__install_gcc_if_system_gcc_older_than_5_1\n\nCommit: e675ad2aedb79a05fe6c9626019ea68505c73cca\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/27853\n\n",
    "closed_at": "2022-08-02T06:31:03Z",
    "created_at": "2019-05-20T03:09:42Z",
    "labels": [
        "component: packages: experimental"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "polymake: Install gcc if system gcc older than 5.1",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/27853",
    "user": "https://github.com/mkoeppe"
}
```


CC:  @dimpase

Reviewer: Dima Pasechnik

Branch: u/mkoeppe/polymake__install_gcc_if_system_gcc_older_than_5_1

Commit: e675ad2aedb79a05fe6c9626019ea68505c73cca

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/27853





---

archive/issue_comments_538584.json:
```json
{
    "body": "<a id='comment:1'></a>\nCan this be done with spkg-configure?",
    "created_at": "2019-05-20T03:29:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538584",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'></a>
Can this be done with spkg-configure?



---

archive/issue_comments_538585.json:
```json
{
    "body": "<a id='comment:2'></a>\nDo you mean the  spkg-configure of polymake?\n\nThis might be a bit tricky, I'd just implement a version check, which errors\nout with a message to install a newer gcc. You if needed might tweak spkg-configure of gcc to make it easier.\n\nAs well, very old gcc's (say, 4.4) cannot even compile Sage's gcc.\n\nAlso note that clang typically fakes a gcc, sometimes (on OSX) quite old one.",
    "created_at": "2019-05-20T06:25:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538585",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:2'></a>
Do you mean the  spkg-configure of polymake?

This might be a bit tricky, I'd just implement a version check, which errors
out with a message to install a newer gcc. You if needed might tweak spkg-configure of gcc to make it easier.

As well, very old gcc's (say, 4.4) cannot even compile Sage's gcc.

Also note that clang typically fakes a gcc, sometimes (on OSX) quite old one.



---

archive/issue_comments_538586.json:
```json
{
    "body": "<a id='comment:3'></a>\non the other hand, yes, in `polymake/spkg-config.m4` you can do something like \n\n```\nSAGE_SPKG_CONFIGURE([gcc], [\n    sage_spkg_install_polymake=yes\n    AC_REQUIRE([SAGE_SPKG_CONFIGURE_GMP])\n    if test $IS_REALLY_GCC = yes ; then\n       AS_CASE([\"$GXX_VERSION.0\"],\n           [[[0-4]].*|5.0.*], [\n               SAGE_MUST_INSTALL_GCC([you have $CXX version $GXX_VERSION, which is too old to build polymake])\n                ])\n    fi\n```\n\nbut I am not sure whether this would kick in regardless of the experimental status of polymake.\n\nIn fact I would be in favours of dropping gcc 4 support completely, and require at least 5.1 globally.",
    "created_at": "2019-05-20T11:51:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538586",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:3'></a>
on the other hand, yes, in `polymake/spkg-config.m4` you can do something like 

```
SAGE_SPKG_CONFIGURE([gcc], [
    sage_spkg_install_polymake=yes
    AC_REQUIRE([SAGE_SPKG_CONFIGURE_GMP])
    if test $IS_REALLY_GCC = yes ; then
       AS_CASE(["$GXX_VERSION.0"],
           [[[0-4]].*|5.0.*], [
               SAGE_MUST_INSTALL_GCC([you have $CXX version $GXX_VERSION, which is too old to build polymake])
                ])
    fi
```

but I am not sure whether this would kick in regardless of the experimental status of polymake.

In fact I would be in favours of dropping gcc 4 support completely, and require at least 5.1 globally.



---

archive/issue_comments_538587.json:
```json
{
    "body": "Changing branch from \"\" to \"u/mkoeppe/polymake__install_gcc_if_system_gcc_older_than_5_1\"",
    "created_at": "2019-05-20T13:42:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538587",
    "user": "https://github.com/mkoeppe"
}
```

Changing branch from "" to "u/mkoeppe/polymake__install_gcc_if_system_gcc_older_than_5_1"



---

archive/issue_comments_538588.json:
```json
{
    "body": "<a id='comment:5'></a>\nI tried this (see branch) but it seems that this script is always run, even if polymake is not requested. \n\n---\nNew commits:\n|                                                                                                                                          |                                                                 |\n|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------|\n|[e675ad2](https://git.sagemath.org/sage.git/commit?id=e675ad2aedb79a05fe6c9626019ea68505c73cca)|`If polymake is to be installed, install gcc if systen gcc < 5.1`|",
    "created_at": "2019-05-20T13:43:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538588",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>
I tried this (see branch) but it seems that this script is always run, even if polymake is not requested. 

---
New commits:
|                                                                                                                                          |                                                                 |
|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------|
|[e675ad2](https://git.sagemath.org/sage.git/commit?id=e675ad2aedb79a05fe6c9626019ea68505c73cca)|`If polymake is to be installed, install gcc if systen gcc < 5.1`|



---

archive/issue_comments_538589.json:
```json
{
    "body": "Changing commit from \"\" to \"e675ad2aedb79a05fe6c9626019ea68505c73cca\"",
    "created_at": "2019-05-20T13:43:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538589",
    "user": "https://github.com/mkoeppe"
}
```

Changing commit from "" to "e675ad2aedb79a05fe6c9626019ea68505c73cca"



---

archive/issue_comments_538590.json:
```json
{
    "body": "<a id='comment:6'></a>\nOne can have a `--with-polymake=install` (with default value `no`) switch tested in spkg-config, so that its contents is skipped, unless `install` is given as the value of this parameter. Except this would be a non-standard way to install a package. \n\n[Jeez, how much I hate `sage -i/-f` thing... :-)]",
    "created_at": "2019-05-20T16:24:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538590",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:6'></a>
One can have a `--with-polymake=install` (with default value `no`) switch tested in spkg-config, so that its contents is skipped, unless `install` is given as the value of this parameter. Except this would be a non-standard way to install a package. 

[Jeez, how much I hate `sage -i/-f` thing... :-)]



---

archive/issue_comments_538591.json:
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [dimpase](#comment%3A6):\n> One can have a `--with-polymake=install` (with default value `no`) switch tested in spkg-config, so that its contents is skipped, unless `install` is given as the value of this parameter. Except this would be a non-standard way to install a package. \n\n\nHardly an improvement.\n\n> [Jeez, how much I hate `sage -i/-f` thing... :-)]\n\n\nWhere's the ticket that will change \"sage -i PACKAGE\" to just reconfiguration with \"--enable-PACKAGE\" appended to the configure command line and \"make build\"?",
    "created_at": "2019-05-20T16:34:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538591",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'></a>
Replying to [dimpase](#comment%3A6):
> One can have a `--with-polymake=install` (with default value `no`) switch tested in spkg-config, so that its contents is skipped, unless `install` is given as the value of this parameter. Except this would be a non-standard way to install a package. 


Hardly an improvement.

> [Jeez, how much I hate `sage -i/-f` thing... :-)]


Where's the ticket that will change "sage -i PACKAGE" to just reconfiguration with "--enable-PACKAGE" appended to the configure command line and "make build"?



---

archive/issue_comments_538592.json:
```json
{
    "body": "<a id='comment:8'></a>\nYou can read on recent (mis)adventures in this domain on #27373, #27642...\n\nFeel free to open such a ticket though and cc me on it.",
    "created_at": "2019-05-20T16:46:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538592",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:8'></a>
You can read on recent (mis)adventures in this domain on #27373, #27642...

Feel free to open such a ticket though and cc me on it.



---

archive/issue_comments_538593.json:
```json
{
    "body": "<a id='comment:9'></a>\nhow about we re-target this ticket to bump up the minimal gcc version to 5.1?",
    "created_at": "2019-05-20T17:24:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538593",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:9'></a>
how about we re-target this ticket to bump up the minimal gcc version to 5.1?



---

archive/issue_comments_538594.json:
```json
{
    "body": "<a id='comment:10'></a>\nReplying to [dimpase](#comment%3A9):\n> how about we re-target this ticket to bump up the minimal gcc version to 5.1?\n\nIt's better if that goal is not associated with an experimental package.",
    "created_at": "2019-05-20T18:55:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538594",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'></a>
Replying to [dimpase](#comment%3A9):
> how about we re-target this ticket to bump up the minimal gcc version to 5.1?

It's better if that goal is not associated with an experimental package.



---

archive/issue_comments_538595.json:
```json
{
    "body": "<a id='comment:11'></a>\nAs the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).",
    "created_at": "2019-06-14T14:55:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538595",
    "user": "https://github.com/embray"
}
```

<a id='comment:11'></a>
As the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).



---

archive/issue_events_069039.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T18:16:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69039"
}
```



---

archive/issue_comments_538596.json:
```json
{
    "body": "<a id='comment:12'></a>\nMoving some tickets to 9.2. This is not a promise that I will be working on them.",
    "created_at": "2020-05-01T18:16:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538596",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:12'></a>
Moving some tickets to 9.2. This is not a promise that I will be working on them.



---

archive/issue_events_069040.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69040"
}
```



---

archive/issue_events_069041.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69041"
}
```



---

archive/issue_comments_538597.json:
```json
{
    "body": "<a id='comment:14'></a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538597",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:14'></a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_069042.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69042"
}
```



---

archive/issue_events_069043.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69043"
}
```



---

archive/issue_events_069044.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-14T01:38:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69044"
}
```



---

archive/issue_events_069045.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-14T01:38:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69045"
}
```



---

archive/issue_comments_538598.json:
```json
{
    "body": "<a id='comment:16'></a>\noutdated, should close",
    "created_at": "2022-07-27T05:33:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538598",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:16'></a>
outdated, should close



---

archive/issue_events_069046.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-07-27T05:33:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69046"
}
```



---

archive/issue_events_069047.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-07-27T05:33:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69047"
}
```



---

archive/issue_comments_538599.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2022-07-27T05:33:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538599",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_538600.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-07-27T08:20:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538600",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_538601.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Dima Pasechnik\"",
    "created_at": "2022-07-27T08:20:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538601",
    "user": "https://github.com/dimpase"
}
```

Changing reviewer from "" to "Dima Pasechnik"



---

archive/issue_comments_538602.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2022-08-02T06:31:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27853#issuecomment-538602",
    "user": "https://github.com/mkoeppe"
}
```

Resolution: invalid



---

archive/issue_events_069048.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-02T06:31:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/27853",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27853#event-69048"
}
```
