# Issue 27582: py3: fix bug in all_paths

archive/issues_027345.json:
```json
{
    "assignees": [],
    "body": "With #27501, we introduced an error in `all_paths` for Python 3. Indeed, `my_dict.keys()` returns a list in Python 2 but it is an iterator in Python 3, so the following code breaks as the size of the dictionary change during iteration.\n\n```\n                for u, v in my_dict.keys():\n                    my_dict[v, u] = my_dict[u, v]\n```\n\nWe fix this issue using `list(my_dict.keys())`. On the way, we also rename `my_dict` to `edge_labels`.\n\nCC:  @rajat1433\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[934c54d](https://github.com/sagemath/sagetrac-mirror/commit/934c54da862f86d3844b4a7a0f5830f21514a95b)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27582_\n\n",
    "closed_at": "2019-04-03T18:38:33Z",
    "created_at": "2019-03-31T17:38:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fix bug in all_paths",
    "type": "issue",
    "updated_at": "2019-04-03T18:38:33Z",
    "url": "https://github.com/sagemath/sage/issues/27582",
    "user": "https://github.com/dcoudert"
}
```
With #27501, we introduced an error in `all_paths` for Python 3. Indeed, `my_dict.keys()` returns a list in Python 2 but it is an iterator in Python 3, so the following code breaks as the size of the dictionary change during iteration.

```
                for u, v in my_dict.keys():
                    my_dict[v, u] = my_dict[u, v]
```

We fix this issue using `list(my_dict.keys())`. On the way, we also rename `my_dict` to `edge_labels`.

CC:  @rajat1433

Keywords: **py3, graph**

Branch/Commit: **[934c54d](https://github.com/sagemath/sagetrac-mirror/commit/934c54da862f86d3844b4a7a0f5830f21514a95b)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/27582_





---

archive/issue_events_360490.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-03-31T17:38:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27582#event-360490"
}
```



---

archive/issue_events_360491.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-03-31T17:38:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27582#event-360491"
}
```



---

archive/issue_events_360492.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-03-31T17:38:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27582#event-360492"
}
```



---

archive/issue_events_360493.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-03-31T17:38:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27582#event-360493"
}
```



---

archive/issue_comments_430278.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4324b87cdad0c79baba49d2565f1689f4b35595e\">4324b87</a></td><td><code>trac #27582: fix bug and rename my_dict as edge_labels</code></td></tr></table>\n",
    "created_at": "2019-03-31T17:40:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430278",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4324b87cdad0c79baba49d2565f1689f4b35595e">4324b87</a></td><td><code>trac #27582: fix bug and rename my_dict as edge_labels</code></td></tr></table>




---

archive/issue_events_360494.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-03-31T17:40:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27582#event-360494"
}
```



---

archive/issue_comments_430279.json:
```json
{
    "body": "Commit: **[4324b87](https://github.com/sagemath/sagetrac-mirror/commit/4324b87cdad0c79baba49d2565f1689f4b35595e)**",
    "created_at": "2019-03-31T17:40:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430279",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[4324b87](https://github.com/sagemath/sagetrac-mirror/commit/4324b87cdad0c79baba49d2565f1689f4b35595e)**



---

archive/issue_comments_430280.json:
```json
{
    "body": "Branch: **[public/graphs/27582_all_paths](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/27582_all_paths)**",
    "created_at": "2019-03-31T17:40:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430280",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/graphs/27582_all_paths](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/27582_all_paths)**



---

archive/issue_comments_430281.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nyou can also remove the `.keys()`",
    "created_at": "2019-03-31T18:34:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430281",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

you can also remove the `.keys()`



---

archive/issue_comments_430282.json:
```json
{
    "body": "Changed commit from **[4324b87](https://github.com/sagemath/sagetrac-mirror/commit/4324b87cdad0c79baba49d2565f1689f4b35595e)** to **[4483ba6](https://github.com/sagemath/sagetrac-mirror/commit/4483ba60722f020ea5b3111497dc02a489cbe271)**",
    "created_at": "2019-03-31T18:41:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430282",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4324b87](https://github.com/sagemath/sagetrac-mirror/commit/4324b87cdad0c79baba49d2565f1689f4b35595e)** to **[4483ba6](https://github.com/sagemath/sagetrac-mirror/commit/4483ba60722f020ea5b3111497dc02a489cbe271)**



---

archive/issue_comments_430283.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4483ba60722f020ea5b3111497dc02a489cbe271\">4483ba6</a></td><td><code>trac #27582: remove .keys()</code></td></tr></table>\n",
    "created_at": "2019-03-31T18:41:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430283",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4483ba60722f020ea5b3111497dc02a489cbe271">4483ba6</a></td><td><code>trac #27582: remove .keys()</code></td></tr></table>




---

archive/issue_comments_430284.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nRight, done.",
    "created_at": "2019-03-31T18:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430284",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:4" align="right">Comment 4</div>

Right, done.



---

archive/issue_comments_430285.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nsome `.keys()` are still there",
    "created_at": "2019-03-31T19:23:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430285",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

some `.keys()` are still there



---

archive/issue_comments_430286.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/934c54da862f86d3844b4a7a0f5830f21514a95b\">934c54d</a></td><td><code>trac #27582: remove .keys()</code></td></tr></table>\n",
    "created_at": "2019-03-31T19:53:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430286",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/934c54da862f86d3844b4a7a0f5830f21514a95b">934c54d</a></td><td><code>trac #27582: remove .keys()</code></td></tr></table>




---

archive/issue_comments_430287.json:
```json
{
    "body": "Changed commit from **[4483ba6](https://github.com/sagemath/sagetrac-mirror/commit/4483ba60722f020ea5b3111497dc02a489cbe271)** to **[934c54d](https://github.com/sagemath/sagetrac-mirror/commit/934c54da862f86d3844b4a7a0f5830f21514a95b)**",
    "created_at": "2019-03-31T19:53:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430287",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4483ba6](https://github.com/sagemath/sagetrac-mirror/commit/4483ba60722f020ea5b3111497dc02a489cbe271)** to **[934c54d](https://github.com/sagemath/sagetrac-mirror/commit/934c54da862f86d3844b4a7a0f5830f21514a95b)**



---

archive/issue_comments_430288.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nShould be OK now. Thanks.",
    "created_at": "2019-03-31T19:54:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430288",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:7" align="right">Comment 7</div>

Should be OK now. Thanks.



---

archive/issue_comments_430289.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\ngood to go if the patchbot is green",
    "created_at": "2019-03-31T20:11:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430289",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">Comment 8</div>

good to go if the patchbot is green



---

archive/issue_comments_430290.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-04-01T07:24:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430290",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_360495.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-04-01T07:24:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27582#event-360495"
}
```



---

archive/issue_events_360496.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-04-01T07:24:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27582#event-360496"
}
```



---

archive/issue_comments_430291.json:
```json
{
    "body": "Changed branch from **[public/graphs/27582_all_paths](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/27582_all_paths)** to **[934c54d](https://github.com/sagemath/sagetrac-mirror/commit/934c54da862f86d3844b4a7a0f5830f21514a95b)**",
    "created_at": "2019-04-03T18:38:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27582#issuecomment-430291",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/27582_all_paths](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/27582_all_paths)** to **[934c54d](https://github.com/sagemath/sagetrac-mirror/commit/934c54da862f86d3844b4a7a0f5830f21514a95b)**



---

archive/issue_events_360497.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-04-03T18:38:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27582#event-360497"
}
```



---

archive/issue_events_360498.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c45f8a6ec26099eedfae27185035430ba7e429de",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-04-03T18:38:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27582",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27582#event-360498"
}
```
