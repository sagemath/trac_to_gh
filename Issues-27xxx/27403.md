# Issue 27403: pplpy full build failure

archive/issues_027166.json:
```json
{
    "body": "Non-incremental sage builds apparently depend on the system sphinx-build, if any: \n\n```\n[pplpy-0.8.4] Copying package files from temporary location /var/lib/buildbot/slave/sage3_git/build/local/var/tmp/sage/build/pplpy-0.8.4/inst to /var/lib/buildbot/slave/sage3_git/build/local\n[pplpy-0.8.4] Running post-install script for pplpy-0.8.4.\n[pplpy-0.8.4] make[2]: Entering directory '/var/lib/buildbot/slave/sage3_git/build/local/var/tmp/sage/build/pplpy-0.8.4/src/docs'\n[pplpy-0.8.4] /bin/sh: 1: sphinx-build: not found\n[pplpy-0.8.4] Makefile:20: recipe for target 'html' failed\n[pplpy-0.8.4] make[2]: *** [html] Error 127\n[pplpy-0.8.4] make[2]: Leaving directory '/var/lib/buildbot/slave/sage3_git/build/local/var/tmp/sage/build/pplpy-0.8.4/src/docs'\n[pplpy-0.8.4] cp: cannot stat 'build/html/*': No such file or directory\n[pplpy-0.8.4] \n[pplpy-0.8.4] real\t0m0.011s\n[pplpy-0.8.4] user\t0m0.004s\n[pplpy-0.8.4] sys\t0m0.000s\n[pplpy-0.8.4] ************************************************************************\n[pplpy-0.8.4] Error running the postinst script for pplpy-0.8.4.\n[pplpy-0.8.4] ************************************************************************\n[pplpy-0.8.4] Please email sage-devel (http://groups.google.com/group/sage-devel)\n[pplpy-0.8.4] explaining the problem and including the log file\n[pplpy-0.8.4]   /var/lib/buildbot/slave/sage3_git/build/logs/pkgs/pplpy-0.8.4.log\n[pplpy-0.8.4] Describe your computer, operating system, etc.\n[pplpy-0.8.4] ************************************************************************\nMakefile:2156: recipe for target '/var/lib/buildbot/slave/sage3_git/build/local/var/lib/sage/installed/pplpy-0.8.4' failed\nmake[1]: *** [/var/lib/buildbot/slave/sage3_git/build/local/var/lib/sage/installed/pplpy-0.8.4] Error 1\nmake[1]: Leaving directory '/var/lib/buildbot/slave/sage3_git/build/build/make'\n```\n\nCC:  @videlec @vinklein\n\nBranch/Commit: 3c51fe3555cbf571673ad58dba4673e1c69a83c4\n\nReviewer: Volker Braun\n\nAuthor: Vincent Delecroix\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/27403\n\n",
    "closed_at": "2019-03-03T22:37:53Z",
    "created_at": "2019-03-03T11:53:54Z",
    "labels": [
        "component: packages: standard",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.7",
    "title": "pplpy full build failure",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/27403",
    "user": "https://github.com/vbraun"
}
```
Non-incremental sage builds apparently depend on the system sphinx-build, if any: 

```
[pplpy-0.8.4] Copying package files from temporary location /var/lib/buildbot/slave/sage3_git/build/local/var/tmp/sage/build/pplpy-0.8.4/inst to /var/lib/buildbot/slave/sage3_git/build/local
[pplpy-0.8.4] Running post-install script for pplpy-0.8.4.
[pplpy-0.8.4] make[2]: Entering directory '/var/lib/buildbot/slave/sage3_git/build/local/var/tmp/sage/build/pplpy-0.8.4/src/docs'
[pplpy-0.8.4] /bin/sh: 1: sphinx-build: not found
[pplpy-0.8.4] Makefile:20: recipe for target 'html' failed
[pplpy-0.8.4] make[2]: *** [html] Error 127
[pplpy-0.8.4] make[2]: Leaving directory '/var/lib/buildbot/slave/sage3_git/build/local/var/tmp/sage/build/pplpy-0.8.4/src/docs'
[pplpy-0.8.4] cp: cannot stat 'build/html/*': No such file or directory
[pplpy-0.8.4] 
[pplpy-0.8.4] real	0m0.011s
[pplpy-0.8.4] user	0m0.004s
[pplpy-0.8.4] sys	0m0.000s
[pplpy-0.8.4] ************************************************************************
[pplpy-0.8.4] Error running the postinst script for pplpy-0.8.4.
[pplpy-0.8.4] ************************************************************************
[pplpy-0.8.4] Please email sage-devel (http://groups.google.com/group/sage-devel)
[pplpy-0.8.4] explaining the problem and including the log file
[pplpy-0.8.4]   /var/lib/buildbot/slave/sage3_git/build/logs/pkgs/pplpy-0.8.4.log
[pplpy-0.8.4] Describe your computer, operating system, etc.
[pplpy-0.8.4] ************************************************************************
Makefile:2156: recipe for target '/var/lib/buildbot/slave/sage3_git/build/local/var/lib/sage/installed/pplpy-0.8.4' failed
make[1]: *** [/var/lib/buildbot/slave/sage3_git/build/local/var/lib/sage/installed/pplpy-0.8.4] Error 1
make[1]: Leaving directory '/var/lib/buildbot/slave/sage3_git/build/build/make'
```

CC:  @videlec @vinklein

Branch/Commit: 3c51fe3555cbf571673ad58dba4673e1c69a83c4

Reviewer: Volker Braun

Author: Vincent Delecroix

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/27403





---

archive/issue_comments_528269.json:
```json
{
    "body": "<a id='comment:1'></a>Indeed, `pplpy` depends on sphinx (for building the documentation) and this is not specified in `build/pkgs/pplpy/dependencies`.",
    "created_at": "2019-03-03T12:23:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528269",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'></a>Indeed, `pplpy` depends on sphinx (for building the documentation) and this is not specified in `build/pkgs/pplpy/dependencies`.



---

archive/issue_comments_528270.json:
```json
{
    "body": "<a id='comment:2'></a>Should we consider buildbots trying to build each individual package against its minimal set of dependencies?",
    "created_at": "2019-03-03T12:30:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528270",
    "user": "https://github.com/videlec"
}
```

<a id='comment:2'></a>Should we consider buildbots trying to build each individual package against its minimal set of dependencies?



---

archive/issue_comments_528271.json:
```json
{
    "body": "Changing commit from \"\" to \"5f2aa5e427977fd1d6ef1a5f2645adad66ef0833\"",
    "created_at": "2019-03-03T12:34:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528271",
    "user": "https://github.com/videlec"
}
```

Changing commit from "" to "5f2aa5e427977fd1d6ef1a5f2645adad66ef0833"



---

archive/issue_comments_528272.json:
```json
{
    "body": "<a id='comment:3'></a>New commits:",
    "created_at": "2019-03-03T12:34:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528272",
    "user": "https://github.com/videlec"
}
```

<a id='comment:3'></a>New commits:



---

archive/issue_comments_528273.json:
```json
{
    "body": "Changing author from \"\" to \"Vincent Delecroix\"",
    "created_at": "2019-03-03T12:34:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528273",
    "user": "https://github.com/videlec"
}
```

Changing author from "" to "Vincent Delecroix"



---

archive/issue_comments_528274.json:
```json
{
    "body": "Changing branch from \"\" to \"u/vdelecroix/27403\"",
    "created_at": "2019-03-03T12:34:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528274",
    "user": "https://github.com/videlec"
}
```

Changing branch from "" to "u/vdelecroix/27403"



---

archive/issue_comments_528275.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-03-03T12:34:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528275",
    "user": "https://github.com/videlec"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_528276.json:
```json
{
    "body": "<a id='comment:4'></a>(I tested building `pplpy` from scratch: `make base && make pplpy` and it succeeded)",
    "created_at": "2019-03-03T12:55:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528276",
    "user": "https://github.com/videlec"
}
```

<a id='comment:4'></a>(I tested building `pplpy` from scratch: `make base && make pplpy` and it succeeded)



---

archive/issue_comments_528277.json:
```json
{
    "body": "<a id='comment:5'></a>I get this problem, but also even with this branch, another problem:\n\n```\n    ppl/linear_algebra.cpp:8:17: warning: '/*' within block comment [-Wcomment]\n                \"ppl/*\",\n                    ^\n    ppl/linear_algebra.cpp:641:10: fatal error: 'ppl.hh' file not found\n    #include \"ppl.hh\"\n             ^~~~~~~~\n    1 warning and 1 error generated.\n    error: command 'gcc' failed with exit status 1\n```\nShould `ppl` be a dependency? It is listed as a requirement at the [pplpy gitlab page](https://gitlab.com/videlec/pplpy).",
    "created_at": "2019-03-03T18:06:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528277",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>I get this problem, but also even with this branch, another problem:

```
    ppl/linear_algebra.cpp:8:17: warning: '/*' within block comment [-Wcomment]
                "ppl/*",
                    ^
    ppl/linear_algebra.cpp:641:10: fatal error: 'ppl.hh' file not found
    #include "ppl.hh"
             ^~~~~~~~
    1 warning and 1 error generated.
    error: command 'gcc' failed with exit status 1
```
Should `ppl` be a dependency? It is listed as a requirement at the [pplpy gitlab page](https://gitlab.com/videlec/pplpy).



---

archive/issue_comments_528278.json:
```json
{
    "body": "<a id='comment:6'></a>It should. As I have ppl installed on my system I did not noticed it... (hence my comment [comment:2]).",
    "created_at": "2019-03-03T18:20:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528278",
    "user": "https://github.com/videlec"
}
```

<a id='comment:6'></a>It should. As I have ppl installed on my system I did not noticed it... (hence my comment [comment:2]).



---

archive/issue_comments_528279.json:
```json
{
    "body": "<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2019-03-03T18:21:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528279",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_528280.json:
```json
{
    "body": "Changing commit from \"5f2aa5e427977fd1d6ef1a5f2645adad66ef0833\" to \"3c51fe3555cbf571673ad58dba4673e1c69a83c4\"",
    "created_at": "2019-03-03T18:21:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528280",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "5f2aa5e427977fd1d6ef1a5f2645adad66ef0833" to "3c51fe3555cbf571673ad58dba4673e1c69a83c4"



---

archive/issue_comments_528281.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-03-03T18:48:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528281",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_528282.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Volker Braun\"",
    "created_at": "2019-03-03T18:48:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528282",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "" to "Volker Braun"



---

archive/issue_events_068061.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-03-03T22:37:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27403#event-68061"
}
```



---

archive/issue_comments_528283.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-03-03T22:37:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528283",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_528284.json:
```json
{
    "body": "Changing branch from \"u/vdelecroix/27403\" to \"3c51fe3555cbf571673ad58dba4673e1c69a83c4\"",
    "created_at": "2019-03-03T22:37:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27403",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27403#issuecomment-528284",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/vdelecroix/27403" to "3c51fe3555cbf571673ad58dba4673e1c69a83c4"
