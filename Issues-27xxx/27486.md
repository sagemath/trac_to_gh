# Issue 27486: Use sdh_cmake for packages that use cmake

archive/issues_027249.json:
```json
{
    "assignees": [],
    "body": "#27484 adds an `sdh_cmake` helper function for use by our handful of (all currently optional/experimental) packages.  We should try updating those packages to use the new helper:\n\n* cryptominisat\n* primecount (#27485)\n* qhull\n* scipoptsuite\n\nOf these, only scipoptsuite is somewhat unusual in that it does a sort of VPATH build in that it cds into a `build/` directory and then runs `cmake ..`.  This is incompatible with the current implementation of `sdh_cmake`.  So for that package either we don't use `sdh_cmake`, or forego the use of the `build/` directory, which may not be necessary anyways.\n\nCC:  @embray @jdemeyer\n\nBranch/Commit: **[1df46ef](https://github.com/sagemath/sagetrac-mirror/commit/1df46ef49bb561de096cb9cb09305085a3358e50)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27486_\n\n",
    "closed_at": "2019-06-02T22:04:18Z",
    "created_at": "2019-03-14T16:22:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Use sdh_cmake for packages that use cmake",
    "type": "issue",
    "updated_at": "2019-06-02T22:04:18Z",
    "url": "https://github.com/sagemath/sage/issues/27486",
    "user": "https://github.com/embray"
}
```
#27484 adds an `sdh_cmake` helper function for use by our handful of (all currently optional/experimental) packages.  We should try updating those packages to use the new helper:

* cryptominisat
* primecount (#27485)
* qhull
* scipoptsuite

Of these, only scipoptsuite is somewhat unusual in that it does a sort of VPATH build in that it cds into a `build/` directory and then runs `cmake ..`.  This is incompatible with the current implementation of `sdh_cmake`.  So for that package either we don't use `sdh_cmake`, or forego the use of the `build/` directory, which may not be necessary anyways.

CC:  @embray @jdemeyer

Branch/Commit: **[1df46ef](https://github.com/sagemath/sagetrac-mirror/commit/1df46ef49bb561de096cb9cb09305085a3358e50)**

Reviewer: **Dima Pasechnik**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/27486_





---

archive/issue_events_359254.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-14T16:22:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27486#event-359254"
}
```



---

archive/issue_events_359255.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-14T16:22:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27486#event-359255"
}
```



---

archive/issue_events_359256.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-14T16:22:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27486#event-359256"
}
```



---

archive/issue_comments_428360.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nneither qhull nor cryptominisat try to install anything in lib64 or so, thus it's really not that urgent to fix.",
    "created_at": "2019-03-19T11:42:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27486#issuecomment-428360",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:1" align="right">Comment 1</div>

neither qhull nor cryptominisat try to install anything in lib64 or so, thus it's really not that urgent to fix.



---

archive/issue_comments_428361.json:
```json
{
    "body": "Branch: **[u/chapoton/27486](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27486)**",
    "created_at": "2019-04-13T14:11:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27486#issuecomment-428361",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/27486](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27486)**



---

archive/issue_events_359257.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-04-13T14:11:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27486#event-359257"
}
```



---

archive/issue_comments_428362.json:
```json
{
    "body": "Commit: **[1df46ef](https://github.com/sagemath/sagetrac-mirror/commit/1df46ef49bb561de096cb9cb09305085a3358e50)**",
    "created_at": "2019-04-13T14:11:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27486#issuecomment-428362",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[1df46ef](https://github.com/sagemath/sagetrac-mirror/commit/1df46ef49bb561de096cb9cb09305085a3358e50)**



---

archive/issue_comments_428363.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1df46ef49bb561de096cb9cb09305085a3358e50\">1df46ef</a></td><td><code>using cmake for qhull and cryptominisat</code></td></tr></table>\n",
    "created_at": "2019-04-13T14:11:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27486#issuecomment-428363",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1df46ef49bb561de096cb9cb09305085a3358e50">1df46ef</a></td><td><code>using cmake for qhull and cryptominisat</code></td></tr></table>




---

archive/issue_comments_428364.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nplease have a look",
    "created_at": "2019-05-24T07:16:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27486#issuecomment-428364",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

please have a look



---

archive/issue_comments_428365.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-05-24T07:16:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27486#issuecomment-428365",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_428366.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nlooks good to me",
    "created_at": "2019-05-29T12:28:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27486#issuecomment-428366",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:4" align="right">Comment 4</div>

looks good to me



---

archive/issue_events_359258.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-29T12:28:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27486#event-359258"
}
```



---

archive/issue_events_359259.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-29T12:28:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27486#event-359259"
}
```



---

archive/issue_comments_428367.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2019-05-29T12:28:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27486#issuecomment-428367",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_428368.json:
```json
{
    "body": "Changed branch from **[u/chapoton/27486](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27486)** to **[1df46ef](https://github.com/sagemath/sagetrac-mirror/commit/1df46ef49bb561de096cb9cb09305085a3358e50)**",
    "created_at": "2019-06-02T22:04:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27486#issuecomment-428368",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/27486](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27486)** to **[1df46ef](https://github.com/sagemath/sagetrac-mirror/commit/1df46ef49bb561de096cb9cb09305085a3358e50)**



---

archive/issue_events_359260.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-06-02T22:04:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27486#event-359260"
}
```



---

archive/issue_events_359261.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "59df72bad038675a40578640a23fd9c037fc867b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-06-02T22:04:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27486",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27486#event-359261"
}
```
