# Issue 27128: avoid splitting the problem if ncpus=1 in dancing_links

archive/issues_026891.json:
```json
{
    "assignees": [],
    "body": "Because for some cases, it is more efficient to search serially without using parallel computations. It all depends on the instance.\n\nKeywords: **thursdaysbdx**\n\nBranch/Commit: **[f17229b](https://github.com/sagemath/sagetrac-mirror/commit/f17229b017183f5a656fc1d68e452bcec31093b0)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **S\u00e9bastien Labb\u00e9**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27128_\n\n",
    "closed_at": "2019-02-04T23:02:19Z",
    "created_at": "2019-01-25T15:39:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "avoid splitting the problem if ncpus=1 in dancing_links",
    "type": "issue",
    "updated_at": "2019-02-04T23:02:19Z",
    "url": "https://github.com/sagemath/sage/issues/27128",
    "user": "https://github.com/seblabbe"
}
```
Because for some cases, it is more efficient to search serially without using parallel computations. It all depends on the instance.

Keywords: **thursdaysbdx**

Branch/Commit: **[f17229b](https://github.com/sagemath/sagetrac-mirror/commit/f17229b017183f5a656fc1d68e452bcec31093b0)**

Reviewer: **Travis Scrimshaw**

Author: **Sébastien Labbé**

_Issue created by migration from https://trac.sagemath.org/ticket/27128_





---

archive/issue_events_354670.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2019-01-25T15:39:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27128#event-354670"
}
```



---

archive/issue_events_354671.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2019-01-25T15:39:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27128#event-354671"
}
```



---

archive/issue_events_354672.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2019-01-25T15:39:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27128#event-354672"
}
```



---

archive/issue_events_354673.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2019-01-25T15:39:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27128#event-354673"
}
```



---

archive/issue_comments_421520.json:
```json
{
    "body": "Branch: **[u/slabbe/27128](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/27128)**",
    "created_at": "2019-01-25T15:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27128#issuecomment-421520",
    "user": "https://github.com/seblabbe"
}
```

Branch: **[u/slabbe/27128](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/27128)**



---

archive/issue_comments_421521.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f17229b017183f5a656fc1d68e452bcec31093b0\">f17229b</a></td><td><code>27128: avoid splitting the problem if ncpus=1 in dancing_links</code></td></tr></table>\n",
    "created_at": "2019-01-25T15:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27128#issuecomment-421521",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f17229b017183f5a656fc1d68e452bcec31093b0">f17229b</a></td><td><code>27128: avoid splitting the problem if ncpus=1 in dancing_links</code></td></tr></table>




---

archive/issue_comments_421522.json:
```json
{
    "body": "Commit: **[f17229b](https://github.com/sagemath/sagetrac-mirror/commit/f17229b017183f5a656fc1d68e452bcec31093b0)**",
    "created_at": "2019-01-25T15:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27128#issuecomment-421522",
    "user": "https://github.com/seblabbe"
}
```

Commit: **[f17229b](https://github.com/sagemath/sagetrac-mirror/commit/f17229b017183f5a656fc1d68e452bcec31093b0)**



---

archive/issue_events_354674.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2019-01-25T15:40:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27128#event-354674"
}
```



---

archive/issue_comments_421523.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-01-25T19:46:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27128#issuecomment-421523",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_421524.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLGTM once you set the author field.",
    "created_at": "2019-01-25T19:46:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27128#issuecomment-421524",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
LGTM once you set the author field.



---

archive/issue_events_354675.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2019-01-27T20:12:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27128#event-354675"
}
```



---

archive/issue_events_354676.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2019-01-27T20:12:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27128#event-354676"
}
```



---

archive/issue_comments_421525.json:
```json
{
    "body": "Author: **S\u00e9bastien Labb\u00e9**",
    "created_at": "2019-01-27T20:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27128#issuecomment-421525",
    "user": "https://github.com/seblabbe"
}
```

Author: **Sébastien Labbé**



---

archive/issue_comments_421526.json:
```json
{
    "body": "Changed keywords from **** to **thursdaysbdx**",
    "created_at": "2019-01-27T20:13:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27128#issuecomment-421526",
    "user": "https://github.com/seblabbe"
}
```

Changed keywords from **** to **thursdaysbdx**



---

archive/issue_comments_421527.json:
```json
{
    "body": "Changed branch from **[u/slabbe/27128](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/27128)** to **[f17229b](https://github.com/sagemath/sagetrac-mirror/commit/f17229b017183f5a656fc1d68e452bcec31093b0)**",
    "created_at": "2019-02-04T23:02:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27128#issuecomment-421527",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/slabbe/27128](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/27128)** to **[f17229b](https://github.com/sagemath/sagetrac-mirror/commit/f17229b017183f5a656fc1d68e452bcec31093b0)**



---

archive/issue_events_354677.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-02-04T23:02:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27128#event-354677"
}
```



---

archive/issue_events_354678.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "25340496e6696929b583b22152e0ae90de3fedb3",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-02-04T23:02:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27128",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27128#event-354678"
}
```
