# Issue 27755: some fixes and pep cleanup in number fields orders

archive/issues_027518.json:
```json
{
    "body": "most important change:\n\nSome orders are built inside a smaller number field.\n\n**CC:**  @roed314 @tscrim @JohnCremona @jdemeyer\n\n**Branch/Commit:** [e053068b002e5b1a8eb56b624e507ce5de2db71d](https://github.com/sagemath/sagetrac-mirror/commit/e053068b002e5b1a8eb56b624e507ce5de2db71d)\n\n**Reviewer:** Travis Scrimshaw\n\n**Author:** Fr\u00e9d\u00e9ric Chapoton\n\nIssue created by migration from https://trac.sagemath.org/ticket/27755\n\n",
    "closed_at": "2019-05-12T21:30:16Z",
    "created_at": "2019-05-01T09:20:18Z",
    "labels": [
        "component: number fields",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "title": "some fixes and pep cleanup in number fields orders",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/27755",
    "user": "https://github.com/fchapoton"
}
```
most important change:

Some orders are built inside a smaller number field.

**CC:**  @roed314 @tscrim @JohnCremona @jdemeyer

**Branch/Commit:** [e053068b002e5b1a8eb56b624e507ce5de2db71d](https://github.com/sagemath/sagetrac-mirror/commit/e053068b002e5b1a8eb56b624e507ce5de2db71d)

**Reviewer:** Travis Scrimshaw

**Author:** Frédéric Chapoton

Issue created by migration from https://trac.sagemath.org/ticket/27755





---

archive/issue_comments_433362.json:
```json
{
    "body": "**Branch:** [u/chapoton/27755](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27755)",
    "created_at": "2019-05-01T09:21:15Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433362",
    "user": "https://github.com/fchapoton"
}
```

**Branch:** [u/chapoton/27755](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27755)



---

archive/issue_comments_433363.json:
```json
{
    "body": "<a id='comment:1'></a>\n**New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ab206f2d5eea080700b3a0cc3810e1d7103df058\">ab206f2</a></td><td><code>some fixes and pep cleanup in number fields orders</code></td></tr></table>\n",
    "created_at": "2019-05-01T09:21:15Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433363",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'></a>
**New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ab206f2d5eea080700b3a0cc3810e1d7103df058">ab206f2</a></td><td><code>some fixes and pep cleanup in number fields orders</code></td></tr></table>




---

archive/issue_comments_433364.json:
```json
{
    "body": "**Commit:** [ab206f2d5eea080700b3a0cc3810e1d7103df058](https://github.com/sagemath/sagetrac-mirror/commit/ab206f2d5eea080700b3a0cc3810e1d7103df058)",
    "created_at": "2019-05-01T09:21:15Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433364",
    "user": "https://github.com/fchapoton"
}
```

**Commit:** [ab206f2d5eea080700b3a0cc3810e1d7103df058](https://github.com/sagemath/sagetrac-mirror/commit/ab206f2d5eea080700b3a0cc3810e1d7103df058)



---

archive/issue_events_244927.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-05-01T09:21:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27755#event-244927"
}
```



---

archive/issue_comments_433365.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Branch pushed to git repo; I updated commit sha1.** **New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e053068b002e5b1a8eb56b624e507ce5de2db71d\">e053068</a></td><td><code>fixes for 27755</code></td></tr></table>\n",
    "created_at": "2019-05-01T09:25:41Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433365",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:2'></a>
**Branch pushed to git repo; I updated commit sha1.** **New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e053068b002e5b1a8eb56b624e507ce5de2db71d">e053068</a></td><td><code>fixes for 27755</code></td></tr></table>




---

archive/issue_comments_433366.json:
```json
{
    "body": "**Changing commit** from \"[ab206f2d5eea080700b3a0cc3810e1d7103df058](https://github.com/sagemath/sagetrac-mirror/commit/ab206f2d5eea080700b3a0cc3810e1d7103df058)\" to \"[e053068b002e5b1a8eb56b624e507ce5de2db71d](https://github.com/sagemath/sagetrac-mirror/commit/e053068b002e5b1a8eb56b624e507ce5de2db71d)\".",
    "created_at": "2019-05-01T09:25:41Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433366",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

**Changing commit** from "[ab206f2d5eea080700b3a0cc3810e1d7103df058](https://github.com/sagemath/sagetrac-mirror/commit/ab206f2d5eea080700b3a0cc3810e1d7103df058)" to "[e053068b002e5b1a8eb56b624e507ce5de2db71d](https://github.com/sagemath/sagetrac-mirror/commit/e053068b002e5b1a8eb56b624e507ce5de2db71d)".



---

archive/issue_comments_433367.json:
```json
{
    "body": "<a id='comment:3'></a>\ngreen bot, please review",
    "created_at": "2019-05-01T12:08:51Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433367",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'></a>
green bot, please review



---

archive/issue_comments_433368.json:
```json
{
    "body": "**Reviewer:** Travis Scrimshaw",
    "created_at": "2019-05-01T12:19:21Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433368",
    "user": "https://github.com/tscrim"
}
```

**Reviewer:** Travis Scrimshaw



---

archive/issue_comments_433369.json:
```json
{
    "body": "<a id='comment:4'></a>\nWhy did it change from `V` to `W` here?\n\n```diff\n-                z = V.random_element()\n-                alpha = from_V(z)\n+                alpha = from_V(W.random_element())\n```\nSo I can say every other change is good except for the behavior change noted in the ticket description, which is this change:\n\n```diff\n-            # We move each element of W to this subfield.\n-            c = alpha.coordinates_in_terms_of_powers()\n+            # We move each generator of W to this subfield.\n+            K, _ = K.subfield(alpha, 'beta')\n+            gens = [K(x) for x in gens]\n+            V, from_V, to_V = K.vector_space()\n+            mod_gens = [to_V(x) for x in gens]\n+            ambient = ZZ**V.dimension()\n+            W = ambient.span(mod_gens)\n```\nFor this, I would like someone who knows the theory to look at this change.",
    "created_at": "2019-05-01T12:19:21Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433369",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'></a>
Why did it change from `V` to `W` here?

```diff
-                z = V.random_element()
-                alpha = from_V(z)
+                alpha = from_V(W.random_element())
```
So I can say every other change is good except for the behavior change noted in the ticket description, which is this change:

```diff
-            # We move each element of W to this subfield.
-            c = alpha.coordinates_in_terms_of_powers()
+            # We move each generator of W to this subfield.
+            K, _ = K.subfield(alpha, 'beta')
+            gens = [K(x) for x in gens]
+            V, from_V, to_V = K.vector_space()
+            mod_gens = [to_V(x) for x in gens]
+            ambient = ZZ**V.dimension()
+            W = ambient.span(mod_gens)
```
For this, I would like someone who knows the theory to look at this change.



---

archive/issue_comments_433370.json:
```json
{
    "body": "<a id='comment:5'></a>\nWell, I agree that an expert's eye is welcome, but this is just linear algebra.\n\nThis part of the code was just unused and plain wrong.\n\nDo you know who we should put in cc ?",
    "created_at": "2019-05-01T12:21:26Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433370",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'></a>
Well, I agree that an expert's eye is welcome, but this is just linear algebra.

This part of the code was just unused and plain wrong.

Do you know who we should put in cc ?



---

archive/issue_comments_433371.json:
```json
{
    "body": "<a id='comment:6'></a>\nJohn, Jeroen, could one of you have a quick look at this to make sure the change mentioned in [comment:4](#comment%3A4) is okay (if you know about this part that is)?\n\n@fchapoton This is a little too far outside my area to say confidently enough that it was wrong and this is the fix. However, if it is unused, then maybe this discussion is moot since that code will never be called.",
    "created_at": "2019-05-01T12:28:48Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433371",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'></a>
John, Jeroen, could one of you have a quick look at this to make sure the change mentioned in [comment:4](#comment%3A4) is okay (if you know about this part that is)?

@fchapoton This is a little too far outside my area to say confidently enough that it was wrong and this is the fix. However, if it is unused, then maybe this discussion is moot since that code will never be called.



---

archive/issue_comments_433372.json:
```json
{
    "body": "<a id='comment:7'></a>\nWell, now it is being used by the doctest whose output changed. Previously, the option `allow_subfield=True` was never changing the field at all.",
    "created_at": "2019-05-01T12:31:29Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433372",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:7'></a>
Well, now it is being used by the doctest whose output changed. Previously, the option `allow_subfield=True` was never changing the field at all.



---

archive/issue_comments_433373.json:
```json
{
    "body": "<a id='comment:8'></a>\nReplying to [chapoton](#comment%3A7):\n> Well, now it is being used by the doctest whose output changed. Previously, the option `change_field=True` was never changing the field at all.\n\n\nI see. I was misinterpreting what you meant by \"unused\".",
    "created_at": "2019-05-01T12:36:19Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433373",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:8'></a>
Replying to [chapoton](#comment%3A7):
> Well, now it is being used by the doctest whose output changed. Previously, the option `change_field=True` was never changing the field at all.


I see. I was misinterpreting what you meant by "unused".



---

archive/issue_comments_433374.json:
```json
{
    "body": "<a id='comment:9'></a>\nThe changed code looks good to me but I have not tested it on examples.  The first change in Comment 4 is certainly necessary:  alpha must be an element whose coordinate vector lies in W as that is the span of the gens provided.  Otherwise it would be a random element of the big field K, and the whole point of this block of code is that the gens provided may lie in a proper subfield in which case the order returned is an order in that subfield.\n\nOne point caught my eye:  the coordinate vectors of the gens need not be vectors of integers (this is *not* the same as checking that the egns are algebraic integers since there is no reason to suppose that the vector space basis of K is an integral basis).  So W is defined to be the ZZ_span in ZZ^n of vectors which lie in QQ^n but not necessarily ZZ^n.  I hope that works as expected.",
    "created_at": "2019-05-01T15:09:12Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433374",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:9'></a>
The changed code looks good to me but I have not tested it on examples.  The first change in Comment 4 is certainly necessary:  alpha must be an element whose coordinate vector lies in W as that is the span of the gens provided.  Otherwise it would be a random element of the big field K, and the whole point of this block of code is that the gens provided may lie in a proper subfield in which case the order returned is an order in that subfield.

One point caught my eye:  the coordinate vectors of the gens need not be vectors of integers (this is *not* the same as checking that the egns are algebraic integers since there is no reason to suppose that the vector space basis of K is an integral basis).  So W is defined to be the ZZ_span in ZZ^n of vectors which lie in QQ^n but not necessarily ZZ^n.  I hope that works as expected.



---

archive/issue_comments_433375.json:
```json
{
    "body": "<a id='comment:10'></a>\nSeems to work fine:\n\n```\nsage: ambient=ZZ**2\nsage: ambient.span([vector(QQ,[1/2,1/2])])\nFree module of degree 2 and rank 1 over Integer Ring\nEchelon basis matrix:\n[1/2 1/2]\nsage: ambient.span([vector(QQ,[1/2,1/2]),vector(QQ,[1/2,-1/2])])\nFree module of degree 2 and rank 2 over Integer Ring\nEchelon basis matrix:\n[1/2 1/2]\n[  0   1]\n```",
    "created_at": "2019-05-01T18:33:08Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433375",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:10'></a>
Seems to work fine:

```
sage: ambient=ZZ**2
sage: ambient.span([vector(QQ,[1/2,1/2])])
Free module of degree 2 and rank 1 over Integer Ring
Echelon basis matrix:
[1/2 1/2]
sage: ambient.span([vector(QQ,[1/2,1/2]),vector(QQ,[1/2,-1/2])])
Free module of degree 2 and rank 2 over Integer Ring
Echelon basis matrix:
[1/2 1/2]
[  0   1]
```



---

archive/issue_comments_433376.json:
```json
{
    "body": "<a id='comment:11'></a>\nreview, please ?",
    "created_at": "2019-05-08T07:18:25Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433376",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:11'></a>
review, please ?



---

archive/issue_events_244928.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2019-05-08T07:54:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27755#event-244928"
}
```



---

archive/issue_events_244929.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2019-05-08T07:54:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27755#event-244929"
}
```



---

archive/issue_comments_433377.json:
```json
{
    "body": "**Changing branch** from \"[u/chapoton/27755](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27755)\" to \"[e053068b002e5b1a8eb56b624e507ce5de2db71d](https://github.com/sagemath/sagetrac-mirror/commit/e053068b002e5b1a8eb56b624e507ce5de2db71d)\".",
    "created_at": "2019-05-12T21:30:16Z",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27755#issuecomment-433377",
    "user": "https://github.com/vbraun"
}
```

**Changing branch** from "[u/chapoton/27755](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27755)" to "[e053068b002e5b1a8eb56b624e507ce5de2db71d](https://github.com/sagemath/sagetrac-mirror/commit/e053068b002e5b1a8eb56b624e507ce5de2db71d)".



---

archive/issue_events_244930.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-12T21:30:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27755#event-244930"
}
```



---

archive/issue_events_244931.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "41b531ebb0afa99ddcc0117d4a1b5c994b7cb8fb",
    "created_at": "2019-05-12T21:30:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27755",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27755#event-244931"
}
```
