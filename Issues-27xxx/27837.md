# Issue 27837: fixup: verbosier NonGrowthGroup short representations

archive/issues_027600.json:
```json
{
    "assignees": [],
    "body": "The short representation `S` for signs non-growth group is not self explanatory which might confuse some user. For roots of unity groups, we switch to `UU`. E.g.:\n\n```\n        sage: GrowthGroup('QQ^n * n^ZZ')\n        Growth Group QQ^n * n^ZZ * Signs^n\n        sage: GrowthGroup('(QQ_+)^n * n^ZZ * UU^n')\n        Growth Group QQ^n * n^ZZ * UU^n\n```\nThese changes also avoid the clash one has when using single letter variables.\n\nThis is a follow up for the sequence of tickets #26587, #26588, #26681, #26682, which were merged in a recent beta.\n\nCC:  @behackl\n\nBranch/Commit: **[b4ee24d](https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9)**\n\nReviewer: **Benjamin Hackl**\n\nAuthor: **Daniel Krenn**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27837_\n\n",
    "closed_at": "2019-05-24T18:29:37Z",
    "created_at": "2019-05-16T07:32:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fixup: verbosier NonGrowthGroup short representations",
    "type": "issue",
    "updated_at": "2019-05-24T18:29:37Z",
    "url": "https://github.com/sagemath/sage/issues/27837",
    "user": "https://github.com/dkrenn"
}
```
The short representation `S` for signs non-growth group is not self explanatory which might confuse some user. For roots of unity groups, we switch to `UU`. E.g.:

```
        sage: GrowthGroup('QQ^n * n^ZZ')
        Growth Group QQ^n * n^ZZ * Signs^n
        sage: GrowthGroup('(QQ_+)^n * n^ZZ * UU^n')
        Growth Group QQ^n * n^ZZ * UU^n
```
These changes also avoid the clash one has when using single letter variables.

This is a follow up for the sequence of tickets #26587, #26588, #26681, #26682, which were merged in a recent beta.

CC:  @behackl

Branch/Commit: **[b4ee24d](https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9)**

Reviewer: **Benjamin Hackl**

Author: **Daniel Krenn**

_Issue created by migration from https://trac.sagemath.org/ticket/27837_





---

archive/issue_events_363632.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-05-16T07:32:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27837#event-363632"
}
```



---

archive/issue_events_363633.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-05-16T07:32:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27837#event-363633"
}
```



---

archive/issue_events_363634.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-05-16T07:32:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27837#event-363634"
}
```



---

archive/issue_events_363635.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-05-16T07:32:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
    "label_color": "0000ff",
    "label_name": "component: asymptotic expansions",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27837#event-363635"
}
```



---

archive/issue_comments_435033.json:
```json
{
    "body": "Branch: **[u/dkrenn/fixup-nongrowthgroup-shortrepr](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fixup-nongrowthgroup-shortrepr)**",
    "created_at": "2019-05-16T07:38:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435033",
    "user": "https://github.com/dkrenn"
}
```

Branch: **[u/dkrenn/fixup-nongrowthgroup-shortrepr](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fixup-nongrowthgroup-shortrepr)**



---

archive/issue_events_363636.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-05-16T07:38:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27837#event-363636"
}
```



---

archive/issue_comments_435034.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/00c834081ccc8e642c4511c9683ad60a31ff9bfc\">00c8340</a></td><td><code>Trac #26504: allow internal rule in substitute</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9687881dea7f3db74d4b058d61ce338dfeead55c\">9687881</a></td><td><code>Trac #26504: add doctest</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/963cf921eabf04c454efeb54fbb42bd913e6e8d8\">963cf92</a></td><td><code>Merge tag '8.8.beta5' into t/26504/internal-subs</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/458baae2d21dca2f090f535f5386ad2cf9636b58\">458baae</a></td><td><code>Trac #26504: add doctest for _one_</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/95c4ed002e369af4122b231ef4f79f93c8d78b06\">95c4ed0</a></td><td><code>Trac #27837: change short repr</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d9cfb0a355feef480c366651ef4ca8b0d724a1ba\">d9cfb0a</a></td><td><code>Trac #27837: update doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4e958f1915ef12ec37473acae7ba39f58a62702c\">4e958f1</a></td><td><code>Trac #27837: fixup docstrings</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f6c875451cf9175d2170538d80c4533e830f1925\">f6c8754</a></td><td><code>Trac #27837: fix all doctests in sage.rings.asymptotic</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d291a6bcb539c9eee59e24c3c89463e82ffc01ff\">d291a6b</a></td><td><code>Trac #27837: make variable names more consistent</code></td></tr></table>\n",
    "created_at": "2019-05-16T07:38:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435034",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/00c834081ccc8e642c4511c9683ad60a31ff9bfc">00c8340</a></td><td><code>Trac #26504: allow internal rule in substitute</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9687881dea7f3db74d4b058d61ce338dfeead55c">9687881</a></td><td><code>Trac #26504: add doctest</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/963cf921eabf04c454efeb54fbb42bd913e6e8d8">963cf92</a></td><td><code>Merge tag '8.8.beta5' into t/26504/internal-subs</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/458baae2d21dca2f090f535f5386ad2cf9636b58">458baae</a></td><td><code>Trac #26504: add doctest for _one_</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/95c4ed002e369af4122b231ef4f79f93c8d78b06">95c4ed0</a></td><td><code>Trac #27837: change short repr</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d9cfb0a355feef480c366651ef4ca8b0d724a1ba">d9cfb0a</a></td><td><code>Trac #27837: update doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4e958f1915ef12ec37473acae7ba39f58a62702c">4e958f1</a></td><td><code>Trac #27837: fixup docstrings</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f6c875451cf9175d2170538d80c4533e830f1925">f6c8754</a></td><td><code>Trac #27837: fix all doctests in sage.rings.asymptotic</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d291a6bcb539c9eee59e24c3c89463e82ffc01ff">d291a6b</a></td><td><code>Trac #27837: make variable names more consistent</code></td></tr></table>




---

archive/issue_comments_435035.json:
```json
{
    "body": "Commit: **[d291a6b](https://github.com/sagemath/sagetrac-mirror/commit/d291a6bcb539c9eee59e24c3c89463e82ffc01ff)**",
    "created_at": "2019-05-16T07:38:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435035",
    "user": "https://github.com/dkrenn"
}
```

Commit: **[d291a6b](https://github.com/sagemath/sagetrac-mirror/commit/d291a6bcb539c9eee59e24c3c89463e82ffc01ff)**



---

archive/issue_comments_435036.json:
```json
{
    "body": "Changed commit from **[d291a6b](https://github.com/sagemath/sagetrac-mirror/commit/d291a6bcb539c9eee59e24c3c89463e82ffc01ff)** to **[9aad19c](https://github.com/sagemath/sagetrac-mirror/commit/9aad19cbe92846003111dde944122440cc963824)**",
    "created_at": "2019-05-16T07:47:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435036",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d291a6b](https://github.com/sagemath/sagetrac-mirror/commit/d291a6bcb539c9eee59e24c3c89463e82ffc01ff)** to **[9aad19c](https://github.com/sagemath/sagetrac-mirror/commit/9aad19cbe92846003111dde944122440cc963824)**



---

archive/issue_comments_435037.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/82f235d70612f2faca33661341b252472eb305b3\">82f235d</a></td><td><code>Trac #27837: change short repr</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/627ba1c5616333e409548a8ac32ab0c64c62f96b\">627ba1c</a></td><td><code>Trac #27837: update doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/26e84eb4f11569e586221f56ccfb212a71b4df50\">26e84eb</a></td><td><code>Trac #27837: fixup docstrings</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/687fee537b08fa4018b258748499f7fe413f5f59\">687fee5</a></td><td><code>Trac #27837: fix all doctests in sage.rings.asymptotic</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4ac605ed4f2be866962a3b31560f6a5602ab8b2d\">4ac605e</a></td><td><code>Trac #27837: make variable names more consistent</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9aad19cbe92846003111dde944122440cc963824\">9aad19c</a></td><td><code>Trac #27837: fix some more doctests</code></td></tr></table>\n",
    "created_at": "2019-05-16T07:47:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435037",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/82f235d70612f2faca33661341b252472eb305b3">82f235d</a></td><td><code>Trac #27837: change short repr</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/627ba1c5616333e409548a8ac32ab0c64c62f96b">627ba1c</a></td><td><code>Trac #27837: update doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/26e84eb4f11569e586221f56ccfb212a71b4df50">26e84eb</a></td><td><code>Trac #27837: fixup docstrings</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/687fee537b08fa4018b258748499f7fe413f5f59">687fee5</a></td><td><code>Trac #27837: fix all doctests in sage.rings.asymptotic</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4ac605ed4f2be866962a3b31560f6a5602ab8b2d">4ac605e</a></td><td><code>Trac #27837: make variable names more consistent</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9aad19cbe92846003111dde944122440cc963824">9aad19c</a></td><td><code>Trac #27837: fix some more doctests</code></td></tr></table>




---

archive/issue_comments_435038.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nNow really for \"needs review\" ;)",
    "created_at": "2019-05-16T07:47:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435038",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:5" align="right">Comment 5</div>

Now really for "needs review" ;)



---

archive/issue_comments_435039.json:
```json
{
    "body": "Changed commit from **[9aad19c](https://github.com/sagemath/sagetrac-mirror/commit/9aad19cbe92846003111dde944122440cc963824)** to **[b4ee24d](https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9)**",
    "created_at": "2019-05-16T09:22:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435039",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9aad19c](https://github.com/sagemath/sagetrac-mirror/commit/9aad19cbe92846003111dde944122440cc963824)** to **[b4ee24d](https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9)**



---

archive/issue_comments_435040.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9\">b4ee24d</a></td><td><code>Trac #27837: fixup doctest in sage.rings.big_oh</code></td></tr></table>\n",
    "created_at": "2019-05-16T09:22:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435040",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9">b4ee24d</a></td><td><code>Trac #27837: fixup doctest in sage.rings.big_oh</code></td></tr></table>




---

archive/issue_comments_435041.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nFixed one more doctest; lets see now ;)\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9\">b4ee24d</a></td><td><code>Trac #27837: fixup doctest in sage.rings.big_oh</code></td></tr></table>\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9\">b4ee24d</a></td><td><code>Trac #27837: fixup doctest in sage.rings.big_oh</code></td></tr></table>\n",
    "created_at": "2019-05-16T09:22:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435041",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:7" align="right">Comment 7</div>

Fixed one more doctest; lets see now ;)

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9">b4ee24d</a></td><td><code>Trac #27837: fixup doctest in sage.rings.big_oh</code></td></tr></table>

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9">b4ee24d</a></td><td><code>Trac #27837: fixup doctest in sage.rings.big_oh</code></td></tr></table>




---

archive/issue_comments_435042.json:
```json
{
    "body": "Reviewer: **Benjamin Hackl**",
    "created_at": "2019-05-16T09:33:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435042",
    "user": "https://github.com/behackl"
}
```

Reviewer: **Benjamin Hackl**



---

archive/issue_comments_435043.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nI reviewed the changes, everything looks good to me.\n\nWaiting for the output of the patchbot.",
    "created_at": "2019-05-16T09:33:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435043",
    "user": "https://github.com/behackl"
}
```

<div id="comment:8" align="right">Comment 8</div>

I reviewed the changes, everything looks good to me.

Waiting for the output of the patchbot.



---

archive/issue_events_363637.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2019-05-16T11:53:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27837#event-363637"
}
```



---

archive/issue_events_363638.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2019-05-16T11:53:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27837#event-363638"
}
```



---

archive/issue_events_363639.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-24T18:29:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27837#event-363639"
}
```



---

archive/issue_events_363640.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "0f7cf438cc49f08f507441e424cd6ede7724f456",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-05-24T18:29:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27837#event-363640"
}
```



---

archive/issue_comments_435044.json:
```json
{
    "body": "Changed branch from **[u/dkrenn/fixup-nongrowthgroup-shortrepr](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fixup-nongrowthgroup-shortrepr)** to **[b4ee24d](https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9)**",
    "created_at": "2019-05-24T18:29:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27837#issuecomment-435044",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dkrenn/fixup-nongrowthgroup-shortrepr](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fixup-nongrowthgroup-shortrepr)** to **[b4ee24d](https://github.com/sagemath/sagetrac-mirror/commit/b4ee24dd93484eb06e45432b2c2f70fe61aa98e9)**
