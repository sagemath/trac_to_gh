...
```json
{
    "body": "<a id='comment:1'></a>There seem to be places where B1 is actually overridden in the glue code.\n\nThe intended semantics of e.g. `ECM(1000).factor(n, 2000)` are not very clear to me.",
...
<a id='comment:1'></a>There seem to be places where B1 is actually overridden in the glue code.
...
<a id='comment:2'></a>another issue reported by Emmanuel:
...
<a id='comment:3'></a>another issue (still from Emmanuel):
...
<a id='comment:4'></a>those issues are addressed in the following patch:
...
<a id='comment:6'></a>note: another interface to GMP-ECM exists in sage.libs.libecm:
...
<a id='comment:7'></a>note that the old text interface is advertised from the `factor` command:
...
```json
{
    "body": "<a id='comment:11'></a>I made a git branch from your patch. I will also incorporate proper doctests.\n\n---\nNew commits:",
...
<a id='comment:11'></a>I made a git branch from your patch. I will also incorporate proper doctests.
...
```json
{
    "body": "<a id='comment:13'></a>For unifying the interfaces, it deserves a distinct ticket and I opened #29207.",
...
<a id='comment:13'></a>For unifying the interfaces, it deserves a distinct ticket and I opened #29207.
...
```json
{
    "body": "<a id='comment:15'></a>thank you Vincent!",
...
<a id='comment:15'></a>thank you Vincent!
