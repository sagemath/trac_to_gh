# Issue 27802: polymake interface: Convert Python dicts to Perl hashes

archive/issues_027565.json:
```json
{
    "assignees": [],
    "body": "This allows to use polyDB queries as follows:\n\n```\nsage: L = polymake.db_query({'\"_id\"': '\"F.4D.0047\"'}, db='\"LatticePolytopes\"', collection='\"SmoothReflexive\"')\nsage: L\nBigObjectArray\nsage: len(L)\n1\nsage: P = L[0]\nsage: P.list_properties()\nLATTICE_POINTS_GENERATORS, LINEALITY_SPACE, F_VECTOR, EHRHART_POLYNOMIAL, NORMAL, VERY_AMPLE, AFFINE_HULL, N_BOUNDARY_LATTICE_POINTS, H_STAR_VECTOR, LATTICE_CODEGREE, FACETS, N_LATTICE_POINTS, ESSENTIALLY_GENERIC, CONE_DIM, GORENSTEIN, FEASIBLE, VERTICES, CENTROID, LATTICE_VOLUME, FULL_DIM, CONE_AMBIENT_DIM, FACET_WIDTHS, SMOOTH, N_INTERIOR_LATTICE_POINTS, POINTED, LATTICE_DEGREE, N_FACETS, REFLEXIVE, N_VERTICES\nsage: P.F_VECTOR\n20 40 29 9\n```\n\nDepends on #22704\n\nCC:  @dimpase @jplab @simon-king-jena @kliem\n\nBranch/Commit: **[1958872](https://github.com/sagemath/sagetrac-mirror/commit/1958872872f420a8e7166f147db58b216b9f5f79)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27802_\n\n",
    "closed_at": "2019-05-21T12:15:38Z",
    "created_at": "2019-05-08T15:47:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "polymake interface: Convert Python dicts to Perl hashes",
    "type": "issue",
    "updated_at": "2019-05-21T12:15:38Z",
    "url": "https://github.com/sagemath/sage/issues/27802",
    "user": "https://github.com/mkoeppe"
}
```
This allows to use polyDB queries as follows:

```
sage: L = polymake.db_query({'"_id"': '"F.4D.0047"'}, db='"LatticePolytopes"', collection='"SmoothReflexive"')
sage: L
BigObjectArray
sage: len(L)
1
sage: P = L[0]
sage: P.list_properties()
LATTICE_POINTS_GENERATORS, LINEALITY_SPACE, F_VECTOR, EHRHART_POLYNOMIAL, NORMAL, VERY_AMPLE, AFFINE_HULL, N_BOUNDARY_LATTICE_POINTS, H_STAR_VECTOR, LATTICE_CODEGREE, FACETS, N_LATTICE_POINTS, ESSENTIALLY_GENERIC, CONE_DIM, GORENSTEIN, FEASIBLE, VERTICES, CENTROID, LATTICE_VOLUME, FULL_DIM, CONE_AMBIENT_DIM, FACET_WIDTHS, SMOOTH, N_INTERIOR_LATTICE_POINTS, POINTED, LATTICE_DEGREE, N_FACETS, REFLEXIVE, N_VERTICES
sage: P.F_VECTOR
20 40 29 9
```

Depends on #22704

CC:  @dimpase @jplab @simon-king-jena @kliem

Branch/Commit: **[1958872](https://github.com/sagemath/sagetrac-mirror/commit/1958872872f420a8e7166f147db58b216b9f5f79)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/27802_





---

archive/issue_events_363170.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-08T15:47:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363170"
}
```



---

archive/issue_events_363171.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-08T15:47:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363171"
}
```



---

archive/issue_events_363172.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-08T15:47:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363172"
}
```



---

archive/issue_events_363173.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-08T15:47:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363173"
}
```



---

archive/issue_comments_434342.json:
```json
{
    "body": "Branch: **[public/27802](https://github.com/sagemath/sagetrac-mirror/tree/public/27802)**",
    "created_at": "2019-05-08T15:52:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434342",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[public/27802](https://github.com/sagemath/sagetrac-mirror/tree/public/27802)**



---

archive/issue_events_363174.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-08T15:57:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363174"
}
```



---

archive/issue_comments_434343.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/666db96aad37cc2b5d3567eec49905a54a5f2f01\">666db96</a></td><td><code>Added two missing optional tags</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4e91dcc26a4c1be6ce6b298b572a9d748933a719\">4e91dcc</a></td><td><code>Change format of a 'not tested' doc test annotation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/05f3e612e8f32af5d453f2968eaab81fa4427912\">05f3e61</a></td><td><code>update docs, as discussed on #24905</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/53c8ee65d8803ffe4762d260bc173ae6763a7203\">53c8ee6</a></td><td><code>Mention libterm-readkey-perl</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b613e57983e061c8ed3413b1e6e7877608cc2d41\">b613e57</a></td><td><code>Mention libterm-readline-gnu-perl</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e5c3249a42a583a76c10268509ce295a34c8c8fc\">e5c3249</a></td><td><code>polymake: Mention MOngoDB for polydb</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b89376a1961208aff924359f4d2b6573c02c2be2\">b89376a</a></td><td><code>polymake: Update info on macOS</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0c1b044b7717f8cd3aed07a24e37f8f82246b1f9\">0c1b044</a></td><td><code>polymake: update MongoDB information for linux distros</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e46cd380e9645b0e7e6a26474aa2f69999f7cdc3\">e46cd38</a></td><td><code>Merge branch 't/24905/public/upgrade_polymake_to_version_3_2r4' into t/22704/public/22704</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/31070519fd113c0dab67adac8bd9713e9623507c\">3107051</a></td><td><code>polymake interface: Convert Python dicts to Perl hashes</code></td></tr></table>\n",
    "created_at": "2019-05-08T15:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434343",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/666db96aad37cc2b5d3567eec49905a54a5f2f01">666db96</a></td><td><code>Added two missing optional tags</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4e91dcc26a4c1be6ce6b298b572a9d748933a719">4e91dcc</a></td><td><code>Change format of a 'not tested' doc test annotation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/05f3e612e8f32af5d453f2968eaab81fa4427912">05f3e61</a></td><td><code>update docs, as discussed on #24905</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/53c8ee65d8803ffe4762d260bc173ae6763a7203">53c8ee6</a></td><td><code>Mention libterm-readkey-perl</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b613e57983e061c8ed3413b1e6e7877608cc2d41">b613e57</a></td><td><code>Mention libterm-readline-gnu-perl</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e5c3249a42a583a76c10268509ce295a34c8c8fc">e5c3249</a></td><td><code>polymake: Mention MOngoDB for polydb</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b89376a1961208aff924359f4d2b6573c02c2be2">b89376a</a></td><td><code>polymake: Update info on macOS</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0c1b044b7717f8cd3aed07a24e37f8f82246b1f9">0c1b044</a></td><td><code>polymake: update MongoDB information for linux distros</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e46cd380e9645b0e7e6a26474aa2f69999f7cdc3">e46cd38</a></td><td><code>Merge branch 't/24905/public/upgrade_polymake_to_version_3_2r4' into t/22704/public/22704</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/31070519fd113c0dab67adac8bd9713e9623507c">3107051</a></td><td><code>polymake interface: Convert Python dicts to Perl hashes</code></td></tr></table>




---

archive/issue_comments_434344.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2019-05-08T15:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434344",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_434345.json:
```json
{
    "body": "Commit: **[3107051](https://github.com/sagemath/sagetrac-mirror/commit/31070519fd113c0dab67adac8bd9713e9623507c)**",
    "created_at": "2019-05-08T15:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434345",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[3107051](https://github.com/sagemath/sagetrac-mirror/commit/31070519fd113c0dab67adac8bd9713e9623507c)**



---

archive/issue_comments_434346.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOK - modulo doctest errors mentioned on #22704.\n\nHow about a doctest/example as in the ticket description?",
    "created_at": "2019-05-08T21:44:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434346",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:4" align="right">Comment 4</div>

OK - modulo doctest errors mentioned on #22704.

How about a doctest/example as in the ticket description?



---

archive/issue_comments_434347.json:
```json
{
    "body": "Changed commit from **[3107051](https://github.com/sagemath/sagetrac-mirror/commit/31070519fd113c0dab67adac8bd9713e9623507c)** to **[d3bf6ec](https://github.com/sagemath/sagetrac-mirror/commit/d3bf6ec6e2587b13a3a8b419e3e6ef4a4795ceb9)**",
    "created_at": "2019-05-09T09:06:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434347",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3107051](https://github.com/sagemath/sagetrac-mirror/commit/31070519fd113c0dab67adac8bd9713e9623507c)** to **[d3bf6ec](https://github.com/sagemath/sagetrac-mirror/commit/d3bf6ec6e2587b13a3a8b419e3e6ef4a4795ceb9)**



---

archive/issue_comments_434348.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d3bf6ec6e2587b13a3a8b419e3e6ef4a4795ceb9\">d3bf6ec</a></td><td><code>polymake: Add db_query doctest</code></td></tr></table>\n",
    "created_at": "2019-05-09T09:06:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434348",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d3bf6ec6e2587b13a3a8b419e3e6ef4a4795ceb9">d3bf6ec</a></td><td><code>polymake: Add db_query doctest</code></td></tr></table>




---

archive/issue_comments_434349.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nOK!",
    "created_at": "2019-05-10T11:01:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434349",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:6" align="right">Comment 6</div>

OK!



---

archive/issue_comments_434350.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2019-05-10T11:01:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434350",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_363175.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-10T11:01:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363175"
}
```



---

archive/issue_events_363176.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-10T11:01:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363176"
}
```



---

archive/issue_events_363177.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-13T16:55:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363177"
}
```



---

archive/issue_events_363178.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-13T16:55:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363178"
}
```



---

archive/issue_comments_434351.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nSee patchbot",
    "created_at": "2019-05-13T16:55:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434351",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:7" align="right">Comment 7</div>

See patchbot



---

archive/issue_comments_434352.json:
```json
{
    "body": "Changed commit from **[d3bf6ec](https://github.com/sagemath/sagetrac-mirror/commit/d3bf6ec6e2587b13a3a8b419e3e6ef4a4795ceb9)** to **[7e1cc25](https://github.com/sagemath/sagetrac-mirror/commit/7e1cc2534e39ffb99ec3eb28cf2bb3d63c092149)**",
    "created_at": "2019-05-13T21:38:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434352",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d3bf6ec](https://github.com/sagemath/sagetrac-mirror/commit/d3bf6ec6e2587b13a3a8b419e3e6ef4a4795ceb9)** to **[7e1cc25](https://github.com/sagemath/sagetrac-mirror/commit/7e1cc2534e39ffb99ec3eb28cf2bb3d63c092149)**



---

archive/issue_comments_434353.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2eda6e11325d5d997f0de64e9d9f7add42fa7adb\">2eda6e1</a></td><td><code>polymake: add to fedora instructions</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/59161415289ce3256a260a8d798df7d95071658a\">5916141</a></td><td><code>Fix polymake lrslib detection</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/086bb2d3b48442f7f68e3f25baa80ce0376f9ce6\">086bb2d</a></td><td><code>Merge branch 't/27803/public/27803-polymake-lrslib-detection' into t/22704/public/22704</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1b479117463bb1a8d6372455f8723150313fc3ab\">1b47911</a></td><td><code>Configure lrslib with LRS_QUIET</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c84d89d552396a83638ee7a95de77c1a01c54e58\">c84d89d</a></td><td><code>Merge branch 't/27805/configure_lrslib_with_lrs_quiet' into t/22704/public/22704</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/58aa1ad052d7c2901b919cac9863d8d867d574c1\">58aa1ad</a></td><td><code>Mark issue of #27807 as known bug</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8d9e2c26686c4bbb0fac7d9b30fd60a11ef37446\">8d9e2c2</a></td><td><code>Merge tag '8.8.beta5' into t/22704/public/22704</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b6e11b25939a866d9fb672955b13ebc9b3049c93\">b6e11b2</a></td><td><code>Merge branch 't/22704/public/22704' into t/27802/public/27802</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ec06acde4935dc457febb215d0a59a3da10b14ad\">ec06acd</a></td><td><code>PolymakeJuPyMake: Fix _is_running, add missing tests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7e1cc2534e39ffb99ec3eb28cf2bb3d63c092149\">7e1cc25</a></td><td><code>polymake.py: Remove unused imports (pyflakes)</code></td></tr></table>\n",
    "created_at": "2019-05-13T21:38:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434353",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2eda6e11325d5d997f0de64e9d9f7add42fa7adb">2eda6e1</a></td><td><code>polymake: add to fedora instructions</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/59161415289ce3256a260a8d798df7d95071658a">5916141</a></td><td><code>Fix polymake lrslib detection</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/086bb2d3b48442f7f68e3f25baa80ce0376f9ce6">086bb2d</a></td><td><code>Merge branch 't/27803/public/27803-polymake-lrslib-detection' into t/22704/public/22704</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1b479117463bb1a8d6372455f8723150313fc3ab">1b47911</a></td><td><code>Configure lrslib with LRS_QUIET</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c84d89d552396a83638ee7a95de77c1a01c54e58">c84d89d</a></td><td><code>Merge branch 't/27805/configure_lrslib_with_lrs_quiet' into t/22704/public/22704</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/58aa1ad052d7c2901b919cac9863d8d867d574c1">58aa1ad</a></td><td><code>Mark issue of #27807 as known bug</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8d9e2c26686c4bbb0fac7d9b30fd60a11ef37446">8d9e2c2</a></td><td><code>Merge tag '8.8.beta5' into t/22704/public/22704</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b6e11b25939a866d9fb672955b13ebc9b3049c93">b6e11b2</a></td><td><code>Merge branch 't/22704/public/22704' into t/27802/public/27802</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ec06acde4935dc457febb215d0a59a3da10b14ad">ec06acd</a></td><td><code>PolymakeJuPyMake: Fix _is_running, add missing tests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7e1cc2534e39ffb99ec3eb28cf2bb3d63c092149">7e1cc25</a></td><td><code>polymake.py: Remove unused imports (pyflakes)</code></td></tr></table>




---

archive/issue_events_363179.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-13T21:38:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363179"
}
```



---

archive/issue_events_363180.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-13T21:38:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363180"
}
```



---

archive/issue_comments_434354.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nThe `oldstyle_print` patchbot errors are false positives (Perl syntax).\n\nI have fixed the other errors. The new doctests revealed a mistake in the implementation of `_is_running`, fixed.\n\nNeeds review again.",
    "created_at": "2019-05-13T21:47:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434354",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:10" align="right">Comment 10</div>

The `oldstyle_print` patchbot errors are false positives (Perl syntax).

I have fixed the other errors. The new doctests revealed a mistake in the implementation of `_is_running`, fixed.

Needs review again.



---

archive/issue_comments_434355.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nlooks OK",
    "created_at": "2019-05-14T10:46:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434355",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:11" align="right">Comment 11</div>

looks OK



---

archive/issue_events_363181.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-14T10:46:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363181"
}
```



---

archive/issue_events_363182.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-14T10:46:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363182"
}
```



---

archive/issue_events_363183.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-05-14T22:55:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363183"
}
```



---

archive/issue_events_363184.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-05-14T22:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363184"
}
```



---

archive/issue_comments_434356.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nTests for `_coerce_impl` need to be tagged optional for polymake. It is interesting that stuff that involves polymake directly was forgotten but not the jupymake ones.",
    "created_at": "2019-05-14T22:55:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434356",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:12" align="right">Comment 12</div>

Tests for `_coerce_impl` need to be tagged optional for polymake. It is interesting that stuff that involves polymake directly was forgotten but not the jupymake ones.



---

archive/issue_comments_434357.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1958872872f420a8e7166f147db58b216b9f5f79\">1958872</a></td><td><code>Add optional - polymake</code></td></tr></table>\n",
    "created_at": "2019-05-15T09:42:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434357",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:13" align="right">Comment 13</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1958872872f420a8e7166f147db58b216b9f5f79">1958872</a></td><td><code>Add optional - polymake</code></td></tr></table>




---

archive/issue_comments_434358.json:
```json
{
    "body": "Changed commit from **[7e1cc25](https://github.com/sagemath/sagetrac-mirror/commit/7e1cc2534e39ffb99ec3eb28cf2bb3d63c092149)** to **[1958872](https://github.com/sagemath/sagetrac-mirror/commit/1958872872f420a8e7166f147db58b216b9f5f79)**",
    "created_at": "2019-05-15T09:42:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434358",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7e1cc25](https://github.com/sagemath/sagetrac-mirror/commit/7e1cc2534e39ffb99ec3eb28cf2bb3d63c092149)** to **[1958872](https://github.com/sagemath/sagetrac-mirror/commit/1958872872f420a8e7166f147db58b216b9f5f79)**



---

archive/issue_events_363185.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-15T09:48:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363185"
}
```



---

archive/issue_events_363186.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-15T09:48:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363186"
}
```



---

archive/issue_events_363187.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-05-15T10:08:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363187"
}
```



---

archive/issue_events_363188.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-05-15T10:08:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363188"
}
```



---

archive/issue_comments_434359.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nLGTM",
    "created_at": "2019-05-15T10:08:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434359",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:15" align="right">Comment 15</div>

LGTM



---

archive/issue_events_363189.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-21T12:15:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363189"
}
```



---

archive/issue_events_363190.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "992d0a6d280e7dc2fae3f5f83970f23e343b3978",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-05-21T12:15:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27802#event-363190"
}
```



---

archive/issue_comments_434360.json:
```json
{
    "body": "Changed branch from **[public/27802](https://github.com/sagemath/sagetrac-mirror/tree/public/27802)** to **[1958872](https://github.com/sagemath/sagetrac-mirror/commit/1958872872f420a8e7166f147db58b216b9f5f79)**",
    "created_at": "2019-05-21T12:15:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27802#issuecomment-434360",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/27802](https://github.com/sagemath/sagetrac-mirror/tree/public/27802)** to **[1958872](https://github.com/sagemath/sagetrac-mirror/commit/1958872872f420a8e7166f147db58b216b9f5f79)**
