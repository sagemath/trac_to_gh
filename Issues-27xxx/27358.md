...
```json
{
    "body": "<a id='comment:22'></a>The test I wrote in comment:19 does is slightly different as `coercion_model.get_action` runs `Parent.get_action`, which does not put the action into the `_action_maps` `TripleDict`. Although I do believe the cycle mentioned in comment:15 is getting created, and it is that `coercion_model._action_maps` that nails it in memory.",
...
<a id='comment:22'></a>The test I wrote in comment:19 does is slightly different as `coercion_model.get_action` runs `Parent.get_action`, which does not put the action into the `_action_maps` `TripleDict`. Although I do believe the cycle mentioned in comment:15 is getting created, and it is that `coercion_model._action_maps` that nails it in memory.
...
<a id='comment:24'></a>Hmm...the leak in comment:23 might be a different issue as when I run
