# Issue 27563: Fixes cycle basis for multiedges

archive/issues_027326.json:
```json
{
    "assignees": [],
    "body": "It fixes #27538. Now, cycle_basis() for multiedges works correctly. \n\n```\nsage: G = Graph([(1,2),(2,3),(2,3),(3,4),(1,4),(1,4),(4,5),(5,6),(4,6),(6,7)],multiedges=True)\nsage: G.cycle_basis()\n[[1, 4], [2, 3], [4, 3, 2, 1], [6, 5, 4]]\n```\n\\\\ Currently, I am importing Graph from sage.graphs.graph in cycle_basis function to create instance of Graph. Suggestions are invited for any alternate approach.\n\nCC:  @dcoudert @rajat1433\n\nBranch/Commit: **[u/gh-abhayptp/fixes_cycle_basis_for_multiedges](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-abhayptp/fixes_cycle_basis_for_multiedges) @ [b2c82ec](https://github.com/sagemath/sagetrac-mirror/commit/b2c82eca89e7ea2c88d2ce94f8904e12b969ccac)**\n\nReviewer: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27563_\n\n",
    "closed_at": "2020-09-13T09:20:34Z",
    "created_at": "2019-03-28T21:41:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fixes cycle basis for multiedges",
    "type": "issue",
    "updated_at": "2020-09-13T09:20:34Z",
    "url": "https://github.com/sagemath/sage/issues/27563",
    "user": "https://github.com/abhayptp"
}
```
It fixes #27538. Now, cycle_basis() for multiedges works correctly. 

```
sage: G = Graph([(1,2),(2,3),(2,3),(3,4),(1,4),(1,4),(4,5),(5,6),(4,6),(6,7)],multiedges=True)
sage: G.cycle_basis()
[[1, 4], [2, 3], [4, 3, 2, 1], [6, 5, 4]]
```
\\ Currently, I am importing Graph from sage.graphs.graph in cycle_basis function to create instance of Graph. Suggestions are invited for any alternate approach.

CC:  @dcoudert @rajat1433

Branch/Commit: **[u/gh-abhayptp/fixes_cycle_basis_for_multiedges](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-abhayptp/fixes_cycle_basis_for_multiedges) @ [b2c82ec](https://github.com/sagemath/sagetrac-mirror/commit/b2c82eca89e7ea2c88d2ce94f8904e12b969ccac)**

Reviewer: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/27563_





---

archive/issue_events_360230.json:
```json
{
    "actor": "https://github.com/abhayptp",
    "created_at": "2019-03-28T21:41:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360230"
}
```



---

archive/issue_events_360231.json:
```json
{
    "actor": "https://github.com/abhayptp",
    "created_at": "2019-03-28T21:41:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360231"
}
```



---

archive/issue_comments_429875.json:
```json
{
    "body": "Branch: **[u/gh-abhayptp/fixes_cycle_basis_for_multiedges](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-abhayptp/fixes_cycle_basis_for_multiedges)**",
    "created_at": "2019-03-28T21:54:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429875",
    "user": "https://github.com/abhayptp"
}
```

Branch: **[u/gh-abhayptp/fixes_cycle_basis_for_multiedges](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-abhayptp/fixes_cycle_basis_for_multiedges)**



---

archive/issue_comments_429876.json:
```json
{
    "body": "Commit: **[b2c82ec](https://github.com/sagemath/sagetrac-mirror/commit/b2c82eca89e7ea2c88d2ce94f8904e12b969ccac)**",
    "created_at": "2019-03-28T22:03:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429876",
    "user": "https://github.com/abhayptp"
}
```

Commit: **[b2c82ec](https://github.com/sagemath/sagetrac-mirror/commit/b2c82eca89e7ea2c88d2ce94f8904e12b969ccac)**



---

archive/issue_events_360232.json:
```json
{
    "actor": "https://github.com/abhayptp",
    "created_at": "2019-03-28T22:03:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360232"
}
```



---

archive/issue_comments_429877.json:
```json
{
    "body": "Assignee: **@abhayptp**",
    "created_at": "2019-03-28T22:03:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429877",
    "user": "https://github.com/abhayptp"
}
```

Assignee: **@abhayptp**



---

archive/issue_events_360233.json:
```json
{
    "actor": "https://github.com/abhayptp",
    "created_at": "2019-03-28T22:03:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360233"
}
```



---

archive/issue_comments_429878.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,7 @@\n+It fixes #27538. Now, cycle_basis() for multiedges works correctly. \\\\ Currently, I am importing sage.graphs.graph in cycle_basis to use create Graph. Suggestions are invited for any alternate approach.\n \n+```\n+sage: G = Graph([(1,2),(2,3),(2,3),(3,4),(1,4),(1,4),(4,5),(5,6),(4,6),(6,7)],multiedges=True)\n+sage: G.cycle_basis()\n+[[1, 4], [2, 3], [4, 3, 2, 1], [6, 5, 4]]\n+```\n``````\n",
    "created_at": "2019-03-28T22:03:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429878",
    "user": "https://github.com/abhayptp"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,7 @@
+It fixes #27538. Now, cycle_basis() for multiedges works correctly. \\ Currently, I am importing sage.graphs.graph in cycle_basis to use create Graph. Suggestions are invited for any alternate approach.
 
+```
+sage: G = Graph([(1,2),(2,3),(2,3),(3,4),(1,4),(1,4),(4,5),(5,6),(4,6),(6,7)],multiedges=True)
+sage: G.cycle_basis()
+[[1, 4], [2, 3], [4, 3, 2, 1], [6, 5, 4]]
+```
``````




---

archive/issue_comments_429879.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b2c82eca89e7ea2c88d2ce94f8904e12b969ccac\">b2c82ec</a></td><td><code>Fixes cycle basis for multiedges</code></td></tr></table>\n",
    "created_at": "2019-03-28T22:03:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429879",
    "user": "https://github.com/abhayptp"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b2c82eca89e7ea2c88d2ce94f8904e12b969ccac">b2c82ec</a></td><td><code>Fixes cycle basis for multiedges</code></td></tr></table>




---

archive/issue_comments_429880.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,8 @@\n-It fixes #27538. Now, cycle_basis() for multiedges works correctly. \\\\ Currently, I am importing sage.graphs.graph in cycle_basis to use create Graph. Suggestions are invited for any alternate approach.\n+It fixes #27538. Now, cycle_basis() for multiedges works correctly. \n \n ```\n sage: G = Graph([(1,2),(2,3),(2,3),(3,4),(1,4),(1,4),(4,5),(5,6),(4,6),(6,7)],multiedges=True)\n sage: G.cycle_basis()\n [[1, 4], [2, 3], [4, 3, 2, 1], [6, 5, 4]]\n ```\n+\\\\ Currently, I am importing Graph from sage.graphs.graph in cycle_basis function to create instance of Graph. Suggestions are invited for any alternate approach.\n``````\n",
    "created_at": "2019-03-28T22:17:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429880",
    "user": "https://github.com/abhayptp"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,8 @@
-It fixes #27538. Now, cycle_basis() for multiedges works correctly. \\ Currently, I am importing sage.graphs.graph in cycle_basis to use create Graph. Suggestions are invited for any alternate approach.
+It fixes #27538. Now, cycle_basis() for multiedges works correctly. 
 
 ```
 sage: G = Graph([(1,2),(2,3),(2,3),(3,4),(1,4),(1,4),(4,5),(5,6),(4,6),(6,7)],multiedges=True)
 sage: G.cycle_basis()
 [[1, 4], [2, 3], [4, 3, 2, 1], [6, 5, 4]]
 ```
+\\ Currently, I am importing Graph from sage.graphs.graph in cycle_basis function to create instance of Graph. Suggestions are invited for any alternate approach.
``````




---

archive/issue_comments_429881.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nDuplicate. See #27538.",
    "created_at": "2019-03-29T14:00:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429881",
    "user": "https://github.com/abhayptp"
}
```

<a id='comment:4'>Comment 4:</a>
Duplicate. See #27538.



---

archive/issue_comments_429882.json:
```json
{
    "body": "Removed assignee **@abhayptp**",
    "created_at": "2019-03-29T14:01:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429882",
    "user": "https://github.com/abhayptp"
}
```

Removed assignee **@abhayptp**



---

archive/issue_events_360234.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-03-29T15:00:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360234"
}
```



---

archive/issue_events_360235.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-03-29T15:00:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360235"
}
```



---

archive/issue_events_360236.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-03-29T15:00:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360236"
}
```



---

archive/issue_events_360237.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-09-12T10:08:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360237"
}
```



---

archive/issue_comments_429883.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2020-09-12T10:08:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429883",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_events_360238.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-09-12T10:09:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360238"
}
```



---

archive/issue_events_360239.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-09-12T10:09:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360239"
}
```



---

archive/issue_comments_429884.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nWe can close this ticket.",
    "created_at": "2020-09-12T10:09:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27563#issuecomment-429884",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:8'>Comment 8:</a>
We can close this ticket.



---

archive/issue_events_360240.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-13T09:20:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360240"
}
```



---

archive/issue_events_360241.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-13T09:20:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360241"
}
```



---

archive/issue_events_360242.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-13T09:20:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27563",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27563#event-360242"
}
```
