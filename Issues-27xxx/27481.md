# Issue 27481: sphinx_plot: Fix regression for 3d plots, suppress axes frame

archive/issues_027244.json:
```json
{
    "assignees": [],
    "body": "As reported in https://groups.google.com/forum/#!topic/sage-devel/qivRW2D2Fgg\n\n\nCC:  @egourgoulhon\n\nBranch/Commit: **[013415d](https://github.com/sagemath/sagetrac-mirror/commit/013415d709a3a5dabc7a57cfbd3706a23e6bb128)**\n\nReviewer: **Eric Gourgoulhon**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27481_\n\n",
    "closed_at": "2019-03-25T19:44:04Z",
    "created_at": "2019-03-14T02:53:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sphinx_plot: Fix regression for 3d plots, suppress axes frame",
    "type": "issue",
    "updated_at": "2019-03-25T19:44:04Z",
    "url": "https://github.com/sagemath/sage/issues/27481",
    "user": "https://github.com/mkoeppe"
}
```
As reported in https://groups.google.com/forum/#!topic/sage-devel/qivRW2D2Fgg


CC:  @egourgoulhon

Branch/Commit: **[013415d](https://github.com/sagemath/sagetrac-mirror/commit/013415d709a3a5dabc7a57cfbd3706a23e6bb128)**

Reviewer: **Eric Gourgoulhon**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/27481_





---

archive/issue_events_359193.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-03-14T02:53:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359193"
}
```



---

archive/issue_events_359194.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-03-14T02:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359194"
}
```



---

archive/issue_events_359195.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-03-14T02:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359195"
}
```



---

archive/issue_events_359196.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-03-14T02:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359196"
}
```



---

archive/issue_comments_428245.json:
```json
{
    "body": "Branch: **[u/mkoeppe/sphinx_plot__fix_regression_for_3d_plots__suppress_axes_frame](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sphinx_plot__fix_regression_for_3d_plots__suppress_axes_frame)**",
    "created_at": "2019-03-14T07:30:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428245",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/sphinx_plot__fix_regression_for_3d_plots__suppress_axes_frame](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sphinx_plot__fix_regression_for_3d_plots__suppress_axes_frame)**



---

archive/issue_comments_428246.json:
```json
{
    "body": "Commit: **[013415d](https://github.com/sagemath/sagetrac-mirror/commit/013415d709a3a5dabc7a57cfbd3706a23e6bb128)**",
    "created_at": "2019-03-14T07:31:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428246",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[013415d](https://github.com/sagemath/sagetrac-mirror/commit/013415d709a3a5dabc7a57cfbd3706a23e6bb128)**



---

archive/issue_comments_428247.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/013415d709a3a5dabc7a57cfbd3706a23e6bb128\">013415d</a></td><td><code>sphinx_plot: For 3d objects plotted via png, turn off axes frame</code></td></tr></table>\n",
    "created_at": "2019-03-14T07:31:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428247",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/013415d709a3a5dabc7a57cfbd3706a23e6bb128">013415d</a></td><td><code>sphinx_plot: For 3d objects plotted via png, turn off axes frame</code></td></tr></table>




---

archive/issue_comments_428248.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks for the fix! Is the ticket ready for review?",
    "created_at": "2019-03-14T10:11:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428248",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks for the fix! Is the ticket ready for review?



---

archive/issue_comments_428249.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI'd say this is pretty important since without it there's a noticeable, if not mostly aesthetic, regression in the documentation.",
    "created_at": "2019-03-14T15:30:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428249",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

I'd say this is pretty important since without it there's a noticeable, if not mostly aesthetic, regression in the documentation.



---

archive/issue_events_359197.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-14T15:30:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359197"
}
```



---

archive/issue_events_359198.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-14T15:30:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359198"
}
```



---

archive/issue_comments_428250.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nReplying to [@egourgoulhon](#comment%3A3):\n> Thanks for the fix! Is the ticket ready for review?\n\nI've checked the ticket branch: it fixes the issue. So I am ready to give a 'positive review', all the more that the release of 8.7.rc0 is approaching... If you agree, please set the ticket to 'needs review'.",
    "created_at": "2019-03-15T13:44:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428250",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:5" align="right">Comment 5</div>

Replying to [@egourgoulhon](#comment%3A3):
> Thanks for the fix! Is the ticket ready for review?

I've checked the ticket branch: it fixes the issue. So I am ready to give a 'positive review', all the more that the release of 8.7.rc0 is approaching... If you agree, please set the ticket to 'needs review'.



---

archive/issue_events_359199.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-03-15T13:47:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359199"
}
```



---

archive/issue_comments_428251.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2019-03-15T13:47:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428251",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_428252.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks!",
    "created_at": "2019-03-15T13:52:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428252",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks!



---

archive/issue_comments_428253.json:
```json
{
    "body": "Reviewer: **Eric Gourgoulhon**",
    "created_at": "2019-03-15T13:52:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428253",
    "user": "https://github.com/egourgoulhon"
}
```

Reviewer: **Eric Gourgoulhon**



---

archive/issue_events_359200.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2019-03-15T13:52:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359200"
}
```



---

archive/issue_events_359201.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2019-03-15T13:52:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359201"
}
```



---

archive/issue_events_359202.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:41:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359202"
}
```



---

archive/issue_events_359203.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:41:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359203"
}
```



---

archive/issue_comments_428254.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nMoving all blocker/critical issues from 8.7 to 8.8.",
    "created_at": "2019-03-25T10:41:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428254",
    "user": "https://github.com/embray"
}
```

<div id="comment:8" align="right">Comment 8</div>

Moving all blocker/critical issues from 8.7 to 8.8.



---

archive/issue_events_359204.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-03-25T19:44:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359204"
}
```



---

archive/issue_events_359205.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2bd7fba98e6415148137b069183135a7f388487e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-03-25T19:44:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27481#event-359205"
}
```



---

archive/issue_comments_428255.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/sphinx_plot__fix_regression_for_3d_plots__suppress_axes_frame](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sphinx_plot__fix_regression_for_3d_plots__suppress_axes_frame)** to **[013415d](https://github.com/sagemath/sagetrac-mirror/commit/013415d709a3a5dabc7a57cfbd3706a23e6bb128)**",
    "created_at": "2019-03-25T19:44:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27481#issuecomment-428255",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/sphinx_plot__fix_regression_for_3d_plots__suppress_axes_frame](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sphinx_plot__fix_regression_for_3d_plots__suppress_axes_frame)** to **[013415d](https://github.com/sagemath/sagetrac-mirror/commit/013415d709a3a5dabc7a57cfbd3706a23e6bb128)**
