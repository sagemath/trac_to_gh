# Issue 27577: add conversion of abs back from fricas

archive/issues_027340.json:
```json
{
    "assignees": [],
    "body": "When Fricas uses abs() in its return value, this function is translated back to sage as a new symbolic, unevaluable function. See this sage-devel [post](https://groups.google.com/forum/#!topic/sage-devel/UAimnIeyGX8) for illustration.\n\nCC:  @mantepse @rwst\n\nKeywords: **FriCAS**\n\nBranch/Commit: **[2b35b3b](https://github.com/sagemath/sagetrac-mirror/commit/2b35b3b2fccdbc3eedda5b7f0a57d5da08de135c)**\n\nReviewer: **Martin Rubey**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27577_\n\n",
    "closed_at": "2019-04-02T21:30:05Z",
    "created_at": "2019-03-30T20:02:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add conversion of abs back from fricas",
    "type": "issue",
    "updated_at": "2019-04-02T21:30:05Z",
    "url": "https://github.com/sagemath/sage/issues/27577",
    "user": "https://github.com/fchapoton"
}
```
When Fricas uses abs() in its return value, this function is translated back to sage as a new symbolic, unevaluable function. See this sage-devel [post](https://groups.google.com/forum/#!topic/sage-devel/UAimnIeyGX8) for illustration.

CC:  @mantepse @rwst

Keywords: **FriCAS**

Branch/Commit: **[2b35b3b](https://github.com/sagemath/sagetrac-mirror/commit/2b35b3b2fccdbc3eedda5b7f0a57d5da08de135c)**

Reviewer: **Martin Rubey**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/27577_





---

archive/issue_events_360416.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-30T20:02:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360416"
}
```



---

archive/issue_events_360417.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-30T20:02:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360417"
}
```



---

archive/issue_events_360418.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-30T20:02:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360418"
}
```



---

archive/issue_events_360419.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-30T20:02:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360419"
}
```



---

archive/issue_comments_430221.json:
```json
{
    "body": "Commit: **[2b35b3b](https://github.com/sagemath/sagetrac-mirror/commit/2b35b3b2fccdbc3eedda5b7f0a57d5da08de135c)**",
    "created_at": "2019-03-30T20:02:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430221",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[2b35b3b](https://github.com/sagemath/sagetrac-mirror/commit/2b35b3b2fccdbc3eedda5b7f0a57d5da08de135c)**



---

archive/issue_comments_430222.json:
```json
{
    "body": "Branch: **[u/chapoton/27577](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27577)**",
    "created_at": "2019-03-30T20:02:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430222",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/27577](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27577)**



---

archive/issue_comments_430223.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2b35b3b2fccdbc3eedda5b7f0a57d5da08de135c\">2b35b3b</a></td><td><code>add abs conversion back from Fricas</code></td></tr></table>\n",
    "created_at": "2019-03-30T20:02:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430223",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2b35b3b2fccdbc3eedda5b7f0a57d5da08de135c">2b35b3b</a></td><td><code>add abs conversion back from Fricas</code></td></tr></table>




---

archive/issue_events_360420.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-30T20:02:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360420"
}
```



---

archive/issue_comments_430224.json:
```json
{
    "body": "Changed keywords from **** to **FriCAS**",
    "created_at": "2019-03-30T20:08:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430224",
    "user": "https://github.com/mantepse"
}
```

Changed keywords from **** to **FriCAS**



---

archive/issue_events_360421.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2019-03-31T00:06:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360421"
}
```



---

archive/issue_events_360422.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2019-03-31T00:06:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360422"
}
```



---

archive/issue_comments_430225.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks good!",
    "created_at": "2019-03-31T00:06:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430225",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:3'>Comment 3:</a>
Looks good!



---

archive/issue_comments_430226.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThx. reviewer name, please",
    "created_at": "2019-03-31T07:06:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430226",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
Thx. reviewer name, please



---

archive/issue_events_360423.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2019-03-31T08:22:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360423"
}
```



---

archive/issue_events_360424.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2019-03-31T08:22:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360424"
}
```



---

archive/issue_comments_430227.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nAdded a bit of description.\n\nThanks !",
    "created_at": "2019-03-31T08:22:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430227",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:5'>Comment 5:</a>
Added a bit of description.

Thanks !



---

archive/issue_comments_430228.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+When Fricas uses abs() in its return value, this function is translated back to sage as a new symbolic, unevaluable function. See this sage-devel [post](https://groups.google.com/forum/#!topic/sage-devel/UAimnIeyGX8) for illustration.\n``````\n",
    "created_at": "2019-03-31T08:22:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430228",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+When Fricas uses abs() in its return value, this function is translated back to sage as a new symbolic, unevaluable function. See this sage-devel [post](https://groups.google.com/forum/#!topic/sage-devel/UAimnIeyGX8) for illustration.
``````




---

archive/issue_comments_430229.json:
```json
{
    "body": "Reviewer: **Martin Rubey**",
    "created_at": "2019-03-31T09:25:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430229",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Martin Rubey**



---

archive/issue_events_360425.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-04-02T21:30:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360425"
}
```



---

archive/issue_events_360426.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a58438cf932fcefa931c48cac60dc0639ba21f0c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-04-02T21:30:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27577#event-360426"
}
```



---

archive/issue_comments_430230.json:
```json
{
    "body": "Changed branch from **[u/chapoton/27577](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27577)** to **[2b35b3b](https://github.com/sagemath/sagetrac-mirror/commit/2b35b3b2fccdbc3eedda5b7f0a57d5da08de135c)**",
    "created_at": "2019-04-02T21:30:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27577#issuecomment-430230",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/27577](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27577)** to **[2b35b3b](https://github.com/sagemath/sagetrac-mirror/commit/2b35b3b2fccdbc3eedda5b7f0a57d5da08de135c)**
