# Issue 27861: Polymake: Better syntax to call functions with <template arguments>

archive/issues_027624.json:
```json
{
    "assignees": [],
    "body": "Currently we have to write:\n\n```\nsage: polymake(\"tropical::uniform_linear_space<Max>(3,2)\")\n```\nor\n\n```\nsage: getattr(polymake, \"tropical::uniform_linear_space<Max>\")(3, 2)\n```\nor\n\n```\nsage: polymake.function_call(\"tropical::uniform_linear_space<Max>\", [3, 2])\n```\nbecause the template brackets cannot become part of the Python function name, so the normal `.` syntax cannot work.\n \n(Also, but not the point of this ticket - #27736: Polymake: Turn polymake applications into objects with directories.)\n\nAlso, \n\n```\n    polymake(\"tropical::uniform_linear_space<Max>\")(3,2)\n```\ndoes not work. It looks for an overloaded function without arguments, which fails:\n\n```\n    TypeError: no matching overloaded instance of Polymake::tropical::uniform_linear_space<Max>()\n```\n\nAlso, \n\n```\nsage: polymake.call(\"tropical::uniform_linear_space<Max>\", 3, 2)\n```\ndoes not work -- the undocumented and untested `call` method passes the `args` tuple to `function_call`, which expects lists instead of tuples.\n\nCC:  @simon-king-jena @jplab @dimpase\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27861_\n\n",
    "created_at": "2019-05-23T00:57:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Polymake: Better syntax to call functions with <template arguments>",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/27861",
    "user": "https://github.com/mkoeppe"
}
```
Currently we have to write:

```
sage: polymake("tropical::uniform_linear_space<Max>(3,2)")
```
or

```
sage: getattr(polymake, "tropical::uniform_linear_space<Max>")(3, 2)
```
or

```
sage: polymake.function_call("tropical::uniform_linear_space<Max>", [3, 2])
```
because the template brackets cannot become part of the Python function name, so the normal `.` syntax cannot work.
 
(Also, but not the point of this ticket - #27736: Polymake: Turn polymake applications into objects with directories.)

Also, 

```
    polymake("tropical::uniform_linear_space<Max>")(3,2)
```
does not work. It looks for an overloaded function without arguments, which fails:

```
    TypeError: no matching overloaded instance of Polymake::tropical::uniform_linear_space<Max>()
```

Also, 

```
sage: polymake.call("tropical::uniform_linear_space<Max>", 3, 2)
```
does not work -- the undocumented and untested `call` method passes the `args` tuple to `function_call`, which expects lists instead of tuples.

CC:  @simon-king-jena @jplab @dimpase

_Issue created by migration from https://trac.sagemath.org/ticket/27861_





---

archive/issue_events_363922.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-23T00:57:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363922"
}
```



---

archive/issue_events_363923.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-23T00:57:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "component: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363923"
}
```



---

archive/issue_events_363924.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-23T00:57:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363924"
}
```



---

archive/issue_events_363925.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-23T00:57:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363925"
}
```



---

archive/issue_comments_435642.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAs the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).",
    "created_at": "2019-06-14T14:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27861#issuecomment-435642",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
As the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).



---

archive/issue_events_363926.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:55:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363926"
}
```



---

archive/issue_comments_435643.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,7 +3,14 @@\n ```\n     polymake(\"tropical::uniform_linear_space<Max>(3,2)\")\n ```\n-because the template brackets cannot become part of the Python function name.\n+or\n+\n+```\n+sage: getattr(polymake, \"tropical::uniform_linear_space<Max>\")(3, 2)\n+Uniform linear space of dimension 2 in dimension 3\n+```\n+because the template brackets cannot become part of the Python function name, so the normal `.` syntax cannot work.\n+ \n (Also, but not the point of this ticket - #27736: Polymake: Turn polymake applications into objects with directories.)\n \n Also, \n``````\n",
    "created_at": "2019-07-26T15:42:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27861#issuecomment-435643",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,7 +3,14 @@
 ```
     polymake("tropical::uniform_linear_space<Max>(3,2)")
 ```
-because the template brackets cannot become part of the Python function name.
+or
+
+```
+sage: getattr(polymake, "tropical::uniform_linear_space<Max>")(3, 2)
+Uniform linear space of dimension 2 in dimension 3
+```
+because the template brackets cannot become part of the Python function name, so the normal `.` syntax cannot work.
+ 
 (Also, but not the point of this ticket - #27736: Polymake: Turn polymake applications into objects with directories.)
 
 Also, 
``````




---

archive/issue_comments_435644.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,13 +1,17 @@\n Currently we have to write:\n \n ```\n-    polymake(\"tropical::uniform_linear_space<Max>(3,2)\")\n+sage: polymake(\"tropical::uniform_linear_space<Max>(3,2)\")\n ```\n or\n \n ```\n sage: getattr(polymake, \"tropical::uniform_linear_space<Max>\")(3, 2)\n-Uniform linear space of dimension 2 in dimension 3\n+```\n+or\n+\n+```\n+sage: polymake.function_call(\"tropical::uniform_linear_space<Max>\", [3, 2])\n ```\n because the template brackets cannot become part of the Python function name, so the normal `.` syntax cannot work.\n  \n@@ -18,10 +22,15 @@\n ```\n     polymake(\"tropical::uniform_linear_space<Max>\")(3,2)\n ```\n-does not work -- we cannot seem to create this function object. It looks for an overloaded function without arguments, which fails:\n+does not work. It looks for an overloaded function without arguments, which fails:\n \n ```\n     TypeError: no matching overloaded instance of Polymake::tropical::uniform_linear_space<Max>()\n ```\n \n+Also, \n \n+```\n+sage: polymake.call(\"tropical::uniform_linear_space<Max>\", 3, 2)\n+```\n+does not work -- the undocumented and untested `call` method passes the `args` tuple to `function_call`, which expects lists instead of tuples.\n``````\n",
    "created_at": "2019-07-29T23:06:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27861#issuecomment-435644",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,13 +1,17 @@
 Currently we have to write:
 
 ```
-    polymake("tropical::uniform_linear_space<Max>(3,2)")
+sage: polymake("tropical::uniform_linear_space<Max>(3,2)")
 ```
 or
 
 ```
 sage: getattr(polymake, "tropical::uniform_linear_space<Max>")(3, 2)
-Uniform linear space of dimension 2 in dimension 3
+```
+or
+
+```
+sage: polymake.function_call("tropical::uniform_linear_space<Max>", [3, 2])
 ```
 because the template brackets cannot become part of the Python function name, so the normal `.` syntax cannot work.
  
@@ -18,10 +22,15 @@
 ```
     polymake("tropical::uniform_linear_space<Max>")(3,2)
 ```
-does not work -- we cannot seem to create this function object. It looks for an overloaded function without arguments, which fails:
+does not work. It looks for an overloaded function without arguments, which fails:
 
 ```
     TypeError: no matching overloaded instance of Polymake::tropical::uniform_linear_space<Max>()
 ```
 
+Also, 
 
+```
+sage: polymake.call("tropical::uniform_linear_space<Max>", 3, 2)
+```
+does not work -- the undocumented and untested `call` method passes the `args` tuple to `function_call`, which expects lists instead of tuples.
``````




---

archive/issue_comments_435645.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMoving some tickets to 9.2. This is not a promise that I will be working on them.",
    "created_at": "2020-05-01T18:16:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27861#issuecomment-435645",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Moving some tickets to 9.2. This is not a promise that I will be working on them.



---

archive/issue_events_363927.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T18:16:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363927"
}
```



---

archive/issue_events_363928.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363928"
}
```



---

archive/issue_events_363929.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363929"
}
```



---

archive/issue_events_363930.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363930"
}
```



---

archive/issue_events_363931.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363931"
}
```



---

archive/issue_comments_435646.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27861#issuecomment-435646",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'>Comment 6:</a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_363932.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363932"
}
```



---

archive/issue_events_363933.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363933"
}
```



---

archive/issue_events_363934.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363934"
}
```



---

archive/issue_events_363935.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363935"
}
```



---

archive/issue_events_363936.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363936"
}
```



---

archive/issue_events_363937.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363937"
}
```



---

archive/issue_events_363938.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-06T19:55:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363938"
}
```



---

archive/issue_events_363939.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-06T19:55:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363939"
}
```



---

archive/issue_events_363940.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363940"
}
```



---

archive/issue_events_363941.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27861",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27861#event-363941"
}
```
