...
```json
{
    "body": "I realise that it is not optimal, but I haven't got a clue how to do it properly.",
    "created_at": "2019-03-24T20:22:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27542",
    "type": "issue_comment",
...
```json
{
    "body": "Making this a blocker because it turns out that the interface is almost unusable for serious work on python2 now, and there is a fix.",
...
```json
{
    "body": "1. I'm wondering how stable the findstat output is. My impression is that it's not and that's the reason why they were marked `# random` in the first place. If you remove `# random`, then you might get failures in the future whenever the output from findstat changes.\n\n2. Wouldn't it make sense to move the encoding to `__repr__`? In other words, use `unicode` everywhere except at the very end when you're returning from `__repr__`.",
...
```json
{
    "body": "Alternatively, if you just want to test that `repr()` works, it suffices to add a doctest\n\n```\nx = repr(findstat(...))  # check that repr() doesn't raise an exception\n```",
...
```json
{
    "body": "Lgtm, everything seems to work and bot is green.",
    "created_at": "2019-04-04T08:44:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27542",
    "type": "issue_comment",
