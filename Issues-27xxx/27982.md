# Issue 27982: Make Lattes map for projective space coerce base ring

archive/issues_027745.json:
```json
{
    "assignees": [],
    "body": "This ticket makes the following work:\n\n```\nP.<x, y> = ProjectiveSpace(GF(37), 1)\nE = EllipticCurve([1, 1])\nf = P.Lattes_map(E, 2)\n```\nIt failed up to now since `E` is defined over `QQ` and `P` is defined over `GF(37)`.\nWe should try to coerce to the base ring of projective space.\n\nKeywords: **SI2019**\n\nBranch/Commit: **[7683702](https://github.com/sagemath/sagetrac-mirror/commit/7683702f243964cb93228bf5d261291e516ed118)**\n\nReviewer: **Adam Towsley**\n\nAuthor: **Ben Hutz**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27982_\n\n",
    "closed_at": "2019-07-07T12:36:09Z",
    "created_at": "2019-06-13T13:57:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20dynamics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make Lattes map for projective space coerce base ring",
    "type": "issue",
    "updated_at": "2019-07-07T12:36:09Z",
    "url": "https://github.com/sagemath/sage/issues/27982",
    "user": "https://github.com/bhutz"
}
```
This ticket makes the following work:

```
P.<x, y> = ProjectiveSpace(GF(37), 1)
E = EllipticCurve([1, 1])
f = P.Lattes_map(E, 2)
```
It failed up to now since `E` is defined over `QQ` and `P` is defined over `GF(37)`.
We should try to coerce to the base ring of projective space.

Keywords: **SI2019**

Branch/Commit: **[7683702](https://github.com/sagemath/sagetrac-mirror/commit/7683702f243964cb93228bf5d261291e516ed118)**

Reviewer: **Adam Towsley**

Author: **Ben Hutz**

_Issue created by migration from https://trac.sagemath.org/ticket/27982_





---

archive/issue_events_365531.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-06-13T13:57:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365531"
}
```



---

archive/issue_events_365532.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-06-13T13:57:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20dynamics",
    "label_color": "0000ff",
    "label_name": "component: dynamics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365532"
}
```



---

archive/issue_events_365533.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-06-13T13:57:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365533"
}
```



---

archive/issue_events_365534.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-06-13T13:57:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365534"
}
```



---

archive/issue_comments_438245.json:
```json
{
    "body": "Author: **Ben Hutz**",
    "created_at": "2019-06-13T15:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438245",
    "user": "https://github.com/bhutz"
}
```

Author: **Ben Hutz**



---

archive/issue_comments_438246.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7683702f243964cb93228bf5d261291e516ed118\">7683702</a></td><td><code>27982: added base ring check for lattes maps</code></td></tr></table>\n",
    "created_at": "2019-06-13T15:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438246",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7683702f243964cb93228bf5d261291e516ed118">7683702</a></td><td><code>27982: added base ring check for lattes maps</code></td></tr></table>




---

archive/issue_comments_438247.json:
```json
{
    "body": "Commit: **[7683702](https://github.com/sagemath/sagetrac-mirror/commit/7683702f243964cb93228bf5d261291e516ed118)**",
    "created_at": "2019-06-13T15:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438247",
    "user": "https://github.com/bhutz"
}
```

Commit: **[7683702](https://github.com/sagemath/sagetrac-mirror/commit/7683702f243964cb93228bf5d261291e516ed118)**



---

archive/issue_comments_438248.json:
```json
{
    "body": "Branch: **[u/bhutz/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/27982_lattes)**",
    "created_at": "2019-06-13T15:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438248",
    "user": "https://github.com/bhutz"
}
```

Branch: **[u/bhutz/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/27982_lattes)**



---

archive/issue_events_365535.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-06-13T15:13:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365535"
}
```



---

archive/issue_comments_438249.json:
```json
{
    "body": "Changed branch from **[u/bhutz/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/27982_lattes)** to **[u/gh-Zopherus/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Zopherus/27982_lattes)**",
    "created_at": "2019-06-13T15:14:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438249",
    "user": "https://github.com/Zopherus"
}
```

Changed branch from **[u/bhutz/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/27982_lattes)** to **[u/gh-Zopherus/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Zopherus/27982_lattes)**



---

archive/issue_comments_438250.json:
```json
{
    "body": "Changed branch from **[u/gh-Zopherus/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Zopherus/27982_lattes)** to **[u/bhutz/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/27982_lattes)**",
    "created_at": "2019-06-13T15:19:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438250",
    "user": "https://github.com/bhutz"
}
```

Changed branch from **[u/gh-Zopherus/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Zopherus/27982_lattes)** to **[u/bhutz/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/27982_lattes)**



---

archive/issue_events_365536.json:
```json
{
    "actor": "https://github.com/sagetrac-atowsley",
    "created_at": "2019-06-13T18:00:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365536"
}
```



---

archive/issue_events_365537.json:
```json
{
    "actor": "https://github.com/sagetrac-atowsley",
    "created_at": "2019-06-13T18:00:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365537"
}
```



---

archive/issue_comments_438251.json:
```json
{
    "body": "Reviewer: **Adam Towsley**",
    "created_at": "2019-06-13T18:14:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438251",
    "user": "https://github.com/sagetrac-atowsley"
}
```

Reviewer: **Adam Towsley**



---

archive/issue_comments_438252.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,9 +1,9 @@\n The following fails\n \n ```\n-P.<x,y>=ProjectiveSpace(GF(37),1)\n-E=EllipticCurve([1,1])\n-f=P.Lattes_map(E,2)\n+P.<x, y>=ProjectiveSpace(GF(37), 1)\n+E = EllipticCurve([1, 1])\n+f = P.Lattes_map(E, 2)\n ```\n \n-since E is defined over QQ and P is defined over GF(37). We should try to coerce to the base ring of projective space.\n+since `E` is defined over `QQ` and `P` is defined over `GF(37)`. We should try to coerce to the base ring of projective space.\n``````\n",
    "created_at": "2019-06-13T21:46:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438252",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,9 +1,9 @@
 The following fails
 
 ```
-P.<x,y>=ProjectiveSpace(GF(37),1)
-E=EllipticCurve([1,1])
-f=P.Lattes_map(E,2)
+P.<x, y>=ProjectiveSpace(GF(37), 1)
+E = EllipticCurve([1, 1])
+f = P.Lattes_map(E, 2)
 ```
 
-since E is defined over QQ and P is defined over GF(37). We should try to coerce to the base ring of projective space.
+since `E` is defined over `QQ` and `P` is defined over `GF(37)`. We should try to coerce to the base ring of projective space.
``````




---

archive/issue_events_365538.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-13T21:48:21Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "title_is": "Make Lattes map for projective space coerce base ring",
    "title_was": "Lattes map for projective space fails to coerce base ring",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365538"
}
```



---

archive/issue_comments_438253.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,9 +1,9 @@\n-The following fails\n+This ticket makes the following work:\n \n ```\n-P.<x, y>=ProjectiveSpace(GF(37), 1)\n+P.<x, y> = ProjectiveSpace(GF(37), 1)\n E = EllipticCurve([1, 1])\n f = P.Lattes_map(E, 2)\n ```\n-\n-since `E` is defined over `QQ` and `P` is defined over `GF(37)`. We should try to coerce to the base ring of projective space.\n+It failed up to now since `E` is defined over `QQ` and `P` is defined over `GF(37)`.\n+We should try to coerce to the base ring of projective space.\n``````\n",
    "created_at": "2019-06-13T21:48:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438253",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,9 +1,9 @@
-The following fails
+This ticket makes the following work:
 
 ```
-P.<x, y>=ProjectiveSpace(GF(37), 1)
+P.<x, y> = ProjectiveSpace(GF(37), 1)
 E = EllipticCurve([1, 1])
 f = P.Lattes_map(E, 2)
 ```
-
-since `E` is defined over `QQ` and `P` is defined over `GF(37)`. We should try to coerce to the base ring of projective space.
+It failed up to now since `E` is defined over `QQ` and `P` is defined over `GF(37)`.
+We should try to coerce to the base ring of projective space.
``````




---

archive/issue_events_365539.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-30T12:33:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365539"
}
```



---

archive/issue_events_365540.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-30T12:33:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365540"
}
```



---

archive/issue_events_365541.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-07T12:36:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365541"
}
```



---

archive/issue_events_365542.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2a677d8639f3d090bd1d0f37c6f76ee7fb98fe6c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-07-07T12:36:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27982#event-365542"
}
```



---

archive/issue_comments_438254.json:
```json
{
    "body": "Changed branch from **[u/bhutz/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/27982_lattes)** to **[7683702](https://github.com/sagemath/sagetrac-mirror/commit/7683702f243964cb93228bf5d261291e516ed118)**",
    "created_at": "2019-07-07T12:36:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27982",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27982#issuecomment-438254",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/bhutz/27982_lattes](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/27982_lattes)** to **[7683702](https://github.com/sagemath/sagetrac-mirror/commit/7683702f243964cb93228bf5d261291e516ed118)**
