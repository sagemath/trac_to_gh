# Issue 27167: py3: fix doctest in distances_all_pairs.pyx

archive/issues_026930.json:
```json
{
    "assignees": [],
    "body": "again the error is due to the difference of ordering of the keys of dictionaries in py2 and py3. So we mark the doctest with `# py2` and `# py3`.\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[0a2d935](https://github.com/sagemath/sagetrac-mirror/commit/0a2d935e8e0db759aee7e178eefdf3d0c5a15b9c)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27167_\n\n",
    "closed_at": "2019-02-04T23:01:45Z",
    "created_at": "2019-01-29T16:24:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fix doctest in distances_all_pairs.pyx",
    "type": "issue",
    "updated_at": "2019-02-04T23:01:45Z",
    "url": "https://github.com/sagemath/sage/issues/27167",
    "user": "https://github.com/dcoudert"
}
```
again the error is due to the difference of ordering of the keys of dictionaries in py2 and py3. So we mark the doctest with `# py2` and `# py3`.

Keywords: **py3, graph**

Branch/Commit: **[0a2d935](https://github.com/sagemath/sagetrac-mirror/commit/0a2d935e8e0db759aee7e178eefdf3d0c5a15b9c)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/27167_





---

archive/issue_events_355146.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-29T16:24:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355146"
}
```



---

archive/issue_events_355147.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-29T16:24:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355147"
}
```



---

archive/issue_events_355148.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-29T16:24:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355148"
}
```



---

archive/issue_events_355149.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-29T16:24:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355149"
}
```



---

archive/issue_comments_422126.json:
```json
{
    "body": "Branch: **[public/27167_distances_all_pairs](https://github.com/sagemath/sagetrac-mirror/tree/public/27167_distances_all_pairs)**",
    "created_at": "2019-01-29T16:25:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27167#issuecomment-422126",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/27167_distances_all_pairs](https://github.com/sagemath/sagetrac-mirror/tree/public/27167_distances_all_pairs)**



---

archive/issue_events_355150.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-29T16:25:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355150"
}
```



---

archive/issue_events_355151.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-29T16:25:59Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "title_is": "py3: fix doctest in distances_all_pairs.pyx",
    "title_was": "py3: fix doctest in distance_all_pairs.pyx",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355151"
}
```



---

archive/issue_comments_422127.json:
```json
{
    "body": "Commit: **[0a2d935](https://github.com/sagemath/sagetrac-mirror/commit/0a2d935e8e0db759aee7e178eefdf3d0c5a15b9c)**",
    "created_at": "2019-01-29T16:25:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27167#issuecomment-422127",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[0a2d935](https://github.com/sagemath/sagetrac-mirror/commit/0a2d935e8e0db759aee7e178eefdf3d0c5a15b9c)**



---

archive/issue_comments_422128.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0a2d935e8e0db759aee7e178eefdf3d0c5a15b9c\">0a2d935</a></td><td><code>trac #27167: doctest in distances_all_pairs.pyx</code></td></tr></table>\n",
    "created_at": "2019-01-29T16:25:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27167#issuecomment-422128",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0a2d935e8e0db759aee7e178eefdf3d0c5a15b9c">0a2d935</a></td><td><code>trac #27167: doctest in distances_all_pairs.pyx</code></td></tr></table>




---

archive/issue_comments_422129.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-01-30T13:33:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27167#issuecomment-422129",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_422130.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nok",
    "created_at": "2019-01-30T13:33:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27167#issuecomment-422130",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
ok



---

archive/issue_events_355152.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-30T13:33:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355152"
}
```



---

archive/issue_events_355153.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-30T13:33:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355153"
}
```



---

archive/issue_events_355154.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-02-04T23:01:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355154"
}
```



---

archive/issue_events_355155.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f0b0e54d32b992320990821c077ca4d467a294a5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-02-04T23:01:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27167#event-355155"
}
```



---

archive/issue_comments_422131.json:
```json
{
    "body": "Changed branch from **[public/27167_distances_all_pairs](https://github.com/sagemath/sagetrac-mirror/tree/public/27167_distances_all_pairs)** to **[0a2d935](https://github.com/sagemath/sagetrac-mirror/commit/0a2d935e8e0db759aee7e178eefdf3d0c5a15b9c)**",
    "created_at": "2019-02-04T23:01:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27167#issuecomment-422131",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/27167_distances_all_pairs](https://github.com/sagemath/sagetrac-mirror/tree/public/27167_distances_all_pairs)** to **[0a2d935](https://github.com/sagemath/sagetrac-mirror/commit/0a2d935e8e0db759aee7e178eefdf3d0c5a15b9c)**
