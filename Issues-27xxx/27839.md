# Issue 27839: Remove some long() calls

archive/issues_027602.json:
```json
{
    "assignees": [],
    "body": "In various places in Sage, there are explicit `long(...)` calls being done. Some of them are superfluous or can be replaced by `int`.\n\nWe don't try to fix all `long()` calls, just a few obviously avoidable ones.\n\nCC:  @jhpalmieri\n\nBranch/Commit: **[acb54fa](https://github.com/sagemath/sagetrac-mirror/commit/acb54fac7d059c81ceb406e9a2f1704b69f18ed1)**\n\nReviewer: **Vincent Klein**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27839_\n\n",
    "closed_at": "2019-05-21T12:15:28Z",
    "created_at": "2019-05-16T11:44:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove some long() calls",
    "type": "issue",
    "updated_at": "2019-05-21T12:15:28Z",
    "url": "https://github.com/sagemath/sage/issues/27839",
    "user": "https://github.com/jdemeyer"
}
```
In various places in Sage, there are explicit `long(...)` calls being done. Some of them are superfluous or can be replaced by `int`.

We don't try to fix all `long()` calls, just a few obviously avoidable ones.

CC:  @jhpalmieri

Branch/Commit: **[acb54fa](https://github.com/sagemath/sagetrac-mirror/commit/acb54fac7d059c81ceb406e9a2f1704b69f18ed1)**

Reviewer: **Vincent Klein**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/27839_





---

archive/issue_events_363648.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2019-05-16T11:44:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27839#event-363648"
}
```



---

archive/issue_events_363649.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2019-05-16T11:44:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27839#event-363649"
}
```



---

archive/issue_events_363650.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2019-05-16T11:44:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27839#event-363650"
}
```



---

archive/issue_events_363651.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2019-05-16T11:44:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27839#event-363651"
}
```



---

archive/issue_comments_435063.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-In various places in Sage, there are explicit `long(...)` calls. In the light of Python 3, these should be removed or replaced with `int(...)`.\n+In various places in Sage (and unfortunately also in `fpylll`), there are explicit `long(...)` calls. In the light of Python 3, these should be removed or replaced with `int(...)`.\n``````\n",
    "created_at": "2019-05-16T11:44:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27839#issuecomment-435063",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-In various places in Sage, there are explicit `long(...)` calls. In the light of Python 3, these should be removed or replaced with `int(...)`.
+In various places in Sage (and unfortunately also in `fpylll`), there are explicit `long(...)` calls. In the light of Python 3, these should be removed or replaced with `int(...)`.
``````




---

archive/issue_comments_435064.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-In various places in Sage (and unfortunately also in `fpylll`), there are explicit `long(...)` calls. In the light of Python 3, these should be removed or replaced with `int(...)`.\n+In various places in Sage (and unfortunately also in `fpylll`), there are explicit `long(...)` calls being done on Sage types. In the light of Python 3, these should be removed or replaced with `int(...)`.\n``````\n",
    "created_at": "2019-05-16T11:48:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27839#issuecomment-435064",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-In various places in Sage (and unfortunately also in `fpylll`), there are explicit `long(...)` calls. In the light of Python 3, these should be removed or replaced with `int(...)`.
+In various places in Sage (and unfortunately also in `fpylll`), there are explicit `long(...)` calls being done on Sage types. In the light of Python 3, these should be removed or replaced with `int(...)`.
``````




---

archive/issue_comments_435065.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n-In various places in Sage (and unfortunately also in `fpylll`), there are explicit `long(...)` calls being done on Sage types. In the light of Python 3, these should be removed or replaced with `int(...)`.\n+In various places in Sage, there are explicit `long(...)` calls being done. Some of them are superfluous or can be replaced by `int`.\n+\n+We don't try to fix all `long()` calls, just a few obviously avoidable ones.\n``````\n",
    "created_at": "2019-05-16T13:12:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27839#issuecomment-435065",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
-In various places in Sage (and unfortunately also in `fpylll`), there are explicit `long(...)` calls being done on Sage types. In the light of Python 3, these should be removed or replaced with `int(...)`.
+In various places in Sage, there are explicit `long(...)` calls being done. Some of them are superfluous or can be replaced by `int`.
+
+We don't try to fix all `long()` calls, just a few obviously avoidable ones.
``````




---

archive/issue_comments_435066.json:
```json
{
    "body": "Branch: **[u/jdemeyer/remove_some_long___calls](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_some_long___calls)**",
    "created_at": "2019-05-16T13:27:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27839#issuecomment-435066",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/remove_some_long___calls](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_some_long___calls)**



---

archive/issue_comments_435067.json:
```json
{
    "body": "Commit: **[acb54fa](https://github.com/sagemath/sagetrac-mirror/commit/acb54fac7d059c81ceb406e9a2f1704b69f18ed1)**",
    "created_at": "2019-05-16T13:31:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27839#issuecomment-435067",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[acb54fa](https://github.com/sagemath/sagetrac-mirror/commit/acb54fac7d059c81ceb406e9a2f1704b69f18ed1)**



---

archive/issue_events_363652.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2019-05-16T13:31:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27839#event-363652"
}
```



---

archive/issue_comments_435068.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/acb54fac7d059c81ceb406e9a2f1704b69f18ed1\">acb54fa</a></td><td><code>Avoid a few long() calls</code></td></tr></table>\n",
    "created_at": "2019-05-16T13:31:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27839#issuecomment-435068",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'>Comment 5:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/acb54fac7d059c81ceb406e9a2f1704b69f18ed1">acb54fa</a></td><td><code>Avoid a few long() calls</code></td></tr></table>




---

archive/issue_comments_435069.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nLooks good to me.\nGreen bot => positive review.",
    "created_at": "2019-05-16T14:39:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27839#issuecomment-435069",
    "user": "https://github.com/vinklein"
}
```

<a id='comment:6'>Comment 6:</a>
Looks good to me.
Green bot => positive review.



---

archive/issue_comments_435070.json:
```json
{
    "body": "Reviewer: **Vincent Klein**",
    "created_at": "2019-05-16T14:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27839#issuecomment-435070",
    "user": "https://github.com/vinklein"
}
```

Reviewer: **Vincent Klein**



---

archive/issue_events_363653.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2019-05-17T05:28:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27839#event-363653"
}
```



---

archive/issue_events_363654.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2019-05-17T05:28:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27839#event-363654"
}
```



---

archive/issue_comments_435071.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/remove_some_long___calls](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_some_long___calls)** to **[acb54fa](https://github.com/sagemath/sagetrac-mirror/commit/acb54fac7d059c81ceb406e9a2f1704b69f18ed1)**",
    "created_at": "2019-05-21T12:15:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27839#issuecomment-435071",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/remove_some_long___calls](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_some_long___calls)** to **[acb54fa](https://github.com/sagemath/sagetrac-mirror/commit/acb54fac7d059c81ceb406e9a2f1704b69f18ed1)**



---

archive/issue_events_363655.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-21T12:15:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27839#event-363655"
}
```



---

archive/issue_events_363656.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ef24a11f75fe247cf12fbcbc6327e8449c047e94",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-05-21T12:15:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27839",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27839#event-363656"
}
```
