# Issue 27007: py3: avoid .vertices() in planarity.pyx

archive/issues_026770.json:
```json
{
    "assignees": [],
    "body": "We change the mappings used in method `is_planar` from file `planarity.pyx` to avoid calls to `.vertices()`.\n\nThe changes done here were inducing a doctest error in method `genus` for\n\n```\nsage: cube.genus(circular=['01','10'], on_embedding=True)\n```\nAs the documentation indicates that <code>If \\`\\`circular\\`\\` is defined, \\`\\`on_embedding\\`\\` is not a valid option</code>, we now raise an error in this situation.\n\nThis patch fixes some doctests that where failing with py3 in methods\n- `TutteGraph` of `smallgraphs.py`\n- `apex_vertices` of `graph.py`\n- `genus` of `generic_graph.py`\n\nCC:  @tscrim @fchapoton\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[536843f](https://github.com/sagemath/sagetrac-mirror/commit/536843ff03c5d740b55c481ebf84636bb1b1ca53)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27007_\n\n",
    "closed_at": "2019-01-22T12:40:16Z",
    "created_at": "2019-01-03T13:00:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: avoid .vertices() in planarity.pyx",
    "type": "issue",
    "updated_at": "2019-01-22T12:40:16Z",
    "url": "https://github.com/sagemath/sage/issues/27007",
    "user": "https://github.com/dcoudert"
}
```
We change the mappings used in method `is_planar` from file `planarity.pyx` to avoid calls to `.vertices()`.

The changes done here were inducing a doctest error in method `genus` for

```
sage: cube.genus(circular=['01','10'], on_embedding=True)
```
As the documentation indicates that <code>If \`\`circular\`\` is defined, \`\`on_embedding\`\` is not a valid option</code>, we now raise an error in this situation.

This patch fixes some doctests that where failing with py3 in methods
- `TutteGraph` of `smallgraphs.py`
- `apex_vertices` of `graph.py`
- `genus` of `generic_graph.py`

CC:  @tscrim @fchapoton

Keywords: **py3, graph**

Branch/Commit: **[536843f](https://github.com/sagemath/sagetrac-mirror/commit/536843ff03c5d740b55c481ebf84636bb1b1ca53)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/27007_





---

archive/issue_events_340445.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-03T13:00:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340445"
}
```



---

archive/issue_events_340446.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-03T13:00:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340446"
}
```



---

archive/issue_events_340447.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-03T13:00:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340447"
}
```



---

archive/issue_events_340448.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-03T13:00:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340448"
}
```



---

archive/issue_comments_419292.json:
```json
{
    "body": "Branch: **[public/27007_planarity](https://github.com/sagemath/sagetrac-mirror/tree/public/27007_planarity)**",
    "created_at": "2019-01-03T13:03:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27007#issuecomment-419292",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/27007_planarity](https://github.com/sagemath/sagetrac-mirror/tree/public/27007_planarity)**



---

archive/issue_comments_419293.json:
```json
{
    "body": "Commit: **[536843f](https://github.com/sagemath/sagetrac-mirror/commit/536843ff03c5d740b55c481ebf84636bb1b1ca53)**",
    "created_at": "2019-01-03T13:03:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27007#issuecomment-419293",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[536843f](https://github.com/sagemath/sagetrac-mirror/commit/536843ff03c5d740b55c481ebf84636bb1b1ca53)**



---

archive/issue_comments_419294.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/536843ff03c5d740b55c481ebf84636bb1b1ca53\">536843f</a></td><td><code>trac #27007: avoid .vertices() in planarity.pyx</code></td></tr></table>\n",
    "created_at": "2019-01-03T13:03:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27007#issuecomment-419294",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/536843ff03c5d740b55c481ebf84636bb1b1ca53">536843f</a></td><td><code>trac #27007: avoid .vertices() in planarity.pyx</code></td></tr></table>




---

archive/issue_comments_419295.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,4 +5,9 @@\n ```\n sage: cube.genus(circular=['01','10'], on_embedding=True)\n ```\n-As the documentation indicates that \"If ``circular`` is defined, ``on_embedding`` is not a valid option\", we now raise an error.\n+As the documentation indicates that <code>If \\`\\`circular\\`\\` is defined, \\`\\`on_embedding\\`\\` is not a valid option</code>, we now raise an error in this situation.\n+\n+This patch fixes some doctests that where failing with py3 in methods\n+- `TutteGraph` of `smallgraphs.py`\n+- `apex_vertices` of `graph.py`\n+- `genus` of `generic_graph.py`\n``````\n",
    "created_at": "2019-01-03T13:34:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27007#issuecomment-419295",
    "user": "https://github.com/dcoudert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,4 +5,9 @@
 ```
 sage: cube.genus(circular=['01','10'], on_embedding=True)
 ```
-As the documentation indicates that "If ``circular`` is defined, ``on_embedding`` is not a valid option", we now raise an error.
+As the documentation indicates that <code>If \`\`circular\`\` is defined, \`\`on_embedding\`\` is not a valid option</code>, we now raise an error in this situation.
+
+This patch fixes some doctests that where failing with py3 in methods
+- `TutteGraph` of `smallgraphs.py`
+- `apex_vertices` of `graph.py`
+- `genus` of `generic_graph.py`
``````




---

archive/issue_events_340449.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-03T13:34:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340449"
}
```



---

archive/issue_comments_419296.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLGTM.",
    "created_at": "2019-01-03T18:06:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27007#issuecomment-419296",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
LGTM.



---

archive/issue_comments_419297.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-01-03T18:06:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27007#issuecomment-419297",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_340450.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-01-03T18:06:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340450"
}
```



---

archive/issue_events_340451.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-01-03T18:06:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340451"
}
```



---

archive/issue_events_340452.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-15T18:16:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340452"
}
```



---

archive/issue_events_340453.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-15T18:16:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340453"
}
```



---

archive/issue_comments_419298.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nRetarging tickets optimistically to the next milestone.  If you are responsible for this ticket (either its reporter or owner) and don't believe you are likely to complete this ticket before the next release (8.7) please retarget this ticket's milestone to sage-pending or sage-wishlist.",
    "created_at": "2019-01-15T18:16:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27007#issuecomment-419298",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'>Comment 4:</a>
Retarging tickets optimistically to the next milestone.  If you are responsible for this ticket (either its reporter or owner) and don't believe you are likely to complete this ticket before the next release (8.7) please retarget this ticket's milestone to sage-pending or sage-wishlist.



---

archive/issue_comments_419299.json:
```json
{
    "body": "Changed branch from **[public/27007_planarity](https://github.com/sagemath/sagetrac-mirror/tree/public/27007_planarity)** to **[536843f](https://github.com/sagemath/sagetrac-mirror/commit/536843ff03c5d740b55c481ebf84636bb1b1ca53)**",
    "created_at": "2019-01-22T12:40:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27007#issuecomment-419299",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/27007_planarity](https://github.com/sagemath/sagetrac-mirror/tree/public/27007_planarity)** to **[536843f](https://github.com/sagemath/sagetrac-mirror/commit/536843ff03c5d740b55c481ebf84636bb1b1ca53)**



---

archive/issue_events_340454.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-01-22T12:40:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340454"
}
```



---

archive/issue_events_340455.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "010e1e893b1bed3129cc30a82b599e8ed948d66c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-01-22T12:40:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27007",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27007#event-340455"
}
```
