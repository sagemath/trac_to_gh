# Issue 27179: py3: fix doctest in generic_graph (part 2) -- cycle_basis

archive/issues_026942.json:
```json
{
    "assignees": [],
    "body": "Some difficulties are that 1) the method of `networkx` that we use can either return cycle `[0, 1, 2, 3, 0]` or cycle `[0, 3, 2, 1, 0]`, and 2) the cycle basis is not unique. So we mark doctests as py3 or py3.\n\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[8136fe7](https://github.com/sagemath/sagetrac-mirror/commit/8136fe70ee76b1bd5095d65ea56656b70b5677ae)**\n\nReviewer: **Jean-Florent Raymond**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27179_\n\n",
    "closed_at": "2019-02-26T23:36:49Z",
    "created_at": "2019-01-31T10:19:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fix doctest in generic_graph (part 2) -- cycle_basis",
    "type": "issue",
    "updated_at": "2019-02-26T23:36:49Z",
    "url": "https://github.com/sagemath/sage/issues/27179",
    "user": "https://github.com/dcoudert"
}
```
Some difficulties are that 1) the method of `networkx` that we use can either return cycle `[0, 1, 2, 3, 0]` or cycle `[0, 3, 2, 1, 0]`, and 2) the cycle basis is not unique. So we mark doctests as py3 or py3.


Keywords: **py3, graph**

Branch/Commit: **[8136fe7](https://github.com/sagemath/sagetrac-mirror/commit/8136fe70ee76b1bd5095d65ea56656b70b5677ae)**

Reviewer: **Jean-Florent Raymond**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/27179_





---

archive/issue_events_355288.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-31T10:19:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27179#event-355288"
}
```



---

archive/issue_events_355289.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-31T10:19:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27179#event-355289"
}
```



---

archive/issue_events_355290.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-31T10:19:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27179#event-355290"
}
```



---

archive/issue_events_355291.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-31T10:19:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27179#event-355291"
}
```



---

archive/issue_comments_422316.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7995a07e0eb51747cc944d18e6ddb12a93dd1837\">7995a07</a></td><td><code>trac #27179: fix doctests in cycle_basis</code></td></tr></table>\n",
    "created_at": "2019-01-31T10:21:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27179#issuecomment-422316",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7995a07e0eb51747cc944d18e6ddb12a93dd1837">7995a07</a></td><td><code>trac #27179: fix doctests in cycle_basis</code></td></tr></table>




---

archive/issue_events_355292.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-31T10:21:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27179#event-355292"
}
```



---

archive/issue_comments_422317.json:
```json
{
    "body": "Commit: **[7995a07](https://github.com/sagemath/sagetrac-mirror/commit/7995a07e0eb51747cc944d18e6ddb12a93dd1837)**",
    "created_at": "2019-01-31T10:21:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27179#issuecomment-422317",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[7995a07](https://github.com/sagemath/sagetrac-mirror/commit/7995a07e0eb51747cc944d18e6ddb12a93dd1837)**



---

archive/issue_comments_422318.json:
```json
{
    "body": "Branch: **[public/27179_cycle_basis](https://github.com/sagemath/sagetrac-mirror/tree/public/27179_cycle_basis)**",
    "created_at": "2019-01-31T10:21:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27179#issuecomment-422318",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/27179_cycle_basis](https://github.com/sagemath/sagetrac-mirror/tree/public/27179_cycle_basis)**



---

archive/issue_comments_422319.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8136fe70ee76b1bd5095d65ea56656b70b5677ae\">8136fe7</a></td><td><code>trac #27179: revert suppression of empty line to avoid conflict with 27180</code></td></tr></table>\n",
    "created_at": "2019-01-31T10:38:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27179#issuecomment-422319",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8136fe70ee76b1bd5095d65ea56656b70b5677ae">8136fe7</a></td><td><code>trac #27179: revert suppression of empty line to avoid conflict with 27180</code></td></tr></table>




---

archive/issue_comments_422320.json:
```json
{
    "body": "Changed commit from **[7995a07](https://github.com/sagemath/sagetrac-mirror/commit/7995a07e0eb51747cc944d18e6ddb12a93dd1837)** to **[8136fe7](https://github.com/sagemath/sagetrac-mirror/commit/8136fe70ee76b1bd5095d65ea56656b70b5677ae)**",
    "created_at": "2019-01-31T10:38:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27179#issuecomment-422320",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7995a07](https://github.com/sagemath/sagetrac-mirror/commit/7995a07e0eb51747cc944d18e6ddb12a93dd1837)** to **[8136fe7](https://github.com/sagemath/sagetrac-mirror/commit/8136fe70ee76b1bd5095d65ea56656b70b5677ae)**



---

archive/issue_comments_422321.json:
```json
{
    "body": "Reviewer: **Jean-Florent Raymond**",
    "created_at": "2019-02-24T10:45:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27179#issuecomment-422321",
    "user": "https://github.com/jfraymond"
}
```

Reviewer: **Jean-Florent Raymond**



---

archive/issue_events_355293.json:
```json
{
    "actor": "https://github.com/jfraymond",
    "created_at": "2019-02-24T10:45:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27179#event-355293"
}
```



---

archive/issue_events_355294.json:
```json
{
    "actor": "https://github.com/jfraymond",
    "created_at": "2019-02-24T10:45:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27179#event-355294"
}
```



---

archive/issue_comments_422322.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nOK",
    "created_at": "2019-02-24T10:45:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27179#issuecomment-422322",
    "user": "https://github.com/jfraymond"
}
```

<div id="comment:3" align="right">Comment 3</div>

OK



---

archive/issue_events_355295.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-02-26T23:36:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27179#event-355295"
}
```



---

archive/issue_events_355296.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5fb4d426a23da0488943d123e0c68056f9595b73",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-02-26T23:36:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27179#event-355296"
}
```



---

archive/issue_comments_422323.json:
```json
{
    "body": "Changed branch from **[public/27179_cycle_basis](https://github.com/sagemath/sagetrac-mirror/tree/public/27179_cycle_basis)** to **[8136fe7](https://github.com/sagemath/sagetrac-mirror/commit/8136fe70ee76b1bd5095d65ea56656b70b5677ae)**",
    "created_at": "2019-02-26T23:36:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27179#issuecomment-422323",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/27179_cycle_basis](https://github.com/sagemath/sagetrac-mirror/tree/public/27179_cycle_basis)** to **[8136fe7](https://github.com/sagemath/sagetrac-mirror/commit/8136fe70ee76b1bd5095d65ea56656b70b5677ae)**
