...
<a id='comment:32'></a>Replying to [dimpase](#comment%3A30):
> sage -i mpir being installed from sources no matter what.
...
```json
{
    "body": "<a id='comment:40'></a>Rebased. I believe that the way of[comment:34](#comment%3A34) (with ./configure run before or after the build - to me it's more natural before) is implementable via rules in build/make/Makefile.in, but I'd rather have Erik on it.\n\n(Or I can do it, but then I first need more info on the inner working of that .dummy targets...)",
...
<a id='comment:40'></a>Rebased. I believe that the way of[comment:34](#comment%3A34) (with ./configure run before or after the build - to me it's more natural before) is implementable via rules in build/make/Makefile.in, but I'd rather have Erik on it.
...
<a id='comment:41'></a>Replying to [jdemeyer](#comment%3A37):
> Replying to [embray](#comment%3A36):
> > I think you might still have things backwards: What I'm proposing is that configure should be re-run *before* installing the SPKG.  That makes it clear, from the Makefile on down, that our build is configured to use the mpir SPKG.
...
<a id='comment:55'></a>Replying to [dimpase](#comment%3A53):
> `sage -i mpir` triggers a rebuild of its dependencies without proper changes in the environment,
> so the dependencies will get rebuild against the external "mpir/gmp".
...
<a id='comment:66'></a>Replying to [fbissey](#comment%3A65):
> I guess it will take some time to digest properly.
