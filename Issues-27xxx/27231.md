...
<a id='comment:10'></a>Replying to [dcoudert](#comment%3A5):
>  You can add a test to show that 1) `directg/watercluster2` generates more graphs than what we get with `orientations`,  and 2) that each digraph obtained with `orientations` is in the set of digraphs obtained with `directg`.
...
```json
{
    "body": "<a id='comment:12'></a>Reviewer's comments implemented apart from comparison with `orientations`'s set (see[comment:10](#comment%3A10)).",
...
<a id='comment:12'></a>Reviewer's comments implemented apart from comparison with `orientations`'s set (see[comment:10](#comment%3A10)).
