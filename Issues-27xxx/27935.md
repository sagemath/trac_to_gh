# Issue 27935: A regression in divisors of function fields

archive/issues_027698.json:
```json
{
    "assignees": [],
    "body": "The ticket  #27607 introduced a regression: function field divisors are not hashable anymore! \n\nAs of sage 8.8.beta7:              \n\n```\nsage: K.<x> = FunctionField(GF(2)); R.<t> = K[]\nsage: L.<y> = K.extension(t^3 + x^3*t + x)\nsage: f = x/(y+1)\nsage: f.divisor()     \n- Place (1/x, 1/x^3*y^2 + 1/x)\n + Place (1/x, 1/x^3*y^2 + 1/x^2*y + 1)\n + 3*Place (x, y)\n - Place (x^3 + x + 1, y + 1)\nsage: d = _\nsage: {d: 1}\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n...\nTypeError: <class 'sage.rings.function_field.divisor.DivisorGroup\n_with_category.element_class'> is not hashable\n\n```\n\nBranch/Commit: **[3c41314](https://github.com/sagemath/sagetrac-mirror/commit/3c41314266709879ffa0b2a6f12d4d22a65328b2)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Kwankyu Lee**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27935_\n\n",
    "closed_at": "2019-06-06T22:26:16Z",
    "created_at": "2019-06-05T11:46:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "A regression in divisors of function fields",
    "type": "issue",
    "updated_at": "2019-06-06T22:26:16Z",
    "url": "https://github.com/sagemath/sage/issues/27935",
    "user": "https://github.com/kwankyu"
}
```
The ticket  #27607 introduced a regression: function field divisors are not hashable anymore! 

As of sage 8.8.beta7:              

```
sage: K.<x> = FunctionField(GF(2)); R.<t> = K[]
sage: L.<y> = K.extension(t^3 + x^3*t + x)
sage: f = x/(y+1)
sage: f.divisor()     
- Place (1/x, 1/x^3*y^2 + 1/x)
 + Place (1/x, 1/x^3*y^2 + 1/x^2*y + 1)
 + 3*Place (x, y)
 - Place (x^3 + x + 1, y + 1)
sage: d = _
sage: {d: 1}
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
...
TypeError: <class 'sage.rings.function_field.divisor.DivisorGroup
_with_category.element_class'> is not hashable

```

Branch/Commit: **[3c41314](https://github.com/sagemath/sagetrac-mirror/commit/3c41314266709879ffa0b2a6f12d4d22a65328b2)**

Reviewer: **Travis Scrimshaw**

Author: **Kwankyu Lee**

_Issue created by migration from https://trac.sagemath.org/ticket/27935_





---

archive/issue_events_364856.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2019-06-05T11:46:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27935#event-364856"
}
```



---

archive/issue_events_364857.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2019-06-05T11:46:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27935#event-364857"
}
```



---

archive/issue_events_364858.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2019-06-05T11:46:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27935#event-364858"
}
```



---

archive/issue_events_364859.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2019-06-05T11:46:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27935#event-364859"
}
```



---

archive/issue_comments_437044.json:
```json
{
    "body": "Branch: **[u/klee/27935](https://github.com/sagemath/sagetrac-mirror/tree/u/klee/27935)**",
    "created_at": "2019-06-05T11:48:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27935#issuecomment-437044",
    "user": "https://github.com/kwankyu"
}
```

Branch: **[u/klee/27935](https://github.com/sagemath/sagetrac-mirror/tree/u/klee/27935)**



---

archive/issue_comments_437045.json:
```json
{
    "body": "Author: **Kwankyu Lee**",
    "created_at": "2019-06-05T11:48:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27935#issuecomment-437045",
    "user": "https://github.com/kwankyu"
}
```

Author: **Kwankyu Lee**



---

archive/issue_comments_437046.json:
```json
{
    "body": "Commit: **[3c41314](https://github.com/sagemath/sagetrac-mirror/commit/3c41314266709879ffa0b2a6f12d4d22a65328b2)**",
    "created_at": "2019-06-05T11:48:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27935#issuecomment-437046",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[3c41314](https://github.com/sagemath/sagetrac-mirror/commit/3c41314266709879ffa0b2a6f12d4d22a65328b2)**



---

archive/issue_comments_437047.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3c41314266709879ffa0b2a6f12d4d22a65328b2\">3c41314</a></td><td><code>Fix a regression in divisor</code></td></tr></table>\n",
    "created_at": "2019-06-05T11:48:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27935#issuecomment-437047",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3c41314266709879ffa0b2a6f12d4d22a65328b2">3c41314</a></td><td><code>Fix a regression in divisor</code></td></tr></table>




---

archive/issue_events_364860.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2019-06-05T11:55:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27935#event-364860"
}
```



---

archive/issue_comments_437048.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,22 @@\n-The ticket  #27607 introduced a regression: function field divisors are not hashable anymore!\n+The ticket  #27607 introduced a regression: function field divisors are not hashable anymore! \n+\n+As of sage 8.8.beta7:              \n+\n+```\n+sage: K.<x> = FunctionField(GF(2)); R.<t> = K[]\n+sage: L.<y> = K.extension(t^3 + x^3*t + x)\n+sage: f = x/(y+1)\n+sage: f.divisor()     \n+- Place (1/x, 1/x^3*y^2 + 1/x)\n+ + Place (1/x, 1/x^3*y^2 + 1/x^2*y + 1)\n+ + 3*Place (x, y)\n+ - Place (x^3 + x + 1, y + 1)\n+sage: d = _\n+sage: {d: 1}\n+---------------------------------------------------------------------------\n+TypeError                                 Traceback (most recent call last)\n+...\n+TypeError: <class 'sage.rings.function_field.divisor.DivisorGroup\n+_with_category.element_class'> is not hashable\n+\n+```\n``````\n",
    "created_at": "2019-06-05T11:55:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27935#issuecomment-437048",
    "user": "https://github.com/kwankyu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,22 @@
-The ticket  #27607 introduced a regression: function field divisors are not hashable anymore!
+The ticket  #27607 introduced a regression: function field divisors are not hashable anymore! 
+
+As of sage 8.8.beta7:              
+
+```
+sage: K.<x> = FunctionField(GF(2)); R.<t> = K[]
+sage: L.<y> = K.extension(t^3 + x^3*t + x)
+sage: f = x/(y+1)
+sage: f.divisor()     
+- Place (1/x, 1/x^3*y^2 + 1/x)
+ + Place (1/x, 1/x^3*y^2 + 1/x^2*y + 1)
+ + 3*Place (x, y)
+ - Place (x^3 + x + 1, y + 1)
+sage: d = _
+sage: {d: 1}
+---------------------------------------------------------------------------
+TypeError                                 Traceback (most recent call last)
+...
+TypeError: <class 'sage.rings.function_field.divisor.DivisorGroup
+_with_category.element_class'> is not hashable
+
+```
``````




---

archive/issue_comments_437049.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-06-05T12:24:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27935#issuecomment-437049",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_364861.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-06-05T12:24:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27935#event-364861"
}
```



---

archive/issue_events_364862.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-06-05T12:24:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27935#event-364862"
}
```



---

archive/issue_comments_437050.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nWhoops, sorry I missed that the `__hash__` had been removed.",
    "created_at": "2019-06-05T12:24:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27935#issuecomment-437050",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

Whoops, sorry I missed that the `__hash__` had been removed.



---

archive/issue_comments_437051.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nYeah, I missed that too.\n\nIt passed all the tests because the only test that checked it had been deleted along with the code.",
    "created_at": "2019-06-05T15:51:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27935#issuecomment-437051",
    "user": "https://github.com/BrentBaccala"
}
```

<div id="comment:5" align="right">Comment 5</div>

Yeah, I missed that too.

It passed all the tests because the only test that checked it had been deleted along with the code.



---

archive/issue_comments_437052.json:
```json
{
    "body": "Changed branch from **[u/klee/27935](https://github.com/sagemath/sagetrac-mirror/tree/u/klee/27935)** to **[3c41314](https://github.com/sagemath/sagetrac-mirror/commit/3c41314266709879ffa0b2a6f12d4d22a65328b2)**",
    "created_at": "2019-06-06T22:26:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27935#issuecomment-437052",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/klee/27935](https://github.com/sagemath/sagetrac-mirror/tree/u/klee/27935)** to **[3c41314](https://github.com/sagemath/sagetrac-mirror/commit/3c41314266709879ffa0b2a6f12d4d22a65328b2)**



---

archive/issue_events_364863.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-06-06T22:26:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27935#event-364863"
}
```



---

archive/issue_events_364864.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "12d8480936a4aea35c181787484ee75c5fd19863",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-06-06T22:26:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27935#event-364864"
}
```
