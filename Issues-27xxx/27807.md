# Issue 27807: Polymake-jupymake interface: Fix polymake's error handling in Shell::Mock

archive/issues_027570.json:
```json
{
    "assignees": [],
    "body": "This is a follow-up on #22704.\n\nA patch suggested by Benjamin Lorenz fixes the following errors in error handling.\n\n```\nIn [1]: import JuPyMake\nIn [2]: JuPyMake.InitializePolymake()\nOut[2]: True\nIn [4]: JuPyMake.ExecuteCommand('application(\"xyzzy\");')\nOut[4]:\n(True,\n '',\n '',\n 'Can\\'t locate object method \"compile_scope\" via package\n\"Polymake::Core::Shell::Mock\" at\n/Users/mkoeppe/s/sage/sage-rebasing/local/share/polymake/perllib/Polymake/utils.pl\nline 247.\\n')\nIn [6]: JuPyMake.ExecuteCommand('save(17);')\nOut[6]:\n(True,\n '',\n '',\n 'Can\\'t locate object method \"compile_scope\" via package\n\"Polymake::Core::Shell::Mock\" at\n/Users/mkoeppe/s/sage/sage-rebasing/local/share/polymake/perllib/Polymake/utils.pl\nline 247.\\n')\n```\n\n\nDepends on #22704\n\nCC:  @dimpase lorenz@math.tu-berlin.de\n\nBranch/Commit: **[799e6a4](https://github.com/sagemath/sagetrac-mirror/commit/799e6a4f581a8256b4c9a38faeb1ba0ac342708c)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27807_\n\n",
    "closed_at": "2019-05-14T12:37:14Z",
    "created_at": "2019-05-09T16:00:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Polymake-jupymake interface: Fix polymake's error handling in Shell::Mock",
    "type": "issue",
    "updated_at": "2019-05-14T12:37:14Z",
    "url": "https://github.com/sagemath/sage/issues/27807",
    "user": "https://github.com/mkoeppe"
}
```
This is a follow-up on #22704.

A patch suggested by Benjamin Lorenz fixes the following errors in error handling.

```
In [1]: import JuPyMake
In [2]: JuPyMake.InitializePolymake()
Out[2]: True
In [4]: JuPyMake.ExecuteCommand('application("xyzzy");')
Out[4]:
(True,
 '',
 '',
 'Can\'t locate object method "compile_scope" via package
"Polymake::Core::Shell::Mock" at
/Users/mkoeppe/s/sage/sage-rebasing/local/share/polymake/perllib/Polymake/utils.pl
line 247.\n')
In [6]: JuPyMake.ExecuteCommand('save(17);')
Out[6]:
(True,
 '',
 '',
 'Can\'t locate object method "compile_scope" via package
"Polymake::Core::Shell::Mock" at
/Users/mkoeppe/s/sage/sage-rebasing/local/share/polymake/perllib/Polymake/utils.pl
line 247.\n')
```


Depends on #22704

CC:  @dimpase lorenz@math.tu-berlin.de

Branch/Commit: **[799e6a4](https://github.com/sagemath/sagetrac-mirror/commit/799e6a4f581a8256b4c9a38faeb1ba0ac342708c)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/27807_





---

archive/issue_events_350366.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-09T16:00:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27807#event-350366"
}
```



---

archive/issue_events_350367.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-09T16:00:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27807#event-350367"
}
```



---

archive/issue_events_350368.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-09T16:00:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27807#event-350368"
}
```



---

archive/issue_events_350369.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-09T16:00:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27807#event-350369"
}
```



---

archive/issue_comments_434398.json:
```json
{
    "body": "Branch: **[u/mkoeppe/polymake_jupymake_interface__fix_polymake_s_error_handling_in_shell__mock](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/polymake_jupymake_interface__fix_polymake_s_error_handling_in_shell__mock)**",
    "created_at": "2019-05-10T13:06:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27807#issuecomment-434398",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/polymake_jupymake_interface__fix_polymake_s_error_handling_in_shell__mock](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/polymake_jupymake_interface__fix_polymake_s_error_handling_in_shell__mock)**



---

archive/issue_comments_434399.json:
```json
{
    "body": "Commit: **[799e6a4](https://github.com/sagemath/sagetrac-mirror/commit/799e6a4f581a8256b4c9a38faeb1ba0ac342708c)**",
    "created_at": "2019-05-10T13:10:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27807#issuecomment-434399",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[799e6a4](https://github.com/sagemath/sagetrac-mirror/commit/799e6a4f581a8256b4c9a38faeb1ba0ac342708c)**



---

archive/issue_comments_434400.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/799e6a4f581a8256b4c9a38faeb1ba0ac342708c\">799e6a4</a></td><td><code>polymake doctest: Remove 'known bug' annotation</code></td></tr></table>\n",
    "created_at": "2019-05-10T13:10:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27807#issuecomment-434400",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/799e6a4f581a8256b4c9a38faeb1ba0ac342708c">799e6a4</a></td><td><code>polymake doctest: Remove 'known bug' annotation</code></td></tr></table>




---

archive/issue_comments_434401.json:
```json
{
    "body": "Dependencies: **#22704**",
    "created_at": "2019-05-10T13:11:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27807#issuecomment-434401",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#22704**



---

archive/issue_comments_434402.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,6 @@\n This is a follow-up on #22704.\n+\n+A patch suggested by Benjamin Lorenz fixes the following errors in error handling.\n \n ```\n In [1]: import JuPyMake\n``````\n",
    "created_at": "2019-05-10T13:11:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27807#issuecomment-434402",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,6 @@
 This is a follow-up on #22704.
+
+A patch suggested by Benjamin Lorenz fixes the following errors in error handling.
 
 ```
 In [1]: import JuPyMake
``````




---

archive/issue_events_350370.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-10T13:11:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27807#event-350370"
}
```



---

archive/issue_comments_434403.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2019-05-10T13:11:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27807#issuecomment-434403",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_434404.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nOK, good, seems to be fixed.",
    "created_at": "2019-05-10T15:39:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27807#issuecomment-434404",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:4'>Comment 4:</a>
OK, good, seems to be fixed.



---

archive/issue_events_350371.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-10T15:39:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27807#event-350371"
}
```



---

archive/issue_events_350372.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-10T15:39:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27807#event-350372"
}
```



---

archive/issue_comments_434405.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2019-05-10T15:39:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27807#issuecomment-434405",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_350373.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-14T12:37:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27807#event-350373"
}
```



---

archive/issue_events_350374.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a9f92c4fa6401b2b77fb4847b19f2ea145f95941",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-05-14T12:37:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27807#event-350374"
}
```



---

archive/issue_comments_434406.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/polymake_jupymake_interface__fix_polymake_s_error_handling_in_shell__mock](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/polymake_jupymake_interface__fix_polymake_s_error_handling_in_shell__mock)** to **[799e6a4](https://github.com/sagemath/sagetrac-mirror/commit/799e6a4f581a8256b4c9a38faeb1ba0ac342708c)**",
    "created_at": "2019-05-14T12:37:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27807#issuecomment-434406",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/polymake_jupymake_interface__fix_polymake_s_error_handling_in_shell__mock](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/polymake_jupymake_interface__fix_polymake_s_error_handling_in_shell__mock)** to **[799e6a4](https://github.com/sagemath/sagetrac-mirror/commit/799e6a4f581a8256b4c9a38faeb1ba0ac342708c)**
