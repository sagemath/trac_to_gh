...
<a id='comment:15'></a>Your timings in comment:8 do not (necessarily) represent a fair comparison because you have generated random elements (length matters a lot here). You have to use the same elements.
...
<a id='comment:21'></a>Replying to [comment:5 tscrim]:
> It probably is better, but I do not think they count as bugs outright. I was mainly pointing out that enforcing that will slow things down by going through `ZZ`.
...
<a id='comment:32'></a>Replying to [comment:30 mantepse]:
> Replying to [comment:28 tscrim]:
> > I think a better error message for the `map(ZZ, lst)` would be something like
> > 
> > ```
...
<a id='comment:39'></a>Replying to [comment:38 mantepse]:
> Replying to [comment:37 vdelecroix]:
> 
> > Or even better:
> > 
...
<a id='comment:41'></a>Replying to [comment:36 mantepse]:
> Replying to [comment:34 vdelecroix]:
> > I don't understand part of your change
> > ...
...
```json
{
    "body": "<a id='comment:46'></a>That is why I said you should check `x in ZZ` in comment:28.",
...
<a id='comment:46'></a>That is why I said you should check `x in ZZ` in comment:28.
...
<a id='comment:47'></a>Replying to [comment:41 mantepse]:
> Replying to [comment:36 mantepse]:
> > Replying to [comment:34 vdelecroix]:
> > > I don't understand part of your change
> > > ...
...
<a id='comment:48'></a>Replying to [comment:44 tscrim]:
> I also suspect Python does some stuff lazily with list copying, so `x[1:]` doesn't actually do any copying until you modify the `x[1:]` list.
