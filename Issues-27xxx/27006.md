# Issue 27006: cannot convert back polynomials from libgap

archive/issues_026769.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: x=polygen(QQ,'x')\nsage: f=x+1\nsage: g=libgap(f)\nsage: g\nx+1\nsage: g.sage()\n---------------------------------------------------------------------------\nNotImplementedError                       Traceback (most recent call last)\n<ipython-input-71-2462cdd49bf3> in <module>()\n----> 1 g.sage()\n\n/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/libs/gap/element.pyx in sage.libs.gap.element.GapElement.sage (build/cythonized/sage/libs/gap/element.c:12055)()\n   1209             return -Infinity\n   1210 \n-> 1211         raise NotImplementedError('cannot construct equivalent Sage object')\n   1212 \n   1213 \n\nNotImplementedError: cannot construct equivalent Sage object\n```\n\nCC:  @embray\n\nKeywords: **libgap**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27006_\n\n",
    "closed_at": "2019-01-03T17:47:01Z",
    "created_at": "2019-01-03T10:55:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "cannot convert back polynomials from libgap",
    "type": "issue",
    "updated_at": "2019-01-03T17:47:01Z",
    "url": "https://github.com/sagemath/sage/issues/27006",
    "user": "https://github.com/fchapoton"
}
```

```
sage: x=polygen(QQ,'x')
sage: f=x+1
sage: g=libgap(f)
sage: g
x+1
sage: g.sage()
---------------------------------------------------------------------------
NotImplementedError                       Traceback (most recent call last)
<ipython-input-71-2462cdd49bf3> in <module>()
----> 1 g.sage()

/home/chapoton/sage3/local/lib/python3.6/site-packages/sage/libs/gap/element.pyx in sage.libs.gap.element.GapElement.sage (build/cythonized/sage/libs/gap/element.c:12055)()
   1209             return -Infinity
   1210 
-> 1211         raise NotImplementedError('cannot construct equivalent Sage object')
   1212 
   1213 

NotImplementedError: cannot construct equivalent Sage object
```

CC:  @embray

Keywords: **libgap**

_Issue created by migration from https://trac.sagemath.org/ticket/27006_





---

archive/issue_events_352962.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-03T10:55:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352962"
}
```



---

archive/issue_events_352963.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-03T10:55:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352963"
}
```



---

archive/issue_events_352964.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-03T10:55:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352964"
}
```



---

archive/issue_events_352965.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-03T10:55:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352965"
}
```



---

archive/issue_events_352966.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-03T14:15:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352966"
}
```



---

archive/issue_events_352967.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-03T14:15:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352967"
}
```



---

archive/issue_events_352968.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-03T14:15:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352968"
}
```



---

archive/issue_comments_419289.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nduplicate of #21020, please close",
    "created_at": "2019-01-03T14:15:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27006#issuecomment-419289",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

duplicate of #21020, please close



---

archive/issue_events_352969.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-01-03T14:15:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352969"
}
```



---

archive/issue_events_352970.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-03T17:47:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352970"
}
```



---

archive/issue_events_352971.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-03T17:47:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352971"
}
```



---

archive/issue_events_352972.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-03T17:47:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27006",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27006#event-352972"
}
```
