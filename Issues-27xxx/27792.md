# Issue 27792: RIF encapusulated in SR fails comprison is_zero()

archive/issues_027555.json:
```json
{
    "assignees": [],
    "body": "We have\n\n```\nsage: SR(RIF(-1,1)).is_zero()\n...\nTypeError: \n```\nI.e. it fails with an error saying absolutely nothing.\n\nThis is somehow surprising as \n\n```\nsage: RIF(-1,1).is_zero()\nFalse\n```\nworks\n\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27792_\n\n",
    "created_at": "2019-05-07T11:53:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "RIF encapusulated in SR fails comprison is_zero()",
    "type": "issue",
    "updated_at": "2019-06-14T14:55:02Z",
    "url": "https://github.com/sagemath/sage/issues/27792",
    "user": "https://github.com/dkrenn"
}
```
We have

```
sage: SR(RIF(-1,1)).is_zero()
...
TypeError: 
```
I.e. it fails with an error saying absolutely nothing.

This is somehow surprising as 

```
sage: RIF(-1,1).is_zero()
False
```
works


_Issue created by migration from https://trac.sagemath.org/ticket/27792_





---

archive/issue_events_363062.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-05-07T11:53:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27792",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27792#event-363062"
}
```



---

archive/issue_events_363063.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-05-07T11:53:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27792",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27792#event-363063"
}
```



---

archive/issue_events_363064.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-05-07T11:53:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27792",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27792#event-363064"
}
```



---

archive/issue_events_363065.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-05-07T11:53:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27792",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27792#event-363065"
}
```



---

archive/issue_events_363066.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:55:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27792",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27792#event-363066"
}
```



---

archive/issue_comments_434219.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAs the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).",
    "created_at": "2019-06-14T14:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27792#issuecomment-434219",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
As the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).
