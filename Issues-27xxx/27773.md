# Issue 27773: py3: fix round issue with cplex and gurobi

archive/issues_027536.json:
```json
{
    "assignees": [],
    "body": "When cplex is installed, the following occur\n\n```\nsage: g = graphs.PetersenGraph()\nsage: g.vertex_connectivity()\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n<ipython-input-4-8e3aa0688fc2> in <module>()\n----> 1 g.vertex_connectivity()\n\n/Users/dcoudert/sage3/sage/local/lib/python3.7/site-packages/sage/graphs/connectivity.pyx in sage.graphs.connectivity.vertex_connectivity (build/cythonized/sage/graphs/connectivity.c:17319)()\n   1451 \n   1452     if value_only:\n-> 1453         return Integer(round(p.solve(objective_only=True, log=verbose)))\n   1454 \n   1455     val = Integer(round(p.solve(log=verbose)))\n\nTypeError: type sage.rings.real_double.RealDoubleElement doesn't define __round__ method\n```\n\n\n\nCC:  @seblabbe\n\nKeywords: **py3, thursdaysbdx**\n\nBranch/Commit: **[57b4539](https://github.com/sagemath/sagetrac-mirror/commit/57b4539e2e7ba00298aa5ba3cd92b0fcd47cab0a)**\n\nReviewer: **S\u00e9bastien Labb\u00e9**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27773_\n\n",
    "closed_at": "2019-05-25T08:18:29Z",
    "created_at": "2019-05-05T08:49:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fix round issue with cplex and gurobi",
    "type": "issue",
    "updated_at": "2019-05-25T08:18:29Z",
    "url": "https://github.com/sagemath/sage/issues/27773",
    "user": "https://github.com/dcoudert"
}
```
When cplex is installed, the following occur

```
sage: g = graphs.PetersenGraph()
sage: g.vertex_connectivity()
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-4-8e3aa0688fc2> in <module>()
----> 1 g.vertex_connectivity()

/Users/dcoudert/sage3/sage/local/lib/python3.7/site-packages/sage/graphs/connectivity.pyx in sage.graphs.connectivity.vertex_connectivity (build/cythonized/sage/graphs/connectivity.c:17319)()
   1451 
   1452     if value_only:
-> 1453         return Integer(round(p.solve(objective_only=True, log=verbose)))
   1454 
   1455     val = Integer(round(p.solve(log=verbose)))

TypeError: type sage.rings.real_double.RealDoubleElement doesn't define __round__ method
```



CC:  @seblabbe

Keywords: **py3, thursdaysbdx**

Branch/Commit: **[57b4539](https://github.com/sagemath/sagetrac-mirror/commit/57b4539e2e7ba00298aa5ba3cd92b0fcd47cab0a)**

Reviewer: **Sébastien Labbé**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/27773_





---

archive/issue_events_362847.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-05-05T08:49:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362847"
}
```



---

archive/issue_events_362848.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-05-05T08:49:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362848"
}
```



---

archive/issue_events_362849.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-05-05T08:49:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362849"
}
```



---

archive/issue_events_362850.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-05-05T08:49:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362850"
}
```



---

archive/issue_comments_433904.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/57b4539e2e7ba00298aa5ba3cd92b0fcd47cab0a\">57b4539</a></td><td><code>trac #27773: issue with cplex</code></td></tr></table>\n",
    "created_at": "2019-05-05T08:52:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27773#issuecomment-433904",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/57b4539e2e7ba00298aa5ba3cd92b0fcd47cab0a">57b4539</a></td><td><code>trac #27773: issue with cplex</code></td></tr></table>




---

archive/issue_comments_433905.json:
```json
{
    "body": "Branch: **[public/numerical/27773_cplex](https://github.com/sagemath/sagetrac-mirror/tree/public/numerical/27773_cplex)**",
    "created_at": "2019-05-05T08:52:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27773#issuecomment-433905",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/numerical/27773_cplex](https://github.com/sagemath/sagetrac-mirror/tree/public/numerical/27773_cplex)**



---

archive/issue_events_362851.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-05-05T08:52:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362851"
}
```



---

archive/issue_comments_433906.json:
```json
{
    "body": "Commit: **[57b4539](https://github.com/sagemath/sagetrac-mirror/commit/57b4539e2e7ba00298aa5ba3cd92b0fcd47cab0a)**",
    "created_at": "2019-05-05T08:52:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27773#issuecomment-433906",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[57b4539](https://github.com/sagemath/sagetrac-mirror/commit/57b4539e2e7ba00298aa5ba3cd92b0fcd47cab0a)**



---

archive/issue_comments_433907.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI tried to put a doctest with `sage: round(p.solve())`, but it's not using the same `round`...",
    "created_at": "2019-05-05T08:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27773#issuecomment-433907",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:2" align="right">Comment 2</div>

I tried to put a doctest with `sage: round(p.solve())`, but it's not using the same `round`...



---

archive/issue_comments_433908.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -17,3 +17,5 @@\n \n TypeError: type sage.rings.real_double.RealDoubleElement doesn't define __round__ method\n ```\n+\n+\n``````\n",
    "created_at": "2019-05-05T08:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27773#issuecomment-433908",
    "user": "https://github.com/dcoudert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -17,3 +17,5 @@
 
 TypeError: type sage.rings.real_double.RealDoubleElement doesn't define __round__ method
 ```
+
+
``````




---

archive/issue_events_362852.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-05-06T11:53:27Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "title_is": "py3: fix round issue with cplex and gurobi",
    "title_was": "py3: fix round issue with cplex",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362852"
}
```



---

archive/issue_comments_433909.json:
```json
{
    "body": "Reviewer: **S\u00e9bastien Labb\u00e9**",
    "created_at": "2019-05-23T19:05:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27773#issuecomment-433909",
    "user": "https://github.com/seblabbe"
}
```

Reviewer: **Sébastien Labbé**



---

archive/issue_events_362853.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2019-05-23T19:05:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362853"
}
```



---

archive/issue_events_362854.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2019-05-23T19:05:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362854"
}
```



---

archive/issue_comments_433910.json:
```json
{
    "body": "Changed keywords from **py3** to **py3, thursdaysbdx**",
    "created_at": "2019-05-23T19:05:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27773#issuecomment-433910",
    "user": "https://github.com/seblabbe"
}
```

Changed keywords from **py3** to **py3, thursdaysbdx**



---

archive/issue_comments_433911.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nAs a remark, I do not have cplex installed, but I confirm it works with Gurobi.",
    "created_at": "2019-05-23T19:07:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27773#issuecomment-433911",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:5" align="right">Comment 5</div>

As a remark, I do not have cplex installed, but I confirm it works with Gurobi.



---

archive/issue_events_362855.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-25T08:18:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362855"
}
```



---

archive/issue_events_362856.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1100723e76a9fafa2a5ef1f899be10f7ab32ea30",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-05-25T08:18:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27773#event-362856"
}
```



---

archive/issue_comments_433912.json:
```json
{
    "body": "Changed branch from **[public/numerical/27773_cplex](https://github.com/sagemath/sagetrac-mirror/tree/public/numerical/27773_cplex)** to **[57b4539](https://github.com/sagemath/sagetrac-mirror/commit/57b4539e2e7ba00298aa5ba3cd92b0fcd47cab0a)**",
    "created_at": "2019-05-25T08:18:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27773#issuecomment-433912",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/numerical/27773_cplex](https://github.com/sagemath/sagetrac-mirror/tree/public/numerical/27773_cplex)** to **[57b4539](https://github.com/sagemath/sagetrac-mirror/commit/57b4539e2e7ba00298aa5ba3cd92b0fcd47cab0a)**
