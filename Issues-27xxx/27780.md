# Issue 27780: _flint_rep_abs of zero causes segmentation fault

archive/issues_027543.json:
```json
{
    "assignees": [],
    "body": "On version 8.7.\nFor a p-adic extension, _flint_rep_abs of zero breaks sage:\n\n```\nsage: F=Qp(2)\nsage: L.<w> = F.ext(x^2+x+1)\nsage: L(0)._flint_rep_abs()\nsig_error() without sig_on()\n------------------------------------------------------------------------\n/home/parzan/sage-8.7/local/lib/python2.7/site-packages/cysignals/signals.so(+0x759b)[0x7fdd7629859b]\n/home/parzan/sage-8.7/local/lib/python2.7/site-packages/cysignals/signals.so(+0x7658)[0x7fdd76298658]\n/home/parzan/sage-8.7/local/lib/python2.7/site-packages/cysignals/signals.so(+0xa1a5)[0x7fdd7629b1a5]\n/lib/x86_64-linux-gnu/libc.so.6(+0x3ef20)[0x7fdd85cf0f20]\n/lib/x86_64-linux-gnu/libc.so.6(kill+0x7)[0x7fdd85cf1187]\n/home/parzan/sage-8.7/local/lib/python2.7/site-packages/sage/ext/memory.so(+0x2e46)[0x7fd57aacbe46]\n/home/parzan/sage-8.7/local/lib/libgmp.so.23(__gmpz_realloc+0x31)[0x7fdd7d50c7f1]\n/home/parzan/sage-8.7/local/lib/libgmp.so.23(__gmpz_n_pow_ui+0x77e)[0x7fdd7d508e6e]\n/home/parzan/sage-8.7/local/lib/libflint.so.13(fmpz_pow_ui+0xab)[0x7fdd6b21dcdb]\n/home/parzan/sage-8.7/local/lib/python2.7/site-packages/sage/rings/padics/pow_computer_flint.so(+0x7c4e)[0x7fd56ba56c4e]\n/home/parzan/sage-8.7/local/lib/python2.7/site-packages/sage/rings/padics/qadic_flint_CR.so(+0x19fe7)[0x7fd56acd1fe7]\n/home/parzan/sage-8.7/local/lib/python2.7/site-packages/sage/rings/padics/qadic_flint_CR.so(+0x2d1a1)[0x7fd56ace51a1]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x7f11)[0x7fdd861b51e1]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCode+0x19)[0x7fdd861b71a9]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x68b6)[0x7fdd861b3b86]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCode+0x19)[0x7fdd861b71a9]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyRun_FileExFlags+0x8a)[0x7fdd861d9eda]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyRun_SimpleFileExFlags+0xf1)[0x7fdd861dbe21]\n/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(Py_Main+0xb78)[0x7fdd861f5118]\n/lib/x86_64-linux-gnu/libc.so.6(__libc_start_main+0xe7)[0x7fdd85cd3b97]\n/home/parzan/sage-8.7/local/bin/python2(_start+0x2a)[0x55c630c7677a]\n------------------------------------------------------------------------\nAttaching gdb to process id 14681.\n```\n\n\nKeywords: **padic extension flint, padicBordeaux**\n\nBranch/Commit: **[bbe3349](https://github.com/sagemath/sagetrac-mirror/commit/bbe334918d6e27839269c5644fe87970c248bed3)**\n\nReviewer: **Travis Scrimshaw, Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Roe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27780_\n\n",
    "closed_at": "2019-10-03T17:58:12Z",
    "created_at": "2019-05-06T11:18:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "_flint_rep_abs of zero causes segmentation fault",
    "type": "issue",
    "updated_at": "2019-10-03T17:58:12Z",
    "url": "https://github.com/sagemath/sage/issues/27780",
    "user": "https://github.com/oriparzan"
}
```
On version 8.7.
For a p-adic extension, _flint_rep_abs of zero breaks sage:

```
sage: F=Qp(2)
sage: L.<w> = F.ext(x^2+x+1)
sage: L(0)._flint_rep_abs()
sig_error() without sig_on()
------------------------------------------------------------------------
/home/parzan/sage-8.7/local/lib/python2.7/site-packages/cysignals/signals.so(+0x759b)[0x7fdd7629859b]
/home/parzan/sage-8.7/local/lib/python2.7/site-packages/cysignals/signals.so(+0x7658)[0x7fdd76298658]
/home/parzan/sage-8.7/local/lib/python2.7/site-packages/cysignals/signals.so(+0xa1a5)[0x7fdd7629b1a5]
/lib/x86_64-linux-gnu/libc.so.6(+0x3ef20)[0x7fdd85cf0f20]
/lib/x86_64-linux-gnu/libc.so.6(kill+0x7)[0x7fdd85cf1187]
/home/parzan/sage-8.7/local/lib/python2.7/site-packages/sage/ext/memory.so(+0x2e46)[0x7fd57aacbe46]
/home/parzan/sage-8.7/local/lib/libgmp.so.23(__gmpz_realloc+0x31)[0x7fdd7d50c7f1]
/home/parzan/sage-8.7/local/lib/libgmp.so.23(__gmpz_n_pow_ui+0x77e)[0x7fdd7d508e6e]
/home/parzan/sage-8.7/local/lib/libflint.so.13(fmpz_pow_ui+0xab)[0x7fdd6b21dcdb]
/home/parzan/sage-8.7/local/lib/python2.7/site-packages/sage/rings/padics/pow_computer_flint.so(+0x7c4e)[0x7fd56ba56c4e]
/home/parzan/sage-8.7/local/lib/python2.7/site-packages/sage/rings/padics/qadic_flint_CR.so(+0x19fe7)[0x7fd56acd1fe7]
/home/parzan/sage-8.7/local/lib/python2.7/site-packages/sage/rings/padics/qadic_flint_CR.so(+0x2d1a1)[0x7fd56ace51a1]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x7f11)[0x7fdd861b51e1]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCode+0x19)[0x7fdd861b71a9]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x68b6)[0x7fdd861b3b86]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalFrameEx+0x6713)[0x7fdd861b39e3]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCodeEx+0x7b8)[0x7fdd861b6f98]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyEval_EvalCode+0x19)[0x7fdd861b71a9]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyRun_FileExFlags+0x8a)[0x7fdd861d9eda]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(PyRun_SimpleFileExFlags+0xf1)[0x7fdd861dbe21]
/home/parzan/sage-8.7/local/lib/libpython2.7.so.1.0(Py_Main+0xb78)[0x7fdd861f5118]
/lib/x86_64-linux-gnu/libc.so.6(__libc_start_main+0xe7)[0x7fdd85cd3b97]
/home/parzan/sage-8.7/local/bin/python2(_start+0x2a)[0x55c630c7677a]
------------------------------------------------------------------------
Attaching gdb to process id 14681.
```


Keywords: **padic extension flint, padicBordeaux**

Branch/Commit: **[bbe3349](https://github.com/sagemath/sagetrac-mirror/commit/bbe334918d6e27839269c5644fe87970c248bed3)**

Reviewer: **Travis Scrimshaw, Frédéric Chapoton**

Author: **David Roe**

_Issue created by migration from https://trac.sagemath.org/ticket/27780_





---

archive/issue_events_362917.json:
```json
{
    "actor": "https://github.com/oriparzan",
    "created_at": "2019-05-06T11:18:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362917"
}
```



---

archive/issue_events_362918.json:
```json
{
    "actor": "https://github.com/oriparzan",
    "created_at": "2019-05-06T11:18:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362918"
}
```



---

archive/issue_events_362919.json:
```json
{
    "actor": "https://github.com/oriparzan",
    "created_at": "2019-05-06T11:18:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362919"
}
```



---

archive/issue_events_362920.json:
```json
{
    "actor": "https://github.com/oriparzan",
    "created_at": "2019-05-06T11:18:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362920"
}
```



---

archive/issue_comments_433987.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAs the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).",
    "created_at": "2019-06-14T14:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433987",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
As the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).



---

archive/issue_events_362921.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:55:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362921"
}
```



---

archive/issue_comments_433988.json:
```json
{
    "body": "Changed keywords from **padic extension flint** to **padic extension flint, padicBordeaux**",
    "created_at": "2019-09-07T12:11:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433988",
    "user": "https://github.com/roed314"
}
```

Changed keywords from **padic extension flint** to **padic extension flint, padicBordeaux**



---

archive/issue_comments_433989.json:
```json
{
    "body": "Branch: **[u/roed/27780_flint_rep_abs](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/27780_flint_rep_abs)**",
    "created_at": "2019-09-12T23:54:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433989",
    "user": "https://github.com/roed314"
}
```

Branch: **[u/roed/27780_flint_rep_abs](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/27780_flint_rep_abs)**



---

archive/issue_events_362922.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-12T23:55:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362922"
}
```



---

archive/issue_comments_433990.json:
```json
{
    "body": "Author: **David Roe**",
    "created_at": "2019-09-12T23:55:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433990",
    "user": "https://github.com/roed314"
}
```

Author: **David Roe**



---

archive/issue_comments_433991.json:
```json
{
    "body": "Changed branch from **[u/roed/27780_flint_rep_abs](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/27780_flint_rep_abs)** to none",
    "created_at": "2019-09-12T23:55:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433991",
    "user": "https://github.com/roed314"
}
```

Changed branch from **[u/roed/27780_flint_rep_abs](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/27780_flint_rep_abs)** to none



---

archive/issue_comments_433992.json:
```json
{
    "body": "Commit: **[6b466f1](https://github.com/sagemath/sagetrac-mirror/commit/6b466f1d3d065ba9c86e2d7e374139ec7127d031)**",
    "created_at": "2019-09-12T23:55:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433992",
    "user": "https://github.com/roed314"
}
```

Commit: **[6b466f1](https://github.com/sagemath/sagetrac-mirror/commit/6b466f1d3d065ba9c86e2d7e374139ec7127d031)**



---

archive/issue_comments_433993.json:
```json
{
    "body": "Branch: **[u/roed/27780_flint_rep_abs](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/27780_flint_rep_abs)**",
    "created_at": "2019-09-12T23:55:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433993",
    "user": "https://github.com/roed314"
}
```

Branch: **[u/roed/27780_flint_rep_abs](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/27780_flint_rep_abs)**



---

archive/issue_comments_433994.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6b466f1d3d065ba9c86e2d7e374139ec7127d031\">6b466f1</a></td><td><code>Fix segfault</code></td></tr></table>\n",
    "created_at": "2019-09-12T23:55:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433994",
    "user": "https://github.com/roed314"
}
```

<a id='comment:5'>Comment 5:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6b466f1d3d065ba9c86e2d7e374139ec7127d031">6b466f1</a></td><td><code>Fix segfault</code></td></tr></table>




---

archive/issue_comments_433995.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-09-13T00:12:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433995",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_433996.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nMinor nitpick, but we (generally try to) follow the Python convention of error messages starting with a lowercase letter. Once changed, you can set a positive review on my behalf.",
    "created_at": "2019-09-13T00:12:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433996",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
Minor nitpick, but we (generally try to) follow the Python convention of error messages starting with a lowercase letter. Once changed, you can set a positive review on my behalf.



---

archive/issue_comments_433997.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/47ec10ba15ede9d667a5daf6a8372642d1ebb15c\">47ec10b</a></td><td><code>Capitalization</code></td></tr></table>\n",
    "created_at": "2019-09-13T07:28:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433997",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/47ec10ba15ede9d667a5daf6a8372642d1ebb15c">47ec10b</a></td><td><code>Capitalization</code></td></tr></table>




---

archive/issue_comments_433998.json:
```json
{
    "body": "Changed commit from **[6b466f1](https://github.com/sagemath/sagetrac-mirror/commit/6b466f1d3d065ba9c86e2d7e374139ec7127d031)** to **[47ec10b](https://github.com/sagemath/sagetrac-mirror/commit/47ec10ba15ede9d667a5daf6a8372642d1ebb15c)**",
    "created_at": "2019-09-13T07:28:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433998",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[6b466f1](https://github.com/sagemath/sagetrac-mirror/commit/6b466f1d3d065ba9c86e2d7e374139ec7127d031)** to **[47ec10b](https://github.com/sagemath/sagetrac-mirror/commit/47ec10ba15ede9d667a5daf6a8372642d1ebb15c)**



---

archive/issue_events_362923.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-13T07:30:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362923"
}
```



---

archive/issue_events_362924.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-13T07:30:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362924"
}
```



---

archive/issue_comments_433999.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nThanks!",
    "created_at": "2019-09-13T07:30:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-433999",
    "user": "https://github.com/roed314"
}
```

<a id='comment:8'>Comment 8:</a>
Thanks!



---

archive/issue_comments_434000.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nmissing milestone added",
    "created_at": "2019-09-13T13:47:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-434000",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'>Comment 9:</a>
missing milestone added



---

archive/issue_events_362925.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-13T13:47:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362925"
}
```



---

archive/issue_comments_434001.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nand the doctest will not pass (capital Z)",
    "created_at": "2019-09-13T13:48:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-434001",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:10'>Comment 10:</a>
and the doctest will not pass (capital Z)



---

archive/issue_events_362926.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-13T13:48:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362926"
}
```



---

archive/issue_events_362927.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-13T13:48:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362927"
}
```



---

archive/issue_comments_434002.json:
```json
{
    "body": "Changed commit from **[47ec10b](https://github.com/sagemath/sagetrac-mirror/commit/47ec10ba15ede9d667a5daf6a8372642d1ebb15c)** to **[bbe3349](https://github.com/sagemath/sagetrac-mirror/commit/bbe334918d6e27839269c5644fe87970c248bed3)**",
    "created_at": "2019-09-13T23:51:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-434002",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[47ec10b](https://github.com/sagemath/sagetrac-mirror/commit/47ec10ba15ede9d667a5daf6a8372642d1ebb15c)** to **[bbe3349](https://github.com/sagemath/sagetrac-mirror/commit/bbe334918d6e27839269c5644fe87970c248bed3)**



---

archive/issue_comments_434003.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bbe334918d6e27839269c5644fe87970c248bed3\">bbe3349</a></td><td><code>Fix doctest</code></td></tr></table>\n",
    "created_at": "2019-09-13T23:51:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-434003",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bbe334918d6e27839269c5644fe87970c248bed3">bbe3349</a></td><td><code>Fix doctest</code></td></tr></table>




---

archive/issue_events_362928.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-13T23:54:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362928"
}
```



---

archive/issue_events_362929.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-13T23:54:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362929"
}
```



---

archive/issue_comments_434004.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nOops!  I'll let the patchbot run.",
    "created_at": "2019-09-13T23:54:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-434004",
    "user": "https://github.com/roed314"
}
```

<a id='comment:12'>Comment 12:</a>
Oops!  I'll let the patchbot run.



---

archive/issue_comments_434005.json:
```json
{
    "body": "Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-09-14T06:03:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-434005",
    "user": "https://github.com/fchapoton"
}
```

Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Frédéric Chapoton**



---

archive/issue_events_362930.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-14T06:03:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362930"
}
```



---

archive/issue_events_362931.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-14T06:03:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362931"
}
```



---

archive/issue_comments_434006.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nok",
    "created_at": "2019-09-14T06:03:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-434006",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:13'>Comment 13:</a>
ok



---

archive/issue_events_362932.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-30T08:12:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362932"
}
```



---

archive/issue_events_362933.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-30T08:12:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362933"
}
```



---

archive/issue_comments_434007.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nmoving milestone to 9.0 (after release of 8.9)",
    "created_at": "2019-09-30T08:12:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-434007",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'>Comment 14:</a>
moving milestone to 9.0 (after release of 8.9)



---

archive/issue_comments_434008.json:
```json
{
    "body": "Changed branch from **[u/roed/27780_flint_rep_abs](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/27780_flint_rep_abs)** to **[bbe3349](https://github.com/sagemath/sagetrac-mirror/commit/bbe334918d6e27839269c5644fe87970c248bed3)**",
    "created_at": "2019-10-03T17:58:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27780#issuecomment-434008",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/roed/27780_flint_rep_abs](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/27780_flint_rep_abs)** to **[bbe3349](https://github.com/sagemath/sagetrac-mirror/commit/bbe334918d6e27839269c5644fe87970c248bed3)**



---

archive/issue_events_362934.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-03T17:58:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362934"
}
```



---

archive/issue_events_362935.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "01e4b5aa2512bb525b13e1dbb41d3b001b0aa6bf",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-10-03T17:58:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27780",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27780#event-362935"
}
```
