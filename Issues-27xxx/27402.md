# Issue 27402: remove the doctest flag "optional - gmpy2"

archive/issues_027165.json:
```json
{
    "assignees": [],
    "body": "#23024 made `gmpy2` a standard package but there are still some doctest flags in `src/sage/arith/misc.py` and `src/sage/structure/coerce.pyx`. The presence of these flags imply that the corresponding tests are not run.\n\nCC:  @vinklein\n\nBranch/Commit: **[635c4ab](https://github.com/sagemath/sagetrac-mirror/commit/635c4ab027b959e588ec3ccce7c6b95faadb1571)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27402_\n\n",
    "closed_at": "2019-03-05T23:11:10Z",
    "created_at": "2019-03-03T10:04:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "remove the doctest flag \"optional - gmpy2\"",
    "type": "issue",
    "updated_at": "2019-03-05T23:11:10Z",
    "url": "https://github.com/sagemath/sage/issues/27402",
    "user": "https://github.com/videlec"
}
```
#23024 made `gmpy2` a standard package but there are still some doctest flags in `src/sage/arith/misc.py` and `src/sage/structure/coerce.pyx`. The presence of these flags imply that the corresponding tests are not run.

CC:  @vinklein

Branch/Commit: **[635c4ab](https://github.com/sagemath/sagetrac-mirror/commit/635c4ab027b959e588ec3ccce7c6b95faadb1571)**

Reviewer: **Jeroen Demeyer**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/27402_





---

archive/issue_events_358228.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-03-03T10:04:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358228"
}
```



---

archive/issue_events_358229.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-03-03T10:04:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358229"
}
```



---

archive/issue_events_358230.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-03-03T10:04:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358230"
}
```



---

archive/issue_comments_426767.json:
```json
{
    "body": "Branch: **[u/vdelecroix/27402](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/27402)**",
    "created_at": "2019-03-03T10:05:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426767",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/27402](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/27402)**



---

archive/issue_comments_426768.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/58f2cec98c378ff406296e34e31a389388eff237\">58f2cec</a></td><td><code>remove the doctest flag \"optional - gmpy2\"</code></td></tr></table>\n",
    "created_at": "2019-03-03T10:05:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426768",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/58f2cec98c378ff406296e34e31a389388eff237">58f2cec</a></td><td><code>remove the doctest flag "optional - gmpy2"</code></td></tr></table>




---

archive/issue_events_358231.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-03-03T10:05:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358231"
}
```



---

archive/issue_comments_426769.json:
```json
{
    "body": "Commit: **[58f2cec](https://github.com/sagemath/sagetrac-mirror/commit/58f2cec98c378ff406296e34e31a389388eff237)**",
    "created_at": "2019-03-03T10:05:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426769",
    "user": "https://github.com/videlec"
}
```

Commit: **[58f2cec](https://github.com/sagemath/sagetrac-mirror/commit/58f2cec98c378ff406296e34e31a389388eff237)**



---

archive/issue_events_358232.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-03T18:11:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358232"
}
```



---

archive/issue_events_358233.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-03T18:11:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358233"
}
```



---

archive/issue_comments_426770.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nfailing doctests, see patchbot",
    "created_at": "2019-03-03T18:11:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426770",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
failing doctests, see patchbot



---

archive/issue_comments_426771.json:
```json
{
    "body": "Changed commit from **[58f2cec](https://github.com/sagemath/sagetrac-mirror/commit/58f2cec98c378ff406296e34e31a389388eff237)** to **[635c4ab](https://github.com/sagemath/sagetrac-mirror/commit/635c4ab027b959e588ec3ccce7c6b95faadb1571)**",
    "created_at": "2019-03-03T18:48:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426771",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[58f2cec](https://github.com/sagemath/sagetrac-mirror/commit/58f2cec98c378ff406296e34e31a389388eff237)** to **[635c4ab](https://github.com/sagemath/sagetrac-mirror/commit/635c4ab027b959e588ec3ccce7c6b95faadb1571)**



---

archive/issue_comments_426772.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/635c4ab027b959e588ec3ccce7c6b95faadb1571\">635c4ab</a></td><td><code>remove the doctest flag \"optional - gmpy2\"</code></td></tr></table>\n",
    "created_at": "2019-03-03T18:48:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426772",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/635c4ab027b959e588ec3ccce7c6b95faadb1571">635c4ab</a></td><td><code>remove the doctest flag "optional - gmpy2"</code></td></tr></table>




---

archive/issue_comments_426773.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI don't know why I missed one flag...",
    "created_at": "2019-03-03T18:49:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426773",
    "user": "https://github.com/videlec"
}
```

<a id='comment:4'>Comment 4:</a>
I don't know why I missed one flag...



---

archive/issue_events_358234.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-03-03T18:49:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358234"
}
```



---

archive/issue_events_358235.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-03-03T18:49:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358235"
}
```



---

archive/issue_events_358236.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-03T19:24:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358236"
}
```



---

archive/issue_comments_426774.json:
```json
{
    "body": "Assignee: **@vinklein**",
    "created_at": "2019-03-04T08:06:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426774",
    "user": "https://github.com/vinklein"
}
```

Assignee: **@vinklein**



---

archive/issue_comments_426775.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2019-03-04T08:46:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426775",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_358237.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2019-03-04T08:46:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358237"
}
```



---

archive/issue_events_358238.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2019-03-04T08:46:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358238"
}
```



---

archive/issue_comments_426776.json:
```json
{
    "body": "Removed assignee **@vinklein**",
    "created_at": "2019-03-04T08:50:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426776",
    "user": "https://github.com/vinklein"
}
```

Removed assignee **@vinklein**



---

archive/issue_events_358239.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-03-05T23:11:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358239"
}
```



---

archive/issue_events_358240.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "839360d9eede1b880f46977bfe5383742816c52d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-03-05T23:11:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27402#event-358240"
}
```



---

archive/issue_comments_426777.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/27402](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/27402)** to **[635c4ab](https://github.com/sagemath/sagetrac-mirror/commit/635c4ab027b959e588ec3ccce7c6b95faadb1571)**",
    "created_at": "2019-03-05T23:11:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27402#issuecomment-426777",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/27402](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/27402)** to **[635c4ab](https://github.com/sagemath/sagetrac-mirror/commit/635c4ab027b959e588ec3ccce7c6b95faadb1571)**
