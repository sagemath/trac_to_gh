# Issue 27611: normalize return value of basis_function_space()

archive/issues_027374.json:
```json
{
    "assignees": [],
    "body": "In the function field code, the Hess algorithm can produce basis elements with unnecessarily large coefficients.  One way to deal with this is to normalize the results by making a distinguished polynomial monic.\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27611_\n\n",
    "created_at": "2019-04-05T21:45:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "normalize return value of basis_function_space()",
    "type": "issue",
    "updated_at": "2019-06-14T14:55:02Z",
    "url": "https://github.com/sagemath/sage/issues/27611",
    "user": "https://github.com/BrentBaccala"
}
```
In the function field code, the Hess algorithm can produce basis elements with unnecessarily large coefficients.  One way to deal with this is to normalize the results by making a distinguished polynomial monic.

_Issue created by migration from https://trac.sagemath.org/ticket/27611_





---

archive/issue_events_360805.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-04-05T21:45:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27611",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27611#event-360805"
}
```



---

archive/issue_events_360806.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-04-05T21:45:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27611",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27611#event-360806"
}
```



---

archive/issue_events_360807.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-04-05T21:45:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27611",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27611#event-360807"
}
```



---

archive/issue_events_360808.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-04-05T21:45:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27611",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27611#event-360808"
}
```



---

archive/issue_events_360809.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:55:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27611",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27611#event-360809"
}
```



---

archive/issue_comments_430714.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAs the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).",
    "created_at": "2019-06-14T14:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27611#issuecomment-430714",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
As the Sage-8.8 release milestone is pending, we should delete the sage-8.8 milestone for tickets that are not actively being worked on or that still require significant work to move forward.  If you feel that this ticket should be included in the next Sage release at the soonest please set its milestone to the next release milestone (sage-8.9).
