# Issue 27718: graphs with edges twice don't raise warning

archive/issues_027481.json:
```json
{
    "body": "When using the short form for graph generation with a dictionary, the following is accepted:\n\n```\nsage: G1 = Graph({1: [3], 2: [0], 1: [2]})\n```\n\nBut when plotting this, it becomes clear that the vertex `3` is wiped out.  So ... probably this is user error, I admit.\n\n**But** I think it is a user error that should be warned against, as especially with typos it would be very easy to occur (not to mention bad programming where a loop recurs without intent).  Any ideas?\n\nReviewer: David Coudert\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/27718\n\n",
    "closed_at": "2019-05-09T19:18:54Z",
    "created_at": "2019-04-25T02:01:39Z",
    "labels": [
        "component: graph theory",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "graphs with edges twice don't raise warning",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/27718",
    "user": "https://github.com/kcrisman"
}
```
When using the short form for graph generation with a dictionary, the following is accepted:

```
sage: G1 = Graph({1: [3], 2: [0], 1: [2]})
```

But when plotting this, it becomes clear that the vertex `3` is wiped out.  So ... probably this is user error, I admit.

**But** I think it is a user error that should be warned against, as especially with typos it would be very easy to occur (not to mention bad programming where a loop recurs without intent).  Any ideas?

Reviewer: David Coudert

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/27718





---

archive/issue_comments_535288.json:
```json
{
    "body": "<a id='comment:1'></a>If you want to change that, you must change `dict`, not graph constructors.\n\n```\nsage: D = {1: [3], 2: [0], 1: [2]}\nsage: D\n{1: [2], 2: [0]}\n```",
    "created_at": "2019-04-25T07:03:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535288",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'></a>If you want to change that, you must change `dict`, not graph constructors.

```
sage: D = {1: [3], 2: [0], 1: [2]}
sage: D
{1: [2], 2: [0]}
```



---

archive/issue_comments_535289.json:
```json
{
    "body": "<a id='comment:2'></a>> If you want to change that, you must change `dict`, not graph constructors.\n> \n> ```\n> sage: D = {1: [3], 2: [0], 1: [2]}\n> sage: D\n> {1: [2], 2: [0]}\n> ```\n\n\nHaha, I didn't check that!  Shows it's been a while since I did that.  Well, then user error it is and wontfix.  Too bad, as I could imagine some much more subtle examples - mine is obviously easy to fix, but if auto-generated one could imagine something like this slipping through more easily.\n\nFeel free to put positive review and your name in the Reviewer section.  Or do you think it would be worth adding an example in the documentation (for graphs, I mean) to warn about this for people who aren't familiar with (or forgot!) this property of `dict`?",
    "created_at": "2019-04-25T11:52:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535289",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>> If you want to change that, you must change `dict`, not graph constructors.
> 
> ```
> sage: D = {1: [3], 2: [0], 1: [2]}
> sage: D
> {1: [2], 2: [0]}
> ```


Haha, I didn't check that!  Shows it's been a while since I did that.  Well, then user error it is and wontfix.  Too bad, as I could imagine some much more subtle examples - mine is obviously easy to fix, but if auto-generated one could imagine something like this slipping through more easily.

Feel free to put positive review and your name in the Reviewer section.  Or do you think it would be worth adding an example in the documentation (for graphs, I mean) to warn about this for people who aren't familiar with (or forgot!) this property of `dict`?



---

archive/issue_comments_535290.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-04-25T11:52:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535290",
    "user": "https://github.com/kcrisman"
}
```

Changing status from new to needs_review.



---

archive/issue_events_068728.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2019-04-25T11:52:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27718#event-68728"
}
```



---

archive/issue_comments_535291.json:
```json
{
    "body": "Changing reviewer from \"\" to \"David Coudert\"",
    "created_at": "2019-04-25T11:57:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535291",
    "user": "https://github.com/dcoudert"
}
```

Changing reviewer from "" to "David Coudert"



---

archive/issue_comments_535292.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-04-25T11:57:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535292",
    "user": "https://github.com/dcoudert"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_535293.json:
```json
{
    "body": "<a id='comment:3'></a>No need to add example. Users should learn a bit of Python / coding ...",
    "created_at": "2019-04-25T11:57:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535293",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:3'></a>No need to add example. Users should learn a bit of Python / coding ...



---

archive/issue_comments_535294.json:
```json
{
    "body": "Changing author from \"\" to \"Karl-Dieter Crisman\"",
    "created_at": "2019-04-25T12:07:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535294",
    "user": "https://github.com/dcoudert"
}
```

Changing author from "" to "Karl-Dieter Crisman"



---

archive/issue_comments_535295.json:
```json
{
    "body": "<a id='comment:4'></a>Author might be useful as well ;)",
    "created_at": "2019-04-25T12:07:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535295",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:4'></a>Author might be useful as well ;)



---

archive/issue_comments_535296.json:
```json
{
    "body": "<a id='comment:5'></a>Ah, but there are no authors with wontfix/duplicates :-)",
    "created_at": "2019-04-25T19:36:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535296",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:5'></a>Ah, but there are no authors with wontfix/duplicates :-)



---

archive/issue_comments_535297.json:
```json
{
    "body": "Changing author from \"Karl-Dieter Crisman\" to \"\"",
    "created_at": "2019-04-25T19:36:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535297",
    "user": "https://github.com/kcrisman"
}
```

Changing author from "Karl-Dieter Crisman" to ""



---

archive/issue_comments_535298.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2019-05-09T19:18:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27718#issuecomment-535298",
    "user": "https://github.com/fchapoton"
}
```

Resolution: invalid



---

archive/issue_events_068729.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-05-09T19:18:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/27718",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27718#event-68729"
}
```
