# Issue 27127: py3: avoid .vertices() in method ear_decomposition

archive/issues_026890.json:
```json
{
    "assignees": [],
    "body": "We avoid using `.vertices()` in this method and fix a failing doctest for py3.\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[8f382df](https://github.com/sagemath/sagetrac-mirror/commit/8f382df077017468f9a135e066c34015ae4bf5ab)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27127_\n\n",
    "closed_at": "2019-01-27T10:50:39Z",
    "created_at": "2019-01-25T14:50:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: avoid .vertices() in method ear_decomposition",
    "type": "issue",
    "updated_at": "2019-01-27T10:50:39Z",
    "url": "https://github.com/sagemath/sage/issues/27127",
    "user": "https://github.com/dcoudert"
}
```
We avoid using `.vertices()` in this method and fix a failing doctest for py3.

Keywords: **py3, graph**

Branch/Commit: **[8f382df](https://github.com/sagemath/sagetrac-mirror/commit/8f382df077017468f9a135e066c34015ae4bf5ab)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/27127_





---

archive/issue_events_354661.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-25T14:50:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27127#event-354661"
}
```



---

archive/issue_events_354662.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-25T14:50:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27127#event-354662"
}
```



---

archive/issue_events_354663.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-25T14:50:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27127#event-354663"
}
```



---

archive/issue_events_354664.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-25T14:50:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27127#event-354664"
}
```



---

archive/issue_comments_421512.json:
```json
{
    "body": "Commit: **[8f382df](https://github.com/sagemath/sagetrac-mirror/commit/8f382df077017468f9a135e066c34015ae4bf5ab)**",
    "created_at": "2019-01-25T14:52:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27127#issuecomment-421512",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[8f382df](https://github.com/sagemath/sagetrac-mirror/commit/8f382df077017468f9a135e066c34015ae4bf5ab)**



---

archive/issue_events_354665.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-01-25T14:52:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27127#event-354665"
}
```



---

archive/issue_comments_421513.json:
```json
{
    "body": "Branch: **[public/27127_ear_decomposition](https://github.com/sagemath/sagetrac-mirror/tree/public/27127_ear_decomposition)**",
    "created_at": "2019-01-25T14:52:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27127#issuecomment-421513",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/27127_ear_decomposition](https://github.com/sagemath/sagetrac-mirror/tree/public/27127_ear_decomposition)**



---

archive/issue_comments_421514.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8f382df077017468f9a135e066c34015ae4bf5ab\">8f382df</a></td><td><code>trac #27127: avoid .vertices() in ear_decomposition</code></td></tr></table>\n",
    "created_at": "2019-01-25T14:52:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27127#issuecomment-421514",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8f382df077017468f9a135e066c34015ae4bf5ab">8f382df</a></td><td><code>trac #27127: avoid .vertices() in ear_decomposition</code></td></tr></table>




---

archive/issue_comments_421515.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWhy this change:\n\n```diff\n-            sage: g = graphs.CubeGraph(2)\n+            sage: g = graphs.CycleGraph(4)\n             sage: g.ear_decomposition()\n-            [['00', '01', '11', '10', '00']]\n+            [[0, 3, 2, 1, 0]]\n```",
    "created_at": "2019-01-25T20:05:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27127#issuecomment-421515",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
Why this change:

```diff
-            sage: g = graphs.CubeGraph(2)
+            sage: g = graphs.CycleGraph(4)
             sage: g.ear_decomposition()
-            [['00', '01', '11', '10', '00']]
+            [[0, 3, 2, 1, 0]]
```



---

archive/issue_comments_421516.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis change is independent of using `.vertices()` or not.\nThis doctest was failing with py3. My understanding is that neighbors of a vertex are visited in different orders in py2 and py3 as the order in which elements of a set are listed might differ (see `G._backend.iterator_nbrs??`). Here, if the first visited neighbor of `'00'` is `'01'` or `'10'`, we get different answers. With py3 we get `[This is the Trac macro *'00', '10', '11', '01', '00'* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#'00', '10', '11', '01', '00'-macro)` and so the doctest was  failing.\n\nUsing `CycleGraph(4)`, the doctest is more stable and we get the same answer with py2 and py3. `CubeGraph(2)` is a cycle of order 4, so it's essentially the same test.",
    "created_at": "2019-01-26T09:08:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27127#issuecomment-421516",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:3'>Comment 3:</a>
This change is independent of using `.vertices()` or not.
This doctest was failing with py3. My understanding is that neighbors of a vertex are visited in different orders in py2 and py3 as the order in which elements of a set are listed might differ (see `G._backend.iterator_nbrs??`). Here, if the first visited neighbor of `'00'` is `'01'` or `'10'`, we get different answers. With py3 we get `[This is the Trac macro *'00', '10', '11', '01', '00'* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#'00', '10', '11', '01', '00'-macro)` and so the doctest was  failing.

Using `CycleGraph(4)`, the doctest is more stable and we get the same answer with py2 and py3. `CubeGraph(2)` is a cycle of order 4, so it's essentially the same test.



---

archive/issue_events_354666.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-01-26T20:16:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27127#event-354666"
}
```



---

archive/issue_events_354667.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-01-26T20:16:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27127#event-354667"
}
```



---

archive/issue_comments_421517.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI don't think it is any more stable of a test, just something that happens to work on both. Anyways, this is okay with me since there is not such an easy way out of this.",
    "created_at": "2019-01-26T20:16:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27127#issuecomment-421517",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
I don't think it is any more stable of a test, just something that happens to work on both. Anyways, this is okay with me since there is not such an easy way out of this.



---

archive/issue_comments_421518.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-01-26T20:16:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27127#issuecomment-421518",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_421519.json:
```json
{
    "body": "Changed branch from **[public/27127_ear_decomposition](https://github.com/sagemath/sagetrac-mirror/tree/public/27127_ear_decomposition)** to **[8f382df](https://github.com/sagemath/sagetrac-mirror/commit/8f382df077017468f9a135e066c34015ae4bf5ab)**",
    "created_at": "2019-01-27T10:50:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27127#issuecomment-421519",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/27127_ear_decomposition](https://github.com/sagemath/sagetrac-mirror/tree/public/27127_ear_decomposition)** to **[8f382df](https://github.com/sagemath/sagetrac-mirror/commit/8f382df077017468f9a135e066c34015ae4bf5ab)**



---

archive/issue_events_354668.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-01-27T10:50:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27127#event-354668"
}
```



---

archive/issue_events_354669.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3ea02b3862316e8ab108511fc158fe78fa318a63",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-01-27T10:50:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27127",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27127#event-354669"
}
```
