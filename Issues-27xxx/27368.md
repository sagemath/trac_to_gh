# Issue 27368: Allow supercrystals category to be inherited by default for subcrystals

archive/issues_027131.json:
```json
{
    "assignees": [],
    "body": "This should be inherited by default so that it behaves properly.\n\nBefore this ticket:\n\n```\nsage: T = crystals.Tableaux(['A',[1,1]], [2,1])\nsage: S = T.subcrystal(max_depth=3)\nsage: S.category()\nCategory of finite crystals\n```\n\nAfter this ticket:\n\n```\nsage: T = crystals.Tableaux(['A',[1,1]], [2,1])\nsage: S = T.subcrystal(max_depth=3)\nsage: S.category()\nCategory of regular super crystals\n```\n\nCC:  @anneschilling @bsalisbury1\n\nKeywords: **crystals, supercrystals**\n\nBranch: **[0c02439](https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36)**\n\nReviewer: **Ben Salisbury**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27368_\n\n",
    "closed_at": "2019-03-03T22:38:06Z",
    "created_at": "2019-02-26T22:54:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Allow supercrystals category to be inherited by default for subcrystals",
    "type": "issue",
    "updated_at": "2019-03-04T17:15:02Z",
    "url": "https://github.com/sagemath/sage/issues/27368",
    "user": "https://github.com/tscrim"
}
```
This should be inherited by default so that it behaves properly.

Before this ticket:

```
sage: T = crystals.Tableaux(['A',[1,1]], [2,1])
sage: S = T.subcrystal(max_depth=3)
sage: S.category()
Category of finite crystals
```

After this ticket:

```
sage: T = crystals.Tableaux(['A',[1,1]], [2,1])
sage: S = T.subcrystal(max_depth=3)
sage: S.category()
Category of regular super crystals
```

CC:  @anneschilling @bsalisbury1

Keywords: **crystals, supercrystals**

Branch: **[0c02439](https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36)**

Reviewer: **Ben Salisbury**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/27368_





---

archive/issue_events_345130.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-02-26T22:54:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27368#event-345130"
}
```



---

archive/issue_events_345131.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-02-26T22:54:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27368#event-345131"
}
```



---

archive/issue_events_345132.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-02-26T22:54:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27368#event-345132"
}
```



---

archive/issue_events_345133.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-02-26T22:54:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27368#event-345133"
}
```



---

archive/issue_comments_426232.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36\">0c02439</a></td><td><code>Allow supercrystals to be automatically inherited for subcrystals.</code></td></tr></table>\n",
    "created_at": "2019-02-26T22:55:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27368#issuecomment-426232",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36">0c02439</a></td><td><code>Allow supercrystals to be automatically inherited for subcrystals.</code></td></tr></table>




---

archive/issue_events_345134.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-02-26T22:55:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27368#event-345134"
}
```



---

archive/issue_comments_426233.json:
```json
{
    "body": "Commit: **[0c02439](https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36)**",
    "created_at": "2019-02-26T22:55:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27368#issuecomment-426233",
    "user": "https://github.com/tscrim"
}
```

Commit: **[0c02439](https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36)**



---

archive/issue_comments_426234.json:
```json
{
    "body": "Branch: **[public/crystals/subsupercrystals-27368](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/subsupercrystals-27368)**",
    "created_at": "2019-02-26T22:55:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27368#issuecomment-426234",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/crystals/subsupercrystals-27368](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/subsupercrystals-27368)**



---

archive/issue_comments_426235.json:
```json
{
    "body": "Reviewer: **Ben Salisbury**",
    "created_at": "2019-03-01T18:37:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27368#issuecomment-426235",
    "user": "https://github.com/bsalisbury1"
}
```

Reviewer: **Ben Salisbury**



---

archive/issue_events_345135.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2019-03-01T18:37:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27368#event-345135"
}
```



---

archive/issue_events_345136.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2019-03-01T18:37:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27368#event-345136"
}
```



---

archive/issue_comments_426236.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThanks Travis.",
    "created_at": "2019-03-01T18:37:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27368#issuecomment-426236",
    "user": "https://github.com/bsalisbury1"
}
```

<a id='comment:2'>Comment 2:</a>
Thanks Travis.



---

archive/issue_comments_426237.json:
```json
{
    "body": "Changed branch from **[public/crystals/subsupercrystals-27368](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/subsupercrystals-27368)** to **[0c02439](https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36)**",
    "created_at": "2019-03-03T22:38:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27368#issuecomment-426237",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/crystals/subsupercrystals-27368](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/subsupercrystals-27368)** to **[0c02439](https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36)**



---

archive/issue_events_345137.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-03-03T22:38:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27368#event-345137"
}
```



---

archive/issue_events_345138.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6abccaec4c61f929c358918595ac71264f812c75",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-03-03T22:38:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27368#event-345138"
}
```



---

archive/issue_comments_426238.json:
```json
{
    "body": "Changed commit from **[0c02439](https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36)** to none",
    "created_at": "2019-03-04T17:15:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27368#issuecomment-426238",
    "user": "https://github.com/slel"
}
```

Changed commit from **[0c02439](https://github.com/sagemath/sagetrac-mirror/commit/0c024397c954c2d457f24755c48187ee9679fd36)** to none



---

archive/issue_comments_426239.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,6 @@\n-This should be inherited by default so that it behaves properly:\n+This should be inherited by default so that it behaves properly.\n+\n+Before this ticket:\n \n ```\n sage: T = crystals.Tableaux(['A',[1,1]], [2,1])\n@@ -7,3 +9,11 @@\n Category of finite crystals\n ```\n \n+After this ticket:\n+\n+```\n+sage: T = crystals.Tableaux(['A',[1,1]], [2,1])\n+sage: S = T.subcrystal(max_depth=3)\n+sage: S.category()\n+Category of regular super crystals\n+```\n``````\n",
    "created_at": "2019-03-04T17:15:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27368#issuecomment-426239",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,6 @@
-This should be inherited by default so that it behaves properly:
+This should be inherited by default so that it behaves properly.
+
+Before this ticket:
 
 ```
 sage: T = crystals.Tableaux(['A',[1,1]], [2,1])
@@ -7,3 +9,11 @@
 Category of finite crystals
 ```
 
+After this ticket:
+
+```
+sage: T = crystals.Tableaux(['A',[1,1]], [2,1])
+sage: S = T.subcrystal(max_depth=3)
+sage: S.category()
+Category of regular super crystals
+```
``````

