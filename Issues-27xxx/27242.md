# Issue 27242: py3: strengthen a doctest in vertex_separation.pyx

archive/issues_027005.json:
```json
{
    "assignees": [],
    "body": "The order in which solutions are found by the branch and bound algorithm depends on the order in which vertices are visited. Hence, this test sometimes answers 10 and some other times 11, and both answers are correct.\n\n```\nsage: from sage.graphs.graph_decompositions import vertex_separation as VS\nsage: G = graphs.MycielskiGraph(5)\nsage: vs, seq = VS.vertex_separation_BAB(G, cut_off=11); vs\n10 or 11\n```\nWe change it to a robust form\n\n```\nsage: VS.vertex_separation_BAB(G, cut_off=11)[0] <= 11\nTrue\n```\n\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[42dc1a5](https://github.com/sagemath/sagetrac-mirror/commit/42dc1a54b65323cd86bdf83f2dc353d4320c32e8)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27242_\n\n",
    "closed_at": "2019-02-11T23:06:49Z",
    "created_at": "2019-02-09T10:11:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: strengthen a doctest in vertex_separation.pyx",
    "type": "issue",
    "updated_at": "2019-02-11T23:06:49Z",
    "url": "https://github.com/sagemath/sage/issues/27242",
    "user": "https://github.com/dcoudert"
}
```
The order in which solutions are found by the branch and bound algorithm depends on the order in which vertices are visited. Hence, this test sometimes answers 10 and some other times 11, and both answers are correct.

```
sage: from sage.graphs.graph_decompositions import vertex_separation as VS
sage: G = graphs.MycielskiGraph(5)
sage: vs, seq = VS.vertex_separation_BAB(G, cut_off=11); vs
10 or 11
```
We change it to a robust form

```
sage: VS.vertex_separation_BAB(G, cut_off=11)[0] <= 11
True
```


Keywords: **py3, graph**

Branch/Commit: **[42dc1a5](https://github.com/sagemath/sagetrac-mirror/commit/42dc1a54b65323cd86bdf83f2dc353d4320c32e8)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/27242_





---

archive/issue_events_356134.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-02-09T10:11:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27242#event-356134"
}
```



---

archive/issue_events_356135.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-02-09T10:11:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27242#event-356135"
}
```



---

archive/issue_events_356136.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-02-09T10:11:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27242#event-356136"
}
```



---

archive/issue_events_356137.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-02-09T10:11:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27242#event-356137"
}
```



---

archive/issue_events_356138.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-02-09T10:12:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27242#event-356138"
}
```



---

archive/issue_comments_423652.json:
```json
{
    "body": "Branch: **[u/dcoudert/27242_vertex_separation](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/27242_vertex_separation)**",
    "created_at": "2019-02-09T10:12:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27242#issuecomment-423652",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[u/dcoudert/27242_vertex_separation](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/27242_vertex_separation)**



---

archive/issue_comments_423653.json:
```json
{
    "body": "Commit: **[42dc1a5](https://github.com/sagemath/sagetrac-mirror/commit/42dc1a54b65323cd86bdf83f2dc353d4320c32e8)**",
    "created_at": "2019-02-09T10:12:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27242#issuecomment-423653",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[42dc1a5](https://github.com/sagemath/sagetrac-mirror/commit/42dc1a54b65323cd86bdf83f2dc353d4320c32e8)**



---

archive/issue_comments_423654.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/42dc1a54b65323cd86bdf83f2dc353d4320c32e8\">42dc1a5</a></td><td><code>trac #27242: strengthen doctests</code></td></tr></table>\n",
    "created_at": "2019-02-09T10:12:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27242#issuecomment-423654",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/42dc1a54b65323cd86bdf83f2dc353d4320c32e8">42dc1a5</a></td><td><code>trac #27242: strengthen doctests</code></td></tr></table>




---

archive/issue_comments_423655.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-02-09T15:28:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27242#issuecomment-423655",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_356139.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-02-09T15:28:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27242#event-356139"
}
```



---

archive/issue_events_356140.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-02-09T15:28:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27242#event-356140"
}
```



---

archive/issue_comments_423656.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nok",
    "created_at": "2019-02-09T15:28:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27242#issuecomment-423656",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
ok



---

archive/issue_comments_423657.json:
```json
{
    "body": "Changed branch from **[u/dcoudert/27242_vertex_separation](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/27242_vertex_separation)** to **[42dc1a5](https://github.com/sagemath/sagetrac-mirror/commit/42dc1a54b65323cd86bdf83f2dc353d4320c32e8)**",
    "created_at": "2019-02-11T23:06:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27242#issuecomment-423657",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dcoudert/27242_vertex_separation](https://github.com/sagemath/sagetrac-mirror/tree/u/dcoudert/27242_vertex_separation)** to **[42dc1a5](https://github.com/sagemath/sagetrac-mirror/commit/42dc1a54b65323cd86bdf83f2dc353d4320c32e8)**



---

archive/issue_events_356141.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-02-11T23:06:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27242#event-356141"
}
```



---

archive/issue_events_356142.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "24bdd0ba2b891536b03c98d0e31aa0c0409fb3bd",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-02-11T23:06:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27242",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27242#event-356142"
}
```
