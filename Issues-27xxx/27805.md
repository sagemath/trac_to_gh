# Issue 27805: Configure lrslib with LRS_QUIET

archive/issues_027568.json:
```json
{
    "assignees": [],
    "body": "This disables output on stdout and stderr during the use of the library (observed in #22704).\nThis configuration is standard on Debian.\n\nCC:  @dimpase @vbraun\n\nBranch/Commit: **[u/mkoeppe/configure_lrslib_with_lrs_quiet](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/configure_lrslib_with_lrs_quiet) @ [1b47911](https://github.com/sagemath/sagetrac-mirror/commit/1b479117463bb1a8d6372455f8723150313fc3ab)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27805_\n\n",
    "closed_at": "2019-06-08T04:18:31Z",
    "created_at": "2019-05-09T12:49:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Configure lrslib with LRS_QUIET",
    "type": "issue",
    "updated_at": "2019-06-08T04:18:31Z",
    "url": "https://github.com/sagemath/sage/issues/27805",
    "user": "https://github.com/mkoeppe"
}
```
This disables output on stdout and stderr during the use of the library (observed in #22704).
This configuration is standard on Debian.

CC:  @dimpase @vbraun

Branch/Commit: **[u/mkoeppe/configure_lrslib_with_lrs_quiet](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/configure_lrslib_with_lrs_quiet) @ [1b47911](https://github.com/sagemath/sagetrac-mirror/commit/1b479117463bb1a8d6372455f8723150313fc3ab)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/27805_





---

archive/issue_events_363217.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-09T12:49:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27805#event-363217"
}
```



---

archive/issue_events_363218.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-09T12:49:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27805#event-363218"
}
```



---

archive/issue_events_363219.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-09T12:49:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27805#event-363219"
}
```



---

archive/issue_events_363220.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-09T12:49:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27805#event-363220"
}
```



---

archive/issue_comments_434388.json:
```json
{
    "body": "Branch: **[u/mkoeppe/configure_lrslib_with_lrs_quiet](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/configure_lrslib_with_lrs_quiet)**",
    "created_at": "2019-05-09T12:56:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27805#issuecomment-434388",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/configure_lrslib_with_lrs_quiet](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/configure_lrslib_with_lrs_quiet)**



---

archive/issue_comments_434389.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1b479117463bb1a8d6372455f8723150313fc3ab\">1b47911</a></td><td><code>Configure lrslib with LRS_QUIET</code></td></tr></table>\n",
    "created_at": "2019-05-09T12:59:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27805#issuecomment-434389",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1b479117463bb1a8d6372455f8723150313fc3ab">1b47911</a></td><td><code>Configure lrslib with LRS_QUIET</code></td></tr></table>




---

archive/issue_comments_434390.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2019-05-09T12:59:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27805#issuecomment-434390",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_363221.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-09T12:59:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27805#event-363221"
}
```



---

archive/issue_comments_434391.json:
```json
{
    "body": "Commit: **[1b47911](https://github.com/sagemath/sagetrac-mirror/commit/1b479117463bb1a8d6372455f8723150313fc3ab)**",
    "created_at": "2019-05-09T12:59:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27805#issuecomment-434391",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[1b47911](https://github.com/sagemath/sagetrac-mirror/commit/1b479117463bb1a8d6372455f8723150313fc3ab)**



---

archive/issue_comments_434392.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2019-05-09T15:04:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27805#issuecomment-434392",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_363222.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-09T15:04:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27805#event-363222"
}
```



---

archive/issue_events_363223.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-05-09T15:04:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27805#event-363223"
}
```



---

archive/issue_comments_434393.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok, this works",
    "created_at": "2019-05-09T15:04:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27805#issuecomment-434393",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok, this works



---

archive/issue_comments_434394.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis branch has already been merged as part of #22704.",
    "created_at": "2019-06-08T04:18:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27805#issuecomment-434394",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

This branch has already been merged as part of #22704.



---

archive/issue_events_363224.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-06-08T04:18:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27805#event-363224"
}
```



---

archive/issue_events_363225.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-06-08T04:18:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27805",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27805#event-363225"
}
```
