# Issue 27549: Clean up some more code blocks in Sage's documentation

archive/issues_027312.json:
```json
{
    "assignees": [],
    "body": "This is a follow-up to #27528: we should change things like\n\n```\nHere is how to run long doctests::\n\n    $ make ptestlong\n```\nto\n\n```\nHere is how to run long doctests:\n\n.. CODE-BLOCK:: shell-session\n\n    $ make ptestlong\n```\nso that Sphinx highlights it correctly. Note that in addition to `shell-session`, there are many other language options. See the file `pygments/lexers/_mapping.py` for big list.\n\n\nDepends on #27528\nDepends on #27512\n\nBranch/Commit: **[95114ce](https://github.com/sagemath/sagetrac-mirror/commit/95114cebba886ca8ee13e1fd6fa182983ce23281)**\n\nReviewer: **Simon Brandhorst, Travis Scrimshaw**\n\nAuthor: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27549_\n\n",
    "closed_at": "2019-04-27T17:44:22Z",
    "created_at": "2019-03-26T04:18:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean up some more code blocks in Sage's documentation",
    "type": "issue",
    "updated_at": "2019-04-27T17:44:22Z",
    "url": "https://github.com/sagemath/sage/issues/27549",
    "user": "https://github.com/jhpalmieri"
}
```
This is a follow-up to #27528: we should change things like

```
Here is how to run long doctests::

    $ make ptestlong
```
to

```
Here is how to run long doctests:

.. CODE-BLOCK:: shell-session

    $ make ptestlong
```
so that Sphinx highlights it correctly. Note that in addition to `shell-session`, there are many other language options. See the file `pygments/lexers/_mapping.py` for big list.


Depends on #27528
Depends on #27512

Branch/Commit: **[95114ce](https://github.com/sagemath/sagetrac-mirror/commit/95114cebba886ca8ee13e1fd6fa182983ce23281)**

Reviewer: **Simon Brandhorst, Travis Scrimshaw**

Author: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/27549_





---

archive/issue_events_360047.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-03-26T04:18:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360047"
}
```



---

archive/issue_events_360048.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-03-26T04:18:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360048"
}
```



---

archive/issue_events_360049.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-03-26T04:18:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360049"
}
```



---

archive/issue_events_360050.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-03-26T04:18:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360050"
}
```



---

archive/issue_comments_429645.json:
```json
{
    "body": "Branch: **[u/jhpalmieri/code-blocks-in-docs](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/code-blocks-in-docs)**",
    "created_at": "2019-03-26T04:19:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429645",
    "user": "https://github.com/jhpalmieri"
}
```

Branch: **[u/jhpalmieri/code-blocks-in-docs](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/code-blocks-in-docs)**



---

archive/issue_events_360051.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-03-26T04:20:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360051"
}
```



---

archive/issue_comments_429646.json:
```json
{
    "body": "Commit: **[1cacccc](https://github.com/sagemath/sagetrac-mirror/commit/1cacccc7df42c101f634eaa0d668165506eb30d4)**",
    "created_at": "2019-03-26T04:20:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429646",
    "user": "https://github.com/jhpalmieri"
}
```

Commit: **[1cacccc](https://github.com/sagemath/sagetrac-mirror/commit/1cacccc7df42c101f634eaa0d668165506eb30d4)**



---

archive/issue_comments_429647.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI think this takes care of most (all?) of the code-blocks in `src/doc`. The reference manual remains.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/48b7acd9c132462e57317ededf422269e4e389d3\">48b7acd</a></td><td><code>trac 27528: remove some patches for Sphinx and pygments.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b23b8ccc43bdb63e6606e39cc73b2b4984bdfa7c\">b23b8cc</a></td><td><code>trac 27528: use a custom Sphinx transform to handle <BLANKLINE></code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/687b3385e80af4efd6ee8db79200c518a731af1d\">687b338</a></td><td><code>trac 27528: clean up some code-blocks in the reST documentation.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1cacccc7df42c101f634eaa0d668165506eb30d4\">1cacccc</a></td><td><code>trac 27549: clean up some code blocks in Sage's documentation.</code></td></tr></table>\n",
    "created_at": "2019-03-26T04:20:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429647",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">Comment 2</div>

I think this takes care of most (all?) of the code-blocks in `src/doc`. The reference manual remains.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/48b7acd9c132462e57317ededf422269e4e389d3">48b7acd</a></td><td><code>trac 27528: remove some patches for Sphinx and pygments.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b23b8ccc43bdb63e6606e39cc73b2b4984bdfa7c">b23b8cc</a></td><td><code>trac 27528: use a custom Sphinx transform to handle <BLANKLINE></code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/687b3385e80af4efd6ee8db79200c518a731af1d">687b338</a></td><td><code>trac 27528: clean up some code-blocks in the reST documentation.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1cacccc7df42c101f634eaa0d668165506eb30d4">1cacccc</a></td><td><code>trac 27549: clean up some code blocks in Sage's documentation.</code></td></tr></table>




---

archive/issue_comments_429648.json:
```json
{
    "body": "Dependencies: **#27528**",
    "created_at": "2019-03-26T04:21:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429648",
    "user": "https://github.com/jhpalmieri"
}
```

Dependencies: **#27528**



---

archive/issue_comments_429649.json:
```json
{
    "body": "Changed commit from **[1cacccc](https://github.com/sagemath/sagetrac-mirror/commit/1cacccc7df42c101f634eaa0d668165506eb30d4)** to **[b7b13a9](https://github.com/sagemath/sagetrac-mirror/commit/b7b13a9ba3347806954af6a72feb1cfb3e0051fd)**",
    "created_at": "2019-03-26T18:16:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429649",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1cacccc](https://github.com/sagemath/sagetrac-mirror/commit/1cacccc7df42c101f634eaa0d668165506eb30d4)** to **[b7b13a9](https://github.com/sagemath/sagetrac-mirror/commit/b7b13a9ba3347806954af6a72feb1cfb3e0051fd)**



---

archive/issue_comments_429650.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/278fa3b3fca010621d41df9d63e0b11801875adc\">278fa3b</a></td><td><code>Bump package versions</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a323c0965f3020045900cdb0fcb7702d4ec56f88\">a323c09</a></td><td><code>Fix a few CODE-BLOCK indentations</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4cabfa73badacb19731e95d959a1cf4bedcf531c\">4cabfa7</a></td><td><code>Fix _test_enough_doctests()</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/83436ea82a17406bbc3fdd9e8c386208abc93727\">83436ea</a></td><td><code>Fix usage of IPythonConsoleLexer</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b7b13a9ba3347806954af6a72feb1cfb3e0051fd\">b7b13a9</a></td><td><code>trac 27549: clean up some code blocks in Sage's documentation.</code></td></tr></table>\n",
    "created_at": "2019-03-26T18:16:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429650",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/278fa3b3fca010621d41df9d63e0b11801875adc">278fa3b</a></td><td><code>Bump package versions</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a323c0965f3020045900cdb0fcb7702d4ec56f88">a323c09</a></td><td><code>Fix a few CODE-BLOCK indentations</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4cabfa73badacb19731e95d959a1cf4bedcf531c">4cabfa7</a></td><td><code>Fix _test_enough_doctests()</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/83436ea82a17406bbc3fdd9e8c386208abc93727">83436ea</a></td><td><code>Fix usage of IPythonConsoleLexer</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b7b13a9ba3347806954af6a72feb1cfb3e0051fd">b7b13a9</a></td><td><code>trac 27549: clean up some code blocks in Sage's documentation.</code></td></tr></table>




---

archive/issue_comments_429651.json:
```json
{
    "body": "Changed commit from **[b7b13a9](https://github.com/sagemath/sagetrac-mirror/commit/b7b13a9ba3347806954af6a72feb1cfb3e0051fd)** to **[90dc135](https://github.com/sagemath/sagetrac-mirror/commit/90dc135b79478a194914d6252c48667625f2cd95)**",
    "created_at": "2019-03-26T18:36:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429651",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b7b13a9](https://github.com/sagemath/sagetrac-mirror/commit/b7b13a9ba3347806954af6a72feb1cfb3e0051fd)** to **[90dc135](https://github.com/sagemath/sagetrac-mirror/commit/90dc135b79478a194914d6252c48667625f2cd95)**



---

archive/issue_comments_429652.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1868426429b5a7c85163c7d94c8635e449d72366\">1868426</a></td><td><code>Fixed typos in thematic_tutorial</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a23f1c29fd1b364917f59cc4066b799bcab75f17\">a23f1c2</a></td><td><code>trac 27528: remove some patches for Sphinx and pygments.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/33e4fb7de80d75ada1d9bf6c3124aa42a1f53b69\">33e4fb7</a></td><td><code>trac 27528: use a custom Sphinx transform to handle <BLANKLINE></code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c1d8e58bf84526d3fcd4d6d494c47b48b31127dc\">c1d8e58</a></td><td><code>trac 27528: clean up some code-blocks in the reST documentation.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b92eb0280a2555857c60b8d97d3f90a564181013\">b92eb02</a></td><td><code>Bump package versions</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f57615e68e35e670f4a894da8d53dd6288f0a150\">f57615e</a></td><td><code>Fix a few CODE-BLOCK indentations</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0c60af72c73ff7cc72c91bde53ec0e85bce9dcd8\">0c60af7</a></td><td><code>Fix _test_enough_doctests()</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a45179eda4c6bfe88e0868396a0e584e95b22bd\">2a45179</a></td><td><code>Fix usage of IPythonConsoleLexer</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/90dc135b79478a194914d6252c48667625f2cd95\">90dc135</a></td><td><code>trac 27549: clean up some code blocks in Sage's documentation.</code></td></tr></table>\n",
    "created_at": "2019-03-26T18:36:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429652",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1868426429b5a7c85163c7d94c8635e449d72366">1868426</a></td><td><code>Fixed typos in thematic_tutorial</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a23f1c29fd1b364917f59cc4066b799bcab75f17">a23f1c2</a></td><td><code>trac 27528: remove some patches for Sphinx and pygments.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/33e4fb7de80d75ada1d9bf6c3124aa42a1f53b69">33e4fb7</a></td><td><code>trac 27528: use a custom Sphinx transform to handle <BLANKLINE></code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c1d8e58bf84526d3fcd4d6d494c47b48b31127dc">c1d8e58</a></td><td><code>trac 27528: clean up some code-blocks in the reST documentation.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b92eb0280a2555857c60b8d97d3f90a564181013">b92eb02</a></td><td><code>Bump package versions</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f57615e68e35e670f4a894da8d53dd6288f0a150">f57615e</a></td><td><code>Fix a few CODE-BLOCK indentations</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0c60af72c73ff7cc72c91bde53ec0e85bce9dcd8">0c60af7</a></td><td><code>Fix _test_enough_doctests()</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a45179eda4c6bfe88e0868396a0e584e95b22bd">2a45179</a></td><td><code>Fix usage of IPythonConsoleLexer</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/90dc135b79478a194914d6252c48667625f2cd95">90dc135</a></td><td><code>trac 27549: clean up some code blocks in Sage's documentation.</code></td></tr></table>




---

archive/issue_comments_429653.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nRebased to include #27512, which touches one of the same files.",
    "created_at": "2019-03-26T18:37:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429653",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:6" align="right">Comment 6</div>

Rebased to include #27512, which touches one of the same files.



---

archive/issue_comments_429654.json:
```json
{
    "body": "Changed dependencies from **#27528** to **#27528, #27512**",
    "created_at": "2019-03-26T18:37:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429654",
    "user": "https://github.com/jhpalmieri"
}
```

Changed dependencies from **#27528** to **#27528, #27512**



---

archive/issue_comments_429655.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nMerge conflict",
    "created_at": "2019-04-17T08:03:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429655",
    "user": "https://github.com/simonbrandhorst"
}
```

<div id="comment:7" align="right">Comment 7</div>

Merge conflict



---

archive/issue_comments_429656.json:
```json
{
    "body": "Reviewer: **Simon Brandhorst**",
    "created_at": "2019-04-17T08:03:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429656",
    "user": "https://github.com/simonbrandhorst"
}
```

Reviewer: **Simon Brandhorst**



---

archive/issue_events_360052.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2019-04-17T08:03:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360052"
}
```



---

archive/issue_events_360053.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2019-04-17T08:03:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360053"
}
```



---

archive/issue_comments_429657.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cf39c2b4a19e9a7b45c9f70efaa1772ffa4ddb56\">cf39c2b</a></td><td><code>trac 27549: clean up some code blocks in Sage's documentation.</code></td></tr></table>\n",
    "created_at": "2019-04-17T18:11:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429657",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cf39c2b4a19e9a7b45c9f70efaa1772ffa4ddb56">cf39c2b</a></td><td><code>trac 27549: clean up some code blocks in Sage's documentation.</code></td></tr></table>




---

archive/issue_comments_429658.json:
```json
{
    "body": "Changed commit from **[90dc135](https://github.com/sagemath/sagetrac-mirror/commit/90dc135b79478a194914d6252c48667625f2cd95)** to **[cf39c2b](https://github.com/sagemath/sagetrac-mirror/commit/cf39c2b4a19e9a7b45c9f70efaa1772ffa4ddb56)**",
    "created_at": "2019-04-17T18:11:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429658",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[90dc135](https://github.com/sagemath/sagetrac-mirror/commit/90dc135b79478a194914d6252c48667625f2cd95)** to **[cf39c2b](https://github.com/sagemath/sagetrac-mirror/commit/cf39c2b4a19e9a7b45c9f70efaa1772ffa4ddb56)**



---

archive/issue_comments_429659.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nRebased.",
    "created_at": "2019-04-17T18:11:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429659",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:9" align="right">Comment 9</div>

Rebased.



---

archive/issue_events_360054.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-04-17T18:11:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360054"
}
```



---

archive/issue_events_360055.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-04-17T18:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360055"
}
```



---

archive/issue_comments_429660.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nNitpicks:\n\n`installation/source.rst`:\n\n```diff\n-.. highlight:: shell-session\n+.. HIGHLIGHT:: shell-session\n```\n`thematic_tutorials/cython_interface.rst`:\n\n```diff\n-.. code-block:: shell-session\n+.. CODE-BLOCK:: shell-session\n```\n\nOnce changed, you can set a positive review on my behalf unless Simon has any additional comments.",
    "created_at": "2019-04-17T22:41:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429660",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:10" align="right">Comment 10</div>

Nitpicks:

`installation/source.rst`:

```diff
-.. highlight:: shell-session
+.. HIGHLIGHT:: shell-session
```
`thematic_tutorials/cython_interface.rst`:

```diff
-.. code-block:: shell-session
+.. CODE-BLOCK:: shell-session
```

Once changed, you can set a positive review on my behalf unless Simon has any additional comments.



---

archive/issue_comments_429661.json:
```json
{
    "body": "Changed reviewer from **Simon Brandhorst** to **Simon Brandhorst, Travis Scrimshaw**",
    "created_at": "2019-04-17T22:41:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429661",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Simon Brandhorst** to **Simon Brandhorst, Travis Scrimshaw**



---

archive/issue_comments_429662.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThanks for the comments. To handle the second of these, I decided I also needed to change the half-dozen already present `.. code-block` lines to `.. CODE-BLOCK`.",
    "created_at": "2019-04-17T23:30:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429662",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:11" align="right">Comment 11</div>

Thanks for the comments. To handle the second of these, I decided I also needed to change the half-dozen already present `.. code-block` lines to `.. CODE-BLOCK`.



---

archive/issue_comments_429663.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/95114cebba886ca8ee13e1fd6fa182983ce23281\">95114ce</a></td><td><code>trac 27549: more consistent capitalization in reST directives.</code></td></tr></table>\n",
    "created_at": "2019-04-17T23:30:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429663",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/95114cebba886ca8ee13e1fd6fa182983ce23281">95114ce</a></td><td><code>trac 27549: more consistent capitalization in reST directives.</code></td></tr></table>




---

archive/issue_comments_429664.json:
```json
{
    "body": "Changed commit from **[cf39c2b](https://github.com/sagemath/sagetrac-mirror/commit/cf39c2b4a19e9a7b45c9f70efaa1772ffa4ddb56)** to **[95114ce](https://github.com/sagemath/sagetrac-mirror/commit/95114cebba886ca8ee13e1fd6fa182983ce23281)**",
    "created_at": "2019-04-17T23:30:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429664",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[cf39c2b](https://github.com/sagemath/sagetrac-mirror/commit/cf39c2b4a19e9a7b45c9f70efaa1772ffa4ddb56)** to **[95114ce](https://github.com/sagemath/sagetrac-mirror/commit/95114cebba886ca8ee13e1fd6fa182983ce23281)**



---

archive/issue_events_360056.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-04-17T23:45:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360056"
}
```



---

archive/issue_events_360057.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-04-17T23:45:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360057"
}
```



---

archive/issue_comments_429665.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nThank you. LGTM.\n\nSimon, if you disagree, feel free to revert.",
    "created_at": "2019-04-17T23:45:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429665",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:13" align="right">Comment 13</div>

Thank you. LGTM.

Simon, if you disagree, feel free to revert.



---

archive/issue_events_360058.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-04-27T17:44:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360058"
}
```



---

archive/issue_events_360059.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a7cfa197361a62b62d9cb8b1aab8ece7920dd66c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-04-27T17:44:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27549#event-360059"
}
```



---

archive/issue_comments_429666.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/code-blocks-in-docs](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/code-blocks-in-docs)** to **[95114ce](https://github.com/sagemath/sagetrac-mirror/commit/95114cebba886ca8ee13e1fd6fa182983ce23281)**",
    "created_at": "2019-04-27T17:44:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27549",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27549#issuecomment-429666",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/code-blocks-in-docs](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/code-blocks-in-docs)** to **[95114ce](https://github.com/sagemath/sagetrac-mirror/commit/95114cebba886ca8ee13e1fd6fa182983ce23281)**
