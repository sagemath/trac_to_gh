# Issue 27566: Unhandled configure error when multiple installation records exist for the same SPKG

archive/issues_027329.json:
```json
{
    "assignees": [],
    "body": "If, by some accident, one ends up with multiple stamp files for the same package in `local/var/lib/sage/installed`, an error occurs when running `./configure`.\n\nFor example, in my case, in the course of development I somehow accidentally wound up with two install records for different versions of mpir.  This results in an error appearing during `./configure` like:\n\n```\n...\n    mpc-1.1.0\n    mpfi-1.5.2\n    mpfr-4.0.1.p0\n    mpfrcx-0.5\n./configure: line 10692: test: local/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868: binary operator expected\n    mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868.p0\n    mpmath-1.1.0\n    nauty-26r1.p0\n```\n\nOf course, this should never happen during normal development or usage, but apparently it *can* since I somehow made it happen.\n\nAttached is a possible fix: The configure script now detects this condition as an error, and aborts with an error message that's at least a bit more useful than none at all:\n\n```\n...\n    mpfr-4.0.1.p0\n    mpfrcx-0.5\nconfigure: error: multiple installation records for mpir at\nlocal/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868\nlocal/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868.p0\nonly one should exist so please delete one or both files and re-configure\n```\n\nIt's impossible to know for sure what the best course of action is here, so we give the user a hint and leave it up to them to make a decision and retry.\n\nThe branch is on top of #28788.\n\nCC:  @dimpase @jhpalmieri @saraedum @orlitzky\n\nBranch/Commit: **[ec9ab87](https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9)**\n\nReviewer: **Michael Orlitzky**\n\nAuthor: **Erik Bray, Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27566_\n\n",
    "closed_at": "2020-03-18T22:40:36Z",
    "created_at": "2019-03-29T15:41:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Unhandled configure error when multiple installation records exist for the same SPKG",
    "type": "issue",
    "updated_at": "2020-03-18T22:40:36Z",
    "url": "https://github.com/sagemath/sage/issues/27566",
    "user": "https://github.com/embray"
}
```
If, by some accident, one ends up with multiple stamp files for the same package in `local/var/lib/sage/installed`, an error occurs when running `./configure`.

For example, in my case, in the course of development I somehow accidentally wound up with two install records for different versions of mpir.  This results in an error appearing during `./configure` like:

```
...
    mpc-1.1.0
    mpfi-1.5.2
    mpfr-4.0.1.p0
    mpfrcx-0.5
./configure: line 10692: test: local/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868: binary operator expected
    mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868.p0
    mpmath-1.1.0
    nauty-26r1.p0
```

Of course, this should never happen during normal development or usage, but apparently it *can* since I somehow made it happen.

Attached is a possible fix: The configure script now detects this condition as an error, and aborts with an error message that's at least a bit more useful than none at all:

```
...
    mpfr-4.0.1.p0
    mpfrcx-0.5
configure: error: multiple installation records for mpir at
local/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868
local/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868.p0
only one should exist so please delete one or both files and re-configure
```

It's impossible to know for sure what the best course of action is here, so we give the user a hint and leave it up to them to make a decision and retry.

The branch is on top of #28788.

CC:  @dimpase @jhpalmieri @saraedum @orlitzky

Branch/Commit: **[ec9ab87](https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9)**

Reviewer: **Michael Orlitzky**

Author: **Erik Bray, Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/27566_





---

archive/issue_events_360282.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-29T15:41:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360282"
}
```



---

archive/issue_events_360283.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-29T15:41:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360283"
}
```



---

archive/issue_events_360284.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-29T15:41:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360284"
}
```



---

archive/issue_events_360285.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-29T15:41:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360285"
}
```



---

archive/issue_comments_429927.json:
```json
{
    "body": "Commit: **[939c342](https://github.com/sagemath/sagetrac-mirror/commit/939c3429b0af97121527290044b17aa33219a546)**",
    "created_at": "2019-03-29T15:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429927",
    "user": "https://github.com/embray"
}
```

Commit: **[939c342](https://github.com/sagemath/sagetrac-mirror/commit/939c3429b0af97121527290044b17aa33219a546)**



---

archive/issue_comments_429928.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -16,3 +16,16 @@\n \n Of course, this should never happen during normal development or usage, but apparently it *can* since I somehow made it happen.\n \n+Attached is a possible fix: The configure script now detects this condition as an error, and aborts with an error message that's at least a bit more useful than none at all:\n+\n+```\n+...\n+    mpfr-4.0.1.p0\n+    mpfrcx-0.5\n+configure: error: multiple installation records for mpir at\n+local/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868\n+local/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868.p0\n+only one should exist so please delete one or both files and re-configure\n+```\n+\n+It's impossible to know for sure what the best course of action is here, so we give the user a hint and leave it up to them to make a decision and retry.\n``````\n",
    "created_at": "2019-03-29T15:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429928",
    "user": "https://github.com/embray"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -16,3 +16,16 @@
 
 Of course, this should never happen during normal development or usage, but apparently it *can* since I somehow made it happen.
 
+Attached is a possible fix: The configure script now detects this condition as an error, and aborts with an error message that's at least a bit more useful than none at all:
+
+```
+...
+    mpfr-4.0.1.p0
+    mpfrcx-0.5
+configure: error: multiple installation records for mpir at
+local/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868
+local/var/lib/sage/installed/mpir-3.0.0-644faf502c56f97d9accd301965fc57d6ec70868.p0
+only one should exist so please delete one or both files and re-configure
+```
+
+It's impossible to know for sure what the best course of action is here, so we give the user a hint and leave it up to them to make a decision and retry.
``````




---

archive/issue_comments_429929.json:
```json
{
    "body": "Branch: **[u/embray/build/ticket-27566](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/build/ticket-27566)**",
    "created_at": "2019-03-29T15:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429929",
    "user": "https://github.com/embray"
}
```

Branch: **[u/embray/build/ticket-27566](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/build/ticket-27566)**



---

archive/issue_comments_429930.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/939c3429b0af97121527290044b17aa33219a546\">939c342</a></td><td><code>Trac #27566: Detect the condition of multiple SPKG stamp files and error out of the configure script</code></td></tr></table>\n",
    "created_at": "2019-03-29T15:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429930",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/939c3429b0af97121527290044b17aa33219a546">939c342</a></td><td><code>Trac #27566: Detect the condition of multiple SPKG stamp files and error out of the configure script</code></td></tr></table>




---

archive/issue_comments_429931.json:
```json
{
    "body": "Author: **Erik Bray**",
    "created_at": "2019-03-29T15:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429931",
    "user": "https://github.com/embray"
}
```

Author: **Erik Bray**



---

archive/issue_events_360286.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-29T15:45:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360286"
}
```



---

archive/issue_events_360287.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-29T15:50:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360287"
}
```



---

archive/issue_events_360288.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-29T15:50:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
    "label_color": "000080",
    "label_name": "component: build: configure",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360288"
}
```



---

archive/issue_comments_429932.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMoving tickets from the Sage 8.8 milestone that have been actively worked on in the last six months to the next release milestone (optimistically).",
    "created_at": "2019-07-03T11:37:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429932",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'>Comment 3:</a>
Moving tickets from the Sage 8.8 milestone that have been actively worked on in the last six months to the next release milestone (optimistically).



---

archive/issue_events_360289.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-03T11:37:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360289"
}
```



---

archive/issue_events_360290.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-03T11:37:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360290"
}
```



---

archive/issue_comments_429933.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429933",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'>Comment 4:</a>
Ticket retargeted after milestone closed



---

archive/issue_events_360291.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360291"
}
```



---

archive/issue_events_360292.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360292"
}
```



---

archive/issue_comments_429934.json:
```json
{
    "body": "Dependencies: **#28788**",
    "created_at": "2020-02-09T23:12:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429934",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#28788**



---

archive/issue_comments_429935.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThis should be rebased on top of #28788 because it touches the same code.",
    "created_at": "2020-02-09T23:12:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429935",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'>Comment 5:</a>
This should be rebased on top of #28788 because it touches the same code.



---

archive/issue_comments_429936.json:
```json
{
    "body": "Changed branch from **[u/embray/build/ticket-27566](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/build/ticket-27566)** to **[u/mkoeppe/build/ticket-27566](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build/ticket-27566)**",
    "created_at": "2020-02-09T23:53:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429936",
    "user": "https://github.com/mkoeppe"
}
```

Changed branch from **[u/embray/build/ticket-27566](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/build/ticket-27566)** to **[u/mkoeppe/build/ticket-27566](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build/ticket-27566)**



---

archive/issue_comments_429937.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/83633dfb419a5c930c3848e10cc538cb3f59f22a\">83633df</a></td><td><code>Merge branch 'develop' into t/28788/mrs/38/verbose-configure</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/684b80c788b687794a81f03aec68ecdc412bf787\">684b80c</a></td><td><code>Merge tag '9.1.beta3' into t/28788/mrs/38/verbose-configure</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1dd1252f72726e76268b23bc37f4e5276fcf33f6\">1dd1252</a></td><td><code>Put configure box messages also into log, via AS_MESSAGE_FD</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/192c24d142f0cfa84b11de9a8a40c891a4d995f7\">192c24d</a></td><td><code>Send configure's macOS message to the right FD</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0196d25af19c57086407b141f8cecf3729e072e7\">0196d25</a></td><td><code>fixup log</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5bcc0e4dbe1e6a946d9ee09152daba13c61af3e1\">5bcc0e4</a></td><td><code>tox.ini [check_configure]: New - test that 'configure --quiet' is quiet</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bef6d579447b90e4976c5bf9ce29390100e03311\">bef6d57</a></td><td><code>Improve spkg messages</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bf4ee6c73892af9ce238b04b0242dbf4d27287b0\">bf4ee6c</a></td><td><code>Trac #27566: Detect the condition of multiple SPKG stamp files and error out of the configure script</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/383290af524482ab0ad8f8e2cd304fc320538f23\">383290a</a></td><td><code>m4/sage_spkg_enable.m4: SAGE_SPKG_ENABLE: Do not trip over multiple install records</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/51dc4b3801eb7cc99ea50a9a04f5577ee6c8d84f\">51dc4b3</a></td><td><code>Rewrite to avoid use of wc</code></td></tr></table>\n",
    "created_at": "2020-02-09T23:54:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429937",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'>Comment 7:</a>
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/83633dfb419a5c930c3848e10cc538cb3f59f22a">83633df</a></td><td><code>Merge branch 'develop' into t/28788/mrs/38/verbose-configure</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/684b80c788b687794a81f03aec68ecdc412bf787">684b80c</a></td><td><code>Merge tag '9.1.beta3' into t/28788/mrs/38/verbose-configure</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1dd1252f72726e76268b23bc37f4e5276fcf33f6">1dd1252</a></td><td><code>Put configure box messages also into log, via AS_MESSAGE_FD</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/192c24d142f0cfa84b11de9a8a40c891a4d995f7">192c24d</a></td><td><code>Send configure's macOS message to the right FD</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0196d25af19c57086407b141f8cecf3729e072e7">0196d25</a></td><td><code>fixup log</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5bcc0e4dbe1e6a946d9ee09152daba13c61af3e1">5bcc0e4</a></td><td><code>tox.ini [check_configure]: New - test that 'configure --quiet' is quiet</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bef6d579447b90e4976c5bf9ce29390100e03311">bef6d57</a></td><td><code>Improve spkg messages</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bf4ee6c73892af9ce238b04b0242dbf4d27287b0">bf4ee6c</a></td><td><code>Trac #27566: Detect the condition of multiple SPKG stamp files and error out of the configure script</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/383290af524482ab0ad8f8e2cd304fc320538f23">383290a</a></td><td><code>m4/sage_spkg_enable.m4: SAGE_SPKG_ENABLE: Do not trip over multiple install records</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/51dc4b3801eb7cc99ea50a9a04f5577ee6c8d84f">51dc4b3</a></td><td><code>Rewrite to avoid use of wc</code></td></tr></table>




---

archive/issue_comments_429938.json:
```json
{
    "body": "Changed commit from **[939c342](https://github.com/sagemath/sagetrac-mirror/commit/939c3429b0af97121527290044b17aa33219a546)** to **[51dc4b3](https://github.com/sagemath/sagetrac-mirror/commit/51dc4b3801eb7cc99ea50a9a04f5577ee6c8d84f)**",
    "created_at": "2020-02-09T23:54:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429938",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[939c342](https://github.com/sagemath/sagetrac-mirror/commit/939c3429b0af97121527290044b17aa33219a546)** to **[51dc4b3](https://github.com/sagemath/sagetrac-mirror/commit/51dc4b3801eb7cc99ea50a9a04f5577ee6c8d84f)**



---

archive/issue_comments_429939.json:
```json
{
    "body": "Changed author from **Erik Bray** to **Erik Bray, Matthias Koeppe**",
    "created_at": "2020-02-09T23:54:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429939",
    "user": "https://github.com/mkoeppe"
}
```

Changed author from **Erik Bray** to **Erik Bray, Matthias Koeppe**



---

archive/issue_comments_429940.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -29,3 +29,5 @@\n ```\n \n It's impossible to know for sure what the best course of action is here, so we give the user a hint and leave it up to them to make a decision and retry.\n+\n+The branch is on top of #28788.\n``````\n",
    "created_at": "2020-02-09T23:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429940",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -29,3 +29,5 @@
 ```
 
 It's impossible to know for sure what the best course of action is here, so we give the user a hint and leave it up to them to make a decision and retry.
+
+The branch is on top of #28788.
``````




---

archive/issue_comments_429941.json:
```json
{
    "body": "Changed commit from **[51dc4b3](https://github.com/sagemath/sagetrac-mirror/commit/51dc4b3801eb7cc99ea50a9a04f5577ee6c8d84f)** to **[e5678b4](https://github.com/sagemath/sagetrac-mirror/commit/e5678b4598f3d69f1fe2f5daf26159f149a5500a)**",
    "created_at": "2020-02-16T20:28:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429941",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[51dc4b3](https://github.com/sagemath/sagetrac-mirror/commit/51dc4b3801eb7cc99ea50a9a04f5577ee6c8d84f)** to **[e5678b4](https://github.com/sagemath/sagetrac-mirror/commit/e5678b4598f3d69f1fe2f5daf26159f149a5500a)**



---

archive/issue_comments_429942.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e5678b4598f3d69f1fe2f5daf26159f149a5500a\">e5678b4</a></td><td><code>Merge tag '9.1.beta4' into t/27566/build/ticket-27566</code></td></tr></table>\n",
    "created_at": "2020-02-16T20:28:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429942",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:9'>Comment 9:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e5678b4598f3d69f1fe2f5daf26159f149a5500a">e5678b4</a></td><td><code>Merge tag '9.1.beta4' into t/27566/build/ticket-27566</code></td></tr></table>




---

archive/issue_comments_429943.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nNeeds review",
    "created_at": "2020-03-01T00:03:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429943",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'>Comment 10:</a>
Needs review



---

archive/issue_comments_429944.json:
```json
{
    "body": "Changed commit from **[e5678b4](https://github.com/sagemath/sagetrac-mirror/commit/e5678b4598f3d69f1fe2f5daf26159f149a5500a)** to **[ec9ab87](https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9)**",
    "created_at": "2020-03-02T01:52:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429944",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e5678b4](https://github.com/sagemath/sagetrac-mirror/commit/e5678b4598f3d69f1fe2f5daf26159f149a5500a)** to **[ec9ab87](https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9)**



---

archive/issue_comments_429945.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9\">ec9ab87</a></td><td><code>Merge tag '9.1.beta6' into t/27566/build/ticket-27566</code></td></tr></table>\n",
    "created_at": "2020-03-02T01:52:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429945",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9">ec9ab87</a></td><td><code>Merge tag '9.1.beta6' into t/27566/build/ticket-27566</code></td></tr></table>




---

archive/issue_comments_429946.json:
```json
{
    "body": "Changed dependencies from **#28788** to none",
    "created_at": "2020-03-02T01:52:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429946",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#28788** to none



---

archive/issue_comments_429947.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9\">ec9ab87</a></td><td><code>Merge tag '9.1.beta6' into t/27566/build/ticket-27566</code></td></tr></table>\n",
    "created_at": "2020-03-02T01:52:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429947",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:12'>Comment 12:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9">ec9ab87</a></td><td><code>Merge tag '9.1.beta6' into t/27566/build/ticket-27566</code></td></tr></table>




---

archive/issue_comments_429948.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\n@embray please take a look.",
    "created_at": "2020-03-06T16:53:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429948",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:13'>Comment 13:</a>
@embray please take a look.



---

archive/issue_comments_429949.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nWaiting for review",
    "created_at": "2020-03-15T23:11:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429949",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:15'>Comment 15:</a>
Waiting for review



---

archive/issue_events_360293.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-03-15T23:57:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360293"
}
```



---

archive/issue_events_360294.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-03-15T23:57:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360294"
}
```



---

archive/issue_comments_429950.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nI hate to slow down the configure script even more to display a message about a one-in-a-million error condition. The change to `sage_spkg_enable.m4` alone avoids the error. But everything works as advertised.",
    "created_at": "2020-03-15T23:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429950",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:16'>Comment 16:</a>
I hate to slow down the configure script even more to display a message about a one-in-a-million error condition. The change to `sage_spkg_enable.m4` alone avoids the error. But everything works as advertised.



---

archive/issue_comments_429951.json:
```json
{
    "body": "Reviewer: **Michael Orlitzky**",
    "created_at": "2020-03-15T23:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429951",
    "user": "https://github.com/orlitzky"
}
```

Reviewer: **Michael Orlitzky**



---

archive/issue_comments_429952.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nThanks!",
    "created_at": "2020-03-15T23:58:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429952",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:17'>Comment 17:</a>
Thanks!



---

archive/issue_comments_429953.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/build/ticket-27566](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build/ticket-27566)** to **[ec9ab87](https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9)**",
    "created_at": "2020-03-18T22:40:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27566#issuecomment-429953",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/build/ticket-27566](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build/ticket-27566)** to **[ec9ab87](https://github.com/sagemath/sagetrac-mirror/commit/ec9ab87bfdde7f14176d875d74feda838894a0e9)**



---

archive/issue_events_360295.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-03-18T22:40:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360295"
}
```



---

archive/issue_events_360296.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fee8b8628bed1a14e487ceffc1bd92b160deaddc",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-03-18T22:40:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27566",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27566#event-360296"
}
```
