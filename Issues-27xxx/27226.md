# Issue 27226: Convert back vectors from Maple

archive/issues_026989.json:
```json
{
    "assignees": [],
    "body": "With Maple installed, and with this ticket, we get:\n\n```\nsage: m = vector([1, x, 2, 3])\nsage: mm = maple(m)\nsage: mm\nVector[row](4, [1,x,2,3])\nsage: mm.sage()\n(1, x, 2, 3)\nsage: mm.sage() == m\nTrue\n```\n\nCompare to the situation with Maple installed, before this ticket:\n\n```\nsage: m = vector([1, x, 2, 3])\nsage: mm = maple(m)\nsage: mm\nVector[row](4, [1,x,2,3])\nsage: mm.sage()\nTraceback (most recent call last)\n...\nNotImplementedError: Unable to parse Maple output: Vector[row](4, [1,x,2,3])\n```\n\n\nCC:  @slel\n\nKeywords: **maple, vector**\n\nBranch/Commit: **[2c28757](https://github.com/sagemath/sagetrac-mirror/commit/2c2875794354be480898f20d95143939ae85c22b)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27226_\n\n",
    "closed_at": "2019-02-08T23:21:36Z",
    "created_at": "2019-02-05T12:39:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Convert back vectors from Maple",
    "type": "issue",
    "updated_at": "2019-02-08T23:21:36Z",
    "url": "https://github.com/sagemath/sage/issues/27226",
    "user": "https://github.com/fchapoton"
}
```
With Maple installed, and with this ticket, we get:

```
sage: m = vector([1, x, 2, 3])
sage: mm = maple(m)
sage: mm
Vector[row](4, [1,x,2,3])
sage: mm.sage()
(1, x, 2, 3)
sage: mm.sage() == m
True
```

Compare to the situation with Maple installed, before this ticket:

```
sage: m = vector([1, x, 2, 3])
sage: mm = maple(m)
sage: mm
Vector[row](4, [1,x,2,3])
sage: mm.sage()
Traceback (most recent call last)
...
NotImplementedError: Unable to parse Maple output: Vector[row](4, [1,x,2,3])
```


CC:  @slel

Keywords: **maple, vector**

Branch/Commit: **[2c28757](https://github.com/sagemath/sagetrac-mirror/commit/2c2875794354be480898f20d95143939ae85c22b)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/27226_





---

archive/issue_events_355940.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-02-05T12:39:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355940"
}
```



---

archive/issue_events_355941.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-02-05T12:39:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355941"
}
```



---

archive/issue_events_355942.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-02-05T12:39:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355942"
}
```



---

archive/issue_events_355943.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-02-05T12:39:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355943"
}
```



---

archive/issue_comments_423370.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cba2467344c83209219596420b04bfc15502b130\">cba2467</a></td><td><code>convert back vectors from maple</code></td></tr></table>\n",
    "created_at": "2019-02-05T12:39:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423370",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cba2467344c83209219596420b04bfc15502b130">cba2467</a></td><td><code>convert back vectors from maple</code></td></tr></table>




---

archive/issue_comments_423371.json:
```json
{
    "body": "Commit: **[cba2467](https://github.com/sagemath/sagetrac-mirror/commit/cba2467344c83209219596420b04bfc15502b130)**",
    "created_at": "2019-02-05T12:39:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423371",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[cba2467](https://github.com/sagemath/sagetrac-mirror/commit/cba2467344c83209219596420b04bfc15502b130)**



---

archive/issue_events_355944.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-02-05T12:39:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355944"
}
```



---

archive/issue_comments_423372.json:
```json
{
    "body": "Branch: **[u/chapoton/27226](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27226)**",
    "created_at": "2019-02-05T12:39:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423372",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/27226](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27226)**



---

archive/issue_comments_423373.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e6f3dcf9a94b55584a0d9c3c434586471530e3cb\">e6f3dcf</a></td><td><code>fix maple conversion of vectors</code></td></tr></table>\n",
    "created_at": "2019-02-05T12:45:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423373",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e6f3dcf9a94b55584a0d9c3c434586471530e3cb">e6f3dcf</a></td><td><code>fix maple conversion of vectors</code></td></tr></table>




---

archive/issue_comments_423374.json:
```json
{
    "body": "Changed commit from **[cba2467](https://github.com/sagemath/sagetrac-mirror/commit/cba2467344c83209219596420b04bfc15502b130)** to **[e6f3dcf](https://github.com/sagemath/sagetrac-mirror/commit/e6f3dcf9a94b55584a0d9c3c434586471530e3cb)**",
    "created_at": "2019-02-05T12:45:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423374",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[cba2467](https://github.com/sagemath/sagetrac-mirror/commit/cba2467344c83209219596420b04bfc15502b130)** to **[e6f3dcf](https://github.com/sagemath/sagetrac-mirror/commit/e6f3dcf9a94b55584a0d9c3c434586471530e3cb)**



---

archive/issue_comments_423375.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1d4ff57561a5c65121a9467f90eb8940106bad06\">1d4ff57</a></td><td><code>fix</code></td></tr></table>\n",
    "created_at": "2019-02-05T12:47:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423375",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1d4ff57561a5c65121a9467f90eb8940106bad06">1d4ff57</a></td><td><code>fix</code></td></tr></table>




---

archive/issue_comments_423376.json:
```json
{
    "body": "Changed commit from **[e6f3dcf](https://github.com/sagemath/sagetrac-mirror/commit/e6f3dcf9a94b55584a0d9c3c434586471530e3cb)** to **[1d4ff57](https://github.com/sagemath/sagetrac-mirror/commit/1d4ff57561a5c65121a9467f90eb8940106bad06)**",
    "created_at": "2019-02-05T12:47:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423376",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e6f3dcf](https://github.com/sagemath/sagetrac-mirror/commit/e6f3dcf9a94b55584a0d9c3c434586471530e3cb)** to **[1d4ff57](https://github.com/sagemath/sagetrac-mirror/commit/1d4ff57561a5c65121a9467f90eb8940106bad06)**



---

archive/issue_comments_423377.json:
```json
{
    "body": "Changed commit from **[1d4ff57](https://github.com/sagemath/sagetrac-mirror/commit/1d4ff57561a5c65121a9467f90eb8940106bad06)** to **[2c28757](https://github.com/sagemath/sagetrac-mirror/commit/2c2875794354be480898f20d95143939ae85c22b)**",
    "created_at": "2019-02-05T12:48:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423377",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1d4ff57](https://github.com/sagemath/sagetrac-mirror/commit/1d4ff57561a5c65121a9467f90eb8940106bad06)** to **[2c28757](https://github.com/sagemath/sagetrac-mirror/commit/2c2875794354be480898f20d95143939ae85c22b)**



---

archive/issue_comments_423378.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2c2875794354be480898f20d95143939ae85c22b\">2c28757</a></td><td><code>convert back vectors from maple</code></td></tr></table>\n",
    "created_at": "2019-02-05T12:48:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423378",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2c2875794354be480898f20d95143939ae85c22b">2c28757</a></td><td><code>convert back vectors from maple</code></td></tr></table>




---

archive/issue_comments_423379.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-02-05T13:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423379",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_355945.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-02-05T13:43:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355945"
}
```



---

archive/issue_events_355946.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-02-05T13:43:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355946"
}
```



---

archive/issue_comments_423380.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLGTM.",
    "created_at": "2019-02-05T13:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423380",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
LGTM.



---

archive/issue_comments_423381.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,26 @@\n+With Maple installed, and with this ticket, we get:\n \n+```\n+sage: m = vector([1, x, 2, 3])\n+sage: mm = maple(m)\n+sage: mm\n+Vector[row](4, [1,x,2,3])\n+sage: mm.sage()\n+(1, x, 2, 3)\n+sage: mm.sage() == m\n+True\n+```\n+\n+Compare to the situation with Maple installed, before this ticket:\n+\n+```\n+sage: m = vector([1, x, 2, 3])\n+sage: mm = maple(m)\n+sage: mm\n+Vector[row](4, [1,x,2,3])\n+sage: mm.sage()\n+Traceback (most recent call last)\n+...\n+NotImplementedError: Unable to parse Maple output: Vector[row](4, [1,x,2,3])\n+```\n+\n``````\n",
    "created_at": "2019-02-08T02:15:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423381",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,26 @@
+With Maple installed, and with this ticket, we get:
 
+```
+sage: m = vector([1, x, 2, 3])
+sage: mm = maple(m)
+sage: mm
+Vector[row](4, [1,x,2,3])
+sage: mm.sage()
+(1, x, 2, 3)
+sage: mm.sage() == m
+True
+```
+
+Compare to the situation with Maple installed, before this ticket:
+
+```
+sage: m = vector([1, x, 2, 3])
+sage: mm = maple(m)
+sage: mm
+Vector[row](4, [1,x,2,3])
+sage: mm.sage()
+Traceback (most recent call last)
+...
+NotImplementedError: Unable to parse Maple output: Vector[row](4, [1,x,2,3])
+```
+
``````




---

archive/issue_events_355947.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-02-08T02:15:18Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "title_is": "Convert back vectors from Maple",
    "title_was": "convert back vectors from maple",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355947"
}
```



---

archive/issue_comments_423382.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAdding brief overview of what this ticket achieves to the ticket description.",
    "created_at": "2019-02-08T02:15:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423382",
    "user": "https://github.com/slel"
}
```

<a id='comment:6'>Comment 6:</a>
Adding brief overview of what this ticket achieves to the ticket description.



---

archive/issue_comments_423383.json:
```json
{
    "body": "Changed keywords from **** to **maple, vector**",
    "created_at": "2019-02-08T02:15:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423383",
    "user": "https://github.com/slel"
}
```

Changed keywords from **** to **maple, vector**



---

archive/issue_events_355948.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-02-08T23:21:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355948"
}
```



---

archive/issue_events_355949.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "aa1b68b91c31a6be27f7196eda5d42c0c06a8b6f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-02-08T23:21:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27226#event-355949"
}
```



---

archive/issue_comments_423384.json:
```json
{
    "body": "Changed branch from **[u/chapoton/27226](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27226)** to **[2c28757](https://github.com/sagemath/sagetrac-mirror/commit/2c2875794354be480898f20d95143939ae85c22b)**",
    "created_at": "2019-02-08T23:21:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27226#issuecomment-423384",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/27226](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27226)** to **[2c28757](https://github.com/sagemath/sagetrac-mirror/commit/2c2875794354be480898f20d95143939ae85c22b)**
