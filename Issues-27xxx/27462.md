...
```json
{
    "body": "I mean: create sage-toolchain, a helper package that might be needed to build Sage on tricky systems, but not needed in most cases.\n\nWith gcc, gfortran, mpir, gmp, mpfr (and maybe some more things like patch) moved over there.",
...
```json
{
    "body": "But mpir, mpfr, mpc are already existing packages \"like cysignals\". So you're suggesting to bundle these packages in a bigger package, adding another layer of packaging? And this is somehow going to simplify things?",
...
```json
{
    "body": "A package in sense of autotools, if you like. Which can be built and installed somewhere, maybe with `sudo`, using a slimmed down Sage build system.",
...
```json
{
    "body": "I suggest to close this ticket because its description is unclear.",
...
```json
{
    "body": "the question is of course whether to do such a separation is too much work for little gain.",
...
```json
{
    "body": "Are you talking about a binary distribution?",
...
```json
{
    "body": "Where would it be installed?",
