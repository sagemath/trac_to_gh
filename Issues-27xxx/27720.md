# Issue 27720: polymake interface using JuPyMake

archive/issues_027483.json:
```json
{
    "body": "Replace pseudo terminal interactions by use of JuPyMake.\n\nIssue created by migration from https://trac.sagemath.org/ticket/27720\n\n",
    "closed_at": "2019-05-10T05:57:11Z",
    "created_at": "2019-04-25T09:23:26Z",
    "labels": [
        "component: interfaces: optional",
        "enhancement",
        "duplicate"
    ],
    "title": "polymake interface using JuPyMake",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/27720",
    "user": "https://github.com/mkoeppe"
}
```
Replace pseudo terminal interactions by use of JuPyMake.

Issue created by migration from https://trac.sagemath.org/ticket/27720





---

archive/issue_comments_432402.json:
```json
{
    "body": "<a id='comment:1'></a>\ndup of #22704, sorry",
    "created_at": "2019-04-25T10:24:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27720",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27720#issuecomment-432402",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'></a>
dup of #22704, sorry



---

archive/issue_events_244596.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-04-25T10:24:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/27720",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27720#event-244596"
}
```



---

archive/issue_events_244597.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-04-25T10:24:11Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/27720",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27720#event-244597"
}
```



---

archive/issue_events_244598.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-04-25T12:48:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/27720",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27720#event-244598"
}
```



---

archive/issue_events_244599.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-04-25T12:48:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/27720",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27720#event-244599"
}
```



---

archive/issue_comments_432403.json:
```json
{
    "body": "**Description changed:**\n``````diff\n\n``````\n",
    "created_at": "2019-04-30T23:41:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/27720",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/27720#issuecomment-432403",
    "user": "https://github.com/slel"
}
```

**Description changed:**
``````diff

``````




---

archive/issue_events_244600.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-05-10T05:57:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/27720",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27720#event-244600"
}
```



---

archive/issue_events_244601.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-05-10T05:57:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/27720",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27720#event-244601"
}
```



---

archive/issue_events_244602.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-05-10T05:57:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/27720",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/27720#event-244602"
}
```
