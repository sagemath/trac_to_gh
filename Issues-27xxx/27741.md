# Issue 27741: bug in sympy conversion of hypergeometric with tuple arg of length 1

archive/issues_027504.json:
```json
{
    "assignees": [],
    "body": "This fails, because the argument [1] is converted to (1) by the symbolic ring, and then to 1 by sympy itself. Sigh.\n\n```\nsage: from sage.symbolic.expression_conversions import sympy_converter\nsage: F=hypergeometric([1/3,2/3],[1,1],x)\nsage: F._sympy_()\nhyper((1/3, 2/3), (1, 1), x)\nsage: F=hypergeometric([1/3,2/3],[1],x)\nsage: F._sympy_()\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n<ipython-input-61-3877d08c877a> in <module>()\n----> 1 F._sympy_()\n\n/home/chapoton/sage3/local/lib/python3.7/site-packages/sage/symbolic/expression.pyx in sage.symbolic.expression.Expression._sympy_ (build/cythonized/sage/symbolic/expression.cpp:11351)()\n   1456         \"\"\"\n   1457         from sage.symbolic.expression_conversions import sympy_converter\n-> 1458         return sympy_converter(self)\n   1459 \n   1460     def _fricas_init_(self):\n\n/home/chapoton/sage3/local/lib/python3.7/site-packages/sage/symbolic/expression_conversions.py in __call__(self, ex)\n    223             return self.tuple(ex)\n    224         else:\n--> 225             return self.composition(ex, operator)\n    226 \n    227     def get_fake_div(self, ex):\n\n/home/chapoton/sage3/local/lib/python3.7/site-packages/sage/symbolic/expression_conversions.py in composition(self, ex, operator)\n    791         f_sympy = getattr(sympy, f, None)\n    792         if f_sympy:\n--> 793             return f_sympy(*sympy.sympify(g, evaluate=False))\n    794         else:\n    795             return sympy.Function(str(f))(*g, evaluate=False)\n\n/home/chapoton/sage3/local/lib/python3.7/site-packages/sympy/functions/special/hyper.py in __new__(cls, ap, bq, z)\n    181     def __new__(cls, ap, bq, z):\n    182         # TODO should we check convergence conditions?\n--> 183         return Function.__new__(cls, _prep_tuple(ap), _prep_tuple(bq), z)\n    184 \n    185     @classmethod\n\n/home/chapoton/sage3/local/lib/python3.7/site-packages/sympy/functions/special/hyper.py in _prep_tuple(v)\n     43     \"\"\"\n     44     from sympy import unpolarify\n---> 45     return TupleArg(*[unpolarify(x) for x in v])\n     46 \n     47 \n\nTypeError: 'One' object is not iterable\n```\n\nCC:  @rwst @tscrim\n\nBranch/Commit: **[af8fa63](https://github.com/sagemath/sagetrac-mirror/commit/af8fa638df92f4e2e36258a61fa78b6857769608)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/27741_\n\n",
    "closed_at": "2019-05-06T11:56:14Z",
    "created_at": "2019-04-28T09:09:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bug in sympy conversion of hypergeometric with tuple arg of length 1",
    "type": "issue",
    "updated_at": "2019-05-06T11:56:14Z",
    "url": "https://github.com/sagemath/sage/issues/27741",
    "user": "https://github.com/fchapoton"
}
```
This fails, because the argument [1] is converted to (1) by the symbolic ring, and then to 1 by sympy itself. Sigh.

```
sage: from sage.symbolic.expression_conversions import sympy_converter
sage: F=hypergeometric([1/3,2/3],[1,1],x)
sage: F._sympy_()
hyper((1/3, 2/3), (1, 1), x)
sage: F=hypergeometric([1/3,2/3],[1],x)
sage: F._sympy_()
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-61-3877d08c877a> in <module>()
----> 1 F._sympy_()

/home/chapoton/sage3/local/lib/python3.7/site-packages/sage/symbolic/expression.pyx in sage.symbolic.expression.Expression._sympy_ (build/cythonized/sage/symbolic/expression.cpp:11351)()
   1456         """
   1457         from sage.symbolic.expression_conversions import sympy_converter
-> 1458         return sympy_converter(self)
   1459 
   1460     def _fricas_init_(self):

/home/chapoton/sage3/local/lib/python3.7/site-packages/sage/symbolic/expression_conversions.py in __call__(self, ex)
    223             return self.tuple(ex)
    224         else:
--> 225             return self.composition(ex, operator)
    226 
    227     def get_fake_div(self, ex):

/home/chapoton/sage3/local/lib/python3.7/site-packages/sage/symbolic/expression_conversions.py in composition(self, ex, operator)
    791         f_sympy = getattr(sympy, f, None)
    792         if f_sympy:
--> 793             return f_sympy(*sympy.sympify(g, evaluate=False))
    794         else:
    795             return sympy.Function(str(f))(*g, evaluate=False)

/home/chapoton/sage3/local/lib/python3.7/site-packages/sympy/functions/special/hyper.py in __new__(cls, ap, bq, z)
    181     def __new__(cls, ap, bq, z):
    182         # TODO should we check convergence conditions?
--> 183         return Function.__new__(cls, _prep_tuple(ap), _prep_tuple(bq), z)
    184 
    185     @classmethod

/home/chapoton/sage3/local/lib/python3.7/site-packages/sympy/functions/special/hyper.py in _prep_tuple(v)
     43     """
     44     from sympy import unpolarify
---> 45     return TupleArg(*[unpolarify(x) for x in v])
     46 
     47 

TypeError: 'One' object is not iterable
```

CC:  @rwst @tscrim

Branch/Commit: **[af8fa63](https://github.com/sagemath/sagetrac-mirror/commit/af8fa638df92f4e2e36258a61fa78b6857769608)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/27741_





---

archive/issue_events_362354.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-04-28T09:09:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27741#event-362354"
}
```



---

archive/issue_events_362355.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-04-28T09:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27741#event-362355"
}
```



---

archive/issue_events_362356.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-04-28T09:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27741#event-362356"
}
```



---

archive/issue_events_362357.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-04-28T09:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27741#event-362357"
}
```



---

archive/issue_comments_432915.json:
```json
{
    "body": "Branch: **[u/chapoton/27741](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27741)**",
    "created_at": "2019-04-28T09:51:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432915",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/27741](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27741)**



---

archive/issue_comments_432916.json:
```json
{
    "body": "Commit: **[af8fa63](https://github.com/sagemath/sagetrac-mirror/commit/af8fa638df92f4e2e36258a61fa78b6857769608)**",
    "created_at": "2019-04-28T09:51:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432916",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[af8fa63](https://github.com/sagemath/sagetrac-mirror/commit/af8fa638df92f4e2e36258a61fa78b6857769608)**



---

archive/issue_comments_432917.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-04-28T09:51:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432917",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_432918.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/af8fa638df92f4e2e36258a61fa78b6857769608\">af8fa63</a></td><td><code>adding proper conversion of tuples to sympy</code></td></tr></table>\n",
    "created_at": "2019-04-28T09:51:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432918",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/af8fa638df92f4e2e36258a61fa78b6857769608">af8fa63</a></td><td><code>adding proper conversion of tuples to sympy</code></td></tr></table>




---

archive/issue_comments_432919.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\ngreen bot, please review",
    "created_at": "2019-04-28T11:46:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432919",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
green bot, please review



---

archive/issue_events_362358.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-04-28T12:25:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27741#event-362358"
}
```



---

archive/issue_comments_432920.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-04-28T12:25:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432920",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_432921.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMakes sense and LGTM. By [comment:2](#comment%3A2), I am assuming this is ready for review.",
    "created_at": "2019-04-28T12:25:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432921",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
Makes sense and LGTM. By [comment:2](#comment%3A2), I am assuming this is ready for review.



---

archive/issue_events_362359.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-04-28T12:25:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27741#event-362359"
}
```



---

archive/issue_events_362360.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-04-28T12:25:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27741#event-362360"
}
```



---

archive/issue_comments_432922.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nIf not, please revert.",
    "created_at": "2019-04-28T12:25:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432922",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
If not, please revert.



---

archive/issue_comments_432923.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nyes, thanks a lot for the super-fast review!",
    "created_at": "2019-04-28T12:32:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432923",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
yes, thanks a lot for the super-fast review!



---

archive/issue_events_362361.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-06T11:56:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27741#event-362361"
}
```



---

archive/issue_events_362362.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c1b8bd1bcf96f9bc8919a9010d494a317cdf2e0d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-05-06T11:56:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/27741#event-362362"
}
```



---

archive/issue_comments_432924.json:
```json
{
    "body": "Changed branch from **[u/chapoton/27741](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27741)** to **[af8fa63](https://github.com/sagemath/sagetrac-mirror/commit/af8fa638df92f4e2e36258a61fa78b6857769608)**",
    "created_at": "2019-05-06T11:56:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/27741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/27741#issuecomment-432924",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/27741](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/27741)** to **[af8fa63](https://github.com/sagemath/sagetrac-mirror/commit/af8fa638df92f4e2e36258a61fa78b6857769608)**
