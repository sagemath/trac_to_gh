# Issue 18033: Refactor graphics / graphics array

archive/issues_017796.json:
```json
{
    "assignees": [],
    "body": "In graphics.py, a bit of refactoring could be done\nby creating a common parent for `Graphics`\nand `GraphicsArray` (maybe something like `GraphicsBase`).\n\nVarious show methods should maybe get moved to the\n`SageObject` class, or perhaps in some category.\n\n\nDepends on #17821\n\nCC:  @novoselt @slel\n\nComponent: **graphics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18033_\n\n",
    "created_at": "2015-03-21T16:08:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/wishlist%20item",
        "https://github.com/sagemath/sage/labels/needs%20info"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Refactor graphics / graphics array",
    "type": "issue",
    "updated_at": "2021-05-02T23:37:21Z",
    "url": "https://github.com/sagemath/sage/issues/18033",
    "user": "https://github.com/vbraun"
}
```
In graphics.py, a bit of refactoring could be done
by creating a common parent for `Graphics`
and `GraphicsArray` (maybe something like `GraphicsBase`).

Various show methods should maybe get moved to the
`SageObject` class, or perhaps in some category.


Depends on #17821

CC:  @novoselt @slel

Component: **graphics**

_Issue created by migration from https://trac.sagemath.org/ticket/18033_





---

archive/issue_events_256508.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-21T16:08:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18033#event-256508"
}
```



---

archive/issue_events_256509.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-21T16:08:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18033#event-256509"
}
```



---

archive/issue_events_256510.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-21T16:08:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18033#event-256510"
}
```



---

archive/issue_events_256511.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-21T16:08:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18033#event-256511"
}
```



---

archive/issue_comments_245557.json:
```json
{
    "body": "Dependencies: **#17821**",
    "created_at": "2015-03-21T16:09:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18033#issuecomment-245557",
    "user": "https://github.com/vbraun"
}
```

Dependencies: **#17821**



---

archive/issue_comments_245558.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,4 @@\n In graphics.py, a bit of refactoring could be done by creating a common parent for Graphics and GraphicsArray (maybe something like GraphicsBase)\n \n+Various show methods should maybe get moved to the SageObject class, or perhaps in some category.\n+\n``````\n",
    "created_at": "2015-03-21T16:14:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18033#issuecomment-245558",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,4 @@
 In graphics.py, a bit of refactoring could be done by creating a common parent for Graphics and GraphicsArray (maybe something like GraphicsBase)
 
+Various show methods should maybe get moved to the SageObject class, or perhaps in some category.
+
``````




---

archive/issue_comments_245559.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nIf #27865 solved this, set the ticket's milestone\nto sage-duplicate/invalid/wontfix, so we can close.\n\nOtherwise, it would be good to assess more precisely\nwhat is left to do here.",
    "created_at": "2021-05-02T23:37:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18033#issuecomment-245559",
    "user": "https://github.com/slel"
}
```

<div id="comment:4" align="right">comment:4</div>

If #27865 solved this, set the ticket's milestone
to sage-duplicate/invalid/wontfix, so we can close.

Otherwise, it would be good to assess more precisely
what is left to do here.



---

archive/issue_events_256512.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-05-02T23:37:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18033#event-256512"
}
```



---

archive/issue_events_256513.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-05-02T23:37:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18033#event-256513"
}
```



---

archive/issue_events_256514.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-05-02T23:37:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18033#event-256514"
}
```



---

archive/issue_events_256515.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-05-02T23:37:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18033#event-256515"
}
```



---

archive/issue_comments_245560.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,7 @@\n-In graphics.py, a bit of refactoring could be done by creating a common parent for Graphics and GraphicsArray (maybe something like GraphicsBase)\n+In graphics.py, a bit of refactoring could be done\n+by creating a common parent for `Graphics`\n+and `GraphicsArray` (maybe something like `GraphicsBase`).\n \n-Various show methods should maybe get moved to the SageObject class, or perhaps in some category.\n+Various show methods should maybe get moved to the\n+`SageObject` class, or perhaps in some category.\n \n``````\n",
    "created_at": "2021-05-02T23:37:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18033#issuecomment-245560",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,7 @@
-In graphics.py, a bit of refactoring could be done by creating a common parent for Graphics and GraphicsArray (maybe something like GraphicsBase)
+In graphics.py, a bit of refactoring could be done
+by creating a common parent for `Graphics`
+and `GraphicsArray` (maybe something like `GraphicsBase`).
 
-Various show methods should maybe get moved to the SageObject class, or perhaps in some category.
+Various show methods should maybe get moved to the
+`SageObject` class, or perhaps in some category.
 
``````

