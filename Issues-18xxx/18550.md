# Issue 18550: Preserve matrix subdivisions with sage input command

archive/issues_018313.json:
```json
{
    "assignees": [],
    "body": "A matrix can carry subdivisions.  In this case, the `sage_input()` command should output an expression that creates the matrix **along with** the necessary input to also create the subdivisions.\n\nDepends on #18481\n\nCC:  @ThomasGagne\n\nKeywords: **matrix subdivision sage_input**\n\nBranch/Commit: **[u/rbeezer/preserve-matrix-subdivisions](https://github.com/sagemath/sagetrac-mirror/tree/u/rbeezer/preserve-matrix-subdivisions) @ [316fcd7](https://github.com/sagemath/sagetrac-mirror/commit/316fcd7e7b41d75298c2b9ddd8dc2e321b10eff0)**\n\nAuthor: **Rob Beezer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18550_\n\n",
    "created_at": "2015-05-30T02:54:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Preserve matrix subdivisions with sage input command",
    "type": "issue",
    "updated_at": "2022-12-29T01:32:56Z",
    "url": "https://github.com/sagemath/sage/issues/18550",
    "user": "https://github.com/rbeezer"
}
```
A matrix can carry subdivisions.  In this case, the `sage_input()` command should output an expression that creates the matrix **along with** the necessary input to also create the subdivisions.

Depends on #18481

CC:  @ThomasGagne

Keywords: **matrix subdivision sage_input**

Branch/Commit: **[u/rbeezer/preserve-matrix-subdivisions](https://github.com/sagemath/sagetrac-mirror/tree/u/rbeezer/preserve-matrix-subdivisions) @ [316fcd7](https://github.com/sagemath/sagetrac-mirror/commit/316fcd7e7b41d75298c2b9ddd8dc2e321b10eff0)**

Author: **Rob Beezer**

_Issue created by migration from https://trac.sagemath.org/ticket/18550_





---

archive/issue_events_245964.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2015-05-30T02:54:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18550#event-245964"
}
```



---

archive/issue_events_245965.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2015-05-30T02:54:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18550#event-245965"
}
```



---

archive/issue_events_245966.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2015-05-30T02:54:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18550#event-245966"
}
```



---

archive/issue_events_245967.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2015-05-30T02:54:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18550#event-245967"
}
```



---

archive/issue_comments_260047.json:
```json
{
    "body": "Dependencies: **#18481**",
    "created_at": "2015-05-30T03:21:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18550#issuecomment-260047",
    "user": "https://github.com/rbeezer"
}
```

Dependencies: **#18481**



---

archive/issue_comments_260048.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e974b54136e531ca6040455a1f5af72dc0a3a234\">e974b54</a></td><td><code>18481: optionally copy matrix when adding subdivisions</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/316fcd7e7b41d75298c2b9ddd8dc2e321b10eff0\">316fcd7</a></td><td><code>18550: preserve matrix subdivisions for sage_input</code></td></tr></table>\n",
    "created_at": "2015-05-30T03:21:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18550#issuecomment-260048",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e974b54136e531ca6040455a1f5af72dc0a3a234">e974b54</a></td><td><code>18481: optionally copy matrix when adding subdivisions</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/316fcd7e7b41d75298c2b9ddd8dc2e321b10eff0">316fcd7</a></td><td><code>18550: preserve matrix subdivisions for sage_input</code></td></tr></table>




---

archive/issue_comments_260049.json:
```json
{
    "body": "Commit: **[316fcd7](https://github.com/sagemath/sagetrac-mirror/commit/316fcd7e7b41d75298c2b9ddd8dc2e321b10eff0)**",
    "created_at": "2015-05-30T03:21:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18550#issuecomment-260049",
    "user": "https://github.com/rbeezer"
}
```

Commit: **[316fcd7](https://github.com/sagemath/sagetrac-mirror/commit/316fcd7e7b41d75298c2b9ddd8dc2e321b10eff0)**



---

archive/issue_comments_260050.json:
```json
{
    "body": "Branch: **[u/rbeezer/preserve-matrix-subdivisions](https://github.com/sagemath/sagetrac-mirror/tree/u/rbeezer/preserve-matrix-subdivisions)**",
    "created_at": "2015-05-30T03:21:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18550#issuecomment-260050",
    "user": "https://github.com/rbeezer"
}
```

Branch: **[u/rbeezer/preserve-matrix-subdivisions](https://github.com/sagemath/sagetrac-mirror/tree/u/rbeezer/preserve-matrix-subdivisions)**



---

archive/issue_comments_260051.json:
```json
{
    "body": "Author: **Rob Beezer**",
    "created_at": "2015-05-30T03:21:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18550#issuecomment-260051",
    "user": "https://github.com/rbeezer"
}
```

Author: **Rob Beezer**



---

archive/issue_events_245968.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2015-05-30T03:21:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18550#event-245968"
}
```



---

archive/issue_events_245969.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2017-01-12T06:56:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18550#event-245969"
}
```



---

archive/issue_events_245970.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2017-01-12T06:56:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18550#event-245970"
}
```



---

archive/issue_comments_260052.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nmerge conflict.",
    "created_at": "2017-01-12T06:56:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18550#issuecomment-260052",
    "user": "https://github.com/cheuberg"
}
```

<a id='comment:2'>Comment 2:</a>
merge conflict.



---

archive/issue_events_245971.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:32:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18550",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18550#event-245971"
}
```
