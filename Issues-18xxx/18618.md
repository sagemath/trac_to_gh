...
```json
{
    ...
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262228"
}
```
...
```json
{
    ...
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262229"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262230"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262231"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262232"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262233"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262234"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262235"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262236"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262237"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262238"
}
```
...
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nReplying to [@sagetrac-dperkinson](#comment%3A25):\n> Replying to [@sagetrac-dperkinson](#comment%3A24):\n> > Replying to [@sagetrac-dperkinson](#comment%3A22):\n> > > Replying to [@tscrim](#comment%3A21):\n> > > > Replying to [@sagetrac-dperkinson](#comment%3A20):\n> > > > > Replying to [@tscrim](#comment%3A19):\n> > > > > > Replying to [@sagetrac-dperkinson](#comment%3A18):\n> > > > > > > Should I remove every instance of INPUT for which there is no input (None) and, separately, every instance of OUTPUT for which there is no output (None)?  The documentation for developers at sagemath.org states that every function must have an INPUT and OUTPUT statement, and further that this \"is not optional\".\n\n> > > > > > \n> > > > > > *tries to put politics aside* These add clutter to the docstring and are a *very* good exception to the rule. So yes, please remove them.\n> > > > > > \n\n> > > > > \n> > > > > Just to be completely clear: suppose, for example, that I have a docstring with None listed under INPUT and integer listed under OUTPUT.  Then I should remove the INPUT section, leaving the OUTPUT section, right?\n\n> > > > \n> > > > Yes, that is correct. \n> > > > \n> > > > > > > At any rate, for this new version, with my local version of Sage, all tests passed for \"sage -t\", and the html and pdf documentation built without problems.\n\n> > > > > > \n> > > > > > However there is currently a merge conflict with the latest beta. So you'll need to rebase it.\n\n> > > > > \n> > > > > I am new to git, so I could use some guidance here.  I have been working in a directory called git-trac-command.  The output from \"git branch\" shows two branches: ``master`` and ``t/18618/sandpile_revisions``, with an asterisk next to the latter.  After my initial git push command, I have modified files in another, separate, copy of sage.  When I done making and testing the revisions, I copy the relevant modified files (sandpile.py, examples.py, etc.) to git-trac-command/src/sage/sandpiles.  I then run ``./sage -br``, ``git add NEW FILES``, ``git commit``, and ``git trac push``.\n> > > > > \n> > > > > What is the sequence of commands I should enter to resolve the current conflict?  Also, what should I have done originally to have avoided the conflict?\n> > > > > \n> > > > > Thanks very much for your time and advice!\n\n> > > > \n> > > > What you'll need to do is the following:\n> > > > \n> > > > ```\n> > > > $ git fetch <remote_name> develop:develop\n> > > > $ git checkout t/18618/sandpile_revisions     # If you aren't already in that branch\n> > > > $ git merge develop\n> > > > \n> > > >     Do the rebasing\n> > > > \n> > > > $ git commit\n> > > > $ make           # In the $SAGE_ROOT directory. This will take a *long* time\n> > > > $ git trac push\n> > > > ```\n> > > > where `<remote_name>` is likely `origin` (possibly `trac`).\n\n> > > >\n\n> > > \n> > > Running \"git merge develop\" leads to the error:\n> > > \n> > > Automatic merge failed; fix conflicts and then commit the result.\n> > >  \n> > > > For rebuilding Sage with the `make`, you might want to do these beforehand to help speed it up:\n> > > > \n> > > > ```\n> > > > $ make doc-clean\n> > > > $ export SAGE_NUM_THREADS=n\n> > > > $ export MAKE='make -jn'\n> > > > $ export SAGE_PARALLEL_SPKG_BUILD\n> > > > ```\n> > > > where `n` is the number of processors on your computer. If it happens to fail during the doc-building part at the end, don't worry about it.\n\n> > \n> > Oops.  Entered \"make doc clean\" by accident.  Advice?\n\n> \n> I just hit ctrl-C, then entered \"make distclean\" and proceeded as suggested above.  Hope that works.  Thanks again for your help.\n\nDone.  Hope that worked.",
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262239"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262240"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262241"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262242"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262243"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262244"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262245"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262246"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262247"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262248"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262249"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262250"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262251"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262252"
}
```
...
```json
{
    ...
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262253"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sage/issues/18618",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18618#event-262254"
}
```
