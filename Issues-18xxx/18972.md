...
```json
{
    "body": "TODO: constructing the descent graph directly from a graph should be much faster.",
...
```json
{
    "body": "Should we add individual graphs to the database here?",
...
```json
{
    "body": "Well, unless you expect the code to be long/complicated, I'd say yes.\n\nNathann",
...
```json
{
    "body": "I already made comments on the code of `seidel_switching`, and you need to document what a `twograph_descendant` is. I am not sure that this function should be in `Graph`, though. If it is just a bit faster than using the `TwoGraph` class, maybe it is enough to make it appear in a `seealso` inside of `twograph` and keep it in the `two_graph` module.\n\nBut really, all in all, I don't see the point of creating a new module, a new class and 6 functions, if all it takes to build the descendant of a graph is 5 lines of code.\n\nNathann",
...
```json
{
    "body": "Is there a general way to *guess* how those 2-graph entries are produced? Couldn't we turn this construction into something automatic? Or will those 2-graph construction all have to be added manually to the list?",
...
```json
{
    "body": "please have a look at the latest commit in particular.",
    "created_at": "2015-08-05T14:50:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18972",
    "type": "issue_comment",
...
```json
{
    "body": "You really need to start thinking of what exactly your code does. Plus it is python, not lisp.\n\n```\nreturn TwoGraph(filter(lambda x: not list(x) in self.blocks(), \n                combinations(self.ground_set(), 3)))\n```",
...
```json
{
    "body": "Seriously? `from sage.misc.functional import is_even`\n\nIs it now wrong to write 'x%2==0' ?\n\nNathann",
...
```json
{
    "body": "The branch is in conflict with the latest beta.",
...
```json
{
    "body": "merged with #19019. I have addressed the issues from comment 53, etc.",
...
```json
{
    "body": "> > It's surprising, at least for me. Would you object if I added a commit that makes it an inplace function, with an optional `inplace=False` to get a copy?\n\n> \n> no problem, go ahead with this.\n\n\nI added the commit at the same location as previously.\n\nNathann",
...
```json
{
    "body": "Set it to `needs_review` when it will be ready.",
...
```json
{
    "body": "Note that the tests do not pass on the patchbot",
...
```json
{
    "body": "> I don't call `is_t_design()` this way!\n\n\nLine 2 of `is_twograph`.\n\nNathann",
...
```json
{
    "body": "Can you add my two speedup commits to your branch, or do you have something against them?\n\nCould you also answer my question about `designs.TwoGraph`?\n\nNathann",
...
```json
{
    "body": "> degree of a transitive permutation group is the size of its natural domain.\n\n\nOh. Okay `:-P`\n\n> see #19080.\n\n\nThanks !\n\nNathann",
...
```json
{
    "body": "Arg. You already did. I thought that you were justifying why you wanted to keep it.",
...
```json
{
    "body": "> May I refer to a precedent of having classes in `design` catalog to have `TwoGraph` here?\n\n\nAn error is not a 'precedent'.\n\nNathann",
...
```json
{
    "body": "> I will mess with `__init__` on #19098, if you don't mind.\n\n\nNo prob, thanks!\n\nNathann\n\nP.S.: Please fill in your name in the Author field",
