# Issue 18069: Two bugs of sparse polynomials with interval coefficients

archive/issues_017832.json:
```json
{
    "assignees": [],
    "body": "A bug affecting their creation from a coefficient list:\n\n```\nsage: pol = PolynomialRing(RIF, 'z', sparse=True)([RIF(-1, 1), RIF(-1,1)])\nsage: pol.degree()\n-1\n```\nand a related printing bug:\n\n```\nsage: pol = PolynomialRing(RIF, 'z', sparse=True)(RIF['x']([RIF(-1, 1), RIF(-1,1)]))\nsage: pol.degree()\n1\nsage: pol\n0\n```\n\nBranch/Commit: **[c492cad](https://github.com/sagemath/sagetrac-mirror/commit/c492cad52756b644486647ac001ea4bef40cb2ec)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Marc Mezzarobba**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18069_\n\n",
    "closed_at": "2015-04-14T19:43:29Z",
    "created_at": "2015-03-27T13:48:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Two bugs of sparse polynomials with interval coefficients",
    "type": "issue",
    "updated_at": "2015-04-14T19:43:29Z",
    "url": "https://github.com/sagemath/sage/issues/18069",
    "user": "https://github.com/mezzarobba"
}
```
A bug affecting their creation from a coefficient list:

```
sage: pol = PolynomialRing(RIF, 'z', sparse=True)([RIF(-1, 1), RIF(-1,1)])
sage: pol.degree()
-1
```
and a related printing bug:

```
sage: pol = PolynomialRing(RIF, 'z', sparse=True)(RIF['x']([RIF(-1, 1), RIF(-1,1)]))
sage: pol.degree()
1
sage: pol
0
```

Branch/Commit: **[c492cad](https://github.com/sagemath/sagetrac-mirror/commit/c492cad52756b644486647ac001ea4bef40cb2ec)**

Reviewer: **Jeroen Demeyer**

Author: **Marc Mezzarobba**

_Issue created by migration from https://trac.sagemath.org/ticket/18069_





---

archive/issue_events_239309.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-27T13:48:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18069#event-239309"
}
```



---

archive/issue_events_239310.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-27T13:48:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18069#event-239310"
}
```



---

archive/issue_events_239311.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-27T13:48:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18069#event-239311"
}
```



---

archive/issue_events_239312.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-27T13:48:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18069#event-239312"
}
```



---

archive/issue_comments_248794.json:
```json
{
    "body": "Commit: **[c492cad](https://github.com/sagemath/sagetrac-mirror/commit/c492cad52756b644486647ac001ea4bef40cb2ec)**",
    "created_at": "2015-03-27T13:54:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18069#issuecomment-248794",
    "user": "https://github.com/mezzarobba"
}
```

Commit: **[c492cad](https://github.com/sagemath/sagetrac-mirror/commit/c492cad52756b644486647ac001ea4bef40cb2ec)**



---

archive/issue_comments_248795.json:
```json
{
    "body": "Branch: **[u/mmezzarobba/18069-sparse_interval_polys](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/18069-sparse_interval_polys)**",
    "created_at": "2015-03-27T13:54:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18069#issuecomment-248795",
    "user": "https://github.com/mezzarobba"
}
```

Branch: **[u/mmezzarobba/18069-sparse_interval_polys](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/18069-sparse_interval_polys)**



---

archive/issue_events_239313.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-27T13:54:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18069#event-239313"
}
```



---

archive/issue_comments_248796.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c492cad52756b644486647ac001ea4bef40cb2ec\">c492cad</a></td><td><code>#18069 Fix two incorrect zero tests in sparse polynomials</code></td></tr></table>\n",
    "created_at": "2015-03-27T13:54:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18069#issuecomment-248796",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c492cad52756b644486647ac001ea4bef40cb2ec">c492cad</a></td><td><code>#18069 Fix two incorrect zero tests in sparse polynomials</code></td></tr></table>




---

archive/issue_comments_248797.json:
```json
{
    "body": "Author: **Marc Mezzarobba**",
    "created_at": "2015-03-27T13:54:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18069#issuecomment-248797",
    "user": "https://github.com/mezzarobba"
}
```

Author: **Marc Mezzarobba**



---

archive/issue_comments_248798.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-03-27T15:01:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18069#issuecomment-248798",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_239314.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-03-27T15:01:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18069#event-239314"
}
```



---

archive/issue_events_239315.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-03-27T15:01:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18069#event-239315"
}
```



---

archive/issue_comments_248799.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks!",
    "created_at": "2015-03-27T15:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18069#issuecomment-248799",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks!



---

archive/issue_events_239316.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-14T19:43:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18069#event-239316"
}
```



---

archive/issue_events_239317.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "900eee3bbc69b61daf9c127c5e73d53c8d73fd7e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-04-14T19:43:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18069#event-239317"
}
```



---

archive/issue_comments_248800.json:
```json
{
    "body": "Changed branch from **[u/mmezzarobba/18069-sparse_interval_polys](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/18069-sparse_interval_polys)** to **[c492cad](https://github.com/sagemath/sagetrac-mirror/commit/c492cad52756b644486647ac001ea4bef40cb2ec)**",
    "created_at": "2015-04-14T19:43:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18069",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18069#issuecomment-248800",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmezzarobba/18069-sparse_interval_polys](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/18069-sparse_interval_polys)** to **[c492cad](https://github.com/sagemath/sagetrac-mirror/commit/c492cad52756b644486647ac001ea4bef40cb2ec)**
