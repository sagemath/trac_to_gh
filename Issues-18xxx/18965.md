# Issue 18965: New Method: FiniteStateMachine.concatenation

archive/issues_018728.json:
```json
{
    "assignees": [],
    "body": "Implement concatenation of two finite state machines as method `.concatenation` as well as `__mul__`.\n\nDepends on #18557\nDepends on #18973\n\nCC:  @sagetrac-skropf @dkrenn @seblabbe\n\nBranch/Commit: **[76e05d6](https://github.com/sagemath/sagetrac-mirror/commit/76e05d6b41f9829af1666bec6c2eee8bda7121a6)**\n\nReviewer: **Daniel Krenn**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18965_\n\n",
    "closed_at": "2015-09-09T22:14:57Z",
    "created_at": "2015-07-29T17:07:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "New Method: FiniteStateMachine.concatenation",
    "type": "issue",
    "updated_at": "2015-09-09T22:14:57Z",
    "url": "https://github.com/sagemath/sage/issues/18965",
    "user": "https://github.com/cheuberg"
}
```
Implement concatenation of two finite state machines as method `.concatenation` as well as `__mul__`.

Depends on #18557
Depends on #18973

CC:  @sagetrac-skropf @dkrenn @seblabbe

Branch/Commit: **[76e05d6](https://github.com/sagemath/sagetrac-mirror/commit/76e05d6b41f9829af1666bec6c2eee8bda7121a6)**

Reviewer: **Daniel Krenn**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/18965_





---

archive/issue_events_251473.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-07-29T17:07:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251473"
}
```



---

archive/issue_events_251474.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-07-29T17:07:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
    "label_color": "0000ff",
    "label_name": "component: finite state machines",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251474"
}
```



---

archive/issue_events_251475.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-07-29T17:07:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251475"
}
```



---

archive/issue_events_251476.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-07-29T17:07:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251476"
}
```



---

archive/issue_comments_269086.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c\">ea021cc</a></td><td><code>Trac #18556: FiniteStateMachine.is_deterministic: several initial states</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ff3a5d8dd34e337dfb885e8b3323276813d30cab\">ff3a5d8</a></td><td><code>Trac #18557: Implement FiniteStateMachine.disjoint_union (and .__or__)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a89d3eacd542878fba7b6037412095a3aafeafb4\">a89d3ea</a></td><td><code>Trac #18557: One typo</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fee74b1e0e2e372bde97f3e71107da1f98688d1f\">fee74b1</a></td><td><code>Trac #18557: Remark on alternative notation in documentation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2583d431566b9cc35b0decd0fcc396decbbf7b2e\">2583d43</a></td><td><code>Trac #18973: New automaton recognizing a word</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cb888a6a3e04749591ebc5779352645e628f4606\">cb888a6</a></td><td><code>Trac #18965: Merge #18557 and #18973</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b4a7317bdedd93ad9c31265d8f77177d2811ff52\">b4a7317</a></td><td><code>Trac #18965: Implement FiniteStateMachine.concatenation</code></td></tr></table>\n",
    "created_at": "2015-08-01T03:50:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269086",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c">ea021cc</a></td><td><code>Trac #18556: FiniteStateMachine.is_deterministic: several initial states</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ff3a5d8dd34e337dfb885e8b3323276813d30cab">ff3a5d8</a></td><td><code>Trac #18557: Implement FiniteStateMachine.disjoint_union (and .__or__)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a89d3eacd542878fba7b6037412095a3aafeafb4">a89d3ea</a></td><td><code>Trac #18557: One typo</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fee74b1e0e2e372bde97f3e71107da1f98688d1f">fee74b1</a></td><td><code>Trac #18557: Remark on alternative notation in documentation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2583d431566b9cc35b0decd0fcc396decbbf7b2e">2583d43</a></td><td><code>Trac #18973: New automaton recognizing a word</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cb888a6a3e04749591ebc5779352645e628f4606">cb888a6</a></td><td><code>Trac #18965: Merge #18557 and #18973</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b4a7317bdedd93ad9c31265d8f77177d2811ff52">b4a7317</a></td><td><code>Trac #18965: Implement FiniteStateMachine.concatenation</code></td></tr></table>




---

archive/issue_comments_269087.json:
```json
{
    "body": "Branch: **[u/cheuberg/fsm/concatenation](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/concatenation)**",
    "created_at": "2015-08-01T03:50:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269087",
    "user": "https://github.com/cheuberg"
}
```

Branch: **[u/cheuberg/fsm/concatenation](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/concatenation)**



---

archive/issue_events_251477.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-08-01T03:50:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251477"
}
```



---

archive/issue_comments_269088.json:
```json
{
    "body": "Dependencies: **#18557, #18973**",
    "created_at": "2015-08-01T03:50:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269088",
    "user": "https://github.com/cheuberg"
}
```

Dependencies: **#18557, #18973**



---

archive/issue_comments_269089.json:
```json
{
    "body": "Commit: **[b4a7317](https://github.com/sagemath/sagetrac-mirror/commit/b4a7317bdedd93ad9c31265d8f77177d2811ff52)**",
    "created_at": "2015-08-01T03:50:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269089",
    "user": "https://github.com/cheuberg"
}
```

Commit: **[b4a7317](https://github.com/sagemath/sagetrac-mirror/commit/b4a7317bdedd93ad9c31265d8f77177d2811ff52)**



---

archive/issue_comments_269090.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/70d37e150750d45e54fee05cfba46f4e76beeb3b\">70d37e1</a></td><td><code>Trac #18965: run determine_alphabets by default</code></td></tr></table>\n",
    "created_at": "2015-08-03T05:37:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269090",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/70d37e150750d45e54fee05cfba46f4e76beeb3b">70d37e1</a></td><td><code>Trac #18965: run determine_alphabets by default</code></td></tr></table>




---

archive/issue_comments_269091.json:
```json
{
    "body": "Changed commit from **[b4a7317](https://github.com/sagemath/sagetrac-mirror/commit/b4a7317bdedd93ad9c31265d8f77177d2811ff52)** to **[70d37e1](https://github.com/sagemath/sagetrac-mirror/commit/70d37e150750d45e54fee05cfba46f4e76beeb3b)**",
    "created_at": "2015-08-03T05:37:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269091",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b4a7317](https://github.com/sagemath/sagetrac-mirror/commit/b4a7317bdedd93ad9c31265d8f77177d2811ff52)** to **[70d37e1](https://github.com/sagemath/sagetrac-mirror/commit/70d37e150750d45e54fee05cfba46f4e76beeb3b)**



---

archive/issue_comments_269092.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThe input alphabet should be the union of the two input alphabets (instead of `determine_alphabets`).",
    "created_at": "2015-08-05T05:10:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269092",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:3" align="right">Comment 3</div>

The input alphabet should be the union of the two input alphabets (instead of `determine_alphabets`).



---

archive/issue_comments_269093.json:
```json
{
    "body": "Work Issues: **input alphabet**",
    "created_at": "2015-08-05T05:10:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269093",
    "user": "https://github.com/cheuberg"
}
```

Work Issues: **input alphabet**



---

archive/issue_events_251478.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-08-05T05:10:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251478"
}
```



---

archive/issue_events_251479.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-08-05T05:10:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251479"
}
```



---

archive/issue_comments_269094.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e926533e5e28c3ffc01bdd247515b9beb8c03323\">e926533</a></td><td><code>Revert \"Trac #18965: run determine_alphabets by default\"</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cdbe026a999486b107129555e5a3291d6f49d120\">cdbe026</a></td><td><code>Trac #18965: Handle input_alphabet as in disjoint_union (#18989)</code></td></tr></table>\n",
    "created_at": "2015-08-05T06:08:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269094",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e926533e5e28c3ffc01bdd247515b9beb8c03323">e926533</a></td><td><code>Revert "Trac #18965: run determine_alphabets by default"</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cdbe026a999486b107129555e5a3291d6f49d120">cdbe026</a></td><td><code>Trac #18965: Handle input_alphabet as in disjoint_union (#18989)</code></td></tr></table>




---

archive/issue_comments_269095.json:
```json
{
    "body": "Changed commit from **[70d37e1](https://github.com/sagemath/sagetrac-mirror/commit/70d37e150750d45e54fee05cfba46f4e76beeb3b)** to **[cdbe026](https://github.com/sagemath/sagetrac-mirror/commit/cdbe026a999486b107129555e5a3291d6f49d120)**",
    "created_at": "2015-08-05T06:08:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269095",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[70d37e1](https://github.com/sagemath/sagetrac-mirror/commit/70d37e150750d45e54fee05cfba46f4e76beeb3b)** to **[cdbe026](https://github.com/sagemath/sagetrac-mirror/commit/cdbe026a999486b107129555e5a3291d6f49d120)**



---

archive/issue_comments_269096.json:
```json
{
    "body": "Changed work issues from **input alphabet** to none",
    "created_at": "2015-08-05T06:58:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269096",
    "user": "https://github.com/cheuberg"
}
```

Changed work issues from **input alphabet** to none



---

archive/issue_events_251480.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-08-05T06:58:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251480"
}
```



---

archive/issue_events_251481.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-08-05T06:58:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251481"
}
```



---

archive/issue_comments_269097.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThe branch does not apply cleanly (marked in red above).",
    "created_at": "2015-08-12T12:07:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269097",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:6" align="right">Comment 6</div>

The branch does not apply cleanly (marked in red above).



---

archive/issue_events_251482.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-08-12T12:07:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251482"
}
```



---

archive/issue_events_251483.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-08-12T12:07:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251483"
}
```



---

archive/issue_comments_269098.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@seblabbe](#comment%3A6):\n> The branch does not apply cleanly (marked in red above).\n\nBranch merges without any problems. This is a \"problem\" of trac (git-plugin or something similar), which does not detect that git merge will be successful. No extra merge commit of 6.9.beta1 is needed; thus set back to \"needs review\"",
    "created_at": "2015-08-12T12:51:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269098",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@seblabbe](#comment%3A6):
> The branch does not apply cleanly (marked in red above).

Branch merges without any problems. This is a "problem" of trac (git-plugin or something similar), which does not detect that git merge will be successful. No extra merge commit of 6.9.beta1 is needed; thus set back to "needs review"



---

archive/issue_events_251484.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2015-08-12T12:51:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251484"
}
```



---

archive/issue_events_251485.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2015-08-12T12:51:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251485"
}
```



---

archive/issue_comments_269099.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/concatenation](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/concatenation)** to **[u/dkrenn/fsm/concatenation](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/concatenation)**",
    "created_at": "2015-09-08T17:51:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269099",
    "user": "https://github.com/dkrenn"
}
```

Changed branch from **[u/cheuberg/fsm/concatenation](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/concatenation)** to **[u/dkrenn/fsm/concatenation](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/concatenation)**



---

archive/issue_comments_269100.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nOne minor reviewer commit (docstring). Is fine --> positive.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/76e05d6b41f9829af1666bec6c2eee8bda7121a6\">76e05d6</a></td><td><code>minor corrections (spacings, .) in docstring</code></td></tr></table>\n",
    "created_at": "2015-09-08T17:53:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269100",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:9" align="right">Comment 9</div>

One minor reviewer commit (docstring). Is fine --> positive.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/76e05d6b41f9829af1666bec6c2eee8bda7121a6">76e05d6</a></td><td><code>minor corrections (spacings, .) in docstring</code></td></tr></table>




---

archive/issue_comments_269101.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2015-09-08T17:53:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269101",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_events_251486.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2015-09-08T17:53:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251486"
}
```



---

archive/issue_events_251487.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2015-09-08T17:53:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251487"
}
```



---

archive/issue_comments_269102.json:
```json
{
    "body": "Changed commit from **[cdbe026](https://github.com/sagemath/sagetrac-mirror/commit/cdbe026a999486b107129555e5a3291d6f49d120)** to **[76e05d6](https://github.com/sagemath/sagetrac-mirror/commit/76e05d6b41f9829af1666bec6c2eee8bda7121a6)**",
    "created_at": "2015-09-08T17:53:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269102",
    "user": "https://github.com/dkrenn"
}
```

Changed commit from **[cdbe026](https://github.com/sagemath/sagetrac-mirror/commit/cdbe026a999486b107129555e5a3291d6f49d120)** to **[76e05d6](https://github.com/sagemath/sagetrac-mirror/commit/76e05d6b41f9829af1666bec6c2eee8bda7121a6)**



---

archive/issue_comments_269103.json:
```json
{
    "body": "Changed branch from **[u/dkrenn/fsm/concatenation](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/concatenation)** to **[76e05d6](https://github.com/sagemath/sagetrac-mirror/commit/76e05d6b41f9829af1666bec6c2eee8bda7121a6)**",
    "created_at": "2015-09-09T22:14:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18965#issuecomment-269103",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dkrenn/fsm/concatenation](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/concatenation)** to **[76e05d6](https://github.com/sagemath/sagetrac-mirror/commit/76e05d6b41f9829af1666bec6c2eee8bda7121a6)**



---

archive/issue_events_251488.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-09T22:14:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251488"
}
```



---

archive/issue_events_251489.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cb90e2d687e682860e8bdbdd3d2868ceb5857a73",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-09-09T22:14:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18965",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18965#event-251489"
}
```
