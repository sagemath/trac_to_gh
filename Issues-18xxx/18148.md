# Issue 18148: No lexicographic iterator over finite binary words

archive/issues_017911.json:
```json
{
    "assignees": [],
    "body": "Sage implements an iterator on finite words n lexicographic order, and such a thing is not possible. In particular:\n\n```\nsage: w=Words([0,1],infinite=False).__iter__()\nsage: w.next()\nword: \nsage: w.next()\nword: 0\nsage: \nsage: w.next()\nword: 1\nsage: w.next()\nword: 00\n```\n\nI see two ways out: stop claiming that it is lexicographic, or remove the function. What do you think?\n\nNathann\n\nP.S.: To whoever types \"git blame\" (like and did) and notices that I am the author of the function: please look at the whole commit, I only moved it to a different place `:-PPP`\n\nCC:  @nthiery @tscrim @nathanncohen @videlec @bgillesp @anneschilling @jdemeyer @seblabbe\n\nBranch/Commit: **[1344cb7](https://github.com/sagemath/sagetrac-mirror/commit/1344cb77f30c0424a2fcc7113467d384eed1ed1a)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18148_\n\n",
    "closed_at": "2015-04-14T19:43:04Z",
    "created_at": "2015-04-09T08:56:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "No lexicographic iterator over finite binary words",
    "type": "issue",
    "updated_at": "2015-04-14T19:43:04Z",
    "url": "https://github.com/sagemath/sage/issues/18148",
    "user": "https://github.com/nathanncohen"
}
```
Sage implements an iterator on finite words n lexicographic order, and such a thing is not possible. In particular:

```
sage: w=Words([0,1],infinite=False).__iter__()
sage: w.next()
word: 
sage: w.next()
word: 0
sage: 
sage: w.next()
word: 1
sage: w.next()
word: 00
```

I see two ways out: stop claiming that it is lexicographic, or remove the function. What do you think?

Nathann

P.S.: To whoever types "git blame" (like and did) and notices that I am the author of the function: please look at the whole commit, I only moved it to a different place `:-PPP`

CC:  @nthiery @tscrim @nathanncohen @videlec @bgillesp @anneschilling @jdemeyer @seblabbe

Branch/Commit: **[1344cb7](https://github.com/sagemath/sagetrac-mirror/commit/1344cb77f30c0424a2fcc7113467d384eed1ed1a)**

Reviewer: **Vincent Delecroix**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/18148_





---

archive/issue_events_240355.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-09T08:56:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240355"
}
```



---

archive/issue_events_240356.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-09T08:56:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240356"
}
```



---

archive/issue_events_240357.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-09T08:56:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240357"
}
```



---

archive/issue_events_240358.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-09T08:57:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240358"
}
```



---

archive/issue_comments_250452.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nIt is not lexicographic but [shortlex](http://en.wikipedia.org/wiki/Shortlex_order) (and natural and useful)! So I would vote for keeping it and adapt the documentation accordingly.\n\nVincent",
    "created_at": "2015-04-09T09:01:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250452",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">Comment 2</div>

It is not lexicographic but [shortlex](http://en.wikipedia.org/wiki/Shortlex_order) (and natural and useful)! So I would vote for keeping it and adapt the documentation accordingly.

Vincent



---

archive/issue_comments_250453.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMakes sense to me. Should we raise a warning for a while, or do we \"not care\" and just change the doc?\n\nNathann",
    "created_at": "2015-04-09T09:03:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250453",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">Comment 3</div>

Makes sense to me. Should we raise a warning for a while, or do we "not care" and just change the doc?

Nathann



---

archive/issue_comments_250454.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nJust change the doc. The iterator does not have to be compatible with the ordering. And the class is not called `FiniteWordsLexIterator` ;-P\n\nVincent",
    "created_at": "2015-04-09T09:16:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250454",
    "user": "https://github.com/videlec"
}
```

<div id="comment:4" align="right">Comment 4</div>

Just change the doc. The iterator does not have to be compatible with the ordering. And the class is not called `FiniteWordsLexIterator` ;-P

Vincent



---

archive/issue_comments_250455.json:
```json
{
    "body": "Commit: **[1344cb7](https://github.com/sagemath/sagetrac-mirror/commit/1344cb77f30c0424a2fcc7113467d384eed1ed1a)**",
    "created_at": "2015-04-09T09:26:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250455",
    "user": "https://github.com/nathanncohen"
}
```

Commit: **[1344cb7](https://github.com/sagemath/sagetrac-mirror/commit/1344cb77f30c0424a2fcc7113467d384eed1ed1a)**



---

archive/issue_comments_250456.json:
```json
{
    "body": "Branch: **[public/18148](https://github.com/sagemath/sagetrac-mirror/tree/public/18148)**",
    "created_at": "2015-04-09T09:26:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250456",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[public/18148](https://github.com/sagemath/sagetrac-mirror/tree/public/18148)**



---

archive/issue_events_240359.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-09T09:26:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240359"
}
```



---

archive/issue_comments_250457.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1344cb77f30c0424a2fcc7113467d384eed1ed1a\">1344cb7</a></td><td><code>trac #18148: No lexicographic iterator over finite binary words</code></td></tr></table>\n",
    "created_at": "2015-04-09T09:26:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250457",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:5" align="right">Comment 5</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1344cb77f30c0424a2fcc7113467d384eed1ed1a">1344cb7</a></td><td><code>trac #18148: No lexicographic iterator over finite binary words</code></td></tr></table>




---

archive/issue_comments_250458.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2015-04-09T09:29:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250458",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_comments_250459.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nWonderful ;-) I like very much one minute tickets!\n\nVincent",
    "created_at": "2015-04-09T09:29:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250459",
    "user": "https://github.com/videlec"
}
```

<div id="comment:6" align="right">Comment 6</div>

Wonderful ;-) I like very much one minute tickets!

Vincent



---

archive/issue_events_240360.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-09T09:29:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240360"
}
```



---

archive/issue_events_240361.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-09T09:29:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240361"
}
```



---

archive/issue_comments_250460.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nWould you have another minute for #18067 ? `:-P`\n\nNathann",
    "created_at": "2015-04-09T09:33:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250460",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:7" align="right">Comment 7</div>

Would you have another minute for #18067 ? `:-P`

Nathann



---

archive/issue_comments_250461.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nauthor name...",
    "created_at": "2015-04-12T09:27:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250461",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:8" align="right">Comment 8</div>

author name...



---

archive/issue_events_240362.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-12T09:27:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240362"
}
```



---

archive/issue_events_240363.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-12T09:27:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240363"
}
```



---

archive/issue_comments_250462.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2015-04-12T09:28:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250462",
    "user": "https://github.com/videlec"
}
```

Author: **Nathann Cohen**



---

archive/issue_events_240364.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-12T09:28:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240364"
}
```



---

archive/issue_events_240365.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-12T09:28:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240365"
}
```



---

archive/issue_comments_250463.json:
```json
{
    "body": "Changed branch from **[public/18148](https://github.com/sagemath/sagetrac-mirror/tree/public/18148)** to **[1344cb7](https://github.com/sagemath/sagetrac-mirror/commit/1344cb77f30c0424a2fcc7113467d384eed1ed1a)**",
    "created_at": "2015-04-14T19:43:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18148#issuecomment-250463",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/18148](https://github.com/sagemath/sagetrac-mirror/tree/public/18148)** to **[1344cb7](https://github.com/sagemath/sagetrac-mirror/commit/1344cb77f30c0424a2fcc7113467d384eed1ed1a)**



---

archive/issue_events_240366.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-14T19:43:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240366"
}
```



---

archive/issue_events_240367.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "888e50c76c09147ac1d9a6d9e8f38d008161521a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-04-14T19:43:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18148",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18148#event-240367"
}
```
