...
<a id='comment:5'></a>Helloooooooooooooooooooo !
...
<a id='comment:10'></a>Helloooooooooooooooooooooo,
...
```json
{
    "body": "<a id='comment:11'></a>Also, remember to change the description of the ticket to reflect what it actually does now.",
...
<a id='comment:11'></a>Also, remember to change the description of the ticket to reflect what it actually does now.
...
<a id='comment:14'></a>Replying to [ncohen](#comment%3A10):
> Helloooooooooooooooooooooo,
...
<a id='comment:24'></a>Replying to [dlucas](#comment%3A21):
> Hello,
> 
> Regarding the naming problem 
> [...]
...
<a id='comment:26'></a>Hello Johan, hello David,
...
<a id='comment:31'></a>Hi Vincent and David,
...
```json
{
    "body": "<a id='comment:34'></a>And I reinstated the `Element ...` line as we will discuss category-related stuff in #18150\n\n---\nNew commits:",
...
<a id='comment:34'></a>And I reinstated the `Element ...` line as we will discuss category-related stuff in #18150
...
<a id='comment:36'></a>Replying to [jsrn](#comment%3A31):
> Also, as David mentions, it is really important for
> us that the generator matrix is not explicitly calculated except when asked for
> (possibly indirectly) by the user. If inheriting `FreeModule_submodule_field`
> would require us to explicitly calculate the generator matrix, I down-vote that
...
<a id='comment:37'></a>>when you are done with your corrections, you should switch the state to needs_review. That way, you have a chance that a reviewer comes back and the patch bot to test your ticket.
...
<a id='comment:42'></a>Hi Johan, hi David,
...
<a id='comment:43'></a>Hi Vincent and David,
...
```json
{
    "body": "<a id='comment:45'></a>I removed remaining trailing whitespaces (I hope I caught them all this time...) and I changed `LinearCode` docstring accordingly to comment 43.",
...
<a id='comment:45'></a>I removed remaining trailing whitespaces (I hope I caught them all this time...) and I changed `LinearCode` docstring accordingly to comment 43.
...
```json
{
    "body": "<a id='comment:46'></a>It is fine with trailing whitespaces now. And all test pass.\n\nVincent",
...
<a id='comment:46'></a>It is fine with trailing whitespaces now. And all test pass.
