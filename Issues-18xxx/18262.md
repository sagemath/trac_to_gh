# Issue 18262: Make centrality_betweenness interruptible

archive/issues_018025.json:
```json
{
    "assignees": [],
    "body": "Make centrality_betweenness interruptible. This is achieved with a try/finally.\n\nDepends on #18260\n\nCC:  @dcoudert borassi\n\nBranch/Commit: **[d665ec7](https://github.com/sagemath/sagetrac-mirror/commit/d665ec7dc38353a36ef6212db70cf25ccde97bc3)**\n\nReviewer: **David Coudert**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18262_\n\n",
    "closed_at": "2015-04-21T01:13:25Z",
    "created_at": "2015-04-20T19:06:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make centrality_betweenness interruptible",
    "type": "issue",
    "updated_at": "2015-04-21T01:13:25Z",
    "url": "https://github.com/sagemath/sage/issues/18262",
    "user": "https://github.com/nathanncohen"
}
```
Make centrality_betweenness interruptible. This is achieved with a try/finally.

Depends on #18260

CC:  @dcoudert borassi

Branch/Commit: **[d665ec7](https://github.com/sagemath/sagetrac-mirror/commit/d665ec7dc38353a36ef6212db70cf25ccde97bc3)**

Reviewer: **David Coudert**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/18262_





---

archive/issue_events_241946.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-20T19:06:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18262#event-241946"
}
```



---

archive/issue_events_241947.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-20T19:06:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18262#event-241947"
}
```



---

archive/issue_events_241948.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-20T19:06:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18262#event-241948"
}
```



---

archive/issue_events_241949.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-20T19:06:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18262#event-241949"
}
```



---

archive/issue_comments_252977.json:
```json
{
    "body": "Commit: **[d665ec7](https://github.com/sagemath/sagetrac-mirror/commit/d665ec7dc38353a36ef6212db70cf25ccde97bc3)**",
    "created_at": "2015-04-20T19:07:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18262#issuecomment-252977",
    "user": "https://github.com/nathanncohen"
}
```

Commit: **[d665ec7](https://github.com/sagemath/sagetrac-mirror/commit/d665ec7dc38353a36ef6212db70cf25ccde97bc3)**



---

archive/issue_comments_252978.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e25a95ecf922156826c5fee75dc73ae62deb279f\">e25a95e</a></td><td><code>trac #18260: Small fix for directed graphs</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b83d82833e0b6fcce0700f24c6c029958c761311\">b83d828</a></td><td><code>trac #18260: Move centrality functions from graph.py to generic_graph.py</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e55042e6eae922a7ea483fc1a62ef1694071b38d\">e55042e</a></td><td><code>trac #18260: A new doctest</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/731445932eb551241b7f4e0ad10a4d533850c1e7\">7314459</a></td><td><code>trac #18260: Broken link in the doc</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4bc0de8139d72fb4a2c19cd3cb0d2f61e1b7bb60\">4bc0de8</a></td><td><code>trac #18260: Reviewer's comments</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/487b3a29bc0b5eb4ec4b6b2ec0d5503dc77df461\">487b3a2</a></td><td><code>trac #18262: regroup mallocs</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/960fdaaa53945a65d11bb86fb34d7e576430b75f\">960fdaa</a></td><td><code>trac #18262: increase identation (and nothing else)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a8c015933b1e3f3bd8b316829f1a9df0c3a95676\">a8c0159</a></td><td><code>trac #18262: Exception check</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d665ec7dc38353a36ef6212db70cf25ccde97bc3\">d665ec7</a></td><td><code>trac #18262: Simplify memory allocation</code></td></tr></table>\n",
    "created_at": "2015-04-20T19:07:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18262#issuecomment-252978",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e25a95ecf922156826c5fee75dc73ae62deb279f">e25a95e</a></td><td><code>trac #18260: Small fix for directed graphs</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b83d82833e0b6fcce0700f24c6c029958c761311">b83d828</a></td><td><code>trac #18260: Move centrality functions from graph.py to generic_graph.py</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e55042e6eae922a7ea483fc1a62ef1694071b38d">e55042e</a></td><td><code>trac #18260: A new doctest</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/731445932eb551241b7f4e0ad10a4d533850c1e7">7314459</a></td><td><code>trac #18260: Broken link in the doc</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4bc0de8139d72fb4a2c19cd3cb0d2f61e1b7bb60">4bc0de8</a></td><td><code>trac #18260: Reviewer's comments</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/487b3a29bc0b5eb4ec4b6b2ec0d5503dc77df461">487b3a2</a></td><td><code>trac #18262: regroup mallocs</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/960fdaaa53945a65d11bb86fb34d7e576430b75f">960fdaa</a></td><td><code>trac #18262: increase identation (and nothing else)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a8c015933b1e3f3bd8b316829f1a9df0c3a95676">a8c0159</a></td><td><code>trac #18262: Exception check</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d665ec7dc38353a36ef6212db70cf25ccde97bc3">d665ec7</a></td><td><code>trac #18262: Simplify memory allocation</code></td></tr></table>




---

archive/issue_events_241950.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-20T19:07:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18262#event-241950"
}
```



---

archive/issue_comments_252979.json:
```json
{
    "body": "Branch: **[public/18262](https://github.com/sagemath/sagetrac-mirror/tree/public/18262)**",
    "created_at": "2015-04-20T19:07:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18262#issuecomment-252979",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[public/18262](https://github.com/sagemath/sagetrac-mirror/tree/public/18262)**



---

archive/issue_comments_252980.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2015-04-20T19:47:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18262#issuecomment-252980",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_comments_252981.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nGood. I like the `check_malloc` and `sig_check` methods. I will learn to use them.\n\nAll test pass, so it's good for me.",
    "created_at": "2015-04-20T19:47:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18262#issuecomment-252981",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:2'>Comment 2:</a>
Good. I like the `check_malloc` and `sig_check` methods. I will learn to use them.

All test pass, so it's good for me.



---

archive/issue_events_241951.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-04-20T19:47:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18262#event-241951"
}
```



---

archive/issue_events_241952.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-04-20T19:47:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18262#event-241952"
}
```



---

archive/issue_events_241953.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-21T01:13:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18262#event-241953"
}
```



---

archive/issue_events_241954.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9ea870ee1a49e88a75099d8f211b77557a9e425a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-04-21T01:13:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18262#event-241954"
}
```



---

archive/issue_comments_252982.json:
```json
{
    "body": "Changed branch from **[public/18262](https://github.com/sagemath/sagetrac-mirror/tree/public/18262)** to **[d665ec7](https://github.com/sagemath/sagetrac-mirror/commit/d665ec7dc38353a36ef6212db70cf25ccde97bc3)**",
    "created_at": "2015-04-21T01:13:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18262",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18262#issuecomment-252982",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/18262](https://github.com/sagemath/sagetrac-mirror/tree/public/18262)** to **[d665ec7](https://github.com/sagemath/sagetrac-mirror/commit/d665ec7dc38353a36ef6212db70cf25ccde97bc3)**
