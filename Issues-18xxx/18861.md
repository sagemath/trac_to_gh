# Issue 18861: Three apparently useless polyhedron methods

archive/issues_018624.json:
```json
{
    "assignees": [],
    "body": "It seems that the three following functions are not used anywhere\n\n```\n(polgraph|\u271a2\u2026)~/sage/geometry$ grep _make_polyh . -R\n./polyhedron/base.py:#    _make_polyhedron_face.\n./polyhedron/base.py:    def _make_polyhedron_face(self, Vindices, Hindices):\n./polyhedron/base.py:            sage: square._make_polyhedron_face((0,2), (1,))\n./polyhedron/base.py:            return self._make_polyhedron_face(Vindices, Hindices)\n(polgraph|\u271a2\u2026)~/sage/geometry$ grep _init_fac . -R  \n./polyhedron/base.py:#    _init_facet_adjacency_matrix, _init_vertex_adjacency_matrix, and\n./polyhedron/backend_cdd.py:    def _init_facet_adjacency_matrix(self, verbose=False):\n./polyhedron/backend_cdd.py:            sage: p._init_facet_adjacency_matrix()\n(polgraph|\u271a2\u2026)~/sage/geometry$ grep _init_vertex . -R\n./polyhedron/base.py:#    _init_facet_adjacency_matrix, _init_vertex_adjacency_matrix, and\n./polyhedron/backend_cdd.py:    def _init_vertex_adjacency_matrix(self, verbose=False):\n./polyhedron/backend_cdd.py:            sage: p._init_vertex_adjacency_matrix()\n```\n\nShould they be removed, or used somewhere?\n\nNathann\n\nCC:  @dimpase @vbraun @videlec @fchapoton\n\nBranch/Commit: **[9f7a12b](https://github.com/sagemath/sagetrac-mirror/commit/9f7a12bb908fdb0ffba1cc110a93e84645d3cbc2)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jean-Philippe Labb\u00e9**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18861_\n\n",
    "closed_at": "2019-10-29T23:42:01Z",
    "created_at": "2015-07-07T18:27:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Three apparently useless polyhedron methods",
    "type": "issue",
    "updated_at": "2019-10-29T23:42:01Z",
    "url": "https://github.com/sagemath/sage/issues/18861",
    "user": "https://github.com/nathanncohen"
}
```
It seems that the three following functions are not used anywhere

```
(polgraph|✚2…)~/sage/geometry$ grep _make_polyh . -R
./polyhedron/base.py:#    _make_polyhedron_face.
./polyhedron/base.py:    def _make_polyhedron_face(self, Vindices, Hindices):
./polyhedron/base.py:            sage: square._make_polyhedron_face((0,2), (1,))
./polyhedron/base.py:            return self._make_polyhedron_face(Vindices, Hindices)
(polgraph|✚2…)~/sage/geometry$ grep _init_fac . -R  
./polyhedron/base.py:#    _init_facet_adjacency_matrix, _init_vertex_adjacency_matrix, and
./polyhedron/backend_cdd.py:    def _init_facet_adjacency_matrix(self, verbose=False):
./polyhedron/backend_cdd.py:            sage: p._init_facet_adjacency_matrix()
(polgraph|✚2…)~/sage/geometry$ grep _init_vertex . -R
./polyhedron/base.py:#    _init_facet_adjacency_matrix, _init_vertex_adjacency_matrix, and
./polyhedron/backend_cdd.py:    def _init_vertex_adjacency_matrix(self, verbose=False):
./polyhedron/backend_cdd.py:            sage: p._init_vertex_adjacency_matrix()
```

Should they be removed, or used somewhere?

Nathann

CC:  @dimpase @vbraun @videlec @fchapoton

Branch/Commit: **[9f7a12b](https://github.com/sagemath/sagetrac-mirror/commit/9f7a12bb908fdb0ffba1cc110a93e84645d3cbc2)**

Reviewer: **Frédéric Chapoton**

Author: **Jean-Philippe Labbé**

_Issue created by migration from https://trac.sagemath.org/ticket/18861_





---

archive/issue_events_250052.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-07-07T18:27:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250052"
}
```



---

archive/issue_events_250053.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-07-07T18:27:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250053"
}
```



---

archive/issue_events_250054.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-07-07T18:27:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250054"
}
```



---

archive/issue_events_250055.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-07-07T18:27:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250055"
}
```



---

archive/issue_comments_266665.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\n`_make_polyhedron_face` is used in `face_lattice`.",
    "created_at": "2016-03-29T05:48:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266665",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

`_make_polyhedron_face` is used in `face_lattice`.



---

archive/issue_events_250056.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-26T11:49:29Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250056"
}
```



---

archive/issue_events_250057.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-26T11:49:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250057"
}
```



---

archive/issue_events_250058.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-26T11:49:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250058"
}
```



---

archive/issue_comments_266666.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nWith sage8.9:\n\n```\nsage/src/sage/geometry$ grep _make_polyh . -R\n./polyhedron/base.py:#    _make_polyhedron_face.\n./polyhedron/base.py:    def _make_polyhedron_face(self, Vindices, Hindices):\n./polyhedron/base.py:            sage: square._make_polyhedron_face((0,2), (1,)).ambient_V_indices()\n./polyhedron/base.py:            return self._make_polyhedron_face(Vindices, Hindices)\nsage/src/sage/geometry$ grep _init_fac . -R\nsage/src/sage/geometry$ grep _init_vertex . -R\n```\n\nThis has been indirectly taken care of. The first function is used in `face_lattice`.\n\nFor this reason, I would set this as a \"won't fix\".",
    "created_at": "2019-10-26T11:49:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266666",
    "user": "https://github.com/jplab"
}
```

<div id="comment:2" align="right">Comment 2</div>

With sage8.9:

```
sage/src/sage/geometry$ grep _make_polyh . -R
./polyhedron/base.py:#    _make_polyhedron_face.
./polyhedron/base.py:    def _make_polyhedron_face(self, Vindices, Hindices):
./polyhedron/base.py:            sage: square._make_polyhedron_face((0,2), (1,)).ambient_V_indices()
./polyhedron/base.py:            return self._make_polyhedron_face(Vindices, Hindices)
sage/src/sage/geometry$ grep _init_fac . -R
sage/src/sage/geometry$ grep _init_vertex . -R
```

This has been indirectly taken care of. The first function is used in `face_lattice`.

For this reason, I would set this as a "won't fix".



---

archive/issue_events_250059.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-26T11:49:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250059"
}
```



---

archive/issue_comments_266667.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nMaybe `_make_polyhedron_face` could just be inserted where it is called, as it is very short and called in just one place ?",
    "created_at": "2019-10-26T20:05:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266667",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

Maybe `_make_polyhedron_face` could just be inserted where it is called, as it is very short and called in just one place ?



---

archive/issue_comments_266668.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nReplying to [@fchapoton](#comment%3A4):\n> Maybe `_make_polyhedron_face` could just be inserted where it is called, as it is very short and called in just one place ?\n\nGood idea. I'll do that.",
    "created_at": "2019-10-27T07:37:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266668",
    "user": "https://github.com/jplab"
}
```

<div id="comment:5" align="right">Comment 5</div>

Replying to [@fchapoton](#comment%3A4):
> Maybe `_make_polyhedron_face` could just be inserted where it is called, as it is very short and called in just one place ?

Good idea. I'll do that.



---

archive/issue_events_250060.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-27T15:29:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250060"
}
```



---

archive/issue_comments_266669.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nIt turns out that it can simply be deleted and merged into the already present function.\n\n... when `face_lattice` will be renovated through CombinatorialPolyhedron the face lattice function will change again, but it will make the transition a bit easier. So I would say it is a good first step.\n  \n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a7792fa237026eaf9c4369999d294f72b923dc14\">a7792fa</a></td><td><code>removed _make_polyhedron_face</code></td></tr></table>\n",
    "created_at": "2019-10-27T15:29:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266669",
    "user": "https://github.com/jplab"
}
```

<div id="comment:6" align="right">Comment 6</div>

It turns out that it can simply be deleted and merged into the already present function.

... when `face_lattice` will be renovated through CombinatorialPolyhedron the face lattice function will change again, but it will make the transition a bit easier. So I would say it is a good first step.
  
---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a7792fa237026eaf9c4369999d294f72b923dc14">a7792fa</a></td><td><code>removed _make_polyhedron_face</code></td></tr></table>




---

archive/issue_events_250061.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-27T15:29:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250061"
}
```



---

archive/issue_events_250062.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-27T15:29:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250062"
}
```



---

archive/issue_comments_266670.json:
```json
{
    "body": "Commit: **[a7792fa](https://github.com/sagemath/sagetrac-mirror/commit/a7792fa237026eaf9c4369999d294f72b923dc14)**",
    "created_at": "2019-10-27T15:29:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266670",
    "user": "https://github.com/jplab"
}
```

Commit: **[a7792fa](https://github.com/sagemath/sagetrac-mirror/commit/a7792fa237026eaf9c4369999d294f72b923dc14)**



---

archive/issue_comments_266671.json:
```json
{
    "body": "Author: **Jean-Philippe Labb\u00e9**",
    "created_at": "2019-10-27T15:29:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266671",
    "user": "https://github.com/jplab"
}
```

Author: **Jean-Philippe Labbé**



---

archive/issue_comments_266672.json:
```json
{
    "body": "Branch: **[u/jipilab/18861](https://github.com/sagemath/sagetrac-mirror/tree/u/jipilab/18861)**",
    "created_at": "2019-10-27T15:29:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266672",
    "user": "https://github.com/jplab"
}
```

Branch: **[u/jipilab/18861](https://github.com/sagemath/sagetrac-mirror/tree/u/jipilab/18861)**



---

archive/issue_comments_266673.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\n* you forgot the first argument `self` in the function call.\n\n* I would put the import outside of the auxiliary function\n\n* One could replace \n`if len(atoms) == 0:`\nby\n`if not atoms`",
    "created_at": "2019-10-27T16:28:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266673",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:7" align="right">Comment 7</div>

* you forgot the first argument `self` in the function call.

* I would put the import outside of the auxiliary function

* One could replace 
`if len(atoms) == 0:`
by
`if not atoms`



---

archive/issue_comments_266674.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nMerde. I thought about it when I cut the function, and after I pasted, I forgot to change it's format.\n\nUpcoming...",
    "created_at": "2019-10-27T16:44:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266674",
    "user": "https://github.com/jplab"
}
```

<div id="comment:8" align="right">Comment 8</div>

Merde. I thought about it when I cut the function, and after I pasted, I forgot to change it's format.

Upcoming...



---

archive/issue_comments_266675.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dc629e0cf7d7bfc14b850f259dca0ec7c774b735\">dc629e0</a></td><td><code>removed _make_polyhedron_face</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/542d772958bb6e6dd2a4c2b4656672ac76444ca2\">542d772</a></td><td><code>changed if in face_lattice</code></td></tr></table>\n",
    "created_at": "2019-10-27T16:50:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266675",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dc629e0cf7d7bfc14b850f259dca0ec7c774b735">dc629e0</a></td><td><code>removed _make_polyhedron_face</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/542d772958bb6e6dd2a4c2b4656672ac76444ca2">542d772</a></td><td><code>changed if in face_lattice</code></td></tr></table>




---

archive/issue_comments_266676.json:
```json
{
    "body": "Changed commit from **[a7792fa](https://github.com/sagemath/sagetrac-mirror/commit/a7792fa237026eaf9c4369999d294f72b923dc14)** to **[542d772](https://github.com/sagemath/sagetrac-mirror/commit/542d772958bb6e6dd2a4c2b4656672ac76444ca2)**",
    "created_at": "2019-10-27T16:50:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266676",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a7792fa](https://github.com/sagemath/sagetrac-mirror/commit/a7792fa237026eaf9c4369999d294f72b923dc14)** to **[542d772](https://github.com/sagemath/sagetrac-mirror/commit/542d772958bb6e6dd2a4c2b4656672ac76444ca2)**



---

archive/issue_comments_266677.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nI looked at it and I think that the first two items are not correct:\n\nwhat happened is\n\n- the method `_make_polyhedron_face` is removed.\n- the only place where it was used was related to an internally defined function `face_constructor` inside of the `face_lattice` method.\n\nSo, I believe that the import should be inside the internal function, right?\n\nThus, there is no need of the self argument.\n\nI changed the if statement. Let's see what the bot says, just to make sure that the calls are correct. If I looked correctly, it should essentially be the same, but only function call less...",
    "created_at": "2019-10-27T16:54:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266677",
    "user": "https://github.com/jplab"
}
```

<div id="comment:10" align="right">Comment 10</div>

I looked at it and I think that the first two items are not correct:

what happened is

- the method `_make_polyhedron_face` is removed.
- the only place where it was used was related to an internally defined function `face_constructor` inside of the `face_lattice` method.

So, I believe that the import should be inside the internal function, right?

Thus, there is no need of the self argument.

I changed the if statement. Let's see what the bot says, just to make sure that the calls are correct. If I looked correctly, it should essentially be the same, but only function call less...



---

archive/issue_comments_266678.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nStill very very broken...",
    "created_at": "2019-10-27T20:14:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266678",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:11" align="right">Comment 11</div>

Still very very broken...



---

archive/issue_comments_266679.json:
```json
{
    "body": "Changed commit from **[542d772](https://github.com/sagemath/sagetrac-mirror/commit/542d772958bb6e6dd2a4c2b4656672ac76444ca2)** to **[9f7a12b](https://github.com/sagemath/sagetrac-mirror/commit/9f7a12bb908fdb0ffba1cc110a93e84645d3cbc2)**",
    "created_at": "2019-10-28T09:16:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266679",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[542d772](https://github.com/sagemath/sagetrac-mirror/commit/542d772958bb6e6dd2a4c2b4656672ac76444ca2)** to **[9f7a12b](https://github.com/sagemath/sagetrac-mirror/commit/9f7a12bb908fdb0ffba1cc110a93e84645d3cbc2)**



---

archive/issue_comments_266680.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9f7a12bb908fdb0ffba1cc110a93e84645d3cbc2\">9f7a12b</a></td><td><code>added missing argument</code></td></tr></table>\n",
    "created_at": "2019-10-28T09:16:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266680",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9f7a12bb908fdb0ffba1cc110a93e84645d3cbc2">9f7a12b</a></td><td><code>added missing argument</code></td></tr></table>




---

archive/issue_comments_266681.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nWow, yes... I missed that `self` argument... Should be better now...",
    "created_at": "2019-10-28T09:17:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266681",
    "user": "https://github.com/jplab"
}
```

<div id="comment:13" align="right">Comment 13</div>

Wow, yes... I missed that `self` argument... Should be better now...



---

archive/issue_events_250063.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-10-28T13:41:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250063"
}
```



---

archive/issue_events_250064.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-10-28T13:41:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250064"
}
```



---

archive/issue_comments_266682.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-10-28T13:41:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266682",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_266683.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nMerci, feu vert.",
    "created_at": "2019-10-28T13:41:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266683",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:14" align="right">Comment 14</div>

Merci, feu vert.



---

archive/issue_comments_266684.json:
```json
{
    "body": "Changed branch from **[u/jipilab/18861](https://github.com/sagemath/sagetrac-mirror/tree/u/jipilab/18861)** to **[9f7a12b](https://github.com/sagemath/sagetrac-mirror/commit/9f7a12bb908fdb0ffba1cc110a93e84645d3cbc2)**",
    "created_at": "2019-10-29T23:42:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18861#issuecomment-266684",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jipilab/18861](https://github.com/sagemath/sagetrac-mirror/tree/u/jipilab/18861)** to **[9f7a12b](https://github.com/sagemath/sagetrac-mirror/commit/9f7a12bb908fdb0ffba1cc110a93e84645d3cbc2)**



---

archive/issue_events_250065.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-29T23:42:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250065"
}
```



---

archive/issue_events_250066.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ae14d8e5ae9dfeef87482a7d4e085285dc51d4b2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-10-29T23:42:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18861",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18861#event-250066"
}
```
