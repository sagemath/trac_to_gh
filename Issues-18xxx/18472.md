# Issue 18472: Python 3: The semantic of map() function is changed (part1)

archive/issues_018235.json:
```json
{
    "assignees": [],
    "body": "This ticket is part of #16073 to simplify review.\n\nIn Py2 `map()` returns a list, while in Py3 `map()` returns an iterator.\n\nCC:  @jdemeyer\n\nKeywords: **python3**\n\nBranch/Commit: **[80a75ca](https://github.com/sagemath/sagetrac-mirror/commit/80a75ca98c78757d0e19d3d3a2f0619ed4e6ab0f)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Andr\u00e9 Apitzsch**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18472_\n\n",
    "closed_at": "2015-05-25T21:09:29Z",
    "created_at": "2015-05-22T16:27:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Python 3: The semantic of map() function is changed (part1)",
    "type": "issue",
    "updated_at": "2015-05-25T21:09:29Z",
    "url": "https://github.com/sagemath/sage/issues/18472",
    "user": "https://github.com/a-andre"
}
```
This ticket is part of #16073 to simplify review.

In Py2 `map()` returns a list, while in Py3 `map()` returns an iterator.

CC:  @jdemeyer

Keywords: **python3**

Branch/Commit: **[80a75ca](https://github.com/sagemath/sagetrac-mirror/commit/80a75ca98c78757d0e19d3d3a2f0619ed4e6ab0f)**

Reviewer: **Frédéric Chapoton**

Author: **André Apitzsch**

_Issue created by migration from https://trac.sagemath.org/ticket/18472_





---

archive/issue_events_235929.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-05-22T16:27:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18472#event-235929"
}
```



---

archive/issue_events_235930.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-05-22T16:27:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18472#event-235930"
}
```



---

archive/issue_events_235931.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-05-22T16:27:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18472#event-235931"
}
```



---

archive/issue_comments_257490.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n This ticket is part of #16073 to simplify review.\n \n-In Py2 `map()` returns a list, while in Py3 `map()` returns an iterator (as `itertools.imap()` does in Py2).\n+In Py2 `map()` returns a list, while in Py3 `map()` returns an iterator.\n``````\n",
    "created_at": "2015-05-22T16:29:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18472#issuecomment-257490",
    "user": "https://github.com/a-andre"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 This ticket is part of #16073 to simplify review.
 
-In Py2 `map()` returns a list, while in Py3 `map()` returns an iterator (as `itertools.imap()` does in Py2).
+In Py2 `map()` returns a list, while in Py3 `map()` returns an iterator.
``````




---

archive/issue_events_235932.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-05-22T16:29:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18472#event-235932"
}
```



---

archive/issue_comments_257491.json:
```json
{
    "body": "Changed commit from **[5749bf4](https://github.com/sagemath/sagetrac-mirror/commit/5749bf4408fe27b8d3c55c24bd943c9862ae0778)** to **[80a75ca](https://github.com/sagemath/sagetrac-mirror/commit/80a75ca98c78757d0e19d3d3a2f0619ed4e6ab0f)**",
    "created_at": "2015-05-24T20:03:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18472#issuecomment-257491",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[5749bf4](https://github.com/sagemath/sagetrac-mirror/commit/5749bf4408fe27b8d3c55c24bd943c9862ae0778)** to **[80a75ca](https://github.com/sagemath/sagetrac-mirror/commit/80a75ca98c78757d0e19d3d3a2f0619ed4e6ab0f)**



---

archive/issue_comments_257492.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nlooks good to me\n\nyou can set to positive review if you agree with my short commit\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7c9280a331a5a275c9921977e9d6c58da6f70112\">7c9280a</a></td><td><code>Merge branch 'u/aapitzsch/map_p1' into 6.8.b0</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/80a75ca98c78757d0e19d3d3a2f0619ed4e6ab0f\">80a75ca</a></td><td><code>trac #18472 one fix</code></td></tr></table>\n",
    "created_at": "2015-05-24T20:03:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18472#issuecomment-257492",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
looks good to me

you can set to positive review if you agree with my short commit

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7c9280a331a5a275c9921977e9d6c58da6f70112">7c9280a</a></td><td><code>Merge branch 'u/aapitzsch/map_p1' into 6.8.b0</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/80a75ca98c78757d0e19d3d3a2f0619ed4e6ab0f">80a75ca</a></td><td><code>trac #18472 one fix</code></td></tr></table>




---

archive/issue_comments_257493.json:
```json
{
    "body": "Changed branch from **[u/aapitzsch/map_p1](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/map_p1)** to **[public/ticket/18472](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/18472)**",
    "created_at": "2015-05-24T20:03:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18472#issuecomment-257493",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/aapitzsch/map_p1](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/map_p1)** to **[public/ticket/18472](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/18472)**



---

archive/issue_events_235933.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-05-24T22:28:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18472#event-235933"
}
```



---

archive/issue_events_235934.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-05-24T22:28:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18472#event-235934"
}
```



---

archive/issue_comments_257494.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2015-05-24T22:28:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18472#issuecomment-257494",
    "user": "https://github.com/a-andre"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_257495.json:
```json
{
    "body": "Changed branch from **[public/ticket/18472](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/18472)** to **[80a75ca](https://github.com/sagemath/sagetrac-mirror/commit/80a75ca98c78757d0e19d3d3a2f0619ed4e6ab0f)**",
    "created_at": "2015-05-25T21:09:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18472#issuecomment-257495",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/ticket/18472](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/18472)** to **[80a75ca](https://github.com/sagemath/sagetrac-mirror/commit/80a75ca98c78757d0e19d3d3a2f0619ed4e6ab0f)**



---

archive/issue_events_235935.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-25T21:09:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18472#event-235935"
}
```



---

archive/issue_events_235936.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "94368fbe12eaa9da5e7c389fef94c64c38659fb1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-05-25T21:09:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18472",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18472#event-235936"
}
```
