# Issue 18105: No conversion to CIF for some symbolic expression

archive/issues_017868.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: CIF(cos(2/3))\nTraceback (most recent call last):\n...\nTypeError: unable to simplify to a complex interval approximation\n```\nThe answer should just be\n\n```\nsage: CIF(RIF(cos(2/3)))\n0.7858872607769480?\n```\n\nCC:  @rwst\n\nKeywords: **sd66**\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18105_\n\n",
    "closed_at": "2015-04-02T22:27:36Z",
    "created_at": "2015-04-02T08:57:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "No conversion to CIF for some symbolic expression",
    "type": "issue",
    "updated_at": "2015-04-02T22:27:36Z",
    "url": "https://github.com/sagemath/sage/issues/18105",
    "user": "https://github.com/videlec"
}
```

```
sage: CIF(cos(2/3))
Traceback (most recent call last):
...
TypeError: unable to simplify to a complex interval approximation
```
The answer should just be

```
sage: CIF(RIF(cos(2/3)))
0.7858872607769480?
```

CC:  @rwst

Keywords: **sd66**

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/18105_





---

archive/issue_events_239788.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-02T08:57:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239788"
}
```



---

archive/issue_events_239789.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-02T08:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239789"
}
```



---

archive/issue_events_239790.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-02T08:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239790"
}
```



---

archive/issue_events_239791.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-02T08:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239791"
}
```



---

archive/issue_comments_249466.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,3 +5,9 @@\n ...\n TypeError: unable to simplify to a complex interval approximation\n ```\n+The answer should just be\n+\n+```\n+sage: CIF(RIF(cos(2/3)))\n+0.7858872607769480?\n+```\n``````\n",
    "created_at": "2015-04-02T08:58:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18105#issuecomment-249466",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,3 +5,9 @@
 ...
 TypeError: unable to simplify to a complex interval approximation
 ```
+The answer should just be
+
+```
+sage: CIF(RIF(cos(2/3)))
+0.7858872607769480?
+```
``````




---

archive/issue_events_239792.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-02T08:58:22Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "title_is": "No conversion to CIF for some symbolic expression",
    "title_was": "No coercion to CIF for some symbolic expression",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239792"
}
```



---

archive/issue_events_239793.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T09:10:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239793"
}
```



---

archive/issue_events_239794.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T09:10:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239794"
}
```



---

archive/issue_events_239795.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T09:10:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239795"
}
```



---

archive/issue_events_239796.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T09:10:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239796"
}
```



---

archive/issue_comments_249467.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nDuplicate of #17285 I guess.",
    "created_at": "2015-04-02T09:10:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18105#issuecomment-249467",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

Duplicate of #17285 I guess.



---

archive/issue_comments_249468.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-04-02T09:10:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18105#issuecomment-249468",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_239797.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T09:11:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239797"
}
```



---

archive/issue_events_239798.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T09:11:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239798"
}
```



---

archive/issue_events_239799.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-02T22:27:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239799"
}
```



---

archive/issue_events_239800.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-02T22:27:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239800"
}
```



---

archive/issue_events_239801.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-02T22:27:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18105",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18105#event-239801"
}
```
