...
```json
{
    "body": "If somebody actually knows this file, it would be nice to think of what should be done with `random_dag`, `random_digraph` and `random_tree`, which are all exported into the global namespace (and we have `graphs.<tab>` for these things already).",
...
```json
{
    "body": "But please post your conclusion on the `random_*` methods on a separate ticket.",
...
```json
{
    "body": "Of course of course, this belongs to a separate ticket. I do not believe in opening tickets which I do not address myself, so I hoped that the guy who will know that file and work on this ticket can do something about that too.\n\nBut in another ticket, of course.\n\nNathann",
...
```json
{
    "body": "The problem that you met here is not related to copy. The problem is that `SandPile` inherits from `Graph`, but that its `__init__` function does not accept all arguments that `Graph.__init__` accepts.\n\nNathann",
...
```json
{
    "body": "Well, add a `**kwds` to the constructor, forward everything to `super().__init__` and you are done.\n\nNathann",
...
```json
{
    "body": "Nuuh, `GenericGraph.copy` will still not pass positional arguments to `Sandpile.__init__`.",
...
```json
{
    "body": "In other words I also have to override `copy` in addition to `__copy__`. How very convenient. This is why Peter originally factored out the `self.__class__` call inside `__copy__`, to provide a hook to override this call. It is all a logical consequence of having `__copy__` do non-copy stuff...",
...
```json
{
    "body": "Thats not an improvement since `plot()` still calls `G.copy()` (which shouldn't have been called copy) instead of `G.__copy__()`  to make a copy.",
...
```json
{
    "body": "> But `__copy__` is not supposed to take arguments precisely because you would then fall into this trap.\n\n\nI fail to see why this problem could happen with `__copy__` and not with other functions.\n\nNathann",
...
```json
{
    "body": "Maybe its clearer now, just fixing `GenericGraph.__copy__` also fixes the Sandpile plotting.",
    "created_at": "2015-04-09T12:04:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18032",
    "type": "issue_comment",
...
```json
{
    "body": "You are making the default behaviour of `copy` to *change* the backend?\n\nNathann",
...
```json
{
    "body": "Only if necessary (i.e. only if the current backend doesn't support mutable graphs). The expected behavior of copy is that the copy is mutable.",
...
```json
{
    "body": "> The expected behavior of copy is that the copy is mutable.\n\n\nNo.\n\n```\nsage: copy((1,2,3))\n(1, 2, 3)\n```\n\nNathann",
...
```json
{
    "body": "Its a Sage convention, not a Python convention. Python doesn't have a concept of (imm)mutability for a single class, just completely different classes. Of course copy has to return an instance of the same class.",
...
```json
{
    "body": "Its the same interface that vectors and matrices have. If you disagree with these too then fine. But my change is a big improvement to the graph code.",
