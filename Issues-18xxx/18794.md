# Issue 18794: StandardError is not Python 3 compatible

archive/issues_018557.json:
```json
{
    "assignees": [],
    "body": "The simple solution is to replace all usage of `StandardError` with `Exception`.\n\nThis is a follow-up of #15620.\n\nKeywords: **python3**\n\nBranch/Commit: **[d4b0cdb](https://github.com/sagemath/sagetrac-mirror/commit/d4b0cdb1602458cbf707cfee62d5a0b3f47cf310)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Andr\u00e9 Apitzsch**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18794_\n\n",
    "closed_at": "2015-06-26T20:28:15Z",
    "created_at": "2015-06-26T14:34:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "StandardError is not Python 3 compatible",
    "type": "issue",
    "updated_at": "2015-06-26T20:28:15Z",
    "url": "https://github.com/sagemath/sage/issues/18794",
    "user": "https://github.com/a-andre"
}
```
The simple solution is to replace all usage of `StandardError` with `Exception`.

This is a follow-up of #15620.

Keywords: **python3**

Branch/Commit: **[d4b0cdb](https://github.com/sagemath/sagetrac-mirror/commit/d4b0cdb1602458cbf707cfee62d5a0b3f47cf310)**

Reviewer: **Travis Scrimshaw**

Author: **André Apitzsch**

_Issue created by migration from https://trac.sagemath.org/ticket/18794_





---

archive/issue_events_249150.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-06-26T14:34:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18794#event-249150"
}
```



---

archive/issue_events_249151.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-06-26T14:34:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18794#event-249151"
}
```



---

archive/issue_events_249152.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-06-26T14:34:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18794#event-249152"
}
```



---

archive/issue_comments_265126.json:
```json
{
    "body": "Commit: **[d4b0cdb](https://github.com/sagemath/sagetrac-mirror/commit/d4b0cdb1602458cbf707cfee62d5a0b3f47cf310)**",
    "created_at": "2015-06-26T14:37:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18794#issuecomment-265126",
    "user": "https://github.com/a-andre"
}
```

Commit: **[d4b0cdb](https://github.com/sagemath/sagetrac-mirror/commit/d4b0cdb1602458cbf707cfee62d5a0b3f47cf310)**



---

archive/issue_events_249153.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-06-26T14:37:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18794#event-249153"
}
```



---

archive/issue_comments_265127.json:
```json
{
    "body": "Branch: **[u/aapitzsch/ticket/18794](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/18794)**",
    "created_at": "2015-06-26T14:37:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18794#issuecomment-265127",
    "user": "https://github.com/a-andre"
}
```

Branch: **[u/aapitzsch/ticket/18794](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/18794)**



---

archive/issue_comments_265128.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2015-06-26T14:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18794#issuecomment-265128",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_265129.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2015-06-26T14:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18794#issuecomment-265129",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_events_249154.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-06-26T14:51:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18794#event-249154"
}
```



---

archive/issue_events_249155.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-06-26T14:51:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18794#event-249155"
}
```



---

archive/issue_comments_265130.json:
```json
{
    "body": "Changed keywords from none to **python3**",
    "created_at": "2015-06-26T16:11:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18794#issuecomment-265130",
    "user": "https://github.com/wluebbe"
}
```

Changed keywords from none to **python3**



---

archive/issue_comments_265131.json:
```json
{
    "body": "Changed branch from **[u/aapitzsch/ticket/18794](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/18794)** to **[d4b0cdb](https://github.com/sagemath/sagetrac-mirror/commit/d4b0cdb1602458cbf707cfee62d5a0b3f47cf310)**",
    "created_at": "2015-06-26T20:28:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18794#issuecomment-265131",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/aapitzsch/ticket/18794](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/18794)** to **[d4b0cdb](https://github.com/sagemath/sagetrac-mirror/commit/d4b0cdb1602458cbf707cfee62d5a0b3f47cf310)**



---

archive/issue_events_249156.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-06-26T20:28:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18794#event-249156"
}
```



---

archive/issue_events_249157.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a265460a6d7fd4c943b818ca859ac24e871dd232",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-06-26T20:28:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18794",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18794#event-249157"
}
```
