# Issue 18312: Construction of a sparse matrix from sparse vectors does not exploit sparseness

archive/issues_018075.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: n = 10^3\nsage: F = FreeModule(QQ, n, sparse=True)\nsage: v = F.an_element()\nsage: vectors = [v]*n\n\nsage: %time a = matrix(vectors, sparse=True)\nCPU times: user 2.01 s, sys: 38.4 ms, total: 2.05 s\nWall time: 1.96 s\n```\n\nCompare with the construction of the same matrix through an\nintermediate dictionary:\n\n```\nsage: %time b = matrix(QQ, n, {(i,j): c for i,v in enumerate(vectors) for j,c in v.iteritems()}, sparse=True)\nCPU times: user 7.33 ms, sys: 124 \u00b5s, total: 7.46 ms\nWall time: 6.06 ms\nsage: a == b\nTrue\n```\n\nRunning `%prun` shows that the input vectors are converted to dense\nlists and back (gasp!), which gives a complexity of `n^2` instead of\nlinear in the number of nonzero entries as would be expected.\n\nSee also: #10312\n\nCC:  @jdemeyer @videlec\n\nKeywords: **sparse matrix constructor**\n\nBranch/Commit: **[2044e52](https://github.com/sagemath/sagetrac-mirror/commit/2044e5259d0b897c0b65d430c3c06f4a5444561a)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18312_\n\n",
    "closed_at": "2019-08-18T18:46:50Z",
    "created_at": "2015-04-27T16:03:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Construction of a sparse matrix from sparse vectors does not exploit sparseness",
    "type": "issue",
    "updated_at": "2019-08-18T18:46:50Z",
    "url": "https://github.com/sagemath/sage/issues/18312",
    "user": "https://github.com/nthiery"
}
```

```
sage: n = 10^3
sage: F = FreeModule(QQ, n, sparse=True)
sage: v = F.an_element()
sage: vectors = [v]*n

sage: %time a = matrix(vectors, sparse=True)
CPU times: user 2.01 s, sys: 38.4 ms, total: 2.05 s
Wall time: 1.96 s
```

Compare with the construction of the same matrix through an
intermediate dictionary:

```
sage: %time b = matrix(QQ, n, {(i,j): c for i,v in enumerate(vectors) for j,c in v.iteritems()}, sparse=True)
CPU times: user 7.33 ms, sys: 124 µs, total: 7.46 ms
Wall time: 6.06 ms
sage: a == b
True
```

Running `%prun` shows that the input vectors are converted to dense
lists and back (gasp!), which gives a complexity of `n^2` instead of
linear in the number of nonzero entries as would be expected.

See also: #10312

CC:  @jdemeyer @videlec

Keywords: **sparse matrix constructor**

Branch/Commit: **[2044e52](https://github.com/sagemath/sagetrac-mirror/commit/2044e5259d0b897c0b65d430c3c06f4a5444561a)**

Reviewer: **Vincent Delecroix**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/18312_





---

archive/issue_events_242614.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-04-27T16:03:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242614"
}
```



---

archive/issue_events_242615.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-04-27T16:03:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242615"
}
```



---

archive/issue_events_242616.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-04-27T16:03:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242616"
}
```



---

archive/issue_events_242617.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-04-27T16:03:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242617"
}
```



---

archive/issue_comments_254123.json:
```json
{
    "body": "Changed keywords from **** to **sparse matrix constructor**",
    "created_at": "2015-04-27T16:05:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254123",
    "user": "https://github.com/nthiery"
}
```

Changed keywords from **** to **sparse matrix constructor**



---

archive/issue_comments_254124.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -22,6 +22,7 @@\n ```\n \n Running `%prun` shows that the input vectors are converted to dense\n-lists and back (gasp), which gives a complexity of `n^2` instead of\n+lists and back (gasp!), which gives a complexity of `n^2` instead of\n linear in the number of nonzero entries as would be expected.\n \n+See also: #10312\n``````\n",
    "created_at": "2015-04-27T16:05:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254124",
    "user": "https://github.com/nthiery"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -22,6 +22,7 @@
 ```
 
 Running `%prun` shows that the input vectors are converted to dense
-lists and back (gasp), which gives a complexity of `n^2` instead of
+lists and back (gasp!), which gives a complexity of `n^2` instead of
 linear in the number of nonzero entries as would be expected.
 
+See also: #10312
``````




---

archive/issue_comments_254125.json:
```json
{
    "body": "Commit: **[5a1fe38](https://github.com/sagemath/sagetrac-mirror/commit/5a1fe385e9843ac1a4fde84075cb78d172d3e29e)**",
    "created_at": "2017-10-26T07:29:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254125",
    "user": "https://github.com/tscrim"
}
```

Commit: **[5a1fe38](https://github.com/sagemath/sagetrac-mirror/commit/5a1fe385e9843ac1a4fde84075cb78d172d3e29e)**



---

archive/issue_comments_254126.json:
```json
{
    "body": "Branch: **[public/linear_algebra/sparse_matrix_from_sparse_vectors-18312](https://github.com/sagemath/sagetrac-mirror/tree/public/linear_algebra/sparse_matrix_from_sparse_vectors-18312)**",
    "created_at": "2017-10-26T07:29:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254126",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/linear_algebra/sparse_matrix_from_sparse_vectors-18312](https://github.com/sagemath/sagetrac-mirror/tree/public/linear_algebra/sparse_matrix_from_sparse_vectors-18312)**



---

archive/issue_events_242618.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-10-26T07:29:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242618"
}
```



---

archive/issue_comments_254127.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2017-10-26T07:29:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254127",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_comments_254128.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nWith my branch, I get:\n\n```\nsage: n = 10^3\nsage: F = FreeModule(QQ, n, sparse=True)\nsage: v = F.an_element()\nsage: vecs = [v]*n\nsage: %time a = matrix(vecs, sparse=True)\nCPU times: user 8 ms, sys: 0 ns, total: 8 ms\nWall time: 4.76 ms\nsage: MS = MatrixSpace(QQ, n, sparse=True)\nsage: %time b = MS(vecs)\nCPU times: user 4 ms, sys: 0 ns, total: 4 ms\nWall time: 1.31 ms\nsage: %time c = matrix(QQ, n, {(i,j): c for i,v in enumerate(vecs) for j,c in v.iteritems()}, sparse=True)\nCPU times: user 4 ms, sys: 0 ns, total: 4 ms\nWall time: 2.93 ms\n```\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f1acf034be0131f892a3182c635da3c3a57591c1\">f1acf03</a></td><td><code>Faster construction of sparse matrix from list of sparse vectors.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/774b5100963d314f21c13b4f379a05a117bf074d\">774b510</a></td><td><code>Faster construction of sparse matrix from list of sparse vectors for matrix().</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5a1fe385e9843ac1a4fde84075cb78d172d3e29e\">5a1fe38</a></td><td><code>Fixing a few cases.</code></td></tr></table>\n",
    "created_at": "2017-10-26T07:29:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254128",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

With my branch, I get:

```
sage: n = 10^3
sage: F = FreeModule(QQ, n, sparse=True)
sage: v = F.an_element()
sage: vecs = [v]*n
sage: %time a = matrix(vecs, sparse=True)
CPU times: user 8 ms, sys: 0 ns, total: 8 ms
Wall time: 4.76 ms
sage: MS = MatrixSpace(QQ, n, sparse=True)
sage: %time b = MS(vecs)
CPU times: user 4 ms, sys: 0 ns, total: 4 ms
Wall time: 1.31 ms
sage: %time c = matrix(QQ, n, {(i,j): c for i,v in enumerate(vecs) for j,c in v.iteritems()}, sparse=True)
CPU times: user 4 ms, sys: 0 ns, total: 4 ms
Wall time: 2.93 ms
```

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f1acf034be0131f892a3182c635da3c3a57591c1">f1acf03</a></td><td><code>Faster construction of sparse matrix from list of sparse vectors.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/774b5100963d314f21c13b4f379a05a117bf074d">774b510</a></td><td><code>Faster construction of sparse matrix from list of sparse vectors for matrix().</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5a1fe385e9843ac1a4fde84075cb78d172d3e29e">5a1fe38</a></td><td><code>Fixing a few cases.</code></td></tr></table>




---

archive/issue_events_242619.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-10-26T07:29:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242619"
}
```



---

archive/issue_events_242620.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-10-26T07:29:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242620"
}
```



---

archive/issue_comments_254129.json:
```json
{
    "body": "Changed commit from **[5a1fe38](https://github.com/sagemath/sagetrac-mirror/commit/5a1fe385e9843ac1a4fde84075cb78d172d3e29e)** to **[dff2da1](https://github.com/sagemath/sagetrac-mirror/commit/dff2da179d6042e6633bfa5d0dc09269b45158db)**",
    "created_at": "2018-02-04T23:50:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254129",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[5a1fe38](https://github.com/sagemath/sagetrac-mirror/commit/5a1fe385e9843ac1a4fde84075cb78d172d3e29e)** to **[dff2da1](https://github.com/sagemath/sagetrac-mirror/commit/dff2da179d6042e6633bfa5d0dc09269b45158db)**



---

archive/issue_comments_254130.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dff2da179d6042e6633bfa5d0dc09269b45158db\">dff2da1</a></td><td><code>Merge branch 'public/linear_algebra/sparse_matrix_from_sparse_vectors-18312' of git://trac.sagemath.org/sage into public/linear_algebra/sparse_matrix_from_sparse_vectors-18312</code></td></tr></table>\n",
    "created_at": "2018-02-04T23:50:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254130",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dff2da179d6042e6633bfa5d0dc09269b45158db">dff2da1</a></td><td><code>Merge branch 'public/linear_algebra/sparse_matrix_from_sparse_vectors-18312' of git://trac.sagemath.org/sage into public/linear_algebra/sparse_matrix_from_sparse_vectors-18312</code></td></tr></table>




---

archive/issue_events_242621.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-02-04T23:50:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242621"
}
```



---

archive/issue_events_242622.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-02-04T23:50:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242622"
}
```



---

archive/issue_events_242623.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-07-03T20:35:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242623"
}
```



---

archive/issue_events_242624.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-07-03T20:35:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242624"
}
```



---

archive/issue_events_242625.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-07-03T20:35:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242625"
}
```



---

archive/issue_events_242626.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-07-03T20:35:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242626"
}
```



---

archive/issue_comments_254131.json:
```json
{
    "body": "Work Issues: **merge in develop**",
    "created_at": "2018-07-03T20:35:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254131",
    "user": "https://github.com/simonbrandhorst"
}
```

Work Issues: **merge in develop**



---

archive/issue_events_242627.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242627"
}
```



---

archive/issue_events_242628.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242628"
}
```



---

archive/issue_comments_254132.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nupdate milestone 8.3 -> 8.4",
    "created_at": "2018-08-03T19:20:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254132",
    "user": "https://github.com/videlec"
}
```

<div id="comment:6" align="right">Comment 6</div>

update milestone 8.3 -> 8.4



---

archive/issue_comments_254133.json:
```json
{
    "body": "Changed commit from **[dff2da1](https://github.com/sagemath/sagetrac-mirror/commit/dff2da179d6042e6633bfa5d0dc09269b45158db)** to **[c426c0c](https://github.com/sagemath/sagetrac-mirror/commit/c426c0c734bdb614c18b1a047dbe36e1d24bf86e)**",
    "created_at": "2019-08-03T10:02:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254133",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[dff2da1](https://github.com/sagemath/sagetrac-mirror/commit/dff2da179d6042e6633bfa5d0dc09269b45158db)** to **[c426c0c](https://github.com/sagemath/sagetrac-mirror/commit/c426c0c734bdb614c18b1a047dbe36e1d24bf86e)**



---

archive/issue_comments_254134.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c426c0c734bdb614c18b1a047dbe36e1d24bf86e\">c426c0c</a></td><td><code>Extended MatrixArgs to special case list of vectors when sparse.</code></td></tr></table>\n",
    "created_at": "2019-08-03T10:02:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254134",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c426c0c734bdb614c18b1a047dbe36e1d24bf86e">c426c0c</a></td><td><code>Extended MatrixArgs to special case list of vectors when sparse.</code></td></tr></table>




---

archive/issue_comments_254135.json:
```json
{
    "body": "Changed commit from **[c426c0c](https://github.com/sagemath/sagetrac-mirror/commit/c426c0c734bdb614c18b1a047dbe36e1d24bf86e)** to **[2044e52](https://github.com/sagemath/sagetrac-mirror/commit/2044e5259d0b897c0b65d430c3c06f4a5444561a)**",
    "created_at": "2019-08-03T10:32:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254135",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c426c0c](https://github.com/sagemath/sagetrac-mirror/commit/c426c0c734bdb614c18b1a047dbe36e1d24bf86e)** to **[2044e52](https://github.com/sagemath/sagetrac-mirror/commit/2044e5259d0b897c0b65d430c3c06f4a5444561a)**



---

archive/issue_comments_254136.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2044e5259d0b897c0b65d430c3c06f4a5444561a\">2044e52</a></td><td><code>Fixing a bug with rank.</code></td></tr></table>\n",
    "created_at": "2019-08-03T10:32:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254136",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2044e5259d0b897c0b65d430c3c06f4a5444561a">2044e52</a></td><td><code>Fixing a bug with rank.</code></td></tr></table>




---

archive/issue_events_242629.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-08-03T10:32:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242629"
}
```



---

archive/issue_events_242630.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-08-03T10:32:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242630"
}
```



---

archive/issue_events_242631.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-08-03T10:32:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242631"
}
```



---

archive/issue_events_242632.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-08-03T10:32:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242632"
}
```



---

archive/issue_comments_254137.json:
```json
{
    "body": "Changed work issues from **merge in develop** to none",
    "created_at": "2019-08-03T10:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254137",
    "user": "https://github.com/tscrim"
}
```

Changed work issues from **merge in develop** to none



---

archive/issue_comments_254138.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nRebased on the matrix constructor and tests pass in the matrix folder.",
    "created_at": "2019-08-03T10:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254138",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">Comment 9</div>

Rebased on the matrix constructor and tests pass in the matrix folder.



---

archive/issue_comments_254139.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nIn\n\n```\nif isinstance(e[0], Vector) and all(vec.is_sparse() for vec in e):\n```\nyou seem to assume that if the first element of the list is a vector than all elements are. This would break on something like\n\n```\nmatrix(3, [vector([0,1,0],sparse=True), [0,1,0], [1,1,1]])\n```",
    "created_at": "2019-08-03T14:05:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254139",
    "user": "https://github.com/videlec"
}
```

<div id="comment:10" align="right">Comment 10</div>

In

```
if isinstance(e[0], Vector) and all(vec.is_sparse() for vec in e):
```
you seem to assume that if the first element of the list is a vector than all elements are. This would break on something like

```
matrix(3, [vector([0,1,0],sparse=True), [0,1,0], [1,1,1]])
```



---

archive/issue_comments_254140.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nI know, but that matches what `sequence_type()` does. So I am just following the same assumption.",
    "created_at": "2019-08-03T15:45:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254140",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:11" align="right">Comment 11</div>

I know, but that matches what `sequence_type()` does. So I am just following the same assumption.



---

archive/issue_events_242633.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-08-16T20:19:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242633"
}
```



---

archive/issue_events_242634.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-08-16T20:19:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242634"
}
```



---

archive/issue_comments_254141.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nIt is quite fragile then! It is not uncommon to have a bunch of vectors that we want to use to construct rows `0,...,k` and then feed a list for the `k+1`-th row.\nSince it follows the design I guess it is what should be done...",
    "created_at": "2019-08-16T20:19:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254141",
    "user": "https://github.com/videlec"
}
```

<div id="comment:12" align="right">Comment 12</div>

It is quite fragile then! It is not uncommon to have a bunch of vectors that we want to use to construct rows `0,...,k` and then feed a list for the `k+1`-th row.
Since it follows the design I guess it is what should be done...



---

archive/issue_comments_254142.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2019-08-16T20:19:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254142",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_comments_254143.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nYea, I agree, but it probably requires a little larger of a change to fix it properly. Thank you for the review.",
    "created_at": "2019-08-16T22:54:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254143",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:13" align="right">Comment 13</div>

Yea, I agree, but it probably requires a little larger of a change to fix it properly. Thank you for the review.



---

archive/issue_comments_254144.json:
```json
{
    "body": "Changed branch from **[public/linear_algebra/sparse_matrix_from_sparse_vectors-18312](https://github.com/sagemath/sagetrac-mirror/tree/public/linear_algebra/sparse_matrix_from_sparse_vectors-18312)** to **[2044e52](https://github.com/sagemath/sagetrac-mirror/commit/2044e5259d0b897c0b65d430c3c06f4a5444561a)**",
    "created_at": "2019-08-18T18:46:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18312#issuecomment-254144",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/linear_algebra/sparse_matrix_from_sparse_vectors-18312](https://github.com/sagemath/sagetrac-mirror/tree/public/linear_algebra/sparse_matrix_from_sparse_vectors-18312)** to **[2044e52](https://github.com/sagemath/sagetrac-mirror/commit/2044e5259d0b897c0b65d430c3c06f4a5444561a)**



---

archive/issue_events_242635.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-08-18T18:46:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242635"
}
```



---

archive/issue_events_242636.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6fcbcf19be2df8988ad13ea91b4efe7b8a9f6a9b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-08-18T18:46:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18312",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18312#event-242636"
}
```
