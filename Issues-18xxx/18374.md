# Issue 18374: Inconsistency in dimension of total ideals.

archive/issues_018137.json:
```json
{
    "body": "As mentioned in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/hT_ySC_477E), there is an inconsistency in the dimension of total ideals.\n\nSingular returns -1 for the rings it handles, whereas our fallback implementation returns 0.\n\nCC:  jacobkroeker\n\nBranch/Commit: b8051ee8f65fb6fca0d5f2b928942ff4054cbc23\n\nReviewer: Grayson Jorgenson\n\nAuthor: Miguel Marco\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/18374\n\n",
    "closed_at": "2015-05-20T17:45:14Z",
    "created_at": "2015-05-06T19:31:51Z",
    "labels": [
        "component: algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.7",
    "title": "Inconsistency in dimension of total ideals.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/18374",
    "user": "https://github.com/miguelmarco"
}
```
As mentioned in [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/hT_ySC_477E), there is an inconsistency in the dimension of total ideals.

Singular returns -1 for the rings it handles, whereas our fallback implementation returns 0.

CC:  jacobkroeker

Branch/Commit: b8051ee8f65fb6fca0d5f2b928942ff4054cbc23

Reviewer: Grayson Jorgenson

Author: Miguel Marco

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/18374





---

archive/issue_comments_319797.json:
```json
{
    "body": "Changing branch from \"\" to \"bugdimension\"",
    "created_at": "2015-05-06T19:34:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319797",
    "user": "https://github.com/miguelmarco"
}
```

Changing branch from "" to "bugdimension"



---

archive/issue_comments_319798.json:
```json
{
    "body": "Changing branch from \"bugdimension\" to \"u/mmarco/bugdimension\"",
    "created_at": "2015-05-06T19:35:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319798",
    "user": "https://github.com/miguelmarco"
}
```

Changing branch from "bugdimension" to "u/mmarco/bugdimension"



---

archive/issue_comments_319799.json:
```json
{
    "body": "<a id='comment:3'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                                                  |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------|\n|[b8051ee](http://git.sagemath.org/sage.git/commit/?id=b8051ee8f65fb6fca0d5f2b928942ff4054cbc23)|`Added the case dimension=-1 for total ideals over rings not handled by Singular.`|",
    "created_at": "2015-05-06T19:41:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319799",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:3'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                                                  |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------|
|[b8051ee](http://git.sagemath.org/sage.git/commit/?id=b8051ee8f65fb6fca0d5f2b928942ff4054cbc23)|`Added the case dimension=-1 for total ideals over rings not handled by Singular.`|



---

archive/issue_comments_319800.json:
```json
{
    "body": "Changing commit from \"\" to \"b8051ee8f65fb6fca0d5f2b928942ff4054cbc23\"",
    "created_at": "2015-05-06T19:41:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319800",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "" to "b8051ee8f65fb6fca0d5f2b928942ff4054cbc23"



---

archive/issue_comments_319801.json:
```json
{
    "body": "<a id='comment:4'></a>\nHi, this simplifies some of the functionality in a ticket I have been working on. Is this ticket ready for review?",
    "created_at": "2015-05-18T15:33:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319801",
    "user": "https://trac.sagemath.org/admin/accounts/users/gjorgenson"
}
```

<a id='comment:4'></a>
Hi, this simplifies some of the functionality in a ticket I have been working on. Is this ticket ready for review?



---

archive/issue_comments_319802.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-05-18T15:37:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319802",
    "user": "https://github.com/miguelmarco"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_319803.json:
```json
{
    "body": "<a id='comment:5'></a>\nYes, it should be ready for review.",
    "created_at": "2015-05-18T15:37:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319803",
    "user": "https://github.com/miguelmarco"
}
```

<a id='comment:5'></a>
Yes, it should be ready for review.



---

archive/issue_comments_319804.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-05-19T21:13:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319804",
    "user": "https://trac.sagemath.org/admin/accounts/users/gjorgenson"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_319805.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Grayson Jorgenson\"",
    "created_at": "2015-05-19T21:13:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319805",
    "user": "https://trac.sagemath.org/admin/accounts/users/gjorgenson"
}
```

Changing reviewer from "" to "Grayson Jorgenson"



---

archive/issue_comments_319806.json:
```json
{
    "body": "<a id='comment:6'></a>\nCorrect dimension is now returned and everything seems to be working properly.",
    "created_at": "2015-05-19T21:13:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319806",
    "user": "https://trac.sagemath.org/admin/accounts/users/gjorgenson"
}
```

<a id='comment:6'></a>
Correct dimension is now returned and everything seems to be working properly.



---

archive/issue_events_051811.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-20T17:45:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18374#event-51811"
}
```



---

archive/issue_comments_319807.json:
```json
{
    "body": "Changing branch from \"u/mmarco/bugdimension\" to \"b8051ee8f65fb6fca0d5f2b928942ff4054cbc23\"",
    "created_at": "2015-05-20T17:45:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319807",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/mmarco/bugdimension" to "b8051ee8f65fb6fca0d5f2b928942ff4054cbc23"



---

archive/issue_comments_319808.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-05-20T17:45:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18374#issuecomment-319808",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
