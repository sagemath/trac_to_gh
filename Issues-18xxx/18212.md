# Issue 18212: fix NameError in mathematica_free integration

archive/issues_017975.json:
```json
{
    "assignees": [],
    "body": "It seems like the purpose was to normalize the variable of integration to `x`.\n\nThe line in question substitutes x for `dvar` rather than the variable of integration passed in, `v`.\n\nhttps://github.com/sagemath/sage/pull/37\n\nBranch/Commit: **[5d3207a](https://github.com/sagemath/sagetrac-mirror/commit/5d3207aad5e434be48f7dfd4c3db5cbd4c1b000b)**\n\nReviewer: **Karl-Dieter Crisman, Travis Scrimshaw**\n\nAuthor: **Buck Evan, Karl-Dieter Crisman**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18212_\n\n",
    "closed_at": "2017-05-14T08:20:11Z",
    "created_at": "2015-04-15T20:32:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix NameError in mathematica_free integration",
    "type": "issue",
    "updated_at": "2017-05-14T08:20:11Z",
    "url": "https://github.com/sagemath/sage/issues/18212",
    "user": "https://github.com/bukzor"
}
```
It seems like the purpose was to normalize the variable of integration to `x`.

The line in question substitutes x for `dvar` rather than the variable of integration passed in, `v`.

https://github.com/sagemath/sage/pull/37

Branch/Commit: **[5d3207a](https://github.com/sagemath/sagetrac-mirror/commit/5d3207aad5e434be48f7dfd4c3db5cbd4c1b000b)**

Reviewer: **Karl-Dieter Crisman, Travis Scrimshaw**

Author: **Buck Evan, Karl-Dieter Crisman**

_Issue created by migration from https://trac.sagemath.org/ticket/18212_





---

archive/issue_events_241216.json:
```json
{
    "actor": "https://github.com/bukzor",
    "created_at": "2015-04-15T20:32:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241216"
}
```



---

archive/issue_events_241217.json:
```json
{
    "actor": "https://github.com/bukzor",
    "created_at": "2015-04-15T20:32:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241217"
}
```



---

archive/issue_events_241218.json:
```json
{
    "actor": "https://github.com/sagetrac-github",
    "created_at": "2015-04-15T20:32:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241218"
}
```



---

archive/issue_comments_251797.json:
```json
{
    "body": "Branch: **[u/github/ticket/18212](https://github.com/sagemath/sagetrac-mirror/tree/u/github/ticket/18212)**",
    "created_at": "2015-04-15T20:32:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251797",
    "user": "https://github.com/sagetrac-github"
}
```

Branch: **[u/github/ticket/18212](https://github.com/sagemath/sagetrac-mirror/tree/u/github/ticket/18212)**



---

archive/issue_comments_251798.json:
```json
{
    "body": "Commit: **[1b5e936](https://github.com/sagemath/sagetrac-mirror/commit/1b5e93620d3ac86ea9601db270c0f87548ae36e0)**",
    "created_at": "2015-04-15T20:34:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251798",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[1b5e936](https://github.com/sagemath/sagetrac-mirror/commit/1b5e93620d3ac86ea9601db270c0f87548ae36e0)**



---

archive/issue_comments_251799.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1b5e93620d3ac86ea9601db270c0f87548ae36e0\">1b5e936</a></td><td><code>fix NameError in mathematica_free integration</code></td></tr></table>\n",
    "created_at": "2015-04-15T20:34:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251799",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1b5e93620d3ac86ea9601db270c0f87548ae36e0">1b5e936</a></td><td><code>fix NameError in mathematica_free integration</code></td></tr></table>




---

archive/issue_events_241219.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-05-12T19:25:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241219"
}
```



---

archive/issue_events_241220.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-05-12T19:25:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241220"
}
```



---

archive/issue_comments_251800.json:
```json
{
    "body": "Author: **Buck Evan**",
    "created_at": "2015-05-29T02:10:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251800",
    "user": "https://github.com/kcrisman"
}
```

Author: **Buck Evan**



---

archive/issue_comments_251801.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nCan you give an example of where this `NameError` occurs?  It would make little sense to change this without somehow doctesting what it was that went wrong.  In the most recent Sage devel version I get\n\n```\nsage: integral(sin(x)^2, x, algorithm='mathematica_free')\n-1/2*cos(x)*sin(x) + 1/2*x\n```",
    "created_at": "2015-05-29T04:29:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251801",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:5'>Comment 5:</a>
Can you give an example of where this `NameError` occurs?  It would make little sense to change this without somehow doctesting what it was that went wrong.  In the most recent Sage devel version I get

```
sage: integral(sin(x)^2, x, algorithm='mathematica_free')
-1/2*cos(x)*sin(x) + 1/2*x
```



---

archive/issue_events_241221.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-05-29T04:30:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241221"
}
```



---

archive/issue_events_241222.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-05-29T04:30:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241222"
}
```



---

archive/issue_comments_251802.json:
```json
{
    "body": "Work Issues: **example needed, doctest**",
    "created_at": "2015-05-29T04:30:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251802",
    "user": "https://github.com/kcrisman"
}
```

Work Issues: **example needed, doctest**



---

archive/issue_comments_251803.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\ngithub/kcrisman wrote:\n\nHi!  Let's continue the discussion at http://trac.sagemath.org/ticket/18212.",
    "created_at": "2015-05-29T04:31:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251803",
    "user": "https://github.com/sagetrac-github"
}
```

<a id='comment:7'>Comment 7:</a>
github/kcrisman wrote:

Hi!  Let's continue the discussion at http://trac.sagemath.org/ticket/18212.



---

archive/issue_comments_251804.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\n> Can you give an example of where this `NameError` occurs?  It would make little sense to change this without somehow doctesting what it was that went wrong.  In the most recent Sage devel version I get\n\nFigured it out.\n\n```\nsage: var('y')\ny\nsage: integral(sin(y)^2, y, algorithm='mathematica_free')\n---------------------------------------------------------------------------\nNameError: global name 'dvar' is not defined\n```",
    "created_at": "2015-05-29T04:31:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251804",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:8'>Comment 8:</a>
> Can you give an example of where this `NameError` occurs?  It would make little sense to change this without somehow doctesting what it was that went wrong.  In the most recent Sage devel version I get

Figured it out.

```
sage: var('y')
y
sage: integral(sin(y)^2, y, algorithm='mathematica_free')
---------------------------------------------------------------------------
NameError: global name 'dvar' is not defined
```



---

archive/issue_comments_251805.json:
```json
{
    "body": "Changed commit from **[1b5e936](https://github.com/sagemath/sagetrac-mirror/commit/1b5e93620d3ac86ea9601db270c0f87548ae36e0)** to **[5078cf8](https://github.com/sagemath/sagetrac-mirror/commit/5078cf84761975ad3539bef966655ef328f1e0f5)**",
    "created_at": "2015-05-29T04:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251805",
    "user": "https://github.com/kcrisman"
}
```

Changed commit from **[1b5e936](https://github.com/sagemath/sagetrac-mirror/commit/1b5e93620d3ac86ea9601db270c0f87548ae36e0)** to **[5078cf8](https://github.com/sagemath/sagetrac-mirror/commit/5078cf84761975ad3539bef966655ef328f1e0f5)**



---

archive/issue_events_241223.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-05-29T04:51:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241223"
}
```



---

archive/issue_events_241224.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-05-29T04:51:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241224"
}
```



---

archive/issue_comments_251806.json:
```json
{
    "body": "Changed branch from **[u/github/ticket/18212](https://github.com/sagemath/sagetrac-mirror/tree/u/github/ticket/18212)** to **[u/kcrisman/ticket/18212](https://github.com/sagemath/sagetrac-mirror/tree/u/kcrisman/ticket/18212)**",
    "created_at": "2015-05-29T04:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251806",
    "user": "https://github.com/kcrisman"
}
```

Changed branch from **[u/github/ticket/18212](https://github.com/sagemath/sagetrac-mirror/tree/u/github/ticket/18212)** to **[u/kcrisman/ticket/18212](https://github.com/sagemath/sagetrac-mirror/tree/u/kcrisman/ticket/18212)**



---

archive/issue_comments_251807.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI'm pushing a branch with doctests, and in doing this made a few more optional for internet so one doesn't have the `optional=internet,sage` problem as usual, then I replicated a different test elsewhere so it still gets tested ordinarily.  Basically just needs review that this all works.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c254015bdf37600d409a9f1d264c7e8462de7698\">c254015</a></td><td><code>Merge branch 'u/github/ticket/18212' of git://trac.sagemath.org/sage into ticket/18212</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5078cf84761975ad3539bef966655ef328f1e0f5\">5078cf8</a></td><td><code>Doctest Trac 18212 for mma_free integration fix</code></td></tr></table>\n",
    "created_at": "2015-05-29T04:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251807",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:9'>Comment 9:</a>
I'm pushing a branch with doctests, and in doing this made a few more optional for internet so one doesn't have the `optional=internet,sage` problem as usual, then I replicated a different test elsewhere so it still gets tested ordinarily.  Basically just needs review that this all works.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c254015bdf37600d409a9f1d264c7e8462de7698">c254015</a></td><td><code>Merge branch 'u/github/ticket/18212' of git://trac.sagemath.org/sage into ticket/18212</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5078cf84761975ad3539bef966655ef328f1e0f5">5078cf8</a></td><td><code>Doctest Trac 18212 for mma_free integration fix</code></td></tr></table>




---

archive/issue_comments_251808.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman**",
    "created_at": "2015-05-29T04:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251808",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Karl-Dieter Crisman**



---

archive/issue_comments_251809.json:
```json
{
    "body": "Changed work issues from **example needed, doctest** to none",
    "created_at": "2015-05-29T04:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251809",
    "user": "https://github.com/kcrisman"
}
```

Changed work issues from **example needed, doctest** to none



---

archive/issue_comments_251810.json:
```json
{
    "body": "Changed author from **Buck Evan** to **Buck Evan, Karl-Dieter Crisman**",
    "created_at": "2015-05-29T04:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251810",
    "user": "https://github.com/kcrisman"
}
```

Changed author from **Buck Evan** to **Buck Evan, Karl-Dieter Crisman**



---

archive/issue_comments_251811.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nWhy are these tests marked `# optional`?\n\n```\nsage: _ = var('x, y, z') # optional - internet\nsage: f = sin(x^2) + y^z # optional - internet\n```",
    "created_at": "2015-06-19T19:51:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251811",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'>Comment 10:</a>
Why are these tests marked `# optional`?

```
sage: _ = var('x, y, z') # optional - internet
sage: f = sin(x^2) + y^z # optional - internet
```



---

archive/issue_events_241225.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-19T19:51:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241225"
}
```



---

archive/issue_events_241226.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-19T19:51:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241226"
}
```



---

archive/issue_comments_251812.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nSame question here:\n\n```\nsage: from sage.symbolic.integration.external import mma_free_integrator # optional - internet\n```\n\nIf importing the module requires internet, you're doing it wrong...",
    "created_at": "2015-06-19T19:52:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251812",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:11'>Comment 11:</a>
Same question here:

```
sage: from sage.symbolic.integration.external import mma_free_integrator # optional - internet
```

If importing the module requires internet, you're doing it wrong...



---

archive/issue_comments_251813.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nAnd there is this oddity:\n\n```\nTest a few imports, without internet::\n```\n???",
    "created_at": "2015-06-19T19:54:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251813",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'>Comment 12:</a>
And there is this oddity:

```
Test a few imports, without internet::
```
???



---

archive/issue_comments_251814.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\n> I'm pushing a branch with doctests, and in doing this made a few more optional for internet so one doesn't have the `optional=internet,sage` problem as usual, then I replicated a different test elsewhere so it still gets tested ordinarily. \n\nI hope that this quote from [comment:9](#comment%3A9) answers your questions.  It's very annoying to do `--optional=internet` and then have tests fail because the \"prereq\" lines weren't evaluated.  So that is my philosophy, to avoid `--optional=sage,internet` which I view as more than annoying to remember.",
    "created_at": "2015-06-19T20:06:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251814",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:13'>Comment 13:</a>
> I'm pushing a branch with doctests, and in doing this made a few more optional for internet so one doesn't have the `optional=internet,sage` problem as usual, then I replicated a different test elsewhere so it still gets tested ordinarily. 

I hope that this quote from [comment:9](#comment%3A9) answers your questions.  It's very annoying to do `--optional=internet` and then have tests fail because the "prereq" lines weren't evaluated.  So that is my philosophy, to avoid `--optional=sage,internet` which I view as more than annoying to remember.



---

archive/issue_comments_251815.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nAh, I see now.",
    "created_at": "2015-06-19T20:59:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251815",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:14'>Comment 14:</a>
Ah, I see now.



---

archive/issue_comments_251816.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nIf you really have problems with the fact that `./sage -t --optional=internet` tests do not work, the proper fix would simply be to change the doctest framework (after sending a message to sage-devel of course). As far as I know, when we rewrote the doctest framework in #12415, we implemented `--optional` the way it is simply to emulate historical behaviour, I don't think it was a conscious choice.",
    "created_at": "2015-06-19T21:09:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251816",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:15'>Comment 15:</a>
If you really have problems with the fact that `./sage -t --optional=internet` tests do not work, the proper fix would simply be to change the doctest framework (after sending a message to sage-devel of course). As far as I know, when we rewrote the doctest framework in #12415, we implemented `--optional` the way it is simply to emulate historical behaviour, I don't think it was a conscious choice.



---

archive/issue_comments_251817.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nThe fact that your patch actually needs to duplicate a doctest (once with `# optional` and once without) shows to me that your solution is not good.",
    "created_at": "2015-06-19T21:10:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251817",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:16'>Comment 16:</a>
The fact that your patch actually needs to duplicate a doctest (once with `# optional` and once without) shows to me that your solution is not good.



---

archive/issue_comments_251818.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nWell, I only do that so that it's 100% sure to be doctested.  Submit a change you like and I'll review it, this isn't that horrible in any case, the key is to fix the error.",
    "created_at": "2015-06-20T13:29:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251818",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:17'>Comment 17:</a>
Well, I only do that so that it's 100% sure to be doctested.  Submit a change you like and I'll review it, this isn't that horrible in any case, the key is to fix the error.



---

archive/issue_events_241227.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-23T13:49:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241227"
}
```



---

archive/issue_events_241228.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-23T13:49:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241228"
}
```



---

archive/issue_comments_251819.json:
```json
{
    "body": "Changed branch from **[u/kcrisman/ticket/18212](https://github.com/sagemath/sagetrac-mirror/tree/u/kcrisman/ticket/18212)** to **[public/18212](https://github.com/sagemath/sagetrac-mirror/tree/public/18212)**",
    "created_at": "2017-05-11T18:50:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251819",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/kcrisman/ticket/18212](https://github.com/sagemath/sagetrac-mirror/tree/u/kcrisman/ticket/18212)** to **[public/18212](https://github.com/sagemath/sagetrac-mirror/tree/public/18212)**



---

archive/issue_comments_251820.json:
```json
{
    "body": "Changed commit from **[5078cf8](https://github.com/sagemath/sagetrac-mirror/commit/5078cf84761975ad3539bef966655ef328f1e0f5)** to **[5d3207a](https://github.com/sagemath/sagetrac-mirror/commit/5d3207aad5e434be48f7dfd4c3db5cbd4c1b000b)**",
    "created_at": "2017-05-11T18:50:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251820",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[5078cf8](https://github.com/sagemath/sagetrac-mirror/commit/5078cf84761975ad3539bef966655ef328f1e0f5)** to **[5d3207a](https://github.com/sagemath/sagetrac-mirror/commit/5d3207aad5e434be48f7dfd4c3db5cbd4c1b000b)**



---

archive/issue_comments_251821.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5d3207aad5e434be48f7dfd4c3db5cbd4c1b000b\">5d3207a</a></td><td><code>Merge branch 'u/kcrisman/ticket/18212' in 8.0.b5</code></td></tr></table>\n",
    "created_at": "2017-05-11T18:50:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251821",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:19'>Comment 19:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5d3207aad5e434be48f7dfd4c3db5cbd4c1b000b">5d3207a</a></td><td><code>Merge branch 'u/kcrisman/ticket/18212' in 8.0.b5</code></td></tr></table>




---

archive/issue_events_241229.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-05-11T18:52:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241229"
}
```



---

archive/issue_events_241230.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-05-11T18:52:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241230"
}
```



---

archive/issue_events_241231.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-05-11T18:52:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241231"
}
```



---

archive/issue_events_241232.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-05-11T18:52:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241232"
}
```



---

archive/issue_comments_251822.json:
```json
{
    "body": "Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Travis Scrimshaw**",
    "created_at": "2017-05-13T01:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251822",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Travis Scrimshaw**



---

archive/issue_comments_251823.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nWhile I agree with Jeroen that we should have the minimal amount of `# optional` tags as needed, I see this as essentially bikeshedding. So I'm setting this to a positive review.",
    "created_at": "2017-05-13T01:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251823",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:21'>Comment 21:</a>
While I agree with Jeroen that we should have the minimal amount of `# optional` tags as needed, I see this as essentially bikeshedding. So I'm setting this to a positive review.



---

archive/issue_events_241233.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-13T01:17:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241233"
}
```



---

archive/issue_events_241234.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-13T01:17:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241234"
}
```



---

archive/issue_comments_251824.json:
```json
{
    "body": "Changed branch from **[public/18212](https://github.com/sagemath/sagetrac-mirror/tree/public/18212)** to **[5d3207a](https://github.com/sagemath/sagetrac-mirror/commit/5d3207aad5e434be48f7dfd4c3db5cbd4c1b000b)**",
    "created_at": "2017-05-14T08:20:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18212#issuecomment-251824",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/18212](https://github.com/sagemath/sagetrac-mirror/tree/public/18212)** to **[5d3207a](https://github.com/sagemath/sagetrac-mirror/commit/5d3207aad5e434be48f7dfd4c3db5cbd4c1b000b)**



---

archive/issue_events_241235.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-05-14T08:20:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241235"
}
```



---

archive/issue_events_241236.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1d9e2a5d457184e44429cea9a7c8bc034ade8472",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-05-14T08:20:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18212",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18212#event-241236"
}
```
