# Issue 18108: Deprecate sage.rings.field

archive/issues_017871.json:
```json
{
    "assignees": [],
    "body": "This module contains just one function which is used nowhere in the Sage library. Moreover, it's confusing that often `Field` is imported from `sage.rings.field` while it's actually defined in `sage.rings.ring` (in Cython, so Cython modules can take advantage of that!)\n\nKeywords: **sd66**\n\nBranch/Commit: **[8a84e75](https://github.com/sagemath/sagetrac-mirror/commit/8a84e75974b3f739beeb84bb7fab73e4dd913424)**\n\nReviewer: **Ralf Stephan**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18108_\n\n",
    "closed_at": "2015-04-14T19:43:20Z",
    "created_at": "2015-04-02T14:01:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Deprecate sage.rings.field",
    "type": "issue",
    "updated_at": "2015-04-14T19:43:20Z",
    "url": "https://github.com/sagemath/sage/issues/18108",
    "user": "https://github.com/jdemeyer"
}
```
This module contains just one function which is used nowhere in the Sage library. Moreover, it's confusing that often `Field` is imported from `sage.rings.field` while it's actually defined in `sage.rings.ring` (in Cython, so Cython modules can take advantage of that!)

Keywords: **sd66**

Branch/Commit: **[8a84e75](https://github.com/sagemath/sagetrac-mirror/commit/8a84e75974b3f739beeb84bb7fab73e4dd913424)**

Reviewer: **Ralf Stephan**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/18108_





---

archive/issue_events_231139.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T14:01:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18108#event-231139"
}
```



---

archive/issue_events_231140.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T14:01:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18108#event-231140"
}
```



---

archive/issue_events_231141.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T14:01:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18108#event-231141"
}
```



---

archive/issue_events_231142.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T14:01:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18108#event-231142"
}
```



---

archive/issue_comments_249519.json:
```json
{
    "body": "Branch: **[u/jdemeyer/deprecate_sage_rings_field](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/deprecate_sage_rings_field)**",
    "created_at": "2015-04-02T14:59:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18108#issuecomment-249519",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/deprecate_sage_rings_field](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/deprecate_sage_rings_field)**



---

archive/issue_comments_249520.json:
```json
{
    "body": "Commit: **[8a84e75](https://github.com/sagemath/sagetrac-mirror/commit/8a84e75974b3f739beeb84bb7fab73e4dd913424)**",
    "created_at": "2015-04-02T15:01:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18108#issuecomment-249520",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[8a84e75](https://github.com/sagemath/sagetrac-mirror/commit/8a84e75974b3f739beeb84bb7fab73e4dd913424)**



---

archive/issue_events_231143.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T15:01:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18108#event-231143"
}
```



---

archive/issue_comments_249521.json:
```json
{
    "body": "Changed keywords from **** to **sd66**",
    "created_at": "2015-04-02T15:01:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18108#issuecomment-249521",
    "user": "https://github.com/jdemeyer"
}
```

Changed keywords from **** to **sd66**



---

archive/issue_comments_249522.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8a84e75974b3f739beeb84bb7fab73e4dd913424\">8a84e75</a></td><td><code>Deprecate sage.rings.field</code></td></tr></table>\n",
    "created_at": "2015-04-02T15:01:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18108#issuecomment-249522",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8a84e75974b3f739beeb84bb7fab73e4dd913424">8a84e75</a></td><td><code>Deprecate sage.rings.field</code></td></tr></table>




---

archive/issue_comments_249523.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\n\n```\n+        sage: import sage.rings.field\n+        doctest:...: DeprecationWarning: the module sage.rings.field is deprecated and will be removed\n+        See http://trac.sagemath.org/18108 for details.\n```\nSo ... what are the details?  (That is, there is no reason apparent in the message, but presumably there is one obvious to those who use this, just would be helpful to have it here.)",
    "created_at": "2015-04-02T15:06:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18108#issuecomment-249523",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'>Comment 3:</a>

```
+        sage: import sage.rings.field
+        doctest:...: DeprecationWarning: the module sage.rings.field is deprecated and will be removed
+        See http://trac.sagemath.org/18108 for details.
```
So ... what are the details?  (That is, there is no reason apparent in the message, but presumably there is one obvious to those who use this, just would be helpful to have it here.)



---

archive/issue_comments_249524.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+This module contains just one function which is used nowhere in the Sage library. Moreover, it's confusing that often `Field` is imported from `sage.rings.field` while it's actually defined in `sage.rings.ring` (in Cython, so Cython modules can take advantage of that!)\n``````\n",
    "created_at": "2015-04-02T15:08:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18108#issuecomment-249524",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+This module contains just one function which is used nowhere in the Sage library. Moreover, it's confusing that often `Field` is imported from `sage.rings.field` while it's actually defined in `sage.rings.ring` (in Cython, so Cython modules can take advantage of that!)
``````




---

archive/issue_comments_249525.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLooking fine and simple. Passes all tests in http://patchbot.sagemath.org/log/18108/Ubuntu/14.04/i686/3.13.0-40-generic/arando/2015-04-03%2001:27:54%20+0100?short\n\nYour arando patchbot run, however, again seems to have problems, modules are not found.",
    "created_at": "2015-04-03T07:36:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18108#issuecomment-249525",
    "user": "https://github.com/rwst"
}
```

<a id='comment:5'>Comment 5:</a>
Looking fine and simple. Passes all tests in http://patchbot.sagemath.org/log/18108/Ubuntu/14.04/i686/3.13.0-40-generic/arando/2015-04-03%2001:27:54%20+0100?short

Your arando patchbot run, however, again seems to have problems, modules are not found.



---

archive/issue_comments_249526.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2015-04-03T07:36:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18108#issuecomment-249526",
    "user": "https://github.com/rwst"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_events_231144.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-04-03T07:36:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18108#event-231144"
}
```



---

archive/issue_events_231145.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-04-03T07:36:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18108#event-231145"
}
```



---

archive/issue_comments_249527.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/deprecate_sage_rings_field](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/deprecate_sage_rings_field)** to **[8a84e75](https://github.com/sagemath/sagetrac-mirror/commit/8a84e75974b3f739beeb84bb7fab73e4dd913424)**",
    "created_at": "2015-04-14T19:43:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18108#issuecomment-249527",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/deprecate_sage_rings_field](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/deprecate_sage_rings_field)** to **[8a84e75](https://github.com/sagemath/sagetrac-mirror/commit/8a84e75974b3f739beeb84bb7fab73e4dd913424)**



---

archive/issue_events_231146.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-14T19:43:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18108#event-231146"
}
```



---

archive/issue_events_231147.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4c4c4bcef0918644b62ac8b4b87c8b7380988851",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-04-14T19:43:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18108",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18108#event-231147"
}
```
