# Issue 18886: Remove comparison boilerplate - part 3

archive/issues_018649.json:
```json
{
    "assignees": [],
    "body": "Note: in `src/sage/numerical/linear_functions.pyx`, I changed a few explicit calls to `_richcmp` by calls to `PyObject_RichCompare`. This is to allow a future removal of `_richcmp`.\n\nCC:  @rwst @sagetrac-jpflori\n\nBranch/Commit: **[06aa927](https://github.com/sagemath/sagetrac-mirror/commit/06aa927d52d3ce444d2fec85675f1bf1488033ff)**\n\nReviewer: **Marc Mezzarobba**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18886_\n\n",
    "closed_at": "2015-09-04T23:52:12Z",
    "created_at": "2015-07-13T08:47:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove comparison boilerplate - part 3",
    "type": "issue",
    "updated_at": "2015-09-04T23:52:12Z",
    "url": "https://github.com/sagemath/sage/issues/18886",
    "user": "https://github.com/jdemeyer"
}
```
Note: in `src/sage/numerical/linear_functions.pyx`, I changed a few explicit calls to `_richcmp` by calls to `PyObject_RichCompare`. This is to allow a future removal of `_richcmp`.

CC:  @rwst @sagetrac-jpflori

Branch/Commit: **[06aa927](https://github.com/sagemath/sagetrac-mirror/commit/06aa927d52d3ce444d2fec85675f1bf1488033ff)**

Reviewer: **Marc Mezzarobba**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/18886_





---

archive/issue_events_250369.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T08:47:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250369"
}
```



---

archive/issue_events_250370.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T08:47:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250370"
}
```



---

archive/issue_events_250371.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T08:47:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250371"
}
```



---

archive/issue_events_250372.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T08:47:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250372"
}
```



---

archive/issue_comments_267065.json:
```json
{
    "body": "Branch: **[u/jdemeyer/remove_comparison_boilerplate___part_3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_comparison_boilerplate___part_3)**",
    "created_at": "2015-07-13T09:09:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18886#issuecomment-267065",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/remove_comparison_boilerplate___part_3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_comparison_boilerplate___part_3)**



---

archive/issue_comments_267066.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Note: in `src/sage/numerical/linear_functions.pyx`, I changed a few explicit calls to `_richcmp` by calls to `PyObject_RichCompare`. This is to allow a future removal of `_richcmp`.\n``````\n",
    "created_at": "2015-07-13T09:11:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18886#issuecomment-267066",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Note: in `src/sage/numerical/linear_functions.pyx`, I changed a few explicit calls to `_richcmp` by calls to `PyObject_RichCompare`. This is to allow a future removal of `_richcmp`.
``````




---

archive/issue_comments_267067.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/06aa927d52d3ce444d2fec85675f1bf1488033ff\">06aa927</a></td><td><code>Remove comparison boilerplate</code></td></tr></table>\n",
    "created_at": "2015-07-13T09:11:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18886#issuecomment-267067",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/06aa927d52d3ce444d2fec85675f1bf1488033ff">06aa927</a></td><td><code>Remove comparison boilerplate</code></td></tr></table>




---

archive/issue_events_250373.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T09:11:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250373"
}
```



---

archive/issue_comments_267068.json:
```json
{
    "body": "Commit: **[06aa927](https://github.com/sagemath/sagetrac-mirror/commit/06aa927d52d3ce444d2fec85675f1bf1488033ff)**",
    "created_at": "2015-07-13T09:11:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18886#issuecomment-267068",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[06aa927](https://github.com/sagemath/sagetrac-mirror/commit/06aa927d52d3ce444d2fec85675f1bf1488033ff)**



---

archive/issue_events_250374.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T09:51:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250374"
}
```



---

archive/issue_events_250375.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T09:51:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250375"
}
```



---

archive/issue_comments_267069.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nFalse alarm. I got doctest failures, but due to an unrelated problem.",
    "created_at": "2015-07-13T10:22:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18886#issuecomment-267069",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

False alarm. I got doctest failures, but due to an unrelated problem.



---

archive/issue_events_250376.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T10:22:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250376"
}
```



---

archive/issue_events_250377.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T10:22:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250377"
}
```



---

archive/issue_events_250378.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-09-03T12:07:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250378"
}
```



---

archive/issue_events_250379.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-09-03T12:07:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250379"
}
```



---

archive/issue_comments_267070.json:
```json
{
    "body": "Reviewer: **Marc Mezzarobba**",
    "created_at": "2015-09-03T12:07:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18886#issuecomment-267070",
    "user": "https://github.com/mezzarobba"
}
```

Reviewer: **Marc Mezzarobba**



---

archive/issue_events_250380.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-04T23:52:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250380"
}
```



---

archive/issue_events_250381.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4a698a881f0d1c78ea678148aa9b9e2a05cc6d97",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-09-04T23:52:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18886#event-250381"
}
```



---

archive/issue_comments_267071.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/remove_comparison_boilerplate___part_3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_comparison_boilerplate___part_3)** to **[06aa927](https://github.com/sagemath/sagetrac-mirror/commit/06aa927d52d3ce444d2fec85675f1bf1488033ff)**",
    "created_at": "2015-09-04T23:52:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18886#issuecomment-267071",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/remove_comparison_boilerplate___part_3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_comparison_boilerplate___part_3)** to **[06aa927](https://github.com/sagemath/sagetrac-mirror/commit/06aa927d52d3ce444d2fec85675f1bf1488033ff)**
