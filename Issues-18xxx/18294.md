# Issue 18294: Cython memleak

archive/issues_018057.json:
```json
{
    "assignees": [],
    "body": "Upstream fix: [https://github.com/cython/cython/commit/bb4d9c2de71b7c7e1e02d9dfeae53f4547fa9d7d](https://github.com/cython/cython/commit/bb4d9c2de71b7c7e1e02d9dfeae53f4547fa9d7d)\n\nDepends on #18121\n\nBranch/Commit: **[ad3170e](https://github.com/sagemath/sagetrac-mirror/commit/ad3170e29e719419a850a628df130aef5bf17699)**\n\nUpstream: **Fixed upstream, but not in a stable release.**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18294_\n\n",
    "closed_at": "2015-04-25T22:53:56Z",
    "created_at": "2015-04-24T15:59:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cython memleak",
    "type": "issue",
    "updated_at": "2015-04-25T22:53:56Z",
    "url": "https://github.com/sagemath/sage/issues/18294",
    "user": "https://github.com/jdemeyer"
}
```
Upstream fix: [https://github.com/cython/cython/commit/bb4d9c2de71b7c7e1e02d9dfeae53f4547fa9d7d](https://github.com/cython/cython/commit/bb4d9c2de71b7c7e1e02d9dfeae53f4547fa9d7d)

Depends on #18121

Branch/Commit: **[ad3170e](https://github.com/sagemath/sagetrac-mirror/commit/ad3170e29e719419a850a628df130aef5bf17699)**

Upstream: **Fixed upstream, but not in a stable release.**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/18294_





---

archive/issue_events_242390.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-24T15:59:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18294#event-242390"
}
```



---

archive/issue_events_242391.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-24T15:59:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18294#event-242391"
}
```



---

archive/issue_events_242392.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-24T15:59:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18294#event-242392"
}
```



---

archive/issue_events_242393.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-24T15:59:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18294#event-242393"
}
```



---

archive/issue_comments_253785.json:
```json
{
    "body": "Upstream: **Reported upstream. Developers acknowledge bug.**",
    "created_at": "2015-04-24T16:00:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253785",
    "user": "https://github.com/jdemeyer"
}
```

Upstream: **Reported upstream. Developers acknowledge bug.**



---

archive/issue_comments_253786.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Upstream fix: [https://github.com/cython/cython/commit/bb4d9c2de71b7c7e1e02d9dfeae53f4547fa9d7d](https://github.com/cython/cython/commit/bb4d9c2de71b7c7e1e02d9dfeae53f4547fa9d7d)\n``````\n",
    "created_at": "2015-04-24T16:02:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253786",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Upstream fix: [https://github.com/cython/cython/commit/bb4d9c2de71b7c7e1e02d9dfeae53f4547fa9d7d](https://github.com/cython/cython/commit/bb4d9c2de71b7c7e1e02d9dfeae53f4547fa9d7d)
``````




---

archive/issue_comments_253787.json:
```json
{
    "body": "Changed upstream from **Reported upstream. Developers acknowledge bug.** to **Fixed upstream, but not in a stable release.**",
    "created_at": "2015-04-24T16:02:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253787",
    "user": "https://github.com/jdemeyer"
}
```

Changed upstream from **Reported upstream. Developers acknowledge bug.** to **Fixed upstream, but not in a stable release.**



---

archive/issue_comments_253788.json:
```json
{
    "body": "Dependencies: **#18121**",
    "created_at": "2015-04-24T16:04:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253788",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#18121**



---

archive/issue_comments_253789.json:
```json
{
    "body": "Branch: **[u/jdemeyer/cython_memleak](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/cython_memleak)**",
    "created_at": "2015-04-24T16:09:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253789",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/cython_memleak](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/cython_memleak)**



---

archive/issue_comments_253790.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a10dcbaaaefb87d48cf337be367cd48f11ed6edf\">a10dcba</a></td><td><code>Use PyTypeObject from Cython</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/77e38d28e608521c7a79decf16bfb3f09b089737\">77e38d2</a></td><td><code>Define 3-argument version Py_VISIT3 of Py_VISIT</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/58c17fbe84b31e534be250907717c52b8827e1d8\">58c17fb</a></td><td><code>cimport PyWeakref_GetObject() from cpython.weakref</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ad3170e29e719419a850a628df130aef5bf17699\">ad3170e</a></td><td><code>Add patch from upstream for Cython memleak</code></td></tr></table>\n",
    "created_at": "2015-04-24T16:09:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253790",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'>Comment 5:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a10dcbaaaefb87d48cf337be367cd48f11ed6edf">a10dcba</a></td><td><code>Use PyTypeObject from Cython</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/77e38d28e608521c7a79decf16bfb3f09b089737">77e38d2</a></td><td><code>Define 3-argument version Py_VISIT3 of Py_VISIT</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/58c17fbe84b31e534be250907717c52b8827e1d8">58c17fb</a></td><td><code>cimport PyWeakref_GetObject() from cpython.weakref</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ad3170e29e719419a850a628df130aef5bf17699">ad3170e</a></td><td><code>Add patch from upstream for Cython memleak</code></td></tr></table>




---

archive/issue_comments_253791.json:
```json
{
    "body": "Commit: **[ad3170e](https://github.com/sagemath/sagetrac-mirror/commit/ad3170e29e719419a850a628df130aef5bf17699)**",
    "created_at": "2015-04-24T16:09:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253791",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[ad3170e](https://github.com/sagemath/sagetrac-mirror/commit/ad3170e29e719419a850a628df130aef5bf17699)**



---

archive/issue_events_242394.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-24T16:09:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18294#event-242394"
}
```



---

archive/issue_comments_253792.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nlgtm",
    "created_at": "2015-04-24T18:58:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253792",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:6'>Comment 6:</a>
lgtm



---

archive/issue_events_242395.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-24T18:58:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18294#event-242395"
}
```



---

archive/issue_events_242396.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-24T18:58:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18294#event-242396"
}
```



---

archive/issue_comments_253793.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-04-24T18:59:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253793",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_253794.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nSo, where are the fancy charts? :-P",
    "created_at": "2015-04-25T01:50:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253794",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:8'>Comment 8:</a>
So, where are the fancy charts? :-P



---

archive/issue_comments_253795.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nAt https://github.com/vbraun/GraphPG :-P",
    "created_at": "2015-04-25T01:56:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253795",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:9'>Comment 9:</a>
At https://github.com/vbraun/GraphPG :-P



---

archive/issue_comments_253796.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/cython_memleak](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/cython_memleak)** to **[ad3170e](https://github.com/sagemath/sagetrac-mirror/commit/ad3170e29e719419a850a628df130aef5bf17699)**",
    "created_at": "2015-04-25T22:53:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18294#issuecomment-253796",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/cython_memleak](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/cython_memleak)** to **[ad3170e](https://github.com/sagemath/sagetrac-mirror/commit/ad3170e29e719419a850a628df130aef5bf17699)**



---

archive/issue_events_242397.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-25T22:53:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18294#event-242397"
}
```



---

archive/issue_events_242398.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "0e2f5833c4b301d4a1c58754d6cb684cf73b60cb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-04-25T22:53:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18294",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18294#event-242398"
}
```
