...
<a id='comment:7'></a>Made updates incorporating your suggestions. 
...
```json
{
    "body": "<a id='comment:9'></a>You use `self._connectivity(X, E-X)` now, but what I meant was `self.connectivity(X)`. That one simply computes rank(X)+rank(E-X)-rank(E), the other one does the whole matroid matching thing again.",
...
<a id='comment:9'></a>You use `self._connectivity(X, E-X)` now, but what I meant was `self.connectivity(X)`. That one simply computes rank(X)+rank(E-X)-rank(E), the other one does the whole matroid matching thing again.
...
<a id='comment:12'></a>Code looks good to me. It's a lot faster than before too..
