# Issue 18482: Unicode art for symbolics

archive/issues_018245.json:
```json
{
    "assignees": [],
    "body": "Example:\n\n```\nsage: %display unicode_art\nsage: integral(exp(sqrt(x))/(x+pi), x)\n\u2320          \n\u23ae    ___   \n\u23ae  \u2572\u2571 x    \n\u23ae \u212f        \n\u23ae \u2500\u2500\u2500\u2500\u2500\u2500 dx\n\u23ae x + \u03c0    \n\u2321          \n```\n\nDepends on #18357\n\nCC:  @videlec\n\nBranch/Commit: **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18482_\n\n",
    "closed_at": "2015-06-20T09:20:23Z",
    "created_at": "2015-05-24T09:27:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Unicode art for symbolics",
    "type": "issue",
    "updated_at": "2015-06-20T09:20:23Z",
    "url": "https://github.com/sagemath/sage/issues/18482",
    "user": "https://github.com/vbraun"
}
```
Example:

```
sage: %display unicode_art
sage: integral(exp(sqrt(x))/(x+pi), x)
⌠          
⎮    ___   
⎮  ╲╱ x    
⎮ ℯ        
⎮ ────── dx
⎮ x + π    
⌡          
```

Depends on #18357

CC:  @videlec

Branch/Commit: **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)**

Reviewer: **Vincent Delecroix**

Author: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/18482_





---

archive/issue_events_244881.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-24T09:27:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244881"
}
```



---

archive/issue_events_244882.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-24T09:27:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244882"
}
```



---

archive/issue_events_244883.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-24T09:41:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244883"
}
```



---

archive/issue_comments_257591.json:
```json
{
    "body": "Dependencies: **#18357**",
    "created_at": "2015-05-24T09:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257591",
    "user": "https://github.com/vbraun"
}
```

Dependencies: **#18357**



---

archive/issue_events_244884.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-24T09:41:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244884"
}
```



---

archive/issue_comments_257592.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,13 @@\n+Example:\n \n+```\n+sage: %display unicode_art\n+sage: integral(exp(sqrt(x))/(x+pi), x)\n+\u2320          \n+\u23ae    ___   \n+\u23ae  \u2572\u2571 x    \n+\u23ae \u212f        \n+\u23ae \u2500\u2500\u2500\u2500\u2500\u2500 dx\n+\u23ae x + \u03c0    \n+\u2321          \n+```\n``````\n",
    "created_at": "2015-05-24T10:01:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257592",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,13 @@
+Example:
 
+```
+sage: %display unicode_art
+sage: integral(exp(sqrt(x))/(x+pi), x)
+⌠          
+⎮    ___   
+⎮  ╲╱ x    
+⎮ ℯ        
+⎮ ────── dx
+⎮ x + π    
+⌡          
+```
``````




---

archive/issue_comments_257593.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2015-05-24T10:01:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257593",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_257594.json:
```json
{
    "body": "Branch: **[u/vbraun/unicode_art_for_symbolics](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/unicode_art_for_symbolics)**",
    "created_at": "2015-05-24T10:10:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257594",
    "user": "https://github.com/vbraun"
}
```

Branch: **[u/vbraun/unicode_art_for_symbolics](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/unicode_art_for_symbolics)**



---

archive/issue_comments_257595.json:
```json
{
    "body": "Commit: **[ef5b6a4](https://github.com/sagemath/sagetrac-mirror/commit/ef5b6a42b991ff971730ed6b7792847c19f6a200)**",
    "created_at": "2015-05-24T11:32:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257595",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[ef5b6a4](https://github.com/sagemath/sagetrac-mirror/commit/ef5b6a42b991ff971730ed6b7792847c19f6a200)**



---

archive/issue_comments_257596.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ef5b6a42b991ff971730ed6b7792847c19f6a200\">ef5b6a4</a></td><td><code>fix docbuild and doctests in sage.typeset</code></td></tr></table>\n",
    "created_at": "2015-05-24T11:32:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257596",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ef5b6a42b991ff971730ed6b7792847c19f6a200">ef5b6a4</a></td><td><code>fix docbuild and doctests in sage.typeset</code></td></tr></table>




---

archive/issue_events_244885.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-24T11:33:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244885"
}
```



---

archive/issue_comments_257597.json:
```json
{
    "body": "Work Issues: **rebase**",
    "created_at": "2015-06-01T19:14:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257597",
    "user": "https://github.com/videlec"
}
```

Work Issues: **rebase**



---

archive/issue_events_244886.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-06-01T19:14:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244886"
}
```



---

archive/issue_events_244887.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-06-01T19:14:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244887"
}
```



---

archive/issue_comments_257598.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3f68c336f942d02baba722231e290e9ab4ad5622\">3f68c33</a></td><td><code>Merge 6.8.beta2 into #18482</code></td></tr></table>\n",
    "created_at": "2015-06-02T22:23:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257598",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3f68c336f942d02baba722231e290e9ab4ad5622">3f68c33</a></td><td><code>Merge 6.8.beta2 into #18482</code></td></tr></table>




---

archive/issue_comments_257599.json:
```json
{
    "body": "Changed commit from **[ef5b6a4](https://github.com/sagemath/sagetrac-mirror/commit/ef5b6a42b991ff971730ed6b7792847c19f6a200)** to **[3f68c33](https://github.com/sagemath/sagetrac-mirror/commit/3f68c336f942d02baba722231e290e9ab4ad5622)**",
    "created_at": "2015-06-02T22:23:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257599",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ef5b6a4](https://github.com/sagemath/sagetrac-mirror/commit/ef5b6a42b991ff971730ed6b7792847c19f6a200)** to **[3f68c33](https://github.com/sagemath/sagetrac-mirror/commit/3f68c336f942d02baba722231e290e9ab4ad5622)**



---

archive/issue_comments_257600.json:
```json
{
    "body": "Changed work issues from **rebase** to none",
    "created_at": "2015-06-02T22:30:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257600",
    "user": "https://github.com/vbraun"
}
```

Changed work issues from **rebase** to none



---

archive/issue_comments_257601.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nfixed",
    "created_at": "2015-06-02T22:30:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257601",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:8" align="right">Comment 8</div>

fixed



---

archive/issue_events_244888.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-06-02T22:30:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244888"
}
```



---

archive/issue_events_244889.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-06-02T22:30:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244889"
}
```



---

archive/issue_comments_257602.json:
```json
{
    "body": "Changed commit from **[3f68c33](https://github.com/sagemath/sagetrac-mirror/commit/3f68c336f942d02baba722231e290e9ab4ad5622)** to **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)**",
    "created_at": "2015-06-09T17:10:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257602",
    "user": "https://github.com/videlec"
}
```

Changed commit from **[3f68c33](https://github.com/sagemath/sagetrac-mirror/commit/3f68c336f942d02baba722231e290e9ab4ad5622)** to **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)**



---

archive/issue_comments_257603.json:
```json
{
    "body": "Changed branch from **[u/vbraun/unicode_art_for_symbolics](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/unicode_art_for_symbolics)** to **[public/18482](https://github.com/sagemath/sagetrac-mirror/tree/public/18482)**",
    "created_at": "2015-06-09T17:10:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257603",
    "user": "https://github.com/videlec"
}
```

Changed branch from **[u/vbraun/unicode_art_for_symbolics](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/unicode_art_for_symbolics)** to **[public/18482](https://github.com/sagemath/sagetrac-mirror/tree/public/18482)**



---

archive/issue_comments_257604.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2015-06-09T17:10:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257604",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_comments_257605.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nHello,\n\nWaiting for the patchbot final check. But this is good for me.\n\nVincent\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f13355f88a6c2460f93cd549a5da51cc7724f4a7\">f13355f</a></td><td><code>Trac #18482: remove trailing Volker's whitespaces</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e\">db1c0e4</a></td><td><code>Trac #18482: fix sage_object.pyx</code></td></tr></table>\n",
    "created_at": "2015-06-09T17:10:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257605",
    "user": "https://github.com/videlec"
}
```

<div id="comment:9" align="right">Comment 9</div>

Hello,

Waiting for the patchbot final check. But this is good for me.

Vincent

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f13355f88a6c2460f93cd549a5da51cc7724f4a7">f13355f</a></td><td><code>Trac #18482: remove trailing Volker's whitespaces</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e">db1c0e4</a></td><td><code>Trac #18482: fix sage_object.pyx</code></td></tr></table>




---

archive/issue_comments_257606.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e01e0888b6d129d9cd50f5af37d43bc70574efbb\">e01e088</a></td><td><code>Merge branch 'develop' into t/18482/public/18482</code></td></tr></table>\n",
    "created_at": "2015-06-18T13:01:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257606",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e01e0888b6d129d9cd50f5af37d43bc70574efbb">e01e088</a></td><td><code>Merge branch 'develop' into t/18482/public/18482</code></td></tr></table>




---

archive/issue_comments_257607.json:
```json
{
    "body": "Changed commit from **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)** to **[e01e088](https://github.com/sagemath/sagetrac-mirror/commit/e01e0888b6d129d9cd50f5af37d43bc70574efbb)**",
    "created_at": "2015-06-18T13:01:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257607",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)** to **[e01e088](https://github.com/sagemath/sagetrac-mirror/commit/e01e0888b6d129d9cd50f5af37d43bc70574efbb)**



---

archive/issue_comments_257608.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nPatchbot passes with only some long test timeouts. The patchbot ascii plugin fails and I have difficulties making `doc-pdf` in `ru/tutorial` but that could be spurious.",
    "created_at": "2015-06-18T15:40:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257608",
    "user": "https://github.com/rwst"
}
```

<div id="comment:11" align="right">Comment 11</div>

Patchbot passes with only some long test timeouts. The patchbot ascii plugin fails and I have difficulties making `doc-pdf` in `ru/tutorial` but that could be spurious.



---

archive/issue_comments_257609.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nreplying to [comment:11](#comment%3A11): What did you do in `e01e088`? If this is only a merge commit you can remove it. If there is something else it needs to be separated.",
    "created_at": "2015-06-18T16:50:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257609",
    "user": "https://github.com/videlec"
}
```

<div id="comment:12" align="right">Comment 12</div>

replying to [comment:11](#comment%3A11): What did you do in `e01e088`? If this is only a merge commit you can remove it. If there is something else it needs to be separated.



---

archive/issue_comments_257610.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nReplying to [@videlec](#comment%3A12):\n> replying to [comment:11](#comment%3A11): What did you do in `e01e088`? If this is only a merge commit you can remove it. If there is something else it needs to be separated.\n\nThe branch link was red, so I merged develop. Indeed there was no merge conflict when I did. I'll remove it so we can see if it goes red again.",
    "created_at": "2015-06-19T06:52:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257610",
    "user": "https://github.com/rwst"
}
```

<div id="comment:13" align="right">Comment 13</div>

Replying to [@videlec](#comment%3A12):
> replying to [comment:11](#comment%3A11): What did you do in `e01e088`? If this is only a merge commit you can remove it. If there is something else it needs to be separated.

The branch link was red, so I merged develop. Indeed there was no merge conflict when I did. I'll remove it so we can see if it goes red again.



---

archive/issue_comments_257611.json:
```json
{
    "body": "Changed branch from **[public/18482](https://github.com/sagemath/sagetrac-mirror/tree/public/18482)** to **[public/18482-1](https://github.com/sagemath/sagetrac-mirror/tree/public/18482-1)**",
    "created_at": "2015-06-19T06:56:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257611",
    "user": "https://github.com/rwst"
}
```

Changed branch from **[public/18482](https://github.com/sagemath/sagetrac-mirror/tree/public/18482)** to **[public/18482-1](https://github.com/sagemath/sagetrac-mirror/tree/public/18482-1)**



---

archive/issue_comments_257612.json:
```json
{
    "body": "Changed commit from **[e01e088](https://github.com/sagemath/sagetrac-mirror/commit/e01e0888b6d129d9cd50f5af37d43bc70574efbb)** to **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)**",
    "created_at": "2015-06-19T06:57:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257612",
    "user": "https://github.com/rwst"
}
```

Changed commit from **[e01e088](https://github.com/sagemath/sagetrac-mirror/commit/e01e0888b6d129d9cd50f5af37d43bc70574efbb)** to **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)**



---

archive/issue_comments_257613.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nIt's red again, see for yourself.",
    "created_at": "2015-06-19T06:57:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257613",
    "user": "https://github.com/rwst"
}
```

<div id="comment:15" align="right">Comment 15</div>

It's red again, see for yourself.



---

archive/issue_comments_257614.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nReplying to [@rwst](#comment%3A15):\n> It's red again, see for yourself.\n\nI see. It was discussed on sage-devel. The merge test of the trac plugin is less concilient than the default git merge.\n\nBut still, I do not see the need of an additional commit.",
    "created_at": "2015-06-19T07:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257614",
    "user": "https://github.com/videlec"
}
```

<div id="comment:16" align="right">Comment 16</div>

Replying to [@rwst](#comment%3A15):
> It's red again, see for yourself.

I see. It was discussed on sage-devel. The merge test of the trac plugin is less concilient than the default git merge.

But still, I do not see the need of an additional commit.



---

archive/issue_events_244890.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-06-19T08:26:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244890"
}
```



---

archive/issue_events_244891.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-06-19T08:26:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244891"
}
```



---

archive/issue_comments_257615.json:
```json
{
    "body": "Changed branch from **[public/18482-1](https://github.com/sagemath/sagetrac-mirror/tree/public/18482-1)** to **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)**",
    "created_at": "2015-06-20T09:20:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18482#issuecomment-257615",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/18482-1](https://github.com/sagemath/sagetrac-mirror/tree/public/18482-1)** to **[db1c0e4](https://github.com/sagemath/sagetrac-mirror/commit/db1c0e4e9af353f8bc0f389477e76e7a8b58ff0e)**



---

archive/issue_events_244892.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-06-20T09:20:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244892"
}
```



---

archive/issue_events_244893.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "db62f344f4a29201e61ecadc52fdc3377710b52a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-06-20T09:20:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18482",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18482#event-244893"
}
```
