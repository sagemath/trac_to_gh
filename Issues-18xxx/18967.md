# Issue 18967: Silence the messages about deleting empty directories

archive/issues_018730.json:
```json
{
    "assignees": [],
    "body": "At the beginning of docbuilding, there are lots of messages of the form\n\n```\nDeleting empty directory /Users/palmieri/...../sage/src/doc/ca/intro/static\n```\nI for one am tired of seeing these, so let's silence them.\n\nWe should also move this clean-up so that it's done at the end of docbuilding: if docbuilding produces a bunch of useless empty directories, maybe it should clean up after itself.\n\n\nCC:  @vbraun\n\nBranch/Commit: **[54af37e](https://github.com/sagemath/sagetrac-mirror/commit/54af37ebf9f9aa1c8349feee4410dcb14e252da1)**\n\nReviewer: **Volker Braun**\n\nAuthor: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18967_\n\n",
    "closed_at": "2015-07-31T17:52:16Z",
    "created_at": "2015-07-29T18:08:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Silence the messages about deleting empty directories",
    "type": "issue",
    "updated_at": "2015-07-31T17:52:16Z",
    "url": "https://github.com/sagemath/sage/issues/18967",
    "user": "https://github.com/jhpalmieri"
}
```
At the beginning of docbuilding, there are lots of messages of the form

```
Deleting empty directory /Users/palmieri/...../sage/src/doc/ca/intro/static
```
I for one am tired of seeing these, so let's silence them.

We should also move this clean-up so that it's done at the end of docbuilding: if docbuilding produces a bunch of useless empty directories, maybe it should clean up after itself.


CC:  @vbraun

Branch/Commit: **[54af37e](https://github.com/sagemath/sagetrac-mirror/commit/54af37ebf9f9aa1c8349feee4410dcb14e252da1)**

Reviewer: **Volker Braun**

Author: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/18967_





---

archive/issue_events_251503.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-07-29T18:08:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18967#event-251503"
}
```



---

archive/issue_events_251504.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-07-29T18:08:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18967#event-251504"
}
```



---

archive/issue_events_251505.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-07-29T18:08:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18967#event-251505"
}
```



---

archive/issue_events_251506.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-07-29T18:09:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18967#event-251506"
}
```



---

archive/issue_events_251507.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-07-29T18:09:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18967#event-251507"
}
```



---

archive/issue_comments_269114.json:
```json
{
    "body": "Branch: **[u/jhpalmieri/silence_deleting_empty_directories](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/silence_deleting_empty_directories)**",
    "created_at": "2015-07-29T18:11:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18967#issuecomment-269114",
    "user": "https://github.com/jhpalmieri"
}
```

Branch: **[u/jhpalmieri/silence_deleting_empty_directories](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/silence_deleting_empty_directories)**



---

archive/issue_comments_269115.json:
```json
{
    "body": "Commit: **[54af37e](https://github.com/sagemath/sagetrac-mirror/commit/54af37ebf9f9aa1c8349feee4410dcb14e252da1)**",
    "created_at": "2015-07-29T18:13:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18967#issuecomment-269115",
    "user": "https://github.com/jhpalmieri"
}
```

Commit: **[54af37e](https://github.com/sagemath/sagetrac-mirror/commit/54af37ebf9f9aa1c8349feee4410dcb14e252da1)**



---

archive/issue_comments_269116.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nActually, let's clean up the empty directories at both the end (for general clean-up) and the beginning (in case docbuilding was interrupted before clean-up).\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/54af37ebf9f9aa1c8349feee4410dcb14e252da1\">54af37e</a></td><td><code>trac 18967: Silence messages about deleting empty directories during docbuilding,</code></td></tr></table>\n",
    "created_at": "2015-07-29T18:13:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18967#issuecomment-269116",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">Comment 3</div>

Actually, let's clean up the empty directories at both the end (for general clean-up) and the beginning (in case docbuilding was interrupted before clean-up).

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/54af37ebf9f9aa1c8349feee4410dcb14e252da1">54af37e</a></td><td><code>trac 18967: Silence messages about deleting empty directories during docbuilding,</code></td></tr></table>




---

archive/issue_comments_269117.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-07-29T18:28:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18967#issuecomment-269117",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_251508.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-29T18:28:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18967#event-251508"
}
```



---

archive/issue_events_251509.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-29T18:28:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18967#event-251509"
}
```



---

archive/issue_events_251510.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-31T17:52:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18967#event-251510"
}
```



---

archive/issue_events_251511.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9559da63a8d2917b6187d17d441bfd31f8a876b0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-07-31T17:52:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18967#event-251511"
}
```



---

archive/issue_comments_269118.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/silence_deleting_empty_directories](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/silence_deleting_empty_directories)** to **[54af37e](https://github.com/sagemath/sagetrac-mirror/commit/54af37ebf9f9aa1c8349feee4410dcb14e252da1)**",
    "created_at": "2015-07-31T17:52:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18967#issuecomment-269118",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/silence_deleting_empty_directories](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/silence_deleting_empty_directories)** to **[54af37e](https://github.com/sagemath/sagetrac-mirror/commit/54af37ebf9f9aa1c8349feee4410dcb14e252da1)**
