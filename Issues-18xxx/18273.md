# Issue 18273: SymmetricGroup(...).algebra(..., category=...) for consistency

archive/issues_018036.json:
```json
{
    "assignees": [],
    "body": "The generic algebra method takes a category option. This option is missing for `SymmetricGroup` and `Permutations` which use `SymmetricGroupAlgebra`. This ticket fixes this.\n\nCC:  @tscrim @darijgr @sagetrac-sage-combinat @avirmaux\n\nBranch/Commit: **[db95e00](https://github.com/sagemath/sagetrac-mirror/commit/db95e000594f8997731f92f11683cbf9c1241aca)**\n\nReviewer: **Aladin Virmaux**\n\nAuthor: **Nicolas M. Thi\u00e9ry**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18273_\n\n",
    "closed_at": "2015-04-23T03:21:54Z",
    "created_at": "2015-04-22T09:34:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SymmetricGroup(...).algebra(..., category=...) for consistency",
    "type": "issue",
    "updated_at": "2015-04-23T03:21:54Z",
    "url": "https://github.com/sagemath/sage/issues/18273",
    "user": "https://github.com/nthiery"
}
```
The generic algebra method takes a category option. This option is missing for `SymmetricGroup` and `Permutations` which use `SymmetricGroupAlgebra`. This ticket fixes this.

CC:  @tscrim @darijgr @sagetrac-sage-combinat @avirmaux

Branch/Commit: **[db95e00](https://github.com/sagemath/sagetrac-mirror/commit/db95e000594f8997731f92f11683cbf9c1241aca)**

Reviewer: **Aladin Virmaux**

Author: **Nicolas M. Thiéry**

_Issue created by migration from https://trac.sagemath.org/ticket/18273_





---

archive/issue_events_242140.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-04-22T09:34:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18273#event-242140"
}
```



---

archive/issue_events_242141.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-04-22T09:34:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18273#event-242141"
}
```



---

archive/issue_events_242142.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-04-22T09:34:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18273#event-242142"
}
```



---

archive/issue_events_242143.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-04-22T09:34:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18273#event-242143"
}
```



---

archive/issue_comments_253240.json:
```json
{
    "body": "Branch: **[u/nthiery/categories/symmetricgroup_algebra_category_option-18273](https://github.com/sagemath/sagetrac-mirror/tree/u/nthiery/categories/symmetricgroup_algebra_category_option-18273)**",
    "created_at": "2015-04-22T10:08:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18273#issuecomment-253240",
    "user": "https://github.com/nthiery"
}
```

Branch: **[u/nthiery/categories/symmetricgroup_algebra_category_option-18273](https://github.com/sagemath/sagetrac-mirror/tree/u/nthiery/categories/symmetricgroup_algebra_category_option-18273)**



---

archive/issue_comments_253241.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db95e000594f8997731f92f11683cbf9c1241aca\">db95e00</a></td><td><code>18273: add category option to SymmetricGroup.algebra and Permutations.algebra for consistency</code></td></tr></table>\n",
    "created_at": "2015-04-22T10:08:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18273#issuecomment-253241",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db95e000594f8997731f92f11683cbf9c1241aca">db95e00</a></td><td><code>18273: add category option to SymmetricGroup.algebra and Permutations.algebra for consistency</code></td></tr></table>




---

archive/issue_comments_253242.json:
```json
{
    "body": "Commit: **[db95e00](https://github.com/sagemath/sagetrac-mirror/commit/db95e000594f8997731f92f11683cbf9c1241aca)**",
    "created_at": "2015-04-22T10:08:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18273#issuecomment-253242",
    "user": "https://github.com/nthiery"
}
```

Commit: **[db95e00](https://github.com/sagemath/sagetrac-mirror/commit/db95e000594f8997731f92f11683cbf9c1241aca)**



---

archive/issue_events_242144.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-04-22T10:08:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18273#event-242144"
}
```



---

archive/issue_comments_253243.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThis should be a rather trivial review. Thanks in advance!",
    "created_at": "2015-04-22T10:08:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18273#issuecomment-253243",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:3" align="right">Comment 3</div>

This should be a rather trivial review. Thanks in advance!



---

archive/issue_comments_253244.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLooks good to me.",
    "created_at": "2015-04-22T10:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18273#issuecomment-253244",
    "user": "https://github.com/avirmaux"
}
```

<div id="comment:5" align="right">Comment 5</div>

Looks good to me.



---

archive/issue_events_242145.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-04-22T10:57:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18273#event-242145"
}
```



---

archive/issue_events_242146.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-04-22T10:57:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18273#event-242146"
}
```



---

archive/issue_comments_253245.json:
```json
{
    "body": "Changed reviewer from **Darij Grinberg** to **Aladin Virmaux**",
    "created_at": "2015-04-22T11:30:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18273#issuecomment-253245",
    "user": "https://github.com/nthiery"
}
```

Changed reviewer from **Darij Grinberg** to **Aladin Virmaux**



---

archive/issue_comments_253246.json:
```json
{
    "body": "Changed branch from **[u/nthiery/categories/symmetricgroup_algebra_category_option-18273](https://github.com/sagemath/sagetrac-mirror/tree/u/nthiery/categories/symmetricgroup_algebra_category_option-18273)** to **[db95e00](https://github.com/sagemath/sagetrac-mirror/commit/db95e000594f8997731f92f11683cbf9c1241aca)**",
    "created_at": "2015-04-23T03:21:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18273#issuecomment-253246",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/nthiery/categories/symmetricgroup_algebra_category_option-18273](https://github.com/sagemath/sagetrac-mirror/tree/u/nthiery/categories/symmetricgroup_algebra_category_option-18273)** to **[db95e00](https://github.com/sagemath/sagetrac-mirror/commit/db95e000594f8997731f92f11683cbf9c1241aca)**



---

archive/issue_events_242147.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-23T03:21:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18273#event-242147"
}
```



---

archive/issue_events_242148.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9c013fbc9dc1074d876f9e0f82c216de5ab651b1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-04-23T03:21:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18273",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18273#event-242148"
}
```
