...
```json
{
    "body": "OK I will check all of these with `readelf` in sage-on-gentoo. Since It is all compiled with `-as-needed` we'll definitely see if anything needs cblas directly. If the dependency is indirect will it break on cygwin?",
...
```json
{
    "body": "All these only link to `libc` and `libpython` which I could have seen straight away from `module_list.py`. We definitely want that simplification. I cannot see how we would introduce side effects by removing these linkings.",
