# Issue 18296: Emphasize that Graph.subgraph_search ignores edge labels

archive/issues_018059.json:
```json
{
    "assignees": [],
    "body": "A post on sage-support [1] reported that the `subgraph_search` functions ignored edge labels, while the doc apparently claims the opposite.\n\nIndeed, the common terminology of \"labelled copies\" and \"unlabelled copies\" is pretty misleading with respect to actual labels on the edge/vertices of the graph. With this branch, I hope to make this mistake less likely.\n\nNathann\n\n[1] https://groups.google.com/d/topic/sage-support/CTeSnBZN7Sc/discussion\n\n\n\nCC:  @sagetrac-tmonteil @videlec @dcoudert\n\nBranch/Commit: **[87d0324](https://github.com/sagemath/sagetrac-mirror/commit/87d03249071edf4b2907304f77093c8cd8849f0b)**\n\nReviewer: **David Coudert**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18296_\n\n",
    "closed_at": "2015-05-06T23:57:43Z",
    "created_at": "2015-04-25T07:37:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Emphasize that Graph.subgraph_search ignores edge labels",
    "type": "issue",
    "updated_at": "2015-05-06T23:57:43Z",
    "url": "https://github.com/sagemath/sage/issues/18296",
    "user": "https://github.com/nathanncohen"
}
```
A post on sage-support [1] reported that the `subgraph_search` functions ignored edge labels, while the doc apparently claims the opposite.

Indeed, the common terminology of "labelled copies" and "unlabelled copies" is pretty misleading with respect to actual labels on the edge/vertices of the graph. With this branch, I hope to make this mistake less likely.

Nathann

[1] https://groups.google.com/d/topic/sage-support/CTeSnBZN7Sc/discussion



CC:  @sagetrac-tmonteil @videlec @dcoudert

Branch/Commit: **[87d0324](https://github.com/sagemath/sagetrac-mirror/commit/87d03249071edf4b2907304f77093c8cd8849f0b)**

Reviewer: **David Coudert**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/18296_





---

archive/issue_events_242414.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-25T07:37:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242414"
}
```



---

archive/issue_events_242415.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-25T07:37:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242415"
}
```



---

archive/issue_events_242416.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-25T07:37:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242416"
}
```



---

archive/issue_events_242417.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-25T07:37:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242417"
}
```



---

archive/issue_comments_253810.json:
```json
{
    "body": "Branch: **[public/18296](https://github.com/sagemath/sagetrac-mirror/tree/public/18296)**",
    "created_at": "2015-04-25T07:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253810",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[public/18296](https://github.com/sagemath/sagetrac-mirror/tree/public/18296)**



---

archive/issue_comments_253811.json:
```json
{
    "body": "Commit: **[52ccff9](https://github.com/sagemath/sagetrac-mirror/commit/52ccff914d2f9e7836ab8cd36afe60724734e8b3)**",
    "created_at": "2015-04-25T07:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253811",
    "user": "https://github.com/nathanncohen"
}
```

Commit: **[52ccff9](https://github.com/sagemath/sagetrac-mirror/commit/52ccff914d2f9e7836ab8cd36afe60724734e8b3)**



---

archive/issue_events_242418.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-25T07:45:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242418"
}
```



---

archive/issue_comments_253812.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/52ccff914d2f9e7836ab8cd36afe60724734e8b3\">52ccff9</a></td><td><code>trac #18296: Emphasize that Graph.subgraph_search ignores edge labels</code></td></tr></table>\n",
    "created_at": "2015-04-25T07:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253812",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/52ccff914d2f9e7836ab8cd36afe60724734e8b3">52ccff9</a></td><td><code>trac #18296: Emphasize that Graph.subgraph_search ignores edge labels</code></td></tr></table>




---

archive/issue_comments_253813.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-A post on sage-support [1] reported that the `subgraph_search` functions ignored edge labels, despite claiming the opposite.\n+A post on sage-support [1] reported that the `subgraph_search` functions ignored edge labels, while the doc apparently claims the opposite.\n \n Indeed, the common terminology of \"labelled copies\" and \"unlabelled copies\" is pretty misleading with respect to actual labels on the edge/vertices of the graph. With this branch, I hope to make this mistake less likely.\n \n``````\n",
    "created_at": "2015-04-25T08:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253813",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-A post on sage-support [1] reported that the `subgraph_search` functions ignored edge labels, despite claiming the opposite.
+A post on sage-support [1] reported that the `subgraph_search` functions ignored edge labels, while the doc apparently claims the opposite.
 
 Indeed, the common terminology of "labelled copies" and "unlabelled copies" is pretty misleading with respect to actual labels on the edge/vertices of the graph. With this branch, I hope to make this mistake less likely.
 
``````




---

archive/issue_events_242419.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-04-25T21:55:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242419"
}
```



---

archive/issue_events_242420.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-04-25T21:55:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242420"
}
```



---

archive/issue_comments_253814.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\n`aglgorithm` -> `algorithm`\n\notherwise, the patch seems OK",
    "created_at": "2015-04-25T21:55:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253814",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:3" align="right">Comment 3</div>

`aglgorithm` -> `algorithm`

otherwise, the patch seems OK



---

archive/issue_comments_253815.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4c944996e14eee4b5abb07e5f1f42124a3695ab6\">4c94499</a></td><td><code>trac #18296: Emphasize that Graph.subgraph_search ignores edge labels</code></td></tr></table>\n",
    "created_at": "2015-04-25T22:15:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253815",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4c944996e14eee4b5abb07e5f1f42124a3695ab6">4c94499</a></td><td><code>trac #18296: Emphasize that Graph.subgraph_search ignores edge labels</code></td></tr></table>




---

archive/issue_comments_253816.json:
```json
{
    "body": "Changed commit from **[52ccff9](https://github.com/sagemath/sagetrac-mirror/commit/52ccff914d2f9e7836ab8cd36afe60724734e8b3)** to **[4c94499](https://github.com/sagemath/sagetrac-mirror/commit/4c944996e14eee4b5abb07e5f1f42124a3695ab6)**",
    "created_at": "2015-04-25T22:15:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253816",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[52ccff9](https://github.com/sagemath/sagetrac-mirror/commit/52ccff914d2f9e7836ab8cd36afe60724734e8b3)** to **[4c94499](https://github.com/sagemath/sagetrac-mirror/commit/4c944996e14eee4b5abb07e5f1f42124a3695ab6)**



---

archive/issue_events_242421.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-25T22:16:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242421"
}
```



---

archive/issue_events_242422.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-04-25T22:16:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242422"
}
```



---

archive/issue_comments_253817.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2015-05-05T16:47:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253817",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_events_242423.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-05-05T16:47:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242423"
}
```



---

archive/issue_events_242424.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-05-05T16:47:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242424"
}
```



---

archive/issue_comments_253818.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nFor me the patch is good to go (install, doctest, docbuild, etc.).\nDavid.",
    "created_at": "2015-05-05T16:47:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253818",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:6" align="right">Comment 6</div>

For me the patch is good to go (install, doctest, docbuild, etc.).
David.



---

archive/issue_comments_253819.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks !",
    "created_at": "2015-05-05T16:49:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253819",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks !



---

archive/issue_comments_253820.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\ndocbuild fails",
    "created_at": "2015-05-06T20:13:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253820",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:8" align="right">Comment 8</div>

docbuild fails



---

archive/issue_events_242425.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-06T20:13:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242425"
}
```



---

archive/issue_events_242426.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-06T20:13:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242426"
}
```



---

archive/issue_comments_253821.json:
```json
{
    "body": "Changed commit from **[4c94499](https://github.com/sagemath/sagetrac-mirror/commit/4c944996e14eee4b5abb07e5f1f42124a3695ab6)** to **[87d0324](https://github.com/sagemath/sagetrac-mirror/commit/87d03249071edf4b2907304f77093c8cd8849f0b)**",
    "created_at": "2015-05-06T20:30:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253821",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4c94499](https://github.com/sagemath/sagetrac-mirror/commit/4c944996e14eee4b5abb07e5f1f42124a3695ab6)** to **[87d0324](https://github.com/sagemath/sagetrac-mirror/commit/87d03249071edf4b2907304f77093c8cd8849f0b)**



---

archive/issue_comments_253822.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/93a1ece7169f3fd5bdfe40f759efabc628c3f4af\">93a1ece</a></td><td><code>trac #18296: Merged with 6.7.beta4</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/87d03249071edf4b2907304f77093c8cd8849f0b\">87d0324</a></td><td><code>trac #18296: Broken doc</code></td></tr></table>\n",
    "created_at": "2015-05-06T20:30:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253822",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/93a1ece7169f3fd5bdfe40f759efabc628c3f4af">93a1ece</a></td><td><code>trac #18296: Merged with 6.7.beta4</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/87d03249071edf4b2907304f77093c8cd8849f0b">87d0324</a></td><td><code>trac #18296: Broken doc</code></td></tr></table>




---

archive/issue_comments_253823.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nI don't see how it was caused by this branch, but indeed there is a mistake `O_o`",
    "created_at": "2015-05-06T20:30:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253823",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:10" align="right">Comment 10</div>

I don't see how it was caused by this branch, but indeed there is a mistake `O_o`



---

archive/issue_events_242427.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-05-06T20:30:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242427"
}
```



---

archive/issue_events_242428.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-05-06T20:30:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242428"
}
```



---

archive/issue_events_242429.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-06T23:57:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242429"
}
```



---

archive/issue_events_242430.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f861b79051fa77975da4d374b7205350d869e521",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-05-06T23:57:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18296#event-242430"
}
```



---

archive/issue_comments_253824.json:
```json
{
    "body": "Changed branch from **[public/18296](https://github.com/sagemath/sagetrac-mirror/tree/public/18296)** to **[87d0324](https://github.com/sagemath/sagetrac-mirror/commit/87d03249071edf4b2907304f77093c8cd8849f0b)**",
    "created_at": "2015-05-06T23:57:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18296#issuecomment-253824",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/18296](https://github.com/sagemath/sagetrac-mirror/tree/public/18296)** to **[87d0324](https://github.com/sagemath/sagetrac-mirror/commit/87d03249071edf4b2907304f77093c8cd8849f0b)**
