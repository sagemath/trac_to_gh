# Issue 18702: Simplify generation of smallgraphs

archive/issues_018465.json:
```json
{
    "assignees": [],
    "body": "Avoid the networkx to Graph conversion when creating some small graphs.\n\nCC:  @nathanncohen @sagetrac-borassi\n\nBranch/Commit: **[ce460ff](https://github.com/sagemath/sagetrac-mirror/commit/ce460ff8d0ef5d2c68859870a524ed09f9b93c26)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18702_\n\n",
    "closed_at": "2015-06-14T21:43:27Z",
    "created_at": "2015-06-14T17:00:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Simplify generation of smallgraphs",
    "type": "issue",
    "updated_at": "2015-06-14T21:43:27Z",
    "url": "https://github.com/sagemath/sage/issues/18702",
    "user": "https://github.com/dcoudert"
}
```
Avoid the networkx to Graph conversion when creating some small graphs.

CC:  @nathanncohen @sagetrac-borassi

Branch/Commit: **[ce460ff](https://github.com/sagemath/sagetrac-mirror/commit/ce460ff8d0ef5d2c68859870a524ed09f9b93c26)**

Reviewer: **Nathann Cohen**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/18702_





---

archive/issue_events_247984.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-06-14T17:00:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18702#event-247984"
}
```



---

archive/issue_events_247985.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-06-14T17:00:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18702#event-247985"
}
```



---

archive/issue_events_247986.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-06-14T17:00:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18702#event-247986"
}
```



---

archive/issue_events_247987.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-06-14T17:00:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18702#event-247987"
}
```



---

archive/issue_comments_263293.json:
```json
{
    "body": "Branch: **[public/18702](https://github.com/sagemath/sagetrac-mirror/tree/public/18702)**",
    "created_at": "2015-06-14T17:01:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18702#issuecomment-263293",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/18702](https://github.com/sagemath/sagetrac-mirror/tree/public/18702)**



---

archive/issue_comments_263294.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d5400ef42261666485f04ca21cda40ebd21d9b2c\">d5400ef</a></td><td><code>trac #18702: Chvatal graph</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0c5a5aff97231dc4a68b24caa6d92d346d22281b\">0c5a5af</a></td><td><code>trac #18702: doctest for Chvatal graph</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/12388606bdc6fc22934b903ca69ce1764363640f\">1238860</a></td><td><code>trac #18702: Frucht Graph</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6e6c1be23ceca5156aa45b5e0a007c3f40e24e28\">6e6c1be</a></td><td><code>trac #18702: Heawood Graph</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6b8f672d3ac3ec810174b3e4a87d72487942effe\">6b8f672</a></td><td><code>trac #18702: Krackhardt Kite Graph</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ce460ff8d0ef5d2c68859870a524ed09f9b93c26\">ce460ff</a></td><td><code>trac #18702: Thomsen Graph</code></td></tr></table>\n",
    "created_at": "2015-06-14T17:35:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18702#issuecomment-263294",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d5400ef42261666485f04ca21cda40ebd21d9b2c">d5400ef</a></td><td><code>trac #18702: Chvatal graph</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0c5a5aff97231dc4a68b24caa6d92d346d22281b">0c5a5af</a></td><td><code>trac #18702: doctest for Chvatal graph</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/12388606bdc6fc22934b903ca69ce1764363640f">1238860</a></td><td><code>trac #18702: Frucht Graph</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6e6c1be23ceca5156aa45b5e0a007c3f40e24e28">6e6c1be</a></td><td><code>trac #18702: Heawood Graph</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6b8f672d3ac3ec810174b3e4a87d72487942effe">6b8f672</a></td><td><code>trac #18702: Krackhardt Kite Graph</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ce460ff8d0ef5d2c68859870a524ed09f9b93c26">ce460ff</a></td><td><code>trac #18702: Thomsen Graph</code></td></tr></table>




---

archive/issue_comments_263295.json:
```json
{
    "body": "Commit: **[ce460ff](https://github.com/sagemath/sagetrac-mirror/commit/ce460ff8d0ef5d2c68859870a524ed09f9b93c26)**",
    "created_at": "2015-06-14T17:35:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18702#issuecomment-263295",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[ce460ff](https://github.com/sagemath/sagetrac-mirror/commit/ce460ff8d0ef5d2c68859870a524ed09f9b93c26)**



---

archive/issue_events_247988.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-06-14T17:38:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18702#event-247988"
}
```



---

archive/issue_comments_263296.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nShould be easy to review when you have time",
    "created_at": "2015-06-14T17:38:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18702#issuecomment-263296",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:3" align="right">Comment 3</div>

Should be easy to review when you have time



---

archive/issue_comments_263297.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nGood to go!\n\nNathann",
    "created_at": "2015-06-14T18:10:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18702#issuecomment-263297",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

Good to go!

Nathann



---

archive/issue_events_247989.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-06-14T18:10:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18702#event-247989"
}
```



---

archive/issue_events_247990.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-06-14T18:10:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18702#event-247990"
}
```



---

archive/issue_comments_263298.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2015-06-14T18:10:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18702#issuecomment-263298",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_263299.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThanks!",
    "created_at": "2015-06-14T18:29:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18702#issuecomment-263299",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thanks!



---

archive/issue_events_247991.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-06-14T21:43:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18702#event-247991"
}
```



---

archive/issue_events_247992.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "50db962dadfe2c94e4c7fdcb1a23c8b18ab8f90c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-06-14T21:43:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18702#event-247992"
}
```



---

archive/issue_comments_263300.json:
```json
{
    "body": "Changed branch from **[public/18702](https://github.com/sagemath/sagetrac-mirror/tree/public/18702)** to **[ce460ff](https://github.com/sagemath/sagetrac-mirror/commit/ce460ff8d0ef5d2c68859870a524ed09f9b93c26)**",
    "created_at": "2015-06-14T21:43:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18702#issuecomment-263300",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/18702](https://github.com/sagemath/sagetrac-mirror/tree/public/18702)** to **[ce460ff](https://github.com/sagemath/sagetrac-mirror/commit/ce460ff8d0ef5d2c68859870a524ed09f9b93c26)**
