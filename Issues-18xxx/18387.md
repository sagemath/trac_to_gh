# Issue 18387: Use type(x) instead of x.__class__ in Cython

archive/issues_018150.json:
```json
{
    "assignees": [],
    "body": "In cython code, replace\n\n```\ncdef type t = x.__class__\n```\nby\n\n```\ncdef type t = type(x)\n```\nFor Cython extension types, we know that `type(x)` is `x.__class__`, so this code should be equivalent, but `type(x)` is a lot faster because Cython optimizes it.\n\nBranch/Commit: **[63d29a3](https://github.com/sagemath/sagetrac-mirror/commit/63d29a34206ec89c81f71147c44d32de7b8583a5)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18387_\n\n",
    "closed_at": "2015-05-13T11:33:50Z",
    "created_at": "2015-05-09T08:57:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/performance",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Use type(x) instead of x.__class__ in Cython",
    "type": "issue",
    "updated_at": "2015-05-13T11:33:50Z",
    "url": "https://github.com/sagemath/sage/issues/18387",
    "user": "https://github.com/jdemeyer"
}
```
In cython code, replace

```
cdef type t = x.__class__
```
by

```
cdef type t = type(x)
```
For Cython extension types, we know that `type(x)` is `x.__class__`, so this code should be equivalent, but `type(x)` is a lot faster because Cython optimizes it.

Branch/Commit: **[63d29a3](https://github.com/sagemath/sagetrac-mirror/commit/63d29a34206ec89c81f71147c44d32de7b8583a5)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/18387_





---

archive/issue_events_243634.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-05-09T08:57:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243634"
}
```



---

archive/issue_events_243635.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-05-09T08:57:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "label": "https://github.com/sagemath/sage/labels/performance",
    "label_color": "696969",
    "label_name": "performance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243635"
}
```



---

archive/issue_events_243636.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-05-09T08:57:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243636"
}
```



---

archive/issue_events_243637.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-05-09T08:57:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243637"
}
```



---

archive/issue_events_243638.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-05-09T09:02:10Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "title_is": "Use type(x) instead of x.__class__ in Cython",
    "title_was": "Use type(x) instead of x.__class__ when assigning to cdef type",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243638"
}
```



---

archive/issue_comments_255618.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,11 +1,11 @@\n In cython code, replace\n \n ```\n-cdef type t = x.__class__\n+self.__class__\n ```\n by\n \n ```\n-cdef type t = type(x)\n+type(self)\n ```\n For Cython extension types, we know that `type(x)` is `x.__class__`, so this code should be equivalent, but `type(x)` is a lot faster because Cython optimizes it.\n``````\n",
    "created_at": "2015-05-09T09:02:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18387#issuecomment-255618",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,11 +1,11 @@
 In cython code, replace
 
 ```
-cdef type t = x.__class__
+self.__class__
 ```
 by
 
 ```
-cdef type t = type(x)
+type(self)
 ```
 For Cython extension types, we know that `type(x)` is `x.__class__`, so this code should be equivalent, but `type(x)` is a lot faster because Cython optimizes it.
``````




---

archive/issue_comments_255619.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,11 +1,11 @@\n In cython code, replace\n \n ```\n-self.__class__\n+cdef type t = x.__class__\n ```\n by\n \n ```\n-type(self)\n+cdef type t = type(x)\n ```\n For Cython extension types, we know that `type(x)` is `x.__class__`, so this code should be equivalent, but `type(x)` is a lot faster because Cython optimizes it.\n``````\n",
    "created_at": "2015-05-09T09:37:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18387#issuecomment-255619",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,11 +1,11 @@
 In cython code, replace
 
 ```
-self.__class__
+cdef type t = x.__class__
 ```
 by
 
 ```
-type(self)
+cdef type t = type(x)
 ```
 For Cython extension types, we know that `type(x)` is `x.__class__`, so this code should be equivalent, but `type(x)` is a lot faster because Cython optimizes it.
``````




---

archive/issue_comments_255620.json:
```json
{
    "body": "Branch: **[u/jdemeyer/use_type_x__instead_of_x___class___when_assigning_to_cdef_type](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/use_type_x__instead_of_x___class___when_assigning_to_cdef_type)**",
    "created_at": "2015-05-09T09:48:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18387#issuecomment-255620",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/use_type_x__instead_of_x___class___when_assigning_to_cdef_type](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/use_type_x__instead_of_x___class___when_assigning_to_cdef_type)**



---

archive/issue_events_243639.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-05-09T09:50:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243639"
}
```



---

archive/issue_comments_255621.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/63d29a34206ec89c81f71147c44d32de7b8583a5\">63d29a3</a></td><td><code>Replace x.__class__ by type(x)</code></td></tr></table>\n",
    "created_at": "2015-05-09T09:50:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18387#issuecomment-255621",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/63d29a34206ec89c81f71147c44d32de7b8583a5">63d29a3</a></td><td><code>Replace x.__class__ by type(x)</code></td></tr></table>




---

archive/issue_comments_255622.json:
```json
{
    "body": "Commit: **[63d29a3](https://github.com/sagemath/sagetrac-mirror/commit/63d29a34206ec89c81f71147c44d32de7b8583a5)**",
    "created_at": "2015-05-09T09:50:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18387#issuecomment-255622",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[63d29a3](https://github.com/sagemath/sagetrac-mirror/commit/63d29a34206ec89c81f71147c44d32de7b8583a5)**



---

archive/issue_events_243640.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-05-09T17:57:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243640"
}
```



---

archive/issue_events_243641.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-05-09T17:57:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243641"
}
```



---

archive/issue_comments_255623.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLGTM.",
    "created_at": "2015-05-09T17:57:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18387#issuecomment-255623",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
LGTM.



---

archive/issue_comments_255624.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2015-05-09T17:57:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18387#issuecomment-255624",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_243642.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-13T11:33:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243642"
}
```



---

archive/issue_events_243643.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f86b7b04ea003b8ef1c469fb5071af6a980f14f9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-05-13T11:33:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18387#event-243643"
}
```



---

archive/issue_comments_255625.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/use_type_x__instead_of_x___class___when_assigning_to_cdef_type](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/use_type_x__instead_of_x___class___when_assigning_to_cdef_type)** to **[63d29a3](https://github.com/sagemath/sagetrac-mirror/commit/63d29a34206ec89c81f71147c44d32de7b8583a5)**",
    "created_at": "2015-05-13T11:33:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18387#issuecomment-255625",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/use_type_x__instead_of_x___class___when_assigning_to_cdef_type](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/use_type_x__instead_of_x___class___when_assigning_to_cdef_type)** to **[63d29a3](https://github.com/sagemath/sagetrac-mirror/commit/63d29a34206ec89c81f71147c44d32de7b8583a5)**
