# Issue 18939: allow conversion of Arc objects to Bezier paths

archive/issues_018702.json:
```json
{
    "assignees": [],
    "body": "As a first step towards hyperbolic polygons in the Poincar\u00e9 disk model,\nlet us introduce the possibility to convert Arc objects to Bezier paths.\n\nAnother ticket will use this for hyperbolic polygons.\n\nKeywords: **arc bezier path plot**\n\nBranch/Commit: **[e94cf83](https://github.com/sagemath/sagetrac-mirror/commit/e94cf83e5699cca28746cdf70f201f3080440b18)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18939_\n\n",
    "closed_at": "2015-10-12T07:16:12Z",
    "created_at": "2015-07-22T15:18:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "allow conversion of Arc objects to Bezier paths",
    "type": "issue",
    "updated_at": "2015-10-12T07:16:12Z",
    "url": "https://github.com/sagemath/sage/issues/18939",
    "user": "https://github.com/fchapoton"
}
```
As a first step towards hyperbolic polygons in the Poincaré disk model,
let us introduce the possibility to convert Arc objects to Bezier paths.

Another ticket will use this for hyperbolic polygons.

Keywords: **arc bezier path plot**

Branch/Commit: **[e94cf83](https://github.com/sagemath/sagetrac-mirror/commit/e94cf83e5699cca28746cdf70f201f3080440b18)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/18939_





---

archive/issue_events_251089.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-22T15:18:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251089"
}
```



---

archive/issue_events_251090.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-22T15:18:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251090"
}
```



---

archive/issue_events_251091.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-22T15:18:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251091"
}
```



---

archive/issue_events_251092.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-22T15:18:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251092"
}
```



---

archive/issue_comments_268473.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2015-07-22T15:20:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268473",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_268474.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1eb53c38e523f7850b27f8d0e6d07c0e013b7e6f\">1eb53c3</a></td><td><code>adding conversion from Arc class to Bezier paths.</code></td></tr></table>\n",
    "created_at": "2015-07-22T15:20:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268474",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1eb53c38e523f7850b27f8d0e6d07c0e013b7e6f">1eb53c3</a></td><td><code>adding conversion from Arc class to Bezier paths.</code></td></tr></table>




---

archive/issue_comments_268475.json:
```json
{
    "body": "Branch: **[u/chapoton/18939](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/18939)**",
    "created_at": "2015-07-22T15:20:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268475",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/18939](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/18939)**



---

archive/issue_comments_268476.json:
```json
{
    "body": "Commit: **[1eb53c3](https://github.com/sagemath/sagetrac-mirror/commit/1eb53c38e523f7850b27f8d0e6d07c0e013b7e6f)**",
    "created_at": "2015-07-22T15:20:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268476",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[1eb53c3](https://github.com/sagemath/sagetrac-mirror/commit/1eb53c38e523f7850b27f8d0e6d07c0e013b7e6f)**



---

archive/issue_events_251093.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-22T15:21:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251093"
}
```



---

archive/issue_comments_268477.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nno working correctly, proportions are not correct",
    "created_at": "2015-07-23T08:52:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268477",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
no working correctly, proportions are not correct



---

archive/issue_events_251094.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-23T08:52:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251094"
}
```



---

archive/issue_events_251095.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-23T08:52:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251095"
}
```



---

archive/issue_comments_268478.json:
```json
{
    "body": "Changed commit from **[1eb53c3](https://github.com/sagemath/sagetrac-mirror/commit/1eb53c38e523f7850b27f8d0e6d07c0e013b7e6f)** to **[b262d97](https://github.com/sagemath/sagetrac-mirror/commit/b262d972aa792e70bb0f3c1afded2aaeb102dd77)**",
    "created_at": "2015-07-23T12:06:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268478",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1eb53c3](https://github.com/sagemath/sagetrac-mirror/commit/1eb53c38e523f7850b27f8d0e6d07c0e013b7e6f)** to **[b262d97](https://github.com/sagemath/sagetrac-mirror/commit/b262d972aa792e70bb0f3c1afded2aaeb102dd77)**



---

archive/issue_comments_268479.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/04d61196d4b7a148caa7c31f9af7cb692bc64191\">04d6119</a></td><td><code>Merge branch 'u/chapoton/18939' of trac.sagemath.org:sage into 6.8.rc1</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b262d972aa792e70bb0f3c1afded2aaeb102dd77\">b262d97</a></td><td><code>trac #18939 now with correct options and proportions + fully pep8/pyflakes ok</code></td></tr></table>\n",
    "created_at": "2015-07-23T12:06:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268479",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/04d61196d4b7a148caa7c31f9af7cb692bc64191">04d6119</a></td><td><code>Merge branch 'u/chapoton/18939' of trac.sagemath.org:sage into 6.8.rc1</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b262d972aa792e70bb0f3c1afded2aaeb102dd77">b262d97</a></td><td><code>trac #18939 now with correct options and proportions + fully pep8/pyflakes ok</code></td></tr></table>




---

archive/issue_comments_268480.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThis is now working, I think.\n\nI have taken the opportunity to clean up the unique changed file, putting everything into pep8 format and pyflakes standard.\n\nBack to needs review.",
    "created_at": "2015-07-23T12:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268480",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
This is now working, I think.

I have taken the opportunity to clean up the unique changed file, putting everything into pep8 format and pyflakes standard.

Back to needs review.



---

archive/issue_comments_268481.json:
```json
{
    "body": "Changed keywords from **** to **arc bezier path plot**",
    "created_at": "2015-07-23T12:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268481",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **arc bezier path plot**



---

archive/issue_comments_268482.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,4 @@\n-As a first step towards hyperbolic polygons in the Poincare disk model,\n+As a first step towards hyperbolic polygons in the Poincar\u00e9 disk model,\n let us introduce the possibility to convert Arc objects to Bezier paths.\n+\n+Another ticket will use this for hyperbolic polygons.\n``````\n",
    "created_at": "2015-07-23T12:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268482",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,4 @@
-As a first step towards hyperbolic polygons in the Poincare disk model,
+As a first step towards hyperbolic polygons in the Poincaré disk model,
 let us introduce the possibility to convert Arc objects to Bezier paths.
+
+Another ticket will use this for hyperbolic polygons.
``````




---

archive/issue_events_251096.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-23T12:11:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251096"
}
```



---

archive/issue_events_251097.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-23T12:11:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251097"
}
```



---

archive/issue_events_251098.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-29T06:38:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251098"
}
```



---

archive/issue_events_251099.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-29T06:38:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251099"
}
```



---

archive/issue_comments_268483.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2015-10-07T22:03:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268483",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_268484.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nPositive review modulo making the alpha non-zero so we can see the drawn line.",
    "created_at": "2015-10-07T22:03:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268484",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:7'>Comment 7:</a>
Positive review modulo making the alpha non-zero so we can see the drawn line.



---

archive/issue_comments_268485.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/de604f0cd94031931e0def3a319b7ccb80f1d892\">de604f0</a></td><td><code> Merge branch 'u/chapoton/18939' into 6.9.rc3</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e94cf83e5699cca28746cdf70f201f3080440b18\">e94cf83</a></td><td><code>trac #18939 turn on visibility of example (alpha=1)</code></td></tr></table>\n",
    "created_at": "2015-10-08T07:46:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268485",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:8'>Comment 8:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/de604f0cd94031931e0def3a319b7ccb80f1d892">de604f0</a></td><td><code> Merge branch 'u/chapoton/18939' into 6.9.rc3</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e94cf83e5699cca28746cdf70f201f3080440b18">e94cf83</a></td><td><code>trac #18939 turn on visibility of example (alpha=1)</code></td></tr></table>




---

archive/issue_comments_268486.json:
```json
{
    "body": "Changed commit from **[b262d97](https://github.com/sagemath/sagetrac-mirror/commit/b262d972aa792e70bb0f3c1afded2aaeb102dd77)** to **[e94cf83](https://github.com/sagemath/sagetrac-mirror/commit/e94cf83e5699cca28746cdf70f201f3080440b18)**",
    "created_at": "2015-10-08T07:46:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268486",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b262d97](https://github.com/sagemath/sagetrac-mirror/commit/b262d972aa792e70bb0f3c1afded2aaeb102dd77)** to **[e94cf83](https://github.com/sagemath/sagetrac-mirror/commit/e94cf83e5699cca28746cdf70f201f3080440b18)**



---

archive/issue_comments_268487.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nok, I did set alpha to 1 in the example.\n\nThanks Travis.",
    "created_at": "2015-10-08T07:49:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268487",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'>Comment 9:</a>
ok, I did set alpha to 1 in the example.

Thanks Travis.



---

archive/issue_events_251100.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-08T07:49:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251100"
}
```



---

archive/issue_events_251101.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-08T07:49:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251101"
}
```



---

archive/issue_comments_268488.json:
```json
{
    "body": "Changed branch from **[u/chapoton/18939](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/18939)** to **[e94cf83](https://github.com/sagemath/sagetrac-mirror/commit/e94cf83e5699cca28746cdf70f201f3080440b18)**",
    "created_at": "2015-10-12T07:16:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18939#issuecomment-268488",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/18939](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/18939)** to **[e94cf83](https://github.com/sagemath/sagetrac-mirror/commit/e94cf83e5699cca28746cdf70f201f3080440b18)**



---

archive/issue_events_251102.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-10-12T07:16:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251102"
}
```



---

archive/issue_events_251103.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1afc4071dfcd493ffca3f72facaf2fa39a357c25",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-10-12T07:16:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18939",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18939#event-251103"
}
```
