# Issue 18111: Polynomial ring reverse: Consistency in optional degree-at-which-to-reverse keyword

archive/issues_017874.json:
```json
{
    "assignees": [],
    "body": "Currently we have\n\n```\n        sage: Fx.<x> = GF(2)[]\n        sage: p = x + 1\n        sage: p.reverse(degree=5)\n        x^5 + x^4\n```\n\nHowever:\n\n```\n        sage: Qx.<x> = QQ[]\n        sage: p = x + 2\n        sage: p.reverse(n=5)\n        2*x^4 + x^3\n```\n\nSo not only is the name of the optional arguments different, it also has subtly different semantics.\n\nThe optional argument should be the same, and `degree` makes most sense, and the behaviour should be the same.\n\nOverview of some other base rings:\n\n* `ZZ` is like `GF(2)`\n* `RR` takes NO keyword\n* `CC` is like `GF(2)`\n* `GF(p^m)` is like `GF(2)`\n* generic base ring is like `GF(2)`\n\nAre there other to check?\n\n\nKeywords: **polynomial, sd66**\n\nComponent: **algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18111_\n\n",
    "created_at": "2015-04-02T15:15:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Polynomial ring reverse: Consistency in optional degree-at-which-to-reverse keyword",
    "type": "issue",
    "updated_at": "2022-12-29T01:31:15Z",
    "url": "https://github.com/sagemath/sage/issues/18111",
    "user": "https://github.com/johanrosenkilde"
}
```
Currently we have

```
        sage: Fx.<x> = GF(2)[]
        sage: p = x + 1
        sage: p.reverse(degree=5)
        x^5 + x^4
```

However:

```
        sage: Qx.<x> = QQ[]
        sage: p = x + 2
        sage: p.reverse(n=5)
        2*x^4 + x^3
```

So not only is the name of the optional arguments different, it also has subtly different semantics.

The optional argument should be the same, and `degree` makes most sense, and the behaviour should be the same.

Overview of some other base rings:

* `ZZ` is like `GF(2)`
* `RR` takes NO keyword
* `CC` is like `GF(2)`
* `GF(p^m)` is like `GF(2)`
* generic base ring is like `GF(2)`

Are there other to check?


Keywords: **polynomial, sd66**

Component: **algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/18111_





---

archive/issue_events_257560.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2015-04-02T15:15:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18111",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18111#event-257560"
}
```



---

archive/issue_events_257561.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2015-04-02T15:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18111",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18111#event-257561"
}
```



---

archive/issue_events_257562.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2015-04-02T15:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18111",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18111#event-257562"
}
```



---

archive/issue_events_257563.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2015-04-02T15:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18111",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18111#event-257563"
}
```



---

archive/issue_comments_247168.json:
```json
{
    "body": "Changed keywords from **polynomial, sagedays66** to **polynomial, sd66**",
    "created_at": "2015-04-03T08:19:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18111#issuecomment-247168",
    "user": "https://github.com/videlec"
}
```

Changed keywords from **polynomial, sagedays66** to **polynomial, sd66**



---

archive/issue_events_257564.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:31:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18111",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18111#event-257564"
}
```
