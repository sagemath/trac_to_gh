...
```json
{
    "body": "<a id='comment:6'></a>Interestingly, neither the first snippet in the description nor the BOOM snippet in comment:1 will crash on 6.7beta1/OpenSuSE. What machine/system is that, Viviane/buck/nbruin?",
...
<a id='comment:6'></a>Interestingly, neither the first snippet in the description nor the BOOM snippet in comment:1 will crash on 6.7beta1/OpenSuSE. What machine/system is that, Viviane/buck/nbruin?
...
<a id='comment:12'></a>Replying to [comment:11 buck]:
> I also get no segfault in 6.7, but the numerical issue persists.
> Separate issue I suppose.
...
<a id='comment:23'></a>Replying to [comment:22 buck]:
> `@`rws: Could you expand on the "two different bugs" and link them (and/or create tickets for them)?
...
<a id='comment:39'></a>Replying to [comment:38 slelievre]:
> This ticket seems to be solved now in Sage 7.3.beta9.
