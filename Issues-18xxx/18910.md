# Issue 18910: Boost minimum spanning tree

archive/issues_018673.json:
```json
{
    "assignees": [],
    "body": "Include Boost implementation of Kruskal and Prim algorithms for minimum spanning tree.\n\nDepends on #18876\nDepends on #18906\n\nCC:  @nathanncohen @dcoudert Stefan Rudi\n\nKeywords: **Boost, minimum spanning tree**\n\nBranch/Commit: **[38a817d](https://github.com/sagemath/sagetrac-mirror/commit/38a817d387325a25c62df70c6e77d7c06169f152)**\n\nReviewer: **David Coudert**\n\nAuthor: **Michele Borassi**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18910_\n\n",
    "closed_at": "2015-07-31T17:52:20Z",
    "created_at": "2015-07-16T08:36:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Boost minimum spanning tree",
    "type": "issue",
    "updated_at": "2015-07-31T17:52:20Z",
    "url": "https://github.com/sagemath/sage/issues/18910",
    "user": "https://github.com/sagetrac-borassi"
}
```
Include Boost implementation of Kruskal and Prim algorithms for minimum spanning tree.

Depends on #18876
Depends on #18906

CC:  @nathanncohen @dcoudert Stefan Rudi

Keywords: **Boost, minimum spanning tree**

Branch/Commit: **[38a817d](https://github.com/sagemath/sagetrac-mirror/commit/38a817d387325a25c62df70c6e77d7c06169f152)**

Reviewer: **David Coudert**

Author: **Michele Borassi**

_Issue created by migration from https://trac.sagemath.org/ticket/18910_





---

archive/issue_events_250672.json:
```json
{
    "actor": "https://github.com/sagetrac-borassi",
    "created_at": "2015-07-16T08:36:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250672"
}
```



---

archive/issue_events_250673.json:
```json
{
    "actor": "https://github.com/sagetrac-borassi",
    "created_at": "2015-07-16T08:36:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250673"
}
```



---

archive/issue_comments_267521.json:
```json
{
    "body": "Author: **Michele Borassi**",
    "created_at": "2015-07-16T08:39:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267521",
    "user": "https://github.com/sagetrac-borassi"
}
```

Author: **Michele Borassi**



---

archive/issue_comments_267522.json:
```json
{
    "body": "Dependencies: **#18876, #18906**",
    "created_at": "2015-07-16T08:39:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267522",
    "user": "https://github.com/sagetrac-borassi"
}
```

Dependencies: **#18876, #18906**



---

archive/issue_events_250674.json:
```json
{
    "actor": "https://github.com/sagetrac-borassi",
    "created_at": "2015-07-16T08:39:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250674"
}
```



---

archive/issue_comments_267523.json:
```json
{
    "body": "Changed keywords from **** to **Boost, minimum spanning tree**",
    "created_at": "2015-07-16T08:39:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267523",
    "user": "https://github.com/sagetrac-borassi"
}
```

Changed keywords from **** to **Boost, minimum spanning tree**



---

archive/issue_events_250675.json:
```json
{
    "actor": "https://github.com/sagetrac-borassi",
    "created_at": "2015-07-16T08:39:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250675"
}
```



---

archive/issue_comments_267524.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Include Boost implementation of Kruskal and Prim algorithms for minimum spanning tree.\n``````\n",
    "created_at": "2015-07-16T08:39:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267524",
    "user": "https://github.com/sagetrac-borassi"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Include Boost implementation of Kruskal and Prim algorithms for minimum spanning tree.
``````




---

archive/issue_comments_267525.json:
```json
{
    "body": "Branch: **[u/borassi/boost_minimum_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/u/borassi/boost_minimum_spanning_tree)**",
    "created_at": "2015-07-19T16:30:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267525",
    "user": "https://github.com/sagetrac-borassi"
}
```

Branch: **[u/borassi/boost_minimum_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/u/borassi/boost_minimum_spanning_tree)**



---

archive/issue_comments_267526.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nHello!\n\nThis is the first version of Boost minimum spanning tree. With this code, I'm inserting in Sage the code to interface with Boost weighted graphs. The improvement on this specific algorithm is not striking (about 2x over the old algorithm), but I still set the Boost algorithm as default.\n\nSome benchmark:\n\n```\nsage: sage: def random_weighted_graph(n, m, lower_weight = 1, upper_weight = 100): \n\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0 import random\n\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0 g = graphs.RandomGNM(n,m)\n\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0 m = g.num_edges()\n\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0 weights = [random.randint(lower_weight, upper_weight) for r in xrange(m)]\n\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0 uw_edges = g.edges()\n\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0 w_edges = [(uw_edges[i][0], uw_edges[i][1], weights[i]) for i in xrange(m)]\n\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0 return Graph(w_edges, weighted = True)\n....: \nsage: g = random_weighted_graph(20000,200000)\nsage: %timeit(g.min_spanning_tree(algorithm=\"Prim_Boost\"))\n1 loops, best of 3: 279 ms per loop\nsage: %timeit(g.min_spanning_tree(algorithm=\"Kruskal_Boost\"))\n1 loops, best of 3: 361 ms per loop\nsage: %timeit(g.min_spanning_tree(algorithm=\"Kruskal\"))\n1 loops, best of 3: 492 ms per loop\nsage: %timeit(g.min_spanning_tree(algorithm=\"NetworkX\"))\n1 loops, best of 3: 1.92 s per loop\nsage: %timeit(g.min_spanning_tree(algorithm=\"Prim_edge\"))\n1 loops, best of 3: 17.3 s per loop\nsage: %timeit(g.min_spanning_tree(algorithm=\"Prim_fringe\"))\n1 loops, best of 3: 36.1 s per loop\n```\nHope you like it!\n\nMichele",
    "created_at": "2015-07-19T16:34:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267526",
    "user": "https://github.com/sagetrac-borassi"
}
```

<a id='comment:3'>Comment 3:</a>
Hello!

This is the first version of Boost minimum spanning tree. With this code, I'm inserting in Sage the code to interface with Boost weighted graphs. The improvement on this specific algorithm is not striking (about 2x over the old algorithm), but I still set the Boost algorithm as default.

Some benchmark:

```
sage: sage: def random_weighted_graph(n, m, lower_weight = 1, upper_weight = 100): 
        import random
        g = graphs.RandomGNM(n,m)
        m = g.num_edges()
        weights = [random.randint(lower_weight, upper_weight) for r in xrange(m)]
        uw_edges = g.edges()
        w_edges = [(uw_edges[i][0], uw_edges[i][1], weights[i]) for i in xrange(m)]
        return Graph(w_edges, weighted = True)
....: 
sage: g = random_weighted_graph(20000,200000)
sage: %timeit(g.min_spanning_tree(algorithm="Prim_Boost"))
1 loops, best of 3: 279 ms per loop
sage: %timeit(g.min_spanning_tree(algorithm="Kruskal_Boost"))
1 loops, best of 3: 361 ms per loop
sage: %timeit(g.min_spanning_tree(algorithm="Kruskal"))
1 loops, best of 3: 492 ms per loop
sage: %timeit(g.min_spanning_tree(algorithm="NetworkX"))
1 loops, best of 3: 1.92 s per loop
sage: %timeit(g.min_spanning_tree(algorithm="Prim_edge"))
1 loops, best of 3: 17.3 s per loop
sage: %timeit(g.min_spanning_tree(algorithm="Prim_fringe"))
1 loops, best of 3: 36.1 s per loop
```
Hope you like it!

Michele



---

archive/issue_comments_267527.json:
```json
{
    "body": "Commit: **[8278b0b](https://github.com/sagemath/sagetrac-mirror/commit/8278b0bf8b3b4c86bdc5b543f5ca57c8b682f83a)**",
    "created_at": "2015-07-19T16:34:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267527",
    "user": "https://github.com/sagetrac-borassi"
}
```

Commit: **[8278b0b](https://github.com/sagemath/sagetrac-mirror/commit/8278b0bf8b3b4c86bdc5b543f5ca57c8b682f83a)**



---

archive/issue_events_250676.json:
```json
{
    "actor": "https://github.com/sagetrac-borassi",
    "created_at": "2015-07-21T05:58:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250676"
}
```



---

archive/issue_comments_267528.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThe weight function you use does not allow to access all possible stuff that might be stored on an edge. For instance, I may have an edge like `(u, v, {'weight':3, 'toto':4})` in which case a function like `weight_function=lambda e:e[2]['weight']` is possible. With current implementation, you assume that `weight_function((u,v))` returns the weight. Better to have `weight_function(e)` or `weight_function((u,v,l))`.\n\nAlso, could you be a bit more specific with the description of the weight function\n\n```\n- ``weight_function`` -- A function that takes an edge and returns a\n          numeric weight. If ``None`` (default), the algorithm uses the edge\n          weights, if available, otherwise it assigns weight 1 to each edge (in\n          the latter case, the output can be any spanning tree).\n```\nIn particular, the default behavior assume that edge weights are numbers, that is if `e=(u,v,l)` then `l` is a number.\n\nDavid.",
    "created_at": "2015-07-21T08:15:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267528",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:5'>Comment 5:</a>
The weight function you use does not allow to access all possible stuff that might be stored on an edge. For instance, I may have an edge like `(u, v, {'weight':3, 'toto':4})` in which case a function like `weight_function=lambda e:e[2]['weight']` is possible. With current implementation, you assume that `weight_function((u,v))` returns the weight. Better to have `weight_function(e)` or `weight_function((u,v,l))`.

Also, could you be a bit more specific with the description of the weight function

```
- ``weight_function`` -- A function that takes an edge and returns a
          numeric weight. If ``None`` (default), the algorithm uses the edge
          weights, if available, otherwise it assigns weight 1 to each edge (in
          the latter case, the output can be any spanning tree).
```
In particular, the default behavior assume that edge weights are numbers, that is if `e=(u,v,l)` then `l` is a number.

David.



---

archive/issue_comments_267529.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ceda13f0b83784adce78b527099e08e7e003c6ac\">ceda13f</a></td><td><code>Improved explanation of weight_function</code></td></tr></table>\n",
    "created_at": "2015-07-21T13:40:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267529",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ceda13f0b83784adce78b527099e08e7e003c6ac">ceda13f</a></td><td><code>Improved explanation of weight_function</code></td></tr></table>




---

archive/issue_comments_267530.json:
```json
{
    "body": "Changed commit from **[8278b0b](https://github.com/sagemath/sagetrac-mirror/commit/8278b0bf8b3b4c86bdc5b543f5ca57c8b682f83a)** to **[ceda13f](https://github.com/sagemath/sagetrac-mirror/commit/ceda13f0b83784adce78b527099e08e7e003c6ac)**",
    "created_at": "2015-07-21T13:40:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267530",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8278b0b](https://github.com/sagemath/sagetrac-mirror/commit/8278b0bf8b3b4c86bdc5b543f5ca57c8b682f83a)** to **[ceda13f](https://github.com/sagemath/sagetrac-mirror/commit/ceda13f0b83784adce78b527099e08e7e003c6ac)**



---

archive/issue_comments_267531.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nHello!\n\nyou are absolutely right: I have made the modification you asked, I have improved the explanation of the weight function, and I have added an example in the generic_graph file.\n\nThank you very much!\n\nMichele",
    "created_at": "2015-07-21T13:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267531",
    "user": "https://github.com/sagetrac-borassi"
}
```

<a id='comment:7'>Comment 7:</a>
Hello!

you are absolutely right: I have made the modification you asked, I have improved the explanation of the weight function, and I have added an example in the generic_graph file.

Thank you very much!

Michele



---

archive/issue_comments_267532.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nHello,\n\nI know that dealing with weights is not easy. However, here we have different behaviors.\n\n```\nsage: g = Graph([(0,1,{'blop':1}), (1,2,{'blop':'b'})])\nsage: g.min_spanning_tree(algorithm=\"Prim_Boost\", weight_function=lambda e:e[2]['blop'])\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n...\nTypeError: a float is required\nsage: g = Graph([(0,1,1), (1,2,'b')])\nsage: g.min_spanning_tree(algorithm=\"Prim_Boost\", weight_function=lambda e:e[2])\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n...\nTypeError: a float is required\nsage: g.min_spanning_tree(algorithm=\"Prim_Boost\")\n[(0, 1, 1), (1, 2, 'b')]\n```\n\nDavid.",
    "created_at": "2015-07-21T14:59:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267532",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:8'>Comment 8:</a>
Hello,

I know that dealing with weights is not easy. However, here we have different behaviors.

```
sage: g = Graph([(0,1,{'blop':1}), (1,2,{'blop':'b'})])
sage: g.min_spanning_tree(algorithm="Prim_Boost", weight_function=lambda e:e[2]['blop'])
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
...
TypeError: a float is required
sage: g = Graph([(0,1,1), (1,2,'b')])
sage: g.min_spanning_tree(algorithm="Prim_Boost", weight_function=lambda e:e[2])
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
...
TypeError: a float is required
sage: g.min_spanning_tree(algorithm="Prim_Boost")
[(0, 1, 1), (1, 2, 'b')]
```

David.



---

archive/issue_comments_267533.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nHello!\n\nYeah, I have some problems, too: it is difficult to decide which is the correct behavior. In your examples, I have trouble understanding which improvement you are suggesting: could you be a bit more specific?\n\nIn particular, in the first two cases, you provide a weight function which is not correct, so a TypeError is raised (if you provide a function, the algorithm raises an error if it is not correct). In the third case, your graph is not weighted:\n\n```\nsage: g = Graph([(0,1,1), (1,2,'b')])\nsage: g.weighted()\nFalse\n```\nHence, the algorithm simply says \"Since the graph is not weighted, I ignore the label and set weight 1 to each edge\".\n\nMaybe a problem occurs if the graph is weighted and no function is provided: in this case, the algorithm tries to use labels even if they are not numbers. In Boost algorithms, non-numeric weights are assumed to be 1, while in other algorithms labels are simply compared, and Python decides which is the biggest.\n\n```\nsage: g = Graph([(0,1,1), (1,2,'b')], weighted=True)\nsage: g.min_spanning_tree(algorithm=\"Prim_Boost\")\n[(0, 1, 1), (1, 2, 'b')]\nsage: g.min_spanning_tree(algorithm=\"Prim_fringe\")\n[(0, 1, 1), (1, 2, 'b')]\nsage: g.min_spanning_tree(algorithm=\"Prim_edge\")\n[(0, 1, 1), (1, 2, 'b')]\nsage: g.min_spanning_tree(algorithm=\"Kruskal\")\n[(0, 1, 1), (1, 2, 'b')]\nsage: g.min_spanning_tree(algorithm=\"Kruskal_Boost\")\n[(0, 1, 1), (1, 2, 'b')]\nsage: g.min_spanning_tree(algorithm=\"NetworkX\")\n[(0, 1, 1), (1, 2, 'b')]\n```\nIn the latter example, do you think I should only improve the explanation, which is in the INPUT section of the documentation and in the examples? Or do you think I should raise an exception if a weight is not a number?\n\nThank you very much!\n\nMichele",
    "created_at": "2015-07-21T16:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267533",
    "user": "https://github.com/sagetrac-borassi"
}
```

<a id='comment:9'>Comment 9:</a>
Hello!

Yeah, I have some problems, too: it is difficult to decide which is the correct behavior. In your examples, I have trouble understanding which improvement you are suggesting: could you be a bit more specific?

In particular, in the first two cases, you provide a weight function which is not correct, so a TypeError is raised (if you provide a function, the algorithm raises an error if it is not correct). In the third case, your graph is not weighted:

```
sage: g = Graph([(0,1,1), (1,2,'b')])
sage: g.weighted()
False
```
Hence, the algorithm simply says "Since the graph is not weighted, I ignore the label and set weight 1 to each edge".

Maybe a problem occurs if the graph is weighted and no function is provided: in this case, the algorithm tries to use labels even if they are not numbers. In Boost algorithms, non-numeric weights are assumed to be 1, while in other algorithms labels are simply compared, and Python decides which is the biggest.

```
sage: g = Graph([(0,1,1), (1,2,'b')], weighted=True)
sage: g.min_spanning_tree(algorithm="Prim_Boost")
[(0, 1, 1), (1, 2, 'b')]
sage: g.min_spanning_tree(algorithm="Prim_fringe")
[(0, 1, 1), (1, 2, 'b')]
sage: g.min_spanning_tree(algorithm="Prim_edge")
[(0, 1, 1), (1, 2, 'b')]
sage: g.min_spanning_tree(algorithm="Kruskal")
[(0, 1, 1), (1, 2, 'b')]
sage: g.min_spanning_tree(algorithm="Kruskal_Boost")
[(0, 1, 1), (1, 2, 'b')]
sage: g.min_spanning_tree(algorithm="NetworkX")
[(0, 1, 1), (1, 2, 'b')]
```
In the latter example, do you think I should only improve the explanation, which is in the INPUT section of the documentation and in the examples? Or do you think I should raise an exception if a weight is not a number?

Thank you very much!

Michele



---

archive/issue_comments_267534.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nI think it's a weird behavior to assume that an edge like `(1, 2, 'b')` has weight 1. However, it was the original behavior of the method, so I propose to keep it unchanged.\nSo don't raise exception, but if you can improve further the doc, it should be ok.\nThanks.",
    "created_at": "2015-07-21T17:07:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267534",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:10'>Comment 10:</a>
I think it's a weird behavior to assume that an edge like `(1, 2, 'b')` has weight 1. However, it was the original behavior of the method, so I propose to keep it unchanged.
So don't raise exception, but if you can improve further the doc, it should be ok.
Thanks.



---

archive/issue_comments_267535.json:
```json
{
    "body": "Changed commit from **[ceda13f](https://github.com/sagemath/sagetrac-mirror/commit/ceda13f0b83784adce78b527099e08e7e003c6ac)** to **[4bbb298](https://github.com/sagemath/sagetrac-mirror/commit/4bbb2980aab4b0a81fc07cf62afa5395ee7719c1)**",
    "created_at": "2015-07-22T11:05:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267535",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ceda13f](https://github.com/sagemath/sagetrac-mirror/commit/ceda13f0b83784adce78b527099e08e7e003c6ac)** to **[4bbb298](https://github.com/sagemath/sagetrac-mirror/commit/4bbb2980aab4b0a81fc07cf62afa5395ee7719c1)**



---

archive/issue_comments_267536.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4bbb2980aab4b0a81fc07cf62afa5395ee7719c1\">4bbb298</a></td><td><code>Corrected edge weights</code></td></tr></table>\n",
    "created_at": "2015-07-22T11:05:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267536",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4bbb2980aab4b0a81fc07cf62afa5395ee7719c1">4bbb298</a></td><td><code>Corrected edge weights</code></td></tr></table>




---

archive/issue_comments_267537.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nHello!\n\nI have changed the behavior: now, if edge weights are not convertible to floats through `float()` function, an error is raised. I have tried to clarify this behavior as much as possible in the documentation.\n\nThank you very much!\n\nMichele",
    "created_at": "2015-07-22T11:07:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267537",
    "user": "https://github.com/sagetrac-borassi"
}
```

<a id='comment:12'>Comment 12:</a>
Hello!

I have changed the behavior: now, if edge weights are not convertible to floats through `float()` function, an error is raised. I have tried to clarify this behavior as much as possible in the documentation.

Thank you very much!

Michele



---

archive/issue_comments_267538.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nThe documentation builds properly and is now good.\n\nI tracked all calls to `min_spanning_tree` to launch tests. All tests pass in graphs and homology.\nHowever, one test is broken in `src/sage/matroids/utilities.py`, method `lift_cross_ratios`. I suspect that this is due to the change in the output ordering of edges.\nI don't know what to do here. I have forced you to sort edges because all methods where not returning edges in the same order, and I think it is better now (the output is independent on the algorithm). \n\nThe set T constructed in this method is, before applying this patch:\n\n```\nset([((0, 0), (0, 1), (0, 0)), ((0, 0), (2, 1), (0, 2)), ((0, 1), (1, 0), (1, 0)), ((1, 0), (1, 1), (1, 1)), ((1, 1), (2, 0), (2, 1)), ((0, 0), (3, 1), (0, 3)), ((0, 0), (4, 1), (0, 4))])\n```\nand with this patch:\n\n```\nset([((0, 0), (0, 1), (0, 0)), ((0, 0), (2, 1), (0, 2)), ((0, 1), (1, 0), (1, 0)), ((1, 0), (1, 1), (1, 1)), ((2, 0), (2, 1), (2, 2)), ((0, 0), (3, 1), (0, 3)), ((0, 0), (4, 1), (0, 4))])\n```\nSo only one entry differs: `((1, 1), (2, 0), (2, 1))` becomes `((2, 0), (2, 1), (2, 2))`.\n\nI don't know if we can safely update the expected output of this test or not. Nathann?",
    "created_at": "2015-07-22T13:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267538",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:13'>Comment 13:</a>
The documentation builds properly and is now good.

I tracked all calls to `min_spanning_tree` to launch tests. All tests pass in graphs and homology.
However, one test is broken in `src/sage/matroids/utilities.py`, method `lift_cross_ratios`. I suspect that this is due to the change in the output ordering of edges.
I don't know what to do here. I have forced you to sort edges because all methods where not returning edges in the same order, and I think it is better now (the output is independent on the algorithm). 

The set T constructed in this method is, before applying this patch:

```
set([((0, 0), (0, 1), (0, 0)), ((0, 0), (2, 1), (0, 2)), ((0, 1), (1, 0), (1, 0)), ((1, 0), (1, 1), (1, 1)), ((1, 1), (2, 0), (2, 1)), ((0, 0), (3, 1), (0, 3)), ((0, 0), (4, 1), (0, 4))])
```
and with this patch:

```
set([((0, 0), (0, 1), (0, 0)), ((0, 0), (2, 1), (0, 2)), ((0, 1), (1, 0), (1, 0)), ((1, 0), (1, 1), (1, 1)), ((2, 0), (2, 1), (2, 2)), ((0, 0), (3, 1), (0, 3)), ((0, 0), (4, 1), (0, 4))])
```
So only one entry differs: `((1, 1), (2, 0), (2, 1))` becomes `((2, 0), (2, 1), (2, 2))`.

I don't know if we can safely update the expected output of this test or not. Nathann?



---

archive/issue_comments_267539.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nHello,\n\n> I don't know if we can safely update the expected output of this test or not. Nathann?\n\nNot sure. Possible, but not sure. The best is to ask the matroid guys. Stefan, Rudi? Does it look okay to you if by changing the behaviour of a graph spanning tree routine, the output of \n\n```\nsage: Z = lift_cross_ratios(A, to_sixth_root_of_unity)\n```\n\nat line 416 of matroids/utilities.py changes like that?\n\n```\nFailed example:\n    Z\nExpected:\n    [ 1  0  1  1  1]\n    [ 1  1  0  0  z]\n    [ 0  1 -z -1  0]\nGot:\n    [     1      0      1      1      1]\n    [     1      1      0      0      z]\n    [     0  z - 1      1 -z + 1      0]\n```\n\nIt may be caused by a different spanning tree, or by a reordering of its edges.\n\nNathann",
    "created_at": "2015-07-22T14:28:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267539",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:14'>Comment 14:</a>
Hello,

> I don't know if we can safely update the expected output of this test or not. Nathann?

Not sure. Possible, but not sure. The best is to ask the matroid guys. Stefan, Rudi? Does it look okay to you if by changing the behaviour of a graph spanning tree routine, the output of 

```
sage: Z = lift_cross_ratios(A, to_sixth_root_of_unity)
```

at line 416 of matroids/utilities.py changes like that?

```
Failed example:
    Z
Expected:
    [ 1  0  1  1  1]
    [ 1  1  0  0  z]
    [ 0  1 -z -1  0]
Got:
    [     1      0      1      1      1]
    [     1      1      0      0      z]
    [     0  z - 1      1 -z + 1      0]
```

It may be caused by a different spanning tree, or by a reordering of its edges.

Nathann



---

archive/issue_comments_267540.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nReplying to [@nathanncohen](#comment%3A14):\n> Stefan, Rudi? Does it look okay to you if by changing the behaviour of a graph spanning tree routine, the output of \n> \n> ```\n> sage: Z = lift_cross_ratios(A, to_sixth_root_of_unity)\n> ```\n> \n> at line 416 of matroids/utilities.py changes like that?\n> \n> ```\n> Failed example:\n>     Z\n> Expected:\n>     [ 1  0  1  1  1]\n>     [ 1  1  0  0  z]\n>     [ 0  1 -z -1  0]\n> Got:\n>     [     1      0      1      1      1]\n>     [     1      1      0      0      z]\n>     [     0  z - 1      1 -z + 1      0]\n> ```\n> \n> It may be caused by a different spanning tree, or by a reordering of its edges.\n\nYes, that looks OK. This output matrix Z is fixed up to scaling of rows and columns, and the tree is used to determine the scaling. Looks like in this example the bottom row is scaled by  z-1 (which happens to equal -z<sup>-1</sup> in that ring, z is is a sixth root of unity). \n\nSo it's safe to just update the output of the test.",
    "created_at": "2015-07-22T14:42:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267540",
    "user": "https://github.com/sagetrac-Rudi"
}
```

<a id='comment:15'>Comment 15:</a>
Replying to [@nathanncohen](#comment%3A14):
> Stefan, Rudi? Does it look okay to you if by changing the behaviour of a graph spanning tree routine, the output of 
> 
> ```
> sage: Z = lift_cross_ratios(A, to_sixth_root_of_unity)
> ```
> 
> at line 416 of matroids/utilities.py changes like that?
> 
> ```
> Failed example:
>     Z
> Expected:
>     [ 1  0  1  1  1]
>     [ 1  1  0  0  z]
>     [ 0  1 -z -1  0]
> Got:
>     [     1      0      1      1      1]
>     [     1      1      0      0      z]
>     [     0  z - 1      1 -z + 1      0]
> ```
> 
> It may be caused by a different spanning tree, or by a reordering of its edges.

Yes, that looks OK. This output matrix Z is fixed up to scaling of rows and columns, and the tree is used to determine the scaling. Looks like in this example the bottom row is scaled by  z-1 (which happens to equal -z<sup>-1</sup> in that ring, z is is a sixth root of unity). 

So it's safe to just update the output of the test.



---

archive/issue_comments_267541.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\n> Yes, that looks OK.\n\nGreat. Thanks for your answer `:-)`\n\nNathann",
    "created_at": "2015-07-22T15:11:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267541",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:16'>Comment 16:</a>
> Yes, that looks OK.

Great. Thanks for your answer `:-)`

Nathann



---

archive/issue_comments_267542.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fd9e2320a51353b9d90879e237869e4971613a5e\">fd9e232</a></td><td><code>Corrected matroids test</code></td></tr></table>\n",
    "created_at": "2015-07-22T15:24:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267542",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:17'>Comment 17:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fd9e2320a51353b9d90879e237869e4971613a5e">fd9e232</a></td><td><code>Corrected matroids test</code></td></tr></table>




---

archive/issue_comments_267543.json:
```json
{
    "body": "Changed commit from **[4bbb298](https://github.com/sagemath/sagetrac-mirror/commit/4bbb2980aab4b0a81fc07cf62afa5395ee7719c1)** to **[fd9e232](https://github.com/sagemath/sagetrac-mirror/commit/fd9e2320a51353b9d90879e237869e4971613a5e)**",
    "created_at": "2015-07-22T15:24:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267543",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4bbb298](https://github.com/sagemath/sagetrac-mirror/commit/4bbb2980aab4b0a81fc07cf62afa5395ee7719c1)** to **[fd9e232](https://github.com/sagemath/sagetrac-mirror/commit/fd9e2320a51353b9d90879e237869e4971613a5e)**



---

archive/issue_comments_267544.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nDone! Thank you very much, Rudi!",
    "created_at": "2015-07-22T15:25:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267544",
    "user": "https://github.com/sagetrac-borassi"
}
```

<a id='comment:18'>Comment 18:</a>
Done! Thank you very much, Rudi!



---

archive/issue_comments_267545.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2015-07-22T15:51:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267545",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_events_250677.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-07-22T15:51:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250677"
}
```



---

archive/issue_events_250678.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-07-22T15:51:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250678"
}
```



---

archive/issue_comments_267546.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nGood!",
    "created_at": "2015-07-22T15:51:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267546",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:19'>Comment 19:</a>
Good!



---

archive/issue_comments_267547.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6c084ecd07a17b4ca7c63c6cb7f244fe0dde0545\">6c084ec</a></td><td><code>Merge branch 'develop' into t/18876/boost_cuthill_mckee__king_ordering</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3847037e70388b86146c5f9e97e25bb92ec6e949\">3847037</a></td><td><code>Merge branch 'develop' into t/18876/boost_cuthill_mckee__king_ordering</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/66c6aea291d11529833931cc094abd7b35df794c\">66c6aea</a></td><td><code>Merge branch 't/18876/boost_cuthill_mckee__king_ordering' into t/18910/boost_minimum_spanning_tree</code></td></tr></table>\n",
    "created_at": "2015-07-30T10:39:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267547",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:20'>Comment 20:</a>
Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6c084ecd07a17b4ca7c63c6cb7f244fe0dde0545">6c084ec</a></td><td><code>Merge branch 'develop' into t/18876/boost_cuthill_mckee__king_ordering</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3847037e70388b86146c5f9e97e25bb92ec6e949">3847037</a></td><td><code>Merge branch 'develop' into t/18876/boost_cuthill_mckee__king_ordering</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/66c6aea291d11529833931cc094abd7b35df794c">66c6aea</a></td><td><code>Merge branch 't/18876/boost_cuthill_mckee__king_ordering' into t/18910/boost_minimum_spanning_tree</code></td></tr></table>




---

archive/issue_comments_267548.json:
```json
{
    "body": "Changed commit from **[fd9e232](https://github.com/sagemath/sagetrac-mirror/commit/fd9e2320a51353b9d90879e237869e4971613a5e)** to **[66c6aea](https://github.com/sagemath/sagetrac-mirror/commit/66c6aea291d11529833931cc094abd7b35df794c)**",
    "created_at": "2015-07-30T10:39:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267548",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[fd9e232](https://github.com/sagemath/sagetrac-mirror/commit/fd9e2320a51353b9d90879e237869e4971613a5e)** to **[66c6aea](https://github.com/sagemath/sagetrac-mirror/commit/66c6aea291d11529833931cc094abd7b35df794c)**



---

archive/issue_events_250679.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2015-07-30T10:39:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250679"
}
```



---

archive/issue_events_250680.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2015-07-30T10:39:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250680"
}
```



---

archive/issue_comments_267549.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nI have merged this ticket with the latest version of #18876 to avoid conflicts.",
    "created_at": "2015-07-30T10:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267549",
    "user": "https://github.com/sagetrac-borassi"
}
```

<a id='comment:21'>Comment 21:</a>
I have merged this ticket with the latest version of #18876 to avoid conflicts.



---

archive/issue_comments_267550.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nWhen I click on the link to the code (to of the page, branch `u/borassi/boost_minimum_spanning_tree`), for file `bandwidth.pyx` I see the following line which has been changed in #18876. \n\n```\n+    :meth:`bandwidth_heuristics()<sage.graphs.base.boost_graph.bandwidth_heuristics>` | Uses Boost heuristics to approximate the bandwidth of the input graph\n```\nIs it normal??",
    "created_at": "2015-07-30T11:30:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267550",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:22'>Comment 22:</a>
When I click on the link to the code (to of the page, branch `u/borassi/boost_minimum_spanning_tree`), for file `bandwidth.pyx` I see the following line which has been changed in #18876. 

```
+    :meth:`bandwidth_heuristics()<sage.graphs.base.boost_graph.bandwidth_heuristics>` | Uses Boost heuristics to approximate the bandwidth of the input graph
```
Is it normal??



---

archive/issue_comments_267551.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nYes, it is normal, I think: I modified that line only after I merged #18876. However, I do not think it is a problem, because the new commit in #18876 should not create conflicts (I have tested the merge).\n\nIn any case, I re-merged them with this new commit.\n\nReplying to [@dcoudert](#comment%3A22):\n\n\n> When I click on the link to the code (to of the page, branch `u/borassi/boost_minimum_spanning_tree`), for file `bandwidth.pyx` I see the following line which has been changed in #18876. \n> \n> ```\n> +    :meth:`bandwidth_heuristics()<sage.graphs.base.boost_graph.bandwidth_heuristics>` | Uses Boost heuristics to approximate the bandwidth of the input graph\n> ```\n> Is it normal??",
    "created_at": "2015-07-30T12:47:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267551",
    "user": "https://github.com/sagetrac-borassi"
}
```

<a id='comment:23'>Comment 23:</a>
Yes, it is normal, I think: I modified that line only after I merged #18876. However, I do not think it is a problem, because the new commit in #18876 should not create conflicts (I have tested the merge).

In any case, I re-merged them with this new commit.

Replying to [@dcoudert](#comment%3A22):


> When I click on the link to the code (to of the page, branch `u/borassi/boost_minimum_spanning_tree`), for file `bandwidth.pyx` I see the following line which has been changed in #18876. 
> 
> ```
> +    :meth:`bandwidth_heuristics()<sage.graphs.base.boost_graph.bandwidth_heuristics>` | Uses Boost heuristics to approximate the bandwidth of the input graph
> ```
> Is it normal??



---

archive/issue_comments_267552.json:
```json
{
    "body": "<a id='comment:24'>Comment 24:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ecda30fc9954a53b8104d9cf14edd1f73a7a4cad\">ecda30f</a></td><td><code>Applied Nathann's suggestion</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/38a817d387325a25c62df70c6e77d7c06169f152\">38a817d</a></td><td><code>Merge branch 't/18876/boost_cuthill_mckee__king_ordering' into t/18910/boost_minimum_spanning_tree</code></td></tr></table>\n",
    "created_at": "2015-07-30T12:48:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267552",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:24'>Comment 24:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ecda30fc9954a53b8104d9cf14edd1f73a7a4cad">ecda30f</a></td><td><code>Applied Nathann's suggestion</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/38a817d387325a25c62df70c6e77d7c06169f152">38a817d</a></td><td><code>Merge branch 't/18876/boost_cuthill_mckee__king_ordering' into t/18910/boost_minimum_spanning_tree</code></td></tr></table>




---

archive/issue_comments_267553.json:
```json
{
    "body": "Changed commit from **[66c6aea](https://github.com/sagemath/sagetrac-mirror/commit/66c6aea291d11529833931cc094abd7b35df794c)** to **[38a817d](https://github.com/sagemath/sagetrac-mirror/commit/38a817d387325a25c62df70c6e77d7c06169f152)**",
    "created_at": "2015-07-30T12:48:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267553",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[66c6aea](https://github.com/sagemath/sagetrac-mirror/commit/66c6aea291d11529833931cc094abd7b35df794c)** to **[38a817d](https://github.com/sagemath/sagetrac-mirror/commit/38a817d387325a25c62df70c6e77d7c06169f152)**



---

archive/issue_events_250681.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-07-30T16:24:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250681"
}
```



---

archive/issue_events_250682.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2015-07-30T16:24:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250682"
}
```



---

archive/issue_comments_267554.json:
```json
{
    "body": "<a id='comment:25'>Comment 25:</a>\nfor me the patch is good to go !",
    "created_at": "2015-07-30T16:24:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267554",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:25'>Comment 25:</a>
for me the patch is good to go !



---

archive/issue_comments_267555.json:
```json
{
    "body": "Changed branch from **[u/borassi/boost_minimum_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/u/borassi/boost_minimum_spanning_tree)** to **[38a817d](https://github.com/sagemath/sagetrac-mirror/commit/38a817d387325a25c62df70c6e77d7c06169f152)**",
    "created_at": "2015-07-31T17:52:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18910#issuecomment-267555",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/borassi/boost_minimum_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/u/borassi/boost_minimum_spanning_tree)** to **[38a817d](https://github.com/sagemath/sagetrac-mirror/commit/38a817d387325a25c62df70c6e77d7c06169f152)**



---

archive/issue_events_250683.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-31T17:52:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250683"
}
```



---

archive/issue_events_250684.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "571ad54c290060676caf69919448dc50a9faffff",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-07-31T17:52:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18910",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18910#event-250684"
}
```
