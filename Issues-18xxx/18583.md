# Issue 18583: Clean up PARI declarations

archive/issues_018346.json:
```json
{
    "assignees": [],
    "body": "We should really clean up the declarations in `sage/libs/paridecl.pxd`, analogous to what has been done for `gmp` for example.\n\nThis also fixes the unneeded linking of `pari` in the following Cython extensions:\n\n```\nsrc/build/cythonized/sage/rings/number_field/totallyreal.c\nsrc/build/cythonized/sage/rings/padics/common_conversion.c\nsrc/build/cythonized/sage/rings/padics/padic_capped_absolute_element.c\nsrc/build/cythonized/sage/rings/padics/padic_capped_relative_element.c\nsrc/build/cythonized/sage/rings/padics/padic_fixed_mod_element.c\nsrc/build/cythonized/sage/rings/real_double.c\n```\n\nCC:  @defeo @sagetrac-jpflori\n\nBranch/Commit: **[f7efbfe](https://github.com/sagemath/sagetrac-mirror/commit/f7efbfe22d1cf953cace8e57f27e81be92cd6197)**\n\nReviewer: **Jean-Pierre Flori**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18583_\n\n",
    "closed_at": "2015-07-27T15:15:56Z",
    "created_at": "2015-06-02T10:01:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean up PARI declarations",
    "type": "issue",
    "updated_at": "2015-07-27T15:15:56Z",
    "url": "https://github.com/sagemath/sage/issues/18583",
    "user": "https://github.com/jdemeyer"
}
```
We should really clean up the declarations in `sage/libs/paridecl.pxd`, analogous to what has been done for `gmp` for example.

This also fixes the unneeded linking of `pari` in the following Cython extensions:

```
src/build/cythonized/sage/rings/number_field/totallyreal.c
src/build/cythonized/sage/rings/padics/common_conversion.c
src/build/cythonized/sage/rings/padics/padic_capped_absolute_element.c
src/build/cythonized/sage/rings/padics/padic_capped_relative_element.c
src/build/cythonized/sage/rings/padics/padic_fixed_mod_element.c
src/build/cythonized/sage/rings/real_double.c
```

CC:  @defeo @sagetrac-jpflori

Branch/Commit: **[f7efbfe](https://github.com/sagemath/sagetrac-mirror/commit/f7efbfe22d1cf953cace8e57f27e81be92cd6197)**

Reviewer: **Jean-Pierre Flori**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/18583_





---

archive/issue_events_246420.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-02T10:01:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246420"
}
```



---

archive/issue_events_246421.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-02T10:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246421"
}
```



---

archive/issue_events_246422.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-02T10:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246422"
}
```



---

archive/issue_events_246423.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-02T10:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246423"
}
```



---

archive/issue_events_246424.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-02T10:01:30Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "title_is": "Clean up PARI declarations",
    "title_was": "Clean up PARI declaration",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246424"
}
```



---

archive/issue_comments_260795.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,12 @@\n We should really clean up the declarations in `sage/libs/paridecl.pxd`, analogous to what has been done for `gmp` for example.\n+\n+This also fixes the unneeded linking of `pari` in the following Cython extensions:\n+\n+```\n+src/build/cythonized/sage/rings/number_field/totallyreal.c\n+src/build/cythonized/sage/rings/padics/common_conversion.c\n+src/build/cythonized/sage/rings/padics/padic_capped_absolute_element.c\n+src/build/cythonized/sage/rings/padics/padic_capped_relative_element.c\n+src/build/cythonized/sage/rings/padics/padic_fixed_mod_element.c\n+src/build/cythonized/sage/rings/real_double.c\n+```\n``````\n",
    "created_at": "2015-06-26T09:37:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260795",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,12 @@
 We should really clean up the declarations in `sage/libs/paridecl.pxd`, analogous to what has been done for `gmp` for example.
+
+This also fixes the unneeded linking of `pari` in the following Cython extensions:
+
+```
+src/build/cythonized/sage/rings/number_field/totallyreal.c
+src/build/cythonized/sage/rings/padics/common_conversion.c
+src/build/cythonized/sage/rings/padics/padic_capped_absolute_element.c
+src/build/cythonized/sage/rings/padics/padic_capped_relative_element.c
+src/build/cythonized/sage/rings/padics/padic_fixed_mod_element.c
+src/build/cythonized/sage/rings/real_double.c
+```
``````




---

archive/issue_comments_260796.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2015-06-26T09:37:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260796",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_260797.json:
```json
{
    "body": "Changed dependencies from **#18450** to none",
    "created_at": "2015-06-26T09:37:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260797",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **#18450** to none



---

archive/issue_comments_260798.json:
```json
{
    "body": "Branch: **[u/jdemeyer/clean_up_pari_declarations](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/clean_up_pari_declarations)**",
    "created_at": "2015-06-26T09:38:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260798",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/clean_up_pari_declarations](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/clean_up_pari_declarations)**



---

archive/issue_comments_260799.json:
```json
{
    "body": "Commit: **[d9c97ac](https://github.com/sagemath/sagetrac-mirror/commit/d9c97acc74bebfb486d55a9e35263851f42e6845)**",
    "created_at": "2015-06-26T09:41:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260799",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[d9c97ac](https://github.com/sagemath/sagetrac-mirror/commit/d9c97acc74bebfb486d55a9e35263851f42e6845)**



---

archive/issue_events_246425.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-26T09:41:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246425"
}
```



---

archive/issue_comments_260800.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d9c97acc74bebfb486d55a9e35263851f42e6845\">d9c97ac</a></td><td><code>Clean up PARI declarations</code></td></tr></table>\n",
    "created_at": "2015-06-26T09:41:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260800",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">Comment 5</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d9c97acc74bebfb486d55a9e35263851f42e6845">d9c97ac</a></td><td><code>Clean up PARI declarations</code></td></tr></table>




---

archive/issue_comments_260801.json:
```json
{
    "body": "Changed commit from **[d9c97ac](https://github.com/sagemath/sagetrac-mirror/commit/d9c97acc74bebfb486d55a9e35263851f42e6845)** to **[f2854be](https://github.com/sagemath/sagetrac-mirror/commit/f2854be020aa0dd368ea2941ed5d589be797aa13)**",
    "created_at": "2015-07-19T08:58:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260801",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d9c97ac](https://github.com/sagemath/sagetrac-mirror/commit/d9c97acc74bebfb486d55a9e35263851f42e6845)** to **[f2854be](https://github.com/sagemath/sagetrac-mirror/commit/f2854be020aa0dd368ea2941ed5d589be797aa13)**



---

archive/issue_comments_260802.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f2854be020aa0dd368ea2941ed5d589be797aa13\">f2854be</a></td><td><code>Fix paricom.h declarations</code></td></tr></table>\n",
    "created_at": "2015-07-19T08:58:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260802",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f2854be020aa0dd368ea2941ed5d589be797aa13">f2854be</a></td><td><code>Fix paricom.h declarations</code></td></tr></table>




---

archive/issue_comments_260803.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nCould you just add a comment on why `types.pxd` should not call any PARI function?\n\nI guess it is to avoid the need to link to PARI when cimporting taht file, but that's not stated.",
    "created_at": "2015-07-20T11:55:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260803",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:8" align="right">Comment 8</div>

Could you just add a comment on why `types.pxd` should not call any PARI function?

I guess it is to avoid the need to link to PARI when cimporting taht file, but that's not stated.



---

archive/issue_comments_260804.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f7efbfe22d1cf953cace8e57f27e81be92cd6197\">f7efbfe</a></td><td><code>Comment about types.pxd</code></td></tr></table>\n",
    "created_at": "2015-07-21T09:35:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260804",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f7efbfe22d1cf953cace8e57f27e81be92cd6197">f7efbfe</a></td><td><code>Comment about types.pxd</code></td></tr></table>




---

archive/issue_comments_260805.json:
```json
{
    "body": "Changed commit from **[f2854be](https://github.com/sagemath/sagetrac-mirror/commit/f2854be020aa0dd368ea2941ed5d589be797aa13)** to **[f7efbfe](https://github.com/sagemath/sagetrac-mirror/commit/f7efbfe22d1cf953cace8e57f27e81be92cd6197)**",
    "created_at": "2015-07-21T09:35:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260805",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f2854be](https://github.com/sagemath/sagetrac-mirror/commit/f2854be020aa0dd368ea2941ed5d589be797aa13)** to **[f7efbfe](https://github.com/sagemath/sagetrac-mirror/commit/f7efbfe22d1cf953cace8e57f27e81be92cd6197)**



---

archive/issue_comments_260806.json:
```json
{
    "body": "Reviewer: **Jean-Pierre Flori**",
    "created_at": "2015-07-21T12:27:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260806",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Reviewer: **Jean-Pierre Flori**



---

archive/issue_events_246426.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-07-21T12:27:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246426"
}
```



---

archive/issue_events_246427.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-07-21T12:27:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246427"
}
```



---

archive/issue_events_246428.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-27T15:15:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246428"
}
```



---

archive/issue_events_246429.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9a1899b3424405220579cd4c22c9569f48f77f43",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-07-27T15:15:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18583#event-246429"
}
```



---

archive/issue_comments_260807.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/clean_up_pari_declarations](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/clean_up_pari_declarations)** to **[f7efbfe](https://github.com/sagemath/sagetrac-mirror/commit/f7efbfe22d1cf953cace8e57f27e81be92cd6197)**",
    "created_at": "2015-07-27T15:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18583#issuecomment-260807",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/clean_up_pari_declarations](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/clean_up_pari_declarations)** to **[f7efbfe](https://github.com/sagemath/sagetrac-mirror/commit/f7efbfe22d1cf953cace8e57f27e81be92cd6197)**
