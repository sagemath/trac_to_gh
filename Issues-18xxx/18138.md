# Issue 18138: Mathematica WSTP/MathLink Interface

archive/issues_017901.json:
```json
{
    "assignees": [],
    "body": "The current Mathematica interface to Sage relies on the command-line interface that ships with Mathematica and can break if WRI makes changes to that command-line interface (see #13892, #15318, and #16703). This ticket replaces WRI's command-line interface with an equivalent interface (which itself uses WSTP/MathLink) that is a drop-in replacement. The Sage project can thus guarantee the stability of the interface under the assumption of the stability of the WSTP/MathLink protocol. \n\nCC:  @mkoeppe @dimpase @yuan-zhou\n\nBranch/Commit: **[u/RobertJacobson/mathematica_wstp_mathlink_interface](https://github.com/sagemath/sagetrac-mirror/tree/u/RobertJacobson/mathematica_wstp_mathlink_interface) @ [a6d1b77](https://github.com/sagemath/sagetrac-mirror/commit/a6d1b779093be9a9e544a23aef433522e7ed2590)**\n\nAuthor: **Robert Jacobson**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18138_\n\n",
    "created_at": "2015-04-07T21:07:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Mathematica WSTP/MathLink Interface",
    "type": "issue",
    "updated_at": "2022-12-29T01:38:39Z",
    "url": "https://github.com/sagemath/sage/issues/18138",
    "user": "https://github.com/rljacobson"
}
```
The current Mathematica interface to Sage relies on the command-line interface that ships with Mathematica and can break if WRI makes changes to that command-line interface (see #13892, #15318, and #16703). This ticket replaces WRI's command-line interface with an equivalent interface (which itself uses WSTP/MathLink) that is a drop-in replacement. The Sage project can thus guarantee the stability of the interface under the assumption of the stability of the WSTP/MathLink protocol. 

CC:  @mkoeppe @dimpase @yuan-zhou

Branch/Commit: **[u/RobertJacobson/mathematica_wstp_mathlink_interface](https://github.com/sagemath/sagetrac-mirror/tree/u/RobertJacobson/mathematica_wstp_mathlink_interface) @ [a6d1b77](https://github.com/sagemath/sagetrac-mirror/commit/a6d1b779093be9a9e544a23aef433522e7ed2590)**

Author: **Robert Jacobson**

_Issue created by migration from https://trac.sagemath.org/ticket/18138_





---

archive/issue_events_240254.json:
```json
{
    "actor": "https://github.com/rljacobson",
    "created_at": "2015-04-07T21:07:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18138#event-240254"
}
```



---

archive/issue_events_240255.json:
```json
{
    "actor": "https://github.com/rljacobson",
    "created_at": "2015-04-07T21:07:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18138#event-240255"
}
```



---

archive/issue_comments_250346.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+The current Mathematica interface to Sage relies on the command-line interface that ships with Mathematica and can break if WRI makes changes to that command-line interface (see #13892, #15318, and #16703). This ticket replaces WRI's command-line interface with an equivalent interface (which itself uses WSTP/MathLink) that is a drop-in replacement. The Sage project can thus guarantee the stability of the interface under the assumption of the stability of the WSTP/MathLink protocol. \n``````\n",
    "created_at": "2015-04-07T21:17:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18138#issuecomment-250346",
    "user": "https://github.com/rljacobson"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+The current Mathematica interface to Sage relies on the command-line interface that ships with Mathematica and can break if WRI makes changes to that command-line interface (see #13892, #15318, and #16703). This ticket replaces WRI's command-line interface with an equivalent interface (which itself uses WSTP/MathLink) that is a drop-in replacement. The Sage project can thus guarantee the stability of the interface under the assumption of the stability of the WSTP/MathLink protocol. 
``````




---

archive/issue_comments_250347.json:
```json
{
    "body": "Author: **Robert Jacobson**",
    "created_at": "2015-04-07T21:17:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18138#issuecomment-250347",
    "user": "https://github.com/rljacobson"
}
```

Author: **Robert Jacobson**



---

archive/issue_events_240256.json:
```json
{
    "actor": "https://github.com/rljacobson",
    "created_at": "2015-04-07T21:17:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18138#event-240256"
}
```



---

archive/issue_comments_250348.json:
```json
{
    "body": "Branch: **[u/RobertJacobson/mathematica_wstp_mathlink_interface](https://github.com/sagemath/sagetrac-mirror/tree/u/RobertJacobson/mathematica_wstp_mathlink_interface)**",
    "created_at": "2017-03-08T01:17:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18138#issuecomment-250348",
    "user": "https://github.com/rljacobson"
}
```

Branch: **[u/RobertJacobson/mathematica_wstp_mathlink_interface](https://github.com/sagemath/sagetrac-mirror/tree/u/RobertJacobson/mathematica_wstp_mathlink_interface)**



---

archive/issue_comments_250349.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/529019cb17e9f537eeac715abd935679670df613\">529019c</a></td><td><code>Added experimental package MathLine, a textual Mathematica front end.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a6d1b779093be9a9e544a23aef433522e7ed2590\">a6d1b77</a></td><td><code>Changed capitalization of binary.</code></td></tr></table>\n",
    "created_at": "2017-03-08T12:17:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18138#issuecomment-250349",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/529019cb17e9f537eeac715abd935679670df613">529019c</a></td><td><code>Added experimental package MathLine, a textual Mathematica front end.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a6d1b779093be9a9e544a23aef433522e7ed2590">a6d1b77</a></td><td><code>Changed capitalization of binary.</code></td></tr></table>




---

archive/issue_events_240257.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2017-03-08T12:17:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18138#event-240257"
}
```



---

archive/issue_events_240258.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2017-03-08T12:17:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18138#event-240258"
}
```



---

archive/issue_comments_250350.json:
```json
{
    "body": "Commit: **[a6d1b77](https://github.com/sagemath/sagetrac-mirror/commit/a6d1b779093be9a9e544a23aef433522e7ed2590)**",
    "created_at": "2017-03-08T12:17:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18138#issuecomment-250350",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[a6d1b77](https://github.com/sagemath/sagetrac-mirror/commit/a6d1b779093be9a9e544a23aef433522e7ed2590)**



---

archive/issue_events_240259.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2017-03-08T12:17:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18138#event-240259"
}
```



---

archive/issue_events_240260.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:38:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18138",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18138#event-240260"
}
```
