# Issue 18079: Update openssl to 1.0.2a

archive/issues_017842.json:
```json
{
    "assignees": [],
    "body": "Some [new security fixes](http://openssl.org/news/secadv_20150319.txt) appeared in openssl, including a \"highly severe\" one affecting 1.0.2.\n\nTarball available at : http://openssl.org/source/openssl-1.0.2a.tar.gz\n\n\nKeywords: **sd66**\n\nBranch/Commit: **[8c4a4b2](https://github.com/sagemath/sagetrac-mirror/commit/8c4a4b247190f2bebe1c73212bd9e7356ec1dd6d)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Thierry Monteil**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18079_\n\n",
    "closed_at": "2015-04-03T23:13:13Z",
    "created_at": "2015-03-29T08:09:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update openssl to 1.0.2a",
    "type": "issue",
    "updated_at": "2015-04-03T23:13:13Z",
    "url": "https://github.com/sagemath/sage/issues/18079",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
Some [new security fixes](http://openssl.org/news/secadv_20150319.txt) appeared in openssl, including a "highly severe" one affecting 1.0.2.

Tarball available at : http://openssl.org/source/openssl-1.0.2a.tar.gz


Keywords: **sd66**

Branch/Commit: **[8c4a4b2](https://github.com/sagemath/sagetrac-mirror/commit/8c4a4b247190f2bebe1c73212bd9e7356ec1dd6d)**

Reviewer: **Jeroen Demeyer**

Author: **Thierry Monteil**

_Issue created by migration from https://trac.sagemath.org/ticket/18079_





---

archive/issue_events_239460.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-03-29T08:09:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239460"
}
```



---

archive/issue_events_239461.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-03-29T08:09:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239461"
}
```



---

archive/issue_events_239462.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-03-29T08:09:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239462"
}
```



---

archive/issue_events_239463.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-03-29T08:09:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239463"
}
```



---

archive/issue_comments_249007.json:
```json
{
    "body": "Branch: **[u/tmonteil/update_openssl_to_1_0_2a](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/update_openssl_to_1_0_2a)**",
    "created_at": "2015-03-29T08:18:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18079#issuecomment-249007",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Branch: **[u/tmonteil/update_openssl_to_1_0_2a](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/update_openssl_to_1_0_2a)**



---

archive/issue_comments_249008.json:
```json
{
    "body": "Author: **Thierry Monteil**",
    "created_at": "2015-03-29T13:44:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18079#issuecomment-249008",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Author: **Thierry Monteil**



---

archive/issue_comments_249009.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Some [openssl.org/news/secadv_20150319.txt new security fixes] appeared in openssl, including a \"highly severe\" one affecting 1.0.2.\n+Some [new security fixes](http://openssl.org/news/secadv_20150319.txt) appeared in openssl, including a \"highly severe\" one affecting 1.0.2.\n \n Tarball available at : http://openssl.org/source/openssl-1.0.2a.tar.gz\n \n``````\n",
    "created_at": "2015-03-29T13:44:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18079#issuecomment-249009",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Some [openssl.org/news/secadv_20150319.txt new security fixes] appeared in openssl, including a "highly severe" one affecting 1.0.2.
+Some [new security fixes](http://openssl.org/news/secadv_20150319.txt) appeared in openssl, including a "highly severe" one affecting 1.0.2.
 
 Tarball available at : http://openssl.org/source/openssl-1.0.2a.tar.gz
 
``````




---

archive/issue_events_239464.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-03-29T13:44:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239464"
}
```



---

archive/issue_comments_249010.json:
```json
{
    "body": "Commit: **[8c4a4b2](https://github.com/sagemath/sagetrac-mirror/commit/8c4a4b247190f2bebe1c73212bd9e7356ec1dd6d)**",
    "created_at": "2015-03-29T13:44:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18079#issuecomment-249010",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Commit: **[8c4a4b2](https://github.com/sagemath/sagetrac-mirror/commit/8c4a4b247190f2bebe1c73212bd9e7356ec1dd6d)**



---

archive/issue_comments_249011.json:
```json
{
    "body": "Changed keywords from **** to **sd66**",
    "created_at": "2015-03-29T13:44:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18079#issuecomment-249011",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Changed keywords from **** to **sd66**



---

archive/issue_events_239465.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T09:50:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239465"
}
```



---

archive/issue_events_239466.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T09:50:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239466"
}
```



---

archive/issue_comments_249012.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-04-02T09:50:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18079#issuecomment-249012",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_239467.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-02T12:09:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239467"
}
```



---

archive/issue_events_239468.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-02T12:09:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239468"
}
```



---

archive/issue_comments_249013.json:
```json
{
    "body": "Changed branch from **[u/tmonteil/update_openssl_to_1_0_2a](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/update_openssl_to_1_0_2a)** to **[8c4a4b2](https://github.com/sagemath/sagetrac-mirror/commit/8c4a4b247190f2bebe1c73212bd9e7356ec1dd6d)**",
    "created_at": "2015-04-03T23:13:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18079#issuecomment-249013",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/tmonteil/update_openssl_to_1_0_2a](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/update_openssl_to_1_0_2a)** to **[8c4a4b2](https://github.com/sagemath/sagetrac-mirror/commit/8c4a4b247190f2bebe1c73212bd9e7356ec1dd6d)**



---

archive/issue_events_239469.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-03T23:13:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239469"
}
```



---

archive/issue_events_239470.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "299ac2e763b94ced1c138b3bc28763f78f73fc7b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-04-03T23:13:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18079",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18079#event-239470"
}
```
