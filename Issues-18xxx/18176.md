...
```json
{
    "body": "The code is only tested with `--optional=imagemagick`, so in other words it was not tested.",
...
```json
{
    "body": "And I don't have a particular preference for whether to make it a blocker or not. If somebody cares about animations then they can always review this ticket before sage-6.6 is out (i.e. very soon)",
...
```json
{
    "body": "Given that Andrey may not have had a chance to respond about this yet, I'm moving it to blocker until either I get a chance to review it (though all seems straightforward) or he can say it's not that high.  `a` works, just not `a.show`.   Does it seems like Martin's changes or the display changes are more likely to have caused this?",
    "created_at": "2015-04-13T15:17:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18176",
    "type": "issue_comment",
...
```json
{
    "body": "I notice that `delay` and `iterations` only have limited support from ffmpeg. The docs say as much. What does `sage.misc.sage_ostools.have_program(\"convert\")` report for you?",
...
```json
{
    "body": "And I do have imagemagick installed, the tests are just not run by default (See also #13540)",
...
```json
{
    "body": "Animations will be totally broken in Sage-6.6 since apparently that is preferable to not supporting some optional keyword argument. Propose to close this as wontfix until we can rewrite the animations so that no keyword is left behind.",
...
```json
{
    "body": "I know, the tests with enabling optional stuff are generally broken.",
...
```json
{
    "body": "Oh, duh!  I have to do `all` or `sage` as well - http://www.sagemath.org/doc/developer/doctesting.html#run-optional-tests.  Sorry, my bad.",
...
```json
{
    "body": "Give me another hour - I have to run an time-constrained errand right now but will return to this shortly.",
...
```json
{
    "body": "Wow, you pick up your kid, and the world keeps rotating ;-)  Nice!!!",
...
```json
{
    "body": "Just forgot to use `r\"\"\"` for the docstring once I started using escape sequences in string literals within that docstring.\n\n---\nNew commits:",
