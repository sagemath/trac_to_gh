# Issue 18901: Bug in continued fractions of real literal

archive/issues_018664.json:
```json
{
    "assignees": [],
    "body": "As reported in #14567, `continued_fraction` fails for real literals\n\n```\nsage: continued_fraction_list(1.575709393346379)\nError in lines 1-1\nTraceback (most recent call last):\n...\nValueError: does not know how to compute the continued fraction of 1.57570939334638\n```\n\nCC:  @williamstein\n\nBranch/Commit: **[8d86cb3](https://github.com/sagemath/sagetrac-mirror/commit/8d86cb325724472a7fe55e1a351ab5958b474ddf)**\n\nReviewer: **S\u00e9bastien Labb\u00e9**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18901_\n\n",
    "closed_at": "2015-07-29T09:22:39Z",
    "created_at": "2015-07-15T08:19:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in continued fractions of real literal",
    "type": "issue",
    "updated_at": "2015-07-29T09:22:39Z",
    "url": "https://github.com/sagemath/sage/issues/18901",
    "user": "https://github.com/videlec"
}
```
As reported in #14567, `continued_fraction` fails for real literals

```
sage: continued_fraction_list(1.575709393346379)
Error in lines 1-1
Traceback (most recent call last):
...
ValueError: does not know how to compute the continued fraction of 1.57570939334638
```

CC:  @williamstein

Branch/Commit: **[8d86cb3](https://github.com/sagemath/sagetrac-mirror/commit/8d86cb325724472a7fe55e1a351ab5958b474ddf)**

Reviewer: **Sébastien Labbé**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/18901_





---

archive/issue_events_250558.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-07-15T08:19:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250558"
}
```



---

archive/issue_events_250559.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-07-15T08:19:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250559"
}
```



---

archive/issue_events_250560.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-07-15T08:19:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250560"
}
```



---

archive/issue_events_250561.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-07-15T08:19:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250561"
}
```



---

archive/issue_events_250562.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-07-15T08:25:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250562"
}
```



---

archive/issue_comments_267285.json:
```json
{
    "body": "Commit: **[81d11d0](https://github.com/sagemath/sagetrac-mirror/commit/81d11d0cba0813b49f675cb4eb673367b9e0cd7c)**",
    "created_at": "2015-07-15T08:25:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267285",
    "user": "https://github.com/videlec"
}
```

Commit: **[81d11d0](https://github.com/sagemath/sagetrac-mirror/commit/81d11d0cba0813b49f675cb4eb673367b9e0cd7c)**



---

archive/issue_comments_267286.json:
```json
{
    "body": "Branch: **[u/vdelecroix/18901](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/18901)**",
    "created_at": "2015-07-15T08:25:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267286",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/18901](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/18901)**



---

archive/issue_comments_267287.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/81d11d0cba0813b49f675cb4eb673367b9e0cd7c\">81d11d0</a></td><td><code>Trac #18901: continued fraction of real literals</code></td></tr></table>\n",
    "created_at": "2015-07-15T08:25:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267287",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/81d11d0cba0813b49f675cb4eb673367b9e0cd7c">81d11d0</a></td><td><code>Trac #18901: continued fraction of real literals</code></td></tr></table>




---

archive/issue_comments_267288.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/18901](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/18901)** to **[public/18901](https://github.com/sagemath/sagetrac-mirror/tree/public/18901)**",
    "created_at": "2015-07-15T12:01:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267288",
    "user": "https://github.com/seblabbe"
}
```

Changed branch from **[u/vdelecroix/18901](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/18901)** to **[public/18901](https://github.com/sagemath/sagetrac-mirror/tree/public/18901)**



---

archive/issue_comments_267289.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI tested this ticket. The problem is fixed. The handling of real litteral is really done in the continued_fraction_list function, so I moved the new doctest there. I posted this changes in the branch public/18901. Tell me if you do not agree or set this to positive review.\n\nS\u00e9bastien\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e55a0d43c35d6e1aadc1818811918c111a1fed82\">e55a0d4</a></td><td><code>Trac #18901: moved new test to continued_fraction_list</code></td></tr></table>\n",
    "created_at": "2015-07-15T12:01:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267289",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:2" align="right">Comment 2</div>

I tested this ticket. The problem is fixed. The handling of real litteral is really done in the continued_fraction_list function, so I moved the new doctest there. I posted this changes in the branch public/18901. Tell me if you do not agree or set this to positive review.

Sébastien

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e55a0d43c35d6e1aadc1818811918c111a1fed82">e55a0d4</a></td><td><code>Trac #18901: moved new test to continued_fraction_list</code></td></tr></table>




---

archive/issue_comments_267290.json:
```json
{
    "body": "Changed commit from **[81d11d0](https://github.com/sagemath/sagetrac-mirror/commit/81d11d0cba0813b49f675cb4eb673367b9e0cd7c)** to **[e55a0d4](https://github.com/sagemath/sagetrac-mirror/commit/e55a0d43c35d6e1aadc1818811918c111a1fed82)**",
    "created_at": "2015-07-15T12:01:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267290",
    "user": "https://github.com/seblabbe"
}
```

Changed commit from **[81d11d0](https://github.com/sagemath/sagetrac-mirror/commit/81d11d0cba0813b49f675cb4eb673367b9e0cd7c)** to **[e55a0d4](https://github.com/sagemath/sagetrac-mirror/commit/e55a0d43c35d6e1aadc1818811918c111a1fed82)**



---

archive/issue_comments_267291.json:
```json
{
    "body": "Reviewer: **S\u00e9bastien Labb\u00e9**",
    "created_at": "2015-07-15T12:02:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267291",
    "user": "https://github.com/seblabbe"
}
```

Reviewer: **Sébastien Labbé**



---

archive/issue_comments_267292.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b8eedc4cff4964408608f897c842c8c479738d84\">b8eedc4</a></td><td><code>Trac #18901: readd part of test to continued_fraction</code></td></tr></table>\n",
    "created_at": "2015-07-15T12:09:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267292",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b8eedc4cff4964408608f897c842c8c479738d84">b8eedc4</a></td><td><code>Trac #18901: readd part of test to continued_fraction</code></td></tr></table>




---

archive/issue_comments_267293.json:
```json
{
    "body": "Changed commit from **[e55a0d4](https://github.com/sagemath/sagetrac-mirror/commit/e55a0d43c35d6e1aadc1818811918c111a1fed82)** to **[b8eedc4](https://github.com/sagemath/sagetrac-mirror/commit/b8eedc4cff4964408608f897c842c8c479738d84)**",
    "created_at": "2015-07-15T12:09:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267293",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e55a0d4](https://github.com/sagemath/sagetrac-mirror/commit/e55a0d43c35d6e1aadc1818811918c111a1fed82)** to **[b8eedc4](https://github.com/sagemath/sagetrac-mirror/commit/b8eedc4cff4964408608f897c842c8c479738d84)**



---

archive/issue_comments_267294.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nHello,\n\nThis is wrong\n\n```\nTESTS::\n\nFixed ...\n```\n\nIt should be\n\n```\nTESTS:\n\nFixed ...\n```\n\nVincent",
    "created_at": "2015-07-17T10:15:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267294",
    "user": "https://github.com/videlec"
}
```

<div id="comment:5" align="right">Comment 5</div>

Hello,

This is wrong

```
TESTS::

Fixed ...
```

It should be

```
TESTS:

Fixed ...
```

Vincent



---

archive/issue_events_250563.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-07-17T10:15:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250563"
}
```



---

archive/issue_events_250564.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-07-17T10:15:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250564"
}
```



---

archive/issue_comments_267295.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8d86cb325724472a7fe55e1a351ab5958b474ddf\">8d86cb3</a></td><td><code>Trac #18901: Fix ReST syntax</code></td></tr></table>\n",
    "created_at": "2015-07-25T20:08:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267295",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8d86cb325724472a7fe55e1a351ab5958b474ddf">8d86cb3</a></td><td><code>Trac #18901: Fix ReST syntax</code></td></tr></table>




---

archive/issue_comments_267296.json:
```json
{
    "body": "Changed commit from **[b8eedc4](https://github.com/sagemath/sagetrac-mirror/commit/b8eedc4cff4964408608f897c842c8c479738d84)** to **[8d86cb3](https://github.com/sagemath/sagetrac-mirror/commit/8d86cb325724472a7fe55e1a351ab5958b474ddf)**",
    "created_at": "2015-07-25T20:08:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267296",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b8eedc4](https://github.com/sagemath/sagetrac-mirror/commit/b8eedc4cff4964408608f897c842c8c479738d84)** to **[8d86cb3](https://github.com/sagemath/sagetrac-mirror/commit/8d86cb325724472a7fe55e1a351ab5958b474ddf)**



---

archive/issue_events_250565.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-07-25T20:25:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250565"
}
```



---

archive/issue_events_250566.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-07-25T20:25:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250566"
}
```



---

archive/issue_events_250567.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-07-28T08:05:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250567"
}
```



---

archive/issue_events_250568.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-07-28T08:05:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250568"
}
```



---

archive/issue_comments_267297.json:
```json
{
    "body": "Changed branch from **[public/18901](https://github.com/sagemath/sagetrac-mirror/tree/public/18901)** to **[8d86cb3](https://github.com/sagemath/sagetrac-mirror/commit/8d86cb325724472a7fe55e1a351ab5958b474ddf)**",
    "created_at": "2015-07-29T09:22:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18901#issuecomment-267297",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/18901](https://github.com/sagemath/sagetrac-mirror/tree/public/18901)** to **[8d86cb3](https://github.com/sagemath/sagetrac-mirror/commit/8d86cb325724472a7fe55e1a351ab5958b474ddf)**



---

archive/issue_events_250569.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-29T09:22:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250569"
}
```



---

archive/issue_events_250570.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "30db7a2328eee696c4d9e5f99f87c31ba64171eb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-07-29T09:22:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18901",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18901#event-250570"
}
```
