# Issue 18064: sage_doc crashes on cached_function when __doc__ is empty

archive/issues_017827.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: @cached_function\n....: def f():\n....:     return 3\nsage: f._sage_doc_()  # BOOM\n```\nAs a consequence, such a function would prevent the documentation to build.\n\nBranch/Commit: **[d0b2083](https://github.com/sagemath/sagetrac-mirror/commit/d0b2083b62bcfbf74b4cf2c26c7ce7bd9fc5d8e2)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18064_\n\n",
    "closed_at": "2015-04-02T00:14:53Z",
    "created_at": "2015-03-26T19:52:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage_doc crashes on cached_function when __doc__ is empty",
    "type": "issue",
    "updated_at": "2015-04-02T00:14:53Z",
    "url": "https://github.com/sagemath/sage/issues/18064",
    "user": "https://github.com/videlec"
}
```

```
sage: @cached_function
....: def f():
....:     return 3
sage: f._sage_doc_()  # BOOM
```
As a consequence, such a function would prevent the documentation to build.

Branch/Commit: **[d0b2083](https://github.com/sagemath/sagetrac-mirror/commit/d0b2083b62bcfbf74b4cf2c26c7ce7bd9fc5d8e2)**

Reviewer: **Nathann Cohen**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/18064_





---

archive/issue_events_239243.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-26T19:52:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239243"
}
```



---

archive/issue_events_239244.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-26T19:52:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239244"
}
```



---

archive/issue_events_239245.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-26T19:52:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239245"
}
```



---

archive/issue_events_239246.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-26T19:52:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239246"
}
```



---

archive/issue_comments_248689.json:
```json
{
    "body": "Branch: **[u/vdelecroix/18064](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/18064)**",
    "created_at": "2015-03-26T19:53:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248689",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/18064](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/18064)**



---

archive/issue_comments_248690.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/81d9a83a5ac76fa25b42cd04144c62341eadc3d6\">81d9a83</a></td><td><code>Trac 18064: fix cached_function._sage_doc_</code></td></tr></table>\n",
    "created_at": "2015-03-26T19:53:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248690",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/81d9a83a5ac76fa25b42cd04144c62341eadc3d6">81d9a83</a></td><td><code>Trac 18064: fix cached_function._sage_doc_</code></td></tr></table>




---

archive/issue_events_239247.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-26T19:53:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239247"
}
```



---

archive/issue_comments_248691.json:
```json
{
    "body": "Commit: **[81d9a83](https://github.com/sagemath/sagetrac-mirror/commit/81d9a83a5ac76fa25b42cd04144c62341eadc3d6)**",
    "created_at": "2015-03-26T19:53:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248691",
    "user": "https://github.com/videlec"
}
```

Commit: **[81d9a83](https://github.com/sagemath/sagetrac-mirror/commit/81d9a83a5ac76fa25b42cd04144c62341eadc3d6)**



---

archive/issue_comments_248692.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e436b6eb1d8b7e5ed689137a947cdbc8c9ccaf53\">e436b6e</a></td><td><code>Trac 18064: fix cached_function._sage_doc_</code></td></tr></table>\n",
    "created_at": "2015-03-26T19:54:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248692",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e436b6eb1d8b7e5ed689137a947cdbc8c9ccaf53">e436b6e</a></td><td><code>Trac 18064: fix cached_function._sage_doc_</code></td></tr></table>




---

archive/issue_comments_248693.json:
```json
{
    "body": "Changed commit from **[81d9a83](https://github.com/sagemath/sagetrac-mirror/commit/81d9a83a5ac76fa25b42cd04144c62341eadc3d6)** to **[e436b6e](https://github.com/sagemath/sagetrac-mirror/commit/e436b6eb1d8b7e5ed689137a947cdbc8c9ccaf53)**",
    "created_at": "2015-03-26T19:54:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248693",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[81d9a83](https://github.com/sagemath/sagetrac-mirror/commit/81d9a83a5ac76fa25b42cd04144c62341eadc3d6)** to **[e436b6e](https://github.com/sagemath/sagetrac-mirror/commit/e436b6eb1d8b7e5ed689137a947cdbc8c9ccaf53)**



---

archive/issue_events_239248.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-03-27T10:39:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239248"
}
```



---

archive/issue_events_239249.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-03-27T10:39:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239249"
}
```



---

archive/issue_comments_248694.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2015-03-27T10:39:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248694",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_239250.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-30T08:04:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239250"
}
```



---

archive/issue_events_239251.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-30T08:04:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239251"
}
```



---

archive/issue_comments_248695.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\n\n```\nsage -t --long src/sage/misc/cachefunc.pyx\n**********************************************************************\nFile \"src/sage/misc/cachefunc.pyx\", line 800, in sage.misc.cachefunc.CachedFunction._sage_doc_\nFailed example:\n    f._sage_doc_()\nExpected:\n    'File: <doctest ...> (starting at line 1)\\n'\nGot:\n    'File: /scratch/buildbot/sage/redhawk-1/sage_git/build/<doctest sage.misc.cachefunc.CachedFunction._sage_doc_[7]> (starting at line 1)\\n'\n**********************************************************************\n1 item had failures:\n   1 of  10 in sage.misc.cachefunc.CachedFunction._sage_doc_\n    [751 tests, 1 failure, 27.69 s]\n```",
    "created_at": "2015-03-30T08:04:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248695",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>


```
sage -t --long src/sage/misc/cachefunc.pyx
**********************************************************************
File "src/sage/misc/cachefunc.pyx", line 800, in sage.misc.cachefunc.CachedFunction._sage_doc_
Failed example:
    f._sage_doc_()
Expected:
    'File: <doctest ...> (starting at line 1)\n'
Got:
    'File: /scratch/buildbot/sage/redhawk-1/sage_git/build/<doctest sage.misc.cachefunc.CachedFunction._sage_doc_[7]> (starting at line 1)\n'
**********************************************************************
1 item had failures:
   1 of  10 in sage.misc.cachefunc.CachedFunction._sage_doc_
    [751 tests, 1 failure, 27.69 s]
```



---

archive/issue_comments_248696.json:
```json
{
    "body": "Changed commit from **[e436b6e](https://github.com/sagemath/sagetrac-mirror/commit/e436b6eb1d8b7e5ed689137a947cdbc8c9ccaf53)** to **[d0b2083](https://github.com/sagemath/sagetrac-mirror/commit/d0b2083b62bcfbf74b4cf2c26c7ce7bd9fc5d8e2)**",
    "created_at": "2015-03-30T08:19:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248696",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e436b6e](https://github.com/sagemath/sagetrac-mirror/commit/e436b6eb1d8b7e5ed689137a947cdbc8c9ccaf53)** to **[d0b2083](https://github.com/sagemath/sagetrac-mirror/commit/d0b2083b62bcfbf74b4cf2c26c7ce7bd9fc5d8e2)**



---

archive/issue_comments_248697.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d0b2083b62bcfbf74b4cf2c26c7ce7bd9fc5d8e2\">d0b2083</a></td><td><code>Trac 18064: fix doctest</code></td></tr></table>\n",
    "created_at": "2015-03-30T08:19:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248697",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d0b2083b62bcfbf74b4cf2c26c7ce7bd9fc5d8e2">d0b2083</a></td><td><code>Trac 18064: fix doctest</code></td></tr></table>




---

archive/issue_comments_248698.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nNow, there should not be any problem with the name of the file...",
    "created_at": "2015-03-30T08:22:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248698",
    "user": "https://github.com/videlec"
}
```

<div id="comment:6" align="right">Comment 6</div>

Now, there should not be any problem with the name of the file...



---

archive/issue_events_239252.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-30T08:22:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239252"
}
```



---

archive/issue_events_239253.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-30T08:22:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239253"
}
```



---

archive/issue_comments_248699.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/18064](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/18064)** to **[d0b2083](https://github.com/sagemath/sagetrac-mirror/commit/d0b2083b62bcfbf74b4cf2c26c7ce7bd9fc5d8e2)**",
    "created_at": "2015-04-02T00:14:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18064#issuecomment-248699",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/18064](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/18064)** to **[d0b2083](https://github.com/sagemath/sagetrac-mirror/commit/d0b2083b62bcfbf74b4cf2c26c7ce7bd9fc5d8e2)**



---

archive/issue_events_239254.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-02T00:14:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239254"
}
```



---

archive/issue_events_239255.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5e1051258071c2f0bf66396c9d23890a222f45b4",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-04-02T00:14:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18064",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18064#event-239255"
}
```
