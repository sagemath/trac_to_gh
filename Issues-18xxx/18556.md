# Issue 18556: FiniteStateMachine.is_deterministic: machines with >1 initial states are non-deterministic

archive/issues_018319.json:
```json
{
    "assignees": [],
    "body": "If `F` is a finite state machine with more than one initial state, `F.is_deterministic` should return `False`.\n\nCC:  skropf @dkrenn\n\nKeywords: **is_deterministic**\n\nBranch/Commit: **[ea021cc](https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c)**\n\nReviewer: **Sara Kropf**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18556_\n\n",
    "closed_at": "2015-07-31T17:52:25Z",
    "created_at": "2015-05-31T08:09:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "FiniteStateMachine.is_deterministic: machines with >1 initial states are non-deterministic",
    "type": "issue",
    "updated_at": "2015-07-31T17:52:25Z",
    "url": "https://github.com/sagemath/sage/issues/18556",
    "user": "https://github.com/cheuberg"
}
```
If `F` is a finite state machine with more than one initial state, `F.is_deterministic` should return `False`.

CC:  skropf @dkrenn

Keywords: **is_deterministic**

Branch/Commit: **[ea021cc](https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c)**

Reviewer: **Sara Kropf**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/18556_





---

archive/issue_events_246055.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-05-31T08:09:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18556#event-246055"
}
```



---

archive/issue_events_246056.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-05-31T08:09:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
    "label_color": "0000ff",
    "label_name": "component: finite state machines",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18556#event-246056"
}
```



---

archive/issue_events_246057.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-05-31T08:09:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18556#event-246057"
}
```



---

archive/issue_events_246058.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-05-31T08:09:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18556#event-246058"
}
```



---

archive/issue_events_246059.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-05-31T09:06:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18556#event-246059"
}
```



---

archive/issue_comments_260142.json:
```json
{
    "body": "Branch: **[fsm/is_deterministic](https://github.com/sagemath/sagetrac-mirror/tree/fsm/is_deterministic)**",
    "created_at": "2015-05-31T09:06:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18556#issuecomment-260142",
    "user": "https://github.com/cheuberg"
}
```

Branch: **[fsm/is_deterministic](https://github.com/sagemath/sagetrac-mirror/tree/fsm/is_deterministic)**



---

archive/issue_comments_260143.json:
```json
{
    "body": "Changed branch from **[fsm/is_deterministic](https://github.com/sagemath/sagetrac-mirror/tree/fsm/is_deterministic)** to **[u/cheuberg/fsm/is_deterministic](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/is_deterministic)**",
    "created_at": "2015-05-31T09:06:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18556#issuecomment-260143",
    "user": "https://github.com/cheuberg"
}
```

Changed branch from **[fsm/is_deterministic](https://github.com/sagemath/sagetrac-mirror/tree/fsm/is_deterministic)** to **[u/cheuberg/fsm/is_deterministic](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/is_deterministic)**



---

archive/issue_comments_260144.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c\">ea021cc</a></td><td><code>Trac #18556: FiniteStateMachine.is_deterministic: several initial states</code></td></tr></table>\n",
    "created_at": "2015-05-31T09:06:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18556#issuecomment-260144",
    "user": "https://github.com/cheuberg"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c">ea021cc</a></td><td><code>Trac #18556: FiniteStateMachine.is_deterministic: several initial states</code></td></tr></table>




---

archive/issue_comments_260145.json:
```json
{
    "body": "Commit: **[ea021cc](https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c)**",
    "created_at": "2015-05-31T09:06:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18556#issuecomment-260145",
    "user": "https://github.com/cheuberg"
}
```

Commit: **[ea021cc](https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c)**



---

archive/issue_comments_260146.json:
```json
{
    "body": "Reviewer: **Sara Kropf**",
    "created_at": "2015-07-29T17:12:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18556#issuecomment-260146",
    "user": "https://github.com/sagetrac-skropf"
}
```

Reviewer: **Sara Kropf**



---

archive/issue_events_246060.json:
```json
{
    "actor": "https://github.com/sagetrac-skropf",
    "created_at": "2015-07-29T17:12:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18556#event-246060"
}
```



---

archive/issue_events_246061.json:
```json
{
    "actor": "https://github.com/sagetrac-skropf",
    "created_at": "2015-07-29T17:12:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18556#event-246061"
}
```



---

archive/issue_comments_260147.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis is ok.",
    "created_at": "2015-07-29T17:12:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18556#issuecomment-260147",
    "user": "https://github.com/sagetrac-skropf"
}
```

<a id='comment:3'>Comment 3:</a>
This is ok.



---

archive/issue_comments_260148.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/is_deterministic](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/is_deterministic)** to **[ea021cc](https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c)**",
    "created_at": "2015-07-31T17:52:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18556#issuecomment-260148",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/cheuberg/fsm/is_deterministic](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/is_deterministic)** to **[ea021cc](https://github.com/sagemath/sagetrac-mirror/commit/ea021cc968a1028286ebc4b877bea2f8cb41d73c)**



---

archive/issue_events_246062.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-31T17:52:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18556#event-246062"
}
```



---

archive/issue_events_246063.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f4a0ac67b5ed6053f4398fe6c07ac9deca480e6d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-07-31T17:52:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18556",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18556#event-246063"
}
```
