...
<a id='comment:5'></a>Helloooooooo Michele,
...
<a id='comment:8'></a>Hello again Michele,
...
```json
{
    "body": "<a id='comment:9'></a>Oh, and something else I forgot: it is not sufficient to add the edges in order to get the graph: isolated vertices would not appear in this case.\n\nNathann",
...
<a id='comment:9'></a>Oh, and something else I forgot: it is not sufficient to add the edges in order to get the graph: isolated vertices would not appear in this case.
...
<a id='comment:12'></a>Hello again Michele,
...
```json
{
    "body": "<a id='comment:18'></a>> > In order to solve this problem, I have added a check in the spkg-install script, and I had to modify the sage-spkg file to allow an error which depends on the user. Do you agree with this modification?\n> > \n\n> \n> Oh. Well, I did not want to say that it had to be modified on our side. I merely wanted to tell you what had happened and that it had been reported upstream. For me it is not a very important problem, and for as long as upstream knows and fixes it I do not think that we should care `O_o`\n\n\nOk, I went back to the previous version: if it is not installed, an error explaining the problem is raised before any installation.",
...
<a id='comment:18'></a>> > In order to solve this problem, I have added a check in the spkg-install script, and I had to modify the sage-spkg file to allow an error which depends on the user. Do you agree with this modification?
> > 
...
```json
{
    "body": "<a id='comment:19'></a>Okayyyyyy, good !\n\nNathann",
    "created_at": "2015-07-28T15:10:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18929",
    "type": "issue_comment",
...
<a id='comment:19'></a>Okayyyyyy, good !
...
```json
{
    "body": "<a id='comment:20'></a>Wait a second: I was trying to make a better interface, that takes into account vertex/edge labels and weight... To be uploaded soon!",
...
<a id='comment:20'></a>Wait a second: I was trying to make a better interface, that takes into account vertex/edge labels and weight... To be uploaded soon!
...
<a id='comment:25'></a>Helooooooo Michele,
...
<a id='comment:29'></a>Did you follow the whole procedure to install igraph? In particular, you should do the following:
...
<a id='comment:30'></a>My first trial was with the source code from [1] and [2].
One difficulty is that [2] gives `python-igraph...` and not `python_igraph...` 
...
```json
{
    "body": "<a id='comment:32'></a>could you resolve the merge conflict with last beta (beta1). Thanks.",
...
<a id='comment:32'></a>could you resolve the merge conflict with last beta (beta1). Thanks.
...
```json
{
    "body": "<a id='comment:34'></a>> could you resolve the merge conflict with last beta (beta1). Thanks.\n\n\nDone!",
...
<a id='comment:34'></a>> could you resolve the merge conflict with last beta (beta1). Thanks.
...
<a id='comment:35'></a>> Hmmmmm, good question. I added igraph to the dependencies of python_igraph, but maybe it is not sufficient (see comments 4-5)... Nathann??
...
<a id='comment:36'></a>> I will now try to understand the point of disagreement between you and Nathann.
...
```json
{
    "body": "<a id='comment:39'></a>Sorry, wrong merge... Now it should be correct!",
...
<a id='comment:39'></a>Sorry, wrong merge... Now it should be correct!
...
<a id='comment:40'></a>One last thing: in the doc of the `igraph_graph` method, you should mention explicitely that it requires the optional package `python_igraph`. 
It does not appear on html page `sage/src/doc/output/html/en/reference/graphs/sage/graphs/generic_graph.html#sage.graphs.generic_graph.GenericGraph.igraph_graph`
...
```json
{
    "body": "<a id='comment:41'></a>sorry... needs work for the doc ;)",
...
<a id='comment:41'></a>sorry... needs work for the doc ;)
...
```json
{
    "body": "<a id='comment:43'></a>Done, and I have also modified the error message if python_igraph is not installed.",
...
<a id='comment:43'></a>Done, and I have also modified the error message if python_igraph is not installed.
...
```json
{
    "body": "<a id='comment:44'></a>Perfect!",
