# Issue 18899: Givaro testsuite fails in test-crt

archive/issues_018662.json:
```json
{
    "assignees": [],
    "body": "Randomly fails:\n\n```\nlibtool: link: g++ -O0 -Wall -g -DNDEBUG -UGIVARO_DEBUG -UDEBUG -I/mnt/disk/home/buildslave-sage/slave/sage_git/build/local//include -fPIC -I/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/include -o test-ratrecon test-ratrecon.o  -L../src -L../src/.libs /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/var/tmp/sage/build/givaro-3.7.1/src/src/.libs/libgivaro.a -L/mnt/disk/home/buildslave-sage/slave/sage_git/build/local//lib /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/libgmpxx.so /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/libgmp.so -Wl,-rpath -Wl,/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib -Wl,-rpath -Wl,/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib\nPASS: test-random\nPASS: test-ifactor\nPASS: test-ffarith\nPASS: test-ringarith\nPASS: test-mod\nPASS: test-modsqroot\nPASS: test-trunc\n/bin/sh: line 5: 16255 Floating point exception(core dumped) ${dir}$tst\nFAIL: test-crt\nPASS: test-geom\nPASS: test-integer\nPASS: test-conversion\nPASS: test-ratrecon\n=============================================\n1 of 12 tests failed\nPlease report to Jean-Guillaume.Dumas@imag.fr\n=============================================\nMakefile:604: recipe for target 'check-TESTS' failed\n```\n\nKeywords: **random_fail**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18899_\n\n",
    "created_at": "2015-07-15T07:26:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Givaro testsuite fails in test-crt",
    "type": "issue",
    "updated_at": "2022-12-29T01:32:56Z",
    "url": "https://github.com/sagemath/sage/issues/18899",
    "user": "https://github.com/vbraun"
}
```
Randomly fails:

```
libtool: link: g++ -O0 -Wall -g -DNDEBUG -UGIVARO_DEBUG -UDEBUG -I/mnt/disk/home/buildslave-sage/slave/sage_git/build/local//include -fPIC -I/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/include -o test-ratrecon test-ratrecon.o  -L../src -L../src/.libs /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/var/tmp/sage/build/givaro-3.7.1/src/src/.libs/libgivaro.a -L/mnt/disk/home/buildslave-sage/slave/sage_git/build/local//lib /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/libgmpxx.so /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/libgmp.so -Wl,-rpath -Wl,/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib -Wl,-rpath -Wl,/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib
PASS: test-random
PASS: test-ifactor
PASS: test-ffarith
PASS: test-ringarith
PASS: test-mod
PASS: test-modsqroot
PASS: test-trunc
/bin/sh: line 5: 16255 Floating point exception(core dumped) ${dir}$tst
FAIL: test-crt
PASS: test-geom
PASS: test-integer
PASS: test-conversion
PASS: test-ratrecon
=============================================
1 of 12 tests failed
Please report to Jean-Guillaume.Dumas@imag.fr
=============================================
Makefile:604: recipe for target 'check-TESTS' failed
```

Keywords: **random_fail**

_Issue created by migration from https://trac.sagemath.org/ticket/18899_





---

archive/issue_events_250548.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-15T07:26:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18899",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18899#event-250548"
}
```



---

archive/issue_events_250549.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-15T07:26:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18899",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18899#event-250549"
}
```



---

archive/issue_events_250550.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-15T07:26:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18899",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18899#event-250550"
}
```



---

archive/issue_events_250551.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-15T07:26:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18899",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18899#event-250551"
}
```



---

archive/issue_comments_267284.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Randomly segfaults\n+Randomly fails:\n \n ```\n libtool: link: g++ -O0 -Wall -g -DNDEBUG -UGIVARO_DEBUG -UDEBUG -I/mnt/disk/home/buildslave-sage/slave/sage_git/build/local//include -fPIC -I/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/include -o test-ratrecon test-ratrecon.o  -L../src -L../src/.libs /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/var/tmp/sage/build/givaro-3.7.1/src/src/.libs/libgivaro.a -L/mnt/disk/home/buildslave-sage/slave/sage_git/build/local//lib /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/libgmpxx.so /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/libgmp.so -Wl,-rpath -Wl,/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib -Wl,-rpath -Wl,/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib\n``````\n",
    "created_at": "2015-07-15T07:26:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18899#issuecomment-267284",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Randomly segfaults
+Randomly fails:
 
 ```
 libtool: link: g++ -O0 -Wall -g -DNDEBUG -UGIVARO_DEBUG -UDEBUG -I/mnt/disk/home/buildslave-sage/slave/sage_git/build/local//include -fPIC -I/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/include -o test-ratrecon test-ratrecon.o  -L../src -L../src/.libs /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/var/tmp/sage/build/givaro-3.7.1/src/src/.libs/libgivaro.a -L/mnt/disk/home/buildslave-sage/slave/sage_git/build/local//lib /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/libgmpxx.so /mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/libgmp.so -Wl,-rpath -Wl,/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib -Wl,-rpath -Wl,/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib
``````




---

archive/issue_events_250552.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:32:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18899",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18899#event-250552"
}
```
