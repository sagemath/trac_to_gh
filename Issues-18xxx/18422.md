# Issue 18422: Documentation bug for binary quadratic form action

archive/issues_018185.json:
```json
{
    "assignees": [],
    "body": "The documentation of `matrix_action_left` and `matrix_action_right` for binary quadratic forms are reversed.\n\n```\nsage: Q = BinaryQF([1,2,3]); Q\nx^2 + 2*x*y + 3*y^2\nsage: A = matrix(QQ,2,2,[1,2,3,4])\nsage: R.<x,y> = QQ[]\nsage: Q(x+2*y,3*x+4*y)\n34*x^2 + 96*x*y + 68*y^2\nsage: Q.matrix_action_left(A)\n17*x^2 + 74*x*y + 81*y^2\nsage: Q.matrix_action_left?\n...\nHere the action of the matrix `M = \\begin{pmatrix} a & b \\\\ c & d\n\\end{pmatrix}` on the form `Q(x, y)` produces the form `Q(ax+by,\ncx+dy)`.\n...\n```\n\nKeywords: **documentation**\n\nBranch/Commit: **[a57fad6](https://github.com/sagemath/sagetrac-mirror/commit/a57fad6f3cac6106d96b4e973903f02647258565)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Roe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18422_\n\n",
    "closed_at": "2015-05-14T22:40:02Z",
    "created_at": "2015-05-14T15:58:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Documentation bug for binary quadratic form action",
    "type": "issue",
    "updated_at": "2015-05-14T22:40:02Z",
    "url": "https://github.com/sagemath/sage/issues/18422",
    "user": "https://github.com/roed314"
}
```
The documentation of `matrix_action_left` and `matrix_action_right` for binary quadratic forms are reversed.

```
sage: Q = BinaryQF([1,2,3]); Q
x^2 + 2*x*y + 3*y^2
sage: A = matrix(QQ,2,2,[1,2,3,4])
sage: R.<x,y> = QQ[]
sage: Q(x+2*y,3*x+4*y)
34*x^2 + 96*x*y + 68*y^2
sage: Q.matrix_action_left(A)
17*x^2 + 74*x*y + 81*y^2
sage: Q.matrix_action_left?
...
Here the action of the matrix `M = \begin{pmatrix} a & b \\ c & d
\end{pmatrix}` on the form `Q(x, y)` produces the form `Q(ax+by,
cx+dy)`.
...
```

Keywords: **documentation**

Branch/Commit: **[a57fad6](https://github.com/sagemath/sagetrac-mirror/commit/a57fad6f3cac6106d96b4e973903f02647258565)**

Reviewer: **Frédéric Chapoton**

Author: **David Roe**

_Issue created by migration from https://trac.sagemath.org/ticket/18422_





---

archive/issue_events_244096.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2015-05-14T15:58:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18422#event-244096"
}
```



---

archive/issue_events_244097.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2015-05-14T15:58:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
    "label_color": "0000ff",
    "label_name": "component: quadratic forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18422#event-244097"
}
```



---

archive/issue_events_244098.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2015-05-14T15:58:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18422#event-244098"
}
```



---

archive/issue_events_244099.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2015-05-14T15:58:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18422#event-244099"
}
```



---

archive/issue_comments_256332.json:
```json
{
    "body": "Branch: **[u/roed/t/18422](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/t/18422)**",
    "created_at": "2015-05-14T15:59:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18422#issuecomment-256332",
    "user": "https://github.com/roed314"
}
```

Branch: **[u/roed/t/18422](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/t/18422)**



---

archive/issue_comments_256333.json:
```json
{
    "body": "Author: **David Roe**",
    "created_at": "2015-05-14T16:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18422#issuecomment-256333",
    "user": "https://github.com/roed314"
}
```

Author: **David Roe**



---

archive/issue_comments_256334.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a57fad6f3cac6106d96b4e973903f02647258565\">a57fad6</a></td><td><code>Fix documentation of binary quadratic form action</code></td></tr></table>\n",
    "created_at": "2015-05-14T16:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18422#issuecomment-256334",
    "user": "https://github.com/roed314"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a57fad6f3cac6106d96b4e973903f02647258565">a57fad6</a></td><td><code>Fix documentation of binary quadratic form action</code></td></tr></table>




---

archive/issue_comments_256335.json:
```json
{
    "body": "Changed keywords from **** to **documentation**",
    "created_at": "2015-05-14T16:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18422#issuecomment-256335",
    "user": "https://github.com/roed314"
}
```

Changed keywords from **** to **documentation**



---

archive/issue_comments_256336.json:
```json
{
    "body": "Commit: **[a57fad6](https://github.com/sagemath/sagetrac-mirror/commit/a57fad6f3cac6106d96b4e973903f02647258565)**",
    "created_at": "2015-05-14T16:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18422#issuecomment-256336",
    "user": "https://github.com/roed314"
}
```

Commit: **[a57fad6](https://github.com/sagemath/sagetrac-mirror/commit/a57fad6f3cac6106d96b4e973903f02647258565)**



---

archive/issue_events_244100.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2015-05-14T16:00:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18422#event-244100"
}
```



---

archive/issue_comments_256337.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nlgtm",
    "created_at": "2015-05-14T19:40:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18422#issuecomment-256337",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
lgtm



---

archive/issue_events_244101.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-05-14T19:40:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18422#event-244101"
}
```



---

archive/issue_events_244102.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-05-14T19:40:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18422#event-244102"
}
```



---

archive/issue_comments_256338.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2015-05-14T19:40:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18422#issuecomment-256338",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_256339.json:
```json
{
    "body": "Changed branch from **[u/roed/t/18422](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/t/18422)** to **[a57fad6](https://github.com/sagemath/sagetrac-mirror/commit/a57fad6f3cac6106d96b4e973903f02647258565)**",
    "created_at": "2015-05-14T22:40:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18422#issuecomment-256339",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/roed/t/18422](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/t/18422)** to **[a57fad6](https://github.com/sagemath/sagetrac-mirror/commit/a57fad6f3cac6106d96b4e973903f02647258565)**



---

archive/issue_events_244103.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-14T22:40:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18422#event-244103"
}
```



---

archive/issue_events_244104.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d6a9122c787292e9e3ebe4238f5e25d288b5182d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-05-14T22:40:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18422",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18422#event-244104"
}
```
