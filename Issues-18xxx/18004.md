# Issue 18004: Too small relative size of axes labels w.r.t. tick marks in 2D plots

archive/issues_017767.json:
```json
{
    "body": "Consider this plot:\n\n```\nsage: plot(sin(x^2), (x, -3, 3), axes_labels=['$x$','$y$'])\n```\n\nThe size of the labels x and y is quite small and barely readable when the figure is reproduced in a article with moderate size (cf. the attached figure).\nIn the current settings, the only way to enlarge it is via the optional argument fontsize:\n\n```\nsage: plot(sin(x^2), (x, -3, 3), axes_labels=['$x$','$y$'], fontsize=20)\n```\n\nBut then the tick marks are too large! \nOne should be able to set separately the size of the axes labels and that of the tick marks.\n\nCC:  @videlec @kcrisman\n\nKeywords: days64\n\nReviewer: Vincent Delecroix\n\nAuthor: Eric Gourgoulhon\n\nBranch: 69354fcb35c701ea3aa4311381acc4f8319cf85a\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/18004\n\n",
    "closed_at": "2015-04-21T00:10:55Z",
    "created_at": "2015-03-19T23:13:43Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.7",
    "title": "Too small relative size of axes labels w.r.t. tick marks in 2D plots",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/18004",
    "user": "https://github.com/egourgoulhon"
}
```
Consider this plot:

```
sage: plot(sin(x^2), (x, -3, 3), axes_labels=['$x$','$y$'])
```

The size of the labels x and y is quite small and barely readable when the figure is reproduced in a article with moderate size (cf. the attached figure).
In the current settings, the only way to enlarge it is via the optional argument fontsize:

```
sage: plot(sin(x^2), (x, -3, 3), axes_labels=['$x$','$y$'], fontsize=20)
```

But then the tick marks are too large! 
One should be able to set separately the size of the axes labels and that of the tick marks.

CC:  @videlec @kcrisman

Keywords: days64

Reviewer: Vincent Delecroix

Author: Eric Gourgoulhon

Branch: 69354fcb35c701ea3aa4311381acc4f8319cf85a

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/18004





---

archive/issue_comments_310226.json:
```json
{
    "body": "Changing commit from \"\" to \"10b69240dfceaec610564bb26e2ab7eb1a4a5b45\"",
    "created_at": "2015-03-19T23:27:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310226",
    "user": "https://github.com/egourgoulhon"
}
```

Changing commit from "" to "10b69240dfceaec610564bb26e2ab7eb1a4a5b45"



---

archive/issue_comments_310227.json:
```json
{
    "body": "Changing branch from \"\" to \"public/18004\"",
    "created_at": "2015-03-19T23:27:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310227",
    "user": "https://github.com/egourgoulhon"
}
```

Changing branch from "" to "public/18004"



---

archive/issue_comments_310228.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [axes_labels.png](tarball://root/attachments/some-uuid/ticket18004/axes_labels.png) by @egourgoulhon created at 2015-03-19 23:27:31\n\nNew commits:",
    "created_at": "2015-03-19T23:27:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310228",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:1'></a>Attachment [axes_labels.png](tarball://root/attachments/some-uuid/ticket18004/axes_labels.png) by @egourgoulhon created at 2015-03-19 23:27:31

New commits:



---

archive/issue_comments_310229.json:
```json
{
    "body": "<a id='comment:2'></a>The attached commit introduces an optional parameter for 2D graphics, `axes_labels_size`, which sets the size of axes labels relative to that of tick marks. The default value of this parameter has been set to 1.7 (the previous behavior would have corresponded to `axes_labels_size=1`.).",
    "created_at": "2015-03-19T23:34:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310229",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:2'></a>The attached commit introduces an optional parameter for 2D graphics, `axes_labels_size`, which sets the size of axes labels relative to that of tick marks. The default value of this parameter has been set to 1.7 (the previous behavior would have corresponded to `axes_labels_size=1`.).



---

archive/issue_comments_310230.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-03-19T23:36:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310230",
    "user": "https://github.com/egourgoulhon"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_310231.json:
```json
{
    "body": "<a id='comment:6'></a>Seems fine on the face of it but I truly have nearly zero Sage reviewing time these days.  Wait until school lets out and I'll have a mountain of reviews for you, I'm sure :-)",
    "created_at": "2015-03-24T17:39:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310231",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:6'></a>Seems fine on the face of it but I truly have nearly zero Sage reviewing time these days.  Wait until school lets out and I'll have a mountain of reviews for you, I'm sure :-)



---

archive/issue_comments_310232.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2015-04-11T13:25:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310232",
    "user": "https://github.com/videlec"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_310233.json:
```json
{
    "body": "<a id='comment:7'></a>Hello,\n\nIf you set\n\n```\n+ self._axes_labels_size = 1.7\n```\nin the class Graphics, why would you need this `try/except` block\n\n```\n+ try:\n+     return self._axes_labels_size\n+ except AttributeError:\n+     self._axes_labels_size = 1.7\n+     return self._axes_labels_size\n```\n\nVincent",
    "created_at": "2015-04-11T13:25:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310233",
    "user": "https://github.com/videlec"
}
```

<a id='comment:7'></a>Hello,

If you set

```
+ self._axes_labels_size = 1.7
```
in the class Graphics, why would you need this `try/except` block

```
+ try:
+     return self._axes_labels_size
+ except AttributeError:
+     self._axes_labels_size = 1.7
+     return self._axes_labels_size
```

Vincent



---

archive/issue_comments_310234.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Vincent Delecroix\"",
    "created_at": "2015-04-11T13:26:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310234",
    "user": "https://github.com/videlec"
}
```

Changing reviewer from "" to "Vincent Delecroix"



---

archive/issue_comments_310235.json:
```json
{
    "body": "<a id='comment:9'></a>Hi Vincent,\n\nYou are perfectly right! I am going to change this.\nBtw, the same occurs for the attribute `_fontsize` (probably I did a copy-and-paste of the latter...): there is\n\n```\nself._fontsize = 10\n```\nin `Graphics.__init__` and\n\n```\n            try:\n                return self._fontsize\n            except AttributeError:\n                self._fontsize = 10\n                return self._fontsize\n```\nin `Graphics.fontsize`. \nShall we correct it as well, although this does not pertain to the current ticket ? \n\nReplying to [vdelecroix](#comment%3A7):\n> Hello,\n> \n> If you set\n> \n> ```\n> + self._axes_labels_size = 1.7\n> ```\n> in the class Graphics, why would you need this `try/except` block\n> \n> ```\n> + try:\n> +     return self._axes_labels_size\n> + except AttributeError:\n> +     self._axes_labels_size = 1.7\n> +     return self._axes_labels_size\n> ```\n> \n> Vincent",
    "created_at": "2015-04-11T14:08:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310235",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:9'></a>Hi Vincent,

You are perfectly right! I am going to change this.
Btw, the same occurs for the attribute `_fontsize` (probably I did a copy-and-paste of the latter...): there is

```
self._fontsize = 10
```
in `Graphics.__init__` and

```
            try:
                return self._fontsize
            except AttributeError:
                self._fontsize = 10
                return self._fontsize
```
in `Graphics.fontsize`. 
Shall we correct it as well, although this does not pertain to the current ticket ? 

Replying to [vdelecroix](#comment%3A7):
> Hello,
> 
> If you set
> 
> ```
> + self._axes_labels_size = 1.7
> ```
> in the class Graphics, why would you need this `try/except` block
> 
> ```
> + try:
> +     return self._axes_labels_size
> + except AttributeError:
> +     self._axes_labels_size = 1.7
> +     return self._axes_labels_size
> ```
> 
> Vincent



---

archive/issue_comments_310236.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [egourgoulhon](#comment%3A9):\n> Hi Vincent,\n> \n> You are perfectly right! I am going to change this.\n> Btw, the same occurs for the attribute `_fontsize` (probably I did a copy-and-paste of the latter...): there is\n> \n> ```\n> self._fontsize = 10\n> ```\n> in `Graphics.__init__` and\n> \n> ```\n>             try:\n>                 return self._fontsize\n>             except AttributeError:\n>                 self._fontsize = 10\n>                 return self._fontsize\n> ```\n> in `Graphics.fontsize`. \n> Shall we correct it as well, although this does not pertain to the current ticket ? \n\n\nSure. Do it at the same time.\n\nVincent",
    "created_at": "2015-04-11T14:09:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310236",
    "user": "https://github.com/videlec"
}
```

<a id='comment:10'></a>Replying to [egourgoulhon](#comment%3A9):
> Hi Vincent,
> 
> You are perfectly right! I am going to change this.
> Btw, the same occurs for the attribute `_fontsize` (probably I did a copy-and-paste of the latter...): there is
> 
> ```
> self._fontsize = 10
> ```
> in `Graphics.__init__` and
> 
> ```
>             try:
>                 return self._fontsize
>             except AttributeError:
>                 self._fontsize = 10
>                 return self._fontsize
> ```
> in `Graphics.fontsize`. 
> Shall we correct it as well, although this does not pertain to the current ticket ? 


Sure. Do it at the same time.

Vincent



---

archive/issue_comments_310237.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [vdelecroix](#comment%3A10):\n> \n> > Shall we correct it as well, although this does not pertain to the current ticket ? \n\n> \n> Sure. Do it at the same time.\n\n\nA systematic search for `AttributeError` in graphics.py reveals that the same issue occurs for the attributes\n`_show_axes`, `_axes_color`, `_axes_label_color`, `_axes_width` and `_tick_label_colors`.\nSo I guess we shall correct these as well?... The only possible justification is that the current implementation is more robust in case of a derived class of `Graphics`, `A` let us say, such that `A.__init__()` does not call `Graphics.__init__()` (but this is bad design IMHO).  \n\nEric.",
    "created_at": "2015-04-11T14:42:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310237",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:11'></a>Replying to [vdelecroix](#comment%3A10):
> 
> > Shall we correct it as well, although this does not pertain to the current ticket ? 

> 
> Sure. Do it at the same time.


A systematic search for `AttributeError` in graphics.py reveals that the same issue occurs for the attributes
`_show_axes`, `_axes_color`, `_axes_label_color`, `_axes_width` and `_tick_label_colors`.
So I guess we shall correct these as well?... The only possible justification is that the current implementation is more robust in case of a derived class of `Graphics`, `A` let us say, such that `A.__init__()` does not call `Graphics.__init__()` (but this is bad design IMHO).  

Eric.



---

archive/issue_comments_310238.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [egourgoulhon](#comment%3A11):\n> Replying to [vdelecroix](#comment%3A10):\n> > \n> > > Shall we correct it as well, although this does not pertain to the current ticket ? \n\n> > \n> > Sure. Do it at the same time.\n  \n> \n> A systematic search for `AttributeError` in graphics.py reveals that the same issue occurs for the attributes\n> `_show_axes`, `_axes_color`, `_axes_label_color`, `_axes_width` and `_tick_label_colors`.\n> So I guess we shall correct these as well?... The only possible justification is that the current implementation is more robust in case of a derived class of `Graphics`, `A` let us say, such that `A.__init__()` does not call `Graphics.__init__()` (but this is bad design IMHO).  \n\n\nRight. The simplest would be to let it as you initially did... sorry.\n\nVincent",
    "created_at": "2015-04-11T14:44:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310238",
    "user": "https://github.com/videlec"
}
```

<a id='comment:12'></a>Replying to [egourgoulhon](#comment%3A11):
> Replying to [vdelecroix](#comment%3A10):
> > 
> > > Shall we correct it as well, although this does not pertain to the current ticket ? 

> > 
> > Sure. Do it at the same time.
  
> 
> A systematic search for `AttributeError` in graphics.py reveals that the same issue occurs for the attributes
> `_show_axes`, `_axes_color`, `_axes_label_color`, `_axes_width` and `_tick_label_colors`.
> So I guess we shall correct these as well?... The only possible justification is that the current implementation is more robust in case of a derived class of `Graphics`, `A` let us say, such that `A.__init__()` does not call `Graphics.__init__()` (but this is bad design IMHO).  


Right. The simplest would be to let it as you initially did... sorry.

Vincent



---

archive/issue_comments_310239.json:
```json
{
    "body": "<a id='comment:13'></a>Replying to [vdelecroix](#comment%3A12):\n> Replying to [egourgoulhon](#comment%3A11):\n> > Replying to [vdelecroix](#comment%3A10):\n> > > \n> > > > Shall we correct it as well, although this does not pertain to the current ticket ? \n\n> > > \n> > > Sure. Do it at the same time.\n  \n> > \n> > A systematic search for `AttributeError` in graphics.py reveals that the same issue occurs for the attributes\n> > `_show_axes`, `_axes_color`, `_axes_label_color`, `_axes_width` and `_tick_label_colors`.\n> > So I guess we shall correct these as well?... The only possible justification is that the current implementation is more robust in case of a derived class of `Graphics`, `A` let us say, such that `A.__init__()` does not call `Graphics.__init__()` (but this is bad design IMHO).  \n\n> \n> Right. The simplest would be to let it as you initially did... sorry.\n\n\nOK. \nBtw note that the case of `_axes_width` looks strange: in `Graphics.__init__()`, one has\n\n```\n        self._axes_width = 0.8\n```\nwhile in `Graphics.axes_width()` there is\n\n```\n            try:\n                return self._axes_width\n            except AttributeError:\n                self._axes_width = True\n                return self._axes_width\n```\nShouldn't it be there `self._axes_width = 0.8` for consistency ? \n\nEric.\n\n\n> \n> Vincent",
    "created_at": "2015-04-11T15:32:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310239",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:13'></a>Replying to [vdelecroix](#comment%3A12):
> Replying to [egourgoulhon](#comment%3A11):
> > Replying to [vdelecroix](#comment%3A10):
> > > 
> > > > Shall we correct it as well, although this does not pertain to the current ticket ? 

> > > 
> > > Sure. Do it at the same time.
  
> > 
> > A systematic search for `AttributeError` in graphics.py reveals that the same issue occurs for the attributes
> > `_show_axes`, `_axes_color`, `_axes_label_color`, `_axes_width` and `_tick_label_colors`.
> > So I guess we shall correct these as well?... The only possible justification is that the current implementation is more robust in case of a derived class of `Graphics`, `A` let us say, such that `A.__init__()` does not call `Graphics.__init__()` (but this is bad design IMHO).  

> 
> Right. The simplest would be to let it as you initially did... sorry.


OK. 
Btw note that the case of `_axes_width` looks strange: in `Graphics.__init__()`, one has

```
        self._axes_width = 0.8
```
while in `Graphics.axes_width()` there is

```
            try:
                return self._axes_width
            except AttributeError:
                self._axes_width = True
                return self._axes_width
```
Shouldn't it be there `self._axes_width = 0.8` for consistency ? 

Eric.


> 
> Vincent



---

archive/issue_comments_310240.json:
```json
{
    "body": "<a id='comment:14'></a>Replying to [egourgoulhon](#comment%3A13):\n> Shouldn't it be there `self._axes_width = 0.8` for consistency ? \n\n\nTrue. But I suggest to not do it here and open a new ticket and get rid of all these try/except.\n\nVincent",
    "created_at": "2015-04-11T16:13:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310240",
    "user": "https://github.com/videlec"
}
```

<a id='comment:14'></a>Replying to [egourgoulhon](#comment%3A13):
> Shouldn't it be there `self._axes_width = 0.8` for consistency ? 


True. But I suggest to not do it here and open a new ticket and get rid of all these try/except.

Vincent



---

archive/issue_comments_310241.json:
```json
{
    "body": "<a id='comment:15'></a>Hi,\n\nAnother point that could be discussed is the default value of the new parameter `axes_labels_size`: I've set it to 1.7 for it looks good for small labels like $x$ and $y$, but maybe it can be lowered a little bit, to 1.6 or 1.5 say, to accomodate for larger labels. Of course, this can be adjusted by the user (the whole point of this ticket!), but it's always nice if the default value fits most usages.",
    "created_at": "2015-04-15T11:41:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310241",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:15'></a>Hi,

Another point that could be discussed is the default value of the new parameter `axes_labels_size`: I've set it to 1.7 for it looks good for small labels like $x$ and $y$, but maybe it can be lowered a little bit, to 1.6 or 1.5 say, to accomodate for larger labels. Of course, this can be adjusted by the user (the whole point of this ticket!), but it's always nice if the default value fits most usages.



---

archive/issue_events_051163.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2015-04-15T11:41:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "milestone": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18004#event-51163"
}
```



---

archive/issue_comments_310242.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2015-04-20T05:47:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310242",
    "user": "https://github.com/egourgoulhon"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_310243.json:
```json
{
    "body": "<a id='comment:17'></a>Hello Eric,\n\nYou added two trailing whitespaces! This is very bad.\n\nThe problem with the right size is that it depends if the label is in math mode (like `$x$`) or not (like `x`). I would say that:\n- 1.4 is a good default for non math mode\n- 1.7 is a good default for math mode\n\nI just tried\n\n```\nplot(x^2, (x,0,1), axes_labels=('x','$x$'), axes_labels_size=1.7)\n```\nThere is a big difference in the aspects of both letters!\n\nWhat do people use, `label='x'` or `label='$x$'`?\n\nVincent",
    "created_at": "2015-04-20T09:32:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310243",
    "user": "https://github.com/videlec"
}
```

<a id='comment:17'></a>Hello Eric,

You added two trailing whitespaces! This is very bad.

The problem with the right size is that it depends if the label is in math mode (like `$x$`) or not (like `x`). I would say that:
- 1.4 is a good default for non math mode
- 1.7 is a good default for math mode

I just tried

```
plot(x^2, (x,0,1), axes_labels=('x','$x$'), axes_labels_size=1.7)
```
There is a big difference in the aspects of both letters!

What do people use, `label='x'` or `label='$x$'`?

Vincent



---

archive/issue_comments_310244.json:
```json
{
    "body": "<a id='comment:18'></a>Hi Vincent, \n\nReplying to [vdelecroix](#comment%3A17):\n> \n> You added two trailing whitespaces! This is very bad.\n\n\nSorry about this. Yet I've configured my editor to automatically removing trailing whitespaces while saving a file. Apparently two of them went through...\n\n> \n> The problem with the right size is that it depends if the label is in math mode (like `$x$`) or not (like `x`). I would say that:\n> - 1.4 is a good default for non math mode\n> - 1.7 is a good default for math mode\n> \n> I just tried\n> \n> ```\n> plot(x^2, (x,0,1), axes_labels=('x','$x$'), axes_labels_size=1.7)\n> ```\n> There is a big difference in the aspects of both letters!\n\n\nYou are right!\n\n> \n> What do people use, `label='x'` or `label='$x$'`?\n> \n\n\nProbably it depends, but if you want to have nice labels (in particular labels that coincide with the symbols used in the main text of your article or labels with subscripts or Greek letters), LaTeX seems mandatory. So I would favor the math mode. Maybe lowered to 1.6 as a compromise?...\n \nEric.",
    "created_at": "2015-04-20T11:43:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310244",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:18'></a>Hi Vincent, 

Replying to [vdelecroix](#comment%3A17):
> 
> You added two trailing whitespaces! This is very bad.


Sorry about this. Yet I've configured my editor to automatically removing trailing whitespaces while saving a file. Apparently two of them went through...

> 
> The problem with the right size is that it depends if the label is in math mode (like `$x$`) or not (like `x`). I would say that:
> - 1.4 is a good default for non math mode
> - 1.7 is a good default for math mode
> 
> I just tried
> 
> ```
> plot(x^2, (x,0,1), axes_labels=('x','$x$'), axes_labels_size=1.7)
> ```
> There is a big difference in the aspects of both letters!


You are right!

> 
> What do people use, `label='x'` or `label='$x$'`?
> 


Probably it depends, but if you want to have nice labels (in particular labels that coincide with the symbols used in the main text of your article or labels with subscripts or Greek letters), LaTeX seems mandatory. So I would favor the math mode. Maybe lowered to 1.6 as a compromise?...
 
Eric.



---

archive/issue_comments_310245.json:
```json
{
    "body": "<a id='comment:19'></a>Replying to [egourgoulhon](#comment%3A18):\n> Replying to [vdelecroix](#comment%3A17):\n> So I would favor the math mode. Maybe lowered to 1.6 as a compromise?...\n\n\nLooks reasonable to me.\n\nVincent",
    "created_at": "2015-04-20T11:47:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310245",
    "user": "https://github.com/videlec"
}
```

<a id='comment:19'></a>Replying to [egourgoulhon](#comment%3A18):
> Replying to [vdelecroix](#comment%3A17):
> So I would favor the math mode. Maybe lowered to 1.6 as a compromise?...


Looks reasonable to me.

Vincent



---

archive/issue_comments_310246.json:
```json
{
    "body": "<a id='comment:20'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-04-20T15:31:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310246",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:20'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_310247.json:
```json
{
    "body": "Changing commit from \"10b69240dfceaec610564bb26e2ab7eb1a4a5b45\" to \"69354fcb35c701ea3aa4311381acc4f8319cf85a\"",
    "created_at": "2015-04-20T15:31:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310247",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "10b69240dfceaec610564bb26e2ab7eb1a4a5b45" to "69354fcb35c701ea3aa4311381acc4f8319cf85a"



---

archive/issue_comments_310248.json:
```json
{
    "body": "<a id='comment:21'></a>Replying to [vdelecroix](#comment%3A19):\n> Replying to [egourgoulhon](#comment%3A18):\n> > Replying to [vdelecroix](#comment%3A17):\n> > So I would favor the math mode. Maybe lowered to 1.6 as a compromise?...\n\n> \n> Looks reasonable to me.\n\n\nAll right; in the new commit, I've set the default value to 1.6. \n\nEric.",
    "created_at": "2015-04-20T15:34:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310248",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:21'></a>Replying to [vdelecroix](#comment%3A19):
> Replying to [egourgoulhon](#comment%3A18):
> > Replying to [vdelecroix](#comment%3A17):
> > So I would favor the math mode. Maybe lowered to 1.6 as a compromise?...

> 
> Looks reasonable to me.


All right; in the new commit, I've set the default value to 1.6. 

Eric.



---

archive/issue_comments_310249.json:
```json
{
    "body": "<a id='comment:22'></a>Good to go then. Thanks.\n\nVincent",
    "created_at": "2015-04-20T16:00:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310249",
    "user": "https://github.com/videlec"
}
```

<a id='comment:22'></a>Good to go then. Thanks.

Vincent



---

archive/issue_comments_310250.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-04-20T16:00:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310250",
    "user": "https://github.com/videlec"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_310251.json:
```json
{
    "body": "<a id='comment:23'></a>Thanks for the review Vincent!",
    "created_at": "2015-04-20T19:23:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310251",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:23'></a>Thanks for the review Vincent!



---

archive/issue_events_051164.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-21T00:10:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18004#event-51164"
}
```



---

archive/issue_comments_310252.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-04-21T00:10:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310252",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_310253.json:
```json
{
    "body": "Changing branch from \"public/18004\" to \"69354fcb35c701ea3aa4311381acc4f8319cf85a\"",
    "created_at": "2015-04-21T00:10:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310253",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/18004" to "69354fcb35c701ea3aa4311381acc4f8319cf85a"



---

archive/issue_comments_310254.json:
```json
{
    "body": "Changing commit from \"69354fcb35c701ea3aa4311381acc4f8319cf85a\" to \"\"",
    "created_at": "2015-05-14T15:00:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310254",
    "user": "https://github.com/kcrisman"
}
```

Changing commit from "69354fcb35c701ea3aa4311381acc4f8319cf85a" to ""



---

archive/issue_comments_310255.json:
```json
{
    "body": "<a id='comment:25'></a>See #18421 for followup.",
    "created_at": "2015-05-14T15:00:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18004",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18004#issuecomment-310255",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:25'></a>See #18421 for followup.
