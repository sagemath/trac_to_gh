...
```json
{
    "body": "<a id='comment:16'></a>Before we close it as a duplicate, it would be good to do some comparison timings IMO as Vincent suggested in[comment:12](#comment%3A12). Or does the PARI version use the same algorithm?",
...
<a id='comment:16'></a>Before we close it as a duplicate, it would be good to do some comparison timings IMO as Vincent suggested in[comment:12](#comment%3A12). Or does the PARI version use the same algorithm?
