# Issue 18969: SageTeX fails its test suite

archive/issues_018732.json:
```json
{
    "body": "When building with `SAGE_CHECK=yes` I am getting in the log\n\n```\n...\nPlot 5\nCode block begin...sys:1: DeprecationWarning: The use of coeffs() is now deprecated in favor of coefficients(sparse=False).\nSee http://trac.sagemath.org/17518 for details.\nend\nPlot 6\nInline formula 13\nCode block begin...Traceback (most recent call last):\n  File \"example.sagetex.sage.py\", line 192, in <module>\n    _st_.plot(_sage_const_7 , format='notprovided', _p_=plot3d(sin(pi*(x**_sage_const_2 +y**_sage_const_2 ))/_sage_const_2 ,(x,-_sage_const_1 ,_sage_const_1 ),(y,-_sage_const_1 ,_sage_const_1 )))\n  File \"/home/novoselt/sage/local/var/tmp/sage/build/sagetex-2.3.4/src/sagetex.py\", line 252, in plot\n    _p_.save(filename=plotfilename, **kwargs)\n  File \"sage/plot/plot3d/base.pyx\", line 1530, in sage.plot.plot3d.base.Graphics3d.save (build/cythonized/sage/plot/plot3d/base.c:17649)\n    raise ValueError('filetype {} not supported by save()'.format(ext))\nValueError: filetype .eps not supported by save()\nend\nPlot 7\n\n**** Error in Sage code on line 278 of example.tex! Traceback follows.\n\n**** Running Sage on example.sage failed! Fix example.tex and try again.\nError running Sage on example.sagetex.sage!\n```\n\n**CC:**  @dandrake\n\nIssue created by migration from https://trac.sagemath.org/ticket/18969\n\n",
    "closed_at": "2015-09-12T14:04:32Z",
    "created_at": "2015-07-30T00:42:22Z",
    "labels": [
        "component: packages: standard",
        "bug",
        "duplicate"
    ],
    "title": "SageTeX fails its test suite",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/18969",
    "user": "https://github.com/novoselt"
}
```
When building with `SAGE_CHECK=yes` I am getting in the log

```
...
Plot 5
Code block begin...sys:1: DeprecationWarning: The use of coeffs() is now deprecated in favor of coefficients(sparse=False).
See http://trac.sagemath.org/17518 for details.
end
Plot 6
Inline formula 13
Code block begin...Traceback (most recent call last):
  File "example.sagetex.sage.py", line 192, in <module>
    _st_.plot(_sage_const_7 , format='notprovided', _p_=plot3d(sin(pi*(x**_sage_const_2 +y**_sage_const_2 ))/_sage_const_2 ,(x,-_sage_const_1 ,_sage_const_1 ),(y,-_sage_const_1 ,_sage_const_1 )))
  File "/home/novoselt/sage/local/var/tmp/sage/build/sagetex-2.3.4/src/sagetex.py", line 252, in plot
    _p_.save(filename=plotfilename, **kwargs)
  File "sage/plot/plot3d/base.pyx", line 1530, in sage.plot.plot3d.base.Graphics3d.save (build/cythonized/sage/plot/plot3d/base.c:17649)
    raise ValueError('filetype {} not supported by save()'.format(ext))
ValueError: filetype .eps not supported by save()
end
Plot 7

**** Error in Sage code on line 278 of example.tex! Traceback follows.

**** Running Sage on example.sage failed! Fix example.tex and try again.
Error running Sage on example.sagetex.sage!
```

**CC:**  @dandrake

Issue created by migration from https://trac.sagemath.org/ticket/18969





---

archive/issue_comments_270656.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis has been fixed: https://github.com/dandrake/sagetex/commit/8bffdfdc32a1cd36e82900712349200f7aa7e43b",
    "created_at": "2015-08-22T18:42:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18969#issuecomment-270656",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:1'></a>
This has been fixed: https://github.com/dandrake/sagetex/commit/8bffdfdc32a1cd36e82900712349200f7aa7e43b



---

archive/issue_comments_270657.json:
```json
{
    "body": "<a id='comment:2'></a>\n#19072 is the upgrade ticket; if that ticket is closed, this should be too.",
    "created_at": "2015-08-22T18:50:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18969#issuecomment-270657",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:2'></a>
#19072 is the upgrade ticket; if that ticket is closed, this should be too.



---

archive/issue_events_173966.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-08-27T01:31:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "label": "bug",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18969#event-173966"
}
```



---

archive/issue_events_173967.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18969#event-173967"
}
```



---

archive/issue_events_173968.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "milestone": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18969#event-173968"
}
```



---

archive/issue_events_173969.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18969#event-173969"
}
```



---

archive/issue_events_173970.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18969#event-173970"
}
```



---

archive/issue_events_173971.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18969#event-173971"
}
```



---

archive/issue_events_173972.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:04:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18969#event-173972"
}
```



---

archive/issue_events_173973.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:04:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18969#event-173973"
}
```



---

archive/issue_events_173974.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:04:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/18969",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18969#event-173974"
}
```
