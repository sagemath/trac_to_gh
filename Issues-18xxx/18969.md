# Issue 18969: SageTeX fails its test suite

archive/issues_018732.json:
```json
{
    "assignees": [],
    "body": "When building with `SAGE_CHECK=yes` I am getting in the log\n\n```\n...\nPlot 5\nCode block begin...sys:1: DeprecationWarning: The use of coeffs() is now deprecated in favor of coefficients(sparse=False).\nSee http://trac.sagemath.org/17518 for details.\nend\nPlot 6\nInline formula 13\nCode block begin...Traceback (most recent call last):\n  File \"example.sagetex.sage.py\", line 192, in <module>\n    _st_.plot(_sage_const_7 , format='notprovided', _p_=plot3d(sin(pi*(x**_sage_const_2 +y**_sage_const_2 ))/_sage_const_2 ,(x,-_sage_const_1 ,_sage_const_1 ),(y,-_sage_const_1 ,_sage_const_1 )))\n  File \"/home/novoselt/sage/local/var/tmp/sage/build/sagetex-2.3.4/src/sagetex.py\", line 252, in plot\n    _p_.save(filename=plotfilename, **kwargs)\n  File \"sage/plot/plot3d/base.pyx\", line 1530, in sage.plot.plot3d.base.Graphics3d.save (build/cythonized/sage/plot/plot3d/base.c:17649)\n    raise ValueError('filetype {} not supported by save()'.format(ext))\nValueError: filetype .eps not supported by save()\nend\nPlot 7\n\n**** Error in Sage code on line 278 of example.tex! Traceback follows.\n\n**** Running Sage on example.sage failed! Fix example.tex and try again.\nError running Sage on example.sagetex.sage!\n```\n\nCC:  @dandrake\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18969_\n\n",
    "closed_at": "2015-09-12T14:04:32Z",
    "created_at": "2015-07-30T00:42:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SageTeX fails its test suite",
    "type": "issue",
    "updated_at": "2015-09-12T14:04:32Z",
    "url": "https://github.com/sagemath/sage/issues/18969",
    "user": "https://github.com/novoselt"
}
```
When building with `SAGE_CHECK=yes` I am getting in the log

```
...
Plot 5
Code block begin...sys:1: DeprecationWarning: The use of coeffs() is now deprecated in favor of coefficients(sparse=False).
See http://trac.sagemath.org/17518 for details.
end
Plot 6
Inline formula 13
Code block begin...Traceback (most recent call last):
  File "example.sagetex.sage.py", line 192, in <module>
    _st_.plot(_sage_const_7 , format='notprovided', _p_=plot3d(sin(pi*(x**_sage_const_2 +y**_sage_const_2 ))/_sage_const_2 ,(x,-_sage_const_1 ,_sage_const_1 ),(y,-_sage_const_1 ,_sage_const_1 )))
  File "/home/novoselt/sage/local/var/tmp/sage/build/sagetex-2.3.4/src/sagetex.py", line 252, in plot
    _p_.save(filename=plotfilename, **kwargs)
  File "sage/plot/plot3d/base.pyx", line 1530, in sage.plot.plot3d.base.Graphics3d.save (build/cythonized/sage/plot/plot3d/base.c:17649)
    raise ValueError('filetype {} not supported by save()'.format(ext))
ValueError: filetype .eps not supported by save()
end
Plot 7

**** Error in Sage code on line 278 of example.tex! Traceback follows.

**** Running Sage on example.sage failed! Fix example.tex and try again.
Error running Sage on example.sagetex.sage!
```

CC:  @dandrake

_Issue created by migration from https://trac.sagemath.org/ticket/18969_





---

archive/issue_events_251513.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-07-30T00:42:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251513"
}
```



---

archive/issue_events_251514.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-07-30T00:42:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251514"
}
```



---

archive/issue_events_251515.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-07-30T00:42:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251515"
}
```



---

archive/issue_comments_269136.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis has been fixed: https://github.com/dandrake/sagetex/commit/8bffdfdc32a1cd36e82900712349200f7aa7e43b",
    "created_at": "2015-08-22T18:42:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18969#issuecomment-269136",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:1'>Comment 1:</a>
This has been fixed: https://github.com/dandrake/sagetex/commit/8bffdfdc32a1cd36e82900712349200f7aa7e43b



---

archive/issue_comments_269137.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\n#19072 is the upgrade ticket; if that ticket is closed, this should be too.",
    "created_at": "2015-08-22T18:50:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18969#issuecomment-269137",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:2'>Comment 2:</a>
#19072 is the upgrade ticket; if that ticket is closed, this should be too.



---

archive/issue_events_251516.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-08-27T01:31:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251516"
}
```



---

archive/issue_events_251517.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251517"
}
```



---

archive/issue_events_251518.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251518"
}
```



---

archive/issue_events_251519.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251519"
}
```



---

archive/issue_events_251520.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251520"
}
```



---

archive/issue_events_251521.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251521"
}
```



---

archive/issue_events_251522.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2015-09-04T23:12:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251522"
}
```



---

archive/issue_events_251523.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:04:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251523"
}
```



---

archive/issue_events_251524.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:04:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251524"
}
```



---

archive/issue_events_251525.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:04:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18969",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18969#event-251525"
}
```
