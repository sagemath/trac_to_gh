# Issue 18256: Recythonize only if source files really changed

archive/issues_018019.json:
```json
{
    "body": "Cython uses timestamps for dependency checking, often causing recythonizing when it's not needed.\n\nReviewer: Jeroen Demeyer\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/18256\n\n",
    "closed_at": "2015-04-22T00:07:44Z",
    "created_at": "2015-04-20T07:14:36Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Recythonize only if source files really changed",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/18256",
    "user": "https://github.com/rwst"
}
```
Cython uses timestamps for dependency checking, often causing recythonizing when it's not needed.

Reviewer: Jeroen Demeyer

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/18256





---

archive/issue_comments_316862.json:
```json
{
    "body": "<a id='comment:1'></a>This was already reported upstream in #17851. We should not have a Sage specific solution for that! Of course we would have to set again the cache once this will be fixed upstream...",
    "created_at": "2015-04-20T19:16:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316862",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'></a>This was already reported upstream in #17851. We should not have a Sage specific solution for that! Of course we would have to set again the cache once this will be fixed upstream...



---

archive/issue_comments_316863.json:
```json
{
    "body": "<a id='comment:2'></a>If the Cython version number was touched, it means that Cython really changed (otherwise, we wouldn't have changed the Cython version number...). So I don't really understand the issue.",
    "created_at": "2015-04-21T06:00:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316863",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>If the Cython version number was touched, it means that Cython really changed (otherwise, we wouldn't have changed the Cython version number...). So I don't really understand the issue.



---

archive/issue_comments_316864.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [jdemeyer](#comment%3A2):\n> If the Cython version number was touched, it means that Cython really changed (otherwise, we wouldn't have changed the Cython version number...). So I don't really understand the issue.\n\nMaybe I'm stupid but somewhere the dependencies are described, e.g., in a Makefile or other config, which trigger the complete build. Instead, you trigger it when the version string changes, not when the timestamp changes. This results in identical behaviour on upgrade, but spares full recythonize when I do an old branch checkout followed by develop merge.",
    "created_at": "2015-04-21T06:34:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316864",
    "user": "https://github.com/rwst"
}
```

<a id='comment:3'></a>Replying to [jdemeyer](#comment%3A2):
> If the Cython version number was touched, it means that Cython really changed (otherwise, we wouldn't have changed the Cython version number...). So I don't really understand the issue.

Maybe I'm stupid but somewhere the dependencies are described, e.g., in a Makefile or other config, which trigger the complete build. Instead, you trigger it when the version string changes, not when the timestamp changes. This results in identical behaviour on upgrade, but spares full recythonize when I do an old branch checkout followed by develop merge.



---

archive/issue_comments_316865.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [rws](#comment%3A3):\n> Replying to [jdemeyer](#comment%3A2):\n> > If the Cython version number was touched, it means that Cython really changed (otherwise, we wouldn't have changed the Cython version number...). So I don't really understand the issue.\n\n> Maybe I'm stupid but somewhere the dependencies are described, e.g., in a Makefile or other config, which trigger the complete build. Instead, you trigger it when the version string changes, not when the timestamp changes. This results in identical behaviour on upgrade, but spares full recythonize when I do an old branch checkout followed by develop merge.\n???\n\nI really don't understand what you are saying. Could you please clearly indicate *the problem* (not an analysis or a fix, just the problem).",
    "created_at": "2015-04-21T07:43:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316865",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>Replying to [rws](#comment%3A3):
> Replying to [jdemeyer](#comment%3A2):
> > If the Cython version number was touched, it means that Cython really changed (otherwise, we wouldn't have changed the Cython version number...). So I don't really understand the issue.

> Maybe I'm stupid but somewhere the dependencies are described, e.g., in a Makefile or other config, which trigger the complete build. Instead, you trigger it when the version string changes, not when the timestamp changes. This results in identical behaviour on upgrade, but spares full recythonize when I do an old branch checkout followed by develop merge.
???

I really don't understand what you are saying. Could you please clearly indicate *the problem* (not an analysis or a fix, just the problem).



---

archive/issue_comments_316866.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [jdemeyer](#comment%3A4):\n> I really don't understand what you are saying. Could you please clearly indicate *the problem* (not an analysis or a fix, just the problem).\n\nI frequently checkout older branches to do a fix or a review. After checkout I often merge develop. Now when I `make`, due to (my guess) the change back and forth in cython version a full recythonize is triggered. But it is IMHO not necessary because I merged develop and so, only some Python code really changed, or only specific `pyx` files. The proposal here is to optimize the `make` behaviour in such cases.",
    "created_at": "2015-04-21T07:55:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316866",
    "user": "https://github.com/rwst"
}
```

<a id='comment:5'></a>Replying to [jdemeyer](#comment%3A4):
> I really don't understand what you are saying. Could you please clearly indicate *the problem* (not an analysis or a fix, just the problem).

I frequently checkout older branches to do a fix or a review. After checkout I often merge develop. Now when I `make`, due to (my guess) the change back and forth in cython version a full recythonize is triggered. But it is IMHO not necessary because I merged develop and so, only some Python code really changed, or only specific `pyx` files. The proposal here is to optimize the `make` behaviour in such cases.



---

archive/issue_comments_316867.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-In #17851 cycache was necessarily disabled. This now leads to recythonizing everything when only the cython version **number was touched** (e.g. by checking out an old branch followed by merging develop). Ideally, cythonizing everything should only happen if the cython binary is different when compared to the last cythonizing.\n+I frequently checkout older branches to do a fix or a review. After checkout I often merge develop. Now when I `make`, due to the change back and forth in a package version, a full rebuild of that package is triggered. But it is IMHO not necessary because I merged develop and so, only some Python code really changed, or only specific `pyx` files. The proposal here is to optimize the `make` behaviour in such cases.\n``````\n",
    "created_at": "2015-04-21T08:00:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316867",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-In #17851 cycache was necessarily disabled. This now leads to recythonizing everything when only the cython version **number was touched** (e.g. by checking out an old branch followed by merging develop). Ideally, cythonizing everything should only happen if the cython binary is different when compared to the last cythonizing.
+I frequently checkout older branches to do a fix or a review. After checkout I often merge develop. Now when I `make`, due to the change back and forth in a package version, a full rebuild of that package is triggered. But it is IMHO not necessary because I merged develop and so, only some Python code really changed, or only specific `pyx` files. The proposal here is to optimize the `make` behaviour in such cases.
``````




---

archive/issue_comments_316868.json:
```json
{
    "body": "Changing component from cython to build.",
    "created_at": "2015-04-21T08:00:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316868",
    "user": "https://github.com/jdemeyer"
}
```

Changing component from cython to build.



---

archive/issue_comments_316869.json:
```json
{
    "body": "<a id='comment:6'></a>This has nothing to do with Cython, but with `make`.",
    "created_at": "2015-04-21T08:00:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316869",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>This has nothing to do with Cython, but with `make`.



---

archive/issue_comments_316870.json:
```json
{
    "body": "Changing keywords from \"cythonize\" to \"\".",
    "created_at": "2015-04-21T08:00:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316870",
    "user": "https://github.com/jdemeyer"
}
```

Changing keywords from "cythonize" to "".



---

archive/issue_comments_316871.json:
```json
{
    "body": "<a id='comment:8'></a>To clarify: this would happen with every package upgrade. Since building Cython and re-cythonizing takes a long time, you just notice it more when it's Cython which is being rebuilt.",
    "created_at": "2015-04-21T08:03:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316871",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>To clarify: this would happen with every package upgrade. Since building Cython and re-cythonizing takes a long time, you just notice it more when it's Cython which is being rebuilt.



---

archive/issue_comments_316872.json:
```json
{
    "body": "<a id='comment:9'></a>Hmm no, that's not true. There is logic in `src/bin/sage-spkg` to not re-build packages if you're installing the version which is already installed.",
    "created_at": "2015-04-21T08:07:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316872",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'></a>Hmm no, that's not true. There is logic in `src/bin/sage-spkg` to not re-build packages if you're installing the version which is already installed.



---

archive/issue_comments_316873.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-I frequently checkout older branches to do a fix or a review. After checkout I often merge develop. Now when I `make`, due to the change back and forth in a package version, a full rebuild of that package is triggered. But it is IMHO not necessary because I merged develop and so, only some Python code really changed, or only specific `pyx` files. The proposal here is to optimize the `make` behaviour in such cases.\n+???\n``````\n",
    "created_at": "2015-04-21T08:07:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316873",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-I frequently checkout older branches to do a fix or a review. After checkout I often merge develop. Now when I `make`, due to the change back and forth in a package version, a full rebuild of that package is triggered. But it is IMHO not necessary because I merged develop and so, only some Python code really changed, or only specific `pyx` files. The proposal here is to optimize the `make` behaviour in such cases.
+???
``````




---

archive/issue_comments_316874.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [rws](#comment%3A5):\n> a full recythonize is triggered.\n\nAre you sure it's really a full recythonize and not just a rebuild of all files depending on a common file like `src/ext/interrupt.pxi`? It's true that Cython (just like `make`) uses timestamps to trigger rebuilds, so if the timestamp of `src/ext/interrupt.pxi` changes, then all Cython extensions depending on that (i.e. almost all of them) would be recythonized.\n\nIf this is the problem, I would like to close this ticket as `wontfix` since it's a feature, not a bug, to use timestamps for dependency checking. And even if it would be a bug, it's in upstream Cython, not Sage.",
    "created_at": "2015-04-21T08:11:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316874",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>Replying to [rws](#comment%3A5):
> a full recythonize is triggered.

Are you sure it's really a full recythonize and not just a rebuild of all files depending on a common file like `src/ext/interrupt.pxi`? It's true that Cython (just like `make`) uses timestamps to trigger rebuilds, so if the timestamp of `src/ext/interrupt.pxi` changes, then all Cython extensions depending on that (i.e. almost all of them) would be recythonized.

If this is the problem, I would like to close this ticket as `wontfix` since it's a feature, not a bug, to use timestamps for dependency checking. And even if it would be a bug, it's in upstream Cython, not Sage.



---

archive/issue_comments_316875.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [jdemeyer](#comment%3A10):\n> ...so if the timestamp of `src/ext/interrupt.pxi` changes, then all Cython extensions depending on that (i.e. almost all of them) would be recythonized.\n\nThen that seems to have been the reason. If there is no way to make that more fine-grained I agree with wontfix here.",
    "created_at": "2015-04-21T08:25:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316875",
    "user": "https://github.com/rwst"
}
```

<a id='comment:11'></a>Replying to [jdemeyer](#comment%3A10):
> ...so if the timestamp of `src/ext/interrupt.pxi` changes, then all Cython extensions depending on that (i.e. almost all of them) would be recythonized.

Then that seems to have been the reason. If there is no way to make that more fine-grained I agree with wontfix here.



---

archive/issue_comments_316876.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2015-04-21T08:31:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316876",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_events_051560.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-21T08:31:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18256#event-51560"
}
```



---

archive/issue_comments_316877.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-04-21T08:31:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316877",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_316878.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-04-21T08:31:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316878",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_316879.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-???\n+Cython uses timestamps for dependency checking, often causing recythonizing when it's not needed.\n``````\n",
    "created_at": "2015-04-21T08:32:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316879",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-???
+Cython uses timestamps for dependency checking, often causing recythonizing when it's not needed.
``````




---

archive/issue_events_051561.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-22T00:07:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18256#event-51561"
}
```



---

archive/issue_comments_316880.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2015-04-22T00:07:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18256#issuecomment-316880",
    "user": "https://github.com/vbraun"
}
```

Resolution: wontfix
