# Issue 18977: ncurses fails to build with GCC 5.2

archive/issues_018740.json:
```json
{
    "assignees": [],
    "body": "The problem described in #18301 still happens with GCC 5.2.0. We should assume that all versions of GCC 5 are affected to fix this issue.\n\nCC:  jpflori @vbraun @nexttime\n\nKeywords: **ncurses, cpp**\n\nBranch/Commit: **[4689b7c](https://github.com/sagemath/sagetrac-mirror/commit/4689b7c4c4c6c8923df917c22a6aa90ff8908bed)**\n\nReviewer: **Andr\u00e9 Apitzsch**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18977_\n\n",
    "closed_at": "2015-08-01T23:07:51Z",
    "created_at": "2015-08-01T05:03:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ncurses fails to build with GCC 5.2",
    "type": "issue",
    "updated_at": "2015-08-01T23:07:51Z",
    "url": "https://github.com/sagemath/sage/issues/18977",
    "user": "https://github.com/saraedum"
}
```
The problem described in #18301 still happens with GCC 5.2.0. We should assume that all versions of GCC 5 are affected to fix this issue.

CC:  jpflori @vbraun @nexttime

Keywords: **ncurses, cpp**

Branch/Commit: **[4689b7c](https://github.com/sagemath/sagetrac-mirror/commit/4689b7c4c4c6c8923df917c22a6aa90ff8908bed)**

Reviewer: **André Apitzsch**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/18977_





---

archive/issue_events_242543.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2015-08-01T05:03:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18977#event-242543"
}
```



---

archive/issue_events_242544.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2015-08-01T05:03:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18977#event-242544"
}
```



---

archive/issue_comments_269389.json:
```json
{
    "body": "Branch: **[u/saraedum/ncurses_fails_to_build_with_gcc_5_2](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ncurses_fails_to_build_with_gcc_5_2)**",
    "created_at": "2015-08-01T05:11:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18977#issuecomment-269389",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/ncurses_fails_to_build_with_gcc_5_2](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ncurses_fails_to_build_with_gcc_5_2)**



---

archive/issue_comments_269390.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4689b7c4c4c6c8923df917c22a6aa90ff8908bed\">4689b7c</a></td><td><code>Add patch to let ncurses build with GCC 5.2</code></td></tr></table>\n",
    "created_at": "2015-08-01T05:12:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18977#issuecomment-269390",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4689b7c4c4c6c8923df917c22a6aa90ff8908bed">4689b7c</a></td><td><code>Add patch to let ncurses build with GCC 5.2</code></td></tr></table>




---

archive/issue_comments_269391.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+The problem described in #18301 still happens with GCC 5.2.0. We should assume that all versions of GCC 5 are affected to fix this issue.\n``````\n",
    "created_at": "2015-08-01T05:12:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18977#issuecomment-269391",
    "user": "https://github.com/saraedum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+The problem described in #18301 still happens with GCC 5.2.0. We should assume that all versions of GCC 5 are affected to fix this issue.
``````




---

archive/issue_comments_269392.json:
```json
{
    "body": "Author: **Julian R\u00fcth**",
    "created_at": "2015-08-01T05:12:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18977#issuecomment-269392",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rüth**



---

archive/issue_comments_269393.json:
```json
{
    "body": "Commit: **[4689b7c](https://github.com/sagemath/sagetrac-mirror/commit/4689b7c4c4c6c8923df917c22a6aa90ff8908bed)**",
    "created_at": "2015-08-01T05:12:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18977#issuecomment-269393",
    "user": "https://github.com/saraedum"
}
```

Commit: **[4689b7c](https://github.com/sagemath/sagetrac-mirror/commit/4689b7c4c4c6c8923df917c22a6aa90ff8908bed)**



---

archive/issue_comments_269394.json:
```json
{
    "body": "Changed keywords from **** to **ncurses, cpp**",
    "created_at": "2015-08-01T05:12:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18977#issuecomment-269394",
    "user": "https://github.com/saraedum"
}
```

Changed keywords from **** to **ncurses, cpp**



---

archive/issue_events_242545.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2015-08-01T05:12:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18977#event-242545"
}
```



---

archive/issue_events_242546.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2015-08-01T05:12:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18977#event-242546"
}
```



---

archive/issue_comments_269395.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLGTM.",
    "created_at": "2015-08-01T09:32:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18977#issuecomment-269395",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:3'>Comment 3:</a>
LGTM.



---

archive/issue_comments_269396.json:
```json
{
    "body": "Reviewer: **Andr\u00e9 Apitzsch**",
    "created_at": "2015-08-01T09:32:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18977#issuecomment-269396",
    "user": "https://github.com/a-andre"
}
```

Reviewer: **André Apitzsch**



---

archive/issue_events_242547.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-08-01T09:32:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18977#event-242547"
}
```



---

archive/issue_events_242548.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-08-01T09:32:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18977#event-242548"
}
```



---

archive/issue_comments_269397.json:
```json
{
    "body": "Changed branch from **[u/saraedum/ncurses_fails_to_build_with_gcc_5_2](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ncurses_fails_to_build_with_gcc_5_2)** to **[4689b7c](https://github.com/sagemath/sagetrac-mirror/commit/4689b7c4c4c6c8923df917c22a6aa90ff8908bed)**",
    "created_at": "2015-08-01T23:07:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18977#issuecomment-269397",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/ncurses_fails_to_build_with_gcc_5_2](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ncurses_fails_to_build_with_gcc_5_2)** to **[4689b7c](https://github.com/sagemath/sagetrac-mirror/commit/4689b7c4c4c6c8923df917c22a6aa90ff8908bed)**



---

archive/issue_events_242549.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-08-01T23:07:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18977#event-242549"
}
```



---

archive/issue_events_242550.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "af59a3fd1ddaa66cd4eb621ef46881c784a4323a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-08-01T23:07:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18977",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18977#event-242550"
}
```
