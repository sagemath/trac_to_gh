# Issue 18918: Maxima interface bug with divergent integral

archive/issues_018681.json:
```json
{
    "assignees": [],
    "body": "From https://groups.google.com/forum/?hl=en#!topic/sage-support/OJAN4pk4dwQ:\n\n```\nsage: integrate(x/(x^2+1),x,0,oo)\n0\n```\nbut in Maxima:\n\n```\n(%i11) integrate(x/(x^2+1),x,0,+inf);\ndefint: integral is divergent.\n```\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18918_\n\n",
    "closed_at": "2015-07-26T13:32:09Z",
    "created_at": "2015-07-18T07:21:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Maxima interface bug with divergent integral",
    "type": "issue",
    "updated_at": "2015-07-26T13:32:09Z",
    "url": "https://github.com/sagemath/sage/issues/18918",
    "user": "https://github.com/rwst"
}
```
From https://groups.google.com/forum/?hl=en#!topic/sage-support/OJAN4pk4dwQ:

```
sage: integrate(x/(x^2+1),x,0,oo)
0
```
but in Maxima:

```
(%i11) integrate(x/(x^2+1),x,0,+inf);
defint: integral is divergent.
```

_Issue created by migration from https://trac.sagemath.org/ticket/18918_





---

archive/issue_events_250758.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-07-18T07:21:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250758"
}
```



---

archive/issue_events_250759.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-07-18T07:21:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250759"
}
```



---

archive/issue_events_250760.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-07-18T07:21:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250760"
}
```



---

archive/issue_events_250761.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-07-18T07:21:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250761"
}
```



---

archive/issue_comments_267674.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nDuplicate of #18920",
    "created_at": "2015-07-19T08:50:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18918#issuecomment-267674",
    "user": "https://github.com/rwst"
}
```

<div id="comment:1" align="right">Comment 1</div>

Duplicate of #18920



---

archive/issue_events_250762.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-07-19T08:50:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250762"
}
```



---

archive/issue_events_250763.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-07-19T08:50:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250763"
}
```



---

archive/issue_events_250764.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-07-19T08:50:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250764"
}
```



---

archive/issue_events_250765.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-26T13:01:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250765"
}
```



---

archive/issue_events_250766.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-26T13:01:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250766"
}
```



---

archive/issue_events_250767.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-26T13:01:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250767"
}
```



---

archive/issue_events_250768.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-26T13:32:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250768"
}
```



---

archive/issue_events_250769.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-26T13:32:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250769"
}
```



---

archive/issue_events_250770.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-26T13:32:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18918",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18918#event-250770"
}
```
