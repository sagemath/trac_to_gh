# Issue 18191: Automaton: representation string wrong for Automata with 1 state

archive/issues_017954.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: A = Automaton()\nsage: A.add_state(42)\n42\nsage: A\nAutomaton with 1 states\n```\nThe last line is wrong and should be \"...with 1 state\".\n\nKeywords: **sd64.25**\n\nBranch/Commit: **[9bd9f30](https://github.com/sagemath/sagetrac-mirror/commit/9bd9f3045d9b7c8529bbb8be71dbf386708f68a6)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Karen Kohl**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18191_\n\n",
    "closed_at": "2015-05-26T23:34:19Z",
    "created_at": "2015-04-14T15:08:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Automaton: representation string wrong for Automata with 1 state",
    "type": "issue",
    "updated_at": "2015-05-26T23:34:19Z",
    "url": "https://github.com/sagemath/sage/issues/18191",
    "user": "https://github.com/dkrenn"
}
```

```
sage: A = Automaton()
sage: A.add_state(42)
42
sage: A
Automaton with 1 states
```
The last line is wrong and should be "...with 1 state".

Keywords: **sd64.25**

Branch/Commit: **[9bd9f30](https://github.com/sagemath/sagetrac-mirror/commit/9bd9f3045d9b7c8529bbb8be71dbf386708f68a6)**

Reviewer: **Vincent Delecroix**

Author: **Karen Kohl**

_Issue created by migration from https://trac.sagemath.org/ticket/18191_





---

archive/issue_events_241002.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2015-04-14T15:08:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241002"
}
```



---

archive/issue_events_241003.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2015-04-14T15:08:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
    "label_color": "0000ff",
    "label_name": "component: finite state machines",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241003"
}
```



---

archive/issue_events_241004.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2015-04-14T15:08:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241004"
}
```



---

archive/issue_events_241005.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2015-04-14T15:08:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241005"
}
```



---

archive/issue_events_241006.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2015-04-14T15:29:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241006"
}
```



---

archive/issue_comments_251426.json:
```json
{
    "body": "Author: **Karen Kohl**",
    "created_at": "2015-05-25T19:58:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251426",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Author: **Karen Kohl**



---

archive/issue_comments_251427.json:
```json
{
    "body": "Branch: **[u/ktkohl/automaton__representation_string_wrong_for_automata_with_1_state](https://github.com/sagemath/sagetrac-mirror/tree/u/ktkohl/automaton__representation_string_wrong_for_automata_with_1_state)**",
    "created_at": "2015-05-25T20:01:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251427",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Branch: **[u/ktkohl/automaton__representation_string_wrong_for_automata_with_1_state](https://github.com/sagemath/sagetrac-mirror/tree/u/ktkohl/automaton__representation_string_wrong_for_automata_with_1_state)**



---

archive/issue_comments_251428.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nAlso made the same correction for Transducer and FiniteStateMachine in addition to Automaton.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b0c5a38590417f1bb52b132ccbf73aeb55276b8c\">b0c5a38</a></td><td><code>Changed representation string for Automaton, Transducer, and FiniteStateMachine.</code></td></tr></table>\n",
    "created_at": "2015-05-25T20:05:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251428",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

<div id="comment:4" align="right">Comment 4</div>

Also made the same correction for Transducer and FiniteStateMachine in addition to Automaton.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b0c5a38590417f1bb52b132ccbf73aeb55276b8c">b0c5a38</a></td><td><code>Changed representation string for Automaton, Transducer, and FiniteStateMachine.</code></td></tr></table>




---

archive/issue_comments_251429.json:
```json
{
    "body": "Commit: **[b0c5a38](https://github.com/sagemath/sagetrac-mirror/commit/b0c5a38590417f1bb52b132ccbf73aeb55276b8c)**",
    "created_at": "2015-05-25T20:05:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251429",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Commit: **[b0c5a38](https://github.com/sagemath/sagetrac-mirror/commit/b0c5a38590417f1bb52b132ccbf73aeb55276b8c)**



---

archive/issue_events_241007.json:
```json
{
    "actor": "https://github.com/sagetrac-ktkohl",
    "created_at": "2015-05-25T20:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241007"
}
```



---

archive/issue_comments_251430.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nHello,\n\nWhile you are at it, this is also wrong\n\n```\nsage: FiniteStateMachine()\nFinite state machine with 0 states\n```\nIt should rather be `Empty finite state machine`.\n\nBest\nVincent",
    "created_at": "2015-05-25T21:05:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251430",
    "user": "https://github.com/videlec"
}
```

<div id="comment:5" align="right">Comment 5</div>

Hello,

While you are at it, this is also wrong

```
sage: FiniteStateMachine()
Finite state machine with 0 states
```
It should rather be `Empty finite state machine`.

Best
Vincent



---

archive/issue_events_241008.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-05-25T21:05:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241008"
}
```



---

archive/issue_events_241009.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-05-25T21:05:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241009"
}
```



---

archive/issue_comments_251431.json:
```json
{
    "body": "Changed commit from **[b0c5a38](https://github.com/sagemath/sagetrac-mirror/commit/b0c5a38590417f1bb52b132ccbf73aeb55276b8c)** to **[9bd9f30](https://github.com/sagemath/sagetrac-mirror/commit/9bd9f3045d9b7c8529bbb8be71dbf386708f68a6)**",
    "created_at": "2015-05-25T21:37:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251431",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b0c5a38](https://github.com/sagemath/sagetrac-mirror/commit/b0c5a38590417f1bb52b132ccbf73aeb55276b8c)** to **[9bd9f30](https://github.com/sagemath/sagetrac-mirror/commit/9bd9f3045d9b7c8529bbb8be71dbf386708f68a6)**



---

archive/issue_comments_251432.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9bd9f3045d9b7c8529bbb8be71dbf386708f68a6\">9bd9f30</a></td><td><code>Changed representation string to say \"empty\" instead of \"0 states\" in all of Automaton, Finite State Machine, and Transducer.</code></td></tr></table>\n",
    "created_at": "2015-05-25T21:37:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251432",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9bd9f3045d9b7c8529bbb8be71dbf386708f68a6">9bd9f30</a></td><td><code>Changed representation string to say "empty" instead of "0 states" in all of Automaton, Finite State Machine, and Transducer.</code></td></tr></table>




---

archive/issue_events_241010.json:
```json
{
    "actor": "https://github.com/sagetrac-ktkohl",
    "created_at": "2015-05-25T22:31:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241010"
}
```



---

archive/issue_events_241011.json:
```json
{
    "actor": "https://github.com/sagetrac-ktkohl",
    "created_at": "2015-05-25T22:31:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241011"
}
```



---

archive/issue_comments_251433.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nHello,\n\nFor the future: the commit message should not be that long. It must start with a one-line description (~70 characters). Possibly you can add some more comments after a line break. Something like\n\n```\n\"0 states\" -> \"empty\" for Finite State Machine\n\nWe change the method _repr_ for Automaton, Finite State Machine and\nTransducer to be more friendly when there is no state.\n```\n\nVincent",
    "created_at": "2015-05-26T09:47:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251433",
    "user": "https://github.com/videlec"
}
```

<div id="comment:8" align="right">Comment 8</div>

Hello,

For the future: the commit message should not be that long. It must start with a one-line description (~70 characters). Possibly you can add some more comments after a line break. Something like

```
"0 states" -> "empty" for Finite State Machine

We change the method _repr_ for Automaton, Finite State Machine and
Transducer to be more friendly when there is no state.
```

Vincent



---

archive/issue_comments_251434.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2015-05-26T09:47:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251434",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_events_241012.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-05-26T09:47:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241012"
}
```



---

archive/issue_events_241013.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-05-26T09:47:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241013"
}
```



---

archive/issue_comments_251435.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThanks for letting me know and for all your help.",
    "created_at": "2015-05-26T14:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251435",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

<div id="comment:9" align="right">Comment 9</div>

Thanks for letting me know and for all your help.



---

archive/issue_events_241014.json:
```json
{
    "actor": "https://github.com/sagetrac-ktkohl",
    "created_at": "2015-05-26T14:57:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241014"
}
```



---

archive/issue_events_241015.json:
```json
{
    "actor": "https://github.com/sagetrac-ktkohl",
    "created_at": "2015-05-26T14:57:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241015"
}
```



---

archive/issue_comments_251436.json:
```json
{
    "body": "Changed keywords from **sd67** to **sd64.25**",
    "created_at": "2015-05-26T14:57:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251436",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Changed keywords from **sd67** to **sd64.25**



---

archive/issue_events_241016.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-26T23:34:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241016"
}
```



---

archive/issue_events_241017.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5f347df85c4551f4f809b551a7ab246e77d4cd67",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-05-26T23:34:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18191#event-241017"
}
```



---

archive/issue_comments_251437.json:
```json
{
    "body": "Changed branch from **[u/ktkohl/automaton__representation_string_wrong_for_automata_with_1_state](https://github.com/sagemath/sagetrac-mirror/tree/u/ktkohl/automaton__representation_string_wrong_for_automata_with_1_state)** to **[9bd9f30](https://github.com/sagemath/sagetrac-mirror/commit/9bd9f3045d9b7c8529bbb8be71dbf386708f68a6)**",
    "created_at": "2015-05-26T23:34:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18191",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18191#issuecomment-251437",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/ktkohl/automaton__representation_string_wrong_for_automata_with_1_state](https://github.com/sagemath/sagetrac-mirror/tree/u/ktkohl/automaton__representation_string_wrong_for_automata_with_1_state)** to **[9bd9f30](https://github.com/sagemath/sagetrac-mirror/commit/9bd9f3045d9b7c8529bbb8be71dbf386708f68a6)**
