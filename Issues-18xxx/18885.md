# Issue 18885: ecm fail to build with "no such instruction: vfmadd312sd"

archive/issues_018648.json:
```json
{
    "assignees": [],
    "body": "Ubuntu 12.04 64-bit \n\n```\n/tmp/ccaWsLeq.s: Assembler messages:\n/tmp/ccaWsLeq.s:137: Error: no such instruction: `vfmadd312sd offset(%rip),%xmm0,%xmm3'\n/tmp/ccaWsLeq.s:713: Error: no such instruction: `vfmadd312sd .LC12(%rip),%xmm0,%xmm2'\n```\n\nBranch/Commit: **[f708487](https://github.com/sagemath/sagetrac-mirror/commit/f7084871d4054445de04338b6b5d5d50fd67a5a6)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18885_\n\n",
    "closed_at": "2015-09-18T19:10:44Z",
    "created_at": "2015-07-12T17:48:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ecm fail to build with \"no such instruction: vfmadd312sd\"",
    "type": "issue",
    "updated_at": "2015-09-18T19:10:44Z",
    "url": "https://github.com/sagemath/sage/issues/18885",
    "user": "https://github.com/vbraun"
}
```
Ubuntu 12.04 64-bit 

```
/tmp/ccaWsLeq.s: Assembler messages:
/tmp/ccaWsLeq.s:137: Error: no such instruction: `vfmadd312sd offset(%rip),%xmm0,%xmm3'
/tmp/ccaWsLeq.s:713: Error: no such instruction: `vfmadd312sd .LC12(%rip),%xmm0,%xmm2'
```

Branch/Commit: **[f708487](https://github.com/sagemath/sagetrac-mirror/commit/f7084871d4054445de04338b6b5d5d50fd67a5a6)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/18885_





---

archive/issue_events_250354.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-12T17:48:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250354"
}
```



---

archive/issue_events_250355.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-12T17:48:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250355"
}
```



---

archive/issue_events_250356.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-12T17:48:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250356"
}
```



---

archive/issue_events_250357.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-12T17:48:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250357"
}
```



---

archive/issue_comments_267053.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nPreviously reported at https://groups.google.com/d/msg/sage-support/4s2akLrlLzs/77YujREL9YAJ",
    "created_at": "2015-07-12T17:48:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267053",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'>Comment 1:</a>
Previously reported at https://groups.google.com/d/msg/sage-support/4s2akLrlLzs/77YujREL9YAJ



---

archive/issue_comments_267054.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWhat is your `gcc --version`?",
    "created_at": "2015-07-13T06:09:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267054",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
What is your `gcc --version`?



---

archive/issue_comments_267055.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\n\n```\ngcc version 4.6.3 (Ubuntu/Linaro 4.6.3-1ubuntu5) \n```",
    "created_at": "2015-07-13T07:31:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267055",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'>Comment 3:</a>

```
gcc version 4.6.3 (Ubuntu/Linaro 4.6.3-1ubuntu5) 
```



---

archive/issue_comments_267056.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLog: http://build.sagedev.org/release/builders/%20%20slow%20AIMS%20bu12_64s02%20%28Ubuntu%2012.04%2064%20bit%29%20incremental/builds/7/steps/compile/logs/ecm",
    "created_at": "2015-07-13T07:31:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267056",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'>Comment 4:</a>
Log: http://build.sagedev.org/release/builders/%20%20slow%20AIMS%20bu12_64s02%20%28Ubuntu%2012.04%2064%20bit%29%20incremental/builds/7/steps/compile/logs/ecm



---

archive/issue_comments_267057.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nIs it time to blacklist GCC 4.6.x?\n\n(edit: we're already doing that!)",
    "created_at": "2015-07-13T08:34:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267057",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'>Comment 5:</a>
Is it time to blacklist GCC 4.6.x?

(edit: we're already doing that!)



---

archive/issue_comments_267058.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nWhat does `g++ -dumpversion` answer?",
    "created_at": "2015-07-13T08:35:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267058",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
What does `g++ -dumpversion` answer?



---

archive/issue_comments_267059.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ecm_fail_to_build_with__no_such_instruction__vfmadd312sd_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ecm_fail_to_build_with__no_such_instruction__vfmadd312sd_)**",
    "created_at": "2015-07-13T08:39:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267059",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ecm_fail_to_build_with__no_such_instruction__vfmadd312sd_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ecm_fail_to_build_with__no_such_instruction__vfmadd312sd_)**



---

archive/issue_comments_267060.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2015-07-13T08:39:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267060",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_events_250358.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T08:39:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250358"
}
```



---

archive/issue_comments_267061.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/27e7a380fdecea27f68e088d3118f527cee60ce8\">27e7a38</a></td><td><code>Fix for Debian/Ubuntu GCC version numbers</code></td></tr></table>\n",
    "created_at": "2015-07-13T08:39:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267061",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'>Comment 8:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/27e7a380fdecea27f68e088d3118f527cee60ce8">27e7a38</a></td><td><code>Fix for Debian/Ubuntu GCC version numbers</code></td></tr></table>




---

archive/issue_events_250359.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T08:39:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250359"
}
```



---

archive/issue_events_250360.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-13T08:39:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250360"
}
```



---

archive/issue_comments_267062.json:
```json
{
    "body": "Commit: **[27e7a38](https://github.com/sagemath/sagetrac-mirror/commit/27e7a380fdecea27f68e088d3118f527cee60ce8)**",
    "created_at": "2015-07-13T08:39:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267062",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[27e7a38](https://github.com/sagemath/sagetrac-mirror/commit/27e7a380fdecea27f68e088d3118f527cee60ce8)**



---

archive/issue_comments_267063.json:
```json
{
    "body": "Changed commit from **[27e7a38](https://github.com/sagemath/sagetrac-mirror/commit/27e7a380fdecea27f68e088d3118f527cee60ce8)** to **[f708487](https://github.com/sagemath/sagetrac-mirror/commit/f7084871d4054445de04338b6b5d5d50fd67a5a6)**",
    "created_at": "2015-08-27T13:51:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267063",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[27e7a38](https://github.com/sagemath/sagetrac-mirror/commit/27e7a380fdecea27f68e088d3118f527cee60ce8)** to **[f708487](https://github.com/sagemath/sagetrac-mirror/commit/f7084871d4054445de04338b6b5d5d50fd67a5a6)**



---

archive/issue_comments_267064.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f7084871d4054445de04338b6b5d5d50fd67a5a6\">f708487</a></td><td><code>Fix for Debian/Ubuntu GCC version numbers</code></td></tr></table>\n",
    "created_at": "2015-08-27T13:51:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267064",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:9'>Comment 9:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f7084871d4054445de04338b6b5d5d50fd67a5a6">f708487</a></td><td><code>Fix for Debian/Ubuntu GCC version numbers</code></td></tr></table>




---

archive/issue_events_250361.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-17T20:47:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250361"
}
```



---

archive/issue_events_250362.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-17T20:47:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250362"
}
```



---

archive/issue_comments_267065.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-09-17T20:47:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267065",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_267066.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ecm_fail_to_build_with__no_such_instruction__vfmadd312sd_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ecm_fail_to_build_with__no_such_instruction__vfmadd312sd_)** to **[f708487](https://github.com/sagemath/sagetrac-mirror/commit/f7084871d4054445de04338b6b5d5d50fd67a5a6)**",
    "created_at": "2015-09-18T19:10:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18885#issuecomment-267066",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ecm_fail_to_build_with__no_such_instruction__vfmadd312sd_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ecm_fail_to_build_with__no_such_instruction__vfmadd312sd_)** to **[f708487](https://github.com/sagemath/sagetrac-mirror/commit/f7084871d4054445de04338b6b5d5d50fd67a5a6)**



---

archive/issue_events_250363.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-18T19:10:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250363"
}
```



---

archive/issue_events_250364.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "49350d64ddb3c7acd1960ae545c4861c828b2189",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-09-18T19:10:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18885",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18885#event-250364"
}
```
