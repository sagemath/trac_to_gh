# Issue 18565: catalog of sequences in SageMath

archive/issues_018328.json:
```json
{
    "body": "similar to\n\n```\n    graphs.some_graph\n```\nor to\n\n```\n    codes.some_code\n```\n(and similar), I propose to introduce\n\n```\n    sequences.some_sequence\n```\nwhich should provide a unified access point to all possible sequences in SageMath. For example, there is `fibonacci_sequence` in `sage.combinat.combinat`, `binary_recurring_sequence` (somewhere in combinat) and various other code.\n\nThis will use homogenous sequences (#19896), which them-self use lazy lists.\n\nSee also the meta-ticket #16107.\n\n---\nOld: See the discussion on [sage-devel](https://groups.google.com/forum/#!topic/sage-devel/L-wSCu4fXe8)\n\nCC:  @mantepse\n\nAuthor: Daniel Krenn\n\nBranch: u/dkrenn/sequences/base\n\nStatus: new\n\nDependencies: #19896\n\nCommit: b0bb4fa7a2a17ed74d25402a29fd4143473f3580\n\nIssue created by migration from https://trac.sagemath.org/ticket/18565\n\n",
    "created_at": "2015-06-01T09:49:23Z",
    "labels": [
        "component: combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.8",
    "title": "catalog of sequences in SageMath",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/18565",
    "user": "https://github.com/dkrenn"
}
```
similar to

```
    graphs.some_graph
```
or to

```
    codes.some_code
```
(and similar), I propose to introduce

```
    sequences.some_sequence
```
which should provide a unified access point to all possible sequences in SageMath. For example, there is `fibonacci_sequence` in `sage.combinat.combinat`, `binary_recurring_sequence` (somewhere in combinat) and various other code.

This will use homogenous sequences (#19896), which them-self use lazy lists.

See also the meta-ticket #16107.

---
Old: See the discussion on [sage-devel](https://groups.google.com/forum/#!topic/sage-devel/L-wSCu4fXe8)

CC:  @mantepse

Author: Daniel Krenn

Branch: u/dkrenn/sequences/base

Status: new

Dependencies: #19896

Commit: b0bb4fa7a2a17ed74d25402a29fd4143473f3580

Issue created by migration from https://trac.sagemath.org/ticket/18565





---

archive/issue_comments_325903.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -18,3 +18,4 @@\n One motivation for creating this, is to include here many sequences, which can be calculated term-by-term (from the previously calculated ones, i.e., recursively) or where the first N terms can be calculated at once efficiently.\n Nevertheless, it can also provide an interface to sequences, where each term can be calculated explicitly (without dealing with the previous ones).\n \n+See the discussion on [sage-devel](https://groups.google.com/forum/#!topic/sage-devel/L-wSCu4fXe8)\n``````\n",
    "created_at": "2015-06-01T09:51:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325903",
    "user": "https://github.com/dkrenn"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -18,3 +18,4 @@
 One motivation for creating this, is to include here many sequences, which can be calculated term-by-term (from the previously calculated ones, i.e., recursively) or where the first N terms can be calculated at once efficiently.
 Nevertheless, it can also provide an interface to sequences, where each term can be calculated explicitly (without dealing with the previous ones).
 
+See the discussion on [sage-devel](https://groups.google.com/forum/#!topic/sage-devel/L-wSCu4fXe8)
``````




---

archive/issue_comments_325904.json:
```json
{
    "body": "<a id='comment:2'></a>\nHello Daniel,\n\nCould you define what you mean by **sequence**. Is it sequence of numbers? Rational numbers? Real numbers? An element of an **infinite cartesian power**? Because there also are\n\n```\nsage: words.FibonacciWord('ab')\nword: abaababaabaababaababaabaababaabaababaaba...\n```\nwhich is an infinite sequence of elements in `{a,b}`.\n\nVincent",
    "created_at": "2015-06-01T10:20:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325904",
    "user": "https://github.com/videlec"
}
```

<a id='comment:2'></a>
Hello Daniel,

Could you define what you mean by **sequence**. Is it sequence of numbers? Rational numbers? Real numbers? An element of an **infinite cartesian power**? Because there also are

```
sage: words.FibonacciWord('ab')
word: abaababaabaababaababaabaababaabaababaaba...
```
which is an infinite sequence of elements in `{a,b}`.

Vincent



---

archive/issue_comments_325905.json:
```json
{
    "body": "<a id='comment:3'></a>\nDear Vincent,\n\nReplying to [vdelecroix](#comment%3A2):\n> Could you define what you mean by **sequence**. Is it sequence of numbers? Rational numbers? Real numbers? An element of an **infinite cartesian power**? Because there also are\n> \n> ```\n> sage: words.FibonacciWord('ab')\n> word: abaababaabaababaababaabaababaabaababaaba...\n> ```\n> which is an infinite sequence of elements in `{a,b}`.\n\n\nWhat I head in mind is a sequence of numbers (usually intergers, rationals, maybe reals). But of course words are very much related to it. So any ideas/suggestions are very welcome.",
    "created_at": "2015-06-01T10:24:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325905",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:3'></a>
Dear Vincent,

Replying to [vdelecroix](#comment%3A2):
> Could you define what you mean by **sequence**. Is it sequence of numbers? Rational numbers? Real numbers? An element of an **infinite cartesian power**? Because there also are
> 
> ```
> sage: words.FibonacciWord('ab')
> word: abaababaabaababaababaabaababaabaababaaba...
> ```
> which is an infinite sequence of elements in `{a,b}`.


What I head in mind is a sequence of numbers (usually intergers, rationals, maybe reals). But of course words are very much related to it. So any ideas/suggestions are very welcome.



---

archive/issue_comments_325906.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [dkrenn](#comment%3A3):\n> Replying to [vdelecroix](#comment%3A2):\n> > Could you define what you mean by **sequence**. Is it sequence of numbers? Rational numbers? Real numbers? An element of an **infinite cartesian power**? Because there also are\n> > \n> > ```\n> > sage: words.FibonacciWord('ab')\n> > word: abaababaabaababaababaabaababaabaababaaba...\n> > ```\n> > which is an infinite sequence of elements in `{a,b}`.\n\n> \n> What I head in mind is a sequence of numbers (usually intergers, rationals, maybe reals). But of course words are very much related to it. So any ideas/suggestions are very welcome.\n\n\nThen you are more restrictive than [wikipedia](http://en.wikipedia.org/wiki/Sequence) in which a sequence is even allowed to be finite. I do not have suggestion beyond *having precise specifications*. You can perhaps even be explicit in the names by using `number_sequences.*` or `sequences.numbers.*`? But those two looks a bit too much.\n\nVincent",
    "created_at": "2015-06-01T10:29:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325906",
    "user": "https://github.com/videlec"
}
```

<a id='comment:4'></a>
Replying to [dkrenn](#comment%3A3):
> Replying to [vdelecroix](#comment%3A2):
> > Could you define what you mean by **sequence**. Is it sequence of numbers? Rational numbers? Real numbers? An element of an **infinite cartesian power**? Because there also are
> > 
> > ```
> > sage: words.FibonacciWord('ab')
> > word: abaababaabaababaababaabaababaabaababaaba...
> > ```
> > which is an infinite sequence of elements in `{a,b}`.

> 
> What I head in mind is a sequence of numbers (usually intergers, rationals, maybe reals). But of course words are very much related to it. So any ideas/suggestions are very welcome.


Then you are more restrictive than [wikipedia](http://en.wikipedia.org/wiki/Sequence) in which a sequence is even allowed to be finite. I do not have suggestion beyond *having precise specifications*. You can perhaps even be explicit in the names by using `number_sequences.*` or `sequences.numbers.*`? But those two looks a bit too much.

Vincent



---

archive/issue_comments_325907.json:
```json
{
    "body": "<a id='comment:5'></a>\nAnd you also have orthogonal polynomials, orbit of a dynamical system, permutations (i.e. image of the letters), ... Depending on who you are talking to a sequence can be many things!",
    "created_at": "2015-06-01T10:30:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325907",
    "user": "https://github.com/videlec"
}
```

<a id='comment:5'></a>
And you also have orthogonal polynomials, orbit of a dynamical system, permutations (i.e. image of the letters), ... Depending on who you are talking to a sequence can be many things!



---

archive/issue_comments_325908.json:
```json
{
    "body": "<a id='comment:6'></a>\nWhat about 'integer sequences', like for Sloane? Words are already stored in `word.<tab>`",
    "created_at": "2015-06-01T11:04:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325908",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'></a>
What about 'integer sequences', like for Sloane? Words are already stored in `word.<tab>`



---

archive/issue_comments_325909.json:
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [vdelecroix](#comment%3A4):\n> Replying to [dkrenn](#comment%3A3):\n> > Replying to [vdelecroix](#comment%3A2):\n> > > Could you define what you mean by **sequence**. Is it sequence of numbers? Rational numbers? Real numbers? An element of an **infinite cartesian power**? \n\n> > What I head in mind is a sequence of numbers (usually intergers, rationals, maybe reals). [...]\n> \n> Then you are more restrictive than [wikipedia](http://en.wikipedia.org/wiki/Sequence) in which a sequence is even allowed to be finite. \n\n\nI am happy to make it as general as needed.\n\n> I do not have suggestion beyond *having precise specifications*.\n\n\nLike: Finite and infinite sequences where the elements can be any sage objects (but all of the same type/parent)?\n\nBest,\n\nDaniel",
    "created_at": "2015-06-01T11:18:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325909",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:7'></a>
Replying to [vdelecroix](#comment%3A4):
> Replying to [dkrenn](#comment%3A3):
> > Replying to [vdelecroix](#comment%3A2):
> > > Could you define what you mean by **sequence**. Is it sequence of numbers? Rational numbers? Real numbers? An element of an **infinite cartesian power**? 

> > What I head in mind is a sequence of numbers (usually intergers, rationals, maybe reals). [...]
> 
> Then you are more restrictive than [wikipedia](http://en.wikipedia.org/wiki/Sequence) in which a sequence is even allowed to be finite. 


I am happy to make it as general as needed.

> I do not have suggestion beyond *having precise specifications*.


Like: Finite and infinite sequences where the elements can be any sage objects (but all of the same type/parent)?

Best,

Daniel



---

archive/issue_comments_325910.json:
```json
{
    "body": "<a id='comment:8'></a>\nReplying to [ncohen](#comment%3A6):\n> What about 'integer sequences', like for Sloane? Words are already stored in `word.<tab>`\n\n\nOk. Do you think here of a parametrisation (parameter `ZZ` for integer sequences; like the base for some parent)? I ask, because maybe someone wants to have a sequence of rationals (e.g. Bernoulli numbers) or polynomials (again e.g. Bernoulli). This would then be similar to words with an infinite alphabet...",
    "created_at": "2015-06-01T11:24:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325910",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:8'></a>
Replying to [ncohen](#comment%3A6):
> What about 'integer sequences', like for Sloane? Words are already stored in `word.<tab>`


Ok. Do you think here of a parametrisation (parameter `ZZ` for integer sequences; like the base for some parent)? I ask, because maybe someone wants to have a sequence of rationals (e.g. Bernoulli numbers) or polynomials (again e.g. Bernoulli). This would then be similar to words with an infinite alphabet...



---

archive/issue_comments_325911.json:
```json
{
    "body": "<a id='comment:9'></a>\n> Ok. Do you think here of a parametrisation (parameter `ZZ` for integer sequences; like the base for some parent)?\n\n\nNo idea. But I do not think that it is related to what this ticket is about. It seems that you want to create a catalog of some sorts, and we just have to figure out a sensible name for it. I do not think that there is any reason to define any kind of interface for the object that it contains.\n\nI also expect that you will find in the (many) combinat class something which can already handle sequences. In this case, it would be better to use it rather than build a new one.\n\nNathann",
    "created_at": "2015-06-01T11:33:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325911",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:9'></a>
> Ok. Do you think here of a parametrisation (parameter `ZZ` for integer sequences; like the base for some parent)?


No idea. But I do not think that it is related to what this ticket is about. It seems that you want to create a catalog of some sorts, and we just have to figure out a sensible name for it. I do not think that there is any reason to define any kind of interface for the object that it contains.

I also expect that you will find in the (many) combinat class something which can already handle sequences. In this case, it would be better to use it rather than build a new one.

Nathann



---

archive/issue_comments_325912.json:
```json
{
    "body": "<a id='comment:11'></a>\nLet me try to summarize the two apparently orthogonal issues here:\n\n* provide an interface for playing with sequences.  My favorite ticket attempting this is #16137, together with #16107.  This is for sequences indexed by natural numbers, although possibly finite.  If I recall correctly, a generalization might be provided by `Family`.\n\n* provide a catalogue of named (\"famous\") sequences.  I believe that #16137 should be general enough to be able to deal with just about any sequence.  Is there any need to restrict to a certain type of sequence?\n\nIf the scope of this ticket is the second item, there is no conflict.  However, a backend would need to be chosen...",
    "created_at": "2015-06-02T08:06:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325912",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:11'></a>
Let me try to summarize the two apparently orthogonal issues here:

* provide an interface for playing with sequences.  My favorite ticket attempting this is #16137, together with #16107.  This is for sequences indexed by natural numbers, although possibly finite.  If I recall correctly, a generalization might be provided by `Family`.

* provide a catalogue of named ("famous") sequences.  I believe that #16137 should be general enough to be able to deal with just about any sequence.  Is there any need to restrict to a certain type of sequence?

If the scope of this ticket is the second item, there is no conflict.  However, a backend would need to be chosen...



---

archive/issue_comments_325913.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -15,7 +15,9 @@\n ```\n which should provide a unified access point to all possible sequences in SageMath. For example, there is `fibonacci_sequence` in `sage.combinat.combinat`, `binary_recurring_sequence` (somewhere in combinat) and various other code.\n \n-One motivation for creating this, is to include here many sequences, which can be calculated term-by-term (from the previously calculated ones, i.e., recursively) or where the first N terms can be calculated at once efficiently.\n-Nevertheless, it can also provide an interface to sequences, where each term can be calculated explicitly (without dealing with the previous ones).\n+This will use homogenous sequences (#19896), which them-self use lazy lists.\n \n-See the discussion on [sage-devel](https://groups.google.com/forum/#!topic/sage-devel/L-wSCu4fXe8)\n+See also the meta-ticket #16107.\n+\n+---\n+Old: See the discussion on [sage-devel](https://groups.google.com/forum/#!topic/sage-devel/L-wSCu4fXe8)\n``````\n",
    "created_at": "2016-01-17T11:09:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325913",
    "user": "https://github.com/dkrenn"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -15,7 +15,9 @@
 ```
 which should provide a unified access point to all possible sequences in SageMath. For example, there is `fibonacci_sequence` in `sage.combinat.combinat`, `binary_recurring_sequence` (somewhere in combinat) and various other code.
 
-One motivation for creating this, is to include here many sequences, which can be calculated term-by-term (from the previously calculated ones, i.e., recursively) or where the first N terms can be calculated at once efficiently.
-Nevertheless, it can also provide an interface to sequences, where each term can be calculated explicitly (without dealing with the previous ones).
+This will use homogenous sequences (#19896), which them-self use lazy lists.
 
-See the discussion on [sage-devel](https://groups.google.com/forum/#!topic/sage-devel/L-wSCu4fXe8)
+See also the meta-ticket #16107.
+
+---
+Old: See the discussion on [sage-devel](https://groups.google.com/forum/#!topic/sage-devel/L-wSCu4fXe8)
``````




---

archive/issue_comments_325914.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#19896\"",
    "created_at": "2016-01-17T11:09:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325914",
    "user": "https://github.com/dkrenn"
}
```

Changing dependencies from "" to "#19896"



---

archive/issue_comments_325915.json:
```json
{
    "body": "<a id='comment:14'></a>\nBranch pushed to git repo; I updated commit sha1. Last 10 new commits:\n|                                                                                                                                          |                                      |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------|\n|[cc6d05c](http://git.sagemath.org/sage.git/commit/?id=cc6d05c70830ab52b1ed8869c6219ce113af4379)|`move old sequences file to new place`|\n|[1136a64](http://git.sagemath.org/sage.git/commit/?id=1136a64a08784bb730ca6616338078fb6b287ae2)|`fit start on iteration`|\n|[8e4a562](http://git.sagemath.org/sage.git/commit/?id=8e4a56241ae5a3d2ae9b66dafecdf8708e6b8774)|`add a doctest in dropwhile`|\n|[a544d6d](http://git.sagemath.org/sage.git/commit/?id=a544d6dae78eb105fa5c648f4fcd05cd8c07b4f0)|`correct nasty bug in takewhile (complete rewrite of method)`|\n|[3b63c47](http://git.sagemath.org/sage.git/commit/?id=3b63c4792714b2e57dda38b6d9a69e2cf663ba94)|`simplify __repr__ code`|\n|[8354beb](http://git.sagemath.org/sage.git/commit/?id=8354beba255cba9de2b3688cf193dc662c9956b0)|`Merge branch 'extend_lazy_lists' into t/19896/sequences/homogenous-seq`|\n|[0330c78](http://git.sagemath.org/sage.git/commit/?id=0330c7862174182f3aedeb627c5256df17549612)|`Merge branch 'u/dkrenn/sequences/homogenous-seq' of trac.sagemath.org:sage into u/dakrenn/sequences-catalog`|\n|[d8f6707](http://git.sagemath.org/sage.git/commit/?id=d8f670709ad81e0a0309f8f7cf86b93676f7f3e0)|`rewrite sequence generators to new code`|\n|[74f4c4a](http://git.sagemath.org/sage.git/commit/?id=74f4c4a959f29a195de8999005127b39733c0a06)|`sequences in global name space (lazy import)`|\n|[d29cbcd](http://git.sagemath.org/sage.git/commit/?id=d29cbcd24b28c5bf4ababc8ff5e7b17993e28bd7)|`use new code for fibonacci and catalan and deprecate fibonacci_sequence and fibonacci_xrange`|",
    "created_at": "2016-01-17T11:17:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325915",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:14'></a>
Branch pushed to git repo; I updated commit sha1. Last 10 new commits:
|                                                                                                                                          |                                      |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------|
|[cc6d05c](http://git.sagemath.org/sage.git/commit/?id=cc6d05c70830ab52b1ed8869c6219ce113af4379)|`move old sequences file to new place`|
|[1136a64](http://git.sagemath.org/sage.git/commit/?id=1136a64a08784bb730ca6616338078fb6b287ae2)|`fit start on iteration`|
|[8e4a562](http://git.sagemath.org/sage.git/commit/?id=8e4a56241ae5a3d2ae9b66dafecdf8708e6b8774)|`add a doctest in dropwhile`|
|[a544d6d](http://git.sagemath.org/sage.git/commit/?id=a544d6dae78eb105fa5c648f4fcd05cd8c07b4f0)|`correct nasty bug in takewhile (complete rewrite of method)`|
|[3b63c47](http://git.sagemath.org/sage.git/commit/?id=3b63c4792714b2e57dda38b6d9a69e2cf663ba94)|`simplify __repr__ code`|
|[8354beb](http://git.sagemath.org/sage.git/commit/?id=8354beba255cba9de2b3688cf193dc662c9956b0)|`Merge branch 'extend_lazy_lists' into t/19896/sequences/homogenous-seq`|
|[0330c78](http://git.sagemath.org/sage.git/commit/?id=0330c7862174182f3aedeb627c5256df17549612)|`Merge branch 'u/dkrenn/sequences/homogenous-seq' of trac.sagemath.org:sage into u/dakrenn/sequences-catalog`|
|[d8f6707](http://git.sagemath.org/sage.git/commit/?id=d8f670709ad81e0a0309f8f7cf86b93676f7f3e0)|`rewrite sequence generators to new code`|
|[74f4c4a](http://git.sagemath.org/sage.git/commit/?id=74f4c4a959f29a195de8999005127b39733c0a06)|`sequences in global name space (lazy import)`|
|[d29cbcd](http://git.sagemath.org/sage.git/commit/?id=d29cbcd24b28c5bf4ababc8ff5e7b17993e28bd7)|`use new code for fibonacci and catalan and deprecate fibonacci_sequence and fibonacci_xrange`|



---

archive/issue_comments_325916.json:
```json
{
    "body": "Changing commit from \"fa5c40a727f7149fee86e893fc52f27455622929\" to \"d29cbcd24b28c5bf4ababc8ff5e7b17993e28bd7\"",
    "created_at": "2016-01-17T11:17:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325916",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "fa5c40a727f7149fee86e893fc52f27455622929" to "d29cbcd24b28c5bf4ababc8ff5e7b17993e28bd7"



---

archive/issue_comments_325917.json:
```json
{
    "body": "<a id='comment:15'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------|\n|[b384631](http://git.sagemath.org/sage.git/commit/?id=b384631b2e5f3c20e33584ccb9e60d1b7777821c)|`sequences in index.rst`|\n|[87136e5](http://git.sagemath.org/sage.git/commit/?id=87136e55997556ad1f0924d5c1c87d385d691dce)|`rewrite fibonacci-code default argument for \"algorithm\"`|\n|[b0bb4fa](http://git.sagemath.org/sage.git/commit/?id=b0bb4fa7a2a17ed74d25402a29fd4143473f3580)|`extend docs of sequence_generators`|",
    "created_at": "2016-01-17T15:21:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325917",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:15'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                        |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------|
|[b384631](http://git.sagemath.org/sage.git/commit/?id=b384631b2e5f3c20e33584ccb9e60d1b7777821c)|`sequences in index.rst`|
|[87136e5](http://git.sagemath.org/sage.git/commit/?id=87136e55997556ad1f0924d5c1c87d385d691dce)|`rewrite fibonacci-code default argument for "algorithm"`|
|[b0bb4fa](http://git.sagemath.org/sage.git/commit/?id=b0bb4fa7a2a17ed74d25402a29fd4143473f3580)|`extend docs of sequence_generators`|



---

archive/issue_comments_325918.json:
```json
{
    "body": "Changing commit from \"d29cbcd24b28c5bf4ababc8ff5e7b17993e28bd7\" to \"b0bb4fa7a2a17ed74d25402a29fd4143473f3580\"",
    "created_at": "2016-01-17T15:21:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18565#issuecomment-325918",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "d29cbcd24b28c5bf4ababc8ff5e7b17993e28bd7" to "b0bb4fa7a2a17ed74d25402a29fd4143473f3580"
