...
<a id='comment:6'></a>`random_error_position` is not an appropriate name. Perhaps `range_sample` or `sample_range`. The description is not appropriate as well. Should be something like
...
<a id='comment:7'></a>I did some changes, as stated by commit messages above.
...
<a id='comment:10'></a>Replying to [dlucas](#comment%3A7):
> I did some changes, as stated by commit messages above.
> 
> Some remarks:
> 
...
<a id='comment:12'></a>> 13. I would rather implement `StaticErrorRateChannel.transmit_unsafe` as
> 
> ```
> def transmit_unsafe(self, msg):
>     w = msg.copy()
...
<a id='comment:15'></a>Thank you again for the advice!
...
<a id='comment:18'></a>>If you look at the table of contents of coding you have
>[...]
>Not very nice. Though not for this ticket. Think about it for later on the table of contents of code documentation is a mess! But of course the priority is to clean the code. 
...
<a id='comment:23'></a>21. In each class/function/method the documentation should be:
   - one line description
   - then more details in one or several paragraphs if needed. It is not the case in `StaticErrorRateChannel` and `ErrorErasureChannel`.
...
<a id='comment:26'></a>24. In the string representation of channels the input space/output space does not appear
...
```json
{
    "body": "<a id='comment:28'></a>> For the `ErrorErasureChannel` the output space is `CartesianProduct`. For mysterious reason, it is adviced to use `cartesian_product` from `sage.categories.cartesian_product` (see #15425). \n\n\nThanks for the information.\n\nI did the changes (and added input and output space in the representation methods for channels).",
    "created_at": "2015-04-27T11:09:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18269",
    "type": "issue_comment",
...
<a id='comment:28'></a>> For the `ErrorErasureChannel` the output space is `CartesianProduct`. For mysterious reason, it is adviced to use `cartesian_product` from `sage.categories.cartesian_product` (see #15425). 
...
<a id='comment:31'></a>Replying to [dlucas](#comment%3A30):
> Hello,
> 
> > Basically: one of your ticket gets reviewed -> you review one ticket.
