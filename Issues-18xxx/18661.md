# Issue 18661: @interact in ipython notebook

archive/issues_018424.json:
```json
{
    "assignees": [],
    "body": "Apparently `@interact` doesn't work in the Sage 6.7 IPython notebook. The following prints a bunch of HTML code, apparently describing the slider controls:\n\n```\n@interact\ndef f(t=(-1,1)):\n  print(t)\n```\n\nIt would be nice if this could be made to work. If getting there is going to take a bit longer, then it would be nice to see a explanatory message instead of that HTML output.\n\nI'm a bit surprised I don't see a ticket for this yet, but [my search](http://trac.sagemath.org/query?status=needs_info&status=needs_review&status=needs_work&status=new&status=positive_review&component=notebook&summary=~interact&or&status=needs_info&status=needs_review&status=needs_work&status=new&status=positive_review&keywords=~ipython&component=interact&col=id&col=summary&col=status&col=type&col=priority&col=milestone&col=component&order=priority) didn't turn up any likely candidates so far.\n\nKeywords: **ipython interact**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18661_\n\n",
    "closed_at": "2017-07-04T13:49:31Z",
    "created_at": "2015-06-10T12:04:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "@interact in ipython notebook",
    "type": "issue",
    "updated_at": "2017-07-04T13:49:31Z",
    "url": "https://github.com/sagemath/sage/issues/18661",
    "user": "https://github.com/gagern"
}
```
Apparently `@interact` doesn't work in the Sage 6.7 IPython notebook. The following prints a bunch of HTML code, apparently describing the slider controls:

```
@interact
def f(t=(-1,1)):
  print(t)
```

It would be nice if this could be made to work. If getting there is going to take a bit longer, then it would be nice to see a explanatory message instead of that HTML output.

I'm a bit surprised I don't see a ticket for this yet, but [my search](http://trac.sagemath.org/query?status=needs_info&status=needs_review&status=needs_work&status=new&status=positive_review&component=notebook&summary=~interact&or&status=needs_info&status=needs_review&status=needs_work&status=new&status=positive_review&keywords=~ipython&component=interact&col=id&col=summary&col=status&col=type&col=priority&col=milestone&col=component&order=priority) didn't turn up any likely candidates so far.

Keywords: **ipython interact**

_Issue created by migration from https://trac.sagemath.org/ticket/18661_





---

archive/issue_events_247423.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2015-06-10T12:04:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18661#event-247423"
}
```



---

archive/issue_events_247424.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2015-06-10T12:04:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18661#event-247424"
}
```



---

archive/issue_events_247425.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2015-06-10T12:04:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18661#event-247425"
}
```



---

archive/issue_events_247426.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2015-06-10T12:04:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18661#event-247426"
}
```



---

archive/issue_comments_262433.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis is because `@`interact is \"designed\" around print()-ing html: See #18292",
    "created_at": "2015-06-28T16:51:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18661#issuecomment-262433",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'>Comment 1:</a>
This is because `@`interact is "designed" around print()-ing html: See #18292



---

archive/issue_events_247427.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T13:49:31Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18661#event-247427"
}
```



---

archive/issue_events_247428.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T13:49:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18661#event-247428"
}
```



---

archive/issue_events_247429.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T13:49:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18661#event-247429"
}
```



---

archive/issue_events_247430.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T13:49:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18661#event-247430"
}
```



---

archive/issue_events_247431.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-07-04T13:49:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18661#event-247431"
}
```



---

archive/issue_comments_262434.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nFixed by #21267",
    "created_at": "2017-07-04T13:49:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18661#issuecomment-262434",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
Fixed by #21267
