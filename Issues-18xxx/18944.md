...
```json
{
    "body": "But in any case, if we just add a check for input to be comparable elements, then `chain_polytope` will broke. It relies on this (odd / buggy?) behavior.\n\nAnd who will want \"partial maximal chains\" in first place? To compare: there is `exclude`-option in `chains()`.",
...
```json
{
    "body": "Also `chain_polytope()` (not only `order_complex()`) relies on this (IMO) broken `maximal_chains()` -function. Hence I made an internal copy of it with name `_max_chains()`. At least all doctests pass now. For a user perspective everything should be as before, except that `partial`-option is gone.",
    "created_at": "2015-09-15T06:46:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18944",
    "type": "issue_comment",
...
```json
{
    "body": "I agree with Kevin, bad input is bad input and you should not expect correct behavior. So there is no counterexample to the function working as documented, but we should raise an error when not given a saturated chain.",
...
```json
{
    "body": "I am still wondering about this... Other parts of Sage relies on broken or non-defined behaviour of `partial` argument. It feels bad design.",
...
```json
{
    "body": "My guess is like you said, that it that every maximal chain must contain elements (i.e., this partial maximal chain).",
...
```json
{
    "body": "Let us just fix the documentation.\n\n---\nNew commits:",
