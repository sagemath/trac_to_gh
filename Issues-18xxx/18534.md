# Issue 18534: Posets: Adding function from categories to index of functions

archive/issues_018297.json:
```json
{
    "body": "It is more logical for a user to think \"What can I do with this poset?\" than \"What functions are defined in this file?\". This is a suggestion to add `is_selfdual` and `is_lattice` to index of functions in `posets.py`.\n\nCC:  @fchapoton @nathanncohen vivianepons\n\nAuthor: Jori M\u00e4ntysalo\n\nBranch: u/jmantysalo/posets__adding_function_from_categories_to_index_of_functions\n\nCommit: b31c216bae73e63b9066778e2a3d569536ca91d2\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/18534\n\n",
    "closed_at": "2015-09-12T14:08:18Z",
    "created_at": "2015-05-28T09:30:23Z",
    "labels": [
        "component: documentation",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Posets: Adding function from categories to index of functions",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/18534",
    "user": "https://github.com/jm58660"
}
```
It is more logical for a user to think "What can I do with this poset?" than "What functions are defined in this file?". This is a suggestion to add `is_selfdual` and `is_lattice` to index of functions in `posets.py`.

CC:  @fchapoton @nathanncohen vivianepons

Author: Jori Mäntysalo

Branch: u/jmantysalo/posets__adding_function_from_categories_to_index_of_functions

Commit: b31c216bae73e63b9066778e2a3d569536ca91d2

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/18534





---

archive/issue_comments_324775.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jmantysalo/posets__adding_function_from_categories_to_index_of_functions\"",
    "created_at": "2015-05-28T09:32:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324775",
    "user": "https://github.com/jm58660"
}
```

Changing branch from "" to "u/jmantysalo/posets__adding_function_from_categories_to_index_of_functions"



---

archive/issue_comments_324776.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-05-28T09:32:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324776",
    "user": "https://github.com/jm58660"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_324777.json:
```json
{
    "body": "<a id='comment:2'></a>What you think of this?\n\n---\nNew commits:",
    "created_at": "2015-05-28T09:32:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324777",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:2'></a>What you think of this?

---
New commits:



---

archive/issue_comments_324778.json:
```json
{
    "body": "Changing commit from \"\" to \"b31c216bae73e63b9066778e2a3d569536ca91d2\"",
    "created_at": "2015-05-28T09:32:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324778",
    "user": "https://github.com/jm58660"
}
```

Changing commit from "" to "b31c216bae73e63b9066778e2a3d569536ca91d2"



---

archive/issue_comments_324779.json:
```json
{
    "body": "<a id='comment:3'></a>I believe that you should move those functions from categories files to posets files. What you say in the ticket's description is already partly wrong: the index of poset function indicates the functions defined in `FinitePoset`, but not the functions inherited from more abstract classes: I expect that you do not plan to see all poset functions if we ever have an index of functions in `lattices.py`?\n\nNathann",
    "created_at": "2015-05-28T09:37:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324779",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>I believe that you should move those functions from categories files to posets files. What you say in the ticket's description is already partly wrong: the index of poset function indicates the functions defined in `FinitePoset`, but not the functions inherited from more abstract classes: I expect that you do not plan to see all poset functions if we ever have an index of functions in `lattices.py`?

Nathann



---

archive/issue_comments_324780.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [ncohen](#comment%3A3):\n> I believe that you should move those functions from categories files to posets files. What you say in the ticket's description is already partly wrong: the index of poset function indicates the functions defined in `FinitePoset`, but not the functions inherited from more abstract classes: I expect that you do not plan to see all poset functions if we ever have an index of functions in `lattices.py`?\n\n\nSidenote, we do have index in `lattices.py` now: #18489.\n\nI asked about moving functions about a year ago. There was some reasons for not doing it.\n\nI think that everyone using lattices will know that a lattice is a poset, and so can find the functions. This is totally different in this case: plain mathematical knowledge can not reveal at all that some functions are available but from another file.\n\n(There is a real corner case: Student playing with groups may not know what is magma or semigroup. In principle there should be no `.is_abelian()` (or something like that) directly on groups. It should be inherited from magmas. But that might be a problem from documentation point of view.)",
    "created_at": "2015-05-28T09:52:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324780",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:4'></a>Replying to [ncohen](#comment%3A3):
> I believe that you should move those functions from categories files to posets files. What you say in the ticket's description is already partly wrong: the index of poset function indicates the functions defined in `FinitePoset`, but not the functions inherited from more abstract classes: I expect that you do not plan to see all poset functions if we ever have an index of functions in `lattices.py`?


Sidenote, we do have index in `lattices.py` now: #18489.

I asked about moving functions about a year ago. There was some reasons for not doing it.

I think that everyone using lattices will know that a lattice is a poset, and so can find the functions. This is totally different in this case: plain mathematical knowledge can not reveal at all that some functions are available but from another file.

(There is a real corner case: Student playing with groups may not know what is magma or semigroup. In principle there should be no `.is_abelian()` (or something like that) directly on groups. It should be inherited from magmas. But that might be a problem from documentation point of view.)



---

archive/issue_comments_324781.json:
```json
{
    "body": "<a id='comment:5'></a>We both agree that the index of `Lattice` should not list all `Poset` methods, and also that those two functions should appear in the index of `Poset`.\n\nWhat I do not like is to have an index listing things which are not in its module, and that's why I believe that all poset functions should be in the same place, not scattered among `posets.py` and `categories`. I do not mind if somebody comes and gives this ticket a positive review, but as I do not quite agree with what is being done here, please give me the freedom to not stamp it myself.\n\nNathann",
    "created_at": "2015-05-28T10:28:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324781",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:5'></a>We both agree that the index of `Lattice` should not list all `Poset` methods, and also that those two functions should appear in the index of `Poset`.

What I do not like is to have an index listing things which are not in its module, and that's why I believe that all poset functions should be in the same place, not scattered among `posets.py` and `categories`. I do not mind if somebody comes and gives this ticket a positive review, but as I do not quite agree with what is being done here, please give me the freedom to not stamp it myself.

Nathann



---

archive/issue_comments_324782.json:
```json
{
    "body": "<a id='comment:7'></a>Ping. Anybody wants to review a two line patch?",
    "created_at": "2015-07-24T15:12:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324782",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:7'></a>Ping. Anybody wants to review a two line patch?



---

archive/issue_comments_324783.json:
```json
{
    "body": "<a id='comment:8'></a>I finally have a chance to start reviewing things again.\n\nIs there any chance you could look up the justification for keeping some poset functions in `posets.py` and some in `categories`? Because I agree that unifying them would be the better solution.\n\nAlso, why does the index of functions not make any reference to other modules that it may be inheriting functions from? Even if we can expect that (for example) a user will know that every lattice is a poset, the documentation should directly provide a link from the index of functions for Lattices to the index of functions for Posets, rather than forcing them to manually find the relevant page.",
    "created_at": "2015-08-06T18:03:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324783",
    "user": "https://github.com/kevindilks"
}
```

<a id='comment:8'></a>I finally have a chance to start reviewing things again.

Is there any chance you could look up the justification for keeping some poset functions in `posets.py` and some in `categories`? Because I agree that unifying them would be the better solution.

Also, why does the index of functions not make any reference to other modules that it may be inheriting functions from? Even if we can expect that (for example) a user will know that every lattice is a poset, the documentation should directly provide a link from the index of functions for Lattices to the index of functions for Posets, rather than forcing them to manually find the relevant page.



---

archive/issue_comments_324784.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [kdilks](#comment%3A8):\n\n> Is there any chance you could look up the justification for keeping some poset functions in `posets.py` and some in `categories`? Because I agree that unifying them would be the better solution.\n\n\nThis has been discussed on sage-devel. I did not understood all of it, but in any case there will be functions in two places.\n\n> Also, why does the index of functions not make any reference to other modules that it may be inheriting functions from? Even if we can expect that (for example) a user will know that every lattice is a poset, the documentation should directly provide a link from the index of functions for Lattices to the index of functions for Posets, rather than forcing them to manually find the relevant page.\n\n\nCan be done. However, there is BIG difference here: it needs \"only\" mathematical knowledge to know that a lattice is a poset, but it needs knowledge of Sage internals to know that more functions may be found on categories.\n\nBut this is another story and does not belong to this patch.",
    "created_at": "2015-08-08T18:55:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324784",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:9'></a>Replying to [kdilks](#comment%3A8):

> Is there any chance you could look up the justification for keeping some poset functions in `posets.py` and some in `categories`? Because I agree that unifying them would be the better solution.


This has been discussed on sage-devel. I did not understood all of it, but in any case there will be functions in two places.

> Also, why does the index of functions not make any reference to other modules that it may be inheriting functions from? Even if we can expect that (for example) a user will know that every lattice is a poset, the documentation should directly provide a link from the index of functions for Lattices to the index of functions for Posets, rather than forcing them to manually find the relevant page.


Can be done. However, there is BIG difference here: it needs "only" mathematical knowledge to know that a lattice is a poset, but it needs knowledge of Sage internals to know that more functions may be found on categories.

But this is another story and does not belong to this patch.



---

archive/issue_comments_324785.json:
```json
{
    "body": "<a id='comment:10'></a>Snif. My poset documentation polishing project (see #18959, #18925, #18941) will stuck, as nobody wants to review this, but either has said that this should be rejected.\n\nTwo lines only...",
    "created_at": "2015-08-16T04:58:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324785",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:10'></a>Snif. My poset documentation polishing project (see #18959, #18925, #18941) will stuck, as nobody wants to review this, but either has said that this should be rejected.

Two lines only...



---

archive/issue_comments_324786.json:
```json
{
    "body": "<a id='comment:11'></a>Well. You say that nobody rejected it, but I am personally against the changes from two of those tickets. You are only right in the sense that I don't believe that 'not agreeing' is sufficient to 'reject it'.\n\nBy the way, you should really stop with those two-lines tickets and merge those superficial changes into something else (if you find somebody who agrees with it).",
    "created_at": "2015-08-16T07:35:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324786",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:11'></a>Well. You say that nobody rejected it, but I am personally against the changes from two of those tickets. You are only right in the sense that I don't believe that 'not agreeing' is sufficient to 'reject it'.

By the way, you should really stop with those two-lines tickets and merge those superficial changes into something else (if you find somebody who agrees with it).



---

archive/issue_comments_324787.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [ncohen](#comment%3A11):\n> Well. You say that nobody rejected it, but I am personally against the changes from two of those tickets. You are only right in the sense that I don't believe that 'not agreeing' is sufficient to 'reject it'.\n> \n> By the way, you should really stop with those two-lines tickets and merge those superficial changes into something else (if you find somebody who agrees with it).\n\n\nAlready lowering ticket counter with #19023.\n\nI think that the point of this ticket is not about those two lines. It is the discussion about how to enhance documentation of the software.\n\nI would say \"reject\" to leaving things as they are. This patch is my suggestion one. Suggestion two is to add index to category of posets, add index to category of finite posets, and link them at the beginning of index of finite posets. Suggestion three is to make a new page that contains index of poset functions and a short introduction to posets in Sage. All these suggestions assumes that the code itself will not be refactored.\n\n(You may mean that I should stop doing tickets like #18925. If that is the case, I must say that it is hard. To get unified interface it is needed to sometimes go throught a bigger part of Sage. OTOH if I make a big but simple patch of doctrings, it will take a year to write, will needs rebasing ten times, and may never get positive_review.)",
    "created_at": "2015-08-16T08:32:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324787",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:12'></a>Replying to [ncohen](#comment%3A11):
> Well. You say that nobody rejected it, but I am personally against the changes from two of those tickets. You are only right in the sense that I don't believe that 'not agreeing' is sufficient to 'reject it'.
> 
> By the way, you should really stop with those two-lines tickets and merge those superficial changes into something else (if you find somebody who agrees with it).


Already lowering ticket counter with #19023.

I think that the point of this ticket is not about those two lines. It is the discussion about how to enhance documentation of the software.

I would say "reject" to leaving things as they are. This patch is my suggestion one. Suggestion two is to add index to category of posets, add index to category of finite posets, and link them at the beginning of index of finite posets. Suggestion three is to make a new page that contains index of poset functions and a short introduction to posets in Sage. All these suggestions assumes that the code itself will not be refactored.

(You may mean that I should stop doing tickets like #18925. If that is the case, I must say that it is hard. To get unified interface it is needed to sometimes go throught a bigger part of Sage. OTOH if I make a big but simple patch of doctrings, it will take a year to write, will needs rebasing ten times, and may never get positive_review.)



---

archive/issue_comments_324788.json:
```json
{
    "body": "<a id='comment:13'></a>> I would say \"reject\" to leaving things as they are. This patch is my suggestion one. Suggestion two is to add index to category of posets, add index to category of finite posets, and link them at the beginning of index of finite posets. Suggestion three is to make a new page that contains index of poset functions and a short introduction to posets in Sage. All these suggestions assumes that the code itself will not be refactored.\n\n\nMove the two functions from categories to posets.py and the problem is solved. I don't see the added value of having those two lone functions in another file, where nobody will find them. Move them to `posets.py` and they can obviously be added to the doc.\n\n> (You may mean that I should stop doing tickets like #18925. If that is the case, I must say that it is hard. To get unified interface it is needed to sometimes go throught a bigger part of Sage. OTOH if I make a big but simple patch of doctrings, it will take a year to write, will needs rebasing ten times, and may never get positive_review.)\n\n\nWe could start getting rid of those big index, and of the duplication of [1) the description of the function that follows the name of the function in the index 2) the first line of the docstring of a function] by using #18926.\n\nOnce this is done, the individual modifications do not need to be rebased as often, for the parts of code that get modified are not all located in the index: only the individual docstrings need be modified, and git has less problems with that.\n\nNathann",
    "created_at": "2015-08-16T08:40:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324788",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:13'></a>> I would say "reject" to leaving things as they are. This patch is my suggestion one. Suggestion two is to add index to category of posets, add index to category of finite posets, and link them at the beginning of index of finite posets. Suggestion three is to make a new page that contains index of poset functions and a short introduction to posets in Sage. All these suggestions assumes that the code itself will not be refactored.


Move the two functions from categories to posets.py and the problem is solved. I don't see the added value of having those two lone functions in another file, where nobody will find them. Move them to `posets.py` and they can obviously be added to the doc.

> (You may mean that I should stop doing tickets like #18925. If that is the case, I must say that it is hard. To get unified interface it is needed to sometimes go throught a bigger part of Sage. OTOH if I make a big but simple patch of doctrings, it will take a year to write, will needs rebasing ten times, and may never get positive_review.)


We could start getting rid of those big index, and of the duplication of [1) the description of the function that follows the name of the function in the index 2) the first line of the docstring of a function] by using #18926.

Once this is done, the individual modifications do not need to be rebased as often, for the parts of code that get modified are not all located in the index: only the individual docstrings need be modified, and git has less problems with that.

Nathann



---

archive/issue_comments_324789.json:
```json
{
    "body": "<a id='comment:14'></a>Replying to [ncohen](#comment%3A13):\n \n> Move the two functions from categories to posets.py and the problem is solved. I don't see the added value of having those two lone functions in another file, where nobody will find them. Move them to `posets.py` and they can obviously be added to the doc.\n\n\nThat would be my suggestion number zero. But I wrote about that on sage-devel, and was clearly rejected. (And this is not only about two function, see for example `is_antichain_of_poset()`. -- it does not help if you look only the category of *finite* posets.)\n\n> We could start getting rid of those big index, and of the duplication of [1) the description of the function that follows the name of the function in the index 2) the first line of the docstring of a function] by using #18926.\n\n\nThe fact that #18926 is a great improvement (thanks!) does not mean that it will make documentation good. Good index is arranged by topic. And actually I have used a little different phrasing in the index compared to function.",
    "created_at": "2015-08-16T09:08:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324789",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:14'></a>Replying to [ncohen](#comment%3A13):
 
> Move the two functions from categories to posets.py and the problem is solved. I don't see the added value of having those two lone functions in another file, where nobody will find them. Move them to `posets.py` and they can obviously be added to the doc.


That would be my suggestion number zero. But I wrote about that on sage-devel, and was clearly rejected. (And this is not only about two function, see for example `is_antichain_of_poset()`. -- it does not help if you look only the category of *finite* posets.)

> We could start getting rid of those big index, and of the duplication of [1) the description of the function that follows the name of the function in the index 2) the first line of the docstring of a function] by using #18926.


The fact that #18926 is a great improvement (thanks!) does not mean that it will make documentation good. Good index is arranged by topic. And actually I have used a little different phrasing in the index compared to function.



---

archive/issue_comments_324790.json:
```json
{
    "body": "<a id='comment:15'></a>> That would be my suggestion number zero. But I wrote about that on sage-devel, and was clearly rejected.\n\n\nIs \"Clearly rejected\" the difference between my \"I don't like it, so I will not review it\" and somebody else's \"I don't like it, so you must not do it\"?\n\nI still don't know what there is to reject. There was a Poset class with everything inside, then somebody created a Poset category and suddenly it magically becomes \"the best place\" to store code, even though there is nothing in there.\n\n> The fact that #18926 is a great improvement (thanks!) does not mean that it will make documentation good. Good index is arranged by topic. And actually I have used a little different phrasing in the index compared to function.\n\n\nYou can do it with #18926 too. Instead of telling it to print all the functions of the class, you can create several small tables from lists of functions. This way the list is always sorted lexicographically, and you also don't have to copy/paste the first line of each docstring (which gets updated automatically).\n\nFurthermore, because we will have a Python list of the function that are indexed, we can later \"check\" that all functions appear, which we cannot do at the moment.\n\nNathann",
    "created_at": "2015-08-16T09:43:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324790",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:15'></a>> That would be my suggestion number zero. But I wrote about that on sage-devel, and was clearly rejected.


Is "Clearly rejected" the difference between my "I don't like it, so I will not review it" and somebody else's "I don't like it, so you must not do it"?

I still don't know what there is to reject. There was a Poset class with everything inside, then somebody created a Poset category and suddenly it magically becomes "the best place" to store code, even though there is nothing in there.

> The fact that #18926 is a great improvement (thanks!) does not mean that it will make documentation good. Good index is arranged by topic. And actually I have used a little different phrasing in the index compared to function.


You can do it with #18926 too. Instead of telling it to print all the functions of the class, you can create several small tables from lists of functions. This way the list is always sorted lexicographically, and you also don't have to copy/paste the first line of each docstring (which gets updated automatically).

Furthermore, because we will have a Python list of the function that are indexed, we can later "check" that all functions appear, which we cannot do at the moment.

Nathann



---

archive/issue_comments_324791.json:
```json
{
    "body": "<a id='comment:16'></a>Replying to [ncohen](#comment%3A15):\n \n> Is \"Clearly rejected\" the difference between my \"I don't like it, so I will not review it\" and somebody else's \"I don't like it, so you must not do it\"?\n\n\nI remember that it was several peoples \"I don't like\". And now when I check, \"several\" was actually \"two\"... They were Travis and Nicolas, and I am not sure about Nicolas. (But wait - was this discussed twice? Not sure.)\n\n> I still don't know what there is to reject. There was a Poset class with everything inside, then somebody created a Poset category and suddenly it magically becomes \"the best place\" to store code, even though there is nothing in there.\n\n\nI don't know. There was some explanation, of which I think I didn't understood all.\n\n> > The fact that #18926 is a great improvement (thanks!) does not mean that it will make documentation good. Good index is arranged by topic. And actually I have used a little different phrasing in the index compared to function.\n\n> \n> You can do it with #18926 too. Instead of telling it to print all the functions of the class, you can create several small tables from lists of functions.\n\n\nThat sounds better. Lexicographic order is not always the best possible, but that's a minor thing.",
    "created_at": "2015-08-16T10:01:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324791",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:16'></a>Replying to [ncohen](#comment%3A15):
 
> Is "Clearly rejected" the difference between my "I don't like it, so I will not review it" and somebody else's "I don't like it, so you must not do it"?


I remember that it was several peoples "I don't like". And now when I check, "several" was actually "two"... They were Travis and Nicolas, and I am not sure about Nicolas. (But wait - was this discussed twice? Not sure.)

> I still don't know what there is to reject. There was a Poset class with everything inside, then somebody created a Poset category and suddenly it magically becomes "the best place" to store code, even though there is nothing in there.


I don't know. There was some explanation, of which I think I didn't understood all.

> > The fact that #18926 is a great improvement (thanks!) does not mean that it will make documentation good. Good index is arranged by topic. And actually I have used a little different phrasing in the index compared to function.

> 
> You can do it with #18926 too. Instead of telling it to print all the functions of the class, you can create several small tables from lists of functions.


That sounds better. Lexicographic order is not always the best possible, but that's a minor thing.



---

archive/issue_comments_324792.json:
```json
{
    "body": "<a id='comment:17'></a>> I don't know. There was some explanation, of which I think I didn't understood all.\n\n\nI see. Well, if you ever want to move two functions back to the Poset class, just add me in Cc.\n\n> That sounds better. Lexicographic order is not always the best possible, but that's a minor thing.\n\n\nYou can disable it, too.\n\nIf it makes sense, we could even update the code so that it can take as argument a list of functions \"with label\", group the functions according to the lablels and build the corresponding tables. Should take 10 lines at most.\n\nNathann",
    "created_at": "2015-08-16T10:08:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324792",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:17'></a>> I don't know. There was some explanation, of which I think I didn't understood all.


I see. Well, if you ever want to move two functions back to the Poset class, just add me in Cc.

> That sounds better. Lexicographic order is not always the best possible, but that's a minor thing.


You can disable it, too.

If it makes sense, we could even update the code so that it can take as argument a list of functions "with label", group the functions according to the lablels and build the corresponding tables. Should take 10 lines at most.

Nathann



---

archive/issue_comments_324793.json:
```json
{
    "body": "<a id='comment:18'></a>It seems that majority on sage-devel are against this. See for example thread https://groups.google.com/forum/#!topic/sage-devel/rnye9oBdgKk . Hence I move this to wontfix-milestone.",
    "created_at": "2015-09-01T10:00:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324793",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:18'></a>It seems that majority on sage-devel are against this. See for example thread https://groups.google.com/forum/#!topic/sage-devel/rnye9oBdgKk . Hence I move this to wontfix-milestone.



---

archive/issue_events_052111.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-09-01T10:00:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18534#event-52111"
}
```



---

archive/issue_comments_324794.json:
```json
{
    "body": "<a id='comment:19'></a>wontfix tickets must be set to `positive_review`, otherwise they will not be closed.",
    "created_at": "2015-09-02T13:08:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324794",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:19'></a>wontfix tickets must be set to `positive_review`, otherwise they will not be closed.



---

archive/issue_comments_324795.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-09-02T13:08:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324795",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_052112.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:08:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/18534#event-52112"
}
```



---

archive/issue_comments_324796.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2015-09-12T14:08:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/18534#issuecomment-324796",
    "user": "https://github.com/vbraun"
}
```

Resolution: wontfix
