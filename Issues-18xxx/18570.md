# Issue 18570: Cache the result of have_dot2tex

archive/issues_018333.json:
```json
{
    "assignees": [],
    "body": "Currently, `have_dot2tex` is not cached and tests for dot2tex by executing a dot2tex command on required functionality, causing unnecessary slowdown. We cache this result.\n\nAssignee: **@tscrim**\n\nCC:  @nathanncohen @jm58660\n\nKeywords: **dot2tex**\n\nBranch/Commit: **[f930901](https://github.com/sagemath/sagetrac-mirror/commit/f930901163d6e0d4760e6f8a655173dcdb430e45)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18570_\n\n",
    "closed_at": "2015-06-02T20:18:09Z",
    "created_at": "2015-06-01T15:12:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/performance",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cache the result of have_dot2tex",
    "type": "issue",
    "updated_at": "2015-06-02T20:18:09Z",
    "url": "https://github.com/sagemath/sage/issues/18570",
    "user": "https://github.com/tscrim"
}
```
Currently, `have_dot2tex` is not cached and tests for dot2tex by executing a dot2tex command on required functionality, causing unnecessary slowdown. We cache this result.

Assignee: **@tscrim**

CC:  @nathanncohen @jm58660

Keywords: **dot2tex**

Branch/Commit: **[f930901](https://github.com/sagemath/sagetrac-mirror/commit/f930901163d6e0d4760e6f8a655173dcdb430e45)**

Reviewer: **Nathann Cohen**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/18570_





---

archive/issue_events_246269.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-06-01T15:12:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18570#event-246269"
}
```



---

archive/issue_events_246270.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-06-01T15:12:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "label": "https://github.com/sagemath/sage/labels/performance",
    "label_color": "696969",
    "label_name": "performance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18570#event-246270"
}
```



---

archive/issue_events_246271.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-06-01T15:12:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18570#event-246271"
}
```



---

archive/issue_events_246272.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-06-01T15:12:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18570#event-246272"
}
```



---

archive/issue_comments_260577.json:
```json
{
    "body": "Branch: **[public/speedup_dot2tex-18570](https://github.com/sagemath/sagetrac-mirror/tree/public/speedup_dot2tex-18570)**",
    "created_at": "2015-06-01T15:13:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18570#issuecomment-260577",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/speedup_dot2tex-18570](https://github.com/sagemath/sagetrac-mirror/tree/public/speedup_dot2tex-18570)**



---

archive/issue_events_246273.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-06-01T15:13:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18570#event-246273"
}
```



---

archive/issue_comments_260578.json:
```json
{
    "body": "Commit: **[f930901](https://github.com/sagemath/sagetrac-mirror/commit/f930901163d6e0d4760e6f8a655173dcdb430e45)**",
    "created_at": "2015-06-01T15:13:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18570#issuecomment-260578",
    "user": "https://github.com/tscrim"
}
```

Commit: **[f930901](https://github.com/sagemath/sagetrac-mirror/commit/f930901163d6e0d4760e6f8a655173dcdb430e45)**



---

archive/issue_comments_260579.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f930901163d6e0d4760e6f8a655173dcdb430e45\">f930901</a></td><td><code>Cache the result of have_dot2tex.</code></td></tr></table>\n",
    "created_at": "2015-06-01T15:13:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18570#issuecomment-260579",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f930901163d6e0d4760e6f8a655173dcdb430e45">f930901</a></td><td><code>Cache the result of have_dot2tex.</code></td></tr></table>




---

archive/issue_comments_260580.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThanks !",
    "created_at": "2015-06-01T15:15:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18570#issuecomment-260580",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:2" align="right">Comment 2</div>

Thanks !



---

archive/issue_comments_260581.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2015-06-01T15:15:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18570#issuecomment-260581",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_246274.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-06-01T15:15:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18570#event-246274"
}
```



---

archive/issue_events_246275.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-06-01T15:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18570#event-246275"
}
```



---

archive/issue_comments_260582.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI wonder if this has gotten the fastest ticket of all time... :P",
    "created_at": "2015-06-01T15:19:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18570#issuecomment-260582",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

I wonder if this has gotten the fastest ticket of all time... :P



---

archive/issue_comments_260583.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNo, I have one which is below 30 seconds. Fr\u00e9d\u00e9ric Chapoton had a ticket fixing a one-letter typo `:-P`\n\nNathann",
    "created_at": "2015-06-01T15:20:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18570#issuecomment-260583",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

No, I have one which is below 30 seconds. Frédéric Chapoton had a ticket fixing a one-letter typo `:-P`

Nathann



---

archive/issue_events_246276.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-06-02T20:18:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18570#event-246276"
}
```



---

archive/issue_events_246277.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "df98039dc3158f27d3ea0b0605e2ca2c8de1a27a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-06-02T20:18:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18570#event-246277"
}
```



---

archive/issue_comments_260584.json:
```json
{
    "body": "Changed branch from **[public/speedup_dot2tex-18570](https://github.com/sagemath/sagetrac-mirror/tree/public/speedup_dot2tex-18570)** to **[f930901](https://github.com/sagemath/sagetrac-mirror/commit/f930901163d6e0d4760e6f8a655173dcdb430e45)**",
    "created_at": "2015-06-02T20:18:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18570#issuecomment-260584",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/speedup_dot2tex-18570](https://github.com/sagemath/sagetrac-mirror/tree/public/speedup_dot2tex-18570)** to **[f930901](https://github.com/sagemath/sagetrac-mirror/commit/f930901163d6e0d4760e6f8a655173dcdb430e45)**
