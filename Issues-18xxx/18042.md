# Issue 18042: importing Sage library in .spyx files

archive/issues_017805.json:
```json
{
    "assignees": [],
    "body": "Create a file `testing.spyx` containing\n\n```\nfrom sage.all import *\nprint ZZ\n```\nand then run `sage testing.spyx`. This will produce an error:\n\n```\nNameError: name 'ZZ' is not defined\n```\nThe same happens if the import statement is `from sage.all import ZZ`. This file works properly if it is attached or loaded.\n\nI think this used to work and I think this ought to work. Is the problem in `sage-run-cython`? Or in `sage.repl`?\n\nAny fix should add a doctest (or modify the existing doctest) in sage.tests.cmdline for running a `.spyx` file.\n\nCC:  @jdemeyer @vbraun\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18042_\n\n",
    "closed_at": "2017-07-13T07:54:31Z",
    "created_at": "2015-03-23T17:54:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "importing Sage library in .spyx files",
    "type": "issue",
    "updated_at": "2017-07-13T07:54:31Z",
    "url": "https://github.com/sagemath/sage/issues/18042",
    "user": "https://github.com/jhpalmieri"
}
```
Create a file `testing.spyx` containing

```
from sage.all import *
print ZZ
```
and then run `sage testing.spyx`. This will produce an error:

```
NameError: name 'ZZ' is not defined
```
The same happens if the import statement is `from sage.all import ZZ`. This file works properly if it is attached or loaded.

I think this used to work and I think this ought to work. Is the problem in `sage-run-cython`? Or in `sage.repl`?

Any fix should add a doctest (or modify the existing doctest) in sage.tests.cmdline for running a `.spyx` file.

CC:  @jdemeyer @vbraun

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/18042_





---

archive/issue_events_238925.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-03-23T17:54:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238925"
}
```



---

archive/issue_events_238926.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-03-23T17:54:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238926"
}
```



---

archive/issue_events_238927.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-03-23T17:54:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238927"
}
```



---

archive/issue_events_238928.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-03-23T17:54:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238928"
}
```



---

archive/issue_comments_248200.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI have no idea why this happens...",
    "created_at": "2015-03-23T17:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18042#issuecomment-248200",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'>Comment 1:</a>
I have no idea why this happens...



---

archive/issue_comments_248201.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nA workaround, in case anyone needs to do this: `sage -c \"load('filename')\"`",
    "created_at": "2015-03-23T17:58:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18042#issuecomment-248201",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'>Comment 2:</a>
A workaround, in case anyone needs to do this: `sage -c "load('filename')"`



---

archive/issue_comments_248202.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMore generally, first importing all of `sage` fixes the problem.",
    "created_at": "2015-03-23T19:23:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18042#issuecomment-248202",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
More generally, first importing all of `sage` fixes the problem.



---

archive/issue_comments_248203.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2017-06-02T09:44:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18042#issuecomment-248203",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_238929.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-02T09:44:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238929"
}
```



---

archive/issue_events_238930.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-02T09:44:11Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238930"
}
```



---

archive/issue_events_238931.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-02T09:44:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238931"
}
```



---

archive/issue_events_238932.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-02T09:44:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238932"
}
```



---

archive/issue_comments_248204.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nWorks for me now.",
    "created_at": "2017-06-02T09:44:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18042#issuecomment-248204",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
Works for me now.



---

archive/issue_events_238933.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-02T09:44:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238933"
}
```



---

archive/issue_events_238934.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-02T09:44:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238934"
}
```



---

archive/issue_events_238935.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238935"
}
```



---

archive/issue_events_238936.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238936"
}
```



---

archive/issue_events_238937.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18042#event-238937"
}
```



---

archive/issue_comments_248205.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nClosing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).",
    "created_at": "2017-07-13T07:54:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18042#issuecomment-248205",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'>Comment 6:</a>
Closing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).
