...
<a id='comment:3'></a>For performance, replace
...
```json
{
    "body": "<a id='comment:4'></a>Can you please support all memory-allocation functions, like `calloc`, `realloc`, `(re)allocarray` and replace the dangerous(*) calls like\n\n```\nmem.malloc( n * sizeof(unsigned short *))\n```\nby\n\n```\nmem.allocarray(n, sizeof(unsigned short *))\n```\n\n(*) the multiplication can overflow.",
...
<a id='comment:4'></a>Can you please support all memory-allocation functions, like `calloc`, `realloc`, `(re)allocarray` and replace the dangerous(*) calls like
...
```json
{
    "body": "<a id='comment:5'></a>Also for performance: keep `calloc`, do not replace it with `malloc + memset`.",
...
<a id='comment:5'></a>Also for performance: keep `calloc`, do not replace it with `malloc + memset`.
...
```json
{
    "body": "<a id='comment:6'></a>I guess that `realloc(array)` might not be so easy to support, so you can forget about that. But I would certainly support `allocarray` and `calloc`.\n\nAnd I don't understand why you use\n\n```\ncdef void * val = malloc(size)\nif val == NULL:\n    raise MemoryError()\n```\ninstead of\n\n```\ncdef void * val = check_malloc(size)\n```",
...
<a id='comment:6'></a>I guess that `realloc(array)` might not be so easy to support, so you can forget about that. But I would certainly support `allocarray` and `calloc`.
...
```json
{
    "body": "<a id='comment:7'></a>[1] recommends `PyMem_Malloc, PyMem_Realloc, PyMem_Free` over the system functions. Why do you \nstick with `malloc` etc?",
...
<a id='comment:7'></a>[1] recommends `PyMem_Malloc, PyMem_Realloc, PyMem_Free` over the system functions. Why do you 
stick with `malloc` etc?
...
<a id='comment:8'></a>More about performance: note that your use of `MemoryAllocator` needs at least 2 extra memory allocation calls compared to not using `MemoryAllocator`: one to allocate the `MemoryAllocator` object and one to allocate the `pointers` member. If you allocate several pointers, it's worse because you again need more additional `realloc()` calls for `pointers.`
...
<a id='comment:10'></a>More for performance: replace
...
<a id='comment:11'></a>One more detail: replace
...
<a id='comment:15'></a>> This `include 'sage/ext/interrupt.pxi'` should be in the .pyx file.
...
<a id='comment:16'></a>Replying to [dimpase](#comment%3A7):
> [1] recommends `PyMem_Malloc, PyMem_Realloc, PyMem_Free` over the system functions.
...
<a id='comment:18'></a>Replying to [ncohen](#comment%3A15):
> You can add a commit if you like, though please keep the
> code simple and understandable if you do.
...
<a id='comment:19'></a>> I thought that speed was an important reason to invent this new class.
...
<a id='comment:20'></a>Replying to [ncohen](#comment%3A15):
> Makes no difference in any of the functions I touched, so I did not do it. Less
> readable.
...
<a id='comment:22'></a>I wonder whether there are ways to avoid syntactic clutter such as
...
```json
{
    "body": "<a id='comment:23'></a>> I wonder whether there are ways to avoid syntactic clutter such as\n\n\n+1 `:-/`\n\nNathann",
...
<a id='comment:23'></a>> I wonder whether there are ways to avoid syntactic clutter such as
...
<a id='comment:24'></a>Replying to [ncohen](#comment%3A19):
> I need real C arrays. The time it takes to allocate them is not a problem in the graph code.
...
<a id='comment:25'></a>> A memoryview like `cdef int[::1]` should be as fast as a C pointer.
...
```json
{
    "body": "<a id='comment:27'></a>I looked into #18870 and personally, I don't like it.",
...
<a id='comment:27'></a>I looked into #18870 and personally, I don't like it.
...
```json
{
    "body": "<a id='comment:28'></a>Why wouldn't we discuss it with the cython guys? Perhaps they would be willing to provide something we could use?",
...
<a id='comment:28'></a>Why wouldn't we discuss it with the cython guys? Perhaps they would be willing to provide something we could use?
...
```json
{
    "body": "<a id='comment:32'></a>This is ok for me, but someone needs to review my changes.",
    "created_at": "2015-07-08T18:55:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18868",
    "type": "issue_comment",
...
<a id='comment:32'></a>This is ok for me, but someone needs to review my changes.
...
```json
{
    "body": "<a id='comment:33'></a>still, how about `PyMem_Malloc, PyMem_Realloc, PyMem_Free`, as mentioned in  http://docs.cython.org/src/tutorial/memory_allocation.html ?\n\nApart from performance, there could be advantages in sense of debugging, as one can \n`./configure` Python with `--with-pydebug` to catch memory errors...",
...
<a id='comment:33'></a>still, how about `PyMem_Malloc, PyMem_Realloc, PyMem_Free`, as mentioned in  http://docs.cython.org/src/tutorial/memory_allocation.html ?
...
```json
{
    "body": "<a id='comment:35'></a>Third, Python's memory allocator does not support `calloc()` or `allocarray()`, two very useful functions.\n\nAnd if you're speaking about performance, recall that `malloc()` + `memset()` is slower than `calloc()`.",
...
<a id='comment:35'></a>Third, Python's memory allocator does not support `calloc()` or `allocarray()`, two very useful functions.
...
```json
{
    "body": "<a id='comment:36'></a>Can somebody review this please?",
...
<a id='comment:36'></a>Can somebody review this please?
...
<a id='comment:39'></a>Your were faster than me.
I can at least confirm that all long tests pass on `src/sage/graphs/`.
David.
...
```json
{
    "body": "<a id='comment:40'></a>Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. Last 10 new commits:",
    "created_at": "2015-07-24T09:40:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18868",
    "type": "issue_comment",
...
<a id='comment:40'></a>Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. Last 10 new commits:
...
```json
{
    "body": "<a id='comment:43'></a>Conflict with #18868",
    "created_at": "2015-07-28T18:42:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18868",
    "type": "issue_comment",
...
<a id='comment:43'></a>Conflict with #18868
