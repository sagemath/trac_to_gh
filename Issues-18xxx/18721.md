# Issue 18721: Race condition in pexpect - part 2

archive/issues_018484.json:
```json
{
    "assignees": [],
    "body": "This is a follow-up to #17686 and #17924.\n\nThere are still failures in `sagenb`, we should replace all uses of `pexpect.spawn` by `SageSpawn`.\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18721_\n\n",
    "closed_at": "2017-03-30T19:04:36Z",
    "created_at": "2015-06-17T17:50:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Race condition in pexpect - part 2",
    "type": "issue",
    "updated_at": "2017-03-30T19:04:36Z",
    "url": "https://github.com/sagemath/sage/issues/18721",
    "user": "https://github.com/jdemeyer"
}
```
This is a follow-up to #17686 and #17924.

There are still failures in `sagenb`, we should replace all uses of `pexpect.spawn` by `SageSpawn`.

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/18721_





---

archive/issue_events_248210.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-17T17:50:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248210"
}
```



---

archive/issue_events_248211.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-17T17:50:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248211"
}
```



---

archive/issue_events_248212.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-17T17:50:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248212"
}
```



---

archive/issue_events_248213.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-17T17:50:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248213"
}
```



---

archive/issue_comments_263688.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nErik and I just ran \"make ptestlong\" with Sage 7.1.beta2 inside the docker image on\nhttps://hub.docker.com/r/sagemath/sagemath-develop/, and all tests passed.\n\nJeroen suggests the failures may have been solved by the upgrade of pexpect in Sage 7.0: #10295.\n\nLet's wait a bit for more data points, and then maybe we can close this ticket.",
    "created_at": "2016-02-18T15:10:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18721#issuecomment-263688",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:1'>Comment 1:</a>
Erik and I just ran "make ptestlong" with Sage 7.1.beta2 inside the docker image on
https://hub.docker.com/r/sagemath/sagemath-develop/, and all tests passed.

Jeroen suggests the failures may have been solved by the upgrade of pexpect in Sage 7.0: #10295.

Let's wait a bit for more data points, and then maybe we can close this ticket.



---

archive/issue_events_248214.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2016-02-18T15:10:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248214"
}
```



---

archive/issue_comments_263689.json:
```json
{
    "body": "Work Issues: **Run more systematic tests**",
    "created_at": "2016-02-18T15:10:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18721#issuecomment-263689",
    "user": "https://github.com/nthiery"
}
```

Work Issues: **Run more systematic tests**



---

archive/issue_comments_263690.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nClose?",
    "created_at": "2016-04-06T18:36:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18721#issuecomment-263690",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
Close?



---

archive/issue_events_248215.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-30T19:04:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248215"
}
```



---

archive/issue_events_248216.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-30T19:04:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248216"
}
```



---

archive/issue_events_248217.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-30T19:04:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248217"
}
```



---

archive/issue_comments_263691.json:
```json
{
    "body": "Changed work issues from **Run more systematic tests** to none",
    "created_at": "2017-03-30T19:04:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18721#issuecomment-263691",
    "user": "https://github.com/jdemeyer"
}
```

Changed work issues from **Run more systematic tests** to none



---

archive/issue_events_248218.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-30T19:04:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248218"
}
```



---

archive/issue_events_248219.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-30T19:04:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248219"
}
```



---

archive/issue_events_248220.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-30T19:04:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18721#event-248220"
}
```



---

archive/issue_comments_263692.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2017-03-30T19:04:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18721#issuecomment-263692",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**
