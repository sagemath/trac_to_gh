...
```json
{
    "body": "For me, all but two tests fail. Both tests fail with \"connection timed out\" during access to a web data base, and both work fine when done in an interactive session.\n\nHas something in the test framework changed, so that web access via `urllib2` fails during tests?",
    "created_at": "2015-05-26T09:08:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18514",
    "type": "issue_comment",
...
```json
{
    "body": "I see you are compiling some extensions with the `csage` library but there is a ticket (#17854) which will make that library disappear. Ideally, you should only compile against `csage` only if it actually exists.",
...
```json
{
    "body": "The `sig_on()` framework was in `csage` until 6.7 and got moved to the Sage library in 6.8.beta0 (#18027). So you can probably just link against `csage` if the Sage version is <= 6.7",
...
```json
{
    "body": "Bad news. It builds, but I can't import it. Which brings up the question again why there are all these incompatible changes in Sage. They didn't use to occur so frequently in earlier versions of Sage, IIRC.",
...
```json
{
    "body": "Oh, and I also created a new module `pGroupCohomology.auxiliaries`, in which I've put some auxiliary functions that have previously been in `pGroupCohomology.resolution`.",
...
```json
{
    "body": "I hope it is safe to make #18494 a dependency. If I understand correctly, there is only a very trivial doctest failure, and so hopefully it will be in Sage soonish.\n\nMeanwhile I almost gave up to make my package backwards compatible with sage-3.x...",
...
```json
{
    "body": "With `sage_include_directories()` I will not need to construct `CSAGE_PATH`, right?",
...
```json
{
    "body": "I have updated the spkg, and successfully installed it on the Sage version that is provided by #18494.",
...
```json
{
    "body": "Is there a reason not to make a new-style packages for `p_group_cohomology` ?",
...
```json
{
    "body": "Sigh. Talking about complications: The spkg is too large for attaching it on trac. But sage.math.washington isn't available.",
...
```json
{
    "body": "There is #12103 for using MeatAxe implementation of matrix multiplication over small finite non-prime fields. But that was based on version 2.2.4, not 2.4.xx.",
...
```json
{
    "body": "For now, by the way, I think the easiest path is the right one: use your current versions of everything, don't bother trying to use newer versions of MeatAxe, etc.",
...
```json
{
    "body": "So, what is the plan here? Keep the old-style package on this ticket or change to a new-style package? In the latter case, the ticket should be set back to needs_work.",
...
```json
{
    "body": "Argh. There is more work to do. 2.1.5 installs, but doesn't work for non-primepower groups. The error message mentions multiple values for a keyword argument. Strange, I haven't seen that before.",
...
```json
{
    "body": "Sigh. Apparently there is also a change in the pickling of Singular objects that I am responsible for and that I have now to cope with...",
...
```json
{
    "body": "In the previous versions, I already had a class GapPickler to help with pickles of Gap interface objects (by default, their content is lost when pickling, although it can be reconstructed from the string representation in most cases). I guess I could do basically the same for other interfaces.",
...
```json
{
    "body": "I replaced the spkg by a version that *almost* passes the test suite. The only remaining problem is that in the stored data the old pickling of Singular elements is used, which results in a deprecation warning.\n\nSo, my plan is: Re-compute the cohomology for all groups of order 64, store them in a database, put the database into the package, and put it online.\n\nThen, finally create a basic version of a new-style package.",
...
```json
{
    "body": "Is there anything I can do to help right now?",
...
```json
{
    "body": "A more hack solution would be to make the meataxe that you include in this spkg have a different name, and so that it could be used along #12103. However, as you said, the cleanest solution would be to base this off of the meataxe spkg on #12103 as a dependency.\n\nI would imagine that the majority of the work would just be some name/type replacements as long as you weren't doing anything too exotic...at least I doubt they made major changes to their API...",
...
```json
{
    "body": "At least the data structure has remained basically the same (representing matrices as a contiguous block of memory organised in rows of the matrix, each byte contains up to 8 matrix coefficients, depending on the field size, and all arithmetic operations are done by table lookup).",
...
```json
{
    "body": "you might consider packaging your GAP code as a GAP package...",
...
```json
{
    "body": "by the way, would you mind renaming GAP source code files, by giving them suffix, '.g' or '.gap' ?",
...
```json
{
    "body": "Still I'd like to understand what went wrong in the scenario of comment:69 and comment:70. After all, before I can autotoolize it, I should probably first be able to build it.",
...
```json
{
    "body": "How to indicate that the package-to-be depends both on meataxe and I guess as well on some toolchain? I see the following in some `dependencies` files:\n\n```\n$(INST)/$(SAGE_MP_LIBRARY) $(INST)/$(ZLIB)\n\n----------\nAll lines of this file are ignored except the first.\nIt is copied by SAGE_ROOT/build/make/install into SAGE_ROOT/build/make/Makefile.\n```\nor\n\n```\n$(INST)/$(MARKUPSAFE) $(INST)/$(SETUPTOOLS) $(INST)/$(DOCUTILS)\n\n----------\nAll lines of this file are ignored except the first.\nIt is copied by SAGE_ROOT/build/make/install into SAGE_ROOT/build/make/Makefile.\n```\n\nThe developer manual doesn't seem to mention it.",
...
```json
{
    "body": "Please update the ticket description (the last comment you made is a good starting point).",
...
```json
{
    "body": "It turns out that I need to expose more stuff in modular_resolution.h. My wrapper in the old spkg imports functions from various headers but I suppose it is nicer if modular_resolution-1.0 only installs a single header, modular_resolution.h, and not in addition nDiag.h etc.",
...
```json
{
    "body": "Now I have a technical problem: I can not scp the new tarball to my account in Jena. rcp doesn't work (I suppose Jena is blocking it), scp doesn't work (my landlord is blocking ssh).\n\nDo you have a better solution than to instrument sagemathcloud as a tool to scp the file?",
...
```json
{
    "body": "as a last resort, share it on sagemathcloud, and we'll put it somewhere.",
...
```json
{
    "body": "I need to work on the package a bit more anyway: As it turns out, I didn't put into the library what I want to import later. So, I can post the new package tomorrow, when I am at university.",
...
```json
{
    "body": "Autotools by itself doesn't care about pic (since it doesn't know anything about shared libraries, really). Libtool does the right thing, though. \n\nAlso, I'm confused: libmodres.a is a static library, but the ld error message refers to shared library creation.",
...
```json
{
    "body": "I have updated the tarball, consequently updated the checksum, and added a .pxd header for the package. The package contains a test (\"Out of given basic data for the dihedral group of order 8, compute some additional data, and test that these data are readable and contain a certain result\").\n\nA little sanity test that I did:\n\n```\nsage: cython(\"\"\"#!clib modres mtx\n....: from sage.libs.modular_resolution cimport *\n....: def test():\n....:     cdef group_t *G = newGroupRecord()\n....:     freeGroupRecord(G)\n....: \"\"\")\nsage: test()\n```",
...
```json
{
    "body": "`libmodres_a_LDFLAGS  = -static`?",
...
```json
{
    "body": "If I should change to creating a static library then please tell me how, and if you say that I am creating the dynamic library in the wrong way then please tell me how to do better.",
...
```json
{
    "body": "There is one more file whose new location has to be determined: `GroupNames.sobj`. Thus, it is a Sage pickle, and it contains a dictionary assigning human-readable names (such as DihedralGroup(8)) to some addresses from the small groups library (such as SmallGroup(8,3)).\n\nSo, questions:\n- What is a natural location for a data file that is part of the [SageMath](SageMath) distribution?\n- Do you happen to know a Gap function that can provide human-readable names (and other useful information) for a wider range of groups?",
...
```json
{
    "body": "I have posted an updated version of the upstream tarball. After changing from `lib_LIBRARIES` to `lib_LTLIBRARIES`, using -fPIC was not needed any longer.\n\nNow, I try to move the Cython wrappper.",
...
```json
{
    "body": "Perhaps I should have a single tar file containing all the data, and then add an install-data-hook that untars the data file?",
...
```json
{
    "body": "A question about copyright statements: When I put all the Cython and Python files into the Sage library, should I preserve the old copyright statements such as `Copyright (C) 2009 Simon A. King <simon.king`@`uni-jena.de>`, or should I add new ones, i.e. `Copyright (C) 2015 Simon A. King <simon.king`@`uni-koeln.de>`?",
...
```json
{
    "body": "Information on \"Debug the modification of the modular resolution code\": The old meataxe had row and column numbers one-based, the new has it zero-based. It seems to me that in some locations of the modular resolution code, this needs to be coped with.",
...
```json
{
    "body": "I need some modifications to the MeatAxe wrapper, and I'd appreciate it to be reviewed tat #21437.",
...
```json
{
    "body": "If `database_gap` is a dependency of `meataxe` (it seems so from the commands above) then it should be in `build/pkgs/meataxe/dependencies`.\n\nSame commentary for `build/pkgs/modular_resolution/dependencies` - it should mention `meataxe`.",
...
```json
{
    "body": "files in `src/sage/groups/modular_cohomology/` are not python3-ready (all these `print blah` instead of `print(blah)`), this causes havoc while attempting to run tests, it seems. E.g.\n\n```\n       print CohomologyRing(Integer(64),Integer(12))  # indirect doctest\n                           ^\n    SyntaxError: invalid syntax\n```",
...
```json
{
    "body": "I created #24359 to change MeatAxe into a dynamic library.",
...
```json
{
    "body": "autotools defaults to `-g -O2` for `CFLAGS`.",
...
```json
{
    "body": "The old Sage meataxe spkg used `-O` (which means `-O1`) as optimization flag. So the change of flag might explain the increase in performance that you saw.",
...
```json
{
    "body": "Is it advised to replace \"old\" string formatting such as `\"%s bar\"%foo` with \"new\" string formatting such as `\"{} bar\".format(foo)`? If it is: Is there a tool that converts the first syntax into the second? I wouldn't like to change it manually, as there are hundreds of occurrences in the code.",
...
```json
{
    "body": "In recent versions of Python and Cython, there is the even newer syntax `f\"{foo} bar\"`. Unless you specifically want to support older versions, I would recommend that.",
...
```json
{
    "body": "Replying to [comment:225 jdemeyer]:\n> Replying to [comment:223 SimonKing]:\n> > So, the patch seems pointless.\n\n> \n> That is because the monkey-patch is already applied in Sage.\n\n\nI see.\n\n> But I just realized that you are right: even with `binding=True`, *default* values are not displayed in the docs. So you found an actual bug!\n\n\nHooray!!",
...
```json
{
    "body": "GAP 4.9 (a beta released 1 Feb) has SmallGroups licensed under Artistic License 2.0, which is GPL-compatible. So we should just make database_gap standard, and you should not try to work around this non-problem...",
...
```json
{
    "body": "This ticket makes some seemingly unrelated changes to `src/sage/libs/meataxe` and `src/sage/matrix`. Is that intentional?",
...
```json
{
    "body": "Is there a public website or repository for this package? If so, it should be put in `SPKG.txt`. And I would remove the version number from the first line of that file, you'll just forget to update it anyway.",
...
```json
{
    "body": "Don't be afraid of Unicode: you can put names like Sikiri\u0107 and Poincar\u00e9 in `SPKG.txt`",
...
```json
{
    "body": "I have modified cochain.pyx so that it builds with the current pxd header in sage.structure.element. The SPKG.txt is modified as requested. The checksum is updated. The tarball is updated as well (still at the location stated in the ticket description), still with version number 3.0. And I just checked that the package still installs.\n\nConcerning the dependencies, I think it was consented at some point that the `dependencies` files list build time dependencies, so that all packages can be built in a correct order. spkg-install does test whether the SmallGroups library is available and prints a warning if it isn't, but this won't prevent the package from being installable.\n\nNeeds review, then...",
...
```json
{
    "body": "And the smallgroups warning is broken too:\n\n```\n./spkg-install: line 33: [: too many arguments\n```",
...
```json
{
    "body": "The `modular_resolution` installation got broken by #24106.",
...
```json
{
    "body": "A solution for #24106 would be to use plain `$MAKE install` instead of `sdh_make_install`.",
...
```json
{
    "body": "The Python package is packaged badly. It contains a left-over `build` directory. Really, the proper way to package this is to use `python setup.py sdist` for the Python part and `make dist` for the autotools part.\n\nAlso, given that the package is rather large, you could consider using `.xz` compression. Note that `xz` is a standard package in Sage, just add `| xz` to the dependencies.",
...
```json
{
    "body": "I think that `global-include` is meant to specify files, not paths. \n\nSee https://docs.python.org/2/distutils/sourcedist.html#commands",
...
```json
{
    "body": "Sorry, but in the next few days I will very probably not find the time to fix it.",
...
```json
{
    "body": "Meanwhile I got the impression that the reason for `ValueError: Cannot initialise non-square matrix from 1` stems from a change in Sage and not from a change in my code. Namely, I went back to a version in which I know that all doctests used to pass, changed some `cdef _mul_` into `cpdef _mul_` and tested again. But I still got the same ValueError.\n\nSo, apparently I need to initialise the matrix differently.",
...
```json
{
    "body": "As it turns out, the whole optional meataxe backend somehow got broken. There have been some matrix related changes in Sage recently - and apparently they haven't been tested with optional packages installed. I'll open a ticket for that issue.",
...
```json
{
    "body": "With the current upstream tarball (posted at the location stated in the ticket description) and the current branch (and the dependency), the package builds and passes its test suite. Finally it can be reverted to \"needs review\"!",
...
```json
{
    "body": "To clarify, I only ran `./sage -i -c p_group_cohomology`, at which point Sage installed `database_gap` and `meataxe` and then `p_group_cohomology`, running the test suite for each.  Running `./sage -b` and then repeating the `./sage -i -c ...` command seems to have worked: doctests are passing.\n\nA question: why not run long tests, since I see that a few tests are marked `# long time`? I haven't tried that; how much time would it add to the tests?",
...
```json
{
    "body": "Actually the time for `sage -t --long` happened to be shorter than the time for `sage -t` (so, perhaps my laptop was more busy when I installed and tested the package in its current version and had less load when I re-tested with long tests).\n\nThat indicates that the long tests should be part of the test suite. I'll change it accordingly, but will wait for input of sage-devel concerning the other issues.",
...
```json
{
    "body": "If the testsuite depends on running Sage, then `$(SAGERUNTIME)` must be specified as dependency: just append `$(SAGERUNTIME)` to `build/pkgs/p_group_cohomology/dependencies`.",
...
```json
{
    "body": "I would prefer a random test and just the raw figures. That will be more robust.",
...
```json
{
    "body": "Replying to [comment:334 dimpase]:\n> I do not think it is worthwhile to spend any nontrivial amount of time on \"work without the SmallGroups library\". The new GAP release, 4.9, with the changed SmallGroups library license will be released very soon. While it might take some time to get 4.9 into Sage (!libGAP, you know...), we can trivially (I think) backport the \"new\" SmallGroups, if needed.\n\n\nMaking database_gap a *standard* spkg?",
...
```json
{
    "body": "Replying to [comment:336 SimonKing]:\n\n> Making database_gap a *standard* spkg?\n\nyep.",
...
```json
{
    "body": "Note that I'm not suggesting that optional modules in the Sage library is the best solution. I'm just saying what the possibilities are.",
...
```json
{
    "body": "It's generally a good idea to run `autoreconf -fiv` before packaging a release as it ensures that all autotools-generated files are up-to-date. This includes also `config.guess` which has regularly been an issue in the past (see #19714 for example).",
...
```json
{
    "body": "Strangely, the tests eventually are executed. But certainly Sage shouldn't be sleeping during tests.",
...
```json
{
    "body": "Also, when I interrupt the tests with Ctrl-c, I don't get a bash. So, somehow Sage hangs even when it is interrupted. I will now re-try with verbose tests.",
...
```json
{
    "body": "I just tested in a new Sage session: Yes, `H1 = CohomologyRing(8,3)` does what I expected. It just takes the ring out of the data base and is done in a fraction of a second.",
...
```json
{
    "body": "During the tests, basically *each* cohomology ring definition hangs for several minutes before it suddenly works.\n\nCan some of you please run the tests, too?",
...
```json
{
    "body": "I confirm that the testsuite is connecting to the internet.",
...
```json
{
    "body": "Your package is trying to read database files that do not exist. For example `pGroupCohomology/4gp2/H4gp2.sobj` but there is no `4gp2` directory.",
...
```json
{
    "body": "Is it intentional that the *tarball* `pGroupCohomology/8gp3.tar.gz` is installed? That looks like a bug to me.",
...
```json
{
    "body": "Do you still have the previous version which didn't have the \"internet connection\" problem?",
...
```json
{
    "body": "When I fixed the internet problem (I DID fix it!), I made sure that all tests pass, regardless whether internet connection is available or not. Even the tests that were explicitly about accessing the database in the web has been modified so that only local file access is needed.\n\nReplying to [comment:373 jhpalmieri]:\n> Replying to [comment:370 jdemeyer]:\n> > Is it intentional that the *tarball* `pGroupCohomology/8gp3.tar.gz` is installed? That looks like a bug to me.\n\n> \n> The bug is probably that `8gp3.tar.gz` is in `group_cohomology_database.tar`.\n\n\nIf I recall correctly, `8gp3.tar.gz` was specifically added in order to be able to simulate a web database by local file access in doctests. So, rather not a bug.",
...
```json
{
    "body": "Note that the cpu time is only a bit more than 10 minutes. So, it must be something outside of Sage. Could even be file access.",
...
```json
{
    "body": "If my error is not reproducible for anyone else, it may be due to breakage with the most recent Xcode on OS X: that has at least caused some problems which might be related to Singular. People should test it on other platforms, though.",
...
```json
{
    "body": "Would it not be possible to put all files needed for the testsuite in the package? That way, you avoid the internet connection.",
...
```json
{
    "body": "Replying to [comment:401 jdemeyer]:\n> Replying to [comment:375 SimonKing]:\n> > If I recall correctly, `8gp3.tar.gz` was specifically added in order to be able to simulate a web database by local file access in doctests. So, rather not a bug.\n\n> \n> If that file is only needed for the testsuite, it shouldn't be installed. You could ship it in the sources in some `testdata` subdirectory or whatever.\n\n\nGood idea!",
...
```json
{
    "body": "Let's say that there should at least be a global option to specify the sources. I let you decide whether you want an additional argument when creating the ring.",
...
```json
{
    "body": "That is apparently something sage-specific as part of `sage-pip-install`. This has only apparently appeared only on one other ticket #22756, but that doesn't seem to have any useful information about the warning message. Might be worthwhile to look at what the precise output of your `setup.py`.",
...
```json
{
    "body": "That is a general Sage issue: we do not compile optional extensions after `/sage -i`.",
...
```json
{
    "body": "Right...hmm...IDK. I don't have my laptop in front of me to check in more detail. I will do so later today.",
...
```json
{
    "body": "The problem is in running the tests, so would it make more sense for p_group_cohomology's `spkg-check` to test whether meataxe can be imported?",
...
```json
{
    "body": "In any case, I'm not sure that this is an obstacle to giving a positive review here.",
...
```json
{
    "body": "If there are no good solutions, I am willing to give this a positive review. (Although I like the idea of testing to see if `sage.libs.meataxe` can be imported at the end of `spkg-install` and printing a helpful message if not.)",
...
```json
{
    "body": "Is my change to spkg-install acceptable? I cannot test it at the moment, as I am running a computation that I wouldn't like to interrupt (it will fun for a week or so).\n\nPS: I'm doing the test for proper installation of meataxe the beginning of spkg-install.",
...
```json
{
    "body": "Can you please suggest `make sagelib` instead of `sage -b`? The comand `sage -b` is really a more low-level command that you should only use if you know what you are doing. I would never recommend it like that.",
...
```json
{
    "body": "Also the error message `MeatAxe cannot be imported` is not really right. It should be `the Sage interface to MeatAxe cannot be imported` or something like that.",
...
```json
{
    "body": "For me personally, the standard command that I use is `make build` because it's fast (unlike plain `make` which also builds the doc) and it works well. The only time that I run `sage -b` is indirectly by running `sage -bt ...` (combining `sage -b` and `sage -t`).",
...
```json
{
    "body": "Typo: `sage.lib.meataxe` -> `sage.libs.meataxe`",
...
```json
{
    "body": "Replying to [comment:461 jdemeyer]:\n> Typo: `sage.lib.meataxe` -> `sage.libs.meataxe`\n\n\nIn the error message, you mean. Fixed!",
...
```json
{
    "body": "Thank you for the explanation. I have changed the error message accordingly.",
...
```json
{
    "body": "I am a bit worried: From a discussion on sage-devel, I got the impression that a recently merged ticket broke several optional packages, including database_gap, which also affects p_group_cohomology. So, what's the status? Is it safe to use the current Sage beta release to work with optional packages?\n\nAlso: Is there more than #25079 that prevents completion of the review of this ticket?",
...
```json
{
    "body": "The corresponding (sub)issue is fixed in #25323, and the whole (hopefully) problem in #25332, should be in the coming beta.",
...
```json
{
    "body": "what does this ticket have to do with #25106 ?",
...
```json
{
    "body": "Replying to [comment:475 dimpase]:\n> what does this ticket have to do with #25106 ?\n\n\nNothing whatsoever, I believe.",
...
```json
{
    "body": "https://en.wikipedia.org/wiki/Wikipedia:Chesterton%27s_fence",
    "created_at": "2018-05-21T20:02:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18514",
    "type": "issue_comment",
...
```json
{
    "body": "You could at least have read [comment:362] which added the dependency and ticket #25106 which explains the problem.",
...
```json
{
    "body": "Note that #25106 doesn't need to be dependency but then [comment:362] needs to be fixed in a different way.",
...
```json
{
    "body": "Since #25106 has a positive review, what difference does it make now? Can we move on?",
...
```json
{
    "body": "Replying to [comment:482 jhpalmieri]:\n> Since #25106 has a positive review, what difference does it make now? Can we move on?\n\n\n+1\n\nI'll take care of #25079 soon.",
...
```json
{
    "body": "I just tested that both #25079 and #25490 cleanly merge with the current branch from here. So, I see no need to post a merge commit here. Do you?",
...
```json
{
    "body": "Simon: do you mind if I rewrite git history on this ticket and its dependencies? The dependencies are becoming a mess with various tickets depending on each other. I'd like to make the dependency graph linear:\n\n#25476 -> #25079 -> #25511 -> #18514",
...
```json
{
    "body": "Replying to [comment:497 jdemeyer]:\n> I just saw this commit here:\n> ...\n> I guess that should be moved to #25511.\n\n\nIndeed.",
...
```json
{
    "body": "There is also a conflict of sorts with #25490 because both add the `MPB` field.",
...
```json
{
    "body": "Rebased everything and added an intermediate ticket #25518. The dependency chain is now\n\n#25476 -> #25079 -> #25518 -> #25511 -> #18514\n\nI'm done for now. Simon, feel free to continue working on this.",
...
```json
{
    "body": "I don't understand why Singular gets involved, but the error in the previous comment occurs when trying to call `Matrix_gfpn_dense._new`, which apparently got removed in #25511. So, I now know how to work around. But the compiler error in comment:505 still needs explanation.",
...
```json
{
    "body": "The checksums didn't work for me, but once I fixed those, all tests in the test suite passed. Is there anything else that needs to be done here?",
...
```json
{
    "body": "I prefer to do a final review after all dependencies have been merged. You can remind me when 8.3.beta6 is released.",
...
```json
{
    "body": "Since there are no longer any changes to Sage here, it's just a matter of checking that the package builds and passes its testsuite, ideally on several platforms.",
...
```json
{
    "body": "Why does \"git trac checkout 18514\" followed by \"git trac pull\" NOT yield the current branch?",
...
```json
{
    "body": "I guess using new_mtx instead of makeMTX is a good idea, partly because in many cases there will be a template, so that the creation of the new matrix will be even faster.",
...
```json
{
    "body": "I tried \"pip uninstall pip\", which first seemed to work. But now it seems that I have to rebuild Sage from scratch.",
...
```json
{
    "body": "There is an undeclared dependency on `matplotlib`: the testsuite plots stuff, so you should add `matplotlib` as dependency (order-only, so after `|`).",
    "created_at": "2018-06-30T09:40:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18514",
    "type": "issue_comment",
...
```json
{
    "body": "OK, let me test this new version on a few machines. If everything goes well, I agree that positive review can be given.",
...
```json
{
    "body": "I have simplified the tests and still think that they cover a wide range of functionality. This time I didn't get a warning (and I tested 10 times):\n\n```\n$ ./sage -t --warn-long src/sage/tests/modular_group_cohomology.py\nRunning doctests with ID 2018-07-01-00-21-35-4bcb0f8f.\nGit branch: t/18514/upgrade_of_group_cohomology_spkg\nUsing --optional=database_gap,meataxe,mpir,p_group_cohomology,python2,sage\nDoctesting 1 file.\nsage -t --warn-long src/sage/tests/modular_group_cohomology.py\n    [17 tests, 5.15 s]\n----------------------------------------------------------------------\nAll tests passed!\n----------------------------------------------------------------------\nTotal time for all tests: 5.2 seconds\n    cpu time: 4.0 seconds\n    cumulative wall time: 5.2 seconds\n```",
