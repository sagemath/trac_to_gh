...
```json
{
    "body": "<a id='comment:4'></a>fixed a failing doctest\n\n---\nNew commits:",
    "created_at": "2015-04-19T10:08:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18223",
    "type": "issue_comment",
...
<a id='comment:4'></a>fixed a failing doctest
...
<a id='comment:6'></a>This is forced to me by git, even when the merge is trivial. I work like that:
...
<a id='comment:10'></a>Salut Vincent!
...
<a id='comment:11'></a>Salut Nicolas,
...
<a id='comment:13'></a>I've now derived a class from `sets.cartesian_product.CartesianProduct` for posets. One can specify by a parameter which order one wants to take. User defined orderes can be used as well.
...
```json
{
    "body": "<a id='comment:14'></a>This new implementation is now used in #18587 for the asymptotic growth groups (#17600, underlying the asymptotic ring #17601)",
...
<a id='comment:14'></a>This new implementation is now used in #18587 for the asymptotic growth groups (#17600, underlying the asymptotic ring #17601)
...
<a id='comment:19'></a>5. Instead of `order=components` I would use `order=product` which is the standard name (see [wikipedia](https://en.wikipedia.org/wiki/Product_order)). And put it first in your list in view of comment 4.
...
<a id='comment:21'></a>Thanks for having a look at the code Vincent! I'd like to add some more comments as well.
...
<a id='comment:25'></a>Replying to [vdelecroix](#comment%3A18):
> 1. I think that the class `CartesianProductPosets` would better be somewhere in `combinat/posets` rather in `sets/cartesian_product`. It is always better to split files.
...
<a id='comment:27'></a>Hi Daniel, hi Benjamin,
...
<a id='comment:31'></a>Replying to [vdelecroix](#comment%3A27):
> Concerning the `ElementWrapper` I indeed remember that I had the same trouble when converting some code to the new testing framework.
...
<a id='comment:32'></a>Instead of using `QQ` which is not a poset in the doctest why not use available ones from `Posets.<tab>`?
...
<a id='comment:35'></a>Replying to [vdelecroix](#comment%3A32):
> Instead of using `QQ` which is not a poset in the doctest why not use available ones from `Posets.<tab>`?
...
<a id='comment:36'></a>Replying to [dkrenn](#comment%3A35):
> Replying to [vdelecroix](#comment%3A32):
...
<a id='comment:38'></a>Replying to [vdelecroix](#comment%3A36):
> Replying to [dkrenn](#comment%3A35):
> > > 12. What is the point of adding `*kwargs` in the `__init__` method of `CartesianProduct`? If this argument is not supported then it should simply not exists.
...
```json
{
    "body": "<a id='comment:39'></a>two failing doctests",
...
<a id='comment:39'></a>two failing doctests
...
<a id='comment:40'></a>I fixed the two failing doctests (in `6.9.beta7` the category of the cartesian product of `ZZ` with itself changed) and merged `6.9.rc0` into this branch.
...
<a id='comment:41'></a>Replying to [behackl](#comment%3A40):
> Vincent, as you have done quite some work here would you like to add your name to the reviewers?
...
```json
{
    "body": "<a id='comment:42'></a>Replying to [vdelecroix](#comment%3A41):\n> The branch is in good shape. I would like to go through the whole changes a last time. If everything is fine I will set the positive review. Is that ok?\n\n\nSure! Thanks for going over it once again! `:-)`\n\nBenjamin",
...
<a id='comment:42'></a>Replying to [vdelecroix](#comment%3A41):
> The branch is in good shape. I would like to go through the whole changes a last time. If everything is fine I will set the positive review. Is that ok?
...
<a id='comment:43'></a>16. The name `CartesianProductPosets` looks weird. I would rather go for `CartesianProductPoset` (no `s`) or `PosetsCartesianProduct`. As it is currently it looks like *the set of posets that are made from cartesian products*.
...
```json
{
    "body": "<a id='comment:46'></a>All right. Thanks for the last changes.\n\nVincent",
    "created_at": "2015-09-30T13:32:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/18223",
    "type": "issue_comment",
...
<a id='comment:46'></a>All right. Thanks for the last changes.
