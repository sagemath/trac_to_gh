# Issue 3696: [with spkg, positive review] Fix gmp.spkg build issue on Solaris

archive/issues_003696.json:
```json
{
    "body": "Due to bashisms on Solaris gmp fails to build. This is fixed by using \"/usr/bin/env bash\" instead of \"/bin/sh\" in spkg/install. That is the only change to \n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.6/alpha1/gmp-4.2.2.p1.fake.spkg\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3696\n\n",
    "closed_at": "2008-07-21T19:14:40Z",
    "created_at": "2008-07-21T19:07:54Z",
    "labels": [
        "component: porting: solaris",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.6",
    "title": "[with spkg, positive review] Fix gmp.spkg build issue on Solaris",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3696",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Due to bashisms on Solaris gmp fails to build. This is fixed by using "/usr/bin/env bash" instead of "/bin/sh" in spkg/install. That is the only change to 

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.6/alpha1/gmp-4.2.2.p1.fake.spkg

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3696





---

archive/issue_events_013589.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3696",
    "rename": {
        "from": "[with spkg, needs review] Fix gmp.spkg build issue on Solaris",
        "to": "[with spkg, positive review] Fix gmp.spkg build issue on Solaris"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3696#event-13589"
}
```



---

archive/issue_comments_027621.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.0.6.rc0",
    "created_at": "2008-07-21T19:14:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3696#issuecomment-27621",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.0.6.rc0



---

archive/issue_comments_027622.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-07-21T19:14:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3696#issuecomment-27622",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_013590.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-21T19:14:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3696",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3696#event-13590"
}
```
