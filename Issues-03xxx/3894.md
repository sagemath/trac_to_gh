# Issue 3894: in tutorial, live version, triple dots are not visible

archive/issues_003894.json:
```json
{
    "assignees": [],
    "body": "In examples like this in the tutorial:\n\n```\nsage: def is_even(n):\n...       return n%2 == 0\n```\nthe three dots are not actually visible in the 'live' version of the documentation.  So change the documentation to try to reflect this.  (It would be better to have text printed conditionally, depending on whether it was for the live version, the static version, or the dvi/pdf version, but I don't know how to do that.)\n\n\n\nKeywords: **tutorial**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3894_\n\n",
    "closed_at": "2008-09-16T03:50:55Z",
    "created_at": "2008-08-19T03:25:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "in tutorial, live version, triple dots are not visible",
    "type": "issue",
    "updated_at": "2008-09-16T03:50:55Z",
    "url": "https://github.com/sagemath/sage/issues/3894",
    "user": "https://github.com/jhpalmieri"
}
```
In examples like this in the tutorial:

```
sage: def is_even(n):
...       return n%2 == 0
```
the three dots are not actually visible in the 'live' version of the documentation.  So change the documentation to try to reflect this.  (It would be better to have text printed conditionally, depending on whether it was for the live version, the static version, or the dvi/pdf version, but I don't know how to do that.)



Keywords: **tutorial**

_Issue created by migration from https://trac.sagemath.org/ticket/3894_





---

archive/issue_events_038553.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-08-19T03:25:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "milestone_number": null,
    "milestone_title": "sage-3.1.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38553"
}
```



---

archive/issue_events_038554.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-08-19T03:25:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38554"
}
```



---

archive/issue_events_038555.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-08-19T03:25:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38555"
}
```



---

archive/issue_events_038556.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-08-19T03:25:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38556"
}
```



---

archive/issue_events_038557.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-08-19T03:25:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38557"
}
```



---

archive/issue_events_038558.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-09-16T02:10:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38558"
}
```



---

archive/issue_comments_022606.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[3894.patch.gz](https://github.com/sagemath/sage/files/ticket3894/3894.patch.gz)**\n\nLooks good.  I've made this change in the ReST version of the tutorial too.",
    "created_at": "2008-09-16T02:10:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3894#issuecomment-22606",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[3894.patch.gz](https://github.com/sagemath/sage/files/ticket3894/3894.patch.gz)**

Looks good.  I've made this change in the ReST version of the tutorial too.



---

archive/issue_events_038559.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-16T03:50:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38559"
}
```



---

archive/issue_events_038560.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-16T03:50:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38560"
}
```



---

archive/issue_comments_022607.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nMerged in Sage 3.1.2.rc5",
    "created_at": "2008-09-16T03:50:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3894#issuecomment-22607",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">Comment 2</div>

Merged in Sage 3.1.2.rc5



---

archive/issue_events_038561.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-16T03:50:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "milestone_number": null,
    "milestone_title": "sage-3.1.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38561"
}
```



---

archive/issue_events_038562.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-16T03:50:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3894",
    "milestone_number": null,
    "milestone_title": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3894#event-38562"
}
```
