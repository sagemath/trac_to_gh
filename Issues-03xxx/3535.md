# Issue 3535: notebook -- A "getting started" worksheet for new users

archive/issues_003535.json:
```json
{
    "body": "\n\n**Assignee:** boothby\n\nIssue created by migration from https://trac.sagemath.org/ticket/3535\n\n",
    "closed_at": "2009-11-19T21:48:45Z",
    "created_at": "2008-06-30T00:53:08Z",
    "labels": [
        "component: notebook",
        "enhancement",
        "wontfix"
    ],
    "title": "notebook -- A \"getting started\" worksheet for new users",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3535",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```


**Assignee:** boothby

Issue created by migration from https://trac.sagemath.org/ticket/3535





---

archive/attachments_003532.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage-3535.patch",
    "asset_url": "tarball://root/attachments/ticket3535/sage-3535.patch",
    "created_at": "2008-09-08T15:57:41Z",
    "issue": "https://github.com/sagemath/sage/issues/3535",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3535/sage-3535.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```



---

archive/issue_comments_020077.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [sage-3535.patch](https://github.com/sagemath/sage/files/ticket3535/sage-3535.patch) by TimothyClemans created at 2008-09-08 15:57:41\n\nThis is no longer needed thanks to the welcome message on a user's worksheet listing page.",
    "created_at": "2008-09-08T15:57:41Z",
    "issue": "https://github.com/sagemath/sage/issues/3535",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3535#issuecomment-20077",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

<a id='comment:1'></a>
Attachment [sage-3535.patch](https://github.com/sagemath/sage/files/ticket3535/sage-3535.patch) by TimothyClemans created at 2008-09-08 15:57:41

This is no longer needed thanks to the welcome message on a user's worksheet listing page.



---

archive/issue_comments_020078.json:
```json
{
    "body": "<a id='comment:2'></a>\nI think this is still needed, since pressing S-Enter may not be obvious to users with no experience with notebook-style interfaces.",
    "created_at": "2009-11-19T19:35:51Z",
    "issue": "https://github.com/sagemath/sage/issues/3535",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3535#issuecomment-20078",
    "user": "https://github.com/TimDumol"
}
```

<a id='comment:2'></a>
I think this is still needed, since pressing S-Enter may not be obvious to users with no experience with notebook-style interfaces.



---

archive/issue_events_019517.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-19T21:48:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3535",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3535#event-19517"
}
```



---

archive/issue_events_019518.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-19T21:48:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3535",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3535#event-19518"
}
```



---

archive/issue_comments_020079.json:
```json
{
    "body": "<a id='comment:3'></a>\n> I think this is still needed, since pressing S-Enter may not be \n> obvious to users with no experience with notebook-style interfaces. \n\n\nYes, but I don't think a new worksheet with tutorial stuff in it is the solution.   This has been discussed a lot before, and we decided to put an \"evaluate\" button in for such users, which helps a lot.  Once people get familiar with sage, they click \"help\" in the upper right, and then they quickly learn about shift-enter.",
    "created_at": "2009-11-19T21:48:45Z",
    "issue": "https://github.com/sagemath/sage/issues/3535",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3535#issuecomment-20079",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
> I think this is still needed, since pressing S-Enter may not be 
> obvious to users with no experience with notebook-style interfaces. 


Yes, but I don't think a new worksheet with tutorial stuff in it is the solution.   This has been discussed a lot before, and we decided to put an "evaluate" button in for such users, which helps a lot.  Once people get familiar with sage, they click "help" in the upper right, and then they quickly learn about shift-enter.



---

archive/issue_events_019519.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-12-08T23:45:57Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3535",
    "milestone": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3535#event-19519"
}
```
