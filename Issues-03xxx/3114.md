# Issue 3114: blacklist  gcc version 4.0.0 on OSX

archive/issues_003114.json:
```json
{
    "body": "Any gcc 4.0.0 provided by Apple is buggier than a Florida swamp. Refuse to build Sage with it at start up!\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3114\n\n",
    "closed_at": "2010-01-17T06:32:54Z",
    "created_at": "2008-05-06T22:06:10Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.3.1",
    "title": "blacklist  gcc version 4.0.0 on OSX",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3114",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Any gcc 4.0.0 provided by Apple is buggier than a Florida swamp. Refuse to build Sage with it at start up!

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3114





---

archive/issue_events_007032.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T06:32:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3114",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3114#event-7032"
}
```



---

archive/issue_comments_023267.json:
```json
{
    "body": "<a id='comment:2'></a>`prereq` currently checks for gcc >= 4.0.1, so this has already been taken care of.",
    "created_at": "2010-01-17T06:32:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3114#issuecomment-23267",
    "user": "https://github.com/wjp"
}
```

<a id='comment:2'></a>`prereq` currently checks for gcc >= 4.0.1, so this has already been taken care of.



---

archive/issue_comments_023268.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2010-01-17T06:32:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3114#issuecomment-23268",
    "user": "https://github.com/wjp"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_023269.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-01-17T06:32:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3114#issuecomment-23269",
    "user": "https://github.com/wjp"
}
```

Resolution: fixed
