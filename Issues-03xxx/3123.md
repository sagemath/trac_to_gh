# Issue 3123: blacklist "gcc version 4.1.0 (SUSE Linux)"

archive/issues_003123.json:
```json
{
    "body": "gcc version 4.1.0 (SUSE Linux) from OpenSuSE 10.1 is know broken and will fail to file Sage with\n\n```\nsage/modules/real_double_vector.c: In function \u2018__pyx_pf_4sage_7modules_18real_double_vector_28RealDoubleVectorSpaceElement_\n__init__\u2019:\nsage/modules/real_double_vector.c:2012: internal compiler error: in merge_alias_info, at tree-ssa-copy.c:235\nPlease submit a full bug report,\nwith preprocessed source if appropriate.\nSee <URL:http://www.suse.de/feedback> for instructions.\nerror: command 'gcc' failed with exit status 1\n```\nBlacklist it.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/3123\n\n",
    "closed_at": "2013-05-21T07:23:36Z",
    "created_at": "2008-05-07T14:27:31Z",
    "labels": [
        "component: build",
        "bug",
        "invalid"
    ],
    "title": "blacklist \"gcc version 4.1.0 (SUSE Linux)\"",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3123",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
gcc version 4.1.0 (SUSE Linux) from OpenSuSE 10.1 is know broken and will fail to file Sage with

```
sage/modules/real_double_vector.c: In function ‘__pyx_pf_4sage_7modules_18real_double_vector_28RealDoubleVectorSpaceElement_
__init__’:
sage/modules/real_double_vector.c:2012: internal compiler error: in merge_alias_info, at tree-ssa-copy.c:235
Please submit a full bug report,
with preprocessed source if appropriate.
See <URL:http://www.suse.de/feedback> for instructions.
error: command 'gcc' failed with exit status 1
```
Blacklist it.

Cheers,

Michael

**Assignee:** mabshoff

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/3123





---

archive/issue_events_016950.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T23:19:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "label": "blocker",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3123#event-16950"
}
```



---

archive/issue_events_016951.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-19T13:02:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3123#event-16951"
}
```



---

archive/issue_comments_017649.json:
```json
{
    "body": "<a id='comment:3'></a>\nFixed by the GCC spkg.",
    "created_at": "2013-05-19T13:02:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3123#issuecomment-17649",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>
Fixed by the GCC spkg.



---

archive/issue_comments_017650.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2013-05-19T13:02:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3123#issuecomment-17650",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_comments_017651.json:
```json
{
    "body": "**Upstream:** N/A",
    "created_at": "2013-05-19T13:02:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3123#issuecomment-17651",
    "user": "https://github.com/jdemeyer"
}
```

**Upstream:** N/A



---

archive/issue_events_016952.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-19T13:02:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3123#event-16952"
}
```



---

archive/issue_events_016953.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-19T13:02:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3123#event-16953"
}
```



---

archive/issue_events_016954.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-19T13:02:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "milestone": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3123#event-16954"
}
```



---

archive/issue_events_016955.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-19T13:02:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3123#event-16955"
}
```



---

archive/issue_events_016956.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-21T07:23:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3123#event-16956"
}
```



---

archive/issue_events_016957.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-21T07:23:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3123",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3123#event-16957"
}
```
