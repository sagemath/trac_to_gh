# Issue 3867: [with patch, positive review] sage-3.1.alpha2: graph.py numerical noise

archive/issues_003867.json:
```json
{
    "body": "\n```\n         [6.9 s]\nsage -t -long devel/sage/sage/graphs/graph.py               **********************************************************************\nFile \"/home/was/build/sage-3.1.alpha2/tmp/graph.py\", line 5233:\n    sage: G.get_pos()\nExpected: \n    {0: [-0.39..., 0.06...],\n     1: [-0.26..., 0.94...],\n     2: [-0.29..., 0.43...],\n     3: [-0.40..., -0.70...],\n     4: [-0.88..., -0.46...],\n     5: [0.75..., -0.16...],\n     6: [0.32..., 0.28...],\n     7: [0.67..., 0.52...],\n     8: [0.44..., -0.72...],\n     9: [0.05..., -0.19...]}\nGot:\n    {0: [-0.39224507078505738, 0.064423438834940699], 1: [-0.2616497839815306, 0.94923940803846718], 2: [-0.29753651819885235, 0.43107759793361461], 3: [-0.4079690141981453, -0.70787463618828417], 4: [-0.88764715713391851, -0.46052418441605503], 5: [0.75138278139293446, -0.17000101533997652], 6: [0.32390924920002728, 0.28668268344881481], 7: [0.67388121773389487, 0.52838252201932256], 8: [0.44564785869604684, -0.72172615352578218], 9: [0.052226437274600954, -0.19967966080505992]}\n**********************************************************************\n1 items had failures:\n   1 of  48 in __main__.example_124\n***Test Failed*** 1 failures.\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3867\n\n",
    "closed_at": "2008-08-15T08:26:10Z",
    "created_at": "2008-08-15T08:02:01Z",
    "labels": [
        "component: doctest coverage",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1",
    "title": "[with patch, positive review] sage-3.1.alpha2: graph.py numerical noise",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3867",
    "user": "https://github.com/williamstein"
}
```

```
         [6.9 s]
sage -t -long devel/sage/sage/graphs/graph.py               **********************************************************************
File "/home/was/build/sage-3.1.alpha2/tmp/graph.py", line 5233:
    sage: G.get_pos()
Expected: 
    {0: [-0.39..., 0.06...],
     1: [-0.26..., 0.94...],
     2: [-0.29..., 0.43...],
     3: [-0.40..., -0.70...],
     4: [-0.88..., -0.46...],
     5: [0.75..., -0.16...],
     6: [0.32..., 0.28...],
     7: [0.67..., 0.52...],
     8: [0.44..., -0.72...],
     9: [0.05..., -0.19...]}
Got:
    {0: [-0.39224507078505738, 0.064423438834940699], 1: [-0.2616497839815306, 0.94923940803846718], 2: [-0.29753651819885235, 0.43107759793361461], 3: [-0.4079690141981453, -0.70787463618828417], 4: [-0.88764715713391851, -0.46052418441605503], 5: [0.75138278139293446, -0.17000101533997652], 6: [0.32390924920002728, 0.28668268344881481], 7: [0.67388121773389487, 0.52838252201932256], 8: [0.44564785869604684, -0.72172615352578218], 9: [0.052226437274600954, -0.19967966080505992]}
**********************************************************************
1 items had failures:
   1 of  48 in __main__.example_124
***Test Failed*** 1 failures.
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3867





---

archive/issue_comments_029052.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [sage-3867.patch](tarball://root/attachments/some-uuid/ticket3867/sage-3867.patch) by @williamstein created at 2008-08-15 08:11:13",
    "created_at": "2008-08-15T08:11:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3867#issuecomment-29052",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [sage-3867.patch](tarball://root/attachments/some-uuid/ticket3867/sage-3867.patch) by @williamstein created at 2008-08-15 08:11:13



---

archive/issue_events_014363.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3867",
    "rename": {
        "from": "sage-3.1.alpha2: graph.py numerical noise",
        "to": "[with patch; needs review] sage-3.1.alpha2: graph.py numerical noise"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3867#event-14363"
}
```



---

archive/issue_events_014364.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3867",
    "rename": {
        "from": "[with patch; needs review] sage-3.1.alpha2: graph.py numerical noise",
        "to": "[with patch, positive review] sage-3.1.alpha2: graph.py numerical noise"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3867#event-14364"
}
```



---

archive/issue_comments_029053.json:
```json
{
    "body": "<a id='comment:2'></a>\nPositive review. :)\n\nCheers,\n\nMichael",
    "created_at": "2008-08-15T08:22:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3867#issuecomment-29053",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Positive review. :)

Cheers,

Michael



---

archive/issue_comments_029054.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-15T08:26:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3867#issuecomment-29054",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_014365.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-15T08:26:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3867",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3867#event-14365"
}
```



---

archive/issue_comments_029055.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.rc0",
    "created_at": "2008-08-15T08:26:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3867#issuecomment-29055",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.rc0
