# Issue 3254: improvements and doctests for CachedFunction

archive/issues_003254.json:
```json
{
    "body": "CachedFunction now works with class methods.\n\n**Assignee:** cwitty\n\nIssue created by migration from https://trac.sagemath.org/ticket/3254\n\n",
    "closed_at": "2008-06-04T20:54:04Z",
    "created_at": "2008-05-19T00:49:38Z",
    "labels": [
        "component: misc",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.3",
    "title": "improvements and doctests for CachedFunction",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3254",
    "user": "https://github.com/mwhansen"
}
```
CachedFunction now works with class methods.

**Assignee:** cwitty

Issue created by migration from https://trac.sagemath.org/ticket/3254





---

archive/attachments_003176.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "3254.patch",
    "asset_url": "tarball://root/attachments/ticket3254/3254.patch",
    "created_at": "2008-06-03T14:33:35Z",
    "issue": "https://github.com/sagemath/sage/issues/3254",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3254/3254.patch",
    "user": "https://github.com/malb"
}
```



---

archive/issue_comments_017989.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [3254.patch](https://github.com/sagemath/sage/files/ticket3254/3254.patch) by @malb created at 2008-06-03 14:33:35\n\nVery cool, I want this. Is `CachedFunction` the right name though? Shouldn't it be `cached_function`? I think there is a different style convention for the persistent and the cached functions which is a bummer. This is not a blocker for merging this code though, since it wasn't introduced by this patch.",
    "created_at": "2008-06-03T14:33:35Z",
    "issue": "https://github.com/sagemath/sage/issues/3254",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3254#issuecomment-17989",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'></a>
Attachment [3254.patch](https://github.com/sagemath/sage/files/ticket3254/3254.patch) by @malb created at 2008-06-03 14:33:35

Very cool, I want this. Is `CachedFunction` the right name though? Shouldn't it be `cached_function`? I think there is a different style convention for the persistent and the cached functions which is a bummer. This is not a blocker for merging this code though, since it wasn't introduced by this patch.



---

archive/issue_events_017390.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-06-03T14:33:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3254",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3254#event-17390"
}
```



---

archive/issue_comments_017990.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.0.3.alpha1",
    "created_at": "2008-06-04T20:54:04Z",
    "issue": "https://github.com/sagemath/sage/issues/3254",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3254#issuecomment-17990",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.0.3.alpha1



---

archive/issue_events_017391.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-04T20:54:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3254",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3254#event-17391"
}
```



---

archive/issue_events_017392.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-04T20:54:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3254",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3254#event-17392"
}
```
