# Issue 3634: minpoly still slow for elements of finte fields

archive/issues_003634.json:
```json
{
    "assignees": [],
    "body": "The improvement at #3620 is significant, but NTL does have minimal polynomial computations, though provided in http://www.shoup.net/ntl/doc/GF2X.txt rather than http://www.shoup.net/ntl/doc/GF2E.txt . We should probably use the proof flag to decide the algorithm. Trace could be wrapped as well.\n\nAlso, the computation of matrix() is using the completely generic code, which has got to be sub-optimal for manipulating elements of GF(2).\n\n\nComponent: **algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3634_\n\n",
    "closed_at": "2008-07-11T18:09:48Z",
    "created_at": "2008-07-10T17:11:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "minpoly still slow for elements of finte fields",
    "type": "issue",
    "updated_at": "2008-07-11T18:09:48Z",
    "url": "https://github.com/sagemath/sage/issues/3634",
    "user": "https://github.com/robertwb"
}
```
The improvement at #3620 is significant, but NTL does have minimal polynomial computations, though provided in http://www.shoup.net/ntl/doc/GF2X.txt rather than http://www.shoup.net/ntl/doc/GF2E.txt . We should probably use the proof flag to decide the algorithm. Trace could be wrapped as well.

Also, the computation of matrix() is using the completely generic code, which has got to be sub-optimal for manipulating elements of GF(2).


Component: **algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/3634_





---

archive/issue_events_039980.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-07-10T17:11:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "milestone_number": null,
    "milestone_title": "sage-3.0.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39980"
}
```



---

archive/issue_events_039981.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-07-10T17:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39981"
}
```



---

archive/issue_events_039982.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-07-10T17:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39982"
}
```



---

archive/issue_events_039983.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-07-10T17:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39983"
}
```



---

archive/issue_events_039984.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-07-10T18:07:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39984"
}
```



---

archive/issue_comments_019856.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[3634-gf2e-minpoly.patch.gz](https://github.com/sagemath/sage/files/ticket3634/3634-gf2e-minpoly.patch.gz)**\n\n\n```\nsage: sage: k.<a> = GF(2^500)\n\nsage: sage: time g = k.random_element()\nCPU times: user 0.07 s, sys: 0.00 s, total: 0.07 s\nWall time: 0.07 s\n\nsage: time f = g.minpoly()\nCPU times: user 0.00 s, sys: 0.00 s, total: 0.01 s\nWall time: 0.00 s\n\nsage: f(g)\n 0\nsage: timeit(\"g.minpoly()\")\n125 loops, best of 3: 4.03 ms per loop\n```",
    "created_at": "2008-07-10T18:07:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3634#issuecomment-19856",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[3634-gf2e-minpoly.patch.gz](https://github.com/sagemath/sage/files/ticket3634/3634-gf2e-minpoly.patch.gz)**


```
sage: sage: k.<a> = GF(2^500)

sage: sage: time g = k.random_element()
CPU times: user 0.07 s, sys: 0.00 s, total: 0.07 s
Wall time: 0.07 s

sage: time f = g.minpoly()
CPU times: user 0.00 s, sys: 0.00 s, total: 0.01 s
Wall time: 0.00 s

sage: f(g)
 0
sage: timeit("g.minpoly()")
125 loops, best of 3: 4.03 ms per loop
```



---

archive/issue_comments_019857.json:
```json
{
    "body": "Attachment: **[sage-3634-referee.patch.gz](https://github.com/sagemath/sage/files/ticket3634/sage-3634-referee.patch.gz)**\n\nadd fast charpoly",
    "created_at": "2008-07-10T23:21:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3634#issuecomment-19857",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[sage-3634-referee.patch.gz](https://github.com/sagemath/sage/files/ticket3634/sage-3634-referee.patch.gz)**

add fast charpoly



---

archive/issue_comments_019858.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nGreat work Robert!\n\nI added a patch that adds a fast charpoly method by the way.\n\nApply both of them.",
    "created_at": "2008-07-10T23:22:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3634#issuecomment-19858",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

Great work Robert!

I added a patch that adds a fast charpoly method by the way.

Apply both of them.



---

archive/issue_events_039985.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-10T23:22:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39985"
}
```



---

archive/issue_events_039986.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-10T23:22:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39986"
}
```



---

archive/issue_comments_019859.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nCharpoly method is good too. Apply both patches.",
    "created_at": "2008-07-11T17:43:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3634#issuecomment-19859",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:3" align="right">comment:3</div>

Charpoly method is good too. Apply both patches.



---

archive/issue_events_039987.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-11T17:46:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "milestone_number": null,
    "milestone_title": "sage-3.0.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39987"
}
```



---

archive/issue_events_039988.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-11T17:46:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "milestone_number": null,
    "milestone_title": "sage-3.0.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39988"
}
```



---

archive/issue_comments_019860.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis is one of the few patches that will be merged in 3.0.5 :)\n\nCheers,\n\nMichael",
    "created_at": "2008-07-11T17:46:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3634#issuecomment-19860",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

This is one of the few patches that will be merged in 3.0.5 :)

Cheers,

Michael



---

archive/issue_events_039989.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-11T18:09:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39989"
}
```



---

archive/issue_events_039990.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-11T18:09:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3634",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3634#event-39990"
}
```
