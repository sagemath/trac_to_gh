# Issue 3625: [with patch; positive review] fix latex2html'ing of the sage docs, which is broken

archive/issues_003625.json:
```json
{
    "body": "For some reason running latex2html to make the sage docs doesn't work -- the images rarely get built.  This must be fixed, because it causes massive frustration when I try to do releases, etc.\n\nI suspect mistakes in paths or something else.  I don't know. \n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3625\n\n",
    "closed_at": "2008-07-10T04:34:12Z",
    "created_at": "2008-07-09T18:28:37Z",
    "labels": [
        "component: documentation",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.4",
    "title": "[with patch; positive review] fix latex2html'ing of the sage docs, which is broken",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3625",
    "user": "https://github.com/williamstein"
}
```
For some reason running latex2html to make the sage docs doesn't work -- the images rarely get built.  This must be fixed, because it causes massive frustration when I try to do releases, etc.

I suspect mistakes in paths or something else.  I don't know. 

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3625





---

archive/issue_events_013300.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-09T18:46:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3625",
    "milestone": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3625#event-13300"
}
```



---

archive/issue_events_013301.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3625",
    "rename": {
        "from": "fix latex2html'ing of the sage docs, which is broken",
        "to": "[with patch; needs review] fix latex2html'ing of the sage docs, which is broken"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3625#event-13301"
}
```



---

archive/issue_comments_026993.json:
```json
{
    "body": "<a id='comment:2'></a>\nhttp://sage.math.washington.edu/home/was/patches/doc-3625.patch",
    "created_at": "2008-07-10T03:03:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3625#issuecomment-26993",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
http://sage.math.washington.edu/home/was/patches/doc-3625.patch



---

archive/issue_events_013302.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3625",
    "rename": {
        "from": "[with patch; needs review] fix latex2html'ing of the sage docs, which is broken",
        "to": "[with patch; positive review] fix latex2html'ing of the sage docs, which is broken"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3625#event-13302"
}
```



---

archive/issue_comments_026994.json:
```json
{
    "body": "<a id='comment:3'></a>\nPositive review. There might be some spacing issues with the pdf, but we can sort those issues out later since an actually building documentation is much more important than some small spacing issues :)\n\nCheers,\n\nMichael",
    "created_at": "2008-07-10T04:33:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3625#issuecomment-26994",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Positive review. There might be some spacing issues with the pdf, but we can sort those issues out later since an actually building documentation is much more important than some small spacing issues :)

Cheers,

Michael



---

archive/issue_events_013303.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-10T04:34:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3625",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3625#event-13303"
}
```



---

archive/issue_comments_026995.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.4.rc3",
    "created_at": "2008-07-10T04:34:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3625#issuecomment-26995",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.4.rc3



---

archive/issue_comments_026996.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-07-10T04:34:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3625#issuecomment-26996",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
