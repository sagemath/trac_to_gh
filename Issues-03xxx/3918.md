# Issue 3918: notebook -- MAJOR BUG involving uploading file from URL

archive/issues_003918.json:
```json
{
    "assignees": [],
    "body": "1. Go to Data --> Upload File\n2. Select a URL that takes \"forever\" to download.\n3. Now the ENTIRE SERVER hangs \"forever\".  \n\nThis is clearly very much not good. \n\nAssignee: **@mwhansen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3918_\n\n",
    "closed_at": "2008-09-09T00:40:30Z",
    "created_at": "2008-08-20T23:17:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook -- MAJOR BUG involving uploading file from URL",
    "type": "issue",
    "updated_at": "2008-09-09T00:40:30Z",
    "url": "https://github.com/sagemath/sage/issues/3918",
    "user": "https://github.com/williamstein"
}
```
1. Go to Data --> Upload File
2. Select a URL that takes "forever" to download.
3. Now the ENTIRE SERVER hangs "forever".  

This is clearly very much not good. 

Assignee: **@mwhansen**

_Issue created by migration from https://trac.sagemath.org/ticket/3918_





---

archive/issue_events_037926.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-20T23:17:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "milestone_number": null,
    "milestone_title": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-37926"
}
```



---

archive/issue_events_037927.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-20T23:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-37927"
}
```



---

archive/issue_events_037928.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-20T23:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-37928"
}
```



---

archive/issue_events_037929.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-20T23:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-37929"
}
```



---

archive/issue_events_037930.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-09-08T10:25:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-37930"
}
```



---

archive/issue_comments_022765.json:
```json
{
    "body": "Changed assignee from **boothby** to **@mwhansen**",
    "created_at": "2008-09-08T10:25:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3918#issuecomment-22765",
    "user": "https://github.com/mwhansen"
}
```

Changed assignee from **boothby** to **@mwhansen**



---

archive/issue_events_037931.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-09-08T10:25:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-37931"
}
```



---

archive/issue_comments_022766.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_3918.patch.gz](https://github.com/sagemath/sage/files/ticket3918/trac_3918.patch.gz)**",
    "created_at": "2008-09-08T10:25:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3918#issuecomment-22766",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_3918.patch.gz](https://github.com/sagemath/sage/files/ticket3918/trac_3918.patch.gz)**



---

archive/issue_comments_022767.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nFor review, I first reproduced the bug using sage-3.1.1 and then tried to reproduce it with the patch applied. The bug seems to be fixed with the patch applied. I also made sure that uploading a file worked in general.",
    "created_at": "2008-09-08T11:12:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3918#issuecomment-22767",
    "user": "https://github.com/sagetrac-TimothyClemans"
}
```

<a id='comment:2'>Comment 2:</a>
For review, I first reproduced the bug using sage-3.1.1 and then tried to reproduce it with the patch applied. The bug seems to be fixed with the patch applied. I also made sure that uploading a file worked in general.



---

archive/issue_events_037932.json:
```json
{
    "actor": "https://github.com/sagetrac-TimothyClemans",
    "created_at": "2008-09-08T11:12:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-37932"
}
```



---

archive/issue_comments_022768.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMerged in Sage 3.1.2.rc1",
    "created_at": "2008-09-09T00:40:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3918#issuecomment-22768",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Merged in Sage 3.1.2.rc1



---

archive/issue_events_037933.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-09T00:40:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-37933"
}
```



---

archive/issue_events_037934.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-09T00:40:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-37934"
}
```
