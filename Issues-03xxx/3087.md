# Issue 3087: [with patch; positive review] Stupid mistakes in Debian palp copyright files

archive/issues_003087.json:
```json
{
    "body": "My new copyright file for palp doesn't quite have the right format.  Attached is a patch to fix this.\n\nAssignee: @timabbott\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3087\n\n",
    "closed_at": "2008-05-03T14:13:44Z",
    "created_at": "2008-05-03T04:41:21Z",
    "labels": [
        "component: debian-package",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.1",
    "title": "[with patch; positive review] Stupid mistakes in Debian palp copyright files",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3087",
    "user": "https://github.com/timabbott"
}
```
My new copyright file for palp doesn't quite have the right format.  Attached is a patch to fix this.

Assignee: @timabbott

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3087





---

archive/issue_comments_023004.json:
```json
{
    "body": "Attachment [palp-copyright-fix.patch](tarball://root/attachments/some-uuid/ticket3087/palp-copyright-fix.patch) by @timabbott created at 2008-05-03 04:41:31",
    "created_at": "2008-05-03T04:41:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3087",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3087#issuecomment-23004",
    "user": "https://github.com/timabbott"
}
```

Attachment [palp-copyright-fix.patch](tarball://root/attachments/some-uuid/ticket3087/palp-copyright-fix.patch) by @timabbott created at 2008-05-03 04:41:31



---

archive/issue_comments_023005.json:
```json
{
    "body": "<a id='comment:1'></a>Patch looks good to me. Slipped into palp-1.1.p1.spkg without bumping the release number.\n\nCheers,\n\nMichael",
    "created_at": "2008-05-03T14:13:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3087",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3087#issuecomment-23005",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>Patch looks good to me. Slipped into palp-1.1.p1.spkg without bumping the release number.

Cheers,

Michael



---

archive/issue_events_006970.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-03T14:13:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3087",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3087#event-6970"
}
```



---

archive/issue_comments_023006.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-03T14:13:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3087",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3087#issuecomment-23006",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_023007.json:
```json
{
    "body": "<a id='comment:2'></a>Merged in Sage 3.0.1.final",
    "created_at": "2008-05-03T14:13:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3087",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3087#issuecomment-23007",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>Merged in Sage 3.0.1.final
