# Issue 3341: [with patch, positive review] fix minor issue with creating skew partitions by dividing partitions

archive/issues_003341.json:
```json
{
    "body": "\n```\nsage: Partition([2,1])/Partition([1])\n/home/was/s/local/lib/python2.5/site-packages/sage/combinat/partition.py in __div__(self, p)\n    325             \n    326         \"\"\"\n--> 327         if not self.dominates(Partition_class(p)):\n    328             raise ValueError, \"the partition must dominate p\"\n    329 \n\n/home/was/s/local/lib/python2.5/site-packages/sage/combinat/combinat.py in __init__(self, l)\n    546         \"\"\"\n    547         if not isinstance(l, list):\n--> 548             raise ValueError, 'l must be a list'\n    549         self._list = l\n    550         self._hash = None\n\nValueError: l must be a list\n```\n\n\n\nAssignee: @mwhansen\n\nCC:  sage-combinat\n\nKeywords: editor_mhansen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3341\n\n",
    "closed_at": "2008-06-23T07:37:25Z",
    "created_at": "2008-05-31T06:56:01Z",
    "labels": [
        "component: combinatorics",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.4",
    "title": "[with patch, positive review] fix minor issue with creating skew partitions by dividing partitions",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3341",
    "user": "https://github.com/mwhansen"
}
```

```
sage: Partition([2,1])/Partition([1])
/home/was/s/local/lib/python2.5/site-packages/sage/combinat/partition.py in __div__(self, p)
    325             
    326         """
--> 327         if not self.dominates(Partition_class(p)):
    328             raise ValueError, "the partition must dominate p"
    329 

/home/was/s/local/lib/python2.5/site-packages/sage/combinat/combinat.py in __init__(self, l)
    546         """
    547         if not isinstance(l, list):
--> 548             raise ValueError, 'l must be a list'
    549         self._list = l
    550         self._hash = None

ValueError: l must be a list
```



Assignee: @mwhansen

CC:  sage-combinat

Keywords: editor_mhansen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3341





---

archive/issue_comments_024362.json:
```json
{
    "body": "Attachment [3341.patch](tarball://root/attachments/some-uuid/ticket3341/3341.patch) by @mwhansen created at 2008-05-31 06:59:25",
    "created_at": "2008-05-31T06:59:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3341#issuecomment-24362",
    "user": "https://github.com/mwhansen"
}
```

Attachment [3341.patch](tarball://root/attachments/some-uuid/ticket3341/3341.patch) by @mwhansen created at 2008-05-31 06:59:25



---

archive/issue_events_011922.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-05-31T06:59:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "milestone": "sage-3.0.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3341#event-11922"
}
```



---

archive/issue_comments_024363.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-05-31T06:59:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3341#issuecomment-24363",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_events_011923.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "rename": {
        "from": "fix minor issue with creating skew partitions by dividng partitions",
        "to": "[with patch, needs review] fix minor issue with creating skew partitions by dividng partitions"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3341#event-11923"
}
```



---

archive/issue_comments_024364.json:
```json
{
    "body": "Changing keywords from \"\" to \"editor_mhansen\".",
    "created_at": "2008-06-15T22:01:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3341#issuecomment-24364",
    "user": "https://github.com/craigcitro"
}
```

Changing keywords from "" to "editor_mhansen".



---

archive/issue_events_011924.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "rename": {
        "from": "[with patch, needs review] fix minor issue with creating skew partitions by dividng partitions",
        "to": "[with patch, needs review] fix minor issue with creating skew partitions by dividing partitions"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3341#event-11924"
}
```



---

archive/issue_events_011925.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "rename": {
        "from": "[with patch, needs review] fix minor issue with creating skew partitions by dividing partitions",
        "to": "[with patch, positive review] fix minor issue with creating skew partitions by dividing partitions"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3341#event-11925"
}
```



---

archive/issue_comments_024365.json:
```json
{
    "body": "<a id='comment:4'></a>\ngood work.",
    "created_at": "2008-06-19T20:44:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3341#issuecomment-24365",
    "user": "https://github.com/malb"
}
```

<a id='comment:4'></a>
good work.



---

archive/issue_events_011926.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-23T07:37:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3341#event-11926"
}
```



---

archive/issue_comments_024366.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-06-23T07:37:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3341#issuecomment-24366",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_024367.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 3.0.4.alpha0",
    "created_at": "2008-06-23T07:37:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3341#issuecomment-24367",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 3.0.4.alpha0



---

archive/issue_events_011927.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-23T07:37:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "milestone": "sage-3.0.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3341#event-11927"
}
```



---

archive/issue_events_011928.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-23T07:37:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3341",
    "milestone": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3341#event-11928"
}
```
