# Issue 3336: [with patch, positive review] DyckWords(n) should use an iterator

archive/issues_003336.json:
```json
{
    "body": "Currently, DyckWords(n) creates a list, which uses a lot of memory and is slow. See [this sage-devel thread](http://groups.google.com/group/sage-devel/browse_thread/thread/8b739bb399f2e3d4).\n\nAssignee: @dandrake\n\nCC:  sage-combinat\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3336\n\n",
    "closed_at": "2008-05-31T05:56:41Z",
    "created_at": "2008-05-30T00:44:34Z",
    "labels": [
        "component: combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.3",
    "title": "[with patch, positive review] DyckWords(n) should use an iterator",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3336",
    "user": "https://github.com/dandrake"
}
```
Currently, DyckWords(n) creates a list, which uses a lot of memory and is slow. See [this sage-devel thread](http://groups.google.com/group/sage-devel/browse_thread/thread/8b739bb399f2e3d4).

Assignee: @dandrake

CC:  sage-combinat

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3336





---

archive/issue_events_011896.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3336",
    "rename": {
        "from": "DyckWords(n) should use an iterator",
        "to": "[with patch, needs review] DyckWords(n) should use an iterator"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3336#event-11896"
}
```



---

archive/issue_comments_024280.json:
```json
{
    "body": "<a id='comment:1'></a>\nPatch attached.",
    "created_at": "2008-05-30T06:42:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3336",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3336#issuecomment-24280",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:1'></a>
Patch attached.



---

archive/issue_events_011897.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3336",
    "rename": {
        "from": "[with patch, needs review] DyckWords(n) should use an iterator",
        "to": "[with patch, positive review] DyckWords(n) should use an iterator"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3336#event-11897"
}
```



---

archive/issue_comments_024281.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [dyckword-3336.patch](tarball://root/attachments/some-uuid/ticket3336/dyckword-3336.patch) by @mwhansen created at 2008-05-31 03:53:44\n\nLooks good.  Thanks for this Dan!  I also added it to 2144.",
    "created_at": "2008-05-31T03:53:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3336",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3336#issuecomment-24281",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Attachment [dyckword-3336.patch](tarball://root/attachments/some-uuid/ticket3336/dyckword-3336.patch) by @mwhansen created at 2008-05-31 03:53:44

Looks good.  Thanks for this Dan!  I also added it to 2144.



---

archive/issue_events_011898.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-31T05:56:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3336",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3336#event-11898"
}
```



---

archive/issue_comments_024282.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-31T05:56:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3336",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3336#issuecomment-24282",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_024283.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.3.alpha1",
    "created_at": "2008-05-31T05:56:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3336",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3336#issuecomment-24283",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.3.alpha1
