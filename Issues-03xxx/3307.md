# Issue 3307: [with patch; positive review] Move genus2reduction to /usr/lib for Debian package

archive/issues_003307.json:
```json
{
    "body": "I've attached a patch that moves genus2reduction to /usr/lib in my Debian package because it's not a program you run directly and doesn't have a man page.  The patch also fixes all the other minor issues with the package I'm aware of.\n\nAssignee: @timabbott\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3307\n\n",
    "closed_at": "2008-05-28T06:41:52Z",
    "created_at": "2008-05-26T04:24:51Z",
    "labels": [
        "component: debian-package",
        "blocker"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.3",
    "title": "[with patch; positive review] Move genus2reduction to /usr/lib for Debian package",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3307",
    "user": "https://github.com/timabbott"
}
```
I've attached a patch that moves genus2reduction to /usr/lib in my Debian package because it's not a program you run directly and doesn't have a man page.  The patch also fixes all the other minor issues with the package I'm aware of.

Assignee: @timabbott

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3307





---

archive/issue_comments_024004.json:
```json
{
    "body": "Attachment [genus2reduction-lib.patch](tarball://root/attachments/some-uuid/ticket3307/genus2reduction-lib.patch) by @timabbott created at 2008-05-26 04:25:03",
    "created_at": "2008-05-26T04:25:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3307#issuecomment-24004",
    "user": "https://github.com/timabbott"
}
```

Attachment [genus2reduction-lib.patch](tarball://root/attachments/some-uuid/ticket3307/genus2reduction-lib.patch) by @timabbott created at 2008-05-26 04:25:03



---

archive/issue_comments_024005.json:
```json
{
    "body": "<a id='comment:1'></a>\nPatch looks good to me. I have slipped it into\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/genus2reduction-0.3.p3.spkg\n\nwithout incrementing the patch level to avoid rebuilds.\n\nCheers,\n\nMichael",
    "created_at": "2008-05-28T06:41:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3307#issuecomment-24005",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Patch looks good to me. I have slipped it into

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/genus2reduction-0.3.p3.spkg

without incrementing the patch level to avoid rebuilds.

Cheers,

Michael



---

archive/issue_events_011803.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3307",
    "rename": {
        "from": "[with patch; needs review] Move genus2reduction to /usr/lib for Debian package",
        "to": "[with patch; positive review] Move genus2reduction to /usr/lib for Debian package"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3307#event-11803"
}
```



---

archive/issue_comments_024006.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-28T06:41:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3307#issuecomment-24006",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_011804.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-28T06:41:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3307",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3307#event-11804"
}
```



---

archive/issue_comments_024007.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.0.3.alpha0",
    "created_at": "2008-05-28T06:41:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3307#issuecomment-24007",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.0.3.alpha0
