# Issue 3307: Move genus2reduction to /usr/lib for Debian package

archive/issues_003307.json:
```json
{
    "assignees": [
        "https://github.com/timabbott"
    ],
    "body": "I've attached a patch that moves genus2reduction to /usr/lib in my Debian package because it's not a program you run directly and doesn't have a man page.  The patch also fixes all the other minor issues with the package I'm aware of.\n\nComponent: **debian-package**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3307_\n\n",
    "closed_at": "2008-05-28T06:41:52Z",
    "created_at": "2008-05-26T04:24:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Move genus2reduction to /usr/lib for Debian package",
    "type": "issue",
    "updated_at": "2008-05-28T06:41:52Z",
    "url": "https://github.com/sagemath/sage/issues/3307",
    "user": "https://github.com/timabbott"
}
```
I've attached a patch that moves genus2reduction to /usr/lib in my Debian package because it's not a program you run directly and doesn't have a man page.  The patch also fixes all the other minor issues with the package I'm aware of.

Component: **debian-package**

_Issue created by migration from https://trac.sagemath.org/ticket/3307_





---

archive/issue_events_035570.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-26T04:24:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "milestone_number": null,
    "milestone_title": "sage-3.0.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3307#event-35570"
}
```



---

archive/issue_events_035571.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-26T04:24:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "000080",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3307#event-35571"
}
```



---

archive/issue_events_035572.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-26T04:24:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3307#event-35572"
}
```



---

archive/issue_events_035573.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-26T04:24:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3307#event-35573"
}
```



---

archive/issue_events_035574.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-26T04:24:51Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "subject": "https://github.com/timabbott",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3307#event-35574"
}
```



---

archive/issue_events_035575.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-26T04:24:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3307#event-35575"
}
```



---

archive/issue_comments_017486.json:
```json
{
    "body": "Attachment: **[genus2reduction-lib.patch.gz](https://github.com/sagemath/sage/files/ticket3307/genus2reduction-lib.patch.gz)**",
    "created_at": "2008-05-26T04:25:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3307#issuecomment-17486",
    "user": "https://github.com/timabbott"
}
```

Attachment: **[genus2reduction-lib.patch.gz](https://github.com/sagemath/sage/files/ticket3307/genus2reduction-lib.patch.gz)**



---

archive/issue_comments_017487.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nPatch looks good to me. I have slipped it into\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/genus2reduction-0.3.p3.spkg\n\nwithout incrementing the patch level to avoid rebuilds.\n\nCheers,\n\nMichael",
    "created_at": "2008-05-28T06:41:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3307#issuecomment-17487",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Patch looks good to me. I have slipped it into

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/genus2reduction-0.3.p3.spkg

without incrementing the patch level to avoid rebuilds.

Cheers,

Michael



---

archive/issue_events_035576.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-28T06:41:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3307#event-35576"
}
```



---

archive/issue_events_035577.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-28T06:41:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3307#event-35577"
}
```



---

archive/issue_events_035578.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-28T06:41:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3307#event-35578"
}
```



---

archive/issue_comments_017488.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.0.3.alpha0",
    "created_at": "2008-05-28T06:41:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3307#issuecomment-17488",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.0.3.alpha0
