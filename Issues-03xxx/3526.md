# Issue 3526: notebook -- change favicon to the new one seen at the new sagemath.org web site

archive/issues_003526.json:
```json
{
    "assignees": [
        "https://github.com/TimothyClemans"
    ],
    "body": "\n\nKeywords: **editor_wstein**\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3526_\n\n",
    "closed_at": "2008-07-06T02:38:43Z",
    "created_at": "2008-06-28T09:01:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook -- change favicon to the new one seen at the new sagemath.org web site",
    "type": "issue",
    "updated_at": "2008-07-06T02:38:43Z",
    "url": "https://github.com/sagemath/sage/issues/3526",
    "user": "https://github.com/TimothyClemans"
}
```


Keywords: **editor_wstein**

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/3526_





---

archive/issue_events_038563.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-06-28T09:01:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38563"
}
```



---

archive/issue_events_038564.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-06-28T09:01:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38564"
}
```



---

archive/issue_events_038565.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-06-28T09:01:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38565"
}
```



---

archive/issue_events_038566.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-06-28T09:01:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38566"
}
```



---

archive/issue_events_038567.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-06-28T09:01:55Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "subject": "https://github.com/TimothyClemans",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38567"
}
```



---

archive/issue_events_038568.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-06-28T09:04:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38568"
}
```



---

archive/issue_comments_019183.json:
```json
{
    "body": "Changed keywords from none to **editor_wstein**",
    "created_at": "2008-06-28T09:04:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3526#issuecomment-19183",
    "user": "https://github.com/TimothyClemans"
}
```

Changed keywords from none to **editor_wstein**



---

archive/issue_events_038569.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-06-28T09:07:53Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "subject": "https://github.com/TimothyClemans",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38569"
}
```



---

archive/issue_events_038570.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-06-28T09:07:53Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "subject": "https://github.com/TimothyClemans",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38570"
}
```



---

archive/issue_events_038571.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-06-28T09:07:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38571"
}
```



---

archive/issue_events_038572.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-06-28T09:07:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38572"
}
```



---

archive/issue_events_038573.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-06-28T09:07:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38573"
}
```



---

archive/issue_events_038574.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-06T02:18:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38574"
}
```



---

archive/issue_comments_019184.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis patch is basically empty since favicon.ico is binary and not in the patch:\n\n```\ncat trac_3526_extcode-3526.patch \n# HG changeset patch\n# User Timothy Clemans <timothy.clemans@gmail.com>\n# Date 1214643704 25200\n# Node ID c6d839efcf8a7730a04e74399930ffab23651a5f\n# Parent  c3d96fbf0f19bf8b2c0c1c5188943ba54829f947\n#3526\n\ndiff -r c3d96fbf0f19 -r c6d839efcf8a notebook/images/favicon.ico\nBinary file notebook/images/favicon.ico has changed\n```\n\nCheers,\n\nMichael",
    "created_at": "2008-07-06T02:18:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3526#issuecomment-19184",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

This patch is basically empty since favicon.ico is binary and not in the patch:

```
cat trac_3526_extcode-3526.patch 
# HG changeset patch
# User Timothy Clemans <timothy.clemans@gmail.com>
# Date 1214643704 25200
# Node ID c6d839efcf8a7730a04e74399930ffab23651a5f
# Parent  c3d96fbf0f19bf8b2c0c1c5188943ba54829f947
#3526

diff -r c3d96fbf0f19 -r c6d839efcf8a notebook/images/favicon.ico
Binary file notebook/images/favicon.ico has changed
```

Cheers,

Michael



---

archive/issue_comments_019185.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nFrom http://developer.mozilla.org/en/docs/Mercurial_FAQ#How_can_I_diff_and_patch_files.3F:\n\n* If you are changing binary files or renaming files you may want to use git style patches with hg diff -g to retain that sort of information in the patch.\n* If you have a git style patch with renames and/or binary changes you can use hg import --no-commit to apply the patch to your tree or use hg qimport to import the patch to mq. \n\nCheers,\n\nMichael",
    "created_at": "2008-07-06T02:23:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3526#issuecomment-19185",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

From http://developer.mozilla.org/en/docs/Mercurial_FAQ#How_can_I_diff_and_patch_files.3F:

* If you are changing binary files or renaming files you may want to use git style patches with hg diff -g to retain that sort of information in the patch.
* If you have a git style patch with renames and/or binary changes you can use hg import --no-commit to apply the patch to your tree or use hg qimport to import the patch to mq. 

Cheers,

Michael



---

archive/issue_comments_019186.json:
```json
{
    "body": "Attachment: **[favicon.ico.gz](https://github.com/sagemath/sage/files/ticket3526/favicon.ico.gz)**",
    "created_at": "2008-07-06T02:30:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3526#issuecomment-19186",
    "user": "https://github.com/TimothyClemans"
}
```

Attachment: **[favicon.ico.gz](https://github.com/sagemath/sage/files/ticket3526/favicon.ico.gz)**



---

archive/issue_events_038575.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-06T02:38:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38575"
}
```



---

archive/issue_events_038576.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-06T02:38:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38576"
}
```



---

archive/issue_comments_019187.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[extcode-3526.2.patch.gz](https://github.com/sagemath/sage/files/ticket3526/extcode-3526.2.patch.gz)**\n\nPatch looks good to me, positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-06T02:38:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3526#issuecomment-19187",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[extcode-3526.2.patch.gz](https://github.com/sagemath/sage/files/ticket3526/extcode-3526.2.patch.gz)**

Patch looks good to me, positive review.

Cheers,

Michael



---

archive/issue_events_038577.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-06T02:38:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38577"
}
```



---

archive/issue_events_038578.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-06T02:38:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "milestone_number": null,
    "milestone_title": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38578"
}
```



---

archive/issue_events_038579.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-06T02:38:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38579"
}
```



---

archive/issue_events_038580.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-06T02:38:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3526#event-38580"
}
```



---

archive/issue_comments_019188.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.0.4.alpha2",
    "created_at": "2008-07-06T02:38:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3526",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3526#issuecomment-19188",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.0.4.alpha2
