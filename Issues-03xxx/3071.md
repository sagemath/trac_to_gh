# Issue 3071: [with patch, positive review] Using pbuild does not create site-packages sage symlink

archive/issues_003071.json:
```json
{
    "body": "If you use pbuild during the initial setup, it does not create the sage symlink in site-packages.  This can be fixed by performing:\nln -s devel/sage/build/sage/ local/lib/python/site-packages/sage\nduring the install process.  \n\nAssignee: @garyfurnish\n\nKeywords: pbuild\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3071\n\n",
    "closed_at": "2008-05-02T12:02:16Z",
    "created_at": "2008-05-01T10:26:22Z",
    "labels": [
        "component: build",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.1",
    "title": "[with patch, positive review] Using pbuild does not create site-packages sage symlink",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3071",
    "user": "https://github.com/garyfurnish"
}
```
If you use pbuild during the initial setup, it does not create the sage symlink in site-packages.  This can be fixed by performing:
ln -s devel/sage/build/sage/ local/lib/python/site-packages/sage
during the install process.  

Assignee: @garyfurnish

Keywords: pbuild

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3071





---

archive/issue_comments_022247.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-05-02T00:18:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3071#issuecomment-22247",
    "user": "https://github.com/garyfurnish"
}
```

Changing status from new to assigned.



---

archive/issue_comments_022248.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe attached patch should fix this issue and the linking issues reported on the mailing list.  It also fixes the banner.",
    "created_at": "2008-05-02T00:18:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3071#issuecomment-22248",
    "user": "https://github.com/garyfurnish"
}
```

<a id='comment:1'></a>
The attached patch should fix this issue and the linking issues reported on the mailing list.  It also fixes the banner.



---

archive/issue_comments_022249.json:
```json
{
    "body": "Changing assignee from mabshoff to @garyfurnish.",
    "created_at": "2008-05-02T00:18:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3071#issuecomment-22249",
    "user": "https://github.com/garyfurnish"
}
```

Changing assignee from mabshoff to @garyfurnish.



---

archive/issue_events_010882.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "rename": {
        "from": "Using pbuild does not create site-packages sage symlink",
        "to": "[with patch, needs review] Using pbuild does not create site-packages sage symlink"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3071#event-10882"
}
```



---

archive/issue_comments_022250.json:
```json
{
    "body": "Attachment [trac_3071.patch](tarball://root/attachments/some-uuid/ticket3071/trac_3071.patch) by @garyfurnish created at 2008-05-02 00:21:33",
    "created_at": "2008-05-02T00:21:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3071#issuecomment-22250",
    "user": "https://github.com/garyfurnish"
}
```

Attachment [trac_3071.patch](tarball://root/attachments/some-uuid/ticket3071/trac_3071.patch) by @garyfurnish created at 2008-05-02 00:21:33



---

archive/issue_comments_022251.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac_3071_2.patch](tarball://root/attachments/some-uuid/ticket3071/trac_3071_2.patch) by @wjp created at 2008-05-02 10:18:40",
    "created_at": "2008-05-02T10:18:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3071#issuecomment-22251",
    "user": "https://github.com/wjp"
}
```

<a id='comment:2'></a>
Attachment [trac_3071_2.patch](tarball://root/attachments/some-uuid/ticket3071/trac_3071_2.patch) by @wjp created at 2008-05-02 10:18:40



---

archive/issue_events_010883.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "rename": {
        "from": "[with patch, needs review] Using pbuild does not create site-packages sage symlink",
        "to": "[with patch, positive review] Using pbuild does not create site-packages sage symlink"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3071#event-10883"
}
```



---

archive/issue_comments_022252.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.1.rc0",
    "created_at": "2008-05-02T12:02:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3071#issuecomment-22252",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.1.rc0



---

archive/issue_events_010884.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-02T12:02:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3071#event-10884"
}
```



---

archive/issue_comments_022253.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-02T12:02:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3071",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3071#issuecomment-22253",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
