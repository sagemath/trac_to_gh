# Issue 3996: doctest the Singular interface

archive/issues_003996.json:
```json
{
    "assignees": [],
    "body": "\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3996_\n\n",
    "closed_at": "2008-08-30T00:39:17Z",
    "created_at": "2008-08-29T21:57:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "doctest the Singular interface",
    "type": "issue",
    "updated_at": "2008-08-30T00:39:17Z",
    "url": "https://github.com/sagemath/sage/issues/3996",
    "user": "https://github.com/mwhansen"
}
```


Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/3996_





---

archive/issue_events_038821.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-29T21:57:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "milestone_number": null,
    "milestone_title": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38821"
}
```



---

archive/issue_events_038822.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-29T21:57:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38822"
}
```



---

archive/issue_events_038823.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-29T21:57:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38823"
}
```



---

archive/issue_events_038824.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-29T21:57:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38824"
}
```



---

archive/issue_events_038825.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-29T21:57:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38825"
}
```



---

archive/issue_events_038826.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-08-29T22:08:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38826"
}
```



---

archive/issue_comments_023356.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\n**Review**\n* I don't think you need `sage: os.unlink(filename)` since the file is in tmp anyway\n* Sometimes the **r** before `\"\"\"` is missing but e.g. `\\var` is used\n\n\nIf those are fixed, I'll give it a positive review.",
    "created_at": "2008-08-29T22:08:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3996#issuecomment-23356",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'>Comment 1:</a>
**Review**
* I don't think you need `sage: os.unlink(filename)` since the file is in tmp anyway
* Sometimes the **r** before `"""` is missing but e.g. `\var` is used


If those are fixed, I'll give it a positive review.



---

archive/issue_comments_023357.json:
```json
{
    "body": "Attachment: **[trac_3996.patch.gz](https://github.com/sagemath/sage/files/ticket3996/trac_3996.patch.gz)**",
    "created_at": "2008-08-29T22:10:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3996#issuecomment-23357",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_3996.patch.gz](https://github.com/sagemath/sage/files/ticket3996/trac_3996.patch.gz)**



---

archive/issue_events_038827.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-08-29T22:11:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38827"
}
```



---

archive/issue_events_038828.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-08-29T22:11:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38828"
}
```



---

archive/issue_comments_023358.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nUnfortunately against my current alpha3 merge tree (the only relevant patch here over alpha2 is probably #3988):\n\n```\nmabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.1.2.alpha3/devel/sage$ patch -p1 --dry-run < trac_3996.patch \npatching file sage/interfaces/singular.py\nHunk #13 succeeded at 1010 with fuzz 2.\nHunk #14 succeeded at 1106 (offset 44 lines).\nHunk #15 FAILED at 1168.\nHunk #16 succeeded at 1350 (offset 44 lines).\nHunk #17 succeeded at 1375 (offset 44 lines).\nHunk #18 succeeded at 1399 (offset 44 lines).\nHunk #19 succeeded at 1474 (offset 44 lines).\nHunk #20 succeeded at 1505 (offset 44 lines).\nHunk #21 succeeded at 1524 (offset 44 lines).\nHunk #22 succeeded at 1575 (offset 44 lines).\nHunk #23 succeeded at 1625 (offset 44 lines).\n1 out of 23 hunks FAILED -- saving rejects to file sage/interfaces/singular.py.rej\n```\n\nCheers,\n\nMichael",
    "created_at": "2008-08-29T22:37:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3996#issuecomment-23358",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Unfortunately against my current alpha3 merge tree (the only relevant patch here over alpha2 is probably #3988):

```
mabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.1.2.alpha3/devel/sage$ patch -p1 --dry-run < trac_3996.patch 
patching file sage/interfaces/singular.py
Hunk #13 succeeded at 1010 with fuzz 2.
Hunk #14 succeeded at 1106 (offset 44 lines).
Hunk #15 FAILED at 1168.
Hunk #16 succeeded at 1350 (offset 44 lines).
Hunk #17 succeeded at 1375 (offset 44 lines).
Hunk #18 succeeded at 1399 (offset 44 lines).
Hunk #19 succeeded at 1474 (offset 44 lines).
Hunk #20 succeeded at 1505 (offset 44 lines).
Hunk #21 succeeded at 1524 (offset 44 lines).
Hunk #22 succeeded at 1575 (offset 44 lines).
Hunk #23 succeeded at 1625 (offset 44 lines).
1 out of 23 hunks FAILED -- saving rejects to file sage/interfaces/singular.py.rej
```

Cheers,

Michael



---

archive/issue_events_038829.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-29T22:37:57Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "title_is": "[needs rebase] doctest the Singular interface",
    "title_was": "doctest the Singular interface",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38829"
}
```



---

archive/issue_comments_023359.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAttachment: **[trac_3996-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket3996/trac_3996-rebase.patch.gz)**\n\ntrac_3996-rebase.patch should apply cleanly.",
    "created_at": "2008-08-29T22:54:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3996#issuecomment-23359",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:4'>Comment 4:</a>
Attachment: **[trac_3996-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket3996/trac_3996-rebase.patch.gz)**

trac_3996-rebase.patch should apply cleanly.



---

archive/issue_events_038830.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-29T22:54:09Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "title_is": "doctest the Singular interface",
    "title_was": "[needs rebase] doctest the Singular interface",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38830"
}
```



---

archive/issue_comments_023360.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nMerged trac_3996-rebase.patch in Sage 3.1.2.alpha3",
    "created_at": "2008-08-30T00:39:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3996#issuecomment-23360",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:5'>Comment 5:</a>
Merged trac_3996-rebase.patch in Sage 3.1.2.alpha3



---

archive/issue_events_038831.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-30T00:39:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38831"
}
```



---

archive/issue_events_038832.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-30T00:39:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3996",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3996#event-38832"
}
```
