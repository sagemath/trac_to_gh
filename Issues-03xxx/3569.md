# Issue 3569: optimize import of sage.dsage.interface.dsage_interface

archive/issues_003569.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-cwitty"
    ],
    "body": "BEFORE\n\n```\nteragon-2:misc was$ sage -startuptime|grep dsage_interface\n        sage.dsage.interface.dsage_interface: 0.092 (dist_function)\n         twisted.cred.credentials: 0.009 (sage.dsage.interface.dsage_interface)\n         twisted.internet.threads: 0.011 (sage.dsage.interface.dsage_interface)\n         twisted.internet.interfaces: 0.040 (sage.dsage.interface.dsage_interface)\n0.092 sage.dsage.interface.dsage_interface (dist_function)\n0.040 twisted.internet.interfaces (sage.dsage.interface.dsage_interface)\n```\n\nThis is after using it multiple times (so caching).\n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3569_\n\n",
    "closed_at": "2008-07-07T02:38:12Z",
    "created_at": "2008-07-06T20:13:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "optimize import of sage.dsage.interface.dsage_interface",
    "type": "issue",
    "updated_at": "2008-07-07T02:38:12Z",
    "url": "https://github.com/sagemath/sage/issues/3569",
    "user": "https://github.com/williamstein"
}
```
BEFORE

```
teragon-2:misc was$ sage -startuptime|grep dsage_interface
        sage.dsage.interface.dsage_interface: 0.092 (dist_function)
         twisted.cred.credentials: 0.009 (sage.dsage.interface.dsage_interface)
         twisted.internet.threads: 0.011 (sage.dsage.interface.dsage_interface)
         twisted.internet.interfaces: 0.040 (sage.dsage.interface.dsage_interface)
0.092 sage.dsage.interface.dsage_interface (dist_function)
0.040 twisted.internet.interfaces (sage.dsage.interface.dsage_interface)
```

This is after using it multiple times (so caching).

Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/3569_





---

archive/issue_events_039123.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-06T20:13:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "milestone_number": null,
    "milestone_title": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3569#event-39123"
}
```



---

archive/issue_events_039124.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-06T20:13:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3569#event-39124"
}
```



---

archive/issue_events_039125.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-06T20:13:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3569#event-39125"
}
```



---

archive/issue_events_039126.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2008-07-06T20:13:04Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3569#event-39126"
}
```



---

archive/issue_comments_019469.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[sage-3569.patch.gz](https://github.com/sagemath/sage/files/ticket3569/sage-3569.patch.gz)**\n\nAFTER\n\n```\nteragon-2:dsage was$ sage -startuptime|grep twisted\n```\n\n```\nteragon-2:dsage was$ sage -startuptime|grep dsage_interface\n        sage.dsage.interface.dsage_interface: 0.007 (dist_function)\n         sage.dsage.misc.misc: 0.005 (sage.dsage.interface.dsage_interface)\n```",
    "created_at": "2008-07-06T20:26:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3569#issuecomment-19469",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[sage-3569.patch.gz](https://github.com/sagemath/sage/files/ticket3569/sage-3569.patch.gz)**

AFTER

```
teragon-2:dsage was$ sage -startuptime|grep twisted
```

```
teragon-2:dsage was$ sage -startuptime|grep dsage_interface
        sage.dsage.interface.dsage_interface: 0.007 (dist_function)
         sage.dsage.misc.misc: 0.005 (sage.dsage.interface.dsage_interface)
```



---

archive/issue_events_039127.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-06T20:26:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3569#event-39127"
}
```



---

archive/issue_comments_019470.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\n+1",
    "created_at": "2008-07-06T20:28:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3569#issuecomment-19470",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

+1



---

archive/issue_events_039128.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-07-06T20:28:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3569#event-39128"
}
```



---

archive/issue_events_039129.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-07-06T20:28:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3569#event-39129"
}
```



---

archive/issue_events_039130.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-07T02:38:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3569#event-39130"
}
```



---

archive/issue_events_039131.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-07T02:38:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3569#event-39131"
}
```



---

archive/issue_comments_019471.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.0.4.alpha2",
    "created_at": "2008-07-07T02:38:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3569#issuecomment-19471",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.0.4.alpha2
