# Issue 3452: [with patch, positive review] implement inline error reporting for registration

archive/issues_003452.json:
```json
{
    "body": "Currently when a user has incorrectly filled out the registration form she is sent to a page that states the error.\n\nThe error should be reported under the input box related to the error. Also form should stay filled in except for the password input boxes.\n\nAssignee: TimothyClemans\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3452\n\n",
    "closed_at": "2008-06-25T05:49:13Z",
    "created_at": "2008-06-17T22:28:52Z",
    "labels": [
        "component: notebook"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.4",
    "title": "[with patch, positive review] implement inline error reporting for registration",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3452",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```
Currently when a user has incorrectly filled out the registration form she is sent to a page that states the error.

The error should be reported under the input box related to the error. Also form should stay filled in except for the password input boxes.

Assignee: TimothyClemans

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3452





---

archive/issue_comments_025601.json:
```json
{
    "body": "Changing assignee from boothby to TimothyClemans.",
    "created_at": "2008-06-17T22:29:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3452#issuecomment-25601",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

Changing assignee from boothby to TimothyClemans.



---

archive/issue_comments_025602.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-06-17T22:29:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3452#issuecomment-25602",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

Changing status from new to assigned.



---

archive/issue_comments_025603.json:
```json
{
    "body": "Attachment [extcode-3452.patch](tarball://root/attachments/some-uuid/ticket3452/extcode-3452.patch) by TimothyClemans created at 2008-06-18 00:28:55",
    "created_at": "2008-06-18T00:28:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3452#issuecomment-25603",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

Attachment [extcode-3452.patch](tarball://root/attachments/some-uuid/ticket3452/extcode-3452.patch) by TimothyClemans created at 2008-06-18 00:28:55



---

archive/issue_comments_025604.json:
```json
{
    "body": "Attachment [sage-3452.patch](tarball://root/attachments/some-uuid/ticket3452/sage-3452.patch) by TimothyClemans created at 2008-06-18 00:29:09",
    "created_at": "2008-06-18T00:29:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3452#issuecomment-25604",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

Attachment [sage-3452.patch](tarball://root/attachments/some-uuid/ticket3452/sage-3452.patch) by TimothyClemans created at 2008-06-18 00:29:09



---

archive/issue_events_012467.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "rename": {
        "from": "implement inline error reporting for registration",
        "to": "[with patch, needs review] implement inline error reporting for registration"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3452#event-12467"
}
```



---

archive/issue_events_012468.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "rename": {
        "from": "[with patch, needs review] implement inline error reporting for registration",
        "to": "[with patch, positive review] implement inline error reporting for registration"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3452#event-12468"
}
```



---

archive/issue_comments_025605.json:
```json
{
    "body": "<a id='comment:3'></a>\nsweet!",
    "created_at": "2008-06-24T07:06:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3452#issuecomment-25605",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

<a id='comment:3'></a>
sweet!



---

archive/issue_events_012469.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-06-24T07:09:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "milestone": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3452#event-12469"
}
```



---

archive/issue_comments_025606.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-06-25T05:49:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3452#issuecomment-25606",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_012470.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-25T05:49:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3452#event-12470"
}
```



---

archive/issue_comments_025607.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged both patches in Sage 3.0.4.alpha1",
    "created_at": "2008-06-25T05:49:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3452#issuecomment-25607",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged both patches in Sage 3.0.4.alpha1



---

archive/issue_events_012471.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-25T05:49:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "milestone": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3452#event-12471"
}
```



---

archive/issue_events_012472.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-25T05:49:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3452",
    "milestone": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3452#event-12472"
}
```
