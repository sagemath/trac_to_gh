# Issue 3610: [with patch, positive review] __contains__ for RealIntervalFields does not work correctly.

archive/issues_003610.json:
```json
{
    "body": "\n```\nsage: (R(2.1) + R(2.2))^2 in R\nFalse\nsage: R = RealIntervalField(32)\nsage: a = (R(2.1) + R(2.2))^2 \nsage: a\n[18.489999987 .. 18.490000010]\nsage: a in R\nFalse\nsage: a.parent()\nReal Interval Field with 32 bits of precision\nsage: a == a\nFalse\n```\n\nThis is caused by the following code in which gets inherited from parent.pyx:\n\n```\n        try:\n            x2 = self(x)\n            return bool(x2 == x)\n        except TypeError:\n            return False\n```\n\nSince equality is not reflexive for RealIntervals, this doesn't work as intended.\n\nAssignee: somebody\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3610\n\n",
    "closed_at": "2009-01-24T19:31:24Z",
    "created_at": "2008-07-08T17:41:30Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch, positive review] __contains__ for RealIntervalFields does not work correctly.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3610",
    "user": "https://github.com/mwhansen"
}
```

```
sage: (R(2.1) + R(2.2))^2 in R
False
sage: R = RealIntervalField(32)
sage: a = (R(2.1) + R(2.2))^2 
sage: a
[18.489999987 .. 18.490000010]
sage: a in R
False
sage: a.parent()
Real Interval Field with 32 bits of precision
sage: a == a
False
```

This is caused by the following code in which gets inherited from parent.pyx:

```
        try:
            x2 = self(x)
            return bool(x2 == x)
        except TypeError:
            return False
```

Since equality is not reflexive for RealIntervals, this doesn't work as intended.

Assignee: somebody

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3610





---

archive/issue_comments_026816.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac3610.patch](tarball://root/attachments/some-uuid/ticket3610/trac3610.patch) by cwitty created at 2009-01-24 03:04:27",
    "created_at": "2009-01-24T03:04:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3610#issuecomment-26816",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:1'></a>
Attachment [trac3610.patch](tarball://root/attachments/some-uuid/ticket3610/trac3610.patch) by cwitty created at 2009-01-24 03:04:27



---

archive/issue_events_013211.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3610",
    "rename": {
        "from": "__contains__ for RealIntervalFields does not work correctly.",
        "to": "[with patch, needs review] __contains__ for RealIntervalFields does not work correctly."
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3610#event-13211"
}
```



---

archive/issue_events_013212.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3610",
    "rename": {
        "from": "[with patch, needs review] __contains__ for RealIntervalFields does not work correctly.",
        "to": "[with patch, positive review] __contains__ for RealIntervalFields does not work correctly."
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3610#event-13212"
}
```



---

archive/issue_comments_026817.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me, and is probably an worthwhile optimization as well.",
    "created_at": "2009-01-24T04:28:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3610#issuecomment-26817",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:2'></a>
Looks good to me, and is probably an worthwhile optimization as well.



---

archive/issue_comments_026818.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-01-24T19:31:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3610#issuecomment-26818",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_013213.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-24T19:31:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3610",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3610#event-13213"
}
```



---

archive/issue_comments_026819.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.3.alpha2\n\nCheers,\n\nMichael",
    "created_at": "2009-01-24T19:31:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3610#issuecomment-26819",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.3.alpha2

Cheers,

Michael



---

archive/issue_events_013214.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-24T19:31:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3610",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3610#event-13214"
}
```
