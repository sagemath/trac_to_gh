# Issue 3832: [with spkg, positive review] add experimental Quantlib.spkg and Quantlib-SWIG.spkg to the repo

archive/issues_003832.json:
```json
{
    "body": "Quantlib and its Swig bindings are available at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.1/alpha2/quantlib-0.9.6.spkg\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.1/alpha2/quantlib-0.9.6-swig.spkg\n\nCaution: Building the spkgs only works on Linux (and potentially Solaris), takes about 2 hours CPUTIME, 1.9+ GB disk space and about 2GB RAM. Not for the faint of heart. This also requires the experimental boost-1.34_1.spkg.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3832\n\n",
    "closed_at": "2008-08-13T06:36:25Z",
    "created_at": "2008-08-13T04:33:57Z",
    "labels": [
        "component: packages: experimental",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1",
    "title": "[with spkg, positive review] add experimental Quantlib.spkg and Quantlib-SWIG.spkg to the repo",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3832",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Quantlib and its Swig bindings are available at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.1/alpha2/quantlib-0.9.6.spkg
http://sage.math.washington.edu/home/mabshoff/release-cycles-3.1/alpha2/quantlib-0.9.6-swig.spkg

Caution: Building the spkgs only works on Linux (and potentially Solaris), takes about 2 hours CPUTIME, 1.9+ GB disk space and about 2GB RAM. Not for the faint of heart. This also requires the experimental boost-1.34_1.spkg.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3832





---

archive/issue_comments_028698.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-08-13T04:34:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3832",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3832#issuecomment-28698",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_events_014196.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3832",
    "rename": {
        "from": "[with spkg, needs review] add experimental Quantlib.spkg and Quantlib-SWIG.spkg to the repo",
        "to": "[with spkg, positive review] add experimental Quantlib.spkg and Quantlib-SWIG.spkg to the repo"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3832#event-14196"
}
```



---

archive/issue_events_014197.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-13T06:36:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3832",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3832#event-14197"
}
```



---

archive/issue_comments_028699.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.alpha2",
    "created_at": "2008-08-13T06:36:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3832",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3832#issuecomment-28699",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.alpha2



---

archive/issue_comments_028700.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-13T06:36:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3832",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3832#issuecomment-28700",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
