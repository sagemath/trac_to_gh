# Issue 3073: [with patch, positive review] allow variable width text input_box in interact

archive/issues_003073.json:
```json
{
    "body": "This is just a tiny patch so that the text inputs for `@`interact can be changed in size.\n\nAssignee: somebody\n\nCC:  @williamstein\n\nKeywords: interact, input_box\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3073\n\n",
    "closed_at": "2008-06-15T20:03:33Z",
    "created_at": "2008-05-01T17:49:26Z",
    "labels": [
        "component: notebook",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.3",
    "title": "[with patch, positive review] allow variable width text input_box in interact",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3073",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```
This is just a tiny patch so that the text inputs for `@`interact can be changed in size.

Assignee: somebody

CC:  @williamstein

Keywords: interact, input_box

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3073





---

archive/issue_comments_022257.json:
```json
{
    "body": "Attachment [trac_3073_interact_width.patch](tarball://root/attachments/some-uuid/ticket3073/trac_3073_interact_width.patch) by mhampton created at 2008-05-01 17:52:43\n\nadds width option to input_box in interact",
    "created_at": "2008-05-01T17:52:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3073#issuecomment-22257",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

Attachment [trac_3073_interact_width.patch](tarball://root/attachments/some-uuid/ticket3073/trac_3073_interact_width.patch) by mhampton created at 2008-05-01 17:52:43

adds width option to input_box in interact



---

archive/issue_events_010888.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3073",
    "rename": {
        "from": "allow variable width text input_box in interact",
        "to": "[with patch, needs review] allow variable width text input_box in interact"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3073#event-10888"
}
```



---

archive/issue_events_010889.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3073",
    "rename": {
        "from": "[with patch, needs review] allow variable width text input_box in interact",
        "to": "[with patch, positive review] allow variable width text input_box in interact"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3073#event-10889"
}
```



---

archive/issue_comments_022258.json:
```json
{
    "body": "<a id='comment:2'></a>\nWorks for me.",
    "created_at": "2008-06-15T00:53:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3073#issuecomment-22258",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

<a id='comment:2'></a>
Works for me.



---

archive/issue_comments_022259.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.3.rc0",
    "created_at": "2008-06-15T20:03:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3073#issuecomment-22259",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.3.rc0



---

archive/issue_events_010890.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-15T20:03:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3073",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3073#event-10890"
}
```



---

archive/issue_comments_022260.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-06-15T20:03:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3073#issuecomment-22260",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_010891.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-15T20:04:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3073",
    "milestone": "sage-3.0.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3073#event-10891"
}
```
