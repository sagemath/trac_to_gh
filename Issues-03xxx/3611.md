# Issue 3611: [with spkg, positive review] sympow: make it use $CC instead of cc

archive/issues_003611.json:
```json
{
    "body": "Make sympow default to $CC[=gcc] instead of cc since that can cause trouble by picking up another compiler\n\nCheers,\n\nMichael\n\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3611\n\n",
    "closed_at": "2008-07-29T15:33:11Z",
    "created_at": "2008-07-08T17:42:57Z",
    "labels": [
        "component: build",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.6",
    "title": "[with spkg, positive review] sympow: make it use $CC instead of cc",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3611",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Make sympow default to $CC[=gcc] instead of cc since that can cause trouble by picking up another compiler

Cheers,

Michael


Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3611





---

archive/issue_comments_026105.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-07-08T17:43:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3611#issuecomment-26105",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_026106.json:
```json
{
    "body": "<a id='comment:2'></a>An spkg which picks the default gcc from $PATH is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.6/final/sympow-1.018.1.p5.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-07-29T15:16:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3611#issuecomment-26106",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>An spkg which picks the default gcc from $PATH is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.6/final/sympow-1.018.1.p5.spkg

Cheers,

Michael



---

archive/issue_events_008279.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-29T15:16:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3611",
    "milestone": "sage-3.0.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3611#event-8279"
}
```



---

archive/issue_comments_026107.json:
```json
{
    "body": "<a id='comment:3'></a>Looks safe enough.",
    "created_at": "2008-07-29T15:24:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3611#issuecomment-26107",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>Looks safe enough.



---

archive/issue_comments_026108.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-07-29T15:33:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3611#issuecomment-26108",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_008280.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-29T15:33:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3611",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3611#event-8280"
}
```



---

archive/issue_comments_026109.json:
```json
{
    "body": "<a id='comment:4'></a>Merged in Sage 3.0.6.final",
    "created_at": "2008-07-29T15:33:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3611#issuecomment-26109",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>Merged in Sage 3.0.6.final
