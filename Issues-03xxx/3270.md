# Issue 3270: [with patch, positive review] trivial 100x speedup in coding theory

archive/issues_003270.json:
```json
{
    "body": "Was:\n\n```\nsage: C = ExtendedBinaryGolayCode()\nsage: time G = C.permutation_automorphism_group()\nCPU times: user 2.39 s, sys: 0.58 s, total: 2.97 s\nWall time: 24.32\n```\nNow:\n\n```\nsage: C = ExtendedBinaryGolayCode()\nsage: time G = C.permutation_automorphism_group()\nCPU times: user 0.19 s, sys: 0.04 s, total: 0.23 s\nWall time: 0.24\n```\n\nAssignee: @rlmill\n\nCC:  @wdjoyner\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3270\n\n",
    "closed_at": "2008-05-22T10:36:46Z",
    "created_at": "2008-05-22T00:26:04Z",
    "labels": [
        "component: coding theory",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.2",
    "title": "[with patch, positive review] trivial 100x speedup in coding theory",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3270",
    "user": "https://github.com/rlmill"
}
```
Was:

```
sage: C = ExtendedBinaryGolayCode()
sage: time G = C.permutation_automorphism_group()
CPU times: user 2.39 s, sys: 0.58 s, total: 2.97 s
Wall time: 24.32
```
Now:

```
sage: C = ExtendedBinaryGolayCode()
sage: time G = C.permutation_automorphism_group()
CPU times: user 0.19 s, sys: 0.04 s, total: 0.23 s
Wall time: 0.24
```

Assignee: @rlmill

CC:  @wdjoyner

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3270





---

archive/issue_events_011672.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3270",
    "rename": {
        "from": "[with patch, easy review] trivial 100x speedup in coding theory",
        "to": "[with patch, positive review] trivial 100x speedup in coding theory"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3270#event-11672"
}
```



---

archive/issue_comments_023753.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [3270-bc_default.patch](tarball://root/attachments/some-uuid/ticket3270/3270-bc_default.patch) by @mwhansen created at 2008-05-22 09:16:43\n\nLooks good to me.",
    "created_at": "2008-05-22T09:16:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3270#issuecomment-23753",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [3270-bc_default.patch](tarball://root/attachments/some-uuid/ticket3270/3270-bc_default.patch) by @mwhansen created at 2008-05-22 09:16:43

Looks good to me.



---

archive/issue_comments_023754.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-22T10:36:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3270#issuecomment-23754",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_011673.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-22T10:36:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3270",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3270#event-11673"
}
```



---

archive/issue_events_011674.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-22T10:36:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3270",
    "milestone": "sage-3.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3270#event-11674"
}
```



---

archive/issue_comments_023755.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.0.2.rc0",
    "created_at": "2008-05-22T10:36:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3270#issuecomment-23755",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.0.2.rc0
