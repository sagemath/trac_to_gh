# Issue 3806: [three positive reviews] improvements to plot.py

archive/issues_003806.json:
```json
{
    "body": "If you do\n\n```\nsage: plot(sin(x), 100, 120))\n```\n\nyou get a plot which goes from -1 to 120 which is mostly empty space.  The is due to the behavior of Graphics() and `_extend_`axes.  Many of the other graphics objects suffer this same problem.  This patch fixes that and cleans up some of the useless code factoring in plot.py which hopefully makes it easier to understand.\n\n**Assignee:** @williamstein\n\n**Resolution:** fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3806\n\n",
    "closed_at": "2008-08-12T06:30:51Z",
    "created_at": "2008-08-11T19:57:07Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1",
    "title": "[three positive reviews] improvements to plot.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3806",
    "user": "https://github.com/mwhansen"
}
```
If you do

```
sage: plot(sin(x), 100, 120))
```

you get a plot which goes from -1 to 120 which is mostly empty space.  The is due to the behavior of Graphics() and `_extend_`axes.  Many of the other graphics objects suffer this same problem.  This patch fixes that and cleans up some of the useless code factoring in plot.py which hopefully makes it easier to understand.

**Assignee:** @williamstein

**Resolution:** fixed

Issue created by migration from https://trac.sagemath.org/ticket/3806





---

archive/attachments_003917.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_3806-1.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket3806/trac_3806-1.patch",
    "created_at": "2008-08-11T19:58:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/mwhansen"
}
```



---

archive/issue_comments_031234.json:
```json
{
    "body": "",
    "created_at": "2008-08-11T19:58:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31234",
    "user": "https://github.com/mwhansen"
}
```





---

archive/issue_comments_031235.json:
```json
{
    "body": "**Changing status** from new to needs_review.",
    "created_at": "2008-08-11T19:59:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31235",
    "user": "https://github.com/mwhansen"
}
```

**Changing status** from new to needs_review.



---

archive/attachments_003918.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_3806-2.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket3806/trac_3806-2.patch",
    "created_at": "2008-08-11T19:59:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/mwhansen"
}
```



---

archive/issue_comments_031236.json:
```json
{
    "body": "<a id='comment:1'></a>\n",
    "created_at": "2008-08-11T19:59:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31236",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>




---

archive/issue_comments_031237.json:
```json
{
    "body": "**Changing status** from needs_review to positive_review.",
    "created_at": "2008-08-12T01:52:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31237",
    "user": "https://trac.sagemath.org/admin/accounts/users/ekirkman"
}
```

**Changing status** from needs_review to positive_review.



---

archive/issue_comments_031238.json:
```json
{
    "body": "<a id='comment:2'></a>\nmabshoff is going to test it, but super-dooper +1 on style!",
    "created_at": "2008-08-12T01:52:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31238",
    "user": "https://trac.sagemath.org/admin/accounts/users/ekirkman"
}
```

<a id='comment:2'></a>
mabshoff is going to test it, but super-dooper +1 on style!



---

archive/issue_comments_031239.json:
```json
{
    "body": "<a id='comment:3'></a>\nThe following test fails:\n\n```\nmabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.1.alpha2$ ./sage -t -long devel/sage/sage/schemes/elliptic_curves/ell_point.py\nsage -t -long devel/sage/sage/schemes/elliptic_curves/ell_point.py\n**********************************************************************\nFile \"/scratch/mabshoff/release-cycle/sage-3.1.alpha2/tmp/ell_point.py\", line 392:\n    sage: P.plot(pointsize=30, rgbcolor=(1,0,0))\nException raised:\n    Traceback (most recent call last):\n      File \"/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/doctest.py\", line 1228, in __run\n        compileflags, 1) in test.globs\n      File \"<doctest `__main__`.example_18[3]>\", line 1, in <module>\n        P.plot(pointsize=Integer(30), rgbcolor=(Integer(1),Integer(0),Integer(0)))###line 392:\n    sage: P.plot(pointsize=30, rgbcolor=(1,0,0))\n      File \"sage_object.pyx\", line 92, in sage.structure.sage_object.SageObject.__repr__ (sage/structure/sage_object.c:795)\n      File \"/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/plot.py\", line 740, in `_repr_`\n        self.show()\n      File \"/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/plot.py\", line 1242, in show\n        aspect_ratio=aspect_ratio)\n      File \"/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/plot.py\", line 1421, in save\n        xmin, xmax, ymin, ymax = sage_axes.add_xy_axes(subplot, xmin, xmax, ymin, ymax)\n      File \"/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/axes.py\", line 320, in add_xy_axes\n        y_axis_xpos, xstep, xtslminor, xtslmajor = self._find_axes(xmin, xmax)\n      File \"/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/axes.py\", line 234, in `_find_`axes\n        raise ValueError, \"maxval >= minval is required\"\n    ValueError: maxval >= minval is required\n**********************************************************************\n1 items had failures:\n   1 of   4 in `__main__`.example_18\n***Test Failed*** 1 failures.\nFor whitespace errors, see the file /scratch/mabshoff/release-cycle/sage-3.1.alpha2/tmp/.doctest_ell_point.py\n         [11.0 s]\nexit code: 1024\n```",
    "created_at": "2008-08-12T02:08:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31239",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
The following test fails:

```
mabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.1.alpha2$ ./sage -t -long devel/sage/sage/schemes/elliptic_curves/ell_point.py
sage -t -long devel/sage/sage/schemes/elliptic_curves/ell_point.py
**********************************************************************
File "/scratch/mabshoff/release-cycle/sage-3.1.alpha2/tmp/ell_point.py", line 392:
    sage: P.plot(pointsize=30, rgbcolor=(1,0,0))
Exception raised:
    Traceback (most recent call last):
      File "/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/doctest.py", line 1228, in __run
        compileflags, 1) in test.globs
      File "<doctest `__main__`.example_18[3]>", line 1, in <module>
        P.plot(pointsize=Integer(30), rgbcolor=(Integer(1),Integer(0),Integer(0)))###line 392:
    sage: P.plot(pointsize=30, rgbcolor=(1,0,0))
      File "sage_object.pyx", line 92, in sage.structure.sage_object.SageObject.__repr__ (sage/structure/sage_object.c:795)
      File "/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/plot.py", line 740, in `_repr_`
        self.show()
      File "/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/plot.py", line 1242, in show
        aspect_ratio=aspect_ratio)
      File "/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/plot.py", line 1421, in save
        xmin, xmax, ymin, ymax = sage_axes.add_xy_axes(subplot, xmin, xmax, ymin, ymax)
      File "/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/axes.py", line 320, in add_xy_axes
        y_axis_xpos, xstep, xtslminor, xtslmajor = self._find_axes(xmin, xmax)
      File "/scratch/mabshoff/release-cycle/sage-3.1.alpha2/local/lib/python2.5/site-packages/sage/plot/axes.py", line 234, in `_find_`axes
        raise ValueError, "maxval >= minval is required"
    ValueError: maxval >= minval is required
**********************************************************************
1 items had failures:
   1 of   4 in `__main__`.example_18
***Test Failed*** 1 failures.
For whitespace errors, see the file /scratch/mabshoff/release-cycle/sage-3.1.alpha2/tmp/.doctest_ell_point.py
         [11.0 s]
exit code: 1024
```



---

archive/issue_events_011374.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "rename": {
        "from": "improvements to plot.py",
        "to": "[needs doctest fix] improvements to plot.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3806#event-11374"
}
```



---

archive/issue_comments_031240.json:
```json
{
    "body": "**Changing status** from positive_review to positive_review.",
    "created_at": "2008-08-12T02:08:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31240",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Changing status** from positive_review to positive_review.



---

archive/issue_events_011375.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "rename": {
        "from": "[needs doctest fix] improvements to plot.py",
        "to": "[two positive reviews, needs doctest fix] improvements to plot.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3806#event-11375"
}
```



---

archive/issue_comments_031241.json:
```json
{
    "body": "<a id='comment:5'></a>\nEquivalently,\n\n```\nsage: point((-1,1),pointsize=30, rgbcolor=(1,0,0))\n<boom>\n```",
    "created_at": "2008-08-12T05:46:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31241",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:5'></a>
Equivalently,

```
sage: point((-1,1),pointsize=30, rgbcolor=(1,0,0))
<boom>
```



---

archive/attachments_003919.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_3806-bug.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket3806/trac_3806-bug.patch",
    "created_at": "2008-08-12T06:00:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/mwhansen"
}
```



---

archive/issue_comments_031242.json:
```json
{
    "body": "<a id='comment:6'></a>\n+1 to rlm's patch",
    "created_at": "2008-08-12T06:00:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31242",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:6'></a>
+1 to rlm's patch



---

archive/issue_events_011376.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "rename": {
        "from": "[two positive reviews, needs doctest fix] improvements to plot.py",
        "to": "[three positive reviews] improvements to plot.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3806#event-11376"
}
```



---

archive/issue_comments_031243.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2008-08-12T06:30:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31243",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Resolution:** fixed



---

archive/issue_comments_031244.json:
```json
{
    "body": "<a id='comment:7'></a>\nMerged all three patches in Sage 3.1.alpha2",
    "created_at": "2008-08-12T06:30:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3806#issuecomment-31244",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:7'></a>
Merged all three patches in Sage 3.1.alpha2



---

archive/issue_events_011377.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-12T06:30:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3806",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3806#event-11377"
}
```
