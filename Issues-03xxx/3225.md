# Issue 3225: [with spkg, positive review] add 64 bit OSX build support for cddlib

archive/issues_003225.json:
```json
{
    "body": "spkg coming up\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3225\n\n",
    "closed_at": "2008-05-18T23:54:56Z",
    "created_at": "2008-05-16T19:39:32Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.2",
    "title": "[with spkg, positive review] add 64 bit OSX build support for cddlib",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3225",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
spkg coming up

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3225





---

archive/issue_comments_023411.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-05-16T19:39:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3225#issuecomment-23411",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_023412.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [cddlib-094b.p1-64bit-osx.patch](tarball://root/attachments/some-uuid/ticket3225/cddlib-094b.p1-64bit-osx.patch) by mabshoff created at 2008-05-18 23:44:30\n\nThe updated spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/cddlib-094b.p2.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-05-18T23:44:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3225#issuecomment-23412",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Attachment [cddlib-094b.p1-64bit-osx.patch](tarball://root/attachments/some-uuid/ticket3225/cddlib-094b.p1-64bit-osx.patch) by mabshoff created at 2008-05-18 23:44:30

The updated spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/cddlib-094b.p2.spkg

Cheers,

Michael



---

archive/issue_events_011492.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3225",
    "rename": {
        "from": "add 64 bit OSX build support for cddlib",
        "to": "[with spkg, needs review] add 64 bit OSX build support for cddlib"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3225#event-11492"
}
```



---

archive/issue_comments_023413.json:
```json
{
    "body": "<a id='comment:3'></a>\ngood",
    "created_at": "2008-05-18T23:52:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3225#issuecomment-23413",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
good



---

archive/issue_events_011493.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3225",
    "rename": {
        "from": "[with spkg, needs review] add 64 bit OSX build support for cddlib",
        "to": "[with spkg, positive review] add 64 bit OSX build support for cddlib"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3225#event-11493"
}
```



---

archive/issue_events_011494.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-18T23:54:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3225",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3225#event-11494"
}
```



---

archive/issue_comments_023414.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.2.alpha1",
    "created_at": "2008-05-18T23:54:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3225#issuecomment-23414",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.2.alpha1



---

archive/issue_comments_023415.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-18T23:54:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3225#issuecomment-23415",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
