# Issue 3680: NumberFieldFractionalIdeal should not be a subclass of Ideal_generic

archive/issues_003680.json:
```json
{
    "body": "Why is `NumberFieldFractionalIdeal` a subclass of `Ideal_generic`?\n\nA fractional ideal is **not** an ideal.\n\nThis makes about as much as sense as having `Rational` be a subclass of `Integer`.\n\nThis has been discussed before:\n\nhttp://groups.google.com/group/sage-devel/browse_thread/thread/0b01d58d8c3565c2/c081ba96b5fed6eb?#c081ba96b5fed6eb\n\nAnd it came up again recently in #1367.\n\nThere seem to be serious design issues with the whole algebraic number theory setup in Sage which make it very frustrating to do any serious work on things like #1367.\n\n\n**Assignee:** @loefflerd\n\n**CC:**  alexghitza @pjbruin\n\n**Status:** new\n\nIssue created by migration from https://trac.sagemath.org/ticket/3680\n\n",
    "created_at": "2008-07-19T13:36:55Z",
    "labels": [
        "component: number fields",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "NumberFieldFractionalIdeal should not be a subclass of Ideal_generic",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3680",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```
Why is `NumberFieldFractionalIdeal` a subclass of `Ideal_generic`?

A fractional ideal is **not** an ideal.

This makes about as much as sense as having `Rational` be a subclass of `Integer`.

This has been discussed before:

http://groups.google.com/group/sage-devel/browse_thread/thread/0b01d58d8c3565c2/c081ba96b5fed6eb?#c081ba96b5fed6eb

And it came up again recently in #1367.

There seem to be serious design issues with the whole algebraic number theory setup in Sage which make it very frustrating to do any serious work on things like #1367.


**Assignee:** @loefflerd

**CC:**  alexghitza @pjbruin

**Status:** new

Issue created by migration from https://trac.sagemath.org/ticket/3680





---

archive/issue_events_011043.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-19T23:12:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "milestone": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11043"
}
```



---

archive/issue_events_011044.json:
```json
{
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "label": "component: number theory",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11044"
}
```



---

archive/issue_comments_026098.json:
```json
{
    "body": "**Assignee:** @williamstein",
    "created_at": "2009-01-28T22:01:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3680#issuecomment-26098",
    "user": "https://github.com/aghitza"
}
```

**Assignee:** @williamstein



---

archive/issue_events_011045.json:
```json
{
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "label": "component: number theory",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11045"
}
```



---

archive/issue_comments_026099.json:
```json
{
    "body": "**Changing assignee** from @williamstein to @loefflerd.",
    "created_at": "2009-07-20T20:08:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3680#issuecomment-26099",
    "user": "https://github.com/loefflerd"
}
```

**Changing assignee** from @williamstein to @loefflerd.



---

archive/issue_events_011046.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "milestone": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11046"
}
```



---

archive/issue_events_011047.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11047"
}
```



---

archive/issue_events_011048.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11048"
}
```



---

archive/issue_events_011049.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11049"
}
```



---

archive/issue_comments_026100.json:
```json
{
    "body": "**Upstream:** N/A",
    "created_at": "2014-04-24T22:40:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3680#issuecomment-26100",
    "user": "https://github.com/pjbruin"
}
```

**Upstream:** N/A



---

archive/issue_events_011050.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11050"
}
```



---

archive/issue_events_011051.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11051"
}
```



---

archive/issue_events_011052.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11052"
}
```



---

archive/issue_events_011053.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3680",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3680#event-11053"
}
```
