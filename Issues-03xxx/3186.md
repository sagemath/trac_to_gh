# Issue 3186: [with spkg, with positive review] fix 64 bit OSX build support for numpy

archive/issues_003186.json:
```json
{
    "body": "We need to create a fake gcc injecting \"-m64\" in the argument list since otherwise the conftest will fail.\n\nSpkg is coming up.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3186\n\n",
    "closed_at": "2008-05-19T04:01:34Z",
    "created_at": "2008-05-13T13:48:57Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.2",
    "title": "[with spkg, with positive review] fix 64 bit OSX build support for numpy",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3186",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
We need to create a fake gcc injecting "-m64" in the argument list since otherwise the conftest will fail.

Spkg is coming up.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3186





---

archive/issue_comments_023131.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-05-13T13:49:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3186#issuecomment-23131",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_023132.json:
```json
{
    "body": "Attachment [numpy-20080104-1.0.4.p2-64bit-osx.patch](tarball://root/attachments/some-uuid/ticket3186/numpy-20080104-1.0.4.p2-64bit-osx.patch) by mabshoff created at 2008-05-19 03:49:45",
    "created_at": "2008-05-19T03:49:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3186#issuecomment-23132",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [numpy-20080104-1.0.4.p2-64bit-osx.patch](tarball://root/attachments/some-uuid/ticket3186/numpy-20080104-1.0.4.p2-64bit-osx.patch) by mabshoff created at 2008-05-19 03:49:45



---

archive/issue_comments_023133.json:
```json
{
    "body": "Attachment [numpy-20080104-1.0.4.p2-64bit-osx_part2.patch](tarball://root/attachments/some-uuid/ticket3186/numpy-20080104-1.0.4.p2-64bit-osx_part2.patch) by mabshoff created at 2008-05-19 03:49:55",
    "created_at": "2008-05-19T03:49:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3186#issuecomment-23133",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [numpy-20080104-1.0.4.p2-64bit-osx_part2.patch](tarball://root/attachments/some-uuid/ticket3186/numpy-20080104-1.0.4.p2-64bit-osx_part2.patch) by mabshoff created at 2008-05-19 03:49:55



---

archive/issue_events_011332.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3186",
    "rename": {
        "from": "fix 64 bit OSX build support for numpy",
        "to": "[with spkg, needs review] fix 64 bit OSX build support for numpy"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3186#event-11332"
}
```



---

archive/issue_comments_023134.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [numpy-20080104-1.0.4.p2-64bit-osx_part3.patch](tarball://root/attachments/some-uuid/ticket3186/numpy-20080104-1.0.4.p2-64bit-osx_part3.patch) by mabshoff created at 2008-05-19 03:57:37\n\nThe updated spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/numpy-20080104-1.0.4.p4.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-05-19T03:57:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3186#issuecomment-23134",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Attachment [numpy-20080104-1.0.4.p2-64bit-osx_part3.patch](tarball://root/attachments/some-uuid/ticket3186/numpy-20080104-1.0.4.p2-64bit-osx_part3.patch) by mabshoff created at 2008-05-19 03:57:37

The updated spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/numpy-20080104-1.0.4.p4.spkg

Cheers,

Michael



---

archive/issue_events_011333.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3186",
    "rename": {
        "from": "[with spkg, needs review] fix 64 bit OSX build support for numpy",
        "to": "[with spkg, with positive review] fix 64 bit OSX build support for numpy"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3186#event-11333"
}
```



---

archive/issue_events_011334.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-19T04:01:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3186",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3186#event-11334"
}
```



---

archive/issue_comments_023135.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.2.alpha1",
    "created_at": "2008-05-19T04:01:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3186#issuecomment-23135",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.2.alpha1



---

archive/issue_comments_023136.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-19T04:01:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3186",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3186#issuecomment-23136",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
