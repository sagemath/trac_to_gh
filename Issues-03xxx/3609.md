# Issue 3609: Update eclib to eclib-20080310.p5.spkg

archive/issues_003609.json:
```json
{
    "assignees": [
        "https://github.com/JohnCremona"
    ],
    "body": "A slightly updated spkg is at\n\nhttp://www.warwick.ac.uk/staff/J.E.Cremona/eclib-20080310.p5.spkg\n\nAccording to John:\n\n```\nThere's a new version with that file deleted (and the mercurial logs\nup to date) in the same place as before, called eclib-20080310.p5.spkg\n\nJohn\n```\n\nJohn is talking about src/procs/ressol.c which is copyrighted by \"Copyright (c) 1994, 1995 by the LiDIA Group\". LiDIA is now GPL, but since the file is no longer used it was completely deleted.\n\nCheers,\n\nMichael\n\nKeywords: **editor_mabshoff**\n\nComponent: **build**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3609_\n\n",
    "closed_at": "2008-07-31T01:51:49Z",
    "created_at": "2008-07-08T11:56:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update eclib to eclib-20080310.p5.spkg",
    "type": "issue",
    "updated_at": "2008-07-31T01:51:49Z",
    "url": "https://github.com/sagemath/sage/issues/3609",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
A slightly updated spkg is at

http://www.warwick.ac.uk/staff/J.E.Cremona/eclib-20080310.p5.spkg

According to John:

```
There's a new version with that file deleted (and the mercurial logs
up to date) in the same place as before, called eclib-20080310.p5.spkg

John
```

John is talking about src/procs/ressol.c which is copyrighted by "Copyright (c) 1994, 1995 by the LiDIA Group". LiDIA is now GPL, but since the file is no longer used it was completely deleted.

Cheers,

Michael

Keywords: **editor_mabshoff**

Component: **build**

_Issue created by migration from https://trac.sagemath.org/ticket/3609_





---

archive/issue_events_039624.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-08T11:56:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39624"
}
```



---

archive/issue_events_039625.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-08T11:56:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "000080",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39625"
}
```



---

archive/issue_events_039626.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-08T11:56:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39626"
}
```



---

archive/issue_events_039627.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-08T11:56:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39627"
}
```



---

archive/issue_events_039628.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-07-08T11:56:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39628"
}
```



---

archive/issue_events_039629.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-09T16:29:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39629"
}
```



---

archive/issue_comments_019685.json:
```json
{
    "body": "Changed keywords from none to **editor_mabshoff**",
    "created_at": "2008-07-09T16:29:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3609#issuecomment-19685",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Changed keywords from none to **editor_mabshoff**



---

archive/issue_comments_019686.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nSpkg looks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-31T01:50:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3609#issuecomment-19686",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Spkg looks good to me.

Cheers,

Michael



---

archive/issue_events_039630.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:50:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39630"
}
```



---

archive/issue_events_039631.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:50:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39631"
}
```



---

archive/issue_events_039632.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:50:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39632"
}
```



---

archive/issue_events_039633.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:50:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39633"
}
```



---

archive/issue_comments_019687.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.1.alpha0",
    "created_at": "2008-07-31T01:51:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3609#issuecomment-19687",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.1.alpha0



---

archive/issue_events_039634.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:51:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39634"
}
```



---

archive/issue_events_039635.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:51:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-39635"
}
```
