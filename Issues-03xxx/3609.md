# Issue 3609: Update eclib to eclib-20080310.p5.spkg

archive/issues_003609.json:
```json
{
    "assignees": [],
    "body": "A slightly updated spkg is at\n\nhttp://www.warwick.ac.uk/staff/J.E.Cremona/eclib-20080310.p5.spkg\n\nAccording to John:\n\n```\nThere's a new version with that file deleted (and the mercurial logs\nup to date) in the same place as before, called eclib-20080310.p5.spkg\n\nJohn\n```\n\nJohn is talking about src/procs/ressol.c which is copyrighted by \"Copyright (c) 1994, 1995 by the LiDIA Group\". LiDIA is now GPL, but since the file is no longer used it was completely deleted.\n\nCheers,\n\nMichael\n\n**Assignee:** @JohnCremona\n\n**Keywords:** editor_mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3609\n\n",
    "closed_at": "2008-07-31T01:51:49Z",
    "created_at": "2008-07-08T11:56:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update eclib to eclib-20080310.p5.spkg",
    "type": "issue",
    "updated_at": "2008-07-31T01:51:49Z",
    "url": "https://github.com/sagemath/sage/issues/3609",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
A slightly updated spkg is at

http://www.warwick.ac.uk/staff/J.E.Cremona/eclib-20080310.p5.spkg

According to John:

```
There's a new version with that file deleted (and the mercurial logs
up to date) in the same place as before, called eclib-20080310.p5.spkg

John
```

John is talking about src/procs/ressol.c which is copyrighted by "Copyright (c) 1994, 1995 by the LiDIA Group". LiDIA is now GPL, but since the file is no longer used it was completely deleted.

Cheers,

Michael

**Assignee:** @JohnCremona

**Keywords:** editor_mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3609





---

archive/issue_events_020047.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-09T16:29:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-20047"
}
```



---

archive/issue_comments_020537.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"editor_mabshoff\".",
    "created_at": "2008-07-09T16:29:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3609#issuecomment-20537",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

**Changing keywords** from "" to "editor_mabshoff".



---

archive/issue_comments_020538.json:
```json
{
    "body": "<a id='comment:2'></a>\nSpkg looks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-31T01:50:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3609#issuecomment-20538",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'></a>
Spkg looks good to me.

Cheers,

Michael



---

archive/issue_events_020048.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:50:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-20048"
}
```



---

archive/issue_events_020049.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:50:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-20049"
}
```



---

archive/issue_events_020050.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:50:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-20050"
}
```



---

archive/issue_events_020051.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:50:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-20051"
}
```



---

archive/issue_comments_020539.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.alpha0",
    "created_at": "2008-07-31T01:51:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3609#issuecomment-20539",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.alpha0



---

archive/issue_events_020052.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:51:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-20052"
}
```



---

archive/issue_events_020053.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T01:51:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3609",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3609#event-20053"
}
```
