# Issue 3609: [with spkg, positive review] Update eclib to eclib-20080310.p5.spkg

archive/issues_003609.json:
```json
{
    "body": "A slightly updated spkg is at\n\nhttp://www.warwick.ac.uk/staff/J.E.Cremona/eclib-20080310.p5.spkg\n\nAccording to John:\n\n```\nThere's a new version with that file deleted (and the mercurial logs\nup to date) in the same place as before, called eclib-20080310.p5.spkg\n\nJohn\n```\n\nJohn is talking about src/procs/ressol.c which is copyrighted by \"Copyright (c) 1994, 1995 by the LiDIA Group\". LiDIA is now GPL, but since the file is no longer used it was completely deleted.\n\nCheers,\n\nMichael\n\nAssignee: @JohnCremona\n\nKeywords: editor_mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3609\n\n",
    "closed_at": "2008-07-31T01:51:49Z",
    "created_at": "2008-07-08T11:56:38Z",
    "labels": [
        "component: build",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1",
    "title": "[with spkg, positive review] Update eclib to eclib-20080310.p5.spkg",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3609",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
A slightly updated spkg is at

http://www.warwick.ac.uk/staff/J.E.Cremona/eclib-20080310.p5.spkg

According to John:

```
There's a new version with that file deleted (and the mercurial logs
up to date) in the same place as before, called eclib-20080310.p5.spkg

John
```

John is talking about src/procs/ressol.c which is copyrighted by "Copyright (c) 1994, 1995 by the LiDIA Group". LiDIA is now GPL, but since the file is no longer used it was completely deleted.

Cheers,

Michael

Assignee: @JohnCremona

Keywords: editor_mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3609





---

archive/issue_events_013207.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3609",
    "rename": {
        "from": "Update eclib to eclib-20080310.p5.spkg",
        "to": "[with spkg, needs review] Update eclib to eclib-20080310.p5.spkg"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3609#event-13207"
}
```



---

archive/issue_comments_026812.json:
```json
{
    "body": "Changing keywords from \"\" to \"editor_mabshoff\".",
    "created_at": "2008-07-09T16:29:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3609#issuecomment-26812",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing keywords from "" to "editor_mabshoff".



---

archive/issue_comments_026813.json:
```json
{
    "body": "<a id='comment:2'></a>\nSpkg looks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-31T01:50:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3609#issuecomment-26813",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Spkg looks good to me.

Cheers,

Michael



---

archive/issue_events_013208.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3609",
    "rename": {
        "from": "[with spkg, needs review] Update eclib to eclib-20080310.p5.spkg",
        "to": "[with spkg, positive review] Update eclib to eclib-20080310.p5.spkg"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3609#event-13208"
}
```



---

archive/issue_events_013209.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-31T01:50:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3609",
    "milestone": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3609#event-13209"
}
```



---

archive/issue_comments_026814.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.alpha0",
    "created_at": "2008-07-31T01:51:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3609#issuecomment-26814",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.alpha0



---

archive/issue_comments_026815.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-07-31T01:51:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3609",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3609#issuecomment-26815",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_013210.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-31T01:51:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3609",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3609#event-13210"
}
```
