# Issue 3340: update givaro to 3.2.11 release

archive/issues_003340.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "We're currently using the givaro 3.2.10rc3 release candidate; we should update to the actual release.  \n\nThe differences are somewhat substantial:\n\n[tabbott@debuild tmp$] diff -ur givaro-3.2.10/ ../givaro-3.2.10~rc3/ | diffstat\n<SNIP>\n 123 files changed, 883 insertions(+), 1413 deletions(-)\n\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3340_\n\n",
    "closed_at": "2008-06-26T03:42:33Z",
    "created_at": "2008-05-30T17:40:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "update givaro to 3.2.11 release",
    "type": "issue",
    "updated_at": "2008-06-26T03:42:33Z",
    "url": "https://github.com/sagemath/sage/issues/3340",
    "user": "https://github.com/timabbott"
}
```
We're currently using the givaro 3.2.10rc3 release candidate; we should update to the actual release.  

The differences are somewhat substantial:

[tabbott@debuild tmp$] diff -ur givaro-3.2.10/ ../givaro-3.2.10~rc3/ | diffstat
<SNIP>
 123 files changed, 883 insertions(+), 1413 deletions(-)


Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/3340_





---

archive/issue_events_035971.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-30T17:40:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35971"
}
```



---

archive/issue_events_035972.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-30T17:40:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35972"
}
```



---

archive/issue_events_035973.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-30T17:40:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35973"
}
```



---

archive/issue_events_035974.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-05-30T17:40:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35974"
}
```



---

archive/issue_events_035975.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-30T17:40:22Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "subject": "https://github.com/timabbott",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35975"
}
```



---

archive/issue_comments_017769.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nClement Pernet's latest Givaro.spkg is at\n\nhttp://sage.math.washington.edu/home/pernet/givaro-3.2.11.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-06-26T03:25:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3340#issuecomment-17769",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Clement Pernet's latest Givaro.spkg is at

http://sage.math.washington.edu/home/pernet/givaro-3.2.11.spkg

Cheers,

Michael



---

archive/issue_events_035976.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-26T03:25:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35976"
}
```



---

archive/issue_events_035977.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-26T03:25:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "milestone_number": null,
    "milestone_title": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35977"
}
```



---

archive/issue_events_035978.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-26T03:25:50Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "title_is": "update givaro to 3.2.11 release",
    "title_was": "update givaro to 3.2.10 release",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35978"
}
```



---

archive/issue_events_035979.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-26T03:25:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35979"
}
```



---

archive/issue_comments_017770.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe spkg builds fine on OSX and x86-64 Linux and doctests fine. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-06-26T03:42:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3340#issuecomment-17770",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

The spkg builds fine on OSX and x86-64 Linux and doctests fine. Positive review.

Cheers,

Michael



---

archive/issue_events_035980.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-26T03:42:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35980"
}
```



---

archive/issue_events_035981.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-26T03:42:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35981"
}
```



---

archive/issue_events_035982.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-26T03:42:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35982"
}
```



---

archive/issue_events_035983.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-26T03:42:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3340#event-35983"
}
```



---

archive/issue_comments_017771.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.0.4.alpha1",
    "created_at": "2008-06-26T03:42:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3340",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3340#issuecomment-17771",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.0.4.alpha1
