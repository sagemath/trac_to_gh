# Issue 3334: [with patch, positive review] Matrix_mod2_dense may crash due to missing _sig_on/_sig_off

archive/issues_003334.json:
```json
{
    "body": "See patch.\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3334\n\n",
    "closed_at": "2008-05-31T06:21:56Z",
    "created_at": "2008-05-29T22:44:27Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.3",
    "title": "[with patch, positive review] Matrix_mod2_dense may crash due to missing _sig_on/_sig_off",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3334",
    "user": "https://github.com/malb"
}
```
See patch.

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3334





---

archive/issue_comments_024274.json:
```json
{
    "body": "Attachment [m4ri_mul_sigsegv.patch](tarball://root/attachments/some-uuid/ticket3334/m4ri_mul_sigsegv.patch) by @malb created at 2008-05-29 22:44:52",
    "created_at": "2008-05-29T22:44:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3334#issuecomment-24274",
    "user": "https://github.com/malb"
}
```

Attachment [m4ri_mul_sigsegv.patch](tarball://root/attachments/some-uuid/ticket3334/m4ri_mul_sigsegv.patch) by @malb created at 2008-05-29 22:44:52



---

archive/issue_comments_024275.json:
```json
{
    "body": "<a id='comment:1'></a>\nPatch looks good to me since it is obviously correct ;) Famous last words ...\n\nCheers,\n\nMichael",
    "created_at": "2008-05-31T06:18:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3334#issuecomment-24275",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Patch looks good to me since it is obviously correct ;) Famous last words ...

Cheers,

Michael



---

archive/issue_events_011893.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3334",
    "rename": {
        "from": "[with patch, needs review] Matrix_mod2_dense may crash due to missing _sig_on/_sig_off",
        "to": "[with patch, positive review] Matrix_mod2_dense may crash due to missing _sig_on/_sig_off"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3334#event-11893"
}
```



---

archive/issue_events_011894.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-31T06:21:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3334",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3334#event-11894"
}
```



---

archive/issue_comments_024276.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-31T06:21:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3334#issuecomment-24276",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_024277.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.0.3.alpha1",
    "created_at": "2008-05-31T06:21:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3334#issuecomment-24277",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.0.3.alpha1
