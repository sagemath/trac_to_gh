...
```json
{
    "body": "<a id='comment:3'></a>Please remember to put the \"[with patch, needs review]\" at the *beginning* of the summary.",
...
<a id='comment:3'></a>Please remember to put the "[with patch, needs review]" at the *beginning* of the summary.
...
```json
{
    "body": "<a id='comment:4'></a>I do not like the \"-fPIC\" -> \"-fpic\" change, otherwise I am fine with the patch.\n\nCheers,\n\nMichael",
...
<a id='comment:4'></a>I do not like the "-fPIC" -> "-fpic" change, otherwise I am fine with the patch.
...
<a id='comment:6'></a>OK new patches to ungnu-ify the makefiles and build shared and static objects
properly and use the bin folder. Tested with bsd make and gnu make - didn't have 
time for sun make, should work though. 
spkg-install also updated. everything is against eclib-20080310.p4
...
```json
{
    "body": "<a id='comment:7'></a>updated the patches to 20080310.p5.",
...
<a id='comment:7'></a>updated the patches to 20080310.p5.
...
```json
{
    "body": "<a id='comment:8'></a>I'm probably not the best person to review this but if there's anything I can do, please let me know.  John",
...
<a id='comment:8'></a>I'm probably not the best person to review this but if there's anything I can do, please let me know.  John
...
<a id='comment:9'></a>I think there are some issues with eclib-ungnu.patch:
...
<a id='comment:12'></a>OK it took me a little bit more time than expected - because of life and going back to this uncovered several "itches". So in summary there are two patches.
eclib-ungnu.patch:
*rebased on 20080310.p7
*do away with multiple building options we only build with ntl - that simplifies the suffixes of object files.
*created an install target
...
<a id='comment:13'></a>Thanks Francois,
...
<a id='comment:14'></a>Oh dear me just seen that in the need review message.
I have to check if it needs a lot of rebasing as it was 
against p5. Then may be David Kirby should have a look.
...
```json
{
    "body": "<a id='comment:15'></a>Found problems with qrank's binary and the tests.\nBack to need work. \nHopefully I will update this soonish.",
...
<a id='comment:15'></a>Found problems with qrank's binary and the tests.
Back to need work. 
Hopefully I will update this soonish.
...
```json
{
    "body": "<a id='comment:16'></a>This is obsolete, solved by the current eclib, we should close it.",
...
<a id='comment:16'></a>This is obsolete, solved by the current eclib, we should close it.
...
```json
{
    "body": "<a id='comment:19'></a>May I remind you to set the milestone to \"sage-duplicate/invalid/wontfix\" in such a case?",
...
<a id='comment:19'></a>May I remind you to set the milestone to "sage-duplicate/invalid/wontfix" in such a case?
