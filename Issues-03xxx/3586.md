...
```json
{
    "body": "<a id='comment:1'></a>It seems that on the machine where this fails for me, this is probably a race condition between the notebook starting and the first connection being made. At least, when I add a sleep(1) after the test_notebook() call, I can no longer reproduce this.\n\nMaybe we should make the notebook server print a small log message when it is ready to receive connections, and make run_notebook wait for that (through pexpect) when fork=True ?",
...
<a id='comment:1'></a>It seems that on the machine where this fails for me, this is probably a race condition between the notebook starting and the first connection being made. At least, when I add a sleep(1) after the test_notebook() call, I can no longer reproduce this.
...
<a id='comment:2'></a>wjp is exactly right -- it's a race condition.
