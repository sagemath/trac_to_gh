# Issue 3032: [with patch, positive review] minor docstring cleanup in crystals.py and tensor_product.py

archive/issues_003032.json:
```json
{
    "body": "This removes a test in tensor_product.py that is a repetition of a previous test (at line 193).\nIt also removes an obsolete comment in crystals.py.\n\nAssignee: @mwhansen\n\nCC:  sage-combinat\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3032\n\n",
    "closed_at": "2008-04-26T21:21:49Z",
    "created_at": "2008-04-26T16:31:36Z",
    "labels": [
        "component: combinatorics",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.1",
    "title": "[with patch, positive review] minor docstring cleanup in crystals.py and tensor_product.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3032",
    "user": "https://github.com/dwbump"
}
```
This removes a test in tensor_product.py that is a repetition of a previous test (at line 193).
It also removes an obsolete comment in crystals.py.

Assignee: @mwhansen

CC:  sage-combinat

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3032





---

archive/issue_comments_021911.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [9607.patch](tarball://root/attachments/some-uuid/ticket3032/9607.patch) by @mwhansen created at 2008-04-26 19:33:12",
    "created_at": "2008-04-26T19:33:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3032#issuecomment-21911",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [9607.patch](tarball://root/attachments/some-uuid/ticket3032/9607.patch) by @mwhansen created at 2008-04-26 19:33:12



---

archive/issue_events_010731.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3032",
    "rename": {
        "from": "minor docstring cleanup in crystals.py and tensor_product.py",
        "to": "[with patch, positive review] minor docstring cleanup in crystals.py and tensor_product.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3032#event-10731"
}
```



---

archive/issue_events_010732.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-26T21:21:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3032",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3032#event-10732"
}
```



---

archive/issue_comments_021912.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-26T21:21:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3032#issuecomment-21912",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_021913.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.0.1.alpha1",
    "created_at": "2008-04-26T21:21:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3032#issuecomment-21913",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.0.1.alpha1



---

archive/issue_events_010733.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-26T21:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3032",
    "milestone": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3032#event-10733"
}
```
