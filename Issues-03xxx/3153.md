# Issue 3153: [with patch, positive review] make finite_field_ntl_gf2e use randstate framework

archive/issues_003153.json:
```json
{
    "body": "The patch from #3020 added a new call to a pseudo-random-number generator, not under the control of the Sage randstate framework.  This patch fixes that up, as well as adding some new documentation to randstate.  Applies on top of the patch from #3020.\n\nAssignee: tbd\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3153\n\n",
    "closed_at": "2008-05-17T18:31:11Z",
    "created_at": "2008-05-10T22:41:39Z",
    "labels": [
        "component: algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.2",
    "title": "[with patch, positive review] make finite_field_ntl_gf2e use randstate framework",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3153",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
The patch from #3020 added a new call to a pseudo-random-number generator, not under the control of the Sage randstate framework.  This patch fixes that up, as well as adding some new documentation to randstate.  Applies on top of the patch from #3020.

Assignee: tbd

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3153





---

archive/issue_comments_022940.json:
```json
{
    "body": "Attachment [GF2X_sparse_poly_fix_random.patch](tarball://root/attachments/some-uuid/ticket3153/GF2X_sparse_poly_fix_random.patch) by cwitty created at 2008-05-10 22:41:57",
    "created_at": "2008-05-10T22:41:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3153#issuecomment-22940",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Attachment [GF2X_sparse_poly_fix_random.patch](tarball://root/attachments/some-uuid/ticket3153/GF2X_sparse_poly_fix_random.patch) by cwitty created at 2008-05-10 22:41:57



---

archive/issue_events_011222.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3153",
    "rename": {
        "from": "make finite_field_ntl_gf2e use randstate framework",
        "to": "[with patch, needs review] make finite_field_ntl_gf2e use randstate framework"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3153#event-11222"
}
```



---

archive/issue_events_011223.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3153",
    "rename": {
        "from": "[with patch, needs review] make finite_field_ntl_gf2e use randstate framework",
        "to": "[with patch, positive review] make finite_field_ntl_gf2e use randstate framework"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3153#event-11223"
}
```



---

archive/issue_comments_022941.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is a very good idea indeed.  The patch looks fine to me though it would also be agood idea if someone more familiar with Sage's random framework looked at it too.",
    "created_at": "2008-05-14T21:43:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3153#issuecomment-22941",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:2'></a>
This is a very good idea indeed.  The patch looks fine to me though it would also be agood idea if someone more familiar with Sage's random framework looked at it too.



---

archive/issue_events_011224.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-17T18:31:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3153",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3153#event-11224"
}
```



---

archive/issue_comments_022942.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-17T18:31:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3153#issuecomment-22942",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_022943.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.2.alpha1",
    "created_at": "2008-05-17T18:31:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3153",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3153#issuecomment-22943",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.2.alpha1
