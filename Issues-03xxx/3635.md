# Issue 3635: [with patch, positive review] If m is a matrix, then m.plot() should call matrix_plot

archive/issues_003635.json:
```json
{
    "body": "Currently this just puts the text description on an empty set of axes.\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3635\n\n",
    "closed_at": "2008-08-27T22:08:24Z",
    "created_at": "2008-07-10T20:41:22Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] If m is a matrix, then m.plot() should call matrix_plot",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3635",
    "user": "https://github.com/jasongrout"
}
```
Currently this just puts the text description on an empty set of axes.

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3635





---

archive/issue_events_013338.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "rename": {
        "from": "If m is a matrix, then m.plot() should call matrix_plot",
        "to": "[with patch, needs review] If m is a matrix, then m.plot() should call matrix_plot"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3635#event-13338"
}
```



---

archive/issue_comments_027047.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [matrix_plot.patch](tarball://root/attachments/some-uuid/ticket3635/matrix_plot.patch) by @malb created at 2008-08-18 21:17:32\n\nThe attached patch implements the requested behavior. Jason, do you want to review it?",
    "created_at": "2008-08-18T21:17:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3635#issuecomment-27047",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'></a>
Attachment [matrix_plot.patch](tarball://root/attachments/some-uuid/ticket3635/matrix_plot.patch) by @malb created at 2008-08-18 21:17:32

The attached patch implements the requested behavior. Jason, do you want to review it?



---

archive/issue_comments_027048.json:
```json
{
    "body": "<a id='comment:2'></a>\nWhoever reviews it and/or malb should note that the patches to #3853 would imply the call to MatrixPlotFactory is superfluous (though something may still have to be imported), but of course that patch has not yet been merged either.",
    "created_at": "2008-08-20T01:35:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3635#issuecomment-27048",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>
Whoever reviews it and/or malb should note that the patches to #3853 would imply the call to MatrixPlotFactory is superfluous (though something may still have to be imported), but of course that patch has not yet been merged either.



---

archive/issue_comments_027049.json:
```json
{
    "body": "Attachment [matrix_plot-rebased.patch](tarball://root/attachments/some-uuid/ticket3635/matrix_plot-rebased.patch) by @jasongrout created at 2008-08-27 14:44:02",
    "created_at": "2008-08-27T14:44:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3635#issuecomment-27049",
    "user": "https://github.com/jasongrout"
}
```

Attachment [matrix_plot-rebased.patch](tarball://root/attachments/some-uuid/ticket3635/matrix_plot-rebased.patch) by @jasongrout created at 2008-08-27 14:44:02



---

archive/issue_events_013339.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "rename": {
        "from": "[with patch, needs review] If m is a matrix, then m.plot() should call matrix_plot",
        "to": "[with patch, positive review] If m is a matrix, then m.plot() should call matrix_plot"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3635#event-13339"
}
```



---

archive/issue_comments_027050.json:
```json
{
    "body": "<a id='comment:3'></a>\nApply the rebased patch *AFTER* the first patch.  Sorry, I should have used a better word that rebased.\n\nWorks great. Positive review.",
    "created_at": "2008-08-27T14:46:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3635#issuecomment-27050",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'></a>
Apply the rebased patch *AFTER* the first patch.  Sorry, I should have used a better word that rebased.

Works great. Positive review.



---

archive/issue_comments_027051.json:
```json
{
    "body": "<a id='comment:4'></a>\nDoctests in matrix/matrix2.pyx pass after both patches have been applied.",
    "created_at": "2008-08-27T14:47:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3635#issuecomment-27051",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'></a>
Doctests in matrix/matrix2.pyx pass after both patches have been applied.



---

archive/issue_comments_027052.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-27T22:08:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3635#issuecomment-27052",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_027053.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged both patches in Sage 3.1.2.alpha2",
    "created_at": "2008-08-27T22:08:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3635#issuecomment-27053",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged both patches in Sage 3.1.2.alpha2



---

archive/issue_events_013340.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-27T22:08:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3635",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3635#event-13340"
}
```
