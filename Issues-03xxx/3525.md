# Issue 3525: [with patches, don't review] notebook -- new welcome page

archive/issues_003525.json:
```json
{
    "body": "Take the static page at http://timothyclemans.com/nb_homepage/ and merge it into the Sage Notebook code.\n\n**Assignee:** TimothyClemans\n\n**Keywords:** editor_wstein, stale\n\n**Reviewer:** Karl-Dieter Crisman\n\nIssue created by migration from https://trac.sagemath.org/ticket/3525\n\n",
    "closed_at": "2014-09-18T17:56:59Z",
    "created_at": "2008-06-28T06:40:47Z",
    "labels": [
        "component: notebook",
        "enhancement",
        "wontfix"
    ],
    "title": "[with patches, don't review] notebook -- new welcome page",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3525",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```
Take the static page at http://timothyclemans.com/nb_homepage/ and merge it into the Sage Notebook code.

**Assignee:** TimothyClemans

**Keywords:** editor_wstein, stale

**Reviewer:** Karl-Dieter Crisman

Issue created by migration from https://trac.sagemath.org/ticket/3525





---

archive/attachments_003655.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "extcode-3525.patch",
    "asset_url": "tarball://root/attachments/ticket3525/extcode-3525.patch",
    "created_at": "2008-06-28T08:06:35Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3525/extcode-3525.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```



---

archive/issue_comments_020001.json:
```json
{
    "body": "**Attachment:** [extcode-3525.patch](https://github.com/sagemath/sage/files/ticket3525/extcode-3525.patch)",
    "created_at": "2008-06-28T08:06:35Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20001",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

**Attachment:** [extcode-3525.patch](https://github.com/sagemath/sage/files/ticket3525/extcode-3525.patch)



---

archive/issue_events_019424.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2008-06-28T08:07:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "label": "component: number theory",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19424"
}
```



---

archive/issue_events_019425.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2008-06-28T08:07:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "label": "component: notebook",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19425"
}
```



---

archive/issue_comments_020002.json:
```json
{
    "body": "**Changing assignee** from @williamstein to TimothyClemans.",
    "created_at": "2008-06-28T08:07:43Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20002",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

**Changing assignee** from @williamstein to TimothyClemans.



---

archive/issue_events_019426.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2008-06-28T08:07:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19426"
}
```



---

archive/issue_events_019427.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2008-06-28T08:07:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19427"
}
```



---

archive/attachments_003656.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage-3525.patch",
    "asset_url": "tarball://root/attachments/ticket3525/sage-3525.patch",
    "created_at": "2008-06-28T08:07:56Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3525/sage-3525.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```



---

archive/issue_comments_020003.json:
```json
{
    "body": "**Attachment:** [sage-3525.patch](https://github.com/sagemath/sage/files/ticket3525/sage-3525.patch)",
    "created_at": "2008-06-28T08:07:56Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20003",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

**Attachment:** [sage-3525.patch](https://github.com/sagemath/sage/files/ticket3525/sage-3525.patch)



---

archive/issue_comments_020004.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"editor_wstein\".",
    "created_at": "2008-06-28T08:16:31Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20004",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

**Changing keywords** from "" to "editor_wstein".



---

archive/issue_comments_020005.json:
```json
{
    "body": "<a id='comment:3'></a>\nFrom Harald Schilly:\n\n```\nLooks good, yes. Just small remarks what I would change...\n\n1) The border around the sign-in formular is thin. Why not the same as\nthose two below? I would suggest to make all three borders small...\n\n2) The top header is an image. The serif-italic letters don't look\nvery well on my screen. (no subpixel hinting compared to directly\nrendered italic letters on my lcd) .. I would suggest to not use an\nimage, just the text string with a fixed font size (16pt?, italic,\nserif) and absolute placement.\n\n3) The forgot password page does not exist. I think, it should look\nthe same, except the login box and the middle content replaced by an\nexplanation and so on.\n\n4) maybe less screenshots (just a 3x2 matrix) but with larger images?\nso that someone could see more!\n\n5) it's not xhtml valid\nI strongly suggest to use xhtml transitional, since this is better for\ncompatibility and browsers like ie6 work better with that. xhtml\nstrict is just some sort of a more theoretical wish that will never\ncome true ;)\nbut anyways, ending a formular input tag with /> is not ok. browsers\ndon't understand real xml ...\n\n6) maybe more margin-borders on the left and right side of the middle\ncontent, to give everything a bit more \"air\" so that it looks less\ntight...\n\ngreetings Harald\n\n\nOn Sat, Jun 28, 2008 at 08:38, William Stein <wstein@gmail.com> wrote:\n> What do you think of\n>\n> http://timothyclemans.com/nb_homepage/\n>\n> --\n> William Stein\n> Associate Professor of Mathematics\n> University of Washington\n> http://wstein.org\n>\n```",
    "created_at": "2008-06-28T10:34:59Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20005",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

<a id='comment:3'></a>
From Harald Schilly:

```
Looks good, yes. Just small remarks what I would change...

1) The border around the sign-in formular is thin. Why not the same as
those two below? I would suggest to make all three borders small...

2) The top header is an image. The serif-italic letters don't look
very well on my screen. (no subpixel hinting compared to directly
rendered italic letters on my lcd) .. I would suggest to not use an
image, just the text string with a fixed font size (16pt?, italic,
serif) and absolute placement.

3) The forgot password page does not exist. I think, it should look
the same, except the login box and the middle content replaced by an
explanation and so on.

4) maybe less screenshots (just a 3x2 matrix) but with larger images?
so that someone could see more!

5) it's not xhtml valid
I strongly suggest to use xhtml transitional, since this is better for
compatibility and browsers like ie6 work better with that. xhtml
strict is just some sort of a more theoretical wish that will never
come true ;)
but anyways, ending a formular input tag with /> is not ok. browsers
don't understand real xml ...

6) maybe more margin-borders on the left and right side of the middle
content, to give everything a bit more "air" so that it looks less
tight...

greetings Harald


On Sat, Jun 28, 2008 at 08:38, William Stein <wstein@gmail.com> wrote:
> What do you think of
>
> http://timothyclemans.com/nb_homepage/
>
> --
> William Stein
> Associate Professor of Mathematics
> University of Washington
> http://wstein.org
>
```



---

archive/issue_comments_020006.json:
```json
{
    "body": "used Tidy to deal with point 5 of Harald's criticism",
    "created_at": "2008-06-28T10:57:34Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20006",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

used Tidy to deal with point 5 of Harald's criticism



---

archive/issue_events_019428.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-06-28T12:30:57Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "rename": {
        "from": "notebook -- new welcome page",
        "to": "[needs works] notebook -- new welcome page"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19428"
}
```



---

archive/attachments_003657.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "extcode-3525_2.patch",
    "asset_url": "tarball://root/attachments/ticket3525/extcode-3525_2.patch",
    "created_at": "2008-06-28T12:30:57Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3525/extcode-3525_2.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_020007.json:
```json
{
    "body": "<a id='comment:4'></a>\n**Attachment:** [extcode-3525_2.patch](https://github.com/sagemath/sage/files/ticket3525/extcode-3525_2.patch)",
    "created_at": "2008-06-28T12:30:57Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20007",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:4'></a>
**Attachment:** [extcode-3525_2.patch](https://github.com/sagemath/sage/files/ticket3525/extcode-3525_2.patch)



---

archive/issue_comments_020008.json:
```json
{
    "body": "<a id='comment:5'></a>\nRegarding the negative review could you tell me if you want all of Harald's suggestions implemented? \n\n1) I can do easily by switching in the css which background is called for a:link and a:hover\n\n2) Maybe do this in another ticket if others also complain\n\n3) A very minimalistic \"account recovery\" page will be in sage-3.0.4. Anything regarding it should be a different ticket\n\n4) I don't want to change them because A) I like it the way it is B) it would be very time consuming for me\n\n5) Done\n\n6) Sure",
    "created_at": "2008-06-28T14:04:21Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20008",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

<a id='comment:5'></a>
Regarding the negative review could you tell me if you want all of Harald's suggestions implemented? 

1) I can do easily by switching in the css which background is called for a:link and a:hover

2) Maybe do this in another ticket if others also complain

3) A very minimalistic "account recovery" page will be in sage-3.0.4. Anything regarding it should be a different ticket

4) I don't want to change them because A) I like it the way it is B) it would be very time consuming for me

5) Done

6) Sure



---

archive/issue_comments_020009.json:
```json
{
    "body": "<a id='comment:6'></a>\nFor a positive review it is enough to do 1, 5, 6.",
    "created_at": "2008-06-28T14:21:28Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20009",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:6'></a>
For a positive review it is enough to do 1, 5, 6.



---

archive/issue_events_019429.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2008-06-28T14:52:42Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "rename": {
        "from": "[needs works] notebook -- new welcome page",
        "to": "notebook -- new welcome page"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19429"
}
```



---

archive/issue_events_019430.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2008-06-28T14:52:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19430"
}
```



---

archive/attachments_003658.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "extcode-3525_3.patch",
    "asset_url": "tarball://root/attachments/ticket3525/extcode-3525_3.patch",
    "created_at": "2008-06-28T14:52:42Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3525/extcode-3525_3.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```



---

archive/issue_comments_020010.json:
```json
{
    "body": "<a id='comment:7'></a>\n**Attachment:** [extcode-3525_3.patch](https://github.com/sagemath/sage/files/ticket3525/extcode-3525_3.patch)\n\nmargins increased blue line under interactive computer programming changed to reflect new margins\n\nBorder for \"Sign up\" and \"Published worksheets\" is now 1px.",
    "created_at": "2008-06-28T14:52:42Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20010",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

<a id='comment:7'></a>
**Attachment:** [extcode-3525_3.patch](https://github.com/sagemath/sage/files/ticket3525/extcode-3525_3.patch)

margins increased blue line under interactive computer programming changed to reflect new margins

Border for "Sign up" and "Published worksheets" is now 1px.



---

archive/attachments_003659.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "extcode-3525_4.patch",
    "asset_url": "tarball://root/attachments/ticket3525/extcode-3525_4.patch",
    "created_at": "2008-06-28T14:58:59Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3525/extcode-3525_4.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```



---

archive/issue_comments_020011.json:
```json
{
    "body": "**Attachment:** [extcode-3525_4.patch](https://github.com/sagemath/sage/files/ticket3525/extcode-3525_4.patch)\n\nforgot to change which images are preloaded (the ones for signup and published worksheets)",
    "created_at": "2008-06-28T14:58:59Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20011",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

**Attachment:** [extcode-3525_4.patch](https://github.com/sagemath/sage/files/ticket3525/extcode-3525_4.patch)

forgot to change which images are preloaded (the ones for signup and published worksheets)



---

archive/issue_comments_020012.json:
```json
{
    "body": "<a id='comment:8'></a>\nIn extcode-3525_3.patch I see \n\n```\ndiff -r 84e3731cc21b -r 40c4b0aa9491 notebook/images/head.gif\nBinary file notebook/images/head.gif has changed\n```\n\nIs the actual image stored in the patch? If not then how do I put package images for inclusion?",
    "created_at": "2008-06-28T21:47:18Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20012",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

<a id='comment:8'></a>
In extcode-3525_3.patch I see 

```
diff -r 84e3731cc21b -r 40c4b0aa9491 notebook/images/head.gif
Binary file notebook/images/head.gif has changed
```

Is the actual image stored in the patch? If not then how do I put package images for inclusion?



---

archive/issue_comments_020013.json:
```json
{
    "body": "<a id='comment:9'></a>\nextcode_combined.patch is a git style patch that should contained the required images",
    "created_at": "2008-07-06T02:37:11Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20013",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

<a id='comment:9'></a>
extcode_combined.patch is a git style patch that should contained the required images



---

archive/issue_events_019431.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2008-07-07T22:45:15Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "rename": {
        "from": "notebook -- new welcome page",
        "to": "[with patches, don't review] notebook -- new welcome page"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19431"
}
```



---

archive/attachments_003660.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "extcode_combined.patch",
    "asset_url": "tarball://root/attachments/ticket3525/extcode_combined.patch",
    "created_at": "2008-07-07T22:45:15Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3525/extcode_combined.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```



---

archive/issue_comments_020014.json:
```json
{
    "body": "<a id='comment:10'></a>\n**Attachment:** [extcode_combined.patch](https://github.com/sagemath/sage/files/ticket3525/extcode_combined.patch)\n\nThe combined extcode patch is mess. Various fixes aren't in the patch ...",
    "created_at": "2008-07-07T22:45:15Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20014",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

<a id='comment:10'></a>
**Attachment:** [extcode_combined.patch](https://github.com/sagemath/sage/files/ticket3525/extcode_combined.patch)

The combined extcode patch is mess. Various fixes aren't in the patch ...



---

archive/issue_comments_020015.json:
```json
{
    "body": "<a id='comment:11'></a>\nWhat is the status of this patch, Timothy?",
    "created_at": "2010-08-27T15:38:38Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20015",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:11'></a>
What is the status of this patch, Timothy?



---

archive/issue_comments_020016.json:
```json
{
    "body": "**Changing keywords** from \"editor_wstein\" to \"editor_wstein, stale\".",
    "created_at": "2010-08-27T15:39:04Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20016",
    "user": "https://github.com/jasongrout"
}
```

**Changing keywords** from "editor_wstein" to "editor_wstein, stale".



---

archive/issue_events_019432.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19432"
}
```



---

archive/issue_events_019433.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19433"
}
```



---

archive/issue_events_019434.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19434"
}
```



---

archive/issue_events_019435.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19435"
}
```



---

archive/issue_events_019436.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19436"
}
```



---

archive/issue_events_019437.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19437"
}
```



---

archive/issue_events_019438.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19438"
}
```



---

archive/issue_events_019439.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19439"
}
```



---

archive/issue_events_019440.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-09-17T18:52:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19440"
}
```



---

archive/issue_comments_020017.json:
```json
{
    "body": "**Reviewer:** Karl-Dieter Crisman",
    "created_at": "2014-09-17T18:52:56Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20017",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Karl-Dieter Crisman



---

archive/issue_events_019441.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-09-17T18:52:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19441"
}
```



---

archive/issue_events_019442.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-09-17T18:52:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19442"
}
```



---

archive/issue_comments_020018.json:
```json
{
    "body": "<a id='comment:17'></a>\nThis is not a bad idea in general, but at this point the discussion about looks are subsumed into ideas about \"newUI\" and \"themes\" upstream.  Any new welcome page discussion should arrive there.  Sadly, wontfix is probably most appropriate for this six-year-old ticket.",
    "created_at": "2014-09-17T18:52:56Z",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3525#issuecomment-20018",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:17'></a>
This is not a bad idea in general, but at this point the discussion about looks are subsumed into ideas about "newUI" and "themes" upstream.  Any new welcome page discussion should arrive there.  Sadly, wontfix is probably most appropriate for this six-year-old ticket.



---

archive/issue_events_019443.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-18T17:56:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19443"
}
```



---

archive/issue_events_019444.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-18T17:56:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19444"
}
```



---

archive/issue_events_019445.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-18T17:56:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3525",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3525#event-19445"
}
```
