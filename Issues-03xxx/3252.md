# Issue 3252: [with patch, positive review] add kbase functionality to libsingular

archive/issues_003252.json:
```json
{
    "body": "implemented a cython wrapper for singular's kbase command. This is significantly faster than doing singular.kbase() because it doesn't have the pexpect overhead. \n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3252\n\n",
    "closed_at": "2008-05-18T16:18:01Z",
    "created_at": "2008-05-18T03:35:33Z",
    "labels": [
        "component: algebraic geometry"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.2",
    "title": "[with patch, positive review] add kbase functionality to libsingular",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3252",
    "user": "https://github.com/yqiang"
}
```
implemented a cython wrapper for singular's kbase command. This is significantly faster than doing singular.kbase() because it doesn't have the pexpect overhead. 

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3252





---

archive/issue_comments_023608.json:
```json
{
    "body": "Attachment [libsingular_kbase.patch](tarball://root/attachments/some-uuid/ticket3252/libsingular_kbase.patch) by @yqiang created at 2008-05-18 03:35:48",
    "created_at": "2008-05-18T03:35:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3252#issuecomment-23608",
    "user": "https://github.com/yqiang"
}
```

Attachment [libsingular_kbase.patch](tarball://root/attachments/some-uuid/ticket3252/libsingular_kbase.patch) by @yqiang created at 2008-05-18 03:35:48



---

archive/issue_events_011590.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "rename": {
        "from": "add kbase functionality to libsingular",
        "to": "[with patch, needs review] add kbase functionality to libsingular"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3252#event-11590"
}
```



---

archive/issue_events_011591.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "rename": {
        "from": "[with patch, needs review] add kbase functionality to libsingular",
        "to": "[with patch, positive review pending doctests] add kbase functionality to libsingular"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3252#event-11591"
}
```



---

archive/issue_comments_023609.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe patch looks good, applies cleanly and does as advertised. However, two new functions are introduced without doctests. Though they are only wrappers every new function must have a doctest these days.",
    "created_at": "2008-05-18T14:35:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3252#issuecomment-23609",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'></a>
The patch looks good, applies cleanly and does as advertised. However, two new functions are introduced without doctests. Though they are only wrappers every new function must have a doctest these days.



---

archive/issue_comments_023610.json:
```json
{
    "body": "Attachment [libsingular_kbase_doctest.patch](tarball://root/attachments/some-uuid/ticket3252/libsingular_kbase_doctest.patch) by @yqiang created at 2008-05-18 15:12:45\n\nadd doctests as requested by reviewer",
    "created_at": "2008-05-18T15:12:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3252#issuecomment-23610",
    "user": "https://github.com/yqiang"
}
```

Attachment [libsingular_kbase_doctest.patch](tarball://root/attachments/some-uuid/ticket3252/libsingular_kbase_doctest.patch) by @yqiang created at 2008-05-18 15:12:45

add doctests as requested by reviewer



---

archive/issue_comments_023611.json:
```json
{
    "body": "<a id='comment:3'></a>\nAttached a patch which doctests the 2 functions mentioned. Apply after the first patch.",
    "created_at": "2008-05-18T15:13:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3252#issuecomment-23611",
    "user": "https://github.com/yqiang"
}
```

<a id='comment:3'></a>
Attached a patch which doctests the 2 functions mentioned. Apply after the first patch.



---

archive/issue_events_011592.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "rename": {
        "from": "[with patch, positive review pending doctests] add kbase functionality to libsingular",
        "to": "[with patch, positive review] add kbase functionality to libsingular"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3252#event-11592"
}
```



---

archive/issue_comments_023612.json:
```json
{
    "body": "<a id='comment:4'></a>\nGreat! and even greater: We know have a new libSingular developer :-)",
    "created_at": "2008-05-18T15:29:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3252#issuecomment-23612",
    "user": "https://github.com/malb"
}
```

<a id='comment:4'></a>
Great! and even greater: We know have a new libSingular developer :-)



---

archive/issue_comments_023613.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-18T16:18:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3252#issuecomment-23613",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_023614.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged both patches in Sage 3.0.2.alpha1",
    "created_at": "2008-05-18T16:18:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3252#issuecomment-23614",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged both patches in Sage 3.0.2.alpha1



---

archive/issue_events_011593.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-18T16:18:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3252",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3252#event-11593"
}
```
