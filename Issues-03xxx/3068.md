...
```json
{
    "body": "<a id='comment:1'></a>This seems to be gone as of 3.1.1. Could someone close this?\n\n```\nsage: m = matrix([])\nsage: m.smith_form()\n ([], [], [])\n```",
...
<a id='comment:1'></a>This seems to be gone as of 3.1.1. Could someone close this?
...
<a id='comment:2'></a>In 3.2.2.alpha0 this works fine as long as the matrix has 0 rows and 0 columns, but for 0 rows and a nonzero number of columns (or vice versa) we still get the error. This arises from the fact that Pari doesn't have the notion of a matrix with 0 rows and 1 column, so the matrix gets "truncated" (!) before being passed to Pari.
...
```json
{
    "body": "<a id='comment:4'></a>I'm sorry for overstepping my authority. Anyway, the fix is now up at #4681.",
...
<a id='comment:4'></a>I'm sorry for overstepping my authority. Anyway, the fix is now up at #4681.
...
```json
{
    "body": "<a id='comment:6'></a>Fixed in Sage 3.2.2.alpha1 via #4681.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-10T11:27:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3068",
    "type": "issue_comment",
...
<a id='comment:6'></a>Fixed in Sage 3.2.2.alpha1 via #4681.
