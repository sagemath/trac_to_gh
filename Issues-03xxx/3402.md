# Issue 3402: [with patch, with positive review] Digraph.incoming_edges forgets labels

archive/issues_003402.json:
```json
{
    "body": "\n```\nsage: dg = DiGraph({0 : [1], 1 : [0]})\nsage: dg.set_edge_label(0,1,5)\nsage: dg.set_edge_label(1,0,9)\nsage: dg.edges()\n[(0, 1, 5), (1, 0, 9)]\nsage: dg.outgoing_edges([1])\n[(1, 0, 9)]\nsage: dg.incoming_edges([1])\n[(0, 1, None)]\nsage: dg.outgoing_edges(0)\n[(0, 1, 5)]\nsage: dg.incoming_edges(0)\n[(1, 0, None)]\n```\n\nAs you can see, outgoing_edges remembers the labels but incoming_edges does not.\n\nAssignee: @rlmill\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3402\n\n",
    "closed_at": "2008-06-16T04:30:01Z",
    "created_at": "2008-06-12T05:48:36Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.3",
    "title": "[with patch, with positive review] Digraph.incoming_edges forgets labels",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3402",
    "user": "https://github.com/dwbump"
}
```

```
sage: dg = DiGraph({0 : [1], 1 : [0]})
sage: dg.set_edge_label(0,1,5)
sage: dg.set_edge_label(1,0,9)
sage: dg.edges()
[(0, 1, 5), (1, 0, 9)]
sage: dg.outgoing_edges([1])
[(1, 0, 9)]
sage: dg.incoming_edges([1])
[(0, 1, None)]
sage: dg.outgoing_edges(0)
[(0, 1, 5)]
sage: dg.incoming_edges(0)
[(1, 0, None)]
```

As you can see, outgoing_edges remembers the labels but incoming_edges does not.

Assignee: @rlmill

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3402





---

archive/issue_comments_025102.json:
```json
{
    "body": "Changing assignee from tbd to @rlmill.",
    "created_at": "2008-06-12T05:54:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3402#issuecomment-25102",
    "user": "https://github.com/mwhansen"
}
```

Changing assignee from tbd to @rlmill.



---

archive/issue_comments_025103.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac3402-set_edge_label.patch](tarball://root/attachments/some-uuid/ticket3402/trac3402-set_edge_label.patch) by @rlmill created at 2008-06-12 19:19:09",
    "created_at": "2008-06-12T19:19:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3402#issuecomment-25103",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:2'></a>
Attachment [trac3402-set_edge_label.patch](tarball://root/attachments/some-uuid/ticket3402/trac3402-set_edge_label.patch) by @rlmill created at 2008-06-12 19:19:09



---

archive/issue_events_012223.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3402",
    "rename": {
        "from": "Digraph.incoming_edges forgets labels",
        "to": "[with patch, needs review] Digraph.incoming_edges forgets labels"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3402#event-12223"
}
```



---

archive/issue_comments_025104.json:
```json
{
    "body": "<a id='comment:3'></a>\nThe incoming edge iterator depends on self._backend._nxg which is a NetworkX XDigraph. This digraph has separate iterators for incoming and outgoing edges that depend on dictionaries self._backend._nxg.succ and self._backend._nxg.pred. Before the patch, self._backend._nxg.pred was not getting initialized.\n\nThe patch is obviously correct and I've also tested it.",
    "created_at": "2008-06-13T14:28:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3402#issuecomment-25104",
    "user": "https://github.com/dwbump"
}
```

<a id='comment:3'></a>
The incoming edge iterator depends on self._backend._nxg which is a NetworkX XDigraph. This digraph has separate iterators for incoming and outgoing edges that depend on dictionaries self._backend._nxg.succ and self._backend._nxg.pred. Before the patch, self._backend._nxg.pred was not getting initialized.

The patch is obviously correct and I've also tested it.



---

archive/issue_events_012224.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3402",
    "rename": {
        "from": "[with patch, needs review] Digraph.incoming_edges forgets labels",
        "to": "[with patch, with positive review] Digraph.incoming_edges forgets labels"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3402#event-12224"
}
```



---

archive/issue_events_012225.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-16T04:30:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3402",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3402#event-12225"
}
```



---

archive/issue_comments_025105.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.3.rc0",
    "created_at": "2008-06-16T04:30:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3402#issuecomment-25105",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.3.rc0



---

archive/issue_comments_025106.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-06-16T04:30:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3402#issuecomment-25106",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_012226.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-16T04:30:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3402",
    "milestone": "sage-3.0.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3402#event-12226"
}
```
