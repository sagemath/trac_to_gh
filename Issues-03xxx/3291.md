# Issue 3291: pbuild doesn't properly compile mwrank.so on some systems

archive/issues_003291.json:
```json
{
    "assignees": [
        "https://github.com/garyfurnish"
    ],
    "body": "On some systems pbuild seems to leave out wrap.o of mwrank.so, resulting in a undefined symbol error\n\nComponent: **pbuild**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3291_\n\n",
    "closed_at": "2008-05-24T00:27:43Z",
    "created_at": "2008-05-23T22:38:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "pbuild doesn't properly compile mwrank.so on some systems",
    "type": "issue",
    "updated_at": "2008-05-24T00:27:43Z",
    "url": "https://github.com/sagemath/sage/issues/3291",
    "user": "https://github.com/garyfurnish"
}
```
On some systems pbuild seems to leave out wrap.o of mwrank.so, resulting in a undefined symbol error

Component: **pbuild**

_Issue created by migration from https://trac.sagemath.org/ticket/3291_





---

archive/issue_events_035377.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-23T22:38:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "milestone_number": null,
    "milestone_title": "sage-3.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3291#event-35377"
}
```



---

archive/issue_events_035378.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-23T22:38:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "000080",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3291#event-35378"
}
```



---

archive/issue_events_035379.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-23T22:38:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3291#event-35379"
}
```



---

archive/issue_events_035380.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-23T22:38:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3291#event-35380"
}
```



---

archive/issue_events_035381.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-23T22:38:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "subject": "https://github.com/garyfurnish",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3291#event-35381"
}
```



---

archive/issue_events_035382.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-23T22:38:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3291#event-35382"
}
```



---

archive/issue_comments_017403.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_3291_extcode.patch.gz](https://github.com/sagemath/sage/files/ticket3291/trac_3291_extcode.patch.gz)**",
    "created_at": "2008-05-23T22:41:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3291#issuecomment-17403",
    "user": "https://github.com/garyfurnish"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_3291_extcode.patch.gz](https://github.com/sagemath/sage/files/ticket3291/trac_3291_extcode.patch.gz)**



---

archive/issue_events_035383.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2008-05-23T23:28:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3291#event-35383"
}
```



---

archive/issue_comments_017404.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe patch fixed the issue with build on Fedora 9, 32 bits, 2 cpu's\n\nJaap",
    "created_at": "2008-05-23T23:28:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3291#issuecomment-17404",
    "user": "https://github.com/jaapspies"
}
```

<div id="comment:2" align="right">comment:2</div>

The patch fixed the issue with build on Fedora 9, 32 bits, 2 cpu's

Jaap



---

archive/issue_events_035384.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-24T00:27:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3291#event-35384"
}
```



---

archive/issue_events_035385.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-24T00:27:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3291#event-35385"
}
```



---

archive/issue_comments_017405.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.0.2.rc3",
    "created_at": "2008-05-24T00:27:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3291",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3291#issuecomment-17405",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.0.2.rc3
