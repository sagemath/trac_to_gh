# Issue 3722: notebook -- public notebook server worksheets say "(running)" forever, but presumably they are timed out

archive/issues_003722.json:
```json
{
    "assignees": [],
    "body": "The fix will be to make it so that the running indicator gets changed when the worksheet process is killed by the server. \n\nAssignee: **boothby**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3722_\n\n",
    "closed_at": "2009-01-23T13:44:32Z",
    "created_at": "2008-07-25T10:26:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook -- public notebook server worksheets say \"(running)\" forever, but presumably they are timed out",
    "type": "issue",
    "updated_at": "2009-01-23T13:44:32Z",
    "url": "https://github.com/sagemath/sage/issues/3722",
    "user": "https://github.com/williamstein"
}
```
The fix will be to make it so that the running indicator gets changed when the worksheet process is killed by the server. 

Assignee: **boothby**

_Issue created by migration from https://trac.sagemath.org/ticket/3722_





---

archive/issue_events_035717.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-25T10:26:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3722",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3722#event-35717"
}
```



---

archive/issue_events_035718.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-25T10:26:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3722",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3722#event-35718"
}
```



---

archive/issue_events_035719.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-25T10:26:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3722",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3722#event-35719"
}
```



---

archive/issue_events_035720.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-23T13:44:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3722",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3722#event-35720"
}
```



---

archive/issue_comments_021346.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis is no longer an issue in Sage 3.2.3.",
    "created_at": "2009-01-23T13:44:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3722#issuecomment-21346",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'>Comment 1:</a>
This is no longer an issue in Sage 3.2.3.



---

archive/issue_events_035721.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-23T13:44:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3722",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3722#event-35721"
}
```
