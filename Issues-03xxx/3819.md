# Issue 3819: [with patch, positive review] Sage 3.1.alpha1: time_series.pyx numerical noise doctest failures

archive/issues_003819.json:
```json
{
    "body": "Reported by John Cremona:\n\n```\n*******************\nFile \"/home/john/sage-3.1.alpha1/tmp/time_series.py\", line 1507:\n    sage: finance.TimeSeries([z.hurst_exponent() for z in y]).mean()\nExpected:\n    0.57984822577934747\nGot:\n    0.57984822577934769\n**********************************************************************\nFile \"/home/john/sage-3.1.alpha1/tmp/time_series.py\", line 1515:\n    sage: finance.TimeSeries([z.hurst_exponent() for z in y]).mean()\nExpected:\n    0.2861023256237053\nGot:\n    0.28610232562370524\n**********************************************************************\n1 items had failures:\n   2 of  16 in __main__.example_46\n***Test Failed*** 2 failures.\nFor whitespace errors, see the file\n/home/john/sage-3.1.alpha1/tmp/.doctest_time_series.py\n         [9.8 s]\nexit code: 1024\n```\n\nAssignee: mabshoff\n\nCC:  @JohnCremona\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3819\n\n",
    "closed_at": "2008-08-13T00:20:25Z",
    "created_at": "2008-08-12T15:33:51Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1",
    "title": "[with patch, positive review] Sage 3.1.alpha1: time_series.pyx numerical noise doctest failures",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3819",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Reported by John Cremona:

```
*******************
File "/home/john/sage-3.1.alpha1/tmp/time_series.py", line 1507:
    sage: finance.TimeSeries([z.hurst_exponent() for z in y]).mean()
Expected:
    0.57984822577934747
Got:
    0.57984822577934769
**********************************************************************
File "/home/john/sage-3.1.alpha1/tmp/time_series.py", line 1515:
    sage: finance.TimeSeries([z.hurst_exponent() for z in y]).mean()
Expected:
    0.2861023256237053
Got:
    0.28610232562370524
**********************************************************************
1 items had failures:
   2 of  16 in __main__.example_46
***Test Failed*** 2 failures.
For whitespace errors, see the file
/home/john/sage-3.1.alpha1/tmp/.doctest_time_series.py
         [9.8 s]
exit code: 1024
```

Assignee: mabshoff

CC:  @JohnCremona

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3819





---

archive/issue_comments_028600.json:
```json
{
    "body": "<a id='comment:1'></a>\nJohn,\n\ncan you review this?\n\nCheers,\n\nMichael",
    "created_at": "2008-08-12T17:13:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3819#issuecomment-28600",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
John,

can you review this?

Cheers,

Michael



---

archive/issue_events_014136.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3819",
    "rename": {
        "from": "Sage 3.1.alpha1> time_series.pyx numerical noise doctest failures",
        "to": "[with patch, needs review] Sage 3.1.alpha1: time_series.pyx numerical noise doctest failures"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3819#event-14136"
}
```



---

archive/issue_comments_028601.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-08-12T17:13:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3819#issuecomment-28601",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_028602.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac_3819.patch](tarball://root/attachments/some-uuid/ticket3819/trac_3819.patch) by mabshoff created at 2008-08-12 17:23:00\n\nOops, adding John to the CC would help a lot when asking for his review :)\n\nCheers,\n\nMichael",
    "created_at": "2008-08-12T17:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3819#issuecomment-28602",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Attachment [trac_3819.patch](tarball://root/attachments/some-uuid/ticket3819/trac_3819.patch) by mabshoff created at 2008-08-12 17:23:00

Oops, adding John to the CC would help a lot when asking for his review :)

Cheers,

Michael



---

archive/issue_events_014137.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3819",
    "rename": {
        "from": "[with patch, needs review] Sage 3.1.alpha1: time_series.pyx numerical noise doctest failures",
        "to": "[with patch, positive review] Sage 3.1.alpha1: time_series.pyx numerical noise doctest failures"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3819#event-14137"
}
```



---

archive/issue_comments_028603.json:
```json
{
    "body": "<a id='comment:3'></a>\nI agree with this patch.",
    "created_at": "2008-08-13T00:18:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3819#issuecomment-28603",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
I agree with this patch.



---

archive/issue_events_014138.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-13T00:20:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3819",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3819#event-14138"
}
```



---

archive/issue_comments_028604.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-13T00:20:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3819#issuecomment-28604",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_028605.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.alpha2",
    "created_at": "2008-08-13T00:20:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3819#issuecomment-28605",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.alpha2
