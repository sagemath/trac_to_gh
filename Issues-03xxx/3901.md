# Issue 3901: automated doctesting elementary number theory book

archive/issues_003901.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "\n```\n10:49 < cwitty> wstein: You should put a file in Sage with all your code samples from your number theory book as \n                doctests.\n10:49 < cwitty> Then we can't accidentally break your book.\n10:49 < wstein> cwitty -- great idea.\n10:49 < wstein> i'll make a ticket.\n```\n\nSame comments about my modular forms book.\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3901_\n\n",
    "closed_at": "2008-10-26T14:13:20Z",
    "created_at": "2008-08-19T17:53:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "automated doctesting elementary number theory book",
    "type": "issue",
    "updated_at": "2008-10-26T14:13:20Z",
    "url": "https://github.com/sagemath/sage/issues/3901",
    "user": "https://github.com/williamstein"
}
```

```
10:49 < cwitty> wstein: You should put a file in Sage with all your code samples from your number theory book as 
                doctests.
10:49 < cwitty> Then we can't accidentally break your book.
10:49 < wstein> cwitty -- great idea.
10:49 < wstein> i'll make a ticket.
```

Same comments about my modular forms book.

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/3901_





---

archive/issue_events_043398.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-19T17:53:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "milestone_number": null,
    "milestone_title": "sage-3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43398"
}
```



---

archive/issue_events_043399.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-19T17:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43399"
}
```



---

archive/issue_events_043400.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-19T17:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43400"
}
```



---

archive/issue_events_043401.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-19T17:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43401"
}
```



---

archive/issue_events_043402.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-19T17:53:02Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43402"
}
```



---

archive/issue_events_043403.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-19T17:53:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43403"
}
```



---

archive/issue_events_043404.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-19T17:53:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43404"
}
```



---

archive/issue_events_043405.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-10-24T02:58:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43405"
}
```



---

archive/issue_comments_021757.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe attached patch creates two new files in the sage/tests/ directory.  \n\n* book_stein_modform.py -- all examples in my AMS Modular Forms book.\n\n* book_stein_ent.py -- all examples in my new Springer Elementary Number Theory Book\n\nThis adds about 736 new lines of input examples to the Sage doctest suite, and adds about 45 seconds more time to doctesting.",
    "created_at": "2008-10-24T02:58:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3901#issuecomment-21757",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

The attached patch creates two new files in the sage/tests/ directory.  

* book_stein_modform.py -- all examples in my AMS Modular Forms book.

* book_stein_ent.py -- all examples in my new Springer Elementary Number Theory Book

This adds about 736 new lines of input examples to the Sage doctest suite, and adds about 45 seconds more time to doctesting.



---

archive/issue_comments_021758.json:
```json
{
    "body": "Attachment: **[sage-3901.patch.gz](https://github.com/sagemath/sage/files/ticket3901/sage-3901.patch.gz)**",
    "created_at": "2008-10-24T02:59:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3901#issuecomment-21758",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[sage-3901.patch.gz](https://github.com/sagemath/sage/files/ticket3901/sage-3901.patch.gz)**



---

archive/issue_events_043406.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-10-24T03:19:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43406"
}
```



---

archive/issue_events_043407.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-10-24T03:19:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43407"
}
```



---

archive/issue_comments_021759.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nTests pass for me.  It's good to get these in.",
    "created_at": "2008-10-24T03:19:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3901#issuecomment-21759",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Tests pass for me.  It's good to get these in.



---

archive/issue_comments_021760.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nNote that coverage of those files is \"bad\" since we cannot deal with the autogenerated files:\n\n```\nmabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.2.alpha1$ ./sage -coverageall devel/sage/sage/tests/\nbenchmark.py: 76% (87 of 114)\nbook_stein_ent.py: 0% (0 of 119)\nbook_stein_modform.py: 0% (0 of 51)\n\nOverall weighted coverage score:  30.5%\nTotal number of functions:  284\n```\n\nCheers,\n\nMichael",
    "created_at": "2008-10-26T14:07:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3901#issuecomment-21760",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Note that coverage of those files is "bad" since we cannot deal with the autogenerated files:

```
mabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.2.alpha1$ ./sage -coverageall devel/sage/sage/tests/
benchmark.py: 76% (87 of 114)
book_stein_ent.py: 0% (0 of 119)
book_stein_modform.py: 0% (0 of 51)

Overall weighted coverage score:  30.5%
Total number of functions:  284
```

Cheers,

Michael



---

archive/issue_comments_021761.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in Sage 3.2.alpha1",
    "created_at": "2008-10-26T14:13:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3901#issuecomment-21761",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in Sage 3.2.alpha1



---

archive/issue_events_043408.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-26T14:13:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43408"
}
```



---

archive/issue_events_043409.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-26T14:13:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3901",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3901#event-43409"
}
```
