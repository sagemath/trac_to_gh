# Issue 3177: [with spkg, with positive review] fix 64 bit OSX build support for python

archive/issues_003177.json:
```json
{
    "body": "spkg coming up\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3177\n\n",
    "closed_at": "2008-05-19T03:16:13Z",
    "created_at": "2008-05-13T11:07:35Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.2",
    "title": "[with spkg, with positive review] fix 64 bit OSX build support for python",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3177",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
spkg coming up

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3177





---

archive/issue_comments_023089.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-05-13T11:17:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3177#issuecomment-23089",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_023090.json:
```json
{
    "body": "Attachment [python-2.5.2-spkg-install-64bit-osx.patch](tarball://root/attachments/some-uuid/ticket3177/python-2.5.2-spkg-install-64bit-osx.patch) by mabshoff created at 2008-05-19 02:56:10",
    "created_at": "2008-05-19T02:56:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3177#issuecomment-23090",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [python-2.5.2-spkg-install-64bit-osx.patch](tarball://root/attachments/some-uuid/ticket3177/python-2.5.2-spkg-install-64bit-osx.patch) by mabshoff created at 2008-05-19 02:56:10



---

archive/issue_events_011303.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3177",
    "rename": {
        "from": "fix 64 bit OSX build support for python",
        "to": "[with spkg, needs review] fix 64 bit OSX build support for python"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3177#event-11303"
}
```



---

archive/issue_comments_023091.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/python-2.5.2.p0.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-05-19T03:02:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3177#issuecomment-23091",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
The spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/python-2.5.2.p0.spkg

Cheers,

Michael



---

archive/issue_events_011304.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3177",
    "rename": {
        "from": "[with spkg, needs review] fix 64 bit OSX build support for python",
        "to": "[with spkg, with positive review] fix 64 bit OSX build support for python"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3177#event-11304"
}
```



---

archive/issue_comments_023092.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.2.alpha1",
    "created_at": "2008-05-19T03:16:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3177#issuecomment-23092",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.2.alpha1



---

archive/issue_comments_023093.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-19T03:16:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3177#issuecomment-23093",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_011305.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-19T03:16:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3177",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3177#event-11305"
}
```
