# Issue 3054: [with patch, positive review] copying a graph doesn't copy _pos or _boundary

archive/issues_003054.json:
```json
{
    "body": "This means that changing the position of nodes in the copy will affect the original.\n\nAssignee: @rlmill\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3054\n\n",
    "closed_at": "2008-04-30T02:17:36Z",
    "created_at": "2008-04-29T20:40:10Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.1",
    "title": "[with patch, positive review] copying a graph doesn't copy _pos or _boundary",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3054",
    "user": "https://github.com/jasongrout"
}
```
This means that changing the position of nodes in the copy will affect the original.

Assignee: @rlmill

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3054





---

archive/issue_events_010816.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3054",
    "rename": {
        "from": "copying a graph doesn't copy _pos or _boundary",
        "to": "[with patch, needs review] copying a graph doesn't copy _pos or _boundary"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3054#event-10816"
}
```



---

archive/issue_comments_022136.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac-3054-graph-copy.patch](tarball://root/attachments/some-uuid/ticket3054/trac-3054-graph-copy.patch) by @jasongrout created at 2008-04-29 21:09:03\n\nWith #3054 and #3055 applied, doctests pass in the graphs/ directory.",
    "created_at": "2008-04-29T21:09:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3054#issuecomment-22136",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
Attachment [trac-3054-graph-copy.patch](tarball://root/attachments/some-uuid/ticket3054/trac-3054-graph-copy.patch) by @jasongrout created at 2008-04-29 21:09:03

With #3054 and #3055 applied, doctests pass in the graphs/ directory.



---

archive/issue_events_010817.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3054",
    "rename": {
        "from": "[with patch, needs review] copying a graph doesn't copy _pos or _boundary",
        "to": "[with patch, positive review pending tests] copying a graph doesn't copy _pos or _boundary"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3054#event-10817"
}
```



---

archive/issue_comments_022137.json:
```json
{
    "body": "<a id='comment:2'></a>\nHave not run doctests, but I support this fix.",
    "created_at": "2008-04-29T21:43:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3054#issuecomment-22137",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:2'></a>
Have not run doctests, but I support this fix.



---

archive/issue_events_010818.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3054",
    "rename": {
        "from": "[with patch, positive review pending tests] copying a graph doesn't copy _pos or _boundary",
        "to": "[with patch, positive review] copying a graph doesn't copy _pos or _boundary"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3054#event-10818"
}
```



---

archive/issue_comments_022138.json:
```json
{
    "body": "<a id='comment:3'></a>\n#3054 and #3055 applied to my current merge tree doctest clean. So I am considering this a positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-30T02:16:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3054#issuecomment-22138",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
#3054 and #3055 applied to my current merge tree doctest clean. So I am considering this a positive review.

Cheers,

Michael



---

archive/issue_comments_022139.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-30T02:17:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3054#issuecomment-22139",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_022140.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.1.alpha1",
    "created_at": "2008-04-30T02:17:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3054#issuecomment-22140",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.1.alpha1



---

archive/issue_events_010819.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-30T02:17:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3054",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3054#event-10819"
}
```
