# Issue 3054: copying a graph doesn't copy _pos or _boundary

archive/issues_003054.json:
```json
{
    "assignees": [
        "https://github.com/rlmill"
    ],
    "body": "This means that changing the position of nodes in the copy will affect the original.\n\nComponent: **graph theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3054_\n\n",
    "closed_at": "2008-04-30T02:17:36Z",
    "created_at": "2008-04-29T20:40:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "copying a graph doesn't copy _pos or _boundary",
    "type": "issue",
    "updated_at": "2008-04-30T02:17:36Z",
    "url": "https://github.com/sagemath/sage/issues/3054",
    "user": "https://github.com/jasongrout"
}
```
This means that changing the position of nodes in the copy will affect the original.

Component: **graph theory**

_Issue created by migration from https://trac.sagemath.org/ticket/3054_





---

archive/issue_events_032540.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-29T20:40:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "milestone_number": null,
    "milestone_title": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32540"
}
```



---

archive/issue_events_032541.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-29T20:40:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32541"
}
```



---

archive/issue_events_032542.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-29T20:40:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32542"
}
```



---

archive/issue_events_032543.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-29T20:40:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32543"
}
```



---

archive/issue_events_032544.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-04-29T20:40:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32544"
}
```



---

archive/issue_events_032545.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-29T21:09:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32545"
}
```



---

archive/issue_comments_016089.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac-3054-graph-copy.patch.gz](https://github.com/sagemath/sage/files/ticket3054/trac-3054-graph-copy.patch.gz)**\n\nWith #3054 and #3055 applied, doctests pass in the graphs/ directory.",
    "created_at": "2008-04-29T21:09:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3054#issuecomment-16089",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac-3054-graph-copy.patch.gz](https://github.com/sagemath/sage/files/ticket3054/trac-3054-graph-copy.patch.gz)**

With #3054 and #3055 applied, doctests pass in the graphs/ directory.



---

archive/issue_events_032546.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-04-29T21:43:50Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "title_is": "[positive review pending tests] copying a graph doesn't copy _pos or _boundary",
    "title_was": "copying a graph doesn't copy _pos or _boundary",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32546"
}
```



---

archive/issue_comments_016090.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nHave not run doctests, but I support this fix.",
    "created_at": "2008-04-29T21:43:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3054#issuecomment-16090",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:2" align="right">comment:2</div>

Have not run doctests, but I support this fix.



---

archive/issue_events_032547.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-30T02:16:42Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "title_is": "copying a graph doesn't copy _pos or _boundary",
    "title_was": "[positive review pending tests] copying a graph doesn't copy _pos or _boundary",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32547"
}
```



---

archive/issue_events_032548.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-30T02:16:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32548"
}
```



---

archive/issue_events_032549.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-30T02:16:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32549"
}
```



---

archive/issue_comments_016091.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\n#3054 and #3055 applied to my current merge tree doctest clean. So I am considering this a positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-30T02:16:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3054#issuecomment-16091",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

#3054 and #3055 applied to my current merge tree doctest clean. So I am considering this a positive review.

Cheers,

Michael



---

archive/issue_comments_016092.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.0.1.alpha1",
    "created_at": "2008-04-30T02:17:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3054#issuecomment-16092",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.0.1.alpha1



---

archive/issue_events_032550.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-30T02:17:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32550"
}
```



---

archive/issue_events_032551.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-30T02:17:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3054",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3054#event-32551"
}
```
