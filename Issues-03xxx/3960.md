# Issue 3960: "edit this" on published worksheets doesn't work anymore in 3.1.1

archive/issues_003960.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "Way to reproduce:\n\n- start notebook\n- create worksheet\n- publish worksheet\n- go to \"published worksheets\"\n- click published worksheet\n\nresult:\n\n\"edit this\" or \"edit a copy\" link lead to another \"published\" version, not a normal worksheet\n\nexpected result:\n\nan editable worksheet\n\nNOTE: This worked properly in 3.0.6\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3960_\n\n",
    "closed_at": "2008-09-04T02:02:46Z",
    "created_at": "2008-08-26T18:27:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "\"edit this\" on published worksheets doesn't work anymore in 3.1.1",
    "type": "issue",
    "updated_at": "2008-09-04T02:02:46Z",
    "url": "https://github.com/sagemath/sage/issues/3960",
    "user": "https://github.com/nbruin"
}
```
Way to reproduce:

- start notebook
- create worksheet
- publish worksheet
- go to "published worksheets"
- click published worksheet

result:

"edit this" or "edit a copy" link lead to another "published" version, not a normal worksheet

expected result:

an editable worksheet

NOTE: This worked properly in 3.0.6

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/3960_





---

archive/issue_events_044177.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2008-08-26T18:27:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "milestone_number": null,
    "milestone_title": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44177"
}
```



---

archive/issue_events_044178.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2008-08-26T18:27:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44178"
}
```



---

archive/issue_events_044179.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2008-08-26T18:27:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44179"
}
```



---

archive/issue_events_044180.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2008-08-26T18:27:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44180"
}
```



---

archive/issue_events_044181.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-08-26T18:27:26Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "subject": "https://github.com/nbruin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44181"
}
```



---

archive/issue_comments_022203.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is definitely a blocker for 3.1.2.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-27T07:56:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3960#issuecomment-22203",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

This is definitely a blocker for 3.1.2.

Cheers,

Michael



---

archive/issue_events_044182.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-27T07:56:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44182"
}
```



---

archive/issue_events_044183.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-27T07:56:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44183"
}
```



---

archive/issue_comments_022204.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI believe that this is because somewhere the wrong username is taken into account. Here is an example: publish a worksheet, and navigate to it. The link should say \"edit this\", not \"edit a copy\".\n\nAlternatively, I found that the username passed to worksheet_html() in notebook.py is \"pub\", but was unable to find the source of the problem.",
    "created_at": "2008-08-27T16:21:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3960#issuecomment-22204",
    "user": "https://github.com/itolkov"
}
```

<div id="comment:2" align="right">comment:2</div>

I believe that this is because somewhere the wrong username is taken into account. Here is an example: publish a worksheet, and navigate to it. The link should say "edit this", not "edit a copy".

Alternatively, I found that the username passed to worksheet_html() in notebook.py is "pub", but was unable to find the source of the problem.



---

archive/issue_events_044184.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-09-03T00:53:00Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44184"
}
```



---

archive/issue_events_044185.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-09-03T00:53:00Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44185"
}
```



---

archive/issue_comments_022205.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[trac_3960.patch.gz](https://github.com/sagemath/sage/files/ticket3960/trac_3960.patch.gz)**",
    "created_at": "2008-09-04T01:35:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3960#issuecomment-22205",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[trac_3960.patch.gz](https://github.com/sagemath/sage/files/ticket3960/trac_3960.patch.gz)**



---

archive/issue_events_044186.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-09-04T01:35:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44186"
}
```



---

archive/issue_events_044187.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-04T01:43:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44187"
}
```



---

archive/issue_events_044188.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-04T01:43:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44188"
}
```



---

archive/issue_comments_022206.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThis patch fixes the issue and Mike walked through the code with me. Positive review. Mike is also writing a Selenium doctest for this bug, so we will catch it in the future.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-04T01:43:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3960#issuecomment-22206",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

This patch fixes the issue and Mike walked through the code with me. Positive review. Mike is also writing a Selenium doctest for this bug, so we will catch it in the future.

Cheers,

Michael



---

archive/issue_events_044189.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-04T02:02:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44189"
}
```



---

archive/issue_events_044190.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-04T02:02:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3960#event-44190"
}
```



---

archive/issue_comments_022207.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.1.2.rc0",
    "created_at": "2008-09-04T02:02:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3960#issuecomment-22207",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.1.2.rc0
