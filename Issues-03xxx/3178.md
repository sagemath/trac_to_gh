# Issue 3178: add 64 bit OSX build support to freetype

archive/issues_003178.json:
```json
{
    "body": "spkg coming up.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3178\n\n",
    "closed_at": "2008-05-19T03:16:06Z",
    "created_at": "2008-05-13T11:17:31Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.2",
    "title": "add 64 bit OSX build support to freetype",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3178",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
spkg coming up.

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3178





---

archive/attachments_003098.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "freetype-2.3.5-spkg-install-64bit-osx.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket3178/freetype-2.3.5-spkg-install-64bit-osx.patch",
    "created_at": "2008-05-19T03:08:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```



---

archive/issue_comments_017948.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe updated spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/freetype-2.3.5.p0.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-05-19T03:08:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3178#issuecomment-17948",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
The updated spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/freetype-2.3.5.p0.spkg

Cheers,

Michael



---

archive/issue_events_017308.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-19T03:08:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3178#event-17308"
}
```



---

archive/issue_events_017309.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-19T03:14:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3178#event-17309"
}
```



---

archive/issue_events_017310.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-19T03:14:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3178#event-17310"
}
```



---

archive/issue_events_017311.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-19T03:16:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3178#event-17311"
}
```



---

archive/issue_events_017312.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-19T03:16:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3178#event-17312"
}
```



---

archive/issue_comments_017949.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.2.alpha1",
    "created_at": "2008-05-19T03:16:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3178#issuecomment-17949",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.2.alpha1
