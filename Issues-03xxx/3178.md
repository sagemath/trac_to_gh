# Issue 3178: [with spkg, with positive review] add 64 bit OSX build support to freetype

archive/issues_003178.json:
```json
{
    "body": "spkg coming up.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3178\n\n",
    "closed_at": "2008-05-19T03:16:06Z",
    "created_at": "2008-05-13T11:17:31Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.2",
    "title": "[with spkg, with positive review] add 64 bit OSX build support to freetype",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3178",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
spkg coming up.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3178





---

archive/issue_comments_023094.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-05-13T11:17:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3178#issuecomment-23094",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_023095.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [freetype-2.3.5-spkg-install-64bit-osx.patch](tarball://root/attachments/some-uuid/ticket3178/freetype-2.3.5-spkg-install-64bit-osx.patch) by mabshoff created at 2008-05-19 03:08:37\n\nThe updated spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/freetype-2.3.5.p0.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-05-19T03:08:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3178#issuecomment-23095",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Attachment [freetype-2.3.5-spkg-install-64bit-osx.patch](tarball://root/attachments/some-uuid/ticket3178/freetype-2.3.5-spkg-install-64bit-osx.patch) by mabshoff created at 2008-05-19 03:08:37

The updated spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.2/alpha1/freetype-2.3.5.p0.spkg

Cheers,

Michael



---

archive/issue_events_011306.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "rename": {
        "from": "add 64 bit OSX build support to freetype",
        "to": "[with spkg, needs review] add 64 bit OSX build support to freetype"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3178#event-11306"
}
```



---

archive/issue_events_011307.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "rename": {
        "from": "[with spkg, needs review] add 64 bit OSX build support to freetype",
        "to": "[with spkg, with positive review] add 64 bit OSX build support to freetype"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3178#event-11307"
}
```



---

archive/issue_comments_023096.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-19T03:16:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3178#issuecomment-23096",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_011308.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-19T03:16:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3178#event-11308"
}
```



---

archive/issue_comments_023097.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.2.alpha1",
    "created_at": "2008-05-19T03:16:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3178#issuecomment-23097",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.2.alpha1
