# Issue 3419: 100% coverage for sage.coding.binary_code

archive/issues_003419.json:
```json
{
    "assignees": [],
    "body": "\n```\n$ ./sage -coverage devel/sage/sage/coding/binary_code.pyx\n----------------------------------------------------------------------\ndevel/sage/sage/coding/binary_code.pyx\nSCORE devel/sage/sage/coding/binary_code.pyx: 100% (41 of 41)\n\nPossibly wrong (function name doesn't occur in doctests):\n * int put_in_std_form(self)\n\n----------------------------------------------------------------------\n```\n\nThere seems to be a little bug in sage-coverage related to cpdef functions: the function name definitely occurs in the doctest. Has #1795 gone stale?!\n\nAssignee: **@rlmill**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3419_\n\n",
    "closed_at": "2008-06-23T03:02:35Z",
    "created_at": "2008-06-13T18:41:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "100% coverage for sage.coding.binary_code",
    "type": "issue",
    "updated_at": "2015-09-06T15:54:48Z",
    "url": "https://github.com/sagemath/sage/issues/3419",
    "user": "https://github.com/rlmill"
}
```

```
$ ./sage -coverage devel/sage/sage/coding/binary_code.pyx
----------------------------------------------------------------------
devel/sage/sage/coding/binary_code.pyx
SCORE devel/sage/sage/coding/binary_code.pyx: 100% (41 of 41)

Possibly wrong (function name doesn't occur in doctests):
 * int put_in_std_form(self)

----------------------------------------------------------------------
```

There seems to be a little bug in sage-coverage related to cpdef functions: the function name definitely occurs in the doctest. Has #1795 gone stale?!

Assignee: **@rlmill**

_Issue created by migration from https://trac.sagemath.org/ticket/3419_





---

archive/issue_events_032210.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-13T18:41:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "milestone_number": null,
    "milestone_title": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32210"
}
```



---

archive/issue_events_032211.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-13T18:41:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
    "label_color": "0000ff",
    "label_name": "component: coding theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32211"
}
```



---

archive/issue_events_032212.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-13T18:41:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32212"
}
```



---

archive/issue_events_032213.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-13T18:41:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32213"
}
```



---

archive/issue_events_032214.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-13T18:41:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32214"
}
```



---

archive/issue_comments_019371.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac3419-100_binary_code.patch.gz](https://github.com/sagemath/sage/files/ticket3419/trac3419-100_binary_code.patch.gz)**\n\nThe cpdef patch ought to get merged this week, I am not sure what the current merge status is.\n\nCheers,\n\nMichael",
    "created_at": "2008-06-13T18:47:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3419#issuecomment-19371",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac3419-100_binary_code.patch.gz](https://github.com/sagemath/sage/files/ticket3419/trac3419-100_binary_code.patch.gz)**

The cpdef patch ought to get merged this week, I am not sure what the current merge status is.

Cheers,

Michael



---

archive/issue_comments_019372.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPatch looks good.",
    "created_at": "2008-06-15T19:03:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3419#issuecomment-19372",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'>Comment 2:</a>
Patch looks good.



---

archive/issue_events_032215.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-06-15T19:03:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32215"
}
```



---

archive/issue_comments_019373.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nApply after #3471.",
    "created_at": "2008-06-22T20:54:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3419#issuecomment-19373",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:3'>Comment 3:</a>
Apply after #3471.



---

archive/issue_events_032216.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-22T20:54:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32216"
}
```



---

archive/issue_events_032217.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-22T20:54:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32217"
}
```



---

archive/issue_events_032218.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-23T02:56:57Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32218"
}
```



---

archive/issue_events_032219.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-23T02:56:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "milestone_number": null,
    "milestone_title": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32219"
}
```



---

archive/issue_events_032220.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-23T03:02:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32220"
}
```



---

archive/issue_events_032221.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-23T03:02:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3419#event-32221"
}
```



---

archive/issue_comments_019374.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nMerged in Sage 3.0.4.alpha0",
    "created_at": "2008-06-23T03:02:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3419#issuecomment-19374",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:5'>Comment 5:</a>
Merged in Sage 3.0.4.alpha0



---

archive/issue_comments_019375.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,7 +6,7 @@\n SCORE devel/sage/sage/coding/binary_code.pyx: 100% (41 of 41)\n \n Possibly wrong (function name doesn't occur in doctests):\n-\t * int put_in_std_form(self)\n+ * int put_in_std_form(self)\n \n ----------------------------------------------------------------------\n ```\n``````\n",
    "created_at": "2015-09-06T15:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3419#issuecomment-19375",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,7 +6,7 @@
 SCORE devel/sage/sage/coding/binary_code.pyx: 100% (41 of 41)
 
 Possibly wrong (function name doesn't occur in doctests):
-	 * int put_in_std_form(self)
+ * int put_in_std_form(self)
 
 ----------------------------------------------------------------------
 ```
``````

