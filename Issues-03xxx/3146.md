# Issue 3146: (latex(a\v), a) gives an error about calling something with too many arguments.

archive/issues_003146.json:
```json
{
    "body": "```\na=matrix(QQ,3,3,range(9))\nv=matrix(QQ,3,1,range(3))\n(latex(a\\v), a)\n```\n\ngives an error.\n\nI think it has to do with the parsing of latex(a\\v); it seems to try doing \"(latex(a._backslash_(v), a)\" (note the missing parenthesis in the call to latex.\n\nYou see this parsing in the error from:\n\n```\n@interact\ndef _(a=matrix(QQ,3,3,range(9)), v=matrix(QQ,3,1,range(3))):\n    html('$$%s %s = %s$$'%(latex(a), latex(a\\v), latex(v)))\n```\n\nwith the patch from #3121\n\n\n\nAssignee: @williamstein\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/3146\n\n",
    "closed_at": "2009-06-04T22:57:37Z",
    "created_at": "2008-05-09T20:13:03Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "(latex(a\\v), a) gives an error about calling something with too many arguments.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3146",
    "user": "https://github.com/jasongrout"
}
```
```
a=matrix(QQ,3,3,range(9))
v=matrix(QQ,3,1,range(3))
(latex(a\v), a)
```

gives an error.

I think it has to do with the parsing of latex(a\v); it seems to try doing "(latex(a._backslash_(v), a)" (note the missing parenthesis in the call to latex.

You see this parsing in the error from:

```
@interact
def _(a=matrix(QQ,3,3,range(9)), v=matrix(QQ,3,1,range(3))):
    html('$$%s %s = %s$$'%(latex(a), latex(a\v), latex(v)))
```

with the patch from #3121



Assignee: @williamstein

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/3146





---

archive/issue_events_007108.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T22:57:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3146",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3146#event-7108"
}
```



---

archive/issue_comments_022349.json:
```json
{
    "body": "<a id='comment:1'></a>This now works (most likely with the change to how the backslash operator is implemented)\n\n```\n----------------------------------------------------------------------\n----------------------------------------------------------------------\nsage: a=matrix(QQ,3,3,range(9))\nsage: v=matrix(QQ,3,1,range(3))\nsage: (latex(a\\v), a)\n| Sage Version 4.0.1.rc1, Release Date: 2009-06-04                   |\n| Type notebook() for the GUI, and license() for information.        |\n(\\left(\\begin{array}{r}\n\\frac{1}{3} \\\\\n0 \\\\\n0\n\\end{array}\\right),\n [0 1 2]\n[3 4 5]\n[6 7 8])\n```",
    "created_at": "2009-06-04T22:57:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3146#issuecomment-22349",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>This now works (most likely with the change to how the backslash operator is implemented)

```
----------------------------------------------------------------------
----------------------------------------------------------------------
sage: a=matrix(QQ,3,3,range(9))
sage: v=matrix(QQ,3,1,range(3))
sage: (latex(a\v), a)
| Sage Version 4.0.1.rc1, Release Date: 2009-06-04                   |
| Type notebook() for the GUI, and license() for information.        |
(\left(\begin{array}{r}
\frac{1}{3} \\
0 \\
0
\end{array}\right),
 [0 1 2]
[3 4 5]
[6 7 8])
```



---

archive/issue_events_007109.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T22:57:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3146",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3146#event-7109"
}
```



---

archive/issue_comments_022350.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2009-06-04T22:57:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3146#issuecomment-22350",
    "user": "https://github.com/mwhansen"
}
```

Resolution: invalid
