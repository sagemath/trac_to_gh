# Issue 3193: [with spkg, with positive review] fix 64 bit OSX build support for twisted

archive/issues_003193.json:
```json
{
    "body": "This is slightly trickier - see ToDo in http://wiki.sagemath.org/osx64 - but spkg is coming up.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3193\n\n",
    "closed_at": "2008-05-28T13:19:19Z",
    "created_at": "2008-05-13T14:51:37Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.3",
    "title": "[with spkg, with positive review] fix 64 bit OSX build support for twisted",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3193",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
This is slightly trickier - see ToDo in http://wiki.sagemath.org/osx64 - but spkg is coming up.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3193





---

archive/issue_comments_023166.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-05-13T14:51:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3193",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3193#issuecomment-23166",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_023167.json:
```json
{
    "body": "Attachment [twisted-8.0.1-64bit-osx-support.patch](tarball://root/attachments/some-uuid/ticket3193/twisted-8.0.1-64bit-osx-support.patch) by mabshoff created at 2008-05-28 09:55:34",
    "created_at": "2008-05-28T09:55:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3193",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3193#issuecomment-23167",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [twisted-8.0.1-64bit-osx-support.patch](tarball://root/attachments/some-uuid/ticket3193/twisted-8.0.1-64bit-osx-support.patch) by mabshoff created at 2008-05-28 09:55:34



---

archive/issue_comments_023168.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe spkg at \n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/twisted-8.0.1.p0.spkg\n\nprovides 64 bit OSX build support. Build tested on Linux and 32 & 64 bit OSX. It is required to merge the updated python.spkg from #3318 for 64 bit OSX for this spkg to work.\n\nCheers,\n\nMichael",
    "created_at": "2008-05-28T10:02:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3193",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3193#issuecomment-23168",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
The spkg at 

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/twisted-8.0.1.p0.spkg

provides 64 bit OSX build support. Build tested on Linux and 32 & 64 bit OSX. It is required to merge the updated python.spkg from #3318 for 64 bit OSX for this spkg to work.

Cheers,

Michael



---

archive/issue_events_011354.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3193",
    "rename": {
        "from": "fix 64 bit OSX build support for twisted",
        "to": "[with spkg, needs review] fix 64 bit OSX build support for twisted"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3193#event-11354"
}
```



---

archive/issue_events_011355.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3193",
    "rename": {
        "from": "[with spkg, needs review] fix 64 bit OSX build support for twisted",
        "to": "[with spkg, with positive review] fix 64 bit OSX build support for twisted"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3193#event-11355"
}
```



---

archive/issue_comments_023169.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-28T13:19:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3193",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3193#issuecomment-23169",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_023170.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.3.alpha0",
    "created_at": "2008-05-28T13:19:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3193",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3193#issuecomment-23170",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.3.alpha0



---

archive/issue_events_011356.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-28T13:19:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3193",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3193#event-11356"
}
```
