# Issue 3563: [with patch, positive review] sage/misc/cython.py: make "def atlas()" deal with the Accelerate Framework on OSX

archive/issues_003563.json:
```json
{
    "body": "The review of #3530 exposed a bug hidden in sage/misc/cython.py: \"def atlas()\" would not return anything useful on OSX since we need to link against the Accelerate Framework.\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3563\n\n",
    "closed_at": "2008-09-15T12:22:50Z",
    "created_at": "2008-07-06T12:04:15Z",
    "labels": [
        "component: build",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] sage/misc/cython.py: make \"def atlas()\" deal with the Accelerate Framework on OSX",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3563",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
The review of #3530 exposed a bug hidden in sage/misc/cython.py: "def atlas()" would not return anything useful on OSX since we need to link against the Accelerate Framework.

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3563





---

archive/issue_events_012993.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-14T11:13:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "milestone": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3563#event-12993"
}
```



---

archive/issue_comments_026482.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-09-14T11:13:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3563#issuecomment-26482",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_026483.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis issue actually breaks Cython on OSX 10.4 since the linker fails after it complains about a missing ATLAS.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-14T11:13:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3563#issuecomment-26483",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
This issue actually breaks Cython on OSX 10.4 since the linker fails after it complains about a missing ATLAS.

Cheers,

Michael



---

archive/issue_comments_026484.json:
```json
{
    "body": "Attachment [trac_3563.2.patch](tarball://root/attachments/some-uuid/ticket3563/trac_3563.2.patch) by mabshoff created at 2008-09-15 12:11:06",
    "created_at": "2008-09-15T12:11:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3563#issuecomment-26484",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [trac_3563.2.patch](tarball://root/attachments/some-uuid/ticket3563/trac_3563.2.patch) by mabshoff created at 2008-09-15 12:11:06



---

archive/issue_events_012994.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "rename": {
        "from": "sage/misc/cython.py: make \"def atlas()\" deal with the Accelerate Framework on OSX",
        "to": "[with patch, needs review] sage/misc/cython.py: make \"def atlas()\" deal with the Accelerate Framework on OSX"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3563#event-12994"
}
```



---

archive/issue_events_012995.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "rename": {
        "from": "[with patch, needs review] sage/misc/cython.py: make \"def atlas()\" deal with the Accelerate Framework on OSX",
        "to": "[with patch, positive review] sage/misc/cython.py: make \"def atlas()\" deal with the Accelerate Framework on OSX"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3563#event-12995"
}
```



---

archive/issue_comments_026485.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good to me, and\n\n```\n[05:11am] mabshoff: ok, fixed the tests, passes on OSX and not-OSX: #3563\n```",
    "created_at": "2008-09-15T12:20:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3563#issuecomment-26485",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:3'></a>
Looks good to me, and

```
[05:11am] mabshoff: ok, fixed the tests, passes on OSX and not-OSX: #3563
```



---

archive/issue_comments_026486.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-15T12:22:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3563#issuecomment-26486",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_026487.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.2.rc4",
    "created_at": "2008-09-15T12:22:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3563#issuecomment-26487",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.2.rc4



---

archive/issue_events_012996.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-15T12:22:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3563",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3563#event-12996"
}
```
