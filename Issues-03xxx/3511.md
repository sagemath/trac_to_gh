# Issue 3511: [with patch, positive review] make laplacian_matrix a synonym for kirchoff_matrix

archive/issues_003511.json:
```json
{
    "body": "In my education, this matrix was always referred to as the laplacian of the graph; I think the term may be more common.\n\nAssignee: @rlmill\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3511\n\n",
    "closed_at": "2008-06-26T23:04:21Z",
    "created_at": "2008-06-25T17:32:51Z",
    "labels": [
        "component: graph theory",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.4",
    "title": "[with patch, positive review] make laplacian_matrix a synonym for kirchoff_matrix",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3511",
    "user": "https://github.com/jasongrout"
}
```
In my education, this matrix was always referred to as the laplacian of the graph; I think the term may be more common.

Assignee: @rlmill

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3511





---

archive/issue_comments_026011.json:
```json
{
    "body": "<a id='comment:1'></a>\nGreat, do it!",
    "created_at": "2008-06-25T17:35:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3511#issuecomment-26011",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:1'></a>
Great, do it!



---

archive/issue_comments_026012.json:
```json
{
    "body": "Attachment [3511-laplacian.patch](tarball://root/attachments/some-uuid/ticket3511/3511-laplacian.patch) by @jasongrout created at 2008-06-26 11:48:34",
    "created_at": "2008-06-26T11:48:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3511#issuecomment-26012",
    "user": "https://github.com/jasongrout"
}
```

Attachment [3511-laplacian.patch](tarball://root/attachments/some-uuid/ticket3511/3511-laplacian.patch) by @jasongrout created at 2008-06-26 11:48:34



---

archive/issue_events_012749.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3511",
    "rename": {
        "from": "make laplacian_matrix a synonym for kirchoff_matrix",
        "to": "[with patch, needs review] make laplacian_matrix a synonym for kirchoff_matrix"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3511#event-12749"
}
```



---

archive/issue_events_012750.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-06-26T15:45:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3511",
    "milestone": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3511#event-12750"
}
```



---

archive/issue_events_012751.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3511",
    "rename": {
        "from": "[with patch, needs review] make laplacian_matrix a synonym for kirchoff_matrix",
        "to": "[with patch, positive review] make laplacian_matrix a synonym for kirchoff_matrix"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3511#event-12751"
}
```



---

archive/issue_comments_026013.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-06-26T23:04:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3511#issuecomment-26013",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_012752.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-26T23:04:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3511",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3511#event-12752"
}
```



---

archive/issue_comments_026014.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.4.alpha1",
    "created_at": "2008-06-26T23:04:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3511#issuecomment-26014",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.4.alpha1
