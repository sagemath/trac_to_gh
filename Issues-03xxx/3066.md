# Issue 3066: [with patch; positive review] empty matrices: gram_schmidt() throws a NameError

archive/issues_003066.json:
```json
{
    "body": "Looks like an explicit import is the only thing missing on this one:\n\n```\nsage: a = matrix([])\nsage: m.gram_schmidt()\n<type 'exceptions.NameError'>: global name 'ZZ' is not defined\n```\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3066\n\n",
    "closed_at": "2008-05-01T05:47:16Z",
    "created_at": "2008-04-30T15:20:39Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.1",
    "title": "[with patch; positive review] empty matrices: gram_schmidt() throws a NameError",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3066",
    "user": "https://github.com/dfdeshom"
}
```
Looks like an explicit import is the only thing missing on this one:

```
sage: a = matrix([])
sage: m.gram_schmidt()
<type 'exceptions.NameError'>: global name 'ZZ' is not defined
```

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3066





---

archive/issue_comments_022213.json:
```json
{
    "body": "Attachment [3066.patch](tarball://root/attachments/some-uuid/ticket3066/3066.patch) by @dfdeshom created at 2008-04-30 17:35:01",
    "created_at": "2008-04-30T17:35:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3066#issuecomment-22213",
    "user": "https://github.com/dfdeshom"
}
```

Attachment [3066.patch](tarball://root/attachments/some-uuid/ticket3066/3066.patch) by @dfdeshom created at 2008-04-30 17:35:01



---

archive/issue_comments_022214.json:
```json
{
    "body": "<a id='comment:1'></a>\nPatch attached.",
    "created_at": "2008-04-30T17:35:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3066#issuecomment-22214",
    "user": "https://github.com/dfdeshom"
}
```

<a id='comment:1'></a>
Patch attached.



---

archive/issue_events_010863.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3066",
    "rename": {
        "from": "empty matrices: gram_schmidt() throws a NameError",
        "to": "[with patch; needs review] empty matrices: gram_schmidt() throws a NameError"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3066#event-10863"
}
```



---

archive/issue_events_010864.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3066",
    "rename": {
        "from": "[with patch; needs review] empty matrices: gram_schmidt() throws a NameError",
        "to": "[with patch; positive review] empty matrices: gram_schmidt() throws a NameError"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3066#event-10864"
}
```



---

archive/issue_comments_022215.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-01T05:47:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3066#issuecomment-22215",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_022216.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.1.alpha1",
    "created_at": "2008-05-01T05:47:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3066#issuecomment-22216",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.1.alpha1



---

archive/issue_events_010865.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-01T05:47:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3066",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3066#event-10865"
}
```
