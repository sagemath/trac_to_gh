# Issue 3780: [with patch, positive review] add fast col_swap method for dense matrices over GF(2).

archive/issues_003780.json:
```json
{
    "body": "depends on #3324\n\nAssignee: @malb\n\nKeywords: m4ri\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3780\n\n",
    "closed_at": "2008-08-31T00:53:39Z",
    "created_at": "2008-08-06T16:28:22Z",
    "labels": [
        "component: linear algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] add fast col_swap method for dense matrices over GF(2).",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3780",
    "user": "https://github.com/malb"
}
```
depends on #3324

Assignee: @malb

Keywords: m4ri

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3780





---

archive/issue_comments_028301.json:
```json
{
    "body": "Attachment [m4ri_col_swap.patch](tarball://root/attachments/some-uuid/ticket3780/m4ri_col_swap.patch) by @malb created at 2008-08-06 16:29:12",
    "created_at": "2008-08-06T16:29:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-28301",
    "user": "https://github.com/malb"
}
```

Attachment [m4ri_col_swap.patch](tarball://root/attachments/some-uuid/ticket3780/m4ri_col_swap.patch) by @malb created at 2008-08-06 16:29:12



---

archive/issue_events_013949.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "rename": {
        "from": "[with patch, depends on #3324",
        "to": "[with patch, needs review] depends on #3324"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3780#event-13949"
}
```



---

archive/issue_events_013950.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "rename": {
        "from": "[with patch, needs review] depends on #3324",
        "to": "[with patch, needs review] add fast col_swap method for dense matrices over GF(2)."
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3780#event-13950"
}
```



---

archive/issue_comments_028302.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-add fast col_swap method for dense matrices over GF(2).\n+depends on #3324\n``````\n",
    "created_at": "2008-08-06T19:44:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-28302",
    "user": "https://github.com/mwhansen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-add fast col_swap method for dense matrices over GF(2).
+depends on #3324
``````




---

archive/issue_events_013951.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "rename": {
        "from": "[with patch, needs review] add fast col_swap method for dense matrices over GF(2).",
        "to": "[with patch, positive review] add fast col_swap method for dense matrices over GF(2)."
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3780#event-13951"
}
```



---

archive/issue_comments_028303.json:
```json
{
    "body": "<a id='comment:3'></a>\nI haven't tested this, but it looks fine. If it works, apply.",
    "created_at": "2008-08-31T00:11:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-28303",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:3'></a>
I haven't tested this, but it looks fine. If it works, apply.



---

archive/issue_comments_028304.json:
```json
{
    "body": "<a id='comment:4'></a>\n...other than the missing output from the last doctest! :-[",
    "created_at": "2008-08-31T00:21:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-28304",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:4'></a>
...other than the missing output from the last doctest! :-[



---

archive/issue_comments_028305.json:
```json
{
    "body": "<a id='comment:5'></a>\nNote that the doctests only pass with #3376 applied, where the missing output that rlm mentions above is actually added :)\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T00:34:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-28305",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Note that the doctests only pass with #3376 applied, where the missing output that rlm mentions above is actually added :)

Cheers,

Michael



---

archive/issue_comments_028306.json:
```json
{
    "body": "<a id='comment:6'></a>\nMerged in Sage 3.1.2.alpha3",
    "created_at": "2008-08-31T00:53:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-28306",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
Merged in Sage 3.1.2.alpha3



---

archive/issue_comments_028307.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-31T00:53:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-28307",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_013952.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-31T00:53:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3780#event-13952"
}
```



---

archive/issue_comments_028308.json:
```json
{
    "body": "<a id='comment:7'></a>\nNote that the patch attached to this ticket is a diff. I did commit it in Martin's name.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T03:55:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3780#issuecomment-28308",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:7'></a>
Note that the patch attached to this ticket is a diff. I did commit it in Martin's name.

Cheers,

Michael
