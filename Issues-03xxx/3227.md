# Issue 3227: add 64 bit OSX build support for tachyon

archive/issues_003227.json:
```json
{
    "body": "spkg coming up.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3227\n\n",
    "closed_at": "2009-01-23T00:11:50Z",
    "created_at": "2008-05-16T19:58:43Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "add 64 bit OSX build support for tachyon",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3227",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
spkg coming up.

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3227





---

archive/issue_events_017619.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-22T19:16:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3227",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3227#event-17619"
}
```



---

archive/issue_comments_018191.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/spkgs/tachyon-0.98beta.p7.spkg\n\nCheers,\n\nMichael",
    "created_at": "2009-01-22T19:16:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3227#issuecomment-18191",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
The spkg is at

http://sage.math.washington.edu/home/mabshoff/spkgs/tachyon-0.98beta.p7.spkg

Cheers,

Michael



---

archive/issue_events_017620.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-23T00:11:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3227",
    "milestone": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3227#event-17620"
}
```



---

archive/issue_events_017621.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-23T00:11:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3227",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3227#event-17621"
}
```



---

archive/issue_comments_018192.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.3.alpha1",
    "created_at": "2009-01-23T00:11:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3227#issuecomment-18192",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.3.alpha1



---

archive/issue_events_017622.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-23T00:11:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/3227",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3227#event-17622"
}
```



---

archive/issue_events_017623.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-23T00:11:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3227",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3227#event-17623"
}
```
