# Issue 3298: Cython warnings for PolyBoRi

archive/issues_003298.json:
```json
{
    "assignees": [
        "https://github.com/burcin"
    ],
    "body": "\n```\npython2.5 `which cython` --embed-positions --incref-local-binop -I/usr/local/sage-3.0/devel/sage-main -opbori.pyx\nwarning: /usr/local/sage-3.0/devel/sage-main/sage/rings/polynomial/../../libs/polybori/decl.pxi:40:56: Function signature does not match previous declaration\nwarning: /usr/local/sage-3.0/devel/sage-main/sage/rings/polynomial/../../libs/polybori/decl.pxi:41:59: Function signature does not match previous declaration\nwarning: /usr/local/sage-3.0/devel/sage-main/sage/rings/polynomial/../../libs/polybori/decl.pxi:200:10: Function signature does not match previous declaration\nFinished updating Cython code (time = 2.520616 seconds)\n```\n\nI couldn't figure out what is wrong. Maybe a false positive?\n\nCC:  @robertwb @burcin\n\nKeywords: **polybori, cython**\n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3298_\n\n",
    "closed_at": "2009-01-24T19:13:54Z",
    "created_at": "2008-05-25T13:41:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cython warnings for PolyBoRi",
    "type": "issue",
    "updated_at": "2009-01-24T19:13:54Z",
    "url": "https://github.com/sagemath/sage/issues/3298",
    "user": "https://github.com/malb"
}
```

```
python2.5 `which cython` --embed-positions --incref-local-binop -I/usr/local/sage-3.0/devel/sage-main -opbori.pyx
warning: /usr/local/sage-3.0/devel/sage-main/sage/rings/polynomial/../../libs/polybori/decl.pxi:40:56: Function signature does not match previous declaration
warning: /usr/local/sage-3.0/devel/sage-main/sage/rings/polynomial/../../libs/polybori/decl.pxi:41:59: Function signature does not match previous declaration
warning: /usr/local/sage-3.0/devel/sage-main/sage/rings/polynomial/../../libs/polybori/decl.pxi:200:10: Function signature does not match previous declaration
Finished updating Cython code (time = 2.520616 seconds)
```

I couldn't figure out what is wrong. Maybe a false positive?

CC:  @robertwb @burcin

Keywords: **polybori, cython**

Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/3298_





---

archive/issue_events_035466.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-05-25T13:41:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35466"
}
```



---

archive/issue_events_035467.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-05-25T13:41:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35467"
}
```



---

archive/issue_events_035468.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-05-25T13:41:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35468"
}
```



---

archive/issue_events_035469.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2008-05-25T13:41:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "subject": "https://github.com/malb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35469"
}
```



---

archive/issue_comments_017444.json:
```json
{
    "body": "fix cython warnings for pbori.pyx",
    "created_at": "2009-01-24T19:05:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3298#issuecomment-17444",
    "user": "https://github.com/burcin"
}
```

fix cython warnings for pbori.pyx



---

archive/issue_events_035470.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-01-24T19:07:29Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "subject": "https://github.com/burcin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35470"
}
```



---

archive/issue_events_035471.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-01-24T19:07:29Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "subject": "https://github.com/burcin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35471"
}
```



---

archive/issue_comments_017445.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_3298-pbori_cython_warnings.patch.gz](https://github.com/sagemath/sage/files/ticket3298/trac_3298-pbori_cython_warnings.patch.gz)**\n\nThrough a chain of typedefs the parameter expected by these functions was defined to be int, not the enum declared in the .pxi file. With the attached patch, the warnings go away. :)",
    "created_at": "2009-01-24T19:07:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3298#issuecomment-17445",
    "user": "https://github.com/burcin"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_3298-pbori_cython_warnings.patch.gz](https://github.com/sagemath/sage/files/ticket3298/trac_3298-pbori_cython_warnings.patch.gz)**

Through a chain of typedefs the parameter expected by these functions was defined to be int, not the enum declared in the .pxi file. With the attached patch, the warnings go away. :)



---

archive/issue_events_035472.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-01-24T19:07:29Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35472"
}
```



---

archive/issue_events_035473.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-01-24T19:07:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35473"
}
```



---

archive/issue_events_035474.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-01-24T19:07:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35474"
}
```



---

archive/issue_events_035475.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T19:08:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35475"
}
```



---

archive/issue_events_035476.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T19:08:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35476"
}
```



---

archive/issue_comments_017446.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-24T19:08:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3298#issuecomment-17446",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good.

Cheers,

Michael



---

archive/issue_events_035477.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T19:13:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35477"
}
```



---

archive/issue_events_035478.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T19:13:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3298#event-35478"
}
```



---

archive/issue_comments_017447.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.3.alpha2",
    "created_at": "2009-01-24T19:13:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3298#issuecomment-17447",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.3.alpha2
