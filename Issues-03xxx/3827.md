# Issue 3827: [with patch, positive review] finance.TimeSeries -- missng docstring input option

archive/issues_003827.json:
```json
{
    "body": "TimeSeries.randomize has a lognormal distribution generator built in along with uniform, normal, and semicircle. However, there is no docstring that says so! Simply need to add a line that looks like\n\n```\nINPUT:\n    distribution -- 'lognormal': mean loc and standard deviation scale\n```\n\nSee patch below.\n\nAssignee: @cswiercz\n\nCC:  @cswiercz\n\nKeywords: finance, timeseries\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3827\n\n",
    "closed_at": "2008-08-15T06:14:33Z",
    "created_at": "2008-08-12T23:44:08Z",
    "labels": [
        "component: finance",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1",
    "title": "[with patch, positive review] finance.TimeSeries -- missng docstring input option",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3827",
    "user": "https://github.com/cswiercz"
}
```
TimeSeries.randomize has a lognormal distribution generator built in along with uniform, normal, and semicircle. However, there is no docstring that says so! Simply need to add a line that looks like

```
INPUT:
    distribution -- 'lognormal': mean loc and standard deviation scale
```

See patch below.

Assignee: @cswiercz

CC:  @cswiercz

Keywords: finance, timeseries

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3827





---

archive/issue_comments_028662.json:
```json
{
    "body": "Attachment [sage-randomize.patch](tarball://root/attachments/some-uuid/ticket3827/sage-randomize.patch) by @cswiercz created at 2008-08-12 23:44:22",
    "created_at": "2008-08-12T23:44:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3827#issuecomment-28662",
    "user": "https://github.com/cswiercz"
}
```

Attachment [sage-randomize.patch](tarball://root/attachments/some-uuid/ticket3827/sage-randomize.patch) by @cswiercz created at 2008-08-12 23:44:22



---

archive/issue_comments_028663.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-08-12T23:44:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3827#issuecomment-28663",
    "user": "https://github.com/cswiercz"
}
```

Changing status from new to assigned.



---

archive/issue_events_014175.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3827",
    "rename": {
        "from": "finance.TimeSeries -- missng docstring input option",
        "to": "[with patch, needs review] finance.TimeSeries -- missng docstring input option"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3827#event-14175"
}
```



---

archive/issue_comments_028664.json:
```json
{
    "body": "<a id='comment:3'></a>\nThat's one trivial to review, patch works with 3.1-alpha2 (with 71 lines offset)...",
    "created_at": "2008-08-14T17:48:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3827#issuecomment-28664",
    "user": "https://trac.sagemath.org/admin/accounts/users/aginiewicz"
}
```

<a id='comment:3'></a>
That's one trivial to review, patch works with 3.1-alpha2 (with 71 lines offset)...



---

archive/issue_events_014176.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3827",
    "rename": {
        "from": "[with patch, needs review] finance.TimeSeries -- missng docstring input option",
        "to": "[with patch, positive review] finance.TimeSeries -- missng docstring input option"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3827#event-14176"
}
```



---

archive/issue_comments_028665.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-15T06:14:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3827#issuecomment-28665",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_028666.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.rc0",
    "created_at": "2008-08-15T06:14:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3827#issuecomment-28666",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.rc0



---

archive/issue_events_014177.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-15T06:14:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3827",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3827#event-14177"
}
```
