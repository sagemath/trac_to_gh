# Issue 3721: Use SAGE_TESTDIR for dsage unit tests

archive/issues_003721.json:
```json
{
    "assignees": [],
    "body": "dsage tests currently ignore the SAGE_TESTDIR environment variable, resulting in permission denied errors for users who don't have write access to their Sage install.\n\nI've attached a patch to fix this.\n\nAssignee: **@yqiang**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3721_\n\n",
    "closed_at": "2008-07-31T00:58:36Z",
    "created_at": "2008-07-25T05:31:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20dsage",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Use SAGE_TESTDIR for dsage unit tests",
    "type": "issue",
    "updated_at": "2008-07-31T00:58:36Z",
    "url": "https://github.com/sagemath/sage/issues/3721",
    "user": "https://github.com/timabbott"
}
```
dsage tests currently ignore the SAGE_TESTDIR environment variable, resulting in permission denied errors for users who don't have write access to their Sage install.

I've attached a patch to fix this.

Assignee: **@yqiang**

_Issue created by migration from https://trac.sagemath.org/ticket/3721_





---

archive/issue_events_035706.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-07-25T05:31:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35706"
}
```



---

archive/issue_events_035707.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-07-25T05:31:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20dsage",
    "label_color": "0000ff",
    "label_name": "component: dsage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35707"
}
```



---

archive/issue_events_035708.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-07-25T05:31:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35708"
}
```



---

archive/issue_events_035709.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-07-25T05:31:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35709"
}
```



---

archive/issue_events_035710.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-07-25T05:31:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35710"
}
```



---

archive/issue_comments_021343.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[sage_scripts-dsage-testdir.patch.gz](https://github.com/sagemath/sage/files/ticket3721/sage_scripts-dsage-testdir.patch.gz)**",
    "created_at": "2008-07-29T17:16:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3721#issuecomment-21343",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[sage_scripts-dsage-testdir.patch.gz](https://github.com/sagemath/sage/files/ticket3721/sage_scripts-dsage-testdir.patch.gz)**



---

archive/issue_events_035711.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-29T17:16:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35711"
}
```



---

archive/issue_events_035712.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T00:56:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35712"
}
```



---

archive/issue_events_035713.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T00:56:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35713"
}
```



---

archive/issue_events_035714.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T00:56:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35714"
}
```



---

archive/issue_comments_021344.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPatch looks good to me. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-31T00:56:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3721#issuecomment-21344",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Patch looks good to me. Positive review.

Cheers,

Michael



---

archive/issue_comments_021345.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMerged in Sage 3.1.alpha0",
    "created_at": "2008-07-31T00:58:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3721#issuecomment-21345",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Merged in Sage 3.1.alpha0



---

archive/issue_events_035715.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T00:58:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35715"
}
```



---

archive/issue_events_035716.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-31T00:58:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3721",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3721#event-35716"
}
```
