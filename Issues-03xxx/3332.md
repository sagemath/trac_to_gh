# Issue 3332: Switch pbuild to -O2

archive/issues_003332.json:
```json
{
    "assignees": [
        "https://github.com/garyfurnish"
    ],
    "body": "pbuild and sbuild currently compile with -O3; we should use -O2 for stability reasons (and -O3 may result in larger, slower code).\n\nKeywords: **pbuild**\n\nComponent: **pbuild**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3332_\n\n",
    "closed_at": "2008-06-09T21:20:21Z",
    "created_at": "2008-05-29T19:27:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Switch pbuild to -O2",
    "type": "issue",
    "updated_at": "2008-06-09T21:20:21Z",
    "url": "https://github.com/sagemath/sage/issues/3332",
    "user": "https://github.com/garyfurnish"
}
```
pbuild and sbuild currently compile with -O3; we should use -O2 for stability reasons (and -O3 may result in larger, slower code).

Keywords: **pbuild**

Component: **pbuild**

_Issue created by migration from https://trac.sagemath.org/ticket/3332_





---

archive/issue_events_035873.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-29T19:27:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "milestone_number": null,
    "milestone_title": "sage-3.0.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35873"
}
```



---

archive/issue_events_035874.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-29T19:27:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "000080",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35874"
}
```



---

archive/issue_events_035875.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-29T19:27:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35875"
}
```



---

archive/issue_events_035876.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-29T19:27:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35876"
}
```



---

archive/issue_events_035877.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-29T19:27:26Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "subject": "https://github.com/garyfurnish",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35877"
}
```



---

archive/issue_comments_017689.json:
```json
{
    "body": "Attachment: **[trac_3332_extcode.patch.gz](https://github.com/sagemath/sage/files/ticket3332/trac_3332_extcode.patch.gz)**",
    "created_at": "2008-05-29T19:27:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3332#issuecomment-17689",
    "user": "https://github.com/garyfurnish"
}
```

Attachment: **[trac_3332_extcode.patch.gz](https://github.com/sagemath/sage/files/ticket3332/trac_3332_extcode.patch.gz)**



---

archive/issue_events_035878.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-29T19:28:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35878"
}
```



---

archive/issue_events_035879.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-29T19:28:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35879"
}
```



---

archive/issue_comments_017690.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPositive review.",
    "created_at": "2008-06-09T21:18:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3332#issuecomment-17690",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Positive review.



---

archive/issue_events_035880.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-09T21:18:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35880"
}
```



---

archive/issue_comments_017691.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.0.3.alpha0",
    "created_at": "2008-06-09T21:20:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3332#issuecomment-17691",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.0.3.alpha0



---

archive/issue_events_035881.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-09T21:20:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35881"
}
```



---

archive/issue_events_035882.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-09T21:20:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3332",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3332#event-35882"
}
```
