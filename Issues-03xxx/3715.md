# Issue 3715: [with spkg, positive review] Bug in lisp startup script (trivial to do bug fix)

archive/issues_003715.json:
```json
{
    "body": "\n```\nWhile debugging the new release of the FriCAS optional package Waldek\nHebisch discovered a small bug in the script that starts clisp. There\nare missing argument quotes that can cause problems if some spkg (such\nas fricas-1.0.3.spkg) needs to pass arguments containing spaces.\n\nhttp://sage.math.washington.edu/home/page/packages/lisp.diff\n\n--- lisp.orig   2008-07-20 17:22:27.000000000 -0400\n+++ lisp        2008-07-20 03:04:00.000000000 -0400\n@@ -1,2 +1,2 @@\n #!/bin/sh\n-\"$SAGE_ROOT/local/bin/clisp.bin\" -B \"$SAGE_ROOT/local/lib/clisp-2.46\" $@\n+\"$SAGE_ROOT/local/bin/clisp.bin\" -B \"$SAGE_ROOT/local/lib/clisp-2.46\" \"$@\"\n\n-----\n\nRegards,\nBill Page.\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3715\n\n",
    "closed_at": "2008-07-29T16:55:51Z",
    "created_at": "2008-07-23T18:39:20Z",
    "labels": [
        "component: misc",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.6",
    "title": "[with spkg, positive review] Bug in lisp startup script (trivial to do bug fix)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3715",
    "user": "https://github.com/williamstein"
}
```

```
While debugging the new release of the FriCAS optional package Waldek
Hebisch discovered a small bug in the script that starts clisp. There
are missing argument quotes that can cause problems if some spkg (such
as fricas-1.0.3.spkg) needs to pass arguments containing spaces.

http://sage.math.washington.edu/home/page/packages/lisp.diff

--- lisp.orig   2008-07-20 17:22:27.000000000 -0400
+++ lisp        2008-07-20 03:04:00.000000000 -0400
@@ -1,2 +1,2 @@
 #!/bin/sh
-"$SAGE_ROOT/local/bin/clisp.bin" -B "$SAGE_ROOT/local/lib/clisp-2.46" $@
+"$SAGE_ROOT/local/bin/clisp.bin" -B "$SAGE_ROOT/local/lib/clisp-2.46" "$@"

-----

Regards,
Bill Page.
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3715





---

archive/issue_events_013662.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "rename": {
        "from": "trivial to do bug fix",
        "to": "Bug in lisp startup script (trivial to do bug fix)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3715#event-13662"
}
```



---

archive/issue_events_013663.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-29T16:00:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "milestone": "sage-3.0.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3715#event-13663"
}
```



---

archive/issue_events_013664.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "rename": {
        "from": "Bug in lisp startup script (trivial to do bug fix)",
        "to": "[with spkg, needs review] Bug in lisp startup script (trivial to do bug fix)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3715#event-13664"
}
```



---

archive/issue_comments_027762.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe spgk at #3712 fixes the issue.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-29T16:00:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3715#issuecomment-27762",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
The spgk at #3712 fixes the issue.

Cheers,

Michael



---

archive/issue_events_013665.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "rename": {
        "from": "[with spkg, needs review] Bug in lisp startup script (trivial to do bug fix)",
        "to": "[with spkg, positive review] Bug in lisp startup script (trivial to do bug fix)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3715#event-13665"
}
```



---

archive/issue_comments_027763.json:
```json
{
    "body": "Changing assignee from cwitty to mabshoff.",
    "created_at": "2008-07-29T16:52:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3715#issuecomment-27763",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing assignee from cwitty to mabshoff.



---

archive/issue_comments_027764.json:
```json
{
    "body": "<a id='comment:3'></a>\nDue to the positive review at #3712 I am changing this ticket to a positive review also.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-29T16:52:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3715#issuecomment-27764",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Due to the positive review at #3712 I am changing this ticket to a positive review also.

Cheers,

Michael



---

archive/issue_comments_027765.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-07-29T16:52:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3715#issuecomment-27765",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_events_013666.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-29T16:55:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3715#event-13666"
}
```



---

archive/issue_comments_027766.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-07-29T16:55:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3715#issuecomment-27766",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_027767.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.6.final",
    "created_at": "2008-07-29T16:55:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3715#issuecomment-27767",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.6.final
