# Issue 3481: [with patch, positive review] remove sage/plot/plot3dsoya.py from sage

archive/issues_003481.json:
```json
{
    "body": "Reason for removal: that soya3d project is sort of deprecated and we don't support it at all. \n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3481\n\n",
    "closed_at": "2008-08-31T04:22:40Z",
    "created_at": "2008-06-20T05:44:21Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] remove sage/plot/plot3dsoya.py from sage",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3481",
    "user": "https://github.com/williamstein"
}
```
Reason for removal: that soya3d project is sort of deprecated and we don't support it at all. 

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3481





---

archive/issue_comments_025798.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Reason for removal: that soya3d project is sort of deprecated and we don't support it at all. \n``````\n",
    "created_at": "2008-06-20T05:45:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3481#issuecomment-25798",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Reason for removal: that soya3d project is sort of deprecated and we don't support it at all. 
``````




---

archive/issue_events_012612.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3481",
    "rename": {
        "from": "remove sage/plot/plot3dsoya.py from sage!!!",
        "to": "remove sage/plot/plot3dsoya.py from sage"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3481#event-12612"
}
```



---

archive/issue_events_012613.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3481",
    "rename": {
        "from": "remove sage/plot/plot3dsoya.py from sage",
        "to": "[with patch, needs review] remove sage/plot/plot3dsoya.py from sage"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3481#event-12613"
}
```



---

archive/issue_comments_025799.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac_3481.patch](tarball://root/attachments/some-uuid/ticket3481/trac_3481.patch) by @mwhansen created at 2008-08-31 03:51:05",
    "created_at": "2008-08-31T03:51:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3481#issuecomment-25799",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Attachment [trac_3481.patch](tarball://root/attachments/some-uuid/ticket3481/trac_3481.patch) by @mwhansen created at 2008-08-31 03:51:05



---

archive/issue_events_012614.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3481",
    "rename": {
        "from": "[with patch, needs review] remove sage/plot/plot3dsoya.py from sage",
        "to": "[with patch, positive review] remove sage/plot/plot3dsoya.py from sage"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3481#event-12614"
}
```



---

archive/issue_comments_025800.json:
```json
{
    "body": "<a id='comment:3'></a>\nPatch looks good to me. Getting rid of crufty code is good!\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T03:57:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3481#issuecomment-25800",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Patch looks good to me. Getting rid of crufty code is good!

Cheers,

Michael



---

archive/issue_comments_025801.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.2.alpha3",
    "created_at": "2008-08-31T04:22:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3481#issuecomment-25801",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.2.alpha3



---

archive/issue_comments_025802.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-31T04:22:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3481#issuecomment-25802",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_012615.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-31T04:22:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3481",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3481#event-12615"
}
```
