# Issue 3993: implicit multiplication doesn't work in notebook

archive/issues_003993.json:
```json
{
    "body": "In a notebook cell, do\n\n```\nimplicit_multiplication(True)\n2x\n```\n\nIt returns a syntax error.  Things work fine from the command line.\n\nAssignee: somebody\n\nCC:  @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3993\n\n",
    "closed_at": "2010-01-02T08:08:25Z",
    "created_at": "2008-08-29T18:00:58Z",
    "labels": [
        "component: notebook"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.3.1",
    "title": "implicit multiplication doesn't work in notebook",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3993",
    "user": "https://github.com/jasongrout"
}
```
In a notebook cell, do

```
implicit_multiplication(True)
2x
```

It returns a syntax error.  Things work fine from the command line.

Assignee: somebody

CC:  @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3993





---

archive/issue_comments_031061.json:
```json
{
    "body": "Changing type from defect to enhancement.",
    "created_at": "2009-01-22T18:18:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3993#issuecomment-31061",
    "user": "https://github.com/aghitza"
}
```

Changing type from defect to enhancement.



---

archive/issue_comments_031062.json:
```json
{
    "body": "<a id='comment:2'></a>To release manager:  Is this dealt with by one of the SageNB upgrades?  I think there was a recent post about this.",
    "created_at": "2009-11-18T17:30:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3993#issuecomment-31062",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>To release manager:  Is this dealt with by one of the SageNB upgrades?  I think there was a recent post about this.



---

archive/issue_comments_031063.json:
```json
{
    "body": "Changing component from basic arithmetic to notebook.",
    "created_at": "2010-01-02T02:13:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3993#issuecomment-31063",
    "user": "https://github.com/aghitza"
}
```

Changing component from basic arithmetic to notebook.



---

archive/issue_comments_031064.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2010-01-02T02:13:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3993#issuecomment-31064",
    "user": "https://github.com/aghitza"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_031065.json:
```json
{
    "body": "<a id='comment:4'></a>This is fixed by #7514.",
    "created_at": "2010-01-02T08:08:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3993#issuecomment-31065",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:4'></a>This is fixed by #7514.



---

archive/issue_comments_031066.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-01-02T08:08:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3993#issuecomment-31066",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed



---

archive/issue_events_009152.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-02T08:08:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3993",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3993#event-9152"
}
```
