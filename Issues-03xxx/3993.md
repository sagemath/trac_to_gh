# Issue 3993: implicit multiplication doesn't work in notebook

archive/issues_003993.json:
```json
{
    "assignees": [],
    "body": "In a notebook cell, do\n\n```\nimplicit_multiplication(True)\n2x\n```\n\nIt returns a syntax error.  Things work fine from the command line.\n\n**CC:**  @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/3993\n\n",
    "closed_at": "2010-01-02T08:08:25Z",
    "created_at": "2008-08-29T18:00:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "implicit multiplication doesn't work in notebook",
    "type": "issue",
    "updated_at": "2010-01-02T08:08:25Z",
    "url": "https://github.com/sagemath/sage/issues/3993",
    "user": "https://github.com/jasongrout"
}
```
In a notebook cell, do

```
implicit_multiplication(True)
2x
```

It returns a syntax error.  Things work fine from the command line.

**CC:**  @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/3993





---

archive/issue_events_022868.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-01-22T18:18:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3993",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3993#event-22868"
}
```



---

archive/issue_events_022869.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-01-22T18:18:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3993",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3993#event-22869"
}
```



---

archive/issue_comments_023340.json:
```json
{
    "body": "<a id='comment:2'></a>\nTo release manager:  Is this dealt with by one of the SageNB upgrades?  I think there was a recent post about this.",
    "created_at": "2009-11-18T17:30:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3993#issuecomment-23340",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>
To release manager:  Is this dealt with by one of the SageNB upgrades?  I think there was a recent post about this.



---

archive/issue_events_022870.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-02T02:13:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3993",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "08517b",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3993#event-22870"
}
```



---

archive/issue_events_022871.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-02T02:13:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3993",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "08517b",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3993#event-22871"
}
```



---

archive/issue_comments_023341.json:
```json
{
    "body": "<a id='comment:4'></a>\nThis is fixed by #7514.",
    "created_at": "2010-01-02T08:08:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3993#issuecomment-23341",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:4'></a>
This is fixed by #7514.



---

archive/issue_events_022872.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-02T08:08:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3993",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3993#event-22872"
}
```
