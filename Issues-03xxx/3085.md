# Issue 3085: identity matrix docs

archive/issues_003085.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "The docs for identity matrix contain a \"\\t\", so the string should be a raw string so \\t doesn't expand to a tab.\n\nComponent: **linear algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3085_\n\n",
    "closed_at": "2008-05-03T13:34:55Z",
    "created_at": "2008-05-02T22:08:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "identity matrix docs",
    "type": "issue",
    "updated_at": "2008-05-03T13:34:55Z",
    "url": "https://github.com/sagemath/sage/issues/3085",
    "user": "https://github.com/jasongrout"
}
```
The docs for identity matrix contain a "\t", so the string should be a raw string so \t doesn't expand to a tab.

Component: **linear algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/3085_





---

archive/issue_events_032902.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-05-02T22:08:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "milestone_number": null,
    "milestone_title": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32902"
}
```



---

archive/issue_events_032903.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-05-02T22:08:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32903"
}
```



---

archive/issue_events_032904.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-05-02T22:08:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32904"
}
```



---

archive/issue_events_032905.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-05-02T22:08:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32905"
}
```



---

archive/issue_events_032906.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-05-02T22:08:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32906"
}
```



---

archive/issue_comments_016254.json:
```json
{
    "body": "Attachment: **[identity-matrix.patch.gz](https://github.com/sagemath/sage/files/ticket3085/identity-matrix.patch.gz)**",
    "created_at": "2008-05-02T22:09:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3085#issuecomment-16254",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[identity-matrix.patch.gz](https://github.com/sagemath/sage/files/ticket3085/identity-matrix.patch.gz)**



---

archive/issue_comments_016255.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nCredit goes to Geoff Tims for reporting this.",
    "created_at": "2008-05-02T22:09:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3085#issuecomment-16255",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:1" align="right">comment:1</div>

Credit goes to Geoff Tims for reporting this.



---

archive/issue_events_032907.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-02T22:10:36Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "title_is": "identity matrix docs",
    "title_was": "[with patch, needs *really* easy review] identity matrix docs",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32907"
}
```



---

archive/issue_events_032908.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-02T22:10:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32908"
}
```



---

archive/issue_comments_016256.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nuh, in the interest of full disclosure, I didn't doctest this, but I did sage -br and check the docs to make sure they look better.",
    "created_at": "2008-05-02T22:13:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3085#issuecomment-16256",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:3" align="right">comment:3</div>

uh, in the interest of full disclosure, I didn't doctest this, but I did sage -br and check the docs to make sure they look better.



---

archive/issue_events_032909.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-03T03:27:29Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "milestone_number": null,
    "milestone_title": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32909"
}
```



---

archive/issue_events_032910.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-03T03:27:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "milestone_number": null,
    "milestone_title": "sage-3.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32910"
}
```



---

archive/issue_comments_016257.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAfter looking at the patch I am convinced it will not harm anybody when we merge this in 3.0.1.\n\nCheers,\n\nMichael",
    "created_at": "2008-05-03T04:43:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3085#issuecomment-16257",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

After looking at the patch I am convinced it will not harm anybody when we merge this in 3.0.1.

Cheers,

Michael



---

archive/issue_events_032911.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-03T04:43:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "milestone_number": null,
    "milestone_title": "sage-3.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32911"
}
```



---

archive/issue_events_032912.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-03T04:43:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "milestone_number": null,
    "milestone_title": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32912"
}
```



---

archive/issue_comments_016258.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.0.1.final",
    "created_at": "2008-05-03T13:34:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3085#issuecomment-16258",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.0.1.final



---

archive/issue_events_032913.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-03T13:34:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32913"
}
```



---

archive/issue_events_032914.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-03T13:34:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3085",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3085#event-32914"
}
```
