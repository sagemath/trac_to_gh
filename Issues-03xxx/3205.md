# Issue 3205: [with patch, positive review] when the notebook scrolls to a new cell that is created, the jsmath box obscures the bottom cell

archive/issues_003205.json:
```json
{
    "body": "When scrolling through the notebook with the cursor keys (i.e., moving from cell to cell), if you move to a cell below the currently visible ones, the browser automatically scrolls down to let you see the new cell, but the jsmath box is sitting right there at the start of the cell, so you can't see what is in the start of the cell.\n\nFix 1: scroll a bit more than we do now\n\nFix 2: move the jsmath box to the bottom right instead of the bottom left\n\nFix 3: Get rid of the jsmath box and instead just make a menu option or some other way to access the jsmath controls.  Once the jsmath controls are set, they are very rarely accessed, at least for me.\n\n\nAssignee: boothby\n\nKeywords: editor_mhansen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3205\n\n",
    "closed_at": "2008-06-23T12:55:15Z",
    "created_at": "2008-05-14T21:55:56Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.4",
    "title": "[with patch, positive review] when the notebook scrolls to a new cell that is created, the jsmath box obscures the bottom cell",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3205",
    "user": "https://github.com/jasongrout"
}
```
When scrolling through the notebook with the cursor keys (i.e., moving from cell to cell), if you move to a cell below the currently visible ones, the browser automatically scrolls down to let you see the new cell, but the jsmath box is sitting right there at the start of the cell, so you can't see what is in the start of the cell.

Fix 1: scroll a bit more than we do now

Fix 2: move the jsmath box to the bottom right instead of the bottom left

Fix 3: Get rid of the jsmath box and instead just make a menu option or some other way to access the jsmath controls.  Once the jsmath controls are set, they are very rarely accessed, at least for me.


Assignee: boothby

Keywords: editor_mhansen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3205





---

archive/issue_events_011393.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "rename": {
        "from": "when the notebook scrolls to a new cell that is created, the jsmath box obscures the bottom cell",
        "to": "[with patch, needs review] when the notebook scrolls to a new cell that is created, the jsmath box obscures the bottom cell"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3205#event-11393"
}
```



---

archive/issue_comments_023246.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [sage-3205-jsmath-button.patch](tarball://root/attachments/some-uuid/ticket3205/sage-3205-jsmath-button.patch) by @jasongrout created at 2008-05-14 22:21:28",
    "created_at": "2008-05-14T22:21:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3205#issuecomment-23246",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
Attachment [sage-3205-jsmath-button.patch](tarball://root/attachments/some-uuid/ticket3205/sage-3205-jsmath-button.patch) by @jasongrout created at 2008-05-14 22:21:28



---

archive/issue_comments_023247.json:
```json
{
    "body": "<a id='comment:2'></a>\nWorks for me in FF and Opera.  Need review from IE / Safari.",
    "created_at": "2008-06-15T00:32:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3205#issuecomment-23247",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

<a id='comment:2'></a>
Works for me in FF and Opera.  Need review from IE / Safari.



---

archive/issue_events_011394.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "rename": {
        "from": "[with patch, needs review] when the notebook scrolls to a new cell that is created, the jsmath box obscures the bottom cell",
        "to": "[with patch, needs review (one positive)] when the notebook scrolls to a new cell that is created, the jsmath box obscures the bottom cell"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3205#event-11394"
}
```



---

archive/issue_comments_023248.json:
```json
{
    "body": "Changing keywords from \"\" to \"editor_mhansen\".",
    "created_at": "2008-06-15T21:43:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3205#issuecomment-23248",
    "user": "https://github.com/craigcitro"
}
```

Changing keywords from "" to "editor_mhansen".



---

archive/issue_events_011395.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "rename": {
        "from": "[with patch, needs review (one positive)] when the notebook scrolls to a new cell that is created, the jsmath box obscures the bottom cell",
        "to": "[with patch, positive review] when the notebook scrolls to a new cell that is created, the jsmath box obscures the bottom cell"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3205#event-11395"
}
```



---

archive/issue_comments_023249.json:
```json
{
    "body": "<a id='comment:4'></a>\nI tested this in IE and Safari and everything looks good.",
    "created_at": "2008-06-19T19:58:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3205#issuecomment-23249",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:4'></a>
I tested this in IE and Safari and everything looks good.



---

archive/issue_comments_023250.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 3.0.4.alpha0",
    "created_at": "2008-06-23T12:55:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3205#issuecomment-23250",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 3.0.4.alpha0



---

archive/issue_events_011396.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-23T12:55:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "milestone": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3205#event-11396"
}
```



---

archive/issue_events_011397.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-23T12:55:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3205#event-11397"
}
```



---

archive/issue_comments_023251.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-06-23T12:55:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3205#issuecomment-23251",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
