# Issue 3712: clisp+nohup eats ones disc

archive/issues_003712.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "\n```\nABORT          :R364    Abort debug loop\nABORT          :R365    Abort debug loop\nABORT          :R366    Abort debug loop\nABORT          :R367    Abort debug loop\nABORT          :R368    Abort debug loop\nABORT          :R369    Abort debug loop\nABORT          :R370    Abort debug loop\nABORT          :R371    Abort debug loop\nABORT        tee: write e\n```\n\nComponent: **build**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3712_\n\n",
    "closed_at": "2008-07-29T16:55:45Z",
    "created_at": "2008-07-23T13:17:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "clisp+nohup eats ones disc",
    "type": "issue",
    "updated_at": "2008-07-29T16:55:45Z",
    "url": "https://github.com/sagemath/sage/issues/3712",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

```
ABORT          :R364    Abort debug loop
ABORT          :R365    Abort debug loop
ABORT          :R366    Abort debug loop
ABORT          :R367    Abort debug loop
ABORT          :R368    Abort debug loop
ABORT          :R369    Abort debug loop
ABORT          :R370    Abort debug loop
ABORT          :R371    Abort debug loop
ABORT        tee: write e
```

Component: **build**

_Issue created by migration from https://trac.sagemath.org/ticket/3712_





---

archive/issue_events_040960.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-23T13:17:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "milestone_number": null,
    "milestone_title": "sage-3.0.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40960"
}
```



---

archive/issue_events_040961.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-23T13:17:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "000080",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40961"
}
```



---

archive/issue_events_040962.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-23T13:17:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40962"
}
```



---

archive/issue_events_040963.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-23T13:17:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40963"
}
```



---

archive/issue_events_040964.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-23T13:17:01Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40964"
}
```



---

archive/issue_comments_020420.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is likely fixed by putting William's workaround in back into spkg-install. Nils Bruin hot a likely related issue on OSX 10.4.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-25T10:34:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3712#issuecomment-20420",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

This is likely fixed by putting William's workaround in back into spkg-install. Nils Bruin hot a likely related issue on OSX 10.4.

Cheers,

Michael



---

archive/issue_comments_020421.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe spkg at \n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.6/final/clisp-2.46.p6.spkg\n\nreintroduces the old fix from the previous spkg. Notice that this spkg also fixes #3715.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-29T16:00:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3712#issuecomment-20421",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

The spkg at 

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.6/final/clisp-2.46.p6.spkg

reintroduces the old fix from the previous spkg. Notice that this spkg also fixes #3715.

Cheers,

Michael



---

archive/issue_events_040965.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-29T16:01:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40965"
}
```



---

archive/issue_events_040966.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-29T16:15:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40966"
}
```



---

archive/issue_events_040967.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-29T16:15:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40967"
}
```



---

archive/issue_comments_020422.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in Sage 3.0.6.final",
    "created_at": "2008-07-29T16:55:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3712#issuecomment-20422",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in Sage 3.0.6.final



---

archive/issue_events_040968.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-29T16:55:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40968"
}
```



---

archive/issue_events_040969.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-07-29T16:55:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3712",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3712#event-40969"
}
```
