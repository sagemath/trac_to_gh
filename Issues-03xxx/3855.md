# Issue 3855: point3d size default is too small to see

archive/issues_003855.json:
```json
{
    "body": "\n```\npoint3d((0,0,0))\n```\nversus\n\n```\npoint3d((0,0,0), size=10)\n```\n\nEasy to fix...\n\n**Assignee:** anakha\n\nIssue created by migration from https://trac.sagemath.org/ticket/3855\n\n",
    "closed_at": "2008-09-23T00:35:31Z",
    "created_at": "2008-08-14T19:31:44Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.3",
    "title": "point3d size default is too small to see",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3855",
    "user": "https://github.com/rlmill"
}
```

```
point3d((0,0,0))
```
versus

```
point3d((0,0,0), size=10)
```

Easy to fix...

**Assignee:** anakha

Issue created by migration from https://trac.sagemath.org/ticket/3855





---

archive/issue_events_021837.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/anakha",
    "created_at": "2008-09-21T19:47:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3855#event-21837"
}
```



---

archive/issue_comments_022234.json:
```json
{
    "body": "**Changing assignee** from @williamstein to anakha.",
    "created_at": "2008-09-21T19:47:09Z",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3855#issuecomment-22234",
    "user": "https://trac.sagemath.org/admin/accounts/users/anakha"
}
```

**Changing assignee** from @williamstein to anakha.



---

archive/attachments_004012.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_3855.patch",
    "asset_url": "tarball://root/attachments/ticket3855/trac_3855.patch",
    "created_at": "2008-09-21T19:47:09Z",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3855/trac_3855.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/anakha"
}
```



---

archive/issue_comments_022235.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_3855.patch](https://github.com/sagemath/sage/files/ticket3855/trac_3855.patch) by anakha created at 2008-09-21 19:47:09\n\nYes, easy.",
    "created_at": "2008-09-21T19:47:09Z",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3855#issuecomment-22235",
    "user": "https://trac.sagemath.org/admin/accounts/users/anakha"
}
```

<a id='comment:1'></a>
Attachment [trac_3855.patch](https://github.com/sagemath/sage/files/ticket3855/trac_3855.patch) by anakha created at 2008-09-21 19:47:09

Yes, easy.



---

archive/issue_events_021838.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/anakha",
    "created_at": "2008-09-21T19:47:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3855#event-21838"
}
```



---

archive/issue_events_021839.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/aginiewicz",
    "created_at": "2008-09-21T22:56:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3855#event-21839"
}
```



---

archive/issue_comments_022236.json:
```json
{
    "body": "<a id='comment:3'></a>\n5 looks good - so it's easy positive review",
    "created_at": "2008-09-21T22:56:20Z",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3855#issuecomment-22236",
    "user": "https://trac.sagemath.org/admin/accounts/users/aginiewicz"
}
```

<a id='comment:3'></a>
5 looks good - so it's easy positive review



---

archive/issue_events_021840.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-23T00:35:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3855#event-21840"
}
```



---

archive/issue_events_021841.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-23T00:35:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3855#event-21841"
}
```



---

archive/issue_comments_022237.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.3.alpha1",
    "created_at": "2008-09-23T00:35:31Z",
    "issue": "https://github.com/sagemath/sage/issues/3855",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3855#issuecomment-22237",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.3.alpha1
