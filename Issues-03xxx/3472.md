# Issue 3472: [with proto-patch, positive review] Running sage from a non-existent directory pretends to work

archive/issues_003472.json:
```json
{
    "assignees": [],
    "body": "Running sage from a directory that doesn't exist thinks it's working, but really just fails. I'm attaching a new `$SAGE_ROOT/sage` replacement that checks this on startup.\n\nAssignee: **@craigcitro**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3472_\n\n",
    "closed_at": "2008-06-27T00:14:23Z",
    "created_at": "2008-06-19T21:13:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[with proto-patch, positive review] Running sage from a non-existent directory pretends to work",
    "type": "issue",
    "updated_at": "2008-06-27T00:14:23Z",
    "url": "https://github.com/sagemath/sage/issues/3472",
    "user": "https://github.com/craigcitro"
}
```
Running sage from a directory that doesn't exist thinks it's working, but really just fails. I'm attaching a new `$SAGE_ROOT/sage` replacement that checks this on startup.

Assignee: **@craigcitro**

_Issue created by migration from https://trac.sagemath.org/ticket/3472_





---

archive/issue_events_032859.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2008-06-19T21:13:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "milestone_number": null,
    "milestone_title": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3472#event-32859"
}
```



---

archive/issue_events_032860.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2008-06-19T21:13:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3472#event-32860"
}
```



---

archive/issue_events_032861.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2008-06-19T21:13:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3472#event-32861"
}
```



---

archive/issue_comments_019696.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[sage-root.sage.gz](https://github.com/sagemath/sage/files/ticket3472/sage-root.sage.gz)**",
    "created_at": "2008-06-26T06:37:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3472#issuecomment-19696",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[sage-root.sage.gz](https://github.com/sagemath/sage/files/ticket3472/sage-root.sage.gz)**



---

archive/issue_events_032862.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-26T06:37:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3472#event-32862"
}
```



---

archive/issue_events_032863.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-27T00:14:12Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "title_is": "[with proto-patch, positive review] Running sage from a non-existent directory pretends to work",
    "title_was": "Running sage from a non-existent directory pretends to work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3472#event-32863"
}
```



---

archive/issue_comments_019697.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPositive review to the changes made by Craig. As it turned out `sage -upgrade` does not fix (this is now #3517)\n\nCheers,\n\nMichael",
    "created_at": "2008-06-27T00:14:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3472#issuecomment-19697",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Positive review to the changes made by Craig. As it turned out `sage -upgrade` does not fix (this is now #3517)

Cheers,

Michael



---

archive/issue_events_032864.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-27T00:14:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3472#event-32864"
}
```



---

archive/issue_events_032865.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-27T00:14:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "milestone_number": null,
    "milestone_title": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3472#event-32865"
}
```



---

archive/issue_comments_019698.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMerged in Sage 3.0.4.alpha1",
    "created_at": "2008-06-27T00:14:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3472#issuecomment-19698",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Merged in Sage 3.0.4.alpha1



---

archive/issue_events_032866.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-06-27T00:14:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3472",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3472#event-32866"
}
```
