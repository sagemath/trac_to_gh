# Issue 3390: update numpy to the 1.1.0 release

archive/issues_003390.json:
```json
{
    "body": "\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3390\n\n",
    "closed_at": "2008-08-27T11:29:52Z",
    "created_at": "2008-06-10T19:05:17Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "title": "update numpy to the 1.1.0 release",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3390",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```


**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3390





---

archive/issue_comments_019053.json:
```json
{
    "body": "<a id='comment:1'></a>\nAn spkg for this is up at:\n\nhttp://sage.math.washington.edu/home/jason/numpy-1.1.0.spkg\n\nThere are lots of doctest errors stemming I believe from matplotlib referencing things in old locations.  See #3392 for the matplotlib upgrading ticket.",
    "created_at": "2008-07-14T20:44:41Z",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3390#issuecomment-19053",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
An spkg for this is up at:

http://sage.math.washington.edu/home/jason/numpy-1.1.0.spkg

There are lots of doctest errors stemming I believe from matplotlib referencing things in old locations.  See #3392 for the matplotlib upgrading ticket.



---

archive/attachments_003335.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "numpy-1.1.patch",
    "asset_url": "tarball://root/attachments/ticket3390/numpy-1.1.patch",
    "created_at": "2008-08-21T22:36:18Z",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3390/numpy-1.1.patch",
    "user": "https://github.com/jasongrout"
}
```



---

archive/issue_comments_019054.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [numpy-1.1.patch](https://github.com/sagemath/sage/files/ticket3390/numpy-1.1.patch) by @jasongrout created at 2008-08-21 22:36:18",
    "created_at": "2008-08-21T22:36:18Z",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3390#issuecomment-19054",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
Attachment [numpy-1.1.patch](https://github.com/sagemath/sage/files/ticket3390/numpy-1.1.patch) by @jasongrout created at 2008-08-21 22:36:18



---

archive/issue_events_018355.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-08-21T22:36:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3390#event-18355"
}
```



---

archive/issue_comments_019055.json:
```json
{
    "body": "<a id='comment:3'></a>\nSpkg looks good to me. I did some cleanups in the patches directory. That spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.2/alpha1/numpy-1.1.0.p0.spkg\n\nAlso positive review for the patch.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-27T10:29:12Z",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3390#issuecomment-19055",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Spkg looks good to me. I did some cleanups in the patches directory. That spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.2/alpha1/numpy-1.1.0.p0.spkg

Also positive review for the patch.

Cheers,

Michael



---

archive/issue_events_018356.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-27T10:29:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3390#event-18356"
}
```



---

archive/issue_events_018357.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-27T10:29:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3390#event-18357"
}
```



---

archive/issue_comments_019056.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.2.alpha1",
    "created_at": "2008-08-27T11:29:52Z",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3390#issuecomment-19056",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.2.alpha1



---

archive/issue_events_018358.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-27T11:29:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3390#event-18358"
}
```



---

archive/issue_events_018359.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-27T11:29:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3390",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3390#event-18359"
}
```
