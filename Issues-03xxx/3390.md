# Issue 3390: [with patch, positive review] update numpy to the 1.1.0 release

archive/issues_003390.json:
```json
{
    "body": "\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3390\n\n",
    "closed_at": "2008-08-27T11:29:52Z",
    "created_at": "2008-06-10T19:05:17Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] update numpy to the 1.1.0 release",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3390",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```


Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3390





---

archive/issue_comments_024947.json:
```json
{
    "body": "<a id='comment:1'></a>\nAn spkg for this is up at:\n\nhttp://sage.math.washington.edu/home/jason/numpy-1.1.0.spkg\n\nThere are lots of doctest errors stemming I believe from matplotlib referencing things in old locations.  See #3392 for the matplotlib upgrading ticket.",
    "created_at": "2008-07-14T20:44:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3390#issuecomment-24947",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
An spkg for this is up at:

http://sage.math.washington.edu/home/jason/numpy-1.1.0.spkg

There are lots of doctest errors stemming I believe from matplotlib referencing things in old locations.  See #3392 for the matplotlib upgrading ticket.



---

archive/issue_comments_024948.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [numpy-1.1.patch](tarball://root/attachments/some-uuid/ticket3390/numpy-1.1.patch) by @jasongrout created at 2008-08-21 22:36:18",
    "created_at": "2008-08-21T22:36:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3390#issuecomment-24948",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
Attachment [numpy-1.1.patch](tarball://root/attachments/some-uuid/ticket3390/numpy-1.1.patch) by @jasongrout created at 2008-08-21 22:36:18



---

archive/issue_events_012177.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3390",
    "rename": {
        "from": "update numpy to the 1.1.0 release",
        "to": "[with patch, needs review] update numpy to the 1.1.0 release"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3390#event-12177"
}
```



---

archive/issue_comments_024949.json:
```json
{
    "body": "<a id='comment:3'></a>\nSpkg looks good to me. I did some cleanups in the patches directory. That spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.2/alpha1/numpy-1.1.0.p0.spkg\n\nAlso positive review for the patch.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-27T10:29:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3390#issuecomment-24949",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Spkg looks good to me. I did some cleanups in the patches directory. That spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.2/alpha1/numpy-1.1.0.p0.spkg

Also positive review for the patch.

Cheers,

Michael



---

archive/issue_events_012178.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3390",
    "rename": {
        "from": "[with patch, needs review] update numpy to the 1.1.0 release",
        "to": "[with patch, positive review] update numpy to the 1.1.0 release"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3390#event-12178"
}
```



---

archive/issue_comments_024950.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-27T11:29:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3390#issuecomment-24950",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_024951.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.2.alpha1",
    "created_at": "2008-08-27T11:29:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3390#issuecomment-24951",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.2.alpha1



---

archive/issue_events_012179.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-27T11:29:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3390",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3390#event-12179"
}
```
