# Issue 3268: Fix GAP interface printing

archive/issues_003268.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "I might be wrong, but it looks like output isn't getting printed:\n\n```\n# From a pure GAP session:\nGAP4, Version: 4.4.10 of 02-Oct-2007, i686-apple-darwin9.2.2-gcc\ngap> g := Group((1,3,2),(2,4,3));\nGroup([ (1,3,2), (2,4,3) ])\ngap> Stabilizer(g,4);\nGroup([ (1,3,2) ])\ngap> \n\n# From a Sage session:\nsage: %gap\n\n  --> Switching to Gap <-- \n\n''\ngap: g := Group((1,3,2),(2,4,3));\n\ngap: Stabilizer(g,4);\n\ngap: \n```\n\nComponent: **interfaces**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3268_\n\n",
    "closed_at": "2009-01-24T23:00:41Z",
    "created_at": "2008-05-21T16:43:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix GAP interface printing",
    "type": "issue",
    "updated_at": "2009-01-24T23:00:41Z",
    "url": "https://github.com/sagemath/sage/issues/3268",
    "user": "https://github.com/rlmill"
}
```
I might be wrong, but it looks like output isn't getting printed:

```
# From a pure GAP session:
GAP4, Version: 4.4.10 of 02-Oct-2007, i686-apple-darwin9.2.2-gcc
gap> g := Group((1,3,2),(2,4,3));
Group([ (1,3,2), (2,4,3) ])
gap> Stabilizer(g,4);
Group([ (1,3,2) ])
gap> 

# From a Sage session:
sage: %gap

  --> Switching to Gap <-- 

''
gap: g := Group((1,3,2),(2,4,3));

gap: Stabilizer(g,4);

gap: 
```

Component: **interfaces**

_Issue created by migration from https://trac.sagemath.org/ticket/3268_





---

archive/issue_events_035124.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-05-21T16:43:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35124"
}
```



---

archive/issue_events_035125.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-05-21T16:43:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "c: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35125"
}
```



---

archive/issue_events_035126.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-05-21T16:43:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35126"
}
```



---

archive/issue_events_035127.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-05-21T16:43:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35127"
}
```



---

archive/issue_events_035128.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-05-21T16:43:12Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "subject": "https://github.com/rlmill",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35128"
}
```



---

archive/issue_events_035129.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-25T04:18:58Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "title_is": "Fix GAP interface for Stabilizer()",
    "title_was": "Fix GAP interface",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35129"
}
```



---

archive/issue_events_035130.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-05-25T06:25:24Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "title_is": "Fix GAP interface printing",
    "title_was": "Fix GAP interface for Stabilizer()",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35130"
}
```



---

archive/issue_comments_017288.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis isn't just for Stabilizer(), since the group isn't printing either...",
    "created_at": "2008-05-25T06:25:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3268#issuecomment-17288",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:2" align="right">comment:2</div>

This isn't just for Stabilizer(), since the group isn't printing either...



---

archive/issue_comments_017289.json:
```json
{
    "body": "Attachment: **[trac_3268.patch.gz](https://github.com/sagemath/sage/files/ticket3268/trac_3268.patch.gz)**",
    "created_at": "2009-01-23T09:29:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3268#issuecomment-17289",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_3268.patch.gz](https://github.com/sagemath/sage/files/ticket3268/trac_3268.patch.gz)**



---

archive/issue_events_035131.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-23T09:31:05Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35131"
}
```



---

archive/issue_events_035132.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-23T09:31:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35132"
}
```



---

archive/issue_comments_017290.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nNote that there is no good way to test this as you can't access the processed line as it's completely internal to the function.",
    "created_at": "2009-01-23T09:31:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3268#issuecomment-17290",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Note that there is no good way to test this as you can't access the processed line as it's completely internal to the function.



---

archive/issue_events_035133.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-23T09:31:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35133"
}
```



---

archive/issue_events_035134.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-01-23T22:11:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35134"
}
```



---

archive/issue_events_035135.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2009-01-23T22:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35135"
}
```



---

archive/issue_comments_017291.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nNice catch.",
    "created_at": "2009-01-23T22:11:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3268#issuecomment-17291",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:4" align="right">comment:4</div>

Nice catch.



---

archive/issue_events_035136.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T23:00:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35136"
}
```



---

archive/issue_events_035137.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T23:00:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35137"
}
```



---

archive/issue_events_035138.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T23:00:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35138"
}
```



---

archive/issue_events_035139.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-24T23:00:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3268#event-35139"
}
```



---

archive/issue_comments_017292.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in Sage 3.3.alpha2.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-24T23:00:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3268",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3268#issuecomment-17292",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in Sage 3.3.alpha2.

Cheers,

Michael
