# Issue 3322: [with spkg, positive review] new python spkg with -j enabled

archive/issues_003322.json:
```json
{
    "body": "Using -j with make for python seems to be fixed.  It passes stress tests on both my machine and sage-math.  The spkg is based off of the one in #3318 and is located at http://sage.math.washington.edu/home/gfurnish/spkg/python-2.5.2.p1.spkg\n\nAssignee: mabshoff\n\nKeywords: python\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/3322\n\n",
    "closed_at": "2008-05-28T14:29:42Z",
    "created_at": "2008-05-28T14:05:59Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.3",
    "title": "[with spkg, positive review] new python spkg with -j enabled",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/3322",
    "user": "https://github.com/garyfurnish"
}
```
Using -j with make for python seems to be fixed.  It passes stress tests on both my machine and sage-math.  The spkg is based off of the one in #3318 and is located at http://sage.math.washington.edu/home/gfurnish/spkg/python-2.5.2.p1.spkg

Assignee: mabshoff

Keywords: python

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/3322





---

archive/issue_events_011856.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3322",
    "rename": {
        "from": "[with spkg needs review] new python spkg with -j enabled",
        "to": "[with spkg, needs review] new python spkg with -j enabled"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3322#event-11856"
}
```



---

archive/issue_comments_024186.json:
```json
{
    "body": "<a id='comment:2'></a>\nA properly updated spkg is at:\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/python-2.5.2.p2.spkg\n\nSince this is Gary's fix I give it a positive review. I did multiple runs with \"-j64\".\n\nCheers,\n\nMichael",
    "created_at": "2008-05-28T14:27:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3322#issuecomment-24186",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
A properly updated spkg is at:

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/python-2.5.2.p2.spkg

Since this is Gary's fix I give it a positive review. I did multiple runs with "-j64".

Cheers,

Michael



---

archive/issue_events_011857.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/3322",
    "rename": {
        "from": "[with spkg, needs review] new python spkg with -j enabled",
        "to": "[with spkg, positive review] new python spkg with -j enabled"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3322#event-11857"
}
```



---

archive/issue_comments_024187.json:
```json
{
    "body": "<a id='comment:3'></a>\nAttachment [trac_3322.patch](tarball://root/attachments/some-uuid/ticket3322/trac_3322.patch) by mabshoff created at 2008-05-28 14:29:42\n\nMerged in Sage 3.0.3.alpha0",
    "created_at": "2008-05-28T14:29:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3322#issuecomment-24187",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Attachment [trac_3322.patch](tarball://root/attachments/some-uuid/ticket3322/trac_3322.patch) by mabshoff created at 2008-05-28 14:29:42

Merged in Sage 3.0.3.alpha0



---

archive/issue_comments_024188.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-05-28T14:29:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/3322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/3322#issuecomment-24188",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_011858.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-28T14:29:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/3322",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/3322#event-11858"
}
```
