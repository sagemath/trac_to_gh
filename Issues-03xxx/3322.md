# Issue 3322: new python spkg with -j enabled

archive/issues_003322.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "Using -j with make for python seems to be fixed.  It passes stress tests on both my machine and sage-math.  The spkg is based off of the one in #3318 and is located at http://sage.math.washington.edu/home/gfurnish/spkg/python-2.5.2.p1.spkg\n\nKeywords: **python**\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3322_\n\n",
    "closed_at": "2008-05-28T14:29:42Z",
    "created_at": "2008-05-28T14:05:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "new python spkg with -j enabled",
    "type": "issue",
    "updated_at": "2008-05-28T14:29:42Z",
    "url": "https://github.com/sagemath/sage/issues/3322",
    "user": "https://github.com/garyfurnish"
}
```
Using -j with make for python seems to be fixed.  It passes stress tests on both my machine and sage-math.  The spkg is based off of the one in #3318 and is located at http://sage.math.washington.edu/home/gfurnish/spkg/python-2.5.2.p1.spkg

Keywords: **python**

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/3322_





---

archive/issue_events_035761.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-28T14:05:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "milestone_number": null,
    "milestone_title": "sage-3.0.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35761"
}
```



---

archive/issue_events_035762.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-28T14:05:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35762"
}
```



---

archive/issue_events_035763.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-28T14:05:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35763"
}
```



---

archive/issue_events_035764.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-28T14:05:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35764"
}
```



---

archive/issue_events_035765.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-28T14:05:59Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "subject": "https://github.com/garyfurnish",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35765"
}
```



---

archive/issue_events_035766.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-28T14:06:19Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "title_is": "new python spkg with -j enabled",
    "title_was": "[with spkg needs review] new python spkg with -j enabled",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35766"
}
```



---

archive/issue_events_035767.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-05-28T14:06:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35767"
}
```



---

archive/issue_comments_017629.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nA properly updated spkg is at:\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/python-2.5.2.p2.spkg\n\nSince this is Gary's fix I give it a positive review. I did multiple runs with \"-j64\".\n\nCheers,\n\nMichael",
    "created_at": "2008-05-28T14:27:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3322#issuecomment-17629",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

A properly updated spkg is at:

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha0/python-2.5.2.p2.spkg

Since this is Gary's fix I give it a positive review. I did multiple runs with "-j64".

Cheers,

Michael



---

archive/issue_events_035768.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-28T14:27:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35768"
}
```



---

archive/issue_events_035769.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-28T14:27:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35769"
}
```



---

archive/issue_comments_017630.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[trac_3322.patch.gz](https://github.com/sagemath/sage/files/ticket3322/trac_3322.patch.gz)**\n\nMerged in Sage 3.0.3.alpha0",
    "created_at": "2008-05-28T14:29:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3322#issuecomment-17630",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[trac_3322.patch.gz](https://github.com/sagemath/sage/files/ticket3322/trac_3322.patch.gz)**

Merged in Sage 3.0.3.alpha0



---

archive/issue_events_035770.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-28T14:29:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35770"
}
```



---

archive/issue_events_035771.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-28T14:29:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3322",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3322#event-35771"
}
```
