# Issue 21425: Fix initialization of PARI precision

archive/issues_021188.json:
```json
{
    "assignees": [],
    "body": "The precision in PARI is initialized wrongly:\n\nBy default, we get 2 significant words while we really should get only 1:\n\n```\nsage: pari('Pi').debug()\n[&=0000000004fc9620] REAL(lg=4):0400000000000004 (+,expo=1):6000000000000001 c90fdaa22168c234 c4c6628b80dc1cd1\n```\n\nIf we change the precision and then change it back, we get 1 word:\n\n```\nsage: n = pari.get_real_precision(); pari.set_real_precision(100); pari.set_real_precision(n);\nsage: pari('Pi').debug()\n[&=00000000012bf200] REAL(lg=3):0400000000000003 (+,expo=1):6000000000000001 c90fdaa22168c235\n```\n\nCC:  @defeo\n\nBranch/Commit: **[9de0279](https://github.com/sagemath/sagetrac-mirror/commit/9de0279571ee243f179b50dc283c68d1380d08f1)**\n\nReviewer: **Luca De Feo**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21425_\n\n",
    "closed_at": "2016-09-10T09:00:16Z",
    "created_at": "2016-09-05T15:55:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix initialization of PARI precision",
    "type": "issue",
    "updated_at": "2016-09-10T09:00:16Z",
    "url": "https://github.com/sagemath/sage/issues/21425",
    "user": "https://github.com/jdemeyer"
}
```
The precision in PARI is initialized wrongly:

By default, we get 2 significant words while we really should get only 1:

```
sage: pari('Pi').debug()
[&=0000000004fc9620] REAL(lg=4):0400000000000004 (+,expo=1):6000000000000001 c90fdaa22168c234 c4c6628b80dc1cd1
```

If we change the precision and then change it back, we get 1 word:

```
sage: n = pari.get_real_precision(); pari.set_real_precision(100); pari.set_real_precision(n);
sage: pari('Pi').debug()
[&=00000000012bf200] REAL(lg=3):0400000000000003 (+,expo=1):6000000000000001 c90fdaa22168c235
```

CC:  @defeo

Branch/Commit: **[9de0279](https://github.com/sagemath/sagetrac-mirror/commit/9de0279571ee243f179b50dc283c68d1380d08f1)**

Reviewer: **Luca De Feo**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/21425_





---

archive/issue_events_283066.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-09-05T15:55:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21425#event-283066"
}
```



---

archive/issue_events_283067.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-09-05T15:55:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21425#event-283067"
}
```



---

archive/issue_events_283068.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-09-05T15:55:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21425#event-283068"
}
```



---

archive/issue_events_283069.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-09-05T15:55:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21425#event-283069"
}
```



---

archive/issue_comments_316396.json:
```json
{
    "body": "Branch: **[u/jdemeyer/fix_initialization_of_pari_precision](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_initialization_of_pari_precision)**",
    "created_at": "2016-09-06T16:25:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21425#issuecomment-316396",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/fix_initialization_of_pari_precision](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_initialization_of_pari_precision)**



---

archive/issue_comments_316397.json:
```json
{
    "body": "Commit: **[9de0279](https://github.com/sagemath/sagetrac-mirror/commit/9de0279571ee243f179b50dc283c68d1380d08f1)**",
    "created_at": "2016-09-08T10:30:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21425#issuecomment-316397",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[9de0279](https://github.com/sagemath/sagetrac-mirror/commit/9de0279571ee243f179b50dc283c68d1380d08f1)**



---

archive/issue_comments_316398.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9de0279571ee243f179b50dc283c68d1380d08f1\">9de0279</a></td><td><code>Fix initialization of PARI precision</code></td></tr></table>\n",
    "created_at": "2016-09-08T10:30:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21425#issuecomment-316398",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9de0279571ee243f179b50dc283c68d1380d08f1">9de0279</a></td><td><code>Fix initialization of PARI precision</code></td></tr></table>




---

archive/issue_events_283070.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-09-08T10:30:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21425#event-283070"
}
```



---

archive/issue_comments_316399.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPasses tests on 32-bit and 64-bit.",
    "created_at": "2016-09-08T10:35:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21425#issuecomment-316399",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
Passes tests on 32-bit and 64-bit.



---

archive/issue_comments_316400.json:
```json
{
    "body": "Reviewer: **Luca De Feo**",
    "created_at": "2016-09-08T20:04:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21425#issuecomment-316400",
    "user": "https://github.com/defeo"
}
```

Reviewer: **Luca De Feo**



---

archive/issue_comments_316401.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThen I'm good with it.",
    "created_at": "2016-09-08T20:04:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21425#issuecomment-316401",
    "user": "https://github.com/defeo"
}
```

<a id='comment:5'>Comment 5:</a>
Then I'm good with it.



---

archive/issue_events_283071.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2016-09-08T20:04:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21425#event-283071"
}
```



---

archive/issue_events_283072.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2016-09-08T20:04:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21425#event-283072"
}
```



---

archive/issue_comments_316402.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/fix_initialization_of_pari_precision](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_initialization_of_pari_precision)** to **[9de0279](https://github.com/sagemath/sagetrac-mirror/commit/9de0279571ee243f179b50dc283c68d1380d08f1)**",
    "created_at": "2016-09-10T09:00:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21425#issuecomment-316402",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/fix_initialization_of_pari_precision](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_initialization_of_pari_precision)** to **[9de0279](https://github.com/sagemath/sagetrac-mirror/commit/9de0279571ee243f179b50dc283c68d1380d08f1)**



---

archive/issue_events_283073.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-09-10T09:00:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21425#event-283073"
}
```



---

archive/issue_events_283074.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c04f8a24641ea5bd798e0b6c0f32da0c33adf043",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-09-10T09:00:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21425",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21425#event-283074"
}
```
