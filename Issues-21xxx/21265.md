# Issue 21265: pynac does not build on Cygwin

archive/issues_021028.json:
```json
{
    "assignees": [],
    "body": "pynac-0.6.8 fails on 7.4.beta0 with multiple definition of functions such as\n\n```\n.libs/libpynac_la-lst.o: In function `std::_List_const_iterator<GiNaC::ex>::operator++()':\n/home/Travis/sage/local/var/tmp/sage/build/pynac-0.6.8/src/ginac/container.h:521: multiple definition of `GiNaC::container<std::list>::info(unsigned int) const'\n.libs/libpynac_la-inifcns_trans.o:/home/Travis/sage/local/var/tmp/sage/build/pynac-0.6.8/src/ginac/container.h:365: first defined here\ncollect2: error: ld returned 1 exit status\n```\n\nCC:  @embray jpflori gouezel @rwst\n\nBranch/Commit: **[u/embray/ticket-21265](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/ticket-21265) @ [7b7025b](https://github.com/sagemath/sagetrac-mirror/commit/7b7025bfdbad228b0170282427fe72c0fc126ea0)**\n\nReviewer: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21265_\n\n",
    "closed_at": "2016-08-30T13:32:25Z",
    "created_at": "2016-08-17T14:15:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "pynac does not build on Cygwin",
    "type": "issue",
    "updated_at": "2016-08-30T13:32:25Z",
    "url": "https://github.com/sagemath/sage/issues/21265",
    "user": "https://github.com/tscrim"
}
```
pynac-0.6.8 fails on 7.4.beta0 with multiple definition of functions such as

```
.libs/libpynac_la-lst.o: In function `std::_List_const_iterator<GiNaC::ex>::operator++()':
/home/Travis/sage/local/var/tmp/sage/build/pynac-0.6.8/src/ginac/container.h:521: multiple definition of `GiNaC::container<std::list>::info(unsigned int) const'
.libs/libpynac_la-inifcns_trans.o:/home/Travis/sage/local/var/tmp/sage/build/pynac-0.6.8/src/ginac/container.h:365: first defined here
collect2: error: ld returned 1 exit status
```

CC:  @embray jpflori gouezel @rwst

Branch/Commit: **[u/embray/ticket-21265](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/ticket-21265) @ [7b7025b](https://github.com/sagemath/sagetrac-mirror/commit/7b7025bfdbad228b0170282427fe72c0fc126ea0)**

Reviewer: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/21265_





---

archive/issue_events_281066.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-08-17T14:15:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281066"
}
```



---

archive/issue_events_281067.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-08-17T14:15:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281067"
}
```



---

archive/issue_events_281068.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-08-17T14:15:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281068"
}
```



---

archive/issue_events_281069.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-08-17T14:15:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281069"
}
```



---

archive/issue_comments_313554.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nConfirmed the same.",
    "created_at": "2016-08-29T14:51:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313554",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
Confirmed the same.



---

archive/issue_comments_313555.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis looks to be related to this: http://www.ginac.de/ginac.git/?p=ginac.git;a=commit;h=e5eeee53d814cedc12cd725e76b0eb87859cdd77  But in a different incarnation since (I think?) the ginac that the current pynac is based on is much older?\n\nNot entirely sure why it only appears on Windows, but it wouldn't be the first time that the different linking routines used on Windows expose link issues that don't necessarily appear elsewhere (even if maybe they should).",
    "created_at": "2016-08-29T15:33:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313555",
    "user": "https://github.com/embray"
}
```

<a id='comment:2'>Comment 2:</a>
This looks to be related to this: http://www.ginac.de/ginac.git/?p=ginac.git;a=commit;h=e5eeee53d814cedc12cd725e76b0eb87859cdd77  But in a different incarnation since (I think?) the ginac that the current pynac is based on is much older?

Not entirely sure why it only appears on Windows, but it wouldn't be the first time that the different linking routines used on Windows expose link issues that don't necessarily appear elsewhere (even if maybe they should).



---

archive/issue_comments_313556.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThat could be the cause. IDK how much changed in the latest upgrade of Pynac. Ralf, do you have any thoughts on this?",
    "created_at": "2016-08-29T15:37:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313556",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
That could be the cause. IDK how much changed in the latest upgrade of Pynac. Ralf, do you have any thoughts on this?



---

archive/issue_comments_313557.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nWas there a version that compiled?",
    "created_at": "2016-08-29T15:45:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313557",
    "user": "https://github.com/rwst"
}
```

<a id='comment:4'>Comment 4:</a>
Was there a version that compiled?



---

archive/issue_comments_313558.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI recall that some version somewhere in the 7.2 betas worked for me. Erik and/or JP likely have more precise details. Erik had compiled Sage on Cygwin64, which I believe was using 7.3, so probably the Pynac version just before 0.6.8 (which was included in 7.4.beta0). Please correct me if I'm wrong.",
    "created_at": "2016-08-29T15:54:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313558",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
I recall that some version somewhere in the 7.2 betas worked for me. Erik and/or JP likely have more precise details. Erik had compiled Sage on Cygwin64, which I believe was using 7.3, so probably the Pynac version just before 0.6.8 (which was included in 7.4.beta0). Please correct me if I'm wrong.



---

archive/issue_comments_313559.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nI can check against 7.3 tonight (I need my machine on my Ubuntu boot to do my work for today).",
    "created_at": "2016-08-29T15:55:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313559",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
I can check against 7.3 tonight (I need my machine on my Ubuntu boot to do my work for today).



---

archive/issue_comments_313560.json:
```json
{
    "body": "Commit: **[7b7025b](https://github.com/sagemath/sagetrac-mirror/commit/7b7025bfdbad228b0170282427fe72c0fc126ea0)**",
    "created_at": "2016-08-29T16:04:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313560",
    "user": "https://github.com/embray"
}
```

Commit: **[7b7025b](https://github.com/sagemath/sagetrac-mirror/commit/7b7025bfdbad228b0170282427fe72c0fc126ea0)**



---

archive/issue_comments_313561.json:
```json
{
    "body": "Branch: **[u/embray/ticket-21265](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/ticket-21265)**",
    "created_at": "2016-08-29T16:04:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313561",
    "user": "https://github.com/embray"
}
```

Branch: **[u/embray/ticket-21265](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/ticket-21265)**



---

archive/issue_events_281070.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-29T16:04:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281070"
}
```



---

archive/issue_comments_313562.json:
```json
{
    "body": "Author: **Erik Bray**",
    "created_at": "2016-08-29T16:04:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313562",
    "user": "https://github.com/embray"
}
```

Author: **Erik Bray**



---

archive/issue_comments_313563.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nHere's a working patch--this doesn't apply to ginac where all incarnations of this issue are already fixed.\n\nI'm not sure what the procedure here is though.  Since we basically control pynac should the issue just be fixed in pynac, and pynac upgraded in Sage?  Or does it still make sense at this stage to have a patch to pynac in Sage?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7b7025bfdbad228b0170282427fe72c0fc126ea0\">7b7025b</a></td><td><code>Fix building pynac 0.6.8 on Cygwin</code></td></tr></table>\n",
    "created_at": "2016-08-29T16:04:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313563",
    "user": "https://github.com/embray"
}
```

<a id='comment:7'>Comment 7:</a>
Here's a working patch--this doesn't apply to ginac where all incarnations of this issue are already fixed.

I'm not sure what the procedure here is though.  Since we basically control pynac should the issue just be fixed in pynac, and pynac upgraded in Sage?  Or does it still make sense at this stage to have a patch to pynac in Sage?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7b7025bfdbad228b0170282427fe72c0fc126ea0">7b7025b</a></td><td><code>Fix building pynac 0.6.8 on Cygwin</code></td></tr></table>




---

archive/issue_comments_313564.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI can confirm also that before pynac 0.6.8 it worked.",
    "created_at": "2016-08-29T16:04:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313564",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'>Comment 8:</a>
I can confirm also that before pynac 0.6.8 it worked.



---

archive/issue_comments_313565.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nRalf, do you want to cut a new release of Pynac for this or should we just include this as a Sage patch for now?",
    "created_at": "2016-08-29T16:12:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313565",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:9'>Comment 9:</a>
Ralf, do you want to cut a new release of Pynac for this or should we just include this as a Sage patch for now?



---

archive/issue_comments_313566.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nI wanted to do a new release tomorrow anyway. We can close this ticket after new Pynac is in Sage.\n\nThanks for the patch, authorship will be recognized.",
    "created_at": "2016-08-29T17:08:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313566",
    "user": "https://github.com/rwst"
}
```

<a id='comment:10'>Comment 10:</a>
I wanted to do a new release tomorrow anyway. We can close this ticket after new Pynac is in Sage.

Thanks for the patch, authorship will be recognized.



---

archive/issue_comments_313567.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nSounds good to me.",
    "created_at": "2016-08-29T17:14:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313567",
    "user": "https://github.com/embray"
}
```

<a id='comment:11'>Comment 11:</a>
Sounds good to me.



---

archive/issue_comments_313568.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\n~~In any case it turns out my patch to `spkg-install` misused some variables in such a way that somehow caused the build to be skipped, while not producing any actual errors; this was very confusing.  That said, I had already confirmed that pynac does build properly with the patch--it's only the `spkg-install` itself that I messed up.~~\n\nScratch that. That doesn't seem to be it either.  For some reason every time I try to reinstall pynac now it's just a no-op.  It doesn't even run the `spkg-install`.  I've never seen this happen before.",
    "created_at": "2016-08-29T17:31:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313568",
    "user": "https://github.com/embray"
}
```

<a id='comment:12'>Comment 12:</a>
~~In any case it turns out my patch to `spkg-install` misused some variables in such a way that somehow caused the build to be skipped, while not producing any actual errors; this was very confusing.  That said, I had already confirmed that pynac does build properly with the patch--it's only the `spkg-install` itself that I messed up.~~

Scratch that. That doesn't seem to be it either.  For some reason every time I try to reinstall pynac now it's just a no-op.  It doesn't even run the `spkg-install`.  I've never seen this happen before.



---

archive/issue_comments_313569.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nDoes this mean the Pynac code changes are not confirmed to resolve the issue?\n\nTo reinstall Pynac (in the sense that `make` will pick it up) after changes/patches outside the tarball the patchlevel must be increased, i.e. `0.6.8.p0` in `package-version.txt`. However, you can skip that by forcing a reinstall with `sage -p pynac; make`.",
    "created_at": "2016-08-30T05:23:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313569",
    "user": "https://github.com/rwst"
}
```

<a id='comment:13'>Comment 13:</a>
Does this mean the Pynac code changes are not confirmed to resolve the issue?

To reinstall Pynac (in the sense that `make` will pick it up) after changes/patches outside the tarball the patchlevel must be increased, i.e. `0.6.8.p0` in `package-version.txt`. However, you can skip that by forcing a reinstall with `sage -p pynac; make`.



---

archive/issue_comments_313570.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nReplying to [@rwst](#comment%3A13):\n> Does this mean the Pynac code changes are not confirmed to resolve the issue?\n\nIt is confirmed.  I had already made the changes and built pynac from source (that is actually compiled it).\n\n> To reinstall Pynac (in the sense that `make` will pick it up) after changes/patches outside the tarball the patchlevel must be increased, i.e. `0.6.8.p0` in `package-version.txt`. However, you can skip that by forcing a reinstall with `sage -p pynac; make`.\n\nI did bump the patch level.  I think there's just some subtle bug in my edits to `spkg-install` that's causing it to exit prematurely but without error.  But I confirmed the patch worked manually before making those changes.",
    "created_at": "2016-08-30T07:52:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313570",
    "user": "https://github.com/embray"
}
```

<a id='comment:14'>Comment 14:</a>
Replying to [@rwst](#comment%3A13):
> Does this mean the Pynac code changes are not confirmed to resolve the issue?

It is confirmed.  I had already made the changes and built pynac from source (that is actually compiled it).

> To reinstall Pynac (in the sense that `make` will pick it up) after changes/patches outside the tarball the patchlevel must be increased, i.e. `0.6.8.p0` in `package-version.txt`. However, you can skip that by forcing a reinstall with `sage -p pynac; make`.

I did bump the patch level.  I think there's just some subtle bug in my edits to `spkg-install` that's causing it to exit prematurely but without error.  But I confirmed the patch worked manually before making those changes.



---

archive/issue_comments_313571.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nSee #21369.",
    "created_at": "2016-08-30T08:42:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313571",
    "user": "https://github.com/rwst"
}
```

<a id='comment:15'>Comment 15:</a>
See #21369.



---

archive/issue_comments_313572.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nAh, I see what was wrong.  This made `patch` into an infinite recursion.  For some reason that causes bash to just bail.  Weird.  Anyways it's a moot point, this ticket is superseded by #21369.",
    "created_at": "2016-08-30T09:01:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313572",
    "user": "https://github.com/embray"
}
```

<a id='comment:16'>Comment 16:</a>
Ah, I see what was wrong.  This made `patch` into an infinite recursion.  For some reason that causes bash to just bail.  Weird.  Anyways it's a moot point, this ticket is superseded by #21369.



---

archive/issue_events_281071.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-30T10:01:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281071"
}
```



---

archive/issue_events_281072.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-30T10:01:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281072"
}
```



---

archive/issue_comments_313573.json:
```json
{
    "body": "Reviewer: **Erik Bray**",
    "created_at": "2016-08-30T10:01:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313573",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Erik Bray**



---

archive/issue_comments_313574.json:
```json
{
    "body": "Changed author from **Erik Bray** to none",
    "created_at": "2016-08-30T10:01:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313574",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Erik Bray** to none



---

archive/issue_events_281073.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-30T10:01:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281073"
}
```



---

archive/issue_events_281074.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-30T10:01:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281074"
}
```



---

archive/issue_events_281075.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-30T10:01:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281075"
}
```



---

archive/issue_events_281076.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281076"
}
```



---

archive/issue_comments_313575.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nDetermined to be invalid/duplicate/wontfix (closing as \"wontfix\" as a catch-all resolution).",
    "created_at": "2016-08-30T13:32:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21265#issuecomment-313575",
    "user": "https://github.com/embray"
}
```

<a id='comment:18'>Comment 18:</a>
Determined to be invalid/duplicate/wontfix (closing as "wontfix" as a catch-all resolution).



---

archive/issue_events_281077.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281077"
}
```



---

archive/issue_events_281078.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21265",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21265#event-281078"
}
```
