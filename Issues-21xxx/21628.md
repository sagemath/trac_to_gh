...
<a id='comment:2'></a>I've added calls to `sig_check` in the constructor `LinearCodeAutGroupCanLabel` (which might call itself recursively), as well as `sig_on/off` the call to `PartitionRefinementLinearCode`. The latter is, AFAICT, the main work-horse of the algorithm. Both are Cython, but there's more C-like code in `PartitionRefinementLinearCode` (in `codecan.pyx`), so this might be why it used to behave so badly when interrupted.
...
```json
{
    "body": "<a id='comment:3'></a>The call to `PartitionRefinementLinearCode` is a Python call, so `sig_on()` is inappropriate. `sig_on()` is only meant to wrap calls to external libraries that you have no control over.\n\nIn this case, you should use `sig_check()` in the Cython code instead.",
...
<a id='comment:3'></a>The call to `PartitionRefinementLinearCode` is a Python call, so `sig_on()` is inappropriate. `sig_on()` is only meant to wrap calls to external libraries that you have no control over.
...
<a id='comment:5'></a>Well, the cysignals documentation also has a warning box with
> The code inside `sig_on()` should be pure C or Cython code. If you call any Python code or manipulate any Python object (even something trivial like `x = []`), an interrupt can mess up Python’s internal state. When in doubt, try to use `sig_check()` instead.
...
<a id='comment:6'></a>Replying to [jsrn](#comment%3A4):
> Further, removing that `sig_on/sig_off` pair, I get the following segfault when interrupting:
> 
> 
> ```
...
<a id='comment:8'></a>The backtrace from gdb is:
...
<a id='comment:12'></a>I do have gdb since gdb launches when I type gdb. It's not hard to test.
...
```json
{
    "body": "<a id='comment:14'></a>I created #21651 for the existing segfault issue.",
...
<a id='comment:14'></a>I created #21651 for the existing segfault issue.
...
```json
{
    "body": "<a id='comment:15'></a>As far as I can tell, this issue is completely fixed by #21651.",
...
<a id='comment:15'></a>As far as I can tell, this issue is completely fixed by #21651.
...
```json
{
    "body": "<a id='comment:16'></a>Yes, I think so. I tried some days ago to make it crash again, but was unable. I wanted to get back to it and try harder, but never got around to it. Since you also tried since then, it's probably true that the issue got fixed.\n\nLet's close it.",
...
<a id='comment:16'></a>Yes, I think so. I tried some days ago to make it crash again, but was unable. I wanted to get back to it and try harder, but never got around to it. Since you also tried since then, it's probably true that the issue got fixed.
