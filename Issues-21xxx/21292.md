...
```json
{
    "body": "In the description of `ignored`, I'd elaborate the use case you give (which isn't that obvious).\n\n(Just about one sentence I'd say.)\n\n---\n\nWhy did you add `?:` to the `ignored_marker` regex???",
...
```json
{
    "body": "Looks good but the patchbot complains that \"random\" has no doctest. I guess you should add one though it is somewhat artificial.",
    "created_at": "2017-01-13T07:59:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21292",
    "type": "issue_comment",
...
```json
{
    "body": "Revisiting this... To be honest, I never really liked the idea of having two different doctest markers doing the same thing. Wouldn't it be sufficient to use `# random` with additional comments to explain why it's random?\n\n```\nsage: 6*9  # random (depending on default number base)\n42\nsage: moon.phase  # random (depending on the phase of the moon)\n'full'\n```",
...
```json
{
    "body": "The idea was to have `#ignored` eventually replace `#random` since the latter is poorly-named.  It's useful to be able to skip individual doctests that are not \"random\".",
...
```json
{
    "body": "I don't know--I think it's useful to be able to say \"this output is random\".  Depends on the case.",
...
```json
{
    "body": "On second thought, there's so many cases of `# random` in the source code that if I try to change it it may conflict with just about every other unmerged branch.\n\nPerhaps it should suffice to just update the documentation to suggest using `# ignored` instead of `# random` and keep the latter deprecated.",
