...
```json
{
    "body": "Problems in the Singular interface were found. A new version is necessary.",
    "created_at": "2016-11-26T09:14:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21963",
    "type": "issue_comment",
...
```json
{
    "body": "That would be awesome. The next release will take some time anyway.",
...
```json
{
    "body": "So the safe method seems in `configure` to detect `-lao` in the `.la` file and disable Giac in case.",
...
```json
{
    "body": "Dependency needs to include `pkgconf` as the `pkg-config` command needs to be there at configuration time.",
    "created_at": "2016-12-08T01:58:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21963",
    "type": "issue_comment",
...
```json
{
    "body": "The bot will take care of testing debian/ubuntu I guess. But yes one of the next step will be more robust detection of a working `giac` which may be a standard package in 7.5....",
...
```json
{
    "body": "Thanks. Unfortunately I didn't switch on the Giac support in Pynac in this ticket so anything Giac (with it being standard package now) will have to wait for the next Pynac release.",
