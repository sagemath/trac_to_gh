# Issue 21853: Unneeded linking against gmpxx

archive/issues_021616.json:
```json
{
    "assignees": [],
    "body": "Lots of modules are compiled against `gmpxx` even though they don't use it.\n\nCC:  @kiwifb\n\nBranch/Commit: **[8f12228](https://github.com/sagemath/sagetrac-mirror/commit/8f1222870950887f9671a29bc658c60418a63f2c)**\n\nReviewer: **Fran\u00e7ois Bissey**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21853_\n\n",
    "closed_at": "2016-11-12T23:01:24Z",
    "created_at": "2016-11-10T12:12:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Unneeded linking against gmpxx",
    "type": "issue",
    "updated_at": "2016-11-12T23:01:24Z",
    "url": "https://github.com/sagemath/sage/issues/21853",
    "user": "https://github.com/jdemeyer"
}
```
Lots of modules are compiled against `gmpxx` even though they don't use it.

CC:  @kiwifb

Branch/Commit: **[8f12228](https://github.com/sagemath/sagetrac-mirror/commit/8f1222870950887f9671a29bc658c60418a63f2c)**

Reviewer: **François Bissey**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/21853_





---

archive/issue_events_288541.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-10T12:12:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21853#event-288541"
}
```



---

archive/issue_events_288542.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-10T12:12:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21853#event-288542"
}
```



---

archive/issue_events_288543.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-10T12:12:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21853#event-288543"
}
```



---

archive/issue_events_288544.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-10T12:12:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21853#event-288544"
}
```



---

archive/issue_comments_324160.json:
```json
{
    "body": "Branch: **[u/jdemeyer/unneeded_linking_against_gmpxx](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/unneeded_linking_against_gmpxx)**",
    "created_at": "2016-11-10T14:03:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21853#issuecomment-324160",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/unneeded_linking_against_gmpxx](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/unneeded_linking_against_gmpxx)**



---

archive/issue_comments_324161.json:
```json
{
    "body": "Commit: **[8f12228](https://github.com/sagemath/sagetrac-mirror/commit/8f1222870950887f9671a29bc658c60418a63f2c)**",
    "created_at": "2016-11-10T14:04:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21853#issuecomment-324161",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[8f12228](https://github.com/sagemath/sagetrac-mirror/commit/8f1222870950887f9671a29bc658c60418a63f2c)**



---

archive/issue_comments_324162.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8f1222870950887f9671a29bc658c60418a63f2c\">8f12228</a></td><td><code>Fix needless linking to gmpxx</code></td></tr></table>\n",
    "created_at": "2016-11-10T14:04:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21853#issuecomment-324162",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8f1222870950887f9671a29bc658c60418a63f2c">8f12228</a></td><td><code>Fix needless linking to gmpxx</code></td></tr></table>




---

archive/issue_events_288545.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-10T14:04:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21853#event-288545"
}
```



---

archive/issue_events_288546.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-11-10T19:17:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21853#event-288546"
}
```



---

archive/issue_events_288547.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-11-10T19:17:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21853#event-288547"
}
```



---

archive/issue_comments_324163.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me as discussed in [#21749 comment:13](https://github.com/sagemath/sage/issues/21749#comment:13)",
    "created_at": "2016-11-10T19:17:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21853#issuecomment-324163",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me as discussed in [#21749 comment:13](https://github.com/sagemath/sage/issues/21749#comment:13)



---

archive/issue_comments_324164.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2016-11-10T19:17:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21853#issuecomment-324164",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_comments_324165.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/unneeded_linking_against_gmpxx](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/unneeded_linking_against_gmpxx)** to **[8f12228](https://github.com/sagemath/sagetrac-mirror/commit/8f1222870950887f9671a29bc658c60418a63f2c)**",
    "created_at": "2016-11-12T23:01:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21853#issuecomment-324165",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/unneeded_linking_against_gmpxx](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/unneeded_linking_against_gmpxx)** to **[8f12228](https://github.com/sagemath/sagetrac-mirror/commit/8f1222870950887f9671a29bc658c60418a63f2c)**



---

archive/issue_events_288548.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-11-12T23:01:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21853#event-288548"
}
```



---

archive/issue_events_288549.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "93bc66ba8090ea2e7cb600324a5c0faa89a7521b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-11-12T23:01:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21853",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21853#event-288549"
}
```
