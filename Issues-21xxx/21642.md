# Issue 21642: Py3-style raise to list_clone_demo.pyx

archive/issues_021405.json:
```json
{
    "assignees": [],
    "body": "By\n\n```\negrep -R 'raise [a-zA-Z]+[^(]+, ?\"' src/sage --no-filename | egrep -v '^ *#'\n```\n\nI found Python2-style raise from `src/sage/structure/list_clone_demo.pyx`. I think that it is the only one left.\n\nCC:  @fchapoton\n\nBranch/Commit: **[1192079](https://github.com/sagemath/sagetrac-mirror/commit/1192079362103bae16b388fea0420825018316b1)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21642_\n\n",
    "closed_at": "2016-10-08T14:42:04Z",
    "created_at": "2016-10-05T03:40:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Py3-style raise to list_clone_demo.pyx",
    "type": "issue",
    "updated_at": "2016-10-08T14:42:04Z",
    "url": "https://github.com/sagemath/sage/issues/21642",
    "user": "https://github.com/jm58660"
}
```
By

```
egrep -R 'raise [a-zA-Z]+[^(]+, ?"' src/sage --no-filename | egrep -v '^ *#'
```

I found Python2-style raise from `src/sage/structure/list_clone_demo.pyx`. I think that it is the only one left.

CC:  @fchapoton

Branch/Commit: **[1192079](https://github.com/sagemath/sagetrac-mirror/commit/1192079362103bae16b388fea0420825018316b1)**

Reviewer: **Frédéric Chapoton**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/21642_





---

archive/issue_events_285918.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-05T03:40:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21642#event-285918"
}
```



---

archive/issue_events_285919.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-05T03:40:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21642#event-285919"
}
```



---

archive/issue_events_285920.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-05T03:40:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21642#event-285920"
}
```



---

archive/issue_events_285921.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-05T03:40:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21642#event-285921"
}
```



---

archive/issue_comments_320719.json:
```json
{
    "body": "Branch: **[u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx)**",
    "created_at": "2016-10-05T03:41:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21642#issuecomment-320719",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx)**



---

archive/issue_comments_320720.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx)** to none",
    "created_at": "2016-10-05T03:42:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21642#issuecomment-320720",
    "user": "https://github.com/jm58660"
}
```

Changed branch from **[u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx)** to none



---

archive/issue_events_285922.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-05T03:42:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21642#event-285922"
}
```



---

archive/issue_comments_320721.json:
```json
{
    "body": "Branch: **[u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx)**",
    "created_at": "2016-10-05T03:43:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21642#issuecomment-320721",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx)**



---

archive/issue_comments_320722.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-10-05T06:17:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21642#issuecomment-320722",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_285923.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-10-05T06:17:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21642#event-285923"
}
```



---

archive/issue_events_285924.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-10-05T06:17:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21642#event-285924"
}
```



---

archive/issue_comments_320723.json:
```json
{
    "body": "Commit: **[1192079](https://github.com/sagemath/sagetrac-mirror/commit/1192079362103bae16b388fea0420825018316b1)**",
    "created_at": "2016-10-05T06:17:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21642#issuecomment-320723",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[1192079](https://github.com/sagemath/sagetrac-mirror/commit/1192079362103bae16b388fea0420825018316b1)**



---

archive/issue_comments_320724.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nok, thanks\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1192079362103bae16b388fea0420825018316b1\">1192079</a></td><td><code>Raise to py3-style.</code></td></tr></table>\n",
    "created_at": "2016-10-05T06:17:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21642#issuecomment-320724",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

ok, thanks

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1192079362103bae16b388fea0420825018316b1">1192079</a></td><td><code>Raise to py3-style.</code></td></tr></table>




---

archive/issue_events_285925.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-10-08T14:42:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21642#event-285925"
}
```



---

archive/issue_events_285926.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "203a7a8589edc3e80d8db56e730118ccba321ad0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-10-08T14:42:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21642#event-285926"
}
```



---

archive/issue_comments_320725.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx)** to **[1192079](https://github.com/sagemath/sagetrac-mirror/commit/1192079362103bae16b388fea0420825018316b1)**",
    "created_at": "2016-10-08T14:42:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21642#issuecomment-320725",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/py3_style_raise_to_list_clone_demo_pyx)** to **[1192079](https://github.com/sagemath/sagetrac-mirror/commit/1192079362103bae16b388fea0420825018316b1)**
