...
```json
{
    "body": "This is the `pexpect` interface playing up again, isn't it? It was also on OS X last time.",
...
```json
{
    "body": "First somebody needs to double-check that all patches which were removed in #21631 were actually upstreamed.",
...
```json
{
    "body": "`interrupt.patch` is not in current upstream at all. Do we have a PR for it?",
...
```json
{
    "body": "So, we should just wait for `4.0.3p5`?",
...
```json
{
    "body": "Or 4.1.0 as it seems upstream is preparing for it.",
...
```json
{
    "body": "http://www.mathematik.uni-kl.de/ftp/pub/Math/Singular/SOURCES/4-1-0/",
...
```json
{
    "body": "There isn't a matching \"share\" tarball, is this expected?",
...
```json
{
    "body": "Must still be coming otherwise we are without `singular.hlp` unless we can build it without trouble.",
...
```json
{
    "body": "Found it, hiding in the `doc` folder there is a `doc.tbz2` file which seems to have all the useful content of share tarball, including `singular.hlp`.",
...
```json
{
    "body": "OK we need to stop the madness about `singular.hlp` anyway. In 4.0+ it is naturally installed in `$prefix/share/info` as part of the building process (this is a info file with a funny extension). And `singular.idx` is installed in `$prefix/share/singular` without it having to be done by `spkg-install` as well.\n\nSo that part of `spkg-install` should disappear anyway and we should amend the location of `singular.hlp` in `sage/interfaces/singular.py`.",
...
```json
{
    "body": "Great! and no need to repackage the upstream tarball!!!",
...
```json
{
    "body": "I think we will be able to just use vanilla upstream - no more `spkg-src` yeah! I doing some building and testing on sage-on-gentoo to see if we have any new breakages. I'll probably report in a couple of hours.",
...
```json
{
    "body": "The content of the `doc.tbz2` has the wrong permissions, not an issue for sage (unless installed as root) but it is becoming one for me (files are 640 and folders 651 instead of 644 and 655).",
...
```json
{
    "body": "I'm working on an update branch.",
...
```json
{
    "body": "Segfault when computing gr\u00f6bner bases over the integers reported here:\n* https://groups.google.com/forum/#!topic/libsingular-devel/15FF-f-4-uk",
...
```json
{
    "body": "Not ready yet but looking like something more usable.\n\n---\nNew commits:",
...
```json
{
    "body": "* src/sage/interfaces/singular.py : coeff printing function for complex numbers, only in debug mode.\n* src/sage/matrix/matrix_modn_dense_template.pxi : to be investigated",
...
```json
{
    "body": "The last failure seem to be a debug mode only one...",
...
```json
{
    "body": "And the cfinite_sequence one as well...",
...
```json
{
    "body": "Yes that one. Anyway I am running doctests right now.",
...
```json
{
    "body": "The GB errors were due to my patch for GB computation over rings being incomplete. Now upstream patch is used.\n\nThe cmdline error happened because Singular does not like its stdin to be closed. Added a patch for that.",
    "created_at": "2016-12-22T12:46:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21865",
    "type": "issue_comment",
...
```json
{
    "body": "I pulled the changes in the branch but the groebner patch had zero impact on `multi_polynomial_ideal.py` and the other doctest in the documentation. Still failing in the same way. Did you forget a bit?",
...
```json
{
    "body": "Hum yeah, I can now rereproduce it... Not sure of what happened yesterday.",
...
```json
{
    "body": "I think I got what happened yesterday: if I build Singular in debug mode I get no segfault :)",
...
```json
{
    "body": "I running the whole test suite right now but feel free to do it as well.",
...
```json
{
    "body": "Works for me on OS X 10.12.",
...
```json
{
    "body": "The previously failing doctests are now passing. I am just doing a full run to be sure.",
...
```json
{
    "body": "Onward to the bots. Putting this as blocker as well.",
    "created_at": "2016-12-24T07:39:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21865",
    "type": "issue_comment",
