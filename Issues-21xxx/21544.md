# Issue 21544: LatticePoset: Add is_join_distributive (and dual)

archive/issues_021307.json:
```json
{
    "assignees": [],
    "body": "This patch will add function to check if a lattice is join-distributive. (And dually, if it is meet-distributive.)\n\n\nCC:  @tscrim\n\nBranch/Commit: **[4aa0d1c](https://github.com/sagemath/sagetrac-mirror/commit/4aa0d1ca9f8dbd9c7a22a1ce80930294305a500d)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21544_\n\n",
    "closed_at": "2016-10-21T07:04:05Z",
    "created_at": "2016-09-19T17:14:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "LatticePoset: Add is_join_distributive (and dual)",
    "type": "issue",
    "updated_at": "2016-10-21T07:04:05Z",
    "url": "https://github.com/sagemath/sage/issues/21544",
    "user": "https://github.com/jm58660"
}
```
This patch will add function to check if a lattice is join-distributive. (And dually, if it is meet-distributive.)


CC:  @tscrim

Branch/Commit: **[4aa0d1c](https://github.com/sagemath/sagetrac-mirror/commit/4aa0d1ca9f8dbd9c7a22a1ce80930294305a500d)**

Reviewer: **Travis Scrimshaw**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/21544_





---

archive/issue_events_284662.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-09-19T17:14:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284662"
}
```



---

archive/issue_events_284663.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-09-19T17:14:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284663"
}
```



---

archive/issue_events_284664.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-09-19T17:14:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284664"
}
```



---

archive/issue_events_284665.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-09-19T17:14:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284665"
}
```



---

archive/issue_comments_318889.json:
```json
{
    "body": "Branch: **[u/jmantysalo/join-distributive](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/join-distributive)**",
    "created_at": "2016-09-19T17:14:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318889",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/join-distributive](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/join-distributive)**



---

archive/issue_events_284666.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-09-19T17:16:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284666"
}
```



---

archive/issue_comments_318890.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI hope that a reviewer can check this and then return to *needs_work*. This is not yet in the index of functions and the dual function is missing; however, it is trivial to do if this one is accepted.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4dba3b0a947fce4ff29ef7c02c5b0bf3533e7356\">4dba3b0</a></td><td><code>Add is_join_distributive().</code></td></tr></table>\n",
    "created_at": "2016-09-19T17:16:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318890",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:2" align="right">Comment 2</div>

I hope that a reviewer can check this and then return to *needs_work*. This is not yet in the index of functions and the dual function is missing; however, it is trivial to do if this one is accepted.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4dba3b0a947fce4ff29ef7c02c5b0bf3533e7356">4dba3b0</a></td><td><code>Add is_join_distributive().</code></td></tr></table>




---

archive/issue_comments_318891.json:
```json
{
    "body": "Commit: **[4dba3b0](https://github.com/sagemath/sagetrac-mirror/commit/4dba3b0a947fce4ff29ef7c02c5b0bf3533e7356)**",
    "created_at": "2016-09-19T17:16:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318891",
    "user": "https://github.com/jm58660"
}
```

Commit: **[4dba3b0](https://github.com/sagemath/sagetrac-mirror/commit/4dba3b0a947fce4ff29ef7c02c5b0bf3533e7356)**



---

archive/issue_comments_318892.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nDaniel, want to review something?",
    "created_at": "2016-09-22T07:16:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318892",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:3" align="right">Comment 3</div>

Daniel, want to review something?



---

archive/issue_comments_318893.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nDaniel seems to be away. Travis?",
    "created_at": "2016-10-06T11:23:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318893",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:4" align="right">Comment 4</div>

Daniel seems to be away. Travis?



---

archive/issue_comments_318894.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nTwo small things: `iter.next()` to `next(iter)` and remove the comma here <code>Return \\`\\`True\\`\\` if the lattice is join-distributive, and \\`\\`False\\`\\` otherwise.</code>",
    "created_at": "2016-10-06T13:47:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318894",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

Two small things: `iter.next()` to `next(iter)` and remove the comma here <code>Return \`\`True\`\` if the lattice is join-distributive, and \`\`False\`\` otherwise.</code>



---

archive/issue_comments_318895.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-10-06T13:47:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318895",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_318896.json:
```json
{
    "body": "Changed commit from **[4dba3b0](https://github.com/sagemath/sagetrac-mirror/commit/4dba3b0a947fce4ff29ef7c02c5b0bf3533e7356)** to **[810bd13](https://github.com/sagemath/sagetrac-mirror/commit/810bd139209c8e368f7a1ac9a3e26ab5e3c120e5)**",
    "created_at": "2016-10-06T18:13:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318896",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4dba3b0](https://github.com/sagemath/sagetrac-mirror/commit/4dba3b0a947fce4ff29ef7c02c5b0bf3533e7356)** to **[810bd13](https://github.com/sagemath/sagetrac-mirror/commit/810bd139209c8e368f7a1ac9a3e26ab5e3c120e5)**



---

archive/issue_comments_318897.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/810bd139209c8e368f7a1ac9a3e26ab5e3c120e5\">810bd13</a></td><td><code>Py3-style next().</code></td></tr></table>\n",
    "created_at": "2016-10-06T18:13:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318897",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/810bd139209c8e368f7a1ac9a3e26ab5e3c120e5">810bd13</a></td><td><code>Py3-style next().</code></td></tr></table>




---

archive/issue_comments_318898.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@tscrim](#comment%3A5):\n> Two small things: `iter.next()` to `next(iter)`\n\nThis again... corrected.\n\n> and remove the comma here <code>Return \\`\\`True\\`\\` if the lattice is join-distributive, and \\`\\`False\\`\\` otherwise.</code>\n\nDuh. Done, but this exactly same phrase is used in about 20 functions. This is one more reason to have a \"prototype\" of specific kind of functions.\n\nMore to do?",
    "created_at": "2016-10-06T18:21:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318898",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@tscrim](#comment%3A5):
> Two small things: `iter.next()` to `next(iter)`

This again... corrected.

> and remove the comma here <code>Return \`\`True\`\` if the lattice is join-distributive, and \`\`False\`\` otherwise.</code>

Duh. Done, but this exactly same phrase is used in about 20 functions. This is one more reason to have a "prototype" of specific kind of functions.

More to do?



---

archive/issue_comments_318899.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nNope. Positive review.",
    "created_at": "2016-10-06T18:28:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318899",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:8" align="right">Comment 8</div>

Nope. Positive review.



---

archive/issue_events_284667.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-06T18:28:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284667"
}
```



---

archive/issue_events_284668.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-06T18:28:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284668"
}
```



---

archive/issue_comments_318900.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nI'll add the dual function `is_meet_distributive` to this, but I guess I must wait for next beta to get the index of functions done.",
    "created_at": "2016-10-06T20:56:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318900",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:9" align="right">Comment 9</div>

I'll add the dual function `is_meet_distributive` to this, but I guess I must wait for next beta to get the index of functions done.



---

archive/issue_events_284669.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-06T20:56:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284669"
}
```



---

archive/issue_events_284670.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-06T20:56:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284670"
}
```



---

archive/issue_comments_318901.json:
```json
{
    "body": "Changed commit from **[810bd13](https://github.com/sagemath/sagetrac-mirror/commit/810bd139209c8e368f7a1ac9a3e26ab5e3c120e5)** to **[4aa0d1c](https://github.com/sagemath/sagetrac-mirror/commit/4aa0d1ca9f8dbd9c7a22a1ce80930294305a500d)**",
    "created_at": "2016-10-07T05:48:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318901",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[810bd13](https://github.com/sagemath/sagetrac-mirror/commit/810bd139209c8e368f7a1ac9a3e26ab5e3c120e5)** to **[4aa0d1c](https://github.com/sagemath/sagetrac-mirror/commit/4aa0d1ca9f8dbd9c7a22a1ce80930294305a500d)**



---

archive/issue_comments_318902.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4aa0d1ca9f8dbd9c7a22a1ce80930294305a500d\">4aa0d1c</a></td><td><code>Add dual function.</code></td></tr></table>\n",
    "created_at": "2016-10-07T05:48:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318902",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4aa0d1ca9f8dbd9c7a22a1ce80930294305a500d">4aa0d1c</a></td><td><code>Add dual function.</code></td></tr></table>




---

archive/issue_events_284671.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-07T05:49:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284671"
}
```



---

archive/issue_events_284672.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-07T05:49:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284672"
}
```



---

archive/issue_comments_318903.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nMaybe this can be added now; I will try to remember to add links to index of functions in some following ticket.",
    "created_at": "2016-10-07T05:49:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318903",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:11" align="right">Comment 11</div>

Maybe this can be added now; I will try to remember to add links to index of functions in some following ticket.



---

archive/issue_comments_318904.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nTravis, can you check another half? It is direct copy of `is_join_distributive()` that you already reviewed, just everything conversed to dual. (+ seealso-links added)",
    "created_at": "2016-10-15T04:40:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318904",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:12" align="right">Comment 12</div>

Travis, can you check another half? It is direct copy of `is_join_distributive()` that you already reviewed, just everything conversed to dual. (+ seealso-links added)



---

archive/issue_comments_318905.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nLGTM.",
    "created_at": "2016-10-15T13:43:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318905",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:13" align="right">Comment 13</div>

LGTM.



---

archive/issue_events_284673.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-15T13:43:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284673"
}
```



---

archive/issue_events_284674.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-15T13:43:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284674"
}
```



---

archive/issue_comments_318906.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/join-distributive](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/join-distributive)** to **[4aa0d1c](https://github.com/sagemath/sagetrac-mirror/commit/4aa0d1ca9f8dbd9c7a22a1ce80930294305a500d)**",
    "created_at": "2016-10-21T07:04:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21544#issuecomment-318906",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/join-distributive](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/join-distributive)** to **[4aa0d1c](https://github.com/sagemath/sagetrac-mirror/commit/4aa0d1ca9f8dbd9c7a22a1ce80930294305a500d)**



---

archive/issue_events_284675.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-10-21T07:04:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284675"
}
```



---

archive/issue_events_284676.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b48674618bc93c0e0f67b39466a8342f86817740",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-10-21T07:04:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21544",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21544#event-284676"
}
```
