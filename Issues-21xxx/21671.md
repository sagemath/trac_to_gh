# Issue 21671: Bug in infinite LS paths crystal operators

archive/issues_021434.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: LS = crystals.infinity.LSPaths(['G',2])\nsage: LS.highest_weight_vector().f_string([1,1,1,2])\n(5/2*Lambda[1] - Lambda[2], -5/2*Lambda[1] + 5*Lambda[2], Lambda[1] + Lambda[2])\nsage: LS.highest_weight_vector().f_string([1,1,1,2,2])\n(5*Lambda[1] - 2*Lambda[2], -2*Lambda[1] + 4*Lambda[2], -Lambda[1] - Lambda[2])\nsage: LS.highest_weight_vector().f_string([1,1,1,2,2]).e(2)\n(5/2*Lambda[1] - Lambda[2], -5/2*Lambda[1] + 5*Lambda[2], -Lambda[1] - Lambda[2], 2*Lambda[1] + 2*Lambda[2])\n```\nThe problem is the -\\rho shift at the end is wrong, probably an overcorrection.\n\nCC:  @sagetrac-sage-combinat @bsalisbury1 @anneschilling @sagetrac-alubovsky\n\nKeywords: **crystals, infinite crystals**\n\nBranch/Commit: **[8de3433](https://github.com/sagemath/sagetrac-mirror/commit/8de3433483f3c8565b1fda1133f412117cedefd5)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21671_\n\n",
    "closed_at": "2016-10-29T14:27:56Z",
    "created_at": "2016-10-09T17:44:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in infinite LS paths crystal operators",
    "type": "issue",
    "updated_at": "2016-10-29T14:27:56Z",
    "url": "https://github.com/sagemath/sage/issues/21671",
    "user": "https://github.com/tscrim"
}
```

```
sage: LS = crystals.infinity.LSPaths(['G',2])
sage: LS.highest_weight_vector().f_string([1,1,1,2])
(5/2*Lambda[1] - Lambda[2], -5/2*Lambda[1] + 5*Lambda[2], Lambda[1] + Lambda[2])
sage: LS.highest_weight_vector().f_string([1,1,1,2,2])
(5*Lambda[1] - 2*Lambda[2], -2*Lambda[1] + 4*Lambda[2], -Lambda[1] - Lambda[2])
sage: LS.highest_weight_vector().f_string([1,1,1,2,2]).e(2)
(5/2*Lambda[1] - Lambda[2], -5/2*Lambda[1] + 5*Lambda[2], -Lambda[1] - Lambda[2], 2*Lambda[1] + 2*Lambda[2])
```
The problem is the -\rho shift at the end is wrong, probably an overcorrection.

CC:  @sagetrac-sage-combinat @bsalisbury1 @anneschilling @sagetrac-alubovsky

Keywords: **crystals, infinite crystals**

Branch/Commit: **[8de3433](https://github.com/sagemath/sagetrac-mirror/commit/8de3433483f3c8565b1fda1133f412117cedefd5)**

Reviewer: **Frédéric Chapoton**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/21671_





---

archive/issue_events_286225.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-09T17:44:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21671#event-286225"
}
```



---

archive/issue_events_286226.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-09T17:44:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21671#event-286226"
}
```



---

archive/issue_events_286227.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-09T17:44:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21671#event-286227"
}
```



---

archive/issue_events_286228.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-09T17:44:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21671#event-286228"
}
```



---

archive/issue_comments_321046.json:
```json
{
    "body": "Commit: **[8de3433](https://github.com/sagemath/sagetrac-mirror/commit/8de3433483f3c8565b1fda1133f412117cedefd5)**",
    "created_at": "2016-10-09T18:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21671#issuecomment-321046",
    "user": "https://github.com/tscrim"
}
```

Commit: **[8de3433](https://github.com/sagemath/sagetrac-mirror/commit/8de3433483f3c8565b1fda1133f412117cedefd5)**



---

archive/issue_comments_321047.json:
```json
{
    "body": "Branch: **[public/crystals/fix_infinite_LS_paths-21671](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/fix_infinite_LS_paths-21671)**",
    "created_at": "2016-10-09T18:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21671#issuecomment-321047",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/crystals/fix_infinite_LS_paths-21671](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/fix_infinite_LS_paths-21671)**



---

archive/issue_events_286229.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-09T18:49:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21671#event-286229"
}
```



---

archive/issue_comments_321048.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nSimple fix: When we reach 0, don't keep subtracting `\\rho`.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8de3433483f3c8565b1fda1133f412117cedefd5\">8de3433</a></td><td><code>Fixing overzealous subtraction of rho in crystal operators of LS(oo).</code></td></tr></table>\n",
    "created_at": "2016-10-09T18:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21671#issuecomment-321048",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

Simple fix: When we reach 0, don't keep subtracting `\rho`.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8de3433483f3c8565b1fda1133f412117cedefd5">8de3433</a></td><td><code>Fixing overzealous subtraction of rho in crystal operators of LS(oo).</code></td></tr></table>




---

archive/issue_comments_321049.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2016-10-09T18:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21671#issuecomment-321049",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_comments_321050.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-10-11T08:19:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21671#issuecomment-321050",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_321051.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nlet it be",
    "created_at": "2016-10-11T08:19:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21671#issuecomment-321051",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

let it be



---

archive/issue_events_286230.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-10-11T08:19:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21671#event-286230"
}
```



---

archive/issue_events_286231.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-10-11T08:19:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21671#event-286231"
}
```



---

archive/issue_events_286232.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-10-29T14:27:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21671#event-286232"
}
```



---

archive/issue_events_286233.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c8b7472bf343361e4f520451c00d9120cebff8d5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-10-29T14:27:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21671#event-286233"
}
```



---

archive/issue_comments_321052.json:
```json
{
    "body": "Changed branch from **[public/crystals/fix_infinite_LS_paths-21671](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/fix_infinite_LS_paths-21671)** to **[8de3433](https://github.com/sagemath/sagetrac-mirror/commit/8de3433483f3c8565b1fda1133f412117cedefd5)**",
    "created_at": "2016-10-29T14:27:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21671#issuecomment-321052",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/crystals/fix_infinite_LS_paths-21671](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/fix_infinite_LS_paths-21671)** to **[8de3433](https://github.com/sagemath/sagetrac-mirror/commit/8de3433483f3c8565b1fda1133f412117cedefd5)**
