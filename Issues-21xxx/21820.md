# Issue 21820: Split src/sage/libs/pari

archive/issues_021583.json:
```json
{
    "assignees": [],
    "body": "Split the directory `src/sage/libs/pari` in two:\n\n- `src/sage/libs/cypari2` contains what will be the `cypari2` package.\n\n- `src/sage/libs/pari` contains what will remain in Sage.\n\nThis will help further development and will make the final separation of `cypari2` as a package very easy.\n\nCC:  @defeo\n\nBranch/Commit: **[27812d0](https://github.com/sagemath/sagetrac-mirror/commit/27812d05c14501b7bdb501d271f4625704a028b7)**\n\nReviewer: **Luca De Feo**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21820_\n\n",
    "closed_at": "2016-11-15T23:25:07Z",
    "created_at": "2016-11-04T14:40:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Split src/sage/libs/pari",
    "type": "issue",
    "updated_at": "2016-11-15T23:25:07Z",
    "url": "https://github.com/sagemath/sage/issues/21820",
    "user": "https://github.com/jdemeyer"
}
```
Split the directory `src/sage/libs/pari` in two:

- `src/sage/libs/cypari2` contains what will be the `cypari2` package.

- `src/sage/libs/pari` contains what will remain in Sage.

This will help further development and will make the final separation of `cypari2` as a package very easy.

CC:  @defeo

Branch/Commit: **[27812d0](https://github.com/sagemath/sagetrac-mirror/commit/27812d05c14501b7bdb501d271f4625704a028b7)**

Reviewer: **Luca De Feo**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/21820_





---

archive/issue_events_288110.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-04T14:40:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288110"
}
```



---

archive/issue_events_288111.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-04T14:40:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288111"
}
```



---

archive/issue_events_288112.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-04T14:40:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288112"
}
```



---

archive/issue_comments_323686.json:
```json
{
    "body": "Dependencies: **#21703**",
    "created_at": "2016-11-04T14:42:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323686",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#21703**



---

archive/issue_comments_323687.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,5 @@\n - `src/sage/libs/cypari2` contains what will be the `cypari2` package.\n \n - `src/sage/libs/pari` contains what will remain in Sage.\n+\n+This will help further development and will make the final separation of `cypari2` as a package very easy.\n``````\n",
    "created_at": "2016-11-04T14:53:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323687",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,5 @@
 - `src/sage/libs/cypari2` contains what will be the `cypari2` package.
 
 - `src/sage/libs/pari` contains what will remain in Sage.
+
+This will help further development and will make the final separation of `cypari2` as a package very easy.
``````




---

archive/issue_comments_323688.json:
```json
{
    "body": "Branch: **[u/jdemeyer/split_src_sage_libs_pari](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/split_src_sage_libs_pari)**",
    "created_at": "2016-11-04T18:12:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323688",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/split_src_sage_libs_pari](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/split_src_sage_libs_pari)**



---

archive/issue_comments_323689.json:
```json
{
    "body": "Commit: **[c9bc359](https://github.com/sagemath/sagetrac-mirror/commit/c9bc3599d4b11a1774fd6bda5935354007868869)**",
    "created_at": "2016-11-04T18:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323689",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[c9bc359](https://github.com/sagemath/sagetrac-mirror/commit/c9bc3599d4b11a1774fd6bda5935354007868869)**



---

archive/issue_comments_323690.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/313b785064eb21e16d55855c4c2216fa26ca8df0\">313b785</a></td><td><code>Interface PARI precision in bits</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f80485ffbc7af58f7da432afb0c642a65cca1f8d\">f80485f</a></td><td><code>Remove obsolete comment about stack increasing</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b70ec86f4f5aed9152123bb79fc5981e743cd19c\">b70ec86</a></td><td><code>Add reference to precision guide in PariInstance.__init__</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b4bc24f4d8b3394ca02a08e0b09e0ccdd89a9fd0\">b4bc24f</a></td><td><code>Merge tag '7.5.beta1' into t/21703/ticket/21703</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3db32759c7a468f35ea578c16eca353b18663b54\">3db3275</a></td><td><code>Only use precisions which are in [33,64] mod 64</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a7099ae741973f0db3a767c8af1f57989ddacf15\">a7099ae</a></td><td><code>Improve documentation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c9bc3599d4b11a1774fd6bda5935354007868869\">c9bc359</a></td><td><code>Split src/sage/libs/pari</code></td></tr></table>\n",
    "created_at": "2016-11-04T18:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323690",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/313b785064eb21e16d55855c4c2216fa26ca8df0">313b785</a></td><td><code>Interface PARI precision in bits</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f80485ffbc7af58f7da432afb0c642a65cca1f8d">f80485f</a></td><td><code>Remove obsolete comment about stack increasing</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b70ec86f4f5aed9152123bb79fc5981e743cd19c">b70ec86</a></td><td><code>Add reference to precision guide in PariInstance.__init__</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b4bc24f4d8b3394ca02a08e0b09e0ccdd89a9fd0">b4bc24f</a></td><td><code>Merge tag '7.5.beta1' into t/21703/ticket/21703</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3db32759c7a468f35ea578c16eca353b18663b54">3db3275</a></td><td><code>Only use precisions which are in [33,64] mod 64</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a7099ae741973f0db3a767c8af1f57989ddacf15">a7099ae</a></td><td><code>Improve documentation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c9bc3599d4b11a1774fd6bda5935354007868869">c9bc359</a></td><td><code>Split src/sage/libs/pari</code></td></tr></table>




---

archive/issue_events_288113.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-04T18:12:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288113"
}
```



---

archive/issue_comments_323691.json:
```json
{
    "body": "Changed commit from **[c9bc359](https://github.com/sagemath/sagetrac-mirror/commit/c9bc3599d4b11a1774fd6bda5935354007868869)** to **[a40a4e8](https://github.com/sagemath/sagetrac-mirror/commit/a40a4e8f07f77147883e4a1ba40f6d5dfb6ce306)**",
    "created_at": "2016-11-10T14:59:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323691",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c9bc359](https://github.com/sagemath/sagetrac-mirror/commit/c9bc3599d4b11a1774fd6bda5935354007868869)** to **[a40a4e8](https://github.com/sagemath/sagetrac-mirror/commit/a40a4e8f07f77147883e4a1ba40f6d5dfb6ce306)**



---

archive/issue_comments_323692.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a40a4e8f07f77147883e4a1ba40f6d5dfb6ce306\">a40a4e8</a></td><td><code>Merge tag '7.5.beta2' into t/21820/split_src_sage_libs_pari</code></td></tr></table>\n",
    "created_at": "2016-11-10T14:59:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323692",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a40a4e8f07f77147883e4a1ba40f6d5dfb6ce306">a40a4e8</a></td><td><code>Merge tag '7.5.beta2' into t/21820/split_src_sage_libs_pari</code></td></tr></table>




---

archive/issue_comments_323693.json:
```json
{
    "body": "Changed dependencies from **#21703** to none",
    "created_at": "2016-11-10T14:59:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323693",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **#21703** to none



---

archive/issue_comments_323694.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nCompilation fails complaining about missing `sage/libs/pari/paridecl.pxd` during the auto-generation step. Is this expected?",
    "created_at": "2016-11-10T16:26:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323694",
    "user": "https://github.com/defeo"
}
```

<a id='comment:7'>Comment 7:</a>
Compilation fails complaining about missing `sage/libs/pari/paridecl.pxd` during the auto-generation step. Is this expected?



---

archive/issue_events_288114.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-10T16:50:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288114"
}
```



---

archive/issue_events_288115.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-10T16:50:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288115"
}
```



---

archive/issue_comments_323695.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThat was a bad merge with #21821.",
    "created_at": "2016-11-10T16:50:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323695",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'>Comment 9:</a>
That was a bad merge with #21821.



---

archive/issue_comments_323696.json:
```json
{
    "body": "Changed commit from **[a40a4e8](https://github.com/sagemath/sagetrac-mirror/commit/a40a4e8f07f77147883e4a1ba40f6d5dfb6ce306)** to **[0e98379](https://github.com/sagemath/sagetrac-mirror/commit/0e983795ff95d8eaffddfaa4c3a585d051df44bb)**",
    "created_at": "2016-11-10T19:20:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323696",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a40a4e8](https://github.com/sagemath/sagetrac-mirror/commit/a40a4e8f07f77147883e4a1ba40f6d5dfb6ce306)** to **[0e98379](https://github.com/sagemath/sagetrac-mirror/commit/0e983795ff95d8eaffddfaa4c3a585d051df44bb)**



---

archive/issue_comments_323697.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0e983795ff95d8eaffddfaa4c3a585d051df44bb\">0e98379</a></td><td><code>Fix sage_src_pari()</code></td></tr></table>\n",
    "created_at": "2016-11-10T19:20:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323697",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:10'>Comment 10:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0e983795ff95d8eaffddfaa4c3a585d051df44bb">0e98379</a></td><td><code>Fix sage_src_pari()</code></td></tr></table>




---

archive/issue_events_288116.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-10T19:21:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288116"
}
```



---

archive/issue_events_288117.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-10T19:21:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288117"
}
```



---

archive/issue_comments_323698.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nStarting from a non-clean repo, this leaves behind `src/sage/libs/pari/auto_gen.pxi` and `src/sage/libs/pari/auto_instance.pxi` (easily spotted by `git status`). Removing them by hand causes a doctest failure.\n\nIs this the expected behaviour?",
    "created_at": "2016-11-11T00:44:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323698",
    "user": "https://github.com/defeo"
}
```

<a id='comment:12'>Comment 12:</a>
Starting from a non-clean repo, this leaves behind `src/sage/libs/pari/auto_gen.pxi` and `src/sage/libs/pari/auto_instance.pxi` (easily spotted by `git status`). Removing them by hand causes a doctest failure.

Is this the expected behaviour?



---

archive/issue_events_288118.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-11T08:25:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288118"
}
```



---

archive/issue_events_288119.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-11T08:25:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288119"
}
```



---

archive/issue_comments_323699.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nReplying to [@defeo](#comment%3A12):\n> Removing them by hand causes a doctest failure.\n\nDetails please...",
    "created_at": "2016-11-11T15:41:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323699",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:14'>Comment 14:</a>
Replying to [@defeo](#comment%3A12):
> Removing them by hand causes a doctest failure.

Details please...



---

archive/issue_comments_323700.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\n\n```\n$ sage -t src/sage_setup/clean.py\ntoo many failed tests, not using stored timings\nRunning doctests with ID 2016-11-11-16-46-20-c1cba75e.\nGit branch: t/21820/split_src_sage_libs_pari\nUsing --optional=atlas,mpir,python2,sage\nDoctesting 1 file.\nsage -t src/sage_setup/clean.py\n**********************************************************************\nFile \"src/sage_setup/clean.py\", line 87, in sage_setup.clean._find_stale_files\nFailed example:\n    for f in stale_iter:\n        if f.endswith(skip_extensions): continue\n        print('Found stale file: ' + f)\nExpected nothing\nGot:\n    Found stale file: sage/libs/pari/auto_gen.pxi\n    Found stale file: sage/libs/pari/auto_instance.pxi\n**********************************************************************\n1 item had failures:\n   1 of  10 in sage_setup.clean._find_stale_files\n    [15 tests, 1 failure, 0.13 s]\n----------------------------------------------------------------------\nsage -t src/sage_setup/clean.py  # 1 doctest failed\n----------------------------------------------------------------------\nTotal time for all tests: 0.6 seconds\n    cpu time: 0.1 seconds\n    cumulative wall time: 0.1 seconds\n```",
    "created_at": "2016-11-11T15:47:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323700",
    "user": "https://github.com/defeo"
}
```

<a id='comment:15'>Comment 15:</a>

```
$ sage -t src/sage_setup/clean.py
too many failed tests, not using stored timings
Running doctests with ID 2016-11-11-16-46-20-c1cba75e.
Git branch: t/21820/split_src_sage_libs_pari
Using --optional=atlas,mpir,python2,sage
Doctesting 1 file.
sage -t src/sage_setup/clean.py
**********************************************************************
File "src/sage_setup/clean.py", line 87, in sage_setup.clean._find_stale_files
Failed example:
    for f in stale_iter:
        if f.endswith(skip_extensions): continue
        print('Found stale file: ' + f)
Expected nothing
Got:
    Found stale file: sage/libs/pari/auto_gen.pxi
    Found stale file: sage/libs/pari/auto_instance.pxi
**********************************************************************
1 item had failures:
   1 of  10 in sage_setup.clean._find_stale_files
    [15 tests, 1 failure, 0.13 s]
----------------------------------------------------------------------
sage -t src/sage_setup/clean.py  # 1 doctest failed
----------------------------------------------------------------------
Total time for all tests: 0.6 seconds
    cpu time: 0.1 seconds
    cumulative wall time: 0.1 seconds
```



---

archive/issue_comments_323701.json:
```json
{
    "body": "Changed commit from **[0e98379](https://github.com/sagemath/sagetrac-mirror/commit/0e983795ff95d8eaffddfaa4c3a585d051df44bb)** to **[27812d0](https://github.com/sagemath/sagetrac-mirror/commit/27812d05c14501b7bdb501d271f4625704a028b7)**",
    "created_at": "2016-11-11T15:48:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323701",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0e98379](https://github.com/sagemath/sagetrac-mirror/commit/0e983795ff95d8eaffddfaa4c3a585d051df44bb)** to **[27812d0](https://github.com/sagemath/sagetrac-mirror/commit/27812d05c14501b7bdb501d271f4625704a028b7)**



---

archive/issue_comments_323702.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/27812d05c14501b7bdb501d271f4625704a028b7\">27812d0</a></td><td><code>Remove old auto-generated files</code></td></tr></table>\n",
    "created_at": "2016-11-11T15:48:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323702",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:16'>Comment 16:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/27812d05c14501b7bdb501d271f4625704a028b7">27812d0</a></td><td><code>Remove old auto-generated files</code></td></tr></table>




---

archive/issue_events_288120.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-11T15:49:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288120"
}
```



---

archive/issue_events_288121.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-11T15:49:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288121"
}
```



---

archive/issue_events_288122.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2016-11-11T16:23:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288122"
}
```



---

archive/issue_events_288123.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2016-11-11T16:23:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288123"
}
```



---

archive/issue_comments_323703.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nI don't understand why the fix works, but it does. Good to go.",
    "created_at": "2016-11-11T16:23:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323703",
    "user": "https://github.com/defeo"
}
```

<a id='comment:18'>Comment 18:</a>
I don't understand why the fix works, but it does. Good to go.



---

archive/issue_events_288124.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-11-11T17:56:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288124"
}
```



---

archive/issue_events_288125.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-11-11T17:56:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288125"
}
```



---

archive/issue_comments_323704.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nReviweer name",
    "created_at": "2016-11-11T17:56:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323704",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:19'>Comment 19:</a>
Reviweer name



---

archive/issue_comments_323705.json:
```json
{
    "body": "Reviewer: **Luca De Feo**",
    "created_at": "2016-11-11T19:34:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323705",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Luca De Feo**



---

archive/issue_events_288126.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-11T19:34:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288126"
}
```



---

archive/issue_events_288127.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-11T19:34:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288127"
}
```



---

archive/issue_comments_323706.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/split_src_sage_libs_pari](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/split_src_sage_libs_pari)** to **[27812d0](https://github.com/sagemath/sagetrac-mirror/commit/27812d05c14501b7bdb501d271f4625704a028b7)**",
    "created_at": "2016-11-15T23:25:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21820#issuecomment-323706",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/split_src_sage_libs_pari](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/split_src_sage_libs_pari)** to **[27812d0](https://github.com/sagemath/sagetrac-mirror/commit/27812d05c14501b7bdb501d271f4625704a028b7)**



---

archive/issue_events_288128.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-11-15T23:25:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288128"
}
```



---

archive/issue_events_288129.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c9ac50948b490d025c2a03a069a0c753e8140066",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-11-15T23:25:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21820",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21820#event-288129"
}
```
