# Issue 21960: Get rid of six.itervalues in Cython

archive/issues_021723.json:
```json
{
    "assignees": [],
    "body": "In Cython code, there is no need for\n\n```\nfrom six import itervalues\n```\nsince it generates optimized code for `foo.itervalues()` if `foo` is a `dict`.\n\nCC:  @fchapoton\n\nBranch/Commit: **[15b36aa](https://github.com/sagemath/sagetrac-mirror/commit/15b36aa7da170f6e386e609c6fe30522ccefc4bf)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21960_\n\n",
    "closed_at": "2016-11-27T16:45:30Z",
    "created_at": "2016-11-24T14:17:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Get rid of six.itervalues in Cython",
    "type": "issue",
    "updated_at": "2016-11-27T16:45:30Z",
    "url": "https://github.com/sagemath/sage/issues/21960",
    "user": "https://github.com/jdemeyer"
}
```
In Cython code, there is no need for

```
from six import itervalues
```
since it generates optimized code for `foo.itervalues()` if `foo` is a `dict`.

CC:  @fchapoton

Branch/Commit: **[15b36aa](https://github.com/sagemath/sagetrac-mirror/commit/15b36aa7da170f6e386e609c6fe30522ccefc4bf)**

Reviewer: **Frédéric Chapoton**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/21960_





---

archive/issue_events_289817.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-24T14:17:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289817"
}
```



---

archive/issue_events_289818.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-24T14:17:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289818"
}
```



---

archive/issue_events_289819.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-24T14:17:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289819"
}
```



---

archive/issue_events_289820.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-24T14:17:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289820"
}
```



---

archive/issue_comments_326203.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/21960](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/21960)**",
    "created_at": "2016-11-24T14:34:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21960#issuecomment-326203",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/21960](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/21960)**



---

archive/issue_comments_326204.json:
```json
{
    "body": "Commit: **[15b36aa](https://github.com/sagemath/sagetrac-mirror/commit/15b36aa7da170f6e386e609c6fe30522ccefc4bf)**",
    "created_at": "2016-11-24T14:36:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21960#issuecomment-326204",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[15b36aa](https://github.com/sagemath/sagetrac-mirror/commit/15b36aa7da170f6e386e609c6fe30522ccefc4bf)**



---

archive/issue_events_289821.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-24T14:36:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289821"
}
```



---

archive/issue_comments_326205.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/15b36aa7da170f6e386e609c6fe30522ccefc4bf\">15b36aa</a></td><td><code>Get rid six.itervalues in Cython</code></td></tr></table>\n",
    "created_at": "2016-11-24T14:36:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21960#issuecomment-326205",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/15b36aa7da170f6e386e609c6fe30522ccefc4bf">15b36aa</a></td><td><code>Get rid six.itervalues in Cython</code></td></tr></table>




---

archive/issue_events_289822.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-24T14:37:08Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "title_is": "Get rid of six.itervalues in Cython",
    "title_was": "Get rid six.itervalues in Cython",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289822"
}
```



---

archive/issue_events_289823.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-25T07:15:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289823"
}
```



---

archive/issue_events_289824.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-25T07:15:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289824"
}
```



---

archive/issue_comments_326206.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-11-25T07:15:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21960#issuecomment-326206",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_326207.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nok, good",
    "created_at": "2016-11-25T07:15:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21960#issuecomment-326207",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
ok, good



---

archive/issue_comments_326208.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/21960](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/21960)** to **[15b36aa](https://github.com/sagemath/sagetrac-mirror/commit/15b36aa7da170f6e386e609c6fe30522ccefc4bf)**",
    "created_at": "2016-11-27T16:45:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21960#issuecomment-326208",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/21960](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/21960)** to **[15b36aa](https://github.com/sagemath/sagetrac-mirror/commit/15b36aa7da170f6e386e609c6fe30522ccefc4bf)**



---

archive/issue_events_289825.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-11-27T16:45:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289825"
}
```



---

archive/issue_events_289826.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "dc97fb232a19b8bfd7a388bc645819e6a77956c7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-11-27T16:45:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21960",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21960#event-289826"
}
```
