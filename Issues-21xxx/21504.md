# Issue 21504: Numerical noise in polynomial_element.pyx

archive/issues_021267.json:
```json
{
    "body": "```\nsage -t --long src/sage/rings/polynomial/polynomial_element.pyx\n**********************************************************************\nFile \"src/sage/rings/polynomial/polynomial_element.pyx\", line 6533, in sage.rings.polynomial.polynomial_element.Polynomial.roots\nFailed example:\n    [z^5 for z in f.roots(multiplicities=False)]  # abs tol 1e-14\nExpected:\n    [0.9999999999999957 - 1.2864981197413038e-15*I, 0.9999999999999976 + 3.062854959141552e-15*I, 1.0000000000000024 + 1.1331077795295987e-15*I, 0.9999999999999953 - 2.0212861992297117e-15*I]\nGot:\n    [0.9999999999999999 - 3.062854959141559e-15*I,\n     1.0000000000000082 + 3.0628549591415848e-15*I,\n     1.0000000000000038 - 6.432490598706571e-16*I,\n     1.0000000000000022 + 3.3077843189710377e-15*I]\nTolerance exceeded in 1 of 8:\n    0.9999999999999976 vs 1.0000000000000082, tolerance 1e-14 > 1e-14\n**********************************************************************\n1 item had failures:\n   1 of 180 in sage.rings.polynomial.polynomial_element.Polynomial.roots\n    [2123 tests, 1 failure, 37.58 s]\n```\n\nKeywords: random_fail\n\nBranch/Commit: 1c8ad5fbdceedeeb5f489207e2af0541d7bfb44f\n\nReviewer: John Palmieri\n\nAuthor: Volker Braun\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/21504\n\n",
    "closed_at": "2016-10-11T22:44:52Z",
    "created_at": "2016-09-16T06:53:55Z",
    "labels": [
        "component: numerical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.4",
    "title": "Numerical noise in polynomial_element.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/21504",
    "user": "https://github.com/vbraun"
}
```
```
sage -t --long src/sage/rings/polynomial/polynomial_element.pyx
**********************************************************************
File "src/sage/rings/polynomial/polynomial_element.pyx", line 6533, in sage.rings.polynomial.polynomial_element.Polynomial.roots
Failed example:
    [z^5 for z in f.roots(multiplicities=False)]  # abs tol 1e-14
Expected:
    [0.9999999999999957 - 1.2864981197413038e-15*I, 0.9999999999999976 + 3.062854959141552e-15*I, 1.0000000000000024 + 1.1331077795295987e-15*I, 0.9999999999999953 - 2.0212861992297117e-15*I]
Got:
    [0.9999999999999999 - 3.062854959141559e-15*I,
     1.0000000000000082 + 3.0628549591415848e-15*I,
     1.0000000000000038 - 6.432490598706571e-16*I,
     1.0000000000000022 + 3.3077843189710377e-15*I]
Tolerance exceeded in 1 of 8:
    0.9999999999999976 vs 1.0000000000000082, tolerance 1e-14 > 1e-14
**********************************************************************
1 item had failures:
   1 of 180 in sage.rings.polynomial.polynomial_element.Polynomial.roots
    [2123 tests, 1 failure, 37.58 s]
```

Keywords: random_fail

Branch/Commit: 1c8ad5fbdceedeeb5f489207e2af0541d7bfb44f

Reviewer: John Palmieri

Author: Volker Braun

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/21504





---

archive/issue_comments_396190.json:
```json
{
    "body": "<a id='comment:1'></a>Got this once on OSX, doesn't seem to be reproducable.",
    "created_at": "2016-09-16T06:54:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396190",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'></a>Got this once on OSX, doesn't seem to be reproducable.



---

archive/issue_comments_396191.json:
```json
{
    "body": "<a id='comment:2'></a>It's reproducible for me: every time on OS X 10.11.",
    "created_at": "2016-10-07T18:10:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396191",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>It's reproducible for me: every time on OS X 10.11.



---

archive/issue_comments_396192.json:
```json
{
    "body": "Changing branch from \"\" to \"u/vbraun/numerical_noise_in_polynomial_element_pyx\"",
    "created_at": "2016-10-08T23:15:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396192",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "" to "u/vbraun/numerical_noise_in_polynomial_element_pyx"



---

archive/issue_comments_396193.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2016-10-08T23:15:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396193",
    "user": "https://github.com/vbraun"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_396194.json:
```json
{
    "body": "<a id='comment:4'></a>New commits:",
    "created_at": "2016-10-08T23:15:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396194",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'></a>New commits:



---

archive/issue_comments_396195.json:
```json
{
    "body": "Changing author from \"\" to \"Volker Braun\"",
    "created_at": "2016-10-08T23:15:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396195",
    "user": "https://github.com/vbraun"
}
```

Changing author from "" to "Volker Braun"



---

archive/issue_comments_396196.json:
```json
{
    "body": "Changing commit from \"\" to \"1c8ad5fbdceedeeb5f489207e2af0541d7bfb44f\"",
    "created_at": "2016-10-08T23:15:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396196",
    "user": "https://github.com/vbraun"
}
```

Changing commit from "" to "1c8ad5fbdceedeeb5f489207e2af0541d7bfb44f"



---

archive/issue_comments_396197.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-10-09T01:29:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396197",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_396198.json:
```json
{
    "body": "<a id='comment:5'></a>Looks good to me.",
    "created_at": "2016-10-09T01:29:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396198",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>Looks good to me.



---

archive/issue_comments_396199.json:
```json
{
    "body": "Changing reviewer from \"\" to \"John Palmieri\"",
    "created_at": "2016-10-09T01:29:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396199",
    "user": "https://github.com/jhpalmieri"
}
```

Changing reviewer from "" to "John Palmieri"



---

archive/issue_events_057071.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-10-11T22:44:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/21504#event-57071"
}
```



---

archive/issue_comments_396200.json:
```json
{
    "body": "Changing branch from \"u/vbraun/numerical_noise_in_polynomial_element_pyx\" to \"1c8ad5fbdceedeeb5f489207e2af0541d7bfb44f\"",
    "created_at": "2016-10-11T22:44:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396200",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/vbraun/numerical_noise_in_polynomial_element_pyx" to "1c8ad5fbdceedeeb5f489207e2af0541d7bfb44f"



---

archive/issue_comments_396201.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2016-10-11T22:44:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21504#issuecomment-396201",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
