# Issue 21601: Add canonical meetands

archive/issues_021364.json:
```json
{
    "assignees": [],
    "body": "For symmetry with #21597 after that has been closed.\n\nCC:  @tscrim\n\nBranch/Commit: **[cb7b710](https://github.com/sagemath/sagetrac-mirror/commit/cb7b7101a5bb41f76fb059f7d620b1ef3545504f)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21601_\n\n",
    "closed_at": "2016-11-02T19:20:00Z",
    "created_at": "2016-09-26T18:51:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add canonical meetands",
    "type": "issue",
    "updated_at": "2016-11-02T19:20:00Z",
    "url": "https://github.com/sagemath/sage/issues/21601",
    "user": "https://github.com/jm58660"
}
```
For symmetry with #21597 after that has been closed.

CC:  @tscrim

Branch/Commit: **[cb7b710](https://github.com/sagemath/sagetrac-mirror/commit/cb7b7101a5bb41f76fb059f7d620b1ef3545504f)**

Reviewer: **Travis Scrimshaw**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/21601_





---

archive/issue_events_285396.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-09-26T18:51:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285396"
}
```



---

archive/issue_events_285397.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-09-26T18:51:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285397"
}
```



---

archive/issue_events_285398.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-09-26T18:51:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285398"
}
```



---

archive/issue_events_285399.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-09-26T18:51:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285399"
}
```



---

archive/issue_comments_319989.json:
```json
{
    "body": "Branch: **[u/jmantysalo/add_canonical_meetands](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/add_canonical_meetands)**",
    "created_at": "2016-10-23T07:32:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21601#issuecomment-319989",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/add_canonical_meetands](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/add_canonical_meetands)**



---

archive/issue_comments_319990.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nDone this. The code is just a direct dual. To test try\n\n```\nn = 30; seed = 42; p = 0.999\nset_random_seed(seed)\nL = Posets.RandomLattice(n, p)\nL_ = L.dual()\nfor e in L:\n    if L.canonical_joinands(e) is None and L_.canonical_meetands(e) is None:\n        continue\n    if sorted(L.canonical_joinands(e)) != sorted(L_.canonical_meetands(e)):\n        print(\"ERROR\")\n        break\nelse:\n    print(\"OK\")\n```\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cb7b7101a5bb41f76fb059f7d620b1ef3545504f\">cb7b710</a></td><td><code>Add canonical meetands.</code></td></tr></table>\n",
    "created_at": "2016-10-23T07:35:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21601#issuecomment-319990",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:2" align="right">Comment 2</div>

Done this. The code is just a direct dual. To test try

```
n = 30; seed = 42; p = 0.999
set_random_seed(seed)
L = Posets.RandomLattice(n, p)
L_ = L.dual()
for e in L:
    if L.canonical_joinands(e) is None and L_.canonical_meetands(e) is None:
        continue
    if sorted(L.canonical_joinands(e)) != sorted(L_.canonical_meetands(e)):
        print("ERROR")
        break
else:
    print("OK")
```

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cb7b7101a5bb41f76fb059f7d620b1ef3545504f">cb7b710</a></td><td><code>Add canonical meetands.</code></td></tr></table>




---

archive/issue_comments_319991.json:
```json
{
    "body": "Commit: **[cb7b710](https://github.com/sagemath/sagetrac-mirror/commit/cb7b7101a5bb41f76fb059f7d620b1ef3545504f)**",
    "created_at": "2016-10-23T07:35:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21601#issuecomment-319991",
    "user": "https://github.com/jm58660"
}
```

Commit: **[cb7b710](https://github.com/sagemath/sagetrac-mirror/commit/cb7b7101a5bb41f76fb059f7d620b1ef3545504f)**



---

archive/issue_events_285400.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-23T07:35:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285400"
}
```



---

archive/issue_events_285401.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-23T07:35:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285401"
}
```



---

archive/issue_events_285402.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-23T07:35:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285402"
}
```



---

archive/issue_events_285403.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-23T14:06:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285403"
}
```



---

archive/issue_events_285404.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-23T14:06:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285404"
}
```



---

archive/issue_comments_319992.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2016-10-23T14:06:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21601#issuecomment-319992",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_319993.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-10-23T14:06:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21601#issuecomment-319993",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_285405.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-11-02T19:20:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285405"
}
```



---

archive/issue_events_285406.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9f707f87eb81795d36af4d24a3c4ac15c6d7d54d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-11-02T19:20:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21601#event-285406"
}
```



---

archive/issue_comments_319994.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/add_canonical_meetands](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/add_canonical_meetands)** to **[cb7b710](https://github.com/sagemath/sagetrac-mirror/commit/cb7b7101a5bb41f76fb059f7d620b1ef3545504f)**",
    "created_at": "2016-11-02T19:20:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21601#issuecomment-319994",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/add_canonical_meetands](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/add_canonical_meetands)** to **[cb7b710](https://github.com/sagemath/sagetrac-mirror/commit/cb7b7101a5bb41f76fb059f7d620b1ef3545504f)**
