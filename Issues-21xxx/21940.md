# Issue 21940: Doctest for: Positive raised by a positive power is not known to be real

archive/issues_021703.json:
```json
{
    "assignees": [],
    "body": "But it is known to be positive...\n\nSee the following code:\n\n```python\nsage: assume(x>0)\nsage: x.is_real()\nTrue\nsage: x.is_positive()\nTrue\nsage: (x**x).is_positive()\nTrue\nsage: (x**x).is_real()\nFalse\n```\n\nFor comparison, this is how SymPy handles this:\n\n```python\nIn [2]: x = Symbol('x', positive=True)\n\nIn [3]: x.is_positive\nOut[3]: True\n\nIn [4]: x.is_real\nOut[4]: True\n\nIn [5]: (x**x).is_positive\nOut[5]: True\n\nIn [6]: (x**x).is_real\nOut[6]: True\n```\n\nDepends on #21963\n\nCC:  @rwst\n\nKeywords: **is_real, days79**\n\nBranch/Commit: **[c883299](https://github.com/sagemath/sagetrac-mirror/commit/c88329909934b9561314344347a7bd7c3c0ec755)**\n\nReviewer: **Ralf Stephan, Peleg Michaeli**\n\nAuthor: **Peleg Michaeli, Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21940_\n\n",
    "closed_at": "2017-03-27T20:42:47Z",
    "created_at": "2016-11-23T07:50:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Doctest for: Positive raised by a positive power is not known to be real",
    "type": "issue",
    "updated_at": "2017-03-27T20:42:47Z",
    "url": "https://github.com/sagemath/sage/issues/21940",
    "user": "https://github.com/pelegm"
}
```
But it is known to be positive...

See the following code:

```python
sage: assume(x>0)
sage: x.is_real()
True
sage: x.is_positive()
True
sage: (x**x).is_positive()
True
sage: (x**x).is_real()
False
```

For comparison, this is how SymPy handles this:

```python
In [2]: x = Symbol('x', positive=True)

In [3]: x.is_positive
Out[3]: True

In [4]: x.is_real
Out[4]: True

In [5]: (x**x).is_positive
Out[5]: True

In [6]: (x**x).is_real
Out[6]: True
```

Depends on #21963

CC:  @rwst

Keywords: **is_real, days79**

Branch/Commit: **[c883299](https://github.com/sagemath/sagetrac-mirror/commit/c88329909934b9561314344347a7bd7c3c0ec755)**

Reviewer: **Ralf Stephan, Peleg Michaeli**

Author: **Peleg Michaeli, Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/21940_





---

archive/issue_events_289577.json:
```json
{
    "actor": "https://github.com/pelegm",
    "created_at": "2016-11-23T07:50:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289577"
}
```



---

archive/issue_events_289578.json:
```json
{
    "actor": "https://github.com/pelegm",
    "created_at": "2016-11-23T07:50:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289578"
}
```



---

archive/issue_events_289579.json:
```json
{
    "actor": "https://github.com/pelegm",
    "created_at": "2016-11-23T07:50:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289579"
}
```



---

archive/issue_events_289580.json:
```json
{
    "actor": "https://github.com/pelegm",
    "created_at": "2016-11-23T07:50:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289580"
}
```



---

archive/issue_comments_325829.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nWhat Sage version is this? With Sage-7.5beta3 I get:\n\n```\nsage: assume(x>0)\nsage: x.is_real()\nTrue\nsage: (x*x).is_real()\nTrue\n```",
    "created_at": "2016-11-23T14:07:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325829",
    "user": "https://github.com/rwst"
}
```

<a id='comment:3'>Comment 3:</a>
What Sage version is this? With Sage-7.5beta3 I get:

```
sage: assume(x>0)
sage: x.is_real()
True
sage: (x*x).is_real()
True
```



---

archive/issue_comments_325830.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nReplying to [@rwst](#comment%3A3):\n> What Sage version is this? With Sage-7.5beta3 I get:\n> \n> ```\n> sage: assume(x>0)\n> sage: x.is_real()\n> True\n> sage: (x*x).is_real()\n> True\n> ```\n\nHi,\n\nThis was tested on 7.5beta3.  You've checked `x*x`, and I have checked `x**x`.",
    "created_at": "2016-11-25T09:15:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325830",
    "user": "https://github.com/pelegm"
}
```

<a id='comment:4'>Comment 4:</a>
Replying to [@rwst](#comment%3A3):
> What Sage version is this? With Sage-7.5beta3 I get:
> 
> ```
> sage: assume(x>0)
> sage: x.is_real()
> True
> sage: (x*x).is_real()
> True
> ```

Hi,

This was tested on 7.5beta3.  You've checked `x*x`, and I have checked `x**x`.



---

archive/issue_comments_325831.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nFixed in pynac git master, see https://github.com/pynac/pynac/commit/e5c6cd261e2652b7c4e7b4ca5f1bac9565ab0ef8\n\nPlease add a doctest to `Expression.is_real()` in `symbolic/expression.pyx`",
    "created_at": "2016-11-25T14:27:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325831",
    "user": "https://github.com/rwst"
}
```

<a id='comment:5'>Comment 5:</a>
Fixed in pynac git master, see https://github.com/pynac/pynac/commit/e5c6cd261e2652b7c4e7b4ca5f1bac9565ab0ef8

Please add a doctest to `Expression.is_real()` in `symbolic/expression.pyx`



---

archive/issue_comments_325832.json:
```json
{
    "body": "Dependencies: **pynac-0.7.2**",
    "created_at": "2016-11-25T14:27:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325832",
    "user": "https://github.com/rwst"
}
```

Dependencies: **pynac-0.7.2**



---

archive/issue_comments_325833.json:
```json
{
    "body": "Commit: **[65e6c10](https://github.com/sagemath/sagetrac-mirror/commit/65e6c10c2da5e4d20bbc6f7f225ade3f4778fc0c)**",
    "created_at": "2016-11-25T21:29:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325833",
    "user": "https://github.com/pelegm"
}
```

Commit: **[65e6c10](https://github.com/sagemath/sagetrac-mirror/commit/65e6c10c2da5e4d20bbc6f7f225ade3f4778fc0c)**



---

archive/issue_comments_325834.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/65e6c10c2da5e4d20bbc6f7f225ade3f4778fc0c\">65e6c10</a></td><td><code>positive ** positive = real (doctest only) #21940</code></td></tr></table>\n",
    "created_at": "2016-11-25T21:29:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325834",
    "user": "https://github.com/pelegm"
}
```

<a id='comment:6'>Comment 6:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/65e6c10c2da5e4d20bbc6f7f225ade3f4778fc0c">65e6c10</a></td><td><code>positive ** positive = real (doctest only) #21940</code></td></tr></table>




---

archive/issue_comments_325835.json:
```json
{
    "body": "Branch: **[u/pelegm/21940](https://github.com/sagemath/sagetrac-mirror/tree/u/pelegm/21940)**",
    "created_at": "2016-11-25T21:29:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325835",
    "user": "https://github.com/pelegm"
}
```

Branch: **[u/pelegm/21940](https://github.com/sagemath/sagetrac-mirror/tree/u/pelegm/21940)**



---

archive/issue_events_289581.json:
```json
{
    "actor": "https://github.com/pelegm",
    "created_at": "2016-11-25T21:29:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289581"
}
```



---

archive/issue_comments_325836.json:
```json
{
    "body": "Author: **Peleg Michaeli**",
    "created_at": "2016-11-25T21:29:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325836",
    "user": "https://github.com/pelegm"
}
```

Author: **Peleg Michaeli**



---

archive/issue_comments_325837.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nMaybe interesting in this regard that fixing this allows Sage to simplify some apparently complex expressions, e.g.\n\n```\nBefore:\nsqrt(abs(1/25*(3*(2*sqrt(3)*sqrt(2) - 3)*(sqrt(6) - 2) + 16*sqrt(3)*sqrt(2) +\n5*sqrt(6) - 54)^2 + 1/25*(3*(sqrt(6) - 2)*(sqrt(6) - 4) + 14*sqrt(3)*sqrt(2) +\n4*sqrt(6) - 42)^2 + 144/25*(sqrt(3)*sqrt(2) - sqrt(6))^2))\n\nAfter:\nsqrt(1/25*(3*(2*sqrt(3)*sqrt(2) - 3)*(sqrt(6) - 2) + 16*sqrt(3)*sqrt(2) +\n5*sqrt(6) - 54)^2 + 1/25*(3*(sqrt(6) - 2)*(sqrt(6) - 4) + 14*sqrt(3)*sqrt(2) +\n4*sqrt(6) - 42)^2 + 144/25*(sqrt(3)*sqrt(2) - sqrt(6))^2)\n```\n(from a doctest in `matrix_symbolic_dense.pyx`)",
    "created_at": "2016-12-06T08:52:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325837",
    "user": "https://github.com/rwst"
}
```

<a id='comment:7'>Comment 7:</a>
Maybe interesting in this regard that fixing this allows Sage to simplify some apparently complex expressions, e.g.

```
Before:
sqrt(abs(1/25*(3*(2*sqrt(3)*sqrt(2) - 3)*(sqrt(6) - 2) + 16*sqrt(3)*sqrt(2) +
5*sqrt(6) - 54)^2 + 1/25*(3*(sqrt(6) - 2)*(sqrt(6) - 4) + 14*sqrt(3)*sqrt(2) +
4*sqrt(6) - 42)^2 + 144/25*(sqrt(3)*sqrt(2) - sqrt(6))^2))

After:
sqrt(1/25*(3*(2*sqrt(3)*sqrt(2) - 3)*(sqrt(6) - 2) + 16*sqrt(3)*sqrt(2) +
5*sqrt(6) - 54)^2 + 1/25*(3*(sqrt(6) - 2)*(sqrt(6) - 4) + 14*sqrt(3)*sqrt(2) +
4*sqrt(6) - 42)^2 + 144/25*(sqrt(3)*sqrt(2) - sqrt(6))^2)
```
(from a doctest in `matrix_symbolic_dense.pyx`)



---

archive/issue_comments_325838.json:
```json
{
    "body": "Description changed:\n``````diff\n\n``````\n",
    "created_at": "2016-12-06T12:52:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325838",
    "user": "https://github.com/pelegm"
}
```

Description changed:
``````diff

``````




---

archive/issue_comments_325839.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nIs this solving the case\n\n```python\nsage: assume(x, 'real')\nsage: (2**x).is_positive()\nTrue\nsage: (2**x).is_real()\nFalse\n```\nas well?",
    "created_at": "2016-12-06T12:53:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325839",
    "user": "https://github.com/pelegm"
}
```

<a id='comment:9'>Comment 9:</a>
Is this solving the case

```python
sage: assume(x, 'real')
sage: (2**x).is_positive()
True
sage: (2**x).is_real()
False
```
as well?



---

archive/issue_comments_325840.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nRight, now that works too.",
    "created_at": "2016-12-06T15:44:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325840",
    "user": "https://github.com/rwst"
}
```

<a id='comment:10'>Comment 10:</a>
Right, now that works too.



---

archive/issue_comments_325841.json:
```json
{
    "body": "Changed dependencies from **pynac-0.7.2** to **#21963**",
    "created_at": "2016-12-06T15:44:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325841",
    "user": "https://github.com/rwst"
}
```

Changed dependencies from **pynac-0.7.2** to **#21963**



---

archive/issue_comments_325842.json:
```json
{
    "body": "Changed branch from **[u/pelegm/21940](https://github.com/sagemath/sagetrac-mirror/tree/u/pelegm/21940)** to **[u/rws/21940](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/21940)**",
    "created_at": "2016-12-20T15:41:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325842",
    "user": "https://github.com/rwst"
}
```

Changed branch from **[u/pelegm/21940](https://github.com/sagemath/sagetrac-mirror/tree/u/pelegm/21940)** to **[u/rws/21940](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/21940)**



---

archive/issue_events_289582.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-12-20T15:43:51Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "title_is": "Doctest for: Positive raised by a positive power is not known to be real",
    "title_was": "Positive raised by a positive power is not known to be real",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289582"
}
```



---

archive/issue_comments_325843.json:
```json
{
    "body": "Changed author from **Peleg Michaeli** to **Peleg Michaeli, Ralf Stephan**",
    "created_at": "2016-12-20T15:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325843",
    "user": "https://github.com/rwst"
}
```

Changed author from **Peleg Michaeli** to **Peleg Michaeli, Ralf Stephan**



---

archive/issue_comments_325844.json:
```json
{
    "body": "Changed commit from **[65e6c10](https://github.com/sagemath/sagetrac-mirror/commit/65e6c10c2da5e4d20bbc6f7f225ade3f4778fc0c)** to **[c883299](https://github.com/sagemath/sagetrac-mirror/commit/c88329909934b9561314344347a7bd7c3c0ec755)**",
    "created_at": "2016-12-20T15:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325844",
    "user": "https://github.com/rwst"
}
```

Changed commit from **[65e6c10](https://github.com/sagemath/sagetrac-mirror/commit/65e6c10c2da5e4d20bbc6f7f225ade3f4778fc0c)** to **[c883299](https://github.com/sagemath/sagetrac-mirror/commit/c88329909934b9561314344347a7bd7c3c0ec755)**



---

archive/issue_comments_325845.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2016-12-20T15:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325845",
    "user": "https://github.com/rwst"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_comments_325846.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nReplying to [@pelegm](#comment%3A9):\n> Is this solving the case\n> \n> ```python\n> sage: assume(x, 'real')\n> sage: (2**x).is_positive()\n> True\n> sage: (2**x).is_real()\n> False\n> ```\n> as well?\n\nI added this as doctest. Could you review it please?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/23c987e4ec3fa98559f80558a143d73088c64d6c\">23c987e</a></td><td><code>Merge branch 'develop' into t/21940/21940</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c88329909934b9561314344347a7bd7c3c0ec755\">c883299</a></td><td><code>21940: add one doctest</code></td></tr></table>\n",
    "created_at": "2016-12-20T15:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325846",
    "user": "https://github.com/rwst"
}
```

<a id='comment:12'>Comment 12:</a>
Replying to [@pelegm](#comment%3A9):
> Is this solving the case
> 
> ```python
> sage: assume(x, 'real')
> sage: (2**x).is_positive()
> True
> sage: (2**x).is_real()
> False
> ```
> as well?

I added this as doctest. Could you review it please?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/23c987e4ec3fa98559f80558a143d73088c64d6c">23c987e</a></td><td><code>Merge branch 'develop' into t/21940/21940</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c88329909934b9561314344347a7bd7c3c0ec755">c883299</a></td><td><code>21940: add one doctest</code></td></tr></table>




---

archive/issue_comments_325847.json:
```json
{
    "body": "Changed reviewer from **Ralf Stephan** to **Ralf Stephan, Peleg Michaeli**",
    "created_at": "2017-03-12T18:42:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325847",
    "user": "https://github.com/pelegm"
}
```

Changed reviewer from **Ralf Stephan** to **Ralf Stephan, Peleg Michaeli**



---

archive/issue_comments_325848.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nLooks great.",
    "created_at": "2017-03-12T18:42:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325848",
    "user": "https://github.com/pelegm"
}
```

<a id='comment:13'>Comment 13:</a>
Looks great.



---

archive/issue_events_289583.json:
```json
{
    "actor": "https://github.com/pelegm",
    "created_at": "2017-03-12T18:42:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289583"
}
```



---

archive/issue_events_289584.json:
```json
{
    "actor": "https://github.com/pelegm",
    "created_at": "2017-03-12T18:42:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289584"
}
```



---

archive/issue_events_289585.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-03-27T20:42:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289585"
}
```



---

archive/issue_events_289586.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d89d656809385b2a9a75c1452153239ccbdcf824",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-03-27T20:42:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21940#event-289586"
}
```



---

archive/issue_comments_325849.json:
```json
{
    "body": "Changed branch from **[u/rws/21940](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/21940)** to **[c883299](https://github.com/sagemath/sagetrac-mirror/commit/c88329909934b9561314344347a7bd7c3c0ec755)**",
    "created_at": "2017-03-27T20:42:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21940#issuecomment-325849",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/21940](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/21940)** to **[c883299](https://github.com/sagemath/sagetrac-mirror/commit/c88329909934b9561314344347a7bd7c3c0ec755)**
