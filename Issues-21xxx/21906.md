# Issue 21906: Bug in bessel_K

archive/issues_021669.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: bessel_K(0,0)\n...\nNameError: global name 'infinity' is not defined\n```\n\nBranch/Commit: **[bd4d2cf](https://github.com/sagemath/sagetrac-mirror/commit/bd4d2cf752db056b09efba2c0f0c7a3a8d36fe0a)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21906_\n\n",
    "closed_at": "2016-12-05T22:34:48Z",
    "created_at": "2016-11-20T07:31:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in bessel_K",
    "type": "issue",
    "updated_at": "2016-12-05T22:34:48Z",
    "url": "https://github.com/sagemath/sage/issues/21906",
    "user": "https://github.com/rwst"
}
```

```
sage: bessel_K(0,0)
...
NameError: global name 'infinity' is not defined
```

Branch/Commit: **[bd4d2cf](https://github.com/sagemath/sagetrac-mirror/commit/bd4d2cf752db056b09efba2c0f0c7a3a8d36fe0a)**

Reviewer: **Jeroen Demeyer**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/21906_





---

archive/issue_events_289187.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-11-20T07:31:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21906#event-289187"
}
```



---

archive/issue_events_289188.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-11-20T07:31:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21906#event-289188"
}
```



---

archive/issue_events_289189.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-11-20T07:31:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21906#event-289189"
}
```



---

archive/issue_events_289190.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-11-20T07:31:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21906#event-289190"
}
```



---

archive/issue_comments_325395.json:
```json
{
    "body": "Branch: **[u/rws/bug_in_bessel_k](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/bug_in_bessel_k)**",
    "created_at": "2016-12-04T07:27:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21906#issuecomment-325395",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/bug_in_bessel_k](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/bug_in_bessel_k)**



---

archive/issue_events_289191.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-12-04T07:28:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21906#event-289191"
}
```



---

archive/issue_comments_325396.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2016-12-04T07:28:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21906#issuecomment-325396",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_325397.json:
```json
{
    "body": "Commit: **[bd4d2cf](https://github.com/sagemath/sagetrac-mirror/commit/bd4d2cf752db056b09efba2c0f0c7a3a8d36fe0a)**",
    "created_at": "2016-12-04T07:28:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21906#issuecomment-325397",
    "user": "https://github.com/rwst"
}
```

Commit: **[bd4d2cf](https://github.com/sagemath/sagetrac-mirror/commit/bd4d2cf752db056b09efba2c0f0c7a3a8d36fe0a)**



---

archive/issue_comments_325398.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bd4d2cf752db056b09efba2c0f0c7a3a8d36fe0a\">bd4d2cf</a></td><td><code>21906: fix bessel_Y/K(0,0); add tests</code></td></tr></table>\n",
    "created_at": "2016-12-04T07:28:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21906#issuecomment-325398",
    "user": "https://github.com/rwst"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bd4d2cf752db056b09efba2c0f0c7a3a8d36fe0a">bd4d2cf</a></td><td><code>21906: fix bessel_Y/K(0,0); add tests</code></td></tr></table>




---

archive/issue_comments_325399.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2016-12-04T08:35:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21906#issuecomment-325399",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_289192.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-12-04T08:35:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21906#event-289192"
}
```



---

archive/issue_events_289193.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-12-04T08:35:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21906#event-289193"
}
```



---

archive/issue_events_289194.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-12-05T22:34:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21906#event-289194"
}
```



---

archive/issue_events_289195.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e5d3852044e3918118aa1da54393cd979212b26a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-12-05T22:34:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21906#event-289195"
}
```



---

archive/issue_comments_325400.json:
```json
{
    "body": "Changed branch from **[u/rws/bug_in_bessel_k](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/bug_in_bessel_k)** to **[bd4d2cf](https://github.com/sagemath/sagetrac-mirror/commit/bd4d2cf752db056b09efba2c0f0c7a3a8d36fe0a)**",
    "created_at": "2016-12-05T22:34:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21906",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21906#issuecomment-325400",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/bug_in_bessel_k](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/bug_in_bessel_k)** to **[bd4d2cf](https://github.com/sagemath/sagetrac-mirror/commit/bd4d2cf752db056b09efba2c0f0c7a3a8d36fe0a)**
