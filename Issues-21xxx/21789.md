# Issue 21789: Maxima can solve this bvp why not sage?

archive/issues_021552.json:
```json
{
    "assignees": [],
    "body": "According to [this sage-devel post](https://groups.google.com/forum/#!topic/sage-devel/-IESBlP7v0s)\n\n```\ny(x) = function('y')(x); y\neq = x^2*diff(y,x,2)-x*diff(y,x,1)+y==0; eq\nh = desolve(eq, dvar=y, ivar=x,ics=[1,1,e,2*e]); h\n```\ngives an error but Maxima can nonetheless solve it.\n\nCC:  @nbruin\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21789_\n\n",
    "created_at": "2016-11-01T02:13:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Maxima can solve this bvp why not sage?",
    "type": "issue",
    "updated_at": "2022-12-29T01:37:47Z",
    "url": "https://github.com/sagemath/sage/issues/21789",
    "user": "https://github.com/kcrisman"
}
```
According to [this sage-devel post](https://groups.google.com/forum/#!topic/sage-devel/-IESBlP7v0s)

```
y(x) = function('y')(x); y
eq = x^2*diff(y,x,2)-x*diff(y,x,1)+y==0; eq
h = desolve(eq, dvar=y, ivar=x,ics=[1,1,e,2*e]); h
```
gives an error but Maxima can nonetheless solve it.

CC:  @nbruin

_Issue created by migration from https://trac.sagemath.org/ticket/21789_





---

archive/issue_events_287716.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2016-11-01T02:13:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21789",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21789#event-287716"
}
```



---

archive/issue_events_287717.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2016-11-01T02:13:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21789",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21789#event-287717"
}
```



---

archive/issue_events_287718.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2016-11-01T02:13:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21789",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21789#event-287718"
}
```



---

archive/issue_events_287719.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2016-11-01T02:13:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21789",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21789#event-287719"
}
```



---

archive/issue_comments_323146.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nstrangely enough, it works if you use `e^1`, as in:\n\n```\nsage: y(x) = function('y')(x); y\nx |--> y(x)\nsage: eq = x^2*diff(y,x,2)-x*diff(y,x,1)+y==0; eq\nx |--> x^2*diff(y(x), x, x) - x*diff(y(x), x) + y(x) == 0\nsage: h = desolve(eq, dvar=y, ivar=x,ics=[1,1,e^1,2*e^1]); h\nx*(log(x) + 1)\n```",
    "created_at": "2017-05-03T19:38:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21789#issuecomment-323146",
    "user": "https://github.com/mforets"
}
```

<a id='comment:1'>Comment 1:</a>
strangely enough, it works if you use `e^1`, as in:

```
sage: y(x) = function('y')(x); y
x |--> y(x)
sage: eq = x^2*diff(y,x,2)-x*diff(y,x,1)+y==0; eq
x |--> x^2*diff(y(x), x, x) - x*diff(y(x), x) + y(x) == 0
sage: h = desolve(eq, dvar=y, ivar=x,ics=[1,1,e^1,2*e^1]); h
x*(log(x) + 1)
```



---

archive/issue_comments_323147.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nMay be related to #24102 (see [this comment](https://github.com/sagemath/sage/issues/24102#comment:1)...)",
    "created_at": "2020-06-09T06:40:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21789#issuecomment-323147",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:2'>Comment 2:</a>
May be related to #24102 (see [this comment](https://github.com/sagemath/sage/issues/24102#comment:1)...)



---

archive/issue_events_287720.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:37:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21789",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21789#event-287720"
}
```
