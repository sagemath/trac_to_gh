# Issue 21673: Indentation in doctests

archive/issues_021436.json:
```json
{
    "assignees": [],
    "body": "Before this wait until all old-style line continuations are corrected; see for example #21650.\n\nThere are more than thousand doctest lines without 4-space indentation. Those can be found by\n\n```\negrep -R '^ *\\.\\.\\.\\.: {1,3}[^ ]' src/sage  | wc -l\n```\n\nand corrected by Perl.\n\nCC:  @tscrim\n\nReviewer: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21673_\n\n",
    "closed_at": "2017-07-13T07:54:31Z",
    "created_at": "2016-10-10T06:17:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Indentation in doctests",
    "type": "issue",
    "updated_at": "2017-07-13T07:54:31Z",
    "url": "https://github.com/sagemath/sage/issues/21673",
    "user": "https://github.com/jm58660"
}
```
Before this wait until all old-style line continuations are corrected; see for example #21650.

There are more than thousand doctest lines without 4-space indentation. Those can be found by

```
egrep -R '^ *\.\.\.\.: {1,3}[^ ]' src/sage  | wc -l
```

and corrected by Perl.

CC:  @tscrim

Reviewer: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/21673_





---

archive/issue_events_286257.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-10T06:17:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286257"
}
```



---

archive/issue_events_286258.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-10T06:17:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286258"
}
```



---

archive/issue_events_286259.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-10T06:17:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286259"
}
```



---

archive/issue_events_286260.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-10-10T06:17:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286260"
}
```



---

archive/issue_events_286261.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-05-09T14:03:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286261"
}
```



---

archive/issue_events_286262.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-05-09T14:03:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286262"
}
```



---

archive/issue_events_286263.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-05-09T14:03:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286263"
}
```



---

archive/issue_events_286264.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-05-09T14:03:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286264"
}
```



---

archive/issue_comments_321126.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nI was wrong. This is not doable with simple perl. So I suggest closing this as wontfix.",
    "created_at": "2017-05-09T14:03:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21673#issuecomment-321126",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:1" align="right">Comment 1</div>

I was wrong. This is not doable with simple perl. So I suggest closing this as wontfix.



---

archive/issue_comments_321127.json:
```json
{
    "body": "Changed author from **Jori M\u00e4ntysalo** to none",
    "created_at": "2017-05-09T15:03:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21673#issuecomment-321127",
    "user": "https://github.com/tscrim"
}
```

Changed author from **Jori Mäntysalo** to none



---

archive/issue_comments_321128.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-05-09T15:03:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21673#issuecomment-321128",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_321129.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis is something we can do file by file as we come across them.",
    "created_at": "2017-05-09T15:03:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21673#issuecomment-321129",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

This is something we can do file by file as we come across them.



---

archive/issue_events_286265.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-09T15:03:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286265"
}
```



---

archive/issue_events_286266.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-09T15:03:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286266"
}
```



---

archive/issue_events_286267.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286267"
}
```



---

archive/issue_events_286268.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286268"
}
```



---

archive/issue_events_286269.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21673#event-286269"
}
```



---

archive/issue_comments_321130.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nClosing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).",
    "created_at": "2017-07-13T07:54:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21673#issuecomment-321130",
    "user": "https://github.com/embray"
}
```

<div id="comment:3" align="right">Comment 3</div>

Closing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).
