...
```json
{
    "body": "Are you sure this would affect `Element` much? All the things you write are about `CategoryObject` or `Parent`.",
...
```json
{
    "body": "What is wrong with the functions `normalize_names()` and `certify_names`? They have to be put somewhere, why not in `structure/category_object.pyx`?",
