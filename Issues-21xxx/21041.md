...
```json
{
    "body": "Would it be beneficial to expand a rational polytope so that its points are integral, and then filter out the points which would not be integers upon shrinking?\n\nI'm guessing the way to check for translates of a rational/lattice polytope is to actually look at the differences of the vertices relative to a fixed vertex?",
...
```json
{
    "body": "I'm visiting the normaliz people later this week. Maybe we will make a quick libnormaliz interface. Stay tuned.",
