# Issue 21321: Cleanup in sparse modules

archive/issues_021084.json:
```json
{
    "assignees": [],
    "body": "This ticket removes all the `.pxi` files in `sage/modules`, and replaces them by proper `.pxd/pyx` files.\n\nIt adds no new functionnality, but seems to be required to have a C++ binding to linbox.\n\nCC:  @ClementPernet\n\nKeywords: **sd75**\n\nBranch/Commit: **[ce4de73](https://github.com/sagemath/sagetrac-mirror/commit/ce4de7305ff7bf9ddf91fb223f2884b19ce475b9)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Charles Bouillaguet**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21321_\n\n",
    "closed_at": "2016-08-29T16:56:18Z",
    "created_at": "2016-08-23T18:03:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cleanup in sparse modules",
    "type": "issue",
    "updated_at": "2016-08-29T16:56:18Z",
    "url": "https://github.com/sagemath/sage/issues/21321",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```
This ticket removes all the `.pxi` files in `sage/modules`, and replaces them by proper `.pxd/pyx` files.

It adds no new functionnality, but seems to be required to have a C++ binding to linbox.

CC:  @ClementPernet

Keywords: **sd75**

Branch/Commit: **[ce4de73](https://github.com/sagemath/sagetrac-mirror/commit/ce4de7305ff7bf9ddf91fb223f2884b19ce475b9)**

Reviewer: **Jeroen Demeyer**

Author: **Charles Bouillaguet**

_Issue created by migration from https://trac.sagemath.org/ticket/21321_





---

archive/issue_events_281711.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-23T18:03:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281711"
}
```



---

archive/issue_events_281712.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-23T18:03:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281712"
}
```



---

archive/issue_events_281713.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-23T18:03:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281713"
}
```



---

archive/issue_events_281714.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-23T18:03:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281714"
}
```



---

archive/issue_comments_314456.json:
```json
{
    "body": "Branch: **[u/Bouillaguet/module_pxi_must_die](https://github.com/sagemath/sagetrac-mirror/tree/u/Bouillaguet/module_pxi_must_die)**",
    "created_at": "2016-08-23T18:13:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314456",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

Branch: **[u/Bouillaguet/module_pxi_must_die](https://github.com/sagemath/sagetrac-mirror/tree/u/Bouillaguet/module_pxi_must_die)**



---

archive/issue_comments_314457.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0d50a4e7953836d11d1baea347087b3a81bae167\">0d50a4e</a></td><td><code>removes .pxi files in sage/modules, replace by .pyx/pxd</code></td></tr></table>\n",
    "created_at": "2016-08-23T22:39:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314457",
    "user": "https://github.com/ClementPernet"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0d50a4e7953836d11d1baea347087b3a81bae167">0d50a4e</a></td><td><code>removes .pxi files in sage/modules, replace by .pyx/pxd</code></td></tr></table>




---

archive/issue_comments_314458.json:
```json
{
    "body": "Commit: **[0d50a4e](https://github.com/sagemath/sagetrac-mirror/commit/0d50a4e7953836d11d1baea347087b3a81bae167)**",
    "created_at": "2016-08-23T22:39:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314458",
    "user": "https://github.com/ClementPernet"
}
```

Commit: **[0d50a4e](https://github.com/sagemath/sagetrac-mirror/commit/0d50a4e7953836d11d1baea347087b3a81bae167)**



---

archive/issue_comments_314459.json:
```json
{
    "body": "Changed keywords from none to **sd75**",
    "created_at": "2016-08-23T22:39:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314459",
    "user": "https://github.com/ClementPernet"
}
```

Changed keywords from none to **sd75**



---

archive/issue_comments_314460.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. Last 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1126d8a3e18021f00f8a12fc8f420e6d668696f9\">1126d8a</a></td><td><code>forgot module_list.py</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fd3ee8c9a45b75c5384546166496622d6a860f15\">fd3ee8c</a></td><td><code>convert modules/vector_rational_sparse.pxi</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b6bc054f38cc589a8050c5006d8fcf7f59051ad5\">b6bc054</a></td><td><code>Merge commit 'fd3ee8c9a45b75c5384546166496622d6a860f15' into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e5c8348de155f4c5a7f1ea0091240cafcebd4154\">e5c8348</a></td><td><code>fix vector_rational_sparse</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a31fc697619288b58799a3edb80d41bf070c3d50\">a31fc69</a></td><td><code>convert module/vector_integer_sparse.pxi</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/855a54935b962d6ccc5f07713e97a61a413a8903\">855a549</a></td><td><code>Merge commit 'a31fc697619288b58799a3edb80d41bf070c3d50' into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ef6eece2d2af448299a6bc5b04bdf1363ad557d9\">ef6eece</a></td><td><code>fixed vector_integer_sparse</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f0c88db627b31198b630d0f719564e564396ff5e\">f0c88db</a></td><td><code>convert modules/vector_modn_sparse.pxi</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2d7ee590bee822e85b6897f59583d72674f41570\">2d7ee59</a></td><td><code>Merge branch 'modules_pxi_must_die_hard' into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a32caee5c84af293b597434ef5c4449e722ca28e\">a32caee</a></td><td><code>fixed vector_modn_sparse</code></td></tr></table>\n",
    "created_at": "2016-08-23T22:43:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314460",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1126d8a3e18021f00f8a12fc8f420e6d668696f9">1126d8a</a></td><td><code>forgot module_list.py</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fd3ee8c9a45b75c5384546166496622d6a860f15">fd3ee8c</a></td><td><code>convert modules/vector_rational_sparse.pxi</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b6bc054f38cc589a8050c5006d8fcf7f59051ad5">b6bc054</a></td><td><code>Merge commit 'fd3ee8c9a45b75c5384546166496622d6a860f15' into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e5c8348de155f4c5a7f1ea0091240cafcebd4154">e5c8348</a></td><td><code>fix vector_rational_sparse</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a31fc697619288b58799a3edb80d41bf070c3d50">a31fc69</a></td><td><code>convert module/vector_integer_sparse.pxi</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/855a54935b962d6ccc5f07713e97a61a413a8903">855a549</a></td><td><code>Merge commit 'a31fc697619288b58799a3edb80d41bf070c3d50' into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ef6eece2d2af448299a6bc5b04bdf1363ad557d9">ef6eece</a></td><td><code>fixed vector_integer_sparse</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f0c88db627b31198b630d0f719564e564396ff5e">f0c88db</a></td><td><code>convert modules/vector_modn_sparse.pxi</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2d7ee590bee822e85b6897f59583d72674f41570">2d7ee59</a></td><td><code>Merge branch 'modules_pxi_must_die_hard' into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a32caee5c84af293b597434ef5c4449e722ca28e">a32caee</a></td><td><code>fixed vector_modn_sparse</code></td></tr></table>




---

archive/issue_comments_314461.json:
```json
{
    "body": "Changed commit from **[0d50a4e](https://github.com/sagemath/sagetrac-mirror/commit/0d50a4e7953836d11d1baea347087b3a81bae167)** to **[a32caee](https://github.com/sagemath/sagetrac-mirror/commit/a32caee5c84af293b597434ef5c4449e722ca28e)**",
    "created_at": "2016-08-23T22:43:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314461",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0d50a4e](https://github.com/sagemath/sagetrac-mirror/commit/0d50a4e7953836d11d1baea347087b3a81bae167)** to **[a32caee](https://github.com/sagemath/sagetrac-mirror/commit/a32caee5c84af293b597434ef5c4449e722ca28e)**



---

archive/issue_comments_314462.json:
```json
{
    "body": "Changed author from **Bouillaguet** to **Charles Bouillaguet**",
    "created_at": "2016-08-23T22:48:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314462",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

Changed author from **Bouillaguet** to **Charles Bouillaguet**



---

archive/issue_events_281715.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-24T08:58:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281715"
}
```



---

archive/issue_events_281716.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-24T08:58:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281716"
}
```



---

archive/issue_events_281717.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-24T08:58:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281717"
}
```



---

archive/issue_comments_314463.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n This ticket removes all the `.pxi` files in `sage/modules`, and replaces them by proper `.pxd/pyx` files.\n \n-It adds no new functionnality.\n+It adds no new functionnality, but seems to be required to have a C++ binding to linbox.\n``````\n",
    "created_at": "2016-08-24T08:58:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314463",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 This ticket removes all the `.pxi` files in `sage/modules`, and replaces them by proper `.pxd/pyx` files.
 
-It adds no new functionnality.
+It adds no new functionnality, but seems to be required to have a C++ binding to linbox.
``````




---

archive/issue_comments_314464.json:
```json
{
    "body": "Changed commit from **[a32caee](https://github.com/sagemath/sagetrac-mirror/commit/a32caee5c84af293b597434ef5c4449e722ca28e)** to **[206e794](https://github.com/sagemath/sagetrac-mirror/commit/206e794bd92d294d917d4b503a654e13776b5b91)**",
    "created_at": "2016-08-24T15:36:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314464",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a32caee](https://github.com/sagemath/sagetrac-mirror/commit/a32caee5c84af293b597434ef5c4449e722ca28e)** to **[206e794](https://github.com/sagemath/sagetrac-mirror/commit/206e794bd92d294d917d4b503a654e13776b5b91)**



---

archive/issue_comments_314465.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/206e794bd92d294d917d4b503a654e13776b5b91\">206e794</a></td><td><code>Fixed a remaining reference to vector_modn_sparse.pxi</code></td></tr></table>\n",
    "created_at": "2016-08-24T15:36:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314465",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/206e794bd92d294d917d4b503a654e13776b5b91">206e794</a></td><td><code>Fixed a remaining reference to vector_modn_sparse.pxi</code></td></tr></table>




---

archive/issue_comments_314466.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nI might review this.",
    "created_at": "2016-08-25T18:44:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314466",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">Comment 7</div>

I might review this.



---

archive/issue_comments_314467.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nOne thing: you have a very complicated git history here. Can you squash it to just 1 commit?",
    "created_at": "2016-08-25T18:46:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314467",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:8" align="right">Comment 8</div>

One thing: you have a very complicated git history here. Can you squash it to just 1 commit?



---

archive/issue_comments_314468.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\ncysignals stuff should only be in .pyx files.",
    "created_at": "2016-08-25T18:49:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314468",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:9" align="right">Comment 9</div>

cysignals stuff should only be in .pyx files.



---

archive/issue_events_281718.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-25T18:49:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281718"
}
```



---

archive/issue_events_281719.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-25T18:49:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281719"
}
```



---

archive/issue_comments_314469.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nMore generally, .pxd files should only cimport/include what they really need.",
    "created_at": "2016-08-25T19:08:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314469",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:10" align="right">Comment 10</div>

More generally, .pxd files should only cimport/include what they really need.



---

archive/issue_comments_314470.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nDid you check for conflicts with #17635?",
    "created_at": "2016-08-25T19:17:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314470",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:11" align="right">Comment 11</div>

Did you check for conflicts with #17635?



---

archive/issue_comments_314471.json:
```json
{
    "body": "Changed commit from **[206e794](https://github.com/sagemath/sagetrac-mirror/commit/206e794bd92d294d917d4b503a654e13776b5b91)** to **[2036d22](https://github.com/sagemath/sagetrac-mirror/commit/2036d22686f91b6d6155265a7615be2f947f797d)**",
    "created_at": "2016-08-25T19:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314471",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[206e794](https://github.com/sagemath/sagetrac-mirror/commit/206e794bd92d294d917d4b503a654e13776b5b91)** to **[2036d22](https://github.com/sagemath/sagetrac-mirror/commit/2036d22686f91b6d6155265a7615be2f947f797d)**



---

archive/issue_comments_314472.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/305adf677bc58ede88dd13e733b3e1cd59762527\">305adf6</a></td><td><code>sqome more typos</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/72730d6def46d232711ed1511a687c4ac7ddc4a6\">72730d6</a></td><td><code>Updated [SageMath](../wiki/SageMath) version to 7.4.beta1</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2036d22686f91b6d6155265a7615be2f947f797d\">2036d22</a></td><td><code>Convert .pxi files in sage/modules/ into proper .pxd/.pyx files</code></td></tr></table>\n",
    "created_at": "2016-08-25T19:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314472",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/305adf677bc58ede88dd13e733b3e1cd59762527">305adf6</a></td><td><code>sqome more typos</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/72730d6def46d232711ed1511a687c4ac7ddc4a6">72730d6</a></td><td><code>Updated [SageMath](../wiki/SageMath) version to 7.4.beta1</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2036d22686f91b6d6155265a7615be2f947f797d">2036d22</a></td><td><code>Convert .pxi files in sage/modules/ into proper .pxd/.pyx files</code></td></tr></table>




---

archive/issue_comments_314473.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nThanks for your time, Jeroen. I fixed the extra imports in .pxd files, and sqashed all that into a single commit. Up for review again.",
    "created_at": "2016-08-25T20:00:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314473",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

<div id="comment:13" align="right">Comment 13</div>

Thanks for your time, Jeroen. I fixed the extra imports in .pxd files, and sqashed all that into a single commit. Up for review again.



---

archive/issue_events_281720.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-25T20:00:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281720"
}
```



---

archive/issue_events_281721.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-25T20:00:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281721"
}
```



---

archive/issue_comments_314474.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nReplying to [@sagetrac-Bouillaguet](#comment%3A13):\n> and sqashed all that into a single commit.\n\nI think you did something wrong, since I see two unrelated commits. In any case, you should rebase on top of sage-7.4.beta**2** now.",
    "created_at": "2016-08-26T08:23:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314474",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:14" align="right">Comment 14</div>

Replying to [@sagetrac-Bouillaguet](#comment%3A13):
> and sqashed all that into a single commit.

I think you did something wrong, since I see two unrelated commits. In any case, you should rebase on top of sage-7.4.beta**2** now.



---

archive/issue_comments_314475.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2016-08-26T08:23:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314475",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_281722.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-26T08:23:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281722"
}
```



---

archive/issue_events_281723.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-26T08:23:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281723"
}
```



---

archive/issue_comments_314476.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nReplying to [@jdemeyer](#comment%3A11):\n> Did you check for conflicts with #17635?\n\nThis ticket merges cleanly with 17635 and the testsuite passes on my box.",
    "created_at": "2016-08-26T09:08:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314476",
    "user": "https://github.com/ClementPernet"
}
```

<div id="comment:15" align="right">Comment 15</div>

Replying to [@jdemeyer](#comment%3A11):
> Did you check for conflicts with #17635?

This ticket merges cleanly with 17635 and the testsuite passes on my box.



---

archive/issue_comments_314477.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nSince you're moving files anyway, could you move `src/sage/modules/binary_search` to `src/sage/data_structures/binary_search`. That code has nothing to do with modules.",
    "created_at": "2016-08-26T09:16:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314477",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:16" align="right">Comment 16</div>

Since you're moving files anyway, could you move `src/sage/modules/binary_search` to `src/sage/data_structures/binary_search`. That code has nothing to do with modules.



---

archive/issue_comments_314478.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b7066138d2cf0730d4eda3af2c1ad315d1dc9e9b\">b706613</a></td><td><code>Convert .pxi files in sage/modules/ into proper .pxd/.pyx files</code></td></tr></table>\n",
    "created_at": "2016-08-26T20:58:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314478",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:17" align="right">Comment 17</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b7066138d2cf0730d4eda3af2c1ad315d1dc9e9b">b706613</a></td><td><code>Convert .pxi files in sage/modules/ into proper .pxd/.pyx files</code></td></tr></table>




---

archive/issue_comments_314479.json:
```json
{
    "body": "Changed commit from **[2036d22](https://github.com/sagemath/sagetrac-mirror/commit/2036d22686f91b6d6155265a7615be2f947f797d)** to **[b706613](https://github.com/sagemath/sagetrac-mirror/commit/b7066138d2cf0730d4eda3af2c1ad315d1dc9e9b)**",
    "created_at": "2016-08-26T20:58:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314479",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2036d22](https://github.com/sagemath/sagetrac-mirror/commit/2036d22686f91b6d6155265a7615be2f947f797d)** to **[b706613](https://github.com/sagemath/sagetrac-mirror/commit/b7066138d2cf0730d4eda3af2c1ad315d1dc9e9b)**



---

archive/issue_comments_314480.json:
```json
{
    "body": "Changed commit from **[b706613](https://github.com/sagemath/sagetrac-mirror/commit/b7066138d2cf0730d4eda3af2c1ad315d1dc9e9b)** to **[ce4de73](https://github.com/sagemath/sagetrac-mirror/commit/ce4de7305ff7bf9ddf91fb223f2884b19ce475b9)**",
    "created_at": "2016-08-27T13:35:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314480",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b706613](https://github.com/sagemath/sagetrac-mirror/commit/b7066138d2cf0730d4eda3af2c1ad315d1dc9e9b)** to **[ce4de73](https://github.com/sagemath/sagetrac-mirror/commit/ce4de7305ff7bf9ddf91fb223f2884b19ce475b9)**



---

archive/issue_comments_314481.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ce4de7305ff7bf9ddf91fb223f2884b19ce475b9\">ce4de73</a></td><td><code>moved sage.modules.binary_search to sage.data_structures.binary_search</code></td></tr></table>\n",
    "created_at": "2016-08-27T13:35:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314481",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:18" align="right">Comment 18</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ce4de7305ff7bf9ddf91fb223f2884b19ce475b9">ce4de73</a></td><td><code>moved sage.modules.binary_search to sage.data_structures.binary_search</code></td></tr></table>




---

archive/issue_events_281724.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-27T13:38:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281724"
}
```



---

archive/issue_events_281725.json:
```json
{
    "actor": "https://github.com/sagetrac-Bouillaguet",
    "created_at": "2016-08-27T13:38:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281725"
}
```



---

archive/issue_comments_314482.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nJeroen, the ticket is ready for review (again).",
    "created_at": "2016-08-27T13:38:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314482",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

<div id="comment:19" align="right">Comment 19</div>

Jeroen, the ticket is ready for review (again).



---

archive/issue_comments_314483.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nDid you run full doctests with this latest patch?",
    "created_at": "2016-08-27T13:47:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314483",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:20" align="right">Comment 20</div>

Did you run full doctests with this latest patch?



---

archive/issue_comments_314484.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nNo. I'm counting on the patchbot...",
    "created_at": "2016-08-27T13:48:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314484",
    "user": "https://github.com/sagetrac-Bouillaguet"
}
```

<div id="comment:21" align="right">Comment 21</div>

No. I'm counting on the patchbot...



---

archive/issue_events_281726.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-27T20:30:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281726"
}
```



---

archive/issue_events_281727.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-27T20:30:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281727"
}
```



---

archive/issue_comments_314485.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nTests are good.",
    "created_at": "2016-08-27T20:30:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314485",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:22" align="right">Comment 22</div>

Tests are good.



---

archive/issue_comments_314486.json:
```json
{
    "body": "Changed branch from **[u/Bouillaguet/module_pxi_must_die](https://github.com/sagemath/sagetrac-mirror/tree/u/Bouillaguet/module_pxi_must_die)** to **[ce4de73](https://github.com/sagemath/sagetrac-mirror/commit/ce4de7305ff7bf9ddf91fb223f2884b19ce475b9)**",
    "created_at": "2016-08-29T16:56:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21321#issuecomment-314486",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/Bouillaguet/module_pxi_must_die](https://github.com/sagemath/sagetrac-mirror/tree/u/Bouillaguet/module_pxi_must_die)** to **[ce4de73](https://github.com/sagemath/sagetrac-mirror/commit/ce4de7305ff7bf9ddf91fb223f2884b19ce475b9)**



---

archive/issue_events_281728.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-08-29T16:56:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281728"
}
```



---

archive/issue_events_281729.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8238980d4ab6de549c40339d9f95e44618a85ef6",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-08-29T16:56:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21321",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21321#event-281729"
}
```
