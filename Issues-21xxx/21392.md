# Issue 21392: Polynomial should implement quo_rem

archive/issues_021155.json:
```json
{
    "assignees": [],
    "body": "There are implementations of `quo_rem()` in `Polynomial_generic_dense` and the like, but not in the `Polynomial` base class itself, so that specialized implementations of polynomials must implement it to work\u2014while `Polynomial` does provide `_add_`, `_mul_` etc.\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21392_\n\n",
    "created_at": "2016-09-01T15:54:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Polynomial should implement quo_rem",
    "type": "issue",
    "updated_at": "2022-08-31T02:51:13Z",
    "url": "https://github.com/sagemath/sage/issues/21392",
    "user": "https://github.com/mezzarobba"
}
```
There are implementations of `quo_rem()` in `Polynomial_generic_dense` and the like, but not in the `Polynomial` base class itself, so that specialized implementations of polynomials must implement it to work—while `Polynomial` does provide `_add_`, `_mul_` etc.

_Issue created by migration from https://trac.sagemath.org/ticket/21392_





---

archive/issue_events_282606.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-09-01T15:54:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282606"
}
```



---

archive/issue_events_282607.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-09-01T15:54:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282607"
}
```



---

archive/issue_events_282608.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-09-01T15:54:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282608"
}
```



---

archive/issue_events_282609.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-09-01T15:54:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282609"
}
```



---

archive/issue_events_282610.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2020-11-25T15:45:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282610"
}
```



---

archive/issue_comments_315709.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nfixed in #11593",
    "created_at": "2020-11-25T15:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21392#issuecomment-315709",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:1" align="right">Comment 1</div>

fixed in #11593



---

archive/issue_events_282611.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2020-11-25T15:45:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282611"
}
```



---

archive/issue_events_282612.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2020-11-25T15:45:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282612"
}
```



---

archive/issue_events_282613.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2020-11-25T15:45:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282613"
}
```



---

archive/issue_comments_315710.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nhow could this be fixed long before the ticket was open ?",
    "created_at": "2021-01-31T08:25:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21392#issuecomment-315710",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

how could this be fixed long before the ticket was open ?



---

archive/issue_comments_315711.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nUh, good question! Apparently it has **not** been fixed: the ticket I am linking to adds a `quo_rem()` method to `Polynomial_generic_dense`, not `Polynomial`.",
    "created_at": "2021-01-31T08:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21392#issuecomment-315711",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:4" align="right">Comment 4</div>

Uh, good question! Apparently it has **not** been fixed: the ticket I am linking to adds a `quo_rem()` method to `Polynomial_generic_dense`, not `Polynomial`.



---

archive/issue_events_282614.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2021-01-31T08:50:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282614"
}
```



---

archive/issue_events_282615.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2021-01-31T08:50:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282615"
}
```



---

archive/issue_events_282616.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2021-01-31T08:50:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282616"
}
```



---

archive/issue_events_282617.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282617"
}
```



---

archive/issue_events_282618.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282618"
}
```



---

archive/issue_comments_315712.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nSetting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21392#issuecomment-315712",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_282619.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282619"
}
```



---

archive/issue_events_282620.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282620"
}
```



---

archive/issue_events_282621.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-11T17:19:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282621"
}
```



---

archive/issue_events_282622.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-11T17:19:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282622"
}
```



---

archive/issue_events_282623.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282623"
}
```



---

archive/issue_events_282624.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21392",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21392#event-282624"
}
```
