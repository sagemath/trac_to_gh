...
```json
{
    "body": "Thanks--yeah I've encountered a ton of this.  Likewise there are misuses of `$SAGE_LOCAL` scattered about when there are other, more specific variables that could be used.  I mostly fix them when I see them, but haven't considered doing a comprehensive cleanup.",
...
```json
{
    "body": "Has anyone done any work on this yet?  I'm in the mood for a (somewhat) mindless and tedious task.",
...
```json
{
    "body": "Be my guest! I historically reduced the use of `SAGE_ROOT` in the build system and at runtime. Most of the usage in `src/sage`, with the exception of `env.py` is replaced or removed (`package.py`) one way or another in sage-on-gentoo.",
...
```json
{
    "body": "fbissey: Will improvements on this front then cut down on the amount of contortions sage-on-gentoo needs to go through?",
...
```json
{
    "body": "I think #21571 should be considered a dependency, since it's definitely a piece of this puzzle.",
...
```json
{
    "body": "This is outdated/completed, should be closed",
