...
```json
{
    "body": "Hmm. Not very clear.. Does not look like a polynomial ring..\n\n```\nsage: x=polygen(QQ,'x')\nsage: f=x+6\nsage: g=libgap(f)\nsage: g.Ring()\n<ring with 1 generators>\nsage: g.Ring().IsPolynomialRing()\nfalse\n```",
...
```json
{
    "body": "Here is something that seems to work, even if somewhat ugly.",
...
```json
{
    "body": "Dima, do you have any other comments? I am ready to set this to a positive review.",
...
```json
{
    "body": "Can we get doctests for multivariate polynomials? And for different base rings?",
...
```json
{
    "body": "This will not work for multi-variate polynomials so far.",
...
```json
{
    "body": "By all ways, please do that! I have zero knowledge of gap, essentially.",
...
```json
{
    "body": "Contrarily to what I thought, it seems not possible to extract the common ring of the coefficients from GAP. I kept the \"common_parent\".\n\nNow works with (univriate) Laurent polynomials and rational functions.\n\nNeeds review again.\n\n---\nNew commits:",
...
```json
{
    "body": "If this works in GAP then it is matter of putting `CoefficientRing` into an appropriate list of GAP functions, or just calling `libgap.function_factory(CoefficientRing)`",
...
```json
{
    "body": "This looks like a GAP bug to me. I've opened [#3159](https://github.com/gap-system/gap/issues/3159) for GAP.\n\nIndeed, this should not stop us from going ahead with this ticket in some form.",
...
```json
{
    "body": "Then I am satisfied with the current state, and ready to give a positive review.\n\nVincent, d'accord ?",
...
```json
{
    "body": "coercion_model has been moved by ticket #27021\n\nso there is a conflict between here and there..",
...
```json
{
    "body": "let us step back here then",
    "created_at": "2019-01-18T08:02:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21020",
    "type": "issue_comment",
