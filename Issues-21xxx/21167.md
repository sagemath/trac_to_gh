# Issue 21167: Bug with conversion of Singular rings to Sage rings

archive/issues_020930.json:
```json
{
    "assignees": [],
    "body": "Currently there is a bug with converting Singular polynomial rings defined over field extensions to Sage polynomial rings:\n\n```\nR.<x,y> = PolynomialRing(QuadraticField(-1), 2)\nS = R._singular_()\nS.sage()\n```\n\nCC:  @bhutz @miguelmarco\n\nBranch/Commit: **[3ff6dc0](https://github.com/sagemath/sagetrac-mirror/commit/3ff6dc0df3661595edc2cd1cf7b395f1619a96b6)**\n\nReviewer: **Ben Hutz**\n\nAuthor: **Grayson Jorgenson**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21167_\n\n",
    "closed_at": "2016-08-13T22:50:34Z",
    "created_at": "2016-08-04T17:54:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug with conversion of Singular rings to Sage rings",
    "type": "issue",
    "updated_at": "2016-08-13T22:50:34Z",
    "url": "https://github.com/sagemath/sage/issues/21167",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```
Currently there is a bug with converting Singular polynomial rings defined over field extensions to Sage polynomial rings:

```
R.<x,y> = PolynomialRing(QuadraticField(-1), 2)
S = R._singular_()
S.sage()
```

CC:  @bhutz @miguelmarco

Branch/Commit: **[3ff6dc0](https://github.com/sagemath/sagetrac-mirror/commit/3ff6dc0df3661595edc2cd1cf7b395f1619a96b6)**

Reviewer: **Ben Hutz**

Author: **Grayson Jorgenson**

_Issue created by migration from https://trac.sagemath.org/ticket/21167_





---

archive/issue_events_279706.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-08-04T17:54:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279706"
}
```



---

archive/issue_events_279707.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-08-04T17:54:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279707"
}
```



---

archive/issue_events_279708.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-08-04T17:54:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279708"
}
```



---

archive/issue_events_279709.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-08-04T17:54:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279709"
}
```



---

archive/issue_comments_311200.json:
```json
{
    "body": "Branch: **[u/gjorgenson/ticket/21167](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/21167)**",
    "created_at": "2016-08-04T17:55:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21167#issuecomment-311200",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

Branch: **[u/gjorgenson/ticket/21167](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/21167)**



---

archive/issue_comments_311201.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3ff6dc0df3661595edc2cd1cf7b395f1619a96b6\">3ff6dc0</a></td><td><code>21167: fix bug and add example</code></td></tr></table>\n",
    "created_at": "2016-08-04T23:39:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21167#issuecomment-311201",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3ff6dc0df3661595edc2cd1cf7b395f1619a96b6">3ff6dc0</a></td><td><code>21167: fix bug and add example</code></td></tr></table>




---

archive/issue_comments_311202.json:
```json
{
    "body": "Commit: **[3ff6dc0](https://github.com/sagemath/sagetrac-mirror/commit/3ff6dc0df3661595edc2cd1cf7b395f1619a96b6)**",
    "created_at": "2016-08-04T23:39:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21167#issuecomment-311202",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[3ff6dc0](https://github.com/sagemath/sagetrac-mirror/commit/3ff6dc0df3661595edc2cd1cf7b395f1619a96b6)**



---

archive/issue_events_279710.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-08-04T23:42:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279710"
}
```



---

archive/issue_comments_311203.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nOkay, I made the minor change in the `sage_global_ring` function and added an example demonstrating the fix. I ran some doctests and this change doesn't appear to break anything.",
    "created_at": "2016-08-04T23:42:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21167#issuecomment-311203",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

<a id='comment:3'>Comment 3:</a>
Okay, I made the minor change in the `sage_global_ring` function and added an example demonstrating the fix. I ran some doctests and this change doesn't appear to break anything.



---

archive/issue_comments_311204.json:
```json
{
    "body": "Reviewer: **Ben Hutz**",
    "created_at": "2016-08-05T14:39:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21167#issuecomment-311204",
    "user": "https://github.com/bhutz"
}
```

Reviewer: **Ben Hutz**



---

archive/issue_events_279711.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-08-05T14:39:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279711"
}
```



---

archive/issue_events_279712.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-08-05T14:39:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279712"
}
```



---

archive/issue_events_279713.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-08-05T14:39:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279713"
}
```



---

archive/issue_events_279714.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-08-05T14:39:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279714"
}
```



---

archive/issue_comments_311205.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nyes, this looks like the right fix.",
    "created_at": "2016-08-05T14:39:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21167#issuecomment-311205",
    "user": "https://github.com/bhutz"
}
```

<a id='comment:4'>Comment 4:</a>
yes, this looks like the right fix.



---

archive/issue_events_279715.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-08-13T22:50:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279715"
}
```



---

archive/issue_events_279716.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f7123eb4f9785ce05e787b77a459183155edb4be",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-08-13T22:50:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21167#event-279716"
}
```



---

archive/issue_comments_311206.json:
```json
{
    "body": "Changed branch from **[u/gjorgenson/ticket/21167](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/21167)** to **[3ff6dc0](https://github.com/sagemath/sagetrac-mirror/commit/3ff6dc0df3661595edc2cd1cf7b395f1619a96b6)**",
    "created_at": "2016-08-13T22:50:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21167",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21167#issuecomment-311206",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gjorgenson/ticket/21167](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/21167)** to **[3ff6dc0](https://github.com/sagemath/sagetrac-mirror/commit/3ff6dc0df3661595edc2cd1cf7b395f1619a96b6)**
