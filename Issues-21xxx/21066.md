# Issue 21066: Unpickle overrides for finite fields

archive/issues_020829.json:
```json
{
    "assignees": [],
    "body": "Two overrides necessary to make `load('http://rlmiller.org/de_codes/get_code.php?id=16_08')` work - result of using #21065\n\nBranch/Commit: **[526cc78](https://github.com/sagemath/sagetrac-mirror/commit/526cc78d4e04219c0c33d642c1ce8f5eade92b95)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Andrey Novoseltsev**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21066_\n\n",
    "closed_at": "2017-05-23T06:57:32Z",
    "created_at": "2016-07-21T01:18:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20pickling",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Unpickle overrides for finite fields",
    "type": "issue",
    "updated_at": "2017-05-23T06:57:32Z",
    "url": "https://github.com/sagemath/sage/issues/21066",
    "user": "https://github.com/novoselt"
}
```
Two overrides necessary to make `load('http://rlmiller.org/de_codes/get_code.php?id=16_08')` work - result of using #21065

Branch/Commit: **[526cc78](https://github.com/sagemath/sagetrac-mirror/commit/526cc78d4e04219c0c33d642c1ce8f5eade92b95)**

Reviewer: **Travis Scrimshaw**

Author: **Andrey Novoseltsev**

_Issue created by migration from https://trac.sagemath.org/ticket/21066_





---

archive/issue_events_278337.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2016-07-21T01:18:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278337"
}
```



---

archive/issue_events_278338.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2016-07-21T01:18:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20pickling",
    "label_color": "000080",
    "label_name": "component: pickling",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278338"
}
```



---

archive/issue_events_278339.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2016-07-21T01:18:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278339"
}
```



---

archive/issue_events_278340.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2016-07-21T01:18:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278340"
}
```



---

archive/issue_comments_308944.json:
```json
{
    "body": "Branch: **[u/novoselt/unpickle_overrides_for_finite_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/novoselt/unpickle_overrides_for_finite_fields)**",
    "created_at": "2016-07-21T01:19:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21066#issuecomment-308944",
    "user": "https://github.com/novoselt"
}
```

Branch: **[u/novoselt/unpickle_overrides_for_finite_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/novoselt/unpickle_overrides_for_finite_fields)**



---

archive/issue_comments_308945.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/526cc78d4e04219c0c33d642c1ce8f5eade92b95\">526cc78</a></td><td><code>Add two unpickle overrides for backward compatibility.</code></td></tr></table>\n",
    "created_at": "2016-07-21T01:19:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21066#issuecomment-308945",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/526cc78d4e04219c0c33d642c1ce8f5eade92b95">526cc78</a></td><td><code>Add two unpickle overrides for backward compatibility.</code></td></tr></table>




---

archive/issue_comments_308946.json:
```json
{
    "body": "Commit: **[526cc78](https://github.com/sagemath/sagetrac-mirror/commit/526cc78d4e04219c0c33d642c1ce8f5eade92b95)**",
    "created_at": "2016-07-21T01:19:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21066#issuecomment-308946",
    "user": "https://github.com/novoselt"
}
```

Commit: **[526cc78](https://github.com/sagemath/sagetrac-mirror/commit/526cc78d4e04219c0c33d642c1ce8f5eade92b95)**



---

archive/issue_events_278341.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2016-07-21T01:19:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278341"
}
```



---

archive/issue_comments_308947.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nTwo trivial PEP8 things:\n\n```diff\n-register_unpickle_override(\n-    'sage.rings.ring', 'unpickle_FiniteField_prm', unpickle_FiniteField_prm)\n+register_unpickle_override('sage.rings.ring', 'unpickle_FiniteField_prm',\n+                           unpickle_FiniteField_prm)\n```\nand\n\n```diff\n-register_unpickle_override(\n-    'sage.rings.finite_field_prime_modn', 'FiniteField_prime_modn',\n-    FiniteField_prime_modn)\n+register_unpickle_override('sage.rings.finite_field_prime_modn',\n+                           'FiniteField_prime_modn', FiniteField_prime_modn)\n```\nOtherwise LGTM.",
    "created_at": "2017-05-21T02:40:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21066#issuecomment-308947",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
Two trivial PEP8 things:

```diff
-register_unpickle_override(
-    'sage.rings.ring', 'unpickle_FiniteField_prm', unpickle_FiniteField_prm)
+register_unpickle_override('sage.rings.ring', 'unpickle_FiniteField_prm',
+                           unpickle_FiniteField_prm)
```
and

```diff
-register_unpickle_override(
-    'sage.rings.finite_field_prime_modn', 'FiniteField_prime_modn',
-    FiniteField_prime_modn)
+register_unpickle_override('sage.rings.finite_field_prime_modn',
+                           'FiniteField_prime_modn', FiniteField_prime_modn)
```
Otherwise LGTM.



---

archive/issue_comments_308948.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-05-21T02:40:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21066#issuecomment-308948",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_278342.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-21T02:40:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278342"
}
```



---

archive/issue_events_278343.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-21T02:40:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278343"
}
```



---

archive/issue_comments_308949.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThanks for taking a look! I am a bit confused about PEP8, it seems to me that I am using one of the acceptable formats with hanging indent: https://www.python.org/dev/peps/pep-0008/#indentation",
    "created_at": "2017-05-22T16:08:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21066#issuecomment-308949",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:5'>Comment 5:</a>
Thanks for taking a look! I am a bit confused about PEP8, it seems to me that I am using one of the acceptable formats with hanging indent: https://www.python.org/dev/peps/pep-0008/#indentation



---

archive/issue_events_278344.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-22T16:20:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278344"
}
```



---

archive/issue_events_278345.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-22T16:20:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278345"
}
```



---

archive/issue_comments_308950.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAh, right, that is acceptable. I've just been using the first and thought that was the (marginally?) more preferred way. Thanks.",
    "created_at": "2017-05-22T16:20:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21066#issuecomment-308950",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
Ah, right, that is acceptable. I've just been using the first and thought that was the (marginally?) more preferred way. Thanks.



---

archive/issue_events_278346.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-05-23T06:57:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278346"
}
```



---

archive/issue_events_278347.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cbf72fed3951a295903dbf4a955af8b262cef218",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-05-23T06:57:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21066#event-278347"
}
```



---

archive/issue_comments_308951.json:
```json
{
    "body": "Changed branch from **[u/novoselt/unpickle_overrides_for_finite_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/novoselt/unpickle_overrides_for_finite_fields)** to **[526cc78](https://github.com/sagemath/sagetrac-mirror/commit/526cc78d4e04219c0c33d642c1ce8f5eade92b95)**",
    "created_at": "2017-05-23T06:57:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21066#issuecomment-308951",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/novoselt/unpickle_overrides_for_finite_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/novoselt/unpickle_overrides_for_finite_fields)** to **[526cc78](https://github.com/sagemath/sagetrac-mirror/commit/526cc78d4e04219c0c33d642c1ce8f5eade92b95)**
