# Issue 21683: Fix linking some sage modules on cygwin following #17635

archive/issues_021446.json:
```json
{
    "assignees": [],
    "body": "After merging #17635, some modules were failing to link on Cygwin as first reported [here](https://github.com/sagemath/sage/issues/17635#comment:239).\n\nMy testing indicates that this fixes the issue (with thanks to jdemeyer for pointing out `library_order_list`).\n\nCC:  @jdemeyer @jpflori\n\nKeywords: **cygwin windows**\n\nBranch/Commit: **[0e86bfa](https://github.com/sagemath/sagetrac-mirror/commit/0e86bfa475bd29bbf96ae31fadb040f0f68ea015)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21683_\n\n",
    "closed_at": "2016-11-08T23:42:17Z",
    "created_at": "2016-10-11T15:56:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix linking some sage modules on cygwin following #17635",
    "type": "issue",
    "updated_at": "2016-11-08T23:42:17Z",
    "url": "https://github.com/sagemath/sage/issues/21683",
    "user": "https://github.com/embray"
}
```
After merging #17635, some modules were failing to link on Cygwin as first reported [here](https://github.com/sagemath/sage/issues/17635#comment:239).

My testing indicates that this fixes the issue (with thanks to jdemeyer for pointing out `library_order_list`).

CC:  @jdemeyer @jpflori

Keywords: **cygwin windows**

Branch/Commit: **[0e86bfa](https://github.com/sagemath/sagetrac-mirror/commit/0e86bfa475bd29bbf96ae31fadb040f0f68ea015)**

Reviewer: **Jeroen Demeyer**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/21683_





---

archive/issue_events_286374.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-10-11T15:56:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286374"
}
```



---

archive/issue_events_286375.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-10-11T15:56:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286375"
}
```



---

archive/issue_events_286376.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-10-11T15:56:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286376"
}
```



---

archive/issue_events_286377.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-10-11T15:56:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286377"
}
```



---

archive/issue_events_286378.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-10-11T15:56:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286378"
}
```



---

archive/issue_events_286379.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-10-11T18:18:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286379"
}
```



---

archive/issue_events_286380.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-10-11T18:18:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286380"
}
```



---

archive/issue_comments_321319.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI can't test this, but if this works for you...",
    "created_at": "2016-10-11T18:18:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21683#issuecomment-321319",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

I can't test this, but if this works for you...



---

archive/issue_comments_321320.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2016-10-11T18:18:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21683#issuecomment-321320",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_321321.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI mean, I guess as long as it doesn't somehow break things on *other* platforms, though I see no reason it would.",
    "created_at": "2016-10-12T08:43:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21683#issuecomment-321321",
    "user": "https://github.com/embray"
}
```

<div id="comment:3" align="right">Comment 3</div>

I mean, I guess as long as it doesn't somehow break things on *other* platforms, though I see no reason it would.



---

archive/issue_comments_321322.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nCygwin is the only platform where the linking order of libraries matters that much. I guess linkers on other platforms are more intelligent.",
    "created_at": "2016-10-12T08:52:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21683#issuecomment-321322",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

Cygwin is the only platform where the linking order of libraries matters that much. I guess linkers on other platforms are more intelligent.



---

archive/issue_comments_321323.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nMerge conflict",
    "created_at": "2016-10-21T08:22:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21683#issuecomment-321323",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:5" align="right">Comment 5</div>

Merge conflict



---

archive/issue_events_286381.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-10-21T08:22:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286381"
}
```



---

archive/issue_events_286382.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-10-21T08:22:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286382"
}
```



---

archive/issue_comments_321324.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0e86bfa475bd29bbf96ae31fadb040f0f68ea015\">0e86bfa</a></td><td><code>Add fflas_ffpack_libs to library_order_list, to ensure that linbox_libs are linked before fflas_ffpack_libs.</code></td></tr></table>\n",
    "created_at": "2016-11-07T12:33:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21683#issuecomment-321324",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0e86bfa475bd29bbf96ae31fadb040f0f68ea015">0e86bfa</a></td><td><code>Add fflas_ffpack_libs to library_order_list, to ensure that linbox_libs are linked before fflas_ffpack_libs.</code></td></tr></table>




---

archive/issue_comments_321325.json:
```json
{
    "body": "Changed commit from **[7bd021e](https://github.com/sagemath/sagetrac-mirror/commit/7bd021eb5a81dca655f75ff2e556180b9a11d73b)** to **[0e86bfa](https://github.com/sagemath/sagetrac-mirror/commit/0e86bfa475bd29bbf96ae31fadb040f0f68ea015)**",
    "created_at": "2016-11-07T12:33:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21683#issuecomment-321325",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7bd021e](https://github.com/sagemath/sagetrac-mirror/commit/7bd021eb5a81dca655f75ff2e556180b9a11d73b)** to **[0e86bfa](https://github.com/sagemath/sagetrac-mirror/commit/0e86bfa475bd29bbf96ae31fadb040f0f68ea015)**



---

archive/issue_comments_321326.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nRebased.",
    "created_at": "2016-11-07T12:34:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21683#issuecomment-321326",
    "user": "https://github.com/embray"
}
```

<div id="comment:7" align="right">Comment 7</div>

Rebased.



---

archive/issue_events_286383.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-11-07T12:34:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286383"
}
```



---

archive/issue_events_286384.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-11-07T12:34:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286384"
}
```



---

archive/issue_events_286385.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-07T12:43:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286385"
}
```



---

archive/issue_events_286386.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-07T12:43:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286386"
}
```



---

archive/issue_events_286387.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-07T12:43:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286387"
}
```



---

archive/issue_events_286388.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-07T12:43:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286388"
}
```



---

archive/issue_events_286389.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-11-08T23:42:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286389"
}
```



---

archive/issue_events_286390.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "16cbe2c00020c2ba14beb1d827d7ca765bba4f4d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-11-08T23:42:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21683#event-286390"
}
```



---

archive/issue_comments_321327.json:
```json
{
    "body": "Changed branch from **[u/embray/cygwin/fflas-libs](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/fflas-libs)** to **[0e86bfa](https://github.com/sagemath/sagetrac-mirror/commit/0e86bfa475bd29bbf96ae31fadb040f0f68ea015)**",
    "created_at": "2016-11-08T23:42:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21683#issuecomment-321327",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/cygwin/fflas-libs](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/cygwin/fflas-libs)** to **[0e86bfa](https://github.com/sagemath/sagetrac-mirror/commit/0e86bfa475bd29bbf96ae31fadb040f0f68ea015)**
