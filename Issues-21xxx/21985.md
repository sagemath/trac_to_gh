# Issue 21985: Implement _latex_ for FreeAlgebra and IwahoriHeckeAlgebra

archive/issues_021748.json:
```json
{
    "assignees": [],
    "body": "Some objects, such as `FreeAlgebra` and `IwahoriHeckeAlgebra`, do not implement the `_latex_` method.\n\nSee the [_latex_ method](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/algebras/letterplace/free_algebra_letterplace.pyx#n466) of the letterplace implementation of free algebras for an idea how to solve this issue.\n\nKeywords: **days79**\n\nBranch/Commit: **[dcbe269](https://github.com/sagemath/sagetrac-mirror/commit/dcbe269591987e3a9c2c5bd4f7f9416d9ebbc8f3)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21985_\n\n",
    "closed_at": "2016-12-18T23:17:22Z",
    "created_at": "2016-11-28T11:13:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement _latex_ for FreeAlgebra and IwahoriHeckeAlgebra",
    "type": "issue",
    "updated_at": "2016-12-18T23:17:22Z",
    "url": "https://github.com/sagemath/sage/issues/21985",
    "user": "https://github.com/mathzeta"
}
```
Some objects, such as `FreeAlgebra` and `IwahoriHeckeAlgebra`, do not implement the `_latex_` method.

See the [_latex_ method](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/algebras/letterplace/free_algebra_letterplace.pyx#n466) of the letterplace implementation of free algebras for an idea how to solve this issue.

Keywords: **days79**

Branch/Commit: **[dcbe269](https://github.com/sagemath/sagetrac-mirror/commit/dcbe269591987e3a9c2c5bd4f7f9416d9ebbc8f3)**

Reviewer: **Frédéric Chapoton**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/21985_





---

archive/issue_events_290073.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2016-11-28T11:13:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290073"
}
```



---

archive/issue_events_290074.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2016-11-28T11:13:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290074"
}
```



---

archive/issue_events_290075.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2016-11-28T11:13:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290075"
}
```



---

archive/issue_events_290076.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2016-11-28T11:13:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290076"
}
```



---

archive/issue_events_290077.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2016-11-28T11:13:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290077"
}
```



---

archive/issue_comments_326470.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dcbe269591987e3a9c2c5bd4f7f9416d9ebbc8f3\">dcbe269</a></td><td><code>Latex method for free algebras and Iwahori-Hecke algebras.</code></td></tr></table>\n",
    "created_at": "2016-12-06T16:28:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21985#issuecomment-326470",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dcbe269591987e3a9c2c5bd4f7f9416d9ebbc8f3">dcbe269</a></td><td><code>Latex method for free algebras and Iwahori-Hecke algebras.</code></td></tr></table>




---

archive/issue_comments_326471.json:
```json
{
    "body": "Branch: **[public/algebra/latex_some_algebras-21985](https://github.com/sagemath/sagetrac-mirror/tree/public/algebra/latex_some_algebras-21985)**",
    "created_at": "2016-12-06T16:28:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21985#issuecomment-326471",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/algebra/latex_some_algebras-21985](https://github.com/sagemath/sagetrac-mirror/tree/public/algebra/latex_some_algebras-21985)**



---

archive/issue_events_290078.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-12-06T16:28:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290078"
}
```



---

archive/issue_comments_326472.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2016-12-06T16:28:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21985#issuecomment-326472",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_comments_326473.json:
```json
{
    "body": "Commit: **[dcbe269](https://github.com/sagemath/sagetrac-mirror/commit/dcbe269591987e3a9c2c5bd4f7f9416d9ebbc8f3)**",
    "created_at": "2016-12-06T16:28:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21985#issuecomment-326473",
    "user": "https://github.com/tscrim"
}
```

Commit: **[dcbe269](https://github.com/sagemath/sagetrac-mirror/commit/dcbe269591987e3a9c2c5bd4f7f9416d9ebbc8f3)**



---

archive/issue_comments_326474.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-12-14T19:17:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21985#issuecomment-326474",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_326475.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok, thanks",
    "created_at": "2016-12-14T19:17:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21985#issuecomment-326475",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok, thanks



---

archive/issue_events_290079.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-12-14T19:17:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290079"
}
```



---

archive/issue_events_290080.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-12-14T19:17:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290080"
}
```



---

archive/issue_events_290081.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-12-18T23:17:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290081"
}
```



---

archive/issue_events_290082.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "77f3af530868bfaa326fc175cd7868ade1b9e8b3",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-12-18T23:17:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21985#event-290082"
}
```



---

archive/issue_comments_326476.json:
```json
{
    "body": "Changed branch from **[public/algebra/latex_some_algebras-21985](https://github.com/sagemath/sagetrac-mirror/tree/public/algebra/latex_some_algebras-21985)** to **[dcbe269](https://github.com/sagemath/sagetrac-mirror/commit/dcbe269591987e3a9c2c5bd4f7f9416d9ebbc8f3)**",
    "created_at": "2016-12-18T23:17:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21985#issuecomment-326476",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/algebra/latex_some_algebras-21985](https://github.com/sagemath/sagetrac-mirror/tree/public/algebra/latex_some_algebras-21985)** to **[dcbe269](https://github.com/sagemath/sagetrac-mirror/commit/dcbe269591987e3a9c2c5bd4f7f9416d9ebbc8f3)**
