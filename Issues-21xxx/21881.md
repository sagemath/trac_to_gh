# Issue 21881: allow to use vertex_colors in js display of graphs

archive/issues_021644.json:
```json
{
    "assignees": [],
    "body": "a simple change to be able to use the vertex_colors keyword if it is present, in the method='js' option of graphs \"show\"\n\nthe vertex_colors dict will only be used as a partition of vertices, given colors will not be respected\n\nCC:  @tscrim @jm58660 @darijgr\n\nBranch/Commit: **[daf1b2e](https://github.com/sagemath/sagetrac-mirror/commit/daf1b2e3436de0410ddac6c9b79e7bc36e969f4c)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21881_\n\n",
    "closed_at": "2016-11-17T18:01:40Z",
    "created_at": "2016-11-15T16:36:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "allow to use vertex_colors in js display of graphs",
    "type": "issue",
    "updated_at": "2016-11-17T18:01:40Z",
    "url": "https://github.com/sagemath/sage/issues/21881",
    "user": "https://github.com/fchapoton"
}
```
a simple change to be able to use the vertex_colors keyword if it is present, in the method='js' option of graphs "show"

the vertex_colors dict will only be used as a partition of vertices, given colors will not be respected

CC:  @tscrim @jm58660 @darijgr

Branch/Commit: **[daf1b2e](https://github.com/sagemath/sagetrac-mirror/commit/daf1b2e3436de0410ddac6c9b79e7bc36e969f4c)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/21881_





---

archive/issue_events_288905.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-15T16:36:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21881#event-288905"
}
```



---

archive/issue_events_288906.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-15T16:36:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21881#event-288906"
}
```



---

archive/issue_events_288907.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-15T16:36:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21881#event-288907"
}
```



---

archive/issue_events_288908.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-15T16:36:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21881#event-288908"
}
```



---

archive/issue_comments_325100.json:
```json
{
    "body": "Commit: **[daf1b2e](https://github.com/sagemath/sagetrac-mirror/commit/daf1b2e3436de0410ddac6c9b79e7bc36e969f4c)**",
    "created_at": "2016-11-15T16:37:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21881#issuecomment-325100",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[daf1b2e](https://github.com/sagemath/sagetrac-mirror/commit/daf1b2e3436de0410ddac6c9b79e7bc36e969f4c)**



---

archive/issue_comments_325101.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/daf1b2e3436de0410ddac6c9b79e7bc36e969f4c\">daf1b2e</a></td><td><code>allow partial use of vertex_colors in js display of graphs</code></td></tr></table>\n",
    "created_at": "2016-11-15T16:37:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21881#issuecomment-325101",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/daf1b2e3436de0410ddac6c9b79e7bc36e969f4c">daf1b2e</a></td><td><code>allow partial use of vertex_colors in js display of graphs</code></td></tr></table>




---

archive/issue_comments_325102.json:
```json
{
    "body": "Branch: **[u/chapoton/21881](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/21881)**",
    "created_at": "2016-11-15T16:37:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21881#issuecomment-325102",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/21881](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/21881)**



---

archive/issue_events_288909.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-15T16:37:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21881#event-288909"
}
```



---

archive/issue_events_288910.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-11-15T19:45:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21881#event-288910"
}
```



---

archive/issue_events_288911.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-11-15T19:45:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21881#event-288911"
}
```



---

archive/issue_comments_325103.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-11-15T19:45:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21881#issuecomment-325103",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_325104.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\ngreat, thanks !",
    "created_at": "2016-11-15T19:46:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21881#issuecomment-325104",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

great, thanks !



---

archive/issue_events_288912.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-11-17T18:01:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21881#event-288912"
}
```



---

archive/issue_events_288913.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5b94994bdfeecbf9389fab42e5f2ab31aff70d5f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-11-17T18:01:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21881#event-288913"
}
```



---

archive/issue_comments_325105.json:
```json
{
    "body": "Changed branch from **[u/chapoton/21881](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/21881)** to **[daf1b2e](https://github.com/sagemath/sagetrac-mirror/commit/daf1b2e3436de0410ddac6c9b79e7bc36e969f4c)**",
    "created_at": "2016-11-17T18:01:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21881",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21881#issuecomment-325105",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/21881](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/21881)** to **[daf1b2e](https://github.com/sagemath/sagetrac-mirror/commit/daf1b2e3436de0410ddac6c9b79e7bc36e969f4c)**
