# Issue 21040: Display inline graphics in Sage REPL if running in iTerm2 terminal on macOS

archive/issues_020803.json:
```json
{
    "assignees": [],
    "body": "This ticket is to display inline grpahics in the Sage REPL\nif it is running in an iTerm2 terminal on macOS.\n\n- iTerm2: https://www.iterm2.com/\n- Python interface: https://github.com/asmeurer/iterm2-tools\n- Detect whether iTerm2 is running (should work across ssh):\n  https://raw.githubusercontent.com/gnachman/iTerm2/master/tests/isiterm2.sh\n\nRelated ticket:\n\n- #26544: add SIXEL support for terminal display\n\n**CC:**  @slel\n\n**Keywords:** graphics, repl, inline, macOS, terminal\n\nIssue created by migration from https://trac.sagemath.org/ticket/21040\n\n",
    "created_at": "2016-07-17T19:57:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wishlist"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Display inline graphics in Sage REPL if running in iTerm2 terminal on macOS",
    "type": "issue",
    "updated_at": "2018-10-24T15:16:07Z",
    "url": "https://github.com/sagemath/sage/issues/21040",
    "user": "https://github.com/mkoeppe"
}
```
This ticket is to display inline grpahics in the Sage REPL
if it is running in an iTerm2 terminal on macOS.

- iTerm2: https://www.iterm2.com/
- Python interface: https://github.com/asmeurer/iterm2-tools
- Detect whether iTerm2 is running (should work across ssh):
  https://raw.githubusercontent.com/gnachman/iTerm2/master/tests/isiterm2.sh

Related ticket:

- #26544: add SIXEL support for terminal display

**CC:**  @slel

**Keywords:** graphics, repl, inline, macOS, terminal

Issue created by migration from https://trac.sagemath.org/ticket/21040





---

archive/issue_events_256032.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-07-17T19:57:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21040",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "08517b",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21040#event-256032"
}
```



---

archive/issue_events_256033.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-07-17T19:57:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21040",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21040#event-256033"
}
```



---

archive/issue_events_256034.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-07-17T19:57:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21040",
    "label": "https://github.com/sagemath/sage/labels/wishlist",
    "label_color": "008080",
    "label_name": "wishlist",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21040#event-256034"
}
```



---

archive/issue_events_256035.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-10-24T15:16:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21040",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21040#event-256035"
}
```



---

archive/issue_comments_308420.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"graphics, repl, inline, macOS, terminal\".",
    "created_at": "2018-10-24T15:16:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21040",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21040#issuecomment-308420",
    "user": "https://github.com/slel"
}
```

**Changing keywords** from "" to "graphics, repl, inline, macOS, terminal".



---

archive/issue_comments_308421.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,7 +1,11 @@\n-iTerm2:\n- https://www.iterm2.com/\n-Python interface:\n- https://github.com/asmeurer/iterm2-tools\n-Detect whether iTerm2 is running (should work across ssh):\n- https://raw.githubusercontent.com/gnachman/iTerm2/master/tests/isiterm2.sh\n+This ticket is to display inline grpahics in the Sage REPL\n+if it is running in an iTerm2 terminal on macOS.\n \n+- iTerm2: https://www.iterm2.com/\n+- Python interface: https://github.com/asmeurer/iterm2-tools\n+- Detect whether iTerm2 is running (should work across ssh):\n+  https://raw.githubusercontent.com/gnachman/iTerm2/master/tests/isiterm2.sh\n+\n+Related ticket:\n+\n+- #26544: add SIXEL support for terminal display\n``````\n",
    "created_at": "2018-10-24T15:16:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21040",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21040#issuecomment-308421",
    "user": "https://github.com/slel"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,7 +1,11 @@
-iTerm2:
- https://www.iterm2.com/
-Python interface:
- https://github.com/asmeurer/iterm2-tools
-Detect whether iTerm2 is running (should work across ssh):
- https://raw.githubusercontent.com/gnachman/iTerm2/master/tests/isiterm2.sh
+This ticket is to display inline grpahics in the Sage REPL
+if it is running in an iTerm2 terminal on macOS.
 
+- iTerm2: https://www.iterm2.com/
+- Python interface: https://github.com/asmeurer/iterm2-tools
+- Detect whether iTerm2 is running (should work across ssh):
+  https://raw.githubusercontent.com/gnachman/iTerm2/master/tests/isiterm2.sh
+
+Related ticket:
+
+- #26544: add SIXEL support for terminal display
``````




---

archive/issue_events_256036.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-10-24T15:16:07Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/21040",
    "rename": {
        "from": "Display inline graphics in terminal if running in iTerm 2 on Mac OS X",
        "to": "Display inline graphics in Sage REPL if running in iTerm2 terminal on macOS"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21040#event-256036"
}
```
