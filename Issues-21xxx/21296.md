# Issue 21296: get rid of some useless .keys

archive/issues_021059.json:
```json
{
    "assignees": [],
    "body": "with an eye towards python3 compatibility\n\nwhen iterating over a dict, one does not need to call .keys()\n\nand when asking for the list of keys, it is better to call list(d) for compatibility with py3\n\nRemoving .keys() when possible is performed in some files, not everywhere.\n\nCC:  @jm58660 @tscrim @a-andre\n\nBranch: **[45377ee](https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21296_\n\n",
    "closed_at": "2016-08-21T13:13:38Z",
    "created_at": "2016-08-19T13:03:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "get rid of some useless .keys",
    "type": "issue",
    "updated_at": "2016-08-22T08:25:46Z",
    "url": "https://github.com/sagemath/sage/issues/21296",
    "user": "https://github.com/fchapoton"
}
```
with an eye towards python3 compatibility

when iterating over a dict, one does not need to call .keys()

and when asking for the list of keys, it is better to call list(d) for compatibility with py3

Removing .keys() when possible is performed in some files, not everywhere.

CC:  @jm58660 @tscrim @a-andre

Branch: **[45377ee](https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e)**

Reviewer: **Jeroen Demeyer**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/21296_





---

archive/issue_events_281425.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-19T13:03:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21296#event-281425"
}
```



---

archive/issue_events_281426.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-19T13:03:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21296#event-281426"
}
```



---

archive/issue_events_281427.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-19T13:03:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21296#event-281427"
}
```



---

archive/issue_events_281428.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-19T13:03:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21296#event-281428"
}
```



---

archive/issue_events_281429.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-19T13:03:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21296#event-281429"
}
```



---

archive/issue_comments_314086.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e\">45377ee</a></td><td><code>getting rid of some useless .keys()</code></td></tr></table>\n",
    "created_at": "2016-08-19T13:03:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21296#issuecomment-314086",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e">45377ee</a></td><td><code>getting rid of some useless .keys()</code></td></tr></table>




---

archive/issue_comments_314087.json:
```json
{
    "body": "Commit: **[45377ee](https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e)**",
    "created_at": "2016-08-19T13:03:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21296#issuecomment-314087",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[45377ee](https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e)**



---

archive/issue_comments_314088.json:
```json
{
    "body": "Branch: **[u/chapoton/21296](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/21296)**",
    "created_at": "2016-08-19T13:03:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21296#issuecomment-314088",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/21296](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/21296)**



---

archive/issue_comments_314089.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\npatchbot is green (except for a timeout problem in Tutte polynomials which happens for every ticket on this patchbot)",
    "created_at": "2016-08-19T18:21:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21296#issuecomment-314089",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

patchbot is green (except for a timeout problem in Tutte polynomials which happens for every ticket on this patchbot)



---

archive/issue_comments_314090.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2016-08-19T19:13:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21296#issuecomment-314090",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_281430.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-19T19:13:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21296#event-281430"
}
```



---

archive/issue_events_281431.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-08-19T19:13:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21296#event-281431"
}
```



---

archive/issue_comments_314091.json:
```json
{
    "body": "Changed branch from **[u/chapoton/21296](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/21296)** to **[45377ee](https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e)**",
    "created_at": "2016-08-21T13:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21296#issuecomment-314091",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/21296](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/21296)** to **[45377ee](https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e)**



---

archive/issue_events_281432.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-08-21T13:13:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21296#event-281432"
}
```



---

archive/issue_events_281433.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fc7ec95cfed656f24819ea5268884188c7267e71",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-08-21T13:13:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21296#event-281433"
}
```



---

archive/issue_comments_314092.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nsequel in #21304",
    "created_at": "2016-08-22T08:25:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21296#issuecomment-314092",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

sequel in #21304



---

archive/issue_comments_314093.json:
```json
{
    "body": "Changed commit from **[45377ee](https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e)** to none",
    "created_at": "2016-08-22T08:25:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21296",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21296#issuecomment-314093",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[45377ee](https://github.com/sagemath/sagetrac-mirror/commit/45377eecadf076cf5bb0dc771f37250611a6c41e)** to none
