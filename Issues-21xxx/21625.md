# Issue 21625: Strange linking order for omalloc due to pkgconfig bug

archive/issues_021388.json:
```json
{
    "body": "This is a follow up to #17254 which upgraded to singular 4:\n\nThe linking order for Singular libraries is determined by `pkgconfig` which gives\n\n```\n-lomalloc -lSingular -lpolys -lfactory -lresources\n```\nThis does not look that right.\n\nThis is because `pkgconfig` uses sets that don't preserve the order of libraries (and include directories or macros etc...).\n\nA solution is to migrate `pkgconfig`  from sets to list.\n\nhttps://github.com/matze/pkgconfig/issues/10 \n\nhas been opened upstream to explain the rational, along with a pull request to implement it at \n\nhttps://github.com/matze/pkgconfig/pull/11\n\nThis has now been accepted upstream and is part of a new release with a few other fixes.\n\n**Tarball**: [https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz](https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz)\n\nUpstream: Fixed upstream, in a later stable release.\n\nReviewer: Jeroen Demeyer\n\nAuthor: Fran\u00e7ois Bissey\n\nBranch: bd61fc49a33cb0c3c764ba15c0262d344b2e78a1\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/21625\n\n",
    "closed_at": "2016-12-07T23:29:59Z",
    "created_at": "2016-10-03T10:05:10Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.5",
    "title": "Strange linking order for omalloc due to pkgconfig bug",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/21625",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```
This is a follow up to #17254 which upgraded to singular 4:

The linking order for Singular libraries is determined by `pkgconfig` which gives

```
-lomalloc -lSingular -lpolys -lfactory -lresources
```
This does not look that right.

This is because `pkgconfig` uses sets that don't preserve the order of libraries (and include directories or macros etc...).

A solution is to migrate `pkgconfig`  from sets to list.

https://github.com/matze/pkgconfig/issues/10 

has been opened upstream to explain the rational, along with a pull request to implement it at 

https://github.com/matze/pkgconfig/pull/11

This has now been accepted upstream and is part of a new release with a few other fixes.

**Tarball**: [https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz](https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz)

Upstream: Fixed upstream, in a later stable release.

Reviewer: Jeroen Demeyer

Author: François Bissey

Branch: bd61fc49a33cb0c3c764ba15c0262d344b2e78a1

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/21625





---

archive/issue_comments_398991.json:
```json
{
    "body": "Replying to [ticket:21625 jpflori]:\n> This is a follow up to #17254 which upgraded to singular 4:\n> \n> * strange linking order (omalloc comes before singular!) \n\n\nYou mean during the build of Singular itself or Sage modules using Singular?",
    "created_at": "2016-10-21T08:09:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-398991",
    "user": "https://github.com/jdemeyer"
}
```

Replying to [ticket:21625 jpflori]:
> This is a follow up to #17254 which upgraded to singular 4:
> 
> * strange linking order (omalloc comes before singular!) 


You mean during the build of Singular itself or Sage modules using Singular?



---

archive/issue_comments_398992.json:
```json
{
    "body": "<a id='comment:2'></a>\nSee #17254#comment:510 :\n\"\"\"\nDuring linking some Sage files I see -lomalloc -lSingular -lpolys -lfactory -lresources which does not look that right.\n\"\"\"",
    "created_at": "2016-10-21T08:12:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-398992",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

<a id='comment:2'></a>
See #17254#comment:510 :
"""
During linking some Sage files I see -lomalloc -lSingular -lpolys -lfactory -lresources which does not look that right.
"""



---

archive/issue_events_057368.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-10-21T08:16:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "milestone": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/21625#event-57368"
}
```



---

archive/issue_comments_398993.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,8 @@\n This is a follow up to #17254 which upgraded to singular 4:\n \n-* strange linking order (omalloc comes before singular!) \n+The linking order for Singular libraries is determined by `pkgconfig` which gives\n+\n+```\n+-lomalloc -lSingular -lpolys -lfactory -lresources\n+```\n+This does not look that right.\n``````\n",
    "created_at": "2016-10-21T08:16:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-398993",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,8 @@
 This is a follow up to #17254 which upgraded to singular 4:
 
-* strange linking order (omalloc comes before singular!) 
+The linking order for Singular libraries is determined by `pkgconfig` which gives
+
+```
+-lomalloc -lSingular -lpolys -lfactory -lresources
+```
+This does not look that right.
``````




---

archive/issue_comments_398994.json:
```json
{
    "body": "<a id='comment:4'></a>\nThat's an annoying defect that I have noticed before in the python interface to pkg-config. The libraries are put in a set and of course it is not preserving the order. It was already evident when using ATLAS for blas/lapack.\n\n```\n$ pkg-config --libs Singular\n-lSingular -ldl -lpolys -lflint -lmpfr -lntl -lgmp -ldl -lfactory -lflint -lmpfr -lntl -lgmp -lomalloc -lresources \n```\nbut\n\n```\n$ python\nPython 2.7.10 (default, Jun 12 2016, 19:30:46) \n[GCC 5.3.0] on linux2\nType \"help\", \"copyright\", \"credits\" or \"license\" for more information.\n>>> import pkgconfig\n>>> singular_pc = pkgconfig.parse('Singular')\n>>> list(singular_pc['libraries'])\n[u'omalloc', u'dl', u'Singular', u'polys', u'factory', u'flint', u'ntl', u'gmp', u'resources', u'mpfr']\n>>> singular_pc['libraries']\nset([u'omalloc', u'dl', u'Singular', u'polys', u'factory', u'flint', u'ntl', u'gmp', u'resources', u'mpfr'])\n```\nwhich is what you are seeing. If you still have a sage with ATLAS somewhere look at what you get for lapack. This needs to be fixed upstream so that something preserving the order is used instead of a set.",
    "created_at": "2016-10-23T08:23:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-398994",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:4'></a>
That's an annoying defect that I have noticed before in the python interface to pkg-config. The libraries are put in a set and of course it is not preserving the order. It was already evident when using ATLAS for blas/lapack.

```
$ pkg-config --libs Singular
-lSingular -ldl -lpolys -lflint -lmpfr -lntl -lgmp -ldl -lfactory -lflint -lmpfr -lntl -lgmp -lomalloc -lresources 
```
but

```
$ python
Python 2.7.10 (default, Jun 12 2016, 19:30:46) 
[GCC 5.3.0] on linux2
Type "help", "copyright", "credits" or "license" for more information.
>>> import pkgconfig
>>> singular_pc = pkgconfig.parse('Singular')
>>> list(singular_pc['libraries'])
[u'omalloc', u'dl', u'Singular', u'polys', u'factory', u'flint', u'ntl', u'gmp', u'resources', u'mpfr']
>>> singular_pc['libraries']
set([u'omalloc', u'dl', u'Singular', u'polys', u'factory', u'flint', u'ntl', u'gmp', u'resources', u'mpfr'])
```
which is what you are seeing. If you still have a sage with ATLAS somewhere look at what you get for lapack. This needs to be fixed upstream so that something preserving the order is used instead of a set.



---

archive/issue_comments_398995.json:
```json
{
    "body": "<a id='comment:5'></a>\nIs this upstream:\n* https://github.com/matze/pkgconfig\n?\n\nIt does not look very active, last release was three years ago.",
    "created_at": "2016-10-25T12:25:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-398995",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

<a id='comment:5'></a>
Is this upstream:
* https://github.com/matze/pkgconfig
?

It does not look very active, last release was three years ago.



---

archive/issue_comments_398996.json:
```json
{
    "body": "<a id='comment:6'></a>\nDebian also thinks that is the homepage, yes.",
    "created_at": "2016-11-12T18:36:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-398996",
    "user": "https://github.com/infinity0"
}
```

<a id='comment:6'></a>
Debian also thinks that is the homepage, yes.



---

archive/issue_comments_398997.json:
```json
{
    "body": "<a id='comment:7'></a>\nWell `pkg-config` is relatively stable. The nice thing about the set is of course that you don't have repeated elements in it. Is there an \"ordered set\" type in python?",
    "created_at": "2016-11-12T18:55:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-398997",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:7'></a>
Well `pkg-config` is relatively stable. The nice thing about the set is of course that you don't have repeated elements in it. Is there an "ordered set" type in python?



---

archive/issue_comments_398998.json:
```json
{
    "body": "<a id='comment:8'></a>\nAfter a bit of googling I think upstream should either depend from an \"ordered set\" package or switch to a dictionary of list instead of a dictionary of set and then remove duplication.",
    "created_at": "2016-11-12T20:08:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-398998",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:8'></a>
After a bit of googling I think upstream should either depend from an "ordered set" package or switch to a dictionary of list instead of a dictionary of set and then remove duplication.



---

archive/issue_comments_398999.json:
```json
{
    "body": "<a id='comment:9'></a>\nI have been working on making `pkgconfig` use lists instead of sets and then doing some deduplication. \n\nThere are two issues with deduplication - which was originally achieved by using sets:\n* Is it really necessary, that makes longer lines but `pkg-config` doesn't do it in the first place. Which brings us to second issue\n* Currently the first instance of everything is kept. Not a problem for macros, may be for include directories and library directories. Definitely a potential problem for libraries.\n\nI don't think deduplication - potentially from the end of the list rather than the beginning of it, is worth it.\n\nOpinion?",
    "created_at": "2016-11-14T01:38:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-398999",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:9'></a>
I have been working on making `pkgconfig` use lists instead of sets and then doing some deduplication. 

There are two issues with deduplication - which was originally achieved by using sets:
* Is it really necessary, that makes longer lines but `pkg-config` doesn't do it in the first place. Which brings us to second issue
* Currently the first instance of everything is kept. Not a problem for macros, may be for include directories and library directories. Definitely a potential problem for libraries.

I don't think deduplication - potentially from the end of the list rather than the beginning of it, is worth it.

Opinion?



---

archive/issue_comments_399000.json:
```json
{
    "body": "<a id='comment:10'></a>\nI agree with Fran\u00e7ois: we shouldn't be smarter than necessary. There is nothing wrong with longer command lines (as long as it's not excessive).",
    "created_at": "2016-11-14T07:04:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399000",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>
I agree with François: we shouldn't be smarter than necessary. There is nothing wrong with longer command lines (as long as it's not excessive).



---

archive/issue_comments_399001.json:
```json
{
    "body": "<a id='comment:11'></a>\nOK, I definitely think it is not worth the bother so I'll probably go with this branch for my upstream pull request https://github.com/kiwifb/pkgconfig/tree/listify. If anyone wants to comment, now is the time.",
    "created_at": "2016-11-14T20:54:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399001",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:11'></a>
OK, I definitely think it is not worth the bother so I'll probably go with this branch for my upstream pull request https://github.com/kiwifb/pkgconfig/tree/listify. If anyone wants to comment, now is the time.



---

archive/issue_comments_399002.json:
```json
{
    "body": "<a id='comment:12'></a>\nUpstream accepted my change, hopefully that will come with a new release soon. I did some experiments, while we will be able to simplify sage's code to populate the various variables `*_{libs,library_dirs,include_dir}` it should work out of the box without modification (haven't checked cflags actually).",
    "created_at": "2016-11-16T10:28:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399002",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:12'></a>
Upstream accepted my change, hopefully that will come with a new release soon. I did some experiments, while we will be able to simplify sage's code to populate the various variables `*_{libs,library_dirs,include_dir}` it should work out of the box without modification (haven't checked cflags actually).



---

archive/issue_comments_399003.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"Fixed upstream, but not in a stable release.\"",
    "created_at": "2016-11-16T20:18:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399003",
    "user": "https://github.com/kiwifb"
}
```

Changing upstream from "N/A" to "Fixed upstream, but not in a stable release."



---

archive/issue_comments_399004.json:
```json
{
    "body": "<a id='comment:13'></a>\nNot sure when upstream will cut a release, should we patch in the meantime?",
    "created_at": "2016-11-16T20:18:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399004",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:13'></a>
Not sure when upstream will cut a release, should we patch in the meantime?



---

archive/issue_comments_399005.json:
```json
{
    "body": "<a id='comment:14'></a>\nI say we patch.",
    "created_at": "2016-11-17T13:44:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399005",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

<a id='comment:14'></a>
I say we patch.



---

archive/issue_comments_399006.json:
```json
{
    "body": "<a id='comment:15'></a>\nWorking on this. Some adjustments are needed in `modules_list.py`.",
    "created_at": "2016-11-22T22:16:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399006",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:15'></a>
Working on this. Some adjustments are needed in `modules_list.py`.



---

archive/issue_comments_399007.json:
```json
{
    "body": "<a id='comment:16'></a>\nNow ready for review.\n\n---\nNew commits:\n|                                                                                                                                          |                                                |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------|\n|[9d4de4a](https://git.sagemath.org/sage.git/commit?id=9d4de4a9ee44c5d813a6031b93cd882c598af3ba)|`Add https://github.com/matze/pkgconfig/pull/11`|\n|[65a1d2a](https://git.sagemath.org/sage.git/commit?id=65a1d2a379b2fe16f09c598153cae5544d13da15)|`migrating numpy helper script to use list`|\n|[0accd59](https://git.sagemath.org/sage.git/commit?id=0accd599126af79c272238689343bddcc45637c8)|`migrate module_list.py to use list from pkgconfig`|",
    "created_at": "2016-11-22T23:51:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399007",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:16'></a>
Now ready for review.

---
New commits:
|                                                                                                                                          |                                                |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------|
|[9d4de4a](https://git.sagemath.org/sage.git/commit?id=9d4de4a9ee44c5d813a6031b93cd882c598af3ba)|`Add https://github.com/matze/pkgconfig/pull/11`|
|[65a1d2a](https://git.sagemath.org/sage.git/commit?id=65a1d2a379b2fe16f09c598153cae5544d13da15)|`migrating numpy helper script to use list`|
|[0accd59](https://git.sagemath.org/sage.git/commit?id=0accd599126af79c272238689343bddcc45637c8)|`migrate module_list.py to use list from pkgconfig`|



---

archive/issue_comments_399008.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2016-11-22T23:51:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399008",
    "user": "https://github.com/kiwifb"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_399009.json:
```json
{
    "body": "Changing commit from \"\" to \"0accd599126af79c272238689343bddcc45637c8\"",
    "created_at": "2016-11-22T23:51:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399009",
    "user": "https://github.com/kiwifb"
}
```

Changing commit from "" to "0accd599126af79c272238689343bddcc45637c8"



---

archive/issue_comments_399010.json:
```json
{
    "body": "Changing author from \"\" to \"Fran\u00e7ois Bissey\"",
    "created_at": "2016-11-22T23:51:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399010",
    "user": "https://github.com/kiwifb"
}
```

Changing author from "" to "François Bissey"



---

archive/issue_comments_399011.json:
```json
{
    "body": "Changing branch from \"\" to \"u/fbissey/pkgconfig\"",
    "created_at": "2016-11-22T23:51:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399011",
    "user": "https://github.com/kiwifb"
}
```

Changing branch from "" to "u/fbissey/pkgconfig"



---

archive/issue_comments_399012.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2016-11-23T07:11:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399012",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_399013.json:
```json
{
    "body": "<a id='comment:17'></a>\nFor reference, please add a link to the upstream ticket in the patch file and in the ticket description.",
    "created_at": "2016-11-23T07:11:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399013",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:17'></a>
For reference, please add a link to the upstream ticket in the patch file and in the ticket description.



---

archive/issue_comments_399014.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,3 +6,16 @@\n -lomalloc -lSingular -lpolys -lfactory -lresources\n ```\n This does not look that right.\n+\n+This is because `pkgconfig` uses sets that don't preserve the order of libraries (and include directories or macros etc...).\n+\n+A solution is to migrate `pkgconfig`  from sets to list.\n+\n+https://github.com/matze/pkgconfig/issues/10 \n+\n+has been opened upstream to explain the rational, along with a pull request to implement it at \n+\n+https://github.com/matze/pkgconfig/pull/11\n+\n+This has now been accepted upstream but is not part of a release yet.\n+\n``````\n",
    "created_at": "2016-11-24T20:43:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399014",
    "user": "https://github.com/kiwifb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,3 +6,16 @@
 -lomalloc -lSingular -lpolys -lfactory -lresources
 ```
 This does not look that right.
+
+This is because `pkgconfig` uses sets that don't preserve the order of libraries (and include directories or macros etc...).
+
+A solution is to migrate `pkgconfig`  from sets to list.
+
+https://github.com/matze/pkgconfig/issues/10 
+
+has been opened upstream to explain the rational, along with a pull request to implement it at 
+
+https://github.com/matze/pkgconfig/pull/11
+
+This has now been accepted upstream but is not part of a release yet.
+
``````




---

archive/issue_comments_399015.json:
```json
{
    "body": "<a id='comment:19'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                       |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------|\n|[07b5632](https://git.sagemath.org/sage.git/commit/?id=07b5632ab1be45ca816b1396105bfc98743460bf)|`Add upstream information to the patch`|",
    "created_at": "2016-11-24T20:44:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399015",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:19'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                       |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------|
|[07b5632](https://git.sagemath.org/sage.git/commit/?id=07b5632ab1be45ca816b1396105bfc98743460bf)|`Add upstream information to the patch`|



---

archive/issue_comments_399016.json:
```json
{
    "body": "Changing commit from \"0accd599126af79c272238689343bddcc45637c8\" to \"07b5632ab1be45ca816b1396105bfc98743460bf\"",
    "created_at": "2016-11-24T20:44:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399016",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "0accd599126af79c272238689343bddcc45637c8" to "07b5632ab1be45ca816b1396105bfc98743460bf"



---

archive/issue_comments_399017.json:
```json
{
    "body": "<a id='comment:20'></a>\nHope it is satisfactory.",
    "created_at": "2016-11-24T20:44:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399017",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:20'></a>
Hope it is satisfactory.



---

archive/issue_comments_399018.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2016-11-24T20:44:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399018",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_399019.json:
```json
{
    "body": "<a id='comment:21'></a>\nWould it make sense to bump the Singular version to force a rebuild with the new `pkgconfig`?",
    "created_at": "2016-11-25T06:06:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399019",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:21'></a>
Would it make sense to bump the Singular version to force a rebuild with the new `pkgconfig`?



---

archive/issue_comments_399020.json:
```json
{
    "body": "<a id='comment:22'></a>\nI guess so, of course we need to bump `singular` one way or another before 7.5 is released. But I can easily add a commit here and have the singular ticket depend on it.",
    "created_at": "2016-11-25T06:32:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399020",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:22'></a>
I guess so, of course we need to bump `singular` one way or another before 7.5 is released. But I can easily add a commit here and have the singular ticket depend on it.



---

archive/issue_comments_399021.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2016-11-25T09:46:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399021",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_comments_399022.json:
```json
{
    "body": "<a id='comment:24'></a>\nReplying to [fbissey](#comment%3A22):\n> I guess so, of course we need to bump `singular` one way or another before 7.5 is released. But I can easily add a commit here and have the singular ticket depend on it.\n\n\nOn second thought, the problem is really in `pkgconfig` (the Python package) which simply reads the system `.pc` files. So there is no need to regenerate the `.pc` files. So no further action is needed I guess.\n\nI am now going to do a complete build from scratch with this ticket and see if it works...",
    "created_at": "2016-11-25T09:48:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399022",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:24'></a>
Replying to [fbissey](#comment%3A22):
> I guess so, of course we need to bump `singular` one way or another before 7.5 is released. But I can easily add a commit here and have the singular ticket depend on it.


On second thought, the problem is really in `pkgconfig` (the Python package) which simply reads the system `.pc` files. So there is no need to regenerate the `.pc` files. So no further action is needed I guess.

I am now going to do a complete build from scratch with this ticket and see if it works...



---

archive/issue_comments_399023.json:
```json
{
    "body": "<a id='comment:25'></a>\nThis seems to break building scipy, log attached.",
    "created_at": "2016-11-26T09:13:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399023",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:25'></a>
This seems to break building scipy, log attached.



---

archive/issue_comments_399024.json:
```json
{
    "body": "Attachment [scipy-0.17.1.p0.log](tarball://root/attachments/some-uuid/ticket21625/scipy-0.17.1.p0.log) by @jdemeyer created at 2016-11-26 09:14:29\n\nLog of failed scipy build",
    "created_at": "2016-11-26T09:14:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399024",
    "user": "https://github.com/jdemeyer"
}
```

Attachment [scipy-0.17.1.p0.log](tarball://root/attachments/some-uuid/ticket21625/scipy-0.17.1.p0.log) by @jdemeyer created at 2016-11-26 09:14:29

Log of failed scipy build



---

archive/issue_comments_399025.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2016-11-26T09:14:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399025",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_399026.json:
```json
{
    "body": "<a id='comment:27'></a>\nHum, I can reproduce it. It is triggered by the `site.cfg` generated for `numpy` (installed at `local/lib/python2.7/site-packages/numpy/distutils/site.cfg`). There is now a repetition of `openblas` for `blas_libs` and that seems to be the issue strangely enough (it should be able to take more than one library).",
    "created_at": "2016-11-26T09:38:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399026",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:27'></a>
Hum, I can reproduce it. It is triggered by the `site.cfg` generated for `numpy` (installed at `local/lib/python2.7/site-packages/numpy/distutils/site.cfg`). There is now a repetition of `openblas` for `blas_libs` and that seems to be the issue strangely enough (it should be able to take more than one library).



---

archive/issue_comments_399027.json:
```json
{
    "body": "<a id='comment:28'></a>\nOk `pkgconfig` (python) does another thing that's weird compared to `pkg-config`. I can do this\n\n```\n(sage-sh) fbissey@QCD-nzi3:sage$ pkg-config --libs blas cblas\n-L/home/fbissey/sandbox/git-fork/sage/local/lib -lopenblas \n```\nbut `pkgconfig` (python) will do two calls to `pkg-config` and add them together. Of course with sets that wasn't visible. In any case we should be able to survive with just `blas` in `numpy`'s `site.cfg`. I am not sure why Gentoo insists on both - possibly historical reasons.",
    "created_at": "2016-11-26T09:51:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399027",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:28'></a>
Ok `pkgconfig` (python) does another thing that's weird compared to `pkg-config`. I can do this

```
(sage-sh) fbissey@QCD-nzi3:sage$ pkg-config --libs blas cblas
-L/home/fbissey/sandbox/git-fork/sage/local/lib -lopenblas 
```
but `pkgconfig` (python) will do two calls to `pkg-config` and add them together. Of course with sets that wasn't visible. In any case we should be able to survive with just `blas` in `numpy`'s `site.cfg`. I am not sure why Gentoo insists on both - possibly historical reasons.



---

archive/issue_comments_399028.json:
```json
{
    "body": "<a id='comment:29'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                  |\n|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------|\n|[9dd50b6](https://git.sagemath.org/sage.git/commit/?id=9dd50b6a72f15f173bac66720fc77e10c4ee087b)|`Only query blas for numpy's and scipy's site.cfg`|",
    "created_at": "2016-11-26T09:57:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399028",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:29'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                  |
|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------|
|[9dd50b6](https://git.sagemath.org/sage.git/commit/?id=9dd50b6a72f15f173bac66720fc77e10c4ee087b)|`Only query blas for numpy's and scipy's site.cfg`|



---

archive/issue_comments_399029.json:
```json
{
    "body": "Changing commit from \"07b5632ab1be45ca816b1396105bfc98743460bf\" to \"9dd50b6a72f15f173bac66720fc77e10c4ee087b\"",
    "created_at": "2016-11-26T09:57:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399029",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "07b5632ab1be45ca816b1396105bfc98743460bf" to "9dd50b6a72f15f173bac66720fc77e10c4ee087b"



---

archive/issue_comments_399030.json:
```json
{
    "body": "<a id='comment:30'></a>\nYou'll need to rebuild `numpy` before `scipy` for the fix to work.",
    "created_at": "2016-11-26T09:59:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399030",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:30'></a>
You'll need to rebuild `numpy` before `scipy` for the fix to work.



---

archive/issue_comments_399031.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2016-11-26T09:59:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399031",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_399032.json:
```json
{
    "body": "<a id='comment:31'></a>\nI don't really like that fix. It should not be a problem to have `-lopenblas -lopenblas` in the libraries.",
    "created_at": "2016-11-26T11:00:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399032",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:31'></a>
I don't really like that fix. It should not be a problem to have `-lopenblas -lopenblas` in the libraries.



---

archive/issue_comments_399033.json:
```json
{
    "body": "<a id='comment:32'></a>\nI would have to dig inside `numpy`. That's where the problem is, really. But it could need a mirror fix in `scipy`. I think it would be better to go back to `pkgconfig`and add a patch to make its output more consistent, both internally and with `pkg-config`. That wouldn't do anything for `numpy`/`scipy` setup problem of course.",
    "created_at": "2016-11-26T19:11:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399033",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:32'></a>
I would have to dig inside `numpy`. That's where the problem is, really. But it could need a mirror fix in `scipy`. I think it would be better to go back to `pkgconfig`and add a patch to make its output more consistent, both internally and with `pkg-config`. That wouldn't do anything for `numpy`/`scipy` setup problem of course.



---

archive/issue_comments_399034.json:
```json
{
    "body": "<a id='comment:33'></a>\nPlease bump the numpy version number since the change you make is non-trivial.",
    "created_at": "2016-11-26T20:13:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399034",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:33'></a>
Please bump the numpy version number since the change you make is non-trivial.



---

archive/issue_comments_399035.json:
```json
{
    "body": "<a id='comment:34'></a>\nWas thinking about that. I am not satisfied with the branch as it is anyway. I will hammer some more on `pkgconfig` side with another pull upstream as soon as I can. I'll probably revert my last commit but even so it is probably best if `numpy` is bumped as well.",
    "created_at": "2016-11-26T20:16:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399035",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:34'></a>
Was thinking about that. I am not satisfied with the branch as it is anyway. I will hammer some more on `pkgconfig` side with another pull upstream as soon as I can. I'll probably revert my last commit but even so it is probably best if `numpy` is bumped as well.



---

archive/issue_comments_399036.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2016-11-26T20:34:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399036",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_399037.json:
```json
{
    "body": "<a id='comment:35'></a>\nIf you cannot immediately fix the real issue, it's OK for me to merge this hack as-is.\n\nI'm setting it back to needs_work because you still want to work on it.",
    "created_at": "2016-11-26T20:34:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399037",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:35'></a>
If you cannot immediately fix the real issue, it's OK for me to merge this hack as-is.

I'm setting it back to needs_work because you still want to work on it.



---

archive/issue_comments_399038.json:
```json
{
    "body": "<a id='comment:36'></a>\nI have pushed a few more things upstream and requested a release.  The changes I sent upstream makes my last commit unnecessary, so at the very least I'll retract it. If upstream makes a new release, I'll update the package instead of adding more patches.",
    "created_at": "2016-11-29T23:05:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399038",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:36'></a>
I have pushed a few more things upstream and requested a release.  The changes I sent upstream makes my last commit unnecessary, so at the very least I'll retract it. If upstream makes a new release, I'll update the package instead of adding more patches.



---

archive/issue_comments_399039.json:
```json
{
    "body": "<a id='comment:37'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                           |                            |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|\n|[971c9d4](https://git.sagemath.org/sage.git/commit/?id=971c9d47a1985dcf201d4108f29495fd241a942d)|`upgrade pkgconfig to 1.2.2`|\n|[b03524b](https://git.sagemath.org/sage.git/commit/?id=b03524b653cde297552f7f34c3b7522b4ccd013f)|`migrate to list as used by the new pkgconfig 1.2.2.`|",
    "created_at": "2016-12-03T20:09:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399039",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:37'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                           |                            |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|
|[971c9d4](https://git.sagemath.org/sage.git/commit/?id=971c9d47a1985dcf201d4108f29495fd241a942d)|`upgrade pkgconfig to 1.2.2`|
|[b03524b](https://git.sagemath.org/sage.git/commit/?id=b03524b653cde297552f7f34c3b7522b4ccd013f)|`migrate to list as used by the new pkgconfig 1.2.2.`|



---

archive/issue_comments_399040.json:
```json
{
    "body": "Changing commit from \"9dd50b6a72f15f173bac66720fc77e10c4ee087b\" to \"b03524b653cde297552f7f34c3b7522b4ccd013f\"",
    "created_at": "2016-12-03T20:09:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399040",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "9dd50b6a72f15f173bac66720fc77e10c4ee087b" to "b03524b653cde297552f7f34c3b7522b4ccd013f"



---

archive/issue_comments_399041.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -17,5 +17,8 @@\n \n https://github.com/matze/pkgconfig/pull/11\n \n-This has now been accepted upstream but is not part of a release yet.\n+This has now been accepted upstream and is part of a new release with a few other fixes.\n+New tarball from pypi\n+[https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz](https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz)\n \n+\n``````\n",
    "created_at": "2016-12-03T20:20:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399041",
    "user": "https://github.com/kiwifb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -17,5 +17,8 @@
 
 https://github.com/matze/pkgconfig/pull/11
 
-This has now been accepted upstream but is not part of a release yet.
+This has now been accepted upstream and is part of a new release with a few other fixes.
+New tarball from pypi
+[https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz](https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz)
 
+
``````




---

archive/issue_comments_399042.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2016-12-03T20:20:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399042",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_399043.json:
```json
{
    "body": "<a id='comment:38'></a>\nOK, new release from upstream with all fixes included. Now the output is list, which keep the order, and is also strictly mirroring the output of the equivalent `pkg-config` command. The extra 2 minor releases are for various fixes to the tests that we don't use in sage but are important for me in Gentoo since I do run `nose` tests.",
    "created_at": "2016-12-03T20:20:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399043",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:38'></a>
OK, new release from upstream with all fixes included. Now the output is list, which keep the order, and is also strictly mirroring the output of the equivalent `pkg-config` command. The extra 2 minor releases are for various fixes to the tests that we don't use in sage but are important for me in Gentoo since I do run `nose` tests.



---

archive/issue_comments_399044.json:
```json
{
    "body": "Changing upstream from \"Fixed upstream, but not in a stable release.\" to \"Fixed upstream, in a later stable release.\"",
    "created_at": "2016-12-03T20:20:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399044",
    "user": "https://github.com/kiwifb"
}
```

Changing upstream from "Fixed upstream, but not in a stable release." to "Fixed upstream, in a later stable release."



---

archive/issue_comments_399045.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2016-12-04T13:14:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399045",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_399046.json:
```json
{
    "body": "<a id='comment:39'></a>\n[comment:33]",
    "created_at": "2016-12-04T13:14:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399046",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:39'></a>
[comment:33]



---

archive/issue_comments_399047.json:
```json
{
    "body": "<a id='comment:40'></a>\nAll right, I'll do that ASAP.",
    "created_at": "2016-12-04T18:52:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399047",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:40'></a>
All right, I'll do that ASAP.



---

archive/issue_comments_399048.json:
```json
{
    "body": "Changing commit from \"b03524b653cde297552f7f34c3b7522b4ccd013f\" to \"bd61fc49a33cb0c3c764ba15c0262d344b2e78a1\"",
    "created_at": "2016-12-04T20:08:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399048",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "b03524b653cde297552f7f34c3b7522b4ccd013f" to "bd61fc49a33cb0c3c764ba15c0262d344b2e78a1"



---

archive/issue_comments_399049.json:
```json
{
    "body": "<a id='comment:41'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                               |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|\n|[bd61fc4](https://git.sagemath.org/sage.git/commit/?id=bd61fc49a33cb0c3c764ba15c0262d344b2e78a1)|`bump numpy's version to make sure it rebuilds`|",
    "created_at": "2016-12-04T20:08:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399049",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:41'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                               |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|
|[bd61fc4](https://git.sagemath.org/sage.git/commit/?id=bd61fc49a33cb0c3c764ba15c0262d344b2e78a1)|`bump numpy's version to make sure it rebuilds`|



---

archive/issue_comments_399050.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2016-12-04T20:08:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399050",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_399051.json:
```json
{
    "body": "<a id='comment:42'></a>\nDone.",
    "created_at": "2016-12-04T20:08:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399051",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:42'></a>
Done.



---

archive/issue_comments_399052.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -18,7 +18,5 @@\n https://github.com/matze/pkgconfig/pull/11\n \n This has now been accepted upstream and is part of a new release with a few other fixes.\n-New tarball from pypi\n-[https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz](https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz)\n \n-\n+**Tarball**: [https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz](https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz)\n``````\n",
    "created_at": "2016-12-06T11:02:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399052",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -18,7 +18,5 @@
 https://github.com/matze/pkgconfig/pull/11
 
 This has now been accepted upstream and is part of a new release with a few other fixes.
-New tarball from pypi
-[https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz](https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz)
 
-
+**Tarball**: [https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz](https://pypi.python.org/packages/9d/ba/80910bbed2b4e646a6adab4474d2e506744c260c7002a0e6b41ef8750d8d/pkgconfig-1.2.2.tar.gz)
``````




---

archive/issue_comments_399053.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-12-06T14:56:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399053",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_399054.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2016-12-07T23:29:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399054",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_057369.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-12-07T23:29:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/21625#event-57369"
}
```



---

archive/issue_comments_399055.json:
```json
{
    "body": "Changing branch from \"u/fbissey/pkgconfig\" to \"bd61fc49a33cb0c3c764ba15c0262d344b2e78a1\"",
    "created_at": "2016-12-07T23:29:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399055",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/fbissey/pkgconfig" to "bd61fc49a33cb0c3c764ba15c0262d344b2e78a1"



---

archive/issue_comments_399056.json:
```json
{
    "body": "Changing commit from \"bd61fc49a33cb0c3c764ba15c0262d344b2e78a1\" to \"\"",
    "created_at": "2016-12-11T20:05:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399056",
    "user": "https://github.com/jdemeyer"
}
```

Changing commit from "bd61fc49a33cb0c3c764ba15c0262d344b2e78a1" to ""



---

archive/issue_comments_399057.json:
```json
{
    "body": "<a id='comment:46'></a>\nBreakage: #22047",
    "created_at": "2016-12-11T20:05:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21625",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21625#issuecomment-399057",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:46'></a>
Breakage: #22047
