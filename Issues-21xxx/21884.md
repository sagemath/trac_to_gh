# Issue 21884: giacpy_sage doctest failure and new libsingular cf.type == n_unknown ring test

archive/issues_021647.json:
```json
{
    "assignees": [],
    "body": "Related to #21860 : one failure in ptestlong :\n\n```\ncharpent@asus16-ec:/usr/local/sage-7$ sage -t --long --warn-long 100.3 src/sage/rings/polynomial/multi_polynomial_ideal.py\nRunning doctests with ID 2016-11-17-09-17-43-cce569e2.\nGit branch: t/21860/giacblas\nUsing --optional=database_gap,giac,giacpy_sage,mpir,python2,sage\nDoctesting 1 file.\nsage -t --long --warn-long 100.3 src/sage/rings/polynomial/multi_polynomial_ideal.py\n**********************************************************************\nFile \"src/sage/rings/polynomial/multi_polynomial_ideal.py\", line 3533, in sage.rings.polynomial.multi_polynomial_ideal.NCPolynomialIdeal.groebner_basis\nFailed example:\n    ideal(J.transformed_basis()).change_ring(P).interreduced_basis()  # optional - giacpy_sage\nExpected:\n    [a - 60*c^3 + 158/7*c^2 + 8/7*c - 1, b + 30*c^3 - 79/7*c^2 + 3/7*c, c^4 - 10/21*c^3 + 1/84*c^2 + 1/84*c]\nGot:\n    [7*a - 420*c^3 + 158*c^2 + 8*c - 7, 7*b + 210*c^3 - 79*c^2 + 3*c, 84*c^4 - 40*c^3 + c^2 + c]\n**********************************************************************\n1 item had failures:\n   1 of  67 in sage.rings.polynomial.multi_polynomial_ideal.NCPolynomialIdeal.groebner_basis\n    [723 tests, 1 failure, 9.10 s]\n----------------------------------------------------------------------\nsage -t --long --warn-long 100.3 src/sage/rings/polynomial/multi_polynomial_ideal.py  # 1 doctest failed\n----------------------------------------------------------------------\nTotal time for all tests: 9.2 seconds\n    cpu time: 15.5 seconds\n    cumulative wall time: 9.1 seconds\n```\n\nin fact it is related to the singular update #17254 where all the \n\n```\nif r.ringtype == 0:\n```\nwere replaced by \n\n```\nif r.cf.type == n_unknown:\n```\nbut it seems not equivalent with QQ coefficients.\n\nRemarks: The interred_libsingular function of multi_polynomial_ideal_libsingular.pyx\nends with an explicit:\n\n```\n# divide head by coeffi\n...\n```\nand the groebner_basis function doc have still five examples giving:\n\n```\n[a - 60*c^3 + 158/7*c^2 + 8/7*c - 1, b + 30*c^3 - 79/7*c^2 + 3/7*c, c^4 - 10/21*c^3 + 1/84*c^2 + 1/84*c]\n```\nso what is the best thing for the interreduced_basis command to output?\n\nCC:  parisse @frederichan-IMJPRG jpflori\n\nBranch/Commit: **[fd60c06](https://github.com/sagemath/sagetrac-mirror/commit/fd60c06465050a1117e2ef9d93dd6ab8ed2e2550)**\n\nReviewer: **Emmanuel Charpentier**\n\nAuthor: **Frederic Han**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21884_\n\n",
    "closed_at": "2017-03-19T16:25:40Z",
    "created_at": "2016-11-17T08:31:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "giacpy_sage doctest failure and new libsingular cf.type == n_unknown ring test",
    "type": "issue",
    "updated_at": "2017-03-19T16:25:40Z",
    "url": "https://github.com/sagemath/sage/issues/21884",
    "user": "https://github.com/EmmanuelCharpentier"
}
```
Related to #21860 : one failure in ptestlong :

```
charpent@asus16-ec:/usr/local/sage-7$ sage -t --long --warn-long 100.3 src/sage/rings/polynomial/multi_polynomial_ideal.py
Running doctests with ID 2016-11-17-09-17-43-cce569e2.
Git branch: t/21860/giacblas
Using --optional=database_gap,giac,giacpy_sage,mpir,python2,sage
Doctesting 1 file.
sage -t --long --warn-long 100.3 src/sage/rings/polynomial/multi_polynomial_ideal.py
**********************************************************************
File "src/sage/rings/polynomial/multi_polynomial_ideal.py", line 3533, in sage.rings.polynomial.multi_polynomial_ideal.NCPolynomialIdeal.groebner_basis
Failed example:
    ideal(J.transformed_basis()).change_ring(P).interreduced_basis()  # optional - giacpy_sage
Expected:
    [a - 60*c^3 + 158/7*c^2 + 8/7*c - 1, b + 30*c^3 - 79/7*c^2 + 3/7*c, c^4 - 10/21*c^3 + 1/84*c^2 + 1/84*c]
Got:
    [7*a - 420*c^3 + 158*c^2 + 8*c - 7, 7*b + 210*c^3 - 79*c^2 + 3*c, 84*c^4 - 40*c^3 + c^2 + c]
**********************************************************************
1 item had failures:
   1 of  67 in sage.rings.polynomial.multi_polynomial_ideal.NCPolynomialIdeal.groebner_basis
    [723 tests, 1 failure, 9.10 s]
----------------------------------------------------------------------
sage -t --long --warn-long 100.3 src/sage/rings/polynomial/multi_polynomial_ideal.py  # 1 doctest failed
----------------------------------------------------------------------
Total time for all tests: 9.2 seconds
    cpu time: 15.5 seconds
    cumulative wall time: 9.1 seconds
```

in fact it is related to the singular update #17254 where all the 

```
if r.ringtype == 0:
```
were replaced by 

```
if r.cf.type == n_unknown:
```
but it seems not equivalent with QQ coefficients.

Remarks: The interred_libsingular function of multi_polynomial_ideal_libsingular.pyx
ends with an explicit:

```
# divide head by coeffi
...
```
and the groebner_basis function doc have still five examples giving:

```
[a - 60*c^3 + 158/7*c^2 + 8/7*c - 1, b + 30*c^3 - 79/7*c^2 + 3/7*c, c^4 - 10/21*c^3 + 1/84*c^2 + 1/84*c]
```
so what is the best thing for the interreduced_basis command to output?

CC:  parisse @frederichan-IMJPRG jpflori

Branch/Commit: **[fd60c06](https://github.com/sagemath/sagetrac-mirror/commit/fd60c06465050a1117e2ef9d93dd6ab8ed2e2550)**

Reviewer: **Emmanuel Charpentier**

Author: **Frederic Han**

_Issue created by migration from https://trac.sagemath.org/ticket/21884_





---

archive/issue_events_288924.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2016-11-17T08:31:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288924"
}
```



---

archive/issue_events_288925.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2016-11-17T08:31:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288925"
}
```



---

archive/issue_events_288926.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2016-11-17T08:31:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288926"
}
```



---

archive/issue_events_288927.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2016-11-17T08:31:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288927"
}
```



---

archive/issue_comments_325118.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI don't think that it is related to #21860 (giac update), but it is related to the singular upgrade:\n\nwith sage 7.5beta2 (singular4)\n\n```\nsage: P.<a,b,c> = PolynomialRing(QQ,3, order='lex') \nsage: toto=ideal([7*a - 420*c^3 + 158*c^2 + 8*c - 7, 7*b + 210*c^3 - 79*c^2 + 3*c, 84*c^4 - 40*c^3 + c^2 + c])\n....: \nsage: toto.interreduced_basis()\n[7*a - 420*c^3 + 158*c^2 + 8*c - 7, 7*b + 210*c^3 - 79*c^2 + 3*c, 84*c^4 - 40*c^3 + c^2 + c]\nsage: \n```\nbut with older versions with singular3 it gives\n\n```\n[a - 60*c^3 + 158/7*c^2 + 8/7*c - 1, b + 30*c^3 - 79/7*c^2 + 3/7*c, c^4 - 10/21*c^3 + 1/84*c^2 + 1/84*c]\n```",
    "created_at": "2016-11-17T17:31:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325118",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:1'>Comment 1:</a>
I don't think that it is related to #21860 (giac update), but it is related to the singular upgrade:

with sage 7.5beta2 (singular4)

```
sage: P.<a,b,c> = PolynomialRing(QQ,3, order='lex') 
sage: toto=ideal([7*a - 420*c^3 + 158*c^2 + 8*c - 7, 7*b + 210*c^3 - 79*c^2 + 3*c, 84*c^4 - 40*c^3 + c^2 + c])
....: 
sage: toto.interreduced_basis()
[7*a - 420*c^3 + 158*c^2 + 8*c - 7, 7*b + 210*c^3 - 79*c^2 + 3*c, 84*c^4 - 40*c^3 + c^2 + c]
sage: 
```
but with older versions with singular3 it gives

```
[a - 60*c^3 + 158/7*c^2 + 8/7*c - 1, b + 30*c^3 - 79/7*c^2 + 3/7*c, c^4 - 10/21*c^3 + 1/84*c^2 + 1/84*c]
```



---

archive/issue_comments_325119.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nIn fact the doctest was not using the result computed by giac but was recomputing everything with singular so I have changed this.\n\nI have not yet changed the output doc string because it seems not coherent with the documentation of\ninterreduced_basis:\n\n```\n   If this ideal is spanned by (f_1, ..., f_n) this method returns\n   (g_1, ..., g_s) such that:\n\n   * (f_1,...,f_n) = (g_1,...,g_s)\n\n   * LT(g_i) != LT(g_j) for all i != j\n\n   * LT(g_i) does not divide m for all monomials m of\n\n        {g_1,...,g_{i-1},g_{i+1},...,g_s}\n\n   * LC(g_i) == 1 for all i if the coefficient ring is a field.\n\n```\nbut\n\n```\nsage: P.<a,b,c> = PolynomialRing(QQ,3, order='lex')\nsage: toto=ideal([7*a - 420*c^3 + 158*c^2 + 8*c - 7, 7*b + 210*c^3 - 79*c^2 + 3*c, 84*c^4 - 40*c^3 + c^2 + c])\n....: \nsage: g=toto.interreduced_basis()\nsage: g[0].lc()\n7\nsage: toto.base_ring()\nRational Field\n```\n\n---\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b613d7d26d8434c16c6425caa6ff20cedb249d69\">b613d7d</a></td><td><code>Merge branch 'public/giacpyGB' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eb69ec52218caa5a7bb52e048ed2ea51e321482f\">eb69ec5</a></td><td><code>Merge branch 'master' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5e4cb9970ccb17c6a8d826451637d7502e0e0e0c\">5e4cb99</a></td><td><code>Merge branch 'public/giacpyGB' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d17e34825d159926bcd1c5e5b8cd3287ca53fabb\">d17e348</a></td><td><code>Merge branch 'develop' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ff5a75359e87a04537c1fcfb2e5c396d87703e57\">ff5a753</a></td><td><code>Merge branch 'develop' of git://github.com/sagemath/sage into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3ddf1ca3baeb8928ba87075271ef61b242c6a336\">3ddf1ca</a></td><td><code>Merge branch 'develop' of git://github.com/sagemath/sage into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b2d156f510476bdcff8ba7e6368aaf3ecbbf8401\">b2d156f</a></td><td><code>Merge branch 'develop' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/428ab264a5471206be223990f0af65ed37b740ea\">428ab26</a></td><td><code>Merge branch 'develop' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0b6ebcd75e538dde7a1e3900510c55c396fe10c4\">0b6ebcd</a></td><td><code>Merge branch 'develop' of git://github.com/sagemath/sage into develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7accb23f1cdc56138d12e8145fc4d378f0fdf077\">7accb23</a></td><td><code>The doctest was not using the result computed by giacpy</code></td></tr></table>\n",
    "created_at": "2016-11-17T18:23:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325119",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:2'>Comment 2:</a>
In fact the doctest was not using the result computed by giac but was recomputing everything with singular so I have changed this.

I have not yet changed the output doc string because it seems not coherent with the documentation of
interreduced_basis:

```
   If this ideal is spanned by (f_1, ..., f_n) this method returns
   (g_1, ..., g_s) such that:

   * (f_1,...,f_n) = (g_1,...,g_s)

   * LT(g_i) != LT(g_j) for all i != j

   * LT(g_i) does not divide m for all monomials m of

        {g_1,...,g_{i-1},g_{i+1},...,g_s}

   * LC(g_i) == 1 for all i if the coefficient ring is a field.

```
but

```
sage: P.<a,b,c> = PolynomialRing(QQ,3, order='lex')
sage: toto=ideal([7*a - 420*c^3 + 158*c^2 + 8*c - 7, 7*b + 210*c^3 - 79*c^2 + 3*c, 84*c^4 - 40*c^3 + c^2 + c])
....: 
sage: g=toto.interreduced_basis()
sage: g[0].lc()
7
sage: toto.base_ring()
Rational Field
```

---
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b613d7d26d8434c16c6425caa6ff20cedb249d69">b613d7d</a></td><td><code>Merge branch 'public/giacpyGB' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eb69ec52218caa5a7bb52e048ed2ea51e321482f">eb69ec5</a></td><td><code>Merge branch 'master' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5e4cb9970ccb17c6a8d826451637d7502e0e0e0c">5e4cb99</a></td><td><code>Merge branch 'public/giacpyGB' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d17e34825d159926bcd1c5e5b8cd3287ca53fabb">d17e348</a></td><td><code>Merge branch 'develop' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ff5a75359e87a04537c1fcfb2e5c396d87703e57">ff5a753</a></td><td><code>Merge branch 'develop' of git://github.com/sagemath/sage into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3ddf1ca3baeb8928ba87075271ef61b242c6a336">3ddf1ca</a></td><td><code>Merge branch 'develop' of git://github.com/sagemath/sage into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b2d156f510476bdcff8ba7e6368aaf3ecbbf8401">b2d156f</a></td><td><code>Merge branch 'develop' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/428ab264a5471206be223990f0af65ed37b740ea">428ab26</a></td><td><code>Merge branch 'develop' of git://trac.sagemath.org/sage into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0b6ebcd75e538dde7a1e3900510c55c396fe10c4">0b6ebcd</a></td><td><code>Merge branch 'develop' of git://github.com/sagemath/sage into develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7accb23f1cdc56138d12e8145fc4d378f0fdf077">7accb23</a></td><td><code>The doctest was not using the result computed by giacpy</code></td></tr></table>




---

archive/issue_comments_325120.json:
```json
{
    "body": "Changed dependencies from **#21860** to none",
    "created_at": "2016-11-17T18:23:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325120",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

Changed dependencies from **#21860** to none



---

archive/issue_comments_325121.json:
```json
{
    "body": "Commit: **[7accb23](https://github.com/sagemath/sagetrac-mirror/commit/7accb23f1cdc56138d12e8145fc4d378f0fdf077)**",
    "created_at": "2016-11-17T18:23:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325121",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

Commit: **[7accb23](https://github.com/sagemath/sagetrac-mirror/commit/7accb23f1cdc56138d12e8145fc4d378f0fdf077)**



---

archive/issue_comments_325122.json:
```json
{
    "body": "Branch: **[public/giacpy21884](https://github.com/sagemath/sagetrac-mirror/tree/public/giacpy21884)**",
    "created_at": "2016-11-17T18:23:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325122",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

Branch: **[public/giacpy21884](https://github.com/sagemath/sagetrac-mirror/tree/public/giacpy21884)**



---

archive/issue_comments_325123.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -26,3 +26,29 @@\n     cpu time: 15.5 seconds\n     cumulative wall time: 9.1 seconds\n ```\n+\n+in fact it is related to the singular update #17254 where all the \n+\n+```\n+if r.ringtype == 0:\n+```\n+were replaced by \n+\n+```\n+if r.cf.type == n_unknown:\n+```\n+but it seems not equivalent with QQ coefficients.\n+\n+Remarks: The interred_libsingular function of multi_polynomial_ideal_libsingular.pyx\n+ends with an explicit:\n+\n+```\n+# divide head by coeffi\n+...\n+```\n+and the groebner_basis function doc have still five examples giving:\n+\n+```\n+[a - 60*c^3 + 158/7*c^2 + 8/7*c - 1, b + 30*c^3 - 79/7*c^2 + 3/7*c, c^4 - 10/21*c^3 + 1/84*c^2 + 1/84*c]\n+```\n+so what is the best thing for the interreduced_basis command to output?\n``````\n",
    "created_at": "2016-11-19T08:37:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325123",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -26,3 +26,29 @@
     cpu time: 15.5 seconds
     cumulative wall time: 9.1 seconds
 ```
+
+in fact it is related to the singular update #17254 where all the 
+
+```
+if r.ringtype == 0:
+```
+were replaced by 
+
+```
+if r.cf.type == n_unknown:
+```
+but it seems not equivalent with QQ coefficients.
+
+Remarks: The interred_libsingular function of multi_polynomial_ideal_libsingular.pyx
+ends with an explicit:
+
+```
+# divide head by coeffi
+...
+```
+and the groebner_basis function doc have still five examples giving:
+
+```
+[a - 60*c^3 + 158/7*c^2 + 8/7*c - 1, b + 30*c^3 - 79/7*c^2 + 3/7*c, c^4 - 10/21*c^3 + 1/84*c^2 + 1/84*c]
+```
+so what is the best thing for the interreduced_basis command to output?
``````




---

archive/issue_comments_325124.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\n15 commits to change just one line of code? Please squash this to one commit :-)",
    "created_at": "2016-11-19T08:46:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325124",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
15 commits to change just one line of code? Please squash this to one commit :-)



---

archive/issue_events_288928.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-19T08:49:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288928"
}
```



---

archive/issue_events_288929.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-19T08:49:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288929"
}
```



---

archive/issue_comments_325125.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nAnd change the title to something which refers to Singular.",
    "created_at": "2016-11-19T08:49:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325125",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'>Comment 5:</a>
And change the title to something which refers to Singular.



---

archive/issue_comments_325126.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nsorry for the messy branch, here is a new one\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/226744dcf00938804e5de4fa692f9ea6c43db39f\">226744d</a></td><td><code>The result computed by giacpy was not used in the doc and confused the reader</code></td></tr></table>\n",
    "created_at": "2016-11-19T11:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325126",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:6'>Comment 6:</a>
sorry for the messy branch, here is a new one

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/226744dcf00938804e5de4fa692f9ea6c43db39f">226744d</a></td><td><code>The result computed by giacpy was not used in the doc and confused the reader</code></td></tr></table>




---

archive/issue_comments_325127.json:
```json
{
    "body": "Changed commit from **[7accb23](https://github.com/sagemath/sagetrac-mirror/commit/7accb23f1cdc56138d12e8145fc4d378f0fdf077)** to **[226744d](https://github.com/sagemath/sagetrac-mirror/commit/226744dcf00938804e5de4fa692f9ea6c43db39f)**",
    "created_at": "2016-11-19T11:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325127",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

Changed commit from **[7accb23](https://github.com/sagemath/sagetrac-mirror/commit/7accb23f1cdc56138d12e8145fc4d378f0fdf077)** to **[226744d](https://github.com/sagemath/sagetrac-mirror/commit/226744dcf00938804e5de4fa692f9ea6c43db39f)**



---

archive/issue_events_288930.json:
```json
{
    "actor": "https://github.com/frederichan-IMJPRG",
    "created_at": "2016-11-19T11:19:36Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "title_is": "giacpy_sage doctest failure and new libsingular cf.type == 0 ring test",
    "title_was": "giacpy_sage has a doctest failure",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288930"
}
```



---

archive/issue_comments_325128.json:
```json
{
    "body": "Changed branch from **[public/giacpy21884](https://github.com/sagemath/sagetrac-mirror/tree/public/giacpy21884)** to **[public/trac21884](https://github.com/sagemath/sagetrac-mirror/tree/public/trac21884)**",
    "created_at": "2016-11-19T11:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325128",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

Changed branch from **[public/giacpy21884](https://github.com/sagemath/sagetrac-mirror/tree/public/giacpy21884)** to **[public/trac21884](https://github.com/sagemath/sagetrac-mirror/tree/public/trac21884)**



---

archive/issue_comments_325129.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nhow about changing the doc of `interreduced_basis` where it says that LCs will be 1 if working over a field, i.e.\n\n```\n* LC(g_i) == 1 for all i if the coefficient ring is a field.\n```\nis no longer true.",
    "created_at": "2016-11-19T18:28:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325129",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:7'>Comment 7:</a>
how about changing the doc of `interreduced_basis` where it says that LCs will be 1 if working over a field, i.e.

```
* LC(g_i) == 1 for all i if the coefficient ring is a field.
```
is no longer true.



---

archive/issue_events_288931.json:
```json
{
    "actor": "https://github.com/frederichan-IMJPRG",
    "created_at": "2016-11-20T10:05:54Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "title_is": "giacpy_sage doctest failure and new libsingular cf.type == n_unknown ring test",
    "title_was": "giacpy_sage doctest failure and new libsingular cf.type == 0 ring test",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288931"
}
```



---

archive/issue_comments_325130.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBefore doing just that, my question is also about how QQ was detected in the old code and does the new condition detect any ring, because if I grep the old test I already have:\n\n```\nmacbookito(fred)$ grep -r \"ringtype == 0\" src/sage\nsrc/sage/rings/polynomial/multi_polynomial_libsingular.pyx:            if r.ringtype == 0 or r.cf.nDivBy(p_GetCoeff(f._poly, r), p_GetCoeff(g._poly, r)):\nsrc/sage/rings/polynomial/multi_polynomial_ideal_libsingular.pyx:    if r.ringtype == 0:\nsrc/sage/rings/polynomial/plural.pyx:            if r.ringtype == 0 or r.cf.nDivBy(p_GetCoeff(f._poly, r), p_GetCoeff(g._poly, r)):\nsrc/sage/libs/singular/singular.pyx:        if _ring.ringtype == 0:\nsrc/sage/libs/singular/singular.pyx:        if _ring.ringtype == 0:\nmacbookito(fred)$ grep -r \"ringtype != 0\" src/sage\nsrc/sage/rings/polynomial/multi_polynomial_libsingular.pyx:        if r.ringtype != 0:\nsrc/sage/rings/polynomial/multi_polynomial_libsingular.pyx:        if _ring.ringtype != 0:\nsrc/sage/rings/polynomial/multi_polynomial_libsingular.pyx:        if _ring.ringtype != 0:\nsrc/sage/libs/singular/ring.pyx:    if ringtype != 0:\n```",
    "created_at": "2016-11-20T10:05:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325130",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:8'>Comment 8:</a>
Before doing just that, my question is also about how QQ was detected in the old code and does the new condition detect any ring, because if I grep the old test I already have:

```
macbookito(fred)$ grep -r "ringtype == 0" src/sage
src/sage/rings/polynomial/multi_polynomial_libsingular.pyx:            if r.ringtype == 0 or r.cf.nDivBy(p_GetCoeff(f._poly, r), p_GetCoeff(g._poly, r)):
src/sage/rings/polynomial/multi_polynomial_ideal_libsingular.pyx:    if r.ringtype == 0:
src/sage/rings/polynomial/plural.pyx:            if r.ringtype == 0 or r.cf.nDivBy(p_GetCoeff(f._poly, r), p_GetCoeff(g._poly, r)):
src/sage/libs/singular/singular.pyx:        if _ring.ringtype == 0:
src/sage/libs/singular/singular.pyx:        if _ring.ringtype == 0:
macbookito(fred)$ grep -r "ringtype != 0" src/sage
src/sage/rings/polynomial/multi_polynomial_libsingular.pyx:        if r.ringtype != 0:
src/sage/rings/polynomial/multi_polynomial_libsingular.pyx:        if _ring.ringtype != 0:
src/sage/rings/polynomial/multi_polynomial_libsingular.pyx:        if _ring.ringtype != 0:
src/sage/libs/singular/ring.pyx:    if ringtype != 0:
```



---

archive/issue_comments_325131.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nSo if I am right it seems that the old test ringtype != 0 was used as a trick to detect if the  ring of coefficient was not a field, and seems equivalent to \n\n   cf.type ==  one of n_Z, n_Zm, n_Zmn, n_Z2n\n\nin src/sage/rings/polynomial/multi_polynomial_libsingular.pyx\nin the gcd and lcm a test has been added so the code is equivalent to the old one.\n            if r.cf.type == n_Znm or r.cf.type == n_Zn or r.cf.type == n_Z2m :\n                raise NotImplementedError(\"Division of multivariate polynomials over non fields by n\n\nin interreduced the normalisation of the head coefficient may be never executed and is not equivalent to the old code. So I suggest 2 modification.\neither:\n   1)  change the doc a dima suggest to do the same as singular and delete the normalisation code\nor\n   2)  replace the cf.type == n_unknown by  \n            if r.cf.type != n_Znm and r.cf.type != n_Zn and r.cf.type != n_Z2m \n       and keep the doc as is\n\nfor plural.pyx it should be OK because of the or\n\nBut in src/sage/libs/singular/singular.pyx the 2 substitutions gives  strange code:\n\n```\n\n    elif isinstance(base, IntegerModRing_generic):\n        if _ring.cf.type == n_unknown:\n            return base(_ring.cf.cfInt(n, _ring.cf))\n        return si2sa_ZZmod(n, _ring, base)\n\n```\n\n```\n    elif isinstance(elem._parent, IntegerModRing_generic):\n        if _ring.cf.type == n_unknown:\n            return n_Init(int(elem),_ring)\n        return sa2si_ZZmod(elem, _ring)\n\n``` \nfor instance n_Zp had ringtype == 0",
    "created_at": "2016-11-23T11:38:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325131",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:9'>Comment 9:</a>
So if I am right it seems that the old test ringtype != 0 was used as a trick to detect if the  ring of coefficient was not a field, and seems equivalent to 

   cf.type ==  one of n_Z, n_Zm, n_Zmn, n_Z2n

in src/sage/rings/polynomial/multi_polynomial_libsingular.pyx
in the gcd and lcm a test has been added so the code is equivalent to the old one.
            if r.cf.type == n_Znm or r.cf.type == n_Zn or r.cf.type == n_Z2m :
                raise NotImplementedError("Division of multivariate polynomials over non fields by n

in interreduced the normalisation of the head coefficient may be never executed and is not equivalent to the old code. So I suggest 2 modification.
either:
   1)  change the doc a dima suggest to do the same as singular and delete the normalisation code
or
   2)  replace the cf.type == n_unknown by  
            if r.cf.type != n_Znm and r.cf.type != n_Zn and r.cf.type != n_Z2m 
       and keep the doc as is

for plural.pyx it should be OK because of the or

But in src/sage/libs/singular/singular.pyx the 2 substitutions gives  strange code:

```

    elif isinstance(base, IntegerModRing_generic):
        if _ring.cf.type == n_unknown:
            return base(_ring.cf.cfInt(n, _ring.cf))
        return si2sa_ZZmod(n, _ring, base)

```

```
    elif isinstance(elem._parent, IntegerModRing_generic):
        if _ring.cf.type == n_unknown:
            return n_Init(int(elem),_ring)
        return sa2si_ZZmod(elem, _ring)

``` 
for instance n_Zp had ringtype == 0



---

archive/issue_comments_325132.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bde7be034769b603ffadbe827d8db2b801847a4b\">bde7be0</a></td><td><code>change test on cf.type in interred</code></td></tr></table>\n",
    "created_at": "2016-11-24T11:49:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325132",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:10'>Comment 10:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bde7be034769b603ffadbe827d8db2b801847a4b">bde7be0</a></td><td><code>change test on cf.type in interred</code></td></tr></table>




---

archive/issue_comments_325133.json:
```json
{
    "body": "Changed commit from **[226744d](https://github.com/sagemath/sagetrac-mirror/commit/226744dcf00938804e5de4fa692f9ea6c43db39f)** to **[bde7be0](https://github.com/sagemath/sagetrac-mirror/commit/bde7be034769b603ffadbe827d8db2b801847a4b)**",
    "created_at": "2016-11-24T11:49:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325133",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[226744d](https://github.com/sagemath/sagetrac-mirror/commit/226744dcf00938804e5de4fa692f9ea6c43db39f)** to **[bde7be0](https://github.com/sagemath/sagetrac-mirror/commit/bde7be034769b603ffadbe827d8db2b801847a4b)**



---

archive/issue_comments_325134.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nNB: I chose solution 2 because the reduced command of  multi_polynomial_sequence.py also talk of normalized polynomials and use the libgiac interred:\n\n```\n        if isinstance(R,MPolynomialRing_libsingular):\n            return PolynomialSequence(R, interred_libsingular(self), immutable=True)\n```",
    "created_at": "2016-11-24T11:54:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325134",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:11'>Comment 11:</a>
NB: I chose solution 2 because the reduced command of  multi_polynomial_sequence.py also talk of normalized polynomials and use the libgiac interred:

```
        if isinstance(R,MPolynomialRing_libsingular):
            return PolynomialSequence(R, interred_libsingular(self), immutable=True)
```



---

archive/issue_comments_325135.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nNB: I am not confortable enough with sage and singular to modify the == n_unkown in:\nsrc/sage/libs/singular/singular.pyx\n\nDoes anyone plan to change it or can I switch to need review?",
    "created_at": "2016-11-24T12:03:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325135",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:12'>Comment 12:</a>
NB: I am not confortable enough with sage and singular to modify the == n_unkown in:
src/sage/libs/singular/singular.pyx

Does anyone plan to change it or can I switch to need review?



---

archive/issue_comments_325136.json:
```json
{
    "body": "Author: **Frederic Han**",
    "created_at": "2016-11-24T12:03:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325136",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

Author: **Frederic Han**



---

archive/issue_events_288932.json:
```json
{
    "actor": "https://github.com/frederichan-IMJPRG",
    "created_at": "2016-11-25T12:34:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288932"
}
```



---

archive/issue_events_288933.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-10T21:54:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288933"
}
```



---

archive/issue_events_288934.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-10T21:54:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288934"
}
```



---

archive/issue_events_288935.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-10T21:54:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288935"
}
```



---

archive/issue_events_288936.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-10T21:54:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288936"
}
```



---

archive/issue_comments_325137.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/90cc4887b26256473c2c9d90202131c49a69a29c\">90cc488</a></td><td><code>The result computed by giacpy was not used in the doc and confused the reader</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fd60c06465050a1117e2ef9d93dd6ab8ed2e2550\">fd60c06</a></td><td><code>change test on cf.type in interred</code></td></tr></table>\n",
    "created_at": "2017-03-11T09:25:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325137",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:15'>Comment 15:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/90cc4887b26256473c2c9d90202131c49a69a29c">90cc488</a></td><td><code>The result computed by giacpy was not used in the doc and confused the reader</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fd60c06465050a1117e2ef9d93dd6ab8ed2e2550">fd60c06</a></td><td><code>change test on cf.type in interred</code></td></tr></table>




---

archive/issue_comments_325138.json:
```json
{
    "body": "Changed commit from **[bde7be0](https://github.com/sagemath/sagetrac-mirror/commit/bde7be034769b603ffadbe827d8db2b801847a4b)** to **[fd60c06](https://github.com/sagemath/sagetrac-mirror/commit/fd60c06465050a1117e2ef9d93dd6ab8ed2e2550)**",
    "created_at": "2017-03-11T09:25:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325138",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[bde7be0](https://github.com/sagemath/sagetrac-mirror/commit/bde7be034769b603ffadbe827d8db2b801847a4b)** to **[fd60c06](https://github.com/sagemath/sagetrac-mirror/commit/fd60c06465050a1117e2ef9d93dd6ab8ed2e2550)**



---

archive/issue_comments_325139.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nRebased to 7.6.beta6",
    "created_at": "2017-03-11T09:26:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325139",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:16'>Comment 16:</a>
Rebased to 7.6.beta6



---

archive/issue_comments_325140.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nIs it good to go ?",
    "created_at": "2017-03-17T14:22:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325140",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<a id='comment:17'>Comment 17:</a>
Is it good to go ?



---

archive/issue_comments_325141.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nI have not run the test suite for while but we should try if other doctests have been added in between",
    "created_at": "2017-03-17T14:49:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325141",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:18'>Comment 18:</a>
I have not run the test suite for while but we should try if other doctests have been added in between



---

archive/issue_events_288937.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2017-03-18T11:00:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288937"
}
```



---

archive/issue_events_288938.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2017-03-18T11:00:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288938"
}
```



---

archive/issue_comments_325142.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nIn 7.6.rc1 + #20523 + #21884 (present ticket) :\n* `giacpy_sage` passes its own test suite (237 tests pased, 0 failed, 5 items with no test)\n* Sage pass `ptestlong` with one failure :\n\n```\n----------------------------------------------------------------------\nsage -t --long src/sage/homology/simplicial_complex.py  # 1 doctest failed\n----------------------------------------------------------------------\n```\nwhich is transient (i. e. pass without failure when ran standalone).\n\n==> `positive review`.",
    "created_at": "2017-03-18T11:00:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325142",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:19'>Comment 19:</a>
In 7.6.rc1 + #20523 + #21884 (present ticket) :
* `giacpy_sage` passes its own test suite (237 tests pased, 0 failed, 5 items with no test)
* Sage pass `ptestlong` with one failure :

```
----------------------------------------------------------------------
sage -t --long src/sage/homology/simplicial_complex.py  # 1 doctest failed
----------------------------------------------------------------------
```
which is transient (i. e. pass without failure when ran standalone).

==> `positive review`.



---

archive/issue_comments_325143.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nForgot to sign the review...",
    "created_at": "2017-03-18T11:03:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325143",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:20'>Comment 20:</a>
Forgot to sign the review...



---

archive/issue_comments_325144.json:
```json
{
    "body": "Reviewer: **Emmanuel Charpentier**",
    "created_at": "2017-03-18T11:03:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325144",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

Reviewer: **Emmanuel Charpentier**



---

archive/issue_comments_325145.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nNote : I do not understand the patchbot failure. Someone with a better understanding of the patchbot system should investigate the issue...",
    "created_at": "2017-03-18T11:10:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325145",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:21'>Comment 21:</a>
Note : I do not understand the patchbot failure. Someone with a better understanding of the patchbot system should investigate the issue...



---

archive/issue_events_288939.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-03-19T16:25:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288939"
}
```



---

archive/issue_events_288940.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b9bd30557e5000abcb3d07886009c61bf00d1441",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-03-19T16:25:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21884#event-288940"
}
```



---

archive/issue_comments_325146.json:
```json
{
    "body": "Changed branch from **[public/trac21884](https://github.com/sagemath/sagetrac-mirror/tree/public/trac21884)** to **[fd60c06](https://github.com/sagemath/sagetrac-mirror/commit/fd60c06465050a1117e2ef9d93dd6ab8ed2e2550)**",
    "created_at": "2017-03-19T16:25:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21884",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21884#issuecomment-325146",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/trac21884](https://github.com/sagemath/sagetrac-mirror/tree/public/trac21884)** to **[fd60c06](https://github.com/sagemath/sagetrac-mirror/commit/fd60c06465050a1117e2ef9d93dd6ab8ed2e2550)**
