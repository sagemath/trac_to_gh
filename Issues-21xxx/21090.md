# Issue 21090: Fix implicit relative cimports in Cython .pxd/.pxi files

archive/issues_020853.json:
```json
{
    "assignees": [],
    "body": "Change\n\n```\nfrom element cimport Element\n```\nto\n\n```\nfrom .element cimport Element\n```\nand similar in Cython `.pxd` and `.pxi` files.\n\nI also moved some `cimport`s from `.pxd` files to `.pyx` files if they are only used in the `.pyx` file.\n\nCC:  @fchapoton @tscrim @jm58660\n\nBranch/Commit: **[ccfd297](https://github.com/sagemath/sagetrac-mirror/commit/ccfd29740fb9193efe2f71aaccf313c2dd336ab3)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21090_\n\n",
    "closed_at": "2016-07-27T20:24:31Z",
    "created_at": "2016-07-25T10:15:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix implicit relative cimports in Cython .pxd/.pxi files",
    "type": "issue",
    "updated_at": "2016-07-27T20:24:31Z",
    "url": "https://github.com/sagemath/sage/issues/21090",
    "user": "https://github.com/jdemeyer"
}
```
Change

```
from element cimport Element
```
to

```
from .element cimport Element
```
and similar in Cython `.pxd` and `.pxi` files.

I also moved some `cimport`s from `.pxd` files to `.pyx` files if they are only used in the `.pyx` file.

CC:  @fchapoton @tscrim @jm58660

Branch/Commit: **[ccfd297](https://github.com/sagemath/sagetrac-mirror/commit/ccfd29740fb9193efe2f71aaccf313c2dd336ab3)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/21090_





---

archive/issue_events_268633.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-07-25T10:15:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268633"
}
```



---

archive/issue_events_268634.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-07-25T10:15:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268634"
}
```



---

archive/issue_events_268635.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-07-25T10:15:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268635"
}
```



---

archive/issue_events_268636.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-07-25T10:15:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268636"
}
```



---

archive/issue_events_268637.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-07-25T13:02:35Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "title_is": "Fix implicit relative cimports in .pxd/.pxi files",
    "title_was": "Fix implicit relative \"from MODULE cimport SYMBOL\" in .pxd/.pxi files",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268637"
}
```



---

archive/issue_comments_309747.json:
```json
{
    "body": "Branch: **[u/jdemeyer/fix_implicit_relative__from_module_cimport_symbol__in__pxd__pxi_files](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_implicit_relative__from_module_cimport_symbol__in__pxd__pxi_files)**",
    "created_at": "2016-07-25T13:51:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21090#issuecomment-309747",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/fix_implicit_relative__from_module_cimport_symbol__in__pxd__pxi_files](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_implicit_relative__from_module_cimport_symbol__in__pxd__pxi_files)**



---

archive/issue_comments_309748.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ccfd29740fb9193efe2f71aaccf313c2dd336ab3\">ccfd297</a></td><td><code>Fix implicit relative cimports in .pxd/.pxi files</code></td></tr></table>\n",
    "created_at": "2016-07-25T15:59:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21090#issuecomment-309748",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ccfd29740fb9193efe2f71aaccf313c2dd336ab3">ccfd297</a></td><td><code>Fix implicit relative cimports in .pxd/.pxi files</code></td></tr></table>




---

archive/issue_comments_309749.json:
```json
{
    "body": "Commit: **[ccfd297](https://github.com/sagemath/sagetrac-mirror/commit/ccfd29740fb9193efe2f71aaccf313c2dd336ab3)**",
    "created_at": "2016-07-25T15:59:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21090#issuecomment-309749",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[ccfd297](https://github.com/sagemath/sagetrac-mirror/commit/ccfd29740fb9193efe2f71aaccf313c2dd336ab3)**



---

archive/issue_events_268638.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-07-25T15:59:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268638"
}
```



---

archive/issue_comments_309750.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,3 +9,5 @@\n from .element cimport Element\n ```\n and similar in Cython `.pxd` and `.pxi` files.\n+\n+I also moved some `cimport`s from `.pxd` files to `.pyx` files if they are only used in the `.pyx` file.\n``````\n",
    "created_at": "2016-07-25T16:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21090#issuecomment-309750",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,3 +9,5 @@
 from .element cimport Element
 ```
 and similar in Cython `.pxd` and `.pxi` files.
+
+I also moved some `cimport`s from `.pxd` files to `.pyx` files if they are only used in the `.pyx` file.
``````




---

archive/issue_events_268639.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-07-26T07:51:50Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "title_is": "Fix implicit relative cimports in Cython .pxd/.pxi files",
    "title_was": "Fix implicit relative cimports in .pxd/.pxi files",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268639"
}
```



---

archive/issue_comments_309751.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nIt would be nice to have a fast review since this kind of ticket can bit-rot easily.",
    "created_at": "2016-07-26T07:53:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21090#issuecomment-309751",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
It would be nice to have a fast review since this kind of ticket can bit-rot easily.



---

archive/issue_comments_309752.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-07-26T17:24:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21090#issuecomment-309752",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_268640.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-07-26T17:24:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268640"
}
```



---

archive/issue_events_268641.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-07-26T17:24:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268641"
}
```



---

archive/issue_events_268642.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-07-27T20:24:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268642"
}
```



---

archive/issue_events_268643.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bea740cc44bce8f1e9c0782edc44d61d2ed34af0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-07-27T20:24:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21090#event-268643"
}
```



---

archive/issue_comments_309753.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/fix_implicit_relative__from_module_cimport_symbol__in__pxd__pxi_files](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_implicit_relative__from_module_cimport_symbol__in__pxd__pxi_files)** to **[ccfd297](https://github.com/sagemath/sagetrac-mirror/commit/ccfd29740fb9193efe2f71aaccf313c2dd336ab3)**",
    "created_at": "2016-07-27T20:24:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21090",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21090#issuecomment-309753",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/fix_implicit_relative__from_module_cimport_symbol__in__pxd__pxi_files](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_implicit_relative__from_module_cimport_symbol__in__pxd__pxi_files)** to **[ccfd297](https://github.com/sagemath/sagetrac-mirror/commit/ccfd29740fb9193efe2f71aaccf313c2dd336ab3)**
