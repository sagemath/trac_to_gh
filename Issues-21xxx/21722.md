# Issue 21722: Fix build of gap3

archive/issues_021485.json:
```json
{
    "assignees": [],
    "body": "Currently, `gap3` does not build. This branch fixes it.\n\nIt also uses the standard patching procedure and it no longer copies the content of `build/pkgs/gap3` to the install directory.\n\nCC:  @dimpase stumpc5 @tscrim\n\nBranch/Commit: **[6b07191](https://github.com/sagemath/sagetrac-mirror/commit/6b071914705c4d245c981969338a0c6e8f92a574)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21722_\n\n",
    "closed_at": "2016-10-29T14:27:22Z",
    "created_at": "2016-10-18T14:46:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix build of gap3",
    "type": "issue",
    "updated_at": "2016-10-29T14:27:22Z",
    "url": "https://github.com/sagemath/sage/issues/21722",
    "user": "https://github.com/jdemeyer"
}
```
Currently, `gap3` does not build. This branch fixes it.

It also uses the standard patching procedure and it no longer copies the content of `build/pkgs/gap3` to the install directory.

CC:  @dimpase stumpc5 @tscrim

Branch/Commit: **[6b07191](https://github.com/sagemath/sagetrac-mirror/commit/6b071914705c4d245c981969338a0c6e8f92a574)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/21722_





---

archive/issue_events_286879.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-10-18T14:46:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21722#event-286879"
}
```



---

archive/issue_events_286880.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-10-18T14:46:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "component: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21722#event-286880"
}
```



---

archive/issue_events_286881.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-10-18T14:46:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21722#event-286881"
}
```



---

archive/issue_events_286882.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-10-18T14:46:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21722#event-286882"
}
```



---

archive/issue_comments_321950.json:
```json
{
    "body": "Branch: **[u/jdemeyer/fix_build_of_gap3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_build_of_gap3)**",
    "created_at": "2016-10-18T14:50:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21722#issuecomment-321950",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/fix_build_of_gap3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_build_of_gap3)**



---

archive/issue_comments_321951.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6b071914705c4d245c981969338a0c6e8f92a574\">6b07191</a></td><td><code>Fix build of gap3; standardize patches</code></td></tr></table>\n",
    "created_at": "2016-10-18T14:51:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21722#issuecomment-321951",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6b071914705c4d245c981969338a0c6e8f92a574">6b07191</a></td><td><code>Fix build of gap3; standardize patches</code></td></tr></table>




---

archive/issue_comments_321952.json:
```json
{
    "body": "Commit: **[6b07191](https://github.com/sagemath/sagetrac-mirror/commit/6b071914705c4d245c981969338a0c6e8f92a574)**",
    "created_at": "2016-10-18T14:51:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21722#issuecomment-321952",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[6b07191](https://github.com/sagemath/sagetrac-mirror/commit/6b071914705c4d245c981969338a0c6e8f92a574)**



---

archive/issue_events_286883.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-10-18T14:51:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21722#event-286883"
}
```



---

archive/issue_comments_321953.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks for fixing this! Could you tell why it stopped working? Is it a change how to patch packages?",
    "created_at": "2016-10-18T19:29:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21722#issuecomment-321953",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks for fixing this! Could you tell why it stopped working? Is it a change how to patch packages?



---

archive/issue_comments_321954.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI think it has to do with some of the recent changes to the build system that Jeroen, Erik, and Matthias have been doing. Anyways, positive review.",
    "created_at": "2016-10-18T19:49:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21722#issuecomment-321954",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
I think it has to do with some of the recent changes to the build system that Jeroen, Erik, and Matthias have been doing. Anyways, positive review.



---

archive/issue_events_286884.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-18T19:49:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21722#event-286884"
}
```



---

archive/issue_events_286885.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-10-18T19:49:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21722#event-286885"
}
```



---

archive/issue_comments_321955.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-10-18T19:49:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21722#issuecomment-321955",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_321956.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/fix_build_of_gap3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_build_of_gap3)** to **[6b07191](https://github.com/sagemath/sagetrac-mirror/commit/6b071914705c4d245c981969338a0c6e8f92a574)**",
    "created_at": "2016-10-29T14:27:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21722#issuecomment-321956",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/fix_build_of_gap3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_build_of_gap3)** to **[6b07191](https://github.com/sagemath/sagetrac-mirror/commit/6b071914705c4d245c981969338a0c6e8f92a574)**



---

archive/issue_events_286886.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-10-29T14:27:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21722#event-286886"
}
```



---

archive/issue_events_286887.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bcef947a6189f29cdeca54b5463c6ee401f059af",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-10-29T14:27:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21722",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21722#event-286887"
}
```
