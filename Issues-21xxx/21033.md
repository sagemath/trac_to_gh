# Issue 21033: package type = pip needs documentation

archive/issues_020796.json:
```json
{
    "assignees": [],
    "body": "There are now several packages where `build/pkgs/PACKAGE/type` is \"pip\".\n\nIs there any documentation on this?\n\nSome related tickets: \n- #20218: Use pip to install Python dependencies\n- #19213: For packages listed in build/pkgs/piprules, allow 'sage --optional' to list them\n- #19680: Add mock, pytest, tox, virtualenv optional packages\n\nDiscussions:\n- https://groups.google.com/d/topic/sage-devel/gRxr36XAR6I/discussion\n\n\nDepends on #29287\n\nCC:  @videlec @vbraun @embray @jdemeyer @nexttime @jhpalmieri\n\nReviewer: **Matthias Koeppe, Samuel Leli\u00e8vre**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21033_\n\n",
    "closed_at": "2020-05-17T06:15:12Z",
    "created_at": "2016-07-16T14:08:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "package type = pip needs documentation",
    "type": "issue",
    "updated_at": "2020-05-17T06:15:12Z",
    "url": "https://github.com/sagemath/sage/issues/21033",
    "user": "https://github.com/mkoeppe"
}
```
There are now several packages where `build/pkgs/PACKAGE/type` is "pip".

Is there any documentation on this?

Some related tickets: 
- #20218: Use pip to install Python dependencies
- #19213: For packages listed in build/pkgs/piprules, allow 'sage --optional' to list them
- #19680: Add mock, pytest, tox, virtualenv optional packages

Discussions:
- https://groups.google.com/d/topic/sage-devel/gRxr36XAR6I/discussion


Depends on #29287

CC:  @videlec @vbraun @embray @jdemeyer @nexttime @jhpalmieri

Reviewer: **Matthias Koeppe, Samuel Lelièvre**

_Issue created by migration from https://trac.sagemath.org/ticket/21033_





---

archive/issue_events_277955.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-07-16T14:08:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277955"
}
```



---

archive/issue_events_277956.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-07-16T14:08:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277956"
}
```



---

archive/issue_events_277957.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-07-16T14:08:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277957"
}
```



---

archive/issue_events_277958.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-07-16T14:08:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277958"
}
```



---

archive/issue_comments_308430.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI don't know if everybody agrees to \"officially\" support the `pip` type packages. It was mainly meant as stopgap for some disappearing old-style packages.",
    "created_at": "2016-07-17T07:36:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308430",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'>Comment 1:</a>
I don't know if everybody agrees to "officially" support the `pip` type packages. It was mainly meant as stopgap for some disappearing old-style packages.



---

archive/issue_comments_308431.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,9 +1,3 @@\n There are now several packages where `build/pkgs/PACKAGE/type` is \"pip\".\n \n Is there any documentation on this?\n-I tried to add a package of this type for pypolymake (for #20892), but adding the file build/pkgs/pypolymake/type does not seem to enable \"sage -i pypolymake\". What am I missing?\n-\n-Also, shouldn't the way in which a package is installed (traditional way vs. pip way)\n-be orthogonal to whether a package is considered standard, optional, or experimental?\n-\n-\n``````\n",
    "created_at": "2016-07-17T09:13:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308431",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,9 +1,3 @@
 There are now several packages where `build/pkgs/PACKAGE/type` is "pip".
 
 Is there any documentation on this?
-I tried to add a package of this type for pypolymake (for #20892), but adding the file build/pkgs/pypolymake/type does not seem to enable "sage -i pypolymake". What am I missing?
-
-Also, shouldn't the way in which a package is installed (traditional way vs. pip way)
-be orthogonal to whether a package is considered standard, optional, or experimental?
-
-
``````




---

archive/issue_comments_308432.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\n> Also, shouldn't the way in which a package is installed (traditional way vs. pip way) be orthogonal to whether a package is considered standard, optional, or experimental?\n\nCertainly not standard since we need to ship the sources of standard packages with Sage. They aren't really optional nor experimental either. I see it as just a shortcut to `./sage --pip install PKG`.",
    "created_at": "2016-07-17T09:15:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308432",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
> Also, shouldn't the way in which a package is installed (traditional way vs. pip way) be orthogonal to whether a package is considered standard, optional, or experimental?

Certainly not standard since we need to ship the sources of standard packages with Sage. They aren't really optional nor experimental either. I see it as just a shortcut to `./sage --pip install PKG`.



---

archive/issue_comments_308433.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThe pip-type package could be enhanced to use a mirrored tarball if there is one, then we could also use it for standard packages. Though really the benefit seems pretty small compared to putting `pip install` in the `spkg-install`.",
    "created_at": "2016-07-17T09:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308433",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'>Comment 4:</a>
The pip-type package could be enhanced to use a mirrored tarball if there is one, then we could also use it for standard packages. Though really the benefit seems pretty small compared to putting `pip install` in the `spkg-install`.



---

archive/issue_comments_308434.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nReplying to [@jdemeyer](#comment%3A1):\n> I don't know if everybody agrees to \"officially\" support the `pip` type packages. It was mainly meant as stopgap for some disappearing old-style packages.\n\n\"Official\" or not, documentation is better than no documentation, no?",
    "created_at": "2016-07-17T09:54:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308434",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'>Comment 5:</a>
Replying to [@jdemeyer](#comment%3A1):
> I don't know if everybody agrees to "officially" support the `pip` type packages. It was mainly meant as stopgap for some disappearing old-style packages.

"Official" or not, documentation is better than no documentation, no?



---

archive/issue_comments_308435.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nReplying to [@jdemeyer](#comment%3A2):\n Description modified\n \nLet me ask again my question which you removed from the description above:\n\nI tried to add a package of this type for pypolymake (for #20892), but adding the file build/pkgs/pypolymake/type does not seem to enable \"sage -i pypolymake\". What am I missing?",
    "created_at": "2016-07-17T09:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308435",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'>Comment 6:</a>
Replying to [@jdemeyer](#comment%3A2):
 Description modified
 
Let me ask again my question which you removed from the description above:

I tried to add a package of this type for pypolymake (for #20892), but adding the file build/pkgs/pypolymake/type does not seem to enable "sage -i pypolymake". What am I missing?



---

archive/issue_comments_308436.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,8 @@\n There are now several packages where `build/pkgs/PACKAGE/type` is \"pip\".\n \n Is there any documentation on this?\n+\n+Some related tickets: \n+- #20218: Use pip to install Python dependencies\n+- #19213: For packages listed in build/pkgs/piprules, allow 'sage --optional' to list them\n+ \n``````\n",
    "created_at": "2016-07-26T21:31:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308436",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,8 @@
 There are now several packages where `build/pkgs/PACKAGE/type` is "pip".
 
 Is there any documentation on this?
+
+Some related tickets: 
+- #20218: Use pip to install Python dependencies
+- #19213: For packages listed in build/pkgs/piprules, allow 'sage --optional' to list them
+ 
``````




---

archive/issue_comments_308437.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,4 +5,4 @@\n Some related tickets: \n - #20218: Use pip to install Python dependencies\n - #19213: For packages listed in build/pkgs/piprules, allow 'sage --optional' to list them\n- \n+- #19680: Add mock, pytest, tox, virtualenv optional packages\n``````\n",
    "created_at": "2016-08-10T16:24:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308437",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,4 +5,4 @@
 Some related tickets: 
 - #20218: Use pip to install Python dependencies
 - #19213: For packages listed in build/pkgs/piprules, allow 'sage --optional' to list them
- 
+- #19680: Add mock, pytest, tox, virtualenv optional packages
``````




---

archive/issue_events_277959.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-08-28T11:11:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277959"
}
```



---

archive/issue_events_277960.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-08-28T11:11:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277960"
}
```



---

archive/issue_comments_308438.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,3 +6,7 @@\n - #20218: Use pip to install Python dependencies\n - #19213: For packages listed in build/pkgs/piprules, allow 'sage --optional' to list them\n - #19680: Add mock, pytest, tox, virtualenv optional packages\n+\n+Discussions:\n+- https://groups.google.com/d/topic/sage-devel/gRxr36XAR6I/discussion\n+\n``````\n",
    "created_at": "2016-08-28T11:11:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308438",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,3 +6,7 @@
 - #20218: Use pip to install Python dependencies
 - #19213: For packages listed in build/pkgs/piprules, allow 'sage --optional' to list them
 - #19680: Add mock, pytest, tox, virtualenv optional packages
+
+Discussions:
+- https://groups.google.com/d/topic/sage-devel/gRxr36XAR6I/discussion
+
``````




---

archive/issue_events_277961.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-08T00:15:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277961"
}
```



---

archive/issue_events_277962.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-08T00:15:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277962"
}
```



---

archive/issue_events_277963.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-14T21:50:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277963"
}
```



---

archive/issue_comments_308439.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\n#29287 is adding documentation",
    "created_at": "2020-03-14T21:50:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308439",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:11'>Comment 11:</a>
#29287 is adding documentation



---

archive/issue_comments_308440.json:
```json
{
    "body": "Dependencies: **#29287**",
    "created_at": "2020-03-14T21:50:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308440",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#29287**



---

archive/issue_events_277964.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-14T21:50:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277964"
}
```



---

archive/issue_events_277965.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-14T21:50:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277965"
}
```



---

archive/issue_events_277966.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-14T21:50:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277966"
}
```



---

archive/issue_events_277967.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-05-12T11:49:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277967"
}
```



---

archive/issue_events_277968.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-05-12T11:49:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277968"
}
```



---

archive/issue_comments_308441.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe, Samuel Leli\u00e8vre**",
    "created_at": "2020-05-12T11:49:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308441",
    "user": "https://github.com/slel"
}
```

Reviewer: **Matthias Koeppe, Samuel Lelièvre**



---

archive/issue_comments_308442.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nFixed by #29287.",
    "created_at": "2020-05-12T11:49:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21033#issuecomment-308442",
    "user": "https://github.com/slel"
}
```

<a id='comment:13'>Comment 13:</a>
Fixed by #29287.



---

archive/issue_events_277969.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-05-17T06:15:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277969"
}
```



---

archive/issue_events_277970.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-05-17T06:15:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277970"
}
```



---

archive/issue_events_277971.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-05-17T06:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21033",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21033#event-277971"
}
```
