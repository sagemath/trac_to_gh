# Issue 21782: Sage does not build on debian testing

archive/issues_021545.json:
```json
{
    "assignees": [],
    "body": "In debian testing with gcc-6 Sage does not build. flint fails with ld errors. Debian enables -pie by default on gcc-6 with gcc-5 Sage builds perfectly.\n\nNote that gcc-6 will be the default (only!) compiler in next debian release.\n\nApparently the flint build fail is already solved in debian\n\nhttps://bugs.debian.org/cgi-bin/bugreport.cgi?bug=837453\n\nI do not know if further problems appear with other packages.\n\nKeywords: **gcc-6 pie flint**\n\nBranch/Commit: **[96442c6](https://github.com/sagemath/sagetrac-mirror/commit/96442c608b0dd83810aaf9f7eac925d859457747)**\n\nReviewer: **Jeroen Demeyer, Fran\u00e7ois Bissey**\n\nAuthor: **J\u00f6rg-Volker Peetz, Emmanuel Charpentier**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21782_\n\n",
    "closed_at": "2016-11-09T18:16:19Z",
    "created_at": "2016-10-29T09:22:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Sage does not build on debian testing",
    "type": "issue",
    "updated_at": "2016-11-09T18:16:19Z",
    "url": "https://github.com/sagemath/sage/issues/21782",
    "user": "https://github.com/lftabera"
}
```
In debian testing with gcc-6 Sage does not build. flint fails with ld errors. Debian enables -pie by default on gcc-6 with gcc-5 Sage builds perfectly.

Note that gcc-6 will be the default (only!) compiler in next debian release.

Apparently the flint build fail is already solved in debian

https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=837453

I do not know if further problems appear with other packages.

Keywords: **gcc-6 pie flint**

Branch/Commit: **[96442c6](https://github.com/sagemath/sagetrac-mirror/commit/96442c608b0dd83810aaf9f7eac925d859457747)**

Reviewer: **Jeroen Demeyer, François Bissey**

Author: **Jörg-Volker Peetz, Emmanuel Charpentier**

_Issue created by migration from https://trac.sagemath.org/ticket/21782_





---

archive/issue_events_287626.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2016-10-29T09:22:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287626"
}
```



---

archive/issue_events_287627.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2016-10-29T09:22:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287627"
}
```



---

archive/issue_events_287628.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2016-10-29T09:22:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287628"
}
```



---

archive/issue_events_287629.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2016-10-29T09:22:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287629"
}
```



---

archive/issue_events_287630.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2016-11-05T18:21:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287630"
}
```



---

archive/issue_events_287631.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2016-11-05T18:21:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287631"
}
```



---

archive/issue_comments_322999.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nGiven the experience I had with various compiler go ahead with the debian patches, the bots will tell us what things fails if any.",
    "created_at": "2016-11-07T01:13:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-322999",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:2'>Comment 2:</a>
Given the experience I had with various compiler go ahead with the debian patches, the bots will tell us what things fails if any.



---

archive/issue_comments_323000.json:
```json
{
    "body": "Branch: **[u/charpent/fix-flint](https://github.com/sagemath/sagetrac-mirror/tree/u/charpent/fix-flint)**",
    "created_at": "2016-11-07T07:46:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323000",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

Branch: **[u/charpent/fix-flint](https://github.com/sagemath/sagetrac-mirror/tree/u/charpent/fix-flint)**



---

archive/issue_comments_323001.json:
```json
{
    "body": "Author: **J\u00f6rg-Volker, Emmanuel Charpentier**",
    "created_at": "2016-11-07T07:59:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323001",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

Author: **Jörg-Volker, Emmanuel Charpentier**



---

archive/issue_events_287632.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2016-11-07T07:59:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287632"
}
```



---

archive/issue_comments_323002.json:
```json
{
    "body": "Commit: **[e2d10c3](https://github.com/sagemath/sagetrac-mirror/commit/e2d10c35984254e8ef0c78fcb6a4e97172356452)**",
    "created_at": "2016-11-07T07:59:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323002",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

Commit: **[e2d10c3](https://github.com/sagemath/sagetrac-mirror/commit/e2d10c35984254e8ef0c78fcb6a4e97172356452)**



---

archive/issue_comments_323003.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPatch proposed by someone called \"J\u00f6rg-Volker\" in [this post](https://groups.google.com/d/msg/sage-devel/TduebNoZuBE/sEULolL0BQAJ), and packaged by me. Author contacted for complete identity (no answer yet) and credited as such.\n\n* Allows compilation on Debian with gcc 6.2.0.\n* Passes `ptestlong` with no errors (not even the usual transiant error on simplicial_complex).\n* Checked by [Fran\u00e7ois Bissey](https://groups.google.com/d/msg/sage-release/OtYJUXuf2vk/U-ssKihOAQAJ) on :\n    * Gentoo / gcc 5.4.0,\n    * OS X / gcc 4.9.3,\n    * OS X / clang.\n\n==> `needs_review`\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e2d10c35984254e8ef0c78fcb6a4e97172356452\">e2d10c3</a></td><td><code>Patches allowing compilation with gcc >= 6.2.x</code></td></tr></table>\n",
    "created_at": "2016-11-07T07:59:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323003",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:4'>Comment 4:</a>
Patch proposed by someone called "Jörg-Volker" in [this post](https://groups.google.com/d/msg/sage-devel/TduebNoZuBE/sEULolL0BQAJ), and packaged by me. Author contacted for complete identity (no answer yet) and credited as such.

* Allows compilation on Debian with gcc 6.2.0.
* Passes `ptestlong` with no errors (not even the usual transiant error on simplicial_complex).
* Checked by [François Bissey](https://groups.google.com/d/msg/sage-release/OtYJUXuf2vk/U-ssKihOAQAJ) on :
    * Gentoo / gcc 5.4.0,
    * OS X / gcc 4.9.3,
    * OS X / clang.

==> `needs_review`

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e2d10c35984254e8ef0c78fcb6a4e97172356452">e2d10c3</a></td><td><code>Patches allowing compilation with gcc >= 6.2.x</code></td></tr></table>




---

archive/issue_comments_323004.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nCan you inquire about the `-r` option, what it does, and where is there doc for it?",
    "created_at": "2016-11-07T09:04:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323004",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:5'>Comment 5:</a>
Can you inquire about the `-r` option, what it does, and where is there doc for it?



---

archive/issue_events_287633.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-07T10:35:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287633"
}
```



---

archive/issue_events_287634.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-11-07T10:35:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287634"
}
```



---

archive/issue_comments_323005.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nYou should bump the patch level in `package-version.txt` of the packages that you changed.",
    "created_at": "2016-11-07T10:35:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323005",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
You should bump the patch level in `package-version.txt` of the packages that you changed.



---

archive/issue_comments_323006.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nAlso this should be fixed:\n\n```\n(Stripping trailing CRs from patch; use --binary to disable.)\npatching file Makefile.subdirs\n(Stripping trailing CRs from patch; use --binary to disable.)\npatching file fq/Makefile\n(Stripping trailing CRs from patch; use --binary to disable.)\npatching file fq_poly/Makefile\n(Stripping trailing CRs from patch; use --binary to disable.)\npatching file padic_mat/Makefile\n(Stripping trailing CRs from patch; use --binary to disable.)\npatching file padic_poly/Makefile\n(Stripping trailing CRs from patch; use --binary to disable.)\npatching file qadic/Makefile\n```\n\n(both flint and arb patches have the same problem)",
    "created_at": "2016-11-07T10:38:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323006",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
Also this should be fixed:

```
(Stripping trailing CRs from patch; use --binary to disable.)
patching file Makefile.subdirs
(Stripping trailing CRs from patch; use --binary to disable.)
patching file fq/Makefile
(Stripping trailing CRs from patch; use --binary to disable.)
patching file fq_poly/Makefile
(Stripping trailing CRs from patch; use --binary to disable.)
patching file padic_mat/Makefile
(Stripping trailing CRs from patch; use --binary to disable.)
patching file padic_poly/Makefile
(Stripping trailing CRs from patch; use --binary to disable.)
patching file qadic/Makefile
```

(both flint and arb patches have the same problem)



---

archive/issue_comments_323007.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nSeems to work with\n\n```\ngcc (Gentoo 4.9.3 p1.5, pie-0.6.4) 4.9.3\nCopyright (C) 2015 Free Software Foundation, Inc.\nThis is free software; see the source for copying conditions.  There is NO\nwarranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n```",
    "created_at": "2016-11-07T10:40:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323007",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'>Comment 8:</a>
Seems to work with

```
gcc (Gentoo 4.9.3 p1.5, pie-0.6.4) 4.9.3
Copyright (C) 2015 Free Software Foundation, Inc.
This is free software; see the source for copying conditions.  There is NO
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
```



---

archive/issue_comments_323008.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nPlease document (in the `.patch` file) where the patch came from.",
    "created_at": "2016-11-07T11:11:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323008",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'>Comment 9:</a>
Please document (in the `.patch` file) where the patch came from.



---

archive/issue_comments_323009.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nAlso works on\n\n```\nLinux sagebu14_32s02 4.7.0-0.bpo.1-amd64 #1 SMP Debian 4.7.8-1~bpo8+1 (2016-10-19) i686 i686 i686 GNU/Linux\n```\nwith\n\n```\ngcc (Ubuntu 4.8.4-2ubuntu1~14.04.3) 4.8.4\nCopyright (C) 2013 Free Software Foundation, Inc.\nThis is free software; see the source for copying conditions.  There is NO\nwarranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n```\n\nSo, apart from the minor issues with the patch files, this is good for me.",
    "created_at": "2016-11-07T14:23:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323009",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'>Comment 10:</a>
Also works on

```
Linux sagebu14_32s02 4.7.0-0.bpo.1-amd64 #1 SMP Debian 4.7.8-1~bpo8+1 (2016-10-19) i686 i686 i686 GNU/Linux
```
with

```
gcc (Ubuntu 4.8.4-2ubuntu1~14.04.3) 4.8.4
Copyright (C) 2013 Free Software Foundation, Inc.
This is free software; see the source for copying conditions.  There is NO
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
```

So, apart from the minor issues with the patch files, this is good for me.



---

archive/issue_comments_323010.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1833b35e8f1184145f321834bcd752b1b202a220\">1833b35</a></td><td><code>Bump package versions, patch file format nitpicking.</code></td></tr></table>\n",
    "created_at": "2016-11-07T18:15:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323010",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1833b35e8f1184145f321834bcd752b1b202a220">1833b35</a></td><td><code>Bump package versions, patch file format nitpicking.</code></td></tr></table>




---

archive/issue_comments_323011.json:
```json
{
    "body": "Changed commit from **[e2d10c3](https://github.com/sagemath/sagetrac-mirror/commit/e2d10c35984254e8ef0c78fcb6a4e97172356452)** to **[1833b35](https://github.com/sagemath/sagetrac-mirror/commit/1833b35e8f1184145f321834bcd752b1b202a220)**",
    "created_at": "2016-11-07T18:15:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323011",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e2d10c3](https://github.com/sagemath/sagetrac-mirror/commit/e2d10c35984254e8ef0c78fcb6a4e97172356452)** to **[1833b35](https://github.com/sagemath/sagetrac-mirror/commit/1833b35e8f1184145f321834bcd752b1b202a220)**



---

archive/issue_comments_323012.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nReplying to [@kiwifb](#comment%3A5):\n> Can you inquire about the `-r` option, what it does, and where is there doc for it?\n\nI'll do that (probably by going directly to the [gcc mailing list](https://gcc.gnu.org/ml/gcc/)). But this should not, IMHO, delay the fixing of this blocker bug.",
    "created_at": "2016-11-07T18:20:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323012",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:12'>Comment 12:</a>
Replying to [@kiwifb](#comment%3A5):
> Can you inquire about the `-r` option, what it does, and where is there doc for it?

I'll do that (probably by going directly to the [gcc mailing list](https://gcc.gnu.org/ml/gcc/)). But this should not, IMHO, delay the fixing of this blocker bug.



---

archive/issue_events_287635.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2016-11-07T18:23:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287635"
}
```



---

archive/issue_events_287636.json:
```json
{
    "actor": "https://github.com/EmmanuelCharpentier",
    "created_at": "2016-11-07T18:23:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287636"
}
```



---

archive/issue_comments_323013.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nReplying to [git](#comment%3A11):\n> Branch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1833b35e8f1184145f321834bcd752b1b202a220\">1833b35</a></td><td><code>Bump package versions, patch file format nitpicking.</code></td></tr></table>\n\nFulfills comments 6, 7 and 9.\n\n==> `needs_review`",
    "created_at": "2016-11-07T18:23:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323013",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:13'>Comment 13:</a>
Replying to [git](#comment%3A11):
> Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1833b35e8f1184145f321834bcd752b1b202a220">1833b35</a></td><td><code>Bump package versions, patch file format nitpicking.</code></td></tr></table>

Fulfills comments 6, 7 and 9.

==> `needs_review`



---

archive/issue_comments_323014.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nReplying to [@EmmanuelCharpentier](#comment%3A12):\n> Replying to [@kiwifb](#comment%3A5):\n> > Can you inquire about the `-r` option, what it does, and where is there doc for it?\n\n> \n> I'll do that (probably by going directly to the [gcc mailing list](https://gcc.gnu.org/ml/gcc/)). But this should not, IMHO, delay the fixing of this blocker bug.\n\nDone [ https://gcc.gnu.org/ml/gcc/2016-11/msg00014.html here].",
    "created_at": "2016-11-07T18:35:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323014",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:14'>Comment 14:</a>
Replying to [@EmmanuelCharpentier](#comment%3A12):
> Replying to [@kiwifb](#comment%3A5):
> > Can you inquire about the `-r` option, what it does, and where is there doc for it?

> 
> I'll do that (probably by going directly to the [gcc mailing list](https://gcc.gnu.org/ml/gcc/)). But this should not, IMHO, delay the fixing of this blocker bug.

Done [ https://gcc.gnu.org/ml/gcc/2016-11/msg00014.html here].



---

archive/issue_comments_323015.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nReplying to [@EmmanuelCharpentier](#comment%3A14):\n> Replying to [@EmmanuelCharpentier](#comment%3A12):\n> > Replying to [@kiwifb](#comment%3A5):\n> > > Can you inquire about the `-r` option, what it does, and where is there doc for it?\n\n> > \n> > I'll do that (probably by going directly to the [gcc mailing list](https://gcc.gnu.org/ml/gcc/)). But this should not, IMHO, delay the fixing of this blocker bug.\n\n> \n> Done [ https://gcc.gnu.org/ml/gcc/2016-11/msg00014.html here].\n\nThe answer is [here](https://gcc.gnu.org/ml/gcc/2016-11/msg00015.html) : and it is shamefully simple :\n\n    \"The option -r is passed to ld, so you have to look for it in ld's manual  where it is \n    clearly documented.\"\n\n<Sigh...>",
    "created_at": "2016-11-07T20:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323015",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:15'>Comment 15:</a>
Replying to [@EmmanuelCharpentier](#comment%3A14):
> Replying to [@EmmanuelCharpentier](#comment%3A12):
> > Replying to [@kiwifb](#comment%3A5):
> > > Can you inquire about the `-r` option, what it does, and where is there doc for it?

> > 
> > I'll do that (probably by going directly to the [gcc mailing list](https://gcc.gnu.org/ml/gcc/)). But this should not, IMHO, delay the fixing of this blocker bug.

> 
> Done [ https://gcc.gnu.org/ml/gcc/2016-11/msg00014.html here].

The answer is [here](https://gcc.gnu.org/ml/gcc/2016-11/msg00015.html) : and it is shamefully simple :

    "The option -r is passed to ld, so you have to look for it in ld's manual  where it is 
    clearly documented."

<Sigh...>



---

archive/issue_comments_323016.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nYes I could tell that it was passed to `ld`, but normally you need `-Wl,` to do that. And the other options that are passed to `ld` like `-L` and `-l` are documented. Other compilers pass unknown options to the linker (IBM XL familly of compilers if you need an example) but this is not supposed to be case for gcc.\n\nAnd best of all while it is passed to `ld`, how is it passed differently from when it is given by `-Wl,-r` and why doesn't it trigger the incompatibility problem then?\n\nAnyone interested in seeing some gory details of `flint`/`arb` compilation can set `CC=\"gcc -v -Wl,--verbose\"` (I didn't use CFLAGS because the lines with `-r` don't include CFLAGS).",
    "created_at": "2016-11-07T20:21:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323016",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:16'>Comment 16:</a>
Yes I could tell that it was passed to `ld`, but normally you need `-Wl,` to do that. And the other options that are passed to `ld` like `-L` and `-l` are documented. Other compilers pass unknown options to the linker (IBM XL familly of compilers if you need an example) but this is not supposed to be case for gcc.

And best of all while it is passed to `ld`, how is it passed differently from when it is given by `-Wl,-r` and why doesn't it trigger the incompatibility problem then?

Anyone interested in seeing some gory details of `flint`/`arb` compilation can set `CC="gcc -v -Wl,--verbose"` (I didn't use CFLAGS because the lines with `-r` don't include CFLAGS).



---

archive/issue_comments_323017.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nI asked on sage-devel but I'll ask again here, can you post the output of `gcc -dumpspecs`, you can send that privately if you wish. I'd like to see how they enforce the \"pie\" stuff. That may enlighten me.\n\nWe know empirically that the fix works. We just don't know why and that annoys me deeply.",
    "created_at": "2016-11-07T22:56:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323017",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:17'>Comment 17:</a>
I asked on sage-devel but I'll ask again here, can you post the output of `gcc -dumpspecs`, you can send that privately if you wish. I'd like to see how they enforce the "pie" stuff. That may enlighten me.

We know empirically that the fix works. We just don't know why and that annoys me deeply.



---

archive/issue_comments_323018.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nReplying to [@kiwifb](#comment%3A17):\n> I asked on sage-devel \n\nSorry : I missed that one (and can't find it, BTW...)\n\n> but I'll ask again here, can you post the output of `gcc -dumpspecs`,\n\nHere it comes :\n\n```\ncharpent@asus16-ec:~$ gcc -dumpspecs\n*asm:\n%{m16|m32:--32}  %{m16|m32|mx32:;:--64}  %{mx32:--x32}  %{!mno-sse2avx:%{mavx:-msse2avx}} %{msse2avx:%{!mavx:-msse2avx}}\n\n*asm_debug:\n%{!g0:%{gstabs*:--gstabs}%{!gstabs*:%{g*:--gdwarf2}}} %{fdebug-prefix-map=*:--debug-prefix-map %*}\n\n*asm_final:\n%{gsplit-dwarf: \n       objcopy --extract-dwo \t %{c:%{o*:%*}%{!o*:%b%O}}%{!c:%U%O} \t %{c:%{o*:%:replace-extension(%{o*:%*} .dwo)}%{!o*:%b.dwo}}%{!c:%b.dwo} \n       objcopy --strip-dwo \t %{c:%{o*:%*}%{!o*:%b%O}}%{!c:%U%O}     }\n\n*asm_options:\n%{-target-help:%:print-asm-header()} %{v} %{w:-W} %{I*}  %{gz*:%e-gz is not supported in this configuration} %a %Y %{c:%W{o*}%{!o*:-o %w%b%O}}%{!c:-o %d%w%u%O}\n\n*invoke_as:\n%{!fwpa*:   %{fcompare-debug=*|fdump-final-insns=*:%:compare-debug-dump-opt()}   %{!S:-o %|.s |\n as %(asm_options) %m.s %A }  }\n\n*cpp:\n%{posix:-D_POSIX_SOURCE} %{pthread:-D_REENTRANT}\n\n*cpp_options:\n%(cpp_unique_options) %1 %{m*} %{std*&ansi&trigraphs} %{W*&pedantic*} %{w} %{f*} %{g*:%{!g0:%{g*} %{!fno-working-directory:-fworking-directory}}} %{O*} %{undef} %{save-temps*:-fpch-preprocess}\n\n*cpp_debug_options:\n%{d*}\n\n*cpp_unique_options:\n%{!Q:-quiet} %{nostdinc*} %{C} %{CC} %{v} %{I*&F*} %{P} %I %{MD:-MD %{!o:%b.d}%{o*:%.d%*}} %{MMD:-MMD %{!o:%b.d}%{o*:%.d%*}} %{M} %{MM} %{MF*} %{MG} %{MP} %{MQ*} %{MT*} %{!E:%{!M:%{!MM:%{!MT:%{!MQ:%{MD|MMD:%{o*:-MQ %*}}}}}}} %{remap} %{g3|ggdb3|gstabs3|gcoff3|gxcoff3|gvms3:-dD} %{!iplugindir*:%{fplugin*:%:find-plugindir()}} %{H} %C %{D*&U*&A*} %{i*} %Z %i %{E|M|MM:%W{o*}}\n\n*trad_capable_cpp:\ncc1 -E %{traditional|traditional-cpp:-traditional-cpp}\n\n*cc1:\n%{!mandroid|tno-android-cc:%(cc1_cpu) %{profile:-p};:%(cc1_cpu) %{profile:-p} %{!mglibc:%{!muclibc:%{!mbionic: -mbionic}}} %{!fno-pic:%{!fno-PIC:%{!fpic:%{!fPIC: -fPIC}}}}}\n\n*cc1_options:\n%{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}} %{!iplugindir*:%{fplugin*:%:find-plugindir()}} %1 %{!Q:-quiet} %{!dumpbase:-dumpbase %B} %{d*} %{m*} %{aux-info*} %{fcompare-debug-second:%:compare-debug-auxbase-opt(%b)}  %{!fcompare-debug-second:%{c|S:%{o*:-auxbase-strip %*}%{!o*:-auxbase %b}}}%{!c:%{!S:-auxbase %b}}  %{g*} %{O*} %{W*&pedantic*} %{w} %{std*&ansi&trigraphs} %{v:-version} %{pg:-p} %{p} %{f*} %{undef} %{Qn:-fno-ident} %{Qy:} %{-help:--help} %{-target-help:--target-help} %{-version:--version} %{-help=*:--help=%*} %{!fsyntax-only:%{S:%W{o*}%{!o*:-o %b.s}}} %{fsyntax-only:-o %j} %{-param*} %{coverage:-fprofile-arcs -ftest-coverage}\n\n*cc1plus:\n\n\n*link_gcc_c_sequence:\n%{static:--start-group} %G %L %{static:--end-group}%{!static:%G}\n\n*link_ssp:\n%{fstack-protector|fstack-protector-all|fstack-protector-strong|fstack-protector-explicit:}\n\n*endfile:\n%{!mandroid|tno-android-ld:%{Ofast|ffast-math|funsafe-math-optimizations:crtfastmath.o%s}    %{mpc32:crtprec32.o%s}    %{mpc64:crtprec64.o%s}    %{mpc80:crtprec80.o%s} %{fvtable-verify=none:%s;      fvtable-verify=preinit:vtv_end_preinit.o%s;      fvtable-verify=std:vtv_end.o%s}    %{shared:crtendS.o%s;: %{no-pie|static|r|shared:;:crtendS.o%s}    %{no-pie|static:crtend.o%s}} crtn.o%s    ;:%{Ofast|ffast-math|funsafe-math-optimizations:crtfastmath.o%s}    %{mpc32:crtprec32.o%s}    %{mpc64:crtprec64.o%s}    %{mpc80:crtprec80.o%s} %{shared: crtend_so%O%s;: crtend_android%O%s}}\n\n*link:\n%{!r:--build-id} %{!static:--eh-frame-hdr} %{!mandroid|tno-android-ld:%{m16|m32|mx32:;:-m elf_x86_64}                    %{m16|m32:-m elf_i386}                    %{mx32:-m elf32_x86_64}   --hash-style=gnu   %{shared:-shared}   %{!shared:     %{!static:       %{rdynamic:-export-dynamic}       %{m16|m32:-dynamic-linker %{muclibc:/lib/ld-uClibc.so.0;:%{mbionic:/system/bin/linker;:%{mmusl:/lib/ld-musl-i386.so.1;:/lib/ld-linux.so.2}}}}       %{m16|m32|mx32:;:-dynamic-linker %{muclibc:/lib/ld64-uClibc.so.0;:%{mbionic:/system/bin/linker64;:%{mmusl:/lib/ld-musl-x86_64.so.1;:/lib64/ld-linux-x86-64.so.2}}}}       %{mx32:-dynamic-linker %{muclibc:/lib/ldx32-uClibc.so.0;:%{mbionic:/system/bin/linkerx32;:%{mmusl:/lib/ld-musl-x32.so.1;:/libx32/ld-linux-x32.so.2}}}}}     %{static:-static}};:%{m16|m32|mx32:;:-m elf_x86_64}                    %{m16|m32:-m elf_i386}                    %{mx32:-m elf32_x86_64}   --hash-style=gnu   %{shared:-shared}   %{!shared:     %{!static:       %{rdynamic:-export-dynamic}       %{m16|m32:-dynamic-linker %{muclibc:/lib/ld-uClibc.so.0;:%{mbionic:/system/bin/linker;:%{mmusl:/lib/ld-musl-i386.so.1;:/lib/ld-linux.so.2}}}}       %{m16|m32|mx32:;:-dynamic-linker %{muclibc:/lib/ld64-uClibc.so.0;:%{mbionic:/system/bin/linker64;:%{mmusl:/lib/ld-musl-x86_64.so.1;:/lib64/ld-linux-x86-64.so.2}}}}       %{mx32:-dynamic-linker %{muclibc:/lib/ldx32-uClibc.so.0;:%{mbionic:/system/bin/linkerx32;:%{mmusl:/lib/ld-musl-x32.so.1;:/libx32/ld-linux-x32.so.2}}}}}     %{static:-static}} %{shared: -Bsymbolic}}\n\n*lib:\n%{!mandroid|tno-android-ld:%{pthread:-lpthread} %{shared:-lc}    %{!shared:%{mieee-fp:-lieee} %{profile:-lc_p}%{!profile:-lc}};:%{shared:-lc}    %{!shared:%{mieee-fp:-lieee} %{profile:-lc_p}%{!profile:-lc}} %{!static: -ldl}}\n\n*link_gomp:\n\n\n*libgcc:\n%{static|static-libgcc:-lgcc -lgcc_eh}%{!static:%{!static-libgcc:%{!shared-libgcc:-lgcc --as-needed -lgcc_s --no-as-needed}%{shared-libgcc:-lgcc_s%{!shared: -lgcc}}}}\n\n*startfile:\n%{!mandroid|tno-android-ld:%{!shared: %{pg|p|profile:gcrt1.o%s;:     %{no-pie|static|r|shared:;:Scrt1.o%s} %{no-pie|static:crt1.o%s}}}    crti.o%s %{static:crtbeginT.o%s;: %{shared:crtbeginS.o%s} \t      %{no-pie|static|r|shared:;:crtbeginS.o%s} \t      %{no-pie|static:crtbegin.o%s}}    %{fvtable-verify=none:%s;      fvtable-verify=preinit:vtv_start_preinit.o%s;      fvtable-verify=std:vtv_start.o%s}    ;:%{shared: crtbegin_so%O%s;:  %{static: crtbegin_static%O%s;: crtbegin_dynamic%O%s}}}\n\n*cross_compile:\n0\n\n*version:\n6.2.0\n\n*multilib:\n. !m32 !m64 !mx32;32:../lib32:i386-linux-gnu m32 !m64 !mx32;64:../lib:x86_64-linux-gnu !m32 m64 !mx32;x32:../libx32:x86_64-linux-gnux32 !m32 !m64 mx32;\n\n*multilib_defaults:\nm64\n\n*multilib_extra:\n\n\n*multilib_matches:\nm32 m32;m64 m64;mx32 mx32;\n\n*multilib_exclusions:\n\n\n*multilib_options:\nm32/m64/mx32\n\n*multilib_reuse:\n\n\n*linker:\ncollect2\n\n*linker_plugin_file:\n\n\n*lto_wrapper:\n\n\n*lto_gcc:\n\n\n*post_link:\n\n\n*link_libgcc:\n%D\n\n*md_exec_prefix:\n\n\n*md_startfile_prefix:\n\n\n*md_startfile_prefix_1:\n\n\n*startfile_prefix_spec:\n\n\n*sysroot_spec:\n--sysroot=%R\n\n*sysroot_suffix_spec:\n\n\n*sysroot_hdrs_suffix_spec:\n\n\n*self_spec:\n\n\n*cc1_cpu:\n%{march=native:%>march=native %:local_cpu_detect(arch)   %{!mtune=*:%>mtune=native %:local_cpu_detect(tune)}} %{mtune=native:%>mtune=native %:local_cpu_detect(tune)}\n\n*link_command:\n%{!fsyntax-only:%{!c:%{!M:%{!MM:%{!E:%{!S:    %(linker) %{!fno-use-linker-plugin:%{!fno-lto:     -plugin %(linker_plugin_file)     -plugin-opt=%(lto_wrapper)     -plugin-opt=-fresolution=%u.res     %{!nostdlib:%{!nodefaultlibs:%:pass-through-libs(%(link_gcc_c_sequence))}}     }}%{flto|flto=*:%<fcompare-debug*}     %{flto} %{fno-lto} %{flto=*} %l %{no-pie:} %{no-pie|static|r|shared:;:-pie} %{fuse-ld=*:-fuse-ld=%*}  %{gz*:%e-gz is not supported in this configuration} %X %{o*} %{e*} %{N} %{n} %{r}    %{s} %{t} %{u*} %{z} %{Z} %{!nostdlib:%{!nostartfiles:%S}}     %{static:} %{L*} %(mfwrap) %(link_libgcc) %{!nostdlib:%{fvtable-verify=std: -lvtv -u_vtable_map_vars_start -u_vtable_map_vars_end}    %{fvtable-verify=preinit: -lvtv -u_vtable_map_vars_start -u_vtable_map_vars_end}} %{!nostdlib:%{!nodefaultlibs:%{%:sanitize(address):%{!shared:libasan_preinit%O%s} %{static-libasan:%{!shared:-Bstatic --whole-archive -lasan --no-whole-archive -Bdynamic}}%{!static-libasan:-lasan}}     %{%:sanitize(thread):%{static-libtsan:%{!shared:-Bstatic --whole-archive -ltsan --no-whole-archive -Bdynamic}}%{!static-libtsan:-ltsan}}     %{%:sanitize(leak):%{static-liblsan:%{!shared:-Bstatic --whole-archive -llsan --no-whole-archive -Bdynamic}}%{!static-liblsan:-llsan}}}} %o %{!nostdlib:%{!nodefaultlibs:%{mmpx:%{fcheck-pointer-bounds:    %{static:--whole-archive -lmpx --no-whole-archive %:include(libmpx.spec)%(link_libmpx)}    %{!static:%{static-libmpx:-Bstatic --whole-archive}    -lmpx %{static-libmpx:--no-whole-archive -Bdynamic %:include(libmpx.spec)%(link_libmpx)}}}}%{mmpx:%{fcheck-pointer-bounds:%{!fno-chkp-use-wrappers:    %{static:-lmpxwrappers}    %{!static:%{static-libmpxwrappers:-Bstatic --whole-archive}    -lmpxwrappers %{static-libmpxwrappers:--no-whole-archive -Bdynamic}}}}}}} %{mmpx:%{fcheck-pointer-bounds:%{!static:%{m16|m32|mx32:;:-z bndplt }}}}     %{fopenacc|fopenmp|%:gt(%{ftree-parallelize-loops=*:%*} 1):\t%:include(libgomp.spec)%(link_gomp)}    %{fcilkplus:%:include(libcilkrts.spec)%(link_cilkrts)}    %{fgnu-tm:%:include(libitm.spec)%(link_itm)}    %(mflib)  %{fsplit-stack: --wrap=pthread_create}    %{fprofile-arcs|fprofile-generate*|coverage:-lgcov} %{!nostdlib:%{!nodefaultlibs:%{%:sanitize(address): %{static-libasan:%:include(libsanitizer.spec)%(link_libasan)}    %{static:%ecannot specify -static with -fsanitize=address}}    %{%:sanitize(thread): %{static-libtsan:%:include(libsanitizer.spec)%(link_libtsan)}    %{static:%ecannot specify -static with -fsanitize=thread}}    %{%:sanitize(undefined):%{static-libubsan:-Bstatic} -lubsan %{static-libubsan:-Bdynamic} %{static-libubsan:%:include(libsanitizer.spec)%(link_libubsan)}}    %{%:sanitize(leak): %{static-liblsan:%:include(libsanitizer.spec)%(link_liblsan)}}}}     %{!nostdlib:%{!nodefaultlibs:%(link_ssp) %(link_gcc_c_sequence)}}    %{!nostdlib:%{!nostartfiles:%E}} %{T*}  \n%(post_link) }}}}}}\n```\n\n> you can send that privately if you wish. I'd like to see how they enforce the \"pie\" stuff. That may enlighten me.\n> \n> We know empirically that the fix works. We just don't know why and that annoys me deeply.",
    "created_at": "2016-11-08T03:35:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323018",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:18'>Comment 18:</a>
Replying to [@kiwifb](#comment%3A17):
> I asked on sage-devel 

Sorry : I missed that one (and can't find it, BTW...)

> but I'll ask again here, can you post the output of `gcc -dumpspecs`,

Here it comes :

```
charpent@asus16-ec:~$ gcc -dumpspecs
*asm:
%{m16|m32:--32}  %{m16|m32|mx32:;:--64}  %{mx32:--x32}  %{!mno-sse2avx:%{mavx:-msse2avx}} %{msse2avx:%{!mavx:-msse2avx}}

*asm_debug:
%{!g0:%{gstabs*:--gstabs}%{!gstabs*:%{g*:--gdwarf2}}} %{fdebug-prefix-map=*:--debug-prefix-map %*}

*asm_final:
%{gsplit-dwarf: 
       objcopy --extract-dwo 	 %{c:%{o*:%*}%{!o*:%b%O}}%{!c:%U%O} 	 %{c:%{o*:%:replace-extension(%{o*:%*} .dwo)}%{!o*:%b.dwo}}%{!c:%b.dwo} 
       objcopy --strip-dwo 	 %{c:%{o*:%*}%{!o*:%b%O}}%{!c:%U%O}     }

*asm_options:
%{-target-help:%:print-asm-header()} %{v} %{w:-W} %{I*}  %{gz*:%e-gz is not supported in this configuration} %a %Y %{c:%W{o*}%{!o*:-o %w%b%O}}%{!c:-o %d%w%u%O}

*invoke_as:
%{!fwpa*:   %{fcompare-debug=*|fdump-final-insns=*:%:compare-debug-dump-opt()}   %{!S:-o %|.s |
 as %(asm_options) %m.s %A }  }

*cpp:
%{posix:-D_POSIX_SOURCE} %{pthread:-D_REENTRANT}

*cpp_options:
%(cpp_unique_options) %1 %{m*} %{std*&ansi&trigraphs} %{W*&pedantic*} %{w} %{f*} %{g*:%{!g0:%{g*} %{!fno-working-directory:-fworking-directory}}} %{O*} %{undef} %{save-temps*:-fpch-preprocess}

*cpp_debug_options:
%{d*}

*cpp_unique_options:
%{!Q:-quiet} %{nostdinc*} %{C} %{CC} %{v} %{I*&F*} %{P} %I %{MD:-MD %{!o:%b.d}%{o*:%.d%*}} %{MMD:-MMD %{!o:%b.d}%{o*:%.d%*}} %{M} %{MM} %{MF*} %{MG} %{MP} %{MQ*} %{MT*} %{!E:%{!M:%{!MM:%{!MT:%{!MQ:%{MD|MMD:%{o*:-MQ %*}}}}}}} %{remap} %{g3|ggdb3|gstabs3|gcoff3|gxcoff3|gvms3:-dD} %{!iplugindir*:%{fplugin*:%:find-plugindir()}} %{H} %C %{D*&U*&A*} %{i*} %Z %i %{E|M|MM:%W{o*}}

*trad_capable_cpp:
cc1 -E %{traditional|traditional-cpp:-traditional-cpp}

*cc1:
%{!mandroid|tno-android-cc:%(cc1_cpu) %{profile:-p};:%(cc1_cpu) %{profile:-p} %{!mglibc:%{!muclibc:%{!mbionic: -mbionic}}} %{!fno-pic:%{!fno-PIC:%{!fpic:%{!fPIC: -fPIC}}}}}

*cc1_options:
%{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}} %{!iplugindir*:%{fplugin*:%:find-plugindir()}} %1 %{!Q:-quiet} %{!dumpbase:-dumpbase %B} %{d*} %{m*} %{aux-info*} %{fcompare-debug-second:%:compare-debug-auxbase-opt(%b)}  %{!fcompare-debug-second:%{c|S:%{o*:-auxbase-strip %*}%{!o*:-auxbase %b}}}%{!c:%{!S:-auxbase %b}}  %{g*} %{O*} %{W*&pedantic*} %{w} %{std*&ansi&trigraphs} %{v:-version} %{pg:-p} %{p} %{f*} %{undef} %{Qn:-fno-ident} %{Qy:} %{-help:--help} %{-target-help:--target-help} %{-version:--version} %{-help=*:--help=%*} %{!fsyntax-only:%{S:%W{o*}%{!o*:-o %b.s}}} %{fsyntax-only:-o %j} %{-param*} %{coverage:-fprofile-arcs -ftest-coverage}

*cc1plus:


*link_gcc_c_sequence:
%{static:--start-group} %G %L %{static:--end-group}%{!static:%G}

*link_ssp:
%{fstack-protector|fstack-protector-all|fstack-protector-strong|fstack-protector-explicit:}

*endfile:
%{!mandroid|tno-android-ld:%{Ofast|ffast-math|funsafe-math-optimizations:crtfastmath.o%s}    %{mpc32:crtprec32.o%s}    %{mpc64:crtprec64.o%s}    %{mpc80:crtprec80.o%s} %{fvtable-verify=none:%s;      fvtable-verify=preinit:vtv_end_preinit.o%s;      fvtable-verify=std:vtv_end.o%s}    %{shared:crtendS.o%s;: %{no-pie|static|r|shared:;:crtendS.o%s}    %{no-pie|static:crtend.o%s}} crtn.o%s    ;:%{Ofast|ffast-math|funsafe-math-optimizations:crtfastmath.o%s}    %{mpc32:crtprec32.o%s}    %{mpc64:crtprec64.o%s}    %{mpc80:crtprec80.o%s} %{shared: crtend_so%O%s;: crtend_android%O%s}}

*link:
%{!r:--build-id} %{!static:--eh-frame-hdr} %{!mandroid|tno-android-ld:%{m16|m32|mx32:;:-m elf_x86_64}                    %{m16|m32:-m elf_i386}                    %{mx32:-m elf32_x86_64}   --hash-style=gnu   %{shared:-shared}   %{!shared:     %{!static:       %{rdynamic:-export-dynamic}       %{m16|m32:-dynamic-linker %{muclibc:/lib/ld-uClibc.so.0;:%{mbionic:/system/bin/linker;:%{mmusl:/lib/ld-musl-i386.so.1;:/lib/ld-linux.so.2}}}}       %{m16|m32|mx32:;:-dynamic-linker %{muclibc:/lib/ld64-uClibc.so.0;:%{mbionic:/system/bin/linker64;:%{mmusl:/lib/ld-musl-x86_64.so.1;:/lib64/ld-linux-x86-64.so.2}}}}       %{mx32:-dynamic-linker %{muclibc:/lib/ldx32-uClibc.so.0;:%{mbionic:/system/bin/linkerx32;:%{mmusl:/lib/ld-musl-x32.so.1;:/libx32/ld-linux-x32.so.2}}}}}     %{static:-static}};:%{m16|m32|mx32:;:-m elf_x86_64}                    %{m16|m32:-m elf_i386}                    %{mx32:-m elf32_x86_64}   --hash-style=gnu   %{shared:-shared}   %{!shared:     %{!static:       %{rdynamic:-export-dynamic}       %{m16|m32:-dynamic-linker %{muclibc:/lib/ld-uClibc.so.0;:%{mbionic:/system/bin/linker;:%{mmusl:/lib/ld-musl-i386.so.1;:/lib/ld-linux.so.2}}}}       %{m16|m32|mx32:;:-dynamic-linker %{muclibc:/lib/ld64-uClibc.so.0;:%{mbionic:/system/bin/linker64;:%{mmusl:/lib/ld-musl-x86_64.so.1;:/lib64/ld-linux-x86-64.so.2}}}}       %{mx32:-dynamic-linker %{muclibc:/lib/ldx32-uClibc.so.0;:%{mbionic:/system/bin/linkerx32;:%{mmusl:/lib/ld-musl-x32.so.1;:/libx32/ld-linux-x32.so.2}}}}}     %{static:-static}} %{shared: -Bsymbolic}}

*lib:
%{!mandroid|tno-android-ld:%{pthread:-lpthread} %{shared:-lc}    %{!shared:%{mieee-fp:-lieee} %{profile:-lc_p}%{!profile:-lc}};:%{shared:-lc}    %{!shared:%{mieee-fp:-lieee} %{profile:-lc_p}%{!profile:-lc}} %{!static: -ldl}}

*link_gomp:


*libgcc:
%{static|static-libgcc:-lgcc -lgcc_eh}%{!static:%{!static-libgcc:%{!shared-libgcc:-lgcc --as-needed -lgcc_s --no-as-needed}%{shared-libgcc:-lgcc_s%{!shared: -lgcc}}}}

*startfile:
%{!mandroid|tno-android-ld:%{!shared: %{pg|p|profile:gcrt1.o%s;:     %{no-pie|static|r|shared:;:Scrt1.o%s} %{no-pie|static:crt1.o%s}}}    crti.o%s %{static:crtbeginT.o%s;: %{shared:crtbeginS.o%s} 	      %{no-pie|static|r|shared:;:crtbeginS.o%s} 	      %{no-pie|static:crtbegin.o%s}}    %{fvtable-verify=none:%s;      fvtable-verify=preinit:vtv_start_preinit.o%s;      fvtable-verify=std:vtv_start.o%s}    ;:%{shared: crtbegin_so%O%s;:  %{static: crtbegin_static%O%s;: crtbegin_dynamic%O%s}}}

*cross_compile:
0

*version:
6.2.0

*multilib:
. !m32 !m64 !mx32;32:../lib32:i386-linux-gnu m32 !m64 !mx32;64:../lib:x86_64-linux-gnu !m32 m64 !mx32;x32:../libx32:x86_64-linux-gnux32 !m32 !m64 mx32;

*multilib_defaults:
m64

*multilib_extra:


*multilib_matches:
m32 m32;m64 m64;mx32 mx32;

*multilib_exclusions:


*multilib_options:
m32/m64/mx32

*multilib_reuse:


*linker:
collect2

*linker_plugin_file:


*lto_wrapper:


*lto_gcc:


*post_link:


*link_libgcc:
%D

*md_exec_prefix:


*md_startfile_prefix:


*md_startfile_prefix_1:


*startfile_prefix_spec:


*sysroot_spec:
--sysroot=%R

*sysroot_suffix_spec:


*sysroot_hdrs_suffix_spec:


*self_spec:


*cc1_cpu:
%{march=native:%>march=native %:local_cpu_detect(arch)   %{!mtune=*:%>mtune=native %:local_cpu_detect(tune)}} %{mtune=native:%>mtune=native %:local_cpu_detect(tune)}

*link_command:
%{!fsyntax-only:%{!c:%{!M:%{!MM:%{!E:%{!S:    %(linker) %{!fno-use-linker-plugin:%{!fno-lto:     -plugin %(linker_plugin_file)     -plugin-opt=%(lto_wrapper)     -plugin-opt=-fresolution=%u.res     %{!nostdlib:%{!nodefaultlibs:%:pass-through-libs(%(link_gcc_c_sequence))}}     }}%{flto|flto=*:%<fcompare-debug*}     %{flto} %{fno-lto} %{flto=*} %l %{no-pie:} %{no-pie|static|r|shared:;:-pie} %{fuse-ld=*:-fuse-ld=%*}  %{gz*:%e-gz is not supported in this configuration} %X %{o*} %{e*} %{N} %{n} %{r}    %{s} %{t} %{u*} %{z} %{Z} %{!nostdlib:%{!nostartfiles:%S}}     %{static:} %{L*} %(mfwrap) %(link_libgcc) %{!nostdlib:%{fvtable-verify=std: -lvtv -u_vtable_map_vars_start -u_vtable_map_vars_end}    %{fvtable-verify=preinit: -lvtv -u_vtable_map_vars_start -u_vtable_map_vars_end}} %{!nostdlib:%{!nodefaultlibs:%{%:sanitize(address):%{!shared:libasan_preinit%O%s} %{static-libasan:%{!shared:-Bstatic --whole-archive -lasan --no-whole-archive -Bdynamic}}%{!static-libasan:-lasan}}     %{%:sanitize(thread):%{static-libtsan:%{!shared:-Bstatic --whole-archive -ltsan --no-whole-archive -Bdynamic}}%{!static-libtsan:-ltsan}}     %{%:sanitize(leak):%{static-liblsan:%{!shared:-Bstatic --whole-archive -llsan --no-whole-archive -Bdynamic}}%{!static-liblsan:-llsan}}}} %o %{!nostdlib:%{!nodefaultlibs:%{mmpx:%{fcheck-pointer-bounds:    %{static:--whole-archive -lmpx --no-whole-archive %:include(libmpx.spec)%(link_libmpx)}    %{!static:%{static-libmpx:-Bstatic --whole-archive}    -lmpx %{static-libmpx:--no-whole-archive -Bdynamic %:include(libmpx.spec)%(link_libmpx)}}}}%{mmpx:%{fcheck-pointer-bounds:%{!fno-chkp-use-wrappers:    %{static:-lmpxwrappers}    %{!static:%{static-libmpxwrappers:-Bstatic --whole-archive}    -lmpxwrappers %{static-libmpxwrappers:--no-whole-archive -Bdynamic}}}}}}} %{mmpx:%{fcheck-pointer-bounds:%{!static:%{m16|m32|mx32:;:-z bndplt }}}}     %{fopenacc|fopenmp|%:gt(%{ftree-parallelize-loops=*:%*} 1):	%:include(libgomp.spec)%(link_gomp)}    %{fcilkplus:%:include(libcilkrts.spec)%(link_cilkrts)}    %{fgnu-tm:%:include(libitm.spec)%(link_itm)}    %(mflib)  %{fsplit-stack: --wrap=pthread_create}    %{fprofile-arcs|fprofile-generate*|coverage:-lgcov} %{!nostdlib:%{!nodefaultlibs:%{%:sanitize(address): %{static-libasan:%:include(libsanitizer.spec)%(link_libasan)}    %{static:%ecannot specify -static with -fsanitize=address}}    %{%:sanitize(thread): %{static-libtsan:%:include(libsanitizer.spec)%(link_libtsan)}    %{static:%ecannot specify -static with -fsanitize=thread}}    %{%:sanitize(undefined):%{static-libubsan:-Bstatic} -lubsan %{static-libubsan:-Bdynamic} %{static-libubsan:%:include(libsanitizer.spec)%(link_libubsan)}}    %{%:sanitize(leak): %{static-liblsan:%:include(libsanitizer.spec)%(link_liblsan)}}}}     %{!nostdlib:%{!nodefaultlibs:%(link_ssp) %(link_gcc_c_sequence)}}    %{!nostdlib:%{!nostartfiles:%E}} %{T*}  
%(post_link) }}}}}}
```

> you can send that privately if you wish. I'd like to see how they enforce the "pie" stuff. That may enlighten me.
> 
> We know empirically that the fix works. We just don't know why and that annoys me deeply.



---

archive/issue_comments_323019.json:
```json
{
    "body": "Changed commit from **[1833b35](https://github.com/sagemath/sagetrac-mirror/commit/1833b35e8f1184145f321834bcd752b1b202a220)** to **[96442c6](https://github.com/sagemath/sagetrac-mirror/commit/96442c608b0dd83810aaf9f7eac925d859457747)**",
    "created_at": "2016-11-08T21:17:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323019",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1833b35](https://github.com/sagemath/sagetrac-mirror/commit/1833b35e8f1184145f321834bcd752b1b202a220)** to **[96442c6](https://github.com/sagemath/sagetrac-mirror/commit/96442c608b0dd83810aaf9f7eac925d859457747)**



---

archive/issue_comments_323020.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/96442c608b0dd83810aaf9f7eac925d859457747\">96442c6</a></td><td><code>Original author's name received.</code></td></tr></table>\n",
    "created_at": "2016-11-08T21:17:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323020",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:19'>Comment 19:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/96442c608b0dd83810aaf9f7eac925d859457747">96442c6</a></td><td><code>Original author's name received.</code></td></tr></table>




---

archive/issue_comments_323021.json:
```json
{
    "body": "Changed author from **J\u00f6rg-Volker, Emmanuel Charpentier** to **J\u00f6rg-Volker Peetz, Emmanuel Charpentier**",
    "created_at": "2016-11-08T21:18:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323021",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

Changed author from **Jörg-Volker, Emmanuel Charpentier** to **Jörg-Volker Peetz, Emmanuel Charpentier**



---

archive/issue_comments_323022.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nOriginal author's name added.",
    "created_at": "2016-11-08T21:18:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323022",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

<a id='comment:20'>Comment 20:</a>
Original author's name added.



---

archive/issue_comments_323023.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nNeeds to go to the bots to figure out if it breaks something on other distro/compilers. But otherwise look in shape.",
    "created_at": "2016-11-08T21:25:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323023",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:21'>Comment 21:</a>
Needs to go to the bots to figure out if it breaks something on other distro/compilers. But otherwise look in shape.



---

archive/issue_events_287637.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-11-08T21:25:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287637"
}
```



---

archive/issue_events_287638.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-11-08T21:25:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287638"
}
```



---

archive/issue_comments_323024.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer, Fran\u00e7ois Bissey**",
    "created_at": "2016-11-08T21:25:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323024",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **Jeroen Demeyer, François Bissey**



---

archive/issue_events_287639.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-11-09T18:16:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287639"
}
```



---

archive/issue_events_287640.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1d99ec385e555431464a4f2a5c82b5a5318535c9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-11-09T18:16:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21782#event-287640"
}
```



---

archive/issue_comments_323025.json:
```json
{
    "body": "Changed branch from **[u/charpent/fix-flint](https://github.com/sagemath/sagetrac-mirror/tree/u/charpent/fix-flint)** to **[96442c6](https://github.com/sagemath/sagetrac-mirror/commit/96442c608b0dd83810aaf9f7eac925d859457747)**",
    "created_at": "2016-11-09T18:16:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21782",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21782#issuecomment-323025",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/charpent/fix-flint](https://github.com/sagemath/sagetrac-mirror/tree/u/charpent/fix-flint)** to **[96442c6](https://github.com/sagemath/sagetrac-mirror/commit/96442c608b0dd83810aaf9f7eac925d859457747)**
