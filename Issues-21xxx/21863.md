# Issue 21863: Implement colorings of knots and links

archive/issues_021626.json:
```json
{
    "assignees": [],
    "body": "This ticket implements methods to check n-colorability of knots and links ([1]) and adapts the plot method to allow to plot the link with the corresponding coloring.\n\nIt deppends on the `.arcs()` method, implemented in #21815\n\n[1]: https://en.wikipedia.org/wiki/Fox_n-coloring\n\n\n\nCC:  @kcrisman @amitjamadagni @sagetrac-fugelde @tscrim @jdemeyer\n\nKeywords: **knot, link**\n\nBranch/Commit: **[bc9412a](https://github.com/sagemath/sagetrac-mirror/commit/bc9412a86b346cabdc7a076f85d37069f17578e0)**\n\nReviewer: **Miguel Marco, Fr\u00e9d\u00e9ric Chapoton, Travis Scrimshaw**\n\nAuthor: **Miguel Marco, Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21863_\n\n",
    "closed_at": "2019-03-25T19:44:02Z",
    "created_at": "2016-11-11T19:40:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20topology",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement colorings of knots and links",
    "type": "issue",
    "updated_at": "2019-03-25T19:44:02Z",
    "url": "https://github.com/sagemath/sage/issues/21863",
    "user": "https://github.com/miguelmarco"
}
```
This ticket implements methods to check n-colorability of knots and links ([1]) and adapts the plot method to allow to plot the link with the corresponding coloring.

It deppends on the `.arcs()` method, implemented in #21815

[1]: https://en.wikipedia.org/wiki/Fox_n-coloring



CC:  @kcrisman @amitjamadagni @sagetrac-fugelde @tscrim @jdemeyer

Keywords: **knot, link**

Branch/Commit: **[bc9412a](https://github.com/sagemath/sagetrac-mirror/commit/bc9412a86b346cabdc7a076f85d37069f17578e0)**

Reviewer: **Miguel Marco, Frédéric Chapoton, Travis Scrimshaw**

Author: **Miguel Marco, Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/21863_





---

archive/issue_events_288632.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2016-11-11T19:40:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288632"
}
```



---

archive/issue_events_288633.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2016-11-11T19:40:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20topology",
    "label_color": "0000ff",
    "label_name": "component: algebraic topology",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288633"
}
```



---

archive/issue_events_288634.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2016-11-11T19:40:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288634"
}
```



---

archive/issue_events_288635.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2016-11-11T19:40:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288635"
}
```



---

archive/issue_comments_324304.json:
```json
{
    "body": "Dependencies: **#21815**",
    "created_at": "2016-11-11T19:42:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324304",
    "user": "https://github.com/miguelmarco"
}
```

Dependencies: **#21815**



---

archive/issue_comments_324305.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,6 @@\n This ticket implements methods to check n-colorability of knots and links ([1]) and adapts the plot method to allow to plot the link with the corresponding coloring.\n \n+It deppends on the `.arcs()` method, implemented in #21815\n \n [1]: https://en.wikipedia.org/wiki/Fox_n-coloring\n \n``````\n",
    "created_at": "2016-11-11T19:42:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324305",
    "user": "https://github.com/miguelmarco"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,6 @@
 This ticket implements methods to check n-colorability of knots and links ([1]) and adapts the plot method to allow to plot the link with the corresponding coloring.
 
+It deppends on the `.arcs()` method, implemented in #21815
 
 [1]: https://en.wikipedia.org/wiki/Fox_n-coloring
 
``````




---

archive/issue_comments_324306.json:
```json
{
    "body": "Branch: **[u/mmarco/knot-colorings](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/knot-colorings)**",
    "created_at": "2016-11-11T19:42:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324306",
    "user": "https://github.com/miguelmarco"
}
```

Branch: **[u/mmarco/knot-colorings](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/knot-colorings)**



---

archive/issue_comments_324307.json:
```json
{
    "body": "Commit: **[f6483a5](https://github.com/sagemath/sagetrac-mirror/commit/f6483a578404365475b7ec2b624ebaf6c7f9072a)**",
    "created_at": "2019-03-15T20:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324307",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[f6483a5](https://github.com/sagemath/sagetrac-mirror/commit/f6483a578404365475b7ec2b624ebaf6c7f9072a)**



---

archive/issue_events_288636.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-15T20:42:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288636"
}
```



---

archive/issue_events_288637.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-15T20:42:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288637"
}
```



---

archive/issue_events_288638.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-15T20:42:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288638"
}
```



---

archive/issue_comments_324308.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nrebased and refreshed, this seems to be good to go\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ea0358d3a312572a7baad8adcf65c3eaf13e5d31\">ea0358d</a></td><td><code>Implement knot collorings</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f6483a578404365475b7ec2b624ebaf6c7f9072a\">f6483a5</a></td><td><code>Merge branch 'u/mmarco/knot-colorings' in 8.7.b7</code></td></tr></table>\n",
    "created_at": "2019-03-15T20:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324308",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

rebased and refreshed, this seems to be good to go

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ea0358d3a312572a7baad8adcf65c3eaf13e5d31">ea0358d</a></td><td><code>Implement knot collorings</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f6483a578404365475b7ec2b624ebaf6c7f9072a">f6483a5</a></td><td><code>Merge branch 'u/mmarco/knot-colorings' in 8.7.b7</code></td></tr></table>




---

archive/issue_comments_324309.json:
```json
{
    "body": "Changed dependencies from **#21815** to none",
    "created_at": "2019-03-15T20:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324309",
    "user": "https://github.com/fchapoton"
}
```

Changed dependencies from **#21815** to none



---

archive/issue_comments_324310.json:
```json
{
    "body": "Changed branch from **[u/mmarco/knot-colorings](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/knot-colorings)** to **[public/ticket/21863](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/21863)**",
    "created_at": "2019-03-15T20:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324310",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/mmarco/knot-colorings](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/knot-colorings)** to **[public/ticket/21863](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/21863)**



---

archive/issue_comments_324311.json:
```json
{
    "body": "Changed commit from **[f6483a5](https://github.com/sagemath/sagetrac-mirror/commit/f6483a578404365475b7ec2b624ebaf6c7f9072a)** to **[caaac73](https://github.com/sagemath/sagetrac-mirror/commit/caaac73552cee4cfad02d8be922bb70fb33f98c9)**",
    "created_at": "2019-03-16T07:00:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324311",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f6483a5](https://github.com/sagemath/sagetrac-mirror/commit/f6483a578404365475b7ec2b624ebaf6c7f9072a)** to **[caaac73](https://github.com/sagemath/sagetrac-mirror/commit/caaac73552cee4cfad02d8be922bb70fb33f98c9)**



---

archive/issue_comments_324312.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/caaac73552cee4cfad02d8be922bb70fb33f98c9\">caaac73</a></td><td><code>trac 21863 fix some details in colored knots</code></td></tr></table>\n",
    "created_at": "2019-03-16T07:00:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324312",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/caaac73552cee4cfad02d8be922bb70fb33f98c9">caaac73</a></td><td><code>trac 21863 fix some details in colored knots</code></td></tr></table>




---

archive/issue_comments_324313.json:
```json
{
    "body": "Changed commit from **[caaac73](https://github.com/sagemath/sagetrac-mirror/commit/caaac73552cee4cfad02d8be922bb70fb33f98c9)** to **[015455e](https://github.com/sagemath/sagetrac-mirror/commit/015455e3e7f4c7461128f51e49ac07352d603d70)**",
    "created_at": "2019-03-16T08:55:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324313",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[caaac73](https://github.com/sagemath/sagetrac-mirror/commit/caaac73552cee4cfad02d8be922bb70fb33f98c9)** to **[015455e](https://github.com/sagemath/sagetrac-mirror/commit/015455e3e7f4c7461128f51e49ac07352d603d70)**



---

archive/issue_comments_324314.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/015455e3e7f4c7461128f51e49ac07352d603d70\">015455e</a></td><td><code>trac 21863 some fix of doctest for py3 compatibility</code></td></tr></table>\n",
    "created_at": "2019-03-16T08:55:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324314",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/015455e3e7f4c7461128f51e49ac07352d603d70">015455e</a></td><td><code>trac 21863 some fix of doctest for py3 compatibility</code></td></tr></table>




---

archive/issue_comments_324315.json:
```json
{
    "body": "Changed commit from **[015455e](https://github.com/sagemath/sagetrac-mirror/commit/015455e3e7f4c7461128f51e49ac07352d603d70)** to **[7b5a69b](https://github.com/sagemath/sagetrac-mirror/commit/7b5a69b842f49db4807f4fde2a656ba40b3812eb)**",
    "created_at": "2019-03-16T12:10:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324315",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[015455e](https://github.com/sagemath/sagetrac-mirror/commit/015455e3e7f4c7461128f51e49ac07352d603d70)** to **[7b5a69b](https://github.com/sagemath/sagetrac-mirror/commit/7b5a69b842f49db4807f4fde2a656ba40b3812eb)**



---

archive/issue_comments_324316.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7b5a69b842f49db4807f4fde2a656ba40b3812eb\">7b5a69b</a></td><td><code>trac 21863 fix mistake</code></td></tr></table>\n",
    "created_at": "2019-03-16T12:10:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324316",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7b5a69b842f49db4807f4fde2a656ba40b3812eb">7b5a69b</a></td><td><code>trac 21863 fix mistake</code></td></tr></table>




---

archive/issue_comments_324317.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks for the revival. If all test pass, would you agree on a positive review?",
    "created_at": "2019-03-16T12:17:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324317",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks for the revival. If all test pass, would you agree on a positive review?



---

archive/issue_comments_324318.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nWell, almost.\n\n- I am not fully happy with the test for type \"str\" to check if the color is a color or a coloring. Maybe we should rather check for type \"dict\" to recognize colorings ?\n\n- I would still like to add a reference to [Livi1993] (which gives a nice presentation of knots colorings)",
    "created_at": "2019-03-16T12:51:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324318",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">Comment 8</div>

Well, almost.

- I am not fully happy with the test for type "str" to check if the color is a color or a coloring. Maybe we should rather check for type "dict" to recognize colorings ?

- I would still like to add a reference to [Livi1993] (which gives a nice presentation of knots colorings)



---

archive/issue_comments_324319.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nFine. I will try to work on it during the weekend.",
    "created_at": "2019-03-16T13:07:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324319",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:9" align="right">Comment 9</div>

Fine. I will try to work on it during the weekend.



---

archive/issue_comments_324320.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3cfb2d6be7a5cf01d998eee62a8994f70bb0aaa1\">3cfb2d6</a></td><td><code>trac 21863 last minute fixes</code></td></tr></table>\n",
    "created_at": "2019-03-16T17:17:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324320",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3cfb2d6be7a5cf01d998eee62a8994f70bb0aaa1">3cfb2d6</a></td><td><code>trac 21863 last minute fixes</code></td></tr></table>




---

archive/issue_comments_324321.json:
```json
{
    "body": "Changed commit from **[7b5a69b](https://github.com/sagemath/sagetrac-mirror/commit/7b5a69b842f49db4807f4fde2a656ba40b3812eb)** to **[3cfb2d6](https://github.com/sagemath/sagetrac-mirror/commit/3cfb2d6be7a5cf01d998eee62a8994f70bb0aaa1)**",
    "created_at": "2019-03-16T17:17:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324321",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7b5a69b](https://github.com/sagemath/sagetrac-mirror/commit/7b5a69b842f49db4807f4fde2a656ba40b3812eb)** to **[3cfb2d6](https://github.com/sagemath/sagetrac-mirror/commit/3cfb2d6be7a5cf01d998eee62a8994f70bb0aaa1)**



---

archive/issue_comments_324322.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nI have made the changes that I wanted to do. I will now launch my bot. If it comes back green, it will be ready for your approval.",
    "created_at": "2019-03-16T17:18:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324322",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:11" align="right">Comment 11</div>

I have made the changes that I wanted to do. I will now launch my bot. If it comes back green, it will be ready for your approval.



---

archive/issue_comments_324323.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBot is green. I am ok with the code. If you too, please set to positive.",
    "created_at": "2019-03-16T21:11:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324323",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:12" align="right">Comment 12</div>

Bot is green. I am ok with the code. If you too, please set to positive.



---

archive/issue_comments_324324.json:
```json
{
    "body": "Changed author from **Miguel Marco** to **Miguel Marco, Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-03-16T21:11:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324324",
    "user": "https://github.com/fchapoton"
}
```

Changed author from **Miguel Marco** to **Miguel Marco, Frédéric Chapoton**



---

archive/issue_comments_324325.json:
```json
{
    "body": "Reviewer: **Miguel Marco, Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-03-16T21:11:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324325",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Miguel Marco, Frédéric Chapoton**



---

archive/issue_comments_324326.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nNow that I look at it, I am not completely sure that the result of `is_colorable` is mathematically correct in general: it relies on `coloring_matrix`, which takes the next prime of the passed number. So in the case of prime numbers, that is correct, but for non-prime, we are actually saying if it is p-colorable for the next prime p.\n\nMaybe it is ok if we just calrify this in the documentation? Or should we write more complete code to check that there actually exist solutions over the integers mod n for non prime n?",
    "created_at": "2019-03-16T21:36:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324326",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:13" align="right">Comment 13</div>

Now that I look at it, I am not completely sure that the result of `is_colorable` is mathematically correct in general: it relies on `coloring_matrix`, which takes the next prime of the passed number. So in the case of prime numbers, that is correct, but for non-prime, we are actually saying if it is p-colorable for the next prime p.

Maybe it is ok if we just calrify this in the documentation? Or should we write more complete code to check that there actually exist solutions over the integers mod n for non prime n?



---

archive/issue_comments_324327.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nI also have one doc nitpick: `n` should be in code formatting.",
    "created_at": "2019-03-16T23:29:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324327",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:14" align="right">Comment 14</div>

I also have one doc nitpick: `n` should be in code formatting.



---

archive/issue_comments_324328.json:
```json
{
    "body": "Changed commit from **[3cfb2d6](https://github.com/sagemath/sagetrac-mirror/commit/3cfb2d6be7a5cf01d998eee62a8994f70bb0aaa1)** to **[27548b0](https://github.com/sagemath/sagetrac-mirror/commit/27548b069e7f98f08471816aa6294a452d4fcd56)**",
    "created_at": "2019-03-17T07:10:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324328",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3cfb2d6](https://github.com/sagemath/sagetrac-mirror/commit/3cfb2d6be7a5cf01d998eee62a8994f70bb0aaa1)** to **[27548b0](https://github.com/sagemath/sagetrac-mirror/commit/27548b069e7f98f08471816aa6294a452d4fcd56)**



---

archive/issue_comments_324329.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/27548b069e7f98f08471816aa6294a452d4fcd56\">27548b0</a></td><td><code>trac 21863 polishing the details</code></td></tr></table>\n",
    "created_at": "2019-03-17T07:10:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324329",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:15" align="right">Comment 15</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/27548b069e7f98f08471816aa6294a452d4fcd56">27548b0</a></td><td><code>trac 21863 polishing the details</code></td></tr></table>




---

archive/issue_comments_324330.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nok, done",
    "created_at": "2019-03-17T07:15:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324330",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:16" align="right">Comment 16</div>

ok, done



---

archive/issue_comments_324331.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nI am happy with things as the added documentation does clarify what Sage does, but I want to wait to see if Miguel has any additional comments.",
    "created_at": "2019-03-18T09:57:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324331",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:17" align="right">Comment 17</div>

I am happy with things as the added documentation does clarify what Sage does, but I want to wait to see if Miguel has any additional comments.



---

archive/issue_comments_324332.json:
```json
{
    "body": "Changed reviewer from **Miguel Marco, Fr\u00e9d\u00e9ric Chapoton** to **Miguel Marco, Fr\u00e9d\u00e9ric Chapoton, Travis Scrimshaw**",
    "created_at": "2019-03-18T09:57:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324332",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Miguel Marco, Frédéric Chapoton** to **Miguel Marco, Frédéric Chapoton, Travis Scrimshaw**



---

archive/issue_comments_324333.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bc9412a86b346cabdc7a076f85d37069f17578e0\">bc9412a</a></td><td><code>Use next_prime also in colorings</code></td></tr></table>\n",
    "created_at": "2019-03-18T14:42:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324333",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:18" align="right">Comment 18</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bc9412a86b346cabdc7a076f85d37069f17578e0">bc9412a</a></td><td><code>Use next_prime also in colorings</code></td></tr></table>




---

archive/issue_comments_324334.json:
```json
{
    "body": "Changed commit from **[27548b0](https://github.com/sagemath/sagetrac-mirror/commit/27548b069e7f98f08471816aa6294a452d4fcd56)** to **[bc9412a](https://github.com/sagemath/sagetrac-mirror/commit/bc9412a86b346cabdc7a076f85d37069f17578e0)**",
    "created_at": "2019-03-18T14:42:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324334",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[27548b0](https://github.com/sagemath/sagetrac-mirror/commit/27548b069e7f98f08471816aa6294a452d4fcd56)** to **[bc9412a](https://github.com/sagemath/sagetrac-mirror/commit/bc9412a86b346cabdc7a076f85d37069f17578e0)**



---

archive/issue_comments_324335.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nI added a check for next prime also in `colorings`, so it is now totally clear that we only cover the case of prime number of colors. Maybe we should support also non prime number of colors, but that requires slightly more sophisticated algorithms, so it should be better done in another ticket.\n\nIf you are ok with these last changes, positive review.",
    "created_at": "2019-03-18T14:45:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324335",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:19" align="right">Comment 19</div>

I added a check for next prime also in `colorings`, so it is now totally clear that we only cover the case of prime number of colors. Maybe we should support also non prime number of colors, but that requires slightly more sophisticated algorithms, so it should be better done in another ticket.

If you are ok with these last changes, positive review.



---

archive/issue_events_288639.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-18T17:59:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288639"
}
```



---

archive/issue_events_288640.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-18T17:59:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288640"
}
```



---

archive/issue_comments_324336.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nok, good to go.",
    "created_at": "2019-03-18T17:59:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324336",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:20" align="right">Comment 20</div>

ok, good to go.



---

archive/issue_comments_324337.json:
```json
{
    "body": "Changed branch from **[public/ticket/21863](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/21863)** to **[bc9412a](https://github.com/sagemath/sagetrac-mirror/commit/bc9412a86b346cabdc7a076f85d37069f17578e0)**",
    "created_at": "2019-03-25T19:44:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21863#issuecomment-324337",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/ticket/21863](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/21863)** to **[bc9412a](https://github.com/sagemath/sagetrac-mirror/commit/bc9412a86b346cabdc7a076f85d37069f17578e0)**



---

archive/issue_events_288641.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-03-25T19:44:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288641"
}
```



---

archive/issue_events_288642.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c9edc89b09123eb38533f13dc71008c4c08fd80e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-03-25T19:44:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21863",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21863#event-288642"
}
```
