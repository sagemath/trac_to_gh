# Issue 21273: Clean up of mpfi interface and rings/real_mpfi.pyx

archive/issues_021036.json:
```json
{
    "assignees": [],
    "body": "The current `sage/libs/mpfi.pxd` files defines some mpfi function that are for internal use only and are not defined in mpfi.h anymore. Two of these functions are then used in `sage/rings/real_mpfi.pyx`.\n\nSpotted from QA warnings in sage-on-gentoo.\n\nBranch/Commit: **[127d113](https://github.com/sagemath/sagetrac-mirror/commit/127d1139608e1404d281d03f781d31bdb9082759)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Fran\u00e7ois Bissey**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/21273_\n\n",
    "closed_at": "2016-09-04T00:13:55Z",
    "created_at": "2016-08-18T00:43:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean up of mpfi interface and rings/real_mpfi.pyx",
    "type": "issue",
    "updated_at": "2016-09-04T00:13:55Z",
    "url": "https://github.com/sagemath/sage/issues/21273",
    "user": "https://github.com/kiwifb"
}
```
The current `sage/libs/mpfi.pxd` files defines some mpfi function that are for internal use only and are not defined in mpfi.h anymore. Two of these functions are then used in `sage/rings/real_mpfi.pyx`.

Spotted from QA warnings in sage-on-gentoo.

Branch/Commit: **[127d113](https://github.com/sagemath/sagetrac-mirror/commit/127d1139608e1404d281d03f781d31bdb9082759)**

Reviewer: **Jeroen Demeyer**

Author: **François Bissey**

_Issue created by migration from https://trac.sagemath.org/ticket/21273_





---

archive/issue_events_281165.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-08-18T00:43:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21273#event-281165"
}
```



---

archive/issue_events_281166.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-08-18T00:43:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21273#event-281166"
}
```



---

archive/issue_events_281167.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-08-18T00:43:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21273#event-281167"
}
```



---

archive/issue_events_281168.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-08-18T00:43:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21273#event-281168"
}
```



---

archive/issue_events_281169.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-08-18T00:43:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21273#event-281169"
}
```



---

archive/issue_comments_313737.json:
```json
{
    "body": "Changed commit from **[7b1b3ec](https://github.com/sagemath/sagetrac-mirror/commit/7b1b3ec5b6042c57a1d1af4d17e8c081b900572f)** to **[dc150c8](https://github.com/sagemath/sagetrac-mirror/commit/dc150c849f6ef7a6d94bdf5ffff5c38252ee3047)**",
    "created_at": "2016-08-18T00:53:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21273#issuecomment-313737",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7b1b3ec](https://github.com/sagemath/sagetrac-mirror/commit/7b1b3ec5b6042c57a1d1af4d17e8c081b900572f)** to **[dc150c8](https://github.com/sagemath/sagetrac-mirror/commit/dc150c849f6ef7a6d94bdf5ffff5c38252ee3047)**



---

archive/issue_comments_313738.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dc150c849f6ef7a6d94bdf5ffff5c38252ee3047\">dc150c8</a></td><td><code>Clean mpfi interface and its use in rings</code></td></tr></table>\n",
    "created_at": "2016-08-18T00:53:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21273#issuecomment-313738",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dc150c849f6ef7a6d94bdf5ffff5c38252ee3047">dc150c8</a></td><td><code>Clean mpfi interface and its use in rings</code></td></tr></table>




---

archive/issue_comments_313739.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/127d1139608e1404d281d03f781d31bdb9082759\">127d113</a></td><td><code>Merge branch 'develop' into mpfi_cleanup</code></td></tr></table>\n",
    "created_at": "2016-08-30T21:16:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21273#issuecomment-313739",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/127d1139608e1404d281d03f781d31bdb9082759">127d113</a></td><td><code>Merge branch 'develop' into mpfi_cleanup</code></td></tr></table>




---

archive/issue_comments_313740.json:
```json
{
    "body": "Changed commit from **[dc150c8](https://github.com/sagemath/sagetrac-mirror/commit/dc150c849f6ef7a6d94bdf5ffff5c38252ee3047)** to **[127d113](https://github.com/sagemath/sagetrac-mirror/commit/127d1139608e1404d281d03f781d31bdb9082759)**",
    "created_at": "2016-08-30T21:16:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21273#issuecomment-313740",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[dc150c8](https://github.com/sagemath/sagetrac-mirror/commit/dc150c849f6ef7a6d94bdf5ffff5c38252ee3047)** to **[127d113](https://github.com/sagemath/sagetrac-mirror/commit/127d1139608e1404d281d03f781d31bdb9082759)**



---

archive/issue_comments_313741.json:
```json
{
    "body": "Changed author from **Francois Bissey** to **Fran\u00e7ois Bissey**",
    "created_at": "2016-09-01T16:02:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21273#issuecomment-313741",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Francois Bissey** to **François Bissey**



---

archive/issue_events_281170.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-09-01T16:02:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21273#event-281170"
}
```



---

archive/issue_events_281171.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-09-01T16:02:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21273#event-281171"
}
```



---

archive/issue_comments_313742.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2016-09-01T16:02:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21273#issuecomment-313742",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_281172.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-09-04T00:13:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21273#event-281172"
}
```



---

archive/issue_events_281173.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "532cad971d24c9d85db4d6703f927b9fe1c6ed34",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-09-04T00:13:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21273#event-281173"
}
```



---

archive/issue_comments_313743.json:
```json
{
    "body": "Changed branch from **[u/fbissey/mpfi_cleanup](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/mpfi_cleanup)** to **[127d113](https://github.com/sagemath/sagetrac-mirror/commit/127d1139608e1404d281d03f781d31bdb9082759)**",
    "created_at": "2016-09-04T00:13:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/21273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21273#issuecomment-313743",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/fbissey/mpfi_cleanup](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/mpfi_cleanup)** to **[127d113](https://github.com/sagemath/sagetrac-mirror/commit/127d1139608e1404d281d03f781d31bdb9082759)**
