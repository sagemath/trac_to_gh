# Issue 21510: trouble with sage_started.txt

archive/issues_021273.json:
```json
{
    "body": "some file, `sage_started.txt` is falling on our feet in #15105.\n\nthe file is created by the build process in SAGE_ETC, which possibly is not writable at that time.\n\nalso, the file seems to have something to do with the first run of the `sage` script. the first run must be possible to happen before installing anything (such as in `make check`).\n\nit seems that we need a sort of wrapper to implement a functional pre-install `sage`. this wrapper could also handle sage_started, or disable the mechanism.\n\nCC:  @jdemeyer @kiwifb @mkoeppe\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/21510\n\n",
    "closed_at": "2017-09-05T13:09:47Z",
    "created_at": "2016-09-16T23:03:41Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "trouble with sage_started.txt",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/21510",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```
some file, `sage_started.txt` is falling on our feet in #15105.

the file is created by the build process in SAGE_ETC, which possibly is not writable at that time.

also, the file seems to have something to do with the first run of the `sage` script. the first run must be possible to happen before installing anything (such as in `make check`).

it seems that we need a sort of wrapper to implement a functional pre-install `sage`. this wrapper could also handle sage_started, or disable the mechanism.

CC:  @jdemeyer @kiwifb @mkoeppe

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/21510





---

archive/issue_comments_396383.json:
```json
{
    "body": "<a id='comment:1'></a>What exactly is the problem to be fixed here? \"foo is falling on our feet\" is not very descriptive. You really need to give more details. Also, when you say things like \"`SAGE_ETC`, which possibly is not writable at that time.\" you should justify that.",
    "created_at": "2016-10-03T12:40:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21510#issuecomment-396383",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>What exactly is the problem to be fixed here? "foo is falling on our feet" is not very descriptive. You really need to give more details. Also, when you say things like "`SAGE_ETC`, which possibly is not writable at that time." you should justify that.



---

archive/issue_comments_396384.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,8 +6,6 @@\n \n it seems that we need a sort of wrapper to implement a functional pre-install `sage`. this wrapper could also handle sage_started, or disable the mechanism.\n \n-(component should be \"sagelib\" but that is missing in the drop down. choosing \"build\").\n-\n Comment: 1\n \n Issue created by migration from https://trac.sagemath.org/ticket/21510\n``````\n",
    "created_at": "2016-10-03T12:41:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21510#issuecomment-396384",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,8 +6,6 @@
 
 it seems that we need a sort of wrapper to implement a functional pre-install `sage`. this wrapper could also handle sage_started, or disable the mechanism.
 
-(component should be "sagelib" but that is missing in the drop down. choosing "build").
-
 Comment: 1
 
 Issue created by migration from https://trac.sagemath.org/ticket/21510
``````




---

archive/issue_comments_396385.json:
```json
{
    "body": "<a id='comment:3'></a>The description should be clarified to distinguish the following issues:\n- What is `sage_started.txt`?\n- Does it belong into `SAGE_LOCAL` (= `$prefix` by #21479) or rather into the build directory (SAGE_ROOT, distinguished in VPATH builds (#21469) from SAGE_SRC_ROOT)?\n- If it does belong into `SAGE_LOCAL`, then if `SAGE_LOCAL` is only root-writable, then SAGE_SUDO needs to be invoked (#21537).\n- Beyond that, any discussion of \"separating `make` and `make install`\" should please go to ticket #21495, not here.",
    "created_at": "2016-10-03T18:14:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21510#issuecomment-396385",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'></a>The description should be clarified to distinguish the following issues:
- What is `sage_started.txt`?
- Does it belong into `SAGE_LOCAL` (= `$prefix` by #21479) or rather into the build directory (SAGE_ROOT, distinguished in VPATH builds (#21469) from SAGE_SRC_ROOT)?
- If it does belong into `SAGE_LOCAL`, then if `SAGE_LOCAL` is only root-writable, then SAGE_SUDO needs to be invoked (#21537).
- Beyond that, any discussion of "separating `make` and `make install`" should please go to ticket #21495, not here.



---

archive/issue_comments_396386.json:
```json
{
    "body": "<a id='comment:4'></a>Closing because the issue isn't clear and no reply was given by the submitter after ample time.",
    "created_at": "2017-09-05T13:09:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21510#issuecomment-396386",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>Closing because the issue isn't clear and no reply was given by the submitter after ample time.



---

archive/issue_events_057103.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-05T13:09:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/21510",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/21510#event-57103"
}
```



---

archive/issue_comments_396387.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2017-09-05T13:09:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/21510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/21510#issuecomment-396387",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: invalid



---

archive/issue_events_057104.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-05T13:09:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/21510",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/21510#event-57104"
}
```
