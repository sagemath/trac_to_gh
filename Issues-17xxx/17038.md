# Issue 17038: deprecated_callable_import should mention the deprecation in __doc__

archive/issues_016801.json:
```json
{
    "assignees": [],
    "body": "With this branch, the doc of `WittDesign` changes from:\n\n```\n   INPUT:\n\n   * \"n\" is in 9,10,11,12,21,22,23,24.\n\n   Wraps GAP Design's WittDesign. If \"n=24\" then this function returns\n   the large Witt design W_{24}, the unique (up to isomorphism)\n...\n```\n\nto\n\n```\nThis method soon will not be available in that way anymore. To use it,\nyou can now call it by typing designs.WittDesign\n```\n\nRelated sage-devel thread at https://groups.google.com/d/topic/sage-devel/tD8tHOp3q1I/discussion\n\nNathann\n\nCC:  @tscrim @vbraun @williamstein\n\nBranch/Commit: **[869953f](https://github.com/sagemath/sagetrac-mirror/commit/869953f280c819912ed56d44e8c22517a8b732b4)**\n\nReviewer: **Punarbasu Purkayastha**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17038_\n\n",
    "closed_at": "2014-10-02T16:21:51Z",
    "created_at": "2014-09-25T10:03:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "deprecated_callable_import should mention the deprecation in __doc__",
    "type": "issue",
    "updated_at": "2014-10-02T16:21:51Z",
    "url": "https://github.com/sagemath/sage/issues/17038",
    "user": "https://github.com/nathanncohen"
}
```
With this branch, the doc of `WittDesign` changes from:

```
   INPUT:

   * "n" is in 9,10,11,12,21,22,23,24.

   Wraps GAP Design's WittDesign. If "n=24" then this function returns
   the large Witt design W_{24}, the unique (up to isomorphism)
...
```

to

```
This method soon will not be available in that way anymore. To use it,
you can now call it by typing designs.WittDesign
```

Related sage-devel thread at https://groups.google.com/d/topic/sage-devel/tD8tHOp3q1I/discussion

Nathann

CC:  @tscrim @vbraun @williamstein

Branch/Commit: **[869953f](https://github.com/sagemath/sagetrac-mirror/commit/869953f280c819912ed56d44e8c22517a8b732b4)**

Reviewer: **Punarbasu Purkayastha**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/17038_





---

archive/issue_events_225853.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-09-25T10:03:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17038#event-225853"
}
```



---

archive/issue_events_225854.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-09-25T10:03:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17038#event-225854"
}
```



---

archive/issue_events_225855.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-09-25T10:03:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17038#event-225855"
}
```



---

archive/issue_comments_228153.json:
```json
{
    "body": "Branch: **[u/ncohen/17038](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17038)**",
    "created_at": "2014-09-25T10:03:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17038#issuecomment-228153",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/17038](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17038)**



---

archive/issue_events_225856.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-09-25T10:03:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17038#event-225856"
}
```



---

archive/issue_comments_228154.json:
```json
{
    "body": "Commit: **[869953f](https://github.com/sagemath/sagetrac-mirror/commit/869953f280c819912ed56d44e8c22517a8b732b4)**",
    "created_at": "2014-09-25T10:04:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17038#issuecomment-228154",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[869953f](https://github.com/sagemath/sagetrac-mirror/commit/869953f280c819912ed56d44e8c22517a8b732b4)**



---

archive/issue_comments_228155.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/869953f280c819912ed56d44e8c22517a8b732b4\">869953f</a></td><td><code>trac #17038: deprecated_callable_import should mention the deprecation in __doc__</code></td></tr></table>\n",
    "created_at": "2014-09-25T10:04:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17038#issuecomment-228155",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/869953f280c819912ed56d44e8c22517a8b732b4">869953f</a></td><td><code>trac #17038: deprecated_callable_import should mention the deprecation in __doc__</code></td></tr></table>




---

archive/issue_comments_228156.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIs there a way to do something similar for functions like `sage.coding.code_constructions.cyclotomic_cosets`? Otherwise, the patch looks good to me.",
    "created_at": "2014-09-29T13:52:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17038#issuecomment-228156",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:3" align="right">Comment 3</div>

Is there a way to do something similar for functions like `sage.coding.code_constructions.cyclotomic_cosets`? Otherwise, the patch looks good to me.



---

archive/issue_comments_228157.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nHmmmm.. There is something called `deprecated_function_alias` but it does not work when the arguments are not the same. For this, we would need a decorator to deprecate a function with a given warning.\n\nNathann",
    "created_at": "2014-09-29T13:57:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17038#issuecomment-228157",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

Hmmmm.. There is something called `deprecated_function_alias` but it does not work when the arguments are not the same. For this, we would need a decorator to deprecate a function with a given warning.

Nathann



---

archive/issue_comments_228158.json:
```json
{
    "body": "Reviewer: **Punarbasu Purkayastha**",
    "created_at": "2014-09-30T11:18:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17038#issuecomment-228158",
    "user": "https://github.com/ppurka"
}
```

Reviewer: **Punarbasu Purkayastha**



---

archive/issue_comments_228159.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nNever mind. The deprecation is already mentioned in the docstring of the function. Outlying examples such as this can be dealt with in their individual tickets.",
    "created_at": "2014-09-30T11:18:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17038#issuecomment-228159",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:5" align="right">Comment 5</div>

Never mind. The deprecation is already mentioned in the docstring of the function. Outlying examples such as this can be dealt with in their individual tickets.



---

archive/issue_events_225857.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-09-30T11:18:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17038#event-225857"
}
```



---

archive/issue_events_225858.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-09-30T11:18:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17038#event-225858"
}
```



---

archive/issue_comments_228160.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThanks !",
    "created_at": "2014-09-30T11:18:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17038#issuecomment-228160",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thanks !



---

archive/issue_comments_228161.json:
```json
{
    "body": "Changed branch from **[u/ncohen/17038](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17038)** to **[869953f](https://github.com/sagemath/sagetrac-mirror/commit/869953f280c819912ed56d44e8c22517a8b732b4)**",
    "created_at": "2014-10-02T16:21:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17038#issuecomment-228161",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/ncohen/17038](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17038)** to **[869953f](https://github.com/sagemath/sagetrac-mirror/commit/869953f280c819912ed56d44e8c22517a8b732b4)**



---

archive/issue_events_225859.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-02T16:21:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17038#event-225859"
}
```



---

archive/issue_events_225860.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f7d8f46bd024a2875283ccba15085560c0dd4347",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-10-02T16:21:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17038",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17038#event-225860"
}
```
