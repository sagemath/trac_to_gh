# Issue 17235: magma failing tests

archive/issues_016998.json:
```json
{
    "assignees": [],
    "body": "Following doctest involving Magma (V2.17-8)  fail:\n\n```\n./sage -t --long --optional=sage,magma src/sage/rings/polynomial/polynomial_rational_flint.pyx  # 1 doctest failed\n./sage -t --long --optional=sage,magma src/sage/rings/number_field/number_field.py  # 1 doctest failed\n./sage -t --long --optional=sage,magma src/sage/interfaces/magma.py  # 1 doctest failed\n./sage -t --long --optional=sage,magma src/sage/algebras/quatalg/quaternion_algebra.py  # 2 doctests failed\n```\n\n\nKeywords: **magma failing doctests**\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17235_\n\n",
    "closed_at": "2014-10-29T22:12:13Z",
    "created_at": "2014-10-27T14:46:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "magma failing tests",
    "type": "issue",
    "updated_at": "2014-10-29T22:12:13Z",
    "url": "https://github.com/sagemath/sage/issues/17235",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```
Following doctest involving Magma (V2.17-8)  fail:

```
./sage -t --long --optional=sage,magma src/sage/rings/polynomial/polynomial_rational_flint.pyx  # 1 doctest failed
./sage -t --long --optional=sage,magma src/sage/rings/number_field/number_field.py  # 1 doctest failed
./sage -t --long --optional=sage,magma src/sage/interfaces/magma.py  # 1 doctest failed
./sage -t --long --optional=sage,magma src/sage/algebras/quatalg/quaternion_algebra.py  # 2 doctests failed
```


Keywords: **magma failing doctests**

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/17235_





---

archive/issue_events_228394.json:
```json
{
    "actor": "https://github.com/sagetrac-jakobkroeker",
    "created_at": "2014-10-27T14:46:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228394"
}
```



---

archive/issue_events_228395.json:
```json
{
    "actor": "https://github.com/sagetrac-jakobkroeker",
    "created_at": "2014-10-27T14:46:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228395"
}
```



---

archive/issue_events_228396.json:
```json
{
    "actor": "https://github.com/sagetrac-jakobkroeker",
    "created_at": "2014-10-27T15:04:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228396"
}
```



---

archive/issue_comments_231867.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-10-27T20:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17235#issuecomment-231867",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_228397.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-27T20:28:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228397"
}
```



---

archive/issue_events_228398.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-27T20:28:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228398"
}
```



---

archive/issue_events_228399.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-27T20:28:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228399"
}
```



---

archive/issue_events_228400.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-27T20:28:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228400"
}
```



---

archive/issue_comments_231868.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nDuplicate of #13885.",
    "created_at": "2014-10-27T20:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17235#issuecomment-231868",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
Duplicate of #13885.



---

archive/issue_events_228401.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-27T20:28:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228401"
}
```



---

archive/issue_events_228402.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-27T20:28:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228402"
}
```



---

archive/issue_events_228403.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-27T20:28:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228403"
}
```



---

archive/issue_events_228404.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-29T22:12:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228404"
}
```



---

archive/issue_events_228405.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-29T22:12:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228405"
}
```



---

archive/issue_events_228406.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-29T22:12:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17235",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17235#event-228406"
}
```
