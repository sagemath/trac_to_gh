# Issue 17269: Live documentation within Jupyter notebook

archive/issues_017032.json:
```json
{
    "assignees": [],
    "body": "The documentation (reference manual, thematic tutorials,...) is available from the Sage notebook as live worksheets. An important step in supporting Jupyter notebook for Sage is to have this feature available within Jupyter notebook.\n\nI have no idea on the right way to achieve this though (e.g. should it be done by Sage or requested at IPython / Jupyter level ?).\n\n---\n\nA solution to the problem raised in this ticket is provided at #20690.\n\nKeywords: **documentation notebook ipython**\n\nReviewer: **Thierry Monteil**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17269_\n\n",
    "closed_at": "2016-08-30T13:32:25Z",
    "created_at": "2014-10-31T16:05:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Live documentation within Jupyter notebook",
    "type": "issue",
    "updated_at": "2016-08-30T13:32:25Z",
    "url": "https://github.com/sagemath/sage/issues/17269",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
The documentation (reference manual, thematic tutorials,...) is available from the Sage notebook as live worksheets. An important step in supporting Jupyter notebook for Sage is to have this feature available within Jupyter notebook.

I have no idea on the right way to achieve this though (e.g. should it be done by Sage or requested at IPython / Jupyter level ?).

---

A solution to the problem raised in this ticket is provided at #20690.

Keywords: **documentation notebook ipython**

Reviewer: **Thierry Monteil**

_Issue created by migration from https://trac.sagemath.org/ticket/17269_





---

archive/issue_events_228859.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-10-31T16:05:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228859"
}
```



---

archive/issue_events_228860.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-10-31T16:05:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228860"
}
```



---

archive/issue_events_228861.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-10-31T16:05:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228861"
}
```



---

archive/issue_events_228862.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-10-31T16:05:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228862"
}
```



---

archive/issue_comments_233109.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nDuplicate of #20690.",
    "created_at": "2016-08-22T13:39:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17269#issuecomment-233109",
    "user": "https://github.com/slel"
}
```

<div id="comment:1" align="right">Comment 1</div>

Duplicate of #20690.



---

archive/issue_events_228863.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2016-08-22T13:39:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228863"
}
```



---

archive/issue_events_228864.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2016-08-22T13:39:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228864"
}
```



---

archive/issue_events_228865.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2016-08-22T13:39:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228865"
}
```



---

archive/issue_events_228866.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2016-08-22T13:39:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228866"
}
```



---

archive/issue_events_228867.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2016-08-22T19:47:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228867"
}
```



---

archive/issue_events_228868.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2016-08-22T19:47:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228868"
}
```



---

archive/issue_comments_233110.json:
```json
{
    "body": "Reviewer: **Thierry Monteil**",
    "created_at": "2016-08-22T19:47:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17269#issuecomment-233110",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Reviewer: **Thierry Monteil**



---

archive/issue_events_228869.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2016-08-30T08:49:46Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "title_is": "Live documentation within Jupyter notebook",
    "title_was": "Live documentation within ipython notebook",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228869"
}
```



---

archive/issue_comments_233111.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,7 @@\n-The documentation (reference manual, thematic tutorials,...) is available from the Sage notebook as live worksheets. An important step in supporting ipython notebook for Sage is to have this feature available within ipython notebook.\n+The documentation (reference manual, thematic tutorials,...) is available from the Sage notebook as live worksheets. An important step in supporting Jupyter notebook for Sage is to have this feature available within Jupyter notebook.\n \n-I have no idea on the right way to achieve this though (e.g. should it be done by Sage or requested at IPython level ?).\n+I have no idea on the right way to achieve this though (e.g. should it be done by Sage or requested at IPython / Jupyter level ?).\n \n+---\n+\n+A solution to the problem raised in this ticket is provided at #20690.\n``````\n",
    "created_at": "2016-08-30T08:49:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17269#issuecomment-233111",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,7 @@
-The documentation (reference manual, thematic tutorials,...) is available from the Sage notebook as live worksheets. An important step in supporting ipython notebook for Sage is to have this feature available within ipython notebook.
+The documentation (reference manual, thematic tutorials,...) is available from the Sage notebook as live worksheets. An important step in supporting Jupyter notebook for Sage is to have this feature available within Jupyter notebook.
 
-I have no idea on the right way to achieve this though (e.g. should it be done by Sage or requested at IPython level ?).
+I have no idea on the right way to achieve this though (e.g. should it be done by Sage or requested at IPython / Jupyter level ?).
 
+---
+
+A solution to the problem raised in this ticket is provided at #20690.
``````




---

archive/issue_events_228870.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228870"
}
```



---

archive/issue_comments_233112.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nDetermined to be invalid/duplicate/wontfix (closing as \"wontfix\" as a catch-all resolution).",
    "created_at": "2016-08-30T13:32:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17269#issuecomment-233112",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

Determined to be invalid/duplicate/wontfix (closing as "wontfix" as a catch-all resolution).



---

archive/issue_events_228871.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228871"
}
```



---

archive/issue_events_228872.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17269",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17269#event-228872"
}
```
