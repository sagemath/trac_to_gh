# Issue 17310: improvement to StrongTableaux to_transposition algorithm

archive/issues_017073.json:
```json
{
    "body": "In discussions after #17252, Jennifer Morse pointed out that the algorithm to convert a strong marked column strict tableau to a sequence of transpositions could be made more efficient by always choosing the largest rightmost marking as the end point.  The previous version of this algorithm scanned the potential marked column strict tableaux from right to left and pulled off markings if applying a transposition lowered the length of the core by one.  This version will test fewer potential marked ribbons.\n\nCC:  @anneschilling @tscrim\n\nKeywords: tableaux\n\nBranch/Commit: b87a7366fe35b3164a60f0f788ffc96d32071377\n\nReviewer: Travis Scrimshaw, Anne Schilling\n\nAuthor: Mike Zabrocki\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/17310\n\n",
    "closed_at": "2014-11-15T16:16:44Z",
    "created_at": "2014-11-08T20:56:09Z",
    "labels": [
        "component: combinatorics",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "improvement to StrongTableaux to_transposition algorithm",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/17310",
    "user": "https://github.com/zabrocki"
}
```
In discussions after #17252, Jennifer Morse pointed out that the algorithm to convert a strong marked column strict tableau to a sequence of transpositions could be made more efficient by always choosing the largest rightmost marking as the end point.  The previous version of this algorithm scanned the potential marked column strict tableaux from right to left and pulled off markings if applying a transposition lowered the length of the core by one.  This version will test fewer potential marked ribbons.

CC:  @anneschilling @tscrim

Keywords: tableaux

Branch/Commit: b87a7366fe35b3164a60f0f788ffc96d32071377

Reviewer: Travis Scrimshaw, Anne Schilling

Author: Mike Zabrocki

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/17310





---

archive/issue_comments_293604.json:
```json
{
    "body": "Changing branch from \"\" to \"public/zabrocki/17310/strongtableaux/transpositions\"",
    "created_at": "2014-11-09T13:18:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293604",
    "user": "https://github.com/zabrocki"
}
```

Changing branch from "" to "public/zabrocki/17310/strongtableaux/transpositions"



---

archive/issue_comments_293605.json:
```json
{
    "body": "Changing commit from \"\" to \"a17a36a320a99d7a9f6700e8503a1d994c0f38ad\"",
    "created_at": "2014-11-09T13:18:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293605",
    "user": "https://github.com/zabrocki"
}
```

Changing commit from "" to "a17a36a320a99d7a9f6700e8503a1d994c0f38ad"



---

archive/issue_comments_293606.json:
```json
{
    "body": "<a id='comment:1'></a>\nI will continue to run a few tests to make sure, but I think that this version shaves a few seconds off of the tests of k_tableau.py\n\n---\nNew commits:\n|                                                                                                                                          |                                                               |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------|\n|[a17a36a](http://git.sagemath.org/sage.git/commit/?id=a17a36a320a99d7a9f6700e8503a1d994c0f38ad)|`fix to marked_CST_to_transposition_sequence in StrongTableaux`|",
    "created_at": "2014-11-09T13:18:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293606",
    "user": "https://github.com/zabrocki"
}
```

<a id='comment:1'></a>
I will continue to run a few tests to make sure, but I think that this version shaves a few seconds off of the tests of k_tableau.py

---
New commits:
|                                                                                                                                          |                                                               |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------|
|[a17a36a](http://git.sagemath.org/sage.git/commit/?id=a17a36a320a99d7a9f6700e8503a1d994c0f38ad)|`fix to marked_CST_to_transposition_sequence in StrongTableaux`|



---

archive/issue_comments_293607.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1 @@\n-In discussions after #17252, Jennifer Morse pointed out that the algorithm to convert a strong marked column strict tableau to a sequence of transpositions could be made more efficient by always choosing the largest rightmost marking.\n-\n-We implement that improvement.\n+In discussions after #17252, Jennifer Morse pointed out that the algorithm to convert a strong marked column strict tableau to a sequence of transpositions could be made more efficient by always choosing the largest rightmost marking as the end point.  The previous version of this algorithm scanned the potential marked column strict tableaux from right to left and pulled off markings if applying a transposition lowered the length of the core by one.  This version will test fewer potential marked ribbons.\n``````\n",
    "created_at": "2014-11-09T14:40:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293607",
    "user": "https://github.com/zabrocki"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1 @@
-In discussions after #17252, Jennifer Morse pointed out that the algorithm to convert a strong marked column strict tableau to a sequence of transpositions could be made more efficient by always choosing the largest rightmost marking.
-
-We implement that improvement.
+In discussions after #17252, Jennifer Morse pointed out that the algorithm to convert a strong marked column strict tableau to a sequence of transpositions could be made more efficient by always choosing the largest rightmost marking as the end point.  The previous version of this algorithm scanned the potential marked column strict tableaux from right to left and pulled off markings if applying a transposition lowered the length of the core by one.  This version will test fewer potential marked ribbons.
``````




---

archive/issue_comments_293608.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to enhancement.",
    "created_at": "2014-11-09T14:40:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293608",
    "user": "https://github.com/zabrocki"
}
```

Changing type from PLEASE CHANGE to enhancement.



---

archive/issue_comments_293609.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis version seems to be slightly faster in practice.  On my laptop I see around 24-25 seconds with the patch, 24-26 without.",
    "created_at": "2014-11-09T14:40:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293609",
    "user": "https://github.com/zabrocki"
}
```

<a id='comment:2'></a>
This version seems to be slightly faster in practice.  On my laptop I see around 24-25 seconds with the patch, 24-26 without.



---

archive/issue_comments_293610.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2014-11-09T14:40:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293610",
    "user": "https://github.com/zabrocki"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_293611.json:
```json
{
    "body": "Changing commit from \"a17a36a320a99d7a9f6700e8503a1d994c0f38ad\" to \"b87a7366fe35b3164a60f0f788ffc96d32071377\"",
    "created_at": "2014-11-09T20:32:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293611",
    "user": "https://github.com/tscrim"
}
```

Changing commit from "a17a36a320a99d7a9f6700e8503a1d994c0f38ad" to "b87a7366fe35b3164a60f0f788ffc96d32071377"



---

archive/issue_comments_293612.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2014-11-09T20:32:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293612",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_293613.json:
```json
{
    "body": "<a id='comment:3'></a>\nSo I've made some changes and massaged out some more speed. My timings are below. I've also marked two tests as long because the file takes some 110s to run on mine (each were about 9s). However I'll let someone else (Anne) do the final review on the math portion.\n\n---\n\nOriginal timings:\n\n```\nsage: CST_to_trans = StrongTableaux.marked_CST_to_transposition_sequence\nsage: %timeit CST_to_trans([[-1, -1, -1], [1]], 2)\n100 loops, best of 3: 8.32 ms per loop\nsage: %timeit CST_to_trans([[-1, -2, -2, -2, -2], [-2, 2], [2]], 3)\n10 loops, best of 3: 21.1 ms per loop\nsage: %timeit CST_to_trans([[-1, -2, -5, 5, -5, 5, -5], [-3, -4, 5, 5], [5]],3)\n10 loops, best of 3: 104 ms per loop\nsage: %timeit CST_to_trans([[-1, -2, -3, 4, -7], [-4, -6], [-5, 6]],3)\n10 loops, best of 3: 29.7 ms per loop\nsage: %timeit CST_to_trans([], 2)\n100000 loops, best of 3: 2.46 \u00b5s per loop\n```\nWithout my changes:\n\n```\nsage: CST_to_trans = StrongTableaux.marked_CST_to_transposition_sequence\nsage: %timeit CST_to_trans([[-1, -1, -1], [1]], 2)\n100 loops, best of 3: 8.29 ms per loop\nsage: %timeit CST_to_trans([[-1, -2, -2, -2, -2], [-2, 2], [2]], 3)\n10 loops, best of 3: 21 ms per loop\nsage: %timeit CST_to_trans([[-1, -2, -5, 5, -5, 5, -5], [-3, -4, 5, 5], [5]],3)\n10 loops, best of 3: 48 ms per loop\nsage: %timeit CST_to_trans([[-1, -2, -3, 4, -7], [-4, -6], [-5, 6]],3)\n10 loops, best of 3: 29.3 ms per loop\nsage: %timeit CST_to_trans([], 2)\n100000 loops, best of 3: 2.4 \u00b5s per loop\n```\nWith my changes:\n\n```\nsage: CST_to_trans = StrongTableaux.marked_CST_to_transposition_sequence\nsage: %timeit CST_to_trans([[-1, -1, -1], [1]], 2)\n100 loops, best of 3: 7.7 ms per loop\nsage: %timeit CST_to_trans([[-1, -2, -2, -2, -2], [-2, 2], [2]], 3)\n10 loops, best of 3: 18.3 ms per loop\nsage: %timeit CST_to_trans([[-1, -2, -5, 5, -5, 5, -5], [-3, -4, 5, 5], [5]],3)\n10 loops, best of 3: 43.3 ms per loop\nsage: %timeit CST_to_trans([[-1, -2, -3, 4, -7], [-4, -6], [-5, 6]],3)\n10 loops, best of 3: 26.1 ms per loop\nsage: %timeit CST_to_trans([], 2)\n100000 loops, best of 3: 2 \u00b5s per loop\n```\n\n---\nNew commits:\n|                                                                                                                                          |                                                                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------|\n|[b87a736](http://git.sagemath.org/sage.git/commit/?id=b87a7366fe35b3164a60f0f788ffc96d32071377)|`Changed some things around to make it faster and marked tests as long.`|",
    "created_at": "2014-11-09T20:32:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293613",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>
So I've made some changes and massaged out some more speed. My timings are below. I've also marked two tests as long because the file takes some 110s to run on mine (each were about 9s). However I'll let someone else (Anne) do the final review on the math portion.

---

Original timings:

```
sage: CST_to_trans = StrongTableaux.marked_CST_to_transposition_sequence
sage: %timeit CST_to_trans([[-1, -1, -1], [1]], 2)
100 loops, best of 3: 8.32 ms per loop
sage: %timeit CST_to_trans([[-1, -2, -2, -2, -2], [-2, 2], [2]], 3)
10 loops, best of 3: 21.1 ms per loop
sage: %timeit CST_to_trans([[-1, -2, -5, 5, -5, 5, -5], [-3, -4, 5, 5], [5]],3)
10 loops, best of 3: 104 ms per loop
sage: %timeit CST_to_trans([[-1, -2, -3, 4, -7], [-4, -6], [-5, 6]],3)
10 loops, best of 3: 29.7 ms per loop
sage: %timeit CST_to_trans([], 2)
100000 loops, best of 3: 2.46 µs per loop
```
Without my changes:

```
sage: CST_to_trans = StrongTableaux.marked_CST_to_transposition_sequence
sage: %timeit CST_to_trans([[-1, -1, -1], [1]], 2)
100 loops, best of 3: 8.29 ms per loop
sage: %timeit CST_to_trans([[-1, -2, -2, -2, -2], [-2, 2], [2]], 3)
10 loops, best of 3: 21 ms per loop
sage: %timeit CST_to_trans([[-1, -2, -5, 5, -5, 5, -5], [-3, -4, 5, 5], [5]],3)
10 loops, best of 3: 48 ms per loop
sage: %timeit CST_to_trans([[-1, -2, -3, 4, -7], [-4, -6], [-5, 6]],3)
10 loops, best of 3: 29.3 ms per loop
sage: %timeit CST_to_trans([], 2)
100000 loops, best of 3: 2.4 µs per loop
```
With my changes:

```
sage: CST_to_trans = StrongTableaux.marked_CST_to_transposition_sequence
sage: %timeit CST_to_trans([[-1, -1, -1], [1]], 2)
100 loops, best of 3: 7.7 ms per loop
sage: %timeit CST_to_trans([[-1, -2, -2, -2, -2], [-2, 2], [2]], 3)
10 loops, best of 3: 18.3 ms per loop
sage: %timeit CST_to_trans([[-1, -2, -5, 5, -5, 5, -5], [-3, -4, 5, 5], [5]],3)
10 loops, best of 3: 43.3 ms per loop
sage: %timeit CST_to_trans([[-1, -2, -3, 4, -7], [-4, -6], [-5, 6]],3)
10 loops, best of 3: 26.1 ms per loop
sage: %timeit CST_to_trans([], 2)
100000 loops, best of 3: 2 µs per loop
```

---
New commits:
|                                                                                                                                          |                                                                        |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------|
|[b87a736](http://git.sagemath.org/sage.git/commit/?id=b87a7366fe35b3164a60f0f788ffc96d32071377)|`Changed some things around to make it faster and marked tests as long.`|



---

archive/issue_comments_293614.json:
```json
{
    "body": "Changing branch from \"public/zabrocki/17310/strongtableaux/transpositions\" to \"public/combinat/strong_tableaux_transpositions-17310\"",
    "created_at": "2014-11-09T20:32:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293614",
    "user": "https://github.com/tscrim"
}
```

Changing branch from "public/zabrocki/17310/strongtableaux/transpositions" to "public/combinat/strong_tableaux_transpositions-17310"



---

archive/issue_comments_293615.json:
```json
{
    "body": "<a id='comment:4'></a>\nHi Mike, thanks for implementing this! Looks good to me. Anne",
    "created_at": "2014-11-10T00:32:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293615",
    "user": "https://github.com/anneschilling"
}
```

<a id='comment:4'></a>
Hi Mike, thanks for implementing this! Looks good to me. Anne



---

archive/issue_comments_293616.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-11-10T00:32:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293616",
    "user": "https://github.com/anneschilling"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_293617.json:
```json
{
    "body": "Changing reviewer from \"Travis Scrimshaw\" to \"Travis Scrimshaw, Anne Schilling\"",
    "created_at": "2014-11-10T00:32:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293617",
    "user": "https://github.com/anneschilling"
}
```

Changing reviewer from "Travis Scrimshaw" to "Travis Scrimshaw, Anne Schilling"



---

archive/issue_comments_293618.json:
```json
{
    "body": "<a id='comment:6'></a>\nI've been calculating larger examples to make sure going from and to transposition sequences are consistent and have found no faults.\n\nThanks Travis and Anne.",
    "created_at": "2014-11-10T12:26:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293618",
    "user": "https://github.com/zabrocki"
}
```

<a id='comment:6'></a>
I've been calculating larger examples to make sure going from and to transposition sequences are consistent and have found no faults.

Thanks Travis and Anne.



---

archive/issue_comments_293619.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2014-11-15T16:16:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293619",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_293620.json:
```json
{
    "body": "Changing branch from \"public/combinat/strong_tableaux_transpositions-17310\" to \"b87a7366fe35b3164a60f0f788ffc96d32071377\"",
    "created_at": "2014-11-15T16:16:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17310#issuecomment-293620",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/combinat/strong_tableaux_transpositions-17310" to "b87a7366fe35b3164a60f0f788ffc96d32071377"



---

archive/issue_events_050004.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-15T16:16:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/17310",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17310#event-50004"
}
```
