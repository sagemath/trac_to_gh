# Issue 17068: Update SageTeX documentation in tutorial

archive/issues_016831.json:
```json
{
    "assignees": [],
    "body": "I'm preparing a brief talk on using SageTeX, and discovering that the tutorial seems to be incorrect.  \n\nhttp://www.sagemath.org/doc/tutorial/sagetex.html\n\n\"The full documentation can be found in SAGE_ROOT/local/share/texmf/tex/generic/sagetex, where SAGE_ROOT is the directory where your Sage installation is located.\"\n\nIn fact, it refers to this for the doc a few times.  It also refers to the incorrect name for the .sage file produced, which should be st_example.sagetex.sage\n\nThe installation guide seems to be correct...\n\nhttp://www.sagemath.org/doc/installation/sagetex.html\n\nsays\n\n\"While not strictly part of installation, it bears mentioning here that the documentation for SageTeX is maintained in SAGE_ROOT/local/share/doc/sagetex/sagetex.pdf. There is also an example file in the same directory \u2013 see example.tex and example.pdf, the pre-built result of typesetting that file with LaTeX and Sage.\"\n\n\nCC:  @dandrake\n\nBranch/Commit: **[e69695d](https://github.com/sagemath/sagetrac-mirror/commit/e69695d385cc65186972ffd2365a06f0f242d860)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Dan Drake**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17068_\n\n",
    "closed_at": "2014-10-02T16:21:56Z",
    "created_at": "2014-09-29T18:25:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update SageTeX documentation in tutorial",
    "type": "issue",
    "updated_at": "2014-10-02T16:21:56Z",
    "url": "https://github.com/sagemath/sage/issues/17068",
    "user": "https://github.com/kcrisman"
}
```
I'm preparing a brief talk on using SageTeX, and discovering that the tutorial seems to be incorrect.  

http://www.sagemath.org/doc/tutorial/sagetex.html

"The full documentation can be found in SAGE_ROOT/local/share/texmf/tex/generic/sagetex, where SAGE_ROOT is the directory where your Sage installation is located."

In fact, it refers to this for the doc a few times.  It also refers to the incorrect name for the .sage file produced, which should be st_example.sagetex.sage

The installation guide seems to be correct...

http://www.sagemath.org/doc/installation/sagetex.html

says

"While not strictly part of installation, it bears mentioning here that the documentation for SageTeX is maintained in SAGE_ROOT/local/share/doc/sagetex/sagetex.pdf. There is also an example file in the same directory – see example.tex and example.pdf, the pre-built result of typesetting that file with LaTeX and Sage."


CC:  @dandrake

Branch/Commit: **[e69695d](https://github.com/sagemath/sagetrac-mirror/commit/e69695d385cc65186972ffd2365a06f0f242d860)**

Reviewer: **Dima Pasechnik**

Author: **Dan Drake**

_Issue created by migration from https://trac.sagemath.org/ticket/17068_





---

archive/issue_events_226252.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-09-29T18:25:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226252"
}
```



---

archive/issue_events_226253.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-09-29T18:25:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226253"
}
```



---

archive/issue_events_226254.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-09-29T18:25:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226254"
}
```



---

archive/issue_events_226255.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-09-29T18:25:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226255"
}
```



---

archive/issue_comments_228685.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nI fixed the documentation. Now I need to figure out how do the git & trac stuff to get it here...",
    "created_at": "2014-09-29T19:17:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228685",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:1" align="right">Comment 1</div>

I fixed the documentation. Now I need to figure out how do the git & trac stuff to get it here...



---

archive/issue_comments_228686.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nReplying to [@dandrake](#comment%3A1):\n> I fixed the documentation. Now I need to figure out how do the git & trac stuff to get it here...\n\nyou might use this:\nhttps://github.com/sagemath/git-trac-command\n\nif you're very short on time, then just put the patch here, as an attachment...",
    "created_at": "2014-09-29T19:34:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228686",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:2" align="right">Comment 2</div>

Replying to [@dandrake](#comment%3A1):
> I fixed the documentation. Now I need to figure out how do the git & trac stuff to get it here...

you might use this:
https://github.com/sagemath/git-trac-command

if you're very short on time, then just put the patch here, as an attachment...



---

archive/issue_comments_228687.json:
```json
{
    "body": "Branch: **[u/ddrake/update_sagetex_documentation_in_tutorial](https://github.com/sagemath/sagetrac-mirror/tree/u/ddrake/update_sagetex_documentation_in_tutorial)**",
    "created_at": "2014-09-29T20:27:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228687",
    "user": "https://github.com/dandrake"
}
```

Branch: **[u/ddrake/update_sagetex_documentation_in_tutorial](https://github.com/sagemath/sagetrac-mirror/tree/u/ddrake/update_sagetex_documentation_in_tutorial)**



---

archive/issue_events_226256.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2014-09-29T20:30:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226256"
}
```



---

archive/issue_comments_228688.json:
```json
{
    "body": "Commit: **[e69695d](https://github.com/sagemath/sagetrac-mirror/commit/e69695d385cc65186972ffd2365a06f0f242d860)**",
    "created_at": "2014-09-29T20:30:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228688",
    "user": "https://github.com/dandrake"
}
```

Commit: **[e69695d](https://github.com/sagemath/sagetrac-mirror/commit/e69695d385cc65186972ffd2365a06f0f242d860)**



---

archive/issue_comments_228689.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nPatch...er, git branch up. Please review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e69695d385cc65186972ffd2365a06f0f242d860\">e69695d</a></td><td><code>fix directories to reflect current documentation</code></td></tr></table>\n",
    "created_at": "2014-09-29T20:30:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228689",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:4" align="right">Comment 4</div>

Patch...er, git branch up. Please review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e69695d385cc65186972ffd2365a06f0f242d860">e69695d</a></td><td><code>fix directories to reflect current documentation</code></td></tr></table>




---

archive/issue_events_226257.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-09-29T20:36:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226257"
}
```



---

archive/issue_events_226258.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-09-29T20:36:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226258"
}
```



---

archive/issue_comments_228690.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2014-09-29T20:36:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228690",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_228691.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLooks good to me!",
    "created_at": "2014-09-29T20:36:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228691",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:5" align="right">Comment 5</div>

Looks good to me!



---

archive/issue_comments_228692.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nAwesome work, thanks guys!  Now I can tell the students at AIMS to just use the documentation (well, in the next version of Sage...)",
    "created_at": "2014-09-30T05:51:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228692",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:6" align="right">Comment 6</div>

Awesome work, thanks guys!  Now I can tell the students at AIMS to just use the documentation (well, in the next version of Sage...)



---

archive/issue_comments_228693.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nAuthor name missing",
    "created_at": "2014-09-30T10:58:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228693",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:7" align="right">Comment 7</div>

Author name missing



---

archive/issue_events_226259.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-30T10:58:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226259"
}
```



---

archive/issue_events_226260.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-30T10:58:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226260"
}
```



---

archive/issue_events_226261.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-09-30T11:01:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226261"
}
```



---

archive/issue_events_226262.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-09-30T11:01:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226262"
}
```



---

archive/issue_comments_228694.json:
```json
{
    "body": "Author: **Dan Drake**",
    "created_at": "2014-09-30T11:01:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228694",
    "user": "https://github.com/kcrisman"
}
```

Author: **Dan Drake**



---

archive/issue_events_226263.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-02T16:21:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226263"
}
```



---

archive/issue_events_226264.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d72e5bd4a26f09fe787e94bfa39dcb34d4c7a334",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-10-02T16:21:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17068#event-226264"
}
```



---

archive/issue_comments_228695.json:
```json
{
    "body": "Changed branch from **[u/ddrake/update_sagetex_documentation_in_tutorial](https://github.com/sagemath/sagetrac-mirror/tree/u/ddrake/update_sagetex_documentation_in_tutorial)** to **[e69695d](https://github.com/sagemath/sagetrac-mirror/commit/e69695d385cc65186972ffd2365a06f0f242d860)**",
    "created_at": "2014-10-02T16:21:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17068#issuecomment-228695",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/ddrake/update_sagetex_documentation_in_tutorial](https://github.com/sagemath/sagetrac-mirror/tree/u/ddrake/update_sagetex_documentation_in_tutorial)** to **[e69695d](https://github.com/sagemath/sagetrac-mirror/commit/e69695d385cc65186972ffd2365a06f0f242d860)**
