# Issue 17768: Morphism.is_identity() assumes a Morphism is determined by its action on gens()

archive/issues_017531.json:
```json
{
    "assignees": [],
    "body": "Counterexample:\n\n```\nsage: from sage.categories.morphism import SetMorphism\nsage: f = SetMorphism(Hom(QQ, QQ, Sets()), numerator)\nsage: f.is_identity()\nTrue\n```\nSee also #15478, #15381.\n\nCC:  @xcaruso @sagetrac-jakobkroeker\n\nStopgaps: **wrongAnswerMarker**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17768_\n\n",
    "closed_at": "2017-11-28T09:19:48Z",
    "created_at": "2015-02-11T08:09:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Morphism.is_identity() assumes a Morphism is determined by its action on gens()",
    "type": "issue",
    "updated_at": "2017-11-28T09:19:48Z",
    "url": "https://github.com/sagemath/sage/issues/17768",
    "user": "https://github.com/mezzarobba"
}
```
Counterexample:

```
sage: from sage.categories.morphism import SetMorphism
sage: f = SetMorphism(Hom(QQ, QQ, Sets()), numerator)
sage: f.is_identity()
True
```
See also #15478, #15381.

CC:  @xcaruso @sagetrac-jakobkroeker

Stopgaps: **wrongAnswerMarker**

_Issue created by migration from https://trac.sagemath.org/ticket/17768_





---

archive/issue_events_235370.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-02-11T08:09:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17768#event-235370"
}
```



---

archive/issue_events_235371.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-02-11T08:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17768#event-235371"
}
```



---

archive/issue_events_235372.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-02-11T08:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17768#event-235372"
}
```



---

archive/issue_events_235373.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-02-11T08:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17768#event-235373"
}
```



---

archive/issue_comments_242523.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,4 +6,4 @@\n sage: f.is_identity()\n True\n ```\n-See also #15478.\n+See also #15478, #15481.\n``````\n",
    "created_at": "2015-02-11T17:42:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17768#issuecomment-242523",
    "user": "https://github.com/mezzarobba"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,4 +6,4 @@
 sage: f.is_identity()
 True
 ```
-See also #15478.
+See also #15478, #15481.
``````




---

archive/issue_comments_242524.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,4 +6,4 @@\n sage: f.is_identity()\n True\n ```\n-See also #15478, #15481.\n+See also #15478, #15381.\n``````\n",
    "created_at": "2015-02-11T17:42:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17768#issuecomment-242524",
    "user": "https://github.com/mezzarobba"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,4 +6,4 @@
 sage: f.is_identity()
 True
 ```
-See also #15478, #15481.
+See also #15478, #15381.
``````




---

archive/issue_comments_242525.json:
```json
{
    "body": "Stopgaps: **wrongAnswerMarker**",
    "created_at": "2017-02-26T01:12:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17768#issuecomment-242525",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

Stopgaps: **wrongAnswerMarker**



---

archive/issue_events_235374.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-28T09:19:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17768#event-235374"
}
```



---

archive/issue_events_235375.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-28T09:19:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17768#event-235375"
}
```



---

archive/issue_events_235376.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-11-28T09:19:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17768#event-235376"
}
```



---

archive/issue_comments_242526.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nDuplicate of #15381.",
    "created_at": "2017-11-28T09:19:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17768#issuecomment-242526",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

Duplicate of #15381.
