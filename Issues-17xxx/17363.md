# Issue 17363: Ukrainian translation of "Sage Tutorial"

archive/issues_017126.json:
```json
{
    "assignees": [],
    "body": "I've had a private email exchange with Max Harchenco. He pledged to translate the ReST files of the Sage Tutorial into Ukrainian and I promised that we'll then include them properly ;-)\n\n**Keywords:** ukrainian\n\nIssue created by migration from https://trac.sagemath.org/ticket/17363\n\n",
    "created_at": "2014-11-18T15:03:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20translations",
        "https://github.com/sagemath/sage/labels/minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Ukrainian translation of \"Sage Tutorial\"",
    "type": "issue",
    "updated_at": "2015-07-22T13:14:46Z",
    "url": "https://github.com/sagemath/sage/issues/17363",
    "user": "https://github.com/haraldschilly"
}
```
I've had a private email exchange with Max Harchenco. He pledged to translate the ReST files of the Sage Tutorial into Ukrainian and I promised that we'll then include them properly ;-)

**Keywords:** ukrainian

Issue created by migration from https://trac.sagemath.org/ticket/17363





---

archive/issue_events_212368.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2014-11-18T15:03:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17363#event-212368"
}
```



---

archive/issue_events_212369.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2014-11-18T15:03:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "08517b",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17363#event-212369"
}
```



---

archive/issue_events_212370.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2014-11-18T15:03:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "label": "https://github.com/sagemath/sage/labels/minor",
    "label_color": "ff0000",
    "label_name": "minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17363#event-212370"
}
```



---

archive/issue_events_212371.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2014-11-18T15:03:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17363#event-212371"
}
```



---

archive/issue_events_212372.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-11-20T16:21:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "08517b",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17363#event-212372"
}
```



---

archive/issue_events_212373.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-11-20T16:21:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20translations",
    "label_color": "08517b",
    "label_name": "component: translations",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17363#event-212373"
}
```



---

archive/issue_comments_234529.json:
```json
{
    "body": "<a id='comment:2'>**Comment 2:**</a>\nAny news?",
    "created_at": "2014-12-16T13:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17363#issuecomment-234529",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'>**Comment 2:**</a>
Any news?



---

archive/issue_comments_234530.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"ukrainian\".",
    "created_at": "2015-07-22T13:14:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17363#issuecomment-234530",
    "user": "https://github.com/fchapoton"
}
```

**Changing keywords** from "" to "ukrainian".



---

archive/issue_events_212374.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-22T13:14:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17363#event-212374"
}
```



---

archive/issue_events_212375.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-07-22T13:14:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17363",
    "milestone_number": null,
    "milestone_title": "sage-6.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17363#event-212375"
}
```
