...
```json
{
    "body": "Perhaps we should just cache the result of `.automorphism_group()`, given that we have immutable graphs. And we could have notes in the related functions saying that it uses the cache if the graph is immutable ?\n\nNathann",
...
```json
{
    "body": "Yes this looks like a good way to do it. Though it we would then need to address separately this new feature of testing transitivity on subgroups of Aut(G) ?",
