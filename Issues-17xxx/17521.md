# Issue 17521: Dangling lazy import for ClassicalCrystalOfAlcovePaths

archive/issues_017284.json:
```json
{
    "assignees": [],
    "body": "#16689 forgot to remove a lazy import for `ClassicalCrystalOfAlcovePaths`:\n\n```\nsage: ClassicalCrystalOfAlcovePaths\n<repr(<sage.misc.lazy_import.LazyImport at 0x7f499cc9e188>) failed: AttributeError: 'module' object has no attribute 'ClassicalCrystalOfAlcovePaths'>\n```\n\nBranch/Commit: **[9ffcb42](https://github.com/sagemath/sagetrac-mirror/commit/9ffcb425ed6596e36be195db620cf95c70de252d)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Martin von Gagern**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17521_\n\n",
    "closed_at": "2015-01-02T15:46:08Z",
    "created_at": "2014-12-18T01:41:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Dangling lazy import for ClassicalCrystalOfAlcovePaths",
    "type": "issue",
    "updated_at": "2015-01-02T15:46:08Z",
    "url": "https://github.com/sagemath/sage/issues/17521",
    "user": "https://github.com/gagern"
}
```
#16689 forgot to remove a lazy import for `ClassicalCrystalOfAlcovePaths`:

```
sage: ClassicalCrystalOfAlcovePaths
<repr(<sage.misc.lazy_import.LazyImport at 0x7f499cc9e188>) failed: AttributeError: 'module' object has no attribute 'ClassicalCrystalOfAlcovePaths'>
```

Branch/Commit: **[9ffcb42](https://github.com/sagemath/sagetrac-mirror/commit/9ffcb425ed6596e36be195db620cf95c70de252d)**

Reviewer: **Travis Scrimshaw**

Author: **Martin von Gagern**

_Issue created by migration from https://trac.sagemath.org/ticket/17521_





---

archive/issue_events_232068.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-12-18T01:41:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17521#event-232068"
}
```



---

archive/issue_events_232069.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-12-18T01:41:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17521#event-232069"
}
```



---

archive/issue_events_232070.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-12-18T01:41:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17521#event-232070"
}
```



---

archive/issue_events_232071.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-12-18T01:41:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17521#event-232071"
}
```



---

archive/issue_comments_237466.json:
```json
{
    "body": "Branch: **[u/gagern/ticket/17521](https://github.com/sagemath/sagetrac-mirror/tree/u/gagern/ticket/17521)**",
    "created_at": "2014-12-18T01:44:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17521#issuecomment-237466",
    "user": "https://github.com/gagern"
}
```

Branch: **[u/gagern/ticket/17521](https://github.com/sagemath/sagetrac-mirror/tree/u/gagern/ticket/17521)**



---

archive/issue_comments_237467.json:
```json
{
    "body": "Commit: **[9ffcb42](https://github.com/sagemath/sagetrac-mirror/commit/9ffcb425ed6596e36be195db620cf95c70de252d)**",
    "created_at": "2014-12-18T01:44:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17521#issuecomment-237467",
    "user": "https://github.com/gagern"
}
```

Commit: **[9ffcb42](https://github.com/sagemath/sagetrac-mirror/commit/9ffcb425ed6596e36be195db620cf95c70de252d)**



---

archive/issue_comments_237468.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9ffcb425ed6596e36be195db620cf95c70de252d\">9ffcb42</a></td><td><code>Remove lazy import for ClassicalCrystalOfAlcovePaths.</code></td></tr></table>\n",
    "created_at": "2014-12-18T01:44:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17521#issuecomment-237468",
    "user": "https://github.com/gagern"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9ffcb425ed6596e36be195db620cf95c70de252d">9ffcb42</a></td><td><code>Remove lazy import for ClassicalCrystalOfAlcovePaths.</code></td></tr></table>




---

archive/issue_events_232072.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-12-18T01:44:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17521#event-232072"
}
```



---

archive/issue_events_232073.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-12-22T23:35:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17521#event-232073"
}
```



---

archive/issue_events_232074.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-12-22T23:35:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17521#event-232074"
}
```



---

archive/issue_comments_237469.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2014-12-22T23:35:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17521#issuecomment-237469",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_237470.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2014-12-22T23:35:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17521#issuecomment-237470",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_237471.json:
```json
{
    "body": "Changed branch from **[u/gagern/ticket/17521](https://github.com/sagemath/sagetrac-mirror/tree/u/gagern/ticket/17521)** to **[9ffcb42](https://github.com/sagemath/sagetrac-mirror/commit/9ffcb425ed6596e36be195db620cf95c70de252d)**",
    "created_at": "2015-01-02T15:46:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17521#issuecomment-237471",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gagern/ticket/17521](https://github.com/sagemath/sagetrac-mirror/tree/u/gagern/ticket/17521)** to **[9ffcb42](https://github.com/sagemath/sagetrac-mirror/commit/9ffcb425ed6596e36be195db620cf95c70de252d)**



---

archive/issue_events_232075.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-02T15:46:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17521#event-232075"
}
```



---

archive/issue_events_232076.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "98f5fd063b8a0708d1a0cba8156e5326072fc0b8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-02T15:46:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17521",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17521#event-232076"
}
```
