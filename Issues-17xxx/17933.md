# Issue 17933: Ref: add various missing modules to r/matrices

archive/issues_017696.json:
```json
{
    "assignees": [],
    "body": "Add missing modules to the Matrices part of the reference manual.\n\nKeywords: **reference, matrices**\n\nBranch/Commit: **[d6676ff](https://github.com/sagemath/sagetrac-mirror/commit/d6676ff28ea88aecc26d8911cda3974ea7e905fc)**\n\nReviewer: **Julien Puydt**\n\nAuthor: **Marc Mezzarobba**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17933_\n\n",
    "closed_at": "2015-03-12T20:21:54Z",
    "created_at": "2015-03-11T14:01:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Ref: add various missing modules to r/matrices",
    "type": "issue",
    "updated_at": "2015-03-12T20:21:54Z",
    "url": "https://github.com/sagemath/sage/issues/17933",
    "user": "https://github.com/mezzarobba"
}
```
Add missing modules to the Matrices part of the reference manual.

Keywords: **reference, matrices**

Branch/Commit: **[d6676ff](https://github.com/sagemath/sagetrac-mirror/commit/d6676ff28ea88aecc26d8911cda3974ea7e905fc)**

Reviewer: **Julien Puydt**

Author: **Marc Mezzarobba**

_Issue created by migration from https://trac.sagemath.org/ticket/17933_





---

archive/issue_events_237540.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-11T14:01:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17933#event-237540"
}
```



---

archive/issue_events_237541.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-11T14:01:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17933#event-237541"
}
```



---

archive/issue_events_237542.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-11T14:01:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17933#event-237542"
}
```



---

archive/issue_events_237543.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-11T14:01:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17933#event-237543"
}
```



---

archive/issue_events_237544.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-03-11T14:42:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17933#event-237544"
}
```



---

archive/issue_comments_245625.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI read the patch, compiled the documentation, had a look around and everything looked correct.",
    "created_at": "2015-03-11T20:23:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17933#issuecomment-245625",
    "user": "https://github.com/sagetrac-Snark"
}
```

<div id="comment:2" align="right">Comment 2</div>

I read the patch, compiled the documentation, had a look around and everything looked correct.



---

archive/issue_comments_245626.json:
```json
{
    "body": "Reviewer: **Julien Puydt**",
    "created_at": "2015-03-11T20:23:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17933#issuecomment-245626",
    "user": "https://github.com/sagetrac-Snark"
}
```

Reviewer: **Julien Puydt**



---

archive/issue_events_237545.json:
```json
{
    "actor": "https://github.com/sagetrac-Snark",
    "created_at": "2015-03-11T20:23:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17933#event-237545"
}
```



---

archive/issue_events_237546.json:
```json
{
    "actor": "https://github.com/sagetrac-Snark",
    "created_at": "2015-03-11T20:23:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17933#event-237546"
}
```



---

archive/issue_events_237547.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-12T20:21:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17933#event-237547"
}
```



---

archive/issue_events_237548.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-12T20:21:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17933#event-237548"
}
```



---

archive/issue_comments_245627.json:
```json
{
    "body": "Changed branch from **[u/mmezzarobba/ref-matrices](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/ref-matrices)** to **[d6676ff](https://github.com/sagemath/sagetrac-mirror/commit/d6676ff28ea88aecc26d8911cda3974ea7e905fc)**",
    "created_at": "2015-03-12T20:21:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17933#issuecomment-245627",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmezzarobba/ref-matrices](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/ref-matrices)** to **[d6676ff](https://github.com/sagemath/sagetrac-mirror/commit/d6676ff28ea88aecc26d8911cda3974ea7e905fc)**
