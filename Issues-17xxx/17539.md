# Issue 17539: convenience conversion from symbolic to fraction field

archive/issues_017302.json:
```json
{
    "assignees": [],
    "body": "See http://ask.sagemath.org/question/25327/casting-symbolic-fractions-to-fraction-field-of-polynomial-ring/\n\nSomething like\n\n```\nsage: ex=(x+1)/(x-1); ex\n(x + 1)/(x - 1)\nsage: ex.numerator().polynomial(ZZ)/ex.denominator().polynomial(ZZ)\n(x + 1)/(x - 1)\nsage: type(_)\n<type 'sage.rings.fraction_field_element.FractionFieldElement'>\n```\nmade into an `Expression` method, perhaps named `fraction(...)` with the ring as argument.\n\nCC:  @miguelmarco\n\nBranch: **[753dacd](https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04)**\n\nReviewer: **Miguel Marco**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17539_\n\n",
    "closed_at": "2015-01-04T12:43:15Z",
    "created_at": "2014-12-23T07:13:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "convenience conversion from symbolic to fraction field",
    "type": "issue",
    "updated_at": "2015-01-04T14:08:31Z",
    "url": "https://github.com/sagemath/sage/issues/17539",
    "user": "https://github.com/rwst"
}
```
See http://ask.sagemath.org/question/25327/casting-symbolic-fractions-to-fraction-field-of-polynomial-ring/

Something like

```
sage: ex=(x+1)/(x-1); ex
(x + 1)/(x - 1)
sage: ex.numerator().polynomial(ZZ)/ex.denominator().polynomial(ZZ)
(x + 1)/(x - 1)
sage: type(_)
<type 'sage.rings.fraction_field_element.FractionFieldElement'>
```
made into an `Expression` method, perhaps named `fraction(...)` with the ring as argument.

CC:  @miguelmarco

Branch: **[753dacd](https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04)**

Reviewer: **Miguel Marco**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/17539_





---

archive/issue_events_232310.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-23T07:13:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17539#event-232310"
}
```



---

archive/issue_events_232311.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-23T07:13:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17539#event-232311"
}
```



---

archive/issue_events_232312.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-23T07:13:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17539#event-232312"
}
```



---

archive/issue_events_232313.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-23T07:13:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17539#event-232313"
}
```



---

archive/issue_comments_237770.json:
```json
{
    "body": "Branch: **[u/rws/convenience_conversion_from_symbolic_to_fraction_field](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/convenience_conversion_from_symbolic_to_fraction_field)**",
    "created_at": "2015-01-01T09:10:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237770",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/convenience_conversion_from_symbolic_to_fraction_field](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/convenience_conversion_from_symbolic_to_fraction_field)**



---

archive/issue_events_232314.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-01-01T09:11:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17539#event-232314"
}
```



---

archive/issue_comments_237771.json:
```json
{
    "body": "Commit: **[e36f547](https://github.com/sagemath/sagetrac-mirror/commit/e36f547d47db20169a17488b0ac56e87aac0d988)**",
    "created_at": "2015-01-01T09:11:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237771",
    "user": "https://github.com/rwst"
}
```

Commit: **[e36f547](https://github.com/sagemath/sagetrac-mirror/commit/e36f547d47db20169a17488b0ac56e87aac0d988)**



---

archive/issue_comments_237772.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2015-01-01T09:11:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237772",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_237773.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e36f547d47db20169a17488b0ac56e87aac0d988\">e36f547</a></td><td><code>17539: fraction method of expression</code></td></tr></table>\n",
    "created_at": "2015-01-01T09:11:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237773",
    "user": "https://github.com/rwst"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e36f547d47db20169a17488b0ac56e87aac0d988">e36f547</a></td><td><code>17539: fraction method of expression</code></td></tr></table>




---

archive/issue_comments_237774.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nIt looks good, although maybe some more doctest would be better. One with nontrivial numerator,  one with several variables, using other rings, etc.",
    "created_at": "2015-01-02T13:49:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237774",
    "user": "https://github.com/miguelmarco"
}
```

<a id='comment:3'>Comment 3:</a>
It looks good, although maybe some more doctest would be better. One with nontrivial numerator,  one with several variables, using other rings, etc.



---

archive/issue_comments_237775.json:
```json
{
    "body": "Reviewer: **Miguel Marco**",
    "created_at": "2015-01-02T13:49:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237775",
    "user": "https://github.com/miguelmarco"
}
```

Reviewer: **Miguel Marco**



---

archive/issue_comments_237776.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04\">753dacd</a></td><td><code>17539: more doctests</code></td></tr></table>\n",
    "created_at": "2015-01-02T16:21:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237776",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04">753dacd</a></td><td><code>17539: more doctests</code></td></tr></table>




---

archive/issue_comments_237777.json:
```json
{
    "body": "Changed commit from **[e36f547](https://github.com/sagemath/sagetrac-mirror/commit/e36f547d47db20169a17488b0ac56e87aac0d988)** to **[753dacd](https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04)**",
    "created_at": "2015-01-02T16:21:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237777",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e36f547](https://github.com/sagemath/sagetrac-mirror/commit/e36f547d47db20169a17488b0ac56e87aac0d988)** to **[753dacd](https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04)**



---

archive/issue_comments_237778.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nDone. Though I'm of the opinion that every computation path in the associated code needs only be tested once (so it's actually up to doctests in `polynomial_element.__div__` to be exhaustive). But I understand it maybe a demonstration of function behaviour.",
    "created_at": "2015-01-02T16:21:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237778",
    "user": "https://github.com/rwst"
}
```

<a id='comment:6'>Comment 6:</a>
Done. Though I'm of the opinion that every computation path in the associated code needs only be tested once (so it's actually up to doctests in `polynomial_element.__div__` to be exhaustive). But I understand it maybe a demonstration of function behaviour.



---

archive/issue_events_232315.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2015-01-03T13:32:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17539#event-232315"
}
```



---

archive/issue_events_232316.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2015-01-03T13:32:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17539#event-232316"
}
```



---

archive/issue_comments_237779.json:
```json
{
    "body": "Changed branch from **[u/rws/convenience_conversion_from_symbolic_to_fraction_field](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/convenience_conversion_from_symbolic_to_fraction_field)** to **[753dacd](https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04)**",
    "created_at": "2015-01-04T12:43:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237779",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/convenience_conversion_from_symbolic_to_fraction_field](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/convenience_conversion_from_symbolic_to_fraction_field)** to **[753dacd](https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04)**



---

archive/issue_events_232317.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-04T12:43:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17539#event-232317"
}
```



---

archive/issue_events_232318.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d248f67906230f54c04f1a98f2f2155fcc313e1c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-04T12:43:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17539#event-232318"
}
```



---

archive/issue_comments_237780.json:
```json
{
    "body": "Changed commit from **[753dacd](https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04)** to none",
    "created_at": "2015-01-04T14:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237780",
    "user": "https://github.com/rwst"
}
```

Changed commit from **[753dacd](https://github.com/sagemath/sagetrac-mirror/commit/753dacdc39299243f7e379a3c3f67d63f36a8c04)** to none



---

archive/issue_comments_237781.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThanks for the review.",
    "created_at": "2015-01-04T14:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17539",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17539#issuecomment-237781",
    "user": "https://github.com/rwst"
}
```

<a id='comment:9'>Comment 9:</a>
Thanks for the review.
