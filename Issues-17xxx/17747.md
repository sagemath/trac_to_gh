# Issue 17747: Do not remove tutorials without deprecating and redirecting the related pages

archive/issues_017510.json:
```json
{
    "body": "Recent moves in the tutorials removed some pages in the tutorials, which creates dead links both in worksheets (e.g. pointers to the local live doc) and webpages (e.g. pages pointing to http://www.sagemath.org/doc).\n\nThe removed pages should be replaced by a small page containing:\n- a line explaining that the page is obsolete (kind of deprecation warning)\n- a link to its replacement\n\nConcerned tickets seem to be: #17615 #17614 #17555\n\n\nCC:  @nathanncohen @videlec @kiwifb @dcoudert @kcrisman\n\nReviewer: Jeroen Demeyer\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/17747\n\n",
    "closed_at": "2017-07-13T07:54:31Z",
    "created_at": "2015-02-08T16:52:23Z",
    "labels": [
        "component: documentation"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Do not remove tutorials without deprecating and redirecting the related pages",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/17747",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```
Recent moves in the tutorials removed some pages in the tutorials, which creates dead links both in worksheets (e.g. pointers to the local live doc) and webpages (e.g. pages pointing to http://www.sagemath.org/doc).

The removed pages should be replaced by a small page containing:
- a line explaining that the page is obsolete (kind of deprecation warning)
- a link to its replacement

Concerned tickets seem to be: #17615 #17614 #17555


CC:  @nathanncohen @videlec @kiwifb @dcoudert @kcrisman

Reviewer: Jeroen Demeyer

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/17747





---

archive/issue_comments_303951.json:
```json
{
    "body": "<a id='comment:1'></a>More importantly, we should not compile the doc without the --warn-links flag. Otherwise all we can do is fix what we break, instead of not breaking anything in the first place `:-/`",
    "created_at": "2015-02-08T17:09:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303951",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:1'></a>More importantly, we should not compile the doc without the --warn-links flag. Otherwise all we can do is fix what we break, instead of not breaking anything in the first place `:-/`



---

archive/issue_comments_303952.json:
```json
{
    "body": "<a id='comment:2'></a>Not a blocker. \n\nIt would be nice to have stable links, but its not even clear how to do this without making progress impossible. As one data point, Python just has all versions of the docs online. So you can link to a fixed version (stable forever) or to the latest (which might change without notice).",
    "created_at": "2015-02-12T21:13:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303952",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'></a>Not a blocker. 

It would be nice to have stable links, but its not even clear how to do this without making progress impossible. As one data point, Python just has all versions of the docs online. So you can link to a fixed version (stable forever) or to the latest (which might change without notice).



---

archive/issue_comments_303953.json:
```json
{
    "body": "Changing type from defect to enhancement.",
    "created_at": "2015-02-12T21:13:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303953",
    "user": "https://github.com/vbraun"
}
```

Changing type from defect to enhancement.



---

archive/issue_comments_303954.json:
```json
{
    "body": "<a id='comment:3'></a>I am not speaking about keeping links stable forever, but indicating where is the follow-up. My point was just to re-create moved tutorial pages (bordeaux2008.rst, etc) with a reference to their new location.\n\nWithout such a fix, the recent changes in the doc (which were necessary to clean things up) create dead links all over the web (e.g. any webpage pointing to the bordeaux2008 thematic tutorial will point to a non-existing page) as well as on worksheets (this happens when you write links to some live tutorial within a worksheet, e.g. ``LP tutorial </doc/live/thematic_tutorials/linear_programming.html>`__`), which is why i consider this as blocker, at least a very annoying defect, since it actually breaks things.",
    "created_at": "2015-02-12T21:32:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303954",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

<a id='comment:3'></a>I am not speaking about keeping links stable forever, but indicating where is the follow-up. My point was just to re-create moved tutorial pages (bordeaux2008.rst, etc) with a reference to their new location.

Without such a fix, the recent changes in the doc (which were necessary to clean things up) create dead links all over the web (e.g. any webpage pointing to the bordeaux2008 thematic tutorial will point to a non-existing page) as well as on worksheets (this happens when you write links to some live tutorial within a worksheet, e.g. ``LP tutorial </doc/live/thematic_tutorials/linear_programming.html>`__`), which is why i consider this as blocker, at least a very annoying defect, since it actually breaks things.



---

archive/issue_comments_303955.json:
```json
{
    "body": "<a id='comment:4'></a>I agree that its annoying, but\n* creating auxiliary files by hand does not scale, it'll just break again\n* name one other large project that does it by sprinkling auxiliary doc files around\n* is delaying Sage-6.5 going to fix any of this?",
    "created_at": "2015-02-12T21:41:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303955",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'></a>I agree that its annoying, but
* creating auxiliary files by hand does not scale, it'll just break again
* name one other large project that does it by sprinkling auxiliary doc files around
* is delaying Sage-6.5 going to fix any of this?



---

archive/issue_comments_303956.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [vbraun](#comment%3A4):\n> I agree that its annoying, but\n> * creating auxiliary files by hand does not scale, it'll just break again\n\n\nThere could be a deprecation period : \"This page does not exist anymore, please refer to that page. The current page will disappear on XXX.\".\n\n> * name one other large project that does it by sprinkling auxiliary doc files around\n\n\nName one other large project that knows about broken links and just don't care. If there is a better solution, let us take it.\n\n> * is delaying Sage-6.5 going to fix any of this?\n\n\nThe problem is introduced in Sage-6.5, it is not a bad policy to fight against regression from a release to the next one.",
    "created_at": "2015-02-12T21:52:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303956",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

<a id='comment:5'></a>Replying to [vbraun](#comment%3A4):
> I agree that its annoying, but
> * creating auxiliary files by hand does not scale, it'll just break again


There could be a deprecation period : "This page does not exist anymore, please refer to that page. The current page will disappear on XXX.".

> * name one other large project that does it by sprinkling auxiliary doc files around


Name one other large project that knows about broken links and just don't care. If there is a better solution, let us take it.

> * is delaying Sage-6.5 going to fix any of this?


The problem is introduced in Sage-6.5, it is not a bad policy to fight against regression from a release to the next one.



---

archive/issue_comments_303957.json:
```json
{
    "body": "<a id='comment:6'></a>We have no policy on preserving links, and no automatic system to avoid breaking links. I don't agree that we have a regression here, you just haven't noticed the current state of affairs before. We had broken documentation links with every previous release, just not one that you noticed.\n\nAlso, all kinds of file types might move (html, pdf, images, ...). How should we handle them and/or how do we make the web server hand out 301?\n\nI already mentioned the IMHO correct solution: Version the docs on the web server just like Python does (see http://docs.python.org). There is nothing in the Sage repo that can do it, its just a web server configuration issue.",
    "created_at": "2015-02-12T23:25:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303957",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:6'></a>We have no policy on preserving links, and no automatic system to avoid breaking links. I don't agree that we have a regression here, you just haven't noticed the current state of affairs before. We had broken documentation links with every previous release, just not one that you noticed.

Also, all kinds of file types might move (html, pdf, images, ...). How should we handle them and/or how do we make the web server hand out 301?

I already mentioned the IMHO correct solution: Version the docs on the web server just like Python does (see http://docs.python.org). There is nothing in the Sage repo that can do it, its just a web server configuration issue.



---

archive/issue_comments_303958.json:
```json
{
    "body": "<a id='comment:7'></a>The next great thing we should do about the doc is try to make this `--warn-links` option the default one. Actually... Volker: wouldn't it be possible/easy to enable this option by default (in the 'make doc') for \"some\" folders at least ?\n\nAt some point the graph folder did not contain any broken link, and it would be good to keep it this way. On the other hand some folders contain a LOT of broken links, and we would need quite some work before being able to build their doc with this flag.\n\nWhat would you think of enabling it for some folders right now so that we do not introduce new broken links there, and add new folders to this restricted list from time to time so that someday we do not have any broken link (that we can detect) ?\n\nNathann",
    "created_at": "2015-02-13T07:05:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303958",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:7'></a>The next great thing we should do about the doc is try to make this `--warn-links` option the default one. Actually... Volker: wouldn't it be possible/easy to enable this option by default (in the 'make doc') for "some" folders at least ?

At some point the graph folder did not contain any broken link, and it would be good to keep it this way. On the other hand some folders contain a LOT of broken links, and we would need quite some work before being able to build their doc with this flag.

What would you think of enabling it for some folders right now so that we do not introduce new broken links there, and add new folders to this restricted list from time to time so that someday we do not have any broken link (that we can detect) ?

Nathann



---

archive/issue_comments_303959.json:
```json
{
    "body": "<a id='comment:8'></a>Too much time has passed. It's pointless to fix this now.",
    "created_at": "2017-04-18T15:49:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303959",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>Too much time has passed. It's pointless to fix this now.



---

archive/issue_comments_303960.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-04-18T15:49:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303960",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_events_050748.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:49:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17747#event-50748"
}
```



---

archive/issue_comments_303961.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-04-18T15:49:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303961",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_303962.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2017-04-18T15:49:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303962",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_events_050749.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17747#event-50749"
}
```



---

archive/issue_comments_303963.json:
```json
{
    "body": "<a id='comment:10'></a>Closing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).",
    "created_at": "2017-07-13T07:54:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303963",
    "user": "https://github.com/embray"
}
```

<a id='comment:10'></a>Closing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).



---

archive/issue_comments_303964.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2017-07-13T07:54:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17747#issuecomment-303964",
    "user": "https://github.com/embray"
}
```

Resolution: wontfix
