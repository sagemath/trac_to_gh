...
```json
{
    "body": "<a id='comment:3'></a>While the coercion seems to work as expected, it does not solve the \"`apply_map`\" issue. What else is needed for this?\n\n```python\nsage: R.<a,b> = QQ[]\nsage: m = matrix([[a^2-b^2, a^3-a*b^2], [a*b + b^2, -77*a+77*b]])\nsage: m.apply_map(factor)\nTraceback (most recent call last):\n...\nTypeError: x must be a list\n```",
...
<a id='comment:3'></a>While the coercion seems to work as expected, it does not solve the "`apply_map`" issue. What else is needed for this?
...
<a id='comment:4'></a>Well it doesn't work with integers, either:
...
```json
{
    "body": "<a id='comment:5'></a>OTOH I just found a bug (77?).",
    "created_at": "2015-10-13T06:26:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17624",
    "type": "issue_comment",
...
<a id='comment:5'></a>OTOH I just found a bug (77?).
...
<a id='comment:8'></a>It seems that the following, using SR's "factor", works pretty well for the problem in the question:
...
```json
{
    "body": "<a id='comment:9'></a>I think it's safer to explicitly convert the unit also to `SR`.",
...
<a id='comment:9'></a>I think it's safer to explicitly convert the unit also to `SR`.
...
```json
{
    "body": "<a id='comment:11'></a>I thought that would be done by giving it second for multiplication?",
    "created_at": "2015-10-14T12:47:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17624",
    "type": "issue_comment",
...
<a id='comment:11'></a>I thought that would be done by giving it second for multiplication?
...
```json
{
    "body": "<a id='comment:14'></a>So, assuming `SR.unit==1` is safe then...",
    "created_at": "2015-10-14T13:17:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17624",
    "type": "issue_comment",
...
<a id='comment:14'></a>So, assuming `SR.unit==1` is safe then...
...
<a id='comment:16'></a>Replying to [comment:8 nbruin]:
> The problem with doing it the other way around is that the intermediate result would have to be a matrix with elements coming from ... Factorization? While we could have that, it would require a significant design and refactoring effort.
...
<a id='comment:19'></a>Replying to [comment:18 mmezzarobba]:
> make Factorizations over Foo (an alternative representation of) elements *of Foo*.
