# Issue 17356: Deprecation error in poset join_matrix

archive/issues_017119.json:
```json
{
    "assignees": [],
    "body": "`meet_matrix()` and `join_matrix()` were moved from posets to lattices. I forgot to add `from sage.misc.superseded import deprecation` to functions in `posets.py`. Sorry!\n\n\nCC:  @nathanncohen\n\nBranch/Commit: **[da41387](https://github.com/sagemath/sagetrac-mirror/commit/da41387484e70cae665e6890fcfaa088e02603b3)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17356_\n\n",
    "closed_at": "2014-11-19T08:32:08Z",
    "created_at": "2014-11-17T12:22:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Deprecation error in poset join_matrix",
    "type": "issue",
    "updated_at": "2014-11-19T08:32:08Z",
    "url": "https://github.com/sagemath/sage/issues/17356",
    "user": "https://github.com/jm58660"
}
```
`meet_matrix()` and `join_matrix()` were moved from posets to lattices. I forgot to add `from sage.misc.superseded import deprecation` to functions in `posets.py`. Sorry!


CC:  @nathanncohen

Branch/Commit: **[da41387](https://github.com/sagemath/sagetrac-mirror/commit/da41387484e70cae665e6890fcfaa088e02603b3)**

Reviewer: **Nathann Cohen**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/17356_





---

archive/issue_events_229883.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-11-17T12:22:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17356#event-229883"
}
```



---

archive/issue_events_229884.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-11-17T12:22:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17356#event-229884"
}
```



---

archive/issue_events_229885.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-11-17T12:22:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17356#event-229885"
}
```



---

archive/issue_events_229886.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-11-17T12:22:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17356#event-229886"
}
```



---

archive/issue_comments_234469.json:
```json
{
    "body": "Branch: **[u/jmantysalo/deprecation_error_in_poset_join_matrix](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/deprecation_error_in_poset_join_matrix)**",
    "created_at": "2014-11-17T12:43:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17356#issuecomment-234469",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/deprecation_error_in_poset_join_matrix](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/deprecation_error_in_poset_join_matrix)**



---

archive/issue_events_229887.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-11-17T12:44:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17356#event-229887"
}
```



---

archive/issue_comments_234470.json:
```json
{
    "body": "Author: **Jori M\u00e4ntysalo**",
    "created_at": "2014-11-17T12:44:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17356#issuecomment-234470",
    "user": "https://github.com/jm58660"
}
```

Author: **Jori Mäntysalo**



---

archive/issue_comments_234471.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/da41387484e70cae665e6890fcfaa088e02603b3\">da41387</a></td><td><code>Added import statement.</code></td></tr></table>\n",
    "created_at": "2014-11-17T12:44:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17356#issuecomment-234471",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/da41387484e70cae665e6890fcfaa088e02603b3">da41387</a></td><td><code>Added import statement.</code></td></tr></table>




---

archive/issue_comments_234472.json:
```json
{
    "body": "Commit: **[da41387](https://github.com/sagemath/sagetrac-mirror/commit/da41387484e70cae665e6890fcfaa088e02603b3)**",
    "created_at": "2014-11-17T12:44:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17356#issuecomment-234472",
    "user": "https://github.com/jm58660"
}
```

Commit: **[da41387](https://github.com/sagemath/sagetrac-mirror/commit/da41387484e70cae665e6890fcfaa088e02603b3)**



---

archive/issue_comments_234473.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThis would also be done by #17354, if you review it..",
    "created_at": "2014-11-17T15:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17356#issuecomment-234473",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

This would also be done by #17354, if you review it..



---

archive/issue_comments_234474.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOops. Right `:-/`\n\nTurns out the function was only called on lattices, which is a rather good sign.\n\nI don't think it is worth adding a doctest as the function will be removed asap.\n\nNathann",
    "created_at": "2014-11-17T15:23:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17356#issuecomment-234474",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

Oops. Right `:-/`

Turns out the function was only called on lattices, which is a rather good sign.

I don't think it is worth adding a doctest as the function will be removed asap.

Nathann



---

archive/issue_events_229888.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-11-17T15:23:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17356#event-229888"
}
```



---

archive/issue_events_229889.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-11-17T15:23:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17356#event-229889"
}
```



---

archive/issue_comments_234475.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2014-11-17T15:23:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17356#issuecomment-234475",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_229890.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-19T08:32:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17356#event-229890"
}
```



---

archive/issue_events_229891.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7f124cb336e0c14bf5d910ee3b5f8b825ea890ec",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-11-19T08:32:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17356#event-229891"
}
```



---

archive/issue_comments_234476.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/deprecation_error_in_poset_join_matrix](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/deprecation_error_in_poset_join_matrix)** to **[da41387](https://github.com/sagemath/sagetrac-mirror/commit/da41387484e70cae665e6890fcfaa088e02603b3)**",
    "created_at": "2014-11-19T08:32:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17356#issuecomment-234476",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/deprecation_error_in_poset_join_matrix](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/deprecation_error_in_poset_join_matrix)** to **[da41387](https://github.com/sagemath/sagetrac-mirror/commit/da41387484e70cae665e6890fcfaa088e02603b3)**
