# Issue 17843: Make the atlas spkg handle base&fast archs on ARM

archive/issues_017606.json:
```json
{
    "assignees": [],
    "body": "Currently is gives an error saying the platform is unsupported.\n\nBranch/Commit: **[cc5907b](https://github.com/sagemath/sagetrac-mirror/commit/cc5907b7297d42caa3d0c2c23f610fd97768a398)**\n\nReviewer: **Jean-Pierre Flori**\n\nAuthor: **Julien Puydt**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17843_\n\n",
    "closed_at": "2015-02-24T00:39:07Z",
    "created_at": "2015-02-23T16:16:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make the atlas spkg handle base&fast archs on ARM",
    "type": "issue",
    "updated_at": "2015-02-24T00:39:07Z",
    "url": "https://github.com/sagemath/sage/issues/17843",
    "user": "https://github.com/sagetrac-Snark"
}
```
Currently is gives an error saying the platform is unsupported.

Branch/Commit: **[cc5907b](https://github.com/sagemath/sagetrac-mirror/commit/cc5907b7297d42caa3d0c2c23f610fd97768a398)**

Reviewer: **Jean-Pierre Flori**

Author: **Julien Puydt**

_Issue created by migration from https://trac.sagemath.org/ticket/17843_





---

archive/issue_events_236313.json:
```json
{
    "actor": "https://github.com/sagetrac-Snark",
    "created_at": "2015-02-23T16:16:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17843#event-236313"
}
```



---

archive/issue_events_236314.json:
```json
{
    "actor": "https://github.com/sagetrac-Snark",
    "created_at": "2015-02-23T16:16:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17843#event-236314"
}
```



---

archive/issue_events_236315.json:
```json
{
    "actor": "https://github.com/sagetrac-Snark",
    "created_at": "2015-02-23T16:16:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17843#event-236315"
}
```



---

archive/issue_events_236316.json:
```json
{
    "actor": "https://github.com/sagetrac-Snark",
    "created_at": "2015-02-23T16:16:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17843#event-236316"
}
```



---

archive/issue_comments_243868.json:
```json
{
    "body": "Branch: **[u/Snark/17843](https://github.com/sagemath/sagetrac-mirror/tree/u/Snark/17843)**",
    "created_at": "2015-02-23T16:22:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17843#issuecomment-243868",
    "user": "https://github.com/sagetrac-Snark"
}
```

Branch: **[u/Snark/17843](https://github.com/sagemath/sagetrac-mirror/tree/u/Snark/17843)**



---

archive/issue_comments_243869.json:
```json
{
    "body": "Commit: **[cc5907b](https://github.com/sagemath/sagetrac-mirror/commit/cc5907b7297d42caa3d0c2c23f610fd97768a398)**",
    "created_at": "2015-02-23T16:25:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17843#issuecomment-243869",
    "user": "https://github.com/sagetrac-Snark"
}
```

Commit: **[cc5907b](https://github.com/sagemath/sagetrac-mirror/commit/cc5907b7297d42caa3d0c2c23f610fd97768a398)**



---

archive/issue_events_236317.json:
```json
{
    "actor": "https://github.com/sagetrac-Snark",
    "created_at": "2015-02-23T16:25:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17843#event-236317"
}
```



---

archive/issue_comments_243870.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7aa829944912b9ac5a03829567e68099a40e431b\">7aa8299</a></td><td><code>Simple documentation fix</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cc5907b7297d42caa3d0c2c23f610fd97768a398\">cc5907b</a></td><td><code>Proposed fix for #17843 -- atlas base&fast arch on ARM</code></td></tr></table>\n",
    "created_at": "2015-02-23T16:25:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17843#issuecomment-243870",
    "user": "https://github.com/sagetrac-Snark"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7aa829944912b9ac5a03829567e68099a40e431b">7aa8299</a></td><td><code>Simple documentation fix</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cc5907b7297d42caa3d0c2c23f610fd97768a398">cc5907b</a></td><td><code>Proposed fix for #17843 -- atlas base&fast arch on ARM</code></td></tr></table>




---

archive/issue_comments_243871.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks good to me.",
    "created_at": "2015-02-23T16:45:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17843#issuecomment-243871",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:3'>Comment 3:</a>
Looks good to me.



---

archive/issue_comments_243872.json:
```json
{
    "body": "Author: **Julien Puydt**",
    "created_at": "2015-02-23T16:45:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17843#issuecomment-243872",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Author: **Julien Puydt**



---

archive/issue_comments_243873.json:
```json
{
    "body": "Reviewer: **Jean-Pierre Flori**",
    "created_at": "2015-02-23T16:45:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17843#issuecomment-243873",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Reviewer: **Jean-Pierre Flori**



---

archive/issue_events_236318.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-02-23T16:45:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17843#event-236318"
}
```



---

archive/issue_events_236319.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-02-23T16:45:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17843#event-236319"
}
```



---

archive/issue_comments_243874.json:
```json
{
    "body": "Changed branch from **[u/Snark/17843](https://github.com/sagemath/sagetrac-mirror/tree/u/Snark/17843)** to **[cc5907b](https://github.com/sagemath/sagetrac-mirror/commit/cc5907b7297d42caa3d0c2c23f610fd97768a398)**",
    "created_at": "2015-02-24T00:39:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17843#issuecomment-243874",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/Snark/17843](https://github.com/sagemath/sagetrac-mirror/tree/u/Snark/17843)** to **[cc5907b](https://github.com/sagemath/sagetrac-mirror/commit/cc5907b7297d42caa3d0c2c23f610fd97768a398)**



---

archive/issue_events_236320.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-24T00:39:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17843#event-236320"
}
```



---

archive/issue_events_236321.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "05ec1ae628f10024236ac74727b9519ff3019af0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-02-24T00:39:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17843",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17843#event-236321"
}
```
