# Issue 17736: not all fractions get converted with ex.fraction()

archive/issues_017499.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: a = var('a')\nsage: b = (1/a).fraction(QQ)\n...\nTypeError: unsupported operand parent(s) for '/': 'Univariate Polynomial Ring in x over Rational Field' and 'Univariate Polynomial Ring in a over Rational Field'\n```\nThe `1` apparently gets coerced into the `x`-ring, so we need to ensure equal rings somehow.\n\nBranch/Commit: **[b46d1df](https://github.com/sagemath/sagetrac-mirror/commit/b46d1df5a328fd39b2d6fa47e09deb2f8619b8e5)**\n\nReviewer: **Martin Albrecht**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17736_\n\n",
    "closed_at": "2015-05-26T23:34:28Z",
    "created_at": "2015-02-05T16:39:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "not all fractions get converted with ex.fraction()",
    "type": "issue",
    "updated_at": "2015-05-26T23:34:28Z",
    "url": "https://github.com/sagemath/sage/issues/17736",
    "user": "https://github.com/rwst"
}
```

```
sage: a = var('a')
sage: b = (1/a).fraction(QQ)
...
TypeError: unsupported operand parent(s) for '/': 'Univariate Polynomial Ring in x over Rational Field' and 'Univariate Polynomial Ring in a over Rational Field'
```
The `1` apparently gets coerced into the `x`-ring, so we need to ensure equal rings somehow.

Branch/Commit: **[b46d1df](https://github.com/sagemath/sagetrac-mirror/commit/b46d1df5a328fd39b2d6fa47e09deb2f8619b8e5)**

Reviewer: **Martin Albrecht**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/17736_





---

archive/issue_events_234975.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-05T16:39:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234975"
}
```



---

archive/issue_events_234976.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-05T16:39:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234976"
}
```



---

archive/issue_events_234977.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-05T16:39:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234977"
}
```



---

archive/issue_events_234978.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-05T16:39:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234978"
}
```



---

archive/issue_comments_242113.json:
```json
{
    "body": "Branch: **[u/rws/not_all_fractions_get_converted_with_ex_fraction__](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/not_all_fractions_get_converted_with_ex_fraction__)**",
    "created_at": "2015-02-21T17:21:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242113",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/not_all_fractions_get_converted_with_ex_fraction__](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/not_all_fractions_get_converted_with_ex_fraction__)**



---

archive/issue_events_234979.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-21T17:22:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234979"
}
```



---

archive/issue_events_234980.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-21T17:22:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234980"
}
```



---

archive/issue_comments_242114.json:
```json
{
    "body": "Commit: **[b46d1df](https://github.com/sagemath/sagetrac-mirror/commit/b46d1df5a328fd39b2d6fa47e09deb2f8619b8e5)**",
    "created_at": "2015-02-21T17:22:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242114",
    "user": "https://github.com/rwst"
}
```

Commit: **[b46d1df](https://github.com/sagemath/sagetrac-mirror/commit/b46d1df5a328fd39b2d6fa47e09deb2f8619b8e5)**



---

archive/issue_comments_242115.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b46d1df5a328fd39b2d6fa47e09deb2f8619b8e5\">b46d1df</a></td><td><code>17736: not all fractions get converted with ex.fraction()</code></td></tr></table>\n",
    "created_at": "2015-02-21T17:22:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242115",
    "user": "https://github.com/rwst"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b46d1df5a328fd39b2d6fa47e09deb2f8619b8e5">b46d1df</a></td><td><code>17736: not all fractions get converted with ex.fraction()</code></td></tr></table>




---

archive/issue_events_234981.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-21T17:22:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234981"
}
```



---

archive/issue_comments_242116.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2015-02-21T17:22:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242116",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_242117.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nPasses all patchbot tests.",
    "created_at": "2015-04-20T06:17:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242117",
    "user": "https://github.com/rwst"
}
```

<div id="comment:3" align="right">Comment 3</div>

Passes all patchbot tests.



---

archive/issue_comments_242118.json:
```json
{
    "body": "Reviewer: **Martin Albrecht**",
    "created_at": "2015-05-26T09:05:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242118",
    "user": "https://github.com/malb"
}
```

Reviewer: **Martin Albrecht**



---

archive/issue_events_234982.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2015-05-26T09:05:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234982"
}
```



---

archive/issue_events_234983.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2015-05-26T09:05:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234983"
}
```



---

archive/issue_events_234984.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2015-05-26T09:05:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234984"
}
```



---

archive/issue_events_234985.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2015-05-26T09:05:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234985"
}
```



---

archive/issue_comments_242119.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLooks good to me. A potential feature request could be to allow the user to specify the ordering of variables, but perhaps that's overkill?",
    "created_at": "2015-05-26T09:05:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242119",
    "user": "https://github.com/malb"
}
```

<div id="comment:4" align="right">Comment 4</div>

Looks good to me. A potential feature request could be to allow the user to specify the ordering of variables, but perhaps that's overkill?



---

archive/issue_comments_242120.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI'm not so familiar with algebra. Would you suggest specific orderings or is there a convention?",
    "created_at": "2015-05-26T09:27:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242120",
    "user": "https://github.com/rwst"
}
```

<div id="comment:5" align="right">Comment 5</div>

I'm not so familiar with algebra. Would you suggest specific orderings or is there a convention?



---

archive/issue_comments_242121.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nHi, sorry for not being clear:\n\nI merely meant the ordering of variable (names), where you currently call `sort()`. But as I said: it's an optional feature request which could go into a separate ticket.\n\nOf course, arbitrary term ordering (\"lex\" and \"deglex\" etc.) could also be considered. But I'd say a user should do the conversion by hand perhaps in this case.",
    "created_at": "2015-05-26T10:47:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242121",
    "user": "https://github.com/malb"
}
```

<div id="comment:6" align="right">Comment 6</div>

Hi, sorry for not being clear:

I merely meant the ordering of variable (names), where you currently call `sort()`. But as I said: it's an optional feature request which could go into a separate ticket.

Of course, arbitrary term ordering ("lex" and "deglex" etc.) could also be considered. But I'd say a user should do the conversion by hand perhaps in this case.



---

archive/issue_comments_242122.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@malb](#comment%3A6):\n> I merely meant the ordering of variable (names), where you currently call `sort()`. But as I said: it's an optional feature request which could go into a separate ticket.\n\nAh but yes, I agree. Thanks for the review.",
    "created_at": "2015-05-26T12:48:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242122",
    "user": "https://github.com/rwst"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@malb](#comment%3A6):
> I merely meant the ordering of variable (names), where you currently call `sort()`. But as I said: it's an optional feature request which could go into a separate ticket.

Ah but yes, I agree. Thanks for the review.



---

archive/issue_comments_242123.json:
```json
{
    "body": "Changed branch from **[u/rws/not_all_fractions_get_converted_with_ex_fraction__](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/not_all_fractions_get_converted_with_ex_fraction__)** to **[b46d1df](https://github.com/sagemath/sagetrac-mirror/commit/b46d1df5a328fd39b2d6fa47e09deb2f8619b8e5)**",
    "created_at": "2015-05-26T23:34:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17736#issuecomment-242123",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/not_all_fractions_get_converted_with_ex_fraction__](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/not_all_fractions_get_converted_with_ex_fraction__)** to **[b46d1df](https://github.com/sagemath/sagetrac-mirror/commit/b46d1df5a328fd39b2d6fa47e09deb2f8619b8e5)**



---

archive/issue_events_234986.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-26T23:34:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234986"
}
```



---

archive/issue_events_234987.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "159879c863d9d41bfa1d62a29cd75da2389f61c5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-05-26T23:34:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17736",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17736#event-234987"
}
```
