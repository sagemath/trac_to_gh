# Issue 17641: stricmp is being deprecated in Cygwin

archive/issues_017404.json:
```json
{
    "assignees": [],
    "body": "stricmp is being deprecated in Cygwin\nSee https://cygwin.com/ml/cygwin/2014-10/msg00359.html\nOne should use strcasecmp\n\nThis makes Singular build fail on Cygwin32 (not Cygwin64 at the moment).\n\nCC:  @sagetrac-gouezel @tscrim\n\nKeywords: **cygwin singular**\n\nBranch/Commit: **[7727566](https://github.com/sagemath/sagetrac-mirror/commit/772756647f5fdb7fe78bddc8ab43603f93a08a62)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17641_\n\n",
    "closed_at": "2015-02-27T08:47:41Z",
    "created_at": "2015-01-16T08:53:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "stricmp is being deprecated in Cygwin",
    "type": "issue",
    "updated_at": "2015-02-27T08:47:41Z",
    "url": "https://github.com/sagemath/sage/issues/17641",
    "user": "https://github.com/sagetrac-jpflori"
}
```
stricmp is being deprecated in Cygwin
See https://cygwin.com/ml/cygwin/2014-10/msg00359.html
One should use strcasecmp

This makes Singular build fail on Cygwin32 (not Cygwin64 at the moment).

CC:  @sagetrac-gouezel @tscrim

Keywords: **cygwin singular**

Branch/Commit: **[7727566](https://github.com/sagemath/sagetrac-mirror/commit/772756647f5fdb7fe78bddc8ab43603f93a08a62)**

Reviewer: **Travis Scrimshaw**

Author: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/17641_





---

archive/issue_events_233681.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-01-16T08:53:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17641#event-233681"
}
```



---

archive/issue_events_233682.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-01-16T08:53:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17641#event-233682"
}
```



---

archive/issue_events_233683.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-01-16T08:53:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17641#event-233683"
}
```



---

archive/issue_events_233684.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-01-16T08:53:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17641#event-233684"
}
```



---

archive/issue_comments_240317.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,5 @@\n stricmp is being deprecated in Cygwin\n See https://cygwin.com/ml/cygwin/2014-10/msg00359.html\n+One should use strcasecmp\n \n This makes Singular build fail on Cygwin32 (not Cygwin64 at the moment).\n``````\n",
    "created_at": "2015-01-16T09:04:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240317",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,5 @@
 stricmp is being deprecated in Cygwin
 See https://cygwin.com/ml/cygwin/2014-10/msg00359.html
+One should use strcasecmp
 
 This makes Singular build fail on Cygwin32 (not Cygwin64 at the moment).
``````




---

archive/issue_comments_240318.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAttachment: **[stricmp.patch.gz](https://github.com/sagemath/sage/files/ticket17641/stricmp.patch.gz)**",
    "created_at": "2015-01-16T15:23:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240318",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

Attachment: **[stricmp.patch.gz](https://github.com/sagemath/sage/files/ticket17641/stricmp.patch.gz)**



---

archive/issue_comments_240319.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nHi Jean-Pierre,\n\nDid you report upstream? The corresponding field of this track ticket is still N/A.\n\nVincent",
    "created_at": "2015-01-17T17:52:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240319",
    "user": "https://github.com/videlec"
}
```

<div id="comment:3" align="right">Comment 3</div>

Hi Jean-Pierre,

Did you report upstream? The corresponding field of this track ticket is still N/A.

Vincent



---

archive/issue_comments_240320.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNope, and I won't do so before end of february when I come back to France.\nFeel free to do it, but note it is not a high priority as:\n- it only affects Cygwin32\n- I did not check the state of Singular 4-0-x.",
    "created_at": "2015-01-17T19:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240320",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:4" align="right">Comment 4</div>

Nope, and I won't do so before end of february when I come back to France.
Feel free to do it, but note it is not a high priority as:
- it only affects Cygwin32
- I did not check the state of Singular 4-0-x.



---

archive/issue_comments_240321.json:
```json
{
    "body": "Commit: **[7727566](https://github.com/sagemath/sagetrac-mirror/commit/772756647f5fdb7fe78bddc8ab43603f93a08a62)**",
    "created_at": "2015-02-26T13:14:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240321",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Commit: **[7727566](https://github.com/sagemath/sagetrac-mirror/commit/772756647f5fdb7fe78bddc8ab43603f93a08a62)**



---

archive/issue_events_233685.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-02-26T13:14:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17641#event-233685"
}
```



---

archive/issue_comments_240322.json:
```json
{
    "body": "Author: **Jean-Pierre Flori**",
    "created_at": "2015-02-26T13:14:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240322",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Author: **Jean-Pierre Flori**



---

archive/issue_comments_240323.json:
```json
{
    "body": "Changed keywords from **** to **cygwin singular**",
    "created_at": "2015-02-26T13:14:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240323",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Changed keywords from **** to **cygwin singular**



---

archive/issue_comments_240324.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/772756647f5fdb7fe78bddc8ab43603f93a08a62\">7727566</a></td><td><code>stricmp is being depercated on Cygwin.</code></td></tr></table>\n",
    "created_at": "2015-02-26T13:14:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240324",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:5" align="right">Comment 5</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/772756647f5fdb7fe78bddc8ab43603f93a08a62">7727566</a></td><td><code>stricmp is being depercated on Cygwin.</code></td></tr></table>




---

archive/issue_comments_240325.json:
```json
{
    "body": "Branch: **[u/jpflori/ticket/17641](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/17641)**",
    "created_at": "2015-02-26T13:14:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240325",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Branch: **[u/jpflori/ticket/17641](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/17641)**



---

archive/issue_comments_240326.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2015-02-26T20:00:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240326",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_240327.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nLGTM.",
    "created_at": "2015-02-26T20:00:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240327",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

LGTM.



---

archive/issue_events_233686.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-02-26T20:00:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17641#event-233686"
}
```



---

archive/issue_events_233687.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-02-26T20:00:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17641#event-233687"
}
```



---

archive/issue_comments_240328.json:
```json
{
    "body": "Changed branch from **[u/jpflori/ticket/17641](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/17641)** to **[7727566](https://github.com/sagemath/sagetrac-mirror/commit/772756647f5fdb7fe78bddc8ab43603f93a08a62)**",
    "created_at": "2015-02-27T08:47:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17641#issuecomment-240328",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jpflori/ticket/17641](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/17641)** to **[7727566](https://github.com/sagemath/sagetrac-mirror/commit/772756647f5fdb7fe78bddc8ab43603f93a08a62)**



---

archive/issue_events_233688.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-27T08:47:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17641#event-233688"
}
```



---

archive/issue_events_233689.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6e01cb1ae9bac98f4a8136c91a91633832ae8367",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-02-27T08:47:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17641",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17641#event-233689"
}
```
