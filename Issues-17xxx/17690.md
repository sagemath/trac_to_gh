# Issue 17690: Remove outdated SSL section in the "install from source code" documentation

archive/issues_017453.json:
```json
{
    "assignees": [],
    "body": "In the [\"install from source code\" documentation](http://sagemath.org/doc/installation/source.html), there are some sections on how to build Sage with SSL support, some of it in relation to the removed sagenb. \n\nSince sage 9.3, openssl is a normal standard package.\n\nWe remove the outdated and confusing info.\n\n\nCC:  @slel @williamstein @dimpase\n\nBranch/Commit: **[4e84ec8](https://github.com/sagemath/sagetrac-mirror/commit/4e84ec838b688821ac42770f724aa079bfaa7571)**\n\nReviewer: **Thierry Monteil**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17690_\n\n",
    "closed_at": "2021-08-29T09:38:21Z",
    "created_at": "2015-01-29T14:24:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove outdated SSL section in the \"install from source code\" documentation",
    "type": "issue",
    "updated_at": "2021-08-29T09:38:21Z",
    "url": "https://github.com/sagemath/sage/issues/17690",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
In the ["install from source code" documentation](http://sagemath.org/doc/installation/source.html), there are some sections on how to build Sage with SSL support, some of it in relation to the removed sagenb. 

Since sage 9.3, openssl is a normal standard package.

We remove the outdated and confusing info.


CC:  @slel @williamstein @dimpase

Branch/Commit: **[4e84ec8](https://github.com/sagemath/sagetrac-mirror/commit/4e84ec838b688821ac42770f724aa079bfaa7571)**

Reviewer: **Thierry Monteil**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/17690_





---

archive/issue_events_234388.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-01-29T14:24:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234388"
}
```



---

archive/issue_events_234389.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-01-29T14:24:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234389"
}
```



---

archive/issue_events_234390.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-01-29T14:24:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234390"
}
```



---

archive/issue_events_234391.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-01-29T14:24:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234391"
}
```



---

archive/issue_comments_241380.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nGood idea!  In the past this was indeed just sagenb-specific.",
    "created_at": "2015-01-29T17:33:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241380",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'>Comment 1:</a>
Good idea!  In the past this was indeed just sagenb-specific.



---

archive/issue_comments_241381.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWe still need to replace the outdated instructions regarding 'secure notebook' by Jupyter/openssl instructions",
    "created_at": "2021-01-02T18:42:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241381",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
We still need to replace the outdated instructions regarding 'secure notebook' by Jupyter/openssl instructions



---

archive/issue_events_234392.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-02T18:42:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234392"
}
```



---

archive/issue_events_234393.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-02T18:42:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234393"
}
```



---

archive/issue_events_234394.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-02T18:42:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234394"
}
```



---

archive/issue_events_234395.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-02T18:42:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234395"
}
```



---

archive/issue_events_234396.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-18T16:17:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234396"
}
```



---

archive/issue_events_234397.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-18T16:17:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234397"
}
```



---

archive/issue_events_234398.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-13T02:10:01Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "title_is": "Remove outdated SSL section in the \"install from source code\" documentation",
    "title_was": "A SSL section in the \"install from source code\" documentation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234398"
}
```



---

archive/issue_comments_241382.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,2 @@\n-In the [\"install from source code\" documentation](http://sagemath.org/doc/installation/source.html), there are some information on how to build Sage with SSL support. However it is somehow hidden in sage-notebook-specific sections, while SSL is also required to let ipython-notebook and pip work. Let us refactor this.\n+In the [\"install from source code\" documentation](http://sagemath.org/doc/installation/source.html), there are some sections on how to build Sage with SSL support, some of it in relation to the removed sagenb. We remove the outdated info.\n \n``````\n",
    "created_at": "2021-08-13T02:10:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241382",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,2 @@
-In the ["install from source code" documentation](http://sagemath.org/doc/installation/source.html), there are some information on how to build Sage with SSL support. However it is somehow hidden in sage-notebook-specific sections, while SSL is also required to let ipython-notebook and pip work. Let us refactor this.
+In the ["install from source code" documentation](http://sagemath.org/doc/installation/source.html), there are some sections on how to build Sage with SSL support, some of it in relation to the removed sagenb. We remove the outdated info.
 
``````




---

archive/issue_comments_241383.json:
```json
{
    "body": "Branch: **[u/mkoeppe/remove_outdated_ssl_section_in_the__install_from_source_code__documentation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_outdated_ssl_section_in_the__install_from_source_code__documentation)**",
    "created_at": "2021-08-13T03:23:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241383",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/remove_outdated_ssl_section_in_the__install_from_source_code__documentation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_outdated_ssl_section_in_the__install_from_source_code__documentation)**



---

archive/issue_events_234399.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-13T03:25:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234399"
}
```



---

archive/issue_comments_241384.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-08-13T03:25:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241384",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_241385.json:
```json
{
    "body": "Commit: **[4e84ec8](https://github.com/sagemath/sagetrac-mirror/commit/4e84ec838b688821ac42770f724aa079bfaa7571)**",
    "created_at": "2021-08-13T03:25:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241385",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[4e84ec8](https://github.com/sagemath/sagetrac-mirror/commit/4e84ec838b688821ac42770f724aa079bfaa7571)**



---

archive/issue_comments_241386.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8ef92876ebedda88bb48e83525b420a914ac73bd\">8ef9287</a></td><td><code>src/doc/en/installation: Remove outdated ssl info</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4e84ec838b688821ac42770f724aa079bfaa7571\">4e84ec8</a></td><td><code>README.md: Remove outdated ssl info</code></td></tr></table>\n",
    "created_at": "2021-08-13T03:25:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241386",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'>Comment 7:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8ef92876ebedda88bb48e83525b420a914ac73bd">8ef9287</a></td><td><code>src/doc/en/installation: Remove outdated ssl info</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4e84ec838b688821ac42770f724aa079bfaa7571">4e84ec8</a></td><td><code>README.md: Remove outdated ssl info</code></td></tr></table>




---

archive/issue_comments_241387.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,6 @@\n-In the [\"install from source code\" documentation](http://sagemath.org/doc/installation/source.html), there are some sections on how to build Sage with SSL support, some of it in relation to the removed sagenb. We remove the outdated info.\n+In the [\"install from source code\" documentation](http://sagemath.org/doc/installation/source.html), there are some sections on how to build Sage with SSL support, some of it in relation to the removed sagenb. \n \n+Since sage 9.3, openssl is a normal standard package.\n+\n+We remove the outdated and confusing info.\n+\n``````\n",
    "created_at": "2021-08-13T03:25:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241387",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,6 @@
-In the ["install from source code" documentation](http://sagemath.org/doc/installation/source.html), there are some sections on how to build Sage with SSL support, some of it in relation to the removed sagenb. We remove the outdated info.
+In the ["install from source code" documentation](http://sagemath.org/doc/installation/source.html), there are some sections on how to build Sage with SSL support, some of it in relation to the removed sagenb. 
 
+Since sage 9.3, openssl is a normal standard package.
+
+We remove the outdated and confusing info.
+
``````




---

archive/issue_comments_241388.json:
```json
{
    "body": "Reviewer: **Thierry Monteil**",
    "created_at": "2021-08-13T11:05:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241388",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Reviewer: **Thierry Monteil**



---

archive/issue_comments_241389.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nLGTM.",
    "created_at": "2021-08-13T11:05:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241389",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<a id='comment:8'>Comment 8:</a>
LGTM.



---

archive/issue_events_234400.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-08-13T11:05:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234400"
}
```



---

archive/issue_events_234401.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-08-13T11:05:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234401"
}
```



---

archive/issue_comments_241390.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThanks!",
    "created_at": "2021-08-13T17:39:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241390",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:9'>Comment 9:</a>
Thanks!



---

archive/issue_events_234402.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234402"
}
```



---

archive/issue_events_234403.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234403"
}
```



---

archive/issue_events_234404.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-08-29T09:38:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234404"
}
```



---

archive/issue_events_234405.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "390774c2ddbbff4d589023bcd2637700edbd50c7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-08-29T09:38:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17690#event-234405"
}
```



---

archive/issue_comments_241391.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/remove_outdated_ssl_section_in_the__install_from_source_code__documentation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_outdated_ssl_section_in_the__install_from_source_code__documentation)** to **[4e84ec8](https://github.com/sagemath/sagetrac-mirror/commit/4e84ec838b688821ac42770f724aa079bfaa7571)**",
    "created_at": "2021-08-29T09:38:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17690",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17690#issuecomment-241391",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/remove_outdated_ssl_section_in_the__install_from_source_code__documentation](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_outdated_ssl_section_in_the__install_from_source_code__documentation)** to **[4e84ec8](https://github.com/sagemath/sagetrac-mirror/commit/4e84ec838b688821ac42770f724aa079bfaa7571)**
