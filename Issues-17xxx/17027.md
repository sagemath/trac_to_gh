# Issue 17027: K.fetch_int(0r) for K = GF(2^16, 'a') fails

archive/issues_016790.json:
```json
{
    "assignees": [],
    "body": "It fails because we are computing logs to compute the answer.\n\nBranch/Commit: **[72e8d70](https://github.com/sagemath/sagetrac-mirror/commit/72e8d7019989d548942a8273be151b71ab912777)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Martin Albrecht**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17027_\n\n",
    "closed_at": "2014-09-25T12:05:38Z",
    "created_at": "2014-09-22T20:00:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "K.fetch_int(0r) for K = GF(2^16, 'a') fails",
    "type": "issue",
    "updated_at": "2014-09-25T12:05:38Z",
    "url": "https://github.com/sagemath/sage/issues/17027",
    "user": "https://github.com/malb"
}
```
It fails because we are computing logs to compute the answer.

Branch/Commit: **[72e8d70](https://github.com/sagemath/sagetrac-mirror/commit/72e8d7019989d548942a8273be151b71ab912777)**

Reviewer: **Volker Braun**

Author: **Martin Albrecht**

_Issue created by migration from https://trac.sagemath.org/ticket/17027_





---

archive/issue_events_225682.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-09-22T20:00:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225682"
}
```



---

archive/issue_events_225683.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-09-22T20:00:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225683"
}
```



---

archive/issue_events_225684.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-09-22T20:00:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225684"
}
```



---

archive/issue_events_225685.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-09-22T20:00:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225685"
}
```



---

archive/issue_events_225686.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-09-22T20:02:42Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "title_is": "K.fetch_int(0r) for K = GF(2^16, 'a') fails",
    "title_was": "K.fetch_int(0) for K = GF(2^16, 'a') fails",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225686"
}
```



---

archive/issue_comments_227783.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/72e8d7019989d548942a8273be151b71ab912777\">72e8d70</a></td><td><code>Fix GF(2^16,'a').fetch_int(0r)</code></td></tr></table>\n",
    "created_at": "2014-09-22T20:28:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17027#issuecomment-227783",
    "user": "https://github.com/malb"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/72e8d7019989d548942a8273be151b71ab912777">72e8d70</a></td><td><code>Fix GF(2^16,'a').fetch_int(0r)</code></td></tr></table>




---

archive/issue_comments_227784.json:
```json
{
    "body": "Branch: **[u/malb/t17027_fetch_int](https://github.com/sagemath/sagetrac-mirror/tree/u/malb/t17027_fetch_int)**",
    "created_at": "2014-09-22T20:28:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17027#issuecomment-227784",
    "user": "https://github.com/malb"
}
```

Branch: **[u/malb/t17027_fetch_int](https://github.com/sagemath/sagetrac-mirror/tree/u/malb/t17027_fetch_int)**



---

archive/issue_events_225687.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-09-22T20:28:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225687"
}
```



---

archive/issue_comments_227785.json:
```json
{
    "body": "Commit: **[72e8d70](https://github.com/sagemath/sagetrac-mirror/commit/72e8d7019989d548942a8273be151b71ab912777)**",
    "created_at": "2014-09-22T20:28:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17027#issuecomment-227785",
    "user": "https://github.com/malb"
}
```

Commit: **[72e8d70](https://github.com/sagemath/sagetrac-mirror/commit/72e8d7019989d548942a8273be151b71ab912777)**



---

archive/issue_events_225688.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-22T20:56:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225688"
}
```



---

archive/issue_events_225689.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-22T20:56:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225689"
}
```



---

archive/issue_comments_227786.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-09-23T09:07:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17027#issuecomment-227786",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_227787.json:
```json
{
    "body": "Changed branch from **[u/malb/t17027_fetch_int](https://github.com/sagemath/sagetrac-mirror/tree/u/malb/t17027_fetch_int)** to **[72e8d70](https://github.com/sagemath/sagetrac-mirror/commit/72e8d7019989d548942a8273be151b71ab912777)**",
    "created_at": "2014-09-25T12:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17027#issuecomment-227787",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/malb/t17027_fetch_int](https://github.com/sagemath/sagetrac-mirror/tree/u/malb/t17027_fetch_int)** to **[72e8d70](https://github.com/sagemath/sagetrac-mirror/commit/72e8d7019989d548942a8273be151b71ab912777)**



---

archive/issue_events_225690.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-25T12:05:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225690"
}
```



---

archive/issue_events_225691.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d0d5652cc90c7459f43e947956555ae091b30f3e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-09-25T12:05:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17027",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17027#event-225691"
}
```
