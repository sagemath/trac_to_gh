...
```json
{
    "body": "<a id='comment:5'></a>I mean replace `LatticePoset` with `FiniteLatticePoset` as in[comment:2](#comment%3A2) to avoid the input validation checks",
...
<a id='comment:5'></a>I mean replace `LatticePoset` with `FiniteLatticePoset` as in[comment:2](#comment%3A2) to avoid the input validation checks
