# Issue 17990: Implements is_zero and is_commutative for infinity ring

archive/issues_017753.json:
```json
{
    "assignees": [],
    "body": "Infinity ring is commutative and non-zero.\n\nBecause there is no `is_zero` and `is_commutative` method for `InfinityRing`, numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.\n\n```\nsage: m = Matrix([Infinity])\nsage: m.nrows()\n1\nsage: m.rows()\n---------------------------------------------------------------------------\nAttributeError                            Traceback (most recent call last)\n\n[...]\nAttributeError: 'InfinityRing_class' object has no attribute 'is_zero'\n```\n\nCC:  @jplab @rwst\n\nKeywords: **InfinityRing, matrix, days64**\n\nBranch/Commit: **[76307b8](https://github.com/sagemath/sagetrac-mirror/commit/76307b8af01f0af127a7a2abb2cd79851f59028e)**\n\nReviewer: **Jean-Philippe Labb\u00e9**\n\nAuthor: **Aladin Virmaux**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17990_\n\n",
    "closed_at": "2015-05-06T23:57:47Z",
    "created_at": "2015-03-18T21:33:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implements is_zero and is_commutative for infinity ring",
    "type": "issue",
    "updated_at": "2015-05-06T23:57:47Z",
    "url": "https://github.com/sagemath/sage/issues/17990",
    "user": "https://github.com/avirmaux"
}
```
Infinity ring is commutative and non-zero.

Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.

```
sage: m = Matrix([Infinity])
sage: m.nrows()
1
sage: m.rows()
---------------------------------------------------------------------------
AttributeError                            Traceback (most recent call last)

[...]
AttributeError: 'InfinityRing_class' object has no attribute 'is_zero'
```

CC:  @jplab @rwst

Keywords: **InfinityRing, matrix, days64**

Branch/Commit: **[76307b8](https://github.com/sagemath/sagetrac-mirror/commit/76307b8af01f0af127a7a2abb2cd79851f59028e)**

Reviewer: **Jean-Philippe Labbé**

Author: **Aladin Virmaux**

_Issue created by migration from https://trac.sagemath.org/ticket/17990_





---

archive/issue_events_238267.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-03-18T21:33:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238267"
}
```



---

archive/issue_events_238268.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-03-18T21:33:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238268"
}
```



---

archive/issue_events_238269.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-03-18T21:33:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238269"
}
```



---

archive/issue_events_238270.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-03-18T21:33:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238270"
}
```



---

archive/issue_comments_247082.json:
```json
{
    "body": "Branch: **[u/virmaux/matrix_with_infinity](https://github.com/sagemath/sagetrac-mirror/tree/u/virmaux/matrix_with_infinity)**",
    "created_at": "2015-03-18T21:43:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247082",
    "user": "https://github.com/avirmaux"
}
```

Branch: **[u/virmaux/matrix_with_infinity](https://github.com/sagemath/sagetrac-mirror/tree/u/virmaux/matrix_with_infinity)**



---

archive/issue_events_238271.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-03-18T21:44:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238271"
}
```



---

archive/issue_comments_247083.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/270acabc1c9ab7ac116a188643fcc90a01c5043d\">270acab</a></td><td><code>17990: add is_zero and is_commutative methods for InfinityRing</code></td></tr></table>\n",
    "created_at": "2015-03-18T21:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247083",
    "user": "https://github.com/avirmaux"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/270acabc1c9ab7ac116a188643fcc90a01c5043d">270acab</a></td><td><code>17990: add is_zero and is_commutative methods for InfinityRing</code></td></tr></table>




---

archive/issue_comments_247084.json:
```json
{
    "body": "Commit: **[270acab](https://github.com/sagemath/sagetrac-mirror/commit/270acabc1c9ab7ac116a188643fcc90a01c5043d)**",
    "created_at": "2015-03-18T21:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247084",
    "user": "https://github.com/avirmaux"
}
```

Commit: **[270acab](https://github.com/sagemath/sagetrac-mirror/commit/270acabc1c9ab7ac116a188643fcc90a01c5043d)**



---

archive/issue_comments_247085.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nDo other systems (such as in the mission statement) even have matrices with infinity?  I *guess* the infinity ring is commutative...?",
    "created_at": "2015-03-19T02:35:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247085",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">Comment 3</div>

Do other systems (such as in the mission statement) even have matrices with infinity?  I *guess* the infinity ring is commutative...?



---

archive/issue_comments_247086.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nHello!\nI am not sure to understand your question. Are you looking for actual examples in sage of code using matrix for infinity elements ?\n\nThe depth of the problem of the mission statement is that the base ring of `InfinityRing` is itself. The methods rows is looking if self has the attribute is_commutative, and if not suppose that self.base_ring has it.",
    "created_at": "2015-03-19T03:52:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247086",
    "user": "https://github.com/avirmaux"
}
```

<div id="comment:4" align="right">Comment 4</div>

Hello!
I am not sure to understand your question. Are you looking for actual examples in sage of code using matrix for infinity elements ?

The depth of the problem of the mission statement is that the base ring of `InfinityRing` is itself. The methods rows is looking if self has the attribute is_commutative, and if not suppose that self.base_ring has it.



---

archive/issue_comments_247087.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nSorry, what I meant was do other math software systems such as Maple, Mathematica, Magma, ... have matrices with infinity in them?  Does anyone ever use such things?  Or should we instead disallow them?  (I'm not speaking of arrays with `NaN` values, but of matrices specifically and infinity specifically.)",
    "created_at": "2015-03-19T14:12:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247087",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:5" align="right">Comment 5</div>

Sorry, what I meant was do other math software systems such as Maple, Mathematica, Magma, ... have matrices with infinity in them?  Does anyone ever use such things?  Or should we instead disallow them?  (I'm not speaking of arrays with `NaN` values, but of matrices specifically and infinity specifically.)



---

archive/issue_comments_247088.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, a numerous methods fails for matrices with infinity elements.\n+Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, a numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.\n \n ```\n sage: m = Matrix([Infinity])\n``````\n",
    "created_at": "2015-03-19T14:32:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247088",
    "user": "https://github.com/jplab"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, a numerous methods fails for matrices with infinity elements.
+Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, a numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.
 
 ```
 sage: m = Matrix([Infinity])
``````




---

archive/issue_comments_247089.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nCoxeter matrices is an example of matrix where infinity is a possible value. See #17798.",
    "created_at": "2015-03-19T14:32:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247089",
    "user": "https://github.com/jplab"
}
```

<div id="comment:6" align="right">Comment 6</div>

Coxeter matrices is an example of matrix where infinity is a possible value. See #17798.



---

archive/issue_comments_247090.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@jplab](#comment%3A6):\n> Coxeter matrices is an example of matrix where infinity is a possible value. See #17798.\n\nBut matrices over the infinity ring will not allow you to mix infinite entries with integers or whatever else. While\n\n```\nsage: Matrix([[SR(Infinity), 1],[2,3]])\n[+Infinity         1]\n[        2         3]\n```\nworks fine.",
    "created_at": "2015-03-19T15:57:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247090",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@jplab](#comment%3A6):
> Coxeter matrices is an example of matrix where infinity is a possible value. See #17798.

But matrices over the infinity ring will not allow you to mix infinite entries with integers or whatever else. While

```
sage: Matrix([[SR(Infinity), 1],[2,3]])
[+Infinity         1]
[        2         3]
```
works fine.



---

archive/issue_comments_247091.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nHello,\n\n```\nWe check that #17990 is fixed::\n```\nshould be\n\n```\nWe check that :trac:`17990` is fixed::\n```\n\nOn the other hand, a better solution would be to use the new category framework for that and get rid of `ParentWithGens`.\n\nVincent",
    "created_at": "2015-03-21T12:13:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247091",
    "user": "https://github.com/videlec"
}
```

<div id="comment:8" align="right">Comment 8</div>

Hello,

```
We check that #17990 is fixed::
```
should be

```
We check that :trac:`17990` is fixed::
```

On the other hand, a better solution would be to use the new category framework for that and get rid of `ParentWithGens`.

Vincent



---

archive/issue_comments_247092.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThis ticket deals exclusively with `MatrixSpace` over `InfinityRing`, and in this case, it is `InfinityRing` that needs to become a new-style `Parent` class to fix this ticket and perhaps is the better fix IMO for `is_zero` (along with the implementation of `is_commutative`).\n\nIn parallel to this, `MatrixSpace` should become a `Parent`, and since it walks like one, it shouldn't be too hard to convert it over.\n\nFor matrices with infinities, we want to construct a Coxeter matrix without the current hack of representing `oo` with `-1` (in `CoxeterMatrixGroup`), nor do we want to use the `SymbolicRing` as we feel it is way too heavy-handed of a solution. I guess fundamentally we want a 2D array since we don't need the ground set to be a ring (it'd be `NN[oo]`), but the matrix class(es) in Sage have nice displays...",
    "created_at": "2015-03-21T15:32:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247092",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">Comment 9</div>

This ticket deals exclusively with `MatrixSpace` over `InfinityRing`, and in this case, it is `InfinityRing` that needs to become a new-style `Parent` class to fix this ticket and perhaps is the better fix IMO for `is_zero` (along with the implementation of `is_commutative`).

In parallel to this, `MatrixSpace` should become a `Parent`, and since it walks like one, it shouldn't be too hard to convert it over.

For matrices with infinities, we want to construct a Coxeter matrix without the current hack of representing `oo` with `-1` (in `CoxeterMatrixGroup`), nor do we want to use the `SymbolicRing` as we feel it is way too heavy-handed of a solution. I guess fundamentally we want a 2D array since we don't need the ground set to be a ring (it'd be `NN[oo]`), but the matrix class(es) in Sage have nice displays...



---

archive/issue_comments_247093.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nReplying to [@tscrim](#comment%3A9):\n> For matrices with infinities, we want to construct a Coxeter matrix without the current hack of representing `oo` with `-1` (in `CoxeterMatrixGroup`), nor do we want to use the `SymbolicRing` as we feel it is way too heavy-handed of a solution.\n\nSo then I guess you need something like an \"Alexandroff extension functor\" that you could apply to a Parent to adjoin it an element at infinity, don't you?",
    "created_at": "2015-03-21T16:58:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247093",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:10" align="right">Comment 10</div>

Replying to [@tscrim](#comment%3A9):
> For matrices with infinities, we want to construct a Coxeter matrix without the current hack of representing `oo` with `-1` (in `CoxeterMatrixGroup`), nor do we want to use the `SymbolicRing` as we feel it is way too heavy-handed of a solution.

So then I guess you need something like an "Alexandroff extension functor" that you could apply to a Parent to adjoin it an element at infinity, don't you?



---

archive/issue_comments_247094.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nReplying to [@mezzarobba](#comment%3A10):\n> Replying to [@tscrim](#comment%3A9):\n> > For matrices with infinities, we want to construct a Coxeter matrix without the current hack of representing `oo` with `-1` (in `CoxeterMatrixGroup`), nor do we want to use the `SymbolicRing` as we feel it is way too heavy-handed of a solution.\n\n> \n> So then I guess you need something like an \"Alexandroff extension functor\" that you could apply to a Parent to adjoin it an element at infinity, don't you?\n\nIf that means given an ordered set `S`, we can add an element `oo` such that `x < oo` for all in `S`, then yes.",
    "created_at": "2015-03-21T17:02:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247094",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:11" align="right">Comment 11</div>

Replying to [@mezzarobba](#comment%3A10):
> Replying to [@tscrim](#comment%3A9):
> > For matrices with infinities, we want to construct a Coxeter matrix without the current hack of representing `oo` with `-1` (in `CoxeterMatrixGroup`), nor do we want to use the `SymbolicRing` as we feel it is way too heavy-handed of a solution.

> 
> So then I guess you need something like an "Alexandroff extension functor" that you could apply to a Parent to adjoin it an element at infinity, don't you?

If that means given an ordered set `S`, we can add an element `oo` such that `x < oo` for all in `S`, then yes.



---

archive/issue_comments_247095.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nPlease use the trac role <code>:trac:\\`17990\\`</code> instead of just `#17990` in the doc.",
    "created_at": "2015-04-05T18:44:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247095",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:12" align="right">Comment 12</div>

Please use the trac role <code>:trac:\`17990\`</code> instead of just `#17990` in the doc.



---

archive/issue_events_238272.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-04-05T18:44:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238272"
}
```



---

archive/issue_events_238273.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-04-05T18:44:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238273"
}
```



---

archive/issue_comments_247096.json:
```json
{
    "body": "Changed commit from **[270acab](https://github.com/sagemath/sagetrac-mirror/commit/270acabc1c9ab7ac116a188643fcc90a01c5043d)** to **[3626313](https://github.com/sagemath/sagetrac-mirror/commit/36263134d2c65c97ef3e57e8f0efa2e8ba0b52b9)**",
    "created_at": "2015-04-08T19:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247096",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[270acab](https://github.com/sagemath/sagetrac-mirror/commit/270acabc1c9ab7ac116a188643fcc90a01c5043d)** to **[3626313](https://github.com/sagemath/sagetrac-mirror/commit/36263134d2c65c97ef3e57e8f0efa2e8ba0b52b9)**



---

archive/issue_comments_247097.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f8a953684c46d1612493a19d96754e2741bb1d00\">f8a9536</a></td><td><code>Merge branch 'develop' into t/17990/matrix_with_infinity</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/36263134d2c65c97ef3e57e8f0efa2e8ba0b52b9\">3626313</a></td><td><code>17790: correct use of trac role</code></td></tr></table>\n",
    "created_at": "2015-04-08T19:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247097",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:13" align="right">Comment 13</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f8a953684c46d1612493a19d96754e2741bb1d00">f8a9536</a></td><td><code>Merge branch 'develop' into t/17990/matrix_with_infinity</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/36263134d2c65c97ef3e57e8f0efa2e8ba0b52b9">3626313</a></td><td><code>17790: correct use of trac role</code></td></tr></table>




---

archive/issue_events_238274.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-04-08T19:03:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238274"
}
```



---

archive/issue_events_238275.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-04-08T19:03:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238275"
}
```



---

archive/issue_comments_247098.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nI just merged develop and corrected the wrong reference to trac into <code>:trac:\\`17990\\`</code>.",
    "created_at": "2015-04-08T19:03:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247098",
    "user": "https://github.com/avirmaux"
}
```

<div id="comment:14" align="right">Comment 14</div>

I just merged develop and corrected the wrong reference to trac into <code>:trac:\`17990\`</code>.



---

archive/issue_comments_247099.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nReplying to [@tscrim](#comment%3A11):\n> > So then I guess you need something like an \"Alexandroff extension functor\" that you could apply to a Parent to adjoin it an element at infinity, don't you?\n\n> \n> If that means given an ordered set `S`, we can add an element `oo` such that `x < oo` for all in `S`, then yes.\n\nWell, the reference to Alexandroff extensions is not appropriate then, but the idea would be the same.\n\nIn any case, let me ask again: is the following really what you want?\n\n```\nsage: matrix([[Infinity], [1]])\n[               +Infinity]\n[A positive finite number]\n```",
    "created_at": "2015-04-09T07:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247099",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:15" align="right">Comment 15</div>

Replying to [@tscrim](#comment%3A11):
> > So then I guess you need something like an "Alexandroff extension functor" that you could apply to a Parent to adjoin it an element at infinity, don't you?

> 
> If that means given an ordered set `S`, we can add an element `oo` such that `x < oo` for all in `S`, then yes.

Well, the reference to Alexandroff extensions is not appropriate then, but the idea would be the same.

In any case, let me ask again: is the following really what you want?

```
sage: matrix([[Infinity], [1]])
[               +Infinity]
[A positive finite number]
```



---

archive/issue_comments_247100.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nI believe not. What we wish to have is something like this:\n\n```\nsage: Matrix([[oo, 3],[2,3]])\n[+Infinity         1]\n[        2         3]\nsage: M.parent()\nFull MatrixSpace of 2 by 2 dense matrices over (NN\\cup \\{oo\\}) <--------------\n```\n\n... but this is not related to the ticket description. If the ticket is fixed, it will be enough for now.",
    "created_at": "2015-04-13T13:32:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247100",
    "user": "https://github.com/jplab"
}
```

<div id="comment:16" align="right">Comment 16</div>

I believe not. What we wish to have is something like this:

```
sage: Matrix([[oo, 3],[2,3]])
[+Infinity         1]
[        2         3]
sage: M.parent()
Full MatrixSpace of 2 by 2 dense matrices over (NN\cup \{oo\}) <--------------
```

... but this is not related to the ticket description. If the ticket is fixed, it will be enough for now.



---

archive/issue_events_238276.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-04-15T06:39:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238276"
}
```



---

archive/issue_events_238277.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-04-15T06:39:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "milestone_number": null,
    "milestone_title": "sage-6.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238277"
}
```



---

archive/issue_comments_247101.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nComing back to the description of the ticket, does it make sense for Infinityring to have the two methods\n\nis_zero() and is_commutative()?\n\nIMHO, the answer is yes and the patch fixes this. This is the problem raised in the ticket. Otherwise, the description should be changed to be more precise.",
    "created_at": "2015-04-29T09:54:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247101",
    "user": "https://github.com/jplab"
}
```

<div id="comment:18" align="right">Comment 18</div>

Coming back to the description of the ticket, does it make sense for Infinityring to have the two methods

is_zero() and is_commutative()?

IMHO, the answer is yes and the patch fixes this. This is the problem raised in the ticket. Otherwise, the description should be changed to be more precise.



---

archive/issue_events_238278.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2015-04-29T15:56:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238278"
}
```



---

archive/issue_events_238279.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2015-04-29T15:56:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238279"
}
```



---

archive/issue_comments_247102.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, a numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.\n+Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.\n \n ```\n sage: m = Matrix([Infinity])\n``````\n",
    "created_at": "2015-04-29T15:56:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247102",
    "user": "https://github.com/jplab"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, a numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.
+Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.
 
 ```
 sage: m = Matrix([Infinity])
``````




---

archive/issue_comments_247103.json:
```json
{
    "body": "Reviewer: **Jean-Philippe Labb\u00e9**",
    "created_at": "2015-04-29T15:56:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247103",
    "user": "https://github.com/jplab"
}
```

Reviewer: **Jean-Philippe Labbé**



---

archive/issue_comments_247104.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nHi Aladin,\n\nCould you please do the following\n\n-add your full name as an Author,\n-make the title of the ticket reflect the addition of the two methods.\n\nThis ticket is not about matrix with Infinity, but rather about the fact that we need two methods on the infinity ring.",
    "created_at": "2015-04-29T15:56:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247104",
    "user": "https://github.com/jplab"
}
```

<div id="comment:19" align="right">Comment 19</div>

Hi Aladin,

Could you please do the following

-add your full name as an Author,
-make the title of the ticket reflect the addition of the two methods.

This ticket is not about matrix with Infinity, but rather about the fact that we need two methods on the infinity ring.



---

archive/issue_comments_247105.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n+Infinity ring is commutative and non-zero.\n+\n Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.\n \n ```\n``````\n",
    "created_at": "2015-05-03T18:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247105",
    "user": "https://github.com/avirmaux"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
+Infinity ring is commutative and non-zero.
+
 Because there is no `is_zero` and `is_commutative` method for `InfinityRing`, numerous methods fails for matrices with infinity elements. This problem came up in relation with the ticket #17798.
 
 ```
``````




---

archive/issue_comments_247106.json:
```json
{
    "body": "Author: **Aladin Virmaux**",
    "created_at": "2015-05-03T18:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247106",
    "user": "https://github.com/avirmaux"
}
```

Author: **Aladin Virmaux**



---

archive/issue_events_238280.json:
```json
{
    "actor": "https://github.com/avirmaux",
    "created_at": "2015-05-03T18:19:16Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "title_is": "Implements is_zero and is_commutative for infinity ring",
    "title_was": "Matrix with Infinity",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238280"
}
```



---

archive/issue_comments_247107.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nHey, sorry for the delay but I have been really busy these last days. I have also merged develop and corrected a few PEP8.",
    "created_at": "2015-05-03T18:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247107",
    "user": "https://github.com/avirmaux"
}
```

<div id="comment:20" align="right">Comment 20</div>

Hey, sorry for the delay but I have been really busy these last days. I have also merged develop and corrected a few PEP8.



---

archive/issue_comments_247108.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/610c335001404357235a8032b364ad3a26c98fd5\">610c335</a></td><td><code>Merge branch 'develop' into t/17990/matrix_with_infinity</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/76307b8af01f0af127a7a2abb2cd79851f59028e\">76307b8</a></td><td><code>PEP 8</code></td></tr></table>\n",
    "created_at": "2015-05-03T18:25:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247108",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:21" align="right">Comment 21</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/610c335001404357235a8032b364ad3a26c98fd5">610c335</a></td><td><code>Merge branch 'develop' into t/17990/matrix_with_infinity</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/76307b8af01f0af127a7a2abb2cd79851f59028e">76307b8</a></td><td><code>PEP 8</code></td></tr></table>




---

archive/issue_comments_247109.json:
```json
{
    "body": "Changed commit from **[3626313](https://github.com/sagemath/sagetrac-mirror/commit/36263134d2c65c97ef3e57e8f0efa2e8ba0b52b9)** to **[76307b8](https://github.com/sagemath/sagetrac-mirror/commit/76307b8af01f0af127a7a2abb2cd79851f59028e)**",
    "created_at": "2015-05-03T18:25:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247109",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3626313](https://github.com/sagemath/sagetrac-mirror/commit/36263134d2c65c97ef3e57e8f0efa2e8ba0b52b9)** to **[76307b8](https://github.com/sagemath/sagetrac-mirror/commit/76307b8af01f0af127a7a2abb2cd79851f59028e)**



---

archive/issue_comments_247110.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nThis looks good to me.",
    "created_at": "2015-05-06T08:57:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247110",
    "user": "https://github.com/jplab"
}
```

<div id="comment:22" align="right">Comment 22</div>

This looks good to me.



---

archive/issue_events_238281.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2015-05-06T08:57:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238281"
}
```



---

archive/issue_events_238282.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2015-05-06T08:57:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238282"
}
```



---

archive/issue_events_238283.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-06T23:57:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238283"
}
```



---

archive/issue_events_238284.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3b41d147260bd682dc51e247925be07f523a165d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-05-06T23:57:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17990#event-238284"
}
```



---

archive/issue_comments_247111.json:
```json
{
    "body": "Changed branch from **[u/virmaux/matrix_with_infinity](https://github.com/sagemath/sagetrac-mirror/tree/u/virmaux/matrix_with_infinity)** to **[76307b8](https://github.com/sagemath/sagetrac-mirror/commit/76307b8af01f0af127a7a2abb2cd79851f59028e)**",
    "created_at": "2015-05-06T23:57:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17990#issuecomment-247111",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/virmaux/matrix_with_infinity](https://github.com/sagemath/sagetrac-mirror/tree/u/virmaux/matrix_with_infinity)** to **[76307b8](https://github.com/sagemath/sagetrac-mirror/commit/76307b8af01f0af127a7a2abb2cd79851f59028e)**
