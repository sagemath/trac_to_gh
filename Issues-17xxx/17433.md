# Issue 17433: projective point equality fails for quotient base rings

archive/issues_017196.json:
```json
{
    "assignees": [],
    "body": "Since the coordinates of the points are lifted, the current equality check fails to detect equality over quotient rings. The comparisons need to be done in the base ring.\n\n```\nP.<x,y>=ProjectiveSpace(Zmod(10),1)\np1=P(1/3,1)\np2=P.point([1,3],False)\np1==p2\n```\n\nAssignee: **@bhutz**\n\nBranch: **[660e624](https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32)**\n\nReviewer: **Joao Alberto de Faria**\n\nAuthor: **Ben Hutz**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17433_\n\n",
    "closed_at": "2015-01-02T15:46:17Z",
    "created_at": "2014-12-02T21:16:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "projective point equality fails for quotient base rings",
    "type": "issue",
    "updated_at": "2015-02-23T16:24:58Z",
    "url": "https://github.com/sagemath/sage/issues/17433",
    "user": "https://github.com/bhutz"
}
```
Since the coordinates of the points are lifted, the current equality check fails to detect equality over quotient rings. The comparisons need to be done in the base ring.

```
P.<x,y>=ProjectiveSpace(Zmod(10),1)
p1=P(1/3,1)
p2=P.point([1,3],False)
p1==p2
```

Assignee: **@bhutz**

Branch: **[660e624](https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32)**

Reviewer: **Joao Alberto de Faria**

Author: **Ben Hutz**

_Issue created by migration from https://trac.sagemath.org/ticket/17433_





---

archive/issue_events_230848.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-02T21:16:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230848"
}
```



---

archive/issue_events_230849.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-02T21:16:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230849"
}
```



---

archive/issue_events_230850.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-02T21:16:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230850"
}
```



---

archive/issue_events_230851.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-02T21:16:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230851"
}
```



---

archive/issue_comments_235660.json:
```json
{
    "body": "Branch: **[u/bhutz/ticket/17433](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/ticket/17433)**",
    "created_at": "2014-12-08T15:59:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235660",
    "user": "https://github.com/bhutz"
}
```

Branch: **[u/bhutz/ticket/17433](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/ticket/17433)**



---

archive/issue_comments_235661.json:
```json
{
    "body": "Commit: **[41a5b45](https://github.com/sagemath/sagetrac-mirror/commit/41a5b454cb321fc8e5d8fc058b7be77bdf2c6522)**",
    "created_at": "2014-12-08T16:07:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235661",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[41a5b45](https://github.com/sagemath/sagetrac-mirror/commit/41a5b454cb321fc8e5d8fc058b7be77bdf2c6522)**



---

archive/issue_comments_235662.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/41a5b454cb321fc8e5d8fc058b7be77bdf2c6522\">41a5b45</a></td><td><code>17433: add more examples</code></td></tr></table>\n",
    "created_at": "2014-12-08T16:07:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235662",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/41a5b454cb321fc8e5d8fc058b7be77bdf2c6522">41a5b45</a></td><td><code>17433: add more examples</code></td></tr></table>




---

archive/issue_comments_235663.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8e3e894f5f5fac85062f7c035eaf273ca782fa52\">8e3e894</a></td><td><code>17433: fixed some typos</code></td></tr></table>\n",
    "created_at": "2014-12-08T16:10:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235663",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8e3e894f5f5fac85062f7c035eaf273ca782fa52">8e3e894</a></td><td><code>17433: fixed some typos</code></td></tr></table>




---

archive/issue_comments_235664.json:
```json
{
    "body": "Changed commit from **[41a5b45](https://github.com/sagemath/sagetrac-mirror/commit/41a5b454cb321fc8e5d8fc058b7be77bdf2c6522)** to **[8e3e894](https://github.com/sagemath/sagetrac-mirror/commit/8e3e894f5f5fac85062f7c035eaf273ca782fa52)**",
    "created_at": "2014-12-08T16:10:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235664",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[41a5b45](https://github.com/sagemath/sagetrac-mirror/commit/41a5b454cb321fc8e5d8fc058b7be77bdf2c6522)** to **[8e3e894](https://github.com/sagemath/sagetrac-mirror/commit/8e3e894f5f5fac85062f7c035eaf273ca782fa52)**



---

archive/issue_comments_235665.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThe fix has the coordinate of the points in the base ring. This was true for fields but not for rings (the coordinates of quotient rings were lifted). Now it is consistent and this fixes the equality comparison as well.",
    "created_at": "2014-12-08T16:12:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235665",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:4" align="right">Comment 4</div>

The fix has the coordinate of the points in the base ring. This was true for fields but not for rings (the coordinates of quotient rings were lifted). Now it is consistent and this fixes the equality comparison as well.



---

archive/issue_events_230852.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-08T16:12:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230852"
}
```



---

archive/issue_events_230853.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-08T16:12:20Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "title_is": "projective point equality fails for quotient base rings",
    "title_was": "projective point equality fails for quoteitn base rings",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230853"
}
```



---

archive/issue_comments_235666.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLooked over the code, everything ran clean on my end",
    "created_at": "2014-12-12T16:03:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235666",
    "user": "https://github.com/sagetrac-jdefaria"
}
```

<div id="comment:5" align="right">Comment 5</div>

Looked over the code, everything ran clean on my end



---

archive/issue_events_230854.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2014-12-12T16:03:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230854"
}
```



---

archive/issue_events_230855.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2014-12-12T16:03:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230855"
}
```



---

archive/issue_comments_235667.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nAuthor/reviewer name missing",
    "created_at": "2014-12-15T23:25:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235667",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:6" align="right">Comment 6</div>

Author/reviewer name missing



---

archive/issue_events_230856.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-12-15T23:25:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230856"
}
```



---

archive/issue_events_230857.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-12-15T23:25:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230857"
}
```



---

archive/issue_comments_235668.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\n**Ping** Plus a rebase is needed.",
    "created_at": "2014-12-21T01:48:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235668",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:7" align="right">Comment 7</div>

**Ping** Plus a rebase is needed.



---

archive/issue_comments_235669.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/15aa61f96defa91781e02ad90f6649544df45d35\">15aa61f</a></td><td><code>17433: merge in sage.5.beta3</code></td></tr></table>\n",
    "created_at": "2014-12-21T17:01:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235669",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/15aa61f96defa91781e02ad90f6649544df45d35">15aa61f</a></td><td><code>17433: merge in sage.5.beta3</code></td></tr></table>




---

archive/issue_comments_235670.json:
```json
{
    "body": "Changed commit from **[8e3e894](https://github.com/sagemath/sagetrac-mirror/commit/8e3e894f5f5fac85062f7c035eaf273ca782fa52)** to **[15aa61f](https://github.com/sagemath/sagetrac-mirror/commit/15aa61f96defa91781e02ad90f6649544df45d35)**",
    "created_at": "2014-12-21T17:01:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235670",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8e3e894](https://github.com/sagemath/sagetrac-mirror/commit/8e3e894f5f5fac85062f7c035eaf273ca782fa52)** to **[15aa61f](https://github.com/sagemath/sagetrac-mirror/commit/15aa61f96defa91781e02ad90f6649544df45d35)**



---

archive/issue_comments_235671.json:
```json
{
    "body": "Changed commit from **[15aa61f](https://github.com/sagemath/sagetrac-mirror/commit/15aa61f96defa91781e02ad90f6649544df45d35)** to **[d208b8b](https://github.com/sagemath/sagetrac-mirror/commit/d208b8bc9ea28c2f499af23ed8751318db5cd348)**",
    "created_at": "2014-12-21T19:52:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235671",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[15aa61f](https://github.com/sagemath/sagetrac-mirror/commit/15aa61f96defa91781e02ad90f6649544df45d35)** to **[d208b8b](https://github.com/sagemath/sagetrac-mirror/commit/d208b8bc9ea28c2f499af23ed8751318db5cd348)**



---

archive/issue_comments_235672.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d208b8bc9ea28c2f499af23ed8751318db5cd348\">d208b8b</a></td><td><code>17433: merge with sage6.5.beta3</code></td></tr></table>\n",
    "created_at": "2014-12-21T19:52:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235672",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d208b8bc9ea28c2f499af23ed8751318db5cd348">d208b8b</a></td><td><code>17433: merge with sage6.5.beta3</code></td></tr></table>




---

archive/issue_comments_235673.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nI thought I did a bad job on the merge since it still wasn't merging with the master. So I backed up one commit and redid the merge. I did resolve what I thought was the conflict in projective_point.py both times...",
    "created_at": "2014-12-21T19:55:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235673",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:10" align="right">Comment 10</div>

I thought I did a bad job on the merge since it still wasn't merging with the master. So I backed up one commit and redid the merge. I did resolve what I thought was the conflict in projective_point.py both times...



---

archive/issue_comments_235674.json:
```json
{
    "body": "Changed commit from **[d208b8b](https://github.com/sagemath/sagetrac-mirror/commit/d208b8bc9ea28c2f499af23ed8751318db5cd348)** to **[73e233b](https://github.com/sagemath/sagetrac-mirror/commit/73e233bcf25322beb943ea3f9c4157f5f97f01bf)**",
    "created_at": "2014-12-21T21:04:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235674",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d208b8b](https://github.com/sagemath/sagetrac-mirror/commit/d208b8bc9ea28c2f499af23ed8751318db5cd348)** to **[73e233b](https://github.com/sagemath/sagetrac-mirror/commit/73e233bcf25322beb943ea3f9c4157f5f97f01bf)**



---

archive/issue_comments_235675.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fc17d20dcc0db275cd063591beed56fab3ae26be\">fc17d20</a></td><td><code>17433: merge with 6.5.beta3</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/73e233bcf25322beb943ea3f9c4157f5f97f01bf\">73e233b</a></td><td><code>17433: fix remaining merge issues</code></td></tr></table>\n",
    "created_at": "2014-12-21T21:04:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235675",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:11" align="right">Comment 11</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fc17d20dcc0db275cd063591beed56fab3ae26be">fc17d20</a></td><td><code>17433: merge with 6.5.beta3</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/73e233bcf25322beb943ea3f9c4157f5f97f01bf">73e233b</a></td><td><code>17433: fix remaining merge issues</code></td></tr></table>




---

archive/issue_events_230858.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-21T21:05:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230858"
}
```



---

archive/issue_events_230859.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-21T21:05:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230859"
}
```



---

archive/issue_comments_235676.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nok. I think I finally have the merge working. I had to split it slightly into two commits.",
    "created_at": "2014-12-21T21:05:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235676",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:12" align="right">Comment 12</div>

ok. I think I finally have the merge working. I had to split it slightly into two commits.



---

archive/issue_comments_235677.json:
```json
{
    "body": "Reviewer: **Joao de Faria**",
    "created_at": "2014-12-21T21:05:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235677",
    "user": "https://github.com/bhutz"
}
```

Reviewer: **Joao de Faria**



---

archive/issue_comments_235678.json:
```json
{
    "body": "Author: **Ben Hutz**",
    "created_at": "2014-12-21T21:05:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235678",
    "user": "https://github.com/bhutz"
}
```

Author: **Ben Hutz**



---

archive/issue_comments_235679.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nNo merge problems (sometimes trac can be fickle). Also I allowed myself to do some trivial whitespace changes.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6c429ea954fdbce84c5bb38ce22c511f1c039c97\">6c429ea</a></td><td><code>Merge branch 'u/bhutz/ticket/17433' of trac.sagemath.org:sage into u/tscrim/ticket/17433</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32\">660e624</a></td><td><code>Trivial whitespace changes.</code></td></tr></table>\n",
    "created_at": "2014-12-21T23:18:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235679",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:13" align="right">Comment 13</div>

No merge problems (sometimes trac can be fickle). Also I allowed myself to do some trivial whitespace changes.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6c429ea954fdbce84c5bb38ce22c511f1c039c97">6c429ea</a></td><td><code>Merge branch 'u/bhutz/ticket/17433' of trac.sagemath.org:sage into u/tscrim/ticket/17433</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32">660e624</a></td><td><code>Trivial whitespace changes.</code></td></tr></table>




---

archive/issue_events_230860.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-12-21T23:18:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230860"
}
```



---

archive/issue_events_230861.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-12-21T23:18:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230861"
}
```



---

archive/issue_comments_235680.json:
```json
{
    "body": "Changed commit from **[73e233b](https://github.com/sagemath/sagetrac-mirror/commit/73e233bcf25322beb943ea3f9c4157f5f97f01bf)** to **[660e624](https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32)**",
    "created_at": "2014-12-21T23:18:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235680",
    "user": "https://github.com/tscrim"
}
```

Changed commit from **[73e233b](https://github.com/sagemath/sagetrac-mirror/commit/73e233bcf25322beb943ea3f9c4157f5f97f01bf)** to **[660e624](https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32)**



---

archive/issue_comments_235681.json:
```json
{
    "body": "Changed branch from **[u/bhutz/ticket/17433](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/ticket/17433)** to **[u/tscrim/ticket/17433](https://github.com/sagemath/sagetrac-mirror/tree/u/tscrim/ticket/17433)**",
    "created_at": "2014-12-21T23:18:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235681",
    "user": "https://github.com/tscrim"
}
```

Changed branch from **[u/bhutz/ticket/17433](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/ticket/17433)** to **[u/tscrim/ticket/17433](https://github.com/sagemath/sagetrac-mirror/tree/u/tscrim/ticket/17433)**



---

archive/issue_comments_235682.json:
```json
{
    "body": "Changed branch from **[u/tscrim/ticket/17433](https://github.com/sagemath/sagetrac-mirror/tree/u/tscrim/ticket/17433)** to **[660e624](https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32)**",
    "created_at": "2015-01-02T15:46:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235682",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/tscrim/ticket/17433](https://github.com/sagemath/sagetrac-mirror/tree/u/tscrim/ticket/17433)** to **[660e624](https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32)**



---

archive/issue_events_230862.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-02T15:46:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230862"
}
```



---

archive/issue_events_230863.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "84fa5b6dfbc9b16048db909409086ee668fabde7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-02T15:46:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17433#event-230863"
}
```



---

archive/issue_comments_235683.json:
```json
{
    "body": "Changed commit from **[660e624](https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32)** to none",
    "created_at": "2015-02-23T16:24:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235683",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[660e624](https://github.com/sagemath/sagetrac-mirror/commit/660e6247fa4aaa927b302fb191dd3ed71df05b32)** to none



---

archive/issue_comments_235684.json:
```json
{
    "body": "Changed reviewer from **Joao de Faria** to **Joao Alberto de Faria**",
    "created_at": "2015-02-23T16:24:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17433#issuecomment-235684",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Joao de Faria** to **Joao Alberto de Faria**
