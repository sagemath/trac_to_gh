# Issue 17128: Problem with solution_dict=True and solving for multiple variables

archive/issues_016891.json:
```json
{
    "assignees": [],
    "body": "This bug was reported at http://ask.sagemath.org/question/24446/problem-with-solution_dicttrue/:\n\n```\nsage: var('x y')\n(x, y)\nsage: sol=solve(x + y, [x, y], solution_dict=True)\n---------------------------------------------------------------------------\nAttributeError                            Traceback (most recent call last)\n<ipython-input-2-d24a5a0d9f55> in <module>()\n----> 1 sol=solve(x + y, [x, y], solution_dict=True)\n\n/Users/palmieri/Desktop/Sage_stuff/git/sage/local/lib/python2.7/site-packages/sage/symbolic/relation.pyc in solve(f, *args, **kwds)\n    732     from sage.symbolic.expression import is_Expression\n    733     if is_Expression(f): # f is a single expression\n--> 734         ans = f.solve(*args,**kwds)\n    735         return ans\n    736 \n\n/Users/palmieri/Desktop/Sage_stuff/git/sage/local/lib/python2.7/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.solve (build/cythonized/sage/symbolic/expression.cpp:47600)()\n\nAttributeError: 'list' object has no attribute 'left'\n```\nI think this was introduced in #16531.\n\nBranch/Commit: **[b26dbd4](https://github.com/sagemath/sagetrac-mirror/commit/b26dbd46534be67d92bd808d8f45a6519aac957e)**\n\nReviewer: **Ralf Stephan**\n\nAuthor: **Miguel Marco**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17128_\n\n",
    "closed_at": "2014-11-06T11:15:54Z",
    "created_at": "2014-10-10T05:57:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Problem with solution_dict=True and solving for multiple variables",
    "type": "issue",
    "updated_at": "2014-11-06T11:15:54Z",
    "url": "https://github.com/sagemath/sage/issues/17128",
    "user": "https://github.com/jhpalmieri"
}
```
This bug was reported at http://ask.sagemath.org/question/24446/problem-with-solution_dicttrue/:

```
sage: var('x y')
(x, y)
sage: sol=solve(x + y, [x, y], solution_dict=True)
---------------------------------------------------------------------------
AttributeError                            Traceback (most recent call last)
<ipython-input-2-d24a5a0d9f55> in <module>()
----> 1 sol=solve(x + y, [x, y], solution_dict=True)

/Users/palmieri/Desktop/Sage_stuff/git/sage/local/lib/python2.7/site-packages/sage/symbolic/relation.pyc in solve(f, *args, **kwds)
    732     from sage.symbolic.expression import is_Expression
    733     if is_Expression(f): # f is a single expression
--> 734         ans = f.solve(*args,**kwds)
    735         return ans
    736 

/Users/palmieri/Desktop/Sage_stuff/git/sage/local/lib/python2.7/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.solve (build/cythonized/sage/symbolic/expression.cpp:47600)()

AttributeError: 'list' object has no attribute 'left'
```
I think this was introduced in #16531.

Branch/Commit: **[b26dbd4](https://github.com/sagemath/sagetrac-mirror/commit/b26dbd46534be67d92bd808d8f45a6519aac957e)**

Reviewer: **Ralf Stephan**

Author: **Miguel Marco**

_Issue created by migration from https://trac.sagemath.org/ticket/17128_





---

archive/issue_events_226950.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-10-10T05:57:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226950"
}
```



---

archive/issue_events_226951.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-10-10T05:57:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226951"
}
```



---

archive/issue_events_226952.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-10-10T05:57:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226952"
}
```



---

archive/issue_events_226953.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-10-10T05:57:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226953"
}
```



---

archive/issue_comments_229650.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe problem arises in the conversion to dictionary form. This works ok:\n\n```\nsage: var('x,y')\n(x, y)\nsage: solve(x+y,[x,y])\n[[x == -r1, y == r1]]\n\n\n\n```\n\n\nIt should be easy to fix.",
    "created_at": "2014-10-10T12:52:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229650",
    "user": "https://github.com/miguelmarco"
}
```

<a id='comment:1'>Comment 1:</a>
The problem arises in the conversion to dictionary form. This works ok:

```
sage: var('x,y')
(x, y)
sage: solve(x+y,[x,y])
[[x == -r1, y == r1]]



```


It should be easy to fix.



---

archive/issue_comments_229651.json:
```json
{
    "body": "Branch: **[u/mmarco/ticket/17128](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/ticket/17128)**",
    "created_at": "2014-10-10T14:23:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229651",
    "user": "https://github.com/miguelmarco"
}
```

Branch: **[u/mmarco/ticket/17128](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/ticket/17128)**



---

archive/issue_comments_229652.json:
```json
{
    "body": "Commit: **[88d0e11](https://github.com/sagemath/sagetrac-mirror/commit/88d0e11df6fdfdc6e11c1a9b30e09265c5e3c5fa)**",
    "created_at": "2014-10-10T14:23:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229652",
    "user": "https://github.com/miguelmarco"
}
```

Commit: **[88d0e11](https://github.com/sagemath/sagetrac-mirror/commit/88d0e11df6fdfdc6e11c1a9b30e09265c5e3c5fa)**



---

archive/issue_comments_229653.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis should fix it.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/88d0e11df6fdfdc6e11c1a9b30e09265c5e3c5fa\">88d0e11</a></td><td><code>Fix error converting solutions to dict when solving for multiple variables.</code></td></tr></table>\n",
    "created_at": "2014-10-10T14:23:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229653",
    "user": "https://github.com/miguelmarco"
}
```

<a id='comment:3'>Comment 3:</a>
This should fix it.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/88d0e11df6fdfdc6e11c1a9b30e09265c5e3c5fa">88d0e11</a></td><td><code>Fix error converting solutions to dict when solving for multiple variables.</code></td></tr></table>




---

archive/issue_events_226954.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2014-10-10T14:24:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226954"
}
```



---

archive/issue_comments_229654.json:
```json
{
    "body": "Changed branch from **[u/mmarco/ticket/17128](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/ticket/17128)** to **[public/17128](https://github.com/sagemath/sagetrac-mirror/tree/public/17128)**",
    "created_at": "2014-11-02T09:21:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229654",
    "user": "https://github.com/rwst"
}
```

Changed branch from **[u/mmarco/ticket/17128](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/ticket/17128)** to **[public/17128](https://github.com/sagemath/sagetrac-mirror/tree/public/17128)**



---

archive/issue_comments_229655.json:
```json
{
    "body": "Author: **Miguel Marco**",
    "created_at": "2014-11-02T09:24:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229655",
    "user": "https://github.com/rwst"
}
```

Author: **Miguel Marco**



---

archive/issue_comments_229656.json:
```json
{
    "body": "Changed commit from **[88d0e11](https://github.com/sagemath/sagetrac-mirror/commit/88d0e11df6fdfdc6e11c1a9b30e09265c5e3c5fa)** to **[06d61ab](https://github.com/sagemath/sagetrac-mirror/commit/06d61ab93c522511d2e4c60e9953931f2190d858)**",
    "created_at": "2014-11-02T09:24:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229656",
    "user": "https://github.com/rwst"
}
```

Changed commit from **[88d0e11](https://github.com/sagemath/sagetrac-mirror/commit/88d0e11df6fdfdc6e11c1a9b30e09265c5e3c5fa)** to **[06d61ab](https://github.com/sagemath/sagetrac-mirror/commit/06d61ab93c522511d2e4c60e9953931f2190d858)**



---

archive/issue_comments_229657.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2014-11-02T09:24:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229657",
    "user": "https://github.com/rwst"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_events_226955.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-11-02T09:24:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226955"
}
```



---

archive/issue_events_226956.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-11-02T09:24:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226956"
}
```



---

archive/issue_comments_229658.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThe doctest wold only pass with the `r7` variable so I added wildcards. Else a minimal patch.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ee447724c348841b5d5b5d5ceead14172164a421\">ee44772</a></td><td><code>Merge branch 'develop' into t/17128/ticket/17128</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/06d61ab93c522511d2e4c60e9953931f2190d858\">06d61ab</a></td><td><code>17128: reviewer's patch: make doctest pass with any var</code></td></tr></table>\n",
    "created_at": "2014-11-02T09:24:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229658",
    "user": "https://github.com/rwst"
}
```

<a id='comment:6'>Comment 6:</a>
The doctest wold only pass with the `r7` variable so I added wildcards. Else a minimal patch.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ee447724c348841b5d5b5d5ceead14172164a421">ee44772</a></td><td><code>Merge branch 'develop' into t/17128/ticket/17128</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/06d61ab93c522511d2e4c60e9953931f2190d858">06d61ab</a></td><td><code>17128: reviewer's patch: make doctest pass with any var</code></td></tr></table>




---

archive/issue_comments_229659.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nvariable sort order is random:\n\n```\nsage -t --long src/sage/symbolic/expression.pyx\n**********************************************************************\nFile \"src/sage/symbolic/expression.pyx\", line 9448, in sage.symbolic.expression.Expression.solve\nFailed example:\n    f.solve([x, y], solution_dict=True)\nExpected:\n    [{x: -r..., y: r...}]\nGot:\n    [{y: r8, x: -r8}]\n**********************************************************************\n```",
    "created_at": "2014-11-03T12:45:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229659",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:7'>Comment 7:</a>
variable sort order is random:

```
sage -t --long src/sage/symbolic/expression.pyx
**********************************************************************
File "src/sage/symbolic/expression.pyx", line 9448, in sage.symbolic.expression.Expression.solve
Failed example:
    f.solve([x, y], solution_dict=True)
Expected:
    [{x: -r..., y: r...}]
Got:
    [{y: r8, x: -r8}]
**********************************************************************
```



---

archive/issue_events_226957.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-03T12:45:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226957"
}
```



---

archive/issue_events_226958.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-03T12:45:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226958"
}
```



---

archive/issue_comments_229660.json:
```json
{
    "body": "Changed commit from **[06d61ab](https://github.com/sagemath/sagetrac-mirror/commit/06d61ab93c522511d2e4c60e9953931f2190d858)** to **[b26dbd4](https://github.com/sagemath/sagetrac-mirror/commit/b26dbd46534be67d92bd808d8f45a6519aac957e)**",
    "created_at": "2014-11-03T15:59:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229660",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[06d61ab](https://github.com/sagemath/sagetrac-mirror/commit/06d61ab93c522511d2e4c60e9953931f2190d858)** to **[b26dbd4](https://github.com/sagemath/sagetrac-mirror/commit/b26dbd46534be67d92bd808d8f45a6519aac957e)**



---

archive/issue_comments_229661.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8ef5b8313eade52f50e8143455f2efbb6c8e15fd\">8ef5b83</a></td><td><code>Merge branch 'develop' into t/17128/public/17128</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b26dbd46534be67d92bd808d8f45a6519aac957e\">b26dbd4</a></td><td><code>17128: fix doctest</code></td></tr></table>\n",
    "created_at": "2014-11-03T15:59:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229661",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:8'>Comment 8:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8ef5b8313eade52f50e8143455f2efbb6c8e15fd">8ef5b83</a></td><td><code>Merge branch 'develop' into t/17128/public/17128</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b26dbd46534be67d92bd808d8f45a6519aac957e">b26dbd4</a></td><td><code>17128: fix doctest</code></td></tr></table>




---

archive/issue_events_226959.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-11-03T16:00:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226959"
}
```



---

archive/issue_events_226960.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-11-03T16:00:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226960"
}
```



---

archive/issue_events_226961.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-06T11:15:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226961"
}
```



---

archive/issue_events_226962.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cfeb230dd5702f4b50ba3b2584fb35b1ac60e819",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-11-06T11:15:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17128#event-226962"
}
```



---

archive/issue_comments_229662.json:
```json
{
    "body": "Changed branch from **[public/17128](https://github.com/sagemath/sagetrac-mirror/tree/public/17128)** to **[b26dbd4](https://github.com/sagemath/sagetrac-mirror/commit/b26dbd46534be67d92bd808d8f45a6519aac957e)**",
    "created_at": "2014-11-06T11:15:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17128#issuecomment-229662",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/17128](https://github.com/sagemath/sagetrac-mirror/tree/public/17128)** to **[b26dbd4](https://github.com/sagemath/sagetrac-mirror/commit/b26dbd46534be67d92bd808d8f45a6519aac957e)**
