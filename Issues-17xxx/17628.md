# Issue 17628: Wrap gap.AllGroups

archive/issues_017391.json:
```json
{
    "assignees": [],
    "body": "We should provide a wrapper for `gap.AllGroups` feature to iterate through all groups of given order.\n\nCC:  @videlec @williamstein @nathanncohen\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17628_\n\n",
    "created_at": "2015-01-12T21:06:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Wrap gap.AllGroups",
    "type": "issue",
    "updated_at": "2022-12-29T01:30:11Z",
    "url": "https://github.com/sagemath/sage/issues/17628",
    "user": "https://github.com/jdemeyer"
}
```
We should provide a wrapper for `gap.AllGroups` feature to iterate through all groups of given order.

CC:  @videlec @williamstein @nathanncohen

_Issue created by migration from https://trac.sagemath.org/ticket/17628_





---

archive/issue_events_233424.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-12T21:06:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17628",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17628#event-233424"
}
```



---

archive/issue_events_233425.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-12T21:06:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17628",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17628#event-233425"
}
```



---

archive/issue_events_233426.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-12T21:06:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17628",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17628#event-233426"
}
```



---

archive/issue_events_233427.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-12T21:06:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17628",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17628#event-233427"
}
```



---

archive/issue_comments_239702.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis actually needs more than what I thought. The output of `AllGroups` in GAP is not uniform... it can be:\n- a pc group if it is solvable (testable from GAP with `IsPcGroup`)\n- a permutation group otherwise (testable from GAP with `IsPermGroup`)",
    "created_at": "2015-01-12T21:39:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17628#issuecomment-239702",
    "user": "https://github.com/videlec"
}
```

<a id='comment:2'>Comment 2:</a>
This actually needs more than what I thought. The output of `AllGroups` in GAP is not uniform... it can be:
- a pc group if it is solvable (testable from GAP with `IsPcGroup`)
- a permutation group otherwise (testable from GAP with `IsPermGroup`)



---

archive/issue_events_233428.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:30:11Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17628",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17628#event-233428"
}
```
