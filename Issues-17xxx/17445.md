...
```json
{
    "body": "Okay, at least now I actually understand what the different tickets are about.  Phew.",
...
```json
{
    "body": "It would probably be good to add a note that `D[0, 1](f)(x, y)` does not in general equal `D[1, 0](f)(x, y)` (https://en.wikipedia.org/wiki/Symmetry_of_second_derivatives).",
...
```json
{
    "body": "Following on from the logic about function('f', x) being an expression, not a function, why does this fail, then?\n\n```\n# If all of f, g and p are expressions, why does this fail?\nx = var('x')\nf = function('f', x)\nprint type(f)\ng = function('g', x)\nprint type(g)\np = f.diff()\nprint type(p)\nprint p.subs_expr(f==g) # Outputs \"D[0](f)(x)\"\n```",
