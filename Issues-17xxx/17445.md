...
<a id='comment:1'></a>OK, I do not know where it should go in the documentation for best visibility (I'd think in "diff" somewhere), but the explanation should be along the lines of:
...
<a id='comment:2'></a>Are there times where this notation is ambiguous, though?  I seem to recall that being the case.
...
<a id='comment:4'></a>Can you comment on some of the chain-rule type issues in #6480, then?  I have to say that in particular the stuff at http://ask.sagemath.org/question/9932/how-to-substitute-a-function-within-derivatives/ and #6480 is massively confusing.  Heck, let's add #7401 while we're at it.  
...
```json
{
    "body": "<a id='comment:5'></a>Okay, at least now I actually understand what the different tickets are about.  Phew.",
...
<a id='comment:5'></a>Okay, at least now I actually understand what the different tickets are about.  Phew.
...
```json
{
    "body": "<a id='comment:7'></a>It would probably be good to add a note that `D[0, 1](f)(x, y)` does not in general equal `D[1, 0](f)(x, y)` (https://en.wikipedia.org/wiki/Symmetry_of_second_derivatives).",
...
<a id='comment:7'></a>It would probably be good to add a note that `D[0, 1](f)(x, y)` does not in general equal `D[1, 0](f)(x, y)` (https://en.wikipedia.org/wiki/Symmetry_of_second_derivatives).
...
```json
{
    "body": "<a id='comment:9'></a>Following on from the logic about function('f', x) being an expression, not a function, why does this fail, then?\n\n```\n# If all of f, g and p are expressions, why does this fail?\nx = var('x')\nf = function('f', x)\nprint type(f)\ng = function('g', x)\nprint type(g)\np = f.diff()\nprint type(p)\nprint p.subs_expr(f==g) # Outputs \"D[0](f)(x)\"\n```",
...
<a id='comment:9'></a>Following on from the logic about function('f', x) being an expression, not a function, why does this fail, then?
...
<a id='comment:10'></a>Replying to [eviatarbach](#comment%3A7):
> It would probably be good to add a note that `D[0, 1](f)(x, y)` does not in general equal `D[1, 0](f)(x, y)` (https://en.wikipedia.org/wiki/Symmetry_of_second_derivatives).
> 
...
<a id='comment:11'></a>Replying to [nbruin](#comment%3A6):
...
<a id='comment:12'></a>> I was expecting the second to give either `D[0, 1](f)(y, x)` or `D[1, 0](f)(x, y)`. What is going on, is the order of differentiations not honoured in the notation?
...
<a id='comment:13'></a>Replying to [schymans](#comment%3A10):
> I would much prefer this behaviour:
> 
> ```
> sage: f.diff(x,y) == f.diff(y,x)
...
<a id='comment:14'></a>Replying to [schymans](#comment%3A9):
> {{{
> print p.subs_expr(f==g) # Outputs "D[0](f)(x)"
> }}}
...
<a id='comment:15'></a>Replying to [schymans](#comment%3A10):
> I was expecting the second to give either `D[0, 1](f)(y, x)` or `D[1, 0](f)(x, y)`. What is going on, is the order of differentiations not honoured in the notation?
...
<a id='comment:16'></a>Replying to [kcrisman](#comment%3A12):
> Questions:
> * Is it worth trying to distinguish `D[0,1]` and `D[1,0]`?
...
<a id='comment:17'></a>I think it is worth distinguishing. The standard counterexample to equality of mixed partials is `f(x, y) = x*y*(x^2 - y<sup>2)/(x</sup>2 + y^2)`, with `f(0, 0) = 0`, which is a continuous function but the second derivatives at (0, 0) are unequal. This may not come about often in practice (it doesn't seem like we can run this example without having multivariable piecewise functions), but especially if people are using the formal functions it should have correct mathematical properties.
...
<a id='comment:18'></a>Wow, I think the description of this ticket should be changed. It is not any more about the documentation of the D[] notation, but about a meaningful way of using and displaying symbolic differentials. 
...
<a id='comment:19'></a>Replying to [schymans](#comment%3A18):
...
<a id='comment:20'></a>Thanks, Nils, I didn't see that. However, I am still not able to achieve what I was hoping for. Let me give a pratical example. I define an expression for pressure following the ideal gas law:
...
<a id='comment:21'></a>Replying to [schymans](#comment%3A20):
> {{{
> sage: function('p',t)
> sage: function('n', t)
> sage: function('T', t)
...
<a id='comment:23'></a>Related user questions
