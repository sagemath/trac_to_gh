# Issue 17699: Remove bashisms in src/bin shell scripts

archive/issues_017462.json:
```json
{
    "assignees": [],
    "body": "It seems to increase portability, see e.g. #17513.\n\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17699_\n\n",
    "closed_at": "2015-02-16T23:09:58Z",
    "created_at": "2015-01-30T16:21:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove bashisms in src/bin shell scripts",
    "type": "issue",
    "updated_at": "2015-02-16T23:09:58Z",
    "url": "https://github.com/sagemath/sage/issues/17699",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
It seems to increase portability, see e.g. #17513.


Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/17699_





---

archive/issue_events_234517.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-01-30T16:21:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234517"
}
```



---

archive/issue_events_234518.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-01-30T16:21:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234518"
}
```



---

archive/issue_events_234519.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-01-30T16:21:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234519"
}
```



---

archive/issue_events_234520.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-01-30T16:21:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234520"
}
```



---

archive/issue_events_234521.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-31T08:49:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234521"
}
```



---

archive/issue_events_234522.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-31T08:49:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234522"
}
```



---

archive/issue_events_234523.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-31T08:49:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234523"
}
```



---

archive/issue_comments_241566.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nWe have been assuming `bash` for a very long time. Removing bashisms would require replacing those scripts by more complicated code, giving more chances of breakage.\n\nAnd if you stop assuming `bash` as shell, you have to test that things works on all shells. Again giving more chances of breakage.\n\nProposal: close as \"wontfix\".",
    "created_at": "2015-01-31T08:49:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17699#issuecomment-241566",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">Comment 1</div>

We have been assuming `bash` for a very long time. Removing bashisms would require replacing those scripts by more complicated code, giving more chances of breakage.

And if you stop assuming `bash` as shell, you have to test that things works on all shells. Again giving more chances of breakage.

Proposal: close as "wontfix".



---

archive/issue_comments_241567.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-01-31T08:49:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17699#issuecomment-241567",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_234524.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-31T08:49:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234524"
}
```



---

archive/issue_comments_241568.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAs pointed out by Volker in [#15642 comment:13](https://github.com/sagemath/sage/issues/15642#comment:13) and [1]\n> Python is a requirement. Sooner or later Sage won't build without Python\n\nso the shell scripts could be replaced by python scripts as it is done in #15642.\n\n[1] https://groups.google.com/forum/#!topic/sage-devel/akds04MBQYg",
    "created_at": "2015-01-31T10:37:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17699#issuecomment-241568",
    "user": "https://github.com/a-andre"
}
```

<div id="comment:2" align="right">Comment 2</div>

As pointed out by Volker in [#15642 comment:13](https://github.com/sagemath/sage/issues/15642#comment:13) and [1]
> Python is a requirement. Sooner or later Sage won't build without Python

so the shell scripts could be replaced by python scripts as it is done in #15642.

[1] https://groups.google.com/forum/#!topic/sage-devel/akds04MBQYg



---

archive/issue_events_234525.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-14T15:26:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234525"
}
```



---

archive/issue_events_234526.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-14T15:26:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234526"
}
```



---

archive/issue_events_234527.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-16T23:09:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234527"
}
```



---

archive/issue_events_234528.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-16T23:09:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234528"
}
```



---

archive/issue_events_234529.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-16T23:09:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17699",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17699#event-234529"
}
```
