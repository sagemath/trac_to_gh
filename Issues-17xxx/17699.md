# Issue 17699: Remove bashisms in src/bin shell scripts

archive/issues_017462.json:
```json
{
    "body": "It seems to increase portability, see e.g. #17513.\n\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/17699\n\n",
    "closed_at": "2015-02-16T23:09:58Z",
    "created_at": "2015-01-30T16:21:06Z",
    "labels": [
        "component: scripts",
        "enhancement",
        "wontfix"
    ],
    "title": "Remove bashisms in src/bin shell scripts",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/17699",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```
It seems to increase portability, see e.g. #17513.


**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/17699





---

archive/issue_events_162086.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-31T08:49:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "milestone": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17699#event-162086"
}
```



---

archive/issue_events_162087.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-31T08:49:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17699#event-162087"
}
```



---

archive/issue_comments_243094.json:
```json
{
    "body": "<a id='comment:1'></a>\nWe have been assuming `bash` for a very long time. Removing bashisms would require replacing those scripts by more complicated code, giving more chances of breakage.\n\nAnd if you stop assuming `bash` as shell, you have to test that things works on all shells. Again giving more chances of breakage.\n\nProposal: close as \"wontfix\".",
    "created_at": "2015-01-31T08:49:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17699#issuecomment-243094",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>
We have been assuming `bash` for a very long time. Removing bashisms would require replacing those scripts by more complicated code, giving more chances of breakage.

And if you stop assuming `bash` as shell, you have to test that things works on all shells. Again giving more chances of breakage.

Proposal: close as "wontfix".



---

archive/issue_comments_243095.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2015-01-31T08:49:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17699#issuecomment-243095",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_162088.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-31T08:49:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17699#event-162088"
}
```



---

archive/issue_comments_243096.json:
```json
{
    "body": "<a id='comment:2'></a>\nAs pointed out by Volker in ticket:15642#comment:13 and [1]\n> Python is a requirement. Sooner or later Sage won't build without Python\n\n\nso the shell scripts could be replaced by python scripts as it is done in #15642.\n\n[1] https://groups.google.com/forum/#!topic/sage-devel/akds04MBQYg",
    "created_at": "2015-01-31T10:37:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17699#issuecomment-243096",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:2'></a>
As pointed out by Volker in ticket:15642#comment:13 and [1]
> Python is a requirement. Sooner or later Sage won't build without Python


so the shell scripts could be replaced by python scripts as it is done in #15642.

[1] https://groups.google.com/forum/#!topic/sage-devel/akds04MBQYg



---

archive/issue_events_162089.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-14T15:26:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17699#event-162089"
}
```



---

archive/issue_events_162090.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-14T15:26:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17699#event-162090"
}
```



---

archive/issue_events_162091.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-16T23:09:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17699#event-162091"
}
```



---

archive/issue_events_162092.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-16T23:09:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17699#event-162092"
}
```



---

archive/issue_events_162093.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-16T23:09:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/17699",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17699#event-162093"
}
```
