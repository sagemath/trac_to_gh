# Issue 17043: fix scilab interface

archive/issues_016806.json:
```json
{
    "assignees": [],
    "body": "The Sage Scilab interface is broken.   The fix is pretty trivial, and I guess is a change in a command line flag to scilab:\n\n```\ndiff --git a/src/sage/interfaces/scilab.py b/src/sage/interfaces/scilab.py\nindex bc4a508..7e1d475 100644\n--- a/src/sage/interfaces/scilab.py\n+++ b/src/sage/interfaces/scilab.py\n@@ -218,7 +218,7 @@ class Scilab(Expect):\n         Expect.__init__(self,\n                         name = 'scilab',\n                         prompt = '-->',\n-                        command = \"scilab -nogui\",\n+                        command = \"scilab -nw\",\n                         maxread = maxread,\n                         server = server,\n                         server_tmpdir = server_tmpdir,\n@@ -495,7 +495,7 @@ def scilab_console():\n     Scilab, like Sage, remembers its history from one session to\n     another.\n     \"\"\"\n-    os.system('scilab -nogui')\n+    os.system('scilab -nw')\n```\n\nBranch/Commit: **[dfc49d3](https://github.com/sagemath/sagetrac-mirror/commit/dfc49d364d612b494b0dc83b4e32fbb8129a81ca)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17043_\n\n",
    "closed_at": "2014-09-27T12:57:03Z",
    "created_at": "2014-09-25T23:31:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix scilab interface",
    "type": "issue",
    "updated_at": "2014-09-27T12:57:03Z",
    "url": "https://github.com/sagemath/sage/issues/17043",
    "user": "https://github.com/williamstein"
}
```
The Sage Scilab interface is broken.   The fix is pretty trivial, and I guess is a change in a command line flag to scilab:

```
diff --git a/src/sage/interfaces/scilab.py b/src/sage/interfaces/scilab.py
index bc4a508..7e1d475 100644
--- a/src/sage/interfaces/scilab.py
+++ b/src/sage/interfaces/scilab.py
@@ -218,7 +218,7 @@ class Scilab(Expect):
         Expect.__init__(self,
                         name = 'scilab',
                         prompt = '-->',
-                        command = "scilab -nogui",
+                        command = "scilab -nw",
                         maxread = maxread,
                         server = server,
                         server_tmpdir = server_tmpdir,
@@ -495,7 +495,7 @@ def scilab_console():
     Scilab, like Sage, remembers its history from one session to
     another.
     """
-    os.system('scilab -nogui')
+    os.system('scilab -nw')
```

Branch/Commit: **[dfc49d3](https://github.com/sagemath/sagetrac-mirror/commit/dfc49d364d612b494b0dc83b4e32fbb8129a81ca)**

Reviewer: **Dima Pasechnik**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/17043_





---

archive/issue_events_225914.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2014-09-25T23:31:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17043#event-225914"
}
```



---

archive/issue_events_225915.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2014-09-25T23:31:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17043#event-225915"
}
```



---

archive/issue_events_225916.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2014-09-25T23:31:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17043#event-225916"
}
```



---

archive/issue_events_225917.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2014-09-25T23:31:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17043#event-225917"
}
```



---

archive/issue_comments_228263.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,4 +14,10 @@\n                          maxread = maxread,\n                          server = server,\n                          server_tmpdir = server_tmpdir,\n+@@ -495,7 +495,7 @@ def scilab_console():\n+     Scilab, like Sage, remembers its history from one session to\n+     another.\n+     \"\"\"\n+-    os.system('scilab -nogui')\n++    os.system('scilab -nw')\n ```\n``````\n",
    "created_at": "2014-09-25T23:35:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17043#issuecomment-228263",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,4 +14,10 @@
                          maxread = maxread,
                          server = server,
                          server_tmpdir = server_tmpdir,
+@@ -495,7 +495,7 @@ def scilab_console():
+     Scilab, like Sage, remembers its history from one session to
+     another.
+     """
+-    os.system('scilab -nogui')
++    os.system('scilab -nw')
 ```
``````




---

archive/issue_comments_228264.json:
```json
{
    "body": "Commit: **[dfc49d3](https://github.com/sagemath/sagetrac-mirror/commit/dfc49d364d612b494b0dc83b4e32fbb8129a81ca)**",
    "created_at": "2014-09-26T07:35:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17043#issuecomment-228264",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[dfc49d3](https://github.com/sagemath/sagetrac-mirror/commit/dfc49d364d612b494b0dc83b4e32fbb8129a81ca)**



---

archive/issue_comments_228265.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nDone\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dfc49d364d612b494b0dc83b4e32fbb8129a81ca\">dfc49d3</a></td><td><code>trac #17043 correct call to scilab as required by wstein</code></td></tr></table>\n",
    "created_at": "2014-09-26T07:35:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17043#issuecomment-228265",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

Done

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dfc49d364d612b494b0dc83b4e32fbb8129a81ca">dfc49d3</a></td><td><code>trac #17043 correct call to scilab as required by wstein</code></td></tr></table>




---

archive/issue_comments_228266.json:
```json
{
    "body": "Branch: **[u/chapoton/17043](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/17043)**",
    "created_at": "2014-09-26T07:35:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17043#issuecomment-228266",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/17043](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/17043)**



---

archive/issue_events_225918.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-09-26T07:35:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17043#event-225918"
}
```



---

archive/issue_comments_228267.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2014-09-26T07:35:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17043#issuecomment-228267",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_228268.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM",
    "created_at": "2014-09-26T08:05:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17043#issuecomment-228268",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM



---

archive/issue_events_225919.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-09-26T08:05:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17043#event-225919"
}
```



---

archive/issue_events_225920.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-09-26T08:05:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17043#event-225920"
}
```



---

archive/issue_comments_228269.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2014-09-26T08:19:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17043#issuecomment-228269",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_225921.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-27T12:57:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17043#event-225921"
}
```



---

archive/issue_events_225922.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fdb1ac73bf725a7e296b455e6e3e9c1d0c6d20cb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-09-27T12:57:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17043#event-225922"
}
```



---

archive/issue_comments_228270.json:
```json
{
    "body": "Changed branch from **[u/chapoton/17043](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/17043)** to **[dfc49d3](https://github.com/sagemath/sagetrac-mirror/commit/dfc49d364d612b494b0dc83b4e32fbb8129a81ca)**",
    "created_at": "2014-09-27T12:57:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17043#issuecomment-228270",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/17043](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/17043)** to **[dfc49d3](https://github.com/sagemath/sagetrac-mirror/commit/dfc49d364d612b494b0dc83b4e32fbb8129a81ca)**
