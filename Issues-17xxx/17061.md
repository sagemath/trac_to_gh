# Issue 17061: Posets.DiamondPoset, some corrections

archive/issues_016824.json:
```json
{
    "assignees": [],
    "body": "I think that `Posets.DiamondPoset()` should have `facade=True` as default, see #17051.\n\nAlso `Posets.DiamondPoset(2)` returns empty poset, `DiamondPoset(1)` gives error message. Should they both just complain about wrong argument?\n\n\nBranch/Commit: **[u/jmantysalo/posets_diamondposet__some_corrections](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/posets_diamondposet__some_corrections) @ [6556992](https://github.com/sagemath/sagetrac-mirror/commit/65569928253276a136e7df2a38d14cd90b5f5daa)**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17061_\n\n",
    "closed_at": "2014-10-13T15:51:51Z",
    "created_at": "2014-09-28T15:29:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Posets.DiamondPoset, some corrections",
    "type": "issue",
    "updated_at": "2014-10-13T15:51:51Z",
    "url": "https://github.com/sagemath/sage/issues/17061",
    "user": "https://github.com/jm58660"
}
```
I think that `Posets.DiamondPoset()` should have `facade=True` as default, see #17051.

Also `Posets.DiamondPoset(2)` returns empty poset, `DiamondPoset(1)` gives error message. Should they both just complain about wrong argument?


Branch/Commit: **[u/jmantysalo/posets_diamondposet__some_corrections](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/posets_diamondposet__some_corrections) @ [6556992](https://github.com/sagemath/sagetrac-mirror/commit/65569928253276a136e7df2a38d14cd90b5f5daa)**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/17061_





---

archive/issue_events_226164.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-28T15:29:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226164"
}
```



---

archive/issue_events_226165.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-28T15:29:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226165"
}
```



---

archive/issue_events_226166.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-28T15:29:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226166"
}
```



---

archive/issue_events_226167.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-28T15:29:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226167"
}
```



---

archive/issue_comments_228576.json:
```json
{
    "body": "Branch: **[u/jmantysalo/posets_diamondposet__some_corrections](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/posets_diamondposet__some_corrections)**",
    "created_at": "2014-09-28T19:50:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17061#issuecomment-228576",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/posets_diamondposet__some_corrections](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/posets_diamondposet__some_corrections)**



---

archive/issue_comments_228577.json:
```json
{
    "body": "Author: **Jori M\u00e4ntysalo**",
    "created_at": "2014-09-28T19:51:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17061#issuecomment-228577",
    "user": "https://github.com/jm58660"
}
```

Author: **Jori Mäntysalo**



---

archive/issue_comments_228578.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nSimple corrections.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/65569928253276a136e7df2a38d14cd90b5f5daa\">6556992</a></td><td><code>DiamondPoset(n) now gives error if n<3. Also default value for</code></td></tr></table>\n",
    "created_at": "2014-09-28T19:51:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17061#issuecomment-228578",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:3" align="right">Comment 3</div>

Simple corrections.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/65569928253276a136e7df2a38d14cd90b5f5daa">6556992</a></td><td><code>DiamondPoset(n) now gives error if n<3. Also default value for</code></td></tr></table>




---

archive/issue_comments_228579.json:
```json
{
    "body": "Commit: **[6556992](https://github.com/sagemath/sagetrac-mirror/commit/65569928253276a136e7df2a38d14cd90b5f5daa)**",
    "created_at": "2014-09-28T19:51:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17061#issuecomment-228579",
    "user": "https://github.com/jm58660"
}
```

Commit: **[6556992](https://github.com/sagemath/sagetrac-mirror/commit/65569928253276a136e7df2a38d14cd90b5f5daa)**



---

archive/issue_events_226168.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-28T19:51:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226168"
}
```



---

archive/issue_comments_228580.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOh I see. You could have fixed that in the PentagonPoset ticket.\n\nNathann",
    "created_at": "2014-09-28T21:25:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17061#issuecomment-228580",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

Oh I see. You could have fixed that in the PentagonPoset ticket.

Nathann



---

archive/issue_events_226169.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-29T04:52:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226169"
}
```



---

archive/issue_events_226170.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-29T04:52:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226170"
}
```



---

archive/issue_events_226171.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-29T04:52:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226171"
}
```



---

archive/issue_events_226172.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-29T04:52:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226172"
}
```



---

archive/issue_events_226173.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-09-29T04:52:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226173"
}
```



---

archive/issue_comments_228581.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nMoved code to another ticket as suggested by ncohen.",
    "created_at": "2014-09-29T04:52:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17061#issuecomment-228581",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:5" align="right">Comment 5</div>

Moved code to another ticket as suggested by ncohen.



---

archive/issue_events_226174.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:51:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226174"
}
```



---

archive/issue_events_226175.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:51:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226175"
}
```



---

archive/issue_events_226176.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:51:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17061",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17061#event-226176"
}
```
