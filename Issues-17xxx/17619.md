# Issue 17619: Add missing libraries on cygwin

archive/issues_017382.json:
```json
{
    "assignees": [],
    "body": "Linking on cygwin is rather strict, contrary to linux: all libraries should be explicitely specified. This patch adds the missing libraries in module_list.py \n\nCC:  jpflori\n\nBranch/Commit: **[4e37400](https://github.com/sagemath/sagetrac-mirror/commit/4e37400aaa42d454ac8fee1796edbef201d21b8a)**\n\nReviewer: **Jean-Pierre Flori**\n\nAuthor: **Sebastien Gouezel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17619_\n\n",
    "closed_at": "2015-01-13T01:11:28Z",
    "created_at": "2015-01-11T20:45:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add missing libraries on cygwin",
    "type": "issue",
    "updated_at": "2015-01-13T01:11:28Z",
    "url": "https://github.com/sagemath/sage/issues/17619",
    "user": "https://github.com/sagetrac-gouezel"
}
```
Linking on cygwin is rather strict, contrary to linux: all libraries should be explicitely specified. This patch adds the missing libraries in module_list.py 

CC:  jpflori

Branch/Commit: **[4e37400](https://github.com/sagemath/sagetrac-mirror/commit/4e37400aaa42d454ac8fee1796edbef201d21b8a)**

Reviewer: **Jean-Pierre Flori**

Author: **Sebastien Gouezel**

_Issue created by migration from https://trac.sagemath.org/ticket/17619_





---

archive/issue_events_224924.json:
```json
{
    "actor": "https://github.com/sagetrac-gouezel",
    "created_at": "2015-01-11T20:45:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17619#event-224924"
}
```



---

archive/issue_events_224925.json:
```json
{
    "actor": "https://github.com/sagetrac-gouezel",
    "created_at": "2015-01-11T20:45:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17619#event-224925"
}
```



---

archive/issue_comments_239613.json:
```json
{
    "body": "Branch: **[u/gouezel/add_missing_libraries_on_cygwin](https://github.com/sagemath/sagetrac-mirror/tree/u/gouezel/add_missing_libraries_on_cygwin)**",
    "created_at": "2015-01-11T20:47:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17619#issuecomment-239613",
    "user": "https://github.com/sagetrac-gouezel"
}
```

Branch: **[u/gouezel/add_missing_libraries_on_cygwin](https://github.com/sagemath/sagetrac-mirror/tree/u/gouezel/add_missing_libraries_on_cygwin)**



---

archive/issue_comments_239614.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4e37400aaa42d454ac8fee1796edbef201d21b8a\">4e37400</a></td><td><code>Add missing library gmp in module_list.py</code></td></tr></table>\n",
    "created_at": "2015-01-11T20:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17619#issuecomment-239614",
    "user": "https://github.com/sagetrac-gouezel"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4e37400aaa42d454ac8fee1796edbef201d21b8a">4e37400</a></td><td><code>Add missing library gmp in module_list.py</code></td></tr></table>




---

archive/issue_events_224926.json:
```json
{
    "actor": "https://github.com/sagetrac-gouezel",
    "created_at": "2015-01-11T20:55:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17619#event-224926"
}
```



---

archive/issue_comments_239615.json:
```json
{
    "body": "Author: **Sebastien Gouezel**",
    "created_at": "2015-01-11T20:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17619#issuecomment-239615",
    "user": "https://github.com/sagetrac-gouezel"
}
```

Author: **Sebastien Gouezel**



---

archive/issue_comments_239616.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Linking on cygwin is rather strict, contrary to linux: all libraries should be explicitely specified. This patch adds the missing libraries in module_list.py \n``````\n",
    "created_at": "2015-01-11T20:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17619#issuecomment-239616",
    "user": "https://github.com/sagetrac-gouezel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Linking on cygwin is rather strict, contrary to linux: all libraries should be explicitely specified. This patch adds the missing libraries in module_list.py 
``````




---

archive/issue_events_224927.json:
```json
{
    "actor": "https://github.com/sagetrac-gouezel",
    "created_at": "2015-01-11T20:55:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17619#event-224927"
}
```



---

archive/issue_comments_239617.json:
```json
{
    "body": "Commit: **[4e37400](https://github.com/sagemath/sagetrac-mirror/commit/4e37400aaa42d454ac8fee1796edbef201d21b8a)**",
    "created_at": "2015-01-11T20:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17619#issuecomment-239617",
    "user": "https://github.com/sagetrac-gouezel"
}
```

Commit: **[4e37400](https://github.com/sagemath/sagetrac-mirror/commit/4e37400aaa42d454ac8fee1796edbef201d21b8a)**



---

archive/issue_events_224928.json:
```json
{
    "actor": "https://github.com/sagetrac-gouezel",
    "created_at": "2015-01-11T20:55:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17619#event-224928"
}
```



---

archive/issue_comments_239618.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLooks sensible (and needed because of the inclusion of bitset.pxi in these two files).",
    "created_at": "2015-01-11T21:23:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17619#issuecomment-239618",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:4'>Comment 4:</a>
Looks sensible (and needed because of the inclusion of bitset.pxi in these two files).



---

archive/issue_comments_239619.json:
```json
{
    "body": "Reviewer: **Jean-Pierre Flori**",
    "created_at": "2015-01-11T21:23:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17619#issuecomment-239619",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Reviewer: **Jean-Pierre Flori**



---

archive/issue_events_224929.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-01-11T21:23:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17619#event-224929"
}
```



---

archive/issue_events_224930.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2015-01-11T21:23:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17619#event-224930"
}
```



---

archive/issue_events_224931.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-13T01:11:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17619#event-224931"
}
```



---

archive/issue_events_224932.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a4ebfb2bef36b4b41b2da1e409213d17eee1d358",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-13T01:11:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17619#event-224932"
}
```



---

archive/issue_comments_239620.json:
```json
{
    "body": "Changed branch from **[u/gouezel/add_missing_libraries_on_cygwin](https://github.com/sagemath/sagetrac-mirror/tree/u/gouezel/add_missing_libraries_on_cygwin)** to **[4e37400](https://github.com/sagemath/sagetrac-mirror/commit/4e37400aaa42d454ac8fee1796edbef201d21b8a)**",
    "created_at": "2015-01-13T01:11:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17619#issuecomment-239620",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gouezel/add_missing_libraries_on_cygwin](https://github.com/sagemath/sagetrac-mirror/tree/u/gouezel/add_missing_libraries_on_cygwin)** to **[4e37400](https://github.com/sagemath/sagetrac-mirror/commit/4e37400aaa42d454ac8fee1796edbef201d21b8a)**
