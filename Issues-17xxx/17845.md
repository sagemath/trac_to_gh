# Issue 17845: Clean up conjugacy classes

archive/issues_017608.json:
```json
{
    "assignees": [],
    "body": "The method .conjugacy_class is defined in too much places:\n- `categories/finite_groups.py`\n- `groups/matrix_gps/group_element.py`\n- `groups/perm_gps/permgroup_element.pyx`\nWe move the basic definition on elements into the category of groups and on parents in the category of groups and `GroupMixinLibGAP`.\n\nAs a consequence much more groups beneficiate from the GAP implementation (e.g. Weyl groups).\n\nWe also implement the cardinality using `Size()` from GAP (the fallback is the useless `_cardinality_from_iterator`).\n\nCC:  @tscrim @vbraun @sagetrac-jlopez\n\nBranch/Commit: **[c00762c](https://github.com/sagemath/sagetrac-mirror/commit/c00762cce26a680039377c67dff869984f38665d)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17845_\n\n",
    "closed_at": "2015-02-25T22:34:16Z",
    "created_at": "2015-02-23T22:32:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean up conjugacy classes",
    "type": "issue",
    "updated_at": "2015-02-25T22:34:16Z",
    "url": "https://github.com/sagemath/sage/issues/17845",
    "user": "https://github.com/videlec"
}
```
The method .conjugacy_class is defined in too much places:
- `categories/finite_groups.py`
- `groups/matrix_gps/group_element.py`
- `groups/perm_gps/permgroup_element.pyx`
We move the basic definition on elements into the category of groups and on parents in the category of groups and `GroupMixinLibGAP`.

As a consequence much more groups beneficiate from the GAP implementation (e.g. Weyl groups).

We also implement the cardinality using `Size()` from GAP (the fallback is the useless `_cardinality_from_iterator`).

CC:  @tscrim @vbraun @sagetrac-jlopez

Branch/Commit: **[c00762c](https://github.com/sagemath/sagetrac-mirror/commit/c00762cce26a680039377c67dff869984f38665d)**

Reviewer: **Nathann Cohen**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/17845_





---

archive/issue_events_236339.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-02-23T22:32:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17845#event-236339"
}
```



---

archive/issue_events_236340.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-02-23T22:32:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17845#event-236340"
}
```



---

archive/issue_events_236341.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-02-23T22:32:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17845#event-236341"
}
```



---

archive/issue_events_236342.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-02-23T22:32:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17845#event-236342"
}
```



---

archive/issue_comments_243883.json:
```json
{
    "body": "Commit: **[c00762c](https://github.com/sagemath/sagetrac-mirror/commit/c00762cce26a680039377c67dff869984f38665d)**",
    "created_at": "2015-02-23T22:33:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17845#issuecomment-243883",
    "user": "https://github.com/videlec"
}
```

Commit: **[c00762c](https://github.com/sagemath/sagetrac-mirror/commit/c00762cce26a680039377c67dff869984f38665d)**



---

archive/issue_comments_243884.json:
```json
{
    "body": "Branch: **[u/vdelecroix/17845](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17845)**",
    "created_at": "2015-02-23T22:33:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17845#issuecomment-243884",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/17845](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17845)**



---

archive/issue_comments_243885.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c00762cce26a680039377c67dff869984f38665d\">c00762c</a></td><td><code>trac #17845: conjugacy classes cleanup</code></td></tr></table>\n",
    "created_at": "2015-02-23T22:33:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17845#issuecomment-243885",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c00762cce26a680039377c67dff869984f38665d">c00762c</a></td><td><code>trac #17845: conjugacy classes cleanup</code></td></tr></table>




---

archive/issue_events_236343.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-02-23T22:33:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17845#event-236343"
}
```



---

archive/issue_comments_243886.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2015-02-24T10:23:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17845#issuecomment-243886",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_243887.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks good, and all tests pass.\n\nIt also seems that right now, the `__contains__` method of `ConjugacyClass` calls `.set()`, even when the group is supported by GAP. \n\nThe more I think about inheritance, the less I like it. It does hide a lot of things that should be reimplemented properly.\n\nNathann",
    "created_at": "2015-02-24T10:23:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17845#issuecomment-243887",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks good, and all tests pass.

It also seems that right now, the `__contains__` method of `ConjugacyClass` calls `.set()`, even when the group is supported by GAP. 

The more I think about inheritance, the less I like it. It does hide a lot of things that should be reimplemented properly.

Nathann



---

archive/issue_events_236344.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-02-24T10:23:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17845#event-236344"
}
```



---

archive/issue_events_236345.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-02-24T10:23:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17845#event-236345"
}
```



---

archive/issue_comments_243888.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIndeed, within GAP we have the magic `IsConjugate` stuff\n\n```\ngap> IsConjugate(SymmetricGroup(5),(1,2),(3,4));\ntrue\n```\nI opened #17846 for that.",
    "created_at": "2015-02-24T10:36:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17845#issuecomment-243888",
    "user": "https://github.com/videlec"
}
```

<div id="comment:3" align="right">Comment 3</div>

Indeed, within GAP we have the magic `IsConjugate` stuff

```
gap> IsConjugate(SymmetricGroup(5),(1,2),(3,4));
true
```
I opened #17846 for that.



---

archive/issue_comments_243889.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReplying to [@nathanncohen](#comment%3A2):\n\n> The more I think about inheritance, the less I like it. It does hide a lot of things that should be reimplemented properly.\n\nEspecially if the parent is completely useless ;-)",
    "created_at": "2015-02-24T10:36:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17845#issuecomment-243889",
    "user": "https://github.com/videlec"
}
```

<div id="comment:4" align="right">Comment 4</div>

Replying to [@nathanncohen](#comment%3A2):

> The more I think about inheritance, the less I like it. It does hide a lot of things that should be reimplemented properly.

Especially if the parent is completely useless ;-)



---

archive/issue_events_236346.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-25T22:34:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17845#event-236346"
}
```



---

archive/issue_events_236347.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "00bbde9883316e5b35cc1ee3b00482eb2c2dcd11",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-02-25T22:34:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17845#event-236347"
}
```



---

archive/issue_comments_243890.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/17845](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17845)** to **[c00762c](https://github.com/sagemath/sagetrac-mirror/commit/c00762cce26a680039377c67dff869984f38665d)**",
    "created_at": "2015-02-25T22:34:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17845#issuecomment-243890",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/17845](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17845)** to **[c00762c](https://github.com/sagemath/sagetrac-mirror/commit/c00762cce26a680039377c67dff869984f38665d)**
