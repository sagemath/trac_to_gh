# Issue 17091: Update to git 2.1.2

archive/issues_016854.json:
```json
{
    "body": "Git sometimes randomly fails in its testsuite, probably a race:\n\n```\nok 76 - extra args: set-url origin newurl oldurl\n# passed all 76 test(s)\n1..76\nnot ok 71 - rebase -i --root on linear history is a no-op # TODO known breakage\nok 12 - git rebase -i (fixup without conflict)\nok 13 - git rebase -i (double edit)\n# passed all 13 test(s)\n1..13\nnot ok 72 - rebase -p --root on linear history is a no-op # TODO known breakage\nok 73 - rebase  -f --root on linear history causes re-write\nok 74 - rebase -m -f --root on linear history causes re-write\nok 75 - rebase -i -f --root on linear history causes re-write\nok 76 - rebase -p -f --root on linear history causes re-write\n# still have 20 known breakage(s)\n# passed all remaining 56 test(s)\n1..76\nmake[5]: Leaving directory '/home/vbraun/Sage/git-develop/local/var/tmp/sage/build/git-1.8.4.4.p0/src/t'\nMakefile:36: recipe for target 'test' failed\nmake[4]: *** [test] Error 2\nmake[4]: Leaving directory '/home/vbraun/Sage/git-develop/local/var/tmp/sage/build/git-1.8.4.4.p0/src/t'\nMakefile:2254: recipe for target 'test' failed\nmake[3]: *** [test] Error 2\nmake[3]: Leaving directory '/home/vbraun/Sage/git-develop/local/var/tmp/sage/build/git-1.8.4.4.p0/src'\nError running git's test suite.\n```\nWe should probably first upgrade to the latest git before worrying about that.\n\nUpstream tarball: http://sage.sagedev.org/home/vbraun/upstream/git-2.1.2.tar.gz\n\n\nCC:  jpflori @kiwifb\n\nBranch/Commit: db4f2523e14c89b4c087d320e3a5374ca9dfce63\n\nUpstream: Reported upstream. No feedback yet.\n\nReviewer: John Palmieri, Fran\u00e7ois Bissey\n\nAuthor: Volker Braun\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/17091\n\n",
    "closed_at": "2014-10-16T21:10:24Z",
    "created_at": "2014-10-02T17:48:37Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Update to git 2.1.2",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/17091",
    "user": "https://github.com/vbraun"
}
```
Git sometimes randomly fails in its testsuite, probably a race:

```
ok 76 - extra args: set-url origin newurl oldurl
# passed all 76 test(s)
1..76
not ok 71 - rebase -i --root on linear history is a no-op # TODO known breakage
ok 12 - git rebase -i (fixup without conflict)
ok 13 - git rebase -i (double edit)
# passed all 13 test(s)
1..13
not ok 72 - rebase -p --root on linear history is a no-op # TODO known breakage
ok 73 - rebase  -f --root on linear history causes re-write
ok 74 - rebase -m -f --root on linear history causes re-write
ok 75 - rebase -i -f --root on linear history causes re-write
ok 76 - rebase -p -f --root on linear history causes re-write
# still have 20 known breakage(s)
# passed all remaining 56 test(s)
1..76
make[5]: Leaving directory '/home/vbraun/Sage/git-develop/local/var/tmp/sage/build/git-1.8.4.4.p0/src/t'
Makefile:36: recipe for target 'test' failed
make[4]: *** [test] Error 2
make[4]: Leaving directory '/home/vbraun/Sage/git-develop/local/var/tmp/sage/build/git-1.8.4.4.p0/src/t'
Makefile:2254: recipe for target 'test' failed
make[3]: *** [test] Error 2
make[3]: Leaving directory '/home/vbraun/Sage/git-develop/local/var/tmp/sage/build/git-1.8.4.4.p0/src'
Error running git's test suite.
```
We should probably first upgrade to the latest git before worrying about that.

Upstream tarball: http://sage.sagedev.org/home/vbraun/upstream/git-2.1.2.tar.gz


CC:  jpflori @kiwifb

Branch/Commit: db4f2523e14c89b4c087d320e3a5374ca9dfce63

Upstream: Reported upstream. No feedback yet.

Reviewer: John Palmieri, François Bissey

Author: Volker Braun

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/17091





---

archive/issue_comments_287728.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -27,3 +27,5 @@\n Error running git's test suite.\n ```\n We should probably first upgrade to the latest git before worrying about that.\n+\n+Upstream tarball: http://boxen.math.washington.edu/home/vbraun/upstream/git-2.1.2.tar.gz\n``````\n",
    "created_at": "2014-10-02T20:16:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287728",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -27,3 +27,5 @@
 Error running git's test suite.
 ```
 We should probably first upgrade to the latest git before worrying about that.
+
+Upstream tarball: http://boxen.math.washington.edu/home/vbraun/upstream/git-2.1.2.tar.gz
``````




---

archive/issue_comments_287729.json:
```json
{
    "body": "Changing author from \"\" to \"Volker Braun\"",
    "created_at": "2014-10-02T20:16:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287729",
    "user": "https://github.com/vbraun"
}
```

Changing author from "" to "Volker Braun"



---

archive/issue_comments_287730.json:
```json
{
    "body": "Changing branch from \"\" to \"u/vbraun/random_failure_in_the_git_testsuite\"",
    "created_at": "2014-10-02T20:20:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287730",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "" to "u/vbraun/random_failure_in_the_git_testsuite"



---

archive/issue_comments_287731.json:
```json
{
    "body": "Changing commit from \"\" to \"8d74c8819b7b88e1ae5e50595a997ab500334b8e\"",
    "created_at": "2014-10-02T20:26:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287731",
    "user": "https://github.com/vbraun"
}
```

Changing commit from "" to "8d74c8819b7b88e1ae5e50595a997ab500334b8e"



---

archive/issue_comments_287732.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -28,4 +28,5 @@\n ```\n We should probably first upgrade to the latest git before worrying about that.\n \n-Upstream tarball: http://boxen.math.washington.edu/home/vbraun/upstream/git-2.1.2.tar.gz\n+Upstream tarball: http://sage.sagedev.org/home/vbraun/upstream/git-2.1.2.tar.gz\n+\n``````\n",
    "created_at": "2014-10-02T20:26:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287732",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -28,4 +28,5 @@
 ```
 We should probably first upgrade to the latest git before worrying about that.
 
-Upstream tarball: http://boxen.math.washington.edu/home/vbraun/upstream/git-2.1.2.tar.gz
+Upstream tarball: http://sage.sagedev.org/home/vbraun/upstream/git-2.1.2.tar.gz
+
``````




---

archive/issue_comments_287733.json:
```json
{
    "body": "<a id='comment:3'></a>\nStraightforward update.\n\n---\nNew commits:\n|                                                                                                                                          |                     |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------|\n|[8d74c88](http://git.sagemath.org/sage.git/commit/?id=8d74c8819b7b88e1ae5e50595a997ab500334b8e)|`Update to git 2.1.2`|",
    "created_at": "2014-10-02T20:26:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287733",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'></a>
Straightforward update.

---
New commits:
|                                                                                                                                          |                     |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------|
|[8d74c88](http://git.sagemath.org/sage.git/commit/?id=8d74c8819b7b88e1ae5e50595a997ab500334b8e)|`Update to git 2.1.2`|



---

archive/issue_comments_287734.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2014-10-02T20:26:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287734",
    "user": "https://github.com/vbraun"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_287735.json:
```json
{
    "body": "<a id='comment:4'></a>\nI am only at 2.0.4 locally, not that this matter much for a straightforward package. I guess your main problem is that it make the build break with SPKG_CHECK turned on? Otherwise we would care so much about breakages upstream knows about.",
    "created_at": "2014-10-03T00:42:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287735",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:4'></a>
I am only at 2.0.4 locally, not that this matter much for a straightforward package. I guess your main problem is that it make the build break with SPKG_CHECK turned on? Otherwise we would care so much about breakages upstream knows about.



---

archive/issue_comments_287736.json:
```json
{
    "body": "<a id='comment:5'></a>\nThis fails to build on my OS X 10.9 machine. Compilation ends with\n\n```\nIn file included from /System/Library/Frameworks/Security.framework/Headers/SecAsn1Types.h:42:0,\n                 from /System/Library/Frameworks/Security.framework/Headers/oidsalg.h:29,\n                 from /System/Library/Frameworks/Security.framework/Headers/Security.h:44,\n                 from compat/apple-common-crypto.h:14,\n                 from git-compat-util.h:324,\n                 from line-log.c:1:\n/System/Library/Frameworks/CoreFoundation.framework/Headers/CFBase.h:535:6: note: expected 'CFTypeRef' but argument is of type 'int'\nmake: *** [line-range.o] Error 1\nmake: *** [line-log.o] Error 1\nError building git.\n```\nFull log at [http://sage.math.washington.edu/home/palmieri/misc/git-2.1.2.log](http://sage.math.washington.edu/home/palmieri/misc/git-2.1.2.log).",
    "created_at": "2014-10-03T05:40:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287736",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>
This fails to build on my OS X 10.9 machine. Compilation ends with

```
In file included from /System/Library/Frameworks/Security.framework/Headers/SecAsn1Types.h:42:0,
                 from /System/Library/Frameworks/Security.framework/Headers/oidsalg.h:29,
                 from /System/Library/Frameworks/Security.framework/Headers/Security.h:44,
                 from compat/apple-common-crypto.h:14,
                 from git-compat-util.h:324,
                 from line-log.c:1:
/System/Library/Frameworks/CoreFoundation.framework/Headers/CFBase.h:535:6: note: expected 'CFTypeRef' but argument is of type 'int'
make: *** [line-range.o] Error 1
make: *** [line-log.o] Error 1
Error building git.
```
Full log at [http://sage.math.washington.edu/home/palmieri/misc/git-2.1.2.log](http://sage.math.washington.edu/home/palmieri/misc/git-2.1.2.log).



---

archive/issue_comments_287737.json:
```json
{
    "body": "<a id='comment:6'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                              |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------|\n|[331c2c3](http://git.sagemath.org/sage.git/commit/?id=331c2c3c71b1995ce3ce89d343dcbc77465ca045)|`Disable common crypto on OSX`|",
    "created_at": "2014-10-03T10:45:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287737",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                              |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------|
|[331c2c3](http://git.sagemath.org/sage.git/commit/?id=331c2c3c71b1995ce3ce89d343dcbc77465ca045)|`Disable common crypto on OSX`|



---

archive/issue_comments_287738.json:
```json
{
    "body": "Changing commit from \"8d74c8819b7b88e1ae5e50595a997ab500334b8e\" to \"331c2c3c71b1995ce3ce89d343dcbc77465ca045\"",
    "created_at": "2014-10-03T10:45:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287738",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "8d74c8819b7b88e1ae5e50595a997ab500334b8e" to "331c2c3c71b1995ce3ce89d343dcbc77465ca045"



---

archive/issue_comments_287739.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"Reported upstream. No feedback yet.\"",
    "created_at": "2014-10-03T10:47:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287739",
    "user": "https://github.com/vbraun"
}
```

Changing upstream from "N/A" to "Reported upstream. No feedback yet."



---

archive/issue_comments_287740.json:
```json
{
    "body": "<a id='comment:7'></a>\nReported at http://article.gmane.org/gmane.comp.version-control.git/257836\n\nSince we don't really need SSL we can just disable common crypto. Git is part of xcode on OSX, so its unlikely that anybody would even notice.",
    "created_at": "2014-10-03T10:47:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287740",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:7'></a>
Reported at http://article.gmane.org/gmane.comp.version-control.git/257836

Since we don't really need SSL we can just disable common crypto. Git is part of xcode on OSX, so its unlikely that anybody would even notice.



---

archive/issue_comments_287741.json:
```json
{
    "body": "<a id='comment:8'></a>\nI think you need a similar change in `spkg-check`; otherwise, the test suite fails immediately with the same error.",
    "created_at": "2014-10-03T16:32:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287741",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:8'></a>
I think you need a similar change in `spkg-check`; otherwise, the test suite fails immediately with the same error.



---

archive/issue_comments_287742.json:
```json
{
    "body": "Changing commit from \"331c2c3c71b1995ce3ce89d343dcbc77465ca045\" to \"602c0231da57a5c46f202d7b8a81b39ca711a0f3\"",
    "created_at": "2014-10-03T18:36:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287742",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "331c2c3c71b1995ce3ce89d343dcbc77465ca045" to "602c0231da57a5c46f202d7b8a81b39ca711a0f3"



---

archive/issue_comments_287743.json:
```json
{
    "body": "<a id='comment:9'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                   |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------|\n|[602c023](http://git.sagemath.org/sage.git/commit/?id=602c0231da57a5c46f202d7b8a81b39ca711a0f3)|`Also disable apple common crypto in the testsuite`|",
    "created_at": "2014-10-03T18:36:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287743",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:9'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                   |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------|
|[602c023](http://git.sagemath.org/sage.git/commit/?id=602c0231da57a5c46f202d7b8a81b39ca711a0f3)|`Also disable apple common crypto in the testsuite`|



---

archive/issue_comments_287744.json:
```json
{
    "body": "<a id='comment:10'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                       |\n|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|\n|[6165dc0](http://git.sagemath.org/sage.git/commit/?id=6165dc0c39a6a3528ebb88f8c6430abfb804b508)|`Disable network tests`|",
    "created_at": "2014-10-03T21:36:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287744",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:10'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                       |
|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|
|[6165dc0](http://git.sagemath.org/sage.git/commit/?id=6165dc0c39a6a3528ebb88f8c6430abfb804b508)|`Disable network tests`|



---

archive/issue_comments_287745.json:
```json
{
    "body": "Changing commit from \"602c0231da57a5c46f202d7b8a81b39ca711a0f3\" to \"6165dc0c39a6a3528ebb88f8c6430abfb804b508\"",
    "created_at": "2014-10-03T21:36:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287745",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "602c0231da57a5c46f202d7b8a81b39ca711a0f3" to "6165dc0c39a6a3528ebb88f8c6430abfb804b508"



---

archive/issue_comments_287746.json:
```json
{
    "body": "<a id='comment:11'></a>\nOk this time I checked that the testsuite passes on OSX",
    "created_at": "2014-10-03T21:37:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287746",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:11'></a>
Ok this time I checked that the testsuite passes on OSX



---

archive/issue_comments_287747.json:
```json
{
    "body": "<a id='comment:12'></a>\nPasses tests for me, too, on OS X 10.9.",
    "created_at": "2014-10-04T00:55:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287747",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'></a>
Passes tests for me, too, on OS X 10.9.



---

archive/issue_comments_287748.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-10-14T20:34:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287748",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_287749.json:
```json
{
    "body": "Changing reviewer from \"\" to \"John Palmieri, Fran\u00e7ois Bissey\"",
    "created_at": "2014-10-14T20:34:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287749",
    "user": "https://github.com/jhpalmieri"
}
```

Changing reviewer from "" to "John Palmieri, François Bissey"



---

archive/issue_comments_287750.json:
```json
{
    "body": "<a id='comment:14'></a>\n\n```\nsage -t --long --warn-long 71.0 src/sage/dev/sagedev.py  # 1 doctest failed\nsage -t --long --warn-long 71.0 src/sage/dev/git_interface.py  # 6 doctests failed\n```",
    "created_at": "2014-10-15T15:23:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287750",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:14'></a>

```
sage -t --long --warn-long 71.0 src/sage/dev/sagedev.py  # 1 doctest failed
sage -t --long --warn-long 71.0 src/sage/dev/git_interface.py  # 6 doctests failed
```



---

archive/issue_comments_287751.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2014-10-15T15:23:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287751",
    "user": "https://github.com/vbraun"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_287752.json:
```json
{
    "body": "<a id='comment:15'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------|\n|[db4f252](http://git.sagemath.org/sage.git/commit/?id=db4f2523e14c89b4c087d320e3a5374ca9dfce63)|`Fix doctests that depend on the new git stdout`|",
    "created_at": "2014-10-15T15:46:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287752",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:15'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------|
|[db4f252](http://git.sagemath.org/sage.git/commit/?id=db4f2523e14c89b4c087d320e3a5374ca9dfce63)|`Fix doctests that depend on the new git stdout`|



---

archive/issue_comments_287753.json:
```json
{
    "body": "Changing commit from \"6165dc0c39a6a3528ebb88f8c6430abfb804b508\" to \"db4f2523e14c89b4c087d320e3a5374ca9dfce63\"",
    "created_at": "2014-10-15T15:46:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287753",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6165dc0c39a6a3528ebb88f8c6430abfb804b508" to "db4f2523e14c89b4c087d320e3a5374ca9dfce63"



---

archive/issue_comments_287754.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2014-10-15T15:46:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287754",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_287755.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-10-15T19:20:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287755",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_287756.json:
```json
{
    "body": "<a id='comment:17'></a>\nAll long tests pass for me.",
    "created_at": "2014-10-15T19:20:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287756",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:17'></a>
All long tests pass for me.



---

archive/issue_events_049653.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-16T21:10:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17091#event-49653"
}
```



---

archive/issue_comments_287757.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2014-10-16T21:10:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287757",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_287758.json:
```json
{
    "body": "Changing branch from \"u/vbraun/random_failure_in_the_git_testsuite\" to \"db4f2523e14c89b4c087d320e3a5374ca9dfce63\"",
    "created_at": "2014-10-16T21:10:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17091#issuecomment-287758",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/vbraun/random_failure_in_the_git_testsuite" to "db4f2523e14c89b4c087d320e3a5374ca9dfce63"
