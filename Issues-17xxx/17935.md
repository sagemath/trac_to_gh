# Issue 17935: recognize Sympy's NonElementaryIntegral

archive/issues_017698.json:
```json
{
    "assignees": [],
    "body": "We don't translate a Sympy result properly:\n\n```\nsage: integral(log(x)/(x^2 - 1),x,0,infinity, algorithm='sympy')\n...\nAttributeError: 'NonElementaryIntegral' object has no attribute '_sage_'\n```\nThis should likely be done upstream, as previously.\n\nCC:  @kcrisman\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17935_\n\n",
    "closed_at": "2017-12-12T08:23:33Z",
    "created_at": "2015-03-11T14:23:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "recognize Sympy's NonElementaryIntegral",
    "type": "issue",
    "updated_at": "2017-12-12T08:23:33Z",
    "url": "https://github.com/sagemath/sage/issues/17935",
    "user": "https://github.com/rwst"
}
```
We don't translate a Sympy result properly:

```
sage: integral(log(x)/(x^2 - 1),x,0,infinity, algorithm='sympy')
...
AttributeError: 'NonElementaryIntegral' object has no attribute '_sage_'
```
This should likely be done upstream, as previously.

CC:  @kcrisman

_Issue created by migration from https://trac.sagemath.org/ticket/17935_





---

archive/issue_events_237558.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-11T14:23:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237558"
}
```



---

archive/issue_events_237559.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-11T14:23:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237559"
}
```



---

archive/issue_events_237560.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-11T14:23:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237560"
}
```



---

archive/issue_events_237561.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-11T14:23:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237561"
}
```



---

archive/issue_events_237562.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-02T06:47:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237562"
}
```



---

archive/issue_events_237563.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-02T06:47:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237563"
}
```



---

archive/issue_events_237564.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-02T06:47:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237564"
}
```



---

archive/issue_comments_245636.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nSeems to have been resolved en passant in #22891.",
    "created_at": "2017-10-02T06:47:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17935#issuecomment-245636",
    "user": "https://github.com/rwst"
}
```

<div id="comment:2" align="right">Comment 2</div>

Seems to have been resolved en passant in #22891.



---

archive/issue_events_237565.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-02T06:47:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237565"
}
```



---

archive/issue_events_237566.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-02T06:47:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237566"
}
```



---

archive/issue_events_237567.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-10-02T06:47:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237567"
}
```



---

archive/issue_events_237568.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237568"
}
```



---

archive/issue_events_237569.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237569"
}
```



---

archive/issue_events_237570.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17935",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17935#event-237570"
}
```
