# Issue 17724: Rename PowerSeries.reversion() in PowerSeries.reverse()

archive/issues_017487.json:
```json
{
    "assignees": [],
    "body": "The method `PowerSeries.reversion(self)` returns the reverse power series of `self`. Following the apparent naming convention, I propose to rename it `PowerSeries.reverse(self)`. \n\nIt is more consistent with other method such as (for `PowerSeries`): `derivative()` (not `derivation`) or `truncate()` (not `truncation`).\n\nKeywords: **reverse, series**\n\nBranch/Commit: **[05bbf4a](https://github.com/sagemath/sagetrac-mirror/commit/05bbf4ad70765e6a1a8cdb3c3e6930231f2fbec8)**\n\nReviewer: **Ralf Stephan**\n\nAuthor: **Bruno Grenet**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17724_\n\n",
    "closed_at": "2015-02-18T01:27:04Z",
    "created_at": "2015-02-03T17:28:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Rename PowerSeries.reversion() in PowerSeries.reverse()",
    "type": "issue",
    "updated_at": "2015-02-18T01:27:04Z",
    "url": "https://github.com/sagemath/sage/issues/17724",
    "user": "https://github.com/bgrenet"
}
```
The method `PowerSeries.reversion(self)` returns the reverse power series of `self`. Following the apparent naming convention, I propose to rename it `PowerSeries.reverse(self)`. 

It is more consistent with other method such as (for `PowerSeries`): `derivative()` (not `derivation`) or `truncate()` (not `truncation`).

Keywords: **reverse, series**

Branch/Commit: **[05bbf4a](https://github.com/sagemath/sagetrac-mirror/commit/05bbf4ad70765e6a1a8cdb3c3e6930231f2fbec8)**

Reviewer: **Ralf Stephan**

Author: **Bruno Grenet**

_Issue created by migration from https://trac.sagemath.org/ticket/17724_





---

archive/issue_events_234845.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2015-02-03T17:28:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17724#event-234845"
}
```



---

archive/issue_events_234846.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2015-02-03T17:28:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17724#event-234846"
}
```



---

archive/issue_events_234847.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2015-02-03T17:28:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17724#event-234847"
}
```



---

archive/issue_comments_241986.json:
```json
{
    "body": "Branch: **[u/bruno/rename_powerseries_reversion___in_powerseries_reverse__](https://github.com/sagemath/sagetrac-mirror/tree/u/bruno/rename_powerseries_reversion___in_powerseries_reverse__)**",
    "created_at": "2015-02-03T17:44:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17724#issuecomment-241986",
    "user": "https://github.com/bgrenet"
}
```

Branch: **[u/bruno/rename_powerseries_reversion___in_powerseries_reverse__](https://github.com/sagemath/sagetrac-mirror/tree/u/bruno/rename_powerseries_reversion___in_powerseries_reverse__)**



---

archive/issue_comments_241987.json:
```json
{
    "body": "Commit: **[05bbf4a](https://github.com/sagemath/sagetrac-mirror/commit/05bbf4ad70765e6a1a8cdb3c3e6930231f2fbec8)**",
    "created_at": "2015-02-03T17:45:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17724#issuecomment-241987",
    "user": "https://github.com/bgrenet"
}
```

Commit: **[05bbf4a](https://github.com/sagemath/sagetrac-mirror/commit/05bbf4ad70765e6a1a8cdb3c3e6930231f2fbec8)**



---

archive/issue_events_234848.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2015-02-03T17:45:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17724#event-234848"
}
```



---

archive/issue_events_234849.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2015-02-03T17:45:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17724#event-234849"
}
```



---

archive/issue_comments_241988.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/05bbf4ad70765e6a1a8cdb3c3e6930231f2fbec8\">05bbf4a</a></td><td><code>Replace reversion by reverse</code></td></tr></table>\n",
    "created_at": "2015-02-03T17:45:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17724#issuecomment-241988",
    "user": "https://github.com/bgrenet"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/05bbf4ad70765e6a1a8cdb3c3e6930231f2fbec8">05bbf4a</a></td><td><code>Replace reversion by reverse</code></td></tr></table>




---

archive/issue_comments_241989.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2015-02-04T14:17:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17724#issuecomment-241989",
    "user": "https://github.com/rwst"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_comments_241990.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks fine. Thanks.",
    "created_at": "2015-02-04T14:17:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17724#issuecomment-241990",
    "user": "https://github.com/rwst"
}
```

<a id='comment:3'>Comment 3:</a>
Looks fine. Thanks.



---

archive/issue_events_234850.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-04T14:17:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17724#event-234850"
}
```



---

archive/issue_events_234851.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-04T14:17:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17724#event-234851"
}
```



---

archive/issue_events_234852.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-18T01:27:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17724#event-234852"
}
```



---

archive/issue_events_234853.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7fc4cfaa40b8791164967318fd2f29f7376fe011",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-02-18T01:27:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17724#event-234853"
}
```



---

archive/issue_comments_241991.json:
```json
{
    "body": "Changed branch from **[u/bruno/rename_powerseries_reversion___in_powerseries_reverse__](https://github.com/sagemath/sagetrac-mirror/tree/u/bruno/rename_powerseries_reversion___in_powerseries_reverse__)** to **[05bbf4a](https://github.com/sagemath/sagetrac-mirror/commit/05bbf4ad70765e6a1a8cdb3c3e6930231f2fbec8)**",
    "created_at": "2015-02-18T01:27:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17724#issuecomment-241991",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/bruno/rename_powerseries_reversion___in_powerseries_reverse__](https://github.com/sagemath/sagetrac-mirror/tree/u/bruno/rename_powerseries_reversion___in_powerseries_reverse__)** to **[05bbf4a](https://github.com/sagemath/sagetrac-mirror/commit/05bbf4ad70765e6a1a8cdb3c3e6930231f2fbec8)**
