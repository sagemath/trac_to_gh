# Issue 17104: IncidenceStructure.relabel() (no arguments)

archive/issues_016867.json:
```json
{
    "assignees": [],
    "body": "One can relabel a graph to 0,...,n-1 by just calling `.relabel()`, and I found myself typing `IS.relabel({v:i for i,v in enumerate(IS.ground_set())})` very often these last days.\n\nWith this patch one can relabel the point automatically to `0,...,n-1`.\n\nNathann\n\nCC:  @videlec @dimpase\n\nBranch/Commit: **[8615954](https://github.com/sagemath/sagetrac-mirror/commit/8615954a35fdacc6ba0dfe1573ac0e942aeb496e)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17104_\n\n",
    "closed_at": "2014-10-18T18:18:05Z",
    "created_at": "2014-10-06T11:50:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorial%20designs",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "IncidenceStructure.relabel() (no arguments)",
    "type": "issue",
    "updated_at": "2014-10-18T18:18:05Z",
    "url": "https://github.com/sagemath/sage/issues/17104",
    "user": "https://github.com/nathanncohen"
}
```
One can relabel a graph to 0,...,n-1 by just calling `.relabel()`, and I found myself typing `IS.relabel({v:i for i,v in enumerate(IS.ground_set())})` very often these last days.

With this patch one can relabel the point automatically to `0,...,n-1`.

Nathann

CC:  @videlec @dimpase

Branch/Commit: **[8615954](https://github.com/sagemath/sagetrac-mirror/commit/8615954a35fdacc6ba0dfe1573ac0e942aeb496e)**

Reviewer: **Vincent Delecroix**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/17104_





---

archive/issue_events_226678.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-10-06T11:50:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17104#event-226678"
}
```



---

archive/issue_events_226679.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-10-06T11:50:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorial%20designs",
    "label_color": "0000ff",
    "label_name": "component: combinatorial designs",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17104#event-226679"
}
```



---

archive/issue_events_226680.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-10-06T11:50:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17104#event-226680"
}
```



---

archive/issue_events_226681.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-10-06T11:50:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17104#event-226681"
}
```



---

archive/issue_comments_229320.json:
```json
{
    "body": "Branch: **[u/ncohen/17104](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17104)**",
    "created_at": "2014-10-06T11:51:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229320",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/17104](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17104)**



---

archive/issue_events_226682.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-10-06T11:51:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17104#event-226682"
}
```



---

archive/issue_comments_229321.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/795cc45a9c7b320d326b48fb22e740cb40b93e75\">795cc45</a></td><td><code>trac #17104: IncidenceStructure.relabel() (no arguments)</code></td></tr></table>\n",
    "created_at": "2014-10-06T11:51:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229321",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/795cc45a9c7b320d326b48fb22e740cb40b93e75">795cc45</a></td><td><code>trac #17104: IncidenceStructure.relabel() (no arguments)</code></td></tr></table>




---

archive/issue_comments_229322.json:
```json
{
    "body": "Commit: **[795cc45](https://github.com/sagemath/sagetrac-mirror/commit/795cc45a9c7b320d326b48fb22e740cb40b93e75)**",
    "created_at": "2014-10-06T11:51:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229322",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[795cc45](https://github.com/sagemath/sagetrac-mirror/commit/795cc45a9c7b320d326b48fb22e740cb40b93e75)**



---

archive/issue_comments_229323.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nHello,\n\nCompared with the possibilities from graph:\n- what about a `inplace` argument?\n- what about accepting lists as input. The following works with graphs\n\n  ```\n  sage: G = Graphs(... whatever on [0,1,2,3] ...)\n  sage: G.relabel([2,0,1,3])\n  ```\n\nIn the branch I updated (rebase over sage-4.6) I added a commit which do these changes. I also added a nice test which illustrate the usefulness of these additions. What do you think ?\n\nVincent\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c081fd0c4c6ae2051e92cd2e21c9815a9b04c685\">c081fd0</a></td><td><code>trac #17104: IncidenceStructure.relabel() (no arguments)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3ade030d09adfd591c1a8f8bc18ddbe18fe5a43c\">3ade030</a></td><td><code>trac #17104: allow list/tuple + add inplace arg</code></td></tr></table>\n",
    "created_at": "2014-10-17T17:59:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229323",
    "user": "https://github.com/videlec"
}
```

<a id='comment:4'>Comment 4:</a>
Hello,

Compared with the possibilities from graph:
- what about a `inplace` argument?
- what about accepting lists as input. The following works with graphs

  ```
  sage: G = Graphs(... whatever on [0,1,2,3] ...)
  sage: G.relabel([2,0,1,3])
  ```

In the branch I updated (rebase over sage-4.6) I added a commit which do these changes. I also added a nice test which illustrate the usefulness of these additions. What do you think ?

Vincent

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c081fd0c4c6ae2051e92cd2e21c9815a9b04c685">c081fd0</a></td><td><code>trac #17104: IncidenceStructure.relabel() (no arguments)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3ade030d09adfd591c1a8f8bc18ddbe18fe5a43c">3ade030</a></td><td><code>trac #17104: allow list/tuple + add inplace arg</code></td></tr></table>




---

archive/issue_comments_229324.json:
```json
{
    "body": "Changed branch from **[u/ncohen/17104](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17104)** to **[u/vdelecroix/17104](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17104)**",
    "created_at": "2014-10-17T17:59:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229324",
    "user": "https://github.com/videlec"
}
```

Changed branch from **[u/ncohen/17104](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17104)** to **[u/vdelecroix/17104](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17104)**



---

archive/issue_comments_229325.json:
```json
{
    "body": "Changed commit from **[795cc45](https://github.com/sagemath/sagetrac-mirror/commit/795cc45a9c7b320d326b48fb22e740cb40b93e75)** to **[3ade030](https://github.com/sagemath/sagetrac-mirror/commit/3ade030d09adfd591c1a8f8bc18ddbe18fe5a43c)**",
    "created_at": "2014-10-17T17:59:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229325",
    "user": "https://github.com/videlec"
}
```

Changed commit from **[795cc45](https://github.com/sagemath/sagetrac-mirror/commit/795cc45a9c7b320d326b48fb22e740cb40b93e75)** to **[3ade030](https://github.com/sagemath/sagetrac-mirror/commit/3ade030d09adfd591c1a8f8bc18ddbe18fe5a43c)**



---

archive/issue_comments_229326.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nPS: If you have more work based upon this branch I will instead base my commit over yours after a merge of 6-4.beta6 (switching between beta4/beta6 is a mess).",
    "created_at": "2014-10-17T18:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229326",
    "user": "https://github.com/videlec"
}
```

<a id='comment:5'>Comment 5:</a>
PS: If you have more work based upon this branch I will instead base my commit over yours after a merge of 6-4.beta6 (switching between beta4/beta6 is a mess).



---

archive/issue_comments_229327.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nHellooooo !\n\nWell, I don't think I have anything based atop of that. Though you seem to need a 'copy' function that was added in #17047. Though I suppose that `copy` will call it.\n\nHowever, it would be cool if you could indent the description of the different types for `inplace` so that it is a member of the item that begins with <code>\\`\\`inplace\\`\\` -- If \\`\\`True\\`\\` then ...</code>. \n\nAlso, there is no `.points()` function, what we have is `ground_set`.\n\nShort of that it's all fine, thanks !\n\nNathann",
    "created_at": "2014-10-17T21:10:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229327",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'>Comment 6:</a>
Hellooooo !

Well, I don't think I have anything based atop of that. Though you seem to need a 'copy' function that was added in #17047. Though I suppose that `copy` will call it.

However, it would be cool if you could indent the description of the different types for `inplace` so that it is a member of the item that begins with <code>\`\`inplace\`\` -- If \`\`True\`\` then ...</code>. 

Also, there is no `.points()` function, what we have is `ground_set`.

Short of that it's all fine, thanks !

Nathann



---

archive/issue_comments_229328.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8615954a35fdacc6ba0dfe1573ac0e942aeb496e\">8615954</a></td><td><code>trac #17104: reviewer comments</code></td></tr></table>\n",
    "created_at": "2014-10-18T15:00:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229328",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8615954a35fdacc6ba0dfe1573ac0e942aeb496e">8615954</a></td><td><code>trac #17104: reviewer comments</code></td></tr></table>




---

archive/issue_comments_229329.json:
```json
{
    "body": "Changed commit from **[3ade030](https://github.com/sagemath/sagetrac-mirror/commit/3ade030d09adfd591c1a8f8bc18ddbe18fe5a43c)** to **[8615954](https://github.com/sagemath/sagetrac-mirror/commit/8615954a35fdacc6ba0dfe1573ac0e942aeb496e)**",
    "created_at": "2014-10-18T15:00:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229329",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3ade030](https://github.com/sagemath/sagetrac-mirror/commit/3ade030d09adfd591c1a8f8bc18ddbe18fe5a43c)** to **[8615954](https://github.com/sagemath/sagetrac-mirror/commit/8615954a35fdacc6ba0dfe1573ac0e942aeb496e)**



---

archive/issue_comments_229330.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nLooks good ! You can set the ticket to `positive_review` is you see nothing wrong here anymore.\n\nNathann",
    "created_at": "2014-10-18T15:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229330",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:8'>Comment 8:</a>
Looks good ! You can set the ticket to `positive_review` is you see nothing wrong here anymore.

Nathann



---

archive/issue_events_226683.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-10-18T15:09:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17104#event-226683"
}
```



---

archive/issue_events_226684.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-10-18T15:09:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17104#event-226684"
}
```



---

archive/issue_comments_229331.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nDoc compiles and it looks fine!",
    "created_at": "2014-10-18T15:09:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229331",
    "user": "https://github.com/videlec"
}
```

<a id='comment:9'>Comment 9:</a>
Doc compiles and it looks fine!



---

archive/issue_comments_229332.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2014-10-18T15:09:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229332",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_events_226685.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-18T18:18:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17104#event-226685"
}
```



---

archive/issue_events_226686.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6b5ff951c2b53db7925c0b15d4ee4eba9981e727",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-10-18T18:18:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17104#event-226686"
}
```



---

archive/issue_comments_229333.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/17104](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17104)** to **[8615954](https://github.com/sagemath/sagetrac-mirror/commit/8615954a35fdacc6ba0dfe1573ac0e942aeb496e)**",
    "created_at": "2014-10-18T18:18:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17104#issuecomment-229333",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/17104](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17104)** to **[8615954](https://github.com/sagemath/sagetrac-mirror/commit/8615954a35fdacc6ba0dfe1573ac0e942aeb496e)**
