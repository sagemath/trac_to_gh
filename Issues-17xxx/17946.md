# Issue 17946: Use ipython own's function to get its configuration folder

archive/issues_017709.json:
```json
{
    "body": "Currently the IPython code in sage relies on code such as\n\n```\nIPYTHONDIR = os.environ['IPYTHONDIR']\n```\nwhen ipython has its own call to discover what IPYTHONDIR should be. This is particularly important if you want to unset IPYTHONDIR for your own purpose - or you run sage-on-gentoo where we let ipython run with its system default.\n\nBranch/Commit: 191ac98e0a5e252e22d54f7a1251d6ebcdef7df6\n\nReviewer: Travis Scrimshaw\n\nAuthor: Fran\u00e7ois Bissey\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/17946\n\n",
    "closed_at": "2015-03-17T08:34:09Z",
    "created_at": "2015-03-13T10:04:07Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.6",
    "title": "Use ipython own's function to get its configuration folder",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/17946",
    "user": "https://github.com/kiwifb"
}
```
Currently the IPython code in sage relies on code such as

```
IPYTHONDIR = os.environ['IPYTHONDIR']
```
when ipython has its own call to discover what IPYTHONDIR should be. This is particularly important if you want to unset IPYTHONDIR for your own purpose - or you run sage-on-gentoo where we let ipython run with its system default.

Branch/Commit: 191ac98e0a5e252e22d54f7a1251d6ebcdef7df6

Reviewer: Travis Scrimshaw

Author: François Bissey

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/17946





---

archive/issue_comments_308300.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-03-13T12:04:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308300",
    "user": "https://github.com/kiwifb"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_308301.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to defect.",
    "created_at": "2015-03-13T13:01:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308301",
    "user": "https://github.com/kcrisman"
}
```

Changing type from PLEASE CHANGE to defect.



---

archive/issue_comments_308302.json:
```json
{
    "body": "<a id='comment:3'></a>LGTM.",
    "created_at": "2015-03-13T15:01:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308302",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>LGTM.



---

archive/issue_comments_308303.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to build.",
    "created_at": "2015-03-13T15:01:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308303",
    "user": "https://github.com/tscrim"
}
```

Changing component from PLEASE CHANGE to build.



---

archive/issue_comments_308304.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-03-13T15:01:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308304",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_308305.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2015-03-13T15:01:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308305",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_308306.json:
```json
{
    "body": "<a id='comment:4'></a>\n```\nsage -t --long src/sage/repl/ipython_kernel/install.py\n**********************************************************************\nFile \"src/sage/repl/ipython_kernel/install.py\", line 118, in sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax\nFailed example:\n    ipython_dir = get_ipython_dir()\nException raised:\n    Traceback (most recent call last):\n      File \"/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 492, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 854, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax[3]>\", line 1, in <module>\n        ipython_dir = get_ipython_dir()\n    NameError: name 'get_ipython_dir' is not defined\n**********************************************************************\nFile \"src/sage/repl/ipython_kernel/install.py\", line 119, in sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax\nFailed example:\n    mathjax = os.path.join(ipython_dir, 'nbextensions', 'mathjax')\nException raised:\n    Traceback (most recent call last):\n      File \"/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 492, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 854, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax[4]>\", line 1, in <module>\n        mathjax = os.path.join(ipython_dir, 'nbextensions', 'mathjax')\n    NameError: name 'ipython_dir' is not defined\n**********************************************************************\nFile \"src/sage/repl/ipython_kernel/install.py\", line 120, in sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax\nFailed example:\n    os.path.exists(mathjax)\nException raised:\n    Traceback (most recent call last):\n      File \"/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 492, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 854, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax[5]>\", line 1, in <module>\n        os.path.exists(mathjax)\n    NameError: name 'mathjax' is not defined\n**********************************************************************\n1 item had failures:\n   3 of   7 in sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax\n    [37 tests, 3 failures, 0.16 s]\n```",
    "created_at": "2015-03-14T00:31:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308306",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'></a>
```
sage -t --long src/sage/repl/ipython_kernel/install.py
**********************************************************************
File "src/sage/repl/ipython_kernel/install.py", line 118, in sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax
Failed example:
    ipython_dir = get_ipython_dir()
Exception raised:
    Traceback (most recent call last):
      File "/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 492, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 854, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax[3]>", line 1, in <module>
        ipython_dir = get_ipython_dir()
    NameError: name 'get_ipython_dir' is not defined
**********************************************************************
File "src/sage/repl/ipython_kernel/install.py", line 119, in sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax
Failed example:
    mathjax = os.path.join(ipython_dir, 'nbextensions', 'mathjax')
Exception raised:
    Traceback (most recent call last):
      File "/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 492, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 854, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax[4]>", line 1, in <module>
        mathjax = os.path.join(ipython_dir, 'nbextensions', 'mathjax')
    NameError: name 'ipython_dir' is not defined
**********************************************************************
File "src/sage/repl/ipython_kernel/install.py", line 120, in sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax
Failed example:
    os.path.exists(mathjax)
Exception raised:
    Traceback (most recent call last):
      File "/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 492, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/mnt/disk/home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 854, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax[5]>", line 1, in <module>
        os.path.exists(mathjax)
    NameError: name 'mathjax' is not defined
**********************************************************************
1 item had failures:
   3 of   7 in sage.repl.ipython_kernel.install.SageKernelSpec.use_local_mathjax
    [37 tests, 3 failures, 0.16 s]
```



---

archive/issue_comments_308307.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2015-03-14T00:31:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308307",
    "user": "https://github.com/vbraun"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_308308.json:
```json
{
    "body": "<a id='comment:5'></a>Yes I am an idiot, I forgot an essential step in that doctest.",
    "created_at": "2015-03-14T00:45:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308308",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:5'></a>Yes I am an idiot, I forgot an essential step in that doctest.



---

archive/issue_comments_308309.json:
```json
{
    "body": "Changing commit from \"d3a0ed67bb4725020baf72c1a40c59cfadb742f5\" to \"191ac98e0a5e252e22d54f7a1251d6ebcdef7df6\"",
    "created_at": "2015-03-14T00:48:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308309",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "d3a0ed67bb4725020baf72c1a40c59cfadb742f5" to "191ac98e0a5e252e22d54f7a1251d6ebcdef7df6"



---

archive/issue_comments_308310.json:
```json
{
    "body": "<a id='comment:6'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-03-14T00:48:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308310",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_308311.json:
```json
{
    "body": "<a id='comment:7'></a>Fixed, back to need review.",
    "created_at": "2015-03-14T00:50:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308311",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:7'></a>Fixed, back to need review.



---

archive/issue_comments_308312.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2015-03-14T00:50:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308312",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_308313.json:
```json
{
    "body": "<a id='comment:8'></a>Hum didn't know you could go from `needs_work` to `positive_review` in one go.",
    "created_at": "2015-03-14T01:05:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308313",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:8'></a>Hum didn't know you could go from `needs_work` to `positive_review` in one go.



---

archive/issue_comments_308314.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-03-17T08:34:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308314",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_051060.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-17T08:34:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17946#event-51060"
}
```



---

archive/issue_comments_308315.json:
```json
{
    "body": "Changing branch from \"u/fbissey/ipythondir\" to \"191ac98e0a5e252e22d54f7a1251d6ebcdef7df6\"",
    "created_at": "2015-03-17T08:34:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17946",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17946#issuecomment-308315",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/fbissey/ipythondir" to "191ac98e0a5e252e22d54f7a1251d6ebcdef7df6"
