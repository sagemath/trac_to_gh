...
```json
{
    "body": "<a id='comment:3'></a>A mutable poset class should be stored in `sage.combinat.posets`.\n\nNathann",
...
<a id='comment:3'></a>A mutable poset class should be stored in `sage.combinat.posets`.
...
<a id='comment:4'></a>This series of comments are the result of an email discussion. Its aim is to give a better understanding of the needs of the data structure used in an asymptotic expression.
...
<a id='comment:5'></a>We want to perform arithmetic with asymptotic expressions, in particular we need to do efficiently:
1) addition (and multiplication)
2) merging/absorbing terms, e.g.
...
<a id='comment:6'></a>Note that all the terms (individual summands) support one
operation, namely these can be multiplied, maybe coercion is
used, e.g.
...
<a id='comment:8'></a>> This class has nothing to do with combinatorics, therefore I am against `sage.combinat`. And it implements a data structure and we have `sage.data_structures`, so it seems the right place.
...
<a id='comment:10'></a>> A `FinitePoset` inherits from `Parent` and my understanding is that parents must be immutable.
...
<a id='comment:13'></a>> > Graphs are a data structure
...
```json
{
    "body": "<a id='comment:15'></a>If `union()` does same thing as `disjoint_union()` in posets and in graphs, please change name accordingly. It seems unnecessary to have `INPUT`-part at all, if the function has no arguments at all.\n\nJust my two cents. I would give three, if I would have more time.",
...
<a id='comment:15'></a>If `union()` does same thing as `disjoint_union()` in posets and in graphs, please change name accordingly. It seems unnecessary to have `INPUT`-part at all, if the function has no arguments at all.
...
<a id='comment:18'></a>Replying to [dkrenn](#comment%3A16):
> Replying to [jmantysalo](#comment%3A15):
> > If `union()` does same thing as `disjoint_union()` in posets and in graphs - -
...
<a id='comment:20'></a>Replying to [jmantysalo](#comment%3A18):
> > The developer guide says
> > 
> >     An INPUT and an OUTPUT block describing the input/output of the function. This is not optional.
...
```json
{
    "body": "<a id='comment:26'></a>Benjamin, please put your name to reviewers-field. Otherwise Volker will reject this.",
...
<a id='comment:26'></a>Benjamin, please put your name to reviewers-field. Otherwise Volker will reject this.
...
```json
{
    "body": "<a id='comment:27'></a>Thanks for the reminder, Jori!\n\nBenjamin",
...
<a id='comment:27'></a>Thanks for the reminder, Jori!
...
<a id='comment:29'></a>Replying to [behackl](#comment%3A25):
> - `MutablePoset`, `MutablePosetShell`: derived from `object` vs. `SageObject`?
...
<a id='comment:32'></a>Hello Daniel,
...
```json
{
    "body": "<a id='comment:33'></a>Replying to [behackl](#comment%3A32):\n> - Is there a particular reason for deriving this from `object` instead of `SageObject`? Because [the documentation of SageObject](http://doc.sagemath.org/html/en/reference/structure/sage/structure/sage_object.html) states that \"Every object that can end up being returned to the user should inherit from SageObject.\"\n\n\nOk, then `SageObject` it is :)",
...
<a id='comment:33'></a>Replying to [behackl](#comment%3A32):
> - Is there a particular reason for deriving this from `object` instead of `SageObject`? Because [the documentation of SageObject](http://doc.sagemath.org/html/en/reference/structure/sage/structure/sage_object.html) states that "Every object that can end up being returned to the user should inherit from SageObject."
...
<a id='comment:34'></a>A short remark on this change of mind:
...
<a id='comment:36'></a>Replying to [behackl](#comment%3A34):
> This concludes my review; [cheuberg](#comment%3A8) is in the process of reading the code. I'll leave setting this to `positive_review` to him.
...
<a id='comment:43'></a>Thank you for your changes. I added three commits. See my remaining comments below.
...
<a id='comment:45'></a>Replying to [cheuberg](#comment%3A43):
> Thank you for your changes. I added three commits. See my remaining comments below.
...
<a id='comment:49'></a>Added three final reviewer commits.
