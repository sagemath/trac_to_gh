# Issue 17771: Implement the forget cycle map

archive/issues_017534.json:
```json
{
    "assignees": [],
    "body": "Implements the map on S<sub>n</sub> to S<sub>n</sub> which forgets the cycles of a permutation written in standard cyclic form and considers it as a 1-line permutation:\n\n```\n.. [CC13] Mahir Bilen Can and Yonah Cherniavsky.\n   *Omitting parentheses from the cyclic notation*. (2013).\n   :arxiv:`1308.0936v2`.\n```\n\nAssignee: **sage-combinat**\n\nCC:  sage-combinat @VivianePons stumpc5\n\nKeywords: **permutations, cycles**\n\nBranch/Commit: **[5884871](https://github.com/sagemath/sagetrac-mirror/commit/588487196d163547ef9ae0b471543856d6e30247)**\n\nReviewer: **Darij Grinberg**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17771_\n\n",
    "closed_at": "2015-02-18T01:26:48Z",
    "created_at": "2015-02-12T14:07:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement the forget cycle map",
    "type": "issue",
    "updated_at": "2015-02-18T01:26:48Z",
    "url": "https://github.com/sagemath/sage/issues/17771",
    "user": "https://github.com/tscrim"
}
```
Implements the map on S<sub>n</sub> to S<sub>n</sub> which forgets the cycles of a permutation written in standard cyclic form and considers it as a 1-line permutation:

```
.. [CC13] Mahir Bilen Can and Yonah Cherniavsky.
   *Omitting parentheses from the cyclic notation*. (2013).
   :arxiv:`1308.0936v2`.
```

Assignee: **sage-combinat**

CC:  sage-combinat @VivianePons stumpc5

Keywords: **permutations, cycles**

Branch/Commit: **[5884871](https://github.com/sagemath/sagetrac-mirror/commit/588487196d163547ef9ae0b471543856d6e30247)**

Reviewer: **Darij Grinberg**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/17771_





---

archive/issue_events_235396.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-02-12T14:07:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235396"
}
```



---

archive/issue_events_235397.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-02-12T14:07:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235397"
}
```



---

archive/issue_events_235398.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-02-12T14:07:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235398"
}
```



---

archive/issue_events_235399.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-02-12T14:07:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235399"
}
```



---

archive/issue_events_235400.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-02-12T14:12:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235400"
}
```



---

archive/issue_comments_242545.json:
```json
{
    "body": "Commit: **[12790ed](https://github.com/sagemath/sagetrac-mirror/commit/12790edd04b20632e09cc8c611cf57bffe7fa5cc)**",
    "created_at": "2015-02-12T14:12:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242545",
    "user": "https://github.com/tscrim"
}
```

Commit: **[12790ed](https://github.com/sagemath/sagetrac-mirror/commit/12790edd04b20632e09cc8c611cf57bffe7fa5cc)**



---

archive/issue_comments_242546.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/12790edd04b20632e09cc8c611cf57bffe7fa5cc\">12790ed</a></td><td><code>Implemented the forget cycles map.</code></td></tr></table>\n",
    "created_at": "2015-02-12T14:12:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242546",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/12790edd04b20632e09cc8c611cf57bffe7fa5cc">12790ed</a></td><td><code>Implemented the forget cycles map.</code></td></tr></table>




---

archive/issue_comments_242547.json:
```json
{
    "body": "Branch: **[public/combinat/forget_cycles-17771](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/forget_cycles-17771)**",
    "created_at": "2015-02-12T14:12:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242547",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/combinat/forget_cycles-17771](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/forget_cycles-17771)**



---

archive/issue_comments_242548.json:
```json
{
    "body": "Changed commit from **[12790ed](https://github.com/sagemath/sagetrac-mirror/commit/12790edd04b20632e09cc8c611cf57bffe7fa5cc)** to **[5884871](https://github.com/sagemath/sagetrac-mirror/commit/588487196d163547ef9ae0b471543856d6e30247)**",
    "created_at": "2015-02-12T16:41:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242548",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[12790ed](https://github.com/sagemath/sagetrac-mirror/commit/12790edd04b20632e09cc8c611cf57bffe7fa5cc)** to **[5884871](https://github.com/sagemath/sagetrac-mirror/commit/588487196d163547ef9ae0b471543856d6e30247)**



---

archive/issue_comments_242549.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aa5172a4b53f5f62c5e553649998181e4947829e\">aa5172a</a></td><td><code>Merge branch 'public/combinat/forget_cycles-17771' of git://trac.sagemath.org/sage into cycles</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/588487196d163547ef9ae0b471543856d6e30247\">5884871</a></td><td><code>explicited a property of to_cycles() used in the forget_cycles() code</code></td></tr></table>\n",
    "created_at": "2015-02-12T16:41:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242549",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aa5172a4b53f5f62c5e553649998181e4947829e">aa5172a</a></td><td><code>Merge branch 'public/combinat/forget_cycles-17771' of git://trac.sagemath.org/sage into cycles</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/588487196d163547ef9ae0b471543856d6e30247">5884871</a></td><td><code>explicited a property of to_cycles() used in the forget_cycles() code</code></td></tr></table>




---

archive/issue_comments_242550.json:
```json
{
    "body": "Reviewer: **Darij Grinberg**",
    "created_at": "2015-02-12T16:42:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242550",
    "user": "https://github.com/darijgr"
}
```

Reviewer: **Darij Grinberg**



---

archive/issue_comments_242551.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLGTM; if you agree, this is positive_review.",
    "created_at": "2015-02-12T16:42:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242551",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:4'>Comment 4:</a>
LGTM; if you agree, this is positive_review.



---

archive/issue_comments_242552.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLGTM, thanks.",
    "created_at": "2015-02-12T16:46:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242552",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
LGTM, thanks.



---

archive/issue_events_235401.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-02-12T16:46:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235401"
}
```



---

archive/issue_events_235402.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-02-12T16:46:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235402"
}
```



---

archive/issue_events_235403.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-02-12T17:00:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235403"
}
```



---

archive/issue_events_235404.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-02-12T17:00:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235404"
}
```



---

archive/issue_comments_242553.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nGuys, it looks like you are making many implicit assumptions on the output of `to_cycles` here. At the very least (and for a cheap cost) you should *check* them:\n\n1) The first element of each cycle is minimum on the cycle\n2) The cycles are ordered as they should\n\nNathann",
    "created_at": "2015-02-12T17:00:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242553",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'>Comment 6:</a>
Guys, it looks like you are making many implicit assumptions on the output of `to_cycles` here. At the very least (and for a cheap cost) you should *check* them:

1) The first element of each cycle is minimum on the cycle
2) The cycles are ordered as they should

Nathann



---

archive/issue_comments_242554.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI did check them, unless there is some subclass which overrides the method and is unaware of them. And I don't think such a thing exists.",
    "created_at": "2015-02-12T17:03:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242554",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:7'>Comment 7:</a>
I did check them, unless there is some subclass which overrides the method and is unaware of them. And I don't think such a thing exists.



---

archive/issue_events_235405.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2015-02-12T17:03:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235405"
}
```



---

archive/issue_events_235406.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2015-02-12T17:03:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235406"
}
```



---

archive/issue_comments_242555.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\n> I did check them, unless there is some subclass which overrides the method and is unaware of them. And I don't think such a thing exists.\n\nI meant that you could check, in the code of the new function, that those assumptions are valid. It has a cheap cost.\n\nThis way, if some user extends the class (or if we do it ourselves later) there is no risk of returning wrong results.\n\nNathann",
    "created_at": "2015-02-12T17:09:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242555",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:8'>Comment 8:</a>
> I did check them, unless there is some subclass which overrides the method and is unaware of them. And I don't think such a thing exists.

I meant that you could check, in the code of the new function, that those assumptions are valid. It has a cheap cost.

This way, if some user extends the class (or if we do it ourselves later) there is no risk of returning wrong results.

Nathann



---

archive/issue_comments_242556.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nUhm, I don't like this approach -- it means that there will still be a bug if someone extends the class, and that it will just manifest itself as an error being thrown rather than a wrong result, which is only a mitigation, not a fix.\n\nI have codified the added assumptions in the docstring of `to_cycles`, so at least noone who reads the doc should mess it up in a subclass. :)",
    "created_at": "2015-02-12T17:12:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242556",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:9'>Comment 9:</a>
Uhm, I don't like this approach -- it means that there will still be a bug if someone extends the class, and that it will just manifest itself as an error being thrown rather than a wrong result, which is only a mitigation, not a fix.

I have codified the added assumptions in the docstring of `to_cycles`, so at least noone who reads the doc should mess it up in a subclass. :)



---

archive/issue_events_235407.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-18T01:26:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235407"
}
```



---

archive/issue_events_235408.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "54f894d85aec2e8cf1f44d8ce2368e0941968512",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-02-18T01:26:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17771#event-235408"
}
```



---

archive/issue_comments_242557.json:
```json
{
    "body": "Changed branch from **[public/combinat/forget_cycles-17771](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/forget_cycles-17771)** to **[5884871](https://github.com/sagemath/sagetrac-mirror/commit/588487196d163547ef9ae0b471543856d6e30247)**",
    "created_at": "2015-02-18T01:26:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17771",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17771#issuecomment-242557",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/combinat/forget_cycles-17771](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/forget_cycles-17771)** to **[5884871](https://github.com/sagemath/sagetrac-mirror/commit/588487196d163547ef9ae0b471543856d6e30247)**
