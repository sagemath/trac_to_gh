# Issue 17450: Fix category for quotients of polynomial rings

archive/issues_017213.json:
```json
{
    "assignees": [],
    "body": "We currently have the following discrepancy\n\n```\nR.<x> = ZZ[]\nS.<t> = R.quo(x^2+5)\nS in IntegralDomains()\nFalse\nS.is_integral_domain()\nTrue\n```\n\n\nAssignee: **@tscrim**\n\nCC:  @bhutz\n\nKeywords: **quotients, polynomial rings**\n\nBranch/Commit: **[eaf4c10](https://github.com/sagemath/sagetrac-mirror/commit/eaf4c101e899afb963a84922cf5002d6681d7d6b)**\n\nReviewer: **Ben Hutz**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17450_\n\n",
    "closed_at": "2014-12-15T13:31:28Z",
    "created_at": "2014-12-06T16:16:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20categories",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix category for quotients of polynomial rings",
    "type": "issue",
    "updated_at": "2014-12-15T13:31:28Z",
    "url": "https://github.com/sagemath/sage/issues/17450",
    "user": "https://github.com/tscrim"
}
```
We currently have the following discrepancy

```
R.<x> = ZZ[]
S.<t> = R.quo(x^2+5)
S in IntegralDomains()
False
S.is_integral_domain()
True
```


Assignee: **@tscrim**

CC:  @bhutz

Keywords: **quotients, polynomial rings**

Branch/Commit: **[eaf4c10](https://github.com/sagemath/sagetrac-mirror/commit/eaf4c101e899afb963a84922cf5002d6681d7d6b)**

Reviewer: **Ben Hutz**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/17450_





---

archive/issue_events_231105.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-12-06T16:16:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17450#event-231105"
}
```



---

archive/issue_events_231106.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-12-06T16:16:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20categories",
    "label_color": "0000ff",
    "label_name": "component: categories",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17450#event-231106"
}
```



---

archive/issue_events_231107.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-12-06T16:16:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17450#event-231107"
}
```



---

archive/issue_events_231108.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-12-06T16:16:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17450#event-231108"
}
```



---

archive/issue_events_231109.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-12-06T16:27:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17450#event-231109"
}
```



---

archive/issue_comments_236142.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eaf4c101e899afb963a84922cf5002d6681d7d6b\">eaf4c10</a></td><td><code>Put poly quotients domains in the category of IntegralDomains.</code></td></tr></table>\n",
    "created_at": "2014-12-06T16:27:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17450#issuecomment-236142",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eaf4c101e899afb963a84922cf5002d6681d7d6b">eaf4c10</a></td><td><code>Put poly quotients domains in the category of IntegralDomains.</code></td></tr></table>




---

archive/issue_comments_236143.json:
```json
{
    "body": "Branch: **[public/categories/polynomial_quotient_rings-17450](https://github.com/sagemath/sagetrac-mirror/tree/public/categories/polynomial_quotient_rings-17450)**",
    "created_at": "2014-12-06T16:27:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17450#issuecomment-236143",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/categories/polynomial_quotient_rings-17450](https://github.com/sagemath/sagetrac-mirror/tree/public/categories/polynomial_quotient_rings-17450)**



---

archive/issue_comments_236144.json:
```json
{
    "body": "Commit: **[eaf4c10](https://github.com/sagemath/sagetrac-mirror/commit/eaf4c101e899afb963a84922cf5002d6681d7d6b)**",
    "created_at": "2014-12-06T16:27:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17450#issuecomment-236144",
    "user": "https://github.com/tscrim"
}
```

Commit: **[eaf4c10](https://github.com/sagemath/sagetrac-mirror/commit/eaf4c101e899afb963a84922cf5002d6681d7d6b)**



---

archive/issue_events_231110.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-07T00:08:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17450#event-231110"
}
```



---

archive/issue_events_231111.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-07T00:08:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17450#event-231111"
}
```



---

archive/issue_comments_236145.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis fixes the issue for quotients of polynomial rings.",
    "created_at": "2014-12-07T00:08:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17450#issuecomment-236145",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:2" align="right">Comment 2</div>

This fixes the issue for quotients of polynomial rings.



---

archive/issue_comments_236146.json:
```json
{
    "body": "Reviewer: **Ben Hutz**",
    "created_at": "2014-12-07T00:08:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17450#issuecomment-236146",
    "user": "https://github.com/bhutz"
}
```

Reviewer: **Ben Hutz**



---

archive/issue_events_231112.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-12-15T13:31:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17450#event-231112"
}
```



---

archive/issue_events_231113.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "510547f0edb0ca868cd6e8bc951ed4ae03618149",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-12-15T13:31:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17450#event-231113"
}
```



---

archive/issue_comments_236147.json:
```json
{
    "body": "Changed branch from **[public/categories/polynomial_quotient_rings-17450](https://github.com/sagemath/sagetrac-mirror/tree/public/categories/polynomial_quotient_rings-17450)** to **[eaf4c10](https://github.com/sagemath/sagetrac-mirror/commit/eaf4c101e899afb963a84922cf5002d6681d7d6b)**",
    "created_at": "2014-12-15T13:31:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17450",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17450#issuecomment-236147",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/categories/polynomial_quotient_rings-17450](https://github.com/sagemath/sagetrac-mirror/tree/public/categories/polynomial_quotient_rings-17450)** to **[eaf4c10](https://github.com/sagemath/sagetrac-mirror/commit/eaf4c101e899afb963a84922cf5002d6681d7d6b)**
