# Issue 17815: Remove verbose from the global namespace

archive/issues_017578.json:
```json
{
    "assignees": [],
    "body": "It is confusing to users. Why are we exposing our logging system to end users?\n\n#29895 deprecates this import to `sage.misc.all` and hence to the global namespace.\n\nAfter the deprecation period, the present ticket will remove it.\n\nSee also:\n- #21349: build verbose upon Python's logging module\n\n\nDepends on #29895\n\nCC:  @fchapoton @tscrim\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17815_\n\n",
    "created_at": "2015-02-20T09:16:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/pending",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove verbose from the global namespace",
    "type": "issue",
    "updated_at": "2020-06-18T23:57:18Z",
    "url": "https://github.com/sagemath/sage/issues/17815",
    "user": "https://github.com/rwst"
}
```
It is confusing to users. Why are we exposing our logging system to end users?

#29895 deprecates this import to `sage.misc.all` and hence to the global namespace.

After the deprecation period, the present ticket will remove it.

See also:
- #21349: build verbose upon Python's logging module


Depends on #29895

CC:  @fchapoton @tscrim

_Issue created by migration from https://trac.sagemath.org/ticket/17815_





---

archive/issue_events_235948.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-20T09:16:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235948"
}
```



---

archive/issue_events_235949.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-20T09:16:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235949"
}
```



---

archive/issue_events_235950.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-20T09:16:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235950"
}
```



---

archive/issue_events_235951.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-20T09:16:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235951"
}
```



---

archive/issue_comments_243228.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nWrong command! It should be `set_verbose(-1)`.",
    "created_at": "2015-03-22T14:14:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17815#issuecomment-243228",
    "user": "https://github.com/rwst"
}
```

<a id='comment:1'>Comment 1:</a>
Wrong command! It should be `set_verbose(-1)`.



---

archive/issue_events_235952.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-22T14:14:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235952"
}
```



---

archive/issue_events_235953.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-22T14:14:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235953"
}
```



---

archive/issue_events_235954.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-22T14:14:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235954"
}
```



---

archive/issue_events_235955.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-22T14:14:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235955"
}
```



---

archive/issue_events_235956.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-22T14:14:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235956"
}
```



---

archive/issue_events_235957.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-22T14:14:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235957"
}
```



---

archive/issue_events_235958.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-22T22:57:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235958"
}
```



---

archive/issue_events_235959.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-22T22:57:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235959"
}
```



---

archive/issue_events_235960.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-22T22:57:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235960"
}
```



---

archive/issue_events_235961.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-22T22:57:02Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "title_is": "Remove verbose from the global namespace",
    "title_was": "cannot turn off plot warnings with verbose()",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235961"
}
```



---

archive/issue_comments_243229.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,10 +1,3 @@\n-What use is `sage.misc.misc.verbose(\"WARNING:...` in `plot.py` when no possible verbosity level will turn off the output?\n+Its confusing to users. Why are we exposing our logging system to end users?\n \n-```\n-sage: verbose(level=-1)\n-verbose -1 (<module>) \n-5.786\n-sage: plot(sqrt(x),(x,-1,3))\n-verbose 0 (2691: plot.py, generate_plot_points) WARNING: When plotting, failed to evaluate function at 51 points.\n-verbose 0 (2691: plot.py, generate_plot_points) Last error message: 'math domain error'\n-```\n+Stretch goal: Use Python logging instead of reinventing the logging wheel.\n``````\n",
    "created_at": "2015-03-22T22:57:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17815#issuecomment-243229",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,10 +1,3 @@
-What use is `sage.misc.misc.verbose("WARNING:...` in `plot.py` when no possible verbosity level will turn off the output?
+Its confusing to users. Why are we exposing our logging system to end users?
 
-```
-sage: verbose(level=-1)
-verbose -1 (<module>) 
-5.786
-sage: plot(sqrt(x),(x,-1,3))
-verbose 0 (2691: plot.py, generate_plot_points) WARNING: When plotting, failed to evaluate function at 51 points.
-verbose 0 (2691: plot.py, generate_plot_points) Last error message: 'math domain error'
-```
+Stretch goal: Use Python logging instead of reinventing the logging wheel.
``````




---

archive/issue_comments_243230.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI suggest to repurpose the ticket, see new description. The old description follows:\n\nWhat use is `sage.misc.misc.verbose(\"WARNING:...` in `plot.py` when no possible verbosity level will turn off the output?\n\n```\nsage: verbose(level=-1)\nverbose -1 (<module>) \n5.786\nsage: plot(sqrt(x),(x,-1,3))\nverbose 0 (2691: plot.py, generate_plot_points) WARNING: When plotting, failed to evaluate function at 51 points.\nverbose 0 (2691: plot.py, generate_plot_points) Last error message: 'math domain error'\n```",
    "created_at": "2015-03-22T22:57:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17815#issuecomment-243230",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'>Comment 4:</a>
I suggest to repurpose the ticket, see new description. The old description follows:

What use is `sage.misc.misc.verbose("WARNING:...` in `plot.py` when no possible verbosity level will turn off the output?

```
sage: verbose(level=-1)
verbose -1 (<module>) 
5.786
sage: plot(sqrt(x),(x,-1,3))
verbose 0 (2691: plot.py, generate_plot_points) WARNING: When plotting, failed to evaluate function at 51 points.
verbose 0 (2691: plot.py, generate_plot_points) Last error message: 'math domain error'
```



---

archive/issue_comments_243231.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nSee also #21349.",
    "created_at": "2016-08-30T11:59:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17815#issuecomment-243231",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:5'>Comment 5:</a>
See also #21349.



---

archive/issue_events_235962.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T18:29:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235962"
}
```



---

archive/issue_events_235963.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T18:29:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235963"
}
```



---

archive/issue_comments_243232.json:
```json
{
    "body": "Dependencies: **#29895**",
    "created_at": "2020-06-18T20:00:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17815#issuecomment-243232",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#29895**



---

archive/issue_events_235964.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T23:57:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235964"
}
```



---

archive/issue_events_235965.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T23:57:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17815#event-235965"
}
```



---

archive/issue_comments_243233.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,9 @@\n-Its confusing to users. Why are we exposing our logging system to end users?\n+It is confusing to users. Why are we exposing our logging system to end users?\n \n-Stretch goal: Use Python logging instead of reinventing the logging wheel.\n+#29895 deprecates this import to `sage.misc.all` and hence to the global namespace.\n+\n+After the deprecation period, the present ticket will remove it.\n+\n+See also:\n+- #21349: build verbose upon Python's logging module\n+\n``````\n",
    "created_at": "2020-06-18T23:57:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17815#issuecomment-243233",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,9 @@
-Its confusing to users. Why are we exposing our logging system to end users?
+It is confusing to users. Why are we exposing our logging system to end users?
 
-Stretch goal: Use Python logging instead of reinventing the logging wheel.
+#29895 deprecates this import to `sage.misc.all` and hence to the global namespace.
+
+After the deprecation period, the present ticket will remove it.
+
+See also:
+- #21349: build verbose upon Python's logging module
+
``````

