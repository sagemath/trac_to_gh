# Issue 17561: Speed up vectors

archive/issues_017324.json:
```json
{
    "assignees": [],
    "body": "We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:\n\nBefore:\n\n```\nsage: v = vector(RR, range(10000)); timeit('v._add_(v)', repeat=10, number=1000)\n1000 loops, best of 10: 1.85 ms per loop\nsage: v = vector(RR, range(10000), sparse=True); timeit('v._add_(v)', repeat=10, number=1000)\n1000 loops, best of 10: 3.49 ms per loop\n```\n\nAfter:\n\n```\nsage: v = vector(RR, range(10000)); timeit('v._add_(v)', repeat=10, number=1000)\n1000 loops, best of 10: 1.82 ms per loop\nsage: v = vector(RR, range(10000), sparse=True); timeit('v._add_(v)', repeat=10, number=1000)\n1000 loops, best of 10: 2.85 ms per loop\n```\n\nBranch/Commit: **[106f5d4](https://github.com/sagemath/sagetrac-mirror/commit/106f5d41398794b13a6a683236cf219d49602312)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17561_\n\n",
    "closed_at": "2015-01-13T01:11:26Z",
    "created_at": "2014-12-29T08:17:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/performance",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Speed up vectors",
    "type": "issue",
    "updated_at": "2015-01-13T01:11:26Z",
    "url": "https://github.com/sagemath/sage/issues/17561",
    "user": "https://github.com/jdemeyer"
}
```
We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:

Before:

```
sage: v = vector(RR, range(10000)); timeit('v._add_(v)', repeat=10, number=1000)
1000 loops, best of 10: 1.85 ms per loop
sage: v = vector(RR, range(10000), sparse=True); timeit('v._add_(v)', repeat=10, number=1000)
1000 loops, best of 10: 3.49 ms per loop
```

After:

```
sage: v = vector(RR, range(10000)); timeit('v._add_(v)', repeat=10, number=1000)
1000 loops, best of 10: 1.82 ms per loop
sage: v = vector(RR, range(10000), sparse=True); timeit('v._add_(v)', repeat=10, number=1000)
1000 loops, best of 10: 2.85 ms per loop
```

Branch/Commit: **[106f5d4](https://github.com/sagemath/sagetrac-mirror/commit/106f5d41398794b13a6a683236cf219d49602312)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/17561_





---

archive/issue_events_232606.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-12-29T08:17:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232606"
}
```



---

archive/issue_events_232607.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-12-29T08:17:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/performance",
    "label_color": "696969",
    "label_name": "performance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232607"
}
```



---

archive/issue_events_232608.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-12-29T08:17:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232608"
}
```



---

archive/issue_events_232609.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-12-29T08:17:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232609"
}
```



---

archive/issue_comments_238226.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,19 @@\n+We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:\n \n+Before:\n+\n+```\n+sage: v = vector(RR, range(10000)); timeit('v._add_(v)', repeat=10, number=1000)\n+1000 loops, best of 10: 1.85 ms per loop\n+sage: v = vector(RR, range(10000), sparse=True); timeit('v._add_(v)', repeat=10, number=1000)\n+1000 loops, best of 10: 3.49 ms per loop\n+```\n+\n+After:\n+\n+```\n+sage: v = vector(RR, range(10000)); timeit('v._add_(v)', repeat=10, number=1000)\n+1000 loops, best of 10: 1.82 ms per loop\n+sage: v = vector(RR, range(10000), sparse=True); timeit('v._add_(v)', repeat=10, number=1000)\n+1000 loops, best of 10: 2.85 ms per loop\n+```\n``````\n",
    "created_at": "2014-12-29T08:36:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238226",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,19 @@
+We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:
 
+Before:
+
+```
+sage: v = vector(RR, range(10000)); timeit('v._add_(v)', repeat=10, number=1000)
+1000 loops, best of 10: 1.85 ms per loop
+sage: v = vector(RR, range(10000), sparse=True); timeit('v._add_(v)', repeat=10, number=1000)
+1000 loops, best of 10: 3.49 ms per loop
+```
+
+After:
+
+```
+sage: v = vector(RR, range(10000)); timeit('v._add_(v)', repeat=10, number=1000)
+1000 loops, best of 10: 1.82 ms per loop
+sage: v = vector(RR, range(10000), sparse=True); timeit('v._add_(v)', repeat=10, number=1000)
+1000 loops, best of 10: 2.85 ms per loop
+```
``````




---

archive/issue_events_232610.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-12-29T09:57:46Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "title_is": "Speed up vectors",
    "title_was": "Declare types in free_module_element",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232610"
}
```



---

archive/issue_comments_238227.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,24 @@\n-We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:\n+1. Currently, every call to `vector()` computes a basis, only to ask for the ring over which the basis is defined (see #11751). We avoid this by introducing a new method `basis_ring()` for free modules.\n+\n+Before:\n+\n+```\n+sage: L = [RR(x) for x in range(10000)]\n+sage: timeit('vector(RR, L)')\n+5 loops, best of 3: 77.4 ms per loop\n+```\n+\n+After:\n+\n+```\n+sage: L = [RR(x) for x in range(10000)]\n+sage: timeit('vector(RR, L)')\n+125 loops, best of 3: 1.82 ms per loop\n+```\n+\n+---\n+\n+2. We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:\n \n Before:\n \n``````\n",
    "created_at": "2014-12-29T09:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238227",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,24 @@
-We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:
+1. Currently, every call to `vector()` computes a basis, only to ask for the ring over which the basis is defined (see #11751). We avoid this by introducing a new method `basis_ring()` for free modules.
+
+Before:
+
+```
+sage: L = [RR(x) for x in range(10000)]
+sage: timeit('vector(RR, L)')
+5 loops, best of 3: 77.4 ms per loop
+```
+
+After:
+
+```
+sage: L = [RR(x) for x in range(10000)]
+sage: timeit('vector(RR, L)')
+125 loops, best of 3: 1.82 ms per loop
+```
+
+---
+
+2. We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:
 
 Before:
 
``````




---

archive/issue_comments_238228.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/17561](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/17561)**",
    "created_at": "2014-12-29T10:57:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238228",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/17561](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/17561)**



---

archive/issue_comments_238229.json:
```json
{
    "body": "Commit: **[1233d6d](https://github.com/sagemath/sagetrac-mirror/commit/1233d6d27e83a6f359d9e2e1121c7cbbd06803a0)**",
    "created_at": "2014-12-29T12:21:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238229",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[1233d6d](https://github.com/sagemath/sagetrac-mirror/commit/1233d6d27e83a6f359d9e2e1121c7cbbd06803a0)**



---

archive/issue_comments_238230.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1233d6d27e83a6f359d9e2e1121c7cbbd06803a0\">1233d6d</a></td><td><code>Speed up vectors</code></td></tr></table>\n",
    "created_at": "2014-12-29T12:21:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238230",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1233d6d27e83a6f359d9e2e1121c7cbbd06803a0">1233d6d</a></td><td><code>Speed up vectors</code></td></tr></table>




---

archive/issue_events_232611.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-12-29T12:21:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232611"
}
```



---

archive/issue_events_232612.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-12-31T10:45:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232612"
}
```



---

archive/issue_events_232613.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-12-31T10:45:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232613"
}
```



---

archive/issue_comments_238231.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,24 +1,4 @@\n-1. Currently, every call to `vector()` computes a basis, only to ask for the ring over which the basis is defined (see #11751). We avoid this by introducing a new method `basis_ring()` for free modules.\n-\n-Before:\n-\n-```\n-sage: L = [RR(x) for x in range(10000)]\n-sage: timeit('vector(RR, L)')\n-5 loops, best of 3: 77.4 ms per loop\n-```\n-\n-After:\n-\n-```\n-sage: L = [RR(x) for x in range(10000)]\n-sage: timeit('vector(RR, L)')\n-125 loops, best of 3: 1.82 ms per loop\n-```\n-\n----\n-\n-2. We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:\n+We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:\n \n Before:\n \n``````\n",
    "created_at": "2015-01-02T23:44:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238231",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,24 +1,4 @@
-1. Currently, every call to `vector()` computes a basis, only to ask for the ring over which the basis is defined (see #11751). We avoid this by introducing a new method `basis_ring()` for free modules.
-
-Before:
-
-```
-sage: L = [RR(x) for x in range(10000)]
-sage: timeit('vector(RR, L)')
-5 loops, best of 3: 77.4 ms per loop
-```
-
-After:
-
-```
-sage: L = [RR(x) for x in range(10000)]
-sage: timeit('vector(RR, L)')
-125 loops, best of 3: 1.82 ms per loop
-```
-
----
-
-2. We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:
+We should declare the types `_entries` in `free_module_element.pxd`. This leads to a significant speed-up in the sparse case, and a minor speedup in the dense case:
 
 Before:
 
``````




---

archive/issue_comments_238232.json:
```json
{
    "body": "Changed commit from **[1233d6d](https://github.com/sagemath/sagetrac-mirror/commit/1233d6d27e83a6f359d9e2e1121c7cbbd06803a0)** to **[df0fb69](https://github.com/sagemath/sagetrac-mirror/commit/df0fb69e1aa9ab205e35b98ad6d97d0c67b889af)**",
    "created_at": "2015-01-03T00:05:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238232",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1233d6d](https://github.com/sagemath/sagetrac-mirror/commit/1233d6d27e83a6f359d9e2e1121c7cbbd06803a0)** to **[df0fb69](https://github.com/sagemath/sagetrac-mirror/commit/df0fb69e1aa9ab205e35b98ad6d97d0c67b889af)**



---

archive/issue_comments_238233.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/df0fb69e1aa9ab205e35b98ad6d97d0c67b889af\">df0fb69</a></td><td><code>Declare types of _entries</code></td></tr></table>\n",
    "created_at": "2015-01-03T00:05:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238233",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:8'>Comment 8:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/df0fb69e1aa9ab205e35b98ad6d97d0c67b889af">df0fb69</a></td><td><code>Declare types of _entries</code></td></tr></table>




---

archive/issue_events_232614.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-03T00:06:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232614"
}
```



---

archive/issue_events_232615.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-03T00:06:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232615"
}
```



---

archive/issue_comments_238234.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nIs there a reason why you do\n\n```\nif type(entries) is not dict:\n```\ninstead of\n\n```\nif isinstance(entries, dict):\n```\n?",
    "created_at": "2015-01-03T03:27:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238234",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:10'>Comment 10:</a>
Is there a reason why you do

```
if type(entries) is not dict:
```
instead of

```
if isinstance(entries, dict):
```
?



---

archive/issue_comments_238235.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nReplying to [@tscrim](#comment%3A10):\n> Is there a reason why you do\n> \n> ```\n> if type(entries) is not dict:\n> ```\n> instead of\n> \n> ```\n> if isinstance(entries, dict):\n> ```\n> ?\n\nYes, I need to assign it `_entries` which is declared as `dict`. So the type really has to be `dict`, not some derived type.\n\nBut we should support derived types, which is not currently the case.",
    "created_at": "2015-01-03T09:41:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238235",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:11'>Comment 11:</a>
Replying to [@tscrim](#comment%3A10):
> Is there a reason why you do
> 
> ```
> if type(entries) is not dict:
> ```
> instead of
> 
> ```
> if isinstance(entries, dict):
> ```
> ?

Yes, I need to assign it `_entries` which is declared as `dict`. So the type really has to be `dict`, not some derived type.

But we should support derived types, which is not currently the case.



---

archive/issue_events_232616.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-03T09:41:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232616"
}
```



---

archive/issue_events_232617.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-03T09:41:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232617"
}
```



---

archive/issue_comments_238236.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/106f5d41398794b13a6a683236cf219d49602312\">106f5d4</a></td><td><code>Minor improvements in FreeModuleElement_generic_sparse.__init__</code></td></tr></table>\n",
    "created_at": "2015-01-03T10:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238236",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:12'>Comment 12:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/106f5d41398794b13a6a683236cf219d49602312">106f5d4</a></td><td><code>Minor improvements in FreeModuleElement_generic_sparse.__init__</code></td></tr></table>




---

archive/issue_comments_238237.json:
```json
{
    "body": "Changed commit from **[df0fb69](https://github.com/sagemath/sagetrac-mirror/commit/df0fb69e1aa9ab205e35b98ad6d97d0c67b889af)** to **[106f5d4](https://github.com/sagemath/sagetrac-mirror/commit/106f5d41398794b13a6a683236cf219d49602312)**",
    "created_at": "2015-01-03T10:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238237",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[df0fb69](https://github.com/sagemath/sagetrac-mirror/commit/df0fb69e1aa9ab205e35b98ad6d97d0c67b889af)** to **[106f5d4](https://github.com/sagemath/sagetrac-mirror/commit/106f5d41398794b13a6a683236cf219d49602312)**



---

archive/issue_events_232618.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-03T10:18:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232618"
}
```



---

archive/issue_events_232619.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-01-03T10:18:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232619"
}
```



---

archive/issue_comments_238238.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nI see. I was thinking too much like in Java or pointers in C++. LGTM.",
    "created_at": "2015-01-10T03:00:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238238",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:14'>Comment 14:</a>
I see. I was thinking too much like in Java or pointers in C++. LGTM.



---

archive/issue_events_232620.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-01-10T03:00:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232620"
}
```



---

archive/issue_events_232621.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-01-10T03:00:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232621"
}
```



---

archive/issue_comments_238239.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2015-01-10T03:00:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238239",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_232622.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-13T01:11:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232622"
}
```



---

archive/issue_events_232623.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3e63e1a7d7eb9d8be23149faa6645b7c7fb678ba",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-13T01:11:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17561#event-232623"
}
```



---

archive/issue_comments_238240.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/17561](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/17561)** to **[106f5d4](https://github.com/sagemath/sagetrac-mirror/commit/106f5d41398794b13a6a683236cf219d49602312)**",
    "created_at": "2015-01-13T01:11:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17561#issuecomment-238240",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/17561](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/17561)** to **[106f5d4](https://github.com/sagemath/sagetrac-mirror/commit/106f5d41398794b13a6a683236cf219d49602312)**
