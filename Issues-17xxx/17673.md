# Issue 17673: Make NamedBinopMethod remember the name and module

archive/issues_017436.json:
```json
{
    "assignees": [],
    "body": "Currently\n\n```\nsage: from sage.structure.element import coerce_binop\nsage: @coerce_binop\n....: def f(a,b): return a+b\nsage: f.__module__\nTraceback (most recent call last):\n...\nAttributeError: ...\nsage: f.__name__\nTraceback (most recent call last):\n...\nAttributeError: ...\n```\nand with the branch\n\n```\nsage: from sage.structure.element import coerce_binop\nsage: @coerce_binop\n....: def f(a,b): return a+b\nsage: f.__module__\n'__main__'\nsage: f.__name__\n'f'\n```\n\nThat way methods with this decorator will looks like method. In particular it will not break introspection (e.g. access to the documentation with ? and to the source code with ??).\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17673_\n\n",
    "closed_at": "2015-01-29T15:48:19Z",
    "created_at": "2015-01-25T22:05:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make NamedBinopMethod remember the name and module",
    "type": "issue",
    "updated_at": "2015-01-29T15:48:19Z",
    "url": "https://github.com/sagemath/sage/issues/17673",
    "user": "https://github.com/videlec"
}
```
Currently

```
sage: from sage.structure.element import coerce_binop
sage: @coerce_binop
....: def f(a,b): return a+b
sage: f.__module__
Traceback (most recent call last):
...
AttributeError: ...
sage: f.__name__
Traceback (most recent call last):
...
AttributeError: ...
```
and with the branch

```
sage: from sage.structure.element import coerce_binop
sage: @coerce_binop
....: def f(a,b): return a+b
sage: f.__module__
'__main__'
sage: f.__name__
'f'
```

That way methods with this decorator will looks like method. In particular it will not break introspection (e.g. access to the documentation with ? and to the source code with ??).

_Issue created by migration from https://trac.sagemath.org/ticket/17673_





---

archive/issue_events_234141.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-25T22:05:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234141"
}
```



---

archive/issue_events_234142.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-25T22:05:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234142"
}
```



---

archive/issue_events_234143.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-25T22:05:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234143"
}
```



---

archive/issue_events_234144.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-25T22:05:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234144"
}
```



---

archive/issue_comments_241049.json:
```json
{
    "body": "Branch: **[public/17673](https://github.com/sagemath/sagetrac-mirror/tree/public/17673)**",
    "created_at": "2015-01-25T22:08:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17673#issuecomment-241049",
    "user": "https://github.com/videlec"
}
```

Branch: **[public/17673](https://github.com/sagemath/sagetrac-mirror/tree/public/17673)**



---

archive/issue_comments_241050.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7d2009c7539cb638e492e0518f4dc11cc7f1d34c\">7d2009c</a></td><td><code>trac #17673: __module__/__name__ for coerce_binop</code></td></tr></table>\n",
    "created_at": "2015-01-25T22:08:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17673#issuecomment-241050",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7d2009c7539cb638e492e0518f4dc11cc7f1d34c">7d2009c</a></td><td><code>trac #17673: __module__/__name__ for coerce_binop</code></td></tr></table>




---

archive/issue_comments_241051.json:
```json
{
    "body": "Commit: **[7d2009c](https://github.com/sagemath/sagetrac-mirror/commit/7d2009c7539cb638e492e0518f4dc11cc7f1d34c)**",
    "created_at": "2015-01-25T22:08:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17673#issuecomment-241051",
    "user": "https://github.com/videlec"
}
```

Commit: **[7d2009c](https://github.com/sagemath/sagetrac-mirror/commit/7d2009c7539cb638e492e0518f4dc11cc7f1d34c)**



---

archive/issue_events_234145.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-25T22:08:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234145"
}
```



---

archive/issue_events_234146.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-28T15:23:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234146"
}
```



---

archive/issue_events_234147.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-28T15:23:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234147"
}
```



---

archive/issue_comments_241052.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -24,3 +24,5 @@\n sage: f.__name__\n 'f'\n ```\n+\n+That way methods with this decorator will looks like method. In particular it will not break introspection (e.g. access to the documentation with ? and to the source code with ??).\n``````\n",
    "created_at": "2015-01-28T15:23:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17673#issuecomment-241052",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -24,3 +24,5 @@
 sage: f.__name__
 'f'
 ```
+
+That way methods with this decorator will looks like method. In particular it will not break introspection (e.g. access to the documentation with ? and to the source code with ??).
``````




---

archive/issue_comments_241053.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nIt seems that I had a bug on my computer since the introspection is perfectly fine without this... obscure!",
    "created_at": "2015-01-28T15:23:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17673#issuecomment-241053",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">Comment 2</div>

It seems that I had a bug on my computer since the introspection is perfectly fine without this... obscure!



---

archive/issue_events_234148.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-28T15:23:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234148"
}
```



---

archive/issue_events_234149.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-28T15:23:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234149"
}
```



---

archive/issue_comments_241054.json:
```json
{
    "body": "Changed branch from **[public/17673](https://github.com/sagemath/sagetrac-mirror/tree/public/17673)** to none",
    "created_at": "2015-01-28T15:23:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17673#issuecomment-241054",
    "user": "https://github.com/videlec"
}
```

Changed branch from **[public/17673](https://github.com/sagemath/sagetrac-mirror/tree/public/17673)** to none



---

archive/issue_comments_241055.json:
```json
{
    "body": "Changed commit from **[7d2009c](https://github.com/sagemath/sagetrac-mirror/commit/7d2009c7539cb638e492e0518f4dc11cc7f1d34c)** to none",
    "created_at": "2015-01-28T15:23:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17673#issuecomment-241055",
    "user": "https://github.com/videlec"
}
```

Changed commit from **[7d2009c](https://github.com/sagemath/sagetrac-mirror/commit/7d2009c7539cb638e492e0518f4dc11cc7f1d34c)** to none



---

archive/issue_events_234150.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-28T15:23:29Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234150"
}
```



---

archive/issue_events_234151.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-28T15:23:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234151"
}
```



---

archive/issue_events_234152.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-01-28T15:23:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234152"
}
```



---

archive/issue_comments_241056.json:
```json
{
    "body": "Changed author from **Vincent Delecroix** to none",
    "created_at": "2015-01-28T15:23:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17673#issuecomment-241056",
    "user": "https://github.com/videlec"
}
```

Changed author from **Vincent Delecroix** to none



---

archive/issue_events_234153.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-29T15:48:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234153"
}
```



---

archive/issue_events_234154.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-29T15:48:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234154"
}
```



---

archive/issue_events_234155.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-29T15:48:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17673",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17673#event-234155"
}
```
