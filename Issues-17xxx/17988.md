# Issue 17988: Implement fully-packed loops (duplicate of 18003)

archive/issues_017751.json:
```json
{
    "assignees": [],
    "body": "This is a duplicate of Ticket 18003.\n\nImplement fully-packed loops, based on the six vertex model already implemented.\n\nPreviously on branch u/vinceknight/fpl\n\nCC:  @sagetrac-jamespropp @kevindilks @egunawan @theref @drvinceknight @tscrim\n\nKeywords: **days64, asm, fpl**\n\nReviewer: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17988_\n\n",
    "closed_at": "2015-03-24T10:49:21Z",
    "created_at": "2015-03-18T20:41:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement fully-packed loops (duplicate of 18003)",
    "type": "issue",
    "updated_at": "2015-03-24T10:49:21Z",
    "url": "https://github.com/sagemath/sage/issues/17988",
    "user": "https://github.com/jessicapalencia"
}
```
This is a duplicate of Ticket 18003.

Implement fully-packed loops, based on the six vertex model already implemented.

Previously on branch u/vinceknight/fpl

CC:  @sagetrac-jamespropp @kevindilks @egunawan @theref @drvinceknight @tscrim

Keywords: **days64, asm, fpl**

Reviewer: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/17988_





---

archive/issue_events_238246.json:
```json
{
    "actor": "https://github.com/jessicapalencia",
    "created_at": "2015-03-18T20:41:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238246"
}
```



---

archive/issue_events_238247.json:
```json
{
    "actor": "https://github.com/jessicapalencia",
    "created_at": "2015-03-18T20:41:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238247"
}
```



---

archive/issue_events_238248.json:
```json
{
    "actor": "https://github.com/jessicapalencia",
    "created_at": "2015-03-18T20:41:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238248"
}
```



---

archive/issue_events_238249.json:
```json
{
    "actor": "https://github.com/jessicapalencia",
    "created_at": "2015-03-18T20:41:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238249"
}
```



---

archive/issue_comments_247024.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nCurrent plan: Create fullypackedloop.py and class. Making underlying object the corresponding six vertex model. Add methods for ascii/graphical representation by modifying corresponding code for six vertex model. Add method in alternatingsignmatrix.py to map to fully packed loop.\n\nFor later: Add additional structure to FPLs so that the link pattern/non-crossing partition structure can be extracted.",
    "created_at": "2015-03-18T21:01:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247024",
    "user": "https://github.com/kevindilks"
}
```

<div id="comment:1" align="right">Comment 1</div>

Current plan: Create fullypackedloop.py and class. Making underlying object the corresponding six vertex model. Add methods for ascii/graphical representation by modifying corresponding code for six vertex model. Add method in alternatingsignmatrix.py to map to fully packed loop.

For later: Add additional structure to FPLs so that the link pattern/non-crossing partition structure can be extracted.



---

archive/issue_comments_247025.json:
```json
{
    "body": "Author: **James Campbell, Vince Knight, Jessica Striker, Kevin Dilks, Emily Gunawan**",
    "created_at": "2015-03-19T00:12:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247025",
    "user": "https://github.com/theref"
}
```

Author: **James Campbell, Vince Knight, Jessica Striker, Kevin Dilks, Emily Gunawan**



---

archive/issue_comments_247026.json:
```json
{
    "body": "Commit: **[1213313](https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3)**",
    "created_at": "2015-03-20T00:41:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247026",
    "user": "https://github.com/jessicapalencia"
}
```

Commit: **[1213313](https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3)**



---

archive/issue_comments_247027.json:
```json
{
    "body": "Branch: **[u/vinceknight/fpl](https://github.com/sagemath/sagetrac-mirror/tree/u/vinceknight/fpl)**",
    "created_at": "2015-03-20T00:41:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247027",
    "user": "https://github.com/jessicapalencia"
}
```

Branch: **[u/vinceknight/fpl](https://github.com/sagemath/sagetrac-mirror/tree/u/vinceknight/fpl)**



---

archive/issue_comments_247028.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8955b4a91121d9d6b81f8df0c321991250f239af\">8955b4a</a></td><td><code>Adding some more plots</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d76344d7243c881d5225834fc1ac6d833a76b9a4\">d76344d</a></td><td><code>creates tests for ascii</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8d343a8ce6b5be67f969bc62c196edb0d4bcf163\">8d343a8</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 17988</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d6c9546e69afc82c1dcf4dc0528fcd5c21dd8fc9\">d6c9546</a></td><td><code>Have global documentation with reference of many faces</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0af224d7c38dfe5cb4eacc6bf0fbb360f14fe2b0\">0af224d</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 19788</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9334f9e2c5c4dc6f33e1a725f9efb583a49b5a04\">9334f9e</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 19788</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6e1548156e9152391ab02080bbc3e9df66a03962\">6e15481</a></td><td><code>to_fully_packed_loop methods</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bdaeea350ed212ea2e1d6388b2ce433add8f0f72\">bdaeea3</a></td><td><code>Fixing merge conflict and a test</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8232ef78f4deace12866239d4aee512fedccc354\">8232ef7</a></td><td><code>Tweak to the global class docs</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3\">1213313</a></td><td><code>Changing reference</code></td></tr></table>\n",
    "created_at": "2015-03-20T00:41:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247028",
    "user": "https://github.com/jessicapalencia"
}
```

<div id="comment:3" align="right">Comment 3</div>

Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8955b4a91121d9d6b81f8df0c321991250f239af">8955b4a</a></td><td><code>Adding some more plots</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d76344d7243c881d5225834fc1ac6d833a76b9a4">d76344d</a></td><td><code>creates tests for ascii</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8d343a8ce6b5be67f969bc62c196edb0d4bcf163">8d343a8</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 17988</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d6c9546e69afc82c1dcf4dc0528fcd5c21dd8fc9">d6c9546</a></td><td><code>Have global documentation with reference of many faces</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0af224d7c38dfe5cb4eacc6bf0fbb360f14fe2b0">0af224d</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 19788</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9334f9e2c5c4dc6f33e1a725f9efb583a49b5a04">9334f9e</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 19788</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6e1548156e9152391ab02080bbc3e9df66a03962">6e15481</a></td><td><code>to_fully_packed_loop methods</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bdaeea350ed212ea2e1d6388b2ce433add8f0f72">bdaeea3</a></td><td><code>Fixing merge conflict and a test</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8232ef78f4deace12866239d4aee512fedccc354">8232ef7</a></td><td><code>Tweak to the global class docs</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3">1213313</a></td><td><code>Changing reference</code></td></tr></table>




---

archive/issue_events_238250.json:
```json
{
    "actor": "https://github.com/jessicapalencia",
    "created_at": "2015-03-20T00:41:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238250"
}
```



---

archive/issue_comments_247029.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nJust commenting here that this patch includes images in the docs.",
    "created_at": "2015-03-20T18:38:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247029",
    "user": "https://github.com/drvinceknight"
}
```

<div id="comment:4" align="right">Comment 4</div>

Just commenting here that this patch includes images in the docs.



---

archive/issue_comments_247030.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nTrying to reset the trac branch.",
    "created_at": "2015-03-20T20:13:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247030",
    "user": "https://github.com/drvinceknight"
}
```

<div id="comment:5" align="right">Comment 5</div>

Trying to reset the trac branch.



---

archive/issue_comments_247031.json:
```json
{
    "body": "Changed commit from **[1213313](https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3)** to none",
    "created_at": "2015-03-20T20:13:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247031",
    "user": "https://github.com/drvinceknight"
}
```

Changed commit from **[1213313](https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3)** to none



---

archive/issue_comments_247032.json:
```json
{
    "body": "Changed branch from **[u/vinceknight/fpl](https://github.com/sagemath/sagetrac-mirror/tree/u/vinceknight/fpl)** to none",
    "created_at": "2015-03-20T20:13:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247032",
    "user": "https://github.com/drvinceknight"
}
```

Changed branch from **[u/vinceknight/fpl](https://github.com/sagemath/sagetrac-mirror/tree/u/vinceknight/fpl)** to none



---

archive/issue_comments_247033.json:
```json
{
    "body": "Commit: **[1213313](https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3)**",
    "created_at": "2015-03-20T20:13:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247033",
    "user": "https://github.com/drvinceknight"
}
```

Commit: **[1213313](https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3)**



---

archive/issue_comments_247034.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8955b4a91121d9d6b81f8df0c321991250f239af\">8955b4a</a></td><td><code>Adding some more plots</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d76344d7243c881d5225834fc1ac6d833a76b9a4\">d76344d</a></td><td><code>creates tests for ascii</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8d343a8ce6b5be67f969bc62c196edb0d4bcf163\">8d343a8</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 17988</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d6c9546e69afc82c1dcf4dc0528fcd5c21dd8fc9\">d6c9546</a></td><td><code>Have global documentation with reference of many faces</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0af224d7c38dfe5cb4eacc6bf0fbb360f14fe2b0\">0af224d</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 19788</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9334f9e2c5c4dc6f33e1a725f9efb583a49b5a04\">9334f9e</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 19788</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6e1548156e9152391ab02080bbc3e9df66a03962\">6e15481</a></td><td><code>to_fully_packed_loop methods</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bdaeea350ed212ea2e1d6388b2ce433add8f0f72\">bdaeea3</a></td><td><code>Fixing merge conflict and a test</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8232ef78f4deace12866239d4aee512fedccc354\">8232ef7</a></td><td><code>Tweak to the global class docs</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3\">1213313</a></td><td><code>Changing reference</code></td></tr></table>\n",
    "created_at": "2015-03-20T20:13:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247034",
    "user": "https://github.com/drvinceknight"
}
```

<div id="comment:7" align="right">Comment 7</div>

Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8955b4a91121d9d6b81f8df0c321991250f239af">8955b4a</a></td><td><code>Adding some more plots</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d76344d7243c881d5225834fc1ac6d833a76b9a4">d76344d</a></td><td><code>creates tests for ascii</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8d343a8ce6b5be67f969bc62c196edb0d4bcf163">8d343a8</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 17988</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d6c9546e69afc82c1dcf4dc0528fcd5c21dd8fc9">d6c9546</a></td><td><code>Have global documentation with reference of many faces</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0af224d7c38dfe5cb4eacc6bf0fbb360f14fe2b0">0af224d</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 19788</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9334f9e2c5c4dc6f33e1a725f9efb583a49b5a04">9334f9e</a></td><td><code>Merge branch 'u/jcampbell/fpl' of git://trac.sagemath.org/sage into 19788</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6e1548156e9152391ab02080bbc3e9df66a03962">6e15481</a></td><td><code>to_fully_packed_loop methods</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bdaeea350ed212ea2e1d6388b2ce433add8f0f72">bdaeea3</a></td><td><code>Fixing merge conflict and a test</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8232ef78f4deace12866239d4aee512fedccc354">8232ef7</a></td><td><code>Tweak to the global class docs</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3">1213313</a></td><td><code>Changing reference</code></td></tr></table>




---

archive/issue_comments_247035.json:
```json
{
    "body": "Branch: **[u/vinceknight/fpl](https://github.com/sagemath/sagetrac-mirror/tree/u/vinceknight/fpl)**",
    "created_at": "2015-03-20T20:13:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247035",
    "user": "https://github.com/drvinceknight"
}
```

Branch: **[u/vinceknight/fpl](https://github.com/sagemath/sagetrac-mirror/tree/u/vinceknight/fpl)**



---

archive/issue_comments_247036.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e79daf953d9b7ed83725e879ea54d7c74d7a0810\">e79daf9</a></td><td><code>Fixing merge conflict after #17977 got merged in to develop</code></td></tr></table>\n",
    "created_at": "2015-03-20T20:26:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247036",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e79daf953d9b7ed83725e879ea54d7c74d7a0810">e79daf9</a></td><td><code>Fixing merge conflict after #17977 got merged in to develop</code></td></tr></table>




---

archive/issue_comments_247037.json:
```json
{
    "body": "Changed commit from **[1213313](https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3)** to **[e79daf9](https://github.com/sagemath/sagetrac-mirror/commit/e79daf953d9b7ed83725e879ea54d7c74d7a0810)**",
    "created_at": "2015-03-20T20:26:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247037",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1213313](https://github.com/sagemath/sagetrac-mirror/commit/12133131a63c04f65728bc1b6ab95d6afb2152e3)** to **[e79daf9](https://github.com/sagemath/sagetrac-mirror/commit/e79daf953d9b7ed83725e879ea54d7c74d7a0810)**



---

archive/issue_comments_247038.json:
```json
{
    "body": "Changed commit from **[e79daf9](https://github.com/sagemath/sagetrac-mirror/commit/e79daf953d9b7ed83725e879ea54d7c74d7a0810)** to **[7e38bd9](https://github.com/sagemath/sagetrac-mirror/commit/7e38bd9c39c66532d63a9e087533979dbb596277)**",
    "created_at": "2015-03-20T21:27:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247038",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e79daf9](https://github.com/sagemath/sagetrac-mirror/commit/e79daf953d9b7ed83725e879ea54d7c74d7a0810)** to **[7e38bd9](https://github.com/sagemath/sagetrac-mirror/commit/7e38bd9c39c66532d63a9e087533979dbb596277)**



---

archive/issue_comments_247039.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7e38bd9c39c66532d63a9e087533979dbb596277\">7e38bd9</a></td><td><code>Fixing merge conflict and a minor relic in init doc</code></td></tr></table>\n",
    "created_at": "2015-03-20T21:27:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247039",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7e38bd9c39c66532d63a9e087533979dbb596277">7e38bd9</a></td><td><code>Fixing merge conflict and a minor relic in init doc</code></td></tr></table>




---

archive/issue_comments_247040.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nSince I was only involved in putting the method to_fully_packed_loop, I can be one of the reviewers.\nI think we should put the function `__eq__` so that we can test equality using \"==\". For example ...\n\nsage: m=AlternatingSignMatrix([[1,0,0],[0,1,0],[0,0,1]])\nsage: m.to_fully_packed_loop() == FullyPackedLoop(m)\nFalse\nsage: FullyPackedLoop(m) == FullyPackedLoop(m)\nFalse",
    "created_at": "2015-03-22T21:32:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247040",
    "user": "https://github.com/egunawan"
}
```

<div id="comment:10" align="right">Comment 10</div>

Since I was only involved in putting the method to_fully_packed_loop, I can be one of the reviewers.
I think we should put the function `__eq__` so that we can test equality using "==". For example ...

sage: m=AlternatingSignMatrix([[1,0,0],[0,1,0],[0,0,1]])
sage: m.to_fully_packed_loop() == FullyPackedLoop(m)
False
sage: FullyPackedLoop(m) == FullyPackedLoop(m)
False



---

archive/issue_events_238251.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2015-03-24T04:33:20Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "title_is": "Implement fully-packed loops (duplicate of 18003)",
    "title_was": "Implement fully-packed loops",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238251"
}
```



---

archive/issue_comments_247041.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+This is a duplicate of Ticket 18003.\n+\n Implement fully-packed loops, based on the six vertex model already implemented.\n+\n+Previously on branch u/vinceknight/fpl\n``````\n",
    "created_at": "2015-03-24T04:33:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247041",
    "user": "https://github.com/egunawan"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+This is a duplicate of Ticket 18003.
+
 Implement fully-packed loops, based on the six vertex model already implemented.
+
+Previously on branch u/vinceknight/fpl
``````




---

archive/issue_comments_247042.json:
```json
{
    "body": "Changed commit from **[7e38bd9](https://github.com/sagemath/sagetrac-mirror/commit/7e38bd9c39c66532d63a9e087533979dbb596277)** to none",
    "created_at": "2015-03-24T04:33:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247042",
    "user": "https://github.com/egunawan"
}
```

Changed commit from **[7e38bd9](https://github.com/sagemath/sagetrac-mirror/commit/7e38bd9c39c66532d63a9e087533979dbb596277)** to none



---

archive/issue_comments_247043.json:
```json
{
    "body": "Changed branch from **[u/vinceknight/fpl](https://github.com/sagemath/sagetrac-mirror/tree/u/vinceknight/fpl)** to none",
    "created_at": "2015-03-24T04:33:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247043",
    "user": "https://github.com/egunawan"
}
```

Changed branch from **[u/vinceknight/fpl](https://github.com/sagemath/sagetrac-mirror/tree/u/vinceknight/fpl)** to none



---

archive/issue_events_238252.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2015-03-24T04:33:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238252"
}
```



---

archive/issue_events_238253.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2015-03-24T04:33:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238253"
}
```



---

archive/issue_events_238254.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2015-03-24T04:33:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238254"
}
```



---

archive/issue_events_238255.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-03-24T04:58:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238255"
}
```



---

archive/issue_events_238256.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-03-24T04:58:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238256"
}
```



---

archive/issue_comments_247044.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nAgreed; duplicate of #18003.",
    "created_at": "2015-03-24T04:59:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247044",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:13" align="right">Comment 13</div>

Agreed; duplicate of #18003.



---

archive/issue_comments_247045.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2015-03-24T04:59:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247045",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_247046.json:
```json
{
    "body": "Changed author from **James Campbell, Vince Knight, Jessica Striker, Kevin Dilks, Emily Gunawan** to none",
    "created_at": "2015-03-24T04:59:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17988#issuecomment-247046",
    "user": "https://github.com/tscrim"
}
```

Changed author from **James Campbell, Vince Knight, Jessica Striker, Kevin Dilks, Emily Gunawan** to none



---

archive/issue_events_238257.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-24T10:49:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238257"
}
```



---

archive/issue_events_238258.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-24T10:49:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238258"
}
```



---

archive/issue_events_238259.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-24T10:49:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17988",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17988#event-238259"
}
```
