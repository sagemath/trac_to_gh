# Issue 17774: Random failure building Singular

archive/issues_017537.json:
```json
{
    "assignees": [],
    "body": "There is still a race in the Singular build system, sometimes (rarely) it fails with\n\n```\n[...]\ng++ -O2 -g  -fPIC -I.. -I/mnt/disk/home/release/Sage/local -pipe -fno-implicit-templates -I.. -I/mnt/disk/home/release/Sage/local -I. -I.. -I/mnt/disk/home/release/Sage/local -I/mnt/disk/home/release/Sage/local/include -I/mnt/disk/home/release/Sage/local/include -I/mnt/disk/home/release/Sage/local/include   -I/mnt/disk/home/release/Sage/local/include -DNDEBUG -DOM_NDEBUG -Dx86_64_Linux -DHAVE_CONFIG_H -c s_buff.cc\nattrib.cc: In function 'BOOLEAN atATTRIB2(leftv, leftv, leftv)':\nattrib.cc:305:55: warning: cast to pointer from integer of different size [-Wint-to-pointer-cast]\n     res->data=(void *)(rField_is_Ring((ring)v->Data()));\n                                                       ^\ng++ -O2 -g  -fPIC -I.. -I/mnt/disk/home/release/Sage/local -pipe -fno-implicit-templates -I.. -I/mnt/disk/home/release/Sage/local -I. -I.. -I/mnt/disk/home/release/Sage/local -I/mnt/disk/home/release/Sage/local/include -I/mnt/disk/home/release/Sage/local/include -I/mnt/disk/home/release/Sage/local/include   -I/mnt/disk/home/release/Sage/local/include -DNDEBUG -DOM_NDEBUG -Dx86_64_Linux -DHAVE_CONFIG_H -c subexpr.cc\nIn file included from silink.cc:19:0:\n../kernel/febase.h:284:31: error: 'FALSE' was not declared in this scope\n```\nThis is on my 6-core (12 threads) Haswell-E i7 desktop.\n\nKeywords: **random_fail**\n\nBranch: **[be29508](https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17774_\n\n",
    "closed_at": "2015-03-19T23:39:47Z",
    "created_at": "2015-02-12T23:30:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Random failure building Singular",
    "type": "issue",
    "updated_at": "2015-12-13T12:55:02Z",
    "url": "https://github.com/sagemath/sage/issues/17774",
    "user": "https://github.com/vbraun"
}
```
There is still a race in the Singular build system, sometimes (rarely) it fails with

```
[...]
g++ -O2 -g  -fPIC -I.. -I/mnt/disk/home/release/Sage/local -pipe -fno-implicit-templates -I.. -I/mnt/disk/home/release/Sage/local -I. -I.. -I/mnt/disk/home/release/Sage/local -I/mnt/disk/home/release/Sage/local/include -I/mnt/disk/home/release/Sage/local/include -I/mnt/disk/home/release/Sage/local/include   -I/mnt/disk/home/release/Sage/local/include -DNDEBUG -DOM_NDEBUG -Dx86_64_Linux -DHAVE_CONFIG_H -c s_buff.cc
attrib.cc: In function 'BOOLEAN atATTRIB2(leftv, leftv, leftv)':
attrib.cc:305:55: warning: cast to pointer from integer of different size [-Wint-to-pointer-cast]
     res->data=(void *)(rField_is_Ring((ring)v->Data()));
                                                       ^
g++ -O2 -g  -fPIC -I.. -I/mnt/disk/home/release/Sage/local -pipe -fno-implicit-templates -I.. -I/mnt/disk/home/release/Sage/local -I. -I.. -I/mnt/disk/home/release/Sage/local -I/mnt/disk/home/release/Sage/local/include -I/mnt/disk/home/release/Sage/local/include -I/mnt/disk/home/release/Sage/local/include   -I/mnt/disk/home/release/Sage/local/include -DNDEBUG -DOM_NDEBUG -Dx86_64_Linux -DHAVE_CONFIG_H -c subexpr.cc
In file included from silink.cc:19:0:
../kernel/febase.h:284:31: error: 'FALSE' was not declared in this scope
```
This is on my 6-core (12 threads) Haswell-E i7 desktop.

Keywords: **random_fail**

Branch: **[be29508](https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/17774_





---

archive/issue_events_235433.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-12T23:30:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235433"
}
```



---

archive/issue_events_235434.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-12T23:30:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235434"
}
```



---

archive/issue_events_235435.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-12T23:30:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235435"
}
```



---

archive/issue_events_235436.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-12T23:30:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235436"
}
```



---

archive/issue_comments_242569.json:
```json
{
    "body": "Branch: **[u/jdemeyer/random_failure_building_singular](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/random_failure_building_singular)**",
    "created_at": "2015-03-19T16:19:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17774#issuecomment-242569",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/random_failure_building_singular](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/random_failure_building_singular)**



---

archive/issue_comments_242570.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6\">be29508</a></td><td><code>Build Singular serially</code></td></tr></table>\n",
    "created_at": "2015-03-19T16:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17774#issuecomment-242570",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6">be29508</a></td><td><code>Build Singular serially</code></td></tr></table>




---

archive/issue_events_235437.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-03-19T16:28:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235437"
}
```



---

archive/issue_events_235438.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-03-19T16:28:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235438"
}
```



---

archive/issue_events_235439.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-03-19T16:28:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235439"
}
```



---

archive/issue_comments_242571.json:
```json
{
    "body": "Commit: **[be29508](https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6)**",
    "created_at": "2015-03-19T16:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17774#issuecomment-242571",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[be29508](https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6)**



---

archive/issue_comments_242572.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2015-03-19T16:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17774#issuecomment-242572",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_242573.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-03-19T16:54:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17774#issuecomment-242573",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_235440.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-19T16:54:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235440"
}
```



---

archive/issue_events_235441.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-19T16:54:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235441"
}
```



---

archive/issue_events_235442.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-19T23:39:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235442"
}
```



---

archive/issue_events_235443.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2f05bffee8bdd5980ea901a267b1f853a2be9b00",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-03-19T23:39:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17774#event-235443"
}
```



---

archive/issue_comments_242574.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/random_failure_building_singular](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/random_failure_building_singular)** to **[be29508](https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6)**",
    "created_at": "2015-03-19T23:39:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17774#issuecomment-242574",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/random_failure_building_singular](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/random_failure_building_singular)** to **[be29508](https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6)**



---

archive/issue_comments_242575.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nAnd nobody reported this upstream?!?!1?\n\n(We spent weeks fixing race conditions in Singular's build system not that long ago to let it build in parallel for good reason, now apparently all in vain.)",
    "created_at": "2015-12-13T12:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17774#issuecomment-242575",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:5" align="right">Comment 5</div>

And nobody reported this upstream?!?!1?

(We spent weeks fixing race conditions in Singular's build system not that long ago to let it build in parallel for good reason, now apparently all in vain.)



---

archive/issue_comments_242576.json:
```json
{
    "body": "Changed commit from **[be29508](https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6)** to none",
    "created_at": "2015-12-13T12:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17774#issuecomment-242576",
    "user": "https://github.com/nexttime"
}
```

Changed commit from **[be29508](https://github.com/sagemath/sagetrac-mirror/commit/be295080c79acda411ddec33cf529d2d841d3ce6)** to none
