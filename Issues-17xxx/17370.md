# Issue 17370: Setting .copy(immutable=False) everywhere

archive/issues_017133.json:
```json
{
    "assignees": [],
    "body": "After many patches which only fixed a bad call to copy() there and there, here is a patch to fix them all. Err, all that need to be fixed: I went over all of them and added this when needed.\n\nThis branch also fixes a small problem reported today by a friend of mine, i.e. an exception when trying to plot with javascript a graph whose layout is defined with \"complicated\" objects like rationals.\n\n```\ngraphs.CompleteBipartiteGraph(4,5).show(method=\"js\")\n```\n\nCC:  @sagetrac-Simon @tscrim @jm58660\n\nBranch/Commit: **[64f5c3d](https://github.com/sagemath/sagetrac-mirror/commit/64f5c3d90b0f473044a77b6b438f542eed7a4ef5)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17370_\n\n",
    "closed_at": "2015-01-02T15:46:19Z",
    "created_at": "2014-11-20T13:36:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Setting .copy(immutable=False) everywhere",
    "type": "issue",
    "updated_at": "2015-01-02T15:46:19Z",
    "url": "https://github.com/sagemath/sage/issues/17370",
    "user": "https://github.com/nathanncohen"
}
```
After many patches which only fixed a bad call to copy() there and there, here is a patch to fix them all. Err, all that need to be fixed: I went over all of them and added this when needed.

This branch also fixes a small problem reported today by a friend of mine, i.e. an exception when trying to plot with javascript a graph whose layout is defined with "complicated" objects like rationals.

```
graphs.CompleteBipartiteGraph(4,5).show(method="js")
```

CC:  @sagetrac-Simon @tscrim @jm58660

Branch/Commit: **[64f5c3d](https://github.com/sagemath/sagetrac-mirror/commit/64f5c3d90b0f473044a77b6b438f542eed7a4ef5)**

Reviewer: **Frédéric Chapoton**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/17370_





---

archive/issue_events_230044.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-11-20T13:36:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17370#event-230044"
}
```



---

archive/issue_events_230045.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-11-20T13:36:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17370#event-230045"
}
```



---

archive/issue_events_230046.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-11-20T13:36:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17370#event-230046"
}
```



---

archive/issue_events_230047.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-11-20T13:36:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17370#event-230047"
}
```



---

archive/issue_events_230048.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-11-20T13:38:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17370#event-230048"
}
```



---

archive/issue_comments_234745.json:
```json
{
    "body": "Branch: **[u/ncohen/17370](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17370)**",
    "created_at": "2014-11-20T13:38:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17370#issuecomment-234745",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/17370](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17370)**



---

archive/issue_comments_234746.json:
```json
{
    "body": "Commit: **[64f5c3d](https://github.com/sagemath/sagetrac-mirror/commit/64f5c3d90b0f473044a77b6b438f542eed7a4ef5)**",
    "created_at": "2014-11-20T13:38:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17370#issuecomment-234746",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[64f5c3d](https://github.com/sagemath/sagetrac-mirror/commit/64f5c3d90b0f473044a77b6b438f542eed7a4ef5)**



---

archive/issue_comments_234747.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/64f5c3d90b0f473044a77b6b438f542eed7a4ef5\">64f5c3d</a></td><td><code>trac #17370: Setting .copy(immutable=False) everywhere</code></td></tr></table>\n",
    "created_at": "2014-11-20T13:38:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17370#issuecomment-234747",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/64f5c3d90b0f473044a77b6b438f542eed7a4ef5">64f5c3d</a></td><td><code>trac #17370: Setting .copy(immutable=False) everywhere</code></td></tr></table>




---

archive/issue_events_230049.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-12-26T20:49:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17370#event-230049"
}
```



---

archive/issue_events_230050.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-12-26T20:49:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17370#event-230050"
}
```



---

archive/issue_comments_234748.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nlooks good to me, as far as all tests pass",
    "created_at": "2014-12-26T20:49:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17370#issuecomment-234748",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

looks good to me, as far as all tests pass



---

archive/issue_comments_234749.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2014-12-26T20:49:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17370#issuecomment-234749",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_234750.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nTHaaaaaaaaaaanks !\n\nNathann",
    "created_at": "2014-12-27T02:20:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17370#issuecomment-234750",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

THaaaaaaaaaaanks !

Nathann



---

archive/issue_events_230051.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-02T15:46:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17370#event-230051"
}
```



---

archive/issue_events_230052.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d8b0d14ba2b961f88f3c660944b0ba09ecfe71c8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-02T15:46:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17370#event-230052"
}
```



---

archive/issue_comments_234751.json:
```json
{
    "body": "Changed branch from **[u/ncohen/17370](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17370)** to **[64f5c3d](https://github.com/sagemath/sagetrac-mirror/commit/64f5c3d90b0f473044a77b6b438f542eed7a4ef5)**",
    "created_at": "2015-01-02T15:46:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17370#issuecomment-234751",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/ncohen/17370](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/17370)** to **[64f5c3d](https://github.com/sagemath/sagetrac-mirror/commit/64f5c3d90b0f473044a77b6b438f542eed7a4ef5)**
