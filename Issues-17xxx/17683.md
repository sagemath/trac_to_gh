# Issue 17683: Subgraph of Hasse Diagram gives error

archive/issues_017446.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: Poset({0:[1]})._hasse_diagram.subgraph([0])\nNotImplementedError: An immutable graph does not change name\n```\n\nThis same error has already been corrected from several other places.\n\n(This will be needed for #17173, I guess.)\n\n\nCC:  @nathanncohen\n\nBranch/Commit: **[fa5734f](https://github.com/sagemath/sagetrac-mirror/commit/fa5734fd98af91de2692cdcaf1bf5ab88852601e)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17683_\n\n",
    "closed_at": "2015-01-29T13:25:59Z",
    "created_at": "2015-01-28T11:18:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Subgraph of Hasse Diagram gives error",
    "type": "issue",
    "updated_at": "2015-01-29T13:25:59Z",
    "url": "https://github.com/sagemath/sage/issues/17683",
    "user": "https://github.com/jm58660"
}
```

```
sage: Poset({0:[1]})._hasse_diagram.subgraph([0])
NotImplementedError: An immutable graph does not change name
```

This same error has already been corrected from several other places.

(This will be needed for #17173, I guess.)


CC:  @nathanncohen

Branch/Commit: **[fa5734f](https://github.com/sagemath/sagetrac-mirror/commit/fa5734fd98af91de2692cdcaf1bf5ab88852601e)**

Reviewer: **Travis Scrimshaw**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/17683_





---

archive/issue_events_234295.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-01-28T11:18:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17683#event-234295"
}
```



---

archive/issue_events_234296.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-01-28T11:18:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17683#event-234296"
}
```



---

archive/issue_events_234297.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-01-28T11:18:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17683#event-234297"
}
```



---

archive/issue_events_234298.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-01-28T11:18:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17683#event-234298"
}
```



---

archive/issue_comments_241214.json:
```json
{
    "body": "Branch: **[public/17683](https://github.com/sagemath/sagetrac-mirror/tree/public/17683)**",
    "created_at": "2015-01-28T12:30:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17683#issuecomment-241214",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[public/17683](https://github.com/sagemath/sagetrac-mirror/tree/public/17683)**



---

archive/issue_comments_241215.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fa5734fd98af91de2692cdcaf1bf5ab88852601e\">fa5734f</a></td><td><code>trac #17683: Subgraph of Hasse Diagram gives error</code></td></tr></table>\n",
    "created_at": "2015-01-28T12:30:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17683#issuecomment-241215",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fa5734fd98af91de2692cdcaf1bf5ab88852601e">fa5734f</a></td><td><code>trac #17683: Subgraph of Hasse Diagram gives error</code></td></tr></table>




---

archive/issue_comments_241216.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2015-01-28T12:30:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17683#issuecomment-241216",
    "user": "https://github.com/nathanncohen"
}
```

Author: **Nathann Cohen**



---

archive/issue_events_234299.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-01-28T12:30:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17683#event-234299"
}
```



---

archive/issue_comments_241217.json:
```json
{
    "body": "Commit: **[fa5734f](https://github.com/sagemath/sagetrac-mirror/commit/fa5734fd98af91de2692cdcaf1bf5ab88852601e)**",
    "created_at": "2015-01-28T12:30:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17683#issuecomment-241217",
    "user": "https://github.com/nathanncohen"
}
```

Commit: **[fa5734f](https://github.com/sagemath/sagetrac-mirror/commit/fa5734fd98af91de2692cdcaf1bf5ab88852601e)**



---

archive/issue_comments_241218.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2015-01-28T16:18:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17683#issuecomment-241218",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_241219.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2015-01-28T16:18:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17683#issuecomment-241219",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_events_234300.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-01-28T16:18:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17683#event-234300"
}
```



---

archive/issue_events_234301.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-01-28T16:18:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17683#event-234301"
}
```



---

archive/issue_comments_241220.json:
```json
{
    "body": "Changed branch from **[public/17683](https://github.com/sagemath/sagetrac-mirror/tree/public/17683)** to **[fa5734f](https://github.com/sagemath/sagetrac-mirror/commit/fa5734fd98af91de2692cdcaf1bf5ab88852601e)**",
    "created_at": "2015-01-29T13:25:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17683#issuecomment-241220",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/17683](https://github.com/sagemath/sagetrac-mirror/tree/public/17683)** to **[fa5734f](https://github.com/sagemath/sagetrac-mirror/commit/fa5734fd98af91de2692cdcaf1bf5ab88852601e)**



---

archive/issue_events_234302.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-29T13:25:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17683#event-234302"
}
```



---

archive/issue_events_234303.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "711cdcdf9bb26e142b9d2801e4753d7badda4528",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-29T13:25:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17683",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17683#event-234303"
}
```
