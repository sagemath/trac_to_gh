# Issue 17333: Update zeromq to 4.0.5

archive/issues_017096.json:
```json
{
    "assignees": [],
    "body": "The update itself is trivial.\nUpstream tarball at:\n* http://download.zeromq.org/zeromq-4.0.5.tar.gz\n\nThis also fixes a C++ linking issue:\n* https://github.com/zeromq/pyzmq/issues/113\n* https://github.com/zeromq/libzmq/issues/1236\nand ensure that -no-undefined is passed to libtool to build a shared lib (or later on pyzmq will fail to build...) on Cygwin.\n\nCC:  @tscrim @vbraun\n\nBranch: **[8fe27c2](https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1)**\n\nUpstream: **Fixed upstream, in a later stable release.**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17333_\n\n",
    "closed_at": "2014-11-25T11:25:54Z",
    "created_at": "2014-11-13T14:01:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update zeromq to 4.0.5",
    "type": "issue",
    "updated_at": "2016-11-30T14:35:23Z",
    "url": "https://github.com/sagemath/sage/issues/17333",
    "user": "https://github.com/sagetrac-jpflori"
}
```
The update itself is trivial.
Upstream tarball at:
* http://download.zeromq.org/zeromq-4.0.5.tar.gz

This also fixes a C++ linking issue:
* https://github.com/zeromq/pyzmq/issues/113
* https://github.com/zeromq/libzmq/issues/1236
and ensure that -no-undefined is passed to libtool to build a shared lib (or later on pyzmq will fail to build...) on Cygwin.

CC:  @tscrim @vbraun

Branch: **[8fe27c2](https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1)**

Upstream: **Fixed upstream, in a later stable release.**

Reviewer: **Volker Braun**

Author: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/17333_





---

archive/issue_events_229647.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-11-13T14:01:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229647"
}
```



---

archive/issue_events_229648.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-11-13T14:01:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229648"
}
```



---

archive/issue_events_229649.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-11-13T14:01:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229649"
}
```



---

archive/issue_events_229650.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-11-13T14:01:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229650"
}
```



---

archive/issue_comments_234179.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,4 @@\n Some C++ linking issue.\n See:\n * https://github.com/zeromq/pyzmq/issues/113\n+* https://github.com/zeromq/libzmq/issues/1236\n``````\n",
    "created_at": "2014-11-13T14:11:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234179",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,4 @@
 Some C++ linking issue.
 See:
 * https://github.com/zeromq/pyzmq/issues/113
+* https://github.com/zeromq/libzmq/issues/1236
``````




---

archive/issue_comments_234180.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,3 +2,5 @@\n See:\n * https://github.com/zeromq/pyzmq/issues/113\n * https://github.com/zeromq/libzmq/issues/1236\n+\n+We also need to ensure -no-undefined is passed to libtool to build a shared lib (or later on pyzmq will fail to build...)\n``````\n",
    "created_at": "2014-11-13T15:02:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234180",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,3 +2,5 @@
 See:
 * https://github.com/zeromq/pyzmq/issues/113
 * https://github.com/zeromq/libzmq/issues/1236
+
+We also need to ensure -no-undefined is passed to libtool to build a shared lib (or later on pyzmq will fail to build...)
``````




---

archive/issue_comments_234181.json:
```json
{
    "body": "Commit: **[8fe27c2](https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1)**",
    "created_at": "2014-11-24T15:44:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234181",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Commit: **[8fe27c2](https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1)**



---

archive/issue_comments_234182.json:
```json
{
    "body": "Author: **Jean-Pierre Flori**",
    "created_at": "2014-11-24T15:44:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234182",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Author: **Jean-Pierre Flori**



---

archive/issue_comments_234183.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1\">8fe27c2</a></td><td><code>Update zeromq to 4.0.5 and let it compile on Cygwin.</code></td></tr></table>\n",
    "created_at": "2014-11-24T15:44:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234183",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1">8fe27c2</a></td><td><code>Update zeromq to 4.0.5 and let it compile on Cygwin.</code></td></tr></table>




---

archive/issue_comments_234184.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n-Some C++ linking issue.\n-See:\n+The update itself is trivial.\n+\n+This also fixes a C++ linking issue:\n * https://github.com/zeromq/pyzmq/issues/113\n * https://github.com/zeromq/libzmq/issues/1236\n-\n-We also need to ensure -no-undefined is passed to libtool to build a shared lib (or later on pyzmq will fail to build...)\n+and ensure that -no-undefined is passed to libtool to build a shared lib (or later on pyzmq will fail to build...) on Cygwin.\n``````\n",
    "created_at": "2014-11-24T15:44:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234184",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
-Some C++ linking issue.
-See:
+The update itself is trivial.
+
+This also fixes a C++ linking issue:
 * https://github.com/zeromq/pyzmq/issues/113
 * https://github.com/zeromq/libzmq/issues/1236
-
-We also need to ensure -no-undefined is passed to libtool to build a shared lib (or later on pyzmq will fail to build...)
+and ensure that -no-undefined is passed to libtool to build a shared lib (or later on pyzmq will fail to build...) on Cygwin.
``````




---

archive/issue_events_229651.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-11-24T15:44:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229651"
}
```



---

archive/issue_comments_234185.json:
```json
{
    "body": "Branch: **[u/jpflori/ticket/17333](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/17333)**",
    "created_at": "2014-11-24T15:44:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234185",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Branch: **[u/jpflori/ticket/17333](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/17333)**



---

archive/issue_events_229652.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-11-24T15:44:23Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "title_is": "Update zeromq to 4.0.5",
    "title_was": "Zeromq fails to build under Cygwin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229652"
}
```



---

archive/issue_comments_234186.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,6 @@\n The update itself is trivial.\n+Upstream tarball at:\n+* http://download.zeromq.org/zeromq-4.0.5.tar.gz\n \n This also fixes a C++ linking issue:\n * https://github.com/zeromq/pyzmq/issues/113\n``````\n",
    "created_at": "2014-11-24T15:45:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234186",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,6 @@
 The update itself is trivial.
+Upstream tarball at:
+* http://download.zeromq.org/zeromq-4.0.5.tar.gz
 
 This also fixes a C++ linking issue:
 * https://github.com/zeromq/pyzmq/issues/113
``````




---

archive/issue_comments_234187.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nlgtm",
    "created_at": "2014-11-24T15:58:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234187",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:5'>Comment 5:</a>
lgtm



---

archive/issue_events_229653.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-24T15:58:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229653"
}
```



---

archive/issue_events_229654.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-24T15:58:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229654"
}
```



---

archive/issue_comments_234188.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-11-24T18:38:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234188",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_234189.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nFYI, I reported the `-no-undefined` thing upstream as https://github.com/zeromq/libzmq/issues/1263.",
    "created_at": "2014-11-24T23:28:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234189",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:7'>Comment 7:</a>
FYI, I reported the `-no-undefined` thing upstream as https://github.com/zeromq/libzmq/issues/1263.



---

archive/issue_comments_234190.json:
```json
{
    "body": "Changed branch from **[u/jpflori/ticket/17333](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/17333)** to **[8fe27c2](https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1)**",
    "created_at": "2014-11-25T11:25:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234190",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jpflori/ticket/17333](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/17333)** to **[8fe27c2](https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1)**



---

archive/issue_events_229655.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-25T11:25:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229655"
}
```



---

archive/issue_events_229656.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f1ae029d18d5ded509461f9b092e3f48ad0d5e64",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-11-25T11:25:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17333#event-229656"
}
```



---

archive/issue_comments_234191.json:
```json
{
    "body": "Changed commit from **[8fe27c2](https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1)** to none",
    "created_at": "2016-11-30T13:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234191",
    "user": "https://github.com/embray"
}
```

Changed commit from **[8fe27c2](https://github.com/sagemath/sagetrac-mirror/commit/8fe27c26d82e8fe7e467f59aa91783670d07f8b1)** to none



---

archive/issue_comments_234192.json:
```json
{
    "body": "Upstream: **Reported upstream. No feedback yet.**",
    "created_at": "2016-11-30T13:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234192",
    "user": "https://github.com/embray"
}
```

Upstream: **Reported upstream. No feedback yet.**



---

archive/issue_comments_234193.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nNevermind, I think I misunderstood the status of this.  Yes, the upstream issue was already fixed in https://github.com/zeromq/libzmq/pull/1283",
    "created_at": "2016-11-30T14:35:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234193",
    "user": "https://github.com/embray"
}
```

<a id='comment:10'>Comment 10:</a>
Nevermind, I think I misunderstood the status of this.  Yes, the upstream issue was already fixed in https://github.com/zeromq/libzmq/pull/1283



---

archive/issue_comments_234194.json:
```json
{
    "body": "Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**",
    "created_at": "2016-11-30T14:35:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17333",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17333#issuecomment-234194",
    "user": "https://github.com/embray"
}
```

Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, in a later stable release.**
