# Issue 17457: Clean and speedup for permanent

archive/issues_017220.json:
```json
{
    "assignees": [],
    "body": "In view of ticket #16603, I thought it would be nice to clean up a bit the functions related to computation of the permanent in `matrix1.pyx` and `matrix2.pyx`. It ends up with a small speed up (small compared to what #16603 provides).\n\nDepends on #16603\n\nCC:  @jaapspies @nathanncohen pernici\n\nBranch/Commit: **[u/vdelecroix/17457](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17457) @ [a20c391](https://github.com/sagemath/sagetrac-mirror/commit/a20c391a17c86391b762a9b6ac01c52ff7b8ff6c)**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17457_\n\n",
    "created_at": "2014-12-07T18:47:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean and speedup for permanent",
    "type": "issue",
    "updated_at": "2022-12-29T01:30:11Z",
    "url": "https://github.com/sagemath/sage/issues/17457",
    "user": "https://github.com/videlec"
}
```
In view of ticket #16603, I thought it would be nice to clean up a bit the functions related to computation of the permanent in `matrix1.pyx` and `matrix2.pyx`. It ends up with a small speed up (small compared to what #16603 provides).

Depends on #16603

CC:  @jaapspies @nathanncohen pernici

Branch/Commit: **[u/vdelecroix/17457](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17457) @ [a20c391](https://github.com/sagemath/sagetrac-mirror/commit/a20c391a17c86391b762a9b6ac01c52ff7b8ff6c)**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/17457_





---

archive/issue_events_231168.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-12-07T18:47:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17457#event-231168"
}
```



---

archive/issue_events_231169.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-12-07T18:47:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17457#event-231169"
}
```



---

archive/issue_events_231170.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-12-07T18:47:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17457#event-231170"
}
```



---

archive/issue_events_231171.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-12-07T18:47:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17457#event-231171"
}
```



---

archive/issue_comments_236198.json:
```json
{
    "body": "Commit: **[01f783e](https://github.com/sagemath/sagetrac-mirror/commit/01f783e346aa8707f7f3f1d36e9def546da74b8b)**",
    "created_at": "2014-12-07T18:51:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17457#issuecomment-236198",
    "user": "https://github.com/videlec"
}
```

Commit: **[01f783e](https://github.com/sagemath/sagetrac-mirror/commit/01f783e346aa8707f7f3f1d36e9def546da74b8b)**



---

archive/issue_comments_236199.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8013ac0ec55eebf4da64f09c011e90f0e25a145c\">8013ac0</a></td><td><code>trac #17452: clean  min_wt_vec_gap</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/173f3ece741427bbc40f3f91305e0a14391ea7fb\">173f3ec</a></td><td><code>trac #17452: add a check for rank (+ doc)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/01f783e346aa8707f7f3f1d36e9def546da74b8b\">01f783e</a></td><td><code>trac #17457: speed up permanent and associated methods</code></td></tr></table>\n",
    "created_at": "2014-12-07T18:51:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17457#issuecomment-236199",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8013ac0ec55eebf4da64f09c011e90f0e25a145c">8013ac0</a></td><td><code>trac #17452: clean  min_wt_vec_gap</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/173f3ece741427bbc40f3f91305e0a14391ea7fb">173f3ec</a></td><td><code>trac #17452: add a check for rank (+ doc)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/01f783e346aa8707f7f3f1d36e9def546da74b8b">01f783e</a></td><td><code>trac #17457: speed up permanent and associated methods</code></td></tr></table>




---

archive/issue_events_231172.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-12-07T18:51:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17457#event-231172"
}
```



---

archive/issue_comments_236200.json:
```json
{
    "body": "Branch: **[u/vdelecroix/17457](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17457)**",
    "created_at": "2014-12-07T18:51:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17457#issuecomment-236200",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/17457](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/17457)**



---

archive/issue_comments_236201.json:
```json
{
    "body": "Changed commit from **[01f783e](https://github.com/sagemath/sagetrac-mirror/commit/01f783e346aa8707f7f3f1d36e9def546da74b8b)** to none",
    "created_at": "2014-12-07T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17457#issuecomment-236201",
    "user": "https://github.com/videlec"
}
```

Changed commit from **[01f783e](https://github.com/sagemath/sagetrac-mirror/commit/01f783e346aa8707f7f3f1d36e9def546da74b8b)** to none



---

archive/issue_comments_236202.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nsorry... wrong branch!",
    "created_at": "2014-12-07T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17457#issuecomment-236202",
    "user": "https://github.com/videlec"
}
```

<a id='comment:2'>Comment 2:</a>
sorry... wrong branch!



---

archive/issue_comments_236203.json:
```json
{
    "body": "Commit: **[a20c391](https://github.com/sagemath/sagetrac-mirror/commit/a20c391a17c86391b762a9b6ac01c52ff7b8ff6c)**",
    "created_at": "2014-12-07T18:52:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17457#issuecomment-236203",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[a20c391](https://github.com/sagemath/sagetrac-mirror/commit/a20c391a17c86391b762a9b6ac01c52ff7b8ff6c)**



---

archive/issue_comments_236204.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a20c391a17c86391b762a9b6ac01c52ff7b8ff6c\">a20c391</a></td><td><code>trac #17457: speed up permanent and associated methods</code></td></tr></table>\n",
    "created_at": "2014-12-07T18:52:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17457#issuecomment-236204",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a20c391a17c86391b762a9b6ac01c52ff7b8ff6c">a20c391</a></td><td><code>trac #17457: speed up permanent and associated methods</code></td></tr></table>




---

archive/issue_events_231173.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-12-09T15:53:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17457#event-231173"
}
```



---

archive/issue_events_231174.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-12-09T15:53:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17457#event-231174"
}
```



---

archive/issue_comments_236205.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nthere will be some non-trivial rebasing...",
    "created_at": "2014-12-09T15:53:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17457#issuecomment-236205",
    "user": "https://github.com/videlec"
}
```

<a id='comment:4'>Comment 4:</a>
there will be some non-trivial rebasing...



---

archive/issue_comments_236206.json:
```json
{
    "body": "Dependencies: **#16603**",
    "created_at": "2014-12-09T15:53:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17457#issuecomment-236206",
    "user": "https://github.com/videlec"
}
```

Dependencies: **#16603**



---

archive/issue_events_231175.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:30:11Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17457",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17457#event-231175"
}
```
