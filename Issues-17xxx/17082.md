# Issue 17082: Fix height_difference_bound to work over Number Fields

archive/issues_016845.json:
```json
{
    "assignees": [],
    "body": "After checking the math, saw that there is no reason to restrict to QQ or ZZ only, fixing checks at start to reflect this.\n\nBranch: **[0f9a1f3](https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e)**\n\nReviewer: **Ben Hutz**\n\nAuthor: **Joao Alberto de Faria**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17082_\n\n",
    "closed_at": "2015-01-24T23:51:45Z",
    "created_at": "2014-10-01T17:29:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix height_difference_bound to work over Number Fields",
    "type": "issue",
    "updated_at": "2015-02-23T16:25:03Z",
    "url": "https://github.com/sagemath/sage/issues/17082",
    "user": "https://github.com/sagetrac-jdefaria"
}
```
After checking the math, saw that there is no reason to restrict to QQ or ZZ only, fixing checks at start to reflect this.

Branch: **[0f9a1f3](https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e)**

Reviewer: **Ben Hutz**

Author: **Joao Alberto de Faria**

_Issue created by migration from https://trac.sagemath.org/ticket/17082_





---

archive/issue_events_226387.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2014-10-01T17:29:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226387"
}
```



---

archive/issue_events_226388.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2014-10-01T17:29:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226388"
}
```



---

archive/issue_events_226389.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2014-10-01T17:29:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226389"
}
```



---

archive/issue_events_226390.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2014-10-01T17:29:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226390"
}
```



---

archive/issue_comments_228818.json:
```json
{
    "body": "Branch: **[u/jdefaria/ticket/17082](https://github.com/sagemath/sagetrac-mirror/tree/u/jdefaria/ticket/17082)**",
    "created_at": "2014-10-16T01:34:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228818",
    "user": "https://github.com/sagetrac-jdefaria"
}
```

Branch: **[u/jdefaria/ticket/17082](https://github.com/sagemath/sagetrac-mirror/tree/u/jdefaria/ticket/17082)**



---

archive/issue_comments_228819.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a524288bad0774624c4823c37203341041117d65\">a524288</a></td><td><code>Adapted code to not have to check for numerator</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f64e556c31802d06134265b225cc3f6cb715bf28\">f64e556</a></td><td><code>Deleted lines used for testing</code></td></tr></table>\n",
    "created_at": "2014-11-12T18:14:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228819",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a524288bad0774624c4823c37203341041117d65">a524288</a></td><td><code>Adapted code to not have to check for numerator</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f64e556c31802d06134265b225cc3f6cb715bf28">f64e556</a></td><td><code>Deleted lines used for testing</code></td></tr></table>




---

archive/issue_comments_228820.json:
```json
{
    "body": "Commit: **[f64e556](https://github.com/sagemath/sagetrac-mirror/commit/f64e556c31802d06134265b225cc3f6cb715bf28)**",
    "created_at": "2014-11-12T18:14:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228820",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[f64e556](https://github.com/sagemath/sagetrac-mirror/commit/f64e556c31802d06134265b225cc3f6cb715bf28)**



---

archive/issue_events_226391.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2014-11-12T18:15:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226391"
}
```



---

archive/issue_comments_228821.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThis fails doc tests ('Not Implemented Error' spelled wrong) and needs a numberfield example",
    "created_at": "2014-11-13T19:04:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228821",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:4" align="right">Comment 4</div>

This fails doc tests ('Not Implemented Error' spelled wrong) and needs a numberfield example



---

archive/issue_events_226392.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-11-13T19:04:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226392"
}
```



---

archive/issue_events_226393.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-11-13T19:04:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226393"
}
```



---

archive/issue_comments_228822.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/98959fb6c4ae60350aeb171f3740483c8c56dc30\">98959fb</a></td><td><code>17082 - fixed error checks</code></td></tr></table>\n",
    "created_at": "2014-12-03T17:38:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228822",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/98959fb6c4ae60350aeb171f3740483c8c56dc30">98959fb</a></td><td><code>17082 - fixed error checks</code></td></tr></table>




---

archive/issue_comments_228823.json:
```json
{
    "body": "Changed commit from **[f64e556](https://github.com/sagemath/sagetrac-mirror/commit/f64e556c31802d06134265b225cc3f6cb715bf28)** to **[98959fb](https://github.com/sagemath/sagetrac-mirror/commit/98959fb6c4ae60350aeb171f3740483c8c56dc30)**",
    "created_at": "2014-12-03T17:38:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228823",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f64e556](https://github.com/sagemath/sagetrac-mirror/commit/f64e556c31802d06134265b225cc3f6cb715bf28)** to **[98959fb](https://github.com/sagemath/sagetrac-mirror/commit/98959fb6c4ae60350aeb171f3740483c8c56dc30)**



---

archive/issue_comments_228824.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/09349098e9a50e01761b46402198c40ba3becd3c\">0934909</a></td><td><code>17082- added num field example</code></td></tr></table>\n",
    "created_at": "2014-12-03T17:46:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228824",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/09349098e9a50e01761b46402198c40ba3becd3c">0934909</a></td><td><code>17082- added num field example</code></td></tr></table>




---

archive/issue_comments_228825.json:
```json
{
    "body": "Changed commit from **[98959fb](https://github.com/sagemath/sagetrac-mirror/commit/98959fb6c4ae60350aeb171f3740483c8c56dc30)** to **[0934909](https://github.com/sagemath/sagetrac-mirror/commit/09349098e9a50e01761b46402198c40ba3becd3c)**",
    "created_at": "2014-12-03T17:46:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228825",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[98959fb](https://github.com/sagemath/sagetrac-mirror/commit/98959fb6c4ae60350aeb171f3740483c8c56dc30)** to **[0934909](https://github.com/sagemath/sagetrac-mirror/commit/09349098e9a50e01761b46402198c40ba3becd3c)**



---

archive/issue_events_226394.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2014-12-03T17:47:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226394"
}
```



---

archive/issue_events_226395.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2014-12-03T17:47:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226395"
}
```



---

archive/issue_events_226396.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-15T21:19:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226396"
}
```



---

archive/issue_events_226397.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2014-12-15T21:19:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226397"
}
```



---

archive/issue_comments_228826.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nWell, a couple things here. This passes all the long tests, and the functionality change seems ok.  However, there are two things that should be fixed.\n\n- There is an indentation issue with the new example you added.\n\n- you seems to restrict to number fields or ZZ. If it is ZZ the code changes the ring to QQ so that .lift() works. Why can't you do this for any number field ring (change to its field of fractions)?",
    "created_at": "2014-12-15T21:19:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228826",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:8" align="right">Comment 8</div>

Well, a couple things here. This passes all the long tests, and the functionality change seems ok.  However, there are two things that should be fixed.

- There is an indentation issue with the new example you added.

- you seems to restrict to number fields or ZZ. If it is ZZ the code changes the ring to QQ so that .lift() works. Why can't you do this for any number field ring (change to its field of fractions)?



---

archive/issue_comments_228827.json:
```json
{
    "body": "Reviewer: **Ben Hutz**",
    "created_at": "2014-12-15T21:19:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228827",
    "user": "https://github.com/bhutz"
}
```

Reviewer: **Ben Hutz**



---

archive/issue_comments_228828.json:
```json
{
    "body": "Changed commit from **[0934909](https://github.com/sagemath/sagetrac-mirror/commit/09349098e9a50e01761b46402198c40ba3becd3c)** to **[4102df2](https://github.com/sagemath/sagetrac-mirror/commit/4102df29b08838365ab6719f0cb04b7ac43a5bc0)**",
    "created_at": "2015-01-15T20:46:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228828",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0934909](https://github.com/sagemath/sagetrac-mirror/commit/09349098e9a50e01761b46402198c40ba3becd3c)** to **[4102df2](https://github.com/sagemath/sagetrac-mirror/commit/4102df29b08838365ab6719f0cb04b7ac43a5bc0)**



---

archive/issue_comments_228829.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4102df29b08838365ab6719f0cb04b7ac43a5bc0\">4102df2</a></td><td><code>Changed from ZZ to number field rings</code></td></tr></table>\n",
    "created_at": "2015-01-15T20:46:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228829",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4102df29b08838365ab6719f0cb04b7ac43a5bc0">4102df2</a></td><td><code>Changed from ZZ to number field rings</code></td></tr></table>




---

archive/issue_events_226398.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2015-01-15T20:47:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226398"
}
```



---

archive/issue_events_226399.json:
```json
{
    "actor": "https://github.com/sagetrac-jdefaria",
    "created_at": "2015-01-15T20:47:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226399"
}
```



---

archive/issue_comments_228830.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nI opened the documentation, didn't see any problems there, everything else is fixed, needs review",
    "created_at": "2015-01-15T20:47:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228830",
    "user": "https://github.com/sagetrac-jdefaria"
}
```

<div id="comment:10" align="right">Comment 10</div>

I opened the documentation, didn't see any problems there, everything else is fixed, needs review



---

archive/issue_comments_228831.json:
```json
{
    "body": "Changed commit from **[4102df2](https://github.com/sagemath/sagetrac-mirror/commit/4102df29b08838365ab6719f0cb04b7ac43a5bc0)** to **[68978c2](https://github.com/sagemath/sagetrac-mirror/commit/68978c27de6990e4c9f8010a4447264c09aea871)**",
    "created_at": "2015-01-16T17:23:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228831",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4102df2](https://github.com/sagemath/sagetrac-mirror/commit/4102df29b08838365ab6719f0cb04b7ac43a5bc0)** to **[68978c2](https://github.com/sagemath/sagetrac-mirror/commit/68978c27de6990e4c9f8010a4447264c09aea871)**



---

archive/issue_comments_228832.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/68978c27de6990e4c9f8010a4447264c09aea871\">68978c2</a></td><td><code>Fixed small whitespace issues and condensed code</code></td></tr></table>\n",
    "created_at": "2015-01-16T17:23:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228832",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:11" align="right">Comment 11</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/68978c27de6990e4c9f8010a4447264c09aea871">68978c2</a></td><td><code>Fixed small whitespace issues and condensed code</code></td></tr></table>




---

archive/issue_comments_228833.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e\">0f9a1f3</a></td><td><code>More Typos and whitespace errors</code></td></tr></table>\n",
    "created_at": "2015-01-16T18:05:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228833",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e">0f9a1f3</a></td><td><code>More Typos and whitespace errors</code></td></tr></table>




---

archive/issue_comments_228834.json:
```json
{
    "body": "Changed commit from **[68978c2](https://github.com/sagemath/sagetrac-mirror/commit/68978c27de6990e4c9f8010a4447264c09aea871)** to **[0f9a1f3](https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e)**",
    "created_at": "2015-01-16T18:05:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228834",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[68978c2](https://github.com/sagemath/sagetrac-mirror/commit/68978c27de6990e4c9f8010a4447264c09aea871)** to **[0f9a1f3](https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e)**



---

archive/issue_events_226400.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-01-16T18:17:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226400"
}
```



---

archive/issue_events_226401.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-01-16T18:17:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226401"
}
```



---

archive/issue_events_226402.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-24T23:51:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226402"
}
```



---

archive/issue_events_226403.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bb8bb5e67345c4707d14045566daea47db4a2d53",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-24T23:51:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17082#event-226403"
}
```



---

archive/issue_comments_228835.json:
```json
{
    "body": "Changed branch from **[u/jdefaria/ticket/17082](https://github.com/sagemath/sagetrac-mirror/tree/u/jdefaria/ticket/17082)** to **[0f9a1f3](https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e)**",
    "created_at": "2015-01-24T23:51:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228835",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdefaria/ticket/17082](https://github.com/sagemath/sagetrac-mirror/tree/u/jdefaria/ticket/17082)** to **[0f9a1f3](https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e)**



---

archive/issue_comments_228836.json:
```json
{
    "body": "Changed commit from **[0f9a1f3](https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e)** to none",
    "created_at": "2015-02-23T16:25:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228836",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[0f9a1f3](https://github.com/sagemath/sagetrac-mirror/commit/0f9a1f375619114cb9232f9c49dfaacafc03c44e)** to none



---

archive/issue_comments_228837.json:
```json
{
    "body": "Changed author from **Joao de Faria** to **Joao Alberto de Faria**",
    "created_at": "2015-02-23T16:25:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17082",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17082#issuecomment-228837",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Joao de Faria** to **Joao Alberto de Faria**
