# Issue 17710: Context to add signal handler

archive/issues_017473.json:
```json
{
    "assignees": [],
    "body": "Implement\n\n```\nwith SignalHandler(signal.SIGCHILD, handler):\n    # ...\n```\nto temporarily *add* an additional signal handler.\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17710_\n\n",
    "closed_at": "2017-06-09T12:14:06Z",
    "created_at": "2015-02-01T11:24:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Context to add signal handler",
    "type": "issue",
    "updated_at": "2017-06-09T12:14:06Z",
    "url": "https://github.com/sagemath/sage/issues/17710",
    "user": "https://github.com/jdemeyer"
}
```
Implement

```
with SignalHandler(signal.SIGCHILD, handler):
    # ...
```
to temporarily *add* an additional signal handler.

Reviewer: **Jeroen Demeyer**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/17710_





---

archive/issue_events_234664.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-01T11:24:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-234664"
}
```



---

archive/issue_events_234665.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-01T11:24:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-234665"
}
```



---

archive/issue_events_234666.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-01T11:24:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-234666"
}
```



---

archive/issue_events_234667.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-01T11:25:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-234667"
}
```



---

archive/issue_comments_241670.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nSuperseded by http://cysignals.readthedocs.io/en/latest/pysignals.html#cysignals.pysignals.changesignal",
    "created_at": "2017-06-09T12:14:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17710#issuecomment-241670",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

Superseded by http://cysignals.readthedocs.io/en/latest/pysignals.html#cysignals.pysignals.changesignal



---

archive/issue_events_234668.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-09T12:14:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-234668"
}
```



---

archive/issue_events_234669.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-09T12:14:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-234669"
}
```



---

archive/issue_events_234670.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-09T12:14:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-234670"
}
```



---

archive/issue_comments_241671.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2017-06-09T12:14:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17710#issuecomment-241671",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_234671.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-09T12:14:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-234671"
}
```
