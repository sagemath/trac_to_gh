# Issue 17997: stopgap: `InfinityCrystalOfTableaux.Element.reduced_form` output change

archive/issues_017760.json:
```json
{
    "body": "In the process of making `Tableau` immutable (#15862), we found this mutates the supposedly immutable `Tableau` possibly making the result non-straight-shape. Once `CompositionTableau` is implemented, this output should be updated.\n\nCC:  jkeitel @tscrim @darijgr\n\nKeywords: days64, crystals, tableaux\n\nAuthor: Josh Swanson\n\nStatus: new\n\nDependencies: #15862\n\nIssue created by migration from https://trac.sagemath.org/ticket/17997\n\n",
    "created_at": "2015-03-19T00:44:16Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.6",
    "title": "stopgap: `InfinityCrystalOfTableaux.Element.reduced_form` output change",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/17997",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpswanson"
}
```
In the process of making `Tableau` immutable (#15862), we found this mutates the supposedly immutable `Tableau` possibly making the result non-straight-shape. Once `CompositionTableau` is implemented, this output should be updated.

CC:  jkeitel @tscrim @darijgr

Keywords: days64, crystals, tableaux

Author: Josh Swanson

Status: new

Dependencies: #15862

Issue created by migration from https://trac.sagemath.org/ticket/17997





---

archive/issue_comments_253092.json:
```json
{
    "body": "<a id='comment:1'></a>`CompositionTableau` in fact does exist, though it's inappropriate for the return value here since entries weakly decrease from left to right along rows in that class whereas entries weakly increase here. Perhaps a list of lists is the best solution after all?",
    "created_at": "2015-03-26T23:56:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17997#issuecomment-253092",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpswanson"
}
```

<a id='comment:1'></a>`CompositionTableau` in fact does exist, though it's inappropriate for the return value here since entries weakly decrease from left to right along rows in that class whereas entries weakly increase here. Perhaps a list of lists is the best solution after all?



---

archive/issue_comments_253093.json:
```json
{
    "body": "<a id='comment:2'></a>In general, I think there should be an ABC (abstract base class) `Tableau` which should allow any entry. Then it's subclasses, such as `SemistandardTableau`, should be more restrictive on its shapes/entries. Either that, or we should separate out a way to use the tableaux latex/ascii-art code to print tableau-like arrays.",
    "created_at": "2015-03-27T05:23:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17997#issuecomment-253093",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'></a>In general, I think there should be an ABC (abstract base class) `Tableau` which should allow any entry. Then it's subclasses, such as `SemistandardTableau`, should be more restrictive on its shapes/entries. Either that, or we should separate out a way to use the tableaux latex/ascii-art code to print tableau-like arrays.



---

archive/issue_comments_253094.json:
```json
{
    "body": "<a id='comment:3'></a>Indeed, #18013 includes a class much like you describe, `BadShapeTableau`. Cell locations are arbitrary pairs of integers and entries are completely arbitrary. The overall organization we have planned is more or less based on two \"axes\", one being shape restrictions, and the other being entry restrictions. `BadShapeTableau` probably is not appropriate here either because it's so general, though in principle if all you wanted to do was print, it could be used. Ideally there would just be a `CompositionShapedTableau` class with no restrictions on entries, of which `CompositionTableau` would be a subclass, and this `CompositionShapedTableau` class would be used here in `.reduced_form`. I'm not sure if it's worth the effort.",
    "created_at": "2015-03-27T05:54:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17997#issuecomment-253094",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpswanson"
}
```

<a id='comment:3'></a>Indeed, #18013 includes a class much like you describe, `BadShapeTableau`. Cell locations are arbitrary pairs of integers and entries are completely arbitrary. The overall organization we have planned is more or less based on two "axes", one being shape restrictions, and the other being entry restrictions. `BadShapeTableau` probably is not appropriate here either because it's so general, though in principle if all you wanted to do was print, it could be used. Ideally there would just be a `CompositionShapedTableau` class with no restrictions on entries, of which `CompositionTableau` would be a subclass, and this `CompositionShapedTableau` class would be used here in `.reduced_form`. I'm not sure if it's worth the effort.
