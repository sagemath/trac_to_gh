# Issue 17536: quo_rem fails for multivariate polynomial rings over function fields

archive/issues_017299.json:
```json
{
    "body": "The quo_rem function fails for multivariate polynomial rings over function fields. This seems related to change #17033. \n\n```\nR.<c>=FunctionField(QQ)\nS.<x,y>=R[]\nf=x^4*y + 2*c*x^2*y^3 - x*y^4 + (c^2 + c)*y^5\ng=x^2*y - x*y^2 + c*y^3\nf.quo_rem(g)\n```\n\nCC:  @bhutz\n\nReviewer: Travis Scrimshaw\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/17536\n\n",
    "closed_at": "2017-01-21T18:03:11Z",
    "created_at": "2014-12-21T13:47:28Z",
    "labels": [
        "component: algebraic geometry",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "quo_rem fails for multivariate polynomial rings over function fields",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/17536",
    "user": "https://github.com/bhutz"
}
```
The quo_rem function fails for multivariate polynomial rings over function fields. This seems related to change #17033. 

```
R.<c>=FunctionField(QQ)
S.<x,y>=R[]
f=x^4*y + 2*c*x^2*y^3 - x*y^4 + (c^2 + c)*y^5
g=x^2*y - x*y^2 + c*y^3
f.quo_rem(g)
```

CC:  @bhutz

Reviewer: Travis Scrimshaw

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/17536





---

archive/issue_comments_298432.json:
```json
{
    "body": "<a id='comment:1'></a>In working on #17535, I see that the failed example I list actually works with 6.5.beta3. \nPerhaps this has already been fixed somewhere.",
    "created_at": "2014-12-22T02:11:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17536#issuecomment-298432",
    "user": "https://github.com/bhutz"
}
```

<a id='comment:1'></a>In working on #17535, I see that the failed example I list actually works with 6.5.beta3. 
Perhaps this has already been fixed somewhere.



---

archive/issue_events_050374.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2017-01-16T19:45:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17536#event-50374"
}
```



---

archive/issue_comments_298433.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-01-16T19:45:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17536#issuecomment-298433",
    "user": "https://github.com/kwankyu"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_298434.json:
```json
{
    "body": "<a id='comment:3'></a>Do we want to add a doctest to prevent a regression since we don't know exactly what fixed this?",
    "created_at": "2017-01-17T17:40:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17536#issuecomment-298434",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>Do we want to add a doctest to prevent a regression since we don't know exactly what fixed this?



---

archive/issue_comments_298435.json:
```json
{
    "body": "<a id='comment:4'></a>I wish I would have put what the failure was...\n\nLooking at the code it does nothing put pass the input straight to singular and return the output from singular. Maybe the issue was the conversion of the singular output back to Sage or maybe it was issue with the Singular computation. I'm not sure at this point.\n\nI don't think we need another specific test for this as it is tested implicitly in some of the schememorphism_polynomial functionality (for example in dynatomic_polynomial as well as in homogenize/dehomogenize).  That is how I found the issue in the first place.",
    "created_at": "2017-01-17T17:51:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17536#issuecomment-298435",
    "user": "https://github.com/bhutz"
}
```

<a id='comment:4'></a>I wish I would have put what the failure was...

Looking at the code it does nothing put pass the input straight to singular and return the output from singular. Maybe the issue was the conversion of the singular output back to Sage or maybe it was issue with the Singular computation. I'm not sure at this point.

I don't think we need another specific test for this as it is tested implicitly in some of the schememorphism_polynomial functionality (for example in dynatomic_polynomial as well as in homogenize/dehomogenize).  That is how I found the issue in the first place.



---

archive/issue_comments_298436.json:
```json
{
    "body": "<a id='comment:5'></a>Okay. Then we can close as works-for-me.",
    "created_at": "2017-01-17T17:52:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17536#issuecomment-298436",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'></a>Okay. Then we can close as works-for-me.



---

archive/issue_comments_298437.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-01-17T17:52:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17536#issuecomment-298437",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_298438.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2017-01-17T17:52:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17536#issuecomment-298438",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_298439.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2017-01-21T18:03:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17536#issuecomment-298439",
    "user": "https://github.com/vbraun"
}
```

Resolution: invalid



---

archive/issue_events_050375.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/17536",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17536#event-50375"
}
```
