...
<a id='comment:1'></a>I attached a patch to this ticket.
...
```json
{
    "body": "<a id='comment:2'></a>Why the restriction that the right hand side only has one column?",
...
<a id='comment:2'></a>Why the restriction that the right hand side only has one column?
...
<a id='comment:3'></a>Because it did not even occur to me that the right hand side could have more than one. Thanks for pointing that out.
...
```json
{
    "body": "<a id='comment:7'></a>Having a look at this.",
...
<a id='comment:7'></a>Having a look at this.
