# Issue 17973: Better Sage consistency for naming and calling in linear_code

archive/issues_017736.json:
```json
{
    "body": "CC:  @johanrosenkilde @defeo @nathanncohen\n\nSome method and parameter names in linear_code.py file are abbreviated names which might be confusing (for instance the `distance` parameter actually stands for the minimum distance).\nMost importantly, the `gen_mat` method will be renamed `generator_matrix` and the `check_mat` method `parity_check_matrix`. \n\nBesides, some getter methods to access the private fields of linear codes exist but are not used internally in the class.\nTo support subclassing, it is better to use these instead of directly invoking a parameter.\n  \n\nIssue created by migration from https://trac.sagemath.org/ticket/17973\n\n",
    "closed_at": "2015-03-19T03:17:18Z",
    "created_at": "2015-03-17T14:04:54Z",
    "labels": [
        "component: coding theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.6",
    "title": "Better Sage consistency for naming and calling in linear_code",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/17973",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```
CC:  @johanrosenkilde @defeo @nathanncohen

Some method and parameter names in linear_code.py file are abbreviated names which might be confusing (for instance the `distance` parameter actually stands for the minimum distance).
Most importantly, the `gen_mat` method will be renamed `generator_matrix` and the `check_mat` method `parity_check_matrix`. 

Besides, some getter methods to access the private fields of linear codes exist but are not used internally in the class.
To support subclassing, it is better to use these instead of directly invoking a parameter.
  

Issue created by migration from https://trac.sagemath.org/ticket/17973





---

archive/issue_comments_236861.json:
```json
{
    "body": "New commits:",
    "created_at": "2015-03-17T15:40:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236861",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

New commits:



---

archive/issue_comments_236862.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-03-17T15:40:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236862",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_236863.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2015-03-17T15:56:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236863",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_236864.json:
```json
{
    "body": "Hello !\n\nI would say that this is good to go, except for one detail: could you deprecate the methods using `deprecated_function_alias` instead of doing it manually? There are some advantages, like a warning in the doc of the deprecated function. Besides, they do not appear in the lists of undocumented functions.\n\nhttp://www.sagemath.org/doc/developer/coding_in_python.html#deprecation\n\nNathann",
    "created_at": "2015-03-17T15:56:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236864",
    "user": "https://github.com/nathanncohen"
}
```

Hello !

I would say that this is good to go, except for one detail: could you deprecate the methods using `deprecated_function_alias` instead of doing it manually? There are some advantages, like a warning in the doc of the deprecated function. Besides, they do not appear in the lists of undocumented functions.

http://www.sagemath.org/doc/developer/coding_in_python.html#deprecation

Nathann



---

archive/issue_comments_236865.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-03-17T16:21:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236865",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_236866.json:
```json
{
    "body": "It's done! I also replaced \"check matrix\" by \"parity check matrix\" in `parity_check_matrix` docstring for consistency with its new name.",
    "created_at": "2015-03-17T16:22:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236866",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

It's done! I also replaced "check matrix" by "parity check matrix" in `parity_check_matrix` docstring for consistency with its new name.



---

archive/issue_comments_236867.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2015-03-17T16:22:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236867",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_236868.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2015-03-17T16:32:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236868",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_236869.json:
```json
{
    "body": "There are two broken doctests in binary_code.pyx:\n\n```\n(tmp|\u2714)~/sage/coding$ sage -tp 4 -l binary_code.pyx\ntoo many failed tests, not using stored timings\nRunning doctests with ID 2015-03-17-17-31-39-70361bd3.\nGit branch: tmp\nDoctesting 1 file using 4 threads.\nsage -t --long binary_code.pyx\n**********************************************************************\nFile \"binary_code.pyx\", line 1100, in sage.coding.binary_code.BinaryCode.apply_permutation\nFailed example:\n    B = BinaryCode(codes.ExtendedBinaryGolayCode().gen_mat())\nExpected nothing\nGot:\n    doctest:1: DeprecationWarning: gen_mat is deprecated. Please use generator_matrix instead.\n    See http://trac.sagemath.org/17973 for details.\n**********************************************************************\nFile \"binary_code.pyx\", line 3858, in sage.coding.binary_code.BinaryCodeClassifier.put_in_canonical_form\nFailed example:\n    B = BinaryCode(codes.ExtendedBinaryGolayCode().gen_mat())\nExpected nothing\nGot:\n    doctest:1: DeprecationWarning: gen_mat is deprecated. Please use generator_matrix instead.\n    See http://trac.sagemath.org/17973 for details.\n**********************************************************************\n2 items had failures:\n   1 of   6 in sage.coding.binary_code.BinaryCode.apply_permutation\n   1 of   8 in sage.coding.binary_code.BinaryCodeClassifier.put_in_canonical_form\n    [351 tests, 2 failures, 6.60 s]\n----------------------------------------------------------------------\nsage -t --long binary_code.pyx  # 2 doctests failed\n----------------------------------------------------------------------\nTotal time for all tests: 6.7 seconds\n    cpu time: 6.3 seconds\n    cumulative wall time: 6.6 seconds\n\n```",
    "created_at": "2015-03-17T16:32:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236869",
    "user": "https://github.com/nathanncohen"
}
```

There are two broken doctests in binary_code.pyx:

```
(tmp|✔)~/sage/coding$ sage -tp 4 -l binary_code.pyx
too many failed tests, not using stored timings
Running doctests with ID 2015-03-17-17-31-39-70361bd3.
Git branch: tmp
Doctesting 1 file using 4 threads.
sage -t --long binary_code.pyx
**********************************************************************
File "binary_code.pyx", line 1100, in sage.coding.binary_code.BinaryCode.apply_permutation
Failed example:
    B = BinaryCode(codes.ExtendedBinaryGolayCode().gen_mat())
Expected nothing
Got:
    doctest:1: DeprecationWarning: gen_mat is deprecated. Please use generator_matrix instead.
    See http://trac.sagemath.org/17973 for details.
**********************************************************************
File "binary_code.pyx", line 3858, in sage.coding.binary_code.BinaryCodeClassifier.put_in_canonical_form
Failed example:
    B = BinaryCode(codes.ExtendedBinaryGolayCode().gen_mat())
Expected nothing
Got:
    doctest:1: DeprecationWarning: gen_mat is deprecated. Please use generator_matrix instead.
    See http://trac.sagemath.org/17973 for details.
**********************************************************************
2 items had failures:
   1 of   6 in sage.coding.binary_code.BinaryCode.apply_permutation
   1 of   8 in sage.coding.binary_code.BinaryCodeClassifier.put_in_canonical_form
    [351 tests, 2 failures, 6.60 s]
----------------------------------------------------------------------
sage -t --long binary_code.pyx  # 2 doctests failed
----------------------------------------------------------------------
Total time for all tests: 6.7 seconds
    cpu time: 6.3 seconds
    cumulative wall time: 6.6 seconds

```



---

archive/issue_comments_236870.json:
```json
{
    "body": "And many others in `src/doc/en/constructions/linear_codes.rst`. In some situations it is safer to run all of Sage's doctests just to make sure. There may be others in places I could not guess.\n\nNathann",
    "created_at": "2015-03-17T16:36:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236870",
    "user": "https://github.com/nathanncohen"
}
```

And many others in `src/doc/en/constructions/linear_codes.rst`. In some situations it is safer to run all of Sage's doctests just to make sure. There may be others in places I could not guess.

Nathann



---

archive/issue_comments_236871.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-03-17T18:39:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236871",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_236872.json:
```json
{
    "body": "Thanks for the advice. I was indeed able to find some more after a `make -ptestlong`.",
    "created_at": "2015-03-17T18:40:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236872",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Thanks for the advice. I was indeed able to find some more after a `make -ptestlong`.



---

archive/issue_comments_236873.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2015-03-17T18:40:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236873",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_236874.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-03-17T23:01:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236874",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_236875.json:
```json
{
    "body": "OKayyyyyyyyyyy then it's good to go !\n\nNathann",
    "created_at": "2015-03-17T23:01:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236875",
    "user": "https://github.com/nathanncohen"
}
```

OKayyyyyyyyyyy then it's good to go !

Nathann



---

archive/issue_comments_236876.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-03-19T03:17:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17973#issuecomment-236876",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_051115.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-19T03:17:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/17973",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17973#event-51115"
}
```
