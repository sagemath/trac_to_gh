# Issue 17721: variable shadowing: major design flaw in Singular

archive/issues_017484.json:
```json
{
    "body": "\nSingular has an issue of overlapping namespaces\nwhere variable conflicts will lead to bugs without warnings:\n\n```\n//Singular code:\nring rng = 0, p ,dp;\npoly p = p^2 - p - 1;  // **boom**\np;  // (   p = -1,  wrong !  )\n```\n\n\nIn combination with hardcoded variable names or unreflected usage of\n'execute' in Singular's library code this may lead to wrong results as shown in \n\nhttp://www.singular.uni-kl.de:8002/trac/ticket/677.\nSee also: \nthe discussion at http://www.singular.uni-kl.de/forum/viewtopic.php?f=10&t=2482\n\nMain upstream ticket:\nhttp://www.singular.uni-kl.de:8002/trac/ticket/684\n\n\nTrying to translate the example from http://www.singular.uni-kl.de:8002/trac/ticket/677 to sage\nI hit another bug (#17720)\n\n\n\n\n\nCC:  @jdemeyer\n\nKeywords: Singular\n\nUpstream: Fixed upstream, in a later stable release.\n\nStopgaps: todo\n\nReviewer: Dave Morris, Samuel Leli\u00e8vre\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/17721\n\n",
    "closed_at": "2021-08-26T02:08:43Z",
    "created_at": "2015-02-03T15:07:15Z",
    "labels": [
        "component: packages: standard",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "variable shadowing: major design flaw in Singular",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/17721",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Singular has an issue of overlapping namespaces
where variable conflicts will lead to bugs without warnings:

```
//Singular code:
ring rng = 0, p ,dp;
poly p = p^2 - p - 1;  // **boom**
p;  // (   p = -1,  wrong !  )
```


In combination with hardcoded variable names or unreflected usage of
'execute' in Singular's library code this may lead to wrong results as shown in 

http://www.singular.uni-kl.de:8002/trac/ticket/677.
See also: 
the discussion at http://www.singular.uni-kl.de/forum/viewtopic.php?f=10&t=2482

Main upstream ticket:
http://www.singular.uni-kl.de:8002/trac/ticket/684


Trying to translate the example from http://www.singular.uni-kl.de:8002/trac/ticket/677 to sage
I hit another bug (#17720)





CC:  @jdemeyer

Keywords: Singular

Upstream: Fixed upstream, in a later stable release.

Stopgaps: todo

Reviewer: Dave Morris, Samuel Lelièvre

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/17721





---

archive/issue_comments_303533.json:
```json
{
    "body": "<a id='comment:1'></a>`@`William\nI'm taking you out from Cc, just wanted that you notice this issue",
    "created_at": "2015-02-03T15:08:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303533",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:1'></a>`@`William
I'm taking you out from Cc, just wanted that you notice this issue



---

archive/issue_comments_303534.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#17720\"",
    "created_at": "2015-02-03T15:13:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303534",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Changing dependencies from "" to "#17720"



---

archive/issue_comments_303535.json:
```json
{
    "body": "<a id='comment:3'></a>translated example from  www.singular.uni-kl.de:8002/trac/ticket/677 :\n\n```\nsage: K0=QQ\nsage: R0.<p>=K0[]\nsage: K.<p>=QQ.extension(p^2-p-1)\nsage: R1.<w,x,y,z>=K[]\nsage: I = R1.ideal((-12*p-6)*w^5+(16*p+8)*w^3*x^2+(-4*p-2)*w*x^4+(16*p+8)*w^3*y^2+(-8*p-4)*w*x^2*y^2+(-4*p-2)*w*y^4+(16*p+8)*w^3*z^2+(-8*p-4)*w*x^2*z^2+(-8*p-4)*w*y^2*z^2+(-4*p-2)*w*z^4,(8*p+4)*w^4*x+(-8*p-4)*w^2*x^3+(-8*p-4)*w^2*x*y^2+(-48*p-32)*x^3*y^2+(8*p+8)*x*y^4+(-8*p-4)*w^2*x*z^2+(16*p+16)*x^3*z^2+(64*p+32)*x*y^2*z^2+(-24*p-16)*x*z^4,(8*p+4)*w^4*y+(-8*p-4)*w^2*x^2*y+(-24*p-16)*x^4*y+(-8*p-4)*w^2*y^3+(16*p+16)*x^2*y^3+(-8*p-4)*w^2*y*z^2+(64*p+32)*x^2*y*z^2+(-48*p-32)*y^3*z^2+(8*p+8)*y*z^4,(8*p+4)*w^4*z+(-8*p-4)*w^2*x^2*z+(8*p+8)*x^4*z+(-8*p-4)*w^2*y^2*z+(64*p+32)*x^2*y^2*z+(-24*p-16)*y^4*z+(-8*p-4)*w^2*z^3+(-48*p-32)*x^2*z^3+(16*p+16)*y^2*z^3)\nsage: L = I.primary_decomposition(\"gtz\") \nsage: IJ = I\nsage: for J in L:\n....:      IJ = IJ.intersection(J)\n....:     \nsage: IJ == I\nFalse\n\n```",
    "created_at": "2015-02-03T15:22:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303535",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:3'></a>translated example from  www.singular.uni-kl.de:8002/trac/ticket/677 :

```
sage: K0=QQ
sage: R0.<p>=K0[]
sage: K.<p>=QQ.extension(p^2-p-1)
sage: R1.<w,x,y,z>=K[]
sage: I = R1.ideal((-12*p-6)*w^5+(16*p+8)*w^3*x^2+(-4*p-2)*w*x^4+(16*p+8)*w^3*y^2+(-8*p-4)*w*x^2*y^2+(-4*p-2)*w*y^4+(16*p+8)*w^3*z^2+(-8*p-4)*w*x^2*z^2+(-8*p-4)*w*y^2*z^2+(-4*p-2)*w*z^4,(8*p+4)*w^4*x+(-8*p-4)*w^2*x^3+(-8*p-4)*w^2*x*y^2+(-48*p-32)*x^3*y^2+(8*p+8)*x*y^4+(-8*p-4)*w^2*x*z^2+(16*p+16)*x^3*z^2+(64*p+32)*x*y^2*z^2+(-24*p-16)*x*z^4,(8*p+4)*w^4*y+(-8*p-4)*w^2*x^2*y+(-24*p-16)*x^4*y+(-8*p-4)*w^2*y^3+(16*p+16)*x^2*y^3+(-8*p-4)*w^2*y*z^2+(64*p+32)*x^2*y*z^2+(-48*p-32)*y^3*z^2+(8*p+8)*y*z^4,(8*p+4)*w^4*z+(-8*p-4)*w^2*x^2*z+(8*p+8)*x^4*z+(-8*p-4)*w^2*y^2*z+(64*p+32)*x^2*y^2*z+(-24*p-16)*y^4*z+(-8*p-4)*w^2*z^3+(-48*p-32)*x^2*z^3+(16*p+16)*y^2*z^3)
sage: L = I.primary_decomposition("gtz") 
sage: IJ = I
sage: for J in L:
....:      IJ = IJ.intersection(J)
....:     
sage: IJ == I
False

```



---

archive/issue_events_050698.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2021-01-29T03:11:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17721#event-50698"
}
```



---

archive/issue_comments_303536.json:
```json
{
    "body": "Changing dependencies from \"#17720\" to \"\"",
    "created_at": "2021-01-29T03:11:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303536",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changing dependencies from "#17720" to ""



---

archive/issue_comments_303537.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2021-01-29T03:11:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303537",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_303538.json:
```json
{
    "body": "Changing priority from major to minor.",
    "created_at": "2021-01-29T03:11:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303538",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changing priority from major to minor.



---

archive/issue_comments_303539.json:
```json
{
    "body": "<a id='comment:4'></a>This example now returns `True`, as it apparently should. But it takes a long time (about a minute) and this was an upstream issue, so I don't think a doctest is worth the trouble. Let's close this old ticket as \"works for me\".",
    "created_at": "2021-01-29T03:11:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303539",
    "user": "https://github.com/DaveWitteMorris"
}
```

<a id='comment:4'></a>This example now returns `True`, as it apparently should. But it takes a long time (about a minute) and this was an upstream issue, so I don't think a doctest is worth the trouble. Let's close this old ticket as "works for me".



---

archive/issue_comments_303540.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Dave Morris\"",
    "created_at": "2021-01-29T03:11:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303540",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changing reviewer from "" to "Dave Morris"



---

archive/issue_comments_303541.json:
```json
{
    "body": "Changing reviewer from \"Dave Morris\" to \"Dave Morris, Samuel Leli\u00e8vre\"",
    "created_at": "2021-08-19T22:20:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303541",
    "user": "https://github.com/slel"
}
```

Changing reviewer from "Dave Morris" to "Dave Morris, Samuel Lelièvre"



---

archive/issue_comments_303542.json:
```json
{
    "body": "Changing upstream from \"Reported upstream. Developers acknowledge bug.\" to \"Fixed upstream, in a later stable release.\"",
    "created_at": "2021-08-19T22:20:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303542",
    "user": "https://github.com/slel"
}
```

Changing upstream from "Reported upstream. Developers acknowledge bug." to "Fixed upstream, in a later stable release."



---

archive/issue_comments_303543.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2021-08-19T22:20:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303543",
    "user": "https://github.com/slel"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_303544.json:
```json
{
    "body": "<a id='comment:5'></a>Ok.",
    "created_at": "2021-08-19T22:20:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303544",
    "user": "https://github.com/slel"
}
```

<a id='comment:5'></a>Ok.



---

archive/issue_comments_303545.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2021-08-26T02:08:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/17721#issuecomment-303545",
    "user": "https://github.com/mkoeppe"
}
```

Resolution: invalid



---

archive/issue_events_050699.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:08:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/17721",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/17721#event-50699"
}
```
