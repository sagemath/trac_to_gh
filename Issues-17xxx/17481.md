# Issue 17481: Python does not need dpkg-dev anymore on multiarch debians

archive/issues_017244.json:
```json
{
    "assignees": [],
    "body": "See the Python (closed) issue:\n* http://bugs.python.org/issue11715\n\nWe should update:\n* our install doc,\n* the check made by `prereq`.\n\nCC:  @jdemeyer @vbraun\n\nBranch/Commit: **[c354fd4](https://github.com/sagemath/sagetrac-mirror/commit/c354fd403931d61afd0f6dd42e55b3d95520464b)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/17481_\n\n",
    "closed_at": "2015-11-16T18:37:09Z",
    "created_at": "2014-12-10T10:10:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Python does not need dpkg-dev anymore on multiarch debians",
    "type": "issue",
    "updated_at": "2015-11-16T18:37:09Z",
    "url": "https://github.com/sagemath/sage/issues/17481",
    "user": "https://github.com/sagetrac-jpflori"
}
```
See the Python (closed) issue:
* http://bugs.python.org/issue11715

We should update:
* our install doc,
* the check made by `prereq`.

CC:  @jdemeyer @vbraun

Branch/Commit: **[c354fd4](https://github.com/sagemath/sagetrac-mirror/commit/c354fd403931d61afd0f6dd42e55b3d95520464b)**

Reviewer: **Jeroen Demeyer**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/17481_





---

archive/issue_events_231513.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-12-10T10:10:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231513"
}
```



---

archive/issue_events_231514.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-12-10T10:10:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231514"
}
```



---

archive/issue_events_231515.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-12-10T10:10:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231515"
}
```



---

archive/issue_events_231516.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-12-10T10:10:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231516"
}
```



---

archive/issue_comments_236750.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAnd I've installed Sage's Python on a debian-based multiarch system lacking `dpkg-dev` without problem.",
    "created_at": "2014-12-10T10:11:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236750",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:1'>Comment 1:</a>
And I've installed Sage's Python on a debian-based multiarch system lacking `dpkg-dev` without problem.



---

archive/issue_comments_236751.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c9a8a669ed4d37b50b1ce47cfcc8d32b5de837b9\">c9a8a66</a></td><td><code>trac #17481 removing requirement for dpkg-dev in the doc</code></td></tr></table>\n",
    "created_at": "2015-08-30T16:45:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236751",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c9a8a669ed4d37b50b1ce47cfcc8d32b5de837b9">c9a8a66</a></td><td><code>trac #17481 removing requirement for dpkg-dev in the doc</code></td></tr></table>




---

archive/issue_comments_236752.json:
```json
{
    "body": "Commit: **[c9a8a66](https://github.com/sagemath/sagetrac-mirror/commit/c9a8a669ed4d37b50b1ce47cfcc8d32b5de837b9)**",
    "created_at": "2015-08-30T16:45:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236752",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[c9a8a66](https://github.com/sagemath/sagetrac-mirror/commit/c9a8a669ed4d37b50b1ce47cfcc8d32b5de837b9)**



---

archive/issue_comments_236753.json:
```json
{
    "body": "Branch: **[u/chapoton/17481](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/17481)**",
    "created_at": "2015-08-30T16:45:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236753",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/17481](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/17481)**



---

archive/issue_comments_236754.json:
```json
{
    "body": "Changed commit from **[c9a8a66](https://github.com/sagemath/sagetrac-mirror/commit/c9a8a669ed4d37b50b1ce47cfcc8d32b5de837b9)** to **[03f39b2](https://github.com/sagemath/sagetrac-mirror/commit/03f39b268bfae4b58b34d86fb4595846291a9c95)**",
    "created_at": "2015-08-30T16:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236754",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c9a8a66](https://github.com/sagemath/sagetrac-mirror/commit/c9a8a669ed4d37b50b1ce47cfcc8d32b5de837b9)** to **[03f39b2](https://github.com/sagemath/sagetrac-mirror/commit/03f39b268bfae4b58b34d86fb4595846291a9c95)**



---

archive/issue_comments_236755.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/03f39b268bfae4b58b34d86fb4595846291a9c95\">03f39b2</a></td><td><code>trac #17481 remove dpkg-dev requirement in configure</code></td></tr></table>\n",
    "created_at": "2015-08-30T16:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236755",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/03f39b268bfae4b58b34d86fb4595846291a9c95">03f39b2</a></td><td><code>trac #17481 remove dpkg-dev requirement in configure</code></td></tr></table>




---

archive/issue_comments_236756.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cdeb098541f3bd2bbc2a4ef47c23cba5385dd954\">cdeb098</a></td><td><code>Merge branch 'u/chapoton/17481' into 6.10.beta1</code></td></tr></table>\n",
    "created_at": "2015-10-28T19:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236756",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cdeb098541f3bd2bbc2a4ef47c23cba5385dd954">cdeb098</a></td><td><code>Merge branch 'u/chapoton/17481' into 6.10.beta1</code></td></tr></table>




---

archive/issue_comments_236757.json:
```json
{
    "body": "Changed commit from **[03f39b2](https://github.com/sagemath/sagetrac-mirror/commit/03f39b268bfae4b58b34d86fb4595846291a9c95)** to **[cdeb098](https://github.com/sagemath/sagetrac-mirror/commit/cdeb098541f3bd2bbc2a4ef47c23cba5385dd954)**",
    "created_at": "2015-10-28T19:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236757",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[03f39b2](https://github.com/sagemath/sagetrac-mirror/commit/03f39b268bfae4b58b34d86fb4595846291a9c95)** to **[cdeb098](https://github.com/sagemath/sagetrac-mirror/commit/cdeb098541f3bd2bbc2a4ef47c23cba5385dd954)**



---

archive/issue_events_231517.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-28T19:57:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231517"
}
```



---

archive/issue_events_231518.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-28T19:57:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231518"
}
```



---

archive/issue_events_231519.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-28T19:57:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231519"
}
```



---

archive/issue_events_231520.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-29T09:39:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231520"
}
```



---

archive/issue_events_231521.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-29T09:39:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231521"
}
```



---

archive/issue_comments_236758.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nI think you need to change also `build/pkgs/python2/spkg-install` and `build/pkgs/python3/spkg-install`.\n\nAlso, the author name is missing.",
    "created_at": "2015-10-29T09:39:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236758",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
I think you need to change also `build/pkgs/python2/spkg-install` and `build/pkgs/python3/spkg-install`.

Also, the author name is missing.



---

archive/issue_comments_236759.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/207973b08459f37044434c20269aba410a893c4c\">207973b</a></td><td><code>Merge branch 'u/chapoton/17481' into 6.10.b2</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c354fd403931d61afd0f6dd42e55b3d95520464b\">c354fd4</a></td><td><code>trac #17481 remove some references to dpkg</code></td></tr></table>\n",
    "created_at": "2015-10-29T11:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236759",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/207973b08459f37044434c20269aba410a893c4c">207973b</a></td><td><code>Merge branch 'u/chapoton/17481' into 6.10.b2</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c354fd403931d61afd0f6dd42e55b3d95520464b">c354fd4</a></td><td><code>trac #17481 remove some references to dpkg</code></td></tr></table>




---

archive/issue_comments_236760.json:
```json
{
    "body": "Changed commit from **[cdeb098](https://github.com/sagemath/sagetrac-mirror/commit/cdeb098541f3bd2bbc2a4ef47c23cba5385dd954)** to **[c354fd4](https://github.com/sagemath/sagetrac-mirror/commit/c354fd403931d61afd0f6dd42e55b3d95520464b)**",
    "created_at": "2015-10-29T11:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236760",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[cdeb098](https://github.com/sagemath/sagetrac-mirror/commit/cdeb098541f3bd2bbc2a4ef47c23cba5385dd954)** to **[c354fd4](https://github.com/sagemath/sagetrac-mirror/commit/c354fd403931d61afd0f6dd42e55b3d95520464b)**



---

archive/issue_comments_236761.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2015-10-29T11:57:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236761",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_events_231522.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-29T11:57:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231522"
}
```



---

archive/issue_events_231523.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-29T11:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231523"
}
```



---

archive/issue_comments_236762.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-10-29T14:30:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236762",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_236763.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nPositive_review modulo testing that it actually works.\n\nReplying to [jpflori](#comment%3A1):\n> And I've installed Sage's Python on a debian-based multiarch system lacking `dpkg-dev` without problem.\n\nCan you try building the most recent of this branch after `make distclean` on such a system? If you (or somebody else) did and it worked, you can set this ticket to positive review.",
    "created_at": "2015-10-29T14:30:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236763",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'>Comment 9:</a>
Positive_review modulo testing that it actually works.

Replying to [jpflori](#comment%3A1):
> And I've installed Sage's Python on a debian-based multiarch system lacking `dpkg-dev` without problem.

Can you try building the most recent of this branch after `make distclean` on such a system? If you (or somebody else) did and it worked, you can set this ticket to positive review.



---

archive/issue_comments_236764.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nReplying to [@jdemeyer](#comment%3A9):\n> Positive_review modulo testing that it actually works.\n\nIt does.",
    "created_at": "2015-11-13T16:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236764",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:10'>Comment 10:</a>
Replying to [@jdemeyer](#comment%3A9):
> Positive_review modulo testing that it actually works.

It does.



---

archive/issue_events_231524.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-11-13T16:39:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231524"
}
```



---

archive/issue_events_231525.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-11-13T16:39:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231525"
}
```



---

archive/issue_comments_236765.json:
```json
{
    "body": "Changed branch from **[u/chapoton/17481](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/17481)** to **[c354fd4](https://github.com/sagemath/sagetrac-mirror/commit/c354fd403931d61afd0f6dd42e55b3d95520464b)**",
    "created_at": "2015-11-16T18:37:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17481#issuecomment-236765",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/17481](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/17481)** to **[c354fd4](https://github.com/sagemath/sagetrac-mirror/commit/c354fd403931d61afd0f6dd42e55b3d95520464b)**



---

archive/issue_events_231526.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-11-16T18:37:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231526"
}
```



---

archive/issue_events_231527.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "13f0610412c876768479d77fa8890d68a136ba15",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-11-16T18:37:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17481",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17481#event-231527"
}
```
