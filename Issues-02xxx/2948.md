# Issue 2948: (Additional) Docstrings and Doctests for rings/quotient_ring.py

archive/issues_002948.json:
```json
{
    "body": "Provide missing docstrings and doctests for rings/quotient_ring.py. Also provide additional doctests for several functions i.e. imporve the quality of the doctests in this file.\n\n* SCORE quotient_ring.py: 54% (12 of 22)\n\n**Assignee:** @cswiercz\n\n**Keywords:** doctest, docstring, documentation, quotient_ring.py\n\nIssue created by migration from https://trac.sagemath.org/ticket/2948\n\n",
    "closed_at": "2008-09-27T00:53:34Z",
    "created_at": "2008-04-17T22:36:52Z",
    "labels": [
        "component: doctest coverage",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.3",
    "title": "(Additional) Docstrings and Doctests for rings/quotient_ring.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2948",
    "user": "https://github.com/cswiercz"
}
```
Provide missing docstrings and doctests for rings/quotient_ring.py. Also provide additional doctests for several functions i.e. imporve the quality of the doctests in this file.

* SCORE quotient_ring.py: 54% (12 of 22)

**Assignee:** @cswiercz

**Keywords:** doctest, docstring, documentation, quotient_ring.py

Issue created by migration from https://trac.sagemath.org/ticket/2948





---

archive/issue_comments_016196.json:
```json
{
    "body": "<a id='comment:1'></a>\nChris,\n\nIs this on top of #2507? If so you should clearly indicate that.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-17T23:42:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2948",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2948#issuecomment-16196",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Chris,

Is this on top of #2507? If so you should clearly indicate that.

Cheers,

Michael



---

archive/issue_events_015505.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-09-26T19:09:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2948",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2948#event-15505"
}
```



---

archive/attachments_002804.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "2948.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket2948/2948.patch",
    "created_at": "2008-09-26T19:09:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2948",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_016197.json:
```json
{
    "body": "<a id='comment:2'></a>\nI'm providing a patch. The doctesting that I'm providing is pretty superficial -- it doesn't show any of the limitations of the code, for example. If someone who knows the stuff better wants to add more, please go ahead.",
    "created_at": "2008-09-26T19:09:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2948",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2948#issuecomment-16197",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
I'm providing a patch. The doctesting that I'm providing is pretty superficial -- it doesn't show any of the limitations of the code, for example. If someone who knows the stuff better wants to add more, please go ahead.



---

archive/issue_events_015506.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-09-26T22:30:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2948",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2948#event-15506"
}
```



---

archive/issue_events_015507.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-09-26T22:30:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2948",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2948#event-15507"
}
```



---

archive/issue_comments_016198.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.3.alpha2",
    "created_at": "2008-09-27T00:53:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2948",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2948#issuecomment-16198",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.3.alpha2



---

archive/issue_events_015508.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-27T00:53:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2948",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2948#event-15508"
}
```



---

archive/issue_events_015509.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-27T00:53:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2948",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2948#event-15509"
}
```
