...
```json
{
    "body": "<a id='comment:1'></a>Following a lead from the mailing list, I added \"keepfloat: true\" to the init_code for the maxima interfaces.  The patch is attached.",
...
<a id='comment:1'></a>Following a lead from the mailing list, I added "keepfloat: true" to the init_code for the maxima interfaces.  The patch is attached.
...
```json
{
    "body": "<a id='comment:2'></a>Jason, please improve the comment \"# no ascii art output\" to indicate what the new flag does.",
...
<a id='comment:2'></a>Jason, please improve the comment "# no ascii art output" to indicate what the new flag does.
...
```json
{
    "body": "<a id='comment:3'></a>Attachment [keepfloat.patch](tarball://root/attachments/some-uuid/ticket2400/keepfloat.patch) by @jasongrout created at 2008-03-06 01:38:50\n\nI replaced the patch with an updated one addressing dmharvey's concerns.  The patch is ready to be reviewed again.",
...
<a id='comment:3'></a>Attachment [keepfloat.patch](tarball://root/attachments/some-uuid/ticket2400/keepfloat.patch) by @jasongrout created at 2008-03-06 01:38:50
...
<a id='comment:4'></a>This patch fails doctests on
...
<a id='comment:5'></a>The code itself is fine but symbolic matrices with keepfloat appear to be broken in Maxima.  When/if symbolic matrices do not use maxima this will be a good patch.  
...
<a id='comment:6'></a>The matrixexp command seems to work fine in maxima (see below).  How do you reproduce your error?  (can you give a short sage session that gives the error above?)
...
<a id='comment:7'></a>Here is a way to reproduce the above error:
...
```json
{
    "body": "<a id='comment:8'></a>This post talks about the weakness of the maxima linear algebra functionality and explicitly includes the above error: http://www.ma.utexas.edu/pipermail/maxima/2006/003031.html",
...
<a id='comment:8'></a>This post talks about the weakness of the maxima linear algebra functionality and explicitly includes the above error: http://www.ma.utexas.edu/pipermail/maxima/2006/003031.html
...
<a id='comment:9'></a>The last post at http://www.nabble.com/matrix-exponential--td6816907.html suggests using the "diag" package instead of the "linearalgebra" package for calculating the matrix exponential.  This works with keepfloat:
...
<a id='comment:10'></a>The maxima matrixexp function has problems with floating point entries.  I don't think that should keep us from fixing *wrong* output (or at least, unnecessarily rounded) from maxima due to their rounding floating points to rationals.
...
```json
{
    "body": "<a id='comment:12'></a>These two patches apply against 2.10.4.alpha0, fix the problems, and pass tests for me.",
...
<a id='comment:12'></a>These two patches apply against 2.10.4.alpha0, fix the problems, and pass tests for me.
...
```json
{
    "body": "<a id='comment:13'></a>Merged both patches in Sage 2.10.4.rc0",
    "created_at": "2008-03-16T02:48:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2400",
    "type": "issue_comment",
...
<a id='comment:13'></a>Merged both patches in Sage 2.10.4.rc0
...
```json
{
    "body": "<a id='comment:14'></a>For some reason this hack doesn't work anymore in Maxima 5.30, see #13973.",
    "created_at": "2013-06-18T18:22:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2400",
    "type": "issue_comment",
...
<a id='comment:14'></a>For some reason this hack doesn't work anymore in Maxima 5.30, see #13973.
