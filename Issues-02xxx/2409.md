# Issue 2409: plotting constant functions throws an error

archive/issues_002409.json:
```json
{
    "body": "The command \n\n```\nsage: plot(x+1,(x,1,4))\n```\nproduces a very nice line with slope 1 (and I love the syntax which I think is a moderately recent improvement!).\n\nBy analogy,\n\n```\nsage: plot(1,(x,1,4))\n```\nshould produce a horizontal line.\n\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2409\n\n",
    "closed_at": "2008-08-26T21:54:49Z",
    "created_at": "2008-03-06T20:33:24Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "plotting constant functions throws an error",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2409",
    "user": "https://trac.sagemath.org/admin/accounts/users/jbmohler"
}
```
The command 

```
sage: plot(x+1,(x,1,4))
```
produces a very nice line with slope 1 (and I love the syntax which I think is a moderately recent improvement!).

By analogy,

```
sage: plot(1,(x,1,4))
```
should produce a horizontal line.


Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2409





---

archive/issue_events_008427.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-07T02:41:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2409",
    "milestone": "sage-2.10.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2409#event-8427"
}
```



---

archive/issue_events_008428.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2409",
    "rename": {
        "from": "plot'ting constants",
        "to": "plotting constant functions throws an error"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2409#event-8428"
}
```



---

archive/issue_comments_017009.json:
```json
{
    "body": "<a id='comment:3'></a>\nThese will be fixed by #3952",
    "created_at": "2008-08-26T02:00:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2409#issuecomment-17009",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
These will be fixed by #3952



---

archive/issue_comments_017010.json:
```json
{
    "body": "<a id='comment:4'></a>\nNote that this depends on #2410 and #3853.",
    "created_at": "2008-08-26T02:03:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2409#issuecomment-17010",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:4'></a>
Note that this depends on #2410 and #3853.



---

archive/issue_events_008429.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2409",
    "rename": {
        "from": "plotting constant functions throws an error",
        "to": "[with patch, needs review] plotting constant functions throws an error"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2409#event-8429"
}
```



---

archive/issue_events_008430.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2409",
    "rename": {
        "from": "[with patch, needs review] plotting constant functions throws an error",
        "to": "plotting constant functions throws an error"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2409#event-8430"
}
```



---

archive/issue_comments_017011.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-26T21:54:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2409#issuecomment-17011",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_017012.json:
```json
{
    "body": "<a id='comment:7'></a>\nSince #3952 was merged this is fixed.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-26T21:54:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2409#issuecomment-17012",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:7'></a>
Since #3952 was merged this is fixed.

Cheers,

Michael



---

archive/issue_events_008431.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-26T21:54:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2409",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2409#event-8431"
}
```
